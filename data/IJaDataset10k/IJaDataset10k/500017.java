...
public class ApplyLegendTheme extends kidsService {

    /********************************************************************/
    public ApplyLegendTheme() {
    }

    public ApplyLegendTheme(Document doc, Element ele) throws IOException {
        load(doc, ele);
    }
    ...
    public boolean execute(kidsRequest request, kidsResponse response) throws kidsServiceException {
        ...
        FeatureLayer layer = (FeatureLayer) session.getMap().getSelectedFeatureLayer();
        int theme = Integer.parseInt(request.setting);
        layer.applyLegendTheme(theme);
        consumed = true;
        return consumed;
    }
