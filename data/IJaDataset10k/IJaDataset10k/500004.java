...
public abstract class AbstractImportProcess extends SvrProcess {
    ...
    protected int setImportError(String whereClause, String errorMsg) {
        StringBuffer sql = new StringBuffer(" UPDATE " + getImportTableName() + " SET I_IsImported = 'E', " + "     I_ErrorMsg = COALESCE(I_ErrorMsg,'') || " + getSQLErrorMsg(errorMsg) + " WHERE I_IsImported <> 'Y' AND ").append(getSecuritySQLCheck());
        if (whereClause != null && whereClause.length() > 0) sql.append(" AND ").append(whereClause);
    ...
    private ResultSet getRecordsToImport(String whereClause) throws SQLException {
        StringBuffer sql = new StringBuffer(" SELECT * FROM " + getImportTableName() + " WHERE I_IsImported <> 'Y' AND ").append(getSecuritySQLCheck());
        if (whereClause != null && whereClause.length() > 0) sql.append(" AND ").append(whereClause);
    ...
    protected int deleteOldImported() {
        StringBuffer sql = new StringBuffer(" DELETE FROM " + getImportTableName() + " WHERE I_IsImported='Y' AND ").append(getSecuritySQLCheck());
        return DB.executeUpdate(sql.toString());
    }
    ...
    private int updateErrorLines() {
        int qty;
        StringBuffer sql = new StringBuffer(" UPDATE " + getImportTableName() + " SET I_IsImported = 'N', Updated=SysDate " + " WHERE I_IsImported <> 'Y' AND ").append(getSecuritySQLCheck());
    ...
    private void calculateTotalLines() {
        StringBuffer sql = new StringBuffer(" SELECT COUNT(*) " + " FROM " + getImportTableName() + " WHERE I_IsImported <> 'Y' AND ").append(getSecuritySQLCheck());
    ...
    private int resetDefaultData() {
        StringBuffer sql = new StringBuffer(" UPDATE " + getImportTableName() + " SET AD_Client_ID = COALESCE (AD_Client_ID,").append(getAD_Client_ID()).append(")," + "     AD_Org_ID    = COALESCE (AD_Org_ID,").append(getAD_Org_ID()).append(")," + "     IsActive     = COALESCE (IsActive, 'Y')," + "     Created      = COALESCE (Created, SysDate)," + "     CreatedBy    = COALESCE (CreatedBy, 0)," + "     Updated      = COALESCE (Updated, SysDate)," + "     UpdatedBy    = COALESCE (UpdatedBy, 0)," + "     I_ErrorMsg   = NULL," + "     I_IsImported = 'N' " + " WHERE (I_IsImported <> 'Y' OR I_IsImported IS NULL) AND ").append(getSecuritySQLCheck());
        return DB.executeUpdate(sql.toString());
    }
    ...
    private String getResultMsg(String processMsg, boolean error) {
        ...
        if (processMsg != null && processMsg.length() > 0) msg.append(processMsg); else {
            msg.append(Msg.getMsg(getCtx(), "ImportResult", new Object[] { getTotalLines(), getImportedLines(), getErrorLines() }));
        }
        if (error) msg.append(" ERROR: ");
        return msg.toString();
    }
    ...
    protected String getErrorDescription() {
        ...
        if (np != null) {
            ...
            if (name.length() > 0 && value.length() > 0) msg = value + ": " + name; else if (name.length() > 0) msg = name; else if (value.length() > 0) msg = value;
        }
        return msg;
    }
    ...
    public String getImportTableName() {
        String name = "";
        if (getImportTable() != null) name = getImportTable().getTableName();
        return name;
    }
}
