...
public class Loop {
    ...
    public Triangle cut(boolean delaunay) {
        if (isSimplex()) {
            Triangle t = new Triangle(root.getGraphPoint().getPoint(), root.getNext().getGraphPoint().getPoint(), root.getNext().getNext().getGraphPoint().getPoint());
    ...
    private HEdge initFromPolyline(GraphOutline outline, VectorUtil.Winding reqWinding) {
        ...
        final VectorUtil.Winding hasWinding = VectorUtil.getWinding(vertices.get(0).getPoint(), vertices.get(1).getPoint(), vertices.get(2).getPoint());
        final boolean invert = hasWinding != reqWinding && reqWinding == VectorUtil.Winding.CW;
        final int max;
            ...
            for (int pos = 0; pos < vertices.size(); pos++) {
                ...
                if (distance < minDistance) {
                    for (GraphVertex vert : vertices) {
                        if (vert == v || vert == nextV || vert == cand) continue;
                        inValid = VectorUtil.inCircle(v.getPoint(), nextV.getPoint(), cand.getPoint(), vert.getPoint());
                        if (inValid) {
                            break;
                        }
                    }
    ...
    private HEdge findClosestValidNeighbor(HEdge edge, boolean delaunay) {
        ...
        if (!VectorUtil.ccw(root.getGraphPoint().getPoint(), next.getGraphPoint().getPoint(), edge.getGraphPoint().getPoint())) {
            return null;
        }
        HEdge candEdge = edge;
        boolean inValid = false;
        if (delaunay) {
            ...
            while (e != candEdge) {
                if (e.getGraphPoint() == root.getGraphPoint() || e.getGraphPoint() == next.getGraphPoint() || e.getGraphPoint().getPoint() == cand) {
                    e = e.getNext();
                    continue;
                }
                inValid = VectorUtil.inCircle(root.getGraphPoint().getPoint(), next.getGraphPoint().getPoint(), cand, e.getGraphPoint().getPoint());
                if (inValid) {
                    break;
                }
                e = e.getNext();
    ...
    public boolean checkInside(Vertex v) {
        ...
        do {
            ...
            if (((v1.getY() > v.getY()) != (v2.getY() > v.getY())) && (v.getX() < (v2.getX() - v1.getX()) * (v.getY() - v1.getY()) / (v2.getY() - v1.getY()) + v1.getX())) {
                inside = !inside;
            }
            current = next;
