...
public class ExtendedSite extends SiteWithTimestamp {
    ...
    public LiteFeature[] getLiteFeatures() {
        if (getCurrentConfiguredSite() != null) return filterFeatures(getNonFilteredLiteFeatures()); else return getNonFilteredLiteFeatures();
    }

    public void setLiteFeatures(LiteFeature[] liteFeatures) {
        if ((liteFeatures == null) || (liteFeatures.length == 0)) return;
    ...
    public LiteFeature getLiteFeature(VersionedIdentifier vid) {
        if (allLiteFeatures == null) return null;
        for (int i = 0; i < allLiteFeatures.length; i++) {
    ...
    private LiteFeature[] filterFeatures(LiteFeature[] allIncluded) {
        ...
        if (allIncluded != null) {
            for (int i = 0; i < allIncluded.length; i++) {
                ...
                if (UpdateManagerUtils.isValidEnvironment(included)) list.add(included); else {
