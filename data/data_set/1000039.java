...
class SednaStatementImpl implements SednaStatement {
    ...
    SednaStatementImpl(OutputStream outputStream, BufferedInputStream bufInputStream, boolean doTraceOutput) {
    ...
    public boolean execute(InputStream in, ResultType resultType) throws DriverException, IOException {
        ...
        int call_res = 1;
        while (call_res > 0) {
            call_res = in.read(msg.body, 6, NetOps.SEDNA_SOCKET_MSG_BUF_SIZE - 6);
            if (call_res > 0) {
        ...
        try {
            ...
            if (query_bytes.length > NetOps.SEDNA_SOCKET_MSG_BUF_SIZE - 6) {
                ...
                while (bytes_sent < query_bytes.length) {
                    ...
                    setQueryResultType(msg, resultType);
                    System.arraycopy(query_bytes, bytes_sent, msg.body, 6, bytes_to_send);
    ...
    private boolean executeResponseAnalyze(NetOps.Message msg) throws DriverException {
        ...
        if (msg.instruction == NetOps.se_QuerySucceeded) {
            NetOps.StringItem sitem = NetOps.readStringItem(bufInputStream, this.doTraceOutput);
            this.serializedResult = new SednaSerializedResultImpl(sitem.item, sitem.hasNextItem, this.bufInputStream, this.outputStream, this.doTraceOutput);
            return true;
        } else if (msg.instruction == NetOps.se_QueryFailed || msg.instruction == NetOps.se_UpdateFailed || msg.instruction == NetOps.se_ErrorResponse) {
            ...
            return NetOps.bulkLoad(new String(msg.body, 5, msg.length - 5), this.bufInputStream, this.outputStream);
            ...
            DriverException ex = new DriverException(ErrorCodes.SE3008, Integer.toString(msg.instruction));
            if (gotDebug) ex.setDebugInfo(debugInfo);
            throw ex;
        }
    }
    ...
    public void loadDocument(InputStream in, String doc_name) throws DriverException, IOException {
        ...
        } else if (msg.instruction == NetOps.se_ErrorResponse || msg.instruction == NetOps.se_QueryFailed) {
    ...
    public void loadDocument(String xmldoc, String doc_name) throws DriverException, IOException {
    ...
    public void loadDocument(String xmldoc, String doc_name, String col_name) throws DriverException, IOException {
