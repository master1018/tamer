...
public class ProcessHierarchyIOController implements NotationLocalIOController {
     ...
     *  {@inheritDoc}
     */
    public SaveResult saveProjectDiagram(final ProjectDiagram projectDiagram, final String parentLocation, final boolean makeDirs) {
        if (!isValidDiagramModel(projectDiagram)) {
            return SaveResult.ERROR;
        }
        final XMLEncoder encoder;
        final String filePath = parentLocation + System.getProperty("file.separator") + projectDiagram.getDisplayName() + ProjectService.FILE_EXTENSION_DELIMITER + extension;
        try {
    ...
    private void initEncoder(final XMLEncoder encoder) {
        ...
        encoder.setPersistenceDelegate(ProjectDiagram.class, new DefaultPersistenceDelegate(new String[] { DISPLAY_NAME_PROPERTY, NOTATION_IDENTIFIER_PROPERTY, UUID_PROPERTY, DIAGRAM_MODEL_PROPERTY }));
        encoder.setPersistenceDelegate(ProcessHierarchyDiagramModel.class, new DefaultPersistenceDelegate(new String[] { LAYOUT_CACHE_PROPERTY }));
        ...
        encoder.setPersistenceDelegate(GraphLayoutCache.class, new DefaultPersistenceDelegate(new String[] { MODEl_PROPERTY, FACTORY_PROPERTY, CELL_VIEWS_PROPERTY, HIDD_CELLS_PROPERTY, PARTIAL_PROPERTY }));
