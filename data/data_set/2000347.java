...
public class RemoteGassServer {
    ...
    public RemoteGassServer(GSSCredential cred, boolean secure, int port) {
        ...
        this.secure = secure;
        this.port = port;
        options = USER_EXPAND_ENABLE | TILDE_EXPAND_ENABLE | LINE_BUFFER_ENABLE | GassServer.READ_ENABLE | GassServer.WRITE_ENABLE;
    }
    ...
    public void start(String rmc) throws GassException {
        ...
        try {
            ...
            } else if (status == GramJob.STATUS_FAILED || status == GramJob.STATUS_DONE) {
                int errorCode = gassJobListener.getError();
                if (stderrListener.hasData()) {
                    error = stderrListener.getOutput();
                } else if (errorCode != 0) {
                    error = "Remote gass server stopped with error : " + errorCode;
                } else {
    ...
    public boolean shutdown() {
        if (url != null) {
            ...
            try {
                ...
                if (status == GramJob.STATUS_FAILED || status == GramJob.STATUS_DONE) {
                    reset();
                    return true;
                }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            s.setOptions(USER_EXPAND_ENABLE | TILDE_EXPAND_ENABLE | LINE_BUFFER_ENABLE | GassServer.READ_ENABLE | GassServer.WRITE_ENABLE);
            s.start(host);
...
class GassServerListener implements GramJobListener {
    ...
    public static boolean isStartState(int status) {
        return (status == GramJob.STATUS_ACTIVE || status == GramJob.STATUS_FAILED || status == GramJob.STATUS_DONE);
    }
