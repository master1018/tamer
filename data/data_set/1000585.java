...
public class TreeFilterTest {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (File file : sources) {
            ...
            if (parseResults.isValidInput()) {
                final boolean isCopybook = file.getName().toUpperCase().endsWith(".CPY");
                if (acceptedByMyAdaptiveTreeParser(parseResults.getTree(), isCopybook)) {
                    System.out.println("Adaptive match was successful as well.");
                } else {
                    System.out.println("Adaptive match, however, was NOT successful.");
                }
                System.out.println();
                count += 1;
            } else {
        ...
        if (!erroneous.isEmpty()) {
            System.out.println("Following files were found not to parse correctly:");
    ...
    private static boolean acceptedByMyAdaptiveTreeParser(CommonTree tree, boolean isCopybook) {
        Filter filter = null;
        if (isCopybook) {
        ...
        FilteredTreeNodeStream filteredStream = new FilteredTreeNodeStream(tree, filter);
