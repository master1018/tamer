...
public class Cache {
    ...
    public void put_colourmap(int cache_id, IndexColorModel map) throws RdesktopException {
        if (cache_id < colourcache.length) colourcache[cache_id] = map; else throw new RdesktopException("Could not put colourmap with cache_id=" + cache_id);
    }
    ...
    public void saveState() {
        int id, idx;
        for (id = 0; id < bitmapcache.length; id++) if (PstCache.IS_PERSISTENT(id)) for (idx = 0; idx < bitmapcache[id].length; idx++) PstCache.touchBitmap(id, idx, bitmapcache[id][idx].usage);
    }
    ...
    public void putDesktop(int offset, int cx, int cy, int[] data) throws RdesktopException {
        int length = cx * cy;
        int pdata = 0;
        if (offset > highdeskcache.length) offset = 0;
