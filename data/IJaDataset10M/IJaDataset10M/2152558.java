        ...
        if (node.getNumAxes() == 1) {
            switch(printMode) {
                ...
                case UNABBREVIATE:
                    ...
                    break;
                default:
                    throw new IllegalStateException("Unknown PrintModeEnum: " + printMode);
            }
        } else {
            switch(printMode) {
                case ABBREVIATE:
                ...
                case UNABBREVIATE:
                    ...
                    if (testNode instanceof ASTAttributeTest || testNode instanceof ASTSchemaAttributeTest) {
            ...
            case ABBREVIATE:
                switch(axis) {
                    ...
                    case ATTRIBUTE:
                        ...
                        if (sibling instanceof ASTNodeTest && (sibling.jjtGetChild(0) instanceof ASTAttributeTest || sibling.jjtGetChild(0) instanceof ASTSchemaAttributeTest)) {
                        } else {
                            print("@");
                        }
                        break;
        ...
        if (node instanceof ASTParenthesizedExpr) {
            ...
            if (treatExpr != null) {
                ...
                if (n != null) {
                    ...
                    if (functionCall != null && "fn:root".equals(functionCall.getImage()) && functionCall.jjtGetNumChildren() == 1) {
                        n = findOnlyOne(n, ASTStepExpr.class);
                        ASTForwardAxis forwardAxis = (ASTForwardAxis) findOnly(n, ASTForwardAxis.class, 2);
                        if (forwardAxis != null && AxisEnum.SELF == forwardAxis.getAxis(0)) {
    ...
    public Object visit(ASTPathExpr node, Object data) {
        for (int i = 0; i < node.jjtGetNumChildren(); i++) {
            ...
            if (i > 0) {
                ...
                if (printMode == PrintModeEnum.UNABBREVIATE || (!(priorChild instanceof ASTSlash || priorChild instanceof ASTSlashSlash || isRoot(priorChild)) && !(child instanceof ASTSlash || child instanceof ASTSlashSlash || isRoot(child)))) {
                    print("/");
                }
            }
            if (i == 0 && printMode == PrintModeEnum.UNABBREVIATE && child instanceof ASTSlashSlash) {
