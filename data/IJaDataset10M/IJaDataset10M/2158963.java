...
public class ServiceRootSbbPanel extends Composite implements ISelectionChangedListener, ModifyListener {
    ...
    private static final int[] EDITORS = new int[] { EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE };
    ...
    public String getStatus() {
        if (sbbViewer.getTable().getItemCount() == 0) return "There are no valid root SBBs available.  Exit this wizard and verify that any root SBBs have been compiled and that they contain initial events.";
        int index = sbbViewer.getTable().getSelectionIndex();
        if (index == -1) return "Please select a root SBB.";
        try {
        ...
        if (priority > 128 || priority < -127) return "Default priority must be an integer between -127 and 128 inclusive.";
        return null;
    }
    ...
    public void setRootSbb(SbbXML sbb) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (sbb.getName().equals(name) && sbb.getVendor().equals(vendor) && sbb.getVersion().equals(version)) {
    ...
    public void updateWidgets() {
        ...
        if (index == -1) {
            ...
            try {
                ...
                if (sbbXML.getAddressProfileSpecAliasRef() != null) addressProfileEnabled = true; else addressProfileEnabled = false;
