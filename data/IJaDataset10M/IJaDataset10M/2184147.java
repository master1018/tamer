...
public class MysqlDatabaseLoader implements IDatabaseLoader {
    ...
    public MemoryDataSet loadBoundingBox(final double minLat, final double minLon, final double maxLat, final double maxLon) {
        MysqlReader reader;
        if (minLat != Double.MIN_VALUE || maxLat != Double.MAX_VALUE || minLon != Double.MIN_VALUE || maxLon != Double.MAX_VALUE) {
    ...
    public ReleasableIterator<Way> loadAllWaysInBoundingBox(final double minLat, final double minLon, final double maxLat, final double maxLon) {
        ...
        if (minLat != Double.MIN_VALUE || maxLat != Double.MAX_VALUE || minLon != Double.MIN_VALUE || maxLon != Double.MAX_VALUE) {
            reader = new ConstrainedCurrentWayReader(myCredentials, minLat, minLon, maxLat, maxLon);
        } else {
     ...
     * @see org.openstreetmap.osm.io.IDatabaseLoader#loadNodesByName(java.lang.String)
     */
    public ReleasableIterator<Node> loadNodesByName(final String aLookFor, final double minLat, final double minLon, final double maxLat, final double maxLon) {
        Map<String, String> tags = new HashMap<String, String>();
        tags.put(Tags.TAG_NAME, aLookFor);
        tags.put(Tags.TAG_NAME + ":%", aLookFor);
        tags.put(Tags.TAG_REF, aLookFor);
        tags.put(Tags.TAG_REF + ":%", aLookFor);
        if (myNodeLoader != null && myNodeLoader.isReleased()) {
            myNodeLoader.reInitialize(tags, ConstrainedCurrentNodeReader.CONDITION.OR, ConstrainedCurrentNodeReader.COMPARESTYLE.LIKE, minLat, minLon, maxLat, maxLon);
        } else {
            myNodeLoader = new ConstrainedCurrentNodeReader(myCredentials, tags, ConstrainedCurrentNodeReader.CONDITION.OR, ConstrainedCurrentNodeReader.COMPARESTYLE.LIKE, minLat, minLon, maxLat, maxLon);
        }
        return myNodeLoader;
    }
    ...
    public ReleasableIterator<Way> loadWaysByName(final String aLookFor, final double minLat, final double minLon, final double maxLat, final double maxLon) {
        ...
        tags.put(Tags.TAG_REF, aLookFor);
        tags.put(Tags.TAG_REF + ":%", aLookFor);
        return new ConstrainedCurrentWayReader(myCredentials, tags, ConstrainedCurrentNodeReader.CONDITION.OR, ConstrainedCurrentNodeReader.COMPARESTYLE.LIKE, minLat, minLon, maxLat, maxLon);
    }
    ...
    public void createNode(final Node aW) {
        ...
        databaseContext.executeStatement("INSERT INTO current_nodes (id, latitude, longitude, user_id, visible, tags, timestamp, tile) " + "VALUES (" + aW.getId() + ", " + FixedPrecisionCoordinateConvertor.convertToFixed(aW.getLatitude()) + ", " + FixedPrecisionCoordinateConvertor.convertToFixed(aW.getLongitude()) + ", " + "1, true, " + "'" + tagger.format(oldTags).replace("'", "\\'") + "', now(), " + (new TileCalculator()).calculateTile(aW.getLatitude(), aW.getLongitude()) + ")");
    }
    ...
    public void createWay(final Way aNode) {
        DatabaseContext databaseContext = getMydatabaseContext();
        databaseContext.executeStatement("INSERT INTO current_ways (id, user_id, visible, timestamp) " + "VALUES (" + aNode.getId() + ", " + "1, " + "now())");
        int seq = 1;
        for (WayNode node : aNode.getWayNodes()) {
            databaseContext.executeStatement("INSERT INTO current_way_nodes (id, node_id, sequence_id) " + "VALUES (" + aNode.getId() + ", " + node.getNodeId() + ", " + seq + ")");
            seq++;
        }
        for (Tag tag : aNode.getTags()) {
            databaseContext.executeStatement("INSERT INTO current_way_tags (id, k, v) " + "VALUES (" + aNode.getId() + ", '" + tag.getKey().replace("'", "\\'") + "', '" + tag.getValue().replace("'", "\\'") + "')");
        }
    }
    ...
    public void createRelation(final Relation aRelation) {
        DatabaseContext databaseContext = getMydatabaseContext();
        databaseContext.executeStatement("INSERT INTO current_relations (id, user_id, visible, timestamp) " + "VALUES (" + aRelation.getId() + ", " + "1, " + "now())");
        int seq = 1;
        for (RelationMember member : aRelation.getMembers()) {
            databaseContext.executeStatement("INSERT INTO current_relation_members (id, member_type, member_id, member_role) " + "VALUES (" + aRelation.getId() + ", '" + member.getMemberType().toString() + "', " + member.getMemberId() + ", '" + member.getMemberRole().replace("'", "\\'") + "')");
            seq++;
        }
        for (Tag tag : aRelation.getTags()) {
            databaseContext.executeStatement("INSERT INTO current_relation_tags (id, k, v) " + "VALUES (" + aRelation.getId() + ", '" + tag.getKey().replace("'", "\\'") + "', '" + tag.getValue().replace("'", "\\'") + "')");
            seq++;
        }
    }
    ...
    protected DatabaseContext getMydatabaseContext() {
        if (mydatabaseContext == null) mydatabaseContext = new DatabaseContext(myCredentials);
