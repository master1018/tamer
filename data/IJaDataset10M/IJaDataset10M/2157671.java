...
public class ClockMonPanel extends jmri.jmrix.nce.swing.NcePanel implements NcePanelInterface, NceListener {
    ...
    public void reply(NceReply r) {
        if (log.isDebugEnabled() && false) {
            log.debug("nceReplyCatcher() waiting: " + waiting + " watingForRead: " + waitingForCmdRead + " waitingForCmdTime: " + waitingForCmdTime + " waitingForCmd1224: " + waitingForCmd1224 + " waitingForCmdRatio: " + waitingForCmdRatio + " waitingForCmdStop: " + waitingForCmdStop + " waitingForCmdStart: " + waitingForCmdStart);
        }
        if (waiting <= 0) {
    ...
    private void alarmSetup() {
        ...
        timerDisplayUpdate.setRepeats(true);
    ...
    private void alarmSyncInit() {
        ...
        if (alarmSyncUpdate == null) {
            alarmSyncUpdate = new javax.swing.Timer(delay, new java.awt.event.ActionListener() {
    ...
    private double getNceTime() {
        ...
        if (lastClockReadPacket != null) {
            nceTime = (lastClockReadPacket.getElement(CS_CLOCK_HOURS) * 3600) + (lastClockReadPacket.getElement(CS_CLOCK_MINUTES) * 60) + lastClockReadPacket.getElement(CS_CLOCK_SECONDS) + (lastClockReadPacket.getElement(CS_CLOCK_TICK) * 0.25);
        }
        return (nceTime);
    }
        ...
        do {
            ...
            switch(internalSyncRunStateCounter) {
                ...
                case 3:
                    ...
                    if (log.isDebugEnabled() && false) {
                        log.debug("syncStates2 begin. NCE: " + (nceLastHour / 10) + (nceLastHour - ((nceLastHour / 10) * 10)) + rb.getString("LabelTimeSep") + (nceLastMinute / 10) + (nceLastMinute - ((nceLastMinute / 10) * 10)) + rb.getString("LabelTimeSep") + (nceLastSecond / 10) + (nceLastSecond - ((nceLastSecond / 10) * 10)) + " Internal: " + (now.getHours() / 10) + (now.getHours() - ((now.getHours() / 10) * 10)) + rb.getString("LabelTimeSep") + (now.getMinutes() / 10) + (now.getMinutes() - ((now.getMinutes() / 10) * 10)) + rb.getString("LabelTimeSep") + (now.getSeconds() / 10) + (now.getSeconds() - ((now.getSeconds() / 10) * 10)) + " diff: " + diffTime);
                    }
                    ...
                    issueClockSet(now.getHours(), now.getMinutes(), (int) syncInterval);
                    internalSyncRunStateCounter++;
                    break;
                case 4:
                    if (!waitingForCmdTime) {
                ...
                case 7:
                    ...
                    recomputeOffset();
                    if (log.isDebugEnabled() && false) {
                        log.debug("syncState compute offset. NCE: " + (nceLastHour / 10) + (nceLastHour - ((nceLastHour / 10) * 10)) + rb.getString("LabelTimeSep") + (nceLastMinute / 10) + (nceLastMinute - ((nceLastMinute / 10) * 10)) + rb.getString("LabelTimeSep") + (nceLastSecond / 10) + (nceLastSecond - ((nceLastSecond / 10) * 10)) + " Internal: " + (now.getHours() / 10) + (now.getHours() - ((now.getHours() / 10) * 10)) + rb.getString("LabelTimeSep") + (now.getMinutes() / 10) + (now.getMinutes() - ((now.getMinutes() / 10) * 10)) + rb.getString("LabelTimeSep") + (now.getSeconds() / 10) + (now.getSeconds() - ((now.getSeconds() / 10) * 10)));
                    }
                    internalSyncRunStateCounter = 0;
                    break;
                default:
    ...
    private void recomputeInternalSync() {
        ...
        if (log.isDebugEnabled() && false) {
            ...
            log.debug("syncInterval: " + syncInterval + " pCorr: " + fiveDigits.format(pCorr) + " iCorr: " + fiveDigits.format(iCorr) + " dCorr: " + fiveDigits.format(dCorr));
        }
    }
    ...
    private void recomputeNceSync() {
        ...
        if (log.isDebugEnabled() && false) {
            ...
            log.debug("currError: " + fiveDigits.format(currError) + " pCorr: " + fiveDigits.format(pCorr) + " iCorr: " + fiveDigits.format(iCorr) + " dCorr: " + fiveDigits.format(dCorr) + " newInternalRate: " + threeDigits.format(newInternalRate));
        }
    }
    ...
    private void updateNceClockDisplay() {
        ...
        txt = txt + " " + (nceLastHour / 10) + (nceLastHour - ((nceLastHour / 10) * 10)) + rb.getString("LabelTimeSep") + (nceLastMinute / 10) + (nceLastMinute - ((nceLastMinute / 10) * 10)) + rb.getString("LabelTimeSep") + (nceLastSecond / 10) + (nceLastSecond - ((nceLastSecond / 10) * 10));
        if (!nceLast1224) {
            if (nceLastAmPm) {
        ...
        txt = txt + " " + rb.getString("LabelRatio") + " " + nceLastRatio + rb.getString("LabelToOne");
        if (clockMode == SYNCMODE_NCE_MASTER) {
    ...
    private void updateInternalClockDisplay() {
        ...
        txt = txt + " " + (now.getHours() / 10) + (now.getHours() - ((now.getHours() / 10) * 10)) + rb.getString("LabelTimeSep") + (now.getMinutes() / 10) + (now.getMinutes() - ((now.getMinutes() / 10) * 10)) + rb.getString("LabelTimeSep") + (now.getSeconds() / 10) + (now.getSeconds() - ((now.getSeconds() / 10) * 10));
        txt = txt + " " + rb.getString("LabelRatio") + " " + threeDigits.format(internalClock.getRate()) + rb.getString("LabelToOne");
        if (clockMode == SYNCMODE_INTERNAL_MASTER) {
