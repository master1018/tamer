...
public class LaunchInOut extends SvrProcess {
    ...
    private String createReport() throws Exception {
        ...
        if (!Util.isEmpty(inout.getC_Invoice_ID(), true)) {
            invoice = new MInvoice(getCtx(), inout.getC_Invoice_ID(), get_TrxName());
        }
        initializeAmts(order, invoice);
        BigDecimal totalNetLineDiscounts = linesTotalBonusNetAmt.add(linesTotalDocumentDiscountsNetAmt).add(linesTotalLineDiscountsNetAmt);
        BigDecimal totalLineDiscounts = linesTotalBonusAmt.add(linesTotalDocumentDiscountsAmt).add(linesTotalLineDiscountsAmt);
        ...
        MClient client = JasperReportsUtil.getClient(getCtx(), inout.getAD_Client_ID());
        MClientInfo clientInfo = client.getInfo();
        jasperwrapper.addParameter("TIPOCOMPROBANTE", JasperReportsUtil.getDocTypeName(getCtx(), inout.getC_DocType_ID(), "REMITO", get_TrxName()));
        ...
        jasperwrapper.addParameter("DIRECCION", JasperReportsUtil.coalesce(location.getAddress1(), "") + ". " + ". (" + JasperReportsUtil.coalesce(location.getPostal(), "") + "). " + (region == null ? "" : JasperReportsUtil.coalesce(region.getName(), "")));
        ...
        if (!Util.isEmpty(bpartner.getC_Categoria_Iva_ID(), true)) {
            jasperwrapper.addParameter("TIPO_IVA", JasperReportsUtil.getCategoriaIVAName(getCtx(), bpartner.getC_Categoria_Iva_ID(), get_TrxName()));
        }
        ...
        if (!Util.isEmpty(inout.getSalesRep_ID(), true)) {
            jasperwrapper.addParameter("VENDEDOR", JasperReportsUtil.getUserName(getCtx(), inout.getSalesRep_ID(), get_TrxName()));
        }
        jasperwrapper.addParameter("NRODOCORIG", JasperReportsUtil.coalesce(inout.getPOReference(), ""));
        jasperwrapper.addParameter("NRO_OC", order == null ? "" : JasperReportsUtil.coalesce(order.getDocumentNo(), ""));
        if (invoice != null) {
            String fechaVto = (String) JasperReportsUtil.coalesce(getFechaVto(invoice), invoice.getDateInvoiced().toString());
            fechaVto = fechaVto.substring(0, 11);
            jasperwrapper.addParameter("VCTO", "Vencimiento: " + fechaVto);
        } else if (order != null) {
            jasperwrapper.addParameter("VCTO", "Vencimiento: " + inout.getDateOrdered() == null ? "" : (inout.getDateOrdered().toString().substring(0, 11)));
        }
        ...
        jasperwrapper.addParameter("IVA_1", getTaxAmt(order, invoice, new BigDecimal(10.5)));
        jasperwrapper.addParameter("IVA_2", getTaxAmt(order, invoice, new BigDecimal(21)));
        ...
        jasperwrapper.addParameter("CODVTA", order == null ? "" : JasperReportsUtil.getListName(getCtx(), MInvoice.PAYMENTRULE_AD_Reference_ID, order.getPaymentRule()));
        if (invoice != null) {
            jasperwrapper.addParameter("NROCOMPROBANTE", invoice.getDocumentNo());
            jasperwrapper.addParameter("INVOICE", JasperReportsUtil.getPODisplayByIdentifiers(getCtx(), invoice, X_C_Invoice.Table_ID, get_TrxName()));
        }
        if (!Util.isEmpty(inout.getM_Shipper_ID(), true)) {
            jasperwrapper.addParameter("TRANSPORTISTA", JasperReportsUtil.getShipperData(getCtx(), inout.getM_Shipper_ID(), get_TrxName()));
        }
        if (!Util.isEmpty(inout.getC_Currency_ID(), true)) {
            jasperwrapper.addParameter("MONEDA", JasperReportsUtil.getCurrencyDescription(getCtx(), inout.getC_Currency_ID(), get_TrxName()));
            jasperwrapper.addParameter("CURRENCY_SYMBOL", JasperReportsUtil.getCurrencySymbol(getCtx(), inout.getC_Currency_ID(), get_TrxName()));
        }
        ...
        jasperwrapper.addParameter("CLIENT_CATEGORIA_IVA", JasperReportsUtil.getCategoriaIVAName(getCtx(), clientInfo.getC_Categoria_Iva_ID(), get_TrxName()));
        jasperwrapper.addParameter("ORG", JasperReportsUtil.getOrgName(getCtx(), inout.getAD_Org_ID()));
        jasperwrapper.addParameter("ORG_LOCATION_DESCRIPTION", JasperReportsUtil.getLocalizacion(getCtx(), inout.getAD_Client_ID(), inout.getAD_Org_ID(), get_TrxName()));
        if (!Util.isEmpty(inout.getAD_OrgTrx_ID(), true)) {
            jasperwrapper.addParameter("ORGTRX_NAME", JasperReportsUtil.getOrgName(getCtx(), inout.getAD_OrgTrx_ID()));
        }
        if (!Util.isEmpty(inout.getAD_User_ID(), true)) {
            jasperwrapper.addParameter("USER", JasperReportsUtil.getUserName(getCtx(), inout.getAD_User_ID(), get_TrxName()));
        }
        jasperwrapper.addParameter("APPROVAL_AMT", inout.getApprovalAmt());
        if (!Util.isEmpty(inout.getC_Activity_ID(), true)) {
            jasperwrapper.addParameter("ACTIVITY", JasperReportsUtil.getActivityName(getCtx(), inout.getC_Activity_ID(), get_TrxName()));
        }
        if (!Util.isEmpty(inout.getC_Campaign_ID(), true)) {
            jasperwrapper.addParameter("CAMPAIGN", JasperReportsUtil.getCampaignName(getCtx(), inout.getC_Campaign_ID(), get_TrxName()));
        }
        if (!Util.isEmpty(inout.getC_Project_ID(), true)) {
            jasperwrapper.addParameter("PROJECT", JasperReportsUtil.getProjectName(getCtx(), inout.getC_Project_ID(), get_TrxName()));
        }
        if (!Util.isEmpty(inout.getC_Charge_ID(), true)) {
            jasperwrapper.addParameter("CHARGE", JasperReportsUtil.getChargeName(getCtx(), inout.getC_Charge_ID(), get_TrxName()));
        }
        if (!Util.isEmpty(inout.getC_DocType_ID(), true)) {
            jasperwrapper.addParameter("DOCTYPE", JasperReportsUtil.getDocTypeName(getCtx(), inout.getC_DocType_ID(), "REMITO", get_TrxName()));
        }
        ...
        jasperwrapper.addParameter("DELIVERY_RULE", JasperReportsUtil.getListName(getCtx(), X_M_InOut.DELIVERYRULE_AD_Reference_ID, inout.getDeliveryRule()));
        jasperwrapper.addParameter("DELIVERY_VIA_RULE", JasperReportsUtil.getListName(getCtx(), X_M_InOut.DELIVERYVIARULE_AD_Reference_ID, inout.getDeliveryViaRule()));
        jasperwrapper.addParameter("INOUT_DESCRIPTION", inout.getDescription());
        if (!Util.isEmpty(inout.getDoc_User_ID(), true)) {
            jasperwrapper.addParameter("DOC_USER", JasperReportsUtil.getUserName(getCtx(), inout.getDoc_User_ID(), get_TrxName()));
        }
        jasperwrapper.addParameter("DOC_STATUS", JasperReportsUtil.getListName(getCtx(), X_M_InOut.DOCSTATUS_AD_Reference_ID, inout.getDocStatus()));
        jasperwrapper.addParameter("DOC_STATUS_VALUE", inout.getDocStatus());
        jasperwrapper.addParameter("FREIGHT_AMT", inout.getFreightAmt());
        jasperwrapper.addParameter("FREIGHT_COST_RULE", JasperReportsUtil.getListName(getCtx(), X_M_InOut.FREIGHTCOSTRULE_AD_Reference_ID, inout.getFreightCostRule()));
        jasperwrapper.addParameter("INOUT_DATE", inout.getInOutDate());
        jasperwrapper.addParameter("INOUT_RECEPTION_DATE", inout.getInOutReceptionDate());
        if (!Util.isEmpty(inout.getM_Warehouse_ID(), true)) {
            jasperwrapper.addParameter("WAREHOUSE", JasperReportsUtil.getWarehouseName(getCtx(), inout.getM_Warehouse_ID(), get_TrxName()));
        }
        jasperwrapper.addParameter("MOVEMENT_TYPE", JasperReportsUtil.getListName(getCtx(), X_M_InOut.MOVEMENTTYPE_AD_Reference_ID, inout.getMovementType()));
        jasperwrapper.addParameter("PICK_DATE", inout.getPickDate());
        jasperwrapper.addParameter("PRIORITY_RULE", JasperReportsUtil.getListName(getCtx(), X_M_InOut.PRIORITYRULE_AD_Reference_ID, inout.getPriorityRule()));
        jasperwrapper.addParameter("RECEPTION_DATE", inout.getReceptionDate());
        if (!Util.isEmpty(inout.getRef_InOut_ID(), true)) {
            MInOut refInOut = new MInOut(getCtx(), inout.getRef_InOut_ID(), get_TrxName());
            jasperwrapper.addParameter("REF_INOUT", JasperReportsUtil.getPODisplayByIdentifiers(getCtx(), refInOut, X_M_InOut.Table_ID, get_TrxName()));
        }
        ...
        if (!Util.isEmpty(inout.getUser1_ID(), true)) {
            jasperwrapper.addParameter("USER1", JasperReportsUtil.getUserName(getCtx(), inout.getUser1_ID(), get_TrxName()));
        }
        if (!Util.isEmpty(inout.getUser2_ID(), true)) {
            jasperwrapper.addParameter("USER2", JasperReportsUtil.getUserName(getCtx(), inout.getUser2_ID(), get_TrxName()));
        }
