...
public class FilterUtil {
    ...
    protected static final Logger logger = Logger.getLogger(FilterUtil.class);

    private FilterUtil() {
    }

    public static boolean isTypeOnlyLinkFilter(LinkFilter filter) {
        if (filter.getAspect() != LinkFilter.TYPE) return false;
        ObjectFilter of = filter.getFilter();
        return of.getCriterion() instanceof IDSearchCriterion && of.getAspect() instanceof SelfSearchAspect && of.getComparison() instanceof EqualsComparison;
    }
    ...
    public static boolean filtersOn(Filter f, OBOProperty prop) {
        ...
        if (f instanceof LinkFilter) {
            ...
            for (Filter<?> filter : comp.getFilters()) {
                if (filtersOn(filter, prop)) return true;
            }
            return false;
        }
        return false;
    ...
    public static Collection filterCriteriaByInput(Collection allCriteria, Class c) {
        Collection out = new ArrayList();
        Iterator it = allCriteria.iterator();
        while (it.hasNext()) {
            SearchCriterion sc = (SearchCriterion) it.next();
            if (c.isAssignableFrom(sc.getInputType())) out.add(sc);
        }
        return out;
    }
    ...
    public static <T> Filter<T> mergeFilters(Collection<Filter<T>> filters) {
        if (filters.size() == 1) return filters.iterator().next();
        CompoundFilter out = new CompoundFilterImpl(CompoundFilter.AND);
        for (Filter<T> filter : filters) out.addFilter(filter);
        return out;
    }

    public static Collection filterComparisonByInput(Collection allComparisons, Class c) {
        Collection out = new ArrayList();
        ...
        while (it.hasNext()) {
            ...
            if (matches) out.add(sc);
        }
        return out;
    }

    public static String getOBOFilterExpression(Filter filter) {
        if (filter instanceof CompoundFilter) return getOBOFilterExpression((CompoundFilter) filter); else if (filter instanceof ObjectFilter) return getOBOFilterExpression((ObjectFilter) filter); else if (filter instanceof LinkFilter) return getOBOFilterExpression((LinkFilter) filter); else throw new IllegalArgumentException("Cannot create obo filter from filter of type " + filter.getClass());
    }
    ...
    public static String getOBOFilterExpression(ObjectFilter filter) {
        ...
        if (!(filter.getAspect() instanceof SelfSearchAspect)) {
            ...
            if (filter.getTraversalFilter() != null) {
                ...
                } else out.append(getOBOFilterExpression(filter.getTraversalFilter()));
                out.append(")");
            }
            out.append(" ");
        }
        out.append(filter.getCriterion().getID());
        if (filter.getNegate()) out.append(" NOT");
    ...
    public static String getOBOFilterExpression(CompoundFilter filter) {
        ...
        for (Filter<?> f : filter.getFilters()) {
            if (!first) {
                out.append(" ");
                if (filter.getBooleanOperation() == CompoundFilter.AND) out.append("and"); else if (filter.getBooleanOperation() == CompoundFilter.OR) out.append("or"); else out.append("???");
                out.append(" ");
            } else first = false;
            if (f instanceof CompoundFilter) out.append("(");
    ...
    public static Filter loadFilter(String path) throws IOException {
        XMLDecoder d = new XMLDecoder(new BufferedInputStream(IOUtil.getStream(path)));
