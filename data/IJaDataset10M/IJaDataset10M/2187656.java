...
public class Sphere extends Primitive {
   ...
   */
    public Sphere(float radius, int primflags, int divisions, Appearance ap) {
        ...
        GeomBuffer cache = getCachedGeometry(Primitive.SPHERE, radius, 0.0f, 0.0f, divisions, 0, primflags);
        Shape3D shape;
        if (cache != null) {
            ...
            if ((primflags & Primitive.GEOMETRY_NOT_SHARED) == 0) {
                cacheGeometry(Primitive.SPHERE, radius, 0.0f, 0.0f, divisions, 0, primflags, gbuf);
            }
        }
            ...
            if (leftToRight) {
                ...
                for (j = 0; j < i; j++) {
                    ...
                    if (i > 1) {
                        ...
                        if (sign > 0) {
                            gbuf.texCoord2d(0.75 - (startDelta + endDelta) / (4 * Math.PI), 1.0 - (theta - dt) / Math.PI);
                        } else {
                            gbuf.texCoord2d(0.75 - (startDelta + endDelta) / (4 * Math.PI), (theta - dt) / Math.PI);
                        }
