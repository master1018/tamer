...
public class ProductAndCDKeys extends JComponent {
    ...
    public void updateProducts() {
        ...
        for (ProductIDs prod : KeyManager.getProducts()) {
            ...
            if (prod == cs.product) cmbProduct.setSelectedItem(prod);
        }
        updateKeys();
    }
    ...
    private void setOriginalKeys() {
        ...
        if (prod == KeyManager.PRODUCT_ALLNORMAL) throw new RuntimeException("Unknown product " + getProduct());
        for (CDKey key : KeyManager.getKeys(prod)) {
            cmbCDKey.addItem(key);
            if (key.getKey().equalsIgnoreCase(cs.cdkey)) cmbCDKey.setSelectedItem(key);
        }
    }
    ...
    private void setExpansionKeys() {
        ...
        for (CDKey key : KeyManager.getKeys(prod)) {
            ...
            if (key.getKey().equalsIgnoreCase(cs.cdkey2)) cmbCDKey2.setSelectedItem(key);
        }
    }
    ...
    public CDKey getCDKey() {
        if (!cmbCDKey.isVisible()) return null;
        return (CDKey) cmbCDKey.getSelectedItem();
    }

    public CDKey getCDKey2() {
        if (!cmbCDKey2.isVisible()) return null;
