...
public class StatsWindow extends sWindow {
    ...
    public static final int EOG = 0, EOR = 1, IG = 2, TT = 3;
    ...
    public StatsWindow(int type, Vector players, ScorchApplet owner) {
        ...
        switch(type) {
            case IG:
            case EOR:
                name = "Players Statistics for Round #" + owner.getRoundCount() + " out of " + owner.getMaxRounds();
                break;
            case TT:
    ...
    public boolean handleEvent(Event evt) {
        if (evt.id == Event.ACTION_EVENT) {
            if (evt.arg.equals("OK")) {
                close();
                if (type == EOG) ((ScorchApplet) owner).Quit();
                return true;
            }
    ...
    private Panel buildPlayersTable(Vector players) {
        ...
        PlayerProfile spp;
        ScorchPlayer sp;
        Panel pp, rPanel = new Panel(new GridLayout(Math.max(players.size(), ScorchPlayer.MAX_PLAYERS), 1, 0, 0));
        int[] index;
        long[] cash;
        ...
        for (int i = 0; i < players.size(); i++) {
            sp = null;
            if (type == TT) spp = (PlayerProfile) players.elementAt(index[i]); else {
