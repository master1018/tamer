        ...
        try {
            ...
            while ((l = inputStream.readLine()) != null) {
                if (!l.startsWith("#")) {
                    ...
                    if ("intra".equals(t[2])) t1.setTranslocationType(Variant.TRANSLOCATION_INTRACHR); else if ("intra".equals(t[2])) t1.setTranslocationType(Variant.TRANSLOCATION_INTERCHR); else t1.setTranslocationType(Variant.TRANSLOCATION_UNKNOWN);
                    ...
                    if ("intra".equals(t[2])) t2.setTranslocationType(Variant.TRANSLOCATION_INTRACHR); else if ("intra".equals(t[2])) t2.setTranslocationType(Variant.TRANSLOCATION_INTERCHR); else t2.setTranslocationType(Variant.TRANSLOCATION_UNKNOWN);
                    t2.setType(Variant.TRANSLOCATION);
                    t2.setZygosity(Variant.UNKNOWN_ZYGOSITY);
                    store.putMismatch(t1);
                    store.putMismatch(t2);
                    System.out.println(workerName + ": adding translocation to db: " + t1.getContig() + ":" + t1.getStartPosition() + "-" + t1.getStopPosition() + " " + t2.getContig() + ":" + t2.getStartPosition() + "-" + t2.getStopPosition() + " total lines processed so far: " + count);
                }
            }
            inputStream.close();
            System.out.print("\n");
