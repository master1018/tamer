...
public class SecureInputOutputTest extends BaseTestCase {
    ...
    public void testSecureInputOutput() throws IOException {
        ...
        Class[] types = new Class[] { Boolean.class, Byte.class, Short.class, Integer.class, Float.class, Long.class, Double.class, String.class };
        String[] str = new String[] { "Hello World", "LimeWire", "Mojito", "Gnutella Network" };
        ...
        assertEquals(sos.getMessageDigest().getAlgorithm(), sis.getMessageDigest().getAlgorithm());
        for (int i = 0, j = 0, k = 0; i < 4096; i++) {
            Class clazz = types[i % types.length];
            if (clazz.equals(Boolean.class)) {
    ...
    public void testSecureInputOutputHeader() throws IOException {
        String algorithm = "This is a very long MessageDigest algorithm name! " + "In fact its length must be at least 256 bytes to make sure " + "SecureOutputStream's length field in the header is utilitzing " + "more than one byte for the length. So what I am doing here is " + "to define an extremly long algorithm name for the fake MessageDigest " + "implementation. The goal is to make sure the for-loop that re-assembles " + "the length field works correctly";
