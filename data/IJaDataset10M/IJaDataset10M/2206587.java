...
public class OrgMinerOptions extends JPanel {
    ...
    public static final int DEFAULT_MINING = 0;

    public static final int AHC_MINING = 1000;

    public static final int WORKING_TOGETHER = 2000;

    public static final int SIMILAR_TASK = 3000;

    public static final int SOM_MINING = 4000;
    ...
    public static final int EUCLIDIAN_DISTANCE = 0;

    public static final int CORRELATION_COEFFICIENT = 1;

    public static final int SIMILARITY_COEFFICIENT = 2;

    public static final int HAMMING_DISTANCE = 3;

    public static final int SIMULTANEOUS_APPEARANCE_RATIO = 0;

    public static final int DISTANCE_WITHOUT_CAUSALITY = 1;

    public static final int DISTANCE_WITH_CAUSALITY = 2;
    ...
    private void init() {
        ...
        this.add(content1, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.add(content2, new GridBagConstraints(0, 4, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.add(content3, new GridBagConstraints(0, 10, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.add(content4, new GridBagConstraints(0, 16, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.add(content5, new GridBagConstraints(0, 22, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.add(content6, new GridBagConstraints(0, 28, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    }

    public int getSimilarTaskSetting() {
        String st = doingSimilarTask.getValue().toString();
        if (st.equals("Euclidian distance")) return OrgMinerOptions.EUCLIDIAN_DISTANCE; else if (st.equals("Correlation coefficient")) return OrgMinerOptions.CORRELATION_COEFFICIENT; else if (st.equals("Similarity coefficient")) return OrgMinerOptions.SIMILARITY_COEFFICIENT; else return OrgMinerOptions.HAMMING_DISTANCE;
    }

    public int getWorkingTogetherSetting() {
        String st = workingTogether.getValue().toString();
        if (st.equals("Simultaneous appearance ratio")) return OrgMinerOptions.SIMULTANEOUS_APPEARANCE_RATIO; else if (st.equals("Consider distance with causality")) return OrgMinerOptions.DISTANCE_WITH_CAUSALITY; else return OrgMinerOptions.DISTANCE_WITHOUT_CAUSALITY;
    }
    ...
    public int getSelectedMetrics() {
        int nIndex = 0;
        if (group.getSelection() == jrbDefaultMining.getModel()) nIndex = OrgMinerOptions.DEFAULT_MINING; else if (group.getSelection() == jrbHierarchicalMining.getModel()) nIndex = OrgMinerOptions.AHC_MINING; else if (group.getSelection() == jrbDoingSimilarTask.getModel()) nIndex = OrgMinerOptions.SIMILAR_TASK; else if (group.getSelection() == jrbSOMMining.getModel()) nIndex = OrgMinerOptions.SOM_MINING; else nIndex = OrgMinerOptions.WORKING_TOGETHER;
        return nIndex;
    }
