...
public class HttpServerManager implements ManagedServiceFactory {
    ...
    public synchronized void updated(String pid, Dictionary dictionary) throws ConfigurationException {
        ...
        if (httpListener != null) server.addListener(httpListener);
        SocketListener httpsListener = createHttpsListener(dictionary);
        if (httpsListener != null) server.addListener(httpsListener);
        ServletHandler servlets = new Servlet25Handler();
        servlets.setAutoInitializeServlets(true);
        ServletHolder holder = servlets.addServlet("/*", InternalHttpServiceServlet.class.getName());
        holder.setInitOrder(0);
        holder.setInitParameter(Constants.SERVICE_VENDOR, "Eclipse.org");
        holder.setInitParameter(Constants.SERVICE_DESCRIPTION, "Equinox Jetty-based Http Service");
        if (httpListener != null) holder.setInitParameter(JettyConstants.HTTP_PORT, new Integer(httpListener.getPort()).toString());
        if (httpsListener != null) holder.setInitParameter(JettyConstants.HTTPS_PORT, new Integer(httpsListener.getPort()).toString());
        ...
        if (sessionInactiveInterval != null) servlets.setSessionInactiveInterval(sessionInactiveInterval.intValue());
    ...
    private SocketListener createHttpListener(Dictionary dictionary) {
        ...
        if (httpEnabled != null && !httpEnabled.booleanValue()) return null;
    ...
    private SocketListener createHttpsListener(Dictionary dictionary) {
        ...
        if (httpsEnabled == null || !httpsEnabled.booleanValue()) return null;
        ...
        if (keyStore != null) listener.setKeystore(keyStore);
        String password = (String) dictionary.get(JettyConstants.SSL_PASSWORD);
        if (password != null) listener.setPassword(password);
        String keyPassword = (String) dictionary.get(JettyConstants.SSL_KEYPASSWORD);
        if (keyPassword != null) listener.setKeyPassword(keyPassword);
        Object needClientAuth = dictionary.get(JettyConstants.SSL_NEEDCLIENTAUTH);
        if (needClientAuth != null) {
            if (needClientAuth instanceof String) needClientAuth = Boolean.valueOf((String) needClientAuth);
        ...
        if (wantClientAuth != null) {
            if (wantClientAuth instanceof String) wantClientAuth = Boolean.valueOf((String) wantClientAuth);
        ...
        if (algorithm != null) listener.setAlgorithm(algorithm);
        String keystoreType = (String) dictionary.get(JettyConstants.SSL_KEYSTORETYPE);
        if (keystoreType != null) listener.setKeystoreType(keystoreType);
        if (listener.getPort() == 0) {
            try {
                listener.open();
            } catch (IOException e) {
    ...
    private HttpContext createHttpContext(Dictionary dictionary) {
        ...
        if (contextPathProperty == null) contextPathProperty = "/";
