...
public class FastICAFrame extends JFrame {
    ...
    public FastICAFrame() {
        ...
        dataTr.getSelectionModel().setSelectionMode(TreeSelectionModel.SINGLE_TREE_SELECTION);
    ...
    private DefaultMutableTreeNode getSelectedNode() {
        ...
        if (!dataRootNd.isNodeDescendant(selectedNd)) return (null);
        return (selectedNd);
    }
    ...
    private int[] getNodeIndexList(DefaultMutableTreeNode node) {
        ...
        for (int i = 0; i < n; ++i) nodeList[i] = list.get(i).intValue();
        return (nodeList);
    }
    ...
    private void actionMix() {
        ...
        if (selNd != null) {
            ...
            if (selIdxs.length == 1) {
                ...
                if (m > 0) {
                    NumberDialog numberDl = new NumberDialog(this, "Number of output signals...", 1, 1, 32);
                    if (numberDl.open()) {
                        int n = numberDl.getNumber();
                        MatrixDialog matrixDl = new MatrixDialog(this, "Choose the mixing matrix...", Matrix.newMatrix(m, n, 0.0));
                        if (matrixDl.open()) {
    ...
    private void actionAnalyse() {
        ...
        if (selNd != null) {
            ...
            if (selIdxs.length == 1) {
                ...
                int m = audioList.size();
                if (m > 0) {
                    NumberDialog numberDl = new NumberDialog(this, "Number of independent components...", 1, 1, 32);
                    if (numberDl.open()) {
    ...
    protected void processWindowEvent(WindowEvent event) {
        if (event.getID() == WindowEvent.WINDOW_CLOSING) actionExit();
