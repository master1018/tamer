...
public class URI {
    ...
    public URI(URI base, String rel_uri) throws ParseException {
        ...
        if (pos < len - 3 && uri[pos + 3] == ':' && (uri[pos + 0] == 'u' || uri[pos + 0] == 'U') && (uri[pos + 1] == 'r' || uri[pos + 1] == 'R') && (uri[pos + 2] == 'i' || uri[pos + 2] == 'I' || uri[pos + 2] == 'l' || uri[pos + 2] == 'L')) pos += 4;
        idx = pos;
        while (idx < len && uri[idx] != ':' && uri[idx] != '/' && uri[idx] != '?' && uri[idx] != '#') idx++;
        if (idx < len && uri[idx] == ':') {
        ...
        if (type == OPAQUE) {
            if (base != null && scheme == null) throw new ParseException("Can't resolve relative URI for " + "scheme " + final_scheme);
        ...
        if (type == SEMI_GENERIC) {
            ...
            if (pos < len && uri[pos] == '#') this.fragment = escape(rel_uri.substring(pos + 1, len), escpdFragChar, true);
        }
        if (base != null) {
            if (scheme != null && !(scheme.equals(base.scheme) && ENABLE_BACKWARDS_COMPATIBILITY)) return;
            scheme = base.scheme;
            if (host != null) return;
            userinfo = base.userinfo;
            host = base.host;
            port = base.port;
            if (type == SEMI_GENERIC) return;
            ...
            if (path.length() == 0 || path.charAt(0) != '/') {
                ...
                if (idx < 0) path = '/' + path; else path = base.path.substring(0, idx + 1) + path;
    ...
    public static String canonicalizePath(String path) {
        ...
        if (!((idx = path.indexOf("/.")) != -1 && (idx == len - 2 || path.charAt(idx + 2) == '/' || (path.charAt(idx + 2) == '.' && (idx == len - 3 || path.charAt(idx + 3) == '/'))))) return path;
     ...
     */
    private void parse_authority(String authority, String _scheme) throws ParseException {
        ...
        if (idx < len && uri[idx] == '[') {
            while (idx < len && uri[idx] != ']') idx++;
            if (idx == len) throw new ParseException("No closing ']' found for opening '['" + " at position " + pos + " in authority `" + authority + "'");
        ...
        if (pos < (len - 1) && uri[pos] == ':') {
            ...
            if (p == defaultPort(_scheme)) this.port = -1; else this.port = p;
        }
    }
    ...
    public URL toURL() throws MalformedURLException {
        ...
        String hostinfo;
        if (userinfo != null && host != null) hostinfo = userinfo + "@" + host; else if (userinfo != null) hostinfo = userinfo + "@"; else hostinfo = host;
    ...
    private final String stringify(boolean unescape) {
        ...
        if (host != null) {
            if (host.indexOf(':') < 0) uri.append(unescape ? unescapeNoPE(host, resvdHostChar) : host); else uri.append('[').append(host).append(']');
        }
        if (port != -1) {
            uri.append(':');
            uri.append(port);
    ...
    public boolean equals(Object other) {
        if (other instanceof URI) {
            URI o = (URI) other;
            return (scheme.equals(o.scheme) && (type == OPAQUE && areEqual(opaque, o.opaque) || type == SEMI_GENERIC && areEqual(userinfo, o.userinfo) && areEqualIC(host, o.host) && port == o.port && areEqual(path, o.path) || type == GENERIC && areEqual(userinfo, o.userinfo) && areEqualIC(host, o.host) && port == o.port && pathsEqual(path, o.path) && areEqual(query, o.query) && areEqual(fragment, o.fragment)));
        }
        if (other instanceof URL) {
            URL o = (URL) other;
            String h, f;
            if (userinfo != null) h = userinfo + "@" + host; else h = host;
            f = getPathAndQuery();
            return (scheme.equalsIgnoreCase(o.getProtocol()) && (type == OPAQUE && opaque.equals(o.getFile()) || type == SEMI_GENERIC && areEqualIC(h, o.getHost()) && (port == o.getPort() || o.getPort() == defaultPort(scheme)) && areEqual(f, o.getFile()) || type == GENERIC && areEqualIC(h, o.getHost()) && (port == o.getPort() || o.getPort() == defaultPort(scheme)) && pathsEqual(f, o.getFile()) && areEqual(fragment, o.getRef())));
        }
        return false;
    }
    ...
    private static final boolean pathsEqual(String p1, String p2) {
        ...
        while (pos1 < end1 && pos2 < end2) {
            ...
            if (pos1 == end1 && pos2 < end2 || pos2 == end2 && pos1 < end1 || pos1 < end1 && pos2 < end2 && p1.charAt(pos1) != p2.charAt(pos2)) return false;
            if ((!p1.regionMatches(start1, p2, start2, pos1 - start1) || (pos1 - start1) != (pos2 - start2)) && !unescapeNoPE(p1.substring(start1, pos1), null).equals(unescapeNoPE(p2.substring(start2, pos2), null))) return false;
            pos1++;
            pos2++;
        }
     ...
     */
    public int hashCode() {
        if (hashCode == -1) hashCode = (scheme != null ? unescapeNoPE(scheme, null).hashCode() : 0) + (type == OPAQUE ? (opaque != null ? unescapeNoPE(opaque, null).hashCode() : 0) * 7 : (host != null ? unescapeNoPE(host, null).toLowerCase().hashCode() : 0) * 7 + (path != null ? unescapeNoPE(path, null).hashCode() : 0) * 13 + (query != null ? unescapeNoPE(query, null).hashCode() : 0) * 17);
        return hashCode;
    }
    ...
    public static char[] escape(char[] elem, BitSet allowed_char, boolean utf8) {
        ...
        for (int idx = 0; idx < elem.length; idx++) {
            if (!allowed_char.get(elem[idx])) {
                ...
                if (utf8) {
                    ...
                    if ((elem[idx] & 0xFC00) == 0xD800 && idx + 1 < elem.length && (elem[idx + 1] & 0xFC00) == 0xDC00) cnt -= 6;
                }
            }
        }
        ...
        for (int idx = 0, pos = 0; idx < elem.length; idx++) {
            char c = elem[idx];
            if (allowed_char.get(c)) tmp[pos++] = c; else if (utf8) {
                if (c <= 0x007F) {
                    pos = enc(tmp, pos, c);
                } else if (c <= 0x07FF) {
    ...
    public static final String unescape(String str, BitSet reserved) throws ParseException {
            ...
            if (buf[sidx] == '%') {
                ...
                if (utf_len > 0) {
                    if ((ch & 0xC0) != 0x80) {
                        ...
                        if ((utf[0] & 0xE0) == 0xC0) ch = (utf[0] & 0x1F) << 6 | (ch & 0x3F); else if ((utf[0] & 0xF0) == 0xE0) ch = (utf[0] & 0x0F) << 12 | (utf[1] & 0x3F) << 6 | (ch & 0x3F); else ch = (utf[0] & 0x07) << 18 | (utf[1] & 0x3F) << 12 | (utf[2] & 0x3F) << 6 | (ch & 0x3F);
                        if (reserved != null && reserved.get(ch)) didx = copyBuf(utf, utf_idx, ch, res, didx, null, true); else if (utf_len < 4) res[didx++] = (char) ch; else {
                            ch -= 0x10000;
                            res[didx++] = (char) ((ch >> 10) | 0xD800);
                            res[didx++] = (char) ((ch & 0x03FF) | 0xDC00);
                        }
                        utf_len = -1;
                    } else utf[utf_idx++] = (char) ch;
                } else if ((ch & 0xE0) == 0xC0 || (ch & 0xF0) == 0xE0 || (ch & 0xF8) == 0xF0) {
                    if ((ch & 0xE0) == 0xC0) utf_len = 2; else if ((ch & 0xF0) == 0xE0) utf_len = 3; else utf_len = 4;
                    utf[0] = (char) ch;
                    utf_idx = 1;
        ...
        if (utf_len > 0) didx = copyBuf(utf, utf_idx, -1, res, didx, reserved, false);
    ...
    private static final int copyBuf(char[] utf, int utf_idx, int ch, char[] res, int didx, BitSet reserved, boolean escapeAll) {
        ...
        for (int idx = 0; idx < utf_idx; idx++) {
            if (reserved != null && reserved.get(utf[idx]) || escapeAll) didx = enc(res, didx, utf[idx]); else res[didx++] = utf[idx];
        }
        return didx;
    }
    ...
    public static void main(String args[]) throws Exception {
        ...
        testParser(base, "g#s/../x", "http://a/b/c/g#s/../x");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "http:g", "http://a/b/c/g"); else testParser(base, "http:g", "http:g");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "http:", "http://a/b/c/d;p?q"); else testParser(base, "http:", "http:");
        testParser(base, "./g:h", "http://a/b/c/g:h");
        ...
        testParser(base, "ftp:/h", "ftp:/h");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "http:/h", "http://s/h"); else testParser(base, "http:/h", "http:/h");
        ...
        base = new URI("ldap://s");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:", "ldap://s"); else testParser(base, "ldap:", "ldap:");
        testParser(base, "ldap://a", "ldap://a");
        testParser(base, "ldap://a/b", "ldap://a/b");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:/b", "ldap://s/b"); else testParser(base, "ldap:/b", "ldap:/b");
        testParser(base, "ftp:h", "ftp:h");
        testParser(base, "ftp://h", "ftp://h");
        testParser(base, "//g", "ldap://g");
        testParser(base, "//g?h", "ldap://g/?h");
        ...
        testParser(base, "ftp:/h", "ftp:/h");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:/h", "ldap://s/h"); else testParser(base, "ldap:/h", "ldap:/h");
        if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:", "ldap://s"); else testParser(base, "ldap:", "ldap:");
        testParser(base, "ldap://a", "ldap://a");
        testParser(base, "ldap://a/b", "ldap://a/b");
        testParser(base, "ftp:h", "ftp:h");
    ...
    private static void testEscape(String raw, String escaped) throws Exception {
        ...
        if (!test.equals(escaped)) throw new Exception("Test failed: " + nl + "  raw-string: " + raw + nl + "  escaped:    " + test + nl + "  expected:   " + escaped);
    }

    private static void testUnescape(String escaped, String raw) throws Exception {
        if (!unescape(escaped, null).equals(raw)) throw new Exception("Test failed: " + nl + "  escaped-string: " + escaped + nl + "  unescaped:      " + unescape(escaped, null) + nl + "  expected:       " + raw);
    }
}
