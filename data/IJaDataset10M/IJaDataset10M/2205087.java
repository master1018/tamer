...
public class ServerImpl extends SelectedPortUnicastRemoteObject implements Server {
    ...
    private X509Certificate getClientCert(byte[] sessionId) throws Exception {
        ...
        if (clientCerts == null || clientCerts.length == 0) throw new TryAgainException();
        return clientCerts[0];
    }

    private void throwRemoteException(Throwable t) throws RemoteException {
        PrimeLog.throwing(t);
        if (t instanceof RemoteException) throw (RemoteException) t; else throw new RemoteException("server exception", t);
    }
    ...
    public String ping(byte[] sessionID) throws RemoteException {
        try {
            return "ipaddress: " + localAddress.getHostAddress() + ", name: " + localAddress.getHostName() + ", canonical name: " + localAddress.getCanonicalHostName() + ", client name: " + getClientCert(sessionID).getSubjectDN().toString();
        } catch (Throwable t) {
    ...
    private class SignData {
        ...
        private final SignServerSessionLocal signSession;
        ...
        ISignResponse signData(int signerID, ISignRequest request, X509Certificate cert) throws Exception {
            PrimeLog.debug(">signData " + request.getRequestID());
            PrimeLog.debug(cert.toString());
            ISignResponse res = signSession.signData(signerID, request, cert, null);
            PrimeLog.debug("<signData " + request.getRequestID());
            return res;
        }
    }
     ...
     */
    public ISignResponse signData(int signerID, ISignRequest request, byte[] sessionID) throws RemoteException {
        try {
            X509Certificate clientCert = getClientCert(sessionID);
            if (signData == null) signData = new SignData();
