...
public class FVector extends SimpleVector implements Externalizable, Consumable, Comparable {
    ...
    protected void clearBuffer(int start, int count) {
        Object[] d = data;
        while (--count >= 0) d[start++] = null;
    }

    public boolean equals(Object obj) {
        if (obj == null || !(obj instanceof FVector)) return false;
        FVector obj_vec = (FVector) obj;
        int n = size;
        if (obj_vec.data == null || obj_vec.size != n) return false;
        Object[] this_data = data;
        Object[] obj_data = obj_vec.data;
        for (int i = 0; i < n; i++) {
            if (!(this_data[i].equals(obj_data[i]))) return false;
        }
        return true;
    }
    ...
    public final void setAll(Object new_value) {
        Object[] d = data;
        for (int i = size; --i >= 0; ) d[i] = new_value;
    }
    ...
    public void consumePosRange(int iposStart, int iposEnd, Consumer out) {
        ...
        for (; i < end; i++) out.writeObject(data[i]);
    }

    public void consume(Consumer out) {
        out.startElement("#vector");
        int len = size;
        for (int i = 0; i < len; i++) out.writeObject(data[i]);
        out.endElement();
    }
   ...
   */
    public void writeExternal(ObjectOutput out) throws IOException {
        int n = size;
        out.writeInt(n);
        for (int i = 0; i < n; i++) out.writeObject(data[i]);
    }

    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        int n = in.readInt();
        Object[] data = new Object[n];
        for (int i = 0; i < n; i++) data[i] = in.readObject();
        size = n;
        this.data = data;
    }
}
