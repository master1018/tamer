...
public class CaptionDirective implements TemplateDirectiveModel {
    ...
    public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
        ...
        if (params.containsKey("key") && chars != null && chars.length > 0) {
            throw new TemplateModelException("Caption directive expects key parameter or directive body, but not in the same time.");
        }
        if (!params.containsKey("key") && (chars == null || chars.length == 0)) {
            throw new TemplateModelException("Caption directive expects either key parameter or directive body, but none found.");
        }
