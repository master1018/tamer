...
public class PotentiometerPanel extends AbstractPotentiometer {
    ...
    public Area[] getBody() {
        ...
        if (body == null) {
            ...
            switch(orientation) {
                case DEFAULT:
                    body[3] = new Area(new Ellipse2D.Double(controlPoints[0].x + spacing - diameter / 2, controlPoints[0].y - spacing / 2 - diameter, diameter, diameter));
                    for (int i = 0; i < 3; i++) {
                        body[i] = new Area(new Rectangle2D.Double(controlPoints[i].x - holeDiameter / 2, controlPoints[i].y - (spacing + diameter) / 2, holeDiameter, (spacing + diameter) / 2));
                    }
                    break;
                case _90:
                    body[3] = new Area(new Ellipse2D.Double(controlPoints[0].x + spacing / 2, controlPoints[0].y + spacing - diameter / 2, diameter, diameter));
                    for (int i = 0; i < 3; i++) {
                        body[i] = new Area(new Rectangle2D.Double(controlPoints[i].x, controlPoints[i].y - holeDiameter / 2, (spacing + diameter) / 2, holeDiameter));
                    }
                    break;
                case _180:
                    body[3] = new Area(new Ellipse2D.Double(controlPoints[0].x - spacing - diameter / 2, controlPoints[0].y + spacing / 2, diameter, diameter));
                    for (int i = 0; i < 3; i++) {
                        body[i] = new Area(new Rectangle2D.Double(controlPoints[i].x - holeDiameter / 2, controlPoints[i].y, holeDiameter, (spacing + diameter) / 2));
                    }
                    break;
                case _270:
                    body[3] = new Area(new Ellipse2D.Double(controlPoints[0].x - spacing / 2 - diameter, controlPoints[0].y - spacing - diameter / 2, diameter, diameter));
                    for (int i = 0; i < 3; i++) {
                        body[i] = new Area(new Rectangle2D.Double(controlPoints[i].x - (spacing + diameter) / 2, controlPoints[i].y - holeDiameter / 2, (spacing + diameter) / 2, holeDiameter));
                    }
                    break;
                default:
                    break;
            ...
            for (int i = 0; i < 3; i++) {
                body[4 + i].subtract(new Area(new Ellipse2D.Double(controlPoints[i].x - holeDiameter / 2, controlPoints[i].y - holeDiameter / 2, holeDiameter, holeDiameter)));
            }
        }
        return body;
    }
    ...
    public void draw(Graphics2D g2d, ComponentState componentState, boolean outlineMode, Project project, IDrawingObserver drawingObserver) {
        ...
        for (Area shape : getBody()) {
            if (shape != null) {
                ...
                g2d.setColor(componentState == ComponentState.SELECTED || componentState == ComponentState.DRAGGING ? SELECTION_COLOR : borderColor);
                g2d.draw(shape);
            }
        }
        g2d.setFont(LABEL_FONT);
        g2d.setColor(componentState == ComponentState.SELECTED ? LABEL_COLOR_SELECTED : LABEL_COLOR);
        ...
        g2d.drawString(getValue().toString(), (int) (bodyRect.getX() + x), (int) (bodyRect.getY() + y));
    }
