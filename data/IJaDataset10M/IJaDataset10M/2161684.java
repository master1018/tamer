...
public abstract class ReplicatorTemplate {
    ...
    protected <T> T replicate(final Object from, Class<T> toClass) throws SecurityException {
        ...
        if (unenhanced != from && from.getClass() == toClass) {
            @SuppressWarnings("unchecked") Class<T> unenhancedClass = (Class<T>) unenhanced.getClass();
        ...
        if (immutable(toClass) || immutable(unenhanced.getClass())) return beanTransformer.getImmutableReplicatable().replicateImmutable(unenhanced, toClass);
        if (unenhanced instanceof Collection<?>) return beanTransformer.getCollectionReplicatable().replicateCollection((Collection<?>) unenhanced, toClass);
        if (unenhanced.getClass().isArray()) return beanTransformer.getArrayReplicatable().replicateArray(unenhanced, toClass);
        if (unenhanced instanceof Map<?, ?>) {
            @SuppressWarnings("unchecked") T ret = (T) beanTransformer.getMapReplicatable().replicateMap((Map) unenhanced, toClass);
            return ret;
        }
        if (unenhanced instanceof Date) return beanTransformer.getDateReplicatable().replicateDate((Date) unenhanced, toClass);
        if (unenhanced instanceof Calendar) return beanTransformer.getCalendarReplicatable().replicateCalendar((Calendar) unenhanced, toClass);
        if (unenhanced instanceof Blob) return beanTransformer.getBlobReplicatable().replicateBlob((Blob) unenhanced, toClass);
    ...
    protected final <T> Class<T> chooseClass(Class<?> fromClass, Class<T> toClass) {
        if (!toClass.isAssignableFrom(fromClass) || Modifier.isAbstract(fromClass.getModifiers())) return toClass;
    ...
    protected void populateBean(Object fromMember, Object toMember) {
        beanTransformer.getBeanPopulatorSpiFactory().newBeanPopulator(fromMember, toMember).initBeanPopulatorBaseConfig(beanTransformer.getBeanPopulatorBaseConfig()).initTransformer(beanTransformer).populate();
    }
    ...
    protected final <T> T newInstanceAsPrivileged(Class<T> c) throws SecurityException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        if (Modifier.isPublic(constructor.getModifiers())) return c.newInstance();
