...
public abstract class WriterPool {
     ...
     */
    public WriterPool(final AtomicInteger serial, final BasePoolableObjectFactory factory, final WriterPoolSettings settings, final int poolMaximumActive, final int poolMaximumWait) {
        logger.info("Initial configuration:" + " prefix=" + settings.getPrefix() + ", suffix=" + settings.getSuffix() + ", compress=" + settings.isCompressed() + ", maxSize=" + settings.getMaxSize() + ", maxActive=" + poolMaximumActive + ", maxWait=" + poolMaximumWait);
        this.settings = settings;
        this.pool = new FairGenericObjectPool(factory, poolMaximumActive, GenericObjectPool.WHEN_EXHAUSTED_BLOCK, poolMaximumWait, NO_MAX_IDLE);
    ...
    public WriterPoolMember borrowFile() throws IOException {
        ...
        for (int i = 0; f == null; i++) {
            ...
            try {
                ...
                logger.warning(e.getMessage() + ": Retry #" + i + " of " + " max of " + arbitraryRetryMax + ": NSEE Pool State: " + getPoolState(waitStart));
                if (i >= arbitraryRetryMax) {
                    logger.log(Level.SEVERE, "maximum retries exceeded; rethrowing", e);
                    throw e;
                }
            } catch (Exception e) {
                logger.severe(e.getMessage() + ": E Pool State: " + getPoolState(waitStart));
