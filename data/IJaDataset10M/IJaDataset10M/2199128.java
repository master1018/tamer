...
public class JointTimeModeChooserConfigBuilder implements ConfigurationBuilder {
	 ...
	 */
    public JointTimeModeChooserConfigBuilder(final JointPlan plan, final JointTimeModeChooserConfigGroup config, final ScoringFunctionFactory scoringFunctionFactory, final TripRouterFactory tripRouterFactory) {
        this(plan, config, scoringFunctionFactory, tripRouterFactory, null);
    }

    public JointTimeModeChooserConfigBuilder(final JointPlan plan, final JointTimeModeChooserConfigGroup config, final ScoringFunctionFactory scoringFunctionFactory, final TripRouterFactory tripRouterFactory, final String analysisOutputDir) {
        this.plan = plan;
        this.config = config;
    ...
    public void buildConfiguration(final TabuSearchConfiguration configuration) {
        ...
        JointTimeModeChooserSolution initialSolution = new JointTimeModeChooserSolution(plan, tripRouterFactory.createTripRouter());
        ...
        FitnessFunction fitness = new JointTimeModeChooserFitness(config.getNegativeDurationPenalty(), config.getUnsynchronizedPenalty(), scoringFunctionFactory);
        configuration.setFitnessFunction(fitness);
        configuration.setEvolutionMonitor(new ImprovementDelayMonitor(improvementDelay, N_ITER));
        CompositeMoveGenerator generator = new CompositeMoveGenerator();
        generator.add(new FixedStepsIntegerMovesGenerator(initialSolution, config.getDurationSteps(), false));
        generator.add(new AllPossibleModesMovesGenerator(initialSolution, config.getModes()));
        ...
        if (outputDir != null) {
            configuration.addListener(new EvolutionPlotter("score evolution, clique " + plan.getPerson().getId() + ", " + cliqueSize + " members", outputDir + "/" + plan.getPerson().getId() + "-fitness.png"));
        }
    }
}
