...
public class Protocol extends ProtocolBase {
    ...
    public synchronized Message receive() throws IOException {
        ...
        try {
            ...
            length = receive0(m_iport, midletSuite.getID(), connHandle, smsPacket);
            if (length >= 0) {
                String type;
                ...
                message = newMessage(type, new String(ADDRESS_PREFIX + new String(smsPacket.address) + ":" + smsPacket.port));
                String messg = null;
                if (isTextMessage) {
                    if (length > 0) {
    ...
    public void send(Message dmsg) throws IOException {
        ...
        try {
            send0(connHandle, messageType, url.host, url.port, sourcePort, msgBuffer);
        } catch (IOException ex) {
    ...
    public int numberOfSegments(Message msg) {
        ...
        if (msgBuffer != null) {
            ...
            segments = numberOfSegments0(msgBuffer, msgBuffer.length, messageType, hasPort);
        }
        return segments;
    }
    ...
    public synchronized Connection openPrimInternal(String name, int mode, boolean timeouts) throws IOException {
        ...
        int len = openconnections.size();
        for (int i = 0; i < len; i++) {
            if (!(openconnections.elementAt(i) instanceof com.sun.midp.io.j2me.sms.Protocol) || ((Protocol) openconnections.elementAt(i)).url.equals(name)) {
     ...
     */
    private native int open0(String host, int msid, int port) throws IOException;
     ...
     */
    protected int unblock00(int msid) throws IOException {
     ...
     * @exception IOException  if an I/O error occurs
     */
    private static native int send0(int handle, int type, String host, int destPort, int sourcePort, byte[] message) throws IOException;
     ...
     * @exception IOException  if an I/O error occurs
     */
    private static native int receive0(int port, int msid, int handle, SMSPacket smsPacket) throws IOException;
     ...
     * @exception IOException  if an I/O error occurs
     */
    private native int waitUntilMessageAvailable0(int port, int handle) throws IOException;
     ...
     * @exception IOException  if an I/O error occurs
     */
    protected int waitUntilMessageAvailable00(int handle) throws IOException {
     ...
     *	   message.
     */
    private native int numberOfSegments0(byte msgBuffer[], int msgLen, int msgType, boolean hasPort);
}
