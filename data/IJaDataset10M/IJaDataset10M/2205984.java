...
public class PyFileReader extends PyObject {
    ...
    protected char[] needBuffer(int size) {
        if (reuseableBuffer == null) {
            if (size > DEFAULT_BUF_SIZE) return new char[size];
            reuseableBuffer = new char[DEFAULT_BUF_SIZE];
        }
        if (size <= reuseableBuffer.length) return reuseableBuffer;
    ...
    public PyString readline(int max) {
        if (!(max < 0)) throw Py.NotImplementedError("size argument to readline not implemented for PyFileReader");
        synchronized (reader) {
            try {
