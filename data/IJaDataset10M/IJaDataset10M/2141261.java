...
public class MeasureTool extends AVListImpl implements Disposable {
    ...
    protected static class CustomRenderableLayer extends RenderableLayer implements PreRenderable, Renderable {
        ...
        public void render(DrawContext dc) {
            if (dc.isPickingMode() && !this.isPickEnabled()) return;
    ...
    public MeasureTool(final WorldWindow wwd, RenderableLayer applicationLayer) {
        ...
        if (this.applicationLayer != null) this.applicationLayer.addRenderable(this.layer); else this.wwd.getModel().getLayers().add(this.layer);
        ...
        this.controlPointsAttributes.setDistanceMaxScale(1);
    ...
    public void setLabel(String labelName, String label) {
        if (labelName != null && labelName.length() > 0) this.setValue(labelName, label);
    }
    ...
    public void setArmed(boolean state) {
        if (this.controller != null) this.controller.setArmed(state);
    }
    ...
    public void setPositions(ArrayList<? extends Position> newPositions) {
        ...
        if (newPositions.size() < 2) return;
        this.clear();
        boolean closedShape = newPositions.get(0).equals(newPositions.get(newPositions.size() - 1));
        if (newPositions.size() > 2 && closedShape) setMeasureShapeType(SHAPE_POLYGON); else setMeasureShapeType(getPathType(newPositions));
        for (int i = 0; i < newPositions.size(); i++) {
            Position pos = newPositions.get(i);
            this.positions.add(pos);
            if (i < newPositions.size() - 1 || !closedShape) addControlPoint(pos, CONTROL_TYPE_LOCATION_INDEX, this.positions.size() - 1);
        }
        if (this.measureShapeType.equals(SHAPE_LINE)) this.shapeOrientation = LatLon.greatCircleAzimuth(this.positions.get(0), this.positions.get(1));
        updateMeasureShape();
    ...
    public void setLineWidth(double width) {
        ...
        if (this.line != null) this.line.setLineWidth(width);
        if (this.surfaceShape != null) {
            ShapeAttributes attr = this.surfaceShape.getAttributes();
            if (attr == null) attr = new BasicShapeAttributes();
    ...
    public void setPathType(String type) {
        this.pathType = type;
        if (this.line != null) this.line.setPathType(polylinePathTypeFromKey(type));
        if (this.surfaceShape != null) this.surfaceShape.setPathType(type);
        if (this.isRegularShape()) this.updateShapeControlPoints();
        this.wwd.redraw();
    }
     ...
     */
    public void setMeasureShapeType(String shapeType, Position centerPosition, double width, double height, Angle orientation) {
        if (shapeType == null) {
            String msg = Logging.getMessage("nullValue.ShapeType");
            Logging.logger().severe(msg);
            throw new IllegalArgumentException(msg);
    ...
    public void setMeasureShape(Polyline line) {
        ...
        if (this.line != null) this.shapeLayer.removeRenderable(this.line);
        this.line = line;
    ...
    public void setMeasureShape(SurfaceShape surfaceShape) {
        ...
        if (surfaceShape instanceof SurfaceQuad) {
            ...
            this.shapeCenterPosition = new Position(shape.getCenter(), 0);
            this.shapeRectangle = new Rectangle2D.Double(0, 0, shape.getMajorRadius() * 2, shape.getMinorRadius() * 2);
            this.shapeOrientation = shape.getHeading();
            updateShapeControlPoints();
            updatePositionsFromShape();
        } else {
    ...
    protected boolean isRegularShape(String shape) {
        return (shape.equals(SHAPE_CIRCLE) || shape.equals(SHAPE_ELLIPSE) || shape.equals(SHAPE_QUAD) || shape.equals(SHAPE_SQUARE));
    }
    ...
    public boolean isSideControl(ControlPoint controlPoint) {
        String control = controlPoint.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
        return control != null && (control.equals(NORTH) || control.equals(EAST) || control.equals(SOUTH) || control.equals(WEST));
    }

    public boolean isCornerControl(ControlPoint controlPoint) {
        String control = controlPoint.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
        return control != null && (control.equals(NORTHEAST) || control.equals(SOUTHEAST) || control.equals(SOUTHWEST) || control.equals(NORTHWEST));
    }

    public double getLength() {
        Globe globe = this.wwd.getModel().getGlobe();
        if (this.line != null) return this.line.getLength(globe);
        if (this.surfaceShape != null) return this.surfaceShape.getPerimeter(globe);
        return -1;
    }

    public double getArea() {
        Globe globe = this.wwd.getModel().getGlobe();
        if (this.surfaceShape != null) return this.surfaceShape.getArea(globe, this.followTerrain);
        return -1;
    }

    public double getWidth() {
        if (this.shapeRectangle != null) return this.shapeRectangle.width;
        return -1;
    }

    public double getHeight() {
        if (this.shapeRectangle != null) return this.shapeRectangle.height;
        return -1;
    }
    ...
    public void addControlPoint() {
        ...
        if (this.isRegularShape()) {
            ...
            if (!this.measureShapeType.equals(SHAPE_POLYGON) || this.positions.size() <= 1) {
                ...
                addControlPoint(this.positions.get(this.positions.size() - 1), CONTROL_TYPE_LOCATION_INDEX, this.positions.size() - 1);
                ...
                addControlPoint(this.positions.get(this.positions.size() - 2), CONTROL_TYPE_LOCATION_INDEX, this.positions.size() - 2);
            }
        }
        updateMeasureShape();
    ...
    public void removeControlPoint() {
        ...
        if (this.isRegularShape()) {
            ...
            if (!this.measureShapeType.equals(SHAPE_POLYGON) || this.positions.size() == 1) {
                ...
                if (positions.size() == 2) positions.remove(1);
            }
            if (this.controlPoints.size() > 0) this.controlPoints.remove(this.controlPoints.size() - 1);
        }
    ...
    public void moveControlPoint(ControlPoint point, String mode) {
        ...
        if (point.getValue(CONTROL_TYPE_LOCATION_INDEX) != null) {
            ...
            if (measureShapeType.equals(SHAPE_POLYGON) && positions.size() > 2 && positionIndex == 0) positions.set(positions.size() - 1, surfacePosition);
            if (measureShapeType.equals(SHAPE_LINE) && positions.size() > 1) shapeOrientation = LatLon.greatCircleAzimuth(positions.get(0), positions.get(1));
        }
        updateMeasureShape();
    }
    ...
    public void moveMeasureShape(Angle azimuth, Angle distance) {
        ...
        if (this.isRegularShape()) {
            if (controlPoints.size() > 0) {
                ...
                point.setPosition(new Position(LatLon.greatCircleEndPosition(point.getPosition(), azimuth, distance), 0));
                moveControlPoint(point);
            }
        } else {
            for (int i = 0; i < positions.size(); i++) {
                Position newPos = computeSurfacePosition(LatLon.greatCircleEndPosition(positions.get(i), azimuth, distance));
                positions.set(i, newPos);
                if (!this.measureShapeType.equals(SHAPE_POLYGON) || i < positions.size() - 1) ((ControlPoint) controlPoints.get(i)).setPosition(new Position(newPos, 0));
            }
            if (measureShapeType.equals(SHAPE_LINE) && positions.size() > 1) shapeOrientation = LatLon.greatCircleAzimuth(positions.get(0), positions.get(1));
            updateMeasureShape();
        }
    ...
    protected Position computeSurfacePosition(LatLon latLon) {
        Vec4 surfacePoint = wwd.getSceneController().getTerrain().getSurfacePoint(latLon.getLatitude(), latLon.getLongitude());
        if (surfacePoint != null) return wwd.getModel().getGlobe().computePositionFromPoint(surfacePoint); else return new Position(latLon, wwd.getModel().getGlobe().getElevation(latLon.getLatitude(), latLon.getLongitude()));
    }
    ...
    protected void updateShapeOrientation(String control, Position newPosition) {
        Angle controlAzimuth = this.computeControlPointAzimuth(control, this.shapeRectangle.width, this.shapeRectangle.height);
    ...
    protected void updateShapeSize(String control, Position newPosition) {
        if (this.measureShapeType.equals(SHAPE_ELLIPSE) || this.measureShapeType.equals(SHAPE_CIRCLE)) {
            ...
            if (widthMeters <= SHAPE_MIN_WIDTH_METERS) widthMeters = SHAPE_MIN_WIDTH_METERS;
            if (heightMeters <= SHAPE_MIN_HEIGHT_METERS) heightMeters = SHAPE_MIN_HEIGHT_METERS;
            ...
            LatLon newCenterLocation = LatLon.greatCircleEndPosition(oppositeControlPoint.getPosition(), diagonalAzimuth, diagonalArcLength.divide(2d));
            ...
            if (this.measureShapeType.equals(SHAPE_SQUARE)) {
                ...
                LatLon location = this.moveShapeByControlPoint(oppositeControlPoint, this.wwd.getModel().getGlobe(), this.shapeOrientation, newCenterLocation, widthMeters, heightMeters);
                if (location != null) newCenterLocation = location;
            }
    ...
    public ControlPoint getControlPoint(String control) {
        for (Renderable cp : this.controlPoints) {
            ...
            if (value != null && value.equals(control)) return (ControlPoint) cp;
        }
        return null;
    }

    protected String computeCornerControl(Position position) {
        if (this.shapeCenterPosition == null || this.shapeOrientation == null) return null;
        ...
        if (azimuth.degrees < 90) return NORTHEAST; else if (azimuth.degrees < 180) return SOUTHEAST; else if (azimuth.degrees < 270) return SOUTHWEST; else return NORTHWEST;
    }

    protected ControlPoint getOppositeControl(String control) {
        if (this.controlPoints.size() == 0) return null; else if (this.controlPoints.size() == 1) return getControlPoint(CENTER); else if (control.equals(NORTH)) return getControlPoint(SOUTH); else if (control.equals(EAST)) return getControlPoint(WEST); else if (control.equals(SOUTH)) return getControlPoint(NORTH); else if (control.equals(WEST)) return getControlPoint(EAST); else if (control.equals(NORTHEAST)) return getControlPoint(SOUTHWEST); else if (control.equals(SOUTHEAST)) return getControlPoint(NORTHWEST); else if (control.equals(SOUTHWEST)) return getControlPoint(NORTHEAST); else if (control.equals(NORTHWEST)) return getControlPoint(SOUTHEAST);
        return null;
    }

    protected LatLon moveShapeByControlPoint(ControlPoint controlPoint, Globe globe, Angle heading, LatLon center, double width, double height) {
        double globeRadius = globe.getRadiusAt(center);
        String control = controlPoint.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
        if (control == null) return center;
        LatLon newCenterLocation = center;
        for (int i = 0; i < MAX_SHAPE_MOVE_ITERATIONS; i++) {
            LatLon shapeControlLocation = this.computeControlPointLocation(control, globe, heading, newCenterLocation, width, height);
            ...
            if (pathLengthMeters < SHAPE_CONTROL_EPSILON_METERS) break;
    ...
    protected void swapEdgeControls(String control, Position position) {
        ...
        if (this.shapeCenterPosition == null || this.shapeOrientation == null) return;
        Angle azimuth = LatLon.greatCircleAzimuth(this.shapeCenterPosition, position).subtract(this.shapeOrientation);
        azimuth = computeNormalizedHeading(azimuth);
        if ((control.equals(NORTH) && azimuth.degrees < 270 && azimuth.degrees > 90) || (control.equals(SOUTH) && (azimuth.degrees > 270 || azimuth.degrees < 90))) {
            for (Renderable r : this.controlPoints) {
                ControlPoint cp = (ControlPoint) r;
                String c = cp.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
                if (c == null) continue;
                if (c.equals(NORTH)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, SOUTH); else if (c.equals(SOUTH)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, NORTH);
            }
        }
        if ((control.equals(EAST) && (azimuth.degrees < 360 && azimuth.degrees > 180)) || (control.equals(WEST) && (azimuth.degrees < 180 && azimuth.degrees > 0))) {
            for (Renderable r : this.controlPoints) {
                ControlPoint cp = (ControlPoint) r;
                String c = cp.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
                if (c == null) continue;
                if (c.equals(EAST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, WEST); else if (c.equals(WEST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, EAST);
            }
        }
    }
    ...
    protected void swapCornerControls(String control, Position position) {
        ...
        if (control.equals(newControl)) return;
        if (control.length() != 2 || newControl.length() != 2) return;
        if (control.charAt(0) != newControl.charAt(0)) {
            for (Renderable r : this.controlPoints) {
                ControlPoint cp = (ControlPoint) r;
                String c = cp.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
                if (c == null) continue;
                if (c.equals(NORTHEAST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, SOUTHEAST); else if (c.equals(SOUTHEAST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, NORTHEAST); else if (c.equals(SOUTHWEST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, NORTHWEST); else if (c.equals(NORTHWEST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, SOUTHWEST);
            }
        }
        if (control.charAt(1) != newControl.charAt(1)) {
            for (Renderable r : this.controlPoints) {
                ...
                String c = cp.getStringValue(CONTROL_TYPE_REGULAR_SHAPE);
                if (c == null) continue;
                if (c.equals(NORTHEAST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, NORTHWEST); else if (c.equals(SOUTHEAST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, SOUTHWEST); else if (c.equals(SOUTHWEST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, SOUTHEAST); else if (c.equals(NORTHWEST)) cp.setValue(CONTROL_TYPE_REGULAR_SHAPE, NORTHEAST);
            }
        }
    }

    protected LatLon computeControlPointLocation(String control, Globe globe, Angle heading, LatLon center, double width, double height) {
    ...
    protected LatLon computeQuadEdgeMidpointLocation(String control, Globe globe, Angle heading, LatLon center, double width, double height) {
        ...
        if (control.equals(NORTH)) return LatLon.interpolate(this.pathType, 0.5, nw, ne); else if (control.equals(EAST)) return LatLon.interpolate(this.pathType, 0.5, ne, se); else if (control.equals(SOUTH)) return LatLon.interpolate(this.pathType, 0.5, sw, se); else if (control.equals(WEST)) return LatLon.interpolate(this.pathType, 0.5, sw, nw);
        return null;
    }
    ...
    protected Angle computeControlPointAzimuthInShapeCoordinates(String control, Angle azimuth) {
        if (control.equals(NORTHEAST)) return azimuth.subtract(this.shapeOrientation); else if (control.equals(SOUTHEAST)) return this.shapeOrientation.addDegrees(180).subtract(azimuth); else if (control.equals(SOUTHWEST)) return azimuth.subtract(this.shapeOrientation.addDegrees(180)); else if (control.equals(NORTHWEST)) return this.shapeOrientation.subtract(azimuth);
        return null;
    }
    ...
    protected Angle computeControlPointPathLength(String control, double width, double height, double globeRadius) {
        ...
        } else if (control.equals(NORTHEAST) || control.equals(SOUTHEAST) || control.equals(SOUTHWEST) || control.equals(NORTHWEST)) {
    ...
    protected void updateShapeControlPoints() {
        if (this.shapeCenterPosition != null) {
            if (this.controlPoints.size() < 1) addControlPoint(Position.ZERO, CONTROL_TYPE_REGULAR_SHAPE, CENTER);
        ...
        if (this.shapeRectangle != null) {
            ...
            for (Renderable r : this.controlPoints) {
                ...
                LatLon controlLocation = this.computeControlPointLocation(control, globe, this.shapeOrientation, this.shapeCenterPosition, this.shapeRectangle.getWidth(), this.shapeRectangle.getHeight());
                if (controlLocation == null) continue;
    ...
    protected void updateControlPointWithLeader(ControlPointWithLeader cp, LatLon controlLocation) {
        ...
        if (leaderControl == null) return;
        LatLon leaderBegin;
        if (this.measureShapeType.equals(SHAPE_QUAD) || this.measureShapeType.equals(SHAPE_SQUARE)) {
            leaderBegin = this.computeQuadEdgeMidpointLocation(leaderControl, globe, this.shapeOrientation, this.shapeCenterPosition, this.shapeRectangle.getWidth(), this.shapeRectangle.getHeight());
        } else {
            leaderBegin = this.computeControlPointLocation(leaderControl, globe, this.shapeOrientation, this.shapeCenterPosition, this.shapeRectangle.getWidth(), this.shapeRectangle.getHeight());
        }
        if (leaderBegin == null) return;
    ...
    protected void updateMeasureShape() {
        if (this.measureShapeType.equals(SHAPE_LINE) || this.measureShapeType.equals(SHAPE_PATH)) {
            ...
            if (this.positions.size() > 1 && this.line != null) this.line.setPositions(this.positions);
            if (this.surfaceShape != null) {
                this.shapeLayer.removeRenderable(this.surfaceShape);
                this.surfaceShape = null;
            }
            ...
            if (this.shapeCenterPosition != null && this.shapeRectangle != null && this.surfaceShape == null) {
                if (this.measureShapeType.equals(SHAPE_QUAD)) this.surfaceShape = new SurfaceQuad(this.shapeCenterPosition, this.shapeRectangle.width, this.shapeRectangle.height, this.shapeOrientation); else if (this.measureShapeType.equals(SHAPE_SQUARE)) this.surfaceShape = new SurfaceSquare(this.shapeCenterPosition, this.shapeRectangle.width); else if (this.measureShapeType.equals(SHAPE_ELLIPSE)) this.surfaceShape = new SurfaceEllipse(this.shapeCenterPosition, this.shapeRectangle.width / 2, this.shapeRectangle.height / 2, this.shapeOrientation, this.shapeIntervals); else if (this.measureShapeType.equals(SHAPE_CIRCLE)) this.surfaceShape = new SurfaceCircle(this.shapeCenterPosition, this.shapeRectangle.width / 2, this.shapeIntervals);
                ShapeAttributes attr = new BasicShapeAttributes();
                attr.setInteriorMaterial(new Material(this.getFillColor()));
                attr.setInteriorOpacity(this.getFillColor().getAlpha() / 255d);
                attr.setOutlineMaterial(new Material(this.getLineColor()));
            ...
            if (this.surfaceShape != null) {
                ...
                if (this.measureShapeType.equals(SHAPE_ELLIPSE) || this.measureShapeType.equals(SHAPE_CIRCLE)) {
                    ...
                    ((SurfaceEllipse) this.surfaceShape).setRadii(this.shapeRectangle.width / 2, this.shapeRectangle.height / 2);
    ...
    public void dispose() {
        ...
        if (this.applicationLayer != null) this.applicationLayer.removeRenderable(this.layer); else this.wwd.getModel().getLayers().remove(this.layer);
    ...
    protected static class ControlPointWithLeader extends ControlPoint implements PreRenderable {
        ...
        public void preRender(DrawContext dc) {
            ...
            if (this.leaderLine != null) this.leaderLine.preRender(dc);
        }
        ...
        public void render(DrawContext dc) {
            ...
            if (this.leaderLine != null) this.leaderLine.render(dc);
            super.render(dc);
        }
    ...
    protected void addControlPointWithLeader(Position position, String controlKey, Object control, String leaderKey, Object leader) {
        ControlPointWithLeader controlPoint = new ControlPointWithLeader(new Position(position, 0), this.controlPointWithLeaderAttributes, this.leaderAttributes, this);
    ...
    protected String formatCircleMeasurements(Position pos) {
        ...
        if (this.shapeRectangle != null) sb.append(this.unitsFormat.lengthNL(this.getLabel(RADIUS_LABEL), this.shapeRectangle.width / 2d));
        if (this.getCenterPosition() != null && areLocationsRedundant(this.getCenterPosition(), pos)) {
            sb.append(this.unitsFormat.angleNL(this.getLabel(CENTER_LATITUDE_LABEL), this.getCenterPosition().getLatitude()));
            sb.append(this.unitsFormat.angleNL(this.getLabel(CENTER_LONGITUDE_LABEL), this.getCenterPosition().getLongitude()));
        }
    ...
    protected String formatEllipseMeasurements(Position pos) {
        ...
        if (this.getOrientation() != null) sb.append(this.unitsFormat.angleNL(this.getLabel(HEADING_LABEL), this.getOrientation()));
        if (this.getCenterPosition() != null && areLocationsRedundant(this.getCenterPosition(), pos)) {
            sb.append(this.unitsFormat.angleNL(this.getLabel(CENTER_LATITUDE_LABEL), this.getCenterPosition().getLatitude()));
            sb.append(this.unitsFormat.angleNL(this.getLabel(CENTER_LONGITUDE_LABEL), this.getCenterPosition().getLongitude()));
        }
    ...
    protected String formatSquareMeasurements(Position pos) {
        ...
        sb.append(this.unitsFormat.areaNL(this.getLabel(AREA_LABEL), this.getArea()));
        sb.append(this.unitsFormat.lengthNL(this.getLabel(PERIMETER_LABEL), this.getLength()));
        if (this.shapeRectangle != null) sb.append(this.unitsFormat.lengthNL(this.getLabel(WIDTH_LABEL), this.shapeRectangle.width));
        if (this.getOrientation() != null) sb.append(this.unitsFormat.angleNL(this.getLabel(HEADING_LABEL), this.getOrientation()));
        if (this.getCenterPosition() != null && areLocationsRedundant(this.getCenterPosition(), pos)) {
            sb.append(this.unitsFormat.angleNL(this.getLabel(CENTER_LATITUDE_LABEL), this.getCenterPosition().getLatitude()));
            sb.append(this.unitsFormat.angleNL(this.getLabel(CENTER_LONGITUDE_LABEL), this.getCenterPosition().getLongitude()));
    ...
    protected String formatLineMeasurements(Position pos) {
        ...
        if (accumLength != null && accumLength >= 1 && !lengthsEssentiallyEqual(this.getLength(), accumLength)) sb.append(this.unitsFormat.lengthNL(this.getLabel(ACCUMULATED_LABEL), accumLength));
        if (this.getOrientation() != null) sb.append(this.unitsFormat.angleNL(this.getLabel(HEADING_LABEL), this.getOrientation()));
        sb.append(this.unitsFormat.angleNL(this.getLabel(LATITUDE_LABEL), pos.getLatitude()));
        sb.append(this.unitsFormat.angleNL(this.getLabel(LONGITUDE_LABEL), pos.getLongitude()));
        return sb.toString();
    ...
    protected Double computeAccumulatedLength(LatLon pos) {
        if (this.positions.size() <= 2) return null;
        ...
        for (int i = 1; i < this.positions.size(); i++) {
            ...
            if (length1 <= segmentLength && length2 <= segmentLength) {
                ...
                break;
            } else distanceFromStart += segmentLength;
            pos1 = pos2;
        }
    ...
    protected double computePathLength() {
        ...
        for (LatLon pos2 : this.positions) {
            if (pos1 != null) pathLengthRadians += LatLon.greatCircleDistance(pos1, pos2).radians;
            pos1 = pos2;
        }
    ...
    protected Angle computeAngleBetween(LatLon a, LatLon b, LatLon c) {
        Vec4 v0 = new Vec4(b.getLatitude().radians - a.getLatitude().radians, b.getLongitude().radians - a.getLongitude().radians, 0);
        Vec4 v1 = new Vec4(c.getLatitude().radians - b.getLatitude().radians, c.getLongitude().radians - b.getLongitude().radians, 0);
        return v0.angleBetween3(v1);
    }

    protected boolean lengthsEssentiallyEqual(Double l1, Double l2) {
        return Math.abs(l1 - l2) / l1 < 0.001;
    }
    ...
    protected boolean areLocationsRedundant(LatLon locA, LatLon locB) {
        ...
        if (!aLat.equals(bLat)) return false;
