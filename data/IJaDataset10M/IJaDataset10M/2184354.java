...
public class StyleTest extends BuildFileTest {
    ...
    public void testStyleIsSet() throws Exception {
        expectSpecificBuildException("testStyleIsSet", "no stylesheet specified", "specify the " + "stylesheet either as a filename in style " + "attribute or as a nested resource");
    }
    ...
    public void testDefaultMapper(String target) throws Exception {
        assertTrue(!(FileUtils.getFileUtils().resolveFile(getProject().getBaseDir(), "out/data.html")).exists());
        expectFileContains(target, "out/data.html", "set='myvalue'");
    }
    ...
    public void testDirectoryHierarchyWithDirMatching() throws Exception {
        ...
        assertTrue(FILE_UTILS.resolveFile(getProject().getBaseDir(), "out/dest/level1/data.html").exists());
    }

    public void testDirsWithSpaces() throws Exception {
        executeTarget("testDirsWithSpaces");
        assertTrue(FILE_UTILS.resolveFile(getProject().getBaseDir(), "out/d est/data.html").exists());
    }

    public void testWithStyleAttrAndResource() throws Exception {
        expectSpecificBuildException("testWithStyleAttrAndResource", "Must throws a BuildException", "specify the " + "stylesheet either as a filename in style " + "attribute or as a nested resource but not " + "as both");
    }
    ...
    private void expectFileContains(String target, String filename, String contains) throws IOException {
    ...
    private void assertFileContains(String filename, String contains) throws IOException {
        ...
        assertTrue("expecting file " + filename + " to contain " + contains + " but got " + content, content.indexOf(contains) > -1);
    }
}
