...
public class AlternateLocationCollection<T extends AlternateLocation> implements HTTPHeaderValue, Iterable<T> {
    ...
    private AlternateLocationCollection(URN sha1) {
        if (sha1 == null) throw new NullPointerException("null URN");
        if (!sha1.isSHA1()) throw new IllegalArgumentException("URN must be a SHA1");
        SHA1 = sha1;
    }
    ...
    public boolean add(T al) {
        ...
        if (!sha1.equals(SHA1)) throw new IllegalArgumentException("SHA1 does not match");
        synchronized (this) {
    ...
    public boolean remove(T al) {
        URN sha1 = al.getSHA1Urn();
        if (!sha1.equals(SHA1)) return false;
        synchronized (this) {
    ...
    public String httpStringValue() {
        ...
        if (wrote) writeBuffer.setLength(writeBuffer.length() - 2);
