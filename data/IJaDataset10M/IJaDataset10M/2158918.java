...
public class Util {
	...
	private static Class[] paramTypes = { ActivationID.class,
			MarshalledObject.class };

	/** name of the resource containing prohibited proxy interfaces */
	private static final String prohibitedProxyInterfacesResource = "com/sun/jini/proxy/resources/"
	...
	/**
	 * Appends the current thread's stack trace to the stack trace of the given
	 * exception.
	 * 
	 * This method is used for exceptions that have been unmarshalled as the
	 * exceptional result of a remote method invocation, so that the client-side
	 * stack trace gets recorded in the exception (while preserving the
	 * server-side stack trace in the exception as well).
	 * 
	 ...
	 * visually highlight the remote call boundary between the server-side and
	 * the client-side portions to readers of the combined stack trace.
	 */
	...
	private static void __________EXCEPTION_RECEIVED_FROM_SERVER__________(
		...
		StackTraceElement[] combinedTrace = new StackTraceElement[serverTrace.length
				+ clientTrace.length];
	...
	/**
	 * Clear the stack trace of the given exception by replacing it with an
	 * empty StackTraceElement array, and do the same for all of its chained
	 * causative exceptions.
	 * 
	 * This method is used when it is desired for the stack trace data of an
	 * exception to be suppressed before the exception gets marshalled to a
	 * remote virtual machine, perhaps for reasons of confidentiality or
	 * performance.
	 */
	 ...
	 * return values. For primitive types, the primitive type's class should be
	 * specified (i.e., for the type <code>int</code>, specify
	...
	/**
	 * Computes the "method hash" of a remote method, <code>m</code>. The method
	 * hash is a <code>long</code> containing the first 64 bits of the SHA
	 * digest from the UTF encoded string of the method name followed by its
	 * "method descriptor". See section 4.3.3 of The Java(TM) Virtual Machine
	 * Specification for the definition of a "method descriptor".
	 * 
	 * @param m
	 *            remote method
	...
	private static long computeMethodHash(Method m) {
		...
		try {
			...
			DataOutputStream out = new DataOutputStream(new DigestOutputStream(
					sink, md));
	...
	/**
	 * Returns the method hash for the method <code>m</code>. Subsequent calls
	 * to <code>getMethodHash</code> passing the same method argument should be
	 * faster since this method caches internally the result of the method to
	...
	private static class TableCache extends WeakHashMap {
		/**
		 * if true, the tables map methods to method hashes; if false, the
		 * tables map method hashes to methods
		 */
		...
		public Map getTable(Class remoteClass) {
			...
			/*
			 * Check cached reference to method table for this class; if it is
			 * null, go and create the table.
			 */
	 ...
	 * "method table" for a remote class, which maps method hashes to the
	 * appropriate Method objects for the class's remote methods.
	 */

	/**
	 * Returns a string consisting of the given method's name followed by its
	 * "method descriptor", as appropriate for use in the computation of the
	 * "method hash".
	 * 
	 * See section 4.3.3 of The Java(TM) Virtual Machine Specification for the
	...
	/**
	 * Returns the descriptor of a particular type, as appropriate for either a
	 * parameter type or return type in a method descriptor.
	 */
	 ...
	 * method with the same name and parameter types as m that does not declare
	 * RemoteException or a superclass in its throws clause, or if any
	 * superinterface of c has its name in prohibitedProxyInterfaces.
	 */
	public static void checkProxyRemoteMethod(Class c, Method m) {
		...
		if (map == null) {
			...
			synchronized (proxyRemoteMethodCache) {
				SoftReference ref = (SoftReference) proxyRemoteMethodCache
						.get(c);
				map = (ref == null) ? null : (WeakIdentityMap) ref.get();
				if (map == null) {
					map = new WeakIdentityMap();
	...
	private static Collection getProhibitedProxyInterfaces() {
		...
		try {
			enum = ClassLoader
					.getSystemResources(prohibitedProxyInterfacesResource);
		} catch (IOException e) {
		...
		while (enum.hasMoreElements()) {
			...
			try {
				...
				throw new ExceptionInInitializerError(new IOException(
						"problem reading " + url).initCause(e));
			}
		}
		return names;
	}
	...
	/**
	 * Throws IllegalArgumentException if any superinterface of c has its name
	 * in prohibitedProxyInterfaces.
	 */
	...
	/**
	 * Returns true either if both arguments are null or if an invocation of
	 * Object.equals on "subject" with "object" as the argument returns true;
	...
	/**
	 * Returns true either if both arguments are null or if both arguments refer
	 * to objects of the same class and an invocation of Object.equals on
	 * "subject" with "object" as the argument returns true; returns false
	 * otherwise.
	 * 
	 * This method is used to compare to possibly-null references for object
	 * equality when neither object's class is trusted, with the restriction
	...
	public static boolean sameClassAndEquals(Object subject, Object object) {
		return subject == null ? object == null : object != null
				&& subject.getClass() == object.getClass()
				&& subject.equals(object);
	}

	/**
	 * Returns true either if both arguments are null of if "subject" is an
	 * instance of TrustEquivalence and an invocation of
	 * TrustEquivalence.checkTrustEquivalence on "subject" with "object" as the
	...
	public static boolean checkTrustEquivalence(Object subject, Object object) {
		return subject == null ? object == null
				: subject instanceof TrustEquivalence
						&& ((TrustEquivalence) subject)
								.checkTrustEquivalence(object);
	}

	/**
	 * Returns true if proxy2 is a generated Proxy (proxy1 is assumed to be one)
	 * and the classes of both proxies implement the same ordered list of
	 * interfaces, and returns false otherwise.
	 */
	public static boolean sameProxyClass(Object proxy1, Object proxy2) {
		return (proxy1.getClass() == proxy2.getClass() || (Proxy
				.isProxyClass(proxy2.getClass()) && equalInterfaces(proxy1,
				proxy2)));
	}

	/**
	 * Returns true if the interfaces implemented by obj1's class are the same
	 * (and in the same order) as obj2's class.
	 */
	...
	private static class IntegrityEnforcementImpl implements
	...
	public static InetAddress getClientHost() throws ServerNotActiveException {
		ClientHost ch = (ClientHost) ServerContext
	 ...
	 *             if a security manager exists and invoking its
	 *             <code>checkPackageAccess</code> method with the package name
	 *             of the specified class throws a
