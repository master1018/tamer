...
public class BaseUtils {
    ...
    private static final Pattern WORD_BOUNDARY_IN_IDENT_PATTERN = Pattern.compile("(?<= [a-zA-Z])(?= [A-Z][a-z])" + "| (?<= [a-zA-Z])(?= [0-9])" + "| (?<= [0-9])(?= [a-zA-Z])", Pattern.COMMENTS);

    /** Pattern for consecutive underscores. */
    private static final Pattern CONSECUTIVE_UNDERSCORES_PATTERN = Pattern.compile("_ _ _*", Pattern.COMMENTS);
