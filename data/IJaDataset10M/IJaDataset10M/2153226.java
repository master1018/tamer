...
public class DefaultPageCreator implements PageCreator, Serializable {
    ...
    public Page createPage(final WebResponse webResponse, final WebWindow webWindow) throws IOException {
        final String contentType = determineContentType(webResponse.getContentType().toLowerCase(), webResponse.getContentAsStream());
        final Page newPage;
     ...
     * @exception IOException if an IO problem occurs
     */
    protected String determineContentType(final String contentType, final InputStream contentAsStream) throws IOException {
        ...
        try {
            ...
            if (asAsciiString.contains("<HTML")) {
                return "text/html";
            } else if (startsWith(bytes, markerUTF8) || startsWith(bytes, markerUTF16BE) || startsWith(bytes, markerUTF16LE)) {
                return "text/plain";
    ...
    protected String determinePageType(final String contentType) {
        if ("text/html".equals(contentType)) {
            ...
            return "javascript";
        } else if ("text/xml".equals(contentType) || "application/xml".equals(contentType) || "text/vnd.wap.wml".equals(contentType) || contentType.matches(".*\\+xml")) {
            return "xml";
