...
public class ResizingTable extends JTable {
    ...
    public void tableChanged(TableModelEvent event) {
        ...
        if (defaultRenderersByColumnClass != null) setColumnWidths();
    }
    ...
    private void getHeaderWidths(int[] width) {
        ...
        for (int column = 0; column < width.length; column++) {
            ...
            if (prefSize.width > width[column]) width[column] = prefSize.width;
        }
    }
    ...
    private void setCellWidths(int[] width, boolean override) {
        ...
        for (int column = 0; column < width.length; column++) {
            ...
            if (override) tableColumn.setPreferredWidth(newWidth); else {
    ...
    private void getCellMaxWidths(int[] width) {
        ...
        for (int row = 0; row < rowCount; row++) {
            for (int column = 0; column < width.length; column++) {
                ...
                if (renderer == null) renderer = getDefaultRenderer(getModel().getColumnClass(column));
                if (renderer == null) continue;
