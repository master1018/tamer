...
public class ConcatAction implements Action, CrawlListener {
    ...
    public boolean equals(Object object) {
        if (!(object instanceof ConcatAction)) return false;
    ...
    public void started(CrawlEvent event) {
        if (concat == null) {
            try {
                file = (filename != null) ? new File(filename) : Access.getAccess().makeTemporaryFile("concat", ".html");
                concat = new Concatenator(file.toString());
                if (prolog != null) concat.setProlog(prolog);
                if (header != null) concat.setPageHeader(header);
                if (footer != null) concat.setPageFooter(footer);
                if (divider != null) concat.setDivider(divider);
                if (epilog != null) concat.setEpilog(epilog);
            } catch (IOException e) {
