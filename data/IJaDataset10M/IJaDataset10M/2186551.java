...
public class VLM {
    ...
    public void addBroadcast(String name, String input, String output, String[] options, boolean enabled, boolean loop) {
        ...
        jvlc.getLibvlc().libvlc_vlm_add_broadcast(jvlc.getInstance(), name, input, output, options == null ? 0 : options.length, options, enabled ? 1 : 0, loop ? 1 : 0, exception);
    }

    public void addVod(String name, String input, String[] options, boolean enabled, String muxer) {
        libvlc_exception_t exception = new libvlc_exception_t();
        jvlc.getLibvlc().libvlc_vlm_add_vod(jvlc.getInstance(), name, input, options == null ? 0 : options.length, options, enabled ? 1 : 0, muxer, exception);
    }
    ...
    public void changeMedia(String name, String input, String output, String[] options, boolean enabled, boolean loop) {
        libvlc_exception_t exception = new libvlc_exception_t();
        jvlc.getLibvlc().libvlc_vlm_change_media(jvlc.getInstance(), name, input, output, options == null ? 0 : options.length, options, enabled ? 1 : 0, loop ? 1 : 0, exception);
    }
