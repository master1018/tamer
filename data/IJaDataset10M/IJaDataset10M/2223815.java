...
public class CallbackFuture<T> implements Future<T> {
    ...
    public synchronized void complete(T result) {
        if (isDone()) throw new IllegalStateException("JobFuture completion attempted after it was done.");
        this.result = result;
        latch.countDown();
        if (listener != null) result = listener.eval(this);
    }
    ...
    public T get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
        if (latch.await(timeout, unit)) return result; else return null;
    }
