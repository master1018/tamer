...
public class Forum {
    ...
    public static enum PostMode {
        ...
        NEW("newtopic", "f"), REPLY("reply", "t"), EDIT("editpost", "p"), DELETE("delete", "p");
    ...
    public static void post(GameSeries series, PostMode mode, int postId, String subject, String body, boolean disableHtml, boolean disableSmilies, boolean attachSignature, boolean notifyOnReply) throws Exception {
        Client.getAuthCookies();
        String url = URLs.forum.getPostUrl(series, mode, postId);
        Client.makeHeadRequest(url);
        String[] staticData = { "subject", subject, "message", body, "mode", mode.value, "p", String.valueOf(postId), "sid", Client.getPhpBb2MySqlSid(), "post", "Submit" };
