...
public class MichalewiczFunction implements Function, FunctionGradient, FunctionHessian {
    ...
    public double valueAt(Point point) {
        ...
        double sum = 0.0;
        for (int i = 0; i < n; i++) sum += Math.sin(ax[i]) * Math.pow(Math.sin((i + 1) * ax[i] * ax[i] / Math.PI), 2 * m);
        return -sum;
    }
    ...
    public Gradient gradientAt(Point point) {
        ...
        for (int i = 0; i < n; i++) {
            gradient[i] = -Math.cos(ax[i]) * Math.pow(Math.sin((i + 1) * ax[i] * ax[i] / Math.PI), 2 * m) - (1.0 / Math.PI) * (i + 1) * 4 * ax[i] * m * Math.cos((i + 1) * ax[i] * ax[i] / Math.PI) * Math.sin(ax[i]) * Math.pow(Math.sin((i + 1) * ax[i] * ax[i] / Math.PI), -1 + 2 * m);
        }
    ...
    public Hessian hessianAt(Point point) {
        ...
        for (int i = 0; i < n; i++) hessian[i][i] = (Math.pow(Math.sin((i + 1) * ax[i] * ax[i] / Math.PI), -2 + 2 * m) * (-(8 * (i + 1) * (i + 1) * ax[i] * ax[i] * m * (-1 + 2 * m) * Math.cos((i + 1) * ax[i] * ax[i] / Math.PI) * Math.cos((i + 1) * ax[i] * ax[i] / Math.PI) * Math.sin(ax[i]) - (i + 1) * 4 * ax[i] * m * Math.PI * Math.cos(ax[i]) * Math.sin(2 * (i + 1) * ax[i] * ax[i] / Math.PI) + Math.sin(ax[i]) * ((8 * (i + 1) * (i + 1) * ax[i] * ax[i] * m + Math.PI * Math.PI) * Math.sin((i + 1) * ax[i] * ax[i] / Math.PI) * Math.sin((i + 1) * ax[i] * ax[i] / Math.PI) - 2 * (i + 1) * m * Math.PI * Math.sin(2 * (i + 1) * ax[i] * ax[i] / Math.PI))))) / (Math.PI * Math.PI);
