...
public abstract class Buffer<O, I, E> {
    ...
    protected class Slot {
        ...
        protected void update(E object, Function<Object, ?> flush) {
            this.object = object;
            this.flush = flush;
            if (Buffer.this.capacityBytes != Integer.MAX_VALUE) this.sizeBytes = ((SizeAware) object).getMemSize();
        }
        ...
        protected void insert(O owner, I id, E object) {
            ...
            this.object = object;
            if (Buffer.this.capacityBytes != Integer.MAX_VALUE) this.sizeBytes = ((SizeAware) object).getMemSize();
            if ((members = owners.get(owner)) == null) owners.put(owner, members = new HashMap<I, Slot>());
            members.put(id, this);
            size++;
        }
        ...
        protected void remove() {
            if (index < size) {
                ...
                if (members.containsKey(id)) {
                    members.remove(id);
                    if (members.isEmpty()) owners.remove(owner);
                    members = null;
                }
                owner = null;
                if (isFixed()) fixedSlots--;
    ...
    public Buffer(int capacity, int capacityBytes) {
        ...
        for (int i = 0; i < capacity; i++) slots.add(newSlot(i));
    }
    ...
    protected final void handleSizeOverflow() {
        while (sizeBytes > capacityBytes) {
            if (fixedSlots == size()) throw new IllegalStateException("Buffer overflow. Too many slots fixed.");
            Slot vic = victim();
    ...
    protected Slot fix(O owner, I id, Function<? super I, ? extends E> obtain) throws IllegalStateException {
        ...
        if (slot == null) {
            if (fixedSlots == slots.size()) throw new IllegalStateException("Buffer overflow. Too many slots fixed.");
            if (size() == slots.size()) {
                Slot vic = victim();
                sizeBytes -= vic.sizeBytes;
                vic.displace();
    ...
    public void flushAll(O owner) {
        Map<I, Slot> members = owners.get(owner);
        if (members != null) for (Slot slot : members.values()) slot.flush();
    }
    ...
    public void removeAll(O owner) {
        Map<I, Slot> members = owners.get(owner);
        if (members != null) for (Iterator<Entry<I, Slot>> entries = members.entrySet().iterator(); entries.hasNext(); ) {
    ...
    public void checkBuffer() {
        int currentSize = 0;
        for (Map<I, Slot> map : owners.values()) for (Slot slot : map.values()) currentSize += slot.sizeBytes;
        if (currentSize != this.sizeBytes) throw new RuntimeException("The size was not counted correctly (" + currentSize + " instead " + this.sizeBytes + ")");
    }
