...
class BSHMethodDeclaration extends SimpleNode {
	...
	*/
    synchronized void insureNodesParsed() {
        ...
        if (firstNode instanceof BSHReturnType) {
            ...
            if (jjtGetNumChildren() > 2 + numThrows) blockNode = (BSHBlock) jjtGetChild(2 + numThrows);
            ++firstThrowsClause;
        } else {
	...
	*/
    Class evalReturnType(CallStack callstack, Interpreter interpreter) throws EvalError {
        insureNodesParsed();
        if (returnTypeNode != null) return returnTypeNode.evalReturnType(callstack, interpreter); else return null;
    }

    String getReturnTypeDescriptor(CallStack callstack, Interpreter interpreter, String defaultPackage) {
        insureNodesParsed();
        if (returnTypeNode == null) return null; else return returnTypeNode.getTypeDescriptor(callstack, interpreter, defaultPackage);
    }
	...
	*/
    ...
    private void evalNodes(CallStack callstack, Interpreter interpreter) throws EvalError {
        ...
        if (interpreter.getStrictJava()) {
            for (int i = 0; i < paramsNode.paramTypes.length; i++) if (paramsNode.paramTypes[i] == null) throw new EvalError("(Strict Java Mode) Undeclared argument type, parameter: " + paramsNode.getParamNames()[i] + " in method: " + name, this, null);
            if (returnType == null) throw new EvalError("(Strict Java Mode) Undeclared return type for method: " + name, this, null);
        }
    }
