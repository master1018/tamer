...
public class JGObject {
	...
	* playfield.*/
    ...
    private void initObject(JGEngineInterface engine, String name, int collisionid) {
        ...
        if ((int) expiry == suspend_off_view || (int) expiry == suspend_off_view_expire_off_pf) {
            if (!isInView(eng.getOffscreenMarginX(), eng.getOffscreenMarginY())) suspend();
        }
	...
	*/
    public JGRectangle getBBox() {
        if (bbox != null) return new JGRectangle(bbox.x + (int) x, bbox.y + (int) y, bbox.width, bbox.height);
        updateImageBBox();
        if (imgbbox != null) {
            return new JGRectangle(imgbbox.x + (int) x, imgbbox.y + (int) y, imgbbox.width, imgbbox.height);
        }
        return null;
    }
	...
	* @return  copy of bbox in pixel coordinates, null if no bbox */
    public JGRectangle getTileBBox() {
        ...
        return new JGRectangle((int) x + tilebbox.x, (int) y + tilebbox.y, tilebbox.width, tilebbox.height);
    }
	...
	* @return tile index coordinates, null if no bbox */
    public JGRectangle getTiles() {
        ...
        if (eng.getTiles(tiles_copy, temp_bbox_copy)) return tiles_copy;
        return null;
    }
    ...
    private JGRectangle getCenterTiles(JGRectangle bbox, JGRectangle tiles_copy) {
        int px, py;
        if (bbox.x >= 0) {
            px = ((bbox.x + (bbox.width / 2)) / tilewidth) * tilewidth + tilewidth / 2;
            bbox.x = tilewidth * ((px - (bbox.width / 2)) / tilewidth);
        } else {
            px = ((bbox.x + (bbox.width / 2) - tilewidth + 1) / tilewidth) * tilewidth + tilewidth / 2;
            bbox.x = tilewidth * ((px - (bbox.width / 2) - tilewidth + 1) / tilewidth);
        }
        if (bbox.y >= 0) {
            py = ((bbox.y + (bbox.height / 2)) / tileheight) * tileheight + tileheight / 2;
            bbox.y = tileheight * ((py - (bbox.height / 2)) / tileheight);
        } else {
            py = ((bbox.y + (bbox.height / 2) - tileheight + 1) / tileheight) * tileheight + tileheight / 2;
	...
	*/
    public boolean isInView(int marginx, int marginy) {
        ...
        if (temp_bbox_copy.x + temp_bbox_copy.width < viewxofs - marginx) return false;
        if (temp_bbox_copy.y + temp_bbox_copy.height < viewyofs - marginy) return false;
        if (temp_bbox_copy.x > viewxofs + viewwidth + marginx) return false;
        if (temp_bbox_copy.y > viewyofs + viewheight + marginy) return false;
        return true;
    }
