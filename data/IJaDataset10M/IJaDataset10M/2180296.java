...
public class ChordNamingConvention {
    ...
    public String createChordName(int chordTonic, int chordIndex, int alteration, int plusMinus, boolean add, int add5, int add9, int add11, int bassTonic, boolean sharp) {
        ...
        if (!chordName.equals("M")) sb.append(chordName);
        if (add) sb.append("add");
        if (alteration != 0) {
            char lastChar = sb.toString().charAt(sb.toString().length() - 1);
            if (lastChar >= '0' && lastChar <= '9') sb.append("/");
            switch(alteration) {
                case 1:
        ...
        if (add5 != 0) sb.append("/").append(getAdd("5", add5));
        if (add9 != 0) sb.append("/").append(getAdd("9", add9));
        if (add11 != 0) sb.append("/").append(getAdd("11", add11));
