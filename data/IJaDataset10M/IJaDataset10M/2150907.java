...
public class OverlayManager extends BranchGroup implements ComponentListener {
    ...
    public OverlayManager(Canvas3D canvas, UpdateManager updateManager) {
        ...
        } else this.updateManager = updateManager;
    ...
    public void addOverlay(Overlay overlay) {
        ...
        if (overlay instanceof InteractiveOverlay) {
            if (!awtBehavior.getEnable()) awtBehavior.setEnable(true);
    ...
    public void removeOverlay(Overlay overlay) {
        if (overlays.remove(overlay)) {
            if (overlay instanceof InteractiveOverlay) ((InteractiveOverlay) overlay).setInputRequester(null);
            BranchGroup bg = overlay.getRoot();
            bg.detach();
            if (overlays.size() == 0) awtBehavior.setEnable(false);
        }
    }
    ...
    private void calcImagePlate() {
        ...
        if (canvasDim.width != 0) {
            ...
            Vector3d loc = new Vector3d(-c_width / 2, -c_height / 2, -CONSOLE_Z);
