...
public class ShortStockDataPoint implements StockDataPoint, Serializable {
    ...
    public ShortStockDataPoint(Date date, float adjustedOpen, float adjustedHigh, float adjustedLow, float adjustedClose, float volume) throws IllegalArgumentException {
    ...
    /** Create StockDataPoint with given data.
      @throws IllegalArgumentException if any price &gt; 327.67,
      or if round(price*100f)/100f != price. **/
    public ShortStockDataPoint(Date date, float adjustedOpen, float adjustedHigh, float adjustedLow, float adjustedClose, float volume, float dividend, boolean isDividendDate, boolean isSplitDate) throws IllegalArgumentException {
    ...
    private short toCentsShort(float price) {
        ...
        if (centsPrice > Short.MAX_VALUE) throw new IllegalArgumentException(price + " > " + Short.MAX_VALUE + "cents"); else if (centsPrice < Short.MIN_VALUE) throw new IllegalArgumentException(price + " < " + Short.MIN_VALUE + "cents"); else if (centsPrice / 100f != price) throw new IllegalArgumentException(price + " != " + centsPrice + "cents"); else return (short) centsPrice;
    }
    ...
    public String toString() {
        return (DATE_FORMAT.format(this.date) + "[" + DOLLAR_FORMAT.format(this.adjustedOpen) + "(" + DOLLAR_FORMAT.format(this.adjustedHigh) + "," + DOLLAR_FORMAT.format(this.adjustedLow) + ")" + DOLLAR_FORMAT.format(this.adjustedClose) + "]" + "#" + this.volume);
    }
