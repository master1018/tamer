...
public class DOWebFxTree extends DOBaseMenu {
    ...
    public String drawStaticTree(DOMenuModel menuModel) {
        ...
        if (menuModel.getMenuType() != null && menuModel.getMenuType().intValue() == DOMenuModel.MENUTYPE_NAVIGATION && DOGlobals.getInstance().getSessoinContext().getSysTreeRoot() != null) {
            sysTreeRoot = DOGlobals.getInstance().getSessoinContext().getSysTreeRoot();
        }
        StringBuffer buffer = new StringBuffer();
        buffer.append("<div class=tree id='").append(menuModel.getObjUid()).append("'></div> \n <script language='javascript'>webFXTreeHandler.resetContext();");
        buffer.append("var tree11 = new WebFXTree('").append(sysTreeRoot).append("');\n");
        ...
        buffer.append("$('#").append(menuModel.getObjUid()).append("').append(tree11.toHtml());");
	 ...
	 * @param us
	 */
    private void drawStaticTreeHelp(DOMenuModel item, StringBuffer buffer, String parentItemName) {
        String itemName = item.getName();
        itemName = StringUtil.get_Name(itemName);
        buffer.append("var ").append(itemName).append(" = new WebFXTreeItem('").append(item.getL10n()).append("'");
