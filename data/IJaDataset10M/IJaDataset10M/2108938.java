...
public abstract class Assert {
	 ...
	 * <pre class="code">Assert.isFalse(i &gt; 0);</pre>
	 * @param expression a boolean expression
    ...
    public static void doesNotContain(String textToSearch, String substring, String message) {
        if (strHasLength(textToSearch) && strHasLength(substring) && textToSearch.indexOf(substring) != -1) {
            throw new IllegalArgumentException(message);
        }
    }
    ...
    public static void state(boolean expression) {
        state(expression, "[Assertion failed] - this nation invariant must be true");
    }

    private static boolean strHasLength(String str) {
        return (str != null && str.length() > 0);
    }

    private static boolean strHasText(String str) {
        if (!strHasLength(str)) {
            return false;
        }
