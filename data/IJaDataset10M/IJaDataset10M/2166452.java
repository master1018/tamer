...
public final class HttpsEndpoint implements Endpoint, Serializable, TrustEquivalence {
    ...
    /**
	 * @serialField serverHost String The name of the server host.
	 * @serialField port int The server port.
	 * @serialField socketFactory SocketFactory The socket factory for creating
	 *              sockets, or <code>null</code> to use default sockets.
	 */
    private static final ObjectStreamField[] serialPersistentFields = { new ObjectStreamField("serverHost", String.class), new ObjectStreamField("port", int.class), new ObjectStreamField("socketFactory", SocketFactory.class) };

    /**
	 * Maps HttpsEndpointImpls to EndpointInfo instances, each of which contains
	 * a representative endpoint equal to the key and a list of idle
	 * connections.
	 */
    ...
    static final Executor systemExecutor = (Executor) AccessController.doPrivileged(new GetThreadPoolAction(false));
	 ...
	 *             if <code>port</code> is less than or equal to <code>0</code>,
	 *             or greater than <code>65535</code>
	 * @throws NullPointerException
	 *             if <code>serverHost</code> is <code>null</code>
	 */
    ...
    /**
	 * Returns an HTTPS endpoint for the specified server host, port, and socket
	 * factory. A <code>socketFactory</code> of <code>null</code> uses default
	 * sockets.
	 * 
	 * @param serverHost
	 *            the name of the server host
	 ...
	 *            the server port
	 * @param socketFactory
	 *            the socket factory, or <code>null</code>
	 * @return an <code>HttpsEndpoint</code> instance
	 * @throws IllegalArgumentException
	 *             if <code>port</code> is less than or equal to <code>0</code>,
	 *             or greater than <code>65535</code>
	 ...
	 * {@link Socket} instances, or <code>null</code> if it uses default
	 * sockets.
	 * 
    ...
    public boolean equals(Object object) {
        if (this == object) {
            ...
            return object instanceof HttpsEndpoint && impl.equals(((HttpsEndpoint) object).impl);
        }
    }
	 ...
	 * 
	 * <p>
	 * The returned <code>OutboundRequestIterator</code>'s
	 * {@link OutboundRequestIterator#next next} method behaves as follows:
	 * 
	 * <blockquote>
	 * 
	 ...
	 * <code>createSocket</code> methods on the <code>SocketFactory</code> of
	 * this <code>HttpsEndpoint</code> (which produced this iterator) if non-
	 * <code>null</code>, or it will create a <code>Socket</code> directly
	 * otherwise.
	 * 
	 * <p>
	 * When the implementation needs to connect a <code>Socket</code>, if the
	 * host name to connect to (if an HTTP proxy is to be used for the
	 * communication, the proxy's host name; otherwise, this
	 * <code>HttpsEndpoint</code>'s host name) resolves to multiple addresses
	 * (according to {@link InetAddress#getAllByName InetAddress.getAllByName}),
	 * it attempts to connect to the first resolved address; if that attempt
	 * fails with an <code>IOException</code> or (as is possible in the case
	 * that an HTTP proxy is not to be used) a <code>SecurityException</code>,
	 * it then attempts to connect to the next address; and this iteration
	 * continues as long as there is another resolved address and the attempt to
	 * connect to the previous address fails with an <code>IOException</code> or
	 * a <code>SecurityException</code>. If the host name resolves to just one
	 * address, the implementation makes one attempt to connect to that address.
	 * If the host name does not resolve to any addresses (
	 * <code>InetAddress.getAllByName</code> would throw an
	 * <code>UnknownHostException</code>), the implementation still makes an
	 * attempt to connect the <code>Socket</code> to that host name, which could
	 * result in an <code>UnknownHostException</code>. If the final connection
	 * attempt fails with an <code>IOException</code> or a
	 * <code>SecurityException</code>, then if any connection attempt failed
	 * with an <code>IOException</code>, this method throws an
	 * <code>IOException</code>, and otherwise (if all connection attempts
	 * failed with a <code>SecurityException</code>), this method throws a
	 * <code>SecurityException</code>.
	 * 
	 * <p>
	 * If there is a security manager and an HTTP proxy is to be used for the
	 * communication, the security manager's
	 * {@link SecurityManager#checkConnect(String,int) checkConnect} method is
	 * invoked with this <code>HttpsEndpoint</code>'s host and port; if this
	 * results in a <code>SecurityException</code>, this method throws that
	 * exception.
	 * 
	 * <p>
	 * If there is a security manager and an HTTP proxy is not to be used for
	 * the communication:
	 * 
	 * <ul>
	 * 
	 * <li>If a new connection is to be created, the security manager's
	 * {@link SecurityManager#checkConnect(String,int) checkConnect} method is
	 * invoked with this <code>HttpsEndpoint</code>'s host and <code>-1</code>
	 * for the port; if this results in a <code>SecurityException</code>, this
	 * method throws that exception. <code>checkConnect</code> is also invoked
	 * for each connection attempt, with the remote IP address (or the host
	 * name, if it could not be resolved) and port to connect to; this could
	 * result in a <code>SecurityException</code> for that attempt. (Note that
	 * the implementation may carry out these security checks indirectly, such
	 * as through invocations of <code>InetAddress.getAllByName</code> or
	 * <code>Socket</code>'s constructors or <code>connect</code> method.)
	 * 
	 * <li>
	 * <p>
	 * In order to reuse an existing connection for the communication, the
	 * current security context must have all of the permissions that would be
	 * necessary if the connection were being created. Specifically, it must be
	 * possible to invoke <code>checkConnect</code> in the current security
	 * context with this <code>HttpsEndpoint</code>'s host and <code>-1</code>
	 * for the port without resulting in a <code>SecurityException</code>, and
	 * it also must be possible to invoke <code>checkConnect</code> with the
	 * remote IP address and port of the <code>Socket</code> without resulting
	 * in a <code>SecurityException</code> (if the remote socket address is
	 * unresolved, its host name is used instead). If no existing connection
	 * satisfies these requirements, then this method must behave as if there
	 ...
	 * or there are unsupported or conflicting constraints.
	 * 
	 * <p>
	 * Throws {@link SecurityException} if there is a security manager and an
	 * invocation of its <code>checkConnect</code> method fails, or if the
	 ...
	 * <code>HttpsEndpoint</code> with the same values for server host and port;
    ...
    public boolean checkTrustEquivalence(Object obj) {
        if (this == obj) {
            ...
            return obj instanceof HttpsEndpoint && impl.equals(((HttpsEndpoint) obj).impl);
        }
    }
	 ...
	 * <code>socketFactory</code> fields, checks that <code>serverHost</code> is
	 * not <code>null</code> and <code>port</code> is in range, and sets
	 * transient fields.
	 * 
	 * @throws InvalidObjectException
	 *             if <code>serverHost</code> is <code>null</code> or
	 *             <code>port</code> is out of range
	 */
    ...
    private static final class HttpsEndpointImpl extends SslEndpointImpl {
        ...
        /**
		 * Ping the server endpoint to test the connection. Throw (or pass) an
		 * IOException if the server endpoint doesn't respond. If the ping
		 * succeeds, the connection will have been returned to the idle pool.
		 */
    ...
    private static final class HttpsConnection extends SslConnection implements HttpClientSocketFactory {
		 ...
		 * 
		 * @throws SSLException
		 *             if the requested suites cannot be supported
		 * @throws IOException
        ...
        void establishNewSocket() throws IOException {
            if (proxyHost.length() == 0) {
                ...
                try {
                    httpClient = (HttpClient) Security.doPrivileged(new PrivilegedExceptionAction() {

                        public Object run() throws IOException {
                            return new HttpClient(serverHost, port, proxyHost, proxyPort, HttpsConnection.this);
                        }
                    });
        ...
        OutboundRequest newRequest(CallContext callContext) throws IOException {
            OutboundRequest request = new HttpsOutboundRequest(httpClient.newRequest(), this, callContext);
            idleTime = 0;
            return request;
        }
		 ...
		 * milliseconds before now. If the recorded idle time is zero, sets the
		 * recorded idle time to now.
		 */
        ...
        /**
		 * Creates a plain socket to use to talk to the proxy host, else creates
		 * an SSL socket for a direct connection to the server.
		 */
        public Socket createSocket(String host, int port) throws IOException {
            Socket s = createPlainSocket(host, port);
            if (proxyHost.length() == 0) {
                s = setSSLSocket((SSLSocket) sslSocketFactory.createSocket(s, host, port, true));
            }
            return s;
        }

        /**
		 * Creates an SSL socket on top of the one the HTTP code used to connect
		 * through the proxy.
		 */
        public Socket createTunnelSocket(Socket s) throws IOException {
            return setSSLSocket((SSLSocket) sslSocketFactory.createSocket(s, s.getInetAddress().getHostName(), s.getPort(), true));
        }

        /**
		 * Stores the new socket in the sslSocket field, does a handshake on it,
		 * and returns it.
		 */
    ...
    private static final class EndpointInfo {
        ...
        HttpsConnection connect(CallContext context) throws IOException {
            ...
            synchronized (this) {
                ...
                result = (HttpsConnection) endpoint.connect(context, Collections.EMPTY_SET, idle);
                if (result != null) {
                    idle.remove(result);
                } else {
                    result = new HttpsConnection(endpoint, context, endpoint.serverHost, endpoint.port, phost, pport, endpoint.socketFactory);
        ...
        synchronized void noteIdle(HttpsConnection connection) {
            inUse.remove(connection);
            if (proxyHost.equals(connection.proxyHost) && proxyPort == connection.proxyPort) {
                idle.add(connection);
            } else {
                try {
                    connection.close();
    ...
    private static final class Reaper implements Runnable {
		 ...
		 * connection, and, if that returns true, remove the connection from the
		 * list of idle connections. Then shutdown all of the idle connections
		 * that have been collected. Terminate if no connections remain, else
        ...
        public void run() {
            ...
            do {
                ...
                long now = System.currentTimeMillis();
                synchronized (endpointMap) {
                    for (Iterator iter = endpointMap.values().iterator(); iter.hasNext(); ) {
