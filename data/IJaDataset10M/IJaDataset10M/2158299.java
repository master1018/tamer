 ...
 * Quartz user forum or the developer's mail list at <a
 * href="http://www.sourceforge.net/projects/quartz">source forge</a>.
 * </p>
 * 
 * @see CronTrigger
 * @see SimpleTrigger
...
public class TriggerUtils {
    ...
    private static void validateDayOfMonth(int day) {
        if ((day < 1 || day > 31) && day != LAST_DAY_OF_MONTH) throw new IllegalArgumentException("Invalid day of month.");
    }
    ...
    private static void validateYear(int year) {
        if (year < 1970 || year > 2099) throw new IllegalArgumentException("Invalid year (must be >= 1970 and <= 2099.");
    }
     ...
     * 
     * <p>
     * The generated trigger will not have its name, group, or end-time set. The
     ...
     * 
     * <p>
     * The generated trigger will not have its group or end-time set. The Start
     ...
     * firing will not occur that month. (i.e. if dayOfMonth is specified as 31,
     * no firing will occur in the months of the year with fewer than 31 days).
     * </p>
     * 
     * @param dayOfMonth
    ...
    public static Trigger makeMonthlyTrigger(int dayOfMonth, int hour, int minute) {
        ...
        CronTrigger trig = new CronTrigger();
        try {
            if (dayOfMonth != LAST_DAY_OF_MONTH) trig.setCronExpression("0 " + minute + " " + hour + " " + dayOfMonth + " * ?"); else trig.setCronExpression("0 " + minute + " " + hour + " L * ?");
        } catch (Exception ignore) {
            return null;
        }
        trig.setStartTime(new Date());
     ...
     * 
     * @param repeatCount
     *                the number of times to fire the trigger
     * @param repeatInterval
     *                the number of milliseconds to wait between fires
     * @return the newly created trigger
     */
     ...
     * 
     * @param date
     *                the Date to round, if <code>null</code> the current time
     *                will be used
     ...
     * with the time of 08:20:00. But a date with the time 08:53:31 and an input
     * minute-base of 45 would result in 09:00:00, because the even-hour is the
     * next 'base' for 45-minute intervals.
     * </p>
     * 
     * <p>
     * More examples: <table>
     ...
     *                The value (0-23) to give the hours field of the date
     * @param dayOfMonth
     *                The value (1-31) to give the day of month field of the
     *                date
     * @param month
     ...
     * <code>Trigger</code>. The input trigger will be cloned before any work
     * is done, so you need not worry about its state being altered by this
     * method.
     * 
     * @param trigg
     ...
     */
    public static List<Date> computeFireTimes(Trigger trigg, purej.job.Calendar cal, int numTimes) {
     ...
     * trigger will be cloned before any work is done, so you need not worry
     * about its state being altered by this method.
     * 
     * <p>
     * NOTE: if this is a trigger that has previously fired within the given
     * date range, then firings which have already occured will not be listed in
     * the output List.
     * </p>
     * 
     * @param trigg
     ...
     * @return List of java.util.Date objects
     */
    public static List<Date> computeFireTimesBetween(Trigger trigg, purej.job.Calendar cal, Date from, Date to) {
        LinkedList<Date> lst = new LinkedList<Date>();
        Trigger t = (Trigger) trigg.clone();
        if (t.getNextFireTime() == null) {
            t.setStartTime(from);
    ...
    /**
     * Translate a date & time from a users timezone to the another (probably
     * server) timezone to assist in creating a simple trigger with the right
     * date & time.
     * 
     * @param date
    ...
    /**
     * Gets the offset from UT for the given date in the given timezone, taking
     ...
     * 
     * @param date
     *                the date (in milliseconds) that is the base for the offset
     * @param tz
