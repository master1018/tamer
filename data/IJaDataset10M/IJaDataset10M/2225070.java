...
public class Preprocessor {
    ...
    private static final String RULES_KEY_EXT = "_rules";

    private static final String VOCAB_KEY_EXT = "_vocab";
	 ...
	 */
    public static synchronized Preprocessor getInstance(int type, String configPath) {
        if (config == null) {
	 ...
	 * @return
	 */
    public static synchronized Preprocessor getInstance(String vocabFile, String rulesFile) {
	 ...
	 * @return
	 */
    public static synchronized Preprocessor getInstance(int type, String vocabFile, String rulesFile) {
    ...
    public Preprocessor(String rulesFile, String vocabFile) {
        try {
            ...
            System.err.println("problem reading " + vocabFile + " and/or " + rulesFile);
        }
    }
	 ...
	 * @return The Preprocessor.
	 */
    public static synchronized Preprocessor getInstance(int type, InputStream configStream) {
        if (config == null) {
            try {
	 ...
	 * @return
	 */
    public static synchronized Preprocessor getInstance(int type, InputStream configStream, Object context) {
        if (config == null) {
    ...
    private static synchronized Preprocessor setUpInstances(int type, String vocabFile, String rulesFile) {
        if (preprocessors == null) {
            preprocessors = new HashMap<Integer, Preprocessor>(0);
        }
        if (preprocessors.containsKey(type)) {
            ...
            Preprocessor preprocessor = new Preprocessor(type, vocabFile, rulesFile);
    ...
    private Preprocessor(int type) {
        String vocabFile = Preprocessor.getConfigString(String.valueOf(type) + VOCAB_KEY_EXT + _langId);
        String rulesFile = Preprocessor.getConfigString(String.valueOf(type) + RULES_KEY_EXT + _langId);
        if (context == null) {
            try {
                rules = readVocabData(rulesFile);
                vocab = readVocabData(vocabFile);
            } catch (FileNotFoundException fnfe) {
                vocabFile = Preprocessor.getConfigString(String.valueOf(type) + VOCAB_KEY_EXT);
                rulesFile = Preprocessor.getConfigString(String.valueOf(type) + RULES_KEY_EXT);
                try {
                    rules = readVocabData(rulesFile);
                    vocab = readVocabData(vocabFile);
                } catch (Exception e) {
                    System.err.println(" problem openening file " + vocabFile + " and/or " + rulesFile);
                }
            } catch (Exception e) {
                System.err.println(" problem openening file " + vocabFile + " and/or " + rulesFile);
            }
            ...
            if (rulesFile != null && AndroidHelper.isUsable) {
                InputStream is = AndroidHelper.getRessourceInputStream(context, rulesFile);
                try {
                    rules = readVocabData(is);
                } catch (Exception e) {
                    try {
                        rulesFile = Preprocessor.getConfigString(String.valueOf(type) + RULES_KEY_EXT);
                        is = AndroidHelper.getRessourceInputStream(context, rulesFile);
                        rules = readVocabData(is);
                    } catch (Exception e2) {
                        e.printStackTrace();
            ...
            if (vocabFile != null && AndroidHelper.isUsable) {
                InputStream is = AndroidHelper.getRessourceInputStream(context, vocabFile);
                try {
                    vocab = readVocabData(is);
                } catch (Exception e) {
                    try {
                        vocabFile = Preprocessor.getConfigString(String.valueOf(type) + VOCAB_KEY_EXT);
                        is = AndroidHelper.getRessourceInputStream(context, vocabFile);
                        vocab = readVocabData(is);
                    } catch (Exception e2) {
                        e.printStackTrace();
    ...
    private Preprocessor(int type, String vocFile, String rulFile) {
        ...
        if (vocFile != null) vocabFile = vocFile + _langId;
        String rulesFile = rulFile + _langId;
        try {
            rules = readVocabData(rulesFile);
            if (vocFile != null) vocab = readVocabData(vocabFile);
        } catch (FileNotFoundException fnfe) {
            if (vocFile != null) vocabFile = Preprocessor.getConfigString(String.valueOf(type) + VOCAB_KEY_EXT);
            rulesFile = Preprocessor.getConfigString(String.valueOf(type) + RULES_KEY_EXT);
            try {
                rules = readVocabData(rulesFile);
                if (vocFile != null) vocab = readVocabData(vocabFile);
    ...
    private static synchronized void loadConfig(String configPath) {
        config = new KeyValues(new File(configPath), "=", FileUtil.ENCODING_UTF_8);
    }

    private static synchronized void loadConfig(InputStream configStream) throws Exception {
        config = new KeyValues(configStream, "=", FileUtil.ENCODING_UTF_8);
    }

    private KeyValue[] readVocabData(String filename) throws Exception {
        KeyValues keyValues = new KeyValues(new File(filename), "=", FileUtil.ENCODING_UTF_8);
        return keyValues.getKeyValues();
    }

    private KeyValue[] readVocabData(InputStream stream) throws Exception {
        KeyValues keyValues = new KeyValues(stream, "=", FileUtil.ENCODING_UTF_8);
        return keyValues.getKeyValues();
    }
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            ...
            Preprocessor test = Preprocessor.getInstance(1, "res/raw/preprocessor.properties");
            ...
            try {
                ...
                for (Iterator<String> iterator = fileLines.iterator(); iterator.hasNext(); ) {
