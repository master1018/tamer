...
import net.sf.jabref.custom.export.FieldFormatter;
...
public class BibtexEntry {
    ...
    public void setType(BibtexEntryType type) {
        ...
        try {
            _type = type;
            firePropertyChangedEvent(GUIGlobals.TYPE_HEADER, oldType != null ? oldType.getName() : null, type.getName());
        } catch (Exception pve) {
            pve.printStackTrace();
        }
    }
    ...
    public String getCiteKey() {
        return (_fields.containsKey(BibtexFields.KEY_FIELD) ? _fields.get(BibtexFields.KEY_FIELD) : null);
    }
    ...
    private void firePropertyChangedEvent(String fieldName, Object oldValue, Object newValue) throws PropertyVetoException {
        _changeSupport.fireVetoableChange(new PropertyChangeEvent(this, fieldName, oldValue, newValue));
    }
    ...
    public void write(Writer out, FieldFormatter ff, boolean write) throws IOException {
        ...
        for (String field : remainingFields) hasWritten = hasWritten | writeField(field, out, ff, hasWritten);
    ...
    private boolean writeField(String name, Writer out, FieldFormatter ff, boolean isFirst) throws IOException {
        ...
        if (o != null) {
            if (isFirst) out.write("," + Globals.NEWLINE);
            out.write("  " + name + " = ");
            try {
    ...
    public String getAuthorTitleYear(int maxCharacters) {
        ...
        for (int i = 0; i < s.length; ++i) if (s[i] == null) s[i] = "N/A";
