...
public class RubyAutoEditStrategy implements ILinkedModeEditStrategy {
    ...
    public RubyAutoEditStrategy(String partition, ISourceViewer viewer, RubyCompletionProcessor rhtmlCp) {
        fViewer = viewer;
        fRubyCp = rhtmlCp;
    }
        ...
        if (command.text.length() == 1) {
            if (command.text.charAt(0) == '\t') {
                try {
                    ...
                    if (prefix.length() > 0) {
                        Template[] templates = fRubyCp.getTemplates("ruby");
                        for (int i = 0; i < templates.length; i++) {
                            Template template = templates[i];
                            if (template.getName().equals(prefix)) {
                                final int offset = command.offset - prefix.length();
                                TemplateContextType contextType = fRubyCp.getContextType(fViewer, region);
                                ...
                                int end = Math.max(getReplaceEndOffset(context, region), offset);
                                String templateString = templateBuffer.getString();
                                command.text = templateString;
                                command.offset = start;
                                command.length = end - start;
                                fPositionList.add(new LinkedPosition(document, offset + templateString.length(), 0));
                                ...
                                for (int z = 0; z != variables.length; z++) {
                                    ...
                                    for (int j = 0; j != offsets.length; j++) fPositionList.add(new LinkedPosition(document, offsets[j] + start, variablelength));
                                }
                                break;
                            }
                        }
