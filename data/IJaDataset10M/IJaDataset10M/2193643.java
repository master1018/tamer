...
public class ZulFns {
    ...
    public static final void setStripeClass(Component child) {
        ...
        if (child.isVisible() && !(child instanceof Listgroup) && !(child instanceof Group) && !(child instanceof Groupfoot) && !(child instanceof Listgroupfoot)) {
            ...
            if (odd == null || !odd.equals("")) {
                ...
                if (parent instanceof Listbox) parent.setAttribute(Attributes.STRIPE_STATE, ((Listbox) parent).getOddRowSclass()); else parent.setAttribute(Attributes.STRIPE_STATE, ((Grid) parent.getParent()).getOddRowSclass());
            }
        }
    }
    ...
    public static final boolean shallVisitTree(Tree tree, Component child) {
        ...
        if (visited == null) visited = new Integer(shoulbBeVisited ? 1 : count + 1); else visited = new Integer(visited.intValue() + (shoulbBeVisited ? 1 : count + 1));
        Integer total = (Integer) tree.getAttribute(Attributes.VISITED_ITEM_TOTAL);
        if (total == null) total = new Integer(count + 1); else total = new Integer(total.intValue() + count + 1);
    ...
    public static final boolean shallRenderTree(Tree tree) {
        ...
        if (ofs < visited.intValue()) {
            ...
            if (renderedCount == null) renderedCount = new Integer(1); else renderedCount = new Integer(renderedCount.intValue() + 1);
