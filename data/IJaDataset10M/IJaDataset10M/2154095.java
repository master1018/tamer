...
public final class TMHMMPredictionTableParser {
            ...
            while ((line = reader.readLine()) != null) {
                if (checkLineForNewEntry(line)) {
                    ...
                    if (prediction != prevPrediction) {
                        if (prevPrediction != null) {
                            saveTmhmmMatch(seqIdMatchMap, sequenceId, startPos, currentAAPos - 1, prevPrediction, scores);
                        }
    ...
    private void saveTmhmmMatch(Map<String, Set<TMHMMMatch>> proteins, String sequenceIdentifier, int start, int end, TMHMMSignature prediction, List<Float> scores) {
    ...
    private TMHMMMatch.TMHMMLocation buildTmhmmLocation(int start, int end, TMHMMSignature prediction, float currentScore) {
        return new TMHMMMatch.TMHMMLocation.Builder(start, end).prediction(prediction.toString()).score(currentScore).build();
    }

    private TMHMMMatch createNewTmhmmMatch(TMHMMSignature prediction, Set<TMHMMMatch.TMHMMLocation> locations) {
        Signature signature = new Signature.Builder(prediction.getAccession()).description(prediction.getShortDesc()).signatureLibraryRelease(signatureLibraryRelease).build();
