...
public class Field {
    ...
    Field(final FieldProperties properties) throws InvalidFormatException {
        ...
        formatter = createFormatter(type, this.properties.getFormat(), this.properties.getDatabaseSettings());
    }
    ...
    private static Formatter createFormatter(final Type type, final String picture, final DatabaseSettings databaseSettings) {
        switch(type) {
            case A:
            case AA:
            case U:
            ...
            case D:
                return new DateFormatter(picture, databaseSettings.getDefaultDateOrder());
            case T:
                return new TimeFormatter(picture, databaseSettings.getDefaultTimeOrder());
            default:
    ...
    private static Type getTypeFromFormat(final String format) throws InvalidFormatException {
        if (format == null) {
            return Type.NONE;
        }
    ...
    public boolean isComputedField() {
        if (this.getFormat() != null && this.getFormat().contains("::C")) {
            return true;
        }
        return false;
    }
