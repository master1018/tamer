    ...
    public void readFromFile(String file) {
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (total != 5) {
                    System.out.println("GPS_MeasurementList.readFromFile: Number of columns do not match");
                    System.exit(-99);
                }
                ...
                GPS_Measurement meas = new GPS_Measurement(temp[0], temp[1], temp[2], temp[3], sv);
                this.add(meas);
            }
            in.close();
            fr.close();
    ...
    public void sendToFile(String file) {
        try {
            ...
            while (this.hasNext(index)) {
                GPS_Measurement meas = this.get(index);
                pw.println(meas.t() + "\t" + meas.t_mjd() + "\t" + meas.range() + "\t" + meas.phaseRange() + "\t" + meas.svid());
                index = index + 1;
            }
            pw.close();
            outfile.close();
