...
public class DiscountCodeFactory {
    ...
    public static List searchDiscountCode(int discountType, Date startDate, Date endDate, String codeId, String desc, String orderBy, String direction, int limit) {
        ...
        if (UtilMethods.isSet(startDate)) sql += "and start_date >= ? ";
        if (UtilMethods.isSet(endDate)) sql += "and end_date <= ? ";
        ...
        if (discountType != 0) sql += "and discount_type = ? ";
        if (UtilMethods.isSet(orderBy)) sql += "order by " + orderBy; else sql += "order by start_date";
        if (UtilMethods.isSet(direction)) sql += " " + direction;
        DotHibernate dh = new DotHibernate();
        dh.setQuery(sql);
        if (UtilMethods.isSet(startDate)) dh.setParam(startDate);
        if (UtilMethods.isSet(endDate)) dh.setParam(endDate);
        if (UtilMethods.isSet(codeId)) dh.setParam(codeId);
        if (UtilMethods.isSet(desc)) dh.setParam(desc);
        if (discountType != 0) dh.setParam(discountType);
    ...
    public static boolean _potentialDiscount(Holder holder, DiscountCode discount) {
        ...
        if (!(applicableFormat.size() == 0)) {
            for (ProductFormat auxFormat : applicableFormat) {
                if (auxFormat.getInode() == format.getInode()) {
                    potential = true;
                    break;
                }
            }
    ...
    public static boolean _applyDiscount(int quantity, DiscountCode discount, Date now) {
        boolean apply = true;
        if ((discount.getStartDate() != null && now.compareTo(discount.getStartDate()) < 0) || (discount.getEndDate() != null && now.compareTo(discount.getEndDate()) > 0)) {
            apply = false;
        }
