...
import org.saosis.core.models.features.DiversitySegmentFeature;

/**
 * An interface for building and converting DiversitySegmentFeature objects.
 * 
 * @author Daniel Allen Prust (danprust@yahoo.com)
 * 
 */
public class DiversitySegmentFeatureBuilder extends FeatureBuilder implements IFeatureBuilder {

    public Feature buildFeature(FeedbackController feedbackController, String name, ArrayList<FeatureQualifier> qualifiers) {
        DiversitySegmentFeature feature = new DiversitySegmentFeature();
        ...
        for (FeatureQualifier qualifier : qualifiers) {
            try {
                ...
                } else if ("product".equals(qualifier.getName())) {
                    feature.getProducts().add(StringUtility.replaceNewlines(StringUtility.unQuote(qualifier.getValue()), " "));
                } else if ("pseudo".equals(qualifier.getName())) {
    ...
    public ArrayList<FeatureQualifier> buildQualifiers(Entry entry, Feature generalFeature) {
        DiversitySegmentFeature feature = (DiversitySegmentFeature) generalFeature;
        ...
        for (String qualification : feature.getProducts()) {
            value = StringUtility.quote(qualification);
            if (FeatureFormatUtility.isEmptyQualifier(value) == false) qualifiers.add(new FeatureQualifier("product", value));
        }
        if (feature.getPseudo()) qualifiers.add(new FeatureQualifier("pseudo", ""));
