...
public class ProteinTransformer extends AbstractTransformer {
    ...
    public ProteinTransformer(Parser parser) {
        ...
        try {
            ...
            rtIsPartOf = graph.getMetaData().getRelationType(MetaData.RT_IS_PART_OF);
            if (rtIsPartOf == null) {
                Parser.propagateEventOccurred(new RelationTypeMissingEvent(MetaData.RT_IS_PART_OF, Parser.getCurrentMethodName()));
            }
            ccProtein = graph.getMetaData().getConceptClass(MetaData.CC_Protein);
            if (ccProtein == null) {
                Parser.propagateEventOccurred(new ConceptClassMissingEvent(MetaData.CC_Protein, Parser.getCurrentMethodName()));
            }
            ccProteinComplex = graph.getMetaData().getConceptClass(MetaData.CC_PROTEIN_COMPLEX);
            if (ccProteinComplex == null) {
                Parser.propagateEventOccurred(new ConceptClassMissingEvent(MetaData.CC_PROTEIN_COMPLEX, Parser.getCurrentMethodName()));
            }
    ...
    public void nodeToConcept(AbstractNode node) {
        ...
        if (protein.isComplex()) cc = ccProteinComplex; else cc = ccProtein;
        ONDEXConcept concept = graph.getFactory().createConcept(protein.getUniqueId(), dataSourceMetaC, cc, etIMPD);
        if (protein.getSpecies() != null) {
            String species = protein.getSpecies();
            if (Parser.speciesNames.containsKey(species)) species = Parser.speciesNames.get(species);
            String taxId = (String) ValidatorRegistry.validators.get("taxonomy").validate(species);
            if (taxId != null) concept.createAttribute(attTaxId, taxId, false); else System.err.println("No mapping for species: " + protein.getSpecies());
        }
    ...
    public void pointerToRelation(AbstractNode node) {
        ...
        if (protein.getComponentOf().size() > 0) {
            ...
            while (proteins.hasNext()) {
                ...
                super.copyContext(graph.getFactory().createRelation(node.getConcept(), p.getConcept(), rtIsPartOf, etIMPD), nonRedundant);
            }
            ...
            while (enzymes.hasNext()) {
                ...
                super.copyContext(graph.getFactory().createRelation(node.getConcept(), e.getConcept(), rtMemberIsPartOf, etIMPD), nonRedundant);
            }
        }
    }
}
