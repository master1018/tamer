...
public class DataFrame {
    ...
    protected DataFrame(PktBufNode aBufNode, Participant p, int noPkts) {
        ...
        for (i = 0; i < pktCount; i++) {
            ...
            if (marks[i]) anyMarked = true;
            aBufNode = aBufNode.nextFrameNode;
        }
        lastSeqNum = seqNum[i - 1];
        if (noPkts > 0) {
            int seqDiff = firstSeqNum - lastSeqNum;
            if (seqDiff < 0) seqDiff = (Integer.MAX_VALUE - firstSeqNum) + lastSeqNum;
            if (seqDiff == pktCount && pktCount == noPkts) isComplete = 1;
        } else {
            isComplete = -1;
        }
        if (RTPSession.rtpDebugLevel > 6) {
    ...
    public byte[] getConcatenatedData() {
        if (this.noPkts < 2) {
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                if (pos + length > totalLength) length = totalLength - pos;
