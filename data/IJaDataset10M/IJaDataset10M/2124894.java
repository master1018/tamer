...
public class RobocodeCompilerFactory {
    ...
    private static final String COMPILER_CLASSPATH = "-classpath " + getJavaLib() + File.pathSeparator + "robocode.jar" + File.pathSeparator + getRobotPath();
    ...
    public static RobocodeCompiler createCompiler(RobocodeEditor editor) {
        ...
        if (getCompilerProperties().getCompilerBinary() == null || getCompilerProperties().getCompilerBinary().equals("")) {
            if (installCompiler(editor)) {
                return new RobocodeCompiler(editor, getCompilerProperties().getCompilerBinary(), getCompilerProperties().getCompilerOptions(), getCompilerProperties().getCompilerClasspath());
            } else {
    ...
    public static boolean installCompiler(RobocodeEditor editor) {
        ...
        if (!javacOk && !jikesOk) {
            ...
            if (noExtract || extract(new File(Constants.cwd(), jikesJar), new File("."))) {
                ...
                console.scrollToBottom();
                JOptionPane.showMessageDialog(editor, "Robocode was unable to extract Jikes." + "Please consult the console window for errors.\n" + "For help with this, please post to the discussion at:\n" + "http://robocode.sourceforge.net/forum", "Error", JOptionPane.ERROR_MESSAGE);
                compilerOptions = "-deprecation -g";
                getCompilerProperties().setCompilerOptions(compilerOptions);
                getCompilerProperties().setCompilerClasspath(COMPILER_CLASSPATH);
                saveCompilerProperties();
