...
public class GroupingSNASettings extends JPanel {
    ...
    private ArrayList<String> groupNameforResult;
    ...
    private JButton docsButton = new JButton("Plugin documentation...");
    ...
    private void drawRightPane() {
        ...
        for (int i = 0; i < mySnMatrix.getNodeNames().length; i++) {
            uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name", mySnMatrix.getNodeNames()[i]), new GUIPropertyListEnumeration("group", groupNames.getAllValues()));
    ...
    void chooseOrgModelButton_actionPerformed(ActionEvent e) {
        ...
        chooser.setFileFilter(new GenericMultipleExtFilter(new String[] { "xml" }, "XML file (*.xml)"));
    ...
    private void makeRelationship2() {
        ...
        if (selected.equals("role")) groupNameforResult = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_ROLE); else if (selected.equals("mined group")) groupNameforResult = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_MININGRESULT); else groupNameforResult = orgModel.getOrgEntityStringList(OrgEntity.ORGENTITYTYPE_ORGUNIT);
        for (int i = 0; i < mySnMatrix.getNodeNames().length; i++) {
            Resource res = orgModel.getResource(mySnMatrix.getNodeNames()[i]);
            if (selected.equals("role")) {
                uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name", mySnMatrix.getNodeNames()[i]), new GUIPropertyListEnumeration("group", orgModel.getOrgEntityList(res, OrgEntity.ORGENTITYTYPE_ROLE)));
            } else if (selected.equals("mined group")) {
                uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name", mySnMatrix.getNodeNames()[i]), new GUIPropertyListEnumeration("group", orgModel.getOrgEntityList(res, OrgEntity.ORGENTITYTYPE_MININGRESULT)));
            } else {
                uiNodesToGroup[i] = new UiNodesToGroup(new GUIPropertyString("name", mySnMatrix.getNodeNames()[i]), new GUIPropertyListEnumeration("group", orgModel.getOrgEntityList(res, OrgEntity.ORGENTITYTYPE_ORGUNIT)));
            }
            imsiPane.add(uiNodesToGroup[i].getNodeName().getPropertyPanel());
            imsiPane.add(uiNodesToGroup[i].getGroup().getPropertyPanel());
        }
    ...
    public void startAnalysis() {
        ...
        if (uiNodesToGroup[0].getGroup().getValue() != null) {
            ...
            MainUI.getInstance().createAnalysisResultFrame(myAlgorithm, myInput, new SocialNetworkResults(null, mySnMatrix.groupOriginators(groupNameforResult, assignment), null));
        }
    }
