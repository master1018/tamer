...
public class LoadStateTest extends TestCase {
    ...
    double[] DOUBLE_VALUES = { 0.0, 1.0, 2.5, 10.0, 16.0, 16.125, -1.0, 2.0, -2.0, -10.0, -0.25, -25, Integer.MAX_VALUE, Integer.MAX_VALUE - 1, (double) Integer.MAX_VALUE + 1.0, Integer.MIN_VALUE, Integer.MIN_VALUE + 1, (double) Integer.MIN_VALUE - 1.0, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, Double.MAX_VALUE, Double.MAX_VALUE };
    ...
    public void testLongBitsToLuaNumber() {
        for (int i = 0; i < DOUBLE_VALUES.length; ++i) {
            ...
            if (v != Integer.MIN_VALUE) {
                assertEquals("Value " + v + " (at index " + i + ") can be represented as integer but was not", luaNumber instanceof LInteger, v == (double) ((int) v));
            }
        }
    }
    ...
    public void testLongBitsToLuaNumberSpeed() throws InterruptedException {
        ...
        for (; currentTime() < endTime; count++) {
            for (int j = 0; j < n; j++) simpleBitsToLuaNumber(BITS[j]);
        }
        ...
        count = 0;
        for (; currentTime() < endTime; count++) {
            for (int j = 0; j < n; j++) LoadState.longBitsToLuaNumber(BITS[j]);
        }
        complexConversionCount += count;
        System.out.println("conversion counts: simple,complex=" + simpleConversionCount + "," + complexConversionCount);
