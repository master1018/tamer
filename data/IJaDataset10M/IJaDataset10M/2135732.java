...
public class BaseDAOTest extends BaseTest {
    ...
    public void testMerge() {
        ...
        assertEquals("The change should be made.", "Fred", target.find(Person.class, joeA.getId()).getFather().getFirstName());
    ...
    public void testRemove() {
        ...
        for (Project project : target.all(Project.class)) {
            if (project.getMembers() != null) project.getMembers().clear();
        }
        List<Person> list = target.all(Person.class);
        ...
        for (Person person : list) {
            if (person.getId().equals(joeA.getId()) || person.getId().equals(sallyA.getId())) fail("Neither Joe nor Sally should now be in the DB");
        }
        joeA.setId(null);
        sallyA.setId(null);
        target.persist(joeA);
        ...
        for (Person person : list) {
            if (person.getFirstName().equals("Joe") && person.getLastName().equals("Alpha")) joeFound = true;
            if (person.getFirstName().equals("Sally") && person.getLastName().equals("Alpha")) sallyFound = true;
        }
    ...
    public void testCompoundId() {
        ...
        s.addSortAsc("name");
        List<Ingredient> ingredients = target.search(s);
        RecipeIngredient ri = new RecipeIngredient(recipes.get(0), ingredients.get(0), .125f, "cup");
        RecipeIngredient ri2 = new RecipeIngredient(recipes.get(0), ingredients.get(1), .25f, "lb.");
        RecipeIngredient ri3 = new RecipeIngredient(recipes.get(2), ingredients.get(4), 2.25f, "cups");
        RecipeIngredient ri4 = new RecipeIngredient(recipes.get(1), ingredients.get(1), 3f, "lbs.");
