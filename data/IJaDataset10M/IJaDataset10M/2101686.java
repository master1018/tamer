...
public class MacEventHandler {
    ...
    public static synchronized MacEventHandler instance() {
        if (INSTANCE == null) INSTANCE = new MacEventHandler();
        return INSTANCE;
    }
    ...
    public void enable(ExternalControl externalControl, Initializer initializer) {
        ...
        this.enabled = true;
        if (lastFileOpened != null) runFileOpen(lastFileOpened);
    }
    ...
    */
    ...
    private void runFileOpen(File file) {
        ...
        if (filename.endsWith("limestart")) {
            ...
            if (!GUIMediator.isConstructed() || !GuiCoreMediator.getLifecycleManager().isStarted()) externalControl.enqueueControlRequest(file.getAbsolutePath()); else if (file.getAbsolutePath().startsWith("magnet:?xt=urn:btih")) {
                GUIMediator.instance().openTorrentMagnet(file.getAbsolutePath());
            }
        } else if (filename.endsWith("torrent")) {
            if (!GUIMediator.isConstructed() || !GuiCoreMediator.getLifecycleManager().isStarted()) externalControl.enqueueControlRequest(file.getAbsolutePath()); else GUIMediator.instance().openTorrent(file);
        } else {
