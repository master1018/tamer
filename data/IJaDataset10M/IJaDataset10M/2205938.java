...
public class ThreadedAcousticScorer implements AcousticScorer {
     ...
     */
    public static final float PROP_ACOUSTIC_GAIN_DEFAULT = 1F;
    ...
    public void newProperties(PropertySheet ps) throws PropertyException {
        logger = ps.getLogger();
        this.frontEnd = (FrontEnd) ps.getComponent(PROP_FRONTEND, FrontEnd.class);
        boolean cpuRelative = ps.getBoolean(PROP_IS_CPU_RELATIVE, PROP_IS_CPU_RELATIVE_DEFAULT);
        ...
        keepData = ps.getBoolean(PROP_SCOREABLES_KEEP_FEATURE, PROP_SCOREABLES_KEEP_FEATURE_DEFAULT);
        acousticGain = ps.getFloat(PROP_ACOUSTIC_GAIN, PROP_ACOUSTIC_GAIN_DEFAULT);
        if (cpuRelative) {
    ...
    public Scoreable calculateScores(List scoreableList) {
        ...
        try {
            ...
            if (numThreads > 1) {
                ...
                int scoreablesPerThread = (scoreableList.size() + (numThreads - 1)) / numThreads;
                ...
                for (int i = 0; i < nThreads; i++) {
                    ScoreableJob job = new ScoreableJob(scoreableList, i * scoreablesPerThread, scoreablesPerThread);
                    if (i < (nThreads - 1)) {
                        mailbox.post(job);
                    } else {
...
class Semaphore {
    ...
    synchronized void post(Scoreable postedBest) {
        if (bestScoreable == null || postedBest.getScore() > bestScoreable.getScore()) {
...
class ScoreableJob {
    ...
    public String toString() {
        return "List size " + scoreables.size() + " start " + start + " size " + size;
    }
}
