...
public class Pair {
    ...
    public boolean isInFinalForm() {
        return (firstTerm instanceof Variable && !secondTerm.containsVar((Variable) firstTerm));
    }
            ...
            if (secondTerm instanceof ConstructorSymbol) {
                if (firstTerm.equalsName(secondTerm)) {
                    if (firstTerm.getParameters().size() == secondTerm.getParameters().size()) {
                        ...
                        this.lastError = "Dekompositionsregel nicht anwendbar, da die Anzahl der Parameter der Konstruktoren unterschiedlich ist.";
                        return false;
                    }
                } else {
