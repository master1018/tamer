...
import edu.indiana.extreme.xbaya.gpel.DSCUtil;
import edu.indiana.extreme.xbaya.util.StringUtil;
import edu.indiana.extreme.xbaya.util.WSDLUtil;
...
public class ODEWSDLTransformer {
	 ...
	 * @param wsdls
	 */
    public void trasnformToODEWsdls(String workflowName, URI dscUrl, WsdlDefinitions workflowWsdl, Map<String, WsdlDefinitions> wsdls) {
    ...
    private void transformServiceWsdls(Map<String, WsdlDefinitions> wsdls, URI dscUrl) {
        ...
        for (String string : keys) {
            ...
            WSDLUtil.replaceAttributeValue(wsdl.getTypes(), "schemaLocation", "http://www.extreme.indiana.edu/gfac/gfac-simple-types.xsd", "gfac-simple-types.xsd");
    ...
    private void setODEAddress(WsdlDefinitions workflowWsdl, String workflowName) {
        ...
        if (serviceItr.hasNext()) {
            ...
            if (portItr.hasNext()) {
                ...
                if (!(XBayaConstants.DEFAULT_ODE_URL + "/ode/processes/" + StringUtil.convertToJavaIdentifier(workflowName)).equals(address.attributeValue("location"))) {
                    address.removeAllAttributes();
                    address.setAttributeValue("location", XBayaConstants.DEFAULT_ODE_URL + "/ode/processes/" + StringUtil.convertToJavaIdentifier(workflowName));
                }
            }
        }
    }
        ...
        try {
            ...
            while (bindingItr.hasNext()) {
                ...
                while (operationsItr.hasNext()) {
                    ...
                    WsdlMessage wsdlMessage = wsdl.getMessage(findInputMessaQname(wsdlBindingOperation, wsdl).getLocalPart());
                    ...
                    if (null == inputPartName) {
                        throw new XBayaRuntimeException("Could not find a partname in message :" + inputMessage + " for binding :" + wsdlBindingOperation);
                    }
        ...
        for (org.xmlpull.infoset.XmlElement schema : schemas) {
            if (WSDLUtil.attributeExist(schema, "type", "crosscutns:LeadCrosscutParameters")) {
                ...
                for (org.xmlpull.infoset.XmlElement importElement : imports) {
                    found = found || WSDLUtil.attributeExist(importElement, "namespace", "http://lead.extreme.indiana.edu/namespaces/2006/lead-crosscut-parameters/");
                }
                if (!found) {
            ...
            if (WSDLUtil.attributeExist(schema, "type", "globalTypens:LEADFileIDArrayType")) {
                ...
                Iterable<org.xmlpull.infoset.XmlElement> imports = schema.elements(schemaNs, IMPORT);
                for (org.xmlpull.infoset.XmlElement importElement : imports) {
                    found = found || WSDLUtil.attributeExist(importElement, "namespace", "http://www.extreme.indiana.edu/lead/xsd");
                }
                if (!found) {
                    org.xmlpull.infoset.XmlElement crosscutImport = BUILDER.newFragment(schemaNs, "import");
        ...
        if (serviceItr.hasNext()) {
            ...
            if (portItr.hasNext()) {
                ...
                if (address != null) {
                    URI uri = null;
                    try {
                        uri = new URI(ODEEprEndingWithPort + "/ode/processes/" + StringUtil.convertToJavaIdentifier(workflowName));
