...
public class TestErlangOracleLearner {
    ...
    public void testLockerLearning() {
        LearnerEvaluationConfiguration learnerConfig = new LearnerEvaluationConfiguration(ErlangModule.setupErlangConfiguration(new File("ErlangExamples/locker/locker.erl")));
    ...
    public void testExporterLearning() {
        LearnerEvaluationConfiguration learnerConfig = new LearnerEvaluationConfiguration(ErlangModule.setupErlangConfiguration(new File("ErlangExamples/exporter/exporter.erl")));
        learnerConfig.config.setErlangAlphabetAnyElements(EXPANSIONOFANY.ANY_WIBBLE);
    ...
    public void testLearningFromErlangTraceFile2() throws IOException {
        ...
        Set<ErlangLabel> alphabetA = new TreeSet<ErlangLabel>();
        alphabetA.addAll(mod.behaviour.getAlphabet());
        ErlangModule.flushRegistry();
        config = ErlangModule.setupErlangConfiguration(new File(lockerFile));
        config.setErlangAlphabetAnyElements(EXPANSIONOFANY.ANY_WIBBLE);
        ErlangModule modSame = ErlangModule.loadModule(config);
        Assert.assertTrue(alphabetA.equals(modSame.behaviour.getAlphabet()));
        ErlangModule.flushRegistry();
        ErlangOracleLearner learner = ErlangQSMOracle.createLearner(null, "resources/earlier_failure2.txt");
        Set<ErlangLabel> alphabetFull = new TreeSet<ErlangLabel>();
        alphabetFull.addAll(ErlangModule.findModule("locker").behaviour.getAlphabet());
