...
public class CharClassData extends StdWebMacro {
    ...
    private String classDropDown(String old) {
        ...
        for (Enumeration e = CMClass.charClasses(); e.hasMoreElements(); ) {
            ...
            } else if (C2.isGeneric()) str.append("<OPTION VALUE=\"" + C2.ID() + "\" " + ((old.equalsIgnoreCase(C2.ID()) || old.equalsIgnoreCase(C2ID)) ? "SELECTED" : "") + ">" + C2.ID() + " (Generic)"); else str.append("<OPTION VALUE=\"" + C2ID + "\" " + ((old.equalsIgnoreCase(C2.ID()) || old.equalsIgnoreCase(C2ID)) ? "SELECTED" : "") + ">" + C2ID);
        }
        return str.toString();
    }
    ...
    public static StringBuffer cabilities(CharClass E, ExternalHTTPRequests httpReq, Hashtable parms, int borderSize, String font) {
            ...
            for (int lvl = minLvl; lvl <= maxLvl; lvl++) {
                for (int i = 0; i < sortedData1.size(); i++) {
                    ...
                    if (qLvl.intValue() == lvl) {
                        ...
                        theclasses.addElement(aID, qLvl.toString(), Integer.toString(CMLib.ableMapper().getDefaultProficiency(E.ID(), false, aID)), CMLib.ableMapper().getDefaultGain(E.ID(), false, aID) ? "" : "on", CMLib.ableMapper().getSecretSkill(E.ID(), false, aID) ? "on" : "", CMLib.ableMapper().getDefaultParm(E.ID(), false, aID), CMLib.ableMapper().getPreReqStrings(E.ID(), false, aID), CMLib.ableMapper().getExtraMask(E.ID(), false, aID), Integer.toString(CMLib.ableMapper().getMaxProficiency(E.ID(), false, aID)));
                    }
                }
            }
        }
        ...
        if (parms.containsKey("HEADERCOL1") || parms.containsKey("HEADERCOL2") || parms.containsKey("HEADERCOL3") || parms.containsKey("HEADERCOL4") || parms.containsKey("HEADERCOL5")) {
            str.append("<TR><TD WIDTH=40%>");
            if (parms.containsKey("HEADERCOL1")) str.append(sfont + ((String) parms.get("HEADERCOL1")) + efont);
            str.append("</TD><TD WIDTH=10%>");
            if (parms.containsKey("HEADERCOL2")) str.append(sfont + ((String) parms.get("HEADERCOL2")) + efont);
            str.append("</TD><TD WIDTH=10%>");
            if (parms.containsKey("HEADERCOL3")) str.append(sfont + ((String) parms.get("HEADERCOL3")) + efont);
            str.append("</TD><TD WIDTH=10%>");
            if (parms.containsKey("HEADERCOL4")) str.append(sfont + ((String) parms.get("HEADERCOL4")) + efont);
            str.append("</TD><TD WIDTH=30%>");
            if (parms.containsKey("HEADERCOL5")) str.append(sfont + ((String) parms.get("HEADERCOL5")) + efont);
            str.append("</TD></TR>");
        }
        ...
        for (int i = 0; i < theclasses.size(); i++) {
            ...
            if (!showParms) str.append("<INPUT TYPE=HIDDEN NAME=CABPRM" + (i + 1) + " VALUE=\"" + ((String) theclasses.elementAt(i, 6)) + "\">");
            if (!showMasks) str.append("<INPUT TYPE=HIDDEN NAME=CABMSK" + (i + 1) + " VALUE=\"" + ((String) theclasses.elementAt(i, 8)) + "\">");
            if (!showPreReqs) str.append("<INPUT TYPE=HIDDEN NAME=CABPRE" + (i + 1) + " VALUE=\"" + ((String) theclasses.elementAt(i, 7)) + "\">");
            str.append("</TD>");
            str.append("</TR>");
            if (showParms) str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont + "Parameters: " + efont + "<INPUT TYPE=TEXT NAME=CABPRM" + (i + 1) + " VALUE=\"" + ((String) theclasses.elementAt(i, 6)) + "\" SIZE=50 MAXLENGTH=255></TD></TR>");
            if (showMasks) str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont + "Extra Mask: " + efont + "<INPUT TYPE=TEXT NAME=CABMSK" + (i + 1) + " VALUE=\"" + ((String) theclasses.elementAt(i, 8)) + "\" SIZE=50 MAXLENGTH=255></TD></TR>");
            if (showPreReqs) str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont + "Pre-Reqs list: " + efont + "<INPUT TYPE=TEXT NAME=CABPRE" + (i + 1) + " VALUE=\"" + ((String) theclasses.elementAt(i, 7)) + "\" SIZE=50 MAXLENGTH=255></TD></TR>");
        }
        str.append("<TR><TD WIDTH=40%>");
        ...
        for (Enumeration a = CMClass.abilities(); a.hasMoreElements(); ) {
            ...
            String ID = A.ID();
            if (!used.contains(ID)) str.append("<OPTION VALUE=\"" + ID + "\">" + A.Name());
        }
        str.append("</SELECT>");
        str.append("</TD>");
        str.append("<TD WIDTH=10%>");
    ...
    public String runMacro(ExternalHTTPRequests httpReq, String parm) {
        ...
        if ((replaceCommand != null) && (replaceCommand.length() > 0) && (replaceCommand.indexOf('=') > 0)) {
        ...
        if (last.length() > 0) {
            ...
            if (C == null) C = (CharClass) httpReq.getRequestObjects().get("CLASS-" + last);
            if ((C == null) && (newClassID != null) && (newClassID.length() > 0) && (CMClass.getCharClass(newClassID) == null)) {
            ...
            if (C == null) C = CMClass.getCharClass(last);
            if (parms.containsKey("ISNEWCLASS")) return "" + (CMClass.getCharClass(last) == null);
            if (C != null) {
                StringBuffer str = new StringBuffer("");
                ...
                if (parms.containsKey("NAMELIST")) {
                    for (int c = 0; c < C.nameSet().length; c++) str.append(C.nameSet()[c] + ", ");
                }
                if (parms.containsKey("NAMES")) {
                    ...
                    if (old == null) {
                        ...
                        while (httpReq.isRequestParameter("NAME" + (numNames + 1))) numNames++;
                    }
                    if (numNames <= 0) nameSet.addElement(Integer.valueOf(0), C.name()); else for (int i = 0; i < numNames; i++) {
                        ...
                        if (CMath.isInteger(lvlStr)) {
                            ...
                            if ((name != null) && (name.length() > 0)) {
                                if (nameSet.size() == 0) nameSet.addElement(Integer.valueOf(minLevel), name); else {
                                    boolean added = false;
                                    for (int n = 0; n < nameSet.size(); n++) if (minLevel < ((Integer) nameSet.elementAt(n, 1)).intValue()) {
                                    ...
                                    if (!added) nameSet.addElement(Integer.valueOf(minLevel), name);
                                }
                            }
                        }
                    }
                    if (nameSet.size() == 0) nameSet.addElement(Integer.valueOf(0), C.name()); else nameSet.setElementAt(0, 1, Integer.valueOf(0));
                    int borderSize = 1;
                    str.append("<TABLE WIDTH=100% BORDER=\"" + borderSize + "\" CELLSPACING=0 CELLPADDING=0>");
                    String sfont = (parms.containsKey("FONT")) ? ("<FONT " + ((String) parms.get("FONT")) + ">") : "";
                    String efont = (parms.containsKey("FONT")) ? "</FONT>" : "";
                ...
                if (parms.containsKey("BASE")) {
                    ...
                    if (old == null) old = C.baseClass(); else {
                ...
                if (parms.containsKey("ATTATT")) {
                    ...
                    if (CMath.s_int(old) < 0) old = "0";
                    for (int i : CharStats.CODES.BASE()) str.append("<OPTION VALUE=\"" + i + "\"" + ((CMath.s_int(old) == i) ? " SELECTED" : "") + ">" + CMStrings.capitalizeAndLower(CharStats.CODES.DESC(i)));
                    str.append(", ");
                }
                ...
                if (parms.containsKey("ARMOR")) {
                    ...
                    if (CMath.s_int(old) <= 0) old = "0";
                    for (int i = 0; i < CharClass.ARMOR_LONGDESC.length; i++) str.append("<OPTION VALUE=\"" + i + "\"" + ((CMath.s_int(old) == i) ? " SELECTED" : "") + ">" + CMStrings.capitalizeAndLower(CharClass.ARMOR_LONGDESC[i]));
                    str.append(", ");
                }
                ...
                if (parms.containsKey("PLAYER")) {
                    ...
                    long mask = 0;
                    if (old == null) mask = C.availabilityCode(); else mask |= CMath.s_long(old);
                    for (int i = 0; i < Area.THEME_PHRASE_EXT.length; i++) str.append("<OPTION VALUE=" + i + " " + ((i == mask) ? "SELECTED" : "") + ">" + Area.THEME_PHRASE_EXT[i]);
                    str.append(", ");
                }
                if (parms.containsKey("ESTATS") || parms.containsKey("CSTATS") || parms.containsKey("ASTATS") || parms.containsKey("ASTATE") || parms.containsKey("STARTASTATE")) {
                    ...
                    if (parms.containsKey("ESTATS")) {
                        ...
                        if (eStats.length() > 0) {
                            CMLib.coffeeMaker().setEnvStats(adjEStats, eStats);
                        }
                        str.append(RaceData.estats(adjEStats, 'E', httpReq, parms, 0) + ", ");
                    }
                    if (parms.containsKey("CSTATS")) {
                        CharStats setStats = (CharStats) CMClass.getCommon("DefaultCharStats");
                    ...
                    if (parms.containsKey("ASTATS")) {
                        CharStats adjStats = (CharStats) CMClass.getCommon("DefaultCharStats");
                    ...
                    if (parms.containsKey("ASTATE")) {
                        CharState adjState = (CharState) CMClass.getCommon("DefaultCharState");
                    ...
                    if (parms.containsKey("STARTASTATE")) {
                        CharState startAdjState = (CharState) CMClass.getCommon("DefaultCharState");
                ...
                if (parms.containsKey("NOWEAPS")) {
                    ...
                    if (old == null) {
                        ...
                        String id = "";
                        set = new Vector();
                        for (int i = 0; httpReq.isRequestParameter("NOWEAPS" + id); id = "" + (++i)) set.addElement(httpReq.getRequestParameter("NOWEAPS" + id));
                    }
                ...
                if (parms.containsKey("OUTFIT")) str.append(RaceData.itemList(C.outfit(null), 'O', httpReq, parms, 0, false) + ", ");
                if (parms.containsKey("DISFLAGS")) {
                    ...
                    for (int i = 0; i < CharClass.GENFLAG_DESCS.length; i++) {
                        ...
                        if (CMath.bset(flags, CMath.pow(2, i))) str.append(" SELECTED");
                ...
                if (parms.containsKey("SECURITYSETS")) {
                    ...
                    if (old == null) {
                        ...
                        while (httpReq.isRequestParameter("SSET" + (numSSet + 1))) numSSet++;
                    }
                    for (int i = 0; i < numSSet; i++) {
                        ...
                        if (CMath.isInteger(lvlStr)) {
                            ...
                            } else sec = httpReq.getRequestParameter("SSET" + (i + 1));
                            if ((sec != null) && (sec.trim().length() > 0) && (CMParms.parseCommas(sec, true).size() > 0)) {
                                ...
                                if (sSet.size() == 0) sSet.addElement(Integer.valueOf(minLevel), sec); else {
                                    boolean added = false;
                                    for (int n = 0; n < sSet.size(); n++) if (minLevel < ((Integer) sSet.elementAt(n, 1)).intValue()) {
                                    ...
                                    if (!added) sSet.addElement(Integer.valueOf(minLevel), sec);
                                }
                            }
                        }
                    }
                ...
                if (parms.containsKey("WEAPMATS")) {
                    ...
                    if (old == null) {
                        ...
                        String id = "";
                        set = new Vector();
                        for (int i = 0; httpReq.isRequestParameter("WEAPMATS" + id); id = "" + (++i)) if (CMath.isInteger(httpReq.getRequestParameter("WEAPMATS" + id))) set.addElement(httpReq.getRequestParameter("WEAPMATS" + id));
                    }
                ...
                if (parms.containsKey("ARMORMINOR")) {
                    ...
                    } else armorMinor = CMath.s_int(old);
                ...
                if (parms.containsKey("CABILITIES")) str.append(cabilities(C, httpReq, parms, 1, "<FONT SIZE=-1 COLOR=WHITE>"));
                if (parms.containsKey("HELP")) {
                    StringBuilder s = CMLib.help().getHelpText(C.ID(), null, false, true);
                    if (s == null) s = CMLib.help().getHelpText(C.name(), null, false, true);
                    if (s != null) {
                        if (s.toString().startsWith("<CHARCLASS>")) s = new StringBuilder(s.toString().substring(11));
                        int limit = 70;
                        if (parms.containsKey("LIMIT")) limit = CMath.s_int((String) parms.get("LIMIT"));
                        str.append(helpHelp(s, limit));
                    }
                }
                if (parms.containsKey("PLAYABLE")) str.append(Area.THEME_PHRASE_EXT[C.availabilityCode()] + ", ");
                if (parms.containsKey("BASECLASS")) str.append(C.baseClass() + ", ");
                if (parms.containsKey("MAXSTATS")) str.append(C.getMaxStatDesc() + ", ");
                if (parms.containsKey("PRACS")) str.append(C.getPracticeDesc() + ", ");
                if (parms.containsKey("TRAINS")) str.append(C.getTrainDesc() + ", ");
                if (parms.containsKey("DAMAGE")) str.append(C.getDamageDesc() + ", ");
                if (parms.containsKey("QUALDOMAINLIST")) {
                    ...
                    for (Enumeration e = CMClass.abilities(); e.hasMoreElements(); ) {
                        ...
                        if (CMLib.ableMapper().getQualifyingLevel(C.ID(), true, A.ID()) > 0) {
                            if ((A.classificationCode() & Ability.ALL_DOMAINS) == 0) domain = Ability.ACODE_DESCS[A.classificationCode()]; else domain = Ability.DOMAIN_DESCS[(A.classificationCode() & Ability.ALL_DOMAINS) >> 5];
                    ...
                    while (domains.size() > 0) {
                        ...
                        if (winnerI != null) str.append(winner + "(" + winnerI.intValue() + "), ");
                        domains.remove(winner);
                    }
                }
                if (parms.containsKey("HITPOINTS")) str.append(C.getHitPointDesc() + ", ");
                if (parms.containsKey("MANA")) str.append(C.getManaDesc() + ", ");
                if (parms.containsKey("MOVEMENT")) str.append(C.getMovementDesc() + ", ");
                ...
                if (parms.containsKey("PRIME")) str.append(C.getPrimeStatDesc() + ", ");
                if (parms.containsKey("ATTACK")) str.append(C.getAttackDesc() + ", ");
                if (parms.containsKey("WEAPONS")) if (C.getWeaponLimitDesc().length() > 0) str.append(C.getWeaponLimitDesc() + ", "); else str.append("Any, ");
                if (parms.containsKey("ARMORLIMITS")) if (C.getArmorLimitDesc().length() > 0) str.append(C.getArmorLimitDesc() + ", "); else str.append("Any, ");
                if (parms.containsKey("LIMITS")) {
                    StringBuffer limits = new StringBuffer("");
                    if (C.getOtherLimitsDesc().length() > 0) limits.append("  " + C.getOtherLimitsDesc());
                    if (C.getLevelCap() > 0) limits.append("  A player may not gain more than " + C.getLevelCap() + " levels in this class.");
                    if (limits.length() == 0) str.append("None, "); else str.append(limits.toString().trim()).append(", ");
                }
                if (parms.containsKey("BONUSES")) if (C.getOtherBonusDesc().length() > 0) str.append(C.getOtherBonusDesc() + ", "); else str.append("None, ");
                if (parms.containsKey("QUALS")) if (C.getStatQualDesc().length() > 0) str.append(C.getStatQualDesc() + ", ");
                if (parms.containsKey("STARTINGEQ")) {
                    if (C.outfit(null) != null) for (int i = 0; i < C.outfit(null).size(); i++) {
                        Item I = (Item) C.outfit(null).elementAt(i);
                        if (I != null) str.append(I.name() + ", ");
                    }
                }
                if (parms.containsKey("BALANCE")) str.append(balanceChart(C));
            ...
            for (int s = 0; s < set.size(); s++) {
                ...
                for (Enumeration c = CMClass.charClasses(); c.hasMoreElements(); ) {
                    ...
                    if (C2.baseClass().equals(C.baseClass())) {
                        ...
                        if (tlvl > 0) {
                            if (tlvl > l) thisCrossClassLevelDiffs += (tlvl - l); else thisCrossClassLevelDiffs += (l - tlvl);
                            numOthers++;
                        }
                    }
                }
                ...
                if (numOthers == 1) uncommonClassSkills++;
                ...
                if (A == null) continue;
                if ((A.abstractQuality() == Ability.QUALITY_BENEFICIAL_OTHERS) || (A.abstractQuality() == Ability.QUALITY_BENEFICIAL_SELF)) {
                    beneficialSkills++;
