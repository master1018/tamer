...
public class FeatureFactory {
    ...
    static String[][] sOperatorLookup = { { FilteredCombinationIterator.class.getName(), "filter" }, { UnfilteredCombinationIterator.class.getName(), "combine" }, { SynonymIterator.class.getName(), "syn" }, { SynonymIterator.class.getName(), "synonym" }, { ExtentInsideIterator.class.getName(), "inside" }, { OrderedWindowIterator.class.getName(), "ordered" }, { OrderedWindowIterator.class.getName(), "od" }, { UnorderedWindowIterator.class.getName(), "unordered" }, { UnorderedWindowIterator.class.getName(), "uw" }, { ScaleIterator.class.getName(), "scale" } };

    static String[][] sFeatureLookup = { { DirichletScorer.class.getName(), "dirichlet" } };

    static String[] sTraversalList = { AddCombineTraversal.class.getName(), WeightConversionTraversal.class.getName(), IndriWindowCompatibilityTraversal.class.getName(), TextFieldRewriteTraversal.class.getName(), ImplicitFeatureCastTraversal.class.getName() };
    ...
    boolean isUsableConstructor(Class[] types, ArrayList<StructuredIterator> childIterators) {
        if (types.length == 0) return false;
        if (!Parameters.class.isAssignableFrom(types[0])) return false;
        for (int i = 0; i < types.length - 1; ++i) {
            if (types[i].isArray()) return false;
        }
        boolean lastIsArray = types[types.length - 1].isArray();
        int iteratorArgs = 1 + childIterators.size();
        if (iteratorArgs != types.length && !lastIsArray) return false;
        if (lastIsArray && iteratorArgs < types.length - 1) return false;
        int iteratorIndex = 0;
        int typeIndex;
        for (typeIndex = 1; typeIndex < types.length; typeIndex++) {
            ...
            if (currentType.isArray()) {
                ...
                if (iteratorIndex >= childIterators.size()) return false;
                StructuredIterator iterator = childIterators.get(iteratorIndex);
                if (!currentType.isAssignableFrom(iterator.getClass())) return false;
            }
        }
        return true;
    }

    Object[] argsForConstructor(Class[] types, Parameters parameters, ArrayList<StructuredIterator> childIterators) {
        ...
        if (lastType.isArray()) {
            List<StructuredIterator> remaining = childIterators.subList(types.length - 2, childIterators.size());
    ...
    public StructuredIterator getIterator(Node node, ArrayList<StructuredIterator> childIterators) throws Exception {
        ...
        Object[] args = argsForConstructor(constructor.getParameterTypes(), parametersCopy, childIterators);
        RequiredStatistics required = type.getIteratorClass().getAnnotation(RequiredStatistics.class);
        if (required != null) {
    ...
    public List<Traversal> getTraversals(StructuredRetrieval retrieval) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        ArrayList<Traversal> result = new ArrayList<Traversal>();
        for (TraversalSpec spec : traversals) {
            Class<? extends Traversal> traversalClass = (Class<? extends Traversal>) Class.forName(spec.className);
            Constructor<? extends Traversal> constructor = traversalClass.getConstructor(Parameters.class, StructuredRetrieval.class);
