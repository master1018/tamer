...
public class NetworkServer extends Thread {
    ...
    public void run() {
        try {
            ...
            model.getView().setConnectionText("Server Error", "Cannot determine local address");
        } catch (Exception e) {
    ...
    private String handleUserMessage(String message) throws Exception {
        ...
        model.getView().setConnectionText("Server Status", message + " Connected.  " + clients.size() + " clients connected");
        return message;
    }
    ...
    protected class Handler extends Thread {
        ...
        public void run() {
            NetworkView view = model.getView();
            try {
                view.setConnectionText("Server Status", user + " Connected.  " + clients.size() + " clients connected");
                BufferedReader is = new BufferedReader(new InputStreamReader(socket.getInputStream(), "UTF-8"));
                os = new PrintStream(new BufferedOutputStream(socket.getOutputStream()), true, "UTF-8");
                String line;
