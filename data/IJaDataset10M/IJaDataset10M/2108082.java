...
public class CacheFeatureExtractor<E> extends FeatureExtractorFilter<E> implements Serializable {
    ...
    static class Serializer<F> extends AbstractExternalizable {
        ...
        public Object read(ObjectInput in) throws ClassNotFoundException, IOException {
            @SuppressWarnings("unchecked") FeatureExtractor<? super F> extractor = (FeatureExtractor<? super F>) in.readObject();
            @SuppressWarnings("unchecked") Map<F, Map<String, ? extends Number>> cache = (Map<F, Map<String, ? extends Number>>) in.readObject();
