...
public class Commande implements Configurable {
    ...
    public void newProperties(PropertySheet ps) throws PropertyException {
        ...
        try {
            if (cm.lookup(ps.getString("AM", "")) == null) logger.info("pas trouve " + ps.getString("AM", ""));
            ;
            PropertySheet aps = cm.getPropertySheet(ps.getString("AM", ""));
            if (aps == null) logger.info("pas de am dans batch et donc pas de machin"); else {
    ...
    public void faire() {
        ...
        for (int i = first; i <= last; i++) {
            FloatData[] data;
            if (i >= TS.lengthCI()) data = loadData(new File(dataDir, i + ".gz")); else {
                ArrayList<FloatData> d = new ArrayList<FloatData>(1000);
                Integer[] lescds = TS.lesCD(i);
                for (Integer cd : lescds) loadData(new File(dataDir, cd.toString() + ".gz"), d);
                data = d.toArray(new FloatData[0]);
            }
            float[][] noyaux = faireNoyaux(data);
    ...
    private FloatData[] loadData(File f) {
        ArrayList<FloatData> d = new ArrayList<FloatData>(1000);
        loadData(f, d);
        if (d.size() >= 10) return d.toArray(new FloatData[0]);
        return null;
    }
    ...
    private void loadData(File f, ArrayList<FloatData> d) {
        ...
        try {
            ...
            while (s.hasNextFloat()) {
                ...
                for (int i = 0; i < 39; i++) values[i] = s.nextFloat();
    ...
    private float[][] faireNoyaux(FloatData[] lesdatas) {
        ...
        for (int it = 0; it < lesdatas.length; it++) if (Math.random() < tirage) {
            int i;
            float[] values = lesdatas[it].getValues().clone();
            for (i = 1; i < 3 && it < lesdatas.length - 1; i++) {
                float[] tab = lesdatas[++it].getValues();
                for (int l = 0; l < tab.length; l++) values[l] += tab[l];
            }
            for (int l = 0; l < values.length; l++) values[l] /= i;
            d.add(values);
        }
        if (d.size() >= 10) return d.toArray(new float[0][39]);
        return null;
    }
    ...
    public static void main(String[] argv) {
        if (argv.length < 1) {
            System.out.println("Usage: BatchDecoder propertiesFile ");
            System.exit(1);
        }
        ...
        try {
            edu.cmu.sphinx.instrumentation.ConfigMonitor config = (edu.cmu.sphinx.instrumentation.ConfigMonitor) cm.lookup("configMonitor");
            config.run();
        } catch (InstantiationException e) {
