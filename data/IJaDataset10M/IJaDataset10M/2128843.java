...
public class StateTransitionGraph {
    ...
    public StateCache.State getNextFrontierState() {
        ...
        if (state.info == null) throw Util.failure("LegacyState on frontier has no edge info: " + state.getUniqueName());
    ...
    public void setExplored(StateCache.State s) {
        if (isFrontier(s)) throw Util.failure("state cannot be on frontier and explored: " + s.getUniqueName());
        if (!isExplored(s)) {
            s.isExplored = true;
            exploredCount++;
        }
