...
public class SemiringComponent extends JPanel {
    ...
    private void initEditPanel() {
        ...
        okayButton = new JButton(Messages.getString("app", "SemiringComponent.OK"));
    ...
    private void initViewPanel() {
        Dimension panelDimension = new Dimension(this.getWidth(), (int) (InterfaceConstants.formulaNormalFont(beamerMode).getSize() * 4.5f));
    ...
    private int paintString(String string, Graphics2D graphics, Font font, int offsetX, int offsetY) {
    ...
    private int paintStringNormal(String string, Graphics2D graphics, int offsetX, int offsetY) {
        return paintString(string, graphics, normalFont, offsetX, offsetY);
    }

    private int paintStringBold(String string, Graphics2D graphics, int offsetX, int offsetY) {
        return paintString(string, graphics, boldFont, offsetX, offsetY);
    }

    private int paintStringSmall(String string, Graphics2D graphics, int offsetX, int offsetY) {
        return paintString(string, graphics, smallFont, offsetX, offsetY);
    }

    private void paintSemiringDefinition(Graphics2D graphics, SemiRing semiring, int offsetX, int offsetY, int lineHeight) {
        ...
        offsetX = paintStringBold(symbolicRepresentation[0], graphics, offsetX, offsetY);
        lowerOffset = paintStringSmall(symbolicRepresentation[1], graphics, offsetX, offsetY - lineHeight / 4);
        upperOffset = paintStringSmall(symbolicRepresentation[2], graphics, offsetX, offsetY + lineHeight / 4);
    ...
    private void paintView(Graphics2D graphics) {
        ...
        fontHeight = (int) graphics.getFontMetrics(normalFont).getStringBounds("D", graphics).getHeight();
        ...
        offsetY += lineHeight;
        paintSemiringDefinition(graphics, semiring, offsetX, offsetY, lineHeight);
    }
    ...
    public void paint(Graphics graphics) {
        ...
        if (displayMode == SemiringDisplay.TEMPLATE_SELECTION) super.paint(graphics); else this.paintView(g);
    }
    ...
    public void setDisplayMode(SemiringDisplay mode) {
        ...
        switch(this.displayMode) {
            case TEMPLATE_SELECTION:
                if (editPanel == null) initEditPanel();
                add(editPanel);
                break;
            case DISPLAY:
                initViewPanel();
