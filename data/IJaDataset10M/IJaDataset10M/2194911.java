...
public final class VM_Atom implements VM_Constants, VM_ClassLoaderConstants {
   ...
   */
    public final String classNameFromDescriptor() {
        if (VM.VerifyAssertions) VM._assert(val[0] == 'L');
   ...
   */
    final VM_Type parseForReturnType(ClassLoader classloader) {
        ...
        switch(val[i]) {
            ...
            case ClassTypeCode:
            case ArrayTypeCode:
                return VM_ClassLoader.findOrCreateType(findOrCreateAtom(val, i, val.length - i), classloader);
            default:
   ...
   */
    public final VM_Type[] parseForParameterTypes(ClassLoader classloader) {
        ...
        for (int i = 1; ; ) switch(val[i++]) {
            case VoidTypeCode:
                sigs.addElement(VM_Type.VoidType);
                continue;
            case BooleanTypeCode:
                sigs.addElement(VM_Type.BooleanType);
                continue;
            case ByteTypeCode:
                sigs.addElement(VM_Type.ByteType);
                continue;
            case ShortTypeCode:
                sigs.addElement(VM_Type.ShortType);
                continue;
            case IntTypeCode:
                sigs.addElement(VM_Type.IntType);
                continue;
            case LongTypeCode:
                sigs.addElement(VM_Type.LongType);
                continue;
            case FloatTypeCode:
                sigs.addElement(VM_Type.FloatType);
                continue;
            case DoubleTypeCode:
                sigs.addElement(VM_Type.DoubleType);
                continue;
            case CharTypeCode:
                sigs.addElement(VM_Type.CharType);
                continue;
            case ClassTypeCode:
                {
                    int off = i - 1;
   ...
   */
    ...
    final void sysWrite() throws VM_PragmaUninterruptible {
        for (int i = 0, n = val.length; i < n; ++i) VM.sysWrite((char) val[i]);
    }
   ...
   */
    private VM_Atom(byte utf8[]) {
        int hash = 99989;
        for (int i = utf8.length; --i >= 0; ) hash = 99991 * hash + utf8[i];
        this.val = utf8;
        this.hash = hash;
    }
    ...
    private static VM_Atom findOrCreateAtom(byte utf8[], int off, int len) {
        ...
        for (int i = 0; i < len; ++i) val[i] = utf8[off++];
   ...
   */
    static int dictionaryCompare(VM_Atom left, VM_Atom right) {
        ...
        if (left.val.length != right.val.length) return -1;
        byte[] leftVal = left.val;
        byte[] rightVal = right.val;
        for (int i = leftVal.length; --i >= 0; ) if (leftVal[i] != rightVal[i]) return -1;
        return 1;
    }
}
