...
public class JythonResultFormatter {
    ...
    public Data[] formatRawResults(List rawResults, Data[] inputData, Dictionary properties) {
        ...
        dataResults = addMetaData(dataResults, inputData, properties);
    ...
    protected String getResultLabel(Dictionary props, int numResult) {
        String labelKey = JythonFileProperty.RESULT_PREFIX + numResult + JythonFileProperty.LABEL_SUFFIX;
        ...
        if (!(labelValue == null)) {
            ...
            logger.log(LogService.LOG_WARNING, "Label of data returned from " + "jython script not specified in .properties file. " + "Assigning label to '" + labelValueString + "'.");
        }
        return labelValueString;
    }

    protected String getResultType(Dictionary props, int numResult) {
        String typeKey = JythonFileProperty.RESULT_PREFIX + numResult + JythonFileProperty.TYPE_SUFFIX;
        ...
        if (!(typeValue == null)) {
            ...
            logger.log(LogService.LOG_WARNING, "Type of data returned from " + "jython script not specified in .properties file. " + "Assigning type to '" + typeValueString + "'.");
        }
    ...
    protected Data getResultParent(Dictionary props, int numResult, Data[] inputData) {
        String childKey = JythonFileProperty.RESULT_PREFIX + numResult + JythonFileProperty.PARENT_SUFFIX;
        ...
        if (!(parentName == null)) {
            ...
            if (parentDataIndex < inputData.length) {
                ...
                logger.log(LogService.LOG_WARNING, ".properties file " + "tried to assign result" + numResult + "the " + "parent arg" + parentDataIndex + ", which has an " + "index greater than any arg provided. Cannot " + "assign result" + numResult + " a parent.");
                parent = null;
            }
        } else {
            parent = null;
    ...
    protected List convertToData(List rawResults, Dictionary props) {
        ...
        if (formats.length > rawResults.size()) {
            ...
            logger.log(LogService.LOG_WARNING, "More out_data formats " + "provided than an actual data returned. Ignoring extra " + "formats.");
        } else if (rawResults.size() > formats.length) {
            numFinalResults = formats.length;
            logger.log(LogService.LOG_WARNING, "More data returned than " + "out_data formats provided. Ignoring additional data " + "returned");
        } else {
            numFinalResults = rawResults.size();
        }
    ...
    protected void checkType(String ts) {
        if (!(ts.equals(DataProperty.MATRIX_TYPE) || ts.equals(DataProperty.NETWORK_TYPE) || ts.equals(DataProperty.TEXT_TYPE) || ts.equals(DataProperty.OTHER_TYPE))) {
            logger.log(LogService.LOG_WARNING, "JythonRunnerAlgorithm: " + "Assigning return data an unsupported data type " + ts + ". Either the type is invalid or " + "JythonRunnerAlgorithm has not be updated to reflect " + "types introduced in newer versions");
        }
    }
}
