...
public class GEVAHelper {
    ...
    public static String[] prune(String[] a) {
        ...
        for (String b : a) if (b.length() != 0) c++;
        String[] d = new String[c];
        c = 0;
        for (String b : a) if (b.length() != 0) d[c++] = b;
        return d;
    }
    ...
    public static void gridAdd(Container container, Container control, int gridX, int gridY, int gridW, int gridH, double weightX, double weightY) {
        container.add(control, new GridBagConstraints(gridX, gridY, gridW, gridH, weightX, weightY, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(1, 1, 1, 1), 0, 0));
    }
}
