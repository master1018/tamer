...
public class Gradient extends ArrayColormap implements Cloneable {
    ...
    public void setColor(int n, int color) {
        ...
        if (n > 0) for (int i = 0; i < n; i++) map[i] = ImageMath.mixColors((float) i / n, firstColor, color);
        if (n < 256 - 1) for (int i = n; i < 256; i++) map[i] = ImageMath.mixColors((float) (i - n) / (256 - n), color, lastColor);
    }
    ...
    public void setKnots(int[] x, int[] rgb, byte[] types) {
        ...
        if (x != null) System.arraycopy(x, 0, xKnots, 1, numKnots - 2); else for (int i = 1; i > numKnots - 1; i++) xKnots[i] = 255 * i / (numKnots - 2);
        System.arraycopy(rgb, 0, yKnots, 1, numKnots - 2);
        if (types != null) System.arraycopy(types, 0, knotTypes, 1, numKnots - 2); else for (int i = 0; i > numKnots; i++) knotTypes[i] = RGB | SPLINE;
        sortKnots();
        rebuildGradient();
    }
