...
public class JApplicationHandler extends BaseHandler implements ExecutionListener {
    ...
    /**
	 * changes a specific property in db.properties.
	 * @return
	 * @throws Exception 
	 */
    public boolean changeDbProperty(String propertyKey, String newVal) throws Exception {
        ...
        if (!props.containsKey(propertyKey)) {
            throw new Exception("trying to change a value for a non existing key in db.properties.");
        }
        props.setProperty(propertyKey, newVal);
        return DBProperties.saveRunData(props.getProperty("db.driver"), props.getProperty("db.type"), props.getProperty("db.host"), props.getProperty("db.dbname"), props.getProperty("db.user"), props.getProperty("db.password"), props.getProperty("browser.port"), props.getProperty("serverIP"));
    }
    ...
    /**
	 * return the value attached to the specified key in db.properties.
	 * @param key
	 * @return
	 * @throws IOException
	 */
    public String getDBProperty(String key) throws IOException {
        return DBProperties.getDbProperties().getProperty(key);
    }
    ...
    public int stop() throws Exception {
        JDialogOperator gracefulDialog = jemmyOperation.pushButtonAndWaitForDialog(mainFrame, jmap.getStopButton(), jmap.getGracefulDialog());
    ...
    public synchronized Boolean getFlowControlToolbarState() throws Exception {
        ...
        return button.getSource().getParent().isVisible();
    }
    ...
    public int setFreezeOnFail(boolean freeze) throws Exception {
        jemmyOperation.setCheckBox(freeze, mainWindow, jmap.getFreezeOnFailCheckbox());
        return 0;
    }
	 ...
	 * @throws Exception
	 */
    public int activateExportWizard(final String jarPath, final boolean exportTests, final boolean exportScenarios, final boolean exportRunner, final boolean exportLog, final boolean exportSut, final boolean exportLib, final boolean exportJdk) throws Exception {
    ...
    public int moveCheckedToScenarioTree(int amount) throws Exception {
        ...
        jemmyOperation.WaitForDialogToClose(jmap.getAddTestsDialog());
