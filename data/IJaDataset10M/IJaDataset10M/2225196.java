...
public class DefaultMirrorFactory implements MirrorFactory {
    ...
    public <T> Mirror<T> getMirror(Class<T> type, String name) {
        ...
        if (aopConfigration == null) if (ioc.has(AopConfigration.IOCNAME)) aopConfigration = ioc.get(AopConfigration.class, AopConfigration.IOCNAME); else aopConfigration = new AnnotationAopConfigration();
        ...
        if (interceptorPairs == null || interceptorPairs.size() < 1) {
            if (log.isDebugEnabled()) log.debugf("%s , no config to enable AOP.", type);
            return Mirror.me(type);
        }
