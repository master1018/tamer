...
public class UserRoleElementHandler extends AbstractElementHandler {
    ...
    public void startElement(Properties ctx, Element element) throws SAXException {
        ...
        if (count > 0) {
            ...
            if (no == -1) log.info("Update to user roles failed");
        } else {
            Object_Status = "New";
            AD_Backup_ID = 0;
            ...
            int no = DB.executeUpdate(sqlB.toString(), getTrxName(ctx));
            if (no == -1) log.info("Insert to user roles failed");
        }
    }
    ...
    private AttributesImpl createUserAssignBinding(AttributesImpl atts, int user_id, int role_id, int org_id) {
        String sql = null;
        String name = null;
        atts.clear();
