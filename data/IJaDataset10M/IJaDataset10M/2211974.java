...
public class MapViewerFlex implements IMapViewer {
    ...
    private static final Logger LOG = Logger.getLogger(MapViewerFlex.class.getCanonicalName());
    ...
    static {
        ...
        try {
            ...
            URL url = resourcePath.makeUrl("com/esri/gpt/catalog/search/MapViewerFlex.js");
            is = url.openStream();
 ...
 */
    private String getWindowHeight() {
        int height = Val.chkInt(this.getMapViewerConfig().getParameters().get("height"), 400);
 ...
 */
    private String getWindowWidth() {
        int width = Val.chkInt(this.getMapViewerConfig().getParameters().get("width"), 500);
 ...
 */
    private String getWindowTitle() {
        ...
        if (value == null || "".equals(value)) {
            try {
                value = this.getRequestContext().getServletRequest().getServerName();
            } catch (Throwable e) {
                LOG.log(Level.WARNING, "Could not get server name from request context. " + "using  window name = Map");
                value = "Map";
            }
        }
 ...
 */
    private String getRegexCanConsumeUrl() {
        String value = this.getMapViewerConfig().getParameters().get("regexCanConsumeUrl");
        return Val.chkStr(value);
    }
 ...
 */
    public String readAddToMapUrl() {
        ...
        try {
            ...
            String resourceValue = URLEncoder.encode(this.getHintServiceType().toLowerCase() + ":" + this._resourceUri, "UTF-8");
            url += "title=" + URLEncoder.encode(title, "UTF-8");
            if (this.getUrlCanBeJscript() && JSCRIPT != null) {
                url = JSCRIPT.replace("{mapAndResourceUrl}", url + "&resource=" + resourceValue);
                url = url.replace("{jsResourceUrl}", URLDecoder.decode(resourceValue, "UTF-8"));
                ...
                if (this.getUrlCanBeJscript() && JSCRIPT == null) {
                    LOG.warning("Could not create javascript based add to map because " + " of prior initialization errors");
                }
 ...
 */
    public boolean canHandleResource() {
        ...
        if (hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.WMS.name()) || hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.AIMS.name())) {
        ...
        if (hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.AGS.name())) {
            if (resourceUri.toLowerCase().contains("/rest") && resourceUri.toLowerCase().contains("/mapserver")) {
