...
final class SWGSchematicWrapper implements Serializable, SWGGui {
    ...
    List<SWGExperimentWrapper> experiments() {
        synchronized (this) {
            if (expWrappers == null) expWrappers = SWGExperimentWrapper.createInstances(this);
            return expWrappers;
        }
    }
    ...
    SWGSchematic schem() {
        synchronized (schemID) {
            if (schematic == null) schematic = SWGSchematicsManager.getSchematic(schemID());
            return schematic;
        }
    }
     ...
     */
    String serial() {
        return schemSerial == null ? isDefault ? schem().getName() : "-- ? --" : schemSerial;
    }
    ...
    void stock(@SuppressWarnings("hiding") int count) {
        if (count < 0) throw new IllegalArgumentException("Invalid value: " + count);
        this.count = count;
    }
    ...
    static Comparator<SWGSchematicWrapper> comparator() {
        if (wrapperComp == null) wrapperComp = new Comparator<SWGSchematicWrapper>() {

            @Override
            public int compare(SWGSchematicWrapper o1, SWGSchematicWrapper o2) {
                if (o1 == o2) return 0;
