...
public class Section0 extends Section {
    ...
    public String validateAgainstOtherSections(TreeMap sections) {
            ...
            for (int section = 0; section < sectionIDNumbers.length; ++section) {
                ...
                if (sectionLengths[section] != 0) {
                    if (targetSection == null) {
                        ...
                        if (sectionIndexes[section] != targetSection.getSectionHeader().getByteOffset() + 1) {
        ...
        while (i.hasNext()) {
            int wantID = ((Integer) (i.next())).intValue();
            boolean found = false;
            for (int section = 0; section < sectionIDNumbers.length; ++section) {
    ...
    public SCPTreeRecord getTree(SCPTreeRecord parent) {
        if (tree == null) {
            ...
            for (int section = 0; section < sectionIDNumbers.length; ++section) {
                new SCPTreeRecord(tree, "Section " + Integer.toString(sectionIDNumbers[section]), "Length = " + Long.toString(sectionLengths[section]) + " dec (0x" + Long.toHexString(sectionLengths[section]) + ") " + "Index = " + Long.toString(sectionIndexes[section]) + " dec (0x" + Long.toHexString(sectionIndexes[section]) + ")");
            }
        }
        return tree;
    }
