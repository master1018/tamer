...
public abstract class AbstractShapeExtender implements ShapeExtender {
    ...
    public Shape[] extend(Shape a, Shape b) {
        ...
        for (i = 0; i < sourceIndices.length && i < targetIndices.length; i++) {
            int endSourceIndex;
            if (i >= sourceIndices.length - 1) endSourceIndex = source.size() - 1; else endSourceIndex = sourceIndices[i + 1] - 1;
            int endTargetIndex;
            if (i >= targetIndices.length - 1) endTargetIndex = target.size() - 1; else endTargetIndex = targetIndices[i + 1] - 1;
            PathOp endTargetOp = target.getSegment(endTargetIndex);
            PathOp endSourceOp = source.getSegment(endSourceIndex);
            if (endTargetOp.getOp() == PathIterator.SEG_CLOSE && endTargetOp.getOp() != endSourceOp.getOp()) {
                source.addPendingOp(endSourceIndex + 1, new PathOp(PathIterator.SEG_CLOSE, null));
            } else if (endSourceOp.getOp() == PathIterator.SEG_CLOSE && endTargetOp.getOp() != endSourceOp.getOp()) {
                PathOp sourceStartOp = source.getSegment(sourceIndices[i]);
                PathOp newEndOp;
                if (sourceStartOp.getOp() == PathIterator.SEG_MOVETO) {
                    newEndOp = new PathOp(PathIterator.SEG_LINETO, sourceStartOp.getCoords());
                } else {
                    float[] coords = { 0, 0 };
    ...
    protected void appendEmptySubpaths(PathOpList target, PathOpList source, int[] sourceIndices, int subpathCount) {
        ...
        if (target.size() == 0) origin = new Point2D.Float(); else origin = target.getSegmentEndpoint(target.size() - 1, null);
