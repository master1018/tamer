...
public class VariableBlurFilter extends AbstractBufferedImageOp {
    ...
    protected static final Logger logger = Logger.getLogger(VariableBlurFilter.class.getName());
    ...
    public void blur(int[] in, int[] out, int width, int height, int radius, int pass) {
        ...
        for (int y = 0; y < height; y++) {
            int outIndex = y;
            if (blurMask != null) {
                if (pass == 1) getRGB(blurMask, 0, y, width, 1, mask); else getRGB(blurMask, y, 0, 1, width, mask);
            }
            ...
            for (int x = 0; x < width; x++) {
                int ra;
                if (blurMask != null) {
                    if (pass == 1) ra = (int) ((mask[x] & 0xff) * hRadius / 255f); else ra = (int) ((mask[x] & 0xff) * vRadius / 255f);
                } else {
                    if (pass == 1) ra = (int) (blurRadiusAt(x, y, width, height) * hRadius); else ra = (int) (blurRadiusAt(y, x, height, width) * vRadius);
                }
                int divisor = 2 * ra + 1;
