...
public class ReflectionBuilder {
    ...
    protected Map<?, ?> toMap(String sValue) {
        String[] tokens = StringUtils.split(sValue, StringUtils.DEFAULT_DELIMITER_CHAR, StringUtils.DEFAULT_QUOTE_CHAR, StringUtils.DEFAULT_QUOTE_CHAR, true, true);
        if (tokens == null || tokens.length <= 0) {
            return null;
        }
    ...
    protected void applyProperty(Object object, String propertyName, String stringValue) {
        ...
        } else if (isTypedProperty(object, propertyName, List.class) || isTypedProperty(object, propertyName, Collection.class)) {
        ...
        try {
            if (log.isTraceEnabled()) {
                log.trace("Applying property [{}] value [{}] on object of type [{}]", new Object[] { propertyName, value, object.getClass().getName() });
            }
            BeanUtils.setProperty(object, propertyName, value);
        } catch (Exception e) {
            String msg = "Unable to set property '" + propertyName + "' with value [" + stringValue + "] on object " + "of type " + (object != null ? object.getClass().getName() : null) + ".  If " + "'" + stringValue + "' is a reference to another (previously defined) object, prefix it with " + "'" + OBJECT_REFERENCE_BEGIN_TOKEN + "' to indicate that the referenced " + "object should be used as the actual value.  " + "For example, " + OBJECT_REFERENCE_BEGIN_TOKEN + stringValue;
            throw new ConfigurationException(msg, e);
        }
    }
}
