...
/**
 * Represents a constraint on the maximum amount of time to wait for a network
 * connection to be established. The precise meaning of this will vary across
 * communication mechanisms, but in the typical case of {@link java.net.Socket}
 * -based communication, the intention is that this constraint controls the
 * timeout parameter of the
 * {@link java.net.Socket#connect(java.net.SocketAddress,int) connect} method.
 * <p>
 * The duration is translated into an absolute end time at the point of a remote
 * call by adding the caller's current time.
 * 
...
public final class ConnectionRelativeTime implements RelativeTimeConstraint, Serializable {
    ...
    /**
	 * Returns a {@link ConnectionAbsoluteTime} instance with time obtained by
	 * adding the specified base time argument to the duration value from this
	 * instance. If the addition results in overflow, a time value of
	 * <code>Long.MAX_VALUE</code> is used.
	 */
