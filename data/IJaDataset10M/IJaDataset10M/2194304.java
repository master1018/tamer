...
public class LightDistantlight extends LightShader {
    ...
    protected void initDefaults() {
        defaultParameters.addParameter(new UniformScalarFloat(new Declaration("intensity", "uniform float"), 1f));
        defaultParameters.addParameter(new UniformScalarTuple3f(new Declaration("lightcolor", "uniform color"), 1f, 1f, 1f));
        defaultParameters.addParameter(new UniformScalarTuple3f(new Declaration("from", "uniform point"), 0f, 0f, 0f));
        defaultParameters.addParameter(new UniformScalarTuple3f(new Declaration("to", "uniform point"), 0f, 0f, 1f));
    }
    ...
    public boolean shade(ShaderVariables sv, Point3fGrid P, Vector3fGrid N, float angle) {
        ...
        if (attributes.getSpace() == Space.WORLD) shaderTransform = Global.getTransform("camera").concat(shaderTransform);
