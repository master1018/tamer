...
public class MailComposerImplTest {
    ...
    public void getPathToTemplate() {
        ...
        assertEquals(("/a/b/c/default" + File.separator + "priceReduced" + File.separator).replace("\\", "/"), (mailComposer.getPathToTemplate("SHOIP1", "priceReduced")).replace("\\", "/"));
    }
    ...
    public void testComposeMessageInternal3() throws MessagingException, IOException, ClassNotFoundException {
        ...
        mailComposer.composeMessage(message, null, "src/test/resources/mailtemplates/default/", "imtest", "test@localhost.lo", "to@somedomain.com", "cc@somedomain.com", "bcc@somedomain.com", createModel());
        assertTrue(helper.isMultipart());
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        helper.getMimeMessage().writeTo(byteArrayOutputStream);
        String str = byteArrayOutputStream.toString("UTF-8");
