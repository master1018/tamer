...
public class MotorModuleEvent extends AbstractACTREvent<IMotorModule, IMotorModuleListener> {
   ...
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(MotorModuleEvent.class);
    ...
    public MotorModuleEvent(IMotorModule source, MuscleState muscleState, Type type) {
        super(source);
        _type = type;
    ...
    public MuscleState getMuscleState() {
        if (_muscleState == null) _muscleState = getSource().getMuscleManager().getMuscleState(_movement.getMuscleIdentifier());
        return _muscleState;
    }
