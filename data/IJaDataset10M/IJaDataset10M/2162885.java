...
public class VonMisesFit_Modeler extends Modeler implements ItemListener {
    ...
    public void fitCurve(float[] rawDat, float minx, float maxx, JTextArea resultPanelTextArea, boolean rescaleClicked, boolean scaleUp, boolean initReset) {
        try {
            ...
            } else VonMisesDistr = new edu.ucla.stat.SOCR.distributions.VonMisesDistribution(Double.parseDouble(locationParamField.getText()), Double.parseDouble(concentrationParamField.getText()));
    ...
    public double[] generateSamples(int sampleCount) {
        ...
        for (int i = 0; i < sampleCount; i++) dat[i] = VonMisesDistr.simulate();
        return dat;
    }
