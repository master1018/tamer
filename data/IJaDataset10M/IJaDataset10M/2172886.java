 ...
 */
    protected boolean canAdd(PaletteContainer container, PaletteEntry entry) {
        return container.getUserModificationPermission() == PaletteEntry.PERMISSION_FULL_MODIFICATION && container.acceptsType(entry.getType());
    }
 ...
 */
    public boolean canMoveDown(PaletteEntry entry) {
        ...
        if (parent.getChildren().indexOf(entry) + 1 != parent.getChildren().size()) {
            ...
            if (parentPermission != PaletteEntry.PERMISSION_FULL_MODIFICATION || parent.getParent() == null) return false;
            if (canAdd(parent.getParent(), entry)) return true;
            ...
            for (int i = parentIndex + 1; i < children.size(); i++) {
                ...
                if (parentSibling instanceof PaletteContainer) {
                    if (canAdd((PaletteContainer) parentSibling, entry)) return true;
                }
            }
            return false;
        }
 ...
 */
    public void performMoveDown(PaletteEntry entry) {
        ...
        if (!parent.moveDown(entry)) {
            ...
            if (canAdd(newParent, entry)) insertionIndex = newParent.getChildren().indexOf(parent) + 1; else {
                ...
                for (int i = parents.indexOf(parent) + 1; i < parents.size(); i++) {
                    ...
                    if (parentSibling instanceof PaletteContainer) {
                        ...
                        if (canAdd(newParent, entry)) break;
                    }
                }
            }
            parent.remove(entry);
 ...
 */
    public void performMoveUp(PaletteEntry entry) {
        ...
        if (!parent.moveUp(entry)) {
            ...
            PaletteContainer newParent = parent.getParent();
            int insertionIndex = 0;
            if (canAdd(newParent, entry)) insertionIndex = newParent.getChildren().indexOf(parent); else {
                List parents = newParent.getChildren();
