...
public class EducationalFacilitiesGenerator {
    ...
    public static void main(String[] args) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException {
        ...
        while (resultZone.next()) allPostalCodes.put(resultZone.getInt(1), new CoordImpl(resultZone.getDouble(2), resultZone.getDouble(3)));
        resultZone.close();
        ...
        while (educationalFacilitiesResult.next()) {
            if (educationalFacilitiesResult.getDouble(2) != 0) {
                ...
                Coord coord = null;
                if (postalCode == 0) postalCode = getPostalCode(allPostalCodes, new CoordImpl(lon, lat));
                ActivityFacility facility = facilities.getFacilities().get(new IdImpl(postalCode));
                if (facility == null) {
                    if (educationalFacilitiesResult.getDouble(4) != 0) coord = new CoordImpl(lon, lat); else coord = allPostalCodes.get(postalCode);
                    if (coord != null) facility = facilities.createFacility(new IdImpl(postalCode), coordinateTransformation.transform(coord)); else System.out.println(educationalFacilitiesResult.getInt(6));
                }
                if (facility != null) {
                    ActivityOption option = facility.getActivityOptions().get("edu");
                    double capacity = 0;
                    if (option == null) option = ((ActivityFacilityImpl) facility).createActivityOption("edu"); else capacity = option.getCapacity();
                    option.setCapacity((double) ((int) (capacity + educationalFacilitiesResult.getDouble(2))));
                    if (educationalFacilitiesResult.getString(7) != null && (educationalFacilitiesResult.getString(7).toLowerCase().contains("school") || educationalFacilitiesResult.getString(7).toLowerCase().contains("college") || educationalFacilitiesResult.getString(7).toLowerCase().contains("primary") || educationalFacilitiesResult.getString(7).toLowerCase().contains("secondary") || educationalFacilitiesResult.getString(7).toLowerCase().contains("kinder"))) option.addOpeningTime(new OpeningTimeImpl(DayType.wkday, 18000, 64800)); else option.addOpeningTime(new OpeningTimeImpl(DayType.wkday, 18000, 86400));
                    option = facility.getActivityOptions().get("profess");
                    capacity = 0;
                    if (option == null) option = ((ActivityFacilityImpl) facility).createActivityOption("profess"); else capacity = option.getCapacity();
    ...
    private static void writeFacilitiesOnDatabase(ActivityFacilitiesImpl facilities) throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, NoConnectionException {
        ...
        for (ActivityFacility facility : facilities.getFacilities().values()) {
            ...
            } else idFacility = facilityResult.getInt(1);
            ...
            for (ActivityOption option : facility.getActivityOptions().values()) {
                ...
                if (!optionResult.next()) dataBaseFacilities.executeStatement("INSERT INTO Activity_options (type,facility_id,capacity) VALUES ('" + option.getType() + "'," + idFacility + "," + option.getCapacity() + ")"); else dataBaseFacilities.executeStatement("UPDATE Activity_options SET capacity=capacity+" + optionResult.getDouble(1) + " WHERE type='" + option.getType() + "' AND facility_id =" + idFacility);
                for (OpeningTime openingTime : option.getOpeningTimes(DayType.wkday)) dataBaseFacilities.executeStatement("INSERT INTO Opening_times (day_type,start_time,end_time,type,facility_id) VALUES ('" + DayType.wkday + "'," + openingTime.getStartTime() + "," + openingTime.getEndTime() + ",'" + option.getType() + "'," + idFacility + ")");
            }
        }
    }
