...
public class SbbProfilePanel extends Composite implements SelectionListener, DataStoreChangeListener {
    ...
    private static final int[] COLUMN_EDITORS = { EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_TEXT };
    ...
    public SbbProfilePanel(Composite parent, int style) {
        ...
        availableProfiles = new EditableTableViewer(this, SWT.V_SCROLL | SWT.BORDER | SWT.SINGLE, new String[] { "Name", "Version", "Vendor" }, new int[] { EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE }, new String[][] { {}, {}, {} });
    ...
    private void setScopedName(HashMap item) {
        ...
        if (scopedName == null) {
            try {
                ...
                if (scopedName.endsWith("CMP")) scopedName = scopedName.substring(0, scopedName.lastIndexOf("CMP"));
    ...
    public void selectProfile(ProfileSpecJarXML xml, ProfileSpecXML profile) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (name.equals(profile.getName()) && vendor.equals(profile.getVendor()) && version.equals(profile.getVersion()) && xml.equals(storedXML)) {
                store.remove(map);
                setScopedName(map);
    ...
    public HashMap[] getSelectedProfiles() {
        ...
        for (int i = 0; i < profiles.length; i++) out[i] = (HashMap) profiles[i];
        return out;
    }
    ...
    public void populateAddressCombo() {
        ...
        if (origIndex >= 0) selection = addressCombo.getItem(origIndex); else selection = "None";
        Object profiles[] = selectedProfiles.getStore().getElements();
        ...
        for (int i = 0; i < profiles.length; i++) {
            ...
            if (options[i + 1].equals(selection)) newIndex = i + 1;
        }
    ...
    public String getAddressProfileSpec() {
        if (addressCombo.getSelectionIndex() >= 0) return addressCombo.getItem(addressCombo.getSelectionIndex());
        return null;
    }
