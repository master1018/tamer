...
public abstract class Agent {
    ...
    protected Set<LogicGoal> goals;

    private static EnvironmentInterfaceStandard ei;

    private static Collection<String> agents;
    ...
    public Agent(String name, String team) {
        this.name = name;
        this.team = team;
        if (agents.contains(name)) throw new AssertionError("duplicate agent name \"" + name);
        agents.add(name);
    ...
    public boolean equals(Object obj) {
        ...
        if (obj.getClass().equals(this.getClass()) == false) return false;
        if (((Agent) obj).getName().equals(getName())) return true;
        return false;
    }
    ...
    protected void removeGoals(String predicate) {
        ...
        for (LogicGoal g : goals) {
            if (g.getPredicate().equals(predicate)) remove.add(g);
        }
