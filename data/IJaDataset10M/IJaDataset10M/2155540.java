...
public class Path extends AbstractShape {
    ...
    public Sector getSector() {
        if (this.sector == null && this.positions != null) this.sector = Sector.boundingSector(this.positions);
        return this.sector;
    }
    ...
    protected boolean mustRegenerateGeometry(DrawContext dc) {
        ...
        if (this.getAltitudeMode() == WorldWind.ABSOLUTE && this.getCurrentPathData().getGlobeStateKey() != null && this.getCurrentPathData().getGlobeStateKey().equals(dc.getGlobe().getGlobeStateKey(dc))) return false;
    ...
    protected void determineActiveAttributes() {
        ...
        if (this.activeAttributes != null && this.activeAttributes.isDrawInterior() != isDrawInterior) this.getCurrentData().setExpired(true);
    }
    ...
    protected boolean doMakeOrderedRenderable(DrawContext dc) {
        ...
        if (pathData.renderedPath.limit() > previousSize && this.shouldUseVBOs(dc)) this.clearCachedVbos(dc);
    ...
    protected void addOrderedRenderable(DrawContext dc) {
        if (this.isSurfacePath()) {
            dc.addOrderedRenderable(this, true);
        } else {
    ...
    protected void doDrawOutline(DrawContext dc) {
        ...
        try {
            ...
            if (this.shouldUseVBOs(dc)) {
                ...
                if (vboIds != null) this.doDrawOutlineVBO(dc, vboIds, this.getCurrentPathData()); else this.doDrawOutlineVA(dc, this.getCurrentPathData());
            } else {
    ...
    protected void doDrawOutlineVBO(DrawContext dc, int[] vboIds, PathData pathData) {
        ...
        if (pathData.hasExtrusionPoints && this.isDrawVerticals()) this.drawVerticalOutlineVBO(dc, vboIds, pathData);
    ...
    protected void doDrawOutlineVA(DrawContext dc, PathData pathData) {
        ...
        gl.glDrawArrays(GL.GL_LINE_STRIP, 0, pathData.renderedPath.limit() / (pathData.hasExtrusionPoints ? 6 : 3));
        if (pathData.hasExtrusionPoints && this.isDrawVerticals()) this.drawVerticalOutlineVA(dc, pathData);
    }
    ...
    protected void drawVerticalOutlineVBO(DrawContext dc, int[] vboIds, PathData pathData) {
        ...
        if (polePositions == null || polePositions.limit() < 1) return;
        GL gl = dc.getGL();
    ...
    protected void doDrawInterior(DrawContext dc) {
        if (this.shouldUseVBOs(dc)) {
            int[] vboIds = this.getVboIds(dc);
            if (vboIds != null) this.doDrawInteriorVBO(dc, vboIds, this.getCurrentPathData()); else this.doDrawInteriorVA(dc, this.getCurrentPathData());
        } else {
    ...
    protected void computePath(DrawContext dc, List<Position> positions, PathData pathData) {
        ...
        if (this.getAltitudeMode() == WorldWind.CLAMP_TO_GROUND) path = this.computePointsRelativeToTerrain(dc, positions, 0d, path, pathData); else if (this.getAltitudeMode() == WorldWind.RELATIVE_TO_GROUND) path = this.computePointsRelativeToTerrain(dc, positions, null, path, pathData); else path = this.computeAbsolutePoints(dc, positions, path, pathData);
        path.flip();
    ...
    protected FloatBuffer computePointsRelativeToTerrain(DrawContext dc, List<Position> positions, Double altitude, FloatBuffer path, PathData pathData) {
        ...
        if (path == null || path.capacity() < numPoints * 3) path = BufferUtil.newFloatBuffer(3 * numPoints);
        path.clear();
        for (Position pos : positions) {
            ...
            if (extrudeIt) this.appendTerrainPoint(dc, pos, path, pathData);
        }
        return path;
    }
    ...
    protected FloatBuffer computeAbsolutePoints(DrawContext dc, List<Position> positions, FloatBuffer path, PathData pathData) {
        ...
        if (dc.getVerticalExaggeration() != 1) {
            ...
            for (Position pos : positions) {
                Vec4 pt = globe.computePointFromPosition(pos.getLatitude(), pos.getLongitude(), ve * (pos.getAltitude()));
                path.put((float) (pt.x - referencePoint.x));
                path.put((float) (pt.y - referencePoint.y));
                path.put((float) (pt.z - referencePoint.z));
                if (this.isExtrude()) this.appendTerrainPoint(dc, pos, path, pathData);
            }
        } else {
            for (Position pos : positions) {
    ...
    protected void makeTessellatedPositions(DrawContext dc, PathData pathData) {
        ...
        if (pathData.polePositions == null || pathData.polePositions.capacity() < this.numPositions * 2) pathData.polePositions = BufferUtil.newIntBuffer(this.numPositions * 2); else pathData.polePositions.clear();
        Iterator<? extends Position> iter = this.positions.iterator();
        Position posA = iter.next();
        this.addTessellatedPosition(posA, true, pathData);
        Vec4 ptA = this.computePoint(dc.getTerrain(), posA);
        for (int i = 1; i <= this.numPositions; i++) {
            Position posB;
            if (i < this.numPositions) posB = iter.next(); else break;
            Vec4 ptB = this.computePoint(dc.getTerrain(), posB);
            if (this.isSmall(dc, ptA, ptB, 8) || !this.isSegmentVisible(dc, posA, posB, ptA, ptB)) this.addTessellatedPosition(posB, true, pathData); else this.makeSegment(dc, posA, posB, ptA, ptB, pathData);
            posA = posB;
            ptA = ptB;
        }
    }
    ...
    protected void addTessellatedPosition(Position pos, boolean polePosition, PathData pathData) {
        ...
        pathData.tessellatedPositions.add(pos);
    }
    ...
    protected void makeSegment(DrawContext dc, Position posA, Position posB, Vec4 ptA, Vec4 ptB, PathData pathData) {
        ...
        if (arcLength <= 0 || (this.getPathType() == AVKey.LINEAR && !this.isFollowTerrain())) {
            if (!ptA.equals(ptB)) this.addTessellatedPosition(posB, true, pathData);
            return;
        }
        Angle segmentAzimuth = null;
        Angle segmentDistance = null;
        for (double s = 0, p = 0; s < 1; ) {
            if (this.isFollowTerrain()) p += this.terrainConformance * dc.getView().computePixelSizeAtDistance(ptA.distanceTo3(dc.getView().getEyePoint())); else p += arcLength / this.numSubsegments;
            Position pos;
            s = p / arcLength;
            if (s >= 1) {
                pos = posB;
    ...
    protected double computeSegmentLength(DrawContext dc, Position posA, Position posB) {
        ...
        if (this.getAltitudeMode() == WorldWind.CLAMP_TO_GROUND) return ang.radians * (dc.getGlobe().getRadius());
    ...
    protected Vec4 computeReferenceCenter(DrawContext dc) {
        ...
        return dc.getGlobe().computePointFromPosition(pos.getLatitude(), pos.getLongitude(), dc.getVerticalExaggeration() * pos.getAltitude());
    }
    ...
    protected void doExportAsKML(XMLStreamWriter xmlWriter) throws IOException, XMLStreamException {
        ...
        for (Position position : this.positions) {
            xmlWriter.writeCharacters(String.format("%f,%f,%f ", position.getLongitude().getDegrees(), position.getLatitude().getDegrees(), position.getElevation()));
        }
        xmlWriter.writeEndElement();
        xmlWriter.writeEndElement();
        xmlWriter.writeEndElement();
