...
public class View extends JPanel implements ActionListener {
    ...
    public void restartView() throws JuggleExceptionUser, JuggleExceptionInternal {
        if (subview != null) subview.restartView();
    }

    public void restartView(JMLPattern p, AnimatorPrefs c) throws JuggleExceptionUser, JuggleExceptionInternal {
        if (p != null) this.pat = p;
        if (c != null) this.jc = c;
        if (subview != null) subview.restartView(p, c);
    }

    public Dimension getAnimatorSize() {
        if (subview != null) return subview.getAnimatorSize();
        return null;
    }

    public void dispose() {
        if (subview != null) subview.dispose();
        subview = null;
    }
    ...
    public boolean getPaused() {
        if (subview != null) return subview.getPaused();
        return false;
    }

    public void setPaused(boolean pause) {
        if (subview != null) subview.setPaused(pause);
    }
    ...
    public JMenu createFileMenu() {
        ...
        for (int i = 0; i < fileItems.length; i++) {
            if (fileItems[i] == null) filemenu.addSeparator(); else {
                JMenuItem fileitem = new JMenuItem(guistrings.getString(fileItems[i].replace(' ', '_')));
                if (fileShortcuts[i] != ' ') fileitem.setAccelerator(KeyStroke.getKeyStroke(fileShortcuts[i], Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
    ...
    protected static final char[] viewShortcuts = { '1', '2', '3', ' ', ' ', 'P' };
    ...
    public JMenu createViewMenu() {
        ...
        for (int i = 0; i < viewItems.length; i++) {
            if (viewItems[i] == null) {
                ...
                if (viewShortcuts[i] != ' ') viewitem.setAccelerator(KeyStroke.getKeyStroke(viewShortcuts[i], Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        try {
            if (command.equals("close")) doMenuCommand(FILE_CLOSE); else if (command.equals("saveas")) doMenuCommand(FILE_SAVE); else if (command.equals("savegifanim")) {
                if (jugglinglab.core.Constants.INCLUDE_GIF_SAVE) doMenuCommand(FILE_GIFSAVE);
            } else if (command.equals("savehtml")) doMenuCommand(FILE_HTMLSAVE); else if (command.equals("restart")) doMenuCommand(VIEW_RESTART); else if (command.equals("prefs")) doMenuCommand(VIEW_ANIMPREFS); else if (command.equals("simple")) {
                    ...
                    try {
                        ...
                        if (option == JFileChooser.APPROVE_OPTION) {
                            if (PlatformSpecific.getPlatformSpecific().getSelectedFile() != null) {
                                ...
                                if (prefs.length() != 0) pw.println("<param name=\"animprefs\" value=\"" + JMLNode.xmlescape(prefs) + "\">");
    ...
    protected int getViewMode() {
        if (subview == null) return VIEW_NONE;
        if (subview instanceof NormalView) return VIEW_SIMPLE;
        if (subview instanceof EditView) return VIEW_EDIT;
        if (subview instanceof JMLView) return VIEW_JML;
        if (subview instanceof SelectionView) return VIEW_SELECTION;
        return VIEW_NONE;
    }
}
