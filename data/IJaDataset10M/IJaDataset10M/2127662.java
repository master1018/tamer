...
public class PhoneNumberQuery extends BaseQuery<PhoneNumber> {
    ...
    public Expression<Boolean> buildWhere(AbstractSearchAction action, CriteriaBuilder cb, Root<PhoneNumber> from, Expression<Boolean> base) {
        ...
        if (id != null) base = addAndExpression(cb, base, cb.equal(from.get(PhoneNumber_.id), id));
        String number = handler.getString(PhoneNumberConsts.s_number);
        if (number != null) base = addAndExpression(cb, base, cb.like(cb.upper(from.get(PhoneNumber_.number)), number.toUpperCase() + "%"));
        Relation type = handler.getRelation(PhoneNumberConsts.s_type);
        if (type != null) base = addAndExpression(cb, base, cb.equal(from.get(PhoneNumber_.type).get(PhoneNumberType_.id), type.getId()));
        return base;
    }

    public Order getOrderExpression(AbstractSearchAction action, CriteriaBuilder cb, Root<PhoneNumber> from) {
        Order o;
        ...
        if (action.isDescending() == null) o = cb.asc(orderExpr); else if (action.isDescending()) o = cb.desc(orderExpr); else o = cb.asc(orderExpr);
        return o;
    }

    public Expression<Boolean> getSearchExpression(CriteriaBuilder cb, Path<PhoneNumber> from, String value) {
        Expression<Boolean> expr = null;
        expr = addOrExpression(cb, expr, cb.like(cb.upper(from.get(PhoneNumber_.number)), value.toUpperCase() + "%"));
        return expr;
    }
}
