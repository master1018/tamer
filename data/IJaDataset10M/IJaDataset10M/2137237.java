...
public class RootIf extends IfBlock {
    ...
    private boolean endsWithEOFMatcher() {
        ...
        for (IfBlock block : last) {
            if (block.matcher == null) return true;
        }
        return false;
    }
    ...
    private String readMethod() {
        InputType inputType = analalizeInput();
        if (inputType.characterOnly()) return "position==limit ? marker : input[position]"; else return "codePoint()";
    }
    ...
    private void generateRead(Printer printer) {
        ...
        if (first.matcher != null && !first.usesFinishAll() && first.height() == 1) {
            ...
            if (readChar) {
                ...
                if (SyntaxClass.DEBUGGABLE) body.add(currentNode);
                body.add(state.breakStatement());
                printer.printlnIf("(ch=" + readMethod() + ")==EOC", body);
                InputType inputType = analalizeInput();
                if (inputType.characterOnly() && inputType.newLine && inputType.consumes) printer.println("increment = 1;");
            } else {
                if (SyntaxClass.DEBUGGABLE) printer.println(currentNode);
            }
        }
    }
    ...
    public void fillLookAhead(Printer printer, int prevHeight, int curHeight) {
        ...
        printer.printlns(prefix + "(" + condition + "){", PLUS, "if((ch=codePoint())==EOC)", PLUS, state.breakStatement(), MINUS, "addToLookAhead(ch);", MINUS, "}");
    }
}
