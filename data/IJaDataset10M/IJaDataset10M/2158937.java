...
public class Join implements TransactionConstants, AbortParticipant, Configurable, XmlConfigurable {
    ...
    public int prepare(long id, Serializable o) {
        return mergeActions(joinRunners(prepare(createRunners(id, o))));
    }

    public int prepareForAbort(long id, Serializable o) {
        return mergeActions(joinRunners(prepareForAbort(createRunners(id, o))));
    }
    ...
    private Runner[] prepare(Runner[] runners) {
        for (int i = 0; i < runners.length; i++) runners[i].prepare();
        return runners;
    }

    private Runner[] prepareForAbort(Runner[] runners) {
        for (int i = 0; i < runners.length; i++) runners[i].prepareForAbort();
        return runners;
    }

    private Runner[] commit(Runner[] runners) {
        for (int i = 0; i < runners.length; i++) runners[i].commit();
        return runners;
    }

    private Runner[] abort(Runner[] runners) {
        for (int i = 0; i < runners.length; i++) runners[i].abort();
        return runners;
    }
    ...
    private int mergeActions(Runner[] runners) {
        ...
        for (int i = 0; i < runners.length; i++) {
            ...
            if ((action & PREPARED) == ABORTED) prepared = false;
            if ((action & READONLY) != READONLY) readonly = false;
            if ((action & NO_JOIN) != NO_JOIN) no_join = false;
        }
        return (prepared ? PREPARED : ABORTED) | (no_join ? NO_JOIN : 0) | (readonly ? READONLY : 0);
    }
    ...
    public static class Runner implements Runnable {
        ...
        public void run() {
            switch(mode) {
                ...
                case PREPARE_FOR_ABORT:
                    if (p instanceof AbortParticipant) rc = ((AbortParticipant) p).prepareForAbort(id, ctx);
                    break;
                case COMMIT:
                    if ((rc & NO_JOIN) == 0) p.commit(id, ctx);
                    break;
                case ABORT:
                    if ((rc & NO_JOIN) == 0) p.abort(id, ctx);
                    break;
            }
        }
        ...
        private void createThread(int m) {
            ...
            t.setName(MODES[m] + this.getClass().getName() + ":" + p.getClass().getName());
            this.mode = m;
            t.start();
        }
    }
