...
public class PhantomRefAtomCache implements HGAtomCache {
    ...
    private void processRefQueue() throws InterruptedException {
        ...
        while (ref != null) {
            graph.getEventManager().dispatch(graph, new HGAtomEvictEvent(ref, ref.fetchRef()));
            lock.writeLock().lock();
            try {
    ...
    @SuppressWarnings("unchecked")
    public HGLiveHandle atomRead(HGPersistentHandle pHandle, Object atom, final HGAtomAttrib attrib) {
        if (closing) {
        ...
        try {
            ...
            if (h != null) return h;
            if ((attrib.getFlags() & HGSystemFlags.MANAGED) == 0) h = new PhantomHandle(atom, pHandle, attrib.getFlags(), refQueue); else h = new PhantomManagedHandle(atom, pHandle, attrib.getFlags(), refQueue, attrib.getRetrievalCount(), attrib.getLastAccessTime());
            atoms.put(atom, h);
    ...
    public HGLiveHandle atomRefresh(HGLiveHandle handle, Object atom, boolean replace) {
        if (closing) {
            if (handle instanceof PhantomHandle) ((PhantomHandle) handle).storeRef(atom); else ((TempLiveHandle) handle).setRef(atom);
            return handle;
        }
        if (handle == null) throw new NullPointerException("atomRefresh: handle is null.");
        lock.writeLock().lock();
        try {
    ...
    public void close() {
        ...
        while (phantomCleanupThread.isAlive()) try {
            phantomCleanupThread.join();
        ...
        if (incidenceCache instanceof CloseMe) ((CloseMe) incidenceCache).close();
        atoms.clear();
        liveHandles.clear();
    }
    ...
    public void remove(HGHandle handle) {
        ...
        try {
            ...
            if (handle instanceof HGLiveHandle) lhdl = (HGLiveHandle) handle; else lhdl = get(handle.getPersistent());
            if (lhdl != null) {
                atoms.remove(lhdl.getRef());
                if (lhdl instanceof PhantomHandle) ((PhantomHandle) lhdl).storeRef(null);
    ...
    public void freeze(HGLiveHandle handle) {
        ...
        if (atom != null) synchronized (frozenAtoms) {
