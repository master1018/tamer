...
public class GeoEditorThread extends Thread {
    ...
    public void run() {
        try {
            ...
            int timer = 0;
            while (_working) {
                if (!isConnected()) _working = false;
                if (_mode == 2 && timer > _sendDelay) {
                    for (L2PcInstance gm : _gms) if (!gm.getClient().getConnection().isClosed()) sendGmPosition(gm); else _gms.remove(gm);
                    timer = 0;
                }
                try {
                    sleep(100);
                    if (_mode == 2) timer += 100;
                } catch (Exception e) {
                }
            }
    ...
    public void setTimer(int value) {
        if (value < 500) _sendDelay = 500; else if (value > 60000) _sendDelay = 60000; else _sendDelay = value;
    }

    public void addGM(L2PcInstance gm) {
        if (!_gms.contains(gm)) _gms.add(gm);
    }

    public void removeGM(L2PcInstance gm) {
        if (_gms.contains(gm)) _gms.remove(gm);
    }

    public boolean isSend(L2PcInstance gm) {
        if (_mode == 1 && _gms.contains(gm)) return true;
        return false;
    }
