...
public class Injector {
    ...
    public List<Field> getAllSeamInjectedFields(final Object instance) {
        return getAllFieldsWithAnnotation(instance, In.class);
    }
    ...
    private List<Field> getAllFieldsWithAnnotation(final Object instance, final Class<? extends Annotation> annotation) {
        Field[] fields = instance.getClass().getDeclaredFields();
        List<Field> result = new ArrayList<Field>();
        for (int x = 0; x < fields.length; ++x) {
            if (fields[x].getAnnotation(annotation) != null) {
                result.add(fields[x]);
            }
        }
        return result;
    ...
    private final boolean checkClustered = false;
     ...
     */
    public final <T> void injectEjb(final Object injectionTarget, final Class<T> businessInterface, final T ejbMock) {
    ...
    private <T> void injectEJB(final Object injectionTarget, final Class<T> businessInterface, final T ejbMock, final Class<? extends Object> currentClass) {
    ...
    /**
     * Injects a predefined object to all fields with annotation \@Logger .
     *
     * @param target
     *            -- target object of injection
     */
    public final MockLog injectSeamLogger(final Object target) {
        MockLog logMock = new MockLog();
        injectLogger(target, logMock);
        return logMock;
    }

    /**
     * Injects a given mock object to all fields with annotation \@Logger .
     *
     * @param target
     *            -- target object of injection
     * @param log
     *            -- object that is injected
     */
    public final void injectLogger(final Object target, final Log log) {
        injectAllDeclaredFields(target, Logger.class, log);
    }
     ...
     *            -- object that is injected
     */
    public final void injectRequestParameter(final Object injectionTarget, final String requestParameterName, final Object object) {
        injectAllDeclaredFields(injectionTarget, RequestParameter.class, object, new AnnotationChecker<RequestParameter>() {

            public boolean checkAnnotation(final Field field, final RequestParameter annotation) {
                return annotation.value().equals(requestParameterName);
            }
        });
    }
    ...
    /**
     * Injects on the field with a given name and which is annotated with \@In .
     * annotation of Seam
     *
     * @param injectionTarget
     *            -- target object of injection
     ...
     *            -- object that is injected
     */
    public final void injectSeamIn(final Object injectionTarget, final String name, final Object injectedObject) {
        assert injectedObject != null;
        assert name != null;
        final AnnotationChecker<In> seamInChecker = new AnnotationChecker<In>() {

            public boolean checkAnnotation(final Field field, final In annotation) {
                boolean isOK = false;
                if (In.class.isInstance(annotation)) {
                    final In inAnnotation = In.class.cast(annotation);
                    isOK = (name.equals(inAnnotation.value()) || name.equals(field.getName()));
                }
                return isOK;
            }
        };
        injectAllDeclaredFields(injectionTarget, In.class, injectedObject, seamInChecker);
    }
}
