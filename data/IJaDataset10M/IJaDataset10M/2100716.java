...
public final class Board {
    ...
    private static final Board BOARD_Y = new Board();

    private static final Board BOARD_X = new Board();

    private static final Board BOARD_DISP = new Board();
    ...
    */
    private Board() {
        int i, j;
        for (i = 0; i < field.length; i++) for (j = 0; j < field[i].length; j++) {
            field[i][j] = new Node();
        }
    ...
    */
    public void setSize(final int xSize, final int ySize) {
        if (xSize > MAXDIM || ySize > MAXDIM || xSize < MINDIM || ySize < MINDIM) {
            throw new IllegalArgumentException("Size has to be between " + MINDIM + " and " + MAXDIM + ".");
        }
        xsize = xSize;
        ysize = ySize;
    }
    ...
    */
    public boolean pinAllowed(final int xin, final int yin, final int player) {
        return !((player == XPLAYER && (yin < 1 || yin > ysize - 2 || xin < 0 || xin > xsize - 1)) || (player == YPLAYER && (yin < 0 || yin > ysize - 1 || xin < 1 || xin > xsize - 2)) || (field[xin + 3][yin + 3].value != 0));
    }
    ...
    */
    public boolean isConnected(final int xa, final int ya, final int xb, final int yb) {
        if (Math.abs(xa - xb) >= 3 || Math.abs(ya - yb) >= 3 || Math.abs(xa - xb) + Math.abs(ya - yb) != 3) {
    ...
    */
    public static void copyYtoDisplay() {
        ...
        for (int i = 0; i < BOARD_Y.field.length; i++) for (int j = 0; j < BOARD_Y.field[i].length; j++) {
