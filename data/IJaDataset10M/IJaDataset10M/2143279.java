...
public class DatabaseSchemaView extends JPanel implements EventBrokerListener {
    ...
    public Table getTable() {
        final TableInfo tableInfo = (TableInfo) this.tableComboBox.getSelectedItem();
        if (tableInfo == null) {
            return null;
        }
    ...
    public Column getKey() {
        final ColumnInfo columnInfo = (ColumnInfo) this.columnComboBox.getSelectedItem();
        if (columnInfo == null) {
            return null;
        }
    ...
    public void setKey(final String tableName, final String key) {
        for (int i = 0; i < this.tableComboBox.getItemCount(); i++) {
            final Table table = ((TableInfo) this.tableComboBox.getItemAt(i)).getTable();
            if (table.getDisplayName().equals(tableName)) {
                ...
                for (int j = 0; j < this.columnComboBox.getItemCount(); j++) {
                    final Column col = ((ColumnInfo) this.columnComboBox.getItemAt(j)).getColumn();
