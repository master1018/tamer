...
public class VJClassifierValue extends VJClassifierLevoy {
    ...
    public VJAlphaColor alphacolor(VJValue v, VJGradient g) {
        int intensity = v.intvalue & maskIntensity;
        return new VJAlphaColor(opacityTable[intensity], (lut[0] & 0xff), (lut[1] & 0xff), (lut[2] & 0xff));
    }

    public String toLongString() {
        return "Value (" + ((does() == RGB) ? "RGB" : "grays") + ") classifier. Makes voxels more opaque " + " the closer their intensity is to threshold (" + threshold + "). The gradient is not used. " + "Voxel colors determined only from LUT.";
    }
    ...
    protected double opacityCompute(double dfxi, double intensity, double threshold, double width) {
        if (intensity == threshold) return 1; else return Math.max(0, 1.0 - (1.0 / width * 20) * Math.abs(threshold - intensity));
    }
}
