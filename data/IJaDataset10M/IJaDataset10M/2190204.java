...
public class AmountPanel extends ConsistencyCheckedPanel {
    ...
    public AmountPanel() {
        ...
        setBorder(BorderFactory.createTitledBorder(null, LocalizationData.get("Transaction.amount"), TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), new Color(51, 51, 51)));
    ...
    private JRadioButton getAmountEquals() {
        if (amountEquals == null) {
            ...
            amountEquals.setText(LocalizationData.get("CustomFilterPanel.amount.equals"));
            amountEquals.setToolTipText(LocalizationData.get("CustomFilterPanel.amount.equals.toolTip"));
    ...
    private JRadioButton getAmountBetween() {
        if (amountBetween == null) {
            ...
            amountBetween.setText(LocalizationData.get("CustomFilterPanel.amout.between"));
            amountBetween.setToolTipText(LocalizationData.get("CustomFilterPanel.amout.between.toolTip"));
    ...
    private CurrencyWidget getMinAmountWidget() {
        if (minAmountWidget == null) {
            ...
            minAmountWidget.setToolTipText(LocalizationData.get("CustomFilterPanel.amout.minimum"));
    ...
    private CurrencyWidget getMaxAmountWidget() {
        if (maxAmountWidget == null) {
            ...
            maxAmountWidget.setToolTipText(LocalizationData.get("CustomFilterPanel.amount.maximum"));
    ...
    private JRadioButton getAmountAll() {
        if (amountAll == null) {
            ...
            amountAll.setText(LocalizationData.get("CustomFilterPanel.amount.all"));
            amountAll.setToolTipText(LocalizationData.get("CustomFilterPanel.amount.all.toolTip"));
    ...
    protected String computeInconsistencyCause() {
        if (!getMinAmountWidget().isContentValid()) return LocalizationData.get("CustomFilterPanel.error.amountMimimum");
        if (!getMaxAmountWidget().isContentValid()) return LocalizationData.get("CustomFilterPanel.error.amountMaximum");
        if ((getMinAmountWidget().getValue() != null) && (getMaxAmountWidget().getValue() != null) && (getMinAmountWidget().getValue() > getMaxAmountWidget().getValue())) {
