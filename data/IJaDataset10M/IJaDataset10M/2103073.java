...
public class NimRODBorders extends MetalBorders {
    ...
    public static Border getButtonBorder() {
        if (butBorder == null) {
            butBorder = new BorderUIResource.CompoundBorderUIResource(new NimRODBorders.NimRODButtonBorder(), new BasicBorders.MarginBorder());
        }
        return butBorder;
    }
    ...
    public static class NimRODCellFocusBorder extends AbstractBorder implements UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            Color col = NimRODUtils.getColorTercio(NimRODLookAndFeel.getControlTextColor(), NimRODLookAndFeel.getFocusColor());
            g.setColor(col);
    ...
    public static class NimRODRolloverButtonBorder extends AbstractBorder implements UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            if (model.isRollover()) {
                ...
                if (model.isPressed()) {
                    grad = new GradientPaint(0, 0, NimRODUtils.getSombra(), 0, c.getHeight() / 2, NimRODUtils.getBrillo());
                } else {
                    grad = new GradientPaint(0, 0, NimRODUtils.getBrillo(), 0, c.getHeight(), NimRODUtils.getSombra());
                }
                Graphics2D g2D = (Graphics2D) g;
                g2D.setPaint(grad);
                g2D.fill(boton);
    ...
    public static class NimRODInternalFrameBorder extends AbstractBorder implements UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            if (((JInternalFrame) c).isSelected()) {
                ...
                grad = new GradientPaint(0, 0, NimRODLookAndFeel.getControl(), w, 0, NimRODLookAndFeel.getControlDarkShadow());
