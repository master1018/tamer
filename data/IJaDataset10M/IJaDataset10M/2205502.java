 ...
 */
public class PruneFromNodeCommand<N, ET extends DirectionalEdge<N>> implements Command {
	 ...
	 */
    public PruneFromNodeCommand(String editName, DirectionalGraph<N, ET> g, N pruneSourceNode) {
        if (g == null) {
    ...
    public UndoableEdit execute() {
        if (!graph.containsNode(node)) {
            throw new UnsupportedGraphOperationException("Graph did not contain GraphNode to be deleted");
        }
    ...
    private void pruneNode(N pruneNode) {
        ...
        for (ET edge : edges) {
            ...
            for (N checkNode : nodeList) {
                if (!checkNode.equals(pruneNode) && edge.getNodeInterfaceType(checkNode) == DirectionalEdge.SINK && !graph.hasInwardEdge(checkNode)) {
                    pruneNode(checkNode);
                }
            }
        }
