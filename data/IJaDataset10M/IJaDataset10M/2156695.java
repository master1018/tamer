...
public class FuzzySignalContrMultTablesPhaseEnd extends FuzzySignalController {

    List<ExtensionTableWIthQueueLevel> tablesList;

    public FuzzySignalContrMultTablesPhaseEnd(ModelController modelController) {
    ...
    public void addPhase(SignalPhase signalPhase) {
        ...
        ((FuzzySignalPhaseMultTablesPhaseEnd) signalPhase).setListOfTables(tablesList);
    }

    public void setTableListOnAllPhases() {
        if (phaseList != null) {
            for (SignalPhase p : phaseList) {
                ((FuzzySignalPhaseMultTablesPhaseEnd) p).setListOfTables(tablesList);
            }
        }
    }
    ...
    public void paint(Graphics2D g2) {
        if (rectangle != null) {
            ...
            if (phase != null) {
                FuzzySignalPhaseMultTablesPhaseEnd fuzPhase = (FuzzySignalPhaseMultTablesPhaseEnd) phase;
