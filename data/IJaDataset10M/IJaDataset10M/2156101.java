...
public class Transformer extends ONDEXTransformer implements ArgumentNames {
    ...
    public ArgumentDefinition<?>[] getArgumentDefinitions() {
        return new ArgumentDefinition<?>[] { new StringArgumentDefinition(TAXID_ARG, TAXID_ARG_DESC, true, null, true), new BooleanArgumentDefinition(ANALYZE_QUALITY_ARG, ANALYZE_QUALITY_ARG_DESC, false, false), new StringArgumentDefinition(GO_FILE_ARG, GO_FILE_ARG_DESC, true, null, false), new StringArgumentDefinition(GOA_FILE_ARG, GOA_FILE_ARG_DESC, true, null, false), new StringArgumentDefinition(DBID_ARG, DBID_ARG_DESC, true, null, true) };
    }
    ...
    public void start() throws InvalidPluginArgumentException {
        ...
        for (int taxid : taxids) {
            if (qualityAnalysis) qualityAnalysis("before", taxid);
            for (String dbid : dbids) {
                postAnnotationForTaxid(taxid, dbid);
            }
            if (qualityAnalysis) qualityAnalysis("after", taxid);
        }
    ...
    private void fetchMetaData() {
        ...
        if (an_taxid == null) fireEventOccurred(new AttributeNameMissingEvent("Missing attribute name: TAXID", getName()));
        dataSource_uniprot = graph.getMetaData().getDataSource("UNIPROTKB");
        if (dataSource_uniprot == null) fireEventOccurred(new DataSourceMissingEvent("Missing DataSource: UNIPROTKB", getName()));
        dataSource_go = graph.getMetaData().getDataSource("GO");
        if (dataSource_go == null) fireEventOccurred(new DataSourceMissingEvent("Missing DataSource: GO", getName()));
    }
    ...
    private void fetchArguments() throws InvalidPluginArgumentException {
        ...
        if (file instanceof String) {
            ...
            if (fileTester.isAbsolute()) {
                if (fileTester.exists()) goLocation = fileTester.getAbsolutePath(); else fireEventOccurred(new DataFileMissingEvent("File " + fileDummy + " does not exist.", getName()));
            } else {
                if (fileDummy.startsWith(sep)) fileTester = new File(net.sourceforge.ondex.config.Config.ondexDir + fileDummy); else fileTester = new File(net.sourceforge.ondex.config.Config.ondexDir + sep + fileDummy);
                if (fileTester.exists()) goLocation = fileTester.getAbsolutePath(); else fireEventOccurred(new DataFileMissingEvent("File " + fileDummy + " does not exist.", getName()));
            }
        }
        ...
        if (file instanceof String) {
            ...
            File fileTester = new File(fileDummy);
            if (fileTester.isAbsolute()) {
                if (fileTester.exists()) goaLocation = fileTester.getAbsolutePath(); else fireEventOccurred(new DataFileMissingEvent("File " + fileDummy + " does not exist.", getName()));
            } else {
                if (fileDummy.startsWith(sep)) fileTester = new File(net.sourceforge.ondex.config.Config.ondexDir + fileDummy); else fileTester = new File(net.sourceforge.ondex.config.Config.ondexDir + sep + fileDummy);
                if (fileTester.exists()) goaLocation = fileTester.getAbsolutePath(); else fireEventOccurred(new DataFileMissingEvent("File " + fileDummy + " does not exist.", getName()));
            }
            ...
            for (ONDEXConcept c : graph.getConceptsOfAttributeName(an_taxid)) {
                ...
                if (curr_taxid == taxid) {
                    ...
                    for (ConceptAccession acc : c.getConceptAccessions()) {
                        if (acc.getElementOf().equals(dataSource_uniprot)) {
                            String uniprotID = acc.getAccession();
                            Iterator<Integer> goa_it = g.getGoaIteratorForProtID(uniprotID);
                            while (goa_it.hasNext()) {
                                int goa = goa_it.next();
                                goas.add(goa);
                            }
                        } else if (acc.getElementOf().equals(dataSource_go)) {
    ...
    private String buildGoStr(int go) {
        ...
        for (int i = 0; i < zeros; i++) zeroBuffer[i] = '0';
