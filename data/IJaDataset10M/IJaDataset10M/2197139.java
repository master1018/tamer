...
public abstract class StorableSerializer<S extends Storable> {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <S extends Storable> StorableSerializer<S> forType(Class<S> type) throws SupportException {
        synchronized (cCache) {
    ...
    @SuppressWarnings("unchecked")
    private static <S extends Storable> StorableSerializer<S> generateSerializer(Class<S> type) throws SupportException {
        Class<? extends S> abstractClass = StorableGenerator.getAbstractClass(type);
        ClassInjector ci = ClassInjector.create(type.getName(), abstractClass.getClassLoader());
        ClassFile cf = new ClassFile(ci.getClassName(), StorableSerializer.class);
        cf.markSynthetic();
        cf.setSourceFile(StorableSerializer.class.getName());
        cf.setTarget("1.5");
        cf.addDefaultConstructor();
        Map<String, ? extends StorableProperty<S>> propertyMap = StorableIntrospector.examine(type).getAllProperties();
        StorableProperty<S>[] properties;
        {
            List<StorableProperty<S>> list = new ArrayList<StorableProperty<S>>(propertyMap.size());
        ...
        {
            MethodInfo mi = cf.addMethod(Modifiers.PRIVATE.toStatic(true), ENCODE_METHOD_NAME, byteArrayType, new TypeDesc[] { userStorableType });
            CodeBuilder b = new CodeBuilder(mi);
            LocalVariable encodedVar = ges.buildDataEncoding(b, properties, b.getParameter(0), abstractClass, true, -1);
            b.loadLocal(encodedVar);
            b.returnValue(byteArrayType);
        }
        {
            MethodInfo mi = cf.addMethod(Modifiers.PRIVATE.toStatic(true), DECODE_METHOD_NAME, userStorableType, new TypeDesc[] { storageType, byteArrayType });
            CodeBuilder b = new CodeBuilder(mi);
            ...
            ges.buildDataDecoding(b, properties, instanceVar, abstractClass, true, -1, null, encodedVar);
            b.loadLocal(instanceVar);
            b.returnValue(storableType);
        }
        {
            TypeDesc dataOutputType = TypeDesc.forClass(DataOutput.class);
            MethodInfo mi = cf.addMethod(Modifiers.PUBLIC, WRITE_METHOD_NAME, null, new TypeDesc[] { storableType, dataOutputType });
            CodeBuilder b = new CodeBuilder(mi);
        ...
        final TypeDesc storableSerializerType = TypeDesc.forClass(StorableSerializer.class);
        {
            ...
            MethodInfo mi = cf.addMethod(Modifiers.PUBLIC, WRITE_METHOD_NAME, null, new TypeDesc[] { storableType, outputStreamType });
            CodeBuilder b = new CodeBuilder(mi);
            LocalVariable storableVar = b.getParameter(0);
            ...
            b.loadLocal(encodedVar);
            b.arrayLength();
            b.invokeStatic(storableSerializerType, "writeInt", null, new TypeDesc[] { outputStreamType, TypeDesc.INT });
            b.loadLocal(outVar);
            b.loadLocal(encodedVar);
        ...
        }
        {
            ...
            MethodInfo mi = cf.addMethod(Modifiers.PUBLIC, READ_METHOD_NAME, storableType, new TypeDesc[] { storageType, dataInputType });
            CodeBuilder b = new CodeBuilder(mi);
            ...
            b.loadLocal(byteArrayVar);
            b.invokeStatic(DECODE_METHOD_NAME, userStorableType, new TypeDesc[] { storageType, byteArrayType });
            b.returnValue(storableType);
        }
        {
            TypeDesc inputStreamType = TypeDesc.forClass(InputStream.class);
            MethodInfo mi = cf.addMethod(Modifiers.PUBLIC, READ_METHOD_NAME, storableType, new TypeDesc[] { storageType, inputStreamType });
            CodeBuilder b = new CodeBuilder(mi);
            LocalVariable storageVar = b.getParameter(0);
            LocalVariable inVar = b.getParameter(1);
            b.loadLocal(inVar);
            b.invokeStatic(storableSerializerType, "readInt", TypeDesc.INT, new TypeDesc[] { inputStreamType });
            b.newObject(byteArrayType);
            LocalVariable byteArrayVar = b.createLocalVariable(null, byteArrayType);
            b.storeLocal(byteArrayVar);
            b.loadLocal(inVar);
            b.loadLocal(byteArrayVar);
            b.invokeStatic(storableSerializerType, "readFully", null, new TypeDesc[] { inputStreamType, byteArrayType });
            b.loadLocal(storageVar);
            b.loadLocal(byteArrayVar);
            b.invokeStatic(DECODE_METHOD_NAME, userStorableType, new TypeDesc[] { storageType, byteArrayType });
            b.returnValue(storableType);
