...
public class DiffCalculator implements DiffCalculatorInterface {
    ...
    public DiffCalculator(final TaskTransmitterInterface taskTransmitter) throws ConfigurationException {
        ...
        VALUE_MINIMUM_LONGEST_COMMON_SUBSTRING = (Integer) config.getConfigParameter(ConfigurationKeys.VALUE_MINIMUM_LONGEST_COMMON_SUBSTRING);
        MODE_SURROGATES = (SurrogateModes) config.getConfigParameter(ConfigurationKeys.MODE_SURROGATES);
    }
    ...
    protected void transmitAtEndOfTask(final Task<Revision> task, final Task<Diff> result) throws TimeoutException {
        if (task.getTaskType() == TaskTypes.TASK_FULL || task.getTaskType() == TaskTypes.TASK_PARTIAL_LAST) {
            if (this.partCounter > 1) {
    ...
    public void process(final Task<Revision> task) throws DiffException, TimeoutException, UnsupportedEncodingException {
        Revision revision;
                ...
                if (VERIFICATION_DIFF) {
                    ...
                    try {
                        ...
                        if (!revC.equals(revP)) {
                            ...
                            throw ErrorFactory.createDiffException(ErrorKeys.DIFFTOOL_DIFFCONSUMER_DIFF_VERIFICATION_FAILED, "Reconstruction of " + task.toString() + " failed at revision " + revisionCounter + ".");
                        }
                    } catch (DiffException e) {
                        throw e;
                    } catch (Exception e) {
                        throw ErrorFactory.createDiffException(ErrorKeys.DIFFTOOL_DIFFCONSUMER_DIFF_VERIFICATION_FAILED, "Reconstruction of " + task.toString() + " failed at revision " + revisionCounter + ".", e);
                    }
                }
            }
        }
	 ...
	 *             if the character encoding is unsupported
	 */
    private Diff generateDiff(final char[] revA, final char[] revB) throws UnsupportedEncodingException {
        blockCount = 0;
        ...
        while (revAStartIndex <= revAEndIndex && revBStartIndex <= revBEndIndex && revA[revAStartIndex] == revB[revBStartIndex]) {
        ...
        while (revAStartIndex < revAEndIndex && revBStartIndex < revBEndIndex && revA[revAEndIndex] == revB[revBEndIndex]) {
        ...
        if (revAEndIndex + 1 != revA.length) {
            queueA.add(new DiffBlock(this.blockCount, revAEndIndex + 1, revA.length, revBEndIndex + 1, revB.length, true));
            queueB.add(new DiffBlock(this.blockCount, revAEndIndex + 1, revA.length, revBEndIndex + 1, revB.length, false));
            this.blockCount++;
        }
	 ...
	 */
    private boolean findLongestMatch(final char[] revA, final ArrayList<Integer> list, final char[] revB, final int index) {
        int match;
        longestMatch_size = -1;
        int size = list.size();
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!revABlocked[start] && !revBBlocked[index + 1]) {
                ...
                while (end < revAsize && count < revBsize && revA[end] == revB[count] && !revABlocked[end] && !revBBlocked[count]) {
                    end++;
                    count++;
                }
                match = end - start;
        ...
        queueA.add(new DiffBlock(this.blockCount, longestMatch_start, longestMatch_start + longestMatch_size, index, index + longestMatch_size, true));
        queueB.add(new DiffBlock(this.blockCount, longestMatch_start, longestMatch_start + longestMatch_size, index, index + longestMatch_size, false));
        blockCount++;
