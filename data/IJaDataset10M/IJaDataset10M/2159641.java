...
public class CoverageStatsProcessor extends CoverageProcessor implements ProcessorInterface {
    ...
    public Object process(Object obj) {
        ...
        if (newStart < c.getStartPosition()) {
            newStart = c.getStartPosition();
        }
        if (newStop > c.getStopPosition()) {
            newStop = c.getStopPosition();
        }
        for (int i = newStart; i <= newStop; i++) {
            ...
            if (intCov != null) {
                ...
                if (covPositions == null) {
                    covPositions = new HashMap();
                    this.getStats().put("cov_positions", covPositions);
                }
                HashMap covBases = (HashMap) this.getStats().get("cov_bases");
                if (covBases == null) {
                    covBases = new HashMap();
                    this.getStats().put("cov_bases", covBases);
                }
