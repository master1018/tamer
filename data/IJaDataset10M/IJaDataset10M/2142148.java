...
public class HTTPServer {
    ...
    public static synchronized HTTPServer get(IPAddress ipAddress, int port) throws IOException {
	 ...
	 *             the given address.
	 */
    public static synchronized HTTPServer get(IPAddress ipAddress, int port, boolean secure, String alias) throws IOException {
        String key;
        HTTPServer server;
        if (port == 0) {
    ...
    public static synchronized void unregisterAndStop(HTTPServer server) throws IOException {
    ...
    public synchronized void start() throws IOException {
        ...
        if (!isSecure) listener = TCPServer.open(ipAddress, port, handler); else listener = TCPServer.open(ipAddress, port, handler, true, this.alias);
        if (port == 0) {
