...
public class CodeUtilities implements OpCodeConstants, OpCodeInfo {
    ...
    */
    ClassRecord getClassRecord(String className) throws TinyVMException {
        ...
        if (className.startsWith("[")) {
            ...
            if (ret == null) {
                throw new TinyVMException("Bug CU-3: Didn't find class " + className + " from class " + iCF.getClassName());
            }
        }
        return ret;
    }
    ...
    public int processConstantIndex(int aPoolIndex) throws TinyVMException {
        ...
        if (!(pEntry instanceof ConstantInteger) && !(pEntry instanceof ConstantFloat) && !(pEntry instanceof ConstantString) && !(pEntry instanceof ConstantDouble) && !(pEntry instanceof ConstantLong)) {
            throw new TinyVMException("Classfile error: LDC-type instruction " + "does not refer to a suitable constant. Class " + iCF.getClassName() + " method " + iFullName);
        }
        ConstantRecord pRecord = new ConstantRecord(iCF.getConstantPool(), pEntry);
        int pIdx = iBinary.getConstantIndex(pRecord);
        if (pIdx == -1) {
            throw new TinyVMException("Bug CU-2: Didn't find constant " + pEntry + " of class " + iCF.getClassName());
        }
        return pIdx;
    }
    ...
    public int processClassIndex(int aPoolIndex) throws TinyVMException {
        ...
        if (pIdx == -1) {
            throw new TinyVMException("Bug CU-3: Didn't find class " + pEntry + " from class " + iCF.getClassName() + " name " + pClassName);
        }
        return pIdx;
    }
    ...
    public int processMultiArray(int aPoolIndex) throws TinyVMException {
        ...
        ConstantClass pClassEntry = (ConstantClass) pEntry;
        int pTypeDim = getTypeAndDimensions(pClassEntry.getBytes(iCF.getConstantPool()));
        return pTypeDim;
    }

    public int getTypeAndDimensions(String aMultiArrayDesc) throws TinyVMException {
        int i = 0;
        while (aMultiArrayDesc.charAt(i) == '[') i++;
        if (i > TinyVMConstants.MAX_DIMS) {
            throw new TinyVMException("In " + iFullName + ": Multi-dimensional arrays are limited to " + TinyVMConstants.MAX_DIMS);
        }
        ...
        if (typ == TinyVMType.T_OBJECT_TYPE || typ == TinyVMType.T_REFERENCE_TYPE) {
            ...
            if (cls == -1) {
                throw new TinyVMException("Bug CU-3: Didn't find class " + name + " from class " + aMultiArrayDesc);
            }
    ...
    */
    int processField(int aFieldIndex, boolean aStatic) throws TinyVMException {
        Constant pEntry = iCF.getConstantPool().getConstant(aFieldIndex);
        ...
        if (pClassRecord == null) {
            throw new TinyVMException("Attempt to use a field from a primitive array " + className + " from class " + iCF.getClassName() + " method " + iFullName);
        }
        ConstantNameAndType cnat = (ConstantNameAndType) iCF.getConstantPool().getConstant(pFieldEntry.getNameAndTypeIndex());
        ...
        if (aStatic) {
            ...
            if (pFieldRecord == null) {
                throw new TinyVMException("Failed to locate static field " + pName + " refrenced via class " + className + " from class " + iCF.getClassName());
            }
            ...
            if (pOffset == -1) {
                throw new TinyVMException("Error: Didn't find field " + className + ":" + pName + " from class " + iCF.getClassName());
            }
    ...
    */
    public void markClass(int aPoolIndex) throws TinyVMException {
        Constant pEntry = iCF.getConstantPool().getConstant(aPoolIndex);
        if (!(pEntry instanceof ConstantClass)) {
            throw new TinyVMException("Classfile error: Instruction requiring " + "CONSTANT_Class entry got " + (pEntry == null ? "null" : pEntry.getClass().getName()) + " method " + iFullName);
        }
        ConstantClass pClassEntry = (ConstantClass) pEntry;
        String pClassName = pClassEntry.getBytes(iCF.getConstantPool());
        ClassRecord pClassRecord = getClassRecord(pClassName);
        if (pClassRecord != null) iBinary.markClassUsed(pClassRecord, true);
    }
    ...
    */
    void markStaticField(int aFieldIndex) throws TinyVMException {
        ...
        StaticFieldRecord pFieldRecord = pClassRecord.getStaticFieldRecord(pName);
        if (pFieldRecord == null) {
            throw new TinyVMException("Failed to mark/locate static field " + pName + " refrenced via class " + className + " from class " + iCF.getClassName());
        }
    ...
    */
    int processMethod(int aMethodIndex, boolean aSpecial, boolean aInterface) throws TinyVMException {
        Constant pEntry = iCF.getConstantPool().getConstant(aMethodIndex);
        ...
        ConstantNameAndType pNT = (ConstantNameAndType) iCF.getConstantPool().getConstant(pMethodEntry.getNameAndTypeIndex());
        Signature pSig = new Signature(pNT.getName(iCF.getConstantPool()), pNT.getSignature(iCF.getConstantPool()));
        if (className.startsWith("[")) {
            className = "java/lang/Object";
        }
        ClassRecord pClassRecord = getClassRecord(className);
        if (pClassRecord == null) {
            throw new TinyVMException("Bug CU-7: Didn't find class " + className + " from class " + iCF.getClassName() + " method " + iFullName);
        }
        MethodRecord pMethod;
        if (aInterface) pMethod = pClassRecord.getInterfaceMethodRecord(pSig); else pMethod = pClassRecord.getVirtualMethodRecord(pSig);
        if (pMethod == null) {
        ...
        if (aSpecial) {
            ...
            assert pMethodIndex != -1 && pMethodIndex < TinyVMConstants.MAX_METHODS : "Check: method index in range";
    ...
    */
    MethodRecord findMethod(int aMethodIndex, boolean aSpecial, boolean aInterface) throws TinyVMException {
        ...
        ClassRecord pClassRecord = getClassRecord(className);
        if (pClassRecord == null) {
            throw new TinyVMException("Bug CU-7: Didn't find class " + className + " from class " + iCF.getClassName());
        }
        MethodRecord pMethod;
        if (aInterface) pMethod = pClassRecord.getInterfaceMethodRecord(pSig); else pMethod = pClassRecord.getVirtualMethodRecord(pSig);
        return pMethod;
    ...
    static int getAndCopyFourBytesInt(byte[] aCode, int ix, byte[] pOutCode, int ox) {
        ...
        a |= (aCode[ix + 2] & 0xFF) << 8;
        a |= (aCode[ix + 3] & 0xFF) << 0;
        for (int i = 0; i < 4; i++) pOutCode[ox + i] = aCode[ix + i];
        return a;
    }
    ...
    public byte[] processCode(byte[] aCode) throws TinyVMException {
        ...
        while (i < aCode.length) {
            ...
            switch(pOpCode) {
                ...
                case OP_LDC_W:
                case OP_LDC2_W:
                    int pIdx1 = processConstantIndex((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF));
                    pOutCode[i++] = (byte) (pIdx1 >> 8);
                    pOutCode[i++] = (byte) (pIdx1 & 0xFF);
                    break;
                case OP_ANEWARRAY:
                    int pIdx5 = processClassIndex((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF));
                    pOutCode[i++] = (byte) (pIdx5 >> 8);
                    pOutCode[i++] = (byte) (pIdx5 & 0xFF);
                    break;
                case OP_MULTIANEWARRAY:
                    int pIdx2 = processMultiArray((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF));
                ...
                case OP_CHECKCAST:
                case OP_INSTANCEOF:
                    int pIdx3 = processClassIndex((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF));
                    pOutCode[i++] = (byte) (pIdx3 >> 8);
                    pOutCode[i++] = (byte) (pIdx3 & 0xFF);
                    break;
                case OP_NEW:
                    int pIdx4 = processClassIndex((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF));
                ...
                case OP_PUTSTATIC:
                case OP_GETSTATIC:
                    int pWord1 = processField((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), true);
                    pOutCode[i++] = (byte) (pWord1 >> 16);
                    int fldIdx = pWord1 & 0x03FF;
                    if (fldIdx >= 256) {
                        int newOpCode;
                        if (fldIdx > TinyVMConstants.MAX_STATICS) exitOnBadOpCode(pOpCode);
                        if ((aCode[i - 2] & 0xFF) == OP_PUTSTATIC) newOpCode = OP_PUTSTATIC_1 + (fldIdx - 256) / 256 * 2; else newOpCode = OP_GETSTATIC_1 + (fldIdx - 256) / 256 * 2;
                ...
                case OP_GETFIELD:
                    {
                        ...
                        if (false) {
                            ...
                            if (fieldType == TinyVMType.T_BOOLEAN_TYPE || fieldType == TinyVMType.T_BYTE_TYPE) opcode = opcode == OP_GETFIELD ? OP_GETFIELD_S1 : OP_PUTFIELD_S1; else if (fieldType == TinyVMType.T_SHORT_TYPE) opcode = opcode == OP_GETFIELD ? OP_GETFIELD_S2 : OP_PUTFIELD_S2; else if (fieldType == TinyVMType.T_CHAR_TYPE) opcode = opcode == OP_GETFIELD ? OP_GETFIELD_U2 : OP_PUTFIELD_U2; else if (fieldType == TinyVMType.T_INT_TYPE || fieldType == TinyVMType.T_FLOAT_TYPE) opcode = opcode == OP_GETFIELD ? OP_GETFIELD_W4 : OP_PUTFIELD_W4; else if (fieldType == TinyVMType.T_REFERENCE_TYPE) opcode = opcode == OP_GETFIELD ? OP_GETFIELD_A4 : OP_PUTFIELD_A4;
                ...
                case OP_INVOKEINTERFACE:
                    ...
                    int pWord3 = processMethod((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), false, true);
                ...
                case OP_INVOKESTATIC:
                    int pWord4 = processMethod((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), true, false);
                    pOutCode[i++] = (byte) (pWord4 >> 8);
                    pOutCode[i++] = (byte) (pWord4 & 0xFF);
                    break;
                case OP_INVOKEVIRTUAL:
                    int pWord5 = processMethod((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), false, false);
                ...
                case OP_WIDE:
                    if ((aCode[i] & 0xFF) == OP_IINC && aCode[i + 1] == 0) {
                        for (int k = 0; k < 5; k++, i++) pOutCode[i] = aCode[i];
                        break;
                    }
                case OP_GOTO_W:
                case OP_JSR_W:
                ...
                default:
                    ...
                    if (pArgs == -1) {
                        throw new TinyVMException("Bug CU-1: Got " + pOpCode + " in " + iFullName + ".");
                    }
                    for (int ctr = 0; ctr < pArgs; ctr++) pOutCode[i + ctr] = aCode[i + ctr];
                    i += pArgs;
                    break;
            }
        }
        ...
        while (i < aCode.length) {
            ...
            switch(pOpCode) {
                ...
                case OP_GETSTATIC:
                    ...
                    break;
                case OP_INVOKEINTERFACE:
                    MethodRecord pMeth0 = findMethod((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), false, true);
                    if (pMeth0 != null) pMeth0.getClassRecord().markMethod(pMeth0, true);
                    i += 4;
                    break;
                case OP_INVOKEVIRTUAL:
                    MethodRecord pMeth1 = findMethod((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), false, false);
                    if (pMeth1 != null) pMeth1.getClassRecord().markMethod(pMeth1, true);
                    i += 2;
                    break;
                case OP_INVOKESPECIAL:
                case OP_INVOKESTATIC:
                    MethodRecord pMeth2 = findMethod((aCode[i] & 0xFF) << 8 | (aCode[i + 1] & 0xFF), true, false);
