...
public abstract class PrimitiveMetaClass implements MetaClass {
    ...
    public String getFullyQualifiedName(String separator) {
        if (!isProxy) return getSimpleName();
        return JaceConstants.getProxyPackage().asPath().replace("/", separator) + separator + "types" + separator + getSimpleName();
    }
    ...
    public MetaClass proxy() {
        if (isProxy) throw new IllegalStateException(getClass().getSimpleName() + " is already a proxy: " + getFullyQualifiedName("."));
        return newInstance(true);
    }

    @Override
    public MetaClass unProxy() {
        if (!isProxy) throw new IllegalStateException(getClass().getSimpleName() + " is not a proxy: " + getFullyQualifiedName("."));
