    ...
    protected void buildMethodCall(final AstVisitEvent event) {
        ...
        if (elements.length > 0) {
            if (elements[0] instanceof IdentifierElement) {
                ...
                final UnresolvedMethodCallInfo methodCall = new UnresolvedMethodCallInfo(UnresolvedMemberImportStatementInfo.getMemberImportStatements(this.buildDataManager.getAllAvaliableNames()), callee.getOwnerUsage(), callee.getName());
                ...
                for (int i = 1; i < elements.length; i++) {
                    ...
                    if (argument instanceof IdentifierElement) {
                        methodCall.addArgument(((IdentifierElement) argument).resolveAsVariable(this.buildDataManager, true, false));
