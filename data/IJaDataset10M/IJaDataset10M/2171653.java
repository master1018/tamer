...
public class JdbcLogTest extends TestCase {
    ...
    public void testHandlesRecursivelyFailingException() throws Exception {
        setUpJdbcLog(false, null);
        Session.registerSession(new Session(new UnitTestApplication(), new MemoryStoreFactory(), sessionID, new DefaultDataDictionaryProvider(), null, logFactory, new DefaultMessageFactory(), 0));
        dropTable(log.getIncomingMessagesTableName());
        try {
            log.onIncoming("DB is messed up");
        } catch (OutOfMemoryError err) {
            fail("We seem to get an out of memory error b/c of stack overflow b/c we" + "keep calling jdbc logger recursively in case of misconfiguration: " + err.getMessage());
        } finally {
    ...
    private void assertLogData(Connection connection, int rowOffset, SessionID sessionID, String text, String tableName) throws SQLException {
        Statement s = connection.createStatement();
        ResultSet rs = s.executeQuery("select time,beginstring,sendercompid,targetcompid,session_qualifier,text from " + tableName + " WHERE text = '" + text + "'");
        int n = 0;
        boolean rowFound = false;
        while ((rowFound = rs.next()) && n < rowOffset) n++;
    ...
    protected void initializeTableDefinitions(Connection connection) throws ConfigError {
        try {
            JdbcTestSupport.loadSQL(connection, "core/src/main/config/sql/mysql/messages_log_table.sql", new JdbcTestSupport.HypersonicPreprocessor(null));
            JdbcTestSupport.loadSQL(connection, "core/src/main/config/sql/mysql/event_log_table.sql", new JdbcTestSupport.HypersonicPreprocessor(null));
        } catch (Exception e) {
