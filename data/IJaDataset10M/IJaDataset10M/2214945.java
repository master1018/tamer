...
public class ProductionManager extends ArbitratedManager {
    ...
    protected final Map<UnitType, List<UnitType>> productionQueues = new HashMap<UnitType, List<UnitType>>();

    protected final Map<Unit, ProductionUnit> producingUnits = new HashMap<Unit, ProductionUnit>();
        ...
        while (unitIterator.hasNext()) {
            ...
            if (unit.getKey().isLifted()) {
                if (unit.getKey().isIdle()) {
                    unit.getKey().land(placer.getBuildLocationNear(unit.getKey().getTilePosition().opAdd(new TilePosition(0, 1)), unit.getKey().getType()));
                }
            } else {
                ...
                if (unit.getValue().unit == null) {
                    if (Game.getInstance().getFrameCount() > unit.getValue().lastAttemptFrame + Game.getInstance().getLatency().getBwapiOrdinal() * 2 && Game.getInstance().canMake(unit.getKey(), unit.getValue().type)) {
                    ...
                    if (unit.getValue().unit.isCompleted()) {
                        ...
                        if (unit.getValue().unit.exists() && !unit.getValue().unit.getType().equals(unit.getValue().type)) {
    ...
    public boolean train(UnitType type) {
        ...
        if (type.getRace().equals(Race.ZERG) && !type.whatBuilds().getKey().equals(UnitType.ZERG_INFESTED_COMMAND_CENTER)) {
            return false;
        }
