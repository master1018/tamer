...
public class MacOSPlugin extends EBPlugin {
    ...
    public void handleMessage(EBMessage message) {
        if (osok) {
            if (message instanceof BufferUpdate) delegate.handleFileCodes((BufferUpdate) message); else if (message instanceof PropertiesChanged) {
                boolean b = jEdit.getBooleanProperty("MacOSPlugin.useSelection", jEdit.getBooleanProperty("MacOSPlugin.default.useSelection"));
                if (b) jEdit.setColorProperty("view.selectionColor", UIManager.getColor("textHighlight"));
            } else if (!started && message instanceof ViewUpdate) delegate.handleOpenFile((ViewUpdate) message);
        }
    }
