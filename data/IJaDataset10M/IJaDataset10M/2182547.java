...
public final class SWGWeights extends SWGValues {
     ...
     */
    public static final SWGWeights LQ_WEIGHTS = new SWGWeights(new int[] { 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9 });
    ...
    private void aConstructorHelper(int[] weights, boolean validateSum) {
        if (validateSum) validate(weights);
        if (validateSum && !isValid(sum(weights))) throw new IllegalArgumentException(String.format("Invalid sum: %s of %s", Integer.toString(sum(weights)), Arrays.toString(weights)));
    ...
    private double adjCap(int cap) {
        if (cap < 0 || cap > 1000) throw new IllegalArgumentException("Invalid cap: " + cap);
        return cap > 0 ? (1000.0 / cap) : 1.0;
    }
    ...
    private double adjIfZero(int value, boolean zeroIsMax, double weight) {
        return value == 0 && zeroIsMax ? weight * 10.0 : value;
    }
    ...
    public void adjust() {
        ...
        for (Stat s : Stat.values()) values[s.i] = (int) Math.round(values[s.i] * adj);
    }
    ...
    public boolean hasMinimumOneStat(SWGResource res) {
        for (Stat s : Stat.values()) if (values[s.i] > 0 && res.rc().has(s)) return true;
        return false;
    }
    ...
    public double rate(SWGResource res, SWGResourceClass caps, boolean zeroIsMax) {
        if (caps != null && !res.rc().isSub(caps.getClass())) throw new IllegalArgumentException(String.format("%s is not subclass of %s", res.rc().rcName(), caps.rcName()));
        if (!hasMinimumOneStat(res)) return 0.0;
        double[] weights = new double[Stat.COUNT];
        for (Stat s : Stat.values()) weights[s.i] = values[s.i];
        int[] rValues = res.stats().values;
        double weightSA = 0.0;
        if (zeroIsMax) for (Stat s : Stat.values()) {
            if (weights[s.i] > 0 && rValues[s.i] <= 0) weights[s.i] = 0;
            weightSA += weights[s.i];
        } else weightSA = 100.0;
        weightSA = 100 / weightSA;
        ...
        for (Stat s : Stat.values()) {
            if (weights[s.i] > 0) {
                ...
                double multiplier = caps != null ? adjCap(caps.max(s)) : 1.0;
                resWeight += Math.min(1000.0, value * multiplier) * weight / 100.0;
            }
        }
        return resWeight;
    }
    ...
    protected int validate(int value) {
        if (value < 0 || value > 100) throw new IllegalArgumentException(Integer.toString(value));
        return value;
    }
    ...
    public static boolean isValid(int[] values) {
        if (values.length != Stat.COUNT) return false;
        int checkSum = 0;
        for (int w : values) {
    ...
    public static SWGWeights wider(SWGWeights w1, SWGWeights w2) {
        ...
        for (Stat s : Stat.values()) {
            if (ww2.values[s.i] > 0) {
                if (ww1.values[s.i] <= 0) return null;
                shared = true;
            }
        }
        return shared ? ww1 : null;
    }
}
