...
public class SSRCCache {
    ...
    double calcReportInterval(boolean sender, boolean recvfromothers) {
        ...
        if (initial) rtcp_min_time = rtcp_min_time / 2;
        int n = aliveCount();
        if (sendercount > 0 && sendercount < n * rtcp_sender_bw_fraction) if (sender) {
        ...
        if (recvfromothers && rtcp_bw == 0.0D) {
            ...
            if (sendercount > 0 && sendercount < n * 0.25D) if (sender) {
                rtcp_bw *= 0.25D;
                n = sendercount;
            } else {
                rtcp_bw *= 0.75D;
        ...
        if (rtcp_bw != 0.0D) {
            ...
            if (time < rtcp_min_time) time = rtcp_min_time;
        }
        if (recvfromothers) return time; else return time * (Math.random() + 0.5D);
    }
        ...
        synchronized (this) {
            ...
            if (info != null) synchronized (info) {
                if (info.address == null || !info.alive) {
                    ...
                    info.port = port;
                } else if (!info.address.equals(address)) if (info.probation > 0) {
                    info.probation = 2;
                    info.address = address;
                    info.port = port;
                } else {
            ...
            if (info == null) {
                ...
                if (mode == 1) info = new RecvSSRCInfo(this, ssrc);
                if (mode == 2) info = new PassiveSSRCInfo(this, ssrc);
                if (info == null) {
            ...
            if (localcollision) {
                ...
                if (info instanceof RecvSSRCInfo) levt = new LocalCollisionEvent(sm, (ReceiveStream) info, ourssrc.ssrc); else levt = new LocalCollisionEvent(sm, null, ourssrc.ssrc);
    ...
    int getSessionBandwidth() {
        if (sessionbandwidth == 0) throw new IllegalArgumentException("Session Bandwidth not set"); else return sessionbandwidth;
    }

    private void LocalCollision(int ssrc) {
        int newssrc = 0;
        do newssrc = (int) TrueRandom.rand(); while (lookup(newssrc) != null);
    ...
    void setclockrates() {
        for (int i = 0; i < 16; i++) clockrate[i] = 8000;
        clockrate[6] = 16000;
        clockrate[10] = 44100;
        clockrate[11] = 44100;
        ...
        clockrate[17] = 22050;
        clockrate[18] = 44100;
        for (int i = 24; i < 34; i++) clockrate[i] = 0x15f90;
        for (int i = 96; i < 128; i++) {
            javax.media.Format fmt = sm.formatinfo.get(i);
            if (fmt != null && (fmt instanceof AudioFormat)) clockrate[i] = (int) ((AudioFormat) fmt).getSampleRate(); else clockrate[i] = 0x15f90;
        }
    }
