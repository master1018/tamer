...
public final class MyLZWDecompressor {
    ...
    public MyLZWDecompressor(int initialCodeSize, int byteOrder, Listener listener) {
        ...
        eoiCode = clearCode + 1;
        if (null != listener) listener.init(clearCode, eoiCode);
        InitializeTable();
    }
    ...
    private final void InitializeTable() {
        ...
        for (int i = 0; i < intial_entries_count; i++) table[i] = new byte[] { (byte) i };
    }
    ...
    private final int getNextCode(MyBitInputStream is) throws IOException {
        ...
        if (null != listener) listener.code(code);
        return code;
    }

    private final byte[] stringFromCode(int code) throws IOException {
        if ((code >= codes) || (code < 0)) throw new IOException("Bad Code: " + code + " codes: " + codes + " code_size: " + codeSize + ", table: " + table.length);
        return table[code];
    }
    ...
    private final void addStringToTable(byte bytes[]) throws IOException {
        if (codes < (1 << codeSize)) {
            table[codes] = bytes;
            codes++;
        } else throw new IOException("AddStringToTable: codes: " + codes + " code_size: " + codeSize);
        checkCodeSize();
    }
    ...
    public byte[] decompress(InputStream is, int expectedLength) throws IOException {
        ...
        if (tiffLZWMode) mbis.setTiffLZWMode();
    ...
    private final void checkCodeSize() {
        ...
        if (tiffLZWMode) limit--;
        if (codes == limit) incrementCodeSize();
    }
