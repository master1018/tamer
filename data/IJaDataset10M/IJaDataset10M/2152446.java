...
public class Monitor {
    ...
    public Monitor(ReplicationConfig monitorConfig) {
        if (!monitorConfig.getNodeType().equals(NodeType.MONITOR)) {
            throw new IllegalArgumentException("The configured node type was: " + monitorConfig.getNodeType() + " instead of: " + NodeType.MONITOR);
        }
    ...
    public ReplicationNode register() throws EnvironmentFailureException {
        ...
        RepNodeImpl monitorNode = new RepNodeImpl(nameIdPair, NodeType.MONITOR, monitorConfig.getNodeHostname(), monitorConfig.getNodePort());
    ...
    public void startListener(MonitorChangeListener newListener) throws DatabaseException, IOException {
        ...
        Protocol electionProtocol = new Protocol(TimebasedProposalGenerator.getParser(), MasterValue.getParser(), monitorConfig.getGroupName(), nameIdPair, null);
    ...
    private void notifyJoinGroupEventsForActiveNodes(ReplicationGroup group) {
        ...
        for (ReplicationNode repNode : group.getElectableNodes()) {
            MessageExchange me = stateProtocol.new MessageExchange(repNode.getSocketAddress(), NodeStateService.SERVICE_NAME, stateProtocol.new NodeStateRequest(repNode.getName()));
            me.run();
            ...
            if (resp instanceof NodeStateResponse) {
                ...
                notify(new JoinGroupEvent(response.getNodeName(), response.getMasterName(), response.getJoinTime()));
            }
        }
    }
    ...
    private class MasterChangeListener implements Learner.Listener {
        ...
        public void notify(Proposal proposal, Value value) {
            ...
            try {
                ...
                LoggerUtils.logMsg(logger, formatter, Level.INFO, "Monitor notified of new Master: " + currentMasterName);
                if (monitorChangeListener == null) {
                    return;
                }
                monitorChangeListener.notify(new NewMasterEvent(currentValue));
            } catch (Exception e) {
                LoggerUtils.logMsg(logger, formatter, Level.SEVERE, "Monitor change event processing exception: " + e.getMessage());
            }
        }
    }
}
