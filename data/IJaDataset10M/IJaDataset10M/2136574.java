 ...
 *          $
 */
public class ListDatastreamsServlet extends HttpServlet implements Constants {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (URIArray.length == 6 || URIArray.length == 7) {
            try {
                PID = Server.getPID(URIArray[5]).toString();
            } catch (Throwable th) {
            ...
            logger.debug("Listing datastreams (PID=" + PID + ", asOfDate=" + versDateTime + ")");
        } else {
        ...
        try {
            ...
            logger.error("Object not found for request: " + requestURI + " (actionLabel=" + ACTION_LABEL + ")", e);
    ...
    public void listDatastreams(Context context, String PID, Date asOfDateTime, boolean xml, HttpServletRequest request, HttpServletResponse response) throws ServerException {
    ...
    public class DatastreamDefSerializerThread extends Thread {
         ...
         */
        public DatastreamDefSerializerThread(Context context, String PID, DatastreamDef[] dsDefs, Date versDateTime, PipedWriter pw) {
            this.pw = pw;
            this.PID = PID;
            this.dsDefs = dsDefs;
            this.versDateTime = versDateTime;
            fedoraServerPort = context.getEnvironmentValue(HTTP_REQUEST.SERVER_PORT.uri);
            fedoraAppServerContext = context.getEnvironmentValue(Constants.FEDORA_APP_CONTEXT_NAME);
            if (HTTP_REQUEST.SECURE.uri.equals(context.getEnvironmentValue(HTTP_REQUEST.SECURITY.uri))) {
                fedoraServerProtocol = HTTPS;
            } else if (HTTP_REQUEST.INSECURE.uri.equals(context.getEnvironmentValue(HTTP_REQUEST.SECURITY.uri))) {
        ...
        public void run() {
            if (pw != null) {
                try {
                    ...
                    final String baseURL = fedoraServerProtocol + "://" + fedoraServerHost + ":" + fedoraServerPort + "/" + fedoraAppServerContext + "/";
                    ...
                    for (DatastreamDef element : dsDefs) {
                        pw.write("    <datastream " + "dsid=\"" + StreamUtility.enc(element.dsID) + "\" " + "label=\"" + StreamUtility.enc(element.dsLabel) + "\" " + "mimeType=\"" + StreamUtility.enc(element.dsMIME) + "\" />");
                    }
