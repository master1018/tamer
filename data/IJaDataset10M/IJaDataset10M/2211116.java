...
public class ProductManager {
    ...
    public static int getSimilarProduct(Properties ctx, String productName, String trxName) {
        ...
        String whereClause = "AD_Client_id=" + Env.getAD_Client_ID(ctx) + " and Upper(name) =Upper('" + productName + "')";
        int productIds[] = MProduct.getAllIDs(MProduct.Table_Name, whereClause, trxName);
        if (productIds.length == 0) return 0; else return productIds[0];
    }

    public static boolean isBarCodePresent(Properties ctx, String barCode, String trxName) {
        String whereClause = "AD_Client_id=" + Env.getAD_Client_ID(ctx) + " and UPC ='" + barCode + "'";
        int productIds[] = MProduct.getAllIDs(MProduct.Table_Name, whereClause, trxName);
        if (productIds.length == 0) return false; else return true;
    }
    ...
    private static String generateAttributeSearchSQL(Properties ctx, int attributeId, int attributeValueId) throws OperationException {
        String sql = " AND p.M_AttributeSetInstance_ID IN" + " (SELECT M_AttributeSetInstance_ID FROM M_AttributeInstance WHERE M_Attribute_ID=@M_ATTRIBUTE_ID@ AND M_AttributeValue_ID=@M_ATTRIBUTEVALUE_ID@ and AD_CLIENT_ID =  " + Env.getAD_Client_ID(ctx) + " and AD_ORG_ID = " + Env.getAD_Org_ID(ctx) + " )";
    ...
    public static void sortProducts(Properties ctx, ArrayList<ProductBean> products) {
        Comparator<ProductBean> productNameComparator = new Comparator<ProductBean>() {
            ...
            public int compare(ProductBean o1, ProductBean o2) {
                ...
                if (bean1.getProductName() == null) return -1;
                if (bean2.getProductName() == null) return -1;
    ...
    public static ArrayList getProductList(Properties ctx, String prodNameLike, boolean isActive, boolean isWebstoreFeatured) throws OperationException {
        String sql = "select distinct m_product_id, name, isWebstoreFeatured " + ", keyword, keyword2, keyword3, keyword4, upc,CLASSIFICATION, isSelfService " + " from m_product where ad_org_id=" + Env.getAD_Org_ID(ctx) + " and ad_client_id=" + Env.getAD_Client_ID(ctx);
        ...
        if (isActive) sql += " and IsActive='Y' "; else sql += " and IsActive='N' ";
        if (isWebstoreFeatured) sql += " and IsWebstoreFeatured='Y' "; else sql += " and IsWebstoreFeatured='N' ";
        sql += " order by name";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (selfServiceStr != null) isSelfService = "Y".equals(selfServiceStr);
                bean.setIsSelfService(isSelfService);
                boolean isWebstoredFeatured = false;
                String isWStoreFeatured = rs.getString(3);
                if (isWStoreFeatured != null) isWebstoredFeatured = "Y".equals(isWStoreFeatured);
     ...
     */
    public static ArrayList<ProductStatusBean> searchProducts(Properties ctx, int adOrgId, String nameSearch, String descSearch, String trxName) throws OperationException {
     ...
     * @throws OperationException if error occurred in search
     */
    public static ArrayList<ProductStatusBean> searchProducts(Properties ctx, int adOrgId, String nameSearch, String descSearch, int priceListId, int warehouseId, String trxName) throws OperationException {
        if (adOrgId <= 0) {
        ...
        sqlStmt.append("SELECT p.Name, ");
        sqlStmt.append("p.M_Product_ID, p.UPC, p.Description, ");
        sqlStmt.append("CASE WHEN s.QtyOnHand IS NULL THEN 0 ELSE SUM(s.QtyOnHand) END AS QtyOnHand, pp.PriceList ");
        sqlStmt.append("FROM M_Product p ");
        sqlStmt.append("INNER JOIN M_ProductPrice pp ON p.M_Product_ID =pp.M_Product_ID ");
        sqlStmt.append("LEFT JOIN M_Storage s ON (s.M_Product_ID = p.M_Product_ID AND s.M_Locator_ID IN (SELECT M_Locator_ID FROM M_Locator WHERE M_Warehouse_ID = ?))");
        ...
        sqlStmt.append("AND p.IsActive='Y' AND pp.IsActive='Y' ");
        sqlStmt.append("GROUP BY p.Name, p.M_Product_ID, p.UPC, p.Description, s.QtyOnHand, pp.PriceList ");
     ...
     */
    public static ArrayList<ProductStatusBean> searchProductsFromStock(Properties ctx, int orgId, String searchString, String trxName) throws OperationException {
    ...
    public static void addUpdateAttachment(Properties ctx, int productId, MAttachmentEntry entry, String trxName) throws AttachmentNotFoundException, OperationException {
        if (AttachmentManager.hasAttachment(ctx, MProduct.Table_ID, productId, trxName)) {
            ...
            if (index != -1) AttachmentManager.deleteAttachmentEntry(attachment, index);
            AttachmentManager.addAttachmentEntry(attachment, entry);
            PoManager.save(attachment);
        } else AttachmentManager.addAttachment(ctx, MProduct.Table_ID, productId, new MAttachmentEntry[] { entry }, trxName);
    }
    ...
    public static void addAttachment(Properties ctx, int m_product_id, MAttachmentEntry[] entries, String trxName) throws OperationException, AttachmentNotFoundException {
        if (AttachmentManager.hasAttachment(ctx, MProduct.Table_ID, m_product_id, trxName)) removeAttachment(ctx, m_product_id, trxName);
    ...
    public static void removeAttachment(Properties ctx, int m_product_id, String trxName) throws OperationException, AttachmentNotFoundException {
        ...
        if (!attachment.delete(true)) throw new OperationException("Could not delete attachments for product with id: " + m_product_id);
    }
    ...
    public static MAttachment getAttachment(Properties ctx, int product_id, String trxName) throws OperationException, AttachmentNotFoundException {
        ...
        if (attachment == null) throw new AttachmentNotFoundException("Could not find attachment with product Id: " + product_id);
        return attachment;
    }
    ...
    public static byte[] getAttachmentData(MAttachment attachment, String entryName) throws AttachmentNotFoundException, OperationException {
        if (attachment == null) throw new AttachmentNotFoundException("Attachment cannot be null");
        MAttachmentEntry entries[] = attachment.getEntries();
        for (int i = 0; i < entries.length; i++) {
            if (entries[i].getName().equals(entryName)) return entries[i].getData();
        }
    ...
    public static MProduct loadProduct(Properties ctx, String productName, String trxName) throws ProductNotFoundException, OperationException {
        ...
        int productIds[] = MProduct.getAllIDs(MProduct.Table_Name, whereClause, trxName);
        if (productIds.length == 0) throw new ProductNotFoundException("Could not load product with Name: " + productName);
        if (productIds.length > 1) throw new OperationException("More than one product found with Name: " + productName);
    ...
    public static MProduct loadProduct(Properties ctx, int productId, String trxName) throws ProductNotFoundException {
        ...
        if (product.get_ID() == 0) throw new ProductNotFoundException("Could not load product with id: " + productId);
        return product;
    }

    public static String getDistinctFieldSQL(Properties ctx, String fieldName) {
        String sql = "select distinct(" + fieldName + ") from m_product " + " where ad_client_id=" + Env.getAD_Client_ID(ctx) + " and AD_Org_ID=" + Env.getAD_Org_ID(ctx) + " and isActive='Y'" + " and IsWebstoreFeatured='Y'" + " and " + fieldName + " is not null";
        return sql;
    }
    ...
    protected static void setProductsActive(Properties ctx, Integer[] productIds, boolean active, String trxName) throws OperationException {
        if (productIds == null) throw new OperationException("Product Ids cannot be null");
        for (int i = 0; i < productIds.length; i++) {
            MProduct product = new MProduct(ctx, productIds[i].intValue(), trxName);
            if (product.get_ID() == 0) throw new OperationException("Could not load product with id: " + productIds[i]);
            if (active == false) {
                String errorMsg = POSProductManager.checkQty(ctx, product.get_ID(), trxName);
                if (errorMsg != null) throw new CannotInactivateProductException(errorMsg);
            }
    ...
    public static void updateProductsBarcode(Properties ctx, ArrayList productList, String trxName) throws OperationException {
        ...
        while (iter.hasNext()) {
            ...
            if (prodImageBean.getBarCode() != null && prodImageBean.getBarCode().length() > 0) updateProductBarcode(ctx, prodImageBean.getProductId().intValue(), prodImageBean.getBarCode(), trxName);
        }
    }
    ...
    public static void updateProductBarcode(Properties ctx, int productId, String barCode, String trxName) throws BarcodeAlreadyExistsException, OperationException {
        if (isBarCodePresent(ctx, barCode, trxName)) throw new BarcodeAlreadyExistsException("Barcode " + barCode + " has already been assigned to another product");
        MProduct product = new MProduct(ctx, productId, trxName);
        if (product.get_ID() == 0) throw new OperationException("Could not load product with id: " + productId);
    ...
    public static Timestamp getProductLastUpdatedTime(Properties ctx, int productId, String trxName) throws OperationException {
        ...
        try {
            ...
            if (rs.next()) retVal = rs.getTimestamp(1);
            rs.close();
        ...
        if (retVal == null) throw new OperationException("Could not retrieve product updated time with productId: " + productId);
        return retVal;
    }
