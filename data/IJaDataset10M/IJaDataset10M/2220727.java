...
public class Plan {
    ...
    private int getNextWeekToSave(int savedWeek) {
        ...
        while (iterator.hasNext()) {
            ...
            if (date.after(savedDate)) {
                ...
                if ((week > savedWeek) & (week < maxWeek)) maxWeek = week;
            }
        }
    ...
    private boolean saveWeek(int week, boolean isBackup) {
        ...
        if (isBackup) location = location + File.separator + "backup_diary.xml"; else location = location + File.separator + "diary_" + year + "_" + week + ".xml";
        try {
    ...
    public void copyUnfinishedTasks() {
        ...
        while (iterator.hasNext()) {
            ...
            if (task.getState() != Task.STATE_DONE) if (today.getTask(task.getDescription()) == null) today.addTask(task.cloneTask());
        }
    ...
    public Vector getCategories() {
        ...
        for (int i = 0; i < defaults.length; i++) {
            if (!allCategories.contains(defaults[i])) allCategories.add(defaults[i]);
        }
