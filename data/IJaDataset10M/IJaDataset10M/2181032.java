 ...
 */
public class StringExpander implements LogChute {
    ...
    private String getShellOutput(Session session, String command) throws ShellException {
        ...
        Shell shell = session.getShellManager().readShell(command, session.err == session.out);
        BufferedReader in = new BufferedReader(new InputStreamReader(shell.getStdout()));
        try {
            int ch = in.read();
            while (ch != -1) {
    ...
    public void log(int level, String message) {
        if (level >= LogChute.WARN_ID) {
            session.err.println(logPrefix(level) + ": Failed while performing variable expansion processing: " + message);
        }
    }
