...
public class ServerStoredContactListYahooImpl {
    ...
    private static final Logger logger = Logger.getLogger(ServerStoredContactListYahooImpl.class);
    ...
    private ContactListModListenerImpl contactListModListenerImpl = new ContactListModListenerImpl();
    ...
    void addGroupListener(ServerStoredGroupListener listener) {
        synchronized (serverStoredGroupListeners) {
            if (!serverStoredGroupListeners.contains(listener)) this.serverStoredGroupListeners.add(listener);
        }
    }
     ...
     */
    private void fireContactRemoved(ContactGroup parentGroup, ContactYahooImpl contact) {
        if (parentOperationSet == null) {
            logger.debug("No presence op. set available. Bailing out.");
            return;
        }
        parentOperationSet.fireSubscriptionEvent(SubscriptionEvent.SUBSCRIPTION_REMOVED, contact, parentGroup);
    }
     ...
     */
    private void fireContactMoved(ContactGroup oldParentGroup, ContactGroupYahooImpl newParentGroup, ContactYahooImpl contact) {
        if (parentOperationSet == null) {
            logger.debug("No presence op. set available. Bailing out.");
            return;
        }
    ...
    public ContactGroupYahooImpl findContactGroup(String name) {
        ...
        while (contactGroups.hasNext()) {
            ...
            if (contactGroup.getGroupName().equals(name)) return contactGroup;
        }
        return null;
    }
    ...
    public ContactGroup findContactGroup(ContactYahooImpl child) {
        ...
        while (contactGroups.hasNext()) {
            ContactGroupYahooImpl contactGroup = (ContactGroupYahooImpl) contactGroups.next();
            if (contactGroup.findContact(child.getAddress()) != null) return contactGroup;
        }
        return null;
    }
     ...
     */
    public void addContact(final ContactGroupYahooImpl parent, String id) throws OperationFailedException {
    ...
    ContactYahooImpl createVolatileContact(String id) {
        ...
        if (theVolatileGroup == null) {
            theVolatileGroup = new VolatileContactGroupYahooImpl(VOLATILE_GROUP_NAME, this);
    ...
    ContactYahooImpl createUnresolvedContact(ContactGroup parentGroup, String id) {
        ...
        ContactYahooImpl newUnresolvedContact = new ContactYahooImpl(id, this, false, false, false);
     ...
     */
    public void moveContact(ContactYahooImpl contact, ContactGroupYahooImpl newParent) {
    ...
    private ContactGroupYahooImpl getNonPersistentGroup() {
        for (int i = 0; i < getRootGroup().countSubgroups(); i++) {
            ContactGroupYahooImpl gr = (ContactGroupYahooImpl) getRootGroup().getGroup(i);
    ...
    private ContactGroupYahooImpl findContactGroupByYahooId(String id) {
        ...
        while (contactGroups.hasNext()) {
            ContactGroupYahooImpl contactGroup = (ContactGroupYahooImpl) contactGroups.next();
            if (contactGroup.getSourceGroup().getName().equals(id)) return contactGroup;
        }
        return null;
    }
     ...
     * @param contact the contact that was added
     */
    void fireContactAdded(ContactGroup parentGroup, ContactYahooImpl contact) {
        if (parentOperationSet == null) {
            logger.debug("No presence op. set available. Bailing out.");
            return;
        }
        parentOperationSet.fireSubscriptionEvent(SubscriptionEvent.SUBSCRIPTION_CREATED, contact, parentGroup);
    }
     ...
     */
    void fireContactResolved(ContactGroup parentGroup, ContactYahooImpl contact) {
        if (parentOperationSet == null) {
            logger.debug("No presence op. set available. Bailing out.");
            return;
        }
        parentOperationSet.fireSubscriptionEvent(SubscriptionEvent.SUBSCRIPTION_RESOLVED, contact, parentGroup);
    }
     ...
     */
    private class ContactListModListenerImpl extends SessionAdapter {
        ...
        public void authorizationReceived(SessionAuthorizationEvent ev) {
            if (ev.isAuthorizationAccepted()) {
                ...
                if (srcContact == null) logger.trace("No contact found"); else handler.processAuthorizationResponse(new AuthorizationResponse(AuthorizationResponse.ACCEPT, ev.getMessage()), srcContact);
            } else if (ev.isAuthorizationDenied()) {
                logger.trace("authorizationDenied from " + ev.getFrom());
                Contact srcContact = findContactById(ev.getFrom());
                if (srcContact == null) logger.trace("No contact found"); else {
                ...
                if (authResponse.getResponseCode() == AuthorizationResponse.IGNORE) return; else if (authResponse.getResponseCode() == AuthorizationResponse.REJECT) {
                    removeContact((ContactYahooImpl) srcContact);
                    try {
