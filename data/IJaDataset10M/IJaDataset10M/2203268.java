...
public class GigaWebApp {
    ...
    public void deploy() throws Exception {
        ...
        copy(docroot, _tmp);
        System.out.println("\n");
        if (_webAppContext == null) _webAppContext = new WebAppContext();
        ...
        if (contexts != null && contexts.length > 0) _container = (HandlerContainer) contexts[0]; else {
            while (_container != null) {
                if (_container instanceof HandlerWrapper) {
                    ...
                    if (handler instanceof HandlerContainer) _container = (HandlerContainer) handler; else throw new IllegalStateException("No container");
                }
                throw new IllegalStateException("No container");
            }
        }
        _container.addHandler(_webAppContext);
        if (_container.isStarted() || _container.isStarting()) _webAppContext.start();
    }
    ...
    public void undeploy() throws Exception {
        ...
        if (!_webAppContext.isRunning()) return;
        _webAppContext.stop();
        if (_container != null) _server.removeHandler(_webAppContext);
        IO.delete(_tmp);
    }
}
