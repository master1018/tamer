...
/**
 * Represents an activity that can be scheduled and run. This could
 * include graph processing actions and any number of animations.
 * 
 * @version 1.0
...
public abstract class Activity {
     ...
     * 
     * @param duration
     *            the length of this activity. A value of -1 indicates
     *            an infinite running time.
     * @see edu.berkeley.guir.prefuse.activity.Activity#Activity(long,
     *      long, long)
     */
    ...
    /**
     * Schedules this Activity with the ActivityManager. The Activity
     * will begin at the time indicated by this Activity's
     * {@link #getStartTime() getStartTime} method.
     */
    public void run() {
    ...
    /**
     * Schedules this Activity for the specified startTime,
     * overwriting the Activity's currently set startTime.
     * 
     * @param startTime
    ...
    /**
     * Schedules this Activity to start immediately after another
     * Activity. This Activity will be scheduled to start immediately
     * after the first one finishes, overwriting any previously set
     * startTime. If the first Activity is cancelled, this one will
     * not run.
     * 
     * This functionality is provided by using an ActivityListener to
     * monitor the first Activity. The listener is removed upon
     * completion or cancellation of the first Activity.
     * 
     * This method does not in any way affect the scheduling of the
     * first Activity. If the first Activity is never scheduled, this
     * Activity will correspondingly never be run unless scheduled by
     * a separate scheduling call.
     * 
     * @param before
     ...
     * 
     * This functionality is provided by using an ActivityListener to
     * monitor the first Activity. The listener will persist across
     * mulitple runs, meaning the second Activity will always be
    ...
    /**
     * Run this activity one step. Subclasses should override this
     * method to specify the actions this activity should perform.
     * 
     * @param elapsedTime
    ...
    /**
     * Run this activity for a single step. This method is called by
     * the ActivityManager -- outside code should have no need to call
     * or override this method. To implement custom activities,
     * override the run() method instead.
     * 
     * @param currentTime
     *            the time at which this step is being run.
     * @return the time (in milliseconds) when this activity should be
     *         run again. A return value of -1 indicates this activity
     *         is finished.
     */
    long runActivity(long currentTime) {
        ...
        if (m_duration == 0 || currentTime > getStopTime()) {
            if (!isRunning()) {
                setRunning(true);
                if (m_listener != null) m_listener.activityStarted(this);
            }
            if (m_enabled) {
                run(elapsedTime);
                if (m_listener != null) m_listener.activityStepped(this);
            }
            setRunning(false);
            if (m_listener != null) m_listener.activityFinished(this);
    ...
    /**
     * Cancels this activity, if scheduled or running. This will stop
     * a running activity, and will remove the activity from the
     * ActivityManager's schedule.
     */
    public void cancel() {
        if (isScheduled()) {
            if (m_listener != null) m_listener.activityCancelled(this);
            ActivityManager.removeActivity(this);
        }
        setRunning(false);
    }
    ...
    /**
     * Sets whether or not this Activity has been scheduled. This
     * method should only be called by the ActivityManager.
     * 
     * @param s
    ...
    void setScheduled(boolean s) {
        ...
        if (m_listener != null) m_listener.activityScheduled(this);
    }

    /**
     * Sets a flag indicating whether or not this activity is
     * currently running
     * 
     * @param s
    ...
    /**
     * Returns a value between 0 and 1 inclusive, indicating the
     * current position in an animation or other similarly
     * parameterized activity. The returned value is determined by
     * consulting this Activity's pacing function.
     * 
     * @param elapsedTime
     *            the time in milliseconds since the start of this
     *            Activity.
     * @return a value between 0 and 1 indicating the current position
     *         in an animation.
     ...
     * Pacing functions are used to control the pace of animations.
     * 
     * @return this Activity's pacing function. A value of null
     *         indicates a basic, linear pace is used, moving from 0
     *         to 1 uniformly over time.
     */
     ...
     *            indicate a basic, linear pace moving from 0 to 1
     *            uniformly over time.
     */
     ...
     * 
     * @param duration
     *            The new duration, in milliseconds, for this
     *            activity. A value of -1 indicates that this activity
