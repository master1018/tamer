...
public class SMTPHandler implements MessageHandlerFactory {
    ...
    public class Handler implements MessageHandler {
        ...
        public void recipient(String recipient) throws RejectException {
            if (SMTPHandler.this.smtpService.getInjector().accept(recipient)) {
                ...
                if (client == null) {
                    ...
                    try {
                        try {
                            if (!client.sentFrom()) client.from(this.from);
                            client.to(recipient);
        ...
        /** */
        public void done() {
            if (this.fallbackConnection != null) this.fallbackConnection.close();
        }
        ...
        List<Deliverer> makeDeliverers() {
            ...
            for (String list : this.ourLists) result.add(new OurDeliverer(SMTPHandler.this.smtpService.getInjector(), this.from, list));
            if ((this.fallbackConnection != null) && this.fallbackConnection.sentTo()) result.add(new FallbackDeliverer(this.fallbackConnection));
            return result;
        }
    }
}
