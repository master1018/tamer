...
public class LayoutExtra implements Layout, Extra {
    ...
    public void extendMenu(ToolsMenu menu, final MapController mc) {
        if (menu.getName().equals(DefaultMenuFactory.TOOLS_MENU)) menu.addTool(new Tool("LAYOUT_TRIVIAL", LayoutExtra.class.getName()) {
    ...
    public void layout(MapController controller) {
        layout(controller.getConceptMap());
    }
    ...
    public void layout(ContextMap cMap) {
        ...
        Rectangle cmbb = cMap.getBoundingBox();
        Dimension size = cmbb != null ? cmbb.getSize() : new Dimension(0, 0);
        ...
        for (Iterator iter = vnss1.iterator(); iter.hasNext(); ) {
            ...
            ContextMap.BoundingBox newbb = new ContextMap.BoundingBox(bb.dim, p);
    ...
    public void resizeBoxes(ContextMap cMap, MapController controller) {
        ...
        while (it.hasNext()) {
            ...
            java.awt.Dimension dim = controller.getView().getMapScrollPane().getDisplayer().getMapObject(ns.getURI()).getPreferredSize();
            ContextMap.BoundingBox bb = ns.getBoundingBox();
            ns.setBoundingBox(LayoutUtils.preferredBoxOnGrid(gridModel, bb.pos.x, bb.pos.y, dim));
        }
    }

    private void separate(DrawerLayout[] nss, Collection vnss1, Collection vnss2) {
        for (int i = 0; i < nss.length; i++) {
            DrawerLayout ns = nss[i];
            if (ns instanceof StatementLayout) vnss2.add(ns); else vnss1.add(ns);
        }
    }
