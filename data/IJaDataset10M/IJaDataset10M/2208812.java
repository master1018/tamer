...
public class AddEntryExitPairPanel extends jmri.util.swing.JmriPanel {
    ...
    public AddEntryExitPairPanel(LayoutEditor panel) {
        ...
        if (panel != null) selectPanel.setSelectedItem(panel.getLayoutName());
    ...
    private void autoDiscovery() {
        ...
        int retval = JOptionPane.showOptionDialog(null, rb.getString("AutoGenEntryExitMessage"), rb.getString("AutoGenEntryExitTitle"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, null);
        if (retval == 0) {
    ...
    ValidPoints getValidPointFromCombo(JComboBox box) {
        ...
        for (int i = 0; i < validPoints.size(); i++) {
            if (validPoints.get(i).getDescription().equals(item)) return validPoints.get(i);
        }
        return null;
    }
    ...
    private void selectPointsFromPanel() {
        if (selectPanel.getSelectedIndex() == -1) return;
    ...
    String getPointAsString(PositionablePoint p, Boolean direction) {
        ...
        if (direction) {
            if ((p.getEastBoundSignalMast() != null) && (!p.getEastBoundSignalMast().equals(""))) {
                ...
                if ((p.getEastBoundSensor() != null) && (!p.getEastBoundSensor().equals(""))) description = description + " (" + p.getEastBoundSensor() + ")";
            ...
            if ((p.getWestBoundSignalMast() != null) && (!p.getWestBoundSignalMast().equals(""))) {
                ...
                if ((p.getWestBoundSensor() != null) && (!p.getWestBoundSensor().equals(""))) description = description + " (" + p.getWestBoundSensor() + ")";
    ...
    class TableModel extends javax.swing.table.AbstractTableModel implements PropertyChangeListener {
        ...
        public int getRowCount() {
            if (panel != null) return nxPairs.getNxPairNumbers(panel);
            return 0;
        }
        ...
        public void setValueAt(Object value, int row, int col) {
            ...
            if (col == TYPECOL) {
                ...
                if (val.equals("Turnout")) nxPairs.setEntryExitType(source.get(row), panel, dest.get(row), 0x00); else if (val.equals("SignalMast")) nxPairs.setEntryExitType(source.get(row), panel, dest.get(row), 0x01); else if (val.equals("Full InterLock")) nxPairs.setEntryExitType(source.get(row), panel, dest.get(row), 0x02);
            }
        }
        ...
        String isPairActive(int row) {
            if (nxPairs.isPathActive(source.get(row), dest.get(row), panel)) return ("*");
            return ("");
        }
    ...
    protected void configDeleteColumn(JTable table) {
        setColumnToHoldButton(table, DELETECOL, new JButton(rb.getString("ButtonDelete")));
