...
public final class Marker extends SeqPosition {
    ...
    public final boolean isPoint() {
        return buffer != null && sequence == null;
    }
    ...
    public Marker(Marker marker) {
        ...
        if (buffer != null) {
            if (marker.isPoint()) set(buffer, buffer.getDot(), true); else set(marker);
        }
    }
    ...
    public int getOffset() {
        if (buffer == null) return -1; else if (isPoint()) return buffer.getDot();
        return nextIndex();
    }
    ...
    public void set(Buffer newBuffer, int newPosition) {
        if (isPoint()) {
            if (newBuffer != buffer) {
                String msg;
                if (newBuffer == null) msg = "Can't make point-marker point nowhere: "; else msg = "Can't change buffer of point-marker: ";
                throw new Error(msg + this);
            }
            buffer.setDot(newPosition);
        } else {
            if (sequence != null) release();
            sequence = null;
            if (newBuffer == null) {
                buffer = null;
                return;
            }
            if (newPosition < 0) newPosition = 0; else {
    ...
    public void removeChar(int count) {
        if (isPoint()) buffer.removeChar(count); else buffer.removePos(ipos, count);
    }
    ...
    public void insert(String string, Object style) {
        int point = getOffset();
        if (isPoint()) buffer.insert(string, style); else buffer.insert(string, style, ipos);
        point += string.length();
        setDot(point);
    }
    ...
    public String toString() {
        if (buffer == null) return "#<marker in no buffer>";
