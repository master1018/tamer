...
public class AutoCompleteTextField extends JTextField implements KeyListener, DocumentListener {
    ...
    private String getCurrentGuess() {
        if (this.currentGuess != -1) return this.possibilities.get(this.currentGuess);
        return this.getText();
    }
    ...
    private void findCurrentGuess() {
        ...
        if (!this.caseSensitive) entered = entered.toLowerCase();
    ...
    public void paintComponent(Graphics g) {
        ...
        if (!(guess.startsWith(entered))) this.areGuessing = false;
    ...
    public void insertUpdate(DocumentEvent e) {
        ...
        if (temp.length() == 1) this.areGuessing = true;
        if (this.areGuessing) this.findCurrentGuess();
    }

    public void removeUpdate(DocumentEvent e) {
        String temp = this.getText();
        if (!(this.areGuessing)) this.areGuessing = true;
        if (temp.length() == 0) this.areGuessing = false; else if (this.areGuessing) {
