...
public class Coordinate2DInterpolator extends Interpolator {
    ...
    public void addKeyFrame(float key, float coords[]) {
        ...
        if (loc < 0) loc = 0;
        while (loc < currentSize && keys[loc] <= key) loc++;
        realloc();
        if (coords == null) throw new IllegalArgumentException("Coord array is null");
        int len = coords.length;
        if (len < 2 || len % 2 != 0) throw new IllegalArgumentException("Coordinates length not x 2");
        float coords1[] = new float[len];
        System.arraycopy(coords, 0, coords1, 0, len);
        if (valueLength > len || valueLength < 0) valueLength = len;
        if (loc >= currentSize) {
    ...
    public float[] floatValue(float key) {
        if (sharedVector == null || sharedVector.length != valueLength) sharedVector = new float[valueLength];
        int loc = findKeyIndex(key);
        if (loc < 0) System.arraycopy(keyValues[0], 0, sharedVector, 0, valueLength); else if (loc >= currentSize - 1) System.arraycopy(keyValues[currentSize - 1], 0, sharedVector, 0, valueLength); else {
            switch(interpolationType) {
                case LINEAR:
                    ...
                    float diff;
                    if (next_key != prev_key) fraction = (key - prev_key) / (next_key - prev_key);
