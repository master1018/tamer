...
public class KXmlSerializer implements XmlSerializer {
    ...
    private final void writeEscaped(String s, int quot) throws IOException {
        for (int i = 0; i < s.length(); i++) {
            ...
            switch(c) {
                ...
                case '\r':
                case '\t':
                    if (quot == -1) writer.write(c); else writer.write("&#" + ((int) c) + ';');
                    break;
                case '&':
                    writer.write("&amp;");
                    break;
                ...
                default:
                    if (c >= ' ' && c != '@' && (c < 127 || unicode)) writer.write(c); else writer.write("&#" + ((int) c) + ";");
            }
        }
    }
    ...
    private final String getPrefix(String namespace, boolean includeDefault, boolean create) throws IOException {
        ...
        String prefix;
        if ("".equals(namespace)) prefix = ""; else {
            do {
                prefix = "n" + (auto++);
                for (int i = nspCounts[depth + 1] * 2 - 2; i >= 0; i -= 2) {
    ...
    public XmlSerializer endTag(String namespace, String name) throws IOException {
        if (!pending) depth--;
        if ((namespace == null && elementStack[depth * 3] != null) || (namespace != null && !namespace.equals(elementStack[depth * 3])) || !elementStack[depth * 3 + 2].equals(name)) throw new IllegalArgumentException("</{" + namespace + "}" + name + "> does not match start");
        if (pending) {
            check(true);
            depth--;
        } else {
