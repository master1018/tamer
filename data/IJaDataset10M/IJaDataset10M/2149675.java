...
public class Thief_Graffiti extends ThiefSkill {
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        if ((auto) && (givenTarget != null) && (givenTarget instanceof Room)) target = (Room) givenTarget;
        if ((mob.location().domainType() != Room.DOMAIN_OUTDOORS_CITY) && (mob.location().domainType() != Room.DOMAIN_INDOORS_WOOD) && (mob.location().domainType() != Room.DOMAIN_INDOORS_STONE)) {
        ...
        } else beneficialVisualFizzle(mob, target, "<S-NAME> attempt(s) to write graffiti here, but fails.");
        return success;
    }
}
