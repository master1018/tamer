...
public class AutoDeleteMVBTree extends MVBTree {
    ...
    protected static class NotifyHeap<E> extends DynamicHeap<E> {
        ...
        public E dequeueObject() {
            ...
            if (size > 1) {
                int index = sinkIn(1);
                if (index < size - 1) bubbleUp((E) array[size - 1], index);
            }
        ...
        protected void update(int index) {
            ...
            if (i != 0) {
                while (2 * i < size) {
                    ...
                    if (smallerIndex + 1 < size) if (comparator.compare((E) array[smallerIndex], (E) array[smallerIndex + 1]) > 0) smallerIndex++;
    ...
    protected NotifyHeap<LeafInfo> leafInfoHeap = new NotifyHeap<LeafInfo>(new Comparator<LeafInfo>() {
    ...
    public void insert(Version insertVersion, Version deleteVersion, Object data) {
        ...
        if (insertVersion.compareTo(deleteVersion) >= 0) throw new IllegalArgumentException("insertVersion must be smaller than deleteVersion.");
        setCurrentVersion(insertVersion);
        while (!leafInfoHeap.isEmpty() && leafInfoHeap.peek().getUnderflowVersion().compareTo(currentVersion) <= 0) {
    ...
    protected LeafInfo removeLeafInfo(Object nodeId) {
        ...
        if (old != null) leafInfoHeap.remove(old.getHeapPos());
        assert leafInfoMap.size() == leafInfoHeap.size();
        return old;
    }
    ...
    public class Node extends MVBTree.Node {
        ...
        public Iterator getCurrentEntries() {
            return new Filter(iterator(), new AbstractPredicate() {

                public boolean invoke(Object entry) {
                    return ((MVSeparator) separator(entry)).isAlive() || ((MVSeparator) separator(entry)).deleteVersion().compareTo(currentVersion()) > 0;
                }
            });
        }

        protected int countEntriesFromVersion(final Version version) {
            return Cursors.count(new Filter(iterator(), new AbstractPredicate() {
        ...
        protected Version computeUnderflowVersion() {
            if (level() != 0) throw new IllegalArgumentException("Cannot compute underflow version of a non-leaf node.");
            List<LeafEntry> list = new ArrayList<LeafEntry>();
            Iterator<LeafEntry> it = getCurrentEntries();
            while (it.hasNext()) list.add(it.next());
            Collections.sort(list, new Comparator<LeafEntry>() {
        ...
        protected Object computeReferenceKey() {
            if (level() != 0) throw new IllegalArgumentException("Cannot compute reference key of a non-leaf node.");
