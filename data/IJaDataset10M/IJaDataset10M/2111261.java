...
public class BasicDiagnosticFormatter extends AbstractDiagnosticFormatter {
    ...
    public String formatDiagnostic(JCDiagnostic d, Locale l) {
        if (l == null) l = messages.getCurrentLocale();
        ...
        if (depth == 0) return addSourceLineIfNeeded(d, buf.toString()); else return buf.toString();
    }
    ...
    protected String addSourceLineIfNeeded(JCDiagnostic d, String msg) {
        if (!displaySource(d)) return msg; else {
            ...
            if (singleLine || getConfiguration().getSourcePosition() == SourcePosition.BOTTOM) return msg + sourceLine; else return msg.replaceFirst("\n", Matcher.quoteReplacement(sourceLine) + "\n");
        }
    }
            ...
            case 't':
                {
                    ...
                    if (usePrefix) return formatKind(d, l); else return "";
                }
            case 'm':
    ...
    public static class BasicConfiguration extends SimpleConfiguration {
        ...
        public BasicConfiguration(Options options) {
            super(options, EnumSet.of(DiagnosticPart.SUMMARY, DiagnosticPart.DETAILS, DiagnosticPart.SUBDIAGNOSTICS, DiagnosticPart.SOURCE));
            initFormat();
            initIndentation();
            if (options.isSet("oldDiags")) initOldFormat();
            String fmt = options.get("diagsFormat");
            if (fmt != null) {
                if (fmt.equals("OLD")) initOldFormat(); else initFormats(fmt);
            }
            String srcPos = null;
            if ((((srcPos = options.get("sourcePosition")) != null)) && srcPos.equals("bottom")) setSourcePosition(SourcePosition.BOTTOM); else setSourcePosition(SourcePosition.AFTER_SUMMARY);
            ...
            if (indent != null) {
                ...
                try {
                    switch(levels.length) {
                        ...
                        case 4:
                            setIndentation(DiagnosticPart.SUBDIAGNOSTICS, Integer.parseInt(levels[3]));
                        case 3:
        ...
        public BasicConfiguration() {
            super(EnumSet.of(DiagnosticPart.SUMMARY, DiagnosticPart.DETAILS, DiagnosticPart.SUBDIAGNOSTICS, DiagnosticPart.SOURCE));
            initFormat();
            initIndentation();
        }
