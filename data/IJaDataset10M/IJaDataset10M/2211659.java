...
public class BufferQueueInputStream extends InputStream {
        ...
        try {
            synchronized (q) {
                do {
                    ...
                    if (buffer.getLength() == 0 && !buffer.isDiscard()) if (trace) logger.fine("Skipping zero-length buffer in queue");
    ...
    private boolean put(Buffer b, boolean block, boolean clone) {
        if (b.getLength() == -1) throw new IllegalArgumentException();
        if (!(b.getData() instanceof byte[])) throw new IllegalArgumentException("Expected byte array: " + b.getData());
        if (b.isEOM()) {
        ...
        try {
            synchronized (q) {
                if (!block && q.isFull()) return false;
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        final byte[] data = (byte[]) buffer.getData();
        if (data == null) throw new NullPointerException("Buffer has null data.  length=" + buffer.getLength() + " offset=" + buffer.getOffset());
