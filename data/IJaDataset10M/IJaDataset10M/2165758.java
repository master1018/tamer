...
class MultiSegmentReader extends DirectoryIndexReader {
    ...
    public synchronized void norms(String field, byte[] result, int offset) throws IOException {
        ...
        if (bytes != null) System.arraycopy(bytes, 0, result, offset, maxDoc());
        for (int i = 0; i < subReaders.length; i++) subReaders[i].norms(field, result, offset + starts[i]);
    }
    ...
    protected synchronized void doClose() throws IOException {
        for (int i = 0; i < subReaders.length; i++) subReaders[i].decRef();
        super.doClose();
    }
    ...
    public void setTermInfosIndexDivisor(int indexDivisor) throws IllegalStateException {
        for (int i = 0; i < subReaders.length; i++) subReaders[i].setTermInfosIndexDivisor(indexDivisor);
    }

    public int getTermInfosIndexDivisor() throws IllegalStateException {
        if (subReaders.length > 0) return subReaders[0].getTermInfosIndexDivisor(); else throw new IllegalStateException("no readers");
    }
    ...
    static class MultiTermEnum extends TermEnum {
        ...
        public MultiTermEnum(IndexReader[] readers, int[] starts, Term t) throws IOException {
            ...
            for (int i = 0; i < readers.length; i++) {
                ...
                if (t == null ? smi.next() : termEnum.term() != null) queue.put(smi); else smi.close();
            }
        ...
        public boolean next() throws IOException {
            ...
            while (top != null && term.compareTo(top.term) == 0) {
                ...
                if (top.next()) queue.put(top); else top.close();
