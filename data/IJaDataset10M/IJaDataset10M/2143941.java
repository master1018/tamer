        ...
        switch(sUriMatcher.match(data)) {
            ...
            case SHOPPINGLISTS:
                if (affectedRows == null) {
                    context.getContentResolver().delete(VoiceNote.CONTENT_URI, VoiceNote.DATA_URI + " LIKE ?", new String[] { data.toString() + "/%" });
                } else {
                    for (int i = 0; i < affectedRows.length; i++) {
                        Uri specificdata = Uri.withAppendedPath(data, "" + affectedRows[i]);
                        context.getContentResolver().delete(VoiceNote.CONTENT_URI, VoiceNote.DATA_URI + " = ?", new String[] { specificdata.toString() });
                    }
                }
                break;
            case NOTE_ID:
            case SHOPPINGLIST_ID:
                if (affectedRows == null || affectedRows.length >= 1) {
                    context.getContentResolver().delete(VoiceNote.CONTENT_URI, VoiceNote.DATA_URI + " = ?", new String[] { data.toString() });
                } else {
                }
                break;
        }
