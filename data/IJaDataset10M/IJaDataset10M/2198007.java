...
public final class DbUtilities {
     ...
     */
    public static Databases getDatabases(DbEnvironment env, DbTransaction dbTxn, LoggerWrapper logger) {
        ...
        try {
            ...
            if (logger.isLoggable(Level.CONFIG)) {
                logger.log(Level.CONFIG, "Found existing header {0}", DataStoreHeader.headerString(minorVersion));
            }
            ...
            if (logger.isLoggable(Level.CONFIG)) {
                logger.log(Level.CONFIG, "Created new header {0}", DataStoreHeader.headerString());
            }
            create = true;
        }
        try {
            ...
            throw new DataStoreException("Classes database not found: " + e.getMessage(), e);
        }
        try {
            dbs.oids = env.openDatabase(dbTxn, "oids", create);
        } catch (FileNotFoundException e) {
            throw new DataStoreException("Oids database not found: " + e.getMessage(), e);
        }
        try {
            dbs.names = env.openDatabase(dbTxn, "names", create);
        } catch (FileNotFoundException e) {
            throw new DataStoreException("Names database not found: " + e.getMessage(), e);
        }
        return dbs;
    }
     ...
     */
    public static long getNextObjectId(DbDatabase infoDb, DbTransaction dbTxn, int blockSize) {
        return DataStoreHeader.getNextId(NEXT_OBJ_ID_KEY, infoDb, dbTxn, blockSize);
    }
     ...
     */
    public static long getNextNodeId(DbDatabase infoDb, DbTransaction dbTxn, int blockSize) {
        return DataStoreHeader.getNextId(NEXT_NODE_ID_KEY, infoDb, dbTxn, blockSize);
    }
     ...
     */
    public static int getClassId(DbEnvironment env, DbDatabase classesDb, byte[] classInfo, long timeout) {
        ...
        try {
            ...
            if (hashValue != null) {
                ...
                try {
                    result = cursor.findLast() ? getClassIdFromKey(cursor.getKey()) + 1 : 1;
                ...
                boolean success = classesDb.putNoOverwrite(dbTxn, hashKey, DataEncoding.encodeInt(result));
                if (!success) {
     ...
     *		{@code 0}
     */
    public static byte[] getClassInfo(DbEnvironment env, DbDatabase classesDb, int classId, long timeout) {
        if (classId <= 0) {
