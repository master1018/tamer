...
public class LogFile {
    ...
    public void setLocal(boolean val) {
        if (_file == null) throw new IllegalStateException("In order to make a logFile local one must first associate it with an local file");
        _existsLocally = val;
    }
    ...
    public void loadHeaders() {
        if (!this.isLocal()) throw new IllegalStateException("You can not load the headers of a remote-only file");
        if (this.areHeadersLoaded()) return;
    ...
    public void loadFully() {
        if (this.areHeadersLoaded()) loadHeaders();
        try {
    ...
    public void setRemoteAnalyserBits(long bits) {
        if (!this.isRemote()) PacketSamurai.getUserInterface().log("We are setting a remote property on a non-remote file. Things are going  wrong...");
        _remoteBitset = bits;
    }
