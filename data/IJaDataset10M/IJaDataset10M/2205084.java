 ...
 */
    public TriangularDistribution(double low, double high, double peak) throws IllegalArgumentException {
        if (low >= high) throw new IllegalArgumentException("Limits of distribution are equal or reversed");
        if (peak < low || peak > high) throw new IllegalArgumentException("Peak of distribution lies outside the limits");
        a = low;
        b = high;
        c = peak;
    }
 ...
 */
    public TriangularDistribution(Histogram h) {
        b = h.standardDeviation() * 1.73205080756888;
        c = h.average();
        a = c - b;
        b += c;
    }
 ...
 */
    public double distributionValue(double x) {
        if (x < a) return 0; else if (x < c) return (x - a) * (x - a) / ((b - a) * (c - a)); else if (x < b) return 1 - (b - x) * (b - x) / ((b - a) * (b - c)); else return 1;
    }
 ...
 */
    protected double privateInverseDistributionValue(double x) {
        return (x < (c - a) / (b - a)) ? Math.sqrt(x * (b - a) * (c - a)) + a : b - Math.sqrt((1 - x) * (b - a) * (b - c));
    }
 ...
 */
    public double value(double x) {
        if (x < a) return 0; else if (x < c) return 2 * (x - a) / ((b - a) * (c - a)); else if (x < b) return 2 * (b - x) / ((b - a) * (b - c)); else return 0;
    }
