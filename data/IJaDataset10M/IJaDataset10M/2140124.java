...
public class GeneralName {
    ...
    public GeneralName(Object obj, int type) throws ASN1Exception {
        if ((type < 0) || (type > 8)) throw new ASN1Exception("Wrong type");
    ...
    private void init(ASN1Object ss) {
        ...
        int tag = ss.getTag();
        if (tag >= 0 && tag <= 8) generalNameID = tag; else generalNameID = tag - 32;
        if (tag == 0) {
            System.out.println("not Implemented");
        } else if (tag == 0x01) {
            ASN1Object tmp1 = (ASN1Object) ss.getValue();
            if (tmp1.isASN1Type(ASN1Type.OCTET_STRING) == false) return;
    ...
    public Object getValue(int type) {
        ...
        if (type == 1) return rfc822Name; else if (type == 2) return dNSName; else if (type == 4) return directoryName; else if (type == 6) return uRID; else if (type == 7) return ipAddress; else if (type == 8) return registeredOID;
        return null;
    }
    ...
    public ASN1Object toASN1Object() {
        ...
        if (generalNameID == 1 && rfc822Name != null) {
            try {
                mm = new Context(0x01, true, ASN1Object.create(ASN1Type.IA5String, rfc822Name));
                mm.initByteArray();
            } catch (Exception e) {
                System.out.println(e);
            }
            ...
            try {
                mm = new Context(0x02, true, ASN1Object.create(ASN1Type.IA5String, dNSName));
                mm.initByteArray();
            } catch (Exception e) {
                System.out.println(e);
            }
            ...
            try {
                mm = new Context(0x04, false, directoryName.toASN1Object());
                mm.initByteArray();
            } catch (Exception e) {
                System.out.println(e);
            }
            ...
            try {
                mm = new Context(0x06, true, ASN1Object.create(ASN1Type.IA5String, uRID));
                mm.initByteArray();
            } catch (Exception e) {
                System.out.println(e);
            }
            ...
            try {
                mm = new Context(0x07, true, ASN1Object.create(ASN1Type.OCTET_STRING, ipAddress.getBytes()));
                mm.initByteArray();
            } catch (Exception e) {
                System.out.println(e);
            }
            ...
            try {
                mm = new Context(0x08, true, ASN1Object.create(ASN1Type.OBJECT_ID, registeredOID));
                mm.initByteArray();
            } catch (Exception e) {
                System.out.println(e);
            }
