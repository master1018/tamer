...
public class AutoBatchIngest implements Constants {
    ...
    public AutoBatchIngest(String objectDir, String logFile, String logFormat, String objectFormat, String host, String port, String username, String password, String protocol, String context) throws Exception {
    ...
    public static final void main(String[] args) throws Exception {
        ...
        if (args.length == 8 || args.length == 9) {
            if (!new File(args[0]).isDirectory()) {
                System.out.println("Specified object directory: \"" + args[0] + "\" is not a directory.");
                errors = true;
            }
            ...
            if (!args[3].equals(FOXML1_1.uri) && !args[3].equals(METS_EXT1_1.uri)) {
                System.out.println("Object format must must be either: \"" + "\"" + FOXML1_1.uri + "\" or \"" + METS_EXT1_1.uri + "\"");
                errors = true;
            }
            String[] server = args[4].split(":");
            if (server.length != 2) {
                System.out.println("Specified server name does not specify " + "port number: \"" + args[4] + "\" .");
                errors = true;
            }
            ...
            if (!errors) {
                AutoBatchIngest autoBatch = new AutoBatchIngest(args[0], args[1], args[2], args[3], server[0], server[1], args[5], args[6], args[7], context);
            }
        } else {
