...
public class CsvNamesFirstLineFileLoader implements CopybookLoader {
	 ...
	 */
    public ExternalRecord loadCopyBook(String copyBookFile, int splitCopybookOption, int dbIdx, String font, int binFormat, int systemId, AbsSSLogger log) throws IOException, RecordException {
        TextLineReader r = new TextLineReader(new DefaultLineProvider(), true);
        r.setDefaultDelim(fieldSeperator);
        r.open(copyBookFile);
        r.read();
        r.close();
        return ToExternalRecord.getInstance().getExternalRecord(r.getLayout(), Conversion.getCopyBookId(copyBookFile), systemId);
    }
