...
public class ArffTable extends JTable {
   ...
   */
    public TableCellEditor getCellEditor(int row, int column) {
        ...
        if ((getModel() instanceof ArffSortedTableModel) && (((ArffSortedTableModel) getModel()).getType(column) == Attribute.RELATIONAL)) result = new RelationalCellEditor(row, column); else result = super.getCellEditor(row, column);
        return result;
    }
   ...
   */
    public String getPlainColumnName(int columnIndex) {
        ...
        if ((columnIndex >= 0) && (columnIndex < getColumnCount())) {
            if (columnIndex == 0) result = "No."; else result = arffModel.getAttributeAt(columnIndex).name();
        }
        return result;
    }
   ...
   */
    public void setSelectedColumn(int index) {
        ...
        resizeAndRepaint();
        if (getTableHeader() != null) getTableHeader().resizeAndRepaint();
    }
   ...
   */
    private void notifyListener() {
        ...
        while (iter.hasNext()) ((ChangeListener) iter.next()).stateChanged(new ChangeEvent(this));
    }
