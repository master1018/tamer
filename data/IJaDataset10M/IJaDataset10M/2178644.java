 ...
 * 
 * @version DESMO-J, Ver. 2.3.4beta copyright (c) 2011
 ...
 *
 */
public class TextHistogram extends desmoj.core.statistic.StatisticObject {
	 ...
	 */
    public TextHistogram(Model ownerModel, String name, TextSupplier textSup, boolean showInReport, boolean showInTrace) {
        super(ownerModel, name, showInReport, showInTrace);
        if (textSup == null) {
            sendWarning("Attempt to produce a TextHistogram about a non existing " + "TextValueSupplier. The command will be ignored!", "TextHistogram: " + this.getName() + " Constructor: TextHistogram" + " (Model ownerModel, String name, TextSupplier textSup, " + "boolean showInReport, boolean showInTrace)", "The given TextSupplier: textSup is only a null pointer.", "Make sure to pass a valid TextSupplier when constructing a new " + "TextHistogram object.");
            return;
        }
    ...
    public void update() {
        if (this._textSuppl == null) {
            sendWarning("Attempt to update a TextHistogram that is not " + "connected to a TextSupplier. No String is provided with which " + "the histogram could be updated. The command will be ignored!", "TextHistogram: " + this.getName() + " Method: update()", "The given TextSupplier is only a null pointer.", "Make sure to update a TextHistogram only when it is connected " + "to a valid TextSupplier. Or use the update(String updateStr) method.");
            return;
        }
        this._lastString = this._textSuppl.text();
        if (this._lastString == null) {
            sendWarning("Attempt to insert a null pointer to the TextHistogram. " + "The null pointer will not be counted! ", "update(): ", "Invalid reference. ", "Make sure to pass a valid String when calling the " + "update method.");
            return;
        } else {
    ...
    public void update(Observable o, Object arg) {
        if (arg == null || o instanceof SimClock) {
            if (_textSuppl == null) {
                sendWarning("Attempt to update a TextHistogram that is not " + "connected to a TextValueSupplier. No value is provided with which " + "the statistic could be updated. The command will be ignored!", "TextHistogram: " + this.getName() + " Method: update " + "(Observable o, Object arg)", "The given TextValueSupplier: TextValSuppl is only a null pointer.", "Make sure to update a TextHistogram only when it is connected " + "to a valid TextValueSupplier. Or use the update(String) method.");
                return;
            }
            this._lastString = this._textSuppl.text();
            if (this._lastString == null) {
            ...
            if (arg instanceof String) {
                ...
                sendWarning("Attempt to update a TextHistogram with an argument " + "arg, that can not be recognized. The attempted action is ignored!", "TextHistogram: " + this.getName() + " Method: update (Observable " + "o, Object arg)", "The passed Object in the argument arg could not be recognized.", "Make sure to pass null or a String object as the arg argument.");
                return;
            }
        }
        incrementObservations();
