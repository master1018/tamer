...
class SlicingThread implements Runnable {
    ...
    SlicingThread(final List<NodePairInfo> nodepairs, final AtomicInteger index, final List<ClonePairInfo> clonepairList) {
    ...
    public void run() {
        while (true) {
            ...
            if (Configuration.INSTANCE.getS() <= Math.min(clonepair.codecloneA.length(), clonepair.codecloneB.length())) {
    ...
    static synchronized void increaseNumberOfPairs() {
        numberOfPairs++;
    }

    static synchronized void increaseNumberOfComparison() {
        numberOfComparion++;
    }
