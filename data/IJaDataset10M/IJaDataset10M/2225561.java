...
public class TernaryPlot3dDisplay extends CoDa3dDisplay {
    ...
    protected TernaryPlot3dDisplay(TernaryPlot3dBuilder builder) {
        ...
        for (int i = 0; i < gnames.length; i++) legendNames.add(new LegendItem(gnames[i], "data".concat(Integer.toString(i))));
        ...
        for (int i = 0; i < n; i++) {
            showZ[i] = false;
            origZ[i] = CoDaStats.ternaryTransform(data[0][i], data[1][i], data[2][i], data[3][i]);
        }
    }
    ...
    public void setCentered(boolean centered) {
        ...
        if (centered) {
            ...
            for (int i = 0; i < origZ.length; i++) {
                origZ[i] = CoDaStats.ternaryTransform(cdata[0][i], cdata[1][i], cdata[2][i], cdata[3][i]);
            }
        } else {
            for (int i = 0; i < origZ.length; i++) {
                origZ[i] = CoDaStats.ternaryTransform(data[0][i], data[1][i], data[2][i], data[3][i]);
    ...
    public void paintComponent(Graphics2D g2, double width, double height) {
        ...
        defaultTransform = new AffineTransform(factor, 0, 0, -factor, width / 2.3, height / 1.9);
        drawAreas(g2);
        drawAxis(g2);
        drawData(g2);
        drawLabels(g2);
    ...
    private boolean sameSide(double p1, double p2, double q1, double q2, double a1, double a2, double b1, double b2) {
        return crossProduct(b1 - a1, b2 - a2, p1 - a1, p2 - a2) * crossProduct(b1 - a1, b2 - a2, q1 - a1, q2 - a2) < 0 ? false : true;
    }

    private boolean isInside(double p1, double p2, double a1, double a2, double b1, double b2, double c1, double c2) {
        return sameSide(p1, p2, a1, a2, b1, b2, c1, c2) && sameSide(p1, p2, b1, b2, a1, a2, c1, c2) && sameSide(p1, p2, c1, c2, a1, a2, b1, b2);
    }

    private void drawAxis(Graphics2D g2) {
        g2.setColor(CoDaDisplayConfiguration.getColor("axis"));
        BasicStroke normal = new BasicStroke(CoDaDisplayConfiguration.getSize("axis"), BasicStroke.JOIN_MITER, BasicStroke.CAP_ROUND);
        float dash1[] = { 2.0f };
        BasicStroke dashed = new BasicStroke(CoDaDisplayConfiguration.getSize("axis"), BasicStroke.JOIN_MITER, BasicStroke.CAP_ROUND, 1.0f, dash1, 2f);
        g2.setStroke(normal);
        int temp;
        ...
        if (isInside(V[indexOrder[3]][0], V[indexOrder[3]][1], V[indexOrder[2]][0], V[indexOrder[2]][1], V[indexOrder[1]][0], V[indexOrder[1]][1], V[indexOrder[0]][0], V[indexOrder[0]][1])) {
        ...
        } else if (isInside(V[indexOrder[0]][0], V[indexOrder[0]][1], V[indexOrder[2]][0], V[indexOrder[2]][1], V[indexOrder[1]][0], V[indexOrder[1]][1], V[indexOrder[3]][0], V[indexOrder[3]][1])) {
            g2.setStroke(dashed);
    ...
    private void drawData(Graphics2D g2) {
        g2.setStroke(new BasicStroke(0.5f, BasicStroke.JOIN_MITER, BasicStroke.CAP_ROUND));
        ...
        for (int i = 0; i < Z.length; i++) {
            ...
            if (showZ[i] | showAllZ) {
                g2.setColor(Color.BLACK);
                if (this.obsNames == null) g2.drawString(Integer.toString(mapping[i] + 1), (float) o.getX() + s, (float) o.getY() - s); else g2.drawString(obsNames[mapping[i]], (float) o.getX() + s, (float) o.getY() - s);
            }
        }
    }
    ...
    private void drawAreas(Graphics2D g2) {
        ...
        Triangle2D border = new Triangle2D(new Vertex2D(o1), new Vertex2D(o2), new Vertex2D(o3));
        g2.fill(border);
        border = new Triangle2D(new Vertex2D(o2), new Vertex2D(o3), new Vertex2D(o4));
        g2.fill(border);
        border = new Triangle2D(new Vertex2D(o3), new Vertex2D(o4), new Vertex2D(o1));
        g2.fill(border);
        border = new Triangle2D(new Vertex2D(o4), new Vertex2D(o1), new Vertex2D(o2));
        g2.fill(border);
    }
    ...
    private void drawLabels(Graphics2D g2) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            double norm = Math.sqrt(sep[i][0] * sep[i][0] + sep[i][1] * sep[i][1] + sep[i][2] * sep[i][2]);
            sep[i][0] /= norm;
            sep[i][1] /= norm;
        }
        for (short i = 0; i < 4; i++) {
            ...
            g2.drawString(text, (int) o.getX() - metric.stringWidth(text) / 2, (int) o.getY() + metric.getHeight() / 2);
        }
    }
}
