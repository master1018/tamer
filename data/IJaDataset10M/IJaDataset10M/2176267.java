...
public class DefaultCellRenderer extends GridCellRenderer {
    ...
    public void paint(GC gc, Object value) {
        ...
        if (isTree()) {
            ...
            if (item.hasChildren()) toggleRenderer.paint(gc, null);
            if (renderBranches) {
    ...
    private int[] getBranches(GridItem item) {
        ...
        if (item.getParentItem() == null) {
            if (!item.isExpanded() && roots[roots.length - 1].equals(item)) {
                if (item.hasChildren()) branches[item.getLevel()] = BranchRenderer.LAST_ROOT; else branches[item.getLevel()] = BranchRenderer.SMALL_L;
            } else {
                if (item.hasChildren()) branches[item.getLevel()] = BranchRenderer.ROOT; else branches[item.getLevel()] = BranchRenderer.SMALL_T;
            }
        } else if (item.hasChildren()) if (item.isExpanded()) branches[item.getLevel()] = BranchRenderer.NODE; else branches[item.getLevel()] = BranchRenderer.NONE; else branches[item.getLevel()] = BranchRenderer.LEAF;
        GridItem parent = item.getParentItem();
        if (parent == null) return branches;
        if (parent.indexOf(item) < parent.getItemCount() - 1) branches[item.getLevel() - 1] = BranchRenderer.T; else if (parent.getParentItem() == null && !parent.equals(roots[roots.length - 1])) branches[item.getLevel() - 1] = BranchRenderer.T; else branches[item.getLevel() - 1] = BranchRenderer.L;
        Grid grid = item.getParent();
        item = parent;
        parent = item.getParentItem();
        while (item.getLevel() > 0) {
            if (parent.indexOf(item) == parent.getItemCount() - 1) {
                if (parent.getParentItem() == null && !grid.getRootItem(grid.getRootItemCount() - 1).equals(parent)) branches[item.getLevel() - 1] = BranchRenderer.I; else branches[item.getLevel() - 1] = BranchRenderer.NONE;
            } else branches[item.getLevel() - 1] = BranchRenderer.I;
            item = parent;
            parent = item.getParentItem();
        }
        return branches;
    ...
    public Point computeSize(GC gc, int wHint, int hHint, Object value) {
        ...
        if (!isWordWrap()) {
            ...
            int plainTextWidth;
            if (wHint == SWT.DEFAULT) plainTextWidth = getBounds().width - x - rightMargin; else plainTextWidth = wHint - x - rightMargin;
