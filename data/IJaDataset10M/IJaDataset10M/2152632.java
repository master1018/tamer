...
public class BuildingPlacer {
    ...
    protected static final List<UnitType> EXTRA_SPACED = Arrays.asList(UnitType.TERRAN_COMMAND_CENTER, UnitType.TERRAN_FACTORY, UnitType.TERRAN_STARPORT, UnitType.TERRAN_SCIENCE_FACILITY);

    protected int buildDistance;

    protected final boolean[][] reserves = new boolean[Game.getInstance().mapWidth()][Game.getInstance().mapHeight()];
    ...
    public boolean canBuildHereWithSpace(TilePosition position, UnitType type) {
        ...
        int endx = Math.min(position.x() + width + buildDistance, Game.getInstance().mapWidth());
        int endy = Math.min(position.y() + height + buildDistance, Game.getInstance().mapHeight());
    ...
    public TilePosition getBuildLocationNear(TilePosition position, UnitType type) {
        ...
        while (length < Game.getInstance().mapWidth()) {
            if (x >= 0 && x < Game.getInstance().mapWidth() && y >= 0 && y < Game.getInstance().mapHeight() && canBuildHereWithSpace(new TilePosition(x, y), type)) {
