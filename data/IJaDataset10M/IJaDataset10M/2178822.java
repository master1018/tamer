...
class Mode {
   ...
   */
    static final String ANONYMOUS_MODE_NAME_PREFIX = "#anonymous#";

    /**
   * Inline anonymous modes counter.
   */
    static int anonymousModeCounter = 0;
   ...
   */
    private AttributeActionSet getAttributeActionsExplicit(String ns) {
        ...
        if (actions == null && baseMode != null) {
            ...
            if (actions != null) attributeMap.put(ns, actions);
        }
        return actions;
    }
   ...
   */
    int getAttributeProcessing() {
        if (attributeProcessing == -1) {
            if (baseMode != null) attributeProcessing = baseMode.getAttributeProcessing(); else attributeProcessing = ATTRIBUTE_PROCESSING_NONE;
            for (Enumeration en = nssAttributeMap.keys(); en.hasMoreElements() && attributeProcessing != ATTRIBUTE_PROCESSING_FULL; ) {
                NamespaceSpecification nss = (NamespaceSpecification) en.nextElement();
                AttributeActionSet actions = (AttributeActionSet) nssAttributeMap.get(nss);
                if (!actions.getAttach() || actions.getReject() || actions.getSchemas().length > 0) attributeProcessing = ((nss.ns.equals("") || nss.ns.equals(NamespaceSpecification.ANY_NAMESPACE)) ? ATTRIBUTE_PROCESSING_FULL : ATTRIBUTE_PROCESSING_QUALIFIED);
            }
        }
   ...
   */
    ...
    boolean isAnonymous() {
        return name.startsWith(ANONYMOUS_MODE_NAME_PREFIX);
    }
   ...
   */
    void noteUsed(Locator locator) {
        if (whereUsed == null && locator != null) whereUsed = new LocatorImpl(locator);
    }
   ...
   */
    void noteDefined(Locator locator) {
        defined = true;
        if (whereDefined == null && locator != null) whereDefined = new LocatorImpl(locator);
    }
   ...
   */
    boolean bindElement(String ns, String wildcard, ActionSet actions) {
        ...
        if (nssElementMap.get(nss) != null) return false;
   ...
   */
    boolean bindAttribute(String ns, String wildcard, AttributeActionSet actions) {
        NamespaceSpecification nss = new NamespaceSpecification(ns, wildcard);
        if (nssAttributeMap.get(nss) != null) return false;
