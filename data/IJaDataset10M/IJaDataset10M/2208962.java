...
public class BinomialRandomNDistribution extends Distribution {
	...
	*/
    public double getDensity(double x) {
        ...
        } else if (probability == 1) return distribution.getDensity(k); else {
            sum = 0;
            for (int i = 0; i < distribution.getDomain().getSize(); i++) {
                trials = distribution.getDomain().getValue(i);
                sum = sum + distribution.getDensity(trials) * Functions.comb(trials, k) * Math.pow(probability, k) * Math.pow(1 - probability, trials - k);
            }
            return sum;
        }
    }
	...
	*/
    public String toString() {
        return "Randomized binomail distribution [distribution for the number of trials = " + distribution + ", probability of success = " + probability + "]";
    }
}
