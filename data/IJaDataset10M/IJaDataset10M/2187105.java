...
public final class ArgumentParser implements Iterable<ArgumentParser.Option> {
    ...
    public final boolean addOption(final char shortname, final String longname, boolean takesArgument) {
        if (isClosed) throw new java.lang.IllegalStateException("ArgumentParser Closed");
        ...
        if (pos_short < this.optionsShort.size() && this.optionsShort.get(pos_short).shortname == shortname) return false;
        if (pos_long < this.optionsLong.size() && this.optionsLong.get(pos_long).longname.equals(longname)) return false;
    ...
    private final int binarySearch(final String value, final int low, final int high) {
        ...
        if (optionsLong.get(mid).longname.compareTo(value) > 0) return binarySearch(value, low, mid - 1);
        if (optionsLong.get(mid).longname.compareTo(value) < 0) return binarySearch(value, mid + 1, high); else return mid;
    }
    ...
    private final int binarySearch(final char value, final int low, final int high) {
        if (high < low) return low;
        final int mid = low + ((high - low) / 2);
        if (optionsShort.get(mid).shortname > value) return binarySearch(value, low, mid - 1);
        if (optionsShort.get(mid).shortname < value) return binarySearch(value, mid + 1, high); else return mid;
    }
    ...
    public final boolean removeOption(final char shortname) {
        ...
        if (pos_short >= this.optionsShort.size()) return false;
        Option o = this.optionsShort.get(pos_short);
        assert o != null;
        if (o.shortname != shortname) return false;
        final int pos_long = this.binarySearch(o.longname, 0, this.optionsLong.size() - 1);
        if (pos_long >= this.optionsLong.size()) return false;
        Option t1, t2;
    ...
    public final boolean removeOption(final String longname) {
        ...
        if (pos_long >= this.optionsShort.size()) return false;
        Option o = this.optionsLong.get(pos_long);
        assert o != null;
        if (!o.longname.equals(longname)) return false;
                ...
                if (size == 1) {
                    ...
                    if (size == 2) {
                        ...
                        for (; i < args.length; i++) {
                            str = args[i];
                            str = str.trim();
                            if (str.length() > 0) arguments.add(str);
                        }
                        break;
                    } else {
    ...
    private final Option getOptionByPrefix(String prefix) {
        ...
        if (o_center.longname.equals(prefix)) return o_center;
