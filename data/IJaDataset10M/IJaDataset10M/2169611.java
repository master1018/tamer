...
public class NumericalSurface {
    ...
    public void calculateSurface() {
        ...
        for (int i = 0; i < atoms.length; i++) {
            ...
            if (vdwr + solvent_radius > max_radius) max_radius = PeriodicTable.getVdwRadius(atoms[i].getSymbol()) + solvent_radius;
    ...
    public Point3d[] getAllSurfacePoints() {
        int npt = 0;
        for (int i = 0; i < this.surf_points.length; i++) npt += this.surf_points[i].size();
    ...
    private void translatePoints(int atmIdx, Point3d[][] points, int point_density, IAtom atom, Point3d cp) {
        ...
        double volume = vconst * (total_radius * total_radius) * dotp1 + (total_radius * total_radius * total_radius) * points.length;
    ...
    private Point3d[][] atomicSurfacePoints(NeighborList nbrlist, int currAtomIdx, IAtom atom, Tessellate tess) {
        ...
        for (int i = 0; i < tess_points.length; i++) {
            ...
            if (buried == false) {
                ...
                tmp[0] = new Point3d(total_radius * pt.x + atom.getPoint3d().x, total_radius * pt.y + atom.getPoint3d().y, total_radius * pt.z + atom.getPoint3d().z);
                tmp[1] = pt;
                points.add(tmp);
            }
        }
