    ...
    public ONDEXConcept createONDEXConcept() {
        if (!linearStructure.isEmpty()) {
            ...
            if (!structureType.isEmpty()) {
                c.createAttribute(structureTypeAttributeName, structureType, false);
            }
            if (!molecularMass.isEmpty() && !(molecularMass.equals("0"))) {
                c.createAttribute(molecularMassAttributeName, molecularMass, false);
            }
            if (!aglyconInfo.isEmpty()) {
                c.createAttribute(aglyconInfoAttributeName, aglyconInfo, false);
            }
            if (!molecularFormula.isEmpty()) {
                c.createAttribute(molecularFormulaAttributeName, molecularFormula, false);
            }
            if (!trivialName.isEmpty()) {
                c.createAttribute(trivialNameAttributeName, trivialName, false);
            }
            for (DBRef dbref : references) {
                if (!(dbref.dataSource.getId().equals("PMID"))) {
                    c.createConceptAccession(dbref.reference, dbref.dataSource, false);
                } else {
            ...
            throw new RuntimeException("Created without valid structure index tag.");
        }
    }
}
