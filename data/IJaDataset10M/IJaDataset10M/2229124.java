...
public class ByteFromIntArray {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < newArray.length; i++) System.out.print(" " + PadString.padHex(newArray[i], 2));
    }
    ...
    public byte getByte(int[] array, int location) {
        if ((location / 4) >= array.length) throw new ArrayIndexOutOfBoundsException("location = " + location + ", number of bytes = " + (array.length * 4));
        int theInt = location / 4;
        int theByte = location % 4;
        if (littleEndian) theByte = 3 - theByte;
        if (theByte == 0) return (byte) ((array[theInt] & 0x000000FF) >> 0); else if (theByte == 1) return (byte) ((array[theInt] & 0x0000FF00) >> 8); else if (theByte == 2) return (byte) ((array[theInt] & 0x00FF0000) >> 16); else if (theByte == 3) return (byte) ((array[theInt] & 0xFF000000) >> 24);
        return 0;
    }
    ...
    public int[] insertByte(int[] intBuffer, int location, byte b) {
        int theInt = location / 4;
        int theByte = location % 4;
        if (littleEndian == false) theByte = 3 - theByte;
        int replaceInt = intBuffer[theInt];
        int newByte = b << (8 * theByte);
        if (theByte == 0) replaceInt &= 0xFFFFFF00; else if (theByte == 1) replaceInt &= 0xFFFF00FF; else if (theByte == 2) replaceInt &= 0xFF00FFFF; else if (theByte == 3) replaceInt &= 0x00FFFFFF;
