...
public class YVariable implements Cloneable, YVerifiable, Comparable {
    ...
    protected String toXMLGuts() {
            ...
            if (null != _name) {
                ...
                if (_isUntyped) {
                    ...
                    if (null != _namespaceURI) {
                        xml.append("<namespace>" + _namespaceURI + "</namespace>");
                    }
                }
            }
        ...
        if (_initialValue != null) {
            xml.append("<initialValue>" + YTask.marshal(_initialValue) + "</initialValue>");
        }
        if (_defaultValue != null) {
            xml.append("<defaultValue>" + YTask.marshal(_defaultValue) + "</defaultValue>");
        }
        return xml.toString();
    }
    ...
    public List verify() {
        ...
        if (_initialValue != null && _initialValue.indexOf("<") != -1) {
            try {
                ...
                messages.add(new YVerificationMessage(this, "Problem with InitialValue [" + _initialValue + "] of " + this + " " + e.getMessage(), YVerificationMessage.ERROR_STATUS));
            }
        }
        XMLToolsForYAWL xty = new XMLToolsForYAWL();
        if (null != _name) {
            boolean isSchemForSchemType = xty.getSchema4SchemaNameSpace().equals(_namespaceURI);
            if (true == _isUntyped) {
                if (null != _dataTypeName) {
                }
            } else if (!xty.isValidType(_dataTypeName, isSchemForSchemType)) {
                messages.add(new YVerificationMessage(this, "The type library (Schema) in specification contains no " + "type definition with name [" + _dataTypeName + "].  " + "Therefore the decomposition " + _parentDecomposition + " cannot create this variable.", YVerificationMessage.ERROR_STATUS));
            }
        } else if (null != _elementName) {
            boolean schemaContainsElement = xty.getPrimarySchemaElementNames().contains(_elementName);
            if (!schemaContainsElement) {
                messages.add(new YVerificationMessage(this, "The type library (Schema) in specification contains no " + "element definition with name [" + _elementName + "].  " + "\n    Therefore the decomposition " + _parentDecomposition + " cannot create this variable.", YVerificationMessage.ERROR_STATUS));
            }
        } else if (null != _name) {
            if (null != _elementName) {
                messages.add(new YVerificationMessage(this, "name xor element name must be set, not both", YVerificationMessage.ERROR_STATUS));
            }
        } else {
            messages.add(new YVerificationMessage(this, "name or element name must be set", YVerificationMessage.ERROR_STATUS));
        }
        return messages;
    }
