...
public class SVNAnnotationGenerator implements ISVNFileRevisionHandler {
    ...
    public void reportAnnotations(ISVNAnnotateHandler handler, String inputEncoding) throws SVNException {
        ...
        if (myPreviousFile == null) {
            SVNErrorMessage err = SVNErrorMessage.create(SVNErrorCode.UNKNOWN, "ASSERTION FAILURE in SVNAnnotationGenerator.reportAnnotations(): myPreviousFile is null, " + "generator has to have been called at least once");
            SVNErrorManager.error(err, SVNLogType.DEFAULT);
        }
        int mergedCount = -1;
        if (myIncludeMergedRevisions) {
        ...
        try {
            stream = new SVNTranslatorInputStream(SVNFileUtil.openFileForReading(myPreviousFile), SVNProperty.EOL_LF_BYTES, true, null, false);
            ...
            try {
                ...
                for (int i = 0; i < blocksList.size(); i++) {
                    ...
                    if (block.getRightSize() > 0) {
                        insertBlameChunk(myCurrentRevision, myCurrentAuthor, myCurrentDate, myCurrentPath, block.getRightFrom(), block.getRightSize(), chain);
                    }
                }
            } finally {
                result.close();
    ...
    private void normalizeBlames(List chain, List mergedChain) throws SVNException {
        ...
        for (; i < chain.size() - 1 && k < mergedChain.size() - 1; i++, k++) {
            ...
            if (chunk.blockStart != mergedChunk.blockStart) {
                SVNErrorMessage err = SVNErrorMessage.create(SVNErrorCode.UNKNOWN, "ASSERTION FAILURE in SVNAnnotationGenerator.normalizeBlames():" + "current chunks should always start at the same offset");
                SVNErrorManager.error(err, SVNLogType.DEFAULT);
            }
    ...
    private QSequenceLineSimplifier createSimplifier() {
        if (mySimplifier == null) {
            QSequenceLineSimplifier first = myDiffOptions.isIgnoreEOLStyle() ? (QSequenceLineSimplifier) new QSequenceLineEOLUnifyingSimplifier() : (QSequenceLineSimplifier) new QSequenceLineDummySimplifier();
