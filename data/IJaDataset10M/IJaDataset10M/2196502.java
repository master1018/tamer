...
public class Conditional extends Sensor {
    ...
    public Conditional(Sensor condition, Sensor trueCase, Sensor falseCase) {
        if (condition.getType() != ActionParameterType.BOOLEAN) throw new RuntimeException("condition sensor must be a BOOLEAN.");
        if (trueCase.getType() != falseCase.getType()) throw new RuntimeException("caseTrue and caseFalse sensors must be sensors of the same type.");
    ...
    public Conditional(Conditional c) {
        this((Sensor) c._condition.clone(), (Sensor) c._trueCase.clone(), (Sensor) c._falseCase.clone());
    }
    ...
    public Object evaluate(int cycle, GameState gs, String player, Context parameters) {
        if ((Float) _condition.evaluate(cycle, gs, player, parameters) >= 1.0f) return _trueCase.evaluate(cycle, gs, player, parameters); else return _falseCase.evaluate(cycle, gs, player, parameters);
    }
