package org.es.uma.XMLEditor.jedit;
 ...
 */
public class TextAreaPainter extends JComponent implements TabExpander, DragGestureListener, DropTargetListener, DragSourceListener {

    /**
	 * 
	 */
    private static final long serialVersionUID = 4986086647816194076L;
    ...
    public String getToolTipText(MouseEvent evt) {
        if (highlights != null) return highlights.getToolTipText(evt); else return null;
    }
    ...
    public void dragGestureRecognized(DragGestureEvent e) {
        if ((!e.getTriggerEvent().isShiftDown()) && (!e.getTriggerEvent().isControlDown())) return;
        String texto = textArea.getSelectedText();
        if ((texto != null) && (texto.length() > 0)) {
            Transferable transferable = (Transferable) new StringSelection(texto);
            ...
            if (action == DnDConstants.ACTION_COPY) {
                ...
                transferable = (Transferable) new StringSelection("<spl:import XPath=\"a value\" Url=\"a value\"/>");
            }
    ...
    public void drop(DropTargetDropEvent e) {
        try {
            ...
            String texto = (String) tr.getTransferData(DataFlavor.stringFlavor);
            Point loc = e.getLocation();
            textArea.document.insertString(textArea.xyToOffset(loc.x, loc.y), texto, null);
        } catch (Exception ex) {
            e.rejectDrop();
        }
    ...
    public void dragOver(DropTargetDragEvent e) {
        ...
        int caretPosition = textArea.xyToOffset(p.x, p.y);
