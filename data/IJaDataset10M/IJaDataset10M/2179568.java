...
public class PatternValidator extends DtdContext implements Validator, ContentHandler, DTDHandler {
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
        if (collectingCharacters) flushCharacters();
        ...
        if (!setMemo(memo.startTagOpenDeriv(name))) {
            ...
            if (!next.isNotAllowed()) error("required_elements_missing"); else {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!setMemo(memo.startAttributeDeriv(attName))) error("impossible_attribute_ignored", attName); else if (!setMemo(memo.dataDeriv(atts.getValue(i), this))) {
        ...
        if (memo.getPattern().getContentType() == Pattern.DATA_CONTENT_TYPE) startCollectingCharacters();
    }
        ...
        if (collectingCharacters) {
            ...
            if (!setMemo(memo.dataDeriv(data, this))) {
                ...
                if (!memo.isNotAllowed()) {
                    if (!next.isNotAllowed() || fixAfter(memo).dataDeriv(data, this).isNotAllowed()) error("string_not_allowed");
                }
                memo = next;
            }
            ...
            PatternMemo next = memo.recoverAfter();
            if (!memo.isNotAllowed()) {
                if (!next.isNotAllowed() || fixAfter(memo).endTagDeriv().isNotAllowed()) error("unfinished_element");
            }
            memo = next;
        }
    }
    ...
    private void text() throws SAXException {
        if (!setMemo(memo.mixedTextDeriv())) error("text_not_allowed");
    }
    ...
    public void startDocument() throws SAXException {
        if (memo.isNotAllowed()) error("schema_allows_nothing");
    }
    ...
    private void error(String key) throws SAXException {
        if (hadError && memo.isNotAllowed()) return;
        hadError = true;
    ...
    private boolean setMemo(PatternMemo m) {
        if (m.isNotAllowed()) return false; else {
            memo = m;
            return true;
        }
    }
    ...
    private Pattern findElement(Name name) {
        if (recoverPatternTable == null) recoverPatternTable = new Hashtable();
    ...
    public String resolveNamespacePrefix(String prefix) {
        ...
        do {
            if (tem.prefix.equals(prefix)) return tem.namespaceURI;
            tem = tem.previous;
        } while (tem != null);
        return null;
    }
