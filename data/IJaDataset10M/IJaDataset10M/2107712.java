...
public class BreadthFirstTraverseAlgorithm<N, ET extends Edge<N>> {
    ...
    public void traverseFromNode(N gn) {
        ...
        if (gn == null) {
            throw new IllegalArgumentException("Node to traverse from cannot be null");
        }
    ...
    public void traverseFromEdge(ET he) {
        ...
        if (he == null) {
            throw new IllegalArgumentException("Edge to traverse from cannot be null");
        }
    ...
    private void conditionallyVisitEdgesOnNode(N thisNode) {
        for (ET nextEdge : graph.getAdjacentEdges(thisNode)) {
            if (!visitedEdges.contains(nextEdge) && canTraverseEdge(nextEdge, thisNode, DirectionalEdge.SOURCE)) {
    ...
    private void conditionallyVisitNodesOnEdge(ET thisEdge) {
        ...
        for (N node : graphNodes) {
            if (!visitedNodes.contains(node) && canTraverseEdge(thisEdge, node, DirectionalEdge.SINK)) {
