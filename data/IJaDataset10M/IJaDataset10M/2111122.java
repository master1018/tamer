...
public class WhiteboardSessionManager implements WhiteboardObjectListener {
    ...
    public void initWhiteboard(final Contact contact) {
        opSetWb = (OperationSetWhiteboarding) contact.getProtocolProvider().getOperationSet(OperationSetWhiteboarding.class);
        if (opSetWb == null) {
     ...
     */
    public WhiteboardObject sendWhiteboardObject(WhiteboardSession wbSession, WhiteboardShape ws) throws OperationFailedException {
    ...
    private WhiteboardFrame getWhiteboardFrame(Contact contact) {
        ...
        for (int i = 0; i < wbFrames.size(); i++) {
            whiteboardFrame = (WhiteboardFrame) wbFrames.get(i);
            if (whiteboardFrame.getContact() != null && whiteboardFrame.getContact().equals(contact)) return whiteboardFrame;
        }
        return null;
    }
    ...
    private class InvitationListener implements WhiteboardInvitationListener {
        ...
        public void invitationReceived(WhiteboardInvitationReceivedEvent evt) {
            ...
            InvitationReceivedDialog dialog = new InvitationReceivedDialog(WhiteboardSessionManager.this, whiteboardOpSet, evt.getInvitation());
            dialog.pack();
    ...
    public void acceptInvitation(WhiteboardInvitation invitation) {
        ...
        try {
            if (password == null) whiteboard.join(); else whiteboard.join(password);
        } catch (OperationFailedException e) {
            WhiteboardActivator.getUiService().getPopupDialog().showMessagePopupDialog(Resources.getString("failedToJoinWhiteboard", new String[] { whiteboard.getWhiteboardID() }), Resources.getString("error"), PopupDialog.ERROR_MESSAGE);
     ...
     */
    public void rejectInvitation(OperationSetWhiteboarding whiteboardOpSet, WhiteboardInvitation invitation, String reason) {
    ...
    private class PresenceListener implements WhiteboardSessionPresenceListener {
        ...
        public void whiteboardSessionPresenceChanged(WhiteboardSessionPresenceChangeEvent evt) {
            ...
            if (evt.getEventType().equals(WhiteboardSessionPresenceChangeEvent.LOCAL_USER_JOINED)) {
                ...
                WhiteboardActivator.getUiService().getPopupDialog().showMessagePopupDialog(Resources.getString("failedToJoinWhiteboard", new String[] { whiteboardSession.getWhiteboardID() }) + evt.getReason(), Resources.getString("error"), PopupDialog.ERROR_MESSAGE);
