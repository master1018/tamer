...
public class FragmentListBox extends JP2Box {
    ...
    public boolean readBox() throws IOException {
        if (!_module.isJP2HdrSeen()) {
            _repInfo.setMessage(new ErrorMessage(noJP2Hdr, _module.getFilePos()));
            return false;
        }
        initBytesRead();
        ...
        if (_boxHeader.getLength() != 0 && len != 14 * nFrags + 2) {
            _repInfo.setMessage(new ErrorMessage("Fragment Table has invalid length", _module.getFilePos()));
        ...
        for (int i = 0; i < nFrags; i++) {
            ...
            if (dataRef != 0) {
                _fragmentList = null;
                _repInfo.setMessage(new InfoMessage("Document references an external file", _module.getFilePos()));
