...
public final class GtpClient extends GtpClientBase {
    ...
    public GtpClient(String program, File workingDirectory, boolean log, IOCallback callback) throws GtpClient.ExecFailed {
        if (workingDirectory != null && !workingDirectory.isDirectory()) throw new ExecFailed(program, "Invalid working directory \"" + workingDirectory + "\"");
        m_log = log;
        m_callback = callback;
        m_wasKilled = false;
        ...
        m_program = program;
        if (StringUtil.isEmpty(program)) throw new ExecFailed(program, "Command for invoking Go program must be" + " not empty.");
    ...
    public String send(String command, long timeout, TimeoutCallback timeoutCallback) throws GtpError {
        ...
        m_response = "";
        ++m_commandNumber;
        if (m_autoNumber) command = Integer.toString(m_commandNumber) + " " + command;
        if (m_log) logOut(command);
        ...
        try {
            ...
            if (m_callback != null) m_callback.sentCommand(command);
    ...
    public void sendComment(String comment) {
        ...
        if (m_log) logOut(comment);
        if (m_callback != null) m_callback.sentCommand(comment);
    ...
    public void waitForExit(int timeout, TimeoutCallback timeoutCallback) {
        if (m_process == null) return;
        while (true) {
            if (ProcessUtil.waitForExit(m_process, timeout)) break;
        ...
        private void mainLoop() throws InterruptedException {
            while (true) {
                ...
                if (!isResponseStart(line)) {
                    if (!line.trim().equals("")) {
                        if (m_callback != null) m_callback.receivedInvalidResponse(line);
                        if (m_invalidResponseCallback != null) m_invalidResponseCallback.show(line);
                    }
    ...
    private synchronized void logIn(String msg) {
        if (m_logPrefix != null) System.err.print(m_logPrefix);
        System.err.print("<< ");
    ...
    private String readResponse(long timeout) throws GtpError {
        while (true) {
            ...
            if (m_callback != null) m_callback.receivedResponse(error, m_fullResponse);
            assert response.length() >= 3;
            int index = response.indexOf(' ');
            int length = response.length();
            if (index < 0) m_response = response.substring(1, length - 2); else m_response = response.substring(index + 1, length - 2);
            if (error) throw new GtpError(m_response);
            return m_response;
        }
    }
    ...
    private void throwProgramDied() throws GtpError {
        m_isProgramDead = true;
        String name = m_name;
        if (name == null) name = "The Go program";
        if (m_wasKilled) throw new GtpError(name + " terminated."); else throw new GtpError(name + " terminated unexpectedly.");
    }
