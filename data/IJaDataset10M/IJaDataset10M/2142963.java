...
public class CMLElementType implements CMLConstants {
    ...
    private void processContentTypes() {
        List<Node> complexTypes = CMLUtil.getQueryNodes(xsdElement, "./" + XSD_COMPLEX_TYPE, XPATH_XSD);
        ...
        List<Node> simpleTypes = CMLUtil.getQueryNodes(complexType, "./" + XSD_SIMPLE_CONTENT, XPATH_XSD);
    ...
    public void processAttributes(Map<String, CMLAttribute> attributeGroupMap) {
        ...
        if (simpleType != null) {
            List<Node> extensions = CMLUtil.getQueryNodes(simpleType, "./" + XSD_EXTENSION, XPATH_XSD);
        ...
        List<Node> attributes = CMLUtil.getQueryNodes(contentType, "./" + XSD_ATTRIBUTE, XPATH_XSD);
        if (attributes.size() != 0) {
            throw new RuntimeException("Use attributeGroups instead of attributes for " + name);
        }
        List<Node> attributeGroups = CMLUtil.getQueryNodes(contentType, "./" + XSD_ATTRIBUTE_GROUP, XPATH_XSD);
    ...
    public void processComplexContent(Map<String, CMLElementType> elementTypeMap) {
        if (simpleType == null) {
            List<Node> elements = CMLUtil.getQueryNodes(complexType, ".//" + XSD_ELEMENT + "[@ref]", XPATH_XSD);
            if (elements.size() == 0) {
            }
