...
public abstract class AbstractLexicalInvoke extends MultiReturnValueInvokeInstruction {
    ...
    protected AbstractLexicalInvoke(int results[], int exception, CallSiteReference site, Access[] lexicalReads, Access[] lexicalWrites) {
    ...
    public int getNumberOfUses() {
        if (lexicalReads == null) return getNumberOfParameters(); else return getNumberOfParameters() + lexicalReads.length;
    }
    ...
    public int getNumberOfDefs() {
        if (lexicalWrites == null) return super.getNumberOfDefs(); else return super.getNumberOfDefs() + lexicalWrites.length;
    }

    public int getDef(int j) {
        if (j < super.getNumberOfDefs()) return super.getDef(j); else return lexicalWrites[j - super.getNumberOfDefs()].valueNumber;
    }

    private Access[] addAccess(Access[] array, Access access) {
        if (array == null) return new Access[] { access }; else {
    ...
    public String toString(SymbolTable symbolTable) {
        ...
        if (lexicalReads != null) {
            ...
            for (int i = 0; i < lexicalReads.length; i++) {
                s.append(" ").append(lexicalReads[i].variableName).append(":").append(getValueString(symbolTable, lexicalReads[i].valueNumber));
            }
            s.append(")");
        }
        if (lexicalWrites != null) {
            ...
            for (int i = 0; i < lexicalWrites.length; i++) {
                s.append(" ").append(lexicalWrites[i].variableName).append(":").append(getValueString(symbolTable, lexicalWrites[i].valueNumber));
            }
            s.append(")");
        }
        return s.toString();
