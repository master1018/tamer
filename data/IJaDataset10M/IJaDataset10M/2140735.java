...
public class LinkToQuery implements ConditionToQuery {
    ...
    public HGQuery<?> getQuery(HyperGraph graph, HGQueryCondition c) {
        ...
        for (HGHandle t : lc.targets()) L.add(ToQueryMap.toQuery(graph, new IncidentCondition(t)));
        if (L.isEmpty()) return HGQuery.NOP; else if (L.size() == 1) return L.get(0); else {
            Iterator<HGQuery<?>> i = L.iterator();
            IntersectionQuery result = new IntersectionQuery(i.next(), i.next(), new ZigZagIntersectionResult());
            while (i.hasNext()) result = new IntersectionQuery(i.next(), result, new ZigZagIntersectionResult());
            return result;
        }
    }
}
