...
public class IntervalGFFPrinter {
        ...
        try {
            for (int i = 0; i < numFiles; i++) {
                ...
                if (makeSimpleGFF == false) {
                    ...
                    out.println("## Summary scores: intervals use median ratio of the best sub window when present or the user defined best window score, " + "best windows use a user defined score index, best sub window uses the median ratio, oligos use log2(aveT/ aveC), " + "binding peak use a trimmed mean of window ratios.\n\n");
                }
    ...
    public void gff(Interval i, int rank, String name, StringBuffer s) {
        ...
        if (sub != null) s.append(sub.getMedianRatio()); else s.append(i.getBestWindow().getScores()[scoreIndex]);
        s.append("\t");
        s.append(".");
        s.append("\t");
        s.append(".");
        ...
        if (makeSimpleGFF) s.append(nameRank); else {
            s.append("ID=");
            s.append(nameRank);
            s.append("; Name=");
            s.append(nameRank);
                ...
                try {
                    switch(test) {
                        case 'f':
                            ...
                            break;
                        case 'i':
                            scoreIndex = Integer.parseInt(args[i + 1]);
                            i++;
                            break;
                        case 'd':
                            printNonIntervalLines = false;
                            break;
                        case 's':
                            makeSimpleGFF = true;
                            break;
