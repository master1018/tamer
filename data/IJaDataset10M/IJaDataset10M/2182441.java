...
public class PrintAllMIDIEventsInFile {
        ...
        try {
            ...
            for (int trackIndex = 0; trackIndex < tracks.length; trackIndex++) {
                ...
                for (int eventIndex = 0; eventIndex < track.size(); eventIndex++) {
                    ...
                    if (message instanceof ShortMessage) System.out.println("TRACK:" + trackIndex + ", TICK:" + tick + ", " + getShortMessageString((ShortMessage) message)); else if (message instanceof MetaMessage) System.out.println("TRACK:" + trackIndex + ", TICK:" + tick + ", " + getMetaMessageString((MetaMessage) message)); else if (message instanceof SysexMessage) System.out.println("TRACK:" + trackIndex + ", TICK:" + tick + ", " + getSysexMessageString((SysexMessage) message));
                }
            }
            sequencer.close();
    ...
    private static String getMetaMessageString(MetaMessage metaMessage) {
        ...
        return "MetaMessage, " + "status: " + metaMessage.getStatus() + ", " + "type: " + metaMessage.getType() + ", " + "data: " + dataString + " (" + data + ")";
    }

    private static String getSysexMessageString(SysexMessage sysexMessage) {
        int status = sysexMessage.getStatus();
        String statusString = "UNKNOWN";
        if (status == SysexMessage.SPECIAL_SYSTEM_EXCLUSIVE) statusString = "SPECIAL_SYSTEM_EXCLUSIVE";
        if (status == SysexMessage.SYSTEM_EXCLUSIVE) statusString = "SYSTEM_EXCLUSIVE";
        byte[] data = sysexMessage.getData();
        String dataString = new String(data);
        return "SysexMessage, " + "status: " + statusString + " (" + status + ")" + "data: " + dataString + " (" + data + ")";
    }
