...
public class TestUploader {
    ...
    public TestUploader(AlternateLocationFactory alternateLocationFactory, NetworkManager networkManager) {
        ...
        this.networkManagerStub = networkManager instanceof NetworkManagerStub ? (NetworkManagerStub) networkManager : null;
    }
    ...
    public void start(String name, final int port, boolean tls) {
        ...
        reset();
        try {
            if (!tls) server = new ServerSocket(); else {
    ...
    public void setPartial(boolean part) {
        if (part) this.partial = 1; else this.partial = 0;
    }
    ...
    public void setFirewalled(boolean yes) {
        if (networkManagerStub != null) networkManagerStub.setAcceptedIncomingConnection(!yes);
        isFirewalled = yes;
    }
    ...
    private void handleRequest(Socket socket) throws IOException {
        ...
        if (rate > 0) throttle = new BandwidthThrottle(rate * 1024); else throttle = new BandwidthThrottle(Float.MAX_VALUE);
        start = 0;
        ...
        while (true) {
            ...
            if (line == null) throw new IOException("Unexpected close");
            if (line.equals("")) break;
            if (HTTPHeaderName.GNUTELLA_CONTENT_URN.matchesStartOfString(line)) {
                _sha1 = readContentUrn(line);
            }
            if (HTTPHeaderName.NALTS.matchesStartOfString(line) || HTTPHeaderName.BFALT_LOCATION.matchesStartOfString(line)) readAlternateLocations(line, false);
            if (HTTPHeaderName.ALT_LOCATION.matchesStartOfString(line) || HTTPHeaderName.FALT_LOCATION.matchesStartOfString(line)) readAlternateLocations(line, true);
            ...
            if (i == 0) {
                ...
                stop = Math.min(TestFile.length(), Math.max(1, p.b + highChunkOffset));
            }
            i = line.indexOf("GET");
            if (i == 0) {
        ...
        if (thexReq && sendThexTree && !queue) {
            if (_httpListener != null) _httpListener.thexRequestStarted();
            ...
            if (_httpListener != null) _httpListener.requestStarted(this);
            send(output, start, stop);
        }
        if (_httpListener != null) {
            if (thexReq) _httpListener.thexRequestHandled(); else _httpListener.requestHandled();
        }
    }

    private void sendThexTree(OutputStream out) throws IOException {
        HashTreeWriteHandlerFactory tigerWriteHandlerFactory = new TigerWriteHandlerFactoryImpl(new SimpleHashTreeNodeManager());
        HashTreeWriteHandler tigerWriteHandler = tigerWriteHandlerFactory.createTigerWriteHandler(TestFile.tree());
    ...
    private void send(OutputStream out, int start, int stop) throws IOException {
        ...
        if (minPollTime > 0) assertGreaterThan("queued downloader responded too quick by " + (minPollTime - t0) + " mS", minPollTime, t0);
        if (maxPollTime > 0) assertLessThan("queued downloader responded too late, by " + (t0 - maxPollTime) + " mS", maxPollTime, t0);
        String httpValue = respondWithHTTP11 ? "HTTP/1.1" : "HTTP/1.0";
        String str = httpValue + " " + (busy || queue || partial == 1 ? "503 Service Unavailable\r\n" : "200 OK \r\n");
        out.write(str.getBytes());
        ...
        if (queue) {
            ...
            str = "X-Queue: position=" + queuePos + ", pollMin=" + MIN_POLL / 1000 + ", pollMax=" + MAX_POLL / 1000 + "\r\n";
            out.write(str.getBytes());
            str = "\r\n";
            out.write(str.getBytes());
            out.flush();
        ...
        if (start != 0 || (stop - start != TestFile.length())) {
            str = "Content-range: bytes " + start + "-" + (stop - 1) + "/" + TestFile.length() + "\r\n";
            out.write(str.getBytes());
        }
        if (storedGoodLocs != null && storedGoodLocs.hasAlternateLocations()) {
            LOG.debug("Writing alternate location header:\n" + storedGoodLocs + "\n");
            TestUploader.writeHeader(HTTPHeaderName.ALT_LOCATION, storedGoodLocs, out);
        } else {
            LOG.debug("Did not write alt locs:\n" + storedGoodLocs + "\n");
        }
        if (storedBadLocs != null && storedBadLocs.hasAlternateLocations()) {
            LOG.debug("Writing alternate location header:\n" + storedBadLocs + "\n");
            TestUploader.writeHeader(HTTPHeaderName.NALTS, storedBadLocs, out);
        } else {
            LOG.debug("Did not write alt locs:\n" + storedBadLocs + "\n");
        }
        if (creationTime != null) {
            LOG.debug("Writing out Creation Time.");
            TestUploader.writeHeader(HTTPHeaderName.CREATION_TIME, "" + creationTime, out);
        }
        if (sendThexTreeHeader) {
            TestUploader.writeHeader(HTTPHeaderName.THEX_URI, TestFile.tree(), out);
        }
        if (interestedInFalts) {
            ...
            if (features.size() > 0) {
                TestUploader.writeHeader(HTTPHeaderName.FEATURES, new HTTPHeaderValueCollection(features), out);
            }
        }
        ...
        for (int i = start; i < stop; ) {
            if (stopAfter > -1) {
                assertLessThanOrEquals("total " + fullRequestsUploaded + " this " + amountThisRequest + " stop " + stopAfter, stopAfter, fullRequestsUploaded + amountThisRequest);
        ...
        if (sentCorrupt) LOG.debug("corrupt data sent");
        out.flush();
     ...
     */
    private IntPair parseRange(String str) throws IndexOutOfBoundsException, NumberFormatException, IOException {
        int start = 0;
        ...
        if (c == '-') {
            ...
            second = second.trim();
            if (!second.equals("")) try {
                stop = java.lang.Integer.parseInt(second) + 1;
            } catch (NumberFormatException e) {
                throw new IOException();
            }
        }
        if (partial == 3) {
            assertEquals("downloader picked incorrect start range in iteration", 150010, start);
            assertEquals("downloader pick incorrect stop range in iteration", 250020, stop);
        }
        if (partial == 2) {
    ...
    private void readAlternateLocations(String altHeader, final boolean good) {
        ...
        AltLocUtils.parseAlternateLocations(_sha1, alternateLocations, true, alternateLocationFactory, new Function<AlternateLocation, Void>() {
            ...
            public Void apply(AlternateLocation location) {
                if (location instanceof PushAltLoc) ((PushAltLoc) location).updateProxies(good);
                if (good) incomingGoodAltLocs.add(location); else incomingBadAltLocs.add(location);
                return null;
            }
        }, true);
    }
    ...
    private class SocketHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (fullRequestsUploaded < totalAmountToUpload) killedByDownloader = true;
