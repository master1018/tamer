...
public class NetvibesUtil {
    ...
    public static final int NUMBER_OF_WIDGETS_PER_PAGE = 20;

    public static int getNumberOfWidgets() {
        return _numberOfWidgets;
    }

    public static List<NetvibesCategory> getNetvibesCategories() throws Exception {
        ...
        for (int i = 0; i < jsonCategories.length(); i++) {
            ...
            categories.add(new NetvibesCategory(category.getInt("id"), category.getString("label")));
        }
        return categories;
    }
    ...
    public static List<NetvibesWidget> getNetvibesWidgets(String query, String sort, int category, String region, int page) throws Exception {
        List<NetvibesWidget> widgets = new ArrayList<NetvibesWidget>();
        JSONObject received = new JSONObject(getSearchResult(query, sort, category, region, page));
        ...
        for (int i = 0; i < jsonWidgets.length(); i++) {
            ...
            widgets.add(new NetvibesWidget(widget.getString("title"), widget.getString("summary"), widget.getString("link"), widget.getString("thumbnail"), widget.getString("author")));
        }
        return widgets;
    }
    ...
    protected static String getJSONNetvibesCategories() {
        ...
        String key = NetvibesUtil.class.getName() + StringPool.PERIOD + "category";
        try {
    ...
    protected static String getSearchResult(String query, String sort, int category, String region, int page) {
        WebCacheItem wci = new NetvibesSearchResultWebCacheItem(query, sort, category, region, page);
        String key = NetvibesUtil.class.getName() + StringPool.PERIOD + query + StringPool.PERIOD + sort + StringPool.PERIOD + category + StringPool.PERIOD + region + StringPool.PERIOD + page;
        try {
            return (String) WebCachePoolUtil.get(key, wci);
        } catch (ClassCastException cce) {
            WebCachePoolUtil.remove(key);
