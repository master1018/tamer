...
public class ProvaAttachImpl extends ProvaBuiltinImpl {
    ...
    public boolean process(ProvaReagent prova, ProvaDerivationNode node, ProvaGoal goal, List<ProvaLiteral> newLiterals, ProvaRule query) {
        ...
        if (!(lt instanceof ProvaList)) return false;
        ...
        if (!(a2 instanceof ProvaVariable)) return false;
        ...
        } else prefixFixed = prefix.getFixed();
        ...
        if (a1 instanceof ProvaList && !isMonad) {
            ...
            if (tail instanceof ProvaList) len += ((ProvaList) tail).getFixed().length;
            ...
            if (tail instanceof ProvaList) System.arraycopy(((ProvaList) tail).getFixed(), 0, newFixed, prefixFixed.length + suffixFixed.length, ((ProvaList) tail).getFixed().length);
