...
public class RequestUtils {
    ...
    private static transient Log log = LogFactory.getLog(RequestUtils.class);

    /**
	 * 复制request参数值到指定对象 属性.
	 * @param request
	 *            HttpServletRequest
	 * @param bean
	 *            Object
	 * @return Object
	 */
    public static Object getObjectFromRequest(HttpServletRequest request, Object bean) {
        @SuppressWarnings("rawtypes") Enumeration paras = request.getParameterNames();
        while (paras.hasMoreElements()) {
            String name = (String) paras.nextElement();
            try {
                BeanUtils.setProperty(bean, name, request.getParameter(name).trim());
            } catch (Exception ex) {
    ...
    public static Cookie getCookie(HttpServletRequest request, String name) {
        ...
        for (Cookie ck : cookies) {
            if (StringUtils.equalsIgnoreCase(name, ck.getName())) return ck;
        }
        return null;
    }
    ...
    public static String getCookieValue(HttpServletRequest request, String name) {
        ...
        if (cookies == null) return null;
        for (Cookie ck : cookies) {
            if (StringUtils.equalsIgnoreCase(name, ck.getName())) return ck.getValue();
        }
        return null;
    }
    ...
    public static String getDomainOfServerName(String host) {
        ...
        if (len > 3) {
            ...
            if (dp.equalsIgnoreCase("com") || dp.equalsIgnoreCase("gov") || dp.equalsIgnoreCase("net") || dp.equalsIgnoreCase("edu") || dp.equalsIgnoreCase("org")) return makeup(names[len - 3], names[len - 2], names[len - 1]); else return makeup(names[len - 2], names[len - 1]);
        }
        return host;
    }
    ...
    public static boolean isIPAddr(String addr) {
        if (StringUtils.isEmpty(addr)) return false;
