 ...
 */
public class OperationSetMultiUserChatYahooImpl implements OperationSetMultiUserChat, SubscriptionListener {

    private static final Logger logger = Logger.getLogger(OperationSetMultiUserChatYahooImpl.class);
    ...
    */
    private Vector<ChatRoomInvitationRejectionListener> invitationRejectionListeners = new Vector<ChatRoomInvitationRejectionListener>();
    ...
    */
    private Vector<LocalUserChatRoomPresenceListener> presenceListeners = new Vector<LocalUserChatRoomPresenceListener>();

    /**
    * A list of the rooms that are currently open by this account.
    */
    private Hashtable<String, ChatRoomYahooImpl> chatRoomCache = new Hashtable<String, ChatRoomYahooImpl>();
    ...
    */
    OperationSetMultiUserChatYahooImpl(ProtocolProviderServiceYahooImpl yahooProvider) {
        this.yahooProvider = yahooProvider;
        yahooProvider.addRegistrationStateChangeListener(new RegistrationStateListener());
        opSetBasic = (OperationSetBasicInstantMessagingYahooImpl) yahooProvider.getOperationSet(OperationSetBasicInstantMessaging.class);
        OperationSetPersistentPresence presenceOpSet = (OperationSetPersistentPresence) yahooProvider.getOperationSet(OperationSetPersistentPresence.class);
    ...
    */
    public void addInvitationListener(ChatRoomInvitationListener listener) {
        synchronized (invitationListeners) {
            if (!invitationListeners.contains(listener)) invitationListeners.add(listener);
        }
    }
    ...
    */
    public void addInvitationRejectionListener(ChatRoomInvitationRejectionListener listener) {
        synchronized (invitationRejectionListeners) {
            if (!invitationRejectionListeners.contains(listener)) invitationRejectionListeners.add(listener);
        }
    }
    ...
    */
    public void addPresenceListener(LocalUserChatRoomPresenceListener listener) {
        synchronized (presenceListeners) {
            if (!presenceListeners.contains(listener)) presenceListeners.add(listener);
        }
    }
    ...
    */
    public ChatRoom createChatRoom(String roomName, Map<String, Object> roomProperties) throws OperationFailedException, OperationNotSupportedException {
        ...
        try {
            YahooConference conference = yahooProvider.getYahooSession().createConference(new String[] {}, "", yahooProvider.getYahooSession().getLoginIdentity());
    ...
    *             if the server does not support multi user chat
    */
    public ChatRoom findRoom(YahooConference yahooConference) throws OperationFailedException, OperationNotSupportedException {
    ...
    * @return ChatRoom the chat room that we've just created.
    */
    private ChatRoomYahooImpl createLocalChatRoomInstance(YahooConference yahooConference) {
        synchronized (chatRoomCache) {
            ChatRoomYahooImpl newChatRoom = new ChatRoomYahooImpl(yahooConference, yahooProvider);
            return newChatRoom;
        }
    }
    ...
    */
    public boolean isMultiChatSupportedByContact(Contact contact) {
        if (contact.getProtocolProvider().getOperationSet(OperationSetMultiUserChat.class) != null) return true;
        return false;
    }
    ...
    */
    public void rejectInvitation(ChatRoomInvitation invitation, String rejectReason) {
        ChatRoomYahooImpl chatRoom = (ChatRoomYahooImpl) invitation.getTargetChatRoom();
        try {
    ...
    */
    public void fireInvitationEvent(ChatRoom targetChatRoom, String inviter, String reason, byte[] password) {
        ChatRoomInvitationYahooImpl invitation = new ChatRoomInvitationYahooImpl(targetChatRoom, inviter, reason, password);
        ...
        synchronized (invitationListeners) {
            listeners = new ArrayList<ChatRoomInvitationListener>(invitationListeners).iterator();
        }
        while (listeners.hasNext()) {
            ChatRoomInvitationListener listener = (ChatRoomInvitationListener) listeners.next();
    ...
    */
    public void fireInvitationRejectedEvent(ChatRoom sourceChatRoom, String invitee, String reason) {
        ...
        synchronized (invitationRejectionListeners) {
            listeners = new ArrayList<ChatRoomInvitationRejectionListener>(invitationRejectionListeners).iterator();
        }
        while (listeners.hasNext()) {
            ChatRoomInvitationRejectionListener listener = (ChatRoomInvitationRejectionListener) listeners.next();
    ...
    */
    public void fireLocalUserPresenceEvent(ChatRoom chatRoom, String eventType, String reason) {
        ...
        synchronized (presenceListeners) {
            listeners = new ArrayList<LocalUserChatRoomPresenceListener>(presenceListeners).iterator();
        }
        while (listeners.hasNext()) {
    ...
    */
    public Message createMessage(byte[] content, String contentType, String contentEncoding, String subject) {
        return new MessageYahooImpl(new String(content), contentType, contentEncoding, subject);
    }
    ...
    */
    public Message createMessage(String messageText) {
        return new MessageYahooImpl(messageText, OperationSetBasicInstantMessaging.DEFAULT_MIME_TYPE, OperationSetBasicInstantMessaging.DEFAULT_MIME_ENCODING, null);
    }
    ...
    */
    private class YahooMessageListener extends SessionAdapter {
        ...
        public void conferenceInviteDeclinedReceived(SessionConferenceEvent ev) {
            logger.debug("Group Chat invite declined received. " + ev.toString());
            try {
        ...
        public void conferenceInviteReceived(SessionConferenceEvent ev) {
            ...
            try {
                ...
                if (chatRoom != null) {
                    ...
                    fireInvitationEvent(chatRoom, ev.getFrom(), ev.getMessage(), null);
                }
            } catch (Exception e) {
                logger.debug("Error: " + e);
            }
        ...
        public void conferenceLogoffReceived(SessionConferenceEvent ev) {
            ...
            try {
                ChatRoomYahooImpl chatRoom = (ChatRoomYahooImpl) findRoom(ev.getRoom().getName());
                if (chatRoom != null) {
                    ChatRoomMemberYahooImpl member = chatRoom.getChatRoomMember(ev.getFrom());
        ...
        public void conferenceLogonReceived(SessionConferenceEvent ev) {
            ...
            try {
                ChatRoomYahooImpl chatRoom = (ChatRoomYahooImpl) findRoom(ev.getRoom().getName());
                if (chatRoom != null) {
                    ChatRoomMemberYahooImpl newMember = new ChatRoomMemberYahooImpl(chatRoom, ev.getFrom(), ev.getFrom(), ChatRoomMemberRole.MEMBER);
    ...
    */
    ...
    private void updateChatRoomMembers(Contact contact) {
        ...
        while (chatRooms.hasMoreElements()) {
            ...
            ChatRoomMemberYahooImpl member = chatRoom.findMemberForNickName(contact.getAddress());
            if (member != null) {
