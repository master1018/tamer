...
class RegionDemarkator {
    ...
    void start(int offset) {
        if (offset == 0) {
            if (wasZeroOffset) throw new IllegalStateException("zero offset");
            wasZeroOffset = true;
        }
        offset += initialOffset;
        if (startedOffset >= 0) throw new IllegalStateException("allready started");
    ...
    RegionHandle end(int end) {
        ...
        if (startedOffset < 0 || end < startedOffset) throw new IllegalStateException("no start or illegal position");
    ...
    private void addIfNotObsolete(List<IRegion> result, RegionHandle region) {
        if (region != null) if (region.correct()) result.add(region.toRegion());
    }
    ...
    class RegionHandle implements Comparable<RegionHandle> {
        ...
        private boolean correct() {
            ...
            if (finalOffset > initialOffset) {
                if (offset >= finalOffset) return false;
        ...
        private boolean mergeIfPossibleTo(RegionHandle r) {
            ...
            if (offset >= r.offset && offset <= endOfR) {
                ...
                if (requiredLength > endOfR) r.length = requiredLength - r.offset;
                return true;
            }
            return false;
        }
