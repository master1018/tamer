...
public class EditSystem extends JDialog implements AstrogationChangeListener {
    ...
    protected void uwp_actionPerformed(JComboTable t) {
        if (system != null && t.getItemCount() > 0) {
            ...
            if (t.getName().matches("starport")) {
                system.setPort(update);
            } else if (t.getName().matches("worldSize")) {
                system.setSize(update);
            } else if (t.getName().matches("atmosphere")) {
                system.setAtmosphere(update);
            } else if (t.getName().matches("hydrographic")) {
                system.setHydrograph(update);
            } else if (t.getName().matches("population")) {
                system.setPopulation(update);
            } else if (t.getName().matches("government")) {
                system.setGovernment(update);
            } else if (t.getName().matches("lawLevel")) {
                system.setLawlevel(update);
            } else if (t.getName().matches("techLevel")) {
                system.setTechnology(update);
            }
