...
public class TypeChecker extends Visitor implements Opcode, TokenId {
    ...
    protected static StringBuffer typeToString(StringBuffer sbuf, int type, int dim, String cname) {
        String s;
        if (type == CLASS) s = MemberResolver.jvmToJavaName(cname); else if (type == NULL) s = "Object"; else try {
    ...
    public void atNewExpr(NewExpr expr) throws CompileError {
        if (expr.isArray()) atNewArrayExpr(expr); else {
    ...
    public void atNewArrayExpr(NewExpr expr) throws CompileError {
        ...
        if (size.length() > 1) atMultiNewArray(type, classname, size); else {
            ASTree sizeExpr = size.head();
            if (sizeExpr != null) sizeExpr.accept(this);
            exprType = type;
            arrayDim = 1;
            if (type == CLASS) className = resolveClassName(classname); else className = null;
        }
    }
    ...
    public void atAssignExpr(AssignExpr expr) throws CompileError {
        ...
        if (left instanceof Variable) atVariableAssign(expr, op, (Variable) left, ((Variable) left).getDeclarator(), right); else {
    ...
    public void atCondExpr(CondExpr expr) throws CompileError {
        ...
        if (dim1 == 0 && dim1 == arrayDim) if (CodeGen.rightIsStrong(type1, exprType)) expr.setThen(new CastExpr(exprType, 0, expr.thenExpr())); else if (CodeGen.rightIsStrong(exprType, type1)) {
    ...
    public void atBinExpr(BinExpr expr) throws CompileError {
        ...
        if (k >= 0) {
            if (token == '+') {
                ...
                int type1 = exprType;
                right.accept(this);
                if (!isConstant(expr, token, left, right)) computeBinExprType(expr, token, type1);
            }
        } else {
            booleanExpr(expr);
        }
    ...
    private Expr atPlusExpr(BinExpr expr) throws CompileError {
        ...
        String cname = className;
        right.accept(this);
        if (isConstant(expr, '+', left, right)) return null;
    ...
    private boolean isConstant(BinExpr expr, int op, ASTree left, ASTree right) throws CompileError {
        ...
        ASTree newExpr = null;
        if (left instanceof StringL && right instanceof StringL && op == '+') newExpr = new StringL(((StringL) left).get() + ((StringL) right).get()); else if (left instanceof IntConst) newExpr = ((IntConst) left).compute(op, right); else if (left instanceof DoubleConst) newExpr = ((DoubleConst) left).compute(op, right);
        if (newExpr == null) return false; else {
            expr.setOperator('+');
    ...
    static ASTree stripPlusExpr(ASTree expr) {
        if (expr instanceof BinExpr) {
            ...
            if (e.getOperator() == '+' && e.oprand2() == null) return e.getLeft();
            ...
            } else if (op == '+' && e.getRight() == null) return e.getLeft();
    ...
    public static ASTree getConstantFieldValue(CtField f) {
        ...
        if (value instanceof String) return new StringL((String) value); else if (value instanceof Double || value instanceof Float) {
        ...
        } else if (value instanceof Boolean) return new Keyword(((Boolean) value).booleanValue() ? TokenId.TRUE : TokenId.FALSE); else return null;
    }
    ...
    private void computeBinExprType(BinExpr expr, int token, int type1) throws CompileError {
        int type2 = exprType;
        if (token == LSHIFT || token == RSHIFT || token == ARSHIFT) exprType = type1; else insertCast(expr, type1, type2);
        if (CodeGen.isP_INT(exprType)) exprType = INT;
    }
    ...
    private void booleanExpr(ASTree expr) throws CompileError {
        ...
        if (op == EQ) {
            ...
            if (dim1 == 0 && arrayDim == 0) insertCast(bexpr, type1, exprType);
        } else if (op == '!') ((Expr) expr).oprand1().accept(this); else if (op == ANDAND || op == OROR) {
            BinExpr bexpr = (BinExpr) expr;
            bexpr.oprand1().accept(this);
            bexpr.oprand2().accept(this);
    ...
    private void insertCast(BinExpr expr, int type1, int type2) throws CompileError {
        if (CodeGen.rightIsStrong(type1, type2)) expr.setLeft(new CastExpr(type2, 0, expr.oprand1())); else exprType = type1;
    }
    ...
    public void atExpr(Expr expr) throws CompileError {
        ...
        if (token == '.') {
            ...
            if (member.equals("length")) atArrayLength(expr); else if (member.equals("class")) atClassObject(expr); else atFieldRead(expr);
        } else if (token == MEMBER) {
            String member = ((Symbol) expr.oprand2()).get();
            if (member.equals("class")) atClassObject(expr); else atFieldRead(expr);
        } else if (token == ARRAY) atArrayRead(oprand, expr.oprand2()); else if (token == PLUSPLUS || token == MINUSMINUS) atPlusPlus(token, oprand, expr); else if (token == '!') booleanExpr(expr); else if (token == CALL) fatal(); else {
            oprand.accept(this);
            if (!isConstant(expr, token, oprand)) if (token == '-' || token == '~') if (CodeGen.isP_INT(exprType)) exprType = INT;
        }
    }
    ...
    private boolean isConstant(Expr expr, int op, ASTree oprand) {
        ...
        if (oprand instanceof IntConst) {
            ...
            long v = c.get();
            if (op == '-') v = -v; else if (op == '~') v = ~v; else return false;
            c.set(v);
        } else if (oprand instanceof DoubleConst) {
            DoubleConst c = (DoubleConst) oprand;
            if (op == '-') c.set(-c.get()); else return false;
        } else return false;
        expr.setOperator('+');
        return true;
    }
    ...
    public void atCallExpr(CallExpr expr) throws CompileError {
        ...
        if (method instanceof Member) {
            ...
            if (((Keyword) method).get() == SUPER) targetClass = MemberResolver.getSuperclass(thisClass); else targetClass = thisClass;
        } else if (method instanceof Expr) {
            Expr e = (Expr) method;
            mname = ((Symbol) e.oprand2()).get();
            int op = e.getOperator();
            if (op == MEMBER) targetClass = resolver.lookupClass(((Symbol) e.oprand1()).get(), false); else if (op == '.') {
                ASTree target = e.oprand1();
                try {
                    target.accept(this);
                } catch (NoFieldException nfe) {
                    if (nfe.getExpr() != target) throw nfe;
                    exprType = CLASS;
                    arrayDim = 0;
                ...
                if (arrayDim > 0) targetClass = resolver.lookupClass(javaLangObject, true); else if (exprType == CLASS) targetClass = resolver.lookupClassByJvmName(className); else badMethod();
            } else badMethod();
        } else fatal();
    ...
    public MemberResolver.Method atMethodCallCore(CtClass targetClass, String mname, ASTList args) throws CompileError {
        ...
        if (found == null) {
            ...
            String msg;
            if (mname.equals(MethodInfo.nameInit)) msg = "cannot find constructor " + clazz + signature; else msg = mname + signature + " not found in " + clazz;
    ...
    private void atFieldRead(CtField f) throws CompileError {
        ...
        arrayDim = dim;
        exprType = MemberResolver.descToType(c);
        if (c == 'L') className = type.substring(i + 1, type.indexOf(';', i + 1)); else className = null;
    }
    ...
    protected CtField fieldAccess(ASTree expr) throws CompileError {
        if (expr instanceof Member) {
            ...
            try {
                ...
                if (Modifier.isStatic(f.getModifiers())) mem.setField(f);
                return f;
            ...
            if (op == MEMBER) {
                ...
                if (exprType == CLASS && arrayDim == 0) return resolver.lookupFieldByJvmName(className, (Symbol) e.oprand2());
            } catch (NoFieldException nfe) {
                if (nfe.getExpr() != e.oprand1()) throw nfe;
                Member fname = (Member) e.oprand2();
                String jvmClassName = nfe.getField();
                CtField f = resolver.lookupFieldByJvmName2(jvmClassName, fname, expr);
                e.setOperator(MEMBER);
        ...
        if (oprand instanceof Variable) {
            ...
            if (oprand instanceof Expr) {
                ...
                if (e.getOperator() == ARRAY) {
                    ...
                    int t = exprType;
                    if (t == INT || t == BYTE || t == CHAR || t == SHORT) exprType = INT;
                    return;
                }
            }
    ...
    public void atKeyword(Keyword k) throws CompileError {
        ...
        switch(token) {
            ...
            case SUPER:
                exprType = CLASS;
                if (token == THIS) className = getThisName(); else className = getSuperName();
                break;
            default:
                fatal();
        }
    ...
    public void atIntConst(IntConst i) throws CompileError {
        arrayDim = 0;
        int type = i.getType();
        if (type == IntConstant || type == CharConstant) exprType = (type == IntConstant ? INT : CHAR); else exprType = LONG;
    }

    public void atDoubleConst(DoubleConst d) throws CompileError {
        arrayDim = 0;
        if (d.getType() == DoubleConstant) exprType = DOUBLE; else exprType = FLOAT;
    }
}
