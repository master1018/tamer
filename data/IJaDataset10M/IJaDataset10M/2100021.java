...
public class Taxidermy extends CraftingSkill {
        ...
        if (canBeUninvoked()) {
            if ((affected != null) && (affected instanceof MOB)) {
                ...
                if ((building != null) && (!aborted)) {
                    if (messedUp) commonTell(mob, "You've messed up stuffing " + foundShortName + "!"); else dropAWinner(mob, building);
                }
            }
        }
        super.unInvoke();
    ...
    protected Vector loadRecipes() {
        ...
        if (V == null) {
            ...
            for (int v = 0; v < strV.size(); v++) {
                ...
                if (header) {
                    if ((V2 != null) && (V2.size() > 0)) V.addElement(V2);
                    V2 = new Vector();
                }
                if (s.length() == 0) header = true; else if (V2 != null) {
                    V2.addElement(s);
                    header = false;
                }
            }
            if ((V2 != null) && (V2.size() > 0)) V.addElement(V2);
            if (V.size() == 0) Log.errOut("Taxidermy", "Poses not found!");
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        int[][] data = fetchFoundResourceData(mob, woodRequired, "cloth stuffing", pm, 0, null, null, false, 0, null);
        ...
        if ((pose == null) || (C == null)) building.setDisplayText("the stuffed body of " + name + " stands here"); else {
