...
/**
 *  The <code>ISFlowLayout</code> class is a layout manager that
 ...
 */
public class ISRelativeLayout implements LayoutManager2, java.io.Serializable {
	 ...
	 */
    public ISRelativeLayout(int axis) {
        this(axis, 0);
    }
	 ...
	 * <p>
	 * All <code>ISFlowLayout</code> constructors defer to this one.
	 * @param	 axis  X-AXIS or Y_AXIS
	 * @param	 gap   the gap
	 */
    public ISRelativeLayout(int axis, int gap) {
        setAxis(axis);
        setGap(gap);
        setBorderGap(gap);
    }
    ...
    public void setAxis(int axis) {
        if (axis != X_AXIS && axis != Y_AXIS) throw new IllegalArgumentException("invalid axis specified");
        this.axis = axis;
    }
    ...
    public void addLayoutComponent(Component component, Object constraint) {
        if (constraint != null) if (constraint instanceof Float) {
	 ...
	 * @see	   java.awt.ISRelativeLayout#minimumLayoutSize
	 ...
	 *			 subcomponents of the specified container
	 * @see		 java.awt.ISRelativeLayout#preferredLayoutSize
    ...
    public void layoutContainer(Container parent) {
        synchronized (parent.getTreeLock()) {
            if (axis == X_AXIS) layoutContainerHorizontally(parent); else layoutContainerVertically(parent);
        }
    }
    ...
    private void layoutContainerHorizontally(Container parent) {
        ...
        int spaceAvailable = parent.getSize().width - insets.left - insets.right - ((components - 1) * gap) - (2 * borderGap);
        ...
        for (int i = 0; i < components; i++) {
            ...
            Component component = parent.getComponent(i);
            Dimension d = component.getPreferredSize();
            if (fill) d.height = parentHeight - fillGap;
            Float constraint = constraints.get(component);
            if (constraint == null) {
                component.setSize(d);
    ...
    private int getLocationY(Component component, int height) {
        ...
        if (alignmentY == COMPONENT) alignmentY = component.getAlignmentY();
    ...
    private void layoutContainerVertically(Container parent) {
        ...
        float relativeTotal = 0.0f;
        Insets insets = parent.getInsets();
        int spaceAvailable = parent.getSize().height - insets.top - insets.bottom - ((components - 1) * gap) - (2 * borderGap);
        for (int i = 0; i < components; i++) {
            Component component = parent.getComponent(i);
            Float constraint = constraints.get(component);
            if (constraint == null) {
        ...
        for (int i = 0; i < components; i++) {
            ...
            Component component = parent.getComponent(i);
            Dimension d = component.getPreferredSize();
            if (fill) d.width = parentWidth - fillGap;
            Float constraint = constraints.get(component);
            if (constraint == null) {
                component.setSize(d);
    ...
    private int getLocationX(Component component, int width) {
        ...
        if (alignmentX == COMPONENT) alignmentX = component.getAlignmentX();
    ...
    private Dimension getLayoutSize(Container parent, int type) {
        ...
        Insets insets = parent.getInsets();
        int totalGap = ((components - 1) * gap) + (2 * borderGap);
        if (axis == X_AXIS) {
