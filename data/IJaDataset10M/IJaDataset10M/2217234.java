...
public class CacheMap implements Map, Cache, java.io.Serializable, Cloneable {
    ...
    protected static final int EXPUNGE_YES = 0x1;
	 ...
	 */
    protected static final int EXPUNGE_STOP = 0x2;
    ...
    protected int canExpunge(Value v) {
        return _map.size() > getMaxSize() || (System.currentTimeMillis() - v.access) > getLifetime() ? (EXPUNGE_YES | EXPUNGE_CONTINUE) : (EXPUNGE_NO | EXPUNGE_STOP);
    }

    /** Expunges if {@link #shallExpunge} is true. */
    private void tryExpunge() {
        if (shallExpunge()) {
            if (_inExpunge) throw new IllegalStateException("expung in expung?");
            try {
                expunge();
            } finally {
                newRef();
	 ...
	 */
    public int expunge() {
        if (_inExpunge || _map.isEmpty()) return _map.size();
        _inExpunge = true;
        try {
            for (final Iterator it = _map.values().iterator(); it.hasNext(); ) {
                ...
                if ((result & EXPUNGE_STOP) != 0) break;
            }
            return _map.size();
        } finally {
            _inExpunge = false;
	 ...
	 */
    private void newRef() {
        if (_que != null) _ref = new WeakReference(new X(), _que);
    }
    ...
    public boolean containsValue(Object value) {
        ...
        for (final Iterator it = _map.values().iterator(); it.hasNext(); ) {
            final Value v = (Value) it.next();
            if (Objects.equals(v.value, value)) return true;
        }
        return false;
    }
    ...
    public boolean equals(Object o) {
        tryExpunge();
        return o == this || ((o instanceof CacheMap) && _map.equals(((CacheMap) o)._map)) || ((o instanceof Map) && _map.equals((Map) o));
    }
    ...
    public String toString() {
        ...
        if (!_map.isEmpty()) {
            for (final Iterator it = _map.entrySet().iterator(); ; ) {
                ...
                if (it.hasNext()) sb.append(", "); else break;
            }
        }
