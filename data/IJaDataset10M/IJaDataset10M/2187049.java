...
/**
 *<p>
 * This is basically the multiple decision maker equivalent to {@link OrderedAssignmentsOld}.
 * </p>
 * <p>
...
public class GroupOrderedAssignments {
    ...
    public GroupOrderedAssignments(Map<DecisionMaker, Assignments> allAssignments, Categories categories) {
        ...
        m_allAssignments.putAll(allAssignments);
     ...
     */
    public OrderedAssignmentsOld getOrderedAssignments(DecisionMaker dm) {
        if (dm == null) {
        ...
        return new OrderedAssignmentsOld(assignments, m_categories);
    }
     ...
     * @return not <code>null</code>.
     */
    public Map<DecisionMaker, OrderedAssignmentsOld> getAllOrderedAssignments() {
        final HashMap<DecisionMaker, OrderedAssignmentsOld> allOrderedAssignments = new HashMap<DecisionMaker, OrderedAssignmentsOld>();
        for (DecisionMaker dm : getDms()) {
            allOrderedAssignments.put(dm, new OrderedAssignmentsOld(m_allAssignments.get(dm), m_categories));
        }
     ...
     *            not <code>null</code>.
     */
    public void setAllOrderedAssignments(Map<DecisionMaker, OrderedAssignmentsOld> allAssignments) {
        if (allAssignments == null) {
            throw new NullPointerException();
        }
        m_allAssignments = new HashMap<DecisionMaker, Assignments>();
        for (DecisionMaker dm : allAssignments.keySet()) {
            final OrderedAssignmentsOld orderedAssignments = allAssignments.get(dm);
