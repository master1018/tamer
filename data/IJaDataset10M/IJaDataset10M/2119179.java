...
public class DataFormatParser {
    ...
    private Object parseElaboration(int size, ElaborationType elabType) throws IOException {
        ...
        if (reader.read() == '{') {
            ...
            switch(elabType) {
                ...
                case DATE_FORMAT:
                    return parseDateFormat(es);
                case COLOR_FORMAT:
                    return new ColorFormat(size, es);
                case FP_FORMAT:
                    return parseFPFormat(es);
                case VALUE:
                    return parseInt(es);
                case KV_SIGNED:
                    return parseIntStringMap(-1, es);
                case KV_UNSIGNED:
                    return parseIntStringMap(size, es);
                case STRUCT:
            ...
            switch(elabType) {
                ...
                case KV_SIGNED:
                    return new HashMap<BigInteger, String>();
                case KV_UNSIGNED:
                    return new HashMap<BigInteger, String>();
                case STRUCT:
    ...
    private static String elaborationToString(ElaborationType elabType, Object elab) {
        ...
        switch(elabType) {
            case TEXT_ENCODING:
                ...
                break;
            case DATE_FORMAT:
                s.append(dateFormatToString((DateFormat) elab));
                break;
            case COLOR_FORMAT:
                s.append(((ColorFormat) elab).toString());
                break;
            case FP_FORMAT:
                s.append(fpFormatToString((int[]) elab));
                break;
            case VALUE:
            ...
            case KV_UNSIGNED:
                ...
                break;
            case STRUCT:
                @SuppressWarnings("unchecked") List<DataField> format = (List<DataField>) elab;
                s.append("\n\t");
    ...
    private DFExpression parseItemCount() throws IOException {
        ...
        if (reader.read() == '[') {
            ...
            if (cs.length() == 0) return null; else return new DFExpressionParser(new DFExpressionLexer(new StringReader(cs))).parse();
        } else {
            reader.reset();
            return null;
        }
    ...
    private String parseFieldName() throws IOException {
        ...
        if (firstChar == '`') {
            ...
            while (true) {
                ...
                if (nextChar < 0 || nextChar == '`') break; else s.append(Character.toChars(nextChar));
            }
            return s.toString();
            ...
            while (true) {
                int nextChar = reader.read();
                if (nextChar < 0 || !(Character.isDigit(nextChar) || Character.isLetter(nextChar) || (nextChar == '_'))) break;
                numChars++;
            }
            reader.reset();
            StringBuffer s = new StringBuffer();
    ...
    private static String fieldNameToString(String s) {
        if (s.matches("^[A-Za-z_][A-Za-z0-9_]*$")) return s; else return "`" + s.replace("`", "") + "`";
    }
    ...
    private String parseFieldDescription() throws IOException {
        ...
        if (firstChar == '"' || firstChar == '\'') {
            ...
            while (true) {
                int nextChar = reader.read();
                if (nextChar < 0 || nextChar == firstChar) break; else s.append(Character.toChars(nextChar));
            }
            return s.toString();
            ...
            if (ds.length() == 0) return null; else return ds;
        }
    }
    ...
    private DateFormat parseDateFormat(String s) {
        ...
        switch(ss.length) {
            case 7:
                return new DateFormat(Integer.parseInt(ss[0]), Calendar.JANUARY + Integer.parseInt(ss[1]) - 1, Integer.parseInt(ss[2]), Integer.parseInt(ss[3]), Integer.parseInt(ss[4]), Integer.parseInt(ss[5]), Integer.parseInt(ss[6]));
            case 6:
                return new DateFormat(Integer.parseInt(ss[0]), Calendar.JANUARY + Integer.parseInt(ss[1]) - 1, Integer.parseInt(ss[2]), Integer.parseInt(ss[3]), Integer.parseInt(ss[4]), Integer.parseInt(ss[5]));
            case 4:
                return new DateFormat(Integer.parseInt(ss[0]), Calendar.JANUARY + Integer.parseInt(ss[1]) - 1, Integer.parseInt(ss[2]), Integer.parseInt(ss[3]));
            case 3:
                return new DateFormat(Integer.parseInt(ss[0]), Calendar.JANUARY + Integer.parseInt(ss[1]) - 1, Integer.parseInt(ss[2]));
            case 2:
