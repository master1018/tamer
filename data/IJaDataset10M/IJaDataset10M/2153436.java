...
public class SecretKeyFactory_ImplTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        mProv = (new SpiEngUtils()).new MyProvider("MySKFProvider", "Testing provider", srvSecretKeyFactory.concat(".").concat(defaultAlg), SecretKeyFactoryProviderClass);
    ...
    private void checkResult(SecretKeyFactory skf) throws InvalidKeyException, InvalidKeySpecException {
        SecretKey sk;
        KeySpec keySpec;
     ...
     */
    public void testGetInstance01() throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException {
        try {
        ...
        for (int i = 0; i < validValues.length; i++) {
            ...
            assertEquals("Incorrect algorithm", skf.getAlgorithm(), validValues[i]);
     ...
     * returns SecretKeyFactory object
     */
    public void testGetInstance02() throws NoSuchAlgorithmException, NoSuchProviderException, IllegalArgumentException, InvalidKeySpecException, InvalidKeyException {
        try {
        ...
        for (int i = 0; i < invalidValues.length; i++) {
            try {
                SecretKeyFactory.getInstance(invalidValues[i], mProv.getName());
                fail("NoSuchAlgorithmException must be thrown (algorithm: ".concat(invalidValues[i]).concat(")"));
            } catch (NoSuchAlgorithmException e) {
            }
        }
        ...
        SecretKeyFactory skf;
        for (int i = 0; i < validValues.length; i++) {
            skf = SecretKeyFactory.getInstance(validValues[i], mProv.getName());
            assertEquals("Incorrect algorithm", skf.getAlgorithm(), validValues[i]);
            assertEquals("Incorrect provider", skf.getProvider().getName(), mProv.getName());
            checkResult(skf);
        }
    }
     ...
     * returns SecretKeyFactory object
     */
    public void testGetInstance03() throws NoSuchAlgorithmException, IllegalArgumentException, InvalidKeySpecException, InvalidKeyException {
        try {
