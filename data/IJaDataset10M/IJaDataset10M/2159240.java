...
public class AdminEditNpc implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (!Config.ALT_PRIVILEGES_ADMIN) if (!((checkLevel(activeChar.getAccessLevel()) || checkLevel2(activeChar.getAccessLevel())) && activeChar.isGM())) return false;
        if (command.startsWith("admin_showShop ")) {
            String[] args = command.split(" ");
            if (args.length > 1) showShop(activeChar, Integer.parseInt(command.split(" ")[1]));
        } else if (command.startsWith("admin_showShopList ")) {
            String[] args = command.split(" ");
            if (args.length > 2) showShopList(activeChar, Integer.parseInt(command.split(" ")[1]), Integer.parseInt(command.split(" ")[2]));
        } else if (command.startsWith("admin_edit_npc ")) try {
            ...
            if (npcId > 0) showNpcDropList(activeChar, npcId); else activeChar.sendMessage("Usage: //show_droplist <npc_id>");
        } else if (!Config.ALT_PRIVILEGES_ADMIN && !(checkLevel(activeChar.getAccessLevel()) && activeChar.isGM())) return false; else if (command.startsWith("admin_addShopItem ")) {
            String[] args = command.split(" ");
            if (args.length > 1) addShopItem(activeChar, args);
        } else if (command.startsWith("admin_delShopItem ")) {
            String[] args = command.split(" ");
            if (args.length > 2) delShopItem(activeChar, args);
        } else if (command.startsWith("admin_editShopItem ")) {
            String[] args = command.split(" ");
            if (args.length > 2) editShopItem(activeChar, args);
        } else if (command.startsWith("admin_save_npc ")) try {
            ...
            try {
                ...
                } else activeChar.sendMessage("Usage: //edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
            } catch (StringIndexOutOfBoundsException e) {
            ...
            try {
                ...
                } else activeChar.sendMessage("Usage: //add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
            } catch (StringIndexOutOfBoundsException e) {
            ...
            if (npcId > 0) deleteDropData(activeChar, npcId, itemId, category); else activeChar.sendMessage("Usage: //del_drop <npc_id> <item_id> <category>");
            ...
            try {
                ...
                int page = 0;
                if (st.countTokens() <= 2) {
                    if (st.hasMoreTokens()) npcId = Integer.parseInt(st.nextToken());
                    if (st.hasMoreTokens()) page = Integer.parseInt(st.nextToken());
                }
                if (npcId > 0) showNpcSkillList(activeChar, npcId, page); else activeChar.sendMessage("Usage: //show_skilllist_npc <npc_id> <page>");
            } catch (Exception e) {
            ...
            try {
                ...
                if (st.countTokens() == 2) try {
                    npcId = Integer.parseInt(st.nextToken());
                    skillId = Integer.parseInt(st.nextToken());
                    showNpcSkillEdit(activeChar, npcId, skillId);
                } catch (Exception e) {
                } else if (st.countTokens() == 3) try {
                    npcId = Integer.parseInt(st.nextToken());
                    skillId = Integer.parseInt(st.nextToken());
                ...
                } else activeChar.sendMessage("Usage: //edit_skill_npc <npc_id> <item_id> [<level>]");
            } catch (StringIndexOutOfBoundsException e) {
            ...
            try {
                ...
                } else activeChar.sendMessage("Usage: //add_skill_npc <npc_id> [<level>]");
            } catch (StringIndexOutOfBoundsException e) {
            ...
            if (npcId > 0) deleteNpcSkillData(activeChar, npcId, skillId); else activeChar.sendMessage("Usage: //del_skill_npc <npc_id> <skill_id>");
            ...
            if (target instanceof L2BoxInstance) {
                ...
                if (players.length > 1) {
                    ...
                    String msg = "Access:";
                    for (Object p : box.getAccess()) msg += " " + (String) p;
    ...
    private void editShopItem(L2PcInstance activeChar, String[] args) {
        ...
        if (tradeList.getPriceForItemId(itemID) < 0) return;
        if (args.length > 3) {
    ...
    private void showShopList(L2PcInstance activeChar, int tradeListID, int page) {
        L2TradeList tradeList = TradeController.getInstance().getBuyList(tradeListID);
        if (page > tradeList.getItems().size() / PAGE_LIMIT + 1 || page < 1) return;
        NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
    ...
    private TextBuilder itemListHtml(L2TradeList tradeList, int page) {
        ...
        if (page > 1) replyMSG.append("<td><button value=\"Page" + (page - 1) + "\" action=\"bypass -h admin_showShopList " + tradeList.getListId() + " " + (page - 1) + "\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
        if (page < max) {
            if (page <= min) replyMSG.append("<td></td>");
    ...
    private void showShop(L2PcInstance activeChar, int merchantID) {
        ...
        for (L2TradeList tradeList : tradeLists) if (tradeList != null) replyMSG.append("<tr><td><a action=\"bypass -h admin_showShopList " + tradeList.getListId() + " 1\">Trade List " + tradeList.getListId() + "</a></td></tr>");
        replyMSG.append("</table>");
        replyMSG.append("<center>");
        replyMSG.append("<button value=\"Close\" action=\"bypass -h admin_close_window\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        replyMSG.append("</center></body></html>");
    ...
    private void Show_Npc_Property(L2PcInstance activeChar, L2NpcTemplate npc) {
        ...
        } else adminReply.setHtml("<html><head><body>File not found: data/html/admin/editnpc.htm</body></html>");
        activeChar.sendPacket(adminReply);
    }

    private void save_npc_property(L2PcInstance activeChar, String command) {
        String[] commandSplit = command.split(" ");
        if (commandSplit.length < 4) return;
    ...
    private void updateDropData(L2PcInstance activeChar, int npcId, int itemId, int min, int max, int category, int chance) {
        ...
        try {
            ...
            if (npcId > 0) {
                ...
                adminReply.setHtml(replyMSG.toString());
                activeChar.sendPacket(adminReply);
            } else activeChar.sendMessage("unknown error!");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
    ...
    private void showNpcSkillList(L2PcInstance activeChar, int npcId, int page) {
        ...
        if (_skillsize > MaxSkillsPerPage * MaxPages) MaxPages++;
        if (page > MaxPages) page = MaxPages;
        int SkillsStart = MaxSkillsPerPage * page;
        int SkillsEnd = _skillsize;
        if (SkillsEnd - SkillsStart > MaxSkillsPerPage) SkillsEnd = SkillsStart + MaxSkillsPerPage;
        NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
        ...
        for (int x = 0; x < MaxPages; x++) {
            int pagenr = x + 1;
            if (page == x) pages += "<td>Page " + pagenr + "</td>"; else pages += "<td><a action=\"bypass -h admin_show_skilllist_npc " + npcData.getNpcId() + " " + x + "\">Page " + pagenr + "</a></td>";
        }
        ...
        for (int i = 0; i < SkillsStart; i++) if (skillite.hasNext()) skillobj = skillite.next();
        int cnt = SkillsStart;
        ...
        replyMSG.append("<center>");
        replyMSG.append("<button value=\"Add Skill\" action=\"bypass -h admin_add_skill_npc " + npcId + "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        replyMSG.append("<button value=\"Droplist\" action=\"bypass -h admin_show_droplist " + npcId + "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
        replyMSG.append("</center></body></html>");
        adminReply.setHtml(replyMSG.toString());
        activeChar.sendPacket(adminReply);
    }
