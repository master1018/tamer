 ...
 *
 */
public class CovalentRadiusTest extends CDKTavernaTestCase {
    ...
    private String descriptorName = "CovalentRadius";
    ...
    public void test_LocalWorkerCovalentRadius() throws CDKException, Exception {
        CovalentRadius test = new CovalentRadius();
        ...
        if (resultListCalculatedStructures.size() != 0) {
            for (Iterator iter = resultListCalculatedStructures.iterator(); iter.hasNext(); ) {
                ...
                if (!((String) fileName.get(fileName.size() - 1)).equalsIgnoreCase("CovalentRadius_Descriptor")) {
                    hasFileNameAnError = true;
                }
            }
        }
