...
/**
 * Performs tree layout using a radial layout, laying out subsequent
 * depth levels of a tree on circles of progressively increasing
 * radius.
 * 
 * @version 1.0
...
public class RadialTreeLayout extends TreeLayout implements FocusListener {
    ...
    public void run(ItemRegistry registry, double frac) {
        if (m_registry != registry) {
            if (m_registry != null) m_registry.getDefaultFocusSet().removeFocusListener(this);
            m_registry = registry;
    ...
    protected void setScale(Rectangle2D bounds) {
        ...
        if (m_maxDepth > 0) m_radiusInc = (r - 40) / m_maxDepth;
    }
    ...
    private double calcStartingTheta(NodeItem n, NodeItem p) {
        ...
        int cD = 0;
        for (int i = 0; i < pidx; i++) cD += getParams((NodeItem) n.getChild(i)).numDescendants;
        double f = (cD + ((double) pD) / 2.0) / ((double) nD);
        return ptheta - f * TWO_PI;
    }

    /**
     * Computes the number of visible descendant leaf nodes for each
     * visible node.
     */
    ...
    /**
     * Set the position of the given node, given in polar
     * co-ordinates.
     * 
     * @param n
     * @param r
    ...
    public void focusChanged(FocusEvent e) {
        if (e.getEventType() != FocusEvent.FOCUS_SET) return;
