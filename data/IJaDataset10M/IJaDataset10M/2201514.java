...
public class BatchInit {
    ...
    public void run(CommandLine line, String[] args) throws Exception {
        BatchRunner runner;
        if (line.hasOption("interactive")) runner = new BatchRunner(true); else runner = new BatchRunner(false);
    ...
    private SweeperProducer runOpt(CommandLine line) throws IOException, ClassNotFoundException, IllegalAccessException, InstantiationException, MalformedURLException {
        ...
        if (optParser.getParametersFile() != null) {
            ...
            if (chooser == null) producer = new OptimizedXMLSweeperProducer(rrProducer, new File(paramsFile).toURI().toURL()); else producer = new OptimizedXMLSweeperProducer(rrProducer, chooser, new File(paramsFile).toURI().toURL());
        } else {
