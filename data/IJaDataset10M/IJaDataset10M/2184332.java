...
public class ParserTest extends TestCase {
    ...
    public void testGetItem() {
        {
            try {
                StreamingXmlParser parser = new StreamingXmlParser("<doc abc='def' hello=\"world\"><x></x>asdf<z></z></doc>");
                Item item;
                item = parser.getItem();
    ...
    public void xmlPullParserInterfaceTestHelper(boolean useBufferedParser) {
        {
            try {
                XmlPullParser parser = getParser("", useBufferedParser);
                assertFalse(parser.hasNext());
            } catch (XmlParseException e) {
                fail(e.getMessage());
            }
            try {
                XmlPullParser parser = getParser("<doc/>", useBufferedParser);
            ...
            try {
                XmlPullParser parser = getParser("<doc abc='def' hello=\"world\"><x></x>asdf<z></z></doc>", useBufferedParser);
                assertTrue(parser.hasNext());
        ...
        {
            try {
                XmlPullParser parser = getParser("some text &gt; &amp; &lt;", useBufferedParser);
                assertTrue(parser.hasNext());
                assertEquals(ItemType.TEXT, parser.next());
        ...
        {
            try {
                XmlPullParser parser = getParser("<body><?a 'conv/title'='' 'lang'='unknown'?><line/>spelly test<?a 'lang'?></body>", useBufferedParser);
                assertTrue(parser.hasNext());
                assertEquals(ItemType.START_ELEMENT, parser.next());
        ...
        {
            try {
                XmlPullParser parser = getParser("<body></body>hello<a/>", useBufferedParser);
                assertTrue(parser.hasNext());
                assertEquals(ItemType.START_ELEMENT, parser.next());
                assertEquals("body", parser.getTagName());
                assertEquals(ItemType.END_ELEMENT, parser.next());
    ...
    public void testDocProviderParse() {
        String testStr = "<body><?a \"conv/title\"=\"\" \"lang\"=\"unknown\"?><line/>spelly test<?a \"conv/title\" \"lang\"?></body>";
