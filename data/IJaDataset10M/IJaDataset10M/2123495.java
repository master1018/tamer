...
public class JpcapSender extends JpcapInstance {
    ...
    private JpcapSender() throws java.io.IOException {
        if (reserveID() < 0) throw new java.io.IOException("Unable to open a device: " + MAX_NUMBER_OF_INSTANCE + " devices are already opened.");
    }
    ...
    public static JpcapSender openDevice(NetworkInterface device) throws java.io.IOException {
        ...
        if (ret == null) return sender; else throw new java.io.IOException(ret);
    }
    ...
    public void close() {
        if (ID == RAW_SOCKET_ID) nativeCloseRawSocket(); else nativeCloseDevice();
        unreserveID();
    }
    ...
    public void sendPacket(Packet packet) {
        if (ID == RAW_SOCKET_ID) nativeSendPacketViaRawSocket(packet); else nativeSendPacket(packet);
    }
}
