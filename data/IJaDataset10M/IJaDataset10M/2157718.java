...
public class Procedure extends EntryList {
    ...
    void addDefaultParticipants(ProcedureRunner runner) throws Exception {
        ...
        while (it.hasNext()) {
            ...
            Object p = null;
            if (oc_manager != null) p = oc_manager.getObjectCreator().createInstance(cls); else p = cls.newInstance();
    ...
    void clearFields(CompositeMap map) {
        ...
        while (it.hasNext()) {
            ...
            if (!f.isInputField()) map.putObject(f.getPath(), null);
        }
    }

    public void run(ProcedureRunner runner) throws Exception {
        populateContextFields(runner);
        if (exception_handles != null) runner.addExceptionHandles(exception_handles);
    ...
    public void addChild(CompositeMap child) {
        ...
        if (o != null) if (o instanceof IEntry) {
            addEntry((IEntry) o);
        } else {
    ...
    public void addField(Field f) {
        ...
        field_list.add(f);
        if (f.isReturnField()) return_field = f; else if (f.isInputField()) input_fields.add(f);
    }
    ...
    public Field getField(String name) {
        Field f = null;
        if (field_map != null) f = (Field) field_map.get(name);
        if (f != null) return f; else {
            IEntry root = getRootOwner();
            if (root != null && root instanceof Procedure) f = ((Procedure) root).getField(name);
            if (f == null) {
                Procedure parent_proc = getParentProcedure();
                if (parent_proc != null) f = parent_proc.getField(name);
            }
            return f;
        }
    }
    ...
    public Procedure getParentProcedure() {
        for (IEntry p = getOwner(); p != null; p = p.getOwner()) {
            if (p instanceof Procedure) return (Procedure) p;
        }
        return null;
    }
    ...
    public void setExceptionHandles(Collection handles) {
        if (exception_handles == null) exception_handles = handles; else exception_handles.addAll(handles);
    ...
    public void setParticipants(String participants) throws ClassNotFoundException {
        ...
        if (participant_class_list != null) participant_class_list.clear(); else participant_class_list = new LinkedList();
    ...
    public void clear() {
        super.clear();
        if (field_map != null) field_map.clear();
        if (field_list != null) field_list.clear();
        if (input_fields != null) input_fields.clear();
        if (exception_handles != null) exception_handles.clear();
