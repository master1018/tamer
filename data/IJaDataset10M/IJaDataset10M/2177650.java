...
public class ChartDocument implements IAdaptable {
    ...
    public ChartDocument(IChartTemplate template, IHistory history) {
        ...
        for (IChartSection templateSection : template.getSections()) {
            ...
            if (sections.size() == 0) row.addIndicator(null, null, null, new SecurityElement(root));
            sections.add(row);
        }
        if (sections.size() == 0) {
    ...
    public Object getAdapter(Class adapter) {
        if (adapter.isAssignableFrom(IDataSeries.class)) return root;
    ...
    public ChartTemplate getTemplate() {
        ...
        for (int i = 0; i < chartSections.length; i++) chartSections[i] = sections.get(i).getTemplate();
