...
public class CrawlSettingsSAXHandler extends DefaultHandler implements ValueErrorHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        ...
        if (handler != null) {
            ...
            if (((Boolean) skip.peek()).booleanValue()) {
                ...
                String moduleName = attributes.getValue(XMLSettingsHandler.XML_ATTRIBUTE_NAME);
            ...
            String tmp = "Unknown element '" + qName + "' in '" + locator.getSystemId() + "', line: " + locator.getLineNumber() + ", column: " + locator.getColumnNumber();
    ...
    public void illegalElementError(String name) throws SAXParseException {
        throw new SAXParseException("Element '" + name + "' not allowed here", locator);
    }
    ...
    private class RootHandler extends ElementHandler {
        ...
        public void startElement(String name, Attributes atts) throws SAXException {
            if ((name.equals(XMLSettingsHandler.XML_ROOT_ORDER) && settings.getScope() != null) || (name.equals(XMLSettingsHandler.XML_ROOT_HOST_SETTINGS) && settings.getScope() == null) || (name.equals(XMLSettingsHandler.XML_ROOT_REFINEMENT) && !settings.isRefinement())) {
                throw new SAXParseException("Wrong document type '" + name + "'", locator);
            }
        }
    }
    ...
    private class RefinementHandler extends ElementHandler {
        ...
        public void startElement(String name, Attributes atts) throws SAXException {
            stack.push(new Refinement(settings, atts.getValue(XMLSettingsHandler.XML_ELEMENT_REFERENCE)));
        }
    }
    ...
    private class TimespanHandler extends ElementHandler {
        ...
        public void startElement(String name, Attributes atts) throws SAXException {
            if (stack.peek() instanceof Refinement) {
                String from = atts.getValue(XMLSettingsHandler.XML_ATTRIBUTE_FROM);
    ...
    private class ModuleHandler extends ElementHandler {
        ...
        public void startElement(String name, Attributes atts) throws SAXException {
            ...
            if (name.equals(XMLSettingsHandler.XML_ELEMENT_CONTROLLER)) {
                ...
                module = settingsHandler.getSettingsObject(null).getModule(atts.getValue(XMLSettingsHandler.XML_ATTRIBUTE_NAME));
            }
            stack.push(module);
        }
    ...
    private class NewModuleHandler extends ElementHandler {
        ...
        public void startElement(String name, Attributes atts) throws SAXException {
            ...
            try {
                ModuleType module = SettingsHandler.instantiateModuleTypeFromClassName(moduleName, moduleClass);
                try {
                    ...
                    try {
                        ...
                        logger.log(Level.WARNING, "Module '" + moduleName + "' in '" + locator.getSystemId() + "', line: " + locator.getLineNumber() + ", column: " + locator.getColumnNumber() + " is not defined in '" + parentModule.getName() + "'.", ise);
                        throw new SAXException(new AttributeNotFoundException(ise.getMessage()));
                    }
                }
                stack.push(module);
            } catch (InvocationTargetException e) {
                logger.log(Level.WARNING, "Couldn't instantiate " + moduleName + ", from class: " + moduleClass + "' in '" + locator.getSystemId() + "', line: " + locator.getLineNumber() + ", column: " + locator.getColumnNumber(), e);
                throw new SAXException(e);
            ...
            if (container instanceof ComplexType) {
                try {
                    try {
                        ...
                        try {
                            ((ComplexType) container).addElement(settings, new SimpleType(elementName, "", value));
                        } catch (IllegalStateException ise) {
                            logger.warning("Unknown attribute '" + elementName + "' in '" + locator.getSystemId() + "', line: " + locator.getLineNumber() + ", column: " + locator.getColumnNumber());
                        }
                    }
                } catch (InvalidAttributeValueException e) {
                    try {
                        logger.warning("Illegal value '" + value + "' for attribute '" + elementName + "' in '" + locator.getSystemId() + "', line: " + locator.getLineNumber() + ", column: " + locator.getColumnNumber() + ", Value reset to default value: " + ((ComplexType) container).getAttribute(settings, elementName));
                ...
                if (container == null) {
                    logger.severe("Empty container (Was a referenced parent" + " filter removed?).  Element details: elementName " + elementName + ", name " + name);
                } else {
