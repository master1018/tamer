...
public class DriverEventWriter implements DriverEventHandler {
    ...
    private void writeFirstLine(BufferedWriter writer) {
        try {
            writer.write("driver" + semicolon() + "vehicle" + semicolon() + "carrier" + semicolon() + "distance[m]" + semicolon() + "time[sec]" + semicolon() + "capacityUsage" + semicolon() + "performance[unit*m]" + semicolon() + "volumes[units]" + newLine());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
