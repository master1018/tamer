...
final class Concede extends KDialog {
    ...
    private static final Logger LOGGER = Logger.getLogger(Concede.class.getName());
    ...
    private Concede(ClientGUI clientGui, JFrame parentFrame, Legion ally, Legion enemy, boolean flee) {
        super(parentFrame, (flee ? "Flee" : "Concede") + " with Legion " + LegionServerSide.getLongMarkerName(ally.getMarkerId()) + " in " + ally.getCurrentHex().getDescription() + "?", false);
        ...
        try {
            ...
            LOGGER.log(Level.WARNING, "Exception during infoTextCreation: ", e);
        }
        ...
        infoText2.append("This will give player " + enemy.getPlayer().getName() + "  " + resultingPoints + " points, " + "i.e. increase the score from " + score + " to " + newScore + ".");
        ...
        showMapButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                GUIMasterHex hex = gui.getBoard().getGUIHexByMasterHex(attacker.getCurrentHex());
    ...
    private Box showLegion(Legion legion, boolean dead) {
        ...
        Marker marker = new Marker(legion, scale, legion.getLongMarkerId(), gui.getClient(), true);
        pane.add(marker);
        ...
        pane.add(Box.createRigidArea(new Dimension(scale / 4, 0)));
        List<String> imageNames = gui.getGameClientSide().getLegionImageNames(legion);
