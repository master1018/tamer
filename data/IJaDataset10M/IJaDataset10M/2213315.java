...
public class matPanel extends CPanel {
    ...
    private void jbInit() throws Exception {
        ...
        this.add(mat_codiE, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 0), 0, 0));
        this.add(mat_nombE, new GridBagConstraints(2, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 2, 0, 0), 0, 0));
        this.add(Bcons, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.VERTICAL, new Insets(0, 2, 0, 0), 0, 0));
    }
    ...
    public boolean isNull() {
        if (mat_codiE.getValorDec() == 0 || mat_codiE.getText().trim().equals("")) return true; else return false;
    }
   ...
   */
    public void setText(String matCodi, boolean controla) {
        ...
        try {
            if (!swControl) return;
            if (!mat_codiE.isEnabled() || controla) controla(false);
        } catch (Exception k) {
            k.printStackTrace();
        }
    }
   ...
   */
    public void iniciar(DatosTabla datTabla, CInternalFrame intFrame, JLayeredPane layPan, EntornoUsuario entUsu) throws SQLException, java.text.ParseException {
    ...
    public void setFpaNomb(CTextField fpa_nombE) {
        ...
        this.add(mat_codiE, new GridBagConstraints(1, 0, 1, 1, 1.0, 1.0, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
        mat_nombE = fpa_nombE;
    }
   ...
   */
    ...
    public void addFocusListener(FocusListener focLis) {
        if (mat_codiE != null) mat_codiE.addFocusListener(focLis);
    }

    public void setName(String name) {
        if (mat_codiE != null) mat_codiE.setName(name);
    }

    public String getName() {
        if (mat_codiE == null) return null;
   ...
   */
    public void consMat() {
        ...
        try {
            ...
            if (intfr != null) intfr.setEnabled(true);
        }
    }
    ...
    void ej_consMat(ayuMat aymat) {
        if (aymat.consulta) {
            ...
            if (swControl) mat_nombE.setText(aymat.mat_nombT);
        }
        mat_codiE.requestFocus();
        aymat.dispose();
        if (intfr != null) {
   ...
   */
    public boolean controla(boolean reqFocus) throws SQLException, java.text.ParseException {
        ...
        if (isNull()) {
            mat_nombE.setText("");
            if (getAceptaNulo()) return true; else {
                msgError = "Matadero no puede estar en blanco";
                if (reqFocus) mat_codiE.requestFocus();
                return false;
            }
        }
   ...
   */
    ...
    public String getNombMat(int codMat, DatosTabla dt) throws SQLException, java.text.ParseException {
        String s = "SELECT * FROM v_matadero WHERE mat_codi= " + codMat;
        if (!dt.select(s)) return null;
    ...
    */
    public void setMatNomb(CTextField matNomb) {
        if (matNomb == null) swControl = false; else swControl = true;
        this.remove(mat_nombE);
        this.remove(Bcons);
        if (!swControl) return;
        this.remove(mat_codiE);
