...
public class Chant_FeelElectricity extends Chant {
    ...
    public boolean tick(Tickable ticking, int tickID) {
        ...
        if ((affecting() != null) && (affecting() instanceof MOB)) {
            ...
            if ((room != null) && (room.getArea().getClimateObj().weatherType(room) == Climate.WEATHER_THUNDERSTORM) && (CMLib.dice().rollPercentage() > M.charStats().getSave(CharStats.STAT_SAVE_ELECTRIC))) {
                ...
                if ((!M.isInCombat()) && (M != invoker) && (M.location().isInhabitant(invoker)) && (CMLib.flags().canBeSeenBy(invoker, M))) CMLib.combat().postAttack(M, invoker, M.fetchWieldedItem());
            }
        }
        return true;
    }
    ...
    public void unInvoke() {
        ...
        if (canBeUninvoked()) mob.tell("Your charged feeling is gone.");
        super.unInvoke();
    }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        } else return maliciousFizzle(mob, target, "<S-NAME> chant(s) to <T-NAMESELF>, but the magic fades.");
        return success;
    }
}
