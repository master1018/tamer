...
public class ProtocolStack extends Protocol implements Transport {
    ...
    public int getTimerThreads() {
        ...
        if (transport != null) {
            ...
            if (timer != null) return timer.getCorePoolSize();
        }
        return -1;
    }
    ...
    public Map<String, Object> dumpStats() {
        ...
        while (p != null) {
            prot_name = p.getName();
            tmp = p.dumpStats();
            if (prot_name != null && tmp != null) retval.put(prot_name, tmp);
            p = p.getDownProtocol();
        }
        return retval;
    }
    ...
    public Map<String, Object> dumpStats(String protocol_name) {
        ...
        while (p != null) {
            ...
            if (prot_name.equals(protocol_name)) {
                tmp = p.dumpStats();
                if (tmp != null) retval.put(prot_name, tmp);
            }
            p = p.getDownProtocol();
        }
        return retval;
    ...
    public String dumpTimerQueue() {
        ...
        if (transport != null) {
            timer = transport.getTimer();
            if (timer != null) return timer.dumpTaskQueue();
        }
        return "";
    }
    ...
    public void insertProtocol(Protocol prot, int position, String neighbor_prot) throws Exception {
        ...
        if (position != ProtocolStack.ABOVE && position != ProtocolStack.BELOW) throw new IllegalArgumentException("position has to be ABOVE or BELOW");
        Protocol neighbor = findProtocol(neighbor_prot);
        if (neighbor == null) throw new IllegalArgumentException("protocol \"" + neighbor_prot + "\" not found in " + stack.printProtocolSpec(false));
        if (position == ProtocolStack.BELOW && neighbor instanceof TP) throw new IllegalArgumentException("Cannot insert protocol " + prot.getName() + " below transport protocol");
    ...
    private void checkAndSwitchTop(Protocol oldTop, Protocol newTop) {
        if (oldTop == top_prot) top_prot = newTop;
    }
    ...
    public void insertProtocol(Protocol prot, int position, Class<? extends Protocol>... neighbor_prots) throws Exception {
        ...
        if (neighbor == null) throw new IllegalArgumentException("protocol \"" + neighbor_prots.toString() + "\" not found in " + stack.printProtocolSpec(false));
        insertProtocolInStack(prot, neighbor, position);
    }
    ...
    public void insertProtocolAtTop(Protocol prot) {
        ...
        prot.up_prot = this;
        top_prot = prot;
        if (log.isDebugEnabled()) log.debug("inserted " + prot + " on top of the stack");
    }
    ...
    public Protocol findProtocol(String name) {
        ...
        while (tmp != null) {
            ...
            if (prot_name != null && prot_name.equals(name)) return tmp;
                    ...
                    if (transport.isSingleton()) {
                        ...
                        synchronized (singleton_transports) {
                            ...
                            if (val != null) {
                                ...
                                if (num_inits >= 1) {
                                    continue;
                                } else singleton_transports.remove(singleton_name);
                            }
                        }
                    }
                }
                ...
                if (transport.isSingleton() && cluster_name != null) {
                    ...
                    synchronized (singleton_transports) {
                        synchronized (up_prots) {
                            ...
                            if (keys.contains(cluster_name)) throw new IllegalStateException("cluster '" + cluster_name + "' is already connected to singleton " + "transport: " + keys);
