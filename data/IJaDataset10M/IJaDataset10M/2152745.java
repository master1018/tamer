...
public final class MaildirFolder extends Folder {
   ...
   */
    public void open(int mode) throws MessagingException {
        if (this.mode != -1) throw new IllegalStateException("Folder is open");
        if (!maildir.exists() || !maildir.canRead()) throw new FolderNotFoundException(this);
        boolean success = true;
        if (!tmpdir.exists()) success = success && tmpdir.mkdirs();
        if (!newdir.dir.exists()) success = success && newdir.dir.mkdirs();
        if (!curdir.dir.exists()) success = success && curdir.dir.mkdirs();
        if (!success) throw new MessagingException("Unable to create directories");
        if (mode == READ_WRITE) {
            if (!maildir.canWrite()) throw new MessagingException("Folder is read-only");
        }
        this.mode = mode;
   ...
   */
    public void close(boolean expunge) throws MessagingException {
        if (mode == -1) throw new IllegalStateException("Folder is closed");
        if (expunge) expunge();
        mode = -1;
   ...
   */
    public Message[] expunge() throws MessagingException {
        ...
        if (mode == Folder.READ_ONLY) throw new IllegalWriteException();
        Message[] expunged;
        synchronized (this) {
        ...
        if (expunged.length > 0) notifyMessageRemovedListeners(true, expunged);
        return expunged;
    }
   ...
   */
    public synchronized Message getMessage(int msgnum) throws MessagingException {
        ...
        int index = msgnum - 1;
        if (index < 0 || index >= alen) throw new MessagingException("No such message: " + msgnum);
        if (index < clen) return curdir.messages[index]; else return newdir.messages[index - clen];
    }
   ...
   */
    void statDir(MaildirTuple dir) throws MessagingException {
        ...
        if (timestamp == dir.timestamp) return;
   ...
   */
    void setSeen(MaildirMessage message, boolean seen) throws MessagingException {
        ...
        } else dst = new File(newdir.dir, message.uniq);
        if (!src.renameTo(dst)) throw new MessagingException("Unable to move message");
    }
   ...
   */
    public synchronized void appendMessages(Message[] m) throws MessagingException {
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < m.length; i++) {
                if (m[i] instanceof MimeMessage) {
                    ...
                    try {
                        ...
                        if (!tmpfile.createNewFile()) throw new MessagingException("Temporary file already exists");
            ...
            if (n.length > 0) appended.toArray(n);
        }
        if (n.length > 0) notifyMessageAddedListeners(n);
    }
   ...
   */
    static String createUniq() throws MessagingException, IOException {
        ...
        if (urandom.exists() && urandom.canRead()) {
            ...
            int offset = 0;
            while (offset < bytes.length) offset += in.read(bytes, offset, bytes.length - offset);
            in.close();
            for (int i = 0; i < bytes.length; i++) pid |= (((long) bytes[i]) * ((long) Math.pow(i, 2)));
        } else pid += ++deliveryCount;
   ...
   */
    public Folder[] list() throws MessagingException {
        if (type != HOLDS_FOLDERS) throw new MessagingException("This folder can't contain subfolders");
        try {
            String[] files = maildir.list();
            Folder[] folders = new Folder[files.length];
            for (int i = 0; i < files.length; i++) folders[i] = store.getFolder(maildir.getAbsolutePath() + File.separator + files[i]);
            return folders;
        } catch (SecurityException e) {
   ...
   */
    public boolean create(int type) throws MessagingException {
        if (maildir.exists()) throw new MessagingException("Folder already exists");
        switch(type) {
            case HOLDS_FOLDERS:
                try {
                    if (!maildir.mkdirs()) return false;
                    this.type = type;
        ...
        if (recurse) {
            try {
                if (type == HOLDS_FOLDERS) {
                    Folder[] folders = list();
                    for (int i = 0; i < folders.length; i++) if (!folders[i].delete(recurse)) return false;
                }
                if (!delete(maildir)) return false;
   ...
   */
    public boolean renameTo(Folder folder) throws MessagingException {
        try {
            ...
            if (filename != null) {
                if (!maildir.renameTo(new File(filename))) return false;
                notifyFolderRenamedListeners(folder);
                return true;
            } else throw new MessagingException("Illegal filename: null");
        } catch (SecurityException e) {
            throw new MessagingException("Access denied", e);
        }
    }
    ...
    class MaildirListFilter implements FilenameFilter {
        ...
        public boolean accept(File directory, String name) {
            if (asteriskIndex > -1) {
                ...
                return (directory.equals(maildir) && name.startsWith(start) && name.endsWith(end));
            }
