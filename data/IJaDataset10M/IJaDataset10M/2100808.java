...
public class Pull extends Go {
    ...
    public boolean execute(MOB mob, Vector commands, int metaFlags) throws java.io.IOException {
        ...
        if (commands.size() > 1) {
            ...
            if (dirCode >= 0) {
                if ((mob.location().getRoomInDir(dirCode) == null) || (mob.location().getExitInDir(dirCode) == null) || (!mob.location().getExitInDir(dirCode).isOpen())) {
        ...
        if (openThis == null) openThis = mob.location().fetchFromMOBRoomFavorsItems(mob, null, whatToOpen, Wearable.FILTER_ANY);
        ...
        if (mob.location().okMessage(mob, msg)) {
            ...
            if ((dir.length() > 0) && (msg.tool() instanceof Room)) {
                ...
                if ((dirCode >= 0) && (super.move(mob, dirCode, false, false, false, false))) {
                    if (openThis instanceof Item) R.bringItemHere((Item) openThis, CMProps.getIntVar(CMProps.SYSTEMI_EXPIRE_PLAYER_DROP), true); else if (openThis instanceof MOB) move((MOB) openThis, dirCode, ((MOB) openThis).isInCombat(), false, true, true);
                }
            }
        }
        return false;
