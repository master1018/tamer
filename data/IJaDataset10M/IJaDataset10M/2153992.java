...
public class PSOkAssembler implements PacketAssembler<PSOK> {
    ...
    public PSOK process(int step, ByteBuffer buffer, PSOK packet, Object message) {
        ...
        if (step == 1) {
            packet.setStatementId((int) Utils.readLong(buffer.array(), 0, buffer.limit()));
        } else if (step == 2) {
            packet.setColumns((int) Utils.readLong(buffer.array(), 0, buffer.limit()));
        } else if (step == 3) {
            packet.setParameters((int) Utils.readLong(buffer.array(), 0, buffer.limit()));
        }
        return packet;
    }
}
