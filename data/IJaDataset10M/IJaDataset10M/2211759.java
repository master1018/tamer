...
public class MRUMenu implements Serializable {
    ...
    private transient int m_nBaseMenuItemCount = 0;
    ...
    public void setMenu(JMenu menuBase) {
        if (menuBase == null) throw new java.security.InvalidParameterException("menuBase may not be null");
        m_baseMenu = menuBase;
    ...
    public void setListener(MRUMenu.listener listener) {
        if (listener == null) throw new java.security.InvalidParameterException("Listener may not be null");
        m_listener = listener;
    }
    ...
    public void update(File filName) {
        ...
        if (m_setMostRecentlyUsedFiles.size() > m_nMaxEntries) m_setMostRecentlyUsedFiles.remove(m_setMostRecentlyUsedFiles.last());
        showMostRecentlyUsed();
    }

    public void showMostRecentlyUsed() {
        for (int i = m_baseMenu.getItemCount(); i > m_nBaseMenuItemCount; i--) m_baseMenu.remove(i - 1);
