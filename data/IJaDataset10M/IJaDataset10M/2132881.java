...
class RTFAttributes {
    ...
    static Dictionary attributesByKeyword() {
        ...
        int i, m;
        m = attributes.length;
        for (i = 0; i < m; i++) d.put(attributes[i].rtfName(), attributes[i]);
        return d;
    }
    ...
    static class BooleanAttribute extends GenericAttribute implements RTFAttribute {
        ...
        public boolean writeValue(Object o_value, RTFGenerator target, boolean force) throws IOException {
            Boolean val;
            if (o_value == null) val = Boolean.valueOf(swingDefault); else val = (Boolean) o_value;
    ...
    static class AssertiveAttribute extends GenericAttribute implements RTFAttribute {
        ...
        public boolean set(MutableAttributeSet target) {
            if (swingValue == null) target.removeAttribute(swingName); else target.addAttribute(swingName, swingValue);
            return true;
        }
    ...
    static class NumericAttribute extends GenericAttribute implements RTFAttribute {
        ...
        public boolean set(MutableAttributeSet target, int parameter) {
            Number swingValue;
            if (scale == 1f) swingValue = new Integer(parameter); else swingValue = new Float(parameter / scale);
