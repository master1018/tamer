...
public class amlImport implements ImportPlugin, LogReaderConnectionImportPlugin {
    ...
    public MiningResult importFile(InputStream input) throws IOException {
        try {
            ...
            result = traverseAML(result, doc.getDocumentElement(), null, ObjDef_Name, ObjDef_LinkId, modelid_net, function_LinkId);
    ...
    public static void traverseAMLforObjectNames(HashMap partialMap, Node currentNode, HashMap ObjDef_LinkId, HashMap ModelId_ModelType) {
                ...
                if (currentChild.getNodeName().equals("ObjDef")) {
                    ...
                    for (int k = 0; k < currentChildren.getLength(); k++) {
                        ...
                        if (!Child.getNodeName().equals("AttrDef")) {
                            continue;
                        } else if (!Child.getAttributes().getNamedItem("AttrDef.Type").getNodeValue().equals("AT_NAME")) {
                            continue;
                    ...
                    for (int j = 0; j < currentChild.getAttributes().getLength(); j++) {
                        if (currentChild.getAttributes().item(j).getNodeName().equals("LinkedModels.IdRefs")) {
                            String links = currentChild.getAttributes().getNamedItem("LinkedModels.IdRefs").getNodeValue();
    ...
    public EPCResult traverseAML(EPCResult partialResult, Node currentNode, Object parent, HashMap ObjDef_Name, HashMap ObjDef_LinkId, HashMap modelid_net, HashMap function_LinkId) throws Exception {
        if (currentNode.hasChildNodes()) {
            for (int i = 0; i < currentNode.getChildNodes().getLength(); i++) {
                Node currentChild = currentNode.getChildNodes().item(i);
                if (currentChild.getNodeName().equals("Group")) {
                    ...
                    if (currentChild.hasChildNodes()) {
                        ...
                        for (int j = 0; j < currentChildren.getLength(); j++) {
                            ...
                            if (Child.getAttributes().getNamedItem("AttrDef.Type").getNodeValue().equals("AT_NAME")) {
                    ...
                    partialResult = traverseAML(partialResult, currentChild, dir, ObjDef_Name, ObjDef_LinkId, modelid_net, function_LinkId);
                }
                if (currentChild.getNodeName().equals("Model") && currentChild.getAttributes().getNamedItem("Model.Type").getNodeValue().equals("MT_EEPC")) {
                    ...
                    try {
                        ...
                        modelid_net.put(currentChild.getAttributes().getNamedItem("Model.ID").getNodeValue(), net);
                    } catch (Throwable x) {
    ...
    public static EPC read(Node node, HashMap ObjDef_Name, HashMap ObjDef_LinkId, HashMap function_LinkId, String ModelName) throws Exception {
        EPC result = new EPC(true);
        parseNet(node, result, ObjDef_Name, ObjDef_LinkId, function_LinkId, ModelName);
        return result;
    }

    private static void parseNet(Node node, EPC net, HashMap ObjDef_Name, HashMap ObjDef_LinkId, HashMap function_LinkId, String ModelName) throws Exception {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (!(symbolnum.equals("ST_FUNC") || symbolnum.equals("ST_EV") || symbolnum.equals("ST_OPR_AND_1") || symbolnum.equals("ST_OPR_OR_1") || symbolnum.equals("ST_OPR_XOR_1"))) {
                continue;
            }
            ...
            if (symbolnum.equals("ST_FUNC")) {
                if (ObjDef_LinkId.containsKey(ObjDef)) {
                    EPCSubstFunction sf = (EPCSubstFunction) net.addFunction(new EPCSubstFunction(new LogEvent(ownName, "unknown:normal"), net, null));
                    sf.setIdentifier(ownName);
                    mapping.put(id, sf);
                    function_LinkId.put(sf, ObjDef_LinkId.get(ObjDef));
                } else {
                    EPCFunction f = net.addFunction(new EPCFunction(new LogEvent(ownName, "unknown:normal"), net));
                ...
                for (int j = 0; j < n.getChildNodes().getLength(); j++) {
                    if (n.getChildNodes().item(j).getNodeName().equals("CxnOcc")) {
                        ...
                        if (net.addEdge((EPCObject) mapping.get(source), (EPCObject) mapping.get(dest)) == null) {
                            throw (new Exception("<html>Structural properties of EPCs are violated in input file.<br>" + "The following edge could not be added:<br><br>" + mapping.get(source).toString() + " ==> " + mapping.get(dest).toString() + "<br><br>Import aborted.</html>"));
                        }
                    }
                }
            }
