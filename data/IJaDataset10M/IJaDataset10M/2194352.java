...
public class Stress {
    ...
    public void test(int aNumBeaters, int aPoolSize, long aPause, boolean isDebug, boolean doLoad) throws Exception {
    ...
    private class Stresser implements Runnable {
        ...
        Stresser(JavaSpace aSpace, int aPoolSize, long aPause, boolean debug) {
            theSpace = aSpace;
        ...
        public void run() {
            ...
            while (true) {
                try {
                    Integer myValue = new Integer(theRNG.nextInt(thePoolSize));
                    Entry myTemplate = new DummyEntry(myValue.toString());
                    Entry myResult = take(myTemplate, thePause);
                    if (myResult != null) theSpace.write(myTemplate, null, Lease.FOREVER);
                    synchronized (this) {
                        ++theTxns;
                    }
                    if (isDebug) {
    ...
    public static void main(String anArgs[]) {
        ...
        try {
            ...
            new Stress().test(Integer.parseInt(anArgs[0]), Integer.parseInt(anArgs[1]), Long.parseLong(anArgs[2]), Boolean.getBoolean("debug"), Boolean.getBoolean("load"));
