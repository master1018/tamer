...
import nl.huub.van.amelsvoort.client.entity_t;
import nl.huub.van.amelsvoort.qcommon.qfiles;
import nl.huub.van.amelsvoort.qcommon.qfiles.dmdl_t;
import nl.huub.van.amelsvoort.render.Anorms;
import nl.huub.van.amelsvoort.render.image_t;
...
public abstract class Mesh extends Light {
   ...
   */
    boolean R_CullAliasModel(entity_t e) {
        if (HuubHelp.isLog) {
            HuubHelp.log("Mesh R_CullAliasModel x=" + e.origin[0] + " y=" + e.origin[1], HuubHelp.GL);
        }
   ...
   */
    void R_DrawAliasModel(entity_t e) {
        if (HuubHelp.isLog) {
            HuubHelp.log("Mesh R_DrawAliasModel " + e.model.name, HuubHelp.NIET_BELANGRIJK);
        }
        if (e.model.name.contains("normaalstaathierhuub")) {
            alternatiefBmp(e);
        } else {
            alsVanouds(e);
        }
    ...
    private void alsVanouds(entity_t e) {
        ...
        if (HuubHelp.isLog) {
            HuubHelp.log("Mesh R_DrawAliasModel plaatje naam : " + skin.name, HuubHelp.NIET_BELANGRIJK);
        }
        GL_Bind(skin.texnum);
    ...
    /**
   * Tijdelijk om de unwrapped bmp goed te krijgen
   * @param e
   * @param skin
   * @param paliashdr
   */
    private void alternatiefBmp(entity_t e) {
        qfiles.dmdl_t paliashdr = (qfiles.dmdl_t) currentmodel.extradata;
        c_alias_polys += paliashdr.num_tris;
        if ((currententity.flags & Defines.RF_DEPTHHACK) != 0) {
            gl.glDepthRange(gldepthmin, gldepthmin + 0.3 * (gldepthmax - gldepthmin));
        ...
        if (HuubHelp.isLog) {
            HuubHelp.log("Mesh alternatiefBmp plaatje naam : " + skin.name, HuubHelp.MOMENTEEL_BELANGRIJK);
        }
        GL_Bind(skin.texnum);
        gl.glShadeModel(GL_SMOOTH);
        GL_TexEnv(GL_MODULATE);
        ...
        GL_DrawAliasFrameLerpBmp(paliashdr, currententity.backlerp);
        GL_TexEnv(GL_REPLACE);
        gl.glShadeModel(GL_FLAT);
        gl.glPopMatrix();
        gl.glDisable(GL_BLEND);
    ...
    private void GL_DrawAliasFrameLerpBmp(dmdl_t paliashdr, float backlerp) {
        qfiles.daliasframe_t frame = paliashdr.aliasFrames[currententity.frame];
        int[] verts = frame.verts;
        qfiles.daliasframe_t oldframe = paliashdr.aliasFrames[currententity.oldframe];
        int[] ov = oldframe.verts;
        ...
        GL_LerpVertsBmp(paliashdr.num_xyz, ov, verts, move, frontv, backv);
        int pos = 0;
        int[] counts = paliashdr.counts;
        IntBuffer srcIndexBuf = null;
        FloatBuffer dstTextureCoords = textureArrayBuf;
        ...
        int mode;
        int size = counts.length;
        HuubHelp.log("De size " + size, HuubHelp.MOMENTEEL_BELANGRIJK);
        for (int j = 0; j < size; j++) {
            count = counts[j];
            HuubHelp.log("De count " + count, HuubHelp.MOMENTEEL_BELANGRIJK);
            if (count == 0) {
                break;
            }
            srcIndexBuf = paliashdr.indexElements[j];
    ...
    /**
   * Als alternatief om uit te proberen
   * @param nverts
   * @param ov
   * @param v
   * @param move
   ...
   * @param backv
   */
    private void GL_LerpVertsBmp(int nverts, int[] ov, int[] v, float[] move, float[] frontv, float[] backv) {
        FloatBuffer lerp = vertexArrayBuf;
        lerp.limit((nverts << 2) - nverts);
        int ovv, vv;
        if ((currententity.flags & (Defines.RF_SHELL_RED | Defines.RF_SHELL_GREEN | Defines.RF_SHELL_BLUE | Defines.RF_SHELL_DOUBLE | Defines.RF_SHELL_HALF_DAM)) != 0) {
