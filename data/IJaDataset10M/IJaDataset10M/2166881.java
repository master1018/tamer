...
public abstract class AbstractHugeMap<K, KE extends HugeElement<K>, V, VE extends HugeElement<V>, MA extends HugeAllocation> extends AbstractHugeContainer<V, MA> implements HugeMap<K, V> {
    ...
    public static final long HASH_MASK = 0xFFFFFFFFFL;
    ...
    protected AbstractHugeMap(HugeMapBuilder<K, V> hmb) {
        super(hmb);
        for (int i = 0; i < keysBuffers.length; i++) keysBuffers[i] = ByteBuffer.allocate(512 * 1024).order(ByteOrder.nativeOrder()).asIntBuffer();
        ensureCapacity(1);
    }

    VE acquireValueElement(long n) {
        if (valueElements.isEmpty()) return createValueElement(n);
    ...
    KE acquireKeyElement(long n) {
        if (keyElements.isEmpty()) return createKeyElement(n);
    ...
    public void recycle(Object o) {
        ...
        switch(((HugeElement) o).hugeElementType()) {
            case Element:
                if (valueElements.size() < allocationSize) valueElements.add((VE) o);
                break;
            case BeanImpl:
                if (valueImpls.size() < allocationSize) valueImpls.add((V) o);
                break;
            case KeyElement:
                if (keyElements.size() < allocationSize) keyElements.add((KE) o);
                break;
            case KeyImpl:
                if (keyImpls.size() < allocationSize) keyImpls.add((K) o);
                break;
        }
    }

    protected K acquireKeyImpl() {
        if (valueImpls.isEmpty()) return createKeyImpl();
    ...
    protected V acquireValueImpl() {
        if (valueImpls.isEmpty()) return createValueImpl();
            ...
            for (int i = 0, len = keysBuffer.limit(); i < len; i++) {
                ...
                if (i1 == 0) {
                    if (free) {
                        ...
                        if (keysBuffer.position() >= keysBuffer.limit() * 3 / 4) growBuffer(loHash); else if (i > 5 && keysBuffer.position() >= keysBuffer.limit() * 2 / 3) growBuffer(loHash); else if (i > 15 && keysBuffer.position() >= keysBuffer.limit() / 2) growBuffer(loHash); else keysBuffer.position(keysBuffer.position() + 1);
                        longSize++;
                        return loc;
                    }
                    return -1;
    ...
    public int[] sizes() {
        ...
        for (int i = 0; i < keysBuffers.length; i++) sizes[i] = keysBuffers[i].position();
        return sizes;
    }

    public int[] capacities() {
        int[] sizes = new int[keysBuffers.length];
        for (int i = 0; i < keysBuffers.length; i++) sizes[i] = keysBuffers[i].capacity();
        return sizes;
    }
