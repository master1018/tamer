...
public class TypeUtils {
    ...
    public static <T> Constructor<T> getNoArgConstructor(Class<T> clazz) {
        try {
            ...
            if (clazz.isMemberClass() || clazz.isAnonymousClass() || clazz.isLocalClass()) throw new IllegalStateException(clazz.getName() + " must be static and must have a no-arg constructor", e); else throw new IllegalStateException(clazz.getName() + " must have a no-arg constructor", e);
        }
    }
    ...
    public static boolean isOfInterest(Field field) {
        return !field.isAnnotationPresent(Ignore.class) && ((field.getModifiers() & NOT_SAVEABLE_MODIFIERS) == 0) && !field.isSynthetic();
    }
    ...
    public static boolean isOfInterest(Method method) {
        for (Annotation[] annos : method.getParameterAnnotations()) if (getAnnotation(AlsoLoad.class, annos) != null) return true;
        return false;
    }
    ...
    private static void getProperties(ObjectifyFactory fact, Class<?> clazz, List<Property> good, Class<?> topClass) {
        ...
        for (Field field : clazz.getDeclaredFields()) if (isOfInterest(field)) good.add(new FieldProperty(fact, topClass, field));
        for (Method method : clazz.getDeclaredMethods()) if (isOfInterest(method)) good.add(new MethodProperty(method));
    }
    ...
    public static Field getDeclaredField(Class<?> clazz, String fieldName) throws NoSuchFieldException {
        try {
            ...
            if (clazz.getSuperclass() == Object.class) throw ex; else return getDeclaredField(clazz.getSuperclass(), fieldName);
        }
    }
    ...
    public static <A extends Annotation> A getAnnotation(Class<A> annotationType, Annotation[] annotations) {
        for (Annotation anno : annotations) if (annotationType.isAssignableFrom(anno.getClass())) return (A) anno;
        return null;
    }
    ...
    public static <A extends Annotation> A getAnnotation(Class<A> annotationType, Property prop, Class<?> onClass) {
        ...
        if (anno == null) return onClass.getAnnotation(annotationType); else return anno;
    }
}
