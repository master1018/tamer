...
public class TelnetBinding implements Runnable, Binding {
    ...
    public void run() {
        while (!Thread.interrupted()) {
            ...
            Shell shell;
            try {
                shell = new Shell(m_command, new BufferedReader(new InputStreamReader(socket.getInputStream())), new PrintStream(socket.getOutputStream()), new PrintStream(socket.getOutputStream()));
            } catch (IOException e) {
                e.printStackTrace();
                try {
                    socket.close();
