...
public class SIGRecord extends Record {
    ...
    public SIGRecord(Name _name, short _dclass, int _ttl, int length, CountedDataInputStream in, Compression c) throws IOException {
        super(_name, Type.SIG, _dclass, _ttl);
        if (in == null) return;
        int start = in.getPos();
    ...
    public SIGRecord(Name _name, short _dclass, int _ttl, MyStringTokenizer st, Name origin) throws IOException {
        super(_name, Type.SIG, _dclass, _ttl);
        ...
        if (st.hasMoreTokens()) signature = base64.fromString(st.nextToken());
    }
