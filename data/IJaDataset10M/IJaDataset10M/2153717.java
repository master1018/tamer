...
public class ExpRep extends Explanation {
    ...
    public void paintComponent(Graphics g) {
        ...
        int width = 150;
        int height = 50;
        arc_left_top = new Arc2D.Double(x + radius, y, 2 * radius, 2 * radius, 90, 90, Arc2D.OPEN);
        arc_left_bottom = new Arc2D.Double(x + radius, y + height - 2 * radius, 2 * radius, 2 * radius, 180, 90, Arc2D.OPEN);
        arc_right_top = new Arc2D.Double(x + width - 3 * radius, y, 2 * radius, 2 * radius, 0, 90, Arc2D.OPEN);
        arc_right_bottom = new Arc2D.Double(x + width - 3 * radius, y + height - 2 * radius, 2 * radius, 2 * radius, 270, 90, Arc2D.OPEN);
        line_top = new Line2D.Double(x, y, x + width, y);
        line_bottom = new Line2D.Double(x + 2 * radius, y + height, x + width - 2 * radius, y + height);
        line_left = new Line2D.Double(x + radius, y + radius, x + radius, y + height - radius);
        line_right = new Line2D.Double(x + width - radius, y + radius, x + width - radius, y + height - radius);
