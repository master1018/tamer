...
public class PropertyMapTable extends AbstractTable {
    ...
    public static final int TABLE_INDEX = 0x15;

    public static final int MIN_ROW_SIZE = 0x04;
    ...
    protected void fillRow(MetadataModel model, MetadataStream m, StringsStream s, UserStringStream u, GuidStream g, BlobStream b, RenderableCilElement[] entry, int index) {
        ...
        if (parentTypeDefIndex[index] > 0) e.setParent(model.typeDef[parentTypeDefIndex[index] - 1]);
        ...
        if (offset > 0 && offset <= model.property.length) {
            ...
            if (length > model.property.length + 1) length = model.property.length + 1;
            length -= offset;
