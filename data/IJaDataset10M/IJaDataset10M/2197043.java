...
public class ForkedStepRunner extends StepRunner {
    ...
    Process fork(String vmargs) throws UnknownHostException, IOException {
        ...
        String[] args = { getClass().getName(), String.valueOf(localPort), String.valueOf(getStopOnFailure()), String.valueOf(getStopOnError()), String.valueOf(getTerminateOnError()) };
    ...
    public void runStep(Step step) throws Throwable {
        ...
        try {
            ...
            try {
                ...
                if (getStopOnError()) throw io;
            }
        } catch (AssertionFailedError afe) {
            fireStepFailure(step, afe);
            if (getStopOnFailure()) throw afe;
        } catch (Throwable thr) {
            fireStepError(step, thr);
            if (getStopOnError()) throw thr;
        } finally {
    ...
    private void trackScript(Script script) throws IOException, ForkedFailure, ForkedError {
        ...
        while (!stopped() && (ev = receiveEvent(script)) != null) {
            ...
            if (ev.getStep() == script && (StepEvent.STEP_START.equals(ev.getType()) || StepEvent.STEP_END.equals(ev.getType()))) {
                continue;
            }
            ...
            if (err != null) {
                ...
                if (err instanceof AssertionFailedError) {
                    ...
                    if (getStopOnError()) throw (ForkedError) err;
                }
            } else {
                fireStepEvent(ev);
            }
