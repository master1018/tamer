...
public class Q_RightSignedShift extends ExprNode implements Expr, ExprNumeric {
    ...
    Q_RightSignedShift(int id) {
        super(id);
    }

    Q_RightSignedShift(RDQLParser p, int id) {
        super(p, id);
    }
    ...
    public NodeValue eval(Query q, IndexValues env) {
        ...
        if (!x.isNumber()) throw new EvalTypeException("Q_RightSignedShift: Wanted a number: " + x);
        if (!y.isNumber()) throw new EvalTypeException("Q_RightSignedShift: Wanted a number: " + y);
        ...
        if (x.isInt() && y.isInt()) result.setInt(x.getInt() >> y.getInt()); else throw new EvalTypeException("Q_RightSignedShift: one or both operands are doubles: " + x + " >> " + y);
        return result;
    }

    public void jjtClose() {
        int n = jjtGetNumChildren();
        if (n != 2) throw new QueryException("Q_RightSignedShift: Wrong number of children: " + n);
