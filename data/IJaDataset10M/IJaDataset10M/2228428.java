 ...
 * @version 1.0
 */
public class ServerInformationPage extends JPanel implements SetupWizardPage {
    ...
    public static final String DEFAULT_SERVER_TEXT_CONTENT = "Enter the name of your SIP Server (for example,\"sip.company.com\")";

    public static final String DEFAULT_UNAME_TEXT_CONTENT = "Enter the user name given to you by your SIP provider (for example, \"jsmith\".";
    ...
    public void initComponents() {
        try {
            ...
            try {
                serverHelpMessage.setText(readFile("SetupWizardServerInformationPage-Part1.html"));
            ...
            try {
                userNameHelpMessage.setText(readFile("SetupWizardServerInformationPage-Part2.html"));
            } catch (IOException ex) {
            ...
            String server = Utils.getProperty(REGISTRAR_PROPERTY_NAME);
            String userName = Utils.getProperty(USER_NAME_PROPERTY_NAME);
    ...
    private String readFile(String file) throws IOException {
        try {
            ...
            try {
                reader = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("resource" + File.separator + file)));
            } catch (Exception ex) {
    ...
    public void validateContent() throws IllegalArgumentException {
        try {
            console.logEntry();
            if (serverField.getText() == null | serverField.getText().trim().length() == 0) throw new IllegalArgumentException("Please enter a valid server name!");
            if (userNameField.getText() == null | userNameField.getText().trim().length() == 0) throw new IllegalArgumentException("Please enter a valid user name!");
        } finally {
            console.logExit();
        }
    }
