...
public class PlayerWME extends WME {
    ...
    public int getProtossArmySize() {
        int count = 0;
        for (UnitWME unit : ABLStarCraftBot.getGame().getUnits()) {
            if (unit.getPlayerID() == this.playerID && (unit.getTypeID() == StarCraftConstants.Protoss_Zealot || unit.getTypeID() == StarCraftConstants.Protoss_Dragoon || unit.getTypeID() == StarCraftConstants.Protoss_High_Templar || unit.getTypeID() == StarCraftConstants.Protoss_Dark_Templar)) {
                count++;
            }
        }
        return count;
