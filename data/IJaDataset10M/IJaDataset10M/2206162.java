...
public class OrderEditorListView extends ItemListView {
    ...
    protected void setItems() {
        ...
        for (Character c : g.getTurn().getCharacters()) {
            ...
            if (textFilter != null) acceptChar = acceptChar && ((OrderFilter) textFilter).acceptCharacter(c);
    ...
    protected AbstractListViewFilter getTextFilter(String txt) {
        ...
        if (isHexNo) return new OrderHexFilter("Hex", txt);
    ...
    public JPopupMenu getPopupMenu(boolean hasSelectedItem) {
        ...
        CommandGroup group = Application.instance().getActiveWindow().getCommandManager().createCommandGroup("orderCommandGroup", new Object[] { editOrderAction, deleteOrderAction, "separator", new DrawAllOrdersAction(), new UnDrawAllOrdersAction() });
        ...
        public boolean acceptCharacter(Character c) {
            for (Order o : c.getOrders()) {
                ...
                for (int i = 0; i <= o.getLastParamIndex(); i++) {
                    if (o.getParameter(i).equals(hexNo)) return true;
                }
            }
            return false;
        }
    ...
    public class OrderTextFilter extends OrderFilter {
        ...
        public boolean acceptCharacter(Character c) {
            for (Order o : c.getOrders()) {
                if (o.isBlank()) continue;
                for (int i = 0; i <= o.getLastParamIndex(); i++) {
                    if (o.getParameter(i).contains(text)) return true;
                }
            }
            return false;
        }
