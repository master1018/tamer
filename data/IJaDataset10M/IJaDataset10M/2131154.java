...
public final class QualityValue extends Number implements Comparable<QualityValue> {
    ...
    */
    ...
    public boolean equals(Object o) {
        if (o == null || o.getClass() != QualityValue.class) return false;
    ...
    private static int parseAsInteger(String value) {
        ...
        if (length == 0 || length > 5) throw new BadRequestException(MALFORMED_VALUE_MESSAGE);
        if (length > 1 && value.charAt(1) != '.') throw new BadRequestException(MALFORMED_VALUE_MESSAGE);
        int firstCharacter = value.codePointAt(0);
        if (firstCharacter == '1') {
            for (int i = 2; i < length; ++i) if (value.charAt(i) != '0') throw new BadRequestException(MALFORMED_VALUE_MESSAGE);
            return 1000;
            ...
            for (int i = 2; i < 5; ++i) {
                ...
                if (i < length) {
                    ...
                    if (digit < 0 || digit > 9) throw new BadRequestException(MALFORMED_VALUE_MESSAGE);
                    weight += digit;
                }
            }
            return weight;
        } else throw new BadRequestException(MALFORMED_VALUE_MESSAGE);
    }
}
