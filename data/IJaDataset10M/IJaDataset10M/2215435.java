 ...
 *
 * @version $Id: UserManager.java 1403 2010-12-11 18:56:58Z netmackan $
 */
public class UserManager {
        ...
        if (userData == null) {
            ...
            for (IAuthType authType : availableAuthTypes) {
                ...
                if (matchValue != null) {
                    ...
                    try {
                        AuthDataBean authDataBean = (AuthDataBean) workerEntityManager.createNamedQuery("AuthDataBean.findByAuthData").setParameter(1, at).setParameter(2, matchValue).getSingleResult();
    ...
    public List<UserDataBean> listUsers(int organizationId, String role) {
        ...
        try {
            if (role != null) {
                List<?> result = workerEntityManager.createNamedQuery("UserDataBean.findByRoleAndOrg").setParameter(1, "%" + role + ",%").setParameter(2, organizationId).getResultList();
                for (Object o : result) {
                    fetchDeepUserData((UserDataBean) o);
                    retval.add((UserDataBean) o);
                }
            } else {
                List<?> result = workerEntityManager.createNamedQuery("UserDataBean.findByOrg").setParameter(1, organizationId).getResultList();
                for (Object o : result) {
                    fetchDeepUserData((UserDataBean) o);
                    retval.add((UserDataBean) o);
                }
    ...
    public UserDataBean findUser(String userName, int organizationId) {
        ...
        try {
            retval = (UserDataBean) workerEntityManager.createNamedQuery("UserDataBean.findByUserName").setParameter(1, userName).setParameter(2, organizationId).getSingleResult();
            if (retval != null) {
    ...
    public AuthDataBean findAuthData(int authType, String authData) {
        ...
        try {
            retval = (AuthDataBean) workerEntityManager.createNamedQuery("AuthDataBean.findByAuthData").setParameter(1, authType).setParameter(2, authData).getSingleResult();
        } catch (NoResultException e) {
        }
        return retval;
    }
    ...
    public UserAliasDataBean findUserAlias(int userId, String type, String alias) {
        ...
        try {
            return (UserAliasDataBean) workerEntityManager.createNamedQuery("UserAliasDataBean.findByUserAlias").setParameter(1, userId).setParameter(2, type).setParameter(3, alias).getSingleResult();
        } catch (NoResultException e) {
        }
        return retval;
    }
    ...
    public List<UserDataBean> findUserByAlias(int organizationId, String type, String alias) {
        List<UserDataBean> retval = new ArrayList<UserDataBean>();
        try {
            List<UserDataBean> result = workerEntityManager.createNamedQuery("UserAliasDataBean.findUserByAlias").setParameter(1, organizationId).setParameter(2, type).setParameter(3, alias).getResultList();
    ...
    public List<UserDataBean> findUserLikeAlias(int organizationId, String type, String containAlias) {
        List<UserDataBean> retval = new ArrayList<UserDataBean>();
        try {
            List<UserDataBean> result = workerEntityManager.createNamedQuery("UserAliasDataBean.findUserLikeAlias").setParameter(1, organizationId).setParameter(2, type).setParameter(3, "%" + containAlias + "%").getResultList();
            for (UserDataBean ud : result) {
