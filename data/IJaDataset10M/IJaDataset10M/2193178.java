...
public class ArrayClassLoader extends ClassLoader {
   ...
   * Each element is suitable for defineClass. */
    ...
    public ArrayClassLoader(byte[][] classBytes) {
        ...
        for (int i = 0; i < size; i++) classNames[i] = "lambda" + i;
    }
    ...
    private void reserve(int count) {
        ...
        if (loadedClasses == null) loadedClasses = new Class[newLength]; else if (size + count > loadedClasses.length) {
        ...
        if (classBytes == null) classBytes = new byte[newLength][]; else if (size + count > classBytes.length) {
            byte[][] classBytesNew = new byte[newLength][];
            System.arraycopy(classBytes, 0, classBytesNew, 0, size);
            classBytes = classBytesNew;
        }
        if (classNames == null) classNames = new String[newLength]; else if (size + count > classNames.length) {
