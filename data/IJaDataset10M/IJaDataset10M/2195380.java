...
public class PKIApplet extends Applet {
    ...
    void init() {
        ...
        short keyPos = 0;
        short privKeyStart = (short) (Data.PrKDFOffset + Data.newPrivKeyOffset - Data.privKeyRecordSize * cnt);
        short pubKeyStart = (short) (Data.PuKDFOffset + Data.newPubKeyOffset - Data.pubKeyRecordSize * cnt);
        for (short i = 0; i < cnt; i++) {
            ...
            if (key.value != null) {
                ...
                if (tail > 0) {
                    short privOffset = (short) (privKeyStart + Data.privKeyRecordSize * keyPos);
                    Util.arrayCopyNonAtomic(files, (short) (privOffset + Data.privKeyRecordSize), files, privOffset, (short) (Data.privKeyRecordSize * tail));
                    privOffset = (short) ((privKeyStart + Data.privKeyRecordSize * (keyPos + tail)));
                    short pubOffset = (short) (pubKeyStart + Data.pubKeyRecordSize * keyPos);
                    Util.arrayCopyNonAtomic(files, (short) (pubOffset + Data.pubKeyRecordSize), files, pubOffset, (short) (Data.pubKeyRecordSize * tail));
                    pubOffset = (short) ((pubKeyStart + Data.pubKeyRecordSize * (keyPos + tail)));
                }
            }
        }
        Data.newPrivKeyOffset = (short) (privKeyStart - Data.PrKDFOffset + keyPos * Data.privKeyRecordSize);
        Data.newPubKeyOffset = (short) (pubKeyStart - Data.PuKDFOffset + keyPos * Data.pubKeyRecordSize);
    ...
    File readFile(DFile parent) {
        ...
        if ((type & File.DIR) == 0) {
            ...
            if ((type & File.EMPTY) == 0) {
                f = new EFile(parent, id, type, Parser.offset, length, Data.Files);
                Parser.skip(length);
            } else {
                type &= ~File.EMPTY;
                byte[] data = new byte[length];
                short dlen = Parser.getShort();
                Util.arrayCopyNonAtomic(Data.Files, Parser.offset, data, (short) 0, dlen);
    ...
    public void process(APDU apdu) {
        ...
        if (CLA == 0 && INS == (byte) (0xA4) && data[ISO7816.OFFSET_P1] == 4) {
            return;
        }
        if (CLA != (byte) 0x80) {
    ...
    void update(APDU apdu) {
        ...
        short off = 5;
        while (l > 0) {
            Util.arrayCopyNonAtomic(data, off, f.data, (short) (f.offset + offset), l);
            offset += l;
    ...
    void verify(APDU apdu) {
        ...
        byte[] data = apdu.getBuffer();
        short pin_num = -1;
        for (short i = 0; i < (short) (PIN_REFs.length - Data.freePINSlots); i++) {
    ...
    void manageSE(APDU apdu) {
        ...
        if (keyNum == -1 || keyFile == null || keyFile.type != File.PrivateKeyFile) {
            ISOException.throwIt((short) 0x6600);
        }
    ...
    void newKey(APDU apdu) {
        ...
        if (Data.freeKeySlots == 0 || (nonRepudiation && Data.freePINSlots == 0)) {
            ISOException.throwIt((short) 0x9001);
        }
        ...
        try {
            ...
            if (nonRepudiation) {
                ...
                Util.arrayCopy(data, (short) 16, Data.Files, (short) (Data.AODFOffset + Data.newPINOffset + Data.PINLabelOffset), (short) 32);
            }
            KeyPair p = Pairs.getKeyPair(keyLen);
            p.genKeyPair();
            PrivateKey key = new PrivateKey(this, Data.newKeyID, PINs[pinIndex], nonRepudiation, keyLen, (RSAPrivateKey) p.getPrivate());
            ...
            Util.arrayCopy(hash, x0, files, (short) (offset + Data.pubHashOffset), (short) 20);
            Util.setShort(files, (short) (offset + Data.pubKeyLengthOffset), keyLen);
            offset = (short) (Data.PrKDFOffset + Data.newPrivKeyOffset);
            files[(short) (offset + Data.privPINIDOffset)] = nonRepudiation ? Data.newPINID : Data.PIN_G_ID;
            Util.arrayCopy(hash, x0, files, (short) (offset + Data.privHashOffset), (short) 20);
            Util.setShort(files, (short) (offset + Data.privKeyLengthOffset), keyLen);
    ...
    void sign(APDU apdu) {
        ...
        if (len != expected) {
            ...
            cipher.doFinal(signBuffer, x0, digestLength, data, (short) (expected - len));
        }
...
class PrivateKey {
    ...
    PrivateKey(PKIApplet app) {
        ...
        try {
            value = (RSAPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_RSA_PRIVATE, keyLen, false);
     ...
     */
    PrivateKey(PKIApplet app, short id, OwnerPIN PIN, boolean nonRepudiation, short keyLen, RSAPrivateKey value) {
        this.applet = app;
        this.id = id;
        this.PIN = PIN;
