...
public class ParallelSearchManager implements SearchManager {
    ...
    public void newProperties(PropertySheet ps) throws PropertyException {
        linguist = (ParallelSimpleLinguist) ps.getComponent(PROP_LINGUIST, ParallelSimpleLinguist.class);
        scorer = (AcousticScorer) ps.getComponent(PROP_SCORER, AcousticScorer.class);
        activeListFactory = (ActiveListFactory) ps.getComponent(PROP_ACTIVE_LIST_FACTORY, ActiveListFactory.class);
        this.doFeaturePruning = ps.getBoolean(PROP_DO_FEATURE_PRUNING, PROP_DO_FEATURE_PRUNING_DEFAULT);
        this.doCombinePruning = ps.getBoolean(PROP_DO_COMBINE_PRUNING, PROP_DO_COMBINE_PRUNING_DEFAULT);
        if (doFeaturePruning) {
            featureScorePruner = (FeatureScorePruner) ps.getComponent(PROP_FEATURE_SCORE_PRUNER, FeatureScorePruner.class);
        }
        if (doCombinePruning) {
            combinedScorePruner = (CombinedScorePruner) ps.getComponent(PROP_COMBINED_SCORE_PRUNER, CombinedScorePruner.class);
        }
    ...
    public Result recognize(int nFrames) {
        ...
        result = new Result(combinedActiveList, resultList, currentFrameNumber, done);
        return result;
    }
    ...
    private void printActiveLists() {
        ...
        for (Iterator i = linguist.getFeatureStreams(); i.hasNext(); ) {
            FeatureStream stream = (FeatureStream) i.next();
            debugPrint(" ActiveList, " + stream.getName() + ": " + stream.getActiveList().size());
        }
    }
    ...
    private void grow() {
        ...
        for (Iterator i = linguist.getFeatureStreams(); i.hasNext(); ) {
            ...
            for (Iterator t = stream.getActiveList().iterator(); t.hasNext(); ) {
    ...
    private void growDelayedExpansionList() {
        ...
        if (doCombinePruning) {
            delayedExpansionList = combinedScorePruner.prune(delayedExpansionList);
        }
    ...
    private void growParallelToken(ParallelToken token) {
        ...
        for (int i = 0; i < arcs.length; i++) {
            ...
            boolean firstToken = oldNextToken == null || oldNextToken.getFrameNumber() != nextFrameNumber;
            if (nextState.getColor() == Color.RED) {
                ...
                if (firstToken || oldParallelToken == null || oldParallelToken.getScore() <= logEntryScore) {
                ...
                if (firstToken || getBestToken(nextState).getScore() <= logEntryScore) {
                    ParallelToken newToken = new ParallelToken(token, nextState, logEntryScore, token.getCombinedScore(), nextFrameNumber, token.getLastCombineTime());
                ...
                throw new IllegalStateException("Color of state " + nextState.getName() + " not RED or GREEN, its " + nextState.getColor().toString() + "!");
            }
        }
    }
        ...
        for (int a = 0; a < arcs.length; a++) {
            ...
            if (nextState.getColor() == Color.RED) {
                ...
                if (firstToken || oldNextToken.getScore() <= logEntryScore) {
                    ...
                    transitionParallelTokens(token, nextToken, arc.getProbability());
                    setBestToken(nextState, nextToken);
                ...
                if (parallelToken != null) {
                    ...
                    if (firstToken || oldToken.getFeatureScore() <= logEntryScore) {
