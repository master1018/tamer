...
public final class SimpleBookmark implements SimpleXMLDocHandler {
    ...
    private static List<HashMap<String, Object>> bookmarkDepth(PdfReader reader, PdfDictionary outline, IntHashtable pages, boolean processCurrentOutlineOnly) {
        ...
        while (outline != null) {
                ...
                if (dest != null) {
                    ...
                    if (action != null) {
                        if (PdfName.GOTO.equals(PdfReader.getPdfObjectRelease(action.get(PdfName.S)))) {
                            ...
                            dest = PdfReader.getPdfObjectRelease(action.get(PdfName.D));
                            if (dest != null) {
                                if (dest.isString()) map.put("Named", dest.toString()); else if (dest.isName()) map.put("NamedN", PdfName.decodeName(dest.toString())); else if (dest.isArray()) {
                                    PdfArray arr = (PdfArray) dest;
                                    StringBuffer s = new StringBuffer();
                                    s.append(arr.getPdfObject(0).toString());
                                    s.append(' ').append(arr.getPdfObject(1).toString());
                                    for (int k = 2; k < arr.size(); ++k) s.append(' ').append(arr.getPdfObject(k).toString());
                            ...
                            if (file != null) {
                                if (file.isString()) map.put("File", ((PdfString) file).toUnicodeString()); else if (file.isDictionary()) {
                                    file = PdfReader.getPdfObject(((PdfDictionary) file).get(PdfName.F));
                                    if (file.isString()) map.put("File", ((PdfString) file).toUnicodeString());
                                }
                            }
                            PdfObject newWindow = PdfReader.getPdfObjectRelease(action.get(PdfName.NEWWINDOW));
                            if (newWindow != null) map.put("NewWindow", newWindow.toString());
                        } else if (PdfName.LAUNCH.equals(PdfReader.getPdfObjectRelease(action.get(PdfName.S)))) {
                            map.put("Action", "Launch");
                            PdfObject file = PdfReader.getPdfObjectRelease(action.get(PdfName.F));
                            if (file == null) file = PdfReader.getPdfObjectRelease(action.get(PdfName.WIN));
                            if (file != null) {
                                if (file.isString()) map.put("File", ((PdfString) file).toUnicodeString()); else if (file.isDictionary()) {
            ...
            list.add(map);
            if (!processCurrentOutlineOnly) outline = (PdfDictionary) PdfReader.getPdfObjectRelease(outline.get(PdfName.NEXT)); else outline = null;
        }
        return list;
    }

    private static void mapGotoBookmark(HashMap<String, Object> map, PdfObject dest, IntHashtable pages) {
        if (dest.isString()) map.put("Named", dest.toString()); else if (dest.isName()) map.put("Named", PdfName.decodeName(dest.toString())); else if (dest.isArray()) map.put("Page", makeBookmarkParam((PdfArray) dest, pages));
        map.put("Action", "GoTo");
    }
    ...
    private static String makeBookmarkParam(PdfArray dest, IntHashtable pages) {
        ...
        if (obj.isNumber()) s.append(((PdfNumber) obj).intValue() + 1); else s.append(pages.get(getNumber((PdfIndirectReference) obj)));
        s.append(' ').append(dest.getPdfObject(1).toString().substring(1));
        for (int k = 2; k < dest.size(); ++k) s.append(' ').append(dest.getPdfObject(k).toString());
        return s.toString();
    }
    ...
    */
    public static List<HashMap<String, Object>> getBookmark(PdfReader reader, PdfDictionary outline, boolean includeRoot) {
        ...
        if (includeRoot) return bookmarkDepth(reader, outline, pages, true); else return bookmarkDepth(reader, (PdfDictionary) PdfReader.getPdfObjectRelease(outline.get(PdfName.FIRST)), pages, false);
    }
        ...
        for (Iterator<HashMap<String, Object>> it = list.listIterator(); it.hasNext(); ) {
            ...
            if ("GoTo".equals(map.get("Action"))) {
                ...
                if (page != null) {
                    ...
                    int pageNum;
                    if (idx < 0) pageNum = Integer.parseInt(page); else pageNum = Integer.parseInt(page.substring(0, idx));
        ...
        for (Iterator<HashMap<String, Object>> it = list.listIterator(); it.hasNext(); ) {
            ...
            if ("GoTo".equals(map.get("Action"))) {
                ...
                if (page != null) {
                    ...
                    if (idx < 0) pageNum = Integer.parseInt(page); else pageNum = Integer.parseInt(page.substring(0, idx));
                    boolean hit = false;
                    if (pageRange == null) hit = true; else {
                        int len = pageRange.length & 0xfffffffe;
                        for (int k = 0; k < len; k += 2) {
                            if (pageNum >= pageRange[k] && pageNum <= pageRange[k + 1]) {
                                hit = true;
                    ...
                    if (hit) {
                        if (idx < 0) page = Integer.toString(pageNum + pageShift); else page = pageNum + pageShift + page.substring(idx);
                    }
                    map.put("Page", page);
                }
            }
            List<HashMap<String, Object>> kids = (List<HashMap<String, Object>>) map.get("Kids");
            if (kids != null) shiftPageNumbers(kids, pageShift, pageRange);
        }
    }
        ...
        try {
            ...
            if ("GoTo".equals(action)) {
                ...
                if ((p = (String) map.get("Named")) != null) {
                    if (namedAsNames) outline.put(PdfName.DEST, new PdfName(p)); else outline.put(PdfName.DEST, new PdfString(p, null));
                    ...
                    if (!tk.hasMoreTokens()) {
                        ...
                        for (int k = 0; k < 4 && tk.hasMoreTokens(); ++k) {
                            fn = tk.nextToken();
                            if (fn.equals("null")) ar.add(PdfNull.PDFNULL); else ar.add(new PdfNumber(fn));
                        }
                    }
                ...
                if ((p = (String) map.get("Named")) != null) dic.put(PdfName.D, new PdfString(p, null)); else if ((p = (String) map.get("NamedN")) != null) dic.put(PdfName.D, new PdfName(p)); else if ((p = (String) map.get("Page")) != null) {
                    PdfArray ar = new PdfArray();
                    StringTokenizer tk = new StringTokenizer(p);
                ...
                if (dic.size() > 0 && file != null) {
                    ...
                    if (nw != null) {
                        if (nw.equals("true")) dic.put(PdfName.NEWWINDOW, PdfBoolean.PDFTRUE); else if (nw.equals("false")) dic.put(PdfName.NEWWINDOW, PdfBoolean.PDFFALSE);
                    }
    ...
    public static Object[] iterateOutlines(PdfWriter writer, PdfIndirectReference parent, List<HashMap<String, Object>> kids, boolean namedAsNames) throws IOException {
        ...
        for (int k = 0; k < refs.length; ++k) refs[k] = writer.getPdfIndirectReference();
        int ptr = 0;
        int count = 0;
        for (Iterator<HashMap<String, Object>> it = kids.listIterator(); it.hasNext(); ++ptr) {
            ...
            if (subKid != null && !subKid.isEmpty()) lower = iterateOutlines(writer, refs[ptr], subKid, namedAsNames);
            ...
            if (ptr > 0) outline.put(PdfName.PREV, refs[ptr - 1]);
            if (ptr < refs.length - 1) outline.put(PdfName.NEXT, refs[ptr + 1]);
            ...
            if (style != null) {
                ...
                int bits = 0;
                if (style.indexOf("italic") >= 0) bits |= 1;
                if (style.indexOf("bold") >= 0) bits |= 2;
                if (bits != 0) outline.put(PdfName.F, new PdfNumber(bits));
            }
        ...
        for (HashMap<String, Object> map : list) {
            ...
            for (Map.Entry<String, Object> entry : map.entrySet()) {
                ...
                if (key.equals("Title")) {
                    ...
                    if (key.equals("Named") || key.equals("NamedN")) value = SimpleNamedDestination.escapeBinaryString(value);
    ...
    public void endElement(String tag) {
        if (tag.equals("Bookmark")) {
            if (attr.isEmpty()) return; else throw new RuntimeException(MessageLocalization.getComposedMessage("bookmark.end.tag.out.of.place"));
        }
        if (!tag.equals("Title")) throw new RuntimeException(MessageLocalization.getComposedMessage("invalid.end.tag.1", tag));
        HashMap<String, Object> attributes = attr.pop();
        String title = (String) attributes.get("Title");
        attributes.put("Title", title.trim());
        String named = (String) attributes.get("Named");
        if (named != null) attributes.put("Named", SimpleNamedDestination.unEscapeBinaryString(named));
        named = (String) attributes.get("NamedN");
        if (named != null) attributes.put("NamedN", SimpleNamedDestination.unEscapeBinaryString(named));
        if (attr.isEmpty()) topList.add(attributes); else {
    ...
    public void startElement(String tag, Map<String, String> h) {
        if (topList == null) {
            if (tag.equals("Bookmark")) {
                ...
                return;
            } else throw new RuntimeException(MessageLocalization.getComposedMessage("root.element.is.not.bookmark.1", tag));
        }
        if (!tag.equals("Title")) throw new RuntimeException(MessageLocalization.getComposedMessage("tag.1.not.allowed", tag));
