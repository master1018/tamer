...
public class EmailListServlet_JMockTest {
    ...
    public void before() throws ServletException {
        ...
        mockery.checking(new Expectations() {

            {
                ...
                one(servletContext).getAttribute(EmailListService.KEY);
    ...
    public void doGet_without_list() throws Exception {
        mockery.checking(new Expectations() {
            ...
            {
                one(request).getParameter("listName");
                will(returnValue(null));
                one(emailListService).getListByName(null);
    ...
    public void doGet_with_list() throws Exception {
        ...
        mockery.checking(new Expectations() {
            ...
            {
                one(request).getParameter("listName");
                will(returnValue("foolist"));
                final List<String> list = Arrays.asList("larry@stooge.com", "moe@stooge.com", "curley@stooge.com");
                one(emailListService).getListByName("foolist");
                will(returnValue(list));
                one(response).getWriter();
                will(returnValue(writer));
                one(writer).println("larry@stooge.com");
                inSequence(printSequence);
                one(writer).println("moe@stooge.com");
                inSequence(printSequence);
                one(writer).println("curley@stooge.com");
                inSequence(printSequence);
                one(writer).close();
                inSequence(printSequence);
            }
