...
public class LoadDialogAction extends ProModAction {
    ...
    public LoadDialogAction(final String displayName, final Icon icon, final KeyStroke keyStroke) {
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (loadResult.getProjectRoot() == null) {
                JOptionPane.showMessageDialog(ModelerSession.getFrame(), ModelerSession.getCommonResourceBundle().getString("modeler.action.project.load.error.loadfile"), ModelerSession.getCommonResourceBundle().getString("modeler.action.project.load.error.loadfile.title"), JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            final AddProjectItemResult result = ModelerSession.getProjectControlService().addProject(new ProjectRoot(loadResult.getProjectRoot().getDisplayName(), loadResult.getProjectRoot().getProjectLocation()), true);
            switch(result.getStatus()) {
                case SUCCESS:
                    ModelerSession.getProjectControlService().synchronize(loadResult.getProjectRoot().getProjectFile(), null, true, false, false, true);
                    final ProModAction action = ModelerSession.getActionService().getAction(ModelerModel.MODELER_IDENTIFIER, ProjectNavigation.ACTION_EXPAND);
