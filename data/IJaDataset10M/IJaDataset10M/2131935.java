...
public class FieldWriterImpl extends AbstractMemberWriter implements FieldWriter, MemberSummaryWriter {
    ...
    public Content getSignature(FieldDoc field) {
        ...
        Content fieldlink = new RawHtml(writer.getLink(new LinkInfoImpl(LinkInfoImpl.CONTEXT_MEMBER, field.type())));
    ...
    public void addComments(FieldDoc field, Content fieldDocTree) {
        ...
        if (field.inlineTags().length > 0) {
            if (holder.equals(classdoc) || (!(holder.isPublic() || Util.isLinkable(holder, configuration())))) {
                writer.addInlineComment(field, fieldDocTree);
            } else {
                Content link = new RawHtml(writer.getDocLink(LinkInfoImpl.CONTEXT_FIELD_DOC_COPY, holder, field, holder.isIncluded() ? holder.typeName() : holder.qualifiedTypeName(), false));
                Content codeLink = HtmlTree.CODE(link);
                Content strong = HtmlTree.STRONG(holder.isClass() ? writer.descfrmClassLabel : writer.descfrmInterfaceLabel);
    ...
    public void addSummaryLabel(Content memberTree) {
        Content label = HtmlTree.HEADING(HtmlConstants.SUMMARY_HEADING, writer.getResource("doclet.Field_Summary"));
    ...
    public String getTableSummary() {
        return configuration().getText("doclet.Member_Table_Summary", configuration().getText("doclet.Field_Summary"), configuration().getText("doclet.fields"));
    }
    ...
    public String[] getSummaryTableHeader(ProgramElementDoc member) {
        String[] header = new String[] { writer.getModifierTypeHeader(), configuration().getText("doclet.0_and_1", configuration().getText("doclet.Field"), configuration().getText("doclet.Description")) };
        return header;
    }
    ...
    public void addInheritedSummaryAnchor(ClassDoc cd, Content inheritedTree) {
        inheritedTree.addContent(writer.getMarkerAnchor("fields_inherited_from_class_" + configuration().getClassName(cd)));
    }
    ...
    public void addInheritedSummaryLabel(ClassDoc cd, Content inheritedTree) {
        ...
        Content label = new StringContent(cd.isClass() ? configuration().getText("doclet.Fields_Inherited_From_Class") : configuration().getText("doclet.Fields_Inherited_From_Interface"));
    ...
    protected void addSummaryLink(int context, ClassDoc cd, ProgramElementDoc member, Content tdSummary) {
        Content strong = HtmlTree.STRONG(new RawHtml(writer.getDocLink(context, cd, (MemberDoc) member, member.name(), false)));
    ...
    protected Content getDeprecatedLink(ProgramElementDoc member) {
        return writer.getDocLink(LinkInfoImpl.CONTEXT_MEMBER, (MemberDoc) member, ((FieldDoc) member).qualifiedName());
    }
    ...
    protected Content getNavSummaryLink(ClassDoc cd, boolean link) {
        if (link) {
            return writer.getHyperLink("", (cd == null) ? "field_summary" : "fields_inherited_from_class_" + configuration().getClassName(cd), writer.getResource("doclet.navField"));
        } else {
    ...
    protected void addNavDetailLink(boolean link, Content liNav) {
        if (link) {
            liNav.addContent(writer.getHyperLink("", "field_detail", writer.getResource("doclet.navField")));
        } else {
