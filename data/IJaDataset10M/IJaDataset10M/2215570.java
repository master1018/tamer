...
/**
 * This class implements a limited document caching mechanism intended to
 * provide cache for services that have a need for cache to search and exchange
 * jxta documents.
 * <p/>
...
public final class Cm implements Runnable {
	 ...
	 * @param gcinterval
	 *            garbage collect max interval
	 * @param trackDeltas
	 *            when true deltas are tracked
	 * @param areaName
	 *            storage area name
	 */
	 ...
	 * @return List Strings containing the name of the files
	 */
	 ...
	 * @return the absolute time in milliseconds at which this document will
	 *         expire. -1 is returned if the file is not recognized or already
	 *         expired.
	 */
	 ...
	 * should cached by those other than the publisher. This value is either the
	 * cache lifetime or the remaining lifetime of the document, whichever is
	 * less.
	 * 
	 * @param dn
	 *            contains the name of the folder
	 * @param fn
	 *            contains the name of the file
	 * @return number of milliseconds until the file expires or -1 if the file
	 *         is not recognized or already expired.
	 */
	 ...
	 * @throws IOException
	 *             if an I/O error occurs was not possible.
	 */
	 ...
	 *             if an I/O error occurs
	 */
    public List<SrdiMessage.Entry> save(String dn, String fn, Advertisement adv) throws IOException {
        return save(dn, fn, adv, DiscoveryService.INFINITE_LIFETIME, DiscoveryService.NO_EXPIRATION);
    }
	 ...
	 *            file name
	 * @param adv
	 *            Advertisement to save
	 * @param lifetime
	 *            Document (local) lifetime in relative ms
	 * @param expiration
	 *            Document (global) expiration time in relative ms
	 * @throws IOException
	 *             Thrown if there is a problem saving the document.
	 */
    public synchronized List<SrdiMessage.Entry> save(String dn, String fn, Advertisement adv, long lifetime, long expiration) throws IOException {
        try {
            ...
            List<SrdiMessage.Entry> deltas = new ArrayList<SrdiMessage.Entry>();
            while (eachIndex.hasNext()) {
                Map.Entry<String, String> anEntry = eachIndex.next();
                String tattr = anEntry.getKey();
                String tvalue = anEntry.getValue();
                SrdiMessage.Entry entry = new SrdiMessage.Entry(tattr, tvalue, absoluteLifetime);
                deltas.add(entry);
                if (Logging.SHOW_FINE && LOG.isLoggable(Level.FINE)) {
                    LOG.fine("Added temp entry  :" + entry + " to deltas");
                }
            }
            if (Logging.SHOW_FINE && LOG.isLoggable(Level.FINE)) {
    ...
    /**
	 * Store some bytes in specified dir, and file name, and associated doc
	 * timeouts
	 * 
	 * @param dn
	 *            directory name
    ...
    /**
	 * Search and recovers documents that contains at least a macthing pair of
	 * tag/value.
	 * 
	 * @param dn
	 *            contains the name of the folder on which to perform the search
	 * @param value
	 *            contains the value to search on.
	 * @param attribute
    ...
    private synchronized void addDelta(String dn, Map<String, String> indexables, long exp) {
        LOG.info("ADD DELTA1" + trackDeltas);
        if (trackDeltas) {
            LOG.info("ADD DELTA2" + indexables);
            Iterator<Map.Entry<String, String>> eachIndex = indexables.entrySet().iterator();
