...
public final class McIDASUtil {
    ...
    public static int[] mcSecsToDayTime(long secs) {
        ...
        retvals[1] = (cal.get(cal.HOUR_OF_DAY) * 10000) + (cal.get(cal.MINUTE) * 100) + cal.get(cal.SECOND);
        return retvals;
    }
    ...
    public static String intBitsToString(int[] values) {
        ...
        for (int i = 0; i < values.length; i++) sb.append(intBitsToString(values[i]));
        return sb.toString();
    }
    ...
    public static boolean isChar(int value) {
        ...
        for (int i = 0; i < 4; i++) {
            if (!Character.UnicodeBlock.of(chars[i]).equals(Character.UnicodeBlock.BASIC_LATIN) || Character.isISOControl(chars[i])) return false;
        }
        return true;
    }
