...
public class Workspace extends XmlElement implements SwordElementInterface {
    ...
    */
    ...
    private static final XmlName XML_NAME = new XmlName(Namespaces.PREFIX_APP, "workspace", Namespaces.NS_APP);
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (isInstanceOf(element, Title.elementName())) {
                    if (title == null) {
                        ...
                        SwordValidationInfo info = new SwordValidationInfo(Title.elementName(), SwordValidationInfo.DUPLICATE_ELEMENT, SwordValidationInfoType.WARNING);
                    ...
                    validationItems.add(new SwordValidationInfo(new XmlName(element), SwordValidationInfo.UNKNOWN_ELEMENT, SwordValidationInfoType.INFO));
                }
            }
        } catch (Exception ex) {
    ...
    */
    protected SwordValidationInfo validate(ArrayList<SwordValidationInfo> existing, Properties validationContext) {
        ...
        if (collections == null || collections.size() == 0) {
            result.addValidationInfo(new SwordValidationInfo(Collection.elementName(), SwordValidationInfo.MISSING_ELEMENT_WARNING, SwordValidationInfoType.WARNING));
        }
        if (validateAll) {
            if (title != null) {
