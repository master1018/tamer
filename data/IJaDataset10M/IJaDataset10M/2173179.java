...
public class BuildInitialPopulationWithoutDuplicateIndividuals implements BuildPopulation {
    ...
    public BuildInitialPopulationWithoutDuplicateIndividuals(Random gen, LogReader log, GeneticMiningMatrices genMinMatrices) {
        generator = gen;
        logReader = log;
    ...
    private HNSet buildInputSet(int index) {
        return MethodsOverIndividuals.buildSet(geneticMiningMatrices.getStartMatrix().get(index), geneticMiningMatrices.getCausalMatrix().viewColumn(index), generator);
    }

    private HNSet buildOutputSet(int index) {
        return MethodsOverIndividuals.buildSet(geneticMiningMatrices.getEndMatrix().get(index), geneticMiningMatrices.getCausalMatrix().viewRow(index), generator);
    }
}
