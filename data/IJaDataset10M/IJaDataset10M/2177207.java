 ...
 *
 * @author Slava Pestov
 * @version $Id: PerlTokenMarker.java 601 2005-10-05 12:06:23Z jeliot $
 */
public class PerlTokenMarker extends TokenMarker {
    ...
    public byte markTokensImpl(byte _token, Segment line, int lineIndex) {
        ...
        if (token == Token.LITERAL_1 && lineIndex != 0 && lineInfo[lineIndex - 1].obj != null) {
        ...
        loop: for (int i = offset; i < length; i++) {
            ...
            switch(token) {
                case Token.NULL:
                    switch(c) {
                        case '#':
                            ...
                            if (backslash) backslash = false; else {
                        ...
                        case '@':
                            ...
                            if (doKeyword(line, i, c)) break;
                            if (length - i > 1) {
                                if (c == '&' && (array[i1] == '&' || Character.isWhitespace(array[i1]))) i++; else {
                                    addToken(i - lastOffset, token);
                ...
                case S_TWO:
                    if (backslash) backslash = false; else {
                        if (matchChar == '\0') {
                            if (Character.isWhitespace(matchChar) && !matchSpacesAllowed) break; else matchChar = c;
                        } else {
                            switch(matchChar) {
                                case '(':
                                    matchChar = ')';
                            ...
                            if (token == S_TWO) {
                                ...
                                token = S_END;
                                addToken(i1 - lastOffset, Token.LITERAL_2);
                                lastOffset = lastKeyword = i1;
                            }
                        }
                    }
                ...
                case Token.LITERAL_1:
                    if (backslash) backslash = false; else if (c == '"') {
                        addToken(i1 - lastOffset, token);
                        token = Token.NULL;
                        lastOffset = lastKeyword = i1;
                    }
    ...
    private boolean doKeyword(Segment line, int i, char c) {
        ...
        if (id == S_ONE || id == S_TWO) {
            ...
            lastOffset = i;
            lastKeyword = i1;
            if (Character.isWhitespace(c)) matchChar = '\0'; else matchChar = c;
