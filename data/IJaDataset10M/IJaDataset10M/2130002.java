...
public class UserDatabaseWizard extends JFrame implements ActionListener, ListSelectionListener {
    ...
    public UserDatabaseWizard(PublicExposedFunctions out, String user) {
        ...
        for (int i = users.length - 1; i >= 0; i--) model.addElement(users[i]);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == addButton) {
            ...
            if (user != null) model.addElement(user);
    ...
    class CheckPanel extends JPanel implements ChangeListener {
        ...
        private void clear() {
            for (int i = 0; i < checkBoxes.length; i++) setCheckbox(checkBoxes[i], false);
        }
        ...
        private void set(String flags) {
            ...
            for (int i = 0; i < flags.length(); i++) {
                ...
                if (flagIndex < 0 || flagIndex >= checkBoxes.length) continue;
                if (flagIndex >= 0 && flagIndex < checkBoxes.length) setCheckbox(checkBoxes[flagIndex], true);
            }
        }
        ...
        public void stateChanged(ChangeEvent e) {
            if (user != null) {
                String flags = "";
                for (int i = 0; i < checkBoxes.length; i++) if (checkBoxes[i].isSelected()) flags += allFlags.charAt(i);
                out.dbDeleteUser(user);
