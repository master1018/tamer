...
public class Q2Read extends LispReader {
    ...
    int skipIndentation() throws java.io.IOException, SyntaxException {
    ...
    public Object readCommand(boolean forceList) throws java.io.IOException, SyntaxException {
        ...
        for (; ; ) {
            ...
            while (ch == '\r' || ch == '\n') {
                if (singleLine()) return obj;
                ch = read();
                skipIndentation();
                column = port.getColumnNumber();
                ch = peek();
                if (column <= startColumn) break;
            }
            if (column <= startColumn && last != null) break;
            Object next;
            if (column == lastColumn && last != null) next = readCommand(); else if (column < lastColumn && last != null) {
                PairWithPosition p = pair;
                for (; ; ) {
                    Object n = p.getCdr();
                    if (n == LList.Empty) break;
                    PairWithPosition np = (PairWithPosition) n;
                    int pColumn = np.getColumnNumber() - 1;
                    if (pColumn >= column) {
                        if (pColumn > column) error('e', "some tokens on previous line indented more than current line");
                        n = np.getCdr();
                        if (n != LList.Empty) {
                            ...
                            last = (PairWithPosition) makePair(np, port.getLineNumber(), column);
                            p.setCdr(last);
                        }
                        break;
                    }
            ...
            if (last == null) {
                ...
                continue;
            } else last.setCdr(cur);
            last = cur;
        }
        if (!forceList) {
            if (obj == last) obj = last.getCar(); else if (last == null) obj = QuoteExp.voidExp;
        }
        return obj;
    }
...
class Q2ReaderParens extends ReaderDispatchMisc {
    ...
    public Object read(Lexer in, int ch, int count) throws java.io.IOException, SyntaxException {
        ...
        try {
            ...
            if (port.read() != ')') reader.error("missing ')'");
            return result;
        } finally {
