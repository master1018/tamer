...
public class FloatProcessor extends ImageProcessor {
    ...
    public FloatProcessor(int width, int height, float[] pixels, ColorModel cm) {
        if (pixels != null && width * height != pixels.length) throw new IllegalArgumentException(WRONG_LENGTH);
        this.width = width;
        this.height = height;
        this.pixels = pixels;
        this.cm = cm;
    ...
    public FloatProcessor(int width, int height, int[] pixels) {
        this(width, height);
        for (int i = 0; i < pixels.length; i++) this.pixels[i] = (float) (pixels[i]);
    }
	...
	*/
    ...
    public Image createImage() {
        ...
        if (firstTime || !lutAnimation) create8BitImage();
    ...
    protected byte[] create8BitImage() {
        ...
        if (pixels8 == null) pixels8 = new byte[size];
        float value;
        int ivalue;
    ...
    public void snapshot() {
        ...
        if (snapshotPixels == null || (snapshotPixels != null && snapshotPixels.length != pixels.length)) snapshotPixels = new float[width * height];
    ...
    public void reset(ImageProcessor mask) {
        ...
        if (mask.getWidth() != roiWidth || mask.getHeight() != roiHeight) throw new IllegalArgumentException(maskSizeError(mask));
    ...
    public int getPixel(int x, int y) {
        if (x >= 0 && x < width && y >= 0 && y < height) return Float.floatToIntBits(pixels[y * width + x]); else return 0;
    }
    ...
    public double getInterpolatedPixel(double x, double y) {
        if (interpolationMethod == BICUBIC) return getBicubicInterpolatedPixel(x, y, this); else {
            if (x < 0.0) x = 0.0;
    ...
    public final int getPixelInterpolated(double x, double y) {
        if (interpolationMethod == BILINEAR) {
            if (x < 0.0 || y < 0.0 || x >= width - 1 || y >= height - 1) return 0; else return Float.floatToIntBits((float) getInterpolatedPixel(x, y, pixels));
        } else if (interpolationMethod == BICUBIC) return Float.floatToIntBits((float) getBicubicInterpolatedPixel(x, y, this)); else return getPixel((int) (x + 0.5), (int) (y + 0.5));
    }
    ...
    public final void putPixel(int x, int y, int value) {
        if (x >= 0 && x < width && y >= 0 && y < height) pixels[y * width + x] = Float.intBitsToFloat(value);
    }

    /** Stores the specified real value at (x,y). */
    public void putPixelValue(int x, int y, double value) {
        if (x >= 0 && x < width && y >= 0 && y < height) pixels[y * width + x] = (float) value;
    }

    /** Returns the value of the pixel at (x,y) as a float. */
    public float getPixelValue(int x, int y) {
        if (x >= 0 && x < width && y >= 0 && y < height) return pixels[y * width + x]; else return 0f;
    }

    /** Draws a pixel in the current foreground color. */
    public void drawPixel(int x, int y) {
        if (x >= clipXMin && x <= clipXMax && y >= clipYMin && y <= clipYMax) putPixel(x, y, Float.floatToIntBits(fillColor));
    }
	...
	*/
            ...
            for (int x = roiX; x < (roiX + roiWidth); x++) {
                ...
                switch(op) {
                    ...
                    case MULT:
                        ...
                        break;
                    case GAMMA:
                        if (v1 <= 0f) v2 = 0f; else v2 = (float) Math.exp(c * Math.log(v1));
                        break;
                    case LOG:
                        if (v1 <= 0f) v2 = 0f; else v2 = (float) Math.log(v1);
                        break;
                    case EXP:
                    ...
                    case SQR:
                        ...
                        break;
                    case SQRT:
                        if (v1 <= 0f) v2 = 0f; else v2 = (float) Math.sqrt(v1);
                        break;
                    case ABS:
                        v2 = (float) Math.abs(v1);
                        break;
                    case MINIMUM:
                        if (v1 < value) v2 = (float) value; else v2 = v1;
                        break;
                    case MAXIMUM:
                        if (v1 > value) v2 = (float) value; else v2 = v1;
                        break;
    ...
    void filter3x3(int type, int[] kernel) {
        ...
        for (int y = roiY; y < yEnd; y++) {
            ...
            switch(type) {
                case BLUR_MORE:
                    for (int x = roiX; x < xEnd; x++, p++) {
                        ...
                        v8 = v9;
                        v9 = pixels2[p9];
                        pixels[p] = (v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9) * 0.11111111f;
                    }
                    break;
                case FIND_EDGES:
                    for (int x = roiX; x < xEnd; x++, p++) {
            ...
            if (y % inc == 0) showProgress((double) (y - roiY) / roiHeight);
        }
        showProgress(1.0);
    }
	...
	*/
    public void rotate(double angle) {
        ...
        if (interpolationMethod == BICUBIC) ip2 = new FloatProcessor(getWidth(), getHeight(), pixels2, null);
	...
	*/
        ...
        if (interpolationMethod == BICUBIC) {
            ...
            for (int y = ymin; y <= ymax; y++) {
                ...
                for (int x = xmin; x <= xmax; x++) {
                    xs = (x - xCenter) / xScale + xCenter;
                    xsi = (int) xs;
                    if (checkCoordinates && ((xsi < xmin) || (xsi > xmax) || (ysi < ymin) || (ysi > ymax))) pixels[index1++] = (float) getMin(); else {
                        if (interpolationMethod == BILINEAR) {
                            if (xs < 0.0) xs = 0.0;
                            if (xs >= xlimit) xs = xlimit2;
                            pixels[index1++] = (float) getInterpolatedPixel(xs, ys, pixels2);
                        } else pixels[index1++] = pixels2[index2 + xsi];
                    }
                }
                if (y % 30 == 0) showProgress((double) (y - ymin) / height);
            }
        ...
        if (interpolationMethod == BICUBIC) {
            ...
            for (int y = 0; y <= dstHeight - 1; y++) {
                ...
                for (int x = 0; x <= dstWidth - 1; x++) {
                    ...
                    } else pixels2[index2++] = pixels[index1 + (int) xs];
                }
                if (y % 30 == 0) showProgress((double) y / dstHeight);
            }
        }
    ...
    FloatProcessor downsize(int dstWidth, int dstHeight) {
        ...
        if (dstWidth < rWidth) {
            ...
            ip2.setRoi(0, r.y, dstWidth, rHeight);
        }
        if (dstHeight < rHeight) ip2 = ip2.downsize1D(dstWidth, dstHeight);
        if (ip2.getWidth() != dstWidth || ip2.getHeight() != dstHeight) ip2 = (FloatProcessor) ip2.resize(dstWidth, dstHeight);
        return ip2;
    }
    ...
    private FloatProcessor downsize1D(int dstWidth, int dstHeight) {
        ...
        DownsizeTable dt = xDirection ? new DownsizeTable(getWidth(), r.x, r.width, dstWidth, interpolationMethod) : new DownsizeTable(getHeight(), r.y, r.height, dstHeight, interpolationMethod);
        ...
        for (int srcLine = srcLine0, dstLine = 0; dstLine < dstLines; srcLine++, dstLine++) {
            ...
            for (int srcPoint = dt.srcStart, p = srcPoint * srcPointInc + srcLine * srcLineInc; srcPoint <= dt.srcEnd; srcPoint++, p += srcPointInc) {
                float v = pixels[p];
                for (int i = 0; i < dt.kernelSize; i++, tablePointer++) pixels2[dstLineOffset + dt.indices[tablePointer] * dstPointInc] += v * dt.weights[tablePointer];
            }
        }
        return ip2;
    }
    ...
    public double getBicubicInterpolatedPixel(double x0, double y0, ImageProcessor ip2) {
        ...
        if (u0 <= 0 || u0 >= width - 2 || v0 <= 0 || v0 >= height - 2) return ip2.getBilinearInterpolatedPixel(x0, y0);
        double q = 0;
        for (int j = 0; j <= 3; j++) {
            int v = v0 - 1 + j;
            double p = 0;
    ...
    public void setColor(Color color) {
        ...
        } else if (bestIndex == 0 && getMin() > 0.0 && (color.getRGB() & 0xffffff) == 0) fillColor = 0f; else fillColor = (float) (getMin() + (getMax() - getMin()) * (bestIndex / 255.0));
    }
	...
	*/
    public void setPixels(int channelNumber, FloatProcessor fp) {
        if (fp.getPixels() != getPixels()) setPixels(fp.getPixels());
