...
public class JGraphAdapterModel extends DefaultGraphModel {
	 ...
	 * @param attributes
	 */
    public JGraphAdapterModel(List roots, AttributeMap attributes, JGraphAdapterBackend backend) {
    ...
    public void edit(Map attributes, ConnectionSet cs, ParentMap pm, UndoableEdit[] edits) {
        if ((attributes != null && !attributes.isEmpty()) || (cs != null && !cs.isEmpty())) {
            try {
    ...
    protected Object cloneUserObject(Object userObject) {
        if (userObject instanceof JGraphBusinessObject) return ((JGraphBusinessObject) userObject).clone();
    ...
    public void insert(Object[] cells, Map attributes, ConnectionSet cs, ParentMap pm, UndoableEdit[] edits) {
    ...
    public void processInsert(Object[] cells, Map attributes, ConnectionSet cs, ParentMap pm, boolean validate) throws Exception {
        if (cells != null || attributes != null || cs != null) {
            if (cells != null) {
                for (int i = 0; i < cells.length; i++) {
                    if (!isPort(cells[i])) {
                        if (!isEdge(cells[i])) {
                            ...
                            if (!validate) putMapping(getValue(cells[i]), cells[i]);
                        }
                    }
                }
                for (int i = 0; i < cells.length; i++) {
                    if (!isPort(cells[i])) {
                        if (isEdge(cells[i]) && cs != null) {
                            fireEdgeAdded(cells[i], cs.getPort(cells[i], false), cs.getPort(cells[i], true), validate);
                            if (!validate) putMapping(getValue(cells[i]), cells[i]);
                        }
                    }
                }
    ...
    protected void processRemove(Object[] cells, boolean validate) throws Exception {
        if (cells != null) {
            for (int i = 0; i < cells.length; i++) {
                if (!isPort(cells[i])) {
                    fireCellRemoved(cells[i], validate);
                    if (!validate) removeMapping(getValue(cells[i]));
                }
            }
        }
    }
	 ...
	 * @throws Exception
	 */
    protected void processConnectionSet(ConnectionSet cs, boolean validate) throws Exception {
        if (cs != null) {
	 ...
	 * @throws Exception
	 */
    protected void processParentMap(ParentMap pm, boolean validate) throws Exception {
        if (pm != null) {
	 ...
	 * @throws Exception
	 */
    protected void processNestedAttributes(Map nested, boolean validate) throws Exception {
        if (nested != null) {
    ...
    public class BusinessModelEdit extends GraphModelEdit {
		 ...
		 * @param parentMap
		 */
        public BusinessModelEdit(Object[] inserted, Object[] removed, Map attributes, ConnectionSet connectionSet, ParentMap parentMap) {
        ...
        public void execute() {
            ...
            try {
                if (!inProgress) {
                    ...
                    processInsert(remove, previousAttributes, previousConnectionSet, previousParentMap, false);
                    fireCommit();
                }
            } catch (Exception e) {
                e.printStackTrace();
	 ...
	 * @throws Exception
	 */
    public void fireEdgeAdded(Object edge, Object source, Object target, boolean validate) throws Exception {
