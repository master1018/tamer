...
public class SingleTurnoutSignalHeadXml extends jmri.managers.configurexml.AbstractNamedBeanManagerConfigXML {
    ...
    public boolean load(Element element) {
        ...
        SignalHead h;
        if (uname == null) h = new SingleTurnoutSignalHead(sys, lit, on, off); else h = new SingleTurnoutSignalHead(sys, uname, lit, on, off);
    ...
    private int loadAppearance(List<Element> l, String state) {
        for (int i = 0; i < l.size(); i++) {
            if (l.get(i).getAttribute("defines").getValue().equals(state)) return getIntFromColour(l.get(i).getText());
        }
        return 0x00;
    }
    ...
    private int getIntFromColour(String colour) {
        ...
        if (c.equals("red")) return SignalHead.RED; else if (c.equals("yellow")) return SignalHead.YELLOW; else if (c.equals("green")) return SignalHead.GREEN; else if (c.equals("lunar")) return SignalHead.LUNAR; else if (c.equals("dark")) return SignalHead.DARK; else if (c.equals("flashred")) return SignalHead.FLASHRED; else if (c.equals("flashyellow")) return SignalHead.FLASHYELLOW; else if (c.equals("flashgreen")) return SignalHead.FLASHGREEN; else if (c.equals("flashlunar")) return SignalHead.FLASHLUNAR; else log.warn("Unexpected appearance: " + colour);
