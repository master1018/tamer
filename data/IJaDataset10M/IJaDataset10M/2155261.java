...
public class GraphCanvas extends Canvas implements IGraphCanvas {
    ...
    public GraphCanvas(Composite parent, int style) {
        ...
        SelectionListener selListener = new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (e.widget.equals(zoomInItem)) zoomIn(); else if (e.widget.equals(zoomOutItem)) zoomOut(); else if (e.widget.equals(scaleToFitItem)) setScaleToFit(scaleToFitItem.getSelection()); else if (e.widget.equals(zoomReset)) zoomReset();
            }
        };
        ...
        SelectionListener barListener = new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (e.widget.equals(horizontalScrollBar)) xoffset = getHorizontalBar().getSelection(); else if (e.widget.equals(verticalScrollBar)) yoffset = getVerticalBar().getSelection();
                redraw();
            }
        };
    ...
    private void updateScrollers() {
        ...
        if (getGraphWidth() * scale > getClientArea().width) getHorizontalBar().setVisible(true); else getHorizontalBar().setVisible(false);
        if (getGraphHeight() * scale > getClientArea().height) getVerticalBar().setVisible(true); else getVerticalBar().setVisible(false);
    }
    ...
    public String getNameOfCurrentSelectedNode() {
        if (gp.getSelectedNode() != null) return gp.getSelectedNode().getName();
        return null;
    }
    ...
    class ZoomRunnable implements Runnable {
        ...
        public void run() {
            if (isDisposed() || scaleToFit) return;
            long newTime = System.currentTimeMillis();
            if (newTime < endTime) getDisplay().timerExec(33, this); else newTime = endTime;
    ...
    private Transform buildTransform(float xoffset, float yoffset, float scale) {
        ...
        if (getGraphWidth() * scale < getClientArea().width) alignedXOffset += (getClientArea().width - getGraphWidth() * scale) / 2;
        if (getGraphHeight() * scale < getClientArea().height) alignedYOffset -= (getClientArea().height - getGraphHeight() * scale) / 2; else {
