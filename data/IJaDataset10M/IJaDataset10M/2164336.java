...
public class SimpleSegment extends RootSegment {
    ...
    public ResourceInvoker matchSimple(HttpRequest request, String path, int start) {
        ...
        if (start + segment.length() == path.length()) {
            ...
            if (invoker == null) throw new NotFoundException("Could not find resource for relative : " + path + " of full path: " + request.getUri().getRequestUri());
            uriInfo.pushMatchedURI(path, Encode.decode(path));
            return invoker;
        } else {
            try {
                return matchChildren(request, path, start + segment.length() + 1);
            } catch (Failure e) {
                if (locator != null) {
