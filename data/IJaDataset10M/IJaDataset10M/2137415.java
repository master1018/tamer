...
public class BaseRemoteStreamTest extends TestCase {
    ...
    protected List<AccumulateRemoteStreamMonitor<?>> _monitors = new ArrayList<AccumulateRemoteStreamMonitor<?>>();

    protected void checkMonitors(int numExpectedMonitors, boolean expectClean) throws Exception {
    ...
    public static int cycleRead(InputStream in, byte[] tmp, int iteration) throws IOException {
        switch(iteration % 3) {
            case 0:
                int b = in.read();
                if (b >= 0) {
    ...
    public static void cycleWrite(OutputStream out, byte[] tmp, int numBytes, int iteration) throws IOException {
        switch(iteration % 3) {
            case 0:
    ...
    public static class AccumulateRemoteStreamMonitor<S extends RemoteStreamServer> implements RemoteStreamMonitor<S> {
        ...
        public String toString() {
            return "closed " + _closed + "; clean " + _closedClean + "; numWireBytes " + _numWireBytes + " in numPackets " + _numWirePackets + " (skipped " + _numSkippedWireBytes + "; reattempted " + _numReattempts + "); actual local bytes " + (_numLocalBytes + _numSkippedLocalBytes);
        }
    }
}
