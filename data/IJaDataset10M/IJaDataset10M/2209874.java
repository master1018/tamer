...
public class NthComponentFinderTest extends AbstractComponentDriverTest<ComponentDriver<? extends Component>> {
    ...
    @Test
    public void reportsWhenThereAreNoMatchingComponents() {
        JPanel panel = new JPanel();
        panel.setName("panel");
        view(panel);
        prober.setTimeout(100);
        try {
            NthComponentFinder<JPanel> finder = new NthComponentFinder<JPanel>(new RecursiveComponentFinder<JPanel>(JPanel.class, ComponentDriver.named("--no-such-panel--"), frameDriver.component()), 0);
            ...
            assertThat(e.getMessage(), containsInOrder("Tried to look for", "exactly 1", "JLabel", "in the <0> component from those matching JPanel", "with name \"--no-such-panel--\"", "in all top level windows", "and check that it is showing on screen", "but", "all top level windows", "contained 1 JFrame", "with name \"componentViewer\"", "contained 0 JPanel", "with name \"--no-such-panel--\""));
        }
    }
}
