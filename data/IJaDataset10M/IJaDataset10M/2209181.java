...
public class ContextRetrieveWizardPage extends WizardPage {
    ...
    public void createControl(Composite parent) {
        ...
        contextTable.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                if (contextTable.getSelectionIndex() > -1) {
                    selectedContextAttachment = (RepositoryAttachment) contextTable.getItem(contextTable.getSelectionIndex()).getData();
        ...
        if (connector.getAttachmentHandler() != null) {
            contextAttachments = new ArrayList<RepositoryAttachment>(connector.getAttachmentHandler().getContextAttachments(repository, task));
        }
        Collections.sort(contextAttachments, new Comparator<RepositoryAttachment>() {
            ...
            public int compare(RepositoryAttachment attachment1, RepositoryAttachment attachment2) {
                RepositoryTaskData data = TasksUiPlugin.getTaskDataManager().getNewTaskData(task.getRepositoryUrl(), task.getTaskId());
                ...
                if (factory != null) {
                    created1 = factory.getDateForAttributeType(RepositoryTaskAttribute.ATTACHMENT_DATE, attachment1.getDateCreated());
                    created2 = factory.getDateForAttributeType(RepositoryTaskAttribute.ATTACHMENT_DATE, attachment2.getDateCreated());
                }
