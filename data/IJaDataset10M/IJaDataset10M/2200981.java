...
public class R3 implements java.io.Serializable {
    ...
    public R3(double[] arrVals) {
        ...
        if (arrVals.length >= 1) this.x1 = arrVals[0];
        if (arrVals.length >= 2) this.x2 = arrVals[1];
        if (arrVals.length >= 3) this.x3 = arrVals[2];
    }
    ...
    public R3(String strTokens) throws NumberFormatException, IllegalArgumentException {
        ...
        if (tokArgs.countTokens() < 3) throw new IllegalArgumentException("R3(strTokens) - wrong number of token strings: " + strTokens);
        int i;
        for (i = 0; i < 3; i++) {
