...
final class LibraryTree extends JTree implements MouseObserver {
    ...
    private void addNode(LibraryTreeNode parent, LibraryTreeNode child, boolean expand) {
        if (parent.getIndex(child) != -1) {
            if (expand) expandPath(new TreePath(parent.getPath()));
            return;
        }
        ...
        if (!(child.getDirectoryHolder() instanceof SharedFilesDirectoryHolder)) {
            insert = children;
            if (insert != 0 && child == speciallySharedFilesNode && parent.getChildAt(children - 1) == torrentsMetaFilesNode) insert--;
        } else {
            for (; insert < children; insert++) {
                LibraryTreeNode current = (LibraryTreeNode) parent.getChildAt(insert);
                File f = current.getFile();
                if (current == torrentsMetaFilesNode || f == null || StringUtils.compareFullPrimary(f.getName(), child.getFile().getName()) >= 0) break;
            }
        }
        TREE_MODEL.insertNodeInto(child, parent, insert);
        if (expand || (parent == sharedFilesNode && !isExpanded(new TreePath(sharedFilesNode.getPath())))) expandPath(new TreePath(parent.getPath()));
    }
    ...
    private void removeNode(LibraryTreeNode parent, LibraryTreeNode child) {
        ...
        if (parent.getIndex(child) == -1) return;
    ...
    private void addPerMediaTypeDirectories() {
        for (Iterator i = NamedMediaType.getAllNamedMediaTypes().iterator(); i.hasNext(); ) {
            ...
            if (nm.getMediaType().getMimeType().equals(MediaType.SCHEMA_ANY_TYPE)) continue;
    ...
    public void handleFileManagerEvent(final FileManagerEvent evt) {
        switch(evt.getType()) {
            ...
            case REMOVE_FILE:
                if (ssfdh.isEmpty()) {
                    if (ssfdh == getSelectedDirectoryHolder()) setSelectionPath(new TreePath(savedFilesNode.getPath()));
    ...
    void removeFolder(File folder, LibraryTreeNode treeNode) {
        ...
        if (node == null) return;
        if (getSelectedNode() == node) setSelectionPath(new TreePath(treeNode.getPath()));
    ...
    LibraryTreeNode getNodeForFolder(File folder, LibraryTreeNode parent) {
        ...
        for (int i = children - 1; i >= 0; i--) {
            ...
            if (childFile != null) {
                if (childFile.equals(folder)) return child;
                if (child.isAncestorOf(folder)) return getNodeForFolder(folder, child);
            }
        }
        return null;
    }
    ...
    void addPlayListEntries() {
        if (incompleteDirectoryIsSelected() || !GUIMediator.isPlaylistVisible()) return;
        final DirectoryHolder dh = getSelectedDirectoryHolder();
        if (dh == null) return;
        if (PlaylistMediator.getInstance() == null) return;
    ...
    private boolean canBeUnshared(LibraryTreeNode node) {
        ...
        if (node == speciallySharedFilesNode) return false;
        if (node == incompleteFilesNode) return false;
        if (node == sharedFilesNode) return false;
        if (node == torrentsMetaFilesNode) return false;
        if (node.getParent() == null) return false;
        if (node.getParent() == sharedFilesNode) return true;
    ...
    private boolean canBeShared(LibraryTreeNode node) {
        if (node == null || node == incompleteFilesNode || node == torrentsMetaFilesNode) return false;
        File dir = node.getDirectoryHolder().getDirectory();
        if (dir == null || GuiCoreMediator.getFileManager().isFolderShared(dir) || GuiCoreMediator.getFileManager().isStoreDirectory(dir)) return false;
        return true;
    }

    public DirectoryHolder getSelectedDirectoryHolder() {
        TreePath path = getSelectionPath();
        if (path != null) return ((LibraryTreeNode) path.getLastPathComponent()).getDirectoryHolder();
        return null;
    }
    ...
    public DirectoryHolder getHolderForPoint(Point p) {
        ...
        if (path != null) {
            ...
            if (node != null) return node.getDirectoryHolder();
        }
        return null;
    }
    ...
    File[] getSharedDirectories() {
        ...
        for (int i = 0; i < length - 1; i++) {
            ...
            if (node != speciallySharedFilesNode && node != torrentsMetaFilesNode) newFiles.add(node.getDirectoryHolder().getDirectory());
        }
    ...
    void clear() {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (node == getSelectedNode()) selected = true;
            if (node != torrentsMetaFilesNode) sharedFilesNode.remove(i);
        }
        if (selected) setSelectionPath(new TreePath(sharedFilesNode));
        selected = false;
        TREE_MODEL.reload();
    }
    ...
    void unshareLibraryFolder() {
        ...
        if (incompleteDirectoryIsSelected()) {
            ...
            if (response != DialogOption.YES) return;
    ...
    private class LibraryTreeSelectionListener implements TreeSelectionListener {
        ...
        public void valueChanged(TreeSelectionEvent e) {
            ...
            if (node == null) return;
            if (node == sharedFilesNode) LibraryMediator.showSharedFiles(); else LibraryMediator.updateTableFiles(node.getDirectoryHolder());
        }
    }
        ...
        if (node != null && node != incompleteFilesNode && node != sharedFilesNode) {
            ...
            if (files != null && files.length > 0) {
                for (int i = 0; i < files.length; i++) {
                    if (GUIMediator.isPlaylistVisible() && PlaylistMediator.isPlayableFile(files[i])) enqueueable = true;
                }
            }
        }
        return enqueueable;
    ...
    private void makePopupMenu() {
        ...
        DIRECTORY_POPUP.addSeparator();
        DIRECTORY_POPUP.add(new JMenuItem(new ConfigureOptionsAction(OptionsConstructor.SHARED_KEY, I18n.tr("Configure Sharing Options"), I18n.tr("You can configure the folders you share in FrostWire\'s Options."))));
    }
    ...
    private void makeButtonRow() {
        if (hasExploreAction()) {
            BUTTON_ROW = new ButtonRow(new Action[] { refreshAction, exploreAction }, ButtonRow.X_AXIS, ButtonRow.NO_GLUE);
        } else {
            BUTTON_ROW = new ButtonRow(new Action[] { refreshAction }, ButtonRow.X_AXIS, ButtonRow.NO_GLUE);
        }
    }
