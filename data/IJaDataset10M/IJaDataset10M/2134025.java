...
public class CAstDumper {
    ...
    private Collection<CAstEntity> dumpScopedEntities(CAstEntity entity, int indent, StringBuilder buf) {
        ...
        for (Entry<CAstNode, Collection<CAstEntity>> e : entity.getAllScopedEntities().entrySet()) for (CAstEntity scopedEntity : e.getValue()) {
    ...
    private int getNonTrivialChildCount(CAstNode node) {
        int cnt = 0;
        for (int i = 0; i < node.getChildCount(); ++i) if (!isTrivial(node.getChild(i))) ++cnt;
        return cnt;
    }
    ...
    private void dump(CAstNode node, int indent, StringBuilder buf, CAstControlFlowMap cfg) {
        ...
        if (NORMALISE && node.getKind() == CAstNode.BLOCK_EXPR && getNonTrivialChildCount(node) == 1) {
            for (int i = 0; i < node.getChildCount(); ++i) if (!isTrivial(node.getChild(i))) dump(node.getChild(i), indent, buf, cfg);
        } else {
            buf.append(indent(indent) + labeller.addNode(node) + ": ");
            if (node.getKind() == CAstNode.CONSTANT) {
                if (node.getValue() == null) buf.append("null"); else if (node.getValue() instanceof Integer) buf.append(node.getValue() + ""); else buf.append("\"" + node.getValue() + "\"");
            ...
            if (!labels.isEmpty()) {
                ...
                for (Object label : labels) {
                    ...
                    if (label instanceof CAstNode) buf.append("CAstNode@" + labeller.addNode((CAstNode) label) + ": "); else buf.append(label + ": ");
            ...
            for (int i = 0; i < node.getChildCount(); ++i) {
                ...
                if (NORMALISE && node.getKind() == CAstNode.BLOCK_STMT && child != null && child.getKind() == CAstNode.EMPTY) continue;
