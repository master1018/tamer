...
public class FileListTableModel extends AbstractTableModel {
    ...
    public void addEntry(final int index, final FileListEntry entry) {
        synchronized (list) {
            ...
            } else fireTableRowsInserted(index, index);
        }
    }
    ...
    private FileListEntry setContent(String value, boolean firstOnly, boolean deduceUnknownTypes) {
        ...
        for (int i = 0; i < value.length(); i++) {
            ...
            if (!escaped && (c == '\\')) {
                ...
                thisEntry.add(sb.toString());
                sb = new StringBuilder();
                if (firstOnly) return decodeEntry(thisEntry, deduceUnknownTypes); else {
        ...
        if (thisEntry.size() > 0) {
            if (firstOnly) return decodeEntry(thisEntry, deduceUnknownTypes); else newList.add(decodeEntry(thisEntry, deduceUnknownTypes));
        }
        synchronized (list) {
            list.clear();
            list.addAll(newList);
    ...
    private String getElementIfAvailable(ArrayList<String> contents, int index) {
        if (index < contents.size()) return contents.get(index); else return "";
    }
