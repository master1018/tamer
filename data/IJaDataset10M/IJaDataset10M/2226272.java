...
public class TradNaiveBayesClassifier implements JointClassifier<CharSequence>, ObjectHandler<Classified<CharSequence>>, Serializable, Compilable {
    ...
    public JointClassification classify(CharSequence in) {
        ...
        for (String token : tokenizer) {
            ...
            for (int i = 0; i < mCategories.length; ++i) logps[i] += com.aliasi.util.Math.log2(probTokenByIndexArray(i, tokenCounts));
        }
        if ((!Double.isNaN(mLengthNorm)) && (tokenCount > 0)) {
            for (int i = 0; i < logps.length; ++i) logps[i] *= mLengthNorm / tokenCount;
        }
        for (int i = 0; i < logps.length; ++i) logps[i] += com.aliasi.util.Math.log2(probCatByIndex(i));
    ...
    public void trainConditional(CharSequence cSeq, ConditionalClassification classification, double countMultiplier, double minCount) {
        ...
        int numCats = 0;
        while (numCats < classification.size() && classification.conditionalProbability(numCats) * countMultiplier >= minCount) ++numCats;
    ...
    public void train(CharSequence cSeq, Classification classification, double count) {
        ...
        for (String token : tokenizer) {
            double[] tokenCounts = mTokenToCountsMap.get(token);
            if (lengthNormCount < 0 && ((tokenCounts == null) || (tokenCounts[catIndex] < -lengthNormCount))) {
                mCaseCounts[catIndex] -= count;
                mTotalCaseCount -= count;
    ...
    public double log2CaseProb(CharSequence input) {
        ...
        for (int rank = 0; rank < c.size(); ++rank) {
            ...
            if (jointLog2P > maxJointLog2P) maxJointLog2P = jointLog2P;
        }
        double sum = 0.0;
        for (int rank = 0; rank < c.size(); ++rank) sum += Math.pow(2.0, c.jointLog2Probability(rank) - maxJointLog2P);
    ...
    private double probTokenByIndexArray(int catIndex, double[] tokenCounts) {
        ...
        return (tokenCatCount + mTokenInCategoryPrior) / (totalCatCount + mTokenToCountsMap.size() * mTokenInCategoryPrior);
    }

    private double probCatByIndex(int catIndex) {
        double caseCountCat = mCaseCounts[catIndex];
        return (caseCountCat + mCategoryPrior) / (mTotalCaseCount + mCategories.length * mCategoryPrior);
    }
    ...
    private ObjectToCounterMap<String> tokenCountMap(CharSequence cSeq) {
        ...
        char[] cs = Strings.toCharArray(cSeq);
        Tokenizer tokenizer = mTokenizerFactory.tokenizer(cs, 0, cs.length);
        for (String token : tokenizer) tokenCountMap.increment(token);
    ...
    private double lengthMultiplier(ObjectToCounterMap<String> tokenCountMap) {
        ...
        int length = 0;
        for (Counter counter : tokenCountMap.values()) length += counter.intValue();
        return length != 0.0 ? mLengthNorm / length : 1.0;
    }

    private int getIndex(String cat) {
        int catIndex = java.util.Arrays.binarySearch(mCategories, cat);
        if (catIndex < 0) {
            String msg = "Unknown category.  Require category in category set." + " Found category=" + cat + " category set=" + mCategorySet;
            throw new IllegalArgumentException(msg);
        }
        return catIndex;
    }
    ...
    public static TradNaiveBayesClassifier emTrain(TradNaiveBayesClassifier initialClassifier, Factory<TradNaiveBayesClassifier> classifierFactory, Corpus<ObjectHandler<Classified<CharSequence>>> labeledData, Corpus<ObjectHandler<CharSequence>> unlabeledData, double minTokenCount, int maxEpochs, double minImprovement, Reporter reporter) throws IOException {
        ...
        Iterator<TradNaiveBayesClassifier> it = emIterator(initialClassifier, classifierFactory, labeledData, unlabeledData, minTokenCount);
    ...
    static class CaseProbAccumulator implements ObjectHandler<CharSequence> {
    ...
    static class EmIterator extends Iterators.Buffered<TradNaiveBayesClassifier> {
        ...
        void trainUnsup(final Corpus<ObjectHandler<CharSequence>> unlabeledData, final TradNaiveBayesClassifier classifier) {
            try {
        ...
        void compile(TradNaiveBayesClassifier classifier) {
            try {
                @SuppressWarnings("unchecked") JointClassifier<CharSequence> lastClassifier = (JointClassifier<CharSequence>) AbstractExternalizable.compile(classifier);
    ...
    static class Serializer extends AbstractExternalizable {
        ...
        public Object read(ObjectInput in) throws ClassNotFoundException, IOException {
            ...
            for (int i = 0; i < numCats; ++i) categories[i] = in.readUTF();
            ...
            Map<String, double[]> tokenToCountsMap = new HashMap<String, double[]>((tokenToCountsMapSize * 3) / 2);
            for (int k = 0; k < tokenToCountsMapSize; ++k) {
                String key = in.readUTF();
                double[] vals = new double[categories.length];
                for (int i = 0; i < categories.length; ++i) vals[i] = in.readDouble();
                tokenToCountsMap.put(key, vals);
            }
            double[] totalCountsPerCategory = new double[categories.length];
            for (int i = 0; i < categories.length; ++i) totalCountsPerCategory[i] = in.readDouble();
            double[] caseCounts = new double[categories.length];
            for (int i = 0; i < categories.length; ++i) caseCounts[i] = in.readDouble();
        ...
        public void writeExternal(ObjectOutput objOut) throws IOException {
            ...
            for (String category : mClassifier.mCategories) objOut.writeUTF(category);
            ...
            for (Map.Entry<String, double[]> entry : mClassifier.mTokenToCountsMap.entrySet()) {
                ...
                for (int i = 0; i < mClassifier.mCategories.length; ++i) objOut.writeDouble(vals[i]);
            }
            for (int i = 0; i < mClassifier.mCategories.length; ++i) objOut.writeDouble(mClassifier.mTotalCountsPerCategory[i]);
            for (int i = 0; i < mClassifier.mCategories.length; ++i) objOut.writeDouble(mClassifier.mCaseCounts[i]);
    ...
    static class Compiler extends AbstractExternalizable {
        ...
        public void writeExternal(ObjectOutput objOut) throws IOException {
            objOut.writeInt(mClassifier.mCategories.length);
            for (int i = 0; i < mClassifier.mCategories.length; ++i) objOut.writeUTF(mClassifier.mCategories[i]);
            ...
            for (Map.Entry<String, double[]> entry : mClassifier.mTokenToCountsMap.entrySet()) {
                ...
                for (int i = 0; i < mClassifier.mCategories.length; ++i) {
                    ...
                    if (log2Prob > 0.0) {
                        String msg = "key=" + entry.getKey() + " i=" + i + " log2Prob=" + log2Prob + " prob=" + mClassifier.probTokenByIndexArray(i, tokenCounts) + " token counts[" + i + "]=" + tokenCounts[i] + " totalCatCount=" + mClassifier.mTotalCountsPerCategory[i] + " mTokenToCountsMap.size()=" + mClassifier.mTokenToCountsMap.size();
                        throw new IllegalArgumentException(msg);
                    }
                    objOut.writeDouble(log2Prob);
                }
            }
            for (int i = 0; i < mClassifier.mCategories.length; ++i) objOut.writeDouble(com.aliasi.util.Math.log2(mClassifier.probCatByIndex(i)));
            objOut.writeDouble(mClassifier.mLengthNorm);
        }
        ...
        public Object read(ObjectInput in) throws ClassNotFoundException, IOException {
            ...
            for (int i = 0; i < numCategories; ++i) categories[i] = in.readUTF();
            TokenizerFactory tokenizerFactory = (TokenizerFactory) in.readObject();
            int size = in.readInt();
            Map<String, double[]> tokenToLog2ProbsInCats = new HashMap<String, double[]>((size * 3) / 2);
            for (int k = 0; k < size; ++k) {
                String token = in.readUTF();
                double[] log2ProbsInCats = new double[numCategories];
                for (int i = 0; i < numCategories; ++i) log2ProbsInCats[i] = in.readDouble();
                tokenToLog2ProbsInCats.put(token, log2ProbsInCats);
            }
            double[] log2CatProbs = new double[numCategories];
            for (int i = 0; i < numCategories; ++i) log2CatProbs[i] = in.readDouble();
            double lengthNorm = in.readDouble();
    ...
    private static class CompiledBinaryTradNaiveBayesClassifier implements ConditionalClassifier<CharSequence> {
        ...
        public ConditionalClassification classify(CharSequence in) {
            ...
            if ((!Double.isNaN(mLengthNorm)) && (tokenCount > 0)) logDiff *= mLengthNorm / tokenCount;
            logDiff += mLog2CatProbDiff;
            double expProd = Math.exp(logDiff);
            double p0 = expProd / (1.0 + expProd);
            double p1 = 1.0 - p0;
            return (p0 > p1) ? new ConditionalClassification(mCats01, new double[] { p0, p1 }) : new ConditionalClassification(mCats10, new double[] { p1, p0 });
        }
    }
