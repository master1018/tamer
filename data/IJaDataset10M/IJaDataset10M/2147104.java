...
public class OMAPresContentAppUsage extends AppUsage {
	 ...
	 */
    public OMAPresContentAppUsage(Validator schemaValidator, Set<String> encodingsAllowed, int maxDataSize, Set<String> mimetypesAllowed, String presRulesAUID, String presRulesDocumentName) {
        super(ID, DEFAULT_DOC_NAMESPACE, MIMETYPE, schemaValidator, new OMAPresContentAuthorizationPolicy(presRulesAUID, presRulesDocumentName));
        this.encodingsAllowed = encodingsAllowed;
        this.maxDataSize = maxDataSize;
        if (maxDataSize != 0) {
            this.MAX_DATA_SIZE_ERROR_PHRASE = new StringBuilder(MAX_DATA_SIZE_ERROR_PHRASE_PREFIX).append(Integer.toString(maxDataSize)).append(MAX_DATA_SIZE_ERROR_PHRASE_SUFFIX).toString();
        } else {
    ...
    public void checkConstraintsOnPut(Document document, String xcapRoot, DocumentSelector documentSelector, AppUsageDataSource dataSource) throws UniquenessFailureConflictException, InternalServerErrorException, ConstraintFailureConflictException {
        boolean mimytypeFound = mimetypesAllowed == null || mimetypesAllowed.isEmpty();
        boolean encodingFound = encodingsAllowed == null || encodingsAllowed.isEmpty();
        ...
        if (!mimytypeFound || !encodingFound || !dataFound) {
            ...
            for (int i = 0; i < contentChildNodeList.getLength(); i++) {
                ...
                if (!mimytypeFound && DomUtils.isElementNamed(contentChildNode, MIMETYPE_ELEMENT_NAME)) {
                    Element mimeTypeElement = (Element) contentChildNode;
                    if (mimeTypeElement.getTextContent() == null || !mimetypesAllowed.contains(mimeTypeElement.getTextContent())) {
                ...
                } else if (!encodingFound && DomUtils.isElementNamed(contentChildNode, ENCODING_ELEMENT_NAME)) {
                    Element encodingElement = (Element) contentChildNode;
                    if (encodingElement.getTextContent() == null || !encodingsAllowed.contains(encodingElement.getTextContent())) {
                ...
                } else if (!dataFound && DomUtils.isElementNamed(contentChildNode, DATA_ELEMENT_NAME)) {
                    ...
                    if (dataElement.getTextContent() != null) {
                        ...
                        if (dataSize > maxDataSize) {
                            throw new ConstraintFailureConflictException(MAX_DATA_SIZE_ERROR_PHRASE);
                        }
                    }
                    dataFound = true;
