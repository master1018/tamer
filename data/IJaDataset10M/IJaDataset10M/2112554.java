...
public abstract class AtlasCreator {
    ...
    public void initializeMap(MapInterface map, TileProvider mapTileProvider) {
        ...
        if (mapTileProvider == null) throw new NullPointerException();
        ...
        if (!(t instanceof AtlasThread)) throw new RuntimeException("Calling thread must be AtlasThread!");
    ...
    protected void performTest_AtlasTileFormat(EnumSet<TileImageFormat> allowedFormats) throws AtlasTestException {
        for (LayerInterface layer : atlas) {
            for (MapInterface map : layer) {
                ...
                if (!allowedFormats.contains(parameters.getFormat())) throw new AtlasTestException("Selected custom tile format not supported - only the following format(s) are supported: " + allowedFormats, map);
            }
        }
    }
    ...
    protected void performTest_MaxMapZoom(int maxZoom) throws AtlasTestException {
        for (LayerInterface layer : atlas) {
            for (MapInterface map : layer) {
                if (map.getZoom() > maxZoom) throw new AtlasTestException("Maximum zoom is " + maxZoom + " for this atlas format", map);
            }
        }
    }
}
