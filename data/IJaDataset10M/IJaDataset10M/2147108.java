...
public class Touch {
    ...
    public static void scrollTo(TextView widget, Layout layout, int x, int y) {
        ...
        int bottom = layout.getLineForVertical(y + widget.getHeight() - padding);
    ...
    public static int getMaxScrollX(TextView widget, Layout layout, int y) {
        int top = layout.getLineForVertical(y);
        int bottom = layout.getLineForVertical(y + widget.getHeight() - widget.getTotalPaddingTop() - widget.getTotalPaddingBottom());
        int left = Integer.MAX_VALUE;
        int right = 0;
        for (int i = top; i <= bottom; i++) {
            left = (int) Math.min(left, layout.getLineLeft(i));
