...
public class OutputActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ((ImageButton) findViewById(R.id.diskButton)).setOnClickListener(new View.OnClickListener() {

            public void onClick(View v) {
                final CharSequence[] items = { "Save as File..", "Send as Text..", "Attach as .txt ..", "Attach as .xml .." };
                AlertDialog.Builder builder = new AlertDialog.Builder(myOutputActivityInstance);
                ...
                builder.setItems(items, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int item) {
                        if (item == 0) {
                            ...
                            intent.putExtra(Intent.EXTRA_TITLE, "Save as text");
                            ...
                            if (item == 1) {
                                ...
                                String extStorageDirectory = Environment.getExternalStorageDirectory().toString() + "/OOBD/";
                                File file = new File(extStorageDirectory, fileName);
                                saveOutput(file);
                                if (!file.exists() || !file.canRead()) {
                                    Toast.makeText(myOutputActivityInstance, "Attachment Error", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                intent.putExtra(Intent.EXTRA_TEXT, Html.fromHtml("<html><body>With best regards from <a href='http://oobd.org'>OOBD</a></body></html>"));
                                Uri uri = Uri.parse("file://" + file);
                                intent.putExtra(Intent.EXTRA_STREAM, uri);
                            }
                            startActivity(Intent.createChooser(intent, "Forward Output Text..."));
                        }
                    }
                });
    ...
    public synchronized void onActivityResult(final int requestCode, int resultCode, final Intent data) {
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (file.exists()) {
                AlertDialog alertDialog = new AlertDialog.Builder(myOutputActivityInstance).create();
