...
public class StoredQueryRequestSoapValidator extends ValidationEngine {
    ...
    public void run() {
        ...
        if (xds_version == XBaseTransaction.xds_a) {
            if (isSOAP12) newError("A SOAP 1.1 endpoint (XDS.a SQ) was used but message was in SOAP 1.2 format. SOAP Namespace is ").appendError(ns.getNamespaceURI()); else if (isSOAP11) ; else newError("A SOAP 1.1 endpoint (XDS.a SQ) was used but message was in unknown SOAP format. SOAP Namespace is ").appendError(ns.getNamespaceURI());
        } else if (xds_version == XBaseTransaction.xds_b) {
            if (isSOAP11) newError("A SOAP 1.2 endpoint (XDS.b SQ) was used but message was in SOAP 1.1 format. SOAP Namespace is ").appendError(ns.getNamespaceURI()); else if (isSOAP12) ; else newError("A SOAP 1.2 endpoint (XDS.b SQ) was used but message was in unknown SOAP format. SOAP Namespace is ").appendError(ns.getNamespaceURI());
            ...
            if (action == null) newError("A SOAP 1.2 endpoint (XDS.b SQ) was used but SOAP Header <Action> is missing.");
        } else newError("StoredQueryRequestSoapValidator: internal error - request is labeled as neither XDS.a nor XDS.b");
    }
}
