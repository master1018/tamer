...
public class AltLocManager {
    ...
    public boolean add(AlternateLocation al, Object source) {
        ...
        synchronized (data) {
            if (al instanceof DirectAltLoc) {
                if (data.direct == AlternateLocationCollection.EMPTY) data.direct = AlternateLocationCollection.create(sha1);
                dCol = data.direct;
            } else if (al instanceof PushAltLoc) {
                PushAltLoc push = (PushAltLoc) al;
                if (push.supportsFWTVersion() < 1) {
                    if (data.push == AlternateLocationCollection.EMPTY) data.push = AlternateLocationCollection.create(sha1);
                    pCol = data.push;
                } else {
                    if (data.fwt == AlternateLocationCollection.EMPTY) data.fwt = AlternateLocationCollection.create(sha1);
                    pCol = data.fwt;
                }
            } else {
    ...
    public boolean remove(AlternateLocation al, Object source) {
        ...
        synchronized (data) {
            if (al instanceof DirectAltLoc) {
                ...
                PushAltLoc push = (PushAltLoc) al;
                if (push.supportsFWTVersion() < 1) pCol = data.push; else pCol = data.fwt;
            }
        }
        ...
        if (!col.hasAlternateLocations()) removeIfEmpty(sha1, data);
        return ret;
    }
    ...
    private void removeIfEmpty(URN sha1, URNData data) {
        boolean empty = false;
        synchronized (data) {
            if (!data.direct.hasAlternateLocations() && !data.push.hasAlternateLocations() && !data.fwt.hasAlternateLocations() && data.getListeners().isEmpty()) empty = true;
        }
        if (empty) urnMap.remove(sha1);
    }
    ...
    public AlternateLocationCollection<DirectAltLoc> getDirect(URN sha1) {
        URNData data = urnMap.get(sha1);
        if (data == null) return AlternateLocationCollection.getEmptyCollection();
        synchronized (data) {
            return data.direct;
        }
    }
    ...
    private static class URNData {
        ...
        public synchronized boolean hasAltLocs() {
            return direct.hasAlternateLocations() || push.hasAlternateLocations() || fwt.hasAlternateLocations();
        }
            ...
            public Object inspect() {
                ...
                synchronized (urnMap) {
                    for (URNData data : urnMap.values()) {
                        for (DirectAltLoc dal : data.direct) direct.add(dal.getHost().getInetAddress(), 1);
                        for (PushAltLoc push : data.push) pushProxies.addAll(push.getPushAddress().getProxies());
                        for (PushAltLoc push : data.fwt) pushProxies.addAll(push.getPushAddress().getProxies());
                    }
                }
