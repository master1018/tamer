...
public class RenewNotify {
    ...
    public static void main(String args[]) {
        try {
            ...
            RegTicket myTicket = mySpace.notify(myPackedEntry, null, new EventListener(), 30000, new MarshalledObject(new Integer(12345)));
            SpaceUID myNotifyUID = myTicket.getUID();
            new Renewer(mySpace, myNotifyUID).start();
            while (true) {
                System.out.println("Do write");
                SpaceUID myEntryUID = mySpace.write(myPackedEntry, null, 50000).getUID();
                if (mySpace.take(myPackedEntry, null, 50000) == null) throw new Exception("Failed to take");
                Thread.sleep(10000);
            }
        } catch (Exception anE) {
    ...
    private static class EventListener implements RemoteEventListener, Serializable {

        public void notify(RemoteEvent anEvent) {
            try {
                System.out.println("Got event: " + anEvent.getSource() + ", " + anEvent.getID() + ", " + anEvent.getSequenceNumber() + ", " + anEvent.getRegistrationObject().get());
            } catch (Exception anE) {
