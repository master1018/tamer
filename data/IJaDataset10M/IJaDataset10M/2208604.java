...
public class KRKOntologyTBox {
    ...
    public void finishBackgroundForRoles() {
        for (int i = 8; i > 0; i--) {
            ...
            kb.addRBoxAxiom(new ObjectPropertyDomainAxiom(rankDistance, KRKOntologyTBox.Piece));
            kb.addRBoxAxiom(new ObjectPropertyRangeAxiom(rankDistance, KRKOntologyTBox.Piece));
            kb.addRBoxAxiom(new ObjectPropertyDomainAxiom(fileDistance, KRKOntologyTBox.Piece));
            kb.addRBoxAxiom(new ObjectPropertyRangeAxiom(fileDistance, KRKOntologyTBox.Piece));
            ...
            kb.addRBoxAxiom(new ObjectPropertyDomainAxiom(getRole("rankDistanceLessThan" + i), KRKOntologyTBox.Piece));
            kb.addRBoxAxiom(new ObjectPropertyRangeAxiom(getRole("rankDistanceLessThan" + i), KRKOntologyTBox.Piece));
            kb.addRBoxAxiom(new ObjectPropertyDomainAxiom(getRole("fileDistanceLessThan" + i), KRKOntologyTBox.Piece));
            kb.addRBoxAxiom(new ObjectPropertyRangeAxiom(getRole("fileDistanceLessThan" + i), KRKOntologyTBox.Piece));
            if (i == 1) continue;
    ...
    protected static ClassAssertionAxiom getConceptAssertion(String concept, String instance) {
        Individual ind = getIndividual(instance);
        NamedClass c = getAtomicConcept(concept);
        return new ClassAssertionAxiom(c, ind);
    }

    protected static ObjectPropertyAssertion getRoleAssertion(String role, String i1, String i2) {
