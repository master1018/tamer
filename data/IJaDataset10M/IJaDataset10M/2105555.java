...
public final class AccessRuleStatisticsHelper {
        ...
        if (avgDenyAccess == -1) {
            ...
            if (getCount() > 0) {
                for (final AccessRule accessRule : accessRules) {
                    final int size = (accessRule.getLevel().equals(SubversionConstants.SVN_ACCESS_LEVEL_DENY_ACCESS)) ? 1 : 0;
                    avgDenyAccess += size;
                }
    ...
    public double getAvgReadOnly() {
        if (avgReadOnly == -1) {
            ...
            if (getCount() > 0) {
                for (final AccessRule accessRule : accessRules) {
                    final int size = (accessRule.getLevel().equals(SubversionConstants.SVN_ACCESS_LEVEL_READONLY)) ? 1 : 0;
                    avgReadOnly += size;
                }
    ...
    public double getAvgReadWrite() {
        if (avgReadWrite == -1) {
            ...
            if (getCount() > 0) {
                for (final AccessRule accessRule : accessRules) {
                    final int size = (accessRule.getLevel().equals(SubversionConstants.SVN_ACCESS_LEVEL_READWRITE)) ? 1 : 0;
                    avgReadWrite += size;
                }
