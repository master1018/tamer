...
public class ClusterEvaluationAnalyzer extends JPanel {
    ...
    private String getHtmlRepresentationDistance() {
        ...
        for (Cluster cluster : clusters.getClusters()) {
            ...
            sb.append(tag(tempBuffer.toString(), TR));
            if (!String.valueOf(distnace[i - 1]).equals("NaN")) sum += distnace[i - 1];
        }
        sb.append("</table>");
        ...
        for (int i = 0; i < clusters.size(); i++) {
            ...
            while (itr.hasNext()) {
                ...
                while (itr2.hasNext()) {
                    ...
                    if (instancePoints[i].getItemKeys().contains(key)) instancePoints[i].set(key, instancePoints[i].get(key) + iPoint.get(key)); else instancePoints[i].set(key, iPoint.get(key));
                }
            }
