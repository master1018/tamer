...
*/
    ClassLoader getLoader(String name) {
        ClassLoaderBox box = (ClassLoaderBox) loaders.get(name);
        if (box != null) return box.cl; else return null;
    }
        ...
        synchronized (loaders) {
            ...
            while (i.hasNext()) {
                ...
                if (clb.users <= 0 && t - clb.unusedFrom > timeout) {
                    Logger.debug(getClass().getName(), "scheduling removal of " + name + (f.isFile() ? " and " + f : ""));
...
class ShutdownCLMThread extends Thread {
    ...
    void tryRemoval() {
        synchronized (toDelete) {
            ...
            while (i.hasNext()) {
                File f = (File) i.next();
                if (!f.exists() || f.delete()) i.remove(); else Logger.warning(getClass().getName(), "Still couldn't delete " + f, null);
            }
        }
    }
