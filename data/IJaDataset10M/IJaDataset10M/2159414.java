...
class RemotePidIterator implements Iterator<String> {
    ...
    private void beginSearch() throws RemoteException {
        org.fcrepo.server.types.gen.FieldSearchQuery genFieldSearchQuery = TypeUtility.convertFieldSearchQueryToGenFieldSearchQuery(query);
        org.fcrepo.server.types.gen.FieldSearchResult searchResult = apia.findObjects(OBJECT_RESULT_FIELDS, MAX_FIND_RESULTS, genFieldSearchQuery);
        FieldSearchResult fsr = TypeUtility.convertGenFieldSearchResultToFieldSearchResult(searchResult);
    ...
    private void resumeSearch() throws RemoteException {
        org.fcrepo.server.types.gen.FieldSearchResult searchResult = apia.resumeFindObjects(token);
        FieldSearchResult fsr = TypeUtility.convertGenFieldSearchResultToFieldSearchResult(searchResult);
        for (ObjectFields fields : fsr.objectFieldsList()) {
            stash.add(fields.getPid());
        }
