...
public class DistanceCalc {
    ...
    public static double c_defaultSemiMajorAxis = 6378137.0;

    public static double c_defaultFlattening = 0.003352813;

    public static double getApproxMeters(double x1, double y1, double x2, double y2) {
        return (Units.convert(getApproxMiles(x1, y1, x2, y2), Units.MILES, Units.METERS));
    }

    public static double getApproxMeters(Point point1, Point point2) {
        return (getApproxMeters(point1.getX(), point1.getY(), point2.getX(), point2.getY()));
    }

    @SuppressWarnings("unused")
    public static double getApproxMiles(double x1, double y1, double x2, double y2) {
        double dx = x2 - x1;
        double dy = y2 - y1;
        ...
        double cos_phi1 = Math.abs(phi1) == (Math.PI / 2.0) ? 0.0 : Math.cos(phi1);
        double sin_phi1 = Math.sin(phi1);
        double cos_phi2 = Math.abs(phi2) == (Math.PI / 2.0) ? 0.0 : Math.cos(phi2);
        double sin_phi2 = Math.sin(phi2);
        double cos_dlam = Math.abs(d_lam) == (Math.PI / 2.0) ? 0.0 : Math.cos(d_lam);
        double sin_dlam = Math.abs(d_lam) == Math.PI ? 0.0 : Math.sin(d_lam);
        double A = Math.sin((phi2 - phi1) / 2.0);
        double B = Math.sin(d_lam / 2.0);
        double sigma = 2.0 * Math.asin(Math.sqrt(A * A + cos_phi1 * cos_phi2 * B * B));
        double dist3 = ((sigma * c_defaultSemiMajorAxis) * c_inchesPerMeter) / c_inchesPerMile;
        return (dist3);
    }
	 ...
	 * above and P3.
	 */
    public static double distancePointLine(Point location, Point lineStart, Point lineEnd) {
        Point intersection = getIntersection(location, lineStart, lineEnd);
        return (getApproxMiles(location.getX(), location.getY(), intersection.getX(), intersection.getY()));
    }
    ...
    public static double getLineMagnitude(double x1, double y1, double x2, double y2) {
    ...
    public static Point getIntersection(Point location, Point lineStart, Point lineEnd) {
        Point intersection = new Point();
        double lineMag = getLineMagnitude(lineEnd, lineStart);
        double U = (((location.getX() - lineStart.getX()) * (lineEnd.getX() - lineStart.getX())) + ((location.getY() - lineStart.getY()) * (lineEnd.getY() - lineStart.getY()))) / (lineMag * lineMag);
        double X = lineStart.getX() + U * (lineEnd.getX() - lineStart.getX());
        double Y = lineStart.getY() + U * (lineEnd.getY() - lineStart.getY());
        if (U < 0.0f || U > 1.0f) {
            double d1 = getApproxMiles(location.getX(), location.getY(), lineStart.getX(), lineStart.getY());
            double d2 = getApproxMiles(location.getX(), location.getY(), lineEnd.getX(), lineEnd.getY());
            if (d1 < d2) {
    ...
    public static double distancePointLineMeters(Point location, Point lineStart, Point lineEnd) {
        Point intersection = getIntersectionMeters(location, lineStart, lineEnd);
        return (getApproxMeters(location.getX(), location.getY(), intersection.getX(), intersection.getY()));
    }

    public static Point getIntersectionMeters(Point location, Point lineStart, Point lineEnd) {
        Point intersection = new Point();
        double lineMag = getLineMagnitude(lineEnd, lineStart);
        double U = (((location.getX() - lineStart.getX()) * (lineEnd.getX() - lineStart.getX())) + ((location.getY() - lineStart.getY()) * (lineEnd.getY() - lineStart.getY()))) / (lineMag * lineMag);
        double X = lineStart.getX() + U * (lineEnd.getX() - lineStart.getX());
        double Y = lineStart.getY() + U * (lineEnd.getY() - lineStart.getY());
        if (U < 0.0f || U > 1.0f) {
            double d1 = getApproxMeters(location.getX(), location.getY(), lineStart.getX(), lineStart.getY());
            double d2 = getApproxMeters(location.getX(), location.getY(), lineEnd.getX(), lineEnd.getY());
            if (d1 < d2) {
                intersection.setX(lineStart.getX());
                intersection.setY(lineStart.getY());
            } else {
