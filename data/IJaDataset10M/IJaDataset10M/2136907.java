...
public class ColorConstants {
    ...
    static {
        colors = new PersistantMap(new RelativeFile("_Colors.txt"), "These are the color constants that display on the SwingGUI.  Feel free to modify/play with them.");
    ...
    private class ColorEditor extends JFrame implements ListSelectionListener, WindowListener {
        ...
        public void trySave() {
            if (currentKey != null) {
                ...
                String newString = PadString.padHex(newColor.getRed(), 2) + PadString.padHex(newColor.getGreen(), 2) + PadString.padHex(newColor.getBlue(), 2);
                if (original.equalsIgnoreCase(newString) == false) {
                    if (JOptionPane.showConfirmDialog(this, "Save new color?", "Save?", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
