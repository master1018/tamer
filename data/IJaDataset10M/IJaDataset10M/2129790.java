...
public final class HashedTernaryKeyMap<K1, K2, K3, V> extends AbstractHashedMap<TernaryKey<K1, K2, K3>, V> implements TernaryKeyMap<K1, K2, K3, V> {
     ...
     */
    public HashedTernaryKeyMap(final int initialCapacity, final float loadFactor, final int threshold) {
     ...
     * @return true if the keys in the specified entry are equal to the specified keys
     */
    private boolean isEqualKey(final HashEntry<TernaryKey<K1, K2, K3>, V> entry, final K1 key1, final K2 key2, final K3 key3) {
        TernaryKey<K1, K2, K3> key = entry.getKey();
        return (key1 == null ? key.getFirstKey() == null : key1.equals(key.getFirstKey())) && (key2 == null ? key.getSecondKey() == null : key2.equals(key.getSecondKey())) && (key3 == null ? key.getThirdKey() == null : key3.equals(key.getThirdKey()));
    }
