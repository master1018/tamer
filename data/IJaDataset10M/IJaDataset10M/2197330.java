...
public class BIJgifa extends BIJpca {
    ...
    public float[][] compute(float[][] a, int k) {
        ...
        for (int j = 0; j < N; j++) for (int i = 0; i < num; i++) ahat[j][i] = condavg[j / t][i];
        float[][] Cn = new float[num][num];
        for (int i = 0; i < k; i++) {
            float[][] Cnt = new float[t][num];
            for (int h = 0; h < t; h++) for (int g = 0; g < num; g++) Cnt[h][g] = a[h * t][g] - ahat[h * t][g];
    ...
    protected float[][] submean(float[][] F, int k) {
        ...
        for (int n = 0; n < k; n++) for (int j = 0; j < G[0].length; j++) {
            G[n][j] = 0;
            for (int i = 0; i < iavg; i++) G[n][j] += F[iavg * n + i][j];
            G[n][j] /= iavg;
        }
        return G;
    }
