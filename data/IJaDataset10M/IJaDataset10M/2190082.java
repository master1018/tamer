...
public class FilterExpression {
    ...
    public static final String DOCUMENT_CLASSNAME = "cn.myapps.core.dynaform.document.ejb.Document";

    public static final String ITEM_CLASSNAME = "cn.myapps.core.dynaform.document.ejb.Item";
    ...
    */
    public String parseToHql(String text, ParamsTable params) {
        ...
        hql.append("FROM " + DOCUMENT_CLASSNAME + " WHERE istmp <> true AND id IN(");
        ...
        while (text.indexOf("{", fromIndex) != -1) {
            ...
            if (mapStrs.length > 3) {
                ...
                operator = mapStrs[1];
                right = mapStrs[3];
                String varcharvalue = params.getParameterAsString(getValue(right));
                ...
                if (getValue(left).startsWith("$")) {
                    ...
                    if (varcharvalue != null) {
                        hql.append("(doc.id IN (SELECT items.document FROM " + ITEM_CLASSNAME + " items ");
                        hql.append("WHERE items.name='" + getValue(left) + "' ");
