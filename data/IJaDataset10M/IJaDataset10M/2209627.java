...
public class Clock {
    ...
    public boolean fillSaveState(Bundle outState) {
        ...
        if (!isStarted) outState.putLong("clockStopTime", stopTime); else outState.putLong("clockStopTime", -1L);
        if (parent.comment != null) outState.putString("clockComment", parent.comment); else outState.putString("clockComment", "");
        outState.putInt("clockLapCount", laps);
        if (parent.lapView != null) outState.putCharSequence("clockLaps", parent.laps); else outState.putCharSequence("clockLaps", "");
    ...
    public boolean fillSaveState(SharedPreferences outState) {
        ...
        if (notInUse) {
            ...
            if (!isStarted) outPref.putLong("anstop_state_clockStopTime", stopTime); else outPref.putLong("anstop_state_clockStopTime", -1L);
            if (parent.comment != null) outPref.putString("anstop_state_clockComment", parent.comment); else outPref.putString("anstop_state_clockComment", "");
            outPref.putInt("anstop_state_clockLapCount", laps);
            if (parent.lapView != null) outPref.putString("anstop_state_clockLaps", parent.laps.toString()); else outPref.putString("anstop_state_clockLaps", "");
    ...
    public void onAppResume() {
        if (!isStarted) return;
        if (appPauseTime > appStateRestoreTime) adjClockOnAppResume(false, System.currentTimeMillis());
        if (v == Anstop.STOP_LAP) {
            if ((threadS != null) && threadS.isAlive()) threadS.interrupt();
            threadS = new clockThread();
            threadS.start();
        } else {
            if ((threadC != null) && threadC.isAlive()) threadC.interrupt();
    ...
    public boolean restoreFromSaveState(Bundle inState) {
        ...
        if ((inState == null) || !inState.containsKey("clockActive")) return false;
        ...
        if ((parent.comment != null) && (parent.comment.length() == 0)) parent.comment = null;
        ...
        if (parent.lapView != null) {
            ...
            if (laptext != null) parent.laps.append(laptext);
        }
        if ((parent.comment != null) || (parent.lapView != null)) parent.updateStartTimeCommentLapsView();
        if (parent.hourSpinner != null) {
            final int h = inState.getInt("clockCountHour"), m = inState.getInt("clockCountMin"), s = inState.getInt("clockCountSec");
    ...
    public boolean restoreFromSaveState(SharedPreferences inState) {
        long restoredAtTime = System.currentTimeMillis();
        if ((inState == null) || !inState.getBoolean("anstop_in_use", false)) return false;
        appStateRestoreTime = restoredAtTime;
        ...
        if (parent.lapView != null) {
            ...
            if (laptext.length() > 0) parent.laps.append(laptext);
        }
        if ((parent.comment != null) || (parent.lapView != null)) parent.updateStartTimeCommentLapsView();
        if (parent.hourSpinner != null) {
            final int h = inState.getInt("anstop_state_clockCountHour", 0), m = inState.getInt("anstop_state_clockCountMin", 0), s = inState.getInt("anstop_state_clockCountSec", 0);
            countdnTotalSeconds = ((h * 60) + m) * 60 + s;
            parent.hourSpinner.setSelection(h);
            parent.minSpinner.setSelection(m);
            parent.secSpinner.setSelection(s);
	 ...
	 */
    private void adjClockOnAppResume(final boolean adjDisplayOnly, final long resumedAtTime) {
        if (!adjDisplayOnly) {
            long ttotal;
            switch(v) {
                case Anstop.STOP_LAP:
                    ...
                    break;
                default:
                    ttotal = (countdnTotalSeconds * 1000L) - (resumedAtTime - startTimeAdj);
        ...
        if (parent.dsecondsView != null) parent.dsecondsView.setText(Integer.toString(dsec));
        if (parent.secondsView != null) parent.secondsView.setText(nf.format(sec));
        if (parent.minView != null) parent.minView.setText(nf.format(min));
        if (parent.hourView != null) parent.hourView.setText(Integer.toString(hour));
    }
    ...
    public boolean isInUse() {
        return isStarted || (hour > 0) || (min > 0) || (sec > 0) || (dsec > 0);
    }
    ...
    public void count() {
        ...
        if (!isStarted) {
            ...
            stopTime = now;
            if (v == Anstop.STOP_LAP) {
                if (threadS.isAlive()) threadS.interrupt();
            } else {
                if (threadC.isAlive()) threadC.interrupt();
            }
        }
    }
