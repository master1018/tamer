...
class RequestControlContextImpl implements RequestControlContext {
    ...
    public ViewSelection start(Flow flow, AttributeMap input) throws FlowExecutionException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Activating new session for flow '" + flow.getId() + "' in state '" + flow.getStartState().getId() + "' with input " + input);
        }
    ...
    public String toString() {
        return new ToStringCreator(this).append("externalContext", externalContext).append("requestScope", requestScope).append("executionProperties", attributes).append("flowExecution", flowExecution).toString();
    }
}
