...
public class AllApps3D extends RSSurfaceView implements AllAppsView, View.OnClickListener, View.OnLongClickListener, DragSource {
    ...
    public boolean onLongClick(View v) {
        ...
        if (sRollo.checkClickOK() && mCurrentIconIndex == mDownIconIndex && mCurrentIconIndex >= 0 && mCurrentIconIndex < mAllAppsList.size()) {
            ...
            mDragController.startDrag(bmp, screenX, screenY, 0, 0, w, h, this, app, DragController.DRAG_ACTION_COPY);
            mLauncher.closeAllApps(true);
        }
        return true;
    }
    ...
    public static class RolloRS {
        ...
        private void initProgramFragment() {
            ...
            bf.setTexture(ProgramFragment.Builder.EnvMode.MODULATE, ProgramFragment.Builder.Format.RGBA, 0);
            mPFTexMip = bf.create();
            ...
            bf.setTexture(ProgramFragment.Builder.EnvMode.MODULATE, ProgramFragment.Builder.Format.ALPHA, 0);
        ...
        private void initData() {
            ...
            mHomeButtonNormal = Allocation.createFromBitmapResource(sRS, mRes, R.drawable.home_button_normal, Element.RGBA_8888(sRS), false);
            mHomeButtonNormal.uploadToTexture(0);
            mHomeButtonFocused = Allocation.createFromBitmapResource(sRS, mRes, R.drawable.home_button_focused, Element.RGBA_8888(sRS), false);
            mHomeButtonFocused.uploadToTexture(0);
            mHomeButtonPressed = Allocation.createFromBitmapResource(sRS, mRes, R.drawable.home_button_pressed, Element.RGBA_8888(sRS), false);
            ...
            mParams.save();
            mState.save();
            mSelectionBitmap = Bitmap.createBitmap(Defines.SELECTION_TEXTURE_WIDTH_PX, Defines.SELECTION_TEXTURE_HEIGHT_PX, Bitmap.Config.ARGB_8888);
        ...
        private void createAppIconAllocations(int index, ApplicationInfo item) {
            mIcons[index] = Allocation.createFromBitmap(sRS, item.iconBitmap, Element.RGBA_8888(sRS), false);
            mLabels[index] = Allocation.createFromBitmap(sRS, item.titleBitmap, Element.A_8(sRS), false);
        ...
        private void uploadAppIcon(int index, ApplicationInfo item) {
            if (mIconIds[index] != mIcons[index].getID()) {
                throw new IllegalStateException("uploadAppIcon index=" + index + " mIcons[index].getID=" + mIcons[index].getID() + " mIconsIds[index]=" + mIconIds[index] + " item=" + item);
            }
        ...
        void selectIcon(int index, int pressed) {
            ...
            if (appsList == null || index < 0 || index >= appsList.size()) {
                ...
                mSelectedIcon = Allocation.createFromBitmap(sRS, selectionBitmap, Element.RGBA_8888(sRS), false);
