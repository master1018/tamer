...
public class StringUtil {
    ...
    public static String rndString(int n, char[] chars) {
        ...
        int k = chars.length;
        if (k < 2) throw new IllegalArgumentException("chars.length < 2");
        StringBuffer sb = new StringBuffer(n);
        while (n-- > 0) sb.append(chars[(int) (Math.random() * k)]);
        return sb.toString();
    }

    private static final char[] DEFAULT_CHARS = ("BCDFGHJKLMNPQRSTVWXYZ" + "bcdfghjklmnpqrstvwxyz" + "23456789").toCharArray();
    ...
    public static char toHex(int i) {
        ...
        if (i >= 10 && i <= 15) return (char) ('a' + i - 10);
