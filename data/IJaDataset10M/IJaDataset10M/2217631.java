...
public class ScheduleTableModel extends javax.swing.table.AbstractTableModel implements PropertyChangeListener {
    ...
    synchronized void updateList() {
        ...
        for (int i = 0; i < list.size(); i++) {
            _schedule.getItemById(list.get(i)).addPropertyChangeListener(this);
        }
    }
    ...
    void initTable(JTable table, Schedule schedule, Location location, Track track) {
        ...
        _location = location;
        _track = track;
        if (_schedule != null) _schedule.addPropertyChangeListener(this);
    ...
    public String getColumnName(int col) {
        switch(col) {
            ...
            case CURRENTCOLUMN:
                return rb.getString("Current");
            case TYPECOLUMN:
                return rb.getString("Type");
            case DAYCOLUMN:
                return rb.getString("Timetable");
            case ROADCOLUMN:
                return rb.getString("Road");
            case LOADCOLUMN:
                return rb.getString("Receive");
            case SHIPCOLUMN:
                return rb.getString("Ship");
            case DESTCOLUMN:
                return rb.getString("Destination");
            case TRACKCOLUMN:
                return rb.getString("Track");
            case COUNTCOLUMN:
    ...
    public Object getValueAt(int row, int col) {
        if (row >= list.size()) return "ERROR row " + row;
        ScheduleItem si = _schedule.getItemById(list.get(row));
        if (si == null) return "ERROR schedule item unknown " + row;
        switch(col) {
            case IDCOLUMN:
                return si.getId();
            case CURRENTCOLUMN:
            ...
            case WAITCOLUMN:
                return si.getWait();
            case UPCOLUMN:
                return rb.getObject("Up");
            case DOWNCOLUMN:
                return rb.getObject("Down");
            case DELETECOLUMN:
                return rb.getObject("Delete");
            default:
                return "unknown " + col;
        }
    }
    ...
    private String getCurrentPointer(ScheduleItem si) {
        if (_track.getCurrentScheduleItem() == si) if (si.getCount() > 1) return " " + _track.getScheduleCount() + " -->"; else return "    -->"; else return "";
    }

    private String getType(ScheduleItem si) {
        if (_track.acceptsTypeName(si.getType())) return si.getType(); else return MessageFormat.format(rb.getString("NotValid"), new Object[] { si.getType() });
    }
    ...
    private JComboBox getRoadComboBox(ScheduleItem si) {
        ...
        for (int i = 0; i < roads.length; i++) {
            if (_track.acceptsRoadName(roads[i])) {
                ...
                if (car != null) cb.addItem(roads[i]);
            }
        }
    ...
    private void setRoad(Object value, int row) {
        ...
        if (!test.equals(rb.getString("NotValid").substring(0, 11))) si.setRoad(road);
    }
    ...
    private void setLoad(Object value, int row) {
        ...
        if (!test.equals(rb.getString("NotValid").substring(0, 11))) si.setLoad(load);
    }
    ...
    private void setShip(Object value, int row) {
        ...
        String test = load.substring(0, 11);
        if (!test.equals(rb.getString("NotValid").substring(0, 11))) si.setShip(load);
    }
    ...
    private void filterTracks(Location loc, JComboBox cb, String carType, String carRoad, String carLoad) {
        ...
        for (int i = 0; i < tracks.size(); i++) {
            ...
            if (!track.acceptsTypeName(carType) || track.getLocType().equals(Track.STAGING) || (!carRoad.equals("") && !track.acceptsRoadName(carRoad)) || (!carLoad.equals("") && !track.acceptsLoadName(carLoad))) cb.removeItem(track);
        }
    }
    ...
    private void filterLoads(JComboBox cb) {
        for (int i = cb.getItemCount() - 1; i > 0; i--) {
            ...
            if (!loadName.equals("") && !_track.acceptsLoadName(loadName)) cb.removeItem(loadName);
        }
    }
    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        if (e.getPropertyName().equals(CarTypes.CARTYPES_LENGTH_CHANGED_PROPERTY) || e.getPropertyName().equals(Track.TYPES_CHANGED_PROPERTY) || e.getPropertyName().equals(Track.ROADS_CHANGED_PROPERTY) || e.getPropertyName().equals(Track.SCHEDULE_CHANGED_PROPERTY) || e.getPropertyName().equals(Location.TYPES_CHANGED_PROPERTY)) {
            fireTableDataChanged();
        }
    }
    ...
    private void removePropertyChangeScheduleItems() {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (si != null) si.removePropertyChangeListener(this);
        }
    }
