...
public class CSVFileUtility {
    ...
    public static String merge(char delimiter, Object... args) {
        if (delimiter != ',' && delimiter != ';') throw new IllegalArgumentException("Invalid delimiter: " + delimiter);
        StringBuilder sb = new StringBuilder(128);
        for (int i = 0; i < args.length; ++i) sb.append(args[i].toString().trim()).append(delimiter);
        return sb.toString();
    }
    ...
    public static String[] parse(String line, char delimiter, boolean trim) {
        ...
        if (trim) for (int i = 0; i < ret.length; ++i) {
            String s = ret[i];
            if (s.startsWith("\"") && s.endsWith("\"")) s = s.substring(1, s.length());
            ret[i] = s.trim();
        }
        return ret;
    }
