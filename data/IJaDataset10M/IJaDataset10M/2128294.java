...
public class DependencyPatternsResolver implements PatternResolver {
    ...
    public String resolve(String codePattern) {
        ...
        for (Dependency d : context.dependenciesToMock()) {
            ...
            buffer.append(codePattern.replaceAll("\\$\\{dependencyType\\}\\s*\\.\\s*class", resolvedType + ".class").replaceAll("\\$\\{dependencyType\\}", resolvedType + typeParams).replaceAll("\\$\\{dependency\\}", d.name));
        }
