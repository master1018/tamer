...
public class ErrorTest extends AbstractAcceptanceTest {
    ...
    public void testInvalidXml() {
        try {
            ...
            assertContains("unexpected character in markup", e.get("cause-message"));
            assertEquals("/thing/two", e.get("path"));
            assertEquals("3", e.get("line number"));
        }
    }
    ...
    private void assertContains(String expected, String actual) {
        assertTrue("Substring not found. Expected <" + expected + "> but got <" + actual + ">", actual.indexOf(expected) > -1);
    }
}
