...
public abstract class MonadObject implements MonadValidatable, XmlDescriber {
    ...
    public Node getXML(XmlTree tree, Document doc) throws ProgrammerException, UserException, DesignerException {
        ...
        for (Iterator<String> it = tree.keyIterator(); it.hasNext(); ) {
            ...
            String methodName = "get" + methodBase.substring(0, 1).toUpperCase() + methodBase.substring(1);
            try {
                ...
                if (obj != null) {
                    if (obj instanceof Set) {
                        ...
                        for (Iterator jt = ((List) obj).iterator(); jt.hasNext(); ) {
                            node.appendChild(describerNode(doc, (XmlDescriber) jt.next(), nestedTree));
                        }
                    } else {
                        Node newNode = describerNode(doc, (XmlDescriber) obj, nestedTree);
    ...
    private Node describerNode(Document doc, XmlDescriber describer, XmlTree tree) throws ProgrammerException, UserException, DesignerException {
        return (tree == null) ? describer.toXML(doc) : describer.getXML(tree, doc);
    }
