...
class RmiDebugModelImpl extends UnicastRemoteObject implements DebugModel {
    ...
    private static int calculateType(TemplateModel model) {
        ...
        if (model instanceof TemplateHashModelEx) type += TYPE_HASH_EX; else if (model instanceof TemplateHashModel) type += TYPE_HASH;
        if (model instanceof TemplateMethodModelEx) type += TYPE_METHOD_EX; else if (model instanceof TemplateMethodModel) type += TYPE_METHOD;
