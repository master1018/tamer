...
public class AccountsView extends ViewPart implements Observer, ICollectionObserver {
    ...
    private ControlListener columnControlListener = new ControlAdapter() {
        ...
        public void controlResized(ControlEvent e) {
            ...
            for (int i = 0; i < tree.getColumnCount(); i++) sizes.append(String.valueOf(tree.getColumn(i).getWidth()) + ";");
    ...
    public void init(IViewSite site) throws PartInitException {
        ...
        toolBarManager.add(new Separator("begin"));
        toolBarManager.add(new Separator("group1"));
        toolBarManager.add(new Separator("group2"));
        toolBarManager.add(new Separator("group3"));
        toolBarManager.add(new Separator("group4"));
        toolBarManager.add(new Separator("group5"));
        toolBarManager.add(new Separator("group6"));
    ...
    public void createPartControl(Composite parent) {
        ...
        dragSource.addDragListener(new DragSourceListener() {
            ...
            public void dragStart(DragSourceEvent event) {
                ...
                if (selection.length == 0) event.doit = false;
            }

            public void dragSetData(DragSourceEvent event) {
                int count = 0;
                for (int i = 0; i < selection.length; i++) {
                    if (selection[i] instanceof AccountTreeItem) count++;
                }
                AccountTreeItem[] items = new AccountTreeItem[count];
                count = 0;
                for (int i = 0; i < selection.length; i++) {
                    if (selection[i] instanceof AccountTreeItem) items[count++] = (AccountTreeItem) selection[i];
                }
                event.data = items;
            }
            ...
            public void drop(DropTargetEvent event) {
                ...
                if (_accountTreeItemTransfer.isSupportedType(event.currentDataType)) {
                    ...
                    if (items != null) {
                        ...
                        if (item != null) {
                            if (item instanceof GroupTreeItem) group = ((GroupTreeItem) item).getGroup(); else if (item instanceof AccountTreeItem) group = ((AccountTreeItem) item).getAccount().getGroup();
                        }
                        for (int i = 0; i < items.length; i++) {
                            ...
                            if (account.getGroup() != null) account.getGroup().getAccounts().remove(account);
                            account.setGroup(group);
                            if (account.getGroup() != null) account.getGroup().getAccounts().add(account);
        ...
        menuMgr.addMenuListener(new IMenuListener() {
            ...
            public void menuAboutToShow(IMenuManager menuManager) {
                ...
                menuManager.add(new Separator("bottom"));
            }
        });
        ...
        for (int i = 0; i < values.length; i++) expandMap.put(values[i], new Boolean(true));
    ...
    private void updateView() {
        ...
        for (Iterator iter = list.iterator(); iter.hasNext(); ) {
            ...
            if (account.getParent() == null) new GroupTreeItem(account, tree, SWT.NONE);
        }
        list = CorePlugin.getRepository().allAccounts();
        Collections.sort(list, accountComparator);
        for (Iterator iter = list.iterator(); iter.hasNext(); ) {
            Account account = (Account) iter.next();
            if (account.getGroup() == null) new AccountTreeItem(account, tree, SWT.NONE);
        }
        int index = 0;
        TreeItem[] items = tree.getItems();
        for (int i = 0; i < items.length; i++) index = updateItemColors(items[i], index);
        String[] sizes = TradingPlugin.getDefault().getPreferenceStore().getString(PREFS_ACCOUNT_COLUMNS_SIZE).split(";");
        for (int i = 0; i < tree.getColumnCount(); i++) {
            if (i < sizes.length && sizes[i].length() != 0) tree.getColumn(i).setWidth(Integer.parseInt(sizes[i])); else tree.getColumn(i).setWidth(i == 0 ? 100 : 75);
        }
        if ("gtk".equals(SWT.getPlatform())) tree.getColumn(tree.getColumnCount() - 1).pack();
    }
    ...
    private int updateItemColors(TreeItem treeItem, int index) {
        ...
        if (treeItem instanceof GroupTreeItem) {
            ...
            if (expandMap.get(String.valueOf(group.getId())) != null) treeItem.setExpanded(true);
        }
    ...
    private void updateSelection() {
        ...
        if (selection != null && selection.length == 1) {
            ...
            if (item instanceof Account) getSite().getSelectionProvider().setSelection(new AccountSelection((Account) item)); else if (item instanceof AccountGroup) getSite().getSelectionProvider().setSelection(new AccountGroupSelection((AccountGroup) item)); else getSite().getSelectionProvider().setSelection(new NullSelection());
        } else getSite().getSelectionProvider().setSelection(new NullSelection());
    ...
    public void itemAdded(Object o) {
        if (o instanceof Account) {
            ...
            if (account.getGroup() != null) return;
            TreeItem items[] = tree.getItems();
            for (int i = 0; i < items.length; i++) {
                if (!(items[i].getData() instanceof Account)) continue;
    ...
    public void itemRemoved(Object o) {
        TreeItem[] items = tree.getItems();
        for (int i = 0; i < items.length; i++) {
            if (items[i].getData().equals(o)) items[i].dispose();
        }
        updateItemColors();
        updateSelection();
    }
        ...
        tree.getDisplay().asyncExec(new Runnable() {
            ...
            public void run() {
                if (!tree.isDisposed()) {
                    ...
                    for (int i = 0; i < childs.length; i++) updateTree(childs[i]);
                }
            }
        });
    }
    ...
    void updateTree(TreeItem treeItem) {
        if (treeItem instanceof AccountTreeItem) ((AccountTreeItem) treeItem).update(null, null); else {
    ...
    private class AccountTreeItem extends TreeItem implements Observer {
        ...
        private void init(Account account) {
            ...
            if (account.getCurrency() != null) setText(1, account.getCurrency().getSymbol() + " " + nf.format(account.getBalance())); else setText(1, nf.format(account.getBalance()));
            setData(account);
    ...
    private class GroupTreeItem extends TreeItem implements ICollectionObserver, Observer {
        ...
        public void update(Observable o, Object arg) {
            getDisplay().asyncExec(new Runnable() {
                ...
                public void run() {
                    if (!isDisposed()) setText(group.getDescription());
                }
            });
        }
        ...
        public void itemRemoved(Object o) {
            TreeItem[] items = getItems();
            for (int i = 0; i < items.length; i++) {
                if (o.equals(items[i].getData())) items[i].dispose();
            }
            updateItemColors();
        }
    }
    ...
    public class AccountTreeItemTransfer extends ByteArrayTransfer {
        ...
        protected void javaToNative(Object object, TransferData transferData) {
            if (!checkMyType(object) || !isSupportedType(transferData)) DND.error(DND.ERROR_INVALID_DATA);
            try {
                ...
                if (object instanceof AccountTreeItem) {
                    ...
                    for (int i = 0; i < array.length; i++) writeOut.writeObject(array[i].getAccount().getId());
                }
        ...
        protected Object nativeToJava(TransferData transferData) {
            if (isSupportedType(transferData)) {
                ...
                try {
                    ...
                    for (int i = 0; i < length; i++) security[i] = (Integer) readIn.readObject();
                    readIn.close();
                    return security;
                } catch (Exception e) {
                    CorePlugin.logException(e);
