...
public class PropertyHelper {
   ...
   */
    public static float getFloatProperty(Properties properties, Object key) {
        ...
        if (string == null) System.err.println("WARN: couldn't find float value under '" + key + "'");
   ...
   */
    public static int getIntProperty(Properties properties, Object key) {
        String string = (String) properties.get(key);
        if (string == null) System.err.println("WARN: couldn't find integer value under '" + key + "'");
   ...
   */
    public static int getIpProperty(Properties properties, Object key) {
        String string = (String) properties.get(key);
        if (string == null) {
            System.err.println("WARN: couldn't find IP value under '" + key + "'");
            return 0;
        }
        StringTokenizer st = new StringTokenizer(string, ".");
        int address;
        try {
            address = Integer.parseInt(st.nextToken()) << 24 | Integer.parseInt(st.nextToken()) << 16 | Integer.parseInt(st.nextToken()) << 8 | Integer.parseInt(st.nextToken());
        } catch (NoSuchElementException e) {
            address = 0;
            System.err.println("WARN: invalid color spec '" + string + "' in property file");
        }
   ...
   */
    public static boolean getBooleanProperty(Properties properties, Object key) {
        ...
        if (string.toLowerCase().equals("true") || string.toLowerCase().equals("on") || string.toLowerCase().equals("yes") || string.toLowerCase().equals("1")) return true; else return false;
    }

    /**
   * Refresh property settings from disk.
   */
    public static Properties refresh(String name, FileInputStream fis) throws IOException {
    ...
    public static Properties load(String[] paths, String fileName) {
        ...
        try {
            ...
            if (!propertiesFile.exists()) {
                System.err.println("FATAL: could not find file '" + fileName + "' in default search paths: ");
                for (int i = 0; i < paths.length; i++) {
                    System.err.print("'" + paths[i] + "'");
                    if (i < paths.length - 1) System.err.print(", ");
                }
                System.err.println();
                System.exit(1);
            }
            properties = refresh(propertiesFile.getAbsolutePath(), new FileInputStream(propertiesFile));
        } catch (Exception e) {
            System.err.println("ERROR: couldn't load properties from '" + propertiesFile + "'");
        }
        return properties;
    }

    private static String _rcsId = "$Id: PropertyHelper.java 540 2006-06-21 16:40:58 +0200 (Wed, 21 Jun 2006) buchmand $";
}
