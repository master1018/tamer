...
public class Diff {
    ...
    public static void main(String[] args) {
        ...
        if (result.size() < 1) System.out.println("No differences encountered."); else {
    ...
    public static void jarsAsTypeSystems(JarFile jar1, JarFile jar2, List diffs) {
        ...
        for (; entries1.hasMoreElements(); ) {
            ...
            if (name.startsWith("schema" + SchemaTypeSystemImpl.METADATA_PACKAGE_GEN + "/system/s") && name.endsWith(".xsb")) list1.add(ze);
        }
        for (; entries2.hasMoreElements(); ) {
            ZipEntry ze = (ZipEntry) entries2.nextElement();
            String name = ze.getName();
            if (name.startsWith("schema" + SchemaTypeSystemImpl.METADATA_PACKAGE_GEN + "/system/s") && name.endsWith(".xsb")) list2.add(ze);
        }
    ...
    public static void dirsAsTypeSystems(File dir1, File dir2, List diffs) {
        ...
        if (temp1.exists() && temp2.exists()) {
            ...
            if (files1.length == 1 && files2.length == 1) {
                ...
                if (files1.length == 0) temp1 = null;
                if (files2.length == 0) temp2 = null;
                if (files1.length > 1) {
            ...
            if (!temp1.exists()) temp1 = null;
            if (!temp2.exists()) temp2 = null;
        }
        if (temp1 == null && temp2 == null) return; else if (temp1 == null || temp2 == null) {
            if (temp1 == null) diffs.add("No typesystems found in dir \"" + dir1 + "\"");
            if (temp2 == null) diffs.add("No typesystems found in dir \"" + dir2 + "\"");
            return;
        } else {
            dir1 = temp1;
            dir2 = temp2;
        ...
        while (i1 < files1.length && i2 < files2.length) {
            ...
            int dif = name1.compareTo(name2);
            if (dif == 0) {
                if (diffIndex || !files1[i1].getName().equals("index.xsb")) filesAsXsb(files1[i1], files2[i2], diffs);
                i1++;
                i2++;
            } else if (dif < 0) {
    ...
    private static boolean isDiffIndex() {
        ...
        if ("0".equals(prop) || "false".equalsIgnoreCase(prop)) return false;
        return true;
    }
