 ...
 */
public class CallableStatement_base extends PreparedStatement_base implements java.sql.CallableStatement {

    public static final String cvsVersion = "$Id: CallableStatement_base.java,v 1.1 2002-10-14 10:48:59 alin_sinpalean Exp $";
    ...
    public CallableStatement_base(TdsConnection conn_, String sql, int type, int concurrency) throws SQLException {
        ...
        if (pos >= 0) i = pos + 6;
        while (i < sql.length() && !Character.isLetterOrDigit(sql.charAt(i)) && sql.charAt(i) != '#') i++;
        pos = i;
        while (i < sql.length() && (Character.isLetterOrDigit(sql.charAt(i)) || sql.charAt(i) == '#' || sql.charAt(i) == '_')) i++;
        procedureName = sql.substring(pos, i);
        if (procedureName.length() == 0) throw new SQLException("Did not find name in sql string");
    }

    private Object getParam(int index) throws SQLException {
        if (index < 1) throw new SQLException("Invalid parameter index " + index + ". JDBC indexes start at 1.");
        if (index > parameterList.length) throw new SQLException("Invalid parameter index " + index + ". This statement only has " + parameterList.length + " parameters.");
        index--;
    ...
    protected void addOutputParam(Object value) throws SQLException {
        for (lastOutParam++; lastOutParam < parameterList.length; lastOutParam++) if (parameterList[lastOutParam].isOutput) {
