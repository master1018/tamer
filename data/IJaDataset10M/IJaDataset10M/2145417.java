...
public class ObjectStub {
    ...
    private String dequalify(String className) {
        ...
        if (lastPeriod >= 0) className = className.substring(lastPeriod + 1);
        return className;
    }
    ...
    public ObjectStub(String className, String sourceName) {
        className = dequalify(className);
        this.className = className;
        if (sourceName != null) this.sourceName = lowerCaseFirstLetter(sourceName); else this.sourceName = generateName(className);
    }
