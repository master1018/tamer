...
public class Sequence implements Serializable {
    ...
    public synchronized boolean isBefore(Sequence sequence) throws SequenceException {
        ...
        if (!this.isWrapped && !sequence.isWrapped && (this.generation == sequence.generation)) {
    ...
    public synchronized boolean isBeforeOrEqual(Sequence sequence) throws SequenceException {
        assertSameParent(sequence);
        return (isBefore(sequence) || isEqual(sequence));
    }

    public synchronized boolean isAfter(Sequence sequence) throws SequenceException {
        assertSameParent(sequence);
    ...
    public boolean isEqual(Sequence sequence) throws SequenceException {
        assertSameParent(sequence);
        if (currentValue.get() == sequence.currentValue.get() && this.generation == sequence.generation) {
            return true;
        }
        return false;
    }
    ...
    public void assertSameParent(Sequence sequence) throws SequenceException {
        if (!identity.toString().equals(sequence.identity.toString())) {
            throw new SequenceException("Parents differ, this=" + identity + ", sequence=" + sequence.getIdentity());
        }
    }
}
