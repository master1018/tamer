...
public class ConfigSectionIPFilter implements UISWTConfigSection {
    ...
    public void configSectionSave() {
        try {
            if (filter != null) filter.save();
        } catch (Exception e) {
            Logger.log(new LogAlert(LogAlert.UNREPEATABLE, "Save of filter file fails", e));
        }
    }
    ...
    public Composite configSectionCreate(final Composite parent) {
        ...
        btnLoadNow.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(Event event) {
                try {
                    btnLoadNow.getShell().setCursor(btnLoadNow.getDisplay().getSystemCursor(SWT.CURSOR_WAIT));
                    COConfigurationManager.setParameter(IpFilterAutoLoaderImpl.CFG_AUTOLOAD_LAST, 0);
                    filter.reload();
                    btnLoadNow.getShell().setCursor(btnLoadNow.getDisplay().getSystemCursor(SWT.CURSOR_ARROW));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private void resizeTable() {
        ...
        if (iNewWidth > 50) table.getColumn(0).setWidth(iNewWidth);
    }
    ...
    public void refresh() {
        if (table == null || table.isDisposed() || noChange) return;
        noChange = true;
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (desc != null && !desc.equals(tmp)) items[i].setText(0, desc);
            tmp = items[i].getText(1);
            if (range.getStartIp() != null && !range.getStartIp().equals(tmp)) items[i].setText(1, range.getStartIp());
            tmp = items[i].getText(2);
            if (range.getEndIp() != null && !range.getEndIp().equals(tmp)) items[i].setText(2, range.getEndIp());
        }
    }
