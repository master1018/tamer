...
public abstract class RegexValidator extends AbstractValidator<String> {
    ...
    protected void onValidate(IValidatable<String> validatable) {
        ...
        try {
            if (value.matches(regex)) return; else reportError(resourceKeyInvalidValue(), value, regex);
    ...
    private void reportError(String resourceKey, final String value, final String regex) {
        try {
            String message = Application.get().getResourceSettings().getLocalizer().getString(resourceKey, (Component) null, new Model<Serializable>(new Serializable() {
