...
public class EditPreferencesAction extends org.exmaralda.partitureditor.partiture.AbstractTableAction {
    ...
    private void editPreferences(java.awt.event.ActionEvent evt) {
        String mediaPlayer = java.util.prefs.Preferences.userRoot().node("org.sfb538.exmaralda.PartiturEditor").get("PlayerType", "JMF-Player");
        ...
        String[] values = { table.defaultFontName, table.generalPurposeFontName, table.head2HTMLStylesheet, table.speakertable2TranscriptionStylesheet, table.transcription2FormattableStylesheet, table.freeStylesheetVisualisationStylesheet, table.HIATUtteranceList2HTMLStylesheet, new Boolean(table.autoSave).toString(), table.autoSaveThread.FILENAME, table.autoSaveThread.PATH, Integer.toString(table.autoSaveThread.SAVE_INTERVAL / 60000), table.hiatFSM, table.didaFSM, table.gatFSM, table.chatFSM, table.language, mediaPlayer, new Boolean(table.underlineWithDiacritics).toString(), table.underlineCategory, table.preferredSegmentation, showSFB538Menu.toString(), showSinMenu.toString(), showODTSTDMenu.toString(), new Boolean(table.AUTO_ANCHOR).toString(), new Boolean(table.AUTO_REMOVE_UNUSED_TLI).toString(), table.pausePrefix, table.pauseSuffix, Integer.toString(table.pauseDigits), Boolean.toString(table.pauseDecimalComma), Boolean.toString(table.undoEnabled) };
        ...
        if (dialog.editPreferences(values)) {
            ...
            java.util.prefs.Preferences.userRoot().node("org.sfb538.exmaralda.PartiturEditor").put("PlayerType", newValues[16]);
