...
public class SynchronizeTerminology extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        try {
            int no;
            sql = "SELECT DISTINCT ColumnName,  Name,Description, Help, EntityType " + "FROM	AD_COLUMN c WHERE NOT EXISTS " + "(SELECT 1 FROM AD_ELEMENT e " + " WHERE UPPER(c.ColumnName)=UPPER(e.ColumnName))";
            ...
            pstmt.close();
            rs.close();
            sql = "SELECT DISTINCT ColumnName, Name, Description, Help, EntityType " + " FROM	AD_PROCESS_PARA p " + " WHERE NOT EXISTS " + " (SELECT 1 FROM AD_ELEMENT e " + " WHERE UPPER(p.ColumnName)=UPPER(e.ColumnName))";
            ...
            sql = "DELETE	AD_ELEMENT e" + " 	WHERE NOT EXISTS" + " 	(SELECT 1 FROM AD_COLUMN c WHERE UPPER(e.ColumnName)=UPPER(c.ColumnName))" + " 	AND NOT EXISTS" + " 	(SELECT 1 FROM AD_PROCESS_PARA p WHERE UPPER(e.ColumnName)=UPPER(p.ColumnName))";
            no = DB.executeUpdate(sql, false, get_TrxName());
            log.info("  rows deleted: " + no);
            ...
            no = DB.executeUpdate(sql, false, get_TrxName());
            log.info("  rows updated: " + no);
            sql = "UPDATE	AD_PROCESS_PARA f" + " SET	ColumnName = (SELECT e.ColumnName FROM AD_ELEMENT e" + " 			WHERE e.ColumnName=f.ColumnName)" + " WHERE f.IsCentrallyMaintained='Y' AND f.IsActive='Y'" + " AND EXISTS (SELECT 1 FROM AD_ELEMENT e" + " WHERE UPPER(e.ColumnName)=UPPER(f.ColumnName)" + " AND e.ColumnName<>f.ColumnName)";
            no = DB.executeUpdate(sql, false, get_TrxName());
            log.info("  rows updated: " + no);
            ...
            sql = "UPDATE	AD_MENU m" + " SET		Name = (SELECT Name FROM AD_WINDOW w WHERE m.AD_Window_ID=w.AD_Window_ID)," + " Description = (SELECT Description FROM AD_WINDOW w WHERE m.AD_Window_ID=w.AD_Window_ID)" + " WHERE	AD_Window_ID IS NOT NULL" + " AND Action = 'W'";
            no = DB.executeUpdate(sql, false, get_TrxName());
            log.info("  rows updated: " + no);
            sql = "UPDATE	AD_MENU_TRL mt" + " SET		Name = (SELECT wt.Name FROM AD_WINDOW_TRL wt, AD_MENU m " + " WHERE mt.AD_Menu_ID=m.AD_Menu_ID AND m.AD_Window_ID=wt.AD_Window_ID " + " AND mt.AD_LANGUAGE=wt.AD_LANGUAGE)," + " Description = (SELECT wt.Description FROM AD_WINDOW_TRL wt, AD_MENU m " + " WHERE mt.AD_Menu_ID=m.AD_Menu_ID AND m.AD_Window_ID=wt.AD_Window_ID " + " AND mt.AD_LANGUAGE=wt.AD_LANGUAGE)," + " IsTranslated = (SELECT wt.IsTranslated FROM AD_WINDOW_TRL wt, AD_MENU m " + " WHERE mt.AD_Menu_ID=m.AD_Menu_ID AND m.AD_Window_ID=wt.AD_Window_ID " + " AND mt.AD_LANGUAGE=wt.AD_LANGUAGE)" + " WHERE EXISTS (SELECT 1 FROM AD_WINDOW_TRL wt, AD_MENU m " + " WHERE mt.AD_Menu_ID=m.AD_Menu_ID AND m.AD_Window_ID=wt.AD_Window_ID " + " AND mt.AD_LANGUAGE=wt.AD_LANGUAGE" + " AND m.AD_Window_ID IS NOT NULL" + " AND m.Action = 'W')";
            no = DB.executeUpdate(sql, false, get_TrxName());
            log.info("  rows updated: " + no);
            log.info("Synchronizing Menu with Processes");
            sql = "UPDATE	AD_MENU m" + " SET		Name = (SELECT p.Name FROM AD_PROCESS p WHERE m.AD_Process_ID=p.AD_Process_ID)," + " Description = (SELECT p.Description FROM AD_PROCESS p WHERE m.AD_Process_ID=p.AD_Process_ID)" + " WHERE	m.AD_Process_ID IS NOT NULL" + " AND	m.Action IN ('R', 'P')";
            no = DB.executeUpdate(sql, false, get_TrxName());
            log.info("  rows updated: " + no);
