...
public class MotorTableViewerEx extends JPanel {
    ...
    protected static final String[] PARAM_NAMES = new String[] { "State", "Status", "Device name", "Controller", "Index", "SimulationMode", "Position", "DialPosition", "Offset", "Step_per_unit", "Backlash", "Base_rate", "Acceleration", "Deceleration", "Velocity" };

    protected static final int STATE = 0, STATUS = 1, DEV_NAME = 2, CONTROLLER = 3, INDEX = 4, SIMMODE = 5, POSITION = 6, DPOS = 7, OFFSET = 8, SPUNIT = 9, BACKLASH = 10, BASERATE = 11, ACCEL = 12, DECEL = 13, VELOCITY = 14;
    ...
    class MotorsTableModel extends AbstractTableModel {
        ...
        public String getParamName(int row) {
            if (row < PARAM_NAMES.length) return PARAM_NAMES[row];
        ...
        public String getColumnName(int columnIndex) {
            ...
            columnIndex--;
            if (columnIndex < motors.size()) return motors.get(columnIndex).getName();
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            columnIndex--;
            if (columnIndex < motors.size()) return motors.get(columnIndex);
            columnIndex -= motors.size();
            return pseudoMotors.get(columnIndex);
        }

        public boolean isCellEditable(int rowIndex, int columnIndex) {
            if (columnIndex == 0 || rowIndex < 5) return false;
        ...
        protected String buildParamLabel(String paramName, AttributeInfoEx info) {
            String lbl = "";
            if (info == null) lbl = paramName; else {
                lbl = info.label;
                if (info.label != null && info.label.length() > 0) lbl = info.label; else lbl = paramName;
                if (info.unit != null && info.unit.length() > 0 && !info.unit.equalsIgnoreCase("No Unit")) lbl += "(" + info.unit + ")";
            }
            return lbl;
        }
    }
