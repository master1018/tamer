...
public class AlignmentRecord {
    ...
    public AlignmentRecord(int refid, int refstart, int refend, int differences, boolean rc) {
        m_refID = refid;
        m_refStart = refstart;
        m_refEnd = refend;
    ...
    public AlignmentRecord(AlignmentRecord other) {
        set(other);
    }

    public AlignmentRecord(Text t) throws IOException {
        fromText(t);
    }

    public AlignmentRecord(BytesWritable b) throws IOException {
        fromBytes(b);
    }
    ...
    public String toString() {
        builder.setLength(0);
        builder.append(m_refID);
        builder.append('\t');
        builder.append(m_isRC ? 1 : 0);
        builder.append('\t');
        builder.append(m_refStart);
        builder.append('\t');
        builder.append(m_refEnd);
        builder.append('\t');
    ...
    public void fromBytes(BytesWritable t) {
        byte[] raw = t.getBytes();
        m_isRC = raw[0] == 1;
        m_refID = (raw[1] & 0xFF) << 24 | (raw[2] & 0xFF) << 16 | (raw[3] & 0xFF) << 8 | (raw[4] & 0xFF);
        m_refStart = (raw[5] & 0xFF) << 24 | (raw[6] & 0xFF) << 16 | (raw[7] & 0xFF) << 8 | (raw[8] & 0xFF);
        m_refEnd = (raw[9] & 0xFF) << 24 | (raw[10] & 0xFF) << 16 | (raw[11] & 0xFF) << 8 | (raw[12] & 0xFF);
        m_differences = (raw[13] & 0xFF) << 24 | (raw[14] & 0xFF) << 16 | (raw[15] & 0xFF) << 8 | (raw[16] & 0xFF);
    }
}
