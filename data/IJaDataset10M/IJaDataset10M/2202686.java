...
public class BezierPatchGenerator extends PatchGenerator {
    ...
    public BezierPatchGenerator(int widthFacets, int depthFacets) {
        if (widthFacets < 3) throw new IllegalArgumentException("Number of width facets is < 3");
        if (depthFacets < 3) throw new IllegalArgumentException("Number of depth facets is < 3");
    ...
    protected void regeneratePatch() {
        ...
        if ((patchCoordinates == null) || (numPatchValues > patchCoordinates.length) || (numPatchValues > patchCoordinates[0].length)) {
            patchCoordinates = new float[depthFacetCount + 1][numPatchValues];
        }
        if (useControlPointWeights) regenerateWeightedPatch(); else regenerateStandardPatch();
    }
    ...
    private void regenerateStandardPatch() {
        ...
        int ncp = numDepthControlPoints * 3;
        patchCoordinates[depthFacetCount][cnt++] = controlPointCoordinates[numWidthControlPoints - 1][ncp - 3];
        patchCoordinates[depthFacetCount][cnt++] = controlPointCoordinates[numWidthControlPoints - 1][ncp - 2];
        patchCoordinates[depthFacetCount][cnt++] = controlPointCoordinates[numWidthControlPoints - 1][ncp - 1];
    }
    ...
    private double bezierBlend(int k, double mu, int n) {
        ...
        if (k > 0) blend *= Math.pow(mu, (double) k);
        if (n - k > 0) blend *= Math.pow(1 - mu, (double) (n - k));
        return blend;
    }
}
