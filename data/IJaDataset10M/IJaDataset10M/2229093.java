package de.swm.gwt.client.mobile.manifest;
...
public abstract class BaseMobileAppCacheLinker extends AbstractLinker {
	 ...
	 *            der name der start html datei (Landingpage)
	 */
    public BaseMobileAppCacheLinker(boolean isRoot, String warName, String landingpageName) {
        this.isRoot = isRoot;
        this.warName = null;
        this.landingpageName = landingpageName;
    }
    ...
    public ArtifactSet link(TreeLogger logger, LinkerContext context, ArtifactSet artifacts, boolean onePermutation) throws UnableToCompleteException {
        ...
        if (toReturn.find(SelectionInformation.class).isEmpty()) {
            logger.log(TreeLogger.INFO, "DevMode warning: Clobbering " + MANIFEST + " to allow debugging. " + "Recompile before deploying your app!");
            artifacts = null;
        }
    ...
    private Artifact<?> emitLandingPageCacheManifest(LinkerContext context, TreeLogger logger, ArtifactSet artifacts) throws UnableToCompleteException {
        ...
        if (artifacts != null) {
            for (@SuppressWarnings("rawtypes") Artifact artifact : artifacts) {
                if (artifact instanceof EmittedArtifact) {
                    ...
                    if (pathName.endsWith("symbolMap") || pathName.endsWith(".xml.gz") || pathName.endsWith("rpc.log") || pathName.endsWith("gwt.rpc") || pathName.endsWith("manifest.txt") || pathName.startsWith("rpcPolicyManifest") || pathName.endsWith("hosted.html") || pathName.startsWith("soycReport/")) {
                    } else {
        ...
        sb.append("NETWORK:\n");
        sb.append("*\n");
        logger.log(TreeLogger.INFO, "Bitte sicherstellen, dass die html Start-Seite das <html> Tag :" + " <html manifest=" + context.getModuleFunctionName() + "/" + MANIFEST + "\"> definiert!");
