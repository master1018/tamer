...
public class JcrDigitalObjectManagerImpl implements DigitalObjectManager {
	 ...
	 */
    public DigitalObjectContent retrieveContent(URI pdURI) throws ItemNotFoundException, RepositoryException, URISyntaxException {
	 ...
	 */
    public InputStream retrieveContentAsStream(URI pdURI) throws ItemNotFoundException, RepositoryException, URISyntaxException, DigitalObjectNotFoundException {
	 ...
	 */
    public int remove(URI permanentURI) throws DigitalObjectNotRemovedException {
	 ...
	 * @throws DigitalObjectNotRemovedException
	 */
    public int removeAll() throws DigitalObjectNotRemovedException {
        int res = DOJCRConstants.RESULT_OK;
        try {
	 ...
	 * @see eu.planets_project.ifr.core.storage.api.DigitalObjectManager#retrieve(java.net.URI)
	 */
    public DigitalObject retrieve(URI pdURI, boolean includeContent) throws DigitalObjectNotFoundException {
	 ...
	 * @throws DigitalObjectNotFoundException
	 */
    public DigitalObject updateDigitalObject(DigitalObject newObject, boolean includeContent) throws DigitalObjectUpdateException {
        DigitalObject retObj = null;
        try {
	 ...
	 * @throws DigitalObjectNotStoredException
	 */
    public DigitalObject store(URI uri, DigitalObject digitalObject, boolean includeContent) throws DigitalObjectNotStoredException {
        DigitalObject res = null;
        try {
            Event eIngest = buildIngestEvent(includeContent);
            List<Event> lEvents = digitalObject.getEvents();
            lEvents.add(eIngest);
            DigitalObject digoUpdated = new DigitalObject.Builder(digitalObject.getContent()).title(digitalObject.getTitle()).permanentUri(digitalObject.getPermanentUri()).manifestationOf(digitalObject.getManifestationOf()).format(digitalObject.getFormat()).metadata((Metadata[]) digitalObject.getMetadata().toArray(new Metadata[0])).events((Event[]) lEvents.toArray(new Event[0])).build();
    ...
    public Event buildIngestEvent(boolean isIncludeContent) {
        ...
        Property pIngestContent = new Property.Builder(URI.create("planets://data/repository/jcr/contentByReference")).name("content by reference").value(new Boolean(isIncludeContent).toString()).description("This flag is true if content should be stored in JCR and false if not").unit("boolean").type("repository characteristic").build();
    ...
    public Agent getAgent() {
        if (agent == null) agent = new Agent("JCR Repository v1.0", "The Planets Jackrabbit Content Repository", "planets://data/repository");
        return agent;
    }
