...
public class WebAppClassLoader extends URLClassLoader {
    ...
    public WebAppClassLoader(ClassLoader parent, WebAppContext context) throws IOException {
        ...
        _parent = getParent();
        _context = context;
        if (_parent == null) throw new IllegalArgumentException("no parent classloader!");
        if (context.getExtraClasspath() != null) addClassPath(context.getExtraClasspath());
    }
    ...
    public boolean isServerPath(String name) {
        ...
        if (server_classes != null) {
            for (int i = 0; i < server_classes.length; i++) {
                ...
                if (c.endsWith(".")) {
                    if (name.startsWith(c)) return result;
                } else if (name.equals(c)) return result;
            }
        }
        return false;
    }
    ...
    protected synchronized Class loadClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        if (c == null && _parent != null && !tried_parent && !isServerPath(name)) c = _parent.loadClass(name);
        if (c == null) throw ex;
    ...
    public String toString() {
        if (Log.isDebugEnabled()) return "ContextLoader@" + _name + "(" + LazyList.array2List(getURLs()) + ") / " + _parent;
