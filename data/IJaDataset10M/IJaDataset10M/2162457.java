...
public class BaseSolver extends AbstractTypestateSolver {
   ...
   */
    ...
    protected TypeStateProblem createTypeStateProblem(ICFGSupergraph supergraph, Collection<InstanceKey> instances) throws WalaException {
        return new BaseProblem(getCallGraph(), getPointerAnalysis(), supergraph, getDomain(), getDFA(), instances, getLiveObjectAnalysis(), getTraceReporter(), getMergeFactory().create(getDomain()));
    }
   ...
   */
    protected Collection<CGNode> computeNodesThatMatter(final OrdinalSet<InstanceKey> instances) throws WalaException, PropertiesException {
        if (getOptions().shouldSliceSupergraph()) {
            Assertions.productionAssertion(getLiveObjectAnalysis() != null, "expected non-null live object analysis in order to slice supergraph");
            if (DEBUG_LEVEL > 0) {
