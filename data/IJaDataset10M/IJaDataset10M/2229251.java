...
public class CompositeIntervalMapping extends AbstractMapping {
    ...
    public ArrayList MappingProcedure(int i_rep, int isPermutation) {
        ...
        if (isPermutation == 0) {
            ...
            ap.Swith2Permutation(Param1.switch2permutation, Param1.seed * 100000 + isPermutation);
            ...
            for (Iterator e = com.iterator(); e.hasNext(); ) {
                ...
                for (int j = 0; j < iip.length; j++) {
                    ...
                    for (int jj = 0; jj < steps; jj++) {
                        double[][] H1Matrix = imb.getCIMMatrixAtPoint(selectedMarker, s, weight, jj);
                        LinearRegression H1lm = new LinearRegression(H1Matrix, Y);
                        H1lm.MLE();
                        Likelihood lkhd1 = new Likelihood(ap, gs, s);
                        double log1 = lkhd1.LogLikelihoodAlternativeCIM(H1lm, jj, weight);
                        ...
                        PointMappingStatistic pms = new PointMappingStatistic(ChrInt[j][0], ChrInt[j][1], jj, lod, additive, additive_sd, additive_t, additive_t_p, dominant, dominant_sd, dominant_t, dominant_t_p, df, wald, p_wald, weight.length, H1lm.getMSE());
                        t.add(pms);
                    }
                }
            }
