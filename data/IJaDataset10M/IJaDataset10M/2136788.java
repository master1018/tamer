...
abstract class LModifiablePointsShape extends LShape implements IModifiablePointsShape {
    ...
    public boolean setPoint(final double x, final double y, final int position) {
        if (!GLibUtilities.INSTANCE.isValidPoint(x, y) || position < -1 || position > points.size()) return false;
    ...
    public IPoint replacePoint(final IPoint pt, final int position) {
        if (!GLibUtilities.INSTANCE.isValidPoint(pt) || points.contains(pt) || position < -1 || position > points.size()) return null;
        IPoint pRemoved = points.remove(position == -1 ? points.size() - 1 : position);
        if (position == -1 || points.isEmpty()) points.add(pt); else points.add(position, pt);
        return pRemoved;
    }
    ...
    public void addPoint(final IPoint pt, final int position) {
        if (GLibUtilities.INSTANCE.isValidPoint(pt) && position >= -1 && position <= points.size()) if (position == -1 || position == points.size()) points.add(pt); else points.add(position, pt);
    }
}
