 ...
 * method calls the actual entry point in which developers / testers should place their code:
 * EntryPointForPERSISTEvaluators().
 * 
 * @author Patrick Robertson, DLR
 *
 */
public class D55_Test02_ContextSynchLogParser {

    private static final String TEST_CONTEXT = "ContextBroker.Master.Synchronisation";

    private static final String SOURCE_COMPONENT = "ContextMgmt.ContextBroker";

    private static final String CONTEXT_SYNCH_RESPONSE_TIME = "AttributeSynchronisation.ResponseTime";

    private static final int TEST_TABLE_INDEX = 2;
	 ...
	 * 
	 * YOU MUST NOT USE COLONS OR COMMAS!!!
	 * 
	...
	*/
    public static void EntryPointForPERSISTEvaluators() {
        EvaluationFilter filter = new DelayEvaluationFilter(TEST_CONTEXT, SOURCE_COMPONENT, CONTEXT_SYNCH_RESPONSE_TIME, TEST_TABLE_INDEX);
        if (null == filename) {
            filename = ".." + File.separator + ".." + File.separator + "PersistContainer" + File.separator + "PerformanceLog.log";
        }
        int numHistoBins = 10;
        PerformanceStatistic statistic = new PerformanceStatistic(D55_Test02_ContextSynchLogParser.ReturnResultsFromLogFile(filter, filename), numHistoBins);
    ...
    public static void main(String[] args) {
        ...
        D55_Test02_ContextSynchLogParser.EntryPointForPERSISTEvaluators();
    }
