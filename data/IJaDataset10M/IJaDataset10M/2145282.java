...
public class XMLLayoutPanel extends JPanel implements XMLLayoutRoot {
    ...
    public void setIsLayoutRoot(boolean isLayoutRoot) {
        if (isLayoutRoot) {
            ...
            if (layout.getDefaultFont() != null) titledBorder.setTitleFont(layout.getDefaultFont());
    ...
    public void minimizeWindow(final Window window) {
        if (windowButtonPanel != null) {
            ...
            if (title == null || title.length() == 0) title = "Window";
            final JButton maximizeButton = new JButton(title);
            if (layout.getDefaultTabColor() != null) maximizeButton.setBackground(layout.getDefaultTabColor());
            if (layout.getDefaultFont() != null) maximizeButton.setFont(layout.getDefaultFont());
    ...
    public boolean reload() {
        try {
            if (windowButtonPanel != null) windowButtonPanel.setBackground(layout.getDefaultBGColor());
            windowList.clear();
            ...
            if (layout.getRoot().getComponents().size() > 0) add((Component) layout.getRoot().getComponents().get(0), "Center");
