...
public class Saver implements Configurable {
    ...
    protected static final String MIXWCNT_FILE_VERSION = "mixwcnt1.0";

    protected static final String GAUCNT_FILE_VERSION = "1.0";
    ...
    public void save(String ext, double[][] data, int sizeMix, String modif) throws IOException {
        ...
        for (int g = 0; g < data.length; g++) {
            for (int i = 0; i < out.length; i++) out[i] = (float) data[g][i];
    ...
    public void save(double[][] mixw, double[][] means, double[][] vars, String modif) throws IOException {
        ...
        if (false) for (int testId = 0; testId < mixw.length; testId++) {
            ...
            if (testCumul != 0.0) System.err.format(Locale.US, "monophone vu %4d %7g\n", testId, testCumul);
        }
        {
    ...
    private void writeChar(DataOutputStream dos, char character) throws IOException {
    ...
    public void writeFloatArray(DataOutputStream dos, float[] data) throws IOException {
        for (int i = 0; i < data.length; i++) {
            writeFloat(dos, data[i]);
        }
    }

    protected void writeDouble(DataOutputStream dos, double val) throws IOException {
        if (swap) {
    ...
    protected void writeDoubleArray(DataOutputStream dos, double[] data) throws IOException {
        for (int i = 0; i < data.length; i++) {
    ...
    public static void main(String[] argv) throws IOException {
        ...
        for (int i = 0; i <= 15; i++) test[i] = i + 0.5;
        for (int i = 0; i <= 15; i++) dos.writeLong(Double.doubleToRawLongBits(test[i]));
        for (int i = 0; i <= 15; i++) dos.writeLong(Utilities.swapDouble(test[i]));
        dos.close();
    }
}
