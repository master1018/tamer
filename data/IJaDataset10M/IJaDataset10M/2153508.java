...
public class JspHelper {
    ...
    public static final UnixUserGroupInformation webUGI = UnixUserGroupInformation.createImmutable(conf.getStrings(WEB_UGI_PROPERTY_NAME));
    ...
    public void streamBlockInAscii(InetSocketAddress addr, long blockId, long genStamp, long blockSize, long offsetIntoBlock, long chunkSizeToView, JspWriter out) throws IOException {
        if (chunkSizeToView == 0) return;
        Socket s = new Socket();
        s.connect(addr, HdfsConstants.READ_TIMEOUT);
        s.setSoTimeout(HdfsConstants.READ_TIMEOUT);
        long amtToRead = Math.min(chunkSizeToView, blockSize - offsetIntoBlock);
        DFSClient.BlockReader blockReader = DFSClient.BlockReader.newBlockReader(s, addr.toString() + ":" + blockId, blockId, genStamp, offsetIntoBlock, amtToRead, conf.getInt("io.file.buffer.size", 4096));
    ...
    public void DFSNodesStatus(ArrayList<DatanodeDescriptor> live, ArrayList<DatanodeDescriptor> dead) {
        if (fsn != null) fsn.DFSNodesStatus(live, dead);
    }
    ...
    public String getInodeLimitText() {
        ...
        str += ".  Heap Size is " + FsShell.byteDesc(totalMemory) + " / " + FsShell.byteDesc(maxMemory) + " (" + used + "%) <br>";
        return str;
    }
    ...
    public void sortNodeList(ArrayList<DatanodeDescriptor> nodes, String field, String order) {
        class NodeComapare implements Comparator<DatanodeDescriptor> {
            ...
            static final int FIELD_NAME = 1, FIELD_LAST_CONTACT = 2, FIELD_BLOCKS = 3, FIELD_CAPACITY = 4, FIELD_USED = 5, FIELD_PERCENT_USED = 6, FIELD_NONDFS_USED = 7, FIELD_REMAINING = 8, FIELD_PERCENT_REMAINING = 9, SORT_ORDER_ASC = 1, SORT_ORDER_DSC = 2;
    ...
    public static void printPathWithLinks(String dir, JspWriter out, int namenodeInfoPort) throws IOException {
        try {
            ...
            out.print("<a href=\"browseDirectory.jsp" + "?dir=" + Path.SEPARATOR + "&namenodeInfoPort=" + namenodeInfoPort + "\">" + Path.SEPARATOR + "</a>");
    ...
    public static void createTitle(JspWriter out, HttpServletRequest req, String file) throws IOException {
        ...
        if (start != 0) file = "..." + file.substring(start, file.length());
