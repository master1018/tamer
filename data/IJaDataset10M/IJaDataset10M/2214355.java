...
public abstract class CheckNode extends DefaultMutableTreeNode implements Comparable {
    ...
    public static class State {
        ...
        public State(int state) {
            if (state == CheckNode.MIXED) color = Color.LIGHT_GRAY;
        }
        ...
        if (getChildCount() == 0) {
            ...
            for (Enumeration children = children(); children.hasMoreElements(); ) {
                ...
                if (childTreeState == MIXED_STATE) {
                    ...
                    break;
                } else if (child.isSelected) allDeselected = false; else if (!child.isSelected) allSelected = false;
            }
            treeState = allSelected ? ALL_SELECTED_STATE : allDeselected ? ALL_DESELECTED_STATE : MIXED_STATE;
