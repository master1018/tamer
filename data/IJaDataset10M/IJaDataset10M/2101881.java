...
public class CommentXMLOutputter extends XMLOutputter {
    ...
    public static XMLOutputter default_inst = new CommentXMLOutputter(DEFAULT_INDENT, true);
    ...
    static void getAttributeXML(Map map, StringBuffer attribs) {
        ...
        for (int i = 0; i < keys.length; i++) {
            attribs.append(" ").append(XMLWritter.getAttrib(keys[i].toString(), strings.get(keys[i]).toString()));
        }
    }
    ...
    static void subRef(Map map, String uri) {
        ...
        if (count != null) {
            ...
            if (value <= 0) map.remove(uri); else map.put(uri, new Integer(value));
        }
    }
    ...
    public String toXML(CompositeMap map, boolean namespace_in_root) {
        ...
        } else return toXMLWithPrefixMapping(0, map, null, null);
    }
    ...
    String toXMLWithPrefixMapping(int level, CompositeMap map, Map namespaces, Map prefix_mapping) {
        ...
        if (map.getChilds() == null) {
            ...
            if (map.getText() != null) {
                ...
                if (mGenerateCdata) childs.append(CDATA_BEGIN).append(map.getText()).append(CDATA_END); else childs.append(XMLWritter.escape(map.getText()));
            }
        } else {
        ...
        if (xmlns_declare != null) xml.append(xmlns_declare);
        xml.append(attribs);
        if (childs.length() > 0) {
            xml.append('>');
            if (need_new_line_local) xml.append(LINE_SEPARATOR);
            xml.append(childs);
            if (need_new_line_local) xml.append(indent_str);
