...
public class DatagramSocket {
        ...
        if (impl == null) {
            if (factory != null) {
                ...
                if (implClass == null) {
                    String prefix = null;
                    try {
                        prefix = (String) AccessController.doPrivileged(new sun.security.action.GetPropertyAction("impl.prefix.dgram", null));
                        if (prefix == null) {
    ...
    public synchronized void setReuseAddress(boolean on) throws SocketException {
        if (isClosed()) throw new SocketException("Socket is closed");
        if (oldImpl) getImpl().setOption(SocketOptions.SO_REUSEADDR, new Integer(on ? -1 : 0)); else getImpl().setOption(SocketOptions.SO_REUSEADDR, new Boolean(on));
    }
