...
public class VerifyResponse extends Message {
    ...
    protected static final List requiredFields = Arrays.asList(new String[] { "is_valid" });

    protected static final List optionalFields = Arrays.asList(new String[] { "ns", "invalidate_handle" });

    protected VerifyResponse(boolean compatibility) {
        setSignatureVerified(false);
        if (!compatibility) set("ns", OPENID2_NS);
    }
    ...
    public void setSignatureVerified(boolean verified) {
        if (DEBUG) _log.debug("Setting is_valid to: " + verified);
    ...
    public void validate() throws MessageException {
        ...
        if (!"true".equals(getParameterValue("is_valid")) && !"false".equals(getParameterValue("is_valid"))) {
            throw new MessageException("Invalid is_valid value in verification response: " + getParameterValue("is_valid"), OpenIDException.VERIFY_ERROR);
        }
    }
}
