...
public class UpperBoundsConstraintsManager implements ConstraintsManager {
	 ...
	 * previous list
	 */
    public UpperBoundsConstraintsManager(final List<Integer> nDurationGenes, final List<Double> upperBounds) {
    ...
    @Override
    public Tuple<Double, Double> getAllowedRange(final IChromosome chromosome, final int i) {
        if (i < 0 || i >= chromosome.size()) {
            throw new IllegalArgumentException("illegal index " + i + "for chromosome of size " + chromosome.size());
        }
        return new Tuple<Double, Double>(0d, getFreeSpace(chromosome, i));
    }
	 ...
	 */
    private final double getFreeSpace(final IChromosome chromosome, final int indexToMute) {
        int geneCount = 0;
        ...
        for (int i = 0; i < genes.length; i++) {
            ...
            if (gene instanceof DoubleGene) {
                ...
                throw new IllegalArgumentException("gene at index " + i + " is an instance of " + gene.getClass() + ", not a double gene");
            }
        }
        return freeSpace;
    }
    ...
    @Override
    public double getSimpleCrossOverCoef(final IChromosome firstMate, final IChromosome secondMate, final int crossingPoint) {
        ...
        for (int i = crossingPoint; i < mate1Genes.length; i++) {
            if (mate1Genes[i] instanceof DoubleGene) {
                if (countGenes == currentNGenes) {
                    ...
                    currentNGenes = nGenesIterator.next();
                    currentBound = boundsIterator.next();
                    minIndivCoef = Math.min(minIndivCoef, calculatePlanDurCoef(mate1PlanDuration, currentBound, crossOverSurplus));
                    crossOverSurplus = 0d;
	 ...
	 */
    private static double calculatePlanDurCoef(final double mate1Sum, final double upperBound, final double crossOverSurplus) {
    ...
    @Override
    public Tuple<Double, Double> getSingleCrossOverCoefInterval(final IChromosome firstMate, final IChromosome secondMate, final int crossingPoint) {
        return new Tuple<Double, Double>(0d, getMaxSingleCrossOverCoef(firstMate, secondMate, crossingPoint));
    }

    private double getMaxSingleCrossOverCoef(final IChromosome firstMate, final IChromosome secondMate, final int crossingPoint) {
        Gene[] mate1Genes = firstMate.getGenes();
        Gene[] mate2Genes = secondMate.getGenes();
        double mate1PlanDuration = 0d;
        double crossOverSurplus = 0d;
            ...
            if (mate1Genes[i] instanceof DoubleGene) {
                if (countGenes == currentNGenes) {
                    if (!crossingPointIsPast) {
                        ...
                        coef = calculatePlanDurCoef(mate1PlanDuration, currentBound, crossOverSurplus);
                ...
                if (i >= crossingPoint) {
                    crossOverSurplus = ((DoubleGene) mate2Genes[i]).doubleValue() - currentEpisodeDuration;
                }
                countGenes++;
            }
            if (i >= crossingPoint) {
        ...
        throw new RuntimeException("Single cross over coefficient computation failed! " + "Crossing point: " + crossingPoint + ", chromosome size " + mate1Genes.length + ", crossingPointIsPast: " + crossingPointIsPast);
    }
	 ...
	 */
    public void fillChromosome(final IChromosome chromosome) {
    ...
    double getRemainingFreeSpace(final IChromosome chromosome) {
        Iterator<Integer> nGenesIterator = this.nDurationGenes.iterator();
        Iterator<Double> boundsIterator = this.upperBounds.iterator();
        int remainingGenes = nGenesIterator.next();
        double remainingSpace = boundsIterator.next();
