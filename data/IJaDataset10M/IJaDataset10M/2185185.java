...
public class LibSVMSaver extends AbstractFileSaver implements BatchConverter, IncrementalConverter {
   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (tmpStr.length() != 0) setClassIndex(tmpStr); else setClassIndex("last");
    }
   ...
   */
    public void writeBatch() throws IOException {
        ...
        if ((retrieveFile() == null) && (getWriter() == null)) {
            for (int i = 0; i < getInstances().numInstances(); i++) System.out.println(instanceToLibsvm(getInstances().instance(i)));
            setWriteMode(WAIT);
        } else {
            PrintWriter outW = new PrintWriter(getWriter());
            for (int i = 0; i < getInstances().numInstances(); i++) outW.println(instanceToLibsvm(getInstances().instance(i)));
            outW.flush();
            outW.close();
            setWriteMode(WAIT);
            outW = null;
