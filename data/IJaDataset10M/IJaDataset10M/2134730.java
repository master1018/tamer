...
abstract class BasicDirectModule extends AbstractDirectControllable implements Module {
    ...
    private class PlayerConnectionInfo {
        ...
        boolean canAddMIDIChannel(int channel) {
            return ((_added_wholly == false) && ((_channels_mask & (1 << channel)) == 0));
        }
    ...
    private static boolean isPlayerStateAcceptable(Player p) {
        return (p.getState() != Player.PREFETCHED && p.getState() != Player.STARTED);
    }
    ...
    protected abstract void doAddMIDIChannel(Player player, int channel) throws MediaException;
    ...
    public void addMIDIChannel(Player player, int channel) throws MediaException {
        ...
        PlayerConnectionInfo conn = (PlayerConnectionInfo) _players.get(player);
        if (conn != null) {
    ...
    protected boolean isAddedWholly(Player p) {
        boolean added = false;
        PlayerConnectionInfo conn = (PlayerConnectionInfo) _players.get(p);
        if (conn != null) {
