...
public class FD_ALL extends Protocol {
    ...
    public Object up(Event evt) {
        ...
        switch(evt.getType()) {
            case Event.MSG:
                ...
                if (msg_counts_as_heartbeat) update(msg.getSrc());
                if (hdr == null) break;
                switch(hdr.type) {
    ...
    private void update(Address sender) {
        if (sender != null && !sender.equals(local_addr)) timestamps.put(sender, System.currentTimeMillis());
    }
    ...
    private void handleViewChange(View v) {
        ...
        keys.retainAll(mbrs);
        for (Address member : mbrs) update(member);
        if (has_at_least_two) {
    ...
    class HeartbeatSender implements Runnable {
        ...
        public void run() {
            ...
            if (log.isTraceEnabled()) log.trace(local_addr + " sent heartbeat to cluster");
            num_heartbeats_sent++;
        }
    }
    ...
    class TimeoutChecker implements Runnable {
        ...
        public void run() {
            if (log.isTraceEnabled()) log.trace("checking for expired senders, table is:\n" + printTimeStamps());
            ...
            for (Iterator<Entry<Address, Long>> it = timestamps.entrySet().iterator(); it.hasNext(); ) {
                ...
                if (diff > timeout) {
                    if (log.isTraceEnabled()) log.trace("haven't received a heartbeat from " + key + " for " + diff + " ms, suspecting it");
                    suspect(key);
                }
            }
        }
