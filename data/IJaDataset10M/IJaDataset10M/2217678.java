...
public class LicenseUtil implements ISO7816 {
     ...
     */
    static void xor(byte[] in1, short in1_o, byte[] in2, short in2_o, byte[] out, short out_o, short len) {
    ...
    static short min(short a, short b) {
        if (sign(a) == sign(b)) return (a < b ? a : b); else if (sign(a) == 1) return b; else return a;
    }
    ...
    static byte calcLcFromPaddedData(byte[] apdu, short offset, short length) {
        for (short i = (short) (length - 1); i >= 0; i--) if (apdu[(short) (offset + i)] != 0) if ((apdu[(short) (offset + i)] & 0xff) != 0x80) return (byte) (length & 0xff); else return (byte) (i & 0xff);
        return 0;
    }
}
