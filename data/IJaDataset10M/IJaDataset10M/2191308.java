...
public final class Element2Latex extends AbstractModuleVisitor {
    ...
    public void addPredicate(final PredicateDefinition definition) {
        getPredicateDefinitions().put(definition.getName() + "_" + definition.getArgumentNumber(), definition);
    }
    ...
    public void addFunction(final FunctionDefinition definition) {
        getFunctionDefinitions().put(definition.getName() + "_" + definition.getArgumentNumber(), definition);
    }
            ...
            if (definition == null) {
                try {
                    ...
                    if (external >= 0 && Element2Latex.this.getReferences() != null && Element2Latex.this.getReferences().size() > 0) {
                        ...
                        final KernelQedeqBo newProp = (KernelQedeqBo) Element2Latex.this.getReferences().getQedeqBo(label);
                        if (newProp != null) {
                            final String shortName = name.substring(external + 1);
                            if (newProp.getExistenceChecker().predicateExists(shortName, arguments)) {
                                final DefaultExistenceChecker checker = (DefaultExistenceChecker) newProp.getExistenceChecker();
