...
public class MetaDataDedupFile implements java.io.Externalizable {
    ...
    public String getXAttribute(String name) {
        if (this.extendedAttrs.containsKey(name)) return extendedAttrs.get(name); else return null;
    }
    ...
    public IOMonitor getIOMonitor() {
        if (monitor == null) monitor = new IOMonitor();
        return monitor;
    }
    ...
    public DedupFile getDedupFile() throws IOException {
        try {
            ...
            if (this.dfGuid == null) {
                ...
                SDFSLogger.getLog().debug("No DF EXISTS .... Set dedup file for " + this.getPath() + " to " + this.dfGuid);
                this.sync();
                return df;
            } else {
	 ...
	 * @throws IOException
	 */
    public MetaDataDedupFile snapshot(String snaptoPath, boolean overwrite) throws IOException {
        if (!this.isDirectory()) {
            SDFSLogger.getLog().debug("is snapshot file");
            File f = new File(snaptoPath);
            if (f.exists() && !overwrite) throw new IOException("path exists [" + snaptoPath + "]Cannot overwrite existing data "); else if (f.exists()) {
            ...
            _mf.getIOMonitor().setDuplicateBlocks(this.getIOMonitor().getDuplicateBlocks() + this.getIOMonitor().getActualBytesWritten());
    ...
    public void copyTo(String npath, boolean overwrite) throws IOException {
        ...
        File f = new File(snaptoPath);
        if (f.exists() && !overwrite) throw new IOException("path exists [" + snaptoPath + "]Cannot overwrite existing data ");
        if (!this.isDirectory()) {
            if (!f.getParentFile().exists()) f.getParentFile().mkdirs();
            Path p = f.toPath();
            ...
            String cpCmd = "cp -rf --preserve=mode,ownership,timestamps " + this.path + " " + snaptoPath;
            Process p = Runtime.getRuntime().exec(cpCmd);
            try {
                int ecode = p.waitFor();
                if (ecode != 0) throw new IOException("unable to copy " + this.path + " cp exit code is " + ecode);
            } catch (Exception e) {
                throw new IOException(e);
            }
    ...
    private void copyDir(String npath) throws IOException {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (file.isDirectory()) file.copyDir(npath); else {
                SDFSLogger.getLog().debug("copy dedup file for : " + file.getPath() + " guid :" + file.getDfGuid());
                if (file.dfGuid != null) {
                    if (DedupFileStore.fileOpen(file)) file.getDedupFile().copyTo(npath); else {
                        File sdbdirectory = new File(Main.dedupDBStore + File.separator + file.dfGuid.substring(0, 2) + File.separator + file.dfGuid);
                        Path sdbf = new File(sdbdirectory.getPath() + File.separator + file.dfGuid + ".map").toPath();
                        Path sdbc = new File(sdbdirectory.getPath() + File.separator + file.dfGuid + ".chk").toPath();
                        ...
                        Path ddbc = new File(ddbdir.getPath() + File.separator + file.dfGuid + ".chk").toPath();
                        Files.copy(sdbf, ddbf, StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.COPY_ATTRIBUTES);
                        Files.copy(sdbc, ddbc, StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.COPY_ATTRIBUTES);
                    }
                }
            }
        }
    ...
    public long lastModified() {
        if (this.isDirectory()) return new File(this.path).lastModified();
        return lastModified;
    }
