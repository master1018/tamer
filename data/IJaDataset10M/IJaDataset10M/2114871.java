...
public class RankList extends AbstractSequentialList {
    ...
    public ListIterator listIterator(int index) {
        if (index == 0) return new CellIterator(null);
        index--;
        Cell pos = head.s(dim, index);
        if (pos == null) throw new IndexOutOfBoundsException("" + index);
        return new CellIterator(pos);
    }

    public int size() {
        int i = 1;
        for (Cell c = head.s(dim); c != null && !c.equals(head); c = c.s(dim)) i++;
        return i;
    }
	 ...
	 */
        ...
        public Object next() {
            if (pos == null) return (pos = head); else if (hasNext()) return (pos = pos.s(dim)); else throw new NoSuchElementException("after " + pos + " on " + dim);
        }
        ...
        public Object previous() {
            if (pos != null) {
                Cell result = pos;
                if (pos.equals(head)) pos = null; else pos = pos.s(dim, -1);
                return result;
            } else throw new NoSuchElementException("before " + pos + " on " + dim);
        }
        ...
        public int previousIndex() {
            int i = -1;
            for (Cell c = pos; c != null && !c.equals(head); c = c.s(dim, -1)) i++;
            return i;
        }
