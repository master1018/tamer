...
public class SeekBarTest extends ActivityInstrumentationTestCase2<SeekBarStubActivity> {
    ...
    @TestTargets({ @TestTargetNew(level = TestLevel.COMPLETE, method = "SeekBar", args = { android.content.Context.class }), @TestTargetNew(level = TestLevel.COMPLETE, method = "SeekBar", args = { android.content.Context.class, android.util.AttributeSet.class }), @TestTargetNew(level = TestLevel.COMPLETE, method = "SeekBar", args = { android.content.Context.class, android.util.AttributeSet.class, int.class }) })
    ...
    @TestTargetNew(level = TestLevel.COMPLETE, method = "setOnSeekBarChangeListener", args = { android.widget.SeekBar.OnSeekBarChangeListener.class })
    public void testSetOnSeekBarChangeListener() {
        ...
        MotionEvent event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_DOWN, seekBarXY[0], seekBarXY[1], 0);
        ...
        event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_MOVE, seekBarXY[0] + (mSeekBar.getWidth() >> 1), seekBarXY[1], 0);
        mInstrumentation.sendPointerSync(event);
        mInstrumentation.waitForIdleSync();
        assertTrue(listener.hasCalledOnProgressChanged());
        listener.reset();
        downTime = SystemClock.uptimeMillis();
        eventTime = SystemClock.uptimeMillis();
        event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_UP, seekBarXY[0] + (mSeekBar.getWidth() >> 1), seekBarXY[1], 0);
        mInstrumentation.sendPointerSync(event);
        mInstrumentation.waitForIdleSync();
