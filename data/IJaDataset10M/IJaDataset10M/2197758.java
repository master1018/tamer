...
public class StringDataUtils {
   ...
   */
    public static String reduceDataPrecision(String data) {
        if (data.length() > 10) {
            if (data.indexOf('E') != -1) data = data.substring(0, data.indexOf('E') - 1) + "e" + data.substring(data.indexOf('E') + 1);
            if (data.indexOf('e') == -1) data = data.substring(0, 9); else data = String.valueOf(Math.pow(10, Double.valueOf(data.substring(data.indexOf('e') + 1)).doubleValue()) * Double.valueOf(data.substring(0, Math.min(9, data.indexOf('e') - 1))).doubleValue());
        }
        return data;
    }
}
