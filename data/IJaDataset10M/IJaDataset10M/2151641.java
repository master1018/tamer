...
public class PickObject extends Object {
   ...
   */
    public PickShape generatePickRay(int xpos, int ypos) {
        ...
        if (canvas.getView().getProjectionPolicy() == View.PARALLEL_PROJECTION) {
        ...
        if (debug) {
            ...
            System.out.println("after, mouse " + mousePosn + " eye " + eyePosn + " mouseVec " + mouseVec);
        }
   ...
   */
    ...
    private SceneGraphPath[] pickGeomAll(int xpos, int ypos) {
        ...
        pickRay = (PickRay) generatePickRay(xpos, ypos);
        sceneGraphPathArr = pickRoot.pickAll(pickRay);
        if (sceneGraphPathArr == null) return null;
        ...
        for (i = 0; i < sceneGraphPathArr.length; i++) {
            ...
            if (obj instanceof Shape3D) {
                found[i] = ((Shape3D) obj).intersect(sceneGraphPathArr[i], (PickShape) pickRay);
            } else if (obj instanceof Morph) {
                found[i] = ((Morph) obj).intersect(sceneGraphPathArr[i], (PickShape) pickRay);
            }
            if (found[i] == true) cnt++;
        }
        ...
        cnt = 0;
        for (i = 0; i < sceneGraphPathArr.length; i++) {
            if (found[i] == true) newSceneGraphPathArr[cnt++] = sceneGraphPathArr[i];
        }
    ...
    private SceneGraphPath[] pickGeomAllSorted(int xpos, int ypos) {
        ...
        for (i = 0; i < sceneGraphPathArr.length; i++) {
            obj = sceneGraphPathArr[i].getObject();
            if (obj instanceof Shape3D) {
                found[i] = ((Shape3D) obj).intersect(sceneGraphPathArr[i], pickRay, dist);
                distArr[i] = dist[0];
            } else if (obj instanceof Morph) {
                found[i] = ((Morph) obj).intersect(sceneGraphPathArr[i], pickRay, dist);
                distArr[i] = dist[0];
            }
            if (found[i] == true) cnt++;
        }
    ...
    private SceneGraphPath pickGeomAny(int xpos, int ypos) {
        ...
        for (i = 0; i < sceneGraphPathArr.length; i++) {
            obj = sceneGraphPathArr[i].getObject();
            if (obj instanceof Shape3D) {
                if (((Shape3D) obj).intersect(sceneGraphPathArr[i], (PickShape) pickRay)) return sceneGraphPathArr[i];
            } else if (obj instanceof Morph) {
                if (((Morph) obj).intersect(sceneGraphPathArr[i], (PickShape) pickRay)) return sceneGraphPathArr[i];
            }
        }
        return null;
    }
        ...
        if (sgPath != null) {
            ...
            if ((pickedNode instanceof Shape3D) && ((flags & SHAPE3D) != 0)) {
                ...
                for (int j = sgPath.nodeCount() - 1; j >= 0; j--) {
                    ...
                    if ((pickedNode instanceof Primitive) && ((flags & PRIMITIVE) != 0)) {
                        ...
                        return pickedNode;
                    } else if ((pickedNode instanceof TransformGroup) && ((flags & TRANSFORM_GROUP) != 0)) {
                        if (debug) System.out.println("xform group found");
                        return pickedNode;
                    } else if ((pickedNode instanceof BranchGroup) && ((flags & BRANCH_GROUP) != 0)) {
                ...
                if (pickedNode == null) if (debug) System.out.println("ERROR: null SceneGraphPath");
            }
        }
        return null;
    }
            ...
            if ((pickedNode instanceof Shape3D) && ((flags & SHAPE3D) != 0)) {
                ...
                for (int j = 0; j < sgPath.nodeCount(); j++) {
                    ...
                    if ((pickedNode instanceof Group) && ((flags & GROUP) != 0)) {
                        if (debug) System.out.println("Group found");
                        curCnt++;
                        if (curCnt == occurrence) return pickedNode;
                    } else if ((pickedNode instanceof BranchGroup) && ((flags & BRANCH_GROUP) != 0)) {
                        if (debug) System.out.println("Branch group found");
                        curCnt++;
                        if (curCnt == occurrence) return pickedNode;
                        ...
                        curCnt++;
                        if (curCnt == occurrence) return pickedNode;
                    } else if ((pickedNode instanceof Primitive) && ((flags & PRIMITIVE) != 0)) {
                        if (debug) System.out.println("Primitive found");
                        curCnt++;
                        if (curCnt == occurrence) return pickedNode;
                    } else if ((pickedNode instanceof Link) && ((flags & LINK) != 0)) {
