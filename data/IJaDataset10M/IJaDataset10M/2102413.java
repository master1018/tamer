...
public class TypeNameFactory {
    ...
    private static String getPrimitiveDescriptor(String name) {
        ...
        if (postArray.equals("byte")) return array + "B";
        if (postArray.equals("char")) return array + "C";
        if (postArray.equals("double")) return array + "D";
        if (postArray.equals("float")) return array + "F";
        if (postArray.equals("int")) return array + "I";
        if (postArray.equals("long")) return array + "J";
        if (postArray.equals("short")) return array + "S";
        if (postArray.equals("boolean")) return array + "Z";
        if (postArray.equals("void")) return array + "V";
        return null;
    }
	 ...
	 * @author Gili Tzabari
	 */
    private abstract static class AbstractTypeName implements TypeName {
    ...
    private static class FromDescriptor extends AbstractTypeName {
        ...
        public String asIdentifier() {
            ...
            for (int len = descriptor.length(); arrayOffset < len; ++arrayOffset) {
                if (descriptor.charAt(arrayOffset) != '[') break;
            }
            ...
            if (descriptor.length() > arrayOffset + 1) {
                if (postArray.charAt(0) != 'L') throw new IllegalArgumentException("descriptor must begin with a '[' or L': " + descriptor);
                if (postArray.charAt(postArray.length() - 1) != ';') throw new IllegalArgumentException("descriptor must end with a ';': " + descriptor);
                return (array + postArray.substring(1, postArray.length() - 1)).replace("/", ".");
            } else {
                if (postArray.equals("B")) return array + "byte"; else if (postArray.equals("C")) return array + "char"; else if (postArray.equals("D")) return array + "double"; else if (postArray.equals("F")) return array + "float"; else if (postArray.equals("I")) return array + "int"; else if (postArray.equals("J")) return array + "long"; else if (postArray.equals("S")) return array + "short"; else if (postArray.equals("Z")) return array + "boolean"; else if (postArray.equals("V")) return array + "void"; else throw new IllegalArgumentException("descriptor is not a valid primitive type: " + descriptor);
            }
        }
    ...
    private static class FromIdentifier extends AbstractTypeName {
        ...
        @Override
        public String asPath() {
            throw new UnsupportedOperationException("There is no reliable way to convert an identifier to a " + "path: " + name);
        }
