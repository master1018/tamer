...
public class CCNNameEnumerator implements CCNFilterListener, CCNInterestListener {
    ...
    private class NERequest {
        ...
        Interest getInterest(ContentName in) {
            for (Interest i : ongoingInterests) if (i.name().equals(in)) return i;
            return null;
        }
        ...
        void addInterest(Interest i) {
            if (getInterest(i.name()) == null) ongoingInterests.add(i);
        }
        ...
        public boolean containsInterest(Interest interest) {
            for (Interest i : ongoingInterests) {
                if (i.equals(interest)) return true;
            }
            return false;
        }
    }
    ...
    public void registerPrefix(ContentName prefix) throws IOException {
        synchronized (_currentRequests) {
            ...
            ContentName prefixMarked = new ContentName(prefix, CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION.getBytes());
            ...
            if (c != null) {
                ...
                if (responseName == null) {
                    ...
                    if (getIdFromName(interest.name()) != null && getIdFromName(interest.name()).count() > 0) {
                        ...
                        ContentName prefixWithMarker = new ContentName(prefix, CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION.getBytes());
                        Exclude excludes = interest.exclude();
                        if (excludes == null) excludes = new Exclude();
                        ...
                        if (!ner.containsInterest(newInterest)) newInterests.add(newInterest);
                    }
                }
                try {
                ...
                try {
                    ...
                    callback.handleNameEnumerator(interest.name().cut(CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION.getBytes()), names);
    ...
    public boolean containsRegisteredName(ContentName name) {
        ...
        synchronized (_handledResponses) {
            if (_registeredNames.contains(name)) return true; else return false;
        }
    }
    ...
    public void cancelEnumerationsWithPrefix(ContentName prefixToCancel) {
        ...
        synchronized (_currentRequests) {
            ...
            for (NERequest n : _currentRequests) {
                if (prefixToCancel.isPrefixOf(n.prefix)) toRemove.add(n);
            }
            while (!toRemove.isEmpty()) {
                if (cancelPrefix(toRemove.remove(0).prefix)) Log.info("cancelled prefix: {0}", prefixToCancel); else Log.info("could not cancel prefix: {0}", prefixToCancel);
            }
        }
    }
    ...
    private ContentName getIdFromName(ContentName name) {
        ...
        try {
            ...
            if (VersioningProfile.hasTerminalVersion(prefix)) responseName = VersioningProfile.cutLastVersion(prefix); else responseName = prefix;
