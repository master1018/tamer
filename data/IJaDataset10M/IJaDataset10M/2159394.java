...
public class ContextFreezeDryer extends AbstractFreezeDryer<DefaultContext<?>> {
    ...
    protected void storeChildren(FreezeDryedObject fdo, Context context) throws FreezeDryingException {
        for (Object child : (Collection<?>) context) {
            FreezeDryedParentChild cfdo = new FreezeDryedParentChild(context.getClass(), fdo.getId(), child.getClass(), registry.getId(child));
            fdo.addChild(cfdo);
        }
    }
    ...
    protected void storeProjections(FreezeDryedObject fdo, Context<?> context) throws FreezeDryingException {
        ...
        for (Projection proj : context.getProjections()) {
            ...
            if (dryer == null) {
                ...
                LOG.info("Could not find projection dryer for freeze drying projection '" + proj + "' in context '" + context + "', using the default freeze dryer.");
            } else {
        ...
        if (subContexts != null) {
            for (Object obj : subContexts) {
                if (obj instanceof Context) {
                    ...
                    LOG.warn("Found a non-context '" + obj + "' when loading sub contexts for context '" + context + "'.");
                }
            }
        }
    }
    ...
    private void loadValueLayers(FreezeDryedObject fdo, DefaultContext context) throws FreezeDryingException {
        ...
        for (Object obj : layers) {
            if (obj instanceof ValueLayer) {
                ...
                LOG.warn("Found a non-value layer '" + obj + "' when loading value layers for context '" + context + "'.");
            }
        }
    }
    ...
    protected void loadPropertyProjections(FreezeDryedObject fdo, Context context) throws FreezeDryingException {
        ...
        for (Object obj : propertyProjections) {
            if (obj instanceof Map) {
                ...
                if (dryer == null) {
                    LOG.warn("Could not find projection dryer for rehydrating projection with class '" + props.get(PROJECTION_TYPE_KEY) + "' in context '" + context + "'");
                } else {
                    dryer.buildAndAddProjection(context, props);
                }
            } else {
                LOG.warn("Found a non-Map '" + obj + "' when attempting to load up a property projection for context '" + context + "', ignoring and continuing.");
            }
        }
    }
