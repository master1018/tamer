...
public class InteractiveScatter extends Scatter implements ISingleColorable, IInteractive {
    ...
    public void draw(GL2 gl, GLU glu, Camera cam) {
        ...
        if (colors == null) gl.glColor4f(rgb.r, rgb.g, rgb.b, rgb.a);
        if (coordinates != null) {
            int k = 0;
            for (Coord3d c : coordinates) {
                if (colors != null) {
                    if (isHighlighted[k]) gl.glColor4f(highlightColor.r, highlightColor.g, highlightColor.b, highlightColor.a); else gl.glColor4f(colors[k].r, colors[k].g, colors[k].b, colors[k].a);
                    k++;
                }
    ...
    public void setData(Coord3d[] coordinates) {
        ...
        this.isHighlighted = new boolean[coordinates.length];
        bbox.reset();
        for (Coord3d c : coordinates) bbox.add(c);
    }
