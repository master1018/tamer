...
public class TextUtils {
     ...
     */
    public static final void changeCaretPosition(final TextKit textKit, final int newPos, final boolean isMovingCaret) {
    ...
    /**
      ...
      * depending on <code>isMovingCaret</code> value
      */
    public static final void changeCaretPosition(final TextKit textKit, final int newPos, final boolean isMovingCaret, final Position.Bias newBias) {
        TextCaret caret = textKit.getCaret();
        Point pt = caret.getMagicCaretPosition();
        if (isMovingCaret) {
    ...
    public static final int getNextWord(final Document doc, final int pos) throws BadLocationException {
        BreakIterator bi = BreakIterator.getWordInstance();
        int length = doc.getLength();
        if (pos < 0 || pos >= length) {
            throwException(Messages.getString("awt.2F"), pos);
        }
        String content = null;
        content = doc.getText(0, doc.getLength());
        bi.setText(content);
        int iteratorNextWord = bi.following(pos);
        while (iteratorNextWord < length && Character.isWhitespace(content.charAt(iteratorNextWord))) {
    ...
    public static final int getPreviousWord(final Document doc, final int pos) throws BadLocationException {
        BreakIterator bi = BreakIterator.getWordInstance();
        int length = doc.getLength();
        if (pos < 0 || pos > length) {
            throwException(Messages.getString("awt.2F"), pos);
        ...
        while (iteratorPrevWord > 0 && ((content.charAt(iteratorPrevWord) == ' ' || content.charAt(iteratorPrevWord) == '\n') || content.charAt(iteratorPrevWord) == '\t')) {
    ...
    public static final int getTabbedTextWidth(final Segment s, final FontMetrics fm, final int x, final TabExpander t, final int pos) {
        return getTabbedTextEnd(s, fm, x, t, pos, false, null, 0) - x;
    }

    public static final int getTabbedTextOffset(final Segment s, final FontMetrics fm, final int start, final int end, final TabExpander t, final int pos) {
     ...
     */
    public static final int getTabbedTextOffset(final Segment s, final FontMetrics fm, final int start, final int end, final TabExpander t, final int pos, final boolean round) {
        String str = "";
    ...
    public static final int drawTabbedText(final Segment s, final int x, final int y, final Graphics g, final TabExpander t, final int pos) {
        return getTabbedTextEnd(s, g.getFontMetrics(), x, t, pos, true, g, y);
    }

    public static final int drawComposedText(final TextKit textKit, final AttributedString text, final Graphics g, final int x, final int y) {
        ...
        final int width = g.getFontMetrics().getStringBounds(iterator, iterator.getBeginIndex(), iterator.getEndIndex(), g).getBounds().width;
        return width + x;
    }

    public static final int getBreakLocation(final Segment s, final FontMetrics fm, final int start, final int end, final TabExpander t, final int pos) {
        int offset = s.offset;
        int index = TextUtils.getTabbedTextOffset(s, fm, start, end, t, pos, false);
    ...
    public static final int getRowStart(final TextKit tk, final int pos) throws BadLocationException {
        ...
        if (pos < 0 || pos > length) {
            throwException(Messages.getString("awt.2A"), pos);
        }
    ...
    public static final int getWordEnd(final TextKit tk, final int pos) throws BadLocationException {
        Document doc = tk.getDocument();
        BreakIterator bi = BreakIterator.getWordInstance();
        int length = doc.getLength();
        if (pos < 0 || pos > length) {
            throwException(Messages.getString("awt.2B", pos), pos);
        }
    ...
    public static final int getWordStart(final TextKit tk, final int pos) throws BadLocationException {
        Document doc = tk.getDocument();
        BreakIterator bi = BreakIterator.getWordInstance();
        int length = doc.getLength();
        if (pos < 0 || pos > length) {
     ...
     */
    public static final int getPositionAbove(final TextKit textKit, final int p, final int x) throws BadLocationException {
     ...
     * and selects the closest one.
     */
    public static final int getPositionBelow(final TextKit textKit, final int p, final int x) throws BadLocationException {
    ...
    public static final int getRowEnd(final TextKit tk, final int pos) throws BadLocationException {
        Dimension d = tk.getVisibleRect().getSize();
        if (d != null && (d.height == 0 || d.width == 0)) {
            return -1;
        }
    ...
    public static final void setCurrentPositionAsMagic(final TextKit textKit) {
        ...
        try {
            Point pt = textKit.modelToView(newPos, caret.getDotBias()).getLocation();
     ...
     * paragraph element.
     */
    public static final Element getParagraphElement(final Document doc, final int p) {
        if (doc instanceof AbstractDocument) {
            ...
            try {
                ...
                incorrectPosition = (p < 0 || p > length) && (doc instanceof PlainDocument);
    ...
    private static int getTabbedTextEnd(final Segment s, final FontMetrics fm, final int x, final TabExpander t, final int pos, final boolean needDraw, final Graphics g, final int y) {
        int x1 = x < 0 ? x : 0;
        int res = x < 0 ? 0 : x;
        ...
        while (tabIndex >= 0) {
            ...
            res = (int) t.nextTabStop(res + fm.stringWidth(substr), tabIndex + currentIndex);
    ...
    private static void drawString(final String text, final boolean needDraw, final Graphics g, final int x, final int y) {
        if (needDraw) {
    ...
    private static Element getElementByPosition(final Element rootElement, final int pos) {
    ...
    private static int getNextVisualPosition(final View v, final int pos, final Position.Bias b0, final int direction, final Position.Bias[] biasRet) {
        ...
        if (!isBidirectional(document)) {
            return getTrivialVisualPosition(toWest, pos, b0, length, biasRet, true);
        }
        Element bidiRoot = ((AbstractDocument) document).getBidiRootElement();
        ...
        int posInNeighboringElement = toWest ? Math.max(start - 1, 0) : Math.min(end + 1, length);
        Element neighboringElement = getElementByPosition(bidiRoot, posInNeighboringElement);
        Element paragraph = getElementByPosition(document.getDefaultRootElement(), pos);
        int startParagraph = paragraph.getStartOffset();
        int endParagraph = paragraph.getEndOffset() - 1;
        boolean nextIsLTR = isLTR(getElementByPosition(bidiRoot, Math.min(endParagraph + 1, length)));
        int result = checkBoundaryCondition(pos, b0, biasRet, neighboringElement, isLTR, toWest, start, length, startParagraph, endParagraph, nextIsLTR);
        return result >= 0 ? result : getBidiVisualPosition(start, end, neighboringElement, pos, b0, biasRet, length, toWest, isLTR);
    }

    private static int checkBoundaryCondition(final int pos, final Position.Bias b0, final Position.Bias[] biasRet, final Element neighboringElement, final boolean isLTR, final boolean toWest, final int start, final int length, final int startParagraph, final int endParagraph, final boolean nextIsLTR) {
        if (toWest && isLTR) {
    ...
    static final int getBidiVisualPosition(final int start, final int end, final Element neighbouringElement, final int pos, final Position.Bias b0, final Position.Bias[] biasRet, final int length, final boolean toWest, final boolean isLTR) {
    ...
    private static int getTrivialVisualPosition(final boolean toWest, final int pos, final Position.Bias bias, final int docLength, final Position.Bias[] biasRet, final boolean isLTR) {
        boolean condition = (toWest && isLTR) || (!toWest && !isLTR);
        return condition ? Math.max(pos - 1, 0) : Math.min(pos + 1, docLength);
    }

    public static final Shape getFieldViewAllocation(final View v, final TextFieldKit tfk, final Shape shape, final ComponentOrientation orientation) {
    ...
    private static boolean isToLeft(final ComponentOrientation orientation, final int alignment) {
        boolean isRTL = !orientation.isLeftToRight();
        return isRTL && alignment == SwingConstants.TRAILING || !isRTL && alignment == SwingConstants.LEADING || alignment == SwingConstants.LEFT;
    }

    public static Shape paintLayer(final Graphics g, final int p0, final int p1, final Shape shape, final Color color, final View view, final boolean fill) {
        if (shape == null) {
            return null;
        }
        Shape result = null;
    ...
    public static ComposedTextParams getComposedTextParams(final TextKit tk) {
        Document doc = tk.getDocument();
        Object currentProperty = tk.getDocument().getProperty(PropertyNames.COMPOSED_TEXT_PROPERTY);
    ...
    public static void processIMEvent(final InputMethodListener listener, final InputMethodEvent e) {
    ...
    public static final Rectangle getBoundsByOffsets(final TextKit tk, final int p0, final int p1) {
        Rectangle r0 = null;
        Rectangle r1 = null;
        ...
        rect.height = Math.max(r0.y, r1.y) - rect.y + Math.max(r0.height, r1.height);
        return rect;
    }

    public static final void setNativeCaretPosition(final Rectangle rect, final Component comp) {
        ComponentInternals.getComponentInternals().setCaretPos(comp, rect.x, rect.y);
    }
    ...
    public static final void exportToClipboard(final TextKit textKit, final Clipboard clipboard, final int action) {
        if (textKit == null) {
            return;
        }
    ...
    public static final void exportDone(final TextKit textKit, final Transferable transferable, final int action) {
        if (textKit != null && (action & ActionNames.MOVE) > 0) {
            textKit.replaceSelectedText("");
        }
    }

    public static final boolean importData(final TextKit textKit, final Transferable t) {
        if (t == null) {
            return false;
        }
    ...
    public static int getSourceActions(final TextKit textKit) {
        if (textKit != null && !"javax.swing.JPasswordField".equals(textKit.getClass())) {
            return (textKit.isEditable()) ? ActionNames.COPY_OR_MOVE : ActionNames.COPY;
        }
    ...
    public static void setCharacterAttributes(final AttributeSet attr, final boolean replace, final JEditorPane editorPane, final StyledDocument doc, final MutableAttributeSet inputAttrs) {
    ...
    public static void setParagraphAttributes(final AttributeSet attr, final boolean replace, final JEditorPane editorPane, final StyledDocument doc) {
    ...
    public static void selectCalendarField(final JFormattedTextField textField, final int calendarField) {
    ...
    public static Object getNextValue(final Date value, final int calendarField, final Comparable<Date> end) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(value);
        calendar.add(calendarField, 1);
        Date result = calendar.getTime();
        return (end == null) ? result : (end.compareTo(result) < 0) ? null : result;
    }

    public static Object getPreviousValue(final Date value, final int calendarField, final Comparable<Date> start) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(value);
        calendar.add(calendarField, -1);
        Date result = calendar.getTime();
        return (start == null) ? result : (start.compareTo(result) > 0) ? null : result;
    }
     ...
     */
    public static Shape modelToIconOrComponentView(final View view, final int pos, final Shape shape, final Bias bias) throws BadLocationException {
        TextUtils.isPositionValid(view, pos);
        final Rectangle bounds = shape.getBounds();
        final int x = (pos == view.getStartOffset() ? bounds.x : bounds.x + bounds.width);
     ...
     */
    public static void isPositionValid(final View view, final int pos) throws BadLocationException {
        if (pos < view.getStartOffset() || pos > view.getEndOffset()) {
            throw new BadLocationException(Messages.getString("awt.2E", new Object[] { pos, view.getStartOffset(), view.getEndOffset() }), pos);
        }
    }
}
