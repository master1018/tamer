...
public class CollectionSaver extends Saver {
    ...
    public void save(Bean2RDF writer, Resource subject, Property property, Object o) {
        ...
        if (o instanceof Lazy) {
            Lazy lazy = (Lazy) o;
            if (!(lazy.isConnected() && lazy.modified())) return;
        }
        removeAnonymousNodes(subject, property);
        subject.removeAll(property);
        for (Object obj : c) subject.addProperty(property, writer.toRDFNode(obj));
    }
    ...
    private void removeAnonymousNodes(Resource subject, Property property) {
        ...
        while (it.hasNext()) {
            ...
            if (n.isAnon()) anonNodes.add(n.as(Resource.class));
        }
        it.close();
        for (Resource resource : anonNodes) resource.removeProperties();
    }
}
