...
public class Subcontract_CCIDIM extends BasicOperation {
    ...
    public DoubleMatrix2D calculation(double beta, int depth) {
        ...
        while (it.hasNext()) {
            ...
            int minK = 0;
            if (ates.size() < depth) minK = ates.size(); else minK = depth + 1;
            if (minK < 3) minK = 3;
            for (int k = 2; k < minK; k++) {
                ...
                for (int i = 0; i < ates.size() - k; i++) {
                    try {
                        ...
                        if (ate.getOriginator().equals(ate3.getOriginator())) for (int j = i + 1; j < i + k; j++) {
                            ...
                            if (parallelMatrix.get(row_from, column_to) == 0) {
                                ...
                                if (row < 0 || column < 0) {
                                    throw new Error("Implementation error: couldn't find user in the user list: " + ate.getOriginator() + " or " + ate2.getOriginator());
                                }
                                m.set(row, column, 1);
                            }
                        }
                ...
                for (int i = 0; i < users.length; i++) for (int j = 0; j < users.length; j++) D.set(i, j, D.get(i, j) + m.get(i, j) * Math.pow(beta, k - 2));
            }
        }
