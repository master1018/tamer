...
public abstract class MappedSchema<T> implements Schema<T> {
    ...
    public MappedSchema(Class<T> typeClass, Field<T>[] fields, int lastFieldNumber) {
        if (fields.length == 0) throw new IllegalStateException("At least one field is required.");
    ...
    public MappedSchema(Class<T> typeClass, Collection<Field<T>> fields, int lastFieldNumber) {
        if (fields.isEmpty()) throw new IllegalStateException("At least one field is required.");
        this.typeClass = typeClass;
        fieldsByName = new HashMap<String, Field<T>>();
        fieldsByNumber = (Field<T>[]) new Field<?>[lastFieldNumber + 1];
        for (Field<T> f : fields) {
        ...
        for (int i = 1, j = 0; i < fieldsByNumber.length; i++) {
            if (fieldsByNumber[i] != null) this.fields[j++] = fieldsByNumber[i];
        }
        pipeSchema = new PipeSchema();
    }
    ...
    public MappedSchema(Class<T> typeClass, Map<String, Field<T>> fieldsByName, int lastFieldNumber) {
        if (fieldsByName.isEmpty()) throw new IllegalStateException("At least one field is required.");
        this.typeClass = typeClass;
    ...
    public final void mergeFrom(Input input, T message) throws IOException {
        for (int number = input.readFieldNumber(this); number != 0; number = input.readFieldNumber(this)) {
            ...
            if (field == null) input.handleUnknownField(number, this); else field.mergeFrom(input, message);
        }
    }

    public final void writeTo(Output output, T message) throws IOException {
        for (Field<T> f : fields) f.writeTo(output, message);
    }
    ...
    final class PipeSchema extends Pipe.Schema<T> {
        ...
        protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
            for (int number = input.readFieldNumber(MappedSchema.this); number != 0; number = input.readFieldNumber(MappedSchema.this)) {
                final Field<T> field = number < fieldsByNumber.length ? fieldsByNumber[number] : null;
                if (field == null) input.handleUnknownField(number, MappedSchema.this); else field.transfer(pipe, input, output, field.repeated);
            }
        }
    }
