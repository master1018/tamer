 ...
 * 
 */
public class AbstractTypeSystemDeclarativeValidator extends AbstractDeclarativeValidator {

    @Inject
    protected TraceFailureExceptionCollector traceFailureExceptionCollector;

    protected <T> void generateErrors(TypeSystemResult<T> result, EObject eObject, String issueCode) {
        RuleFailedException failure = result.getFailure();
        if (failure == null) return;
        generateErrors(failure, eObject, issueCode);
        List<RuleFailedException> failures = traceFailureExceptionCollector.getFailures(failure);
        for (RuleFailedException ruleFailedException : failures) {
            generateErrors(ruleFailedException, eObject, null);
        }
    }

    protected void generateErrors(RuleFailedException failure, EObject eObject, String issueCode) {
    ...
    protected void generateErrorsForEObject(RuleFailedException failure, EObject eObject, String issueCode) {
        if (eObject != null) error(failure.getMessage(), eObject, eObject.eClass().getEIDAttribute(), issueCode);
    }
	 ...
	 * @return
	 */
    protected TypingJudgmentEnvironment getDefaultTypingJudgmentEnvironmentFor(Object object) {
