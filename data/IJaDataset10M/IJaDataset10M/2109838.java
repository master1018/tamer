...
public class KnowWEArticle extends AbstractType {
    ...
    public static KnowWEArticle createArticle(String text, String title, RootType rootType, String web) {
        return createArticle(text, title, rootType, web, false);
    }

    public static KnowWEArticle createArticle(String text, String title, RootType rootType, String web, boolean fullParse) {
        if (isArticleCurrentlyBuilding(web, title)) {
            Logger.getLogger(KnowWEArticle.class.getName()).severe("The article '" + title + "' is build more than once at the same time, " + "this should not be done! Developer please check with " + "KnowWEArticle#isArticleCurrentlyBuilding(String) first!");
        }
        currentlyBuildingArticles.put(getArticleKey(web, title), null);
        KnowWEArticle article = new KnowWEArticle(text, title, rootType, web, fullParse);
	 ...
	 */
    private KnowWEArticle(String text, String title, RootType rootType, String web, boolean fullParse) {
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "====>> Starting to build article '" + title + "' ====>>");
        ...
        boolean unchangedContent = lastVersion != null && lastVersion.getSection().getText().equals(text);
        this.reParse = unchangedContent && fullParse;
        boolean defFullParse = fullParse || lastVersion == null || KnowWEEnvironment.getInstance().getCompilationMode() == CompilationMode.DEFAULT;
        ...
        if (!defFullParse && !classesCausingFullParse.isEmpty()) {
            Logger.getLogger(this.getClass().getName()).log(Level.INFO, "The following classes " + "caused a full parse:\n" + classesCausingFullParse.toString());
        }
        lastVersion = null;
    }
    ...
    private void build(String text) {
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.FINE, "<- Built KDOM in " + (System.currentTimeMillis() - currentStartTime) + "ms <-");
        currentStartTime = System.currentTimeMillis();
        preCompile();
        compile();
        postCompile();
        Logger.getLogger(this.getClass().getName()).log(Level.FINE, "<- Built Knowledge in " + (System.currentTimeMillis() - currentStartTime) + "ms <-");
        currentStartTime = System.currentTimeMillis();
    }
    ...
    private final Map<String, Map<String, List<Section<?>>>> knownResults = new HashMap<String, Map<String, List<Section<?>>>>();
	 ...
	 */
    public List<Section<? extends Type>> findSectionsWithTypePathAsList(LinkedList<Class<? extends Type>> path) {
    ...
    public void setFullParse(Class<?> source) {
        ...
        classesCausingFullParse.add(source.isAnonymousClass() ? source.getName().substring(source.getName().lastIndexOf(".") + 1) : source.getSimpleName());
        this.fullParse = true;
    }
