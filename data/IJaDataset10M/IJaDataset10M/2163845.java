...
import ist.ac.simulador.assembler.pepe.PepeRecognizer;
import ist.ac.simulador.assembler.pepe.PepeLexer;
...
/** Main Compiler class
 */
public class pepas implements ist.ac.simulador.assembler.IParser {
    ...
    public static void main(String[] args) {
        try {
            ...
            } else System.err.println("Usage: java pepas.Main " + "<directory or file name>");
        } catch (Exception e) {
    ...
    public static void doFile(File f) throws Exception {
        ...
        } else if ((f.getName().length() > 4) && f.getName().substring(f.getName().length() - 4).equals(".asm")) {
    ...
    public static String parseFile(String f, InputStream s, OutputStream o) {
        try {
            PepeLexer lexer = new PepeLexer(s);
            lexer.setFilename(f);
            PepeRecognizer parser = new PepeRecognizer(lexer);
            parser.setFilename(f);
            parser.compilationUnit();
            Object state = parser.getState();
            lexer = new PepeLexer(new FileInputStream(f));
            lexer.setFilename(f);
            parser = new PepeRecognizer(lexer);
            parser.setFilename(f);
