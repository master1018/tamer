...
public class ZStore {
	 ...
	 */
    private final synchronized void init(ZooKeeper zk) throws KeeperException, InterruptedException {
        ZPathUtil.mkdirs(zk, path);
        init.set(true);
    }

    public synchronized void removeExpired(final int seconds) throws IOException, InterruptedException, KeeperException {
        ...
        zk.getChildren(path, false, new AsyncCallback.ChildrenCallback() {
            ...
            @Override
            public void processResult(int rc, String path, Object ctx, List<String> buckets) {
                ZooKeeper zk1;
                try {
                    zk1 = connection.get();
                        ...
                        for (String child : children) {
                            ...
                            if (stat != null) {
                                if ((System.currentTimeMillis() - stat.getMtime()) > expireMilliseconds) {
                                    LOG.info("Deleting: path with mtime: " + new Date(stat.getMtime()));
                                    zk1.delete(childPath, stat.getVersion(), new AsyncCallback.VoidCallback() {
	 ...
	 * @throws KeeperException
	 */
    public byte[] store(String key, Message message) throws IOException, InterruptedException, KeeperException {
        return store(key, message.toByteArray());
    }

    public Message get(String key, Builder<?> builder) throws IOException, InterruptedException, KeeperException {
        byte[] data = get(key);
        return (data == null) ? null : builder.mergeFrom(data).build();
    }

    public byte[] get(String key) throws IOException, InterruptedException, KeeperException {
        ZooKeeper zk = connection.get();
        if (!init.get()) {
            init(zk);
        }
	 ...
	 * @throws KeeperException
	 */
    public byte[] store(String key, byte[] data) throws IOException, InterruptedException, KeeperException {
        ZooKeeper zk = connection.get();
        if (!init.get()) {
            init(zk);
        }
    ...
    public void sync(String key) throws IOException, InterruptedException, KeeperException {
        String keyPath = path + "/" + calcBucket(key) + "/" + key;
        ZooKeeper zk = connection.get();
