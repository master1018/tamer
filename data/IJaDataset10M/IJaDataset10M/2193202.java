...
public class BufferDtdConverter {
   ...
   */
    ...
    private abstract static class VisitorBase implements TopLevelVisitor {
    ...
    private class Analyzer extends VisitorBase implements ModelGroupVisitor, AttributeGroupVisitor {
        ...
        public void choice(ModelGroup[] members) throws Exception {
            for (int i = 0; i < members.length; i++) members[i].accept(this);
        }
    ...
    private class ComponentOutput extends VisitorBase {
        ...
        void finish() {
            if (comments != null) grammar.getFollowingElementAnnotations().addAll(comments);
        }

        private void addComponent(Component c) {
            if (comments != null) {
                if (components.isEmpty()) grammar.getLeadingComments().addAll(comments); else c.getLeadingComments().addAll(comments);
                comments = null;
            }
            components.add(c);
        }
        ...
        public void elementDecl(NameSpec nameSpec, ModelGroup modelGroup) throws Exception {
            ...
            if (options.inlineAttlistDecls) {
                ...
                if (groups != null) {
                    ...
                    for (AttributeGroup group : groups) group.accept(agv);
                }
            } else gp.getChildren().add(ref(attlistDeclName(nameSpec.getValue())));
            Pattern pattern = convert(modelGroup);
            if (gp.getChildren().size() > 0) {
                if (pattern instanceof GroupPattern) gp.getChildren().addAll(((GroupPattern) pattern).getChildren()); else gp.getChildren().add(pattern);
                pattern = gp;
            }
            addComponent(new DefineComponent(elementDeclName(nameSpec.getValue()), new ElementPattern(convertQName(nameSpec.getValue(), true), pattern)));
        ...
        public void attlistDecl(NameSpec nameSpec, AttributeGroup attributeGroup) throws Exception {
            if (options.inlineAttlistDecls) return;
            ...
            if (pattern instanceof EmptyPattern) {
                ...
                if (decls.get(0) != attributeGroup) return;
                attributeNames = new HashSet<String>();
                for (int i = 1, len = decls.size(); i < len; i++) if (!(convert(decls.get(i)) instanceof EmptyPattern)) return;
            }
        ...
        public void attributeGroupDef(String name, AttributeGroup attributeGroup) throws Exception {
            ...
            for (int i = 0; i < members.length; i++) members[i].accept(agv);
        ...
        public void comment(String value) {
            if (comments == null) comments = new Vector<Comment>();
        ...
        public void externalIdRef(String name, ExternalId externalId, String uri, String encoding, TopLevel[] contents) throws Exception {
            ...
            for (int i = 0; i < contents.length; i++) contents[i].accept(co);
            co.finish();
    ...
    private class AttributeGroupOutput implements AttributeGroupVisitor {
        ...
        public void attribute(NameSpec nameSpec, Datatype datatype, AttributeDefault attributeDefault) throws Exception {
            String name = nameSpec.getValue();
            if (attributeNames.contains(name)) return;
            ...
            } else if (datatype.getType() != Datatype.CDATA) dt = convert(datatype); else dt = new TextPattern();
            ...
            if (!attributeDefault.isRequired()) group.add(new OptionalPattern(pattern)); else group.add(pattern);
        }
        ...
        public void attributeGroupRef(String name, AttributeGroup attributeGroup) throws Exception {
            ...
            if (detector.containsDuplicate) attributeGroup.accept(this); else {
                group.add(ref(name));
    ...
    private class DatatypeOutput implements DatatypeVisitor {
        ...
        public void enumDatatype(EnumGroup enumGroup) throws Exception {
            if (enumGroup.getMembers().length == 0) pattern = new NotAllowedPattern(); else {
    ...
    private class ModelGroupOutput implements ModelGroupVisitor {
        ...
        public void choice(ModelGroup[] members) throws Exception {
            if (members.length == 0) pattern = new NotAllowedPattern(); else if (members.length == 1) members[0].accept(this); else {
                ChoicePattern tem = new ChoicePattern();
                pattern = tem;
                List<Pattern> children = tem.getChildren();
                for (int i = 0; i < members.length; i++) children.add(convert(members[i]));
            }
        }

        public void sequence(ModelGroup[] members) throws Exception {
            if (members.length == 0) pattern = new EmptyPattern(); else if (members.length == 1) members[0].accept(this); else {
        ...
        public void any() {
            pattern = ref(anyName);
            if (options.strictAny) pattern = new ZeroOrMorePattern(pattern);
        }
    }
    ...
    private class DuplicateAttributeDetector implements AttributeGroupVisitor {
        ...
        public void attribute(NameSpec nameSpec, Datatype datatype, AttributeDefault attributeDefault) {
            String name = nameSpec.getValue();
            if (attributeNames.contains(name)) containsDuplicate = true;
            names.add(name);
        }
    ...
    public SchemaCollection convert() {
        try {
            ...
            if (options.generateStart) outputStart(grammar.getComponents());
    ...
    private void choosePrefixes() {
        if (options.defaultNamespace != null) {
            if (defaultNamespace != null && !defaultNamespace.equals(options.defaultNamespace)) warning("default_namespace_conflict");
            defaultNamespace = options.defaultNamespace;
        } else if (defaultNamespace == null) defaultNamespace = NameClass.INHERIT_NS;
        for (Map.Entry<String, String> entry : options.prefixMap.entrySet()) {
            String prefix = entry.getKey();
            String ns = entry.getValue();
            String s = prefixTable.get(prefix);
            if (s == null) warning("irrelevant_prefix", prefix); else {
                if (!s.equals("") && !s.equals(ns)) warning("prefix_conflict", prefix);
    ...
    private void chooseAnnotationPrefix() {
        ...
        for (int n = 0; ; n++) {
            ...
            if (prefixTable.get(annotationPrefix) == null) return;
        }
    }
    ...
    private void chooseDeclPatterns() {
        if (options.elementDeclPattern != null) {
            if (patternOk(options.elementDeclPattern, null)) elementDeclPattern = options.elementDeclPattern; else warning("cannot_use_element_decl_pattern");
        }
        if (options.attlistDeclPattern != null) {
            if (patternOk(options.attlistDeclPattern, elementDeclPattern)) attlistDeclPattern = options.attlistDeclPattern; else warning("cannot_use_attlist_decl_pattern");
        }
        if (elementDeclPattern != null && attlistDeclPattern != null) return;
        String pattern = namingPattern();
        if (elementDeclPattern == null) {
            if (patternOk("%", attlistDeclPattern)) elementDeclPattern = "%"; else elementDeclPattern = choosePattern(pattern, ELEMENT_KEYWORDS, attlistDeclPattern);
        }
        if (attlistDeclPattern == null) attlistDeclPattern = choosePattern(pattern, ATTLIST_KEYWORDS, elementDeclPattern);
    }
    ...
    private String choosePattern(String metaPattern, String[] keywords, String otherPattern) {
        for (; ; ) {
            for (int i = 0; i < keywords.length; i++) {
                ...
                if (patternOk(pattern, otherPattern)) return pattern;
            }
    ...
    private String namingPattern() {
        ...
        for (String name : definedNames) {
            for (int i = 0; i < SEPARATORS.length(); i++) {
                ...
                if (k > 0) inc(patternTable, name.substring(0, k + 1) + "%");
                k = name.lastIndexOf(sep);
                if (k >= 0 && k < name.length() - 1) inc(patternTable, "%" + name.substring(k));
            }
        }
        ...
        if (bestPattern == null) return DEFAULT_PATTERN;
        if (bestPattern.charAt(0) == '%') return bestPattern.substring(0, 2) + "#"; else return "#" + bestPattern.substring(bestPattern.length() - 2);
    }

    private static void inc(Map<String, Integer> table, String str) {
        Integer n = table.get(str);
        if (n == null) table.put(str, 1); else table.put(str, n + 1);
    }
    ...
    private boolean patternOk(String pattern, String otherPattern) {
        ...
        for (String s : elementNameTable.keySet()) {
            ...
            if (definedNames.contains(declName)) return false;
            if (otherPattern != null) {
                String otherDeclName = substitute(otherPattern, '%', name);
                if (usedNames.contains(declName) || usedNames.contains(otherDeclName) || declName.equals(otherDeclName)) return false;
    ...
    private void noteAttribute(String name, String defaultValue) {
        if (name.equals("xmlns")) {
            if (defaultValue != null) {
                if (defaultNamespace != null && !defaultNamespace.equals(defaultValue)) error("INCONSISTENT_DEFAULT_NAMESPACE"); else defaultNamespace = defaultValue;
            }
        } else if (name.startsWith("xmlns:")) {
            if (defaultValue != null) {
                String prefix = name.substring(6);
                String ns = prefixTable.get(prefix);
                if (ns != null && !ns.equals("") && !ns.equals(defaultValue)) error("INCONSISTENT_PREFIX", prefix); else if (!prefix.equals("xml")) prefixTable.put(prefix, defaultValue);
            }
        } else {
            if (defaultValue != null) hadDefaultValue = true;
            noteNamePrefix(name);
        }
    }
    ...
    private void noteNamePrefix(String name) {
        ...
        if (prefixTable.get(prefix) == null && !prefix.equals("xml")) prefixTable.put(prefix, "");
    }
    ...
    private void outputAny(List<Component> components) {
        ...
        if (options.strictAny) {
            ...
            group.getChildren().add(new ZeroOrMorePattern(new AttributePattern(new AnyNameNameClass(), new TextPattern())));
    ...
    private NameClass convertQName(String name, boolean useDefault) {
        int i = name.indexOf(':');
        if (i < 0) return new NameNameClass(useDefault ? defaultNamespace : "", name);
        String prefix = name.substring(0, i);
        String localName = name.substring(i + 1);
        String ns;
        if (prefix.equals("xml")) ns = WellKnownNamespaces.XML; else {
