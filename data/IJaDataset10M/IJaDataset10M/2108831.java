...
public abstract class InterfaceBWebsideController {
    ...
    public String checkInWorkItem(String workItemID, Element inputData, Element outputData, String sessionHandle) throws IOException, JDOMException {
        ...
        SpecificationData specData = getSpecificationData(workitem.getSpecificationID(), sessionHandle);
        String filteredOutputData;
        if (!(specData.usesSimpleRootData())) {
            TaskInformation taskInfo = getTaskInformation(workitem.getSpecificationID(), workitem.getTaskID(), sessionHandle);
    ...
    public TaskInformation getTaskInformation(String specificationID, String taskID, String sessionHandle) throws IOException {
        ...
        if (taskInfo == null) {
            String taskInfoASXML = _interfaceBClient.getTaskInformationStr(specificationID, taskID, sessionHandle);
    ...
    public static void logContactError(IOException e, String backEndURIStr) {
        Logger.getLogger(InterfaceBWebsideController.class).error("[error] problem contacting YAWL engine at URI [" + backEndURIStr + "]");
        if (e.getStackTrace() != null) {
            Logger.getLogger(InterfaceBWebsideController.class).error("line of code := " + e.getStackTrace()[0].toString());
        }
    }
    ...
    public List getSpecificationPrototypesList(String sessionHandle) throws IOException {
     ...
     */
    protected List checkOutAllInstancesOfThisTask(WorkItemRecord enabledWorkItem, String sessionHandle) throws IOException, YAWLException {
        if (null == enabledWorkItem) throw new IllegalArgumentException("Param enabledWorkItem cannot be null.");
        if (!enabledWorkItem.getStatus().equals(WorkItemRecord.statusEnabled)) throw new IllegalArgumentException("Param enabledWorkItem must be enabled.");
        ...
        for (int i = 0; i < mixedChildren.size(); i++) {
            ...
            if (WorkItemRecord.statusFired.equals(itemRecord.getStatus())) {
                _logger.debug("Result of item [" + itemRecord.getID() + "] checkout is : " + checkOut(itemRecord.getID(), sessionHandle));
            }
            executingChildrenOfWorkItem = getChildren(enabledWorkItem.getID(), sessionHandle);
        }
    ...
    protected Element prepareReplyRootElement(WorkItemRecord enabledWorkItem, String sessionHandle) throws IOException {
        ...
        SpecificationData sdata = getSpecificationData(enabledWorkItem.getSpecificationID(), sessionHandle);
        TaskInformation taskInfo = getTaskInformation(enabledWorkItem.getSpecificationID(), enabledWorkItem.getTaskID(), sessionHandle);
    ...
    protected Element getResourcingSpecs(String specID, String taskID, String sessionHandle) throws IOException {
        String result = _interfaceBClient.getResourcingSpecs(specID, taskID, sessionHandle);
        if ((result != null) && (!result.equals("")) && (!result.equals("null"))) return JDOMUtil.stringToElement(result); else return null;
    }
}
