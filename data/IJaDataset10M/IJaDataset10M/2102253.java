...
import java.lang.*;
import net.sf.imageCave.database.*;
...
class Server {
    ...
    public void startup() {
        ...
        try {
            ...
            ServerSocket control = new ServerSocket(8888);
            ServerSocket dataSocket = new ServerSocket(9999);
            while (go) {
                System.out.println("listening for new control on " + control.getLocalPort());
                Socket controlconnection = control.accept();
                run(controlconnection, dataSocket, this);
            }
        } catch (Exception e) {
            dbm.stop();
            go = false;
