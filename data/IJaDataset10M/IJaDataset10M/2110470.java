...
public class ScriptingInitialiser {
	 ...
	 */
    public static final ScriptCategory createCategory(PersistenceManager pm, ScriptCategory parent, String organisationID, String scriptRegistryItemType, String scriptRegistryItemID) {
        ...
        try {
            ...
            if (parent == null) pm.makePersistent(category); else parent.addChild(category);
        }
        return category;
    }
	 ...
	 */
    public ScriptingInitialiser(String scriptSubDir, ScriptCategory baseCategory, String scriptRegistryItemType, JFireServerManager jfsm, PersistenceManager pm, String organisationID) {
    ...
    protected ScriptRegistry getScriptRegistry() {
        if (scriptRegistry == null) scriptRegistry = ScriptRegistry.getScriptRegistry(pm);
    ...
    public void initialise() throws ScriptingIntialiserException {
        ...
        if (!scriptDir.exists()) throw new IllegalStateException("Script directory does not exist: " + scriptDir.getAbsolutePath());
    ...
    private Document getCategoryDescriptor(File categoryDir) throws SAXException, IOException {
    ...
    private Node getScriptDescriptor(File scriptFile, Document categoryDocument) throws TransformerException {
        ...
        for (Node scriptNode : nodes) {
            ...
            if (fNode != null && scriptFile.getName().equals(fNode.getTextContent())) return scriptNode;
        }
        return null;
    }

    private ScriptParameterSet createParameterSet(PersistenceManager pm, String organisationID, ScriptRegistryItem registryItem, Node parentNode) throws TransformerException {
        ...
        if (setNode == null) return parameterSet;
        ScriptCategory parent = registryItem.getParent();
        if (parent != null && parent.getParameterSet() != null && parent.getParameterSet().equals(parameterSet)) parameterSet = null;
        ...
        for (Node node : nodes) {
            ...
            if (pIDNode != null && !"".equals(pIDNode.getTextContent())) {
                ...
                } else logger.warn("parameter element of parameter-set has an invalid/missing type attribute");
            } else logger.warn("parameter element of parameter-set has an invalid/missing name attribute");
        }
        return parameterSet;
    }
        ...
        if (elementNode != null) {
            ...
            for (Node node : nodes) {
                ...
                if (lIDNode != null && !"".equals(lIDNode.getTextContent())) {
                    ...
                    nameSet = true;
                } else logger.warn("\" " + elementName + "\" element of node " + elementNode.getNodeName() + " has an invalid/missing language attribute");
            }
        }
        if (!nameSet && def != null) name.setText(Locale.ENGLISH.getLanguage(), def);
    }

    private void createScriptCategories(File dir, ScriptCategory parent) throws ScriptingIntialiserException {
        try {
            ...
            try {
                ...
                if (!(cat instanceof ScriptCategory)) throw new IllegalStateException("Found ScriptRegistryItem for id " + registryItemID + " but it is not an instance of ScriptCategory, it is " + cat.getClass().getName());
            ...
            if (catDocument != null) {
                ...
                if (catNode != null) {
                    ...
                    if (parameterSet != null) category.setParameterSet(parameterSet);
                }
            }
            ...
            for (int j = 0; j < scripts.length; j++) {
                ...
                if (category != null) {
                    if (category.getScriptRegistryItemType() != null) scriptItemType = category.getScriptRegistryItemType();
                }
                if (scriptNode != null) {
                ...
                try {
                    ...
                    try {
                        pm.getExtent(Script.class);
                        script = (Script) pm.getObjectById(ScriptRegistryItemID.create(organisationID, scriptItemType, scriptID));
                    } catch (JDOObjectNotFoundException e) {
                    ...
                    if (scriptNode != null) {
                        ...
                        if (parameterSet != null) script.setParameterSet(parameterSet);
                    }
    ...
    private FilenameFilter scriptFileNameFilter = new FilenameFilter() {
        ...
        public boolean accept(File dir, String name) {
            ...
            for (Iterator<String> it = getFileExtensions().iterator(); it.hasNext(); ) {
                ...
                if (registeredFileExt.equals(fileExtension)) return true;
            }
            return false;
        }
    };
