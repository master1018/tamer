...
public final class RequestSendPost extends L2GameClientPacket {
    ...
    protected void readImpl() {
        ...
        if (attachCount < 0 || attachCount > Config.MAX_ITEM_IN_PACKET || attachCount * BATCH_LENGTH + 8 != _buf.remaining()) return;
        if (attachCount > 0) {
    ...
    public void runImpl() {
        ...
        if (_reqAdena < 0 || _reqAdena > MAX_ADENA) return;
        if (_isCod) {
            if (_reqAdena == 0) {
        ...
        if (level == AccessLevels._masterAccessLevelNum) accessLevel = AccessLevels._masterAccessLevel; else if (level == AccessLevels._userAccessLevelNum) {
            accessLevel = AccessLevels._userAccessLevel;
        } else {
            accessLevel = AccessLevels.getInstance().getAccessLevel(level);
            if (accessLevel == null) accessLevel = AccessLevels._userAccessLevel;
        }
    ...
    private final boolean removeItems(L2PcInstance player, Message msg) {
        ...
        if (playerIU != null) player.sendPacket(playerIU); else player.sendPacket(new ItemList(player, false));
