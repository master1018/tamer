...
public class TableSorter extends TableMap {
    ...
    public final int getRow4Spieler(int spielerid) {
        if (spielerid != 0) {
            for (int i = 0; i < getRowCount(); i++) {
                try {
                    if (spielerid == Integer.parseInt(((ho.core.gui.comp.entry.ColorLabelEntry) getValueAt(i, idSpalte)).getText())) {
                        return i;
                    }
                } catch (Exception e) {
    ...
    public final ho.module.transfer.scout.ScoutEintrag getScoutEintrag(int row) {
        if (row > -1) {
            try {
                return ((ho.module.transfer.scout.TransferTableModel) getModel()).getScoutEintrag(Integer.parseInt(((ColorLabelEntry) getValueAt(row, idSpalte)).getText()));
            } catch (Exception e) {
    ...
    private final int compareRowsByColumn(int i, int j, int k) {
        ...
        if (obj instanceof IHOTableEntry && obj1 instanceof IHOTableEntry) {
