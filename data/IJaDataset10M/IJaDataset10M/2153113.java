...
/**
 * @author Heng Yuan
 * @version $Id: Task.java 682 2009-10-17 05:43:03Z superduperhengyuan $
 */
public class Task extends org.apache.tools.ant.Task {
    ...
    public void setSrcDir(File srcDir) {
        if (!srcDir.exists() || !srcDir.isDirectory()) throw new IllegalArgumentException("Invalid destination directory.");
        m_srcDir = srcDir;
    }

    public void setDestDir(File destDir) {
        if (!destDir.exists() || !destDir.isDirectory()) throw new IllegalArgumentException("Invalid destination directory.");
        m_destDir = destDir;
    }

    public void setLexerTable(String table) {
        if (!"ecs".equals(table) && !"full".equals(table) && !"compressed".equals(table)) throw new IllegalArgumentException("Unknown lexer table: " + table);
        m_lexerTable = table;
    }

    public void setParserTable(String table) {
        if (!"ecs".equals(table) && !"compressed".equals(table)) throw new IllegalArgumentException("Unknown parser table: " + table);
        m_parserTable = table;
    }
    ...
    public void setSrc(String files) {
        ...
        for (String fileName : fileNames) {
            ...
            if (fileName.endsWith(".java")) m_aptFiles.add(fileName); else m_xccFiles.add(fileName);
        }
    }
    ...
    public void execute() {
        if (m_xccFiles.size() > 0) executeCookCC(m_xccFiles.toArray(new String[m_xccFiles.size()]));
        if (m_aptFiles.size() > 0) executeApt(m_aptFiles.toArray(new String[m_aptFiles.size()]));
    }
    ...
    protected void executeCookCC(String[] files) {
        ...
        if (m_analysis) cmd.createArgument().setValue("-analysis");
        if (m_debug) cmd.createArgument().setValue("-debug");
        if (m_defaultReduce) cmd.createArgument().setValue("-defaultreduce");
        ...
        for (Option option : m_options) {
            if (option.m_name == null || option.m_name.length() == 0) continue;
            cmd.createArgument().setValue(option.m_name);
            if (option.m_value == null) continue;
        ...
        for (String file : files) {
            ...
            if (executeCmd(newCmd)) throw new RuntimeException("Error executing CookCC");
        }
    }
    ...
    protected void executeApt(String[] classes) {
        ...
        if (m_srcDir == null) throw new IllegalArgumentException("Source directory is not specified.");
        cmd.createArgument().setValue("-cp");
        ...
        if (m_analysis) cmd.createArgument().setValue("-Aanalysis");
        if (m_debug) cmd.createArgument().setValue("-Adebug");
        if (m_defaultReduce) cmd.createArgument().setValue("-Adefaultreduce");
        if (m_lexerTable != null) cmd.createArgument().setValue("-Alexertable=" + m_lexerTable);
        if (m_parserTable != null) cmd.createArgument().setValue("-Aparsertable=" + m_parserTable);
        if (m_lang != null) cmd.createArgument().setValue("-Alang=" + m_lang);
        if (m_lang == null || "java".equals(m_lang)) {
            if (m_destDir == null) m_destDir = m_srcDir;
        ...
        for (Option option : m_options) {
            ...
            arg = "-A" + arg;
            if (option.m_value != null) arg = arg + '=' + option.m_value;
        ...
        if (executeCmd(cmd)) throw new RuntimeException("Error executing CookCC using APT");
    }
    ...
    protected boolean executeCmd(Commandline cmd) {
        try {
            ...
            exe.execute();
            if (exe.getExitValue() == 0) return false;
        } catch (Exception ex) {
        }
        return true;
    }
