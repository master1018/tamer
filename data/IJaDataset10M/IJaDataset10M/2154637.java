...
public abstract class ContainerBase implements Container, Lifecycle, Pipeline, MBeanRegistration, Serializable {
    ...
    public DirContext getResources() {
        ...
        if (parent != null) return (parent.getResources());
        return (null);
    }
    ...
    public synchronized void start() throws LifecycleException {
        if (started) {
            if (log.isInfoEnabled()) log.info(sm.getString("containerBase.alreadyStarted", logName()));
            return;
        }
        ...
        if ((manager != null) && (manager instanceof Lifecycle)) ((Lifecycle) manager).start();
        ...
        for (int i = 0; i < children.length; i++) {
            if (children[i] instanceof Lifecycle) ((Lifecycle) children[i]).start();
        }
    ...
    public synchronized void stop() throws LifecycleException {
        if (!started) {
            if (log.isInfoEnabled()) log.info(sm.getString("containerBase.notStarted", logName()));
            return;
        }
        ...
        Container children[] = findChildren();
        for (int i = 0; i < children.length; i++) {
            if (children[i] instanceof Lifecycle) ((Lifecycle) children[i]).stop();
        }
        ...
        if (oname != null) {
            try {
                if (controller == oname) {
                    ...
                    if (log.isDebugEnabled()) log.debug("unregistering " + oname);
                }
            } catch (Throwable t) {
    ...
    public ObjectName createObjectName(String domain, ObjectName parent) throws Exception {
        if (log.isDebugEnabled()) log.debug("Create ObjectName " + domain + " " + parent);
        return null;
    }
