...
public class StyleSheet_Large_Test extends BasicSwingTestCase {
    ...
    public void testContainsAttribute() throws Exception {
        assertTrue(attr.containsAttribute(cssAttribute, attr.getAttribute(cssAttribute)));
        assertTrue(attr.containsAttribute(scAttribute, scValue));
    }

    public void testContainsAttributeAnother() throws Exception {
        final AttributeSet another = ss.addAttribute(empty, scAttribute, scValue);
        assertEquals(attr.getAttribute(cssAttribute).toString(), another.getAttribute(cssAttribute).toString());
        if (isHarmony()) {
            assertTrue(attr.containsAttribute(cssAttribute, another.getAttribute(cssAttribute)));
        } else {
            assertFalse(attr.containsAttribute(cssAttribute, another.getAttribute(cssAttribute)));
        }
    }
    ...
    public void testContainsAttributesSmall() throws Exception {
        final AttributeSet another = ss.addAttribute(empty, scAttribute, scValue);
        if (isHarmony()) {
            assertSame(attr.getAttribute(Attribute.FONT_STYLE), another.getAttribute(Attribute.FONT_STYLE));
            assertTrue(attr.containsAttributes(another));
            assertFalse(another.containsAttributes(attr));
        } else {
            assertNotSame(attr.getAttribute(Attribute.FONT_STYLE), another.getAttribute(Attribute.FONT_STYLE));
    ...
    public void testRemoveAttribute() throws Exception {
        ...
        assertEquals(count, mutable.getAttributeCount());
