...
public class SimpleSgsRelocationProtocolImpl extends SimpleSgsProtocolImpl implements SessionRelocationProtocol {
    ...
    /** {@inheritDoc} */
    @Override
    public void channelMessage(BigInteger channelId, ByteBuffer message, Delivery delivery) {
        checkSuspend();
    ...
    public void suspend(RequestCompletionHandler<Void> completionHandler) {
        synchronized (lock) {
            ...
            suspendCompletionFuture = new SuspendMessagesCompletionFuture(completionHandler);
        }
    ...
    /** {@inheritDoc} */
    public void relocate(Set<ProtocolDescriptor> descriptors, ByteBuffer relocationKey, RequestCompletionHandler<Void> completionHandler) {
        synchronized (lock) {
            if (relocationInfo != null) {
                throw new IllegalStateException("session already relocating");
            } else if (suspendCompletionFuture == null || !suspendCompletionFuture.isDone()) {
                throw new IllegalStateException("session is not suspended");
            }
            if (logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "relocating, identity:{0} key:{1}", getIdentity(), HexDumper.toHexString(relocationKey.array()));
            }
            ...
            case SimpleSgsProtocol.RELOCATE_REQUEST:
                ...
                if (version != getProtocolVersion()) {
                    if (logger.isLoggable(Level.SEVERE)) {
                        logger.log(Level.SEVERE, "got protocol version:{0}, " + "expected {1}", version, getProtocolVersion());
                    }
                    close();
                    break;
                }
                ...
                listener.relocatedSession(relocationKey, SimpleSgsRelocationProtocolImpl.this, new RelocateHandler());
                readNow();
                break;
                ...
                synchronized (lock) {
                    if (suspendCompletionFuture != null) {
                        ...
                        if (logger.isLoggable(Level.WARNING)) {
                            logger.log(Level.WARNING, "{0} received unexpected " + "SUSPEND_MESSAGES_COMPLETE");
                        }
                    }
                }
                break;
    ...
    private class RelocationInfo {
        ...
        RelocationInfo(Set<ProtocolDescriptor> descriptors, ByteBuffer relocationKey) {
        ...
        void sendRelocateNotification() {
            for (ProtocolDescriptor descriptor : descriptors) {
                if (acceptor.getDescriptor().supportsProtocol(descriptor)) {
                    byte[] redirectionData = ((SimpleSgsProtocolDescriptor) descriptor).getConnectionData();
                    ByteBuffer buf = ByteBuffer.allocate(1 + redirectionData.length + relocationKey.remaining());
                    buf.put(SimpleSgsProtocol.RELOCATE_NOTIFICATION).put(redirectionData).put(relocationKey).flip();
                    writeNow(buf, true);
                    monitorDisconnection();
                    return;
                }
     ...
     */
    private static class SuspendMessagesCompletionFuture extends AbstractCompletionFuture<Void> {

        SuspendMessagesCompletionFuture(RequestCompletionHandler<Void> completionFuture) {
