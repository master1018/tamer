    ...
    private JComponent loadContentAndBuildViewer() {
        ...
        if (DEBUG.Enabled) {
            ...
            for (Map.Entry<String, List<String>> e : headers.entrySet()) {
                ...
                if (value instanceof Collection && ((Collection) value).size() == 1) value = ((Collection) value).toArray()[0];
                if (e.getKey() == null) fr.setProperty("HTTP-response", value); else fr.setProperty("HTTP:" + e.getKey(), value);
            }
        }
        ...
        if ("This file is an XML representation of some issues".equalsIgnoreCase(desc)) ((URLResource) fr).setSpec(getAddress()); else fr.addPropertyIfContent("Description", desc);
        resources.add(fr);
