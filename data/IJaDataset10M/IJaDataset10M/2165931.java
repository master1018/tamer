...
class ELF64SectionHeader extends FormatStruct {
    ...
    public static final LongAttribute SH_FLAGS = new LongAttribute("sh_flags", 8, 1, false, 16);

    public static final LongAttribute SH_ADDR = new LongAttribute("sh_addr", 16, 1, false, 16);

    public static final LongAttribute SH_OFFSET = new LongAttribute("sh_offset", 24, 1, false, 16);

    public static final LongAttribute SH_SIZE = new LongAttribute("sh_size", 32, 1, false, 16);

    public static final IntAttribute SH_LINK = new IntAttribute("sh_link", 40, 1, false, 16);

    public static final IntAttribute SH_INFO = new IntAttribute("sh_info", 44, 1, false, 16);

    public static final LongAttribute SH_ADDRALIGN = new LongAttribute("sh_addralign", 48, 1, false, 16);

    public static final LongAttribute SH_ENTSIZE = new LongAttribute("sh_entsize", 56, 1, false, 16);

    static {
        SH_TYPE.setValueSymbols(ELFImage.SYM_SH_TYPE);
        SH_FLAGS.setFlagFormatters(ELFImage.SYM_SH_FLAGS64);
    }

    private ELF64SectionHeader(ByteOrder order) {
        super(ELFImage.NAME_ELF64_SECTION_HEADER, order, SH_NAME, SH_TYPE, SH_FLAGS, SH_ADDR, SH_OFFSET, SH_SIZE, SH_LINK, SH_INFO, SH_ADDRALIGN, SH_ENTSIZE);
    }

    /**
	 * Structure instance (LSB).
	 */
    public static final ELF64SectionHeader THIS_LSB = new ELF64SectionHeader(ByteOrder.LITTLE_ENDIAN);

    /**
	 * Structure instance (MSB).
	 */
    public static final ELF64SectionHeader THIS_MSB = new ELF64SectionHeader(ByteOrder.BIG_ENDIAN);
    ...
    public FileScannerResultNode decodeBuffer(FileScannerResultNode parent, long position, ByteBuffer buffer) throws IOException {
        ...
        final long offset = SH_OFFSET.value(buffer);
        final long size = SH_SIZE.value(buffer);
        ...
        if (offset > 0 && size > 0 && SH_TYPE.value(buffer) != 8 && validateOffset(parent.input(), sectionPosition, size)) {
            if (context.shstroff == position) {
                context.strtab = allocateAndReadBuffer(parent.input(), sectionPosition, (int) size, true);
            }
            ...
            final long flags = SH_FLAGS.value(buffer);
            if ((flags & 0x4) == 0x4) {
                if (context.machine == 62) {
                    sectionNode.addDataHandler(new DecodeTextDataHandler(sectionNode, new X86OpcodeDecoderTable(X86OpcodeDecoderTable.Mode.MODE64, context.mapper.map(sectionPosition))));
                }
            }
        }
        return decoded;
