 ...
 * {@link Security#verifyObjectTrust Security.verifyObjectTrust}, allowing the
 * objects that are trusted to be extended.
 * 
...
public interface TrustVerifier {
	 ...
	 * 
	 * @param obj
	 *            the object in which to verify trust
	 * @param ctx
	 *            the trust verifier context, to aid in verification of the
	 *            specified object and its components
	 * @return <code>true</code> if the specified object is known to be trusted
	 *         to correctly implement its contract; <code>false</code> otherwise
	 ...
	 */
    boolean isTrustedObject(Object obj, Context ctx) throws RemoteException;

    /**
	 * Defines the context for trust verification used by {@link TrustVerifier}
	 * instances and {@link Security#verifyObjectTrust
    ...
    public interface Context {
		 ...
		 * {@link TrustVerifier#isTrustedObject isTrustedObject} method of each
		 * verifier contained in this context is called (in order) with the
		 * specified object and this context. If any verifier call returns
		 * <code>true</code>, the object is trusted and this method returns
		 * <code>true</code>. If all of the verifier calls return
		 * <code>false</code>, this method returns <code>false</code>. If one or
		 * more verifier calls throw a <code>RemoteException</code> or
		 * <code>SecurityException</code>, the last such exception is thrown to
		 * the caller (unless some verifier call returns <code>true</code>).
		 * 
		 * @param obj
		 *            the object in which to verify trust
		 * @return <code>true</code> if the specified object is trusted to
		 *         correctly implements its contract; <code>false</code>
		 *         otherwise
		 * @throws RemoteException
		 *             if a communication-related exception occurs
		 * @throws SecurityException
		 ...
		 * {@link net.jini.core.constraint.MethodConstraints} instance could be
		 * used to specify client constraints for any remote calls that trust
		 * verifiers might need to perform.
		 * 
