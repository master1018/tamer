...
public class BaseRequestItem implements RequestItem {
    ...
    public BaseRequestItem(JSONObject parameters, Map<String, FormDataItem> formItems, SecurityToken token, BeanConverter converter, BeanJsonConverter jsonConverter) {
        try {
            this.parameters = Maps.newHashMap();
            @SuppressWarnings("unchecked") Iterator<String> keys = parameters.keys();
            while (keys.hasNext()) {
    ...
    public SortOrder getSortOrder() {
        ...
        try {
            ...
            throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST, "Parameter " + SORT_ORDER + " (" + sortOrder + ") is not valid.");
        }
    }
    ...
    public int getStartIndex() {
        ...
        try {
            ...
            throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST, "Parameter " + START_INDEX + " (" + startIndex + ") is not a number.");
        }
    }
    ...
    public int getCount() {
        ...
        try {
            ...
            throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST, "Parameter " + COUNT + " (" + count + ") is not a number.");
        }
    }
    ...
    public FilterOperation getFilterOperation() {
        ...
        try {
            ...
            throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST, "Parameter " + FILTER_OPERATION + " (" + filterOp + ") is not valid.");
        }
    }
    ...
    public <T> T getTypedParameter(String parameterName, Class<T> dataTypeClass) {
        try {
            ...
            if (e.getCause() instanceof JSONException) throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST, e.getMessage());
            throw e;
        }
    }
    ...
    public <T> T getTypedRequest(Class<T> dataTypeClass) {
        try {
            return jsonConverter.convertToObject(new JSONObject(this.parameters).toString(), dataTypeClass);
        } catch (RuntimeException e) {
            if (e.getCause() instanceof JSONException) throw new ProtocolException(HttpServletResponse.SC_BAD_REQUEST, e.getMessage());
            throw e;
        }
    ...
    public List<String> getListParameter(String paramName) {
        ...
        if (param instanceof String && ((String) param).indexOf(',') != -1) {
            ...
            @SuppressWarnings("unchecked") List<String> listParam = (List<String>) param;
            return listParam;
