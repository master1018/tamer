...
public class NumericBuffer<T extends Number> extends Buffer<T> {
    ...
    public Number average() {
        if (isEmpty()) return 0;
        Number sum = sum();
        if (sum instanceof Double) return sum().doubleValue() / size(); else return sum().longValue() / size();
    }
    ...
    public Number sum() {
        ...
        Number n = first();
        if (n instanceof Float || n instanceof Double) return doubleSum(); else return longSum();
    }
