...
public class LogData {
   ...
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(LogData.class);
    ...
    public LogData(double time, LogSessionDataStream stream) {
        ...
        _timeStream = String.format("%02.0f:%02.0f:%05.2f", hours, minutes, timeInSeconds);
    }
    ...
    protected void conflictResolution() {
        for (StringBuilder sb : _streamContents.values()) if (sb.length() > 0) sb.append("---\n");
    }
    ...
    public String get(String streamName) {
        if (streamName.equalsIgnoreCase(Logger.Stream.TIME.toString())) return _timeStream;
    ...
    public String toString() {
        return String.format("[Log (%.2f) : %s]", getTime(), _streamContents.keySet());
    }
}
