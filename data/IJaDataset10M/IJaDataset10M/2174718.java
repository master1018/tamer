...
public class UnitDecorator extends LabelProvider implements ILightweightLabelDecorator {
    ...
    public void decorate(Object element, IDecoration decoration) {
        ...
        if (javaTypeOfResource == null) return;
    ...
    public ICompilationUnit tryToGetCompilationUnitFromElement(Object element) {
        ...
        if (objectResource.getType() != IResource.FILE) return null;
        IJavaElement javaElement = JavaCore.create(objectResource);
        if (javaElement == null) return null;
        if (javaElement.getElementType() != IJavaElement.COMPILATION_UNIT) return null;
        if (TypeFacade.isTestCase(((ICompilationUnit) javaElement).findPrimaryType())) return null;
    ...
    public static UnitDecorator getUnitDecorator() {
        ...
        if (decoratorManager.getEnabled(MoreUnitContants.TEST_CASE_DECORATOR)) return (UnitDecorator) decoratorManager.getBaseLabelProvider(MoreUnitContants.TEST_CASE_DECORATOR); else return null;
    }

    public void refreshAll() {
        UnitDecorator unitDecorator = getUnitDecorator();
        if (unitDecorator != null) unitDecorator.fireLabelProviderChanged(new LabelProviderChangedEvent(unitDecorator));
    }
}
