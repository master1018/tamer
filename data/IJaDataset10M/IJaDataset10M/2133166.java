...
public class WebServer implements ContentSource {
    ...
    private InheritableThreadLocal effectiveClientSocket = new InheritableThreadLocal();
    ...
    public static final DashboardPermission SET_PASSWORD_PERMISSION = new DashboardPermission("webServer.setPassword");

    public static final DashboardPermission GET_SOCKET_PERMISSION = new DashboardPermission("webServer.getSocket");

    public static final DashboardPermission CREATE_PERMISSION = new DashboardPermission("webServer.create");

    public static final DashboardPermission SET_ROOTS_PERMISSION = new DashboardPermission("webServer.setRoots");

    public static final DashboardPermission SET_HIERARCHY_PERMISSION = new DashboardPermission("webServer.setHierarchy");

    public static final DashboardPermission SET_DATA_PERMISSION = new DashboardPermission("webServer.setDataRepository");

    public static final DashboardPermission SET_CACHE_PERMISSION = new DashboardPermission("webServer.setCache");

    public static final DashboardPermission SET_DASHBOARD_CONTEXT = new DashboardPermission("webServer.setDashboardContext");

    public static final DashboardPermission SET_ALLOW_REMOTE_PERMISSION = new DashboardPermission("webServer.setAllowRemoteConnections");

    public static final DashboardPermission ADD_PORT_PERMISSION = new DashboardPermission("webServer.addPort");

    public static final DashboardPermission QUIT_PERMISSION = new DashboardPermission("webServer.quit");
    ...
    static {
        try {
            ...
            mimeTypes.load(WebServer.class.getResourceAsStream("mime_types"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
    ...
    private class TinyWebThread implements Runnable, HTTPHeaderWriter {
        ...
        private class TinyWebThreadException extends Exception {
            ...
            public TinyWebThreadException(String message) {
                super(message);
            }
        }
        ...
        public synchronized void close() {
            if (isRunning && runnerThread != null) runnerThread.interrupt();
        ...
        public void run() {
            ...
            try {
                if (isClientRequest) effectiveClientSocket.set(clientSocket);
                if (inputStream != null) {
                    isRunning = true;
                    try {
                        handleRequest();
                ...
                if (isClientRequest) effectiveClientSocket.set(null);
            }
        }
        ...
        private URLConnection resolveURL(final String url) throws TinyWebThreadException {
            URLConnection result = (URLConnection) AccessController.doPrivileged(new PrivilegedAction() {
        ...
        private void parseLinkParameters(BufferedReader linkContents) throws IOException {
            ...
            while ((param = linkContents.readLine()) != null) {
                ...
                if (equalsPos == 0 || param.length() == 0) continue; else if (equalsPos == -1) queryString.append("&").append(HTMLUtils.urlEncode(param.trim())); else {
                    name = param.substring(0, equalsPos);
                    val = param.substring(equalsPos + 1);
                    if (val.startsWith("=")) val = val.substring(1);
                    queryString.append("&").append(HTMLUtils.urlEncode(name)).append("=").append(HTMLUtils.urlEncode(val));
                }
            }
        ...
        private void serveCGI(TinyCGI script) throws IOException, TinyWebThreadException {
            ...
            try {
                ...
                if (script instanceof TinyCGIHighVolume) mode = CGIOutputStream.LARGE; else if (script instanceof TinyCGIStreaming) mode = CGIOutputStream.STREAMING;
                cgiOut = new CGIOutputStream(this, outputStream, HEADER_CHARSET, mode);
        ...
        private void buildEnvironment() {
            ...
            if (effectiveClientSocket != null) {
                ...
                env.put("REMOTE_PORT", Integer.toString(effectiveClientSocket.getPort()));
            ...
            if (extraEnv != null) {
                for (Iterator i = extraEnv.entrySet().iterator(); i.hasNext(); ) {
                    ...
                    if (!env.containsKey(e.getKey())) env.put(e.getKey(), e.getValue());
                }
            }
        }
        ...
        private Socket getEffectiveClientSocket() {
            if (clientSocket != null) return clientSocket; else return (Socket) effectiveClientSocket.get();
        }
        ...
        private class CGIPool extends ResourcePool {
            ...
            CGIPool(String name, Class c) throws IllegalArgumentException {
                super(name);
                if (!TinyCGI.class.isAssignableFrom(c)) throw new IllegalArgumentException(c.getName() + " does not implement " + "net.sourceforge.processdash.net.http.TinyCGI");
                cgiClass = c;
            }
        ...
        private ClassLoader getLoader(URLConnection conn) throws MalformedURLException {
            ...
            synchronized (cgiLoaderMap) {
                ...
                if (result == null) {
                    ...
                    if (parent == null) result = new URLClassLoader(pathURL); else result = new URLClassLoader(pathURL, parent);
        ...
        private TinyCGI getScript(final URLConnection conn, final String className) {
            CGIPool pool = null;
            synchronized (cgiCache) {
                pool = (CGIPool) cgiCache.get(path);
                if (pool == null) {
                    pool = (CGIPool) AccessController.doPrivileged(new PrivilegedAction() {

                        public Object run() {
                            return createCgiPool(conn, className);
                        }
                    });
                    if (pool != null) cgiCache.put(path, pool);
                }
            }
            if (pool == null) return null; else return (TinyCGI) pool.get();
        }
        ...
        private CGIPool createCgiPool(URLConnection conn, String className) {
            ...
            try {
                ...
                if (file != null && file.endsWith("/sizeest.class")) pool = new CGIPool(path, net.sourceforge.processdash.ui.web.psp.SizeEstimatingTemplate.class); else t.printStackTrace();
            }
            return pool;
        }
        ...
        private void servePlain(URLConnection conn, String mime_type) throws TinyWebThreadException, IOException {
            ...
            if (preprocess) servePreprocessedFile(content, translate, mime_type); else if (translate && mime_type.startsWith("text/html")) serveTranslatedFile(content, mime_type); else {
                discardHeader();
                sendHeaders(200, "OK", mime_type, getContentLength(conn), getLastModified(conn), null);
        ...
        private void servePreprocessedFile(URLConnection conn, String mimeType) throws TinyWebThreadException, IOException {
            String content = preprocessTextFile(conn.getInputStream(), false, mimeType);
        ...
        private String preprocessTextFile(InputStream in, boolean translate, String mimeType) throws TinyWebThreadException, IOException {
            ...
            HTMLPreprocessor p = new HTMLPreprocessor(WebServer.this, data.getSubcontext(prefix), env);
            if (mimeType != null && mimeType.indexOf("html") != -1) p.setDefaultEchoEncoding("html");
            String result = p.preprocess(content);
            if (translate) result = Translator.translate(result);
            return result;
        }
        ...
        private void checkIP() throws TinyWebThreadException, IOException {
            ...
            if (path.indexOf('/') == -1 || path.startsWith("Images/") || path.startsWith("help/") || path.startsWith("lib/")) return;
            InetAddress remoteIP = effectiveClientSocket.getInetAddress();
            if (remoteIP.isLoopbackAddress() || remoteIP.equals(LOCAL_HOST_ADDR)) return;
            parseHTTPHeaders();
            ...
            if (allowingRemoteConnections != ALLOW_REMOTE_ALWAYS) sendErrorOrAuth(403, "Forbidden", "Not accepting " + "requests from remote IP addresses .");
        }
        ...
        private void sendError(int status, String title, String text, String otherHeaders, Throwable cause) throws TinyWebThreadException {
            TinyWebThreadException result = new TinyWebThreadException(title + ": " + text);
            ...
            try {
                ...
                if (cause != null) {
                    ...
                    if (text == null) text = "Error encountered:<PRE>" + w + "</PRE>"; else text = text + "<!--\n" + w + "\n-->";
                }
        ...
        public void sendHeaders(int status, String title, String mimeType, long length, long mod, String otherHeaders) throws IOException {
    ...
    private URLConnection resolveURL(String url) {
        ...
        if (!TemplateLoader.isValidTemplateURL(url)) return null;
        if (url.startsWith(CMS_URI_PREFIX)) url = CMS_SCRIPT_PATH;
        ...
        for (Iterator v = variations.iterator(); v.hasNext(); ) {
            ...
            for (int i = 0; i < roots.length; i++) try {
                ...
                if (!u.toExternalForm().startsWith(roots[i].toExternalForm())) continue;
    ...
    private String getLocaleVariant(String url) {
        ...
        if (dotPos == -1) return url + "_" + lang; else return url.substring(0, dotPos) + "_" + lang + url.substring(dotPos);
    }
    ...
    public static String calcCredential(String user, String password) {
        ...
        credential = Base64.encodeBytes(credential.getBytes(), Base64.DONT_BREAK_LINES);
     ...
     */
    public static void setPassword(DataRepository data, String prefix, String user, String password) {
    ...
    public String getHostName(boolean forRemoteUse) {
        if (allowingRemoteConnections == ALLOW_REMOTE_NEVER) return "localhost";
        String result = Settings.getVal("http.hostname");
        if (result != null && result.length() > 0) return result;
        if (forRemoteUse) try {
            if (Settings.getBool("http.hostname.useIP", false)) return InetAddress.getLocalHost().getHostAddress(); else return InetAddress.getLocalHost().getHostName();
        } catch (UnknownHostException uhe) {
        }
        return "localhost";
    }
     ...
     */
    public byte[] getRequest(final String uri, boolean skipHeaders, Map extraEnvironment) throws IOException {
    ...
    public byte[] getContent(String context, String uri, boolean raw) throws IOException {
        uri = resolveUriInContext(context, uri);
        if (raw) return getRawRequest(uri); else return getRequest(uri, true);
    }
    ...
    public String getRequestAsString(String uri, Map extraEnv) throws IOException {
        ...
        if (contentType != null) charset = HTTPUtils.getCharset(contentType);
        return new String(response, headerLen, response.length - headerLen, charset);
    }
     ...
     */
    public byte[] getRawRequest(final String uri) throws IOException {
    ...
    private void initAllowRemote() {
        ...
        if ("true".equalsIgnoreCase(setting)) allowingRemoteConnections = ALLOW_REMOTE_ALWAYS; else if ("false".equalsIgnoreCase(setting)) allowingRemoteConnections = ALLOW_REMOTE_MAYBE; else allowingRemoteConnections = ALLOW_REMOTE_NEVER;
        DEFAULT_EXTERNAL_HOSTNAME = getHostName(true);
    }

    private InetAddress getListenAddress() {
        InetAddress listenAddress = null;
        if (allowingRemoteConnections == ALLOW_REMOTE_NEVER) listenAddress = LOOPBACK_ADDR;
     ...
     */
    WebServer(String directoryToServe, int port) throws IOException {
    ...
    public void setData(DataRepository data) {
        if (System.getSecurityManager() != null) SET_DATA_PERMISSION.checkPermission();
        this.data = data;
    ...
    public void setDashboardContext(DashboardContext dashboardContext) {
        ...
        if (dashboardContext == null) DEFAULT_ENV.remove(TinyCGI.DASHBOARD_CONTEXT); else DEFAULT_ENV.put(TinyCGI.DASHBOARD_CONTEXT, dashboardContext);
    }
    ...
    private class ServerSocketListener extends Thread implements PropertyChangeListener {
        ...
        public ServerSocketListener(int port) throws IOException {
            ...
            InternalSettings.addPropertyChangeListener(HTTP_ALLOWREMOTE_SETTING, this);
            openSocket();
        }
        ...
        public void run() {
            while (isRunning) {
                if (serverSocket == null) openSocket();
                try {
    ...
    public void addPort(int port) throws IOException {
        ...
        for (Iterator i = serverSocketListeners.iterator(); i.hasNext(); ) {
            ...
            if (port == s.getPortNumber()) return;
        }
