...
public class SnagsSite implements Urlable, XmlDescriber {
    ...
    private Document document() throws ProgrammerException, UserException, DesignerException {
        ...
        snagsElement.appendChild(dceService.getXML(new XmlTree("provider", new XmlTree("organization")), doc));
        for (SnagSite snag : (List<SnagSite>) Hiber.session().createQuery("from SnagSite snag where snag.dateResolved is null and snag.service = :service order by snag.site.code.string, snag.description, snag.startDate.date").setEntity("service", dceService).setMaxResults(PAGE_SIZE).list()) {
    ...
    public void httpPost(Invocation inv) throws ProgrammerException, UserException, DesignerException, DeployerException {
        if (inv.hasParameter("ignore")) {
            ...
            ScrollableResults snags = Hiber.session().createQuery("from SnagSite snag where snag.service = :service and snag.finishDate < :ignoreDate").setEntity("service", dceService).setTimestamp("ignoreDate", ignoreDate.getDate()).scroll(ScrollMode.FORWARD_ONLY);
            while (snags.next()) {
