   ...
   */
     ...
     */
        public String toString() {
            return (startPC + " " + length + " " + name.toString() + " " + descriptor.toString() + " " + frameIndex + "\n");
        }
    }
    ...
    static LocalVariableTable readLocalVariableTable(DataInputStream input, int[] constantPool) throws IOException {
        ...
        if (numVars > 0) {
            ...
            for (int i = 0; i < numVars; ++i) {
                LocalVariable lv = new LocalVariable(input.readUnsignedShort(), input.readUnsignedShort(), ClassFileReader.getUtf(constantPool, input.readUnsignedShort()), ClassFileReader.getUtf(constantPool, input.readUnsignedShort()), input.readUnsignedShort());
                lvs[i] = lv;
            }
