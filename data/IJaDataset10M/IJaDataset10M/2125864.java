...
import com.sodad.weka.core.xml.KOML;
import com.sodad.weka.core.xml.XStream;
...
public class SerializedModelSaver extends JPanel implements BeanCommon, Visible, BatchClassifierListener, IncrementalClassifierListener, BatchClustererListener, EnvironmentHandler, Serializable {
   ...
   */
    protected transient com.sodad.weka.gui.Logger m_logger = null;
   ...
   */
    ...
    static {
        ...
        s_fileFormatsAvailable.add(new Tag(BINARY, "Binary serialized model file (*" + FILE_EXTENSION + ")", "", false));
        if (KOML.isPresent()) {
            s_fileFormatsAvailable.add(new Tag(KOMLV, "XML serialized model file (*" + KOML.FILE_EXTENSION + FILE_EXTENSION + ")", "", false));
        }
        if (XStream.isPresent()) {
            s_fileFormatsAvailable.add(new Tag(XSTREAM, "XML serialized model file (*" + XStream.FILE_EXTENSION + FILE_EXTENSION + ")", "", false));
        }
    }
   ...
   */
    public void setLog(com.sodad.weka.gui.Logger logger) {
        m_logger = logger;
    }
   ...
   */
    public void acceptClassifier(final BatchClassifierEvent ce) {
        ...
        if (ce.getClassifier() instanceof com.sodad.weka.classifiers.misc.InputMappedClassifier) {
            try {
                trainHeader = ((com.sodad.weka.classifiers.misc.InputMappedClassifier) ce.getClassifier()).getModelHeader(trainHeader);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
