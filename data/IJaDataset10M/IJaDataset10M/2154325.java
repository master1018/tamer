 ...
 */
public class FolderPermissionBusinessImpl extends AbstractBusinessImpl implements FolderPermissionBusiness {

    private static final String PAGE = "/page";

    @Override
    public FolderPermissionEntity getGuestPermission(final FolderEntity folder) {
        GroupEntity guests = getDao().getGroupDao().getGuestsGroup();
        return getGroupPermission(folder, guests.getId());
    }

    @Override
    public FolderPermissionEntity getPermission(final FolderEntity folder, final UserEntity user) {
        if (user == null) {
            return getGuestPermission(folder);
        }
        if (user.isAdmin()) {
            return new FolderPermissionEntity(folder.getId(), FolderPermissionType.ADMIN);
        }
        List<UserGroupEntity> userGroups = getDao().getUserGroupDao().selectByUser(user.getId());
        userGroups.add(new UserGroupEntity(getDao().getGroupDao().getGuestsGroup().getId(), user.getId()));
        List<FolderPermissionEntity> permissions = new ArrayList<FolderPermissionEntity>();
        for (UserGroupEntity userGroup : userGroups) {
            FolderPermissionEntity FolderPermission = getGroupPermission(folder, userGroup.getGroupId());
    ...
    private FolderPermissionEntity consolidatePermissions(List<FolderPermissionEntity> permissions) {
    ...
    private FolderPermissionEntity getGroupPermission(FolderEntity folder, Long groupId) {
        FolderEntity myFolder = folder;
        FolderPermissionEntity result = new FolderPermissionEntity(folder.getId());
        result.setPermission(FolderPermissionType.DENIED);
        while (myFolder != null) {
            FolderPermissionEntity perm = getDao().getFolderPermissionDao().getByFolderGroup(myFolder.getId(), groupId);
            if (perm != null) {
                return perm;
            }
            if (myFolder.isRoot()) {
    ...
    @Override
    public void setPermission(FolderEntity folder, GroupEntity group, FolderPermissionType permission) {
        FolderPermissionEntity perm = getDao().getFolderPermissionDao().getByFolderGroup(folder.getId(), group.getId());
        if (perm == null) {
    ...
    @Override
    public List<FolderPermissionEntity> getInheritedPermissions(final FolderEntity folder) {
        List<FolderPermissionEntity> result = new ArrayList<FolderPermissionEntity>();
        FolderEntity myFolder = getDao().getFolderDao().getById(folder.getId());
        while (myFolder != null) {
            result.addAll(getDao().getFolderPermissionDao().selectByFolder(myFolder.getId()));
            if (myFolder.isRoot()) {
                myFolder = null;
            } else {
                myFolder = getDao().getFolderDao().getById(myFolder.getParent());
    ...
    private FolderPermissionEntity getPagePermission(FolderEntity folder) {
        ...
        if (path.startsWith(PAGE)) {
            ...
            ContentPermissionEntity pagePermission = getContentPermissionBusiness().getPermission(pageUrl, VosaoContext.getInstance().getUser());
            result.setPermission(FolderPermissionType.fromContentPermissionType(pagePermission.getPermission()));
        }
        return result;
    }
    ...
    private List<FolderPermissionEntity> getPagePermissions(FolderEntity folder) {
        String path = getDao().getFolderDao().getFolderPath(folder.getId());
        List<FolderPermissionEntity> result = new ArrayList<FolderPermissionEntity>();
        if (path.startsWith(PAGE)) {
            String pageUrl = getFolderFromPageUrl(path);
            List<ContentPermissionEntity> pagePermissions = getContentPermissionBusiness().selectByUrl(pageUrl);
            for (ContentPermissionEntity perm : pagePermissions) {
                FolderPermissionEntity p = new FolderPermissionEntity();
                p.setId(-perm.getId());
                p.setPermission(FolderPermissionType.fromContentPermissionType(perm.getPermission()));
