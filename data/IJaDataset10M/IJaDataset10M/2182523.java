...
/**
 * <p>
 * Title: ZuordnungAuswahlkriteriumQuantitativLagermittelDAO<br>
 * Description: CRUD Operationen auf Zuordnung Auswahlkriterien Quantitativ - Lagermittel. Die Zuordnung wird �ber die Zuordnung der ID
 * Kriterium - ID Lagermittel hergestellt. Codiert als X-Y-Wert Paar. X = Auswahlkriterium, Y = Lagermittel. Hinter der Zuordnungstabelle
 * verbirgt sich folgende Fachlichkeit: Quantitative Bewertung von Lagermittel anhand von Auswahlkriterien<br>
 ...
 * @version %version: 1%
 */
public class ZuordnungAuswahlkriteriumQuantitativLagermittelDAO {

    private static Logger logger = Logger.getLogger(ZuordnungAuswahlkriteriumQuantitativLagermittelDAO.class);

    private static final String GET_ALL_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM = "SQL_SELECT_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM";

    private static final String DEL_ALL_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM = "SQL_DELETE_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM";

    private static final String SET_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM = "SQL_INSERT_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM";
    ...
    private ArrayList<ZuordnungXYPBean> loadAll() {
        ...
        try {
            ResultSet oResult = sql.executeReadSQL(GET_ALL_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM);
            while (oResult.next()) {
    ...
    private void beforeInsert() throws Exception {
        SQliteSQL sql = new SQliteSQL();
        try {
            sql.executeWriteSQL(DEL_ALL_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM, new Vector<String>());
        } catch (Exception e) {
    ...
    private ZuordnungXYPBean saveInsert(final ZuordnungXYPBean datensatz) throws Exception {
        ...
        try {
            ...
            String strSQLKey = SET_WISSENSDATENBANK_ZUORDNUNG_AUSWAHLKRITERIENQUANTITATIV_LM;
    ...
    /**
	 * Die Zuordnungen von Lagermittel zu quantitativen Auswahlkriterien in Form von XY Paaren.
	 * Codiert als X-Y-Wert Paar. X = Auswahlkriterium, Y = Lagermittel.
	 * 
	 * @return the zuordnungen auswahlkriterium lagermittel
	 */
    public Hashtable<ZuordnungXYTO, Boolean> getZuordnungenAuswahlkriteriumQuantitativLagermittel() {
        ...
        try {
            ...
            logger.debug("[getZuordnungenAuswahlkriteriumQuantitativLagermittel()] ArrayList holds " + zuordnungen.size() + " elements in total.");
        } catch (Exception e) {
            logger.error("[getZuordnungenAuswahlkriteriumQuantitativLagermittel()] Error: " + e.getMessage());
        }
        return zuordnungen;
    }

    /**
	 * saveZuordnungenAuswahlkriteriumQuantitativLagermittel.
	 * 
	 * @param transportobjekte - Transportobjekte die persistiert werden sollen
	 */
    public void saveZuordnungenAuswahlkriteriumQuantitativLagermittel(Hashtable<ZuordnungXYTO, Boolean> transportobjekte) {
        try {
            ArrayList<ZuordnungXYPBean> datensaetze = convertFromTO(transportobjekte);
            saveAll(datensaetze);
        } catch (Exception e) {
            logger.error("[saveZuordnungenAuswahlkriteriumQuantitativLagermittel()] Error: " + e.getMessage());
        }
    }
}
