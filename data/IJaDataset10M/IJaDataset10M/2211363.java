...
public class HVEIP08PredicateOnlyEngine extends PredicateOnlyPairingAsymmetricBlockCipher {
    ...
    public void initialize() {
        if (forEncryption) {
            if (!(key instanceof HVEIP08EncryptionParameters)) throw new IllegalArgumentException("HVEIP08EncryptionParameters are required for encryption.");
        } else {
            if (!(key instanceof HVEIP08SecretKeyParameters)) throw new IllegalArgumentException("HVEIP08SecretKeyParameters are required for decryption.");
        }
    ...
    public byte[] process(byte[] in, int inOff, int inLen) {
        if (key instanceof HVEIP08SecretKeyParameters) {
            ...
            if (secretKey.isAllStar()) return new byte[] { 1 };
            int offset = inOff;
            ...
            if (secretKey.isPreProcessed()) {
                ...
                for (int i = 0; i < secretKey.getParameters().getN(); i++) {
                    if (!secretKey.isStar(i)) {
                        result.mul(secretKey.getPreYAt(i).pairing(X.get(i))).mul(secretKey.getPreLAt(i).pairing(W.get(i)));
                    }
                }
                ...
                for (int i = 0; i < secretKey.getParameters().getN(); i++) {
                    if (!secretKey.isStar(i)) {
                        result.mul(pairing.pairing(secretKey.getYAt(i), X.get(i))).mul(pairing.pairing(secretKey.getLAt(i), W.get(i)));
                    }
                }
            }
            return new byte[] { (byte) (result.isOne() ? 1 : 0) };
        } else {
            if (inLen > inBytes || inLen < inBytes) throw new DataLengthException("input must be of size " + inBytes);
            ...
            for (int i = 0; i < n; i++) {
                ...
                elements.add(pk.getElementPowTAt(i, j).powZn(sMinusSi));
                elements.add(pk.getElementPowVAt(i, j).powZn(si));
            }
            ByteArrayOutputStream outputStream;
            try {
                outputStream = new ByteArrayOutputStream(getOutputBlockSize());
                for (Element element : elements) outputStream.write(element.toBytes());
            } catch (IOException e) {
