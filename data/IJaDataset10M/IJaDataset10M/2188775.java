...
public class JDK14AnnotationFinder implements IAnnotationFinder {
   ...
   */
    private synchronized void parseSource(Class clazz) {
        ...
        final String fileName = innerSignPos == -1 ? className.substring(className.lastIndexOf('.') + 1) : clazz.getName().substring(className.lastIndexOf('.') + 1, innerSignPos);
    ...
    public IAnnotation findAnnotation(Method m, Class annotationClass) {
        IAnnotation result = findMethodAnnotation(m.getName(), m.getParameterTypes(), m.getDeclaringClass(), annotationClass, m_transformer);
    ...
    public IAnnotation findAnnotation(Constructor m, Class annotationClass) {
        ...
        IAnnotation result = findMethodAnnotation(name, m.getParameterTypes(), m.getDeclaringClass(), annotationClass, m_transformer);
