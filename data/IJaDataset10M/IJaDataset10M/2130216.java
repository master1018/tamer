...
public class ZInputStream extends FilterInputStream {
    ...
    public synchronized int read() throws IOException {
        ...
        if (b == 1) return (one[0] & 0xff); else return -1;
    }
    ...
    private void parse_header() throws IOException {
        ...
        if (t < 0) throw new EOFException("Failed to read magic number");
        magic += t & 0xff;
        if (magic != LZW_MAGIC) throw new IOException("Input not in compress format (read " + "magic number 0x" + Integer.toHexString(magic) + ")");
        int header = in.read();
