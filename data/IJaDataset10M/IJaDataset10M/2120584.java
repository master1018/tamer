...
public final class CommandRecruit implements CommandRunnable {
    ...
    public void run(Connection source, BNetUser user, String param, String[] params, boolean whisperBack, Account commanderAccount, boolean superUser) throws Exception {
        ...
        if (commanderAccount == null) throw new CommandFailedWithDetailsException("You must have an account to use recruit.");
        BNetUser bnSubject = source.getCreateBNetUser(params[0], user);
        BNLogin rsSubject = BNLogin.get(bnSubject);
        if (rsSubject == null) throw new NeverSeenUserException(bnSubject);
        if (rsSubject.getAccount() != null) throw new CommandFailedWithDetailsException("That user already has an account!");
        ...
        if (requiredTagPrefix != null) {
            if (bnSubject.getFullAccountName().substring(0, requiredTagPrefix.length()).compareToIgnoreCase(requiredTagPrefix) != 0) throw new CommandFailedWithDetailsException("That user must have the " + requiredTagPrefix + " tag!");
        }
        if (requiredTagSuffix != null) {
            ...
            if (s.compareToIgnoreCase(requiredTagSuffix) != 0) throw new CommandFailedWithDetailsException("That user must have the " + requiredTagSuffix + " tag!");
        }
