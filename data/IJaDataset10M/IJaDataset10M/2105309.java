...
class RenderNode {
    ...
    static void render(GLAutoDrawable drawable, Object3D obj, Transform transform, Transform camGenericMatrix, int alphaMode) {
        if (obj instanceof Node) {
            if (!((Node) obj).isRenderingEnabled()) return;
        }
    ...
    private static void processMesh(GLAutoDrawable drawable, Mesh mesh, Transform transform, Transform camGenericMatrix, int alphaMode) {
        ...
        for (int i = 0; i < mesh.submeshes.length; i++) {
            ...
            if ((opaque && ((alphaMode & M3gRender.SURFACE_OPAQUE) != 0)) || (!opaque && ((alphaMode & M3gRender.SURFACE_TRANSPARENT) != 0))) {
    ...
    private static void processSkinnedMesh(GLAutoDrawable drawable, SkinnedMesh skinnedMesh, Transform transform, Transform camGenericMatrix, int alphaMode) {
        ...
        for (int i = 0; i < positions.length / 3; i++) {
            ...
            for (int j = 0; j < array.length; j++) {
                ...
                if ((i >= transformation.firstVertex) && (i < (transformation.firstVertex + transformation.vertexCount))) {
    ...
    private static void processSprite3D(GLAutoDrawable drawable, Sprite3D sprite3D, Transform transform, Transform camGenericMatrix, int alphaMode) {
        ...
        vertexArrayPoints.set(0, 4, new short[] { (short) (-sprite3D.getCropWidth() / 2), (short) (-sprite3D.getCropHeight() / 2), 0, (short) (sprite3D.getCropWidth() / 2), (short) (-sprite3D.getCropHeight() / 2), 0, (short) (-sprite3D.getCropWidth() / 2), (short) (sprite3D.getCropHeight() / 2), 0, (short) (sprite3D.getCropWidth() / 2), (short) (sprite3D.getCropHeight() / 2), 0 });
        VertexArray vertexArrayNormals = new VertexArray(4, 3, 2);
        vertexArrayNormals.set(0, 4, new short[] { 0, 0, -2, (short) sprite3D.getCropWidth(), 0, -2, 0, (short) sprite3D.getCropHeight(), -2, (short) sprite3D.getCropWidth(), (short) sprite3D.getCropHeight(), -2 });
        VertexArray vertexArrayTexture = new VertexArray(4, 2, 1);
        vertexArrayTexture.set(0, 4, new byte[] { 0, 1, 1, 1, 0, 0, 1, 0 });
        VertexArray vertexArrayColors = new VertexArray(4, 3, 1);
        vertexArrayColors.set(0, 4, new byte[] { 0, 0, (byte) 255, (byte) 255, (byte) 255, (byte) 0, (byte) 255, 0, 0, 0, (byte) 255, 0 });
        VertexBuffer vertexBuffer = new VertexBuffer();
        vertexBuffer.setPositions(vertexArrayPoints, 1, new float[] { sprite3D.getCropX() - sprite3D.getCropWidth() / 2, sprite3D.getCropY() - sprite3D.getCropHeight() / 2, 0 });
        ...
        transformAdd.postScale((1 / val) / (scale[0]), (1 / val) / (scale[1]), (1 / val) / (scale[2]));
