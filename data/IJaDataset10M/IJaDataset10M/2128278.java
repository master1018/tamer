...
public class TfIdfClassifierTrainer<E> implements ObjectHandler<Classified<E>>, Compilable, Serializable {
    ...
    public double idf(String feature) {
        ...
        if (featureId == null) return 0.0;
        ObjectToDoubleMap<Integer> otd = mFeatureToCategoryCount.get(featureId);
        int df = otd.size();
    ...
    void handle(E input, Classification classification) {
        ...
        Map<String, ? extends Number> featureVector = mFeatureExtractor.features(input);
        for (Map.Entry<String, ? extends Number> entry : featureVector.entrySet()) {
            ...
            ObjectToDoubleMap<Integer> categoryCounts = mFeatureToCategoryCount.get(featureId);
    ...
    static class Externalizer<F> extends AbstractExternalizable {
        ...
        public void writeExternal(ObjectOutput out) throws IOException {
            AbstractExternalizable.compileOrSerialize(mTrainer.mFeatureExtractor, out);
            ...
            for (int i = 0; i < numCats; ++i) out.writeUTF(mTrainer.mCategorySymbolTable.idToSymbol(i));
            ...
            for (Map.Entry<Integer, ObjectToDoubleMap<Integer>> entry : mTrainer.mFeatureToCategoryCount.entrySet()) {
            ...
            for (int i = 0; i < catLengths.length; ++i) catLengths[i] = Math.sqrt(catLengths[i]);
            for (int featureId = 0; featureId < numFeatures; ++featureId) {
                ObjectToDoubleMap<Integer> categoryCounts = mTrainer.mFeatureToCategoryCount.get(featureId);
                double idf = idf(categoryCounts.size(), numCatsD);
                for (Map.Entry<Integer, Double> categoryCount : categoryCounts.entrySet()) {
                    int catId = categoryCount.getKey().intValue();
                    double count = categoryCount.getValue().doubleValue();
        ...
        public Object read(ObjectInput objIn) throws ClassNotFoundException, IOException {
            @SuppressWarnings("unchecked") FeatureExtractor<? super F> featureExtractor = (FeatureExtractor<? super F>) objIn.readObject();
            ...
            for (int i = 0; i < numCategories; ++i) categories[i] = objIn.readUTF();
            float[] featureIdfs = new float[featureSymbolTable.numSymbols()];
            for (int i = 0; i < featureIdfs.length; ++i) featureIdfs[i] = objIn.readFloat();
            int[] featureOffsets = new int[numFeatures + 1];
            for (int i = 0; i < numFeatures; ++i) featureOffsets[i] = objIn.readInt();
            ...
            return new TfIdfClassifier<F>(featureExtractor, featureSymbolTable, categories, featureIdfs, featureOffsets, catIds, normedTfIdfs);
        }
    }
    ...
    static class TfIdfClassifier<G> implements ScoredClassifier<G> {
        ...
        public String toString() {
            ...
            for (int i = 0; i < mFeatureIdfs.length; ++i) {
                sb.append("  " + i + "  " + mFeatureSymbolTable.idToSymbol(i) + "   " + mFeatureIdfs[i] + "   " + mFeatureOffsets[i] + "\n");
            }
        ...
        public ScoredClassification classify(G in) {
            ...
            for (Map.Entry<String, ? extends Number> featureValue : featureVector.entrySet()) {
                ...
                for (int offset = mFeatureOffsets[featureId]; offset < mFeatureOffsets[featureId + 1]; ++offset) {
            ...
            List<ScoredObject<String>> catScores = new ArrayList<ScoredObject<String>>(mCategories.length);
    ...
    static class Serializer<F> extends AbstractExternalizable {
        ...
        @Override
        public void writeExternal(ObjectOutput out) throws IOException {
            AbstractExternalizable.serializeOrCompile(mTrainer.mFeatureExtractor, out);
        ...
        public Object read(ObjectInput objIn) throws ClassNotFoundException, IOException {
            @SuppressWarnings("unchecked") FeatureExtractor<? super F> featureExtractor = (FeatureExtractor<? super F>) objIn.readObject();
            @SuppressWarnings("unchecked") Map<Integer, ObjectToDoubleMap<Integer>> featureToCategoryCount = (Map<Integer, ObjectToDoubleMap<Integer>>) objIn.readObject();
            MapSymbolTable featureSymbolTable = (MapSymbolTable) objIn.readObject();
            MapSymbolTable categorySymbolTable = (MapSymbolTable) objIn.readObject();
            return new TfIdfClassifierTrainer<F>(featureExtractor, featureToCategoryCount, featureSymbolTable, categorySymbolTable);
        }
    }
}
