...
public class WMethod {
    ...
    public Collection<List<Label>> getFullTestSet(int extraStates) {
        if (fullTestSet == null) fullTestSet = computeNewTestSet(extraStates);
        return fullTestSet;
    }
    ...
    public void appendAllSequences(PrefixFreeCollection sequences, List<List<Label>> paths) {
        for (List<Label> path : paths) appendSequence(sequences, path);
    }
    ...
    public Collection<List<Label>> computeOldTestSet(int numberOfExtraStates) {
        ...
        if (characterisationSet.isEmpty()) characterisationSet.add(Arrays.asList(new Label[] {}));
    ...
    public static <ELEM> List<List<ELEM>> crossWithSet_One(List<List<ELEM>> a, Collection<ELEM> b) {
        if (b.size() == 1) {
            ...
            for (List<ELEM> elemA : a) elemA.add(element);
            return a;
        }
    ...
    public static class EquivalentStatesException extends Error {
        ...
        public Collection<AMEquivalenceClass<CmpVertex, LearnerGraphCachedData>> getEquivalentStates() {
            ...
            for (AMEquivalenceClass<CmpVertex, LearnerGraphCachedData> eqClass : getStatesToComputeReduction()) if (eqClass.getStates().size() > 1) equivalentStates.add(eqClass);
        ...
        public static Collection<AMEquivalenceClass<CmpVertex, LearnerGraphCachedData>> IdentifyEquivalentStates(final Map<CmpVertex, Integer> equivalenceClasses, LearnerGraph graph, CmpVertex sink) {
            ...
            Collection<AMEquivalenceClass<CmpVertex, LearnerGraphCachedData>> equivalentStates = new LinkedList<AMEquivalenceClass<CmpVertex, LearnerGraphCachedData>>();
            int number = 0;
            for (Entry<CmpVertex, Integer> stateA : equivalenceClasses.entrySet()) if (stateA.getKey() != sink) {
                AMEquivalenceClass<CmpVertex, LearnerGraphCachedData> equiv = mapOfEquivalentStates.get(stateA.getValue());
                if (equiv == null) {
                    equiv = new AMEquivalenceClass<CmpVertex, LearnerGraphCachedData>(number++, graph);
                    mapOfEquivalentStates.put(stateA.getValue(), equiv);
                }
                try {
    ...
    private static class TransitionRowEqClass extends HashMap<Label, Integer> {
        ...
        public boolean equals(Object obj) {
            if (obj == null || !(obj instanceof TransitionRowEqClass)) return false;
        ...
        public Set<Label> computeDistinguishingLabel(TransitionRowEqClass what) {
            ...
            while (mapAiter.hasNext()) {
                ...
                if (!enA.getValue().equals(mapBvalue)) distInputs.add(enA.getKey());
            }
            ...
            while (mapBiter.hasNext()) {
                ...
                if (!enB.getValue().equals(mapAvalue)) distInputs.add(enB.getKey());
            }
    ...
    public static List<List<Label>> computeWSetOrig(LearnerGraph fsm) throws EquivalentStatesException {
        ...
        for (CmpVertex state : fsm.transitionMatrix.keySet()) equivalenceClasses.put(state, state.isAccept() ? 1 : 0);
        ...
        for (Entry<CmpVertex, Integer> stateA : equivalenceClasses.entrySet()) {
            ...
            while (stateB_It.hasNext()) {
                Entry<CmpVertex, Integer> stateB = stateB_It.next();
                if (stateB.getKey().equals(stateA.getKey())) break;
        ...
        do {
            ...
            for (CmpVertex stateA : equivalenceClasses.keySet()) {
                ...
                if (labelNSmap != null) for (Entry<Label, CmpVertex> labelstate : labelNSmap.entrySet()) {
                    int targetEqClass = equivalenceClasses.get(labelstate.getValue());
                    if (targetEqClass != sinkEqClass) map.put(labelstate.getKey(), targetEqClass);
                }
                ...
                if (!sortedRows.containsKey(map)) {
                    ...
                    newEquivClasses.put(stateA, equivalenceClassNumber);
                } else newEquivClasses.put(stateA, sortedRows.get(map));
            }
            for (Entry<CmpVertex, Integer> stateA : equivalenceClasses.entrySet()) {
                Iterator<Entry<CmpVertex, Integer>> stateB_It = equivalenceClasses.entrySet().iterator();
                while (stateB_It.hasNext()) {
                    Entry<CmpVertex, Integer> stateB = stateB_It.next();
                    if (stateB.getKey().equals(stateA.getKey())) break;
                    if (stateA.getValue().equals(stateB.getValue()) && !newEquivClasses.get(stateA.getKey()).equals(newEquivClasses.get(stateB.getKey()))) {
                        List<Label> Wsequence = null;
                        if (Wdata.get(stateA.getKey()).containsKey(stateB.getKey())) Wsequence = Wdata.get(stateA.getKey()).get(stateB.getKey()); else Wsequence = Wdata.get(stateB.getKey()).get(stateA.getKey());
                        ...
                        if (Wdata.get(toA).containsKey(toB)) Wprevious = Wdata.get(toA).get(toB); else Wprevious = Wdata.get(toB).get(toA);
        ...
        if ((statesEquivalentToSink <= 2 && oldEquivalenceClassNumber == fsm.transitionMatrix.size() + 2 - statesEquivalentToSink) || fsm.config.getEquivalentStatesAllowedForW()) {
            boolean sinkAsRealState = !fsm.config.isPrefixClosed() && statesEquivalentToSink == 1;
            for (Entry<CmpVertex, Integer> stateA : equivalenceClasses.entrySet()) if (sinkAsRealState || stateA.getKey() != sink) {
                Iterator<Entry<CmpVertex, Integer>> stateB_It = equivalenceClasses.entrySet().iterator();
                while (stateB_It.hasNext()) {
                    Entry<CmpVertex, Integer> stateB = stateB_It.next();
                    if (stateB.getKey().equals(stateA.getKey())) break;
        ...
        } else throw EquivalentStatesException.construct(equivalenceClasses, fsm, sink);
        return result;
    }
    ...
    public static Label computeTopLabel(Map<CmpVertex, Map<CmpVertex, Set<Label>>> distinguishingLabels) {
        Map<Label, AtomicInteger> distLabelUsage = new HashMap<Label, AtomicInteger>();
        Label topLabel = null;
        int topLabelCounter = -1;
    ...
    public static Collection<List<Label>> computeWSet_reducedmemory(LearnerGraph fsmOrig) throws EquivalentStatesException {
        ...
        Configuration copyConfig = fsmOrig.config.copy();
        copyConfig.setLearnerCloneGraph(false);
        LearnerGraph fsm = new LearnerGraph(fsmOrig, copyConfig);
        CmpVertex sink = generateSinkState(fsm);
        fsm.transitionMatrix.put(sink, fsm.createNewRow());
        Map<CmpVertex, Integer> equivalenceClasses = new LinkedHashMap<CmpVertex, Integer>(), newEquivClasses = new LinkedHashMap<CmpVertex, Integer>();
        Map<Map<Label, Integer>, Integer> sortedRows = new HashMap<Map<Label, Integer>, Integer>();
        ...
        for (CmpVertex stateA : fsm.transitionMatrix.keySet()) {
            ...
            while (stateB_It.hasNext()) {
                Entry<CmpVertex, Integer> stateB = stateB_It.next();
                if (stateB.getKey().equals(stateA)) break;
                int index = fsm.wmethod.vertexToInt(stateA, stateB.getKey());
                WChar[index] = null;
                if (stateAaccept == stateB.getKey().isAccept()) WNext[index] = W_INDIST; else WNext[index] = W_NOPREV;
            }
        }
        int equivalenceClassNumber = 0, oldEquivalenceClassNumber = 0;
        int statesEquivalentToSink = 0;
        do {
            ...
            ArrayList<Label> labelList = new ArrayList<Label>(distinguishingLabels.size());
            labelList.addAll(0, distinguishingLabels.keySet());
        ...
        assert statesEquivalentToSink > 0 : "missing equivalence class associated with sink state";
        if ((statesEquivalentToSink <= 2 && oldEquivalenceClassNumber == fsm.transitionMatrix.size() + 1 - statesEquivalentToSink) || fsm.config.getEquivalentStatesAllowedForW()) {
            boolean sinkAsRealState = !fsm.config.isPrefixClosed() && statesEquivalentToSink == 1;
            for (Entry<CmpVertex, Integer> stateA : equivalenceClasses.entrySet()) if (sinkAsRealState || stateA.getKey() != sink) {
                Iterator<Entry<CmpVertex, Integer>> stateB_It = equivalenceClasses.entrySet().iterator();
                while (stateB_It.hasNext()) {
    ...
    public enum VERTEX_COMPARISON_KIND {
    ...
    private static class CollectionOfPairs {
        ...
        public boolean addAndCheck(StatePair pairAB) {
            ...
            if (row == null) {
                row = new TreeSet<CmpVertex>();
                pairs.put(pairAB.firstElem, row);
            }
            boolean result = row.contains(pairAB.secondElem);
            if (!result) row.add(pairAB.secondElem);
            return !result;
        }
        ...
        public Map<PAIRCOMPATIBILITY, Set<CmpVertex>> statesAssociatedToThoseRelatedTo(CmpVertex state) {
            ...
            if (map != null) for (Entry<CmpVertex, PAIRCOMPATIBILITY> entry : map.entrySet()) {
                Set<CmpVertex> row = result.get(entry.getValue());
                if (row == null) {
                    row = new TreeSet<CmpVertex>();
                    result.put(entry.getValue(), row);
                }
                ...
                if (rightHand == null) throw new DifferentFSMException(errorDescrPart1 + "which does not have a corresponding state in the expected graph, only " + pairs.toString() + " are known");
        ...
        public void checkPairsAssociatedCorrectly() {
            for (Entry<CmpVertex, Set<CmpVertex>> entry : pairs.entrySet()) {
                ...
                for (CmpVertex secondVertex : entry.getValue()) {
                    ...
                    if (!firstMap.equals(secondMap)) throw new DifferentFSMException("state pair " + entry.getKey() + " and " + secondVertex + " have incompatible associations");
                }
            }
        }
    }
    ...
    public static <TARGET_A_TYPE, TARGET_B_TYPE, CACHE_A_TYPE extends CachedData<TARGET_A_TYPE, CACHE_A_TYPE>, CACHE_B_TYPE extends CachedData<TARGET_B_TYPE, CACHE_B_TYPE>> DifferentFSMException checkM(AbstractLearnerGraph<TARGET_A_TYPE, CACHE_A_TYPE> expectedArg, CmpVertex stateExpectedArg, AbstractLearnerGraph<TARGET_B_TYPE, CACHE_B_TYPE> actualArg, CmpVertex stateActualArg, VERTEX_COMPARISON_KIND compareVertices) {
        ...
        if (expectedArgObject instanceof LearnerGraph && actualArgObject instanceof LearnerGraph) {
            expected = (LearnerGraph) expectedArgObject;
            actual = (LearnerGraph) actualArgObject;
        ...
        Queue<StatePair> currentExplorationBoundary = new LinkedList<StatePair>();
        ...
        switch(compareVertices) {
            case DEEP:
                if (!DeterministicDirectedSparseGraph.deepEquals(stateExpected, stateActual)) return new DifferentFSMException("vertices " + stateExpected + " and " + stateActual + " have different values of attributes");
                if (stateActual.getOrigState() == null) {
                    if (stateExpected.getOrigState() != null) return new DifferentFSMException("vertices " + stateExpected + " and " + stateActual + " have different names");
                } else if (!stateActual.getOrigState().equals(stateExpected.getOrigState())) return new DifferentFSMException("vertices " + stateExpected + " and " + stateActual + " have different names");
                break;
            case NAMES:
                if (!stateActual.getOrigState().equals(stateExpected.getOrigState())) return new DifferentFSMException("vertices " + stateExpected + " and " + stateActual + " have different names");
                break;
            case NONE:
                break;
        }
        ...
        while (!currentExplorationBoundary.isEmpty()) {
            ...
            if (statePair.firstElem.isAccept() != statePair.secondElem.isAccept()) return new DifferentFSMException("states " + statePair.firstElem + " and " + statePair.secondElem + " have a different acceptance labelling between the machines");
            ...
            Set<Label> outgoing = new TreeSet<Label>();
            ...
            for (Label label : outgoing) {
                ...
                if (!expectedTargets.containsKey(label)) {
                    if (prefixClosed) return new DifferentFSMException("no transition with expected label \"" + label + "\" from a state \"" + statePair.secondElem + "\" corresponding to \"" + statePair.firstElem + "\"");
                    nextExpectedState = sink;
                } else nextExpectedState = expectedTargets.get(label);
                CmpVertex nextActualState = null;
                if (!actualTargets.containsKey(label)) {
                    if (prefixClosed) return new DifferentFSMException("no transition with actual label \"" + label + "\" from a state \"" + statePair.firstElem + "\" corresponding to \"" + statePair.secondElem + "\"");
                    nextActualState = sink;
                } else nextActualState = actualTargets.get(label);
                StatePair nextPair = new StatePair(nextExpectedState, nextActualState);
                if (statesAddedToBoundary.addAndCheck(nextPair)) {
                    switch(compareVertices) {
                        case DEEP:
                            if (!DeterministicDirectedSparseGraph.deepEquals(nextExpectedState, nextActualState)) return new DifferentFSMException("vertices " + nextExpectedState + " and " + nextActualState + " have different values of attributes");
                            if (nextActualState.getOrigState() == null) {
                                if (nextExpectedState.getOrigState() != null) return new DifferentFSMException("vertices " + nextExpectedState + " and " + nextActualState + " have different names");
                            } else if (!nextActualState.getOrigState().equals(nextExpectedState.getOrigState())) return new DifferentFSMException("vertices " + nextExpectedState + " and " + nextActualState + " have different names");
                            break;
                        case NAMES:
                            if (!nextActualState.getOrigState().equals(nextExpectedState.getOrigState())) return new DifferentFSMException("vertices " + nextExpectedState + " and " + nextActualState + " have different names");
                            break;
                        case NONE:
                            break;
                    }
	 ...
	 * Unreachable states are ignored. 
	 */
    public static <TARGET_A_TYPE, TARGET_B_TYPE, CACHE_A_TYPE extends CachedData<TARGET_A_TYPE, CACHE_A_TYPE>, CACHE_B_TYPE extends CachedData<TARGET_B_TYPE, CACHE_B_TYPE>> DifferentFSMException checkM(AbstractLearnerGraph<TARGET_A_TYPE, CACHE_A_TYPE> expected, AbstractLearnerGraph<TARGET_B_TYPE, CACHE_B_TYPE> graph) {
	 ...
	 * Compatibility labelling other than INCOMPATIBLE is only checked for deterministic graphs.
	 */
    public static <TARGET_A_TYPE, TARGET_B_TYPE, CACHE_A_TYPE extends CachedData<TARGET_A_TYPE, CACHE_A_TYPE>, CACHE_B_TYPE extends CachedData<TARGET_B_TYPE, CACHE_B_TYPE>> DifferentFSMException checkM_and_colours(AbstractLearnerGraph<TARGET_A_TYPE, CACHE_A_TYPE> A, AbstractLearnerGraph<TARGET_B_TYPE, CACHE_B_TYPE> B, VERTEX_COMPARISON_KIND howToCompare) {
        DifferentFSMException ex = WMethod.checkM(A, A.getInit(), B, B.getInit(), howToCompare);
    ...
    public String checkW_is_corrent_boolean(Collection<List<Label>> wset, boolean prefixClosed, Set<Pair<CmpVertex, CmpVertex>> equivalentVertices) {
        for (CmpVertex stateA : coregraph.transitionMatrix.keySet()) {
            for (CmpVertex stateB : coregraph.transitionMatrix.keySet()) if (stateA != stateB && (equivalentVertices == null || (!equivalentVertices.contains(new Pair<CmpVertex, CmpVertex>(stateA, stateB)) && !equivalentVertices.contains(new Pair<CmpVertex, CmpVertex>(stateB, stateA))))) {
                boolean foundString = false;
                Iterator<List<Label>> pathIt = wset.iterator();
                while (pathIt.hasNext() && !foundString) {
                    List<Label> path = pathIt.next();
                    int aResult = coregraph.paths.tracePath(path, stateA, prefixClosed), bResult = coregraph.paths.tracePath(path, stateB, prefixClosed);
                    if ((aResult == AbstractOracle.USER_ACCEPTED && bResult >= 0) || (bResult == AbstractOracle.USER_ACCEPTED && aResult >= 0)) foundString = true;
                }
                if (!foundString) return "W set " + wset + " does not distinguish between " + stateA + " and " + stateB;
            }
        }
        return null;
    }
    ...
    public LearnerGraph Permute(FsmPermutator perm) {
        ...
        for (Entry<CmpVertex, Map<Label, CmpVertex>> row : coregraph.transitionMatrix.entrySet()) for (Entry<Label, CmpVertex> nextState : row.getValue().entrySet()) transitionList.add(new Pair<CmpVertex, Label>(row.getKey(), nextState.getKey()));
        ...
        for (Pair<CmpVertex, Label> p : permutation) {
            CmpVertex from = p.firstElem;
            Label label = p.secondElem;
    ...
    public boolean checkGraphNumeric() {
        for (CmpVertex vert : coregraph.transitionMatrix.keySet()) if (vert.getID().getKind() == DeterministicDirectedSparseGraph.VertexID.VertKind.NONE) return false;
        return true;
    }
