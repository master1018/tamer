   ...
   */
    ...
    public void testIaa() throws Exception {
        ...
        try {
            ...
            nameAnnFeat = "Op";
            isUsingLabel = true;
            testWithfeat(nameAnnSet, nameAnnType, nameAnnFeat, data, isUsingLabel);
            caseN = 2;
            nameAnnType = "Os";
            nameAnnFeat = null;
            isUsingLabel = false;
    ...
    public void testWithfeat(String nameAnnSets, String nameAnnType, String nameAnnFeat, Corpus data, boolean isUsingLabel) {
        ...
        if (isUsingLabel) {
            ...
            String[] labelsArr = new String[labelsSet.size()];
        ...
        } else iaa = new IaaCalculation(nameAnnType, annArr2, verbo);
        ...
        for (int i = 0; i < annArr2.length; ++i) if (!IaaCalculation.isSameInstancesForAnnotators(annArr2[i], 0)) {
            isSuitable = false;
            break;
        }
        AnnotationSet[] refAnnsArr = new AnnotationSet[annArr2.length];
        boolean[] isMerged = new boolean[annArr2.length];
        for (int i = 0; i < annArr2.length; ++i) isMerged[i] = false;
        for (int iJ = 0; iJ < annArr2.length; ++iJ) {
            ...
            for (Annotation ann : mergeInfor.keySet()) {
                ...
                try {
                    annsDoc.add(ann.getStartNode().getOffset(), ann.getEndNode().getOffset(), nameAnnType, featM);
        ...
        int[] nPwKa = new int[3];
        nPwKa[0] = (int) Math.ceil((double) iaa.contingencyOverall.observedAgreement * 100);
        ...
        iaa.allwayIaaKappa();
        int[] nAwKa = new int[3];
        nAwKa[0] = (int) Math.ceil((double) iaa.contingencyOverall.observedAgreement * 100);
    ...
    /** Check the numbers. */
    private void checkNumbers(int[] nPwF, int[] nAwF, int[] nPwKa, int[] nAwKa, boolean isSuitable) {
        switch(caseN) {
            case 1:
