...
public class ERJavaMail extends ERXFrameworkPrincipal {
    ...
    public void initializeFrameworkFromSystemProperties() {
        ...
        if (queueSize >= 1) this.setSenderQueueSize(queueSize);
        ...
        if (milliswait > 1000) this.setMilliSecondsWaitIfSenderOverflowed(milliswait);
    ...
    public String validateEmail(EOEnterpriseObject object, String key, String email) {
        if (email != null) {
            if (!this.isValidEmail(email)) throw ERXValidationFactory.defaultFactory().createException(object, key, email, "malformedEmail");
        }
        return email;
    }
    ...
    public NSArray<String> filterEmailAddresses(NSArray<String> emailAddresses) {
        ...
        if ((emailAddresses != null) && (emailAddresses.count() > 0) && (this.hasWhiteList() || this.hasBlackList())) {
            ...
            if (this.hasBlackList()) {
                @SuppressWarnings("unchecked") NSArray<String> filteredOutAddresses = EOQualifier.filteredArrayWithQualifier(filteredAddresses, blackListQualifier());
                if (filteredOutAddresses.count() > 0) filteredAddresses.removeObjectsInArray(filteredOutAddresses);
                if (log.isDebugEnabled()) {
