...
public class DisplayTest extends TestCase {
    ...
    private static final class MockDisplayable extends Displayable {
    ...
    private static final class MockRunner implements Runnable {
	 ...
	 * @param runner
	 */
    private void assertEventuallyRun(boolean value, MockRunner runner, int timeout) {
        long start = System.currentTimeMillis();
        while (System.currentTimeMillis() < start + timeout) {
            if (value == runner.hasRun) return;
            try {
                Thread.sleep(100);
            } catch (InterruptedException exception) {
                fail("Interrupted");
	 ...
	 * @param timeout
	 */
    private final void assertPrePaintValue(int value, MockDisplayable disp, int timeout) {
        long start = System.currentTimeMillis();
        while (System.currentTimeMillis() < start + timeout) {
            if (value == disp.prePaintCount) return;
            try {
                Thread.sleep(100);
            } catch (InterruptedException exception) {
                fail("Interrupted");
