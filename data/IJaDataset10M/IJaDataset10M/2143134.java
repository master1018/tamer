...
public class CCQuerySupport {
    ...
    private static final Logger logger = Logger.getLogger(CCQuerySupport.class.getName());
     ...
     */
    public static Object execute(XmlRpcClient client, String name, Object... params) throws XmlRpcException {
        try {
            String namespace = CCWebService.getNamespace(client);
            return client.execute(namespace + name, params);
        } catch (XmlRpcException x) {
            String paramStr = (params == null ? "()" : Arrays.asList(params).toString());
            logger.log(Level.WARNING, "Encountered problem when executing XML RPC Query " + name + paramStr, x);
            throw x;
        }
    }
     ...
     *                 if an error is encountered
     */
    public static Object[] querySimple(XmlRpcClient client, String clazz, int max, Object... criteria) throws XmlRpcException {
        Map criteriaMap = new HashMap();
        for (int i = 0; i < criteria.length; i += 2) {
            criteriaMap.put(criteria[i], criteria[i + 1]);
        }
        return (Object[]) execute(client, "queryObjectsSimple", clazz, criteriaMap, max);
    }
     ...
     *                 attribute.
     */
    public static Object lookupSingleValue(XmlRpcClient client, String clazz, String attr, Object... criteria) throws XmlRpcException, SingleValueNotFoundException {
        Object[] list = querySimple(client, clazz, 2, criteria);
        if (list == null || list.length != 1) throw new SingleValueNotFoundException("Expected a single element");
        Map element = (Map) list[0];
        Object result = element.get(attr);
        if (result != null) return result; else throw new SingleValueNotFoundException("Expected " + clazz + " item to have an " + attr);
    }
    ...
    public static final String METADATA_DESCRIPTION_CLASS = NAMESPACE + "MetaDataDescriptionData";

    public static final String STRING_METADATA_CLASS = NAMESPACE + "MetaDataValueStringData";

    public static final String INTEGER_METADATA_CLASS = NAMESPACE + "MetaDataValueIntegerData";

    public static final String SELECT_METADATA_CLASS = NAMESPACE + "MetaDataSelectItemData";
}
