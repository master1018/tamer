...
public class Utilities {
    ...
    public static long byteArrayToLong(byte[] value) {
        long ret = 0;
        for (int i = 0; i < 8; i++) ret |= ((long) value[i] << (8 * i));
        return ret;
    }
    ...
    public static String byteArrayToBitString(byte[] value) {
        ...
        for (int i = value.length - 1; i >= 0; i--) {
            for (int j = 7; j >= 0; j--) {
                if ((value[i] & (int) Math.pow(2, j)) > 0) ret = ret.concat("1"); else ret = ret.concat("0");
            }
    ...
    public static String byteArrayToString(byte[] value) {
        ...
        for (int i = 0; i < value.length; i++) {
            if (value[i] >= 32 && value[i] <= 126) ret = ret + ((char) value[i]); else ret = ret.concat(".");
        }
        return ret;
    }
    ...
    public static String intToBinaryString(int value) {
        ...
        return bin.substring(bin.length() - 32, bin.length() - 24) + " " + bin.substring(bin.length() - 24, bin.length() - 16) + " " + bin.substring(bin.length() - 16, bin.length() - 8) + " " + bin.substring(bin.length() - 8, bin.length());
    }
