package com.farata.cleardatabuilder.validation.correction.proposals;
...
public class AddTransferInfoProposal extends MissingAnnotationAttributesProposal {
    ...
    private Expression newDefaultExpression(AST ast, ITypeBinding type) {
        if (type.isPrimitive()) {
            ...
            if ("boolean".equals(name)) return ast.newBooleanLiteral(false); else return ast.newNumberLiteral("0");
        }
        if (type == ast.resolveWellKnownType("java.lang.String")) return ast.newStringLiteral();
        if (type.isArray()) {
