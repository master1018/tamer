...
public class GraphCentrality extends NodeMetricSupport<Double> {
    ...
    public <V> Double value(Graph<V> graph, V node) {
        ...
        if (useSum) {
            double sum = 0.0;
            for (Integer i : lengths.values()) sum += i;
            return cptSize / n * (n - 1.0) / sum;
        } else {
            double max = 0.0;
            for (Integer i : lengths.values()) max = Math.max(max, i);
            return cptSize / n * 1.0 / max;
        }
    }

    public <V> List<Double> allValues(Graph<V> graph) {
        if (graph.order() == 0) return Collections.emptyList(); else if (graph.order() == 1) return Arrays.asList(0.0);
        ...
        for (Graph<V> cg : components) if (cg.order() == 1) values.put(cg.nodes().get(0), 0.0); else computeAllValuesConnected(cg, values);
        for (Graph<V> cg : components) {
            double multiplier = cg.order() / (double) n;
            for (V v : cg.nodes()) values.put(v, multiplier * values.get(v));
        }
        ArrayList<Double> result = new ArrayList<Double>(n);
        for (V v : graph.nodes()) result.add(values.get(v));
    ...
    private <V> void computeAllValuesConnected(Graph<V> graph, Map<V, Double> values) {
        ...
        for (V start : nodes) {
            ...
            if (useSum) {
                double sum1 = 0.0;
                for (Integer j : lengths.values()) sum1 += j;
                values.put(start, max / sum1);
            } else {
                double max1 = 0.0;
                for (Integer j : lengths.values()) max1 = Math.max(max1, j);
