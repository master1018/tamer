...
class SelectionManager {
    ...
    void addToSelection(Selection addMe) {
        if (addMe.start > addMe.end) {
            ...
            if (addMe instanceof Selection.Range) return; else if (addMe instanceof Selection.Rect) {
                if (((Selection.Rect) addMe).extraEndVirt == 0) return;
            }
        }
    ...
    void setSelection(Selection selection) {
        this.selection.clear();
        if (selection != null) addToSelection(selection);
    }
    ...
    void resizeSelection(int offset, int end, int extraEndVirt, boolean rect) {
        ...
        if (rect) {
            ...
            if (reversed) rectSel.extraStartVirt = extraEndVirt; else rectSel.extraEndVirt = extraEndVirt;
            newSel = rectSel;
        } else newSel = new Selection.Range(offset, end);
    ...
    int[] getSelectionStartAndEnd(int screenLine, int physicalLine, Selection s) {
        ...
        if (end <= s.start || start > s.end) return null;
        int selStartScreenLine;
        if (textArea.displayManager.isLineVisible(s.startLine)) selStartScreenLine = textArea.getScreenLineOfOffset(s.start); else selStartScreenLine = -1;
        int selEndScreenLine;
        if (textArea.displayManager.isLineVisible(s.endLine)) selEndScreenLine = textArea.getScreenLineOfOffset(s.end); else selEndScreenLine = -1;
        ...
        if (s instanceof Selection.Rect) {
            ...
            if (end <= rectStart || start > rectEnd) return null;
