...
public class SourceReorganiser {
    ...
    public static void reorganise(DecompilationContext context, MemoryVirtualFile file) {
            ...
            for (String line : lines) {
                ...
                if (lineNumberMatcher.find()) {
                    ...
                    if (currentBlock.getLastLine() != null && currentBlock.getLastLine().getNumber() == lineNumber) {
                        ...
                        currentBlock.add(new Line(lineStringWithoutNumber, lineNumber, retainLineNumbers));
                    }
                    ...
                    if ("{".equals(trimmedLineString)) {
                        ...
                        if (trimmedLineString.startsWith("}")) {
                            ...
                            currentBlock.add(new Line(line.substring(0, closingBrace + 1)));
                            currentBlock = currentBlock.getParent();
                            line = line.substring(0, closingBrace) + line.substring(closingBrace + 1).trim();
                        }
                        currentBlock = new Block(currentBlock, new Line(line));
    ...
    private static class Line extends Element {
        ...
        Line(String content) {
            this(content, -1, false);
        }

        Line(String content, int number, boolean lineNumberAsSuffix) {
            this.number = number;
    ...
    private static class Block extends Element {
        ...
        private void checkOffLines(Map offLines, Line line, int currentLine, StringWriter out) {
            ...
            for (Line line : lines) {
                if (line.hasNumber()) {
                    if (line.getNumber() < currentLine) {
                        offLines.put(line.getNumber(), line);
                    } else {
