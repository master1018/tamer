...
public class PushToVim implements PushToApplication {
    ...
    private JTextField vimPath = new JTextField(30), vimServer = new JTextField(30), citeCommand = new JTextField(30);
    ...
    public JPanel getSettingsPanel() {
        if (settings == null) initSettingsPanel();
    ...
    public void operationCompleted(BasePanel panel) {
        if (couldNotConnect) JOptionPane.showMessageDialog(panel.frame(), "<HTML>" + Globals.lang("Could not connect to Vim server. Make sure that " + "Vim is running<BR>with correct server name." + "</HTML>"), Globals.lang("Error"), JOptionPane.ERROR_MESSAGE); else if (couldNotRunClient) JOptionPane.showMessageDialog(panel.frame(), Globals.lang("Could not run the 'vim' program."), Globals.lang("Error"), JOptionPane.ERROR_MESSAGE); else {
