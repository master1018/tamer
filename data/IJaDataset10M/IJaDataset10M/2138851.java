...
public class RDFTopicMapReader implements TopicMapReaderIF, TopicMapImporterIF {
   ...
   */
    ...
    public TopicMapIF read() throws IOException {
        ...
        ((InMemoryTopicMapStore) topicmap.getStore()).setBaseAddress(new URILocator(infileurl));
        importInto(topicmap);
        return topicmap;
    }
    ...
    public void importInto(TopicMapIF topicmap) throws IOException {
        try {
            RDFToTopicMapConverter.convert(infileurl, syntax, mappingurl, mappingsyntax, topicmap, lenient);
            if (generate_names) RDFToTopicMapConverter.generateNames(topicmap);
        } catch (JenaException e) {
            throw new OntopiaRuntimeException(e);
        }
        if (duplicate_suppression) DuplicateSuppressionUtils.removeDuplicates(topicmap);
    }
