...
public class PageDaoImpl extends BaseDaoImpl<PageEntity> implements PageDao {
    ...
    public List<PageEntity> selectAllChildren(final String parentUrl, Date startDate, Date endDate) {
        Query q = newQuery();
        q.addFilter("parentUrl", FilterOperator.EQUAL, parentUrl);
        ...
        return select(q, "selectAllChildrenDate", params(parentUrl, startDate, endDate));
    }
    ...
    public String getContent(Long pageId, String languageCode) {
        ContentEntity content = getContentDao().getByLanguage(PAGE_CLASS_NAME, pageId, languageCode);
        if (content != null) {
    ...
    @Override
    public ContentEntity setContent(Long pageId, String languageCode, String content) {
        ContentEntity contentEntity = getContentDao().getByLanguage(PAGE_CLASS_NAME, pageId, languageCode);
        if (contentEntity == null) {
            contentEntity = new ContentEntity(PAGE_CLASS_NAME, pageId, languageCode, content);
        } else {
    ...
    private List<PageEntity> filterLatestVersion(List<PageEntity> list) {
        ...
        for (PageEntity page : list) {
            ...
            if (!pages.containsKey(key) || pages.get(key).getVersion() < page.getVersion()) {
                pages.put(key, page);
            }
        }
    ...
    private List<PageEntity> filterApproved(List<PageEntity> list) {
        Map<String, PageEntity> pages = new HashMap<String, PageEntity>();
        for (PageEntity page : list) {
            if (page.isApproved() && !page.isForInternalUse() && page.isPublished()) {
                String key = page.getFriendlyURL();
                if (!pages.containsKey(key) || pages.get(key).getVersion() < page.getVersion()) {
                    pages.put(key, page);
                }
    ...
    @Override
    public List<PageEntity> getByParentApproved(final String url, Date startDate, Date endDate) {
        List<PageEntity> result = filterApproved(selectAllChildren(url, startDate, endDate));
        Collections.sort(result, PageHelper.PUBLISH_DATE);
        return result;
    }
    ...
    @Override
    public List<PageEntity> selectByStructureTemplate(Long structureTemplateId) {
        Query q = newQuery();
        q.addFilter("structureTemplateId", FilterOperator.EQUAL, structureTemplateId);
        return select(q, "selectByStructureTemplate", params(structureTemplateId));
    }
