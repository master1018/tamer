...
public class InventoryFactory {
    ...
    public Inventory[] getSimilarInventories(String filePath, String inventoryCategory) throws SQLException {
        ...
        Statement stat = null;
        ResultSet resSet = null;
        String SQL_QUERY = "SELECT * from inventory where LOWER(name) like '" + inventoryCategory + "' and pathtofile<>'" + filePath + "' order by name, lastpolltime DESC";
        dbConn = Vault.getDbConnection();
    ...
    public Inventory[] getSimilarInventories(String inventoryCategory) throws SQLException {
        ...
        Statement stat = null;
        ResultSet resSet = null;
        String SQL_QUERY = "SELECT * from inventory where LOWER(name) like '" + inventoryCategory + "' order by lastpolltime DESC";
        dbConn = Vault.getDbConnection();
        stat = dbConn.createStatement();
        resSet = stat.executeQuery(SQL_QUERY);
        Inventory[] inventories = rsToInventory(resSet);
    ...
    public Inventory[] getInventories(String nodeLabel, String ipAddr, String name, String status, int lastpolltimeint) throws SQLException {
        nodeLabel = nodeLabel.toLowerCase();
        name = name.toLowerCase();
        Connection dbConn = null;
        Statement stat = null;
        ResultSet resSet = null;
        String SQL_QUERY = "SELECT * from inventory where pathtofile like '%%'";
        if (nodeLabel != null && !nodeLabel.equals("")) SQL_QUERY += " and nodeid in (select nodeid from node where LOWER(nodelabel) like '%" + nodeLabel + "%')";
        if (ipAddr != null && !ipAddr.equals("")) {
            ipAddr = ipAddr.replace('*', '%');
            SQL_QUERY += (" and nodeid in (select nodeid from ipinterface where ipaddr like '%" + ipAddr + "%')");
        }
        if (!name.equals("0")) SQL_QUERY += (" and lower(name)='" + name + "'");
        if (!status.equals("Y")) SQL_QUERY += (" and status='" + status + "'");
        if (lastpolltimeint != 0) {
            if (lastpolltimeint == 1) SQL_QUERY += " and lastpolltime>=(select now()-interval'1 hour')"; else if (lastpolltimeint == 2) SQL_QUERY += " and lastpolltime>=(select now()-interval'4 hours')"; else if (lastpolltimeint == 3) SQL_QUERY += " and lastpolltime>=(select now()-interval'8 hours')"; else if (lastpolltimeint == 4) SQL_QUERY += " and lastpolltime>=(select now()-interval'12 hours')"; else if (lastpolltimeint == 5) SQL_QUERY += " and lastpolltime>=(select now()-interval'24 hours')"; else if (lastpolltimeint == 6) SQL_QUERY += " and lastpolltime>=(select now()-interval'168 hours')"; else if (lastpolltimeint == 7) SQL_QUERY += " and lastpolltime>=(select now()-interval'744 hours')";
        }
    ...
    private Inventory[] rsToInventory(ResultSet rs) throws SQLException {
