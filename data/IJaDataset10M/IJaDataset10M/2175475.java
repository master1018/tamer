...
public class PointSound extends Sound {
   ...
   * information.
   */
    public static final int ALLOW_POSITION_READ = CapabilityBits.POINT_SOUND_ALLOW_POSITION_READ;

    /**
   * Specifies that this node allows writing to its object's position
   * information.
   */
    public static final int ALLOW_POSITION_WRITE = CapabilityBits.POINT_SOUND_ALLOW_POSITION_WRITE;

    /**
   * Specifies that this node allows access to its object's distance
   * gain attenuation information.
   */
    public static final int ALLOW_DISTANCE_GAIN_READ = CapabilityBits.POINT_SOUND_ALLOW_DISTANCE_GAIN_READ;
   ...
   */
    ...
    private static final int[] readCapabilities = { ALLOW_POSITION_READ, ALLOW_DISTANCE_GAIN_READ };
    ...
    public void setPosition(Point3f position) {
        if (isLiveOrCompiled()) if (!this.getCapability(ALLOW_POSITION_WRITE)) throw new CapabilityNotSetException(Ding3dI18N.getString("PointSound0"));
        ((PointSoundRetained) this.retained).setPosition(position);
    }
    ...
    public void getPosition(Point3f position) {
        if (isLiveOrCompiled()) if (!this.getCapability(ALLOW_POSITION_READ)) throw new CapabilityNotSetException(Ding3dI18N.getString("PointSound2"));
    ...
    public void setDistanceGain(Point2f[] attenuation) {
        if (isLiveOrCompiled()) if (!this.getCapability(ALLOW_DISTANCE_GAIN_WRITE)) throw new CapabilityNotSetException(Ding3dI18N.getString("PointSound3"));
    ...
    public int getDistanceGainLength() {
        if (isLiveOrCompiled()) if (!this.getCapability(ALLOW_DISTANCE_GAIN_READ)) throw new CapabilityNotSetException(Ding3dI18N.getString("PointSound4"));
