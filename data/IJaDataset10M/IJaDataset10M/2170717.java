...
public class SameStampScoreCombiner implements ScoreCombiner {
    ...
    public void combineScore(CombineToken combineToken) {
        List tokenList = Arrays.asList(combineToken.getParallelTokens().toArray());
        ...
        while (tokenList.size() > 0) {
            ...
            List sameStampList = removeTokensByStamp(tokenList, firstToken.getLastCombineStamp());
            double logCombinedScore = getCombinedScore(sameStampList);
            if (logCombinedScore > logHighestCombinedScore || logHighestCombinedScore == Double.NEGATIVE_INFINITY) {
