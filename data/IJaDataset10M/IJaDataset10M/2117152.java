...
public abstract class jq_Type implements Textualizable {
    ...
    private void initializeClassObject() {
        if (jq.RunningNative) this.class_object = ClassLibInterface.DEFAULT.createNewClass(this); else if (USE_CLASS_OBJECT_FIELD) this.class_object = Reflection.getJDKType(this);
    }
    ...
    public final Class getJavaLangClassObject() {
        if (jq.RunningNative && this.class_object == null) initializeClassObject();
        return class_object;
    }
    ...
    public static jq_Type parseType(String s) {
        ...
        if (s.length() == 1) {
            ...
            if (s.length() == 1) {
                jq_Primitive t = (jq_Primitive) PrimordialClassLoader.loader.getBSType(s);
                if (t == null) s = "L" + s + ";";
            } else if (!s.startsWith("[") && !s.endsWith(";")) s = "L" + s + ";";
            while (--arrayDepth >= 0) s = "[" + s;
        }
    ...
    public final boolean isSubtypeOf(jq_Type that) {
        ...
        if (off > 1) {
            if (TRACE) {
                ...
                Debug.write(off);
                Debug.write(" is ");
                if (this.display[off] == null) Debug.writeln("null"); else Debug.writeln(this.display[off].getDesc());
            }
            return false;
        }
        if (this == that) {
