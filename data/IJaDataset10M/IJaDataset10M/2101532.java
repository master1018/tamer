...
public class RBBIAPITest extends com.ibm.icu.dev.test.TestFmwk {
    ...
    public void TestCloneEquals() {
        ...
        logln("Testing == and !=");
        if (!bi1.equals(biequal) || bi1.equals(bi2) || bi1.equals(bi3)) errln("ERROR:1 RBBI's == and !- operator failed.");
        if (bi2.equals(biequal) || bi2.equals(bi1) || biequal.equals(bi3)) errln("ERROR:2 RBBI's == and != operator  failed.");
        logln("Testing clone()");
        ...
        if (!bi1clone.equals(bi1) || !bi1clone.equals(biequal) || bi1clone.equals(bi3) || bi1clone.equals(bi2)) errln("ERROR:1 RBBI's clone() method failed");
        if (bi2clone.equals(bi1) || bi2clone.equals(biequal) || bi2clone.equals(bi3) || !bi2clone.equals(bi2)) errln("ERROR:2 RBBI's clone() method failed");
        if (!bi1.getText().equals(bi1clone.getText()) || !bi2clone.getText().equals(bi2.getText()) || bi2clone.equals(bi1clone)) errln("ERROR: RBBI's clone() method failed");
    }
    ...
    public void TestToString() {
        ...
        if (temp2.equals(temp3) || temp.equals(temp2) || !temp.equals(temp3)) errln("ERROR: error in toString() method");
    }
    ...
    public void TestHashCode() {
        ...
        RuleBasedBreakIterator bi1clone = (RuleBasedBreakIterator) bi1.clone();
        RuleBasedBreakIterator bi2clone = (RuleBasedBreakIterator) bi2.clone();
        if (bi1.hashCode() != bi1clone.hashCode() || bi1.hashCode() != bi3.hashCode() || bi1clone.hashCode() != bi3.hashCode() || bi2.hashCode() != bi2clone.hashCode()) errln("ERROR: identical objects have different hashcodes");
        if (bi1.hashCode() == bi2.hashCode() || bi2.hashCode() == bi3.hashCode() || bi1clone.hashCode() == bi2clone.hashCode() || bi1clone.hashCode() == bi2.hashCode()) errln("ERROR: different objects have same hashcodes");
    }
    ...
    public void TestGetSetText() {
        ...
        if (wordIter1.current() != 0) errln("ERROR:1 setText did not set the iteration position to the beginning of the text, it is" + wordIter1.current() + "\n");
        wordIter1.next(2);
        wordIter1.setText(str2);
        if (wordIter1.current() != 0) errln("ERROR:2 setText did not reset the iteration position to the beginning of the text, it is" + wordIter1.current() + "\n");
    }
    ...
    public void TestFirstNextFollowing() {
        ...
        if (p != RuleBasedBreakIterator.DONE || q != RuleBasedBreakIterator.DONE) errln("ERROR: next()/following() at last position returned #" + p + " and " + q + " instead of" + testString.length() + "\n");
        ...
        if (p != RuleBasedBreakIterator.DONE || q != RuleBasedBreakIterator.DONE) errln("ERROR: following()/next() at last position returned #" + p + " and " + q + " instead of" + testString.length());
    ...
    public void TestLastPreviousPreceding() {
        ...
        if (p != RuleBasedBreakIterator.DONE) errln("ERROR: preceding()  at starting position returned #" + p + " instead of 0");
        testString = "Write hindi here. भारत सुंदर हौ।";
        logln("testing character iteration for string \" " + testString + "\" \n");
        RuleBasedBreakIterator charIter1 = (RuleBasedBreakIterator) RuleBasedBreakIterator.getCharacterInstance(Locale.getDefault());
        charIter1.setText(testString);
        p = charIter1.last();
        if (p != testString.length()) errln("ERROR: first() returned" + p + "instead of" + testString.length());
        ...
        if (p != RuleBasedBreakIterator.DONE || q != RuleBasedBreakIterator.DONE) errln("ERROR: previous()/preceding() at starting position returned #" + p + " and " + q + " instead of 0\n");
        testString = "Hello! how are you? I'am fine. Thankyou. How are you doing? This\n costs $20,00,000.";
        logln("testing sentence iter - String:- \"" + testString + "\"");
        RuleBasedBreakIterator sentIter1 = (RuleBasedBreakIterator) RuleBasedBreakIterator.getSentenceInstance(Locale.getDefault());
        sentIter1.setText(testString);
        p = sentIter1.last();
        if (p != testString.length()) errln("ERROR: last() returned" + p + "instead of " + testString.length());
    ...
    public void doTest(String testString, int start, int gotoffset, int expectedOffset, String expectedString) {
        ...
        if (gotoffset != expectedOffset) errln("ERROR:****returned #" + gotoffset + " instead of #" + expectedOffset);
        if (start <= gotoffset) {
            selected = testString.substring(start, gotoffset);
        } else {
            selected = testString.substring(gotoffset, start);
        }
        if (!selected.equals(expected)) errln("ERROR:****selected \"" + selected + "\" instead of \"" + expected + "\""); else logln("****selected \"" + selected + "\"");
    }
}
