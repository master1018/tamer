 ...
 * 
 * @author mog
 * @version $Id: UserManagerConverter.java 411 2004-02-10 00:03:32Z mog $
 */
public class UserManagerConverter {
    ...
    public static void convert(User from, User to) throws UserFileException {
        ...
        int periods[] = new int[] { Trial.PERIOD_ALL, Trial.PERIOD_DAILY, Trial.PERIOD_MONTHLY, Trial.PERIOD_WEEKLY };
        for (int i = 0; i < periods.length; i++) {
            int period = periods[i];
            to.setUploadedMillisecondsForPeriod(period, from.getUploadedMillisecondsForPeriod(period));
            to.setDownloadedMillisecondsForPeriod(period, from.getDownloadedMillisecondsForPeriod(period));
            to.setUploadedBytesForPeriod(period, from.getUploadedBytesForPeriod(period));
            to.setDownloadedBytesForPeriod(period, from.getDownloadedBytesForPeriod(period));
            to.setUploadedFilesForPeriod(period, from.getUploadedFilesForPeriod(period));
            to.setDownloadedFilesForPeriod(period, from.getDownloadedFilesForPeriod(period));
        }
        to.commit();
    }
}
