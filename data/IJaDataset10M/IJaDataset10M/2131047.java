...
public class GLUquadricImpl implements GLUquadric {
    ...
    public ImmModeSink replaceImmModeSink() {
        ...
        if (useGLSL) {
            immModeSink = ImmModeSink.createGLSL(gl, GL.GL_STATIC_DRAW, 32, 3, GL.GL_FLOAT, 0, GL.GL_FLOAT, USE_NORM ? 3 : 0, normalType, USE_TEXT ? 2 : 0, GL.GL_FLOAT);
        } else {
            immModeSink = ImmModeSink.createFixed(gl, GL.GL_STATIC_DRAW, 32, 3, GL.GL_FLOAT, 0, GL.GL_FLOAT, USE_NORM ? 3 : 0, normalType, USE_TEXT ? 2 : 0, GL.GL_FLOAT);
        }
        return res;
    }
            ...
            case GLU.GLU_FILL:
                {
                    ...
                    for (l = 0; l < loops; l++) {
                        ...
                        if (orientation == GLU.GLU_OUTSIDE) {
                            ...
                            for (s = 0; s <= slices; s++) {
                                float a;
                                if (s == slices) a = 0.0f; else a = s * da;
                                sa = sin(a);
                                ca = cos(a);
            ...
            case GLU.GLU_FILL:
                if (innerRadius == .0f) {
                    ...
                    if (orientation == GLU.GLU_OUTSIDE) {
                        for (i = slices; i >= 0; i--) {
                            if (textureFlag) {
                                glTexCoord2f(gl, texLow * sinCache[i] + 0.5f, texLow * cosCache[i] + 0.5f);
                            }
                ...
                for (j = 0; j < finish; j++) {
                    ...
                    for (i = 0; i <= slices; i++) {
                        if (orientation == GLU.GLU_OUTSIDE) {
                            ...
                            glVertex3f(gl, radiusLow * sinCache[i], radiusLow * cosCache[i], 0.0f);
                            if (textureFlag) {
                                glTexCoord2f(gl, texHigh * sinCache[i] + 0.5f, texHigh * cosCache[i] + 0.5f);
                            }
                            glVertex3f(gl, radiusHigh * sinCache[i], radiusHigh * cosCache[i], 0.0f);
                        } else {
                            if (textureFlag) {
                                glTexCoord2f(gl, texHigh * sinCache[i] + 0.5f, texHigh * cosCache[i] + 0.5f);
                            }
        ...
        if (drawStyle == GLU.GLU_FILL) {
            ...
            if (!textureFlag) {
                ...
                for (j = slices; j >= 0; j--) {
                    ...
                    y = cos(theta) * sin(rho);
                    z = nsign * cos(rho);
                    if (normals) glNormal3f(gl, x * nsign, y * nsign, z * nsign);
                    s -= ds;
                    glVertex3f(gl, x * radius, y * radius, z * radius);
                }
                glEnd(gl);
            ...
            glBegin(gl, GL.GL_POINTS);
            if (normals) glNormal3f(gl, 0.0f, 0.0f, nsign);
