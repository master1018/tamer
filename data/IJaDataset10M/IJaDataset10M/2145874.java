...
public class SeqStat {
    ...
    public SeqStat mergeWith(SeqStat minMax) {
        return new SeqStat(MathUtils.nonNaNMin(min, minMax.min), MathUtils.nonNaNMax(max, minMax.max), sum + minMax.sum, count + minMax.count);
    }
    ...
    public SeqStat mergeWith(double value) {
        if (Double.isNaN(value) || (min <= value && value <= max)) {
            ...
            return new SeqStat(MathUtils.nonNaNMin(min, value), MathUtils.nonNaNMax(max, value), sum + value, count + 1);
        }
    }
   ...
   */
    ...
    public boolean equals(Object obj) {
        ...
        if (Double.doubleToLongBits(sum) != Double.doubleToLongBits(other.sum)) return false;
        return true;
    }
}
