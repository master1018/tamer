...
public class CSDRender {
    ...
    public static synchronized CsdRenderResult generateCSDForBlueLive(BlueData data) {
        ...
        boolean usingAPI = APIUtilities.isCsoundAPIAvailable() && ProgramOptions.getGeneralSettings().isUsingCsoundAPI();
    ...
    public static synchronized CsdRenderResult generateCSD(BlueData data, float startTime, float endTime, boolean isRealTime) throws SoundObjectException {
        ParameterHelper.clearCompilationVarNames(data);
        boolean usingAPI = isRealTime && APIUtilities.isCsoundAPIAvailable() && ProgramOptions.getGeneralSettings().isUsingCsoundAPI();
        ...
        if (isRealTime && !(APIUtilities.isCsoundAPIAvailable() && ProgramOptions.getGeneralSettings().isUsingCsoundAPI())) {
    ...
    private static Instrument createBlueTimePointerInstrument() {
        GenericInstrument instr = new GenericInstrument();
        int fps = ProgramOptions.getPlaybackFPS();
        float time = 1.0f / fps;
    ...
    private static void handleParameters(ArrayList parameters, GlobalOrcSco globalOrcSco, NoteList notes, Arrangement arrangement, float startTime, float endTime, boolean isRealTime) {
        ...
        boolean useAPI = (isRealTime && APIUtilities.isCsoundAPIAvailable() && ProgramOptions.getGeneralSettings().isUsingCsoundAPI());
    ...
    private static void handleParametersForBlueLive(ArrayList parameters, GlobalOrcSco globalOrcSco, NoteList notes, Arrangement arrangement) {
        ...
        StrBuilder initStatements = new StrBuilder();
        StrBuilder paramScore = new StrBuilder();
        boolean useAPI = (APIUtilities.isCsoundAPIAvailable() && ProgramOptions.getGeneralSettings().isUsingCsoundAPI());
        StrBuilder apiParamInstr = new StrBuilder();
        for (int i = 0; i < parameters.size(); i++) {
            Parameter param = (Parameter) parameters.get(i);
            varNum[0] = new Integer(i);
