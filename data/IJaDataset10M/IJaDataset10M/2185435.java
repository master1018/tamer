...
public class StatusTest extends JDWPSyncTestCase {
    ...
    public void testStatus001() {
        ...
        CommandPacket checkedCommand = new CommandPacket(JDWPCommands.ReferenceTypeCommandSet.CommandSetID, JDWPCommands.ReferenceTypeCommandSet.StatusCommand);
        ...
        String statusBitsNames[] = { "VERIFIED", "PREPARED", "INITIALIZED", "ERROR" };
        int statusBitsValues[] = { 0x01, 0x02, 0x04, 0x08 };
        int expectedStatusBits[] = { 0x01, 0x02, 0x04, 0x00 };
        String failMessage = "";
        ...
        for (int i = 0; i < checkedStatusBitsNumber; i++) {
            ...
            if (expectedStatusBits[i] != returnedStatusBit) {
                logWriter.println("\n## FAILURE: " + thisCommandName + " returns unexpected value for status bit \"" + statusBitsNames[i] + "\"");
                logWriter.println("## Expected status bit = 0x" + Integer.toHexString(expectedStatusBits[i]));
                logWriter.println("## Returned status bit = 0x" + Integer.toHexString(returnedStatusBit));
                failMessage = failMessage + thisCommandName + " returns unexpected value for status bit \"" + statusBitsNames[i] + "\"" + ", Expected = 0x" + Integer.toHexString(expectedStatusBits[i]) + ", Returned = 0x" + Integer.toHexString(returnedStatusBit) + "; ";
            } else {
                logWriter.println("\n=> Expected value for status bit \"" + statusBitsNames[i] + "\" (0x" + Integer.toHexString(statusBitsValues[i]) + ") is returned: 0x" + Integer.toHexString(returnedStatusBit));
            }
        }
