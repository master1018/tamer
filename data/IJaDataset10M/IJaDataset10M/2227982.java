...
public class AggregateAttribute implements Comparable<AggregateAttribute> {
    ...
    AggregateAttribute(PropertyDescriptor property, Element elementInfo) {
        ...
        this.toString = String.format("%s '%s' (property '%s' of aggregate %s)", getType().toString().toLowerCase().replace('_', ' '), getName(), property.getName(), property.getReadMethod().getDeclaringClass().getName());
