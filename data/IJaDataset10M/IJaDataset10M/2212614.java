...
public class PlotCliqueSizeDistribution {
    ...
    private static final Log log = LogFactory.getLog(PlotCliqueSizeDistribution.class);
        ...
        try {
            ...
            while (line != null) {
                if (line.matches(START_CLIQUE)) {
                    if (count != 0) {
                        throw new RuntimeException("opening a new clique before " + "closing the previous one.");
                    }
    ...
    private static ChartUtil getHistogram(final List<Integer> sizes) {
        ...
        JFreeChart chart = ChartFactory.createHistogram(TITLE, XLABEL, YLABEL, dataset, PlotOrientation.VERTICAL, true, false, false);
