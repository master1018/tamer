...
public class RevertConfigurationWizardPage extends WizardPage {
    ...
    public void createControl(Composite parent) {
        ...
        if (element != null) configViewer.setSelection(new StructuredSelection(element));
        ...
        configViewer.addSelectionChangedListener(new ISelectionChangedListener() {
            ...
            public void selectionChanged(SelectionChangedEvent e) {
                ...
                for (int i = 0; i < items.length; i++) {
                    ...
                    if (((InstallConfiguration) activity.getInstallConfiguration()).equals(currentConfig)) items[i].setBackground(blueBGColor); else items[i].setBackground(activitiesViewer.getControl().getBackground());
                }
            }
        });
        try {
    ...
    private void createActivitiesSection(Composite parent) {
        ...
        if (configs.length > 0) activitiesViewer.setInput(configs[0].getData());
    ...
    public boolean performFinish() {
        ...
        boolean result = MessageDialog.openQuestion(shell, shell.getText(), UpdateUIMessages.RevertConfigurationWizardPage_question);
    ...
    public boolean performRevert() {
        ...
        IRunnableWithProgress operation = new IRunnableWithProgress() {
            ...
            public void run(IProgressMonitor monitor) throws InvocationTargetException {
                IOperation revertOperation = OperationsManager.getOperationFactory().createRevertConfigurationOperation(target, new UIProblemHandler());
                try {
