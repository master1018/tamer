...
public class MPJRun {
    ...
    final int DEFAULT_PROTOCOL_SWITCH_LIMIT = 128 * 1024;
   ...
   */
    public MPJRun(String args[]) throws Exception {
        ...
        if (deviceName.equals("multicore")) {
            ...
            MulticoreDaemon multicoreDaemon = new MulticoreDaemon(className, applicationClassPathEntry, jarOrClass, nprocs, wdir, jvmArgs, appArgs);
            return;
        } else {
            System.out.println("MPJ Express (" + VERSION + ") is started in the " + "cluster configuration");
        }
        readMachineFile();
        machinesSanityCheck();
        try {
            localaddr = InetAddress.getLocalHost();
            if (hostIP == null) hostIP = localaddr.getHostAddress();
            if (DEBUG && logger.isDebugEnabled()) {
        ...
        for (int j = 0; j < peerChannels.size(); j++) {
            ...
            String hAddress = socketChannel.socket().getInetAddress().getHostAddress();
        ...
        addShutdownHook();
        Wait();
        if (DEBUG && logger.isDebugEnabled()) logger.debug("Calling the finish method now");
        this.finish();
    }
    ...
    private void createLogger(String[] args) throws MPJRuntimeException {
        if (DEBUG && logger == null) {
            ...
            try {
                fileAppender = new DailyRollingFileAppender(new PatternLayout(" %-5p %c %x - %m\n"), mpjHomeDir + "/logs/mpjrun.log", "yyyy-MM-dd-a");
   ...
   */
    private void processInput(String args[]) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-np")) {
                ...
                deviceName = args[i + 1];
                i++;
                if (!(deviceName.equals("niodev") || deviceName.equals("mxdev") || deviceName.equals("multicore"))) {
                    System.out.println("MPJ Express currently does not support the <" + deviceName + "> device.");
                    System.out.println("Possible options are niodev, mxdev, and " + "multicore devices.");
                ...
                if (tFile.exists()) {
                    ...
                    throw new MPJRuntimeException("mpjrun cannot find the jar file <" + args[i + 1] + ">. Make sure this is the right path.");
                }
            } else {
        ...
        if (DEBUG && logger.isDebugEnabled()) {
            ...
            for (int i = 0; i < jArgs.length; i++) {
                if (DEBUG && logger.isDebugEnabled()) logger.debug(" jvmArgs[" + i + "]: <" + jArgs[i] + ">");
            }
            if (DEBUG && logger.isDebugEnabled()) logger.debug("appArgs.length: <" + aArgs.length + ">");
            for (int i = 0; i < aArgs.length; i++) {
                if (DEBUG && logger.isDebugEnabled()) logger.debug(" appArgs[" + i + "]: <" + aArgs[i] + ">");
            }
            if (DEBUG && logger.isDebugEnabled()) logger.debug("###########################");
        }
    }

    private synchronized void Wait() throws Exception {
        if (wait) {
            if (DEBUG && logger.isDebugEnabled()) logger.debug("Waiting ...");
            this.wait();
            if (DEBUG && logger.isDebugEnabled()) logger.debug("Unwaiting ...");
        }
        wait = true;
    }

    private synchronized void Notify() {
        if (DEBUG && logger.isDebugEnabled()) logger.debug("Notifying ...");
        this.notify();
        wait = false;
    }
    ...
    private void assignTasks() throws Exception {
        ...
        if (nprocs <= noOfMachines) {
            if (DEBUG && logger.isDebugEnabled()) {
                logger.debug("Processes Requested " + nprocs + " are less than than machines " + noOfMachines);
                logger.debug("Adding 1 processes to the first " + nprocs + " items");
            }
            for (int i = 0; i < nprocs; i++) {
                procsPerMachineTable.put((String) machineVector.get(i), new Integer(1));
                if (deviceName.equals("niodev")) {
                    CONF_FILE_CONTENTS += ";" + name + "@" + MPJ_SERVER_PORT + "@" + (rank++);
                } else if (deviceName.equals("mxdev")) {
                    CONF_FILE_CONTENTS += ";" + name + "@" + mxBoardNum + "@" + (rank++);
                }
                if (DEBUG && logger.isDebugEnabled()) {
            ...
            if (DEBUG && logger.isDebugEnabled()) {
                logger.debug("Processes Requested " + nprocs + " are greater than than machines " + noOfMachines);
            }
            ...
            for (int i = 0; i < noOfMachines; i++) {
                if (i < remainder) {
                    procsPerMachineTable.put((String) machineVector.get(i), new Integer(divisor + 1));
                    if (DEBUG && logger.isDebugEnabled()) {
                        logger.debug("procPerMachineTable==>" + procsPerMachineTable);
                    }
                    for (int j = 0; j < (divisor + 1); j++) {
                        if (deviceName.equals("niodev")) {
                            CONF_FILE_CONTENTS += ";" + (String) machineVector.get(i) + "@" + (MPJ_SERVER_PORT + (j * 2)) + "@" + (rank++);
                        } else if (deviceName.equals("mxdev")) {
                            CONF_FILE_CONTENTS += ";" + (String) machineVector.get(i) + "@" + (mxBoardNum + j) + "@" + (rank++);
                        }
                    }
                } else if (divisor > 0) {
                    procsPerMachineTable.put((String) machineVector.get(i), new Integer(divisor));
                    if (DEBUG && logger.isDebugEnabled()) {
                        logger.debug("procPerMachineTable==>" + procsPerMachineTable);
                    }
    ...
    public void readMachineFile() throws Exception {
        ...
        try {
            ...
            throw new MPJRuntimeException("<" + machinesFile + "> file cannot " + " be found." + " The starter module assumes " + "it to be in the current directory.");
        }
        boolean loop = true;
        String line = null;
        int machineCount = 0;
        while (machineCount < nprocs) {
            ...
            if (line.startsWith("#") || line.equals("") || (machineVector.size() == nprocs)) {
                continue;
            }
            machineCount++;
            line = line.trim();
            ...
            if (!alreadyPresent) {
                ...
                if (DEBUG && logger.isDebugEnabled()) {
                    logger.debug("Line " + line.trim() + " added to vector " + machineVector);
                }
            }
        }
    }
    ...
    private void clientSocketInit() throws Exception {
        ...
        for (int i = 0; i < machineVector.size(); i++) {
            ...
            try {
                ...
                connected = clientChannels[i].connect(new InetSocketAddress(daemon, D_SER_PORT));
                if (!connected) {
                    throw new MPJRuntimeException("Cannot connect to the daemon " + "at machine <" + daemon + "> and port <" + D_SER_PORT + ">." + "Please make sure that the machine is reachable " + "and running the daemon in 'sane' state");
                }
   ...
   */
    ...
    private void doConnect(SocketChannel peerChannel) {
        if (DEBUG && logger.isDebugEnabled()) logger.debug("---doConnect---");
        try {
            if (DEBUG && logger.isDebugEnabled()) logger.debug("Configuring it to be non-blocking");
            peerChannel.configureBlocking(false);
        } catch (IOException ioe) {
            if (DEBUG && logger.isDebugEnabled()) logger.debug("Closed Channel Exception in doConnect");
            System.exit(0);
        }
        try {
            if (DEBUG && logger.isDebugEnabled()) logger.debug("Registering for OP_READ & OP_WRITE event");
            peerChannel.register(selector, SelectionKey.OP_READ | SelectionKey.OP_WRITE);
   ...
   */
    ...
    Runnable selectorThread = new Runnable() {
        ...
        public void run() {
            if (DEBUG && logger.isDebugEnabled()) logger.debug("selector Thread started ");
            Set readyKeys = null;
            ...
            try {
                while (selector.select() > -1 && selectorFlag == true) {
                    ...
                    while (readyItor.hasNext()) {
                        ...
                        if (DEBUG && logger.isDebugEnabled()) logger.debug("\n---selector EVENT---");
                        if (key.isAcceptable()) {
                            if (DEBUG && logger.isDebugEnabled()) logger.debug("ACCEPT_EVENT");
                        } else if (key.isConnectable()) {
                            if (DEBUG && logger.isDebugEnabled()) logger.debug("CONNECT_EVENT");
                            try {
                            ...
                            if (read == -1) {
                                if (DEBUG && logger.isDebugEnabled()) logger.debug("END_OF_STREAM signal at starter from " + "channel " + socketChannel);
                                streamEndedCount++;
                                if (streamEndedCount == machineVector.size()) {
                                    if (DEBUG && logger.isDebugEnabled()) {
                                        logger.debug("The starter has received " + machineVector.size() + "signals");
                            ...
                            if (line.endsWith("EXIT")) {
                                ...
                                if (endCount == machineVector.size()) {
                                    if (DEBUG && logger.isDebugEnabled()) logger.debug("Notify and exit");
                                    Notify();
                                }
                            } else {
                            ...
                            if (DEBUG && logger.isDebugEnabled()) logger.debug("In, WRITABLE, so changing the interestOps to READ_ONLY");
                            key.interestOps(SelectionKey.OP_READ);
                        }
                    }
                }
            } catch (Exception ioe1) {
                if (DEBUG && logger.isDebugEnabled()) logger.debug("Exception in selector thread ");
                ioe1.printStackTrace();
                System.exit(0);
            }
            if (DEBUG && logger.isDebugEnabled()) logger.debug("Thread getting out");
        }
    };
}
