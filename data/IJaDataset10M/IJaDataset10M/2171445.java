package org.chemicalcovers.ui.component;
...
import org.chemicalcovers.model.Cover;
import org.chemicalcovers.model.CoverListModel;

public class CDCoverFlow extends JPanel {
    ...
    private CoverListModel coverListModel;

    public CDCoverFlow(CoverListModel theModel) {
        coverListModel = theModel;
    ...
    public Dimension getPreferredSize() {
        return new Dimension(displayWidth * 5, (int) (displayHeight * 1.5));
    }
    ...
    private void drawAvatars(Graphics2D g2, DrawableAvatar[] drawableAvatars) {
        for (DrawableAvatar avatar : drawableAvatars) {
            ...
            g2.drawImage(avatars.get(avatar.getIndex()), (int) avatar.getX(), (int) avatar.getY(), avatar.getWidth(), avatar.getHeight(), null);
        }
    }
    ...
    private void drawAvatarName(Graphics2D g2) {
        ...
        g2text.dispose();
        g2.drawImage(fx.createReflectedPicture(textImage, fx.createGradientMask((int) bulletWidth, (int) bulletHeight)), (int) x, (int) y, null);
        g2.setComposite(composite);
    }
    ...
    private void promoteAvatarToDrawable(List<DrawableAvatar> drawables, int x, int y, int width, int height, int offset) {
        ...
        drawables.add(new DrawableAvatar(avatarIndex + offset, avatar_x, avatar_y, newWidth, newHeight, avatarPosition, result));
    }
    ...
    public void loadAvatars() {
        ...
        int i = 0;
        if (coverListModel != null) {
            for (int j = 0; j < coverListModel.getSize(); j++) {
                Cover cover = coverListModel.getElementAt(j);
                BufferedImage image = (BufferedImage) cover.getSmallImage().getImage();
                avatars.add(fx.createReflectedPicture(fx.createCrystalCase(image)));
                avatarsText.add(cover.getCoverInfoResult().getArtist() + " " + cover.getCoverInfoResult().getAlbum());
                if (i++ == avatarAmount) {
    ...
    private DrawableAvatar getHitAvatar(int x, int y) {
        for (DrawableAvatar avatar : drawableAvatars) {
            Rectangle hit = new Rectangle((int) avatar.getX(), (int) avatar.getY(), avatar.getWidth(), avatar.getHeight() / 2);
            if (hit.contains(x, y)) {
                return avatar;
            }
        }
    ...
    private class DrawableAvatar implements Comparable<Object> {
