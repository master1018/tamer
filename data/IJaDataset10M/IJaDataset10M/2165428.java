...
public class HMMSet {
        ...
        Iterator<SingleHMM> it = new Iterator<SingleHMM>() {
            ...
            public SingleHMM next() {
                for (; ; ) {
                    ...
                    if (hmm.getName().indexOf('-') >= 0 || hmm.getName().indexOf('+') >= 0) continue;
                    return hmm;
                }
            }
    ...
    public int getNhmmsMono() {
        ...
        for (SingleHMM hmm : hmms) {
            if (!(hmm.getName().indexOf('-') >= 0 || hmm.getName().indexOf('+') >= 0)) n++;
        }
        return n;
    }
    ...
    public int getStateIdx(int hmmidx, int stateidx) {
        ...
        if (hmm.isEmitting(stateidx)) return nEmittingStates; else return -1;
    }
    ...
    private int loadTrans(BufferedReader f, String nomEtat, String prem) throws IOException {
        String s;
        int nstates = 0;
        if (prem != null) s = prem; else s = f.readLine().trim();
        if (s.startsWith("<TRANSP>")) {
