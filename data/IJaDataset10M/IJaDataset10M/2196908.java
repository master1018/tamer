 ...
 */
    public Parameter(Long id, Report report, String typeName, String name, String question, String arityString) {
        ...
        if (typeName == null || typeName.length() == 0) {
            String str = I18N.get("Parameter.param_cap") + " " + id + ": " + I18N.get("Parameter.missing_type");
            throw new IllegalArgumentException(str);
        }
        typeName = typeName.toLowerCase().trim();
        if ("boolean".equals(typeName)) type = TYPE_BOOLEAN; else if ("string".equals(typeName)) type = TYPE_STRING; else if ("numeric".equals(typeName)) type = TYPE_NUMERIC; else if ("date".equals(typeName)) type = TYPE_DATE; else {
            String str = I18N.get("Parameter.param_cap") + " " + id + ": " + I18N.get("Parameter.illegal_type");
            throw new IllegalArgumentException(str);
        }
        this.name = name;
        this.question = question;
        if (arityString == null || arityString.length() == 0) {
            String str = I18N.get("Parameter.param_cap") + id + ": " + I18N.get("Parameter.missing_arity");
            throw new IllegalArgumentException(str);
        }
        arityString = arityString.toLowerCase().trim();
        if ("single".equals(arityString)) arity = ARITY_ONE; else if ("range".equals(arityString)) arity = ARITY_RANGE; else if ("list-single".equals(arityString)) arity = ARITY_LIST_SINGLE; else if ("list-multiple".equals(arityString)) arity = ARITY_LIST_MULTIPLE; else {
            String str = I18N.get("Parameter.param_cap") + id + ": " + I18N.get("Parameter.illegal_arity");
            throw new IllegalArgumentException(str);
        }
        initialize(id);
    }
 ...
 */
    ...
    private void initialize(Long id) {
        if (id == null) id = report.generateNewParameterId();
        this.id = id;
        switch(type) {
            case TYPE_BOOLEAN:
                if (arity != ARITY_ONE) {
                    String str = I18N.get("Parameter.param_cap") + id + ": " + I18N.get("Parameter.yesno_single");
                    throw new IllegalArgumentException(str);
                }
                break;
            case TYPE_DATE:
                if (arity != ARITY_ONE && arity != ARITY_RANGE) {
                    String str = I18N.get("Parameter.param_cap") + id + ": " + I18N.get("Parameter.date_arity_err");
                    throw new IllegalArgumentException(str);
                }
                break;
        }
    ...
    public Object clone() {
        ...
        for (Iterator iter = defaultValues.iterator(); iter.hasNext(); ) {
            ...
            if (obj instanceof Boolean) p.defaultValues.add(obj); else if (obj instanceof String) p.defaultValues.add(new String((String) obj)); else if (obj instanceof Number) p.defaultValues.add(obj); else if (obj instanceof Date) p.defaultValues.add(((Date) obj).clone());
        }
        return p;
    }
 ...
 */
    public void setQuestion(String newQuestion) {
        if (question != newQuestion && (question == null || !question.equals(newQuestion))) {
    ...
    public void setType(int newType) {
        if (type != newType) {
            ...
            if (type == TYPE_BOOLEAN) {
                if (arity != ARITY_ONE) arity = ARITY_ONE;
            } else if (type == TYPE_DATE) {
                if (arity == ARITY_LIST_SINGLE || arity == ARITY_LIST_MULTIPLE) arity = ARITY_ONE;
            }
            setChanged();
            notifyObservers();
        }
    ...
    public void setArity(int newArity) {
        if (arity != newArity) {
            if (type == TYPE_BOOLEAN) {
                ...
                if (newArity == ARITY_LIST_SINGLE || newArity == ARITY_LIST_MULTIPLE) {
                    String str = I18N.get("Parameter.param_cap") + id + ": " + I18N.get("Parameter.date_arity_err");
                    throw new IllegalArgumentException(str);
                }
            }
 ...
 */
    public Object getDefaultValue(int i) {
        Object val;
        if (i < 0 || i >= defaultValues.size() || (val = defaultValues.get(i)) == null) {
 ...
 */
    public void setDefaultValue(int i, Object newDefaultValue) {
        ...
        if (i < defaultValues.size()) defaultValue = getDefaultValue(i);
        if (defaultValue != newDefaultValue && (defaultValue == null || !defaultValue.equals(newDefaultValue))) {
 ...
 */
    public Object getValue(int i) {
        Object val = null;
        if (i < values.size()) val = values.get(i);
        if (val == null) {
            if (i < defaultValues.size()) val = defaultValues.get(i);
        }
        return val;
    }
 ...
 */
    protected Object convertType(Object val) {
        ...
        switch(type) {
            case TYPE_BOOLEAN:
                if (val instanceof Boolean) return val; else if (val instanceof String) {
                    val = ((String) val).toLowerCase().trim();
                    if ("true".equals(val) || "t".equals(val) || "yes".equals(val) || "y".equals(val)) return Boolean.valueOf(true); else return Boolean.valueOf(false);
            ...
            case TYPE_STRING:
                return val.toString();
            case TYPE_NUMERIC:
                if (val instanceof Number) return val; else {
                    String str = val.toString();
                    if (str.length() == 0) return new Integer(0); else if (str.indexOf(".") == -1) return new Integer(str); else return new Double(str);
                }
            case TYPE_DATE:
                if (val instanceof Date) return val; else {
                    String str = val.toString();
                    if (str.length() == 0) return new Date(); else {
    ...
    public void writeXML(XMLWriter out) {
        ...
        switch(arity) {
            ...
            case ARITY_RANGE:
                ...
                break;
            case ARITY_LIST_SINGLE:
                arityString = "list-single";
                break;
            case ARITY_LIST_MULTIPLE:
                arityString = "list-multiple";
                break;
        }
        ...
        for (Iterator iter = defaultValues.iterator(); iter.hasNext(); ) out.textElement("default", iter.next().toString());
        out.endElement();
    }
