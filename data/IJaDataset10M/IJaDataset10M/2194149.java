...
public class MethodInfo {
    ...
    public boolean addFormal(VarInfo formal) {
        if (formalsNames.contains(formal.name) || localsNames.contains(formal.name)) return false;
        List<VarInfo> nn = new List<VarInfo>(formal, null);
        if (formals == null) formals = formalsTail = nn; else formalsTail = formalsTail.tail = nn;
    ...
    public boolean addLocal(VarInfo local) {
        if (formalsNames.contains(local.name) || localsNames.contains(local.name)) return false;
        List<VarInfo> nn = new List<VarInfo>(local, null);
        if (locals == null) locals = localsTail = nn; else localsTail = localsTail.tail = nn;
