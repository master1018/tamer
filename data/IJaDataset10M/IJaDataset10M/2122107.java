...
public class FormAction extends MultiAction implements InitializingBean {
    ...
    protected void initAction() {
        if (getValidator() != null) {
            ...
            validateMethodInvoker = new DispatchMethodInvoker(getValidator(), new Class[] { getFormObjectClass(), Errors.class });
        }
    }
    ...
    private void putFormObject(RequestContext context, Object formObject) {
        if (logger.isDebugEnabled()) {
            logger.debug("Putting form object of type [" + formObject.getClass() + "] in scope " + getFormObjectScope() + " with name '" + getFormObjectName() + "'");
        }
        ...
        if (!formErrorsExposed(context)) {
            ...
            if (formErrorsValid(context, formObject)) {
                ...
                if (logger.isInfoEnabled()) {
                    logger.info("Fixing inconsistent Errors instance: initializing a new Errors instance " + "wrapping from object '" + formObject + "' in scope '" + getFormErrorsScope() + "' and copying over all existing error information.");
                }
                Errors invalidExistingErrors = getFormObjectAccessor(context).getFormErrors(getFormObjectName(), getFormErrorsScope());
    ...
    private boolean formErrorsValid(RequestContext context, Object formObject) {
        ...
        if (errors instanceof BindingResult) {
            ...
            if (be.getTarget() != formObject) {
                if (logger.isInfoEnabled()) {
                    logger.info("Inconsistency detected: the Errors instance in '" + getFormErrorsScope() + "' does NOT wrap the current form object '" + formObject + "' of class " + formObject.getClass() + "; instead this Errors instance unexpectedly wraps the target object '" + be.getTarget() + "' of class: " + be.getTarget().getClass() + ".");
                }
                return false;
            } else {
                return true;
    ...
    private void reinstallPropertyEditors(RequestContext context) {
        BindingResult errors = (BindingResult) getFormObjectAccessor(context).getFormErrors(getFormObjectName(), getFormErrorsScope());
    ...
    protected Object getFormObject(RequestContext context) throws Exception {
        ...
        if (formObject == null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Found existing form object with name '" + getFormObjectName() + "' of type [" + formObject.getClass() + "] in scope " + getFormObjectScope());
            }
    ...
    protected void doBind(RequestContext context, DataBinder binder) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("Binding allowed request parameters in " + StylerUtils.style(context.getExternalContext().getRequestParameterMap()) + " to form object with name '" + binder.getObjectName() + "', pre-bind formObject toString = " + binder.getTarget());
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Binding completed for form object with name '" + binder.getObjectName() + "', post-bind formObject toString = " + binder.getTarget());
            logger.debug("There are [" + binder.getBindingResult().getErrorCount() + "] errors, details: " + binder.getBindingResult().getAllErrors());
        }
    }
    ...
    public String toString() {
        return new ToStringCreator(this).append("formObjectName", formObjectName).append("formObjectClass", formObjectClass).append("formObjectScope", formObjectScope).toString();
    }
}
