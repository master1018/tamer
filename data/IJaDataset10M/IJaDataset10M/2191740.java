...
public class ByteSourceInputStream extends ByteSource {
    ...
    private CacheBlock readBlock() throws IOException {
        if (null == readBuffer) readBuffer = new byte[BLOCK_SIZE];
        int read = is.read(readBuffer);
        if (read < 1) return null; else if (read < BLOCK_SIZE) {
    ...
    private CacheBlock getFirstBlock() throws IOException {
        if (null == cacheHead) cacheHead = readBlock();
        return cacheHead;
    }
    ...
    private class CacheReadingInputStream extends InputStream {
        ...
        public int read() throws IOException {
            ...
            if (blockIndex >= block.bytes.length) return -1;
            return 0xff & block.bytes[blockIndex++];
        }

        public int read(byte b[], int off, int len) throws IOException {
            if (b == null) throw new NullPointerException(); else if ((off < 0) || (off > b.length) || (len < 0) || ((off + len) > b.length) || ((off + len) < 0)) throw new IndexOutOfBoundsException(); else if (len == 0) return 0;
            if (null == block) {
                if (readFirst) return -1;
                block = getFirstBlock();
                readFirst = true;
    ...
    public byte[] getBlock(int blockStart, int blockLength) throws IOException {
        if ((blockStart < 0) || (blockLength < 0) || (blockStart + blockLength < 0) || (blockStart + blockLength > streamLength.longValue())) {
            throw new IOException("Could not read block (block start: " + blockStart + ", block length: " + blockLength + ", data length: " + streamLength + ").");
        }
        ...
        while (true) {
            ...
            if (read < 1) throw new IOException("Could not read block.");
            total += read;
            if (total >= blockLength) return bytes;
        }
    }
    ...
    public long getLength() throws IOException {
        if (streamLength != null) return streamLength.longValue();
        InputStream is = getInputStream();
        long result = 0;
        long skipped;
        while ((skipped = is.skip(1024)) > 0) result += skipped;
