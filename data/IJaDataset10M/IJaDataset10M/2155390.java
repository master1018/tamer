...
public class CustomSubCatalog extends BooksSubCatalog {
    ...
    private static final Logger logger = Logger.getLogger(CustomSubCatalog.class);
    ...
    public CustomSubCatalog(List<Book> books, String title) {
        super(books);
        this.title = title;
        setStuffToFilterOut(new Vector<Object>() {
    ...
    public Composite<Element, String> getSubCatalogEntry(Breadcrumbs pBreadcrumbs) throws IOException {
        ...
        if (logger.isTraceEnabled()) logger.trace("getSubCatalogEntry  Breadcrumbs=" + pBreadcrumbs.toString());
        String summary = Localization.Main.getText("deeplevel.summary", Summarizer.INSTANCE.getBookWord(getBooks().size()));
        if (logger.isDebugEnabled()) logger.debug("making a deep level");
        boolean weAreAlsoInSubFolder = pBreadcrumbs.size() > 1;
        Element entry = getSubCatalogLevel(pBreadcrumbs, getBooks(), getStuffToFilterOut(), title, summary, urn, filename, null, useExternalIcons ? (weAreAlsoInSubFolder ? "../" : "./") + Icons.ICONFILE_CUSTOM : Icons.ICON_CUSTOM);
