...
public class MoveScreen implements IWorkbenchWindowActionDelegate {
    ...
    void runRefactoring() {
        ...
        for (IFile iFile : values) {
            ...
            try {
                ...
                if (!refactor.ft.get(iFile).equals(iFile)) iFile.delete(true, null); else {
                    System.out.println("不删除文件：" + iFile.getProjectRelativePath());
                }
            } catch (CoreException e) {
                e.printStackTrace();
            }
    ...
    private static void genFile(IFile newFile, IFile oldFile) throws CoreException, ApplicationException {
        CompositeMap cm = CacheManager.getCompositeMap(oldFile);
        String xml_decl = "<?xml version=\"1.0\" encoding=\"" + "UTF-8" + "\"?>\n";
        InputStream is = new ByteArrayInputStream((xml_decl + cm.toXML()).getBytes());
        System.out.println("生成文件：" + newFile.getProjectRelativePath());
        CreateFileOperation op = new CreateFileOperation(newFile, null, is, "Create New File");
        try {
            PlatformUI.getWorkbench().getOperationSupport().getOperationHistory().execute(op, null, WorkspaceUndoUtil.getUIInfoAdapter(new Shell()));
    ...
    private void readconfig() {
        ...
        try {
            resourceAsStream = MoveScreen.class.getResourceAsStream("FunctionService.txt");
