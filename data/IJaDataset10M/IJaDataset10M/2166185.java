...
public class DatasetTestClustNum {

    public static void main(String[] args) throws Exception {
        new DatasetTestClustNum();
    }

    public DatasetTestClustNum() throws Exception {
        NumberFormat nf = NumberFormat.getInstance(Locale.US);
        nf.setMaximumFractionDigits(12);
        System.out.println("dm = euclidean");
        for (int i = 2; i <= 20; i++) {
            Dataset data = DatasetLoader.loadDataset(new File("syntheticdatasets/clusternumber" + i + ".data"));
            XMeans clustering = new XMeans(5, 2, 24, new EuclideanDistance());
            long time = System.currentTimeMillis();
            Dataset[] clusters = clustering.executeClustering(data);
            long timediff = System.currentTimeMillis() - time;
            System.out.print("clusters " + i + "\t time " + timediff + "\t clustersizes \t");
            ...
            System.out.println();
            Export e = new Export();
            e.export(clusters, "results_xmeans/" + XMeans.class.getName().substring(XMeans.class.getName().lastIndexOf('.') + 1) + "clusternumber" + i);
        }
    }
}
