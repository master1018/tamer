...
public abstract class DiameterTestsSbb implements javax.slee.Sbb {
    ...
    public void onAccountingRequest(net.java.slee.resource.diameter.base.events.AccountingRequest acr, ActivityContextInterface aci) {
        ...
        try {
            if (actAsProxy) {
                ...
                if (!hasDestinationHost) avps.add(avpFactory.createAvp(Avp.DESTINATION_HOST, "127.0.0.1".getBytes()));
                if (!hasDestinationRealm) avps.add(avpFactory.createAvp(Avp.DESTINATION_REALM, "mobicents.org".getBytes()));
    ...
    private static final int SUBSCRIPTION_ID_TYPE = 555;

    private static final int SUBSCRIPTION_ID_DATA = 554;

    private static final int SUBSCRIPTION_ID = 553;

    private static final int UNIT_TYPE = 611;

    private static final int VALUE_DIGITS = 617;

    private static final int UNIT_VALUE = 612;

    private static final int EXPONENT = 616;

    private static final int REQUESTED_SERVICE_UNIT = 606;

    private static final int REQUESTED_ACTION = 615;

    private static final int SERVICE_PARAMETER_TYPE = 608;

    private static final int SERVICE_PARAMETER_VALUE = 609;

    private static final int SERVICE_PARAMETER_INFO = 607;
