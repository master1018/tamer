...
public abstract class Util {
    ...
    */
    ...
    public static String concatStrings(String[] strings, String spacer) {
        ...
        int len = 0;
        for (int I = 0; I < strings.length; I++) len += strings[I].length();
        if (strings.length > 1) len += spacer.length() * strings.length;
    ...
    */
    ...
    public static boolean verifyString(String src, String key) {
        ...
        for (int I = 0; I < src.length(); I++) if (key.indexOf(src.charAt(I)) == -1) return false;
        return true;
    }
    ...
    */
    public static String format(String text, int width, boolean cleanWhite) {
        if (text == null || text.length() == 0) return "";
        String o = "", b = "";
        if (cleanWhite) text = cleanWhiteSpace(text);
        text = "  " + text;
        while (text.length() > 0) {
    ...
    public static String[] commandSplit(String command) {
        ...
        int L = 0;
        if (command.charAt(0) == '\"') L = command.indexOf("\"", 1); else if (command.charAt(0) == '(') L = command.indexOf(")", 1); else return split(command, " ");
    ...
    */
    ...
    public static String fixLength(String text, int desiredLen, char filler) {
        if (text == null) text = "";
        int i = text.length();
        if (i == desiredLen) return text; else if (i > desiredLen) return text.substring(0, desiredLen); else {
            byte[] ret = new byte[desiredLen];
            System.arraycopy(text.getBytes(), 0, ret, 0, i);
            text = null;
            for (int I = i; I < desiredLen; I++) ret[I] = (byte) filler;
            return new String(ret);
        }
    }
    ...
    public static String fixLength(String text, int desiredLen, char filler, boolean addLeft) {
        if (addLeft) {
            int i = text.length();
            if (i == desiredLen) return text; else if (i > desiredLen) return text.substring(i - desiredLen); else {
                byte[] ret = new byte[desiredLen];
                int l = desiredLen - i;
                System.arraycopy(text.getBytes(), 0, ret, l, i);
                text = null;
                for (int I = 0; I < l; I++) ret[I] = (byte) filler;
                return new String(ret);
            }
        } else return fixLength(text, desiredLen, filler);
    }

    public static String cleanWhiteSpace(String text) {
        byte space = (byte) ' ', tab = (byte) '\t', cr = (byte) '\r', lf = (byte) '\n';
        byte[] t = text.getBytes();
        int J = 0;
        for (int I = 0; I < t.length; I++) {
            if (t[I] == tab || t[I] == cr || t[I] == lf || t[I] == space) {
                if ((J > 0) && (t[J - 1] != space)) t[J++] = space;
            } else if (J < I) t[J++] = t[I];
        }
        byte[] r = new byte[J];
