...
public class ObjectAttributeListParser {
    ...
    public static ContextImplementation<FCAElement, FCAElement> importOALFile(final File file) throws FileNotFoundException, DataFormatException {
    ...
    public static ContextImplementation<FCAElement, FCAElement> importOALFromReader(final Reader reader, final String name) throws DataFormatException {
        final BufferedReader in = new BufferedReader(reader);
        try {
            final ContextImplementation<FCAElement, FCAElement> context = new ContextImplementation<FCAElement, FCAElement>(name);
            final Collection<FCAElement> objects = context.getObjects();
            final Collection<FCAElement> attributes = context.getAttributes();
            final BinaryRelationImplementation<FCAElement, FCAElement> relation = context.getRelationImplementation();
            ...
            while (curLine != null) {
                ...
                if (objectText.length() != 0) {
                    final FCAElement object = new FCAElementImplementation(objectText);
                    objects.add(object);
                    final String rest = curLine.substring(objectText.length() + 1);
                    tokenizer = new StringTokenizer(rest, ';', '"', '\\');
                    while (tokenizer.hasNext()) {
                        final FCAElement attribute = new FCAElementImplementation(tokenizer.next());
