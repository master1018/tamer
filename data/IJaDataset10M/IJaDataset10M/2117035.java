...
import org.gdbms.parser.SimpleNode;
...
public class ProductExprAdapter extends AbstractExpression implements Expression {
            ...
            if (expr.length == 2) {
                try {
                    if (getOperator(this.getEntity()) == PRODUCTO) {
                        ret = ret.producto(((Expression) expr[1]).evaluateExpression(row));
                    } else if (getOperator(this.getEntity()) == DIVISION) {
                        ret = ret.producto(((Expression) expr[1]).evaluateExpression(row).inversa());
                    }
    ...
    /**
     * @see org.gdbms.engine.instruction.Expression#isAggregated()
     */
