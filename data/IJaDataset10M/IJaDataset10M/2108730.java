...
public class KChoiceGroup extends KItem {
    ...
    protected int getPrefContentHeight(int w) {
        lineHeight = Math.max(getContentStyle(getState()).getFont().getHeight(), KDisplay.SYMBOL_HEIGHT);
    ...
    public void paint(Graphics g, int contentW, int contentH) {
        ...
        if (type == POPUP) {
            KDisplay.drawIcon(g, KDisplay.SYMBOLS, 0, (lineHeight - KDisplay.SYMBOL_HEIGHT) / 2, state == STATE_FOCUSSED ? KDisplay.ICON_LEFT_RIGHT_ON : KDisplay.ICON_LEFT_RIGHT_OFF, 1, KDisplay.SYMBOL_COUNT);
            s.drawString(g, options[selectedIndex], KDisplay.BORDER + KDisplay.SYMBOL_WIDTH, (lineHeight - textHeight) / 2);
        } else {
            int currentY = 0;
            for (int i = 0; i < options.length; i++) {
                if (state == STATE_FOCUSSED) {
                    ...
                    } else g.setColor(getContentStyle(STATE_UNFOCUSSED).color);
                }
                boolean on = (type == EXCLUSIVE && i == selectedIndex) || (type == IMPLICIT && i == focusIndex) || (type == MULTIPLE && selected[i]);
                KDisplay.drawIcon(g, KDisplay.SYMBOLS, KDisplay.BORDER, currentY + (lineHeight - KDisplay.SYMBOL_HEIGHT) / 2, (type - 1) * 2 + (on ? 1 : 0), 1, KDisplay.SYMBOL_COUNT);
                s.drawString(g, options[i], (KDisplay.BORDER * 2) + KDisplay.SYMBOL_WIDTH, currentY + (lineHeight - textHeight) / 2);
                currentY += lineHeight;
            }
        }
    }
    ...
    public boolean traverse(int dir, int w, int h, int[] inout) {
        ...
        } else if (type == POPUP || (dir == Canvas.UP && focusIndex == 0) || dir == Canvas.DOWN && focusIndex == options.length - 1) {
            return false;
        } else {
            if (ownerForm.isInternalTraversal()) keyPressed(dir);
        }
        if (type != POPUP) {
            int y0 = inout[1];
            int vh = inout[3];
