...
public class YamahaFS1RBankDriver extends BankDriver {
    ...
    private static YamahaFS1RBankDriver mInstance;
    ...
    public String getPatchName(Patch ip) {
        try {
            StringBuffer s = new StringBuffer(new String(((Patch) ip).sysex, patchNameStart, patchNameSize, "US-ASCII"));
            return s.toString();
        } catch (UnsupportedEncodingException ex) {
            return "-";
        }
    ...
    public String getPatchName(Patch ip, int patchNum) {
        ...
        if (patchNum > 127) return YamahaFS1RVoiceDriver.getInstance().getPatchName(p, oPatchStart); else return YamahaFS1RPerformanceDriver.getInstance().getPatchName(p, oPatchStart);
    }
    ...
    public void setPatchName(Patch ip, int patchNum, String name) {
        Patch p = (Patch) ip;
        int oPatchStart = getPatchStart(patchNum);
        if (patchNum > 127) YamahaFS1RVoiceDriver.getInstance().setPatchName(p, name, oPatchStart); else YamahaFS1RPerformanceDriver.getInstance().setPatchName(p, name, oPatchStart);
    }
