...
public class MonitorService extends ExecutingService {
    ...
    public ResponseMessage process(JoinGroup joinGroup) {
        monitor.notify(new JoinGroupEvent(joinGroup.getNodeName(), joinGroup.getMasterName(), joinGroup.getJoinTime()));
        return null;
    }
    ...
    public ResponseMessage process(LeaveGroup leaveGroup) {
        monitor.notify(new LeaveGroupEvent(leaveGroup.getNodeName(), leaveGroup.getMasterName(), leaveGroup.getLeaveReason(), leaveGroup.getJoinTime(), leaveGroup.getLeaveTime()));
        return null;
    }
