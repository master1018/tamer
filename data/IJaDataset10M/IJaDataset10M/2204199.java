...
public class ShutdownDashboardPanel extends IzPanel {
    ...
    private ShutdownDashboardPanel(InstallerFrame parent, InstallData idata, LayoutManager2 layout) {
        super(parent, idata, layout);
        String[] text = getStringList("ShutdownDashboardPanel.info");
        for (int i = 0; i < text.length; i++) add(new JLabel(text[i]), NEXT_LINE);
    ...
    private void showErrorDialog() {
        JOptionPane.showMessageDialog(this, getStringList("ShutdownDashboardPanel.error.info"), parent.langpack.getString("ShutdownDashboardPanel.error.title"), JOptionPane.ERROR_MESSAGE);
        needToRestart = true;
    }
    ...
    private synchronized void startScan() {
        ...
        for (int i = 0; i < COMMON_PORTS.length; i++) for (int j = 0; j < SCAN_WIDTH; j++) {
    ...
    private synchronized void searcherFinished(boolean foundDash) {
        ...
        numSearchers--;
        if (foundDash || numSearchers == 0) notify();
    }
