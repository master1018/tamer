...
public class ATreeStatement extends ACompositeStatement implements ITreeStatement {
    ...
    public <E> E getAdapter(Class<E> adapter) {
        if (adapter == ITreeStatement.class) return adapter.cast(this);
        if (adapter == IRule.class) return adapter.cast(getEnclosingRule());
        if (adapter == IGrammar.class) return adapter.cast(getEnclosingRule().getParent());
        return null;
    }
    ...
    public int sourceStart() {
        if (size() > 0) return get(0).sourceStart();
        return 0;
    }

    public int sourceEnd() {
        if (size() > 0) return get(size() - 1).sourceEnd();
        return 0;
    }
