...
public final class BattleMap extends HexMap {
    ...
    public BattleMap(Client client, MasterHex masterHex, Legion attacker, Legion defender, ClientGUI gui) {
        super(masterHex);
        this.client = client;
        this.gui = gui;
        attackerMarker = new Marker(attacker, 3 * Scale.get(), attacker.getLongMarkerId(), false, client);
        defenderMarker = new Marker(defender, 3 * Scale.get(), defender.getLongMarkerId(), true, client);
        ...
        String instanceId = client.getOwningPlayer().getName() + ": " + attacker.getMarkerId() + "/" + defender.getMarkerId() + " (" + count + ")";
        count++;
    ...
    public void paintComponent(Graphics g) {
        ...
        try {
            ...
            if (attackerMarker.getLocation().x > 0) {
                attackerMarker.paintComponent(g);
            }
            if (defenderMarker.getLocation().x > 0) {
