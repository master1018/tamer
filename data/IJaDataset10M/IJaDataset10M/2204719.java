    ...
    public IndianSettlementPanel(FreeColClient freeColClient, GUI gui, IndianSettlement settlement) {
        ...
        if (visited) {
            if (skillType == null) {
                ...
                add(new JLabel(Messages.message(skillType.getNameKey()), gui.getImageIcon(skillType, true), JLabel.CENTER));
            }
        } else {
        ...
        if (!visited) {
            ...
            add(new JLabel(Messages.message(wantedGoods[0].getNameKey()) + ((sale == null) ? "" : " " + sale), gui.getImageIcon(wantedGoods[0], false), JLabel.CENTER));
            n--;
        }
        ...
        if (!visited) {
            ...
            add(new JLabel(Messages.message(wantedGoods[1].getNameKey()) + ((sale == null) ? "" : " " + sale), gui.getImageIcon(wantedGoods[1], false), JLabel.CENTER), "split " + Integer.toString(n));
            for (int i = 2; i < wantedGoods.length; i++) {
                if (wantedGoods[i] != null) {
                    sale = player.getLastSaleString(settlement, wantedGoods[i]);
                    add(new JLabel(Messages.message(wantedGoods[i].getNameKey()) + ((sale == null) ? "" : " " + sale), gui.getImageIcon(wantedGoods[i], false), JLabel.CENTER));
                }
            }
        }
