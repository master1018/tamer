...
class TestSuggestionSource extends AbstractSuggestionSource {
    ...
    private TestSuggestionSource(ComponentName component, long delay, Map<String, List<SuggestionData>> cannedResponses, HashSet<String> errorQueries, boolean queryAfterZeroResults, String label) {
    ...
    protected SuggestionResult getSuggestions(String query, int maxResults, int queryLimit) {
        ...
        return result != null ? new SuggestionResult(this, result) : mEmptyResult;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (mCannedResponses != null ? !mCannedResponses.equals(that.mCannedResponses) : that.mCannedResponses != null) return false;
    ...
    public static SuggestionSource makeCanned(String query, String s, long delay) {
        ...
        final SuggestionData suggestion = new SuggestionData.Builder(name).title(s).intentAction("view").intentData(s).build();
        return new TestSuggestionSource.Builder().setComponent(name).setLabel(s).setDelay(delay).addCannedResponse(query, suggestion).create();
    }
    ...
    static class Builder {
        ...
        private Map<String, List<SuggestionData>> mCannedResponses = new HashMap<String, List<SuggestionData>>();
        ...
        public TestSuggestionSource create() {
            return new TestSuggestionSource(mComponent, mDelay, mCannedResponses, mErrorQueries, mQueryAfterZeroResults, mLabel);
        }
    }
}
