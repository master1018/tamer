...
class FilteredAttributes implements Attributes {
   ...
   */
    private int reverseIndex(int k) {
        if (reverseIndexMap == null) {
            ...
            for (int i = 0, len = indexSet.size(); i < len; i++) reverseIndexMap[indexSet.get(i)] = i + 1;
        }
   ...
   */
    public String getURI(int index) {
        if (index < 0 || index >= indexSet.size()) return null;
   ...
   */
    private int getRealIndex(String uri, String localName) {
        ...
        if (index < 0 || reverseIndex(index) < 0) return -1;
        return index;
    }
