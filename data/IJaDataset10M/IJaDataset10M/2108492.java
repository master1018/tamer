...
public class StdClanFlag extends StdItem implements ClanItem {
    ...
    public void executeMsg(Environmental myHost, CMMsg msg) {
        ...
        if (StdClanItem.stdExecuteMsg(this, msg)) {
            ...
            if ((msg.amITarget(this)) && (clanID().length() > 0) && (msg.source().getClanID().equals(clanID()))) {
                ...
                if ((msg.targetMinor() == CMMsg.TYP_DROP) && (msg.trailerMsgs() == null)) {
                    ...
                    if (s.length() > 0) msg.source().tell(s); else msg.source().tell("This area is under the control of the Archons.");
                    return;
                } else if ((msg.targetMinor() == CMMsg.TYP_SPEAK) && (CMSecurity.isAllowed(msg.source(), R, "CMDROOMS")) && (msg.targetMessage() != null)) {
    ...
    public boolean okMessage(Environmental myHost, CMMsg msg) {
        if ((clanID().length() > 0) && (msg.amITarget(this))) {
            if (!msg.source().getClanID().equals(clanID())) {
                if ((msg.targetMinor() == CMMsg.TYP_GET) || (msg.targetMinor() == CMMsg.TYP_CAST_SPELL)) {
                    ...
                    if (msg.source().getClanID().length() == 0) {
                        ...
                        for (int i = 0; i < R.numInhabitants(); i++) {
                            ...
                            if ((M != null) && (M.isMonster()) && (M.getClanID().equals(clanID()) && (CMLib.flags().aliveAwakeMobileUnbound(M, true)) && (CMLib.flags().canBeSeenBy(this, M)) && (!CMLib.flags().isAnimalIntelligence(M)))) {
                    ...
                    if ((T == null) || ((!T.landOwner().equals(clanID())) && ((!T.landOwner().equals(msg.source().getLiegeID())) || (!msg.source().isMarriedToLiege())) && (!T.landOwner().equals(msg.source().Name())))) {
                        boolean ok = false;
                        if (A != null) {
            ...
            } else if ((msg.targetMinor() == CMMsg.TYP_GET) && (msg.source().location() != null) && (msg.source().isMonster())) {
                boolean foundOne = false;
                for (int i = 0; i < msg.source().location().numInhabitants(); i++) {
                    MOB M = msg.source().location().fetchInhabitant(i);
                    if ((M != null) && (!M.isMonster()) && (M.getClanID().equals(clanID()))) {
                        foundOne = true;
                        break;
                    }
                }
        ...
        if (StdClanItem.stdOkMessage(this, msg)) {
            if ((clanID().length() > 0) && (msg.amITarget(this)) && (msg.targetMinor() == CMMsg.TYP_DROP)) {
                LegalBehavior B = CMLib.law().getLegalBehavior(msg.source().location());
                String rulingClan = (B != null) ? B.rulingOrganization() : "";
                if (rulingClan.length() == 0) msg.source().tell("Area '" + msg.source().location().getArea().name() + "' is presently neutral."); else {
                    ...
                    if (!rulingClan.equals(clanID())) {
                        ...
                        if (C != null) relation = C.getClanRelations(rulingClan); else {
                            C = CMLib.clans().getClan(rulingClan);
                            if (C != null) relation = C.getClanRelations(clanID());
                        }
    ...
    public boolean tick(Tickable ticking, int tickID) {
        if (!StdClanItem.standardTick(this, tickID)) return false;
