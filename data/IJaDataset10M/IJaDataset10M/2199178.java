 ...
 */
public class AttachmentJDialog extends ProgramIconJDialog implements ActionListener {
     ...
     */
    private static final Logger logger = Logger.getLogger(AttachmentJDialog.class);
    ...
    public AttachmentJDialog(ConceptResource conceptResource, DragAndDropReference dragAndDropReference) {
        ...
        JLabel intro = new JLabel("<html>&nbsp;&nbsp;" + Messages.getString("AttachmentJDialog.introduction") + "&nbsp;&nbsp;<br><br></html>");
        p.add(intro, BorderLayout.NORTH);
        p.add(new JLabel("<html>&nbsp;&nbsp;" + Messages.getString("AttachmentJDialog.description") + "</html>"), BorderLayout.CENTER);
    ...
    void attach() {
        ...
        if (StringUtils.isEmpty(attachUrl)) {
            JOptionPane.showMessageDialog(MindRaider.mainJFrame, Messages.getString("AttachmentJDialog.errorMessage"), Messages.getString("AttachmentJDialog.error"), JOptionPane.ERROR_MESSAGE);
            return;
        }
        try {
            ...
            MindRaider.conceptCustodian.addAttachment(conceptResource, new AttachmentResource(txt, attachUrl));
        } catch (Exception e) {
            logger.debug(Messages.getString("AttachmentJDialog.unableToAttachResource", new Object[] { attachUrl, e.getMessage() }));
        }
