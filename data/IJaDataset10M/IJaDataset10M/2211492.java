...
public class Main {
    ...
    private static Reader createReader(String fname) throws IOException {
        ...
        if (encoding == null) ir = new InputStreamReader(fs); else ir = new InputStreamReader(fs, encoding);
    ...
    public static void main(String args[]) {
        ...
        for (i = 0; i < args.length; i++) {
            ...
            if (args[i].equals("-g")) {
                ...
                if (++i >= args.length) incompleteOption("-d"); else destPath = args[i];
            } else if (args[i].equals("-h")) {
                humanHeadable = true;
            } else if (args[i].equals("-e")) {
                if (++i >= args.length) incompleteOption("-e"); else encoding = args[i];
            } else {
