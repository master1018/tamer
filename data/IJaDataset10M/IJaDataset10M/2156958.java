...
public final class InplaceEditorAction<C extends JComponent> extends WidgetAction.LockedAdapter implements InplaceEditorProvider.EditorController {
    ...
    public State mouseClicked(Widget widget, WidgetMouseEvent event) {
        if (event.getButton() == MouseEvent.BUTTON1 && event.getClickCount() == 2) {
            if (openEditor(widget)) return State.createLocked(widget, this);
        }
        return State.REJECTED;
    }

    public State mousePressed(Widget widget, WidgetMouseEvent event) {
        if (editor != null) closeEditor(true);
        return State.REJECTED;
    }
    ...
    public State keyTyped(Widget widget, WidgetKeyEvent event) {
        if (event.getKeyChar() == KeyEvent.VK_ENTER) if (openEditor(widget)) return State.createLocked(widget, this);
        return State.REJECTED;
    }
    ...
    public final boolean openEditor(Widget widget) {
        ...
        if (rectangle.width > size.width) size.width = rectangle.width;
        if (rectangle.height > size.height) size.height = rectangle.height;
        int x = center.x - size.width / 2;
        int y = center.y - size.height / 2;
        if (x + size.width > component.getWidth()) x = component.getWidth() - size.width;
        if (y + size.height > component.getHeight()) y = component.getHeight() - size.height;
        if (x < 0) x = 0;
        if (y < 0) y = 0;
