...
public final class VM_OptMethodSummary implements VM_BytecodeConstants {
   ...
   */
    public static final boolean mayWrite(VM_Method method, VM_Field field) {
        if (method.getBytecodes() == null) return true;
   ...
   */
    public static void summarizeMethod(VM_Method method, byte[] bytecodes, boolean isSynchronized) {
   ...
   */
    private static void storeSummary(VM_Method method, int summary) {
        ...
        if (idx >= summaries.length) {
            ...
            if (idx >= newLength) newLength = idx;
            int[] newarray = new int[newLength];
            for (int i = 0, n = summaries.length; i < n; ++i) newarray[i] = summaries[i];
            summaries = newarray;
        }
   ...
   */
    private static void storeWriteSummary(VM_Method method, VM_Field[] set) {
        ...
        if (idx >= writeSets.length) {
            ...
            for (int i = 0, n = writeSets.length; i < n; ++i) newarray[i] = writeSets[i];
            writeSets = newarray;
        }
        writeSets[idx] = set;
    }
   ...
   */
    ...
    private static int setSize(int s, int size) {
        ...
        if (size > SIZE_MASK) return s |= SIZE_MASK; else return s |= (size);
    }
   ...
   */
    private static void computeSummary(VM_Method method, byte[] bytecodes, boolean isSynchronized) {
        int calleeSize = 0;
        int bcIndex = 0;
        ...
        while (bcIndex < bytecodes.length) {
            ...
            switch(code) {
                ...
                case JBC_invokestatic:
                    ...
                    if (meth.getDeclaringClass().isMagicType() || meth.getDeclaringClass().isAddressType()) {
    ...
    private static int getIntOffset(int index, byte[] bytecodes) {
        return (int) ((((int) bytecodes[index]) << 24) | ((((int) bytecodes[index + 1]) & 0xFF) << 16) | ((((int) bytecodes[index + 2]) & 0xFF) << 8) | (((int) bytecodes[index + 3]) & 0xFF));
    }
}
