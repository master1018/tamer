...
public class PopulateDatabaseAction extends ActionAdapter {
    ...
    private boolean checkRootConcept() throws DAOException {
        ...
        if (!gotRoot) {
            int ok = JOptionPane.showConfirmDialog(AppFrameDispatcher.getFrame(), "Unable to find the root of the knowledgebase. Do you want to create one?", "VARS - No Root Found", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
    ...
    private boolean checkAdmin() throws DAOException {
        ...
        if (!gotAdmins) {
            int ok = JOptionPane.showConfirmDialog(AppFrameDispatcher.getFrame(), "Unable to find any Administrator accounts. Do you want to create one?", "VARS - No Administrator Found", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            if (ok == JOptionPane.YES_OPTION) {
                UserAccount admin = NewUserDialog.showDialog(AppFrameDispatcher.getFrame(), true, "VARS - Create Administrator Account");
                if (admin != null) {
