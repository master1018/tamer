 ...
 */
public class ServerDeliveryProcessorClientSideVoucherPrint extends ServerDeliveryProcessor {

    private static final long serialVersionUID = 1L;

    public static final String ERROR_CODE_MISSING_VOUCHER_LAYOUT = ServerDeliveryProcessorClientSideVoucherPrint.class + "." + "MISSING_VOUCHER_LAYOUT";
    ...
    public static ServerDeliveryProcessorClientSideVoucherPrint getServerDeliveryProcessorClientSideVoucherPrint(PersistenceManager pm) {
        ...
        try {
            ...
            serverDeliveryProcessor = (ServerDeliveryProcessorClientSideVoucherPrint) pm.getObjectById(ServerPaymentProcessorID.create(Organisation.DEV_ORGANISATION_ID, ServerDeliveryProcessorClientSideVoucherPrint.class.getName()));
        } catch (JDOObjectNotFoundException e) {
            serverDeliveryProcessor = new ServerDeliveryProcessorClientSideVoucherPrint(Organisation.DEV_ORGANISATION_ID, ServerDeliveryProcessorClientSideVoucherPrint.class.getName());
            serverDeliveryProcessor.getName().setText(Locale.ENGLISH.getLanguage(), "Server Delivery Processor for delivering vouchers to print on the client");
            serverDeliveryProcessor.getName().setText(Locale.GERMAN.getLanguage(), "Server Lieferprozessor für das Liefern von Gutscheinen zum Drucken auf dem Client");
    ...
    protected RequirementCheckResult _checkRequirements(CheckRequirementsEnvironment checkRequirementsEnvironment) {
        ...
        for (ArticleID articleID : checkRequirementsEnvironment.getArticleIDs()) {
            ...
            if (!(article.getProductType() instanceof VoucherType)) throw new IllegalArgumentException("The Article does not contain VoucherType! " + articleID);
            VoucherType voucherType = (VoucherType) article.getProductType();
            if (voucherType.getVoucherLayout() == null) {
                String message = String.format(Messages.getString("org.nightlabs.jfire.voucher.store.deliver.ServerDeliveryProcessorClientSideVoucherPrint.message.noVoucherLayout"), voucherType.getName().getText(NLLocale.getDefault()));
