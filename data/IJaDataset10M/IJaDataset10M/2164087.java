...
public final class BitVector {
    ...
    public BitVector(int numBits, byte[] rawData) {
        if (numBits > rawData.length * 8) throw new IllegalArgumentException("raw data array too small to represent the requested number of bits");
        bits = numBits;
    ...
    public static int unionAndCount(BitVector... vectors) {
        ...
        for (int i = 0; i < bits; i++) {
            if (i % 8 == 0) {
                int idx = i / 8;
                union = (byte) 0x00;
                for (BitVector v : vectors) union |= v.vector[idx];
            }
            if ((union & (0x01 << i % 8)) != 0) c++;
        }
        return c;
    }
    ...
    public static int intersectAndCount(BitVector... vectors) {
        ...
        for (int i = 0; i < bits; i++) {
            if (i % 8 == 0) {
                ...
                for (BitVector v : vectors) intersection &= v.vector[idx];
            }
            if ((intersection & (0x01 << i % 8)) != 0) c++;
        }
        return c;
    }
