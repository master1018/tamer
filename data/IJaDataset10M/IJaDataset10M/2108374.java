...
public class ASUtils {
     ...
     */
    public static JDialog showDbcsDialog(Window parentWindow, JDBCDataSource dataSource, final Runnable onAccept) {
    ...
    public static JDialog showDbcsDialog(Window parentWindow, JDBCDataSource dataSource, final Runnable onAccept, boolean enforceUniqueName) {
        ...
        JDialog d = DataEntryPanelBuilder.createDataEntryPanelDialog(dbcsPanel, parentWindow, Messages.getString("ASUtils.databaseConnectionDialogTitle", dataSource.getDisplayName()), DataEntryPanelBuilder.OK_BUTTON_LABEL, okCall, cancelCall);
        d.pack();
    ...
    public static DataEntryPanel createDataSourceOptionsPanel(JDBCDataSource ds, boolean enforceUniqueName) {
        ...
        p.addTab(Messages.getString("ASUtils.datasourceOptionsGeneralTab"), generalPanel);
    ...
    public static List<Point2D.Double> getIntersectPoints(Shape s1, Shape s2) {
        ...
        while (!myPI.isDone()) {
            ...
            } else if (mySegType == PathIterator.SEG_MOVETO || mySegType == PathIterator.SEG_CLOSE) {
                myLine.x1 = myCoords[0];
                myLine.y1 = myCoords[1];
                myLine.x2 = myCoords[0];
                myLine.y2 = myCoords[1];
            } else {
                throw new IllegalStateException("Unsupported my PathIterator type " + mySegType + ". Current myLine is " + lineToString(myLine));
            }
            myPI.next();
            ...
            while (!otherPI.isDone()) {
                ...
                } else if (otherSegType == PathIterator.SEG_MOVETO || otherSegType == PathIterator.SEG_CLOSE) {
                    otherLine.x1 = otherCoords[0];
                    otherLine.y1 = otherCoords[1];
                    otherLine.x2 = otherCoords[0];
                    otherLine.y2 = otherCoords[1];
                } else {
                    throw new IllegalStateException("Unsupported other PathIterator type " + otherSegType + ". Current otherLine is " + lineToString(otherLine));
                }
                otherPI.next();
    ...
    public static ArchitectSwingSessionContext getContext() {
        if (context == null) {
            try {
                ...
                showExceptionDialogNoReport(Messages.getString("ASUtils.couldNotLaunchPowerArchitect"), e);
                System.exit(1);
            }
        }
        return context;
        ...
        if (containingSessions.size() == 1) {
            ...
            for (ArchitectSwingSession session : containingSessions) {
                if (!session.getWorkspace().getUUID().equals(currentSession.getWorkspace().getUUID())) {
                    throw new IllegalStateException("The object " + source + " exists in both " + currentSession.getWorkspace() + " and " + session.getWorkspace() + " by UUID equality.");
                }
            }
