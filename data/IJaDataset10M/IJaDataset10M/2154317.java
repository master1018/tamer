...
public class Downloader extends Progressable {
    ...
    protected synchronized void download(FileDownload current) throws IOException {
        URLConnection connection = new URL(current.getURL()).openConnection();
        ...
        if (total == 0) total = (count + currentTotal) * itemTotal / (itemCount + 1);
