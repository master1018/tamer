...
public class ProtocolImpl implements Protocol {
	 ...
	 * @throws IOException
	 */
    public Header read(Configuration conf, DataInputStream inputStream) throws IOException {
        Header header = null;
        try {
            ...
            if (bytesRead != codecNameLen) {
                throw new RuntimeException("The codecLength in the stream is not equal to the Stream Length");
            }
            ...
            if (ioBytesRead != headerLen) {
                throw new RuntimeException("The bytes available in the input stream does not match the header length integer passed in the stream (" + headerLen + ")");
            }
            ByteArrayInputStream byteInput = new ByteArrayInputStream(headerBytes);
            CompressionPool pool = compressionPoolFactory.get(codec);
            CompressionInputStream compressionInput = pool.create(byteInput, waitForCompressionResource, TimeUnit.MILLISECONDS);
            if (compressionInput == null) {
                throw new IOException("No decompression resource available for " + codec);
            }
	 ...
	 */
    public void send(Header header, CompressionCodec codec, DataOutput dataOut) throws IOException, InterruptedException {
        CompressionPool pool = compressionPoolFactory.get(codec);
        ByteArrayOutputStream byteOut = new ByteArrayOutputStream(100);
        CompressionOutputStream compressionOut = pool.create(byteOut, waitForCompressionResource, TimeUnit.MILLISECONDS);
        try {
