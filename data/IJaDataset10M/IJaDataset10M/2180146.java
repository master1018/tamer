...
public class StatisticsTest {
    ...
    private void check(final GenericRiskStatistics s, final String name) {
        for (int i = 0; i < data.size(); i++) s.add(data.get(i), weights.get(i));
        double calculated, expected;
        double tolerance;
        if (s.samples() != data.size()) fail("wrong number of samples \n" + "calculated: " + s.samples() + "\n" + "expected: " + data.size());
        expected = weights.accumulate();
        calculated = s.weightSum();
        if (calculated != expected) fail(name + ": wrong sum of weights\n" + "    calculated: " + calculated + "\n" + "    expected:   " + expected);
        expected = data.min();
        calculated = s.min();
        if (calculated != expected) fail(name + ": wrong minimum value \n" + "calculated: " + calculated + "\n" + "expected: " + expected);
        expected = data.max();
        calculated = s.max();
        if (calculated != expected) fail(name + ": wrong maxmimum value \n" + "calculated: " + expected + "\n" + "expected: " + expected);
        expected = 4.3;
        tolerance = 1.0e-9;
        calculated = s.mean();
        if (Math.abs(calculated - expected) > tolerance) fail(name + "wrong mean value" + "\n" + "calculated: " + calculated + "\n" + "expected: " + expected);
        expected = 2.23333333333;
        calculated = s.variance();
        if (Math.abs(calculated - expected) > tolerance) fail(name + "wrong variance" + "\n" + "calculated: " + calculated + "\n" + "expected: " + expected);
        expected = 1.4944341181;
        calculated = s.standardDeviation();
        if (Math.abs(calculated - expected) > tolerance) fail(name + "wrong standard deviation" + "\n" + "calculated: " + calculated + "\n" + "expected: " + expected);
        expected = 0.359543071407;
        calculated = s.skewness();
        if (Math.abs(calculated - expected) > tolerance) fail(name + "wrong skewness" + "\n" + "calculated: " + calculated + "\n" + "expected: " + expected);
    ...
    private void checkSequence(final GenericRiskStatistics stat, final String name, int dimension) {
        ...
        if (ss.samples() != data.size()) fail("SequenceStatistics<" + name + ">: " + "wrong number of samples\n" + "    calculated: " + ss.samples() + "\n" + "    expected:   " + data.size());
        expected = weights.accumulate(0.0);
        if (ss.weightSum() != expected) fail("SequenceStatistics<" + name + ">: " + "wrong sum of weights\n" + "    calculated: " + ss.weightSum() + "\n" + "    expected:   " + expected);
        expected = data.min();
        calculated = ss.min();
        for (i = 0; i < dimension; i++) {
            if (calculated.get(i) != expected) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong minimum value\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
        expected = data.max();
        calculated = ss.max();
        for (i = 0; i < dimension; i++) {
            if (calculated.get(i) != expected) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong maximun value\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
        expected = 4.3;
        tolerance = 1.0e-9;
        calculated = ss.mean();
        for (i = 0; i < dimension; i++) {
            if (Math.abs(calculated.get(i) - expected) > tolerance) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong mean value\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
        expected = 2.23333333333;
        calculated = ss.variance();
        for (i = 0; i < dimension; i++) {
            if (Math.abs(calculated.get(i) - expected) > tolerance) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong variance\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
        expected = 1.4944341181;
        calculated = ss.standardDeviation();
        for (i = 0; i < dimension; i++) {
            if (Math.abs(calculated.get(i) - expected) > tolerance) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong standard deviation\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
        expected = 0.359543071407;
        calculated = ss.skewness();
        for (i = 0; i < dimension; i++) {
            if (Math.abs(calculated.get(i) - expected) > tolerance) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong skewness\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
        expected = -0.151799637209;
        calculated = ss.kurtosis();
        for (i = 0; i < dimension; i++) {
            if (Math.abs(calculated.get(i) - expected) > tolerance) fail("SequenceStatistics<" + name + ">: " + (i + 1) + " dimension: " + "wrong kurtosis\n" + "    calculated: " + calculated.get(i) + "\n" + "    expected:   " + expected);
        }
    }
    ...
    private void checkConvergence(final GenericRiskStatistics stat, final String name) {
        ...
        if (calculatedSize != expectedSize1) fail("ConvergenceStatistics<" + name + ">: " + "\nwrong convergence-table size" + "\n    calculated: " + calculatedSize + "\n    expected:   " + expectedSize1);
        final double tolerance = 1.0e-9;
        {
            final double expectedValue1 = 4.0;
            final List<Pair<Integer, Double>> table = stats.convergenceTable();
            double calculatedValue = table.get(table.size() - 1).second();
            if (Math.abs(calculatedValue - expectedValue1) > tolerance) fail("wrong last value in convergence table" + "\n    calculated: " + calculatedValue + "\n    expected:   " + expectedValue1);
        }
        {
            final int expectedSampleSize1 = 7;
            final List<Pair<Integer, Double>> table = stats.convergenceTable();
            int calculatedSamples = table.get(table.size() - 1).first();
            if (calculatedSamples != expectedSampleSize1) fail("wrong number of samples in convergence table" + "\n    calculated: " + calculatedSamples + "\n    expected:   " + expectedSampleSize1);
        }
        stats.reset();
        stats.add(1.0);
        stats.add(2.0);
        ...
        if (calculatedSize != expectedSize2) fail("wrong convergence-table size" + "\n    calculated: " + calculatedSize + "\n    expected:   " + expectedSize2);
        {
            final double expectedValue2 = 2.0;
            final List<Pair<Integer, Double>> table = stats.convergenceTable();
            double calculatedValue = table.get(table.size() - 1).second();
            if (Math.abs(calculatedValue - expectedValue2) > tolerance) fail("wrong last value in convergence table" + "\n    calculated: " + calculatedValue + "\n    expected:   " + expectedValue2);
        }
        {
            final int expectedSampleSize2 = 3;
            final List<Pair<Integer, Double>> table = stats.convergenceTable();
            int calculatedSamples = table.get(table.size() - 1).first();
            if (calculatedSamples != expectedSampleSize2) fail("wrong number of samples in convergence table" + "\n    calculated: " + calculatedSamples + "\n    expected:   " + expectedSampleSize2);
        }
    }
}
