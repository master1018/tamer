...
public class IPv6AddressRange implements Comparable<IPv6AddressRange>, Iterable<IPv6Address> {
    ...
    public IPv6AddressRange(IPv6Address first, IPv6Address last) {
        if (first.compareTo(last) > 0) throw new IllegalArgumentException("Cannot create ip address range with last address < first address");
        this.first = first;
        this.last = last;
    }
    ...
    public List<IPv6AddressRange> remove(IPv6Address address) {
        if (address == null) throw new IllegalArgumentException("invalid address [null]");
        if (!contains(address)) return Collections.singletonList(this); else if (address.equals(first) && address.equals(last)) return Collections.emptyList(); else if (address.equals(first)) return Collections.singletonList(new IPv6AddressRange(first.add(1), last)); else if (address.equals(last)) return Collections.singletonList(new IPv6AddressRange(first, last.subtract(1))); else return Arrays.asList(new IPv6AddressRange(first, address.subtract(1)), new IPv6AddressRange(address.add(1), last));
    }
    ...
    public IPv6AddressRange extend(IPv6Address address) {
        if (address.compareTo(first) < 0) return new IPv6AddressRange(address, last); else if (address.compareTo(last) > 0) return new IPv6AddressRange(first, address); else return this;
    }
    ...
    public List<IPv6AddressRange> remove(IPv6Network network) {
        if (network == null) throw new IllegalArgumentException("invalid network [null]");
        if (!contains(network)) return Collections.singletonList(this); else if (this.equals(network)) return Collections.emptyList(); else if (first.equals(network.getFirst())) return Collections.singletonList(new IPv6AddressRange(network.getLast().add(1), last)); else if (last.equals(network.getLast())) return Collections.singletonList(new IPv6AddressRange(first, network.getFirst().subtract(1))); else return Arrays.asList(new IPv6AddressRange(first, network.getFirst().subtract(1)), new IPv6AddressRange(network.getLast().add(1), last));
    }
    ...
    public int compareTo(IPv6AddressRange that) {
        if (this.first != that.first) return this.first.compareTo(that.first); else return this.last.compareTo(that.last);
    }
    ...
    private final class Ipv6AddressRangeIterator implements Iterator<IPv6Address> {
        ...
        public IPv6Address next() {
            if (hasNext()) return current = current.add(1); else throw new NoSuchElementException();
        }
