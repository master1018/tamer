...
public class TypeBuilder extends NodeVisitor {
    ...
    public TypeBuilder pushPackage(Package p) {
        if (Report.should_report(Report.visit, 4)) Report.report(4, "TB pushing package " + p + ": " + context());
        TypeBuilder tb = push();
        tb.inCode = false;
        tb.package_ = p;
        return tb;
    ...
    public TypeBuilder pushCode(CodeDef def) {
        if (Report.should_report(Report.visit, 4)) Report.report(4, "TB pushing code: " + context());
    ...
    public TypeBuilder pushClass(ClassDef classDef) {
        if (Report.should_report(Report.visit, 4)) Report.report(4, "TB pushing class " + classDef + ": " + context());
    ...
    protected ClassDef newClass(Position pos, Flags flags, Name name) {
        ...
        if (inCode) {
            ...
            if (dup != null && dup.fullName().equals(fullName)) {
                job.compiler().errorQueue().enqueue(ErrorInfo.SEMANTIC_ERROR, "Duplicate class \"" + ct.fullName() + "\".", pos);
    ...
    public TypeBuilder pushAnonClass(Position pos) {
        if (Report.should_report(Report.visit, 4)) Report.report(4, "TB pushing anon class: " + this);
        if (!inCode) {
    ...
    public String context() {
        return "(TB " + type + (inCode ? " inCode" : "") + (global ? " global" : "") + (outer == null ? ")" : " " + outer.context() + ")");
    }
}
