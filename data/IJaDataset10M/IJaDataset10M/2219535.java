...
public class Cylinder extends Quadric {
    ...
    public Cylinder(float radius, float zmin, float zmax, float thetaMin, float thetaMax, ParameterList parameters, Attributes attributes) {
    ...
    public BoundingVolume getBoundingVolume() {
        ...
        float yMin;
        float yMax;
        if (thetaMin < Math.PI && thetaMax > Math.PI) xMin = -radius; else {
            xMin = cosThetaMin * radius;
            xMin = Math.min(xMin, cosThetaMax * radius);
        }
        if (thetaMin < Math.PI / 2 && thetaMax > Math.PI / 2) yMax = radius; else {
            yMax = sinThetaMin * radius;
            yMax = Math.max(yMax, sinThetaMax * radius);
        }
        if (thetaMin < Math.PI * 3 / 2 && thetaMax > Math.PI * 3 / 2) yMin = -radius; else {
    ...
    public Primitive[] split() {
        ...
        if ((thetaMax - thetaMin) * radius > zmax - zmin) {
            result[0] = new Cylinder(radius, zmin, zmax, thetaMin, (thetaMin + thetaMax) / 2f, bilinearInterpolateParameters(0f, .5f, 0f, 1f), attributes);
            result[1] = new Cylinder(radius, zmin, zmax, (thetaMin + thetaMax) / 2f, thetaMax, bilinearInterpolateParameters(.5f, 1f, 0f, 1f), attributes);
        } else {
            result[0] = new Cylinder(radius, zmin, (zmin + zmax) / 2f, thetaMin, thetaMax, bilinearInterpolateParameters(0f, 1f, 0f, .5f), attributes);
            result[1] = new Cylinder(radius, (zmin + zmax) / 2f, zmax, thetaMin, thetaMax, bilinearInterpolateParameters(0f, 1f, .5f, 1f), attributes);
        }
        return result;
    }
