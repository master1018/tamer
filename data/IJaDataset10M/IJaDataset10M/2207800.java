...
public class HierarchicalHTMLReportGenerator implements ReportGenerator {
    ...
    private List<NoPerHierarchyLevelType> noPerHLType = new Vector<NoPerHierarchyLevelType>();
     ...
     */
    public void generateReport(Report report) throws TemplateIncompatibleException, ReportIOException, FileCreationException, TemplateParseException, ReportTemplateApplyException {
        this.report = report;
        checkReport();
        try {
            generatePages();
            ...
            String msg = "The method '" + e.getMethodName() + "' called via" + " the VTL reference '" + e.getReferenceName() + "' threw an exception (this might indicate an" + " error in the template): " + stackTrace;
            ReportTemplateApplyException rtae = new ReportTemplateApplyException(msg, e.getWrappedThrowable());
            throw rtae;
        } catch (IOException e) {
            ReportIOException ioe = new ReportIOException("IO error applying a" + " template", e);
            throw ioe;
    ...
    private void checkReport() throws TemplateIncompatibleException {
        ...
        if (report.getTemplate() == null || report.getTemplate().getTemplate() == null) {
            throw new IllegalArgumentException("no <template>-Element given in report");
        }
        ...
        if (version < 1 || READER_VERSION < version) {
            throw new TemplateIncompatibleException("Template version " + version + "is not compatible with this ReportGenerator" + " version " + READER_VERSION + ".", TemplateIncompatibleException.VersionField.OUTER);
        }
    }
    ...
    private void generatePages() throws ParseErrorException, MethodInvocationException, IOException, FileCreationException {
        this.report.getLogger().debug("Generating pages of" + " HierarchicalHTMLReport...");
        final ProgressHandler progressHandler = report.getProgressHandler();
        progressHandler.setProgress(0.05f);
        VelocityHelper vh = new VelocityHelper();
        VelocityContext context = new VelocityContext();
        HierarchyLevel topmostLevel = report.getTestCases().get(0).getTestSession().getTestSessionContainer().getCode();
        ...
        topmostLevel.accept(new HierarchyLevel.Visitor() {
            ...
            public void visit(HierarchyLevel hierarchyLevel) {
                ...
                int i = 0;
                while (i < noPerHLType.size() && nphlt == null) {
        ...
        context.put("dateFormatter", DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG, new Locale(this.language)));
        ...
        for (Metric metric : MetricProvider.getAvailabeMetrics(report.getMetrics(), report.getTestCases().get(0).getTestSession().getTestSessionContainer().getCriteria())) {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (nodes.item(i).getNodeName().equals("text-file")) {
                ...
                osw = new OutputStreamWriter(report.getCreationHandler().createFile(atts.getNamedItem("filename").getNodeValue(), atts.getNamedItem("content-type").getNodeValue()), templateEncoding);
                vh.evaluate(context, osw, "hhr_text-file", textFile.getTextContent(), null);
                osw.close();
            } else if (nodes.item(i).getNodeName().equals("resource")) {
                Node resource = nodes.item(i);
                NamedNodeMap atts = resource.getAttributes();
                java.io.OutputStream os;
                os = report.getCreationHandler().createFile(atts.getNamedItem("filename").getNodeValue(), "application/octet-stream");
        ...
        osw = new OutputStreamWriter(report.getIndexOutputStream(), templateEncoding);
        ...
        for (HierarchyLevel level : topmostLevel.getChildren()) {
            generateReportPage(context, level, "." + FileCreationHandler.SEPARATOR + uniqueNames.get(level), new ArrayList<HierarchyLevel>());
        }
        progressHandler.setProgress(0.95f);
        this.report.getLogger().debug("Finished HierarchicalHTMLReport.");
    }

    private void generateReportPage(VelocityContext fathersContext, HierarchyLevel currentLevel, String path, List<HierarchyLevel> ancestors) throws ParseErrorException, MethodInvocationException, IOException, FileCreationException {
        ...
        OutputStreamWriter osw = new OutputStreamWriter(report.getCreationHandler().createFile(path + FileCreationHandler.SEPARATOR + "index.html", getContentType()), templateEncoding);
        if (currentLevel.getType().getInternalName().equals("method")) {
            HTMLCodeHighlighting codehighlight = new HTMLCodeHighlighting(this.report.getLogger());
            context.put("code", codehighlight);
            codePageTemplateCache = vh.evaluate(context, osw, "hhr_codePage", codePageTemplate, codePageTemplateCache);
        } else {
            selectionPageTemplateCache = vh.evaluate(context, osw, "hhr_selectionPage", selectionPageTemplate, selectionPageTemplateCache);
        }
        osw.close();
        ancestors.add(currentLevel);
        for (HierarchyLevel level : currentLevel.getChildren()) {
            generateReportPage(context, level, path + FileCreationHandler.SEPARATOR + uniqueNames.get(level), new ArrayList<HierarchyLevel>(ancestors));
        }
    }
}
