 ...
 * 
 */
public class BinaryTree implements IKoratTouchable {

    public static class Node implements IKoratTouchable {

        private transient ITester __myTester;

        public Node(ITester __myTester) {
            this.__myTester = __myTester;
        }

        private transient boolean __korat_touched__;

        public void __korat__touch__initialize() {
            if (!__korat_touched__) return;
            __korat_touched__ = false;
            if (left instanceof IKoratTouchable) ((IKoratTouchable) left).__korat__touch__initialize();
            if (right instanceof IKoratTouchable) ((IKoratTouchable) right).__korat__touch__initialize();
        }

        public void __korat__touch() {
            if (__korat_touched__) return;
            __korat_touched__ = true;
            if (left instanceof IKoratTouchable) ((IKoratTouchable) left).__korat__touch();
            if (right instanceof IKoratTouchable) ((IKoratTouchable) right).__korat__touch();
        }

        private Node left;

        private transient int __id_left = -1;
        ...
        private transient int __id_right = -1;
    ...
    public void __korat__touch__initialize() {
        if (!__korat_touched__) return;
        __korat_touched__ = false;
        if (root instanceof IKoratTouchable) ((IKoratTouchable) root).__korat__touch__initialize();
    }

    public void __korat__touch() {
        if (__korat_touched__) return;
        __korat_touched__ = true;
        if (root instanceof IKoratTouchable) ((IKoratTouchable) root).__korat__touch();
    }

    private Node root;

    private transient int __id_root = -1;
    ...
    private transient int __id_size = -1;
    ...
    public boolean has(Node n) {
        ...
        while (!workList.isEmpty()) {
            ...
            if (current.getLeft() != null) workList.add(current.getLeft());
            if (current.getRight() != null) workList.add(current.getRight());
        }
        return false;
    }
    ...
    public boolean repOK() {
        if (getRoot() == null) return getSize() == 0;
    ...
    public static IFinitization finBinaryTree(int numNodes, int minSize, int maxSize) throws Exception {
