...
public class BatikFlowTextElementBridge extends SVGTextElementBridge implements BatikExtConstants {
    ...
    protected void addGlyphPositionAttributes(AttributedString as, Element element, BridgeContext ctx) {
        ...
        if ((!eNS.equals(getNamespaceURI())) && (!eNS.equals(SVG_NAMESPACE_URI))) return;
    ...
    protected RegionInfo buildRegion(UnitProcessor.Context uctx, Element e, float verticalAlignment) {
        ...
        float x = 0;
        if (s.length() != 0) {
            x = UnitProcessor.svgHorizontalCoordinateToUserSpace(s, BATIK_EXT_X_ATTRIBUTE, uctx);
        }
        s = e.getAttribute(BATIK_EXT_Y_ATTRIBUTE);
        float y = 0;
        if (s.length() != 0) {
            y = UnitProcessor.svgVerticalCoordinateToUserSpace(s, BATIK_EXT_Y_ATTRIBUTE, uctx);
        }
        s = e.getAttribute(BATIK_EXT_WIDTH_ATTRIBUTE);
        float w;
        if (s.length() != 0) {
            w = UnitProcessor.svgHorizontalLengthToUserSpace(s, BATIK_EXT_WIDTH_ATTRIBUTE, uctx);
        } else {
            throw new BridgeException(e, ERR_ATTRIBUTE_MISSING, new Object[] { BATIK_EXT_WIDTH_ATTRIBUTE, s });
        }
        if (w == 0) {
            return null;
        }
        ...
        float h;
        if (s.length() != 0) {
            h = UnitProcessor.svgVerticalLengthToUserSpace(s, BATIK_EXT_HEIGHT_ATTRIBUTE, uctx);
        } else {
            throw new BridgeException(e, ERR_ATTRIBUTE_MISSING, new Object[] { BATIK_EXT_HEIGHT_ATTRIBUTE, s });
        }
        if (h == 0) {
            return null;
        }
    ...
    protected void fillAttributedStringBuffer(BridgeContext ctx, Element element, boolean top, Integer bidiLevel, AttributedStringBuffer asb, List lnLocs) {
        ...
        for (Node n = element.getFirstChild(); n != null; n = n.getNextSibling()) {
            if (preserve) {
                ...
                if (asb.length() == 0) prevEndsWithSpace = true; else prevEndsWithSpace = (asb.getLastChar() == ' ');
            }
            switch(n.getNodeType()) {
                case Node.ELEMENT_NODE:
                    if ((!getNamespaceURI().equals(n.getNamespaceURI())) && (!SVG_NAMESPACE_URI.equals(n.getNamespaceURI()))) break;
        ...
        if (top) {
            while ((endLimit < asb.length()) && (asb.getLastChar() == ' ')) asb.stripLast();
        }
    }
