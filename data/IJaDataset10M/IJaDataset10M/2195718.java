...
public class ExtrudedPolygon extends AbstractShape {
        ...
        protected void copySideTextureReferences(ExtrudedPolygon shape) {
            if (shape.sideTextures != null) {
                for (int i = 0; i < this.boundaries.size() && i < shape.sideTextures.size(); i++) {
                    ...
                    if (ebi != null) this.boundaries.get(i).sideTextures = shape.sideTextures.get(i);
                }
            }
        }
    ...
    protected void reset() {
        for (List<? extends LatLon> locations : this.boundaries) {
            if (locations == null || locations.size() < 3) continue;
            if (!WWMath.computeWindingOrderOfLocations(locations).equals(AVKey.COUNTER_CLOCKWISE)) Collections.reverse(locations);
        }
    ...
    public void setOuterBoundary(Iterable<? extends LatLon> corners, Iterable<?> imageSources) {
        ...
        if (imageSources == null && this.sideTextures == null) return;
        if (this.sideTextures == null) this.sideTextures = new ArrayList<List<WWTexture>>();
        ...
        for (ShapeDataCache.ShapeDataCacheEntry entry : this.shapeDataCache) {
            ...
            if (sd.boundaries != null) sd.copySideTextureReferences(this);
        }
    }
    ...
    protected List<? extends LatLon> fillBoundary(Iterable<? extends LatLon> corners) {
        ...
        for (LatLon corner : corners) {
            if (corner != null) list.add(corner);
        }
        if (list.size() < 3) {
        ...
        if (list.size() > 0 && !list.get(0).equals(list.get(list.size() - 1))) list.add(list.get(0));
        list.trimToSize();
        return list;
    }
    ...
    public void addInnerBoundary(Iterable<? extends LatLon> corners, Iterable<?> imageSources) {
        ...
        if (imageSources != null) {
            if (this.sideTextures == null) {
                this.sideTextures = new ArrayList<List<WWTexture>>();
                this.sideTextures.add(new ArrayList<WWTexture>());
            }
            this.sideTextures.add(this.fillImageList(imageSources));
        }
        this.reset();
    ...
    protected List<WWTexture> fillImageList(Iterable<?> imageSources) {
        ...
        for (Object source : imageSources) {
            if (source != null) textures.add(this.makeTexture(source)); else textures.add(null);
        }
        textures.trimToSize();
        return textures;
    }
    ...
    public float[] getTextureCoords() {
        if (this.capTextureCoords == null) return null;
    ...
    public Sector getSector() {
        if (this.sector == null && this.outerBoundary().size() > 2) this.sector = Sector.boundingSector(this.getOuterBoundary());
        return this.sector;
    }
    ...
    public Position getReferencePosition() {
        if (this.referencePosition != null) return this.referencePosition;
        if (this.boundaries.size() > 0 && this.outerBoundary().size() > 0) {
            if (this.outerBoundary().get(0) instanceof Position) this.referencePosition = (Position) this.outerBoundary().get(0); else this.referencePosition = new Position(this.outerBoundary().get(0), 0);
        }
    ...
    public List<List<Object>> getImageSources() {
        if (this.sideTextures == null) return null;
    ...
    public boolean hasSideTextures() {
        if (this.sideTextures == null) return false;
        for (List<WWTexture> textures : this.sideTextures) {
            if (textures != null && textures.size() > 0) return true;
        }
        return false;
    }
    ...
    protected boolean mustApplyTexture(DrawContext dc) {
        if (this.getCapTexture() != null && this.capTextureCoords != null) return true;
    ...
    protected boolean mustApplyLighting(DrawContext dc) {
        if (dc == null || super.mustApplyLighting(dc)) return true;
        return this.getActiveSideAttributes().isEnableLighting();
    }

    protected boolean mustRegenerateGeometry(DrawContext dc) {
        ShapeData shapeData = this.getCurrent();
        if (shapeData.capVertexBuffer == null || shapeData.sideVertexBuffer == null) return true;
        if (dc.getVerticalExaggeration() != shapeData.getVerticalExaggeration()) return true;
        if (this.mustCreateNormals(dc) && (shapeData.capNormalBuffer == null || shapeData.sideNormalBuffer == null)) return true;
    ...
    protected Extent computeExtent(ExtrudedBoundaryInfo outerBoundary, Vec4 refPoint) {
        if (outerBoundary == null || outerBoundary.capVertices == null || outerBoundary.baseVertices == null) return null;
    ...
    protected void determineActiveAttributes() {
        ...
        if (this.isHighlighted()) {
            if (this.getSideHighlightAttributes() != null) this.activeSideAttributes.copy(this.getSideHighlightAttributes()); else {
                if (this.getSideAttributes() != null) this.activeSideAttributes.copy(this.getSideAttributes()); else this.activeSideAttributes.copy(defaultSideAttributes);
    ...
    public void render(DrawContext dc) {
        if (!this.isOuterBoundaryValid()) return;
        super.render(dc);
    }
    ...
    protected boolean doMakeOrderedRenderable(DrawContext dc) {
        if (dc.getSurfaceGeometry() == null || !this.isOuterBoundaryValid()) return false;
    ...
    public void drawOutline(DrawContext dc) {
        if (this.isEnableSides() && getActiveSideAttributes().isDrawOutline()) this.drawSideOutline(dc, this.getCurrent());
        if (this.isEnableCap() && getActiveCapAttributes().isDrawOutline()) this.drawCapOutline(dc, this.getCurrent());
    }

    @Override
    public void drawInterior(DrawContext dc) {
        if (this.isEnableSides() && getActiveSideAttributes().isDrawInterior()) this.drawSideInteriors(dc, this.getCurrent());
        if (this.isEnableCap() && getActiveCapAttributes().isDrawInterior()) this.drawCapInterior(dc, this.getCurrent());
    }
    ...
    public void drawCapOutline(DrawContext dc, ShapeData shapeData) {
        ...
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            if (!dc.isPickingMode() && this.mustApplyLighting(dc, this.getActiveCapAttributes())) gl.glNormalPointer(GL.GL_FLOAT, 0, boundary.capNormalBuffer.rewind());
    ...
    public void drawCapInterior(DrawContext dc, ShapeData shapeData) {
        ...
        GL gl = dc.getGL();
        if (!dc.isPickingMode() && this.mustApplyLighting(dc, this.getActiveCapAttributes())) gl.glNormalPointer(GL.GL_FLOAT, 0, shapeData.capNormalBuffer.rewind());
     ...
     */
    protected void createFullGeometry(DrawContext dc, Terrain terrain, ShapeData shapeData, boolean skipOuterBoundary) {
        for (ExtrudedBoundaryInfo boundary : shapeData) {
        ...
        if (this.isEnableSides() || this.isEnableCap()) this.createVertices(terrain, shapeData, skipOuterBoundary);
        if (this.isEnableSides()) {
            this.createSideGeometry(shapeData);
            if (this.mustApplyLighting(dc, this.getActiveSideAttributes())) this.createSideNormals(shapeData);
            if (!dc.isPickingMode() && this.mustApplySideTextures()) this.createSideTextureCoords(shapeData);
        }
        if (this.isEnableCap()) {
            this.createCapGeometry(dc, shapeData);
            if (this.mustApplyLighting(dc, this.getActiveCapAttributes())) this.createCapNormals(shapeData);
        }
    }
    ...
    protected void createVertices(Terrain terrain, ShapeData shapeData, boolean skipOuterBoundary) {
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            if (boundary != shapeData.getOuterBoundaryInfo() || !skipOuterBoundary) this.computeBoundaryVertices(terrain, boundary, shapeData.getReferencePoint());
        }
    }
    ...
    protected void computeBoundaryVertices(Terrain terrain, ExtrudedBoundaryInfo boundary, Vec4 refPoint) {
        ...
        if (topVertices == null || topVertices.length < boundary.locations.size()) topVertices = new Vec4[boundary.locations.size()];
        Vec4[] bottomVertices = boundary.baseVertices;
        if (bottomVertices == null || bottomVertices.length < boundary.locations.size()) bottomVertices = new Vec4[boundary.locations.size()];
        Vec4 N = null;
        Vec4 vaa = null;
        double vaaLength = 0;
        double vaLength = 0;
    ...
    protected void createSideGeometry(ShapeData shapeData) {
        int vertexCoordCount = this.totalFaceCount * 4 * 3;
        if (shapeData.sideVertexBuffer != null && shapeData.sideVertexBuffer.capacity() >= vertexCoordCount) shapeData.sideVertexBuffer.clear(); else shapeData.sideVertexBuffer = BufferUtil.newFloatBuffer(vertexCoordCount);
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            boundary.sideVertexBuffer = this.fillSideVertexBuffer(boundary.capVertices, boundary.baseVertices, shapeData.sideVertexBuffer.slice());
            shapeData.sideVertexBuffer.position(shapeData.sideVertexBuffer.position() + boundary.sideVertexBuffer.limit());
        }
    }

    protected void createSideNormals(ShapeData shapeData) {
        int vertexCoordCount = this.totalFaceCount * 4 * 3;
        if (shapeData.sideNormalBuffer != null && shapeData.sideNormalBuffer.capacity() >= vertexCoordCount) shapeData.sideNormalBuffer.clear(); else shapeData.sideNormalBuffer = BufferUtil.newFloatBuffer(vertexCoordCount);
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            boundary.sideNormalBuffer = this.fillSideNormalBuffer(boundary.capVertices, boundary.baseVertices, shapeData.sideNormalBuffer.slice());
            shapeData.sideNormalBuffer.position(shapeData.sideNormalBuffer.position() + boundary.sideNormalBuffer.limit());
        }
    }
    ...
    protected void createSideTextureCoords(ShapeData shapeData) {
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            boolean applyTextureToThisBoundary = this.hasSideTextures() && boundary.sideTextures != null && boundary.sideTextures.size() == boundary.faceCount;
            if (applyTextureToThisBoundary) {
                int texCoordSize = boundary.faceCount * 4 * 2;
                if (boundary.sideTextureCoords != null && boundary.sideTextureCoords.capacity() >= texCoordSize) boundary.sideTextureCoords.clear(); else boundary.sideTextureCoords = BufferUtil.newFloatBuffer(texCoordSize);
                this.fillSideTexCoordBuffer(boundary.capVertices, boundary.baseVertices, boundary.sideTextureCoords);
            }
        }
    }
    ...
    protected void createCapGeometry(DrawContext dc, ShapeData shapeData) {
        if (shapeData.capVertexBuffer != null && shapeData.capVertexBuffer.capacity() >= this.totalNumLocations * 3) shapeData.capVertexBuffer.clear(); else shapeData.capVertexBuffer = BufferUtil.newFloatBuffer(this.totalNumLocations * 3);
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            boundary.capVertexBuffer = WWBufferUtil.copyArrayToBuffer(boundary.capVertices, shapeData.capVertexBuffer.slice());
            shapeData.capVertexBuffer.position(shapeData.capVertexBuffer.position() + boundary.capVertexBuffer.limit());
        }
        if (shapeData.cb == null) this.createTessllationGeometry(dc, shapeData);
        this.generateCapInteriorIndices(shapeData);
    }

    protected void createCapNormals(ShapeData shapeData) {
        if (shapeData.capNormalBuffer != null && shapeData.capNormalBuffer.capacity() >= this.totalNumLocations * 3) shapeData.capNormalBuffer.clear(); else shapeData.capNormalBuffer = BufferUtil.newFloatBuffer(shapeData.capVertexBuffer.capacity());
        for (ExtrudedBoundaryInfo boundary : shapeData) {
            boundary.capNormalBuffer = this.computeCapNormals(boundary, shapeData.capNormalBuffer.slice());
            shapeData.capNormalBuffer.position(shapeData.capNormalBuffer.position() + boundary.capNormalBuffer.limit());
        }
    }
    ...
    protected FloatBuffer computeCapNormals(ExtrudedBoundaryInfo boundary, FloatBuffer nBuf) {
        ...
        Vec4 vb = verts[nVerts - 2].subtract3(verts[0]);
    ...
    protected IntBuffer getSideEdgeIndices(int n) {
        ...
        if (ib != null) return ib;
        int nn = n - 1;
        ib = BufferUtil.newIntBuffer((2 * nn) * 3);
        for (int i = 0; i < nn; i++) {
    ...
    protected Vec4 computePolygonNormal(ShapeData shapeData) {
        ...
        if (normal == null) {
            ...
            if (globe != null) normal = globe.computeSurfaceNormalAtLocation(this.getReferencePosition().getLatitude(), this.getReferencePosition().getLongitude());
        }
        return normal;
    }
    ...
    protected void generateCapInteriorIndices(ShapeData shapeData) {
        ...
        if (shapeData.capFillIndices == null || shapeData.capFillIndices.capacity() < cb.getNumIndices()) shapeData.capFillIndices = BufferUtil.newIntBuffer(cb.getNumIndices()); else shapeData.capFillIndices.clear();
        if (shapeData.capFillIndexBuffers == null || shapeData.capFillIndexBuffers.size() < cb.getPrimTypes().size()) shapeData.capFillIndexBuffers = new ArrayList<IntBuffer>(cb.getPrimTypes().size()); else shapeData.capFillIndexBuffers.clear();
    ...
    protected boolean isSameAsPreviousTerrain(Terrain terrain) {
        if (terrain == null || terrain != this.previousIntersectionTerrain) return false;
        if (terrain.getVerticalExaggeration() != this.previousIntersectionTerrain.getVerticalExaggeration()) return false;
    ...
    public List<Intersection> intersect(Line line, Terrain terrain) throws InterruptedException {
        if (!this.isEnableSides() && !this.isEnableCap()) return null;
        Position refPos = this.getReferencePosition();
        if (refPos == null) return null;
        if (!this.isOuterBoundaryValid()) return null;
        ShapeData highResShapeData = this.isSameAsPreviousTerrain(terrain) ? this.previousIntersectionShapeData : null;
        if (highResShapeData == null) {
            highResShapeData = this.createIntersectionGeometry(terrain);
            if (highResShapeData == null) return null;
            this.previousIntersectionShapeData = highResShapeData;
            this.previousIntersectionTerrain = terrain;
        }
        if (highResShapeData.getExtent() != null && highResShapeData.getExtent().intersect(line) == null) return null;
        final Line localLine = new Line(line.getOrigin().subtract3(highResShapeData.getReferencePoint()), line.getDirection());
        ...
        for (ExtrudedBoundaryInfo boundary : highResShapeData) {
            ...
            if (boundaryIntersections != null && boundaryIntersections.size() > 0) intersections.addAll(boundaryIntersections);
        }
        if (this.isEnableCap()) this.intersectCap(localLine, highResShapeData, intersections);
    ...
    protected ShapeData createIntersectionGeometry(Terrain terrain) {
        ...
        if (shapeData.getReferencePoint() == null) return null;
        this.createVertices(terrain, shapeData, false);
        if (this.isEnableSides()) this.createSideGeometry(shapeData);
        if (this.isEnableCap()) this.createCapGeometry(null, shapeData);
     ...
     * @throws InterruptedException if the operation is interrupted.
     */
    protected List<Intersection> intersectBoundarySides(Line line, ExtrudedBoundaryInfo boundary) throws InterruptedException {
        List<Intersection> intersections = new ArrayList<Intersection>();
        Vec4[] topVertices = boundary.capVertices;
        Vec4[] bottomVertices = boundary.baseVertices;
    ...
    protected void intersectCap(Line line, ShapeData shapeData, List<Intersection> intersections) throws InterruptedException {
        if (shapeData.cb.getPrimTypes() == null) return;
        for (int i = 0; i < shapeData.cb.getPrimTypes().size(); i++) {
            IntBuffer ib = shapeData.capFillIndexBuffers.get(i);
            ib.rewind();
            List<Intersection> ti = Triangle.intersectTriangleTypes(line, shapeData.capVertexBuffer, ib, shapeData.cb.getPrimTypes().get(i));
            if (ti != null && ti.size() > 0) intersections.addAll(ti);
        }
    }
    ...
    public void moveTo(Position position) {
        ...
        for (List<? extends LatLon> boundary : this.boundaries) {
            if (boundary == null || boundary.size() == 0) continue;
            List<LatLon> newList = LatLon.computeShiftedLocations(oldPosition, position, boundary);
            if (newList == null) continue;
            for (int i = 0; i < boundary.size(); i++) {
                if (boundary.get(i) instanceof Position) newList.set(i, new Position(newList.get(i), ((Position) boundary.get(i)).getAltitude()));
            }
    ...
    protected void writeKMLBoundaries(XMLStreamWriter xmlWriter) throws IOException, XMLStreamException {
        ...
        if (outerBoundary != null) {
            ...
            if (outerBoundary.iterator().hasNext() && outerBoundary.iterator().next() instanceof Position) this.exportBoundaryAsLinearRing(xmlWriter, outerBoundary); else KMLExportUtil.exportBoundaryAsLinearRing(xmlWriter, outerBoundary, getHeight());
            xmlWriter.writeEndElement();
        }
        Iterator<List<? extends LatLon>> boundaryIterator = this.boundaries.iterator();
        if (boundaryIterator.hasNext()) boundaryIterator.next();
        while (boundaryIterator.hasNext()) {
            ...
            if (boundary.iterator().hasNext() && boundary.iterator().next() instanceof Position) this.exportBoundaryAsLinearRing(xmlWriter, outerBoundary); else KMLExportUtil.exportBoundaryAsLinearRing(xmlWriter, boundary, getHeight());
            xmlWriter.writeEndElement();
        }
    }
     ...
     */
    protected void exportBoundaryAsLinearRing(XMLStreamWriter xmlWriter, Iterable<? extends LatLon> boundary) throws XMLStreamException {
        xmlWriter.writeStartElement("LinearRing");
        xmlWriter.writeStartElement("coordinates");
        for (LatLon location : boundary) {
            if (location instanceof Position) {
                xmlWriter.writeCharacters(String.format("%f,%f,%f ", location.getLongitude().getDegrees(), location.getLatitude().getDegrees(), ((Position) location).getAltitude()));
            } else {
                xmlWriter.writeCharacters(String.format("%f,%f ", location.getLongitude().getDegrees(), location.getLatitude().getDegrees()));
            }
        }
        xmlWriter.writeEndElement();
        xmlWriter.writeEndElement();
