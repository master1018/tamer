...
class Socks5Message extends ProxyMessage {
   ...
   */
    public Socks5Message(int cmd, InetAddress ip, int port) {
        ...
        data[0] = (byte) SOCKS_VERSION;
        data[1] = (byte) command;
        data[2] = (byte) 0;
        data[3] = (byte) addrType;
   ...
   */
    public Socks5Message(int cmd, String hostName, int port) {
        ...
        data[1] = (byte) command;
        data[2] = (byte) 0;
        data[3] = (byte) SOCKS_ATYP_DOMAINNAME;
   ...
   */
    ...
    public void read(InputStream in, boolean clientMode) throws SocksException, IOException {
        ...
        if (clientMode && command != 0) throw new SocksException(command);
   ...
   */
    public void write(OutputStream out) throws SocksException, IOException {
        if (data == null) {
            Socks5Message msg;
            if (addrType == SOCKS_ATYP_DOMAINNAME) msg = new Socks5Message(command, host, port); else {
                if (ip == null) {
                    try {
                        ip = InetAddress.getByName(host);
                    } catch (UnknownHostException uh_ex) {
   ...
   */
    ...
    */
    ...
    public static final int SOCKS_ATYP_IPV4 = 0x1;

    public static final int SOCKS_ATYP_DOMAINNAME = 0x3;
