...
import org.perlipse.internal.core.PerlCorePlugin;
...
public class PerlCompilerOutputParser {
    ...
    private static final Pattern compilerPattern = Pattern.compile("^(.*) at (\\S+) line (\\d+)[\\.,]");
    ...
    private IValidatorProblem createProblem(IResource resource, String message, int lineNo) {
        ...
        problem.addAttribute(PerlValidatorConstants.MARKER_ATTR_ERROR_WARN_DESC, eom.getDescription());
        return problem;
    }
    ...
    private int parseInt(String str, String line) {
        try {
            ...
            PerlCorePlugin.error("unable to parse line number from [" + line + "]", e);
            return -1;
        }
    }
}
