...
import org.simpleframework.util.select.ExecutorReactor;
...
public class TransportTest extends TestCase {
    ...
    private void testTransport(int size, int repeat) throws Exception {
        ...
        Transport transport = new SocketTransport(channel, reactor, 8192);
    ...
    private class SocketConsumer extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                scan: for (int i = 0; i < repeat; i++) {
                    ...
                    if ((byte) octet != (byte) i) {
                        throw new Exception("Wrong sequence of blocks sent, was " + (byte) octet + " should have been " + (byte) i + " count is " + count + " window is " + window + " compare " + explore(it, source, 5));
                    }
            ...
            if (count != size * repeat) {
                new Exception("Invalid number of bytes read, was " + count + " should have been " + (size * repeat)).printStackTrace();
            }
            try {
            } catch (Exception e) {
                e.printStackTrace();
