...
public class ArrayList<E> extends AbstractList<E> implements RandomAccess {
    ...
    private class MyIterator implements ListIterator<E> {
        ...
        private void checkModCount() {
            if (this.modcount != ArrayList.this.modCount) throw new ConcurrentModificationException();
        }

        private void checkLastPos() {
            if (this.lastpos < 0) throw new IllegalStateException();
        }
    ...
    public ArrayList(int initialCapacity) {
        ...
        if (initialCapacity < INITIAL_CAPACITY) initialCapacity = INITIAL_CAPACITY;
        elementCount = 0;
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        for (int j = 0; j < size; j++) elementData[index + j] = i.next();
        elementCount += size;
        return true;
    }
    ...
    public void ensureCapacity(int minCapacity) {
        ...
        if (el < minCapacity) {
            ...
            while (el < minCapacity) el = el * CAPACITY_INCREMENT_NUM / CAPACITY_INCREMENT_DEN;
    ...
    public int lastIndexOf(Object element) {
        if (element == null) {
            ...
            for (int i = elementCount - 1; i >= 0; i--) if (element.equals(elementData[i])) return i;
        }
        return -1;
    }
    ...
    protected void removeRange(int start, int end) {
        ...
        if (end > elementCount) throw new IndexOutOfBoundsException();
        ...
        for (int i = newcount; i < oldcount; i++) elementData[i] = null;
