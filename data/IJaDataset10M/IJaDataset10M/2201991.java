...
public class MusicAPI {
    ...
    MusicAPI(long id, String name, String pkg, String msg, boolean clashWithScrobbleDroid, boolean enabled) {
        super();
        if (name == null) throw new IllegalArgumentException("null music api name");
        if (pkg == null) throw new IllegalArgumentException("null music api pkg");
        this.id = id;
        this.name = name;
        this.pkg = pkg;
        this.msg = msg;
    ...
    public String toString() {
        return "MusicAPI [clashWithScrobbleDroid=" + clashWithScrobbleDroid + ", enabled=" + enabled + ", id=" + id + ", msg=" + msg + ", name=" + name + ", pkg=" + pkg + "]";
    }
	 ...
	 */
    public static MusicAPI fromReceiver(Context ctx, String name, String pkg, String msg, boolean clashWithScrobbleDroid) {
        if (name == null) throw new IllegalArgumentException("null music api name");
        if (pkg == null) throw new IllegalArgumentException("null music api pkg");
        DatabaseHelper dbHelper = new DatabaseHelper(ctx);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ...
        if (!c.isAfterLast()) {
            ...
            if ((mapi.msg == null && msg != null) || (mapi.msg != null && !mapi.msg.equals(msg))) {
                mapi.msg = msg;
                changed = true;
            }
            ...
            if (changed) {
                ...
                String update = "update music_api set " + "name = '" + mapi.name + "', " + "pkg = '" + mapi.pkg + "', " + "msg = " + msgNull + ", " + "sdclash = " + mapi.clashWithScrobbleDroid + " where _id = " + mapi.id;
            ...
            mapi = new MusicAPI(id, name, pkg, msg, clashWithScrobbleDroid, true);
    ...
    public static MusicAPI fromDatabase(Context ctx, long id) {
        ...
        if (!c.isAfterLast()) {
            ...
            mapi = new MusicAPI(-1, ctx.getString(R.string.unknown_mapi), NOT_AN_APPLICATION_PACKAGE + "pre_1_2_3", null, false, true);
        }
        c.close();
        dbHelper.close();
        return mapi;
    ...
    private static MusicAPI readMusicAPI(Cursor c) {
        return new MusicAPI(c.getInt(c.getColumnIndex("_id")), c.getString(c.getColumnIndex("name")), c.getString(c.getColumnIndex("pkg")), c.getString(c.getColumnIndex("msg")), c.getInt(c.getColumnIndex("sdclash")) == 1, c.getInt(c.getColumnIndex("enabled")) == 1);
    }
    ...
    private static final String DATABASE_CREATE_MUSIC_API = "create table music_api (" + "_id integer primary key autoincrement, " + "name text not null, " + "pkg text unique not null, " + "msg text, " + "sdclash integer not null, " + "enabled integer not null);";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            Log.w(TAG, "Upgrading music api database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
