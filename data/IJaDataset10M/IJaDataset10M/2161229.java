...
public class yard {
    ...
    static rectangle spritevisiblearea = new rectangle(0, (24 * 8) - 1, 0, (31 * 8) - 1);
        ...
        public void handler(osd_bitmap bitmap) {
            ...
            for (offs = 0x1000 - 2; offs >= 0; offs -= 2) {
                if ((dirtybuffer[offs] != 0) || (dirtybuffer[offs + 1] != 0)) {
                    ...
                    drawgfx(tmpbitmap, Machine.gfx[0], videoram.read(offs) + 4 * (videoram.read(offs + 1) & 0xc0), videoram.read(offs + 1) & 0x1f, 0, 0, sx, sy, null, TRANSPARENCY_NONE, 0);
                }
            }
            {
            ...
            for (offs = spriteram_size[0] - 4; offs >= 0; offs -= 4) {
                ...
                drawgfx(bitmap, Machine.gfx[bank], sprt, spriteram.read(offs + 1) & 0x3f, spriteram.read(offs + 1) & 0x40, spriteram.read(offs + 1) & 0x80, spriteram.read(offs + 3), 241 - spriteram.read(offs), spritevisiblearea, TRANSPARENCY_PEN, 0);
                sprt = sprt + 0x40;
                if (((spriteram.read(offs + 1) & 0xfb) != 0 && bank == 1) || (bank == 2) || (bank == 1 && sprt < 0x80)) {
                    drawgfx(bitmap, Machine.gfx[bank], sprt, spriteram.read(offs + 1) & 0x3f, spriteram.read(offs + 1) & 0x40, spriteram.read(offs + 1) & 0x80, spriteram.read(offs + 3), 241 - spriteram.read(offs) + 16, spritevisiblearea, TRANSPARENCY_PEN, 0);
                }
            }
        }
    };
