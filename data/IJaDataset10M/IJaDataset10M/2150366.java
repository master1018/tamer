...
public class TechnologyFrontier extends GameDataComponent implements Iterable<TechAdvance> {
    ...
    public void addAdvance(final List<TechAdvance> list) {
        for (final TechAdvance t : list) addAdvance(t);
    }
    ...
    public TechAdvance getAdvanceByProperty(final String property) {
        for (final TechAdvance ta : m_techs) if (ta.getProperty().equals(property)) return ta;
        return null;
    }

    public TechAdvance getAdvanceByName(final String name) {
        for (final TechAdvance ta : m_techs) if (ta.getName().equals(name)) return ta;
        return null;
    }

    public List<TechAdvance> getTechs() {
        if (m_cachedTechs == null) m_cachedTechs = Collections.unmodifiableList(m_techs);
    ...
    public boolean equals(final Object o) {
        if (o == null || !(o instanceof TechnologyFrontier)) return false;
