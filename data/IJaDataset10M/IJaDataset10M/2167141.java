...
public class ConditionedExampleSet extends AbstractExampleSet {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ConditionedExampleSet)) return false;
        ConditionedExampleSet other = (ConditionedExampleSet) o;
        if (this.mapping.length != other.mapping.length) return false;
        for (int i = 0; i < this.mapping.length; i++) if (this.mapping[i] != other.mapping[i]) return false;
        return true;
    }
    ...
    private int[] calculateMapping(Condition condition, boolean inverted) {
        ...
        for (Example example : parent) {
            if (!inverted) {
                if (condition.conditionOk(example)) indices.add(exampleCounter);
            } else {
                if (!condition.conditionOk(example)) indices.add(exampleCounter);
            }
            exampleCounter++;
    ...
    public Example getExample(int index) {
        if ((index < 0) || (index >= this.mapping.length)) throw new RuntimeException("Given index '" + index + "' does not fit the filtered ExampleSet!"); else return parent.getExample(this.mapping[index]);
    }
    ...
    public static Condition createCondition(String name, ExampleSet exampleSet, String parameterString) throws ConditionCreationException {
        ...
        try {
            ...
            if (!Condition.class.isAssignableFrom(clazz)) throw new ConditionCreationException("'" + className + "' does not implement Condition!");
