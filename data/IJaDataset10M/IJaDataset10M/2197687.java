...
public class Settings extends Activity {
    ...
    private CheckBox mRegCheck;

    private CheckBox mStunCheck;
    ...
    private void saveState() {
        ...
        String checkreg = String.valueOf(mRegCheck.isChecked());
        String checkstun = String.valueOf(mStunCheck.isChecked());
        if (mRowId == null) {
            long id = mDbHelper.createSetting(nickname, username, password, registrar, stun, checkreg, checkstun);
            if (id > 0) mRowId = id;
        } else {
            mDbHelper.updateSetting(mRowId, nickname, username, password, registrar, stun, checkreg, checkstun);
        }
    }
    ...
    private void populateFields() {
        if (mRowId != null) {
            ...
            mNicknameSetting.setText(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_NICKNAME)));
            mUsernameSetting.setText(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_USERNAME)));
            mPasswordSetting.setText(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_PASSWORD)));
            mRegistrarSetting.setText(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_REGISTRAR)));
            mStunSetting.setText(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_STUN)));
            mRegCheck.setChecked(Boolean.valueOf(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_CHECKREG))));
            mStunCheck.setChecked(Boolean.valueOf(settingCursor.getString(settingCursor.getColumnIndex(SettingsDbAdapter.KEY_CHECKSTUN))));
        }
    }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mRegCheck = (CheckBox) findViewById(R.id.checkRegister);
        mStunCheck = (CheckBox) findViewById(R.id.checkSTUN);
        Button saveButton = (Button) findViewById(R.id.saveSettings);
        mRowId = icicle != null ? icicle.getLong(SettingsDbAdapter.KEY_ROWID) : null;
        if (mRowId == null) {
