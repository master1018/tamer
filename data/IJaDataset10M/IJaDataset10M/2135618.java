package it.xtypes.example.lambda.tests;

import it.xtypes.example.lambda.lambda.Abstraction;
import it.xtypes.example.lambda.lambda.Application;
import it.xtypes.example.lambda.lambda.ArrowType;
import it.xtypes.example.lambda.lambda.IntType;
import it.xtypes.example.lambda.lambda.Parameter;
import it.xtypes.example.lambda.lambda.Program;
import it.xtypes.example.lambda.lambda.StringType;
import it.xtypes.example.lambda.lambda.Term;
import it.xtypes.example.lambda.lambda.Type;
import it.xtypes.example.lambda.lambda.Variable;
import it.xtypes.runtime.TypeSystemResult;
import it.xtypes.runtime.TypingJudgmentEnvironment;
import static it.xtypes.example.lambda.tests.util.LambdaTestsUtil.*;
...
public class LambdaTypeTest extends LambdaAbstractTests {
    ...
    public void testInferIntArrowTypeFromBody() throws Exception {
        ...
        assertApplicationTrace("./output/testInferIntArrowTypeFromBody.txt", result);
    }
    ...
    public void testTypeAbstractionWithNoType() throws Exception {
        ...
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertApplicationTrace("./output/testTypeAbstractionWithNoType.txt", result);
    ...
    public void testTypeAbstractionWithType() throws Exception {
        ...
        Term term = abstraction;
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertApplicationTrace("./output/testTypeAbstractionWithType.txt", result);
    ...
    public void testOmega() throws Exception {
        ...
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertFailureContainsStringInTrace(result, "rule UnifyVariableArrow failed: [X1 -> (TypeVariable 'X2' -> TypeVariable 'X3'), x -> TypeVariable 'X1'] |- TypeVariable 'X2' == (TypeVariable 'X2' -> TypeVariable 'X3')");
        assertFailureContainsStringInTrace(result, "NotOccurVarInArrow [X1 -> (TypeVariable 'X2' -> TypeVariable 'X3'), x -> TypeVariable 'X1'] |- TypeVariable 'X2' !- (TypeVariable 'X2' -> TypeVariable 'X3')");
    }
    ...
    public void testTypeComposeWithInferredInt() throws Exception {
        ...
        Term term = abstraction;
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertEquals("(int -> a) -> (a -> b) -> int -> b", rep(result.getValue()));
    }
    ...
    public void testTypeComposeWithInferredInt2() throws Exception {
        ...
        Term term = abstraction;
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertEquals("(a -> int) -> (int -> b) -> a -> b", rep(result.getValue()));
    }
    ...
    public void testTypeComposeWithInferredInt3() throws Exception {
        ...
        Term term = abstraction;
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertEquals("(a -> b) -> (b -> int) -> a -> int", rep(result.getValue()));
    }
    ...
    public void testTypeComposeWithInferredInt4() throws Exception {
        ...
        Term term = abstraction;
        TypeSystemResult<Type> result = typeSystem.typeAsType(term);
        assertEquals("(a -> b) -> (int -> a) -> b", rep(result.getValue()));
    }
