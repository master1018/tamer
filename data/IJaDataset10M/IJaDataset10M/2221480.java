...
public class BidPrice extends LabelProvider implements Comparator {
    ...
    public String getText(Object element) {
        if (element instanceof WatchlistItem) {
            ...
            if (quote != null && quote.getBid() != 0) return formatter.format(CurrencyConverter.getInstance().convert(quote.getBid(), item.getSecurity().getCurrency(), item.getParent().getCurrency()));
        }
        return "";
    }
    ...
    private double getValue(WatchlistItem item) {
        ...
        Quote quote = item.getSecurity().getQuote();
        if (quote != null && quote.getBid() != 0) return CurrencyConverter.getInstance().convert(quote.getBid(), item.getSecurity().getCurrency(), item.getParent().getCurrency());
        return 0;
    }
}
