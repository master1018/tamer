...
public class SpecificTypeChangeAction implements ClickMenuAction {
    ...
    protected static final Logger logger = Logger.getLogger(SpecificTypeChangeAction.class);
    ...
    public HistoryItem execute() {
        ...
        for (TreePath path : sources.getPaths()) {
            ...
            TreePath outpath = path.getParentPath().pathByAddingChild(new OBORestrictionImpl(tr.getChild(), tr.getParent(), type));
        ...
        if (historyList.size() == 1) {
            ...
            item = new TermMacroHistoryItem("Changed relationship types to " + type);
            for (int i = 0; i < historyList.size(); i++) {
                ((TermMacroHistoryItem) item).addItem(historyList.get(i));
            }
        }
        TreePath[] outpatharr = new TreePath[pathouts.size()];
        for (int i = 0; i < outpatharr.length; i++) outpatharr[i] = pathouts.get(i);
        GUIUtil.setSelections(item, sources, SelectionManager.createSelectionFromPaths(null, outpatharr, null, SessionManager.getManager().getCurrentLinkDatabase(), RootAlgorithm.GREEDY, true));
        return item;
    }
}
