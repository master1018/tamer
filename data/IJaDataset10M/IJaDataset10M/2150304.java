...
public class ConstTimeRunner extends AbstractRunner {
	 ...
	 */
    private static volatile boolean _finishFlag = false;
    ...
    public static ConstTimeRunner getRunner(String className, int nthread) {
        System.out.println(MessageFormat.format("Requesting const-time runner with className: {0}, " + "thread number: {1}, operation time {2} seconds.", className, nthread, AbstractBaseTest.getOperationTime()));
        ...
        if (runner.executor == null || runner.nthread != nthread) {
            ...
            if (runner.executor == null) {
                runner.executor = (ThreadPoolExecutor) Executors.newFixedThreadPool(nthread);
    ...
    private ConstTimeRunner(Class<?> setClass, int nthread, int opCount) {
        if (setClass != null) this.className = setClass.getCanonicalName(); else className = "";
    ...
    public void runThreads(final Runnable[] tasks, String testName) throws InterruptedException, ExecutionException {
        System.gc();
        resetOpCount();
        ...
        Loggers.performance(className, nthread, getOpCount(), testName, (end - start));
    }
