...
public class Verification extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (userRequest.equals("Save My Information")) {
                ...
                if (mergeAuthCodeSent(user, request)) out.println(Home.getHeader(user) + personalInfoForm(user, false, request) + Home.footer); else out.println("Sorry, the authorization code could not be sent. Please try again later.");
    ...
    String verifiedEmail(boolean verified) {
        ...
        if (verified) buf.append("<h3>Email Verification Complete</h3>" + "Thank you for verifying your email address with ChemVantage.<p>" + "<a href=http://www.chemvantage.org>Go to the ChemVantage login page now</a>."); else buf.append("<h3>Verification Failed</h3>Sorry, this action was unsuccessful. " + "Please be sure that the complete URL was included in the request by copying " + "and pasting it into the URL address bar in your browser. If you need assistance, " + "send email to admin@chemvantage.org");
        return buf.toString();
    }
    ...
    String personalInfoForm(User user, boolean verificationEmailSent, HttpServletRequest request) {
        ...
        try {
            ...
            if (userService.isUserAdmin()) throw new Exception();
            String em = "";
        ...
        try {
            buf.append("<h2>Your ChemVantage Account Profile</h2>" + "ChemVantage protects your personal information. For details, see our <a href=/w3c/privacy.html>Privacy Policy</a>.<br>" + "In order for ChemVantage to function properly as a learning resource, we need to associate your name and email address with your account.<br>" + "This is important for protecting <i>you</i> by making it difficult for someone else to impersonate you or tamper with your account.<p>");
            ...
            if (!emailRequired && !user.verifiedEmail) {
                ...
                if (verificationEmailSent) buf.append("<br><span style='color:red'>A verification email has been sent to your address.</span><br>"); else if (!nameRequired) buf.append("<INPUT TYPE=SUBMIT NAME=UserRequest VALUE='Verify My Email Address'><br>");
            }
            buf.append("</TD></TR>");
            ...
            if (nameRequired || emailRequired) {
                buf.append("<TR><TD><INPUT TYPE=SUBMIT NAME=UserRequest VALUE='Save My Information'></TD>" + "<TD>" + (user.requiresUpdates() ? "<span style=color:red>* <span style=font-size:smaller>required field</span></span>" : "") + "</TD></TR></TABLE></FORM>");
            } else {
                buf.append("</FORM>");
                boolean eligibleToJoin = eligibleToJoin(user);
                if (user.myGroupId <= 0L && eligibleToJoin) {
                    ...
                    if (user.domain == null || user.domain.isEmpty()) allGroups = ofy.query(Group.class).list(); else allGroups = ofy.query(Group.class).filter("domain", user.domain).list();
                    for (Group g : allGroups) if (!g.isActive()) allGroups.remove(g);
                    if (allGroups.size() == 0) {
                        buf.append("No groups have been created in this domain yet.");
                    } else {
                        buf.append("<SELECT NAME=GroupId " + "onChange=\"if(confirm('Are you sure that you  want to join this group? " + "This action can only be reversed by an instructor.'))document.JoinGroup.submit();" + "else document.JoinGroup.GroupId[0].selected=true;\">" + (groupRequired ? "<OPTION VALUE=-1>Please select a ChemVantage group to join:</OPTION>\n" : "") + "<OPTION VALUE=0>I'm not a member of any group or class using ChemVantage</OPTION>\n");
                    ...
                    if (groupRequired) {
                        ...
                        if (allGroups.size() > 0) buf.append("<TR><TD COLSPAN=2><span id=instructions style='color:red'><br>" + "Please select a ChemVantage group. This will give you access to assignments and deadlines.<br>" + "It will also give your instructor and teaching assistant access to your scores.</span></TD></TR>"); else if (user.isInstructor()) buf.append("<TR><TD COLSPAN=2>" + "<span id=instructions style='color:red'><br>Use the 'Instructor' link at the top " + "of the page to create a new ChemVantage group for your chemistry class.</span></TD></TR>");
                    }
                    ...
                    buf.append("<TABLE>" + "<TR><TD ALIGN=CENTER><b>Instant ChemVantage Premium Account Upgrade</b></TD></TR>" + "<TR><TD ALIGN=CENTER><b>$4.99 USD</b></TD></TR><TR><TD ALIGN=CENTER> " + "<form action=https://www.paypal.com/cgi-bin/webscr method=post>" + "<input type=hidden name=cmd value=_s-xclick>" + "<input type=hidden name=hosted_button_id value=" + (user.authDomain.equals("BLTI") ? "U58TNLE8YE4AW" : "HKW9475B55NJU") + ">" + "<input type=hidden name=on0 value=userId><input type=hidden name=os0 value=" + user.id + ">" + "<input type=image src=https://www.paypalobjects.com/en_US/i/btn/btn_buynowCC_LG.gif border=0 name=submit alt='PayPal online payment'>" + "<br><font size=-2>Your payment will be processed by PayPal.com</font>" + "<img alt='' border=0 src=https://www.paypalobjects.com/en_US/i/scr/pixel.gif width=1 height=1>" + "</form></TD>");
                    if (user.myGroupId < 0) buf.append("<TD align=center><form action=Verification method=post><input type=hidden name=GroupId value=0>" + "<input type=hidden name=UserRequest value=JoinGroup>" + "<input type=submit value='No Thanks' style='font-weight:bold;color:white;background-color:red'>" + "<br><font size=-2>You may return to this page later to<br>upgrade your account and join a group.</font></TD>");
                    buf.append("</TR></TABLE>");
                    buf.append("</TD></TR>");
                }
                buf.append("</TABLE>\n");
                buf.append("<h3>Any Corrections Needed?</h3>" + "If your name and/or email shown above is not correct, please send a message to " + "<a href=mailto:admin@chemvantage.org>admin@chemvantage.org</a><br>giving detailed " + "instructions for any changes that are needed.<p>");
                if (!user.requiresUpdates()) {
                    buf.append("<style type='text/css'>a.nav, a.nav:link, a.nav:visited {display:block; width:250px; height:35px; " + "background:red; border:1px solid #000; margin-top:2px; text-align:center; text-decoration:none; " + "font-family:verdana, arial, sans-serif; font-size:15px; color:white; line-height:35px; overflow:hidden;}" + "a.nav:hover {color:#fff; background:#800;}</style>");
            ...
            buf.append("<p><a href=# style='font-size:smaller' onClick=\"javascript: document.getElementById('multi').style.display=''\">I can't find my stuff</a>" + "<div id='multi'" + (showDuplicateAccounts ? ">" : " style='display:none'>"));
            buf.append("<h3>Do You Have Multiple ChemVantage Accounts?</h3>" + "This is fairly common because there are multiple ways of creating ChemVantage accounts.<br>" + "If you think you have more than one account and you don't see options below for merging<br>" + "them below, first check each account to make sure that the email address has been verified<br>" + "by ChemVantage. If that doesn't work, send a detailed account merge request to " + "<a href=mailto:admin@chemvantage.org>admin@chemvantage.org</a>.<p>");
            if (user.verifiedEmail && !user.requiresUpdates()) {
                ...
                for (User u : duplicateEmails) {
                    ...
                    buf.append("<TR><TD>" + u.getFullName() + " (" + u.email + ") - authorization domain=" + consumerKey + "</TD>" + "<TD><FORM NAME=DuplicateEmail" + i + " ACTION=Verification METHOD=POST>" + "<INPUT TYPE=HIDDEN NAME=FromAccount VALUE='" + u.id + "'>" + "<INPUT TYPE=HIDDEN NAME=ToAccount VALUE='" + user.id + "'>" + "<INPUT TYPE=HIDDEN NAME=Code VALUE='" + code + "'>" + "<INPUT TYPE=SUBMIT NAME=UserRequest VALUE='Merge This Account With Mine' " + "onClick=\"javascript: document.DuplicateEmail" + i + ".UserRequest.style='display:none';return confirm('Are you sure? This action cannot be undone.')\">" + "</FORM></TD></TR>");
                }
                ...
                for (User u : duplicateNames) {
                    ...
                    String consumerKey = u.authDomain.equals("BLTI") ? u.id.substring(0, u.id.indexOf(":")) : u.authDomain;
                    buf.append("<TR><TD>" + u.getFullName() + " (" + u.email + ") - authorization domain=" + consumerKey + "</TD>" + "<TD><FORM NAME=DuplicateName" + i + " ACTION=Verification METHOD=POST>" + "<INPUT TYPE=HIDDEN NAME=FromAccount VALUE='" + u.id + "'>" + "<INPUT TYPE=HIDDEN NAME=ToAccount VALUE='" + user.id + "'>");
                    if ("Get Authorization Code".equals(request.getParameter("UserRequest")) && u.id.equals(request.getParameter("FromAccount"))) {
                        buf.append("</TD></TR><TR><TD COLSPAN=2>" + "<span style=color:red>An email has been sent to " + u.email + " with an authorization code. Please enter it here: </span>" + "<INPUT TYPE=TEXT NAME=Code><INPUT TYPE=SUBMIT NAME=UserRequest VALUE='Merge This Account With Mine'>");
                    } else {
