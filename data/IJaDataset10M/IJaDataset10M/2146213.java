...
public abstract class LContainer extends LComponent {
    ...
    public void validatePosition() {
        ...
        if (!this.elastic) {
            for (int i = 0; i < this.childCount; i++) {
                if (this.childs[i].getX() > this.getWidth() || this.childs[i].getY() > this.getHeight() || this.childs[i].getX() + this.childs[i].getWidth() < 0 || this.childs[i].getY() + this.childs[i].getHeight() < 0) {
                    this.elastic = true;
                    break;
                }
            }
