...
public class GroupRequest extends Request {
    ...
    public void receiveResponse(Object response_value, Address sender) {
        ...
        if (!rsp.wasReceived()) {
            if ((responseReceived = (rsp_filter == null) || rsp_filter.isAcceptable(response_value, sender))) rsp.setValue(response_value);
        ...
        try {
            ...
            if (responseReceived || done) completed.signalAll();
    ...
    private void setTargets(Collection<Address> mbrs) {
        for (Address mbr : mbrs) requests.put(mbr, new Rsp(mbr));
    ...
    protected boolean responsesComplete() {
        ...
        switch(options.getMode()) {
            case GET_FIRST:
                if (num_received > 0) return true;
                if (num_suspected >= num_total) return true;
                break;
            case GET_ALL:
                return num_received + num_suspected >= num_total;
            case GET_MAJORITY:
                int majority = determineMajority(num_total);
                if (num_received + num_suspected >= majority) return true;
                break;
            case GET_ABS_MAJORITY:
                majority = determineMajority(num_total);
                if (num_received >= majority) return true;
                break;
            case GET_N:
                return true;
            case GET_NONE:
