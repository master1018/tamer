...
final class ARBDebugOutputUtil {
    ...
    public static void registerHandler(final Handler handler) {
        ...
        if (ctx == null) throw new IllegalStateException("No context is current.");
        if (!ctx.getContextAttribs().isDebug()) throw new IllegalStateException("The current context is not a debug context.");
        if (!GLContext.getCapabilities().GL_ARB_debug_output) throw new IllegalStateException("ARB_debug_output is not supported.");
    ...
    private static void messageCallback(final int source, final int type, final int id, final int severity, final String message, final ByteBuffer userParam) {
        synchronized (GlobalLock.lock) {
            ...
            if (handler != null) handler.handleMessage(source, type, id, severity, message);
        }
    }
}
