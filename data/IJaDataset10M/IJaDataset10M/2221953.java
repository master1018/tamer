...
public class BinaryDelaunay implements BinaryObject {
    ...
    public static final int computeBytes(Delaunay d) {
        ...
        return (1 + 1 + BinaryIntegerMatrix.computeBytes(d.Tri) + 1 + BinaryIntegerMatrix.computeBytes(d.Vertices) + 1 + BinaryIntegerMatrix.computeBytes(d.Walk) + 1 + BinaryIntegerMatrix.computeBytes(d.Edges) + 6);
    }
    ...
    public static final void write(BinaryWriter writer, Delaunay delaunay, Object token) throws IOException {
        if (!isKnownClass(delaunay)) {
            BinarySerializedObject.write(writer, FLD_DELAUNAY_SERIAL, delaunay, token);
            return;
        }
