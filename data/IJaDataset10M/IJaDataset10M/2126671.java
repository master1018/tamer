...
public class PointToolbar extends Canvas implements MouseListener {
	 ...
	 */
    public PointToolbar(final Toolbar previousToolbar, final MainDialog dialog) {
    ...
    public void mousePressed(final MouseEvent e) {
        ...
        boolean doubleClick = ((newTool == getCurrentTool()) && ((System.currentTimeMillis() - mouseDownTime) <= 500L) && (newTool == PointAction.REMOVE_CROSS));
        mouseDownTime = System.currentTimeMillis();
        if (newTool == PointAction.STOP && !dialog.isFinalActionLaunched()) return;
        if (newTool != PointAction.STOP && dialog.isFinalActionLaunched()) return;
        setTool(newTool);
        if (doubleClick) {
        ...
        switch(newTool) {
            case PointAction.FILE:
                IODialog fileDialog = new IODialog(IJ.getInstance(), sourceImp, targetImp, sourcePh, targetPh, dialog);
	 ...
	 */
    public void setSource(final ImagePlus sourceImp, final PointHandler sourcePh) {
	 ...
	 */
    public void setTarget(final ImagePlus targetImp, final PointHandler targetPh) {
    ...
    private void drawButton(final Graphics g, final int tool) {
        ...
        if (tool == PointAction.STOP && !dialog.isFinalActionLaunched()) return;
        if (tool != PointAction.STOP && dialog.isFinalActionLaunched()) return;
