...
public class FastMap<K, V> implements Map<K, V>, Reusable, XMLSerializable, Realtime {
    ...
    private static final int C0 = 1 << B0;

    private static final int B1 = 10;

    private static final int C1 = 1 << B1;

    private static final int B2 = B1 - B0;

    private static final int C2 = 1 << B2;
    ...
    private final Entry getEntry(Object key, int keyHash) {
        ...
        for (int i = keyHash >> map._keyShift; ; i++) {
            ...
            if ((key == entry._key) || ((keyHash == entry._keyHash) && (_isDirectKeyComparator ? key.equals(entry._key) : _keyComparator.areEqual(key, entry._key)))) {
                return entry;
            }
        }
    }
    ...
    public final V put(K key, V value) {
        return (V) put(key, value, _isDirectKeyComparator ? key.hashCode() : _keyComparator.hashCodeOf(key), _isShared, false, false);
    }
    ...
    private final Object put(Object key, Object value, int keyHash, boolean concurrent, boolean noReplace, boolean returnEntry) {
        ...
        for (int i = keyHash >> map._keyShift; ; i++) {
            ...
            if (entry == null) {
                ...
                slot = slot < 0 ? i & mask : slot;
            } else if ((key == entry._key) || ((keyHash == entry._keyHash) && (_isDirectKeyComparator ? key.equals(entry._key) : _keyComparator.areEqual(key, entry._key)))) {
                if (noReplace) {
        ...
        if (!_isShared) {
            ...
            entry._keyHash = keyHash;
            entry._next = _tail;
            entry._previous = _tail._previous;
            entries[slot] = entry;
            map._entryCount += ONE_VOLATILE;
    ...
    private void resizeTable(final boolean isShared) {
        MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
            ...
            public void run() {
                ...
                _useSubMaps = ONE_VOLATILE == 1 ? true : false;
            }
        });
    }
    ...
    public final Entry<K, V> putEntry(K key, V value) {
        return (Entry<K, V>) put(key, value, _isDirectKeyComparator ? key.hashCode() : _keyComparator.hashCodeOf(key), _isShared, false, true);
    }
    ...
    public final V putIfAbsent(K key, V value) {
        return (V) put(key, value, _isDirectKeyComparator ? key.hashCode() : _keyComparator.hashCodeOf(key), _isShared, true, false);
    }
