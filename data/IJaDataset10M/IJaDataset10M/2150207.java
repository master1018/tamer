...
public class BooleanSwitch extends CalSwitch<Boolean> {
    ...
    public Boolean caseExpressionElsif(ExpressionElsif expression) {
        return doSwitch(expression.getCondition()) || doSwitch(expression.getThen());
    }
    ...
    public Boolean caseExpressionIf(ExpressionIf expression) {
        if (doSwitch(expression.getCondition()) || doSwitch(expression.getThen())) {
            return true;
        }
    ...
    public Boolean caseFunction(Function function) {
        ...
        return doSwitch(function.getType()) || doSwitch(function.getExpression());
    }

    @Override
    public Boolean caseGenerator(Generator generator) {
        return (doSwitch(generator.getVariable()) || doSwitch(generator.getLower()) || doSwitch(generator.getHigher()));
    }
    ...
    public Boolean caseStatementForeach(StatementForeach foreach) {
        if (doSwitch(foreach.getVariable()) || doSwitch(foreach.getLower()) || doSwitch(foreach.getHigher())) {
            return true;
        }
