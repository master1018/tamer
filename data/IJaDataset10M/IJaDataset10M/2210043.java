...
public class ObjectResultController {
    ...
    private void removeOldErrorPanel(WaitPanel currentWaitPanel) {
        for (int i = 0; i < _objectResultTabbedPane.getTabCount(); i++) {
            if (currentWaitPanel != _objectResultTabbedPane.getComponentAt(i) && _objectResultTabbedPane.getComponentAt(i) instanceof WaitPanel && ((WaitPanel) _objectResultTabbedPane.getComponentAt(i)).isDisplayingError()) {
                _objectResultTabbedPane.removeTabAt(i);
                break;
            }
        }
