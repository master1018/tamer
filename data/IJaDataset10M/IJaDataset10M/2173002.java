...
public class SimpleReg extends SmartHttpServlet {
    ...
    **/
    ...
    public void doAction() throws Exception {
        ...
        if (mPattern != null) {
            if (mContent != null) {
                if (igpp.util.Text.isMatch(mContent, "HTML")) {
                    String content = streamToString(igpp.util.Text.concatPath(basePath, mPattern), false);
                    igpp.xml.Transform.perform(new StringReader(content), mXSLPathName, mOut);
                } else {
        ...
        if (mPattern != null) {
            ...
            for (String name : matches) {
                ...
                if (test.isDirectory()) {
                    mOut.println("<node pattern=\"" + name.substring(basePath.length()) + "\"" + " id=\"" + name.substring(basePath.length()) + "\"" + " text=\"" + igpp.util.Text.getFileBase(test.getName()) + "\"" + " name=\"" + igpp.util.Text.getFileBase(test.getName()) + "\"" + " />");
                } else {
                    mOut.println("<leaf pattern=\"" + name.substring(basePath.length()) + "\"" + " id=\"" + name.substring(basePath.length()) + "\"" + " text=\"" + igpp.util.Text.getFileBase(test.getName()) + "\"" + " name=\"" + igpp.util.Text.getFileBase(test.getName()) + "\"" + " />");
                }
            }
            mOut.println("</Response>");
        } else {
	 ...
	 **/
    public ArrayList<String> scanPath(String path, String pattern) throws Exception {
        ArrayList<String> matches = new ArrayList<String>();
        ...
        if (endPath.startsWith("//")) endPath = endPath.substring(2);
	 ...
	 * @return an {@link ArrayList} of {@link String} values of paths to the files and folders found.
	 **/
    public ArrayList<String> search(String path, ArrayList<String> words, String identifier) throws Exception {
        ArrayList<String> matches = new ArrayList<String>();
        if (path == null) return matches;
	 ...
	 **/
    public boolean scan(String path, ArrayList<String> words, String identifier) throws Exception {
        boolean match = false;
        boolean done = false;
        String buffer;
        ArrayList<String> foundWords = new ArrayList<String>();
        for (String word : words) {
            if (word.compareTo("*") == 0) return true;
        }
        File file = new File(path);
        BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file)));
        while ((buffer = reader.readLine()) != null) {
            if (identifier != null) {
                if (buffer.indexOf(identifier) != -1) {
                    match = true;
                    break;
                }
            }
            ...
            for (String word : words) {
                ...
                for (String temp : snip) {
                    if (temp.equals(word)) {
                        match = true;
                        if (foundWords.indexOf(word) == -1) foundWords.add(word);
                    }
                    if (match && !mAllWords) {
                        done = true;
                        break;
	 ...
	 **/
    public void stream(String pathname) throws Exception {
	 ...
	 **/
    public void stream(String pathname, boolean stripVersion) throws Exception {
        boolean on = false;
        String buffer;
        ...
        try {
            ...
            while ((buffer = reader.readLine()) != null) {
                if (buffer.indexOf("<Version>") != -1 && stripVersion) continue;
                if (on) {
                    if (buffer.indexOf("</Spase>") != -1) on = false; else mOut.println(buffer);
                }
	 ...
	 **/
    public String streamToString(String pathname, boolean stripVersion) throws Exception {
        boolean on = false;
        String buffer;
        BufferedReader reader = null;
    ...
    private void setXSLPathName(String value) {
        mXSLPathName = value;
    }

    public void setWords(String value) throws Exception {
        if (igpp.util.Text.isEmpty(value)) return;
        if (value.compareTo("*") == 0) {
            mWords.add(value);
            return;
        }
        String[] words = value.split("[\\p{Punct}\\p{Blank}]");
        for (String word : words) {
            word = word.trim();
            if (word.length() > 0) mWords.add(word.toUpperCase());
        }
    }
    ...
    public void setIdentifier(String value) {
        if (igpp.util.Text.isEmpty(value)) return;
        mIdentifier = "<ResourceID>" + value + "</ResourceID>";
    }
    ...
    public void setAllWords(boolean value) {
        mAllWords = value;
    }

    public void setAllWords(String value) throws Exception {
        if (igpp.util.Text.isEmpty(value)) return;
        mAllWords = igpp.util.Text.isTrue(value);
    }
    ...
    public void setCheck(String value) throws Exception {
        if (igpp.util.Text.isEmpty(value)) return;
