...
public abstract class CRUDOperationProvider {
    ...
    public void updateSet(Transaction t, Pointer base, FieldDefinition fi, Object val) {
        if (!fi.getType().equals("set") && !fi.getType().equals("setintEnum") && !fi.getType().equals("setcharEnum")) throw new InvalidFieldTypeException(fi, "set");
        deleteSet(t, base, fi);
        if (val == null || val == Pointer.NullSet || ((Vector) val).size() == 0) return;
     ...
     */
    public abstract void checkInsert(Transaction t, String type, Dictionary fieldsToCheck, Dictionary fieldsToIgnore, Dictionary allFields);
     ...
     *            the entire data to be inserted
     */
    public abstract void checkUpdate(Transaction t, String type, Pointer pointer, Dictionary fieldsToCheck, Dictionary fieldsToIgnore, Dictionary allFields);
