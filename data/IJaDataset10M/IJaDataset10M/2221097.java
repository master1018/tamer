...
public class ASPGenerateLevel extends SvrProcess {
    ...
    protected void prepare() {
        ...
        for (int i = 0; i < para.length; i++) {
            ...
            if (para[i].getParameter() == null) ; else if (name.equals("ASP_Status")) p_ASP_Status = (String) para[i].getParameter(); else if (name.equals("AD_Menu_ID")) p_AD_Menu_ID = para[i].getParameterAsInt(); else if (name.equals("IsGenerateFields")) p_IsGenerateFields = para[i].getParameter().equals("Y"); else log.log(Level.SEVERE, "Unknown Parameter: " + name);
        }
    ...
    protected String doIt() throws Exception {
        ...
        MTreeNode node;
        if (p_AD_Menu_ID > 0) node = thisTree.getRoot().findNode(p_AD_Menu_ID); else node = thisTree.getRoot();
        if (node != null && node.isSummary()) {
            Enumeration en = node.preorderEnumeration();
            while (en.hasMoreElements()) {
                MTreeNode nn = (MTreeNode) en.nextElement();
                if (!nn.isSummary()) addNodeToLevel(nn);
            }
        }
        if (noWindows > 0) addLog("Window " + noWindows);
        if (noTabs > 0) addLog("Tab " + noTabs);
        if (noFields > 0) addLog("Field " + noFields);
        if (noProcesses > 0) addLog("Process " + noProcesses);
        if (noParameters > 0) addLog("Process Parameter " + noParameters);
        if (noForms > 0) addLog("Form " + noForms);
        if (noTasks > 0) addLog("Task " + noTasks);
        if (noWorkflows > 0) addLog("Workflow " + noWorkflows);
        return "@OK@";
    }
    ...
    private void addNodeToLevel(MTreeNode nn) {
        ...
        if (menu.getAction().equals(MMenu.ACTION_Window)) {
            ...
            if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Window WHERE ASP_Level_ID = ? AND AD_Window_ID = ?", p_ASP_Level_ID, window.getAD_Window_ID()) < 1) {
                ...
                if (aspWindow.save()) noWindows++;
            }
            MTab[] tabs = window.getTabs(true, get_TrxName());
            for (int it = 0; it < tabs.length; it++) {
                if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Tab WHERE ASP_Level_ID = ? AND AD_Tab_ID = ?", p_ASP_Level_ID, tabs[it].getAD_Tab_ID()) < 1) {
                    ...
                    if (aspTab.save()) noTabs++;
                    if (p_IsGenerateFields) {
                        MField[] fields = tabs[it].getFields(true, get_TrxName());
                        for (int ifi = 0; ifi < fields.length; ifi++) {
                            if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Field WHERE ASP_Level_ID = ? AND AD_Field_ID = ?", p_ASP_Level_ID, fields[ifi].getAD_Field_ID()) < 1) {
            ...
            if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Process WHERE ASP_Level_ID = ? AND AD_Process_ID = ?", p_ASP_Level_ID, process.getAD_Process_ID()) < 1) {
                ...
                if (aspProcess.save()) noProcesses++;
            }
            MProcessPara[] processparas = process.getParameters();
            for (int ipp = 0; ipp < processparas.length; ipp++) {
                if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Process_Para WHERE ASP_Level_ID = ? AND AD_Process_Para_ID = ?", p_ASP_Level_ID, processparas[ipp].getAD_Process_Para_ID()) < 1) {
                    ...
                    if (aspProcess_Para.save()) noParameters++;
                }
            }
        } else if (menu.getAction().equals(MMenu.ACTION_Form)) {
            MForm form = new MForm(getCtx(), menu.getAD_Form_ID(), get_TrxName());
            if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Form WHERE ASP_Level_ID = ? AND AD_Form_ID = ?", p_ASP_Level_ID, form.getAD_Form_ID()) < 1) {
                ...
                if (aspForm.save()) noForms++;
            }
        } else if (menu.getAction().equals(MMenu.ACTION_Task)) {
            MTask task = new MTask(getCtx(), menu.getAD_Task_ID(), get_TrxName());
            if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Task WHERE ASP_Level_ID = ? AND AD_Task_ID = ?", p_ASP_Level_ID, task.getAD_Task_ID()) < 1) {
            ...
            if (DB.getSQLValue(get_TrxName(), "SELECT COUNT(*) FROM ASP_Workflow WHERE ASP_Level_ID = ? AND AD_Workflow_ID = ?", p_ASP_Level_ID, workflow.getAD_Workflow_ID()) < 1) {
                ...
                if (aspWorkflow.save()) noWorkflows++;
            }
        }
    }
}
