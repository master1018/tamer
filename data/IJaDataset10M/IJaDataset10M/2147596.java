...
public class PharmacophoreMatcher {
    ...
    public boolean matches(IAtomContainer atomContainer, boolean initializeTarget) throws CDKException {
        ...
        if (!checkQuery(pharmacophoreQuery)) throw new CDKException("A problem in the query. Make sure all pharmacophore groups of the same symbol have the same same SMARTS");
        String title = (String) atomContainer.getProperty(CDKConstants.TITLE);
        if (initializeTarget) pharmacophoreMolecule = getPharmacophoreMolecule(atomContainer); else {
    ...
    private IAtomContainer getPharmacophoreMolecule(IAtomContainer atomContainer) throws CDKException {
        ...
        for (IAtom atom : pharmacophoreQuery.atoms()) {
            ...
            if (!map.containsKey(qatom.getSymbol())) map.put(qatom.getSymbol(), smarts); else if (map.get(qatom.getSymbol()).equals(smarts)) {
                continue;
            }
    ...
    private boolean checkQuery(IQueryAtomContainer query) {
        ...
        for (int i = 0; i < query.getAtomCount(); i++) {
            ...
            if (!map.containsKey(label)) map.put(label, smarts); else {
