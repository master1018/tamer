 ...
 *
 *
 * @version $Revision: 317342 $ $Date: 2001-07-11 17:40:10 +0200 (Mi, 11 Jul 2001) $
...
public abstract class BaseMarkupSerializer implements ContentHandler, DocumentHandler, LexicalHandler, DTDHandler, DeclHandler, DOMSerializer, Serializer {
    ...
    public void setOutputByteStream(OutputStream output) {
        if (output == null) throw new NullPointerException("SER001 Argument 'output' is null.");
        _output = output;
        _writer = null;
        reset();
    }
    ...
    public void setOutputCharStream(Writer writer) {
        if (writer == null) throw new NullPointerException("SER001 Argument 'writer' is null.");
        _writer = writer;
        _output = null;
        reset();
    }
    ...
    public void setOutputFormat(OutputFormat format) {
        if (format == null) throw new NullPointerException("SER001 Argument 'format' is null.");
        _format = format;
        reset();
    }

    public boolean reset() {
        if (_elementStateCount > 1) throw new IllegalStateException("Serializer reset in the middle of serialization");
        _prepared = false;
        return true;
    }
    ...
    protected void prepare() throws IOException {
        if (_prepared) return;
        if (_writer == null && _output == null) throw new IOException("SER002 No writer supplied for serializer");
    ...
    public void characters(char[] chars, int start, int length) throws SAXException {
        ...
        try {
            ...
            if (state.inCData || state.doCData) {
                ...
                if (length > 0) printText(chars, start, length, true, true);
    ...
    public void processingInstructionIO(String target, String code) throws IOException {
        ...
        if (index >= 0) buffer.append("<?").append(target.substring(0, index)); else buffer.append("<?").append(target);
        if (code != null) {
            buffer.append(' ');
            index = code.indexOf("?>");
            if (index >= 0) buffer.append(code.substring(0, index)); else buffer.append(code);
        }
        buffer.append("?>");
        if (isDocumentState()) {
    ...
    public void comment(String text) throws IOException {
        ...
        buffer = new StringBuffer(40);
        index = text.indexOf("-->");
        if (index >= 0) buffer.append("<!--").append(text.substring(0, index)).append("-->"); else buffer.append("<!--").append(text).append("-->");
        if (isDocumentState()) {
            if (_preRoot == null) _preRoot = new Vector();
            _preRoot.addElement(buffer.toString());
        } else {
            ...
            case Node.TEXT_NODE:
                {
                    ...
                    if (text != null) if (!_indenting || getElementState().preserveSpace || (text.replace('\n', ' ').trim().length() != 0)) characters(text);
                    break;
                }
            ...
            case Node.DOCUMENT_NODE:
                {
                    ...
                    if (docType != null) {
                        ...
                        try {
                            ...
                            if (internal != null && internal.length() > 0) _printer.printText(internal);
                            endDTD();
    ...
    protected final void printText(char[] chars, int start, int length, boolean preserveSpace, boolean unescaped) throws IOException {
        ...
        if (preserveSpace) {
            while (length-- > 0) {
                ch = chars[start];
                ++start;
                if (ch == '\n' || ch == '\r' || unescaped) _printer.printText(ch); else printEscaped(ch);
            }
        } else {
            while (length-- > 0) {
                ch = chars[start];
    ...
    protected void printEscaped(int ch) throws IOException {
        ...
        if (charRef != null) {
            ...
            _printer.printText(';');
        } else if ((ch >= ' ' && _encodingInfo.isPrintable(ch) && ch != 0xF7) || ch == '\n' || ch == '\r' || ch == '\t') {
            if (ch < 0x10000) {
    ...
    protected ElementState leaveElementState() {
        if (_elementStateCount > 0) {
            ...
            --_elementStateCount;
            return _elementStates[_elementStateCount];
        } else throw new IllegalStateException("Internal error: element state is zero");
    }
    ...
    protected String getPrefix(String namespaceURI) {
        ...
        if (_elementStateCount == 0) return null; else {
