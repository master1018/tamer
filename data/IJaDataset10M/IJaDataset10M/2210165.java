...
public class WSDAIRAccessServiceClient {
    ...
    private static Logger logger = Logger.getLogger(WSDAIRAccessServiceClient.class.getName());
    ...
    private static final String WSDAIR_NAMESPACE = "http://www.ggf.org/namespaces/2005/12/WS-DAIR";

    static final QName SERVICE_NAME = new QName(WSDAIR_NAMESPACE, "SQLAccessService");
	 ...
	 * 	* resolve 
	 */
    static final QName RESOURCE_LIST_PORT_NAME = new QName(WSDAIR_NAMESPACE, "AccessServiceCoreResourceListPT");
	 ...
	 * 	* genericQuery
	 */
    static final QName CORE_ACCESS_PORT_NAME = new QName(WSDAIR_NAMESPACE, "AccessServiceCoreDataAccessPT");
	 ...
	 * 	* SQLExecute
	 */
    static final QName SQL_ACCESS_PORT_NAME = new QName(WSDAIR_NAMESPACE, "AccessServiceAccessPT");
    ...
    public WSDAIRAccessServiceClient(String url) throws MalformedURLException {
        if (logger.isDebugEnabled()) logger.debug("ENTER WSDAIRAccessServiceClient() with URL " + url);
        SERVICE_BASE = url;
        _wsdairCorePT = createCorePT();
        _wsdairCoreAccessPT = createCoreAccessPT();
        _wsdairAccessPT = createSQLAccessPT();
        if (logger.isDebugEnabled()) logger.debug("RETURN WSDAIRAccessServiceClient()");
    }

    private CoreResourceListPT createCorePT() throws MalformedURLException {
        if (logger.isTraceEnabled()) {
            logger.trace("ENTER createCorePT()");
        }
        URL coreResourceListWsdlURL = new URL(SERVICE_BASE + "AccessServiceCoreResourceListPT?wsdl");
        logger.trace("WSDL URL: " + coreResourceListWsdlURL);
        Service service = Service.create(coreResourceListWsdlURL, SERVICE_NAME);
        logger.trace("Service created: " + service.toString() + "\t" + service.getServiceName());
        CoreResourceListPT coreResourceListClient = service.getPort(RESOURCE_LIST_PORT_NAME, CoreResourceListPT.class);
        if (logger.isTraceEnabled()) {
    ...
    private CoreDataAccessPT createCoreAccessPT() throws MalformedURLException {
        if (logger.isTraceEnabled()) {
            logger.trace("ENTER createAccessCorePT()");
        }
        URL coreAccessWsdlURL = new URL(SERVICE_BASE + "AccessServiceCoreDataAccessPT?wsdl");
        logger.trace("WSDL URL: " + coreAccessWsdlURL);
        Service service = Service.create(coreAccessWsdlURL, SERVICE_NAME);
        logger.trace("Service created: " + service.toString() + "\t" + service.getServiceName());
        CoreDataAccessPT coreDataAccessClient = service.getPort(CORE_ACCESS_PORT_NAME, CoreDataAccessPT.class);
        if (logger.isTraceEnabled()) {
    ...
    private SQLAccessPT createSQLAccessPT() throws MalformedURLException {
        if (logger.isTraceEnabled()) {
            logger.trace("ENTER createSQLAccessPT()");
        }
        URL sqlAccessWsdlURL = new URL(SERVICE_BASE + "AccessServiceAccessPT?wsdl");
        logger.trace("WSDL URL: " + sqlAccessWsdlURL);
        Service service = Service.create(sqlAccessWsdlURL, SERVICE_NAME);
        logger.trace("Service created: " + service.toString() + "\t" + service.getServiceName());
        SQLAccessPT sqlAccessClient = service.getPort(SQL_ACCESS_PORT_NAME, SQLAccessPT.class);
        if (logger.isTraceEnabled()) {
    ...
    public GetResourceListResponse getResourceList(GetResourceListRequest request) throws NotAuthorizedFault, ServiceBusyFault, SNEEDataSourceException {
        ...
        if (logger.isDebugEnabled()) logger.debug("RETURN getResourceList() with response " + response);
        return response;
    }

    public SQLPropertyDocumentType getSQLPropertyDocument(GetDataResourcePropertyDocumentRequest request) throws SchemaMetadataException, InvalidResourceNameFault, DataResourceUnavailableFault, NotAuthorizedFault, ServiceBusyFault, SNEEDataSourceException {
        if (logger.isDebugEnabled()) logger.debug("ENTER getSQLPropertyDocument() with " + request.getDataResourceAbstractName());
        ...
        if (logger.isDebugEnabled()) logger.debug("RETURN getPropertyDocument() with property " + "document " + propDoc);
        return propDoc;
    }

    public PropertyDocumentType getPropertyDocument(GetDataResourcePropertyDocumentRequest request) throws SchemaMetadataException, InvalidResourceNameFault, DataResourceUnavailableFault, NotAuthorizedFault, ServiceBusyFault {
        if (logger.isDebugEnabled()) logger.debug("ENTER getPropertyDocument() with " + request.getDataResourceAbstractName());
        PropertyDocumentType propDoc;
        try {
            propDoc = _wsdairCoreAccessPT.getDataResourcePropertyDocument(request);
        } catch (InvalidResourceNameFault e) {
            logger.warn(e.getLocalizedMessage());
            throw e;
        } catch (DataResourceUnavailableFault e) {
    ...
    public SQLExecuteResponse sqlExecute(SQLExecuteRequest request) throws SNEEDataSourceException {
        if (logger.isDebugEnabled()) {
        ...
        try {
            ...
            String msg = "Not authorised to use service " + SERVICE_BASE + ".";
            logger.warn(msg, e);
            ...
            String msg = "Resource " + request.getDataResourceAbstractName() + " currently unavailable on " + SERVICE_BASE + ".";
            logger.warn(msg, e);
            throw new SNEEDataSourceException(msg, e);
        } catch (InvalidResourceNameFault e) {
            String msg = "Resource name " + request.getDataResourceAbstractName() + " unknown to service " + SERVICE_NAME + ".";
            logger.warn(msg, e);
            throw new SNEEDataSourceException(msg, e);
        } catch (ServiceBusyFault e) {
        ...
        if (logger.isDebugEnabled()) logger.debug("RETURN sqlExecute() with " + "response: " + response);
        return response;
    }
}
