...
public class LazyExtendedEmbedding_flat<NodeType, EdgeType> implements GSpanHPEmbedding<NodeType, EdgeType> {
    ...
    LazyExtendedEmbedding_flat(final int edge, final GSpanExtension<NodeType, EdgeType> ext, final int superb, final GSpanHPEmbedding_flat<NodeType, EdgeType> emb) {
        this.edge = edge;
        this.ext = ext;
        this.superb = superb;
        this.cache = emb;
    ...
    public HPEmbedding<NodeType, EdgeType> extend(final GSpanExtension<NodeType, EdgeType> ext, final int edge, final int superB) {
    ...
    private final GSpanHPEmbedding<NodeType, EdgeType> get() {
        if (needsExtension) {
            ...
            final int[] superNodes = cache.getSuperNodes(code.toHPFragment().toHPGraph().getNodeCount());
            ...
            cache = (GSpanHPEmbedding_flat<NodeType, EdgeType>) cache.tenv().getHPEmbedding(code, cache.getDataBaseGraph(), superNodes, freeEdges);
        }
        return cache;
    }
    ...
    public Extension<NodeType, EdgeType> getExtension(final int superNode, final int superEdge) {
    ...
    public int mapExtension(final Extension<NodeType, EdgeType> ext, final BitSet allowedEdges) {
