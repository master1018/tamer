...
public class Vote extends RandomizableMultipleClassifiersCombiner implements TechnicalInformationHandler {
   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (tmpStr.length() != 0) setCombinationRule(new SelectedTag(tmpStr, TAGS_RULES)); else setCombinationRule(new SelectedTag(AVERAGE_RULE, TAGS_RULES));
        ...
        switch(m_CombinationRule) {
            ...
            case MAX_RULE:
                ...
                if (instance.classAttribute().isNominal()) {
                    ...
                    if (dist[index] == 0) result = Instance.missingValue(); else result = index;
   ...
   */
    protected double classifyInstanceMedian(Instance instance) throws Exception {
        ...
        double result;
        for (int i = 0; i < results.length; i++) results[i] = m_Classifiers[i].classifyInstance(instance);
        if (results.length == 0) result = 0; else if (results.length == 1) result = results[0]; else result = Utils.kthSmallestValue(results, results.length / 2);
        return result;
    }
   ...
   */
    public double[] distributionForInstance(Instance instance) throws Exception {
        ...
        if (!instance.classAttribute().isNumeric() && (Utils.sum(result) > 0)) Utils.normalize(result);
        return result;
    }
   ...
   */
    public void setCombinationRule(SelectedTag newRule) {
        if (newRule.getTags() == TAGS_RULES) m_CombinationRule = newRule.getSelectedTag().getID();
    }
