...
public class GatewayDiscover {
    ...
    public Map<InetAddress, GatewayDevice> discover() throws SocketException, UnknownHostException, IOException, SAXException, ParserConfigurationException {
        ...
        String[] SEARCHTYPES = { "urn:schemas-upnp-org:device:InternetGatewayDevice:1", "urn:schemas-upnp-org:service:WANIPConnection:1", "urn:schemas-upnp-org:service:WANPPPConnection:1" };
        for (int i = 0; i < SEARCHTYPES.length; i++) {
            ...
            for (SendDiscoveryThread thread : threads) try {
                thread.join();
    ...
    private List<InetAddress> getLocalInetAddresses(boolean getIPv4, boolean getIPv6, boolean sortIPv4BeforeIPv6) {
        ...
        while (networkInterfaces.hasMoreElements()) {
            ...
            while (addresses.hasMoreElements()) {
                ...
                if (!getIPv4 || !getIPv6) {
                    if (getIPv4 && !Inet4Address.class.isInstance(inetAddress)) continue;
                    if (getIPv6 && !Inet6Address.class.isInstance(inetAddress)) continue;
