package net.sourceforge.ondex.xten.scripting;

import java.util.HashMap;
import java.util.LinkedList;
import java.util.List;
import java.util.Map;
import net.sourceforge.ondex.xten.scripting.ui.CommandEvent;
...
public class BasicInterpretationController implements InterpretationController {
    ...
    public BasicInterpretationController(TemplateBuilder<?>[] builderList, ProxyTemplate[] aspetList, CommandInterpreter[] interpreterList) {
        for (TemplateBuilder<?> ab : builderList) if (ab != null) builders.put(ab.getProxyTemplateType(), ab);
        for (ProxyTemplate a : aspetList) if (a != null) proxyTemplates.put(a.getClass(), a);
        for (CommandInterpreter ci : interpreterList) if (ci != null) interpreters.put(ci.getClass(), ci);
    }
        ...
        try {
            ...
            if (dependancies != null) {
                ...
                for (int i = 0; i < dependancies.size(); i++) {
                    ...
                    if (proxyTemplatesRequired[i] == null) proxyTemplatesRequired[i] = buildproxyTemplate(dependancies.get(i));
                }
            ...
            if (DEBUG) e.printStackTrace(); else throw new FunctionException("could not initialize interpreter: " + e.getMessage(), -1);
        }
    }

    private ProxyTemplate buildproxyTemplate(Class<?> proxyTemplateCls) throws Exception {
        ProxyTemplate result = null;
        TemplateBuilder<?> ab = builders.get(proxyTemplateCls);
        if (ab == null) throw new FunctionException("Cound not change configuaration - missing required proxyTemplate " + proxyTemplateCls.getSimpleName() + ".", -4);
        if (!ab.isInitialised()) {
