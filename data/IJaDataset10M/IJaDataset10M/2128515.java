...
public class Almost implements Serializable, Comparator<Number> {
  ...
  */
   ...
   */
    public int outside(double x, double x1, double x2) {
        int i = 0;
        if (between(x, x1, x2)) i = 0; else if (between(x1, x, x2)) i = -1; else if (between(x2, x, x1)) i = 1;
        return i;
    }
  ...
  */
    public int cmp(double r1, double r2) {
        ...
        if (0 * r1 * r2 != 0) throw new IllegalArgumentException("Comparing a NaN");
        double er1 = _epsilon * ar1;
        double er2 = _epsilon * ar2;
        if (r1 - er1 > r2 + er2) return 1; else if (r1 + er1 < r2 - er2) return -1; else return 0;
    }
  ...
  */
   ...
   */
    public double divide(double top, double bottom, double limit) {
        ...
        if (bottom >= 1. || top < bottom * 0.1 * Float.MAX_VALUE) {
    ...
    @Override
    public int hashCode() {
        return new Long(Double.doubleToLongBits(_epsilon) ^ Double.doubleToLongBits(_minValue)).intValue();
    }
