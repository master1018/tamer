...
public class CFormatAction extends Action implements IEditorActionDelegate {
    ...
    private int getSelectionType() throws Exception {
        ...
        if (mapName.equalsIgnoreCase("script")) return JS;
        if (mapName.toLowerCase().matches(".+-sql")) return SQL;
        return XML;
    }
    ...
    private void formatXML() {
        ...
        try {
            ...
            if (content.equals(formatContent)) return;
            page.refresh(formatContent);
            if (offset >= formatContent.length()) offset = formatContent.length();
    ...
    private void formatSQL() {
        ...
        try {
            ...
            if (sqlCode == null || sqlCode.trim().length() == 0) return;
            String prefix = info.getLeadPrefix();
            String indent = CommentXMLOutputter.DEFAULT_INDENT + prefix;
