...
public class Service implements javax.xml.rpc.Service, Serializable, Referenceable {
    ...
    private void initService(String url, QName serviceName) throws ServiceException {
        try {
            ...
            parser.run(url);
            if (cachingWSDL && this.wsdlLocation != null) cachedWSDL.put(url, parser);
            initService(parser, serviceName);
        } catch (Exception exp) {
            throw new ServiceException(Messages.getMessage("wsdlError00", "" + "", "\n" + exp), exp);
        }
    ...
    private void initService(Parser parser, QName serviceName) throws ServiceException {
        try {
            ...
            if (serviceEntry != null) this.wsdlService = serviceEntry.getService();
            if (this.wsdlService == null) throw new ServiceException(Messages.getMessage("noService00", "" + serviceName));
        } catch (Exception exp) {
            throw new ServiceException(Messages.getMessage("wsdlError00", "" + "", "\n" + exp));
        }
    }
    ...
    public Remote getPort(QName portName, Class proxyInterface) throws ServiceException {
        if (wsdlService == null) throw new ServiceException(Messages.getMessage("wsdlMissing00"));
        Port port = wsdlService.getPort(portName.getLocalPart());
        if (port == null) throw new ServiceException(Messages.getMessage("noPort00", "" + portName));
    ...
    private Remote getGeneratedStub(QName portName, Class proxyInterface) {
        try {
            ...
            if (proxyInterface.isAssignableFrom(stubClass)) {
                ...
                stub._setProperty(Stub.ENDPOINT_ADDRESS_PROPERTY, WSDLUtils.getAddressFromPort(port));
    ...
    public Remote getPort(Class proxyInterface) throws ServiceException {
        ...
        if (ports == null || ports.size() <= 0) throw new ServiceException(Messages.getMessage("noPort00", ""));
    ...
    private Remote getPort(String endpoint, QName portName, Class proxyInterface) throws ServiceException {
        ...
        if (wsdlParser != null) {
            Port port = wsdlService.getPort(portName.getLocalPart());
            if (port == null) throw new ServiceException(Messages.getMessage("noPort00", "" + proxyInterface.getName()));
        ...
        try {
            ...
            javax.xml.rpc.Stub stub = (javax.xml.rpc.Stub) Proxy.newProxyInstance(classLoader, new Class[] { proxyInterface, javax.xml.rpc.Stub.class }, new AxisClientProxy(call, portName));
    ...
    public javax.xml.rpc.Call createCall(QName portName) throws ServiceException {
        ...
        if (wsdlParser == null) return call;
        Port port = wsdlService.getPort(portName.getLocalPart());
        if (port == null) throw new ServiceException(Messages.getMessage("noPort00", "" + portName));
        Binding binding = port.getBinding();
        PortType portType = binding.getPortType();
        if (portType == null) throw new ServiceException(Messages.getMessage("noPortType00", "" + portName));
    ...
    public javax.xml.rpc.Call[] getCalls(QName portName) throws ServiceException {
        if (portName == null) throw new ServiceException(Messages.getMessage("badPort00"));
        if (wsdlService == null) throw new ServiceException(Messages.getMessage("wsdlMissing00"));
        Port port = wsdlService.getPort(portName.getLocalPart());
        if (port == null) throw new ServiceException(Messages.getMessage("noPort00", "" + portName));
        Binding binding = port.getBinding();
        ...
        while (i.hasNext()) {
            ...
            javax.xml.rpc.Call call = createCall(QName.valueOf(port.getName()), QName.valueOf(operation.getName()));
            calls.add(call);
        }
    ...
    public Reference getReference() {
        ...
        if (!classname.equals("org.apache.axis.client.Service")) {
            addr = new StringRefAddr(ServiceFactory.SERVICE_CLASSNAME, classname);
            reference.add(addr);
        } else {
            if (wsdlLocation != null) {
                addr = new StringRefAddr(ServiceFactory.WSDL_LOCATION, wsdlLocation.toString());
                reference.add(addr);
            }
            QName serviceName = getServiceName();
            if (serviceName != null) {
                addr = new StringRefAddr(ServiceFactory.SERVICE_NAMESPACE, serviceName.getNamespaceURI());
                reference.add(addr);
                addr = new StringRefAddr(ServiceFactory.SERVICE_LOCAL_PART, serviceName.getLocalPart());
                reference.add(addr);
            }
        }
        if (maintainSession) {
