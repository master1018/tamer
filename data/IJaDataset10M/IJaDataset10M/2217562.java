...
public class Clock {
     ...
     */
    private static void drawLine(Path path, float radius, float pos, float cx, float cy, float r0, float r1) {
     ...
     * @param path the Path to draw to
     */
    private static void drawVArrow(Path path, float cx, float cy, float width, float height) {
     ...
     * @param path the Path to draw to
     */
    private static void drawHArrow(Path path, float cx, float cy, float width, float height) {
    ...
    private long getOffset(float lerp) {
        long doffset = (long) (mCity.getOffset() * (float) MILLISECONDS_PER_HOUR - mOldOffset);
        int sign;
        if (doffset < 0) {
            doffset = -doffset;
            sign = -1;
     ...
     */
    public void drawClock(Canvas canvas, float cx, float cy, float radius, float alpha, float textAlpha, boolean showCityName, boolean showTime, boolean showUpArrow, boolean showDownArrow, boolean showLeftRightArrows, int prefixChars) {
        Paint paint = new Paint();
        paint.setAntiAlias(true);
        int iradius = (int) radius;
        TimeZone tz = mCity.getTimeZone();
        float lerp = Math.min(1.0f, (System.currentTimeMillis() - mCitySwitchTime) / 500.0f);
        ...
        String time = DateUtils.timeString(cal.getTimeInMillis()) + " " + DateUtils.getDayOfWeekString(cal.get(Calendar.DAY_OF_WEEK), DateUtils.LENGTH_SHORT) + " " + " (UTC" + (offset >= 0 ? "+" : "-") + offsetH + (offsetM == 0 ? "" : ":" + offsetM) + (daylightOffset == 0 ? "" : "+" + daylightOffset) + ")";
        float th = paint.getTextSize();
        float tw;
        paint.setARGB((int) (textAlpha * 255.0f), (int) (mColorRed * 255.0f), (int) (mColorGreen * 255.0f), (int) (mColorBlue * 255.0f));
        ...
        if (showCityName) {
            ...
            canvas.drawText(cityName.substring(0, prefixChars), cx - tw / 2 + 1, cy - radius - th, paint);
        }
        tw = paint.measureText(time);
        if (showTime) {
            canvas.drawText(time, cx - tw / 2, cy + radius + th + 5, paint);
        }
        paint.setARGB((int) (alpha * 255.0f), (int) (mColorRed * 255.0f), (int) (mColorGreen * 255.0f), (int) (mColorBlue * 255.0f));
        paint.setStyle(Paint.Style.FILL);
        canvas.drawOval(new RectF(cx - 2, cy - 2, cx + 2, cy + 2), paint);
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(radius * 0.12f);
        canvas.drawOval(new RectF(cx - iradius, cy - iradius, cx + iradius, cy + iradius), paint);
        ...
        if (showUpArrow) {
            drawVArrow(path, cx + radius * 1.13f, cy - radius, radius * 0.15f, -radius * 0.1f);
        }
        if (showDownArrow) {
            drawVArrow(path, cx + radius * 1.13f, cy + radius, radius * 0.15f, radius * 0.1f);
        }
        if (showLeftRightArrows) {
            drawHArrow(path, cx - radius * 1.3f, cy, -radius * 0.1f, radius * 0.15f);
            drawHArrow(path, cx + radius * 1.3f, cy, radius * 0.1f, radius * 0.15f);
        }
        paint.setStyle(Paint.Style.FILL);
