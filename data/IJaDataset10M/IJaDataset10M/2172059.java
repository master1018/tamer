...
public class Skeleton {
    ...
    public Skeleton duplicate() {
        ...
        for (int i = 0; i < joint.length; i++) s.joint[i] = joint[i].duplicate();
        for (int i = 0; i < joint.length; i++) {
            if (joint[i].parent != null) s.joint[i].parent = s.joint[s.findJointIndex(joint[i].parent.id)];
            s.joint[i].children = new Joint[joint[i].children.length];
            for (int j = 0; j < joint[i].children.length; j++) s.joint[i].children[j] = s.joint[s.findJointIndex(joint[i].children[j].id)];
        }
        return s;
    }
    ...
    public void copy(Skeleton s) {
        ...
        for (int i = 0; i < joint.length; i++) joint[i] = s.joint[i].duplicate();
        for (int i = 0; i < joint.length; i++) {
            if (s.joint[i].parent != null) joint[i].parent = joint[findJointIndex(s.joint[i].parent.id)];
            joint[i].children = new Joint[s.joint[i].children.length];
            for (int j = 0; j < joint[i].children.length; j++) joint[i].children[j] = joint[findJointIndex(s.joint[i].children[j].id)];
        }
    }
    ...
    public boolean equals(Object o) {
        ...
        if (joint.length != s.joint.length) return false;
        for (int i = 0; i < joint.length; i++) if (!joint[i].equals(s.joint[i])) return false;
        return true;
    }
    ...
    public void deleteJoint(int id) {
        ...
        while (j.children.length > 0) deleteJoint(j.children[0].id);
        which = findJointIndex(id);
        Joint newjoint[] = new Joint[joint.length - 1];
        for (int i = 0, k = 0; i < joint.length; i++) if (i != which) newjoint[k++] = joint[i];
        joint = newjoint;
        if (parent == null) return;
        Joint newchildren[] = new Joint[parent.children.length - 1];
        for (int i = 0, k = 0; i < parent.children.length; i++) if (parent.children[i] != j) newchildren[k++] = parent.children[i];
        parent.children = newchildren;
    }
    ...
    public void addAllJoints(Skeleton s) {
        ...
        System.arraycopy(joint, 0, newjoint, 0, joint.length);
        for (int i = 0; i < s.joint.length; i++) newjoint[joint.length + i] = s.joint[i].duplicate();
        for (int i = 0; i < s.joint.length; i++) {
            Joint newj = newjoint[joint.length + i];
            Joint oldj = s.joint[i];
            newj.id = nextID++;
            if (oldj.parent != null) newj.parent = newjoint[joint.length + s.findJointIndex(oldj.parent.id)];
            newj.children = new Joint[oldj.children.length];
            for (int k = 0; k < newj.children.length; k++) newj.children[k] = newjoint[joint.length + s.findJointIndex(oldj.children[k].id)];
        }
        joint = newjoint;
    }
    ...
    public void setJointParent(Joint j, Joint parent) {
        if (j.parent != null) {
            ...
            for (int i = 0, k = 0; i < j.parent.children.length; i++) if (j.parent.children[i] != j) newchildren[k++] = j.parent.children[i];
    ...
    public int findJointIndex(int id) {
        ...
        if (joint.length == 0) return -1;
        while (true) {
            if (joint[current].id == id) return current;
            if (joint[current].id > id) max = current - 1; else min = current + 1;
            if (min >= max) {
                if (min < joint.length && joint[min].id == id) return min;
                return -1;
            }
            current = (min + max) >> 1;
        }
    ...
    public Joint getJoint(int id) {
        int which = findJointIndex(id);
        if (which < 0 || which >= joint.length) return null;
        return joint[which];
    }
    ...
    public Joint[] getJoints() {
        ...
        for (int i = 0; i < j.length; i++) j[i] = joint[i];
        return j;
    }
    ...
    public void scale(double x, double y, double z) {
        for (int i = 0; i < joint.length; i++) {
            ...
            if (len > 0.0) zdir = newzdir.times(1.0 / len);
            Vec3 updir = joint[i].coords.getUpDirection();
            Vec3 newupdir = new Vec3(updir.x * x, updir.y * y, updir.z * z);
            len = newupdir.length();
            if (len > 0.0) updir = newupdir.times(1.0 / len);
            joint[i].coords.setOrientation(zdir, updir);
        }
        for (int i = 0; i < joint.length; i++) {
            if (joint[i].parent == null) joint[i].calcAnglesFromCoords(true); else joint[i].length.pos = joint[i].coords.getOrigin().distance(joint[i].parent.coords.getOrigin());
        }
    }
  ...
  */
    public void blend(Skeleton average, Skeleton s[], double weight[]) {
        ...
        for (int i = 0; i < average.joint.length; i++) if (average.joint[i].parent == null) average.joint[i].recalcCoords(true);
    }
    ...
    private double findOffset(DOF gesture, DOF defaultPose) {
        ...
        if (gesture.loop) {
            ...
            while (diff > gesture.max) diff -= range;
            while (diff < gesture.min) diff += range;
        }
        return diff;
    }
    ...
    public void draw(MeshViewer view, boolean enabled) {
        ...
        Color color = (enabled ? Color.black : Color.gray), col;
        int colInt;
        for (int i = 0; i < joint.length; i++) {
            Joint j = joint[i];
        ...
        for (int i = 0; i < joint.length; i++) {
            ...
            if (render) {
                Vec3 pos[] = new Vec3[] { cx1, cx2, cy1, cy2, joint[i].coords.getOrigin(), joint[parentIndex].coords.getOrigin() };
                for (int k = 1; k < pos.length; k++) for (int m = 0; m < k; m++) view.renderLine(pos[k], pos[m], cam, col);
            } else {
                Vec2 pos[] = new Vec2[] { objToScreen.timesXY(cx1), objToScreen.timesXY(cx2), objToScreen.timesXY(cy1), objToScreen.timesXY(cy2), p[i], p[parentIndex] };
        ...
        for (int i = 0; i < joint.length; i++) {
            Joint j = joint[i];
            if (locked[i]) col = Color.green; else if (j.id == selectedID) col = Color.magenta; else col = color;
            if (render) {
                v1.x = v2.x = p[i].x;
    ...
    public void writeToStream(DataOutputStream out) throws IOException {
        ...
        for (int i = 0; i < joint.length; i++) {
            ...
            for (int k = 0; k < j.children.length; k++) out.writeInt(j.children[k].id);
        }
    }
    ...
    public Skeleton(DataInputStream in) throws IOException, InvalidObjectException {
        ...
        for (int i = 0; i < joint.length; i++) {
            ...
            for (int k = 0; k < childID[i].length; k++) childID[i][k] = in.readInt();
            if (j.id >= nextID) nextID = j.id + 1;
        }
        for (int i = 0; i < joint.length; i++) {
            joint[i].parent = getJoint(parentID[i]);
            joint[i].children = new Joint[childID[i].length];
            for (int k = 0; k < childID[i].length; k++) joint[i].children[k] = getJoint(childID[i][k]);
        }
    }
}
