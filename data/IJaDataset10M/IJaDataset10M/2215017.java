 ...
 */
    public DhbVector(int dimension) throws NegativeArraySizeException {
        if (dimension <= 0) throw new NegativeArraySizeException("Requested vector size: " + dimension);
 ...
 */
    public void accumulate(double[] x) throws DhbIllegalDimension {
        if (this.dimension() != x.length) throw new DhbIllegalDimension("Attempt to add a " + this.dimension() + "-dimension vector to a " + x.length + "-dimension array");
        for (int i = 0; i < this.dimension(); i++) components[i] += x[i];
    }
 ...
 */
    public void accumulate(DhbVector v) throws DhbIllegalDimension {
        if (this.dimension() != v.dimension()) throw new DhbIllegalDimension("Attempt to add a " + this.dimension() + "-dimension vector to a " + v.dimension() + "-dimension vector");
        for (int i = 0; i < this.dimension(); i++) components[i] += v.components[i];
    }
 ...
 */
    public DhbVector add(DhbVector v) throws DhbIllegalDimension {
        ...
        for (int i = 0; i < this.dimension(); i++) newComponents[i] = components[i] + v.components[i];
 ...
 */
    public boolean equals(DhbVector v) {
        ...
        if (v.dimension() != n) return false;
        for (int i = 0; i < n; i++) {
            if (v.components[i] != components[i]) return false;
        }
        return true;
    }
 ...
 */
    public DhbVector normalizedBy(double x) {
        for (int i = 0; i < this.dimension(); i++) components[i] /= x;
        return this;
    }
 ...
 */
    public DhbVector product(double d) {
        ...
        for (int i = 0; i < components.length; i++) newComponents[i] = d * components[i];
        return new DhbVector(newComponents);
    }
 ...
 */
    public double product(DhbVector v) throws DhbIllegalDimension {
        int n = v.dimension();
        if (components.length != n) throw new DhbIllegalDimension("Dot product with mismatched dimensions: " + components.length + ", " + n);
 ...
 */
    public DhbVector product(Matrix a) throws DhbIllegalDimension {
        int n = a.rows();
        int m = a.columns();
        if (this.dimension() != n) throw new DhbIllegalDimension("Product error: transposed of a " + this.dimension() + "-dimension vector cannot be multiplied with a " + n + " by " + m + " matrix");
 ...
 */
    protected double secureProduct(DhbVector v) {
        double sum = 0;
        for (int i = 0; i < v.dimension(); i++) sum += components[i] * v.components[i];
        return sum;
    }
 ...
 */
    protected DhbVector secureProduct(Matrix a) {
        ...
        for (int j = 0; j < m; j++) {
            ...
            for (int i = 0; i < n; i++) vectorComponents[j] += components[i] * a.components[i][j];
        }
 ...
 */
    public Matrix tensorProduct(DhbVector v) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) newComponents[i][j] = components[i] * v.components[j];
        }
        return n == m ? new SymmetricMatrix(newComponents) : new Matrix(newComponents);
    }
