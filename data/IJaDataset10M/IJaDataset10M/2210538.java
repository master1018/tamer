...
public class ArenaService {
    ...
    public boolean isEnemy(Player effector, Player effected) {
        if (effector == effected) return false;
        if (!isInSameGroup(effector, effected) && !isInSameAlliance(effector, effected) && effector.getInArena() && effected.getInArena()) return true;
        return false;
    }
    ...
    public void onDie(final Player player, Creature lastAttacker) {
        ...
        if (summon != null) summon.getController().release(UnsummonType.UNSPECIFIED);
        ...
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            @Override
            public void run() {
                if (isInTrinielArena(player)) TeleportService.teleportTo(player, 120010000, 1, 1005.1f, 1528.9f, 222.1f, 0);
                if (isInSanctumArena(player)) TeleportService.teleportTo(player, 110010000, 1, 1470.3f, 1343.5f, 563.7f, 0);
            }
        }, 5000);
    }

    public boolean isInArena(Player player) {
        if (isInTrinielArena(player) || isInSanctumArena(player)) return true;
        return false;
    }

    private boolean isInTrinielArena(Player player) {
        int world = player.getWorldId();
        if (world == 120010000 && ZoneService.getInstance().isInsideZone(player, ZoneName.TRINIEL_PVP_ZONE)) return true;
        return false;
    }

    private boolean isInSanctumArena(Player player) {
        int world = player.getWorldId();
        if (world == 110010000 && ZoneService.getInstance().isInsideZone(player, ZoneName.COLISEUM_PVP_ZONE)) return true;
        return false;
    }

    public boolean isInSameGroup(Player player1, Player player2) {
        if (player1.isInGroup() && player2.isInGroup()) {
            if (player1.getPlayerGroup().getGroupId() == player2.getPlayerGroup().getGroupId()) return true;
        }
        return false;
    }
    ...
    public boolean isInSameAlliance(Player player1, Player player2) {
        if (player1.isInAlliance() && player2.isInAlliance()) {
            if (player1.getPlayerAlliance().getAllianceIdFor(player1.getObjectId()) == player2.getPlayerAlliance().getAllianceIdFor(player2.getObjectId())) return true;
        }
        return false;
    }
