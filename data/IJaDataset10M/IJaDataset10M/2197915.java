...
public class Tab implements java.io.Serializable {
	 ...
	 */
    public static final String COLLECTION_PREFIX = "xava_collectionTab_";

    public static final String TAB_RESETED_PREFIX = "xava.tab.reseted.";

    private static final String PROPERTIES_NAMES = "propertiesNames";

    private static final String SUM_PROPERTIES_NAMES = "sumPropertiesNames";

    private static final String ROWS_HIDDEN = "rowsHidden";

    private static final String FILTER_VISIBLE = "filterVisible";

    private static final String PAGE_ROW_COUNT = "pageRowCount";

    private static final String COLUMN_WIDTH = "columnWidth.";

    private static final String STARTS_COMPARATOR = "starts_comparator";

    private static final String CONTAINS_COMPARATOR = "contains_comparator";

    private static final String NOT_CONTAINS_COMPARATOR = "not_contains_comparator";

    private static final String YEAR_COMPARATOR = "year_comparator";

    private static final String MONTH_COMPARATOR = "month_comparator";

    private static final String YEAR_MONTH_COMPARATOR = "year_month_comparator";

    private static final String RANGE_COMPARATOR = "range_comparator";

    private static final int MAX_PAGE_ROW_COUNT = 20;
    ...
    private MetaTab getMetaTab() throws XavaException {
        if (metaTab == null) {
            try {
                ...
                if (getModelName().indexOf('.') >= 0 || getTabName().startsWith(COLLECTION_PREFIX)) {
            ...
            for (int i = 0; i < this.conditionValues.length; i++) {
                ...
                if (Is.emptyString(this.conditionComparators[i])) {
                    ...
                    for (CmpField field : fields) {
                        ...
                        comparatorsToWhere.add(this.conditionComparators[i]);
                        if (firstCondition) firstCondition = false; else sb.append(" and ");
                    ...
                    sb.append(" ? ");
                    if (metaPropertiesKey == null) metaPropertiesKey = new ArrayList();
                    if (YEAR_MONTH_COMPARATOR.equals(this.conditionComparators[i]) || RANGE_COMPARATOR.equals(this.conditionComparators[i]) || Timestamp.class.equals(p.getType()) && "eq".equals(this.conditionComparators[i])) {
                    ...
                    try {
                        if (YEAR_COMPARATOR.equals(this.conditionComparators[i]) || MONTH_COMPARATOR.equals(this.conditionComparators[i]) || YEAR_MONTH_COMPARATOR.equals(this.conditionComparators[i])) {
        ...
        for (int i = 0; i < this.conditionValuesToWhere.length; i++) {
            ...
            if (!isEmpty(value)) {
                if (STARTS_COMPARATOR.equals(this.conditionComparatorsToWhere[i])) {
                    ...
                    key.add(value);
                } else if (CONTAINS_COMPARATOR.equals(this.conditionComparatorsToWhere[i]) || NOT_CONTAINS_COMPARATOR.equals(this.conditionComparatorsToWhere[i])) {
                    value = "%" + convertStringArgument(value.toString()) + "%";
                    key.add(value);
                    ...
                    try {
                        ...
                        if (st.hasMoreTokens()) key.add(new Integer(st.nextToken())); else key.add(null);
                        if (st.hasMoreTokens()) key.add(new Integer(st.nextToken())); else key.add(null);
                    } catch (Exception ex) {
                        log.warn(XavaResources.getString("tab_key_value_warning"), ex);
                        key.add(null);
    ...
    public void goAddColumnsPage(int page) {
        if (page < 1) addColumnsPage = 1; else if (page > getAddColumnsLastPage()) addColumnsPage = getAddColumnsLastPage(); else addColumnsPage = page;
    }
    ...
    private void setConditionValuesImpl(String[] values) throws XavaException {
        ...
        if (getMetaPropertiesNotCalculated().size() != values.length) return;
    ...
    private void setConditionComparators(String collectionPrefix) {
        setConditionComparatorsImpl(getConditionFilterParameters(collectionPrefix + "conditionComparator."));
    }

    private void setConditionValues(String collectionPrefix) {
        setConditionValuesImpl(getConditionFilterParameters(collectionPrefix + "conditionValue."));
    }

    private void setConditionValuesTo(String collectionPrefix) {
        setConditionValuesToImpl(getConditionFilterParameters(collectionPrefix + "conditionValueTo."));
    }
    ...
    private void loadUserPreferences() {
        try {
            ...
            if (propertiesNames != null) {
                propertiesNames = removeNonexistentProperties(propertiesNames);
    ...
    private String removeNonexistentProperties(String properties) {
        if (propertiesExists(properties)) return properties;
        StringBuffer sb = new StringBuffer();
        ...
        try {
            ...
            if (columnWidths != null) {
                for (Map.Entry<String, Integer> columnWidth : columnWidths.entrySet()) {
                    preferences.putInt(COLUMN_WIDTH + columnWidth.getKey(), columnWidth.getValue());
                }
            }
            preferences.flush();
        } catch (Exception ex) {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (((MetaProperty) metaPropertiesNC.get(i)).getName().equals(property)) {
                    filterConditionValues[i] = value == null ? null : value.toString();
                    filtered = true;
                }
            }
        }
    ...
    private void setFilteredConditionValues() {
        if (filtered && filterConditionValues != null) {
            ...
            int size = (filterConditionValues.length < conditionValues.length) ? filterConditionValues.length : conditionValues.length;
            for (int i = 0; i < size; i++) {
    ...
    public String getTotalLabel(int row, int column) {
        try {
            ...
            if (!getTotalProperties().containsKey(columnProperty)) return "";
    ...
    private Object getTotal(String qualifiedPropertyName, int index) {
        try {
            if (getCollectionView() != null && getCollectionView().getTotalProperties().containsKey(qualifiedPropertyName)) {
