...
public class SignatureServiceBean implements SignatureServiceEx {
    ...
    public void postSign(byte[] signatureValue, List<X509Certificate> signingCertificateChain) throws SecurityException {
    ...
    private SignatureServiceEx getSignatureService(IdentityDTO identity, byte[] photo) {
        XKMS2Client xkms2Client = this.trustValidationService.getXkms2Client();
        String tspUrl = this.configuration.getValue(ConfigProperty.TSP_URL, String.class);
        String tspPolicyOid = this.configuration.getValue(ConfigProperty.TSP_POLICY_OID, String.class);
        String signTrustDomain = this.configuration.getValue(ConfigProperty.SIGN_TRUST_DOMAIN, String.class);
        ...
        Integer httpProxyPort = this.configuration.getValue(ConfigProperty.HTTP_PROXY_PORT, Integer.class);
        DigestAlgo signatureDigestAlgo = this.configuration.getValue(ConfigProperty.SIGNATURE_DIGEST_ALGO, DigestAlgo.class);
        LOG.debug("signatureDigestAlgo: " + signatureDigestAlgo);
        RevocationDataService revocationDataService = new TrustServiceRevocationDataService(xkms2Client, signTrustDomain);
        SignatureFacet signatureFacet = new SignerCertificateSignatureFacet();
        TimeStampServiceValidator timeStampServiceValidator = new TrustServiceTimeStampServiceValidator(xkms2Client, tsaTrustDomain);
        TSPTimeStampService timeStampService = new TSPTimeStampService(tspUrl, timeStampServiceValidator);
        if (useHttpProxy) {
        ...
        DSSDocumentService documentService = this.servicesManager.getDocumentService();
        SignatureServiceEx signatureService;
        try {
            signatureService = documentService.getSignatureService(documentInputStream, timeStampService, timeStampServiceValidator, revocationDataService, signatureFacet, documentOutputStream, role, identity, photo, signatureDigestAlgo);
        } catch (Exception e) {
            throw new RuntimeException("error retrieving signature service: " + e.getMessage(), e);
        }
    ...
    public DigestInfo preSign(List<DigestInfo> digestInfos, List<X509Certificate> signingCertificateChain, IdentityDTO identity, AddressDTO address, byte[] photo) throws NoSuchAlgorithmException {
        SignatureServiceEx signatureService = getSignatureService(identity, photo);
        return signatureService.preSign(digestInfos, signingCertificateChain, identity, address, photo);
    }
}
