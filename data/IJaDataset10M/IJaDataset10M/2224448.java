...
public class PacketFormat extends ProtocolNode {
    ...
    public static PacketFormat generateFromString(String str, List<Integer> ids) {
        ...
        if (str.charAt(i) == '(') {
            ...
            while (str.charAt(i) != ')') {
                ...
                if (type != null) idparts.add(type); else return null;
                i++;
                if (i >= str.length()) return null;
            }
        ...
        while (i < str.length()) {
            if (str.charAt(i) != '[') {
                ...
                PartType type = PartTypeManager.getInstance().getType(str.substring(i, i + 1));
                if (type != null) pf.getDataFormat().getMainBlock().addPart(new Part(type, -1, "", "", "")); else return null;
                i++;
                if (i >= str.length()) return null;
            }
        }
    ...
    private static List<Part> parseForString(String str) {
        ...
        if (str.charAt(i) != '[') {
            ...
            PartType type = PartTypeManager.getInstance().getType(str.substring(i, i + 1));
            if (type != null) parts.add(new Part(type, -1, "", "", "")); else return null;
            i++;
            if (i >= str.length()) return null;
        }
        return parts;
