...
class WithMarkersOperation {
    ...
    WithMarkersOperation(Automaton oldAutomaton) {
        ...
        if (oldAutomaton.getAcceptations().size() > 0 && oldAutomaton.getAcceptations().first() == Acceptation.ACCEPT) {
            throw new InternalException("invalid operation");
        }
        oldAutomaton = getOptimizedAutomaton(oldAutomaton.longest().minimal()).minimal();
        ...
        this.newAutomaton = new Automaton(oldAutomaton.getAlphabet());
        {
            ...
            progressSet.add(new Pair<State, Pair<Integer, Marker>>(oldAutomaton.getStartState(), null));
            Pair<Marker, Set<Pair<State, Pair<Integer, Marker>>>> progress = new Pair<Marker, Set<Pair<State, Pair<Integer, Marker>>>>(null, progressSet);
        ...
        while (this.workSet.hasNext()) {
            ...
            Pair<Marker, Set<Pair<State, Pair<Integer, Marker>>>> progress = this.progressMap.get(state);
            {
                boolean acceptFound = false;
                for (Pair<State, Pair<Integer, Marker>> progressElement : progress.getRight()) {
                    State oldState = progressElement.getLeft();
                    Pair<Integer, Marker> accept = progressElement.getRight();
                    if (oldState.isAcceptState()) {
                        if (acceptFound) {
                            throw new InternalException("there shouldn't be more than one accepting progress");
                        }
                        acceptFound = true;
                        for (Acceptation acceptation : oldState.getAcceptations()) {
                            Acceptation markedAcceptation;
                            if (accept == null) {
                                markedAcceptation = new Acceptation(acceptation.getName(), 0, null);
                            } else {
                                markedAcceptation = new Acceptation(acceptation.getName(), accept.getLeft(), accept.getRight());
                            }
                            if (!this.newAutomaton.getUnstableAcceptations().contains(markedAcceptation)) {
                                this.newAutomaton.addAcceptation(markedAcceptation);
                            }
    ...
    private void addTransition(State newSourceState, RichSymbol richSymbol) {
        RichSymbol normalRichSymbol = richSymbol.isLookahead() ? null : richSymbol;
        RichSymbol lookaheadRichSymbol = richSymbol.isLookahead() ? richSymbol : richSymbol.getSymbol().getLookaheadRichSymbol();
        Pair<Marker, Set<Pair<State, Pair<Integer, Marker>>>> sourceProgress = this.progressMap.get(newSourceState);
        ...
        for (Pair<State, Pair<Integer, Marker>> sourceProgressElement : sourceProgress.getRight()) {
            State oldSourceState = sourceProgressElement.getLeft();
            Pair<Integer, Marker> sourceAccept = sourceProgressElement.getRight();
            State oldTargetState = oldSourceState.getSingleTarget(lookaheadRichSymbol);
        ...
        for (Pair<State, Pair<Integer, Marker>> sourceProgressElement : sourceProgress.getRight()) {
            ...
            if (normalRichSymbol != null) {
                State oldTargetState = oldSourceState.getSingleTarget(normalRichSymbol);
                if (oldTargetState != null) {
                    if (sourceAccept != null) {
                        throw new InternalException("a normal symbol is not valid after a lookahead symbol");
                    }
                    Pair<State, Pair<Integer, Marker>> targetProgressElement = new Pair<State, Pair<Integer, Marker>>(oldTargetState, null);
                ...
                if (oldTargetState != null) {
                    ...
                    if (sourceAccept != null) {
                        if (sourceAccept.getRight() != null) {
                            ...
                            targetAccept = new Pair<Integer, Marker>(sourceAccept.getLeft() + 1, nextMarker);
                            nextMarkerIsUsed = true;
                        } else {
                            targetAccept = new Pair<Integer, Marker>(sourceAccept.getLeft() + 1, null);
                        }
                    } else if (oldTargetState.isCyclic()) {
                    ...
                    Pair<State, Pair<Integer, Marker>> targetProgressElement = new Pair<State, Pair<Integer, Marker>>(oldTargetState, targetAccept);
                    targetProgressSet.add(targetProgressElement);
                }
            }
        }
        ...
        Pair<Marker, Set<Pair<State, Pair<Integer, Marker>>>> targetProgress = new Pair<Marker, Set<Pair<State, Pair<Integer, Marker>>>>(nextMarkerIsUsed ? nextMarker : null, targetProgressSet);
    ...
    private Automaton getOptimizedAutomaton(Automaton oldAutomaton) {
        ...
        for (State oldSourceState : oldAutomaton.getStates()) {
            State optimizedSourceState = oldStateToOptimizedStateMap.get(oldSourceState);
            if (this.optimizedStates.contains(optimizedSourceState)) {
                continue;
            }
            for (Entry<RichSymbol, SortedSet<State>> entry : oldSourceState.getTransitions().entrySet()) {
                RichSymbol richSymbol = entry.getKey();
                for (State oldTargetState : entry.getValue()) {
                    State optimizedTargetState = oldStateToOptimizedStateMap.get(oldTargetState);
                    optimizedSourceState.addTransition(richSymbol, optimizedTargetState);
                }
            }
        }
    ...
    private State getOptimization(State sourceState) {
        ...
        for (Symbol symbol : alphabet.getSymbols()) {
            targetState = sourceState.getSingleTarget(symbol.getNormalRichSymbol());
            if (targetState != null) {
                return null;
            }
            targetState = sourceState.getSingleTarget(symbol.getLookaheadRichSymbol());
