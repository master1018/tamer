...
public class HideRemoveObject extends EditDialog implements PropertyChangeListener {
    ...
    public HideRemoveObject(JFrame frame, String title, Vector<GeometricalObject> Objects) {
    ...
    private void jbInit() {
        ...
        tabbedPane.addTab("Hide Object", null, pHideObject, "Change visibility status of objects");
    ...
    public void propertyChange(PropertyChangeEvent e) {
        if (isVisible() && (e.getSource() == optionPane) && (JOptionPane.VALUE_PROPERTY.equals(e.getPropertyName()))) {
    ...
    private void hideOrRemoveObject() {
        if (tabbedPane.getSelectedComponent() == pHideObject) {
            ...
            for (int i = 0; i < pObjects.getComponentCount(); i++) if (((JCheckBox) pObjects.getComponent(i)).isSelected()) selectedObj.addElement(new Integer(pObjects.getComponentCount() - i));
            Boolean[] visibilityToggled = new Boolean[objects.size()];
            for (int i = 0; i < objects.size(); i++) if (objects.elementAt(i).isVisible()) {
                visibilityToggled[i] = Boolean.FALSE;
                for (int j = 0; j < selectedObj.size(); j++) if (selectedObj.elementAt(j).intValue() - 1 == i) {
            ...
            ConstructionStep step = new ConstructionStep(this, visibilityToggled, HIDE_OBJECT);
            doConstruction(step);
        }
        if (tabbedPane.getSelectedComponent() == pRemoveObject) {
            GeometricalObject ob = (GeometricalObject) cRemoveObject.getSelectedItem();
            ConstructionStep step = new ConstructionStep(this, new Object[] { ob }, REMOVE_OBJECT);
            doConstruction(step);
        }
    }
    ...
    public void doConstruction(ConstructionStep step) {
        if (step.getConstructionType() == HIDE_OBJECT) {
            ...
            for (int i = 0; i < objects.size(); i++) if (visibilityToggled[i].booleanValue()) objects.elementAt(i).toggleVisible();
            clearAndHide();
            ...
            for (int i = 0; i < objects.size(); i++) objects.elementAt(i).setNumber(i + 1);
            clearAndHide();
            step.setOutputs(new Object[] {});
            fireIrreversibleEditUpdate();
        } else System.err.println("Bad construction type");
    }
    ...
    private void showObjects() {
        ...
        for (int t = (objects.size() - 1); t >= 0; t--) {
            ...
            JCheckBox checkbox = new JCheckBox(objects.elementAt(t).toString(), !(objects.elementAt(t).isVisible()));
            pObjects.add(checkbox);
        }
    }

    public String getPresentationName(ConstructionStep step) {
        if (step.getConstructionType() == HIDE_OBJECT) return "(Un)Hide Objects"; else {
    ...
    public String getDisplayName(ConstructionStep step) {
        if (step.getConstructionType() == HIDE_OBJECT) {
            ...
            for (int i = 0; i < step.getInputs().length; i++) if (((Boolean) step.getInputs()[i]).booleanValue()) displayName += (i + 1) + ", ";
