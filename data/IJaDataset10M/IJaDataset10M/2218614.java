...
final class MemoryResource implements Constants, VM_Uninterruptible {
    ...
    public static final String Id = "$Id: MemoryResource.java 3566 2003-05-22 20:51:25Z dgrove-oss $";
   ...
   */
    private void lock() {
        if (Plan.gcInProgress()) gcLock.acquire(); else mutatorLock.acquire();
    }
   ...
   */
    private void unlock() {
        if (Plan.gcInProgress()) gcLock.release(); else mutatorLock.release();
    }
   ...
   */
    ...
    private static final int MAX_MEMORY_RESOURCES = 20;

    private static final MemoryResource[] allMR = new MemoryResource[MAX_MEMORY_RESOURCES];

    private static int allMRCount = 0;
