...
public class GSignature {
    ...
    public boolean conformsTo(GSignature sig) {
        if (this.procedureName.equals(sig.getProcedureName()) && this.types.size() == sig.getTypes().size()) {
            List<Type> sigTypes = sig.getTypes();
            for (int index = 0; index < this.types.size(); index++) {
                if (!sigTypes.get(index).isSubtypeOf(types.get(index))) return false;
            }
            return true;
        }
        return false;
    ...
    public String toString() {
        return "procedure " + this.procedureName + "(" + StringUtil.fmtSeq(this.types.iterator(), ",") + ")";
    }
