    ...
    class Painting extends JPanel {
        ...
        public void paint(String actionString) {
            ...
            for (i = 0; i < actions.length; i += 6, index++) {
                ...
                colour[index] = 0xFF000000 | (int) (0xFF * r) << 16 | (int) (0xFF * g) << 8 | (int) (0xFF * b);
            }
            index = 0;
            for (pixel = 0; pixel < pixels.length; pixel++) {
                pixels[pixel] = colour[index];
                index++;
                if (index >= colour.length) index = 0;
            }
        ...
        private double stdDev(int[] v) {
            ...
            mean /= v.length;
            for (int i = 0; i < v.length; i++) w[i] = Math.pow(v[i] - mean, 2);
            mean = 0;
            for (int i = 0; i < w.length; i++) mean += w[i];
            mean /= w.length;
