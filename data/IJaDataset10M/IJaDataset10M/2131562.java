...
public class MergeParserCommandGroup extends InputFileCommandGroup {
    ...
    public MergeParserCommandGroup() {
        ...
        file_option.setHelp("Similar to the previous option, but each external EDIF file is named explicitly--including the path to the file.");
        ...
        _replace_srls_option.setHelp("Replace LUT RAMs acting as SRLs in the design with actual SRLs." + " Normally the tool will not replace these LUT RAMs, but if this option is set it will.");
    ...
    public static EdifEnvironment getEdifEnvironment(JSAPResult result) {
        ...
        try {
            ...
            top = EdifMergeParser.parseAndMergeEdif(getInputFileName(result), searchDirs, getIncludeFileCollection(result), primitiveLibrary, allow_open_pins, quitOnError);
