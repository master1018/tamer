...
public class ProjectFloat {
   ...
   */
    private void __gluMultMatrixVecf(float[] matrix, int matrix_offset, float[] in, float[] out) {
        for (int i = 0; i < 4; i++) {
            out[i] = in[0] * matrix[0 * 4 + i + matrix_offset] + in[1] * matrix[1 * 4 + i + matrix_offset] + in[2] * matrix[2 * 4 + i + matrix_offset] + in[3] * matrix[3 * 4 + i + matrix_offset];
        }
    }
   ...
   */
    private void __gluMultMatrixVecf(FloatBuffer matrix, FloatBuffer in, FloatBuffer out) {
        ...
        for (int i = 0; i < 4; i++) {
            out.put(i + outPos, in.get(0 + inPos) * matrix.get(0 * 4 + i + matrixPos) + in.get(1 + inPos) * matrix.get(1 * 4 + i + matrixPos) + in.get(2 + inPos) * matrix.get(2 * 4 + i + matrixPos) + in.get(3 + inPos) * matrix.get(3 * 4 + i + matrixPos));
        }
    }
   ...
   */
    private void gluMultMatricesf(float[] a, int a_offset, float[] b, int b_offset, float[] r) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                r[i * 4 + j] = a[i * 4 + 0 + a_offset] * b[0 * 4 + j + b_offset] + a[i * 4 + 1 + a_offset] * b[1 * 4 + j + b_offset] + a[i * 4 + 2 + a_offset] * b[2 * 4 + j + b_offset] + a[i * 4 + 3 + a_offset] * b[3 * 4 + j + b_offset];
            }
        }
    }
   ...
   */
    public static void gluMultMatricesf(FloatBuffer a, FloatBuffer b, FloatBuffer r) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                r.put(i * 4 + j + rPos, a.get(i * 4 + 0 + aPos) * b.get(0 * 4 + j + bPos) + a.get(i * 4 + 1 + aPos) * b.get(1 * 4 + j + bPos) + a.get(i * 4 + 2 + aPos) * b.get(2 * 4 + j + bPos) + a.get(i * 4 + 3 + aPos) * b.get(3 * 4 + j + bPos));
            }
        }
    }
   ...
   */
    public static void normalize(FloatBuffer v) {
        float r;
        int vPos = v.position();
        r = (float) Math.sqrt(v.get(0 + vPos) * v.get(0 + vPos) + v.get(1 + vPos) * v.get(1 + vPos) + v.get(2 + vPos) * v.get(2 + vPos));
        if (r == 0.0 || r == 1.0) return;
        r = 1.0f / r;
   ...
   * @param upz
   */
    public void gluLookAt(GLMatrixFunc gl, float eyex, float eyey, float eyez, float centerx, float centery, float centerz, float upx, float upy, float upz) {
   ...
   * @return
   */
    public boolean gluProject(float objx, float objy, float objz, float[] modelMatrix, int modelMatrix_offset, float[] projMatrix, int projMatrix_offset, int[] viewport, int viewport_offset, float[] win_pos, int win_pos_offset) {
        float[] in = this.in;
        float[] out = this.out;
        in[0] = objx;
        in[1] = objy;
   ...
   * @return
   */
    public boolean gluProject(float objx, float objy, float objz, FloatBuffer modelMatrix, FloatBuffer projMatrix, IntBuffer viewport, FloatBuffer win_pos) {
   ...
   * @return
   */
    public boolean gluUnProject(float winx, float winy, float winz, float[] modelMatrix, int modelMatrix_offset, float[] projMatrix, int projMatrix_offset, int[] viewport, int viewport_offset, float[] obj_pos, int obj_pos_offset) {
        float[] in = this.in;
        float[] out = this.out;
        gluMultMatricesf(modelMatrix, modelMatrix_offset, projMatrix, projMatrix_offset, matrix);
        if (!gluInvertMatrixf(matrix, matrix)) return false;
        in[0] = winx;
        in[1] = winy;
        in[2] = winz;
        in[3] = 1.0f;
   ...
   * @return
   */
    public boolean gluUnProject(float winx, float winy, float winz, FloatBuffer modelMatrix, FloatBuffer projMatrix, IntBuffer viewport, FloatBuffer obj_pos) {
        FloatBuffer in = this.inBuf;
        FloatBuffer out = this.outBuf;
        gluMultMatricesf(modelMatrix, projMatrix, matrixBuf);
        if (!gluInvertMatrixf(matrixBuf, matrixBuf)) return false;
        in.put(0, winx);
        in.put(1, winy);
        in.put(2, winz);
        in.put(3, 1.0f);
        ...
        if (out.get(3) == 0.0f) return false;
   ...
   * @return
   */
    public boolean gluUnProject4(float winx, float winy, float winz, float clipw, float[] modelMatrix, int modelMatrix_offset, float[] projMatrix, int projMatrix_offset, int[] viewport, int viewport_offset, float near, float far, float[] obj_pos, int obj_pos_offset) {
        float[] in = this.in;
        float[] out = this.out;
        gluMultMatricesf(modelMatrix, modelMatrix_offset, projMatrix, projMatrix_offset, matrix);
        if (!gluInvertMatrixf(matrix, matrix)) return false;
   ...
   * @return
   */
    public boolean gluUnProject4(float winx, float winy, float winz, float clipw, FloatBuffer modelMatrix, FloatBuffer projMatrix, IntBuffer viewport, float near, float far, FloatBuffer obj_pos) {
        FloatBuffer in = this.inBuf;
        FloatBuffer out = this.outBuf;
        gluMultMatricesf(modelMatrix, projMatrix, matrixBuf);
        if (!gluInvertMatrixf(matrixBuf, matrixBuf)) return false;
   ...
   * @param viewport
   */
    public void gluPickMatrix(GLMatrixFunc gl, float x, float y, float deltaX, float deltaY, IntBuffer viewport) {
        if (deltaX <= 0 || deltaY <= 0) {
            return;
        }
        int vPos = viewport.position();
        gl.glTranslatef((viewport.get(2 + vPos) - 2 * (x - viewport.get(0 + vPos))) / deltaX, (viewport.get(3 + vPos) - 2 * (y - viewport.get(1 + vPos))) / deltaY, 0);
   ...
   */
    public void gluPickMatrix(GLMatrixFunc gl, float x, float y, float deltaX, float deltaY, int[] viewport, int viewport_offset) {
        if (deltaX <= 0 || deltaY <= 0) {
            return;
        }
        gl.glTranslatef((viewport[2 + viewport_offset] - 2 * (x - viewport[0 + viewport_offset])) / deltaX, (viewport[3 + viewport_offset] - 2 * (y - viewport[1 + viewport_offset])) / deltaY, 0);
