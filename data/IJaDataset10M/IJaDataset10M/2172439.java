...
public class ScoreDialog extends JDialog {
    ...
    public ScoreDialog(Frame owner, final Listener listener, ScoringMethod initialRules) {
        ...
        box.add(values);
        m_area = createColorEntry("LB_SCORE_AREA", 3, "TT_SCORE_AREA_BLACK", "TT_SCORE_AREA_WHITE", labels, values);
        m_territory = createColorEntry("LB_SCORE_TERRITORY", 3, "TT_SCORE_TERRITORY_BLACK", "TT_SCORE_TERRITORY_WHITE", labels, values);
        m_prisoners = createColorEntry("LB_SCORE_PRISONERS", 3, "TT_SCORE_PRISONERS_BLACK", "TT_SCORE_PRISONERS_WHITE", labels, values);
        m_komi = createKomiEntry(3, labels, values);
        m_resultArea = createEntry("LB_SCORE_RESULT_AREA", 8, "TT_SCORE_RESULT_AREA", labels, values);
        m_resultTerritory = createEntry("LB_SCORE_RESULT_TERRITORY", 8, "TT_SCORE_RESULT_TERRITORY", labels, values);
        createRulesEntry(labels, values);
        m_result = createEntry("LB_SCORE_RESULT", 8, "TT_SCORE_RESULT", labels, values);
        ...
        JOptionPane optionPane = new JOptionPane(outerBox, JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, null, options, options[0]);
    ...
    public void showScore(CountScore countScore, Komi komi) {
        ...
        if (m_score != null) rules = m_score.m_rules;
    ...
    private static final ImageIcon ICON_BLACK = GuiUtil.getIcon("gogui-black-16x16", i18n("LB_BLACK"));

    private static final ImageIcon ICON_WHITE = GuiUtil.getIcon("gogui-white-16x16", i18n("LB_WHITE"));
    ...
    private ColorFields createColorEntry(String labelText, int cols, String toolTipBlack, String toolTipWhite, JComponent labels, JComponent values) {
        ...
        if (toolTipBlack != null) black.setToolTipText(i18n(toolTipBlack));
        panel.add(black);
        ...
        if (toolTipWhite != null) white.setToolTipText(i18n(toolTipWhite));
        panel.add(white);
        values.add(panel);
        return colorFields;
    }
    ...
    private void showScore() {
        ...
        if (m_score.m_komi != null) m_komi.setText(m_score.m_komi.toString());
        ...
        if (m_score.m_rules == TERRITORY) m_useTerritory.setSelected(true); else {
