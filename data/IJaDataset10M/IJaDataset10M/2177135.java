...
public class VirtualKeyboard extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_MOVE) {
            ...
            } else if (srcRect.right > kbdBitmap.getWidth()) {
                srcRect.offset(kbdBitmap.getWidth() - srcRect.right, 0);
            }
            if (srcRect.top < 0) {
                srcRect.offset(0, -srcRect.top);
            } else if (srcRect.bottom > kbdBitmap.getHeight()) {
            ...
            if (pressedButton != null) {
                ...
                if (pressedButton == shiftBtn) {
                    state ^= 1;
                    drawLayout();
                } else if (pressedButton == symbolsBtn) {
                    state ^= 2;
                    drawLayout();
    ...
    Runnable showZoom = new Runnable() {
        ...
        public void run() {
            zoomVisible = true;
            zoom.showAtLocation(VirtualKeyboard.this, Gravity.NO_GRAVITY, getLeft() + dstPoint.x, getTop() + dstPoint.y);
        }
    };
    ...
    class VirtualButton {
        ...
        void draw(Canvas canvas, ButtonDrawable buttonDrawable, String t) {
            ...
            if (bitmap != null) {
                canvas.drawBitmap(bitmap, rect.centerX() - bitmap.getWidth() / 2, rect.centerY() - bitmap.getHeight() / 2, null);
            } else {
