...
public class ResponderImpl implements Responder {
    ...
    public Message onReceiveCfb(MessageCallForBids msg) {
        ...
        if (prot == AuctionTags.REVERSE_FIRST_PRICE_SEALED_AUCTION) {
            if (shouldBid) {
                response = new MessageBid(auctionID, prot, msg.getMessageID(), 0, round);
                priceBid = (float) jobCost;
                ((MessageBid) response).setPrice(priceBid);
                System.out.println(resource.getResourceName() + " bidding for auction " + auctionID + " round " + round + " and price " + priceBid);
            } else {
                response = new MessageRejectCallForBid(auctionID, prot, msg.getMessageID(), 0, round);
            }
