...
public class FlagDependencies {
    ...
    static void introduceFlag(Flag f) {
        if (mySealedFlag) {
            throw new IllegalStateException("Cannot create a new flag after " + "dependency set-up time.  Are you sure you declared your " + "flag static and included a call to your setUpFlags() " + "method in edu.clemson.cs.r2jt.setUpFlagDependencies()?");
        }
    ...
    static void checkFlagDependencies(Flag f, FlagManager m) throws FlagDependencyException {
        ...
        for (Flag excluded : info.excludes) {
            if (m.isFlagSet(excluded)) {
                throw new FlagDependencyException("The -" + m.getFlagReason(f) + " flag and -" + m.getFlagReason(excluded) + " flag may not be on simultaneously.");
            }
        }
    }

    private static void checkThatAtLeastOneIsOn(Set<Flag> s, Flag f, FlagManager m) throws FlagDependencyException {
        boolean found = false;
        ...
        if (!found) {
            ...
            if (s.size() == 1) {
                throw new FlagDependencyException("The -" + m.getFlagReason(f) + " flag requires that the " + flagList + " flag be " + "set.");
            } else {
                throw new FlagDependencyException("The -" + m.getFlagReason(f) + " flag requires that at least one of the following " + " flags be set: " + flagList);
            }
        }
    }
    ...
    public static void addImplies(Flag ifOn, Flag impliesOn) {
        ...
        if (impliesOn.getParameterCount() != 0) {
            throw new IllegalArgumentException("A flag that takes parameters " + "cannot be implied.  " + impliesOn + " takes " + impliesOn.getParameterCount() + " parameters.");
        }
    ...
    private static void checkSealedState() {
        if (mySealedFlag) {
            throw new IllegalStateException("Cannot create dependencies " + "after dependency set-up time.  Are you sure you " + "included a call to your setUpFlags() method in " + "edu.clemson.cs.r2jt.setUpFlagDependencies()?");
        }
    }
        ...
        for (Map.Entry<String, List<Flag>> e : myFlagSections.entrySet()) {
            ...
            for (Flag f : flags) {
                if (f.shouldDisplay(extended)) {
                    retval += wordWrap("    -" + formatInto(f.getName(), 13) + "  " + f.getDescription() + "\n", 80, 20);
                }
            }
        }
        return retval;
