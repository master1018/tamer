...
final class ATextPanelImpl {
    ...
    ATextPanelImpl(RunStrategy runStrategy, TextPanelSettings settings, MConstText initialText, Clipboard clipboard, MTextPanel client, Adjustable horzSb, Adjustable vertSb) {
        ...
        StyledTextClipboard textClipboard = StyledTextClipboard.getClipboardFor(clipboard);
        fText = new StyledText();
        if (initialText != null) {
            fText.append(initialText);
        }
        fTextComponent = new TextComponent(fText, settings.getDefaultValues(), settings.getWraps(), TextComponent.WINDOW_WIDTH, TextComponent.DEFAULT_INSET, textClipboard, settings.getScrollable(), scroller, fBroadcaster);
        if (scroller != null) {
            scroller.setClient(fTextComponent);
        }
        if (settings.getSelectable()) {
            fSelection = new TextSelection(fTextComponent, fBroadcaster, fRunStrategy);
            fSelection.addToOwner(fTextComponent);
            if (settings.getEditable()) {
                fEditBehavior = new TextEditBehavior(fTextComponent, fSelection, fBroadcaster, fRemap);
    ...
    public void replaceRange(MConstText newText, int start, int end) {
        ...
        if (fSelection != null) {
            ...
            TextOffset newSelection = new TextOffset(start + newText.length(), TextOffset.AFTER_OFFSET);
            TextReplacement replacement = new TextReplacement(start, end, newText, newSelection, newSelection);
            fTextComponent.textControlEventOccurred(Behavior.REPLACE, replacement);
            if (fEditBehavior == null) {
    ...
    private static Object consistentCharStyle(MConstText text, int start, int limit, Object key, Object defaultValue) {
        if (start >= limit) {
            throw new IllegalArgumentException("Invalid range.");
        }
        int runStart = start;
        ...
        for (runStart = text.characterStyleLimit(runStart); runStart < limit; runStart = text.characterStyleLimit(runStart)) {
    ...
    public Object getCharacterStyleOverSelection(Object key) {
        TextRange selRange;
        if (fSelection != null) selRange = fSelection.getSelectionRange(); else selRange = new TextRange(0, 0);
        if (selRange.start == selRange.limit) {
            ...
            return consistentCharStyle(fText, selRange.start, selRange.limit, key, getDefaultValues().get(key));
        }
    }
    ...
    public Object getParagraphStyleOverSelection(Object key) {
        ...
        if (selRange.start == selRange.limit) {
            ...
            for (paragraphStart = fText.paragraphLimit(paragraphStart); paragraphStart < selRange.limit; paragraphStart = fText.paragraphLimit(paragraphStart)) {
    ...
    public void setCommandLogSize(int size) {
        fTextComponent.textControlEventOccurred(Behavior.SET_COMMAND_LOG_SIZE, new Integer(size));
    }
    ...
    public void setModified(boolean modified) {
        boolean handled = fTextComponent.textControlEventOccurred(Behavior.SET_MODIFIED, modified ? Boolean.TRUE : Boolean.FALSE);
        if (!handled) {
            fModified = modified;
        }
    }
