...
public class PackageTreeWriter extends AbstractTreeWriter {
    ...
    public PackageTreeWriter(ConfigurationImpl configuration, String path, String filename, PackageDoc packagedoc, PackageDoc prev, PackageDoc next) throws IOException {
        super(configuration, path, filename, new ClassTree(configuration.classDocCatalog.allClasses(packagedoc), configuration), packagedoc);
    ...
    public static void generate(ConfigurationImpl configuration, PackageDoc pkg, PackageDoc prev, PackageDoc next, boolean noDeprecated) {
        ...
        try {
            packgen = new PackageTreeWriter(configuration, path, filename, pkg, prev, next);
    ...
    protected void generatePackageTreeFile() throws IOException {
        ...
        Content headContent = getResource("doclet.Hierarchy_For_Package", Util.getPackageName(packagedoc));
        Content heading = HtmlTree.HEADING(HtmlConstants.TITLE_HEADING, false, HtmlStyle.title, headContent);
    ...
    protected void addLinkToMainTree(Content div) {
        Content span = HtmlTree.SPAN(HtmlStyle.strong, getResource("doclet.Package_Hierarchies"));
        div.addContent(span);
    ...
    protected Content getNavLinkPrevious() {
        if (prev == null) {
            ...
            String path = DirectoryManager.getRelativePath(packagedoc.name(), prev.name());
    ...
    protected Content getNavLinkNext() {
        if (next == null) {
            ...
            String path = DirectoryManager.getRelativePath(packagedoc.name(), next.name());
