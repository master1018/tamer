...
class NXJFlashCommandLineParser extends AbstractCommandLineParser {
    ...
    public void parse(String[] args) throws ParseException {
        ...
        if (this.binary) {
            if (files.length > 1) throw new ParseException("Too many files");
            if (files.length < 1) throw new ParseException("No file specified");
            if (this.format) throw new ParseException("Formatting filesystem not supported in binary mode");
            this.firmwareFile = new File(files[0]);
        } else {
            if (files.length > 0) {
                if (files.length > 2) throw new ParseException("Too many files");
                if (files.length < 2) throw new ParseException("You must provide both firmware and menu file");
                this.firmwareFile = new File(files[0]);
    ...
    private void testFile(File file) throws ParseException {
        if (file != null && !file.exists()) throw new ParseException("File does not exist: " + file);
    }
