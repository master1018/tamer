...
public class AntlrJavaSourcePathComputer extends JavaSourcePathComputer {
    ...
    public ISourceContainer[] computeSourceContainers(ILaunchConfiguration configuration, IProgressMonitor monitor) throws CoreException {
        ISourceContainer[] sourceContainers = super.computeSourceContainers(configuration, monitor);
        String path = LaunchConfigurationUtils.getString(configuration, AntlrDebugConstants.JAVA_EXTERNAL_SOURCE_PATH, null);
        ISourceContainer[] newsc = new ISourceContainer[sourceContainers.length + 1];
        System.arraycopy(sourceContainers, 0, newsc, 1, sourceContainers.length);
