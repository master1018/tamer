...
public class DataSourceConfig implements ILifeCycle {
    ...
    private void registryDataSource(DataSource ds, DatabaseConnection dbConfig, INamedDataSourceProvider dsProvider) throws RuntimeException {
        ...
        if (dataSourceName == null) {
            mLogger.log(Level.CONFIG, "Setting up dataSource url:{0},user:{1}", new Object[] { dbConfig.getUrl(), dbConfig.getUserName() });
            mObjectRegistry.registerInstance(DataSource.class, ds);
        } else {
            ((NamedDataSourceProvider) dsProvider).putDataSource(dbConfig.getName(), ds);
            mLogger.log(Level.CONFIG, "Setting up dataSource url:{0},user:{1},name:{2}", new Object[] { dbConfig.getUrl(), dbConfig.getUserName(), dbConfig.getName() });
        }
    }
    ...
    void cleanDataSource(DataSource ds) {
        if (ds != null) {
            ...
            if (ds instanceof StandardXADataSource) ((StandardXADataSource) ds).shutdown(true);
