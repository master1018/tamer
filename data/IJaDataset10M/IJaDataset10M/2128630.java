...
public class CreateOutlineTree {
    ...
    public void createPdf(String filename) throws IOException, DocumentException, SQLException {
        ...
        for (Movie movie : movies) {
            ...
            if ("3-Iron".equals(title)) title = "빈집";
            movieBookmark = new PdfOutline(root, new PdfDestination(PdfDestination.FITH, writer.getVerticalPosition(true)), title, true);
            movieBookmark.setStyle(Font.BOLD);
            link = new PdfOutline(movieBookmark, new PdfAction(String.format(RESOURCE, movie.getImdb())), "link to IMDB");
            link.setColor(BaseColor.BLUE);
            new PdfOutline(movieBookmark, PdfAction.javaScript(String.format(INFO, movie.getYear(), movie.getDuration()), writer), "instant info");
