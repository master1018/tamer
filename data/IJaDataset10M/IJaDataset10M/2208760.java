...
public class ElementsDockable extends OrganDockable {
    ...
    private static Configuration config = Configuration.getRoot().get(ElementsDockable.class);
    ...
    public ElementsDockable() {
        ...
        tree.setTransferHandler(new TransferHandler() {
            ...
            protected Transferable createTransferable(JComponent c) {
                return new ObjectTransferable(TreeUtils.getSelection(tree).toArray());
            }
            ...
            public boolean importData(JComponent comp, Transferable t) {
                try {
                    final Object[] subElements = (Object[]) ObjectTransferable.getObject(t);
                    session.lookup(UndoManager.class).compound(new Compound() {
                        ...
                        public void run() {
                            ...
                            session.lookup(ElementSelection.class).setSelectedElements(added);
                        }
                    });
                    return true;
    ...
    public void setSession(OrganSession session) {
        if (this.session != null) {
            ...
            this.session.lookup(ElementProblems.class).removeListener((ProblemListener) Spin.over(selectionHandler));
            this.session.lookup(ElementSelection.class).removeListener(selectionHandler);
            initTree();
        }
        ...
        if (this.session != null) {
            ...
            this.session.lookup(ElementProblems.class).addListener((ProblemListener) Spin.over(selectionHandler));
            this.session.lookup(ElementSelection.class).addListener(selectionHandler);
            initTree();
        }
	 ...
	 */
    private class SelectionHandler extends OrganAdapter implements SelectionListener, TreeSelectionListener, ProblemListener {
        ...
        public void selectionChanged() {
            if (!updatingSelection) {
                ...
                for (Element selected : session.lookup(ElementSelection.class).getSelectedElements()) {
        ...
        public void valueChanged(TreeSelectionEvent e) {
            ...
            if (!updatingSelection) {
                ...
                List<Element> selection = TreeUtils.getSelection(tree);
                if (selection.size() == 1) {
                    session.lookup(ElementSelection.class).setSelectedElement(selection.get(0));
                } else if (!selection.isEmpty()) {
                    session.lookup(ElementSelection.class).setSelectedElements(selection);
                }
                updatingSelection = false;
            }
        }
    ...
    private void initTree() {
        if (this.session == null) {
            ...
            if (sortByNameButton.isSelected()) {
                comparator = ComparatorChain.of(new ElementNameComparator(), new ElementTypeComparator());
            } else {
                comparator = ComparatorChain.of(new ElementTypeComparator(), new ElementNameComparator());
            }
            model.setElements(session.getOrgan(), session.getOrgan().getElements(), comparator);
    ...
    private class DuplicateAction extends BaseAction implements TreeSelectionListener, Compound {
        ...
        public void run() {
            ...
            for (Element element : new ArrayList<Element>(session.lookup(ElementSelection.class).getSelectedElements())) {
    ...
    private class RemoveAction extends BaseAction implements TreeSelectionListener, Compound {
