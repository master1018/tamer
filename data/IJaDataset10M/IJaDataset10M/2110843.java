...
public class UserManager {
    ...
    void addOps(String user_list) {
        ...
        for (int i = 0; i < userList.size(); i++) {
            String user = userList.get(i);
            if (user.equalsIgnoreCase(_bot.botname())) _bot.setOp(true); else {
                synchronized (users) {
                    User u;
                    if (!userExist(user)) {
    ...
    void userJoin(String user) {
        ...
        synchronized (users) {
            ...
            if (contains == false) users.add(new User(user, _bot));
        }
        try {
            if (!_bot.isHubSupports("NoGetINFO")) {
                String cmd = "$GetINFO $" + user + " $" + _bot.botname() + "|";
    ...
    public boolean userExist(String user) {
        synchronized (users) {
            for (User u : users) if (u.username().equalsIgnoreCase(user)) return true;
            return false;
        }
    }
    ...
    public List<User> getUserByIP(String ip, boolean isRegx) {
        ...
        synchronized (users) {
            for (User u : users) {
                if (isRegx ? u.getUserIP().matches(ip) : u.getUserIP().equals(ip)) ru.add(u);
            }
        }
        return ru;
    }
    ...
    public User getUser(String user) {
        synchronized (users) {
            for (User u : users) if (u.username().equalsIgnoreCase(user)) {
                return u;
            }
            return null;
        }
    ...
    public User getUserByCID(String CID) {
        synchronized (users) {
            for (User u : users) {
                if (u.getClientID() != null && !u.getClientID().isEmpty() && u.getClientID().equalsIgnoreCase(CID)) return u;
            }
            return null;
        }
    }
    ...
    void SetInfo(String info) {
        ...
        if (strUser.equalsIgnoreCase(_bot.botname())) return;
        index++;
    ...
    private class SendingAll extends Thread {
        ...
        public void run() {
            for (User u : useri) {
                try {
                    if (!(_bot.botname().equalsIgnoreCase(u.username()))) _bot.SendPrivateMessage(u.username(), _pm);
                    sleep(_timeout);
                } catch (Exception e) {
                }
            }
