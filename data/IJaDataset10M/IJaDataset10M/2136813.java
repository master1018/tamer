...
public class ExtendedPetriNet extends ReplayedPetriNet {
	 ...
	 */
    protected Transition makeTransition(Transition template, ReplayedPetriNet targetNet, ArrayList caseIDs) {
	 ...
	 */
    protected Place makePlace(Place template, ReplayedPetriNet targetNet, ArrayList caseIDs) {
	 ...
	 */
    protected PNEdge makeEdge(PNEdge template, Place sourceNode, Transition targetNode, ReplayedPetriNet targetNet, ArrayList caseIDs) {
        return new ExtendedPNEdge((ExtendedPlace) sourceNode, (ExtendedTransition) targetNode, caseIDs);
    }

    /**
	 * {@inheritDoc} Produces a {@link ExtendedPNEdge ExtendedPNEdge}.
	 */
    protected PNEdge makeEdge(PNEdge template, Transition sourceNode, Place targetNode, ReplayedPetriNet targetNet, ArrayList caseIDs) {
        return new ExtendedPNEdge((ExtendedTransition) sourceNode, (ExtendedPlace) targetNode, caseIDs);
    }
    ...
    protected void writeTransitionsToDot(Writer bw) throws IOException {
        ...
        while (it.hasNext()) {
            ...
            try {
                bw.write("t" + myExtendedTransition.getNumber() + " [shape=\"box\",label=\"" + label + "\"" + (myExtendedTransition.getLogEvent() != null ? "" : "style=\"filled\"") + "];\n");
    ...
    protected void writePlacesToDot(Writer bw) throws IOException {
        try {
            ...
            while (it.hasNext()) {
                ...
                bw.write("p" + p.getNumber() + " [shape=\"circle\",color=\"black\"," + determinePlaceColor(p) + ",style=\"filled\",label=\"\",fontcolor=\"black\"];\n");
            ...
            while (it.hasNext()) {
                ...
                if (e.isPT()) {
                    ...
                    if (p.getOutEdges().size() > 1) {
                        double prob = e.getProbability(currentlySelectedInstances, p.getTotalOutEdgeFrequency(currentlySelectedInstances, advancedSettings[1], failedInstances), advancedSettings[1], failedInstances);
