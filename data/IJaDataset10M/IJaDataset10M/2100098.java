...
public class HashTest extends Frame implements ActionListener, WindowListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == bQuit) onQuit(); else if (src == bRun) onRun();
    }
    ...
    private boolean can_do_hash(int in_count, int type) {
        if (in_count == 2) return (type == 0 || type == 1); else return true;
    }
    ...
    private int do_hash(int[] ins, int in_count, int type) {
        if (in_count == 1) {
            ...
            switch(type) {
                case 0:
                    return HashFunctions.hash_pair(ins[0], ins[1]);
                case 1:
                    return HashFunctions.hash_prime(ins[0], ins[1]);
            }
        } else if (in_count == 3) {
            switch(type) {
                case 0:
                    return HashFunctions.hash_pair(ins[0], ins[1], ins[2]);
                case 1:
                    return HashFunctions.hash_prime(ins[0], ins[1], ins[2]);
                case 2:
                    return HashFunctions.hash_jenkins(ins[0], ins[1], ins[2]);
                case 3:
                    return HashFunctions.hash_FNV(ins[0], ins[1], ins[2]);
            }
        }
        return -1;
    }
    ...
    private int do_mix(int in, int type) {
        switch(type) {
            case 1:
                return HashFunctions.mix(in);
            case 2:
                return HashFunctions.mix_wang(in);
            case 3:
                return HashFunctions.mix_jenkins(in);
            default:
                return in;
        }
    }
