...
public class IntlTestNumberFormat extends com.ibm.icu.dev.test.TestFmwk {
    ...
    public void tryIt(double aNumber) {
        ...
        for (i = 0; i < DEPTH; i++) {
            ...
            if (i > 0) {
                if (numberMatch == 0 && number[i] == number[i - 1]) numberMatch = i; else if (numberMatch > 0 && number[i] != number[i - 1]) {
                    errln("**** FAIL: Numeric mismatch after match.");
                    dump = true;
                    break;
                }
                if (stringMatch == 0 && string[i] == string[i - 1]) stringMatch = i; else if (stringMatch > 0 && string[i] != string[i - 1]) {
    ...
    public void tryIt(int aNumber) {
        ...
        if (number != aNumber) {
            errln("**** FAIL: Parse of " + stringNum + " failed. Got:" + number + " Expected:" + aNumber);
        }
    }
