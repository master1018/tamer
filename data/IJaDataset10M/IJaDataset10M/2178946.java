...
public class Polygon extends ConcreteOverlay {
   ...
   * @return a new instance of Polygon.
   */
    public static Polygon fromEncoded(EncodedPolyline[] polylines, boolean fill, String color, double opacity, boolean outline) {
        return new Polygon(nativeFromEncoded(toJsArray(polylines), fill, color, opacity, outline));
    }
   ...
   */
    private static native JsArray<EncodedPolyline> nativeArrayToJavaScriptObject(EncodedPolyline[] array);

    private static native JavaScriptObject nativeFromEncoded(JsArray<EncodedPolyline> polylinesIn);

    private static native JavaScriptObject nativeFromEncoded(JsArray<EncodedPolyline> polylinesIn, boolean fillIn, String colorIn, double opacityIn, boolean outlineIn);
   ...
   */
    public Polygon(LatLng[] points, String strokeColor, int strokeWeight, double strokeOpacity, String fillColor, double fillOpacity) {
        super(PolygonImpl.impl.construct(LatLng.toJsArray(points), strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity));
    }
   ...
   */
    public Polygon(LatLng[] points, String strokeColor, int strokeWeight, double strokeOpacity, String fillColor, double fillOpacity, PolygonOptions options) {
        super(PolygonImpl.impl.construct(LatLng.toJsArray(points), strokeColor, strokeWeight, strokeOpacity, fillColor, fillOpacity, options));
    }
   ...
   * @param handler the handler to call when this event fires.
   */
    public void addPolygonLineUpdatedHandler(final PolygonLineUpdatedHandler handler) {
   ...
   * @param handler the handler to call when this event fires.
   */
    public void addPolygonVisibilityChangedHandler(final PolygonVisibilityChangedHandler handler) {
        ...
        polygonVisibilityChangedHandlers.addHandler(handler, new BooleanCallback() {
            ...
            public void callback(boolean visible) {
                PolygonVisibilityChangedEvent e = new PolygonVisibilityChangedEvent(Polygon.this, visible);
   ...
   */
    ...
    private void maybeInitPolygonCancelLineHandlers() {
        if (polygonCancelLineHandlers == null) {
            polygonCancelLineHandlers = new HandlerCollection<PolygonCancelLineHandler>(jsoPeer, MapEvent.CANCELLINE);
        }
    }

    private void maybeInitPolygonClickHandlers() {
        if (polygonClickHandlers == null) {
            polygonClickHandlers = new HandlerCollection<PolygonClickHandler>(jsoPeer, MapEvent.CLICK);
        }
    }

    private void maybeInitPolygonEndLineHandlers() {
        if (polygonEndLineHandlers == null) {
            polygonEndLineHandlers = new HandlerCollection<PolygonEndLineHandler>(jsoPeer, MapEvent.ENDLINE);
        }
    }

    private void maybeInitPolygonLineUpdatedHandlers() {
        if (polygonLineUpdatedHandlers == null) {
            polygonLineUpdatedHandlers = new HandlerCollection<PolygonLineUpdatedHandler>(jsoPeer, MapEvent.LINEUPDATED);
        }
    }

    private void maybeInitPolygonMouseOutHandlers() {
        if (polygonMouseOutHandlers == null) {
            polygonMouseOutHandlers = new HandlerCollection<PolygonMouseOutHandler>(jsoPeer, MapEvent.MOUSEOUT);
        }
    }

    private void maybeInitPolygonMouseOverHandlers() {
        if (polygonMouseOverHandlers == null) {
            polygonMouseOverHandlers = new HandlerCollection<PolygonMouseOverHandler>(jsoPeer, MapEvent.MOUSEOVER);
        }
    }

    private void maybeInitPolygonRemoveHandlers() {
        if (polygonRemoveHandlers == null) {
            polygonRemoveHandlers = new HandlerCollection<PolygonRemoveHandler>(jsoPeer, MapEvent.REMOVE);
        }
    }

    private void maybeInitPolygonVisibilityChangedHandlers() {
        if (polygonVisibilityChangedHandlers == null) {
            polygonVisibilityChangedHandlers = new HandlerCollection<PolygonVisibilityChangedHandler>(jsoPeer, MapEvent.VISIBILITYCHANGED);
        }
    }
}
