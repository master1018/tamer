...
public class ParseGenome {
    ...
    public static final Pattern tairSpliter = Pattern.compile("[.][0-9]+$");
    ...
    public void parseGenome(String tairReleaseDir, ONDEXGraph graph) {
        ...
        if (parseNCBImapping) {
            ...
            if (refSeqG == null) graph.getMetaData().getFactory().createDataSource(MetaData.ncNM);
            DataSource refSeqP = graph.getMetaData().getDataSource(MetaData.ncNP);
            if (refSeqP == null) graph.getMetaData().getFactory().createDataSource(MetaData.ncNP);
            try {
