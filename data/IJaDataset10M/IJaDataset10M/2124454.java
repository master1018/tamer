...
public class MasterLeatherWorking extends EnhancedCraftingSkill implements ItemCraftor, MendingSkill {
    ...
    public String parametersFormat() {
        return "ITEM_NAME\tITEM_LEVEL\tBUILD_TIME_TICKS\tAMOUNT_MATERIAL_REQUIRED\tITEM_BASE_VALUE\t" + "ITEM_CLASS_ID\tWEAPON_CLASS||CODED_WEAR_LOCATION\t" + "CONTAINER_CAPACITY||LIQUID_CAPACITY||WEAPON_HANDS_REQUIRED\tBASE_DAMAGE||BASE_ARMOR_AMOUNT\t" + "CONTAINER_TYPE\tCODED_SPELL_LIST";
    }
        ...
        if (canBeUninvoked()) {
            if ((affected != null) && (affected instanceof MOB)) {
                ...
                if ((building != null) && (!aborted)) {
                    if (messedUp) {
                        if (mending) messedUpCrafting(mob); else if (refitting) commonEmote(mob, "<S-NAME> mess(es) up refitting " + building.name() + "."); else commonEmote(mob, "<S-NAME> mess(es) up making " + building.name() + ".");
                    } else {
                        if (mending) building.setUsesRemaining(100); else if (refitting) {
    ...
    protected boolean canMend(MOB mob, Environmental E, boolean quiet) {
        ...
        if ((IE.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_LEATHER) {
            if (!quiet) commonTell(mob, "That's not made of any sort of leather.  That can't be mended.");
            return false;
        }
        return true;
    }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        if ((!auto) && (commands.size() > 0) && (((String) commands.firstElement()).equalsIgnoreCase("bundle"))) {
            bundling = true;
            if (super.invoke(mob, commands, givenTarget, auto, asLevel)) return super.bundle(mob, commands);
            return false;
        }
        ...
        if (str.equalsIgnoreCase("list")) {
            ...
            int toggler = 1;
            int toggleTop = 2;
            for (int r = 0; r < toggleTop; r++) buf.append(CMStrings.padRight("Item", 30) + " " + CMStrings.padRight("Lvl", 3) + " " + CMStrings.padRight("Amt", 3) + " ");
            buf.append("\n\r");
            for (int r = 0; r < recipes.size(); r++) {
                ...
                if (V.size() > 0) {
                    ...
                    if ((level + 20 <= xlevel(mob)) && ((mask == null) || (mask.length() == 0) || mask.equalsIgnoreCase("all") || CMLib.english().containsString(item, mask))) {
            ...
            for (int r = 0; r < recipes.size(); r++) {
                ...
                if (V.size() > 0) {
                    ...
                    int wood = CMath.s_int((String) V.elementAt(RCP_WOOD));
                    wood = adjustWoodRequired(wood, mob);
                    if (((level + 25) <= (xlevel(mob))) && ((mask == null) || (mask.length() == 0) || mask.equalsIgnoreCase("all") || CMLib.english().containsString(item, mask))) {
            ...
            for (int r = 0; r < recipes.size(); r++) {
                ...
                if (V.size() > 0) {
                    ...
                    int wood = CMath.s_int((String) V.elementAt(RCP_WOOD));
                    wood = adjustWoodRequired(wood, mob);
                    if (((level + 30) <= xlevel(mob)) && ((mask == null) || (mask.length() == 0) || mask.equalsIgnoreCase("all") || CMLib.english().containsString(item, mask))) {
            ...
            for (int r = 0; r < recipes.size(); r++) {
                ...
                if (V.size() > 0) {
                    ...
                    int wood = CMath.s_int((String) V.elementAt(RCP_WOOD));
                    wood = adjustWoodRequired(wood, mob);
                    if (((level + 35) <= xlevel(mob)) && ((mask == null) || (mask.length() == 0) || mask.equalsIgnoreCase("all") || CMLib.english().containsString(item, mask))) {
            ...
            for (int r = 0; r < recipes.size(); r++) {
                ...
                if (V.size() > 0) {
                    ...
                    int wood = CMath.s_int((String) V.elementAt(RCP_WOOD));
                    wood = adjustWoodRequired(wood, mob);
                    if (((level + 40) <= xlevel(mob)) && ((mask == null) || (mask.length() == 0) || mask.equalsIgnoreCase("all") || CMLib.english().containsString(item, mask))) {
            ...
            for (int r = 0; r < recipes.size(); r++) {
                ...
                if (V.size() > 0) {
                    ...
                    int wood = CMath.s_int((String) V.elementAt(RCP_WOOD));
                    wood = adjustWoodRequired(wood, mob);
                    if (((level + 45) <= xlevel(mob)) && ((mask == null) || (mask.length() == 0) || mask.equalsIgnoreCase("all") || CMLib.english().containsString(item, mask))) {
        ...
        if (str.equalsIgnoreCase("scan")) return publicScan(mob, commands); else if (str.equalsIgnoreCase("mend")) {
            building = null;
            mending = false;
            messedUp = false;
            ...
            for (int r = 0; r < matches.size(); r++) {
                ...
                if (V.size() > 0) {
                    int level = CMath.s_int((String) V.elementAt(RCP_LEVEL));
                    if (((level + 45) <= xlevel(mob)) && (recipeName.toUpperCase().indexOf("BATTLEMOULDED") >= 0)) {
                        multiplier = 9;
                        prefix = "Battlemoulded ";
                        foundRecipe = V;
                        break;
                    } else if (((level + 40) <= xlevel(mob)) && (recipeName.toUpperCase().indexOf("LAMINAR") >= 0)) {
                        multiplier = 8;
                        prefix = "Laminar ";
                        foundRecipe = V;
                        break;
                    } else if ((level + 35) <= (xlevel(mob)) && (recipeName.toUpperCase().indexOf("MASTERWORK") >= 0)) {
                        multiplier = 7;
                        prefix = "Masterwork ";
                        foundRecipe = V;
                        break;
                    } else if (((level + 30) <= xlevel(mob)) && (recipeName.toUpperCase().indexOf("REINFORCED") >= 0)) {
                        multiplier = 6;
                        prefix = "Reinforced ";
                        foundRecipe = V;
                        break;
                    } else if (((level + 25) <= (xlevel(mob))) && (recipeName.toUpperCase().indexOf("CUIRBOULI") >= 0)) {
                        multiplier = 5;
                        prefix = "Cuirbouli ";
                        foundRecipe = V;
                        break;
                    } else if ((level + 20) <= (xlevel(mob))) {
                        multiplier = 4;
                        prefix = "Designer ";
                        foundRecipe = V;
                        break;
            ...
            int[][] data = fetchFoundResourceData(mob, woodRequired, "leather", pm, (multiplier == 6) ? 1 : 0, (multiplier == 6) ? "metal" : null, (multiplier == 6) ? pm1 : null, bundling, autoGenerate, enhancedTypes);
            if (data == null) return false;
            woodRequired = data[0][FOUND_AMT];
            if (!super.invoke(mob, commands, givenTarget, auto, asLevel)) return false;
            int lostValue = autoGenerate > 0 ? 0 : CMLib.materials().destroyResources(mob.location(), woodRequired, data[0][FOUND_CODE], data[1][FOUND_CODE], null);
            ...
            if (bundling) itemName = "a " + woodRequired + "# " + itemName; else if (itemName.endsWith("s")) itemName = "some " + itemName; else itemName = CMLib.english().startWithAorAn(itemName);
            ...
            if (building instanceof Armor) {
                ...
                if (armordmg != 0) ((Armor) building).baseEnvStats().setArmor(armordmg + (abilityCode() - 1) + hardness);
