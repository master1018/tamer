    ...
    public String processResponse(Element element, BotsConfigurationFileParser parser) throws ProcessorException {
        ...
        if (Boolean.valueOf(element.getAttribute(ENABLED)).booleanValue()) {
            ...
            if (!bots.include(botID)) {
                ...
                logger.info(String.format("%,d categories loaded in %.4f seconds.", graphmaster.getCategoryCount() - previousCategoryCount, time / 1000.00));
                ...
                if (dupes > 0) {
                    logger.warn(String.format("%,d path-identical categories were encountered, and handled according to the %s merge policy.", dupes, this.core.getSettings().getMergePolicy()));
                }
            } else {
