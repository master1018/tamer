...
public class ReplMode extends ProcessMode {
    ...
    public ReplMode(Buffer buffer, Language language, Environment environment) throws java.io.IOException {
        ...
        thread = new Future(new kawa.repl(language), environment, in, out, err);
        thread.setPriority(Thread.currentThread().getPriority() + 1);
        thread.start();
    }

    public static void make(Buffer buffer, Object language) throws java.io.IOException {
        Language lang = language instanceof Language ? (Language) language : Language.getInstance(language.toString());
