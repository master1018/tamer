...
final class DispatchComMethod extends ComMethod {
    ...
    DispatchComMethod(Method m) {
        ...
        if (vtid != null) throw new IllegalAnnotationException("@VTID and @DISPID are mutually exclusive: " + m.toGenericString());
        DISPID id = m.getAnnotation(DISPID.class);
        if (id == null) throw new IllegalAnnotationException("@DISPID is missing: " + m.toGenericString());
        dispId = id.value();
        flag = getFlag();
    ...
    private int getFlag() {
        ...
        if (get != null && put != null) throw new IllegalAnnotationException("@PropPut and @PropGet are mutually exclusive: " + method.toGenericString());
        if (get != null) return DISPATCH_PROPERTYGET;
        if (put != null) return DISPATCH_PROPERTYPUT;
