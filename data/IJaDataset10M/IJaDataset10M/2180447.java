...
public class MacrosPage extends JPanel implements PreferencesPage {
        ...
        if (macros_table == null) {
            ...
            table_model = new AbstractTableModel() {
                ...
                public boolean isCellEditable(int row, int col) {
                    if (col != FILENAME_COLUMN) return true;
                    return false;
                }
                            ...
                            if (s.equals(Locale.getString("NO_START"))) {
                                for (int i = 0; i < stored_macros.size(); i++) {
                                    if (i != row) {
                                        ...
                                        if (other_old_seq > 1 && other_old_seq > old_seq) t.setStart_sequence(other_old_seq - 1);
                                    }
                                }
                                if (m.isOn_startup()) startup_items--;
                                setComboEditor();
                                ...
                                for (int i = 0; i < stored_macros.size(); i++) {
                                    if (i != row) {
                                        ...
                                        } else if (t.isOn_startup() && other_old_seq > 1 && other_old_seq > old_seq && other_old_seq <= new_seq) {
                                            t.setStart_sequence(other_old_seq - 1);
                                        }
                                    }
                                }
    ...
    private void setComboEditor() {
        ...
        int add_new = 0;
        if (startup_items != stored_macros.size()) add_new = 1;
        ...
        if (delete_button == null) {
            ...
            delete_button.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    int response = JOptionPane.showInternalConfirmDialog(delete_button, Locale.getString("SURE_DELETE_MACRO", macro.getName(), filepath), Locale.getString("DELETE_MACRO_TITLE"), JOptionPane.YES_NO_OPTION);
                    if (response == JOptionPane.YES_OPTION) {
                        if (macro.delete()) {
                            ...
                            JOptionPane.showInternalMessageDialog(delete_button, Locale.getString("UNABLE_TO_DELETE_NAME_ERROR", filepath), Locale.getString("WARNINGTITLE"), JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
            });
