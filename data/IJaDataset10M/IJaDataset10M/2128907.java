...
public class CDKMCS {
    ...
    public static List<List<CDKRMap>> getSubgraphAtomsMaps(IAtomContainer sourceGraph, IAtomContainer targetGraph, boolean shouldMatchBonds) throws CDKException {
        List<CDKRMap> list = checkSingleAtomCases(sourceGraph, targetGraph);
        if (list == null) {
            return makeAtomsMapsOfBondsMaps(CDKMCS.getSubgraphMaps(sourceGraph, targetGraph, shouldMatchBonds), sourceGraph, targetGraph);
        } else {
            ...
            for (int j = 0; j < ac2.getBondCount(); j++) {
                ...
                if (bondA2 instanceof IQueryBond) {
                    ...
                    if (queryBond.matches(bondA1)) {
                        if (atom1.matches(bondA1.getAtom(0)) && atom2.matches(bondA1.getAtom(1)) || atom1.matches(bondA1.getAtom(1)) && atom2.matches(bondA1.getAtom(0))) {
    ...
    private static boolean isMatchFeasible(IAtomContainer ac1, IBond bondA1, IAtomContainer ac2, IBond bondA2, boolean shouldMatchBonds) {
        ...
        if (DefaultMatcher.isBondMatch(bondMatcher, ac2, bondA2, shouldMatchBonds) && DefaultMatcher.isAtomMatch(atomMatcher1, atomMatcher2, ac2, bondA2, shouldMatchBonds)) {
            return true;
        }
        return false;
    }
