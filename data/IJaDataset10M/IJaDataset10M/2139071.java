...
public class VisometryGraphics<C> {
    ...
    public void setPrimitiveStyle(PrimitiveStyle style) {
        if (style instanceof ArrowStyle) setArrowStyle((ArrowStyle) style); else if (style instanceof PathStyle) setPathStyle((PathStyle) style); else if (style instanceof PointStyle) setPointStyle((PointStyle) style); else if (style instanceof ShapeStyle) setShapeStyle((ShapeStyle) style); else if (style instanceof StringStyle) setStringStyle((StringStyle) style); else System.out.println("Unable to set style");
    }
    ...
    public void drawPoints(C[] coords, PrimitiveStyle<Point2D> style) {
        ...
        for (C c : coords) style.draw(gr, visometry.getWindowPointOf(c), selected);
    }
    ...
    public void drawPoints(C[] coords) {
        ...
        for (int i = 0; i < coords.length; i++) drawPoint(coords[i]);
    }
    ...
    public void drawSegments(C[][] coords, PathStyle style) {
        if (coords == null || coords.length <= 0) return;
        if (coords[0].length != 2) throw new IllegalArgumentException("drawSegments should be called with length 2 arrays: " + coords[0].length);
        for (C[] segment : coords) drawSegment(segment[0], segment[1], style);
    }
    ...
    public void drawPaths(C[][] paths, PathStyle style) {
        for (C[] path : paths) drawPath(path, 0, path.length, style);
    }
    ...
    public void drawPaths(C[][] paths) {
        for (int i = 0; i < paths.length; i++) drawPath(paths[i]);
    }
    ...
    Rectangle2D.Double getRectangle(C corner1, C corner2) {
        ...
        return new Rectangle2D.Double(Math.min(wp1.getX(), wp2.getX()), Math.min(wp1.getY(), wp2.getY()), Math.abs(wp2.getX() - wp1.getX()), Math.abs(wp2.getY() - wp1.getY()));
    }
    ...
    public void drawEllipse(C corner1, C corner2, ShapeStyle style) {
        Point2D wp1 = visometry.getWindowPointOf(corner1);
        Point2D wp2 = visometry.getWindowPointOf(corner2);
        shapeStyle.draw(gr, new Ellipse2D.Double(Math.min(wp1.getX(), wp2.getX()), Math.min(wp1.getY(), wp2.getY()), Math.abs(wp2.getX() - wp1.getX()), Math.abs(wp2.getY() - wp1.getY())), selected);
    }
    ...
    public void drawWinBorder(PrimitiveStyle<Shape> style) {
        style.draw(gr, new Rectangle2D.Double(getWindowMinX(), getWindowMinY(), getWindowWidth() - 1, getWindowHeight() - 1), selected);
    }
