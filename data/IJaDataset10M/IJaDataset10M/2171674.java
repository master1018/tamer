...
public class BlipContentRefs implements Iterable<Range> {
   ...
   */
    public static BlipContentRefs all(Blip blip, String target, int maxResult) {
        return new BlipContentRefs(blip, new BlipIterator.TextIterator(blip, target, maxResult), Parameter.of(ParamsProperty.MODIFY_QUERY, new DocumentModifyQuery(target, maxResult)));
    }
   ...
   * @return an instance of blip references.
   */
    public static BlipContentRefs all(Blip blip, ElementType target, int maxResult, Restriction... restrictions) {
        ...
        return new BlipContentRefs(blip, new BlipIterator.ElementIterator(blip, target, restrictionsAsMap, maxResult), Parameter.of(ParamsProperty.MODIFY_QUERY, new DocumentModifyQuery(target, restrictionsAsMap, maxResult)));
    }
   ...
   */
    public static BlipContentRefs all(Blip blip) {
        return new BlipContentRefs(blip, new BlipIterator.SingleshotIterator(blip, 0, blip.getContent().length()));
    }
   ...
   */
    public static BlipContentRefs range(Blip blip, int start, int end) {
        return new BlipContentRefs(blip, new BlipIterator.SingleshotIterator(blip, start, end), Parameter.of(ParamsProperty.RANGE, new Range(start, end)));
    }
   ...
   */
    ...
    private BlipContentRefs execute(ModifyHow modifyHow, List<BundledAnnotation> bundledAnnotations, Object... arguments) {
        if (!iterator.hasNext()) {
            return this;
        }
        int nextIndex = 0;
        ...
        while (iterator.hasNext()) {
            ...
            } else if ((start >= blip.length() || end > blip.length()) && modifyHow != ModifyHow.INSERT) {
                throw new IndexOutOfBoundsException("Position outside the blip.");
            ...
            switch(modifyHow) {
                case DELETE:
                    ...
                    Iterator<Integer> elementIterator = blip.getElements().subMap(start, end).keySet().iterator();
                    ...
                    iterator.shift(-1);
                    blip.setContent(blip.getContent().substring(0, start) + blip.getContent().substring(end));
                    break;
                case ANNOTATE:
                ...
                case REPLACE:
                    ...
                    blip.setContent(blip.getContent().substring(0, start) + text + blip.getContent().substring(end));
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs insert(List<BundledAnnotation> bundledAnnotations, BlipContent... arguments) {
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs insert(List<BundledAnnotation> bundledAnnotations, BlipContentFunction... functions) {
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs insertAfter(List<BundledAnnotation> bundledAnnotations, BlipContent... arguments) {
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs insertAfter(List<BundledAnnotation> bundledAnnotations, BlipContentFunction... functions) {
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs insertAfter(List<BundledAnnotation> bundledAnnotations, String... arguments) {
        Object[] array = new Plaintext[arguments.length];
        for (int i = 0; i < arguments.length; ++i) {
            array[i] = Plaintext.of(arguments[i]);
        }
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs replace(List<BundledAnnotation> bundledAnnotations, BlipContent... arguments) {
   ...
   * @return an instance of this blip references, for chaining.
   */
    public BlipContentRefs replace(List<BundledAnnotation> bundledAnnotations, BlipContentFunction... functions) {
   ...
   */
    public List<BlipContent> values() {
        ...
        while (iterator.hasNext()) {
            Range range = iterator.next();
            if (range.getEnd() - range.getStart() == 1 && blip.getElements().containsKey(range.getStart())) {
