...
public class DbConnectionPool {
    ...
    DbConnectionPool(Tag tagSQLConfig) {
        ...
        if (tagPools != null) {
            ...
            while (tagPool != null) {
                ...
                if (StringUtil.isEmpty(csPoolName)) csPoolName = "UnknownPoolName";
    ...
    private void addProgram(String csProgramId, String csParentProgramId, DbConnectionColl dbConnectionColl) {
        if (m_hashConnectionsByProgramId == null) m_hashConnectionsByProgramId = new Hashtable<String, DbConnectionColl>();
        ...
        } else m_hashConnectionsByProgramId.put(csProgramId, dbConnectionColl);
    }

    void releaseConnection(DbConnectionBase sqlConnection) {
        if (sqlConnection.m_dbConnectionColl != null) sqlConnection.m_dbConnectionColl.releaseConnection(sqlConnection);
    }

    public synchronized DbConnectionColl getConnectionCollForPref(String csProgramId, String csProgramParent) {
        ...
        if (m_hashConnectionsByProgramId != null) {
            ...
            if (connectionColl == null && !StringUtil.isEmpty(csProgramParent)) connectionColl = m_hashConnectionsByProgramId.get(csProgramId);
            if (connectionColl == null) connectionColl = m_hashConnectionsByProgramId.get("");
        }
        return connectionColl;
    }

    private String makeFullName(String csProgramId, String csProgramParent) {
        if (!StringUtil.isEmpty(csProgramId) && !StringUtil.isEmpty(csProgramParent)) return csProgramId + "$" + csProgramParent; else if (!StringUtil.isEmpty(csProgramId)) return csProgramId;
        return "";
    }
    ...
    public synchronized int getNbUnusedConnections() {
        if (m_hashConnectionsByProgramId == null) return 0;
        int n = 0;
        Collection<DbConnectionColl> colDbConnectionColl = m_hashConnectionsByProgramId.values();
        Iterator<DbConnectionColl> iterDbConnectionColl = colDbConnectionColl.iterator();
        while (iterDbConnectionColl.hasNext()) {
