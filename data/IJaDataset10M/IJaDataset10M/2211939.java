...
public abstract class SimpleConfigurableASTView extends AbstractSessionTimeViewPart {

    /**
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(SimpleConfigurableASTView.class);
    ...
    @Override
    protected void setData(ISession session, String modelName, double time, boolean isPostConflictResolution) {
        _currentConflict = isPostConflictResolution;
        _currentModel = modelName;
        _currentSession = session;
        _currentTime = time;
        _orientedComponent.setData(session, modelName, time, isPostConflictResolution);
    }
        ...
        _root.addControlListener(new ControlListener() {
            ...
            public void controlResized(ControlEvent e) {
                if (determineOrientation() != _currentOrientation) {
                    if (LOGGER.isDebugEnabled()) LOGGER.debug(String.format("Switching orientation!"));
                    ...
                    if (_currentSession == null) noData(); else setData(_currentSession, _currentModel, _currentTime, _currentConflict);
                }
            }
    ...
    protected void createActions() {
        _showPostConflictResolution = new Action("Pre Conflict", IAction.AS_CHECK_BOX) {
            ...
            public void run() {
                ...
                if (isChecked()) {
                    ...
                    setImageDescriptor(UIPlugin.getDefault().getImageDescriptor("postconflict"));
                } else {
                    setText("Pre Conflict");
                    setToolTipText("Press to see post conflict resolution");
                    setImageDescriptor(UIPlugin.getDefault().getImageDescriptor("preconflict"));
                }
                getViewSite().getActionBars().getToolBarManager().update(true);
            }
        };
        _showPostConflictResolution.setEnabled(true);
        _showPostConflictResolution.setImageDescriptor(UIPlugin.getDefault().getImageDescriptor("preconflict"));
    }
    ...
    protected void createToolbar() {
        ...
        if (isSensitiveToConflictResolution()) mgr.add(_showPostConflictResolution);
    }

    private Orientation determineOrientation() {
        Rectangle bounds = _root.getClientArea();
        if (LOGGER.isDebugEnabled()) LOGGER.debug(String.format("Bounds %d x %d", bounds.width, bounds.height));
    ...
    private IOrientedComponent instantiateOrientation(Orientation orientation) {
        ...
        if (orientation == Orientation.VERTICAL) component = instantiateVertical(); else component = instantiateHorizontal();
        component.createPartControl(_root);
        return component;
    }

    protected abstract IOrientedComponent instantiateVertical();

    protected abstract IOrientedComponent instantiateHorizontal();
    ...
    protected abstract boolean isSensitiveToConflictResolution();
}
