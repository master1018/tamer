...
public final class Directory extends Buffer {
    ...
    public Position getInitialDotPos() {
        ...
        while (true) {
            ...
            if (name != null) {
                if (name.equals("..")) upLine = line; else if (!name.equals(".")) return new Position(line, getNameOffset(line));
            }
            line = line.next();
        }
        if (upLine != null) return new Position(upLine, getNameOffset(upLine)); else return new Position(line, getNameOffset(line));
    }

    private void setLimitPattern(String s) {
        if (s == null || s.length() == 0) limitPattern = null; else limitPattern = s;
    }
    ...
    public static void dirLimit(String pattern) {
        final Editor editor = Editor.currentEditor();
        final Buffer buffer = editor.getBuffer();
        if (buffer instanceof Directory) ((Directory) buffer).limit(pattern);
    }
    ...
    public static void dirUnlimit() {
        ...
        if (buffer instanceof Directory) {
            Directory directory = (Directory) buffer;
            if (directory.getLimitPattern() != null) directory.limit(null);
        }
    }
    ...
    private void limit(String pattern) {
        ...
        if (pattern == null && limitPattern != null) reload = true; else if (pattern != null && !pattern.equals(limitPattern)) reload = true;
        if (reload) {
            final Editor editor = Editor.currentEditor();
            editor.setWaitCursor();
            String name = null;
            Line dotLine = editor.getDotLine();
            if (dotLine instanceof DirectoryLine) name = getName(dotLine);
            setLimitPattern(pattern);
            if (getListing() != null) reloadFromListing(); else reload();
            Line line = findName(name);
            Position pos;
            if (line != null) pos = new Position(line, getNameOffset(line)); else pos = getInitialDotPos();
    ...
    public void rescan() {
        ...
        if (file.isRemote()) DirectoryCache.getDirectoryCache().purge(file.getHostName());
        reload();
        if (loadError) return;
            ...
            if (ed.getBuffer() == this) {
                ...
                if (dotLine == null) {
                    ...
                    if (dotLine == null) {
                        ...
                        while (dotLine.next() != null) dotLine = dotLine.next();
                    }
                }
    ...
    private void addEntry(String name) {
        ...
        if (f == null) return;
        DirectoryEntry de;
        if (f.isDirectory()) de = new DirectoryEntry(name, f.lastModified(), 0, true); else de = new DirectoryEntry(name, f.lastModified(), f.length());
        if (!name.equals(".") && !name.equals("..")) {
            try {
                ...
                if (!cp.equals(ap)) {
                    ...
                    if (cp.startsWith(canonicalPath + LocalFile.getSeparator())) de.setLinkedTo(cp.substring(canonicalPath.length() + 1)); else de.setLinkedTo(cp);
                }
            } catch (IOException e) {
                Log.error(e);
            }
    ...
    private synchronized DirectoryEntry findNativeEntry(String string) {
        for (int i = 0; i < entries.size(); i++) {
            DirectoryEntry entry = (DirectoryEntry) entries.get(i);
            if (string.equals(entry.getString())) return entry;
        }
        return null;
    }
    ...
    private synchronized void sort() {
        ...
        if (sortBy == SORT_BY_NAME) sortByName(); else if (sortBy == SORT_BY_DATE) sortByDate(); else if (sortBy == SORT_BY_SIZE) sortBySize();
    }
    ...
    private void addEntriesToBuffer() {
        ...
        if (preferences.getBooleanProperty(Property.DIR_SORT_DIRECTORIES_FIRST, !usingNativeFormat)) {
            for (int i = 0; i < size; i++) {
                DirectoryEntry entry = (DirectoryEntry) entries.get(i);
                if (entry.isDirectory()) appendLine(entry);
            }
            for (int i = 0; i < size; i++) {
                DirectoryEntry entry = (DirectoryEntry) entries.get(i);
                if (!entry.isDirectory()) appendLine(entry);
        ...
        if (getFirstLine() == null) appendLine("");
        setLoaded(true);
    }
    ...
    private synchronized void cycleSortBy() {
        if (sortBy == SORT_BY_NAME) sortBy = SORT_BY_DATE; else if (sortBy == SORT_BY_DATE) sortBy = SORT_BY_SIZE; else sortBy = SORT_BY_NAME;
        resort();
    }
    ...
    private synchronized void loadInternal() {
        boolean useNativeFormat = preferences.getBooleanProperty(Property.DIR_USE_NATIVE_FORMAT, Platform.isPlatformUnix());
        if (useNativeFormat) {
            if (!Utilities.haveLs()) useNativeFormat = false;
        }
        loadError = false;
        try {
            final DirectoryFilenameFilter dff;
            if (limitPattern != null) dff = new DirectoryFilenameFilter(limitPattern); else dff = null;
            final File file = getFile();
            if (useNativeFormat || file.isRemote()) {
                usingNativeFormat = true;
                String flags = "-la";
                if (sortBy == SORT_BY_DATE) flags = "-lat"; else if (sortBy == SORT_BY_SIZE) flags = "-laS";
                ...
                if (getListing() != null) {
                    ...
                    if (getListing() != null) reader = new BufferedReader(new StringReader(getListing())); else loadError = true;
                } else {
                    ...
                    if (Platform.isPlatformUnix()) {
                        ...
                        if (cp.length() == 3 && cp.charAt(1) == ':' && cp.charAt(2) == '\\') cp = "//" + Character.toLowerCase(cp.charAt(0));
                ...
                if (Platform.isPlatformWindows()) {
                    String cp = file.canonicalPath();
                    if (cp.length() == 3 && cp.endsWith(":\\")) dirIsRoot = true;
                }
                if (!dirIsRoot) {
                    addEntry(".");
                    addEntry("..");
                ...
                if (names != null) {
                    if (dff == null) {
                        for (int i = 0; i < names.length; i++) addEntry(names[i]);
                    } else {
                        for (int i = 0; i < names.length; i++) {
                            if (dff.accepts(names[i])) addEntry(names[i]); else {
                                File f = File.getInstance(file, names[i]);
                                if (f != null && f.isDirectory()) addEntry(names[i]);
                            }
                        }
                    }
                }
            ...
            try {
                ...
                if (totalSize > 0) {
                    ...
                    for (int i = s.length(); i > 0; i--) sb.append('-');
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if (endOffset < text.length()) {
                ...
                if (text.charAt(endOffset) == ' ') {
                    ...
                    REMatch match = nativeMoveToFilenameRegExp.getMatch(text);
                    if (match == null) return totalSize = -1;
    ...
    public int load() {
        if (!isLoaded()) {
            ...
            if (editor != null) editor.status(reading);
            loadInternal();
            if (editor != null) editor.status(reading + "done");
        }
    ...
    public void tagFileAtDot() {
        ...
        if (!(line instanceof DirectoryLine)) return;
        String name = getName(line);
        if (name == null) return;
    ...
    public void upDir() {
        ...
        for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
            Editor ed = it.nextEditor();
            if (ed.getBuffer() == this) {
                if (line != null) ed.setDot(line, getNameOffset(line)); else ed.setDot(getFirstLine(), getNameOffset(getFirstLine()));
                ed.moveCaretToDotCol();
            }
        }
    ...
    public String getPathAtDot() {
        ...
        if (!(editor.getDotLine() instanceof DirectoryLine)) return null;
        String name = getName(editor.getDotLine());
        if (name == null) return null;
        final File dir = getFile();
        if (name.equals(".")) return dir.netPath();
        if (name.equals("..")) {
            File parent = dir.getParentFile();
            if (parent == null) return null; else return parent.netPath();
        }
        File file = File.getInstance(dir, name);
        if (file == null) return null; else return file.netPath();
    }

    public static void dir() {
        final Editor editor = Editor.currentEditor();
        final Buffer buffer = editor.getBuffer();
        if (buffer instanceof Directory) return;
        File directory = editor.getCurrentDirectory();
        if (directory == null) return;
        if (directory.getProtocol() == File.PROTOCOL_HTTP) return;
    ...
    private synchronized void openFileAtDot(boolean killDirectory) {
        final Editor editor = Editor.currentEditor();
        editor.setWaitCursor();
        if (!(editor.getDotLine() instanceof DirectoryLine)) return;
        String name = getName(editor.getDotLine());
        if (name == null) return;
        if (name.equals(".")) return;
        if (name.equals("..")) {
        ...
        if (dir.isRemote()) {
            ...
            if (de.isDirectory()) isDirectory = true; else if (de.isLink()) isDirectory = newFile.isDirectory();
            if (isDirectory) {
                setBusy(true);
                history.truncate();
                ...
                if (newFile instanceof FtpFile) buf = new RemoteBuffer((FtpFile) newFile, FtpSession.getSession((FtpFile) newFile)); else if (newFile instanceof SshFile) buf = new RemoteBuffer(newFile); else Debug.assertTrue(false);
                editor.makeNext(buf);
                editor.activate(buf);
            }
            return;
    ...
    private synchronized void changeDirectory(DirectoryHistoryEntry entry) {
        ...
        for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
            ...
            if (ed.getBuffer() == this) {
                ...
                ed.setUpdateFlag(REPAINT);
                if (line != null) ed.setDot(line, entry.offset); else ed.setDot(getFirstLine(), getNameOffset(getFirstLine()));
                ed.setMark(null);
                ed.moveCaretToDotCol();
                ed.updateLocation();
            }
    ...
    private void back(Editor editor) {
        ...
        } else editor.status("Can't go back");
    }
    ...
    private void forward(Editor editor) {
        ...
        if (entry != null) changeDirectory(entry); else editor.status("Can't go forward");
    }
    ...
    public void deleteFiles() {
        if (numMarked > 0) deleteMarkedFiles(); else deleteFileAtDot();
    }
    ...
    private synchronized void deleteMarkedFiles() {
        if (numMarked == 0) return;
        Debug.assertTrue(numMarked > 0);
        String message = null;
        final Editor editor = Editor.currentEditor();
        if (numMarked > 1) message = "Delete " + numMarked + " tagged files?"; else message = "Delete 1 tagged file?";
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            } else isDirectory = file.isDirectory();
            boolean succeeded = false;
            if (file instanceof FtpFile) {
                if (session == null) session = FtpSession.getSession((FtpFile) file);
                if (session != null) {
                    if (isDirectory) succeeded = session.removeDirectory(file.canonicalPath()); else succeeded = session.deleteFile(file.canonicalPath());
                }
            } else {
                file.delete();
            ...
            if (succeeded) {
                if (isDirectory) directoryWasDeleted = true;
            } else {
                if (isDirectory) {
                    confirmed = editor.confirm("Delete Files", "Unable to remove directory " + displayName + ".  Continue?");
                } else {
        ...
        if (directoryWasDeleted) {
            ...
            if (getFile().isRemote()) DirectoryCache.getDirectoryCache().purge(getFile());
            reload();
        }
    }
    ...
    private void deleteFileAtDot() {
        ...
        if (!editor.confirm(title, prompt)) return;
        boolean succeeded = false;
        editor.setWaitCursor();
        if (file instanceof FtpFile) {
        ...
        editor.setDefaultCursor();
        if (!succeeded) {
            if (isDirectory) showMessageDialog("Unable to remove directory " + displayName); else showMessageDialog("Unable to delete " + displayName);
        }
    }

    public static void dirDoShellCommand() {
        if (!Editor.checkExperimental()) return;
        if (!Platform.isPlatformUnix()) return;
        final Editor editor = Editor.currentEditor();
        final Buffer buffer = editor.getBuffer();
        if (buffer instanceof Directory) {
            if (buffer.getFile().isRemote()) {
                MessageDialog.showMessageDialog(editor, "Shell commands are not supported in remote directory buffers.", "Error");
                return;
            }
    ...
    private void doShellCommand(Editor editor) {
        ...
        String prompt = null;
        if (names.size() == 1) prompt = "Command (on " + (String) names.get(0) + "):"; else if (names.size() > 1) prompt = "Command (on " + names.size() + " tagged files):"; else prompt = "Command:";
    ...
    private List getSourceFiles(Editor editor) {
        ...
        if (numMarked > 0) {
            ...
            for (int i = 0; i < names.size(); i++) {
                ...
                if (name != null) sources.add(File.getInstance(getFile(), name));
            }
    ...
    private File getDestination(Editor editor, List sources, String operation) {
        ...
        if (sources.size() == 1) {
            ...
            name = source.getName();
            prompt += name;
        } else prompt += String.valueOf(sources.size()) + " tagged files";
        prompt += " to: ";
    ...
    private void copyFiles() {
        ...
        if (destination.isLocal()) copyLocalToLocal(sources, destination, editor, title); else MessageDialog.showMessageDialog(editor, "Destination must be local!", title);
    }
    ...
    private void copyLocalToLocal(List sources, File destination, Editor editor, String title) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            File to;
            if (destination.isDirectory()) to = File.getInstance(destination, from.getName()); else to = destination;
            ...
            if (Utilities.copyFile(from, to)) {
                ...
                if (i < limit - 1) {
                    text += " Continue?";
                    if (!editor.confirm(title, text)) break;
                } else MessageDialog.showMessageDialog(editor, text, title);
            }
        }
        String statusText = String.valueOf(numFilesCopied) + " file";
        if (numFilesCopied > 1) statusText += 's';
        statusText += " copied";
        ...
        for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
            ...
            if (destDir.equals(ed.getBuffer().getFile())) {
                ...
                if (ed != Editor.currentEditor()) ed.updateDisplay();
            }
        }
    }
    ...
    private void moveFiles() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!success) {
                ...
                } else Log.error("copyFile failed");
            } else Log.debug("renameTo succeeded");
            if (success) {
                for (BufferIterator it = new BufferIterator(); it.hasNext(); ) {
                    Buffer buf = it.nextBuffer();
                    if (source.equals(buf.getFile())) buf.changeFile(destination);
                }
            } else {
    ...
    public void getFileAtDot() {
        ...
        final Runnable successRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (cache != null) Utilities.deleteRename(cache, destination);
                for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
                    Editor ed = it.nextEditor();
                    if (ed.getBuffer() == Directory.this) ed.setDefaultCursor();
                }
            }
        };
        final ErrorRunnable errorRunnable = new ErrorRunnable("Operation failed") {
            ...
            public void run() {
                ...
                if (text == null || text.length() == 0) text = "Unable to retrieve " + name;
    ...
    public String getTitle() {
        ...
        title += " (sorted by ";
        if (sortBy == SORT_BY_NAME) title += "name)"; else if (sortBy == SORT_BY_DATE) title += "date)"; else if (sortBy == SORT_BY_SIZE) title += "size)";
        if (dir.isRemote()) title += "   " + dir.getHostName();
        return title;
    }
    ...
    private String getName(String s) {
        ...
        REMatch match;
        if (usingNativeFormat) match = nativeMoveToFilenameRegExp.getMatch(s); else match = internalMoveToFilenameRegExp.getMatch(s);
        if (match != null) return s.substring(match.getEndIndex());
        return null;
    }
    ...
    public static void chmod() {
        ...
        if (file.isLocal()) {
            ...
            if (protocol != File.PROTOCOL_FTP && protocol != File.PROTOCOL_SSH) return;
        }
            ...
            if (session != null) {
                final Runnable completionRunnable = new Runnable() {
                    ...
                    public void run() {
                        for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
                            Editor ed = it.nextEditor();
                            if (ed.getBuffer() == directory) ed.setDefaultCursor();
                        }
                    }
                };
                final Runnable chmodRunnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
                            Editor ed = it.nextEditor();
                            if (ed.getBuffer() == directory) ed.setWaitCursor();
                        }
    ...
    private int getNameOffset(Line line) {
        if (line != null) {
            REMatch match;
            if (usingNativeFormat) match = nativeMoveToFilenameRegExp.getMatch(line.getText()); else match = internalMoveToFilenameRegExp.getMatch(line.getText());
    ...
    private int getFileSizeEndOffset() {
        ...
        if (line != null) {
            ...
            REMatch match;
            if (usingNativeFormat) match = nativeMoveToFilenameRegExp.getMatch(text); else match = internalMoveToFilenameRegExp.getMatch(text);
            if (match != null) {
        ...
        if (name != null) {
            if (Platform.isPlatformWindows()) {
                ...
                for (Line line = getFirstLine(); line != null; line = line.next()) {
                    ...
                    if (text.indexOf(name) >= 0) if (name.equalsIgnoreCase(getName(line))) return line;
                }
            } else {
                for (Line line = getFirstLine(); line != null; line = line.next()) {
                    if (line.getText().indexOf(name) >= 0) if (name.equals(getName(line))) return line;
                }
            }
        }
        return null;
    ...
    public final String toString() {
        ...
        if (Platform.isPlatformUnix()) {
            ...
            if (userHome != null && userHome.length() > 0) {
                ...
                if (s.equals(userHome)) return "~";
                if (s.startsWith(userHome.concat("/"))) return "~".concat(s.substring(userHome.length()));
            }
        }
...
class DirectoryHistory {
    ...
    DirectoryHistoryEntry getPrevious() {
        ...
        if (index > 0) return (DirectoryHistoryEntry) v.get(--index);
        return null;
    }
    ...
    DirectoryHistoryEntry getNext() {
        ...
        if (index < v.size() - 1) return (DirectoryHistoryEntry) v.get(++index);
        return null;
    }
