...
public class SyncFromFSWorker extends SwingWorker<Boolean, Void> {
    ...
    private static final String LOAD_PROJECT_FILE_ERROR_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.error.load.project.file");

    private static final String INVALID_PROJECT_ROOT_ERROR_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.error.invalid.project.root");

    private static final String SYNC_SUBTREE_ERROR_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.error.subtree");

    private static final String SUBTREE_ROOT_NOT_EXIST_ERROR_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.error.subtree.not.exist");

    private static final String GENERAL_ERROR = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.default.error.message");

    private static final String DIAGRAM_LOAD_ERROR = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.diagram.load.error");

    private static final String NOTATION_IDENRIFIER = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.diagram.error.identifier");

    private static final String NOTATION_EXTENSION = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.diagram.error.extension");

    private static final String CANCELED_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.canceled");

    private static final String OFFSET_ERROR_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.offset.error");

    private static final String DIRECTORY_ERROR_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.directory.error");

    private static final String EXCEPTION_LABEL = ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.exception");
    ...
    public SyncFromFSWorker(final File projectFile, final List<String> pathOffset, final boolean addProjectItems, final boolean overwriteProjectItems, final boolean deleteProjectItems, final SyncDialog syncDialog) {
    ...
    public Boolean doInBackground() {
        showDialog();
        try {
            final ProjectRoot projectRoot = ModelerSession.getProjectControlService().loadProject(projectFile).getProjectRoot();
            if (projectRoot == null) {
                syncDialog.appendErrorInfo(LOAD_PROJECT_FILE_ERROR_LABEL, true);
                return false;
            }
            TreePath loadedProjectTreePath = null;
            for (final TreePath treePath : ModelerSession.getProjectService().getProjectPaths()) {
                ...
                if (projectRoot.getDisplayName().equals(loadedProjectRoot.getDisplayName()) && projectRoot.getProjectLocation().equals(loadedProjectRoot.getProjectLocation())) {
            ...
            if ((pathOffset != null) && (pathOffset.size() != 0)) {
                loadedProjectTreePath = syncPathWithOffset(projectFile.getParent(), pathOffset, loadedProjectTreePath, addProjectItems);
                if (loadedProjectTreePath == null) {
                    LOG.error("Not such a project in project navigation");
                    syncDialog.appendErrorInfo(INVALID_PROJECT_ROOT_ERROR_LABEL, true);
                    return false;
            ...
            return syncSubtreeToPN(loadedProjectTreePath, fsSubtreeLocation, projectRoot, addProjectItems, overwriteProjectItems, deleteProjectItems);
     ...
     */
    private TreePath syncPathWithOffset(final String parent, final List<String> pathOffset, TreePath loadedProjectTreePath, final boolean addProjectItems) throws InterruptedException {
        ...
        for (int i = 0; i < pathOffset.size(); i++) {
            ...
            if (file.isFile()) {
                ...
                if (longerTreePath == null) {
                    if (addProjectItems) {
                        AddProjectItemResult result = ModelerSession.getProjectControlService().addSubFolder(file.getName(), loadedProjectTreePath, false);
                        if (result.getStatus() != AddProjectItemStatus.SUCCESS) {
                            return null;
                        }
                        loadedProjectTreePath = ProjectServiceUtils.findTreePath(file, loadedProjectTreePath);
                    } else {
                        return null;
                    }
                } else {
                    loadedProjectTreePath = longerTreePath;
                }
            } else {
     ...
     */
    private boolean syncSubtreeToPN(final TreePath treePath, final File fsSubtreeLocation, final ProjectRoot projectRoot, final boolean addProjectItems, final boolean overwriteProjectItems, final boolean deleteProjectItems) throws InterruptedException {
        checkInterruption();
        ...
        if (fsSubtreeLocation.isFile() && ProjectServiceUtils.hasNotationFileExtension(name)) {
            ...
            if (childTreePath == null && addProjectItems) {
                ...
                final AddProjectItemResult result = ModelerSession.getProjectControlService().addDiagram(projectDiagram, treePath, false);
                if (result.getStatus() != AddProjectItemStatus.SUCCESS) {
                    syncDialog.appendErrorInfo(SyncWorkersUtils.reportDiagramError(result.getStatus(), fsSubtreeLocation, treePath, false), true);
                    return false;
                }
            } else if (childTreePath != null && overwriteProjectItems) {
                final ProjectDiagram projectDiagram;
                try {
                    projectDiagram = ModelerSession.getNotationService().getNotation(notationIdentifier).getLocalIOController().loadProjectDiagram(fsSubtreeLocation.getAbsolutePath());
                } catch (Exception exception) {
            ...
            if (fsSubtreeLocation.list() == null) {
                ...
                for (final String fsItem : fsSubtreeLocation.list()) {
                    ...
                    if (childFile.isFile()) {
                        ...
                        retValue &= syncSubtreeToPN(treePath, childFile, projectRoot, addProjectItems, overwriteProjectItems, deleteProjectItems);
                        ...
                        if (subfolderTreePath == null) {
                            if (addProjectItems) {
                                ...
                                if (result.getStatus() == AddProjectItemStatus.SUCCESS) {
                                    ...
                                    retValue &= syncSubtreeToPN(newSubfolderPath, childFile, projectRoot, addProjectItems, overwriteProjectItems, deleteProjectItems);
                                } else {
                                    syncDialog.appendErrorInfo(SyncWorkersUtils.reportDiagramError(result.getStatus(), childFile, treePath, false), true);
                                    retValue &= false;
                                }
                            }
                        } else {
                            retValue &= syncSubtreeToPN(subfolderTreePath, childFile, projectRoot, addProjectItems, overwriteProjectItems, deleteProjectItems);
                        }
                    }
                }
            }
    ...
    protected void done() {
        ...
        syncDialog.updatePosition(ModelerSession.getCommonResourceBundle().getString("project.service.sync.dialog.searching.done"));
        try {
            if (!isCancelled()) {
