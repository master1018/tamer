...
public class ShapeMultiPath extends MultiPath implements Shape {
	...
	*/
    public ShapeMultiPath(int dimension) {
        super(dimension);
        if (dimension < 2) throw new IllegalArgumentException("dimension >= 2 required");
    }
	...
	*/
    public void setBasisVectors(int[] b) {
        ...
        if (b0 < 0 || b1 < 0 || b0 >= dimension || b1 >= dimension) throw new IllegalArgumentException("basis vectors must be >= 0 and < dimension");
        ai0 = b0;
        ai1 = b1;
    }
	...
	*/
    public void setWindingRule(int rule) {
        if (rule != PathIterator.WIND_EVEN_ODD && rule != PathIterator.WIND_NON_ZERO) throw new IllegalArgumentException("winding rule must be WIND_EVEN_ODD or WIND_NON_ZERO");
        windingRule = rule;
    }
	...
	*/
    public boolean contains(double x, double y) {
        ...
        if (windingRule == PathIterator.WIND_NON_ZERO) return cross != 0;
        return (cross & 1) != 0;
    }
