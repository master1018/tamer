...
public class TinyStrings {
   ...
   */
    long create(long value) {
        ...
        if (l < 0 || l > MAX_STRING_LENGTH - 1) throw new IndexOutOfBoundsException("length must be between [0.." + (MAX_STRING_LENGTH - 1) + "]. But it is:" + l);
        long s = value >> LENGTH_BIT_SIZE;
        for (int i = 0; i < l; i++) {
            long t = s & CHAR_BIT_MASK;
            if (t < 0 || t >= TurkishAlphabet.ALPHABET_SIZE) throw new IllegalArgumentException("String connot contain characters outside TurkishAlphabet");
            s = s >>> CHAR_BIT_SIZE;
        }
        return value;
    }
   ...
   */
    public long create(String str) {
        ...
        for (int i = str.length() - 1; i >= 0; i--) {
            ...
            if (index == -1) throw new IllegalArgumentException("String connot contain characters outside TurkishAlphabet");
            if (i > 0) t = (t | index) << CHAR_BIT_SIZE; else t = (t | index) << LENGTH_BIT_SIZE;
        }
        t = t | str.length();
        return t;
    }
   ...
   */
    public long create(char c) {
        ...
        if (index == -1) throw new IllegalArgumentException("char:" + c + "cannot be outside TurkishAlphabet");
   ...
   */
    public char charAt(long l, int index) {
        if (index < 0 || index >= length(l)) throw new IndexOutOfBoundsException("index must be between [0.." + length(l) + "). " + index);
        l = l >>> LENGTH_BIT_SIZE;
