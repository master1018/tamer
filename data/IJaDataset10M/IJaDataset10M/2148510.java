...
public class HLNominalDistributionGui extends HLAttributeValueGui {
    ...
    public HLNominalDistributionGui(HLNominalDistribution dist, HLAttributeGui parent) {
        super(parent);
        if (dist != null) {
            possibleValuesNominal = new GuiPropertyStringIntegerMap(null, null, dist.getValuesAndFrequencies(), this, true);
        } else {
            possibleValuesNominal = new GuiPropertyStringIntegerMap(null, null, new HashMap<String, Integer>(), this, true);
        }
    }
    ...
    public JPanel getPanel() {
        if (panel == null) {
            ...
            label.setToolTipText("<HTML>Specifies the relative frequency of the nominal values<br>" + "If all frequencies are equal, then each value is equally probable</HTML>");
