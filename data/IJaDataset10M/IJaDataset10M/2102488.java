...
public class _4082GatheringtheHerbPouches extends QuestHandler {
    ...
    public boolean onItemUseEvent(QuestCookie env, Item item) {
        ...
        if (id != 182209058) return false;
        if (qs == null || qs.getQuestVarById(0) != 0) return false;
        ...
        if (targetId == 205190) {
            if (qs == null || qs.getStatus() == QuestStatus.NONE) {
                if (env.getDialogId() == 26) return sendQuestDialog(env, 1011); else if (env.getDialogId() == 1002) {
                    if (ItemService.addItems(player, Collections.singletonList(new QuestItems(182209058, 1)))) return defaultQuestStartDialog(env);
                    return true;
                } else return defaultQuestStartDialog(env);
            }
            if (qs != null && qs.getStatus() == QuestStatus.START) {
                if (env.getDialogId() == 26) return sendQuestDialog(env, 2375); else if (env.getDialogId() == 33) {
            ...
            } else if (qs != null && qs.getStatus() == QuestStatus.REWARD) return defaultQuestEndDialog(env);
            ...
            switch(targetId) {
                ...
                case 700432:
                    {
                        if (qs.getQuestVarById(0) == 0 && env.getDialogId() == -1) return true;
                    }
            }
        }
        return false;
