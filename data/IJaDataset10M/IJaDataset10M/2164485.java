...
class ProfiledData {
    ...
    /** number of profiled items. the array soften hold extra spaces */
    public String name;

    /** name of the problem */
    public int[] times;

    /** running times. low is better */
    public long[] memories;

    /** memory usage. low is better */
    public int[] ids;
    ...
    public ProfiledData(String name, int n) {
        ...
        this.ids = new int[n];
        this.best_time = this.best_memory = -1;
        this.count = 0;
    }
    ...
    public void insert(int id, int time, long mem) {
        ...
        times[index] = time;
        memories[index] = mem;
        if (count == 1 || best_memory == -1 || (best_memory != -1 && memories[best_memory] > mem)) best_memory = index;
        if (count == 1 || best_time == -1 || (best_time != -1 && times[best_time] > time)) best_time = index;
    }
