 ...
 * @version 1.0
 */
public class ClusterGenericDirectory extends ResourceTree implements Serializable, ResourceNotifier, Directory {
    ...
    protected static final String DEFAULT_CLUSTER_NAME = "default";
    ...
    private DirectorySessionManager sessionManager = null;

    private ResourceNode rootNode = null;

    protected static CommandLineParser parser = new CommandLineParser(null);
     ...
     */
    protected ClusterGenericDirectory(ClusterManagerID managerID, DirectoryResourceFactory factory, DirectoryType type, DirectorySessionManager sessionManager) throws ResourceException, DirectoryNotFoundException {
        super("tungsten");
        ...
        ResourceNode site = getResourceFactory().addInstance(ResourceType.SITE, siteName, getRootNode());
        getResourceFactory().addInstance(ResourceType.CLUSTER, clusterName, site);
    }
     ...
     */
    public String connect(String domain, long handle, String sessionID) throws DirectoryNotFoundException {
        DirectorySession session = sessionManager.connect(domain, handle, sessionID, type);
     ...
     *      long, java.lang.String)
     */
    public void disconnect(String domain, long handle, String sessionID) {
     ...
     *      long)
     */
    public void disconnectAll(String domain, long handle) {
     ...
     */
    public DirectorySession getSession(String sessionID) throws DirectoryNotFoundException {
     ...
     *      java.lang.String)
     */
    public String processCommand(String sessionID, String commandLine) throws Exception {
        ...
        if (command.equals(CD)) {
            ...
            result = formatEntries(entries, startNode, cmd.isLong(), cmd.isAbsolute());
            ...
            String destination = (source != null && params.length == 2) ? params[1] : null;
            ...
            throw new Exception("This interface to CONNECT is no longer supported");
        } else {
            if (isExecutable(sessionID, command)) {
                throw new DirectoryException(String.format("Cannot execute '%s' in this context.", command));
            }
        }
        return result;
    }
     ...
     * @see com.continuent.tungsten.commons.directory.Directory#disconnect(java.lang.String)
     */
    public synchronized void disconnect(String sessionID) throws DirectoryException {
     ...
     *      java.lang.String)
     */
    public String which(String sessionID, String path) throws DirectoryNotFoundException {
        ...
        String thePath = formatPath(getAbsolutePath(getRootNode(), node, true), true);
     ...
     *      int)
     */
    public ResourceNode locateRelative(String sessionID, int levels) throws DirectoryException, DirectoryNotFoundException {
     ...
     */
    public ResourceNode locateRelative(String sessionID, int levels, ResourceNode startNode) throws DirectoryException, DirectoryNotFoundException {
        if (levels == 0) return getCurrentNode(sessionID);
        ...
        for (int level = 0; level < levels; level++) {
            if ((foundNode = nodeToSearch.getParent()) != null) {
                ...
                throw new DirectoryException(String.format("No parent element found for '%s'", formatPath(getAbsolutePath(nodeToSearch, nodeToSearch, true), true)));
            }
        }
        return foundNode;
    }
     ...
     *      java.lang.String)
     */
    public ResourceNode locate(String sessionID, String path) throws DirectoryNotFoundException {
    ...
    public ResourceNode locate(String sessionID, String path, ResourceNode startNode) throws DirectoryNotFoundException {
        ...
        if (path.equals(CURRENT_ELEMENT)) return getCurrentNode(sessionID); else if (path.equals(ROOT_ELEMENT)) return getRootNode();
        ResourceNode foundNode = null;
        ResourceNode nodeToSearch = startNode;
        ...
        for (String element : pathElements) {
            ...
            if (element.equals(PARENT_ELEMENT)) {
                ...
                if (foundNode == null) {
                    throw new DirectoryNotFoundException(String.format("element '%s' not found in path '%s'", element, formatPath(getAbsolutePath(getRootNode(), nodeToSearch, true), true)));
                }
     ...
     */
    private ResourceNode getFirst(Map<String, ResourceNode> map) throws DirectoryNotFoundException {
        for (ResourceNode node : map.values()) return node;
        return null;
    }
     ...
     * @throws DirectoryNotFoundException
     */
    private ResourceNode getStartNode(String sessionID, String path) throws DirectoryNotFoundException {
     ...
     */
    public List<ResourceNode> ls(String sessionID, String path, boolean doRecurse) throws DirectoryNotFoundException {
        ResourceNode startNode = null;
     ...
     */
    public synchronized void cp(String sessionID, String sourcePath, String destinationPath) throws DirectoryException, DirectoryNotFoundException {
        ResourceNode destination = null;
        ResourceNode source = null;
        if (sourcePath == null || destinationPath == null) {
            throw new DirectoryException("cp: <source> <destination>: missing operand");
        }
        try {
            source = locate(sessionID, sourcePath);
        } catch (DirectoryNotFoundException c) {
            throw new DirectoryException(String.format("cp: the source element '%s' does not exist", sourcePath));
        }
        try {
            destination = locate(sessionID, destinationPath);
            if (!destination.getResource().isContainer() || destination.getResource().getType() == source.getResource().getType()) {
                throw new DirectoryException(String.format("cp: cannot copy over an existing element '%s'", destinationPath));
            }
        } catch (DirectoryNotFoundException c) {
        }
        if (destination == null) {
            if (lastElement(destinationPath).equals(elementPrefix(destinationPath))) {
        ...
        Resource copy = null;
        try {
            copy = resourceFactory.copyInstance(source, lastElement(destinationPath), destination);
        } catch (ResourceException c) {
            throw new DirectoryException(String.format("unable to create a copy of '%s', reason='%s'", sourcePath, c.getMessage()));
        }
     ...
     */
    public List<ResourceNode> getEntries(ResourceNode nodeToSearch, List<ResourceNode> entries, boolean doRecurse) {
     ...
     *      java.lang.String)
     */
    public synchronized ResourceNode cd(String sessionID, String path) throws DirectoryNotFoundException, DirectoryException {
        ResourceNode node = null;
        if (path == null) {
            node = getRootNode();
        } else {
        ...
        if (!node.isContainer()) {
            throw new DirectoryException(String.format("the element referenced by '%s' is not a container", path));
        }
     ...
     *      java.lang.String, boolean)
     */
    public synchronized ResourceNode create(String sessionID, String name, boolean createParents) throws DirectoryException, DirectoryNotFoundException {
     ...
     *      com.continuent.tungsten.commons.directory.ResourceNode)
     */
    public synchronized ResourceNode create(String sessionID, String name, ResourceNode parent) throws DirectoryException {
        ResourceNode newElement = null;
        if (name == null) {
            throw new DirectoryException(String.format("mkdir: missing name to create"));
        }
        try {
            newElement = resourceFactory.addInstance(parent.getResource().getChildType(), name, parent);
     ...
     */
    public synchronized ResourceNode create(String sessionID, String path, ResourceNode startNode, boolean createParents) throws DirectoryException {
        String pathElements[] = path.split(PATH_SEPARATOR);
        if (pathElements.length == 0) {
            throw new DirectoryException("missing operand: usage: create <path>");
        }
        ...
        for (int i = 0; i < pathElements.length - 1; i++) {
            ...
            try {
                ...
                if (createParents) {
                    ...
                    String message = String.format("element '%s' does not exist in path '%s'", pathElements[i], path);
                    logger.warn(message);
        ...
        createdNode = create(sessionID, pathElements[pathElements.length - 1], startNode);
        flush();
        return createdNode;
    }
     ...
     *      java.lang.String)
     */
    public synchronized void rm(String sessionID, String path) throws DirectoryNotFoundException, DirectoryException {
    ...
    public String formatPath(Directory directory, String sessionID, ResourceNode node) throws DirectoryNotFoundException {
        return directory.formatPath(directory.getAbsolutePath(directory, sessionID, node), true);
    }
     ...
     *      boolean)
     */
    public String formatPath(List<ResourceNode> pathElements, boolean reverseOrder) {
        StringBuilder builder = new StringBuilder();
        int elementCount = 0;
        for (ResourceNode element : pathElements) {
            String elementString = null;
            if (element.getResource() instanceof Operation) {
                elementString = (element.getResource() != null ? element.getResource().toString() : element.toString());
            } else {
                elementString = (element.getResource() != null ? element.getResource().toString() : element.toString());
            }
            if (reverseOrder) {
     ...
     *      boolean)
     */
    public String formatEntries(List<ResourceNode> entries, ResourceNode startNode, boolean detailed, boolean absolute) {
        StringBuilder builder = new StringBuilder();
        TreeMap<String, String> sortedEntries = new TreeMap<String, String>();
        for (ResourceNode entry : entries) {
            if (detailed) {
                Resource res = entry.getResource();
                String detail = (res != null ? res.describe(detailed) : entry.toString());
                sortedEntries.put(detail, detail);
            } else {
                boolean includeStartNode = (startNode == rootNode ? true : false) || absolute;
                if (absolute) {
                    startNode = rootNode;
                }
                String formattedEntry = formatPath(getAbsolutePath(startNode, entry, includeStartNode), true);
    ...
    public List<ResourceNode> getAbsolutePath(ResourceNode fromNode, ResourceNode toNode, boolean includeFromNode) {
        List<ResourceNode> absolutePath = new LinkedList<ResourceNode>();
        absolutePath.add(toNode);
        ResourceNode parent = toNode.getParent();
        while (parent != null) {
            if (parent == fromNode) {
                if (includeFromNode) absolutePath.add(parent);
                break;
            }
    ...
    public List<ResourceNode> getAbsolutePath(Directory directory, String sessionID, ResourceNode node) throws DirectoryNotFoundException {
     ...
     */
    public ResourceNode getCurrentNode(String sessionID) throws DirectoryNotFoundException {
     ...
     *      com.continuent.tungsten.commons.directory.ResourceNode)
     */
    public void setCurrentNode(String sessionID, ResourceNode currentNode) throws DirectoryNotFoundException {
     ...
     */
    public List<ResourceNode> getCwd(String sessionID) throws DirectoryNotFoundException {
        DirectorySession session = getSession(sessionID);
     ...
     */
    public synchronized Directory merge(Directory source, Directory destination) throws DirectoryException {
        merging = true;
    ...
    private void _merge(ResourceNode sourceNode, ResourceNode destinationNode, Integer nodesMerged) {
        Map<String, ResourceNode> sourceChildren = sourceNode.getChildren();
        Map<String, ResourceNode> destinationChildren = destinationNode.getChildren();
        for (ResourceNode sourceChild : sourceChildren.values()) {
            ResourceNode destinationChild = destinationChildren.get(sourceChild.getKey());
            if (destinationChild == null) {
                destinationChild = destinationNode.addChild(sourceChild.getResource());
                nodesMerged++;
            }
     ...
     * @return
     */
    protected String[] getParams(String[] args, boolean parseFlags) throws DirectoryException {
        if (args == null || args.length == 1) return null;
        int paramIndex = 1;
        if (args[paramIndex].startsWith("-") && parseFlags) {
            byte chars[] = args[paramIndex].getBytes();
            for (int i = 1; i < chars.length; i++) {
                if (chars[i] == FLAG_LONG) detailed = true; else if (chars[i] == FLAG_RECURSIVE) recursive = true; else if (chars[i] == FLAG_PARENTS) createParents = true; else if (chars[i] == FLAG_ABSOLUTE) absolute = true; else throw new DirectoryException(String.format("Unrecognized option '%c'", chars[i]));
            }
            paramIndex++;
        }
        if (args.length == paramIndex) return null;
    ...
    public String toString() {
        String ret = null;
        try {
            List<ResourceNode> listResult = ls(systemSessionID, ROOT_ELEMENT, true);
            Collections.sort(listResult, new ResourceNodeComparator());
            ret = formatEntries(ls(systemSessionID, ROOT_ELEMENT, true), null, detailed, false);
        } catch (Exception e) {
    ...
    class ResourceNodeComparator implements Comparator<ResourceNode>, Serializable {
    ...
    public static boolean isDirectoryCommand(String command) {
        for (String cmd : directoryCommands) {
            if (command.compareToIgnoreCase(cmd) == 0) return true;
        }
        return false;
    }
    ...
    public synchronized void flush() {
        ...
        DirectoryNotification notification = new DirectoryNotification(clusterName, memberName, getClass().getSimpleName(), getClass().getSimpleName(), ResourceState.MODIFIED, resourceProps);
        try {
            notifyListeners(notification);
        } catch (ResourceNotificationException r) {
            logger.error(String.format("Could not send directory synchronization request, reason=%s", r.getLocalizedMessage()), r);
        }
    }
     ...
     * @throws Exception
     */
    public ResourceNode getClusterConfNode(String sessionID, String siteName, String clusterName) throws DirectoryNotFoundException {
     ...
     * @throws Exception
     */
    public ResourceNode getClusterNode(String sessionID, String siteName, String clusterName) throws DirectoryNotFoundException {
     ...
     * @throws Exception
     */
    public ResourceNode getClusterResourceConfNode(String sessionID, String siteName, String clusterName, ResourceType resourceType) throws Exception {
        String path = String.format("/%s/%s/conf/%s", siteName, clusterName, resourceType.toString().toLowerCase());
        return locate(sessionID, path);
    }
     ...
     * @throws Exception
     */
    public ResourceNode getDataServiceConfNode(String sessionID, String siteName, String clusterName, String dataServiceName) throws Exception {
        String path = String.format("/%s/%s/conf/%s/%s", siteName, clusterName, ResourceType.DATASERVICE.toString().toLowerCase(), dataServiceName);
        try {
            return locate(sessionID, path);
    ...
    public void addDataService(String sessionID, String clusterName, String dataServiceName, TungstenProperties serviceProperties) throws Exception {
        logger.info(String.format("Adding DATASERVICE '%s:%s//%s'", siteName, clusterName, dataServiceName));
        ResourceNode dataServiceConfNode = getDataServiceConfNode(sessionID, siteName, clusterName, dataServiceName);
        ResourceConfiguration config = new ResourceConfiguration("global", serviceProperties);
        config.setExecutable(false);
        dataServiceConfNode.addChild(config);
    }

    public void addDataService(String sessionID, DataService dataService) throws Exception {
        logger.info(String.format("Adding DATASERVICE '%s'", dataService.getFqn()));
        ResourceNode dataServiceConfNode = getDataServiceConfNode(sessionID, siteName, clusterName, dataService.getName());
        ResourceConfiguration config = new ResourceConfiguration("global", dataService.getProperties());
        dataServiceConfNode.addChild(config);
        config.setExecutable(false);
        Map<String, DataSource> dsMap = dataService.getAllDataSources();
        for (String dsName : dsMap.keySet()) {
            config = new ResourceConfiguration(dsName, dsMap.get(dsName).toProperties());
            dataServiceConfNode.addChild(config);
            config.setExecutable(false);
        }
    }

    public TungstenProperties getDataServiceConfiguration(String sessionID, String clusterName, String dataServiceName) throws Exception {
        ResourceNode dataServiceConfNode = getDataServiceConfNode(sessionID, siteName, clusterName, dataServiceName);
        ResourceNode configNode = dataServiceConfNode.getChildren().get("global");
        if (configNode != null) {
            return ((ResourceConfiguration) configNode.getResource()).getProperties();
        }
        return null;
    }

    public void removeDataService(String sessionID, String clusterName, String dataServiceName) throws Exception {
        logger.info(String.format("Removing DATASERVICE '%s:%s//%s'", siteName, clusterName, dataServiceName));
        ResourceNode dataServiceConfNode = getDataServiceConfNode(sessionID, siteName, clusterName, dataServiceName);
        dataServiceConfNode.getParent().removeChild(dataServiceConfNode.getKey());
    }

    public void addDataSource(String sessionID, String clusterName, String dsName, TungstenProperties dataSourceProperties) throws Exception {
        logger.info(String.format("Adding DATASOURCE '%s'", dsName));
        ResourceNode dsConfNode = getClusterResourceConfNode(sessionID, siteName, clusterName, ResourceType.DATASOURCE);
        ResourceConfiguration config = new ResourceConfiguration(dataSourceProperties.getString(DataSource.NAME), dataSourceProperties);
        config.setExecutable(false);
        dsConfNode.addChild(config);
    }

    public void removeDataSource(String sessionID, String clusterName, String dsName) throws Exception {
