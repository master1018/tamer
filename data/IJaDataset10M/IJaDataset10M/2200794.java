...
public class Pair extends SchemeUtils {
    ...
    public boolean equals(Object x) {
        if (x == this) return true; else if (!(x instanceof Pair)) return false; else {
            Pair that = (Pair) x;
    ...
    void stringifyPair(boolean quoted, StringBuffer buf) {
        String special = null;
        if ((rest instanceof Pair) && rest(rest) == null) special = (first == "quote") ? "'" : (first == "quasiquote") ? "`" : (first == "unquote") ? "," : (first == "unquote-splicing") ? ",@" : null;
        if (special != null) {
            buf.append(special);
