...
public class SaslInputStream extends InputStream {
   ...
   */
    public int read() throws IOException {
        ...
        if (internalBuf != null && internalBuf.length > 0) {
            ...
            if (internalBuf.length == 1) internalBuf = new byte[0]; else {
   ...
   */
    public int read(byte[] b, int off, int len) throws IOException {
        if (Configuration.DEBUG) log.entering(this.getClass().getName(), "read", new Object[] { b, Integer.valueOf(off), Integer.valueOf(len) });
        if ((off < 0) || (off > b.length) || (len < 0) || ((off + len) > b.length) || ((off + len) < 0)) throw new IndexOutOfBoundsException("off=" + off + ", len=" + len + ", b.length=" + b.length);
        if (len == 0) {
            if (Configuration.DEBUG) log.exiting(this.getClass().getName(), "read", Integer.valueOf(0));
            return 0;
        }
        if (Configuration.DEBUG) log.finer("Available: " + available());
        int result = 0;
        if (internalBuf == null || internalBuf.length < 1) try {
            internalBuf = readSaslBuffer();
            if (internalBuf == null) {
                if (Configuration.DEBUG) {
                    log.finer("Underlying stream empty. Returning -1");
                    log.exiting(this.getClass().getName(), "read", Integer.valueOf(-1));
                }
                return -1;
            }
        ...
        if (len <= internalBuf.length) {
            ...
            if (len == internalBuf.length) internalBuf = null; else {
            ...
            int datalen;
            byte[] data;
            while (len > 0) if (source.available() > 3) {
                data = readSaslBuffer();
                if (data == null) {
                    if (Configuration.DEBUG) log.finer("Underlying stream exhausted. Breaking...");
                    break;
                }
                datalen = data.length;
                ...
                if (Configuration.DEBUG) log.finer("Not enough bytes in source to read a buffer. Breaking...");
                break;
            }
        }
        if (Configuration.DEBUG) {
   ...
   */
    private byte[] readSaslBuffer() throws IOException {
        if (Configuration.DEBUG) log.entering(this.getClass().getName(), "readSaslBuffer()");
        int realLength;
        byte[] result = new byte[4];
        try {
            realLength = source.read(result);
            if (realLength == -1) {
                if (Configuration.DEBUG) log.exiting(this.getClass().getName(), "readSaslBuffer");
                return null;
            }
        } catch (IOException x) {
            if (Configuration.DEBUG) log.throwing(this.getClass().getName(), "readSaslBuffer", x);
            throw x;
        }
        if (realLength != 4) throw new IOException("Was expecting 4 but found " + realLength);
        int bufferLength = result[0] << 24 | (result[1] & 0xFF) << 16 | (result[2] & 0xFF) << 8 | (result[3] & 0xFF);
        if (Configuration.DEBUG) log.finer("SASL buffer size: " + bufferLength);
        if (bufferLength > maxRawSendSize || bufferLength < 0) throw new SaslEncodingException("SASL buffer (security layer) too long");
        ...
        if (realLength != bufferLength) throw new IOException("Was expecting " + bufferLength + " but found " + realLength);
        if (Configuration.DEBUG) {
            log.finer("Incoming buffer (before security) (hex): " + Util.dumpString(result));
            log.finer("Incoming buffer (before security) (str): \"" + new String(result) + "\"");
        }
        if (client != null) result = client.unwrap(result, 0, realLength); else result = server.unwrap(result, 0, realLength);
        if (Configuration.DEBUG) {
            log.finer("Incoming buffer (after security) (hex): " + Util.dumpString(result));
            log.finer("Incoming buffer (after security) (str): \"" + new String(result) + "\"");
