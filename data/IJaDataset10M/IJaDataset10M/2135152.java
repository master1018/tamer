    ...
    private boolean convert(String csCopyClass) {
        ...
        if (bInOpened) {
            ...
            if (m_bHost) {
                ...
                if (m_nLengthRecord == 0) {
                    ...
                    if (m_bVariable4) {
                        ...
                        while (lineRead != null) {
                            ...
                            if (bEbcdicIn && !bEbcdicOut) varDefEncodingConvertibleManagerContainer.getEncodingManagerConvertAndWrite(lineRead, varRoot); else varRoot.setFromLineRead(lineRead);
                ...
                while (lineRead != null) {
                    if (bVariableLength) lineRead.shiftOffset(4);
                    if (bEbcdicIn && !bEbcdicOut) varDefEncodingConvertibleManagerContainer.getEncodingManagerConvertAndWrite(lineRead, varRoot); else varRoot.setFromLineRead(lineRead);
                    if (bVariableLength) lineRead.shiftOffset(-4);
                    m_fileOut.writeFrom(varRoot);
