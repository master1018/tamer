...
public class MySQLAdminInterfaceQueriesDAO implements IAdminInterfaceQueriesDAO {
    ...
    public Collection<AdminModelsQueryResult> performAdminModelsQuery(AdminModelsQuerySortingType sortingType) throws DAOException {
        ...
        try {
            String statementString = "SELECT model.*, COUNT(module.id) n_mod" + " FROM model" + " LEFT JOIN module ON module.model_id = model.id" + " GROUP BY model.id" + " " + orderingString;
    ...
    public Collection<AdminModulesQueryResult> performAdminModulesQuery(AdminModulesQuerySortingType sortingType, Long modelId) throws DAOException {
        ...
        try {
            String statementString = "SELECT model.*, module.*, COUNT(DISTINCT assignment.id) n_ass, COUNT(DISTINCT module_administrators.administrator_id) n_adm" + " FROM module" + " INNER JOIN model ON model.id = module.model_id" + " LEFT JOIN assignment ON assignment.module_id = module.id" + " LEFT JOIN module_administrators ON module_administrators.module_id = module.id" + " WHERE module.model_id = ?" + " GROUP BY module.id" + " " + orderingString;
            PreparedStatement statementObject = connection.prepareStatement(statementString);
