...
public class ContourWidget extends JPanel implements ActionListener, ChangeListener, ItemListener, ControlListener, ScalarMapListener {
    ...
    public ContourWidget(ScalarMap smap, float interv, float min, float max, float ba, float surf, boolean update) throws VisADException, RemoteException {
        ...
        Dimension d = new Dimension(Integer.MAX_VALUE, SurfaceLabel.getMaximumSize().height);
    ...
    void setMinMax(float min, float max) throws VisADException, RemoteException {
        if (!Util.isApproximatelyEqual(cLo, min) || !Util.isApproximatelyEqual(cHi, max)) {
            cLo = min;
            cHi = max;
    ...
    private void detectValues(double[] range) throws VisADException, RemoteException {
        ...
        if (!Util.isApproximatelyEqual(cInterval, fval[1]) || !Util.isApproximatelyEqual(cLo, fval[2]) || !Util.isApproximatelyEqual(cHi, fval[3]) || !Util.isApproximatelyEqual(cBase, fval[4])) {
            cInterval = fval[1];
            cLo = fval[2];
            cHi = fval[3];
            cBase = fval[4];
    ...
    public void controlChanged(ControlEvent e) throws VisADException, RemoteException {
        ...
        if (!Util.isApproximatelyEqual(fvals[2], cLo) || !Util.isApproximatelyEqual(fvals[3], cHi)) {
            cLo = fvals[2];
            cHi = fvals[3];
            updateWidgetRange();
    ...
    public void controlChanged(ScalarMapControlEvent evt) throws RemoteException, VisADException {
        int id = evt.getId();
        if (control != null && (id == ScalarMapEvent.CONTROL_REMOVED || id == ScalarMapEvent.CONTROL_REPLACED)) {
    ...
    class ContourRangeSlider extends RangeSlider implements ScalarMapListener {
        ...
        public void mapChanged(ScalarMapEvent e) {
            ...
            try {
                ...
                if (!Util.isApproximatelyEqual(cLo, minLimit) || !Util.isApproximatelyEqual(cHi, maxLimit)) {
                    cLo = minLimit;
                    cHi = maxLimit;
                }
