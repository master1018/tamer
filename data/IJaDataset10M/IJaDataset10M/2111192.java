...
public class MediaRenderer {
   ...
   */
    public static Dimension getSize(Property root, Graphics graphics) {
        ...
        if (source == null) return new Dimension();
        CacheEntry cached = CACHE.get(root);
        if (cached != null && cached.source.equals(source)) return cached.size;
    ...
    public static InputSource getSource(Property prop) {
        if (prop instanceof PropertyFile) {
            ...
            if (file != null && file.exists()) return InputSource.get(file);
            return null;
        }
        if (prop instanceof PropertyBlob) return InputSource.get(prop.toString(), ((PropertyBlob) prop).getBlobData());
        for (int i = 0; i < prop.getNoOfProperties(); i++) {
            ...
            if (child instanceof PropertyXRef) {
                ...
                if (e instanceof Media) {
                    ...
                    if (BLOB != null) return InputSource.get(prop.toString(), BLOB.getBlobData());
                    File file = m.getFile();
                    if (file != null) return InputSource.get(file);
                }
            }
            if ("OBJE".equals(child.getTag())) {
                ...
                if (file instanceof PropertyFile) {
                    ...
                    if (FILE != null && FILE.getFile() != null && FILE.getFile().exists()) return InputSource.get(FILE.getFile());
                }
            }
        }
        return null;
    ...
    public static void render(Graphics g, Rectangle bounds, Property root) {
        ...
        if (cached != null) {
            if (cached.source.equals(source)) {
                if (cached.size.width == 0 || cached.size.height == 0) return;
                image = cached.image.get();
                if (image != null) {
                    if ((cached.size.width >= bounds.width && image.getWidth(null) < bounds.width) || (cached.size.height >= bounds.height && image.getHeight(null) < bounds.height)) image = null;
                }
            }
        } else {
            cached = new CacheEntry();
                ...
                if (in != null) {
                    ...
                    if (iter.hasNext()) {
                        ...
                        try {
                            ...
                            param.setSourceSubsampling(Math.max(1, (int) Math.floor(cached.size.width / bounds.width)), Math.max(1, (int) Math.floor(cached.size.height / bounds.height)), 0, 0);
        ...
        if (image != null) g.drawImage(image, bounds.x, bounds.y, bounds.x + bounds.width, bounds.y + bounds.height, 0, 0, image.getWidth(null), image.getHeight(null), null);
    }
