...
public class Story {
    ...
    public Story(String[] args) {
        try {
            ...
            StringToken infile = new StringToken("if", "specify input story-file", "", TokenOptions.optDefault, "System.in");
    ...
    public void saveAudio(String filename, String markup) {
        ...
        for (Iterator<TaggedPiece> iterator = phrases.iterator(); iterator.hasNext(); ) {
    ...
    public void playAudioInOneFile(String markup) {
        String tmpWavFile = Emofilt._config.getString("tmpDir") + Emofilt._config.getString("tmpAudioFile");
            ...
            if (StringUtil.checkIfWordsContained(token)) {
                ...
                if (braceIndex >= 0) {
                    if (token.startsWith("/")) {
                        ...
                        String phrase = StringUtil.stripNewlines(token.substring(braceIndex + 1, token.length()).trim());
                        if (phrase.length() > 0) {
                            _logger.debug("token: " + token + ", emotion: " + emotionName + " , text: " + phrase);
    ...
    private void readIn(String filename) {
        ...
        try {
            ...
            for (Iterator<Element> iter = phrases.iterator(); iter.hasNext(); ) {
                ...
                String intensity = category.getAttribute("intensity").getValue();
