...
public class FDados extends FFilho implements ActionListener, KeyListener, InternalFrameListener, PostListener {
    ...
    public void setPKFoco() {
        for (int i = 0; i < lcCampos.getComponentCount(); i++) {
            if (((GuardaCampo) lcCampos.getComponent(i)).ehPK()) ((GuardaCampo) lcCampos.getComponent(i)).getComponente().requestFocus();
        }
    }
    ...
    public void keyPressed(KeyEvent kevt) {
        ...
        if (Ctrl) {
            if (kevt.getKeyCode() == KeyEvent.VK_R) btPrevimp.doClick(); else if (kevt.getKeyCode() == KeyEvent.VK_P) btImp.doClick();
        }
    }

    public void keyReleased(KeyEvent kevt) {
        if (kevt.getKeyCode() == KeyEvent.VK_SHIFT) Shift = false; else if (kevt.getKeyCode() == KeyEvent.VK_CONTROL) Ctrl = false;
    }
    ...
    public JLabelPad adicCampo(JTextFieldPad comp, int X, int Y, int Larg, int Alt, String nome, String label, byte key, JTextFieldFK txtDescFK, boolean req) {
        ...
        comp.setChave(key);
        if (txtDescFK == null) lcSeq.add(new GuardaCampo(comp, nome, label, key, req)); else lcSeq.add(new GuardaCampo(comp, nome, label, key, txtDescFK, req));
        if (navSeq != null) {
    ...
    public JLabelPad adicDBLiv(Component comp, String nome, String label, boolean req) {
        ...
        if (comp instanceof JRadioGroup<?, ?>) ((JRadioGroup<?, ?>) comp).setListaCampos(lcSeq); else if (comp instanceof JCheckBoxPad) ((JCheckBoxPad) comp).setListaCampos(lcSeq); else if (comp instanceof JTextAreaPad) ((JTextAreaPad) comp).setListaCampos(lcSeq);
    ...
    public void adicDBLiv(Component comp, int X, int Y, int Larg, int Alt, String nome, String label, boolean req) {
        ...
        if (comp instanceof JRadioGroup<?, ?>) ((JRadioGroup<?, ?>) comp).setListaCampos(lcSeq); else if (comp instanceof JCheckBoxPad) ((JCheckBoxPad) comp).setListaCampos(lcSeq); else if (comp instanceof JTextAreaPad) {
    ...
    public JLabelPad adicDB(Component comp, int X, int Y, int Larg, int Alt, String nome, String label, boolean req) {
        ...
        comp.setName(nome);
        if (comp instanceof JRadioGroup<?, ?>) ((JRadioGroup<?, ?>) comp).setListaCampos(lcSeq); else if (comp instanceof JCheckBoxPad) ((JCheckBoxPad) comp).setListaCampos(lcSeq); else if (comp instanceof JComboBoxPad) ((JComboBoxPad) comp).setListaCampos(lcSeq); else if (comp instanceof PainelImagem) ((PainelImagem) comp).setListaCampos(lcSeq); else if (comp instanceof JTextAreaPad) {
            ((JTextAreaPad) comp).setListaCampos(lcSeq);
            bScroll = true;
        }
        lcSeq.add(new GuardaCampo(comp, nome, label, ListaCampos.DB_SI, req));
        ...
        adic(lbTmp, X, Y - 20, Larg, 20);
        if (bScroll) adic(new JScrollPane(comp), X, Y, Larg, Alt); else adic(comp, X, Y, Larg, Alt);
        return lbTmp;
    }
