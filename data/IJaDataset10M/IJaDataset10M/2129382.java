...
public class Visualizer {
    ...
    public int getTreeDepth(Sign sign, int level) {
        ...
        if (children != null && sign.getWords().size() > 1) for (int i = 0; i < children.length; i++) {
            depth = getTreeDepth(children[i], level + 1);
            if (depth > max_depth) max_depth = depth;
        } else max_depth = level;
        return max_depth;
    }
    ...
    private int numberOfLeaves(Sign results) {
        ...
        for (int i = 0; i < children.length; i++) totalLeaves += numberOfLeaves(children[i]);
        return totalLeaves;
    }
    ...
    public List<TeXSign> processSign(Sign results, int level, int identation) {
        ...
        int offset = 0;
        if (children != null && results.getWords().size() > 1) for (int i = 0; i < children.length; i++) {
            if (i > 0) offset += numberOfLeaves(children[i - 1]);
    ...
    public boolean show(String fileName) {
        ...
        try {
            ...
            if (System.getProperty("os.name").toUpperCase().startsWith("WINDOWS")) viewerName = "yap"; else viewerName = "xdvi";
    ...
    class myFilter implements FileFilter {
        ...
        public boolean accept(File f) {
            ...
            if (f.getName().toUpperCase().startsWith(baseFileName)) return true; else return false;
        }
    }
    ...
    public boolean cleanFiles(String fileName) {
        try {
            ...
            for (int i = 0; i < allFiles.length; i++) allFiles[i].delete();
        } catch (Exception e) {
    ...
    public class auxFileNameFilter implements FilenameFilter {
        ...
        public boolean accept(File dir, String name) {
            if ((name.startsWith(fn)) && (!((name.toUpperCase().endsWith(".TEX")) || (name.toUpperCase().endsWith(".DVI"))))) return true; else return false;
        }
    }
    ...
    public boolean saveTeXFile(Sign results, String fileName) {
        ...
        try {
            ...
            bw.write(" \\\\\n\\uline{1}");
            for (i = 1; i < results.getWords().size(); i++) bw.write(" & \\uline{1}");
            bw.write(" \\\\\n");
            ...
            for (i = numDerivs; i < signList.size(); i++) {
                ...
                bw.write(ruleStr);
                for (int j = 0; j < texSign.identation; j++) bw.write("&");
    ...
    private List<TeXSign> sortList(List<TeXSign> signList) {
        for (int i = 0; i < signList.size(); i++) for (int j = i; j < signList.size(); j++) {
            ...
            if (texSign1.height == texSign2.height) if (texSign1.identation > texSign2.identation) {
                signList.set(i, texSign2);
                signList.set(j, texSign1);
            }
        }
