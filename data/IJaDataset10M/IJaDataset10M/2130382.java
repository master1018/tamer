...
public class StockMovementAction extends BaseDispatchAction {
    ...
    public ActionForward viewStock(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException, IOException {
        ...
        try {
            ArrayList<MProduct> productList = MMovementManager.getAvailableProducts(ctx, orgFromId, productName, description, barcode, null);
    ...
    public ActionForward viewOrEditMMovement(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward viewMMovement(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward viewMoveConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward viewMMovementLines(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward viewMoveLinesHistory(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward viewMoveConfirmLines(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward createMMovement(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException, IOException {
        ...
        String warehouseFromIdAsStr = request.getParameter("warehouseFromId");
        String warehouseToIdAsStr = request.getParameter("warehouseToId");
        int priceListId = Integer.parseInt(priceListIdAsStr);
        int movementId = Integer.parseInt(movementIdAsStr);
        Integer warehouseFromId = Integer.parseInt(warehouseFromIdAsStr);
        Integer warehouseToId = Integer.parseInt(warehouseToIdAsStr);
        ...
        stockMovementBean.setWarehouseFromId(warehouseFromId);
        stockMovementBean.setWarehouseToId(warehouseToId);
        ...
        try {
            ...
            for (ItemBean itemBean : itemList) {
                MMovementManager.createOrUpdateMaterialMovementLine(ctx, stockMovementBean.getMovementId(), itemBean.getProductId(), itemBean.getQtyToMove(), stockMovementBean.getWarehouseFromId(), stockMovementBean.getWarehouseToId(), trx.getTrxName());
            }
            trx.commit();
    ...
    public ActionForward completeMMovement(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            if (stockMovementBean.getMovementId() == null || stockMovementBean.getMovementId() == 0) {
                ...
                mmovement.setDescription(stockMovementBean.getReference());
                mmovement.save();
                trx.start();
                for (ItemBean itemBean : itemList) {
                    MMovementManager.createOrUpdateMaterialMovementLine(ctx, stockMovementBean.getMovementId(), itemBean.getProductId(), itemBean.getQtyToMove(), stockMovementBean.getWarehouseFromId(), stockMovementBean.getWarehouseToId(), trx.getTrxName());
                }
                MMovementManager.completeMaterialMovement(ctx, stockMovementBean.getMovementId(), stockMovementBean.getReference(), trx.getTrxName());
                trx.commit();
            } else {
        ...
        return new ActionForward("/StockMovementAction.do?action=viewMMovement&movementId=" + stockMovementBean.getMovementId());
    }

    public ActionForward completeMovement(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward completeMoveConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward setOrgFrom(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward setQtyToMove(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ActionForward fwd = init(mapping, form, request, response);
        if (fwd != null) {
            return fwd;
        }
    ...
    public ActionForward editInventoryMove(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ApplicationException, OperationException {
        ...
        String currSymboleSales = PriceListManager.getCurrency(ctx, priceListId);
        request.setAttribute(Constants.CURRENCY_SYMBOLE, currSymboleSales);
        MMovement move = new MMovement(ctx, smb.getMovementId(), null);
        MMovementCartBean mmovementBean = MMovementManager.getMMovementCartBean(ctx, smb.getMovementId(), priceListId);
        String docStatus = "";
        if ((MMovement.DOCSTATUS_Drafted).equals(move.getDocStatus())) {
            docStatus = "STATUS: DRAFTED";
        }
        if ((MMovement.DOCSTATUS_Completed).equals(move.getDocStatus())) {
            docStatus = "STATUS: COMPLETED";
        }
        if ((MMovement.DOCSTATUS_Closed).equals(move.getDocStatus())) {
            docStatus = "STATUS: CLOSED";
        }
        if ((MMovement.DOCSTATUS_InProgress).equals(move.getDocStatus())) {
            docStatus = "STATUS: IN PROGRESS";
        }
        if ((MMovement.DOCSTATUS_Approved).equals(move.getDocStatus())) {
        ...
        request.getSession().setAttribute(movementCart, mmovementBean);
        request.getSession().setAttribute(movementCartItems, mmovementBean.getItems());
        request.getSession().setAttribute(Constants.DOC_NO, move.getDocumentNo());
        request.getSession().setAttribute(Constants.DOC_STATUS, docStatus);
        request.getSession().setAttribute(Constants.DESCRIPTION, move.getDescription());
        request.getSession().setAttribute("warehouseFromId", mmovementBean.getWarehouseFromId());
        request.getSession().setAttribute("warehouseToId", mmovementBean.getWarehouseToId());
        return new ActionForward("/ViewStock.do");
    }
}
