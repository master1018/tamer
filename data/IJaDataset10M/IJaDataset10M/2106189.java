...
public class TableDemo {
    ...
    private static final String[] USAGE_MESSAGE = { "Usage: java TableDemo --username [user] --password [pass] ", "" };

    /** Help on setting up this demo. */
    private static final String[] WELCOME_MESSAGE = { "Using this demo, you can see how tables can be conveniently created,", "updated, queried, and deleted.", "", "Before starting this demo, make sure you create a spreadsheet." };

    /** Help on all available commands. */
    private static final String[] COMMAND_HELP_MESSAGE = { "Commands:", " load                   [[select a spreadsheet]]", " list                   [[shows all entries]]", " add title=Table1, summary=Test Table, worksheet=Sheet1, header=1, " + "startrow=2, numrows=5, columns=a:Name;b:Phone;c:Address " + "[[adds a table]]", " update <tableid> title=AwesomeTable [[updates a table]]", " delete <tableid>       [[deletes; use 'list' first]]", " search AwesomeTable    [[title search]]", " exit" };
   ...
   *        the title.
   */
    public TableEntry setEntryContentsFromString(TableEntry entryToUpdate, String nameValuePairs) {
        ...
        for (String nameValuePair : nameValuePairs.split(",")) {
            ...
            if (parts.length < 2) {
                System.out.println("Attributes are specified as key=value, " + "for example, title=Table1");
            }
            ...
            if (tag.equals("title")) {
            ...
            } else if (tag.equals("startrow") || tag.equals("insertionmode") || tag.equals("numrows")) {
                ...
                for (int i = 0; i < columns.length; i++) {
                    ...
                    if (colInfo.length < 2) {
                        System.out.println("Columns are specified as column:value, for " + " example, B:UpdatedPhone");
                    }
        ...
        Data data = getDataFromParams(entryToUpdate.getData(), dataParams, dataCols);
   ...
   */
    public Data getDataFromParams(Data data, Map<String, String> dataParams, Map<String, String> columnMap) {
   ...
   */
    public void addNewEntry(String nameValuePairs) throws IOException, ServiceException {
        TableEntry newEntry = setEntryContentsFromString(new TableEntry(), nameValuePairs);
   ...
   */
    public void printAndCacheEntry(TableEntry entry) {
        ...
        out.println("<worksheet>" + entry.getWorksheet().getName() + "</worksheet>");
        out.println("<header>" + entry.getHeader().getRow() + "</header>");
        Data data = entry.getData();
        out.println("<data> insertionMode=" + data.getInsertionMode().name() + " startRow=" + data.getStartIndex() + " numRows=" + data.getNumberOfRows());
        for (Column col : data.getColumns()) {
            out.println("     <column>" + col.getIndex() + " " + col.getName() + "</column>");
        }
        out.println("</data>");
    }
   ...
   */
    private int getIndexFromUser(BufferedReader reader, List entries, String type) throws IOException {
   ...
   *         Spreadsheets service.
   */
    public void search(String titleSearchString) throws IOException, ServiceException {
   ...
   */
    public void delete(String idToDelete) throws IOException, ServiceException {
        TableEntry entry = entriesCached.get(idToDelete);
        if (entry != null) {
            entry.delete();
            out.println("Deleted!");
        } else {
   ...
   */
    public static void main(String[] args) throws AuthenticationException {
        ...
        TableDemo demo = new TableDemo(new SpreadsheetService("Table Demo"), System.out);
