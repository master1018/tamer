...
public class MapSourceTestCase extends TestCase {
    ...
    public MapSourceTestCase(Class<? extends HttpMapSource> mapSourceClass) throws InstantiationException, IllegalAccessException {
    ...
    protected void runTest() throws Exception {
        ...
        if (mapSource instanceof AbstractHttpMapSource) try {
        ...
        try {
            ...
            if (imageData.length == 0) throw new MapSourceTestFailedException(mapSource, "Image data empty", c);
            ...
            switch(mapSource.getTileUpdate()) {
                ...
                case IfNoneMatch:
                    if (c.getHeaderField("ETag") == null) {
                        throw new MapSourceTestFailedException(mapSource, "No ETag present but map sources uses " + mapSource.getTileUpdate() + "\n", c);
                    }
                    break;
                case LastModified:
                    if (c.getHeaderField("Last-Modified") == null) throw new MapSourceTestFailedException(mapSource, "No Last-Modified entry present but map sources uses " + mapSource.getTileUpdate() + "\n", c);
                    break;
            }
        } finally {
            c.disconnect();
