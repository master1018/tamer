...
public final class StalledUploadWatchdog extends Periodic {
     ...
     */
    public static long DELAY_TIME = 1000 * 60 * 2;
    ...
    public boolean deactivate() {
        if (LOG.isDebugEnabled()) LOG.debug("Deactivated on " + closer.shutdownable);
        unschedule();
    ...
    public synchronized void activate(Shutdownable shutdownable) {
        if (LOG.isDebugEnabled()) LOG.debug("Activated on: " + shutdownable);
    ...
    private static class Closer implements Runnable {
        ...
        public void run() {
            ...
            if (toShut != null) {
                if (LOG.isDebugEnabled()) LOG.debug("STALLED!  Killing: " + toShut);
                toShut.shutdown();
                shutdownable = null;
            }
        }
