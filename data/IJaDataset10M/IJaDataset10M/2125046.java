    ...
    public void startElement(final String uri, final String localName, final String qName, final Attributes atts) throws SAXException {
        ...
        try {
            ...
            for (int i = 0; i < attsLen; i++) {
                ...
                output.write(new StringBuffer().append(STRING_BLANK).append(attQName).append(EQUAL).append(QUOTATION).append(attValue).append(QUOTATION).toString());
            }
            output.write(GREATER_THAN);
            if (atts.getValue(ATTRIBUTE_NAME_CLASS) != null) {
                if (atts.getValue(ATTRIBUTE_NAME_CLASS).indexOf(TOPIC_METADATA.matcher) != -1 && startTopic && !hasWritten) {
                    hasMetadataTillNow = true;
                    output.write(indexEntries);
                    hasWritten = true;
                }
                if (atts.getValue(ATTRIBUTE_NAME_CLASS).indexOf(TOPIC_PROLOG.matcher) != -1) {
                    hasPrologTillNow = true;
                }
            }
        } catch (final Exception e) {
