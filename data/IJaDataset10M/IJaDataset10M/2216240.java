...
public class HibernateArticleDAO extends HibernateDaoSupport implements ArticleDAO {
    ...
    /**
	 * @see org.contineo.core.document.dao.ArticleDAO#delete(int)
	 */
    public boolean delete(int articleid) {
        boolean result = true;
        try {
            Article article = (Article) getHibernateTemplate().get(Article.class, new Integer(articleid));
            if (article != null) getHibernateTemplate().delete(article);
        } catch (Exception e) {
            if (log.isErrorEnabled()) logger.error(e.getMessage(), e);
            result = false;
        }
        return result;
    }
    ...
    /**
	 * @see org.contineo.core.document.dao.ArticleDAO#findByDocId(int)
	 */
    @SuppressWarnings("unchecked")
    public Collection<Article> findByDocId(int docId) {
        Collection<Article> coll = new ArrayList<Article>();
        try {
            coll = (Collection<Article>) getHibernateTemplate().find("from org.contineo.core.document.Article _article where _article.docId = ? order by _article.articleDate", new Object[] { new Integer(docId) });
        } catch (Exception e) {
            if (log.isErrorEnabled()) logger.error(e.getMessage(), e);
        }
        return coll;
    ...
    /**
	 * @see org.contineo.core.document.dao.ArticleDAO#findByPrimaryKey(int)
	 */
    ...
    /**
	 * @see org.contineo.core.document.dao.ArticleDAO#store(org.contineo.core.document.Article)
	 */
    ...
    /**
	 * @see org.contineo.core.document.dao.ArticleDAO#findByUserName(java.lang.String)
	 */
    @SuppressWarnings("unchecked")
    public Collection<Article> findByUserName(String username) {
        Collection<Article> coll = new ArrayList<Article>();
        try {
            coll = (Collection<Article>) getHibernateTemplate().find("from org.contineo.core.document.Article _article where _article.username = ? order by _article.articleDate", new Object[] { username });
        } catch (Exception e) {
            if (log.isErrorEnabled()) logger.error(e.getMessage(), e);
        }
        return coll;
