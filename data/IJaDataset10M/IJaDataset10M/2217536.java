...
public class MacroBlock {
    ...
    MacroBlock(int x, int y, boolean keepDebugInfo) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                left = null;
                above = null;
                if (j > 0) left = ySubBlocks[j - 1][i];
                if (i > 0) above = ySubBlocks[j][i - 1];
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                left = null;
                above = null;
                if (j > 0) left = uSubBlocks[j - 1][i];
                if (i > 0) above = uSubBlocks[j][i - 1];
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                left = null;
                above = null;
                if (j > 0) left = vSubBlocks[j - 1][i];
                if (i > 0) above = vSubBlocks[j][i - 1];
    ...
    public void decodeMacroBlock(VP8Frame frame) throws IOException {
        ...
        if (mb.getSkipCoeff() > 0) {
            if (mb.getYMode() != Globals.B_PRED) mb.skipInnerLoopFilter = true;
    ...
    public void dequantMacroBlock(VP8Frame frame) {
        ...
        if (mb.getYMode() != Globals.B_PRED) {
            ...
            for (int x = 1; x < 16; x++) input[x] = sb.getTokens()[x] * acQValue;
    ...
    public int getSubblockX(SubBlock sb) {
        if (sb.getPlane() == SubBlock.PLANE.Y1) {
            for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++) if (ySubBlocks[x][y] == sb) return x;
        } else if (sb.getPlane() == SubBlock.PLANE.U) {
            for (int y = 0; y < 2; y++) for (int x = 0; x < 2; x++) if (uSubBlocks[x][y] == sb) return x;
        } else if (sb.getPlane() == SubBlock.PLANE.V) {
            for (int y = 0; y < 2; y++) for (int x = 0; x < 2; x++) if (vSubBlocks[x][y] == sb) return x;
    ...
    public int getSubblockY(SubBlock sb) {
        if (sb.getPlane() == SubBlock.PLANE.Y1) {
            for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++) if (ySubBlocks[x][y] == sb) return y;
        } else if (sb.getPlane() == SubBlock.PLANE.U) {
            for (int y = 0; y < 2; y++) for (int x = 0; x < 2; x++) if (uSubBlocks[x][y] == sb) return y;
        } else if (sb.getPlane() == SubBlock.PLANE.V) {
            for (int y = 0; y < 2; y++) for (int x = 0; x < 2; x++) if (vSubBlocks[x][y] == sb) return y;
        } else if (sb.getPlane() == SubBlock.PLANE.Y2) {
            return 0;
        }
        return -100;
    ...
    public void predictUV(VP8Frame frame) {
        ...
        switch(this.uvMode) {
            case Globals.DC_PRED:
                ...
                int expected_udc = 0;
                int expected_vdc = 0;
                if (x > 0) left_available = true;
                if (y > 0) up_available = true;
                if (up_available || left_available) {
                    ...
                    int shift = 2;
                    if (up_available) shift++;
                    if (left_available) shift++;
                ...
                for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++) ufill[x][y] = expected_udc;
                int vfill[][] = new int[4][4];
                for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++) vfill[x][y] = expected_vdc;
    ...
    public void predictY(VP8Frame frame) {
        ...
        switch(this.yMode) {
            case Globals.DC_PRED:
                ...
                for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++) fill[x][y] = expected_dc;
            ...
            case Globals.V_PRED:
                ...
                for (int x = 0; x < 4; x++) aboveYSb[x] = aboveMb.getYSubBlock(x, 3);
                for (int y = 0; y < 4; y++) {
                    for (int x = 0; x < 4; x++) {
                        SubBlock sb = ySubBlocks[x][y];
            ...
            case Globals.H_PRED:
                ...
                for (int x = 0; x < 4; x++) leftYSb[x] = leftMb.getYSubBlock(3, x);
                for (int y = 0; y < 4; y++) for (int x = 0; x < 4; x++) {
                    SubBlock sb = ySubBlocks[x][y];
                    int block[][] = new int[4][4];
                    for (int j = 0; j < 4; j++) for (int i = 0; i < 4; i++) {
                ...
                SubBlock[] leftUSb = new SubBlock[2];
                for (int x = 0; x < 2; x++) leftUSb[x] = leftMb.getYSubBlock(1, x);
                break;
            case Globals.TM_PRED:
                MacroBlock ALMb = frame.getMacroBlock(x - 1, y - 1);
