...
public class MetalInternalFrameTitlePane extends BasicInternalFrameTitlePane {
   ...
   * pane as appropriate.
   */
    class MetalInternalFrameTitlePanePropertyChangeHandler extends PropertyChangeHandler {
     ...
     */
        public void propertyChange(PropertyChangeEvent e) {
            ...
            if (propName.equals("JInternalFrame.isPalette")) {
                if (e.getNewValue().equals(Boolean.TRUE)) setPalette(true); else setPalette(false);
   ...
   */
     ...
     */
        public void layoutContainer(Container c) {
            ...
            title.setBounds(insets.left + 5, insets.top, Math.min(titlePreferredSize.width, loc - insets.left - 10), height);
        }
     ...
     */
        public Dimension preferredLayoutSize(Container c) {
            if (isPalette) return new Dimension(paletteTitleHeight, paletteTitleHeight); else return new Dimension(22, 22);
        }
   ...
   */
    protected void installDefaults() {
        ...
        title = new JLabel(frame.getTitle(), MetalIconFactory.getInternalFrameDefaultMenuIcon(), SwingConstants.LEFT);
    }
   ...
   */
    public void paintPalette(Graphics g) {
        ...
        if (UIManager.get("InternalFrame.activeTitleGradient") != null && frame.isSelected()) {
            MetalUtils.paintGradient(g, b.x, b.y, b.width, b.height, SwingConstants.VERTICAL, "InternalFrame.activeTitleGradient");
        }
        MetalUtils.fillMetalPattern(this, g, b.x + 4, b.y + 2, b.width - paletteCloseIcon.getIconWidth() - 13, b.height - 5, MetalLookAndFeel.getPrimaryControlHighlight(), MetalLookAndFeel.getBlack());
        Dimension d = getSize();
   ...
   */
    public void paintComponent(Graphics g) {
        Color savedColor = g.getColor();
        if (isPalette) paintPalette(g); else {
            paintTitleBackground(g);
            paintChildren(g);
            Dimension d = getSize();
            if (frame.isSelected()) g.setColor(MetalLookAndFeel.getPrimaryControlDarkShadow()); else g.setColor(MetalLookAndFeel.getControlDarkShadow());
            g.drawLine(0, 0, 0, 0);
            g.drawLine(d.width - 1, 0, d.width - 1, 0);
            g.drawLine(0, d.height - 1, d.width - 1, d.height - 1);
            if (UIManager.get("InternalFrame.activeTitleGradient") != null && frame.isSelected()) {
                MetalUtils.paintGradient(g, 0, 0, getWidth(), getHeight(), SwingConstants.VERTICAL, "InternalFrame.activeTitleGradient");
            }
            Rectangle b = title.getBounds();
            int startX = b.x + b.width + 5;
            int endX = startX;
            if (iconButton.isVisible()) endX = Math.max(iconButton.getX(), endX); else if (maxButton.isVisible()) endX = Math.max(maxButton.getX(), endX); else if (closeButton.isVisible()) endX = Math.max(closeButton.getX(), endX);
            endX -= 7;
            if (endX > startX) MetalUtils.fillMetalPattern(this, g, startX, 3, endX - startX, getHeight() - 6, Color.white, Color.gray);
        }
        g.setColor(savedColor);
    }
   ...
   */
    public void setPalette(boolean b) {
        ...
        if (isPalette) closeButton.setIcon(paletteCloseIcon); else closeButton.setIcon(closeIcon);
    }
