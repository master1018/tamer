...
public class Spell_WeaknessElectricity extends Spell {
    ...
    public void unInvoke() {
        ...
        if (canBeUninvoked()) mob.tell("Your electric weakness is now gone.");
        super.unInvoke();
    }
    ...
    public boolean okMessage(Environmental myHost, CMMsg msg) {
        ...
        MOB mob = (MOB) affected;
        if ((msg.amITarget(mob)) && (msg.targetMinor() == CMMsg.TYP_DAMAGE) && (msg.sourceMinor() == CMMsg.TYP_ELECTRIC)) {
    ...
    public boolean tick(Tickable ticking, int tickID) {
        ...
        if ((affecting() != null) && (affecting() instanceof MOB)) {
            ...
            if ((room != null) && (room.getArea().getClimateObj().weatherType(room) == Climate.WEATHER_THUNDERSTORM) && (CMLib.dice().rollPercentage() > M.charStats().getSave(CharStats.STAT_SAVE_ELECTRIC))) {
                ...
                if ((!M.isInCombat()) && (M != invoker) && (M.location() != null) && (M.location().isInhabitant(invoker)) && (CMLib.flags().canBeSeenBy(invoker, M))) CMLib.combat().postAttack(M, invoker, M.fetchWieldedItem());
            }
        }
        return true;
    }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        if (success) {
            ...
            if (mob.location().okMessage(mob, msg)) {
                ...
                if (msg.value() <= 0) success = maliciousAffect(mob, target, asLevel, 0, -1);
            }
        } else maliciousFizzle(mob, target, "<S-NAME> attempt(s) to invoke weakness to electricity, but fail(s).");
        return success;
    }
}
