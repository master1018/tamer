...
public class LogVerificationInputStream extends InputStream {
     ...
     * fileName.
     */
    LogVerificationInputStream(final EnvironmentImpl envImpl, final InputStream in, final String fileName, final long fileNum) {
        this.in = in;
     ...
     */
    private void verify(final byte[] buf, final int readOffset, final int readLen) throws LogVerificationException {
     ...
     */
    private int processFixedHeader(final byte[] buf, final int curOffset, final int remaining) throws LogVerificationException {
        assert header == null;
    ...
    private int processFileHeaderItem(final byte[] buf, final int curOffset, final int remaining) throws LogVerificationException {
        assert header != null;
        ...
        if (headerBuf.position() == maxSize) {
            ...
            fileHeaderEntry.readEntry(header, headerBuf, true);
     ...
     */
    private int processItem(final byte[] buf, final int curOffset, final int remaining) throws LogVerificationException {
        assert header != null;
    ...
    private LogVerificationException newVerifyException(String reason, Throwable cause) {
        state = State.INVALID;
        return new LogVerificationException("Log is invalid, fileName: " + fileName + " fileNumber: 0x" + Long.toHexString(fileNum) + " logEntryOffset: 0x" + Long.toHexString(entryStart) + " verifyState: " + state + " reason: " + reason, cause);
    }
}
