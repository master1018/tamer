...
public class Menu {
    ...
    protected VText makeLabel(MenuItem item, double xpos, double ypos) {
        VText retval = new VText(xpos, ypos, 0, Color.WHITE, item.getText());
    ...
    public void populate(List<MenuItem> items, Point2D.Double coords) {
        ...
        for (MenuItem item : items) {
            PRectangle menuBox = new PRectangle(xpos, ypos, 0, width, DEFAULT_ITEM_HEIGHT, Utils.makeDiagGradient((float) width, (float) DEFAULT_ITEM_HEIGHT, MENU_BGCOLOR));
            menuBox.setCursorInsidePaint(Utils.makeDiagGradient((float) width, (float) DEFAULT_ITEM_HEIGHT, MENU_HCOLOR));
