...
public class Script_Editor implements PlugIn {
    ...
    public void run(String path) {
        ...
        if (options != null) {
            if (path == null || path.equals("")) path = Macro.getValue(options, "path", null);
            if (path == null) path = Macro.getValue(options, "open", null);
    ...
    public void addToolsJarToClassPath() {
        try {
            if (Class.forName("com.sun.jdi.VirtualMachine") != null) return;
        } catch (ClassNotFoundException e) {
        }
        try {
            FijiClassLoader loader = (FijiClassLoader) IJ.getClassLoader();
            File tools_jar = new File(System.getProperty("java.home") + "/../lib/tools.jar");
            URL url;
            if (tools_jar.exists()) url = tools_jar.toURL(); else {
                String baseURL = gitwebURL + getPlatform() + ".git";
                url = new URL(baseURL + ";a=blob_plain;f=" + getNewestJavaSubdirectory(baseURL) + "/lib/tools.jar");
            }
