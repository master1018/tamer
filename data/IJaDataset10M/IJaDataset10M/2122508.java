...
public class TokenOperatorD3 extends TokenBase implements TokenOperator {
     ...
     */
        OPT {
            ...
            public String expand(String arg, List<Variable> vars, Parameters parameters) {
                for (Variable v : vars) {
                    if (parameters.exists(v.name())) return arg;
                }
                return "";
            }
        }
            ...
            String expand(String arg, List<Variable> vars, Parameters parameters) {
                ...
                for (Iterator<Variable> i = vars.iterator(); i.hasNext(); ) {
                    ...
                    if (parameters.exists(var.name())) {
                        String[] values = var.values(parameters);
                        for (String value : values) {
                            if (!first) expansion.append(arg); else first = false;
   ...
   */
    public TokenOperatorD3(Operator op, String arg, List<Variable> vars) throws NullPointerException {
        ...
        if (op == null || arg == null || vars == null) throw new NullPointerException("The operator must have a value");
        this._operator = op;
        this._arg = arg;
        this._vars = vars;
    }
   ...
   */
    public static Operator toOperator(String op) {
        for (Operator o : Operator.values()) {
            if (o.name().toLowerCase().equals(op)) return o;
        }
        return null;
    }
   ...
   */
    public static TokenOperatorD3 parse(String exp) throws URITemplateSyntaxException {
        ...
        if (parts.length != 3) throw new URITemplateSyntaxException(sexp, "An operator expansion must has three parts");
        String op = parts[0];
        String arg = parts[1];
        String vars = parts[2];
        if (!op.startsWith("-")) throw new URITemplateSyntaxException(op, "An operator must start with '-'");
        Operator operator = TokenOperatorD3.toOperator(op.substring(1));
        if (operator == null) throw new URITemplateSyntaxException(op, "This operator is not supported");
