...
public class MDD {
    ...
    public MDD(IntVar[] vars, int[] diagram, int[] domainLimits) {
        ...
        for (int i = 0; i < vars.length; i++) {
            ...
            if (domainLimits[i] < vars[i].domain.getSize()) throw new IllegalArgumentException("domain limites are smaller than actual domain of an mdd.");
	...
	**/
    ...
    public void addTuple(int[] tuple) {
        ...
        for (int value : tuple) {
            ...
            if (diagram[nodePosition] == NOEDGE) {
                if (varNo == tuple.length) diagram[nodePosition] = TERMINAL; else {
    ...
    public void ensureSize(int size) {
        if (diagram.length < size) {
            int[] newDiagram;
            if (diagram.length * 2 < size) newDiagram = new int[size * 2]; else newDiagram = new int[diagram.length * 2];
    ...
    private void shrink() {
        ...
        if (reducedNodes.isEmpty()) {
            ...
            while (!reducedNodes.isEmpty()) {
                ...
                System.arraycopy(diagram, range[shiftPosition - 1] + previousShift, shrankDiagram, positionInShrankDiagram, range[shiftPosition] - range[shiftPosition - 1] - previousShift);
            ...
            if (positionInShrankDiagram < shrankDiagram.length) System.arraycopy(diagram, range[shiftPosition - 1] + previousShift, shrankDiagram, positionInShrankDiagram, shrankDiagram.length - positionInShrankDiagram);
    ...
    private void mtree(int[][] table) {
        ...
        for (int[] tuple : table) {
            ...
            for (int i = 0; i < tuple.length; i++) {
                ...
                ensureSize(nodePosition + 1);
                if (diagram[nodePosition] == NOEDGE) {
                    if (i + 1 == tuple.length) diagram[nodePosition] = TERMINAL; else {
                        diagram[nodePosition] = freePosition;
    ...
    public int findPosition(int value, int[] values) {
        ...
        while (!(left + 1 >= right)) {
            if (debugAll) System.out.println("left " + left + " right " + right + " position " + position);
    ...
    protected int findRange(int value, int[] values) {
        ...
        if (values[right] <= value) return right; else return left;
    }
    ...
    private int reduce(int node, int level) {
        ...
        if (same[level][numberOfChildren] != null) for (int j = same[level][numberOfChildren].size() - 1; j >= 0; j--) {
            int[] currentNode = same[level][numberOfChildren].get(j);
            boolean equal = true;
            for (int i = currentNode.length - 1; i >= 0 && equal; i--) if (currentNode[i] != nodeChildren[i]) equal = false;
            if (equal) {
    ...
    public String toString() {
        ...
        for (int i = 0; i < diagram.length && i < freePosition; i++) buffer.append(String.valueOf(diagram[i])).append(" ");
