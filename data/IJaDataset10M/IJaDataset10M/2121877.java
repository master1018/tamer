...
abstract class IIRFilterType extends FilterType {
    ...
    void getResponse(double w, Complex c) {
        ...
        off &= ~1;
        if (off < 0) off = 0;
        if (off >= response.length) off = response.length - 1;
    ...
    void setResponse(DirectFilter f) {
        ...
        double maxresp = 0;
        f.bList[0] = 1;
        if (f.aList.length != f.bList.length) System.out.println("length mismatch " + f.aList.length + " " + f.bList.length);
        for (j = 0; j != response.length; j += 2) {
            ...
            top.div(bottom);
            if (top.mag > maxresp) maxresp = top.mag;
            response[j] = top.re;
            response[j + 1] = top.im;
        }
        for (j = 0; j != response.length; j++) response[j] /= maxresp;
    ...
    void setResponse(CascadeFilter f) {
        ...
        for (j = 0; j != response.length; j += 2) {
            ...
            ch.div(cbot);
            if (ch.mag > maxresp) maxresp = ch.mag;
            response[j] = ch.re;
            response[j + 1] = ch.im;
        }
        for (j = 0; j != response.length; j++) response[j] /= maxresp;
    ...
    Filter genFilter() {
        ...
        for (i = s = 0; i != n; i++) {
            getPole(i, c1);
            if (Math.abs(c1.im) < 1e-6) c1.im = 0;
            if (c1.im < 0) continue;
            if (c1.im == 0) {
                double cc0 = -c1.re;
                f.setAStage(-cc0, 0);
        ...
        for (i = s = 0; i != n; i++) {
            ...
            if (Math.abs(c1.im) < 1e-6) c1.im = 0;
            if (c1.im < 0) continue;
            if (c1.im == 0) f.setBStage(-c1.re, 1, 0); else {
                double cc0 = -2 * c1.re;
                double cd0 = c1.magSquared();
