   ...
   */
    ...
    public void resizeColumnWidth(int maxCellWidth) {
        ...
        for (int col = tm.getColumnCount() - 1; col >= 0; col--) {
            ...
            int cellWidth = renderer.getTableCellRendererComponent(this, column.getHeaderValue(), false, false, -1, col).getPreferredSize().width;
            renderer = getDefaultRenderer(tm.getColumnClass(col));
            for (int row = tm.getRowCount() - 1; row >= 0; row--) {
                int tmpWidth = renderer.getTableCellRendererComponent(this, tm.getValueAt(row, col), false, false, row, col).getPreferredSize().width;
                if (tmpWidth > cellWidth) cellWidth = tmpWidth;
            }
            cellWidth += 2;
            if ((maxCellWidth != -1) && (cellWidth > maxCellWidth)) cellWidth = maxCellWidth;
