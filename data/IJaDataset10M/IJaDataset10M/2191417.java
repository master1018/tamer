...
public class CharSequenceIterator implements Iterator<Character> {
    ...
    public CharSequenceIterator(CharSequence sequence, int begin, int end) {
        _sequence = sequence;
        if (begin < 0) throw new IllegalArgumentException("Beginning index cannot be less than 0.");
        _begin = begin;
        if (end > _sequence.length()) throw new IllegalArgumentException("End index cannot be greater than sequence length.");
        _end = end;
        reset();
    }
