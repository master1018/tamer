...
public class TransformationUtils {
    ...
    public static void checkAllProgramElementsInLists(List<CompilationUnit> cus, Collection<TraceItem> pes) {
        ...
        for (TraceItem ti : pes) allPEs.add(ti);
        ...
        if (allPEs.size() > 0) System.exit(-1);
    }
    ...
    private static void checkParentLinks(Set<TraceItem> pes) {
        for (TraceItem ti : pes) {
            if (ti.pe.getASTParent() == null) throw new RuntimeException("Null Parent link:\n" + ti);
            if (ti.pe.getASTParent().getIndexOfChild(ti.pe) == -1) throw new RuntimeException("No valid parent link:\n" + ti);
        }
    }
}
