...
public class BinaryLoader {
    ...
    public static void loadBin(Machine machine, File file, Address address) throws IOException {
        ...
        try {
            machine.memory.writeMemory(address, input, machine.address.getMaxAddr());
    ...
    public static void loadElks(Machine machine, File file) throws IOException {
        ...
        MemoryAccessProtection resmem = (MemoryAccessProtection) machine.getDevice(MemoryAccessProtection.class);
        if (resmem != null) {
            resmem.setProtection(tstart, tlen, MemoryAccessProtection.PROT_EXEC | MemoryAccessProtection.PROT_READ);
            resmem.setProtection(dstart, dlen, MemoryAccessProtection.PROT_READ_WRITE);
            resmem.setProtection(bstart, blen, MemoryAccessProtection.PROT_READ_WRITE);
            int length = 0xA00000 - sstart.getLinearAddress();
            resmem.setProtection(sstart, length, MemoryAccessProtection.PROT_READ_WRITE);
        }
