...
public class kungfum {
    ...
    public static VhConvertColorPromPtr kungfum_vh_convert_color_prom = new VhConvertColorPromPtr() {
        ...
        public void handler(char[] palette, char[] colortable, char[] color_prom) {
            ...
            for (i = 0; i < 512; i++) {
                for (j = 0; j < used; j++) {
                    if (allocated[j] == color_prom[i] && allocated[j + 256] == color_prom[i + 512] && allocated[j + 2 * 256] == color_prom[i + 2 * 512]) break;
                }
                if (j == used) {
    ...
    public static VhStartPtr kungfum_vh_start = new VhStartPtr() {

        public int handler() {
            if ((dirtybuffer = new char[videoram_size[0]]) == null) return 1;
    ...
    public static VhStopPtr kungfum_vh_stop = new VhStopPtr() {
        ...
        public void handler(osd_bitmap bitmap) {
            ...
            for (offs = videoram_size[0] - 1; offs >= 0; offs--) {
                if (dirtybuffer[offs] != 0) {
                    ...
                    sx = 8 * (offs % 64);
                    sy = 8 * (offs / 64);
                    drawgfx(tmpbitmap, Machine.gfx[0], videoram.read(offs) + 4 * (colorram.read(offs) & 0xc0), colorram.read(offs) & 0x1f, colorram.read(offs) & 0x20, 0, sx, sy, null, TRANSPARENCY_NONE, 0);
                }
            }
            {
                int[] scroll = new int[32];
                for (i = 0; i < 6; i++) scroll[i] = -128;
                for (i = 6; i < 32; i++) scroll[i] = -(kungfum_scroll_low.read() + 256 * kungfum_scroll_high.read()) - 128;
            ...
            for (offs = 0; offs < spriteram_size[0]; offs += 8) {
                ...
                char sprite_height_prom[] = { 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x01, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01 };
                ...
                if (code != 0 || bank != 0) {
                    ...
                    do {
                        drawgfx(bitmap, Machine.gfx[1 + bank], code + k, col, flipx, 0, sx, sy + 16 * k, spritevisiblearea, TRANSPARENCY_PEN, 0);
                        k--;
                    } while (k >= 0);
                }
            }
