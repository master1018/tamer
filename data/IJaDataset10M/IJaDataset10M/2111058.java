...
public class POperator extends AbstractPNode {
    ...
    public boolean equals(PNodeI node) {
        ...
        if (args.length != nodeOp.args.length) return false;
        for (int i = 0; i < args.length; ++i) if (!args[i].equals(nodeOp.args[i])) return false;
        return true;
    }
	...
	*/
    ...
    public String toString() {
        if (args.length == 1) {
            if (op.isPrefix()) return "(" + op.getSymbol() + args[0].toString() + ")";
        }
        if (args.length == 2) {
    ...
    public Node toNode() throws ParseException {
        ...
        for (int i = 0; i < args.length; ++i) funargs[i] = args[i].toNode();
    ...
    public PNodeI expand() throws ParseException {
        ...
        for (int i = 0; i < args.length; ++i) newTerms[i] = args[i].expand();
