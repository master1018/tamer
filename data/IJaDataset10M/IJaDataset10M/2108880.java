...
public class Generate {
   ...
   */
    private void generateTopics(OSLSchema schema, Collection ttypes, ClassInstanceIndexIF index) {
   ...
   */
    private void generateAssociations(OSLSchema schema, Collection atypes, ClassInstanceIndexIF index) {
        Iterator it = atypes.iterator();
        while (it.hasNext()) {
            TopicIF atype = (TopicIF) it.next();
            AssociationClassAnalyzer analyzer = new AssociationClassAnalyzer(schema, atype, index.getAssociations(atype));
            analyzer.analyze();
        }
    }
    ...
    public static void main(String[] argv) {
        ...
        try {
            ...
            if (args[1] != null) new OSLSchemaWriter(new File(args[1]), "utf-8").write(schema); else System.err.println("Error: No schema filename given.");
        } catch (IOException e) {
