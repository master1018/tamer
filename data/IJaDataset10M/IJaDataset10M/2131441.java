...
public class ComponentPropertyAssertionProbe<C extends Component, P> implements Probe {
    ...
    public void describeTo(Description description) {
        description.appendDescriptionOf(selector).appendText("\nand check that its ").appendDescriptionOf(propertyValueQuery).appendText(" is ").appendDescriptionOf(propertyValueMatcher);
    }

    public void describeFailureTo(Description description) {
        selector.describeFailureTo(description);
        if (selector.isSatisfied()) {
            description.appendText("\n    ").appendDescriptionOf(propertyValueQuery).appendText(" was ").appendValue(propertyValue);
        }
    }
}
