...
public class ApproximationAlgorithms {
    ...
    public static void approx_vertex_cover_ED(Graph g) {
        AttributeExplorer.changeAllNodesFlag(g, Node.FLAGS_NONE, Node.FLAGS_MARKED);
        AttributeExplorer.setAllEdgesExtra1(g, 1);
    ...
    private static int vc_add_node(Node n) {
        int changed = 0;
    ...
    private static void test_is_vertex_cover(Graph g) {
        for (Enumeration e = g.getEdges().elements(); e.hasMoreElements(); ) {
            Edge ed = (Edge) e.nextElement();
            Test.check((ed.n1.flags & Node.FLAGS_MARKED) != 0 || (ed.n2.flags & Node.FLAGS_MARKED) != 0, "Edge " + ed.getLabel() + " covered");
        }
    }
    ...
    public static void internal_test() {
        ...
        approx_vertex_cover_ED(g);
        test_is_vertex_cover(g);
        approx_vertex_cover_MDG(g);
        test_is_vertex_cover(g);
