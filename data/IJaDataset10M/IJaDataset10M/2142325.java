...
public class DOInputFileDB extends DOBaseForm {
    ...
    public String getHtmlCode(DOIModel property) {
        ...
        buffer.append("<a href='").append(DOGlobals.PRE_FULL_FOLDER).append("file/downloadfile_db.jsp?name=").append(fm.getRelationProperty().getColName()).append("&rename=").append(renameValue).append("&thisuuid=").append(fm.getData().getUid()).append("&bouid=").append(fm.getData().getBo().getObjUid());
        if (downLoadSizeCol != null) {
            buffer.append("&downLoadSizeCol=").append(downLoadSizeCol);
        }
        buffer.append("'>下载</a>");
