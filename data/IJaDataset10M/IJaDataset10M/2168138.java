...
public class Bonne extends WellKnownProjections {

    public static String ABBREVIATION = "BONNE";

    public Bonne() {
        super("Winkel 1", ABBREVIATION);
    }

    public Bonne(Document doc, Element element) throws IOException {
        this();
        load(doc, element);
    }

    public Bonne(AngularUnit angularunit, LinearUnit linearunit, Ellipsoid ellipsoid, double CentralMeridian, double stndard_paral, double false_e, double false_n) {
        this();
    ...
    private void initBonne() {
        ...
        if (super.theellipsoid.isSphere()) {
            e = MathTool.cotan(standard_parallel_rad);
        } else {
            g = super.theellipsoid.meridianDistance(standard_parallel_rad);
            f = a(standard_parallel_rad) / Math.sin(standard_parallel_rad);
        }
    }

    private double a(double d1) {
        double d2 = Math.sin(d1);
        return (super.theellipsoid.getSemiMajorAxis() * Math.cos(d1)) / Math.sqrt(1.0D - super.theellipsoid.getEccentricitySquared() * d2 * d2);
    }

    public boolean equals(Projection projection) {
        if (projection instanceof Bonne) {
            Bonne winkel = (Bonne) projection;
    ...
    public void forward(double srcPts[], int srcOff, double dstPts[], int dstOff, int numPts) {
        while (numPts-- > 0) {
            ...
            if (super.theellipsoid.isSphere()) {
                double d2 = super.theellipsoid.getSemiMajorAxis() * ((e + standard_parallel_rad) - y);
                if (Math.abs(d2) < 9.9999999999999998E-013D) {
                    x = 0;
                    y = 0;
                } else {
                    double d4 = (super.theellipsoid.getSemiMajorAxis() * d1 * Math.cos(y)) / d2;
                    x = d2 * Math.sin(d4);
                    y = (super.theellipsoid.getSemiMajorAxis() * e) - d2 * Math.cos(d4);
                }
            } else {
                double d3 = (f + g) - super.theellipsoid.meridianDistance(y);
                if (Math.abs(d3) < 9.9999999999999998E-013D) {
                    x = 0;
                    y = 0;
                } else {
                    double d5 = (a(y) * d1) / d3;
    ...
    public void inverse(double srcPts[], int srcOff, double dstPts[], int dstOff, int numPts) {
        while (numPts-- > 0) {
            ...
            if (super.theellipsoid.isSphere()) {
                double d1 = semiMajorAxis * e - phi;
                double d3 = MathTool.signum(standard_parallel_rad) * Math.sqrt(lambda * lambda + d1 * d1);
                phi = (e + standard_parallel_rad) - d3 / semiMajorAxis;
                if (MathTool.angleIsZero(phi)) {
                    lambda = lambda0;
                } else {
                    lambda = lambda0 + (d3 * Math.atan(lambda / d1)) / (semiMajorAxis * Math.cos(phi));
                }
            } else {
                double d2 = f - phi;
                double d4 = MathTool.signum(standard_parallel_rad) * Math.sqrt(lambda * lambda + d2 * d2);
                double d5 = (f + g) - d4;
                double d6 = super.theellipsoid.meridianDistanceToRectifying(d5);
                double d7 = super.theellipsoid.rectifyingToLatitude(d6);
                lambda = lambda0 + (d4 * Math.atan(lambda / (f - phi))) / a(d7);
                phi = d7;
            }
    ...
    public static void main(String arg[]) {
        Bonne eckert = new Bonne(AngularUnit.degree, LinearUnit.meter, Ellipsoid.WGS84, 00.0, 60, 0.0, 0.0);
        double input[] = { -75, -35 };
