...
public class FibonacciSeries extends Overlay {
    ...
    public void calculate() {
        ...
        for (int i = 0; i < initial.getItemsCount(); i++) {
            ...
            if (c2.get(Calendar.DAY_OF_MONTH) == properties.getStartingDay() && (c2.get(Calendar.MONTH) + 1) == properties.getStartingMonth() && c2.get(Calendar.YEAR) == properties.getStartingYear() && c2.get(Calendar.HOUR_OF_DAY) == properties.getStartingHour() && c2.get(Calendar.MINUTE) == properties.getStartingMinute()) {
                start = i;
                break;
            }
        }
        if (start != -1) {
            ...
            for (int i = start; i < initial.getItemsCount(); i++) {
                int val = 0;
                int diff = i - start;
                if (diff == 5 || diff == 8 || diff == 13 || diff == 21 || diff == 34 || diff == 55 || diff == 89 || diff == 144 || diff == 233 || diff == 377 || diff == 610 || diff == 987 || diff == 1597 || diff == 2584 || diff == 4181 || diff == 6765) {
                    val = 1;
                }
