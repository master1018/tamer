...
public class DbStreamVerify {
    ...
    private class VerifyReader extends FileReader {
        ...
        VerifyReader(EnvironmentImpl envImpl, PrintStream out, long startLsn, long endLsn) {
            super(envImpl, 10000, true, startLsn, null, DbLsn.NULL_LSN, endLsn);
            this.out1 = out;
        }
        ...
        protected boolean processEntry(ByteBuffer entryBuffer) {
            ...
            if ((lastVLSN != null) && lastVLSN.isNull()) {
                ...
                out1.println("gap of " + (currentVLSN.getSequence() - lastVLSN.getSequence()) + " Last=" + lastVLSN + " at lsn " + DbLsn.getNoFormatString(lastLSN) + " next=" + currentVLSN + " at lsn " + DbLsn.getNoFormatString(currentLSN));
            }
            if (showInvisible && currentEntryHeader.isInvisible()) {
                out1.println("VLSN " + currentVLSN + " at lsn " + DbLsn.getNoFormatString(currentLSN) + " is invisible.");
            }
    ...
    private abstract class DecoderReader extends FileReader {
        ...
        DecoderReader(Environment env, PrintStream out, long startLsn, long endLsn, String dbName, LogEntryType... logEntryTypes) {
            super(DbInternal.getEnvironmentImpl(env), 10000, true, startLsn, null, DbLsn.NULL_LSN, endLsn);
            this.outStream = out;
        ...
        @Override
        protected boolean processEntry(ByteBuffer entryBuffer) {
            targetEntry.readEntry(currentEntryHeader, entryBuffer, true);
    ...
    private class VLSNIndexReader extends DecoderReader {
        ...
        VLSNIndexReader(Environment env, PrintStream out, long startLsn, long endLsn) {
            super(env, out, startLsn, endLsn, DbTree.VLSN_MAP_DB_NAME, new LogEntryType[] { LogEntryType.LOG_LN_TRANSACTIONAL, LogEntryType.LOG_LN });
        }
    ...
    private class RepGroupReader extends DecoderReader {
        ...
        RepGroupReader(Environment env, PrintStream out, long startLsn, long endLsn) {
            super(env, out, startLsn, endLsn, DbTree.REP_GROUP_DB_NAME, new LogEntryType[] { LogEntryType.LOG_LN_TRANSACTIONAL, LogEntryType.LOG_LN });
        }
