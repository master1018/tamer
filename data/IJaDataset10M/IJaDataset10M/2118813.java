...
public class MySampler extends Synth {
    ...
    protected HashMap<Integer, Vector<SoundFontOscillator>> exclusiveClasses = new HashMap<Integer, Vector<SoundFontOscillator>>();
    ...
    public Serializable getSettings() {
        ...
        try {
            if (MySampler.this.getFrinikaSynth().isSaveReferencedData()) throw new Exception("Also saving referenced data");
    ...
    public synchronized void noteOn(int noteNumber, int velocity) {
        if (samplerOscillator.isMonitoring() && !samplerOscillator.isRecording()) {
            switch(recordMode) {
                case RECORDMODE_SELECTION:
                    if (!gui.isNoteInSelection(noteNumber, velocity)) break;
                default:
            ...
            if (!releaseTriggered) {
                ...
                switch(recordMode) {
                    case RECORDMODE_SINGLE_KEY:
                        for (int v = 0; v < 128; v++) ((MySampler) synth).insertSample(sampledSound, noteNumber, v);
                        break;
                    case RECORDMODE_ALL_KEYS:
                        for (int n = 0; n < 96; n++) for (int v = 0; v < 128; v++) ((MySampler) synth).insertSample(sampledSound, n, v);
                        break;
                    case RECORDMODE_SELECTION:
        ...
        public void setNoteNumber(int noteNumber, float customFineTune, float customPan) {
            ...
            if (sampledSound.getExclusiveClass() > 0) {
                ...
                if (exclusiveClasses.containsKey(exclusiveClass)) {
                    Vector<SoundFontOscillator> exOscillators = exclusiveClasses.get(exclusiveClass);
                    if (exOscillators.get(0).noteNumber == noteNumber) exOscillators.add(this); else {
                        for (SoundFontOscillator exOscillator : exOscillators) exOscillator.terminate();
            ...
            for (int n = startBufferPos; n < endBufferPos; ) {
                ...
                if (samplePos < (rightSamples.length - 1)) {
                    ...
                    if (dBFactor > 0.01) {
                        ...
                        if (release) dBFactor *= attenuationPerSample;
                        ...
                        if (sampleMode == SAMPLEMODE_LOOP_CONTINOUSLY && samplePos >= loopEnd) samplePos = loopStart + (samplePos - loopEnd);
                    } else {
                        getAudioOutput().removeTransmitter(this);
                        break;
                    }
