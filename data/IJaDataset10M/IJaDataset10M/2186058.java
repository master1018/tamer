 ...
 */
public class LongHash<V> implements Externalizable, Iterable<LongHash.BucketEntry<V>>, ILongCache<V> {
    ...
    private static int indexFor(final long key, final int mask) {
        return ((int) (key ^ (key >>> 32))) & 0x7fffffff & mask;
    }
    ...
    public static class LongLRUMap<V> extends LongHash<V> {
        ...
        protected void addEntry(int bucket, long key, V value, BucketEntry<V> next) {
            ...
            if (removeEldestEntry()) {
                ...
                if (_size > _threshold) {
                    throw new IllegalStateException("size '" + _size + "' exceeds threshold '" + _threshold + '\'');
                }
            }
        }
