...
public final class CacheService extends IntentService {
    ...
    public static final String[] BUCKET_PROJECTION_IMAGES = new String[] { Images.ImageColumns.BUCKET_ID, Images.ImageColumns.BUCKET_DISPLAY_NAME };

    public static final String[] BUCKET_PROJECTION_VIDEOS = new String[] { Video.VideoColumns.BUCKET_ID, Video.VideoColumns.BUCKET_DISPLAY_NAME };
    ...
    public static final String[] THUMBNAIL_PROJECTION = new String[] { Images.ImageColumns._ID, Images.ImageColumns.DATE_ADDED, Images.ImageColumns.DATA, Images.ImageColumns.ORIENTATION };

    public static final String[] SENSE_PROJECTION = new String[] { Images.ImageColumns.BUCKET_ID, "MAX(" + Images.ImageColumns.DATE_ADDED + "), COUNT(*)" };
    ...
    public static final String[] PROJECTION_VIDEOS = new String[] { Video.VideoColumns._ID, Video.VideoColumns.TITLE, Video.VideoColumns.MIME_TYPE, Video.VideoColumns.LATITUDE, Video.VideoColumns.LONGITUDE, Video.VideoColumns.DATE_TAKEN, Video.VideoColumns.DATE_ADDED, Video.VideoColumns.DATE_MODIFIED, Video.VideoColumns.DATA, Video.VideoColumns.DURATION, Video.VideoColumns.BUCKET_ID };
    ...
    public static final boolean isCacheReady(final boolean onlyMediaSets) {
        if (onlyMediaSets) {
            ...
            return (sAlbumCache.get(ALBUM_CACHE_METADATA_INDEX, 0) != null && sAlbumCache.get(ALBUM_CACHE_DIRTY_INDEX, 0) == null && sAlbumCache.get(ALBUM_CACHE_DIRTY_BUCKET_INDEX, 0) == null);
        }
    }
    ...
    public static final void loadMediaSets(final Context context, final MediaFeed feed, final DataSource source, final boolean includeImages, final boolean includeVideos, final boolean moveCameraToFront) {
        syncCache(context);
    ...
    public static final void loadMediaItemsIntoMediaFeed(final Context context, final MediaFeed feed, final MediaSet set, final int rangeStart, final int rangeEnd, final boolean includeImages, final boolean includeVideos) {
        syncCache(context);
        ...
        if (albumData != null && set.mNumItemsLoaded < set.getNumExpectedItems()) {
            ...
            try {
                ...
                for (int i = 0; i < numItems; ++i) {
                    ...
                    if ((itemMediaType == MediaItem.MEDIA_TYPE_IMAGE && includeImages) || (itemMediaType == MediaItem.MEDIA_TYPE_VIDEO && includeVideos)) {
                        String baseUri = (itemMediaType == MediaItem.MEDIA_TYPE_IMAGE) ? BASE_CONTENT_STRING_IMAGES : BASE_CONTENT_STRING_VIDEOS;
    ...
    private static void syncCache(Context context) {
        if (!isCacheReady(true)) {
            ...
            if (DEBUG) Log.d(TAG, "Refreshing Cache for all items");
            refresh(context);
            sAlbumCache.delete(ALBUM_CACHE_DIRTY_INDEX);
            sAlbumCache.delete(ALBUM_CACHE_DIRTY_BUCKET_INDEX);
        } else if (!isCacheReady(false)) {
            if (DEBUG) Log.d(TAG, "Refreshing Cache for dirty items");
            refreshDirtySets(context);
            sAlbumCache.delete(ALBUM_CACHE_DIRTY_BUCKET_INDEX);
        }
    }

    public static final void populateVideoItemFromCursor(final MediaItem item, final ContentResolver cr, final Cursor cursor, final String baseUri) {
        item.setMediaType(MediaItem.MEDIA_TYPE_VIDEO);
        populateMediaItemFromCursor(item, cr, cursor, baseUri);
    }

    public static final void populateMediaItemFromCursor(final MediaItem item, final ContentResolver cr, final Cursor cursor, final String baseUri) {
        ...
        if (baseUri != null) item.mContentUri = baseUri + item.mId;
    ...
    public static final byte[] queryThumbnail(final Context context, final long thumbId, final long origId, final boolean isVideo, final long timestamp) {
    ...
    private static final byte[] queryThumbnail(final Context context, final long thumbId, final long origId, final boolean isVideo, final DiskCache thumbnailCache, final long timestamp) {
        ...
        if (bitmap == null) {
            ...
            bitmap = buildThumbnailForId(context, thumbnailCache, thumbId, origId, isVideo, DEFAULT_THUMBNAIL_WIDTH, DEFAULT_THUMBNAIL_HEIGHT, timestamp);
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!isInThumbnailerSkipList(thumbnailId)) {
                if (!thumbnailCache.isDataAvailable(thumbnailId, timeModifiedInSec * 1000)) {
                    byte[] retVal = buildThumbnailForId(context, thumbnailCache, thumbnailId, id, false, DEFAULT_THUMBNAIL_WIDTH, DEFAULT_THUMBNAIL_HEIGHT, timeModifiedInSec * 1000);
    ...
    private static final byte[] buildThumbnailForId(final Context context, final DiskCache thumbnailCache, final long thumbId, final long origId, final boolean isVideo, final int thumbnailWidth, final int thumbnailHeight, final long timestamp) {
        ...
        try {
            ...
            if (isVideo) {
                bitmap = MediaStore.Video.Thumbnails.getThumbnail(context.getContentResolver(), origId, MediaStore.Video.Thumbnails.MINI_KIND, null);
            } else {
                bitmap = MediaStore.Images.Thumbnails.getThumbnail(context.getContentResolver(), origId, MediaStore.Images.Thumbnails.MINI_KIND, null);
            }
            if (bitmap == null) {
                return null;
            }
    ...
    public static final byte[] writeBitmapToCache(final DiskCache thumbnailCache, final long thumbId, final long origId, final Bitmap bitmap, final int thumbnailWidth, final int thumbnailHeight, final long timestamp) {
    ...
    private static final void refresh(final Context context) {
    ...
    private static final void refreshDirtySets(final Context context) {
        synchronized (sCacheLock) {
            final byte[] existingData = sAlbumCache.get(ALBUM_CACHE_DIRTY_BUCKET_INDEX, 0);
            if (existingData != null && existingData.length > 0) {
                final long[] ids = toLongArray(existingData);
    ...
    private static final void addNoDupe(ArrayList<Long> array, long value) {
        int size = array.size();
        for (int i = 0; i < size; ++i) {
            if (array.get(i).longValue() == value) return;
        }
        array.add(value);
    }

    private static final void populateMediaItemsForSets(final Context context, final ArrayList<MediaSet> sets, final LongSparseArray<MediaSet> acceleratedSets, boolean useWhere) {
