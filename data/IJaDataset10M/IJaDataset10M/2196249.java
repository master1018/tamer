...
public class RegexpTermAnalyzer implements TermAnalyzerIF {
    ...
    public RegexpTermAnalyzer() {
        ...
        rules.add(new Rule("email address", "[-A-Za-z.0-9]+@([-A-Za-z.0-9]+\\.)+[A-Za-z]+", 0.002d));
        rules.add(new Rule("http URL", "http://[-.A-Za-z?+&=0-9#/]+", 0.002d));
    }
    ...
    public void analyzeTerm(Term term) {
        for (int ix = 0; ix < rules.size(); ix++) {
            ...
            if (rule.matches(term)) term.multiplyScore(rule.getFactor(), "matched " + rule.getName() + " rule");
        }
    }
