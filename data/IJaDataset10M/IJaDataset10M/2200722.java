...
public class OneWireContainer04 extends OneWireContainer implements ClockContainer {
    ...
    */
    public byte[] readDevice() throws OneWireIOException, OneWireException {
        ...
        alarming = isAlarming();
        do {
            if (alarming && (attempt != 0)) clock.read(1, false, read_buf[buf_num], 1, 31); else clock.read(0, false, read_buf[buf_num], 0, 32);
            if (attempt++ != 0) {
                for (i = 1; i < 32; i++) {
                    if ((i != 2) && (i != 7)) {
                        if (read_buf[0][i] != read_buf[1][i]) break;
                    }
                }
                if (i == 32) return read_buf[buf_num];
            }
    ...
    public void writeDevice(byte[] state) throws OneWireIOException, OneWireException {
        ...
        for (i = 0; i < 32; i++) {
            if ((Bit.arrayReadBit(i, BITMAP_OFFSET, state) == 1) && (i != 1)) {
                if (got_block) len++; else {
                    got_block = true;
                    start_offset = i;
                    len = 1;
                }
                if (i == 31) clock.write(start_offset, state, start_offset, len);
            } else if (got_block) {
        ...
        if (Bit.arrayReadBit(CONTROL_OFFSET, BITMAP_OFFSET, state) == 1) {
            ...
            if ((state[CONTROL_OFFSET] & 0x07) != 0) {
                for (i = 0; i < 2; i++) scratch.writeScratchpad(clock.getStartPhysicalAddress() + CONTROL_OFFSET, state, CONTROL_OFFSET, 1);
            }
        }
        for (i = BITMAP_OFFSET; i < state.length; i++) state[i] = 0;
    }
    ...
    */
    public void setClock(long time, byte[] state) {
        ...
        for (int i = 0; i < 5; i++) Bit.arrayWriteBit(1, RTC_OFFSET + i, BITMAP_OFFSET, state);
    }
    ...
    */
    public void setClockAlarm(long time, byte[] state) throws OneWireException {
        ...
        for (int i = 0; i < 5; i++) Bit.arrayWriteBit(1, RTC_ALARM_OFFSET + i, BITMAP_OFFSET, state);
    }
    ...
    */
    public void setIntervalTimer(long time, byte[] state) {
        ...
        for (int i = 0; i < 5; i++) Bit.arrayWriteBit(1, INTERVAL_OFFSET + i, BITMAP_OFFSET, state);
    }
    ...
    */
    public void setCycleCounter(long cycles, byte[] state) {
        ...
        for (int i = 0; i < 4; i++) Bit.arrayWriteBit(1, COUNTER_OFFSET + i, BITMAP_OFFSET, state);
    }
    ...
    */
    public void setIntervalTimerAlarm(long time, byte[] state) {
        ...
        for (int i = 0; i < 5; i++) Bit.arrayWriteBit(1, INTERVAL_ALARM_OFFSET + i, BITMAP_OFFSET, state);
    }
    ...
    */
    public void setCycleCounterAlarm(long cycles, byte[] state) {
        ...
        for (int i = 0; i < 4; i++) Bit.arrayWriteBit(1, COUNTER_ALARM_OFFSET + i, BITMAP_OFFSET, state);
    }
