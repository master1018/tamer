...
public class NavigatorView extends View {
    ...
    private GedcomListener callback = (GedcomListener) Spin.over(new GedcomListenerAdapter() {
        ...
        public void gedcomEntityDeleted(Gedcom gedcom, Entity entity) {
            if (context != null && context.getEntity() == entity) setContext(new Context(gedcom), true);
        }
    });
   ...
   */
    ...
    public void setContext(Context context, boolean isActionPerformed) {
        ...
        if (context.getEntities().size() == 1) {
            ...
            if (context.getEntity() instanceof Fam) setFam((Fam) context.getEntity());
        }
        revalidate();
        repaint();
    }
    ...
    private void setFam(Fam fam) {
        ...
        if (husband == null || wife == null) content.add("parent", create(new CreateParent(fam)));
        Indi[] children = fam.getChildren();
        for (Indi child : children) content.add("child", indi(child));
    ...
    private void setIndi(Indi indi) {
        ...
        for (Indi grandparent : grandparents) content.add("grandparent", indi(grandparent));
        List<Indi> parents = indi.getParents();
        addExpander("parents", parents.size());
        for (Indi parent : parents) content.add("parent", indi(parent));
        if (parents.size() < 2) content.add("parent", create(new CreateParent(indi)));
        Indi[] siblings = indi.getSiblings(false);
        addExpander("siblings", siblings.length);
        for (Indi sibling : siblings) content.add("sibling", indi(sibling));
        content.add("sibling", create(new CreateSibling(indi, true)));
        Indi[] spouses = indi.getPartners();
        addExpander("spouses", spouses.length);
        for (Indi spouse : spouses) content.add("spouse", indi(spouse));
        if (spouses.length == 0) content.add("spouse", create(new CreateSpouse(indi)));
        ...
        for (Indi grandchild : grandchildren) content.add("grandchild", indi(grandchild));
    }
    ...
    private List<Indi> getParents(List<Indi> parents) {
        ...
        for (Indi parent : parents) result.addAll(parent.getParents());
        return result;
    }

    private List<Indi> getChildren(List<Indi> children) {
        List<Indi> result = new ArrayList<Indi>(16);
        for (Indi child : children) result.addAll(Arrays.asList(child.getChildren()));
        return result;
    }
    ...
    private static final MouseListener CLICK = new MouseAdapter() {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (target != null) SelectionSink.Dispatcher.fireSelection(e, new Context(target));
            Action action = (Action) label.getClientProperty(Action.class);
            if (action != null) action.actionPerformed(new ActionEvent(e.getSource(), 0, "", e.getModifiers()));
        }
    };
}
