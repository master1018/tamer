package homura.hde.input.thirdperson;

import homura.hde.input.ChaseCamera;
import homura.hde.input.InputHandler;
import homura.hde.input.MouseInput;
import homura.hde.input.RelativeMouse;
import homura.hde.input.action.InputActionEvent;
import homura.hde.input.action.MouseInputAction;
import homura.hde.main.core.math.FastMath;
import homura.hde.main.core.math.Quaternion;
import homura.hde.main.core.math.Vector3f;
import homura.hde.main.core.scene.Spatial;
...
public class ThirdPersonMouseLook extends MouseInputAction {
     ...
     * applicable.
     * 
     * @see homura.hde.input.action.InputActionInterface#performAction(InputActionEvent)
     */
    public void performAction(InputActionEvent event) {
        ...
        if (updated) camera.getCamera().onFrameChange();
    }
    ...
    private void rotateUp(float amount) {
        ...
        FastMath.cartesianToSpherical(difTemp, sphereTemp);
        camera.getIdealSphereCoords().z = clampUpAngle(sphereTemp.z + (thetaAccel));
    }
    ...
    private void rollIn(float amount) {
        camera.getIdealSphereCoords().x = clampRollIn(camera.getIdealSphereCoords().x + (amount * rollInSpeed));
    }
    ...
    private float clampUpAngle(float r) {
        ...
        if (r > maxAscent) r = maxAscent; else if (r < minAscent) r = minAscent;
        return r;
    }
    ...
    private float clampRollIn(float r) {
        ...
        if (r > maxRollOut) r = maxRollOut; else if (r < minRollOut) r = minRollOut;
        return r;
    }
