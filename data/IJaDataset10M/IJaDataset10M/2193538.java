...
public class CharStyleConverter extends StyleConverter {
    ...
    public void applyDefaultFont(StyleWithProperties style, LaTeXDocumentPortion ldp) {
        ...
        s = nfssShape(style.getProperty(XMLString.FO_FONT_VARIANT), style.getProperty(XMLString.FO_FONT_STYLE));
        if (s != null) {
    ...
    private void applyNfssFamily(StyleWithProperties style, boolean bDecl, boolean bInherit, BeforeAfter ba, Context context) {
        ...
        if (sFontName != null) {
            ...
            if (sFamily.equals(convertFontDeclaration(context.getFontName()))) {
                return;
            }
            if (bDecl) {
        ...
        if (style != null && !bIgnoreAll) {
            ...
            if (sSeries != null) {
                if (!bDecl && "bf".equals(sSeries) && config.getTextAttributeStyleMap().contains("bold")) {
                    ba.add(config.getTextAttributeStyleMap().getBefore("bold"), config.getTextAttributeStyleMap().getAfter("bold"));
                } else {
                    if (style.isAutomatic()) {
                        if (sSeries.equals(nfssSeries(context.getFontWeight()))) {
                            return;
                        }
        ...
        if (style != null && !bIgnoreAll) {
            ...
            if (sShape != null) {
                if (!bDecl && "sc".equals(sShape) && config.getTextAttributeStyleMap().contains("small-caps")) {
                    ba.add(config.getTextAttributeStyleMap().getBefore("small-caps"), config.getTextAttributeStyleMap().getAfter("small-caps"));
                } else if (!bDecl && "it".equals(sShape) && config.getTextAttributeStyleMap().contains("italic")) {
                    ba.add(config.getTextAttributeStyleMap().getBefore("italic"), config.getTextAttributeStyleMap().getAfter("italic"));
                } else {
                    if (style.isAutomatic()) {
                    ...
                    if (bDecl) ba.add("\\" + sShape + "shape", ""); else ba.add("\\text" + sShape + "{", "}");
                }
            }
        }
    }
    ...
    private void applyNfssSize(StyleWithProperties style, boolean bDecl, boolean bInherit, BeforeAfter ba, Context context) {
        if (style == null || bIgnoreFontsize || (bIgnoreHardFontsize && style.isAutomatic())) {
            return;
        }
        if (style.getProperty(XMLString.FO_FONT_SIZE, bInherit) == null) {
            return;
        }
        ...
        if (sSize.equals(nfssSize(context.getFontSize()))) {
            return;
        }
        if (bDecl) {
            ba.add(sSize, "");
    ...
    private void applyTextPosition(StyleWithProperties style, boolean bInherit, BeforeAfter ba) {
        if (style != null && !bIgnoreAll) {
            ...
            if (config.getTextAttributeStyleMap().contains("superscript") && "\\textsuperscript".equals(s)) {
                ba.add(config.getTextAttributeStyleMap().getBefore("superscript"), config.getTextAttributeStyleMap().getAfter("superscript"));
            } else if (config.getTextAttributeStyleMap().contains("subscript") && "\\textsubscript".equals(s)) {
                ba.add(config.getTextAttributeStyleMap().getBefore("subscript"), config.getTextAttributeStyleMap().getAfter("subscript"));
            } else if (s != null) {
                ba.add(s + "{", "}");
            }
        }
    ...
    private void applyUnderline(StyleWithProperties style, boolean bInherit, BeforeAfter ba) {
        ...
        String sTag = ofr.isOpenDocument() ? XMLString.STYLE_TEXT_UNDERLINE_STYLE : XMLString.STYLE_TEXT_UNDERLINE;
    ...
    private void applyCrossout(StyleWithProperties style, boolean bInherit, BeforeAfter ba) {
        ...
        String sTag = ofr.isOpenDocument() ? XMLString.STYLE_TEXT_LINE_THROUGH_STYLE : XMLString.STYLE_TEXT_CROSSING_OUT;
    ...
    private static final String nfssFamily(String sFontFamily, String sFontFamilyGeneric, String sFontPitch) {
        if ("fixed".equals(sFontPitch)) return "tt"; else if ("modern".equals(sFontFamilyGeneric)) return "tt"; else if ("swiss".equals(sFontFamilyGeneric)) return "sf"; else return "rm";
    }

    private static final String nfssSeries(String sFontWeight) {
        if (sFontWeight == null) return null;
        if ("bold".equals(sFontWeight)) return "bf"; else return "md";
    }
    ...
    private static final String nfssShape(String sFontVariant, String sFontStyle) {
        ...
        if ("small-caps".equals(sFontVariant)) return "sc"; else if ("italic".equals(sFontStyle)) return "it"; else if ("oblique".equals(sFontStyle)) return "sl"; else return "up";
    }
