...
public final class LatexTextParser {
        ...
        try {
            {
                ...
                new LatexTextParser(input, printer, false, LoadXmlOperatorListUtility.getOperatorList(new File(ResourceLoaderUtility.getResourceUrl("org/qedeq/kernel/tool/parser/mengenlehreMathOperators.xml").getPath())));
            }
            {
                input = new TextInput(new File(args[0]), IoUtility.getDefaultEncoding());
                final String mathName = "test_math.xml";
                final TextOutput printer = new TextOutput(mathName, new FileOutputStream(mathName));
                new LatexTextParser(input, printer, true, LoadXmlOperatorListUtility.getOperatorList(new File(ResourceLoaderUtility.getResourceUrl("org/qedeq/kernel/tool/parser/mengenlehreMathOperators.xml").getPath())));
            }
        } catch (Exception e) {
     ...
     */
    public LatexTextParser(final TextInput input, final TextOutput output, final boolean mathParsing, final List operators) throws SourceFileException {
