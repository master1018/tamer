...
public class FavScanDialog extends JDialog implements ActionListener, ListSelectionListener {
   ...
   */
    ...
    private void swap(int index1, int index2) {
        ...
        if (value1 == 0 && value2 != 0 && ((index1 < index2 && !canAdd(index1, 1)) || (index1 > index2 && !canAdd(index1 + 1, 1)))) {
            return;
        }
        if (value2 == 0 && value1 != 0 && ((index2 < index1 && !canAdd(index2, 1)) || (index2 > index2 && !canAdd(index2 + 1, 1)))) {
            return;
        }
    ...
    public static FavScan showDialog(Component locationComp, FavScan favScan, RemoteConfiguration config) {
        if (dialog == null) dialog = new FavScanDialog(locationComp);
    ...
    private void setRemoteConfiguration(RemoteConfiguration config) {
        ...
        for (Button b : buttons) {
            if (b.canAssignToFav() || b.canAssignShiftedToFav() || b.canAssignXShiftedToFav()) model.addElement(b);
        }
