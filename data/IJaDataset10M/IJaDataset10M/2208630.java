...
public final class GetFileHistoryController extends AbstractTemplateController {
   ...
   */
    ...
    @Override
    protected ModelAndView svnHandle(final SVNConnection connection, final BaseCommand command, final long headRevision, final UserRepositoryContext userRepositoryContext, final HttpServletRequest request, final HttpServletResponse response, final BindException exception) throws Exception {
        ...
        final List<LogEntry> revisions = getRepositoryService().getLogEntries(connection, command.getName(), command.getRevisionNumber(), Revision.FIRST.getNumber(), command.getPath(), FILE_HISTORY_LIMIT, false, true);
