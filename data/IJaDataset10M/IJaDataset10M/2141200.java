...
public abstract class AbstractAuralModule extends AbstractPerceptualModule implements IAuralModule, IParameterized {
    ...
    public final class DefaultAuralEncodingTimeEquation implements IAuralEncodingTimeEquation {
    ...
    public static final String ENABLE_BUFFER_STUFF_PARAM = "EnableBufferStuff";

    public static final String AURAL_DECAY_TIME_PARAM = "AuralDecayTime";

    public static final String ENCODING_TIME_EQUATION_PARAM = "AuralEncodingTimeEquationClass";

    /**
   * logger definition
   */
    private static final Log LOGGER = LogFactory.getLog(AbstractAuralModule.class);
   ...
   */
    ...
    public void initialize() {
        _modelListener = new ModelListenerAdaptor() {
            ...
            public void cycleStarted(ModelEvent me) {
                if (isBufferStuffEnabled()) {
                    ...
                    } else if (LOGGER.isDebugEnabled()) LOGGER.debug("audicon has not changed since " + _lastStuffAttempt + " now: " + me.getSimulationTime());
                }
            }
        };
        ...
        if (decM == null) throw new IllegalModuleStateException("declarative module is not available");
        ...
        IAuralModuleListener listener = new IAuralModuleListener() {
            ...
            public void perceptAttended(IPerceptualMemoryModuleEvent event) {
                ...
                IIdentifier identifier = (IIdentifier) auralChunk.getMetaData(IPerceptualEncoder.COMMONREALITY_IDENTIFIER_META_KEY);
                if (identifier != null) {
                    if (LOGGER.isDebugEnabled()) LOGGER.debug("assigning finst for " + auralChunk);
                    getAuralMemory().getFINSTFeatureMap().flagAsAttended(identifier, auralChunk, getAuralMemory().getFINSTSpan());
                } else if (LOGGER.isDebugEnabled()) LOGGER.debug("could not find identifier for " + auralChunk);
            }
        ...
        IActivePerceptListener finstListener = new IActivePerceptListener() {
            ...
            public void newPercept(IIdentifier identifier, IChunk chunk) {
                ...
                if (finstMap != null && !finstMap.isAttended(identifier)) finstMap.flagAsNew(identifier, chunk, getAuralMemory().getNewFINSTOnsetDuration());
            }
    ...
    protected void connectToCommonReality() {
        ...
        _auralMemory.attach((ACTRAgent) ACTRRuntime.getRuntime().getConnector().getAgent(getModel()));
    }
   ...
   */
    ...
    public double getRecodeTime(IChunkType chunkType) {
        ...
        if (LOGGER.isWarnEnabled()) LOGGER.warn("No recode time is defined for " + chunkType);
        return 0.5;
    }
   ...
   */
    protected void applyParameters() {
        if (_auralMemory instanceof IParameterized) for (Map.Entry<String, String> entry : _deferredParameters.entrySet()) ((IParameterized) _auralMemory).setParameter(entry.getKey(), entry.getValue());
    }
    ...
    public void setParameter(String key, String value) {
        ...
        } else if (STRICT_SYNCHRONIZATION_PARAM.equalsIgnoreCase(key)) super.setParameter(key, value); else {
    ...
    public String getParameter(String key) {
        if (AURAL_DECAY_TIME_PARAM.equalsIgnoreCase(key)) return "" + getAuralDecayTime(); else if (ENABLE_BUFFER_STUFF_PARAM.equalsIgnoreCase(key)) return "" + isBufferStuffEnabled(); else if (ENCODING_TIME_EQUATION_PARAM.equalsIgnoreCase(key)) return "" + _encodingTime.getClass().getName(); else if (STRICT_SYNCHRONIZATION_PARAM.equalsIgnoreCase(key)) return super.getParameter(key); else return _deferredParameters.get(key);
    }
}
