...
public class SbbChildPanel extends Composite implements SelectionListener {
    ...
    private static final int[] COLUMN_EDITORS = { EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_TEXT, EditableTableViewer.EDITOR_TEXT };
    ...
    public SbbChildPanel(Composite parent, int style) {
        ...
        availableChildren = new EditableTableViewer(this, SWT.V_SCROLL | SWT.BORDER | SWT.SINGLE, new String[] { "Name", "Version", "Vendor" }, new int[] { EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE }, new String[][] { {}, {}, {} });
    ...
    public void widgetSelected(SelectionEvent child) {
        if (child.getSource().equals(selectButton)) {
            ...
            for (int i = 0; i < indices.length; i++) {
                ...
                setScopedName(ev);
                if (ev.get("Default Priority") == null) ev.put("Default Priority", "0");
    ...
    public void selectChild(SbbJarXML xml, SbbXML child) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (name.equals(child.getName()) && vendor.equals(child.getVendor()) && version.equals(child.getVersion()) && xml.equals(storedXML)) {
                store.remove(map);
                setScopedName(map);
                if (map.get("Default Priority") == null) map.put("Default Priority", "0");
    ...
    public HashMap[] getSelectedChildren() {
        ...
        for (int i = 0; i < childs.length; i++) out[i] = (HashMap) childs[i];
        return out;
    }
