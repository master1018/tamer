 ...
 */
public abstract class ReportTextPartManagerBean extends BaseSessionBeanImpl implements SessionBean {
	 ...
	 * @ejb.transaction type="Supports"
	 */
    public ReportTextPartConfiguration getReportTextPartConfiguration(ReportRegistryItemID reportRegistryItemID, boolean synthesize, String[] fetchGroups, int maxFetchDepth) {
        PersistenceManager pm;
        pm = getPersistenceManager();
        try {
            ReportRegistryItem reportRegistryItem = (ReportRegistryItem) pm.getObjectById(reportRegistryItemID);
            ReportTextPartConfiguration configuration = ReportTextPartConfiguration.getReportTextPartConfiguration(pm, reportRegistryItem, synthesize, fetchGroups, maxFetchDepth);
            ...
            } else throw new IllegalStateException("ReportTextPartConfiguration.getReportTextPartConfiguration() returned not-attached and not-synthesized ReportTextPartConfiguration");
        } finally {
            pm.close();
        }
    }
	 ...
	 * @ejb.transaction type="Supports"
	 */
    public Set<ReportTextPartConfiguration> getReportTextPartConfigurations(Set<ReportTextPartConfigurationID> configurationIDs, String[] fetchGroups, int maxFetchDepth) {
        PersistenceManager pm;
        pm = getPersistenceManager();
        try {
	 ...
	 *
	 */
    public ReportTextPartConfiguration getReportTextPartConfiguration(ReportRegistryItemID reportRegistryItemID, ObjectID linkedObjectID, boolean synthesize, String[] fetchGroups, int maxFetchDepth) {
        PersistenceManager pm = getPersistenceManager();
        try {
            ReportTextPartConfiguration configuration = ReportTextPartConfiguration.getReportTextPartConfiguration(pm, reportRegistryItemID, linkedObjectID, synthesize, fetchGroups, maxFetchDepth);
            if (configuration == null) return null;
            if (JDOHelper.getPersistenceManager(configuration) != null) {
                pm.getFetchPlan().setGroups(fetchGroups);
                pm.getFetchPlan().setMaxFetchDepth(maxFetchDepth);
                return pm.detachCopy(configuration);
            } else if (configuration.isSynthetic()) return configuration; else throw new IllegalStateException("ReportTextPartConfiguration.getReportTextPartConfiguration() returned not-attached and not-synthesized ReportTextPartConfiguration");
        } finally {
            pm.close();
        }
    }
	 ...
	 */
    public ReportTextPartConfiguration storeLinkedObjectReportTextPartConfiguration(ReportTextPartConfiguration reportTextPartConfiguration, boolean get, String[] fetchGroups, int maxFetchDepth) {
        PersistenceManager pm = getPersistenceManager();
        try {
	 ...
	 * @ejb.transaction type="Required"
	 */
    public ReportTextPartConfiguration storeReportTextPartConfiguration(ReportTextPartConfiguration reportTextPartConfiguration, boolean get, String[] fetchGroups, int maxFetchDepth) {
        PersistenceManager pm = getPersistenceManager();
        try {
            return NLJDOHelper.storeJDO(pm, reportTextPartConfiguration, get, fetchGroups, maxFetchDepth);
        } finally {
