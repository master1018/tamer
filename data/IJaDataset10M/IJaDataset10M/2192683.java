...
public class CommandlineConfigure {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            if (choice == 1) configurePaths(); else if (choice == 2) configureLoadedBots(); else if (choice == 3) manageBots(); else if (choice == 4) addBot(); else if (choice == 5) copyBot(); else if (choice == 6) removeBot(); else if (choice == 0) {
    ...
    private static void manageBot(String bot) throws IOException {
        while (true) {
            ...
            if (choice == 1) {
                ...
                if (plugins.length == 0) System.out.println("(all plugins are being loaded already)");
                System.out.println();
                System.out.println("0. Back");
                System.out.println();
                int plugin = getNumericInput("Please select a plugin to add", null, plugins.length);
                if (plugin != 0) JavaOpFileStuff.addActivePlugin(bot, plugins[plugin - 1]);
            } else if (choice == 2) {
                String[] plugins = getAndPrintAllActivePlugins(bot, true);
                if (plugins.length == 0) System.out.println("(no plugins are being loaded yet)");
                System.out.println();
                System.out.println("0. Back");
                System.out.println();
                int plugin = getNumericInput("Please select a plugin to remove", null, plugins.length);
                if (plugin != 0) JavaOpFileStuff.removeActivePlugin(bot, plugins[plugin - 1]);
            } else if (choice == 3) {
    ...
    private static String getPatternInput(String prompt, String defaultIn, String pattern) {
        while (true) {
            try {
                ...
                if (text.equals("") && defaultIn != null) return defaultIn;
                if (text.matches(pattern)) return text;
    ...
    private static String[] getAndPrintPluginPaths(boolean printNumbers) throws IOException {
        ...
        for (int i = 0; i < allPlugins.length; i++) System.out.println((printNumbers ? ((i + 1) + ". ") : "* ") + allPlugins[i]);
        return allPlugins;
    }
    ...
    private static String[] getAndPrintDefaultBots(boolean printNumbers) throws IOException {
        ...
        for (int i = 0; i < defaultBots.length; i++) System.out.println((printNumbers ? ((i + 1) + ". ") : "* ") + defaultBots[i]);
        return defaultBots;
    }
    ...
    private static String[] getAndPrintAllBots(boolean printNumbers) throws IOException {
        ...
        for (int i = 0; i < allBots.length; i++) System.out.println((printNumbers ? ((i + 1) + ". ") : "* ") + allBots[i]);
        return allBots;
    }
    ...
    private static String[] getAndPrintAllPlugins(boolean printNumbers) throws IOException {
        ...
        for (int i = 0; i < plugins.length - 1; i++) System.out.println((printNumbers ? ((i + 1) + ". ") : "* ") + plugins[i]);
        return plugins;
    }
    ...
    private static String[] getAndPrintAllActivePlugins(String bot, boolean printNumbers) throws IOException {
        ...
        for (int i = 0; i < plugins.length; i++) System.out.println((printNumbers ? ((i + 1) + ". ") : "* ") + plugins[i]);
        return plugins;
    }

    private static Object[] sortEnumeration(Enumeration<Object> e) {
        Vector<Object> objectVector = new Vector<Object>();
        while (e.hasMoreElements()) objectVector.add(e.nextElement());
