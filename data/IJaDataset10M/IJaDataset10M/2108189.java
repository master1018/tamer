...
public class UTCScale implements TimeScale {
    ...
    /** Reference TAI date. */
    private static final AbsoluteDate TAI_REFERENCE = new AbsoluteDate(DateComponents.J2000_EPOCH, TimeComponents.H12, TAIScale.getInstance());
    ...
    /** {@inheritDoc} */
    public synchronized double offsetToTAI(final DateComponents date, final TimeComponents time) {
    ...
    private synchronized void setCurrent(final double utcTime) {
        while (offsets[current].getValidityStart().durationFrom(TAI_REFERENCE) > (utcTime + offsets[current].getOffset())) {
            --current;
        }
        while (offsets[current].getValidityEnd().durationFrom(TAI_REFERENCE) <= (utcTime + offsets[current].getOffset())) {
            ++current;
        }
    }
