...
public class MainFrame extends JFrame implements ActionListener, Printable {
    ...
    public void doExit() {
        if (chart != null) if (new YesNoBox(this, _("The train graph is has changed.\nDo you want to save the changes?")).askForYes()) doSaveChart();
        try {
    ...
    private void jbInit() throws Exception {
        ...
        this.setIconImage(new ImageIcon(org.paradise.etrc.MainFrame.class.getResource("/pic/icon.gif")).getImage());
        JPanel contentPane;
    ...
    public void setActiceTrainName(String _name) {
        if (_name.equalsIgnoreCase("")) activeTrainName = ""; else activeTrainName = " (" + _name + ") ";
        if (!_name.equalsIgnoreCase("")) {
            if (trainSelectHistory.contains(_name)) trainSelectHistory.remove(_name);
            trainSelectHistory.add(0, _name);
            if (trainSelectHistory.size() > MAX_TRAIN_SELECT_HISTORY_RECORD) for (int i = 12; i < trainSelectHistory.size(); i++) trainSelectHistory.remove(i);
    ...
    public String getCircuitName() {
        String circuitName = "";
        if (chart.circuit.name.equalsIgnoreCase("")) circuitName = ""; else circuitName = " -- [" + chart.circuit.name + "] ";
        return circuitName;
    }
    ...
    private JToolBar loadToolBar() {
        ...
        } else jtButtonDown.setSelected(false);
        ...
        } else jtButtonUp.setSelected(false);
        ...
        cbTrainSelectHistory.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                if (!ae.getActionCommand().equalsIgnoreCase("comboBoxChanged")) return;
                String trainToFind = (String) cbTrainSelectHistory.getSelectedItem();
                if (trainToFind == null) return;
                if (trainToFind.trim().equalsIgnoreCase("")) return;
                if (!chartView.findAndMoveToTrain(trainToFind)) new MessageBox(MainFrame.this, String.format(_("Cannot find train information: %s"), trainToFind)).showMessage();
            }
        });
        jToolBar.addSeparator();
    ...
    private JButton createTBButton(String imgName, String toolTipText, String Command) {
        ...
        ImageIcon imageOpenFile = new ImageIcon(org.paradise.etrc.MainFrame.class.getResource("/pic/" + imgName + ".png"));
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex) {
        ...
        Rectangle2D.Double rec1 = new Rectangle2D.Double(0.0, 0.0, pageFormat.getImageableWidth(), pageFormat.getImageableHeight());
        Rectangle2D.Double rec2 = new Rectangle2D.Double(0.0, 0.0, pageFormat.getImageableWidth() - 1.5, pageFormat.getImageableHeight() - 0.5);
        g2D.draw(rec1);
        g2D.draw(rec2);
    ...
    public void doSaveChartAs() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            File f = chooser.getSelectedFile();
            if (!f.getAbsolutePath().endsWith(".trc")) f = new File(f.getAbsolutePath() + ".trc");
            try {
                chart.saveToFile(f);
    ...
    public void doLoadTrain() {
        if (!(new YesNoBox(this, _("Load train information file and overwrite the existing information. Continue?")).askForYes())) return;
        JFileChooser chooser = new JFileChooser();
        ETRC.setFont(chooser);
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (f.length > 0) System.out.println(f[0]); else System.out.println("No File selected!");
            ...
            for (int j = 0; j < f.length; j++) {
                ...
                if (loadingTrain.isDownTrain(chart.circuit, false) > 0) chart.addTrain(loadingTrain);
            }
            chartView.repaint();
            sheetView.updateData();
