...
import reconcile.weka.clusterers.DensityBasedClusterer;
...
public class PredictionAppender extends JPanel implements DataSource, Visible, BeanCommon, EventConstraints, BatchClassifierListener, IncrementalClassifierListener, BatchClustererListener, Serializable {
   ...
   */
    ...
    protected BeanVisual m_visual = new BeanVisual("PredictionAppender", BeanVisual.ICON_PATH + "PredictionAppender.gif", BeanVisual.ICON_PATH + "PredictionAppender_animated.gif");
   ...
   */
    ...
    protected transient reconcile.weka.gui.Logger m_logger;
   ...
   */
    public void useDefaultVisual() {
        m_visual.loadIcons(BeanVisual.ICON_PATH + "PredictionAppender.gif", BeanVisual.ICON_PATH + "PredictionAppender_animated.gif");
    }

    protected InstanceEvent m_instanceEvent;

    protected float[] m_instanceVals;
   ...
   */
    public void acceptClassifier(IncrementalClassifierEvent e) {
        reconcile.weka.classifiers.Classifier classifier = e.getClassifier();
        ...
        if (status == IncrementalClassifierEvent.NEW_BATCH) {
            ...
            if (!m_appendProbabilities || oldStructure.classAttribute().isNumeric()) {
                try {
                    ...
                    m_instanceVals = new float[m_format.numAttributes()];
                } catch (Exception ex) {
                    ex.printStackTrace();
                    return;
                }
    ...
    public void acceptClassifier(BatchClassifierEvent e) {
        if (m_dataSourceListeners.size() > 0) {
            ...
            if (!m_appendProbabilities || testSet.classAttribute().isNumeric()) {
                try {
                    Instances newInstances = makeDataSetClass(testSet, classifier, relationNameModifier);
                    ...
                    for (int i = 0; i < testSet.numInstances(); i++) {
                        ...
                        newInstances.instance(i).setValue(newInstances.numAttributes() - 1, (float) predClass);
                    }
            ...
            if (m_appendProbabilities) {
                try {
                    Instances newInstances = makeDataSetProbabilities(testSet, classifier, relationNameModifier);
                    notifyDataSetAvailable(new DataSetEvent(this, new Instances(newInstances, 0)));
                    if (e.getTestSet().isStructureOnly()) {
                        m_format = newInstances;
                    }
                    for (int i = 0; i < testSet.numInstances(); i++) {
                        ...
                        for (int j = 0; j < testSet.classAttribute().numValues(); j++) {
                            newInstances.instance(i).setValue(testSet.numAttributes() + j, (float) preds[j]);
                        }
                    }
                    notifyDataSetAvailable(new DataSetEvent(this, newInstances));
                } catch (Exception ex) {
   ...
   */
    public void acceptClusterer(BatchClustererEvent e) {
        if (m_dataSourceListeners.size() > 0) {
            if (e.getTestSet().isStructureOnly()) return;
            Instances testSet = e.getTestSet().getDataSet();
            reconcile.weka.clusterers.Clusterer clusterer = e.getClusterer();
            String test;
            if (e.getTestOrTrain() == 0) test = "test"; else test = "training";
            ...
            if (!m_appendProbabilities || !(clusterer instanceof DensityBasedClusterer)) {
                ...
                try {
                    ...
                    for (int i = 0; i < testSet.numInstances(); i++) {
                        ...
                        newInstances.instance(i).setValue(newInstances.numAttributes() - 1, (float) predCluster);
                    }
                    notifyDataSetAvailable(new DataSetEvent(this, newInstances));
                    return;
                } catch (Exception ex) {
                ...
                try {
                    ...
                    for (int i = 0; i < testSet.numInstances(); i++) {
                        ...
                        for (int j = 0; j < clusterer.numberOfClusters(); j++) {
                            newInstances.instance(i).setValue(testSet.numAttributes() + j, (float) probs[j]);
                        }
                    }
                    notifyDataSetAvailable(new DataSetEvent(this, newInstances));
                } catch (Exception ex) {
    ...
    private Instances makeDataSetProbabilities(Instances format, reconcile.weka.classifiers.Classifier classifier, String relationNameModifier) throws Exception {
        ...
        for (int i = 0; i < format.classAttribute().numValues(); i++) {
            reconcile.weka.filters.unsupervised.attribute.Add addF = new reconcile.weka.filters.unsupervised.attribute.Add();
            addF.setAttributeIndex("last");
            addF.setAttributeName("prob_" + format.classAttribute().value(i));
            addF.setInputFormat(newInstances);
            newInstances = reconcile.weka.filters.Filter.useFilter(newInstances, addF);
        }
        newInstances.setRelationName(format.relationName() + relationNameModifier);
        return newInstances;
    }

    private Instances makeDataSetClass(Instances format, reconcile.weka.classifiers.Classifier classifier, String relationNameModifier) throws Exception {
        reconcile.weka.filters.unsupervised.attribute.Add addF = new reconcile.weka.filters.unsupervised.attribute.Add();
        addF.setAttributeIndex("last");
        ...
        Instances newInstances = reconcile.weka.filters.Filter.useFilter(format, addF);
        newInstances.setRelationName(format.relationName() + relationNameModifier);
        return newInstances;
    }

    private Instances makeClusterDataSetProbabilities(Instances format, reconcile.weka.clusterers.Clusterer clusterer, String relationNameModifier) throws Exception {
        int numOrigAtts = format.numAttributes();
        Instances newInstances = new Instances(format);
    ...
    private Instances makeClusterDataSetClass(Instances format, reconcile.weka.clusterers.Clusterer clusterer, String relationNameModifier) throws Exception {
        reconcile.weka.filters.unsupervised.attribute.Add addF = new reconcile.weka.filters.unsupervised.attribute.Add();
        addF.setAttributeIndex("last");
   ...
   */
    public void setLog(reconcile.weka.gui.Logger logger) {
        m_logger = logger;
    }
    ...
    public boolean eventGeneratable(String eventName) {
        ...
        if (m_listenee instanceof EventConstraints) {
            ...
            if (eventName.equals("dataSet")) {
                if (!((EventConstraints) m_listenee).eventGeneratable("batchClassifier") && !((EventConstraints) m_listenee).eventGeneratable("batchClusterer")) {
                    return false;
                }
            }
        }
