...
class HttpmdURLConnection extends DelegatingHttpURLConnection {
    ...
    HttpmdURLConnection(URL url) throws IOException, MalformedURLException {
        ...
        try {
            ...
            MalformedURLException t = new MalformedURLException("Message digest parameter algorithm is not found: " + algorithm);
            t.initCause(e);
            throw t;
        }
        ...
        try {
            content = new URL("http", url.getHost(), url.getPort(), path.substring(0, semiIndex) + (url.getQuery() == null ? "" : url.getQuery()) + (url.getRef() == null ? "" : url.getRef()));
    ...
    public InputStream getInputStream() throws IOException {
        try {
            return new MdInputStream(url, delegateConnection.getInputStream(), MessageDigest.getInstance(algorithm), expectedDigest);
        } catch (NoSuchAlgorithmException e) {
            IOException t = new IOException("Message digest algorithm not found: " + algorithm);
            t.initCause(e);
            throw t;
        }
    }
