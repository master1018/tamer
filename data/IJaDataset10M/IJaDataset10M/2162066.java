...
public class DnDTabbedPane extends JTabbedPane {
    ...
    private final DragGestureListener dragGestureListener = new DragGestureListener() {
        ...
        public void dragGestureRecognized(DragGestureEvent dge) {
            if (getTabCount() < 2 || !SwingUtilities.isLeftMouseButton((MouseEvent) dge.getTriggerEvent())) return;
            ...
            try {
                dge.startDrag(DragSource.DefaultMoveDrop, emptyImage, emptyPoint, transferable, dragSourceListener);
    ...
    private int getDropIndex(Point p) {
        ...
        if (idx == -1 && getTabCount() > 0) {
            ...
            if ((tabBounds.x + tabBounds.width) <= p.x && tabBounds.y <= (p.y + 1) && p.y <= (tabBounds.y + tabBounds.height)) {
                idx = getTabCount() - 1;
            }
        }
        return idx;
    ...
    private class GlassPane extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (tabOver > -1 && tabOver != dragTabIdx) {
                Rectangle r = SwingUtilities.convertRectangle(DnDTabbedPane.this, getBoundsAt(tabOver), this);
        ...
        public void createImage(Component c) {
            ...
            image = image.getSubimage(rect.x < 0 ? 0 : rect.x, rect.y < 0 ? 0 : rect.y, rect.width, rect.height);
        }
    }
