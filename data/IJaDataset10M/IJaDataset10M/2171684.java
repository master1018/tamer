...
final class Limits {
	 ...
	 */
    private static final Map<String, Limits> TABLE = Collections.synchronizedMap(new HashMap<String, Limits>());
    ...
    private Limits(final AbstractHistogram hist, final boolean ignoreZero, final boolean ignoreFull) {
        super();
        if (hist == null) {
            throw new IllegalArgumentException("Can't have null histogram reference in Limits constructor.");
        }
        histName = hist.getFullName();
        TABLE.put(histName, this);
        sizeX = hist.getSizeX() - 1;
        sizeY = hist.getSizeY() - 1;
        init(ignoreZero, ignoreFull);
        updateModelX();
        updateModelY();
    }
    ...
    private int getMaxCounts2D(final AbstractHist2D hist, final int chminX, final int chmaxX, final int chminY, final int chmaxY) {
        int maxCounts = DEFAULTMAXCOUNTS;
