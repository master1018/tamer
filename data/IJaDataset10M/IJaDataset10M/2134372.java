...
public class RandomNumbersToolTest extends CDKTestCase {
    ...
    public void testFlipCoin() {
        ...
        for (int i = 0; i < ntry; i++) {
            if (RandomNumbersTool.flipCoin(p)) ntrue += 1; else nfalse += 1;
        }
    ...
    public void testGaussianFloat() {
        ...
        Assert.assertTrue("Estimated SD does not match to 2 decimal places", sd >= (dev - epsilon) && sd <= (dev + epsilon));
    }
    ...
    public void testExponentialDouble() {
        ...
        m = m / ntry;
        Assert.assertTrue("Estimated mean does not match to 2 decimal places " + m, m >= (mean - epsilon) && m <= (mean + epsilon));
    }
}
