...
public class UDPServer {
	 ...
	 *             could not be opened.
	 */
    public static synchronized void open(IPAddress ipAddress, int port, String ifaceName, UDPDatagramHandler handler) throws IOException {
        open(ipAddress, port, ifaceName, handler, false);
    }

    public static synchronized void open(IPAddress ipAddress, int port, String ifaceName, UDPDatagramHandler handler, boolean isMulticast) throws IOException {
        if (ipAddress == null) {
	 ...
	 * @throws IOException
	 */
    public static synchronized void close(IPAddress ipAddress, int port, String ifaceName) throws IOException {
        if (ipAddress == null) {
            return;
        }
        if (port < 1 || port > 65535) {
	 ...
	 * @throws IOException
	 */
    public static synchronized void send(IPAddress localAddress, int port, String ifaceName, IPAddress dstAddress, int dstPort, byte[] data, int len) throws IOException {
	 ...
	 * @throws IOException
	 */
    public static synchronized void send(UDPDatagramHandler handler, IPAddress dstAddress, int dstPort, byte[] data, int len) throws IOException {
