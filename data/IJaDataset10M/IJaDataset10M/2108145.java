  ...
  */
   ...
   */
    ...
    private static String escape(String s) {
        ...
        for (int i = 0; i < s.length(); i++) if (s.charAt(i) == '"') result.append("\\\""); else result.append(s.charAt(i));
   ...
   */
    ...
    static String toString(ACLMessage msg) {
        ...
        if (sender != null) str.append(SENDER + " " + sender.toString() + "\n");
        Iterator it = msg.getAllReceiver();
        if (it.hasNext()) {
            str.append(RECEIVER + " (set ");
            while (it.hasNext()) str.append(it.next().toString() + " ");
            str.append(")\n");
        }
        ...
        if (msg.hasByteSequenceContent()) {
            ...
            if (content != null) {
                ...
                if (content.length() > 0) str.append(CONTENT + " \"" + escape(content) + "\" \n");
            }
        }
        ...
        if (d != null) str.append(REPLY_BY + " " + ISO8601.toString(d) + "\n");
        String tmp = msg.getProtocol();
        if (tmp != null) {
            tmp = tmp.trim();
            if (tmp.length() > 0) str.append(PROTOCOL + " " + tmp + "\n");
        }
        ...
        if (userDefProps != null) {
            ...
            while (e.hasMoreElements()) {
                ...
                if (key.indexOf(' ') == -1) {
                    if ((!key.startsWith("X-")) && (!key.startsWith("x-"))) appendACLExpression(str, ":X-" + key, userDefProps.getProperty(key)); else appendACLExpression(str, ":" + key, userDefProps.getProperty(key));
                } else System.err.println("WARNING: The slotName of user-defined parameters cannot contain blanks inside. Therefore " + key + " is not being encoded");
            }
        }
        str.append(")");
        return str.toString();
   ...
   */
    ...
    private static boolean isAWord(String s) {
        ...
        if (illegalFirstChar.indexOf(s.charAt(0)) >= 0) return false;
