...
public class InputQueueItemDAOImpl extends AbstractDAO implements InputQueueItemDAO {
    ...
    public static final String SELECT_NUMBER_ITEMS_STATEMENT = "SELECT count(id) FROM " + Constants.TABLE_INPUT_QUEUE_ITEM;

    /** The Constant INSERT_ITEM_STATEMENT. */
    public static final String INSERT_ITEM_STATEMENT = "INSERT INTO " + Constants.TABLE_INPUT_QUEUE_ITEM + " (path, barcode, ingested) VALUES ((?),(?),(?))";

    /** The Constant FIND_ITEMS_ON_TOP_LVL_STATEMENT. */
    public static final String FIND_ITEMS_ON_TOP_LVL_STATEMENT = "SELECT p.path, p.barcode, p.ingested, n.name FROM " + Constants.TABLE_INPUT_QUEUE_ITEM + " p LEFT JOIN " + Constants.TABLE_INPUT_QUEUE_ITEM_NAME + " n ON(p.path=n.path) " + "WHERE position('" + File.separator + "' IN trim(leading ((?)) FROM p.path)) = 0";

    /** The Constant FIND_ITEMS_ON_TOP_LVL_STATEMENT_ORDERED. */
    public static final String FIND_ITEMS_ON_TOP_LVL_STATEMENT_ORDERED = FIND_ITEMS_ON_TOP_LVL_STATEMENT + " ORDER BY p.path";

    /** The Constant FIND_ITEMS_BY_PATH_STATEMENT. */
    public static final String FIND_ITEMS_BY_PATH_STATEMENT = FIND_ITEMS_ON_TOP_LVL_STATEMENT + " AND p.path LIKE ((?)) ORDER BY p.path";

    public static final String UPDATE_INGEST_INFO = "UPDATE " + Constants.TABLE_INPUT_QUEUE_ITEM + " SET ingested = (?) WHERE path = (?)";

    public static final String SELECT_NAME_ITEM_ID = "SELECT id FROM " + Constants.TABLE_INPUT_QUEUE_ITEM_NAME + " WHERE path=(?)";

    public static final String INSERT_NAME = "INSERT INTO " + Constants.TABLE_INPUT_QUEUE_ITEM_NAME + " (name, path) VALUES ((?),(?))";

    public static final String UPDATE_NAME = "UPDATE " + Constants.TABLE_INPUT_QUEUE_ITEM_NAME + " SET name = (?) WHERE path = (?)";
    ...
    public ArrayList<InputQueueItem> getItems(String prefix) throws DatabaseException {
        ...
        try {
            findSt = getConnection().prepareStatement(top ? FIND_ITEMS_ON_TOP_LVL_STATEMENT_ORDERED : FIND_ITEMS_BY_PATH_STATEMENT);
        } catch (SQLException e) {
        ...
        try {
            ...
            while (rs.next()) {
                retList.add(new InputQueueItem(rs.getString("path"), rs.getString("barcode"), rs.getBoolean("ingested"), rs.getString("name")));
            }
        } catch (SQLException e) {
    ...
    public void updateIngestInfo(boolean ingested, String path) throws DatabaseException {
        ...
        try {
            ...
            if (updated == 1) {
                ...
                LOGGER.error("DB has been updated, with unexpected count of updated lines: " + updated + ". Queries: \"" + updateSt + "\".");
            }
        } catch (SQLException e) {
            LOGGER.error(e);
        } finally {
    ...
    public void updateName(String path, String name) throws DatabaseException {
        ...
        try {
            PreparedStatement updateSt = getConnection().prepareStatement((duplicate < 0) ? INSERT_NAME : UPDATE_NAME);
