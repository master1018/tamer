...
public class RankBasedHistogram implements PointQuery {
    ...
    public static class MaxBased implements ParitioningStrategy {
        ...
        public Bucket[] partitioning(Iterator aggData) {
            ...
            r[nob - 1] = new AverageFrequencyBucket(Cursors.toArray(restObjects.iterator()), rest / restObjects.size());
            return r;
        }
    ...
    public static class OptimizedMaxBased implements ParitioningStrategy {
        ...
        public Bucket[] partitioning(Iterator aggData) {
            ...
            for (int i = 0; i < a.size(); i++) {
                ...
                if (currentnob < nob - 1) {
                    if (!((Object[]) a.get(lastpos))[1].equals(temp[1])) {
                        r[currentnob++] = new AverageFrequencyBucket(Cursors.toArray(buffer.iterator()), bufferAgg / (buffer.size()));
                        buffer.clear();
                        bufferAgg = 0.0;
                    }
                    buffer.add(temp[0]);
            ...
            r[nob - 1] = new AverageFrequencyBucket(Cursors.toArray(buffer.iterator()), bufferAgg / buffer.size());
            return r;
        }
        ...
        public Bucket[] partitioning(Iterator aggData) {
            ...
            int[] indices = Statistics.maxDiff(a.iterator(), new Distance() {
                ...
                public double distance(Object o1, Object o2) {
                    return Math.abs(((Number) ((Object[]) o1)[1]).doubleValue() - ((Number) ((Object[]) o2)[1]).doubleValue());
                }
            }, nob - 1, false);
            Arrays.sort(indices);
            Bucket[] r = new Bucket[nob];
    ...
    public double pointQuery(Object query) throws IllegalArgumentException {
        ...
        if (!found) throw new IllegalArgumentException("Queried object '" + query + "' not found!");
        return r;
    }
