...
public abstract class Curve {
    ...
    public void setCurve(Coordinate[] positions, double[] times, Coordinate start_velocity, Coordinate end_velocity) throws JuggleExceptionInternal {
        this.positions = positions;
        this.times = times;
        this.start_velocity = start_velocity;
        this.end_velocity = end_velocity;
        this.numpoints = positions.length;
        if (numpoints != times.length) throw new JuggleExceptionInternal("Path error 1");
    }
    ...
    protected Coordinate check(Coordinate result, double t, boolean findmax) {
        ...
        if (findmax) result = Coordinate.max(result, loc); else result = Coordinate.min(result, loc);
        return result;
    }
}
