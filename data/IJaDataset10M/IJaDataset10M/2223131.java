...
public class MultiFilter extends SimpleStreamFilter {
   ...
   */
    public Capabilities getCapabilities() {
        if (getFilters().length == 0) return super.getCapabilities(); else return getFilters()[0].getCapabilities();
    }
   ...
   */
    protected String getFilterSpec(Filter filter) {
        ...
        if (filter == null) {
            ...
            if (filter instanceof OptionHandler) result += " " + Utils.joinOptions(((OptionHandler) filter).getOptions());
        }
        return result;
    }
   ...
   */
    public boolean isStreamableFilter() {
        ...
        if (!m_StreamableChecked) {
            ...
            for (i = 0; i < getFilters().length; i++) {
                if (getFilter(i) instanceof MultiFilter) m_Streamable = ((MultiFilter) getFilter(i)).isStreamableFilter(); else if (getFilter(i) instanceof StreamableFilter) m_Streamable = true; else m_Streamable = false;
                if (!m_Streamable) break;
            }
            if (getDebug()) System.out.println("Streamable: " + m_Streamable);
        }
        return m_Streamable;
    }
   ...
   */
    protected Instances determineOutputFormat(Instances inputFormat) throws Exception {
        ...
        for (i = 0; i < getFilters().length; i++) {
            if (!isFirstBatchDone()) getFilter(i).setInputFormat(result);
