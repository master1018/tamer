...
class LuaBaseLib {
    ...
    public static final class luaB_collectgarbage implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            String[] opts = { "stop", "restart", "collect", "count", "step", "setpause", "setstepmul" };
            int[] optsnum = { LuaAPI.LUA_GCSTOP, LuaAPI.LUA_GCRESTART, LuaAPI.LUA_GCCOLLECT, LuaAPI.LUA_GCCOUNT, LuaAPI.LUA_GCSTEP, LuaAPI.LUA_GCSETPAUSE, LuaAPI.LUA_GCSETSTEPMUL };
    ...
    public static final class luaB_error implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            ...
            if (LuaAPI.lua_isstring(thread, 1) && level > 0) {
    ...
    public static final class luaB_print implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            ...
            for (i = 1; i <= n; i++) {
                String s;
                LuaAPI.lua_pushvalue(thread, -1);
                LuaAPI.lua_pushvalue(thread, i);
                LuaAPI.lua_call(thread, 1, 1);
                s = LuaAPI.lua_tostring(thread, -1);
                if (s == null) {
    ...
    public static final class luaB_getfenv implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            ...
            if (LuaAPI.lua_isjavafunction(thread, -1) && mutableValue.m_iCallInfoIndex != 0 && thread.GetCallInfosStack()[mutableValue.m_iCallInfoIndex].GetTailCalls() == 0) {
    ...
    public static final class luaB_select implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            int n = LuaAPI.lua_gettop(thread);
            if (LuaAPI.lua_type(thread, 1) == LuaAPI.LUA_TSTRING && LuaAPI.lua_tostring(thread, 1).charAt(0) == '#') {
    ...
    public static final class luaB_newproxy implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            LuaAPI.lua_settop(thread, 1);
            LuaAPI.lua_newuserdata(thread, null, 0);
            if (LuaAPI.lua_toboolean(thread, 1) == false) {
                ...
                LuaAPI.lua_rawset(thread, LuaAPI.lua_upvalueindex(1));
            } else {
                ...
                LuaAPI.lua_getmetatable(thread, 1);
            }
    ...
    public static final class luaB_load implements JavaFunction {
        ...
        private static String generic_reader(lua_State thread) {
            ...
            LuaAPI.lua_pushvalue(thread, 1);
            LuaAPI.lua_call(thread, 0, 1);
            if (LuaAPI.lua_isnil(thread, -1)) {
                return null;
            } else if (LuaAPI.lua_isstring(thread, -1)) {
                LuaAPI.lua_replace(thread, 3);
    ...
    public static void base_open(lua_State thread) {
        ...
        LuaAPI.lua_setfield(thread, -2, "__mode");
        LuaAPI.lua_pushjavafunction(thread, new luaB_newproxy(), 1);
        LuaAPI.lua_setglobal(thread, "newproxy");
    }
    ...
    public static final class luaB_auxwrap implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            ...
            if (r < 0) {
                if (LuaAPI.lua_isstring(thread, -1)) {
                    LuaAPI.luaL_where(thread, 1);
                    LuaAPI.lua_insert(thread, -2);
                    LuaAPI.lua_concat(thread, 2);
                }
                LuaAPI.lua_error(thread);
    ...
    static int costatus(lua_State thread, lua_State co) {
        if (thread == co) return CO_RUN;
        switch(LuaAPI.lua_status(co)) {
            case LUA_YIELD:
                return CO_SUS;
            case 0:
                {
                    if (LuaAPI.lua_gettop(co) == 0) return CO_DEAD; else return CO_SUS;
                }
            default:
                return CO_DEAD;
        }
    ...
    public static int auxresume(lua_State thread, lua_State co, int narg) {
        ...
        if (status == 0 || status == LUA_YIELD) {
            ...
            if (!LuaAPI.lua_checkstack(thread, nres)) LuaAPI.luaL_error(thread, "too many results to resume");
            LuaAPI.lua_xmove(co, thread, nres);
            return nres;
        } else {
            LuaAPI.lua_xmove(co, thread, 1);
            return -1;
        }
    }
    ...
    public static final class luaopen_base implements JavaFunction {
        ...
        public int Call(lua_State thread) {
            luaL_Reg[] luaReg = new luaL_Reg[] { new luaL_Reg("create", new luaB_cocreate()), new luaL_Reg("resume", new luaB_coresume()), new luaL_Reg("running", new luaB_corunning()), new luaL_Reg("status", new luaB_costatus()), new luaL_Reg("wrap", new luaB_cowrap()), new luaL_Reg("yield", new luaB_yield()) };
            base_open(thread);
