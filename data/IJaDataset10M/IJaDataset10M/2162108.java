 ...
 */
public class smooth extends GLSkeleton<GLJPanel> implements GLEventListener, KeyListener {
    ...
    public void reshape(GLAutoDrawable drawable, int x, int y, int w, int h) {
        ...
        gl.glLoadIdentity();
        if (w <= h) glu.gluOrtho2D(0.0, 30.0, 0.0, 30.0 * (float) h / (float) w); else glu.gluOrtho2D(0.0, 30.0 * (float) w / (float) h, 0.0, 30.0);
