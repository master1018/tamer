 ...
 * of the {@link IDeclarativeModule} without the actual chunk/type creation,
 * search or encoding functions.
 * 
...
public abstract class AbstractDeclarativeModule extends AbstractModule implements IDeclarativeModule {
   ...
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(AbstractDeclarativeModule.class);
    ...
    protected void fireChunkCreated(IChunk chunk) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_CREATED, chunk));
    }

    protected void fireChunkTypeCreated(IChunkType chunkType) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_TYPE_CREATED, chunkType));
    }

    protected void fireChunkAdded(IChunk chunk) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_ADDED, chunk));
    }

    protected void fireChunkDisposed(IChunk chunk) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_DISPOSED, chunk));
    }

    protected void fireChunkTypeAdded(IChunkType chunkType) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_TYPE_ADDED, chunkType));
    }

    protected void fireChunksMerged(IChunk original, IChunk duplicateChunk) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNKS_MERGED, original, duplicateChunk));
    }

    protected void fireChunkTypeDisposed(IChunkType chunkType) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_TYPE_DISPOSED, chunkType));
    }

    protected void fireChunkTypesMerged(IChunkType original, IChunkType duplicate) {
        if (_eventDispatcher.hasListeners()) _eventDispatcher.fire(new DeclarativeModuleEvent(this, DeclarativeModuleEvent.Type.CHUNK_TYPES_MERGED, original, duplicate));
    }
   ...
   */
    public Future<IChunk> createChunk(final IChunkType parent, final String name) {
        if (parent == null) throw new NullPointerException("IChunkType cannot be null");
   ...
   * @return
   */
    protected abstract IChunk createChunkInternal(IChunkType parent, String name);
   ...
   */
    public Future<IChunk> addChunk(final IChunk chunk) {
        if (chunk.isEncoded()) {
            if (LOGGER.isDebugEnabled()) LOGGER.debug(chunk + " has already been encoded, silly");
        ...
        return delayedFuture(new Callable<IChunk>() {
            ...
            public IChunk call() throws Exception {
                try {
                    ...
                    if (_busyChunk == null && name.equals("busy")) _busyChunk = rtn; else if (_emptyChunk == null && name.equals("empty")) _emptyChunk = rtn; else if (_errorChunk == null && name.equals("error")) _errorChunk = rtn; else if (_freeChunk == null && name.equals("free")) _freeChunk = rtn; else if (_fullChunk == null && name.equals("full")) _fullChunk = rtn; else if (_newChunk == null && name.equals("new")) _newChunk = rtn; else if (_requestedChunk == null && name.equals("requested")) _requestedChunk = rtn; else if (_unrequestedChunk == null && name.equals("unrequested")) _unrequestedChunk = rtn;
                    return rtn;
                } catch (Exception e) {
   ...
   */
    ...
    public Future<IChunk> copyChunk(final IChunk sourceChunk) {
        if (sourceChunk == null) throw new NullPointerException("sourceChunk cannot be null");
        return delayedFuture(new Callable<IChunk>() {
   ...
   * @param copy
   */
    protected abstract void copyChunkInternal(IChunk sourceChunk, IChunk copy);
   ...
   */
    ...
    protected abstract IChunk getChunkInternal(String chunkName);
   ...
   */
    ...
    protected abstract Collection<IChunk> getChunksInternal();
   ...
   *      java.lang.String)
   */
    public Future<IChunkType> createChunkType(final Collection<IChunkType> parents, final String name) {
    ...
    public Future<IChunkType> createChunkType(final IChunkType parent, final String name) {
        if (parent != null) return createChunkType(Collections.singleton(parent), name); else return createChunkType(Collections.EMPTY_LIST, name);
    }
   ...
   * @return
   */
    protected abstract IChunkType createChunkTypeInternal(Collection<IChunkType> parents, String name);
   ...
   */
    public Future<IChunkType> addChunkType(final IChunkType chunkType) {
        if (chunkType.isEncoded()) {
            if (LOGGER.isDebugEnabled()) LOGGER.debug(chunkType + " has already been encoded silly");
   ...
   */
    ...
    protected abstract IChunkType getChunkTypeInternal(String name);
   ...
   */
    ...
    protected abstract Collection<IChunkType> getChunkTypesInternal();
}
