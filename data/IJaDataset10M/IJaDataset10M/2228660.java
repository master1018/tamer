 ...
 * 256 colors.
 * */
    public GIFEncoder(byte r[][], byte g[][], byte b[][]) throws Exception {
 ...
 * */
    ...
    void ToIndexedColor(byte r[][], byte g[][], byte b[][]) throws Exception {
        ...
        for (int x = 0; x < width_; ++x) {
            for (int y = 0; y < height_; ++y) {
                int search;
                for (search = 0; search < colornum; ++search) if (colors_[search * 3] == r[x][y] && colors_[search * 3 + 1] == g[x][y] && colors_[search * 3 + 2] == b[x][y]) break;
                if (search > 255) throw new Exception("Too many colors.");
...
class LZWCompressor {
    ...
    public static void LZWCompress(OutputStream output, int codesize, byte toCompress[]) throws IOException {
        ...
        for (int loop = 0; loop < toCompress.length; ++loop) {
            c = toCompress[loop];
            if ((index = strings.FindCharString(prefix, c)) != -1) prefix = index; else {
