...
public class TestCoverageDoclet {
     ...
     */
    public static final String[] COLORS = { "#a0ffa0", "#ffffa0", "#ff8080" };
    ...
    private Map<ExecutableMemberDoc, AnnotationPointer> resolved = new HashMap<ExecutableMemberDoc, AnnotationPointer>(8192);
    ...
    private Map<ClassDoc, List<TestTargetNew>> classToSpecialTargets = new HashMap<ClassDoc, List<TestTargetNew>>();
    ...
    private PrintWriter openFile(String filename, String title) {
        ...
        printer.println("<style type=\"text/css\">\n" + "body, body table tr td { font-size:10pt;font-family: " + " sans-serif; }\n" + "li { padding-bottom:2px }\n" + "table { width:100%; border-width: 0px; border: solid; " + "border-collapse: collapse;}\n" + "table tr td { vertical-align:top; padding:3px; border: " + "1px solid black;}\n" + "</style>");
        printer.println("  </head>");
    ...
    private void process(RootDoc root) {
        ...
        if (_acceptCompleteWithOtherStatus) {
            ...
            System.out.println("- not accepting complete tests with partial " + "tests");
        }
        System.out.println("stage 1 - get targets from all junit test methods");
        PrintWriter pr = openFile("testcoverage/test-annotation.html", "test class annotation coverage");
        TablePrinter printer = new TablePrinter(pr);
        printer.printTableStart();
        printer.printRow("Test package name", "JUnit classes", "Meth", "Unt", "Part", "Compl", "Disab", "Broken", "ToBeFixed", "KnownFail");
        ...
        printer.printTableEnd();
        closeFile(pr);
        System.out.println("stage 2 - proxy test targets to abstract classes" + "and interfaces");
        ClassDoc[] classes = root.classes();
        for (ClassDoc classDoc : classes) {
            if (!extendsJUnitTestCase(classDoc)) {
                MethodDoc[] methods = classDoc.methods();
                for (MethodDoc methodDoc : methods) {
                    AnnotationPointer ap = getAnnotationPointer(methodDoc, false);
                    if (ap != null) {
                        List<MethodDoc> impls = implementingMethods(methodDoc);
                        for (MethodDoc impl : impls) {
                            AnnotationPointer apImpl = getAnnotationPointer(impl, true);
        ...
        printer.printPlainLn("Generated " + new Date().toString() + " - V0.9a<br>");
        printer.printPlainLn("<a href=\"testcoverage/test-annotation.html\">" + "annotation progress of test classes</a><br><br>");
        printer.printTableStart();
        printer.printRow("Package", "Classes", "Methods", "Untested", "Partial", "Complete", "Disabled", "Broken", "ToBeFixed", "KnownFail");
     ...
     */
    private void implementedMethod0(List<MethodDoc> resultmethods, MethodDoc doc, ClassDoc testClass, boolean testMethods) {
        if (testMethods) {
            MethodDoc[] methods = testClass.methods();
            for (int j = 0; j < methods.length; j++) {
                MethodDoc methodDoc = methods[j];
                if ((methodDoc.isAbstract() || testClass.isInterface()) && doc.overrides(methodDoc)) {
    ...
    private ColorStat processTestPackage(PackageDoc packageDoc) {
        ColorStat stats = new ColorStat(packageDoc.name(), getPackageBaseLink(packageDoc) + "/package.html");
        if (hasHideFlag(packageDoc)) {
            stats.ignored = true;
            return stats;
        }
        String file = getPackageDir("testcoverage", packageDoc) + "/package.html";
        PrintWriter pr = openFile(file, "Test package " + packageDoc.name());
        TablePrinter printer = new TablePrinter(pr);
        printer.printTableStart();
        printer.printRow("Class", "Extra", "Meth", "Unt", "Part", "Compl", "Disab", "Broken", "ToBeFixed", "KnownFail");
        ...
        closeFile(pr);
        if (junitCnt == 0) {
            if ((packageDoc.name().contains("tests.") || packageDoc.name().contains("junit.") || packageDoc.name().contains(".testframework")) && !(packageDoc.name().equals("junit.framework"))) {
                System.err.println("warning!: no junit classes in package '" + packageDoc.name() + "' even though package name " + "contains tests.,junit. or .testframework");
            }
            stats = new ColorStat(packageDoc.name(), getPackageBaseLink(packageDoc) + "/package.html");
    ...
    private ColorStat processPackage(PackageDoc packageDoc) {
        ...
        for (ClassDoc clazz : classes) {
            ...
            if (hasHideFlag(clazz)) {
                ...
                printer.printRow("ignored (junit test suite class): " + clazz.name());
                junitCnt++;
            } else {
        ...
        printer.printTableEnd();
        closeFile(pr);
        if (junitCnt == cnt || packageDoc.name().contains("tests.") || packageDoc.name().contains("junit.") || packageDoc.name().contains(".testframework") || packageDoc.name().endsWith(".cts")) {
            stats = new ColorStat(packageDoc.name(), getPackageBaseLink(packageDoc) + "/package.html");
            stats.incColor(TestMethodInformation.Color.GREEN);
            stats.setExtra(junitCnt == cnt ? "Ignored since only Junit test and " + "suites" : "Ignored since \"tests.\" in name - recheck");
            stats.ignored = true;
        } else {
            stats.setExtra("" + cnt);
        }
    ...
    private ColorStat processTestClass(ClassDoc clazz) {
        String file = getPackageDir("testcoverage", clazz.containingPackage()) + "/" + clazz.name() + ".html";
        PrintWriter pr = openFile(file, "Test class " + clazz.qualifiedName());
        TablePrinter printer = new TablePrinter(pr);
        ColorStat classStat = new ColorStat(clazz.name(), clazz.name() + ".html");
        ...
        if (targetClass == null) {
            ...
            AnnotationDesc[] annotsC = testTargetClass.untestedMethods.toArray(new AnnotationDesc[] {});
            if (annotsC.length > 0) {
                ColorStat classLevelStat = new ColorStat(clazz.name(), null);
                TestMethodInformation tminfo = new TestMethodInformation(co, annotsC, targetClass);
        ...
        printer.printTableStart();
        printer.printRow("Method", "Note", "Meth", "Unt", "Part", "Compl", "Disab", "Broken", "ToBeFixed", "KnownFail");
        int methodCnt = 0;
        ...
        for (MethodDoc testMethod : testMethods) {
            ...
            String comments = disTest ? "<b><span style=\"background:red\">" + "DISABLED</span></b>" : null;
            MethodOriginator mo = new MethodOriginator(testMethod, clazz, comments);
            AnnotationDesc[] annots = testMethod.annotations();
            TestMethodInformation minfo = new TestMethodInformation(mo, annots, targetClass);
    ...
    private void linkTargets(List<TestTargetNew> targets) {
        for (TestTargetNew ttn : targets) {
            if (ttn.getTargetMethod() != null) {
                AnnotationPointer tar = getAnnotationPointer(ttn.getTargetMethod(), true);
    ...
    private boolean isGreen(TestMethodInformation.Level level) {
        ...
        return lComplete || lPartialOk || lSufficient || lNotFeasible || lNotNecessary;
    }
    ...
    private ColorStat processClass(ClassDoc clazz) {
        ...
        if (classTargets != null) {
            ...
            for (TestTargetNew ttn : classTargets) {
                ...
                line += " <font color=\"" + (isGreen(lev) ? "green" : "red") + "\"><b>" + lev.name() + "</b></font>" + (ttn.getNotes() != null ? "<br>Notes: " + ttn.getNotes() : "") + "</li>";
        ...
        printer.printTableStart();
        printer.printRow("Method", "Tested by", "Meth", "Unt", "Part", "Compl", "Disab", "Broken", "ToBeFixed", "KnownFail");
        ...
        if (_ignoreInterfacesAndAbstractMethods && clazz.isInterface()) {
            classStats = new ColorStat(clazz.name() + (clazz.isInterface() ? " (Interface)" : ""), clazz.name() + ".html");
            ...
            if (atype.toString().equals("dalvik.annotation.TestTargetClass")) {
                ...
                for (int i = 0; i < cpairs.length; i++) {
                    ...
                    if (elName.equals("value")) {
                        ...
                        AnnotationValue[] targets = (AnnotationValue[]) ev.value().value();
    ...
    private List<MethodDoc> collectAllTestMethods(ClassDoc classDoc) {
        ...
        } while ((curCl = curCl.superclass()) != null && !curCl.qualifiedName().equals("junit.framework.TestCase"));
        return m;
    }
    ...
    private List<MethodDoc> getJunitTestMethods(ClassDoc classDoc) {
        ...
        for (MethodDoc methodDoc : classDoc.methods()) {
            if (methodDoc.isPublic() && (methodDoc.name().startsWith("test") || methodDoc.name().startsWith("_test"))) {
                cl.add(methodDoc);
            }
        }
        return cl;
    ...
    private ColorStat processElement(ExecutableMemberDoc method) {
        ...
        if (_ignoreInterfacesAndAbstractMethods && method instanceof MethodDoc && ((MethodDoc) method).isAbstract()) {
            ...
            if (ap != null) {
                ...
                if (testedByCnt == 0) {
                    throw new RuntimeException("existing annotation pointer with no entries!, " + "method:" + method);
                }
                String by = "<ul>";
                ...
                for (TestTargetNew target : targets) {
                    ...
                    by += " <font color=\"" + (lComplete || lPartialOk || lSufficient || lNotFeasible || lNotNecessary ? "green" : "red") + "\"><b>" + lev.name() + "</b></font>" + (target.getNotes() != null ? "<br>Notes: " + target.getNotes() : "");
                    if ((_doIncludeDisabledTests || !disabledTest) && (!brokenTest) && (!toBeFixed)) {
                        if (lComplete) {
                            completeTestCnt++;
                        } else if (lPartialOk) {
                            partialOkTestCnt++;
                    ...
                    if (DEBUG) {
                        System.out.println("completeTestCnt: " + completeTestCnt + ", partialOkTestCnt: " + partialOkTestCnt + ", partialTestCnt: " + partialTestCnt + ", todoTestCnt: " + todoTestCnt + ", sufficientTestCnt: " + sufficientTestCnt + ", notFeasableTestCnt: " + notFeasableTestCnt + ", notNecessaryTestCnt: " + notNecessaryTestCnt);
                    }
                    by += "</li>";
                }
                String warnings = "";
                int singularTestCnt = notFeasableTestCnt + notNecessaryTestCnt;
                boolean isAbstract = (method.containingClass().isInterface() || (method instanceof MethodDoc) && ((MethodDoc) method).isAbstract());
                if (_acceptCompleteWithOtherStatus && (completeTestCnt > 0 || sufficientTestCnt > 0)) {
                    c = TestMethodInformation.Color.GREEN;
                } else if (_acceptCompleteWithOtherStatus && (partialOkTestCnt > 1)) {
                    c = TestMethodInformation.Color.GREEN;
                } else {
                    if (singularTestCnt > 0) {
                        if (targets.size() > singularTestCnt) {
                            c = TestMethodInformation.Color.RED;
                            warnings += "<b>WARNING:</b>NOT_FEASIBLE or " + "NOT_NECESSARY together with other " + "status!<br>";
                        } else {
                            if (notNecessaryTestCnt > 0 && notFeasableTestCnt > 0) {
                                warnings += "<b>WARNING:</b>both NOT_FEASIBLE " + "and NOT_NECESSARY together!<br>";
                                c = TestMethodInformation.Color.RED;
                            } else {
                                c = TestMethodInformation.Color.GREEN;
                            }
                        ...
                        if (completeTestCnt > 0 || sufficientTestCnt > 0) {
                            if (_acceptCompleteWithOtherStatus) {
                                ...
                                warnings += "<b>WARNING</b>: mixed PARTIAL " + "and COMPLETE status<br>";
                            }
                        }
                    } else if (partialOkTestCnt > 0 || sufficientTestCnt > 0) {
                        c = TestMethodInformation.Color.GREEN;
                        if (partialOkTestCnt == 1) {
                            warnings += "<b>WARNING</b>: only one " + "PARTIAL_COMPLETE status<br>";
                            c = TestMethodInformation.Color.YELLOW;
                        }
                    } else if (completeTestCnt > 0 || singularTestCnt == 1) {
                        c = TestMethodInformation.Color.GREEN;
                    }
                    if (completeTestCnt > 1 && !isAbstract && !_acceptCompleteWithOtherStatus) {
                        warnings += "<b>WARNING</b>: more than one " + "COMPLETE status<br>";
                ...
                if (method.isConstructor() && method.signature().equals("()")) {
                    if (method.position() != null) {
                        ...
                        String classPos = method.containingClass().position().toString();
                        if (constPos.equals(classPos)) {
                            c = TestMethodInformation.Color.GREEN;
                            memberStats.setExtra("automatically marked green " + "since implicit default " + "constructor");
                        }
                    } else {
                        System.err.println("warning: doclet returned null for " + "source position: method:" + method);
                    }
                } else if (method.containingClass().superclass() != null && method.containingClass().superclass().qualifiedName().equals("java.lang.Enum")) {
                    String sig = method.name() + method.signature();
                    if (sig.equals("valueOf(java.lang.String)") || sig.equals("values()")) {
                        c = TestMethodInformation.Color.GREEN;
                        memberStats.setExtra("automatically marked green since " + "generated by compiler for enums");
                    }
                }
            }
        }
    ...
    private String getMethodString(ExecutableMemberDoc method) {
        String methodDesc = (method.isPublic() ? "public " : method.isProtected() ? "protected " : method.isPrivate() ? "private " : "");
        return methodDesc + "<b>" + method.name() + "</b> " + method.signature();
    }

    private String getClassString(ClassDoc clazz) {
        return (clazz.isPublic() ? "public " : clazz.isProtected() ? "protected " : clazz.isPrivate() ? "private " : "") + (clazz.isInterface() ? "interface" : "class") + " " + clazz.name();
    }

    private void printTestStats(TablePrinter printer, ColorStat stat, boolean wantLink) {
        printStats(printer, stat, wantLink);
    }

    private void printStats(TablePrinter printer, ColorStat stat, boolean wantLink) {
        ...
        printer.printRow(namePart, extra, "" + total, "" + redCnt, "" + yellowCnt, "" + greenCnt, "" + disabledCnt, "" + brokenTestCnt, "" + toBeFixedCnt, "" + knownFailureCnt, "" + (redCnt == 0 ? "" : "<span style=\"background:" + COLORS[TestMethodInformation.Color.RED.ordinal()] + "\">" + getDots(redD) + "</span>") + (yellowCnt == 0 ? "" : "<span style=\"background:" + COLORS[TestMethodInformation.Color.YELLOW.ordinal()] + "\">" + getDots(yellowD) + "</span>") + (greenCnt == 0 && total > 0 ? "" : "<span style=\"background:" + COLORS[TestMethodInformation.Color.GREEN.ordinal()] + "\">" + getDots(greenD) + "</span>") + "&nbsp;&nbsp;&nbsp;<span style=\"background:blue\">" + getDots(total / 10) + "</span>");
    }
    ...
    public static boolean start(RootDoc root) {
        try {
            ...
            System.out.println("Please see complete report in " + file.getAbsolutePath());
        } catch (Exception ex) {
            ex.printStackTrace();
            return false;
        }
