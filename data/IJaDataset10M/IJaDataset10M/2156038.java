...
public class RDPAdapter extends RDPCanvas implements InputListener {
    ...
    public void start(ThinClient displayClient, Session session) {
        ...
        if (os.equals("Windows 2000") || os.equals("Windows XP")) Options.built_in_licence = true;
        logger.info("Operating System is " + os + " version " + osvers);
        if (os.startsWith("Linux")) Constants.OS = Constants.LINUX; else if (os.startsWith("Windows")) Constants.OS = Constants.WINDOWS; else if (os.startsWith("Mac")) Constants.OS = Constants.MAC;
        if (Constants.OS == Constants.MAC) Options.caps_sends_up_and_down = false;
        ...
        try {
            InputStream istr = RDPError.class.getResourceAsStream("/" + keyMapPath + mapFile);
            if (istr == null) {
            ...
            EventManager.getInstance().add(new Event("Keymap not loaded", kmEx.getMessage(), Event.TYPE_ERROR));
        ...
        logger.debug("keep_running = " + keep_running);
        {
            ...
            if (rdp != null) {
                try {
                    rdp.connect(Options.username, InetAddress.getByName(server), logonflags, Options.domain, Options.password, Options.command, Options.directory);
                    if (keep_running) {
                        if (!Options.packet_encryption) Options.encryption = false;
                        logger.info("Connection successful");
                        EventManager.getInstance().add(new Event("RDP Connection successfull to " + server, displayClient.getName() + "connected to " + server, Event.TYPE_INFO));
                        rdp.mainLoop(deactivated, ext_disc_reason, this);
                        if (deactivated[0]) {
                            rdp.disconnect();
                        } else {
                            if (ext_disc_reason[0] == RDPError.exDiscReasonAPIInitiatedDisconnect || ext_disc_reason[0] == RDPError.exDiscReasonAPIInitiatedLogoff) {
                                rdp.disconnect();
                            }
                            if (ext_disc_reason[0] >= 2) {
                                String reason = RDPError.getMessageFromDisconnectError(ext_disc_reason[0]);
                                EventManager.getInstance().add(new Event("Connection terminated", reason, Event.TYPE_WARNING));
                                rdp.disconnect();
                            }
                        }
                        keep_running = false;
                        if (!readytosend) {
                            String msg1 = "The RDP server (" + server + ") disconnected before licence negotiation completed.";
                            String msg2 = "Possible cause: terminal server could not issue a licence.";
                            logger.warn(msg1);
                            logger.warn(msg2);
                            EventManager.getInstance().add(new Event("RDP server disconnected", msg1, Event.TYPE_WARNING));
                        }
                    }
                } catch (ConnectionException e) {
                    EventManager.getInstance().add(new Event("RDP connection failed to " + server, e.getMessage(), Event.TYPE_ERROR));
                    e.printStackTrace();
                    rdp.disconnect();
                    ...
                    e.printStackTrace();
                    if (!readytosend) {
                        EventManager.getInstance().add(new Event("RDP connection failed", "RDP server reset connection before licence negotiation", Event.TYPE_WARNING));
                        rdp.disconnect();
                    } else {
                        EventManager.getInstance().add(new Event("RDP connection failed to " + server, e.getMessage(), Event.TYPE_WARNING));
                        rdp.disconnect();
                    }
                } catch (Exception e) {
    ...
    public void repaint(int x, int y, int width, int height) {
        ...
        try {
            ...
            if (DEBUG_REPAINT) {
                try {
                    ImageIO.write(image, "png", new File("out/" + counter + ".png"));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        } catch (Exception e) {
            System.out.println("RDPAdapter.repaint():" + x + "," + y + " " + width + "x" + height);
            e.printStackTrace();
        }
    }
    ...
    public void mouseMoved(int mouseX, int mouseY) {
        ...
        if ((mTime - last_mousemove) < (1000 / 25)) return;
        last_mousemove = mTime;
        if (rdp != null) {
            rdp.sendInput((int) System.currentTimeMillis(), RDP_INPUT_MOUSE, MOUSE_FLAG_MOVE, mouseX, mouseY);
        }
    }
    ...
    public void mousePressed(int button, int mouseX, int mouseY) {
        if (rdp != null) {
            if (button == MouseEvent.BUTTON1) {
                ...
                rdp.sendInput((int) System.currentTimeMillis(), RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON1 | MOUSE_FLAG_DOWN, mouseX, mouseY);
            } else if (button == MouseEvent.BUTTON2) {
                logger.debug("Mouse Button 2 Pressed.");
                rdp.sendInput((int) System.currentTimeMillis(), RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON3 | MOUSE_FLAG_DOWN, mouseX, mouseY);
            } else if (button == MouseEvent.BUTTON3) {
                logger.debug("Middle Mouse Button Pressed.");
                rdp.sendInput((int) System.currentTimeMillis(), RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON2 | MOUSE_FLAG_DOWN, mouseX, mouseY);
            }
        }
    }
    ...
    public void mouseReleased(int button, int mouseX, int mouseY) {
        if (rdp != null) {
            ...
            if (button == MouseEvent.BUTTON1) {
                ...
                rdp.sendInput(time, RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON1, mouseX, mouseY);
            } else if (button == MouseEvent.BUTTON2) {
                logger.debug("Mouse Button 2 released.");
                rdp.sendInput(time, RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON3, mouseX, mouseY);
            } else if (button == MouseEvent.BUTTON3) {
                logger.debug("Mouse Button 31 released.");
                rdp.sendInput(time, RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON2, mouseX, mouseY);
            }
        }
    }

    @Override
    public void keyPressed(int key, boolean shift, boolean ctrl, boolean alt, boolean meta, boolean altGr) {
        KeyEvent e = new KeyEvent(this, KeyEvent.KEY_PRESSED, System.currentTimeMillis(), 0, key);
        ...
        logger.info("PRESSED keychar='" + e.getKeyChar() + "' keycode=0x" + Integer.toHexString(e.getKeyCode()) + " char='" + ((char) e.getKeyCode()) + "'");
        if (rdp != null) {
    ...
    public void keyReleased(int key) {
        KeyEvent e = new KeyEvent(this, KeyEvent.KEY_RELEASED, System.currentTimeMillis(), 0, key);
        ...
        input.modifiersValid = true;
        long time = input.getTime();
        logger.info("RELEASED keychar='" + e.getKeyChar() + "' keycode=0x" + Integer.toHexString(e.getKeyCode()) + " char='" + ((char) e.getKeyCode()) + "'");
        if (rdp != null) {
            long t = input.getTime();
    ...
    public void mouseWheelDown(int mouseX, int mouseY) {
        if (rdp != null) {
            rdp.sendInput((int) System.currentTimeMillis(), RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON5 | MOUSE_FLAG_DOWN, mouseX, mouseY);
        }
    }
    ...
    public void mouseWheelUp(int mouseX, int mouseY) {
        if (rdp != null) {
            rdp.sendInput((int) System.currentTimeMillis(), RDP_INPUT_MOUSE, MOUSE_FLAG_BUTTON4 | MOUSE_FLAG_DOWN, mouseX, mouseY);
        }
    }
    ...
    public int[] getCursor(int w, int h, byte[] andmask, byte[] xormask) {
        ...
        for (int i = 0; i < h; i++) {
            ...
            pcursor = offset;
            for (int j = 0; j < w; j++) {
                cursor[pcursor] = ((xormask[pxormask + 2] << 16) & 0x00ff0000) | ((xormask[pxormask + 1] << 8) & 0x0000ff00) | (xormask[pxormask] & 0x000000ff);
                pxormask += 3;
                pcursor++;
            }
        }
    ...
    public Cursor createCursor(int x, int y, int w, int h, byte[] andmask, byte[] xormask, int cache_idx) {
        ...
        final SetMouseCursorOperation op = new SetMouseCursorOperation(x, y, w, h, Color.WHITE, Color.BLACK, bitmap, mask);
    ...
    public void setCursorFromIdx(int cache_idx) {
        ...
        if (op == null) {
            op = new SetMouseCursorOperation(SetMouseCursorOperation.UNIX_CURSOR);
        }
        DisplayMessage m = new DisplayMessage(client.getWriter());
        m.addOperation(op);
        this.client.getWriter().addMessage(m);
