...
public class ErrorLogManager extends AbstractFeedbackManager {
    ...
    protected static final Logger log = Logger.getLogger(ErrorLogManager.class.getName());
    ...
    public ErrorLogManager(Saros saros, SarosSessionManager sessionManager, StatisticManager statisticManager, SessionIDObservable sessionID) {
        super(saros);
    ...
    public int getErrorLogSubmissionStatus() {
        int status = saros.getConfigPrefs().getInt(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION, UNDEFINED);
        if (status == UNDEFINED) status = saros.getPreferenceStore().getInt(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION);
        return status;
    }
    ...
    public void setErrorLogSubmissionAllowed(boolean allowed) {
        ...
        saros.getConfigPrefs().putInt(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION, submission);
        saros.saveConfigPrefs();
        saros.getPreferenceStore().setValue(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION, submission);
    }
    ...
    public int getFullErrorLogSubmissionStatus() {
        int status = saros.getConfigPrefs().getInt(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION_FULL, UNDEFINED);
        if (status == UNDEFINED) status = saros.getPreferenceStore().getInt(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION_FULL);
        return status;
    }
    ...
    public void setFullErrorLogSubmissionAllowed(boolean allowed) {
        int submission = allowed ? ALLOW : FORBID;
        saros.getConfigPrefs().putInt(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION_FULL, submission);
        saros.saveConfigPrefs();
        saros.getPreferenceStore().setValue(PreferenceConstants.ERROR_LOG_ALLOW_SUBMISSION_FULL, submission);
    }
    ...
    public void submitErrorLog() {
        final File errorLog = isFullErrorLogSubmissionAllowed() ? getFullErrorLogFile() : getErrorsOnlyLogFile();
        if (errorLog == null) return;
        final String logNameExtended = FilenameUtils.getBaseName(errorLog.getName()) + "_" + statisticManager.getUserID() + "_" + currentSessionID;
        new Job("Uploading Error Log...") {

            @Override
            protected IStatus run(IProgressMonitor monitor) {
                try {
                    FileSubmitter.uploadErrorLog(eclipseHelper.getStateLocation().toOSString(), logNameExtended, errorLog, SubMonitor.convert(monitor));
                } catch (IOException e) {
                    String msg = String.format("Couldn't upload file: %s. %s", e.getMessage(), e.getCause() != null ? e.getCause().getMessage() : "");
                    log.error(msg);
