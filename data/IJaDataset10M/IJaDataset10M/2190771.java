...
public class POP3Store implements Observer {
	 ...
	 */
    protected void login() throws IOException, POP3Exception, CommandCancelledException {
        ...
        while (!login) {
            if ((password = Blowfish.decrypt(popItem.getRoot().getAttribute("password", ""))).length == 0) {
                dialog = new PasswordDialog();
                dialog.showDialog(MessageFormat.format(MailResourceLoader.getString("dialog", "password", "enter_password"), new Object[] { popItem.get("user"), popItem.get("host") }), popItem.get("password"), popItem.getBoolean("save_password"));
                if (dialog.success()) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, e.getMessage(), "Authorization failed!", JOptionPane.ERROR_MESSAGE);
    ...
    public static void doPOPbeforeSMTP(PopItem popItem) throws IOException, POP3Exception, CommandCancelledException {
        POP3Store store = new POP3Store(popItem);
        store.ensureTransaction();
        store.logout();
    }

    protected void ensureTransaction() throws IOException, POP3Exception, CommandCancelledException {
        ensureAuthorization();
    ...
    protected void ensureAuthorization() throws IOException, POP3Exception, CommandCancelledException {
	 ...
	 * 
	 */
    private void openConnection() throws IOException, POP3Exception, CommandCancelledException {
    ...
    private int showErrorDialog(String message) {
        int result = JOptionPane.showConfirmDialog(null, message, "Warning", JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION);
        return result;
    }
