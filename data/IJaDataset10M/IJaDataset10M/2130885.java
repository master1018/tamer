 ...
 */
public class XampilExpressionParser extends GJNoArguDepthFirst<InstrBooleanTerm> implements GJNoArguVisitor<InstrBooleanTerm> {
    ...
    public InstrBooleanTerm visit(OrExpression n) {
        ...
        for (Node node : n.f1.nodes) {
            ...
            returnTerm = new InstrOperatorTerm(returnTerm, XampilBooleanOperators.getOrOperator(), term, operatorToken.startOffset, operatorToken.endOffset);
        }
        return returnTerm;
    }
    ...
    public InstrBooleanTerm visit(AndExpression n) {
        ...
        for (Node node : n.f1.nodes) {
            ...
            NodeToken operatorToken = (NodeToken) nodeSequence.nodes.get(0);
            InstrBooleanTerm term = nodeSequence.nodes.get(1).accept(this);
            returnTerm = new InstrOperatorTerm(returnTerm, XampilBooleanOperators.getAndOperator(), term, operatorToken.startOffset, operatorToken.endOffset);
        }
        return returnTerm;
    }
    ...
    public InstrBooleanTerm visit(NotExpression n) {
        ...
        if (n.f0.present()) {
            ...
            returnTerm = new InstrOperatorTerm(XampilBooleanOperators.getNotOperator(), returnTerm, operatorToken.startOffset, operatorToken.endOffset);
        }
        return returnTerm;
    }
    ...
    public InstrBooleanTerm visit(BasicExpression n) {
        ...
        if (n.f0.which == 0) {
            ...
            returnTerm = new InstrOperatorTerm(XampilBooleanOperators.getTrueOperator(), token.startOffset, token.endOffset);
        } else if (n.f0.which == 4) {
            NodeToken token = (NodeToken) n.f0.choice;
            returnTerm = new InstrOperatorTerm(XampilBooleanOperators.getFalseOperator(), token.startOffset, token.endOffset);
