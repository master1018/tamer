...
final class HeadRequester implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (iter.hasNext()) {
                ...
                head.addRequestHeader(HTTPHeaderName.GNUTELLA_CONTENT_URN.httpStringValue(), RESOURCE_NAME.httpStringValue());
                head.addRequestHeader(HTTPHeaderName.ALT_LOCATION.httpStringValue(), TOTAL_ALTS.httpStringValue());
                head.addRequestHeader(HTTPHeaderName.CONNECTION.httpStringValue(), "close");
                head.setFollowRedirects(false);
                HttpClient client = HttpClientManager.getNewClient();
                try {
                    client.executeMethod(head);
                    String contentUrn = getHeader(head, HTTPHeaderName.GNUTELLA_CONTENT_URN);
                    if (contentUrn == null) continue;
                    try {
                    ...
                    String altLocs = getHeader(head, HTTPHeaderName.ALT_LOCATION);
                    if (altLocs == null) continue;
                    AlternateLocationCollection alc = AlternateLocationCollection.createCollectionFromHttpValue(altLocs);
                    if (alc == null) continue;
                    if (alc.getSHA1Urn().equals(COLLECTOR.getSHA1Urn())) COLLECTOR.addAll(alc);
                } catch (IOException e) {
                    continue;
                } finally {
    ...
    private static String getHeader(HttpMethod methid, HTTPHeaderName name) {
        ...
        if (header == null) return null; else return header.getValue();
    }
}
