...
public class AnnotationInjectionContainer extends InjectionContainerBase implements InjectionContainer, RegistrationInjectionContainer {
    ...
    public synchronized void register(InjectionKey key, Class service, SimpleInjection.Scope scope, SimpleInjection.RegisterType type, boolean throwError) {
    ...
    public Class findService(InjectionKey key) {
        ...
        if (serviceRegister != null) {
            ...
            throw new InjectRuntimeException("Service {0} with Name {1} not registered in SimpleInjection", key.getServiceDefinition(), key.getQualifier());
        }
    }
    ...
    private ServiceRegister createAnStoreRegistration(RegistrationInstanceSimple instance, InjectionKey key, RegistrationModuleAnnotation aModule) {
    ...
    private ServiceRegister createServiceRegister(RegistrationInstanceSimple instance, InjectionMetaData injectionMetaData) {
        if (instance.getTheInstance() == null) {
            return new ServiceRegister(instance.getService(), null, getAnnotationScope(injectionMetaData), SimpleInjection.RegisterType.NORMAL);
        } else {
            return new ServiceRegister(instance.getService(), instance.getTheInstance(), SimpleInjection.Scope.SINGLETON, SimpleInjection.RegisterType.NORMAL);
        }
    }
