...
public class JDLabel extends JDRectangular {
   ...
   */
    ...
    public void paint(JDrawEditor parent, Graphics g) {
        ...
        if (w < d.width || h < d.height) {
            for (found = false, hmax = 0, i = 0; i < nb && !found; ) {
                ...
                if (textOrientation == LEFT_TO_RIGHT || textOrientation == RIGHT_TO_LEFT) found = (hmax + bh) > h; else found = (hmax + bh) > w;
                if (!found) {
                    i++;
                    hmax += bh;
                }
    ...
    public void rotate90(double x, double y) {
        super.rotate90(x, y);
        textOrientation++;
        if (textOrientation > TOP_TO_BOTTOM) textOrientation = LEFT_TO_RIGHT;
    }
   ...
   */
    ...
    void saveObject(FileWriter f, int level) throws IOException {
        ...
        String to_write;
        if (theFont.getName() != fontDefault.getName() || theFont.getStyle() != fontDefault.getStyle() || theFont.getSize() != fontDefault.getSize()) {
