...
public class ExponentialDistribution extends ContinousDistribution {
   ...
   */
    ...
    public double getDensityAt(double x) {
        if (x < getMin() || x > getMax()) return 0; else return lambda * Math.exp(-lambda * (x - getMin()));
    }
   ...
   */
    public void setLambda(double lambda) {
        if (lambda <= 0) throw new IllegalArgumentException("Lambda has to be greater than zero.");
        this.lambda = lambda;
    }
