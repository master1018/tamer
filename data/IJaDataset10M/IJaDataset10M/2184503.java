...
public class Subtract extends MarvinAbstractImagePlugin {
    ...
    public void process(MarvinImage a_imageIn, MarvinImage a_imageOut, MarvinAttributes a_attributesOut, MarvinImageMask a_mask, boolean a_previewMode) {
        ...
        int l_red, l_redBg, l_green, l_greenBg, l_blue, l_blueBg;
        for (int y = 0; y < a_imageIn.getHeight(); y++) {
            for (int x = 0; x < a_imageIn.getWidth(); x++) {
                ...
                if (Math.abs(l_redBg - l_red) > colorRange || Math.abs(l_greenBg - l_green) > colorRange || Math.abs(l_blueBg - l_blue) > colorRange) {
