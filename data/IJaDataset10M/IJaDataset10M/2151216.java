...
public final class DefaultRssFeedGenerator implements RssFeedGenerator {
   ...
   */
    ...
    @Override
    public void outputFeed(final RepositoryConfiguration configuration, final List<LogEntry> logEntries, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
   ...
   */
    private List<SyndEntry> createEntries(final RepositoryConfiguration configuration, final List<LogEntry> logEntries, final String baseURL, final HttpServletResponse response) throws IOException {
        ...
        for (final LogEntry logEntry : logEntries) {
            if (logEntry.isAccessible()) {
                ...
                entry.setTitle("Revision " + logEntry.getRevision() + " - " + StringUtils.trimToEmpty(getAbbreviatedLogMessage(StringEscapeUtils.escapeHtml(logEntry.getMessage()), logMessageLength)));
                ...
                description.setValue(HTMLCreator.createRevisionDetailBody(configuration.getRssTemplate(), logEntry, baseURL, configuration.getName(), dateFormat, response));
