 ...
 * @param <R>
 */
public abstract class AbstractAsynchronousModuleDelegate<M extends IAsynchronousModule, R> implements IAsynchronousModuleDelegate<M, R> {

    /**
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(AbstractAsynchronousModuleDelegate.class);
    ...
    public AbstractAsynchronousModuleDelegate(M module, double minimumProcessingTime, R cantProcessResult) {
        _module = module;
    ...
    public Future<R> process(final IRequest request, double processTime, final Object... parameters) {
        double now = processTime;
        double blockAt = now + _minimumProcessingTime;
        boolean canProcess = shouldProcess(request, parameters);
        if (canProcess) {
            final BlockingTimedEvent synchronizationBlock = getModule().synchronizedTimedEvent(now, blockAt);
            ...
            Future<R> rtn = AbstractModule.delayedFuture(new Callable<R>() {
                ...
                public R call() throws Exception {
                    ...
                    double harvestAt = computeHarvestTime(request, result, startTime, parameters);
                    if (LOGGER.isDebugEnabled()) LOGGER.debug(getModule() + " requested, started at " + startTime + " will harvest " + result + " at " + harvestAt);
            ...
            return rtn;
        } else return AbstractModule.immediateReturn(_errorResult);
    }
    ...
    protected ITimedEvent createHarvestTimedEvent(double start, double end, final IRequest request, final R result, final Object... parameters) {
        return new AbstractTimedEvent(start, end) {
            ...
            public void fire(double currentTime) {
                ...
                if (LOGGER.isDebugEnabled()) LOGGER.debug(getModule() + " harvesting " + result + " at " + currentTime);
   ...
   * @return
   */
    protected abstract boolean shouldProcess(IRequest request, Object... parameters);
   ...
   * @return
   */
    protected abstract R processInternal(IRequest request, Object... parameters);
   ...
   * @param result
   */
    protected abstract void finalizeProcessing(IRequest request, R result, Object... parameters);
   ...
   * @return
   */
    protected abstract double computeHarvestTime(IRequest request, R result, double startTime, Object... parameters);
}
