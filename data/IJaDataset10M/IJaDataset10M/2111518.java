...
public class Dialog extends JPanel implements JmolDialogInterface {
    ...
    public String getOpenFileNameFromDialog(String appletContext, JmolViewer viewer, String fileName, Object historyFileObject, String windowName, boolean allowAppend) {
        ...
        if (openPreview == null && (viewer.isApplet() || Boolean.valueOf(System.getProperty("openFilePreview", "true")).booleanValue())) {
            openPreview = new FilePreview(openChooser, viewer.getModelAdapter(), allowAppend, appletContext);
        }
        if (historyFile != null) {
            Dimension dim = historyFile.getWindowSize(windowName);
            if (dim != null) openChooser.setDialogSize(dim);
            Point loc = historyFile.getWindowPosition(windowName);
            if (loc != null) openChooser.setDialogLocation(loc);
        }
        openChooser.resetChoosableFileFilters();
        if (fileName != null) {
            int pt = fileName.lastIndexOf(".");
            String sType = fileName.substring(pt + 1);
            if (pt >= 0 && sType.length() > 0) openChooser.addChoosableFileFilter(new TypeFilter(sType));
            if (fileName.indexOf(".") == 0) fileName = "Jmol" + fileName;
            if (fileName.length() > 0) openChooser.setSelectedFile(new File(fileName));
        }
        ...
        File file = null;
        if (openChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) file = openChooser.getSelectedFile();
        if (file == null) return closePreview();
        if (historyFile != null) historyFile.addWindowInfo(windowName, openChooser.getDialog(), null);
    ...
    String closePreview() {
        if (openPreview != null) openPreview.updatePreview(null);
        return null;
    }
    ...
    public String getSaveFileNameFromDialog(JmolViewer viewer, String fileName, String type) {
        ...
        if (fileName != null) {
            int pt = fileName.lastIndexOf(".");
            String sType = fileName.substring(pt + 1);
            if (pt >= 0 && sType.length() > 0) saveChooser.addChoosableFileFilter(new TypeFilter(sType));
            if (fileName.equals("*")) fileName = viewer.getModelSetFileName();
            if (fileName.indexOf(".") == 0) fileName = "Jmol" + fileName;
            file = new File(fileName);
        }
        if (type != null) saveChooser.addChoosableFileFilter(new TypeFilter(type));
        saveChooser.setSelectedFile(file);
        if ((file = showSaveDialog(this, saveChooser, file)) == null) return null;
        FileManager.setLocalPath(viewer, file.getParent(), true);
    ...
    public String getImageFileNameFromDialog(JmolViewer viewer, String fileName, String type, String[] imageChoices, String[] imageExtensions, int qualityJPG0, int qualityPNG0) {
        if (qualityJPG0 < 0 || qualityJPG0 > 100) qualityJPG0 = qualityJPG;
        if (qualityPNG0 < 0) qualityPNG0 = qualityPNG;
        ...
        if (fileName == null) {
            ...
            for (int i = 0; i < imageExtensions.length; i++) if (type.equals(imageChoices[i]) || type.toLowerCase().equals(imageExtensions[i])) {
                type = imageChoices[i];
                break;
            }
        }
        ...
        if ((file = showSaveDialog(this, imageChooser, file)) == null) return null;
        qualityJPG = qSliderJPEG.getValue();
        qualityPNG = qSliderPNG.getValue();
        if (cb.getSelectedIndex() >= 0) defaultChoice = cb.getSelectedIndex();
        FileManager.setLocalPath(viewer, file.getParent(), true);
        return file.getAbsolutePath();
    }
    ...
    private void createExportPanel(String[] choices, String[] extensions, String type) {
        ...
        for (defaultChoice = choices.length; --defaultChoice >= 1; ) if (choices[defaultChoice].equals(type)) break;
        ...
        qPanelJPEG.setBorder(new TitledBorder(GT._("JPEG Quality ({0})", qualityJPG)));
        ...
        qPanelPNG.setBorder(new TitledBorder(GT._("PNG Compression  ({0})", qualityPNG)));
    ...
    public class ExportChoiceListener implements ItemListener {
        ...
        public void itemStateChanged(ItemEvent e) {
            ...
            if (selectedFile == null) selectedFile = initialFile;
            File newFile = null;
            String name;
            ...
            imageChooser.resetChoosableFileFilters();
            imageChooser.addChoosableFileFilter(new TypeFilter(extension));
            if (newFile != null) imageChooser.setSelectedFile(newFile);
    ...
    private static boolean doOverWrite(JFileChooser chooser, File file) {
        ...
        int opt = JOptionPane.showOptionDialog(chooser, GT._("Do you want to overwrite file {0}?", file.getAbsolutePath()), GT._("Warning"), JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
        return (opt == 0);
    }

    private File showSaveDialog(Component c, JFileChooser chooser, File file) {
        while (true) {
            if (chooser.showSaveDialog(c) != JFileChooser.APPROVE_OPTION) return null;
            if (cb != null && cb.getSelectedIndex() >= 0) defaultChoice = cb.getSelectedIndex();
            if ((file = chooser.getSelectedFile()) == null || !file.exists() || doOverWrite(chooser, file)) return file;
        }
    }
    ...
    public void setupUI(boolean forceNewTranslation) {
        if (forceNewTranslation || !haveTranslations) setupUIManager();
            ...
            SwingUtilities.invokeAndWait(new Runnable() {
                ...
                public void run() {
                    if (dialogType.equals("load")) {
                        outputFileName = getOpenFileNameFromDialog(viewer.getAppletContext(), viewer, inputFileName, null, null, false);
                        return;
                    }
                    if (dialogType.equals("save")) {
                        outputFileName = getSaveFileNameFromDialog(viewer, inputFileName, null);
                        return;
                    }
                    if (dialogType.startsWith("saveImage")) {
                        outputFileName = getImageFileNameFromDialog(viewer, inputFileName, imageType, imageChoices, imageExtensions, qualityJ, qualityP);
                        return;
                    }
                    outputFileName = null;
                }
