...
public class PirolEdge {
    ...
    public static final PirolEdge shiftLine(PirolEdge lineToShift, PirolPoint shiftingVector) throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            if (i == 0) currPkt = lineToShift.getAnfang(); else currPkt = lineToShift.getEnde();
            for (int dim = 0; dim < coords.length; dim++) {
                coords[dim] = currPkt.getCoordinate(dim) + shiftingVector.getCoordinate(dim);
            }
            if (i == 0) spkt = new PirolPoint((double[]) coords.clone()); else epkt = new PirolPoint((double[]) coords.clone());
        }
    ...
    public double getSteigung() throws Exception {
        ...
        if (deltaY > 0) return Double.POSITIVE_INFINITY; else if (deltaY < 0) return Double.NEGATIVE_INFINITY; else throw new Exception("wasn't able to calculate the increase of line (starting point == end point)");
    }
    ...
    public static PirolPoint getSchnittpunkt(PirolEdge k1, PirolEdge k2) throws Exception {
        ...
        if (ab.getX() == 0 && cd.getX() != 0) {
            ...
            if (a.getX() != c.getX()) return null; else if ((c.getY() >= a.getY() && c.getY() <= a.getY() + ab.getY()) || (c.getY() <= a.getY() && c.getY() >= a.getY() + ab.getY())) return new PirolPoint(new double[] { a.getX(), c.getY() }); else if ((a.getY() >= c.getY() && a.getY() <= c.getY() + cd.getY()) || (a.getY() <= c.getY() && a.getY() >= c.getY() + cd.getY())) return new PirolPoint(new double[] { a.getX(), a.getY() }); else return null;
        ...
        if (!k1.isAnfangUnbegrenzt() && n <= 0) return null;
        if (!k1.isEndeUnbegrenzt() && n >= 1) return null;
    ...
    public double getNormalenFaktorZu(PirolPoint pkt) throws Exception {
        ...
        double ergebnis = 0;
        try {
            if (ab.getX() != 0 && ab.getY() != 0) ergebnis = ((punkt1.getY() - punkt3.getY()) - (punkt1.getX() - punkt3.getX()) * ab.getY() / ab.getX()) / (N.getX() * ab.getY() / ab.getX() - N.getY()); else if (ab.getX() == 0) ergebnis = (punkt3.getX() - punkt1.getX()) / N.getX(); else ergebnis = (punkt3.getY() - punkt1.getY()) / N.getY();
        } catch (Exception e) {
            ergebnis = 1;
            e.printStackTrace();
        }
