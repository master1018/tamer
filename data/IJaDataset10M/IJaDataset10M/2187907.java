...
public class BusStopAllocator {
	 ...
	 */
    public BusStopAllocator(NetworkImpl network, Collection<TransitStopFacility> stops, String outputFile) {
    ...
    protected void allocateAllStops1() {
        for (TransitStopFacility stop : stops) allocateStop1(stop);
    }

    protected void allocateAllStops2() {
        for (TransitStopFacility stop : stops) allocateStop2(stop);
    }
    ...
    private void allocateStop1(TransitStopFacility stop) {
        ...
        if (link != null) {
            ...
            if (link.getToNode().equals(node)) isToNode = "to"; else if (link.getFromNode().equals(node)) isToNode = "from"; else isToNode = node.getId().toString();
    ...
    private void allocateStop2(TransitStopFacility stop) {
        ...
        if (link != null) {
            ...
            String isToNode = null;
            if (nodes.contains(link.getToNode())) isToNode = "to"; else if (nodes.contains(link.getFromNode())) isToNode = "from"; else isToNode = network.getNearestNode(stopCoord).getId().toString();
            resultIsToNodes.put(stopId, isToNode);
        }
    }
    ...
    private void output() {
        ...
        for (Id stopId : resultIds.keySet()) writer.writeln(stopId.toString() + "\t" + resultIds.get(stopId).toString() + "\t" + resultIsToNodes.get(stopId));
        writer.close();
    }
    ...
    public static void main(String[] args) {
        ...
        BusStopAllocator stopAllocator = new BusStopAllocator((NetworkImpl) carNetwork, schedule.getFacilities().values(), outputFile);
        stopAllocator.run2();
    }
}
