...
public class KPlexesTest {
    ...
    public void testConnectionsByNode() {
        Map<Integer, Integer> calculations = kp.connectionsByNode(graphWithThreeCliques());
        ...
        for (Integer i = 0; i < answers.length; i++) {
            ...
            assertEquals(node + " has " + answer + " connections", answer, calculated);
        }
    }

    @Test
    public void testConnectednessByNode() {
        Map<Integer, Integer> calculations = kp.connectednessByNode(graphWithThreeCliques());
        Integer[] answers = new Integer[] { 2, 2, 3, 2, 2, 2, 2, 1, 1 };
        for (Integer i = 0; i < answers.length; i++) {
            Integer node = i + 1;
            Integer answer = answers[i];
            Integer calculated = calculations.get(node);
            assertEquals(node + " has connectedness of " + answer, answer, calculated);
        }
    }

    @Test
    public void testConnectionsWithinSubgroup() {
        Map<Integer, Integer> cons = kp.connectionsWithinSubgroup(graphWithThreeCliques(), intSet(1, 2, 3, 4));
        Integer[] answers = new Integer[] { 2, 3, 3, 2, 1, 0, 0, 1, 0 };
        for (Integer i = 0; i < answers.length; i++) {
            Integer node = i + 1;
            Integer answer = answers[i];
            Integer calculated = cons.get(node);
            assertEquals(node + " is connected to " + answer + " members of {1,2,3,4}", answer, calculated);
        }
    }

    @Test
    public void testCriticalNodes() {
        assertEquals("All nodes are critical in a 1-plex.", intSet(7), kp.criticalNodesInKPlex(graphWithThreeCliques(), intSet(7), 1));
    }

    @Test
    public void testSubgraphBoundingFinalKPlex() {
        Set<Integer> finalKPlex = intSet(5, 6, 7);
        Map<Integer, Set<Integer>> subgraph = kp.subgraphBoundingFinalKPlex(graphWithThreeCliques(), intSet(7), 1, 3);
        Set<Integer> boundsKPlex = subgraph.keySet();
        assertTrue(boundsKPlex + " should bound " + finalKPlex + " (full graph is " + subgraph + ")", boundsKPlex.containsAll(finalKPlex));
    }

    @Test
    public void testGrowClique() {
        Set<Integer> clique = kp.growKPlex(graphWithThreeCliques(), intSet(7), 1, 3);
        assertEquals("Expect {7} to grow into 1-plex of {5,6,7}.", intSet(5, 6, 7), clique);
    }

    @Test
    public void testCliqueSearch() {
        Set<Integer> clique = kp.findLargeKPlex(graphWithThreeCliques(), 1);
        assertEquals("Find a clique of size three, because all three cliques have size three.", 3, clique.size());
    }

    @Test
    public void testKPlexSearch() {
        assertEquals("Find a 2-plex of size 4.", intSet(1, 2, 3, 4), kp.findLargeKPlex(graphWithThreeCliques(), 2));
    }
