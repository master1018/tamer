...
public class Production {
    ...
    Production(Grammar grammar, String name) {
        this.grammar = grammar;
        this.name = name;
    }

    public Alternative addAlternative(String shortName) {
        if (this.isStable) {
    ...
    void stabilize() {
        ...
        for (Alternative alternative : this.alternatives) {
            ...
            List<Alternative> alternativeList = nameToAlternativeListMap.get(shortName);
        ...
        for (List<Alternative> alternativeList : nameToAlternativeListMap.values()) {
            if (alternativeList.size() == 1) {
                Alternative alternative = alternativeList.get(0);
                if (alternative.getShortName().equals("") && this.alternatives.size() > 1) {
                ...
                for (Alternative alternative : alternativeList) {
                    alternative.setName(alternative.getShortName() + "$" + index++);
                }
            }
        }
        for (Alternative alternative : this.alternatives) {
    ...
    boolean computeShortestLength() {
        ...
        if (minLength != null) {
            if (this.shortestLength == null || minLength.compareTo(this.shortestLength) < 0) {
    ...
    public Set<Ahead> look(int distance) {
    ...
    Set<Ahead> tryLook(int distance) {
        Set<Ahead> currentLookComputationData = this.distanceToAheadSetMap.get(distance);
        if (currentLookComputationData != null) {
            return currentLookComputationData;
        }
        currentLookComputationData = this.grammar.getCurrentLookComputationData(this, distance);
        if (currentLookComputationData == null) {
            this.grammar.setCurrentLookComputationData(this, distance, this.grammar.getPreviousLookComputationData(this, distance));
            currentLookComputationData = new LinkedHashSet<Ahead>();
            for (Alternative alternative : this.alternatives) {
                currentLookComputationData.addAll(alternative.tryLook(distance));
            }
            this.grammar.setCurrentLookComputationData(this, distance, currentLookComputationData);
        }
        return currentLookComputationData;
    }

    void setLook(Integer distance, Set<Ahead> aheadSet) {
