    ...
    ParagraphBuffer(MCharBuffer charBuffer) {
        ...
        for (int pos = 0; pos < textLength; pos++) {
            if (isParagraphBreak(charBuffer.at(pos))) {
                if (fRunArray.fPosEnd + 1 >= fRunArray.fNegStart) expandStyleTable();
...
*/
    private void shiftTableTo(int pos) {
        ...
        if (oldPosEnd > fRunArray.fPosEnd) System.arraycopy(fStyleTable, fRunArray.fPosEnd + 1, fStyleTable, fRunArray.fNegStart, oldPosEnd - fRunArray.fPosEnd); else if (oldNegStart < fRunArray.fNegStart) System.arraycopy(fStyleTable, oldNegStart, fStyleTable, oldPosEnd + 1, fRunArray.fNegStart - oldNegStart);
    }
...
*/
    public void insertText(int start, char insertedChar) {
        ...
        if (isParagraphBreak(insertedChar)) {
            ...
            fStyleTable[fRunArray.fPosEnd] = (fRunArray.fPosEnd == 0) ? fFirstStyle : fStyleTable[fRunArray.fPosEnd - 1];
...
*/
    public void insertText(int start, char srcChars[], int srcStart, int srcLimit) {
        ...
        for (int i = srcStart; i < srcLimit; i++) if (isParagraphBreak(srcChars[i])) {
            if (fRunArray.fPosEnd + 1 >= fRunArray.fNegStart) expandStyleTable();
...
*/
    public int paragraphLimit(int pos) {
        int run = fRunArray.findRunContaining(pos - 1);
        if (run == fRunArray.fPosEnd) run = fRunArray.fNegStart; else run++;
...
*/
    public AttributeMap paragraphStyleAt(int offset) {
        ...
        if (run < 0) return fFirstStyle; else return fStyleTable[run];
    }
...
*/
    private void setIterator(int pos, ParagraphIterator iter) {
        ...
        int run;
        if (pos > 0) run = fRunArray.findRunContaining(pos - 1); else run = -1;
...
*/
    private void setIteratorUsingRun(int run, ParagraphIterator iter) {
        ...
        if (run == fRunArray.fPosEnd + 1) run = fRunArray.fNegStart; else if (run == fRunArray.fNegStart - 1) run = fRunArray.fPosEnd;
        int runStart;
        AttributeMap style;
        if (run < 0) {
            runStart = 0;
            ...
            if (runStart < 0) runStart += fRunArray.getCurTextLength();
            runStart++;
        }
        int nextRun;
        if (run == fRunArray.fPosEnd) nextRun = fRunArray.fNegStart; else nextRun = run + 1;
        int runLimit;
        if (nextRun >= fRunArray.getArrayLength()) runLimit = fRunArray.getCurTextLength(); else {
            runLimit = fRunArray.fRunStart[nextRun];
            if (runLimit < 0) runLimit += fRunArray.getCurTextLength();
            runLimit++;
        }
...
*/
    public void replace(int start, int limit, MConstText srcText, int srcStart, int srcLimit, int[] damagedRange) {
...
* <tt>style</tt>.
*/
    public boolean modifyParagraphStyles(int start, int limit, StyleModifier modifier, int[] damagedRange) {
    ...
    private static void dumpParagraphStarts(ParagraphBuffer st) {
        System.out.println("fRunArray.fPosEnd=" + st.fRunArray.fPosEnd + ", fRunArray.fNegStart=" + st.fRunArray.fNegStart + ", fRunArray.getArrayLength()=" + st.fRunArray.getArrayLength() + ", fRunArray.getCurTextLength()=" + st.fRunArray.getCurTextLength());
        int i;
        System.out.print("Positives: ");
        for (i = 0; i <= st.fRunArray.fPosEnd; i++) System.out.print(st.fRunArray.fRunStart[i] + " ");
        System.out.print("   Negatives: ");
        for (i = st.fRunArray.fNegStart; i < st.fRunArray.getArrayLength(); i++) System.out.print(st.fRunArray.fRunStart[i] + " ");
        System.out.println(" ");
    }

    private static final int kNoRun = -42;

    private final class ParagraphIterator {
