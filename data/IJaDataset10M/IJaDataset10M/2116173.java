...
public class ChatDisplay extends ScrolledComposite {
    ...
    protected void createOptionComposite() {
        if (this.optionsComposite != null) this.optionsComposite.dispose();
    ...
    public void addChatLine(Object sender, Color color, String message, Date receivedOn) {
        if (lastUser != null && lastUser.equals(sender)) {
        } else {
            ChatLinePartnerChangeSeparator chatPartnerChangeLine = new ChatLinePartnerChangeSeparator(contentComposite, sender.toString(), color, receivedOn);
    ...
    public void refresh() {
        ...
        int verticalBarWidth = (this.getVerticalBar() != null) ? this.getVerticalBar().getSize().x : 0;
        int widthHint = Math.max(computeMaxNonChatLineWidth() + verticalBarWidth, ChatDisplay.this.getClientArea().width);
        final Point neededSize = ChatDisplay.this.contentComposite.computeSize(widthHint, SWT.DEFAULT);
