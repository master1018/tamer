...
public class VisibilityGraph implements Graph2D, WeightedGraph {
   ...
   */
    public VisibilityGraph(Graph2D graph2d) {
        ...
        for (Vertex v : graph2d.getVertices()) holes.add(new Hole(graph2d.getShape(v)));
        ...
        for (Hole dest : destinations) {
            for (int i = 0; i < source.points.size(); i++) {
                ...
                for (int j = 0; j < dest.points.size(); j++) {
                    ...
                    if (!obstructed(sourcePoint, destPoint, holes)) vertex(sourcePoint).sees(vertex(destPoint), connections);
                }
            }
        }
    }
    ...
    private boolean obstructed(Point2D from, Point2D to, List<Hole> holes) {
        for (int i = 0; i < holes.size(); i++) {
            if (holes.get(i).obstructs(from, to)) return true;
        }
        return false;
    }
   ...
   */
    ...
    public Vertex getVertex(Point2D point) throws IllegalArgumentException {
        ...
        if (result == null) throw new IllegalArgumentException("Point " + point + " is not a valid point location");
        return result;
    }
    ...
    private class Hole {
        ...
        Hole(Shape shape) {
            ShapeHelper.iterateShape(Geometry.getConvexHull(shape), new FlattenedPathConsumer() {
                ...
                public boolean consumeLine(Point2D start, Point2D end) {
                    Point s = round(start);
                    if (points.isEmpty() || !points.get(points.size() - 1).equals(s)) points.add(s);
                    Point e = round(end);
                    if (!s.equals(e)) points.add(e);
                    return true;
                }
            });
        ...
        boolean obstructs(Point2D lineStart, Point2D lineEnd) {
            ...
            for (int i = 0; i < points.size(); i++) {
                ...
                if (p != null) {
                    if (!(p.equals(lineStart) || p.equals(lineEnd))) return true;
                    if (++n > 2) return true;
                }
            }
            return false;
    ...
    private static class PointLocation extends Point implements Vertex {
        ...
        void sees(PointLocation that, List<VisibleConnection> connections) {
            for (int i = 0; i < es.size(); i++) {
                ...
                if (e.end.equals(that) || e.start.equals(that)) return;
            }
