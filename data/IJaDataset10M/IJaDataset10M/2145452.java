 ...
 */
public class DependencyShotActivator implements BundleActivator, BundleListener, ServiceListener {
    ...
    public void serviceChanged(ServiceEvent se) {
        ...
        switch(se.getType()) {
            case ServiceEvent.MODIFIED:
                serviceEvent = new ServiceChanged(ref, context);
                break;
            case ServiceEvent.REGISTERED:
                serviceEvent = new ServiceArrival(ref, context);
                break;
            case ServiceEvent.UNREGISTERING:
                serviceEvent = new ServiceDeparture(ref, context);
                break;
        }
    ...
    private void startContainer(Bundle bundle) {
        ...
        if (dict.get(DEPENDENCY_SHOT_ENABLED) != null) {
            ...
            if (binder != null) {
                injector = DependencyShot.configurator(OSGiConfigurator.class).context(bundle.getBundleContext()).withBinder(binder).getInjector();
            } else {
                injector = DependencyShot.configurator(OSGiConfigurator.class).context(bundle.getBundleContext()).getInjector();
            }
