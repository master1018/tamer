...
import org.twdata.kokua.tw.gui.terminal.TerminalHoverListener;
 ...
 */
public class Terminal extends JPanel implements StreamFilter, ClipboardOwner {
    ...
    public void channel(ClipboardSignal signal) {
        ...
        if ("copy".equals(command)) {
            ...
            try {
                byte buffer[] = ((String) t.getTransferData(DataFlavor.stringFlavor)).getBytes("Cp1252");
                try {
                    write(buffer);
                } catch (IOException e) {
                }
    ...
    protected void configure(InputStream in) {
        ...
        if (cFG != null || cBG != null) {
            try {
                Color fg = (cFG == null ? terminal.getBackground() : (Color.getColor(cFG) != null ? Color.getColor(cFG) : Color.decode(cFG)));
                Color bg = (cBG == null ? terminal.getForeground() : (Color.getColor(cBG) != null ? Color.getColor(cBG) : Color.decode(cBG)));
