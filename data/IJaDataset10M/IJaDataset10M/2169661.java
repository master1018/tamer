...
public class StackLayout implements LayoutManager {
    ...
    public void addLayoutComponent(String tag, Component comp) {
        ...
        if ((harg == -1) || (varg == -1)) System.out.println("StackLayout: can't understand \"" + tag + "\""); else {
        ...
        if (orientation == HORIZONTAL) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (comp.isVisible()) {
                    ...
                    Dimension d = (preferred && ((code & SIZEMASK) == FILL) ? comp.preferredSize() : comp.minimumSize());
    ...
    public void layoutContainer(Container parent) {
        ...
        int L = (orientation == HORIZONTAL ? W : H), D = (orientation == HORIZONTAL ? H : W);
            ...
            if (comp.isVisible()) {
                ...
                switch(c & POSMASK) {
                    case CENTER:
                        ...
                        break;
                    case BACK:
                        if (prev != BACK) nRubber++;
                        break;
                }
                if ((c & FLUSH) == 0) sum += 2 * margin;
                prev = (c & POSMASK);
            }
        }
        if (prev == CENTER) nRubber++;
        int rubber = ((nFills != 0) || (nRubber == 0) ? 0 : Math.max(0, (L - sum) / nRubber)), fill = (nFills == 0 ? 0 : Math.max(0, (L - sum) / nFills));
        int r = (orientation == HORIZONTAL ? in.left : in.top), s0 = (orientation == HORIZONTAL ? in.top : in.left), s, l, d, m;
        prev = FRONT;
        for (int i = 0; i < n; i++) {
            int code[] = codes[i];
            int size[] = sizes[i];
            if (code != null) {
                ...
                Component comp = parent.getComponent(i);
                if (orientation == HORIZONTAL) comp.reshape(r, s, l, d); else comp.reshape(s, r, d, l);
                r += l + m;
            }
        }
    }
