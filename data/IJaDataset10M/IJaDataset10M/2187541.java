...
public class TMHMMFilteredMatchDAOImpl extends GenericDAOImpl<TMHMMMatch, Long> implements TMHMMFilteredMatchDAO {
    ...
    private Map<String, Signature> loadPersistedSignatures() {
        ...
        for (final TMHMMSignature tmhmmSignature : TMHMMSignature.values()) {
            ...
            for (final Signature retrievedSignature : retrievedSignatures) {
                if (tmhmmSignature.getAccession().equals(retrievedSignature.getAccession()) && tmhmmSignature.getAccession().equals(retrievedSignature.getAccession())) {
            ...
            if (!found) {
                ...
                final Signature signature = builder.description(tmhmmSignature.getShortDesc()).signatureLibraryRelease(release).build();
    ...
    private Map<String, Protein> getProteinIdToProteinMap(Set<TMHMMProtein> tmhmmProteins) {
        ...
        for (int index = 0; index < proteinIdCount; index += MAXIMUM_IN_CLAUSE_SIZE) {
            ...
            final Query proteinQuery = entityManager.createQuery("select p from Protein p where p.id in (:proteinId)");
