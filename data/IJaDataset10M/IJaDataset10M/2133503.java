...
public class ConstructionStage implements Serializable {
    ...
    public void addWorkTime(double workTime) {
        ...
        if (completedWorkTime > info.getWorkTime()) completedWorkTime = info.getWorkTime();
        if (isSalvaging) site.fireConstructionUpdate(ADD_SALVAGE_WORK_EVENT, this); else site.fireConstructionUpdate(ADD_CONSTRUCTION_WORK_EVENT, this);
    }
    ...
    public String toString() {
        String result = "";
        if (isSalvaging) result = "salvaging " + info.getName(); else result = "constructing " + info.getName();
        return result;
    }
}
