...
public class ChessPage extends WebPage {
    ...
    private static final String[] STATUS_NAME = { "ready", "win", "draw", "loss", "thinking" };

    private static final String[] PIECE_NAME = { null, null, null, null, null, null, null, null, "wk", "wq", "wr", "wb", "wn", "wp", null, null, "bk", "bq", "br", "bb", "bn", "bp", null, null };

    private static final String[] SOUND_NAME = { "click", "illegal", "move", "move2", "capture", "capture2", "special", "special2", "check", "check2", "win", "draw", "loss" };
    ...
    static final String[] LEVEL_STRING = { "Beginner", "Amateur", "Expert", "Master", "Grand Master" };
    ...
    private class ThinkingBehavior extends AbstractAjaxTimerBehavior {
        ...
        protected void onTimer(AjaxRequestTarget target) {
            ...
            int response = pos.inCheck() ? RESP_CHECK2 : pos.specialMove() ? RESP_SPECIAL2 : pos.captured() ? RESP_CAPTURE2 : RESP_MOVE2;
    ...
    private class AjaxBoard {
        ...
        void drawSquare(int sq, boolean bSelected) {
            imgSquares[sq].setImageResourceReference((bSelected ? rrSelected : rrPieces)[pos.squares[sq]]);
    ...
    private int getCookieValue(String name, int min, int max, int defaultValue) {
        ...
        int value = (cookie == null ? defaultValue : Integers.parse(cookie.getValue(), defaultValue));
    ...
    public ChessPage() {
            ...
            imgSquares[sq].add(new AjaxEventBehavior("onClick") {
                ...
                protected void onEvent(AjaxRequestTarget target) {
                    ...
                    if ((pc & Position.SIDE_TAG(pos.sdPlayer)) != 0) {
                        ...
                        if (pos.specialMove()) {
                            if (Position.PIECE_TYPE(pc) == Position.PIECE_KING) {
                                int castling = Position.CASTLING_TYPE(1 - pos.sdPlayer, sqSelected, sq);
                                board.drawSquare(Position.CASTLING_ROOK_SRC[castling]);
                                board.drawSquare(Position.CASTLING_ROOK_DST[castling]);
                            } else {
                        ...
                        int response = pos.inCheck() ? RESP_CHECK : pos.specialMove() ? RESP_SPECIAL : pos.captured() ? RESP_CAPTURE : RESP_MOVE;
                        if (pos.captured() || Position.PIECE_TYPE(pc) == Position.PIECE_PAWN) {
                            pos.setIrrev();
                        }
        ...
        final RadioChoice<String> selFlipped = new RadioChoice<String>("selFlipped", new Model<String>(flipped ? Choices.FLIPPED_TRUE : Choices.FLIPPED_FALSE), Choices.getFlippedTypes()).setSuffix("");
        final DropDownChoice<String> selHandicap = new DropDownChoice<String>("selHandicap", new Model<String>(Choices.getHandicapTypes().get(handicap)), Choices.getHandicapTypes());
        Form<Void> frm = new Form<Void>("frm") {
            ...
            public void onSubmit() {
                if (status != STATUS_THINKING) {
                    addCookieValue("fen", null);
                    addCookieValue("flipped", Boolean.toString(selFlipped.getModelObject().equals(Choices.FLIPPED_TRUE)));
