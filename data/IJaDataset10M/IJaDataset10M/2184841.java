...
public class GraphOptionsView extends JPanel {
    ...
    private final JLabel gridLabel = ModelerSession.getComponentFactoryService().createLabel(Resources.getResources().getString("hierarchy.frame.options.grid"));

    private final JLabel cellSizeLabel = ModelerSession.getComponentFactoryService().createLabel(Resources.getResources().getString("hierarchy.frame.options.cel.size"));

    private final JLabel viewGridLabel = ModelerSession.getComponentFactoryService().createLabel(Resources.getResources().getString("hierarchy.frame.options.grid.view"));

    private final JLabel scaleLabel = ModelerSession.getComponentFactoryService().createLabel(Resources.getResources().getString("hierarchy.frame.options.scale"));

    protected final JButton initialSizeButton = ModelerSession.getComponentFactoryService().createButton(Resources.getResources().getString("hierarchy.frame.options.scale.100"), null);
    ...
    private void initLayout() {
        ...
        final FormLayout layout = new FormLayout("pref, 3dlu, pref, pref:grow", " pref, 3dlu, " + "pref, 3dlu," + "pref, 25dlu," + "pref, 3dlu," + "pref, 3dlu," + "pref");
