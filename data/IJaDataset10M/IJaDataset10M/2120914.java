...
public final class EditorModel {
   ...
   */
    public void selectNewBuffer() {
        final Buffer buffer = new BufferImplementation(m_resources, m_textSourceFactory.create(), createNewBufferName());
        addBuffer(buffer);
        selectBuffer(buffer);
    }
        ...
        if (removed) {
            ...
            if (buffer.equals(getSelectedBuffer())) {
                ...
                synchronized (m_bufferList) {
                    ...
                    bufferToSelect = numberOfBuffers > 0 ? (Buffer) m_bufferList.get(numberOfBuffers - 1) : null;
                }
    ...
    private void parseSelectedProperties(File file) {
        if (file != null && file.equals(getSelectedPropertiesFile())) {
            ...
            try {
                ...
                selectedFile = properties.resolveRelativeFile(properties.getFile(GrinderProperties.SCRIPT, GrinderProperties.DEFAULT_SCRIPT)).getCanonicalFile();
   ...
   */
    public void openWithExternalEditor(final File file) throws ConsoleException {
        ...
        if (externalEditor == null) {
            throw new DisplayMessageConsoleException(m_resources, "externalEditorNotSet.text");
        }
        try {
            externalEditor.open(file);
        } catch (IOException e) {
            throw new DisplayMessageConsoleException(m_resources, "externalEditError.text", e);
        }
    }
   ...
   */
    public void setExternalEditor(File command, String arguments) {
        ...
        if (command == null) {
            ...
            externalEditor = new ExternalEditor(m_agentCacheState, this, command, arguments);
        }
        synchronized (this) {
