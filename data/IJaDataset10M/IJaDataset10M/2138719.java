...
public class SwingDemo extends JFrame {
    ...
    public static void main(String args[]) {
        ...
        if (benchRuns == 0) LayoutUtil.setDesignTime(null, true);
        if (laf.endsWith("WindowsLookAndFeel")) buttonOpaque = false;
        if (laf.endsWith("AquaLookAndFeel")) contentAreaFilled = false;
        ...
        if (sourceTextArea != null) {
            ...
            southTabPane.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    if (e.getClickCount() == 2) showSourceInFrame();
                }
            });
        }
        for (int i = 0; i < panels.length; i++) ((DefaultListModel) pickerList.getModel()).addElement(panels[i][0]);
        try {
                                    ...
                                    try {
                                        SwingUtilities.invokeAndWait(new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                Toolkit.getDefaultToolkit().sync();
                                                if (timeToShowMillis == 0) timeToShowMillis = System.currentTimeMillis() - startupMillis;
                                            }
                                        });
                                    } catch (Exception e) {
                                        e.printStackTrace();
    ...
    private void setSource(String source) {
        if (benchRuns > 0 || sourceTextArea == null) return;
        if (source.length() > 0) {
    ...
    public JComponent createLayout_Showdown() {
        ...
        JPanel p2 = createTabPanel(new MigLayout("", "[]15[][grow,fill]15[][grow,fill]"));
        JScrollPane list2 = new JScrollPane(new JList(new String[] { "Mouse, Mickey" }));
    ...
    public JComponent createWelcome() {
        ...
        if (PlatformDefaults.getCurrentPlatform() == PlatformDefaults.WINDOWS_XP) textArea.setFont(new Font("tahoma", Font.BOLD, 11));
    ...
    public JComponent createDocking() {
        ...
        for (int r = 0; r < data.length; r++) {
            data[r] = new String[6];
            for (int c = 0; c < data[r].length; c++) data[r][c] = "Cell " + (r + 1) + ", " + (c + 1);
        }
    ...
    public JComponent createAbsolute_Position() {
        ...
        posPanel.add(createButton("Drag Window! (pos 500-container.xpos 500-container.ypos)"), "pos 500-container.xpos 500-container.ypos");
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    timer.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            if (alpha < 150) bg.setBackground(new Color(100, 100, 100, alpha));
                            if (alpha > 150 && alpha < 405) {
                                label.setForeground(new Color(255, 255, 255, (alpha - 150)));
                                bg.repaint();
                            }
                            if (alpha > 405) timer.stop();
                        }
                    });
                    timer.start();
                }
    ...
    public JComponent createComponent_Links() {
        ...
        linksPanel.add(createCheck("pos (ta.x+indent) (ta.y2+rel)"), "pos (ta.x+indent) (ta.y2+rel)");
    ...
    private JComponent createPlainImpl(boolean debug) {
        ...
        if (debug) panel.add(createLabel("Red is cell bounds. Blue is component bounds."), "newline,ax left,span,gaptop 40,");
    ...
    public JComponent createCell_Alignments() {
        ...
        for (int c = 0; c < vSizes.length; c++) {
            for (int r = 0; r < 5; r++) {
                ...
                if (r == 4 && c <= 1) b.setFont(new Font("sansserif", Font.PLAIN, 16 + c * 5));
    ...
    public JComponent createAlignments() {
        ...
        for (int c = 0; c < horLabels.length; c++) horTab.add(createLabel(horLabels[c]));
        for (int r = 0; r < horLabels.length; r++) {
            for (int c = 0; c < horNames.length; c++) horTab.add(c == 0 ? createLabel(horNames[r] + ":") : createButton(horNames[r]));
        }
        ...
        for (int c = 0; c < verLabels.length; c++) verTab.add(createLabel(verLabels[c]));
        for (int r = 0; r < verNames.length; r++) {
            for (int c = 0; c < verLabels.length; c++) verTab.add(createButton(verNames[r]));
        }
        for (int c = 0; c < verLabels.length; c++) verTab.add(createTextField("JTextFied"));
        for (int c = 0; c < verLabels.length; c++) verTab.add(createTextArea("JTextArea", 1, 8));
        for (int c = 0; c < verLabels.length; c++) verTab.add(createTextArea("JTextArea\nwith two lines", 1, 10));
        for (int c = 0; c < verLabels.length; c++) verTab.add(createTextAreaScroll("Scrolling JTextArea\nwith two lines", 1, 15, true));
    ...
    public JComboBox createCombo(String[] items) {
        ...
        if (PlatformDefaults.getCurrentPlatform() == PlatformDefaults.MAC_OSX) combo.setOpaque(false);
        return combo;
    }
    ...
    private JButton createButton(String text, boolean bold) {
        ...
        if (bold) b.setFont(b.getFont().deriveFont(Font.BOLD));
        configureActiveComponet(b);
    ...
    private JScrollPane createTextAreaScroll(String text, int rows, int cols, boolean hasVerScroll) {
        ...
        ta.setWrapStyleWord(true);
        ta.setLineWrap(true);
        JScrollPane scroll = new JScrollPane(ta, hasVerScroll ? ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED : ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
        return scroll;
    }
