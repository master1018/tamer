...
public class Calculator {
    ...
    private enum MathOp {
        ...
        LPAREN('('), RPAREN(')'), ADD('+'), SUBTRACT('-'), MULTIPLY('*'), DIVIDE('/'), REM('%'), POWER('^');
    ...
    private EnumSet<MathOp> TradOrLParen = EnumSet.of(MathOp.ADD, MathOp.SUBTRACT, MathOp.LPAREN, MathOp.MULTIPLY, MathOp.DIVIDE, MathOp.REM, MathOp.POWER);
    ...
    public long reduce(int startAtomIndex, boolean stopAtParenClose) {
        ...
        while (startAtomIndex + remaining > i + 1) {
            ...
            atoms.remove(i + 1);
            atoms.remove(i + 1);
            if (op == MathOp.MULTIPLY) atom.val *= nextAtom.val; else if (op == MathOp.DIVIDE) atom.val /= nextAtom.val; else atom.val %= nextAtom.val;
        }
    ...
    public static void main(String[] sa) {
        if (sa.length != 1) throw new IllegalArgumentException("SYNTAX: java Calculator 'expression'");
