...
public class ManageUserProfilePanel extends Composite {
    ...
    private static ManageUserProfilePanelUiBinder uiBinder = GWT.create(ManageUserProfilePanelUiBinder.class);

    private static IPiramideMessages messages = GWT.create(IPiramideMessages.class);

    interface ManageUserProfilePanelUiBinder extends UiBinder<Widget, ManageUserProfilePanel> {
    }
    ...
    private void showAddButton() {
        ...
        for (String capability : UserCapabilities.values()) if (!capabilities.containsKey(capability)) this.capabilityNameListBox.addItem(capability);
    }
    ...
    private void removeCapability(final String capability) {
        for (int rowNumber = 0; rowNumber < ManageUserProfilePanel.this.profileTable.getRowCount(); ++rowNumber) {
            ...
            if (capabilityName.equals(capability)) ManageUserProfilePanel.this.profileTable.removeRow(rowNumber);
        }
    }
}
