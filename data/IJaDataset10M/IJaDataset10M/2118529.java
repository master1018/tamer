...
public class ItemIdentifierPredicate implements BasicPredicateIF {
    ...
    public QueryMatches satisfy(QueryMatches matches, Object[] arguments) throws InvalidQueryException {
        ...
        if (matches.bound(objix) && !matches.bound(locix)) {
            ...
            return PredicateUtils.objectToMany(matches, objix, locix, TMObjectIF.class, PredicateUtils.OBJECT_TO_SRCLOC, null);
        } else if (!matches.bound(objix) && matches.bound(locix)) {
            return PredicateUtils.objectToOne(matches, locix, objix, String.class, PredicateUtils.SRCLOC_TO_OBJECT);
        } else if (matches.bound(objix) && matches.bound(locix)) {
            prefetchSources(matches, arguments, objix);
            return PredicateUtils.filter(matches, objix, locix, TMObjectIF.class, String.class, PredicateUtils.FILTER_SRCLOC);
        } else {
            prefetchSources(matches, arguments, objix);
            return PredicateUtils.generateFromCollection(matches, objix, locix, PredicateUtils.getAllObjects(topicmap), PredicateUtils.GENERATE_SRCLOC);
        }
    }
            ...
            if (types != null) {
                for (int i = 0; i < types.length; i++) {
                    if (TopicIF.class.equals(types[i])) {
                        Prefetcher.prefetch(topicmap, matches, objix, Prefetcher.TopicIF, Prefetcher.TopicIF_sources, false);
                    } else if (TopicNameIF.class.equals(types[i])) {
                        Prefetcher.prefetch(topicmap, matches, objix, Prefetcher.TopicNameIF, Prefetcher.TopicNameIF_sources, false);
                    } else if (AssociationIF.class.equals(types[i])) {
                        Prefetcher.prefetch(topicmap, matches, objix, Prefetcher.AssociationIF, Prefetcher.AssociationIF_sources, false);
                    } else if (OccurrenceIF.class.equals(types[i])) {
                        Prefetcher.prefetch(topicmap, matches, objix, Prefetcher.OccurrenceIF, Prefetcher.OccurrenceIF_sources, false);
                    } else if (AssociationRoleIF.class.equals(types[i])) {
                        Prefetcher.prefetch(topicmap, matches, objix, Prefetcher.AssociationRoleIF, Prefetcher.AssociationRoleIF_sources, false);
                    } else if (VariantNameIF.class.equals(types[i])) {
                        Prefetcher.prefetch(topicmap, matches, objix, Prefetcher.VariantNameIF, Prefetcher.VariantNameIF_sources, false);
                    }
                }
            }
        }
