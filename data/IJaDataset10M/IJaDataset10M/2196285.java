...
public class PolygonOrCircle implements Cloneable {
    ...
    public void RotateRadians(double radians) {
        if (isCircle()) return; else if (isPolygon()) ((Polygon2D) m_Shape).transform(AffineTransform.getRotateInstance(radians, m_centroid.getX(), m_centroid.getY()));
    }
    ...
    public Point2D Scale(double s) {
        if (s <= 0) return new Point2D.Double(this.getX(), this.getY());
        double temp = s;
        s = s / m_Scale;
        m_Scale = temp;
        if (s == 1) return new Point2D.Double(this.getX(), this.getY());
        if (isCircle()) {
            Circle2d c = (Circle2d) m_Shape;
