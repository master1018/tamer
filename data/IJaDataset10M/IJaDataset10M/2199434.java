...
public class BasicOCSPResp implements java.security.cert.X509Extension {
    ...
    public CertStore getCertificates(String type, String provider) throws NoSuchAlgorithmException, NoSuchProviderException, OCSPException {
        try {
            return CertStore.getInstance(type, new CollectionCertStoreParameters(this.getCertList(provider)), provider);
    ...
    public boolean verify(PublicKey key, String sigProvider) throws OCSPException, NoSuchProviderException {
        try {
            java.security.Signature signature = java.security.Signature.getInstance(this.getSignatureAlgName(), sigProvider);
