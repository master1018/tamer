...
public class ClassUseMapper {
    ...
    private void mapExecutable(ExecutableMemberDoc em) {
        ...
        for (int k = 0; k < params.length; k++) {
            ...
            if ((!params[k].type().isPrimitive()) && !classArgs.contains(pcd) && !(pcd instanceof TypeVariable)) {
                ...
                classArgs.add(pcd);
                mapTypeParameters(isConstructor ? classToConstructorDocArgTypeParam : classToExecMemberDocArgTypeParam, pcd, em);
            }
            mapAnnotations(isConstructor ? classToConstructorParamAnnotation : classToExecMemberDocParamAnnotation, params[k], em);
        }
    ...
    private void add(Map map, ClassDoc cd, ProgramElementDoc ref) {
        ...
        classSet(cd).add(ref instanceof MemberDoc ? ((MemberDoc) ref).containingClass() : ref);
    }
    ...
    private void mapAnnotations(Map map, Object doc, Object holder) {
        ...
        for (int i = 0; i < annotations.length; i++) {
            ...
            if (isPackage) refList(map, annotationDoc).add(holder); else add(map, annotationDoc, (ProgramElementDoc) holder);
        }
    }
