...
public class TestXmlBuilder extends AbstractTestCase {

    private void assertRootElement(Document dom, String nsUri, String name, int childCount) throws Exception {
        assertElement(dom, "/*[1]", nsUri, name, childCount);
    }

    private void assertElement(Document dom, String path, String nsUri, String name, int childCount) throws Exception {
    ...
    private static class MockContentHandler implements InvocationHandler {

        public ContentHandler getHandler() throws Exception {
            return (ContentHandler) Proxy.newProxyInstance(ContentHandler.class.getClassLoader(), new Class[] { ContentHandler.class, LexicalHandler.class }, this);
        }
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            for (int ii = 0; ii < args.length; ii++) {
                if (args[ii] instanceof char[]) args[ii] = new String((char[]) args[ii]);
            }
    ...
    public void testNestedElement() throws Exception {
        ...
        MockContentHandler handler = new MockContentHandler();
        node.toSAX(handler.getHandler());
        handler.assertInvocationSequence("startElement", "startElement", "endElement", "endElement");
        handler.assertInvocation(0, "startElement", null, "foo", "foo");
    ...
    public void testAttribute() throws Exception {
        ElementNode node = element("foo", attribute("argle", "bargle", "wargle"), attribute("bar", "baz"));
        Document dom = node.toDOM();
    ...
    public void testTextElement() throws Exception {
        ...
        MockContentHandler handler = new MockContentHandler();
        node.toSAX(handler.getHandler());
        handler.assertInvocationSequence("startElement", "characters", "endElement");
        handler.assertInvocation(0, "startElement", null, "foo", "foo");
    ...
    public void testConsecutiveTextElements() throws Exception {
        ...
        MockContentHandler handler = new MockContentHandler();
        node.toSAX(handler.getHandler());
        handler.assertInvocationSequence("startElement", "characters", "characters", "endElement");
        handler.assertInvocation(0, "startElement", null, "foo", "foo");
        handler.assertInvocation(1, "characters", "bar", 0, 3);
    ...
    public void testPI() throws Exception {
        ...
        MockContentHandler handler = new MockContentHandler();
        node.toSAX(handler.getHandler());
        handler.assertInvocationSequence("startElement", "processingInstruction", "endElement");
