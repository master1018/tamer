...
public class KMeansClustering extends ClusteringAlgorithm implements DotFileWriter {
    ...
    public void build() {
        ...
        for (int i = 0; i < traceSize; i++) traceList.add(Integer.valueOf(i));
        ...
        for (int k = 0; k < maxIterationBox.getValue(); k++) {
            ...
            clusterSet.clear();
            for (int i = 0; i < clusterSize; i++) clusterSet.addLogCluster(new LogCluster());
            ...
            tempPrint();
            if (k == 0) {
                for (int i = 0; i < clusterSize; i++) temp_old[i] = temp[i];
    ...
    private void initCenters() {
        ...
        for (int i = 0; i < clusterSize; i++) {
            ...
            for (int j = 0; j < profileSize; j++) centers.set(i, j, tempPoint.get(j));
        }
    }
    ...
    public void writeToDot(Writer bw) throws IOException {
        ...
        for (LogCluster cluster : clusterSet.getClusters()) {
            ...
            bw.write("cluster" + clusterCounter + " [shape=\"octagon\",style=\"filled\",fillcolor=\"chartreuse3\",label=\"Cluster " + clusterCounter + "\\n" + cluster.size() + "\"];\n");
            clusterCounter++;
        }
        ...
        for (int i = 0; i < clusterSet.size() - 1; i++) {
            ...
            for (int k = i + 1; k < clusterSet.size(); k++) {
                ...
                bw.write("cluster" + clusterCounter + " -- cluster" + clusterCounter2 + " [label=\"" + length.get(i, k) + "\"];\n");
                clusterCounter2++;
            }
            clusterCounter++;
        }
