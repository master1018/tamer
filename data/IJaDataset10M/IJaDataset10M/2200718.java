...
public class PianoRoll extends PianoRollPanelAdapter {
    ...
    public PianoRoll(final ProjectFrame frame, ItemScrollPane scroller) {
        ...
        notesOnScreen = new Iterable<MultiEvent>() {
            ...
            public Iterator<MultiEvent> iterator() {
                return new EventsInPartsIterator(project.getPartSelection().getSelected(), PianoRoll.this);
            }
        };
        notesInFocus = new Iterable<MultiEvent>() {
            ...
            public Iterator<MultiEvent> iterator() {
                ...
                return new EventsInPartsIterator(project.getPartSelection().getFocus(), PianoRoll.this);
            }
        };
        ...
        project.getMultiEventSelection().addSelectionListener(multiEventListener);
    ...
    public void dispose() {
        project.getMultiEventSelection().removeSelectionListener(multiEventListener);
    ...
    public void dragTo(Point p) {
        ...
        dtick = screenToTickRel(xAnchor, dxDragged, project.isPianoRollSnapQuantized());
        int dpitch = 0;
        if (dragMode == OVER_ITEM_MIDDLE) dpitch = -dyDragged / noteItemHeight;
        if (dtick == 0 && dpitch == 0 && !altIsDown) return;
        if (dragArmed) startDrag();
        dragArmed = false;
        List<Item> dragList = project.getDragList();
        if (dragList.isEmpty()) return;
        if (altIsDown && project.isPianoRollSnapQuantized()) {
            ...
            if (dragMode != OVER_ITEM_RIGHT) {
                ...
                long newTick = screenToTickAbs((int) (screenPos + dxDragged), true);
        ...
        if (dpitch != 0) {
            int pitlim;
            if (dpitch > 0) pitlim = 0; else pitlim = 127;
            for (Item it : dragList) {
                NoteEvent ev = (NoteEvent) it;
                int pitch = ((NoteEvent) ev).getNote() + dpitch;
                if (dpitch > 0) pitlim = Math.max(pitch, pitlim); else pitlim = Math.min(pitch, pitlim);
            }
            if (dpitch > 0 && pitlim > 127) dpitch = dpitch - (pitlim - 127); else if (dpitch < 0 && pitlim < 0) dpitch = dpitch - pitlim;
        }
        long dt = 0;
        for (Item it : dragList) {
            NoteEvent ev = (NoteEvent) it;
    ...
    public synchronized void selectInRect(Rectangle rect, boolean shift) {
        ...
        if (focusPart != null) list = ((MidiPart) focusPart).getMultiEvents(); else list = notesOnScreen;
        for (MultiEvent note : list) {
            if (note instanceof NoteEvent) {
                if (rect.intersects(getItemBounds((NoteEvent) note))) {
                    if (shift) {
                        if (note.isSelected()) delTmp.add(note); else addTmp.add(note);
                    } else {
                        addTmp.add(note);
                    }
                }
    ...
    public synchronized void writeDraggedAt(Point p) {
        ...
        if (drumWriteMode) {
            if (tick != tick1) {
                if (writeAtPoint != null) p.y = writeAtPoint.y;
                writeReleasedAt(p);
                writePressedAt(p);
                doit = true;
            }
    ...
    protected synchronized void writePressedAt(Point p) {
        ...
        if (drumWriteMode) {
            newNote = new NoteEvent((MidiPart) focusPart, tick, pitch, velocity, channel, 0);
        } else {
            long quant = (long) project.getPianoRollSnapQuantization();
            if (quant <= 0) quant = project.getTicksPerBeat();
            newNote = new NoteEvent((MidiPart) focusPart, tick, pitch, velocity, channel, quant);
        }
        feedBack(newNote);
        project.getDragList().notifyFeedbackItemListeners(newNote);
        repaintItems();
    ...
    public Item itemAt(Point p) {
        ...
        for (MultiEvent note : notesOnScreen) {
            if (getItemBounds(note).contains(p)) {
                if (note.isSelected()) return note; else if (at == null) at = note;
            }
        }
        return at;
    }
    ...
    public synchronized void paintImageImpl(final Rectangle visibleRect, Graphics2D g) {
        if (g == null) return;
    ...
    private Rectangle getItemBounds(Item it) {
        ...
        if (!e.isDrumHit()) {
            ...
            rectTmp.setBounds((int) x1 - dd, (int) y1 + 1, 2 * dd, noteItemHeight - 2);
        }
        return rectTmp;
    }
    ...
    public int getHoverStateAt(Point p) {
        ...
        int tol = endTol;
        Iterable<? extends MultiEvent> list;
        if (focusPart == null) list = notesOnScreen; else list = ((MidiPart) focusPart).getMultiEvents();
        for (MultiEvent e : list) {
            if (e instanceof NoteEvent) {
                ...
                if (rect.contains(p)) {
                    ...
                    if (rect.width < endTol * 5) tol = rect.width / 3;
                    if ((p.x - rect.x) <= tol) return OVER_ITEM_LEFT;
                    if ((rect.x + rect.width - p.x) <= tol) return OVER_ITEM_RIGHT;
