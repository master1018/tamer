...
public class LocalMailbox extends Mailbox {
    ...
    public LocalMailbox(MailboxURL url) {
        super(url);
        if (url instanceof LocalMailboxURL) mailboxFile = ((LocalMailboxURL) url).getFile();
        init();
    }
    ...
    private void init() {
        ...
        readOnly = true;
        if (mailboxFile != null) title = mailboxFile.canonicalPath();
        setInitialized(true);
    }
    ...
    public Message getMessage(MailboxEntry entry, ProgressNotifier progressNotifier) {
        try {
            ...
            if (contentType != null) charset = Utilities.getCharsetFromContentType(contentType);
        ...
        if (lock()) {
            try {
                ...
                if (toBeDeleted == null) {
                    ...
                    if (!(line instanceof MailboxLine)) return;
                ...
                if (advanceDot) advanceDot(editor.getDotLine());
            } finally {
                unlock();
            }
        } else editor.status("Mailbox is locked");
    }
        ...
        if (lock()) {
            try {
                ...
                if (toBeUndeleted == null) {
                    ...
                    if (getBooleanProperty(Property.UNDELETE_ADVANCE_DOT)) advanceDot = true;
                }
    ...
    private void readMessage(Line line, boolean useOtherWindow) {
        ...
        if (buf == null) buf = new LocalMessageBuffer(this, entry);
    ...
    protected boolean rewriteMailbox(boolean purge) {
        ...
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(mailboxFile.getInputStream(), "ISO8859_1"));
            ...
            while (true) {
                ...
                if (text == null) break;
                if (text.startsWith("From ")) {
                    if (purge) skip = ((MailboxEntry) entries.get(i)).isDeleted(); else skip = false;
                    if (skip) newOffsets[i] = -1; else {
                        newOffsets[i] = offset;
                        writer.write(text);
                        writer.write('\n');
            ...
            if (Utilities.deleteRename(tempFile, mailboxFile)) {
                for (i = 0; i < entries.size(); i++) {
                    ...
                    if (newOffset == -1) {
                        if (!entry.isDeleted()) Debug.bug("expected entry " + i + " to be deleted");
                    }
                ...
                if (purge) {
                    ...
                    for (i = 0; i < entries.size(); i++) {
                        ...
                        if (!entry.isDeleted()) v.add(entry);
                    }
                    v.trimToSize();
                    entries = v;
                }
                ...
                for (i = 0; i < entries.size(); i++) {
                    ...
                    if (lastEntry != null) lastEntry.setNextMessageStart(thisEntry.getMessageStart());
                    lastEntry = thisEntry;
                }
                if (thisEntry != null) thisEntry.setNextMessageStart(offset);
                dirty = false;
                succeeded = true;
            }
        } catch (IOException e) {
    ...
    private boolean isOwned() {
        if (localPrefix == null) localPrefix = Directories.getMailDirectory().canonicalPath().concat(LocalFile.getSeparator());
    ...
    public String toString() {
        final String name;
        if (isOwned()) name = mailboxFile.getParentFile().getName(); else if (mailboxFile.isLocal()) name = mailboxFile.canonicalPath(); else name = mailboxFile.netPath();
