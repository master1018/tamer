...
public class MetricsManager {
    ...
    public Run newRun() throws SQLException {
        ...
        if (currentRun.id >= 0) currentRun.save(database); else currentRun.insert(database);
    ...
    public Metric newMetric(String name, String units) throws SQLException {
        ...
        if (!currentMetric.exists(database)) {
            if (currentMetric.id >= 0) currentMetric.save(database); else currentMetric.insert(database);
        }
    ...
    public void saveValue(Value val) throws SQLException {
        if (val != null) {
            if (val.id >= 0) val.save(database); else val.insert(database);
        }
    }

    private void validate(String project, String buildNumber, File props) {
        if (!validProjectName(project)) throw new IllegalStateException("Invalid project " + project);
        if (!validBuildNumber(buildNumber)) throw new IllegalStateException("Invalid buildNumber " + buildNumber);
        if (!validProps(props)) throw new IllegalStateException("Invalid database properties " + props);
    }
