...
public class AttenuatedEdgeView extends EdgeView implements NodeGeometryObserver {
	 ...
	 * 
	 */
    private static final long serialVersionUID = -1932027860887608164L;
    ...
    public void init() {
        ...
        for (int i = 0; i < tubePoints.length; i++) {
            if (i % 2 == 0) {
                taperedTubePoints[i] = new Point3f(tubePoints[i].x * endRadius, tubePoints[i].y, tubePoints[i].z * endRadius);
                tubeColours[i] = endColour;
            } else {
                taperedTubePoints[i] = new Point3f(-tubePoints[i].x * startRadius, tubePoints[i].y, -tubePoints[i].z * startRadius);
        ...
        tubeGeometryArray = new TriangleStripArray(taperedTubePoints.length, GeometryArray.COORDINATES | GeometryArray.COLOR_3 | GeometryArray.BY_REFERENCE | GeometryArray.NORMALS, tubeStripCounts);
