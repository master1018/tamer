...
/**
 * This class represents the world being simulated. It is the main class in
 * what concerns the simulation model. It contains all the agents, places and
 * context overlays. This is the class you'll be coming to when writing your
 * World, Agent and Context models.
 * 
...
public class World {
	 ...
	 * GUI.
	 * 
	 * @param prefill true to fill the cache with place gradients
	 */
	 ...
	 * @param cacheSize the said amount
	 */
	 ...
	 * 
	 * @param simulation the simulation object which is running this world.
	 * @param simData the simulation data (maps, sprites, classes) for this
	 *            simulation.
	 */
	 ...
	 * 
	 * @param is the InputStream to look for the points
	 ...
	 * @throws AgentNotFoundException if the person does not exist
	 */
    public Agent getPersonByName(final String name) throws AgentNotFoundException {
    ...
    private void createTime() {
        ...
        time.clear();
        time.set(simulationConfig.getInt("starttime.year"), simulationConfig.getInt("starttime.month"), simulationConfig.getInt("starttime.day"), simulationConfig.getInt("starttime.hour"), simulationConfig.getInt("starttime.minute"));
    }
	 ...
	 * 
	 * @param pos the position in which to search
	 * @param visibleOnly set to true to filter out invisible Trackables near
	 *            the position
	 * @return the first Trackable found
	 * @throws NothingNearException if nothing is found nearby
	 */
	 ...
	 * 
	 * @param pos the position in which to search
	 * @param visibleOnly set to true to find only visible agents
	 * @return the first nearby Agent found
	 * @throws NothingNearException if nothing is found nearby
	 */
    public Trackable findAgentNear(final Position pos, final boolean visibleOnly) throws NothingNearException {
	 ...
	 * 
	 * @param pos the position in which to search
	 * @param visibleOnly set to true to find only visible places
	 ...
	 * 
	 * @param pos the position in which to search
	 * @param candidates the nearby trackables that we are willing to accept
	 * @param distance the distance in grid positions to consider a Trackable
	 *            as being near
	 * @param visibleOnly set to true to find only visible trackables
	 * @return the first nearby trackable out of the candidates
	 * @throws NothingNearException if nothing
	 */
    public Trackable findNearOutOf(final Position pos, final Collection candidates, final int distance, final boolean visibleOnly) throws NothingNearException {
        Trackable target = null;
        Trackable candidate = null;
        Iterator candidateIt = candidates.iterator();
	 ...
	 * 
	 * @param pos the position in which to search
	 * @param distance the maximum distance at which we consider the agent as
	 *            being nearby
	 * @param visibleOnly set to true if only visible trackables should be
	 *            returned
	 * @return a collection with all the agents near pos
	 * @throws NothingNearException if there's no agents nearby
	 */
    public ArrayList<Trackable> findAllAgentsNear(final Position pos, final int distance, final boolean visibleOnly) throws NothingNearException {
	 ...
	 * 
	 * @param pos the position in which to search
	 * @param distance the maximum distance at which we consider the place as
	 *            being nearby
	 * @param visibleOnly set to true if only visible trackables should be
	 *            returned
	 ...
	 * @throws NothingNearException if there's no agents nearby
	 */
    public ArrayList<Trackable> findAllPlacesNear(final Position pos, final int distance, final boolean visibleOnly) throws NothingNearException {
	 ...
	 * @throws NothingNearException if nothing
	 */
    public ArrayList<Trackable> findAllNearOutOf(final Position pos, final Collection<? extends Trackable> candidates, final int distance, final boolean visibleOnly) throws NothingNearException {
	 ...
	 * @throws PlaceNotFoundException if the place is not found
	 */
	 ...
	 * 
	 * @param pos the position in which to find a place
	 * @return the place at that position
	 * @throws PlaceNotFoundException if there is no place at that position
	 */
	 ...
	 * @throws PlaceTypeUndefinedException if no place is found
	 */
	 ...
	 * 
	 * @param type the type of place we need
	 * @return a random place of type "type"
	 * @throws PlaceNotFoundException if there are no places of that type
	 */
    public Place getRandomPlaceOfType(final String type) throws PlaceNotFoundException {
        ...
        if (typedPlaces.isEmpty()) {
            throw new PlacesTypeIsEmptyException();
        }
	 ...
	 * 
	 * @param pause true if the simulation should be paused
	 */
	 ...
	 * 
	 * @param stop true if you need a break and want to stop the world, false
	 *            otherwise.
	 */
    ...
    /**
	 * Use the calibration provided in the configuration file to figure out
	 * the coordinates of each map position.
	 * 
	 */
    private void initializeCoordinates() {
        double[] topRight = new double[] { simulationConfig.getDouble("calibration.topright[@latitude]"), simulationConfig.getDouble("calibration.topright[@longitude]") };
        double[] bottomLeft = new double[] { simulationConfig.getDouble("calibration." + "bottomleft[@latitude]"), simulationConfig.getDouble("calibration." + "bottomleft[@longitude]") };
        double[] bottomRight = new double[] { simulationConfig.getDouble("calibration." + "bottomright[@latitude]"), simulationConfig.getDouble("calibration." + "bottomright[@longitude]") };
    ...
    private void createPeople() {
        ...
        try {
            agentModel = (BaseAgentModel) simData.getAgentModelClass().getConstructor(new Class[] { this.getClass() }).newInstance(new Object[] { this });
        } catch (Exception e) {
    ...
    private void freezeInfoFields() {
        ...
        Set<String> standardFields = Agent.getInfoKeys();
        while (peopleIt.hasNext()) {
            Agent p = peopleIt.next();
            Set fields = Agent.getInfoKeys();
            if (!fields.equals(standardFields)) {
                standardFields.removeAll(fields);
                throw new RuntimeException("Agent " + p.getName() + " is missing field " + standardFields);
            }
        }
    ...
    /**
	 * Generate a matrix with the world's walls, out of the image file
	 * provided in the simulation data.
	 * 
	 */
    ...
    /**
	 * Create the world's places by generating them from out of the images,
	 * and then asking the WorldModel to create extra ones if needed.
	 * 
	 */
    private void createPlaces() {
        try {
            worldModel = (BaseWorldModel) simData.getWorldModelClass().getConstructor(new Class[] { this.getClass() }).newInstance(new Object[] { this });
        } catch (Exception e) {
    ...
    private void createOverlays() {
        ...
        try {
            contextModel = (BaseContextModel) simData.getContextModelClass().getConstructor(new Class[] { this.getClass() }).newInstance(new Object[] { this });
        } catch (Exception e) {
    ...
    private ArrayList<Overlay> createOverlaysFromImages() {
        ...
        Iterator listIt = fileList.keySet().iterator();
        while (listIt.hasNext()) {
            String name = (String) listIt.next();
	 ...
	 * 
	 * @param placeType the name of the place type
	 */
