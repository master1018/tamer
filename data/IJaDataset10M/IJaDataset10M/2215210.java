 ...
 */
public class OperationRequestGsonAdaptor implements JsonSerializer<OperationRequest>, JsonDeserializer<OperationRequest> {
   ...
   */
    public OperationRequestGsonAdaptor(String operationNamespace) {
        if (operationNamespace != null && !operationNamespace.isEmpty() && !operationNamespace.endsWith(".")) {
            operationNamespace += ".";
        }
    ...
    @Override
    public OperationRequest deserialize(JsonElement json, Type type, JsonDeserializationContext ctx) throws JsonParseException {
        JsonObject jsonObject = json.getAsJsonObject();
        JsonObject parameters = jsonObject.getAsJsonObject(RequestProperty.PARAMS.key());
        OperationRequest request = new OperationRequest(jsonObject.get(RequestProperty.METHOD.key()).getAsString(), jsonObject.get(RequestProperty.ID.key()).getAsString(), getPropertyAsStringThenRemove(parameters, ParamsProperty.WAVE_ID), getPropertyAsStringThenRemove(parameters, ParamsProperty.WAVELET_ID), getPropertyAsStringThenRemove(parameters, ParamsProperty.BLIP_ID));
