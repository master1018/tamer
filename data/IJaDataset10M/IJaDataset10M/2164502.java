...
class OPT_Dominators {
   ...
   */
    public static void perform(OPT_IR ir) {
        if (ir.hasReachableExceptionHandlers()) throw new OPT_OperationNotImplementedException("IR with exception handlers");
        OPT_DominatorSystem system = new OPT_DominatorSystem(ir);
        if (DEBUG) System.out.print("Solving...");
        if (DEBUG) System.out.println(system);
        system.solve();
        if (DEBUG) System.out.println("done");
        OPT_DF_Solution solution = system.getSolution();
        if (DEBUG) System.out.println("Dominator Solution :" + solution);
        if (DEBUG) System.out.print("Updating blocks ...");
        updateBlocks(solution);
        if (DEBUG) System.out.println("done.");
        if (ir.options.PRINT_DOMINATORS) printDominators(ir);
    }
   ...
   */
    public static void computeApproxPostdominators(OPT_IR ir) {
        ...
        if (DEBUG) System.out.println("done");
        OPT_DF_Solution solution = system.getSolution();
        if (DEBUG) System.out.println("Postdominator Solution :" + solution);
        if (DEBUG) System.out.print("Updating blocks ...");
        updateBlocks(solution);
        if (DEBUG) System.out.println("done.");
        if (ir.options.PRINT_DOMINATORS) printDominators(ir);
   ...
   */
    public static void updateBlocks(OPT_DF_Solution solution) {
        for (java.util.Iterator e = solution.values().iterator(); e.hasNext(); ) {
            ...
            if (DEBUG) System.out.println("Dominators of " + b + ":" + cell.dominators);
        }
    }
...
class OPT_DominatorSystem extends OPT_DF_System {
    ...
    protected void initializeLatticeCells() {
        if (OPT_Dominators.COMPUTE_POST_DOMINATORS) {
            ...
            for (java.util.Iterator e = cells.values().iterator(); e.hasNext(); ) {
                OPT_DominatorCell cell = (OPT_DominatorCell) e.next();
                if (cell == last) cell.addSingleBlock(cell.block); else cell.setTOP(ir);
            }
        } else {
            ...
            for (java.util.Iterator e = cells.values().iterator(); e.hasNext(); ) {
                OPT_DominatorCell cell = (OPT_DominatorCell) e.next();
                if (cell == first) cell.addSingleBlock(cell.block); else cell.setTOP(ir);
            }
        }
    }
   ...
   */
    OPT_DF_LatticeCell[] getCellsForPredecessors(OPT_BasicBlock bb) {
        if (OPT_Dominators.COMPUTE_POST_DOMINATORS) {
            {
                if (OPT_Dominators.DEBUG) VM.sysWrite("LOCATION #2 ...\n");
            ...
            if (OPT_Dominators.DEBUG) System.out.println("LOCATION #3 ...");
...
class OPT_DominatorOperator extends OPT_DF_Operator {
   ...
   */
    boolean evaluate(OPT_DF_LatticeCell[] operands) {
        ...
        if (lhs.dominators.equals(oldSet)) return false; else return true;
    }
