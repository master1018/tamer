...
public class CheckpointEnd implements Loggable {
    ...
    public CheckpointEnd(String invoker, long checkpointStartLsn, long rootLsn, long firstActiveLsn, long lastLocalNodeId, long lastReplicatedNodeId, int lastLocalDbId, int lastReplicatedDbId, long lastLocalTxnId, long lastReplicatedTxnId, long id, boolean cleanedFilesToDelete) {
        if (invoker == null) {
            this.invoker = "";
        } else {
    ...
    public int getLogSize() {
        int size = LogUtils.getStringLogSize(invoker) + LogUtils.getTimestampLogSize(endTime) + LogUtils.getPackedLongLogSize(checkpointStartLsn) + 1 + LogUtils.getPackedLongLogSize(firstActiveLsn) + LogUtils.getPackedLongLogSize(lastLocalNodeId) + LogUtils.getPackedLongLogSize(lastReplicatedNodeId) + LogUtils.getPackedIntLogSize(lastLocalDbId) + LogUtils.getPackedIntLogSize(lastReplicatedDbId) + LogUtils.getPackedLongLogSize(lastLocalTxnId) + LogUtils.getPackedLongLogSize(lastReplicatedTxnId) + LogUtils.getPackedLongLogSize(id);
        if (rootLsnExists) {
    ...
    public void readFromLog(ByteBuffer logBuffer, int entryVersion) {
        ...
        if (version6OrLater) {
            lastReplicatedNodeId = LogUtils.readLong(logBuffer, false);
        }
        lastLocalDbId = LogUtils.readInt(logBuffer, !version6OrLater);
        if (version6OrLater) {
            lastReplicatedDbId = LogUtils.readInt(logBuffer, false);
        }
        lastLocalTxnId = LogUtils.readLong(logBuffer, !version6OrLater);
        if (version6OrLater) {
            lastReplicatedTxnId = LogUtils.readLong(logBuffer, false);
        }
