...
public class phoenix {
    ...
    static rectangle backvisiblearea = new rectangle(3 * 8, 29 * 8 - 1, 3 * 8, 31 * 8 - 1);

    static rectangle backtmparea = new rectangle(3 * 8, 29 * 8 - 1, 0 * 8, 32 * 8 - 1);
	...
	***************************************************************************/
    ...
    public static WriteHandlerPtr phoenix_videoram2_w = new WriteHandlerPtr() {
    ...
    public static WriteHandlerPtr phoenix_scrollreg_w = new WriteHandlerPtr() {
        ...
        public void handler(osd_bitmap bitmap) {
            ...
            for (offs = videoram_size[0] - 1; offs >= 0; offs--) {
                if (dirtybuffer[offs] != 0) {
                    ...
                    sx = 8 * (31 - offs / 32) - 3 * 8;
                    sy = 8 * (offs % 32);
                    drawgfx(tmpbitmap, Machine.gfx[1], videoram.read(offs), (videoram.read(offs) >> 5) + 8 * palette_bank, 0, 0, sx, sy, backtmparea, TRANSPARENCY_NONE, 0);
                }
            }
            for (offs = videoram_size[0] - 1; offs >= 0; offs--) {
                if (dirtybuffer2[offs] != 0) {
                    ...
                    sx = 8 * (31 - offs / 32) - 3 * 8;
                    sy = 8 * (offs % 32);
                    drawgfx(tmpbitmap2, Machine.gfx[0], phoenix_videoram2.read(offs), phoenix_videoram2.read(offs) >> 5, 0, 0, sx, sy, null, TRANSPARENCY_NONE, 0);
                }
            }
            {
                int scroll;
            ...
            for (offs = videoram_size[0] - 1; offs >= 0; offs--) {
                ...
                sx = 8 * (31 - offs / 32) - 3 * 8;
                sy = 8 * (offs % 32);
                if (sy >= 3 * 8 && phoenix_videoram2.read(offs) != 0) drawgfx(bitmap, Machine.gfx[0], phoenix_videoram2.read(offs), (phoenix_videoram2.read(offs) >> 5) + 8 * palette_bank, 0, 0, sx, sy, Machine.drv.visible_area, TRANSPARENCY_PEN, 0);
            }
        }
    };
}
