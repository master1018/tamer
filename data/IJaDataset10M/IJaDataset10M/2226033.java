...
public class Container extends Element {
    ...
    protected String CalcStyle() {
        String CStyle = "";
        if (Width < 0) CStyle += " width: " + (-Width) + "%; "; else if (Width > 0) CStyle += " width: " + Width + "px; ";
        if (Height < 0) CStyle += " height: " + (-Height) + "%; "; else if (Height > 0) CStyle += " height: " + Height + "px; ";
        switch(Alineacion) {
            case CENTER:
            ...
            case JUST:
                ...
                break;
            case LEFT:
                CStyle += " text-align: left; ";
                break;
        }
        if (f != null) CStyle += f.ToHtml();
        if (MargenIzq != 0) CStyle += " margin-left: " + MargenIzq + "px; ";
        if (MargenDer != 0) CStyle += " margin-right: " + MargenDer + "px; ";
        if (ColorBack != null) CStyle += " background-color: rgb(" + ColorBack + "); ";
        if (CStyle.length() != 0) CStyle = " style=\"" + CStyle + "\"";
        if (CSSClass != null) CStyle += " class=\"" + CSSClass + "\" ";
        if (getCSSId() != null) CStyle += " id=\"" + getCSSId() + "\" ";
        if (OnClick != null) CStyle += " onclick=\"" + OnClick + "\" ";
        return (CStyle);
    }
