...
public class VerifyDesignDelegate implements Log {
    ...
    public void execute() {
        if (!designFile.exists() || designFile.isDirectory()) throw new BuildException("design attribute in verifydesign element specified an invalid file=" + designFile);
        verifyJarFilesExist();
        try {
            ...
            if (designErrors.isEmpty()) design.fillInUnusedPackages(designErrors);
            if (!designErrors.isEmpty()) {
                log(designErrors.size() + "Errors.", Project.MSG_WARN);
                if (!fillInBuildException) throw new BuildException("Design check failed due to previous errors");
                throwAllErrors();
            }
        } catch (SAXException e) {
            maybeDeleteFiles();
            if (e.getException() != null && e.getException() instanceof RuntimeException) throw (RuntimeException) e.getException(); else if (e instanceof SAXParseException) {
        ...
        if (!verifiedAtLeastOne) throw new BuildException("Did not find any class or jar files to verify");
    }
    ...
    private void verifyJarFilesExist() {
        ...
        while (pathsEnum.hasMoreElements()) {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (!file.exists()) throw new BuildException(VisitorImpl.getNoFileMsg(file));
            }
        }
    }
    ...
    private void verifyPathAdheresToDesign(Design d, Path p) throws ClassFormatException, IOException {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            } else verifyPartOfPath(files[i], file, d);
        }
    }
    ...
    private void verifyPartOfPath(String fileName, File file, Design d) throws IOException {
        ...
        } else throw new BuildException("Only directories, jars, wars, and class files can be supplied to verify design, not file=" + file.getAbsolutePath());
    }
    ...
    private void verifyJarAdheresToDesign(Design d, JarFile jarFile, File original) throws ClassFormatException, IOException {
        try {
    ...
    private void verifyClassAdheresToDesign(Design d, InputStream in, String name, File originalClassOrJarFile) throws ClassFormatException, IOException {
        try {
            ...
            if (!d.needEvalCurrentClass(className)) return;
    ...
    private void processConstant(ConstantPool pool, Constant c, int i) {
        ...
        switch(tag) {
            case Constants.CONSTANT_Class:
                ...
                if (primitives.contains(firstLetter)) return;
    ...
    private static String getRidOfArray(String className) {
        while (className.startsWith("[")) className = className.substring(1, className.length());
        return className;
    }
    ...
    public static String getPackageName(String className) {
        ...
        if (index > 0) packageName = className.substring(0, index);
        return packageName;
    }
