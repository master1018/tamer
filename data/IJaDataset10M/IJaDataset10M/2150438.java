...
public class ArgParser {
    ...
    */
    public String getStringArg(String key, String def) {
        for (int i = 0; i < args.length; i++) {
            String arg = args[i];
            if (arg.equalsIgnoreCase(key) && args.length >= i + 2) return args[i + 1].trim();
            if (arg.startsWith(key)) return arg.substring(key.length()).trim();
        }
        return def;
    }
    ...
    */
    ...
    public int getIntArg(String key, int def) {
        ...
        if (i == null) return def; else return i.intValue();
    }
