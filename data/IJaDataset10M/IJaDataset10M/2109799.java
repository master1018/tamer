...
public class MonitorAccessExporter implements Exporter {
    ...
    /**
	 * Creates an exporter with an underlying {@link JrmpExporter} that exports
	 * on the standard activation port (1098).
	 */
	 ...
	 * 
	 * @param port
	 *            the port on which to receive calls (if zero, an anonymous port
	 *            will be chosen)
	 */
	 ...
	 * delegating to the specified remote object, exports the wrapper with the
	 * underlying exporter, and returns the resulting proxy. The wrapper is
	 * strongly referenced by this exporter. For all
	 * <code>ActivationMonitor</code> methods, the wrapper throws an
	 * <code>AccessControlException</code> if the client is not calling from the
	 * local host.
	 * 
	 * @throws IllegalArgumentException
	 *             if <code>impl</code> does not implement
    ...
    private static class MonitorImpl extends AbstractMonitor {
        ...
        public void activeObject(ActivationID id, MarshalledObject mobj) throws UnknownObjectException, RemoteException {
            LocalAccess.check();
