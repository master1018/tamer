...
public class RepastBatchMain {
    ...
    public RepastBatchMain() {
        ...
        Option paramfile = OptionBuilder.withArgName("file").hasArg().withDescription("use given parameter sweep file").create("params");
        Option opt = OptionBuilder.withArgName("file").hasOptionalArg().withDescription("use optimizable parameter sweeper with given optimizing properties file").create("opt");
        Option mode = OptionBuilder.withDescription("specifies if the batch mode is interactive").create("interactive");
        Option bootProps = OptionBuilder.withArgName("file").hasArg().withDescription("location of boot.properties file (optional)").create("bootprops");
    ...
    public void run(String[] args) {
        try {
            ...
            if (line.hasOption("bootprops")) {
                ...
                URL runtimeSource = repast.simphony.runtime.RepastMain.class.getProtectionDomain().getCodeSource().getLocation();
