...
public class ScaleShapes extends ShapeAction<IGroup> implements Undoable, Modifying {
    ...
    public void setShape(final IGroup shape) {
        ...
        if (shape != null) updateBound(shape.getTopLeftPoint(), shape.getBottomRightPoint());
    }

    private double getScaledHeight() {
        if (refPosition.isSouth()) return bound.getHeight() + bound.getY() - newY; else if (refPosition.isNorth()) return bound.getHeight() + newY - bound.getMaxY();
        return 0.;
    }

    private double getScaledWidth() {
        if (refPosition.isWest()) return bound.getWidth() + newX - bound.getMaxX(); else if (refPosition.isEast()) return bound.getWidth() + bound.getX() - newX;
        return 0.;
    }
