...
public final class Manual {
    ...
    private enum SubPageProperties {
        ...
        Query_Syntax("query_syntax", "Advanced_Usage"), Portable_Repositories("portable_repos", "Advanced_Usage"), Indexing_Options("indexing_options", "Advanced_Usage"), Regular_Expressions("regular_expressions", "Advanced_Usage"), Release_Notification("release_notification", "Advanced_Usage"), Memory_Limit("memory_limit", "Caveats"), Preferences("preferences", "Subpages");
        ...
        private SubPageProperties(@NotNull String propsKey, @NotNull String backlink) {
            this.propsKey = propsKey;
            this.backlink = backlink;
        }

        public static void initPageTitles(@NotNull PageProperties props) throws IOException {
    ...
    @RecursiveMethod
    private static void convert(@NotNull PegDownProcessor processor, @NotNull PageProperties props, @NotNull File srcDir, @NotNull File dstDir, boolean isTopLevel) throws IOException {
        for (File mdFile : Util.listFiles(srcDir)) {
            ...
            if (mdFile.isDirectory()) {
                ...
                String html = isTopLevel ? convertMainPage(props, mdFile, htmlBody) : convertSubPage(props, mdFile, htmlBody);
    ...
    @NotNull
    private static String convertMainPage(@NotNull PageProperties props, @NotNull File file, @NotNull String htmlBody) throws IOException {
        String path = file.getPath();
        File templateFile = new File(manDir, "template-mainpage.html");
        String template = CharsetDetectorHelper.toString(templateFile);
        Pair[] pairs = new Pair[] { props.docfetcherManual, props.author, props.mainFooter };
        for (Pair pair : pairs) {
            String key = "${" + pair.key + "}";
            template = UtilGlobal.replace(path, template, key, pair.value);
        }
        return UtilGlobal.replace(path, template, "${app_name}", BuildMain.appName, "${version}", BuildMain.version, "${contents}", htmlBody);
    }

    @NotNull
    private static String convertSubPage(@NotNull PageProperties props, @NotNull File file, @NotNull String htmlBody) throws IOException {
        String path = file.getPath();
        File templateFile = new File(manDir, "template-subpage.html");
        String template = CharsetDetectorHelper.toString(templateFile);
        Pair[] pairs = new Pair[] { props.docfetcherManual, props.author, props.backToMainPage };
        for (Pair pair : pairs) {
            String key = "${" + pair.key + "}";
            template = UtilGlobal.replace(path, template, key, pair.value);
        }
