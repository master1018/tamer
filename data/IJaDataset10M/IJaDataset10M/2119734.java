...
public abstract class StringFunctions {
    ...
    public static AttributeValue stringGreaterThan(EvaluationContext ctx, Object[] params) throws IndeterminateException {
        return compareString(params) > 0 ? TRUE : FALSE;
    }

    @XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:function:string-greater-than-or-equal" })
    public static AttributeValue stringGreaterThanOrEqual(EvaluationContext ctx, Object[] params) throws IndeterminateException {
        return compareString(params) >= 0 ? TRUE : FALSE;
    }

    @XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:function:string-less-than" })
    public static AttributeValue stringLessThan(EvaluationContext ctx, Object[] params) throws IndeterminateException {
        return compareString(params) < 0 ? TRUE : FALSE;
    }

    @XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:function:string-less-than-or-equal" })
    public static AttributeValue stringLessThanOrEqual(EvaluationContext ctx, Object[] params) throws IndeterminateException {
        return compareString(params) <= 0 ? TRUE : FALSE;
    }

    @XACMLFunction({ "urn:oasis:names:tc:xacml:2.0:function:string-concatenate" })
    public static AttributeValue stringConcat(EvaluationContext ctx, Object[] params) throws IndeterminateException {
        checkArguments(params, -1);
        if (params.length < 2) {
    ...
    public static AttributeValue uriConcat(EvaluationContext ctx, Object[] params) throws IndeterminateException {
        checkArguments(params, -1);
        if (params.length < 2) {
            throw new IndeterminateException("Expected at least 2 parameters, but only got " + params.length);
        }
