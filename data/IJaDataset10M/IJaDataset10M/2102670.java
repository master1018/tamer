...
public class PluginLoaderServiceImpl implements PluginLoaderService {
    ...
    private static enum LoadPlugInType {
        ...
        NOTATIONS_AND_MODULES, EXTENSION
    }

    private static final String ROOT_TAG = "promod-plugins";

    private static final String NOTATION_TAG = "notation";
    ...
    private static final String NOTATION_INTERFACE_QUALIFIED_NAME = "cz.cvut.promod.plugin.notationSpecificPlugIn.notation.Notation";

    private static final String MODULE_INTERFACE_QUALIFIED_NAME = "cz.cvut.promod.plugin.notationSpecificPlugIn.module.Module";

    private static final String IOCONTROLPLUGIN_INTERFACE_QUALIFIED_NAME = "cz.cvut.promod.plugin.extension.Extension";
    ...
    public boolean loadPlugInsDefinition(final String pluginDefinitionFile) {
        ...
        final InputStream schemaInputStream = ClassLoader.getSystemResourceAsStream(Resources.CONFIG + Resources.PLUGINS_XSD_FILE);
    ...
    private boolean loadPluginDetails(final Document document) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if (notationNode.getNodeName().equals(NOTATION_TAG)) {
                ...
                final PluginDetails notationDetail = new PluginDetails(readAttribute(attributes.getNamedItem(CLASS_ATTRIBUTE)), readAttribute(attributes.getNamedItem(CONFIG_ATTRIBUTE)), readAttribute(attributes.getNamedItem(ALIAS_ATTRIBUTE)));
                ...
                for (int j = 0; j < moduleList.getLength(); j++) {
                    ...
                    if (moduleNode.getNodeName().equals(MODULE_TAG)) {
                        ...
                        final PluginDetails moduleDetail = new PluginDetails(readAttribute(attributes.getNamedItem(CLASS_ATTRIBUTE)), readAttribute(attributes.getNamedItem(CONFIG_ATTRIBUTE)), readAttribute(attributes.getNamedItem(ALIAS_ATTRIBUTE)));
                ...
                final PluginDetails extensionDetail = new PluginDetails(readAttribute(attributes.getNamedItem(CLASS_ATTRIBUTE)), readAttribute(attributes.getNamedItem(CONFIG_ATTRIBUTE)), readAttribute(attributes.getNamedItem(ALIAS_ATTRIBUTE)));
    ...
    private void instantiatePlugins(LoadPlugInType loadPlugInType) {
        ...
        for (final PluginDetails details : pluginDetailsList) {
            String notationIdentifier = null;
            ...
            do {
                ...
                error.setFullClassName(fullClassName);
                ...
                final Plugin plugin = instantiatePlugin(clazz, pluginDetail, error, notationIdentifier, plugInTypeInterface == moduleClass);
                if (plugin == null) {
                    LOG.error("Due to error(s) skipping loading of " + clazz.getName() + ".");
                    break;
                }
     ...
     */
    private Plugin instantiatePlugin(final Class clazz, final PluginDetails pluginDetails, final PluginLoadErrors error, final String notationIdentifier, final boolean isModule) {
        try {
            Plugin plugin;
     ...
     */
    private boolean storePluginObject(final Class loadPlugInType, final Plugin plugin, final PluginLoadErrors error) {
        if (plugin != null) {
    ...
    private boolean storeModule(final Module module, final PluginLoadErrors error) {
        ...
        if (!existUniqueNotationIdentifier(module.getRelatedNotationIdentifier())) {
            LOG.error("Module without related notation has been loaded. Skipping module - " + module.getIdentifier() + ", related notation identifier - " + module.getRelatedNotationIdentifier() + ".");
    ...
    private boolean hasNotationMandatoryData(final Notation notation, final PluginLoadErrors error) {
        ...
        for (final ModifiableNotationSpecificPlugins notationSpecificPlugins : notationSpecificPluginsList) {
            if (notationSpecificPlugins.getNotation().getLocalIOController().getNotationFileExtension().equals(ioController.getNotationFileExtension())) {
