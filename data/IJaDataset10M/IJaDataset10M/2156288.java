...
public class VCreateFromStatement extends VCreateFrom implements VetoableChangeListener {
    ...
    private void loadBankAccount(int C_BankAccount_ID) {
        ...
        sql.append("SELECT ").append("p.DateTrx, ").append("p.C_Payment_ID, ").append("p.DocumentNo, ").append("p.C_Currency_ID, ").append("c.ISO_Code, ").append("p.PayAmt, ").append("currencyConvert(p.PayAmt,p.C_Currency_ID,ba.C_Currency_ID,?,null,p.AD_Client_ID,p.AD_Org_ID) AS ConvertedAmt, ").append("bp.Name AS BPartnerName ").append("FROM C_BankAccount ba ").append("INNER JOIN C_Payment_v p ON (p.C_BankAccount_ID=ba.C_BankAccount_ID) ").append("INNER JOIN C_Currency c ON (p.C_Currency_ID=c.C_Currency_ID) ").append("INNER JOIN C_BPartner bp ON (p.C_BPartner_ID=bp.C_BPartner_ID) ").append("WHERE p.Processed='Y' ").append("AND p.IsReconciled='N' ").append("AND p.DocStatus IN ('CO','CL','RE') ").append("AND p.PayAmt<>0 ").append("AND p.C_BankAccount_ID=? ").append("AND NOT EXISTS ").append("(SELECT * FROM C_BankStatementLine l ").append("WHERE p.C_Payment_ID=l.C_Payment_ID AND l.StmtAmt <> 0)");
    ...
    protected void save() throws CreateFromSaveException {
        ...
        for (SourceEntity sourceEntity : getSelectedSourceEntities()) {
            ...
            bsl.setPayment(pay);
            if (!bsl.save()) {
                throw new CreateFromSaveException("@StatementLineSaveError@ (@C_Paymenty_ID@ # " + payment.documentNo + "):<br>" + CLogger.retrieveErrorAsString());
            }
        }
    }
        ...
        public Object getValueAt(int rowIndex, int colIndex) {
            ...
            switch(colIndex) {
                case COL_IDX_DATETRX:
                    value = payment.dateTrx;
                    break;
                case COL_IDX_DOCUMENTNO:
                    value = payment.documentNo;
                    break;
                case COL_IDX_BPARTNER:
                    value = payment.bPartnerName;
                    break;
                case COL_IDX_CURRENCY:
                    value = payment.currencyISO;
                    break;
                case COL_IDX_AMT:
                    value = payment.payAmt;
                    break;
                case COL_IDX_CONVAMT:
                    value = payment.convertedAmt;
                    break;
                default:
