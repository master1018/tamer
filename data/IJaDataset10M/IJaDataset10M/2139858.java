...
public class VirtualHeapAllocator extends BufferAllocation implements IBufferAllocator {
    ...
    public VirtualHeapAllocator(AbstractBufferContainer container) {
        super(new Buffer("virtual_heap", MIN_SIZE, new DataType("char"), container));
        basePos = 0;
    ...
    public HeapSectionAllocator openNewSection(AbstractBufferContainer codeSection) {
    ...
    public SubBuffer addBuffer(SDFEdge edge, String name, DataType type) {
        ...
        SubBuffer newBuffer = new SubBuffer(name, size, new ConstantExpression("", type, currentPos), new DataType(type), this.getBuffer(), edge, container);
        ...
        while (allocStack.size() > 0) {
            if (allocStack.get(0) instanceof VirtualHeapAllocator) {
                for (BufferAllocation allocToPrint : allocStack.get(0).getBufferAllocations()) {
                    printer.visit((SubBufferAllocation) allocToPrint, CodeZoneId.body, currentLocation);
                }
                allocStack.addAll(((VirtualHeapAllocator) allocStack.get(0)).getChildAllocators());
            } else {
