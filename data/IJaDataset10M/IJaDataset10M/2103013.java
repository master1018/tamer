...
public class RandomProjection extends Filter implements UnsupervisedFilter, OptionHandler, TechnicalInformationHandler {
  ...
  calculating the random matrix */
    public static final Tag[] TAGS_DSTRS_TYPE = { new Tag(SPARSE1, "Sparse 1"), new Tag(SPARSE2, "Sparse 2"), new Tag(GAUSSIAN, "Gaussian") };
   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (mString.length() != 0) {
            setPercent((double) Double.parseDouble(mString));
        } else {
            setPercent(0);
            mString = Utils.getOption('N', options);
            if (mString.length() != 0) setNumberOfAttributes(Integer.parseInt(mString)); else setNumberOfAttributes(10);
        }
        ...
        if (mString.length() != 0) {
            if (mString.equalsIgnoreCase("sparse1")) setDistribution(new SelectedTag(SPARSE1, TAGS_DSTRS_TYPE)); else if (mString.equalsIgnoreCase("sparse2")) setDistribution(new SelectedTag(SPARSE2, TAGS_DSTRS_TYPE)); else if (mString.equalsIgnoreCase("gaussian")) setDistribution(new SelectedTag(GAUSSIAN, TAGS_DSTRS_TYPE));
        }
        if (Utils.getFlag('M', options)) setReplaceMissingValues(true); else setReplaceMissingValues(false);
    }
    ...
    public boolean setInputFormat(Instances instanceInfo) throws Exception {
        ...
        for (int i = 0; i < instanceInfo.numAttributes(); i++) {
            if (i != instanceInfo.classIndex() && instanceInfo.attribute(i).isNominal()) {
                if (instanceInfo.classIndex() >= 0) m_ntob = new weka.filters.supervised.attribute.NominalToBinary(); else m_ntob = new weka.filters.unsupervised.attribute.NominalToBinary();
                break;
            }
        }
        boolean temp = true;
        if (m_replaceMissing != null) {
            ...
            if (m_replaceMissing.setInputFormat(instanceInfo)) temp = true; else temp = false;
        }
        if (m_ntob != null) {
   ...
   */
    protected double rndmNum(boolean useDstrWithZero) {
        if (useDstrWithZero) return sqrt3 * vals[weightedDistribution(weights)]; else return vals2[weightedDistribution(weights2)];
    }
