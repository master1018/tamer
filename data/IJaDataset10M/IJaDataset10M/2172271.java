...
/**
 * @author mog
 * @version $Id: AsyncSlave.java 584 2004-05-21 18:42:01Z zombiewoof64 $
 */
public class AsyncSlave extends Thread implements Slave, Unreferenced {
    ...
    public AsyncSlave(ConnectionManager mgr, Hashtable cfg) throws RemoteException {
        Socket sock = null;
    ...
    public AsyncSlave(ConnectionManager mgr, Hashtable cfg, Socket sock) throws RemoteException {
        _name = (String) cfg.get("name");
        _spsw = (String) cfg.get("slavepass");
        _mpsw = (String) cfg.get("masterpass");
        _host = (String) cfg.get("addr");
    ...
    public void run() {
        ...
        while (true) {
            ...
            while (msg != "") {
                ...
                msg = msg.substring(3);
                if (cmd._name.equals("ping")) processPing(cmd, msg); else if (cmd._name.equals("xfer")) xferMessage(msg); else if (cmd._name.equals("disk")) processDisk(cmd, msg); else if (cmd._name.equals("conn")) processConnect(cmd, msg); else if (cmd._name.equals("send")) processSend(cmd, msg); else if (cmd._name.equals("recv")) processRecv(cmd, msg); else if (cmd._name.equals("renm")) processRename(cmd, msg); else if (cmd._name.equals("dele")) processDelete(cmd, msg); else if (cmd._name.equals("list")) processList(cmd, msg); else if (cmd._name.equals("csum")) processCRC32(cmd, msg); else if (cmd._name.equals("dump")) processDump(cmd, msg);
                msg = readLine(0);
                if (msg == null) {
                    shutdown();
                    return;
    ...
    public void removeTransfer(AsyncTransfer transfer) {
        synchronized (_transfers) {
            ...
            if (!_transfers.remove(transfer)) throw new IllegalStateException();
        }
    }
    ...
    private void processDelete(AsyncCommand cmd, String res) {
        if (res.startsWith("DELFAIL ")) cmd.setStatus(1); else cmd.setStatus(0);
    }

    private void processRename(AsyncCommand cmd, String res) {
        if (res.startsWith("RENFAIL ")) cmd.setStatus(1); else cmd.setStatus(0);
    }
    ...
    private void processConnect(AsyncCommand cmd, String res) {
        if (res.startsWith("CONNFAIL ")) {
            ...
            if (items.length > 2) cmd.getData().put("addr", items[2]);
            cmd.setStatus(0);
        }
    }
    ...
    private void processList(AsyncCommand cmd, String res) {
        ...
        if (res.startsWith("LISTFAIL")) {
            ...
            if (res.equals("LISTEND")) {
                try {
                    LinkedRemoteFile root = MLSTSerialize.unserialize(_cman.getConfig(), new StringReader(buf.toString()), _cman.getSlaveManager().getSlaveList());
                    _root = root;
                } catch (Exception e) {
            ...
            if (!res.startsWith("/") && !res.equals("") && res.indexOf("type=dir;") == -1) buf.append("x.slaves=" + _name + ";");
            buf.append(res);
            buf.append((String) "\n");
        }
    }
    ...
    public SlaveStatus getSlaveStatus() {
        ...
        try {
            return new SlaveStatus(diskfree, disktotal, bytesSent, bytesReceived, throughputUp, transfersUp, throughputDown, transfersDown);
        } catch (Exception ex) {
            ex.printStackTrace();
