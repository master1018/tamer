...
public final class Dimen extends BinFraction {
    ...
    public static Dimen valueOf(String s) throws NumberFormatException {
        ...
        if (pointIndex < 0) return new Dimen(makeRepr(Integer.parseInt(s))); else {
    ...
    public int badness(Dimen limit) {
        int total = value;
        int sum = limit.value;
        if (total == 0) return 0; else if (total < 0) {
            total = -total;
            sum = -sum;
        }
        if (sum <= 0) return INF_BAD;
        int ratio;
        if (total <= 7230584) ratio = total * 297 / sum; else if (sum >= 1663497) ratio = total / (sum / 297); else return INF_BAD;
