...
class GeneralizedVertexList implements GeneralizedStripFlags {
    ...
    GeneralizedVertexList(int vertexFormat, int frontFace, int initSize) {
        ...
        if (initSize == 0) vertices = new ArrayList(); else vertices = new ArrayList(initSize);
        stripCount = 0;
        vertexCount = 0;
        triangleCount = 0;
    }
    ...
    void setVertexFormat(int vertexFormat) {
        ...
        if ((vertexFormat & GeometryArray.NORMALS) != 0) hasNormals = true;
        if ((vertexFormat & GeometryArray.COLOR) != 0) if ((vertexFormat & GeometryArray.WITH_ALPHA) != 0) hasColor4 = true; else hasColor3 = true;
    }
    ...
    class Vertex {
        ...
        Vertex(Point3f p, Vector3f n, Color4f c, int flag) {
            this.flag = flag;
            coord = new Point3f(p);
            if (hasNormals) normal = new Vector3f(n);
            if (hasColor3) color3 = new Color3f(c.x, c.y, c.z); else if (hasColor4) color4 = new Color4f(c);
        }
    }
    ...
    LineStripArray toLineStripArray() {
        ...
        if (stripArray != null) {
            LineStripArray lsa;
            lsa = new LineStripArray(stripArray.vertices.count, vertexFormat, stripArray.stripCounts.trim());
    ...
    TriangleStripArray toTriangleStripArray() {
        GeneralizedStrip.StripArray stripArray = GeneralizedStrip.toTriangleStrips(this, frontFace);
        if (stripArray != null) {
            TriangleStripArray tsa;
            tsa = new TriangleStripArray(stripArray.vertices.count, vertexFormat, stripArray.stripCounts.trim());
    ...
    GeometryStripArray[] toStripAndFanArrays() {
        GeneralizedStrip.StripArray stripArray[] = GeneralizedStrip.toStripsAndFans(this, frontFace);
        GeometryStripArray gsa[] = new GeometryStripArray[2];
        if (stripArray[0] != null) {
            gsa[0] = new TriangleStripArray(stripArray[0].vertices.count, vertexFormat, stripArray[0].stripCounts.trim());
        ...
        if (stripArray[1] != null) {
            gsa[1] = new TriangleFanArray(stripArray[1].vertices.count, vertexFormat, stripArray[1].stripCounts.trim());
    ...
    GeometryArray[] toStripAndTriangleArrays() {
        GeneralizedStrip.StripArray stripArray[] = GeneralizedStrip.toStripsAndTriangles(this, frontFace, 4, 12);
        ...
        if (stripArray[1] != null) {
            ga[1] = new TriangleArray(stripArray[1].vertices.count, vertexFormat);
