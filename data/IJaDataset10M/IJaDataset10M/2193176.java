 ...
 */
public abstract class StopTheWorldGC extends BasePlan implements Constants, VM_Uninterruptible {

    public static final String Id = "$Id: StopTheWorldGC.java 3566 2003-05-22 20:51:25Z dgrove-oss $";
    ...
    protected AddressQueue values;

    protected AddressQueue locations;

    protected AddressQueue rootLocations;

    protected AddressPairQueue interiorRootLocations;
   ...
   */
    ...
    protected abstract void globalPrepare();

    protected abstract void threadLocalPrepare(int order);

    protected abstract void threadLocalRelease(int order);

    protected abstract void globalRelease();

    /**
   * Check whether a stress test GC is required
   */
    protected static final boolean stressTestGCRequired() throws VM_PragmaInline {
        int pagesReserved = Plan.getPagesReserved();
        if (VM_Interface.fullyInitialized() && ((pagesReserved ^ lastStressPagesReserved) > Options.stressTest)) {
   ...
   */
    protected final void prepare() {
        ...
        if (order == 1) baseGlobalPrepare(start);
        VM_CollectorThread.gcBarrier.rendezvous();
   ...
   */
    private final void baseGlobalPrepare(double start) {
        ...
        if ((verbose == 1) || (verbose == 2)) {
            ...
            if (verbose == 1) VM_Interface.sysWrite(" Start ", (gcStartTime - bootTime), " s"); else VM_Interface.sysWrite(" Start ", 1000 * (gcStartTime - bootTime), " ms");
        ...
        if (verbose > 2) {
            ...
            VM_Interface.sysWrite(":        reserved = ");
            writePages(Plan.getPagesReserved(), MB_PAGES);
            VM_Interface.sysWrite("      total = ");
            writePages(getTotalPages(), MB_PAGES);
   ...
   */
    public final void baseThreadLocalPrepare(int order) {
        if (order == NON_PARTICIPANT) VM_Interface.prepareNonParticipating((Plan) this); else VM_Interface.prepareParticipating((Plan) this);
        VM_CollectorThread.gcBarrier.rendezvous();
   ...
   */
    private final void baseGlobalRelease() {
        ...
        if ((verbose == 1) || (verbose == 2)) {
            VM_Interface.sysWrite("-> ");
            VM_Interface.sysWrite(Conversions.pagesToBytes(Plan.getPagesUsed()) >> 10, " KB   ");
            if (verbose == 1) VM_Interface.sysWriteln(1000 * (gcStopTime - gcStartTime), " ms]"); else VM_Interface.sysWriteln("End ", 1000 * (gcStopTime - bootTime), " ms]");
        }
        if (verbose > 2) {
   ...
   */
    private final void processAllWork() throws VM_PragmaNoInline {
        ...
        do {
            ...
            flushRememberedSets();
        } while (!(rootLocations.isEmpty() && interiorRootLocations.isEmpty() && values.isEmpty() && locations.isEmpty()));
