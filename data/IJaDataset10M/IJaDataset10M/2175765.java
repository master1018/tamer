...
public class PlanarMathUtilsTest {
    ...
    public static void assertEqualsPt(Double p1, Double p2, double tolerance) {
        assertEqualsPt(p1.x, p1.y, p2, tolerance);
    }

    public static void assertEqualsPt(double x1, double y1, Double p2, double tolerance) {
        assertEquals(x1, p2.x, tolerance);
        assertEquals(y1, p2.y, tolerance);
    }

    public static void assertArrayEquals(double[] arr1, double[] arr2, double tolerance) {
        assertEquals(arr1.length, arr2.length);
        for (int i = 0; i < arr1.length; i++) {
            assertEquals(arr1[i], arr2[i], tolerance);
        }
    }

    Double[] polar = { new Double(1.0, 0.0), new Double(1.0, Math.PI / 4), new Double(1.0, Math.PI / 2), new Double(2.0, Math.PI), new Double(4.0, -Math.PI / 2) };

    Double[] inputs = { new Double(1.0, 0.0), new Double(0.707106781, 0.707106781), new Double(0.0, 1.0), new Double(-2.0, 0.0), new Double(0.0, -4.0) };

    double[] angles = { 0.0, Math.PI / 4, Math.PI / 2, Math.PI, -Math.PI / 2 };

    double[] norms = { 1.0, 1.0, 1.0, 2.0, 4.0 };

    Double[] normed = { new Double(1.0, 0.0), new Double(0.707106781, 0.707106781), new Double(0.0, 1.0), new Double(-1.0, 0.0), new Double(0.0, -1.0) };
