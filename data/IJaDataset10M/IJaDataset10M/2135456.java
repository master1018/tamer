...
public class RecordReloadHandler extends AbstractLocatableObject implements IEventHandler {
    ...
    public void init(ICacheProvider provider, IObjectRegistry registry) {
        ...
        if (stub == null) throw BuiltinExceptionFactory.createInstanceNotFoundException(this, IMessageStub.class, this.getClass().getName());
        IConsumer consumer = stub.getConsumer(topic);
        if (!(consumer instanceof INoticerConsumer)) throw BuiltinExceptionFactory.createInstanceTypeWrongException(this.getOriginSource(), INoticerConsumer.class, IConsumer.class);
        ...
        if (dsFactory == null) throw BuiltinExceptionFactory.createInstanceNotFoundException(this, DatabaseServiceFactory.class, this.getClass().getName());
        if (operation == null) {
        ...
        if (procedureManager == null) throw BuiltinExceptionFactory.createInstanceNotFoundException(this, IProcedureManager.class, this.getClass().getName());
        serviceFactory = (IServiceFactory) registry.getInstanceOfType(IServiceFactory.class);
        if (serviceFactory == null) throw BuiltinExceptionFactory.createInstanceNotFoundException(this, IServiceFactory.class, this.getClass().getName());
    }
    ...
    public void delete(IMessage message) throws Exception {
        ...
        if (cache == null) throw new IllegalArgumentException("Can't find cache instance from provier:" + provider.getCacheName());
            ...
            if (ICacheProvider.VALUE_TYPE.value.name().equals(type)) {
                ...
                if (childs == null) {
                    ...
                    for (Object child : data.getChilds()) {
                        ...
                        if (new_values == null) throw new IllegalArgumentException("Value type is 'valueSet', please group by the data first!");
                ...
                if (childs == null) {
                    ...
                    for (Object child : data.getChilds()) {
                        ...
                        String key = TextParser.parse(provider.getKey(), record);
                        List new_values = record.getChilds();
                        if (new_values == null) throw new IllegalArgumentException("Value type is 'recordSet', please group by the data first!");
                        List<String> value_list = new LinkedList<String>();
                        cache.setValue(key, value_list);
    ...
    public CompositeMap queryBM(String bm_name, CompositeMap queryMap) throws Exception {
        ...
        try {
            ...
            if (context == null) context = new CompositeMap();
    ...
    protected void executeProc(String procedure, CompositeMap context) {
        ...
        try {
            ...
            if (context != null) ServiceInvoker.invokeProcedureWithTransaction(name, proc, serviceFactory, context); else {
    ...
    public void refresh(IMessage message, String operation) throws Exception {
        ...
        String messageKey = TextParser.parse(provider.getKey(), message.getProperties());
        if (reloadBM != null) {
            if (IEventHandler.OPERATIONS.update.name().equals(operation)) cache.remove(messageKey);
            executeBM(message);
        } else if (reloadProc != null) {
            if (IEventHandler.OPERATIONS.update.name().equals(operation)) cache.remove(messageKey);
