...
public class ControlPanelFrame extends JFrame {
    ...
    private GUIButton openLSMButton = new GUIButton(" Open LSM ", "images/fileopen.png", "Opens a LSM image, image stack or a batch file");
    ...
    private GUIButton macroButton = new GUIButton(" Install M&Ms ", "images/macro.png", "Install Magic Montage Macros ");
    ...
    public void initializeGUI() {
        ...
        if (isPluginInstalled("HyperVolume_Browser") || isImage5DInstalled()) {
            ...
            if (!isPluginInstalled("HyperVolume_Browser")) hyperVolumeItem.setEnabled(false);
    ...
    private void addMacroButtonListener(final JButton button, final JFrame parent) {
        button.addActionListener(new ActionListener() {
    ...
    private void addShowHideInfolistener(final JButton button, final JFrame parent) {
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (infoFrame.isShowing() == false) infoFrame.setVisible(true); else infoFrame.setVisible(false);
            }
        });
    }
    ...
    private void addApplyZStampListener(final JMenuItem item, final JFrame parent) {
        item.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                SelectImageDialog id = new SelectImageDialog(parent, masterModel, "Select an lsm image to apply Z stamps to", false, MasterModel.DEPTH);
    ...
    private void addApplyTStampListener(final JMenuItem item, final JFrame parent) {
        item.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                SelectImageDialog id = new SelectImageDialog(parent, masterModel, "Select an lsm image to apply time stamps to", false, MasterModel.TIME);
                int returnVal = id.showDialog();
                if (returnVal == SelectImageDialog.OK_OPTION) {
                    final int[] imageVals = id.getSelected();
                    if (imageVals == null & imageVals.length > 0) {
                    ...
                    for (int i = 0; i < imageVals.length; i++) {
                        ...
                        if (choice.equals("Dump to textfile")) {
                            String twstr = new String("");
                            for (int k = 0; k < cz.timeStamps.NumberTimeStamps; k++) twstr = twstr + Double.toString(cz.timeStamps.TimeStamps[k]) + "\n";
    ...
    private void addApplyLambdaStampListener(final JMenuItem item, final JFrame parent) {
        item.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                SelectImageDialog id = new SelectImageDialog(parent, masterModel, "Select an lsm image to apply lambda stamps to", false, MasterModel.CHANNEL);
                int returnVal = id.showDialog();
                if (returnVal == SelectImageDialog.OK_OPTION) {
                    final int[] imageVals = id.getSelected();
                    if (imageVals == null & imageVals.length > 0) {
                    ...
                    for (int i = 0; i < imageVals.length; i++) {
                        ...
                        if (choice.equals("Dump to textfile")) {
                            String twstr = new String("");
                            for (int k = 0; k < cz.channelWavelength.Channels; k++) twstr = twstr + Double.toString(cz.channelWavelength.LambdaStamps[k]) + "\n";
    ...
    private void addHyperVolumeBrowseListener(final JMenuItem item, final JFrame parent) {
        item.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                SelectImageDialog id = new SelectImageDialog(parent, masterModel, "Select one or more channels to open with HyperVolume_Browser", true);
                int returnVal = id.showDialog();
                if (returnVal == SelectImageDialog.OK_OPTION) {
                    ...
                    for (int i = 0; i < imageVals.length; i++) {
                        ...
                        if (fi != null && fi instanceof LSMFileInfo) {
                            ...
                            CZLSMInfoExtended cz = (CZLSMInfoExtended) ((ImageDirectory) lsm.imageDirectories.get(0)).TIF_CZ_LSMINFO;
                            long depth = (long) cz.DimensionZ;
                            IJ.selectWindow(imageVals[i]);
                            IJ.runPlugIn("HyperVolume_Browser", "3rd=z depth=" + depth + " 4th=t");
                        }
                    }
                }
            }
    ...
    public void applyZSTAMP(ImagePlus imp, LSMFileInfo lfi) {
        ...
        CZLSMInfoExtended cz = (CZLSMInfoExtended) ((ImageDirectory) lfi.imageDirectories.get(0)).TIF_CZ_LSMINFO;
        if (cz.DimensionZ != 1) {
            Recording r = (Recording) cz.scanInfo.recordings.get(0);
            double planeSpacing = ((Double) r.records.get("PLANE_SPACING")).doubleValue();
            int stackPosition = 1;
    ...
    public void applyTSTAMP(ImagePlus imp, LSMFileInfo lfi) {
        ...
        if (cz.DimensionTime > 1) {
            int stackPosition = 1;
            for (int i = 1; i <= cz.DimensionTime; i++) for (int j = 1; j <= cz.DimensionZ; j++) for (int k = 1; k <= cz.DimensionChannels; k++) {
                if (stackPosition <= imp.getStackSize()) {
                    IJ.showStatus("MinMax: " + stackPosition + "/" + cz.timeStamps.NumberTimeStamps);
                    String s = IJ.d2s(cz.timeStamps.TimeStamps[i - 1], 2) + " s";
                    ip = stack.getProcessor(stackPosition++);
                    ip.setFont(font);
                    float[] hsb = Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), null);
                    ip.setColor(Color.getHSBColor(255, 255, 255 - hsb[2]));
