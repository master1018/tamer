...
public class RawSqlService implements IConfigurable {
    ...
    void setSqlString(String sql) {
        if (mSql == null) mSql = new StringBuffer(sql); else {
            mSql.setLength(0);
            mSql.append(sql);
        }
    }
    ...
    public void parseParameter(SqlServiceContext context) throws ValidationException {
        ...
        if (!parsed) {
            ...
            List desc = null;
            if (mParamList != null) desc = parser.parse(parameter, new ParameterListIterator(mParamList.iterator()));
            if (desc != null) {
    ...
    ParsedSql createStatement(String sql) {
        ...
        if (mParamList != null) stmt.defineParameters(mParamList);
        stmt.parse(sql);
        return stmt;
    }
    ...
    public void query(SqlServiceContext context, IResultSetConsumer consumer, FetchDescriptor desc) throws Exception {
        ...
        try {
            ...
            if (rs != null) {
                ...
                } else mRsLoader.loadByResultSet(rs, desc, consumer);
            }
