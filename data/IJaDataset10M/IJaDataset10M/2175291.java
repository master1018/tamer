...
public class Logical2PhysicalAction extends SelectionAction {
    ...
    public void run() {
        if (!MessageDialog.openConfirm(getWorkbenchPart().getSite().getShell(), DBPlugin.getResourceString("dialog.confirm.title"), DBPlugin.getResourceString("action.logical2physical.confirm"))) {
            return;
        }
