...
public class MemTreeTagNode implements TreeTagNode {
    ...
    public MemTreeTagNode(String id, Object tag, TagManager tagManager) {
        this.id = id;
        this.tag = tag;
        if (tagManager != null) this.tagManager = tagManager; else this.tagManager = new MemTreeTagManager(null);
        order = new Vector();
    ...
    public boolean recursivelyRemoveChild(MutableTreeNode mtn) {
        ...
        while (en.hasMoreElements()) if (((TreeTagNode) en.nextElement()).recursivelyRemoveChild(mtn)) return true;
        return false;
    }
    ...
    public void setChildHidden(String id, boolean hidden) {
        if (getChild(id) == null) return;
        if (hidden) hideIDs.add(id); else hideIDs.remove(id);
    }

    public boolean getChildHidden(String id) {
        if (getChild(id) == null) return false;
    ...
    public void getChildren(Vector collect, int visible, Class restrictToType) {
        ...
        while (or.hasMoreElements()) {
            ...
            switch(visible) {
                case ONLY_VISIBLE:
                    if (hideIDs.contains(ttn.getURI()) || !tagManager.getTagVisible(ttn.getTag())) continue;
                    break;
                case ONLY_INVISIBLE:
                    if (!hideIDs.contains(ttn.getURI()) && tagManager.getTagVisible(ttn.getTag())) continue;
                    break;
            }
            if (restrictToType == null || restrictToType.isInstance(ttn)) collect.add(ttn);
            if (!ttn.isLeaf()) ttn.getChildren(collect, visible, restrictToType);
        }
    }
    ...
    private void refreshRecurse() {
        if (id2rn != null) return;
        id2rn = new Hashtable();
        Enumeration en = children();
        while (en.hasMoreElements()) {
            TreeTagNode ttn = (TreeTagNode) en.nextElement();
            if (ttn.getAllowsChildren()) id2rn.put(ttn.getURI(), ttn);
        }
    }
    ...
    public void IDSet(Set collect) {
        ...
        while (grp.hasMoreElements()) ((GroupLayout) grp.nextElement()).IDSet(collect);
    }
