 ...
 * @version  $Revision: 1930 $ $Date: 2011-04-28 13:06:17 -0400 (Thu, 28 Apr 2011) $
 */
public abstract class AbstractDictionaryRule implements Rule {
   ...
   */
    ...
    public RuleResult validate(final PasswordData passwordData) {
        ...
        if (matchBackwards) {
            text = new StringBuilder(passwordData.getPassword().getText()).reverse().toString();
            matchingWord = doWordSearch(text);
            if (matchingWord != null) {
                result.setValid(false);
                result.getDetails().add(new RuleResultDetail(ERROR_CODE_REVERSED, createRuleResultDetailParameters(matchingWord)));
            }
        }
        return result;
    }
   ...
   */
    ...
    public String toString() {
        return String.format("%s@%h::dictionary=%s,matchBackwards=%s", getClass().getName(), hashCode(), dictionary, matchBackwards);
    }
}
