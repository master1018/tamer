package blue.soundObject.renderer;
...
import blue.ProgramOptions;
import blue.SoundLayer;
import blue.score.SoundObjectView;
import blue.soundObject.AudioFile;
import blue.soundObject.SoundObject;
import blue.soundObject.renderer.audioFile.AudioWaveformCache;
import blue.soundObject.renderer.audioFile.AudioWaveformData;
import blue.soundObject.renderer.audioFile.AudioWaveformListener;
...
public class AudioFileRenderer implements BarRenderer {
    ...
    private void paintWaveform(Graphics2D g, SoundObjectView sObjView, int pixelSeconds) {
        ...
        if (waveData == null) {
            waveData = waveCache.getAudioWaveformData(audioFilename, pixelSeconds);
    ...
    private void paintWaveForm(Graphics2D g, int sObjVisibleHeight, AudioWaveformData waveForm) {
        ...
        for (int j = 0; j < waveForm.data.length; j++) {
            ...
            for (int i = startX; i < endX; i++) {
                ...
                int y1 = (int) (middleZero - (waveForm.data[j][index] * middleZero)) + yAdjust;
                int y2 = (int) (middleZero - (waveForm.data[j][index + 1] * middleZero)) + yAdjust;
                g.drawLine(i, y1, i, y2);
            }
        }
    }

    public void removeNotify(SoundObjectView sObjView) {
        AudioWaveformData waveData = (AudioWaveformData) sObjView.getClientProperty(AUDIO_WAVEFORM_DATA);
        if (waveData != null) {
            waveCache.removeReference(waveData);
        }
