...
public class pte extends PrintStream {
    ...
    public pte(PrintStream first, PrintStream second) {
        super(first);
        if (null == first) throw new IllegalArgumentException("Constructing `pte(2)' with one stream?"); else this.fir = first;
        if (null == second) throw new IllegalArgumentException("Constructing `pte(2)' with one stream?"); else this.sec = second;
    }
    ...
    public pte(pte first, PrintStream third) {
        ...
        if (null == third) throw new IllegalArgumentException("Constructing `pte(3)' with two streams?"); else this.thi = third;
    }

    public PrintStream pteOrig() {
        if (this.fir instanceof pte) return ((pte) this.fir).pteOrig(); else return fir;
    }

    public boolean pteFull() {
        return (null != this.thi);
    }

    public boolean pteEmpty() {
        return (null == this.sec);
    }

    protected void pteAdd(PrintStream ps) {
        if (null != ps) {
            if (null == this.sec) this.sec = ps; else if (null == this.thi) this.thi = ps; else if (this.thi instanceof pte) ((pte) this.thi).pteAdd(ps); else {
    ...
    protected void pteRemove(PrintStream ps) {
        if (null != ps) {
            if (ps == this.thi) this.thi = null; else if (ps == this.sec) {
                if (null != this.thi) {
                ...
                if (null != this.thi) {
                    ...
                    this.sec = null;
                    this.fir = this.sec;
                } else throw new IllegalStateException("Removing root from pte.");
    ...
    public void write(int b) {
        fir.write(b);
        if (null != sec) {
            if (this.debugCopyTrace) trace();
            sec.write(b);
        }
        if (null != thi) thi.write(b);
    }
    ...
    public void write(byte[] b, int ofs, int len) {
        ...
        if (null != thi) thi.write(b, ofs, len);
    }
    ...
    public void flush() {
        ...
        if (null != thi) thi.flush();
    }

    public void close() {
        if (null != fir) fir.close();
        if (null != sec) sec.close();
        if (null != thi) thi.close();
    }
    ...
    public boolean checkError() {
        ...
        if (null != sec) {
            ...
            if (null != thi) return (thi.checkError() || err2 || err1); else return (err2 || err1);
        } else return err1;
    }
