...
public class L2Instruction implements Sizeable {
    ...
    public enum L2InstructionSet {
        ...
        , /** The instruction to set a register as a variable. */
        SetVar(SET_VAR, "set_var", 3), /** The instruction to set a register to a heap location. */
        SetVal(SET_VAL, "set_val", 3), /** The instruction to compare a register to a structure on the heap. */
        ...
        , /** The instruction to unify a register with a variable. */
        UnifyVar(UNIFY_VAR, "unify_var", 3), /** The instruction to unify a register with a location on the heap. */
        UnifyVal(UNIFY_VAL, "unify_val", 3), /** The instruction to copy a heap location into an argument register. */
        ...
        Call(CALL, "call", 5) {
             ...
             */
            public void emmitCode(L2Instruction instruction, int ip, byte[] codeBuf, L2Machine machine) throws LinkageException {
         ...
         * @param machine     The binary machine to write the code into.
         */
        private static void emmitCodeReg1(byte[] codeBuf, byte code, int ip, L2Instruction instruction, L2Machine machine) {
            codeBuf[ip] = code;
            codeBuf[ip + 1] = instruction.mode1;
            codeBuf[ip + 2] = instruction.reg1;
        }
         ...
         * @param machine     The binary machine to write the code into.
         */
        private static void emmitCodeReg1Reg2(byte[] codeBuf, byte code, int ip, L2Instruction instruction, L2Machine machine) {
            codeBuf[ip] = code;
            codeBuf[ip + 1] = instruction.mode1;
            codeBuf[ip + 2] = instruction.reg1;
         ...
         * @param machine     The binary machine to write the code into.
         */
        private static void emmitCodeReg1Fn(byte[] codeBuf, byte code, int ip, L2Instruction instruction, L2Machine machine) {
            codeBuf[ip] = code;
            codeBuf[ip + 1] = instruction.mode1;
            codeBuf[ip + 2] = instruction.reg1;
        ...
        private static String toStringReg1Fn(String pretty, L2Instruction instruction) {
            return pretty + " X" + instruction.reg1 + ", " + ((instruction.fn != null) ? (instruction.fn.getName() + "/" + instruction.fn.getArity()) : "");
        }
