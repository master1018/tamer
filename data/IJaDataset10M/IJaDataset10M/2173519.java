...
public class SAXUnmarshallerHandlerImpl implements SAXUnmarshallerHandler, UnmarshallingContext {
    ...
    public void startElement(String uri, String local, String qname, Attributes atts) throws SAXException {
        ...
        if (result == null) {
            ...
            if (unmarshaller == null) {
                throw new SAXParseException(Messages.format(Messages.UNEXPECTED_ROOT_ELEMENT2, uri, local, computeExpectedRootElements()), getLocator());
            }
    ...
    public Object getResult() throws UnmarshalException {
        if (isUnmarshalInProgress) throw new IllegalStateException();
    ...
    protected void consumeText(String str, boolean ignorable) throws SAXException {
        if (ignorable && str.trim().length() == 0) return;
        getCurrentHandler().text(str);
    }

    private void processText(boolean ignorable) throws SAXException {
        if (shouldCollectText()) consumeText(buffer.toString(), ignorable);
        if (buffer.length() < 1024) buffer.setLength(0); else buffer = new StringBuffer();
    }

    public final void characters(char[] buf, int start, int len) {
        if (shouldCollectText()) buffer.append(buf, start, len);
    }
    ...
    public String resolveNamespacePrefix(String prefix) {
        ...
        for (int i = idxStack[stackTop] - 2; i >= 0; i -= 2) {
            if (prefix.equals(nsBind[i])) return nsBind[i + 1];
        }
        return null;
    }
    ...
    private String[] getPrefixList(int startIndex) {
        ...
        for (int i = 0; i < r.length; i++) r[i] = nsBind[startIndex + i * 2];
        return r;
    }
    ...
    private List getAllPrefixesInList(String uri) {
        ...
        for (int i = nsLen - 2; i >= 0; i -= 2) if (uri.equals(nsBind[i + 1])) if (getNamespaceURI(nsBind[i]).equals(nsBind[i + 1])) a.add(nsBind[i]);
        return a;
    }

    public String getPrefix(String uri) {
        if (uri.equals(XMLConstants.XML_NS_URI)) return XMLConstants.XML_NS_PREFIX;
        if (uri.equals(XMLConstants.XMLNS_ATTRIBUTE_NS_URI)) return XMLConstants.XMLNS_ATTRIBUTE;
        if (uri == null) throw new IllegalArgumentException();
        for (int i = idxStack[stackTop] - 2; i >= 0; i -= 2) if (uri.equals(nsBind[i + 1])) if (getNamespaceURI(nsBind[i]).equals(nsBind[i + 1])) return nsBind[i];
        return null;
    }
    ...
    public void pushAttributes(Attributes atts, boolean collectTextFlag) {
        ...
        if (a == null) attStack[stackTop] = a = new AttributesImpl(); else a.clear();
    ...
    public void addPatcher(Runnable job) {
        if (patchers == null) patchers = new Runnable[32];
    ...
    public void handleEvent(ValidationEvent event, boolean canRecover) throws SAXException {
        ...
        if (!canRecover || !recover) throw new SAXException(new UnmarshalException(event.getMessage(), event.getLinkedException()));
    }
    ...
    public Tracer getTracer() {
        if (tracer == null) tracer = new Tracer.Standard();
        return tracer;
    }
