 ...
 *
 */
final class OPT_DeadStoreElimination extends OPT_OptimizationPlanCompositeElement {

    /**
   * Construct this phase as a composite of several primitive pases.
   */
    OPT_DeadStoreElimination() {
        super("Dead StoreElimination", new OPT_OptimizationPlanElement[] { new OPT_OptimizationPlanAtomicElement(new StoreEliminationPreparation()), new OPT_OptimizationPlanAtomicElement(new OPT_EnterSSA()), new OPT_OptimizationPlanAtomicElement(new OPT_GlobalValueNumber()), new OPT_OptimizationPlanAtomicElement(new StoreElimination()) });
    }
   ...
   */
    private static final class StoreElimination extends OPT_CompilerPhase implements OPT_Operators {
     ...
     */
        private final void eliminateDeadStores(OPT_IR ir, OPT_DF_Solution solution) {
            OPT_SSADictionary ssa = ir.HIRInfo.SSADictionary;
            OPT_GlobalValueNumberState valueNumbers = ir.HIRInfo.valueNumbers;
            for (OPT_Instruction s = ir.firstInstructionInCodeOrder(), nextS = null; s != null; s = nextS) {
                nextS = s.nextInstructionInCodeOrder();
                if (!PutField.conforms(s) && !PutStatic.conforms(s) && !AStore.conforms(s)) continue;
                if (!ssa.defsHeapVariable(s)) continue;
                if (ssa.getNumberOfHeapDefs(s) != 1) continue;
                if (s.isPEI()) continue;
                if (PutField.conforms(s) || PutStatic.conforms(s)) {
                    ...
                    OPT_ObjectPair key = new OPT_ObjectPair(A[0].getHeapVariable(), OPT_DeadStoreSystem.DEAD_DEF);
                    OPT_ValueNumberSetCell cell = (OPT_ValueNumberSetCell) solution.lookup(key);
                    if (cell.contains(v)) {
                        if (DEBUG) System.out.println("ELIMINATING STORE: " + s);
                        s.remove();
                    }
                } else {
                    OPT_HeapOperand A[] = ssa.getHeapDefs(s);
                    ...
                    OPT_ObjectPair key = new OPT_ObjectPair(A[0].getHeapVariable(), OPT_DeadStoreSystem.DEAD_DEF);
                    OPT_ValueNumberPairSetCell cell = (OPT_ValueNumberPairSetCell) solution.lookup(key);
 ...
 * dead store elimination
 */
class OPT_DeadStoreSystem extends OPT_DF_System implements OPT_Operators {
    ...
    private void setupEquations() throws OPT_TooManyEquationsException {
        for (Enumeration e = ir.getBasicBlocks(); e.hasMoreElements(); ) {
            ...
            for (Enumeration e2 = ssa.getAllInstructions(bb); e2.hasMoreElements(); ) {
                ...
                if (!ssa.usesHeapVariable(s) && !ssa.defsHeapVariable(s)) continue;
                if (s.isDynamicLinkingPoint()) processCall(s); else if (GetField.conforms(s)) processNonKiller(s); else if (GetStatic.conforms(s)) processNonKiller(s); else if (New.conforms(s)) processNonKiller(s); else if (NewArray.conforms(s)) processNonKiller(s); else if (s.operator() == ARRAYLENGTH) processNonKiller(s); else if (ALoad.conforms(s)) processNonKiller(s); else if (PutStatic.conforms(s)) processStore(s); else if (PutField.conforms(s)) processStore(s); else if (AStore.conforms(s)) processAStore(s); else if (Call.conforms(s)) processCall(s); else if (CallSpecial.conforms(s)) processCall(s); else if (MonitorOp.conforms(s)) processCall(s); else if (Prepare.conforms(s)) processCall(s); else if (Attempt.conforms(s)) processCall(s); else if (CacheOp.conforms(s)) processCall(s); else if (Phi.conforms(s)) processPhi(s); else if (Return.conforms(s)) processExit(s); else if (s.isPEI()) processExit(s); else if (s.operator() == READ_CEILING) processCall(s); else if (s.operator() == WRITE_FLOOR) processCall(s); else throw new OPT_OptimizingCompilerException("DeadStoreElimination: unexpected operator " + s);
                if (getNumberOfEquations() > ir.options.MAX_DEADSTORE_SYSTEM_SIZE) throw new OPT_TooManyEquationsException();
            }
        }
    }
   ...
   */
    private void processNonKiller(OPT_Instruction s) {
        ...
        if ((A2 != null) && (A2.length > 1)) {
            throw new OPT_OptimizingCompilerException("OPT_DeadStoreSystem.processNonKiller: instruction defs multiple heap variables? " + s);
        }
        if (A2 != null) {
            setKill(A2[0], null);
            computeLive(A2[0]);
   ...
   */
    private void processStore(OPT_Instruction s) {
        ...
        if (A2.length != 1) {
            throw new OPT_OptimizingCompilerException("OPT_DeadStoreSystem.processStore: store instruction defs multiple heap variables?" + s);
        }
   ...
   */
    private void processAStore(OPT_Instruction s) {
        ...
        if (A2.length != 1) {
            throw new OPT_OptimizingCompilerException("OPT_DeadStoreSystem.processAStore: astore instruction defs multiple heap variables?" + s);
        }
        OPT_Operand array = AStore.getArray(s);
        OPT_Operand index = AStore.getIndex(s);
   ...
   */
    private void processPhi(OPT_Instruction s) {
        ...
        if (!(result instanceof OPT_HeapOperand)) return;
   ...
   */
    private void computeLive(OPT_HeapOperand A) {
        ...
        for (java.util.Iterator i = ssa.iterateHeapUses(A.getHeapVariable()); i.hasNext(); ) {
            OPT_HeapOperand H = (OPT_HeapOperand) i.next();
            OPT_Instruction s = H.instruction;
   ...
   */
    private void addToLiveSet(OPT_HeapVariable A, int v) {
        ...
        if (s == null) s = new OPT_ValueNumberSetCell(key);
        s.add(v);
        setLive(A, s);
    }
   ...
   */
    private void setLiveUniversal(OPT_HeapVariable A) {
        if (A.getHeapType() instanceof VM_Type) {
            ...
            OPT_ObjectPair key = new OPT_ObjectPair(A, LIVE_KEY);
            if (s == null) s = new OPT_ValueNumberPairSetCell(key);
            setUniversal(s);
            setLive(A, s);
        } else {
            OPT_ValueNumberSetCell s = (OPT_ValueNumberSetCell) getLive(A);
   ...
   */
    private void addDeadUseEquation(OPT_HeapOperand A0, OPT_HeapOperand A1, OPT_HeapOperand A2) {
   ...
   */
     ...
     */
        boolean evaluateForFields(OPT_DF_LatticeCell l, OPT_DF_LatticeCell r) {
            ...
            boolean changed = OPT_ValueNumberSetCell.setsDiffer(oldNumbers, newNumbers);
            return changed;
        }
     ...
     */
        boolean evaluateForArrays(OPT_DF_LatticeCell l, OPT_DF_LatticeCell r) {
            ...
            boolean changed = OPT_ValueNumberPairSetCell.setsDiffer(oldNumbers, newNumbers);
            return changed;
        }
    }
   ...
   */
     ...
     */
        boolean evaluateForFields(OPT_DF_LatticeCell[] operands) {
            ...
            OPT_ValueNumberSetCell rhs[] = new OPT_ValueNumberSetCell[operands.length - 1];
     ...
     */
        boolean evaluateForArrays(OPT_DF_LatticeCell[] operands) {
            ...
            OPT_ValueNumberPairSetCell rhs[] = new OPT_ValueNumberPairSetCell[operands.length - 1];
            for (int i = 1; i < operands.length; i++) {
            ...
            OPT_ValueNumberPairSetCell LIVE = (OPT_ValueNumberPairSetCell) getLive(liveH);
            if (LIVE != null) {
                OPT_ValueNumberPair[] numbers = lhs.getValueNumbers();
                for (int i = 0; i < numbers.length; i++) {
                    int n1 = numbers[i].v1;
                    int n2 = numbers[i].v2;
                    OPT_ValueNumberPair[] live = ((OPT_ValueNumberPairSetCell) LIVE).getValueNumbers();
                    for (int j = 0; j < live.length; j++) {
                        int v1 = live[j].v1;
                        int v2 = live[j].v2;
...
class OPT_ObjectPair {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof OPT_ObjectPair)) return false;
