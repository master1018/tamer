 ...
 */
    public static String translateNFSPath(String nfspath) throws FileNotFoundException {
        ...
        path = nfspath.substring(vfsroot.getPath().length());
 ...
 */
    ...
    public static String translateVFSPath(String vfspath, String vfscwd) throws FileNotFoundException {
        ...
        try {
            if (!vfspath.startsWith("/")) {
                String path = (((vfscwd == null) || vfscwd.trim().equals("")) ? getVFSHomeDirectory(SshThread.getCurrentThreadUser()) : vfscwd);
            ...
            if (vfspath.startsWith(vfshome)) {
                if (vfspath.length() > vfshome.length()) {
                    return translateCanonicalPath(nfshome + (nfshome.endsWith("/") ? "" : "/") + vfspath.substring(vfshome.length() + 1), nfshome);
                } else {
        ...
        if (vfsroot != null) {
            path = vfsroot.getPath() + (vfsroot.getPath().endsWith("/") ? vfspath.substring(1) : vfspath);
 ...
 */
    public static String translateCanonicalPath(String path, String securemount) throws FileNotFoundException {
        try {
            log.debug("Translating for canonical path " + path + " against secure mount " + securemount);
 ...
 */
    public FileAttributes getFileAttributes(byte[] handle) throws IOException, InvalidHandleException {
        ...
        if (openFiles.containsKey(shandle)) {
            ...
            attrs.setTimes(new UnsignedInteger32(f.lastModified() / 1000), new UnsignedInteger32(f.lastModified() / 1000));
            ...
            attrs.setPermissions((((f.canRead() && permissions.canRead()) ? "r" : "-") + ((f.canWrite() && permissions.canWrite()) ? "w" : "-") + ((canExec && permissions.canExecute()) ? "x" : "-")));
            attrs.setPermissions(new UnsignedInteger32(attrs.getPermissions().longValue() | (f.isDirectory() ? FileAttributes.S_IFDIR : FileAttributes.S_IFREG)));
            return attrs;
        } else {
    ...
    public byte[] openDirectory(String path) throws PermissionDeniedException, FileNotFoundException, IOException {
        ...
        if (f.exists()) {
            if (f.isDirectory()) {
                ...
                throw new IOException(translateNFSPath(path) + " is not a directory");
            }
        } else {
            ...
            if (obj instanceof OpenDirectory) {
                ...
                if (count > 0) {
                    ...
                    for (int i = 0; i < files.length; i++) {
                        ...
                        SftpFile sftpfile = new SftpFile(f.getName(), getFileAttributes(absolutePath));
                        files[i] = sftpfile;
                    }
 ...
 */
    public byte[] openFile(String path, UnsignedInteger32 flags, FileAttributes attrs) throws PermissionDeniedException, FileNotFoundException, IOException {
        ...
        if (!f.exists()) {
            ...
            if (((flags.intValue() & NativeFileSystemProvider.OPEN_CREATE) == NativeFileSystemProvider.OPEN_CREATE) && ((flags.intValue() & NativeFileSystemProvider.OPEN_EXCLUSIVE) == NativeFileSystemProvider.OPEN_EXCLUSIVE)) {
                throw new IOException(translateNFSPath(path) + " already exists");
            }
        }
        String mode = "r" + (((flags.intValue() & NativeFileSystemProvider.OPEN_WRITE) == NativeFileSystemProvider.OPEN_WRITE) ? "ws" : "");
        RandomAccessFile raf = new RandomAccessFile(f, mode);
        if (((flags.intValue() & NativeFileSystemProvider.OPEN_CREATE) == NativeFileSystemProvider.OPEN_CREATE) && ((flags.intValue() & NativeFileSystemProvider.OPEN_TRUNCATE) == NativeFileSystemProvider.OPEN_TRUNCATE)) {
            raf.setLength(0);
        }
    ...
    public byte[] readFile(byte[] handle, UnsignedInteger64 offset, UnsignedInteger32 len) throws InvalidHandleException, EOFException, IOException {
        ...
        if (openFiles.containsKey(shandle)) {
            ...
            if (obj instanceof OpenFile) {
                ...
                if ((file.getFlags().intValue() & NativeFileSystemProvider.OPEN_READ) == NativeFileSystemProvider.OPEN_READ) {
 ...
 */
    public void writeFile(byte[] handle, UnsignedInteger64 offset, byte[] data, int off, int len) throws InvalidHandleException, IOException {
        ...
        if (openFiles.containsKey(shandle)) {
            ...
            if (obj instanceof OpenFile) {
                OpenFile file = (OpenFile) obj;
                if ((file.getFlags().intValue() & NativeFileSystemProvider.OPEN_WRITE) == NativeFileSystemProvider.OPEN_WRITE) {
                    if ((file.getFlags().intValue() & NativeFileSystemProvider.OPEN_APPEND) == NativeFileSystemProvider.OPEN_APPEND) {
    ...
    public void removeFile(String path) throws PermissionDeniedException, IOException, FileNotFoundException {
        ...
        if (f.exists()) {
            try {
                if (f.isFile()) {
                    if (!f.delete()) {
                        throw new IOException("Failed to delete " + translateNFSPath(path));
                    }
                } else {
                    throw new IOException(translateNFSPath(path) + " is a directory, use remove directory command to remove");
                }
 ...
 */
    public void renameFile(String oldpath, String newpath) throws PermissionDeniedException, FileNotFoundException, IOException {
        ...
        if (f.exists()) {
            ...
            if (!f2.exists()) {
                if (!f.renameTo(f2)) {
                    throw new IOException("Failed to rename file " + translateNFSPath(oldpath));
                }
            } else {
                throw new IOException(translateNFSPath(newpath) + " already exists");
            }
        } else {
            throw new FileNotFoundException(translateNFSPath(oldpath) + " does not exist");
        }
    }
        ...
        if (f.isDirectory()) {
            if (f.exists()) {
                if (f.listFiles().length == 0) {
                    if (!f.delete()) {
                        throw new IOException("Failed to remove directory " + translateNFSPath(path));
                    }
                } else {
                    throw new IOException(translateNFSPath(path) + " is not an empty directory");
                }
            } else {
                throw new FileNotFoundException(translateNFSPath(path) + " does not exist");
            }
    ...
    public void verifyPermissions(String username, String path, String permissions) throws PermissionDeniedException, FileNotFoundException, IOException {
        ...
        if (permissionHandler == null) {
            ...
            if (!perm.verifyPermissions(permissions)) {
                throw new PermissionDeniedException("Permission denied for " + translateNFSPath(path));
            }
        } else {
