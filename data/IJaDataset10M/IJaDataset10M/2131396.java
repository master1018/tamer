...
public class SAF extends QBeanSupport implements Runnable, Loggeable {
            ...
            try {
                if (latchMsg()) {
                    ...
                    synchronized (psp) {
                        autoCommitOff();
                        psp.inp(head);
                        if (entry != null) psp.out(head, entry);
                        autoCommitOn();
                    }
                    if (interMessageDelay > 0) ISOUtil.sleep(interMessageDelay);
                }
            } catch (Exception e) {
                getLog().error(e);
                ISOUtil.sleep(15000L);
            }
        }
    }
    ...
    public void setConfiguration(Configuration cfg) throws ConfigurationException {
        ...
        try {
            ...
            throw new ConfigurationException("MUX '" + muxName + "' not found in registrar.");
        }
    ...
    public String getStatus() {
        ...
        if (latched != null) {
            if (latched.count > 0) sb.append(" head transmitted " + latched.count + " time(s)");
        }
        return sb.toString();
    }
    ...
    private void autoCommitOn() {
        if (psp instanceof JDBMSpace) ((JDBMSpace) psp).setAutoCommit(true);
    }

    private void autoCommitOff() {
        if (psp instanceof JDBMSpace) ((JDBMSpace) psp).setAutoCommit(false);
    }

    private Entry send(Entry entry) {
        if (shouldIgnore(entry)) {
            LogEvent evt = getLog().createLogEvent("saf-warning");
            if (isMaxRetransmission(entry)) evt.addMessage("max retransmission count (" + maxRetransmissions + ") has been reached.");
            if (isExpired(entry)) {
        ...
        try {
            ...
            if (resp.hasField(39)) {
                ...
                if (retryResponseCodes != null && retryResponseCodes.indexOf(rc) >= 0) {
                    ...
                    evt.addMessage("response code '" + resp.getString(39) + "' is in retry-response-codes list (" + retryResponseCodes + ")");
                    Logger.log(evt);
                    return entry;
                }
                entry.count++;
                if ("*".equals(validResponseCodes) || validResponseCodes.indexOf(rc) >= 0) {
                    LogEvent evt = createLogEvent("info", entry, resp);
                    evt.addMessage("response code '" + resp.getString(39) + "' is in valid-response-codes list (" + validResponseCodes + ")");
                    Logger.log(evt);
                    if (entry.responseKey != null) {
                        if (entry.wipePreviousResponse) SpaceUtil.wipe(psp, entry.responseKey);
                        if (entry.responseTimeout > 0L) psp.out(entry.responseKey, resp, entry.responseTimeout); else psp.out(entry.responseKey, resp);
                    }
                    return null;
                } else {
                    LogEvent evt = createLogEvent("info", entry, resp);
                    evt.addMessage("response code '" + resp.getString(39) + "' not in valid-response-codes list (" + validResponseCodes + ")");
                    Logger.log(evt);
                }
            }
            if (entry.count == 1 && flagRetransmissions.indexOf(entry.msg.getMTI()) >= 0) entry.msg.setRetransmissionMTI();
    ...
    private boolean isExpired(Entry e) {
        return expiration == 0 ? false : System.currentTimeMillis() > (e.time + expiration);
    }

    private boolean isMaxRetransmission(Entry e) {
        return maxRetransmissions == 0 ? false : e.count > maxRetransmissions;
    }
