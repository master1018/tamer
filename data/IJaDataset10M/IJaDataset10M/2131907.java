...
public class RoseUtil {
    ...
    BasicDiagnostic basicDiagnostic = new BasicDiagnostic(RoseImporterPlugin.getPlugin().getBundle().getSymbolicName(), 0, RoseImporterPlugin.INSTANCE.getString("_UI_ProblemsWereEncounteredLoadingTheRoseModel_message"), null);
    ...
    public UnitTreeNode createRoseUnitTreeAndTable(String fileNameNodeValue, UnitTreeNode topNode) throws Exception {
        ...
        try {
            if (loader.isValid()) {
                basicDiagnostic = new BasicDiagnostic(RoseImporterPlugin.getPlugin().getBundle().getSymbolicName(), 1, RoseImporterPlugin.INSTANCE.getString("_UI_ProblemsWereEncounteredConvertingTheRoseModel_message"), null);
                ...
                if (topNode == null) {
                    ...
                    if (modelTree.getKey().equals("") && (Util.getType(modelTree.getValue())).equals(RoseStrings.CLASS_CATEGORY)) {
                        ...
                        String modelName;
                        if (ind_2 != -1) modelName = fileName.substring(ind_1 + 1, ind_2); else modelName = fileName.substring(ind_1 + 1, fileName.length());
                        String quid = modelTree.getRoseId();
                        if (quid != null) {
                            quid = quid.substring(1, quid.length() - 1);
                ...
                basicDiagnostic.add(new BasicDiagnostic(Diagnostic.INFO, RoseImporterPlugin.getPlugin().getBundle().getSymbolicName(), 0, RoseImporterPlugin.INSTANCE.getString("_UI_TheUnitResolvesTo_message", new Object[] { Util.trimQuotes(fileNameNodeValue), fileName }), null));
                return null;
            }
        } finally {
            loader.close();
    ...
    public void checkConflictFileName(UnitTreeNode unitTree) {
        ...
        if (checkFileName(unitTree, rootEcoreFileName)) {
            ...
            if (index != -1) {
                rootEcoreFileName = rootEcoreFileName.substring(0, index) + "model" + rootEcoreFileName.substring(index, rootEcoreFileName.length());
            }
