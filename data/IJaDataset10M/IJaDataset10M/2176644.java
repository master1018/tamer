...
public class UpdateParametersAction extends YAWLSelectedNetAction implements TooltipTogglingWidget {
   ...
   */
    ...
    public String getDisabledTooltipText() {
        return " You must have task with a decomposition selected" + " to update its parameter mappings ";
    }
...
class UpdateParametersDialog extends AbstractTaskDoneDialog {
   ...
   */
    ...
    public void setScope(YAWLTask task, NetGraph graph) {
        ...
        netVariableTable.setVariableSet(graph.getNetModel().getVariableSet());
        taskVariableTable.setVariableSet(task.getVariables());
        pack();
    }
    ...
    private JPanel getNetPanel() {
        ...
        netScrollPane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);
...
abstract class ParameterUpdatePanel extends AbstractTableUpdatePanel {
    ...
    public void doCreateButtonAction() {
        ...
        updateDialog.setInputVariableScope(getParameterTable().getInputVariableScope());
        updateDialog.setOutputVariableScope(getParameterTable().getOutputVariableScope());
        updateDialog.setParameter(getParameterTable().getParameterAt(getParameterTable().getSelectedRow()));
    ...
    public void doRemoveButtonAction() {
        ...
        int selectedValue = JOptionPane.showConfirmDialog(this, "You are about to permanently delete the parameter for " + "variable \"" + getParameterTable().getVariableAt(oldSelectedRow) + "\".\n\n" + "Are you sure you want to delete it?\n", "Deleting Parameter", JOptionPane.WARNING_MESSAGE, JOptionPane.YES_NO_OPTION);
    ...
    private void refreshSelectedRow() {
        getParameterTable().updateRow(getParameterTable().getSelectedRow());
    }
}
