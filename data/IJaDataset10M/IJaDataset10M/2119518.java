...
public class QueueReader extends Reader implements Appendable {
    ...
    public synchronized void reset() {
        if (readAheadLimit > 0) pos = mark;
    }

    void resize(int len) {
        int cur_size = limit - pos;
        if (readAheadLimit > 0 && pos - mark <= readAheadLimit) cur_size = limit - mark; else mark = pos;
    ...
    protected void reserveSpace(int len) {
        if (buffer == null) buffer = new char[100 + len]; else if (buffer.length < limit + len) resize(len);
    }
