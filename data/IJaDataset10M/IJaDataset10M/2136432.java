...
public class JobTypePanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        g2D.drawString("Waiting jobs", startX + 10 + (cQueued.length * rectWidth), startY + rectHeight - 3);
        startY += incY;
        drawRectColor(g2D, cDone, rectWidth, rectHeight, startX, startY);
        g2D.setColor(Color.BLACK);
        g2D.drawString("Jobs/ARs completed", startX + 10 + (cDone.length * rectWidth), startY + rectHeight - 3);
        startY += incY;
        drawRectColor(g2D, cInExec, rectWidth, rectHeight, startX, startY);
        g2D.setColor(Color.BLACK);
        g2D.drawString("Jobs in execution", startX + 10 + (cInExec.length * rectWidth), startY + rectHeight - 3);
        startY = 10;
        startX = 15 + (int) ((float) panelWidth / 2);
        drawRectColor(g2D, cARNonComm, rectWidth, rectHeight, startX, startY);
        g2D.setColor(Color.BLACK);
        g2D.drawString("ARs not confirmed", startX + 10 + (cARNonComm.length * rectWidth), startY + rectHeight - 3);
        startY += incY;
        drawRectColor(g2D, cARComm, rectWidth, rectHeight, startX, startY);
        g2D.setColor(Color.BLACK);
        g2D.drawString("ARs confirmed", startX + 10 + (cARComm.length * rectWidth), startY + rectHeight - 3);
        startY += incY;
        drawRectColor(g2D, cARExec, rectWidth, rectHeight, startX, startY);
        g2D.setColor(Color.BLACK);
        g2D.drawString("ARs in progress", startX + 10 + (cARExec.length * rectWidth), startY + rectHeight - 3);
    }

    private void drawRectColor(Graphics2D g2D, Color[] colors, int rectWidth, int rectHeight, int startX, int startY) {
