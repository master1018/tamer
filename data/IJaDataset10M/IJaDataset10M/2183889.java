...
public class AabioDocumentMapper extends XMLDocumentMapper {
    ...
    protected void extractProperties(List<ParsedDocument> pds, Document xmlDocument) throws Exception {
        ...
        pds.remove(0);
        Pattern p = Pattern.compile("(?:<p)" + "(?: align=\"center\")?" + "(?:>[\\s]{0,}<b>[\\s]{0,})" + "([" + SCIENTIFIC_NAME_PATTERN + "\\(\\)]{1,})" + "(?:[\\s]{0,}</b>[\\s]{0,}</p>)");
        Pattern p2 = Pattern.compile("(?:<td align=\"center\" height=\"40\" valign=\"top\" width=\"212\">[\\s]{0,}<div align=\"center\">[\\s]{0,}<p>[\\s]{0,}<i>[\\s]{0,})" + "([" + SCIENTIFIC_NAME_PATTERN + "]{1,})" + "(?:[\\s]{0,}</i>[\\s]{0,}</p>[\\s]{0,}</div>[\\s]{0,}</td>[\\s]{0,}<td align=\"center\" height=\"40\" valign=\"top\" width=\"246\">[\\s]{0,}<div align=\"center\">[\\s]{0,}<p>[\\s]{0,})" + "([" + SCIENTIFIC_NAME_PATTERN + "]{1,})" + "(?:[\\s]{0,}</p>[\\s]{0,}</div>[\\s]{0,}</td>[\\s]{0,}<td align=\"center\" height=\"40\" valign=\"top\" width=\"172\">[\\s]{0,}<div align=\"center\">[\\s]{0,}<p>[\\s]{0,})?" + "([" + SCIENTIFIC_NAME_PATTERN + "]{1,})" + "(?:[\\s]{0,}</p>)?");
