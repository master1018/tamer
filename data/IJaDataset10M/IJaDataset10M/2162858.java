...
public class SRPGDelta {
    ...
    public SRPGDelta(double[][] res, double ad1[], double x, double y, double speed) {
        this(res, x, y, speed);
        this.setAverage(ad1);
    }
    ...
    public int[][] drawing(int x, int y) {
        ...
        for (int i = 0; i < delta.length; i++) {
            ...
            double d2 = Math.cos(Math.toRadians(vector + d1)) * d + avg[0] + pos[0] + (double) x;
            double d3 = Math.sin(Math.toRadians(vector + d1)) * d + avg[1] + pos[1] + (double) y;
            res[i][0] = (int) (d2 + 0.5D);
            res[i][1] = (int) (d3 + 0.5D);
        }
        return res;
    ...
    public synchronized void draw(LGraphics g, int x, int y) {
        ...
        for (int i = 0; i < res.length; i++) {
            int index = (i + 1) % 3;
            g.drawLine(res[i][0], height - res[i][1], res[index][0], height - res[index][1]);
        }
    }
    ...
    private double getLine(double d, double d1) {
        return Math.sqrt(Math.pow(Math.abs(d), 2D) + Math.pow(Math.abs(d1), 2D));
    }
