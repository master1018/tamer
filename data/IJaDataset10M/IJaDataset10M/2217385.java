...
public class AssistedObjectChecker {
    ...
    public AssistedObjectChecker(AssistedObject assistedObject, String descriptorName, ObjectDesc objectDescriptor) {
    ...
    private void checkDescriptorCheckKey(String key, IneT ineT) {
        if (!objectDescriptor.containsKey(key)) throw new InvalidKeyException("Key '" + key + "' is not found in descriptor '" + descriptorName + "'");
        IneT fieldT = objectDescriptor.getField(key).getType();
        if (!fieldT.equals(ineT)) throw new InvalidKeyException("The type of field for key '" + key + "' in descriptor '" + descriptorName + "' is '" + fieldT.toString() + "' and not '" + ineT.toString() + "'");
    }
    ...
    public void copyValuesTo(AssistedObject otherKvo) {
        if (!getDescriptorName().equals(otherKvo.getDescriptorName())) throw new IllegalArgumentException();
