...
public class ConsolePlayer {
    ...
    public enum SIDEMUS {
        ...
        EMU_NONE, EMU_RESID, EMU_HARDSID
    }
    ...
    public boolean open() throws InterruptedException {
        ...
        if (driver.getDevice() instanceof CmpMP3File) {
            ((CmpMP3File) driver.getDevice()).setPlayOriginal(audio.isPlayOriginal());
        ...
        player.connectC64AndC1541WithParallelCable(getConfig().c1541().isParallelCable());
        for (int driveNum = 0; driveNum < player.getFloppies().length; driveNum++) {
            ...
            floppy.setRamExpansion(0, getConfig().c1541().isRamExpansionEnabled(0));
            floppy.setRamExpansion(1, getConfig().c1541().isRamExpansionEnabled(1));
            floppy.setRamExpansion(2, getConfig().c1541().isRamExpansionEnabled(2));
            floppy.setRamExpansion(3, getConfig().c1541().isRamExpansionEnabled(3));
            floppy.setRamExpansion(4, getConfig().c1541().isRamExpansionEnabled(4));
        }
        player.turnPrinterOnOff(getConfig().printer().isPrinterOn());
        if (tune != null) {
            SidDatabase database = SidDatabase.getInstance(iniCfg.sidplay2().getHvsc());
            if (database != null && !timer.valid && iniCfg.sidplay2().isEnableDatabase()) {
    ...
    public boolean play() throws InterruptedException {
        ...
        if (seconds != timer.current) {
            ...
            if (iniCfg.sidplay2().isEnableDatabase() && timer.stop != 0 && seconds >= timer.stop) {
                if (track.single) {
                    state = playerExit;
                } else {
                    nextSong();
    ...
    public int args(final String[] argv) {
        ...
        while (i < argv.length) {
            if (argv[i].charAt(0) == '-' && argv[i].length() > 1) {
                if (argv[i].charAt(1) == 'h' || argv[i].equals("--help")) {
                    ...
                    iniCfg.audio().setFrequency(Integer.valueOf(argv[i].substring(2)));
        ...
        {
            if (driver.output.isFileBased() && timer.valid && timer.defaultLength == 0) {
    ...
    void menu() {
        ...
        System.out.println(String.format("%c%s%c", console.getTopLeft(), setfill(console.getHorizontal(), 54), console.getTopRight()));
        System.out.println(String.format("%c%54s%c", console.getVertical(), " Java SIDPLAY - Music Player & C64 SID Chip Emulator  ", console.getVertical()));
        System.out.println(String.format("%c%s%c", console.getJunctionLeft(), setfill(console.getHorizontal(), 54), console.getJunctionRight()));
        if (tuneInfo.numberOfInfoStrings > 0) {
            if (tuneInfo.numberOfInfoStrings == 3) {
                System.out.println(String.format("%c Title        : %37s %c", console.getVertical(), tuneInfo.infoString[0], console.getVertical()));
                System.out.println(String.format("%c Author       : %37s %c", console.getVertical(), tuneInfo.infoString[1], console.getVertical()));
                System.out.println(String.format("%c Released     : %37s %c", console.getVertical(), tuneInfo.infoString[2], console.getVertical()));
            } else {
                for (int i = 0; i < tuneInfo.numberOfInfoStrings; i++) {
                    System.out.println(String.format("%c Description  : %37s %c", console.getVertical(), tuneInfo.infoString[i], console.getVertical()));
                }
            }
            System.out.println(String.format("%c%s%c", console.getJunctionLeft(), setfill(console.getHorizontal(), 54), console.getJunctionRight()));
        }
        if (verboseLevel != 0) {
            System.out.println(String.format("%c File format  : %37s %c", console.getVertical(), tuneInfo.getClass().getSimpleName(), console.getVertical()));
            System.out.println(String.format("%c Filename(s)  : %37s %c", console.getVertical(), tuneInfo.file.getName(), console.getVertical()));
        }
        System.out.print(String.format("%c Playlist     : ", console.getVertical()));
        {
            int i = 1;
            if (!track.single) {
                i = track.selected;
            ...
            System.out.println(String.format("%37s %c", i + "/" + track.songs + " (tune " + tuneInfo.currentSong + "/" + tuneInfo.songs + "[" + tuneInfo.startSong + "])" + (track.loop ? " [LOOPING]" : ""), console.getVertical()));
        }
        if (verboseLevel > 0) {
            System.out.println(String.format("%c%s%c", console.getBottomLeft(), setfill(console.getHorizontal(), 54), console.getBottomRight()));
            System.out.println(String.format("%c Song Speed   : %37s %c", console.getVertical(), tune.getSongSpeed(track.selected), console.getVertical()));
        }
        System.out.print(String.format("%c Song Length  : ", console.getVertical()));
        if (timer.stop != 0) {
            final String time = String.format("%02d:%02d", (timer.stop / 60 % 100), (timer.stop % 60));
            System.out.print(String.format("%37s %c", "" + time, console.getVertical()));
        } else if (timer.valid) {
            System.out.print(String.format("%37s %c", "FOREVER", console.getVertical()));
        } else {
            System.out.print(String.format("%37s %c", "UNKNOWN", console.getVertical()));
        }
        System.out.println();
        if (verboseLevel > 0) {
            System.out.println(String.format("%c%s%c", console.getBottomLeft(), setfill(console.getHorizontal(), 54), console.getBottomRight()));
            ...
            System.out.println(String.format("%c Addresses    : %37s %c", console.getVertical(), line.toString(), console.getVertical()));
            ...
            System.out.println(String.format("%c              : %37s %c", console.getVertical(), line.toString(), console.getVertical()));
            line = new StringBuffer();
            line.append(String.format("Filter = %s", (filterEnable ? "Yes" : "No")));
            line.append(String.format(", Model = %s", (tuneInfo.sid1Model == SidTune.Model.MOS8580 ? "8580" : "6581")));
            System.out.println(String.format("%c SID Details  : %37s %c", console.getVertical(), line.toString(), console.getVertical()));
            if (verboseLevel > 1) {
                line = new StringBuffer();
                System.out.println(String.format("%c Delay        : %37s %c", console.getVertical(), line.toString(), console.getVertical()));
            }
        }
        System.out.println(String.format("%c%s%c", console.getBottomLeft(), setfill(console.getHorizontal(), 54), console.getBottomRight()));
    ...
    public static void main(final String[] args) throws InterruptedException {
        ...
        main_restart: while (true) {
            ...
            while (true) {
                ...
                try {
                    if (player.quietLevel < 2 && (player.getState() == playerPaused || System.in.available() != 0)) {
                        player.decodeKeys();
                    }
                } catch (final IOException e) {
                    e.printStackTrace();
    ...
    public void updateSidEmulation() {
        if (sidEmuFactory != null) {
            ChipModel chipModel = determineSIDModel(iniCfg.emulation().getUserSidModel(), iniCfg.emulation().getDefaultSidModel(), tune != null ? tune.getInfo().sid1Model : null);
            updateSIDEmu(0, chipModel);
            if (driver.channels == 2) {
                ChipModel stereoChipModel = determineSIDModel(iniCfg.emulation().getStereoSidModel(), chipModel, Model.valueOf(chipModel.toString()));
	 ...
	 */
    public static ChipModel determineSIDModel(ChipModel userForcedChoice, ChipModel defaultChoice, Model model) {
    ...
    private void updateSIDEmu(final int chipNum, final ChipModel model) {
        ...
        if (s instanceof ReSID) {
            ((ReSID) s).model(model);
            ((ReSID) s).filter(iniCfg.filter(ChipModel.MOS6581), iniCfg.filter(ChipModel.MOS8580));
        }
