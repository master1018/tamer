package alice.cartago.mansyntax;
...
public class Structure extends Atom {
    ...
    public Structure(Literal l) {
        ...
        for (int i = 0; i < tss; i++) terms.add(l.getTerm(i).clone());
    }
    ...
    protected int calcHashCode() {
        ...
        for (int i = 0; i < ts; i++) result = 7 * result + getTerm(i).hashCode();
        return result;
    }
    ...
    public boolean equals(Object t) {
        ...
        if (t instanceof Structure) {
            ...
            if (!getFunctor().equals(tAsStruct.getFunctor())) return false;
            for (int i = 0; i < ts; i++) if (!getTerm(i).equals(tAsStruct.getTerm(i))) return false;
            return true;
        }
    ...
    @Override
    public int getArity() {
        if (terms == null) return 0; else return terms.size();
    }
    ...
    public boolean hasVar(VarTerm t) {
        final int size = getArity();
        for (int i = 0; i < size; i++) if (getTerm(i).hasVar(t)) return true;
        return false;
    }
    ...
    public List<VarTerm> getSingletonVars() {
        ...
        for (VarTerm k : all.keySet()) {
            if (all.get(k) == 1 && !k.isUnnamedVar()) r.add(k);
        }
        return r;
    }
    ...
    public void countVars(Map<VarTerm, Integer> c) {
        ...
        for (int i = 0; i < tss; i++) getTerm(i).countVars(c);
    }

    public String toString() {
        StringBuilder s = new StringBuilder();
        if (getFunctor() != null) s.append(getFunctor());
        if (getArity() > 0) {
            s.append('(');
