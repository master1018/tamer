...
public class LdapRdnParser implements LdapParser {
    ...
    public List getList() throws InvalidNameException {
        try {
            ...
            throw (InvalidNameException) (new InvalidNameException(Messages.getString("ldap.17") + name).initCause(e));
        }
    ...
    private void checkTypeRestrictions(String rdn) throws InvalidNameException {
        ...
        try {
            if (rdn.substring(0, in).length() == 0) {
                throw new InvalidNameException(Messages.getString("ldap.18") + rdn);
            }
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (chars[i] != '\\') {
                ...
                try {
                    ...
                    if (!isSpecialChar(chars, i + 1) && !isSpecialChar(chars, i + 2)) {
                        try {
