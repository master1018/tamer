...
class HoverInfo implements IDisplayable {
    ...
    private void checkLOS(Vector<String> out) {
        ...
        if ((los == null) && (s != null)) {
            ...
            out.add(Messages.getString("BoardView1.Attacker", new Object[] { (s != null ? s.getDisplayName() : mechInFirst ? Messages.getString("BoardView1.Mech") : Messages.getString("BoardView1.NonMech")), src.getBoardNum() }));
        }
        if (t == null) {
            out.add(Messages.getString("BoardView1.Target", new Object[] { ai.targetHeight == 1 ? Messages.getString("BoardView1.Mech") : Messages.getString("BoardView1.NonMech"), coords.getBoardNum() }));
        }
        if ((t != null) && (s != null)) {
            int i = 0;
            Mounted eq;
            ...
            while ((eq = s.getEquipment(i++)) != null) {
                if ((eq.getType() instanceof WeaponType) && !done.containsKey(eq.getName())) {
                    ToHitData toHit = WeaponAttackAction.toHit(game, s.getId(), t, s.getEquipmentNum(eq), Entity.LOC_NONE, IAimingModes.AIM_MODE_NONE);
                    out.add(eq.getType().getName() + Messages.getString("BoardView1.needs") + toHit.getValueAsString() + " " + toHit.getTableDesc() + " [" + toHit.getDesc() + "]");
            ...
            if (le.isBlocked()) {
                out.add(Messages.getString("BoardView1.LOSBlocked", new Object[] { new Integer(src.distance(coords)) }));
            } else {
                out.add(Messages.getString("BoardView1.LOSNotBlocked", new Object[] { new Integer(src.distance(coords)) }));
                if (le.getHeavyWoods() > 0) {
                    out.add(Messages.getString("BoardView1.HeavyWoods", new Object[] { new Integer(le.getHeavyWoods()) }));
                }
                if (le.getLightWoods() > 0) {
                    out.add(Messages.getString("BoardView1.LightWoods", new Object[] { new Integer(le.getLightWoods()) }));
                }
                if (le.getLightSmoke() > 0) {
                    out.add(Messages.getString("BoardView1.LightSmoke", new Object[] { new Integer(le.getLightSmoke()) }));
                }
                if (le.getHeavySmoke() > 0) {
                    if (game.getOptions().booleanOption("maxtech_fire")) {
                        out.add(Messages.getString("BoardView1.HeavySmoke", new Object[] { new Integer(le.getHeavySmoke()) }));
                    } else {
                        out.add(Messages.getString("BoardView1.Smoke", new Object[] { new Integer(le.getHeavySmoke()) }));
                    }
                }
                if (le.isTargetCover()) {
                    out.add(Messages.getString("BoardView1.TargetPartialCover"));
                }
                if (le.isAttackerCover()) {
                    out.add(Messages.getString("BoardView1.AttackerPartialCover"));
                }
            }
        }
        if (ocoords != null) {
    ...
    private Vector<String> getTipText() {
        ...
        out.add(Messages.getString("BoardView1.Hex") + coords.getBoardNum() + Messages.getString("BoardView1.level") + mhex.getElevation());
        ...
        if (mhex.containsTerrain(Terrains.RUBBLE)) {
            out.add(Messages.getString("BoardView1.Rubble"));
        }
        ...
        if (mhex.containsTerrain(Terrains.FUEL_TANK)) {
            ...
            StringBuffer buf = new StringBuffer(Messages.getString("BoardView1.Height"));
            buf.append(mhex.terrainLevel(Terrains.FUEL_TANK_ELEV));
            buf.append(" ");
            buf.append(bldg.toString());
            buf.append(Messages.getString("BoardView1.CF"));
            ...
            for (int i = 0; i < minefields.size(); i++) {
                ...
                switch(mf.getType()) {
                    case (Minefield.TYPE_CONVENTIONAL):
                        out.add(mf.getName() + Messages.getString("BoardView1.minefield") + " " + owner);
                        break;
                    ...
                    case (Minefield.TYPE_VIBRABOMB):
                        if (mf.getPlayerId() == localPlayer.getId()) {
                            out.add(mf.getName() + Messages.getString("BoardView1.minefield") + "(" + mf.getSetting() + ") " + owner);
                        } else {
                            out.add(mf.getName() + Messages.getString("BoardView1.minefield") + " " + owner);
                        }
                        break;
                    case (Minefield.TYPE_ACTIVE):
                        out.add(mf.getName() + Messages.getString("BoardView1.minefield") + "(" + mf.getDensity() + ")" + owner);
                        break;
        ...
        for (Enumeration<ArtilleryAttackAction> e = game.getArtilleryAttacks(); e.hasMoreElements(); ) {
            ...
            out.add(Messages.getString("BoardView1.ArtilleryAttack", new Object[] { s, new Integer(aaa.turnsTilHit), aaa.toHit(game).getValueAsString() }));
        }
    ...
    private void addEntityText(Vector<String> out, Entity e) {
        ...
        buffer.append(e.getChassis()).append(" (").append(e.getOwner().getName()).append("); ").append(e.getCrew().getGunnery()).append("/").append(e.getCrew().getPiloting()).append(Messages.getString("BoardView1.pilot"));
        ...
        if (numAdv > 0) {
            buffer.append(" <").append(numAdv).append(Messages.getString("BoardView1.advs"));
        }
        if (isMD) {
            buffer.append(Messages.getString("BoardView1.md"));
        }
        ...
        if (ge == null) {
            buffer.append(Messages.getString("BoardView1.move")).append(e.getMovementAbbr(e.moved)).append(":").append(e.delta_distance).append(" (+").append(Compute.getTargetMovementModifier(game, e.getId()).getValue()).append(");").append(Messages.getString("BoardView1.Heat")).append(e.heat);
            if (e.isCharging()) {
                buffer.append(" ").append(Messages.getString("BoardView1.charge1"));
            }
            if (e.isMakingDfa()) {
                buffer.append(" ").append(Messages.getString("BoardView1.DFA1"));
            }
        } else {
        ...
        buffer = new StringBuffer();
        if (ge == null) {
            buffer.append(Messages.getString("BoardView1.Armor")).append(e.getTotalArmor()).append(Messages.getString("BoardView1.internal")).append(e.getTotalInternal());
        }
        out.add(buffer.toString());
    ...
    public void add(AttackAction aa) {
        ...
        if (aa instanceof WeaponAttackAction) {
            ...
            out = wtype.getName() + Messages.getString("BoardView1.needs") + roll + " " + table;
        }
        if (aa instanceof KickAttackAction) {
            ...
            switch(leg) {
                case KickAttackAction.BOTH:
                    ...
                    out = Messages.getString("BoardView1.kickBoth", new Object[] { rollLeft, rollRight });
                    break;
                case KickAttackAction.LEFT:
                    rollLeft = KickAttackAction.toHit(game, attack.getEntityId(), game.getTarget(attack.getTargetType(), attack.getTargetId()), KickAttackAction.LEFT).getValueAsString();
                    out = Messages.getString("BoardView1.kickLeft", new Object[] { rollLeft });
                    break;
                case KickAttackAction.RIGHT:
                    rollRight = KickAttackAction.toHit(game, attack.getEntityId(), game.getTarget(attack.getTargetType(), attack.getTargetId()), KickAttackAction.RIGHT).getValueAsString();
                    out = Messages.getString("BoardView1.kickRight", new Object[] { rollRight });
                    break;
            }
        }
        if (aa instanceof PunchAttackAction) {
            ...
            switch(arm) {
                case PunchAttackAction.BOTH:
                    ...
                    out = Messages.getString("BoardView1.punchBoth", new Object[] { rollLeft, rollRight });
                    break;
                case PunchAttackAction.LEFT:
                    rollLeft = PunchAttackAction.toHit(game, attack.getEntityId(), game.getTarget(attack.getTargetType(), attack.getTargetId()), PunchAttackAction.LEFT).getValueAsString();
                    out = Messages.getString("BoardView1.punchLeft", new Object[] { rollLeft });
                    break;
                case PunchAttackAction.RIGHT:
                    rollRight = PunchAttackAction.toHit(game, attack.getEntityId(), game.getTarget(attack.getTargetType(), attack.getTargetId()), PunchAttackAction.RIGHT).getValueAsString();
                    out = Messages.getString("BoardView1.punchRight", new Object[] { rollRight });
                    break;
            }
        }
