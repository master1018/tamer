package jspacetrader.miglayout.layout;
...
public final class LayoutUtil {
	 ...
	 * overflow.
	 */
    static final int INF = (Integer.MAX_VALUE >> 10) - 100;

    /** Tag int for a value that in considered "not set". Used as "null" element in int arays.
	 */
    static final int NOT_SET = Integer.MIN_VALUE + 12346;
    ...
    public static void setDesignTime(ContainerWrapper cw, boolean b) {
        if (DT_MAP == null) DT_MAP = new WeakHashMap<Object, Boolean>();
    ...
    public static boolean isDesignTime(ContainerWrapper cw) {
        if (DT_MAP == null) return Beans.isDesignTime();
        if (cw != null && DT_MAP.containsKey(cw.getComponent()) == false) cw = null;
    ...
    static void putCCString(Object con, String s) {
        if (s != null && con != null && isDesignTime(null)) {
            if (CR_MAP == null) CR_MAP = new WeakHashMap<Object, String>(64);
            CR_MAP.put(con, s);
        }
    }
	 ...
	 * @param startSizeType The initial size to use. E.g. {@link jspacetrader.miglayout.layout.LayoutUtil#MIN}.
    ...
    static int[] calculateSerial(int[][] sizes, ResizeConstraint[] resConstr, Float[] defGrowWeights, int startSizeType, int bounds) {
        ...
        if (useLengthI != bounds && resConstr != null) {
            ...
            for (int i = 0; i < sizes.length; i++) {
                ...
                if (resC != null) prioList.add(new Integer(isGrow ? resC.growPrio : resC.shrinkPrio));
            }
            Integer[] prioIntegers = prioList.toArray(new Integer[prioList.size()]);
            for (int force = 0; force <= ((isGrow && defGrowWeights != null) ? 1 : 0); force++) {
    ...
    private static int getBrokenBoundary(float sz, int lower, int upper) {
        if (lower != NOT_SET) {
            if (sz < lower) return new Integer(lower);
        } else if (sz < 0f) {
            return new Integer(0);
        }
        if (upper != NOT_SET && sz > upper) return new Integer(upper);
        return NOT_SET;
    }
    ...
    public static int getSizeSafe(int[] sizes, int sizeType) {
        if (sizes == null || sizes[sizeType] == NOT_SET) return sizeType == MAX ? LayoutUtil.INF : 0;
        return sizes[sizeType];
    }

    static BoundSize derive(BoundSize bs, UnitValue min, UnitValue pref, UnitValue max) {
        if (bs == null || bs.isUnset()) return new BoundSize(min, pref, max, null);
    ...
    static void writeXMLObject(OutputStream os, Object o, ExceptionListener listener) {
        ...
        if (listener != null) encoder.setExceptionListener(listener);
    ...
    public static synchronized void writeAsXML(ObjectOutput out, Object o) throws IOException {
        if (writeOutputStream == null) writeOutputStream = new ByteArrayOutputStream(16384);
    ...
    public static synchronized Object readAsXML(ObjectInput in) throws IOException {
        if (readBuf == null) readBuf = new byte[16384];
        ...
        if (oldCL != null) cThread.setContextClassLoader(oldCL);
        return o;
    }
