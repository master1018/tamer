...
public class LexicalisationPanel extends JPanel {
    ...
    public static final String EMPTY_LABEL = Messages.getString("LexicalisationPanel.NoInfoLabel");
    ...
    public LexicalisationPanel() {
        ...
        fragPanel.add(new JLabel("FragURI:"), BorderLayout.NORTH);
        fragPanel.add(fragURI, BorderLayout.CENTER);
        JPanel prefPanel = new JPanel(new BorderLayout());
        prefPanel.add(new JLabel("PrefLabels:"), BorderLayout.NORTH);
        prefPanel.add(prefLabels, BorderLayout.CENTER);
        JPanel altPanel = new JPanel(new BorderLayout());
        altPanel.add(new JLabel("AltLabels:"), BorderLayout.NORTH);
        altPanel.add(altLabels, BorderLayout.CENTER);
        add(fragPanel);
        add(prefPanel);
        add(altPanel);
        setBorder(new TitledBorder(Messages.getString("LexicalisationPanel.LexiconPanelMainTitle")));
    }
    ...
    void setFragURI(String fragURI) {
        if (fragURI == null) this.fragURI.setText(EMPTY_LABEL); else this.fragURI.setText(fragURI);
    }

    private void setLabelsToPanel(JTextArea labelPan, Set<String> labels) {
        if (labels == null || labels.isEmpty()) labelPan.setText(EMPTY_LABEL); else {
