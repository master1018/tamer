...
public class DLRRetorno extends FFDialogo {
    ...
    public boolean CompRetorno() {
        if ((getRetorno().length() < 1) | (getRetorno().equals("<Selecione>"))) {
            Funcoes.mensagemInforma(this, "Escolha um C�digo de Retorno para a consulta! !");
            return false;
        } else return true;
    }
    ...
    public String getValor() {
        String sRetorno = "";
        if (rgOrdem.getVlrString().compareTo("D") == 0) sRetorno = "sgrcheque.DTINS"; else if (rgOrdem.getVlrString().compareTo("N") == 0) sRetorno = "vdcliente.NOMECLI";
        return sRetorno;
    }
}
