...
public class Subset<T> extends AbstractSet<T> implements Serializable {
    ...
    protected class CachedSubsetIterator implements Iterator<T> {
        ...
        public void remove() {
            if (filter.satisfies(lastObject)) internal.remove();
        }
    }

    @Override
    public Iterator iterator() {
        if (cacheSize) return new CachedSubsetIterator(); else {
    ...
    public boolean remove(Object o) {
        if (filter.satisfies((T) o)) return set.remove(o); else return false;
    }
    ...
    public boolean isEmpty() {
        ...
        while (it.hasNext()) {
            if (filter.satisfies(it.next())) return false;
        }
        return true;
    }
    ...
    public boolean add(T o) {
        if (filter.satisfies(o)) return set.add(o); else return false;
    }

    protected void updateRemove(T o) {
        if (filter.satisfies(o)) size--;
    }
    ...
    protected void cacheSize() {
        ...
        Iterator<T> it = set.iterator();
        while (it.hasNext()) {
            if (filter.satisfies(it.next())) size++;
        }
    }
