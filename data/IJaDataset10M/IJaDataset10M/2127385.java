        ...
        Action sendAction = new AbstractAction("Send", sendImg) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    String charset;
                    Envelope env;
                    if (((env = m.getEnvelope()) == null) || ((charset = env.getPayloadEncoding()) == null)) {
                        charset = ACLCodec.DEFAULT_CHARSET;
                    }
                    codec.decode(codec.encode(m, charset), charset);
                    myAgent.send(m);
                } catch (ACLCodec.CodecException ce) {
                    if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "Wrong ACL Message " + m.toString());
                    ce.printStackTrace();
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "File Not Found: " + fileName);
                    } catch (ACLCodec.CodecException e2) {
                        if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "Wrong ACL Message in file: " + fileName);
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "IO Exception reading the number of messages in the queue");
                        }
                        MsgIndication mi;
                        for (int i = 0; i < n; ++i) {
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (logger.isLoggable(Logger.WARNING)) logger.log(Logger.WARNING, "IO Exception writing the number of messages in the queue");
                        }
                        MsgIndication mi;
                        for (int i = 0; i < queuedMsgListModel.getSize(); ++i) {
