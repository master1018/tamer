...
public class PredicateFactory implements PredicateFactoryIF {
    ...
    public PredicateFactory(TopicMapIF topicmap, LocatorIF base) {
        ...
        predicatesById.put("source-locator", new ItemIdentifierPredicate(topicmap, "source-locator"));
    ...
    public PredicateIF createPredicate(TopicIF type, boolean assoc) {
        if (assoc) return new DynamicAssociationPredicate(topicmap, base, type); else return new DynamicOccurrencePredicate(topicmap, base, type);
    }

    public ModuleIF createModule(String uri) {
        if (uri.equals(ExperimentalModule.MODULE_URI)) return new ExperimentalModule(); else if (uri.equals(StringModule.MODULE_URI)) return new JDOBasicModule(new StringModule()); else if (uri.startsWith(JavaModule.MODULE_PREFIX)) return new JDOBasicModule(new JavaModule(topicmap, uri)); else if (uri.equals(NumbersModule.MODULE_URI)) return new NumbersModule(); else return null;
    }
    ...
    class ExperimentalModule implements ModuleIF {
        ...
        public PredicateIF getPredicate(String name) {
            if (name.equals("in")) return new InPredicate(topicmap); else if (name.equals("gt")) return new GreaterThanPredicate(); else if (name.equals("lt")) return new LessThanPredicate(); else if (name.equals("gteq")) return new GreaterThanEqualsPredicate(); else if (name.equals("lteq")) return new LessThanEqualsPredicate(); else if (name.equals("name")) return new NamePredicate(topicmap); else return null;
        }
    }
}
