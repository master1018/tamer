...
public class UploadStreamManager {
    ...
    InputEntityStream getInputEntityStream(User u, InputStream stream) {
        ...
        if (rate != null) uis.setTransferLimit(rate); else uis.setTransferLimit(transferRate);
    ...
    public void setUploadTransferLimit(double rate) {
        ...
        synchronized (uploadStreams) {
            ...
            for (UploadInputStream is : uis) is.setTransferLimit(rate);
        }
    }
    ...
    public void setUploadTransferLimit(User u, double rate) {
        ...
        if (uis != null) uis.setTransferLimit(rate);
        if (rate > 0) customRates.put(u, rate); else customRates.remove(u);
    }
    ...
    public double getUploadSpeed(User u) {
        synchronized (uploadStreams) {
            if (!uploadStreams.containsKey(u)) return 0; else return uploadStreams.get(u).getTransferRate();
        }
    }

    public double getUploadProgress(User u) {
        synchronized (uploadStreams) {
            if (!uploadStreams.containsKey(u)) return 0; else return uploadStreams.get(u).getPercentageCompletion();
        }
    }
    ...
    public double getTimeRemaining(User u) {
        synchronized (uploadStreams) {
            if (!uploadStreams.containsKey(u)) return -1; else return uploadStreams.get(u).getTimeRemaining();
        }
    }
    ...
    private class UploadInputStream extends InputEntityStream {
        ...
        public void close() throws IOException {
            ...
            synchronized (uploadStreams) {
                for (User u : users) if (uploadStreams.get(u).equals(this)) uploadStreams.remove(u);
            }
        }
    }
}
