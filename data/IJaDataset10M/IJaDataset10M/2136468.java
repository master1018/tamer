...
public class VersionActivator implements BundleActivator {
    ...
    public void start(BundleContext context) throws Exception {
        ...
        versionServReg = context.registerService(VersionService.class.getName(), versionServiceImpl, hashtable);
        logger.debug("SIP Protocol Provider Factory ... [REGISTERED]");
        logger.debug("SIP Communicator Version: sip-communicator-" + VersionImpl.currentVersion().toString());
        getConfigurationService().setProperty("sip-communicator.version", VersionImpl.currentVersion().toString(), true);
        getConfigurationService().setProperty("sip-communicator.application.name", VersionImpl.currentVersion().getApplicationName(), true);
    }
