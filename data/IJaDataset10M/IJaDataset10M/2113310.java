...
public class WordCooccurrenceCountingJob {
    ...
    public WordCooccurrenceCountingJob(Properties props) {
        job = new HadoopJob(RawTextCooccurrenceMapper.class, Text.class, TextIntWritable.class, CooccurrenceReducer.class, WordCooccurrenceWritable.class, IntWritable.class, props);
    }
     ...
     */
    public Iterator<WordCooccurrence> execute(Collection<String> inputDirs) throws Exception {
        HadoopJobResults results = job.run(inputDirs);
        return new WordCooccurrenceIterator(results.getFileSystem(), results.getResults().iterator());
    }
     ...
     * the corpus.
     */
    private static class WordCooccurrenceIterator implements Iterator<WordCooccurrence> {
         ...
         */
        public WordCooccurrenceIterator(FileSystem fileSystem, Iterator<Path> files) throws IOException {
        ...
        private void advance() throws IOException {
            if (curFile != null && curFile.hasNext()) {
                ...
                curFile = new FileIterator(new BufferedReader(new InputStreamReader(fileSystem.open(files.next()))));
                next = curFile.next();
            } else {
                next = null;
            }
     ...
     */
    private static class FileIterator implements Iterator<WordCooccurrence> {
        ...
        public WordCooccurrence next() {
            ...
            String[] arr = n.split("\t");
            return new SimpleWordCooccurrence(arr[0], arr[1], Integer.parseInt(arr[2]), Integer.parseInt(arr[3]));
        }
