...
public class ModelComponent extends NamedObject {
    ...
    public Model currentModel() {
        ...
        if (mBuff != null) return mBuff; else return _owner.getExperiment().getModel();
    }
    ...
    public SimTime currentTime() {
        return SimTime.toSimTime(_owner.getExperiment().getSimClock().getTime());
    }
    ...
    public void sendTraceNote(String description) {
        if (currentlySendTraceNotes()) {
            sendMessage(new TraceNote(currentModel(), description, presentTime(), currentEntityAll(), currentEvent()));
        }
    }
    ...
    public void skipTraceNote(int numSkipped) {
        if (numSkipped < 1) return;
        try {
            getModel().getExperiment().getMessageManager().skip(Class.forName("desmoj.core.report.TraceNote"), numSkipped);
        } catch (ClassNotFoundException cnfx) {
            throw new desmoj.core.exception.SimAbortedException(new ErrorMessage(getModel(), "Can not skip tracenotes! Simulation aborted.", "ModelComponent : " + getName() + " Method : skipTraceNote()", "The file for class desmoj.report.TraceNote can not be found by " + "the Java runtime. Following exception was caught : " + cnfx.toString(), "Check that all pathnames for DESMOJ are set in your environment.", presentTime()));
        }
    }
