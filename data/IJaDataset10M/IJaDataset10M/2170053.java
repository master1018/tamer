...
public class SphinxWrapper extends Crawler {
    ...
    public void visit(Page p) {
        if (((maxFetch != -1) && (countFetched.get() >= maxFetch)) || ((maxKeep != -1) && (countKept.get() >= maxKeep))) {
            syncIfNecessary();
            super.stop();
            return;
        }
        ...
        p.discardContent();
        if (doc == null) {
            System.out.println(countKept.toString() + " / " + currentFetched + " [" + depth + "] Drop: " + urlString);
    ...
    private static String convertMimeType(String originalType) {
        ...
        if (originalType.endsWith("xml") && (originalType.startsWith("application") || originalType.startsWith("application"))) {
            result = "text/xml";
        }
        return result;
    }
