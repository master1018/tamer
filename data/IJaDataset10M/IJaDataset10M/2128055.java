...
public class AddComChannelDialog extends JDialog {
    ...
    void updateControllers() {
        ...
        for (Controller ctrl : devicePool.getControllers()) if (ctrl.getType() == ControllerType.Communication) controllerCombo.addItem(ctrl);
        if (channel != null) controllerCombo.setSelectedItem(channel.getController()); else if (controllerCombo.getItemCount() > 0) controllerCombo.setSelectedIndex(0);
    }
    ...
    protected void init() {
        ...
        gbc = new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0);
        add(new JLabel("Controller:"), gbc);
        gbc = new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0);
        add(controllerCombo, gbc);
        gbc = new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0);
        add(new JLabel("Index in controller:"), gbc);
        gbc = new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0);
        add(indexSpinner, gbc);
        gbc = new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0);
        add(new JLabel("Name:"), gbc);
        gbc = new GridBagConstraints(1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0);
        ...
        gbc = new GridBagConstraints(0, 3, 2, 1, 1.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0);
    ...
    protected void createPressed(ActionEvent e) {
        ...
        try {
            ...
            DevVarLongStringArray params = new DevVarLongStringArray(iParams, strParams);
            DeviceData args = new DeviceData();
            args.insert(params);
            devicePool.getDevice().executeCommand(DevicePoolUtils.POOL_CMD_CREATE_COM_CHANNEL, args);
            JOptionPane.showMessageDialog(this, "Communication channel " + name + " sucessfully created", "Sucess!", JOptionPane.INFORMATION_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, buff.toString(), "Error trying to create a new communication channel", JOptionPane.ERROR_MESSAGE);
        }
    ...
    class ChannelNumberModel extends SpinnerNumberModel implements ActionListener {
        ...
        public Object getPreviousValue() {
            ...
            if (previousValue < 1) previousValue = findFirstAvailable();
            return previousValue;
        }

        protected int findFirstAvailable() {
            if (invalidChannelNumbers.length == 0) return 1;
            if (invalidChannelNumbers.length == 1) return invalidChannelNumbers[0] > 1 ? 1 : 2;
            for (int index = 1; index < invalidChannelNumbers.length; index++) {
                if (invalidChannelNumbers[index] - invalidChannelNumbers[index - 1] > 1) return invalidChannelNumbers[index - 1] + 1;
            }
