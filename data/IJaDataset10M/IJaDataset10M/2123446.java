 ...
 */
public class ChatContactListModel extends AbstractListModel {
        ...
        public int compare(ChatContact chatContact0, ChatContact chatContact1) {
            if (chatContact0 instanceof ConferenceChatContact) {
                if (chatContact1 instanceof ConferenceChatContact) {
                    ...
                    int role1 = ((ConferenceChatContact) chatContact1).getRole().getRoleIndex();
                    if (role0 > role1) return -1; else if (role0 < role1) return 1;
                } else return -1;
            } else if (chatContact1 instanceof ConferenceChatContact) return 1;
            return chatContact0.getName().compareToIgnoreCase(chatContact1.getName());
        }
    };
    ...
    public void addElement(ChatContact chatContact) {
        ...
        for (; index < chatContactCount; index++) if (sorter.compare(chatContacts.get(index), chatContact) > 0) break;
    ...
    public void removeElement(ChatContact chatContact) {
        ...
        if (chatContacts.remove(chatContact) && (index >= 0)) fireIntervalRemoved(this, index, index);
    }
}
