...
public class DelayedBufferWriter implements ChannelWriter, InterestWritableByteChannel {
    ...
    public synchronized void interestWrite(WriteObserver observer, boolean status) {
        ...
        if (source != null) source.interestWrite(this, true);
    }
    ...
    public boolean handleWrite() throws IOException {
        ...
        if (upper != null) upper.handleWrite();
        long now = System.nanoTime();
        if (lastFlushTime == 0) lastFlushTime = now;
        if (now - lastFlushTime > delay) flush(now);
        synchronized (this) {
            upper = observer;
            if (upper == null) {
                sink.interestWrite(this, false);
                if (!hasBufferedData()) return false; else {
    ...
    private void flush(long now) throws IOException {
        ...
        if (hasBufferedData()) {
            lastFlushTime = now;
            if (buf.hasRemaining()) buf.compact(); else buf.clear();
        } else {
