...
public class TernaryPlotDisplay extends CoDa2dDisplay {
    ...
    protected TernaryPlotDisplay(TernaryPlotBuilder builder) {
        ...
        for (int i = 0; i < names.length; i++) {
            try {
                iconLabel[i] = new TeXFormula(names[i]).createTeXIcon(TeXConstants.STYLE_DISPLAY, s);
            } catch (ParseException e) {
                iconLabel[i] = new TeXFormula("Item_" + (i + 1)).createTeXIcon(TeXConstants.STYLE_DISPLAY, s);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            origZ[i] = CoDaStats.ternaryTransform(data[0][i], data[1][i], data[2][i]);
        }
    }
    ...
    public final void setGrid(double[] center) {
        for (int i = 0; i < definedGrid.length; i++) {
            origGrid[3 * i][0] = CoDaStats.ternaryTransform((center == null ? definedGrid[i] : definedGrid[i] / center[0]), (center == null ? 1 - definedGrid[i] : (1 - definedGrid[i]) / center[1]), 0);
            origGrid[3 * i][1] = CoDaStats.ternaryTransform((center == null ? definedGrid[i] : definedGrid[i] / center[0]), 0, (center == null ? 1 - definedGrid[i] : (1 - definedGrid[i]) / center[2]));
            origGrid[3 * i + 1][0] = CoDaStats.ternaryTransform((center == null ? 1 - definedGrid[i] : (1 - definedGrid[i]) / center[0]), (center == null ? definedGrid[i] : definedGrid[i] / center[1]), 0);
            origGrid[3 * i + 1][1] = CoDaStats.ternaryTransform(0, (center == null ? definedGrid[i] : definedGrid[i] / center[1]), (center == null ? 1 - definedGrid[i] : (1 - definedGrid[i]) / center[2]));
            origGrid[3 * i + 2][0] = CoDaStats.ternaryTransform(0, (center == null ? 1 - definedGrid[i] : (1 - definedGrid[i]) / center[1]), (center == null ? definedGrid[i] : definedGrid[i] / center[2]));
            origGrid[3 * i + 2][1] = CoDaStats.ternaryTransform((center == null ? 1 - definedGrid[i] : (1 - definedGrid[i]) / center[0]), 0, (center == null ? definedGrid[i] : definedGrid[i] / center[2]));
        }
    }
    ...
    public void setCentered(boolean centered) {
        ...
        if (centered) {
            ...
            for (int i = 0; i < origZ.length; i++) {
                origZ[i] = CoDaStats.ternaryTransform(cdata[0][i], cdata[1][i], cdata[2][i]);
            }
        } else {
            setGrid(null);
            for (int i = 0; i < origZ.length; i++) {
    ...
    public void paintComponent(Graphics2D g2, double width, double height) {
        ...
        defaultTransform = new AffineTransform(factor, 0, 0, -factor, width / 2, height / 1.6);
        drawAreas(g2);
    ...
    public void drawGrid(Graphics2D g2) {
        ...
        g2.setStroke(new BasicStroke(CoDaDisplayConfiguration.getSize("grid"), BasicStroke.JOIN_MITER, BasicStroke.CAP_ROUND));
    ...
    public void drawAxis(Graphics2D g2) {
        ...
        g2.setStroke(new BasicStroke(CoDaDisplayConfiguration.getSize("axis"), BasicStroke.JOIN_MITER, BasicStroke.CAP_ROUND));
    ...
    public void drawLabels(Graphics2D g2) {
        ...
        iconLabel[0].paintIcon(null, g2, (int) o.getX() - iconLabel[0].getIconWidth() / 2, (int) o.getY() - iconLabel[0].getIconHeight() - separation);
        o = defaultTransform.transform(new Point2D.Double(V[1][0], V[1][1]), o);
        iconLabel[1].paintIcon(null, g2, (int) o.getX() - 3 * iconLabel[1].getIconWidth() / 4, (int) o.getY() + separation);
        o = defaultTransform.transform(new Point2D.Double(V[2][0], V[2][1]), o);
        iconLabel[2].paintIcon(null, g2, (int) o.getX() - iconLabel[2].getIconWidth() / 4, (int) o.getY() + separation);
    }
