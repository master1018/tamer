...
public class MUDFight extends StdLibrary implements CombatLibrary {
    ...
    public HashSet allPossibleCombatants(MOB mob, boolean beRuthless) {
        ...
        for (int m = 0; m < thisRoom.numInhabitants(); m++) {
            ...
            if ((inhab != null) && (inhab != mob) && (!h1.contains(inhab)) && (CMLib.flags().isSeen(inhab) || CMLib.flags().canMove(inhab)) && (CMLib.flags().isSeen(inhab) || (!CMLib.flags().isCloaked(inhab))) && ((beRuthless) || (!mob.isMonster()) || (!inhab.isMonster()))) h.add(inhab);
        }
        return h;
    }
    ...
    public HashSet properTargets(Ability A, MOB caster, boolean beRuthless) {
        ...
        if (A.abstractQuality() != Ability.QUALITY_MALICIOUS) {
            if (caster.Name().equalsIgnoreCase("somebody")) h = new HashSet(); else h = caster.getGroupMembers(new HashSet());
            for (Iterator e = ((HashSet) h.clone()).iterator(); e.hasNext(); ) {
                MOB M = (MOB) e.next();
                if (M.location() != caster.location()) h.remove(M);
            }
        } else if (caster.isInCombat()) h = allCombatants(caster); else h = allPossibleCombatants(caster, beRuthless);
        return h;
    }
    ...
    public int adjustedAttackBonus(MOB mob, MOB target) {
        ...
        double[] vars = { mob.envStats().attackAdjustment(), currStr, baseStr, strBonus, (mob.curState().getHunger() < 1) ? 1.0 : 0.0, (mob.curState().getThirst() < 1) ? 1.0 : 0.0, (mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0 : 0.0 };
    ...
    public int adjustedArmor(MOB mob) {
        ...
        double[] vars = { mob.envStats().armor(), currDex, baseDex, dexBonus, (mob.curState().getHunger() < 1) ? 1.0 : 0.0, (mob.curState().getThirst() < 1) ? 1.0 : 0.0, (mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0 : 0.0, CMLib.flags().isSitting(mob) ? 0.0 : 1.0, CMLib.flags().isSleeping(mob) ? 0.0 : 1.0 };
    ...
    public boolean rollToHit(MOB attacker, MOB defender) {
        ...
        double vars[] = { attacker.envStats().level(), defender.envStats().level(), attacker.envStats().level() > defender.envStats().level() ? 1 : -1 };
    ...
    public boolean rollToHit(int attack, int defence, int adjustment) {
        ...
        if (myArmor == 0) myArmor = 1.0; else if (myArmor < 0.0) myArmor = -CMath.div(1.0, myArmor);
        double hisAttack = (double) attack;
        if (hisAttack == 0.0) hisAttack = 1.0; else if (hisAttack < 0.0) hisAttack = -CMath.div(1.0, myArmor);
    ...
    public HashSet allCombatants(MOB mob) {
        ...
        HashSet h1 = null;
        if (mob.Name().equalsIgnoreCase("nobody")) h1 = new HashSet(); else h1 = mob.getGroupMembers(new HashSet());
        for (int m = 0; m < thisRoom.numInhabitants(); m++) {
            MOB inhab = thisRoom.fetchInhabitant(m);
            if ((inhab != null) && (inhab != mob) && ((inhab == mob.getVictim()) || ((inhab != mob) && (inhab.getVictim() != mob.getVictim()) && (CMLib.flags().isSeen(inhab) || (!CMLib.flags().isCloaked(inhab))) && (CMLib.flags().canMove(inhab) || CMLib.flags().isSeen(inhab)) && (!h1.contains(inhab))))) h.add(inhab);
        }
        return h;
    }
    ...
    public void makePeaceInGroup(MOB mob) {
        ...
        for (Iterator e = myGroup.iterator(); e.hasNext(); ) {
            MOB mob2 = (MOB) e.next();
            if (mob2.isInCombat() && (myGroup.contains(mob2.getVictim()))) mob2.makePeace();
        }
    }
    ...
    public void postPanic(MOB mob, CMMsg addHere) {
        ...
        if ((addHere != null) && (addHere.trailerMsgs() != null)) for (int i = 0; i < addHere.trailerMsgs().size(); i++) {
            CMMsg msg = (CMMsg) addHere.trailerMsgs().elementAt(i);
            if ((msg.source() == mob) && ((msg.sourceMinor() == CMMsg.TYP_PANIC)) || (msg.sourceMinor() == CMMsg.TYP_DEATH)) return;
        }
        CMMsg msg = CMClass.getMsg(mob, null, CMMsg.MSG_PANIC, null);
        if (addHere != null) addHere.addTrailerMsg(msg); else if ((mob.location() != null) && (mob.location().okMessage(mob, msg))) mob.location().send(mob, msg);
    }
    ...
    public void postDeath(MOB killerM, MOB deadM, CMMsg addHere) {
        ...
        if ((addHere != null) && (addHere.trailerMsgs() != null)) for (int i = 0; i < addHere.trailerMsgs().size(); i++) {
            CMMsg msg = (CMMsg) addHere.trailerMsgs().elementAt(i);
            if ((msg.source() == deadM) && ((msg.sourceMinor() == CMMsg.TYP_PANIC)) || (msg.sourceMinor() == CMMsg.TYP_DEATH)) return;
        }
        String msp = CMProps.msp("death" + CMLib.dice().roll(1, 7, 0) + ".wav", 50);
        CMMsg msg = null;
        if (isKnockedOutUponDeath(deadM, killerM)) msg = CMClass.getMsg(deadM, null, killerM, CMMsg.MSG_OK_VISUAL, "^f^*^<FIGHT^>!!!!!!!!!YOU ARE DEFEATED!!!!!!!!!!^</FIGHT^>^?^.\n\r" + msp, CMMsg.MSG_OK_VISUAL, null, CMMsg.MSG_DEATH, "^F^<FIGHT^><S-NAME> is DEFEATED!!!^</FIGHT^>^?\n\r" + msp); else msg = CMClass.getMsg(deadM, null, killerM, CMMsg.MSG_OK_VISUAL, "^f^*^<FIGHT^>!!!!!!!!!!!!!!YOU ARE DEAD!!!!!!!!!!!!!!^</FIGHT^>^?^.\n\r" + msp, CMMsg.MSG_OK_VISUAL, null, CMMsg.MSG_DEATH, "^F^<FIGHT^><S-NAME> is DEAD!!!^</FIGHT^>^?\n\r" + msp);
        ...
        if (addHere != null) {
            ...
            if (deathRoom.okMessage(deadM, msg2)) deathRoom.send(deadM, msg2);
        }
    }

    public boolean postAttack(MOB attacker, MOB target, Item weapon) {
        if ((attacker == null) || (!attacker.mayPhysicallyAttack(target))) return false;
        if ((weapon == null) && (CMath.bset(attacker.getBitmap(), MOB.ATT_AUTODRAW))) {
        ...
        if (R != null) if (R.okMessage(attacker, msg)) {
            R.send(attacker, msg);
            return msg.value() > 0;
        }
        return false;
    ...
    public boolean postHealing(MOB healer, MOB target, Environmental tool, int messageCode, int healing, String allDisplayMessage) {
        ...
        Room R = target.location();
        if (R != null) if (R.okMessage(target, msg)) {
            R.send(target, msg);
            return true;
        }
        return false;
    ...
    public String replaceDamageTag(String str, int damage, int damageType) {
        ...
        if (replace >= 0) {
            if (!CMProps.getVar(CMProps.SYSTEM_SHOWDAMAGE).equalsIgnoreCase("YES")) return str.substring(0, replace) + standardHitWord(damageType, damage) + str.substring(replace + 8);
        ...
        if (replace >= 0) {
            ...
            hitWord = CMStrings.replaceAll(hitWord, ")", "");
            if (!CMProps.getVar(CMProps.SYSTEM_SHOWDAMAGE).equalsIgnoreCase("YES")) return str.substring(0, replace) + hitWord + str.substring(replace + 9);
    ...
    public void postDamage(MOB attacker, MOB target, Environmental weapon, int damage, int messageCode, int damageType, String allDisplayMessage) {
        ...
        if ((weapon instanceof Ability) && (damage > 0) && (attacker != target) && (attacker != null) && (target != null) && (attacker.isMine(weapon) || (attacker.envStats().level() > 1))) damage = modifySpellDamage(attacker, target, damage);
        ...
        if (R != null) if (R.okMessage(target, msg)) {
            if (damageType >= 0) msg.modify(msg.source(), msg.target(), msg.tool(), msg.sourceCode(), replaceDamageTag(msg.sourceMessage(), msg.value(), damageType), msg.targetCode(), replaceDamageTag(msg.targetMessage(), msg.value(), damageType), msg.othersCode(), replaceDamageTag(msg.othersMessage(), msg.value(), damageType));
            R.send(target, msg);
        }
    }
    ...
    public int modifySpellDamage(MOB attacker, MOB target, int baseDamage) {
        ...
        double[] vars = { baseDamage, currInt, baseInt, intBonus, (attacker.curState().getHunger() < 1) ? 1.0 : 0.0, (attacker.curState().getThirst() < 1) ? 1.0 : 0.0, (attacker.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0 : 0.0, attacker.envStats().level(), target.envStats().level() };
    ...
    public int adjustedDamage(MOB mob, Weapon weapon, MOB target) {
        ...
        if (target != null) {
            double[] vars = { useDmg.envStats().damage(), mob.charStats().getStat(CharStats.STAT_STRENGTH), mob.envStats().level(), target.envStats().level(), (mob.curState().getHunger() < 1) ? 1.0 : 0.0, (mob.curState().getThirst() < 1) ? 1.0 : 0.0, (mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0 : 0.0, CMLib.flags().canBeSeenBy(target, mob) ? 0 : 1, CMLib.flags().isSleeping(target) ? 1 : 0, CMLib.flags().isSitting(target) ? 1 : 0 };
            if (rangedAttack) damageAmount = CMath.parseMathExpression(targetedRangedDamageFormula, vars, 0.0); else damageAmount = CMath.parseMathExpression(targetedMeleeDamageFormula, vars, 0.0);
        } else {
            double[] vars = { useDmg.envStats().damage(), mob.charStats().getStat(CharStats.STAT_STRENGTH), mob.envStats().level(), 0, (mob.curState().getHunger() < 1) ? 1.0 : 0.0, (mob.curState().getThirst() < 1) ? 1.0 : 0.0, (mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0 : 0.0, 0, 0, 0 };
            if ((weapon != null) && ((weapon.weaponClassification() == Weapon.CLASS_RANGED) || (weapon.weaponClassification() == Weapon.CLASS_THROWN))) damageAmount = CMath.parseMathExpression(staticRangedDamageFormula, vars, 0.0); else damageAmount = CMath.parseMathExpression(staticMeleeDamageFormula, vars, 0.0);
        }
        int maxDex = mob.charStats().getMaxStat(CharStats.STAT_DEXTERITY);
        int currDex = mob.charStats().getStat(CharStats.STAT_DEXTERITY);
        int dexBonus = 0;
        ...
        if (baseDex > maxDex) baseDex = maxDex;
        double[] vars = { damageAmount, currDex, baseDex, dexBonus, (mob.curState().getHunger() < 1) ? 1.0 : 0.0, (mob.curState().getThirst() < 1) ? 1.0 : 0.0, (mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0 : 0.0, mob.envStats().level(), (target == null) ? 0 : target.envStats().level() };
        ...
        if (target != null) {
            vars[0] = damageAmount;
            if (rangedAttack) damageAmount = CMath.parseMathExpression(rangedFudgeDamageFormula, vars, 0.0); else damageAmount = CMath.parseMathExpression(meleeFudgeDamageFormula, vars, 0.0);
        }
    ...
    public void postWeaponDamage(MOB source, MOB target, Item item, boolean success) {
        ...
        if (success) {
            String oldHitString = "^F^<FIGHT^>" + ((weapon != null) ? weapon.hitString(damageInt) : standardHitString(Weapon.CLASS_BLUNT, damageInt, item.name())) + "^</FIGHT^>^?";
            CMMsg msg = CMClass.getMsg(source, target, item, CMMsg.MSG_OK_VISUAL, CMMsg.MSG_DAMAGE, CMMsg.MSG_OK_VISUAL, oldHitString);
            CMLib.color().fixSourceFightColor(msg);
            ...
            if ((room != null) && (room.okMessage(source, msg))) {
                if (msg.targetMinor() == CMMsg.TYP_DAMAGE) {
                    damageInt = msg.value();
                    msg.modify(msg.source(), msg.target(), msg.tool(), msg.sourceCode(), replaceDamageTag(msg.sourceMessage(), msg.value(), damageType), msg.targetCode(), replaceDamageTag(msg.targetMessage(), msg.value(), damageType), msg.othersCode(), replaceDamageTag(msg.othersMessage(), msg.value(), damageType));
                }
                if ((source.mayIFight(target)) && (source.location() == room) && (target.location() == room)) room.send(source, msg);
            }
        } else {
            String missString = "^F^<FIGHT^>" + ((weapon != null) ? weapon.missString() : standardMissString(Weapon.TYPE_BASHING, Weapon.CLASS_BLUNT, item.name(), false)) + "^</FIGHT^>^?";
            CMMsg msg = CMClass.getMsg(source, target, weapon, CMMsg.MSG_NOISYMOVEMENT, missString);
            CMLib.color().fixSourceFightColor(msg);
            Room R = source.location();
            if (R != null) if (R.okMessage(source, msg) && (!source.amDead()) && (!source.amDestroyed())) R.send(source, msg);
        }
    }

    public void processFormation(Vector[] done, MOB leader, int level) {
        for (int i = 0; i < done.length; i++) if ((done[i] != null) && (done[i].contains(leader))) return;
    ...
    public MOB getFollowedLeader(MOB mob) {
        MOB leader = mob;
        if (leader.amFollowing() != null) leader = leader.amUltimatelyFollowing();
        return leader;
    }
    ...
    public Vector getFormationFollowed(MOB mob) {
        ...
        for (int i = 1; i < form.length; i++) {
            if ((form[i] != null) && (form[i].contains(mob))) {
                i--;
                while (i >= 0) {
                    if ((form[i] != null) && (form[i].size() > 0)) return form[i];
                    i--;
                }
            }
        }
    ...
    public int getFormationAbsOrder(MOB mob) {
        Vector[] form = getFormation(mob);
        for (int i = 1; i < form.length; i++) {
            if ((form[i] != null) && (form[i].contains(mob))) return i;
        }
        return 0;
    }
    ...
    public CharClass getCombatDominantClass(MOB killer, MOB killed) {
        ...
        if ((killer != null) && (killer.charStats() != null)) {
            ...
            checked.add(M);
            while (M.isMonster() && (M.amFollowing() != null) && (!checked.contains(M.amFollowing()))) {
                M = M.amFollowing();
                checked.add(M);
            }
            if ((!M.isMonster()) && (M.charStats() != null)) C = M.charStats().getCurrentClass();
        } else C = CMClass.getCharClass("StdCharClass");
        return C;
    }
    ...
    protected HashSet getCombatBeneficiaries(MOB killer, MOB killed, Room deathRoom, HashSet beneficiaries, CharClass combatCharClass) {
        ...
        if (deathRoom != null) {
            for (int m = 0; m < deathRoom.numInhabitants(); m++) {
                ...
                if ((combatCharClass.isValidClassBeneficiary(killer, killed, mob, followers)) && (!beneficiaries.contains(mob))) beneficiaries.add(mob);
            }
        }
        if ((killer != null) && (!beneficiaries.contains(killer)) && (killer != killed) && (CMLib.flags().isInTheGame(killer, true))) beneficiaries.add(killer);
    ...
    protected HashSet getCombatDividers(MOB killer, MOB killed, Room deathRoom, HashSet dividers, CharClass combatCharClass) {
        ...
        if (deathRoom != null) {
            for (int m = 0; m < deathRoom.numInhabitants(); m++) {
                MOB mob = deathRoom.fetchInhabitant(m);
                if ((combatCharClass.isValidClassDivider(killer, killed, mob, followers)) && (!dividers.contains(mob))) dividers.add(mob);
            }
        }
        if ((killer != null) && (!dividers.contains(killer)) && (killer != killed) && (CMLib.flags().isInTheGame(killer, true))) dividers.add(killer);
        return dividers;
    }
    ...
    public DeadBody justDie(MOB source, MOB target) {
        ...
        for (Iterator e = beneficiaries.iterator(); e.hasNext(); ) {
            MOB M = (MOB) e.next();
            if (((CMath.bset(M.getBitmap(), MOB.ATT_AUTOGOLD)) && (!goldLooters.contains(M))) && (M != target) && (M.location() == deathRoom) && (deathRoom.isInhabitant(M))) goldLooters.addElement(M);
        }
        ...
        String[] cmds = null;
        if ((target.isMonster()) || (target.soulMate() != null)) cmds = CMParms.toStringArray(CMParms.parseCommas(CMProps.getVar(CMProps.SYSTEM_MOBDEATH), true)); else cmds = CMParms.toStringArray(CMParms.parseCommas(CMProps.getVar(CMProps.SYSTEM_PLAYERDEATH), true));
        DeadBody body = null;
        if ((!CMParms.containsIgnoreCase(cmds, "RECALL")) && (!isKnockedOutUponDeath(target, source))) body = target.killMeDead(true);
        ...
        if (!isKnockedOutUponDeath(target, source)) {
            ...
            if ((body != null) && (body.owner() instanceof Room) && (((Room) body.owner()).isContent(body))) bodyRoom = (Room) body.owner();
            ...
            if ((!target.isMonster()) && (CMLib.dice().rollPercentage() == 1) && (!CMSecurity.isDisabled("AUTODISEASE"))) {
                ...
                if ((A != null) && (target.fetchEffect(A.ID()) == null)) A.invoke(target, target, true, 0);
            }
            ...
            if ((source != null) && (bodyRoom != null) && (body != null) && (source.location() == bodyRoom) && (bodyRoom.isInhabitant(source)) && (CMath.bset(source.getBitmap(), MOB.ATT_AUTOLOOT))) {
                if ((source.riding() != null) && (source.riding() instanceof MOB)) source.tell("You'll need to dismount to loot the body."); else if ((source.riding() != null) && (source.riding() instanceof MOB)) source.tell("You'll need to disembark to loot the body."); else for (int i = bodyRoom.numItems() - 1; i >= 0; i--) {
                    Item item = bodyRoom.fetchItem(i);
                    if ((item != null) && (item.container() == body) && (CMLib.flags().canBeSeenBy(body, source)) && ((!body.destroyAfterLooting()) || (!(item instanceof RawMaterial))) && (CMLib.flags().canBeSeenBy(item, source))) CMLib.commands().postGet(source, body, item, false);
                }
                if (body.destroyAfterLooting()) bodyRoom.recoverRoomStats();
            }
            Coins C = null;
            if ((deadMoney > 0) && (myAmountOfDeadMoney > 0) && (body != null) && (bodyRoom != null)) for (int g = 0; g < goldLooters.size(); g++) {
                ...
                if (C != null) {
                    ...
                    if (mob.location() == bodyRoom) {
                        if ((mob.riding() != null) && (mob.riding() instanceof MOB)) mob.tell("You'll need to dismount to get " + C.name() + " off the body."); else if ((mob.riding() != null) && (mob.riding() instanceof Item)) mob.tell("You'll need to disembark to get " + C.name() + " off the body."); else if (CMLib.flags().canBeSeenBy(body, mob)) CMLib.commands().postGet(mob, body, C, false);
                    }
                }
            }
            if ((body != null) && (bodyRoom != null) && (body.destroyAfterLooting())) {
                for (int i = bodyRoom.numItems() - 1; i >= 0; i--) {
                    Item item = bodyRoom.fetchItem(i);
                    if ((item != null) && (item.container() == body)) item.setContainer(null);
                }
                body.destroy();
    ...
    private int[] damageThresholds() {
        return CMProps.getI1ListVar(CMProps.SYSTEML_DAMAGE_WORDS_THRESHOLDS);
    }

    private String[][] hitWords() {
        return CMProps.getS2ListVar(CMProps.SYSTEML_DAMAGE_WORDS);
    }

    private String[] armorDescs() {
        return CMProps.getSListVar(CMProps.SYSTEML_ARMOR_DESCS);
    }

    private String[] prowessDescs() {
        return CMProps.getSListVar(CMProps.SYSTEML_PROWESS_DESCS);
    }

    private String[] missWeaponDescs() {
        return CMProps.getSListVar(CMProps.SYSTEML_WEAPON_MISS_DESCS);
    }

    private String[] missDescs() {
        return CMProps.getSListVar(CMProps.SYSTEML_MISS_DESCS);
    }

    public String standardHitWord(int type, int damage) {
        if ((type < 0) || (type >= Weapon.TYPE_DESCS.length)) type = Weapon.TYPE_BURSTING;
        int[] thresholds = damageThresholds();
        int damnCode = thresholds.length - 2;
        for (int i = 0; i < thresholds.length; i++) if (damage <= thresholds[i]) {
            damnCode = i;
            break;
        }
        damnCode++;
        ...
        if (hitWordIndex == null) {
            ...
            for (int w = 0; w < Weapon.TYPE_DESCS.length; w++) {
                ...
                for (int i = 0; i < hitWords.length; i++) {
                    if (hitWords[i][0].equalsIgnoreCase("ALL")) ALL = hitWords[i]; else if (hitWords[i][0].equalsIgnoreCase(Weapon.TYPE_DESCS[w])) {
                        MINE = hitWords[i];
                        break;
                    }
                }
                if (MINE != null) newWordIndex[w] = MINE; else newWordIndex[w] = ALL;
            }
    ...
    public String armorStr(MOB mob) {
        ...
        return (armor < 0) ? armorDescs()[0] : ((armor >= ARMOR_CEILING) ? armorDescs()[armorDescs().length - 1] + (CMStrings.repeat("!", (armor - ARMOR_CEILING) / 100)) + " (" + armor + ")" : (armorDescs()[(int) Math.round(Math.floor(CMath.mul(CMath.div(armor, ARMOR_CEILING), armorDescs().length)))] + " (" + armor + ")"));
    }
    ...
    public String fightingProwessStr(MOB mob) {
        ...
        return (prowess < 0) ? prowessDescs()[0] : ((prowess >= PROWESS_CEILING) ? prowessDescs()[prowessDescs().length - 1] + (CMStrings.repeat("!", (prowess - PROWESS_CEILING) / 100)) + " (" + prowess + ")" : (prowessDescs()[(int) Math.round(Math.floor(CMath.mul(CMath.div(prowess, PROWESS_CEILING), prowessDescs().length)))] + " (" + prowess + ")"));
    }
    ...
    public String standardHitString(int weaponClass, int damageAmount, String weaponName) {
        if ((weaponName == null) || (weaponName.length() == 0)) weaponClass = Weapon.CLASS_NATURAL;
        switch(weaponClass) {
            case Weapon.CLASS_RANGED:
    ...
    public String[] healthDescs() {
        return CMProps.getSListVar(CMProps.SYSTEML_HEALTH_CHART);
    }
    ...
    public void resistanceMsgs(CMMsg msg, MOB source, MOB target) {
        ...
        if (msg.tool() != null) {
            if (msg.tool() instanceof Trap) endPart = "."; else if (msg.tool() instanceof Ability) tool = ((Ability) msg.tool()).name();
        }
        String tackOn = null;
        switch(msg.targetMinor()) {
            case CMMsg.TYP_MIND:
                tackOn = "<S-NAME> shake(s) off the " + ((tool == null) ? "mental attack" : tool) + endPart;
                break;
            case CMMsg.TYP_GAS:
                tackOn = "<S-NAME> resist(s) the " + ((tool == null) ? "noxious fumes" : tool) + endPart;
                break;
            case CMMsg.TYP_COLD:
                tackOn = "<S-NAME> shake(s) off the " + ((tool == null) ? "cold blast" : tool) + endPart;
                break;
            case CMMsg.TYP_ELECTRIC:
                tackOn = "<S-NAME> shake(s) off the " + ((tool == null) ? "electrical attack" : tool) + endPart;
                break;
            case CMMsg.TYP_FIRE:
                tackOn = "<S-NAME> resist(s) the " + ((tool == null) ? "blast of heat" : tool) + endPart;
                break;
            case CMMsg.TYP_WATER:
                tackOn = "<S-NAME> dodge(s) the " + ((tool == null) ? "wet blast" : tool) + endPart;
                break;
            case CMMsg.TYP_UNDEAD:
                tackOn = "<S-NAME> shake(s) off the " + ((tool == null) ? "evil attack" : tool) + endPart;
                break;
            case CMMsg.TYP_POISON:
                tackOn = "<S-NAME> shake(s) off the " + ((tool == null) ? "poison" : tool) + endPart;
                break;
            case CMMsg.TYP_DISEASE:
                tackOn = "<S-NAME> resist(s) the " + ((tool == null) ? "disease" : tool);
                break;
            case CMMsg.TYP_JUSTICE:
                break;
            case CMMsg.TYP_CAST_SPELL:
                tackOn = "<S-NAME> resist(s) the " + ((tool == null) ? "magical attack" : tool) + endPart;
                break;
            case CMMsg.TYP_PARALYZE:
                tackOn = "<S-NAME> resist(s) the " + ((tool == null) ? "paralysis" : tool) + endPart;
                break;
        }
        if (tackOn != null) {
            String newStr = target + "/" + source + "/" + tool;
            if (!newStr.equals(lastStr) || ((System.currentTimeMillis() - lastRes) > 250)) msg.addTrailerMsg(CMClass.getMsg(target, source, CMMsg.MSG_OK_ACTION, tackOn));
            lastStr = newStr;
    ...
    public void handleBeingDamaged(CMMsg msg) {
        ...
        synchronized (("DMG" + target.Name().toUpperCase()).intern()) {
            if ((dmg > 0) && (target.curState().getHitPoints() > 0)) {
                if ((!target.curState().adjHitPoints(-dmg, target.maxState())) && (target.curState().getHitPoints() < 1) && (target.location() != null)) postDeath(attacker, target, msg); else {
                    if ((Math.round(CMath.div(dmg, target.maxState().getHitPoints()) * 100.0) >= CMProps.getIntVar(CMProps.SYSTEMI_INJBLEEDPCTHP)) && bleedableWeapon(msg.tool())) {
                        Ability A2 = CMClass.getAbility("Bleeding");
                        if (A2 != null) A2.invoke(((MOB) target), ((MOB) target), true, 0);
                    }
                    if ((target.curState().getHitPoints() < target.getWimpHitPoint()) && (target.getWimpHitPoint() > 0) && (target.isInCombat())) postPanic(target, msg); else if ((CMProps.getIntVar(CMProps.SYSTEMI_INJPCTHP) >= (int) Math.round(CMath.div(target.curState().getHitPoints(), target.maxState().getHitPoints()) * 100.0)) && (!CMLib.flags().isGolem(target)) && (target.fetchEffect("Injury") == null)) {
    ...
    public void handleDeath(CMMsg msg) {
        ...
        if (!deadmob.amDead()) {
            if ((!deadmob.isMonster()) && (deadmob.soulMate() == null)) {
                ...
                if (!CMLib.flags().isCloaked(deadmob)) for (int i = 0; i < channels.size(); i++) if ((msg.tool() != null) && (msg.tool() instanceof MOB)) CMLib.commands().postChannel((String) channels.elementAt(i), deadmob.getClanID(), deadmob.Name() + " was just killed in " + CMLib.map().getExtendedRoomID(deadmob.location()) + " by " + msg.tool().Name() + ".", true); else CMLib.commands().postChannel((String) channels.elementAt(i), deadmob.getClanID(), deadmob.Name() + " has just died at " + CMLib.map().getExtendedRoomID(deadmob.location()), true);
                if (!CMLib.flags().isCloaked(deadmob)) for (int i = 0; i < channels2.size(); i++) if ((msg.tool() != null) && (msg.tool() instanceof MOB)) CMLib.commands().postChannel((String) channels2.elementAt(i), deadmob.getClanID(), deadmob.Name() + " was just killed.", true);
            }
            if (msg.tool() instanceof MOB) {
                ...
                if ((!deadmob.isMonster()) && (deadmob.soulMate() == null) && (killer != deadmob) && (!killer.isMonster())) {
                    ...
                    if ((deadmob.getClanID().length() > 0) && (killer.getClanID().length() > 0) && (!deadmob.getClanID().equals(killer.getClanID())) && (deadmob.session() != null) && (killer.session() != null) && (!deadmob.session().getAddress().equalsIgnoreCase(killer.session().getAddress()))) {
                        Clan C = CMLib.clans().getClan(killer.getClanID());
                        if (C != null) C.recordClanKill();
                    }
                }
            } else justDie(null, deadmob);
            deadmob.tell(deadmob, msg.target(), msg.tool(), msg.sourceMessage());
            if (deadmob.riding() != null) deadmob.riding().delRider(deadmob);
            if (CMLib.flags().isCataloged(deadmob)) CMLib.catalog().bumpDeathPickup(deadmob);
        }
    }
    ...
    public void handleObserveDeath(MOB observer, MOB fighting, CMMsg msg) {
        ...
        if ((fighting == deadmob) && (R != null)) {
            ...
            for (int r = 0; r < R.numInhabitants(); r++) {
                ...
                if ((M != observer) && (M != deadmob) && (M != null) && (M.getVictim() == observer) && (!M.amDead()) && (CMLib.flags().isInTheGame(M, true))) {
                    newTargetM = M;
                    break;
                }
            }
            if (newTargetM == null) for (int r = 0; r < R.numInhabitants(); r++) {
                MOB M = R.fetchInhabitant(r);
                MOB vic = M.getVictim();
                if ((M != observer) && (M != deadmob) && (M != null) && (hisGroupH.contains(M) || ((vic != null) && (myGroupH.contains(vic)))) && (!M.amDead()) && (CMLib.flags().isInTheGame(M, true))) {
                    newTargetM = M;
                    break;
                }
            }
            if ((newTargetM == null) || (newTargetM.isInCombat())) observer.setVictim(newTargetM);
        }
    }
    ...
    public void handleBeingAssaulted(CMMsg msg) {
        ...
        MOB attacker = msg.source();
        MOB target = (MOB) msg.target();
        if ((!target.isInCombat()) && (target.location() != null) && (target.location().isInhabitant(attacker)) && ((!CMath.bset(msg.sourceCode(), CMMsg.MASK_ALWAYS)) || (!(msg.tool() instanceof DiseaseAffect)))) {
            establishRange(target, attacker, msg.tool());
            target.setVictim(attacker);
        }
        if (target.isInCombat()) {
            if (attacker.session() != null) {
                if (!target.isMonster()) attacker.session().setLastPKFight(); else attacker.session().setLastNPCFight();
            }
            if (msg.targetMinor() == CMMsg.TYP_WEAPONATTACK) {
                Item weapon = attacker.myNaturalWeapon();
                if ((msg.tool() != null) && (msg.tool() instanceof Item)) weapon = (Item) msg.tool();
                if (weapon != null) {
                    boolean isHit = rollToHit(attacker, target);
                    postWeaponDamage(attacker, target, weapon, isHit);
                    if (isHit) msg.setValue(1);
                }
                if ((target.soulMate() == null) && (target.playerStats() != null) && (target.location() != null)) target.playerStats().adjHygiene(PlayerStats.HYGIENE_FIGHTDIRTY);
                if ((attacker.isMonster()) && (!attacker.isInCombat())) attacker.setVictim(target);
            } else if ((msg.tool() != null) && (msg.tool() instanceof Item)) postWeaponDamage(attacker, target, (Item) msg.tool(), true);
        }
        if (CMLib.flags().isSitting(target) || CMLib.flags().isSleeping(target)) CMLib.commands().postStand(target, true);
    }
    ...
    public void makeFollowersFight(MOB observer, MOB target, MOB source) {
        ...
        if ((target.location() != observer.location()) || (target.location() != source.location())) return;
        ...
        if ((observerFollows == target) || (targetFollows == observer) || ((targetFollows != null) && (targetFollows == observerFollows))) {
            observer.setVictim(source);
            establishRange(observer, source, observer.fetchWieldedItem());
        } else if ((observerFollows == source) || (sourceFollows == observer) || ((sourceFollows != null) && (sourceFollows == observerFollows))) {
    ...
    public Vector getAllInProximity(MOB to, int distance) {
        ...
        for (int i = 0; i < R.numInhabitants(); i++) everyV.addElement(R.fetchInhabitant(i));
        if (!everyV.contains(to)) everyV.addElement(to);
        int[][] map = new int[everyV.size()][everyV.size()];
        for (int x = 0; x < map.length; x++) for (int y = 0; y < map.length; y++) map[x][y] = -1;
        return V;
    }
    ...
    public void establishRange(MOB source, MOB target, Environmental tool) {
        if ((source.rangeToTarget() < 0)) {
            if (source.riding() != null) {
                if ((target == source.riding()) || (source.riding().amRiding(target))) source.setAtRange(0); else if ((source.riding() instanceof MOB) && (((MOB) source.riding()).isInCombat()) && (((MOB) source.riding()).getVictim() == target) && (((MOB) source.riding()).rangeToTarget() >= 0) && (((MOB) source.riding()).rangeToTarget() < source.rangeToTarget())) {
                    source.setAtRange(((MOB) source.riding()).rangeToTarget());
                    source.recoverEnvStats();
                    return;
                } else for (int r = 0; r < source.riding().numRiders(); r++) {
                    Rider rider = source.riding().fetchRider(r);
                    if (!(rider instanceof MOB)) continue;
                    MOB otherMOB = (MOB) rider;
                    if ((otherMOB != source) && (otherMOB.isInCombat()) && (otherMOB.getVictim() == target) && (otherMOB.rangeToTarget() >= 0) && (otherMOB.rangeToTarget() < source.rangeToTarget())) {
            ...
            if ((target.getVictim() == source) && (target.rangeToTarget() >= 0)) source.setAtRange(target.rangeToTarget()); else if ((follow != null) && (follow.location() == source.location())) {
                int newRange = follow.fetchFollowerOrder(source);
                if (newRange < 0) {
                    if (follow.rangeToTarget() >= 0) {
                        newRange = follow.rangeToTarget();
                        if (newRange < source.maxRange(tool)) newRange = source.maxRange(tool);
                    } else newRange = source.maxRange(tool);
                } else {
                    if (follow.rangeToTarget() >= 0) newRange = newRange + follow.rangeToTarget();
                }
                if ((source.location() != null) && (source.location().maxRange() < newRange)) newRange = source.location().maxRange();
                source.setAtRange(newRange);
            } else source.setAtRange(source.maxRange(tool));
            source.recoverEnvStats();
        }
    }

    protected void subtickAttack(MOB fighter, Item weapon, int folrange) {
        if ((weapon != null) && (weapon.amWearingAt(Wearable.IN_INVENTORY))) weapon = fighter.fetchWieldedItem();
        if ((!CMath.bset(fighter.getBitmap(), MOB.ATT_AUTOMELEE))) postAttack(fighter, fighter.getVictim(), weapon); else {
            ...
            if ((folrange >= 0) && (fighter.rangeToTarget() >= 0) && (folrange != fighter.rangeToTarget())) {
                if (fighter.rangeToTarget() < folrange) inminrange = false; else if (fighter.rangeToTarget() > folrange) {
                    inminrange = true;
                    inmaxrange = false;
            ...
            if ((!inminrange) && (fighter.curState().getMovement() >= 25)) {
                ...
                if (fighter.location().okMessage(fighter, msg)) fighter.location().send(fighter, msg);
            } else if (inminrange && inmaxrange && ((weapon != null) || (fighter.rangeToTarget() == 0))) postAttack(fighter, fighter.getVictim(), weapon);
        }
    }

    protected void subtickBeforeAttack(MOB fighter) {
        while ((CMProps.getIntVar(CMProps.SYSTEMI_COMBATSYSTEM) == CombatLibrary.COMBAT_QUEUE) && (!fighter.amDead()) && (fighter.dequeCommand())) ;
    }

    protected void subtickAfterAttack(MOB fighter) {
        MOB target = fighter.getVictim();
        if ((target != null) && (fighter.isMonster()) && (target.isMonster()) && (CMLib.dice().rollPercentage() == 1) && ((fighter.amFollowing() == null) || (fighter.amFollowing().isMonster())) && (!target.amDead()) && (fighter.location() != null)) {
            MOB M = null;
            Room R = fighter.location();
            MOB nextVictimM = null;
            for (int m = 0; m < R.numInhabitants(); m++) {
                M = R.fetchInhabitant(m);
                if ((M != null) && (!M.isMonster()) && (M.getVictim() == fighter) && ((nextVictimM == null) || (M.rangeToTarget() < nextVictimM.rangeToTarget()))) nextVictimM = M;
            }
            if ((nextVictimM != null) && (nextVictimM.isInCombat())) fighter.setVictim(nextVictimM);
        }
    }
    ...
    public void tickCombat(MOB fighter) {
        ...
        int folrange = (CMath.bset(fighter.getBitmap(), MOB.ATT_AUTOMELEE) && (fighter.amFollowing() != null) && (fighter.amFollowing().getVictim() == fighter.getVictim()) && (fighter.amFollowing().rangeToTarget() >= 0) && (fighter.amFollowing().fetchFollowerOrder(fighter) >= 0)) ? fighter.amFollowing().fetchFollowerOrder(fighter) + fighter.amFollowing().rangeToTarget() : -1;
        if (CMLib.flags().aliveAwakeMobile(fighter, true)) {
            if ((combatSystem != CombatLibrary.COMBAT_MANUAL) || (fighter.isMonster())) {
                int numAttacks = (int) Math.round(Math.floor(fighter.actions())) - saveAction;
                if ((combatSystem == CombatLibrary.COMBAT_DEFAULT) && (numAttacks > (int) Math.round(Math.floor(fighter.envStats().speed() + 0.9)))) numAttacks = (int) Math.round(Math.floor(fighter.envStats().speed() + 0.9));
                for (int s = 0; s < numAttacks; s++) {
                    if ((!fighter.amDead()) && (fighter.curState().getHitPoints() > 0) && (fighter.isInCombat()) && (fighter.actions() >= 1.0) && ((s == 0) || (CMLib.flags().isStanding(fighter)))) {
                        fighter.setActions(fighter.actions() - 1.0);
                        subtickAttack(fighter, weapon, folrange);
                    } else break;
                }
            }
            if (CMLib.dice().rollPercentage() > (fighter.charStats().getStat(CharStats.STAT_CONSTITUTION) * 4)) fighter.curState().adjMovement(-1, fighter.maxState());
        }
    ...
    public boolean isKnockedOutUponDeath(MOB mob, MOB fighting) {
        ...
        if (((mob.isMonster()) || (mob.soulMate() != null))) whatToDo = CMProps.getVar(CMProps.SYSTEM_MOBDEATH).toUpperCase(); else whatToDo = CMProps.getVar(CMProps.SYSTEM_PLAYERDEATH).toUpperCase();
        ...
        for (int w = 0; w < whatsToDo.size(); w++) {
            ...
            if (whatToDo.startsWith("OUT ") && (CMath.isMathExpression(whatToDo.substring(4).trim(), fakeVarVals))) return true;
        }
        return false;
    }
    ...
    public boolean handleConsequences(MOB mob, MOB fighting, String[] commands, int[] lostExperience, String message) {
        ...
        double[] varVals = { mob.baseEnvStats().level() > mob.envStats().level() ? mob.baseEnvStats().level() : mob.envStats().level(), (fighting != null) ? fighting.envStats().level() : 0, rejuv };
        for (int w = 0; w < commands.length; w++) {
            ...
            if (whatToDo.startsWith("UNL")) {
                ...
                int times = 1;
                if ((V.size() > 1) && (CMath.s_int((String) V.lastElement()) > 1)) times = CMath.s_int((String) V.lastElement());
                for (int t = 0; t < times; t++) CMLib.leveler().unLevel(mob);
            } else if (whatToDo.startsWith("RECALL")) mob.killMeDead(false); else if (whatToDo.startsWith("ASTR")) {
                ...
                if ((A != null) && (mob.fetchAbility(A.ID()) == null)) {
                    ...
                    } else mob.tell("^HFind your corpse have someone resurrect it.^N");
                    mob.addAbility(A);
            ...
            } else if (CMath.isMathExpression(whatToDo, varVals)) {
