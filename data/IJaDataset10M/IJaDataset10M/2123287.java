...
public class OverlayLayout implements LayoutManager2, Serializable {
   ...
   */
    public Dimension preferredLayoutSize(Container target) {
        if (target != this.target) throw new AWTError("OverlayLayout can't be shared");
   ...
   */
    public void layoutContainer(Container target) {
        ...
        for (int i = 0; i < children.length; i++) children[i].setBounds(offsetsX[i], offsetsY[i], spansX[i], spansY[i]);
    }
        ...
        if (xChildren == null || yChildren == null) {
            ...
            for (int i = 0; i < children.length; i++) {
                if (!children[i].isVisible()) {
                    ...
                    xChildren[i] = new SizeRequirements(children[i].getMinimumSize().width, children[i].getPreferredSize().width, children[i].getMaximumSize().width, children[i].getAlignmentX());
                    yChildren[i] = new SizeRequirements(children[i].getMinimumSize().height, children[i].getPreferredSize().height, children[i].getMaximumSize().height, children[i].getAlignmentY());
                }
            }
        }
    }
   ...
   */
    private void checkLayout() {
        if (offsetsX == null || offsetsY == null || spansX == null || spansY == null) {
            checkRequirements();
            checkTotalRequirements();
            ...
            SizeRequirements.calculateAlignedPositions(width, xTotal, xChildren, offsetsX, spansX);
            SizeRequirements.calculateAlignedPositions(height, yTotal, yChildren, offsetsY, spansY);
        }
    }
}
