...
public class DotUtils {
    ...
    public static Color readColor(String v) {
        if (v.equals("none") || v.equals("transparent")) return null;
        try {
    ...
    public static Spline readSpline(String v) {
        ...
        for (int i = 0; i < points.length; i++) {
            if (!points[i].trim().equals("")) readSplinePoint(points[i], spline);
        }
        return spline;
    }
    ...
    public static void readSplinePoint(String v, Spline s) {
        ...
        } else s.addControls(readPoint(v));
    }
    ...
    public static void readStyle(Style s, String v) {
        ...
        for (int i = 0; i < styles.length; i++) {
            ...
            if (found) s.setStyle(value, true); else System.err.println("Style " + styles[i] + " unknown");
        }
    }
    ...
    public static Color[] readColors(String v) {
        ...
        for (int i = 0; i < clrs.length; i++) colors[i] = readColor(clrs[i]);
        return colors;
    }
    ...
    public static void expandRects(Record r) {
        ...
        if (r.rects != null && r.subRecords != null) {
            ...
            for (int i = 0; i < r.rects.length; i++) rects.add(r.rects[i]);
    ...
    private static Vector expandRects(SubRecord sub, Vector rects) {
        if (sub.label != null) if (!sub.label.equals("")) {
            sub.rect = (Rectangle) rects.remove(0);
        }
        if (sub.subRecords != null) for (int i = 0; i < sub.subRecords.length; i++) {
    ...
    public static void readShape(BasicNode basicNode, String v) {
        ...
        if (found) {
            ...
            if (value == BasicNode.POINT) basicNode.color = Color.black;
        } else {
            if (v.equals("box") || v.equals("rect") || v.equals("rectangle") || v.equals("square")) {
                basicNode.shape = BasicNode.POLYGON;
                basicNode.sides = 4;
            } else if (v.endsWith("circle")) {
                basicNode.shape = BasicNode.ELLIPSE;
                basicNode.regular = true;
                if (v.equals("doublecircle")) basicNode.peripheries = 2; else basicNode.peripheries = 1;
            } else if (v.equals("triangle")) {
                basicNode.shape = BasicNode.POLYGON;
                basicNode.sides = 3;
            } else if (v.equals("invtriangle")) basicNode.shape = BasicNode.INVTRIANGLE; else if (v.equals("plaintext") || v.equals("none")) basicNode.shape = BasicNode.NONE; else if (v.equals("diamond")) {
                basicNode.shape = BasicNode.POLYGON;
                basicNode.sides = 4;
                basicNode.regular = true;
                basicNode.rotate = 45;
            } else if (v.equals("trapezium")) basicNode.shape = BasicNode.TRAPEZIUM; else if (v.equals("invtrapezium")) basicNode.shape = BasicNode.INVTRAPEZIUM; else if (v.equals("parallelogram")) {
                basicNode.shape = BasicNode.POLYGON;
                basicNode.sides = 4;
                basicNode.skew = 0.6;
            } else if (v.equals("house")) basicNode.shape = BasicNode.HOUSE; else if (v.equals("invhouse")) basicNode.shape = BasicNode.INVHOUSE; else if (v.endsWith("octagon")) {
                basicNode.shape = BasicNode.POLYGON;
                basicNode.sides = 8;
                if (v.equals("doubleoctagon")) basicNode.peripheries = 2; else if (v.equals("tripleoctagon")) basicNode.peripheries = 3; else basicNode.peripheries = 1;
            } else if (v.endsWith("agon")) {
                basicNode.shape = BasicNode.POLYGON;
                if (v.startsWith("pent")) basicNode.sides = 5;
                if (v.startsWith("hex")) basicNode.sides = 6;
                if (v.startsWith("sept")) basicNode.sides = 7;
            } else System.err.println(v + " is not a BasicNode shape");
        }
    }

    public static void readShape(Record record, String v) {
        if (v.endsWith("record")) record.setRounded(v.charAt(0) == 'M');
    }
