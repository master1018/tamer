...
public class QueryTag extends MakumbaTag implements IterationTag {
    ...
    protected void onlyOuterListArgument(String s) throws JspException {
        ...
        while (t != null && t instanceof ObjectTag) t = (QueryTag) findAncestorWithClass(t, QueryTag.class);
        if (t instanceof QueryTag) treatException(new MakumbaJspException(this, "the " + s + " parameter can only be set for the outermost mak:list tag"));
    }
    ...
    public void setTagKey(MakumbaJspAnalyzer.PageCache pageCache) {
        ...
        for (int i = 0; i < queryProps.length; i++) tagKey.setAt(queryProps[i], i);
    ...
    public void doStartAnalyze(MakumbaJspAnalyzer.PageCache pageCache) {
        ...
        if (countVar != null) pageCache.types.setType(countVar, MakumbaSystem.makeFieldOfType(countVar, "int"), this);
        if (maxCountVar != null) pageCache.types.setType(maxCountVar, MakumbaSystem.makeFieldOfType(maxCountVar, "int"), this);
    }
    ...
    public int doMakumbaStartTag(MakumbaJspAnalyzer.PageCache pageCache) throws LogicException, JspException {
        if (getParentList() == null) QueryExecution.startListGroup(pageContext); else {
        ...
        if (n > 0) {
            if (countVar != null) pageContext.setAttribute(countVar, one);
            pageContext.getRequest().setAttribute(standardCountVar, one);
            return EVAL_BODY_INCLUDE;
        }
        if (countVar != null) pageContext.setAttribute(countVar, zero);
    ...
    protected void setNumberOfIterations(int n) throws JspException {
        ...
        if (maxCountVar != null) pageContext.setAttribute(maxCountVar, cnt);
    ...
    public int doAfterBody() throws JspException {
        ...
        try {
            ...
            if (n != -1) {
                ...
                if (countVar != null) pageContext.setAttribute(countVar, cnt);
    ...
    public int doMakumbaEndTag(MakumbaJspAnalyzer.PageCache pageCache) throws JspException {
        pageContext.getRequest().setAttribute(standardLastCountVar, pageContext.getRequest().getAttribute(standardMaxCountVar));
        ...
        if (getParentList() == null) QueryExecution.endListGroup(pageContext);
        execution = null;
