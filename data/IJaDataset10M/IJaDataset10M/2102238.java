...
public class Server extends Thread {
    ...
    public static Hashtable<Integer, InetAddress> viewers = new Hashtable<Integer, InetAddress>();

    public static void Start() {
        idle = false;
        running = false;
        Config.loadConfiguration();
        if (Config.default_address) System.setProperty("java.rmi.server.hostname", Config.server_address); else System.getProperties().remove("java.rmi.server.hostname");
        try {
    ...
    public static void Stop() {
        ...
        } else SysTray.updateServerStatus(SysTray.CONNECTION_FAILED);
        try {
    ...
    public static void disconnectAllViewers() {
        ...
        while (viewerEnum.hasMoreElements()) removeViewer(viewerEnum.nextElement());
    }

    public static ArrayList<InetAddress> getViewersAds() {
        ArrayList<InetAddress> viewersAds = new ArrayList<InetAddress>();
        for (int i = 0; i < viewers.size(); i++) viewersAds.add(viewers.get(i));
        return viewersAds;
    }
