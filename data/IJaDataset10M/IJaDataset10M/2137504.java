...
public class NGramProfiles {
    ...
    public static final int CHECK_REPEAT = 1000;

    public static final String NOLANGNAME = "--";

    public static final char END_CHAR = (char) 0;
    ...
    public static final double LOWSTATSAFETY = 0.8;
    ...
    protected void init(BufferedReader br) throws IOException {
        ...
        while ((line = br.readLine()) != null) {
            ...
            InputStream is = getClass().getResourceAsStream(line + "." + NGramProfile.NGRAM_PROFILE_EXTENSION);
    ...
    public void info() {
        ...
        for (int i = 0; i < ngs.length; i++) {
            ...
            for (int k = 0; k < profiles.size(); k++) ranks[i].d += vals[i][k] * vals[i][k];
        }
        Arrays.sort(ranks);
        for (int i = 0; i < ranks.length; i++) {
            ...
            System.out.println(" |" + count + " " + (count * (profiles.size() - count)) + " ?? " + t2.getId() + " " + ngs[t2.getId()]);
        }
    ...
    public Ranker getRanker() {
        ...
        synchronized (profiles) {
            if (myTrie == null) {
                ...
                for (int i = 0; i < ngs.length; i++) {
                    CharTrie ct = myTrie.subtrie(ngs[i]);
                    if (i != ct.getId()) System.err.println("PANIK! " + i + ".: <" + ngs[i] + "> " + ct.getId());
                    ngs[i] = reverse(ngs[i]);
                }
        ...
        return new Ranker() {
            ...
            public void account(CharSequence seq) {
                for (int i = 0; i < seq.length(); i++) account(seq, i);
            }

            public void account(Reader reader) throws IOException {
                BufferedReader br;
                if (reader instanceof BufferedReader) br = (BufferedReader) reader; else br = new BufferedReader(reader);
                String line;
                while ((line = br.readLine()) != null) {
                    account(line);
                }
    ...
    public class Pair implements Comparable {
        ...
        public int compareTo(Object o) {
            Pair p2 = (Pair) o;
            double h = d - p2.d;
            if (h < 0.0) return +1; else if (h > 0.0) return -1;
    ...
    public String getProfileName(int i) {
        if (i < 0 || i >= profiles.size()) return NOLANGNAME;
    ...
    public static String reverse(CharSequence seq) {
        ...
        for (int i = 0, j = seq.length(); --j >= 0; i++) chrs[i] = seq.charAt(j);
   ...
   */
    public RankResult rank(NGramMetric metric, NGramProfile profile) {
        ...
        for (int i = 0; i < profiles.size(); i++) scores[i] = metric.diff(profile, (NGramProfile) (profiles.get(i)));
    ...
    private class SimpleRankResult implements RankResult {
        ...
        public double getScore(int pos) {
            if (pos == getLength()) return remain;
            if (pos < 0) pos += getLength();
            return scores[pos];
        }

        public String getName(int pos) {
            if (pos == getLength()) return NOLANGNAME; else if (pos < 0) pos += getLength();
