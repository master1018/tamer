...
public class LICM extends CompilerPhase {
   ...
   */
    public static boolean shouldMove(Instruction inst, IR ir) {
        ...
        if (ir.IRStage != IR.HIR && ((inst.isPEI()) || inst.isThrow() || inst.isImplicitLoad() || inst.isImplicitStore())) {
            return false;
        }
   ...
   */
    private Instruction scheduleScalarDefsEarly(OperandEnumeration e, Instruction earlyPos, Instruction inst) {
        while (e.hasMoreElements()) {
            Operand op = e.next();
   ...
   */
    Instruction definingInstruction(Operand op) {
        if (op instanceof HeapOperand) {
            @SuppressWarnings("unchecked") HeapOperand<Object> hop = (HeapOperand) op;
    ...
    BasicBlock upto(Instruction earlyPos, BasicBlock lateBlock, Instruction inst) {
        ...
        if (VM.VerifyAssertions) {
            if (!dominator.dominates(earlyBlock.getNumber(), _origBlock.getNumber()) || !dominator.dominates(earlyBlock.getNumber(), lateBlock.getNumber())) {
                ...
                VM.sysWrite("> " + earlyBlock.getNumber() + ", " + _origBlock.getNumber() + ", " + lateBlock.getNumber() + "\n");
                VM.sysWrite("" + inst + "\n");
            }
   ...
   */
    ...
    private boolean simplify(Instruction inst, BasicBlock block) {
        if (!Phi.conforms(inst)) return false;
