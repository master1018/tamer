...
public class TeleatlasIvtcheuMerger {
    ...
    private static final Logger log = Logger.getLogger(TeleatlasIvtcheuMerger.class);
    ...
    private static final void deleteLinks(Network networkIvtcheu, String il2deletefile) {
        ...
        try {
            ...
            while ((curr_line = br.readLine()) != null) {
                ...
                if (l == null) {
                    throw new RuntimeException(lineCnt + ": link with id=" + lid + " not found.");
                }
                if (networkIvtcheu.removeLink(l.getId()) != null) {
                    throw new RuntimeException(lineCnt + ": could not remove link with id=" + lid + ".");
                }
                lineCnt++;
            }
            br.close();
    ...
    private static final void mergeNetworks(NetworkImpl networkTeleatlas, NetworkImpl networkIvtcheu, String i2tmappingfile) {
        ...
        try {
            ...
            while ((curr_line = br.readLine()) != null) {
                ...
                if (nodeMapping.put(iNid, tNid) != null) {
                    throw new RuntimeException(lineCnt + ": ivtch node id=" + iNid + " already exists.");
                }
                lineCnt++;
            }
            br.close();
        ...
        if (nodeMapCnt != nodeMapping.size()) {
        ...
        for (Link l : networkIvtcheu.getLinks().values()) {
            ...
            if (nodeMapping.keySet().contains(fromNodeId) && nodeMapping.keySet().contains(toNodeId)) {
                ...
                throw new RuntimeException("HAEH?");
            }
            networkTeleatlas.createAndAddLink(l.getId(), networkTeleatlas.getNodes().get(fromNodeId), networkTeleatlas.getNodes().get(toNodeId), l.getLength(), l.getFreespeed(), l.getCapacity() / 10.0, l.getNumberOfLanes(), ((LinkImpl) l).getOrigId(), ((LinkImpl) l).getType());
        }
        log.info("  number of lines processed: " + lineCnt);
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 5) {
            log.info("wrong number of params");
            return;
        }
