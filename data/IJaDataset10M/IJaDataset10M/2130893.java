...
class LwsFrame extends TextfileParser {
    ...
    LwsFrame(LwsFrame prevFrame, LwsFrame nextFrame, double ratio) {
        ...
        heading = prevFrame.heading + (nextFrame.heading - prevFrame.heading) * ratio;
        pitch = prevFrame.pitch + (nextFrame.pitch - prevFrame.pitch) * ratio;
        bank = prevFrame.bank + (nextFrame.bank - prevFrame.bank) * ratio;
        xScale = prevFrame.xScale + (nextFrame.xScale - prevFrame.xScale) * ratio;
        yScale = prevFrame.yScale + (nextFrame.yScale - prevFrame.yScale) * ratio;
        zScale = prevFrame.zScale + (nextFrame.zScale - prevFrame.zScale) * ratio;
        frameNumber = prevFrame.frameNumber + (nextFrame.frameNumber - prevFrame.frameNumber) * ratio;
    ...
    LwsFrame(LwsFrame prevFrame, LwsFrame frame1, LwsFrame frame2, LwsFrame nextFrame, double u, double adj0, double adj1) {
        ...
        h3 = u3 - u2 - u2 + u;
        h4 = u3 - u2;
        dd0a = (1.0f - frame1.tension) * (1.0f + frame1.continuity) * (1.0f + frame1.bias);
        dd0b = (1.0f - frame1.tension) * (1.0f - frame1.continuity) * (1.0f - frame1.bias);
        ds1a = (1.0f - frame2.tension) * (1.0f - frame2.continuity) * (1.0f + frame2.bias);
        ds1b = (1.0f - frame2.tension) * (1.0f + frame2.continuity) * (1.0f - frame2.bias);
        double[] v = new double[4];
        v[0] = prevFrame.x;
        v[1] = frame1.x;
        v[2] = frame2.x;
        v[3] = nextFrame.x;
        x = computeInterpolation(v, dd0a, dd0b, ds1a, ds1b, adj0, adj1, h1, h2, h3, h4);
        v[0] = prevFrame.y;
        v[1] = frame1.y;
        v[2] = frame2.y;
        v[3] = nextFrame.y;
        y = computeInterpolation(v, dd0a, dd0b, ds1a, ds1b, adj0, adj1, h1, h2, h3, h4);
        v[0] = prevFrame.z;
        v[1] = frame1.z;
        v[2] = frame2.z;
        v[3] = nextFrame.z;
        z = computeInterpolation(v, dd0a, dd0b, ds1a, ds1b, adj0, adj1, h1, h2, h3, h4);
        v[0] = prevFrame.heading;
        v[1] = frame1.heading;
        v[2] = frame2.heading;
        v[3] = nextFrame.heading;
        heading = computeInterpolation(v, dd0a, dd0b, ds1a, ds1b, adj0, adj1, h1, h2, h3, h4);
        v[0] = prevFrame.pitch;
        v[1] = frame1.pitch;
        v[2] = frame2.pitch;
        v[3] = nextFrame.pitch;
        pitch = computeInterpolation(v, dd0a, dd0b, ds1a, ds1b, adj0, adj1, h1, h2, h3, h4);
        v[0] = prevFrame.bank;
        v[1] = frame1.bank;
        v[2] = frame2.bank;
        v[3] = nextFrame.bank;
        bank = computeInterpolation(v, dd0a, dd0b, ds1a, ds1b, adj0, adj1, h1, h2, h3, h4);
        ...
        frameNumber = frame1.frameNumber + (frame2.frameNumber - frame1.frameNumber) * u;
    ...
    double computeInterpolation(double[] value, double dd0a, double dd0b, double ds1a, double ds1b, double adj0, double adj1, double h1, double h2, double h3, double h4) {
        ...
        double result;
        if (adj0 < -0.0001 || adj0 > 0.0001) dd0 = adj0 * (dd0a * (value[1] - value[0]) + dd0b * delta); else dd0 = 0.5f * (dd0a + dd0b) * delta;
        if (adj1 < -0.0001 || adj1 > 0.0001) ds1 = adj1 * (ds1a * delta + ds1b * (value[3] - value[2])); else ds1 = 0.5f * (ds1a + ds1b) * delta;
    ...
    void setRotationMatrix(Matrix4d mat) {
        ...
        debugOutputLn(VALUES, " p, h, b = " + pitch + ", " + heading + ", " + bank);
    ...
    Point3f getScale() {
        if ((xScale < -0.0001 || xScale > 0.0001) && (yScale < -0.0001 || yScale > 0.0001) && (zScale < -0.0001 || zScale > 0.0001)) {
