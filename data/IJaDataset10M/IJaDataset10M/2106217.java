...
public class Contributor extends ContributorAgent {
    ...
    public Object getContribution() {
        return "It's " + System.currentTimeMillis() + " here at " + name;
    }
    ...
    public void collectiveUpdated(ContributionBox peer) {
        Iterator<ContributionBox> contributions = collective.getContributions().iterator();
        while (contributions.hasNext()) {
            ContributionBox cb = contributions.next();
            System.out.println("Contribution from " + cb.contributor.getHost().toString() + "\n" + cb.contribution + "\n");
        }
    }

    public void run() {
        if (root != null) return;
        Iterator<ContributionBox> contributions = getDRM().getContributions().iterator();
        while (contributions.hasNext()) {
            Address target = contributions.next().contributor;
            Contributor child = new Contributor(job, "" + Math.random(), new Address(null, -1, this.name));
            ...
            if (r.getStatus() == IRequest.DONE) System.out.println("Contributor sent to " + target.getHost().toString()); else System.err.println("Contributor couldn't arrive to " + target.getHost().toString());
        }
    }
}
