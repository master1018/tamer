    ...
    public Sequence[] getSequences() throws SequenceSourceException {
        if (sequences == null) {
            ...
            try {
                ...
                throw new SequenceSourceException("Error loading alignment file " + alignmentFile.getAbsolutePath(), e);
            } finally {
                if (inStream != null) {
                    try {
                        inStream.close();
                    } catch (Exception e2) {
                        throw new SequenceSourceException("Error closing alignment file " + alignmentFile.getAbsolutePath(), e2);
                    }
                }
            }
        }
