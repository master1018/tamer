...
public class JvxlReader extends JvxlXmlReader {
    ...
    protected void readParameters() throws Exception {
        ...
        if (line == null || line.length() == 0) line = "Line 1";
        jvxlFileHeaderBuffer.append(line).append('\n');
        if (readLine() == null || line.length() == 0) line = "Line 2";
        jvxlFileHeaderBuffer.append(line).append('\n');
        ...
        if (atomCount == Integer.MIN_VALUE) return;
        ...
        readVoxelVector(2);
        skipComments(true);
        for (int i = 0; i < atomCount; ++i) jvxlFileHeaderBuffer.append(readLine() + "\n");
        Logger.info("Reading extra JVXL information line: " + line);
        nSurfaces = parseInt(line);
        if (!(isJvxl = (nSurfaces < 0))) return;
        nSurfaces = -nSurfaces;
    ...
    protected void gotoData(int n, int nPoints) throws Exception {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Logger.info("JVXL skipping: jvxlSurfaceDataCount=" + surfaceDataCount + " jvxlEdgeDataCount=" + edgeDataCount + " jvxlDataIsColorMapped=" + jvxlDataIsColorMapped);
    ...
    private void jvxlReadDefinitionLine(boolean showMsg) throws Exception {
        ...
        if (showMsg) Logger.info("reading jvxl data set: " + comment + line);
        ...
        int param2 = parseInt();
        int param3 = parseInt();
        if (param3 == Integer.MIN_VALUE || param3 == -1) param3 = 0;
        if (param1 == -1) {
            try {
                params.thePlane = new Point4f(parseFloat(), parseFloat(), parseFloat(), parseFloat());
            } catch (Exception e) {
        ...
        jvxlData.isJvxlPrecisionColor = (param1 == -1 && param2 == -2 || param3 < 0);
        ...
        if (param1 == -1) edgeDataCount = 0; else edgeDataCount = (param2 < -1 ? -param2 : param2 > 0 ? param2 : 0);
        colorDataCount = (params.isBicolorMap ? -param2 : param3 < -1 ? -param3 : param3 > 0 ? param3 : 0);
    ...
    protected void readSurfaceData(boolean isMapDataIgnored) throws Exception {
        ...
        if (readSurfaceData()) return;
        readVolumeFileSurfaceData();
    }

    @Override
    protected void jvxlSkipData(int nPoints, boolean doSkipColorData) throws Exception {
        if (surfaceDataCount > 0) jvxlSkipDataBlock(nPoints, true);
        if (edgeDataCount > 0) jvxlSkipDataBlock(edgeDataCount, false);
        if (jvxlDataIsColorMapped && doSkipColorData) jvxlSkipDataBlock(colorDataCount, false);
    }
