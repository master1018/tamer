...
public class CmsTemplateLayout extends CmsJspActionElement {
    ...
    public static final String VFS_PATH_CSS_DEAFULT_MAIN_NAV = CmsWorkplace.VFS_PATH_MODULES + MODULE_NAME + "/resources/menus/style2/style.css";

    /** The absolute VFS path to the css of the left navigation. */
    public static final String VFS_PATH_CSS_LEFT_NAV = CmsWorkplace.VFS_PATH_MODULES + MODULE_NAME + "/resources/css/nav_left.css";
    ...
    private String getConfigPreset(CmsXmlContent config) {
        String result = null;
        if (config != null) {
            Locale locale = OpenCms.getLocaleManager().getDefaultLocale(getCmsObject(), getCmsObject().getSitePath(m_config.getFile()));
