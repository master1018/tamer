...
public final class HEPsimulator extends Frame implements Runnable {
    ...
    public void run() {
        ...
        if (ingoing[1] != null) uni.addParticle(ingoing[1]);
        try {
            for (int i = 0; i < particles.length; i++) {
                ...
                for (int j = i; j < particles.length; j++) {
                    ...
                    if (uni.conserve(outgoing[0], outgoing[1])) new ResultsDialog(this, ingoing, outgoing, uni.interact()).show();
                }
            }
        } catch (Exception e) {
        }
...
class SetupDialog extends Dialog {
    ...
    public SetupDialog(Frame f, QuantumParticle qpArray[]) {
        ...
        energyA.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if (Double.valueOf(energyA.getText()).doubleValue() < array[particleA.getSelectedIndex()].restMass()) energyA.setText(Double.toString(array[particleA.getSelectedIndex()].restMass()));
            }
        });
        energyB.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent evt) {
                if (Double.valueOf(energyB.getText()).doubleValue() < array[particleB.getSelectedIndex()].restMass()) energyB.setText(Double.toString(array[particleB.getSelectedIndex()].restMass()));
            }
        });
        ok.addActionListener(new ActionListener() {
...
class ResultsDialog extends Dialog {
    ...
    public ResultsDialog(Frame f, QuantumParticle in[], QuantumParticle out[], String interact) {
        ...
        if (in[1] == null) add(new Label(in[0].toString() + "------>")); else add(new Label(in[0].toString() + " --->  <--- " + in[1].toString()));
...
final class CMframe {
    ...
    /**
        * Interaction codes.
        */
    private static final int STRONG = 1, WEAK = 2, EM = 3;
    ...
    public boolean conserve(QuantumParticle a, QuantumParticle b) {
        double Ei, Ej, pi, pj;
        if (a.charge() + b.charge() == Q && a.eLeptonQN() + b.eLeptonQN() == Le && a.muLeptonQN() + b.muLeptonQN() == Lm && a.tauLeptonQN() + b.tauLeptonQN() == Lt && a.baryonQN() + b.baryonQN() == B && Math.abs(S - a.strangeQN() - b.strangeQN()) <= 1) {
        ...
        if (interaction > 0) {
            ...
            pi = (energy * energy * energy * energy + massA * massA * massA * massA + massB * massB * massB * massB - 2.0 * massA * massA * energy * energy - 2.0 * massB * massB * energy * energy - 2.0 * massA * massA * massB * massB);
    ...
    public void addParticle(QuantumParticle qp) {
        ...
        energy += qp.momentum.getComponent(0) * 1000.0;
    }
    ...
    public void setRelCMvel(QuantumParticle a, QuantumParticle b) {
        if (b == null) relCMvel = a.momentum.getComponent(1) / a.momentum.getComponent(0); else relCMvel = (a.momentum.getComponent(1) + b.momentum.getComponent(1)) / (a.momentum.getComponent(0) + b.momentum.getComponent(0));
    }
