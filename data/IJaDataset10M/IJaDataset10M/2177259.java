...
public class PlaylistUtils {
    ...
    public void filesToPlaylist(final Context context, final String path, final String name) {
        ...
        progressDialog = ProgressDialog.show(context, "Please wait", "Scanning module files...", true);
        new Thread() {
            ...
            public void run() {
                ...
                for (File file : modDir.listFiles(new ModFilter())) {
                    ...
                    list.add(filename + ":" + mi.chn + " chn " + mi.type + ":" + mi.name);
                    num++;
                }
                if (num > 0) addToList(context, name, list.toArray(new String[num]));
    ...
    public static String readComment(Context context, String name) {
        ...
        if (comment == null || comment.trim().length() == 0) comment = context.getString(R.string.no_comment);
        return comment;
    }
