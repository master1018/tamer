...
public class Treerow extends XulElement implements org.zkoss.zul.api.Treerow {
    ...
    protected String getRealSclass() {
        ...
        final String added = ti != null ? ti.isDisabled() ? getZclass() + "-disd" : ti.isSelected() ? getZclass() + "-seld" : "" : "";
    ...
    protected boolean isAsapRequired(String evtnm) {
        if (!Events.ON_OPEN.equals(evtnm)) return super.isAsapRequired(evtnm);
        final Treeitem ti = (Treeitem) getParent();
    ...
    public String getOuterAttrs() {
        ...
        if (tree != null && tree.getName() != null) HTMLs.appendAttribute(sb, "z.value", Objects.toString(item.getValue()));
        HTMLs.appendAttribute(sb, "z.pitem", item.getUuid());
        if (item.isSelected()) HTMLs.appendAttribute(sb, "z.sel", true);
        if (item.isDisabled()) HTMLs.appendAttribute(sb, "z.disd", true);
        HTMLs.appendAttribute(sb, "z.rid", tree.getUuid());
        if (item.isContainer() && item.isOpen()) HTMLs.appendAttribute(sb, "z.open", true);
        final Component gp = item.getParent();
        if (gp != null) {
            HTMLs.appendAttribute(sb, "z.ptch", gp.getUuid());
            Component gpitem = gp.getParent();
            if (gpitem instanceof Treeitem) HTMLs.appendAttribute(sb, "z.gpitem", gpitem.getUuid());
        }
        final Treechildren tcsib = getLinkedTreechildren();
        if (tcsib != null) {
            HTMLs.appendAttribute(sb, "z.tchsib", tcsib.getUuid());
        }
        if (tree != null && tree.getModel() != null && !item.isLoaded()) sb.append(" z.lod=\"t\"");
        if (getTree().inPagingMold()) HTMLs.appendAttribute(sb, "z." + Events.ON_OPEN, true); else appendAsapAttr(sb, Events.ON_OPEN);
    ...
    public boolean isVisible() {
        ...
        if (!(comp instanceof Treeitem)) return true;
        if (!comp.isVisible()) return false;
        comp = comp.getParent();
        return !(comp instanceof Treechildren) || ((Treechildren) comp).isVisible();
    }
    ...
    public boolean setVisible(boolean visible) {
        if (isVisible() != visible) smartUpdate("z.visible", visible);
        return super.setVisible(visible);
    }

    public void beforeParentChanged(Component parent) {
        if (parent != null && !(parent instanceof Treeitem)) throw new UiException("Wrong parent: " + parent);
        super.beforeParentChanged(parent);
    }

    public void beforeChildAdded(Component child, Component refChild) {
        if (!(child instanceof Treecell)) throw new UiException("Unsupported child for tree row: " + child);
