...
public class GeneratePays extends CalloutEngine {
    ...
    private boolean checkAutoPeriods() {
        ...
        String sql = "SELECT name " + "FROM c_period " + "WHERE c_year_id=? ";
    ...
    private boolean checkGeneratedPaids() {
        String sql = "SELECT count(*) " + "FROM c_bpartner_pay " + "WHERE c_year_id=? " + "AND c_calendar_id=?" + "AND c_bpartner_salary_id=?";
        PreparedStatement pstmt = DB.prepareStatement(sql);
        try {
            pstmt.setInt(1, Year_ID);
    ...
    private void GenerateExist() {
        X_C_BPartner_Salary Salary = new X_C_BPartner_Salary(ctx, Salary_ID, null);
        int PayNum = 0;
        String sql = "" + "SELECT count(*) " + "FROM c_bpartner_pay Pa " + "WHERE Pa.c_bpartner_salary_id=? " + "AND Pa.c_year_id=? " + "AND Pa.paid='N' " + "AND Pa.c_calendar_id=? ";
        PreparedStatement pstmt = DB.prepareStatement(sql);
        try {
        ...
        sql = "" + "SELECT Pa.c_bpartner_pay_id, Pa.concept, Pe.periodno " + "FROM c_bpartner_pay Pa, c_period Pe " + "WHERE Pa.c_bpartner_salary_id=? " + "AND Pa.c_year_id=? " + "AND Pe.c_period_id=Pa.c_period_id " + "AND Pa.paid='N' " + "AND Pa.c_calendar_id=? ";
    ...
    private void GenerateNew(int nbr) {
        ...
        String sql = "" + "SELECT c_period_id, periodno " + "FROM c_period " + "WHERE c_year_id=? ";
        PreparedStatement pstmt = DB.prepareStatement(sql);
        int summerID = 0;
        int christmasID = 0;
        try {
            ...
            int i = 0;
            while (rs.next()) {
                if (rs.getInt(2) == 7) summerID = rs.getInt(1);
                if (rs.getInt(2) == 12) christmasID = rs.getInt(1);
    ...
    private BigDecimal GenerateMiddlePay(BigDecimal NewPay, BigDecimal OldPay, int day) {
        ...
        BigDecimal Result = NewPayperDay.multiply(BigDecimal.valueOf(30 - day)).add(OldPayperDay.multiply(BigDecimal.valueOf(day)));
        return Result;
    }
