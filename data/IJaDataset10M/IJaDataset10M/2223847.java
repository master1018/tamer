...
public class ComparePlanomat {
    ...
    private static ChartUtil getChart(final Scenario scenarioPlanomat, final ScenarioWithCliques scenarioJPO) {
        ...
        XYLineChart chart = new XYLineChart("average executed score per clique size, iteration 30", "clique size", "score");
        chart.addSeries("planomat", x, yPlanomat);
        chart.addSeries("joint plan optimiser with individual trips", x, yJpo);
        chart.getChart().getXYPlot().setRenderer(new XYLineAndShapeRenderer(true, true));
        return chart;
    }

    private static ChartUtil getLineHistogram(final XYLineHistogramDataset dataset) {
        dataset.setCumulative(false);
        JFreeChart chart = ChartFactory.createXYLineChart("distribution of scores", "score", "frequency", dataset, PlotOrientation.VERTICAL, true, false, false);
        tuneDomainAxis(chart.getXYPlot().getDomainAxis());
        return new WrapperChartUtil(chart);
    }

    private static ChartUtil getCumulative(final XYLineHistogramDataset dataset) {
        dataset.setCumulative(true);
        JFreeChart chart = ChartFactory.createXYLineChart("cumulative distribution of scores", "score", "frequency", dataset, PlotOrientation.VERTICAL, true, false, false);
        tuneDomainAxis(chart.getXYPlot().getDomainAxis());
        return new WrapperChartUtil(chart);
    }
    ...
    private static XYLineHistogramDataset getHistogramDataset(final Scenario scenarioPlanomat, final Scenario scenarioJPO, final String outputPath) {
    ...
    private static XYLineHistogramDataset getHistogramDataset(final List<Double> planomatScores, final List<Double> jpoScores) {
        XYLineHistogramDataset dataset = new XYLineHistogramDataset(BIN_WIDTH);
        dataset.addSeries("planomat", planomatScores);
        dataset.addSeries("joint plan optimisation algorithm", jpoScores);
        return dataset;
    }
    ...
    private static class TupleComparator implements Comparator<Tuple<? extends Comparable, ? extends Object>> {
        ...
        @Override
        public int compare(Tuple<? extends Comparable, ? extends Object> t1, Tuple<? extends Comparable, ? extends Object> t2) {
