...
/**
 * This interface is implemented by servers that export a JavaSpaces(TM)
 * technology service. The operations in this interface are the public methods
 * that all such spaces support.
 * 
...
public interface JavaSpace {
	 ...
	 *            the entry to write
	 * @param txn
	 *            the transaction object, if any, under which to perform the
	 *            write
	 * @param lease
    ...
    /**
	 * Wait for no time at all. This is used as a timeout value in various read
	 * and take calls.
	 * 
	 * @see #read
	 * @see #readIfExists
	 ...
	 *            <code>tmpl</code> with <code>null</code> fields being
	 ...
	 * @param timeout
	 *            How long the client is willing to wait for a transactionally
	 *            proper matching entry. A timeout of <code>NO_WAIT</code> means
	 *            to wait no time at all; this is equivalent to a wait of zero.
	 * 
    ...
    /**
	 * Read any matching entry from the space, returning <code>null</code> if
	 * there is currently is none. Matching and timeouts are done as in
	 ...
	 * @see #read
	 */
    Entry readIfExists(Entry tmpl, Transaction txn, long timeout) throws UnusableEntryException, TransactionException, InterruptedException, RemoteException;

    /**
	 * Take a matching entry from the space, waiting until one exists. Matching
	 * is and timeout done as for <code>read</code>.
	 * 
	 * @param tmpl
	 *            The template used for matching. Matching is done against
	 *            <code>tmpl</code> with <code>null</code> fields being
    ...
    /**
	 * Take a matching entry from the space, returning <code>null</code> if
	 * there is currently is none. Matching is and timeout done as for
	 * <code>read</code>, except that blocking in this call is done only if
	 * necessary to wait for transactional state to settle.
	 * 
	 * @param tmpl
	 ...
	 * <code>listener</code> with a <code>RemoteEvent</code> that includes the
	 * <code>handback</code> object. Matching is done as for <code>read</code>.
	 * 
	 * @param tmpl
	 *            The template used for matching. Matching is done against
	 *            <code>tmpl</code> with <code>null</code> fields being
	 ...
	 *            the requested lease time, in milliseconds
	 * @param handback
	 *            An object to send to the listener as part of the event
	 *            notification.
	 ...
	 *             if a communication error occurs
	 * @throws IllegalArgumentException
	 *             if the lease time requested is not Lease.ANY and is negative
	 * @see #read
	 ...
	 * reasonably avoid this duplicated effort, since they have no efficient way
	 * of checking whether the same template is being used without intervening
	 * modification.
	 * 
	 ...
	 * <code>Entry</code> object that contains a <i>snapshot</i> of the original
	 * entry. Using the returned snapshot entry is equivalent to using the
	 * unmodified original entry in all operations on the same JavaSpaces
	 * service. Modifications to the original entry will not affect the
	 * snapshot. You can <code>snapshot</code> a <code>null</code> template;
	 * <code>snapshot</code> may or may not return null given a
	 * <code>null</code> template.
	 * 
	 * The entry returned from <code>snapshot</code> will be guaranteed
	 * equivalent to the original unmodified object only when used with the
	 * space. Using the snapshot with any other JavaSpaces service will generate
	 * an <code>IllegalArgumentException</code> unless the other space can use
	 * it because of knowledge about the JavaSpaces service that generated the
	 * snapshot. The snapshot will be a different object from the original, may
	 * or may not have the same hash code, and <code>equals</code> may or may
	 * not return <code>true</code> when invoked with the original object, even
	 * if the original object is unmodified.
	 * 
	 * A snapshot is guaranteed to work only within the virtual machine in which
	 * it was generated. If a snapshot is passed to another virtual machine (for
	 * example, in a parameter of an RMI call), using it--even with the same
