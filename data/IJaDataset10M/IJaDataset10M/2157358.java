...
public class Rcl extends JasminCommand {
    ...
    public ParseError validate(Parameters p) {
        ...
        if ((Op.matches(p.type(1), Op.R8) && !(p.argument(1).address == dataspace.CL)) || !Op.matches(p.type(1), Op.R8 | Op.I8)) {
            return new ParseError(p.wholeLine, p.argument(1), "second register must be CL or an 8-bit immediate");
        }
    ...
    public void execute(Parameters p) {
        ...
        if (p.mnemo.startsWith("RC")) {
            p.b = p.b % (bitsize + 1);
            buffersize += 1;
            buffer = (p.get(0) & mask) | ((dataspace.fCarry ? 1L : 0) << bitsize) | ((p.get(0) & shortmask) << (bitsize + 1));
        ...
        if (p.mnemo.endsWith("R")) {
            ...
            dataspace.fCarry = getBit(buffer, (buffersize - p.b - 1) % buffersize);
            if (p.b == 1) {
