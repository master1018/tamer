...
public class ProvaKnowledgeBaseImpl implements ProvaKnowledgeBase {
        ...
        try {
            ...
            for (ProvaRule rule : rules) {
                if (rule.getHead() == null) {
                    ...
                    if (goalResults.length == 0 && rule.getBody().length == 2 && rule.getBody()[1].getPredicate().getArity() != 0) this.getPrintWriter().println("no");
    ...
    public ProvaRuleSet getPredicates(String symbol, int arity) {
        if (arity == -1) {
            ...
            if (predicate != null) return predicate.getClauseSet();
    ...
    public ProvaLiteral generateLiteral(String symbol, ProvaList terms) {
        ...
        if (builtin != null) return new ProvaLiteralImpl(builtin, terms);
        return generateHeadLiteral(symbol, terms);
    }

    @Override
    public ProvaLiteral generateLiteral(String symbol, ProvaList terms, List<ProvaLiteral> guard) {
        if (guard == null) {
            ProvaBuiltin builtin = builtins.get(symbol);
            if (builtin != null) return new ProvaLiteralImpl(builtin, terms);
            return generateHeadLiteral(symbol, terms);
        ...
        ProvaBuiltin builtin = builtins.get(symbol);
        if (builtin != null) return new ProvaGuardedLiteralImpl(builtin, terms, guard);
    ...
    @Override
    public ProvaRule generateGoal(ProvaLiteral[] body, List<ProvaVariable> variables) {
        if (body.length == 0) return null;
        long ruleId = 0;
    ...
    @Override
    public ProvaRule generateRule(ProvaLiteral head, ProvaLiteral[] newGoals, ProvaLiteral[] body, int offset) {
        ProvaLiteral[] combinedBody = new ProvaLiteral[newGoals.length + body.length - 1 - offset];
        int i = 0;
        for (; i < newGoals.length; i++) combinedBody[i] = newGoals[i];
        for (int j = 1 + offset; j < body.length; j++, i++) combinedBody[i] = body[j];
        return new ProvaRuleImpl(0, head, combinedBody);
    }

    @Override
    public ProvaLiteral generateCachedLiteral(String symbol, ProvaList terms, ProvaCacheState cacheState, ProvaLocalAnswers answers) {
        ProvaBuiltin builtin = builtins.get(symbol);
        if (builtin != null) return new ProvaLiteralImpl(builtin, terms);
        final int arity = terms.computeSize();
        if (arity == -1) {
        ...
        if (predicate2 == null) {
            if (predicate == null) predicate2 = new ProvaPredicateImpl(symbol, arity, this); else predicate2 = predicate;
            predicate2.setKnowledgeBase(this);
            predicates.put(key, predicate2);
            if (predicate != null) return new ProvaCachedLiteralImpl(predicate, terms, cacheState, answers);
        }
        return new ProvaCachedLiteralImpl(predicate, terms, cacheState, answers);
    }
    ...
    public ProvaConstant generateGlobalConstant(String name) {
        ...
        if (oldGlobal != null) return oldGlobal;
    ...
    public synchronized void unconsultSync(String src) {
        ...
        for (ProvaRuleSet ruleset : rulesets) ruleset.removeClausesBySrc(src);
        srcMap.remove(src);
    }
}
