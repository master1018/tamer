package org.informaticisenzafrontiere.openstaff.splashHandlers;
...
import org.informaticisenzafrontiere.openstaff.controller.Login;
...
public class InteractiveSplashHandler extends AbstractSplashHandler {
    ...
    private void handleButtonOKWidgetSelected() {
        ...
        if ((username.length() > 0) && (password.length() > 0)) {
            boolean risposta = Login.controlla(utente);
            if (risposta) {
                fAuthenticated = true;
            } else {
                if (Login.getErrorCode() == 1) MessageDialog.openError(getSplash(), MessageBundle.getMessage("openstaff.messaggi.autenticazioneFallitaTitle"), MessageBundle.getMessage("openstaff.messaggi.autenticazioneFallitaPassword")); else MessageDialog.openError(getSplash(), MessageBundle.getMessage("openstaff.messaggi.autenticazioneFallitaTitle"), MessageBundle.getMessage("openstaff.messaggi.autenticazioneFallitaUser"));
            }
        } else {
            MessageDialog.openError(getSplash(), MessageBundle.getMessage("openstaff.messaggi.autenticazioneFallitaTitle"), MessageBundle.getMessage("openstaff.messaggi.autenticazioneFallitaUserPassword"));
        }
    }
    ...
    private void createUITextPassword() {
        ...
        fTextPassword.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                if (e.keyCode == 13) handleButtonOKWidgetSelected();
            }
