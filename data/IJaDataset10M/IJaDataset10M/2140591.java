...
public class CFFFont {
    ...
    protected void getDictItem() {
        ...
        while (!gotKey) {
                ...
                while (!done) {
                    ...
                    switch(nibble) {
                        ...
                        case 0xf:
                            ...
                            break;
                        default:
                            if (nibble >= 0 && nibble <= 9) item += String.valueOf(nibble); else {
            ...
            if (b0 <= 21) {
                gotKey = true;
                if (b0 != 12) key = operatorNames[b0]; else key = operatorNames[32 + getCard8()];
                continue;
            }
        }
    }
    ...
    protected static final class RangeItem extends Item {
        ...
        public void emit(byte[] buffer) {
            try {
                buf.seek(offset);
                for (int i = myOffset; i < myOffset + length; i++) buffer[i] = buf.readByte();
            } catch (Exception e) {
                throw new ExceptionConverter(e);
            }
        }
    ...
    protected static final class StringItem extends Item {
        ...
        public void emit(byte[] buffer) {
            for (int i = 0; i < s.length(); i++) buffer[myOffset + i] = (byte) (s.charAt(i) & 0xff);
        }
    }
    ...
    public byte[] getCID(String fontName) {
        int j;
        for (j = 0; j < fonts.length; j++) if (fontName.equals(fonts[j].name)) break;
        ...
        if (fonts[j].isCID) {
            ...
            if (fdFontName.length() > 127) fdFontName = fdFontName.substring(0, 127);
            String extraStrings = "Adobe" + "Identity" + fdFontName;
            int origStringsLen = stringOffsets[stringOffsets.length - 1] - stringOffsets[0];
            int stringsBaseOffset = stringOffsets[0] - 1;
            byte stringsIndexOffSize;
            if (origStringsLen + extraStrings.length() <= 0xff) stringsIndexOffSize = 1; else if (origStringsLen + extraStrings.length() <= 0xffff) stringsIndexOffSize = 2; else if (origStringsLen + extraStrings.length() <= 0xffffff) stringsIndexOffSize = 3; else stringsIndexOffSize = 4;
            l.addLast(new UInt16Item((char) ((stringOffsets.length - 1) + 3)));
            l.addLast(new UInt8Item((char) stringsIndexOffSize));
            for (int i = 0; i < stringOffsets.length; i++) l.addLast(new IndexOffsetItem(stringsIndexOffSize, stringOffsets[i] - stringsBaseOffset));
    ...
    public boolean isCID(String fontName) {
        int j;
        for (j = 0; j < fonts.length; j++) if (fontName.equals(fonts[j].name)) return fonts[j].isCID;
        return false;
    }

    public boolean exists(String fontName) {
        int j;
        for (j = 0; j < fonts.length; j++) if (fontName.equals(fonts[j].name)) return true;
        return false;
    }

    public String[] getNames() {
        String[] names = new String[fonts.length];
        for (int i = 0; i < fonts.length; i++) names[i] = fonts[i].name;
        return names;
    }
    ...
    public CFFFont(RandomAccessFileOrArray inputbuffer) {
        ...
        for (int j = 0; j < topdictOffsets.length - 1; j++) {
            ...
            while (getPosition() < topdictOffsets[j + 1]) {
                ...
                if (key == "FullName") {
                ...
                } else if (key == "ROS") fonts[j].isCID = true; else if (key == "Private") {
                    ...
                    seek(p);
                } else if (key == "FDArray") fonts[j].fdarrayOffset = ((Integer) args[0]).intValue(); else if (key == "FDSelect") fonts[j].fdselectOffset = ((Integer) args[0]).intValue(); else if (key == "CharstringType") fonts[j].CharstringType = ((Integer) args[0]).intValue();
            }
            if (fonts[j].privateOffset >= 0) {
                seek(fonts[j].privateOffset);
                while (getPosition() < fonts[j].privateOffset + fonts[j].privateLength) {
                    getDictItem();
                    if (key == "Subrs") fonts[j].privateSubrs = ((Integer) args[0]).intValue() + fonts[j].privateOffset;
                }
            }
            if (fonts[j].fdarrayOffset >= 0) {
                ...
                for (int k = 0; k < fdarrayOffsets.length - 1; k++) {
                    ...
                    while (getPosition() < fdarrayOffsets[k + 1]) getDictItem();
                    if (key == "Private") {
