...
public class BeanConnection implements Serializable {
    ...
    private static ArrayList<Vector> TABBED_CONNECTIONS = new ArrayList<Vector>();
   ...
   * listToCheck
   */
    private static boolean checkForSource(BeanInstance candidate, Vector listToCheck, Integer... tab) {
        int tabIndex = 0;
        if (tab.length > 0) {
            tabIndex = tab[0].intValue();
        }
   ...
   */
    private static boolean checkTargetConstraint(BeanInstance candidate, Vector listToCheck, Integer... tab) {
        int tabIndex = 0;
        if (tab.length > 0) {
            tabIndex = tab[0].intValue();
        }
   ...
   * listToCheck
   */
    private static boolean checkForTarget(BeanInstance candidate, Vector listToCheck, Integer... tab) {
        int tabIndex = 0;
        if (tab.length > 0) {
            tabIndex = tab[0].intValue();
        }
   ...
   */
    private static boolean checkSourceConstraint(BeanInstance candidate, Vector listToCheck, Integer... tab) {
        int tabIndex = 0;
        if (tab.length > 0) {
            tabIndex = tab[0].intValue();
        }
    ...
    public static void doMetaConnection(BeanInstance source, BeanInstance target, final EventSetDescriptor esd, final JComponent displayComponent) {
        ...
        if (targetBean instanceof MetaBean) {
            ...
            if (receivers.size() == 1) {
                ...
                BeanConnection bc = new BeanConnection(realSource, realTarget, esd);
            } else {
                int menuItemCount = 0;
                ...
                for (int i = 0; i < receivers.size(); i++) {
                    ...
                    targetItem.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            BeanConnection bc = new BeanConnection(realSource, tempTarget, esd);
