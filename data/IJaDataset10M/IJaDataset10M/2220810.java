package gov.sns.tools.scan.SecondEdition.analysis;

import gov.sns.tools.plot.BasicGraphData;
import gov.sns.tools.plot.FunctionGraphsJPanel;
import gov.sns.tools.plot.GraphDataOperations;
...
public class MainAnalysisController {
	 ...
	 */
    public MainAnalysisController(Object mainScanDocument_In, JPanel parentAnalysisPanel_In, JPanel analysisControlPanel_In, JPanel customGraphPanel_In, ScanVariable scanVariableParameter_In, ScanVariable scanVariable_In, Vector<MeasuredValue> measuredValuesV_In, Vector<Boolean> measuredValuesShowStateV_In, FunctionGraphsJPanel graphScan_In, FunctionGraphsJPanel graphAnalysis_In) {
        ...
        operationChooser.addActionListener(new ActionListener() {
    ...
    public void createChildAnalysis(XmlDataAdaptor analysisConfig) {
        ...
        customControlPanel.removeAll();
        customGraphPanel.removeAll();
        java.util.Iterator<XmlDataAdaptor> childIt = analysisConfig.childAdaptorIterator();
        while (childIt.hasNext()) {
            XmlDataAdaptor analysisConf = childIt.next();
            AnalysisController ac = AnalysisControllerFactory.getAC(this, analysisConf, parentAnalysisPanel, customControlPanel, customGraphPanel, globalButtonsPanel, scanVariableParameter, scanVariable, measuredValuesV, graphAnalysis, messageTextLocal, graphDataLocal);
    ...
    public void dumpChildAnalysisConfig(XmlDataAdaptor analysisConfig) {
        for (int i = 0; i < analysisControllers.length; i++) {
            XmlDataAdaptor aConf = analysisConfig.createChild(analysisControllers[i].getTypeName());
    ...
    private void setAnalysisControlPanel(int indexOfPanelNewIn) {
        if (indexOfPanelNewIn >= 0 && indexOfPanelNewIn < analysisControllers.length) {
            ...
            if (indexOfPanelOld >= 0 && indexOfPanelOld < analysisControllers.length) {
                analysisControllers[indexOfPanelOld].ShutUp();
            }
            if (indexOfPanelNew >= 0 && indexOfPanelNew < analysisControllers.length) {
	 ...
	 */
    public static double getPhaseShift(List<? extends BasicGraphData> gdV) {
        String key = getPhaseShiftKey();
        double shift = 0.;
        int nCount = 0;
        for (int i = 0, n = gdV.size(); i < n; i++) {
    ...
    protected BasicGraphData getChoosenDraphData() {
        ...
        if (Ind != null && Ind.intValue() >= 0) {
            ...
            if (gdV.size() == 1) {
                ...
                Vector<BasicGraphData> gdInsideV = GraphDataOperations.getDataInsideRectangle(gdV, graphAnalysis.getCurrentMinX(), graphAnalysis.getCurrentMaxX(), graphAnalysis.getCurrentMinY(), graphAnalysis.getCurrentMaxY());
                if (gdInsideV.size() == 1) {
    ...
    protected Object[] getChoosenDraphDataAndPoint() {
        ...
        if (Ind != null && IndP != null && Ind.intValue() >= 0 && IndP.intValue() >= 0 && graphAnalysis.getInstanceOfGraphData(Ind.intValue()) != graphDataLocal) {
            ...
            oneV.add(gd);
            objArr = GraphDataOperations.getGraphDataAndPointIndexInside(oneV, graphAnalysis.getCurrentMinX(), graphAnalysis.getCurrentMaxX(), graphAnalysis.getCurrentMinY(), graphAnalysis.getCurrentMaxY());
        }
        return objArr;
    }
