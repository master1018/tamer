...
public class IncludeFileFinder {
        ...
        while (it.hasNext()) {
            ...
            if (url.getProtocol().equals("jar")) {
                ...
                while (entries.hasMoreElements()) {
                    ...
                    if (entry.getName().startsWith("PUSTEFIX-INF/txt") || entry.getName().startsWith("PUSTEFIX-INF/xml")) {
    ...
    private static void find(String dirPath, ServletContext servletContext, IncludeFileVisitor visitor) throws IncludePartsInfoParsingException {
        @SuppressWarnings("unchecked") Set<String> paths = servletContext.getResourcePaths(dirPath);
        if (paths != null) {
    ...
    private static URL getFileUrl(URL url) {
        if (!url.getProtocol().equals("file")) throw new PustefixRuntimeException("Invalid protocol: " + url);
