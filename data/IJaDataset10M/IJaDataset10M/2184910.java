...
public class TypeLoader {
    ...
    private static ClassReader getClassReader(String typeName) {
        try {
            ...
            InputStream is = null;
            String localPath = project.getBaseDirectory();
        ...
        if (loadedClassTypes.add(type)) {
            try {
                ...
                if (projectTypes.contains(type.getName())) {
                    attributes = new Attribute[] { new ClassInvariantAttribute(type), new ConstraintAttribute(type), new ModelFieldAttribute(type), new MethodSpecificationAttribute(), new AssertAttribute(), new AssumeAttribute(), new LoopSpecificationAttribute() };
                } else {
                    attributes = new Attribute[] { new ClassInvariantAttribute(type), new ConstraintAttribute(type), new ModelFieldAttribute(type), new MethodSpecificationAttribute() };
                ...
                if (te.getTroubleMessage().getPosition() == null) {
                    te.getTroubleMessage().setPosition(new TroublePosition(type, null, null));
                }
                throw te;
            }
        }
    ...
    public static ClassNode getASMClassTypeNode(JClassType type) {
        try {
            ...
            ClassReader cr = TypeLoader.getClassReader(type.getName());
    ...
    private static final class JClassTypeBuilder implements ClassVisitor {
        ...
        private final List<BMLConstraint> constraints = new ArrayList<BMLConstraint>();
        ...
        public MethodVisitor visitMethod(int access, String name, String descriptor, String signature, String[] exceptionNames) {
            ...
            final BCMethod method = new BCMethod(access, type, name, returnType, argumentTypes, exceptions);
            methods.add(method);
            try {
        ...
        public void visitEnd() {
            type.setDeclarations(fields.toArray(new BCField[fields.size()]), methods.toArray(new BCMethod[methods.size()]), invariants.toArray(new BMLInvariant[invariants.size()]), constraints.toArray(new BMLConstraint[constraints.size()]));
        }
    }
    ...
    private static final class BCMethodBuilder implements MethodVisitor {
        ...
        private void reportTrouble(TroubleDescription description, Object... parameters) {
            troubleReporter.reportTrouble(new TroubleMessage(new TroublePosition(method, null), description, parameters));
        }
        ...
        public void visitInsn(int opcode) {
            try {
                ...
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[opcode]);
            }
        }
        ...
        public void visitFieldInsn(int opcode, String owner, String name, String descriptor) {
            try {
                addInstruction(InstructionFactory.fromFieldInsn(opcode, TypeLoader.getClassType(owner), name, JType.fromDescriptor(descriptor)));
            } catch (UnsupportedInstructionException e) {
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[opcode]);
            }
        }
        ...
        public void visitMethodInsn(int opcode, String owner, String name, String descriptor) {
            ...
            try {
                addInstruction(InstructionFactory.fromMethodInsn(opcode, ownerType, name, returnType, parameterTypes));
            } catch (UnsupportedInstructionException e) {
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[opcode]);
            }
        }
        ...
        public void visitJumpInsn(int opcode, Label label) {
            try {
                addInstruction(InstructionFactory.fromJumpInsn(opcode, getHandleFor(label)));
            } catch (UnsupportedInstructionException e) {
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[opcode]);
            }
        }
        ...
        public void visitLdcInsn(Object cst) {
            try {
                ...
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[IOpCodes.LDC]);
            }
        }
        ...
        public void visitIincInsn(int var, int increment) {
            try {
                ...
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[IOpCodes.IINC]);
            }
        }
        ...
        public void visitTableSwitchInsn(int min, int max, Label dflt, Label[] labels) {
            ...
            try {
                addInstruction(InstructionFactory.fromTableSwitchInsn(min, max, getHandleFor(dflt), handles));
            } catch (UnsupportedInstructionException e) {
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[IOpCodes.TABLESWITCH]);
            }
        }
        ...
        public void visitLookupSwitchInsn(Label dflt, int[] keys, Label[] labels) {
            ...
            try {
                addInstruction(InstructionFactory.fromLookupSwitchInsn(getHandleFor(dflt), keys, handles));
            } catch (UnsupportedInstructionException e) {
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[IOpCodes.LOOKUPSWITCH]);
            }
        }

        public void visitMultiANewArrayInsn(String descriptor, int dims) {
            try {
                addInstruction(InstructionFactory.fromMultiANewArrayInsn((JArrayType) JType.fromDescriptor(descriptor), dims));
            } catch (UnsupportedInstructionException e) {
                reportTrouble(B2BPLMessages.UNSUPPORTED_INSTRUCTION, IOpCodes.NAMES[IOpCodes.MULTIANEWARRAY]);
            }
        }
        ...
        public void visitTryCatchBlock(Label start, Label end, Label handler, String type) {
            ...
            ExceptionHandler exceptionHandler = new ExceptionHandler(getHandleFor(start), getHandleFor(end), getHandleFor(handler), TypeLoader.getClassType(type));
        ...
        public void visitEnd() {
            ...
            if (instructions != null) {
                method.setCodeInfo(instructions, maxStack, maxLocals, exceptionHandlers.toArray(new ExceptionHandler[exceptionHandlers.size()]));
                instructions = null;
            }
        }
