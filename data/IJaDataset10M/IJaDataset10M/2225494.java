...
public class DocumentReferenceSerializerFactory extends BeanSerializerFactory {
    ...
    public DocumentReferenceSerializerFactory(Config config, CouchDbConnector couchDbConnector) {
        super(config);
        this.couchDbConnector = couchDbConnector;
    }
    ...
    protected JsonSerializer<Object> constructBeanSerializer(SerializationConfig config, BasicBeanDescription beanDesc, BeanProperty property) {
        ...
        if (docRefs.size() > 0) {
            ...
            BeanPropertyWriter[] filter = createDocumentReferenceFieldFilters(docRefs, props);
            BeanSerializerBuilder builder = new BeanSerializerBuilder(beanDesc);
            builder.setFilteredProperties(filter);
            return new BackReferencedBeanSerializer((BeanSerializer) builder.build(), findWriters(docRefs, config, beanDesc, props), couchDbConnector);
        }
    ...
    public boolean hasDocumentReferenceFields(SerializationConfig config, Object value) {
        BasicBeanDescription beanDesc = config.introspect(TypeFactory.type(value.getClass()));
        List<AnnotatedField> docRefs = findDocumentReferenceFields(beanDesc);
    ...
    public BeanPropertyWriter[] createDocumentReferenceFieldFilters(List<AnnotatedField> docRefs, List<BeanPropertyWriter> props) {
        ...
        for (int i = 0; i < writers.length; ++i) {
            for (AnnotatedField field : docRefs) {
                if (writers[i] != null && field.getName().equals(writers[i].getName())) {
                    writers[i] = null;
                }
            }
        }
