...
public abstract class AbstractErrorDecorator<T extends JComponent> implements ErrorDecorator {
    ...
    public Collection<FieldPath> getPaths() {
        if (parent.getObject() == null) return Collections.emptySet();
    ...
    public AbstractErrorDecorator(FieldPathSpec spec, OBOTextEditComponent parent, T c) {
        setSpec(spec);
        setParent(parent);
        setComponent(c);
        init();
