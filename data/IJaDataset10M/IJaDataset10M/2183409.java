package fitService.fitnesse.wiki;

import java.util.*;
import java.util.regex.Pattern;
import fitService.fitnesse.wikitext.widgets.WikiWordWidget;
import fitService.fitnesse.components.FitNesseTraversalListener;
...
public class PageCrawlerImpl implements PageCrawler {
    ...
    public WikiPage getPage(WikiPage context, WikiPagePath path) throws Exception {
        ...
        if (firstPathElement.equals(WikiPagePath.ROOT)) return getPage(getRoot(context), restOfPath);
        WikiPage childPage = context.getChildPage(firstPathElement);
        if (childPage != null) return getPage(childPage, restOfPath); else return getPageAfterDeadEnd(context, firstPathElement, restOfPath);
    }
    ...
    protected WikiPage getPageAfterDeadEnd(WikiPage context, String first, WikiPagePath rest) throws Exception {
        ...
        if (deadEndStrategy != null) return deadEndStrategy.getPageAfterDeadEnd(context, rest, this); else return null;
    }
    ...
    public WikiPagePath getFullPathOfChild(WikiPage parent, WikiPagePath childPath) throws Exception {
        ...
        if (childPath.isAbsolute()) fullPathOfChild = childPath.relativePath(); else {
    ...
    private WikiPage getOrMakePage(WikiPage context, List namePieces) throws Exception {
        ...
        WikiPage current;
        if (context.getChildPage(first) == null) current = context.addChildPage(first); else current = context.getChildPage(first);
        if (rest.size() == 0) return current;
    ...
    public static WikiPage getInheritedPage(String pageName, WikiPage context) throws Exception {
        ...
        for (Iterator iterator = ancestors.iterator(); iterator.hasNext(); ) {
            ...
            if (namedPage != null) return namedPage;
        }
        return null;
    }
    ...
    public WikiPage getRoot(WikiPage page) throws Exception {
        if (isRoot(page)) return page; else return getRoot(page.getParent());
    }

    public void traverse(WikiPage context, FitNesseTraversalListener listener) throws Exception {
        if (context.getClass() == SymbolicPage.class) return;
