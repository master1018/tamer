...
public class ChallengeWebServer {
    ...
    public static class GetRequest extends Request {
        ...
        public GetRequest(String u, Map<String, String> h, String v) {
            super(u, h, v);
        }

        public GetRequest(String u, String v) {
            super(u, v);
        }
    }
    ...
    public static class PostRequest extends Request {
        ...
        public void setBody(byte[] _body) {
            body = new ByteArrayBuffer(_body);
        }

        public void setBody(ByteArrayBuffer _bodyBuf) {
            body = _bodyBuf;
        }
    }
    ...
    public abstract static class Handler extends Thread {
        ...
        protected String readLineFromStream(BufferedInputStream bis) throws IOException {
        ...
        protected void attach(Socket _socket) throws java.io.IOException {
            ...
            output = new WebPrintWriter(new BufferedWriter(new OutputStreamWriter(rawOutput, Charset.forName("UTF-8"))));
        }
        ...
        protected void redirect(String newuri, int code) {
            ...
            output.println("The document can now be found at: <tt>" + newuri + "</tt>");
            printFooter();
        }
        ...
        protected void notFound(String uri) {
            ...
            header("Connection: close");
            endHeaders();
            output.println("<h1>404 Not Found</h1>The URL <tt>" + uri + "</tt> was not found.");
            printFooter();
        }
        ...
        protected String getFooterText() {
            return "Poolboy Java server on " + socket.getLocalAddress().toString().substring(1) + " (<tt>" + getClass() + "</tt>)";
        }
                ...
                if (isGet || isPost) {
                    ...
                    if (isPost) {
                        ...
                        if (contentLength > 0) {
                            ...
                            while ((len = rawInput.read(buf, 0, BUFSIZ)) >= 0) postBuf.append(buf, 0, len);
                        }
                        if (request.startsWith("PUT ")) requestObj = new PutRequest(uri, headers, version, postBuf); else requestObj = new PostRequest(uri, headers, version, postBuf);
                    } else {
                        if (request.startsWith("GET ")) requestObj = new GetRequest(uri, headers, version); else requestObj = new GetRequest(uri, headers, version);
                    }
                    handle(requestObj);
                }
            } catch (Exception e) {
    ...
    public void serve(int port) throws java.io.IOException, java.lang.IllegalAccessException {
        serve(port, true, null);
    }

    public void serve(int port, boolean isPublic) throws java.io.IOException, java.lang.IllegalAccessException {
        serve(port, isPublic, null);
    }

    public void serve(int port, boolean isPublic, Access.SocketRule[] rules) throws java.io.IOException, java.lang.IllegalAccessException {
        ServerSocket s;
        ...
        if (isPublic) {
            ...
            s = new ServerSocket(port, m_connectionBacklog, InetAddress.getByName("localhost"));
        }
        if (!protectedAccess) {
            for (; ; ) {
    ...
    protected int m_connectionBacklog = 64;
    ...
    public enum MessageType {
        ...
        Commit, Cast, Challenge, AdderChallenge, Result, UnknownType
    }
    ...
    private static boolean isRaceImage(String entryName, List<String> langs) {
        if (!entryName.startsWith("media/B")) return false;
        if (!entryName.endsWith(".png")) return false;
        if (entryName.indexOf("_selected_") == -1) return false;
        if (langs != null) if (entryName.indexOf(langs.get(0)) == -1) return false;
        return true;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        class ChallengeHandler extends Handler {
            ...
            public ChallengeHandler(SocketQueue q) {
                super(q);
            }
                ...
                try {
                    ...
                    if (target == null) {
                        ...
                        for (int i = 0; i < timeline.size(); i++) {
                            ...
                            } else if (map.containsKey(MessageType.Cast)) {
                                output.println(" WAS CAST</a><BR>");
                            } else output.println(" IN PROGRESS</a><BR>");
                        }
                    } else {
                        ...
                        if (map.containsKey(MessageType.Cast)) {
                        ...
                        } else if (map.containsKey(MessageType.Result)) {
                            ...
                            for (ASExpression sexp : decryptedBallot) {
                                ...
                                if (value.equals("1")) races.add(raceId);
                            ...
                            Collections.sort(titleSet, new Comparator<Image>() {
                                ...
                                public int compare(Image title1, Image title2) {
                                    String raceId1 = null;
                                    String raceId2 = null;
                                    if (titleToRaces.get(title1).size() != 0) raceId1 = titleToRaces.get(title1).get(0); else raceId1 = "Z98";
                                    if (titleToRaces.get(title2).size() != 0) raceId2 = titleToRaces.get(title2).get(0); else raceId2 = "Z99";
                            ...
                            for (Image title : titleSet) {
                                ...
                                for (Map.Entry<String, Image> entry : _raceToTitle.entrySet()) if (entry.getValue() == title) {
                                    keyForTitle = entry.getKey();
                                    break;
                                }
                                output.println("<tr><td><img src=\"/img/L/" + keyForTitle + "\"></td></tr>");
                                List<String> races = titleToRaces.get(title);
                                for (String race : races) output.println("<tr><td><img src=\"/img/" + race + "\"></td></tr>");
                            }
        ...
        for (int i = 0; i < original.length(); i++) {
            ...
            switch(original.charAt(i)) {
                ...
                case '9':
                    ...
                    break;
                default:
                    throw new RuntimeException("Expected value between 0 & 9, found " + original.charAt(i));
            }
            mutated += c;
        }
        return mutated;
        ...
        for (int i = 0; i < original.length(); i++) {
            ...
            switch(original.charAt(i)) {
                ...
                case 'J':
                    ...
                    break;
                default:
                    throw new RuntimeException("Expected value between 'A' & 'J', found " + original.charAt(i));
            }
            mutated += c;
        }
