package org.genie.cmof.properties.cls;

import org.eclipse.core.runtime.IAdaptable;
import org.eclipse.jface.viewers.ISelection;
import org.eclipse.jface.viewers.IStructuredSelection;
import org.eclipse.swt.SWT;
import org.eclipse.swt.custom.CLabel;
import org.eclipse.swt.events.ModifyEvent;
import org.eclipse.swt.events.ModifyListener;
import org.eclipse.swt.layout.FormAttachment;
import org.eclipse.swt.layout.FormData;
import org.eclipse.swt.widgets.Composite;
import org.eclipse.swt.widgets.Text;
import org.eclipse.ui.IWorkbenchPart;
import org.eclipse.ui.views.properties.tabbed.AbstractPropertySection;
import org.eclipse.ui.views.properties.tabbed.ITabbedPropertyConstants;
import org.eclipse.ui.views.properties.tabbed.TabbedPropertySheetPage;

public class ClassSupertypeSection extends AbstractPropertySection {

    private Text labelText;

    private L1.Class xmiElement;

    @Override
    public void createControls(Composite parent, TabbedPropertySheetPage aTabbedPropertySheetPage) {
        super.createControls(parent, aTabbedPropertySheetPage);
        Composite composite = getWidgetFactory().createFlatFormComposite(parent);
        FormData dataId;
        labelText = getWidgetFactory().createText(composite, "");
        dataId = new FormData();
        dataId.left = new FormAttachment(0, STANDARD_LABEL_WIDTH);
        dataId.right = new FormAttachment(100, 0);
        dataId.top = new FormAttachment(0, ITabbedPropertyConstants.VSPACE);
        labelText.setLayoutData(dataId);
        labelText.addModifyListener(listener);
        CLabel labelId = getWidgetFactory().createCLabel(composite, "Supertype");
        dataId = new FormData();
        dataId.left = new FormAttachment(0, 0);
        dataId.right = new FormAttachment(labelText, -ITabbedPropertyConstants.HSPACE);
        dataId.top = new FormAttachment(labelText, 0, SWT.CENTER);
        labelId.setLayoutData(dataId);
    }

    @Override
    public void setInput(IWorkbenchPart part, ISelection selection) {
        super.setInput(part, selection);
        Object input = ((IStructuredSelection) selection).getFirstElement();
        this.xmiElement = (L1.Class) ((IAdaptable) input).getAdapter(L1.Class.class);
    }

    @Override
    public void refresh() {
        labelText.removeModifyListener(listener);
        StringBuffer buffer = new StringBuffer();
        if (xmiElement.getSuperClass() != null) {
            for (L1.Class superClass : xmiElement.getSuperClass()) {
                if (buffer.length() > 0) buffer.append("; ");
                buffer.append(superClass.getName());
            }
            if (buffer.length() > 0) buffer.append("; ");
            labelText.setText(buffer.toString());
        }
        labelText.addModifyListener(listener);
    }

    private ModifyListener listener = new ModifyListener() {

        public void modifyText(ModifyEvent arg0) {
            xmiElement.setName(getElementName());
        }
    };

    private String getElementName() {
        return (labelText.getText() == null ? "" : labelText.getText());
    }
}
