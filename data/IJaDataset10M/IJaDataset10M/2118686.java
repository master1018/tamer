...
/**
 * Interface implemented by classes which decode multicast request data and
 * additionally support delayed constraint checking.
 * 
...
public interface DelayedMulticastRequestDecoder extends MulticastRequestDecoder {
	 ...
	 * delayed. <code>null</code> constraints are considered equivalent to empty
	 * constraints. Constraint checking may be delayed using the
	 * <code>delayConstraintCheck</code> flag.
	 * <p>
	 * If the <code>delayConstraintCheck</code> flag is <code>true</code>, the
	 ...
	 * {@link MulticastRequest#checkConstraints checkConstraints} method must be
	 * invoked to complete checking of all the constraints.
	 * <li>Constraints which must be checked before accessor methods of the
	 * returned <code>MulticastRequest</code> can be invoked are always checked
	 ...
	 * 
	 * @param buf
	 *            a buffer containing the packet data to decode. The contents of
	 ...
	 *            Additionally, the multicast request data must begin at
	 *            position zero of <code>buf</code>.
	 * @param constraints
	 ...
	 *            <code>null</code>
	 * @param delayConstraintCheck
	 *            flag to control delayed constraint checking
	 * @return the decoded multicast request data
	 * @throws IOException
	 *             if an error occurs in interpreting the data
	 * @throws UnsupportedConstraintException
	 *             if unable to satisfy the specified constraints
	 * @throws SecurityException
	 *             if the given constraints cannot be satisfied due to
	 *             insufficient caller permissions, or if the client subject
	 *             check fails
