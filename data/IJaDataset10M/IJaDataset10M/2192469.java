...
public class RoiDecoder {
    ...
    public Roi getRoi() throws IOException {
        if (path != null) {
            ...
            size = (int) f.length();
            if (!path.endsWith(".roi") && size > 5242880) throw new IOException("This is not an ROI or file size>5MB)");
            name = f.getName();
            is = new FileInputStream(path);
        }
        data = new byte[size];
        int total = 0;
        while (total < size) total += is.read(data, total, size - total);
        is.close();
        if (getByte(0) != 73 || getByte(1) != 111) throw new IOException("This is not an ImageJ ROI");
        ...
        if (name != null && name.endsWith(".roi")) name = name.substring(0, name.length() - 4);
        ...
        switch(type) {
            case rect:
                ...
                if (arcSize > 0) roi.setRoundRectArcSize(arcSize);
                break;
            case oval:
            ...
            case line:
                ...
                if (subtype == ARROW) {
                    ...
                    if (style >= Arrow.FILLED && style <= Arrow.OPEN) ((Arrow) roi).setStyle(style);
                    int headSize = getByte(ARROW_HEAD_SIZE);
                    if (headSize >= 0 && style <= 30) ((Arrow) roi).setHeadSize(headSize);
                } else roi = new Line(x1, y1, x2, y2);
                break;
            case polygon:
            case freehand:
            case traced:
            ...
            case point:
                ...
                int roiType;
                if (type == polygon) roiType = Roi.POLYGON; else if (type == freehand) {
                    roiType = Roi.FREEROI;
                    if (subtype == ELLIPSE) {
                ...
                } else if (type == traced) roiType = Roi.TRACED_ROI; else if (type == polyline) roiType = Roi.POLYLINE; else if (type == freeline) roiType = Roi.FREELINE; else if (type == angle) roiType = Roi.ANGLE; else roiType = Roi.FREEROI;
        ...
        if (version >= 218) {
            ...
            if (splineFit && roi instanceof PolygonRoi) ((PolygonRoi) roi).fitSpline();
        }
        if (version >= 218 && subtype == TEXT) roi = getTextRoi(roi);
        roi.setPosition(position);
        return roi;
    }
    ...
    void getStrokeWidthAndColor(Roi roi) {
        ...
        if (strokeWidth > 0) roi.setStrokeWidth(strokeWidth);
    ...
    public Roi getShapeRoi() throws IOException {
        int type = getByte(TYPE);
        if (type != rect) throw new IllegalArgumentException("Invalid composite ROI type");
        int top = getShort(TOP);
        int left = getShort(LEFT);
        int bottom = getShort(BOTTOM);
        int right = getShort(RIGHT);
    ...
    Roi getTextRoi(Roi roi) {
        ...
        for (int i = 0; i < nameLength; i++) name[i] = (char) getShort(hdrSize + 16 + i * 2);
        for (int i = 0; i < textLength; i++) text[i] = (char) getShort(hdrSize + 16 + nameLength * 2 + i * 2);
