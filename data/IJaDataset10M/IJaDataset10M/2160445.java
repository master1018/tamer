...
public class SubstanceCoreUtilities {
    ...
    public static String clipString(FontMetrics metrics, int availableWidth, String fullText) {
        ...
        for (int i = 0; i < w2; i++) {
            ...
            if ((w - i) > w2) newEnder = fullText.charAt(w - i - 1) + newEnder;
    ...
    public static boolean isSpinnerButton(AbstractButton button) {
        Container parent = button.getParent();
        if (!(button instanceof SubstanceSpinnerButton)) return false;
    ...
    public static boolean isToolBarButton(JComponent component) {
        if (component instanceof SubstanceDropDownButton) return false;
        if (component instanceof SubstanceSpinnerButton) return false;
    ...
    public static boolean isButtonNeverPainted(JComponent button) {
        if (button instanceof JCheckBox) return false;
        if (button instanceof JRadioButton) return false;
        Object prop = button.getClientProperty(SubstanceLookAndFeel.BUTTON_PAINT_NEVER_PROPERTY);
        if (prop != null) {
            if (Boolean.TRUE.equals(prop)) return true;
            if (Boolean.FALSE.equals(prop)) return false;
        }
        if (button != null) {
            Container parent = button.getParent();
            if (parent instanceof JComponent) {
                JComponent jparent = (JComponent) parent;
                Object flatProperty = jparent.getClientProperty(SubstanceLookAndFeel.BUTTON_PAINT_NEVER_PROPERTY);
                if (flatProperty != null) {
                    if (Boolean.TRUE.equals(flatProperty)) return true;
                    if (Boolean.FALSE.equals(flatProperty)) return false;
                }
            }
        }
    ...
    public static FocusKind getFocusKind(Component component) {
        while (component != null) {
            if (component instanceof JComponent) {
                ...
                Object jcompFocusKind = jcomp.getClientProperty(SubstanceLookAndFeel.FOCUS_KIND);
                if (jcompFocusKind instanceof FocusKind) return (FocusKind) jcompFocusKind;
            }
            component = component.getParent();
        }
        Object globalFocusKind = UIManager.get(SubstanceLookAndFeel.FOCUS_KIND);
        if (globalFocusKind instanceof FocusKind) return (FocusKind) globalFocusKind;
    ...
    public static boolean toDrawWatermark(Component component) {
        ...
        while (c != null) {
            if (c instanceof JComponent) {
                JComponent jcomp = (JComponent) component;
                Object obj = jcomp.getClientProperty(SubstanceLookAndFeel.WATERMARK_VISIBLE);
                if (obj != null) {
                    if (Boolean.TRUE.equals(obj)) return true;
                    if (Boolean.FALSE.equals(obj)) return false;
                }
            }
            c = c.getParent();
        }
        ...
        if (Boolean.TRUE.equals(obj)) return true;
        if (Boolean.FALSE.equals(obj)) return false;
        if (component instanceof JList) return false;
        if (component instanceof JTree) return false;
        if (component instanceof JTable) return false;
    ...
    public static SubstanceButtonShaper getButtonShaper(Component comp) {
        if (comp instanceof JComponent) {
            Object prop = ((JComponent) comp).getClientProperty(SubstanceLookAndFeel.BUTTON_SHAPER_PROPERTY);
            if (prop instanceof SubstanceButtonShaper) return (SubstanceButtonShaper) prop;
        }
    ...
    public static boolean isTabModified(Component tabComponent) {
        ...
        if (comp instanceof JComponent) {
            ...
            isWindowModified = Boolean.TRUE.equals(jc.getClientProperty(SubstanceLookAndFeel.WINDOW_MODIFIED));
        }
    ...
    public static boolean isRootPaneModified(JRootPane rootPane) {
        return Boolean.TRUE.equals(rootPane.getClientProperty(SubstanceLookAndFeel.WINDOW_MODIFIED));
    }
    ...
    public static boolean isInternalFrameModified(JInternalFrame internalFrame) {
        return Boolean.TRUE.equals(internalFrame.getRootPane().getClientProperty(SubstanceLookAndFeel.WINDOW_MODIFIED));
    }
    ...
    public static boolean hasCloseButton(JTabbedPane tabbedPane, int tabIndex) {
        ...
        if (!tabbedPane.isEnabledAt(tabIndex)) return false;
        Component tabComponent = tabbedPane.getComponentAt(tabIndex);
        if (tabComponent instanceof JComponent) {
            Object compProp = ((JComponent) tabComponent).getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CLOSE_BUTTONS_PROPERTY);
            if (Boolean.TRUE.equals(compProp)) return true;
            if (Boolean.FALSE.equals(compProp)) return false;
        }
        Object tabProp = tabbedPane.getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CLOSE_BUTTONS_PROPERTY);
        if (Boolean.TRUE.equals(tabProp)) return true;
        if (Boolean.FALSE.equals(tabProp)) return false;
    ...
    public static int getCloseButtonSize(JTabbedPane tabbedPane, int tabIndex) {
        if (!SubstanceCoreUtilities.hasCloseButton(tabbedPane, tabIndex)) return 0;
    ...
    public static TabContentPaneBorderKind getContentBorderKind(JTabbedPane tabbedPane) {
        Object tabProp = tabbedPane.getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CONTENT_BORDER_KIND);
        if (tabProp instanceof TabContentPaneBorderKind) return (TabContentPaneBorderKind) tabProp;
        Object globalProp = UIManager.get(SubstanceLookAndFeel.TABBED_PANE_CONTENT_BORDER_KIND);
        if (globalProp instanceof TabContentPaneBorderKind) return (TabContentPaneBorderKind) globalProp;
    ...
    public static boolean toAnimateCloseIconOfModifiedTab(JTabbedPane tabbedPane, int tabIndex) {
        int tabCount = tabbedPane.getTabCount();
        if ((tabIndex < 0) || (tabIndex >= tabCount)) return false;
        if (!hasCloseButton(tabbedPane, tabIndex)) return false;
        Component tabComponent = tabbedPane.getComponentAt(tabIndex);
        if (tabComponent instanceof JComponent) {
            Object compProp = ((JComponent) tabComponent).getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CLOSE_BUTTONS_MODIFIED_ANIMATION);
            if (Boolean.TRUE.equals(compProp)) return true;
            if (Boolean.FALSE.equals(compProp)) return false;
        }
        Object tabProp = tabbedPane.getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CLOSE_BUTTONS_MODIFIED_ANIMATION);
        if (Boolean.TRUE.equals(tabProp)) return true;
        if (Boolean.FALSE.equals(tabProp)) return false;
    ...
    public static VolatileImage getBlankVolatileImage(int width, int height) {
        ...
        GraphicsDevice d = e.getDefaultScreenDevice();
        GraphicsConfiguration c = d.getDefaultConfiguration();
        VolatileImage compatibleImage = c.createCompatibleVolatileImage(width, height, Transparency.TRANSLUCENT);
        return compatibleImage;
    }
    ...
    public static boolean hasNoMinSizeProperty(AbstractButton button) {
        Object noMinSizeProperty = button.getClientProperty(SubstanceLookAndFeel.BUTTON_NO_MIN_SIZE_PROPERTY);
        if (Boolean.TRUE.equals(noMinSizeProperty)) return true;
        if (Boolean.FALSE.equals(noMinSizeProperty)) return false;
        Container parent = button.getParent();
        if (parent instanceof JComponent) {
            noMinSizeProperty = ((JComponent) parent).getClientProperty(SubstanceLookAndFeel.BUTTON_NO_MIN_SIZE_PROPERTY);
            if (Boolean.TRUE.equals(noMinSizeProperty)) return true;
            if (Boolean.FALSE.equals(noMinSizeProperty)) return false;
        }
    ...
    public static boolean hasFlatAppearance(Component comp, boolean defaultValue) {
        if (comp instanceof JCheckBox) return defaultValue;
        if (comp instanceof JRadioButton) return defaultValue;
        Component c = comp;
        if (c instanceof JComponent) {
            JComponent jcomp = (JComponent) c;
            Object flatProperty = jcomp.getClientProperty(SubstanceLookAndFeel.FLAT_PROPERTY);
            if (flatProperty != null) {
                if (Boolean.TRUE.equals(flatProperty)) return true;
                if (Boolean.FALSE.equals(flatProperty)) return false;
            }
        ...
        if (c != null) {
            ...
            if (parent instanceof JComponent) {
                JComponent jparent = (JComponent) parent;
                Object flatProperty = jparent.getClientProperty(SubstanceLookAndFeel.FLAT_PROPERTY);
                if (flatProperty != null) {
                    if (Boolean.TRUE.equals(flatProperty)) return true;
                    if (Boolean.FALSE.equals(flatProperty)) return false;
                }
    ...
    public static int getPopupFlyoutOrientation(JComboBox combobox) {
        Object comboProperty = combobox.getClientProperty(SubstanceLookAndFeel.COMBO_BOX_POPUP_FLYOUT_ORIENTATION);
        if (comboProperty instanceof Integer) return (Integer) comboProperty;
        Object globalProperty = UIManager.get(SubstanceLookAndFeel.COMBO_BOX_POPUP_FLYOUT_ORIENTATION);
        if (globalProperty instanceof Integer) return (Integer) globalProperty;
    ...
    public static void makeNonOpaque(Component comp, Map<Component, Boolean> opacitySnapshot) {
        ...
        if (comp instanceof Container) {
            ...
            for (int i = 0; i < cont.getComponentCount(); i++) makeNonOpaque(cont.getComponent(i), opacitySnapshot);
        }
    }
    ...
    public static void restoreOpaque(Component comp, Map<Component, Boolean> opacitySnapshot) {
        if (comp instanceof JComponent) {
            JComponent jcomp = (JComponent) comp;
            if (opacitySnapshot.containsKey(comp)) jcomp.setOpaque(opacitySnapshot.get(comp)); else jcomp.setOpaque(true);
        }
        if (comp instanceof Container) {
            Container cont = (Container) comp;
            for (int i = 0; i < cont.getComponentCount(); i++) restoreOpaque(cont.getComponent(i), opacitySnapshot);
        }
    }
    ...
    public static BufferedImage createCompatibleImage(BufferedImage image) {
        ...
        GraphicsDevice d = e.getDefaultScreenDevice();
        GraphicsConfiguration c = d.getDefaultConfiguration();
        BufferedImage compatibleImage = c.createCompatibleImage(image.getWidth(), image.getHeight(), Transparency.TRANSLUCENT);
    ...
    public static TabCloseCallback getTabCloseCallback(MouseEvent me, JTabbedPane tabbedPane, int tabIndex) {
        ...
        Component tabComponent = tabbedPane.getComponentAt(tabIndex);
        if (tabComponent instanceof JComponent) {
            Object compProp = ((JComponent) tabComponent).getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CLOSE_CALLBACK);
            if (compProp instanceof TabCloseCallback) return (TabCloseCallback) compProp;
        }
        Object tabProp = tabbedPane.getClientProperty(SubstanceLookAndFeel.TABBED_PANE_CLOSE_CALLBACK);
        if (tabProp instanceof TabCloseCallback) return (TabCloseCallback) tabProp;
        Object globProp = UIManager.get(SubstanceLookAndFeel.TABBED_PANE_CLOSE_CALLBACK);
        if (globProp instanceof TabCloseCallback) return (TabCloseCallback) globProp;
        return null;
    }
    ...
    public static BufferedImage blendImagesVertical(BufferedImage imageTop, BufferedImage imageBottom, double start, double end) {
        ...
        if (width != imageBottom.getWidth()) throw new IllegalArgumentException("Widths are not the same: " + imageTop.getWidth() + " and " + imageBottom.getWidth());
        int height = imageTop.getHeight();
        if (height != imageBottom.getHeight()) throw new IllegalArgumentException("Heights are not the same: " + imageTop.getHeight() + " and " + imageBottom.getHeight());
    ...
    public static BufferedImage blendImagesHorizontal(BufferedImage imageLeft, BufferedImage imageRight, double start, double end) {
        ...
        if (width != imageRight.getWidth()) throw new IllegalArgumentException("Widths are not the same: " + imageLeft.getWidth() + " and " + imageRight.getWidth());
        int height = imageLeft.getHeight();
        if (height != imageRight.getHeight()) throw new IllegalArgumentException("Heights are not the same: " + imageLeft.getHeight() + " and " + imageRight.getHeight());
        BufferedImage result = getBlankImage(width, height);
        Graphics2D graphics = (Graphics2D) result.getGraphics().create();
    ...
    public static SubstanceColorScheme getOptionPaneColorScheme(int messageType, SubstanceColorScheme mainScheme) {
        if (!SubstanceLookAndFeel.isToUseConstantThemesOnDialogs()) return mainScheme;
        switch(messageType) {
    ...
    public static Object getComboPopupPrototypeDisplayValue(JComboBox combo) {
        Object objProp = combo.getClientProperty(SubstanceLookAndFeel.COMBO_POPUP_PROTOTYPE);
        if (objProp == null) objProp = UIManager.get(SubstanceLookAndFeel.COMBO_POPUP_PROTOTYPE);
	 ...
	 */
    public static ScrollPaneButtonPolicyKind getScrollPaneButtonsPolicyKind(JScrollBar scrollBar) {
        Component parent = scrollBar.getParent();
        if (parent instanceof JScrollPane) {
            Object jspKind = ((JScrollPane) parent).getClientProperty(SubstanceLookAndFeel.SCROLL_PANE_BUTTONS_POLICY);
            if (jspKind instanceof ScrollPaneButtonPolicyKind) return (ScrollPaneButtonPolicyKind) jspKind;
        }
        Object globalJspKind = UIManager.get(SubstanceLookAndFeel.SCROLL_PANE_BUTTONS_POLICY);
        if (globalJspKind instanceof ScrollPaneButtonPolicyKind) return (ScrollPaneButtonPolicyKind) globalJspKind;
    ...
    public static float getToolbarButtonCornerRadius(JComponent button, Insets insets) {
        ...
        Object buttonProp = button.getClientProperty(SubstanceLookAndFeel.CORNER_RADIUS);
        if (buttonProp instanceof Float) return Math.min(maxRadius, ((Float) buttonProp).floatValue());
        Object toolbarProp = toolbar.getClientProperty(SubstanceLookAndFeel.CORNER_RADIUS);
        if (toolbarProp instanceof Float) return Math.min(maxRadius, ((Float) toolbarProp).floatValue());
        Object globalProp = UIManager.get(SubstanceLookAndFeel.CORNER_RADIUS);
        if (globalProp instanceof Float) return Math.min(maxRadius, ((Float) globalProp).floatValue());
        return 2.0f;
    }
    ...
    public static int getEchoPerChar(JPasswordField jpf) {
        Object obj = jpf.getClientProperty(SubstanceLookAndFeel.PASSWORD_ECHO_PER_CHAR);
    ...
    public static boolean toShowExtraWidgets(Component component) {
        ...
        while (c != null) {
            if (c instanceof JComponent) {
                JComponent jcomp = (JComponent) c;
                Object componentProp = jcomp.getClientProperty(SubstanceLookAndFeel.SHOW_EXTRA_WIDGETS);
                if (componentProp != null) {
                    if (Boolean.TRUE.equals(componentProp)) return false;
                    if (Boolean.FALSE.equals(componentProp)) return true;
                }
            }
            c = c.getParent();
        }
        return Boolean.TRUE.equals(UIManager.get(SubstanceLookAndFeel.SHOW_EXTRA_WIDGETS));
    }
    ...
    public static Icon getOriginalIcon(AbstractButton b, Icon defaultIcon) {
        ...
        if (icon.getClass().isAnnotationPresent(TransitionAware.class)) return icon;
        Icon tmpIcon = null;
        if (icon != null) {
            if (!model.isEnabled()) {
                if (model.isSelected()) {
    ...
    public static MenuGutterFillKind getMenuGutterFillKind() {
        Object globalSetting = UIManager.get(SubstanceLookAndFeel.MENU_GUTTER_FILL_KIND);
        if (globalSetting instanceof MenuGutterFillKind) return (MenuGutterFillKind) globalSetting;
    ...
    public static Container getHeaderParent(Component c) {
        ...
        while (comp != null) {
            if ((comp instanceof JLayeredPane) && (result == null)) result = (Container) comp;
            if ((result == null) && (comp instanceof Window)) result = (Container) comp;
            comp = comp.getParent();
        }
        return result;
    }
    ...
    public static void paintFocus(Graphics g, Component mainComp, Component focusedComp, TransitionAwareUI transitionAwareUI, Shape focusShape, Rectangle textRect, float maxAlphaCoef, int extraPadding) {
        ...
        if (focusStrength == 0.0f) return;
        FocusKind focusKind = SubstanceCoreUtilities.getFocusKind(mainComp);
        if (focusKind == FocusKind.NONE) return;
        Graphics2D graphics = (Graphics2D) g.create();
        graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        float alpha = maxAlphaCoef * focusStrength;
        graphics.setComposite(LafWidgetUtilities.getAlphaComposite(mainComp, alpha, g));
    ...
    public static boolean isTitleCloseButton(JComponent ab) {
        if ((ab instanceof SubstanceTitleButton) && Boolean.TRUE.equals(ab.getClientProperty(SubstanceButtonUI.IS_TITLE_CLOSE_BUTTON))) return true;
        return false;
    }
    ...
    public static void uninstallMenu(JMenuItem menuItem) {
        if (menuItem instanceof JMenu) {
            ...
            for (Component comp : menu.getMenuComponents()) if (comp instanceof JMenuItem) SubstanceCoreUtilities.uninstallMenu((JMenuItem) comp);
        }
        ...
        for (ActionListener actionListener : menuItem.getActionListeners()) menuItem.removeActionListener(actionListener);
        menuItem.removeAll();
    }
    ...
    public static void getHierarchy(Component comp, StringBuffer buffer, int level) {
        ...
        if (comp instanceof Dialog) name = ((Dialog) comp).getTitle();
        if (comp instanceof Frame) name = ((Frame) comp).getTitle();
        buffer.append(comp.getClass().getName() + "[" + name + "]\n");
        if (comp instanceof Container) {
            Container cont = (Container) comp;
            for (int i = 0; i < cont.getComponentCount(); i++) getHierarchy(cont.getComponent(i), buffer, level + 1);
        }
    }
    ...
    public static Icon getArrowIcon(JComponent comp, TransitionAwareIcon.TransitionAwareUIDelegate transitionAwareUIDelegate, int orientation) {
        Icon result = new ArrowButtonTransitionAwareIcon(comp, transitionAwareUIDelegate, orientation);
        return result;
    }
    ...
    public static double getColorizationFactor(Component c) {
        ...
        while (c != null) {
            if (c instanceof JComponent) {
                JComponent jcomp = (JComponent) c;
                Object compProp = jcomp.getClientProperty(SubstanceLookAndFeel.COLORIZATION_FACTOR);
                if (compProp instanceof Double) return (Double) compProp;
            }
        ...
        if (popupMenu != null) {
            ...
            if (popupMenu != invoker) return getColorizationFactor(popupMenu.getInvoker());
        }
        Object globalProp = UIManager.get(SubstanceLookAndFeel.COLORIZATION_FACTOR);
    ...
    public static SubstanceSkin getSkin(Component c) {
        if (!SubstanceLookAndFeel.isCurrentLookAndFeel()) return null;
        if (!SubstanceRootPaneUI.hasCustomSkinOnAtLeastOneRootPane()) return SubstanceLookAndFeel.getCurrentSkin();
        ...
        if (rootPane != null) {
            Object skinProp = rootPane.getClientProperty(SubstanceLookAndFeel.SKIN_PROPERTY);
            if (skinProp instanceof SubstanceSkin) return (SubstanceSkin) skinProp;
        }
    ...
    public static void traceSubstanceApiUsage(Component comp, String message) {
        ...
        throw new IllegalArgumentException(message + " [component " + comp.getClass().getSimpleName() + " in window " + w.getClass().getSimpleName() + ":'" + wTitle + "' under " + UIManager.getLookAndFeel().getName() + "]");
    }
    ...
    public static BufferedImage getScaledIconImage(java.util.List<Image> imageList, int width, int height) {
        ...
        for (Iterator<Image> i = imageList.iterator(); i.hasNext(); ) {
            ...
            if (iw > 0 && ih > 0) {
                ...
                double similarity = ((double) width - (double) adjw) / width + ((double) height - (double) adjh) / height + scaleMeasure;
