...
public class VCalendar10Format extends EndMatcher implements PIMFormat {
    ...
    private static final int[] DAYS_OF_WEEK = { RepeatRule.SUNDAY, RepeatRule.MONDAY, RepeatRule.TUESDAY, RepeatRule.WEDNESDAY, RepeatRule.THURSDAY, RepeatRule.FRIDAY, RepeatRule.SATURDAY };
    ...
    private static final int[] WEEKS_OF_MONTH = { RepeatRule.FIRST, RepeatRule.SECOND, RepeatRule.THIRD, RepeatRule.FOURTH, RepeatRule.FIFTH, RepeatRule.LAST, RepeatRule.SECONDLAST, RepeatRule.THIRDLAST, RepeatRule.FOURTHLAST, RepeatRule.FIFTHLAST };
    ...
    private static final int[] MONTHS_IN_YEAR = { 0, RepeatRule.JANUARY, RepeatRule.FEBRUARY, RepeatRule.MARCH, RepeatRule.APRIL, RepeatRule.MAY, RepeatRule.JUNE, RepeatRule.JULY, RepeatRule.AUGUST, RepeatRule.SEPTEMBER, RepeatRule.OCTOBER, RepeatRule.NOVEMBER, RepeatRule.DECEMBER };
    ...
    private String encodeRepeatRule(RepeatRule rule, int startFreq) {
        ...
        int interval = 1;
        if (FormatSupport.contains(fields, RepeatRule.INTERVAL) && (startFreq == 0)) {
            interval = rule.getInt(RepeatRule.INTERVAL);
        }
        String encodedCount = " #0";
        if (FormatSupport.contains(fields, RepeatRule.COUNT) && (startFreq == 0)) {
            encodedCount = " #" + rule.getInt(RepeatRule.COUNT);
        }
        String encodedEndDate = "";
        if (FormatSupport.contains(fields, RepeatRule.END)) {
            encodedEndDate = " " + PIMHandler.getInstance().composeDateTime(rule.getDate(RepeatRule.END));
        }
        switch(frequency) {
            ...
            case RepeatRule.MONTHLY:
                {
                    ...
                    if (FormatSupport.contains(fields, RepeatRule.DAY_IN_MONTH)) {
                        ...
                        sb.append(encodedCount);
                    } else if (FormatSupport.contains(fields, RepeatRule.WEEK_IN_MONTH)) {
            ...
            case RepeatRule.YEARLY:
                {
                    ...
                    if (FormatSupport.contains(fields, RepeatRule.DAY_IN_YEAR)) {
                        ...
                        sb.append(encodedCount);
                    } else if (FormatSupport.contains(fields, RepeatRule.MONTH_IN_YEAR)) {
                        sb.append(FormatSupport.MONTH_IN_YEAR);
                        sb.append(interval);
                        sb.append(encodeRepeatRuleMonthsInYear(fields, rule));
                        sb.append(encodedCount);
                        if (FormatSupport.contains(fields, RepeatRule.DAY_IN_MONTH) || FormatSupport.contains(fields, RepeatRule.WEEK_IN_MONTH)) {
                            sb.append(" " + encodeRepeatRule(rule, RepeatRule.MONTHLY));
                        }
                    }
                    break;
                }
     ...
     */
    private AbstractPIMItem decode(LineReader in, PIMList list) throws IOException {
        while (true) {
            ...
            if (element.propertyName.equals("BEGIN")) {
                ...
                if (!element.data.equals("1.0")) {
                    throw new UnsupportedPIMFormatException("vCalendar version '" + element.data + "' is not supported");
                }
     ...
     */
    private EventImpl decodeEvent(LineReader in, PIMList list) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (element.propertyName.equals("END")) {
                ...
                if (alarmValues > 0 && event.countValues(Event.START) > 0) {
                    int startTime = (int) (event.getDate(Event.START, 0) / 1000);
                    for (int i = 0, j = 0; i < alarmValues; i++, j++) {
                        int alarmTime = event.getInt(Event.ALARM, i);
                        if (alarmTime * 1000 < startTime) {
                            event.setInt(Event.ALARM, i, Event.ATTR_NONE, startTime - alarmTime);
                        } else {
                ...
                if (!element.data.equals("1.0")) {
                    throw new UnsupportedPIMFormatException("Version " + element.data + " is not supported");
                }
            } else if (element.propertyName.equals("CATEGORIES")) {
     ...
     *
     */
    private void decodeExDates(RepeatRule rule, String data) throws IOException {
        Parser parser = new Parser(data);
        long date;
        while (parser.hasNextDate()) {
            date = PIMHandler.getInstance().parseDate(parser.getEndDate());
     ...
     */
    private boolean decodeRepeatRule(RepeatRule rule, String data, boolean isTop) {
        boolean res = true;
        Parser parser = new Parser(data);
        char sym;
        try {
                ...
                case FormatSupport.MONTHLY:
                    ...
                    switch(sym) {
                        case FormatSupport.DAY_IN_MONTH:
                            ...
                            parser.skipBlank();
                            rule.setInt(RepeatRule.DAY_IN_MONTH, parser.readInt());
                            setRepeatRuleCount(parser, rule, isTop);
                            break;
                        case FormatSupport.WEEK_IN_MONTH:
                            ...
                            parser.skipBlank();
                            rule.setInt(RepeatRule.WEEK_IN_MONTH, decodeWeeksInMonth(parser, rule));
                            setRepeatRuleCount(parser, rule, isTop);
                            parser.skipBlank();
                            if (parser.hasMoreChars()) {
                                res = decodeRepeatRule(rule, parser.getRemainder(), false);
                            }
                            break;
                        default:
                            res = false;
                ...
                case FormatSupport.YEARLY:
                    ...
                    switch(sym) {
                        ...
                        case FormatSupport.MONTH_IN_YEAR:
                            ...
                            rule.setInt(RepeatRule.MONTH_IN_YEAR, decodeMonthsInYear(parser, rule));
                            setRepeatRuleCount(parser, rule, isTop);
                            parser.skipBlank();
                            if (parser.hasMoreChars()) {
                                res = decodeRepeatRule(rule, parser.getRemainder(), false);
     ...
     * @throws IOException if a reading error occurs or wrong format
     */
    private int decodeWeeksInMonth(Parser parser, RepeatRule rule) throws IOException {
        int weeksInMonth = 0;
        while (true) {
            parser.skipBlank();
     ...
     * @throws IOException if a reading error occurs or wrong format
     */
    private int decodeMonthsInYear(Parser parser, RepeatRule rule) throws IOException {
        int monthsInYear = 0;
        int monthNum;
        while (true) {
            parser.skipBlank();
     ...
     * @throws IOException if a reading error occurs or wrong format
     */
    private void setRepeatRuleCount(Parser parser, RepeatRule rule, boolean isSetCount) throws IOException {
        parser.skipBlank();
        if (parser.match('#')) {
            parser.skip();
        ...
        if (parser.hasNextDate()) {
            ...
            long date = (dateLen < 15) ? PIMHandler.getInstance().parseDate(parser.getEndDate()) : PIMHandler.getInstance().parseDateTime(parser.getEndDate());
            ...
            case Event.ALARM:
                {
                    ...
                    if (s.length > 0) {
                        ...
                        event.addInt(Event.ALARM, Event.ATTR_NONE, (int) (alarmTime / 1000));
                    }
                    break;
                }
        }
     ...
     */
    private ToDoImpl decodeToDo(LineReader in, PIMList list) throws IOException {
