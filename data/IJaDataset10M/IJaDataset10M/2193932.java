...
public class InterSequenceTransitionArrow extends TransitionArrow {
    ...
    private static class Factory implements ExtraCanvasItemFactory {
        ...
        public CanvasItem createCanvasItem(final SimpleLineDiagram diagram, final Element element) {
            return null;
        }
    }
    ...
    public InterSequenceTransitionArrow(final DiagramNode startNode, final DiagramNode endNode, final ArrowStyle style, final Color secondColor, final double timePos, final AnimationTimeController timeController) {
    ...
    protected Paint calculatePaint(final Color baseColor) {
        ...
        final Color finalStartColor = new Color(baseColor.getRed(), baseColor.getGreen(), baseColor.getBlue(), (int) (alpha * baseColor.getAlpha()));
        final Color finalEndColor = new Color(this.endColor.getRed(), this.endColor.getGreen(), this.endColor.getBlue(), (int) (alpha * this.endColor.getAlpha()));
        final float arrowLength = (float) this.startPoint.distance(this.endPoint);
