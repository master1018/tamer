...
public class MFCC extends FeatureExtractor {
    ...
    public double[] extractFeature(double[] samples, double sampling_rate, double[][] other_feature_values) throws Exception {
        int[] cbin = fe.fftBinIndices(sampling_rate, other_feature_values[0].length);
        double[] fbank = fe.melFilter(other_feature_values[0], cbin);
        ...
        switch(index) {
            case 0:
                try {
                    ...
                    definition = new FeatureDefinition(name, description, true, fe.numCepstra, attributes);
                    if (parent != null) {
                        parent.updateTable();
                    }
