...
public abstract class GenericElement implements Element, Serializable {
    ...
    */
    public void output(OutputStream out) {
        ...
        try {
            ...
            if (getFilterState()) out.write(getFilter().process(getTagText()).getBytes(encoding)); else out.write(getTagText().getBytes(encoding));
            if (getNeedClosingTag()) out.write(createEndTag().getBytes(encoding));
    ...
    */
    public void output(PrintWriter out) {
        ...
        if (getFilterState()) out.write(getFilter().process(getTagText())); else out.write(getTagText());
        if (getNeedClosingTag()) out.write(createEndTag());
    }
	...
	*/
    ...
    protected void putTabs(int tabCount, OutputStream out) throws IOException {
        for (int i = 0; i < tabCount; i++) out.write('\t');
    }

    protected void putTabs(int tabCount, PrintWriter out) {
        for (int i = 0; i < tabCount; i++) out.print('\t');
    }
}
