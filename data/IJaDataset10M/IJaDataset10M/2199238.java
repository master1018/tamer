...
public class CookieAndSessionCheckInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        if (switchOffCookieCheck) return true;
        ...
        if (cookieNameChecked != null) return true;
    ...
    private boolean bypassCookieCheck(HttpServletRequest request, HttpServletResponse response) {
        if (ignoreUrlPatterns != null && !ignoreUrlPatterns.isEmpty()) {
            ...
            for (String ignorePattern : ignoreUrlPatterns) {
                if (requestURI.indexOf(ignorePattern) != -1 || (queryParam != null && queryParam.contains(ignorePattern))) {
                    if (logger.isDebugEnabled()) logger.debug("Bypassing cookie check. URL:" + requestURI + " matches pattern: " + ignorePattern);
                    return true;
                }
            }
        }
	 ...
	 */
    public void setRobotsAgentsAllowedToBypassCookies(Set<String> robotsAgentsAllowedToBypassCookies) {
