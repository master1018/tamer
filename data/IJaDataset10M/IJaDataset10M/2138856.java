...
public class FileFilterPatternsBlock {
    ...
    public FileFilterPatternsBlock(Composite parent, IProject project, boolean isExclusions, List<IPath> initialPatterns) {
    ...
    private void addEntry(ListDialogField field) {
        FileFilterPatternDialog dialog = new FileFilterPatternDialog(shell, project, isExclusions, null);
    ...
    private void addMultipleEntries(ListDialogField field) {
        ...
        String message = "Choose files or folders to " + (isExclusions ? "exclude" : "include") + ":";
        List<IPath> patterns = FileFilterPatternDialog.choosePatterns(shell, title, message, project, null, true);
        if (patterns != null) {
    ...
    private void createContents(Composite parent) {
        ...
        String[] addButtons = new String[] { "Add...", "Add Multiple...", "Edit...", null, "Remove" };
        patternsField = new ListDialogField(new PatternsListSelectionAdapter(), addButtons, new PatternsListLabelProvider());
        patternsField.enableButton(IDX_EDIT, false);
        patternsField.setRemoveButtonIndex(IDX_REMOVE);
        patternsField.setTableColumns(new ListDialogField.ColumnsDescription(1, false));
    ...
    private void editEntry(ListDialogField field) {
        ...
        FileFilterPatternDialog dialog = new FileFilterPatternDialog(shell, project, isExclusions, entry);
        if (dialog.open() == Window.OK) {
