...
import com.googlecode.httl.support.Configurable;
...
public class Engine implements Configurable {
    ...
    public Template getTemplate(String name, String encoding) throws IOException, ParseException {
        ...
        if (template == null || (resource != null && resource.getLastModified() > template.getLastModified())) {
            lock.lock();
            try {
    ...
    public Template parseTemplate(String name, String encoding) throws IOException, ParseException {
        ...
        try {
            ...
            throw new ParseException("Failed to parse template " + name + ", cause: " + e.getMessage() + ". occur to offset: " + offset + (location == null || location.length() == 0 ? "" : ", " + location) + ", stack: " + ClassUtils.toString(e), offset);
        }
    }
