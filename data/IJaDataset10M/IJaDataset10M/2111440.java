...
public class MapPanel extends JPanel {
    ...
    public void paintMap(Graphics2D g2d) {
        ...
        for (int i = 0; i < lines.length; i++) {
            Line2D line = new Line2D.Float(xOffset + lines[i].x1 * pixelsPerUnit, yOffset + lines[i].y1 * pixelsPerUnit, xOffset + lines[i].x2 * pixelsPerUnit, yOffset + lines[i].y2 * pixelsPerUnit);
            g2d.draw(line);
        }
    }
