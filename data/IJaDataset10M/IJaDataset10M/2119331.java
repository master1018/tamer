...
public final class StatisticsPanel extends FreeColPanel implements ActionListener {
    ...
    */
    public StatisticsPanel(FreeColClient freeColClient, GUI gui) {
        ...
        Map<String, String> serverStatistics = getController().getServerStatistics();
        Map<String, String> clientStatistics = getController().getClientStatistics();
        JPanel header = new JPanel();
        this.add(header, BorderLayout.NORTH);
        header.add(new JLabel("Statistics"), JPanel.CENTER_ALIGNMENT);
        JPanel statsPanel = new JPanel(new GridLayout(1, 2));
        JScrollPane scrollPane = new JScrollPane(statsPanel, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
    ...
    private JPanel displayStatsMessage(String title, Map<String, String> stats) {
        final String[] memoryKeys = { "freeMemory", "totalMemory", "maxMemory" };
        ...
        for (String k : memoryKeys) {
            memory.put(Messages.message("menuBar.debug.memoryManager." + k), stats.remove(k));
        }
