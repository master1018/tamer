...
class ExtensionTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int column) {
        if (column == 0) return objs[0].getExtendedParamName(row); else return objs[0].getExtendedParam(row);
    }
    ...
    public void setValueAt(Object e, int row, int col) {
        for (int i = 0; i < objs.length; i++) objs[i].setExtendedParam(row, e.toString());
...
class JDExtensionPanel extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == newExtensionBtn) {
            ...
            if (newExt != null) {
                for (i = 0; i < allObjects.length; i++) allObjects[i].addExtension(newExt);
                JDUtils.modified = true;
