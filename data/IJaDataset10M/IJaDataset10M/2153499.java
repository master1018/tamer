...
public final class Coupling<E> {
    ...
    public void setBuffer() {
        final WorkspaceComponent producerComponent = producer.getParentComponent();
        try {
    ...
    public void update() {
        if ((consumer != null) && (producer != null)) {
            final WorkspaceComponent consumerComponent = consumer.getParentComponent();
            try {
                Workspace.syncRest(consumerComponent.getLocks().iterator(), new Callable<E>() {

                    public E call() throws Exception {
                        consumer.setValue(buffer);
                        LOGGER.debug(consumer.getParentComponent().getDescription() + " just consumed " + producer.getValue() + " from " + producer.getParentComponent().getDescription());
                        return null;
                    }
                });
            } catch (Exception e) {
    ...
    public boolean equals(final Object o) {
        if (o instanceof Coupling) {
            ...
            return other.producer.equals(producer) && other.consumer.equals(consumer);
        } else {
            return false;
        }
    }
