...
public class TextPresentation {
    ...
    public static void applyTextPresentation(TextPresentation presentation, StyledText text) {
        ...
        while (e.hasNext()) ranges[i++] = (StyleRange) e.next();
    ...
    class FilterIterator implements Iterator {
        ...
        protected void computeIndex() {
            while (fIndex < fLength && skip(fRanges.get(fIndex))) ++fIndex;
        }
    }
    ...
    private void applyStyleRange(StyleRange range, boolean merge) {
        ...
        if (fRanges.size() == 0) {
            ...
            if (defaultRange == null) defaultRange = range;
            ...
            for (int i = first; i < last && length > 0; i++) {
                ...
                if (end < currentEnd) currentCopy = (StyleRange) current.clone();
                if (start < currentStart) {
                    StyleRange defaultRange = getDefaultStyleRange();
                    if (defaultRange == null) defaultRange = new StyleRange();
                    defaultRange.start = start;
    ...
    private void applyStyle(StyleRange template, StyleRange target, boolean merge) {
        if (merge) {
            if (template.font != null) target.font = template.font;
            target.fontStyle |= template.fontStyle;
            if (template.metrics != null) target.metrics = template.metrics;
            if (template.foreground != null) target.foreground = template.foreground;
            if (template.background != null) target.background = template.background;
            target.strikeout |= template.strikeout;
            if (template.strikeoutColor != null) target.strikeoutColor = template.strikeoutColor;
            target.underline |= template.underline;
            if (template.underlineStyle != SWT.NONE) target.underlineStyle = template.underlineStyle;
            if (template.underlineColor != null) target.underlineColor = template.underlineColor;
            if (template.borderStyle != SWT.NONE) target.borderStyle = template.borderStyle;
            if (template.borderColor != null) target.borderColor = template.borderColor;
        } else {
    ...
    private void checkConsistency(StyleRange range) {
        ...
        if (fDefaultRange != null) {
            if (range.start < fDefaultRange.start) range.start = fDefaultRange.start;
            int defaultEnd = fDefaultRange.start + fDefaultRange.length;
            int end = range.start + range.length;
            if (end > defaultEnd) range.length -= (end - defaultEnd);
        }
    }
    ...
    private int getFirstIndexInWindow(IRegion window) {
        if (window != null) {
            ...
            while (j - i > 1) {
                ...
                if (r.start + r.length > start) j = k; else i = k;
            }
            return j;
        }
        return 0;
    ...
    private StyleRange createWindowRelativeRange(IRegion window, StyleRange range) {
        if (window == null || range == null) return range;
    ...
    private IRegion createWindowRelativeRegion(IRegion coverage) {
        if (fResultWindow == null || coverage == null) return coverage;
    ...
    public IRegion getExtent() {
        if (fExtent != null) return createWindowRelativeRegion(fExtent);
        return getCoverage();
    }
