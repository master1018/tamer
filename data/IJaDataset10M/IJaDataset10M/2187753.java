...
public class DiagonalMatrix implements SparseMatrix {
    ...
    private void checkIndices(int row, int col) {
        if (row < 0 || col < 0 || row >= values.length || col >= values.length) throw new ArrayIndexOutOfBoundsException();
    }
    ...
    public SparseDoubleVector getColumnVector(int column) {
        checkIndices(0, column);
        SparseDoubleVector columnValues = new SparseHashDoubleVector(values.length);
