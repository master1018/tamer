...
public class DeployPanel extends Composite implements SelectionListener {
    ...
    private static final int[] COLUMN_EDITORS = { EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_NONE, EditableTableViewer.EDITOR_BUTTON };
    ...
    public DeployPanel(Composite parent, int style, String projectName) {
        ...
        availableDeployableUnits = new EditableTableViewer(this, SWT.V_SCROLL | SWT.BORDER | SWT.SINGLE, COLUMN_NAMES, COLUMN_EDITORS, COLUMN_VALUES);
    ...
    private void deploy(String file) {
        try {
            ...
            Object result = (Object) mbeanServer.invoke(deploymentMBean, "install", new Object[] { url }, new String[] { String.class.getName() });
        } catch (Exception e) {
    ...
    private void undeploy(DeployableUnitID id) {
        try {
            if (mbeanServer == null) throw new Exception("Not connected to server.");
            Object result = (Object) mbeanServer.invoke(deploymentMBean, "uninstall", new Object[] { id }, new String[] { DeployableUnitID.class.getName() });
        } catch (Exception e) {
    ...
    private void addRemoteDUs() throws Exception {
        DeployableUnitID[] duIDs = (DeployableUnitID[]) mbeanServer.getAttribute(deploymentMBean, "DeployableUnits");
        DeployableUnitDescriptor[] descriptors = new DeployableUnitDescriptor[duIDs.length];
        for (int q = 0; q < duIDs.length; q++) {
            try {
                DeployableUnitDescriptor descriptor = (DeployableUnitDescriptor) mbeanServer.invoke(deploymentMBean, "getDescriptor", new Object[] { duIDs[q] }, new String[] { DeployableUnitID.class.getName() });
        ...
        for (int q = 0; q < duIDs.length; q++) {
            if (descriptors[q] != null) {
                duToDUDescription.put(getDUFileName(descriptors[q].getURL()), descriptors[q]);
    ...
    private void initDUs() {
        ...
        do {
            ...
            if (it != null) availableDeployableUnits.remove(it);
        } while (it != null);
        try {
            if (mbeanServer != null) addRemoteDUs();
            addUndeployedItems();
            update();
            TableColumn[] cols = availableDeployableUnits.getTable().getColumns();
            for (int q = 0; q < cols.length; q++) cols[q].pack();
        } catch (Exception e) {
            MessageDialog.openWarning(new Shell(), "Connection error", "Error retrieving the installed deployable units from the SLEE server." + e.toString());
        }
    }
    ...
    public void update() {
        super.update();
        if (wizardPage != null) wizardPage.dialogChanged();
    }
