...
public class PeriodicSpline implements java.io.Serializable {
    ...
    public void spline1D(double[] T, double[] Y, double[][] targetCoeff) {
        ...
        ddy[N] = ddy[0];
        for (int i = 0; i < N; i++) {
            targetCoeff[i][0] = (ddy[i + 1] - ddy[i]) / (6 * T[i]);
            targetCoeff[i][1] = ddy[i] / 2;
            targetCoeff[i][2] = (Y[i + 1] - Y[i]) / T[i] - (ddy[i + 1] + 2 * ddy[i]) * T[i] / 6;
            targetCoeff[i][3] = Y[i];
        }
    }
    ...
    private float orientationSpline(float x, float x0, float[] coeff) {
        float xnorm = x - x0;
        return coeff[2] + 2 * xnorm * coeff[1] + 3 * (float) Math.pow(xnorm, 2) * coeff[0];
    }
    ...
    private float curvatureSpline(float x, float x0, float[] coeff) {
        float xnorm = x - x0;
        return 2 * coeff[1] + 6 * xnorm * coeff[0];
    }
    ...
    public float osculatingCircle(float t, int splineIdx, Point2D center) {
        ...
        if (Math.abs(curveDenom) > 1e-10) {
            ...
            if (center != null) {
                center.setLocation(xpos - ydiff * orientNorm / curveDenom, ypos + xdiff * orientNorm / curveDenom);
            }
        } else {
    ...
    LinkedList<Point2D.Float> allPoints(float stepsize) {
        ...
        while ((curT < stopT) && (splineIdx < numXY)) {
            ...
            while (curT < nextStop - stepsize) {
                path.add(new Point2D.Float(evaluateX(curT, splineIdx), evaluateY(curT, splineIdx)));
                curT += stepsize;
            }
    ...
    public int newInterval(float T, int startIdx) {
        ...
        if (idx == 0) return numXY - 1; else return idx - 1;
    }
    ...
    public float getParam(int idx) {
        if ((idx < 0) || (idx > this.numXY)) return Float.NaN; else {
    ...
    public int getPositionAndOrientation(float T, int idx, Point2D pos, Point2D orient) {
        if ((pos == null) || (orient == null)) return -1;
        if (idx > numXY) {
            return -1;
        }
