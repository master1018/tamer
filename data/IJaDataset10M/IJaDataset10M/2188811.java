...
public class NavigateExample extends DefaultActivity {

    private static Binder binder = GWT.create(Binder.class);

    interface Binder extends UiBinder<Widget, NavigateExample> {
    }
    ...
    protected Widget createPlot() {
        ...
        plotOptions.setGlobalSeriesOptions(new GlobalSeriesOptions().setLineSeriesOptions(new LineSeriesOptions().setShow(true).setFill(true)).setShadowSize(0));
        plotOptions.setLegendOptions(new LegendOptions().setShow(false));
        plotOptions.addXAxisOptions(new AxisOptions().setZoomRange(0.1d, 10d).setPanRange(new Double[] { -10d, 10d }));
        plotOptions.addYAxisOptions(new AxisOptions().setZoomRange(0.1d, 10d).setPanRange(new Double[] { -10d, 10d }).setLabelWidth(20));
        plotOptions.setZoomOptions(new ZoomOptions().setInteractive(true)).setPanOptions(new PanOptions().setInteractive(true));
        ...
        plot.addZoomListener(new PlotZoomListener() {
            ...
            public void onPlotZoom(Axes axes) {
                message.setText("Zooming to x=[min:\"" + format.format(axes.getX().getMinimumValue()) + "\", max:\"" + format.format(axes.getX().getMaximumValue()) + "\"], y=[min:\"" + format.format(axes.getY().getMinimumValue()) + "\", max:\"" + format.format(axes.getY().getMaximumValue()) + "\"]");
            }
        });
        plot.addPanListener(new PlotPanListener() {
            ...
            public void onPlotPan(Axes axes) {
                message.setText("Panning to x=[min:\"" + format.format(axes.getX().getMinimumValue()) + "\", max:\"" + format.format(axes.getX().getMaximumValue()) + "\"], y=[min:\"" + format.format(axes.getY().getMinimumValue()) + "\", max:\"" + format.format(axes.getY().getMaximumValue()) + "\"]");
            }
        });
