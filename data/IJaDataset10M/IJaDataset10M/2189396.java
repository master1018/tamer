...
public class CSVParser {
    ...
    public int[] parseInt() throws NumberFormatException {
            ...
            if (rpos <= 0) try {
                ...
                if (range_allowed) {
                    ...
                    try {
                        ...
                        for (int i = iv; i <= fv; ++i) values.add(new Integer(i));
                    } catch (NumberFormatException nfe) {
        ...
        for (int v = 0; v < values.size(); ++v) ivalues[v] = ((Integer) values.elementAt(v)).intValue();
        return ivalues;
    }
    ...
    public double[] parseDouble() throws NumberFormatException {
        ...
        for (int v = 0; v < values.size(); ++v) dvalues[v] = ((Double) values.elementAt(v)).doubleValue();
        return dvalues;
    }
    ...
    public static void main(String args[]) {
        ...
        if (dd != null) for (int i = 0; i < dd.length; ++i) log.debug("array[" + i + "] = " + dd[i]);
        parser = new CSVParser("1,-3,4-8,11");
        int[] ii = parser.parseInt();
        log.debug("Integer values:");
        if (ii != null) for (int i = 0; i < ii.length; ++i) log.debug("array[" + i + "] = " + ii[i]);
    }
}
