...
public class CookiesPage extends WebPage {
    ...
    void setCookies() {
        ...
        ((WebResponse) getResponse()).addCookie(new Cookie(COOKIE_HELPER_NAME, Bytes.toHexUpper(Bytes.random(4))));
    }

    public CookiesPage(String gotoPrompt) {
        newCookieEntries.clear();
        for (Cookie cookie : ((WebRequest) getRequest()).getCookies()) {
            if (!cookie.getName().equalsIgnoreCase("JSESSIONID") && !cookie.getName().equalsIgnoreCase(COOKIE_HELPER_NAME)) {
                oldCookies.add(cookie);
