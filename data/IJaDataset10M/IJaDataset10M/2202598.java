...
public class DmcTypeUserRIREFMAP extends DmcTypeUserRIREF implements Serializable {
    ...
    void initValue() {
        if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED) value = new HashMap<DotName, UserRIREF>(); else value = new TreeMap<DotName, UserRIREF>();
    }
    ...
    public DmcAttribute<UserRIREF> cloneIt() {
        synchronized (this) {
            ...
            for (UserRIREF val : value.values()) try {
                rc.add(val);
    ...
    public UserRIREF del(Object key) {
        synchronized (this) {
            if (key instanceof DotName) return (value.remove(key)); else throw (new IllegalStateException("Incompatible key type: " + key.getClass().getName() + " passed to del():" + getName()));
        }
    }
    ...
    public Iterator<UserRIREF> getMV() {
        synchronized (this) {
            ...
            if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED) clone = new HashMap<DotName, UserRIREF>(value); else clone = new TreeMap<DotName, UserRIREF>(value);
    ...
    public UserRIREF getByKey(Object key) {
        synchronized (this) {
            if (key instanceof DotName) return (value.get((DotName) key)); else throw (new IllegalStateException("Incompatible type: " + key.getClass().getName() + " passed to del():" + getName()));
        }
    }
    ...
    public boolean containsKey(Object key) {
        synchronized (this) {
            if (key instanceof DotName) return (value.containsKey(key));
            return (false);
        }
    }
}
