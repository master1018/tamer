...
public class JSGFRuleAlternatives extends JSGFRule {
    ...
    public void append(JSGFRule rule) {
        assert rule != null;
        rules.add(rule);
        if (weights != null) weights.add(1.0f);
    }
    ...
    public void setWeights(List<Float> newWeights) throws IllegalArgumentException {
        ...
        for (Float w : newWeights) {
            if (Float.isNaN(w)) throw new IllegalArgumentException("illegal weight value: NaN");
            if (Float.isInfinite(w)) throw new IllegalArgumentException("illegal weight value: infinite");
            if (w < 0.0D) {
    ...
    public String toString() {
        ...
        for (int i = 0; i < rules.size(); ++i) {
            if (i > 0) sb.append(" | ");
            if (weights != null) sb.append("/" + weights.get(i) + "/ ");
            JSGFRule r = rules.get(i);
            if (rules.get(i) instanceof JSGFRuleAlternatives) sb.append("( ").append(r).append(" )"); else {
                sb.append(r);
            }
        }
        return sb.toString();
