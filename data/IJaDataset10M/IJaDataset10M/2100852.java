 ...
 */
public class RawDataImporter implements MZmineModule, ActionListener, BatchStep, TaskListener {
    ...
    public void initModule() {
        ...
        desktop.addMenuItem(MZmineMenu.RAWDATA, MODULE_NAME, "This module imports raw data files into the project", KeyEvent.VK_I, true, this, null);
    }
    ...
    public Task[] runModule(RawDataFile[] dataFiles, PeakList[] peakLists, ParameterSet parameters) {
        ...
        String fileNames = (String) rawDataImporterParameters.getParameterValue(RawDataImporterParameters.fileNames);
        ...
        for (int i = 0; i < splitFilenames.length; i++) {
            ...
            try {
                ...
                desktop.displayErrorMessage("Could not create a new temporary file " + e);
            ...
            if (openTasks[i] == null) {
                desktop.displayErrorMessage("Cannot determine file type of file " + file);
    ...
    public void statusChanged(TaskEvent e) {
        if (e.getStatus() == TaskStatus.FINISHED) {
            MZmineCore.getCurrentProject().addFile((RawDataFile) e.getSource().getCreatedObjects()[0]);
        }
    }
    ...
    public ExitCode setupParameters(ParameterSet parameterSet) {
        ...
        String path = (String) parameters.getParameterValue(RawDataImporterParameters.lastDirectory);
        File lastPath = null;
        ...
        if (exitCode == ExitCode.OK) {
            ...
            parameters.setParameterValue(RawDataImporterParameters.lastDirectory, lastDir);
            File[] selectedFiles = dialog.getSelectedFiles();
            if (selectedFiles.length == 0) return ExitCode.CANCEL;
            ...
            parameters.setParameterValue(RawDataImporterParameters.fileNames, fileNames.toString());
        }
        return exitCode;
    }
}
