...
public class Skill_Arrest extends StdSkill {
    ...
    public static Vector getWarrantsOf(MOB target, Area legalA) {
        ...
        if (B != null) {
            ...
            for (int i = warrants.size() - 1; i >= 0; i--) {
                ...
                if (W.crime().equalsIgnoreCase("pardoned")) warrants.removeElementAt(i);
            }
        }
        return warrants;
    }
    ...
    public void makePeace(Room R, MOB mob, MOB target) {
        ...
        for (int i = 0; i < R.numInhabitants(); i++) {
            ...
            if ((inhab != null) && (inhab.isInCombat())) {
                if (inhab.getVictim() == mob) inhab.makePeace();
                if (inhab.getVictim() == target) inhab.makePeace();
            }
        }
    }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        } else return beneficialVisualFizzle(mob, target, "<S-NAME> rear(s) back and attempt(s) to knock <T-NAMESELF> out, but fail(s).");
        return success;
    }
}
