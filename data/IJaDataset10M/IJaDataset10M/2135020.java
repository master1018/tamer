...
public class DAOCotPreco extends AbstractDAO {
    ...
    public boolean[] buscaInfoUsuAtual(boolean baprovacab, boolean bcotacao, String codcc, Integer codempcc, Integer codfilialcc) throws SQLException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (sAprova != null) {
                    if (!sAprova.equals("ND")) {
                        if (sAprova.equals("TD")) baprovacab = true; else if ((codcc.trim().equals(rs.getString("CODCC").trim())) && (codempcc == rs.getInt("CODEMPCC")) && (codfilialcc == rs.getInt("CODFILIALCC")) && (sAprova.equals("CC"))) {
                            baprovacab = true;
                        } else {
                            baprovacab = false;
                        }
            ...
            if (sCotacao != null) {
                if (sCotacao.equals("S")) bcotacao = true; else bcotacao = false;
            }
            getConn().commit();
        } finally {
            rs = null;
