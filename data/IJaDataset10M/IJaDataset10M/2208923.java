...
public final class FormLayout implements LayoutManager2, Serializable {
    ...
    public FormLayout(ColumnSpec[] colSpecs, RowSpec[] rowSpecs) {
        if (colSpecs == null) throw new NullPointerException("The column specifications must not be null.");
        if (rowSpecs == null) throw new NullPointerException("The row specifications must not be null.");
    ...
    public CellConstraints getConstraints(Component component) {
        if (component == null) throw new NullPointerException("The component must not be null.");
        CellConstraints constraints = (CellConstraints) constraintMap.get(component);
        if (constraints == null) throw new NullPointerException("The component has not been added to the container.");
    ...
    public void setConstraints(Component component, CellConstraints constraints) {
        if (component == null) throw new NullPointerException("The component must not be null.");
        if (constraints == null) throw new NullPointerException("The constraints must not be null.");
    ...
    private void initializeColAndRowComponentLists() {
        ...
        for (Iterator i = constraintMap.entrySet().iterator(); i.hasNext(); ) {
            ...
            CellConstraints constraints = (CellConstraints) entry.getValue();
            if (constraints.gridWidth == 1) colComponents[constraints.gridX - 1].add(component);
            if (constraints.gridHeight == 1) rowComponents[constraints.gridY - 1].add(component);
        }
    }
    ...
    private int[] compressedSizes(List formSpecs, int totalSize, int totalMinSize, int totalPrefSize, int[] minSizes, int[] prefSizes) {
        if (totalSize < totalMinSize) return minSizes;
        if (totalSize >= totalPrefSize) return prefSizes;
    ...
    private int[] distributedSizes(List formSpecs, int totalSize, int totalPrefSize, int[] inputSizes) {
        ...
        if (totalFreeSpace < 0) return inputSizes;
        int count = formSpecs.size();
        ...
        if (totalWeight == 0.0) return inputSizes;
        int[] sizes = new int[count];
    ...
    private int[] computeMaximumFixedSpanTable(List formSpecs) {
        ...
        for (int i = size - 1; i >= 0; i--) {
            ...
            if (maximumFixedSpan < Integer.MAX_VALUE) maximumFixedSpan++;
        }
        return table;
    }
