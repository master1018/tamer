...
public class Strings {
    ...
    public static String toAscii(String src) {
        ...
        for (int i = 0; i < src.length(); i++) {
            char c = src.charAt(i);
            if (c <= ' ') buf.append(' '); else if (c < 127) buf.append(c); else {
                switch(c) {
                    case 'Ä':
                        buf.append("Ae");
                        break;
    ...
    public static String beautify(String s) {
        ...
        if (s.length() > 0) {
            ...
            for (int i = 1; i < s.length() - 1; i++) {
                char c = s.charAt(i);
                if (Character.isUpperCase(c) && Character.isLowerCase(s.charAt(i - 1)) && Character.isLowerCase(s.charAt(i + 1))) buf.append(" ");
                buf.append(c);
            }
            if (s.length() > 1) buf.append(s.charAt(s.length() - 1));
        }
        return buf.toString();
    }
    ...
    public static String lTrim(String s, String chars) {
        int i = 0;
        int len = s.length();
        while (i < len && (chars == null ? (s.charAt(i) <= ' ') : (chars.indexOf(s.charAt(i)) != -1))) {
            i++;
        }
    ...
    public static String rTrim(String s, String chars) {
        int i = s.length() - 1;
        while (i >= 0 && (chars == null ? (s.charAt(i) <= ' ') : (chars.indexOf(s.charAt(i)) != -1))) {
            i--;
        }
