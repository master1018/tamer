...
public class ParetoChartPlotter extends LabelRotatingPlotterAdapter {
    ...
    private JFreeChart createChart() {
        if (data.getItemCount() > 0) {
            ...
            final JFreeChart chart = ChartFactory.createBarChart(null, this.dataTable.getColumnName(this.groupByColumn), "Count", categoryDataset, PlotOrientation.VERTICAL, true, true, false);
    ...
    private void updateValueBox() {
        ...
        if (this.dataTable.isNominal(this.countColumn)) {
            ...
            if (countValue != null) countValues.setSelectedItem(countValue);
        }
    }
    ...
    public void setPlotColumn(int index, boolean plot) {
        if (plot) this.countColumn = index; else this.countColumn = -1;
        updateValueBox();
        repaint();
    }
    ...
    public void setAxis(int index, int dimension) {
        if (index == 0) groupByColumn = dimension;
        repaint();
    }

    @Override
    public int getAxis(int index) {
        if (index == 0) return groupByColumn; else return -1;
    }

    @Override
    public String getAxisName(int index) {
        if (index == 0) return "Group-By Column"; else return "Unknown";
    }
    ...
    private void prepareData() {
        synchronized (dataTable) {
            ...
            if (!this.dataTable.isNominal(this.groupByColumn)) return;
            if (!this.dataTable.isNominal(this.countColumn)) return;
    ...
    public List<ParameterType> getAdditionalParameterKeys(InputPort inputPort) {
        ...
        if (inputDeliversAttributes) types.add(new ParameterTypeAttribute(PARAMETER_COUNT_VALUE, "The value which should be counted.", inputPort, false)); else types.add(new ParameterTypeString(PARAMETER_COUNT_VALUE, "The value which should be counted.", false));
