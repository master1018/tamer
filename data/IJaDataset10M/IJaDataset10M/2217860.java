...
public class Maze {
    ...
    public boolean isBorder(int x, int y, int direction) {
        ...
        if (x >= this.columns || y >= this.rows) return true;
        switch(direction) {
            case NORTH:
                if (y < 1) return true;
                break;
    ...
    public boolean isWall(int x, int y, int direction) {
        if (isBorder(x, y, direction)) return true;
        switch(direction) {
            case NORTH:
                if (y <= 0) return true; else return (columnWalls[x][y - 1]);
            case SOUTH:
                if (y >= (this.rows - 1)) return true; else return (columnWalls[x][y]);
            case EAST:
                if (x >= (this.columns - 1)) return true; else return (rowWalls[y][x]);
            case WEST:
                if (x <= 0) return true; else return (rowWalls[y][x - 1]);
        }
        return false;
    }
    ...
    public boolean inTact(int x, int y) {
        ...
        if (!isWall(x, y, NORTH)) return false;
        if (!isWall(x, y, SOUTH)) return false;
        if (!isWall(x, y, EAST)) return false;
        if (!isWall(x, y, WEST)) return false;
        return true;
    }
    ...
    public void generateMaze() {
        ...
        while (visitedCells < totalCells) {
            ...
            if (inTact(currentCell.x - 1, currentCell.y)) neighbors.add(WEST);
            if (inTact(currentCell.x + 1, currentCell.y)) neighbors.add(EAST);
            if (inTact(currentCell.x, currentCell.y + 1)) neighbors.add(SOUTH);
            if (inTact(currentCell.x, currentCell.y - 1)) neighbors.add(NORTH);
            if (neighbors.size() > 0) {
