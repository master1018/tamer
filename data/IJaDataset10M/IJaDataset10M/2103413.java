...
public class If_c extends Stmt_c implements If {
    ...
    public Node visitChildren(NodeVisitor v) {
        ...
        if (consequent instanceof NodeList) consequent = ((NodeList) consequent).toBlock();
        Node alternative = visitChild(this.alternative, v);
        if (alternative instanceof NodeList) alternative = ((NodeList) alternative).toBlock();
    ...
    public String toString() {
        return "if (" + cond + ") " + consequent + (alternative != null ? " else " + alternative : "");
    }
    ...
    public List<Term> acceptCFG(CFGBuilder v, List<Term> succs) {
        if (alternative == null) {
            v.visitCFG(cond, FlowGraph.EDGE_KEY_TRUE, consequent, ENTRY, FlowGraph.EDGE_KEY_FALSE, this, EXIT);
            v.visitCFG(consequent, this, EXIT);
        } else {
            v.visitCFG(cond, FlowGraph.EDGE_KEY_TRUE, consequent, ENTRY, FlowGraph.EDGE_KEY_FALSE, alternative, ENTRY);
            v.visitCFG(consequent, this, EXIT);
