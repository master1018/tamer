...
public class ClassicalColoredSwingClient implements InputOutputClient {
    ...
    public void addToBackStack(String s) {
        if (!(!back.isEmpty() && s.equalsIgnoreCase((String) back.peek()))) back.push(s);
    }

    public void addToForwardStack(String s) {
        if (!(!forward.isEmpty() && s.equalsIgnoreCase((String) back.peek()))) forward.push(s);
    }

    public void forwardStackIntoBackStack() {
        while (!forward.isEmpty()) {
            Object o = forward.pop();
            if (!o.equals("")) back.push(o);
        }
    }
    ...
    public static java.awt.Color stringToColor(String colorString) {
        try {
            String colorClean;
            if (colorString.length() > 0 && colorString.charAt(0) == '#') colorClean = colorString.substring(1); else colorClean = colorString;
    ...
    public void doWrite(String s) {
        ...
        while (st.hasMoreTokens()) {
            String tok = st.nextToken();
            if (iscode) {
                if (tok.equalsIgnoreCase("red")) StyleConstants.setForeground(atributos, Color.red); else if (tok.equalsIgnoreCase("green")) StyleConstants.setForeground(atributos, Color.green); else if (tok.equalsIgnoreCase("white")) StyleConstants.setForeground(atributos, Color.white); else if (tok.equalsIgnoreCase("blue")) StyleConstants.setForeground(atributos, Color.blue); else if (tok.equalsIgnoreCase("yellow")) StyleConstants.setForeground(atributos, Color.blue); else if (tok.equalsIgnoreCase("lightgray")) StyleConstants.setForeground(atributos, Color.blue); else if (tok.equalsIgnoreCase("magenta")) StyleConstants.setForeground(atributos, Color.blue); else if (tok.trim().equalsIgnoreCase("")) StyleConstants.setForeground(atributos, stringToColor(getColorCode("default"))); else {
                    try {
                        String colortok;
                        if (tok.length() > 0 && tok.charAt(0) == '#') colortok = tok.substring(1); else colortok = tok;
