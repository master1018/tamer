package unibo.agent.dsl.instances.parser;
...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
    ...
    private dslInstancesLexer lexer;

    private dslInstancesParser parser;

    public GenParser(InputStream in) {
        this.reader = new InputStreamReader(in);
    }

    public dslInstancesLexer getLexer() {
        if (lexer == null) {
            try {
                this.lexer = new dslInstancesLexer(new ANTLRReaderStream(reader));
            } catch (IOException e) {
                handleError(e);
            }
        }
    ...
    public dslInstancesParser getParser() {
        if (parser == null) {
            parser = new dslInstancesParser(new CommonTokenStream(getLexer()));
        }
        return parser;
    }
    ...
    protected void internalLink() {
        ...
        try {
            ...
            XtendFacade facade = XtendFacade.create(ctx, "unibo::agent::dsl::instances::Linking");
    ...
    public Issues doCheck() {
        ...
        try {
            ...
            String[] checkFiles = new String[] { "unibo::agent::dsl::instances::GenChecks", "unibo::agent::dsl::instances::Checks" };
