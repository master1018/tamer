...
public abstract class GenericAppearance extends DecoratorAppearance {
   ...
   */
    ...
    public Pixmap getPixmap(String key) {
        if (pixmaps == null) return null; else return pixmaps.get(key);
    }

    public void addPixmap(String key, Pixmap pixmap) {
        if (pixmaps == null) pixmaps = new HashMap<String, Pixmap>();
        pixmaps.remove(key);
        pixmaps.put(key, pixmap);
    }

    public Color getColor(String key) {
        if (colors == null) return null; else return colors.get(key);
    }

    public void addColor(String key, Color color) {
        if (colors == null) colors = new HashMap<String, Color>();
        colors.remove(key);
        colors.put(key, color);
    }

    public Integer getInteger(String key) {
        if (integers == null) return null; else {
            IntegerStore store = integers.get(key);
            if (store != null) return integers.get(key).value; else return null;
        }
    }

    public void addInteger(String key, Integer integer) {
        if (integers == null) integers = new HashMap<String, IntegerStore>();
        integers.remove(key);
        integers.put(key, new IntegerStore(integer));
    }

    public Spacing getSpacing(String key) {
        if (spacings == null) return null; else return spacings.get(key);
    }

    public void addSpacing(String key, Spacing spacing) {
        if (spacings == null) spacings = new HashMap<String, Spacing>();
        spacings.remove(key);
        spacings.put(key, spacing);
    }

    public IDecorator getDecorator(String key) {
        if (decorators != null) return decorators.get(key); else return null;
    }

    public void addDecorator(String key, IDecorator decorator) {
        if (decorators == null) decorators = new HashMap<String, IDecorator>();
        decorators.remove(key);
        decorators.put(key, decorator);
    }

    public Alignment getAlignment(String key) {
        if (alignments != null) return alignments.get(key); else return null;
    }

    public void addAlignment(String key, Alignment decorator) {
        if (alignments == null) alignments = new HashMap<String, Alignment>();
    ...
    protected DecoratorAppearance clone(StandardWidget widget) {
        ...
        if (this.alignments != null) {
            ...
            for (Entry<String, Alignment> align : this.alignments.entrySet()) result.alignments.put(align.getKey(), align.getValue());
        }
        if (this.decorators != null) {
            result.decorators = new HashMap<String, IDecorator>(this.decorators.size());
            for (Entry<String, IDecorator> align : this.decorators.entrySet()) result.decorators.put(align.getKey(), align.getValue().copy());
        }
        if (this.spacings != null) {
            result.spacings = new HashMap<String, Spacing>(this.spacings.size());
            for (Entry<String, Spacing> align : this.spacings.entrySet()) result.spacings.put(align.getKey(), align.getValue().clone());
        }
        if (this.colors != null) {
            result.colors = new HashMap<String, Color>(this.colors.size());
            for (Entry<String, Color> align : this.colors.entrySet()) result.colors.put(align.getKey(), align.getValue().clone());
        }
        if (this.integers != null) {
            result.integers = new HashMap<String, IntegerStore>(this.integers.size());
            for (Entry<String, IntegerStore> align : this.integers.entrySet()) result.integers.put(align.getKey(), align.getValue().clone());
        }
        if (this.pixmaps != null) {
            result.pixmaps = new HashMap<String, Pixmap>(this.pixmaps.size());
            for (Entry<String, Pixmap> align : this.pixmaps.entrySet()) result.pixmaps.put(align.getKey(), align.getValue());
        }
        return result;
    }
}
