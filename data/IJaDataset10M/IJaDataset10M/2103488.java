...
public class CMLEigen extends AbstractEigen {
    ...
    public enum Orientation {
        ...
        /** values down */
        VALUES_ROWS("rowVectors", "eigenvalues correspond to rows"), /** values across */
    ...
    public CMLEigen(CMLMatrix eigenvectors, CMLArray eigenvalues, Orientation orient) {
        ...
        if (eigenvectors.getRows() != eigenvectors.getColumns()) {
            throw new RuntimeException("eigenvector matrix must be square: rows(" + eigenvectors.getRows() + ") columns (" + eigenvectors.getColumns() + CMLConstants.S_RBRAK);
        }
        if (eigenvalues.getSize() != eigenvectors.getColumns()) {
            throw new RuntimeException("eigenvector matrix (" + eigenvectors.getColumns() + ") incompatible with eigenvalues (" + eigenvalues.getSize() + CMLConstants.S_RBRAK);
        }
    ...
    public CMLMatrix getEigenvectors() {
        CMLMatrix eigenvectors = (this.getMatrixElements().size() == 1) ? this.getMatrixElements().get(0) : null;
        if (eigenvectors != null && !XSD_DOUBLE.equals(eigenvectors.getDataType())) {
    ...
    public CMLArray getEigenvalues() {
        CMLArray eigenvalues = (this.getArrayElements().size() == 1) ? this.getArrayElements().get(0) : null;
        if (eigenvalues != null && !XSD_DOUBLE.equals(eigenvalues.getDataType())) {
    ...
    public RealArray getEigenvector(int serial) throws RuntimeException {
        ...
        if (matrix != null) {
            ...
            if (matrix != null && serial < matrix.getRows() && XSD_DOUBLE.equals(matrix.getDataType())) {
