...
public class Matrix {
    ...
    public static Matrix identity(int n) {
        ...
        for (int i = 0; i < n; i++) id.values[i][i] = 1;
        return id;
    }
    ...
    public Matrix transpose() {
        ...
        for (int i = 0; i < values.length; i++) for (int j = 0; j < values[0].length; j++) a.values[j][i] = this.values[i][j];
        return a;
    }
    ...
    public Matrix plus(Matrix b) {
        if (b.values.length != this.values.length || b.values[0].length != this.values[0].length) throw new RuntimeException("Illegal values.lengthatrix divalues.lengthensions.");
        Matrix c = new Matrix(values.length, values[0].length);
        for (int i = 0; i < values.length; i++) for (int j = 0; j < values[0].length; j++) c.values[i][j] = this.values[i][j] + b.values[i][j];
        return c;
    }
    ...
    public boolean equals(Object other) {
        ...
        if (b.values.length != this.values.length || b.values[0].length != this.values[0].length) throw new RuntimeException("Illegal values.lengthatrix divalues.lengthensions.");
        for (int i = 0; i < values.length; i++) for (int j = 0; j < values[0].length; j++) if (this.values[i][j] != b.values[i][j]) return false;
        return true;
    }
    ...
    public Matrix times(Matrix b) {
        ...
        for (int i = 0; i < c.values.length; i++) for (int j = 0; j < c.values[0].length; j++) for (int k = 0; k < this.values[0].length; k++) c.values[i][j] += (this.values[i][k] * b.values[k][j]);
        return c;
    }
    ...
    public Matrix solve(Matrix rhs) {
        if (values.length != values[0].length || rhs.values.length != values[0].length || rhs.values[0].length != 1) throw new RuntimeException("Illegal values.lengthatrix divalues.lengthensions.");
        Matrix a = new Matrix(this);
        Matrix b = new Matrix(rhs);
        for (int i = 0; i < values[0].length; i++) {
            int max = i;
            for (int j = i + 1; j < values[0].length; j++) if (Math.abs(a.values[j][i]) > Math.abs(a.values[max][i])) max = j;
            a.swap(i, max);
            b.swap(i, max);
            if (a.values[i][i] == 0.0) throw new RuntimeException("Matrix is singular.");
            for (int j = i + 1; j < values[0].length; j++) b.values[j][0] -= b.values[i][0] * a.values[j][i] / a.values[i][i];
        ...
        for (int j = values[0].length - 1; j >= 0; j--) {
            double t = 0.0;
            for (int k = j + 1; k < values[0].length; k++) t += a.values[j][k] * x.values[k][0];
    ...
    public void show() {
        for (int i = 0; i < values.length; i++) {
            for (int j = 0; j < values[0].length; j++) System.out.printf("%9.4f ", values[i][j]);
            System.out.println();
        }
    }
}
