    ...
    public TableSenone(Loader loader) {
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < cd.length; i++) {
                if (!(cd[i] instanceof CompositeSenone)) cd2ci[(int) cd[i].getID()] = (int) ci[i].getID();
                if (ci[i].getID() >= maxCI) maxCI = (int) ci[i].getID();
            }
        }
        lesCDdeCI = new AI[maxCI + 1];
        for (int i = 0; i < lesCDdeCI.length; i++) lesCDdeCI[i] = new AI();
        for (int i = 0; i < cd2ci.length; i++) lesCDdeCI[cd2ci[i]].add(i);
    }
