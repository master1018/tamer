package org.deveel.csharpcc.csdoc;

import org.deveel.csharpcc.parser.*;

public class Main {

    static void help_message() {
        java.io.PrintWriter ostr = new java.io.PrintWriter(new java.io.OutputStreamWriter(System.out));
        ostr.println("    csdoc option-settings - (to read from standard input)");
        ostr.println("OR");
        ostr.println("    csdoc option-settings inputfile (to read from a file)");
        ostr.println("");
        ostr.println("WHERE");
        ...
        ostr.println("    csdoc -ONE_TABLE=false mygrammar.csc");
        ostr.println("    csdoc - < mygrammar.csc");
        ostr.println("");
        ostr.println("ABOUT CSDoc:");
        ostr.println("    CSDoc generates documentation from CSharpCC grammar files.");
        ostr.println("");
    ...
    public static void main(String args[]) throws Exception {
        CSharpCCGlobals.bannerLine("Documentation Generator", "1.0.0");
        ...
        if (args.length == 0) {
            ...
            System.out.println("(type \"csdoc\" with no arguments for help)");
        }
        CSDocInit();
        ...
        if (args[args.length - 1].equals("-")) {
            ...
            parser = new CSharpCCParser(new java.io.DataInputStream(System.in));
            CSDocGlobals.input_file = "standard input";
            CSDocGlobals.output_file = "standard output";
        } else {
            ...
            try {
                ...
                CSDocGlobals.input_file = fp.getName();
                parser = new CSharpCCParser(new java.io.FileReader(args[args.length - 1]));
        ...
        try {
            parser.minossecc_input();
            CSDoc.start();
            if (CSharpCCErrors.get_error_count() == 0) {
                if (CSharpCCErrors.get_warning_count() == 0) {
                    System.out.println("Grammar documentation generated successfully in " + CSDocGlobals.output_file);
                } else {
                    System.out.println("Grammar documentation generated with 0 errors and " + CSharpCCErrors.get_warning_count() + " warnings.");
                }
                System.exit(0);
            } else {
                System.out.println("Detected " + CSharpCCErrors.get_error_count() + " errors and " + CSharpCCErrors.get_warning_count() + " warnings.");
                System.exit((CSharpCCErrors.get_error_count() == 0) ? 0 : 1);
            }
        } catch (org.deveel.csharpcc.parser.MetaParseException e) {
            System.out.println(e.toString());
            System.out.println("Detected " + CSharpCCErrors.get_error_count() + " errors and " + CSharpCCErrors.get_warning_count() + " warnings.");
            System.exit(1);
        } catch (org.deveel.csharpcc.parser.ParseException e) {
            System.out.println(e.toString());
            System.out.println("Detected " + (CSharpCCErrors.get_error_count() + 1) + " errors and " + CSharpCCErrors.get_warning_count() + " warnings.");
            System.exit(1);
        }
    }

    private static void CSDocInit() {
        Options.CSharpCCInit();
