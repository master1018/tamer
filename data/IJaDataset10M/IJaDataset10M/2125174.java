...
public class Util {
    ...
    public static double getAngleRadiansABC(double[] a, double[] b, double[] c) {
        ...
        return (isNearZero(ab2, 1e-3) || isNearZero(bc2, 1e-3) ? 0 : Math.acos((ab2 + bc2 - ac2) / 2 / Math.sqrt(ab2 * bc2)));
    }

    public static boolean isApprox(Vector3d a, Vector3d b, double precision) {
        return (distance2(a, b) <= precision * precision * Math.min(a.lengthSquared(), b.lengthSquared()));
    }

    static final double max_squarable_double = 1e150;

    static final double min_squarable_double = 1e-150;
    ...
    public static double pointPlaneAngleRadians(Vector3d a, Vector3d b, Vector3d c, Vector3d d, Vector3d v1, Vector3d v2, Vector3d norm) {
        ...
        if (angleNorm > Math.PI / 2) angleNorm = Math.PI - angleNorm;
    ...
    private static double vectorAngleRadians(Vector3d v1, Vector3d v2) {
        double l1 = v1.length();
        double l2 = v2.length();
        return (isNearZero(l1) || isNearZero(l2) ? 0 : Math.acos(v1.dot(v2) / (l1 * l2)));
    }
