...
public class DataCreatorVoucher extends DataCreator {
    ...
    public DataCreatorVoucher(PersistenceManager pm, User user) {
        ...
        rootVoucherType = (VoucherType) pm.getObjectById(ProductTypeID.create(organisationID, VoucherType.class.getName()));
    }
    ...
    public VoucherType createCategory(VoucherType parent, String productTypeID, VoucherLocalAccountantDelegate localAccountantDelegate, IPackagePriceConfig packagePriceConfig, String... names) throws CannotPublishProductTypeException {
        if (parent == null) parent = rootVoucherType;
        try {
            return (VoucherType) pm.getObjectById(ProductTypeID.create(organisationID, productTypeID));
        } catch (JDOObjectNotFoundException x) {
        }
        VoucherType pt = new VoucherType(organisationID, productTypeID, parent, ProductType.INHERITANCE_NATURE_BRANCH, ProductType.PACKAGE_NATURE_OUTER);
        ...
        } else pt.getProductTypeLocal().setLocalAccountantDelegate(parent.getProductTypeLocal().getLocalAccountantDelegate());
        store.setProductTypeStatus_published(user, pt);
        return pt;
    }

    public VoucherType createLeaf(VoucherType category, String productTypeID, IPackagePriceConfig packagePriceConfig, VoucherLocalAccountantDelegate localAccountantDelegate, LegalEntity vendor, String... names) throws CannotPublishProductTypeException, CannotConfirmProductTypeException {
        if (category == null) category = rootVoucherType;
        VoucherType pt = new VoucherType(organisationID, productTypeID, category, ProductType.INHERITANCE_NATURE_LEAF, ProductType.PACKAGE_NATURE_OUTER);
        setNames(pt.getName(), names);
        if (packagePriceConfig != null) {
            pt.getFieldMetaData(ProductType.FieldName.packagePriceConfig).setValueInherited(false);
            pt.setPackagePriceConfig(packagePriceConfig);
        } else pt.setPackagePriceConfig(category.getPackagePriceConfig());
        pt = (VoucherType) store.addProductType(user, pt);
        if (localAccountantDelegate != null) {
            pt.getProductTypeLocal().getFieldMetaData(ProductTypeLocal.FieldName.localAccountantDelegate).setValueInherited(false);
            pt.getProductTypeLocal().setLocalAccountantDelegate(localAccountantDelegate);
        } else pt.getProductTypeLocal().setLocalAccountantDelegate(category.getProductTypeLocal().getLocalAccountantDelegate());
        if (vendor != null) {
            pt.setVendor(vendor);
            pt.setOwner(vendor);
    ...
    public VoucherLayout createVoucherLayout() {
        ...
        if (root == null) {
            ...
            Rectangle pageBounds = PredefinedPageUtil.getPageBounds(root.getModelUnit(), new A4Page());
            root.getCurrentPage().setPageBounds(pageBounds);
            Layer layer = root.getCurrentLayer();
            String fontName = "Bitstream Vera Sans";
            TextScriptDrawComponent scriptText = factory.createTextScriptDrawComponent("Name", fontName, 24, Font.BOLD, 100, 100, layer);
            ...
            TextScriptDrawComponent priceText = factory.createTextScriptDrawComponent("Price", fontName, 18, Font.PLAIN, 100, 500, layer);
            priceText.setScriptRegistryItemID(VoucherScriptingConstants.OID.SCRIPT_REGISTRY_ITEM_ID_SCRIPT_VOUCHER_PRICE);
            layer.addDrawComponent(scriptText);
            BarcodeDrawComponent barcode = factory.createBarcode(BarcodeDrawComponent.Type.TYPE_128, "temp", 1000, 100, BarcodeDrawComponent.WidthScale.SCALE_3, 300, BarcodeDrawComponent.Orientation.HORIZONTAL, true, layer, VoucherScriptingConstants.OID.SCRIPT_REGISTRY_ITEM_ID_SCRIPT_VOUCHER_KEY);
