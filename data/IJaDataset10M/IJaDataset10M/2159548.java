...
public class MonkeyKeyboardTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        rnd = new Random(seed);
        refDate = new DTBuilder(1900 + rnd.nextInt(200), 1, rnd.nextInt(366) + 1).toDate();
    }
    ...
    public void testTimeGoesForward() throws Throwable {
        for (int i = 0; i < 1000; ++i) {
            ...
            task.rrule = rrule;
            boolean needsTime = rrule.getFreq().compareTo(Frequency.DAILY) < 0 || rrule.getByHour().length != 0 || rrule.getByMinute().length != 0 || rrule.getBySecond().length != 0;
            DateValue dtStart = monkeySeeDateValue(needsTime || maybeNot(timed));
            task.dtStart = task.dtStart;
            RecurrenceIterator it;
            try {
                it = RecurrenceIteratorFactory.createRecurrenceIterator(rrule, dtStart, PST);
            } catch (Throwable th) {
                task.cancel();
                continue;
            }
            try {
                ...
                while (it.hasNext() && --tries >= 0) {
                    ...
                    assertTrue("last=" + last + ", current=" + dv, null == last || last.compareTo(dv) < 0);
                    last = dv;
                }
            } catch (Throwable th) {
                task.cancel();
    ...
    public void testNoExceptionsThrownOnWelformedRrules() throws Throwable {
        ...
        for (int i = 0; i < nRuns; ++i) {
            ...
            try {
                RecurrenceIterator it = RecurrenceIteratorFactory.createRecurrenceIterator(rrule, dtStart, PST);
   ...
   */
    RRule monkeySeeRRule(boolean timed, boolean wellFormed) {
        ...
        rrule.setInterval((int) Math.min(0, Math.abs(rnd.nextGaussian() - .5) * 3) + 1);
        ...
        boolean allowSetPos = wellFormed ? largestSetPlural : maybeNot(largestSetPlural);
    ...
    WeekdayNum[] monkeySeeWeekdayNumList(Frequency freq) {
        ...
        for (int i = n; --i >= 0; ) {
            int num = numRange != 0 && rnd.nextInt(10) < 2 ? rnd.nextInt(numRange) : 0;
    ...
    DateValue monkeySeeDateValue(boolean timed) {
        double daysFromNow = (10 * rnd.nextGaussian()) + 10;
        if (timed) {
            return TimeUtils.add(refDate, new DateTimeValueImpl(0, 0, 0, 0, 0, (int) (daysFromNow * 24 * 60 * 60)));
        } else {
...
class Histogram {
    ...
    void dump(int nSlots, String units) {
        ...
        Arrays.sort(samples);
        long min = samples[0], max = samples[samples.length - 1];
        double[] logs = new double[n];
        for (int i = n; --i >= 0; ) {
            logs[i] = Math.log(samples[i] - min + 1d);
        }
        double logmin = logs[0], logmax = logs[logs.length - 1];
        ...
        for (int i = 0; i < nSlots; ++i) {
            System.out.println(String.format("(%08.2f - %08.2f) | %-36s : %d", slotBrackets[i], slotBrackets[i + 1], "**************************************************".substring(0, (countPerSlot[i] * 36) / totalCount), countPerSlot[i]));
        }
        System.out.println("min=" + min + ", max=" + max + ", median=" + samples[samples.length / 2]);
    }
}
...
class DumpStackTask extends TimerTask {
    ...
    @Override
    public void run() {
        System.err.println("\nTime out of " + (rrule != null ? rrule.toIcal() : "<null>") + " / " + dtStart);
