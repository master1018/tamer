...
public class NetworkUtilsTest extends com.limegroup.gnutella.util.BaseTestCase {
     ...
     */
    private static final String[] PRIVATE_ADDRESSES = { "0.0.0.0", "127.1.1.1", "127.0.0.0", "172.16.0.0", "172.31.255.255", "172.17.0.255", "192.168.0.0", "192.168.0.1", "192.168.255.255", "169.254.0.0", "169.254.255.255", "255.1.2.3", "255.0.0.0" };

    /**
     * Array of public addresses for testing.
     */
    private static final String[] PUBLIC_ADDRESSES = { "2.32.0.1", "20.43.2.1", "1.32.0.1", "172.15.0.1", "172.32.0.1", "192.167.0.1", "192.169.0.1", "1.0.0.0", "180.32.0.1", "239.32.0.1", "128.0.0.1", "169.253.0.0", "169.255.0.0" };
    ...
    public void testIsCloseIP() throws Exception {
        ...
        addr0[1] = (byte) 2;
        addr1[1] = (byte) 2;
        assertTrue("should be considered close", NetworkUtils.isCloseIP(addr0, addr1));
        addr0[1] = (byte) 4;
        assertTrue("should be considered close", NetworkUtils.isCloseIP(addr0, addr1));
        addr0[0] = (byte) 0;
        assertTrue("should not be considered close", !NetworkUtils.isCloseIP(addr0, addr1));
    }
    ...
    public void testIsPrivateAddressWithString() throws Exception {
        ...
        for (int i = 0; i < PUBLIC_ADDRESSES.length; i++) {
            ...
            assertTrue("should not be a private address: " + address, !NetworkUtils.isPrivateAddress(address));
        }
        for (int i = 0; i < PRIVATE_ADDRESSES.length; i++) {
            String address = PRIVATE_ADDRESSES[i];
            assertTrue("should be a private address: " + address, NetworkUtils.isPrivateAddress(address));
        }
    }
    ...
    public void testIsPrivateAddress() throws Exception {
        ...
        for (int i = 0; i < PUBLIC_ADDRESSES.length; i++) {
            address = InetAddress.getByName(PUBLIC_ADDRESSES[i]).getAddress();
            assertTrue("should not be a private address" + address, !NetworkUtils.isPrivateAddress(address));
        }
        for (int i = 0; i < PRIVATE_ADDRESSES.length; i++) {
            address = InetAddress.getByName(PRIVATE_ADDRESSES[i]).getAddress();
            assertTrue("should be a private address" + address, NetworkUtils.isPrivateAddress(address));
        }
    }
    ...
    public void testIsMe() throws Exception {
        assertTrue("localhost should be me", NetworkUtils.isMe("localhost", 6346));
        NetworkUtils.isMe("127.1.2.1", 6346);
        assertTrue("localhost bytes should be me", NetworkUtils.isMe(new byte[] { (byte) 127, 0, 0, 0 }, 6346));
    }
}
