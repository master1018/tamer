...
public class RadialTreeLayoutAlgorithm implements LayoutAlgorithm {
	 ...
	 * @param applyToAll
	 *            only the selected nodes are involved unless this is true
	 * @param configuration
	 *            allows for x/y radius and centre coordinates, or width/height
	 *            to be specified
	 */
    public void perform(final JGraph jgraph, final boolean applyToAll, final Properties configuration) {
        if (null == configuration) {
            throw new LayoutException("configuration must not be null");
        }
        this.jgraph = jgraph;
        ...
        double depth = tree.getDepth() + 0.5;
        if (configuration.containsKey(KEY_WIDTH)) {
            double WIDTH = Double.parseDouble(configuration.getProperty(KEY_WIDTH));
            ROOTX = WIDTH / 2.0;
            RADIUSX = ROOTX / depth;
        } else {
            if (configuration.containsKey(KEY_CENTRE_X)) {
                ...
                throw new LayoutException("Must specify one of KEY_WIDTH or KEY_CENTRE_X");
            }
            if (configuration.containsKey(KEY_RADIUS_X)) {
                RADIUSX = Double.parseDouble(configuration.getProperty(KEY_RADIUS_X));
            } else {
                throw new LayoutException("Must specify one of KEY_WIDTH or KEY_RADIUS_X");
            }
        }
        if (configuration.containsKey(KEY_HEIGHT)) {
            double HEIGHT = Double.parseDouble(configuration.getProperty(KEY_HEIGHT));
            ROOTY = HEIGHT / 2.0;
            RADIUSY = ROOTY / depth;
        } else {
            if (configuration.containsKey(KEY_CENTRE_Y)) {
                ...
                throw new LayoutException("Must specify one of KEY_HEIGHT or KEY_CENTRE_Y");
            }
