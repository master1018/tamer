...
public class AutoTitles extends StdLibrary implements AutoTitlesLibrary {
    ...
    public boolean isExistingAutoTitle(String title) {
        if (autoTitles == null) reloadAutoTitles();
        for (int v = 0; v < autoTitles.size(); v++) if (((String) autoTitles.elementAt(v, 1)).toUpperCase().equalsIgnoreCase(title.trim())) return true;
        return false;
    }
        ...
        synchronized (ptV) {
            for (int t = 0; t < autoTitles.size(); t++) {
                ...
                if (CMLib.masking().maskCheck(mask, mob, true)) {
                    if (pdex < 0) {
                        if (ptV.size() > 0) ptV.insertElementAt(title, 0); else ptV.addElement(title);
                        somethingDone = true;
                    }
                } else if (pdex >= 0) {
                    somethingDone = true;
        ...
        for (String playerName : list) {
            ...
            if (M.playerStats() != null) {
                ...
                synchronized (ptV) {
                    ...
                    if (pdex >= 0) {
                        ptV.removeElementAt(pdex);
                        if (!CMLib.flags().isInTheGame(M, true)) CMLib.database().DBUpdatePlayerPlayerStats(M);
                    }
                }
            }
        }
    ...
    public void reloadAutoTitles() {
        ...
        for (int v = 0; v < V.size(); v++) {
            ...
            if (WKID.startsWith("Error: ")) Log.errOut("CharCreation", WKID);
        }
        for (Enumeration e = CMLib.players().players(); e.hasMoreElements(); ) {
            MOB M = (MOB) e.nextElement();
            if (M.playerStats() != null) {
                if ((evaluateAutoTitles(M)) && (!CMLib.flags().isInTheGame(M, true))) CMLib.database().DBUpdatePlayerPlayerStats(M);
            }
        }
    }
}
