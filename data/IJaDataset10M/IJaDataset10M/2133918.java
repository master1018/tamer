...
public class Animation implements Runnable {
    ...
    private Point getResultingPoint(String name, Point currentPoint, int orientation) {
        if (name.equals("run_towards")) {
            if (orientation == 1) {
                return new Point(currentPoint.x + 120, currentPoint.y);
            } else return new Point(currentPoint.x - 120, currentPoint.y);
        } else if (name.equals("walk_towards")) {
            if (orientation == 1) {
                return new Point(currentPoint.x + 60, currentPoint.y);
            } else return new Point(currentPoint.x - 60, currentPoint.y);
