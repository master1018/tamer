...
public class SchedulerView extends ViewPart implements SchedulerListener {
    ...
    public void createPartControl(Composite parent) {
        ...
        up.setToolTipText("Moves the selected item up in the queue if possible. Only" + " queued items can be moved without rescheduling.");
        up.setEnabled(false);
        up.setImage(upImage);
        up.addSelectionListener(new UpButtonListener());
        down = new Button(upAndDown, SWT.PUSH);
        down.setToolTipText("Moves the selected item down in the queue if possible. Only" + " queued items can be moved without rescheduling.");
    ...
    public void algorithmError(final Algorithm algorithm, Throwable error) {
        guiRun(new Runnable() {
            ...
            public void run() {
                ...
                if (schedulerTableItem != null) schedulerTableItem.errorTableEntry(table);
            }
        });
        refresh();
    }
    ...
    public void algorithmRescheduled(Algorithm algorithm, Calendar time) {
        SchedulerTableItem schedulerTableItem = (SchedulerTableItem) algorithmToGuiItemMap.get(algorithm);
        if (schedulerTableItem != null) schedulerTableItem.reschedule(time);
        refresh();
    }
    ...
    private void refresh() {
        for (Iterator i = tableItemToAlgorithmMap.keySet().iterator(); i.hasNext(); ) {
