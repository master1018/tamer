...
public abstract class SearchBaseController extends BaseController {
    ...
    protected ArrayList<HashMap<String, String>> doSearch(HttpServletRequest request, ModelAndView modelAndView, String indexName, String... searchStrings) {
        ...
        if (searchStrings != null && searchStrings.length > 0) {
            ...
            if (!searchStringsEmpty) {
                if (isNumeric(latitude) && isNumeric(longitude) && isNumeric(distance)) {
                    ...
                    results = searcherWebService.searchMultiSpacialAll(indexName, searchStrings, Boolean.TRUE, firstResult, maxResults, Integer.parseInt(distance), Double.parseDouble(latitude), Double.parseDouble(longitude));
                } else {
