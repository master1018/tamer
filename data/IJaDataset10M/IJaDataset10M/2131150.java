...
public class DoubleRangeModel extends BoundedRangeModel<Double> implements RandomGenerator<Double> {
    ...
    public DoubleRangeModel() {
        this(0.0, -1.0, 1.0, 0.1);
    }

    /** Initializes to a given value only. */
    public DoubleRangeModel(double d) {
        this(d, -Double.MAX_VALUE, Double.MAX_VALUE, Double.MIN_VALUE);
    }
    ...
    public Double getRValue() {
        return RandomGenerator.Uniform.getValue(getMinimum(), getMaximum());
    }
    ...
    public Vector<Double> getValueRange(boolean inclusive, Double shift) {
        ...
        while (result.lastElement() <= (maximum - step - shift * step)) {
    ...
    public int getNumSteps() {
        return (int) Math.round((maximum - minimum) / step);
    }

    @Override
    public FiresChangeEvents clone() {
        return new DoubleRangeModel(value, minimum, maximum, step);
    }
