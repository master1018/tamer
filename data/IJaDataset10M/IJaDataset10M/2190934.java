...
class NetMaskImpl extends PrincipalImpl implements Group, Serializable {
    ...
    private byte[] extractSubNet(byte[] b) {
        ...
        int nbbits = prefix % 8;
        int subnetSize = 0;
        if (nbbits == 0) subnetSize = partialyCoveredIndex; else subnetSize = partialyCoveredIndex + 1;
        if (isDebugOn()) {
        ...
        if (p instanceof PrincipalImpl || p instanceof NetMaskImpl) {
            ...
            for (int i = 0; i < subnet.length; i++) {
                if (isDebugOn()) {
                    ...
                    debug("equals", "(recAddr[i] & subnet[i]) :" + ((int) (recAddr[i] & (int) subnet[i]) & 0xFF) + "subnet[i] :" + (int) (subnet[i] & 0xFF));
                }
   ...
   */
    public boolean isMember(Principal p) {
        if ((p.hashCode() & super.hashCode()) == p.hashCode()) return true; else return false;
    }
