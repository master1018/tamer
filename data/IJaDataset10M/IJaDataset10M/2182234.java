...
public class DebugThreads {
    ...
    public int enumerate(DebugThread[] curThreads) {
        ...
        for (int i = threads.length - 1; i >= 0; i--) if (threads[i] != null) {
            DebugThread start = threads[i];
            DebugThread th = threads[i];
            do {
                if (cnt < curThreads.length) curThreads[cnt++] = th;
                th = th.nextThread;
            } while (th != start);
        }
        return cnt;
     ...
     * @param thread
     */
    protected static final native void suspendThread(Object thread);
     ...
     * @param thread
     */
    protected static final native void resumeThread(Object thread);
}
