...
public final class FastStringBuffer {
    ...
    public void getChars(int srcBegin, int srcEnd, char dst[], int dstBegin) {
        if (srcBegin < 0 || srcBegin > srcEnd || srcEnd > used) throw new StringIndexOutOfBoundsException();
    ...
    public FastStringBuffer append(char[] chars) {
        if (used + chars.length > buffer.length) ensureCapacity(used + chars.length);
    ...
    public FastStringBuffer append(char[] chars, int offset, int len) {
        if (offset < 0 || len < 0 || offset + len > chars.length) throw new StringIndexOutOfBoundsException();
        if (used + len > buffer.length) ensureCapacity(used + len);
    ...
    public FastStringBuffer append(char c) {
        if (used + 1 > buffer.length) ensureCapacity(used + 1);
        buffer[used++] = c;
        return this;
    }
