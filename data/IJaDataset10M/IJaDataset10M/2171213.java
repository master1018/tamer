...
public class BehaviorActuator extends Actuator {
    ...
    public BehaviorActuator(Simulator simulator, int id, Robot r, Arguments args) {
        super(simulator, id);
        this.robot = r;
        if (args.getArgumentIsDefined("lock")) if (args.getArgumentAsInt("lock") == 0) lock = false;
    ...
    private void createBehaviors(Arguments args) {
        for (int i = 0; i < args.getNumberOfArguments(); i++) {
            ...
            if (argumentName.equals("turnleft")) addBehavior(new TurnLeftBehavior(simulator, robot, lock));
            if (argumentName.equals("turnright")) addBehavior(new TurnRightBehavior(simulator, robot, lock));
            if (argumentName.equals("moveforward")) addBehavior(new MoveForwardBehavior(simulator, robot, lock));
            if (argumentName.equals("manualwheels")) addBehavior(new ManualWheelsBehavior(simulator, robot, lock));
            if (argumentName.equals("destructive")) addBehavior(new DestructiveBehavior(simulator, robot, lock));
            if (argumentName.equals("layered")) addBehavior(new LayeredBehavior(simulator, robot, lock, (TMazeEnvironment) simulator.getEnvironment()));
            if (argumentName.equals("dummy")) for (int j = 0; j < 4; j++) addBehavior(new DummyBehavior(simulator, robot, lock, (TMazeEnvironment) simulator.getEnvironment(), j));
        }
    }
    ...
    public void apply(Robot robot) {
        if (count == getNumberOfOutputs()) {
            ...
            for (Behavior b : behaviors) if (b.isLocked()) activeBehavior = b;
            Behavior newBehavior;
            if (activeBehavior != null) newBehavior = activeBehavior; else newBehavior = findMaxBehavior();
    ...
    private Behavior findMaxBehavior() {
        int maxIndex = 0;
        for (int i = 1; i < values.length; i++) if (values[maxIndex] < values[i]) maxIndex = i;
    ...
    public int getActiveBehaviorIndex() {
        for (int i = 0; i < behaviors.size(); i++) if (behaviors.get(i).equals(currentlyChosenBehavior)) return i;
        return 999;
    }
