...
/**
 * The <code>DynamicGridLayout</code> class is a layout manager that
 ...
 * default each row and column will be sized in proportion to the minimum size of
 * the largest component in that row or column.  Alternatively, individual rows or
 * columns can be set to a fixed percentage of the container or their minimum size with
 * various options.  Components can also be set to fill the assigned area or use their
 * minimum size and align themselves within that allocated space.  If the grid itself
 * doesn't completely fill the area assigned by the container there are methods to set
 * this alignment too.<p>
 * Here's a simple example of using a DynamicGridLayout.  More concrete examples can be
 * found in DGLSizeTest, DGLAlignmentTest, and DGLRealTest.
 * <p><blockquote><pre>
 ...
 * </pre></blockquote>
 * <p>
 * @author     <A HREF="mailto:nielsen@ccis.adisys.com.au">Robert Nielsen</A>
 * @version    1.00 28 October 1998
 */
public class DynamicGridLayout implements LayoutManager, java.io.Serializable {
   ...
   * <li>DYNAMIC - After the percentages and minumum components are removed from the grid
   ...
   * go smaller than it's minumum size.
   * </ul>
   */
   ...
   */
    private void getWidthsAndHeights(Container parent, boolean preferred) {
        ...
        for (int i = 0; i < ncomponents && i < rows * cols; i++) {
            ...
            Dimension d;
            if (preferred) d = comp.getPreferredSize(); else d = comp.getMinimumSize();
   ...
   */
    private int spaceOut(int len, int[] perc, int[] value) {
        ...
        int size = perc.length;
        int space_left = len;
        boolean check_above_minimum = false;
        ...
        for (int i = 0; i < size; i++) {
            if (perc[i] > 0) {
                ...
                if (perc[i] == BELOW_MARKER) perc[i] = BELOW_MINIMUM;
                if (perc[i] != MINIMUM) check_below_minimum = true;
                space_left -= value[i];
            }
        }
        if (check_below_minimum) for (int i = 0; i < size; i++) if ((perc[i] == BELOW_MINIMUM) && (space_left + value[i]) / (sum + value[i]) < 1) {
            perc[i] = BELOW_MARKER;
            sum += value[i];
            space_left += value[i];
            dyn_cnt++;
        }
        if (check_above_minimum) for (int i = 0; i < size; i++) {
        ...
        if (dyn_cnt > 0) {
            ...
            int lastdyn = -1;
            for (int i = 0; i < size; i++) if (perc[i] == DYNAMIC || perc[i] == BELOW_MARKER || perc[i] == ABOVE_MINIMUM) {
                if (sum == 0) value[i] = 0; else value[i] = Math.round(value[i] * space_left / sum);
                leftover -= value[i];
                lastdyn = i;
            }
            value[lastdyn] += leftover;
            return 0;
        }
        if (space_left < 0) return 0; else return space_left;
    }

    private void placeComponent(Component c, int x, int y, int w, int h, int ra, int ca) {
        Dimension pref = c.getPreferredSize();
        if (ra != FILL && pref.height < h) {
            if (ra == CENTER) y += Math.round((h - pref.height) / 2.0); else if (ra == BOTTOM) y += (h - pref.height);
            h = pref.height;
        }
        if (ca != FILL && pref.width < w) {
            if (ca == CENTER) x += Math.round((w - pref.width) / 2.0); else if (ca == RIGHT) x += (w - pref.width);
            w = pref.width;
        }
        c.setBounds(x, y, w, h);
    }
   ...
   */
    public Dimension preferredLayoutSize(Container parent) {
        ...
        return new Dimension(insets.left + insets.right + w + (cols - 1) * hgap, insets.top + insets.bottom + h + (rows - 1) * vgap);
    }
    ...
    public void layoutContainer(Container parent) {
        ...
        for (int c = 0, x = insets.left + col_indent; c < cols; x += col_width[c++] + hgap) {
            for (int r = 0, y = insets.top + row_indent; r < rows; y += row_height[r++] + vgap) {
                int i = r * cols + c;
                if (i < ncomponents) placeComponent(parent.getComponent(i), x, y, col_width[c], row_height[r], row_align[r], col_align[c]);
            }
        }
    }
