...
public class JListLocation extends ComponentLocation {
    ...
    public int getIndex(JList list) {
        if (value != null) return valueToIndex(list, value);
        if (row != -1) {
            return row;
        }
    ...
    public Point getPoint(Component c) {
        JList list = (JList) c;
        if (value != null || row != -1) return indexToPoint(list, getIndex(list));
    ...
    public boolean equals(Object o) {
        if (o instanceof JListLocation) {
            ...
            if (value != null) return value.equals(loc.value);
            if (row != -1) return row == loc.row;
        }
        return super.equals(o);
    }

    public String toString() {
        if (value != null) return encodeValue(value);
        if (row != -1) return encodeIndex(row);
