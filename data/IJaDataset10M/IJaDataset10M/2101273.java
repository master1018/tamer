...
public class PreferenceObject {
    ...
    public static boolean savePreference(Preference preference) {
        ...
        if (getPreference(preference.getId()) == null) {
            sqlstmt = "insert into tpreference (pr_id, pr_value) values (" + Db.getString(preference.getId()) + ", " + Db.getString(preference.getValue()) + ")";
        } else {
            sqlstmt = "update tpreference set " + "pr_value = " + Db.getString(preference.getValue()) + " where pr_id = " + Db.getString(preference.getId());
        }
    ...
    public static Preference getPreference(String id) {
        G.setHourglass();
        String sqlstmt = "select pr_value" + " from tpreference" + " where" + " pr_id = " + Db.getString(id);
