...
public class CompoundTextCommand extends Command implements TextCommand, AppendableCommand {
    ...
    public void add(TextCommand command) {
        if (command != null) pending.add(command);
    }
    ...
    public void dispose() {
        for (int i = 0; i < applied.size(); i++) ((Command) applied.get(i)).dispose();
        flushPending();
    }
    ...
    public void redo() {
        for (int i = 0; i < applied.size(); i++) ((Command) applied.get(i)).redo();
    }

    public void undo() {
        for (int i = applied.size() - 1; i >= 0; i--) ((Command) applied.get(i)).undo();
    }
}
