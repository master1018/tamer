...
public class PedParser {
    ...
    ScoreData scoreNodeRemainder(Individual seed, HashSet kids, HashSet parents, Hashtable scoresByEdges, Hashtable edgesByInds, Individual edgeInd) {
        ...
        if (edgesByInds.containsKey(seed)) {
            ...
            scores.put(seed, new ScoreData(new NodeScore(new Vector(), new HashSet()), new NodeScore(new Vector(), tmp)));
        }
        ...
        while (kitr.hasNext()) {
            ...
            if (kids.contains(kid)) {
                if (edgesByInds.containsKey(kid)) {
                    ...
                    scores.put(kid, new ScoreData(new NodeScore(new Vector(), new HashSet()), new NodeScore(new Vector(), tmp)));
                }
            }
        }
        if (lookInNode) {
            ...
            while (sitr.hasNext()) {
                ...
                if (parents.contains(currentSpouse)) {
                    ...
                    while (kitr.hasNext()) {
                        ...
                        if ((curKid.mom == currentSpouse || curKid.dad == currentSpouse) && !curKid.missing && !curKid.mom.missing && !curKid.dad.missing) {
                            Trio curTrio = new Trio(curKid, curKid.mom, curKid.dad);
                            boolean addTrio = false;
                            if (spScore.getScoreDifference() < TRIO_DIFFERENTIAL && seScore.getScoreDifference() < TRIO_DIFFERENTIAL) {
                                addTrio = true;
                            ...
                            if (addTrio) {
                                if (curBestTrio != null) {
                                    ...
                                    if (nssScore.getScoreDifference() > spScore.getScoreDifference()) {
                                        curBestTrio = curTrio;
                                    } else if (nssScore.getScoreDifference() == spScore.getScoreDifference() && curTrio.scaledGenoSum > curBestTrio.scaledGenoSum) {
                                        curBestTrio = curTrio;
                                    }
                                } else {
                                    curBestTrio = curTrio;
                    ...
                    while (kitr.hasNext()) {
                        ...
                        if (curKid.dad == seed || curKid.mom == seed) {
                            ...
                            if (curSpouseScore.getScoreDifference() == 0) {
                                ...
                                if ((ckScore.getScoreDifference() > curSpouseScore.getScoreDifference() + seedScore.getScoreDifference()) || (ckScore.getScoreDifference() == curSpouseScore.getScoreDifference() + seedScore.getScoreDifference() && ckScore.getGenoSumDifference() > curSpouseScore.getGenoSumDifference())) {
                                    if (ckScore.getScoreDifference() > curBestSD.getScoreDifference()) {
                                        ...
                                        if (ckScore.getGenoSumDifference() - curSpouseScore.getGenoSumDifference() > curBestGSDiff) {
                                            curBestKid = curKid;
                                            curBestSD = ckScore;
                                            curBestGSDiff = ckScore.getGenoSumDifference() - curSpouseScore.getGenoSumDifference();
                                        }
            ...
            if (curBestSingleton != null && curBestTrio != null && ((ScoreData) scores.get(curBestSingleton)).getScoreDifference() > 1) {
