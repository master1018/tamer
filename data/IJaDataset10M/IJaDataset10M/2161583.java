...
public class SettingParser extends AbstractBeneratorDescriptorParser {
    ...
    public SettingParser() {
        super(DescriptorConstants.EL_SETTING, CollectionUtil.toSet(ATT_NAME), CollectionUtil.toSet(ATT_DEFAULT, ATT_VALUE, ATT_REF, ATT_SOURCE));
    }
    ...
    public Statement doParse(Element element, Statement[] parentPath, BeneratorParseContext context) {
        ...
        if (element.hasAttribute(ATT_DEFAULT)) return parseDefault(propertyName, element.getAttribute(ATT_DEFAULT)); else {
    ...
    public static Expression<?> parseValue(Element element) {
        if (element.hasAttribute(ATT_VALUE)) return DescriptorParserUtil.parseScriptableStringAttribute(ATT_VALUE, element); else if (element.hasAttribute(ATT_REF)) return new ContextReference(element.getAttribute(ATT_REF)); else if (element.hasAttribute(ATT_SOURCE)) return parseSource(element.getAttribute(ATT_SOURCE)); else {
            Element[] childElements = XMLUtil.getChildElements(element);
            Expression[] subExpressions = new Expression[childElements.length];
            for (int j = 0; j < childElements.length; j++) subExpressions[j] = BeanParser.parseBeanExpression(childElements[j]);
    ...
    public static class SourceExpression<E> extends DynamicExpression<E> {
        ...
        public E evaluate(Context context) {
            ...
            if (wrapper == null) throw new ConfigurationError("Generator not available: " + generator);
