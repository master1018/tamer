...
public class ButtonBarBuilder2 extends AbstractButtonPanelBuilder {
    ...
    public void addButton(JComponent[] buttons) {
        if (buttons == null) throw new NullPointerException("The button array must not be null.");
        int length = buttons.length;
        if (length == 0) throw new IllegalArgumentException("The button array must not be empty.");
        for (int i = 0; i < length; i++) {
            int index = leftToRight ? i : length - 1 - i;
            addButton(buttons[index]);
            if (i < buttons.length - 1) addRelatedGap();
        }
    }
    ...
    public void addButton(Action action) {
        if (action == null) throw new NullPointerException("The button Action must not be null.");
    ...
    public void addButton(Action[] actions) {
        if (actions == null) throw new NullPointerException("The Action array must not be null.");
        int length = actions.length;
        if (length == 0) throw new IllegalArgumentException("The Action array must not be empty.");
