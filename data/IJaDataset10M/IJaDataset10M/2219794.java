...
public class Util {
   ...
   */
    public static byte[] toBytesFromString(String s) {
        ...
        int i = 0, j = 0;
        if ((limit % 2) == 1) result[j++] = (byte) fromDigit(s.charAt(i++));
        while (i < limit) {
   ...
   */
    public static byte[] toReversedBytesFromString(String s) {
        ...
        byte[] result = new byte[((limit + 1) / 2)];
        int i = 0;
        if ((limit % 2) == 1) result[i++] = (byte) fromDigit(s.charAt(--limit));
        while (limit > 0) {
   ...
   */
    public static int fromDigit(char c) {
        if (c >= '0' && c <= '9') return c - '0'; else if (c >= 'A' && c <= 'F') return c - 'A' + 10; else if (c >= 'a' && c <= 'f') return c - 'a' + 10; else throw new IllegalArgumentException("Invalid hexadecimal digit: " + c);
    }
   ...
   */
    public static String dumpString(byte[] data, int offset, int length, String m) {
        ...
        if (length > 32) sb.append(m).append("Hexadecimal dump of ").append(length).append(" bytes...\n");
        ...
        for (; offset < end; offset += 32) {
            ...
            int i = 0;
            for (; i < 32 && offset + i + 7 < end; i += 8) sb.append(toString(data, offset + i, 8)).append(' ');
            if (i < 32) for (; i < 32 && offset + i < end; i++) sb.append(byteToString(data[offset + i]));
            sb.append('\n');
        }
        return sb.toString();
    }
   ...
   */
    public static final String toBase64(byte[] buffer) {
        ...
        if (notleading) return sb.toString();
        return "0";
    }
