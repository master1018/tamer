...
class LBezierCurveView extends LModifiablePointsShapeView<IBezierCurve> implements IViewBezierCurve {
    ...
    public void paintShowPointsDots(final Graphics2D g) {
        ...
        if (!arrow1Drawable || isClosed) fillCircle(d, pts.get(0), width, g);
        if (!arrow2Drawable || isClosed) fillCircle(d, pts.get(size - 1), width, g);
        for (i = 1; i < size - 1; i++) {
            fillCircle(d, pts.get(i), width, g);
            fillCircle(d, ctrlPts2.get(i), width, g);
        }
        for (i = 0; i < size; i++) fillCircle(d, ctrlPts1.get(i), width, g);
        if (shape.isClosed()) {
    ...
    public void paintShowPointsLines(final Graphics2D g) {
        ...
        int i;
        g.setStroke(new BasicStroke(thick / 2f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.f, new float[] { (float) shape.getDashSepBlack(), (float) shape.getDashSepWhite() }, 0));
        g.setColor(shape.getLineColour());
        for (i = 3; i < size; i += 2) {
    ...
    protected void setPath(final boolean close) {
        ...
        path.curveTo(ctrlPts1.get(0).getX(), ctrlPts1.get(0).getY(), ctrlPts1.get(1).getX(), ctrlPts1.get(1).getY(), pts.get(1).getX(), pts.get(1).getY());
        for (int i = 2, size = pts.size(); i < size; i++) {
            ctrl1 = ctrlPts2.get(i - 1);
            path.curveTo(ctrl1.getX(), ctrl1.getY(), ctrlPts1.get(i).getX(), ctrlPts1.get(i).getY(), pts.get(i).getX(), pts.get(i).getY());
        }
        if (shape.isClosed()) {
