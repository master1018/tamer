...
public class BeanUtils {
    ...
    public static String dotsToPathSeparatots(String str) {
        ...
        while (st.hasMoreTokens()) {
            if (first) first = false; else sb.append(File.pathSeparator);
    ...
    public static File classNameToFile(File parentDir, String cname, String suffix) {
        ...
        if (null != suffix) fname = fname + suffix;
        File f;
        if (null == parentDir) f = new File(fname); else f = new File(parentDir, fname);
        return f;
    }
	 ...
	 * properties.
	 */
    public static void populateBean(Object bean, Map data) throws LTSException {
        String className = null;
        if (null != bean) className = bean.getClass().getName();
        ...
        try {
            ...
            if (null == propertyName) {
                msg = "Error trying to get the properies for bean of class " + className;
            } else {
                msg = "Error trying to write bean property " + propertyName + ", of class " + className + ", to value " + value;
            }
            throw new LTSException(msg, e);
        }
    }

    public static void populateBean(Object bean, Object[] data) throws LTSException {
    ...
    public static Object toPrimitiveValue(Class clazz, String s) {
        Object value = null;
        if (Boolean.TYPE == clazz) value = new Boolean(s); else if (Character.TYPE == clazz) {
            char c = s.charAt(0);
            value = new Character(c);
        } else if (Byte.TYPE == clazz) value = new Byte(s); else if (Short.TYPE == clazz) value = new Short(s); else if (Integer.TYPE == clazz) value = new Integer(s); else if (Long.TYPE == clazz) value = new Long(s); else if (Float.TYPE == clazz) value = new Float(s); else if (Double.TYPE == clazz) value = new Double(s); else if (String.class == clazz) value = s; else {
	 ...
	 */
    public static void setPrimitiveProperty(Object bean, String strValue, PropertyDescriptor prop) {
        Object value;
        if (null == strValue) value = null; else value = toPrimitiveValue(prop.getPropertyType(), strValue);
        try {
    ...
    public static String toStringValue(Object bean, PropertyDescriptor prop) {
        try {
            ...
            if (null == o) return null; else return o.toString();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static Constructor getDefaultConstructor(Class clazz) {
        ...
        for (Constructor cons : constructors) {
            ...
            if (null == formalParams || formalParams.length < 1) return cons;
        }
        return null;
    }
    ...
    public static Class[] SimpleClasses = { Boolean.class, Byte.class, Character.class, Double.class, Float.class, Integer.class, Long.class, Short.class, String.class, Boolean.TYPE, Byte.TYPE, Character.TYPE, Double.TYPE, Float.TYPE, Integer.TYPE, Long.TYPE, Short.TYPE };

    public static boolean isSimpleType(Class clazz) {
        return clazz.isPrimitive() || clazz == Byte.class || clazz == Character.class || clazz == Double.class || clazz == Short.class || clazz == Integer.class || clazz == Long.class || clazz == Float.class || clazz == String.class;
    }

    public static Object toSimpleValue(Class clazz, String s) {
        Object value = null;
        if (clazz == Byte.class || clazz == Byte.TYPE) value = new Byte(s); else if (clazz == Short.class || clazz == Short.TYPE) value = new Short(s); else if (clazz == Integer.class || clazz == Integer.TYPE) value = new Integer(s); else if (clazz == Long.class || clazz == Long.TYPE) value = new Long(s); else if (clazz == Float.class || clazz == Float.TYPE) value = new Float(s); else if (clazz == Double.class || clazz == Double.TYPE) value = new Double(s); else if (clazz == Character.class || clazz == Character.TYPE) value = new Character(s.charAt(0)); else value = s;
        return value;
    }
    ...
    public static String toAbsoluteName(Class beanClass, PropertyDescriptor property) {
        return beanClass.getName() + "." + property.getName();
    }
    ...
    public static String getSimpleName(String name) {
        ...
        if (-1 != index) name = name.substring(1 + index);
        return name;
    }
    ...
    public static Object createInstance(Class clazz) throws LTSException {
        try {
            ...
            String msg = "Caught exception while trying to instantiate " + clazz.getName();
