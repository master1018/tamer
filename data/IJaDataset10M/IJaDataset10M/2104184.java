...
public class DrawRepl implements MouseMotionListener, WindowListener, ActionListener, Runnable {
    ...
    AWTEvent copyEvent(Component src, AWTEvent evt) {
        ...
        if (evt instanceof ActionEvent) return new ActionEvent(src, evt.getID(), ((ActionEvent) evt).getActionCommand(), ((ActionEvent) evt).getModifiers());
        if (evt instanceof PaintEvent) return new PaintEvent(src, evt.getID(), ((PaintEvent) evt).getUpdateRect());
        if (evt instanceof FocusEvent) return new FocusEvent(src, evt.getID(), ((FocusEvent) evt).isTemporary());
        if (evt instanceof ComponentEvent) return new ComponentEvent(src, evt.getID());
        return null;
    }

    void dispatch(Object src, AWTEvent evt) {
        if (src instanceof Component) ((Component) src).dispatchEvent(evt); else if (src instanceof MenuComponent) ((MenuComponent) src).dispatchEvent(evt); else System.err.println("++++++++++");
    }

    public Component findComponent(Container parent, String comp_name) {
        Component retval = null;
        if (comp_name != null && comp_name.equals(parent.getName())) return parent;
        ...
        for (int i = ncomponents - 1; i >= 0; i--) {
            ...
            if (comp != null) {
                ...
                } else if (comp_name.equals(comp.getName())) return comp;
            }
        }
        return retval;
    }
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                if (obj instanceof Component) comp_name = ((Component) obj).getName(); else if (obj instanceof MenuComponent) comp_name = ((MenuComponent) obj).getName(); else {
                ...
                MethodCall call = new MethodCall("processEvent", new Object[] { comp_name, evt }, new String[] { String.class.getName(), AWTEvent.class.getName() });
    ...
    public void go() {
        ...
        if (current_color == null) current_color = Color.red;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if ("Clear".equals(command)) clearPanel(); else if ("Exit".equals(command)) {
