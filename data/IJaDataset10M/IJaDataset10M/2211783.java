...
public class JavaMode extends AbstractMode implements Constants, Mode {
    ...
    private static final String[] javaConditionals = { "if", "else", "do", "while", "for", "switch", "try", "catch", "finally", "synchronized" };
    ...
    public static Mode getMode() {
        if (mode == null) mode = new JavaMode();
        return mode;
    }
    ...
    protected void setKeyMapDefaults(KeyMap km) {
        ...
        km.mapKey(0xffc9, 0, "wrapComment");
    ...
    public void populateModeMenu(Editor editor, Menu menu) {
        ...
        if (jdb != null) jdbMenuItem.setEnabled(false); else {
            try {
    ...
    public NavigationComponent getSidebarComponent(Editor editor) {
        if (getId() == JAVA_MODE) {
            ...
            if (view.getSidebarComponent() == null) view.setSidebarComponent(new JavaTree(editor));
    ...
    public int getCorrectIndentation(final Line line, final Buffer buffer) {
        if (line.flags() == STATE_COMMENT) return indentComment(line, buffer);
        final String text = line.trim();
        final char textFirstChar = text.length() > 0 ? text.charAt(0) : 0;
        if (textFirstChar == '}') return indentClosingBrace(line, buffer);
        if (textFirstChar == 'c' || textFirstChar == 'd') {
            final String firstIdentifier = getFirstIdentifier(text);
            if (firstIdentifier.equals("case") || firstIdentifier.equals("default")) return indentSwitchLabel(line, buffer);
            ...
            if (firstIdentifier.equals("else") || firstIdentifier.equals("elseif")) {
                ...
                if (match != null) return buffer.getIndentation(match.getLine());
            }
        } else if (textFirstChar == '"') {
            if (line.charAt(0) == '"') return 0;
        }
        Position paren = findEnclosingParen(new Position(line, 0));
        if (paren != null) return indentInParen(paren, buffer);
        ...
        if (modelLastChar == '{') return indentAfterOpeningBrace(model, modelText, buffer);
        if (modelLastChar == ')') return indentAfterCloseParen(model, text, textFirstChar, buffer);
        final String lastIdentifier = getLastIdentifier(modelText);
        if (lastIdentifier != null && lastIdentifier.equals("else")) return indentAfterElse(model, text, textFirstChar, buffer);
        ...
        if (modelFirstChar == 'c' || modelFirstChar == 'd') {
            ...
            if (modelFirstIdentifier.equals("case") || modelFirstIdentifier.equals("default")) return indentAfterSwitchLabel(model, text, textFirstChar, buffer);
        }
        final int indent = getIndentationOfEnclosingScope(line, buffer);
        if (textFirstChar == '{') {
            if (buffer.getBooleanProperty(Property.INDENT_BEFORE_BRACE)) {
                if (!isOpeningBraceOfClassOrMethod(line)) return indent + indentSize;
            }
            return indent;
        }
        if (textFirstChar == '=') return indent + indentSize;
        if (modelLastChar == ',') {
            if (buffer.getModeId() == CPP_MODE && modelFirstChar == ':') {
                return indent + 2;
            }
            if (isInArrayInitializer(line)) return indent;
            return indent + indentSize;
        }
        if (isContinued(modelText, modelLastChar)) return indent + indentSize;
        return indent;
    }
    ...
    private final int indentComment(Line line, Buffer buffer) {
        ...
        if (model.trim().startsWith("/*")) if (line.trim().startsWith("*")) return indent + 1;
        return indent;
    }

    protected int indentClosingBrace(Line line, Buffer buffer) {
        Position pos = matchClosingBrace(new Position(line, 0));
        if (isOpeningBraceOfClassOrMethod(pos.getLine())) pos = findBeginningOfStatement(pos); else if (!pos.getLine().trim().startsWith("{")) pos = findPreviousConditional(pos);
    ...
    private final int indentSwitchLabel(Line line, Buffer buffer) {
        ...
        if (switchLine != null) return buffer.getIndentation(switchLine) + buffer.getIndentSize();
        return 0;
    }

    private final int indentInParen(Position posParen, Buffer buffer) {
        final Line line = posParen.getLine();
        if (line.trim().endsWith("(") || !buffer.getBooleanProperty(Property.LINEUP_ARGLIST)) return buffer.getIndentation(line) + buffer.getIndentSize();
        final int limit = line.length();
        int offset = posParen.getOffset();
        do {
            ++offset;
        } while (offset < limit && line.charAt(offset) <= ' ');
        if (offset <= limit) return buffer.getCol(line, offset);
        return 0;
    }
    ...
    private final int indentAfterOpeningBrace(Line model, String modelText, Buffer buffer) {
        ...
        if (isOpeningBraceOfClassOrMethod(model)) {
            ...
            if (buffer.getBooleanProperty(Property.INDENT_AFTER_OPENING_BRACE)) indent += indentSize;
            return indent;
        }
        Position pos = new Position(model, model.length() - 1);
        if (modelText.charAt(0) != '{') pos = findPreviousConditional(pos);
        int indent = buffer.getIndentation(pos.getLine());
        if (buffer.getBooleanProperty(Property.INDENT_AFTER_BRACE)) indent += indentSize;
        final boolean indentBeforeBrace = buffer.getBooleanProperty(Property.INDENT_BEFORE_BRACE);
        if (indentBeforeBrace && pos.getLine() != model) indent += indentSize;
        return indent;
    }
    ...
    private final int indentAfterElse(Line model, String text, char textFirstChar, Buffer buffer) {
        int indent = buffer.getIndentation(model);
        final boolean indentBeforeBrace = buffer.getBooleanProperty(Property.INDENT_BEFORE_BRACE);
        if (indentBeforeBrace || textFirstChar != '{') return indent + buffer.getIndentSize(); else return indent;
    }
    ...
    private final int indentAfterCloseParen(Line model, String text, char textFirstChar, Buffer buffer) {
        ...
        if (Utilities.isOneOf(s, indentAfter)) {
            ...
            if (s.equals("synchronized")) indent = true;
        } else if (buffer.getModeId() == PHP_MODE) {
            if (s.equals("elseif") || s.equals("foreach")) indent = true;
        }
        if (indent) {
            int modelIndent = buffer.getIndentation(pos.getLine());
            if (textFirstChar != '{' || buffer.getBooleanProperty(Property.INDENT_BEFORE_BRACE)) return modelIndent + buffer.getIndentSize(); else return modelIndent;
        }
        if (buffer.getModeId() == JAVA_MODE) {
            RE re = new UncheckedRE("\\s+new\\s+");
            if (re.getMatch(pos.getLine().getText().substring(0, pos.getOffset())) != null) indent = true;
        }
        int modelIndent = buffer.getIndentation(findBeginningOfStatement(pos).getLine());
        if (indent && (textFirstChar != '{' || buffer.getBooleanProperty(Property.INDENT_BEFORE_BRACE))) return modelIndent + buffer.getIndentSize(); else return modelIndent;
    }
                ...
                case '{':
                    {
                        ...
                        if (modelText.equals("{")) return buffer.getIndentation(model) + buffer.getIndentSize();
                        return indentAfterOpeningBrace(model, modelText, buffer);
                    }
                case ':':
                    {
                        String firstIdentifier = getFirstIdentifier(it.getLine());
                        if (firstIdentifier.equals("case") || firstIdentifier.equals("default")) return buffer.getIndentation(it.getLine()) + buffer.getIndentSize();
                        String trim = it.getLine().trim();
                        if (trim.startsWith(": ") || trim.startsWith(":\t")) continue;
                        if (trim.startsWith("public:") || trim.startsWith("private:") || trim.startsWith("protected:")) return buffer.getIndentation(it.getLine()) + buffer.getIndentSize();
                        break;
                    }
    ...
    protected static Line findModel(Line line) {
        ...
        if (line.flags() == STATE_COMMENT) {
            while (model != null && model.isBlank()) model = model.previous();
        } else {
            while (model != null) {
                if (isAcceptableModel(model)) break; else model = model.previous();
            }
        }
        return model;
    }
    ...
    private static final boolean isAcceptableModel(Line line) {
        ...
        if (flags == STATE_COMMENT || flags == STATE_QUOTE) return false;
        if (line.isBlank()) return false;
        String trim = line.trim();
        char firstChar = trim.charAt(0);
        if (firstChar == '/') {
            if (trim.length() > 1 && trim.charAt(1) == '/') return false;
        } else if (firstChar == '#') return false; else if (firstChar == '=') return false; else if (firstChar == ':') {
    ...
    private boolean isOpeningBraceOfClassOrMethod(Line line) {
        ...
        final String firstIdentifier = getFirstIdentifier(text);
        if (Utilities.isOneOf(firstIdentifier, conditionals)) return false;
        if (firstIdentifier.equals("case") || firstIdentifier.equals("default")) return false;
        return true;
    }
    ...
    protected final String getLastIdentifier(String s) {
        ...
        if (i >= 0 && i < s.length()) return s.substring(i);
        return null;
    }

    private final String getIdentifierBefore(Position pos) {
        while (pos.prev()) if (!Character.isWhitespace(pos.getChar())) break;
    ...
    protected final Position matchClosingBrace(Position start) {
        ...
        while (true) {
            switch(it.prevChar()) {
                ...
                case '{':
                    --count;
                    if (count == 0) return it.getPosition();
                    break;
                case SyntaxIterator.DONE:
        ...
        while (true) {
            switch(it.prevChar()) {
                ...
                case '}':
                    ...
                    break;
                case ';':
                    if (seenBrace) return null;
                    break;
                case ')':
                    ++parenCount;
                    break;
                case '(':
                    if (parenCount == 0) return it.getPosition();
                    --parenCount;
                    break;
                case SyntaxIterator.DONE:
                    return null;
    ...
    protected final Line findSwitch(Line line) {
        ...
        if (pos != null) {
            ...
            do {
                ...
                if (s.equals("switch")) return line;
    ...
    public Position findBeginningOfStatement(Position start) {
        ...
        if (posParen != null) pos = posParen;
        final String trim = trimSyntacticWhitespace(pos.getLine().getText());
        final String lastIdentifier = getLastIdentifier(trim);
        if (lastIdentifier != null && lastIdentifier.equals("else")) return new Position(pos.getLine(), 0);
        final String firstIdentifier = getFirstIdentifier(trim);
        if (firstIdentifier != null && (firstIdentifier.equals("case") || firstIdentifier.equals("default"))) return new Position(pos.getLine(), 0);
        ...
        while (true) {
            char c = it.prevChar();
            if (c == SyntaxIterator.DONE) return it.getPosition();
            if (inParen) {
                if (c == ')') ++count; else if (c == '(') {
                    --count;
                    if (count == 0) inParen = false;
                }
                continue;
            }
            if (c == ')') {
    ...
    public Position findPreviousConditional(Position start) {
        ...
        while (true) {
            ...
            if (previousLine == null) return new Position(pos.getLine(), 0);
            pos.moveTo(previousLine, previousLine.length());
            if (pos.getLine().flags() == STATE_COMMENT) continue;
    ...
    public boolean isInComment(Buffer buffer, Position pos) {
        ...
        for (int i = 0; i < length; i++) {
            if (i == offset) return state == STATE_COMMENT;
            char c = chars[i];
            if (c == '\\' && i < length - 1) {
                continue;
            }
            if (state == STATE_QUOTE) {
                if (c == '"') state = STATE_NEUTRAL;
                continue;
            }
    ...
    public static void insertComment() {
        ...
        if (toBeInserted == null) toBeInserted = "/**\\n * |\\n */";
        ...
        if (caretPos != null) editor.moveDotTo(caretPos);
    ...
    public static void newlineAndIndentForComment() {
        final Editor editor = Editor.currentEditor();
        if (!editor.checkReadOnly()) return;
        ...
        if (flags == STATE_COMMENT) {
            if (s.startsWith("*") && !s.endsWith("*/")) commentPrefix = "* ";
        } else {
            if (s.startsWith("/*") && !s.endsWith("*/")) commentPrefix = "* "; else if (s.startsWith("//")) commentPrefix = "// ";
        }
        ...
        if (editor.getMark() != null) editor.deleteRegion();
    ...
    public Expression getExpressionAtDot(final Editor editor, final boolean exact) {
        if (editor.getModeId() == OBJC_MODE) return super.getExpressionAtDot(editor, exact);
        if (editor.getDot() == null) return null;
        Position begin;
        if (editor.getMark() != null) {
            Region r = new Region(editor);
            begin = r.getBegin();
        } else begin = editor.getDot();
        final Line line = begin.getLine();
        final int offset = begin.getOffset();
        Position posExpr = null;
        if (exact) {
            if (offset < line.length() && isIdentifierPart(line.charAt(offset))) posExpr = findIdentifierStart(line, offset);
            if (posExpr == null) return null;
        }
        if (posExpr == null) {
            ...
            while ((match = re.getMatch(text, index)) != null) {
                ...
                if (!isKeyword(identifier)) {
                    ...
                    if (match.getEndIndex() > offset) break;
                }
                index = match.getEndIndex();
            }
        }
        if (posExpr == null) {
            if (offset < line.length() && isIdentifierStart(line.charAt(offset))) posExpr = findIdentifierStart(line, offset);
        }
        if (posExpr == null) return null;
        ...
        final int arity;
        if (editor.getModeId() == JAVASCRIPT_MODE) arity = -1; else arity = getArity(editor, pos);
        if (arity >= 0) return new JavaExpression(name, arity); else return new JavaExpression(name, arity, TAG_UNKNOWN);
    }

    private int getArity(Editor editor, Position pos) {
        if (pos.getChar() != '(') return -1;
            ...
            if (c == ')') {
                ...
                if (parenCount < 0) {
                    if (arity == 0) {
                        ...
                        if (r.toString().trim().length() > 0) arity = 1;
                    } else ++arity;
                    return arity;
                }
                pos.next();
