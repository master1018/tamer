...
public class Dress extends StdCommand {
    ...
    public boolean execute(MOB mob, Vector commands, int metaFlags) throws java.io.IOException {
        ...
        if ((target.willFollowOrdersOf(mob)) || (CMLib.flags().isBoundOrHeld(target))) {
            ...
            if (CMSecurity.isAllowed(mob, mob.location(), "ORDER") || (CMSecurity.isAllowed(mob, mob.location(), "CMDROOMS") && (target.isMonster())) || (CMSecurity.isAllowed(mob, mob.location(), "CMDMOBS") && (target.isMonster()))) {
                ...
                if ((item.rawProperLocationBitmap() != 0) && (item.amWearingAt(Wearable.IN_INVENTORY)) && (target.isMonster())) {
                    if (item.rawLogicalAnd()) item.wearAt(item.rawProperLocationBitmap()); else {
                        for (long wornCode : Wearable.CODES.ALL()) if (wornCode != Wearable.IN_INVENTORY) {
                            if (item.fitsOn(wornCode) && (wornCode != Wearable.WORN_HELD)) {
                                item.wearAt(wornCode);
                                break;
                            }
                        }
                        if (item.amWearingAt(Wearable.IN_INVENTORY)) item.wearAt(Wearable.WORN_HELD);
                    }
                }
                ...
                if (mob.location().okMessage(mob, msg)) {
                    if (CMLib.commands().postDrop(mob, item, true, false)) {
                        ...
                        if (mob.location().okMessage(mob, msg)) {
                            ...
                            } else mob.tell("You cannot seem to get " + item.name() + " on " + target.name() + ".");
                        } else mob.tell("You cannot seem to get " + item.name() + " to " + target.name() + ".");
                    }
                }
            }
        } else mob.tell(target.name() + " won't let you.");
        return false;
    }
