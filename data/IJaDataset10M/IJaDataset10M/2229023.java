...
final class LibraryTableCellEditor implements TableCellEditor {
    ...
    public boolean isCellEditable(EventObject event) {
        if (event instanceof MouseEvent) {
            ...
            if (VersionUtils.isJava16OrAbove()) {
                if (table.getSelectedRow() == lastRow) return (me.getClickCount() < 2); else {
                    lastRow = table.getSelectedRow();
                    return false;
                }
            } else return (me.getClickCount() < 2);
    ...
    void fireEditingStopped() {
        if (_cellEditorListener != null) _cellEditorListener.editingStopped(new ChangeEvent(this));
    }
    ...
    void fireEditingCanceled() {
        if (_cellEditorListener != null) _cellEditorListener.editingCanceled(new ChangeEvent(this));
    }
