...
public class ImageUtil {
        ...
        for (int dy = 0; dy < destHeight; dy++) {
            for (int dx = 0; dx < destWidth; dx++) {
                ...
                if (vec.x >= 0 && vec.y >= 0 && vec.x <= (sourceWidth - 1) && vec.y <= (sourceHeight - 1)) {
                    ...
                    double yf = vec.y - y0;
                    int color = interpolateColor(xf, yf, image.getRGB(x0, y0), image.getRGB(x1, y0), image.getRGB(x0, y1), image.getRGB(x1, y1));
     ...
     */
    public static Sector warpImageWithControlPoints(BufferedImage sourceImage, java.awt.geom.Point2D[] imagePoints, LatLon[] geoPoints, BufferedImage destImage) {
        if (sourceImage == null) {
     ...
     */
    public static Sector warpImageWithControlPoints4(BufferedImage sourceImage, java.awt.geom.Point2D[] imagePoints, LatLon[] geoPoints, BufferedImage destImage) {
        if (sourceImage == null) {
            String message = Logging.getMessage("nullValue.SourceImageIsNull");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
     ...
     *                                  <code>imagePoints</code> or <code>geoPoints</code> have length less than 3.
     */
    public static Sector warpImageWithControlPoints3(BufferedImage sourceImage, java.awt.geom.Point2D[] imagePoints, LatLon[] geoPoints, BufferedImage destImage) {
        if (sourceImage == null) {
            String message = Logging.getMessage("nullValue.SourceImageIsNull");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
        ...
        if (Sector.isSector(corners) && destSector.isSameSector(corners)) {
            ...
            transform = transform.multiply(Matrix.fromImageToGeographic(destImage.getWidth(), destImage.getHeight(), destSector));
     ...
     */
    public static Sector warpImageWithWorldFile(BufferedImage sourceImage, AVList worldFileParams, BufferedImage destImage) {
        if (sourceImage == null) {
            String message = Logging.getMessage("nullValue.SourceImageIsNull");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
     ...
     *                                  than 3.
     */
    public static void computeBestFittingControlPoints4(java.awt.geom.Point2D[] imagePoints, LatLon[] geoPoints, java.awt.geom.Point2D[] outImagePoints, LatLon[] outGeoPoints) {
        String message = validateControlPoints(4, imagePoints, geoPoints);
        if (message != null) {
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
        ...
        for (int[] indices : new int[][] { { 0, 1, 2 }, { 0, 1, 3 }, { 1, 2, 3 }, { 0, 2, 3 } }) {
            java.awt.geom.Point2D[] points = new java.awt.geom.Point2D[] { imagePoints[indices[0]], imagePoints[indices[1]], imagePoints[indices[2]] };
            ...
            for (int j = 0; j < 4; j++) {
                ...
                double d = diff.getLatitude().degrees * diff.getLatitude().degrees + diff.getLongitude().degrees * diff.getLongitude().degrees;
                error += d;
            }
            if (error < minError) {
    ...
    private static String validateControlPoints(int numExpected, java.awt.geom.Point2D[] imagePoints, LatLon[] geoPoints) {
        if (imagePoints == null) {
    ...
    public static void mergeImage(Sector canvasSector, Sector imageSector, double aspectRatio, BufferedImage image, BufferedImage canvas) {
        ...
        if (!(canvasSector.intersects(imageSector))) return;
        ...
        double sh = ((double) subHeight / (double) image.getHeight()) * (imageSector.getDeltaLat().divide(canvasSector.getDeltaLat()));
        double sw = ((double) subWidth / (double) image.getWidth()) * (imageSector.getDeltaLon().divide(canvasSector.getDeltaLon()));
        double dh = subHeight * (-imageSector.getMaxLatitude().subtract(canvasSector.getMaxLatitude()).degrees / canvasSector.getDeltaLat().degrees);
        double dw = subWidth * (imageSector.getMinLongitude().subtract(canvasSector.getMinLongitude()).degrees / canvasSector.getDeltaLon().degrees);
    ...
    public static Sector positionImage(BufferedImage sourceImage, Point[] imagePoints, LatLon[] geoPoints, BufferedImage destImage) {
        if (imagePoints.length == 3) return positionImage3(sourceImage, imagePoints, geoPoints, destImage); else if (imagePoints.length == 4) return positionImage4(sourceImage, imagePoints, geoPoints, destImage); else return null;
    }
    ...
    public static Sector positionImage4(BufferedImage sourceImage, Point[] imagePoints, LatLon[] geoPoints, BufferedImage destImage) {
        BarycentricQuadrilateral sourceLatLon = new BarycentricQuadrilateral(geoPoints[0], geoPoints[1], geoPoints[2], geoPoints[3]);
        BarycentricQuadrilateral sourcePixels = new BarycentricQuadrilateral(imagePoints[0], imagePoints[1], imagePoints[2], imagePoints[3]);
        ArrayList<LatLon> extremes = new ArrayList<LatLon>(4);
        double[] bc = sourcePixels.getBarycentricCoords(new Vec4(0, sourceImage.getHeight(), 0));
        extremes.add(sourceLatLon.getLocation(bc));
        bc = sourcePixels.getBarycentricCoords(new Vec4(sourceImage.getWidth(), sourceImage.getHeight(), 0));
    ...
    public static int getMipmapType(int imageType) {
        if (imageType == BufferedImage.TYPE_CUSTOM) return BufferedImage.TYPE_INT_ARGB;
        return imageType;
    }
    ...
    public static BufferedImage convertToPowerOfTwoImage(BufferedImage image, boolean scaleToFit) {
        ...
        BufferedImage potImage = new BufferedImage(potWidth, potHeight, image.getColorModel().hasAlpha() ? BufferedImage.TYPE_4BYTE_ABGR : BufferedImage.TYPE_3BYTE_BGR);
    ...
    public static AVList openSpatialImage(File imageFile, int interpolation_mode) throws IOException {
        ...
        BufferedImage image;
        Sector sector;
        if ((imageFile.getName().toLowerCase().endsWith(".tiff") || (imageFile.getName().toLowerCase().endsWith(".tif")))) {
        ...
        if (sector == null) ImageUtil.reprojectUtmToGeographic(values, interpolation_mode);
        sector = (Sector) values.getValue(AVKey.SECTOR);
        if (sector == null) {
    ...
    public static AVList readGeoKeys(GeotiffReader reader, int imageIndex, AVList values) throws IOException {
        if (null == values) values = new AVListImpl();
        if (null == reader) return values;
     ...
     * @throws IOException if there is a problem opening the file.
     */
    private static AVList handleGeotiff(BufferedImage image, GeotiffReader reader, int imageIndex, int interpolation_mode) throws IOException {
        AVList values = new AVListImpl();
        if (null != image) {
            values.setValue(AVKey.IMAGE, image);
        ...
        if (AVKey.COORDINATE_SYSTEM_PROJECTED.equals(values.getValue(AVKey.COORDINATE_SYSTEM))) ImageUtil.reprojectUtmToGeographic(values, interpolation_mode);
        return values;
    }
    ...
    public static Sector calcBoundingBoxForUTM(AVList params) throws IOException {
        ...
        UTMCoord utmUpperLeft = UTMCoord.fromUTM(zone, hemisphere, upperLeft.getEasting() - xPixelSize * .5, upperLeft.getNorthing() - yPixelSize * .5);
        UTMCoord utmLowerRight = UTMCoord.fromUTM(zone, hemisphere, utmUpperLeft.getEasting() + (width * xPixelSize), utmUpperLeft.getNorthing() + (height * yPixelSize));
        UTMCoord utmLowerLeft = UTMCoord.fromUTM(zone, upperLeft.getHemisphere(), utmUpperLeft.getEasting(), utmLowerRight.getNorthing());
        UTMCoord utmUpperRight = UTMCoord.fromUTM(zone, upperLeft.getHemisphere(), utmLowerRight.getEasting(), utmUpperLeft.getNorthing());
    ...
    public static void reprojectUtmToGeographic(AVList values, int mode) {
        ...
        if ((image.getColorModel() != null) && (image.getColorModel() instanceof IndexColorModel)) {
            biOut = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB, (IndexColorModel) image.getColorModel());
        } else {
        ...
        TMCoord tmUpperLeft = TMCoord.fromLatLon(utmUpperLeft.getLatitude(), utmUpperLeft.getLongitude(), earth, null, null, Angle.fromDegrees(0.0), utmUpperLeft.getCentralMeridian(), False_Easting, False_Northing, Scale);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                TMCoord TM = TMCoord.fromLatLon(Angle.fromDegreesLatitude(yTarget), Angle.fromDegreesLongitude(xTarget), earth, null, null, Angle.fromDegrees(0.0), utmUpperLeft.getCentralMeridian(), False_Easting, False_Northing, Scale);
                ...
                if (mode == ImageUtil.BILINEAR_INTERPOLATION) {
                    ...
                    double dx = rxD - iX;
                    double dy = ryD - iY;
                    if ((iX > 0) && (iY > 0)) if ((iX < width - 1) && (iY < height - 1)) {
                    ...
                    } else biOut.setRGB(x, y, 0);
                } else {
                    if ((rx > 0) && (ry > 0)) if ((rx < width) && (ry < height)) biOut.setRGB(x, y, image.getRGB(Long.valueOf(rx).intValue(), Long.valueOf(ry).intValue())); else biOut.setRGB(x, y, 0);
                }
            }
        }
     ...
     *                              thrown.
     */
    public static AlignedImage alignImage(BufferedImage sourceImage, float[] latitudes, float[] longitudes) throws InterruptedException {
     ...
     *                              thrown.
     */
    public static AlignedImage alignImage(BufferedImage sourceImage, float[] latitudes, float[] longitudes, Sector sector, Dimension dimension) throws InterruptedException {
        if (sourceImage == null) {
            String message = Logging.getMessage("nullValue.ImageIsNull");
            Logging.logger().severe(message);
            throw new IllegalStateException(message);
        ...
        GeographicImageInterpolator grid = new GeographicImageInterpolator(new Dimension(sourceWidth, sourceHeight), longitudes, latitudes, 10, 1);
        if (sector == null) {
        ...
        if (dimension == null) {
            ...
            dimension = new Dimension((int) Math.round(pixelsPerDegree * sector.getDeltaLonDegrees()), (int) Math.round(pixelsPerDegree * sector.getDeltaLatDegrees()));
        }
        ...
        for (int j = 0; j < dimension.height; j++) {
            ...
            for (int i = 0; i < dimension.width; i++) {
                ...
                if (cell != null) {
                    int color = interpolateColor(cell.uv[0], cell.uv[1], sourceColors[cell.fieldIndices[0]], sourceColors[cell.fieldIndices[1]], sourceColors[cell.fieldIndices[3]], sourceColors[cell.fieldIndices[2]]);
    ...
    public static void alignImageDump(BufferedImage sourceImage, float[] latitudes, float[] longitudes) {
        try {
            ...
            if (!imageFile.getName().endsWith(".png")) imageFile = new File(imageFile.getPath() + ".png");
    ...
    public static BufferedImage toCompatibleImage(BufferedImage image) {
        ...
        if (java.awt.GraphicsEnvironment.isHeadless()) return image;
        if (!isCompatibleImage(image) && (image.getWidth() <= MAX_IMAGE_SIZE_TO_CONVERT) && (image.getHeight() <= MAX_IMAGE_SIZE_TO_CONVERT)) {
            java.awt.image.BufferedImage compatibleImage = createCompatibleImage(image.getWidth(), image.getHeight(), image.getTransparency());
    ...
    public static BufferedImage mapColors(BufferedImage sourceImage, int[] originalColors, int[] newColors) {
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                int index = j * width + i;
                for (int c : originalColors) {
                    if (sourceColors[index] == originalColors[c]) destColors[index] = newColors[c];
                }
            }
        }
        sourceColors = null;
    ...
    public static BufferedImage visualize(BufferWrapperRaster raster) {
        ...
        short[][] data = new short[][] { new short[size], new short[size] };
        ...
        ColorModel cm = new ComponentColorModel(cs, nBits, hasVoids, false, hasVoids ? Transparency.TRANSLUCENT : Transparency.OPAQUE, DataBuffer.TYPE_USHORT);
