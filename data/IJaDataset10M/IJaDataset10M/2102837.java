...
final class JikesRVMSocketImpl extends SocketImpl implements VM_SizeConstants {
   ...
   */
    protected synchronized void shutdownInput() throws IOException {
        ...
        if (sysCall.sysNetSocketShutdown(native_fd, CLOSE_INPUT) != 0) throw new IOException("could not close input side of socket");
    }
   ...
   */
    protected synchronized void shutdownOutput() throws IOException {
        if (native_fd == -1) throw new IOException("socket already closed");
        if (sysCall.sysNetSocketShutdown(native_fd, CLOSE_OUTPUT) != 0) throw new IOException("could not close input side of socket");
    }
   ...
   */
    private void connectInternal(InetAddress remoteAddr, int remotePort, int timeoutMillis) throws IOException {
        int rc = -1;
        double totalWaitTimeSeconds = (timeoutMillis > 0) ? ((double) timeoutMillis) / 1000.0 : VM_ThreadEventConstants.WAIT_INFINITE;
