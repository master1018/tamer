...
public class ProtocolFastAbcast extends ProtocolModule implements Trigger {
    ...
    public ProtocolFastAbcast(String name, ProtocolStack stack, Abcast abcast, FD fd, RPT2PT rpt2pt) throws AlreadyExistingProtocolModuleException {
        ...
        for (int i = 0; i < 2 * MAX_PROCESSES; i++) initiatedAbcast.add(ServiceCallOrResponse.createServiceCallOrResponse(rpt2pt, true));
            ...
            public void evaluate(Object params, Message dmessage) {
                synchronized (this.parent) {
                    ...
                    try {
                        ...
                        ex.printStackTrace();
                        throw new RuntimeException("ProtocolFastAbcast: " + "abcastExecuter: " + ex.getMessage());
                    }
                }
            }
        };
        LinkedList<ServiceCallOrResponse> initiatedRpt2pt = new LinkedList<ServiceCallOrResponse>();
        for (int i = 0; i < 3 * MAX_PROCESSES; i++) initiatedRpt2pt.add(ServiceCallOrResponse.createServiceCallOrResponse(rpt2pt, true));
        for (int i = 0; i < MAX_MESSAGES; i++) initiatedRpt2pt.add(ServiceCallOrResponse.createServiceCallOrResponse(abcast, false));
            ...
            public void evaluate(RPT2PTResponseParameters infos, Transportable message) {
                synchronized (this.parent) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("ProtocolFastAbcast: " + "rpt2ptListener: " + ex.getMessage());
                    }
                }
            }
        };
        LinkedList<ServiceCallOrResponse> initiatedFd = new LinkedList<ServiceCallOrResponse>();
        for (int i = 0; i < 3 * MAX_PROCESSES; i++) initiatedFd.add(ServiceCallOrResponse.createServiceCallOrResponse(rpt2pt, true));
        fdListener = fd.new Listener(this, initiatedFd) {

            public void evaluate(FDResponseParameters infos, Transportable message) {
                synchronized (this.parent) {
                    GroupCommEventArgs ga = new GroupCommEventArgs();
                    ...
                    try {
                        ...
                        throw new RuntimeException("ProtocolFastAbcast: " + "fdListener: " + ex.getMessage());
                    }
                }
            }
        };
        ...
        switch(type) {
            case Constants.JOINREMOVELIST:
                ...
                while (itStart.hasNext()) {
                    RPT2PTCallParameters jparams = new RPT2PTCallParameters((PID) itStart.next(), new TBoolean(true), new TBoolean(false));
                    rpt2pt.call(jparams, null);
                }
                Iterator itStop = stop.iterator();
                while (itStop.hasNext()) {
                    RPT2PTCallParameters jparams = new RPT2PTCallParameters((PID) itStop.next(), new TBoolean(false), new TBoolean(false));
                    rpt2pt.call(jparams, null);
                }
                break;
            case Constants.PT2PTSEND:
                ...
                RPT2PTCallParameters rparams = new RPT2PTCallParameters(ppid, promisc, new TBoolean(true));
