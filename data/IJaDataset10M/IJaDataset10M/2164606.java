...
public class EnergyEndpointerTest {
    ...
    public static void main(String[] argv) {
        if (argv.length < 3) {
            System.out.println("Usage: java testClass <testName> " + "<propertiesFile> <audiofilename>");
        }
        try {
            ...
            Preemphasizer preemphasizer = new Preemphasizer("Preemphasizer", testName, props, fet.getAudioSource());
            Windower windower = new Windower("HammingWindow", testName, props, preemphasizer);
            SpectrumAnalyzer spectrumAnalyzer = new SpectrumAnalyzer("FFT", testName, props, windower);
            MelFilterbank melFilterbank = new MelFilterbank("MelFilter", testName, props, spectrumAnalyzer);
            MelCepstrumProducer melCepstrum = new MelCepstrumProducer("MelCepstrumProducer", testName, props, melFilterbank);
            EnergyEndpointer endpointer = new EnergyEndpointer();
            endpointer.initialize("EnergyEndpointer", testName, props, melCepstrum);
            NonSpeechFilter nonSpeechFilter = new NonSpeechFilter("NonSpeechFilter", testName, props, endpointer);
            CepstrumSource finalSource = endpointer;
            if (Boolean.getBoolean("removeNonSpeech")) {
                finalSource = nonSpeechFilter;
            }
            final CepstraViewer cepstraViewer = new CepstraViewer("EndpointerTest");
            cepstraViewer.show();
            CepstraGroupProducer groupProducer = new CepstraGroupProducer("CepstraGroupProducer", testName, finalSource) {
