 ...
 * constraint. The semantics of this aggregate constraint are that at least one
 * of the individual constraint alternatives must be satisfied. The alternatives
 * do not have to be instances of the same type, but they cannot themselves be
 * <code>ConstraintAlternatives</code> instances.
 * <p>
 ...
 * <p>
 * An instance containing an exhaustive list of alternatives (for example, an
 * instance containing both <code>ClientAuthentication.YES</code> and
 * <code>ClientAuthentication.NO</code>) serves no useful purpose, as a
 * requirement or as a preference. A <i>don't care</i> condition should be
 * expressed by the <i>absence</i> of constraints.
 * 
...
public final class ConstraintAlternatives implements RelativeTimeConstraint, Serializable {
    ...
    /**
	 * @serialField constraints InvocationConstraint[] The alternative
	 *              constraints.
	 */
    private static final ObjectStreamField[] serialPersistentFields = { new ObjectStreamField("constraints", InvocationConstraint[].class, true) };
	 ...
	 *             if the argument is <code>null</code> or any element is
	 *             <code>null</code>
	 * @throws IllegalArgumentException
	 *             if any of the elements are instances of
	 *             <code>ConstraintAlternatives</code>, or if fewer than two
	 *             elements remain after duplicate constraints are removed
	 */
    public ConstraintAlternatives(InvocationConstraint[] constraints) {
        this.constraints = reduce((InvocationConstraint[]) constraints.clone());
        setRelative();
    }
	 ...
	 * @throws IllegalArgumentException
	 *             if any of the elements are instances of
	 *             <code>ConstraintAlternatives</code>, or if the elements are
	 *             not all instances of <code>InvocationConstraint</code>, or if
	 *             fewer than two elements remain after duplicate constraints
	 *             are removed
	 */
	 ...
	 * modified nor retained; subsequent changes to that argument have no effect
	 ...
	 *             <code>null</code>
	 * @throws IllegalArgumentException
	 *             if the argument is empty, or if any of the elements are
	 ...
	 * non-null and not ConstraintAlternatives instances. Removes duplicates and
	 * returns a single constraint if there's only one left, otherwise returns
	 * an ConstraintAlternatives containing the remaining constraints. The
    ...
    /**
	 * Verifies that the array has at least min elements, and that the elements
	 * are all non-null and not ConstraintAlternatives instances.
	 */
    ...
    /**
	 * Eliminates duplicates, modifying the array in place, and returns the
    ...
    /**
	 * Returns an immutable set of all of the constraints. Any attempt to modify
	 * this set results in an {@link UnsupportedOperationException} being
	 * thrown.
	 * 
	 ...
	 * time, and invoking the <code>create</code> method of this class with the
	 * revised collection of constraints.
	 */
	 ...
	 * <code>null</code> and none are instances of this class, and that there
	 * are no duplicates.
	 * 
	 * @throws InvalidObjectException
	 *             if there are less than two constraints, or any constraint is
