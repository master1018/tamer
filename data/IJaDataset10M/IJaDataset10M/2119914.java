...
public class OCL2TestSuiteRunner {
    ...
    protected static final String SHOW_WARNINGS = "ExtensibleTestSuite_showWarnings";

    private static final String EXECUTABLE_EXTENSION = "testSuite";

    private static final String CONFIGURATION_ELEMENT = "testSuites";

    private static final String TESTSUITE_EXTENSIONPOINT = "tudresden.ocl20.pivot.testsuite";
    ...
    public static Test suite() throws CoreException {
        ...
        for (Test registeredTest : registeredTestSuites) allTestSuite.addTest(registeredTest);
        return new TestSetup(allTestSuite) {

            @Override
            protected void tearDown() {
                String messages = StringBufferAppender.getMessages();
                if (messages.length() == 0) return; else {
                    Preferences preferences = TestSuitePlugin.getDefault().getPluginPreferences();
                    boolean showWarnings = preferences.getBoolean(SHOW_WARNINGS);
                    try {
                        preferences.store(new FileOutputStream(TestSuitePlugin.getDefault().getStateLocation().append("showWarnings.pref").toFile()), null);
                    } catch (FileNotFoundException e) {
                    } catch (IllegalStateException e) {
                    } catch (IOException e) {
                    }
                    if (showWarnings) MessageDialog.openInformation(parentShell, "Warnings", messages);
                }
            }
        };
    }
    ...
    private static List<Test> getRegisteredTestSuites() throws CoreException {
        ...
        IExtensionPoint extensionPoint = extensionRegistry.getExtensionPoint(TESTSUITE_EXTENSIONPOINT);
        for (IConfigurationElement configElement : extensionPoint.getConfigurationElements()) {
            if (configElement.getName().equals(CONFIGURATION_ELEMENT)) {
                Object testSuite = configElement.createExecutableExtension(EXECUTABLE_EXTENSION);
