...
public abstract class Complex extends Quantity {
    ...
    public static CComplex imOne() {
        if (imOne == null) imOne = new CComplex(IntNum.zero(), IntNum.one());
        return imOne;
    }

    public static CComplex imMinusOne() {
        if (imMinusOne == null) imMinusOne = new CComplex(IntNum.zero(), IntNum.minusOne());
        return imMinusOne;
    }
    ...
    public static Complex make(RealNum re, RealNum im) {
        ...
        if (!re.isExact() || !im.isExact()) return new DComplex(re.doubleValue(), im.doubleValue());
        return new CComplex(re, im);
    }

    public static Complex make(double re, double im) {
        if (im == 0.0) return new DFloNum(re);
    ...
    public static Complex power(Complex x, Complex y) {
        if (y instanceof IntNum) return (Complex) x.power((IntNum) y);
        double x_re = x.doubleRealValue();
        double x_im = x.doubleImagValue();
        double y_re = y.doubleRealValue();
        double y_im = y.doubleImagValue();
        if (x_im == 0.0 && y_im == 0 && (x_re >= 0 || Double.isInfinite(x_re) || Double.isNaN(x_re))) return new DFloNum(Math.pow(x_re, y_re));
    ...
    public int compare(Object obj) {
        if (!(obj instanceof Complex)) return ((Numeric) obj).compareReversed(this);
    ...
    public String toString(int radix) {
        if (im().isZero()) return re().toString(radix);
        String imString = im().toString(radix) + "i";
        if (imString.charAt(0) != '-') imString = "+" + imString;
        if (re().isZero()) return imString;
    ...
    public Numeric add(Object y, int k) {
        if (y instanceof Complex) return add(this, (Complex) y, k);
        return ((Numeric) y).addReversed(this, k);
    }

    public Numeric addReversed(Numeric x, int k) {
        if (x instanceof Complex) return add((Complex) x, this, k);
    ...
    public Numeric mul(Object y) {
        if (y instanceof Complex) return times(this, (Complex) y);
        return ((Numeric) y).mulReversed(this);
    }

    public Numeric mulReversed(Numeric x) {
        if (x instanceof Complex) return times((Complex) x, this);
        throw new IllegalArgumentException();
    }

    public static Complex divide(Complex x, Complex y) {
        if (!x.isExact() || !y.isExact()) return DComplex.div(x.doubleRealValue(), x.doubleImagValue(), y.doubleRealValue(), y.doubleImagValue());
        RealNum x_re = x.re();
        RealNum x_im = x.im();
        RealNum y_re = y.re();
        RealNum y_im = y.im();
    ...
    public Numeric div(Object y) {
        if (y instanceof Complex) return divide(this, (Complex) y);
        return ((Numeric) y).divReversed(this);
    }

    public Numeric divReversed(Numeric x) {
        if (x instanceof Complex) return divide((Complex) x, this);
        throw new IllegalArgumentException();
    }
