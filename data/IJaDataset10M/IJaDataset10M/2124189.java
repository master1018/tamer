...
public class EditCopyBudgetCategoryValuesToPrevious extends MossMenuItem {
    ...
    public EditCopyBudgetCategoryValuesToPrevious(MainFrame frame) {
        super(frame, TextFormatter.getTranslation(MenuKeys.MENU_EDIT_COPY_VALUES_TO_PREVIOUS_BUDGET_PERIOD), KeyStroke.getKeyStroke(KeyEvent.VK_COMMA, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask() + KeyEvent.SHIFT_DOWN_MASK + KeyEvent.ALT_DOWN_MASK));
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        for (BudgetCategory bc : ((MainFrame) getFrame()).getBudgetCategoriesInSelectedPeriod()) {
            ...
            try {
                if (bc.getAmount(bc.getBudgetPeriodType().getBudgetPeriodOffset(currentlySelectedDate, -1)) == 0) bc.setAmount(bc.getBudgetPeriodType().getBudgetPeriodOffset(currentlySelectedDate, -1), amount);
    ...
    public void updateMenus() {
        ...
        this.setText(TextFormatter.getTranslation(MenuKeys.MENU_EDIT_COPY_VALUES_TO_PREVIOUS_BUDGET_PERIOD) + " (" + treeTableModel.getColumnName(2) + " " + TextFormatter.getTranslation(BuddiKeys.TO) + " " + treeTableModel.getColumnName(1) + ")");
