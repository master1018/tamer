 ...
 */
public class IdentifyJcrTemplate extends WorkflowTemplateHelper implements WorkflowTemplate {
    ...
    public String describe() {
        return "This template performs the identification and storing steps " + "of the Testbed's experiment. It implements insert in JCR repository.";
    }
    ...
    public WorkflowResult execute(DigitalObject dgoA) {
        ...
        try {
            ...
            wfResultItem.addLogInfo("STEP 3: Update digital object in JCR repository. initial digital object: " + dgoB.toString());
    ...
    public Event buildEvent(URI format) {
        ...
        Property pIdentificationContent = new Property.Builder(URI.create(FORMAT_EVENT_TYPE)).name("content by reference").value(format.toString()).description("This is a format for initial document identified by identification service").unit("URI").type("digital object format").build();
        pList.add(pIdentificationContent);
        Event eIdentifyFormat = new Event(IDENTIFY_EVENT, System.currentTimeMillis() + "", new Double(100), new Agent("JCR Repository v1.0", "The Planets Jackrabbit Content Repository", "planets://data/repository"), pList);
