...
public class ImgReaderPGX extends ImgReader implements EndianType {
    ...
    public ImgReaderPGX(File in) throws IOException {
        ...
        try {
            ...
            if (!(st.nextToken()).equals("PG")) throw new IOException(in.getName() + " is not a PGX file");
            String tmp = st.nextToken();
            if (tmp.equals("LM")) byteOrder = LITTLE_ENDIAN; else if (tmp.equals("ML")) byteOrder = BIG_ENDIAN; else throw new IOException(in.getName() + " is not a PGX file");
            if (nTokens == 6) {
                tmp = st.nextToken();
                if (tmp.equals("+")) isSigned = false; else if (tmp.equals("-")) isSigned = true; else throw new IOException(in.getName() + " is not a PGX file");
            }
            try {
                bitDepth = (new Integer(st.nextToken())).intValue();
                if ((bitDepth <= 0) || (bitDepth > 31)) throw new IOException(in.getName() + " is not a valid PGX file");
        ...
        nc = 1;
        if (bitDepth <= 8) packBytes = 1; else if (bitDepth <= 16) packBytes = 2; else packBytes = 4;
    }
    ...
    public DataBlk getInternCompData(DataBlk blk, int c) {
        ...
        if (blk.getDataType() != DataBlk.TYPE_INT) {
            if (intBlk == null) intBlk = new DataBlkInt(blk.ulx, blk.uly, blk.w, blk.h); else {
                intBlk.ulx = blk.ulx;
                intBlk.uly = blk.uly;
                intBlk.w = blk.w;
                intBlk.h = blk.h;
                ...
                case 1:
                    ...
                    if (isSigned) {
                        for (i = blk.uly; i < mi; i++) {
                            ...
                            in.read(buf, 0, blk.w);
                            for (k = (i - blk.uly) * blk.w + blk.w - 1, j = blk.w - 1; j >= 0; k--) barr[k] = (((buf[j--] & 0xFF) << paddingLength) >> paddingLength);
                        }
                    } else {
                        for (i = blk.uly; i < mi; i++) {
                            in.seek(offset + i * w + blk.ulx);
                            in.read(buf, 0, blk.w);
                            for (k = (i - blk.uly) * blk.w + blk.w - 1, j = blk.w - 1; j >= 0; k--) barr[k] = (((buf[j--] & 0xFF) << paddingLength) >>> paddingLength) - levShift;
                        }
                    }
                    break;
                case 2:
