...
public class MATSimNet2QGIS {
    ...
    public static class ShapeFileWriter2 {
        ...
        public static void writeGeometries(Collection<Feature> features, String filename) throws IOException, FactoryException, SchemaException {
            ShapefileDataStore datastore = new ShapefileDataStore((new File(filename)).toURI().toURL());
            FeatureType ft = (features.iterator().next()).getFeatureType();
            datastore.createSchema(ft);
            ((FeatureStore) (datastore.getFeatureSource(ft.getTypeName()))).addFeatures(DataUtilities.reader(features));
        }
    }
    ...
    public void addParameter(String paraName, Class clazz, Map<String, ?> parameters) {
