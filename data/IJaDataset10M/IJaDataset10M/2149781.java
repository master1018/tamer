...
import jimo.spi.im.api.Response;
import jimo.spi.im.api.TextComponent;
import jimo.spi.im.api.exception.IMException;
import jimo.spi.im.api.exception.IllegalArgumentException;
...
class NotificationServer extends MsnServer {
	 ...
	 */
    NotificationServer(MsnProtocol protocol, EventProcessor processor, String host, int port, ProxyInfo info) throws java.io.IOException, jimo.spi.im.api.exception.IllegalStateException {
    ...
    void removeBuddyFromForwardList(Buddy buddy) throws IllegalArgumentException {
        String group = buddy.getGroup();
        if (group == null) throw new IllegalArgumentException("Group name of the buddy is not specified");
    ...
    private void handleSBInvitation(AbstractCommand cmd) {
        ...
        try {
            ...
            if (res.isAccepted()) joinSBSession(sessionID, server, authString, conf);
        } catch (IMException e) {
        }
    }
