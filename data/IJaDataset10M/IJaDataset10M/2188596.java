...
public class ParsedHTML {
    ...
    private static final String[] TEXT_ELEMENTS = { "p", "h1", "h2", "h3", "h4", "h5", "h6" };
    ...
    private HTMLElement toNoscriptElement(Element element) {
        ...
        if (!_enableNoScriptNodes) result = new NoScriptElement(element);
        return result;
    }
    ...
    static class FormControlFactory extends HTMLElementFactory {
        ...
        private HTMLElement newControlWithoutForm(ParsedHTML parsedHTML, Element element) {
            if ((element.getNodeName().equalsIgnoreCase("button") || element.getNodeName().equalsIgnoreCase("input")) && isValidNonFormButtonType(NodeUtils.getNodeAttribute(element, "type"))) {
    ...
    void setRootNode(Node rootNode) {
        if (_rootNode != null && rootNode != _rootNode) throw new IllegalStateException("The root node has already been defined as " + _rootNode + " and cannot be redefined as " + rootNode);
        _rootNode = rootNode;
