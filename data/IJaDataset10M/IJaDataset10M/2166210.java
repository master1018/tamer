...
public class InferenceTestSuite {
    ...
    public static boolean compareAnswers(ProofProcessor pp, ArrayList answerList) {
        ...
        for (int j = 0; j < pp.numAnswers(); j++) {
            ...
            if (!pp.equalsAnswer(j, (String) answerList.get(j))) return true;
        }
        return false;
    }
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].endsWith(".tq")) {
                ...
                while (it.hasNext()) {
                    ...
                    if (formula.indexOf(";") != -1) formula = formula.substring(0, formula.indexOf(";"));
                    System.out.println("INFO in InferenceTestSuite.test(): Formula: " + formula);
                    if (formula.startsWith("(note")) note = formula.substring(6, formula.length() - 1); else if (formula.startsWith("(query")) query = formula.substring(7, formula.length() - 1); else if (formula.startsWith("(answer")) answerList.add(formula.substring(8, formula.length() - 1)); else if (formula.startsWith("(time")) timeout = Integer.parseInt(formula.substring(6, formula.length() - 1)); else kb.tell(formula);
                }
