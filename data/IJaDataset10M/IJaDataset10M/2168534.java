...
public class JavaSearcherPredicate implements BasicPredicateIF {
    ...
    public QueryMatches satisfy(QueryMatches matches, Object[] arguments) throws InvalidQueryException {
        ...
        if (!matches.bound(qix)) throw new InvalidQueryException("Second argument to " + getName() + " must " + "be bound");
        if (six >= 0 && matches.bound(six)) throw new InvalidQueryException("Third argument to " + getName() + " must " + "be unbound");
            ...
            try {
                while (sr.next()) {
                    ...
                    if (valtype == SearcherIF.OCCURRENCE_URI) {
                        OccurrenceIndexIF occindex = (OccurrenceIndexIF) topicmap.getIndex("net.ontopia.topicmaps.core.index.OccurrenceIndexIF");
