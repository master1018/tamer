...
public final class ProjectListFormsResource extends AbstractProjectResource {
   ...
   */
    private Response getFormResponse(String id) {
        Request reqGET = new Request(Method.GET, RIAPUtils.getRiapBase() + application.getSettings().getString(Consts.APP_DATASETS_URL) + "/" + id + "/forms");
        ...
        @SuppressWarnings("unchecked") ObjectRepresentation<Response> or = (ObjectRepresentation<Response>) response.getEntity();
        try {
            return or.getObject();
        } catch (IOException e) {
