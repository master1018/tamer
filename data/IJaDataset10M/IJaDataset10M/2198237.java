...
public class GenerateMSPFileForGivenBins {
    ...
    public static void main(String args[]) throws Exception {
        if (args.length <= 6) {
            ...
            System.out.println("args[5...n]: bin ids to generate msp files");
        ...
        Session session = HibernateFactory.newInstance().getSession();
        FileDestination dest = new FileDestination(args[4]);
        dest.setIdentifier("result.msp");
        OutputStream out = dest.getOutputStream();
        OutputStreamWriter writer = new OutputStreamWriter(out);
        for (int id : ids) {
            Bin bin = (Bin) session.load(Bin.class, id);
            String msp = BinToMspConverter.convertBinToMsp(bin);
            writer.write(msp);
            writer.write("\n");
        }
        writer.flush();
