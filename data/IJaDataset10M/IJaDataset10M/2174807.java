...
public class DGRMConfigurator extends VisPlugin {
    ...
    private static final int IDX_SRC = 0;

    private static final int IDX_DST = 1;

    private static final int IDX_RATIO = 2;

    private static final int IDX_DELAY = 3;

    private static final int IDX_DEL = 4;

    private static final String[] columns = new String[] { "Source", "Destination", "Success Ratio (%)", "Delay (ms)", "Delete" };
    ...
    private void doAddLink() {
        ...
        Object description[] = { columns[0], source, columns[1], dest, columns[2], ratio, columns[3], delay };
        ...
        if (dest.getSelectedItem() instanceof Mote) {
            newEdge = new DirectedGraphMedium.Edge(((Mote) source.getSelectedItem()).getInterfaces().getRadio(), new DGRMDestinationRadio(((Mote) dest.getSelectedItem()).getInterfaces().getRadio(), ((Number) ratio.getValue()).doubleValue(), ((Number) delay.getValue()).longValue()));
        } else {
            newEdge = new DirectedGraphMedium.Edge(((Mote) source.getSelectedItem()).getInterfaces().getRadio(), new DGRMDestinationRadio(null, ((Number) ratio.getValue()).doubleValue(), ((Number) delay.getValue()).longValue()));
        }
    ...
    final AbstractTableModel model = new AbstractTableModel() {
        ...
        public void setValueAt(Object value, int row, int column) {
            ...
            if (column == IDX_DELAY) {
                try {
                    ((DGRMDestinationRadio) edge.superDest).delay = ((Number) value).longValue() * Simulation.MILLISECOND;
                    radioMedium.requestEdgeAnalysis();
                } catch (ClassCastException e) {
                }
                return;
