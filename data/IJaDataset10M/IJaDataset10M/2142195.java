...
public class CodeAttribute extends ClassAttribute {
    ...
    public void setTheCode(Insn insn) {
        makeValid();
        if (insn != null && insn.opcode() != Insn.opc_target) throw new InsnError("The initial instruction in all methods must be a target");
        theCode = insn;
    }
        ...
        if (theCodeBytes != null) {
            ...
            while (currInsn != null) {
                ...
                if (off > 0) {
                    ...
                    if (targ != null) prevInsn.setNext(targ);
                }
                prevInsn = currInsn;
