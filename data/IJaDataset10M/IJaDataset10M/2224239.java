...
public class GaussianMixtureDistribution implements RandomDistribution {
    ...
    public GaussianMixtureDistribution(int nbGaussians) {
        if (nbGaussians <= 0) throw new IllegalArgumentException("Argument must be strictly " + "positive");
        ...
        for (int i = 0; i < nbGaussians; i++) means[i] = (1. + 2. * (double) i) / (2. * (double) nbGaussians);
        Arrays.fill(variances, 1.);
        Arrays.fill(proportions, 1. / ((double) nbGaussians));
        for (int i = 0; i < distributions.length; i++) distributions[i] = new GaussianDistribution(means[i], variances[i]);
    }
    ...
    public GaussianMixtureDistribution(double[] means, double[] variances, double[] proportions) {
        if (means.length == 0 || means.length != variances.length || means.length != proportions.length) throw new IllegalArgumentException();
        distributions = new GaussianDistribution[means.length];
        this.proportions = new double[means.length];
        for (int i = 0; i < distributions.length; i++) distributions[i] = new GaussianDistribution(means[i], variances[i]);
        double sum = 0.;
        for (int i = 0; i < proportions.length; i++) sum += proportions[i];
        for (int i = 0; i < proportions.length; i++) this.proportions[i] = proportions[i] / sum;
    }
    ...
    public double generate() {
        ...
        for (int i = 0; i < proportions.length; i++) {
            sum += proportions[i];
            if (r <= sum) return distributions[i].generate();
        }
    ...
    public double probability(double n) {
        double sum = 0.;
        for (int i = 0; i < distributions.length; i++) sum += distributions[i].probability(n) * proportions[i];
        return sum;
    }
