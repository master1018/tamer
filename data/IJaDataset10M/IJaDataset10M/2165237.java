...
public class ServerDrivenNegotiation {
    ...
    public void setAcceptHeaders(List<String> headerValues) {
        ...
        for (String headerValue : headerValues) {
            ...
            if (requested == null) requested = mapping; else requested.putAll(mapping);
        }
    ...
    public Variant getBestMatch(List<Variant> available) {
        ...
        for (Variant option : available) {
            ...
            if (!applyMediaType(option, quality)) continue;
            if (!applyCharacterSet(option, quality)) continue;
            if (!applyEncoding(option, quality)) continue;
            if (!applyLanguage(option, quality)) continue;
        ...
        if (bestType != null && optionType != null) {
            if (bestType.getType().equals(optionType.getType())) {
                if (bestType.getSubtype().equals(optionType.getSubtype())) {
                    ...
                    if (optionCount > bestCount) return true; else if (optionCount < bestCount) return false;
    ...
    private boolean applyMediaType(Variant option, VariantQuality quality) {
        ...
        for (MediaType requested : requestedMediaTypes.keySet()) {
            ...
            if (type != null) {
                ...
                if (requestedType.equals(type)) ++matchCount; else if (!"*".equals(requestedType)) continue;
            }
            if (subtype != null) {
                String requestedSubtype = requested.getSubtype();
                if (requestedSubtype.equals(subtype)) ++matchCount; else if (!"*".equals(requestedSubtype)) continue;
            }
            if (parameters != null) {
                Map<String, String> requestedParameters = requested.getParameters();
                if (!hasRequiredParameters(requestedParameters, parameters)) continue;
            ...
            if (matchCount > bestMatchCount) {
                ...
                if (bestQuality.compareTo(qualityValue) < 0) bestQuality = qualityValue;
            }
        }
        if (!bestQuality.isAcceptable()) return false;
    ...
    private boolean hasRequiredParameters(Map<String, String> required, Map<String, String> available) {
        for (Entry<String, String> requiredEntry : required.entrySet()) {
            ...
            if (availableValue == null && "charset".equals(name)) {
                if (requestedCharacterSets != null && !requestedCharacterSets.containsKey(null) && !requestedCharacterSets.containsKey(value)) return false;
            } else if (!value.equals(availableValue)) return false;
        }
        return true;
    }

    private boolean applyCharacterSet(Variant option, VariantQuality quality) {
        if (requestedCharacterSets == null) return true;
        MediaType mediaType = option.getMediaType();
        if (mediaType == null) return true;
        String charsetParameter = mediaType.getParameters().get("charset");
        if (charsetParameter == null) return true;
        QualityValue value = requestedCharacterSets.get(charsetParameter);
        if (value == null) value = requestedCharacterSets.get(null);
