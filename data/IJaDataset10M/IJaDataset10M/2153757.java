...
public class EncoderUtil {
    ...
    public enum Encoding {
        ...
        B, /** The Q encoding (similar to quoted-printable defined in RFC 2045). */
        Q
    }
    ...
    public static String encodeIfNecessary(String text, Usage usage, int usedCharacters) {
        if (hasToBeEncoded(text, usedCharacters)) return encodeEncodedWord(text, usage, usedCharacters); else return text;
    }
    ...
    public static boolean hasToBeEncoded(String text, int usedCharacters) {
        ...
        if (usedCharacters < 0 || usedCharacters > MAX_USED_CHARACTERS) throw new IllegalArgumentException();
    ...
    public static String encodeEncodedWord(String text, Usage usage, int usedCharacters, Charset charset, Encoding encoding) {
        if (text == null) throw new IllegalArgumentException();
        if (usedCharacters < 0 || usedCharacters > MAX_USED_CHARACTERS) throw new IllegalArgumentException();
        if (charset == null) charset = determineCharset(text);
        String mimeCharset = CharsetUtil.toMimeCharset(charset.name());
        if (mimeCharset == null) {
            throw new IllegalArgumentException("Unsupported charset");
        }
        byte[] bytes = encode(text, charset);
        if (encoding == null) encoding = determineEncoding(bytes, usage);
    ...
    public static boolean isToken(String str) {
        ...
        for (int idx = 0; idx < length; idx++) {
            ...
            if (!TOKEN_CHARS.get(ch)) return false;
        }
        return true;
    }
    ...
    private static boolean isDotAtomText(String str) {
        ...
        for (int idx = 0; idx < length; idx++) {
            char ch = str.charAt(idx);
            if (ch == '.') {
                if (prev == '.' || idx == length - 1) return false;
            } else {
                if (!ATEXT_CHARS.get(ch)) return false;
            }
            prev = ch;
        }
        return true;
    ...
    private static Encoding determineEncoding(byte[] bytes, Usage usage) {
        if (bytes.length == 0) return Encoding.Q;
        BitSet qChars = usage == Usage.TEXT_TOKEN ? Q_REGULAR_CHARS : Q_RESTRICTED_CHARS;
        int qEncoded = 0;
