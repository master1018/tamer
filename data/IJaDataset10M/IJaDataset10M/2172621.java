...
public final class LocalTaskQueue implements LocalRpcService {
    ...
    private final Map<String, DevQueue> queues = Collections.synchronizedMap(new TreeMap<String, DevQueue>());
    ...
    public void init(LocalServiceContext context, Map<String, String> properties) {
        ...
        } else reader = new QueueXmlReader(this.localServerEnvironment.getAppDir().getPath());
    ...
    private void start_() {
        ...
        fetchService.start();
        UrlFetchJob.initialize(fetchService, this.localServerEnvironment, this.clock);
        ...
        if (this.queueXml != null) {
            for (QueueXml.Entry entry : this.queueXml.getEntries()) {
                this.queues.put(entry.getName(), new DevQueue(entry, this.taskNameGenerator, this.scheduler, baseUrl, this.clock));
            }
        }
    ...
    static String getBaseUrl(LocalServerEnvironment localServerEnvironment) {
        return String.format("http://%s:%d", new Object[] { localServerEnvironment.getAddress(), Integer.valueOf(localServerEnvironment.getPort()) });
    }
    ...
    static Scheduler startScheduler(boolean disableAutoTaskExecution) {
        try {
            ...
            if (!(disableAutoTaskExecution)) {
                System.out.println("scheduler not start, since this is the local version, " + " all jobs start immediately!");
            }
            return scheduler;
