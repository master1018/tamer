...
public class AdvSendFakeServerPacket extends AdminCommand {
    ...
    private void send(Player sender, final Player target, Packets packets) {
        ...
        for (final Packet packetTemplate : packets) {
            ...
            for (Part part : packetTemplate.getParts()) {
                ...
                if (value.indexOf("${objectId}") != -1) value = value.replace("${objectId}", targetObjectId);
                if (value.indexOf("${senderObjectId}") != -1) value = value.replace("${senderObjectId}", senderObjectId);
                if (value.indexOf("${targetObjectId}") != -1) value = value.replace("${targetObjectId}", targetObjectId);
                if (part.getRepeatCount() == 1) {
                    packet.addElement(byCode, value);
                } else {
                    for (int i = 0; i < part.getRepeatCount(); i++) packet.addElement(byCode, value);
                }
            }
    ...
    private Player getTargetPlayer(Player admin) {
        if (admin.getTarget() instanceof Player) return (Player) admin.getTarget(); else return admin;
    }
