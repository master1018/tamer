 ...
 * @author Ben
 */
public class BoardSelectionDialog extends JDialog implements ActionListener, IMapSettingsObserver, ListSelectionListener {
    ...
    private JLabel labBoardSize = new JLabel(Messages.getString("BoardSelectionDialog.BoardSize"), JLabel.RIGHT);

    private JLabel labBoardDivider = new JLabel("x", JLabel.CENTER);

    private JTextField texBoardWidth = new JTextField(2);

    private JTextField texBoardHeight = new JTextField(2);

    private JLabel labMapSize = new JLabel(Messages.getString("BoardSelectionDialog.MapSize"), JLabel.RIGHT);

    private JLabel labMapDivider = new JLabel("x", JLabel.CENTER);
    ...
    private JLabel labBoardsSelected = new JLabel(Messages.getString("BoardSelectionDialog.MapsSelected"), JLabel.CENTER);
    ...
    private JLabel labBoardsAvailable = new JLabel(Messages.getString("BoardSelectionDialog.mapsAvailable"), JLabel.CENTER);
    ...
    private JLabel labButtonSpace = new JLabel("", JLabel.CENTER);
    ...
    public BoardSelectionDialog(ClientGUI client) {
        super(client.frame, Messages.getString("BoardSelectionDialog.EditBoardLaout"), true);
        this.client = client;
        ...
        mapPreviewW = new JDialog(this.client.frame, Messages.getString("BoardSelectionDialog.MapPreview"), false);
        ...
        pack();
        setResizable(false);
        setLocation(client.frame.getLocation().x + client.frame.getSize().width / 2 - getSize().width / 2, client.frame.getLocation().y + client.frame.getSize().height / 2 - getSize().height / 2);
    }
    ...
    private void refreshBoardsSelected() {
        ...
        for (Enumeration i = mapSettings.getBoardsSelected(); i.hasMoreElements(); ) {
            ((DefaultListModel) lisBoardsSelected.getModel()).addElement(index++ + ": " + i.nextElement());
        }
    ...
    private void refreshSelectAllCheck() {
        ...
        if (chkSelectAll.isSelected() != newVal) chkSelectAll.setSelected(newVal);
    }
    ...
    private void change(String board) {
        ...
        for (final int newVar : selected) {
            String name = board;
            if (!MapSettings.BOARD_RANDOM.equals(name) && !MapSettings.BOARD_SURPRISE.equals(name) && chkRotateBoard.isSelected()) {
                name = Board.BOARD_REQUEST_ROTATION + name;
            }
            ((DefaultListModel) lisBoardsSelected.getModel()).setElementAt(newVar + ": " + name, newVar);
    ...
    private void apply() {
        ...
        refreshMapButtons();
        ((DefaultListModel) lisBoardsSelected.getModel()).removeAllElements();
        ((DefaultListModel) lisBoardsSelected.getModel()).addElement(Messages.getString("BoardSelectionDialog.Updating"));
        ((DefaultListModel) lisBoardsAvailable.getModel()).removeAllElements();
        ((DefaultListModel) lisBoardsAvailable.getModel()).addElement(Messages.getString("BoardSelectionDialog.Updating"));
     ...
     */
    private void send() {
        if (!texBoardWidth.getText().equals(Integer.toString(mapSettings.getBoardWidth())) || !texBoardHeight.getText().equals(Integer.toString(mapSettings.getBoardHeight())) || !texMapWidth.getText().equals(Integer.toString(mapSettings.getMapWidth())) || !texMapHeight.getText().equals(Integer.toString(mapSettings.getMapHeight()))) {
