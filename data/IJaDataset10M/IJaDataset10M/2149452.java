...
public abstract class RDFTreeTagNode extends RDFComponent implements TreeTagNode {
    ...
    public static void setUnEditedRecursively(RDFTreeTagNode rttn) {
        ...
        while (it.hasNext()) setUnEditedRecursively((RDFTreeTagNode) it.next());
    }

    /** Recusively retrieves all relevant models from the trees nodes and adds them
     *  to the set given as first argument.
     */
    public static void getAllRelevantModelsRecursively(Set set, RDFTreeTagNode rttn) {
        set.addAll(rttn.getComponentManager().getLoadedRelevantContainers());
        Iterator it = rttn.getChildren().iterator();
        while (it.hasNext()) getAllRelevantModelsRecursively(set, (RDFTreeTagNode) it.next());
    }
    ...
    private void removeFromModel(RDFModel model, boolean recursively) {
        for (Iterator childs = new Vector(getChildren()).iterator(); childs.hasNext(); ) {
            ...
            mirror.remove(ttn);
            if (recursively) ttn.removeFromModel(model, recursively);
        }
        Resource object = model.getResource(getURI());
    ...
    public boolean recursivelyRemoveChild(MutableTreeNode ttn) {
        ...
        while (en.hasMoreElements()) if (((TreeTagNode) en.nextElement()).recursivelyRemoveChild(ttn)) return true;
        return false;
    }
    ...
    public void getChildren(Vector collect, int visible, Class restrictedToType) {
    ...
    protected RDFModel getCurrentModel() {
        if (((RDFContainerManager) rcm.getContainerManager()).getTreeTagNodeLoadContainerDomination()) return getLoadModel(); else return (RDFModel) rcm.getCurrentLayoutContainer();
    }
