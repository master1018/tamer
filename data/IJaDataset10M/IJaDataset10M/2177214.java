 ...
 *
 * @author	BitWise
 * @version	$Id: Interval.java 369 2009-01-13 10:15:21Z andrew_jacobs $
 * @since	TFP 1.0
 */
public final class Interval implements Serializable {
    ...
    public boolean isMultipleOf(final Interval other) {
        int value = 0;
        if ((multiplier == 1) && (period == Period.TERM) && (other.multiplier >= 1)) return (true);
        if ((multiplier > 0) && (other.multiplier == 1) && (other.period == Period.DAY)) return (true);
        if (period == other.period) value = multiplier; else if ((period == Period.WEEK) && (other.period == Period.DAY)) value = 7 * multiplier; else if ((period == Period.YEAR) && (other.period == Period.MONTH)) value = 12 * multiplier;
    ...
    public boolean dividesDates(Date first, Date last) {
        ...
        if (period == Period.TERM) return (multiplier == 1);
    ...
    public Interval plus(Interval other) {
        ...
        if (period == other.period) return (new Interval(multiplier + other.multiplier, period));
        if ((period == Period.YEAR) && (other.period == Period.MONTH)) return (new Interval(12 * multiplier + other.multiplier, Period.MONTH));
        if ((period == Period.MONTH) && (other.period == Period.YEAR)) return (new Interval(multiplier + 12 * other.multiplier, Period.MONTH));
        if ((period == Period.WEEK) && (other.period == Period.DAY)) return (new Interval(7 * multiplier + other.multiplier, Period.DAY));
        if ((period == Period.DAY) && (other.period == Period.WEEK)) return (new Interval(multiplier + 7 * other.multiplier, Period.DAY));
    ...
    public boolean equals(final Interval other) {
        if (period == other.period) return (multiplier == other.multiplier);
        if (period == Period.YEAR) {
            if (other.period == Period.MONTH) return ((multiplier * 12) == other.multiplier);
            return (false);
        }
        if (period == Period.MONTH) {
            if (other.period == Period.YEAR) return (multiplier == (other.multiplier * 12));
            return (false);
        }
        if (period == Period.WEEK) {
            if (other.period == Period.DAY) return ((multiplier * 7) == other.multiplier);
            return (false);
        }
        if (period == Period.DAY) {
            if (other.period == Period.WEEK) return (multiplier == (other.multiplier * 7));
            return (false);
        }
        return (false);
    }
