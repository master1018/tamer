...
public class Inval implements PeerCommandSend {
    ...
    public void run() {
        if (frame.getFull()) {
            ...
            peer.sendFrame(new ProtocolControlFrame(fullFrame.getDestCallNo(), false, fullFrame.getSrcCallNo(), fullFrame.getTimestamp(), fullFrame.getIseqno(), fullFrame.getOseqno(), false, ProtocolControlFrame.INVAL_SC));
        } else {
            MiniFrame miniFrame = (MiniFrame) frame;
            peer.sendFrame(new ProtocolControlFrame(0, false, miniFrame.getSrcCallNo(), miniFrame.getTimestamp(), 0, 0, false, ProtocolControlFrame.INVAL_SC));
        }
    }
}
