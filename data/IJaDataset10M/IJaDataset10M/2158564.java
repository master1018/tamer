...
public class Syllable {
    ...
    public Object clone() {
        ...
        if (focusStressed) syl.setFocusStressed();
        if (wordStressed) syl.setWordStressed();
    ...
    public void centralize() {
        for (int i = 0; i < phonemes.size(); i++) {
            ...
            pho.centralize();
            if (pho.getDur() < Constants.frameSize) {
                phonemes.remove(pho);
                i--;
            }
        }
    ...
    public void changeF0Range(int rate, double mean) {
        for (Iterator iter = phonemes.iterator(); iter.hasNext(); ) {
            Phoneme element = (Phoneme) iter.next();
            if (element.hasF0Description()) element.changeF0Range(rate, mean);
        }
    }
    ...
    public void addJitter(int rate) {
        for (Iterator iter = phonemes.iterator(); iter.hasNext(); ) {
            Phoneme element = (Phoneme) iter.next();
            if (element.hasF0Description()) element.addJitter(rate);
        }
    }
    ...
    public void changeContour(String contour, int gradient) {
        ...
        if (contour.compareTo(Constants.CONTOUR_STRAIGHT) == 0) {
            ...
            for (int phoi = 0; phoi < phonemes.size(); phoi++) {
                Phoneme p = (Phoneme) phonemes.elementAt(phoi);
                if (p.hasF0Description()) {
                    end = start - ((int) Util.gradient2Hz(start, p.getDur(), gradient) - start);
                    p.interpolateF0Vec(start, end);
                    start = end;
                }
            }
    ...
    public void changeVariability(int rate) {
        for (Iterator iter = phonemes.iterator(); iter.hasNext(); ) {
            Phoneme element = (Phoneme) iter.next();
            if (element.hasF0Description()) element.changeF0Range(rate, this.getF0Mean());
        }
    }
    ...
    public double getF0Mean() {
        ...
        if (num > 0) return sum / num; else return 0;
    }
    ...
    public String printStress() {
        if (isFocusStressed()) return Constants.FOCUS_STRESS;
        if (isWordStressed()) return Constants.WORD_STRESS; else return Constants.UNSTRESS;
    }
