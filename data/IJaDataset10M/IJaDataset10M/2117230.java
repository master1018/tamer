...
public class ToyStorableGenerator<S extends Storable> {
     ...
     */
    public static <S extends Storable> Class<? extends S> getGeneratedClass(Class<S> type) throws SupportException {
        synchronized (cCache) {
    ...
    private ToyStorableGenerator(Class<S> type) throws SupportException {
        mStorableType = type;
        EnumSet<MasterFeature> features = EnumSet.of(MasterFeature.VERSIONING, MasterFeature.INSERT_SEQUENCES);
        final Class<? extends S> abstractClass = MasterStorableGenerator.getAbstractClass(mStorableType, features);
        mClassInjector = ClassInjector.create(mStorableType.getName(), abstractClass.getClassLoader());
    ...
    private Class<? extends S> generateAndInjectClass() {
        ...
        generateDelegatedMethod(MasterStorableGenerator.DO_TRY_LOAD_MASTER_METHOD_NAME, "doTryLoad");
        generateDelegatedMethod(MasterStorableGenerator.DO_TRY_INSERT_MASTER_METHOD_NAME, "doTryInsert");
        generateDelegatedMethod(MasterStorableGenerator.DO_TRY_UPDATE_MASTER_METHOD_NAME, "doTryUpdate");
        generateDelegatedMethod(MasterStorableGenerator.DO_TRY_DELETE_MASTER_METHOD_NAME, "doTryDelete");
    ...
    private void generateDelegatedMethod(String masterMethodName, String supportMethodName) {
        ...
        MethodInfo mi = mClassFile.addMethod(Modifiers.PROTECTED, masterMethodName, TypeDesc.BOOLEAN, null);
        CodeBuilder b = new CodeBuilder(mi);
        b.loadThis();
