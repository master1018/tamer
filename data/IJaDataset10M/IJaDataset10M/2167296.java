...
/**
 * Serialize an instanceof CallAction into XML file
 */
public class CallActionSerializer implements Serializer {

    private CallAction obj;

    private String oid;

    private Element element;

    public CallActionSerializer(CallAction obj) {
        this.obj = obj;
        this.oid = null;
        this.element = null;
    }

    public CallActionSerializer(CallAction obj, Element element, String oid) {
        this.obj = obj;
    ...
    public void addToElement(Element root, XmlWriteObjectRepository objRep) {
        ActionSerializer sserializer = new ActionSerializer(obj);
        sserializer.addToElement(root, objRep);
        root.setAttribute("oid", objRep.getOid(obj));
        root.setAttribute("classname", "CallAction");
        Element attr = null;
        Text val = null;
        if (obj.getOperation() != null) {
            attr = objRep.createElement("attribute");
            attr.setAttribute("name", "operation");
            attr.setAttribute("type", "objectref");
            val = objRep.createTextNode(objRep.getOid(obj.getOperation()));
            attr.appendChild(val);
            root.appendChild(attr);
            Operation operation = (Operation) obj.getOperation();
            OperationSerializer operationSerializer = new OperationSerializer(operation);
            objRep.getSerializers().add(operationSerializer);
        }
    }
    ...
    public void loadFromElement(XmlReadObjectRepository objRep) {
        ActionSerializer sserializer = new ActionSerializer(obj, element, oid);
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (el.getTagName() != null && el.getTagName().equalsIgnoreCase("attribute")) {
                ...
                if (name.equals("operation")) {
                    OperationSerializer s = new OperationSerializer(null, null, val);
                    if (obj.getOperation() == null) {
                        obj.setOperation((Operation) s.createFromObjectRepository(objRep));
                    }
                }
            }
        }
    ...
    public Object createFromObjectRepository(XmlReadObjectRepository objRep) {
        if (oid != null) obj = (CallAction) objRep.findObject(oid);
        if (obj != null) return obj;
        if (element == null) {
            if (oid == null || oid.length() == 0) {
                element = objRep.findClassElement("CallAction");
            } else {
                element = objRep.findElement(oid);
            }
        }
        if (element != null) {
            ...
            CallActionSerializer serializer = new CallActionSerializer(obj, element, oid);
    ...
    public static Object readXml(java.io.InputStream in, java.util.Map parameters) {
        ...
        Serializer objSer = new CallActionSerializer(null, null, "");
