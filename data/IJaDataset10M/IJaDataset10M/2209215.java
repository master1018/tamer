...
public class MinimalHTMLWriterTest extends SwingTestCase {
    ...
    private static class TestMinimalHTMLWriter extends MinimalHTMLWriter {
        ...
        public TestMinimalHTMLWriter(final Writer w, final StyledDocument doc, final int pos, final int len) {
            super(w, doc, pos, len);
        }
    ...
    public void testWriteAttributes() throws Exception {
        ...
        if (isHarmony()) {
            assertTrue("  font-style: italic;~  font-family: serif;~".equals(out.toString()) ^ "  font-family: serif;~  font-style: italic;~".equals(out.toString()));
        } else {
            assertTrue("  family:serif;~  italic:italic;~".equals(out.toString()) ^ "  italic:italic;~  family:serif;~".equals(out.toString()));
        }
    }
    ...
    public void testStartFontTag() throws Exception {
        ...
        assertEquals("  <font style=\"italic\">~    <font style=\"bold\">~", out.toString());
    }
    ...
    public void testEndFontTag() throws Exception {
        ...
        assertEquals("  <font style=\"italic\">~~  </font>~", out.toString());
    }

    public void testWriteBody() throws Exception {
        writer = new TestMinimalHTMLWriter(out, doc) {

            protected void writeContent(final Element elem, final boolean needsIndenting) throws IOException, BadLocationException {
                if (needsIndenting) {
                    indent();
                }
                write("_content_");
        ...
        setupWriter();
        writer.writeBody();
        assertEquals("  <body>~    <p class=default>~" + "      _content__content__content_~    </p>~" + "    <p class=myStylea>~      _content__leaf__leaf_~" + "    </p>~  </body>~", out.toString());
        out = new StringWriter();
        Element e = doc.getParagraphElement(0).getElement(1);
        writer = new TestMinimalHTMLWriter(out, doc, e.getStartOffset(), e.getEndOffset() - e.getStartOffset());
        setupWriter();
        writer.writeBody();
        assertEquals("  <body>~    <p class=default>~      <i>italic text</i>~" + "    </p>~  </body>~", out.toString());
    }

    public void testWriteContent() throws Exception {
        writer.writeContent(doc.getCharacterElement(0), true);
        writer.writeContent(doc.getCharacterElement(0), true);
        assertEquals("  <b>bold text   bold text ", out.toString());
        out = new StringWriter();
        createWriter();
        writer.writeContent(doc.getCharacterElement(0), false);
        writer.writeContent(doc.getParagraphElement(doc.getLength() - 1).getElement(0), false);
        assertEquals("<b>bold text </b>  <span style=\"color: #00ff00\">~" + " more green text", out.toString());
    }
    ...
    public void testWriteHeader() throws Exception {
        ...
        assertEquals("  <head>~    <style type=\"text/css\">~" + "      <!--~        ...styles...~" + "      -->~    </style>~  </head>~", result);
    }
    ...
    public void testWriteNonHTMLAttributes() throws Exception {
        ...
        int indentLevel = writer.getIndentLevel();
        writer.writeNonHTMLAttributes(attrs);
        assertEquals("  <span style=\"color: #ff0000\">~", out.toString());
        assertTrue(writer.inFontTag());
        assertEquals(indentLevel + 1, writer.getIndentLevel());
    ...
    public void testWriteEndParagraph() throws Exception {
        ...
        writer.writeHTMLTags(attrs);
        writer.writeEndParagraph();
        assertEquals("~</p>~  <span style=\"font-family: serif\">~<i></i>" + "~  </span>~</p>~", out.toString());
    }
    ...
    public void testWriteStyles() throws Exception {
        ...
        assertEquals("  p.myStyle {~    _attributes_~  }~" + "  p.myStyle {~    _attributes_~  }~", output);
    }
    ...
    private void createDocument() throws Exception {
        ...
        doc.insertString(doc.getLength(), "green text\n more green text", colorStyle);
        ...
        attrs.addAttribute(AbstractDocument.ElementNameAttribute, StyleConstants.ComponentElementName);
