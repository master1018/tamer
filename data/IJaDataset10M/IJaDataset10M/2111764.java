...
public class BenchmarkMatrixMatrixMultTransAB {
    ...
    public static long mult(DenseMatrix64F matA, DenseMatrix64F matB, DenseMatrix64F matResult, int numTrials) {
    ...
    public static long multSmall(DenseMatrix64F matA, DenseMatrix64F matB, DenseMatrix64F matResult, int numTrials) {
        long prev = System.currentTimeMillis();
        for (int i = 0; i < numTrials; i++) {
    ...
    public static long multAux(DenseMatrix64F matA, DenseMatrix64F matB, DenseMatrix64F matResult, int numTrials) {
        long prev = System.currentTimeMillis();
        for (int i = 0; i < numTrials; i++) {
    ...
    public static void performTests(int numRows, int numCols, int numK, int numTrials) {
        ...
        System.out.printf("Mult: %7d  Small %7d  Aux %7d\n", mult(matA, matB, matResult, numTrials), multSmall(matA, matB, matResult, numTrials), multAux(matA, matB, matResult, numTrials));
        System.gc();
    }
