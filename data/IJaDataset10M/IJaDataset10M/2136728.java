...
final class Client extends Thread {
    ...
    Client(IO client) {
        ...
        this.client = client;
        if (!(client instanceof IOMSB)) swap = true; else swap = false;
        ...
        try {
            while (true) {
                ...
                switch(reqType) {
                    ...
                    case 112:
                        length--;
                        if (data == AllTemporary || data == RetainPermanent || data == RetainTemporary) {
                            closeDownMode = data;
                        } else {
                            errorValue = data;
                            errorReason = 2;
                ...
                if (errorReason != 0) {
                    ...
                    cevent.mkError(errorReason, errorValue, (reqType <= 127 ? 0 : data), reqType);
    ...
    private static final void reqGrabServer(Client c) throws IOException {
        ...
        while (true) {
            ...
            for (i = 1; i < clients.length; i++) {
                if (i == c.index || clients[i] == null || !clients[i].isAlive() || clients[i].waitforreq || clients[i].suspended) continue;
                i = 0;
                break;
            }
            if (i != 0) break;
    ...
    static final void closeDownRetainedResources() {
        synchronized (LOCK) {
            for (int i = 1; i < clients.length; i++) {
                Client c = clients[i];
                if (c != null && (c.closeDownMode == RetainTemporary) && (c.clientGone)) {
                    c.closeDown();
                }
            }
        }
    ...
    final int sendEvent(Event event, int count, int mask, int filter, Grab grab) throws java.io.IOException {
