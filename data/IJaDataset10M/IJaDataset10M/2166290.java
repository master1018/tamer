...
public final class Check extends Proc {
   ...
   */
    ...
    protected boolean exec(final PrintOutput out) {
        ...
        final Proc p = MetaData.found(path, db, context.prop) ? new Open(db) : new CreateDB(path);
   ...
   */
    public static Data check(final Context ctx, final String path) throws IOException {
        final IO f = IO.get(path);
        final String db = f.dbname();
        return MetaData.found(path, db, ctx.prop) ? Open.open(ctx, db) : CreateDB.xml(ctx, f, db);
    }
}
