...
public class ServiceLookupFacilityImpl implements ServiceLookupFacility {
    ...
    public ReceivableService getReceivableService(ServiceID serviceID) throws NullPointerException, UnrecognizedServiceException, FacilityException {
        if (serviceID == null) {
            throw new NullPointerException("null ServiceID");
        }
        ServiceComponent serviceComponent = container.getComponentRepository().getComponentByID(serviceID);
	 ...
	 * @return
	 */
    private ReceivableService createReceivableService(ServiceComponent serviceComponent) {
        ...
        for (SbbID sbbID : serviceComponent.getSbbIDs(componentRepository)) {
            ...
            for (EventEntryDescriptor eventEntry : sbbComponent.getDescriptor().getEventEntries().values()) {
                ...
                if (allowedEventTypes == null || allowedEventTypes.contains(eventTypeID)) {
                    ReceivableEventImpl receivableEventImpl = new ReceivableEventImpl(eventTypeID, eventEntry.getResourceOption(), eventEntry.isInitialEvent());
