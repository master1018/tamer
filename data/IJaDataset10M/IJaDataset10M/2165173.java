...
public class TElement implements Transportable {
    ...
    public TElement(uka.transport.UnmarshalStream _stream) throws java.io.IOException, ClassNotFoundException {
        this(_stream, _SIZE);
        _stream.accept(_SIZE);
    }

    protected TElement(uka.transport.UnmarshalStream _stream, int _size) throws java.io.IOException, ClassNotFoundException {
    ...
    protected void marshalReferences(uka.transport.MarshalStream _stream) throws java.io.IOException {
        ...
        for (int i = 0; i < _nbelements; i++) _stream.writeObject(new TElement((Element) content.getChildren().get(i)));
    }
