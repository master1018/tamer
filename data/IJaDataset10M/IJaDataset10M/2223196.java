...
public class Node implements Cloneable {
    ...
    private String toString(int level) {
        ...
        for (int i = 0; i < level - 2; i++) bfr.append(' ');
        if (level >= 2) bfr.append("\\-");
        bfr.append(value).append('\n');
        if (left != null) bfr.append(left.toString(level + 2));
        if (right != null) bfr.append(right.toString(level + 2));
        return bfr.toString();
    }
    ...
    public Node deepestLeft() {
        ...
        while (true) {
            Node n2 = n.left;
            if (n2 == null) n2 = n.right;
            if (n2 == null) break; else n = n2;
        }
        return n;
    }
    ...
    private Node(int id, Node left, Node right) {
        ...
        this.left = left;
        this.right = right;
        if (left != null) alphabet.addAll(left.alphabet);
        if (right != null) alphabet.addAll(right.alphabet);
    }
    ...
    public static Node createIntervalNode(char c1, char c2) {
        ...
        for (char c = c1; c <= c2; c++) n.alphabet.add(c);
        StringBuffer bfr = new StringBuffer("[");
        for (Integer i : n.alphabet) bfr.append((char) i.intValue());
        bfr.append("]");
    ...
    public static Node createComplementNode(Node n) {
        ...
        if (!n.alphabet.contains('\t')) nn.alphabet.add('\t');
        if (!n.alphabet.contains('\n')) nn.alphabet.add('\n');
        if (!n.alphabet.contains('\r')) nn.alphabet.add('\r');
        if (!n.alphabet.contains(' ')) nn.alphabet.add(' ');
        for (char c = 32; c <= 126; c++) if (!n.alphabet.contains(c)) nn.alphabet.add(c);
        for (char c = 161; c <= 255; c++) if (!n.alphabet.contains(c)) nn.alphabet.add(c);
        StringBuffer bfr = new StringBuffer("[");
        for (Integer i : nn.alphabet) bfr.append((char) i.intValue());
        bfr.append("]");
    ...
    public static Node createAllNode() {
        Node n = new Node(CHAR);
        n.alphabet.add('\t');
        for (char c = 32; c <= 126; c++) n.alphabet.add(c);
        for (char c = 161; c <= 255; c++) n.alphabet.add(c);
        StringBuffer bfr = new StringBuffer("[");
        for (Integer i : n.alphabet) bfr.append((char) i.intValue());
        bfr.append("]");
        n.value = bfr.toString();
    ...
    public Object clone() {
        try {
            ...
            newMe.context = -1;
            newMe.end = -1;
            if (left != null) newMe.left = (Node) left.clone();
            if (right != null) newMe.right = (Node) right.clone();
            return newMe;
