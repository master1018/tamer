 ...
 */
public class FirstWizardPage extends JPanel implements WizardPage, DocumentListener {

    public static final String FIRST_PAGE_IDENTIFIER = "FirstPageIdentifier";

    private JPanel labelsPanel = new JPanel();

    private JLabel infoTitle = new JLabel(Resources.getString("rssAccountInfoTitle"));

    private JPanel infoTitlePanel = new JPanel(new FlowLayout(FlowLayout.CENTER));

    private JLabel existingAccountLabel = new JLabel(Resources.getString("rssAccountExist"));

    private JPanel existingAccountPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));

    private JTextArea accountInfoArea = new JTextArea(Resources.getString("rssAccountInfo"));

    private JLabel accountInfoAttentionArea = new JLabel(Resources.getString("rssAccountAttention"));

    private JPanel accountInfoAttentionPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
     ...
     * be added
     */
    public FirstWizardPage(RssAccountRegistration registration, WizardContainer wizardContainer) {
        ...
        this.infoTitle.setFont(infoTitle.getFont().deriveFont(Font.BOLD, 14.0f));
        this.infoTitlePanel.add(infoTitle);
        this.labelsPanel.add(infoTitlePanel);
        this.accountInfoAttentionArea.setFont(infoTitle.getFont().deriveFont(Font.BOLD, 14.0f));
    ...
    public void pageShowing() {
        if (isExistingAccount("RSS")) setNextButtonAccordingToUserID(false); else setNextButtonAccordingToUserID(true);
    }
    ...
    private boolean isExistingAccount(String userID) {
        ProtocolProviderFactory factory = RssAccRegWizzActivator.getRssProtocolProviderFactory();
