...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener, ClipboardOwner {
    ...
    public void showHistogram(ImagePlus imp, ImageStatistics stats) {
        ...
        if (limitToThreshold && histogram.length == 256) {
            ...
            if (ip.getMinThreshold() != ImageProcessor.NO_THRESHOLD) {
                ...
                for (int i = 0; i < lower; i++) histogram[i] = 0;
                for (int i = upper + 1; i < 256; i++) histogram[i] = 0;
            }
        }
        lut = imp.createLut();
        ...
        if (color) ip = ip.convertToRGB();
        drawHistogram(imp, ip, fixedRange, stats.histMin, stats.histMax);
        if (color) this.imp.setProcessor(null, ip); else this.imp.updateAndDraw();
    }
    ...
    public void mouseMoved(int x, int y) {
        if (value == null || count == null) return;
    ...
    void drawHistogram(ImagePlus imp, ImageProcessor ip, boolean fixedRange, double xMin, double xMax) {
        ...
        if (imp == null) lut.drawUnscaledColorBar(ip, x - 1, y, 256, BAR_HEIGHT); else drawAlignedColorBar(imp, xMin, xMax, ip, x - 1, y, 256, BAR_HEIGHT);
        y += BAR_HEIGHT + 15;
    ...
    void drawAlignedColorBar(ImagePlus imp, double xMin, double xMax, ImageProcessor ip, int x, int y, int width, int height) {
        ...
        } else pixels = new float[width * height];
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) pixels[i + width * j] = (float) (xMin + i * (xMax - xMin) / (width - 1));
        }
        if (!(ipSource instanceof ColorProcessor)) {
        ...
        if (ip instanceof ColorProcessor) bar = ipRamp.convertToRGB(); else bar = ipRamp.convertToByte(true);
        ip.insert(bar, x, y);
        ip.setColor(Color.black);
    ...
    int scaleDown(ImageProcessor ip, double threshold) {
        ...
        if (max > min) return (int) (((threshold - min) / (max - min)) * 255.0); else return 0;
    }
    ...
    void drawText(ImageProcessor ip, int x, int y, boolean fixedRange) {
        ...
        if (fixedRange && !cal.calibrated() && hmin == 0 && hmax == 255) range = 256;
    ...
    String d2s(double d) {
        if (d == Double.MAX_VALUE || d == -Double.MAX_VALUE) return "0"; else if (Double.isNaN(d)) return ("NaN"); else if (Double.isInfinite(d)) return ("Infinity"); else if ((int) d == d) return ResultsTable.d2s(d, 0); else return ResultsTable.d2s(d, decimalPlaces);
    }
    ...
    protected void showList() {
        ...
        if (cal.calibrated() && !cal.isSigned16Bit()) {
            for (int i = 0; i < stats.nBins; i++) sb.append(i + "\t" + ResultsTable.d2s(cal.getCValue(stats.histMin + i * stats.binSize), digits) + "\t" + histogram[i] + "\n");
            TextWindow tw = new TextWindow(getTitle(), "level\t" + vheading + "\tcount", sb.toString(), 200, 400);
        } else {
            for (int i = 0; i < stats.nBins; i++) sb.append(ResultsTable.d2s(cal.getCValue(stats.histMin + i * stats.binSize), digits) + "\t" + histogram[i] + "\n");
    ...
    protected void copyToClipboard() {
        ...
        for (int i = 0; i < stats.nBins; i++) pw.print(ResultsTable.d2s(cal.getCValue(stats.histMin + i * stats.binSize), digits) + "\t" + histogram[i] + "\n");
    ...
    void replot() {
        ...
        if (logScale) {
            drawLogPlot(yMax > 0 ? yMax : newMaxCount, ip);
            drawPlot(yMax > 0 ? yMax : newMaxCount, ip);
        } else drawPlot(yMax > 0 ? yMax : newMaxCount, ip);
        this.imp.updateAndDraw();
    }

    public void actionPerformed(ActionEvent e) {
        Object b = e.getSource();
        if (b == list) showList(); else if (b == copy) copyToClipboard(); else if (b == log) replot();
    }
    ...
    public double[] getXValues() {
        ...
        for (int i = 0; i < stats.nBins; i++) values[i] = cal.getCValue(stats.histMin + i * stats.binSize);
        return values;
    }
}
