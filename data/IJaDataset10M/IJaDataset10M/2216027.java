...
public final class OPT_Instruction implements VM_Constants, OPT_Operators, OPT_Constants {
        ...
        switch(getOpcode()) {
            ...
            default:
                if (MIR_Branch.conforms(this)) {
                    ...
                    throw new OPT_OptimizingCompilerException("getBranchTarget()", "operator not implemented", operator.toString());
                }
        }
    }
            ...
            default:
                if (MIR_Branch.conforms(this)) {
                    ...
                    for (int i = 0; i < MIR_LowTableSwitch.getNumberOfTargets(this); i++) {
                        e.addPossiblyDuplicateElement(MIR_LowTableSwitch.getTarget(this, i).target.getBasicBlock());
                    }
                } else if (MIR_CondBranch2.conforms(this)) {
                    throw new OPT_OptimizingCompilerException("getBranchTargets()", "operator not implemented", operator().toString());
                } else {
                    throw new OPT_OptimizingCompilerException("getBranchTargets()", "operator not implemented", operator().toString());
                }
        }
   ...
   */
    ...
    private abstract static class BASE_OE implements OPT_OperandEnumeration {
        ...
        public final OPT_Operand nextElement() {
            return next();
        }
