...
public class TranspTextWizardPage extends WizardPage implements Listener {
    ...
    public TranspTextWizardPage() {
        super(Messages.TranspTextWizardPage_textwizardtitle, "", null);
    ...
    public final void createControl(final Composite parent) {
        ...
        pageComposite.setLayoutData(pageCompositeLayoutData);
        {
            ...
            group2.setText(Messages.TranspTextWizardPage_text);
            {
                ...
                {
                    analysisText = new Text(textfieldComp, SWT.MULTI | SWT.WRAP | SWT.V_SCROLL | SWT.BORDER);
                    ...
                    analysisText.addModifyListener(new ModifyListener() {
                        ...
                        public void modifyText(ModifyEvent evt) {
                            textFieldChanged(previousText, analysisText.getText());
            ...
            }
            {
                ...
                {
                    chooseEditorButton = new Button(editorChooseComp, SWT.PUSH | SWT.CENTER);
                    GridData chooseEditorButtonLData = new GridData();
                    chooseEditorButton.setLayoutData(chooseEditorButtonLData);
                    chooseEditorButton.setText(Messages.TranspTextWizardPage_loadtextfromeditor);
                    chooseEditorButton.addSelectionListener(new SelectionAdapter() {

                        public void widgetSelected(SelectionEvent evt) {
                            if (editorRefs != null && editorRefs.size() >= editorChooser.getItemCount()) {
                                IEditorPart editor = editorRefs.get(editorChooser.getSelectionIndex()).getEditor(false);
                                if (editor != null) {
                                    InputStream input = EditorsManager.getInstance().getContentInputStream(editor);
                ...
                {
                    bottonOwnText = new Button(editorChooseComp, SWT.PUSH | SWT.CENTER);
                    GridData bottonOwnTextLData = new GridData();
                    bottonOwnText.setLayoutData(bottonOwnTextLData);
                    bottonOwnText.setText(Messages.TranspTextWizardPage_inputowntext);
        ...
        PlatformUI.getWorkbench().getHelpSystem().setHelp(getControl(), TranspositionAnalysisPlugin.PLUGIN_ID + ".wizard");
    ...
    private void preview() {
        ...
        transpositionTable1.setReadInOrder(directionChooserIn.getInput().getContent());
        transpositionTable1.setText(chosenText, blockL, !getCrop(), getCropsize());
    }
