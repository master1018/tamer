...
public class MXPElement implements Cloneable {
    ...
    public MXPElement(String newName, String theDefinition, String theAttributes, String theFlag, int theBitmap) {
        ...
        flag = theFlag;
        bitmap = theBitmap;
        if ((!isCommand()) && (theDefinition.toUpperCase().indexOf("&TEXT;") >= 0)) bitmap = bitmap | BIT_NEEDTEXT;
    }
    ...
    public Vector getUnsupportedParms() {
        if ((unsupportedParms == null) || (unsupportedParms.trim().length() == 0)) return new Vector();
    ...
    public void setAttributeValue(String tag, String value) {
        ...
        if (value != null) attributeValues.put(tag, value);
    }
    ...
    public synchronized Vector getParsedAttributes() {
        ...
        while ((++i) < buf.length()) {
            switch(buf.charAt(i)) {
                ...
                case '\t':
                    if (quotes == '\0') {
                        if ((!firstEqual) && (bit.length() > 0)) parsedAttributes.addElement(bit.toString().toUpperCase().trim());
                        bit = new StringBuffer("");
                        firstEqual = false;
                    } else bit.append(buf.charAt(i));
                    break;
                case '"':
                case '\'':
                    if (lastC == '\\') bit.append(buf.charAt(i)); else if ((lastC == '=') || (quotes != '\0') || ((quotes == '\0') && ((lastC == ' ') || (lastC == '\t')))) {
                        if ((quotes != '\0') && (quotes == buf.charAt(i))) {
                            ...
                            if (quotes != '\0') bit.append(buf.charAt(i)); else quotes = buf.charAt(i);
                        }
                    } else bit.append(buf.charAt(i));
                    break;
                default:
    ...
    public Vector getCloseTags(String desc) {
        ...
        while ((++i) < buf.length()) {
            switch(buf.charAt(i)) {
                case '<':
                    if (quotes != '\0') bit = null; else if (bit != null) {
                        if (MXP.tagDebug) {
                    ...
                    break;
                case '>':
                    if ((quotes == '\0') && (bit != null) && (bit.toString().trim().length() > 0)) tags.add(bit.toString().toUpperCase().trim());
                    bit = null;
                    break;
                case ' ':
                case '\t':
                ...
                case '"':
                case '\'':
                    if (lastC == '\\') bit = null; else if ((quotes != '\0') && (quotes == buf.charAt(i))) quotes = '\0'; else if (quotes == '\0') quotes = buf.charAt(i);
                    bit = null;
                    break;
                default:
                    if ((bit != null) && (Character.isLetterOrDigit(buf.charAt(i)))) bit.append(buf.charAt(i)); else bit = null;
                    break;
            }
            lastC = buf.charAt(i);
        }
