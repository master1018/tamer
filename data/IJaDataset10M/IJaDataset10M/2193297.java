...
public class DataMapDefaults extends _DataMapDefaults {
    ...
    public void updateSuperclassPackage(DataMap dataMap, boolean isClient) {
        ...
        if (Util.isEmptyString(storedPackage) || DEFAULT_SUPERCLASS_PACKAGE_SUFFIX.equals(storedPackage)) {
            String mapPackage = (isClient) ? dataMap.getDefaultClientPackage() : dataMap.getDefaultPackage();
            if (!Util.isEmptyString(mapPackage)) {
                if (mapPackage.endsWith(".")) {
                    mapPackage = mapPackage.substring(mapPackage.length() - 1);
                }
                if (!Util.isEmptyString(mapPackage)) {
                    String newPackage = mapPackage + "." + DEFAULT_SUPERCLASS_PACKAGE_SUFFIX;
    ...
    public void setPersistenceState(int persistenceState) {
        if (this.persistenceState == PersistenceState.TRANSIENT && persistenceState == PersistenceState.NEW) {
