...
public abstract class GPNodeBuilder implements Prototype {
    ...
    public Object clone() {
        try {
            ...
            if (sizeDistribution != null) c.sizeDistribution = (float[]) (sizeDistribution.clone());
            return c;
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (state.parameters.exists(base.push(P_MINSIZE), def.push(P_MINSIZE))) {
            if (!(state.parameters.exists(base.push(P_MAXSIZE), def.push(P_MAXSIZE)))) state.output.fatal("This GPNodeBuilder has a " + P_MINSIZE + " but not a " + P_MAXSIZE + ".");
            minSize = state.parameters.getInt(base.push(P_MINSIZE), def.push(P_MINSIZE), 1);
            if (minSize == 0) state.output.fatal("The GPNodeBuilder must have a min size >= 1.", base.push(P_MINSIZE), def.push(P_MINSIZE));
            maxSize = state.parameters.getInt(base.push(P_MAXSIZE), def.push(P_MAXSIZE), 1);
            if (maxSize == 0) state.output.fatal("The GPNodeBuilder must have a max size >= 1.", base.push(P_MAXSIZE), def.push(P_MAXSIZE));
            if (minSize > maxSize) state.output.fatal("The GPNodeBuilder must have min size <= max size.", base.push(P_MINSIZE), def.push(P_MINSIZE));
        } else if (state.parameters.exists(base.push(P_MAXSIZE), def.push(P_MAXSIZE))) state.output.fatal("This GPNodeBuilder has a " + P_MAXSIZE + " but not a " + P_MINSIZE + ".", base.push(P_MAXSIZE), def.push(P_MAXSIZE)); else if (state.parameters.exists(base.push(P_NUMSIZES), def.push(P_NUMSIZES))) {
            int siz = state.parameters.getInt(base.push(P_NUMSIZES), def.push(P_NUMSIZES), 1);
            if (siz == 0) state.output.fatal("The number of sizes in the GPNodeBuilder's distribution must be >= 1. ");
            sizeDistribution = new float[siz];
            if (state.parameters.exists(base.push(P_SIZE).push("0"), def.push(P_SIZE).push("0"))) state.output.warning("GPNodeBuilder does not use size #0 in the distribution", base.push(P_SIZE).push("0"), def.push(P_SIZE).push("0"));
            float sum = 0.0f;
            for (int x = 0; x < siz; x++) {
                sizeDistribution[x] = state.parameters.getFloat(base.push(P_SIZE).push("" + (x + 1)), def.push(P_SIZE).push("" + (x + 1)), 0.0f);
            ...
            if (sum > 1.0) state.output.warning("Distribution sums to greater than 1.0", base.push(P_SIZE), def.push(P_SIZE));
            if (sum == 0.0) state.output.fatal("Distribution is all 0's", base.push(P_SIZE), def.push(P_SIZE));
            RandomChoice.organizeDistribution(sizeDistribution);
        }
    }

    public abstract GPNode newRootedTree(final EvolutionState state, final GPType type, final int thread, final GPNodeParent parent, final GPFunctionSet set, final int argposition, final int requestedSize);
    ...
    protected void warnAboutNoTerminalWithType(GPType type, boolean fail, EvolutionState state) {
        ...
        if (fail) state.output.fatal("" + this.getClass() + " can't find a terminal type-compatable with " + type + " and cannot replace it with a nonterminal.  You may need to try a different node-builder algorithm."); else state.output.warnOnce("" + this.getClass() + " can't find a terminal type-compatable with " + type);
    }
    ...
    protected void warnAboutNonTerminalWithType(GPType type, boolean fail, EvolutionState state) {
        ...
        if (fail) state.output.fatal("" + this.getClass() + " can't find a non-terminal type-compatable with " + type + " and cannot replace it with a terminal.  You may need to try a different node-builder algorithm."); else state.output.warnOnce("" + this.getClass() + " can't find a non-terminal type-compatable with " + type);
    }
