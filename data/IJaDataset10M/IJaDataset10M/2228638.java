...
public class ZipFileIndex {
    ...
    * Returns a list of all ZipFileIndex entries
    * 
    * @param openedOnly If true it returns a list of only opened ZipFileIndex
    * entries, otherwise all ZipFileEntry(s) are included into the list.
    * @return A list of ZipFileIndex entries, or an empty list
    */
    ...
    */
    ...
    private boolean isUpToDate() {
        if (zipFile != null && ((!NON_BATCH_MODE) || zipFileLastModified == zipFile.lastModified()) && hasPopulatedData) {
            return true;
        }
        return false;
    }
    ...
    */
    ...
    public byte[] read(String path) throws IOException {
        ...
        try {
            ZipFileIndexEntry entry = getZipIndexEntry(path);
            if (entry == null) throw new FileNotFoundException(MessageFormat.format("Path not found in ZIP: {0}", path));
            return read(entry);
        } finally {
            lock.unlock();
        }
    ...
    */
    ...
    private byte[] getHeader(ZipFileIndexEntry entry) throws IOException {
        ...
        if (get4ByteLittleEndian(header, 0) != 0x04034b50) throw new ZipException("corrupted zip file");
        if ((get2ByteLittleEndian(header, 6) & 1) != 0) throw new ZipException("encrypted zip file");
        return header;
    }
    ...
    private class ZipDirectory {
        ...
        private void findCENRecord(long start, long end) throws IOException {
            ...
            while (endbufend >= 22) {
                ...
                int i = endbuflen - 22;
                while (i >= 0 && !(endbuf[i] == 0x50 && endbuf[i + 1] == 0x4b && endbuf[i + 2] == 0x05 && endbuf[i + 3] == 0x06 && endbufpos + i + 22 + get2ByteLittleEndian(endbuf, i + 20) == totalLength)) {
                    i--;
                }
                if (i >= 0) {
        ...
        private int readEntry(int pos, List<ZipFileIndexEntry> entryList, Map<String, DirectoryEntry> directories) throws IOException {
            ...
            if (fileStart == dirStart) directory = ""; else if (lastDir != null && lastLen == fileStart - dirStart - 1) {
                int index = lastLen - 1;
            ...
            if (fileStart != fileEnd) {
                ZipFileIndexEntry entry = new ZipFileIndexEntry(directory, new String(zipDir, fileStart, fileEnd - fileStart, "UTF-8"));
            ...
            return pos + 46 + get2ByteLittleEndian(zipDir, pos + 28) + get2ByteLittleEndian(zipDir, pos + 30) + get2ByteLittleEndian(zipDir, pos + 32);
        }
    }
    ...
    */
    static class DirectoryEntry {
        ...
        private void initEntries() {
            ...
            if (!zipFileIndex.readFromIndex) {
                int from = -Arrays.binarySearch(zipFileIndex.entries, new ZipFileIndexEntry(dirName, ZipFileIndex.MIN_CHAR)) - 1;
                int to = -Arrays.binarySearch(zipFileIndex.entries, new ZipFileIndexEntry(dirName, MAX_CHAR)) - 1;
    ...
    private File getIndexFile() {
        if (zipIndexFile == null) {
            ...
            zipIndexFile = new File((preindexedCacheLocation == null ? "" : preindexedCacheLocation) + zipFile.getName() + ".index");
        }
        return zipIndexFile;
    }
