...
public class GaborPatterns {
    ...
    public static void addTo(double[] to, int w, int h, int dx, int dy, double[] pattern, int pw, int ph) {
        for (int y = 0; y < ph; y++) {
            int ty = (y + dy) % h;
            for (int x = 0; x < pw; x++) {
                int tx = (x + dx) % w;
    ...
    public static int interp(double mult, int i0, int i1) {
        return (int) (mult * i0 + (1 - mult) * i1);
    }

    static double gaborRot(double x, double y, double orientation, double frequency, double size, double aspect, double phase) {
    ...
    static double gabor(double x1, double y1, double frequency, double size, double aspect, double phase) {
        return Math.exp(-(x1 * x1 + aspect * y1 * y1) / (2 * size * size)) * Math.cos(frequency * x1 + phase);
    }
}
