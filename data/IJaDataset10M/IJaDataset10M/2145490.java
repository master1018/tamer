...
public class TextAreaMouseHandler extends MouseInputAdapter {
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        if (textArea.getBuffer().isLoading()) return;
        int x = evt.getX();
        int y = evt.getY();
    ...
    protected void doSingleClick(MouseEvent evt) {
        ...
        if (((control && ctrlForRectangularSelection) || textArea.isRectangularSelectionEnabled()) && textArea.isEditable()) {
            ...
            if (screenLine > textArea.getLastScreenLine()) screenLine = textArea.getLastScreenLine();
        ...
        if (evt.isShiftDown()) {
            ...
            if (!quickCopyDrag) textArea.moveCaretPosition(dragStart, false);
        ...
        if (!quickCopyDrag) textArea.moveCaretPosition(dragStart, false);
        if (!(textArea.isMultipleSelectionEnabled() || quickCopyDrag)) textArea.selectNone();
    }

    protected void doDoubleClick() {
        if (textArea.getLineLength(dragStartLine) == 0) return;
        String lineText = textArea.getLineText(dragStartLine);
        String noWordSep = textArea.getBuffer().getStringProperty("noWordSep");
        if (dragStartOffset == textArea.getLineLength(dragStartLine)) dragStartOffset--;
        ...
        if (textArea.isMultipleSelectionEnabled()) textArea.addToSelection(sel); else textArea.setSelection(sel);
        if (quickCopyDrag) quickCopyDrag = false;
    ...
    protected void doTripleClick() {
        ...
        if (dragStartLine == textArea.getLineCount() - 1) newCaret--;
    ...
    public void mouseDragged(MouseEvent evt) {
        ...
        if (textArea.isDragInProgress()) return;
        if (textArea.getBuffer().isLoading()) return;
        ...
        if (evt.getY() < 0) {
            ...
            if (textArea.lastLinePartial) delta--;
            textArea.setFirstLine(textArea.getFirstLine() + delta);
        }
        switch(clickCount) {
            case 1:
    ...
    private void doSingleDrag(MouseEvent evt) {
        ...
        int x = evt.getX();
        int y = evt.getY();
        if (y < 0) y = 0; else if (y >= painter.getHeight()) y = painter.getHeight() - 1;
        ...
        if (textArea.chunkCache.getLineInfo(textArea.getLastScreenLine()).lastSubregion) {
            ...
            if (x > dotLineWidth) {
                ...
                if (!painter.isBlockCaretEnabled() && !textArea.isOverwriteEnabled() && (x - textArea.getHorizontalOffset()) % textArea.charWidth > textArea.charWidth / 2) extraEndVirt++;
            }
        }
        ...
        if (quickCopyDrag) {
            ...
            if (dot != textArea.getCaretPosition()) textArea.moveCaretPosition(dot, false);
    ...
    private void doDoubleDrag(MouseEvent evt) {
        ...
        if (lineStart + offset == textArea.getCaretPosition()) return;
    ...
    private void doTripleDrag(MouseEvent evt) {
        ...
        if (dragStartLine > mouseLine) {
            ...
            if (offset == textArea.getLineEndOffset(mouseLine) - 1) mouse = offset; else mouse = textArea.getLineStartOffset(mouseLine);
        } else {
            mark = textArea.getLineStartOffset(dragStartLine);
            if (offset == textArea.getLineStartOffset(mouseLine)) mouse = offset; else if (offset == textArea.getLineEndOffset(mouseLine) - 1 && mouseLine != textArea.getLineCount() - 1) mouse = textArea.getLineEndOffset(mouseLine); else mouse = textArea.getLineEndOffset(mouseLine) - 1;
        }
        mouse = Math.min(textArea.getBuffer().getLength(), mouse);
        if (mouse == textArea.getCaretPosition()) return;
    ...
    public static boolean isMiddleButton(int modifiers) {
        if (OperatingSystem.isMacOS()) {
            if ((modifiers & InputEvent.BUTTON1_MASK) != 0) return (modifiers & InputEvent.ALT_MASK) != 0; else return (modifiers & InputEvent.BUTTON2_MASK) != 0;
        } else return (modifiers & InputEvent.BUTTON2_MASK) != 0;
    }
    ...
    public static boolean isRightButton(int modifiers) {
        if (OperatingSystem.isMacOS()) {
            if ((modifiers & InputEvent.BUTTON1_MASK) != 0) return (modifiers & InputEvent.CTRL_MASK) != 0; else return (modifiers & InputEvent.BUTTON3_MASK) != 0;
        } else return (modifiers & InputEvent.BUTTON3_MASK) != 0;
    }
