...
public class ClanVote extends StdCommand {
    ...
    public boolean execute(MOB mob, Vector commands, int metaFlags) throws java.io.IOException {
        ...
        if ((mob.getClanID() == null) || (mob.getClanID().equalsIgnoreCase("")) || (CMLib.clans().getClan(mob.getClanID()) == null)) {
            ...
            for (Enumeration e = C.votes(); e.hasMoreElements(); ) {
                ...
                if (((CV.function == Clan.FUNC_CLANASSIGN) && (C.allowedToDoThis(mob, Clan.FUNC_CLANVOTEASSIGN) >= 0)) || ((CV.function != Clan.FUNC_CLANASSIGN) && (C.allowedToDoThis(mob, Clan.FUNC_CLANVOTEOTHER) >= 0))) votesForYou.addElement(CV);
            }
            if (commands.size() < 2) {
                if (votesForYou.size() == 0) msg.append("Your " + C.typeName() + " does not have anything up for your vote."); else {
                    msg.append(" " + CMStrings.padRight("#", 3) + CMStrings.padRight("Status", 15) + "Command to execute\n\r");
                    for (int v = 0; v < votesForYou.size(); v++) {
                        ...
                        msg.append((ivoted ? "*" : " ") + CMStrings.padRight("" + (v + 1), 3) + CMStrings.padRight(((CV.voteStatus == Clan.VSTAT_STARTED) ? (votesCast + " votes cast") : (Clan.VSTAT_DESCS[CV.voteStatus])), 15) + CMStrings.padRight(CV.matter, 55) + "\n\r");
                    }
                ...
                if ((which >= 0) && (which < votesForYou.size())) CV = (Clan.ClanVote) votesForYou.elementAt(which);
                if (CV == null) msg.append("That vote does not exist.  Use CLANVOTE to see a list."); else {
                    int yeas = 0;
                    int nays = 0;
                    Boolean myVote = null;
                    if (CV.votes != null) for (int vs = 0; vs < CV.votes.size(); vs++) {
                        if (((String) CV.votes.elementAt(vs, 1)).equals(mob.Name())) myVote = (Boolean) CV.votes.elementAt(vs, 2);
                        if (((Boolean) CV.votes.elementAt(vs, 2)).booleanValue()) yeas++; else nays++;
                    }
                    msg.append("Vote       : " + (which + 1) + "\n\r");
                    msg.append("Started by : " + CV.voteStarter + "\n\r");
                    if (CV.voteStatus == Clan.VSTAT_STARTED) msg.append("Started on : " + CMLib.time().date2String(CV.voteStarted) + "\n\r"); else msg.append("Ended on   : " + CMLib.time().date2String(CV.voteStarted) + "\n\r");
                    ...
                    if ((CV.voteStatus == Clan.VSTAT_STARTED) && (myVote == null)) {
                        ...
                        if (myVote != null) enterWhat = ("to keep (" + (myVote.booleanValue() ? "Y" : "N") + ") ");
                        ...
                        if ((prompt.length() > 0) && (mob.session() != null)) {
                            ...
                            if (answer.length() > 0) switch(answer.toUpperCase().charAt(0)) {
                                case 'Y':
                                ...
                                case 'N':
                                    ...
                                    break;
                                case 'C':
                                    if ((mob.session() != null) && (mob.session().confirm("This will cancel this entire vote, are you sure (N/y)?", "N"))) {
                                        C.delVote(CV);
                        ...
                        if (numVotes <= (yeas + nays)) {
                            updateVote = true;
                            if (yeas <= nays) CV.voteStatus = Clan.VSTAT_FAILED; else {
                                ...
                                if (mob2.location() == null) {
                                    ...
                                    if (mob2.location() == null) mob2.setLocation(CMLib.map().getRandomRoom());
                                }
