...
public class RandomModule extends Module {
    ...
    public RandomModule(Point position) {
        super("Random", new IOPort[] { new IOPort(IOPort.NUMBER, IOPort.INPUT, IOPort.LEFT, new String[] { "Input", "(time)" }), new IOPort(IOPort.NUMBER, IOPort.INPUT, IOPort.LEFT, new String[] { "Noise", "(0.5)" }) }, new IOPort[] { new IOPort(IOPort.NUMBER, IOPort.OUTPUT, IOPort.RIGHT, new String[] { "Output" }) }, position);
        octaves = 3;
        amplitude = 1.0;
    ...
    public boolean edit(BFrame fr, Scene theScene) {
        ...
        ComponentsDialog dlg = new ComponentsDialog(fr, Translate.text("selectRandomProperties"), new Widget[] { ampField, octavesField }, new String[] { Translate.text("Amplitude"), Translate.text("Octaves") });
        if (!dlg.clickedOk()) return false;
