...
public class cuiControl {
    ...
    public static final int EVENT_KEYPRESS = 1;

    public static final int EVENT_MOUSEBTTNDOWN = 2;

    public static final int EVENT_MOUSEDBLDOWN = 4;

    public static final int EVENT_MOUSEMOVE = 8;

    public static final int EVENT_MOUSEMASK = 0xFFFFFFF1;

    public static final int EVENT_TIMEOUT = 16;

    public static final int EVENT_CANCEL = 20;

    public static final int EVENT_OK = 21;

    public static final int EVENT_REDRAW = 30;

    public static final int EVENT_SHUTDOWN = 99;

    protected static final byte FLAGS_NOMOUSEFOCUS = 1;

    protected static final byte FLAGS_NOKEYBOARDFOCUS = 2;

    protected static final byte FLAGS_OWNERFOCUS = 4;
    ...
    public void delete() {
        if (isFocus) focus.unFocus();
        if (next != null) next.delete();
        if (this == listTop) listTop = prev;
        if (prev != null) {
            prev.next = null;
            prev = null;
        }
    ...
    public cuiControl setFocus() {
        ...
        if (!isFocus) {
            if ((flags & FLAGS_OWNERFOCUS) != 0) {
                if (!owner.isFocus) owner.setFocus();
            } else {
                if (prevFocus != null) prevFocus.unFocus();
                focus = this;
                isFocus = true;
            }
        }
    ...
    public void nextFocus(cuiControl wrapFocus) {
        ...
        unFocus();
        if (ctrlFocus == null) ctrlFocus = wrapFocus;
    ...
    public static void eventDispatch(int mask, int timeOut) {
        ...
        while (input != 0) {
            if ((input & cui.INPUT_KEYBOARD) != 0) {
                ...
                if (focus != null && focus.event(EVENT_KEYPRESS, 0)) msg = 0; else msg = EVENT_KEYPRESS;
            } else if ((input & cui.INPUT_MOUSEBTTNDOWN) != 0) {
                input &= ~cui.INPUT_MOUSEBTTNDOWN;
                if ((cui.timeMouseEvent - prevMouseTime < 500) && (cui.xMouse == prevMouseX) && (cui.yMouse == prevMouseY)) msg = EVENT_MOUSEDBLDOWN; else msg = EVENT_MOUSEBTTNDOWN;
                prevMouseTime = cui.timeMouseEvent;
                prevMouseX = cui.xMouse;
                prevMouseY = cui.yMouse;
                if ((focus != null) && (cui.xMouse >= focus.x) && (cui.xMouse < focus.right) && (cui.yMouse >= focus.y) && (cui.yMouse < focus.bottom) && focus.event(msg, 0)) msg = 0;
            ...
            if (msg != 0) {
                for (cuiControl ctrlHandler = listTop; ctrlHandler != null; ctrlHandler = ctrlHandler.prev) {
                    if ((msg & EVENT_MOUSEMASK) == 0) {
                        if ((cui.xMouse >= ctrlHandler.x) && (cui.xMouse < ctrlHandler.right) && (cui.yMouse >= ctrlHandler.y) && (cui.yMouse < ctrlHandler.bottom)) {
                            if ((ctrlHandler.flags & FLAGS_OWNERFOCUS) == 0) {
                                if (!ctrlHandler.isFocus && ((ctrlHandler.flags & FLAGS_NOMOUSEFOCUS) == 0)) ctrlHandler.setFocus();
                            } else {
                                if (!ctrlHandler.owner.isFocus && ((ctrlHandler.owner.flags & FLAGS_NOMOUSEFOCUS) == 0)) ctrlHandler.owner.setFocus();
                            }
                            ctrlHandler.event(msg, 0);
                            break;
                        }
                    } else if ((focus != ctrlHandler) && ctrlHandler.event(msg, 0)) break;
                }
            }
        }
    }
