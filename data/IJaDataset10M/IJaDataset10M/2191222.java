...
class SchedulesByLoadFrame extends OperationsFrame implements java.beans.PropertyChangeListener {
    ...
    public void comboBoxActionPerformed(java.awt.event.ActionEvent ae) {
        if (ae.getSource() == typesComboBox) updateLoadComboBox();
        if (ae.getSource() == loadsComboBox) updateLocations();
    }
                ...
                for (int j = 0; j < spurs.size(); j++) {
                    ...
                    if (sch != null) {
                        ...
                        for (int k = 0; k < items.size(); k++) {
                            ...
                            if (item.getType().equals(type) && item.getLoad().equals(load) || item.getType().equals(type) && item.getLoad().equals("") || item.getType().equals(type) && item.getShip().equals(load) || item.getType().equals(type) && item.getShip().equals("")) {
                                addItemLeft(locationsPanel, new JLabel(spur.getName() + " (" + spur.getScheduleName() + ")"), 1, x);
                                String s = item.getType();
                                if (!item.getTrainScheduleId().equals("")) s = s + ", " + TrainScheduleManager.instance().getScheduleById(item.getTrainScheduleId()).getName();
                                if (!item.getRoad().equals("")) s = s + ", " + item.getRoad();
                                s = s + ", " + item.getLoad();
                                ...
                                if (item.getDestination() != null) addItemLeft(locationsPanel, new JLabel(item.getDestinationName() + " (" + item.getDestinationTrackName() + ")"), 4, x - 1);
                            }
                        }
                    }
                }
    ...
    public void propertyChange(java.beans.PropertyChangeEvent e) {
        ...
        if (e.getPropertyName().equals(CarTypes.CARTYPES_LENGTH_CHANGED_PROPERTY)) CarTypes.instance().updateComboBox(typesComboBox);
        if (e.getSource().getClass().equals(CarLoads.class)) CarLoads.instance().updateComboBox((String) typesComboBox.getSelectedItem(), loadsComboBox);
        if (e.getSource().getClass().equals(Schedule.class) || e.getSource().getClass().equals(LocationManager.class)) updateLocations();
    }
