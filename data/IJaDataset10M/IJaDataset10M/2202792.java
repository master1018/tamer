...
public class NetDummyMode extends DummyMode implements NetLobbyListener {
    ...
    public boolean onMove(GameEngine engine, int playerID) {
        if ((engine.ending == 0) && (engine.statc[0] == 0) && (engine.holdDisable == false) && (netIsNetPlay) && (!netIsWatch) && ((netNumSpectators > 0) || (netForceSendMovements))) {
            netSendField(engine);
            netSendStats(engine);
        }
        if ((engine.ending == 0) && (netIsNetPlay) && (!netIsWatch) && (engine.nowPieceObject != null) && ((netNumSpectators > 0) || (netForceSendMovements))) {
    ...
    protected int updateCursor(GameEngine engine, int maxCursor, int playerID) {
        ...
        if ((engine.ctrl.isMenuRepeatKey(Controller.BUTTON_UP) || engine.ctrl.isMenuRepeatKey(Controller.BUTTON_DOWN)) && netIsNetPlay && ((netNumSpectators > 0) || (netForceSendMovements))) {
    ...
    protected void netDrawPlayerName(GameEngine engine) {
        if ((netPlayerName != null) && (netPlayerName.length() > 0)) {
            ...
            owner.receiver.drawTTFDirectFont(engine, engine.playerID, owner.receiver.getFieldDisplayPositionX(engine, engine.playerID), owner.receiver.getFieldDisplayPositionY(engine, engine.playerID) - 20, name);
        }
    }
    ...
    protected boolean netSendPieceMovement(GameEngine engine, boolean forceSend) {
        if (((engine.nowPieceObject == null) && (netPrevPieceID != Piece.PIECE_NONE)) || (engine.manualLock)) {
            netPrevPieceID = Piece.PIECE_NONE;
            netLobby.netPlayerClient.send("game\tpiece\t" + netPrevPieceID + "\t" + netPrevPieceX + "\t" + netPrevPieceY + "\t" + netPrevPieceDir + "\t" + 0 + "\t" + engine.getSkin() + "\t" + false + "\n");
            return true;
        } else if ((engine.nowPieceObject.id != netPrevPieceID) || (engine.nowPieceX != netPrevPieceX) || (engine.nowPieceY != netPrevPieceY) || (engine.nowPieceObject.direction != netPrevPieceDir) || (forceSend)) {
            ...
            netLobby.netPlayerClient.send("game\tpiece\t" + netPrevPieceID + "\t" + x + "\t" + y + "\t" + netPrevPieceDir + "\t" + engine.nowPieceBottomY + "\t" + engine.ruleopt.pieceColor[netPrevPieceID] + "\t" + engine.getSkin() + "\t" + engine.nowPieceObject.big + "\n");
            return true;
        }
        return false;
    }
    ...
    protected void netRecvPieceMovement(GameEngine engine, String[] message) {
        ...
        if (id >= 0) {
            ...
            if ((engine.stat != GameEngine.STAT_EXCELLENT) && (engine.stat != GameEngine.STAT_GAMEOVER) && (engine.stat != GameEngine.STAT_RESULT)) {
        ...
        if (netIsNetRankingDisplayMode) {
            ...
            if (!netRankingNoDataFlag[d] && netRankingReady[d] && (netRankingPlace != null) && (netRankingPlace[d] != null)) {
                ...
                if (engine.ctrl.isPush(Controller.BUTTON_A)) {
                    engine.playSE("decide");
                    String strMsg = "spdownload\t" + NetUtil.urlEncode(netCurrentRoomInfo.ruleName) + "\t" + NetUtil.urlEncode(getName()) + "\t" + goaltype + "\t" + (netRankingView != 0) + "\t" + NetUtil.urlEncode(netRankingName[d].get(netRankingCursor[d])) + "\n";
            ...
            if (engine.ctrl.isPush(Controller.BUTTON_LEFT) || engine.ctrl.isPush(Controller.BUTTON_RIGHT)) {
                if (netRankingView == 0) netRankingView = 1; else netRankingView = 0;
    ...
    protected void netOnRenderNetPlayRanking(GameEngine engine, int playerID, EventReceiver receiver) {
        if (netIsNetRankingDisplayMode) {
            ...
            if (!netRankingNoDataFlag[d] && netRankingReady[d] && (netRankingPlace != null) && (netRankingPlace[d] != null)) {
                ...
                receiver.drawMenuFont(engine, playerID, 3, 1, ((d != 0) ? "DAILY" : "ALL-TIME") + " RANKING (" + (netRankingCursor[d] + 1) + "/" + netRankingPlace[d].size() + ")", EventReceiver.COLOR_GREEN);
                ...
                if ((netRankingCursor[d] >= 0) && (netRankingCursor[d] < netRankingDate[d].size())) {
                    ...
                    receiver.drawMenuFont(engine, playerID, 1, 26, "GAMERATE:" + ((gamerate == 0f) ? "UNKNOWN" : (100 * gamerate) + "%"), EventReceiver.COLOR_CYAN);
                }
                receiver.drawMenuFont(engine, playerID, 1, 27, "A(" + strBtnA + " KEY):DOWNLOAD\nB(" + strBtnB + " KEY):BACK LEFT/RIGHT:" + ((d == 0) ? "DAILY" : "ALL-TIME"), EventReceiver.COLOR_ORANGE);
            } else if (netRankingNoDataFlag[d]) {
                receiver.drawMenuFont(engine, playerID, 0, 1, "<<", EventReceiver.COLOR_ORANGE);
                receiver.drawMenuFont(engine, playerID, 38, 1, ">>", EventReceiver.COLOR_ORANGE);
                receiver.drawMenuFont(engine, playerID, 3, 1, ((d != 0) ? "DAILY" : "ALL-TIME") + " RANKING", EventReceiver.COLOR_GREEN);
                receiver.drawMenuFont(engine, playerID, 1, 3, "NO DATA", EventReceiver.COLOR_DARKBLUE);
                receiver.drawMenuFont(engine, playerID, 1, 28, "B(" + strBtnB + " KEY):BACK LEFT/RIGHT:" + ((d == 0) ? "DAILY" : "ALL-TIME"), EventReceiver.COLOR_ORANGE);
    ...
    protected void netEnterNetPlayRankingScreen(GameEngine engine, int playerID, int goaltype) {
        ...
        netLobby.netPlayerClient.send("spranking\t" + NetUtil.urlEncode(rule) + "\t" + NetUtil.urlEncode(getName()) + "\t" + goaltype + "\t" + false + "\n");
        netLobby.netPlayerClient.send("spranking\t" + NetUtil.urlEncode(rule) + "\t" + NetUtil.urlEncode(getName()) + "\t" + goaltype + "\t" + true + "\n");
    }
