 ...
 *
 */
    private static Record CustomStruct = null;
 ...
 */
    ...
    private static ObjectsCache CustomObjectsCache = null;
...
*/
    ...
    Record getRecordStruct() throws PDException {
        if (CustomStruct == null) CustomStruct = CreateRecordStruct();
...
* @return
*/
    private static synchronized Record CreateRecordStruct() throws PDException {
...
*/
    ...
    protected void VerifyAllowedIns() throws PDException {
        if (!getDrv().getUser().getName().equals("Install")) if (!getDrv().getUser().getRol().isAllowCreateCustom()) PDExceptionFunc.GenPDException("Customization_creation_not_allowed_to_user", getName());
    }

    protected void VerifyAllowedDel() throws PDException {
        if (!getDrv().getUser().getRol().isAllowMaintainCustom()) PDExceptionFunc.GenPDException("Customization_delete_not_allowed_to_user", getName());
    }

    protected void VerifyAllowedUpd() throws PDException {
        if (!getDrv().getUser().getRol().isAllowMaintainCustom()) PDExceptionFunc.GenPDException("Customization_modification_not_allowed_to_user", getName());
    }
 ...
 */
    protected ObjectsCache getObjCache() {
        if (CustomObjectsCache == null) CustomObjectsCache = new ObjectsCache("Custom");
