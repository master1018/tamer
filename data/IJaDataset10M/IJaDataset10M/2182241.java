...
final class BindingCacheEntry extends BasicCacheEntry<BindingKey, Long> {
    ...
    private static final LoggerWrapper logger = new LoggerWrapper(Logger.getLogger(BindingCacheEntry.class.getName()));
    ...
    private BindingCacheEntry(BindingKey key, long contextId, State state) {
        ...
        if (key == BindingKey.FIRST) {
            throw new IllegalArgumentException("The key must not be BindingKey.FIRST");
        }
    }
     ...
     */
    private BindingCacheEntry(BindingKey key, long value, long contextId, State state) {
     ...
     */
    static BindingCacheEntry createCached(BindingKey key, long contextId, long value, boolean forUpdate) {
        return new BindingCacheEntry(key, value, contextId, forUpdate ? State.CACHED_WRITE : State.CACHED_READ);
    }
    ...
    static BindingCacheEntry createLast(long contextId) {
        return new BindingCacheEntry(BindingKey.LAST, contextId, State.FETCHING_READ);
    }

    @Override
    public String toString() {
        return "BindingCacheEntry[" + "name:" + key + (!hasValue() ? "" : ", value:" + getValue()) + ", contextId:" + getContextId() + ", state:" + getState() + ", previousKey:" + previousKey + (previousKey == null ? "" : ", previousKeyUnbound:" + previousKeyUnbound) + ", pendingPrevious:" + pendingPrevious + "]";
    }
    ...
    void setValue(Long newValue) {
        if (newValue == null) {
            throw new NullPointerException("New value must not be null");
        } else if (key == BindingKey.LAST) {
            if (newValue != -1) {
                throw new IllegalStateException("New value must be -1 for LAST entry");
            }
            newValue = null;
        } else if (newValue < 0) {
            throw new IllegalArgumentException("New value must not be negative");
        }
     ...
     */
    void updatePreviousKey(BindingKey newPreviousKey, BindingState newPreviousKeyState) {
        assert newPreviousKeyState != null;
        if (newPreviousKey.compareTo(key) >= 0) {
            throw new IllegalArgumentException("New previous key is too large");
    ...
    void checkState(Cache cache, long lockTimeout) {
        ...
        synchronized (lock) {
            if (getDecached()) {
                throw new AssertionError("Decached binding entry found: " + this);
            }
            if (key == BindingKey.FIRST) {
                throw new AssertionError("No binding entry should be present for first key: " + this);
            }
            if (getState() == State.FETCHING_READ) {
                if (!pendingPrevious) {
                    throw new AssertionError("Binding entry that is FETCHING_READ should also be" + " pending previous: " + this);
                }
            } else if (getState() == State.FETCHING_UPGRADE) {
                throw new AssertionError("Binding entries should not be FETCHING_UPGRADE: " + this);
            } else if (getState() == State.FETCHING_WRITE) {
                throw new AssertionError("Binding entries should not be FETCHING_WRITE: " + this);
            } else if (getDecaching()) {
                if (pendingPrevious) {
                    throw new AssertionError("Binding entries that are being evicted should not" + " be pending previous: " + this);
                }
            } else if (getDowngrading()) {
                if (pendingPrevious) {
                    throw new AssertionError("Binding entries that are being downgraded should" + " not be pending previous: " + this);
                }
            }
            try {
                if (!awaitNotPendingPrevious(lock, System.currentTimeMillis() + lockTimeout)) {
                    return;
                }
            } catch (TransactionTimeoutException e) {
                if (logger.isLoggable(WARNING)) {
                    logger.log(WARNING, "Unable to check entry's previous key due to" + " timeout on pending previous:" + this);
                }
                return;
            }
            ...
            if (previousKey != null) {
                ...
                if (compareTo > 0) {
                    throw new AssertionError("Binding entry's previous key should not be lower" + " than previous entry's key: " + this + ", previous entry key: " + previousEntryKey);
                } else if (compareTo == 0 && previousKeyUnbound) {
                    throw new AssertionError("Binding entry should not record that the previous " + " key entry is unbound when that key is bound: " + this + ", previous entry key: " + previousEntryKey);
                }
            } else if (previousEntryKey.compareTo(key) >= 0) {
                throw new AssertionError("Binding entry's key should be greater than the previous" + " entry's key: " + this + ", previous entry key: " + previousEntryKey);
            }
        }
    }
    ...
    boolean isNextEntry(BindingKey forKey) {
        assert forKey != null;
        return forKey.compareTo(key) < 0 && previousKey != null && previousKey.compareTo(forKey) <= 0;
    }
    ...
    boolean awaitNotPendingPrevious(Object lock, long stop) {
        ...
        while (now < stop) {
            ...
            try {
                ...
                throw new TransactionInterruptedException("Interrupt while waiting for lock: " + this, e);
            }
            now = System.currentTimeMillis();
        }
        throw new TransactionTimeoutException("Timeout after " + (now - start) + " ms waiting for lock: " + this);
    }
    ...
    void setPendingPrevious() {
        if (pendingPrevious) {
            throw new IllegalStateException("Already pending previous: " + this);
        }
        assert !getUpgrading() && !getDowngrading() && !getDecaching() : "Setting binding entry pending previous while busy: " + this;
        assert key == BindingKey.LAST || !getReading() : "Setting non-last binding entry pending previous while" + " reading: " + this;
