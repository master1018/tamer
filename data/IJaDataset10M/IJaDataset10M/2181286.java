...
public class CalibreQueryInterpreter {
    ...
    private static final Logger logger = Logger.getLogger(CalibreQueryInterpreter.class);
    ...
    private static final String WORD_PUBLISHER = "publisher:";
    ...
    private BookFilter getFilterForNode(Tree node) throws CalibreSavedSearchInterpretException {
        ...
        if (template.toLowerCase().equals(WORD_TAG_TRUE)) {
            ...
            String tag = template.substring(WORD_TAG.length() + (isEqualsPresent ? 2 : 1), template.length() - 1);
            ...
            String lang = template.substring(WORD_LANGUAGE.length() + 2, template.length() - 1);
            ...
            char rating;
            if (comparator == '<' || comparator == '>' || comparator == '=') rating = parameter.charAt(1); else {
        ...
        if (template.toLowerCase().equals(WORD_AUTHOR_TRUE)) {
            ...
            String tag = template.substring(WORD_AUTHOR.length() + (isEqualsPresent ? 2 : 1), template.length() - 1);
        ...
        if (template.toLowerCase().equals(WORD_SERIES_TRUE)) {
            ...
            String tag = template.substring(WORD_SERIES.length() + (isEqualsPresent ? 2 : 1), template.length() - 1);
        ...
        if (template.toLowerCase().equals(WORD_FORMAT_TRUE)) {
            ...
            String tag = template.substring(WORD_FORMAT.length() + (isEqualsPresent ? 2 : 1), template.length() - 1);
        ...
        if (template.toLowerCase().equals(WORD_PUBLISHER_TRUE)) {
            ...
            String tag = template.substring(WORD_PUBLISHER.length() + (isEqualsPresent ? 2 : 1), template.length() - 1);
   ...
   */
    ...
    public static BookFilter interpret(String calibreSearchQueryOrName) throws CalibreSavedSearchInterpretException, CalibreSavedSearchNotFoundException {
        if (logger.isTraceEnabled()) logger.trace("CalibreQueryInterpreter.interpret:" + calibreSearchQueryOrName);
        ...
        if (Helper.isNotNullOrEmpty(calibreSearchQueryOrName)) {
            ...
            if (calibreSearchQueryOrName.toUpperCase(Locale.ENGLISH).startsWith("SAVED:")) {
                ...
                if (logger.isTraceEnabled()) logger.trace("searching for saved search " + calibreSavedSearchName);
                calibreQuery = DataModel.INSTANCE.getMapOfSavedSearches().get(calibreSavedSearchName);
                if (Helper.isNullOrEmpty(calibreQuery)) calibreQuery = DataModel.INSTANCE.getMapOfSavedSearches().get(calibreSavedSearchName.toUpperCase());
            }
            if (Helper.isNullOrEmpty(calibreQuery)) throw new CalibreSavedSearchNotFoundException(calibreSavedSearchName); else {
                if (logger.isDebugEnabled()) logger.trace("interpreting " + calibreQuery);
