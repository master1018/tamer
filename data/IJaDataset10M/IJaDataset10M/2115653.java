...
public class GridPolicy extends MainPolicy {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if ((configElement.getNodeName() == "simulationOutput")) {
                ...
                for (int j = 0; j < numberAttributes; j++) {
                    ...
                    } else if (configElement.getAttributes().item(j).getNodeName() == "nameOutputFile") {
                        setNameOutputFile(simulationConfig.getAttribute("nameOutputFile"));
                    } else {
                    }
                }
            }
            if ((configElement.getNodeName() == "imageSize")) {
                ...
                for (int j = 0; j < numberAttributes; j++) {
                    if (configElement.getAttributes().item(j).getNodeName() == "height") {
                        setSvgImageHeight(simulationConfig.getAttribute("height"));
                    } else if (configElement.getAttributes().item(j).getNodeName() == "width") {
            ...
            if ((configElement.getNodeName() == "gridConfig")) {
                Element simulationConfig = (Element) configDocument.getElementsByTagName("gridConfig").item(0);
                int numberAttributes = simulationConfig.getChildNodes().getLength();
                for (int j = 0; j < numberAttributes; j++) {
                    if (simulationConfig.getChildNodes().item(j).getNodeName() == "mainStyle") {
                        Element mainStyle = (Element) configDocument.getElementsByTagName("mainStyle").item(0);
                        int lengthMainStyle = mainStyle.getAttributes().getLength();
                        for (int a = 0; a < lengthMainStyle; a++) {
                            if (mainStyle.getAttributes().item(a).getNodeName() == "strokeColour") {
                                gridStrokeColour = mainStyle.getAttribute("strokeColour");
                            } else if (mainStyle.getAttributes().item(a).getNodeName() == "backgroundColour0") {
                                gridBackgroundColour0 = mainStyle.getAttribute("backgroundColour0");
                            } else if (mainStyle.getAttributes().item(a).getNodeName() == "backgroundColour1") {
                                gridBackgroundColour1 = mainStyle.getAttribute("backgroundColour1");
                            } else if (mainStyle.getAttributes().item(a).getNodeName() == "strokeWidth") {
                                gridLineStrength = Double.parseDouble(mainStyle.getAttribute("strokeWidth"));
                            } else if (mainStyle.getAttributes().item(a).getNodeName() == "cellHeight") {
                                cellHeight = Double.parseDouble(mainStyle.getAttribute("cellHeight"));
                            } else if (mainStyle.getAttributes().item(a).getNodeName() == "cellWidth") {
                                cellWidth = Double.parseDouble(mainStyle.getAttribute("cellWidth"));
                            } else {
                            }
                        }
                    } else if (simulationConfig.getChildNodes().item(j).getNodeName() == "textStyle") {
                        Element textStyle = (Element) configDocument.getElementsByTagName("textStyle").item(0);
                        int lengthTextStyle = textStyle.getAttributes().getLength();
                        for (int a = 0; a < lengthTextStyle; a++) {
                            if (textStyle.getAttributes().item(a).getNodeName() == "font-size") {
                                gridFontSize = Double.parseDouble(textStyle.getAttribute("font-size"));
                            } else if (textStyle.getAttributes().item(a).getNodeName() == "font-family") {
                                gridFontFamily = textStyle.getAttribute("font-family");
                            } else {
                            }
                        }
                    } else {
            ...
            if ((configElement.getNodeName() == "agentConfig")) {
                Element agentConfig = (Element) configDocument.getElementsByTagName("agentConfig").item(0);
                int numberAttributes = agentConfig.getChildNodes().getLength();
                for (int j = 0; j < numberAttributes; j++) {
                    if (agentConfig.getChildNodes().item(j).getNodeName() == "labelStyle") {
                        Element agentStyle = (Element) configDocument.getElementsByTagName("labelStyle").item(0);
                        int lengthAgentStyle = agentStyle.getAttributes().getLength();
                        for (int a = 0; a < lengthAgentStyle; a++) {
                            if (agentStyle.getAttributes().item(a).getNodeName() == "colour") {
                            } else if (agentStyle.getAttributes().item(a).getNodeName() == "textColour") {
                            } else {
                            }
                        }
                    }
                    if (agentConfig.getChildNodes().item(j).getNodeName() == "headStyle") {
                        Element agentStyle = (Element) configDocument.getElementsByTagName("headStyle").item(0);
                        int lengthAgentStyle = agentStyle.getAttributes().getLength();
                        for (int a = 0; a < lengthAgentStyle; a++) {
                            if (agentStyle.getAttributes().item(a).getNodeName() == "colour") {
                            } else if (agentStyle.getAttributes().item(a).getNodeName() == "textColour") {
    ...
    public void setGridStatistic(Map<String, String> myMap) {
        ...
        this.doc = grid.setGridStatistic(numberCellGridX, numberCellGridY, doc, myMap);
        double statisticWidth = grid.getStatisticWidth();
        widthGrid = statisticWidth;
    }

    public void drawGoldDigger(long id, long teamId, int x, int y, String rotation) {
    ...
    public void drawGoldDigger(long id, String teamId, int x, int y, String rotation) {
        Agent agent = new Agent();
        agent.setDimBoxHeight(cellHeight);
        agent.setDimBoxWidth(cellWidth);
        agent.setDimLine();
    ...
    public void drawStable(String id, int x, int y, String rotation, boolean team) {
        ItemsAndObstacles depot = new ItemsAndObstacles();
        depot.setDimBoxHeight(cellHeight);
        depot.setDimBoxWidth(cellWidth);
        if (!team) {
            doc = depot.makeRedStable(doc, "depot_0", x, y, setRedDepot, team, rotation);
            setRedDepot = true;
        } else {
            doc = depot.makeBlueStable(doc, "depot_1", x, y, setBlueDepot, team, rotation);
            setBlueDepot = true;
        }
    }
    ...
    public void drawStable(String id, String text, int x, int y, String rotation, boolean team) {
        ...
        if (!team) {
            ...
            doc = depot.makeBlueStable(doc, "depot_1", x, y, setRedDepot, team, rotation);
            setBlueDepot = true;
        }
    }
