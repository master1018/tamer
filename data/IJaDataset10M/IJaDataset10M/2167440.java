 ...
 * @author Markus Kilås
 * @version $Id: GenericProcessServlet.java 2177 2012-02-29 13:10:59Z malu9369 $
 */
public class GenericProcessServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        if (data.length > MAX_UPLOAD_SIZE) {
            ...
            res.sendError(HttpServletResponse.SC_REQUEST_ENTITY_TOO_LARGE, "Maximum content length is 100 MB");
        } else {
    ...
    private void processRequest(HttpServletRequest req, HttpServletResponse res, int workerId, byte[] data, String fileName, String pdfPassword) throws java.io.IOException, ServletException {
        ...
        logMap.put(IWorkerLogger.LOG_XFORWARDEDFOR, req.getHeader("X-Forwarded-For"));
        if (fileName != null) {
        ...
        try {
            response = (GenericServletResponse) getWorkerSession().process(workerId, new GenericServletRequest(requestId, data, req), context);
            if (response.getRequestID() != requestId) {
                LOG.error("Response ID " + response.getRequestID() + " not matching request ID " + requestId);
                res.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Request and response ID missmatch");
                return;
            }
