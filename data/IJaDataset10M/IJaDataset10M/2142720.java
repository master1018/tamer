...
public class PullStreamServiceClient {
    ...
    private static Logger logger = Logger.getLogger(PullStreamServiceClient.class.getName());

    private static final QName SERVICE_NAME = new QName("http://www.semsorgrid4env.eu/namespace/2009/10/SDS/Pull", "PullStreamService");
    ...
    public PullStreamServiceClient(String url) throws MalformedURLException {
        if (logger.isDebugEnabled()) logger.debug("ENTER PullStreamServiceClient() with URL " + url);
        _serviceUrl = url;
        _pullSource = createService(_serviceUrl);
        if (logger.isDebugEnabled()) logger.debug("RETURN PullStreamServiceClient()");
    }

    private PullStreamInterface createService(String url) throws MalformedURLException {
        if (logger.isTraceEnabled()) logger.trace("ENTER createService() with URL " + url);
        PullStreamInterface pullSource;
        try {
            URL wsdlURL = new URL(url);
            PullStreamService ss = new PullStreamService(wsdlURL, SERVICE_NAME);
        ...
        if (logger.isTraceEnabled()) logger.trace("RETURN createService()");
        return pullSource;
    }

    public GetResourceListResponse getResourceList(GetResourceListRequest request) throws NotAuthorizedFault, ServiceBusyFault {
        if (logger.isDebugEnabled()) logger.debug("ENTER getResourceList()");
    ...
    public PropertyDocumentType getPropertyDocument(GetDataResourcePropertyDocumentRequest request) throws SchemaMetadataException, InvalidResourceNameFault, DataResourceUnavailableFault, NotAuthorizedFault, ServiceBusyFault {
        ...
        try {
            propDoc = (PullStreamPropertyDocumentType) _pullSource.getDataResourcePropertyDocument(request);
    ...
    public GenericQueryResponse getStreamItems(GetStreamItemRequest request) throws SNEEDataSourceException {
        ...
        try {
            ...
            String msg = "Resource " + request.getDataResourceAbstractName() + " currently unavailable on " + SERVICE_NAME + ".";
            logger.warn(msg, e);
            throw new SNEEDataSourceException(msg, e);
        } catch (NotAuthorizedFault e) {
            ...
            logger.warn("Unable to connect to service " + _serviceUrl + " with resource " + request.getDataResourceAbstractName() + ". Carry on.");
        } finally {
            if (response == null) {
                logger.warn("Unexpected problem connecting to " + _serviceUrl + " when issuing GetStreamItems " + "call with parameters:" + "\n\tResource name: " + request.getDataResourceAbstractName() + "\n\tDataset Format: " + request.getDatasetFormatURI() + "\n\tPosition: " + request.getPosition() + "\n\tCount: " + request.getCount() + "\n\tMax Size: " + request.getMaximumTuples());
            }
        }
        if (logger.isTraceEnabled()) logger.trace("RETURN getStreamItemsFromService() with " + "response: " + response);
        return response;
    }

    public GenericQueryResponse getStreamNewestItem(GetStreamNewestItemRequest request) throws SNEEDataSourceException {
        if (logger.isTraceEnabled()) {
        ...
        if (logger.isDebugEnabled()) logger.debug("RETURN getStreamNewestItem() with response " + response);
        return response;
    }
}
