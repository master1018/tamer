...
public class RuleCrossoverPipeline extends BreedingPipeline {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (ruleCrossProbability > 1.0 || ruleCrossProbability < 0.0) state.output.fatal("Rule cross probability must be between 0 and 1", base.push(P_CROSSOVERPROB), def.push(P_CROSSOVERPROB));
    }
    ...
    public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds, final EvolutionState state, final int thread) {
        ...
        if (n < min) n = min;
        if (n > max) n = max;
        if (!state.random[thread].nextBoolean(likelihood)) return reproduce(n, start, subpopulation, inds, state, thread, true);
        ...
        for (int q = start; q < n + start; ) {
            if (sources[0] == sources[1]) {
                ...
                if (!(sources[0] instanceof BreedingPipeline)) parents[0] = (RuleIndividual) (parents[0].clone());
                if (!(sources[1] instanceof BreedingPipeline)) parents[1] = (RuleIndividual) (parents[1].clone());
            }
            ...
            if (parents[0].rulesets.length != parents[1].rulesets.length) {
                state.output.fatal("The number of rule sets should be identical in both parents ( " + parents[0].rulesets.length + " : " + parents[1].rulesets.length + " ).");
            }
            for (int x = 0; x < parents[0].rulesets.length; x++) {
                RuleSet[] temp = new RuleSet[2];
                while (true) {
                    for (int i = 0; i < 2; i++) temp[i] = new RuleSet();
                    ...
                    if (temp[0].numRules >= parents[0].rulesets[x].constraints(initializer).minSize && temp[0].numRules <= parents[0].rulesets[x].constraints(initializer).maxSize && temp[1].numRules >= parents[1].rulesets[x].constraints(initializer).minSize && temp[1].numRules <= parents[1].rulesets[x].constraints(initializer).maxSize) break;
                    temp = new RuleSet[2];
                }
