...
public class SBSegmentation implements Iterator<ArrayList<SBPendingVertical>> {
    ...
    private SBPendingVertical expandSBPendingVertical(SBPendingVertical lineIn) {
        ...
        if (!_pixelCompare.newSimilar(offset + lineIn.xMin) || !_pixelCompare.newSimilar(offset + lineIn.xMax)) return lineIn;
        int i_low;
    ...
    public void segmentAll(int color) {
        ...
        for (int y = _min_y; y <= _max_y; y++) {
            ...
            for (int x = _min_x; x <= _max_x; x++) {
                ...
                if (!_pixelCompare.isHandled(index) && _pixelCompare.similar(index)) {
                    segment(x, y, true);
                }
            }
        }
    ...
    public void segment(int x, int y, boolean useReferenceColor) {
        ...
        if (!useReferenceColor) effectiveColor = _pixelCompare.getColorAsInt(index);
        if (_segmentAreaFactory != null) _currentSegmentArea = _segmentAreaFactory.makePixelArea(x, y, effectiveColor);
    ...
    boolean atEdge(SBPendingVertical curLine) {
        if (curLine.y == _max_y && curLine.isSearchUp()) return true;
        if (curLine.y == _min_y && !curLine.isSearchUp()) return true;
        return false;
    }
    ...
    void handleLine(SBPendingVertical curLine) {
        ...
        int y = curLine.y;
        for (int i = curLine.xMin; i <= curLine.xMax; i++) {
            if (_pixelCompare.isHandled(offset + i)) continue;
            if (!_pixelCompare.isHandled(offset + i)) {
                _pixelCompare.action(offset + i);
                _currentArea++;
                _pixelCompare.setHandled(offset + i);
                if (_currentSegmentArea != null) _currentSegmentArea.putPixel(i, y, _pixelCompare.getColorAsInt(offset + i));
            }
        }
    }
    ...
    void handleNextLine(SBPendingVertical curLine) {
        if (atEdge(curLine)) return;
        boolean insideSimilar = false;
        int lowX = 0;
        int direction = -1;
        if (curLine.isSearchUp()) direction = 1;
        int yNew = curLine.y + direction;
        if (!(_min_y <= yNew && yNew <= _max_y)) return;
        int offset = offsetToLineStart(yNew);
        for (int i = curLine.xMin; i <= curLine.xMax; i++) {
            boolean curSimilar = _pixelCompare.newSimilar(offset + i);
            if (!insideSimilar && curSimilar) {
                lowX = i;
                insideSimilar = true;
            } else if (insideSimilar && !curSimilar) {
                SBPendingVertical newLine = new SBPendingVertical(lowX, i - 1, yNew, curLine.isSearchUp());
                storeLine(newLine);
                insideSimilar = false;
            }
        }
        if (insideSimilar) {
            SBPendingVertical newLine = new SBPendingVertical(lowX, curLine.xMax, yNew, curLine.isSearchUp());
            storeLine(newLine);
        }
    }
    ...
    public String getStatus() {
        if (_status == null || "".equals(_status)) _status = findStatus();
        return _status;
    }
    ...
    public String findStatus() {
        ...
        if (_segmentAreaFactory != null) {
            ...
            if (0 < areas) status += "\nPixels per area = " + _slImage.getPixelCount() / areas; else status += ", segmentation was not run.";
        }
        return status;
    }
    ...
    boolean checkLine(SBPendingVertical curLine) {
        ...
        for (int i = curLine.xMin; i <= curLine.xMax; i++) {
            if (_pixelCompare.similar(offset + i)) continue; else {
    ...
    void storeLine(SBPendingVertical curLine) {
        if (_slowTestMode && !checkLine(curLine)) checkLine(curLine);
