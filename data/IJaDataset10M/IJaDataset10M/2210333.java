...
public class IconSelectionPopupDialog extends JDialog implements KeyListener, MouseListener {
    ...
    public IconSelectionPopupDialog(JFrame caller, Vector icons, FreeMindMain freeMindMain) {
        ...
        if (numOfIcons <= xDimension * (xDimension - 1)) yDimension = xDimension - 1; else yDimension = xDimension;
    ...
    private int findIndexByKeyEvent(KeyEvent keyEvent) {
        for (int i = 0; i < icons.size(); i++) {
            ...
            if (iconKeyStroke != null && (keyEvent.getKeyCode() == iconKeyStroke.getKeyCode() && keyEvent.getKeyCode() != 0 && (iconKeyStroke.getModifiers() & KeyEvent.SHIFT_MASK) == (keyEvent.getModifiers() & KeyEvent.SHIFT_MASK) || keyEvent.getKeyChar() == iconKeyStroke.getKeyChar()) && keyEvent.getKeyChar() != 0 && keyEvent.getKeyChar() != KeyEvent.CHAR_UNDEFINED) {
                return i;
            }
        }
        return -1;
