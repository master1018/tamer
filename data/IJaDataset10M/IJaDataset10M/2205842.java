 ...
 */
public final class KeySignatureStamping extends Stamping {
    ...
    public KeySignatureStamping(TraditionalKey traditionalKey, int linePositionC4, int linePositionMin, float positionX, StaffStamping parentStaff, SymbolPool symbolPool, LayoutSettings layoutSettings) {
        super(parentStaff, Level.Music, null, createBoundingShape(traditionalKey, parentStaff, linePositionC4, linePositionMin, positionX, symbolPool, layoutSettings));
    ...
    private static Shape createBoundingShape(TraditionalKey traditionalKey, StaffStamping parentStaff, int linePositionC4, int linePositionMin, float positionX, SymbolPool symbolPool, LayoutSettings layoutSettings) {
        int fifth = traditionalKey.fifth;
        if (fifth == 0) return null;
        boolean useSharps = (fifth > 0);
        float distance = (useSharps ? layoutSettings.widthSharp : layoutSettings.widthFlat);
        Symbol symbol = symbolPool.getSymbol(useSharps ? CommonSymbol.AccidentalSharp : CommonSymbol.AccidentalFlat);
        ...
        for (int i = 0; i < fifth; i++) {
            ...
            bounds = bounds.move(positionX + i * distance * interlineSpace, parentStaff.computeYMm(linePosition));
            if (shape == null) shape = bounds; else shape = shape.extend(bounds);
        }
        return shape;
    }
