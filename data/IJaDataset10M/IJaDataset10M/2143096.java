...
public final class GoGuiSettings {
    ...
    public GoGuiSettings(String args[], Class c) throws ErrorMessage {
        ...
        String options[] = { "analyze-commands:", "auto", "command:", "computer-black", "computer-both", "computer-none", "computer-white", "config:", "gtpfile:", "help", "komi:", "laf:", "move:", "program:", "register", "size:", "time:", "verbose", "version" };
        ...
        if (analyzeCommandsFilename != null) m_analyzeCommands = new File(analyzeCommandsFilename);
        ...
        if (m_register && m_program == null) throw new ErrorMessage("Option -register can be used only with option -program");
        m_gtpFile = opt.get("gtpfile", "");
        m_gtpCommand = opt.get("command", "");
        if (opt.contains("komi")) m_prefs.putDouble("komi", opt.getDouble("komi"));
        m_lookAndFeel = opt.get("laf", null);
        m_move = opt.getInteger("move", -1);
        if (opt.contains("size")) m_prefs.putInt("boardsize", opt.getInteger("size"));
        m_time = opt.get("time", null);
        m_verbose = opt.contains("verbose");
        ArrayList<String> arguments = opt.getArguments();
        m_file = null;
        if (arguments.size() == 1) m_file = new File(arguments.get(0)); else if (arguments.size() > 1) throw new ErrorMessage("Only one argument allowed");
        validate();
    }
    ...
    private void validate() throws ErrorMessage {
        ...
        if (size < 1 || size > GoPoint.MAX_SIZE) throw new ErrorMessage("Invalid board size: " + size);
    }
}
