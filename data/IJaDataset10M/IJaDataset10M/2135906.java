...
public class Qedeq2UnicodeVisitor extends ControlVisitor implements ReferenceFinder {
     ...
     */
    public Qedeq2UnicodeVisitor(final Plugin plugin, final KernelQedeqBo prop, final boolean info, final int maximumColumn, final boolean addWarnings, final boolean brief) {
        super(plugin, prop);
        this.info = info;
     ...
     */
    public void generateUtf8(final AbstractOutput printer, final String language, final String level) throws SourceFileExceptionList, IOException {
     ...
     */
    public void setParameters(final AbstractOutput printer, final String language) {
    ...
    public void visitEnter(final Header header) {
        ...
        if (email != null && email.length() > 0) {
            ...
            printer.println();
            if ("de".equals(language)) {
                printer.println("Bei Fragen, Anregungen oder Bitte um Aufnahme in die Liste der" + " abhängigen Module schicken Sie bitte eine EMail an die Adresse " + email);
                printer.println();
                printer.println();
                ...
                printer.println("If you have any questions, suggestions or want to add something" + " to the list of modules that use this one, please send an email to the" + " address " + email);
                printer.println();
                printer.println();
    ...
    private void setFormula(final Formula f) {
        if (f != null && f.getElement() != null) {
            lineData.setFormula(getUtf8(f.getElement(), formulaWidth - tab.length()));
        }
    }
    ...
    public void visitEnter(final Universal r) throws ModuleDataException {
        String buffer = r.getName();
        if (r.getSubjectVariable() != null) {
            buffer += " with " + getQedeqBo().getElement2Utf8().getUtf8(r.getSubjectVariable());
        }
        if (r.getReference() != null) {
            buffer += " in " + getReference(r.getReference());
        }
    ...
    public void visitEnter(final Rule rule) {
        ...
        printer.println();
        printer.println((rule.getName() != null ? "  Name: " + rule.getName() : "") + (rule.getVersion() != null ? "  -  Version: " + rule.getVersion() : ""));
        printer.println();
    ...
    private String getLatexListEntry(final String method, final LatexList list) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (null != list.get(i) && null != list.get(i).getLatex() && list.get(i).getLatex().trim().length() > 0) {
                    return "MISSING! OTHER: " + getUtf8(list.get(i));
                }
            }
            return "MISSING!";
     ...
     */
    private ModuleContext getCurrentContext(final SourcePosition startDelta, final SourcePosition endDelta) {
        if (subContext.length() == 0) {
            return super.getCurrentContext();
        }
        final ModuleContext c = new ModuleContext(super.getCurrentContext().getModuleLocation(), super.getCurrentContext().getLocationWithinModule() + "." + subContext, startDelta, endDelta);
        return c;
    }
     ...
     *                      be <code>null</code>
     */
    public void addWarning(final int code, final String msg, final SourcePosition startDelta, final SourcePosition endDelta) {
        Trace.param(CLASS, this, "addWarning", "msg", msg);
        if (addWarnings) {
            addWarning(new UnicodeException(code, msg, getCurrentContext(startDelta, endDelta)));
        }
    }
    ...
    public String getReferenceLink(final String reference, final SourcePosition start, final SourcePosition end) {
        final Reference ref = getReference(reference, getCurrentContext(start, end), addWarnings, false);
        ...
        if (!ref.isExternal()) {
            return getNodeDisplay(ref.getNodeLabel(), ref.getNode()) + (ref.isSubReference() ? " (" + ref.getSubLabel() + ")" : "") + (ref.isProofLineReference() ? " (" + ref.getProofLineLabel() + ")" : "");
        }
        if (ref.isExternalModuleReference()) {
            return "[" + ref.getExternalQedeqLabel() + "]";
        }
        return getNodeDisplay(ref.getNodeLabel(), ref.getNode()) + (ref.isSubReference() ? " (" + ref.getSubLabel() + ")" : "") + (ref.isProofLineReference() ? " (" + ref.getProofLineLabel() + ")" : "") + (ref.isExternal() ? " [" + ref.getExternalQedeqLabel() + "]" : "");
    }
