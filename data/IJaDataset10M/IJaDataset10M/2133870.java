...
public class ResultSetTest extends DatabaseTestCase {
    ...
    public void testResultSetScroll3() throws Exception {
        ...
        rs.close();
        stmt2.close();
        pstmt = con.prepareStatement("SELECT data FROM #resultSetScroll3", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        assertTrue("Was expecting a ResultSet", pstmt.execute());
        rs = pstmt.getResultSet();
        assertEquals("ResultSet not scrollable", ResultSet.TYPE_SCROLL_INSENSITIVE, rs.getType());
        rs.close();
        pstmt.close();
        CallableStatement cstmt = con.prepareCall("{call #procResultSetScroll3}", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ...
    public void testResultSetMetaData() throws Exception {
        ...
        assertNotNull(rsmd);
        assertEquals("idx", rsmd.getColumnName(1));
    ...
    public void testCursorWarning() throws Exception {
        ...
        stmt.close();
        CallableStatement cstmt = con.prepareCall("{call #SPTESTCW(?)}", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    public void testCancelResultSet() throws Exception {
        ...
        for (int i = 1; i < 1000; i++) {
            stmt.executeUpdate("INSERT INTO #TEST VALUES (" + i + ", 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" + "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX')");
        }
    ...
    public void testEmptyInsertRow() throws Exception {
        int rows = 10;
        Statement stmt = con.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
        stmt.executeUpdate("create table #emptyInsertRow (id int identity, val int default 10)");
    ...
    public void testInsertRowVisible() throws Exception {
        ...
        stmt.executeUpdate("create table #insertRowNotVisible (val int primary key)");
    ...
    public void testUpdateRowDuplicatesRow() throws Exception {
        Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
        stmt.executeUpdate("create table #updateRowDuplicatesRow (val int primary key)");
        stmt.executeUpdate("insert into #updateRowDuplicatesRow (val) values (1)");
        stmt.executeUpdate("insert into #updateRowDuplicatesRow (val) values (2)");
        stmt.executeUpdate("insert into #updateRowDuplicatesRow (val) values (3)");
        ResultSet rs = stmt.executeQuery("select val from #updateRowDuplicatesRow order by val");
        for (int i = 0; i < 3; i++) {
            assertTrue(rs.next());
    ...
    public void testUpdateRowUpdatesRow() throws Exception {
        Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
        stmt.executeUpdate("create table #updateRowUpdatesRow (id int primary key, val int)");
        stmt.executeUpdate("insert into #updateRowUpdatesRow (id, val) values (1, 1)");
        stmt.executeUpdate("insert into #updateRowUpdatesRow (id, val) values (2, 2)");
        stmt.executeUpdate("insert into #updateRowUpdatesRow (id, val) values (3, 3)");
        ResultSet rs = stmt.executeQuery("select id, val from #updateRowUpdatesRow order by id");
        for (int i = 0; i < 3; i++) {
            assertTrue(rs.next());
            assertFalse(rs.rowUpdated());
            assertFalse(rs.rowInserted());
    ...
    public void testDeleteRowMarksDeleted() throws Exception {
        Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
        stmt.executeUpdate("create table #deleteRowMarksDeleted (val int primary key)");
        stmt.executeUpdate("insert into #deleteRowMarksDeleted (val) values (1)");
        stmt.executeUpdate("insert into #deleteRowMarksDeleted (val) values (2)");
        stmt.executeUpdate("insert into #deleteRowMarksDeleted (val) values (3)");
        ResultSet rs = stmt.executeQuery("select val from #deleteRowMarksDeleted order by val");
        for (int i = 0; i < 3; i++) {
            assertTrue(rs.next());
            assertFalse(rs.rowUpdated());
            assertFalse(rs.rowInserted());
    ...
    public void testCursorFetch() throws Exception {
        int rows = 10;
        Statement stmt = con.createStatement();
        stmt.executeUpdate("create table #testCursorFetch (id int primary key, val int)");
        stmt.close();
        PreparedStatement pstmt = con.prepareStatement("insert into #testCursorFetch (id, val) values (?, ?)");
        for (int i = 1; i <= rows; i++) {
            pstmt.setInt(1, i);
            pstmt.setInt(2, i);
            pstmt.executeUpdate();
        ...
        pstmt.close();
        CallableStatement cstmt = con.prepareCall("{?=call sp_cursoropen(?, ?, ?, ?, ?)}");
        cstmt.registerOutParameter(1, Types.INTEGER);
    ...
    public void testAbsoluteMinusOne() throws Exception {
        Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        stmt.executeUpdate("create table #absoluteMinusOne (val int primary key)");
        stmt.executeUpdate("insert into #absoluteMinusOne (val) values (1)");
        stmt.executeUpdate("insert into #absoluteMinusOne (val) values (2)");
        stmt.executeUpdate("insert into #absoluteMinusOne (val) values (3)");
        ResultSet rs = stmt.executeQuery("select val from #absoluteMinusOne order by val");
        rs.absolute(-1);
    ...
    public void testAbsoluteLargeValue() throws SQLException {
        Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        stmt.executeUpdate("create table #absoluteLargeValue (val int primary key)");
        stmt.executeUpdate("insert into #absoluteLargeValue (val) values (1)");
        stmt.executeUpdate("insert into #absoluteLargeValue (val) values (2)");
        stmt.executeUpdate("insert into #absoluteLargeValue (val) values (3)");
        ResultSet rs = stmt.executeQuery("select val from #absoluteLargeValue order by val");
    ...
    public void testRelativeLargeValue() throws SQLException {
        Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        stmt.executeUpdate("create table #relativeLargeValue (val int primary key)");
        stmt.executeUpdate("insert into #relativeLargeValue (val) values (1)");
        stmt.executeUpdate("insert into #relativeLargeValue (val) values (2)");
        stmt.executeUpdate("insert into #relativeLargeValue (val) values (3)");
        ResultSet rs = stmt.executeQuery("select val from #relativeLargeValue order by val");
    ...
    public void testCursorMaxRows() throws Exception {
        ...
        stmt.close();
        PreparedStatement pstmt = con.prepareStatement("insert into #cursorMaxRows (val) values (?)");
        for (int i = 0; i < 10; i++) {
            pstmt.setInt(1, i);
            assertEquals(1, pstmt.executeUpdate());
        }
    ...
    public void testSetObjectScale() throws Exception {
        ...
        PreparedStatement pstmt = con.prepareStatement("insert into #testsetobj values(?)");
    ...
    public void testCursorPrevious() throws Exception {
        ...
        stmt.close();
        PreparedStatement pstmt = con.prepareStatement("insert into #cursorPrevious (val) values (?)");
        for (int i = 0; i < 10; i++) {
            pstmt.setInt(1, i);
            assertEquals(1, pstmt.executeUpdate());
        }
    ...
    public void testUpdateableClientCursor() throws Exception {
        Statement stmt = con.createStatement();
        stmt.executeUpdate("create table #testUpdateableClientCursor " + "(i int primary key, v varchar(100))");
        stmt.executeUpdate("insert into #testUpdateableClientCursor " + "(i, v) values (1, 'This is a test')");
        stmt.close();
        ...
        pstmt.close();
        stmt = con.createStatement();
        rs = stmt.executeQuery("select * from #testUpdateableClientCursor");
        assertTrue(rs.next());
        assertEquals(1, rs.getInt(1));
    ...
    public void testDistinctBug() throws Exception {
        ...
        ResultSet rs = stmt.executeQuery("SELECT DISTINCT c FROM #testdistinct");
        assertNotNull(rs);
        int rowCount = 0;
        while (rs.next()) {
            rowCount++;
    ...
    public void testPessimisticConcurrency() throws Exception {
        ...
        try {
            ...
            final Statement stmt2 = con2.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE + 1);
    ...
    public void testDynamicCursors() throws Exception {
        ...
        Connection con2 = getConnection();
        try {
            Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE + 1, ResultSet.CONCUR_READ_ONLY);
            ...
            assertEquals(1, stmt2.executeUpdate("INSERT INTO dynamicCursors VALUES(" + ROWS + ", 'Table A line " + ROWS + "')"));
            stmt2.close();
            int cnt;
    ...
    public void testRowstat() throws Exception {
        PreparedStatement stmt = con.prepareStatement("SELECT 'STRING' str", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
