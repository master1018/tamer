...
class Statistics extends AbstractTableModel implements Observer {
    ...
    public Object getValueAt(int row, int col) {
        ...
        if (col == 0) return signal.getName();
        return new Integer((int) signal.getEmitCount());
    }

    public String getColumnName(int i) {
        if (i == 0) return "Signal Description";
    ...
    public void setEnvView(EnvView view) {
        if (_view != null) _view.signalSelectionChanged.deleteObserver(this);
        _view = view;
    ...
    public Window(final Env env) {
        ...
        clear.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                final int rv = JOptionPane.showConfirmDialog(Window.this, "Are you sure you want to clear the environment", "Confirm clear", JOptionPane.YES_NO_OPTION);
        ...
        revert.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (_lastFile == null) {
                    JOptionPane.showMessageDialog(Window.this, "You did not load or save this environment yet.\nPlease load or save first", "Nothing to revert to", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                final int rv = JOptionPane.showConfirmDialog(Window.this, "Are you sure you want revert to " + _lastFile.getPath(), "Confirm revert to saved", JOptionPane.YES_NO_OPTION);
                if (rv != JOptionPane.YES_OPTION) return;
                try {
                    env.clear();
        ...
        resize.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String size = (String) JOptionPane.showInputDialog(Window.this, "Resize environment (X * Y) (X, Y) (X x Y) (X Y)", "Resize environment", JOptionPane.PLAIN_MESSAGE, null, null, env.getWidth() + " * " + env.getHeight());
                if ((size != null) && (size.length() > 0)) {
                    ...
                    try {
                        ...
                        JOptionPane.showMessageDialog(Window.this, "Invalid number format, it should be one of (X * Y) (X, Y) (X x Y) (X Y)", "Error parsing size", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
        });
        ...
        setid.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String objType = ((String) JOptionPane.showInputDialog(Window.this, "Enter default identifier for new bombs/traps (all char, first char in lowercase)", "Set Default Object APLIdentifier", JOptionPane.PLAIN_MESSAGE, null, null, env.getObjType()));
                ...
                if (objType.length() == 0 || invalid) {
                    JOptionPane.showMessageDialog(Window.this, "The object type identifier must be an all-character string with the first character in lowercase", "Invalid object type", JOptionPane.ERROR_MESSAGE);
                    return;
                }
        ...
        senserange.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String range = (String) JOptionPane.showInputDialog(Window.this, "Set agent sensor range in cells", "Set Sensor Range in Cells", JOptionPane.PLAIN_MESSAGE, null, null, Integer.toString(env.getSenseRange()));
                if ((range != null) && (range.length() > 0)) env.setSenseRange(Integer.parseInt(range));
            }
        });
