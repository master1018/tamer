        ...
        if (ProtocolUtils.verifySignature(in, DATA_SIGNATURE)) {
            ...
            if (size > 0) {
                ...
                for (Integer i = 0; i < size; i++) {
                    ...
                    if (customValues.containsKey(i)) customValues.get(i).setValue(value);
                }
            }
            if (!ProtocolUtils.verifySignature(in, END_SIGNATURE)) throw new RuntimeException("Message ending signature doesn't match.");
        } else throw new RuntimeException("Message starting signature doesn't match.");
        return result;
    }
}
