...
public class SequenceSimulator {
    ...
    int[] sequence2intArray(Sequence seq) {
        if (seq.getLength() != m_sequenceLength) {
            throw new RuntimeException("Ancestral sequence length has " + seq.getLength() + " characters " + "expecting " + m_sequenceLength + " characters");
        }
        int array[] = new int[m_sequenceLength];
        for (int i = 0; i < m_sequenceLength; i++) {
            array[i] = m_siteModel.getFrequencyModel().getDataType().getState(seq.getChar(i));
        }
        return array;
    }
    ...
    public static XMLObjectParser PARSER = new AbstractXMLObjectParser() {
        ...
        public Object parseXMLObject(XMLObject xo) throws XMLParseException {
            ...
            if (rateModel == null) rateModel = new DefaultBranchRateModel();
        ...
        private XMLSyntaxRule[] rules = new XMLSyntaxRule[] { new ElementRule(Tree.class), new ElementRule(SiteModel.class), new ElementRule(BranchRateModel.class, true), new ElementRule(Sequence.class, true), AttributeRule.newIntegerRule(REPLICATIONS) };
    };
