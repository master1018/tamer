...
public class GridStatistics extends Sim_entity {
    ...
    public GridStatistics(String name, String fileName, boolean append, String[] excludeFromFile, String[] excludeFromProcessing) throws Exception {
        ...
        try {
            ...
            System.out.println("GridStatistics() : Error - " + "unable to open " + fileName);
        }
    }
    ...
    public Accumulator accumulate(String category) {
        ...
        int pos = Collections.binarySearch(statList, new Stat(0.0, category, "", ""), new OrderByCategory());
        if (pos < 0) {
            return acc;
        }
    ...
    public Accumulator accumulate(String category, String counter) {
        ...
        int pos = Collections.binarySearch(statList, new Stat(0.0, counter, "", ""), new OrderByCategory());
        if (pos < 0) {
            return acc;
        }
        Stat statObj = (Stat) statList.get(pos);
        int j = Integer.parseInt(statObj.getName().substring(statObj.getName().indexOf('_') + 1));
        timeList.add(new Times(j, Integer.parseInt(statObj.getData())));
        int i = 0;
        for (i = pos + 1; i < statList.size(); i++) {
            statObj = (Stat) statList.get(i);
            if (statObj.getCategory().equals(counter)) {
                j = Integer.parseInt(statObj.getName().substring(statObj.getName().indexOf('_') + 1));
                timeList.add(new Times(j, Integer.parseInt(statObj.getData())));
            } else {
                break;
            }
        ...
        pos = Collections.binarySearch(statList, new Stat(0.0, category, "", ""), new OrderByCategory());
        if (pos < 0) {
            return acc;
        }
        statObj = (Stat) statList.get(pos);
        j = Integer.parseInt(statObj.getName().substring(statObj.getName().indexOf('_') + 1));
        int k = Collections.binarySearch(timeList, new Times(j, 0), new OrderByIndex());
        acc.add(new Double(statObj.getData()).doubleValue(), ((Times) timeList.get(k)).getValue());
        for (i = pos + 1; i < statList.size(); i++) {
            statObj = (Stat) statList.get(i);
            if (statObj.getCategory().equals(category)) {
                j = Integer.parseInt(statObj.getName().substring(statObj.getName().indexOf('_') + 1));
                k = Collections.binarySearch(timeList, new Times(j, 0), new OrderByIndex());
                acc.add(new Double(statObj.getData()).doubleValue(), ((Times) timeList.get(k)).getValue());
            } else {
                break;
            }
        ...
        for (sim_get_next(ev); ev.get_tag() != GridSimTags.END_OF_SIMULATION; sim_get_next(ev)) {
            ...
            switch(ev.get_tag()) {
                ...
                case GridSimTags.RETURN_ACC_STATISTICS_BY_CATEGORY:
                    ...
                    break;
                default:
                    System.out.println("GridStatistics.body() : " + "Unable to handle request from GridSimTags " + "with constant number " + ev.get_tag());
                    break;
            }
        }
        if (active_) {
