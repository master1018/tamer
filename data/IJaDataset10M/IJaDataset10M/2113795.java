 ...
 * @author hindlem
 */
public class NumericalGDSAwareState extends State {
     ...
     */
    public NumericalGDSAwareState(ConceptClass cc, AttributeName att, NumericalComparison comp, boolean ignoreConceptWithoutAttribute, Number referenceValue) {
        super(cc);
        this.comp = comp;
    ...
    public boolean isValid(ONDEXConcept c, EvidencePathNode<ONDEXConcept, ONDEXRelation, State> path) {
        try {
            GDS gds = c.getGDS(att);
            Object value = gds.getValue();
            Number number;
