...
public class CommentTrimmer {
    ...
    private static String trim(String value) {
        ...
        if (start < 0 || len < value.length()) return value.substring(start, len);
        return value;
    }
    ...
    private static String unindent(String value) {
        ...
        for (int i = value.indexOf('\n'), len = value.length(); i >= 0; i = value.indexOf('\n', i)) {
            ...
            if (i >= len) break;
            if (currentIndent < minIndent || minIndent < 0) minIndent = currentIndent;
        }
        ...
        for (int i = 0, len = value.length(); i < len; i++) {
            ...
            switch(c) {
                case ' ':
                    if (currentIndent >= 0) currentIndent++; else buf.append(c);
                    break;
                case '\t':
                    if (currentIndent >= 0) currentIndent = ((currentIndent / 8) + 1) * 8; else buf.append(c);
                    break;
                case '\n':
                    buf.append(c);
                    currentIndent = 0;
