...
public class SecondaryDatabase extends Database {
     ...
     */
    SecondaryDatabase(final Environment env, final SecondaryConfig secConfig, final Database primaryDatabase) throws DatabaseException {
        super(env);
     ...
     */
    @Override
    void initNew(final Environment env, final Locker locker, final String databaseName, final DatabaseConfig dbConfig) throws DatabaseException {
     ...
     */
    @Override
    void initExisting(final Environment env, final Locker locker, final DatabaseImpl database, final DatabaseConfig dbConfig) throws DatabaseException {
     ...
     * retained.
     */
    private void init(final Locker locker) throws DatabaseException {
     ...
     */
    public SecondaryCursor openSecondaryCursor(final Transaction txn, final CursorConfig cursorConfig) throws DatabaseException {
     ...
     */
    @Override
    public SecondaryCursor openCursor(final Transaction txn, final CursorConfig cursorConfig) throws DatabaseException {
     ...
     */
    @Override
    Cursor newDbcInstance(final Transaction txn, final CursorConfig cursorConfig) throws DatabaseException {
     ...
     */
    @Override
    public OperationStatus delete(final Transaction txn, final DatabaseEntry key) throws DeleteConstraintException, LockConflictException, DatabaseException, UnsupportedOperationException, IllegalArgumentException {
        checkEnv();
        ...
        try {
            locker = LockerFactory.getWritableLocker(envHandle, txn, isTransactional(), getDatabaseImpl().isReplicated());
     ...
     */
    @Override
    public OperationStatus get(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data, final LockMode lockMode) throws DatabaseException {
     ...
     */
    public OperationStatus get(final Transaction txn, final DatabaseEntry key, final DatabaseEntry pKey, final DatabaseEntry data, LockMode lockMode) throws DatabaseException {
        checkEnv();
        DatabaseUtil.checkForNullDbt(key, "key", true);
        ...
        try {
            locker = LockerFactory.getReadableLocker(envHandle, txn, isTransactional(), false, cursorConfig.getReadCommitted());
            cursor = new SecondaryCursor(this, locker, cursorConfig);
            commitStatus = cursor.search(key, pKey, data, lockMode, SearchMode.SET);
            return commitStatus;
        } catch (Error E) {
            DbInternal.getEnvironmentImpl(envHandle).invalidate(E);
            throw E;
     ...
     */
    @Override
    public OperationStatus getSearchBoth(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data, final LockMode lockMode) throws UnsupportedOperationException {
     ...
     * @throws IllegalArgumentException if an invalid parameter is specified.
     */
    public OperationStatus getSearchBoth(final Transaction txn, final DatabaseEntry key, final DatabaseEntry pKey, final DatabaseEntry data, LockMode lockMode) throws DatabaseException {
        checkEnv();
        DatabaseUtil.checkForNullDbt(key, "key", true);
        ...
        try {
            locker = LockerFactory.getReadableLocker(envHandle, txn, isTransactional(), false, cursorConfig.getReadCommitted());
            cursor = new SecondaryCursor(this, locker, cursorConfig);
            commitStatus = cursor.search(key, pKey, data, lockMode, SearchMode.BOTH);
            return commitStatus;
        } catch (Error E) {
            DbInternal.getEnvironmentImpl(envHandle).invalidate(E);
            throw E;
     ...
     */
    @Override
    public OperationStatus put(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data) throws UnsupportedOperationException {
        throw notAllowedException();
    }
     ...
     */
    @Override
    public OperationStatus putNoOverwrite(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data) throws UnsupportedOperationException {
        throw notAllowedException();
    }
     ...
     */
    @Override
    public OperationStatus putNoDupData(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data) throws UnsupportedOperationException {
        throw notAllowedException();
    }
     ...
     */
    @Override
    public JoinCursor join(final Cursor[] cursors, final JoinConfig config) throws UnsupportedOperationException {
        throw notAllowedException();
    }
     ...
     * has been deleted.
     */
    void updateSecondary(final Locker locker, Cursor cursor, final DatabaseEntry priKey, final DatabaseEntry oldData, final DatabaseEntry newData) throws DatabaseException {
            ...
            if (!oldKeys.equals(newKeys)) {
                ...
                try {
                    ...
                    if (oldKeys != EMPTY_SET) {
                        ...
                        for (Iterator<DatabaseEntry> i = oldKeys.iterator(); i.hasNext(); ) {
                    ...
                    if (newKeys != EMPTY_SET) {
                        ...
                        for (Iterator<DatabaseEntry> i = newKeys.iterator(); i.hasNext(); ) {
     ...
     */
    private void deleteKey(final Cursor cursor, final DatabaseEntry priKey, final DatabaseEntry oldSecKey) throws DatabaseException {
        ...
        if (status == OperationStatus.SUCCESS) {
            ...
            throw new SecondaryIntegrityException(cursor.getCursorImpl().getLocker(), "Secondary is corrupt: the primary record contains a key " + "that is not present in the secondary", getDebugName(), oldSecKey, priKey);
        }
    }
     ...
     */
    private void insertKey(final Locker locker, final Cursor cursor, final DatabaseEntry priKey, final DatabaseEntry newSecKey) throws DatabaseException {
        Database foreignDb = secondaryConfig.getForeignKeyDatabase();
        if (foreignDb != null) {
            ...
            try {
                ...
                if (status != OperationStatus.SUCCESS) {
                    throw new ForeignConstraintException(locker, "Secondary " + getDebugName() + " foreign key not allowed: it is not" + " present in the foreign database " + foreignDb.getDebugName(), getDebugName(), newSecKey, priKey);
                }
            } finally {
        ...
        if (configuration.getSortedDuplicates()) {
            OperationStatus status = cursor.putInternal(newSecKey, priKey, PutMode.NO_DUP_DATA);
            if (status != OperationStatus.SUCCESS) {
                throw new SecondaryIntegrityException(locker, "Secondary/primary record already present", getDebugName(), newSecKey, priKey);
            }
        } else {
            OperationStatus status = cursor.putInternal(newSecKey, priKey, PutMode.NO_OVERWRITE);
            if (status != OperationStatus.SUCCESS) {
                throw new UniqueConstraintException(locker, "Unique secondary key is already present", getDebugName(), newSecKey, priKey);
            }
        }
    }
     ...
     */
    void onForeignKeyDelete(final Locker locker, final DatabaseEntry secKey) throws DatabaseException {
        ...
        try {
            ...
            while (status == OperationStatus.SUCCESS) {
                if (deleteAction == ForeignKeyDeleteAction.ABORT) {
                    throw new DeleteConstraintException(locker, "Secondary refers to a deleted foreign key", getDebugName(), secKey, priKey);
    ...
    SecondaryIntegrityException secondaryRefersToMissingPrimaryKey(final Locker locker, final DatabaseEntry secKey, final DatabaseEntry priKey) throws DatabaseException {
        return new SecondaryIntegrityException(locker, "Secondary refers to a missing key in the primary database", getDebugName(), secKey, priKey);
    }
    ...
    void trace(final Level level, final String methodName) {
        if (logger.isLoggable(level)) {
            ...
            LoggerUtils.logMsg(logger, envHandle.getEnvironmentImpl(), level, sb.toString());
        }
    }
}
