...
public class WorkSetLoadThread extends Thread {
    ...
    private static class Task {
        ...
        public void issueCallbacks() {
            for (Runnable run : callbacks) run.run();
        }
    ...
    public static void obtainDatafiles(WorkSet df, IWorkSetProgress progress, Runnable run) {
        ...
        owsm.workset = df;
        owsm.callback = run;
        if (progress != null) owsm.progress = progress; else owsm.progress = dummyWorkSetProgress;
        ...
        fileCacheUpdateCallback = new FileCacheUpdateCallback() {
            ...
            class CleanupTasksRunnable implements Runnable {
                ...
                public void run() {
                    ...
                    for (Task t : taskList) {
                        if (url.equals(t.obo)) t.oboDownloaded = true;
                        if (url.equals(t.assoc)) t.assocDownloaded = true;
                        if (t.oboDownloaded) {
                            try {
                    ...
                    if (taskList.size() == 0) FileCache.removeUpdateCallback(fileCacheUpdateCallback);
                }
            }
    ...
    private void addTask(Task newTask) {
        if (taskList.size() == 0) FileCache.addUpdateCallback(fileCacheUpdateCallback);
