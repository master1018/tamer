 ...
 */
    public Attr(String name, int data_type, AttrWriteType w_type, String assoc) throws DevFailed {
        ...
        check_type();
        if ((writable.value() == AttrWriteType._WRITE) && !assoc_name.equals(Tango_AssocWritNotSpec)) {
            ...
            Except.throw_exception("API_AttrWrongDefined", o.toString(), "Attr.Attr");
        }
        if ((writable.value() == AttrWriteType._READ_WITH_WRITE) && assoc_name.equals(Tango_AssocWritNotSpec)) {
            Util.out3.println("Attr.Attr throwing exception");
            StringBuffer o = new StringBuffer("Attribute : ");
            o.append(name);
            o.append(": ");
        ...
        if (writable.value() == AttrWriteType._READ_WRITE) assoc_name = name;
    }
 ...
 *
 */
    public Attr(String name, int data_type, AttrWriteType w_type, String assoc, DispLevel level) throws DevFailed {
        this.name = name;
        type = data_type;
        writable = w_type;
        assoc_name = assoc;
 ...
 * @param	level	The attribute display type
 */
    public Attr(String name, int data_type, AttrWriteType w_type, DispLevel level) throws DevFailed {
        this.name = name;
        type = data_type;
        writable = w_type;
        disp_level = level;
    ...
    void check_type() throws DevFailed {
        ...
        if (type == Tango_DEV_SHORT) unsuported = false; else if (type == Tango_DEV_USHORT) unsuported = false; else if (type == Tango_DEV_LONG) unsuported = false; else if (type == Tango_DEV_LONG64) unsuported = false; else if (type == Tango_DEV_ULONG) unsuported = false; else if (type == Tango_DEV_ULONG64) unsuported = false; else if (type == Tango_DEV_BOOLEAN) unsuported = false; else if (type == Tango_DEV_FLOAT) unsuported = false; else if (type == Tango_DEV_DOUBLE) unsuported = false; else if (type == Tango_DEV_STRING) unsuported = false; else if (type == Tango_DEV_STATE) unsuported = false; else if (type == Tango_DEV_ENCODED) unsuported = false;
        if (unsuported == true) {
            Util.out3.println("Attr.check_type throwing exception");
            StringBuffer o = new StringBuffer("Attribute : ");
            o.append(name).append(": Data type is not supported");
            Except.throw_exception("API_AttrWrongDefined", o.toString(), "Attr.check_type");
        }
    }
 ...
 */
    public void set_default_properties(UserDefaultAttrProp prop_list) {
        if (prop_list.label != null) user_default_properties.addElement(new AttrProperty("label", prop_list.label));
        if (prop_list.description != null) user_default_properties.addElement(new AttrProperty("description", prop_list.description));
        if (prop_list.unit != null) user_default_properties.addElement(new AttrProperty("unit", prop_list.unit));
        if (prop_list.standard_unit != null) user_default_properties.addElement(new AttrProperty("standard_unit", prop_list.standard_unit));
        if (prop_list.display_unit != null) user_default_properties.addElement(new AttrProperty("display_unit", prop_list.display_unit));
        if (prop_list.format != null) user_default_properties.addElement(new AttrProperty("format", prop_list.format));
        if (prop_list.min_value != null) user_default_properties.addElement(new AttrProperty("min_value", prop_list.min_value));
        if (prop_list.max_value != null) user_default_properties.addElement(new AttrProperty("max_value", prop_list.max_value));
        if (prop_list.min_alarm != null) user_default_properties.addElement(new AttrProperty("min_alarm", prop_list.min_alarm));
        if (prop_list.max_alarm != null) user_default_properties.addElement(new AttrProperty("max_alarm", prop_list.max_alarm));
    }
