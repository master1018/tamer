...
public class Grapher {
    ...
    void NextToken() {
        int startPos = 0;
        while (startPos < execList.length() && execList.charAt(startPos) == ' ') startPos++;
        int endPos = startPos;
        while (endPos < execList.length() && execList.charAt(endPos) != ' ') endPos++;
    ...
    public void paint(Graphics g, int xleft, int ytop, Dimension d) {
        ...
        if (xRange.TickSize() * xtick < xmin) xtick = xtick + 1;
        double xstart = xRange.TickSize() * xtick;
        {
            ...
            if (yRange.TickSize() * ytick < ymin) ytick = ytick + 1;
