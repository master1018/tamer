...
public class LinkDatabaseCanvas extends ExtensibleCanvas implements ObjectSelector, RightClickMenuProvider, Filterable, FilteredRenderable {

    protected static final Logger logger = Logger.getLogger(LinkDatabaseCanvas.class);
    ...
    public static final Comparator<Object> LAYOUT_ORDERING_COMPARATOR = new Comparator<Object>() {
        ...
        public int compare(Object o1, Object o2) {
            if (o1 instanceof PNode && o2 instanceof PNode) {
                ...
                n1.invalidatePaint();
                n2.invalidatePaint();
                if (o1 instanceof OENode && o2 instanceof OELink) return 1; else if (o2 instanceof OENode && o1 instanceof OELink) return -1; else return 0;
            } else return 0;
        }
    };
    ...
    public static void decorateNode(PRoot root, PNode canvas, Collection<NodeDecorator> decorators, boolean noAnimation, boolean postLayout) {
    ...
    protected static Collection<IdentifiedObject> getLinkedObjectCollection(Collection<?> pcs) {
        ...
        for (Object pc : pcs) {
            if (pc instanceof LinkedObject) {
                ...
                if (node.getName() != null) out.add((LinkedObject) pc);
                ...
                if (SHOW_SYMMETRIC_RELATIONS_AS_LEAVES) {
                    ...
                    if (relationshipType != null && relationshipType.isSymmetric()) {
                        ...
                        if (link.getParent() != null) out.add(link.getParent());
                    }
                } else {
                    out.add(link.getChild());
                    if (link.getParent() != null) out.add(link.getParent());
    ...
    protected SelectionListener globalSelectionNotifier = new SelectionListener() {
        ...
        public void selectionChanged(SelectionEvent e) {
            if (isLive()) {
                SelectionManager.getManager().removeSelectionListener(globalSelectionListener);
                SelectionManager.setGlobalSelection(getSelection());
                SelectionManager.getManager().addSelectionListener(globalSelectionListener);
            }
        }
    };
    ...
    protected long layoutDuration = new Long(System.getProperty("LAYOUT_DURATION", DEFAULT_LAYOUT_DURATION + ""));
    ...
    protected static boolean SHOW_SYMMETRIC_RELATIONS_AS_LEAVES = true;
    ...
    public LinkDatabaseCanvas(GraphLayout graphLayout) {
        ...
        setNodeLabelProvider(new HTMLNodeLabelProvider(this, CollectionUtil.list(BackgroundColorSpecField.FIELD), null, defaultObjectRenderers));
    ...
    protected void installDefaultRenderers() {
        defaultObjectRenderers.add(new RenderedFilter(new GeneralRendererSpec(HTMLSpecField.FIELD, "<center><font face='Arial'>$term$</font></center>")));
    }
    ...
    public void addExpansionListener(ExpandCollapseListener listener) {
        if (linkDatabase != null) linkDatabase.addListener(listener);
    ...
    public void fillInMenu(MouseEvent e, JPopupMenu menu) {
        ...
        for (RightClickMenuFactory factory : menuFactories) {
            ...
            for (JMenuItem item : factories) {
                ...
                if (item == RightClickMenuFactory.SEPARATOR_ITEM) menu.addSeparator(); else menu.add(item);
            }
        }
    }

    protected void fireFocusedNodeChanged(PathCapable oldNode, PathCapable newNode) {
    ...
    protected void fireRelayoutCompleteEvent() {
        for (RelayoutListener listener : new LinkedList<RelayoutListener>(layoutListeners)) listener.relayoutComplete();
    }

    protected void fireRelayoutStartingEvent() {
        for (RelayoutListener listener : new LinkedList<RelayoutListener>(layoutListeners)) listener.relayoutStarting();
    }
    ...
    public PBounds getBounds(Collection<PathCapable> pcs) {
        ...
        for (PathCapable pc : pcs) {
            ...
            if (bounds == null) {
                bounds = new PBounds(node.getXOffset(), node.getYOffset(), node.getWidth(), node.getHeight());
            } else bounds.add(new PBounds(node.getXOffset(), node.getYOffset(), node.getWidth(), node.getHeight()));
        }
        return bounds;
    }
    ...
    public TypeColorManager getColorManager() {
        if (nodeFactory instanceof DefaultNodeFactory) return ((DefaultNodeFactory) nodeFactory).getColorManager(); else return null;
    }
    ...
    public PNode getFocusedNode() {
        if (focusedObject == null) return null; else return getNode(focusedObject);
    }
    ...
    public TypeIconManager getIconManager() {
        if (nodeFactory instanceof DefaultNodeFactory) return ((DefaultNodeFactory) nodeFactory).getIconManager(); else return null;
    }
    ...
    public float getMinZoom() {
        ...
        float minZoom = Math.min(Math.min(viewWidth / zoomWidth, viewHeight / zoomHeight), 1f);
        return minZoom;
    }
    ...
    public PNode getNewLayer() {
        if (!isLayingOut()) throw new IllegalStateException("getNewLayer() can only be called while the canvas is laying out");
        return newLayer;
    }

    public PCNode getNode(int x, int y) {
        PPickPath path = getCamera().pick(x, y, 1);
        PCNode node = (PCNode) PiccoloUtil.getNodeOfClass(path, OENode.class, OELink.class);
        return node;
    }
    ...
    public Selection getPickedAsSelection(PInputEvent event) {
        PCNode originNode = PiccoloUtil.getNodeOfClass(event.getPath(), PCNode.class);
        if (originNode == null) return SelectionManager.createEmptySelection(this); else return SelectionManager.createSelection(this, originNode.getObject(), RootAlgorithm.GREEDY, getLinkProviderDatabase());
    }
    ...
    public Selection getPickerSelection(PInputEvent event) {
        PCNode originNode = (PCNode) PiccoloUtil.getNodeOfClass(event.getPath(), OENode.class, OELink.class);
        if (originNode != null) {
            if (getSelection().getAllSelectedObjects().contains(originNode.getObject())) {
                return getSelection();
            } else {
                return SelectionManager.createSelection(this, originNode.getObject(), RootAlgorithm.GREEDY, getLinkProviderDatabase());
            }
        } else return null;
    }
    ...
    public ReasonedLinkDatabase getReasoner() {
        ...
        } else return SessionManager.getManager().getReasoner();
    }
    ...
    public Selection getSelection(MouseEvent e) {
        ...
        if (node == null) return SelectionManager.createEmptySelection(this);
        return getSelection();
    }

    public GestureTarget getTarget(int x, int y) {
        PCNode node = getNode(x, y);
        if (node == null) return SelectionManager.createEmptyTarget();
        return SelectionManager.createGestureTarget(this, getLinkDatabase(), getRootAlgorithm(), node.getObject());
    }
    ...
    public Collection<PCNode> getVisibleNodes() {
        ...
        for (PathCapable pc : getVisibleObjects()) {
            PNode node = getNode(pc);
            if (node != null && node instanceof PCNode) out.add((PCNode) node);
        }
        return out;
    }
    ...
    public void panToObjects() {
        ...
        getCamera().animateViewToCenterBounds(centerBounds, false, getLayoutDuration());
    }
    ...
    public void popupInFrame(JComponent component, String title, int x, int y) {
        ...
        internalFrame.pack();
        Dimension d = new Dimension(Math.min(internalFrame.getWidth(), getWidth()), Math.min(internalFrame.getHeight(), getHeight()));
        internalFrame.setSize(d);
        if (x + d.getWidth() > getWidth()) x -= (x + d.getWidth()) - getWidth();
        if (y + d.getHeight() > getHeight()) y -= (y + d.getHeight()) - getHeight();
    ...
    public void relayout() {
        ...
        relayoutActivity = morpher.morph(getLayer(), newLayer, getLayoutDuration());
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                if (relayoutActivity == null) return;
    ...
    public void removeExpansionListener(ExpandCollapseListener listener) {
        if (linkDatabase != null) linkDatabase.removeListener(listener);
    ...
    public void select(final Selection selection) {
        ...
        if (selection.getTermSubSelection() != null && !selection.getAllSelectedObjects().contains(selection.getTermSubSelection())) logger.info("LinkDatabaseCanvas.select: weird selection: " + selection);
        if (selection.getTermSubSelection() != null) visible.add(selection.getTermSubSelection());
        ...
        if (isExpandSelectionPaths() && !ObjectUtil.equals(selection.getSelector(), this)) {
            ...
            if (paths == null || paths.length == 0) {
                ...
                paths = PathUtil.getBestPaths(selection.getAllSelectedObjects(), getRootAlgorithm(), getLinkDatabase());
            }
    ...
    public void setColorManager(TypeColorManager typeManager) {
        if (nodeFactory instanceof DefaultNodeFactory) ((DefaultNodeFactory) nodeFactory).setColorManager(typeManager);
    }
    ...
    public void setGraphLayout(GraphLayout layout) {
        ...
        if (linkDatabase != null) layoutEngine.setLinkDatabase(linkDatabase);
    }

    public void setIconManager(TypeIconManager iconManager) {
        if (nodeFactory instanceof DefaultNodeFactory) ((DefaultNodeFactory) nodeFactory).setIconManager(iconManager);
    }
    ...
    public void setLinkDatabase(LinkDatabase linkDatabase) {
        if (this.linkDatabase == null) {
            ...
            this.linkDatabase = new CollapsibleLinkDatabase(filteredLinkDatabase);
            for (ExpandCollapseListener listener : expansionListeners) this.linkDatabase.addListener(listener);
            this.linkDatabase.addListener(expandCollapseListener);
            if (layoutEngine != null) layoutEngine.setLinkDatabase(this.linkDatabase);
        } else {
            this.filteredLinkDatabase.setLinkDatabase(linkDatabase);
        }
        filteredLinkDatabase.setLinkFilter(FilterManager.getManager().getAugmentedLinkFilter(getLinkFilter()));
        filteredLinkDatabase.setTermFilter(FilterManager.getManager().getAugmentedTermFilter(getTermFilter()));
    }
    ...
    public void setVisibleObjects(Collection<?> visible) {
        linkDatabase.setVisibleObjects(getLinkedObjectCollection(visible), false);
    }
    ...
    public void updateDatasources() {
        ...
        if (newLinkDB == null) newLinkDB = SessionManager.getManager().getSession().getLinkDatabase();
