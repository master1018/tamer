...
public class PackerImpl implements Pack200.Packer {
    ...
    private class DoPack {
...

        {
            ...
            if (attrDefs.size() > 0) this.attrDefs = attrDefs; else this.attrDefs = null;
            if (attrCommands.size() > 0) this.attrCommands = attrCommands; else this.attrCommands = null;
        }

        final boolean keepFileOrder = _props.getBoolean(Pack200.Packer.KEEP_FILE_ORDER);

        final boolean keepClassOrder = _props.getBoolean(Utils.PACK_KEEP_CLASS_ORDER);

        final boolean keepModtime = Pack200.Packer.KEEP.equals(_props.getProperty(Pack200.Packer.MODIFICATION_TIME));

        final boolean latestModtime = Pack200.Packer.LATEST.equals(_props.getProperty(Pack200.Packer.MODIFICATION_TIME));

        final boolean keepDeflateHint = Pack200.Packer.KEEP.equals(_props.getProperty(Pack200.Packer.DEFLATE_HINT));
        ...
        {
            long limit;
            if (_props.getProperty(Pack200.Packer.SEGMENT_LIMIT, "").equals("")) limit = -1; else limit = _props.getLong(Pack200.Packer.SEGMENT_LIMIT);
        ...
        {
            int ver;
            if ((ver = _props.getInteger(Utils.COM_PREFIX + "min.class.majver")) != 0) pkg.min_class_majver = (short) ver;
            if ((ver = _props.getInteger(Utils.COM_PREFIX + "min.class.minver")) != 0) pkg.min_class_minver = (short) ver;
            if ((ver = _props.getInteger(Utils.COM_PREFIX + "max.class.majver")) != 0) pkg.max_class_majver = (short) ver;
            if ((ver = _props.getInteger(Utils.COM_PREFIX + "max.class.minver")) != 0) pkg.max_class_minver = (short) ver;
            if ((ver = _props.getInteger(Utils.COM_PREFIX + "package.minver")) != 0) pkg.package_minver = (short) ver;
            if ((ver = _props.getInteger(Utils.COM_PREFIX + "package.majver")) != 0) pkg.package_majver = (short) ver;
        }
        ...
        class InFile {
            ...
            InputStream getInputStream() throws IOException {
                if (jf != null) return jf.getInputStream(je); else return new FileInputStream(f);
            }
