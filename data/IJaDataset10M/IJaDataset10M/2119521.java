...
public class CommandOnTaskContextualized<T> {
    ...
    public static <T> CommandOnTaskContextualized<T> create(ICommandOnTask<T> commandOnTask, IDomainAndBeansMapper<T> mapper, IContext<T> context) {
    ...
    private CommandOnTaskContextualized(ICommandOnTask<T> commandOnTask, IDomainAndBeansMapper<T> mapper, IContext<T> context) {
    ...
    public void doAction(TaskComponent taskComponent) {
        doAction(ContextRelativeToOtherComponent.makeRelativeTo(context, taskComponent), domainObjectFor(taskComponent.getTask()));
    }
    ...
    private void doAction(IContext<T> context, T domainObject) {
        IContextWithPlannerTask<T> contextWithTask = ContextWithPlannerTask.create(context, mapper.findAssociatedBean(domainObject));
