...
public class NaviGoalFindSpace extends NaviGoal {
    ...
    public NaviGoalFindSpace(float alpha, float betha, float rho, float minDist, float targetDist) {
        ...
        if (Odometry.normalizeOri(alpha - Odometry.ori) > 0.0f) mode = MODE_FIRST_CLOCKWISE; else mode = MODE_FIRST_COUNTERCLOCK;
    }

    public boolean work() {
        float dori;
        if (mode == MODE_FIRST_CLOCKWISE | mode == MODE_FIRST_COUNTERCLOCK) dori = alpha; else dori = betha;
        ...
        switch(mode) {
            case MODE_FIRST_CLOCKWISE:
                if (dori < 0) {
                    ...
                    foundSpace = false;
                } else Navigator.goRotate(Navigator.checkTurnSpeed(Navigator.MAX_TURN_SPEED, alpha));
                break;
            ...
            case MODE_SEC_CLOCKWISE:
                if (dori < 0) return false; else Navigator.goRotate(Navigator.checkTurnSpeed(Navigator.MAX_TURN_SPEED, betha));
                break;
