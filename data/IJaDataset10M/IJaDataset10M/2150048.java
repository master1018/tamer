...
*/
    public IxFeat[] getfeats() {
        return getfeats(null);
    }

    public IxFeat[] getfeats(String[] types) {
        if (list == null) return null;
        int n = list.size();
        if (types == null) return (IxFeat[]) list.toArray(new IxFeat[n]); else {
            ArrayList vals = new ArrayList(n);
            List typel = Arrays.asList(types);
            for (int i = 0; i < n; i++) {
                IxFeat ft = (IxFeat) list.get(i);
                if (typel.contains(ft.get("tag"))) vals.add(ft);
            }
...
*/
    public HashMap getlocs() {
        ...
        for (Iterator i = locsrc.keySet().iterator(); i.hasNext(); ) {
            ...
            Arrays.sort(ar);
            ArrayList sl = new ArrayList(Arrays.asList(ar));
    ...
    public int printObj(PrintStream out, int depth) {
        ...
        if (hloc != null) {
            for (Iterator li = hloc.keySet().iterator(); li.hasNext(); ) {
                ...
                out.print(tab + "loc." + src + "=" + join(",", al.toArray()) + "\n");
            }
        }
        ...
        if (attrs != null) {
            ...
            for (int i = 0; i < attrs.length; i++) {
                if (i == 0) out.print(tab); else if (ln > 80) {
                    out.print("\n" + tab);
                    ln = depth * 2;
                }
                ln += attrs[i].printObj(out, depth);
                nd++;
            }
            if (nd > 0) out.print("\n");
        }
