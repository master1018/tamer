...
public class InputLineTokenizer extends Tokenizer {
    ...
    public Token nextToken() {
        for (; ; ) {
            ...
            if (code == InputLine.EOL) return Token.NULL; else switch(maker.scanCat(code)) {
                case ESCAPE:
                    code = line.getNext();
                ...
                case ENDLINE:
                    line.skipAll();
                    if (state == MID_LINE) return maker.makeSpace(); else if (state == NEW_LINE) return maker.makePar();
                    break;
                case SPACER:
                    if (state == MID_LINE) {
                        state = SKIP_BLANKS;
