...
public class PlanePolarGrid extends Plottable<Point2D.Double> {
    ...
    public void draw(VisometryGraphics<Point2D.Double> vg) {
        ...
        for (Double r : rValues) pg.drawCircle(0.0, 0.0, r);
        for (double theta = 0; theta < 2 * Math.PI; theta += 2 * Math.PI / THETA_STEPS) pg.drawSegment(minR * Math.cos(theta), minR * Math.sin(theta), maxR * Math.cos(theta), maxR * Math.sin(theta));
    }
