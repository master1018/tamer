...
public class JoshSingleLineTracker extends EventFilter2D implements FrameAnnotater {
    ...
    public EventPacket<?> filterPacket(EventPacket<?> in) {
        ...
        if (maxDist == 0) maxDist = Math.sqrt(chip.getSizeX() * chip.getSizeX() + chip.getSizeY() * chip.getSizeY());
    ...
    public void annotate(GLAutoDrawable drawable) {
        ...
        gl.glVertex2d(xBottom < 0 ? 0 : (xBottom > chip.getSizeX() ? chip.getSizeX() : xBottom), yLeft < 0 ? 0 : (yLeft > chip.getSizeY() ? chip.getSizeY() : yLeft));
        gl.glVertex2d(xTop < 0 ? 0 : (xTop > chip.getSizeX() ? chip.getSizeX() : xTop), yRight < 0 ? 0 : (yRight > chip.getSizeY() ? chip.getSizeY() : yRight));
        gl.glEnd();
        gl.glBegin((GL.GL_LINES));
        gl.glColor3d(1, 0, 0);
    ...
    private void updateCoefficients(double x, double y) {
        ...
        if (xHorizontal) dist += Math.sqrt(Math.pow(x - center, 2) + Math.pow(y - center1, 2)); else dist += Math.sqrt(Math.pow(y - center, 2) + Math.pow(x - center1, 2));
