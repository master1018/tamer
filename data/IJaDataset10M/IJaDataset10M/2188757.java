...
public class SolrMeterMenuController {
    ...
    @Inject
    public SolrMeterMenuController(StatisticsRepository repository, QueryExecutor queryExecutor, OptimizeExecutor optimizeExecutor, UpdateExecutor updateExecutor) {
        super();
    ...
    public void onExportMenu() {
        try {
            ...
            if (returnValue == JFileChooser.APPROVE_OPTION) {
                ...
                if (selectedFile.exists()) {
                    int optionResultPane = JOptionPane.showConfirmDialog(SolrMeterMain.mainFrame, I18n.get("menu.file.export.fileExists.message"), I18n.get("menu.file.export.fileExists.title"), JOptionPane.WARNING_MESSAGE, JOptionPane.OK_CANCEL_OPTION);
    ...
    public void onRestartMenu() {
        int optionResultPane = JOptionPane.showConfirmDialog(SolrMeterMain.mainFrame, I18n.get("menu.edit.restart.confirm.message"), I18n.get("menu.edit.restart.confirm.title"), JOptionPane.OK_CANCEL_OPTION);
        if (optionResultPane == JOptionPane.OK_OPTION) {
            queryExecutor.stop();
            updateExecutor.stop();
            SolrMeterMain.restartApplication();
