...
public class JPanelCloseMoney extends JPanel implements JPanelView, BeanFactoryApp {
    ...
    public void init(AppView app) throws BeanFactoryException {
        ...
        m_jTicketTable.setDefaultRenderer(Object.class, new TableRendererBasic(new Formats[] { new FormatsPayment(), Formats.CURRENCY }));
        ...
        m_jsalestable.setDefaultRenderer(Object.class, new TableRendererBasic(new Formats[] { Formats.STRING, Formats.CURRENCY, Formats.CURRENCY }));
    ...
    private void m_jCloseCashActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (res == JOptionPane.YES_OPTION) {
            ...
            try {
                new StaticSentence(m_App.getSession(), "INSERT INTO CHANGEPOINTER (ID,EVENTTIME,HANDLED,HANDLINGERROR,OBJECTTYPE,OBJECTID,CHANGETYPE) VALUES (?,?,?,?,?,?,?)", new SerializerWriteBasic(new Datas[] { Datas.STRING, Datas.TIMESTAMP, Datas.BOOLEAN, Datas.BOOLEAN, Datas.STRING, Datas.STRING, Datas.STRING })).exec(new Object[] { UUID.randomUUID().toString(), new Date(), false, false, "CASHCLOSED", m_App.getActiveCashIndex(), "CREATE" });
            } catch (BasicException e) {
                MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, AppLocal.getIntString("message.cannotclosecash"), e);
                msg.show(this);
            }
            try {
                ...
                m_dlSystem.execInsertCash(new Object[] { m_App.getActiveCashIndex(), m_App.getProperties().getHost(), m_App.getActiveCashDateStart(), m_App.getActiveCashDateEnd() });
