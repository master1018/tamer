...
public class TaskOrgEntityTablePanel extends JSplitPane implements TableModelListener {
    ...
    public void tableChanged(TableModelEvent e) {
        if (e.getSource() instanceof OrgEntitySelectTable) {
            if (e.getColumn() == 0) {
                ...
                } else ((OrgEntitySelectTable) orgEntitySelectTable.getModel()).updateTable();
            }
        }
    }

    public OrgEntity getSeletedOrgEntity() {
        if (orgEntitySelectTable.getSelectedRow() < 0) return null; else return ((OrgEntitySelectTable) orgEntitySelectTable.getModel()).getOrgEntity(orgEntitySelectTable.getSelectedRow());
    }

    public Task getSeletedTask() {
        if (taskTable.getSelectedRow() < 0) return null; else return ((TaskTable) taskTable.getModel()).getTask(taskTable.getSelectedRow());
    }
    ...
    private class TaskTable extends AbstractTableModel {
        ...
        public int getRowCount() {
            if (tasks == null) return 0; else return tasks.size();
        }
        ...
        public Object getValueAt(int row, int column) {
            ...
            if (column == 0) return ((Task) tasks.get(row)).getName(); else return ((Task) tasks.get(row)).getEventType();
        }
    ...
    private class OrgEntitySelectTable extends AbstractTableModel {
        ...
        public OrgEntitySelectTable() {
            if (orgEntities != null) for (int i = 0; i < orgEntities.size(); i++) choose.add(new Boolean(false));
        }
