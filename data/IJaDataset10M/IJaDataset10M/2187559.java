...
public class ColoredPetriNet extends PetriNet {
    ...
    private void establishPetriNetStructure(PetriNet net) {
        ...
        while (transitions.hasNext()) {
            ...
            if (e1.object != null && e1.object instanceof Transition) {
                ...
                int width = (int) (((Double) n.getAttributeValue(Grappa.WIDTH_ATTR)).doubleValue() * ManagerLayout.getInstance().getStretchFactor());
                int height = (int) (((Double) n.getAttributeValue(Grappa.HEIGHT_ATTR)).doubleValue() * ManagerLayout.getInstance().getStretchFactor());
                ColoredTransition simTransition = new ColoredTransition((Transition) e1.object, this, x, y, width, height);
        ...
        while (places.hasNext()) {
            ...
            if (e1.object != null && e1.object instanceof Place) {
                ...
                int width = (int) (((Double) n.getAttributeValue(Grappa.WIDTH_ATTR)).doubleValue() * ManagerLayout.getInstance().getStretchFactor());
                int height = (int) (((Double) n.getAttributeValue(Grappa.HEIGHT_ATTR)).doubleValue() * ManagerLayout.getInstance().getStretchFactor());
                ColoredPlace simPlace = new ColoredPlace((Place) e1.object, this, x, y, width, height);
    ...
    private void evaluateOrderRanks() {
        ...
        for (HLChoice choice : process.getChoices()) {
            ...
            for (HLCondition cond : choice.getConditions()) {
                if (cond.getExpression().getRootExpressionElement() == null && cond.getExpression().getExpressionString().equals("default")) {
    ...
    public void writeToFile(BufferedWriter bw, BufferedWriter currentStateWriter, String smlName) {
        try {
            ...
            ManagerXml.writeControlFlowDec(bw, translator.variableTranslator.getCpnVarForCaseId(), translator.colorsetTranslator.getColorSetCaseID(), isTimed);
            if (isRedesign) {
                ManagerXml.writeRedesignDec(bw, RedesignAnalysisUI.locationForCurrentSimSettingsInCPNExport);
            }
            if ((ManagerConfiguration.getInstance().isResourcePerspectiveEnabled()) && process.getGroups().size() > 0) {
            ...
            if (ManagerConfiguration.getInstance().isDataPerspectiveEnabled() && process.getAttributes().size() > 0) {
                ...
                ManagerXml.writeDataDec(bw, cpnVarsDataAttr, translator.colorsetTranslator.getColorSetDataAttributes(), translator.colorsetTranslator.getColorSetsSeparateDataAttributes());
            ...
            if (ManagerConfiguration.getInstance().isThroughputTimeMonitorEnabled() || isRedesign) {
                ManagerXml.writeStartCase(bw, translator.variableTranslator.getCpnVarForStartCase(), translator.colorsetTranslator.getColorSetStartCase());
            }
            if (isRedesign) {
                ManagerXml.writeProductColorSetsForPlaces(bw, translator.colorsetTranslator.getProductColorSetsForPlaces(), true);
            } else {
                ManagerXml.writeProductColorSetsForPlaces(bw, translator.colorsetTranslator.getProductColorSetsForPlaces(), isTimed);
            }
            ...
            while (choices.hasNext()) {
                ...
                if (choice.getChoiceConfiguration().equals(ChoiceEnum.PROB)) {
                    ManagerXml.writePossibilityDependencies(bw, translator.variableTranslator.getCpnVarsForProbDep());
                    break;
                }
            }
            ...
            if (ManagerConfiguration.getInstance().isActivityLoggingEnabled()) {
                ManagerXml.writeLogDeclarations(bw, ManagerConfiguration.getInstance().getYearOffset(), process.getGlobalInfo().getTimeUnit());
            }
            ...
            if (ManagerConfiguration.getInstance().isResourcePerspectiveEnabled() && process.getResources().size() > 0 && ManagerConfiguration.getInstance().isResourcesMonitorEnabled()) {
                ManagerXml.writeMarkingSizeMonitorResources(bw, this.translator.getInfoForMonitoringResources());
                Message.add("Resource monitor has been written", Message.TEST);
            }
            if (ManagerConfiguration.getInstance().isThroughputTimeMonitorEnabled() && !isRedesign) {
                String dataVar = "";
                if (process.getAttributes().size() > 0 && ManagerConfiguration.getInstance().isDataPerspectiveEnabled()) {
                    dataVar = translator.variableTranslator.getCpnVarForDataAttributes().getVarName();
                }
                ManagerXml.writeThroughputTimeMonitor(bw, translator.getCpnIdMonitorTroughputTime(), translator.getPageinstancesMonitorThroughputTime(), dataVar, this.getHighLevelProcess().getHLProcess().getGlobalInfo().getTimeUnit());
        ...
        if (!isAlreadyWrittenToCPN()) {
            ...
            while (transitions.hasNext()) {
                ...
                if (transition.getSubpage() == null) {
                    if (this.getIdentifier().equals("Environment") && transition.getIdentifier().equals("Clean-up")) {
