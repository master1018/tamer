...
public class CashManager {
    ...
    public static MCash CloseCashBook(Properties ctx, CashBookDetailBean bean, String trxName) throws NullTransferAmountException, TransferAmountExceedsTotalAmountException, OperationException {
        ...
        if (bean.getTransferAllAmount().booleanValue()) bean.setTransferAmount(bean.getEndingBalanceAsString());
        if (bean.getTransferAmount() == null) throw new NullTransferAmountException("Transfer Amount can not be null");
        double transferAmt = Double.parseDouble(bean.getTransferAmount().replaceAll(",", ""));
        if (transferAmt > Double.parseDouble(bean.getEndingBalanceAsString())) throw new TransferAmountExceedsTotalAmountException("cannot transfer amount more than the ending balance");
        if (bean.getDocStatus().equals(DocumentEngine.STATUS_Completed) && transferAmt > 0) throw new CanNotCloseTillException("Already closed,Cannot close till ");
    ...
    public static void adjustCashBook(Properties ctx, CashBookDetailBean bean, String trxName) throws OperationException {
        ...
        if (bean.getAdjustmentAmount() == null) throw new NullTransferAmountException("Amount can not be null");
        if (cash == null) {
            int posId = Env.getContextAsInt(ctx, UdiConstants.POS_ID);
            MPOS pos = new MPOS(ctx, posId, trxName);
            int cashBookId = pos.getC_CashBook_ID();
        ...
        if (bean.getTransferType().equalsIgnoreCase(MCashLine.CASHTYPE_GeneralReceipts)) createCashLine(ctx, cash, new BigDecimal(bean.getAdjustmentAmount()), MCashLine.CASHTYPE_GeneralReceipts); else if (bean.getTransferType().equalsIgnoreCase(MCashLine.CASHTYPE_GeneralExpense)) {
            if (Double.parseDouble(bean.getAdjustmentAmount()) > cash.getEndingBalance().doubleValue()) throw new TransferAmountExceedsTotalAmountException("cannot transfer amount more than the ending balance");
    ...
    public static ArrayList<CashBookDetailBean> getCashBookDetails(Properties ctx) throws OperationException {
        ...
        String sql = "select * from " + "(select cash.C_CASH_ID," + "cash.C_CASHBOOK_ID," + "cash.NAME," + "cash.DESCRIPTION," + "cash.DATEACCT," + "cash.BEGINNINGBALANCE," + "cash.ENDINGBALANCE," + "cash.STATEMENTDIFFERENCE," + "cash.DOCSTATUS," + "book.name as bookName" + " from C_CASH cash,C_CASHBOOK book " + " where cash.C_CASHBOOK_ID=book.C_CASHBOOK_ID" + " and book.C_CashBook_ID = (SELECT Max(C_CashBook_ID)from C_CashBook WHERE" + " C_CASHBOOK_ID=" + cashBookId + ")" + " and book.ad_client_id=" + Env.getAD_Client_ID(ctx) + " and book.AD_ORG_ID=" + Env.getAD_Org_ID(ctx) + " order by cash.created desc) cashDets";
    ...
    public static ArrayList<CashBookDetailBean> getCashBookDetailsForTill(Properties ctx, int cashBookId, String fromDate, String toDate) throws OperationException {
        String sql1 = "select c_cash_id from C_CASH where C_CASHBOOK_ID=" + cashBookId + " and created between " + fromDate + " and " + toDate;
        String sql = "select nvl(sum(AMOUNT),0)" + " from C_CASHLINE" + " where C_CASH_ID in (" + sql1 + ")" + " and AD_CLIENT_ID= " + Env.getAD_Client_ID(ctx) + " and CASHTYPE='I'" + "and created between " + toDate + " and " + fromDate;
        CashBookDetailBean bean;
        ArrayList<CashBookDetailBean> list = new ArrayList<CashBookDetailBean>();
        PreparedStatement pstmt = DB.prepareStatement(sql, null);
        try {
    ...
    public static ArrayList<CashBookDetailBean> getCashBookDetailsFyracle(Properties ctx) throws OperationException {
        Integer cashBookId = Integer.valueOf(POSTerminalManager.getPOSDefaultCashBook(ctx).get_ID());
        String sql = "select cash.C_CASH_ID," + " cash.C_CASHBOOK_ID," + " cash.NAME," + " cash.DESCRIPTION," + " cash.DATEACCT," + " cash.BEGINNINGBALANCE," + " cash.ENDINGBALANCE," + " cash.STATEMENTDIFFERENCE," + " cash.DOCSTATUS," + " book.name bookName," + " cash.created, " + " cash.AD_CLIENT_ID," + " cash.AD_ORG_ID" + " from C_CASH cash,C_CASHBOOK book " + " where cash.C_CASHBOOK_ID=book.C_CASHBOOK_ID" + " and cash.C_CASHBOOK_ID=" + cashBookId + " and cash.ad_client_id=" + Env.getAD_Client_ID(ctx) + " and cash.ad_org_id=" + Env.getAD_Org_ID(ctx) + " order by cash.created desc";
        CashBookDetailBean bean;
        ArrayList<CashBookDetailBean> list = new ArrayList<CashBookDetailBean>();
        PreparedStatement pstmt = DB.prepareStatement(sql, null);
        try {
    ...
    public static void updateBeginningBalance(Properties ctx, int posId, String trxName) throws OperationException {
        ...
        if (bean2 != null) endingBalance = bean2.getEndingBalance();
        if (endingBalance == null) endingBalance = new BigDecimal(0);
    ...
    private static String getDataSQL(Properties ctx, int cashBookId) {
        String sql = "select * from " + " (select cash.C_CASH_ID," + "cash.C_CASHBOOK_ID," + "cash.ENDINGBALANCE," + "cash.DOCSTATUS," + "cash.AD_CLIENT_ID," + " cash.AD_ORG_ID," + " cash.created" + " from C_CASH cash,C_CASHBOOK book" + " where cash.C_CASHBOOK_ID=book.C_CASHBOOK_ID" + "@DOCSTATUS@" + " and book.C_CashBook_ID=" + cashBookId + " and cash.ad_client_id=?" + " and cash.AD_ORG_ID=? order by cash.Created Desc) cashDetails";
        return sql;
    }
    ...
    public static CashBean getData(Properties ctx, int cashBookId, boolean isLatest, String trxName) throws OperationException {
        ...
        if (isLatest) sql = sql.replaceAll("@DOCSTATUS@", " and cash.docstatus <> 'CO'"); else sql = sql.replaceAll("@DOCSTATUS@", " and cash.docstatus = 'CO'");
    ...
    private static MCash getMcash(Properties ctx, int C_CashBook_ID, Timestamp dateAcct, String trxName) throws OperationException {
        MCash retValue = null;
        String sql;
        sql = "SELECT * FROM C_Cash c " + "WHERE c.C_CashBook_ID=?" + " AND c.Processed='N'" + " AND c.docstatus='DR'";
        ...
        try {
            ...
            if (rs.next()) retValue = new MCash(ctx, rs, trxName);
            rs.close();
        } catch (Exception e) {
            throw new OperationException(e);
        } finally {
    ...
    public static CashBookDetailBean CloseTill(Properties ctx, CashBookDetailBean bean, String trxName) throws NullTransferAmountException, TransferAmountExceedsTotalAmountException, OperationException {
        ...
        if (cash == null) cash = MCash.get(ctx, cashBookId, JulianDate.getTodayDateOnly(), trxName);
        if (bean.getTransferAmount() == null) bean.setTransferAmount("0");
        ...
        if (bean.getBeginingBalance() == null) bean.setBeginingBalance(new BigDecimal(0));
        ...
        if (bean.getDocStatus().equals(DocumentEngine.STATUS_Completed) && transferAmt.doubleValue() > 0) throw new CanNotCloseTillException("Already closed,Cannot close till ");
        ...
        if (bean.getCardTotal() == null) bean.setCardTotal(new BigDecimal(0));
        if (bean.getChequeTotal() == null) bean.setChequeTotal(new BigDecimal(0));
        if (bean.getCardTotal() != null && !cardTotal.equals(bean.getCardTotal())) bean.setCardDifference(bean.getCardTotal().subtract(cardTotal)); else bean.setCardDifference(new BigDecimal(0));
        if (bean.getChequeTotal() != null && !chequeTotal.equals(bean.getChequeTotal())) bean.setChequeDifference(bean.getChequeTotal().subtract(chequeTotal)); else bean.setChequeDifference(new BigDecimal(0));
        cash.save();
        UDIMCash udiCash = new UDIMCash(cash);
        udiCash.processIt(DocumentEngine.ACTION_Complete);
        return bean;
    ...
    public static void closePreviousDraftedCashjournals(Properties ctx, int cashBookId, String trxName) throws OperationException {
        String sql = "SELECT C_CASH_ID FROM C_Cash c " + "WHERE c.C_CashBook_ID=?" + " AND c.Processed='N'" + " AND c.docstatus='DR'" + " AND TRUNC(c.StatementDate)<?" + " order by c.StatementDate asc";
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        MCash cash = null;
        BigDecimal endingBalance = null;
        try {
            ...
            while (rs.next()) {
                ...
                if (endingBalance != null) cash.setBeginningBalance(cash.getBeginningBalance().add(endingBalance));
                cash.save();
    ...
    public static ArrayList<CashBean> getCashDetails(Properties ctx, Integer month, Integer year, String trxName) throws OperationException {
        ...
        String sql = "Select C_Cash_ID, Name, Created, Updated, StatementDate, DateAcct, BeginningBalance, EndingBalance, DocStatus " + " from C_Cash" + " where AD_Client_ID=" + adClientId + " and AD_Org_ID in (" + userOrg + ")" + " and IsActive='Y'";
        if (month != null) {
        ...
        if (year != null) sql = sql + " and to_char(Created, 'yyyy') = '" + year + "'";
    ...
    public static MCash loadCash(Properties ctx, int cashId, String trxName) throws OperationException {
        ...
        if (cash.get_ID() == 0) throw new OperationException("Could not load cash with i12,435.96d: " + cashId);
        return cash;
    }

    public static CashSummaryBean getCashSummary(Properties ctx, int cashId, String trxName) throws OperationException {
        String sql = "select ca.CashType, sum(ca.Amount)" + " from C_CASHLINE ca where C_cash_ID=" + cashId + " group by ca.cashtype";
    ...
    public static CashSummaryBean getCashSummary(Properties ctx, String fromDate, String toDate, String trxName) throws OperationException {
        int adClientId = Env.getAD_Client_ID(ctx);
        String userOrg = Env.getContext(ctx, UdiConstants.USER_ORG_CTX_PARAM);
        String sql = "select ca.CashType, sum(ca.Amount)" + " from C_CASHLINE ca where ca.AD_Client_ID=" + adClientId + " and ca.AD_Org_ID in (" + userOrg + ")" + " and ca.Created between " + fromDate + " and " + toDate + " group by ca.cashtype";
        return getCashSummary(ctx, sql, trxName);
    }
    ...
    public static ArrayList<CashLineBean> getCashLineHistory(Properties ctx, String fromDate, String todate, String trxName) throws OperationException {
        int adClientId = Env.getAD_Client_ID(ctx);
        String userOrg = Env.getContext(ctx, UdiConstants.USER_ORG_CTX_PARAM);
        String sql = "select cl.C_CashLine_ID, cl.CashType, rl.name, cl.Created, cl.C_Cash_ID, cl.C_Invoice_ID, cl.Amount, cu.CurSymbol " + " from C_CashLine cl, AD_Ref_List rl, C_Currency cu " + " where cl.AD_Client_ID= " + adClientId + " and cl.AD_Org_ID in (" + userOrg + ") " + " and rl.AD_Reference_ID = " + MCashLine.CASHTYPE_AD_Reference_ID + " and cl.CashType = rl.Value" + " and cu.C_Currency_ID = cl.C_Currency_ID" + " and cl.Created between " + fromDate + " and " + todate + " order by cl.created";
    ...
    public static ArrayList<CashLineBean> getCashLineHistory(Properties ctx, int cashId, String trxName) throws OperationException {
        int adClientId = Env.getAD_Client_ID(ctx);
        String userOrg = Env.getContext(ctx, UdiConstants.USER_ORG_CTX_PARAM);
        String sql = "select cl.C_CashLine_ID, cl.CashType, rl.name, cl.Created, cl.C_Cash_ID, cl.C_Invoice_ID, cl.Amount, cu.CurSymbol " + " from C_CashLine cl, AD_Ref_List rl, C_Currency cu " + " where cl.AD_Client_ID= " + adClientId + " and cl.AD_Org_ID in (" + userOrg + ") " + " and rl.AD_Reference_ID = " + MCashLine.CASHTYPE_AD_Reference_ID + " and cl.CashType = rl.Value" + " and cu.C_Currency_ID = cl.C_Currency_ID" + " and cl.C_Cash_ID=" + cashId + " order by cl.created";
        return getCashLineHistory(ctx, sql, trxName);
    }
    ...
    public static MCashLine createCashLineForInvoice(Properties ctx, OpenItemBean bean, String trxName) throws OperationException {
        ...
        if (cashLines.length == 0) updateBeginningBalance(ctx, pos.get_ID(), trxName);
    ...
    public static WebDocumentBean getWebCashPaymentBean(Properties ctx, int cashLineId) throws OperationException, DataException {
        ...
        if (cashLine == null) throw new OperationException("Invalid operation payment is null");
        if (cashLine.get_ID() == 0) throw new OperationException("You have deleted this payment. You cannot view this payment.");
        ...
        if (meLocation.length == 0) throw new OperationException("No location has been set for your organisation. Please ask your administrator to set one for you");
        MLocation location = new MLocation(ctx, meLocation[0].getC_Location_ID(), null);
        if (location == null) throw new OperationException("You must have a location set for your business partner, Please ask your administrator to set one for you");
        ...
        if (youBPLocation.length == 0) throw new OperationException("No location has been set for the dealer organisation. Please ask your administrator to set one for you");
