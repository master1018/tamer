...
public class MathMLDocumentJSciImpl extends MathMLDocumentImpl {
    ...
    public Element createVector(DoubleVector v) {
        ...
        for (int i = 0; i < v.dimension(); i++) vector.appendChild(createNumber(v.getComponent(i)));
        return vector;
    }
    ...
    public Element createMatrix(DoubleMatrix m) {
        ...
        for (int j, i = 0; i < m.rows(); i++) {
            ...
            for (j = 0; j < m.columns(); j++) row.appendChild(createNumber(m.getElement(i, j)));
    ...
    public Element createSet(FiniteSet s) {
        ...
        while (iter.hasNext()) set.appendChild(createVariable(iter.next()));
        return set;
    }
    ...
    public Element createApply(MathMLExpression expr) {
        ...
        for (int i = 0; i < expr.length(); i++) {
            ...
            if (arg instanceof MathMLExpression) apply.appendChild(createApply((MathMLExpression) arg)); else if (arg instanceof MathDouble) apply.appendChild(createNumber(((MathDouble) arg).value())); else if (arg instanceof Complex) apply.appendChild(createNumber((Complex) arg)); else if (arg instanceof DoubleVector) apply.appendChild(createVector((DoubleVector) arg)); else if (arg instanceof DoubleMatrix) apply.appendChild(createMatrix((DoubleMatrix) arg)); else apply.appendChild(createVariable(arg));
        }
        return apply;
    }
    ...
    private void printNode(Writer out, Node n) throws IOException {
        if (n.hasChildNodes()) {
            ...
            for (int j = 0; j < attr.getLength(); j++) out.write(" " + attr.item(j).getNodeName() + "=\"" + attr.item(j).getNodeValue() + "\"");
            out.write(">");
            if (n.getFirstChild().getNodeType() != Node.TEXT_NODE) out.write("\n");
            final NodeList nl = n.getChildNodes();
            for (int i = 0; i < nl.getLength(); i++) printNode(out, nl.item(i));
            out.write("</" + n.getNodeName() + ">\n");
        } else {
            if (n.getNodeType() == Node.TEXT_NODE) {
                ...
                out.write("<" + n.getNodeName() + "/>");
                if (n.getNextSibling().getNodeType() != Node.TEXT_NODE) out.write("\n");
            }
        }
    }
}
