...
public class CStubGenerator {
    ...
    static String strsub(String src, char substitute) {
    ...
    class StubGenerator extends util.SignatureIterator {
        ...
        private void suffix(String fmt, String val, String rtn) {
            if (tracemode) {
                if (fmt == null) result += "	EXIT_VOID_STUB(" + quotedClassFunc + ");\n"; else result += "	EXIT_STUB(" + quotedClassFunc + ",\"" + fmt + "\"," + val + ");\n";
            }
            result += rtn;
        }
    ...
    private boolean printStub(MethodInfo m) {
        if ((m.access & Const.ACC_NATIVE) == 0) return false;
    ...
    private void dumpStubs(ClassInfo c) {
        className = strsub(c.className, CDelim);
        if ((c.methods == null) || (c.methods.length == 0)) return;
        prolog();
        MethodInfo t[] = c.methods;
        for (int i = 0; i < t.length; i++) {
            if ((t[i].access & Const.ACC_NATIVE) != 0) printStub(t[i]);
        }
        epilog();
    }

    public synchronized void writeStubs(ClassInfo c[], int nclasses, ClassnameFilterList nativeTypes) {
        fileProlog();
