...
import kr.ac.ssu.imc.durubi.report.designer.xml.DRXml;

public class DRRegion extends DRVirtualObject {
    ...
    public DRRegion(int nId, Point tStartLoc, Dimension tSize, DRManager tGlobalData, Vector Objects) {
        super(nId);
        rPoint = tStartLoc;
        rSize = tSize;
        rIdent = ++Count;
        ...
        for (int i = 0; i < Objects.size(); i++) {
            DRObject tList = ((DRObject) Objects.get(i));
        ...
        for (int i = 0; i < rObjects.size(); i++) {
            Vector a = ((DRObject) rObjects.get(i)).getQuerys();
            if (a.size() > 1) bCreate = false;
            if (a.size() == 1 && !((String) a.get(0)).equals(OwnQuery)) bCreate = false;
        }
        if (bCreate) {
            for (int i = 0; i < rObjects.size(); i++) {
                ((DRObject) rObjects.get(i)).RegionId = rIdent;
                ((DRObject) rObjects.get(i)).oSelectView.setStyle();
            }
        } else {
            Count--;
        }
    ...
    public void Free() {
        for (int i = 0; i < rObjects.size(); i++) {
            ((DRObject) rObjects.get(i)).RegionId = 0;
            ((DRObject) rObjects.get(i)).oSelectView.setStyle();
        }
    }

    public void View(DRReportCreate tGDDoc) {
        tGDDoc.getCurrentPage().unselectObjects();
        for (int i = 0; i < rObjects.size(); i++) {
            tGDDoc.getCurrentPage().selectObject((DRObject) rObjects.get(i), false);
        }
    }
    ...
    public static DRRegion createRDRegion(org.w3c.dom.Element tElement) {
        DRRegion tRegion = new DRRegion();
    ...
    public void SelectObject(Vector Objects) {
        rObjects = new Vector();
        for (int i = 0; i < Objects.size(); i++) {
            if (((DRObject) Objects.get(i)).RegionId == rIdent) {
                rObjects.add(Objects.get(i));
                ((DRObject) Objects.get(i)).oSelectView.setStyle();
            }
        }
    }
