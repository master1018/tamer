...
public final class Package {
    ...
    public static String getFullPackageDescription() {
        ...
        return new StringBuilder(pkg.getImplementationTitle()).append(" version ").append(pkg.getImplementationVersion()).append(" (").append(pkg.getImplementationVendor()).append(")").toString();
    }
    ...
    private static void throwVersionException(String name, String desired) {
        throw new RuntimeException(getProductName() + " only runs with '" + name + "' version " + desired + " as a minimum requirement! " + "If you are sure, that all is correct, you may disable this version checking by setting the Java system property '" + SKIP_VERSION_CHECK_PROPERTY + "' to 'true'.");
    }

    private static void checkPackage(java.lang.Package pkg, String name, String nameMatch, String desired) {
        String s;
        if (pkg == null || !(((s = pkg.getSpecificationTitle()) != null && s.toLowerCase(Locale.ENGLISH).indexOf(nameMatch) >= 0) || ((s = pkg.getImplementationTitle()) != null && s.toLowerCase(Locale.ENGLISH).indexOf(nameMatch) >= 0))) {
            log.warn("Cannot determine version of component '" + name + "'. " + getProductName() + " may work, but it would be better to check that a minimum version " + desired + " is installed in classpath!");
        } else if (!(isVersionCompatible(pkg.getSpecificationVersion(), desired) || isVersionCompatible(pkg.getImplementationVersion(), desired))) throwVersionException(name, desired);
    }
