...
public class DistanceBinsAssigner {
    ...
    private static final Logger log = Logger.getLogger(DistanceBinsAssigner.class);
    ...
    public DistanceBinsAssigner(double linearLimitDuration, String mode, Population plans, DistanceBins distanceBins, BinsOld durationDistanceBins) {
    ...
    private ArrayList<DesiredDurationPerson> movePlansWithLeisurePriorToWorkToHead(ArrayList<DesiredDurationPerson> linearBins) {
        ...
        Collections.reverse(linearBins);
        int cnt = 0;
        ...
        for (int j = 0; j < actslegs.size(); j = j + 2) {
            ...
            if (act.getType().startsWith("leisure")) {
                ...
                if (leg.getMode().equals(this.mode)) {
                    int randomDistance = (int) Math.round(this.distanceBins.getRandomDistance(desiredDurationPerson.planContainsLeisurePriorToWork()));
