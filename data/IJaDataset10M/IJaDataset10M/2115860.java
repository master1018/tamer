...
public class UO extends IndicatorPlugin {
    ...
    public void calculate() {
        ...
        int loop;
        for (loop = 0; loop < getBarData().size(); loop++) f.append(getBarData().getClose(loop) - getBarData().getLow(loop));
    ...
    PlotLine getTR() {
        ...
        for (loop = 0; loop < getBarData().size(); loop++) {
            ...
            double close;
            if (loop > 0) close = getBarData().getClose(loop - 1); else close = high;
            double t = high - low;
