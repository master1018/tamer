...
public abstract class AbstractInformationControlManager {
    ...
    private static final Anchor[] ANCHORS = { new Anchor(SWT.TOP), new Anchor(SWT.BOTTOM), new Anchor(SWT.LEFT), new Anchor(SWT.RIGHT) };
	 ...
	 * @since 2.1
	 */
    public static final Anchor ANCHOR_GLOBAL = new Anchor(SWT.CENTER);
    ...
    void setInformationControlReplacer(InformationControlReplacer replacer) {
        if (fInformationControlReplacer != null) fInformationControlReplacer.dispose();
    ...
    boolean canReplace(IInformationControl iControl) {
        return iControl instanceof IInformationControlExtension3 && iControl instanceof IInformationControlExtension5 && ((IInformationControlExtension5) iControl).getInformationPresenterControlCreator() != null;
    }
    ...
    protected void setCustomInformationControlCreator(IInformationControlCreator informationControlCreator) {
        if (informationControlCreator != null && fCustomInformationControlCreator instanceof IInformationControlCreatorExtension) {
            ...
            if (extension.canReplace(informationControlCreator)) return;
        }
    ...
    public void install(Control subjectControl) {
        if (fSubjectControl != null && !fSubjectControl.isDisposed() && fSubjectControlDisposeListener != null) fSubjectControl.removeDisposeListener(fSubjectControlDisposeListener);
        fSubjectControl = subjectControl;
        if (fSubjectControl != null) fSubjectControl.addDisposeListener(getSubjectControlDisposeListener());
        if (fInformationControlCloser != null) fInformationControlCloser.setSubjectControl(subjectControl);
        setEnabled(true);
        fDisposed = false;
    }
    ...
    protected Point computeSizeConstraints(Control subjectControl, IInformationControl informationControl) {
        if (fSizeConstraints == null) {
            if (informationControl instanceof IInformationControlExtension5) {
                ...
                if (fSizeConstraints != null) return Geometry.copy(fSizeConstraints);
            }
            if (subjectControl == null) return null;
    ...
    protected void handleInformationControlDisposed() {
        ...
        if (fInformationControl instanceof IInformationControlExtension5) fSizeConstraints = null;
    ...
    protected IInformationControl getInformationControl() {
        if (fDisposed) return fInformationControl;
        ...
        if (fCustomInformationControlCreator == null) {
            ...
            if (creator instanceof IInformationControlCreatorExtension) {
                ...
                if (fInformationControl != null && extension.canReuse(fInformationControl)) return fInformationControl;
            }
        ...
        if (fInformationControl == null) {
            ...
            if (fInformationControlCloser != null) fInformationControlCloser.setInformationControl(fInformationControl);
        }
        return fInformationControl;
    }
    ...
    protected boolean updateLocation(Point location, Point size, Rectangle displayArea, Anchor anchor) {
        ...
        if (ANCHOR_BOTTOM == anchor || ANCHOR_TOP == anchor) {
            if (ANCHOR_BOTTOM == anchor) {
                if (lowerRightY > displayLowerRightY) return false;
            } else {
                if (location.y < displayArea.y) return false;
            }
            if (lowerRightX > displayLowerRightX) location.x = location.x - (lowerRightX - displayLowerRightX);
            ...
            if (ANCHOR_RIGHT == anchor) {
                if (lowerRightX > displayLowerRightX) return false;
            } else {
                if (location.x < displayArea.x) return false;
            }
            if (lowerRightY > displayLowerRightY) location.y = location.y - (lowerRightY - displayLowerRightY);
            return (location.x >= displayArea.x && location.y >= displayArea.y);
    ...
    protected Anchor getNextFallbackAnchor(Anchor anchor) {
        if (anchor == null || fFallbackAnchors == null) return null;
        for (int i = 0; i < fFallbackAnchors.length; i++) {
            if (fFallbackAnchors[i] == anchor) return fFallbackAnchors[i + 1 == fFallbackAnchors.length ? 0 : i + 1];
        }
        return null;
    }
    ...
    protected Point computeInformationControlLocation(Rectangle subjectArea, Point controlSize) {
        ...
        do {
            ...
            if (updateLocation(upperLeft, controlSize, monitor.getClientArea(), testAnchor)) return upperLeft;
        ...
        if (bestAnchor != ANCHOR_GLOBAL) Geometry.set(controlSize, Geometry.getSize(bestBounds));
    ...
    private Monitor getClosestMonitor(Rectangle area, Anchor anchor) {
        Point center;
        if (ANCHOR_GLOBAL == anchor) center = Geometry.centerPoint(area); else center = Geometry.centerPoint(Geometry.getExtrudedEdge(area, 0, anchor.getSWTFlag()));
    ...
    protected void presentInformation() {
        ...
        if (fInformation instanceof String) hasContents = ((String) fInformation).trim().length() > 0; else hasContents = (fInformation != null);
        if (fSubjectArea != null && hasContents) internalShowInformationControl(fSubjectArea, fInformation); else hideInformationControl();
    }
    ...
    private void internalShowInformationControl(Rectangle subjectArea, Object information) {
        ...
        if (informationControl != null) {
            ...
            if (informationControl instanceof IInformationControlExtension2) ((IInformationControlExtension2) informationControl).setInput(information); else informationControl.setInformation(information.toString());
            ...
            if (bounds != null) {
                if (bounds.x > -1 && bounds.y > -1) location = Geometry.getLocation(bounds);
    ...
    protected void hideInformationControl() {
        if (fInformationControl != null) {
            ...
            if (fInformationControlCloser != null) fInformationControlCloser.stop();
        }
    }
    ...
    protected void showInformationControl(Rectangle subjectArea) {
        ...
        if (fTakesFocusWhenVisible) fInformationControl.setFocus();
        if (fInformationControlCloser != null) fInformationControlCloser.start(subjectArea);
    }
    ...
    protected void storeInformationControlBounds() {
        if (fDialogSettings == null || fInformationControl == null || !(fIsRestoringLocation || fIsRestoringSize)) return;
        if (!(fInformationControl instanceof IInformationControlExtension3)) throw new UnsupportedOperationException();
    ...
    protected Rectangle restoreInformationControlBounds() {
        if (fDialogSettings == null || !(fIsRestoringLocation || fIsRestoringSize)) return null;
        if (!(fInformationControl instanceof IInformationControlExtension3)) throw new UnsupportedOperationException();
        boolean controlRestoresSize = ((IInformationControlExtension3) fInformationControl).restoresSize();
        boolean controlRestoresLocation = ((IInformationControlExtension3) fInformationControl).restoresLocation();
        ...
        if (bounds.x == -1 && bounds.y == -1 && bounds.width == -1 && bounds.height == -1) return null;
        Rectangle maxBounds = null;
        if (fSubjectControl != null && !fSubjectControl.isDisposed()) maxBounds = fSubjectControl.getDisplay().getBounds(); else {
            Display display = Display.getCurrent();
            if (display == null) display = Display.getDefault();
            if (display != null && !display.isDisposed()) maxBounds = display.getBounds();
        }
