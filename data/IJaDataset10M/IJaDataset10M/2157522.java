...
public class Track {
    ...
    protected int _trainDir = EAST + WEST + NORTH + SOUTH;
    ...
    protected int _numberCarsInRoute = 0;
    ...
    protected String _loadOption = ALLLOADS;
    ...
    protected int _reservedInRoute = 0;
    ...
    public void setLocType(String type) {
        String old = _locType;
        _locType = type;
        if (!old.equals(type)) firePropertyChange(TRACK_TYPE_CHANGED_PROPERTY, old, type);
    }

    public void setLength(int length) {
        int old = _length;
        _length = length;
        if (old != length) firePropertyChange(LENGTH_CHANGED_PROPERTY, Integer.toString(old), Integer.toString(length));
    }
    ...
    public void setMinimumLength(int length) {
        ...
        if (old != length) firePropertyChange(MIN_LENGTH_CHANGED_PROPERTY, Integer.toString(old), Integer.toString(length));
    }
    ...
    public void setReserved(int reserved) {
        int old = _reserved;
        _reserved = reserved;
        if (old != reserved) firePropertyChange("reserved", Integer.toString(old), Integer.toString(reserved));
    }
    ...
    public void addReservedInRoute(Car car) {
        ...
        if (old != _reservedInRoute) firePropertyChange("reservedInRoute", Integer.toString(old), Integer.toString(_reservedInRoute));
    }
    ...
    public void setAlternativeTrack(Track track) {
        ...
        if (track != null) _alternativeTrackId = track.getId(); else _alternativeTrackId = "";
    ...
    public boolean isSpaceAvailable(Car car) {
        ...
        if (car.getKernel() != null) length = car.getKernel().getLength();
        if (getLength() * getReservationFactor() / 100 - (getReservedInRoute() + length) > 0) return true; else return false;
    }

    public void setUsedLength(int length) {
        int old = _usedLength;
        _usedLength = length;
        if (old != length) firePropertyChange("usedLength", Integer.toString(old), Integer.toString(length));
    }
    ...
    private void setNumberRS(int number) {
        int old = _numberRS;
        _numberRS = number;
        if (old != number) firePropertyChange("numberRS", Integer.toString(old), Integer.toString(number));
    }
    ...
    private void setNumberCars(int number) {
        int old = _numberCars;
        _numberCars = number;
        if (old != number) firePropertyChange("numberCars", Integer.toString(old), Integer.toString(number));
    }
    ...
    private void setNumberEngines(int number) {
        ...
        if (old != number) firePropertyChange("numberEngines", Integer.toString(old), Integer.toString(number));
    }
    ...
    public void addRS(RollingStock rs) {
        ...
        if (rs.getClass() == Car.class) setNumberCars(getNumberCars() + 1); else if (rs.getClass() == Engine.class) setNumberEngines(getNumberEngines() + 1);
    ...
    public void addPickupRS(RollingStock rs) {
        int old = _pickupRS;
        _pickupRS++;
        if (Setup.isBuildAggressive()) setReserved(getReserved() - (Integer.parseInt(rs.getLength()) + RollingStock.COUPLER));
    ...
    public String[] getTypeNames() {
        ...
        for (int i = 0; i < _typeList.size(); i++) types[i] = _typeList.get(i);
        return types;
    }
    ...
    private void setTypeNames(String[] types) {
        ...
        for (int i = 0; i < types.length; i++) _typeList.add(types[i]);
    }

    public void addTypeName(String type) {
        if (_typeList.contains(type)) return;
    ...
    public boolean acceptsTypeName(String type) {
        if (!CarTypes.instance().containsName(type) && !EngineTypes.instance().containsName(type)) return false;
        if (!_location.acceptsTypeName(type)) return false;
    ...
    public void setTrainDirections(int direction) {
        int old = _trainDir;
        _trainDir = direction;
        if (old != direction) firePropertyChange(TRAINDIRECTION_CHANGED_PROPERTY, Integer.toString(old), Integer.toString(direction));
    }
    ...
    public String[] getRoadNames() {
        ...
        for (int i = 0; i < _roadList.size(); i++) roads[i] = _roadList.get(i);
        if (_roadList.size() == 0) return roads;
    ...
    private void setRoadNames(String[] roads) {
        ...
        for (int i = 0; i < roads.length; i++) {
            if (!roads[i].equals("")) _roadList.add(roads[i]);
        }
    }

    public void addRoadName(String road) {
        if (_roadList.contains(road)) return;
        _roadList.add(road);
    ...
    private void setLoadNames(String[] loads) {
        ...
        for (int i = 0; i < loads.length; i++) {
            if (!loads[i].equals("")) _loadList.add(loads[i]);
        }
    }
    ...
    public String[] getLoadNames() {
        ...
        for (int i = 0; i < _loadList.size(); i++) loads[i] = _loadList.get(i);
        if (_loadList.size() == 0) return loads;
        jmri.util.StringUtil.sort(loads);
        return loads;
    }
    ...
    public boolean addLoadName(String load) {
        if (_loadList.contains(load)) return false;
        _loadList.add(load);
    ...
    public void setDropOption(String option) {
        ...
        _dropOption = option;
        if (!old.equals(option)) _dropList.clear();
    ...
    public void setPickupOption(String option) {
        ...
        if (!old.equals(option)) _pickupList.clear();
    ...
    public String[] getDropIds() {
        ...
        for (int i = 0; i < _dropList.size(); i++) ids[i] = _dropList.get(i);
        return ids;
    }

    private void setDropIds(String[] ids) {
        if (ids.length == 0) return;
        for (int i = 0; i < ids.length; i++) _dropList.add(ids[i]);
    }

    public void addDropId(String id) {
        if (_dropList.contains(id)) return;
        _dropList.add(id);
    ...
    public boolean acceptsDropTrain(Train train) {
        if (_dropOption.equals(ANY)) return true;
        if (getLocType().equals(YARD)) return true;
        if (_dropOption.equals(TRAINS)) return containsDropId(train.getId()); else if (train.getRoute() == null) return false;
        return acceptsDropRoute(train.getRoute());
    }

    public boolean acceptsDropRoute(Route route) {
        if (_dropOption.equals(ANY)) return true;
        if (_dropOption.equals(TRAINS)) return false;
    ...
    public String[] getPickupIds() {
        ...
        for (int i = 0; i < _pickupList.size(); i++) ids[i] = _pickupList.get(i);
        return ids;
    }

    private void setPickupIds(String[] ids) {
        if (ids.length == 0) return;
        for (int i = 0; i < ids.length; i++) _pickupList.add(ids[i]);
    }
    ...
    public void addPickupId(String id) {
        if (_pickupList.contains(id)) return;
        _pickupList.add(id);
    ...
    public boolean acceptsPickupTrain(Train train) {
        if (_pickupOption.equals(ANY)) return true;
        if (getLocType().equals(YARD)) return true;
        if (_pickupOption.equals(TRAINS)) return containsPickupId(train.getId()); else if (train.getRoute() == null) return false;
        return acceptsPickupRoute(train.getRoute());
    }

    public boolean acceptsPickupRoute(Route route) {
        if (_pickupOption.equals(ANY)) return true;
        if (_pickupOption.equals(TRAINS)) return false;
        ...
        if (Car.class.isInstance(rs)) {
            ...
            if (car.getKernel() != null && car.getKernel().isLead(car)) {
                ...
                for (int i = 0; i < cars.size(); i++) {
                    Car c = cars.get(i);
                    if (c.getTrack() != null && c.getTrack().equals(this) || c.getDestinationTrack() != null && c.getDestinationTrack().equals(this)) continue;
        ...
        if (Engine.class.isInstance(rs)) {
            ...
            if (eng.getConsist() != null && eng.getConsist().isLead(eng)) {
                ...
                for (int i = 0; i < engines.size(); i++) {
                    ...
                    if (e.getTrack() != null && e.getTrack().equals(this) || e.getDestinationTrack() != null && e.getDestinationTrack().equals(this)) continue;
                    length = length + Integer.parseInt(e.getLength()) + RollingStock.COUPLER;
                }
            }
        }
        if (rs.getTrack() != this && rs.getDestinationTrack() != this && (getUsedLength() + getReserved() + length) > getLength()) {
            if (getPool() != null && getPool().requestTrackLength(this, length)) return OKAY;
    ...
    public String getServiceOrder() {
        if (getLocType().equals(SIDING) || getLocType().equals(STAGING)) return NORMAL;
        return _order;
    }
    ...
    public String getScheduleName() {
        if (getScheduleId().equals("")) return "";
    ...
    public Schedule getSchedule() {
        if (getScheduleId().equals("")) return null;
    ...
    public String getScheduleId() {
        if (!getLocType().equals(Track.SIDING)) return "";
    ...
    public void setScheduleId(String id) {
        ...
        if (!old.equals(id)) {
            ...
            if (schedule.getItemsBySequenceList().size() > 0) setScheduleItemId(schedule.getItemsBySequenceList().get(0));
            setScheduleCount(0);
    ...
    public void bumpSchedule() {
        setMoves(getMoves() + 1);
        if (getScheduleMode() == Track.MATCH) return;
        setScheduleCount(getScheduleCount() + 1);
        if (getScheduleCount() < getCurrentScheduleItem().getCount()) return;
        setScheduleCount(0);
        getNextScheduleItem();
    }
    ...
    public String checkScheduleValid() {
        String status = "";
        if (getScheduleId().equals("")) return status;
        Schedule schedule = getSchedule();
        if (schedule == null) return MessageFormat.format(rb.getString("CanNotFindSchedule"), new Object[] { getScheduleId() });
    ...
    public void enableLoadSwaps(boolean enable) {
        if (enable) _loadOptions = _loadOptions | SWAP_GENERIC_LOADS; else _loadOptions = _loadOptions & 0xFFFF - SWAP_GENERIC_LOADS;
    }
    ...
    public void enableSetLoadEmpty(boolean enable) {
        if (enable) _loadOptions = _loadOptions | EMPTY_GENERIC_LOADS; else _loadOptions = _loadOptions & 0xFFFF - EMPTY_GENERIC_LOADS;
    }
    ...
    public void enableRemoveLoads(boolean enable) {
        if (enable) _loadOptions = _loadOptions | EMPTY_SCHEDULE_LOADS; else _loadOptions = _loadOptions & 0xFFFF - EMPTY_SCHEDULE_LOADS;
    }
    ...
    public void enableAddLoads(boolean enable) {
        if (enable) _loadOptions = _loadOptions | GENERATE_SCHEDULE_LOADS; else _loadOptions = _loadOptions & 0xFFFF - GENERATE_SCHEDULE_LOADS;
    }
    ...
    public void enableAddLoadsAnySiding(boolean enable) {
        if (enable) _loadOptions = _loadOptions | GENERATE_SCHEDULE_LOADS_ANY_SIDING; else _loadOptions = _loadOptions & 0xFFFF - GENERATE_SCHEDULE_LOADS_ANY_SIDING;
    }
    ...
    public String getPoolName() {
        if (getPool() != null) return getPool().getName();
        return "";
    }
    ...
    public Track(org.jdom.Element e, Location location) {
        ...
        if ((a = e.getAttribute("id")) != null) _id = a.getValue(); else log.warn("no id attribute in track element when reading operations");
    ...
    public org.jdom.Element store() {
        ...
        for (int i = 0; i < types.length; i++) {
            if (CarTypes.instance().containsName(types[i]) || EngineTypes.instance().containsName(types[i])) buf.append(types[i] + "%%");
        }
        ...
        if (getSchedule() != null) {
            ...
            if (getAlternativeTrack() != null) e.setAttribute("alternative", getAlternativeTrack().getId());
        }
        if (_loadOptions != 0) e.setAttribute("loadOptions", Integer.toString(_loadOptions));
        if (!getServiceOrder().equals(NORMAL)) e.setAttribute("order", getServiceOrder());
