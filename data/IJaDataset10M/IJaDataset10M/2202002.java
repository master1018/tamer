...
public class NetworkCapacitiesPainter extends NetworkPainter {
    ...
    public NetworkCapacitiesPainter(Network network, boolean isA, Map<Link, Tuple<Link, Double>> linksChanged) {
        ...
        for (Entry<Link, Tuple<Link, Double>> capacitiesL : linksChanged.entrySet()) {
            if (capacitiesL.getValue().getSecond() > maxCapacity) maxCapacity = capacitiesL.getValue().getSecond();
            if (capacitiesL.getValue().getSecond() < minCapacity) minCapacity = capacitiesL.getValue().getSecond();
            if (capacitiesL.getValue().getSecond() - capacitiesL.getKey().getCapacity() > maxChangeCapacityUp) maxChangeCapacityUp = capacitiesL.getValue().getSecond() - capacitiesL.getKey().getCapacity();
            if (capacitiesL.getValue().getSecond() - capacitiesL.getKey().getCapacity() < maxChangeCapacityDown) maxChangeCapacityDown = capacitiesL.getValue().getSecond() - capacitiesL.getKey().getCapacity();
        }
    ...
    public void paint(Graphics2D g2, LayersPanel layersPanel) {
        ...
        if (withSelected) paintSelected(g2, layersPanel);
    }
    ...
    private void paintSelected(Graphics2D g2, LayersPanel layersPanel) {
        ...
        if (link != null) paintLink(g2, layersPanel, link, selectedStroke, 2, selectedLinkColor);
        Node node = networkPainterManager.getSelectedNode();
        if (node != null) paintCross(g2, layersPanel, node.getCoord(), 4, selectedNodeColor);
    }
