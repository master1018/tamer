...
public class Ant {
    ...
    public Ant(Graph g) {
        ...
        current = -1;
        score = 0;
        for (int i = 0; i < g.size; i++) restliste.add(i);
    }
    ...
    public void step() {
        ...
        for (int i : restliste) {
            wert += (Math.pow(g.pheromon[current][i], ACO.alpha) * Math.pow(g.getPrior(current, i), ACO.beta)) / sum;
            if (rnd < wert) {
    ...
    public double[] getProbabilities() {
        ...
        double sum = calcSum();
        for (int i = 0; i < 5; i++) {
            if (restliste.contains(i)) prob[i] = (Math.pow(g.pheromon[current][i], ACO.alpha) * Math.pow(g.getPrior(current, i), ACO.beta)) / sum; else prob[i] = -1;
        }
        return prob;
    }
    ...
    private double calcSum() {
        double sum = 0;
        for (int i : restliste) {
            sum += Math.pow(g.pheromon[current][i], ACO.alpha) * Math.pow(g.getPrior(current, i), ACO.beta);
        }
        return sum;
    }
    ...
    public void dropPheromon() {
        ...
        for (int i = 0; i < tabuliste.size() - 1; i++) {
            g.pheromon[tabuliste.elementAt(i)][tabuliste.elementAt(i + 1)] += (1 - ACO.evaporation) * score;
        }
    }
}
