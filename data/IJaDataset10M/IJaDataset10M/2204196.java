...
public class HyperGraphStoreFactory implements ClauseStoreFactory {
    ...
    private HGHandle getTermHandle(JavaLibrary jl, Term t) {
        HGHandle h = null;
        t = t.getTerm();
        if (t instanceof HGAtomTerm) h = ((HGAtomTerm) t).getHandle(); else if (t instanceof Struct) {
            Struct s = (Struct) t;
            if (!s.isGround()) return null;
            Object x = null;
            try {
                if (s.getName().startsWith(jl.getDynamicObjectIdPrefix())) x = jl.getRegisteredDynamicObject(s); else x = jl.getRegisteredObject(s);
            } catch (Throwable ex) {
            }
            if (x instanceof HGHandle) h = (HGHandle) x; else if (x != null) {
                h = graph.getHandle(x);
                if (h == null) throw new RuntimeException("Expected an atom for " + t + " in HyperGraph query.");
            }
        }
        return h;
    }
    ...
    private HGSearchResult<?> query(HGQueryCondition cond, Prolog prolog, Struct struct) {
        ...
        for (int i = 0; i < struct.getArity(); i++) if ((atoms[i] = getTermHandle(jl, struct.getArg(i))) == null) atoms[i] = graph.getHandleFactory().anyHandle(); else atomConditions.add(hg.incident(atoms[i]));
        if (!atomConditions.isEmpty()) {
            And and;
            if (cond instanceof And) and = (And) cond; else {
                and = new And(cond);
            }
            for (HGQueryCondition c : atomConditions) and.add(c);
    ...
    public ClauseStore buildClause(Prolog prolog, Term goal, List<?> varList) {
        if (!(goal instanceof Struct)) return null;
        ...
        if (cond != null) {
            ...
            return new HyperGraphClauseStore(prolog, graph, struct.getName(), struct.getArity(), rs);
        }
        return null;
    }
}
