...
public class ERXFetchSpecificationBatchIterator implements Iterator, Enumeration {
    ...
    public void setBatchSize(int batchSize) {
        if (batchSize <= 0) throw new RuntimeException("Attempting to set a batch size of negative value.");
        if (batchSize > DefaultBatchSize) log.warn("Batches larger than the the default batch size of " + DefaultBatchSize + " might cause JDBC issues.");
    ...
    public NSArray batchWithRange(NSRange requestedRange) {
        ...
        NSRange range = requestedRange.rangeByIntersectingRange(new NSRange(0, count()));
        if (range.length() > 0) {
    ...
    protected NSArray primaryKeys() {
        if (primaryKeys == null) {
            if (editingContext() == null) throw new RuntimeException("Attempting to fetch the primary keys for a null editingContext");
            EOEntity entity = EOUtilities.entityNamed(editingContext(), fetchSpecification.entityName());
            if (entity.primaryKeyAttributes().count() > 1) throw new RuntimeException("ERXFetchSpecificationBatchIterator: Currently only single primary key entities are supported.");
            EOFetchSpecification pkFetchSpec = ERXEOControlUtilities.primaryKeyFetchSpecificationForEntity(editingContext(), fetchSpecification.entityName(), fetchSpecification.qualifier(), fetchSpecification.sortOrderings(), null);
    ...
    public Object next() {
        if (cachedBatch == null) {
            NSArray nextBatch = _fetchNextBatch();
            while (nextBatch.count() == 0 && hasNextBatch()) {
