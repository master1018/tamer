...
public class FieldsTest extends JDWPSyncTestCase {
    ...
    public void testFields001() {
        ...
        CommandPacket fieldsCommand = new CommandPacket(JDWPCommands.ReferenceTypeCommandSet.CommandSetID, JDWPCommands.ReferenceTypeCommandSet.FieldsCommand);
        ...
        String fieldNames[] = { "staticIntField", "stringField", "objectField" };
        String fieldSignatures[] = { "I", "Ljava/lang/String;", "Ljava/lang/Object;" };
        int fieldModifiers[] = { 0x8, 0x0, 0x0 };
        boolean fieldFound[] = { false, false, false };
        ...
        for (int i = 0; i < returnedFieldsNumber; i++) {
            ...
            for (; k < expectedFieldsNumber; k++) {
                ...
                if (fieldFound[k]) {
                    ...
                    failMessage = "The field is found repeatedly in the list: " + returnedFieldName;
                    break;
                }
                fieldFound[k] = true;
                if (!fieldSignatures[k].equals(returnedFieldSignature)) {
                    ...
                    testStatus = testStatusFailed;
                    failMessage = "Unexpected signature is returned for field: " + returnedFieldName + ", expected: " + fieldSignatures[k] + ", returned: " + returnedFieldSignature;
                }
                if (fieldModifiers[k] != returnedFieldModifiers) {
                    logWriter.println("\n## FAILURE: Unexpected field modifiers are returned:");
                    logWriter.println("## Field name = " + returnedFieldName);
                    logWriter.println("## Expected modifiers = 0x" + Integer.toHexString(fieldModifiers[k]));
                    logWriter.println("## Returned modifiers = 0x" + Integer.toHexString(returnedFieldModifiers));
                    testStatus = testStatusFailed;
                    failMessage = "Unexpected modifiers are returned for field: " + returnedFieldName + ", expected: 0x" + Integer.toHexString(fieldModifiers[k]) + ", returned: 0x" + Integer.toHexString(returnedFieldModifiers);
                }
                break;
            }
            if (k == expectedFieldsNumber) {
                ...
                logWriter.println("## Field modifiers = 0x" + Integer.toHexString(returnedFieldModifiers));
                testStatus = testStatusFailed;
                failMessage = "Unexpected returned field: " + returnedFieldName + ", signature = " + returnedFieldSignature + ", modifiers = 0x" + Integer.toHexString(returnedFieldModifiers);
            }
        }
        for (int k = 0; k < expectedFieldsNumber; k++) {
            if (!fieldFound[k]) {
                ...
                testStatus = testStatusFailed;
                failMessage = "Expected field is NOT found in the list of retuned fields: " + fieldNames[k];
            }
        }
        if (testStatus == testStatusPassed) {
            logWriter.println("=> CHECK PASSED: All expected fields are found out and have expected attributes");
        }
