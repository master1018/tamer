...
public class MessageBox2 extends Dialog implements LayoutManager {
    ...
    public void setBackground(Color c) {
        ...
        for (int i = 0; buttons != null && i < buttons.size(); i++) ((Component) buttons.elementAt(i)).setBackground(c);
    ...
    public void setForeground(Color c) {
        ...
        for (int i = 0; i < buttons.size(); i++) ((Component) buttons.elementAt(i)).setForeground(c);
    }

    public void setButtons(Vector b) {
        if (buttons != null) for (int i = 0; i < buttons.size(); i++) remove((Component) buttons.elementAt(i));
        buttons = b;
        for (int i = 0; i < buttons.size(); i++) add((Component) buttons.elementAt(i));
        invalidate();
        validate();
    }
    ...
    private Dimension getButtonsSize() {
        if (buttons == null || buttons.size() == 0) return new Dimension(0, 0);
        int h = 23, w = 48;
    ...
    public void layoutContainer(Container parent) {
        ...
        if (text != null) {
            ...
            if (width - tsize.width > 3 * addIc) addIc = 0; else addIc -= (width - tsize.width - addIc) / 2;
