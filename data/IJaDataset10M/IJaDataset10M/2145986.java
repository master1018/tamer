...
import traffic.object.MotionlessObject;
import traffic.object.MovingObject;
import traffic.object.Node;
import traffic.object.PointObject;
...
import traffic.object.River;
import traffic.object.RiverNode;
import traffic.object.Road;
import traffic.object.RoadCell;
import traffic.object.World;
...
public class WorldModel implements Constants {
    ...
    public void add(RealObject obj) {
        ...
        if (obj instanceof MovingObject) {
            ...
            if (obj instanceof AmbulanceTeam) ambulanceTeamList.add(obj);
            if (obj instanceof FireBrigade) fireBrigadeList.add(obj);
        }
    }
    ...
    public void update(InputBuffer in, int time) {
        ...
        while (true) {
            ...
            if (obj == null) {
                ...
                if (obj instanceof RealObject) add((RealObject) obj);
            }
            int prop;
    ...
    private RescueObject newRescueObject(int type, int id) {
        ...
        switch(type) {
            default:
                if (ASSERT) Util.myassert(false, "illeagle object type", type);
    ...
    private void setProperty(int property, InputBuffer data, RescueObject obj) {
        ...
        if (val != null) obj.input(property, val);
    }
    ...
    private void parseAK_MOVE(InputBuffer in) {
        ...
        for (int i = 0; i < length; i++) submitedRoute.add((MotionlessObject) get(path[i]));
    ...
    private void parseAK_LOAD(InputBuffer in) {
        ...
        if (ambulance.position() == mv.position() || (!(mv.position() instanceof Building) && (ambulance.position() instanceof Node && ((Node) ambulance.position()).edges().contains(mv.position()) || ambulance.position() instanceof Road && (((Road) ambulance.position()).head() == mv.position() || ((Road) ambulance.position()).tail() == mv.position())))) {
            ambulance.setLoad(mv);
        } else {
            printError(sender, "Wrong position: This AmbulanceTeam's position (" + ambulance.position().id + ") is not the same position of the target's (" + mv.position().id + ").");
        }
        return;
    }
    ...
    private void parseAK_UNLOAD(InputBuffer in) {
        ...
        AmbulanceTeam ambulance = (AmbulanceTeam) sender;
        if (ambulance.isLoading()) ambulance.setUnload(); else printError(sender, "This AmbulanceTeam is not loading an injured person now.");
        return;
    }

    private void printError(RealObject obj, String message) {
        if (PRINT_REASON_WHY_AGENT_COMMAND_WAS_NOT_EXECUTED) System.err.println("[Wrong Agent Command] time:" + WORLD.time() + ", agentID:" + obj.id + "\n  " + message);
    }
}
