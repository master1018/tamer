...
class ScriptFunction {
    ...
    void setVariables(Hashtable contextVariables, Vector params) {
        ...
        for (int i = names.size(); --i >= 0; ) {
            ...
            ScriptVariable var = (i < nParameters && i < nParams ? (ScriptVariable) params.get(i) : null);
            if (var != null && var.tok != Token.list) var = new ScriptVariable(var);
            contextVariables.put(name, (var == null ? (new ScriptVariable(Token.string, "")).setName(name) : var));
        }
    ...
    void addVariable(String name, boolean isParameter) {
        ...
        names.add(name);
        if (isParameter) nParameters++;
    }

    static void setFunction(ScriptFunction function, String script, int ichCurrentCommand, int pt, short[] lineNumbers, int[][] lineIndices, Vector lltoken) {
        ...
        for (int i = 0; i < nCommands; i++) {
            ...
            if (aatoken[i].length > 0) {
                ...
                if (Token.tokAttr(tokenCommand.tok, Token.flowCommand)) tokenCommand.intValue -= (tokenCommand.intValue < 0 ? -cmdpt0 : cmdpt0);
            }
        }
    ...
    public String toString() {
        ...
        if (script != null) s.append(script);
        s.append("}\n");
        return s.toString();
    }
}
