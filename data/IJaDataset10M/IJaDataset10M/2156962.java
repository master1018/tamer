...
public abstract class Schema {
    ...
    void setThemeData(Properties data) {
        ...
        while (keys.hasMoreElements()) {
            ...
            if (key.equalsIgnoreCase(HOBooleanName.IMAGEPANEL_BG_PAINTED)) cache.put(key, Boolean.valueOf(value.equalsIgnoreCase("true"))); else cache.put(key, createObjectFromXmlValue(value));
        }
    }
    ...
    private Object createObjectFromXmlValue(String value) {
        if (Pattern.matches("\\d{1,3}(,\\d{1,3}){1,3}", value)) {
            ...
            if (rgb.length == 3) return new Color(r, g, b); else if (rgb.length == 4) {
