 ...
 * @version $Id: Class.java 1941 2004-09-30 03:37:06Z joewhaley $
 */
public class Class {
    ...
    private static Class forName0(java.lang.String name, boolean initialize, ClassLoader loader) throws java.lang.ClassNotFoundException {
    ...
    private java.lang.Object newInstance0() throws java.lang.InstantiationException, java.lang.IllegalAccessException {
        jq_Type jq_type = this.jq_type;
        if (!jq_type.isClassType()) throw new java.lang.InstantiationException(jq_type.getDesc() + " is not a class type");
        jq_Class jq_class = (jq_Class) jq_type;
        jq_class.load();
        if (jq_class.isAbstract()) throw new java.lang.InstantiationException("cannot instantiate abstract " + this);
        jq_Initializer i = jq_class.getInitializer(new jq_NameAndDesc(Utf8.get("<init>"), Utf8.get("()V")));
        if (i == null) throw new java.lang.InstantiationException("no empty arg initializer in " + this);
    ...
    public java.lang.String getName() {
        jq_Type jq_type = this.jq_type;
        if (jq_type.isArrayType()) return jq_type.getDesc().toString().replace('/', '.'); else return jq_type.getName().toString();
    }
    ...
    public java.lang.Class[] getInterfaces() {
        ...
        jq_Class[] ins;
        jq_type.load();
        if (jq_type.isArrayType()) ins = jq_Array.array_interfaces; else ins = ((jq_Class) jq_type).getDeclaredInterfaces();
    ...
    private java.lang.reflect.Field[] getFields0(int which) {
        ...
        if (which == java.lang.reflect.Member.DECLARED) {
            ...
            int current = -1;
            for (int j = 0; j < sfs.length; ++j) {
                if (sfs[j].isPublic()) fs[++current] = (java.lang.reflect.Field) sfs[j].getJavaLangReflectMemberObject();
            }
            for (int j = 0; j < ifs.length; ++j) {
                if (ifs[j].isPublic()) fs[++current] = (java.lang.reflect.Field) ifs[j].getJavaLangReflectMemberObject();
            }
    ...
    private java.lang.reflect.Method[] getMethods0(int which) {
        ...
        if (which == java.lang.reflect.Member.DECLARED) {
            ...
            for (int j = 0; j < sfs.length; ++j) {
                if (sfs[j] instanceof jq_ClassInitializer) continue;
                if (sfs[j].isPublic()) fs[++k] = (java.lang.reflect.Method) sfs[j].getJavaLangReflectMemberObject();
            }
            for (int j = 0; j < ifs.length; ++j) {
                if (ifs[j] instanceof jq_Initializer) continue;
                if (ifs[j].isPublic()) fs[++k] = (java.lang.reflect.Method) ifs[j].getJavaLangReflectMemberObject();
            }
            Assert._assert(k == fs.length - 1);
            return fs;
        }
    ...
    private java.lang.reflect.Constructor[] getConstructors0(int which) {
        ...
        int size = 0;
        for (int j = 0; j < ifs.length; ++j) {
            if (which == java.lang.reflect.Member.PUBLIC && !ifs[j].isPublic()) continue;
            if (ifs[j] instanceof jq_Initializer) ++size;
        }
    ...
    private java.lang.reflect.Field getField0(java.lang.String name, int which) throws java.lang.NoSuchFieldException {
        jq_Type jq_type = this.jq_type;
        if (!jq_type.isClassType()) throw new java.lang.NoSuchFieldException(jq_type.getJDKDesc());
        jq_Class c = (jq_Class) jq_type;
        c.load();
    ...
    private java.lang.reflect.Method getMethod0(java.lang.String name, java.lang.Class[] parameterTypes, int which) throws java.lang.NoSuchMethodException {
        jq_Type jq_type = this.jq_type;
        if (!jq_type.isClassType()) throw new java.lang.NoSuchMethodException(jq_type.getJDKDesc());
        if (name.equals("<init>") || name.equals("<clinit>")) throw new java.lang.NoSuchMethodException(name + " is an illegal method name");
        jq_Class c = (jq_Class) jq_type;
        ...
        for (int i = 0; i < sms.length; ++i) {
            if (sms[i].getName() == utf8name) {
                if (which == java.lang.reflect.Member.PUBLIC && !sms[i].isPublic()) continue;
                if (sms[i].getDesc().toString().startsWith(args)) return (java.lang.reflect.Method) sms[i].getJavaLangReflectMemberObject();
            }
        }
        if (which == java.lang.reflect.Member.DECLARED) {
    ...
    private java.lang.reflect.Constructor getConstructor0(java.lang.Class[] parameterTypes, int which) throws java.lang.NoSuchMethodException {
        jq_Type jq_type = this.jq_type;
        if (!jq_type.isClassType()) throw new java.lang.NoSuchMethodException(jq_type.getJDKDesc());
        jq_Class c = (jq_Class) jq_type;
        ...
        if (i == null || (which == java.lang.reflect.Member.PUBLIC && !i.isPublic())) throw new java.lang.NoSuchMethodException(c.getJDKName() + utf8desc);
    ...
    public static Utf8 buildMethodDescriptor(java.lang.Class[] args, java.lang.Class returnType) {
        ...
        sb.append(')');
        if (returnType == null) sb.append('V'); else sb.append(Reflection.getJQType(returnType).getDesc().toString());
