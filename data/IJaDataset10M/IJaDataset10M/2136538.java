package org.jjsip.sip.transaction;

import org.jjsip.sip.message.Message;
import org.jjsip.sip.provider.ConnectionIdentifier;
import org.jjsip.sip.provider.SipProvider;
import org.jjsip.sip.provider.SipStack;
import org.jjsip.sip.provider.TransactionIdentifier;
import org.jjsip.tools.LogLevel;
import org.jjsip.tools.Timer;

/**
 * Generic server transaction as defined in RFC 3261 (Section 17.2.2). A
 * TransactionServer is responsible to create a new SIP transaction that starts
 ...
 * When constructing a new TransactionServer, the transaction type is passed as
 * String parameter to the constructor (e.g. "CANCEL", "BYE", etc..)
 */
public class TransactionServer extends Transaction {
    ...
    /** Constructs a new TransactionServer. */
    ...
    public void respondWith(Message resp) {
        ...
        if (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING)) {
            ...
            if (code >= 200 && code < 700) {
                ...
                if (connection_id == null) clearing_to.start(); else {
