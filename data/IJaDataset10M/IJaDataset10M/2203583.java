...
public class FlattenPostProcessor implements PostProcessor {
    ...
    public void postProcess(Sentence sentence) {
        ...
        while (unhandledMentions.size() > 0) {
            ...
            if (overlapping.size() > 1) flattenType.handle(sentence, overlapping);
    ...
    private static Set<Mention> getOverlappingMentions(Sentence sentence, Mention mention) {
        ...
        for (Mention mention2 : sentence.getMentions()) if (mention.overlaps(mention2)) overlapping.add(mention2);
        return overlapping;
    }
    ...
    public enum FlattenType {
        ...
        , HighestProbability {
            ...
            public void handle(Sentence sentence, Set<Mention> overlapping) {
                ...
                for (Mention mention : overlapping) if (bestMention == null || mention.getProbability() > bestMention.getProbability()) bestMention = mention;
                for (Mention mention : overlapping) if (mention != bestMention) sentence.removeMention(mention);
            }
        }
        ;
