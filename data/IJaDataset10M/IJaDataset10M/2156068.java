...
public class SubjectPublicKeyInfo extends ASN1Sequence {
    ...
    public SubjectPublicKeyInfo(AlgorithmIdentifier aid, byte[] key) {
        super(2);
        if (aid == null || key == null) throw new NullPointerException("Some arg is null!");
        algorithm_ = aid;
        add(algorithm_);
    ...
    public ASN1Type getDecodedRawKey() throws CorruptedCodeException {
        ...
        ASN1Type raw;
        try {
            dec = new DERDecoder(new ByteArrayInputStream(encodedKey_.getBytes()));
            raw = dec.readType();
            dec.close();
            return raw;
        } catch (ASN1Exception e) {
    ...
    public void setAlgorithmIdentifier(AlgorithmIdentifier aid) {
        if (aid == null) throw new NullPointerException("Need an algorithm identifier!");
        set(0, aid);
        algorithm_ = aid;
    }
    ...
    public ASN1Type getKeyStruct(boolean derDecode) throws CorruptedCodeException {
        if (derDecode) return getDecodedRawKey();
        return encodedKey_;
    }
