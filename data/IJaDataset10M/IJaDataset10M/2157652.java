...
public class ImageSequenceImpl extends FieldImpl implements ImageSequence {
    ...
    private static SampledSet makeDomainSet(SingleBandedImage[] images) throws VisADException, RemoteException {
        if (images == null) throw new VisADException("images can't be null");
        DateTime[] startTimes = new DateTime[images.length];
        for (int i = 0; i < images.length; i++) {
            startTimes[i] = images[i].getStartTime();
        }
        return (startTimes.length > 1) ? (SampledSet) DateTime.makeTimeSet(startTimes) : (SampledSet) new SingletonSet(new RealTuple(new Real[] { startTimes[0] }));
    }
    ...
    public DateTime[] getImageTimes() throws VisADException {
        DateTime[] times = null;
        if (getDomainSet().getLength() > 1) times = DateTime.timeSetToArray((Gridded1DDoubleSet) getDomainSet()); else times = new DateTime[] { new DateTime(((SingletonSet) getDomainSet()).getDoubles()[0][0]) };
    ...
    private static FunctionType ensureFunctionType(FunctionType type) throws VisADException {
        if (type.getDomain().equals(DataUtility.ensureRealTupleType(RealType.Time)) && type.getRange() instanceof FunctionType && ((RealTupleType) ((FunctionType) type.getRange()).getFlatRange()).getDimension() == 1) return type; else throw new VisADException("Not a valid ImageSequence type: " + type);
    }
}
