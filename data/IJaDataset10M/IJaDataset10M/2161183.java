...
public class Call extends Step {
    ...
    public void setMethodName(String mn) {
        if (mn == null) usage(Strings.get("MethodNameMissing"));
        methodName = mn;
    }
    ...
    public void setTargetClassName(String cn) {
        if (cn == null) usage(Strings.get("ClassNameMissing"));
        targetClassName = cn;
    }
    ...
    protected Object invoke() throws Throwable {
        try {
            ...
            Object[] params = (Object[]) ArgumentParser.eval((Resolver) getResolver(), (String[]) args, (Class[]) m.getParameterTypes());
    ...
    protected Method resolveMethod(String name, Class cls, Class returnType) {
        ...
        for (int i = 0; i < mlist.length; i++) {
            ...
            if (m.getName().equals(name) && params.length == args.length && (returnType == null || m.getReturnType().equals(returnType))) {
                found.add(m);
            }
        }
        if (found.size() == 0) {
            throw new IllegalArgumentException(Strings.get("NoMatchingMethod", new Object[] { name, (returnType == null ? "void" : returnType.toString()), String.valueOf(args.length), cls }));
