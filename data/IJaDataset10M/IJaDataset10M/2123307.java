    ...
    void updateMouse() {
        if (Mouse.isCreated()) {
            while (Mouse.next()) {
                ...
                int touchY = Math.round(((float) (LSystem.screenRect.height * LSystem.scaleHeight) - Mouse.getEventY() - 1) / LSystem.scaleHeight);
                ...
                switch(finalTouch.type) {
                    case Touch.TOUCH_DOWN:
                        if (handler.emulatorButtons != null && handler.emulatorButtons.isVisible()) {
                            handler.emulatorButtons.hit(finalTouch.x(), finalTouch.y());
                        }
