...
public class HtmlUnitWebElement implements RenderedWebElement, FindsById, FindsByLinkText, FindsByXPath, FindsByTagName, WrapsDriver {
    ...
    public void click() {
        ...
        if (!isDisplayed()) throw new ElementNotVisibleException("You may only click visible elements");
        try {
    ...
    public void sendKeys(CharSequence... value) {
        ...
        if (!isDisplayed()) throw new ElementNotVisibleException("You may only sendKeys to visible elements");
        ...
        if (parent.isJavascriptEnabled() && !oldActiveElement.equals(this) && !oldActiveElement.getTagName().toLowerCase().equals("body")) {
    ...
    private int indexOfSubmitKey(HtmlElement element, StringBuilder builder) {
        if (!(element instanceof HtmlInput)) return -1;
    ...
    public boolean toggle() {
        assertElementNotStale();
        if (!isDisplayed()) throw new ElementNotVisibleException("You may only toggle visible elements");
        try {
    ...
    public void setSelected() {
        assertElementNotStale();
        if (!isDisplayed()) throw new ElementNotVisibleException("You may only select visible elements");
    ...
    public boolean isDisplayed() {
        assertElementNotStale();
        if (!parent.isJavascriptEnabled()) return true;
