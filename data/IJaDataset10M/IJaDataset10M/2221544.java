 ...
 *
 */
public abstract class BasicEscapeDirective extends SoyAbstractTofuPrintDirective implements SoyJsSrcPrintDirective, SoyJavaSrcPrintDirective {
   ...
   */
    ...
    public JavaExpr applyForJavaSrc(JavaExpr value, List<JavaExpr> args) {
        return new JavaExpr(JavaCodeUtils.genNewStringData(JavaCodeUtils.genFunctionCall(JavaCodeUtils.UTILS_LIB + ".$$" + name.substring(1), JavaCodeUtils.genMaybeCast(value, SoyData.class))), StringData.class, Integer.MAX_VALUE);
    }

    @Override
    public JsExpr applyForJsSrc(JsExpr value, List<JsExpr> args) {
        return new JsExpr("soy.$$" + name.substring(1) + "(" + value.getText() + ")", Integer.MAX_VALUE);
    }
}
