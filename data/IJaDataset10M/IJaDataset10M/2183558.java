    ...
    public static void classify(Iterable<Document> testFilenames, boolean inMemory) {
        ...
        if (runClassifier) {
            ...
            for (Document doc : testFilenames) {
                if (inMemory) learner.test(doc.getFeatureReader(), doc.getPredictionWriter(), options); else learner.test(doc.getFeatureFile(), doc.getPredictionFile(), fullModelFN, options);
            }
        }
    }
