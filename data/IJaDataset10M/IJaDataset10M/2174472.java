...
public class FilterDescriptor implements Comparable, IPluginContribution {
    ...
    private static final String EXTENSION_POINT_NAME = "rubyElementFilters";
    ...
    public static FilterDescriptor[] getFilterDescriptors(String targetId) {
        ...
        for (int i = 0; i < filterDescs.length; i++) {
            ...
            if (WorkbenchActivityHelper.filterItem(filterDescs[i])) continue;
            if (tid == null || tid.equals(targetId)) result.add(filterDescs[i]);
        }
    ...
    public String getId() {
        if (isPatternFilter()) {
            ...
            if (targetId == null) return PATTERN_FILTER_ID_PREFIX + getPattern(); else return targetId + PATTERN_FILTER_ID_PREFIX + getPattern();
        } else return fElement.getAttribute(ID_ATTRIBUTE);
    }
    ...
    public String getName() {
        ...
        if (name == null && isPatternFilter()) name = getPattern();
        return name;
    }
    ...
    public boolean isEnabled() {
        ...
        if (strVal == null) strVal = fElement.getAttribute(SELECTED_ATTRIBUTE);
        return strVal == null || Boolean.valueOf(strVal).booleanValue();
    }

    public int compareTo(Object o) {
        if (o instanceof FilterDescriptor) return Collator.getInstance().compare(getName(), ((FilterDescriptor) o).getName()); else return Integer.MIN_VALUE;
    }
