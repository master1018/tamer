...
public class Thief_UndergroundConnections extends ThiefSkill {
    ...
    public String name() {
        return "Underground Connections";
    }
    ...
    public boolean tick(Tickable ticking, int tickID) {
        if ((pathOut != null) && (tickID == Tickable.TICKID_MOB)) {
            if ((pathOut.size() == 0) || (theGroup == null) || (currRoom == null)) unInvoke(); else {
                    ...
                    if ((lastDesc == null) || (!roomDesc.equalsIgnoreCase(lastDesc))) {
                        ...
                        for (int g = 0; g < theGroup.size(); g++) {
                            ...
                            switch(CMLib.dice().roll(1, 10, 0)) {
                                case 1:
                                    M.tell("You think you are being taken through '" + roomDesc + "'.");
                                    break;
                                case 2:
                                    M.tell("You might be going through '" + roomDesc + "' now.");
                                    break;
                                case 3:
                                    M.tell("Now you are definitely going through '" + roomDesc + "'.");
                                    break;
                                case 4:
                                    M.tell("You are being taken through '" + roomDesc + "', you think.");
                                    break;
                                case 5:
                                    M.tell("Sounds like '" + roomDesc + "' now.");
                                    break;
                                case 6:
                                    M.tell("Now this might be '" + roomDesc + "'.");
                                    break;
                                case 7:
                                    M.tell("You're going through '" + roomDesc + "'.");
                                    break;
                                case 8:
                                    M.tell("Sounds like this could be '" + roomDesc + "'.");
                                    break;
                                case 9:
                                    M.tell("You are probably going through '" + roomDesc + "' now.");
                                    break;
                                case 10:
                                    M.tell("Sounds like you are being taken through '" + roomDesc + "'.");
                                    break;
                            }
                        }
                    }
    ...
    public void unInvoke() {
        if (pathOut != null) {
            ...
            if (theGroup != null) for (int g = 0; g < theGroup.size(); g++) {
                MOB M = (MOB) theGroup.elementAt(g);
            ...
            if (storage != null) for (int s = 0; s < storage.size(); s++) ((Room) storage.elementAt(s)).destroy();
            pathOut = null;
            currRoom = null;
    ...
    public void bringMOBSHere(Room newRoom, Vector group, String enterStr, String leaveStr) {
        for (int g = group.size() - 1; g >= 0; g--) {
            ...
            if (!bringMOBHere(newRoom, follower, enterStr, leaveStr)) group.removeElementAt(g);
        }
    }
    ...
    public void bringMOBSLikeHere(Vector rooms, Room newRoom, Vector group, String enterStr, String leaveStr) {
        for (int g = group.size() - 1; g >= 0; g--) {
            ...
            if (!bringMOBHere(R, follower, enterStr, leaveStr)) group.removeElementAt(g); else rooms.addElement(R);
        }
    }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        if ((!CMLib.law().isACity(A)) && (!auto)) {
            mob.tell("You can only use this skill in cities.");
            return false;
        }
        TrackingLibrary.TrackingFlags flags = new TrackingLibrary.TrackingFlags();
        flags.add(TrackingLibrary.TrackingFlag.NOEMPTYGRIDS).add(TrackingLibrary.TrackingFlag.NOAIR).add(TrackingLibrary.TrackingFlag.NOWATER);
        ...
        for (int c = 0; c < trail.size(); c++) {
            ...
            if ((R.getArea() != A) && (!CMath.bset(R.domainType(), Room.INDOORS)) && (CMLib.flags().canAccess(target, R))) {
                Room checkR = null;
                for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
                    checkR = R.getRoomInDir(d);
                    if ((checkR != null) && (checkR.getArea() == A) && (!CMath.bset(checkR.domainType(), Room.INDOORS)) && (trail.indexOf(checkR) < c)) {
        ...
        if (!success) return beneficialVisualFizzle(mob, null, auto ? "" : "<S-NAME> can't seem to contact <S-HIS-HER> underground connections here."); else if (mob.location().okMessage(mob, msg)) {
            ...
            if (underA != null) {
                ...
                for (Iterator i = H.iterator(); i.hasNext(); ) {
                    MOB M = (MOB) i.next();
                    if ((M != null) && (M.location() == thisRoom) && (CMLib.flags().isInTheGame(M, true)) && (!group.contains(M))) group.addElement(M);
                }
