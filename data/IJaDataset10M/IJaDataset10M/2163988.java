...
public class PMS {
	 ...
	 */
    @Deprecated
    public static final SimpleDateFormat sdfDate = new SimpleDateFormat("HH:mm:ss.SSS", Locale.US);

    /**
	 * @deprecated This field is not used and will be removed in the future. 
	 */
    @Deprecated
    public static final SimpleDateFormat sdfHour = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.US);
    ...
    private boolean init() throws Exception {
        ...
        if (profileFile.exists()) {
            String status = String.format("%s%s", profileFile.canRead() ? "r" : "-", profileFile.canWrite() ? "w" : "-");
    ...
    public void registerPlayer(Player p) {
        ...
        if (ok) {
            logger.info("Registering transcoding engine: " + p);
            players.add(p);
        }
    }
