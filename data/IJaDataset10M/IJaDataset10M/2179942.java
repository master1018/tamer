...
public class IntervalEncoder {
    ...
    public static void encode(long size, GGEP g, IntervalSet s) {
        ...
        for (int i : s.encode(size)) {
            long l = i & 0xFFFFFFFF;
            if (l > 0xFFFFFFL) ints.add(i); else if (l > 0xFFFF) b24.add(i); else if (l > 0xFF) shorts.add(i); else bytes.add(i);
        }
        ...
        if (bytes.size() + shorts.size() + b24.size() + ints.size() == 0) g.put(GGEP.GGEP_HEADER_PARTIAL_RESULT_PREFIX + "0");
    }
    ...
    public static IntervalSet decode(long size, GGEP ggep) throws BadGGEPPropertyException {
        if (ggep.hasKey(GGEP.GGEP_HEADER_PARTIAL_RESULT_PREFIX + "0")) return new IntervalSet();
