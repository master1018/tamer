...
public class TestSetSubjectIndicator extends AbstractWebedTestCase {
    ...
    public void testNormalOperation() throws IOException {
        ...
        ActionParametersIF params = makeParameters(makeList(topic, newSL), "http://www.freshmeat.net");
        ActionResponseIF response = makeResponse();
        action.perform(params, response);
        assertFalse("Topic not added", numSLs > topic.getSubjectIdentifiers().size());
        LocatorIF loc = (LocatorIF) topic.getSubjectIdentifiers().iterator().next();
        assertFalse("SI not set to new value", !(loc.getAddress().equals("http://www.freshmeat.net/")));
    }
    ...
    public void testWrongURL() throws IOException {
        TopicIF topic = getTopicById(tm, "gamst");
        LocatorIF newSL = new URILocator("http://www.slashdot.org");
        ActionParametersIF params = makeParameters(makeList(topic, newSL), "snurrepip");
        ActionResponseIF response = makeResponse();
        try {
            action.perform(params, response);
            fail("Bad url i req");
    ...
    public void testSameURL() throws IOException {
        ...
        int numSLs = topic.getSubjectIdentifiers().size();
        LocatorIF newSL = new URILocator("http://www.slashdot.org");
        ActionParametersIF params = makeParameters(makeList(topic, newSL), "http://www.slashdot.org");
        ActionResponseIF response = makeResponse();
        action.perform(params, response);
        assertFalse("added another SA with same URL", numSLs < topic.getSubjectIdentifiers().size());
    }
    ...
    public void testEmptyURL() throws IOException {
        ...
        assertTrue("Number of subject indicators changed from " + sisbefore + " to " + sisnow, sisbefore == sisnow);
    }
}
