...
public abstract class AbstractSessionManager implements SessionManager {
    ...
    protected static final int GLOBAL_SESSION_TIMEOUT = 60 * 30;
    ...
    public void init() {
        if (sessionDAO == null) {
            String msg = "sessionDAO property has not been set.  The sessionDAO is required to " + "access session objects during runtime.";
        ...
        if (sessionEventSender == null) {
            if (log.isInfoEnabled()) {
                String msg = "sessionEventSender property has not been set.  SessionEvents will " + "not be propagated.";
                log.info(msg);
            }
        }
    }
    ...
    protected void send(SessionEvent event) {
        if (this.sessionEventSender != null) {
            ...
            if (log.isTraceEnabled()) {
                String msg = "No sessionEventSender set.  Event of type [" + event.getClass().getName() + "] will not be propagated.";
                log.trace(msg);
            }
        }
    }
    ...
    protected void validate(InetAddress originatingHost) throws IllegalArgumentException {
        if (originatingHost == null) {
            String msg = "originatingHost argument is null.  A valid non-null originating " + "host address must be specified when initiating a session";
    ...
    protected void validate(Session session) throws InvalidSessionException {
        if (isExpired(session)) {
            ...
            String msg = "Session with id [" + sessionId + "] has expired. " + "Last access time: " + df.format(lastAccessTime) + ".  Current time: " + df.format(new Date()) + ".  Session timeout is set to " + timeout + " seconds (" + timeout / 60 + " minutes)";
            throw new ExpiredSessionException(msg, sessionId);
        }
        if (session.getStopTimestamp() != null) {
            String msg = "Session with id [" + session.getSessionId() + "] has been " + "explicitly stopped.  No further interaction under this session is " + "allowed.";
    ...
    protected Session newSessionInstance() {
        try {
            ...
            String msg = "Unable to instantiate an instance of class [" + getSessionClass().getName() + "]";
    ...
    protected Session createSession(InetAddress originatingHost) {
        ...
        if (isValidateHost()) {
            if (log.isDebugEnabled()) {
                log.debug("Host validation enabled.  Validating originating host [" + originatingHost + "]");
            }
    ...
    protected Session retrieveSession(Serializable sessionId) {
        ...
        if (s == null) {
            String msg = "There is no session in the EIS database with session id [" + sessionId + "]";
        ...
        if (isExpirationEnabled(session)) {
            ...
            if (timeout >= 0) {
                ...
                if (lastAccessTime == null) {
                    String msg = "session.lastAccessTime for session with id [" + session.getSessionId() + "] is null.  This value must be set at " + "least once.  Please check the " + session.getClass().getName() + " implementation and ensure " + "this value will be set (perhaps in the constructor?)";
                    throw new IllegalStateException(msg);
                }
                ...
                if (log.isInfoEnabled()) {
                    log.info("No timeout for session with id [" + session.getSessionId() + "].  Session is not considered expired.");
                }
            }
        } else {
            if (log.isInfoEnabled()) {
                log.info("Time-out is disabled for Session with id [" + session.getSessionId() + "].  Session is not expired.");
            }
        }
        return false;
    }
