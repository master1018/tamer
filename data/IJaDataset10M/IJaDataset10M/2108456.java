...
public class Mol2Image {
    ...
    private Mol2Image() {
        defaultWidth = BasicPropertyHolder.instance().getInt(this, "defaultWidth", DEFAULT_WIDTH);
        defaultHeight = BasicPropertyHolder.instance().getInt(this, "defaultHeight", DEFAULT_HEIGHT);
    }
    ...
    public static StringString[] parseOptions(String options) {
        ...
        List<StringString> validOpts = new Vector<StringString>(singleOpts.size());
        String opt;
        for (int i = 0; i < singleOpts.size(); i++) {
            ...
            if (opt.startsWith(SHOW_NUMBERS)) {
                ...
                validOpts.add(new BasicStringString(TITLE, opt.substring(TITLE.length() + 1)));
            } else if (opt.startsWith(SMARTS)) {
                validOpts.add(new BasicStringString(SMARTS, opt.substring(SMARTS.length() + 1)));
            } else if (opt.startsWith(ARROWS)) {
                validOpts.add(new BasicStringString(ARROWS, opt.substring(ARROWS.length() + 1)));
            } else if (opt.startsWith(ORTHO_LINES)) {
                validOpts.add(new BasicStringString(ORTHO_LINES, opt.substring(ORTHO_LINES.length() + 1)));
            } else if (opt.startsWith(CONJ_RING)) {
                validOpts.add(new BasicStringString(CONJ_RING, opt.substring(CONJ_RING.length() + 1)));
            } else if (opt.startsWith(LABELS)) {
                validOpts.add(new BasicStringString(LABELS, opt.substring(LABELS.length() + 1)));
            } else if (opt.startsWith(WIDTH)) {
                validOpts.add(new BasicStringString(WIDTH, opt.substring(WIDTH.length() + 1)));
            } else if (opt.startsWith(HEIGHT)) {
                validOpts.add(new BasicStringString(HEIGHT, opt.substring(HEIGHT.length() + 1)));
            } else if (opt.startsWith(ROTATE)) {
                validOpts.add(new BasicStringString(ROTATE, opt.substring(ROTATE.length() + 1)));
            }
        }
        if (validOpts.size() == 0) {
            return null;
    ...
    public BufferedImage mol2image(Molecule mol, String title, SMARTSPatternMatcher smarts, String eTransfer, String retroSynth, String conjRing, String labels) {
