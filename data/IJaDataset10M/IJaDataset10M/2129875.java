package homura.hde.core.texture;
...
import homura.hde.util.export.binary.BinaryImporter;
import homura.hde.util.geom.BufferUtils;
import homura.hde.util.image.BitmapHeader;
import homura.hde.util.image.Image;
import homura.hde.util.image.ImageLoader;
import homura.hde.util.image.util.DDSLoader;
import homura.hde.util.image.util.TGALoader;
import homura.hde.util.resource.ResourceLocatorTool;
...
public final class TextureManager {
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(String file, int minFilter, int magFilter) {
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(String file, int minFilter, int magFilter, float anisoLevel, boolean flipped) {
        return loadTexture(file, minFilter, magFilter, (COMPRESS_BY_DEFAULT ? Image.GUESS_FORMAT : Image.GUESS_FORMAT_NO_S3TC), anisoLevel, flipped);
    }
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(String file, int minFilter, int magFilter, int imageType, float anisoLevel, boolean flipped) {
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(URL file) {
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(URL file, boolean flipped) {
        return loadTexture(file, DEFAULT_MIN_FILTER, DEFAULT_MAG_FILTER, (COMPRESS_BY_DEFAULT ? Image.GUESS_FORMAT : Image.GUESS_FORMAT_NO_S3TC), DEFAULT_ANISO_LEVEL, flipped);
    }
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(URL file, int minFilter, int magFilter) {
        return loadTexture(file, minFilter, magFilter, (COMPRESS_BY_DEFAULT ? Image.GUESS_FORMAT : Image.GUESS_FORMAT_NO_S3TC), DEFAULT_ANISO_LEVEL, true);
    }

    public static homura.hde.core.texture.Texture loadTexture(URL file, int minFilter, int magFilter, float anisoLevel, boolean flipped) {
        return loadTexture(file, minFilter, magFilter, (COMPRESS_BY_DEFAULT ? Image.GUESS_FORMAT : Image.GUESS_FORMAT_NO_S3TC), anisoLevel, flipped);
    }
     ...
     *         null is returned.
     */
    public static homura.hde.core.texture.Texture loadTexture(URL file, int minFilter, int magFilter, int imageType, float anisoLevel, boolean flipped) {
        if (null == file) {
    ...
    public static homura.hde.core.texture.Texture loadTexture(TextureKey tkey) {
        return loadTexture(null, tkey);
    }

    public static homura.hde.core.texture.Texture loadTexture(Texture texture, TextureKey tkey) {
        return loadTexture(texture, tkey, null, DEFAULT_MIN_FILTER, DEFAULT_MAG_FILTER, DEFAULT_ANISO_LEVEL);
    }

    public static homura.hde.core.texture.Texture loadTexture(Texture texture, TextureKey tkey, homura.hde.util.image.Image imageData, int minFilter, int magFilter, float anisoLevel) {
        if (tkey == null) {
        ...
        if (imageData == null) imageData = loadImage(tkey);
        if (null == imageData) {
            logger.warning("(image null) Could not load: " + (tkey.getLocation() != null ? tkey.getLocation().getFile() : tkey.getFileType()));
            return TextureState.getDefaultTexture();
        }
        TextureState state = null;
        state = (TextureState) Renderer.defaultStateList[RenderState.RS_TEXTURE];
        if (tkey.imageType != Image.GUESS_FORMAT_NO_S3TC && tkey.imageType != Image.GUESS_FORMAT) {
            imageData.setType(tkey.imageType);
        } else if (tkey.imageType == Image.GUESS_FORMAT && state != null && state.isS3TCSupported()) {
            if (imageData.getType() == homura.hde.util.image.Image.RGB888) {
                imageData.setType(homura.hde.util.image.Image.RGB888_DXT1);
            } else if (imageData.getType() == homura.hde.util.image.Image.RGBA8888) {
                imageData.setType(homura.hde.util.image.Image.RGBA8888_DXT5);
            }
        }
    ...
    public static void addToCache(Texture t) {
        if (TextureState.getDefaultTexture() == null || (t != TextureState.getDefaultTexture() && t.getImage() != TextureState.getDefaultTextureImage())) {
            m_tCache.put(t.getTextureKey(), t);
        }
    }

    public static homura.hde.core.texture.Texture loadTexture(java.awt.Image image, int minFilter, int magFilter, boolean flipped) {
        return loadTexture(image, minFilter, magFilter, DEFAULT_ANISO_LEVEL, (COMPRESS_BY_DEFAULT ? Image.GUESS_FORMAT : Image.GUESS_FORMAT_NO_S3TC), flipped);
    }

    public static homura.hde.core.texture.Texture loadTexture(java.awt.Image image, int minFilter, int magFilter, float anisoLevel, boolean flipped) {
        return loadTexture(image, minFilter, magFilter, anisoLevel, (COMPRESS_BY_DEFAULT ? Image.GUESS_FORMAT : Image.GUESS_FORMAT_NO_S3TC), flipped);
    }

    public static homura.hde.core.texture.Texture loadTexture(java.awt.Image image, int minFilter, int magFilter, float anisoLevel, int imageFormat, boolean flipped) {
        homura.hde.util.image.Image imageData = loadImage(image, flipped);
        TextureKey tkey = new TextureKey(null, flipped, imageFormat);
        if (image != null) tkey.setFileType("" + image.hashCode());
        return loadTexture(null, tkey, imageData, minFilter, magFilter, anisoLevel);
    }

    public static homura.hde.util.image.Image loadImage(TextureKey key) {
        if (key == null) {
            return null;
        }
        if ("savable".equalsIgnoreCase(key.fileType)) {
            ...
            if (s instanceof homura.hde.util.image.Image) {
                return (Image) s;
            }
    ...
    public static homura.hde.util.image.Image loadImage(URL file, boolean flipped) {
        if (file == null) {
    ...
    public static homura.hde.util.image.Image loadImage(String fileName, boolean flipped) {
        return loadImage(getTextureURL(fileName), flipped);
    }

    public static homura.hde.util.image.Image loadImage(String fileExt, InputStream stream, boolean flipped) {
        homura.hde.util.image.Image imageData = null;
        try {
            ImageLoader loader = loaders.get(fileExt.toLowerCase());
            if (loader != null) imageData = loader.load(stream); else if (".TGA".equalsIgnoreCase(fileExt)) {
                imageData = TGALoader.loadImage(stream, flipped);
            } else if (".DDS".equalsIgnoreCase(fileExt)) {
     ...
     */
    public static homura.hde.util.image.Image loadImage(java.awt.Image image, boolean flipImage) {
        ...
        if (flipImage || !(image instanceof BufferedImage) || (hasAlpha ? ((BufferedImage) image).getType() != BufferedImage.TYPE_4BYTE_ABGR : ((BufferedImage) image).getType() != BufferedImage.TYPE_3BYTE_BGR)) {
            try {
                tex = new BufferedImage(image.getWidth(null), image.getHeight(null), hasAlpha ? BufferedImage.TYPE_4BYTE_ABGR : BufferedImage.TYPE_3BYTE_BGR);
        ...
        scratch.put(data);
        scratch.flip();
        homura.hde.util.image.Image textureImage = new homura.hde.util.image.Image();
        textureImage.setType(hasAlpha ? homura.hde.util.image.Image.RGBA8888 : homura.hde.util.image.Image.RGB888);
    ...
    public static void doTextureCleanup(Renderer r) {
        if (r == null) return;
    ...
    public static void deleteTextureFromCard(Renderer r, Texture tex) {
