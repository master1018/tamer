...
public class GramTest extends TestCase {
    ...
    private static Log logger = LogFactory.getLog(GramTest.class.getName());

    private static final String CONFIG = "org/globus/gram/tests/test.properties";
    ...
    public void testJobStatusPoll() throws Exception {
        GramJob job = new GramJob(util.get("job.long"));
    ...
    public void testRedirect() throws Exception {
        ...
        try {
            ...
            rsl.append("&(executable=").append(util.get("stdin.exe")).append(")");
            rsl.append("(rsl_substitution=(GLOBUSRUN_GASS_URL ").append(url).append("))");
            rsl.append("(stdin=$(GLOBUSRUN_GASS_URL)/").append(util.get("stdin.file")).append(")");
    ...
    class DoneStatusListener implements GramJobListener {
        ...
        public synchronized boolean waitFor(int timeout) throws Exception {
            wait(timeout);
            return notified;
        }
    }
