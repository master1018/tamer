...
public class OrganPanel extends JPanel implements SessionAware, ConsoleStack {
    ...
    private static Configuration config = Configuration.getRoot().get(OrganPanel.class);
    ...
    public void setSession(OrganSession session) {
        if (this.session != null) {
            this.session.getOrgan().removeOrganListener((OrganListener) Spin.over(eventsListener));
            this.session.lookup(OrganPlay.class).removePlayerListener((PlayListener) Spin.over(eventsListener));
            this.session.lookup(ElementProblems.class).removeListener((ProblemListener) Spin.over(eventsListener));
            this.session.lookup(ElementSelection.class).removeListener(eventsListener);
            this.session.removeListener((SessionListener) Spin.over(eventsListener));
            ...
            for (Object key : editors.getDockableKeys()) {
                AbstractEditor editor = (AbstractEditor) editors.removeDockable(key);
                if (editor != null) {
        ...
        if (this.session != null) {
            ...
            this.session.lookup(ElementSelection.class).addListener(eventsListener);
            this.session.lookup(ElementProblems.class).addListener((ProblemListener) Spin.over(eventsListener));
            this.session.lookup(OrganPlay.class).addPlayerListener((PlayListener) Spin.over(eventsListener));
            this.session.getOrgan().addOrganListener((OrganListener) Spin.over(eventsListener));
            for (Object key : views.getDockableKeys()) {
                AbstractView view = (AbstractView) views.getDockable(key);
                if (view != null) {
            ...
            for (Element element : new ReverseIterable<Element>(session.getOrgan().getElements(Element.class))) {
                addEditor(element);
            }
        }
    }
	 ...
	 * The listener to events.
	 */
    private class EventsListener extends OrganAdapter implements PlayListener, ProblemListener, SelectionListener, SessionListener {
