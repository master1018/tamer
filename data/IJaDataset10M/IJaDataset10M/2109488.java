...
public class Export extends Action {
    ...
    protected void doActionBody() {
        ...
        exported = true;
        if (response != JFileChooser.APPROVE_OPTION || f == null) exported = false; else {
            if (f.getName().toLowerCase().indexOf(format.getFileExtension().toLowerCase()) == -1) f = new File(f.getPath() + format.getFileExtension());
            if (f.exists()) {
                int replace = JOptionPane.showConfirmDialog(null, LangTool.INSTANCE.getStringLaTeXDrawFrame("LaTeXDrawFrame.173"), Exporter.TITLE_DIALOG_EXPORT, JOptionPane.YES_NO_OPTION);
                if (replace == JOptionPane.NO_OPTION) exported = false;
            }
        }
    ...
    protected boolean export(final File file) {
        switch(format) {
            case BMP:
                return exportAsBMP(file);
            case EPS_LATEX:
                return exportAsPS(file);
            case JPG:
                return exportAsJPG(file);
            case PDF:
                return exportAsPDF(file);
            case PDF_CROP:
                return exportAsPDF(file);
            case PNG:
                return exportAsPNG(file);
            case TEX:
    ...
    protected boolean exportAsBMP(final File file) {
        ...
        try {
            ...
            final ImageWriter iw = ImageIO.getImageWritersByFormatName("bmp").next();
            final ImageOutputStream ios = ImageIO.createImageOutputStream(file);
            iwparam.setCompressionMode(ImageWriteParam.MODE_EXPLICIT);
            iw.setOutput(ios);
            iw.write(null, new IIOImage(rendImage, null, null), iwparam);
    ...
    protected BufferedImage createRenderedImage() {
        ...
        for (IViewShape view : canvas.getViews()) view.paint(graphic);
