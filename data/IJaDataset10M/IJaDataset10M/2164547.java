...
public class MessagesTest extends FreeColTestCase {
    ...
    public void testMessageString() {
        ...
        assertEquals("Food is necessary to feed your colonists and to breed horses. " + "A new colonist is born whenever a colony has 200 units of food or more.", Messages.message("model.goods.food.description"));
    ...
    public void testMessageWithSpecialChars() {
        ...
        try {
            message = Messages.message(StringTemplate.template("model.history.FOUND_COLONY").addName("%colony%", colNameWithSpecialChars));
    ...
    public void testStringTemplates() {
        ...
        StringTemplate t1 = StringTemplate.template("model.goods.goodsAmount").add("%goods%", "model.goods.food.name").addName("%amount%", "100");
        assertEquals(2, t1.getKeys().size());
        assertEquals(2, t1.getReplacements().size());
        assertEquals(StringTemplate.TemplateType.KEY, t1.getReplacements().get(0).getTemplateType());
        assertEquals(StringTemplate.TemplateType.NAME, t1.getReplacements().get(1).getTemplateType());
        assertEquals("model.goods.goodsAmount", t1.getId());
        assertEquals("100 Food", Messages.message(t1));
        StringTemplate t2 = StringTemplate.label(" / ").add("model.goods.food.name").addName("xyz");
        assertEquals("Food / xyz", Messages.message(t2));
        Game game = getGame();
        game.setMap(getTestMap());
        Colony colony = getStandardColony();
        assertEquals("New Amsterdam", colony.getName());
        StringTemplate t3 = StringTemplate.template("inLocation").addName("%location%", colony.getName());
        assertEquals("In New Amsterdam", Messages.message(t3));
        StringTemplate t4 = StringTemplate.label("").addName("(").add("model.goods.food.name").addName(")");
    ...
    public void testReplaceNumber() {
        double[] numbers = new double[] { -1.3, -1, -0.5, 0, 0.33, 1, 1.2, 2, 2.7, 3, 3.4, 11, 13, 27, 100 };
        ...
        for (double d : numbers) {
            assertEquals("abcother|xyz", Messages.message(StringTemplate.template("some.key").addAmount("%number%", d)));
        }
        Messages.setGrammaticalNumber(NumberRules.PLURAL_NUMBER_RULE);
        for (double d : numbers) {
            if (d == 1) {
                assertEquals("abcone|xyz", Messages.message(StringTemplate.template("some.key").addAmount("%number%", d)));
            } else {
                assertEquals("abcother|xyz", Messages.message(StringTemplate.template("some.key").addAmount("%number%", d)));
            }
        }
    ...
    public void testReplaceArbitraryTag() {
        StringTemplate template = StringTemplate.template("tutorial.startGame").add("%direction%", "east");
        ...
        template = StringTemplate.template("tutorial.startGame").add("%direction%", "west");
        ...
        assertEquals(expected, Messages.message(template));
        template = StringTemplate.template("tutorial.startGame").add("%direction%", "whatever");
    ...
    public void testReplaceChoicesPlural() {
        ...
        assertEquals("This is one of several tests.", Messages.message(StringTemplate.template("some.key").addAmount("%number%", 0)));
        assertEquals("This is a test.", Messages.message(StringTemplate.template("some.key").addAmount("%number%", 1)));
        assertEquals("This is one of several tests.", Messages.message(StringTemplate.template("some.key").addAmount("%number%", 2)));
        assertEquals("This is one of several tests.", Messages.message(StringTemplate.template("some.key").addAmount("%number%", 24)));
        StringTemplate template = StringTemplate.template("unit.template").addAmount("%number%", 1).add("%unit%", "unit.key");
    ...
    public void testReplaceChoicesGrammar() {
        ...
        StringTemplate t1 = StringTemplate.template("key.france").add("%randomKey%", "country");
        assertEquals("France", Messages.message(t1));
        StringTemplate t2 = StringTemplate.template("greeting1").add("%nation%", "key.france");
        assertEquals("The French people are happy to see you.", Messages.message(t2));
        StringTemplate t3 = StringTemplate.template("greeting2").add("%nation%", "key.france");
    ...
    public void testNestedChoices() {
        ...
        ByteArrayInputStream stream = new ByteArrayInputStream(mapping.getBytes());
        Messages.loadResources(stream);
        StringTemplate t = StringTemplate.template("key1").addName("%colony%", "someColony").add("%goods%", "key2");
    ...
    public void testLabels() {
        ...
        StringTemplate template = StringTemplate.template("model.monarch.action.ADD_TO_REF").addAmount("%number%", 1).add("%unit%", spec().getUnitType("model.unit.kingsRegular").getNameKey());
        String expected = "The Crown has added 1 King's Regular to the Royal Expeditionary Force." + " Colonial leaders express concern.";
        assertEquals(expected, Messages.message(template));
        template = StringTemplate.template("model.monarch.action.ADD_TO_REF").addAmount("%number%", 2).add("%unit%", spec().getUnitType("model.unit.artillery").getNameKey());
        expected = "The Crown has added 2 Pieces of Artillery to the Royal Expeditionary Force." + " Colonial leaders express concern.";
        assertEquals(expected, Messages.message(template));
        template = StringTemplate.template("model.monarch.action.ADD_TO_REF").addAmount("%number%", 3).add("%unit%", spec().getUnitType("model.unit.manOWar").getNameKey());
