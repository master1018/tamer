...
public class SilentImageStackProcessor {
    ...
    public SilentImageStackProcessor(ImageStack stack, ImageProcessor ip) {
        this.stack = stack;
        this.ip = ip;
        nSlices = stack.getSize();
        if (nSlices > 1 && ip != null) ip.setProgressBar(null);
    }
    ...
    void process(int command) {
        ...
        for (int i = 1; i <= nSlices; i++) {
            ...
            if (nSlices == 1 && i == 1 && command == SCALE) ip2.snapshot();
            switch(command) {
                case FLIPH:
                    ip2.flipHorizontal();
                    break;
    ...
    public ImageStack resize(int newWidth, int newHeight, boolean averageWhenDownsizing) {
        ...
        try {
            ...
            while (stack.getSize() > 1) stack.deleteLastSlice();
    ...
    ImageStack rotate90Degrees(boolean clockwise) {
        ...
        for (int i = 1; i <= nSlices; i++) {
            ...
            String label = stack.getSliceLabel(1);
            stack.deleteSlice(1);
            if (clockwise) ip2 = ip.rotateRight(); else ip2 = ip.rotateLeft();
            if (ip2 != null) stack2.addSlice(label, ip2);
            if (!Interpreter.isBatchMode()) IJ.showProgress((double) i / nSlices);
        }
        if (!Interpreter.isBatchMode()) IJ.showProgress(1.0);
        return stack2;
    }
    ...
    private void copyBits(ImageProcessor srcIp, ImageStack srcStack, int xloc, int yloc, int mode) {
        ...
        for (int i = 1; i <= nSlices; i++) {
            if (stackSource) srcIp = srcStack.getProcessor(i);
