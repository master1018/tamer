...
public class PajekNetReader<G extends Graph<V, E>, V, E> {
    ...
    protected SettableTransformer<E, Number> edge_weights = new MapSettableTransformer<E, Number>(new HashMap<E, Number>());
    ...
    public G load(Reader reader, G g) throws IOException {
        ...
        if (curLine == null) return g;
        StringTokenizer st = new StringTokenizer(curLine);
        st.nextToken();
        int num_vertices = Integer.parseInt(st.nextToken());
        List<V> id = null;
        if (vertex_factory != null) {
            for (int i = 1; i <= num_vertices; i++) g.addVertex(vertex_factory.create());
    ...
    private void readVertex(String curLine, List<V> id, int num_vertices) {
        ...
        if (curLine.indexOf('"') != -1) {
            ...
            if (initial_split.length < 2 || initial_split.length > 3) throw new IllegalArgumentException("Unbalanced (or too many) " + "quote marks in " + curLine);
        ...
        if (v_id >= num_vertices || v_id < 0) throw new IllegalArgumentException("Vertex number " + v_id + "is not in the range [1," + num_vertices + "]");
        if (id != null) v = id.get(v_id); else v = (V) (new Integer(v_id));
    ...
    private String readArcsOrEdges(String curLine, BufferedReader br, Graph<V, E> g, List<V> id, Factory<E> edge_factory) throws IOException {
        ...
        if (!c_pred.evaluate(curLine)) nextLine = skip(br, c_pred);
        ...
        if (e_pred.evaluate(nextLine)) {
            if (g instanceof DirectedGraph) throw new IllegalArgumentException("Supplied directed-only graph cannot be populated with undirected edges"); else reading_edges = true;
        ...
        while (br.ready()) {
            ...
            V v1;
            if (id != null) v1 = id.get(vid1); else v1 = (V) new Integer(vid1);
            if (is_list) {
                do {
                ...
                if (edge_weights != null && st.hasMoreTokens()) edge_weights.set(e, new Float(st.nextToken()));
            }
        }
        return nextLine;
    }
    ...
    protected E createAddEdge(StringTokenizer st, V v1, EdgeType directed, Graph<V, E> g, List<V> id, Factory<E> edge_factory) {
        ...
        V v2;
        if (id != null) v2 = id.get(vid2); else v2 = (V) new Integer(vid2);
    ...
    protected String skip(BufferedReader br, Predicate<String> p) throws IOException {
        while (br.ready()) {
            ...
            if (p.evaluate(curLine)) return curLine;
        }
        return null;
    }
