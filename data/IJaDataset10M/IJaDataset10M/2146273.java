...
public class URL extends URI {
   ...
   */
    public URL(String nurl) throws MalformedURIException {
        super(nurl);
        if (uri.length() < colonLocation + 4) throw new MalformedURIException("URL is incomplete: \"" + uri + "\"", uri);
        if ((uri.charAt(colonLocation + 1) != '/') || (uri.charAt(colonLocation + 2) != '/')) throw new MalformedURIException("URL does not begin with '://': \"" + uri + "\"", uri);
        portColonLocation = uri.indexOf(':', colonLocation + 3);
        pathLocation = uri.indexOf('/', colonLocation + 3);
        if (pathLocation == colonLocation + 3) throw new MalformedURIException("URL has no host: \"" + uri + "\"", uri);
        if (pathLocation == -1) throw new MalformedURIException("URL has no '/': \"" + uri + "\"", uri);
        if (pathLocation > fragmentLocation) throw new MalformedURIException("URL has no '/' before fragment: \"" + uri + "\"", uri);
        port = -1;
        if (portColonLocation >= 0 && portColonLocation < pathLocation) {
            try {
                port = Integer.parseInt(uri.substring(portColonLocation + 1, pathLocation), 10);
            } catch (NumberFormatException e) {
                throw new MalformedURIException("Invalid port number \"" + uri.substring(portColonLocation + 1, pathLocation) + "\" in \"" + uri + "\"", uri);
            }
            if (port <= 0 || port > 65535) throw new MalformedURIException("Invalid port number \"" + port + "\" in \"" + uri + "\"", uri);
        } else portColonLocation = pathLocation;
    }
   ...
   */
    ...
    public String makeRelative(URI other, boolean allowDotDot) throws MalformedURIException {
        if (!(other instanceof URL)) return other.toString();
