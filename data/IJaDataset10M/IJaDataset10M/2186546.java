...
public class UIMapViewFrame extends UIViewFrame {
    ...
    public UIMapViewFrame(JFrame parent, View view, String title) {
        super(parent, view, title);
        setTitle(title);
        init(view);
    }
    ...
    private UIAerialDialog createAerialDialog(Rectangle dialogBounds) {
        oAerialDialog = new UIAerialDialog(super.parent, this, oView, dialogBounds);
        oAerialViewPane = oAerialDialog.getViewPane();
        if (dialogBounds != null) {
            oAerialDialog.setLocation(dialogBounds.x, dialogBounds.y);
        } else {
            oAerialDialog.setLocation(100, 100);
        }
    ...
    public UIAerialDialog showArialView(Rectangle dialogBounds) {
        if (oAerialDialog == null) createAerialDialog(dialogBounds);
    ...
    public void updateAerialLabelFonts(Font labelFont) {
        if (oAerialViewPane == null) return;
    ...
    public void refreshAerialIcons(boolean refreshFrameIcons) {
        ...
        for (int j = 0; j < array.length; j++) {
            ...
            if (nType == ICoreConstants.REFERENCE || nType == ICoreConstants.REFERENCE_SHORTCUT) {
                ...
                if (image != null && !image.equals("")) uinode.setReferenceIcon(image); else {
                    uinode.setReferenceIcon(node.getSource());
                }
            } else if (nType == ICoreConstants.MAPVIEW || nType == ICoreConstants.MAP_SHORTCUT || nType == ICoreConstants.LISTVIEW || nType == ICoreConstants.LIST_SHORTCUT) {
                String image = node.getImage();
                if (image != null && !image.equals("")) uinode.setReferenceIcon(image); else {
    ...
    public void refreshNode(NodeSummary node) {
        if (oAerialDialog != null) oAerialDialog.refreshNode(node);
    }
    ...
    public void rescaleAerial(Point oPoint) {
        if (oAerialDialog != null) oAerialDialog.rescale(oPoint);
    }
    ...
    public void addAerialNode(NodePosition oNodePos) {
        if (oNodePos == null || oAerialDialog == null) return;
    ...
    public void removeAerialNode(NodeSummary oNode) {
        if (oNode == null || oAerialDialog == null) return;
    ...
    public void setAerialRolloverNode(UINode node, boolean rollover) {
        if (node == null || oAerialDialog == null) return;
        UINode oNode = (UINode) oAerialViewPane.get(node.getNode().getId());
        if (oNode != null) {
    ...
    public void addAerialLink(Link oLink) {
        if (oLink == null || oAerialDialog == null) return;
    ...
    private void adjustScrollBars() {
        ...
        if (viewPosition.x == 0 && viewPosition.y == 0) return;
    ...
    public double onZoomNextUp() {
        ...
        if (scale == 0.25) scale = 0.50; else if (scale == 0.50) scale = 0.75; else if (scale == 0.75) scale = 1.00; else scale = 1.00;
        pane.setZoom(scale);
        pane.scale();
        adjustScrollBars();
        isFocusedNode = false;
    ...
    public double onZoomNextDown() {
        ...
        System.out.flush();
        if (scale == 1.00) scale = 0.75; else if (scale == 0.75) scale = 0.50; else if (scale == 0.50) scale = 0.25; else scale = 0.25;
        pane.setZoom(scale);
        pane.scale();
        adjustScrollBars();
        isFocusedNode = false;
    ...
    public void onZoomNext() {
        UIViewPane pane = getViewPane();
        double scale = pane.getZoom();
        if (scale == 0.75) scale = 0.50; else if (scale == 0.50) scale = 0.25; else if (scale == 0.25) scale = 1.00; else scale = 0.75;
        pane.setZoom(scale);
        pane.scale();
        adjustScrollBars();
        isFocusedNode = false;
    ...
    public boolean onZoomRefocused() {
        ...
        if (node == null) {
            JOptionPane.showMessageDialog(null, "Please select a node first Re-Focused Normal");
            return false;
        } else {
