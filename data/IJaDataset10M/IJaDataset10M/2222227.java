...
public class SimpleNode<T> implements Node<T> {
    ...
    public Node<T> child(int index) {
        if (hasChild()) return firstChild.next(index);
        return null;
    }
    ...
    public Node<T> insertBefore(int index, Node<T> node) {
        ...
        if (null != me) {
            ...
            me.prev(node);
            node.parent(this);
            if (firstChild == me) firstChild = node;
        }
        return this;
    }
    ...
    public Node<T> pop() {
        ...
        if (null == lastChild) firstChild = null; else lastChild.next(null);
    ...
    public Node<T> popFirst() {
        ...
        if (null == firstChild) lastChild = null; else firstChild.prev(null);
        return re.prev(null).next(null);
    }
    ...
    public Node<T> removeChild(int index) {
        if (hasChild()) {
            ...
            if (null == node) return null; else if (node.isLast()) return pop(); else if (node.isFirst()) return popFirst();
    ...
    static class InnerIterator<T> implements Iterator<Node<T>> {
        ...
        InnerIterator(Node<T> node) {
            this.root = node;
            if (root.hasChild()) this.node = root.child(0); else this.node = root;
        }
