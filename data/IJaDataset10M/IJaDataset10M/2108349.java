...
public final class DoubleDiagonalMatrix extends DoubleTridiagonalMatrix {
    ...
    public DoubleDiagonalMatrix(final double array[][]) {
        ...
        if (array.length == array[0].length) {
            for (int i = 0; i < numRows; i++) diag[i] = array[i][i];
        } else {
            matrix = null;
    ...
    public boolean equals(Object m) {
        if (m != null && (m instanceof DoubleDiagonalMatrix) && numRows == ((DoubleDiagonalMatrix) m).rows()) {
            final DoubleDiagonalMatrix ddm = (DoubleDiagonalMatrix) m;
            for (int i = 0; i < numRows; i++) {
                if (Math.abs(diag[i] - ddm.getElement(i, i)) > GlobalSettings.ZERO_TOL) return false;
            }
            return true;
        } else return false;
    }
    ...
    public IntegerMatrix toIntegerMatrix() {
        ...
        for (int i = 0; i < numRows; i++) array[i] = Math.round((float) diag[i]);
    ...
    public double getElement(final int i, final int j) {
        if (i >= 0 && i < numRows && j >= 0 && j < numCols) {
            if (i == j) return diag[i]; else return 0.0;
    ...
    public void setElement(final int i, final int j, final double x) {
        if (i >= 0 && i < numRows && j >= 0 && j < numCols && i == j) diag[i] = x; else throw new MatrixDimensionException(getInvalidElementMsg(i, j));
    }
    ...
    public double det() {
        double det = diag[0];
        for (int i = 1; i < numRows; i++) det *= diag[i];
        return det;
    }
    ...
    public double trace() {
        double tr = diag[0];
        for (int i = 1; i < numRows; i++) tr += diag[i];
        return tr;
    }
    ...
    public double frobeniusNorm() {
        ...
        for (int i = 1; i < numRows; i++) result = ExtraMath.hypot(result, diag[i]);
        return result;
    }
            ...
            default:
                if (numRows == m.rows() && numCols == m.columns()) {
                    ...
                    for (int j, i = 0; i < numRows; i++) {
                        ...
                        for (j = 1; j < numCols; j++) array[i][j] = m.getElement(i, j);
                    }
                    for (int i = 0; i < numRows; i++) array[i][i] += diag[i];
    ...
    private DoubleSquareMatrix rawAdd(final DoubleMatrix m) {
        if (numRows == m.numRows && numCols == m.numCols) {
            final double array[][] = new double[numRows][numCols];
            for (int i = 0; i < numRows; i++) System.arraycopy(m.matrix[i], 0, array[i], 0, numRows);
            for (int i = 0; i < numRows; i++) array[i][i] += diag[i];
            return new DoubleSquareMatrix(array);
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
    ...
    private DoubleTridiagonalMatrix rawAddTridiagonal(final DoubleMatrix m) {
        if (numRows == m.numRows) {
            ...
            for (int i = 1; i < numRows; i++) ans.diag[i] = diag[i] + m.matrix[1][i];
            return ans;
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
    ...
    private DoubleDiagonalMatrix rawAddDiagonal(final DoubleMatrix m) {
        if (numRows == m.numRows) {
            ...
            for (int i = 1; i < numRows; i++) array[i] = diag[i] + m.matrix[0][i];
            return new DoubleDiagonalMatrix(array);
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
    ...
    private DoubleSquareMatrix rawSubtract(final DoubleMatrix m) {
        if (numRows == m.numRows && numCols == m.numCols) {
            ...
            for (int j, i = 0; i < numRows; i++) {
                ...
                for (j = 1; j < numCols; j++) array[i][j] = -m.matrix[i][j];
            }
            for (int i = 0; i < numRows; i++) array[i][i] += diag[i];
            return new DoubleSquareMatrix(array);
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    ...
    public DoubleMatrix scalarMultiply(final double x) {
        final double array[] = new double[numRows];
        array[0] = x * diag[0];
        for (int i = 1; i < numRows; i++) array[i] = x * diag[i];
        return new DoubleDiagonalMatrix(array);
    }
        ...
        switch(m.storageFormat) {
            ...
            default:
                if (numRows == m.rows() && numCols == m.columns()) {
                    ...
                    for (int i = 1; i < numRows; i++) ans += diag[i] * m.getElement(i, i);
                    return ans;
                } else throw new MatrixDimensionException("Matrices are different sizes.");
        }
    }
    ...
    private double rawScalarProduct(final DoubleMatrix m) {
        if (numRows == m.numRows && numCols == m.numCols) {
            ...
            for (int i = 1; i < numRows; i++) ans += diag[i] * m.matrix[i][i];
            return ans;
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
    ...
    private double rawScalarProductTridiagonal(final DoubleMatrix m) {
        if (numRows == m.numRows) {
            ...
            for (int i = 1; i < numRows; i++) ans += diag[i] * m.matrix[1][i];
            return ans;
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
    ...
    private double rawScalarProductDiagonal(final DoubleMatrix m) {
        if (numRows == m.numRows) {
            double ans = diag[0] * m.matrix[0][0];
            for (int i = 1; i < numRows; i++) ans += diag[i] * m.matrix[0][i];
            return ans;
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
    ...
    public DoubleVector multiply(final DoubleVector v) {
        if (numCols == v.dimension()) {
            ...
            for (int i = 1; i < numRows; i++) array[i] = diag[i] * v.getComponent(i);
            ...
            default:
                if (numCols == m.rows()) {
                    ...
                    for (int j, i = 0; i < numRows; i++) {
                        ...
                        for (j = 1; j < m.columns(); j++) array[i][j] = diag[i] * m.getElement(i, j);
                    }
    ...
    private DoubleMatrix rawMultiply(final DoubleMatrix m) {
        if (numCols == m.numRows) {
            ...
            for (int j, i = 0; i < numRows; i++) {
                ...
                for (j = 1; j < numCols; j++) array[i][j] = diag[i] * m.matrix[i][j];
            }
            return new DoubleMatrix(array);
        } else throw new MatrixDimensionException("Matrices are different sizes.");
    }
        ...
        switch(m.storageFormat) {
            ...
            default:
                if (numCols == m.rows()) {
                    ...
                    for (int j, i = 0; i < numRows; i++) {
                        array[i][0] = diag[i] * m.getElement(i, 0);
                        for (j = 1; j < numCols; j++) array[i][j] = diag[i] * m.getElement(i, j);
                    }
                    return new DoubleSquareMatrix(array);
                } else throw new MatrixDimensionException("Matrices are different sizes.");
        }
    ...
    public DoubleSquareMatrix inverse() {
        final double array[] = new double[numRows];
        array[0] = 1.0 / diag[0];
        for (int i = 1; i < numRows; i++) array[i] = 1.0 / diag[i];
        return new DoubleDiagonalMatrix(array);
    }
    ...
    public DoubleSquareMatrix[] luDecompose(int pivot[]) {
        ...
        if (pivot == null) pivot = new int[numRows + 1];
        for (int i = 0; i < numRows; i++) pivot[i] = i;
        pivot[numRows] = 1;
    ...
    public DoubleSquareMatrix[] choleskyDecompose() {
        ...
        for (int i = 1; i < numRows; i++) array[i] = Math.sqrt(diag[i]);
    ...
    public DoubleMatrix mapElements(final Mapping f) {
        ...
        for (int i = 1; i < numRows; i++) array[i] = f.map(diag[i]);
        return new DoubleDiagonalMatrix(array);
    }
}
