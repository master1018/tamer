...
public class Modal {
    ...
    private static Integer defaultModalDepth = Integer.valueOf(JLayeredPane.DEFAULT_LAYER + 1);

    private static Hashtable<RootPaneContainer, Modal> modals = new Hashtable<RootPaneContainer, Modal>();
   ...
   */
    public static void closeModal(Component component) {
        synchronized (modals) {
            ...
            Modal modal = null;
            Iterator<Entry<RootPaneContainer, Modal>> it = modals.entrySet().iterator();
            while (it.hasNext()) {
                Entry<RootPaneContainer, Modal> entry = it.next();
                modal = entry.getValue();
                frame = entry.getKey();
                if (frame != null && modal != null && modal.components.contains(component)) {
                    break;
                } else {
                    frame = null;
                    modal = null;
