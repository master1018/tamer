...
public class GeneralEditDistanceTest extends TestCase {
   ...
   */
     ...
     */
        public void testLevenshteinOnWords() {
            for (int i = 0; i < words.length; i++) {
                for (int j = 0; j < words.length; j++) {
                    ...
                    specificAlgorithmVerify(ed, words[i], words[j], wordsDistances[i][j]);
                }
            }
        }
        ...
        public void testLongerPattern() {
            genericLevenshteinVerify("abcdefghijklmnopqrstuvwxyz", "abcefghijklMnopqrStuvwxyz..", 5);
        }
        ...
        void genericLevenshteinVerify(CharSequence s1, CharSequence s2, int expectedResult) {
            specificAlgorithmVerify(g.getInstance(s1), s1, s2, expectedResult);
        }
    }
   ...
   */
    static String[] words = { "", "a", "b", "c", "ab", "ace", "fortressing", "inadequately", "prank", "authored", "fortresing", "inadeqautely", "prang", "awthered", "cruller's", "fanatic", "Laplace", "recollections", "Kevlar", "underpays", "jalapeño", "châtelaine", "kevlar", "overpaid", "jalapeno", "chatelaine", "A survey of algorithms for running text search by Navarro appeared", "in ACM Computing Surveys 33#1: http://portal.acm.org/citation.cfm?...", "Another algorithm (Four Russians) that Navarro", "long patterns and high limits was not evaluated for inclusion here.", "long patterns and low limits were evaluated for inclusion here.", "Filtering algorithms also improve running search", "for pure edit distance." };
    ...
    static {
        ...
        for (int i = 0; i < words.length; i++) {
            for (int j = 0; j < i; j++) {
                expect[i][j] = GeneralEditDistanceTest.dynamicProgrammingLevenshtein(words[i], words[j]);
   ...
   */
    public static int performSomeEdits(StringBuilder b, String alphabet, int replaces, int inserts) {
   ...
   */
    ...
    static void genericVerification(GeneralEditDistance ed, CharSequence s1, CharSequence s2, int expectedResult) {
        if (s1.length() < 500) {
    ...
    static void specificAlgorithmVerify(GeneralEditDistance ed, CharSequence s1, CharSequence s2, int expectedResult) {
   ...
   */
    static void testSomeEdits(Factory g, String original, int replaces, int inserts) {
   ...
   */
    static void verifyResult(CharSequence s1, CharSequence s2, int expectedResult, int k, int d) {
        if (k >= expectedResult) {
            assertEquals("Distance from " + s1 + " to " + s2 + " should be " + expectedResult + " (within limit=" + k + ") but was " + d, expectedResult, d);
        } else {
            assertTrue("Distance from " + s1 + " to " + s2 + " should be " + expectedResult + " (exceeding limit=" + k + ") but was " + d, d > k);
        }
    }
