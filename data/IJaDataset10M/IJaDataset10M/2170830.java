    ...
    public Shape[] getBody() {
        if (body == null) {
            ...
            if (polarity == BJTPolarity.NPN) {
                ...
                arrow = new Area(new Polygon(new int[] { x + pinSpacing, x + pinSpacing, x + pinSpacing * 10 / 6 }, new int[] { y - pinSpacing / 5 + pinSpacing / 2, y + pinSpacing / 5 + pinSpacing / 2, y + pinSpacing / 2 }, 3));
                arrow.transform(AffineTransform.getRotateInstance(theta, x + pinSpacing / 2, y + pinSpacing / 2));
            } else {
                theta = -Math.atan(1.0 / 3);
                arrow = new Area(new Polygon(new int[] { x + pinSpacing, x + pinSpacing * 10 / 6, x + pinSpacing * 10 / 6 }, new int[] { y - pinSpacing / 2, y - pinSpacing / 5 - pinSpacing / 2, y + pinSpacing / 5 - pinSpacing / 2 }, 3));
                arrow.transform(AffineTransform.getRotateInstance(theta, x + pinSpacing / 2, y - pinSpacing / 2));
            }
            body[2] = arrow;
        }
