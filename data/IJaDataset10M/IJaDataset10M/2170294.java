...
public class FindInfo implements Finder {
    ...
    public FindInfo(Vectorer vec, String sText, int iField, boolean bMatchCase, int iMatch) {
        ...
        if (iMatch == MATCH_REGEXP) {
            m_TextPattern = m_bMatchCase ? Pattern.compile(m_sText) : Pattern.compile(m_sText, Pattern.CASE_INSENSITIVE);
        } else {
            m_sTextCompare = m_bMatchCase ? m_sText : m_sText.toUpperCase();
        }
    }
    ...
    public boolean match(Object obj) throws BasicException {
        ...
        String sField = m_bMatchCase ? v[m_iField] : v[m_iField].toUpperCase();
        switch(m_iMatch) {
