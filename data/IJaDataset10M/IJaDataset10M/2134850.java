...
public class PmdSettingsPanel extends DefaultTraversablePanel {
    ...
    private static final String PLUGIN_HOME = "/extensions/net.sourceforge.pmd.jdeveloper/conf/";

    private static final String PLUGIN_PROPS = PLUGIN_HOME + "pmd.plugin.properties";

    private static final String RULE_PROPS = PLUGIN_HOME + "pmd.rule.properties";
    ...
    private class ExportListener implements ActionListener {
        ...
        public void actionPerformed(final ActionEvent evt) {
            final FileDialog fdlg = new FileDialog(new Frame(), "Export", FileDialog.SAVE);
            fdlg.setVisible(true);
            if (fdlg.getFile() == null) {
                return;
            }
    ...
    private class FindListener implements ActionListener {
        ...
        public void actionPerformed(final ActionEvent evt) {
            final FileDialog fdlg = new FileDialog(new Frame(), "Find", FileDialog.LOAD);
            fdlg.setVisible(true);
            final String selected = fdlg.getDirectory() + fdlg.getFile();
            if (fdlg.getFile() == null) {
                return;
    ...
    private static FileStorage pluginProps = new FileStorage(new File(Version.getJdevHome() + PLUGIN_PROPS));
    ...
    public void onEntry(final TraversableContext tcon) {
        ...
        try {
            rulesStoredSepBox = new JCheckBox("", Boolean.valueOf(pluginProps.load(STORED_SEPARATELY)).booleanValue());
        } catch (SettingsException se) {
            Util.logMessage(se.getStackTrace());
            Util.showError(se, PmdAddin.PMD_TITLE);
        }
    ...
    private JPanel createTopPanel() {
        ...
        custStoragePanel.add(custStorTxtField, BorderLayout.SOUTH);
    ...
    public void onExit(final TraversableContext tcon) {
        ...
        properties.setProperty(STORED_SEPARATELY, String.valueOf(rulesStoredSepBox.isSelected()));
        properties.setProperty(SEL_FILENAME, rulesSeparateFile.getText());
        try {
            pluginProps.save(properties);
            rules.save(createSettingsStorage());
        } catch (SettingsException se) {
            JOptionPane.showMessageDialog(null, "Can't save settings :" + se.getMessage(), "Can't save settings", JOptionPane.ERROR_MESSAGE);
        }
    }
