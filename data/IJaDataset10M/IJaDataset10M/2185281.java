...
public class VolumeSelectionTable extends SelectionTable {
    ...
    private static class VolumeLabelProvider extends LabelProvider implements ITableLabelProvider {
        ...
        public Image getColumnImage(Object obj, int index) {
            if (index == 0) return Ec2Plugin.getDefault().getImageRegistry().get("volume");
            return null;
        }
        ...
        protected int compareIgnoringDirection(Viewer viewer, Object e1, Object e2) {
            ...
            switch(this.sortColumn) {
                ...
                case ATTACHED_INSTANCE_COLUMN:
                    String instanceId1 = "";
                    String instanceId2 = "";
                    for (VolumeAttachment att : volume1.getAttachments()) instanceId1 = att.getInstanceId();
                    for (VolumeAttachment att : volume2.getAttachments()) instanceId2 = att.getInstanceId();
        ...
        public void run() {
            try {
                ...
                if (AwsToolkitCore.getDefault().getAccountInfo().isValid()) {
                    Status status = new Status(Status.ERROR, Ec2Plugin.PLUGIN_ID, "Unable to refresh EBS volumes: " + e.getMessage(), e);
        ...
        public void run() {
            for (VolumeAttachment attachmentInfo : volume.getAttachments()) {
                ...
                try {
                    ...
                    Status status = new Status(Status.ERROR, Ec2Plugin.PLUGIN_ID, "Unable to detach EBS volume: " + e.getMessage(), e);
    ...
    private class CreateVolumeThread extends Thread {
        ...
        public void run() {
            try {
                ...
                Status status = new Status(Status.ERROR, Ec2Plugin.PLUGIN_ID, "Unable to create EBS volume: " + e.getMessage(), e);
                StatusManager.getManager().handle(status, StatusManager.SHOW | StatusManager.LOG);
            }
        }
    }
    ...
    private class ReleaseVolumesThread extends Thread {
        ...
        public void run() {
            try {
                ...
                Status status = new Status(Status.ERROR, Ec2Plugin.PLUGIN_ID, "Unable to release EBS volume: " + e.getMessage(), e);
                StatusManager.getManager().handle(status, StatusManager.SHOW | StatusManager.LOG);
            }
            refreshVolumes();
        }
    ...
    private class CreateSnapshotThread extends Thread {
        ...
        public void run() {
            try {
                ...
                Status status = new Status(Status.ERROR, Ec2Plugin.PLUGIN_ID, "Unable to create snapshot: " + e.getMessage(), e);
                StatusManager.getManager().handle(status, StatusManager.SHOW | StatusManager.LOG);
            }
        }
    }
