...
public class ReaderManage extends HttpServlet {
    ...
    protected void preEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        @SuppressWarnings("rawtypes") List list = session.createQuery("from ReaderCategoryBean").list();
    ...
    protected void list(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
        session.beginTransaction();
        @SuppressWarnings("rawtypes") List list = session.createQuery("from ReaderBean").list();
        System.out.println(list.size());
        @SuppressWarnings("rawtypes") List categoryList = session.createQuery("from ReaderCategoryBean").list();
        request.setAttribute("list", list);
        request.setAttribute("categoryList", categoryList);
        request.getRequestDispatcher("/admin/showAllReaders.jsp").forward(request, response);
        session.getTransaction().commit();
    }
