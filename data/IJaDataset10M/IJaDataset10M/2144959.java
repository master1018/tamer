...
public class RestfulXmlPeopleTest extends AbstractLargeRestfulTests {
    ...
    protected void setUp() throws Exception {
        ...
        org.apache.shindig.protocol.model.Enum<LookingFor> lookingForRandom = new EnumImpl<LookingFor>(LookingFor.RANDOM, "Random");
        ...
        canonical.setUrls(Lists.<Url>newArrayList(new UrlImpl("http://www.example.org/?id=1", "my profile", "Profile"), new UrlImpl("http://www.example.org/pic/?id=1", "my awesome picture", "Thumbnail")));
    }
   ...
   */
    ...
    private void assertAddressField(Address expected, Map<String, List<String>> actual) {
    ...
    private void assertOrganizationField(Organization expected, Node orgNode) {
        ...
        assertStringField(childNodesToMap(actualNode.get(Organization.Field.ADDRESS.toString()).get(0)), expected.getAddress().getFormatted(), Address.Field.FORMATTED);
    ...
    private void assertBooleanField(Map<String, List<String>> result, boolean expected, Object field) {
        assertEquals(expected, Boolean.parseBoolean(result.get(field.toString()).get(0)));
    }

    @SuppressWarnings("boxing")
    private void assertFloatField(Map<String, List<String>> result, Float expected, Object field) {
        assertEquals(expected.floatValue(), Float.valueOf(result.get(field.toString()).get(0)));
    }

    private void assertStringField(Map<String, List<String>> result, String expected, Object field) {
    ...
    private void assertStringListField(Map<String, List<String>> result, List<String> list, Person.Field field) {
    ...
    private void assertEnumField(Map<String, List<Node>> result, Enum<?> expected, Person.Field field) {
        Map<String, List<String>> actual = childNodesToMap(result.get(field.toString()).get(0));
    ...
    private void assertListEnumField(Map<String, List<Node>> result, List<? extends Enum<? extends Enum.EnumKey>> expected, Person.Field field) {
        List<Node> actual = result.get(field.toString());
        for (int i = 0; i < actual.size(); i++) {
            Map<String, List<String>> nm = childNodesToMap(actual.get(i));
            assertEquals(expected.get(i).getDisplayValue(), nm.get("displayValue").get(0));
            assertEquals(expected.get(i).getValue().toString(), nm.get("value").get(0));
        }
    }
   ...
   */
    ...
    private void assertPerson(Map<String, List<Node>> person, String expectedId, String expectedName) throws Exception {
        assertEquals(expectedId, person.get("id").get(0).getTextContent());
        assertEquals(expectedName, childNodesToMap(person.get("name").get(0)).get("formatted").get(0));
    }
}
