...
public class CmpInstruction extends TypedInstruction {
    ...
    private static Class[][] _mappings = new Class[][] { { int.class, long.class }, { byte.class, long.class }, { char.class, long.class }, { short.class, long.class }, { boolean.class, long.class }, { void.class, long.class }, { Object.class, long.class } };
    ...
    public TypedInstruction setType(String type) {
        ...
        if (type == null) return (TypedInstruction) setOpcode(Constants.NOP);
        int opcode = getOpcode();
        switch(type.charAt(0)) {
            case 'l':
                return (TypedInstruction) setOpcode(Constants.LCMP);
            case 'f':
                if ((opcode == Constants.FCMPL) || (opcode == Constants.DCMPL)) return (TypedInstruction) setOpcode(Constants.FCMPL);
                return (TypedInstruction) setOpcode(Constants.FCMPG);
            case 'd':
                if ((opcode == Constants.FCMPL) || (opcode == Constants.DCMPL)) return (TypedInstruction) setOpcode(Constants.DCMPL);
    ...
    public CmpInstruction setNaNValue(int nan) {
        switch(getOpcode()) {
            case Constants.FCMPL:
            case Constants.FCMPG:
                if (nan == 1) setOpcode(Constants.FCMPG); else if (nan == -1) setOpcode(Constants.FCMPL); else throw new IllegalArgumentException("Invalid nan for type");
            case Constants.DCMPL:
            case Constants.DCMPG:
                if (nan == 1) setOpcode(Constants.DCMPG); else if (nan == -1) setOpcode(Constants.DCMPL); else throw new IllegalArgumentException("Invalid nan for type");
            default:
                if (nan != 0) throw new IllegalArgumentException("Invalid nan for type");
        }
        return this;
    }
