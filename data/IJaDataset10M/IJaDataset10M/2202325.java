...
public class SummarizeStrategyMap extends SummarizeStrategy {
    ...
    TreeMap<Record, List<Summary>> results = new TreeMap<Record, List<Summary>>();
    ...
    Row get(Dir dir, boolean rewound) {
        ...
        if (rewound) {
            ...
            if (dir == Dir.NEXT) cur = selected.firstEntry(); else cur = selected.lastEntry();
        } else if (dir == Dir.NEXT) cur = selected.higherEntry(cur.getKey()); else cur = selected.lowerEntry(cur.getKey());
        if (cur == null) {
            rewound = true;
            return null;
        }
    ...
    void process() {
        ...
        while (null != (row = source.get(Dir.NEXT))) {
            ...
            for (int i = 0; i < sums.size(); ++i) sums.get(i).add(row.getval(q.getHdr(), q.on.get(i)));
        }
    }
}
