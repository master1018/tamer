...
public class RenamedResultSetMetaData extends WrappedResultSetMetaData {
    ...
    public RenamedResultSetMetaData(ResultSetMetaData metaData, String newTableName, int[] columns, String... newNames) {
        ...
        try {
            if (columns.length != newNames.length) throw new MetaDataException("the number of specified indices and column names does not match");
            int columnCount = metaData.getColumnCount();
            HashSet<String> hashSet = new HashSet<String>(columns.length);
            renamings = new HashMap<Integer, String>(columns.length);
            for (int i = 0; i < columns.length; i++) {
                if (columns[i] < 1 || columnCount < columns[i]) throw new MetaDataException("the specified column " + columns[i] + " cannot be renamed because it does not exist in the underlying meta data");
                if (hashSet.contains(newNames[i])) throw new MetaDataException("renaming cannot be applied because the name " + newNames[i] + " is specified twice");
                hashSet.add(newNames[i]);
                renamings.put(columns[i], newNames[i]);
            }
            for (int i = 1; i <= columnCount; i++) if (!renamings.containsKey(i) && hashSet.contains(getColumnName(i))) throw new MetaDataException("renaming cannot be applied because the name " + newNames[i] + " is already exists in the underlying meta data");
    ...
    public String getTableName(int column) throws SQLException {
        return renaming != null ? renaming : metaData.getTableName(column);
    }
    ...
    public String getColumnName(int column) throws SQLException {
        return renamings.containsKey(column = originalColumnIndex(column)) ? renamings.get(column) : metaData.getColumnName(column);
    }
