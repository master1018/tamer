...
public class SimpleClient implements ServerSession {
    ...
    public void login(Properties props) throws IOException {
        synchronized (this) {
            if (connectionStateChanging || clientConnection != null) {
                RuntimeException re = new IllegalStateException("Session already connected or connecting");
    ...
    public void logout(boolean force) {
        ...
        if (force) {
            ...
            try {
                ByteBuffer msg = ByteBuffer.wrap(new byte[] { SimpleSgsProtocol.LOGOUT_REQUEST });
	 ...
	 */
        private void sendLoginRequest() {
            ...
            MessageBuffer msg = new MessageBuffer(2 + MessageBuffer.getSize(user) + MessageBuffer.getSize(pass));
            msg.putByte(SimpleSgsProtocol.LOGIN_REQUEST).putByte(SimpleSgsProtocol.VERSION).putString(user).putString(pass);
            try {
	 ...
	 */
        private void sendRelocateRequest() {
            ...
            buf.put(SimpleSgsProtocol.RELOCATE_REQUEST).put(SimpleSgsProtocol.VERSION).put(relocateKey).flip();
            try {
        ...
        public void disconnected(boolean graceful, byte[] message) {
            ...
            for (SimpleClientChannel channel : channels.values()) {
                try {
                    ...
                    logger.logThrow(Level.FINE, e, "During leftChannel ({0}) on disconnect:", channel.getName());
                }
            }
            channels.clear();
            if (loginFailed) {
        ...
        public void receivedMessage(byte[] message) {
            try {
                ...
                if (logger.isLoggable(Level.FINER)) {
                    String logMessage = String.format("Message length:%d", message.length);
                ...
                if (isConnected()) {
                    try {
                        ...
                        logger.logThrow(Level.FINEST, e2, "Disconnect failed after {0}", e.getMessage());
                    }
                }
            }
        }
        ...
        private void handleApplicationMessage(MessageBuffer msg) throws IOException {
            ...
            switch(command) {
                ...
                case SimpleSgsProtocol.CHANNEL_MESSAGE:
                    ...
                    break;
                default:
                    throw new IOException(String.format("Unknown session opcode: 0x%02X", command));
            }
        }
        ...
        private void handleLoginFailure(MessageBuffer msg) {
            ...
            try {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.logThrow(Level.FINE, e, "Disconnecting after login failure throws");
                }
            }
        ...
        private void handleLoginRedirect(MessageBuffer msg) throws IOException {
            ...
            redirectOrRelocateToNewNode(host, port, new SimpleClientConnectionListener(authentication));
        }
	 ...
	 * Process a suspend messages request.
         ...
         * @throws IOException if an IO problem occurs
         */
        private void handleSuspendMessages(MessageBuffer msg) throws IOException {
	 ...
	 */
        private void handleRelocateNotification(MessageBuffer msg) throws IOException {
            String host = msg.getString();
            int port = msg.getInt();
            byte[] relocateKey = msg.getBytes(msg.limit() - msg.position());
            logger.log(Level.FINER, "Relocate notification: {0}:{1}", host, port);
            checkLoggedIn();
            synchronized (lock) {
                if (!isSuspended) {
                    try {
                        ...
                        if (logger.isLoggable(Level.FINE)) {
                            logger.logThrow(Level.FINE, e, "Disconnecting (because " + "relocate notification received before " + "suspend) throws");
                        }
                    }
                    throw new IllegalStateException("client not suspended");
                }
            }
            redirectOrRelocateToNewNode(host, port, new SimpleClientConnectionListener(relocateKey));
        }
	 ...
	 * @throws IOException if an I/O problem occurs
	 */
        private void redirectOrRelocateToNewNode(String host, int port, ClientConnectionListener listener) throws IOException {
            ...
            try {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.logThrow(Level.FINE, e, "Disconnecting after login redirect " + "throws");
                }
            }
	 ...
	 */
        private void handleRelocateFailure(MessageBuffer msg) {
            ...
            try {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.logThrow(Level.FINE, e, "Disconnecting after relocate failure throws");
                }
            }
        }
        ...
        private void handleSessionMessage(MessageBuffer msg) {
            ...
            try {
                ...
                if (logger.isLoggable(Level.WARNING)) {
                    logger.logThrow(Level.WARNING, e, "SimpleClientListener.receivedMessage callback " + "throws");
                }
            }
        }
        ...
        private void handleReconnectFailure(MessageBuffer msg) {
            try {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.logThrow(Level.FINE, e, "Disconnecting a failed reconnect throws");
                }
            }
        }
        ...
        private void handleLogoutSuccess(MessageBuffer msg) {
            ...
            try {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.logThrow(Level.FINE, e, "Disconnecting after graceful logout " + "throws");
                }
            }
        }
        ...
        private void handleChannelJoin(MessageBuffer msg) {
            ...
            SimpleClientChannel channel = new SimpleClientChannel(channelName, channelId);
            if (channels.putIfAbsent(channelId, channel) == null) {
                channel.joined();
            } else {
                logger.log(Level.WARNING, "Cannot join channel {0}: already a member", channelName);
            }
        }
        ...
        private void handleChannelLeave(MessageBuffer msg) {
            ...
            if (channel != null) {
                ...
                logger.log(Level.WARNING, "Cannot leave channel {0}: not a member", channelId);
            }
        }
        ...
        private void handleChannelMessage(MessageBuffer msg) {
            ...
            if (channel == null) {
                logger.log(Level.WARNING, "Ignore message on channel {0}: not a member", channelId);
                return;
            }
            byte[] msgBytes = msg.getBytes(msg.limit() - msg.position());
            ByteBuffer buf = ByteBuffer.wrap(msgBytes);
        ...
        public void reconnected(byte[] message) {
            RuntimeException re = new UnsupportedOperationException("Not supported by SimpleClient");
    ...
    final class SimpleClientChannel implements ClientChannel {
        ...
        public void send(ByteBuffer message) throws IOException {
            synchronized (lock) {
                ...
                if (!isJoined.get()) {
                    throw new IllegalStateException("Cannot send on unjoined channel " + channelName);
                }
                byte[] idBytes = channelId.toByteArray();
                ByteBuffer msg = ByteBuffer.allocate(3 + idBytes.length + message.remaining());
                msg.put(SimpleSgsProtocol.CHANNEL_MESSAGE).putShort((short) idBytes.length).put(idBytes).put(message).flip();
                sendRaw(msg);
            }
        }
        ...
        void joined() {
            if (!isJoined.compareAndSet(false, true)) {
                throw new IllegalStateException("Already joined to channel " + channelName);
            }
        ...
        void left() {
            if (!isJoined.compareAndSet(true, false)) {
                throw new IllegalStateException("Cannot leave unjoined channel " + channelName);
            }
        ...
        void receivedMessage(ByteBuffer message) {
            if (!isJoined.get()) {
                throw new IllegalStateException("Cannot receive on unjoined channel " + channelName);
            }
