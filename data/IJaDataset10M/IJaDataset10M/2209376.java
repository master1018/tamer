...
public class ProcessHTTPrequest extends Thread implements ExternalHTTPRequests {
    ...
    protected String requestParametersEncoded = null;

    private Hashtable requestParametersTable = null;

    protected String mimetype = "text/html";

    private static final String mimePrefix = "MIME";

    protected boolean headersOnly = false;

    protected boolean isAdminServer = false;

    private static final String S_200 = "200 OK";

    private static final String S_301 = "301 Moved Permanently";

    private static final String S_302 = "302 Found";

    private static final String S_303 = "303 See Other";

    private static final String S_307 = "307 Temporary Redirect";

    private static final String S_400 = "400 Bad Request";

    private static final String S_401 = "401 Unauthorized";

    private static final String S_404 = "404 Not Found";

    private static final String S_500 = "500 Internal Server Error";

    private static final String S_501 = "501 Not Implemented";
    ...
    public ProcessHTTPrequest(Socket a_sock, HTTPserver a_webServer, CMProps a_page, boolean a_isAdminServer) {
    ...
    public Hashtable getVirtualDirectories() {
        return webServer.getVirtualDirectories();
    }
    ...
    public String getHTTPstatusInfo() {
        return statusExtra == null ? "" : statusExtra;
    }
    ...
    protected boolean process(String inLine) throws Exception {
        ...
        try {
            ...
            if (command.startsWith("[")) {
                int err = 400;
                if (command.length() > 1) err = CMath.s_int(command.substring(1));
                switch(err) {
                    case 200:
                        status = S_200;
                        break;
            ...
            if (command.equalsIgnoreCase("HEAD")) {
                headersOnly = true;
                command = "GET";
            } else if (command.equalsIgnoreCase("MUD")) {
                ...
                if (p == -1) {
                    ...
                    if (p == 0) {
                        ...
                        if (p < request.length()) requestParametersEncoded = request.substring(p + 1);
                    }
                }
                try {
    ...
    private Hashtable getRequestParameters() {
        if (requestParametersTable != null) return requestParametersTable;
        requestParametersTable = new Hashtable();
        if (requestParametersEncoded == null) return requestParametersTable;
        ...
        while (pStr != null) {
            ...
            if (p == -1) {
                ...
                if (p < pStr.length()) pStr = pStr.substring(p + 1); else pStr = null;
            }
            ...
            if (eq == -1) {
                ...
                if (eq < thisParam.length()) {
                    try {
                        if (thisParamName.equalsIgnoreCase("RAWTEXT")) thisParamValue = URLDecoder.decode(thisParam.substring(eq + 1), "UTF-8"); else thisParamValue = preFilter(new StringBuffer(URLDecoder.decode(thisParam.substring(eq + 1), "UTF-8")));
                    } catch (UnsupportedEncodingException e) {
                        Log.errOut(getName(), "Received wrong encoding.2");
                    } catch (Exception e2) {
                        if ((!Log.isMaskedErrMsg(e2.getMessage())) || (CMSecurity.isDebugging("HTTPERR"))) Log.errOut(getName(), e2.getMessage());
                        if (CMSecurity.isDebugging("HTTPERREXT")) Log.errOut(getName(), e2);
                    }
                }
            }
            if (thisParamName.length() > 0) {
                if (!requestParametersTable.containsKey(thisParamName.toUpperCase())) requestParametersTable.put(thisParamName.toUpperCase(), thisParamValue); else for (int i = 1; ; i++) if (!requestParametersTable.containsKey(thisParamName.toUpperCase() + (Integer.toString(i)))) {
    ...
    protected String parseFoundMacro(StringBuffer s, int i, boolean lookOnly) {
        ...
        if ((i < s.length() - 2) && ((s.charAt(i + 1) == 'X') || (s.charAt(i + 1) == 'x')) && (Character.isDigit(s.charAt(i + 2)) || (s.charAt(i + 1) == 'x') || (s.charAt(i + 1) == 'X'))) return null;
        for (int x = i + 1; x < s.length(); x++) {
            if ((s.charAt(x) == '@') && (extend) && (x < (s.length() - 1)) && (s.charAt(x + 1) == '@')) {
                if (!lookOnly) s.deleteCharAt(x);
                while ((x < s.length()) && (s.charAt(x) == '@')) x++;
                x--;
            } else if ((s.charAt(x) == '@') && ((!extend) || (x >= s.length() - 1) || (s.charAt(x + 1) != '@'))) {
                foundMacro = s.substring(i + 1, x);
                break;
            } else if ((s.charAt(x) == '?') && (Character.isLetterOrDigit(s.charAt(x - 1)))) extend = true; else if (((x - i) > CMClass.longestWebMacro) && (!extend)) break;
        }
        return foundMacro;
    }
        ...
        for (; i < s.length(); i++) {
            if (s.charAt(i) == '@') {
                ...
                if ((foundMacro != null) && (foundMacro.length() > 0)) {
                    if (foundMacro.equalsIgnoreCase("loop")) backsToFind++; else if (foundMacro.equalsIgnoreCase("back")) {
                        backsToFind--;
                        if (backsToFind <= 0) return i;
                    }
                }
            }
        }
    ...
    protected String preFilter(StringBuffer input) {
        ...
        while (x < input.length()) {
            ...
            if (c == '\'') input.setCharAt(x, '`'); else if (c == 8) {
                String newStr = input.toString();
                if (x == 0) input = new StringBuffer(newStr.substring(x + 1)); else {
    ...
    protected int myEndif(StringBuffer s, int i) {
        ...
        for (; i < s.length(); i++) {
            if (s.charAt(i) == '@') {
                String foundMacro = parseFoundMacro(s, i, true);
                if ((foundMacro != null) && (foundMacro.length() > 0)) {
                    if (foundMacro.startsWith("if?")) endifsToFind++; else if (foundMacro.equalsIgnoreCase("endif")) {
                        endifsToFind--;
                        if (endifsToFind <= 0) return i;
                    }
                }
            }
        }
    ...
    private int myEndJScript(StringBuffer s, int i) {
        for (; i < s.length(); i++) {
            if (s.charAt(i) == '@') {
                String foundMacro = parseFoundMacro(s, i, true);
                if ((foundMacro != null) && (foundMacro.length() > 0)) {
                    if (foundMacro.equalsIgnoreCase("/jscript")) return i;
                }
            }
        }
        return -1;
    ...
    protected String runMacro(String foundMacro) throws HTTPRedirectException, HTTPServerException {
        ...
        if (x >= 0) {
            ...
            while (y >= 0) {
                ...
                if ((newFoundMacro != null) && (newFoundMacro.length() > 0)) {
                    ...
                    if (qq != null) parms.replace(y, y + l + 2, qq); else parms.replace(y, y + l + 2, "[error]");
                } else break;
                y = parms.indexOf("@");
            }
        }
        if (foundMacro.length() == 0) return "";
        ...
        if (W != null) {
            ...
            if (!isAdminServer && W.isAdminMacro()) {
                ...
                if (bin == null) q = " @break@"; else q = new String(bin);
            } else q = W.runMacro(this, (parms == null) ? null : parms.toString());
            if (q != null) return q;
            return "[error]";
        }
        return null;
        ...
        for (; i < end; i++) {
            if (s.charAt(i) == '@') {
                ...
                if ((foundMacro != null) && (foundMacro.length() > 0)) {
                    if (foundMacro.startsWith("if?")) endifsToFind++; else if (foundMacro.equalsIgnoreCase("endif")) {
                        endifsToFind--;
                        if (endifsToFind <= 0) return -1;
                    } else if ((foundMacro.equalsIgnoreCase("else")) && (endifsToFind == 1)) return i;
                }
            }
        }
        return -1;
    ...
    public StringBuffer doVirtualPage(StringBuffer s) throws HTTPRedirectException {
        ...
        if ((webServer != null) && (!webServer.isAdminServer) && (processStartTime > 0) && (System.currentTimeMillis() - processStartTime) > (120 * 1000)) {
        ...
        try {
                ...
                if (s.charAt(i) == '@') {
                    ...
                    if ((foundMacro != null) && (foundMacro.length() > 0)) {
                        if (foundMacro.startsWith("if?")) {
                            ...
                            int v = myEndif(s, i + l);
                            if (v < 0) s.replace(i, i + l, "[if without endif]"); else {
                                ...
                                try {
                                    ...
                                    if ((q != null) && (q.equalsIgnoreCase(compare))) {
                                        if (v2 >= 0) s.replace(v2, v + 7, ""); else s.replace(v, v + 7, "");
                                        s.replace(i, i + l, "");
                                    } else {
                                        if (v2 >= 0) s.replace(i, v2 + 6, ""); else s.replace(i, v + 7, "");
                                    }
                            ...
                            if (v < 0) s.replace(i, i + l, "[jscript without /jscript]"); else {
                                Context cx = Context.enter();
                                try {
                                    String script = s.substring(i + l, v);
                                    JScriptablePage scope = new JScriptablePage(this);
                                    cx.initStandardObjects(scope);
                                    scope.defineFunctionProperties(JScriptablePage.functions, JScriptablePage.class, ScriptableObject.DONTENUM);
                            ...
                            int v = myBack(s, i + 6);
                            if (v < 0) s.replace(i, i + 6, "[loop without back]"); else {
                        ...
                        if (foundMacro.length() > 0) {
                            try {
                                ...
                                String q = runMacro(foundMacro);
                                if (q != null) {
                                    if ((analLogging) && (q.toUpperCase().indexOf("@BREAK@") >= 0)) Log.infoOut(getName(), "WebMacro:" + foundMacro + " generated a BREAK! at " + i);
                                    s.replace(i, i + l + 2, q);
                                } else s.replace(i, i + l + 2, "[error]");
                            } catch (HTTPRedirectException e) {
                                redirectTo = e.getMessage();
                            }
                        }
            ...
            if ((errMsg != null) && ((!Log.isMaskedErrMsg(errMsg)) || (CMSecurity.isDebugging("HTTPERR")))) {
    ...
    public String makeRedirectPage(String url) {
        return "<html><head>" + "<META HTTP-EQUIV=\"Expires\" CONTENT=\"Mon, 06 Jan 1990 00:00:01 GMT\">" + "<meta HTTP-EQUIV=\"Refresh\" CONTENT=\"2; URL=" + url + "\">" + "<script>self.location.href = \"" + url + "\";</script>" + "</head><body>" + "<br>Redirecting in 2 seconds; if redirection does not work, " + "<a href=\"" + url + "\">click here!</a><br></body></html>";
    }
            ...
            try {
                ...
                if (processOK) {
                    ...
                    if ((W != null) && (W.isAWebPath()) && ((!W.isAdminMacro()) || (isAdminServer))) {
                        requestedFile = null;
                        virtualPage = false;
                        if (W.preferBinary()) replyData = W.runBinaryMacro(this, ""); else {
                            virtualPage = true;
                            replyData = W.runMacro(this, "").getBytes();
                        }
                        filename = W.getFilename(this, filename);
                    } else requestedFile = webServer.pageGrabber.grabFile(filename);
                    if (W != null) contentHeader = W.getSpecialContentHeader(filename);
                    if (requestedFile != null) switch(requestedFile.state) {
                        case GrabbedFile.STATE_OK:
                            break;
                        case GrabbedFile.STATE_IS_DIRECTORY:
                    ...
                    if (processOK) {
                        ...
                        if (page.getStr("VIRTUALPAGEEXTENSION").equalsIgnoreCase(exten)) virtualPage = true;
                ...
                if (!processOK || replyData == null) {
                    if (totalRequest.equalsIgnoreCase("MUD")) return;
                ...
                if (contentHeader != null) sout.writeBytes(contentHeader); else sout.writeBytes("Content-Type: " + mimetype + cr);
                if ((replyData != null)) {
                ...
                if ((errMsg != null) && ((!Log.isMaskedErrMsg(errMsg)) || (CMSecurity.isDebugging("HTTPERR")))) {
                    ...
                    if ((!(e instanceof java.net.SocketException)) || (CMSecurity.isDebugging("HTTPERREXT"))) Log.errOut(getName(), e);
                }
            }
            if ((Log.debugChannelOn()) && (CMSecurity.isDebugging("HTTPREQ")) && (replyData != null)) Log.debugOut(getName(), sock.getInetAddress().getHostAddress() + ":" + (command == null ? "(null)" : command + " " + (request == null ? "(null)" : request)) + ":" + status + " (" + replyData.length + ")");
            try {
                if (sout != null) {
                    sout.flush();
                    OutputStream o = sock.getOutputStream();
                    if (bout != null) o.write(bout.toByteArray());
                    o.flush();
                    sout.close();
                    o.close();
                    sout = null;
    ...
    public String getHTTPclientIP() {
        if (sock != null) return sock.getInetAddress().getHostAddress();
        return "[NOT CONNECTED]";
    }

    public InetAddress getServerAddress() {
        if (sock != null) return sock.getLocalAddress();
        return null;
    }
    ...
    public String getPageContent(String filename) {
        try {
            ...
            if ((requestedFile == null) || (requestedFile.state != GrabbedFile.STATE_OK)) return "";
            ...
            if (page.getStr("VIRTUALPAGEEXTENSION").equalsIgnoreCase(exten)) replyData = doVirtualPage(replyData);
    ...
    protected Vector getData(InputStream sin) {
        ...
        try {
            ...
            int byteCounter = 0;
            while (((contentLength < 0) || (out.size() < contentLength)) && ((System.currentTimeMillis() - startTime) < 20000)) {
                c = (byte) BR.read();
                ...
                } else if ((c != 10) || ((contentLength > 0) && (out.size() > 0))) {
                    out.write(c);
                    if ((++byteCounter) > 1024) {
                        byteCounter = 0;
            ...
            if ((!Log.isMaskedErrMsg(e.getMessage())) || (CMSecurity.isDebugging("HTTPERR"))) Log.errOut(getName(), e.getMessage());
            if (CMSecurity.isDebugging("HTTPERREXT")) Log.errOut(getName(), e);
        }
        if (data.size() == 0) data.addElement(new String(out.toByteArray())); else if (out.size() > 0) data.addElement(out.toByteArray());
        return data;
    }
    ...
    public String getContentType(Vector data) {
        for (int s = 0; s < data.size(); s++) {
            ...
            if (O instanceof String) {
                String str = (String) O;
                if (str.toLowerCase().startsWith("content-type: ")) return str.substring(14).trim();
            }
        }
        return "";
    }
    ...
    public int getContentLength(Vector data) {
        for (int s = 0; s < data.size(); s++) {
            ...
            if (O instanceof String) {
                String str = (String) O;
                if (str.toLowerCase().startsWith("content-length: ")) return CMath.s_int(str.substring(16).trim());
            }
        }
        return -1;
    }
    ...
    public String getBoundary(Vector data) {
        for (int s = 0; s < data.size(); s++) {
            ...
            if (O instanceof String) {
                ...
                if (x >= 0) return str.substring(x + 9).trim();
            }
        }
        return "";
    }
    ...
    public String getHTTPRequest(InputStream sin) {
        try {
            ...
            if ((inData == null) || (inData.size() == 0) || (!(inData.elementAt(0) instanceof String))) return "[400 -- no request received]";
            String inLine = (String) inData.elementAt(0);
            if (CMSecurity.isDebugging("HTTPACCESS")) Log.debugOut("HTTP", inLine);
            if ((inLine.startsWith("GET") || inLine.startsWith("HEAD") || inLine.equalsIgnoreCase("MUD"))) return inLine; else if (inLine.startsWith("POST")) {
                String type = getContentType(inData);
                if (type.length() == 0) return "";
                if (!(inData.lastElement() instanceof byte[])) return "[400 -- no content data received]";
                if (type.toLowerCase().indexOf("multipart/form-data") >= 0) {
                    ...
                    while (s < buf.length) {
                        if (byteCompare(buf, s, bounBytes)) {
                            if (lastEnd > 0) {
                                for (int i = lastEnd; i < s; i++) if (byteCompare(buf, i, CRCR)) {
                                    ...
                                    if (nameDex >= 0) {
                                        ...
                                        if (nameEnDex > nameDex) name = header.substring(nameDex + 6, nameEnDex);
                                    }
                                    nameDex = header.indexOf("filename=\"");
                                    String filename = null;
                                    if (nameDex >= 0) {
                                        int nameEnDex = header.indexOf("\"", nameDex + 10);
                                        if (nameEnDex > nameDex) filename = header.substring(nameDex + 10, nameEnDex);
                                    }
                                    ...
                                    if (!binary) {
                                        ...
                                        for (int i3 = 0; i3 < filename.length(); i3++) {
                                            if ((filename.charAt(i3) == '\\') || (filename.charAt(i3) == '/')) {
                                                c3 = filename.charAt(i3);
                                                break;
                                            }
                                        }
                                        if (c3 != ' ') filename = filename.substring(filename.lastIndexOf(c3) + 1);
                    ...
                    if (parms.length() > 0) return inLine.substring(0, x) + "?" + (parms.toString().substring(1));
                    return inLine.substring(0, x);
                } else if (type.toLowerCase().indexOf("urlencoded") >= 0) {
                    String lastLine = new String((byte[]) inData.lastElement());
                    int x = inLine.lastIndexOf(" ");
                    if (x < 0) return "[400 -- improperly formatted post request]";
                    return inLine.substring(0, x) + "?" + lastLine + inLine.substring(x);
                } else return "[501 -- unsupported post type]";
            } else return "[501 -- command not implemented]";
        } catch (Exception e) {
            String errMsg = e.getMessage() == null ? e.toString() : e.getMessage();
            if ((errMsg != null) && ((!Log.isMaskedErrMsg(errMsg)) || (CMSecurity.isDebugging("HTTPERR")))) {
                Log.errOut(getName(), "Exception: " + errMsg);
    ...
    protected static class JScriptablePage extends ScriptableObject {
        ...
        public ExternalHTTPRequests request() {
            return req;
        }

        public JScriptablePage(ExternalHTTPRequests requests) {
            req = requests;
        }
    ...
    public String getWebServerPortStr() {
        return getWebServer().getPortStr();
    }

    public String getWebServerPartialName() {
        return getWebServer().getPartialName();
    }

    public MudHost getMUD() {
        return getWebServer().getMUD();
    }
}
