...
public abstract class TypeElementBuilder extends ExpressionBuilder {
     ...
     */
    public TypeElementBuilder(final ExpressionElementManager expressionManager, final BuildDataManager buildDataManager) {
    ...
    protected void afterExited(final AstVisitEvent event) {
        ...
        final UnresolvedUnitInfo<? extends UnitInfo> outerUnit = this.buildDataManager.getCurrentUnit();
        ...
        if (token.isTypeDescription()) {
            ...
            this.buildConstantElement((ConstantToken) token, outerUnit, fromLine, fromColumn, toLine, toColumn);
        }
    }

    protected void buildArrayType(final UnresolvedUnitInfo<? extends UnitInfo> outerUnit, final int fromLine, final int fromColumn, final int toLine, final int toColumn) {
        ...
        if (elements.length > 0) {
            if (elements[0] instanceof IdentifierElement) {
                final UnresolvedReferenceTypeInfo<? extends ReferenceTypeInfo> referenceType = this.buildReferenceType(elements);
                typeElement = new TypeElement(UnresolvedArrayTypeInfo.getType(referenceType, 1), outerUnit, fromLine, fromColumn, toLine, toColumn);
            } else if (elements[0] instanceof TypeElement) {
                typeElement = ((TypeElement) elements[0]).getArrayDimensionInclementedInstance(outerUnit, fromLine, fromColumn, toLine, toColumn);
            }
        }
        if (null != typeElement) {
    ...
    protected void buildType(final UnresolvedUnitInfo<? extends UnitInfo> outerUnit, final int fromLine, final int fromColumn, final int toLine, final int toColumn) {
        final ExpressionElement[] elements = this.getAvailableElements();
        assert (elements.length > 0) : "Illegal state: type description was not found.";
        if (elements.length > 0) {
            if (elements[0] instanceof IdentifierElement) {
                this.pushElement(new TypeElement(this.buildReferenceType(elements), outerUnit, fromLine, fromColumn, toLine, toColumn));
            } else if (elements[0] instanceof TypeElement) {
    ...
    protected void buildTypeWildCard(final UnresolvedUnitInfo<? extends UnitInfo> outerUnit, final int fromLine, final int fromColumn, final int toLine, final int toColumn) {
    ...
    protected UnresolvedReferenceTypeInfo<? extends ReferenceTypeInfo> buildReferenceType(final ExpressionElement[] elements) {
        ...
        UnresolvedClassTypeInfo resultType = new UnresolvedClassTypeInfo(UnresolvedClassImportStatementInfo.getClassImportStatements(this.buildDataManager.getAllAvaliableNames()), typeName);
    ...
    protected void buildBuiltinType(final BuiltinTypeToken token, final UnresolvedUnitInfo<?> outerUnit) {
        this.pushElement(TypeElement.getBuiltinTypeElement(token, outerUnit));
    }

    protected void buildConstantElement(final ConstantToken token, final UnresolvedUnitInfo<? extends UnitInfo> outerUnit, final int fromLine, final int fromColumn, final int toLine, final int toColumn) {
        final UnresolvedLiteralUsageInfo literal = new UnresolvedLiteralUsageInfo(token.toString(), token.getType());
    ...
    protected boolean isTriggerToken(final AstToken token) {
        return token.isBuiltinType() || token.isTypeDescription() || token.isConstant() || token.isArrayDeclarator() || token.isTypeArgument() || token.isTypeWildcard();
    }
}
