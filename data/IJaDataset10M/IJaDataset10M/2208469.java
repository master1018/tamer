package de.uniwue.dltk.textmarker.internal.core.codeassist.completion;
...
import de.uniwue.dltk.textmarker.parser.ast.TextMarkerStatement;
...
public class CompletionOnKeywordArgumentOrFunctionArgument extends SimpleReference implements ICompletionOnKeyword {
    ...
    public CompletionOnKeywordArgumentOrFunctionArgument(String token, ASTNode completionNode, TextMarkerStatement node, String[] KeywordspossibleKeywords) {
    ...
    public CompletionOnKeywordArgumentOrFunctionArgument(String token, TextMarkerStatement node, String[] possibleKeywords, int position) {
    ...
    public int argumentIndex() {
        if (this.completionNode == null) {
            ...
            if (statement.getCount() > 2 && statement.getAt(0).sourceEnd() <= sourceStart() && sourceEnd() <= statement.getAt(1).sourceStart()) {
                return 1;
            }
            return -1;
        }
