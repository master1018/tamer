...
public class FontDialog extends javax.swing.JDialog {
    ...
    public FontDialog(java.awt.Frame parent, boolean modal) {
        ...
        lmSize.addElement(8);
        lmSize.addElement(9);
        lmSize.addElement(10);
        lmSize.addElement(11);
        lmSize.addElement(12);
        lmSize.addElement(14);
        lmSize.addElement(16);
        lmSize.addElement(18);
        lmSize.addElement(20);
        lmSize.addElement(22);
        lmSize.addElement(24);
        lmSize.addElement(26);
        lmSize.addElement(28);
        lmSize.addElement(36);
        lmSize.addElement(48);
        lmSize.addElement(52);
        lmSize.addElement(64);
        lmSize.addElement(72);
        getFontXList();
        String lastFx = null;
        for (FontX fx : fxlist) {
    ...
    public void setFont(Font f) {
        ...
        switch(f.getStyle()) {
            case Font.PLAIN:
                tfStyle.setText(sPlain);
                break;
            case Font.BOLD:
                tfStyle.setText(sBold);
                break;
            case Font.ITALIC:
                tfStyle.setText(sItalic);
                break;
            case Font.BOLD + Font.ITALIC:
                tfStyle.setText(sBoldItalic);
                break;
            default:
                tfStyle.setText(sPlain);
                break;
    ...
    public Font getFont() {
        try {
            Font newFont = new Font(tfFont.getText(), getFontStyleFromText(), Integer.parseInt(tfSize.getText()));
            return newFont;
        } catch (Exception exc) {
            return f;
        }
    ...
    private int getFontStyleFromText() {
        if (tfStyle.getText().equalsIgnoreCase(sPlain)) {
            return Font.PLAIN;
        }
        if (tfStyle.getText().equalsIgnoreCase(sBold)) {
            return Font.BOLD;
        }
        if (tfStyle.getText().equalsIgnoreCase(sItalic)) {
            return Font.ITALIC;
        }
        if (tfStyle.getText().equalsIgnoreCase(sBoldItalic)) {
    ...
    public void setLanguageAndConfigure(Language lang) {
        ...
        if (lang.getValueOf("titleFDL") != null) {
            setTitle(lang.getValueOf("titleFDL"));
        }
        ...
        if (lang.getValueOf("tbdFont") != null) {
            tb.setTitle(lang.getValueOf("tbdFont"));
        }
        tb = (javax.swing.border.TitledBorder) panStyle.getBorder();
        if (lang.getValueOf("tbdStyle") != null) {
            tb.setTitle(lang.getValueOf("tbdStyle"));
        }
        tb = (javax.swing.border.TitledBorder) panSize.getBorder();
        if (lang.getValueOf("tbdSize") != null) {
            tb.setTitle(lang.getValueOf("tbdSize"));
        }
        tb = (javax.swing.border.TitledBorder) panPreview.getBorder();
        if (lang.getValueOf("tbdPreview") != null) {
            tb.setTitle(lang.getValueOf("tbdPreview"));
        }
    }
    ...
    private void mnuAbcdActionPerformed(java.awt.event.ActionEvent evt) {
        lblPreview.setText("<html>ABCDEFGHIJKLMNOPQRSTUVWXYZ<br />" + "abcdefghijklmnopqrstuvwxyz<br />" + "0123456789</html>");
    }

    private void mnuEnActionPerformed(java.awt.event.ActionEvent evt) {
        lblPreview.setText("<html>It doesn't matter...<br />" + "Enjoy it!</html>");
    }

    private void mnuFrActionPerformed(java.awt.event.ActionEvent evt) {
        lblPreview.setText("<html>À cœur vaillant rien d'impossible.<br />" + "Il n'y a que la vérité qui blesse.</html>");
    }

    private void mnuSpActionPerformed(java.awt.event.ActionEvent evt) {
        lblPreview.setText("<html>Pensar en la mañana.<br />" + "¿Verdad que es bonita?</html>");
    }
    ...
    */
    ...
    public class FontX extends Object {
        ...
        private void initFontStyle(Font font) {
            if (font.getFontName(Locale.US).toLowerCase().endsWith("bold italic") && font.getFontName(Locale.US).length() > "bold italic".length()) {
                fs = FontStyle.BOLDITALIC;
            } else if (font.getFontName(Locale.US).toLowerCase().endsWith("bold-italic") && font.getFontName(Locale.US).length() > "bold-italic".length()) {
                fs = FontStyle.BOLDITALIC;
            } else if (font.getFontName(Locale.US).toLowerCase().endsWith("bolditalic") && font.getFontName(Locale.US).length() > "bolditalic".length()) {
                fs = FontStyle.BOLDITALIC;
            } else if (font.getFontName(Locale.US).toLowerCase().endsWith("italic") && font.getFontName(Locale.US).length() > "italic".length()) {
                fs = FontStyle.ITALIC;
            } else if (font.getFontName(Locale.US).toLowerCase().endsWith("bold") && font.getFontName(Locale.US).length() > "bold".length()) {
                fs = FontStyle.BOLD;
            } else {
                fs = FontStyle.PLAIN;
            }
