...
public final class HTMLCache {
    ...
    private static final File HTML_ROOT = new File(GeneralConfig.HTML_ROOT);
    ...
    private File getCacheFile() {
        return new File(GeneralConfig.HTML_CACHE_FILE);
    }
    ...
    private String compactHtml(StringBuilder sb, String html) {
        sb.setLength(0);
        sb.append(html);
        for (int i = 0; i < sb.length(); i++) if (Character.isWhitespace(sb.charAt(i))) sb.setCharAt(i, ' ');
        replaceAll(sb, "  ", " ");
        replaceAll(sb, "< ", "<");
        replaceAll(sb, " >", ">");
        ...
        while (fromIndex < toIndex && sb.charAt(fromIndex) == ' ') fromIndex++;
        while (fromIndex < toIndex && sb.charAt(toIndex - 1) == ' ') toIndex--;
        return sb.substring(fromIndex, toIndex);
    }

    private void replaceAll(StringBuilder sb, String pattern, String value) {
        for (int index = 0; (index = sb.indexOf(pattern, index)) != -1; ) sb.replace(index, index + pattern.length(), value);
    }
    ...
    public void parseDir(File dir) {
        for (File file : dir.listFiles(HTML_FILTER)) {
            if (!file.isDirectory()) loadFile(file); else parseDir(file);
        }
    }
    ...
    public String loadFile(File file) {
        if (isLoadable(file)) {
            ...
            try {
                ...
                bis.read(raw);
                String content = new String(raw, GeneralConfig.HTML_ENCODING);
                String relpath = getRelativePath(HTML_ROOT, file);
                size += content.length();
                String oldContent = cache.get(relpath);
                if (oldContent == null) loadedFiles++; else size -= oldContent.length();
