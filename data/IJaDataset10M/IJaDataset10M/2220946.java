...
public class PositionTracker {
    ...
    public PositionTracker(PositionalPredicate positionPredicate) {
        if (positionPredicate != null) {
            ...
            while ((positionPredicate = positionPredicate.next) != null) positionMatches = positionMatches.next = new PositionMatches(positionPredicate);
        }
    }
    ...
    private PositionMatches getPositionMatches(Expression predicate) {
        ...
        do {
            if (positionMatches.predicate == predicate) return positionMatches;
    ...
    void removePositionListener(PositionalEvaluation evaluation) {
        ...
        if (positionalListeners == null) evaluation.disposed = true; else positionalListeners.removeListener(evaluation);
    }
    ...
    void addLastLitener(PositionalEvaluation evaluation) {
        ...
        if (predicate == null) {
            if (lastListenerTail == null) lastListenerHead = lastListenerTail = evaluation; else {
            ...
            if (matches.lastListenerTail == null) matches.lastListenerHead = matches.lastListenerTail = evaluation; else {
    ...
    void removeLastLitener(PositionalEvaluation evaluation) {
        ...
        Expression predicate = evaluation.expression.predicate;
        if (predicate == null) {
            if (prev != null) prev.next = next; else lastListenerHead = next;
            if (next != null) next.previous = prev; else lastListenerTail = prev;
        } else {
            PositionMatches matches = getPositionMatches(predicate);
            if (prev != null) prev.next = next; else matches.lastListenerHead = next;
            if (next != null) next.previous = prev; else matches.lastListenerTail = prev;
        }
    }
    ...
    public void expired() {
        if (lastListenerHead != null) {
            ...
            for (PositionalEvaluation lastEval = lastListenerHead; lastEval != null; lastEval = lastEval.next) lastEval.setResult(last);
