        ...
        return new AbstractResolver() {
            ...
            public void resolve(Identifier id, Input input) throws IOException, ResolverException {
                ...
                if (id instanceof ExternalIdentifier) {
                    ...
                    type = XML_TYPE;
                } else if (id instanceof XMLDocumentIdentifier) type = ((XMLDocumentIdentifier) id).getNamespaceUri();
                if (type == null) {
                    String mediaType = id.getMediaType();
                    if (mediaType.indexOf('*') < 0) type = IANA_MEDIA_TYPE_URI + mediaType;
                }
                String targetNamespace = null;
                if (id instanceof TargetNamespaceIdentifier) targetNamespace = ((TargetNamespaceIdentifier) id).getTargetNamespace();
                LSInput lsInput = resourceResolver.resolveResource(type, targetNamespace, publicId, id.getUriReference(), base);
                if (lsInput == null) return;
