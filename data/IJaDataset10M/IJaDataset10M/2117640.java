...
public class ScanObject implements VM_Constants, Constants {
   ...
   */
    public static void enumeratePointers(VM_Address object, Plan plan) throws VM_PragmaUninterruptible, VM_PragmaInline {
   ...
   */
    private static void scan(VM_Address objRef, boolean root, Plan plan, boolean trace) throws VM_PragmaUninterruptible, VM_PragmaInline {
        if (VM.VerifyAssertions) VM._assert(!objRef.isZero());
        if (MM_Interface.MOVES_OBJECTS) VM_ObjectModel.gcProcessTIB(objRef, root);
        ...
        if (type.isClassType()) {
            ...
            for (int i = 0, n = referenceOffsets.length; i < n; i++) {
                if (trace) MM_Interface.processPtrField(objRef.add(referenceOffsets[i]), root); else VM_Interface.enumeratePtrLoc(objRef.add(referenceOffsets[i]), plan);
            }
            ...
            if (elementType.isReferenceType()) {
                ...
                VM_Address location = objRef;
                VM_Address end = objRef.add(numBytes);
                while (location.LT(end)) {
                    if (trace) MM_Interface.processPtrField(location, root); else VM_Interface.enumeratePtrLoc(location, plan);
                    location = location.add(WORD_SIZE);
                }
    ...
    public static boolean validateRefs(VM_Address ref, int depth) throws VM_PragmaUninterruptible, VM_PragmaNoInline {
        VM_Type type;
