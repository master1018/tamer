 ...
 */
public class OperationSetMultiUserChatJabberImpl implements OperationSetMultiUserChat {
    ...
    public ChatRoom createChatRoom(String roomName, Hashtable roomProperties) throws OperationFailedException, OperationNotSupportedException {
        ...
        if (room != null) throw new OperationFailedException("There is already a room with", OperationFailedException.IDENTIFICATION_CONFLICT);
     ...
     * multi user chat
     */
    public List getCurrentlyJoinedChatRooms(Contact contact) throws OperationFailedException, OperationNotSupportedException {
        assertSupportedAndConnected();
     ...
     * multi user chat
     */
    public List getExistingChatRooms() throws OperationFailedException, OperationNotSupportedException {
        assertSupportedAndConnected();
    ...
    private void assertSupportedAndConnected() throws OperationFailedException, OperationNotSupportedException {
        ...
        if (MultiUserChat.isServiceEnabled(getXmppConnection(), jabberProvider.getAccountID().getUserID())) {
            throw new OperationNotSupportedException("Chat rooms not supported on server " + jabberProvider.getAccountID().getService() + " for user " + jabberProvider.getAccountID().getUserID());
        }
    }
    ...
    private String getCanonicalRoomName(String roomName) throws OperationFailedException {
        ...
        while (serviceNamesIter.hasNext()) {
            return roomName + "@" + (String) serviceNamesIter.next() + jabberProvider.getAccountID().getService();
        }
     ...
     */
    public List getCurrentlyJoinedChatRooms(ChatRoomMember chatRoomMember) throws OperationFailedException, OperationNotSupportedException {
        return null;
    }
