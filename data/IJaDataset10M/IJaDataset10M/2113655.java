...
public class ClustersUI extends JPanel {
    ...
    public void LogPreviewUI() {
        ...
        FlatTabbedPane tabbedPane = new FlatTabbedPane("Groups", new Color(240, 240, 240, 230), new Color(180, 180, 180, 120), new Color(220, 220, 220, 150));
    ...
    protected void clusterSelectionChanged() {
        ...
        if (clusters != null && clusters.size() > 0) {
            ...
            for (int i = 0; i < clusters.size(); i++) {
                ...
                for (int j = 0; j < selectedIndices.length; j++) if (selectedIndices[j] == i) isSelected = true;
                while (it.hasNext()) {
                    String user = it.next();
                    if (!user.equals("_reserved")) {
                        userNameList.add(user);
                        if (isSelected) list.add("+ " + user); else list.add("- " + user);
                    }
                }
            }
