...
public class AnalysisListener extends JPanel implements ActionListener, Serializable {
    ...
    public AnalysisListener(Hashtable _map) {
        map = _map;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        _table_1 = new ReportTable(new String[] { "Record Value", "Frequency", "% Freq." });
        _table_2 = new ReportTable(new String[] { "Range Metric", "Metric Value" });
        _table_3 = new ReportTable(new String[] { "Percentile %", "Record Upper Value", "Samples Below" });
        QueryBuilder a_q = new QueryBuilder(_dsn, _table, _col, Jdbc_conn123.getDBType());
        ...
        try {
            ...
            ResultSet rs = Jdbc_conn123.runQuery(aggr_query);
            while (rs.next()) {
                ...
                if (count_d <= 0) {
                    JOptionPane.showMessageDialog(null, "No Data to Analyse", "Error Message", JOptionPane.ERROR_MESSAGE);
                    rs.close();
                    return;
                }
                ...
                _table_2.addFillRow(new String[] { "Range", Double.toString(max_d - min_d) });
            ...
            rs = Jdbc_conn123.runQuery(all_q);
            while (rs.next()) {
            ...
            _table_2.addFillRow(new String[] { "Avg. Absolute Dev.(AAD)", Double.toString(aad) });
            _table_2.addFillRow(new String[] { "Variance", Double.toString(variance) });
            _table_2.addFillRow(new String[] { "Std. Dev.(SD)", Double.toString(Math.sqrt(variance)) });
            _table_2.addFillRow(new String[] { "Std. Error of Mean(SE)", Double.toString(Math.sqrt(variance) / Math.sqrt(count_d)) });
            _table_2.addFillRow(new String[] { "Skewness", Double.toString(skew / ((count_d - 1) * Math.pow(variance, 1.5))) });
            _table_2.addFillRow(new String[] { "Kurtosis", Double.toString(kurt / ((count_d - 1) * Math.pow(variance, 2))) });
            _table_3.addFillRow(new String[] { "1", Double.toString(perv_a[0]), Long.toString(perc_a[0]) });
            for (int i = 1; i < 20; i++) {
                _table_3.addFillRow(new String[] { Integer.toString(i * 5), Double.toString(perv_a[i]), Long.toString(perc_a[i]) });
            }
            _table_3.addFillRow(new String[] { "99", Double.toString(perv_a[20]), Long.toString(perc_a[20]) });
            _table_2.addRow();
            _table_2.addFillRow(new String[] { "Mid Range", Double.toString((max_d + min_d) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(1%-99%)", Double.toString((perv_a[0] + perv_a[20]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(5%-95%)", Double.toString((perv_a[1] + perv_a[19]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(10%-90%)", Double.toString((perv_a[2] + perv_a[18]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(15%-85%)", Double.toString((perv_a[3] + perv_a[17]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(20%-80%)", Double.toString((perv_a[4] + perv_a[16]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(25%-75%)", Double.toString((perv_a[5] + perv_a[15]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(30%-70%)", Double.toString((perv_a[6] + perv_a[14]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(35%-65%)", Double.toString((perv_a[7] + perv_a[13]) / 2) });
            _table_2.addFillRow(new String[] { "Mid Range(40%-60%)", Double.toString((perv_a[8] + perv_a[12]) / 2) });
        } catch (Exception sql_e) {
            ConsoleFrame.addText("\n ERROR: Advance Query execution failed");
            JOptionPane.showMessageDialog(null, sql_e.getMessage(), "Error Message", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                _table_1.addFillRow(new String[] { col_name, col_count, Double.toString((col_count_d / count_d) * 100) });
            }
            rs.close();
            Jdbc_conn123.closeConn();
        } catch (Exception sql_e) {
    ...
    private void createAndShowGUI() {
        ...
        JLabel time_l = new JLabel("Profile Time: " + new Date(System.currentTimeMillis()).toString());
        ...
        open_m.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_MASK));
        file_m.add(open_m);
