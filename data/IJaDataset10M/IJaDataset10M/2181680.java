...
public class ELANWaveHeader {
    ...
    /** Holds value of property DOCUMENT ME! */
    public static final String[] formatDescriptions = { "Unknown", "PCM (uncompressed)", "MS ADPCM", "IEEE float", "", "IBM CVSD", "8-bit ITU-T G.711 A-law", "8-bit ITU-T G.711 µ-law" };
    ...
    public String toString() {
        StringBuffer output = new StringBuffer("File size: " + fileSize + " Bytes");
        try {
    ...
    private static ELANWaveCuePoint[] getCuePoints(RandomAccessFile soundFile) throws IOException {
        byte[] b = new byte[4];
        soundFile.read(b);
        soundFile.read(b);
        ...
        for (int i = 0; i < cuePoints.length; i++) {
            ...
            if ("data".equals(dataChunkID)) {
                cuePoints[i] = new ELANWaveCuePoint(ID, position, chunkStart, blockStart, sampleOffset);
            } else {
    ...
    private static ELANWaveCueSection getCueSection(RandomAccessFile soundFile, ELANWaveCuePoint[] cuePoints) throws IOException {
        byte[] b = new byte[4];
        byte[] s = new byte[2];
        byte[] t;
        ...
        for (int i = 0; i < cuePoints.length; i++) {
            if (cuePoints[i].getID() == cuePointID) {
                cueSection = new ELANWaveCueSection(cuePoints[i], sampleLength, purposeID, country, language, dialect, codePage, label);
                break;
            }
        }
    ...
    private static String getInfo(RandomAccessFile soundFile) throws IOException {
        ...
        soundFile.seek(soundFile.getFilePointer() + (soundFile.getFilePointer() % 2));
        return info;
    }
        ...
        try {
            while (soundFile.getFilePointer() < soundFile.length()) {
                ...
                if ("ltxt".equals(getString(b))) {
                    ELANWaveCueSection cueSection = getCueSection(soundFile, cuePoints);
                    if (cueSection != null) {
    ...
    private static void readCuePointLabels(RandomAccessFile soundFile, ELANWaveCuePoint[] cuePoints) throws IOException {
        byte[] b = new byte[4];
        byte[] t;
        String label;
        long seek = soundFile.getFilePointer();
    ...
    private static void readCuePointNotes(RandomAccessFile soundFile, ELANWaveCuePoint[] cuePoints) throws IOException {
        byte[] b = new byte[4];
        byte[] t;
        String label;
        long seek = soundFile.getFilePointer();
        ...
        try {
            ...
            while (soundFile.getFilePointer() < soundFile.length()) {
                ...
                if ("cue ".equals(getString(b))) {
                    ...
                    long endOfChunk = (soundFile.getFilePointer() + listChunkSize) - 4;
