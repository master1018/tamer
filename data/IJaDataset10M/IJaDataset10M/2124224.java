...
public class ResultMatrixPlainText extends ResultMatrix {
   ...
   */
    public String toStringHeader() {
        ...
        for (i = 0; i < data.length; i++) data[i][0] = padString(data[i][0], size);
        for (i = 0; i < data.length; i++) result += data[i][0] + " " + data[i][1] + "\n";
        return result;
    }
   ...
   */
    public String toStringMatrix() {
        ...
        for (i = 1; i < cells.length - 1; i++) {
            ...
            for (n = 0; n < cells[0].length; n++) {
                ...
                if (n == 0) {
                    ...
                    if (!isAverage(i)) line += padString("(" + Utils.doubleToString(getCount(getDisplayRow(i - 1)), 0) + ")", getCountWidth(), true); else line += padString("", getCountWidth(), true);
                } else {
                    if (isMean(n)) line += "  ";
                    if (getShowStdDev()) {
                        if (isMean(n - 1)) {
                            if (!cells[i][n].trim().equals("")) line += "(" + cells[i][n] + ")"; else line += " " + cells[i][n] + " ";
                        } else line += " " + cells[i][n];
                    } else {
                        line += " " + cells[i][n];
                    }
        ...
        for (n = 1; n < cells[0].length; n++) {
            if (isMean(n)) {
                i++;
                if (i == 0) line = padString(line, startMeans[i] - getCountWidth()); else if (i == 1) line = padString(line, startMeans[i] - " |".length()); else if (i > 1) line = padString(line, startMeans[i]);
                if (i == 1) line += " |";
                line += " " + cells[0][n];
            }
        }
    ...
    public String toStringSummary() {
        ...
        for (i = 0; i < getColCount(); i++) {
            ...
            for (j = 0; j < getColCount(); j++) {
                ...
                result += " ";
                if (j == i) result += Utils.padLeft("-", resultsetLength * 2 + 3); else result += Utils.padLeft("" + m_NonSigWins[i][j] + " (" + m_Wins[i][j] + ")", resultsetLength * 2 + 3);
            }
    ...
    public static void main(String[] args) {
        ...
        for (i = 0; i < matrix.getRowCount(); i++) {
            for (n = 0; n < matrix.getColCount(); n++) {
                ...
                if (i == n) {
                    if (i % 2 == 1) matrix.setSignificance(n, i, SIGNIFICANCE_WIN); else matrix.setSignificance(n, i, SIGNIFICANCE_LOSS);
                }
            }
        }
