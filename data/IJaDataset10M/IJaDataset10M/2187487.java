 ...
 *
 * @author Markus Kilås
 * @version $Id: SignServerAdminGUIApplication.java 2085 2012-01-27 17:26:30Z netmackan $
 */
public class SignServerAdminGUIApplication extends SingleFrameApplication {

    /** Logger for this class. */
    private static Logger LOG = Logger.getLogger(SignServerAdminGUIApplication.class);
    ...
    static {
        ...
        OPTIONS.addOption(OPTION_CONNECTFILE, true, "Configuration file to read WS connection properties from");
        OPTIONS.addOption(OPTION_DEFAULTCONNECTFILE, true, "Default WS connection configuration file");
    }
    ...
    private static boolean isNamingContextAvailable() {
        boolean result;
        try {
            final InitialContext ignored = new InitialContext();
            result = true;
    ...
    public static AdminWS getAdminWS() {
        if (adminWS == null) {
            if (Protocol.WS == protocol) {
                ...
                final ConnectDialog dlg = new ConnectDialog(null, true, connectFile, defaultConnectFile);
                dlg.setVisible(true);
                adminWS = dlg.getWS();
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(null, "Startup failed. Are the application server running?\n" + ex.getMessage(), "SignServer Administration GUI startup", JOptionPane.ERROR_MESSAGE);
                    System.exit(1);
                }
            }
        }
