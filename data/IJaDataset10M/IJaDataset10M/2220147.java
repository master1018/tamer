...
public class QuickTagParser {
    ...
    void appendParsedTag(int index, TagProcessor processor, StringBuffer buf, TagParseHandle handle) {
        ...
        if (tag != null) if (tag.length() > 0) str = handle.ProcessTag(index, tag);
    ...
    public String parse(Reader reader, TagParseHandle handle) throws IOException {
        ...
        if (processor.isEscapeState()) appendParsedTag(tag_begin, processor, result, handle);
        if (pending_char.length() > 0) appendNormalStrings(index - pending_char.length(), handle, result, pending_char);
