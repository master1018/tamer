...
public class MIDD extends Classifier implements OptionHandler, MultiInstanceCapabilitiesHandler, TechnicalInformationHandler {
     ...
     */
        protected double objectiveFunction(double[] x) {
            ...
            for (int i = 0; i < m_Classes.length; i++) {
                ...
                for (int j = 0; j < nI; j++) {
                    double ins = 0.0;
                    for (int k = 0; k < m_Data[i].length; k++) ins += (m_Data[i][k][j] - x[k * 2]) * (m_Data[i][k][j] - x[k * 2]) * x[k * 2 + 1] * x[k * 2 + 1];
                    ins = Math.exp(-ins);
                    ins = 1.0 - ins;
                    if (m_Classes[i] == 1) bag += Math.log(ins); else {
     ...
     */
        protected double[] evaluateGradient(double[] x) {
            ...
            for (int i = 0; i < m_Classes.length; i++) {
                ...
                for (int j = 0; j < nI; j++) {
                    double exp = 0.0;
                    for (int k = 0; k < m_Data[i].length; k++) exp += (m_Data[i][k][j] - x[k * 2]) * (m_Data[i][k][j] - x[k * 2]) * x[k * 2 + 1] * x[k * 2 + 1];
                    exp = Math.exp(-exp);
                    exp = 1.0 - exp;
                    if (m_Classes[i] == 1) denom += Math.log(exp);
                    if (exp <= m_Zero) exp = m_Zero;
                    for (int p = 0; p < m_Data[i].length; p++) {
                        numrt[2 * p] += (1.0 - exp) * 2.0 * (x[2 * p] - m_Data[i][p][j]) * x[p * 2 + 1] * x[p * 2 + 1] / exp;
                        numrt[2 * p + 1] += 2.0 * (1.0 - exp) * (x[2 * p] - m_Data[i][p][j]) * (x[2 * p] - m_Data[i][p][j]) * x[p * 2 + 1] / exp;
                    }
                }
    ...
    public void buildClassifier(Instances train) throws Exception {
        ...
        for (int h = 0; h < nC; h++) {
            ...
            if (m_Classes[h] == 1) {
                ...
                } else if (nI == maxSz) maxSzIdx.addElement(new Integer(h));
            }
        }
        if (m_filterType == FILTER_STANDARDIZE) m_Filter = new Standardize(); else if (m_filterType == FILTER_NORMALIZE) m_Filter = new Normalize(); else m_Filter = null;
        if (m_Filter != null) {
        ...
        for (int s = 0; s < maxSzIdx.size(); s++) {
            ...
            for (int p = 0; p < m_Data[exIdx][0].length; p++) {
                ...
                if (nll < bestnll) {
                    ...
                    if (m_Debug) System.out.println("!!!!!!!!!!!!!!!!Smaller NLL found: " + nll);
                }
                if (m_Debug) System.out.println(exIdx + ":  -------------<Converged>--------------");
            }
        }
    }
   ...
   */
    public double[] distributionForInstance(Instance exmp) throws Exception {
        ...
        if (m_Filter != null) ins = Filter.useFilter(ins, m_Filter);
        ...
        for (int i = 0; i < nI; i++) {
            double exp = 0.0;
            for (int r = 0; r < nA; r++) exp += (m_Par[r * 2] - dat[i][r]) * (m_Par[r * 2] - dat[i][r]) * m_Par[r * 2 + 1] * m_Par[r * 2 + 1];
            exp = Math.exp(-exp);
