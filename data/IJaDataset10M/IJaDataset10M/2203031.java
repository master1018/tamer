...
public class _2007WheresRaeThisTime extends QuestHandler {
    ...
    public void register() {
        ...
        for (int id : talkNpcs) qe.setNpcQuestData(id).addOnTalkEvent(questId);
    }
        ...
        if (qs.getStatus() == QuestStatus.START) {
            switch(targetId) {
                ...
                case 203554:
                    switch(env.getDialogId()) {
                        case 26:
                            if (var == 4) return sendQuestDialog(env, 2375); else if (var == 8) return sendQuestDialog(env, 2716);
                        case 10004:
                            if (var == 4) {
                                qs.setQuestVar(5);
                                updateQuestStatus(env);
