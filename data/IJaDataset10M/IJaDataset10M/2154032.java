 ...
 * <p>
 * 
 * Does not support heterogeneous constraint alternatives. As a result, callers
 * can be assured that the choice of possible principals or of Integrity.YES is
 * independent of the suites, insuring that those items can be picked before
...
final class ConnectionContext extends Utilities {
    ...
    /**
	 * Whether the connection is being considered on the client side, which does
	 * not support relative time constraints.
	 */
	 ...
	 * and constraints. Null values for the principals mean they are anonymous.
	 * Non-X.500 principals are permitted to allow specifying a dummy principal
	 * if the principal is unknown. Returns null if the constraints are not
	 * supported.
	 */
	 ...
	 * not supported, INTEGRITY if the constraint is Integrity.YES or constraint
	 * alternatives with elements of type Integrity, the connection time if the
	 * constraint is an instance of ConnectionAbsoluteTime or constraint
	 * alternatives of them, and otherwise OK.
	 */
    private long supported(InvocationConstraint constraint) {
        if (constraint instanceof ConstraintAlternatives) {
            ...
            return integrity && constraint == Integrity.YES ? INTEGRITY : NOT_SUPPORTED;
        } else if (constraint instanceof Confidentiality) {
            return ok(doesEncryption(cipherSuite) == (constraint == Confidentiality.YES));
        } else if (constraint instanceof ConfidentialityStrength) {
            return ok(!doesEncryption(cipherSuite) || (hasStrongCipherAlgorithm(cipherSuite) == (constraint == ConfidentialityStrength.STRONG)));
        } else if (constraint instanceof ClientAuthentication) {
            return ok((client == null) == (constraint == ClientAuthentication.NO));
        } else if (constraint instanceof ClientMinPrincipalType) {
            return ok(client == null || constraint.equals(clientMinPrincipalType));
        } else if (constraint instanceof ClientMaxPrincipalType) {
            return ok(client == null || ((ClientMaxPrincipalType) constraint).elements().contains(X500Principal.class));
            ...
            return ok(client == null || ((ClientMaxPrincipal) constraint).elements().contains(client));
            ...
            return ok((server == null) == (constraint == ServerAuthentication.NO));
	 ...
	 * NOT_SUPPORTED if the elements have different types or none are supported,
	 * INTEGRITY if the elements are instances of Integrity, the largest
	 * connection time if the elements are instances of ConnectionAbsoluteTime,
	 * and otherwise OK.
	 */
