...
public class StandardPluginClassLoader extends PluginClassLoader {
    ...
    private Class<?> loadLocalClass(final String name, final boolean resolve, final StandardPluginClassLoader requestor) {
        ...
        synchronized (stickySynchronizing ? requestor : this) {
            ...
            if (result != null) {
                if (debugEnabled) {
                    log.debug("loadLocalClass: found loaded class, class=" + result + ", this=" + this + ", requestor=" + requestor);
                }
                return result;
            }
            try {
                ...
                if (debugEnabled) {
                    log.debug("loadLocalClass: class loading failed," + " name=" + name + ", this=" + this + ", requestor=" + requestor, le);
                }
                throw le;
            } catch (ClassNotFoundException cnfe) {
            }
            if (result != null) {
                if (debugEnabled) {
                    log.debug("loadLocalClass: found class, class=" + result + ", this=" + this + ", requestor=" + requestor);
                }
                if (resolve) {
                    resolveClass(result);
                }
    ...
    private Class<?> loadPluginClass(final String name, final boolean resolve, final boolean tryLocal, final StandardPluginClassLoader requestor, final Set<String> seenPlugins) throws ClassNotFoundException {
        ...
        if ((descr != null) && !seen.contains(descr.getId())) {
            if (debugEnabled) {
                log.debug("loadPluginClass: trying plug-in guess, name=" + name + ", this=" + this + ", requestor=" + requestor);
            }
            ...
            if (result != null) {
                if (debugEnabled) {
                    log.debug("loadPluginClass: plug-in guess succeeds, name=" + name + ", this=" + this + ", requestor=" + requestor);
                }
                return result;
            }
        }
        ...
        if (debugEnabled) {
            log.debug("loadPluginClass: local class not found, name=" + name + ", this=" + this + ", requestor=" + requestor);
        }
    ...
    protected void checkClassVisibility(final Class<?> cls, final StandardPluginClassLoader requestor) throws ClassNotFoundException {
        ...
        if (loader != this) {
            ...
            if (filter == null) {
                log.warn("class not visible, no class filter found, lib=" + lib + ", class=" + cls + ", this=" + this + ", requestor=" + requestor);
            ...
            if (!filter.isClassVisible(cls.getName())) {
                log.warn("class not visible, lib=" + lib + ", class=" + cls + ", this=" + this + ", requestor=" + requestor);
    ...
    protected String findLibrary(final String name) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("findLibrary(String): name=" + name + ", libname=" + libname + ", result=" + result + ", this=" + this);
        }
        return result;
    }
    ...
    protected URL findResource(final String name, final StandardPluginClassLoader requestor, final Set<String> seenPlugins) {
        ...
        if (result != null) {
            if (log.isDebugEnabled()) {
                log.debug("findResource(...): resource found in classpath, name=" + name + " URL=" + result + ", this=" + this + ", requestor=" + requestor);
            }
        ...
        if (resourceLoader != null) {
            ...
            if (result != null) {
                if (log.isDebugEnabled()) {
                    log.debug("findResource(...): resource found in libraries, name=" + name + ", URL=" + result + ", this=" + this + ", requestor=" + requestor);
                }
                if (isResourceVisible(name, result, requestor)) {
                    return result;
                }
        ...
        if (log.isDebugEnabled()) {
            log.debug("findResource(...): resource not found, name=" + name + ", this=" + this + ", requestor=" + requestor);
        }
        seen.add(getPluginDescriptor().getId());
        for (PluginDescriptor element : publicImports) {
            if (seen.contains(element.getId())) {
    ...
    protected boolean isResourceVisible(final String name, final URL url, final StandardPluginClassLoader requestor) {
        ...
        ResourceFilter filter = resourceFilters.get(lib.toExternalForm());
        if (filter == null) {
            log.warn("no resource filter found for library " + lib + ", name=" + name + ", URL=" + url + ", this=" + this + ", requestor=" + requestor);
            return false;
        }
        if (!filter.isResourceVisible(name)) {
            log.warn("resource not visible, name=" + name + ", URL=" + url + ", this=" + this + ", requestor=" + requestor);
            return false;
        }
        return true;
    }
