...
public class BalloonController extends MouseAdapter implements SelectListener {
     ...
     */
    protected long retrievalTimeout = 30 * 1000;
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        try {
            if (this.isBalloonTrigger(e)) {
                ...
                if (sameObjectSelected && balloonVisible) return;
    ...
    protected boolean isResizeControl(PickedObject po) {
        return po != null && AVKey.RESIZE.equals(po.getStringValue(AVKey.ACTION)) && po.getObject() instanceof Balloon;
    }
    ...
    protected KMLAbstractFeature getContext(PickedObject pickedObject) {
        ...
        if (context instanceof KMLAbstractFeature) return (KMLAbstractFeature) context; else return null;
    }
    ...
    protected void onBalloonAction(AbstractBrowserBalloon browserBalloon, String action) {
        if (AVKey.CLOSE.equals(action)) {
            if (browserBalloon == this.balloon) this.hideBalloon(); else browserBalloon.setVisible(false);
        } else if (AVKey.BACK.equals(action)) browserBalloon.goBack(); else if (AVKey.FORWARD.equals(action)) browserBalloon.goForward();
    }
    ...
    protected void createResizeController(Balloon balloon) {
        if (this.resizeController != null) return;
    ...
    protected void destroyResizeController(SelectEvent event) {
        if (this.resizeController != null) {
            try {
                if (event != null) this.resizeController.selected(event);
    ...
    protected void onLinkActivated(SelectEvent event, String url) {
        ...
        if (kmlFeature != null) contextDoc = kmlFeature.getRoot();
        if (isKmlUrl) {
            targetDoc = this.findOpenKmlDocument(linkBase);
            if (targetDoc == null) {
                if (this.isLinkActivationTrigger(event)) this.requestDocument(linkBase, contextDoc, linkRef);
                event.consume();
                return;
            }
        } else {
        ...
        if (!isKmlUrl && !foundLocalFeature) {
            ...
            if ("_blank".equalsIgnoreCase(target)) {
                if (this.isLinkActivationTrigger(event)) this.openInNewBrowser(event, url);
                event.consume();
            }
        }
    }
    ...
    protected boolean onFeatureLinkActivated(KMLRoot doc, String linkFragment, SelectEvent event) {
        ...
        if (!WWUtil.isEmpty(featureId) && doc != null) {
            ...
            if (o instanceof KMLAbstractFeature) {
                if (event == null || this.isLinkActivationTrigger(event)) this.doFeatureLinkActivated((KMLAbstractFeature) o, directive);
                return true;
            }
        }
        return false;
    ...
    protected boolean isKmlUrl(String url, String contentType) {
        if (WWUtil.isEmpty(url)) return false;
        String suffix = WWIO.getSuffix(url);
        return "kml".equalsIgnoreCase(suffix) || "kmz".equalsIgnoreCase(suffix) || KMLConstants.KML_MIME_TYPE.equals(contentType) || KMLConstants.KMZ_MIME_TYPE.equals(contentType);
    }
    ...
    public void showBalloon(KMLAbstractFeature feature) {
        ...
        if (balloon == null && this.canShowBalloon(feature)) balloon = this.createBalloon(feature);
        if (balloon != null) {
    ...
    public boolean canShowBalloon(KMLAbstractFeature feature) {
        ...
        return (hasBalloonStyle || !WWUtil.isEmpty(feature.getDescription()) || feature.getExtendedData() != null) && !isBalloonHidden;
    }
    ...
    protected Balloon getBalloon(PickedObject pickedObject) {
        ...
        if (pickedObject.hasKey(AVKey.CONTEXT)) {
            ...
            if (contextObj instanceof KMLAbstractFeature) {
                ...
                if (balloonObj == null && this.canShowBalloon(feature)) balloonObj = this.createBalloon(feature);
            }
        }
        ...
        if (balloonObj instanceof Balloon) return (Balloon) balloonObj; else return null;
    }
    ...
    protected Balloon createBalloon(KMLAbstractFeature feature) {
        KMLBalloonStyle balloonStyle = (KMLBalloonStyle) feature.getSubStyle(new KMLBalloonStyle(null), KMLConstants.NORMAL);
        ...
        if (AVKey.GLOBE.equals(this.getAttachmentMode(feature))) {
            ...
            if (this.isUseBrowserBalloon()) balloon = new GlobeBrowserBalloon(text, Position.ZERO); else balloon = new GlobeAnnotationBalloon(text, Position.ZERO);
            kmlBalloon = new KMLGlobeBalloonImpl(balloon, feature);
        } else {
            ScreenBalloon balloon;
            if (this.isUseBrowserBalloon()) balloon = new ScreenBrowserBalloon(text, new Point(0, 0)); else balloon = new ScreenAnnotationBalloon(text, new Point(0, 0));
    ...
    protected void configureBalloon(Balloon balloon, KMLAbstractFeature feature) {
        if (feature instanceof KMLAbstractContainer || feature instanceof KMLNetworkLink) {
            ...
            Size maxSize = new Size(Size.EXPLICIT_DIMENSION, 0.8, AVKey.FRACTION, Size.EXPLICIT_DIMENSION, 0.8, AVKey.FRACTION);
            attrs.setSize(size);
            ...
            BalloonAttributes attrs = new BasicBalloonAttributes();
            Size size = new Size(Size.NATIVE_DIMENSION, 0.0, null, Size.NATIVE_DIMENSION, 0.0, null);
            Size maxSize = new Size(Size.EXPLICIT_DIMENSION, 0.5, AVKey.FRACTION, Size.EXPLICIT_DIMENSION, 0.4, AVKey.FRACTION);
            attrs.setSize(size);
            attrs.setMaximumSize(maxSize);
            balloon.setAttributes(attrs);
        }
    ...
    protected String getAttachmentMode(KMLAbstractFeature feature) {
        if (feature instanceof KMLPlacemark || feature instanceof KMLGroundOverlay) return AVKey.GLOBE; else return AVKey.SCREEN;
    }
    ...
    protected void showBalloon(Balloon balloon, Object balloonObject, Point point) {
        if (balloon instanceof ScreenBalloon) ((ScreenBalloon) balloon).setScreenLocation(point); else if (balloon instanceof GlobeBalloon) {
        ...
        if (this.mustAdjustPosition(balloon)) this.adjustPosition(balloon, point);
    ...
    protected void showBalloon(Balloon balloon, Position position) {
        Vec4 screenVec4 = this.wwd.getView().project(this.wwd.getModel().getGlobe().computePointFromPosition(position));
        Point screenPoint = new Point((int) screenVec4.x, (int) (this.wwd.getView().getViewport().height - screenVec4.y));
        ...
        if (this.mustAdjustPosition(balloon)) this.adjustPosition(balloon, screenPoint);
        this.balloon = balloon;
        this.balloon.setVisible(true);
    }
    ...
    protected void adjustPosition(Balloon balloon, Point screenPoint) {
        ...
        if (highlightAttributes != null) highlightAttributes.setOffset(offset);
    }
    ...
    protected Position computeIntersection(AbstractShape shape, Point screenPoint) {
        try {
            ...
            if (intersections != null && !intersections.isEmpty()) return intersections.get(0).getIntersectionPosition();
    ...
    protected Point getBalloonPointForScreenOverlay(KMLScreenOverlay overlay) {
        ...
        Offset offset = new Offset(xy.getX(), xy.getY(), KMLUtil.kmlUnitsToWWUnits(xy.getXunits()), KMLUtil.kmlUnitsToWWUnits(xy.getYunits()));
        Rectangle viewport = this.wwd.getView().getViewport();
    ...
    protected KMLRoot findOpenKmlDocument(String url) {
        ...
        if (o instanceof KMLRoot) return (KMLRoot) o; else return null;
    }
    ...
    protected void requestDocument(String url, KMLRoot context, String featureRef) {
        ...
        docLoader.scheduleAtFixedRate(new DocumentRetrievalTask(url, context, featureRef, this.retrievalTimeout), 0, this.retrievalPollInterval);
    }
    ...
    protected void onDocumentLoaded(String url, KMLRoot document, String featureRef) {
        ...
        if (featureRef != null) this.onFeatureLinkActivated(document, featureRef, null);
    }
    ...
    protected class DocumentRetrievalTask extends TimerTask {
        ...
        public void run() {
            ...
            try {
                ...
                if (System.currentTimeMillis() > this.start + this.timeout) throw new WWTimeoutException(Logging.getMessage("generic.CannotOpenFile", this.docUrl));
                Object docSource;
                if (this.context != null) docSource = this.context.resolveReference(this.docUrl); else docSource = WorldWind.getDataFileStore().requestFile(this.docUrl);
