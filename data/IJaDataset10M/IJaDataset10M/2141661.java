...
public class ConverterOptions {
    ...
    private static File optionsFile = new File(Config.getUserSettingsPath(), "kawaiiriver.conf");
    ...
    public static String getOptionsText() {
        ...
        if (input != null) {
            try {
                String line;
                while ((line = input.readLine()) != null) text += line + "\n";
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public static void writeOption(String option, String setting) {
        try {
            ...
            while ((line = input.readLine()) != null) if (line.indexOf(option + "=") > -1) {
                options.append(option + "=" + setting + "\n");
                setOption = true;
            } else options.append(line + "\n");
            if (!setOption) options.append(option + "=" + setting + "\n");
            input.close();
    ...
    public static void createConfDirectory() {
        if (!optionsFile.exists()) {
            ...
            try {
                ...
                throw new RuntimeException("Could not create conf file " + optionsFile);
            }
        }
        if (!optionsFile.getParentFile().isDirectory()) {
            throw new RuntimeException("Could not create dir: " + optionsFile.getParentFile());
        } else {
            Logger.logMessage("Created " + optionsFile, Logger.INFO);
        }
    }
}
