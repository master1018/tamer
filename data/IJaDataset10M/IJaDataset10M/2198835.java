...
public class Spell_ImprovedPolymorph extends Spell {
    ...
    public void affectEnvStats(Environmental affected, EnvStats affectableStats) {
        ...
        if (newRace != null) {
            if (affected.name().indexOf(" ") > 0) affectableStats.setName("a " + newRace.name() + " called " + affected.name()); else affectableStats.setName(affected.name() + " the " + newRace.name());
    ...
    public void affectCharStats(MOB affected, CharStats affectableStats) {
        ...
        if (newRace != null) {
            ...
            if (affected.baseCharStats().getStat(CharStats.STAT_AGE) > 0) affectableStats.setStat(CharStats.STAT_AGE, newRace.getAgingChart()[oldCat]);
        }
    }
    ...
    public void unInvoke() {
        ...
        super.unInvoke();
        if (canBeUninvoked()) if ((mob.location() != null) && (!mob.amDead())) mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL, "<S-NAME> morph(s) back to <S-HIS-HER> normal form.");
    }

    public int castingQuality(MOB mob, Environmental target) {
        if (mob != null) {
            if (target instanceof MOB) {
                if ((mob.getVictim() == target) && (target.fetchEffect(ID()) == null)) return Ability.QUALITY_MALICIOUS;
            }
        }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        if ((R == null) && (!auto)) {
            if (mob.isMonster()) R = CMClass.randomRace(); else {
                mob.tell("You can't turn " + target.name() + " into a '" + race + "'!");
                return false;
            }
        } else if (R == null) R = CMClass.randomRace();
        ...
        fakeMOB.recoverMaxState();
        int fakeStatTotal = 0;
        for (int s : CharStats.CODES.BASE()) fakeStatTotal += fakeMOB.charStats().getStat(s);
        ...
        } else return beneficialWordsFizzle(mob, target, "<S-NAME> form(s) an improved spell around <T-NAMESELF>, but the spell fizzles.");
        return success;
    }
}
