 ...
 *
 */
public class OperationSetBasicInstantMessagingZeroconfImpl implements OperationSetBasicInstantMessaging {

    private static final Logger logger = Logger.getLogger(OperationSetBasicInstantMessagingZeroconfImpl.class);
     ...
     */
    public OperationSetBasicInstantMessagingZeroconfImpl(ProtocolProviderServiceZeroconfImpl provider, OperationSetPersistentPresenceZeroconfImpl opSetPersPresence) {
    ...
    public Message createMessage(byte[] content, String contentType, String contentEncoding, String subject) {
        return new MessageZeroconfImpl(new String(content), contentEncoding, contentType, MessageZeroconfImpl.MESSAGE);
    }
    ...
    public Message createMessage(String messageText) {
        return new MessageZeroconfImpl(messageText, DEFAULT_MIME_ENCODING, DEFAULT_MIME_TYPE, MessageZeroconfImpl.MESSAGE);
    }
    ...
    public void sendInstantMessage(Contact to, Message message) throws IllegalStateException, IllegalArgumentException {
        if (!(to instanceof ContactZeroconfImpl)) throw new IllegalArgumentException("The specified contact is not a Zeroconf contact." + to);
        MessageZeroconfImpl msg = (MessageZeroconfImpl) createMessage(message.getContent());
        MessageDeliveredEvent msgDeliveredEvt = new MessageDeliveredEvent(msg, to, new Date());
    ...
    private void deliverMessage(Message message, ContactZeroconfImpl to) {
        ...
        try {
            if (thread == null) {
                Socket sock;
                logger.debug("ZEROCONF: Creating a chat connexion to " + to.getIpAddress() + ":" + to.getPort());
    ...
    public void fireMessageReceived(Message message, Contact from) {
        MessageReceivedEvent evt = new MessageReceivedEvent(message, from, new Date());
        Iterator listeners = null;
        synchronized (messageListeners) {
            listeners = new ArrayList(messageListeners).iterator();
        }
