...
public class XMappingResolverDefaultImpl implements IXMappingResolver, ApplicationComponent {
   ...
   */
    @NotNull
    public String resolveTargetText(@NotNull XmlTag xmlTag, @NotNull IXMappingExp mappingExp) throws XMappingException {
   ...
   */
    ...
    private String internalResolveSingleExp(XmlTag xmlTag, String rawExp) throws XMappingException {
    ...
    private String internalResolveSingleExpPart(XmlTag xmlTag, String mappingExp) throws XMappingException {
        int startNode = mappingExp.indexOf('/');
        int startAtt = mappingExp.indexOf('@');
        if ((startAtt > -1) && (startAtt < startNode)) {
            throw new XMappingException("Bad expression, attribute may only be declared on last node : " + mappingExp);
        }
        if (startNode == -1) {
            if (startAtt == 0) {
            ...
            if (startAtt > 0) {
                throw new XMappingException("Bad expression, attribute should be declared as @att : " + mappingExp);
            }
        }
