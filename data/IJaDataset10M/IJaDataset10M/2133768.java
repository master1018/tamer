...
public class SensorSimulatorView extends JPanel {
    ...
    public SensorSimulatorView(SensorSimulatorModel model) {
        ...
        leftPaneScroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        leftPaneScroll.setPreferredSize(new Dimension((int) (Global.W_FRAME * Global.SENSOR_SPLIT_LEFT), Global.H_TABS + 2 * Global.H_BUTTONS + Global.H_CONTENT));
        mSensorsTabbedPane = fillRightPanel();
        JScrollPane tabbedScroll = new JScrollPane(mSensorsTabbedPane);
        tabbedScroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        tabbedScroll.setPreferredSize(new Dimension(750, 2 * Global.H_BUTTONS + Global.H_CONTENT));
        JSplitPane splitUpPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftPaneScroll, tabbedScroll);
    ...
    private JScrollPane fillSensorOutputArea() {
        ...
        scrollPaneSensorData.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        scrollPaneSensorData.setPreferredSize(new Dimension((int) (Global.W_FRAME * Global.SENSOR_SPLIT_LEFT), (int) (1.6 * Global.H_INFO)));
    ...
    private JScrollPane fillInfoOutput() {
        ...
        infoText.append("Write emulator command port and click on " + "set to create connection. Possible IP addresses:");
        try {
            ...
            for (NetworkInterface netint : Collections.list(nets)) {
                ...
                for (InetAddress inetAddress : Collections.list(inetAddresses)) {
                    ...
                    if (address.compareTo("/127.0.0.1") != 0 && !address.contains(":")) {
        ...
        areaScrollPane.setPreferredSize(new Dimension((int) (Global.W_FRAME * Global.SENSOR_SPLIT_LEFT - 50), Global.H_INFO));
    ...
    private JPanel fillSettingsPanel() {
        ...
        label = new JLabel("Distance in time between two consecutive states", SwingConstants.LEFT);
        layout.gridy++;
        settingsPanel.add(label, layout);
        label = new JLabel("(for an accurate state transition description)", SwingConstants.LEFT);
        layout.gridy++;
        settingsPanel.add(label, layout);
        layout.gridwidth = 1;
        layout.gridy++;
        ...
        layout.gridwidth = 3;
        settingsPanel.add(new JSeparator(), layout);
        label = new JLabel("Distance in time between playback states", SwingConstants.LEFT);
        layout.gridy++;
        settingsPanel.add(label, layout);
        label = new JLabel("(used in interpolation, for an accurate playback)", SwingConstants.LEFT);
        layout.gridy++;
        settingsPanel.add(label, layout);
        layout.gridwidth = 1;
        layout.gridy++;
    ...
    private JTabbedPane fillRightPanel() {
        ...
        scenarioScroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        scenarioScroll.setPreferredSize(new Dimension((int) (Global.W_FRAME * Global.SENSOR_SPLIT_RIGHT), 2 * Global.H_BUTTONS + Global.H_CONTENT));
        ...
        quickSettingsScroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        quickSettingsScroll.setPreferredSize(new Dimension((int) (Global.W_FRAME * Global.SENSOR_SPLIT_RIGHT), 2 * Global.H_BUTTONS + Global.H_CONTENT));
    ...
    private JPanel fillSpecificPerSensorPanel() {
        ...
        mSensorsButtonsPanel.setLayout(new BoxLayout(mSensorsButtonsPanel, BoxLayout.X_AXIS));
        JScrollPane sensorsBarScroll = new JScrollPane(mSensorsButtonsPanel);
        sensorsBarScroll.setPreferredSize(new Dimension((int) (Global.W_FRAME * Global.SENSOR_SPLIT_RIGHT), Global.H_BUTTONS * 2));
        ...
        for (final SensorView sensor : mSensors) {
            ...
            final SensorButton sensorButton = new SensorButton(sensorModel.getName(), sensor);
    ...
    public LinearAccelerationView getLinearAceleration() {
        return (LinearAccelerationView) mSensors.get(SensorModel.POZ_LINEAR_ACCELERATION);
    }
