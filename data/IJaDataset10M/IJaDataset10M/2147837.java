...
public final class LegacyDatabaseConnectionFactory implements DataSource {
    ...
    static final class CachedConnection implements Connection {
        ...
        public synchronized void close() throws SQLException {
            ...
            if (!m_inUse) log.warn("The in use flag was not set but the connection is being closed!", new Throwable());
            try {
                if (m_delegate.getAutoCommit() == false) m_delegate.setAutoCommit(true);
                if (m_delegate.isReadOnly()) {
                    if (isTracing) log.debug("connection is read-only, setting bad flag");
                    m_hadError = true;
                }
                if (m_delegate.isClosed()) {
                    if (isTracing) log.debug("connection is closed, setting bad flag");
                    m_hadError = true;
                }
            } catch (SQLException ex) {
                if (isTracing) log.debug("setting bad flag true", ex);
                m_hadError = true;
            }
            m_inUse = false;
            if (m_hadError) {
                ...
                synchronized (m_owner.m_dbcCache) {
                    if (isTracing) log.debug("adding connection back into pool [id=" + this + "]");
        ...
        public synchronized boolean isClosed() throws SQLException {
            try {
                if (m_inUse) return m_delegate.isClosed(); else return true;
            } catch (SQLException ex) {
                m_hadError = true;
    ...
    LegacyDatabaseConnectionFactory(String configFile) throws IOException, MarshalException, ValidationException, ClassNotFoundException {
        ...
        try {
            ...
            for (int i = 0; i < parms.length; i++) {
                if (parms[i].getName().equals("user")) m_driverUser = parms[i].getValue(); else if (parms[i].getName().equals("password")) m_driverPass = parms[i].getValue();
            }
    ...
    public static synchronized DataSource getInstance() {
        if (!m_loaded) throw new IllegalStateException("The factory has not been initialized");
        return m_singleton;
    }
    ...
    public Connection getConnection() throws SQLException {
        synchronized (m_dbcCache) {
            ...
            while (cdbc == null && !m_dbcCache.isEmpty()) {
                ...
                synchronized (cdbc) {
                    if (cdbc.isAvailable() && cdbc.isBad()) {
                        if (isTracing) log.debug("removed bad connection from pool [id=" + cdbc + "]");
                        continue;
                    } else if (cdbc.isAvailable()) {
                        cdbc.markUsed();
                        if (isTracing) log.debug("reusing previous connection [id=" + cdbc + "]");
                    }
                }
                if (!m_dbcCache.isEmpty()) {
                    ...
                    if (age >= MAX_AGE) {
                        ...
                        if (isTracing) log.debug("removing expired connection [id=" + disconnect + "]");
                        try {
                            disconnect.m_delegate.close();
                        } catch (SQLException e) {
                            if (isTracing) log.debug("An error occured closing delegate", e);
                        }
                    } else if (isTracing) {
            ...
            if (cdbc == null) {
                ...
                cdbc.markUsed();
                if (isTracing) log.debug("created new JDBC connection, no previous reference available");
            }
            return cdbc;
        }
    }
