...
public class SoarCommandLineClient {
    ...
    public void setSoarHome(String soarHome) {
        if (soarHome == null) throw new IllegalArgumentException("Must provide path to Soar home.");
    ...
    public SoarCommandLineMXBean openDebuggerProxy(String agent, String host, int port) throws IOException {
        ...
        while (tryCount < 20 && (!managed || isProcessRunning())) {
            try {
                ...
                return (SoarCommandLineMXBean) MBeanServerInvocationHandler.newProxyInstance(conn, objectName, SoarCommandLineMXBean.class, false);
    ...
    public boolean spawnDebuggerWithJMX(String host, int port) throws IOException {
        File debugger = new File(soarHome.getAbsolutePath() + File.separator + "bin" + File.separator + "SoarJavaDebugger.jar");
        ...
        if (isMacOSX()) cmd.add("-XstartOnFirstThread");
        cmd.add("-jar");
        ...
        if (envMap.containsKey("PATH")) temp += File.pathSeparator + envMap.get("PATH");
        envMap.put("PATH", temp);
        temp = soarHome.getAbsolutePath() + File.separator + "lib";
        if (envMap.containsKey("LD_LIBRARY_PATH")) temp += File.pathSeparator + envMap.get("LD_LIBRARY_PATH");
        envMap.put("LD_LIBRARY_PATH", temp);
        temp = soarHome.getAbsolutePath() + File.separator + "lib";
        if (envMap.containsKey("DYLD_LIBRARY_PATH")) temp += File.pathSeparator + envMap.get("DYLD_LIBRARY_PATH");
    ...
    public SoarCommandLineMXBean startDebuggerGetProxy() throws IOException {
        ...
        if (!spawnDebuggerWithJMX(host, port)) return null;
        managed = true;
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (proxy != null) proxy.executeCommandLine("p s1");
    }
}
