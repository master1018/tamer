...
public class PlotterPanel extends JPanel implements MouseListener, ActionListener, Serializable {

    private class MyEllipse extends java.awt.geom.Ellipse2D.Float implements Serializable {
    ...
    public void setxValues(double ad[]) {
        ...
        for (int i = 0; i < ad.length; i++) xValues[i] = ad[i];
    }

    public void setxValues(Object aobj[]) {
        xValues = new double[aobj.length];
        scaledXValues = new int[aobj.length];
        for (int i = 0; i < aobj.length; i++) xValues[i] = ((Number) aobj[i]).doubleValue();
    }
    ...
    public void setXLabel(Object aobj[]) {
        ...
        for (int i = 0; i < aobj.length; i++) XLabel[i] = aobj[i].toString();
    }
    ...
    public Paint getPaint(int i) {
        switch(i) {
            case 0:
                return new GradientPaint(0.0F, 0.0F, new Color(255, 0, 0), 0.0F, getHeight() / 2, new Color(255, 200, 200), true);
            case 1:
                return new GradientPaint(0.0F, 0.0F, new Color(0, 255, 0), 0.0F, getHeight() / 2, new Color(200, 255, 200), true);
            case 2:
                return new GradientPaint(0.0F, 0.0F, new Color(255, 255, 0), 0.0F, getHeight() / 2, new Color(255, 255, 200), true);
            case 3:
                return new GradientPaint(0.0F, 0.0F, new Color(0, 0, 255), 0.0F, getHeight() / 2, new Color(200, 200, 255), true);
        }
        return Color.black;
    }
    ...
    protected void showDataLabels(Graphics g) {
        ...
        for (int i1 = 0; i1 < k; i1++) {
            ...
            if (xValues[i1] < 0.0D) g.drawString(" N/A", j1, j - 25); else g.drawString(String.valueOf(Math.round(xValues[i1])), (j1 + l / 4) - (String.valueOf(Math.round(xValues[i1])).length() * 4), k1);
        }
    }
    ...
    protected void showXLabels(Graphics g) {
        ...
        for (int l1 = 0; l1 < j1; l1++) {
            int l = byte0 + l1 * k1;
            if (fm.stringWidth(XLabel[l1]) < (k1 - 10)) g.drawString(XLabel[l1], l + 10, i1); else {
    ...
    public void scaleValuesUniformly() {
        ...
        scale = d2 * zoomFactor;
        for (int j = 0; j < scaledXValues.length; j++) scaledXValues[j] = scale == 0.0D ? (int) (25D + xValues[j]) : (int) (25D + xValues[j] / scale);
    }
    ...
    public void scaleBubble(Vector vector) {
        ...
        double d1 = ((Double) vector.elementAt(vector.size() - 1)).doubleValue();
        double d2 = 0.0D;
        d2 = d1 - d == 0.0D ? 1.0D : (d1 - d) / (double) (getWidth() - 75);
        scale = d2;
    }
    ...
    private void drawSlideBubble(double d, double d1, Graphics g) {
        ...
        for (int j1 = lo_i; j1 <= hi_i; j1++) {
            double d3 = (double) (byte0 + 10) + (((Double) vc.elementAt(j1)).doubleValue() - d) / scale;
            float f1;
            if (d3 > d2 + 6D) {
                f1 = j - (byte0 + 6);
                l = 1;
    ...
    private void drawBubble(Vector vector, Graphics g) {
        ...
        for (int i1 = 0; i1 < l; i1++) {
            double d3 = (double) (byte0 + 10) + (((Double) vector.elementAt(i1)).doubleValue() - d) / scale;
            float f1;
            if (d3 > d1 + 6D) {
                f1 = j - (byte0 + 6);
                k = 1;
    ...
    public static int max(int ai[]) {
        if (ai == null || ai.length == 0) return 0;
        int i = ai[0];
        for (int j = 0; j < ai.length; j++) if (ai[j] > i) i = ai[j];
        return i;
    }

    public static double max(double ad[]) {
        if (ad == null || ad.length == 0) return 0.0D;
        double d = ad[0];
        for (int i = 0; i < ad.length; i++) if (ad[i] > d) d = ad[i];
        return d;
    }
    ...
    public String getToolTipText(MouseEvent mouseevent) {
        ...
        if (XLabel.length == 0) return null;
        for (int j1 = 0; j1 < XLabel.length; j1++) d += xValues[j1];
        if (d == 0.0D) return null;
        int k1 = (k - 2 * byte0) / XLabel.length;
        int l1 = 0;
        do {
            if (l1 >= xValues.length) break;
            d1 += xValues[l1];
    ...
    public void mouseClicked(MouseEvent mouseevent) {
        ...
        int l1 = getWidth();
        int i2 = getHeight();
        if (j <= l1 / 2) i1 = l1 / 2 + 50; else i1 = 100;
        Font font = new Font("Helvetika", 0, 12);
        g.setFont(font);
        FontMetrics fontmetrics = getFontMetrics(font);
        String s1 = "Group ID: " + Integer.toString(l + 1);
        String s2 = "Group Avg: " + Double.toString(((Double) vc.elementAt(l)).doubleValue());
