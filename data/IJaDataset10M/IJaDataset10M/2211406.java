...
public class Directory {
    ...
    public Directory(File file) {
        if (!file.isDirectory()) throw new RuntimeException("File is not a directory: " + file.getAbsolutePath());
        current = file;
    }
    ...
    public Directory bd() {
        if (current.getParentFile() != null) current = current.getParentFile();
        return this;
    }
    ...
    public Directory[] getDirs() {
        ...
        int currentPos = 0;
        for (int j = 0; j < i; j++) if (files[j].isDirectory()) dirs[currentPos++] = new Directory(files[j]);
    ...
    public File[] getFiles() {
        ...
        int currentPos = 0;
        for (int j = 0; j < i; j++) if (!files[j].isDirectory()) returnValue[currentPos++] = files[j];
