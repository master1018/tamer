...
public class PathManager {
    ...
    private void loadPathDatabase() throws IOException {
        if (null != handleDatabase) {
            if (handleDatabase.exists() && !handleDatabase.canWrite()) throw new IOException("The handle database must be writable.");
            final File tmp = File.createTempFile("paths", ".db", handleDatabase.getAbsoluteFile().getParentFile());
            if (null == tmp) throw new IOException("Can't create tmp handle database at " + tmp);
            tmp.delete();
            if (handleDatabase.exists()) {
                ...
                final BufferedReader br = new BufferedReader(new FileReader(handleDatabase));
                while (true) {
                    ...
                    try {
                        ...
                        if (!(path.exists() || path.isHidden())) {
                            if (logger.isInfoEnabled()) logger.info("Not loading nonexistent path " + path);
                            continue;
                        }
                        ...
                        for (final NFSExport export : exports) {
                            ...
                            if (path.getAbsolutePath().startsWith(exportRoot) && exportRoot.length() > bestLength) {
                                bestMatch = export;
                                bestLength = exportRoot.length();
                            }
                        }
                        if (null == bestMatch) {
                            if (logger.isInfoEnabled()) logger.info("Path seems to be no longer exported: " + path);
                            continue;
                        }
                ...
                for (final Enumeration<NFSFile> i = handlesToFiles.elements(); i.hasMoreElements(); ) {
                    ...
                    if (null == parentHandle && !file.getFile().equals(file.getExport().getRoot())) {
                        logger.warn("Parent for file " + file.getFile() + " not found in handle database.");
                        filesToRemove.add(file);
                    } else if (null == parentHandle) {
                    } else {
                        final NFSFile parentFile = (NFSFile) handlesToFiles.get(handleToInt(parentHandle));
                        if (null == parentFile) {
                            logger.warn("Parent file for handle not found. Should not happen!");
                            filesToRemove.add(file);
                        } else file.setParentDirectory(parentFile);
                    }
                }
    ...
    public synchronized void flushPathDatabase() throws IOException {
        if (null != handleDatabase && isChanged) {
            if (handleDatabase.exists() && !handleDatabase.canWrite()) throw new IOException("The handle database must be writable.");
            final File tmp = File.createTempFile("paths", ".db", handleDatabase.getParentFile());
            if (null == tmp) throw new IOException("Can't create tmp handle database at " + tmp);
    ...
    private char intToChar(int i) {
        if (i >= 0 && i <= 9) return (char) (i + '0'); else if (i >= 0xa && i <= 0xf) return (char) (i + 'a' - 0xa); else throw new IllegalArgumentException();
    }
    ...
    private static int charToInt(char c) {
        if (c >= '0' && c <= '9') return c - '0'; else if (c >= 'a' && c <= 'f') return c - 'a' + 0xa; else if (c >= 'A' && c <= 'F') return c - 'A' + 0xa; else throw new NumberFormatException("Illegal hex character " + c);
    }
    ...
    public NFSFile getNFSFileByHandle(nfs_fh fh) throws StaleHandleException {
        ...
        if (null == nfsFile) throw new StaleHandleException();
        if (!nfsFile.validateHandle(fh)) throw new StaleHandleException("Handle was defined, but wrong generation");
    ...
    private nfs_fh createHandleForFile(File f, NFSExport export) throws StaleHandleException {
        ...
        if (null == export) {
            ...
            if (null == parentHandle) throw new StaleHandleException(f + " doesn't have a parent handle");
            final int id = getIDFromHandle(parentHandle);
            final NFSFile parent = (NFSFile) handlesToFiles.get(id);
            if (null == parent) throw new StaleHandleException("Not NFS file for parent handle for " + f);
            handlesToFiles.put(h, new NFSFile(fh, f, parent, parent.getExport()));
        } else handlesToFiles.put(h, new NFSFile(fh, f, null, export));
        isChanged = true;
        return fh;
    }
    ...
    public nfs_fh getHandleForExport(NFSExport e) throws StaleHandleException {
        File root = e.getRoot();
        if (!root.isAbsolute()) root = root.getAbsoluteFile();
    ...
    public synchronized void movePath(File from, File to) {
        ...
        final LinkedHashMap<File, File> recursiveMoveMap = getMoveMap(from, to, moveMap);
    ...
    private synchronized LinkedHashMap<File, File> getMoveMap(File from, File to, LinkedHashMap<File, File> sortedMoveMap) {
        if (!from.isAbsolute()) from = from.getAbsoluteFile();
        if (!to.isAbsolute()) to = to.getAbsoluteFile();
        ...
        if (to.isDirectory()) for (final Iterator<File> i = filesToHandles.keySet().iterator(); i.hasNext(); ) {
            final File f = i.next();
    ...
    private synchronized void moveMapEntries(File from, File to) {
        ...
        if (null != fhFrom) {
            ...
            final NFSFile nfsFileFrom = (NFSFile) handlesToFiles.get(handleToInt(fhFrom));
            final File parentFileTo = to.getParentFile();
            nfs_fh parentHandleTo = filesToHandles.get(parentFileTo);
            if (null == parentHandleTo) try {
            ...
            if (null != parentHandleTo) {
                ...
                if (null != parentTo) {
                    final NFSFile nfsFileTo = new NFSFile(nfsFileFrom.getHandle(), to, parentTo, parentTo.getExport());
                    filesToHandles.put(to, fhFrom);
                    handlesToFiles.put(handleToInt(fhFrom), nfsFileTo);
                } else logger.warn("missing handle->file map for parentHandleTo: " + parentHandleTo);
            } else logger.warn("missing file->handle map for parentFileTo: " + parentFileTo);
            isChanged = true;
        } else logger.warn("missing file->handle map for from: " + from);
    }
    ...
    public boolean createMissigHandles(File f) {
        ...
        if (handleForFileExists(f)) return true;
        File firstParent = f;
        ...
        try {
            final NFSExport export = getNFSFileByHandle(getHandleByFile(firstParent)).getExport();
            for (final Iterator i = filesToAdd.iterator(); i.hasNext(); ) {
                ...
                new NFSFile(getHandleByFile(fileToAdd), fileToAdd, getNFSFileByHandle(fileToAddParentNfsHandle), export);
            }
            return true;
        } catch (final Exception e) {
            e.printStackTrace();
    ...
    public boolean removeFileFromNFS(Collection<File> filesList) {
        for (File file : filesList) {
            flushFile(file);
            if (file.delete()) {
                if (handleForFileExists(file)) try {
