...
public class BarcodeEAN128 extends Barcode128 {
    ...
    static {
        ...
        for (int i = 3100; i < 3700; i++) ids.put("" + i, new IdentifierEAN("" + i, "n2+n...8", -1, true));
        ...
        for (int k = 0; k < 95; ++k) codesB.put(k + ' ', k);
        for (int k = 95; k < 196; ++k) codesB.put(k - 95 + 'Ã', k);
    }
    ...
    public Rectangle getBarcodeSize() {
        ...
        if (font != null) {
            ...
            if (codeType == CODE128_RAW) {
                ...
                if (idx < 0) fullCode = ""; else fullCode = code.substring(idx + 1);
            }
            fontX = font.getWidthPoint(fullCode, size);
        }
        if (codeType == CODE128_RAW) {
            int idx = code.indexOf('￿');
            if (idx >= 0) fullCode = code.substring(0, idx); else fullCode = code;
        } else fullCode = (char) START_C + FNC1 + code;
