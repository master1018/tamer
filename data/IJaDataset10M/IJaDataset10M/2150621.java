...
public class DoorwayGuardian extends StdBehavior {
    ...
    public void setParms(String parms) {
        ...
        if (V.size() > 0) mask = CMParms.combineWithQuotes(V, 0);
    }
    ...
    public Exit[] getParmExits(MOB monster) {
        ...
        if (dirs != null) for (Enumeration<Integer> dirE = dirs.elements(); dirE.hasMoreElements(); ) {
    ...
    public boolean okMessage(Environmental oking, CMMsg msg) {
        ...
        } else if (!canFreelyBehaveNormal(oking)) return true;
        MOB monster = (MOB) oking;
        if ((mob.location() == monster.location()) && (mob != monster) && (msg.target() != null) && (!BrotherHelper.isBrother(mob, monster, false)) && (CMLib.flags().canSenseMoving(mob, monster) || nosneak)) {
            if (msg.target() instanceof Exit) {
                Exit exit = (Exit) msg.target();
                Exit texit[] = getParmExits(monster);
                if ((texit != null) && (texit[0] != exit) && (texit[1] != exit)) return true;
                if ((msg.targetMinor() != CMMsg.TYP_CLOSE) && (msg.targetMinor() != CMMsg.TYP_LOCK) && ((mask == null) || CMLib.masking().maskCheck(mask, mob, false))) {
            ...
            } else if ((msg.tool() != null) && (msg.target() instanceof Room) && (msg.tool() instanceof Exit) && ((mask == null) || CMLib.masking().maskCheck(mask, mob, false))) {
