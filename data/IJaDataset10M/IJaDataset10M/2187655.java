...
/**
 * This class is an enhanced version of the JUnit
 * junit.textui.TestRunner class to be run inside a
 * servlet/JSP with an output writer.  Here is the remainder
 * of the class definition from JUnit:
...
public class JUnitTestRunner extends BaseTestRunner {
    ...
    PrintWriter fWriter = new PrintWriter(System.out);
	 ...
	 */
    public JUnitTestRunner(PrintWriter writer) {
        this();
    ...
    public static void main(String args[]) {
        JUnitTestRunner aTestRunner = new JUnitTestRunner();
        try {
    ...
    public void printErrors(TestResult result) {
        if (result.errorCount() != 0) {
            if (result.errorCount() == 1) writer().println("There was " + result.errorCount() + " error:"); else writer().println("There were " + result.errorCount() + " errors:");
            int i = 1;
    ...
    public void printFailures(TestResult result) {
        if (result.failureCount() != 0) {
            if (result.failureCount() == 1) writer().println("There was " + result.failureCount() + " failure:"); else writer().println("There were " + result.failureCount() + " failures:");
            int i = 1;
    ...
    public TestResult start(String args[]) throws Exception {
        ...
        try {
            AppLog.info("Trying to retrieve and run JUnit test '" + testCase + "'");
