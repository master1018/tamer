...
public class GAResultModule extends AbstractTable implements Module {
    ...
    private class Model extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int column) {
            ...
            if (column == START_DATE_COLUMN) return result.getStartDate(); else if (column == END_DATE_COLUMN) return result.getEndDate(); else if (column == SYMBOLS_COLUMN) return result.getSymbols(); else if (column == BUY_RULE_COLUMN) return result.getBuyRule(); else if (column == SELL_RULE_COLUMN) return result.getSellRule(); else if (column == GENERATION_COLUMN) return new Integer(result.getGeneration()); else if (column == TRADE_COST_COLUMN) return result.getTradeCost(); else if (column == NUMBER_OF_TRADES_COLUMN) return new Integer(result.getNumberTrades()); else if (column == FINAL_CAPITAL_COLUMN) return result.getFinalCapital(); else if (column == INITIAL_CAPITAL_COLUMN) return result.getInitialCapital(); else if (column == PERCENT_RETURN_COLUMN) return new ChangeFormat(result.getInitialCapital(), result.getFinalCapital()); else {
                assert false;
                return "";
            }
        }
    ...
    private List createColumns() {
        ...
        columns.add(new Column(START_DATE_COLUMN, Locale.getString("START_DATE"), Locale.getString("START_DATE_COLUMN_HEADER"), TradingDate.class, Column.HIDDEN));
        columns.add(new Column(END_DATE_COLUMN, Locale.getString("END_DATE"), Locale.getString("END_DATE_COLUMN_HEADER"), TradingDate.class, Column.HIDDEN));
        ...
        columns.add(new Column(GENERATION_COLUMN, Locale.getString("GENERATION_NUMBER"), Locale.getString("GENERATION_NUMBER_COLUMN_HEADER"), Integer.class, Column.VISIBLE));
        columns.add(new Column(INITIAL_CAPITAL_COLUMN, Locale.getString("INITIAL_CAPITAL"), Locale.getString("INITIAL_CAPITAL_COLUMN_HEADER"), Money.class, Column.HIDDEN));
