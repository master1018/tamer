...
public class SimpleXMLPrettyPrinter extends DefaultHandler {
    ...
    private void wrapUpCharacters() throws IOException {
        if (lastSawCharacters) {
            pp.end();
    ...
    public void printAttributes(Attributes atts) throws IOException {
        if (atts.getLength() > 0) {
            pp.print(" ").beginC(0);
            for (int i = 0; i < atts.getLength(); i++) {
                pp.print(atts.getLocalName(i) + "=" + XMLUtils.quoteAttrValue(atts.getValue(i)));
    ...
    public void process(String urlString) throws Exception {
