...
public class DistributeShardsThread extends Thread {
    ...
    protected static final Logger LOG = Logger.getLogger(DistributeShardsThread.class);
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    while (!(_statusUpdate.hasChanges(liveIndexes, liveNodes) || !unbalancedIndexes.isEmpty()) || _statusUpdate.getNodes().isEmpty()) {
    ...
    private void runInSafeMode(Set<String> liveNodes, Set<String> liveIndexes) throws InterruptedException {
        _safeMode = true;
        try {
            while (_statusUpdate.lastChangeTimeStamp() + _safeModeMaxTime > System.currentTimeMillis() || _statusUpdate.getNodes().isEmpty() || areNodesConnecting(_statusUpdate.getNodes())) {
    ...
    private Set<String> getUnderreplicatedIndexes(int nodeCount) throws KattaException {
        ...
        for (final String index : _zkClient.getChildren(_conf.getZKIndicesPath())) {
            ...
            if (indexMetaData.getState() != IndexState.ERROR && indexMetaData.getState() != IndexState.DEPLOYING && indexMetaData.getState() != IndexState.REPLICATING) {
                ...
                if (minimalReplicationCount < desiredReplicationCount) {
                    if (minimalReplicationCount >= nodeCount) {
                        LOG.warn("found index '" + index + "' underreplicated but skip replicating since not enough nodes connected");
                    } else {
    ...
    private boolean isOverReplicated(final String indexZkPath, final IndexMetaData indexMetaData) throws KattaException {
        final List<String> shards = _zkClient.getChildren(indexZkPath);
        final Map<String, List<String>> currentShard2NodesMap = readShard2NodesMapFromZk(_zkClient, new HashSet<String>(shards));
        for (final String shard : shards) {
    ...
    private void distributeIndexes(final Set<String> affectedIndexes, final IndexState state, Set<String> liveNodes) throws KattaException {
    ...
    private void distributeIndexShards(final String index, final IndexMetaData indexMD, final Set<String> indexShards, final Map<String, AssignedShard> shard2AssignedShardMap, Collection<String> liveNodes) throws KattaException {
        ...
        final Map<String, List<String>> distributionMap = _deployPolicy.createDistributionPlan(currentShard2NodesMap, currentNodeToShardsMap, new ArrayList<String>(liveNodes), indexMD.getReplicationLevel());
    ...
    private static Map<String, List<String>> readShard2NodesMapFromZk(final ZKClient zkClient, final Set<String> indexShards) throws KattaException {
    ...
    private void writeShardDistributionMapToZK(final Map<String, List<String>> distributionMap, final Map<String, AssignedShard> shard2AssignedShardMap) throws KattaException {
    ...
    private Map<String, AssignedShard> readShardsFromFs(final String index, final IndexMetaData indexMetaData) throws IndexInvalidException {
        final String indexPathString = indexMetaData.getPath();
        URI uri;
        try {
            uri = new URI(indexPathString);
        } catch (final URISyntaxException e) {
            throw new IndexInvalidException("unable to parse index path uri '" + indexPathString + "', make sure it starts with file:// or hdfs:// ", e);
        }
        FileSystem fileSystem;
        try {
            fileSystem = FileSystem.get(uri, new Configuration());
        } catch (final IOException e) {
            throw new IndexInvalidException("unable to retrive file system for index path '" + indexPathString + "', make sure your path starts with hadoop support prefix like file:// or hdfs://", e);
        }
    ...
    protected class IndexStateListener implements IZkChildListener {
        ...
        public IndexStateListener(final String index, final IndexMetaData indexMetaData, final Set<String> shards, final int nodeCount) {
            _index = index;
        ...
        private void switchIndexState(final IndexState indexState) throws KattaException {
            ...
            LOG.info("switching index '" + _index + "' from state " + _indexMetaData.getState() + " into state " + indexState);
