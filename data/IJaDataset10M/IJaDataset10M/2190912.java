...
public abstract class ConfigurableTree extends JTree {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (pathForLocation != null) {
                    ...
                    if (!(selectedNode instanceof ConfigurableNode)) return;
    ...
    public void setFilter(String filterText) {
        ...
        if (filterText == null) filteredConfigClasses.addAll(allConfigClasses); else {
            for (Class<? extends Configurable> aClass : allConfigClasses) {
                if (doPassFilter(filterText.toLowerCase(), aClass)) filteredConfigClasses.add(aClass);
            }
        }
        rebuildTree();
    }
    ...
    private boolean doPassFilter(String filterText, Class aClass) {
        ...
        if (aClass.getName().toLowerCase().contains(filterText)) return true;
        if (doPassFilter(filterText, aClass.getSuperclass())) return true;
        for (Class aInterface : aClass.getInterfaces()) {
            if (doPassFilter(filterText, aInterface)) return true; else if (doPassFilter(filterText, aInterface.getSuperclass())) return true;
        }
        return false;
    }
...
class TransferableConfigurable implements Transferable {
    ...
    public Object getTransferData(DataFlavor flavor) throws UnsupportedFlavorException, IOException {
        if (isDataFlavorSupported(flavor)) return confClass; else return null;
    }
}
