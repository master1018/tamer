...
public class JLScheduler extends Scheduler {
    ...
    public Goal Serialized(Job job) {
        ...
        if (compiler.serializeClassInfo()) {
            return new VisitorGoal("Serialized", job, new ClassSerializer(ts, nf, job.source().lastModified(), compiler.errorQueue(), extInfo.version())).intern(this);
        } else {
    ...
    @Override
    public Goal LookupGlobalTypeDefAndSetFlags(LazyRef<ClassDef> sym, QName className, Flags flags) {
    ...
    protected static class LookupGlobalTypeDefAndSetFlags extends TypeObjectGoal_c<ClassDef> {
        ...
        public String toString() {
            if (flags == null) return name + "(" + className + ")"; else return name + "(" + className + ", " + flags + ")";
        }
