...
class EOP05C04FilesLoader implements DataFileLoader {
     ...
     */
    private static final Pattern LINE_PATTERN = Pattern.compile("^\\d+ +\\d+ +\\d+ +\\d+(?: +-?\\d+\\.\\d+){12}$");
    ...
    public void loadData(final InputStream input, final String name) throws IOException, OrekitException {
        ...
        for (String line = reader.readLine(); line != null; line = reader.readLine()) {
            ...
            if (!(inHeader || parsed)) {
                throw new OrekitException("unable to parse line {0} in IERS data file {1}", new Object[] { Integer.valueOf(lineNumber), name });
            }
        }
        if (inHeader) {
            throw new OrekitException("file {0} is not a supported IERS data file", new Object[] { name });
        }
    }
