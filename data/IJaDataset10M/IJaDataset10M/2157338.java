...
public class ResolveDependencies extends JDialog implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == cancel) {
            ...
            for (PluginObject plugin : automatic) plugin.setFirstValidAction(new Action[] { Action.INSTALL, Action.UPDATE, Action.UNINSTALL });
            dispose();
        }
    }
    ...
    public boolean resolve() {
        listIssues();
        if (panel.getDocument().getLength() > 0) show();
        return !wasCanceled;
    }
    ...
    void listIssues() {
        ...
        if (forUpload) listUploadIssues(); else listUpdateIssues();
        enableOKIfValid();
        if (isShowing()) {
            if (panel.getStyledDocument().getLength() == 0) addText("No more issues to be resolved!", italic);
    ...
    void listUpdateIssues() {
        ...
        for (PluginObject plugin : toInstall.keySet()) if (toUninstall.get(plugin) != null) bothInstallAndUninstall(plugin); else if (!plugin.willBeUpToDate()) {
            if (plugin.isLocallyModified()) locallyModified(plugin); else automatic.add(plugin);
        }
        for (PluginObject plugin : toUninstall.keySet()) if (toInstall.get(plugin) != null && !plugin.willNotBeInstalled()) needUninstall(plugin);
        if (automatic.size() > 0) {
    ...
    void bothInstallAndUninstall(PluginObject plugin) {
        PluginCollection reasons = PluginCollection.clone(toInstall.get(plugin));
    ...
    void locallyModified(PluginObject plugin) {
        ...
        addButton((toInstall ? "Install" : "Update") + " " + plugin, plugin, toInstall ? Action.INSTALL : Action.UPDATE);
    }

    void listUploadIssues() {
        toInstall = new PluginCollection.DependencyMap();
        for (PluginObject plugin : plugins.toUpload()) for (Dependency dependency : plugin.getDependencies()) {
            PluginObject dep = plugins.getPlugin(dependency.filename);
            if (dep == null || ignore.contains(dep)) continue;
            if (dep.isInstallable() || !dep.isFiji() || dep.isObsolete() || (dep.getStatus().isValid(Action.UPLOAD) && dep.getAction() != Action.UPLOAD)) toInstall.add(dep, plugin);
        }
        for (PluginObject plugin : toInstall.keySet()) needUpload(plugin);
    }
    ...
    void needUpload(final PluginObject plugin) {
        ...
        addText(plugin.getFilename(), bold);
        addText(" is " + (notFiji ? "not a Fiji component yet" : (notInstalled ? "not installed" : (obsolete ? "marked obsolete" : "locally modified"))) + " but a dependency of\n\n");
        addList(reasons);
        addText("\n    ");
        if (!notFiji && !obsolete) {
        ...
        if (!notInstalled) {
            addButton("Upload " + plugin + (obsolete ? " again" : ""), plugin, Action.UPLOAD);
            addText("    ");
        }
        addButton("Break the dependency", new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                for (PluginObject other : reasons) other.removeDependency(plugin.getFilename());
                listIssues();
            }
        });
    }
    ...
    void addButton(String label, final Collection<PluginObject> plugins, final Action action) {
        addButton(label, new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                for (PluginObject plugin : plugins) if (action == null) plugin.setNoAction(); else plugin.setAction(action);
                listIssues();
            }
        });
    }
