...
public class CompilationUnitDeclaration extends ASTNode implements ProblemSeverities, ReferenceContext {
    ...
    private void cleanUp(TypeDeclaration type) {
        ...
        if (type.binding != null && type.binding.isAnnotationType()) this.compilationResult.hasAnnotations = true;
        ...
        if (!hasErrors) {
            ...
            if (severity != ProblemSeverities.Ignore) {
                ...
                for (int iSuppress = 0, suppressCount = this.suppressWarningsCount; iSuppress < suppressCount; iSuppress++) {
                    ...
                    if (unusedWarningTokenIsWarning && irritants.areAllSet()) continue;
                    if (irritants != foundIrritants[iSuppress]) {
                                    ...
                                    if (inits != null) {
                                        for (int iToken = 0, tokenCount = inits.length; iToken < tokenCount; iToken++) {
                                            ...
                                            if (cst != Constant.NotAConstant && cst.typeID() == TypeIds.T_JavaLangString) {
                                                ...
                                                if (tokenIrritants != null && !tokenIrritants.areAllSet() && options.isAnyEnabled(tokenIrritants) && (foundIrritants[iSuppress] == null || !foundIrritants[iSuppress].isAnySet(tokenIrritants))) {
                                                    if (unusedWarningTokenIsWarning) {
                                                        ...
                                                        nextSuppress: for (int jSuppress = iSuppress - 1; jSuppress >= 0; jSuppress--) {
                                                            ...
                                                            if (start < startSuppress) continue nextSuppress;
                                                            if (end > endSuppress) continue nextSuppress;
                                                            if (this.suppressWarningIrritants[jSuppress].areAllSet()) break pairLoop;
                                                        }
                                                    }
 ...
 */
    ...
    public char[] getMainTypeName() {
        if (this.compilationResult.compilationUnit == null) {
            ...
            if (start == 0 || start < CharOperation.lastIndexOf('\\', fileName)) start = CharOperation.lastIndexOf('\\', fileName) + 1;
    ...
    public StringBuffer print(int indent, StringBuffer output) {
        ...
        if (this.imports != null) for (int i = 0; i < this.imports.length; i++) {
