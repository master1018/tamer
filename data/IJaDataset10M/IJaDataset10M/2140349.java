...
public class NIODispatcher implements Runnable {
    ...
    public void addThrottle(final NBThrottle t) {
        if (Thread.currentThread() == dispatchThread) THROTTLE.add(t); else {
        ...
        try {
            ...
            if (sk != null && sk.isValid()) {
                ...
                synchronized (sk.attachment()) {
                    ...
                    if (on) sk.interestOps(oldOps | op); else sk.interestOps(oldOps & ~op);
                }
                if (on && (oldOps & op) != op) wakeup();
            }
    ...
    private Selector getSelectorFor(SelectableChannel channel) {
        ...
        if (sel == null) return primarySelector; else return sel;
    }
    ...
    private void processRead(long now, ReadObserver handler, Attachment proxy) throws IOException {
        if (LOG.isDebugEnabled()) LOG.debug("Handling read: " + handler);
    ...
    private void processWrite(long now, WriteObserver handler, Attachment proxy) throws IOException {
        if (LOG.isDebugEnabled()) LOG.debug("Handling write: " + handler);
        handler.handleWrite();
    }
    ...
    private void registerImpl(Selector selector, SelectableChannel channel, int op, IOErrorObserver attachment, int timeout) {
        try {
            ...
            guard.setKey(key);
            if (timeout != 0) guard.addTimeout(System.currentTimeMillis(), timeout); else if ((op & SelectionKey.OP_READ) != 0) guard.changeReadStatus(true);
        } catch (IOException iox) {
    ...
    private void runPendingTasks() {
        ...
        for (NBThrottle t : THROTTLE) t.tick(now);
    }
    ...
    private Collection<SelectionKey> pollOtherSelectors() {
        ...
        if (ret == null) return Collections.emptySet(); else return ret;
    }
    ...
    private void readyThrottles(Collection<SelectionKey> keys) {
        for (int i = 0; i < THROTTLE.size(); i++) THROTTLE.get(i).selectableKeys(keys);
    }
    ...
    private void process() throws ProcessingException, SpinningException {
        ...
        while (true) {
            ...
            try {
                if (!immediate && checkTime) startSelect = System.currentTimeMillis();
                if (!immediate) {
                    ...
                    if (delay == 0) {
                        ...
                        try {
                            if (Thread.interrupted()) LOG.warn("interrupted?");
            ...
            if (!immediate && !wokeup) {
                if (keys.isEmpty()) {
                    ...
                    if (startSelect == -1) {
                        ...
                        if (LOG.isWarnEnabled()) LOG.warn("Spinning detected, current spins: " + zeroes + " startSelect " + startSelect + " now " + now + " keys " + primarySelector.keys());
                        if (zeroes++ > SPIN_AMOUNT) throw new SpinningException();
                    } else {
                        checkTime = false;
                        startSelect = -1;
                        zeroes = 0;
            ...
            if (LOG.isTraceEnabled()) LOG.trace("Selected keys: (" + keys.size() + "), polled: (" + polled.size() + "). wokeup " + wokeup + " immediate " + immediate);
            ...
            long now = System.currentTimeMillis();
            for (SelectionKey sk : allKeys) process(now, sk, sk.attachment(), 0xFFFF);
            keys.clear();
            iteration++;
            TIMEOUTER.processTimeouts(now);
            wokeup = false;
    ...
    private long nextSelectTimeout() {
        ...
        for (Throttle t : THROTTLE) next = Math.min(next, t.nextTickTime());
        long now = System.currentTimeMillis();
        next -= now;
        if (next <= 0) return 0;
        long timeout = TIMEOUTER.getNextExpireTime();
        if (timeout > -1) next = Math.min(next, timeout - now);
        if (next <= 0) return 0;
        Delayed nextScheduled = DELAYED.peek();
        if (nextScheduled != null) next = Math.min(next, nextScheduled.getDelay(TimeUnit.MILLISECONDS));
    ...
    void process(long now, SelectionKey sk, Object proxyAttachment, int allowedOps) {
        ...
        if (proxy.lastMod <= iteration) proxy.handled = 0;
        ...
        if (sk.isValid()) {
            ...
            if (LOG.isErrorEnabled()) LOG.error("SelectionKey cancelled for: " + attachment);
            safeCancel(sk, attachment);
        }
    }
    ...
    public void run() {
        while (true) {
            try {
                if (primarySelector == null) primarySelector = Selector.open();
                process();
    ...
    class Attachment implements Timeoutable {
        ...
        synchronized void changeReadStatus(boolean reading) {
            if (!discarded) {
                if (reading) updateReadTimeout(System.currentTimeMillis()); else clearTimeout();
            }
        }
        ...
        public void notifyTimeout(long now, long expireTime, long timeoutLength) {
            if (!discarded) {
                ...
                synchronized (this) {
                    if (timeoutActive) {
                        if (expireTime == storedExpireTime) {
                            ...
                            storedExpireTime = -1;
                            if (LOG.isWarnEnabled()) LOG.warn("Ignoring extra timeout for: " + attachment);
                        }
                    } else {
                        storedExpireTime = -1;
                ...
                if (cancel) {
                    if (LOG.isDebugEnabled()) LOG.debug("Closing due to read timeout: " + attachment);
