 ...
 */
public class TypeMemberPrinter extends JavaSchemaPrinterModule implements JavaStructuredTypeMemberVisitor {
    ...
    public TypeMemberPrinter(BasePrinter basePrinter, JavaStructuredType<?> type) {
        super(basePrinter);
        this.type = type;
    ...
    public void printMembers(Collection<? extends JavaStructuredTypeMember> members) {
        JavaStructuredTypeMember prev = null;
        for (final JavaStructuredTypeMember member : members) {
            if (prev != null && !(prev instanceof JavaMemberVariable && member instanceof JavaMemberVariable)) {
                printNewLine();
            }
            member.accept(this);
            prev = member;
    ...
    private static final EnumSet<JavaFunctionModifier> INTF_FUNC_EXCL = EnumSet.of(JavaFunctionModifier.ABSTRACT);
    ...
    public void visit(JavaFunction obj) {
        ...
        basePrinter.printModifiers(modifiers, JavaFunctionModifier.class, isIntf ? INTF_FUNC_EXCL : null);
        ...
        if (modifiers.contains(JavaFunctionModifier.ABSTRACT) || modifiers.contains(JavaFunctionModifier.NATIVE)) {
            fmt.printTrailingToken(";");
            printNewLine();
        } else {
            printBody(obj.getBody());
