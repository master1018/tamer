...
public class Environment {
  ...
  **/
    public Object lookup(Symbol var, int up, int in, Object vars) {
        if (this == GLOBAL) return var; else if (vars == var) return new LocalVariable(up, in, var); else if (!U.isPair(vars)) return parent.lookup(var, up + 1, 0, parent.vars); else if (U.first(vars) == var) return new LocalVariable(up, in, var); else return lookup(var, up, in + 1, U.rest(vars));
    }
    ...
    private String toString0() {
        if (this == GLOBAL) return "GLOBAL"; else return U.stringify(vars) + " " + U.stringify(vals) + ((parent == null) ? "" : "|" + parent.toString0());
    }
