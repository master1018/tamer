...
class TableColumnsTab extends JPanel implements PrefsTab {
    ...
    public TableColumnsTab(JabRefPreferences prefs, JabRefFrame frame) {
        ...
        TableModel tm = new AbstractTableModel() {
            ...
            public Class<?> getColumnClass(int column) {
                if (column == 0) return String.class; else return Integer.class;
            }
            ...
            public void setValueAt(Object value, int row, int col) {
                ...
                if (col == 0) {
                    ...
                    if (value == null) rowContent.length = -1; else rowContent.length = Integer.parseInt(value.toString());
                }
            }
        };
    ...
    public void setValues() {
        ...
        for (int i = 0; i < names.length; i++) {
            if (i < lengths.length) tableRows.add(new TableRow(names[i], Integer.parseInt(lengths[i]))); else tableRows.add(new TableRow(names[i]));
        }
    ...
    class AddRowAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (int i = 0; i < rows.length; i++) {
                if (rows[i] + i - 1 < tableRows.size()) tableRows.add(Math.max(0, rows[i] + i - 1), new TableRow(GUIGlobals.DEFAULT_FIELD_LENGTH));
            }
    ...
    abstract class AbstractMoveRowAction extends AbstractAction {
        ...
        protected void swap(int i, int j) {
            if (i < 0 || i >= tableRows.size()) return;
            if (j < 0 || j >= tableRows.size()) return;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (selected.length > 0 && selected[0] > 1) {
                ...
                for (int i = 0; i < colSetup.getRowCount(); i++) {
                    if (!newSelected[i]) colSetup.removeRowSelectionInterval(i, i);
                }
                colSetup.revalidate();
                colSetup.repaint();
                tableChanged = true;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (int i = 1; i < colMod.getColumnCount(); i++) {
                try {
                    ...
                    if ((i <= tableRows.size()) && (((String) colSetup.getValueAt(i, 0)).toLowerCase()).equals(name)) colSetup.setValueAt("" + width, i, 1); else {
    ...
    public void storeSettings() {
        ...
        if (tableChanged) {
            ...
            while (i < tableRows.size()) {
                if (tableRows.elementAt(i).name.equals("")) tableRows.removeElementAt(i); else i++;
            }
