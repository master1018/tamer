...
public class RDN {
    ...
    public void addEscaped(String rdnfragment) throws InvalidNameException {
        ...
        if (equalpos <= 0 || equalpos == rdnfragment.length() - 1) throw new InvalidNameException("RDN.add(): invalid rdn fragment '" + ((rdnfragment == null) ? "<null>" : rdnfragment) + "' (can't find equal sign)");
        if (ldapEscapedRDN.length() > 0) ldapEscapedRDN += "+" + rdnfragment; else ldapEscapedRDN = rdnfragment;
    }
    ...
    public void addRaw(String rdnfragment) throws InvalidNameException {
        int equalpos = NameUtility.next(rdnfragment, 0, '=');
        if (equalpos <= 0 || equalpos == rdnfragment.length() - 1) throw new InvalidNameException("RDN.addRaw(): invalid rdn fragment '" + ((rdnfragment == null) ? "<null>" : rdnfragment) + "' (can't find equal sign)");
     ...
     */
    public void dump() {
        if (status == UNTESTED) checkForMultiValued();
    ...
    public String getElement(int i) {
        if (status == UNTESTED) checkForMultiValued();
        if (status == SINGLEVALUED && i == 0) return ldapEscapedRDN;
        if (i < 0 || elements == null || elements.length <= i + 1) return "error VII";
    ...
    public String[] getElements() {
        if (status == UNTESTED) checkForMultiValued();
        if (status == SINGLEVALUED) return new String[] { ldapEscapedRDN };
        if (elements == null) return new String[] { "error VIIB" };
        String[] elementArray = new String[elements.length - 1];
        for (int i = 0; i < (elements.length - 1); i++) elementArray[i] = ldapEscapedRDN.substring(elements[i] + 1, elements[i + 1]);
        return elementArray;
    }
    ...
    public void setElement(int i, String ldapEscapedElement) throws InvalidNameException {
        ...
        if (status == SINGLEVALUED) {
            if (i == 0) ldapEscapedRDN = ldapEscapedElement; else throw new InvalidNameException("cannot set non zero element of single valued rdn.");
        } else {
            if (i < 0 || i >= size()) throw new InvalidNameException("attempt to set element " + i + " of rdn: '" + ldapEscapedRDN + "' (size = " + size() + ")");
            ldapEscapedRDN = ldapEscapedRDN.substring(0, elements[i] + 1) + ldapEscapedElement + ldapEscapedRDN.substring(elements[i + 1]);
            parseMultiValued();
        }
    }
    ...
    public String getAttID(int i) {
        if (status == UNTESTED) checkForMultiValued();
        if (status == SINGLEVALUED && i != 0) return "rdn error VIII";
    ...
    public boolean contains(String attributeType) {
        if (status == UNTESTED) checkForMultiValued();
        if (attributeType == null || attributeType.length() == 0) return false;
        for (int i = 0; i < size(); i++) if (attributeType.equalsIgnoreCase(getAttID(i))) return true;
        return false;
    }
    ...
    public String getRawVal(String attributeType) {
        if (status == UNTESTED) checkForMultiValued();
        if (attributeType == null || attributeType.length() == 0) return null;
        for (int i = 0; i < size(); i++) if (attributeType.equalsIgnoreCase(getAttID(i))) return getRawVal(i);
        return null;
    }
    ...
    public String getRawVal(int i) {
        if (status == UNTESTED) checkForMultiValued();
        if (status == SINGLEVALUED && i != 0) return "rdn error X";
        String element = getElement(i);
        int pos = element.indexOf('=');
        if (pos == -1) {
            return "rdn error XI";
    ...
    public void setRawVal(String v, int i) throws InvalidNameException {
        ...
        if (att == null || att.length() == 0) throw new InvalidNameException("can't parse old RDN '" + ldapEscapedRDN);
    ...
    */
    public boolean equals(RDN test) {
        if (test == null) return false; else if (test.size() != size()) return false;
        if (isMultiValued()) {
            String[] atts = getAttIDs();
            String[] vals = getRawVals();
            String[] testAtts = test.getAttIDs();
            String[] testVals = test.getRawVals();
            for (int i = 0; i < size(); i++) if (!elementsEqual(atts[i], testAtts[i], vals[i], testVals[i])) return false;
            return true;
        } else {
            return elementsEqual(getAttID(), test.getAttID(), getRawVal(), test.getRawVal());
        }
    }
    ...
    */
    private boolean elementsEqual(String att1, String att2, String val1, String val2) {
        if (att1.equalsIgnoreCase(att2) == false) return false;
        if (val1.equalsIgnoreCase(val2) == false) return false;
        return true;
    }
    ...
    */
    public boolean equals(Object o) {
        ...
        if (o instanceof RDN) return equals((RDN) o); else return (ldapEscapedRDN.equalsIgnoreCase(o.toString()));
    }
    ...
    private void checkForMultiValued() {
        ...
        if (NameUtility.next(ldapEscapedRDN, 0, '+') == -1) {
            status = SINGLEVALUED;
        } else {
            status = MULTIVALUED;
    ...
    private void parseMultiValued(int max) {
        ...
        try {
            ...
            pos1 = temp[numElements - 1] + 1;
            pos2 = temp[numElements];
            if (debug) System.out.println("found " + numElements + " -th element at " + pos + " = string " + pos1 + " -> " + pos2 + " final len: " + ldapEscapedRDN.length());
            if (debug) System.out.println(" = '" + ldapEscapedRDN.substring(pos1, pos2) + "'");
