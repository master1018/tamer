...
public class PrayTimeConfigBean extends PrayTime {
    ...
    public PrayTimeConfigBean(String location, double x, double y, int z, int offset, int s, int j) {
    ...
    public PrayTimeConfigBean(HttpServletRequest request) {
        int s = 0;
        int j = 0;
        if (StringUtils.isNotBlank(request.getQueryString()) || request.getMethod().toUpperCase().equals("POST")) {
                    ...
                    if (cookie.getName().equals("CONFIG")) {
                        try {
                            ...
                            for (String pair : pairs) {
                                ...
                                if (tokens[0].equals("l")) location = tokens[1]; else if (tokens[0].equals("x")) x = new Double(tokens[1]); else if (tokens[0].equals("y")) y = new Double(tokens[1]); else if (tokens[0].equals("z")) z = new Integer(tokens[1]); else if (tokens[0].equals("s")) s = new Integer(tokens[1]); else if (tokens[0].equals("j")) j = new Integer(tokens[1]);
                            }
                            hasCookie = true;
                        } catch (Exception e) {
                            e.printStackTrace();
    ...
    private void construct(String location, double x, double y, int z, int offset, int s, int j) {
    ...
    public String getLocation() {
        if (location != null && (location.equals("null") || location.trim().equals(""))) location = null;
        return location;
    }
    ...
    public String getCalendarFeed() throws Exception {
        ...
        return new StringBuilder("/").append(l).append(".ics?").append("x=").append(getX()).append("&y=").append(getY()).append("&z=").append(getZ()).append("&s=").append(getFajrIshaMethod()).append("&j=").append(getAsrMethod()).toString();
    }
