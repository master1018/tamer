...
public class Storage {
     ...
     * @param saveTo Directory to download [represented by torrent] file to
     * @exception IOException when creating and/or checking files fails.
     */
    public Storage(MetaInfo metainfo, StorageListener listener, String saveTo) throws IOException {
    ...
    private static void addFiles(List<File> l, File f) {
        if (!f.isDirectory()) {
            ...
            if (files == null) {
                log.warn("Skipping '" + f + "' not a normal file.");
                return;
            }
    ...
    public void check() throws IOException {
        File base = new File(saveTo + File.separator + filterName(metainfo.getName()));
        log.debug("base is: " + filterName(metainfo.getName()));
        List files = metainfo.getFiles();
        if (files == null) {
            log.debug("Creating/Checking file: " + base);
            ...
            log.debug("Creating/Checking directory: " + base);
    ...
    private void checkCreateFiles() throws IOException {
        ...
        for (int i = 0; i < rafs.length; i++) {
            ...
            if (length == lengths[i]) {
                ...
                log.debug("Truncating '" + names[i] + "' from " + lengths + " to " + lengths[i] + "bytes");
    ...
    private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(Storage.class.getName());
}
