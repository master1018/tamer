...
public class UpSellingExampleSetGenerator extends AbstractExampleSource {
    ...
    private static int[] VALUE_TYPES = { Ontology.NOMINAL, Ontology.INTEGER, Ontology.NOMINAL, Ontology.INTEGER, Ontology.NOMINAL, Ontology.NOMINAL, Ontology.NOMINAL, Ontology.INTEGER };

    private static String[][] POSSIBLE_VALUES = { null, null, { "healthy", "active", "cozily" }, null, { "married", "single" }, { "practical", "expensive" }, { "soccer", "badminton", "athletics" }, null };
    ...
    public ExampleSet createExampleSet() throws OperatorException {
        ...
        for (int m = 0; m < ATTRIBUTE_NAMES.length; m++) {
            ...
            if (possibleValues != null) {
                for (int v = 0; v < possibleValues.length; v++) current.getMapping().mapString(possibleValues[v]);
            }
        ...
        for (int n = 0; n < numberOfExamples; n++) {
            ...
            if (values[1] > 65) {
                if (random.nextDouble() > 0.05) values[8] = label.getMapping().mapString("product_2");
            } else if (values[1] > 60) {
                if (random.nextDouble() > 0.1) values[8] = label.getMapping().mapString("product_2");
            } else if (values[1] > 55) {
                if (random.nextDouble() > 0.2) values[8] = label.getMapping().mapString("product_2");
            } else if (values[3] < 15000) {
                if (random.nextDouble() > 0.1) values[8] = label.getMapping().mapString("product_3");
