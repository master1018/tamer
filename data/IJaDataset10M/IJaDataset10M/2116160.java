...
public class ComponentPluginManager extends PluginManager {
    ...
    public ComponentPluginManager(String xmlName) {
        super(xmlName, LafComponentPlugin.TAG_MAIN, LafComponentPlugin.COMPONENT_TAG_PLUGIN_CLASS);
    }
    ...
    public void initializeAll() {
        ...
        for (Iterator iterator = availablePlugins.iterator(); iterator.hasNext(); ) {
            Object pluginObject = iterator.next();
            if (pluginObject instanceof LafComponentPlugin) ((LafComponentPlugin) pluginObject).initialize();
        }
    }
    ...
    public void uninitializeAll() {
        ...
        for (Iterator iterator = availablePlugins.iterator(); iterator.hasNext(); ) {
            Object pluginObject = iterator.next();
            if (pluginObject instanceof LafComponentPlugin) ((LafComponentPlugin) pluginObject).uninitialize();
        }
    }
    ...
    public void processAllDefaultsEntries(UIDefaults table, Object themeInfo) {
        ...
        for (Iterator iterator = availablePlugins.iterator(); iterator.hasNext(); ) {
            ...
            if (pluginObject instanceof LafComponentPlugin) {
                Object[] defaults = ((LafComponentPlugin) pluginObject).getDefaults(themeInfo);
                if (defaults != null) {
