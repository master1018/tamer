...
public class AlignedFlowLayout extends AbstractHintLayout {
    ...
    public void layout(IFigure parent) {
        ...
        int wHint = -1;
        int hHint = -1;
        if (isHorizontal()) wHint = parent.getClientArea().width; else hHint = parent.getClientArea().height;
        ...
        while (iterator.hasNext()) {
            ...
            if (fillParent) data.rowHeight = Math.max(data.area.height, r.height);
            data.row[data.rowCount] = f;
            data.bounds[data.rowCount] = r;
            data.rowCount++;
            i++;
        }
        if (data.rowCount != 0) layoutRow(parent);
        data = null;
    }
