...
public abstract class GAString extends GA {
     ...
     * @throws GAException
     */
    public GAString(int chromosomeDim, int populationDim, double crossoverProb, int randomSelectionChance, int maxGenerations, int numPrelimRuns, int maxPrelimGenerations, double mutationProb, int chromDecPts, String possGeneValues, int crossoverType, boolean computeStatistics) throws GAException {
        ...
        if (possGeneValues.length() < 2) throw new GAException("There must be at least 2 possible gene values");
    ...
    protected double chromStrToFloat(String sChromosome, int iNumDecPts) {
        String sFloat;
        int iLen;
        if (iNumDecPts == 0) return (binaryStrToInt(sChromosome)); else {
            iLen = sChromosome.length() - iNumDecPts;
            sFloat = sChromosome.substring(0, iLen) + "." + sChromosome.substring(iLen, iLen + iNumDecPts);
    ...
    protected void initPopulation() {
        for (int i = 0; i < populationDim; i++) {
            for (int iGene = 0; iGene < chromosomeDim; iGene++) ((ChromChars) this.chromosomes[i]).genes[iGene] = getRandomGeneFromPossGenes();
    ...
    protected void doOnePtCrossover(Chromosome Chrom1, Chromosome Chrom2) {
        ...
        sNewChrom1 = sChrom1.substring(0, iCrossoverPoint) + sChrom2.substring(iCrossoverPoint, chromosomeDim);
        sNewChrom2 = sChrom2.substring(0, iCrossoverPoint) + sChrom1.substring(iCrossoverPoint, chromosomeDim);
    ...
    protected void doTwoPtCrossover(Chromosome Chrom1, Chromosome Chrom2) {
        ...
        if (iCrossoverPoint2 == (iCrossoverPoint1 + 1)) doOnePtCrossover(Chrom1, Chrom2); else {
            sChrom1 = Chrom1.getGenesAsStr();
            sChrom2 = Chrom2.getGenesAsStr();
            sNewChrom1 = sChrom1.substring(0, iCrossoverPoint1) + sChrom2.substring(iCrossoverPoint1, iCrossoverPoint2) + sChrom1.substring(iCrossoverPoint2, chromosomeDim);
            sNewChrom2 = sChrom2.substring(0, iCrossoverPoint1) + sChrom1.substring(iCrossoverPoint1, iCrossoverPoint2) + sChrom2.substring(iCrossoverPoint2, chromosomeDim);
            ((ChromChars) Chrom1).setGenesFromStr(sNewChrom1);
            ((ChromChars) Chrom2).setGenesFromStr(sNewChrom2);
        }
    }
