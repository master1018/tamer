...
public class GridOutput extends AbstractOutput {
    ...
    public void paint(Graphics2D g, int preferredWidth) {
        ...
        for (int row = 0; row <= grid.getNumRows(); row++) {
            g.drawLine(0, row * (3 + cellSize.height), grid.getNumColumns() * (3 + cellSize.width), row * (3 + cellSize.height));
        }
        for (int col = 0; col <= grid.getNumColumns(); col++) {
            g.drawLine(col * (3 + cellSize.width), 0, col * (3 + cellSize.width), grid.getNumRows() * (3 + cellSize.height));
        }
        for (int row = 0; row < grid.getNumRows(); row++) {
            for (int col = 0; col < grid.getNumColumns(); col++) {
    ...
    private static Dimension calculateGridImageDimensions(Grid grid, Icon[] icons, Dimension cellSize, int gap, int lineThickness) {
        return new Dimension((grid.getNumColumns() * (cellSize.width + 2 * gap + lineThickness)) + lineThickness, (grid.getNumRows() * (cellSize.height + 2 * gap + lineThickness)) + lineThickness);
    }
    ...
    public static Icon getIcon(String iconName) {
        if (!ICON_MAP.containsKey(iconName)) {
            ...
            if (url == null) url = GridOutput.class.getResource("resources/" + iconName + ".gif");
            if (url == null) ICON_MAP.put(iconName, null); else ICON_MAP.put(iconName, new ImageIcon(url));
        }
