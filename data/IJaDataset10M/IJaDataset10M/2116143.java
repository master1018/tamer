...
public class TreeCompiler {
    ...
    private void compileClass(Node ast) {
        ...
        _classWriter.visit(_java5 ? V1_5 : V1_4, accessFlags, _name, classSig, className(extractClass(extendsT)), extractClassNames(implementsTs));
        ...
        if (isInterface) {
            _classWriter.visitField(ACC_PUBLIC | ACC_STATIC | ACC_FINAL | ACC_SYNTHETIC, ADAPTER_FIELD, EVALUATION_ADAPTER_DESCRIPTOR, null, null).visitEnd();
        } else {
            _classWriter.visitField(ACC_PRIVATE | ACC_STATIC | ACC_FINAL | ACC_SYNTHETIC, ADAPTER_FIELD, EVALUATION_ADAPTER_DESCRIPTOR, null, null).visitEnd();
            _classWriter.visitField(ACC_FINAL | ACC_SYNTHETIC, BINDINGS_FACTORY_FIELD, BINDINGS_FACTORY_DESCRIPTOR, null, null).visitEnd();
        }
    ...
    private void compileAnonymousConstructor(AnonymousAllocation ast) {
        MethodVisitor mv = _classWriter.visitMethod(ACC_PUBLIC, "<init>", "(" + RUNTIME_BINDINGS_DESCRIPTOR + ")V", null, null);
        StackSizeTracker stack = new StackSizeTracker(2);
        mv.visitCode();
        ...
        if (superC.isInterface()) {
            emitDefaultSuperCall(mv, TypeSystem.OBJECT, 1, stack);
        } else {
            stack.mark();
            ...
            mv.visitMethodInsn(INVOKESPECIAL, className(superC), "<init>", paramListDescriptor(extraArg, superTarget.parameters()) + "V");
            stack.reset();
        }
        for (Runnable2<MethodVisitor, StackSizeTracker> initCode : _instanceInits) {
            initCode.run(mv, stack);
    ...
    private void compileAnonymousInnerConstructor(AnonymousInnerAllocation ast) {
        ...
        mv.visitVarInsn(ALOAD, 1);
        stack.adjust(3);
        mv.visitMethodInsn(INVOKEVIRTUAL, EVALUATION_ADAPTER_NAME, "evaluateExpression", EVALUATE_EXPRESSION_DESCRIPTOR);
        stack.adjust(-2);
        DJConstructor superTarget = NodeProperties.getConstructor(ast).declaredSignature();
        List<Expression> superArgs = ast.getArguments();
        if (superArgs != null) {
        ...
        String callDescriptor = paramListDescriptor(typeDescriptor(SymbolUtil.thisType(superOuterC)), superTarget.parameters()) + "V";
    ...
    private void compileConstructor(ConstructorDeclaration ast, Type extendsT) {
        ...
        MethodVisitor mv = _classWriter.visitMethod(access, "<init>", methodDescriptor, methodSig, extractClassNames(exceptions));
        ...
        if (call == null) {
            ...
            if (call.getExpression() == null) {
                ...
                stack.adjust(2);
                mv.visitMethodInsn(INVOKEVIRTUAL, EVALUATION_ADAPTER_NAME, "evaluateConstructorCallArg", EVALUATE_CONSTRUCTOR_CALL_ARG_DESCRIPTOR);
                stack.adjust(-4);
            }
            int i = 0;
            for (Pair<LocalVariable, Expression> arg : IterUtil.zip(callTarget.parameters(), call.getArguments())) {
            ...
            mv.visitMethodInsn(INVOKESPECIAL, callsSuper ? className(extendsC) : _name, "<init>", paramListDescriptor(extraArg, callTarget.parameters()) + "V");
            stack.reset();
        }
        if (callsSuper) {
            for (Runnable2<MethodVisitor, StackSizeTracker> initCode : _instanceInits) {
        ...
        mv.visitVarInsn(ALOAD, boxedParamsVar);
        stack.adjust(1);
        mv.visitMethodInsn(INVOKEVIRTUAL, EVALUATION_ADAPTER_NAME, "evaluateConstructorBody", EVALUATE_CONSTRUCTOR_BODY_DESCRIPTOR);
        stack.adjust(-4);
        mv.visitInsn(RETURN);
        mv.visitMaxs(stack.maxStack(), stack.maxLocals());
        mv.visitEnd();
    ...
    private void compileClassInitializer() {
        ...
        stack.adjust(1);
        mv.visitMethodInsn(INVOKESTATIC, "java/lang/Class", "forName", "(" + STRING_DESCRIPTOR + ")" + CLASS_DESCRIPTOR);
        mv.visitMethodInsn(INVOKEVIRTUAL, "java/lang/Class", "getClassLoader", "()" + CLASS_LOADER_DESCRIPTOR);
        mv.visitTypeInsn(CHECKCAST, TREE_CLASS_LOADER_NAME);
        mv.visitLdcInsn(_treeClass.fullName());
        stack.adjust(1);
        mv.visitMethodInsn(INVOKEVIRTUAL, TREE_CLASS_LOADER_NAME, "getAdapter", "(" + STRING_DESCRIPTOR + ")" + EVALUATION_ADAPTER_DESCRIPTOR);
        stack.adjust(-1);
    ...
    private void compileMethod(MethodDeclaration ast, boolean isInterface) {
        ...
        final MethodVisitor mv = _classWriter.visitMethod(access, ast.getName(), methodDescriptor, methodSig, extractClassNames(exceptions));
        if (!Modifier.isAbstract(access) && !Modifier.isNative(access)) {
            ...
            mv.visitVarInsn(ALOAD, boxedParamsVar);
            stack.adjust(1);
            mv.visitMethodInsn(INVOKEVIRTUAL, EVALUATION_ADAPTER_NAME, "evaluateMethod", EVALUATE_METHOD_DESCRIPTOR);
            stack.adjust(-3);
            stack.mark();
    ...
    private void compileField(final FieldDeclaration ast, boolean isInterface) {
        ...
        if (isStatic && init != null) {
            val = expressionConstantVal(init);
        }
        _classWriter.visitField(access, ast.getName(), typeDescriptor(t), typeSignature(t), val).visitEnd();
    ...
    private void compileInitializerBlock(Initializer ast, final boolean isStatic) {
        ...
        (isStatic ? _staticInits : _instanceInits).add(new Runnable2<MethodVisitor, StackSizeTracker>() {
            ...
            public void run(MethodVisitor mv, StackSizeTracker stack) {
                ...
                mv.visitMethodInsn(INVOKEVIRTUAL, EVALUATION_ADAPTER_NAME, "evaluateInitializer", EVALUATE_INITIALIZER_DESCRIPTOR);
                stack.adjust(-3);
            }
        });
    }
    ...
    private void emitConstant(MethodVisitor mv, Object val, StackSizeTracker stack) {
        if (val instanceof Integer) {
            emitIntConstant(mv, (Integer) val, stack);
        } else {
            mv.visitLdcInsn(val);
   ...
   */
    private void emitBindingsFactoryAssignment(MethodVisitor mv, int bindingsVar, StackSizeTracker stack) {
        ...
        mv.visitVarInsn(ALOAD, bindingsVar);
        stack.adjust(3);
        mv.visitMethodInsn(INVOKEVIRTUAL, EVALUATION_ADAPTER_NAME, "makeBindingsFactory", MAKE_BINDINGS_FACTORY_DESCRIPTOR);
        stack.adjust(-1);
    ...
    private void emitDefaultSuperCall(MethodVisitor mv, Type extendsT, int bindingsVar, StackSizeTracker stack) {
        ...
        if (extendsC.hasRuntimeBindingsParams()) {
            mv.visitVarInsn(ALOAD, bindingsVar);
            stack.adjust(1);
            mv.visitMethodInsn(INVOKESPECIAL, className(extendsC), "<init>", "(" + RUNTIME_BINDINGS_DESCRIPTOR + ")V");
            stack.adjust(-2);
        } else {
   ...
   */
        ...
        t.apply(new TypeAbstractVisitor_void() {
            ...
            private void generateBoxingCode(int loadOp, int size, String className, String boxedDescriptor, String primDescriptor) {
                if (_java5) {
                    ...
                    stack.adjust(size);
                    mv.visitMethodInsn(INVOKESTATIC, className, "valueOf", "(" + primDescriptor + ")" + boxedDescriptor);
                    stack.adjust(-size + 1);
                } else {
    ...
    private String typeDescriptor(Type t) {
        return encodeType(_opt.typeSystem().erase(t));
    }
    ...
    private static String typeParamListSignature(Iterable<? extends TypeParameter> params) {
        ...
        for (TypeParameter p : params) {
            ...
            if (!validIntersection) {
                result.append(':').append(typeSignature(upper));
            }
        }
        result.append('>');
    ...
    private static String paramListSignature(String prefix, Iterable<LocalVariable> params) {
        ...
        for (LocalVariable var : params) {
            result.append(typeSignature(var.type()));
        }
        result.append(')');
        return result.toString();
    }
    ...
    private String paramListDescriptor(String prefix, Iterable<LocalVariable> params) {
        ...
        result.append(prefix);
        for (LocalVariable var : params) {
            result.append(typeDescriptor(var.type()));
        }
        result.append(')');
        return result.toString();
    }
   ...
   */
    private static Object expressionConstantVal(Expression exp) {
        ...
        if (NodeProperties.hasValue(exp)) {
            ...
            if (!(result instanceof Integer || result instanceof Float || result instanceof Long || result instanceof Double || result instanceof String)) {
                result = null;
            }
        }
        return result;
   ...
   */
     ...
     */
        public void mark() {
            _markedStack = _currentStack;
        }

        /** Reset stack size to the previously-marked location. */
        public void reset() {
            _currentStack = _markedStack;
        }
   ...
   */
     ...
     */
        public Object evaluateConstructorCallArg(String key, int index, RuntimeBindings bindings, Object[] args) throws Throwable {
            ...
            Expression exp;
            if (index == -1) {
                exp = decl.getConstructorCall().getExpression();
            } else {
                exp = decl.getConstructorCall().getArguments().get(index);
            }
     ...
     */
        ...
        private Object evaluateExpression(Expression exp, RuntimeBindings bindings) throws Throwable {
            try {
                return new ExpressionEvaluator(bindings, _opt).value(exp);
    ...
    public static class BindingsFactory implements Lambda<Object, RuntimeBindings> {
        ...
        public RuntimeBindings value(Object thisVal) {
            if (_cachedResult == null) {
                _cachedResult = new RuntimeBindings(_bindings, _thisClass, thisVal);
            }
