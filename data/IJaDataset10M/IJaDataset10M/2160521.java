...
public class VelocityResourceLoader extends ResourceLoader {
    ...
    protected static final Logger logger = Logger.getLogger(VelocityResourceLoader.class);
    ...
    public void init(ExtendedProperties configuration) {
        this.resourceLoader = (org.frameworkset.util.io.ResourceLoader) this.rsvc.getApplicationAttribute(RESOURCE_LOADER);
        String resourceLoaderPath = (String) this.rsvc.getApplicationAttribute(RESOURCE_LOADER_PATH);
        if (this.resourceLoader == null) {
            throw new IllegalArgumentException("'resourceLoader' application attribute must be present for BbossResourceLoader");
        }
        if (resourceLoaderPath == null) {
            throw new IllegalArgumentException("'resourceLoaderPath' application attribute must be present for BbossResourceLoader");
        }
        ...
        if (logger.isInfoEnabled()) {
            logger.info("BbossResourceLoader for Velocity: using resource loader [" + this.resourceLoader + "] and resource loader paths " + Arrays.asList(this.resourceLoaderPaths));
        }
    }
    ...
    public InputStream getResourceStream(String source) throws ResourceNotFoundException {
        ...
        for (int i = 0; i < this.resourceLoaderPaths.length; i++) {
            org.frameworkset.util.io.Resource resource = this.resourceLoader.getResource(this.resourceLoaderPaths[i] + source);
            try {
        ...
        throw new ResourceNotFoundException("Could not find resource [" + source + "] in Bboss resource loader path");
    }
