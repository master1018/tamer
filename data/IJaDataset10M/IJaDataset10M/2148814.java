...
public class SnapshotProfileListener implements ProfileListener {
    ...
    private static final String PROP_BASE = SnapshotProfileListener.class.getName();
     ...
     */
    public SnapshotProfileListener(Properties properties, Identity owner, ComponentRegistry registry) throws IOException {
        PropertiesWrapper wrappedProps = new PropertiesWrapper(properties);
        int port = wrappedProps.getIntProperty(PORT_PROPERTY, DEFAULT_PORT);
        networkReporter = new NetworkReporter(port);
        long reportPeriod = wrappedProps.getLongProperty(PERIOD_PROPERTY, DEFAULT_PERIOD);
        handle = registry.getComponent(TaskScheduler.class).scheduleRecurringTask(new SnapshotRunnable(reportPeriod), owner, System.currentTimeMillis() + reportPeriod, reportPeriod);
        handle.start();
    }
        ...
        public void run() throws Exception {
            ...
            synchronized (lock) {
                try {
                    ...
                    reportStr.format("  AverageQueueSize=%2.2f tasks%n%n", ((double) readyCount / (double) totalCount));
                } finally {
                    successCount = 0;
                    totalCount = 0;
                    readyCount = 0;
