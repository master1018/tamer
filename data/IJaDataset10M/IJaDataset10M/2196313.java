 ...
 * @version 17. 1. 2011
 */
public class DBSchemaDAOImpl extends AbstractDAO implements DBSchemaDAO {

    public static final String SELECT_VERSION = "SELECT * FROM " + Constants.TABLE_VERSION_NAME + " WHERE id = 1";

    public static final String UPDATE_VERSION = "UPDATE " + Constants.TABLE_VERSION_NAME + " SET version = (?) WHERE id = 1";
    ...
    public void updateSchema(int version, String pathPrefix) throws DatabaseException {
        ...
        if (!dbSchema.exists()) {
            throw new DatabaseException("Unable to find the file with DB schema " + dbSchema.getAbsolutePath());
        }
        if (!dbSchema.canRead()) {
            throw new DatabaseException("Unable to read from the file with DB schema " + dbSchema.getAbsolutePath());
        }
        try {
            ...
            try {
                ...
                e.printStackTrace();
                throw new DatabaseException("Unable to find the file with DB schema " + dbSchema.getAbsolutePath(), e);
            } catch (IOException e) {
                LOGGER.error(e.getMessage());
                e.printStackTrace();
                throw new DatabaseException("Unable to read from file with DB schema " + dbSchema.getAbsolutePath(), e);
            } catch (SQLException e) {
                LOGGER.error(e.getMessage());
                e.printStackTrace();
            ...
            Writer writer = null;
            try {
                writer = new BufferedWriter(new FileWriter(pathPrefix + File.separator + Constants.SCHEMA_VERSION_PATH));
                writer.write(String.valueOf(version));
                writer.flush();
            } catch (IOException e) {
                LOGGER.error(e.getMessage());
                e.printStackTrace();
                throw new DatabaseException("Unable to write to file with DB schema version " + dbSchema.getAbsolutePath(), e);
            } finally {
                if (writer != null) {
                    try {
                        writer.close();
