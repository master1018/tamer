...
public class CalendarH extends JPanel {
     ...
     */
    private void deleteActionsAllData(long startTmp, long endTmp, boolean isDelAllDataTmp) {
        if (isDelAllDataTmp) {
            if (!Options.getInstance().getEnvDir().trim().equalsIgnoreCase(envDir.trim())) {
                if (JOptionPane.showConfirmDialog(this, Options.getInstance().getResource("delete all data?"), Options.getInstance().getResource("attention"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == 0) {
                    monthView.setEnabled(false);
                    jButtonSelect.setEnabled(false);
                    jButtonDelete.setEnabled(false);
                    mainPreview.addListener(this);
                    mainPreview.deleteAndClearBDBLogsRun(startTmp, endTmp, envDir, isDelAllDataTmp);
                }
            } else {
                JOptionPane.showConfirmDialog(this, Options.getInstance().getResource("cant delete all data"), Options.getInstance().getResource("attention"), JOptionPane.CLOSED_OPTION, JOptionPane.ERROR_MESSAGE);
            }
        } else {
            if (JOptionPane.showConfirmDialog(this, Options.getInstance().getResource("delete data?"), Options.getInstance().getResource("attention"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == 0) {
    ...
    boolean isDeleteAllData() {
        boolean out = false;
        if ((getStartSelection() <= getStartBDB()) && (getEndSelectionPlus2659() >= getEndBDB())) {
            out = true;
        }
        return out;
    }
    ...
    long getEndSelectionPlus2659() {
        ...
        DateTime endDaySelectPlus2659 = new DateTime(endDaySelect.getYear(), endDaySelect.getMonthOfYear(), endDaySelect.getDayOfMonth(), 23, 59, 59, 999);
    ...
    private final class ActionListenerMonthView implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            DateSpan dateSpanFromCalendar = ((JXMonthView) e.getSource()).getSelectedDateSpan();
            DateSpan dateSpanBeginEndBDB = new DateSpan(startBDB, endBDB);
            ...
            DateTime startBDBPlus0000 = new DateTime(startDayBDB.getYear(), startDayBDB.getMonthOfYear(), startDayBDB.getDayOfMonth(), 0, 0, 0, 0);
            DateTime startBDBPlus2359 = new DateTime(startDayBDB.getYear(), startDayBDB.getMonthOfYear(), startDayBDB.getDayOfMonth(), 23, 59, 59, 999);
            DateTime endDayCalendar = new DateTime(dateSpanFromCalendar.getEnd());
            DateTime endCalendarPlus2359 = new DateTime(endDayCalendar.getYear(), endDayCalendar.getMonthOfYear(), endDayCalendar.getDayOfMonth(), 23, 59, 59, 999);
            if (dateSpanBeginEndBDB.intersects(dateSpanFromCalendar.getStart(), endCalendarPlus2359.getMillis())) {
                buttonSelect.setEnabled(true);
            } else {
                buttonSelect.setEnabled(false);
            }
            if (dateSpanFromCalendar.intersects(startBDBPlus0000.getMillis(), startBDBPlus2359.getMillis())) {
    ...
    private long[] getFraggedDays(long begin0, long end0) {
        ...
        DateTime beginDayPlus0000 = new DateTime(beginTmp.getYear(), beginTmp.getMonthOfYear(), beginTmp.getDayOfMonth(), 0, 0, 0, 0);
        DateTime endTmp = new DateTime(end);
        DateTime endPlus2359 = new DateTime(endTmp.getYear(), endTmp.getMonthOfYear(), endTmp.getDayOfMonth(), 23, 59, 59, 999);
    ...
    private String getPeriodBDB() {
        ...
        if (period.getMonths() > 0) out = period.getMonths() + " month(s) ";
        if (period.getDays() > 0) out = out + period.getDays() + " day(s) ";
        if (period.getHours() > 0) out = out + period.getHours() + " hour(s) ";
        if (period.getMinutes() > 0) out = out + period.getMinutes() + " minute(s) ";
        return out;
    }
