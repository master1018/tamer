...
public class FloodFiller {
    ...
    public boolean fill(int x, int y) {
        ...
        while (true) {
            ...
            int x1 = x;
            int x2 = x;
            while (ip.getPixel(x1, y) == color && x1 >= 0) x1--;
            x1++;
            while (ip.getPixel(x2, y) == color && x2 < width) x2++;
            x2--;
            fillLine(ip, x1, x2, y);
            ...
            for (int i = x1; i <= x2; i++) {
                if (!inScanLine && y > 0 && ip.getPixel(i, y - 1) == color) {
                    push(i, y - 1);
                    inScanLine = true;
                } else if (inScanLine && y > 0 && ip.getPixel(i, y - 1) != color) inScanLine = false;
            }
            inScanLine = false;
            for (int i = x1; i <= x2; i++) {
                if (!inScanLine && y < height - 1 && ip.getPixel(i, y + 1) == color) {
                    push(i, y + 1);
                    inScanLine = true;
                } else if (inScanLine && y < height - 1 && ip.getPixel(i, y + 1) != color) inScanLine = false;
            }
        }
    }
        ...
        while (true) {
            ...
            for (int i = x1; i <= x2; i++) {
                if (!inScanLine && y < hm1 && ip.getPixel(i, y + 1) == color) {
                    push(i, y + 1);
                    inScanLine = true;
                } else if (inScanLine && y < hm1 && ip.getPixel(i, y + 1) != color) inScanLine = false;
            }
        }
    }
    ...
    public void particleAnalyzerFill(int x, int y, double level1, double level2, ImageProcessor mask, Rectangle bounds) {
        ...
        while (true) {
            ...
            int x1 = x;
            int x2 = x;
            while (inParticle(x1, y, level1, level2) && x1 >= 0) x1--;
            x1++;
            while (inParticle(x2, y, level1, level2) && x2 < width) x2++;
            x2--;
            fillLine(mask, x1 - bounds.x, x2 - bounds.x, y - bounds.y);
            fillLine(ip, x1, x2, y);
            boolean inScanLine = false;
            if (x1 > 0) x1--;
            if (x2 < width - 1) x2++;
            for (int i = x1; i <= x2; i++) {
                if (!inScanLine && y > 0 && inParticle(i, y - 1, level1, level2)) {
                    push(i, y - 1);
                    inScanLine = true;
                } else if (inScanLine && y > 0 && !inParticle(i, y - 1, level1, level2)) inScanLine = false;
            }
            inScanLine = false;
            for (int i = x1; i <= x2; i++) {
                if (!inScanLine && y < height - 1 && inParticle(i, y + 1, level1, level2)) {
                    push(i, y + 1);
                    inScanLine = true;
                } else if (inScanLine && y < height - 1 && !inParticle(i, y + 1, level1, level2)) inScanLine = false;
            }
        }
    }

    final boolean inParticle(int x, int y, double level1, double level2) {
        if (isFloat) return ip.getPixelValue(x, y) >= level1 && ip.getPixelValue(x, y) <= level2; else {
    ...
    final int popx() {
        if (stackSize == 0) return -1; else return xstack[stackSize - 1];
    }
    ...
    final void fillLine(ImageProcessor ip, int x1, int x2, int y) {
        ...
        for (int x = x1; x <= x2; x++) ip.drawPixel(x, y);
    }
}
