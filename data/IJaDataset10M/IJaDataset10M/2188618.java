...
public class BmProcessor extends AbstractProcessor {
    ...
    public void processMap(BuildContext bc) {
        if (BuildContext.LEVEL_UNDEFINED_BM == 0) return;
        processAttribute(bc);
    }
    ...
    protected void visitAttribute(Attribute a, BuildContext bc) {
        ...
        if (SxsdUtil.isBMReference(type)) {
            ...
            if (bmf == null) {
                ...
                String msg = null;
                if (bm.length() == 0) msg = String.format(BuildMessages.get("build.notbeempty"), name); else msg = String.format(BuildMessages.get("build.notexists"), name, bm);
                AuroraBuilder.addMarker(bc.file, msg, line + 1, region, BuildContext.LEVEL_UNDEFINED_BM, AuroraBuilder.UNDEFINED_BM);
            }
        }
    }
