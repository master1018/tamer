...
public class JavassitMethodSynthesizer implements MethodSynthesizer {
    ...
    public DataReader synthesizeReader(final PropertyDescriptor descriptor) {
        ...
        if (reader == null) {
            ...
            try {
                do {
                    reader = makeReaderClass(pool, getClassLoader(), readerClassName, sourceClassNameFull, sourceClassGetterMethodName, readMethod.getGenericReturnType());
                    if (reader == null) {
                        reader = getFromCacheOrCreateFromClassLoader(readerClassName, READER_CACHE, getClassLoader());
                    }
                } while (reader == null);
    ...
    private <T> T getFromCacheOrCreateFromClassLoader(final String readerClassName, final Cache<String, Object> cache, final ClassLoader classLoader) {
        Object instance;
    ...
    private DataReader makeReaderClass(final ClassPool pool, final ClassLoader loader, final String readerClassName, final String sourceClassNameFull, final String sourceClassGetterMethodName, final Type sourceClassGetterMethodReturnType) {
        ...
        if (sourceClassGetterMethodReturnType instanceof Class) {
            ...
            methodReturnType = Object.class.getCanonicalName();
            methodReturnTypePrimitiveName = null;
        } else {
        ...
        try {
            ...
            final StringBuilder getReturnTypeMethodCode = new StringBuilder().append("public Class getReturnType() {\n").append("return ").append(methodReturnType).append(".class;\n").append("}");
            ...
            ctClass.detach();
            final DataReader reader = (DataReader) ctClass.toClass(loader, DataReader.class.getProtectionDomain()).newInstance();
    ...
    public DataWriter synthesizeWriter(final PropertyDescriptor descriptor) {
        ...
        if (writer == null) {
            ...
            try {
                do {
                    writer = makeWriterClass(pool, getClassLoader(), writerClassName, classNameFull, methodName, writeMethod.getParameterTypes()[0]);
                    if (writer == null) {
                        writer = getFromCacheOrCreateFromClassLoader(writerClassName, WRITER_CACHE, getClassLoader());
                    }
                } while (writer == null);
    ...
    private DataWriter makeWriterClass(final ClassPool pool, final ClassLoader loader, final String writerClassName, final String sourceClassNameFull, final String sourceClassSetterMethodName, final Class<?> sourceClassSetterMethodArgumentClass) {
        ...
        try {
            ...
            final StringBuilder writeMethodCode = new StringBuilder().append("public void write(Object source, Object value) {\n").append("final ").append(sourceClassNameFull).append(" clazz = (").append(sourceClassNameFull).append(") source;\n").append("clazz.").append(sourceClassSetterMethodName).append("(");
            if (methodArgPrimitiveName == null) {
                writeMethodCode.append("(").append(methodArgType).append(") value");
            } else {
                appendPrimitiveValue(methodArgPrimitiveName, writeMethodCode, "value");
            }
            writeMethodCode.append(");\n}");
            final StringBuilder getParameterTypeMethodCode = new StringBuilder().append("public Class getParameterType() {\n").append("return ").append(methodArgType).append(".class;\n").append("}");
            if (LOG.isDebugEnabled()) {
            ...
            ctClass.detach();
            final DataWriter writer = (DataWriter) ctClass.toClass(loader, DataWriter.class.getProtectionDomain()).newInstance();
    ...
    private void appendPrimitiveValue(final String primitiveTypeName, final StringBuilder toAppendTo, final String valueOf) {
        toAppendTo.append("((").append(PRIMITIVE_TO_WRAPPER.get(primitiveTypeName)).append(") ").append(valueOf).append(")").append(WRAPPER_TO_PRIMITIVE.get(primitiveTypeName));
    }
}
