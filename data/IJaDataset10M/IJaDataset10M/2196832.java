...
public class UDPTester {
    ...
    private static void startUDPServices(int port) throws IOException {
        ...
        udpSelectorProvider = new UDPSelectorProvider(new DefaultRUDPContext(factory, NIODispatcher.instance().getTransportListener(), service, new DefaultRUDPSettings()));
    ...
    public static void echoServer(String host, int port) throws IOException {
        ...
        for (int i = 0; i < Integer.MAX_VALUE; i += len) {
            ...
            total += len;
            if (len != 512) System.out.println("Abnormal data size: " + len + " loc: " + i);
            for (int j = 0; j < len; j++) {
                int btest = data[j] & 0xff;
                AssertComparisons.assertEquals("Read unexpected value at offset " + (i + j), btest, (i + j) % 256);
            }
            updateBandwidth(total);
        }
    }
