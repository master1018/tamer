...
public class RegRes implements Cloneable {
    ...
    public void copyOutOf(RegRes r) {
        if (r.marks == null) marks = null; else try {
            marks = new int[r.marks.length];
            for (int i = 0; i < marks.length; i++) marks[i] = r.marks[i];
        } catch (Throwable t) {
        }
    ...
    public boolean equals(RegRes r) {
        if (charsMatched_ != r.charsMatched_ || matchFrom_ != r.matchFrom_ || didMatch_ != r.didMatch_ || numSubs_ != r.numSubs_ || !src.unwrap().equals(r.src.unwrap())) return false;
        if (marks == null && r.marks != null) return false;
        if (marks != null && r.marks == null) return false;
        for (int i = 1; i <= numSubs_; i++) {
            if (matchedFrom(i) != r.matchedFrom(i)) return false; else if (charsMatched(i) != r.charsMatched(i)) return false;
        }
        return true;
    }
    ...
    public String stringMatched() {
        ...
        return !didMatch_ || mf < 0 || cm < 0 ? null : src.substring(mf, mf + cm);
    }
    ...
    public String right() {
        int mf = matchedFrom(), cm = charsMatched();
        return !didMatch_ || mf < 0 || cm < 0 ? null : src.substring(mf + cm, src.length());
    }
