 ...
 */
public class VoucherDeliveryNoteActionHandler extends DeliveryNoteActionHandler {
    ...
    public VoucherStore getVoucherStore() {
        if (voucherStore == null) voucherStore = VoucherStore.getVoucherStore(getPersistenceManager());
        return voucherStore;
    }

    @Override
    public void onDeliverBegin(User user, DeliveryData deliveryData, DeliveryNote deliveryNote) throws Exception, DeliveryException {
        if (logger.isDebugEnabled()) logger.debug("onDeliverBegin: entered for " + JDOHelper.getObjectId(deliveryData) + " and " + JDOHelper.getObjectId(deliveryNote));
        Delivery delivery = deliveryData.getDelivery();
        if (delivery.isPostponed()) return;
        ...
        for (Article article : delivery.getArticles()) {
            ...
            if (product instanceof Voucher) {
                ...
                if (article.isReversing()) continue;
                Collection<? extends VoucherKey> voucherKeys = VoucherKey.getVoucherKeys(pm, voucher, VoucherKey.VALIDITY_VALID);
                if (!voucherKeys.isEmpty()) throw new IllegalStateException("There should be no valid VoucherKey for this Voucher, but there is/are " + voucherKeys.size() + "!!! VoucherPK=" + voucher.getPrimaryKey());
                ...
                voucher.setVoucherKey(voucherKey);
            }
        }
    }
    ...
    public void onDeliverEnd(User user, DeliveryData deliveryData, DeliveryNote deliveryNote) {
        if (logger.isDebugEnabled()) logger.debug("onDeliverEnd: entered for " + JDOHelper.getObjectId(deliveryData) + " and " + JDOHelper.getObjectId(deliveryNote));
        Delivery delivery = deliveryData.getDelivery();
        if (delivery.isPostponed()) return;
        for (Article article : delivery.getArticles()) {
            Product product = article.getProduct();
            if (product instanceof Voucher) {
                Voucher voucher = (Voucher) product;
                if (article.isReversing()) voucher.getVoucherKey().setValidity(VoucherKey.VALIDITY_REVERSED, user); else voucher.getVoucherKey().setValidity(VoucherKey.VALIDITY_VALID, user);
            }
        }
    }
}
