...
public abstract class _BaseRootDAO {
    ...
    public static void initialize() {
        pallamiabo.dao._RootDAO.initialize((String) null);
    }
    ...
    public static void initialize(String configFileName) {
        pallamiabo.dao._RootDAO.initialize(configFileName, pallamiabo.dao._RootDAO.getNewConfiguration(null));
    }

    public static void initialize(String configFileName, Configuration configuration) {
        if (null == configFileName && null != sessionFactory) return; else if (null != sessionFactoryMap && null != sessionFactoryMap.get(configFileName)) return; else {
            if (null == configFileName) {
                configuration.configure();
                pallamiabo.dao._RootDAO.setSessionFactory(configuration.buildSessionFactory());
            } else {
                configuration.configure(configFileName);
                pallamiabo.dao._RootDAO.setSessionFactory(configFileName, configuration.buildSessionFactory());
            }
        }
    }
    ...
    protected SessionFactory getSessionFactory(String configFile) {
        if (null == configFile) {
            if (null == sessionFactory) throw new RuntimeException("The session factory has not been initialized (or an error occured during initialization)"); else return sessionFactory;
        } else {
            if (null == sessionFactoryMap) throw new RuntimeException("The session factory for '" + configFile + "' has not been initialized (or an error occured during initialization)"); else {
                SessionFactory sf = (SessionFactory) sessionFactoryMap.get(configFile);
                if (null == sf) throw new RuntimeException("The session factory for '" + configFile + "' has not been initialized (or an error occured during initialization)"); else return sf;
            }
        }
    }
    ...
    protected void throwException(Throwable t) {
        if (t instanceof HibernateException) throw (HibernateException) t; else if (t instanceof RuntimeException) throw (RuntimeException) t; else throw new HibernateException(t);
    }
    ...
    public class TransactionPointer {
        ...
        public Object waitUntilFinish(long timeout) throws Throwable {
            ...
            if (null != thrownException) throw thrownException; else return returnValue;
        }
    }
