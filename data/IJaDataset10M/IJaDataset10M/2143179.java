...
public class BufferStateUtilities {
   ...
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(BufferStateUtilities.class);
   ...
   * @return
   */
    public static Collection<CommonTree> expandVariable(String slotName, String variableName, BufferEndStates endStates) {
        Collection<CommonTree> rtn = new ArrayList<CommonTree>();
        Set<String> slotNamesToExpand = new TreeSet<String>();
        for (CommonTree condition : endStates.getMapOfTrees(JACTRBuilder.MATCH_CONDITION).values()) {
            slotNamesToExpand.clear();
            Map<Integer, Collection<CommonTree>> allSlots = getSlots(condition);
            if (allSlots.containsKey(JACTRBuilder.VARIABLE)) for (CommonTree slot : allSlots.get(JACTRBuilder.VARIABLE)) if (conditionIs(slot, IConditionalSlot.EQUALS) && variableName.equalsIgnoreCase(getContent(slot))) slotNamesToExpand.add(ASTSupport.getName(slot).toLowerCase());
            for (CommonTree slot : getNonVariables(allSlots)) {
                String name = ASTSupport.getName(slot);
                if (slotNamesToExpand.contains(name.toLowerCase())) rtn.add(BufferEndState.getSupport().createSlot(slotName, (CommonTree) slot.getChild(1), (CommonTree) slot.getChild(2)));
            }
        }
        if (rtn.size() == 0) {
            rtn.add(BufferEndState.getSupport().createSlot(slotName, IConditionalSlot.NOT_EQUALS, BufferEndState.NULL));
            rtn.add(BufferEndState.getSupport().createSlot(slotName, IConditionalSlot.NOT_EQUALS, BufferEndState.NIL));
        }
        return rtn;
    }
   ...
   * @return
   */
    public static Map<Integer, Collection<CommonTree>> getSlots(CommonTree conditionOrAction) {
        Map<Integer, Collection<CommonTree>> rtn = new TreeMap<Integer, Collection<CommonTree>>();
        for (CommonTree slot : ASTSupport.getTrees(conditionOrAction, JACTRBuilder.SLOT)) {
   ...
   * @return
   */
    public static Collection<CommonTree> getNonVariables(Map<Integer, Collection<CommonTree>> mapOfSlots) {
        ArrayList<CommonTree> rtn = new ArrayList<CommonTree>();
        for (int type : new int[] { JACTRBuilder.IDENTIFIER, JACTRBuilder.NUMBER, JACTRBuilder.STRING }) {
   ...
   * @return
   */
    public static Collection<CommonTree> getAssignments(Map<Integer, Collection<CommonTree>> mapOfTrees) {
        ArrayList<CommonTree> rtn = new ArrayList<CommonTree>();
        for (Collection<CommonTree> slots : mapOfTrees.values()) for (CommonTree slot : slots) if (conditionIs(slot, IConditionalSlot.EQUALS)) rtn.add(slot);
        return rtn;
    }

    public static boolean contentIsType(CommonTree slot, int type) {
        return slot.getChild(2).getType() == type;
    }

    public static String getContent(CommonTree slot) {
   ...
   * @return
   */
    public static boolean conditionIs(CommonTree slot, int condition) {
        int cond = slot.getChild(1).getType();
        if (cond == JACTRBuilder.EQUALS && condition == IConditionalSlot.EQUALS) return true;
        if (cond == JACTRBuilder.NOT && condition == IConditionalSlot.NOT_EQUALS) return true;
        if (cond == JACTRBuilder.GT && condition == IConditionalSlot.GREATER_THAN) return true;
        if (cond == JACTRBuilder.GTE && condition == IConditionalSlot.GREATER_THAN_EQUALS) return true;
        if (cond == JACTRBuilder.LT && condition == IConditionalSlot.LESS_THAN) return true;
        if (cond == JACTRBuilder.LTE && condition == IConditionalSlot.LESS_THAN_EQUALS) return true;
        return false;
    }
   ...
   * @return
   */
    public static boolean slotIsApplicable(CommonTree endStateSlot, CommonTree conditionalSlot) {
        return conditionalSlot.getChild(0).getText().equalsIgnoreCase(endStateSlot.getChild(0).getText());
    }
   ...
   */
    public static Consistent isConsistentWith(CommonTree endStateSlot, CommonTree conditionalSlot) {
        int endCondition = endStateSlot.getChild(1).getType();
        int queryCondition = conditionalSlot.getChild(1).getType();
        if (endStateSlot.getChild(2).getType() == JACTRBuilder.VARIABLE || conditionalSlot.getChild(2).getType() == JACTRBuilder.VARIABLE) throw new IllegalStateException("neither conditional nor end state slots may be variables");
        ...
        if (endCondition == JACTRBuilder.EQUALS) {
            if (queryCondition == endCondition) if (endContent.equalsIgnoreCase(queryContent)) return Consistent.YES; else return Consistent.NO;
            if (queryCondition == JACTRBuilder.NOT) if (endContent.equalsIgnoreCase(queryContent)) return Consistent.NO; else return Consistent.YES;
            return Consistent.AMBIGUOUS;
        } else if (endCondition == JACTRBuilder.NOT) {
            if (queryCondition == endCondition) return Consistent.YES;
            if (queryCondition == JACTRBuilder.EQUALS) if (endContent.equalsIgnoreCase(queryContent)) return Consistent.NO; else return Consistent.YES;
            return Consistent.AMBIGUOUS;
        } else return Consistent.AMBIGUOUS;
    }
}
