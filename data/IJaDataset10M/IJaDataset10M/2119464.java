...
public final class boReferencesManager {
    ...
    */
        ...
        for (int i = 0; i < dataSet.getRowCount(); i++) {
            ...
            if (rebuild) {
                ...
                if (!row.isNew() && row.getFlashBackRow() != null) {
                    ...
                    if (!ClassUtils.compare(newvalue, oldvalue) || (isBridge ? !row.getBigDecimal("LIN").equals(row.getFlashBackRow().getBigDecimal("LIN")) : false)) {
                        if (oldvalue != null) {
    ...
    public static final referenceHandler[] getReferences(EboContext ctx, long boui, byte type, long start, long end) {
        ...
        try {
            cn = ctx.getConnectionData();
            if (type == TYPE_REFERENCES) pstm = cn.prepareStatement(DAO_SQL_SELECTREFERENCES, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY); else if (type == TYPE_REFERENCEDBY) pstm = cn.prepareStatement(DAO_SQL_SELECTREFERENCEDBY, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            pstm.setLong(1, boui);
            rslt = pstm.executeQuery();
            if (start >= 0 && end >= 0 && start <= end) {
                int index = 0;
                while (rslt.next()) {
                    if (index >= start && index <= end) list.add(new referenceHandler(rslt.getString(2), rslt.getBigDecimal(1), null));
                    index++;
                }
            } else {
                while (rslt.next()) {
