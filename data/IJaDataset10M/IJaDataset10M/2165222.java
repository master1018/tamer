...
public class Combobox extends Textbox {
    ...
    public void setMultiline(boolean multiline) {
        if (multiline) throw new UnsupportedOperationException("multiline");
    }

    public void setRows(int rows) throws WrongValueException {
        if (rows != 1) throw new UnsupportedOperationException("rows");
    }

    public String getOuterAttrs() {
        final String attrs = super.getOuterAttrs();
        final boolean asapOpen = isAsapRequired("onOpen"), aco = isAutocomplete(), adr = isAutodrop();
        if (!asapOpen && !aco && !adr) return attrs;
        final StringBuffer sb = new StringBuffer(64).append(attrs);
        if (asapOpen) sb.append(" zk_onOpen=\"true\"");
