...
import net.sf.jabref.custom.BibtexDatabase;
import net.sf.jabref.custom.BibtexEntry;
import net.sf.jabref.custom.BibtexEntryType;
import net.sf.jabref.custom.GUIGlobals;
import net.sf.jabref.custom.Globals;
import net.sf.jabref.custom.OutputPrinter;
import net.sf.jabref.custom.Util;
...
public class IEEEXploreFetcher implements EntryFetcher {
    ...
    private final Pattern idPattern = Pattern.compile("<input name=\"\" type=\"checkbox\" value=\"\"\\s*" + "id=\"([0-9]+)\"/>");
    ...
    ArrayList<BibtexEntry> entriesFound = null;
    ...
    public boolean processQuery(String query, ImportInspector dialog, OutputPrinter status) {
        ...
        try {
            ...
            if (hits > MAX_FETCH) {
                status.showMessage(Globals.lang("%0 entries found. To reduce server load, " + "only %1 will be downloaded.", new String[] { String.valueOf(hits), String.valueOf(MAX_FETCH) }), Globals.lang("Search IEEEXplore"), JOptionPane.INFORMATION_MESSAGE);
                hits = MAX_FETCH;
            }
    ...
    private void parse(ImportInspector dialog, String text, int startIndex, int firstEntryNumber) {
        ...
        entriesFound = new ArrayList<BibtexEntry>();
        if (importBibtex) {
    ...
    private BibtexEntry parseNextEntry(String allText, int startIndex) {
        ...
        if (index >= 0 && endIndex > 0) {
            ...
            if (typeMatcher.find()) {
                ...
                if (typeName.equalsIgnoreCase("IEEE Journals") || typeName.equalsIgnoreCase("IEEE Early Access") || typeName.equalsIgnoreCase("IET Journals") || typeName.equalsIgnoreCase("AIP Journals") || typeName.equalsIgnoreCase("AVS Journals") || typeName.equalsIgnoreCase("IBM Journals")) {
    ...
    public ArrayList<BibtexEntry> getEntriesFound() {
        return entriesFound;
    }
}
