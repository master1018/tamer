...
public class BeanDefinitionValueResolver {
    ...
    private Object resolveReference(String argName, RuntimeBeanReference ref) throws BeansException {
        ...
        try {
            ...
            throw new BeanCreationException(this.beanDefinition.getResourceDescription(), this.beanName, "Can't resolve reference to bean '" + ref.getBeanName() + "' while setting property '" + argName + "'", ex);
        }
    }
    ...
    private List resolveManagedList(String argName, List ml) throws BeansException {
        ...
        for (int i = 0; i < ml.size(); i++) {
            resolved.add(resolveValueIfNecessary(argName + BeanWrapper.PROPERTY_KEY_PREFIX + i + BeanWrapper.PROPERTY_KEY_SUFFIX, ml.get(i)));
        }
        return resolved;
    }
    ...
    private Set resolveManagedSet(String argName, Set ms) throws BeansException {
        ...
        for (Iterator it = ms.iterator(); it.hasNext(); ) {
            resolved.add(resolveValueIfNecessary(argName + BeanWrapper.PROPERTY_KEY_PREFIX + i + BeanWrapper.PROPERTY_KEY_SUFFIX, it.next()));
            i++;
        }
        return resolved;
    }
    ...
    private Map resolveManagedMap(String argName, Map mm) throws BeansException {
        ...
        while (it.hasNext()) {
            ...
            resolved.put(resolveValueIfNecessary(argName, entry.getKey()), resolveValueIfNecessary(argName + BeanWrapper.PROPERTY_KEY_PREFIX + entry.getKey() + BeanWrapper.PROPERTY_KEY_SUFFIX, entry.getValue()));
        }
        return resolved;
    }
}
