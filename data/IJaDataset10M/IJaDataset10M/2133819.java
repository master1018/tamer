...
public class OGLTextRenderer extends TextRenderer {
    ...
    public static final BufferedImage getBufImg(byte[] pixels, int width, int height, Color color) {
        ...
        WritableRaster wr = Raster.createPackedRaster(new DataBufferByte(pixels, pixels.length), (pixels.length / height) << 3, height, 1, null);
    ...
    public void drawString(Graphics2D g, String str, float x, float y) {
        ...
        @SuppressWarnings("deprecation") final FontPeerImpl peer = ((FontPeerImpl) font.getPeer());
        ...
        GlyphHashtable glyphHash = (GlyphHashtable) (intHash2glyphHash.containsKey(intHash) ? intHash2glyphHash.get(intHash) : null);
        if (glyphHash == null) {
    ...
    private void createColorGlyphDL(Graphics2D g, Glyph glyph, GlyphHashtable glyphHash, Font font, Character ch, Color col, boolean isAntialias) {
        ...
        @SuppressWarnings("deprecation") final Glyph newGlyph = ((FontPeerImpl) (font.deriveFont((float) (font.getSize2D() * texSize))).getPeer()).getGlyph(ch.charValue());
        byte[] pixels = newGlyph.getBitmap();
        BufferedImage bim = getBufImg(pixels, newGlyph.getPointWidth(), newGlyph.getPointHeight(), col);
        Rectangle.Double rect = new Rectangle.Double(Math.round(glyph.getGlyphPointMetrics().getLSB()), Math.round(-newGlyph.bmp_top / texSize), glyph.getPointWidth(), glyph.getPointHeight());
        Surface sur = Surface.getImageSurface(bim);
        OGLBlitter.OGLTextureParams tp = OGLBlitter.getInstance().blitImg2OGLTexCached(sur, bim.getWidth(), bim.getHeight(), true);
    ...
    private static final class SoftHashtable extends Hashtable<Integer, Object> {
        ...
        public Object put(Integer key, Object obj) {
            @SuppressWarnings("unchecked") final SoftReference<GlyphHashtable> ref = (SoftReference<GlyphHashtable>) super.put(key, new SoftReference<GlyphHashtable>((GlyphHashtable) obj));
            return ref == null ? null : ref.get();
        }

        @Override
        public Object get(Object key) {
            @SuppressWarnings("unchecked") final SoftReference<GlyphHashtable> ref = (SoftReference<GlyphHashtable>) super.get(key);
            return ref == null ? null : ref.get();
        }

        @Override
        public Object remove(Object key) {
            @SuppressWarnings("unchecked") final SoftReference<GlyphHashtable> ref = (SoftReference<GlyphHashtable>) super.remove(key);
            return ref == null ? null : ref.get();
        }
    }
