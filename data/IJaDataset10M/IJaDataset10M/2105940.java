...
public class DocumentImpl extends CoreDocumentImpl implements DocumentTraversal, DocumentEvent, DocumentRange {
    ...
    public TreeWalker createTreeWalker(Node root, int whatToShow, NodeFilter filter, boolean entityReferenceExpansion) {
        if (root == null) {
            throw new DOMException(DOMException.NOT_SUPPORTED_ERR, "DOM007 Not supported");
        }
    ...
    void deletedText(NodeImpl node, int offset, int count) {
        if (ranges != null) {
            Enumeration en = ranges.elements();
            while (en.hasMoreElements()) {
                ((RangeImpl) en.nextElement()).receiveDeletedText(node, offset, count);
            }
        }
    }
    ...
    void insertedText(NodeImpl node, int offset, int count) {
        if (ranges != null) {
            Enumeration en = ranges.elements();
            while (en.hasMoreElements()) {
                ((RangeImpl) en.nextElement()).receiveInsertedText(node, offset, count);
            }
        }
    }
    ...
    void splitData(Node node, Node newNode, int offset) {
        if (ranges != null) {
            Enumeration en = ranges.elements();
            while (en.hasMoreElements()) {
                ((RangeImpl) en.nextElement()).receiveSplitData(node, newNode, offset);
            }
        }
    }
    ...
    public Event createEvent(String type) throws DOMException {
        ...
        if (type.equalsIgnoreCase("MutationEvents") || "MutationEvent".equals(type)) return new MutationEventImpl(); else throw new DOMException(DOMException.NOT_SUPPORTED_ERR, "DOM007 Not supported");
    }
    ...
    protected void addEventListener(NodeImpl node, String type, EventListener listener, boolean useCapture) {
        ...
        if (useCapture) ++lc.captures; else ++lc.bubbles;
    }
    ...
    */
    protected boolean dispatchEvent(NodeImpl node, Event event) {
        ...
        if (!evt.initialized || evt.type == null || evt.type.equals("")) throw new EventException(EventException.UNSPECIFIED_EVENT_TYPE_ERR, "DOM010 Unspecified event type");
        LCount lc = LCount.lookup(evt.getType());
        if (lc.captures + lc.bubbles + lc.defaults == 0) return evt.preventDefault;
        evt.target = node;
    ...
    protected void dispatchEventToSubtree(NodeImpl node, Node n, Event e) {
        Vector nodeListeners = getEventListeners(node);
        if (nodeListeners == null || n == null) return;
        ((NodeImpl) n).dispatchEvent(e);
        if (n.getNodeType() == Node.ELEMENT_NODE) {
            NamedNodeMap a = n.getAttributes();
            for (int i = a.getLength() - 1; i >= 0; --i) dispatchEventToSubtree(node, a.item(i), e);
        }
    ...
    protected void dispatchAggregateEvents(NodeImpl node, EnclosingAttr ea) {
        if (ea != null) dispatchAggregateEvents(node, ea.node, ea.oldvalue, MutationEvent.MODIFICATION); else dispatchAggregateEvents(node, null, null, (short) 0);
    }
        ...
        if (lc.captures + lc.bubbles + lc.defaults > 0) {
            ...
            while (true) {
                ...
                if (type == Node.ATTRIBUTE_NODE) {
                    ...
                    return;
                } else if (type == Node.ENTITY_REFERENCE_NODE) eventAncestor = eventAncestor.parentNode(); else return;
            }
        }
    }
    ...
    void removedAttrNode(AttrImpl attr, NodeImpl oldOwner, String name) {
        if (mutationEvents) {
            ...
            if (lc.captures + lc.bubbles + lc.defaults > 0) {
                MutationEventImpl me = new MutationEventImpl();
                me.initMutationEvent(MutationEventImpl.DOM_ATTR_MODIFIED, true, false, null, attr.getNodeValue(), null, name, MutationEvent.REMOVAL);
