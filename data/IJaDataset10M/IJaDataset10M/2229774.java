...
public class InputBuffer {
   ...
   */
    public InputBuffer(byte[] frame) throws SaslEncodingException {
        this();
        if (frame.length < 4) throw new SaslEncodingException("SASL buffer header too short");
        length = (frame[0] & 0xFF) << 24 | (frame[1] & 0xFF) << 16 | (frame[2] & 0xFF) << 8 | (frame[3] & 0xFF);
        if (length > Registry.SASL_BUFFER_MAX_LIMIT || length < 0) throw new SaslEncodingException("SASL buffer size limit exceeded");
   ...
   */
    public static int twoBytesToLength(byte[] b) throws SaslEncodingException {
        ...
        if (result > Registry.SASL_TWO_BYTE_MAX_LIMIT) throw new SaslEncodingException("SASL MPI/Text size limit exceeded");
        return result;
    }
   ...
   */
    public long getScalar(int count) throws IOException {
        ...
        if (!hasMoreElements()) throw new SaslEncodingException("Not enough bytes for a scalar in buffer");
        if (in.available() < count) throw new SaslEncodingException("Illegal SASL scalar encoding");
   ...
   */
    public byte[] getOS() throws IOException {
        if (!hasMoreElements()) throw new SaslEncodingException("Not enough bytes for an octet-sequence in buffer");
        final int elementLength = in.read();
        if (elementLength > Registry.SASL_ONE_BYTE_MAX_LIMIT) throw new SaslEncodingException("SASL octet-sequence size limit exceeded");
        if (in.available() < elementLength) throw new SaslEncodingException("Illegal SASL octet-sequence encoding");
   ...
   */
    public byte[] getEOS() throws IOException {
        if (in.available() < 2) throw new SaslEncodingException("Not enough bytes for an extended octet-sequence in buffer");
        byte[] elementLengthBytes = new byte[2];
        in.read(elementLengthBytes);
        final int elementLength = twoBytesToLength(elementLengthBytes);
        if (in.available() < elementLength) throw new SaslEncodingException("Illegal SASL extended octet-sequence encoding");
        byte[] result = new byte[elementLength];
        in.read(result);
        return result;
    }
   ...
   */
    public BigInteger getMPI() throws IOException {
        if (in.available() < 2) throw new SaslEncodingException("Not enough bytes for an MPI in buffer");
        byte[] elementLengthBytes = new byte[2];
        in.read(elementLengthBytes);
        final int elementLength = twoBytesToLength(elementLengthBytes);
        if (in.available() < elementLength) throw new SaslEncodingException("Illegal SASL multi-precision integer encoding");
   ...
   */
    public String getText() throws IOException {
        if (in.available() < 2) throw new SaslEncodingException("Not enough bytes for a text in buffer");
        byte[] elementLengthBytes = new byte[2];
        in.read(elementLengthBytes);
        final int elementLength = twoBytesToLength(elementLengthBytes);
        if (in.available() < elementLength) throw new SaslEncodingException("Illegal SASL text encoding");
        byte[] element = new byte[elementLength];
        in.read(element);
