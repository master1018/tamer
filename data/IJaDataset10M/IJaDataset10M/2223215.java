...
public class PivotAdapterGeneratorUtil {
    ...
    private static String reservedKeywords[] = new String[] { "package", "class", "interface" };

    private static String pivotModelTypes[] = new String[] { "Enumeration", "EnumerationLiteral", "Namespace", "Operation", "Parameter", "PrimitiveType", "Property", "Type" };
    ...
    public static final String TEMPLATE_LOCATION = PivotAdapterGeneratorPlugin.INSTANCE.getBaseURL().toString() + "templates";

    public static final String CLASSPATH_VARIABLE_NAME = "TUDRESDEN_OCL20_PIVOT_ADAPTER";
	 ...
	 *         type
	 */
    protected static EClass getCommonSuperType(EClass commonSuperType, EClass dslType) {
        EList<EClass> allSuperTypes = dslType.getEAllSuperTypes();
        if (commonSuperType.equals(dslType) || allSuperTypes.contains(commonSuperType)) return commonSuperType;
        ...
        while (!queue.isEmpty()) {
            ...
            for (EClass directSuperType : directSuperTypes) {
                if (allSuperTypes.contains(directSuperType)) return directSuperType;
            }
    ...
    public static String getCommonSuperType(GenModel genModel) {
        if (dSL2PivotModel == null) createAnnotatedElements(genModel);
    ...
    public static String getCommonSuperTypeForDSLTypes(GenModel genModel) {
        ...
        if (pivotModel2DSL == null) createAnnotatedElements(genModel);
    ...
    public static String getPackageNameFor(GenClass genClass) {
        return (genClass.getGenPackage().getInterfacePackageName() == null ? "" : genClass.getGenPackage().getInterfacePackageName() + ".");
    }
    ...
    protected static String getAnnotationDetails(EAnnotation annotation) {
        if (annotation.getSource().equalsIgnoreCase("http://www.tu-dresden.de/ocl20/pivot/2007/pivotmodel")) {
            EMap<String, String> annotationDetails = annotation.getDetails();
            if (annotationDetails != null) {
                if (annotationDetails.containsKey("PivotModel")) return annotationDetails.get("PivotModel");
                if (annotationDetails.containsKey("Pivotmodel")) return annotationDetails.get("Pivotmodel");
                if (annotationDetails.containsKey("pivotModel")) return annotationDetails.get("pivotModel");
                if (annotationDetails.containsKey("pivotmodel")) return annotationDetails.get("pivotmodel");
            }
    ...
    private static void getGenPackages(GenPackage genPackage) {
        ...
        for (GenClass genClass : genPackage.getGenClasses()) {
            packageNames.put(genClass.getName(), (genPackage.getInterfacePackageName() == null ? "" : genPackage.getInterfacePackageName() + "." + genClass.getName()));
        }
    }
    ...
    public static String getGenClassPackage(GenModel genModel, String genClassName) {
        if (packageNames == null) createPackageNames(genModel);
        if (packageNames.containsKey(genClassName)) return packageNames.get(genClassName); else return genClassName;
    }
        ...
        for (GenClass genClass : genPackage.getGenClasses()) {
            for (EAnnotation annotation : genClass.getEcoreClass().getEAnnotations()) {
                ...
                if (pivotModelTypeName != null) {
                    ...
                    if (pivotModel2DSL.containsKey(pivotModelTypeName)) pivotModel2DSL.get(pivotModelTypeName).add(genClass.getName()); else pivotModel2DSL.put(pivotModelTypeName, new LinkedList<String>(Arrays.asList(genClass.getName())));
                    if (pivotModelTypeName.equals("Type") || pivotModelTypeName.equals("PrimitiveType") || pivotModelTypeName.equals("Enumeration")) allDSLTypes.add(genClass.getEcoreClass());
                }
            }
        }
    }
	 ...
	 *         element
	 */
    public static String getDSLModelUniqueTypeName(GenModel genModel, GenClass genClass) {
        ...
        if (dSL2PivotModel.containsKey(genClass)) {
            ...
            if (Arrays.asList(pivotModelTypes).contains(dslModelTypeName)) return getGenClassPackage(genModel, dslModelTypeName); else {
                for (String keyword : reservedKeywords) {
                    if (dslModelTypeName.equalsIgnoreCase(keyword)) return getGenClassPackage(genModel, dslModelTypeName);
                }
	 ...
	 */
    public static List<String> getDSLModelTypeNames(GenModel genModel, String givenPivotModelType) {
        if (pivotModel2DSL == null) createAnnotatedElements(genModel);
        ...
        for (String pivotModelName : givenPivotModelTypes) {
            if (pivotModel2DSL.containsKey(pivotModelName)) retList.addAll(pivotModel2DSL.get(pivotModelName));
        }
        return retList;
    }
	 ...
	 */
    public static List<String> getDSLModelUniqueTypeNames(GenModel genModel, String givenPivotModelType) {
        if (pivotModel2DSL == null) createAnnotatedElements(genModel);
        List<String> givenPivotModelTypes = new LinkedList<String>();
        givenPivotModelTypes.add(givenPivotModelType);
        if (givenPivotModelType.equals("Type")) {
            ...
            if (pivotModel2DSL.containsKey(pivotModelType)) {
                ...
                for (String dslModelTypeName : dslModelTypeNames) {
                    ...
                    if (Arrays.asList(pivotModelTypes).contains(dslModelTypeName)) {
                        dslModelUniqueTypeNames.add(getGenClassPackage(genModel, dslModelTypeName));
                        withPackage = true;
                    } else {
                        for (String keyword : reservedKeywords) {
                    ...
                    if (!withPackage) dslModelUniqueTypeNames.add(dslModelTypeName);
                }
            }
        }
    ...
    public static String getDSLElementName(String dslElement) {
        ...
        for (String keyword : reservedKeywords) {
            if (dslElement.equalsIgnoreCase(keyword)) dslElement = "a" + startWithCapitalLetter(dslElement);
        }
        if (dslElement.contains(".")) return "dsl" + dslElement.substring(dslElement.lastIndexOf(".") + 1, dslElement.length());
    ...
    public static String getImplementThis() {
        return "// TODO: implement this method" + System.getProperty("line.separator") + "\t\t" + "// don't forget to remove the @generated tag or change it to \"@generated NOT\"";
    }
	 ...
	 */
    public static String getAdapteeClassImport(GenModel genModel, GenClass genClass) {
        String uniqueTypeName = getDSLModelUniqueTypeName(genModel, genClass);
        String packageName = getPackageNameFor(genClass);
        if (packageName.contains(".") && uniqueTypeName.contains(".")) return packageName + uniqueTypeName.substring(uniqueTypeName.lastIndexOf(".") + 1); else return packageName + uniqueTypeName;
    }
    ...
    public static String getTypeNameWithoutPackage(String typeName) {
        typeName = startWithLowerCaseLetter(typeName.substring(typeName.lastIndexOf(".") + 1));
        if (Arrays.asList(reservedKeywords).contains(typeName)) typeName = "a" + startWithCapitalLetter(typeName);
        return typeName;
    }
    ...
    public static String getProjectName(GenModel genModel) {
        return "tudresden.ocl20.pivot.metamodels." + genModel.getModelName().toLowerCase();
    }
    ...
    public static String getResourceType(GenModel genModel) {
        ...
        for (GenPackage genPackage : genModel.getGenPackages()) {
            for (EAnnotation annotation : genPackage.getEcorePackage().getEAnnotations()) {
                if (annotation.getSource().equalsIgnoreCase("http://www.tu-dresden.de/ocl20/pivot/2007/pivotmodel")) {
                    EMap<String, String> annotationDetails = annotation.getDetails();
                    if (annotationDetails != null) {
