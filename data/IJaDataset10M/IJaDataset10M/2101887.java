...
class AddEditLinguisticTermPanel extends Composite {
    ...
    private static AddEditLinguisticTermPanelUiBinder uiBinder = GWT.create(AddEditLinguisticTermPanelUiBinder.class);
    ...
    AddEditLinguisticTermPanel(IMainWindow mainWindow, DialogBox dialogBox, VariableItem variableItem) {
        ...
        for (String term : this.variableItem.getVariable().getTerms()) this.linguisticTermsListBox.addItem(term);
    ...
    private int findPosition(ListBox box, String name) {
        for (int i = 0; i < box.getItemCount(); ++i) if (box.getItemText(i).equals(name)) return i;
        return 0;
    }
    ...
    private AddEditLinguisticTermPanel(IMainWindow mainWindow, DialogBox dialogBox) {
        ...
        for (String deviceCapability : DeviceCapabilities.values()) this.deviceNameListBox.addItem(deviceCapability);
        for (String userCapability : UserCapabilities.values()) this.userNameListBox.addItem(userCapability);
    }
    ...
    void onSaveButtonClicked(ClickEvent event) {
        ...
        if (this.variableItem != null) {
            ...
            if (this.isInputSelected()) {
                ...
                } else config.getInput().getUserVariables().add(var);
            } else config.getOutput().getVariables().add(var);
        }
    }
    ...
    private String getName() {
        if (!isInputSelected()) return this.nameTextBox.getText();
        if (isDeviceSelected()) return this.deviceNameListBox.getItemText(this.deviceNameListBox.getSelectedIndex());
        return this.userNameListBox.getItemText(this.userNameListBox.getSelectedIndex());
    }

    private String[] getTerms() {
        final List<String> terms = new ArrayList<String>();
        for (int i = 0; i < this.linguisticTermsListBox.getItemCount(); ++i) terms.add(this.linguisticTermsListBox.getItemText(i));
    ...
    void onAddLinguisticTermButtonClicked(ClickEvent event) {
        ...
        final KeyPressHandler keyboardHandler = new KeyPressHandler() {
            ...
            public void onKeyPress(KeyPressEvent event) {
                if (event.getCharCode() == KeyCodes.KEY_ENTER) handler.onClick(null);
            }
        };
    ...
    void onRemoveLinguisticTermButtonClicked(ClickEvent event) {
        for (int i = 0; i < this.linguisticTermsListBox.getItemCount(); ++i) if (this.linguisticTermsListBox.isItemSelected(i)) {
