...
public class DistanceDistribution {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        for (Person p : population.getPersons().values()) {
            ...
            if ((c.getX() >= xmin) && (c.getX() <= xmax) && (c.getY() >= ymin) && (c.getY() <= ymax)) persons2.add(p);
        }
        ...
        for (Person p1 : population.getPersons().values()) {
            ...
            count++;
            if (count % 1000 == 0) {
                System.out.println(String.format("Processed %1$s of %2$s persons. (%3$s )", count, population.getPersons().size(), count / (double) population.getPersons().size()));
            }
        }
