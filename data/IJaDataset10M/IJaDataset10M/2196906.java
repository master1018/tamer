...
public class OIgui implements DynamicFactsProvider {
    ...
    public void executeAct(Literal action) {
        ...
        try {
            Collection<Literal> result = schInterpreter.verifyNorms();
    ...
    public void executeAct(LogicalFormula expr) {
        try {
            ...
            if (!i.hasNext()) txtLog.append("-- no solution --");
            while (i.hasNext()) {
                Unifier u = i.next();
                Term t = expr.clone();
                t.apply(u);
    ...
    void updateOE() {
        ...
        if (!curOE.equals(lastOEStr)) txtOE.setText(curOE);
        lastOEStr = curOE;
    }
    ...
    void updateNS() {
        try {
            ...
            if (!curStr.equals(lastNSStr)) txtNS.setText(curStr);
            lastNSStr = curStr;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    void initComponents() throws Exception {
        ...
        getOSTransformer().transform(new DOMSource(getParser().parse(si)), new StreamResult(so));
        ...
        final JTextField cag = new JTextField(10);
        commitp.add(cag);
        cag.setText("jaime");
        final JTextField cmi = new JTextField(10);
        commitp.add(cmi);
        cmi.setText("mManager");
        JButton cbt = new JButton("commit");
        commitp.add(cbt);
        ...
        final JTextField aag = new JTextField(10);
        achp.add(aag);
        aag.setText("jaime");
        final JTextField ago = new JTextField(10);
        achp.add(ago);
        ago.setText("wtitle");
        JButton abt = new JButton("achieve");
        achp.add(abt);
        actionsp.add(achp);
        ...
        final JTextField tquery = new JTextField(20);
        queryp.add(tquery);
        tquery.setText("well_formed(sch2)");
        JButton qbt = new JButton("query");
        queryp.add(qbt);
        actionsp.add(queryp);
    ...
    private DocumentBuilder getParser() throws ParserConfigurationException {
        if (parser == null) parser = DOMUtils.getParser();
        return parser;
    }
