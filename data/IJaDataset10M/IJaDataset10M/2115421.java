...
abstract class CompressionStreamElement {
     ...
     */
    static final int quantizationMask[] = { 0xFFFF0000, 0xFFFF8000, 0xFFFFC000, 0xFFFFE000, 0xFFFFF000, 0xFFFFF800, 0xFFFFFC00, 0xFFFFFE00, 0xFFFFFF00, 0xFFFFFF80, 0xFFFFFFC0, 0xFFFFFFE0, 0xFFFFFFF0, 0xFFFFFFF8, 0xFFFFFFFC, 0xFFFFFFFE, 0xFFFFFFFF };
    ...
    private static final int getLength(int number) {
        if (number == 0) return 0; else if ((number & 0x8000) > 0) {
    ...
    final void computeLengthShift(int n0, int n1) {
        int s0 = n0 & 0x8000;
        int s1 = n1 & 0x8000;
        if (s0 == s1) if (s0 == 0) this.length = getLength(n0 | n1); else this.length = getLength(n0 & n1); else this.length = getMaximum(getLength(n0), getLength(n1));
    ...
    final void computeLengthShift(int n0, int n1, int n2) {
        ...
        int s1 = n1 & 0x8000;
        int s2 = n2 & 0x8000;
        if (s0 == s1) if (s1 == s2) if (s2 == 0) this.length = getLength(n0 | n1 | n2); else this.length = getLength(n0 & n1 & n2); else if (s1 == 0) this.length = getMaximum(getLength(n0 | n1), getLength(n2)); else this.length = getMaximum(getLength(n0 & n1), getLength(n2)); else if (s1 == s2) if (s2 == 0) this.length = getMaximum(getLength(n1 | n2), getLength(n0)); else this.length = getMaximum(getLength(n1 & n2), getLength(n0)); else if (s0 == 0) this.length = getMaximum(getLength(n0 | n2), getLength(n1)); else this.length = getMaximum(getLength(n0 & n2), getLength(n1));
    ...
    final void computeLengthShift(int n0, int n1, int n2, int n3) {
        this.length = getMaximum(getLength(n0), getLength(n1), getLength(n2), getLength(n3));
    ...
    private static final int getMaximum(int x, int y, int z, int w) {
        int n0, n1;
        if (x > y) n0 = x; else n0 = y;
        if (z > w) n1 = z; else n1 = w;
        if (n0 > n1) return n0; else return n1;
    }
}
