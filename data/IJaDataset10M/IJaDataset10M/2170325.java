...
public class TodoManager extends AbstractStartable implements Runnable {
	 ...
	 */
    public TodoManager(int maxEntries, int entryDelay, int minWorkers, int maxWorkers, int workerLinger, int threshold) {
        if (maxEntries < 1) throw new IllegalArgumentException("maxEntries < 1");
        if (entryDelay < 1) throw new IllegalArgumentException("entryDelay < 1");
        if (minWorkers < 0) throw new IllegalArgumentException("minWorkers < 0");
        if (maxWorkers < 1) throw new IllegalArgumentException("maxWorkers < 1");
        if (maxWorkers < minWorkers) throw new IllegalArgumentException("maxWorkers < minWorkers");
        if (workerLinger < 1) throw new IllegalArgumentException("workerLinger < 1");
    ...
    public synchronized void add(Todo todo) throws InterruptedException {
        ...
        if (n > maxEntries) Thread.sleep(entryDelay);
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (needsAdjust) workers.adjust(-1);
        }
    }
    ...
    private synchronized void considerStartingAWorker(int qlen) {
        ...
        if (n == 0 || ((qlen + n - 1) / n) > threshold) startAWorker();
    }
    ...
    private synchronized int addEntry(Todo todo) {
        Entry e = new Entry();
        e.todo = todo;
        if (tail != null) tail.next = e; else head = e;
        tail = e;
    ...
    public static void shutdown() throws Exception {
        ...
        if (oldTodoManager != null) oldTodoManager.stop();
    }
