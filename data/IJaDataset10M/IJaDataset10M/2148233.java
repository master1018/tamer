...
final class TabTestTab {
    ...
    private JComponent buildHorizontalSplit() {
        JSplitPane pane = Factory.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT, buildMainLeftPanel(), buildMainRightPanel(), 0.2f);
    ...
    private JComponent buildSplittedTabs(int tabPlacement) {
        int orientation = (tabPlacement == JTabbedPane.TOP || tabPlacement == JTabbedPane.BOTTOM) ? JSplitPane.HORIZONTAL_SPLIT : JSplitPane.VERTICAL_SPLIT;
        JComponent split = Factory.createStrippedSplitPane(orientation, buildTabPanel(tabPlacement, JTabbedPane.WRAP_TAB_LAYOUT), buildTabPanel(tabPlacement, JTabbedPane.SCROLL_TAB_LAYOUT), 0.5f);
