...
public class MinConflictsStrategy extends SolutionStrategy {
    ...
    private List<Variable> getConflictedVariables(Assignment assignment, CSP csp) {
        ...
        for (Constraint constraint : csp.getConstraints()) {
            if (!constraint.isSatisfiedWith(assignment)) for (Variable var : constraint.getScope()) if (!result.contains(var)) result.add(var);
        }
        return result;
    }
    ...
    private Object getMinConflictValueFor(Variable var, Assignment assignment, CSP csp) {
        ...
        if (!resultCandidates.isEmpty()) return Util.selectRandomlyFromList(resultCandidates); else return null;
    }

    private int countConflicts(Assignment assignment, List<Constraint> constraints) {
        int result = 0;
        for (Constraint constraint : constraints) if (!constraint.isSatisfiedWith(assignment)) result++;
        return result;
    }
}
