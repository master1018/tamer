...
/**
 * @author mog
 * @version $Id: JDOMRemoteFile.java 543 2004-05-12 00:45:20Z mog $
 */
public class JDOMRemoteFile implements RemoteFileInterface {
    ...
    public JDOMRemoteFile(Element element, Hashtable allSlaves) {
        try {
            ...
            if (element.getName().equals("directory")) {
                ...
                try {
                    this.checkSum = Long.parseLong(element.getChildText("checksum"), 16);
                ...
                for (Iterator iter = element.getChild("slaves").getChildren("slave").iterator(); iter.hasNext(); ) {
                    ...
                    if (rslave == null) {
                        logger.log(Level.WARN, slaveName + " not in slavelist, not adding file: " + getName());
                        continue;
                    }
            ...
            this.lastModified = Long.parseLong(element.getChild("lastModified").getText());
        } catch (NumberFormatException ex) {
    ...
    public String toString() {
        ...
        if (isDirectory()) ret.append("[directory: " + getFiles().size() + "]");
        if (isFile()) ret.append("[file: true]");
        ret.append(getName());
        return ret.toString();
    }
}
