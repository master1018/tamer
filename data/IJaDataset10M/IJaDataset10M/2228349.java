...
import src.projects.SNP_Database.Iterators.Generic_SNP_Iterator;
import src.projects.SNP_Database.libs.PSQL_SNP_insert;

public class SNP_Parser {
    ...
    private static String cancer_type;
    ...
    private static String sample_organ;

    private static String cell_type;
    ...
    private static void parse_input(HashMap<String, String> Variables) {
        ...
        if (Variables.containsKey("cancer_type")) {
            CommandLine.test_parameter_count(LB, "cancer_type", Variables.get("cancer_type"), 1);
            cancer_type = Variables.get("cancer_type");
            LB.notice(" * Cancer type       : " + cancer_type);
        } else {
            cancer_type = "";
            LB.notice(" * Cancer type       : none");
        }
        ...
        LB.notice(" * Set as Cancer     : " + cancer_type);
        ...
        if (Variables.containsKey("pubmed_id")) {
            CommandLine.test_parameter_count(LB, "pubmed_id", Variables.get("pubmed_id"), 1);
            pubmed_id = Variables.get("pubmed_id");
            LB.notice(" * Pubmed Id         : " + pubmed_id);
        } else {
            pubmed_id = "";
            LB.notice(" * Pubmed Id         : none");
        }
        ...
        if (Variables.containsKey("sample_organ")) {
            CommandLine.test_parameter_count(LB, "sample_organ", Variables.get("sample_organ"), 1);
            sample_organ = Variables.get("sample_organ");
            LB.notice(" * Sample organ src. : " + sample_organ);
        } else {
            sample_organ = "";
            LB.notice(" * Sample organ src. : none");
        }
        ...
        if (Variables.containsKey("cell_type")) {
            CommandLine.test_parameter_count(LB, "cell_type", Variables.get("cell_type"), 1);
            cell_type = Variables.get("cell_type");
            LB.notice(" * Sample cell type  : " + cell_type);
        } else {
            cell_type = "";
            LB.notice(" * Sample cell type  : none");
        }
        ...
        Variables.remove("cancer_type");
        Variables.remove("contact");
        Variables.remove("protocol");
        Variables.remove("pubmed_id");
        Variables.remove("readlength");
        Variables.remove("sample_organ");
    ...
    public static void main(String[] args) {
        ...
        LB.Version("SNP_Parser", "$Revision: 2210 $");
        PSQL_SNP_insert psqlsi = new PSQL_SNP_insert(LB, psql_file);
        ...
        while (gsi.hasNext()) {
            ...
            count += 1;
            if (count % 50000 == 0) {
                psqlsi.insert_SNPs(VSNP, "hg18", min_aln_quality, min_base_quality, aligner, aligner_ver, snp_caller, snp_caller_ver, name, reference, seq_type, readlength, 0, cancer, cancer_type, sample_organ, "", pubmed_id, input_file, contact, protocol, cell_type, platform);
                VSNP.clear();
            }
        }
