 ...
 * @version $Revision: 391 $, $Date: 2007-01-18 08:31:58 -0500 (Thu, 18 Jan 2007) $
 * @since 1.7
 */
public final class MatcherEditorListDialog extends JDialog {
    ...
    private final class AddMatcherAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            final MatcherEditorDialog dialog = new MatcherEditorDialog(MatcherEditorListDialog.this, true);
            ...
            try {
                ...
                editorsList.repaint();
                if (log.isDebugEnabled()) {
                    log.debug(MessageFormat.format(DEBUG_ADDED, new Object[] { new Integer(editorsList.getModel().getSize()) }));
                }
            } finally {
                lock.unlock();
            }
    ...
    private static final RvSnoopMatcherEditor PROTOTYPE = new RvSnoopMatcherEditor(DataAccessorFactory.getInstance().createSendSubjectAccessor(), PredicateFactory.getInstance().createStringStartsWithPredicate("PROTOTYPE", false));

    private static final long serialVersionUID = -642470709671436909L;

    static {
        NLSUtils.internationalize(MatcherEditorListDialog.class);
    }
