    ...
    public boolean execute(MOB mob, Vector commands, int metaFlags) throws java.io.IOException {
        ...
        if ((mypolls[0].size() == 0) && (mypolls[2].size() == 0)) {
            if ((commands != null) && (mypolls[1].size() == 0)) {
                ...
                if (mypolls[1].size() > 0) mob.tell(mypolls[1].size() + " poll(s) <-IS-ARE> awaiting your participation.");
                return false;
            }
        }
        ...
        for (int i = 0; i < mypolls[2].size(); i++) {
            ...
            if (P.mayISeeResults(mob)) {
                CMLib.polls().processResults(P, mob);
                if (i < mypolls[2].size() - 1) mob.session().prompt("Press ENTER to continue:\n\r");
            }
        }
        return false;
    }
