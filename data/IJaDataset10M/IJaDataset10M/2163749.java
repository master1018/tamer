...
public class DXAttributesTest extends TestCase {
    ...
    public void testEquality() {
        try {
            ...
            assertEquals("chack that DXAttributes containing the same unordered list of attribute objects are equal", atts1, atts2);
            atts2.remove("att2");
            assertTrue("check that DXAttributes with different sizes aren't equal", !atts1.equals(atts2));
            atts2.put(attribute5);
            assertTrue("check that DXAttributes with same sizes but different att objects aren't equal", !atts1.equals(atts2));
            assertTrue("check null not equal...", !empty.equals(null));
            atts2.remove("att1");
            atts2.remove("att3");
            atts2.remove("att4");
            atts2.remove("att5");
            assertEquals("check empty atts always equal...", empty, atts2);
    ...
    public void DONTtestExpandAllAttributes1() throws Exception {
    ...
    public void DONTtestExpandAllAttributes2() throws Exception {
        ConnectionData cData = new ConnectionData();
        cData.setURL("ldap://betch01:19389");
        DirContext ctx = BasicOps.openContext(cData);
        DXAttributes a = new DXAttributes(ctx.getAttributes("ou=Manufacturing,o=DEMOCORP,c=AU"));
    ...
    public void DONTtestExpandAllAttributes3() throws Exception {
        ConnectionData cData = new ConnectionData();
        cData.setURL("ldap://betch01:19389");
        DirContext ctx = BasicOps.openContext(cData);
        DXAttributes a = new DXAttributes(ctx.getAttributes("ou=Manufacturing,o=DEMOCORP,c=AU"));
    ...
    public void DONTtestExpandAllAttributes4() throws Exception {
        ConnectionData cData = new ConnectionData();
        cData.setURL("ldap://betch01:19389");
        DirContext ctx = BasicOps.openContext(cData);
        DXAttributes a = new DXAttributes(ctx.getAttributes("ou=Manufacturing,o=DEMOCORP,c=AU"));
    ...
    public void testGetDeletionSet() {
        ...
        try {
            ...
            assertEquals("The size of the deletion set is wrong.  Was " + deletionSet.size() + ", should be 1.", 1, deletionSet.size());
