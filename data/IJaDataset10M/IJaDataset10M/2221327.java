...
public class AtomAPIMMessage implements APIMMessage {
    ...
    private static Logger LOG = Logger.getLogger(AtomAPIMMessage.class.getName());
    ...
    public AtomAPIMMessage(FedoraMethod method, String fedoraBaseUrl, String serverVersion, String format) throws MessagingException {
        ...
        if (method.getContext() != null) {
            author = method.getContext().getSubjectValue(Constants.SUBJECT.LOGIN_ID.uri);
        } else {
            author = "unknown";
        }
        ...
        setReturnValue();
        if (serverVersion != null && !serverVersion.equals("")) entry.addCategory(versionPredicate, serverVersion, null);
        if (format != null && !format.equals("")) entry.addCategory(formatPredicate, format, null);
    }
