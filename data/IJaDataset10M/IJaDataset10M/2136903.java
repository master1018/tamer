...
public class ComplexVector extends MathVector implements HilbertSpace.Member {
    ...
    public boolean equals(Object a) {
        if (a != null && (a instanceof ComplexVector) && N == ((ComplexVector) a).N) {
            ...
            for (int i = 0; i < N; i++) {
                if (!cv.getComponent(i).equals(vectorRe[i], vectorIm[i])) return false;
            }
            return true;
        } else return false;
    }
    ...
    public Complex getComponent(final int n) {
        if (n >= 0 && n < N) return new Complex(vectorRe[n], vectorIm[n]); else throw new VectorDimensionException(getInvalidComponentMsg(n));
    }
    ...
    public void setComponent(final int n, final Complex z) {
        if (n >= 0 && n < N) {
            vectorRe[n] = z.real();
            vectorIm[n] = z.imag();
        } else throw new VectorDimensionException(getInvalidComponentMsg(n));
    }
    ...
    public double norm() {
        ...
        for (int i = 1; i < N; i++) answer += vectorRe[i] * vectorRe[i] + vectorIm[i] * vectorIm[i];
    ...
    public ComplexVector conjugate() {
        final double arrayIm[] = new double[N];
        arrayIm[0] = -vectorIm[0];
        for (int i = 1; i < N; i++) arrayIm[i] = -vectorIm[i];
    ...
    public AbelianGroup.Member add(final AbelianGroup.Member v) {
        if (v instanceof ComplexVector) return add((ComplexVector) v); else if (v instanceof DoubleVector) return add((DoubleVector) v); else if (v instanceof IntegerVector) return add((IntegerVector) v); else throw new IllegalArgumentException("Member class not recognised by this method.");
    }
        ...
        switch(v.storageFormat) {
            ...
            default:
                if (N == v.N) {
                    ...
                    return new ComplexVector(arrayRe, arrayIm);
                } else throw new VectorDimensionException("Vectors are different sizes.");
        }
    }
        ...
        switch(v.storageFormat) {
            ...
            default:
                if (N == v.N) {
                    ...
                    for (int i = 1; i < N; i++) arrayRe[i] = vectorRe[i] + v.getComponent(i);
    ...
    private ComplexVector rawAdd(final DoubleVector v) {
        if (N == v.N) {
            ...
            for (int i = 1; i < N; i++) arrayRe[i] = vectorRe[i] + v.vector[i];
            return new ComplexVector(arrayRe, vectorIm);
        } else throw new VectorDimensionException("Vectors are different sizes.");
    }
    ...
    public AbelianGroup.Member subtract(final AbelianGroup.Member v) {
        if (v instanceof ComplexVector) return subtract((ComplexVector) v); else if (v instanceof DoubleVector) return subtract((DoubleVector) v); else if (v instanceof IntegerVector) return subtract((IntegerVector) v); else throw new IllegalArgumentException("Member class not recognised by this method.");
    }
    ...
    public VectorSpace.Member scalarDivide(Field.Member x) {
        if (x instanceof Complex) return scalarDivide((Complex) x); else if (x instanceof MathDouble) return scalarDivide(((MathDouble) x).value()); else throw new IllegalArgumentException("Member class not recognised by this method.");
    }
    ...
    public Complex scalarProduct(HilbertSpace.Member v) {
        if (v instanceof ComplexVector) return scalarProduct((ComplexVector) v); else throw new IllegalArgumentException("Member class not recognised by this method.");
    }
    ...
    public ComplexVector mapComponents(final ComplexMapping f) {
        ...
        for (int i = 1; i < N; i++) array[i] = f.map(vectorRe[i], vectorIm[i]);
