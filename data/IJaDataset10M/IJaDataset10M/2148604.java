...
public class Permission extends PersistentEntity {
    ...
    public static Permission getPermission(Long id) throws ProgrammerException {
        try {
    ...
    static void methodsAllowed(User user, MonadUri uriPattern, List<Invocation.HttpMethod> methods) throws ProgrammerException, UserException {
        for (Invocation.HttpMethod method : methods) {
            if (!Chellow.methodAllowed(user, uriPattern.toUri(), method)) {
                throw UserException.newInvalidParameter("You can't assign greater permissions that you have.");
            }
        }
    }
    ...
    public Permission(Role role, MonadUri uriPattern, Boolean isOptionsAllowed, Boolean isGetAllowed, Boolean isHeadAllowed, Boolean isPostAllowed, Boolean isPutAllowed, Boolean isDeleteAllowed, Boolean isTraceAllowed) throws ProgrammerException, UserException {
        this();
        setRole(role);
        update(uriPattern, isOptionsAllowed, isGetAllowed, isHeadAllowed, isPostAllowed, isPutAllowed, isDeleteAllowed, isTraceAllowed);
    }

    public void update(MonadUri uriPattern, Boolean isOptionsAllowed, Boolean isGetAllowed, Boolean isHeadAllowed, Boolean isPostAllowed, Boolean isPutAllowed, Boolean isDeleteAllowed, Boolean isTraceAllowed) throws ProgrammerException {
        if (uriPattern == null) {
    ...
    public Element toXML(Document doc) throws ProgrammerException, UserException {
        ...
        element.setAttributeNode(MonadBoolean.toXml(doc, "is-post-allowed", isPostAllowed));
        element.setAttributeNode(MonadBoolean.toXml(doc, "is-get-allowed", isGetAllowed));
        return element;
    }
    ...
    public void httpPost(Invocation inv) throws ProgrammerException, UserException, DesignerException {
