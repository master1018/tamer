...
public class CmsNewResourceFolder extends A_CmsListResourceTypeDialog {
    ...
    public CmsNewResourceFolder(CmsJspActionElement jsp) {
        super(jsp, A_CmsListResourceTypeDialog.LIST_ID, Messages.get().container(Messages.GUI_NEWFOLDER_SELECT_INDEX_TYPE_0), A_CmsListResourceTypeDialog.LIST_COLUMN_NAME, CmsListOrderEnum.ORDER_ASCENDING, null);
    }
    ...
    public boolean actionCreateResource() throws JspException {
        try {
            ...
            List properties = CmsNewResource.createResourceProperties(getCms(), fullResourceName, CmsResourceTypeFolder.getStaticTypeName(), title);
    ...
    protected List getAvailableResTypes() {
        ...
        if (availableResTypes == null) {
            try {
                I_CmsResourceType resType = OpenCms.getResourceManager().getResourceType(CmsResourceTypeFolder.getStaticTypeId());
    ...
    protected List getListItems() {
        ...
        while (k.hasNext()) {
            ...
            if (set != null) {
                ...
                item.set(LIST_COLUMN_ICON, "<img src=\"" + getSkinUri() + "filetypes/" + set.getIcon() + "\" style=\"width: 16px; height: 16px;\" />");
                ret.add(item);
            }
        }
        return ret;
