...
public class IRCModeParser {
    ...
    private void parse(String modes, String args) {
        ...
        for (int i = 0, j = 0, n = 0; i < modesLen; i++) {
            c = modes.charAt(i);
            if (c == '+' || c == '-') operator = c; else {
                operatorsArr[n] = operator;
                modesArr[n] = c;
                if (c == 'o' || c == 'v' || c == 'b' || c == 'k' || c == 'l' && operator == '+') argsArr[n] = j < argsTmp.length ? argsTmp[j++] : ""; else argsArr[n] = "";
                n++;
            }
        }
    }
    ...
    private int getModesCount(String modes) {
        int count = 0;
        for (int i = 0, c, len = modes.length(); i < len; i++) if ((c = modes.charAt(i)) != '+' && c != '-') count++;
        return count;
    }
