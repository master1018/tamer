...
public class SearchDlg extends JDialog {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (actionCmd.equals("PacketSelected")) {
                ...
                for (Part part : _currentFormat.getDataFormat().getMainBlock().getParts()) {
                    if (part instanceof ForPart) continue;
    ...
    public int search(int startIndex) {
        ...
        if (gameSessionViewer == null) return -1;
                            ...
                            case 0:
                                if ((_currentPart.getType() != PartType.S) && (_currentPart.getType() != PartType.s)) {
                                    try {
                                        ...
                                        if (value == partValue) return i;
                                    } catch (NumberFormatException nfe) {
                                    }
                                } else {
                                    if (((StringValuePart) gp.getRootNode().getPartByName(_currentPart.getName())).getStringValue().equalsIgnoreCase(_valueField.getText())) return i;
                                }
                                break;
                            case 1:
                                if ((_currentPart.getType() != PartType.S) && (_currentPart.getType() != PartType.s)) {
