...
public class SpringBeanRouter extends Router implements ApplicationContextAware, BeanFactoryPostProcessor {
    ...
    private void attachAllResources(ListableBeanFactory beanFactory) {
        for (String beanName : getBeanNamesByType(org.restlet.resource.Resource.class, beanFactory)) {
    ...
    private String[] getBeanNamesByType(Class<?> beanClass, ListableBeanFactory beanFactory) {
        return isFindingInAncestors() ? BeanFactoryUtils.beanNamesForTypeIncludingAncestors(beanFactory, beanClass, true, true) : beanFactory.getBeanNamesForType(beanClass, true, true);
    }
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        if (getAttachments() != null) {
            for (Map.Entry<String, String> attachment : getAttachments().entrySet()) {
                ...
                if (org.restlet.resource.Resource.class.isAssignableFrom(beanType)) {
                    attachResource(uri, beanName, source);
                } else if (org.restlet.resource.ServerResource.class.isAssignableFrom(beanType)) {
                    attachResource(uri, beanName, source);
