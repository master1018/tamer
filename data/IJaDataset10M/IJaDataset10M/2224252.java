...
import net.sf.xws4j.actions.ActionPulldown;
import net.sf.xws4j.exceptions.Xws4jException;
import net.sf.xws4j.preferences.PreferenceConstants;
import net.sf.xws4j.views.servicediscovery.ServiceDiscoveryView;
...
public class DefaultClientCurator {
    ...
    private boolean isAccountDataSet() {
        if (preferences != null) {
            if (preferences.getString(PreferenceConstants.P_STRING_SERVER).length() > 0 && preferences.getString(PreferenceConstants.P_STRING_JID).length() > 0 && preferences.getString(PreferenceConstants.P_STRING_PASSWORD).length() > 0 && preferences.getString(PreferenceConstants.P_STRING_RESOURCE).length() > 0 && preferences.getString(PreferenceConstants.P_STRING_SERVERPORT).length() > 0) return true;
        }
        return false;
    }
    ...
    private boolean isAccountDataChanged() {
        if (preferences != null) {
            if (server.equals(preferences.getString(PreferenceConstants.P_STRING_SERVER)) && clientJID.equals(preferences.getString(PreferenceConstants.P_STRING_JID) + "/" + preferences.getString(PreferenceConstants.P_STRING_RESOURCE)) && pwd.equals(preferences.getString(PreferenceConstants.P_STRING_PASSWORD)) && server_port.equals(preferences.getString(PreferenceConstants.P_STRING_SERVERPORT))) return false;
        }
        return true;
    }
    ...
    public Client getDefaultClient() throws Xws4jException {
        ...
        if (!isAccountDataSet()) {
            XwsConsole.writeToConsoleBlueT("Could not create default client:" + " Invalid or missing XMPP account data in preferences.");
            throw new Xws4jException("Could not create default client:" + " Invalid or missing XMPP account data in preferences.");
        }
        clientJID = preferences.getString(PreferenceConstants.P_STRING_JID) + "/" + preferences.getString(PreferenceConstants.P_STRING_RESOURCE);
        ...
        try {
            default_client = new Client(clientJID, pwd, server, Integer.parseInt(server_port), exec);
        } catch (Exception e) {
        ...
        XwsConsole.writeToConsoleBlueT("Created client: JID=" + clientJID + " Server=" + server + ":" + server_port);
    ...
    public boolean isClientConnected() {
        if (default_client != null) return default_client.isConnected();
        return false;
    }

    public void connectClient(final boolean with_GUI_error) throws Xws4jException {
                ...
                try {
                    ...
                    if (with_GUI_error) {
                        Runnable r = new Runnable() {
                            ...
                            public void run() {
                                MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "XMPP Error", "Connecting to XMPP server failed." + System.getProperty("line.separator") + System.getProperty("line.separator") + error_s);
                            }
                        };
                        Display.getDefault().asyncExec(r);
                    }
