 ...
 * @version 1.0
 */
public class ZoomFitObjectsAction extends SketchAction {

    private static final long serialVersionUID = 1L;

    public ZoomFitObjectsAction(SketchManager owner) {
        super(owner, Messages.getString("ZoomFitObjectsAction.0"), getImageIcon(Messages.getString("ZoomFitObjectsAction.1")));
    }

    public void actionPerformed(ActionEvent e) {
        try {
            this.getOwner().getCommandManager().zoomFitObjects();
    ...
    public void selectionChanged(SketchSelectionManager selection) {
        BPMNPageGraphic page = selection.getPage();
        boolean bEnable = false;
        if (page != null) {
            bEnable = !page.getGraphicsStore().isEmpty();
        }
