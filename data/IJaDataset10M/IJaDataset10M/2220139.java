...
public class SetEmployedFlagFromMz {
    ...
    private static final Logger log = Logger.getLogger(SetEmployedFlagFromMz.class);
...
class MzEmployementData {
    ...
    private static final List<String> EMPLOYED_VALUES = Arrays.asList("1", "2", "3");

    private static final List<String> UNEMPLOYED_VALUES = Arrays.asList("4", "5", "6", "7", "9", "10");

    private static final List<String> EDUCATION_VALUES = Arrays.asList("8");

    private static final List<String> NO_ANSWER_VALUES = Arrays.asList("11", "-97");
    ...
    private void processMz(final String mzPersonFile) {
        ...
        for (int i = 0; i < line.length; i++) {
            if (line[i].equals(ID_FIELD)) idIndex = i; else if (line[i].equals(EMPLOYEMENT_FIELD)) emplIndex = i;
        }
        line = nextLine(reader);
        while (line != null) {
            count.incCounter();
            if (line.length != length) {
                throw new RuntimeException("found a line of " + line.length + "fields. " + length + " fields were expected.");
            }
            Id id = new IdImpl(line[idIndex]);
            employed.put(id, employement(line[emplIndex].trim()));
            student.put(id, education(line[emplIndex].trim()));
            line = nextLine(reader);
        }
        status.done();
    }
    ...
    private void processId(final String file) {
        ...
        Counter count = new Counter("importing line #");
        for (int i = 0; i < line.length; i++) {
            if (line[i].equals(WG_ID_FIELD)) idIndex = i; else if (line[i].equals(WG_ZP_FIELD)) zpIndex = i;
        }
        line = nextLine(reader);
        while (line != null) {
            count.incCounter();
            ...
            idToZpnr.put(new IdImpl(line[idIndex]), new IdImpl(line[zpIndex]));
            line = nextLine(reader);
        }
        status.done();
    }
    ...
    private Boolean employement(final String emplValue) {
        if (EMPLOYED_VALUES.contains(emplValue)) {
            return true;
        } else if (UNEMPLOYED_VALUES.contains(emplValue) || EDUCATION_VALUES.contains(emplValue)) {
            return false;
    ...
    private Boolean education(final String emplValue) {
        if (EDUCATION_VALUES.contains(emplValue)) {
            return true;
        } else if (UNEMPLOYED_VALUES.contains(emplValue) || EMPLOYED_VALUES.contains(emplValue)) {
            return false;
        } else if (NO_ANSWER_VALUES.contains(emplValue)) {
            return null;
        }
