package net.sourceforge.ondex.parser.kegg2.sink;
...
public class Concept implements Comparable<Concept> {
    ...
    private Set<ConceptName> conceptNames = new ObjectOpenHashSet<ConceptName>(4);

    private Set<ConceptAcc> conceptAccs = new ObjectOpenHashSet<ConceptAcc>(4);
 ...
 */
    ...
    public Concept clone(String new_id, String new_element_of, String new_of_type_fk) {
        ...
        if (this.description != null) newConcept.setDescription(this.getDescription());
        newConcept.setGraphical_x(this.getGraphical_x());
        newConcept.setGraphical_y(this.getGraphical_y());
        if (this.getTaxid() != null) newConcept.setTaxid(this.getTaxid());
        if (this.url != null) newConcept.setUrl(this.getUrl());
        if (this.getConceptAccs().size() > 0) {
            Iterator<ConceptAcc> it = this.conceptAccs.iterator();
            while (it.hasNext()) {
                ConceptAcc ca = it.next();
                ConceptAcc newCA = new ConceptAcc(new_id, ca.getConcept_accession(), ca.getElement_of());
                newCA.setAmbiguous(ca.isAmbiguous());
                newConcept.getConceptAccs().add(newCA);
            }
        }
        if (this.getConceptNames().size() > 0) {
            Iterator<ConceptName> it = this.getConceptNames().iterator();
            while (it.hasNext()) {
                ConceptName cn = it.next();
                ConceptName newCN = new ConceptName(new_id, cn.getName());
    ...
    public void setElement_of(String element_of) {
        if (element_of == null || element_of.length() == 0) throw new NullPointerException("ElementOf is null or empty");
        this.element_of = element_of.intern();
    }
