...
public class AstroFormat implements Constants {
    ...
    public static String dm(double value) {
        ...
        if (isNegative) {
            if (degree > 0) degree = (-degree); else if (minute > 0) minute = (-minute);
        }
        StringBuffer result = new StringBuffer().append(Format.format(degree, ' ', 4, 0, degreeSign)).append(Format.format(minute, '0', 2, 0, minuteSign));
    ...
    public static String dm(double value, String tag) {
        ...
        if (value == 0.0) ; else if (value > 0.0) text.insert(0, tag.charAt(0)); else {
    ...
    public static String dms(double value) {
        ...
        int second = (int) ((value - (double) minute) * 60.0);
        if (isNegative) {
            if (degree > 0) degree = (-degree); else if (minute > 0) minute = (-minute); else second = (-second);
        }
        StringBuffer result = new StringBuffer().append(Format.format(degree, ' ', 4, degreeSign)).append(Format.format(minute, '0', 2, minuteSign)).append(Format.format(second, '0', 4, secondSign));
        return (result.toString());
    }
    ...
    public static String hm(double value) {
        if (value == ABOVE_HORIZON) return ("above"); else if (value == BELOW_HORIZON) return ("below"); else {
            ...
            StringBuffer result = new StringBuffer().append(Format.format(hour, '0', 2, ':')).append(Format.format(minute, '0', 2, Format.NO_CHAR));
            return (result.toString());
        }
    }
    ...
    public static String hms(double value, int secondPrecision, boolean isCivil) {
        ...
        StringBuffer result = new StringBuffer().append(Format.format(hour, ' ', 2, hourChar)).append(Format.format(minute, '0', 2, minuteChar));
        if (secondPrecision == 0) {
    ...
    public static String timeZoneString(long timezone) {
        ...
        timezone /= 60000;
        if (timezone > 0) {
            result.append(Format.format(timezone / 60, '0', 2, 0, ':')).append(Format.format(timezone % 60, '0', 2)).append(" East");
        } else if (timezone == 0) result.append("00:00"); else {
            timezone = (-timezone);
            result.append(Format.format(timezone / 60, '0', 2, 0, ':')).append(Format.format(timezone % 60, '0', 2)).append(" West");
        }
        return (result.toString());
    }
}
