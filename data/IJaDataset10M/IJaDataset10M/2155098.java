...
abstract class SpartanInputReader extends AtomSetCollectionReader {
    ...
    protected void readInputRecords() {
        ...
        try {
            ...
            while (readLine() != null) {
                ...
                if (tokens.length == 2 && parseInt(tokens[0]) != Integer.MIN_VALUE && parseInt(tokens[1]) >= 0) break;
            }
            if (line == null) return;
            readInputAtoms();
            discardLinesUntilContains("ATOMLABELS");
            if (line != null) readAtomNames();
            discardLinesUntilContains("HESSIAN");
            if (line != null) readBonds(atomCount0);
            while (line != null && line.indexOf("END ") < 0 && line.indexOf("MOLSTATE") < 0) readLine();
            if (line != null && line.indexOf("MOLSTATE") >= 0) readTransform();
            if (atomSetCollection.getAtomCount() > 0) atomSetCollection.setAtomSetName(modelName);
        } catch (Exception e) {
            setError(e);
        }
    }
    ...
    private void readInputAtoms() throws Exception {
        ...
        if (Logger.debugging) Logger.debug(atomCount + " atoms read");
    }
    ...
    private void readBonds(int atomCount0) throws Exception {
        ...
        while (readLine() != null && !line.startsWith("ENDHESS")) {
            ...
            if (nAtoms == 0) {
                ...
                if (bondOrder > 0) {
                    atomSetCollection.addBond(new Bond(sourceIndex, targetIndex, bondOrder < 4 ? bondOrder : bondOrder == 5 ? JmolAdapter.ORDER_AROMATIC : 1));
                }
            } else {
        ...
        if (Logger.debugging) Logger.debug(bondCount + " bonds read");
    }
}
