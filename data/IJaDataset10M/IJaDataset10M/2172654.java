...
final class SWFTreeBuilder {
    ...
    static void addNode(DefaultMutableTreeNode node, SWFHeader header) {
        DefaultMutableTreeNode headerNode = addParentNode(node, "<html><id:1xf><font color=\"#00A000\">SWF Header</html>");
    ...
    private static String getPushDescription(Push push) {
        ...
        for (Iterator i = push.getValues().iterator(); i.hasNext(); ) {
            StackValue value = (StackValue) i.next();
    ...
    private static void addNode(DefaultMutableTreeNode node, MalformedTag tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, "<html><body bgcolor=\"#FF0000\"><font color=\"#FFFFFF\">Malformed tag</font></body></html>");
        short code = tag.getTagHeader().getCode();
        addLeaf(tagNode, "code: " + code + " (" + TagConstants.getTagName(code) + ")");
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineBits tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineBits"));
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineBitsJPEG2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineBitsJPEG2"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "jpegData: " + " byte[" + tag.getJpegData().length + "]");
    }

    private static void addNode(DefaultMutableTreeNode node, DefineBitsJPEG3 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineBitsJPEG3"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "jpegData: " + " byte[" + tag.getJpegData().length + "]");
        addLeaf(tagNode, "bitmapAlphaData: " + " byte[" + tag.getBitmapAlphaData().length + "]");
    }

    private static void addNode(DefaultMutableTreeNode node, DefineBitsLossless tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineBitsLossless"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        ...
        if (format == DefineBitsLossless.FORMAT_8_BIT_COLORMAPPED) {
            addNode(tagNode, "zlibBitmapData: ", (ColorMapData) tag.getZlibBitmapData());
        } else {
            addNode(tagNode, "zlibBitmapData: ", (BitmapData) tag.getZlibBitmapData(), format);
        }
    }
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, BitmapData data, short format) {
        ...
        if (format == DefineBitsLossless.FORMAT_15_BIT_RGB) {
            addLeaf(newNode, "bitmapPixelData: Pix15[" + data.getBitmapPixelData().length + "]");
        } else {
            addLeaf(newNode, "bitmapPixelData: Pix24[" + data.getBitmapPixelData().length + "]");
        }
    }

    private static void addNode(DefaultMutableTreeNode node, String var, ColorMapData data) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "ColorMapData");
        RGB[] colorTable = data.getColorTableRGB();
        DefaultMutableTreeNode colorTableNode = addParentNode(newNode, "colorTableRGB: RGB[" + colorTable.length + "]");
        for (int i = 0; i < colorTable.length; i++) {
            addLeaf(colorTableNode, colorTable[i].toString());
        }
        addLeaf(newNode, "colorMapPixelData: short[" + data.getColorMapPixelData().length + "]");
    }

    private static void addNode(DefaultMutableTreeNode node, String var, AlphaColorMapData data) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "AlphaColorMapData");
        RGBA[] colorTable = data.getColorTableRGBA();
        DefaultMutableTreeNode colorTableNode = addParentNode(newNode, "colorTableRGBA: RGBA[" + colorTable.length + "]");
        for (int i = 0; i < colorTable.length; i++) {
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineBitsLossless2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineBitsLossless2"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        short format = tag.getFormat();
        String formatString = "";
        switch(format) {
        ...
        addLeaf(tagNode, "height: " + tag.getHeight());
        if (format == DefineBitsLossless.FORMAT_8_BIT_COLORMAPPED) {
            addNode(tagNode, "zlibBitmapData: ", (AlphaColorMapData) tag.getZlibBitmapData());
        } else {
            DefaultMutableTreeNode zlibNode = addParentNode(tagNode, "zlibBitmapData: AlphaBitMapData");
            addLeaf(zlibNode, "bitmapPixelData: RGBA[" + ((AlphaBitmapData) (tag.getZlibBitmapData())).getBitmapPixelData().length + "]");
        }
    }

    private static void addNode(DefaultMutableTreeNode node, DefineButton tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineButton"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineButton2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineButton2"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, ButtonCondAction[] actions) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "ButtonCondAction[" + actions.length + "]");
        for (int i = 0; i < actions.length; i++) {
            DefaultMutableTreeNode actionNode = addParentNode(newNode, "ButtonCondAction");
            addLeaf(actionNode, "idleToOverDown: " + actions[i].isIdleToOverDown());
            addLeaf(actionNode, "outDownToIdle: " + actions[i].isOutDownToIdle());
            addLeaf(actionNode, "outDownToOverDown: " + actions[i].isOutDownToOverDown());
            addLeaf(actionNode, "overDownToOutDown: " + actions[i].isOverDownToOutDown());
            addLeaf(actionNode, "overDownToOverUp: " + actions[i].isOverDownToOverUp());
            addLeaf(actionNode, "overUpToOverDown: " + actions[i].isOverUpToOverDown());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, ButtonRecord[] characters) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "ButtonRecord[" + characters.length + "]");
        for (int i = 0; i < characters.length; i++) {
            DefaultMutableTreeNode recordNode = addParentNode(newNode, "ButtonRecord");
            ...
            if (colorTransform != null) {
                addNode(recordNode, "colorTransform: ", characters[i].getColorTransform());
            }
        }
    }

    private static void addNode(DefaultMutableTreeNode node, DefineButtonCXform tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineButtonCXform"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineButtonSound tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineButtonSound"));
        ...
        if (tag.getOverUpToIdleSoundId() != 0) {
            ...
            addNode(tagNode, "overUpToIdleSoundInfo: ", tag.getOverUpToIdleSoundInfo());
        }
        if (tag.getIdleToOverUpSoundId() != 0) {
            addLeaf(tagNode, "idleToOverUpSoundId: " + tag.getIdleToOverUpSoundId());
            addNode(tagNode, "idleToOverUpSoundInfo: ", tag.getIdleToOverUpSoundInfo());
        }
        if (tag.getOverUpToOverDownSoundId() != 0) {
            addLeaf(tagNode, "overUpToOverDownSoundId: " + tag.getOverUpToOverDownSoundId());
            addNode(tagNode, "overUpToOverDownSoundInfo: ", tag.getOverUpToOverDownSoundInfo());
        }
        if (tag.getOverDownToOverUpSoundId() != 0) {
            addLeaf(tagNode, "overDownToOverUpSoundId: " + tag.getOverDownToOverUpSoundId());
            addNode(tagNode, "overDownToOverUpSoundInfo: ", tag.getOverDownToOverUpSoundInfo());
        }
    }

    private static void addNode(DefaultMutableTreeNode node, DefineEditText tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineEditText"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFont tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineFont"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        Shape[] shapes = tag.getGlyphShapeTable();
        DefaultMutableTreeNode shapeNode = addParentNode(tagNode, "glyphShapeTable: Shape[" + shapes.length + "]");
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFont2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineFont2"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        ...
        if (shapes != null) {
            ...
            DefaultMutableTreeNode codeTableNode = addParentNode(tagNode, "codeTable: char[" + table.length + "]");
        ...
        if (tag.hasLayout()) {
            ...
            if (shapes != null) {
                ...
                DefaultMutableTreeNode advanceTableNode = addParentNode(tagNode, "advanceTable: short[" + advanceTable.length + "]");
                for (int i = 0; i < advanceTable.length; i++) {
                    addLeaf(advanceTableNode, i + ": " + advanceTable[i]);
                }
                Rect[] boundsTable = tag.getBoundsTable();
                DefaultMutableTreeNode boundsTableNode = addParentNode(tagNode, "boundsTable: Rect[" + boundsTable.length + "]");
            ...
            if ((kerningTable != null) && (kerningTable.length > 0)) {
                DefaultMutableTreeNode kerningTableNode = addParentNode(tagNode, "kerningTable: KerningRecord[" + kerningTable.length + "]");
                for (int i = 0; i < kerningTable.length; i++) {
                    KerningRecord kerningRecord = kerningTable[i];
                    DefaultMutableTreeNode kerningRecordNode = addParentNode(kerningTableNode, "KerningRecord");
                    addLeaf(kerningRecordNode, "left: " + kerningRecord.getLeft());
                    addLeaf(kerningRecordNode, "right: " + kerningRecord.getRight());
                    addLeaf(kerningRecordNode, "adjustment: " + kerningRecord.getAdjustment());
                }
            }
        }
    }
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFont3 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineFont3"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "smallText: " + tag.isSmallText());
        addLeaf(tagNode, "italic: " + tag.isItalic());
        addLeaf(tagNode, "bold: " + tag.isBold());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFontInfo tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineFontInfo"));
        addLeaf(tagNode, "fontId: " + tag.getFontId());
        addLeaf(tagNode, "fontName: " + tag.getFontName());
        addLeaf(tagNode, "smallText: " + tag.isSmallText());
        addLeaf(tagNode, "shiftJIS: " + tag.isShiftJIS());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFontInfo2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineFontInfo2"));
        addLeaf(tagNode, "fontId: " + tag.getFontId());
        addLeaf(tagNode, "fontName: " + tag.getFontName());
        addLeaf(tagNode, "smallText: " + tag.isSmallText());
        addLeaf(tagNode, "italic: " + tag.isItalic());
    ...
    private static void addNode(DefaultMutableTreeNode node, FlashTypeSettings tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("FlashTypeSettings"));
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFontAlignment tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineFontAlignment"));
        addLeaf(tagNode, "fontId: " + tag.getFontId());
    ...
    private static void addNode(DefaultMutableTreeNode node, AlignmentZone[] alignmentZones) {
        DefaultMutableTreeNode newNode = addParentNode(node, "alignmentZones: AlignmentZone[" + alignmentZones.length + "]");
        for (int i = 0; i < alignmentZones.length; i++) {
            AlignmentZone zone = alignmentZones[i];
            DefaultMutableTreeNode zoneNode = addParentNode(newNode, "AlignmentZone " + i);
            if (zone.hasX()) {
                addLeaf(zoneNode, "x: left=" + zone.getLeft() + " width=" + zone.getWidth());
            }
            if (zone.hasY()) {
                addLeaf(zoneNode, "y: baseline=" + zone.getBaseline() + " height=" + zone.getHeight());
            }
        }
    }

    private static void addNode(DefaultMutableTreeNode node, DefineMorphShape tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineMorphShape"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineMorphShape2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineMorphShape2"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, Shape shape) {
        ...
        if (shapeRecords.length > 0) {
            DefaultMutableTreeNode recordsNode = addParentNode(newNode, "shapeRecords: ShapeRecord[" + shapeRecords.length + "]");
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, MorphLineStyles morphLineStyles) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "MorphLineStyles (" + morphLineStyles.getSize() + " styles)");
        for (int i = 1; i <= morphLineStyles.getSize(); i++) {
            DefaultMutableTreeNode styleNode = addParentNode(newNode, "MorphLineStyle " + i);
    ...
    private static void addNode(DefaultMutableTreeNode styleNode, MorphLineStyle2 style) {
        addLeaf(styleNode, "startWidth: " + style.getStartWidth());
        addLeaf(styleNode, "endWidth: " + style.getEndWidth());
        addLeaf(styleNode, "startCapStyle: " + getCapStyleString(style.getStartCapStyle()));
        addLeaf(styleNode, "endCapStyle: " + getCapStyleString(style.getEndCapStyle()));
        ...
        addLeaf(styleNode, "scaleStroke: " + getScaleStrokeString(style.getScaleStroke()));
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, MorphFillStyles morphFillStyles) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "MorphFillStyles (" + morphFillStyles.getSize() + " styles)");
    ...
    private static void addNode(DefaultMutableTreeNode node, MorphFillStyle fillStyle, int index) {
        DefaultMutableTreeNode newNode = addParentNode(node, "MorphFillStyle " + index);
        ...
        switch(type) {
            ...
            case MorphFillStyle.TYPE_LINEAR_GRADIENT:
                ...
                addLeaf(newNode, "endGradientMatrix: " + fillStyle.getStartGradientMatrix());
    ...
    private static void addNode(DefaultMutableTreeNode node, MorphGradient gradient) {
        DefaultMutableTreeNode morphGradNode = addParentNode(node, ((gradient instanceof FocalMorphGradient) ? "FocalMorphGradient" : "MorphGradient"));
        addLeaf(morphGradNode, "spreadMethod: " + getSpreadMethodString(gradient.getSpreadMethod()));
        addLeaf(morphGradNode, "interpolationMethod: " + getInterpolationMethodString(gradient.getInterpolationMethod()));
        if (gradient instanceof FocalMorphGradient) {
            FocalMorphGradient focalMorphGradient = (FocalMorphGradient) gradient;
            addLeaf(morphGradNode, "startFocalPointRatio: " + (focalMorphGradient).getStartFocalPointRatio());
            addLeaf(morphGradNode, "endFocalPointRatio: " + (focalMorphGradient).getEndFocalPointRatio());
        }
        MorphGradRecord[] records = gradient.getGradientRecords();
        for (int i = 0; i < records.length; i++) {
            DefaultMutableTreeNode recordNode = addParentNode(morphGradNode, "MorphGradRecord");
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineShape tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineShape"));
        addLeaf(tagNode, "characterID: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineShape2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineShape2"));
        addLeaf(tagNode, "characterID: " + tag.getCharacterId());
        addLeaf(tagNode, "shapeBounds: " + tag.getShapeBounds());
        addNode(tagNode, "shapes: ", tag.getShapes());
    }
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineShape3 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineShape3"));
        addLeaf(tagNode, "characterID: " + tag.getCharacterId());
        addLeaf(tagNode, "shapeBounds: " + tag.getShapeBounds());
        addNode(tagNode, "shapes: ", tag.getShapes());
    }
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineShape4 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineShape4"));
        addLeaf(tagNode, "characterID: " + tag.getCharacterId());
        addLeaf(tagNode, "shapeBounds: " + tag.getShapeBounds());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, ShapeWithStyle shapeWithStyle) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "ShapeWithStyle");
    ...
    private static void addNode(DefaultMutableTreeNode node, StyleChangeRecord record) {
        ...
        if (record.hasNewStyles()) {
            DefaultMutableTreeNode newStylesNode = addParentNode(newNode, "NewStyles");
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, FillStyleArray fillStyleArray) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "FillStyleArray (" + fillStyleArray.getSize() + " styles)");
    ...
    private static void addNode(DefaultMutableTreeNode node, LineStyle2 lineStyle, int index) {
        ...
        addLeaf(newNode, "width: " + lineStyle.getWidth());
        addLeaf(newNode, "startCapStyle: " + getCapStyleString(lineStyle.getStartCapStyle()));
        addLeaf(newNode, "endCapStyle: " + getCapStyleString(lineStyle.getEndCapStyle()));
        ...
        addLeaf(newNode, "scaleStroke: " + getScaleStrokeString(lineStyle.getScaleStroke()));
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, Gradient gradient) {
        DefaultMutableTreeNode newNode = addParentNode(node, ((gradient instanceof FocalGradient) ? "FocalGradient" : "Gradient"));
        addLeaf(newNode, "spreadMethod: " + getSpreadMethodString(gradient.getSpreadMethod()));
        addLeaf(newNode, "interpolationMethod: " + getInterpolationMethodString(gradient.getInterpolationMethod()));
        if (gradient instanceof FocalGradient) {
            addLeaf(newNode, "focalPointRatio: " + ((FocalGradient) gradient).getFocalPointRatio());
        }
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, LineStyleArray lineStyleArray) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "LineStyleArray (" + lineStyleArray.getSize() + " styles)");
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineSound tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineSound"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        byte format = tag.getFormat();
        addLeaf(tagNode, "format: " + format + " (" + getSoundFormatString(format) + ")");
        ...
        addLeaf(tagNode, "soundData: " + " byte[" + tag.getSoundData().length + "]");
    }

    private static void addNode(DefaultMutableTreeNode node, DefineSprite tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineSprite"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "frameCount: " + tag.getFrameCount());
        List ctrlTags = tag.getControlTags();
        DefaultMutableTreeNode ctrlTagNode = addParentNode(tagNode, "controlTags: Tag[" + ctrlTags.size() + "]");
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineText tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineText"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineText2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineText2"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "textBounds: " + tag.getTextBounds());
        addLeaf(tagNode, "textMatrix: " + tag.getTextMatrix());
        addNode(tagNode, "textRecords: ", tag.getTextRecords());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, TextRecord[] textRecords) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "TextRecord[" + textRecords.length + "]");
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, GlyphEntry[] glyphEntries) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "GlyphEntry[" + glyphEntries.length + "]");
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineVideoStream tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("DefineVideoStream"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, DoAction tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("DoAction"));
        addNode(tagNode, "actions: ", tag.getActions());
    }

    private static void addNode(DefaultMutableTreeNode node, DoInitAction tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("DoInitAction"));
    ...
    private static void addNode(DefaultMutableTreeNode node, EnableDebugger tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("EnableDebugger"));
    ...
    private static void addNode(DefaultMutableTreeNode node, EnableDebugger2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("EnableDebugger2"));
        String password = tag.getPassword();
        if (password == null) {
            addLeaf(tagNode, "No password");
        } else {
    ...
    private static void addNode(DefaultMutableTreeNode node, ExportAssets tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("ExportAssets"));
        ExportMapping[] exportMappings = tag.getExportMappings();
        for (int i = 0; i < exportMappings.length; i++) {
            addLeaf(tagNode, "characterId: " + exportMappings[i].getCharacterId() + ", name: " + exportMappings[i].getName());
        }
    }

    private static void addNode(DefaultMutableTreeNode node, FileAttributes tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("FileAttributes"));
    ...
    private static void addNode(DefaultMutableTreeNode node, FrameLabel tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("FrameLabel"));
    ...
    private static void addNode(DefaultMutableTreeNode node, ImportAssets tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag((tag instanceof ImportAssets2) ? "ImportAssets2" : "ImportAssets"));
        addLeaf(tagNode, "url: " + tag.getUrl());
        ImportMapping[] importMappings = tag.getImportMappings();
        DefaultMutableTreeNode mappingsNode = addParentNode(tagNode, "importMappings[" + importMappings.length + "]");
        for (int i = 0; i < importMappings.length; i++) {
            addLeaf(mappingsNode, "name: " + importMappings[i].getName() + ", characterId: " + importMappings[i].getCharacterId());
        }
    }

    private static void addNode(DefaultMutableTreeNode node, JPEGTables tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatDefTag("JPEGTables"));
        addLeaf(tagNode, "jpegData: " + " byte[" + tag.getJpegData().length + "]");
    }

    private static void addNode(DefaultMutableTreeNode node, Metadata tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("Metadata"));
        addLeaf(tagNode, "data: " + tag.getDataString());
    }

    private static void addNode(DefaultMutableTreeNode node, PlaceObject tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("PlaceObject"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, CXformWithAlpha xform) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "CXformWithAlpha");
    ...
    private static void addNode(DefaultMutableTreeNode node, PlaceObject2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("PlaceObject2"));
        addLeaf(tagNode, "depth: " + tag.getDepth());
    ...
    private static void addNode(DefaultMutableTreeNode node, PlaceObject3 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("PlaceObject3"));
        addLeaf(tagNode, "depth: " + tag.getDepth());
        addLeaf(tagNode, "move: " + tag.isMove());
        if (tag.hasCharacter()) {
            addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        ...
        if (tag.hasFilters()) {
            ...
            DefaultMutableTreeNode filtersNode = addParentNode(tagNode, "filters: Filter[" + count + "]");
    ...
    private static void addNode(DefaultMutableTreeNode node, Filter filter) {
        ...
        if (filter instanceof ColorMatrixFilter) {
            ...
            addLeaf(filterNode, "preserveAlpha: " + convolutionFilter.isPreserveAlpha());
            ...
            DefaultMutableTreeNode controlPointsNode = addParentNode(filterNode, "control points[" + controlPointsCount + "]");
            for (int i = 0; i < controlPointsCount; i++) {
                addLeaf(controlPointsNode, "color " + i + ": " + colors[i] + " ratio: " + ratios[i]);
            }
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, ActionBlockReader actionBlock) {
        List actionRecords = actionBlock.getActions();
        DefaultMutableTreeNode newNode = addParentNode(node, var + actionRecords.size() + " actions; size : " + actionBlock.getSize());
    ...
    private static void addNode(DefaultMutableTreeNode node, Action action) {
        ...
        switch(action.getCode()) {
            ...
            case ActionConstants.IF:
                actionDescription += ("If branchLabel: <code>" + ((If) action).getBranchLabel() + "</code> " + "branchOffset: " + ((If) action).getBranchOffset());
                break;
            case ActionConstants.JUMP:
                actionDescription += ("Jump branchLabel: <code>" + ((Jump) action).getBranchLabel() + "</code> " + "branchOffset: " + ((Jump) action).getBranchOffset());
                break;
            default:
    ...
    private static void addNode(DefaultMutableTreeNode node, DefineFunction2 defineFunction2) {
        ...
        DefaultMutableTreeNode paramsNode = addParentNode(headerNode, "parameters: RegisterParam[" + regParameters.length + "]");
        for (int i = 0; i < regParameters.length; i++) {
            DefaultMutableTreeNode paramNode = addParentNode(paramsNode, "RegisterParam");
        ...
        addLeaf(headerNode, "suppressArguments: " + defineFunction2.suppressesArguments());
        addLeaf(headerNode, "preloadArguments: " + defineFunction2.preloadsArguments());
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, ClipEventFlags clipEventFlags) {
        DefaultMutableTreeNode newNode = addParentNode(node, var + "ClipEventFlags");
    ...
    private static void addNode(DefaultMutableTreeNode node, Protect tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("Protect"));
        String password = tag.getPassword();
        if (password == null) {
            addLeaf(tagNode, "No password");
        } else {
    ...
    private static void addNode(DefaultMutableTreeNode node, RemoveObject tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("RemoveObject"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "depth: " + tag.getDepth());
    }

    private static void addNode(DefaultMutableTreeNode node, RemoveObject2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("RemoveObject2"));
        addLeaf(tagNode, "depth: " + tag.getDepth());
    }

    private static void addNode(DefaultMutableTreeNode node, ScriptLimits tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("ScriptLimits"));
    ...
    private static void addNode(DefaultMutableTreeNode node, SetBackgroundColor tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("SetBackgroundColor"));
        addLeaf(tagNode, "color: " + tag.getColor());
    }

    private static void addNode(DefaultMutableTreeNode node, SetTabIndex tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("SetTabIndex"));
        addLeaf(tagNode, "depth: " + tag.getDepth());
    ...
    private static void addNode(DefaultMutableTreeNode node, SoundStreamBlock tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("SoundStreamBlock"));
        addLeaf(tagNode, "streamSoundData: byte[" + tag.getStreamSoundData().length + "]");
    }

    private static void addNode(DefaultMutableTreeNode node, Scale9Grid tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("Scale9Grid"));
        addLeaf(tagNode, "characterId: " + tag.getCharacterId());
        addLeaf(tagNode, "grid: " + tag.getGrid());
    }

    private static void addNode(DefaultMutableTreeNode node, SoundStreamHead tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("SoundStreamHead"));
    ...
    private static void addNode(DefaultMutableTreeNode node, SoundStreamHead2 tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("SoundStreamHead2"));
        byte rate = tag.getPlaybackRate();
        addLeaf(tagNode, "playbackRate: " + getSoundRateString(rate));
    ...
    private static void addNode(DefaultMutableTreeNode node, StartSound tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("StartSound"));
    ...
    private static void addNode(DefaultMutableTreeNode node, String var, SoundInfo info) {
        ...
        if (info.getEnvelopeRecords() != null) {
            ...
            DefaultMutableTreeNode envRecordsNode = addParentNode(siNode, "envelopeRecords: SoundEnvelope[" + records.length + "]");
            for (int i = 0; i < records.length; i++) {
                SoundEnvelope env = records[i];
                DefaultMutableTreeNode recNode = addParentNode(envRecordsNode, "SoundEnvelope");
    ...
    private static void addNode(DefaultMutableTreeNode node, UnknownTag tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("Unknown tag (" + tag.getCode() + ")"));
    ...
    private static void addNode(DefaultMutableTreeNode node, VideoFrame tag) {
        DefaultMutableTreeNode tagNode = addParentNode(node, formatControlTag("VideoFrame"));
