 ...
 */
public class AddContactDialog extends SIPCommDialog implements ActionListener {
     ...
     */
    public AddContactDialog(MainFrame mainFrame, MetaContact metaContact, ProtocolProviderService pps) {
        super(mainFrame);
     ...
     * @param pps The <tt>ProtocolProviderService</tt>.
     */
    public AddContactDialog(MainFrame mainFrame, MetaContactGroup group, ProtocolProviderService pps) {
        super(mainFrame);
        this.mainFrame = mainFrame;
        this.clist = mainFrame.getContactList();
        this.group = group;
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            if (errorCode == MetaContactListException.CODE_CONTACT_ALREADY_EXISTS_ERROR) {
                                new ErrorDialog(mainFrame, Messages.getI18NString("addContactErrorTitle").getText(), Messages.getI18NString("addContactExistError", new String[] { uin }).getText(), ex).showDialog();
                            } else if (errorCode == MetaContactListException.CODE_LOCAL_IO_ERROR) {
                                new ErrorDialog(mainFrame, Messages.getI18NString("addContactErrorTitle").getText(), Messages.getI18NString("addContactError", new String[] { uin }).getText(), ex).showDialog();
