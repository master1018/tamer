...
public class AssemblyTable extends AbstractTable {
    ...
    public static final int TABLE_INDEX = 0x20;

    public static final int MIN_ROW_SIZE = 0x16;
    ...
    protected int readRow(byte[] data, int offset, int row) {
        int rowSize = offset;
        hashAlgId[row] = ByteReader.getUInt32(data, offset);
        offset += 4;
        assert (hashAlgId[row] == 0 || hashAlgId[row] == 0x8003 || hashAlgId[row] == 0x8004);
        majorVersion[row] = ByteReader.getUInt16(data, offset);
        offset += 2;
        minorVersion[row] = ByteReader.getUInt16(data, offset);
        offset += 2;
        buildNumber[row] = ByteReader.getUInt16(data, offset);
        offset += 2;
        revisionNumber[row] = ByteReader.getUInt16(data, offset);
        offset += 2;
        assemblyFlags[row] = ByteReader.getUInt32(data, offset);
        offset += 4;
