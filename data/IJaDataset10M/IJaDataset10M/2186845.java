...
public class FlowsmapGraphAggLayersTest {
    ...
    public void setup() {
        ...
        builder.addAggregationLayer("src-to-all", "src-node", FlowMapGraphEdgeAggregator.GroupFunctions.MERGE_ALL);
        builder.addAggregationLayer("target-to-all", "target-node", FlowMapGraphEdgeAggregator.GroupFunctions.MERGE_ALL);
    ...
    public void test_expandSource_expandBothTarget_collapseOneTarget_collapseSourceOverLevel() {
        ...
        assertEquals(ImmutableSet.of("1->2: 200.0", "1->3: 300.0", "1->4: 400.0", "2->1: 100.0", "2->4: 200.0"), serializeVisibleEdges(layers));
        layers.collapseTarget(e_target1_1);
        assertEquals(ImmutableSet.of("1->2: 200.0", "1->3: 300.0", "1->4: 400.0", "2->1,4: 300.0"), serializeVisibleEdges(layers));
    ...
    private static Set<String> serializeVisibleEdges(FlowMapGraphAggLayers ft) {
        ...
        for (Edge edge : ft.getEdges()) {
            set.add(ft.getSourceNodeId(edge) + "->" + ft.getTargetNodeId(edge) + ": " + ft.getEdgeWeight(edge, "value"));
        }
        return set;
    }
}
