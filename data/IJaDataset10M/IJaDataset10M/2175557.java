...
public class GSSCredentialImpl implements GSSCredential {
    ...
    public void add(GSSName name, int initLifetime, int acceptLifetime, Oid mech, int usage) throws GSSException {
        ...
        if (credentials.containsKey(credentialType)) {
            throw new GSSException(GSSException.DUPLICATE_ELEMENT, GSSUtils.DEFAULT_GSSEXCEPTION_MINOR_CODE, mech + " " + usage);
        }
        GSSCredentialElement credentialElement = managerImpl.createCredentialElement(name, initLifetime, acceptLifetime, mech, usage);
    ...
    public GSSName getName(Oid mech) throws GSSException {
        ...
        for (Entry<GSSCredentialType, GSSCredentialElement> entry : credentials.entrySet()) {
        ...
        if (null == credential) {
            throw new GSSException(GSSException.BAD_MECH, GSSUtils.DEFAULT_GSSEXCEPTION_MINOR_CODE, "fail to get name for " + mech);
        }
    ...
    public int getRemainingAcceptLifetime(Oid mech) throws GSSException {
        ...
        credential = credentials.get(new GSSCredentialType(mech, GSSCredential.INITIATE_ONLY));
        if (credential != null) {
            remainingAcceptLifetime = credential.getRemainingAcceptLifetime();
        }
        GSSCredentialElement tempCredential = credentials.get(new GSSCredentialType(mech, GSSCredential.INITIATE_AND_ACCEPT));
        if (tempCredential != null) {
            credential = tempCredential;
            remainingAcceptLifetime = Math.max(remainingAcceptLifetime, credential.getRemainingAcceptLifetime());
        }
        if (credential == null) {
            throw new GSSException(GSSException.BAD_MECH, GSSUtils.DEFAULT_GSSEXCEPTION_MINOR_CODE, "no credential for mech " + mech);
        }
    ...
    public int getRemainingInitLifetime(Oid mech) throws GSSException {
        ...
        if (tempCredential != null) {
            credential = tempCredential;
            remainingInitLifetime = Math.max(remainingInitLifetime, credential.getRemainingInitLifetime());
        }
        if (credential == null) {
            throw new GSSException(GSSException.BAD_MECH, GSSUtils.DEFAULT_GSSEXCEPTION_MINOR_CODE, "no credential for mech " + mech);
        }
    ...
    public int getRemainingLifetime() throws GSSException {
        ...
        for (Entry<GSSCredentialType, GSSCredentialElement> credential : credentials.entrySet()) {
            ...
            switch(credentialType.usage) {
                case GSSCredential.INITIATE_ONLY:
                    credentialRemainingLifeTime = credentialElement.getRemainingInitLifetime();
                    break;
                case GSSCredential.ACCEPT_ONLY:
                    credentialRemainingLifeTime = credentialElement.getRemainingAcceptLifetime();
                    break;
                default:
                    credentialRemainingLifeTime = Math.min(credentialElement.getRemainingInitLifetime(), credentialElement.getRemainingAcceptLifetime());
                    break;
            }
            remainingLifeTime = Math.min(remainingLifeTime, credentialRemainingLifeTime);
        }
    ...
    public int getUsage() throws GSSException {
        ...
        throw new GSSException(GSSException.FAILURE, GSSUtils.DEFAULT_GSSEXCEPTION_MINOR_CODE, "no credential element in this credential");
    }
    ...
    private void checkDisposed() throws GSSException {
        if (disposed) {
            throw new GSSException(GSSUtils.DEFAULT_GSSEXCEPTION_MAJOR_CODE, GSSUtils.DEFAULT_GSSEXCEPTION_MINOR_CODE, "credential disposed");
        }
    }
