...
public class View {
    ...
    public static final double MAX_ZOOM_OUT = 1.0d / 16.0d;
    ...
    public View(int w, int h, double zoomFactor, double viewWorldX, double viewWorldY) {
        ...
        visibleWorld = new DoubleRectangle();
        centerViewPoint();
    }

    public View(int w, int h) {
        this(w, h, NO_ZOOM, 0.0d, 0.0d);
    }

    public View(int w, int h, double zoomFactor) {
        this(w, h, zoomFactor, 0.0d, 0.0d);
    }
    ...
    public void pan(int viewXDistance, int viewYDistance) {
        setWorldViewPoint(viewCenterWorldX - convertXDistanceViewToWorld(viewXDistance), viewCenterWorldY - convertYDistanceViewToWorld(viewYDistance));
    }
    ...
    public boolean setXFitZoom(double xDistanceToFit) {
        ...
        setZoomFactor((double) viewWidth / (Math.abs(xDistanceToFit) * WYSIWYGScaleX));
        return (zoomFactor != oldZoomFactor);
    }

    public boolean setYFitZoom(double yDistanceToFit) {
        double oldZoomFactor = zoomFactor;
        setZoomFactor((double) viewHeight / (Math.abs(yDistanceToFit) * WYSIWYGScaleY));
        return (zoomFactor != oldZoomFactor);
    }

    public boolean setXYFitZoom(double xDistanceToFit, double yDistanceToFit) {
        double oldZoomFactor = zoomFactor;
        setZoomFactor(Math.min((double) viewWidth / (Math.abs(xDistanceToFit) * WYSIWYGScaleX), (double) viewHeight / (Math.abs(yDistanceToFit) * WYSIWYGScaleY)));
        return (zoomFactor != oldZoomFactor);
    }
