...
public class ItemService {
    ...
    public static void splitItem(Player player, int itemObjId, long splitAmount, int slotNum, int sourceStorageType, int destinationStorageType) {
        ...
        if (itemToSplit.getItemTemplate().isKinah()) {
            if (!decreaseKinah(player, sourceStorage, splitAmount)) return;
    ...
    public static boolean removeItem(Player player, Storage storage, Item item, boolean persist, boolean sendPacket) {
        ...
        if (sendPacket) sendDeleteItemPacket(player, storage.getStorageType(), item.getObjectId());
        return true;
    }
    ...
    public static void mergeItems(Player player, int sourceItemObjId, long itemAmount, int destinationObjId, int sourceStorageType, int destinationStorageType) {
        ...
        if (sourceItemObjId == destinationObjId) return;
        Storage sourceStorage = player.getStorage(sourceStorageType);
        Storage destinationStorage = player.getStorage(destinationStorageType);
        ...
        if (sourceItem == null || destinationItem == null) return;
        if (sourceItem.getItemTemplate().getTemplateId() != destinationItem.getItemTemplate().getTemplateId()) return;
        if (sourceItem.getItemCount() < itemAmount) return;
    ...
    public static boolean addItem(Player player, int itemId, long count) {
        if (count < 1) return false;
        if (OptionsConfig.LOG_ITEM) log.info(String.format("[ITEM] ID/Count - %d/%d to player %s.", itemId, count, player.getName()));
    ...
    public static void moveItem(Player player, int itemObjId, int sourceStorageType, int destinationStorageType, int slot) {
        ...
        Storage destinationStorage = player.getStorage(destinationStorageType);
        if (ItemService.removeItem(player, sourceStorage, item, false)) ItemService.addFullItem(player, destinationStorage, item);
    }

    public static void addItemPacket(Player player, int storageType, Item item) {
        if (storageType == StorageType.CUBE.getId()) PacketSendUtility.sendPacket(player, new SM_ADD_ITEMS(Collections.singletonList(item))); else PacketSendUtility.sendPacket(player, new SM_WAREHOUSE_UPDATE(item, storageType));
    }

    private static void sendDeleteItemPacket(Player player, int storageType, int itemObjId) {
        if (storageType == StorageType.CUBE.getId()) PacketSendUtility.sendPacket(player, new SM_DELETE_ITEM(itemObjId)); else PacketSendUtility.sendPacket(player, new SM_DELETE_WAREHOUSE_ITEM(storageType, itemObjId));
    }

    private static void sendUpdateItemPacket(Player player, int storageType, Item item) {
        if (storageType == StorageType.CUBE.getId()) PacketSendUtility.sendPacket(player, new SM_UPDATE_ITEM(item)); else PacketSendUtility.sendPacket(player, new SM_UPDATE_WAREHOUSE_ITEM(item, storageType));
    }
    ...
    public static ManaStone addManaStone(Item item, int itemId) {
        ...
        if (manaStones.size() > 6) return null;
        int nextSlot = 0;
        ...
        if (!slotFound) nextSlot = manaStones.size();
    ...
    public static void removeManastone(Player player, int itemObjId, int slotNum) {
        ...
        if (itemStones.size() <= slotNum) return;
        int counter = 0;
    ...
    public static void socketGodstone(Player player, int weaponId, int stoneId) {
        ...
        if (!ItemService.decreaseKinah(player, socketPrice)) return;
    ...
    public static boolean addItems(Player player, List<QuestItems> questItems) {
        ...
        for (QuestItems qi : questItems) {
            if (qi.getItemId() != ItemId.KINAH.value() && qi.getCount() != 0) {
                ...
                if (qi.getCount() % stackCount != 0) count++;
                needSlot += count;
            }
        }
        ...
        for (QuestItems qi : questItems) addItem(player, qi.getItemId(), qi.getCount());
        return true;
    }
