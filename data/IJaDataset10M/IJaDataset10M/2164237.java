   ...
   */
     ...
     */
        ...
        public void replaceValue(int from, int to) {
            ...
            if (locals != null) for (int i = 0; i < locals.length; i++) if (locals[i] == from) locals[i] = to;
        }

        public boolean hasValue(int val) {
            if (stack != null) for (int i = 0; i < stackHeight; i++) if (stack[i] == val) return true;
            if (locals != null) for (int i = 0; i < locals.length; i++) if (locals[i] == val) return true;
            return false;
        }
        ...
        boolean stateEquals(MachineState exit) {
            ...
            if (locals == null) {
                ...
                if (exit.locals == null) return false; else if (locals.length != exit.locals.length) return false;
            }
