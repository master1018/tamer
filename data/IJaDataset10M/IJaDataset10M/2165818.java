...
public class IntegerField extends StringRepresentableField {
    ...
    public IntegerField(byte[] fieldData, int offset, IntegerFieldBits bits, Signedness signedness, Endianness endianness, IntegerFieldRepresentation representation, String unitComponent) {
        ...
        if (fieldData == null) throw new IllegalArgumentException("fieldData == null");
        if (bits == null) throw new IllegalArgumentException("bits == null");
        if (signedness == null) throw new IllegalArgumentException("signedness == null");
        if (endianness == null) throw new IllegalArgumentException("endianness == null");
        if (representation == null) throw new IllegalArgumentException("representation == null");
        if (fieldData.length - offset < bits.getBytes()) throw new IllegalArgumentException("Not enough data left in fieldData!");
    ...
    public BigInteger getValueAsBigInteger() {
        byte[] data;
        if (endianness == Endianness.LITTLE_ENDIAN) data = Util.createReverseCopy(fieldData, offset, bits.getBytes()); else if (endianness == Endianness.BIG_ENDIAN) data = Util.createCopy(fieldData, offset, bits.getBytes()); else throw new RuntimeException("Illegal endianness value: " + endianness);
        if (signedness == Signedness.SIGNED) return new BigInteger(data); else if (signedness == Signedness.UNSIGNED) return new BigInteger(1, data); else throw new RuntimeException("Illegal signedness value: " + signedness);
    }
    ...
    public void setStringValue(String value) throws IllegalArgumentException {
        ...
        if (validateMsg == null) {
            ...
            if (signedness == Signedness.SIGNED && ba.length != bits.getBytes()) throw new RuntimeException("UNEXPECTED: ba.length (" + ba.length + ") != bits.getBytes()(" + bits.getBytes() + ")");
            if (signedness == Signedness.UNSIGNED && ba.length != (bits.getBytes() + 1)) throw new RuntimeException("UNEXPECTED: ba.length (" + ba.length + ") != bits.getBytes()+1(" + bits.getBytes() + "+1=" + (bits.getBytes() + 1) + ")");
            byte[] trueContents;
            if (endianness == Endianness.LITTLE_ENDIAN) trueContents = Util.createReverseCopy(ba, ba.length - bits.getBytes(), bits.getBytes()); else if (endianness == Endianness.BIG_ENDIAN) trueContents = Util.createCopy(ba, ba.length - bits.getBytes(), bits.getBytes()); else throw new RuntimeException("Illegal endianness value: " + endianness);
            System.arraycopy(trueContents, 0, fieldData, offset, bits.getBytes());
        } else throw new IllegalArgumentException("Invalid string value! Message: " + validateMsg);
    }
