...
public class Board {
    ...
    public void endRound() {
        ...
        if (movesInRound != 2) {
            throw new IllegalStateException("Both players have to move." + " Moves registered: " + movesInRound);
        }
        System.out.println("*** Valid round!");
    }

    public boolean putStone(int x, int y, int a_color) throws IllegalArgumentException {
    ...
    public int readField(int x, int y) throws IllegalArgumentException {
        if (x < 1 || y < 1 || x > WIDTH || y > HEIGHT) {
    ...
    public int readField(int a_index) throws IllegalArgumentException {
        if (a_index < 1 || a_index > (WIDTH * HEIGHT)) {
            throw new IllegalArgumentException("Index must be between 1.." + (WIDTH * HEIGHT));
        }
