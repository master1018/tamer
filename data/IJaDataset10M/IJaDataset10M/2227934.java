...
public class LeaseResponseMsg {
    ...
    public LeaseResponseMsg(Element root) {
        ...
        if (!doc.getName().equals(getMessageType())) {
            throw new IllegalArgumentException("Could not construct : " + getClass().getName() + " from doc containing a '" + doc.getName() + "'. Should be : " + getMessageType());
        }
    ...
    public Document getDocument(MimeMediaType mediaType) {
        ...
        Element e;
        if (null != serverAdv) {
            e = StructuredDocumentUtils.copyElements(msg, msg, (StructuredDocument) serverAdv.getDocument(mediaType), SERVER_ADV_TAG);
