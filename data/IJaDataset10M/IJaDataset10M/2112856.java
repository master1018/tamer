...
public class ContextCreatorCanvas extends PCanvas {
    ...
    private Color[] colors = { Color.RED, Color.BLUE, Color.BLACK, Color.CYAN, Color.DARK_GRAY, Color.GRAY, Color.GREEN, Color.LIGHT_GRAY, Color.MAGENTA, Color.ORANGE, Color.PINK, Color.WHITE, Color.YELLOW };
    ...
    public ContextCreatorCanvas(ContextDescriptor descriptor) {
        ...
        for (NetworkDescriptor network : descriptor.getNetworkDescriptors()) {
            NetworkLayer layer = new NetworkLayer(new BasicStroke(1, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0f, new float[] { dash, 5 }, 0.0f));
            dash = dash + 5;
    ...
    public void addAgentDescriptor(int index, AgentDescriptor desc, Point2D position) {
    ...
    public void addRelationshipDescriptor(NetworkDescriptor nd, RelationshipDescriptor desc) {
    ...
    public void removeRelationshipDescriptor(NetworkDescriptor networkDesc, AgentDescriptor agent) {
    ...
    public void removeRelationshipDescriptor(NetworkDescriptor desc, RelationshipDescriptor relationship) {
        networkLayerMap.get(desc).removeRelationship(nodeMap.get(relationship.getSource()), nodeMap.get(relationship.getTarget()));
    }
