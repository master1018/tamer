...
*/
public class BarbRendererJ3D extends DefaultRendererJ3D implements BarbRenderer {
    ...
    public float[] makeVector(boolean south, float x, float y, float z, float scale, float pt_size, float f0, float f1, float[] vx, float[] vy, float[] vz, int[] numv, float[] tx, float[] ty, float[] tz, int[] numt) {
        return bmr.makeVector(south, x, y, z, scale, pt_size, f0, f1, vx, vy, vz, numv, tx, ty, tz, numt);
    }
    ...
    public static void main(String args[]) throws VisADException, RemoteException {
        ...
        if (args.length > 1) {
            ...
            RealTupleType flowds = new RealTupleType(new RealType[] { flow_degree, flow_speed }, new WindPolarCoordinateSystem(flowxy), null);
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                ...
                double fy = 30.0 * v;
                if (args.length > 1) {
                    values[2][m] = Data.RADIANS_TO_DEGREES * Math.atan2(-fx, -fy);
