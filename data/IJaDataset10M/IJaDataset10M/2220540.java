...
public final class ExtractedInfo {
    ...
    public void trimStart(char[] sortedToTrim) {
        if (customExtractedString != null) customExtractedString = StringHelper.trimStart(customExtractedString, sortedToTrim); else while (extractedFrom < extractedTo && Arrays.binarySearch(sortedToTrim, line.getLine()[extractedFrom]) >= 0) extractedFrom++;
    }

    public void TrimEnd(char[] sortedToTrim) {
        if (customExtractedString != null) customExtractedString = StringHelper.trimEnd(customExtractedString, sortedToTrim); else while (extractedTo > extractedFrom && Arrays.binarySearch(sortedToTrim, line.getLine()[extractedTo]) >= 0) extractedTo--;
    }
    ...
    public boolean hasOnlyThisChars(char[] sortedArray) {
        if (customExtractedString != null) {
            int pos = 0;
            while (pos < customExtractedString.length() && Arrays.binarySearch(sortedArray, customExtractedString.charAt(pos)) >= 0) {
                pos++;
            }
