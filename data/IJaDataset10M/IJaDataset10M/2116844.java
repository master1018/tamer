...
class BackEndTest extends jdwp implements VMConnection {
    ...
    * incorrect content
    */
    private void openConnections(String debugee_server, int debugee_port) throws ConnectException, DebugeeException {
        try {
            Tools.wait(1000);
    ...
    */
    ...
    public void printReplies() throws IOException {
        debug.receive();
        if (debug.Replies.size() == 0) return;
        print("\nReplies:\n");
        for (int i = 0; i < debug.Replies.size(); i++) print("\n" + (Reply) debug.Replies.elementAt(i) + "\n");
    }
     ...
     */
    public Reply checkReply(Command c) throws IOException, DebugeeException {
        sendCommand(c);
     ...
     */
    public Reply checkReply(Command c, int ErrorCode, String Description) throws IOException, DebugeeException {
        Reply r = checkReply(c);
        if (r.getErrorCode() != ErrorCode) {
            print("\nCommand:\n" + c + "\nReply:\n" + r);
            String m = "Unexpected error code";
            if (!Description.equals("")) Description = "Unexpected error code (" + Description + ")."; else Description = "Unexpected error code.";
