...
public class JavaOpBotMenu extends JMenuBar implements ActionListener {
    ...
    private final Hashtable<JMenuItem, ActionListener> callbacks = new Hashtable<JMenuItem, ActionListener>();
    ...
    public void addItem(String name, String whichMenu, int index, char mnemonic, KeyStroke hotkey, Icon icon, ActionListener callback) {
        ...
        if (hotkey != null) menu.setAccelerator(hotkey);
        if (icon != null) menu.setIcon(icon);
        if (index >= 0) menu.add(newItem, index); else menu.add(newItem);
        if (callback != null) callbacks.put(newItem, callback);
        this.invalidate();
        this.validate();
    }
    ...
    public void removeItem(String name, String whichMenu) {
        ...
        for (int i = 0; i < items.length; i++) if (((JMenuItem) items[i]).getText().equalsIgnoreCase(name)) menu.remove(items[i]);
    }
    ...
    public void addMenu(String name, int index, char mnemonic, Icon icon, ActionListener callback) {
        ...
        if (icon != null) menu.setIcon(icon);
        if (callback != null) menu.addActionListener(callback);
        if (index != -1) this.add(menu, index); else this.add(menu);
        this.invalidate();
        this.validate();
