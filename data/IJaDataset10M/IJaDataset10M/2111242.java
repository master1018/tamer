...
public class IntegerArrayConverter extends SizeConverter<int[]> {
    ...
    public int[] read(DataInput dataInput, int[] object) throws IOException {
        ...
        if (length == -1) length = dataInput.readInt();
        if (object == null) object = new int[length];
        for (int i = 0; i < object.length; i++) object[i] = dataInput.readInt();
        return object;
    }
    ...
    public void write(DataOutput dataOutput, int[] object) throws IOException {
        ...
        for (int i = 0; i < len; i++) dataOutput.writeInt(object[i]);
    }
    ...
    public int getSerializedSize(int[] object) {
        return arraySize == -1 ? getSizeForArray(true, object.length) : getSizeForArray(false, arraySize);
    }
}
