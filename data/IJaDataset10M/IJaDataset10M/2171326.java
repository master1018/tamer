...
public class CLDevice extends CLAbstractEntity<cl_device_id> {
    ...
    public enum ExecutionCapability {
        ...
        Kernel, @EnumValue(CL_EXEC_NATIVE_KERNEL)
        NativeKernel;
    ...
    public enum Type {
        ...
        CPU, @EnumValue(CL_DEVICE_TYPE_GPU)
        GPU, @EnumValue(CL_DEVICE_TYPE_ACCELERATOR)
        Accelerator, @EnumValue(CL_DEVICE_TYPE_DEFAULT)
        Default;
    ...
    public enum SingleFPConfig {
        ...
        Denorm, /** INF and quiet NaNs are supported.                      */
        @EnumValue(CL_FP_INF_NAN)
        InfNaN, /** round to nearest even rounding mode supported          */
        @EnumValue(CL_FP_ROUND_TO_NEAREST)
        RoundToNearest, /** round to zero rounding mode supported                  */
        @EnumValue(CL_FP_ROUND_TO_ZERO)
        RoundToZero, /** round to +ve and -ve infinity rounding modes supported */
        @EnumValue(CL_FP_ROUND_TO_INF)
        RoundToInf, /** IEEE754-2008 fused multiply-add is supported.          */
        @EnumValue(CL_FP_FMA)
        FMA;
    ...
    public enum GlobalMemCacheType {
        ...
        @EnumValue(CL_NONE)
        None, @EnumValue(CL_READ_ONLY_CACHE)
        ReadOnlyCache, @EnumValue(CL_READ_WRITE_CACHE)
        ReadWriteCache;
    ...
    public String[] getExtensions() {
        if (extensions == null) extensions = infos.getString(get(), CL_DEVICE_EXTENSIONS).split("\\s+");
        return extensions;
    }
    ...
    boolean hasExtension(String name) {
        name = name.trim();
        for (String x : getExtensions()) if (name.equals(x.trim())) return true;
        return false;
    }
    ...
    public enum QueueProperties {
        ...
        OutOfOrderExecModeEnable, @EnumValue(CL_QUEUE_PROFILING_ENABLE)
        ProfilingEnable;
