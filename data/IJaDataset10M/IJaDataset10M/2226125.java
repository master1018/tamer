...
public class Curve3DDisplay extends Canvas {
    ...
    private static final float DEFAULT_DISTANCE = 4;

    private static final float DEFAULT_TRANSLATION = 0;
    ...
    private float lightValue = 1.25f, kd = 0.7f, ambientLightValue = 0.25f, ka = 0.7f;
    ...
    public void computeCurve(CompiledFunction function, double minimumX, double maximumX, int meshSize) {
        this.minimumX = minimumX;
        this.maximumX = maximumX;
        this.meshSize = meshSize;
        distance = distance - (float) minimumX;
        computeValuesArray(function, new Double(minimumX), new Double(maximumX), new Double((maximumX - minimumX) / meshSize), new Double(minimumX), new Double(maximumX), new Double((maximumX - minimumX) / meshSize));
    }
    ...
    private void invalidateSurface() {
        points3D = null;
        if (values != null) repaint();
    }

    private synchronized void computeValuesArray(CompiledFunction function, Double minimumX, Double maximumX, Double deltaX, Double minimumZ, Double maximumZ, Double deltaZ) {
        int dimX, dimZ, xIndex, zIndex;
        double[] parameters = new double[2];
        float valX, valZ, pasXReel = (float) deltaX.doubleValue(), pasZReel = (float) deltaZ.doubleValue();
        ...
        for (valX = (float) minimumX.doubleValue(), xIndex = 1; xIndex <= dimX; valX += pasXReel, xIndex++) {
            values[xIndex][0] = valX;
            parameters[0] = valX;
            for (valZ = (float) minimumZ.doubleValue(), zIndex = 1; zIndex <= dimZ; valZ += pasZReel, zIndex++) {
                if (xIndex == 1) values[0][zIndex] = valZ;
                parameters[1] = valZ;
    ...
    private synchronized void compute3DPoints() {
        ...
        Matrix3D displayMatrix = Matrix3D.getDisplayMatrix(center, new Vector3D(rotX, rotY, rotZ), rotationOrder, translation, 0, 0, distance, new Point3D(size.width / 2, size.height / 2, 0), new Vector3D(scale, -scale, scale));
        for (int xIndex = 0; xIndex < points3D.length; xIndex++) for (int zIndex = 0; zIndex < points3D[0].length; zIndex++) points3D[xIndex][zIndex] = new Point3D(values[xIndex + 1][0], values[xIndex + 1][zIndex + 1], values[0][zIndex + 1]).multiplyMatrixByPoint(displayMatrix);
    }

    private Color getTriangleColor(Point3D pointA, Point3D pointB, Point3D pointC) {
        Vector3D pointsABCNormal = (new Vector3D(pointA, pointB)).getCrossProduct(new Vector3D(pointA, pointC));
        if (pointsABCNormal.getZ() <= 0) return curveBackColor;
        float lightFactor = pointsABCNormal.getDotProduct(lightDirection) / (pointsABCNormal.getLength() * lightDirection.getLength());
        float light = kd * lightValue * Math.abs(lightFactor) + ka * ambientLightValue;
        if (light > 1) light = 1;
        return new Color((int) (curveColor.getRed() * light), (int) (curveColor.getGreen() * light), (int) (curveColor.getBlue() * light));
    }

    private void displayPolygon(Graphics gc, boolean lit, int xIndex, int yIndex) {
        Dimension[] points = new Dimension[4];
        boolean allOutside = true;
        for (int dx = 0; dx < 2; dx++) for (int dy = 0; dy < 2; dy++) if (Double.isNaN(values[xIndex + 1 + dx][yIndex + 1 + dy])) return; else {
            ...
            if (allOutside) allOutside = !inside(points[2 * dx + dy].width, points[2 * dx + dy].height) || points3D[xIndex + dx][yIndex + dy].getZ() < userDistance;
        }
        if (allOutside) return;
        gc.setColor(lit ? getTriangleColor(points3D[xIndex][yIndex], points3D[xIndex + 1][yIndex], points3D[xIndex][yIndex + 1]) : curveColor);
    ...
    private void displaySurface(Graphics gc, boolean lit) {
        int startX = 0, startY = 0, endX = 0, endY = 0, deltaX = 1, deltaY = 1;
        Point3D farthestPoint3D = points3D[0][0];
        for (int xIndex = 0; xIndex < points3D.length; xIndex += points3D.length - 1) for (int yIndex = 0; yIndex < points3D[0].length; yIndex += points3D[0].length - 1) if (xIndex != 0 || yIndex != 0) if (points3D[xIndex][yIndex].getZ() < farthestPoint3D.getZ()) {
        ...
        if (!goThroughAxisX) for (int xIndex = startX; xIndex != endX; xIndex += deltaX) for (int yIndex = startY; yIndex != endY; yIndex += deltaY) displayPolygon(gc, lit, xIndex, yIndex); else for (int yIndex = startY; yIndex != endY; yIndex += deltaY) for (int xIndex = startX; xIndex != endX; xIndex += deltaX) displayPolygon(gc, lit, xIndex, yIndex);
    }

    private void displayMesh(Graphics gc) {
        gc.setColor(curveMeshColor);
        for (int xIndex = 0; xIndex < points3D.length; xIndex++) for (int yIndex = 1; yIndex < points3D[0].length; yIndex++) if (values[xIndex + 1][yIndex] == values[xIndex + 1][yIndex] && values[xIndex + 1][yIndex + 1] == values[xIndex + 1][yIndex + 1]) {
            ...
            if (inside(startX, startY) && points3D[xIndex][yIndex - 1].getZ() >= userDistance || inside(endX, endY) && points3D[xIndex][yIndex].getZ() >= userDistance) gc.drawLine(startX, startY, endX, endY);
        }
        for (int yIndex = 0; yIndex < points3D[0].length; yIndex++) for (int xIndex = 1; xIndex < points3D.length; xIndex++) if (values[xIndex][yIndex + 1] == values[xIndex][yIndex + 1] && values[xIndex + 1][yIndex + 1] == values[xIndex + 1][yIndex + 1]) {
            int startX = (int) points3D[xIndex - 1][yIndex].getX();
            int startY = (int) points3D[xIndex - 1][yIndex].getY();
            int endX = (int) points3D[xIndex][yIndex].getX();
            int endY = (int) points3D[xIndex][yIndex].getY();
            if (inside(startX, startY) && points3D[xIndex - 1][yIndex].getZ() >= userDistance || inside(endX, endY) && points3D[xIndex][yIndex].getZ() >= userDistance) gc.drawLine(startX, startY, endX, endY);
        }
    }
    ...
    public void paint(Graphics gc) {
        ...
        } else if (points3D == null) paintSurface(bitmap.getGraphics());
        gc.drawImage(bitmap, 0, 0, this);
    }

    private synchronized void paintSurface(Graphics gc) {
        Dimension size = size();
        gc.setColor(getBackground());
        gc.fillRect(0, 0, size.width, size.height);
        if (points3D == null) compute3DPoints();
        if (points3D != null) switch(displayMode) {
            case DISPLAY_WITH_LIGHTING:
                displaySurface(gc, true);
                break;
            case DISPLAY_MESH:
                displaySurface(gc, false);
                break;
            case DISPLAY_ALL_MESH:
                displayMesh(gc);
                break;
        }
        gc.setColor(getBackground());
