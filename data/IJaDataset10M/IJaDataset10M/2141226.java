...
public class Disc extends Controller {
    ...
    private boolean needsLookUp(DiscWs2 disc, ReleaseIncludesWs2 inc) {
        return (getDiscWs2() == null || super.needsLookUp(inc) || inc.isLabel() || inc.isReleaseGroups() || inc.isArtistCredits() || inc.isMedia() || inc.isRecordings() || inc.isDiscids());
    }
    ...
    public DiscWs2 lookUp(DiscWs2 disc) throws MBWS2Exception {
        ...
        if ((disc.getDiscId() == null || disc.getDiscId().isEmpty()) && (disc.getToc() == null || disc.getToc().isEmpty()) && (disc.getTracks() == null || disc.getTracks().isEmpty())) {
            throw new MBWS2Exception("Invalid request");
        }
        if ((disc.getToc() == null || disc.getToc().isEmpty()) && disc.getTracks() != null && !disc.getTracks().isEmpty()) {
            ...
            } else if (transit.getDiscId() != null && !transit.getDiscId().equals(disc.getDiscId())) {
                log.warn("libdiscId changed discId");
            }
            ;
        } else if ((disc.getTracks() == null || disc.getTracks().isEmpty()) && disc.getToc() != null && !disc.getToc().isEmpty()) {
            ...
            if (!transit.getToc().equals(disc.getToc())) throw new MBWS2Exception("Invalid request: non corresponding toc and trackList");
            if (disc.getDiscId() == null || disc.getDiscId().isEmpty()) {
                disc.setDiscId(transit.getDiscId());
            } else if (transit.getDiscId() != null && !transit.getDiscId().equals(disc.getDiscId())) {
                log.warn("libdiscId changed discId");
    ...
    private List<DiscTrackWs2> copyAndOrderTrackList(List<DiscTrackWs2> in) throws MBWS2Exception {
        ...
        for (DiscTrackWs2 t : in) {
            if (t.getTracknum() < 1) throw new MBWS2Exception("invalid trackList");
            if (t.getTracknum() > in.size() + 1) throw new MBWS2Exception("invalid trackList");
            if (ordered.get(t.getTracknum() - 1) != null) throw new MBWS2Exception("invalid trackList");
    ...
    private int[] calcOffsets(DiscWs2 disc) throws MBWS2Exception {
        if (disc.getTracks() == null || disc.getTracks().isEmpty()) throw new MBWS2Exception("Invalid track list");
