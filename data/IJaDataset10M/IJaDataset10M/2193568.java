    ...
    private static class NQCActionsFunction implements ActionsFunction {
        ...
        public Set<Action> actions(Object state) {
            ...
            for (int i = 0; i < board.getSize(); i++) for (int j = 0; j < board.getSize(); j++) {
                ...
                if (!board.queenExistsAt(loc)) actions.add(new QueenAction(QueenAction.MOVE_QUEEN, loc));
            }
            return actions;
        }
    }
    ...
    private static class NQResultFunction implements ResultFunction {
        ...
        public Object result(Object s, Action a) {
            if (a instanceof QueenAction) {
                ...
                if (qa.getName() == QueenAction.PLACE_QUEEN) newBoard.addQueenAt(qa.getLocation()); else if (qa.getName() == QueenAction.REMOVE_QUEEN) newBoard.removeQueenFrom(qa.getLocation()); else if (qa.getName() == QueenAction.MOVE_QUEEN) newBoard.moveQueenTo(qa.getLocation());
                s = newBoard;
            }
            return s;
        }
