...
import org.openscience.cdk.applications.taverna.qsar.descriptors.atomic.SigmaElectronegativity;
 ...
 *
 */
public class SigmaElectronegativityTest extends CDKTavernaTestCase {
    ...
    private String descriptorName = "SigmaElectronegativity";
    ...
    public void test_LocalWorkerSigmaElectronegativity() throws CDKException, Exception {
        SigmaElectronegativity test = new SigmaElectronegativity();
        ...
        if (resultListCalculatedStructures.size() != 0) {
            for (Iterator iter = resultListCalculatedStructures.iterator(); iter.hasNext(); ) {
                ...
                if (!((String) fileName.get(fileName.size() - 1)).equalsIgnoreCase("SigmaElectronegativityDescriptor")) {
                    hasFileNameAnError = true;
                }
            }
        }
