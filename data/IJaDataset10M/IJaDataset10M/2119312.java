 ...
 * <p>
 * IntervalsList: This class represents an intervals list.
 * <p>
 * An interval list represent a list of contiguous regions on the real line. All
 * intervals of the list are disjoints to each other, they are stored in
 * ascending order.
 * </p>
 * <p>
 ...
 * </p>
 * 
 * @see Interval Created on 21 mars 2005
 * @author Gillet
 */
public class IntervalsList {
    ...
    */
    public IntervalsList(Interval i1, Interval i2) {
        intervals = new ArrayList<Interval>();
        if (i1.intersects(i2)) {
            intervals.add(new Interval(Math.min(i1.getInf(), i2.getInf()), Math.max(i1.getSup(), i2.getSup())));
    ...
    */
    public IntervalsList(IntervalsList list) {
        intervals = new ArrayList<Interval>(list.intervals.size());
    ...
    * 
    * @return lower bound of the list or Double.NaN if the list does not contain
    *         any interval
    */
    ...
    * 
    * @return upper bound of the list or Double.NaN if the list does not contain
    *         any interval
    */
    public double getSup() {
        return intervals.isEmpty() ? Double.NaN : ((Interval) intervals.get(intervals.size() - 1)).getSup();
    }
    ...
    * from the list containing only the [0, 10] interval, the result will be the
    * [0, 2] U [3, 10] intervals list.
    * </p>
    * 
    ...
    */
    public void subtractFromSelf(Interval i) {
        ...
        intersectSelf(new IntervalsList(new Interval(a - 1.0, i.getInf()), new Interval(i.getSup(), b + 1.0)));
    }
    ...
    */
    ...
    /**
    * Clean all singleton interval ([n, n]) from this list.
    */
    ...
    /**
    * Clean specified singleton interval ([v, v]) from this list.
    */
    public void cleanSingleton(double v) {
        int s = intervals.size();
        for (int i = 0; i < s; i++) {
            Interval interv = (Interval) intervals.get(i);
            if (interv.getLength() == 0) {
                if (interv.getInf() == v && interv.getSup() == v) {
                    intervals.remove(i);
                    i--;
                    s--;
                }
