...
class PSTTextView extends PSTShapeView<IText> {
    ...
    public void updateCache(final IPoint origin, final float ppc) {
        if (!GLibUtilities.INSTANCE.isValidPoint(origin) || ppc < 1) return;
        final StringBuilder rot = getRotationHeaderCode(ppc, origin);
        emptyCache();
        if (rot != null) cache.append(rot);
        String colorName;
        Color lineCol = shape.getLineColour();
        if (lineCol.equals(PSTricksConstants.DEFAULT_LINE_COLOR)) colorName = null; else {
            colorName = getColourName(shape.getLineColour());
            addColour(colorName);
        }
        cache.append("\\rput[").append(shape.getTextPosition().getLatexToken()).append(']').append('(');
        ...
        if (colorName != null) cache.append('}');
        if (rot != null) cache.append('}');
    }
}
