...
public class Guild {
    ...
    public static class GuildMember {
        ...
        public void setAllRights(int rank, byte xp, int right) {
            if (rank == -1) rank = this._rank;
            if (xp < 0) xp = this._pXpGive;
            if (xp > 90) xp = 90;
            if (right == -1) right = this._rights;
            this._rank = rank;
            this._pXpGive = xp;
            if (right != this._rights && right != 1) parseIntToRight(right);
            this._rights = right;
        ...
        public void parseIntToRight(int total) {
            ...
            if (total == 1) return;
            if (haveRight.size() > 0) haveRight.clear();
            initRight();
            Integer[] mapKey = haveRight.keySet().toArray(new Integer[haveRight.size()]);
            while (total > 0) {
    ...
    public Guild(int id, String name, String emblem, int lvl, long xp, int capital, int nbrmax, String sorts, String stats) {
        _id = id;
        _name = name;
        _emblem = emblem;
        _xp = xp;
    ...
    public void addXp(long xp) {
        this._xp += xp;
        while (_xp >= World.getGuildXpMax(_lvl) && _lvl < 200) levelUp();
    }
    ...
    public void decompileSpell(String spellStr) {
        int id;
        int lvl;
