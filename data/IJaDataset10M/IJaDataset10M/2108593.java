...
public class ForceDirectedEdgeBundler {
    ...
    public ForceDirectedEdgeBundler(FlowMapGraph flowMapGraph, ForceDirectedBundlerParameters params) {
    ...
    private void init(ProgressTracker progressTracker) {
        ...
        for (int i = 0; i < numEdges; i++) {
            ...
            edgeStarts[i] = (flowMapGraph.getEdgeSourcePoint(edge));
            edgeEnds[i] = (flowMapGraph.getEdgeTargetPoint(edge));
    ...
    private double calcStandardEdgeCompatibility(int i, int j) {
        ...
        double Cv;
        if (Ca * Cs * Cp > .9) {
            Cv = Math.min(visibilityCompatibility(edgeStarts[i], edgeEnds[i], edgeStarts[j], edgeEnds[j]), visibilityCompatibility(edgeStarts[j], edgeEnds[j], edgeStarts[i], edgeEnds[i]));
        } else {
            Cv = 1.0;
        }
        assert (Ca >= 0 && Ca <= 1);
    ...
    private static double visibilityCompatibility(Point p0, Point p1, Point q0, Point q1) {
        ...
        return Math.max(0, 1 - 2 * pm.distanceTo(im) / i0.distanceTo(i1));
    }
                ...
                for (int i = 0; i < P; i++) {
                    ...
                    for (int ci = 0, size = compatible.size(); ci < size; ci++) {
                        ...
                        if (Math.abs(v_x) > EPS || Math.abs(v_y) > EPS) {
                            double d = Math.sqrt(v_x * v_x + v_y * v_y);
                            double m;
    ...
    private void copy(Point[][] src, Point[][] dest) {
        ...
        for (int i = 0; i < src.length; i++) {
            for (int j = 0, len = src[i].length; j < len; j++) {
                Point ps = src[i][j];
                if (ps == null) dest[i][j] = null; else dest[i][j] = new Point(ps.x(), ps.y());
            }
        }
    }
}
