...
public final class FindInFiles extends Replacement implements Constants, BackgroundProcess {
    ...
    public void setFiles(String files) throws Exception {
        ...
        if (currentDir == null || currentDir.isRemote()) throw new Exception("Operation not supported for remote files");
    ...
    private void runInternal() {
        ...
        if (defaultExcludes) {
            try {
                String excludesPattern = Editor.preferences().getStringProperty(Property.FILENAME_COMPLETIONS_EXCLUDE_PATTERN);
        ...
        for (Iterator it = filters.iterator(); it.hasNext(); ) {
            ...
            if (dir == null) dir = getEditor().getCurrentDirectory();
            ...
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    if (outputBuffer != null) {
                        if (cancelled) getEditor().status("Search cancelled"); else getEditor().status("Search completed");
                        FastStringBuffer sb = new FastStringBuffer("Pattern found in ");
                        sb.append(results.size());
                        sb.append(" of ");
                        sb.append(numFilesExamined);
                        sb.append(" files examined");
                        if (cancelled) sb.append(" (search cancelled by user)");
    ...
    private void searchDirectory(File dir, Filter filter, RE excludesRE) {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (excludesRE != null && excludesRE.isMatch(files[i])) continue;
            if (file.isDirectory()) {
                if (includeSubdirs) searchDirectory(file, filter, excludesRE);
                continue;
            }
            if (!filter.accepts(files[i])) continue;
        ...
        try {
            ...
            while ((s = reader.readLine()) != null) {
                ...
                if (found) {
                    ...
                    try {
                        if (matches == 0) {
                            if (!listEachOccurrence && results.size() == 0) outputBuffer.appendLine("Found in:");
                            outputBuffer.appendFileLine(file, listEachOccurrence);
                            results.add(file);
                            update = true;
                        }
                        ++matches;
                        if (listEachOccurrence) outputBuffer.appendOccurrenceLine(s, lineNumber); else break;
                    } finally {
                        outputBuffer.renumber();
            ...
            if (update) SwingUtilities.invokeLater(updateDisplayRunnable);
        } catch (IOException e) {
            Log.error(e);
        }
    }
    ...
    private void processFileInternal(File file, Mode mode, Position pos) {
        if (!listEachOccurrence && results.size() == 1) outputBuffer.appendLine("Found in:");
        outputBuffer.appendFileLine(file, listEachOccurrence);
        if (listEachOccurrence) {
            ...
            while (pos.getLine().next() != null) {
                ...
                if ((pos = find(mode, pos)) != null) outputBuffer.appendOccurrenceLine(pos.getLine()); else break;
            }
        }
    }
    ...
    private void handleSaveException(final SaveException e) {
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                String title = "Replace In Files";
                String message = e.getMessage();
                if (message != null) MessageDialog.showMessageDialog(message, title);
    ...
    private void replaceInFile(final File file) throws CheckFileException, SaveException {
        ...
        if (confirmChanges()) {
            if (SwingUtilities.isEventDispatchThread()) {
                ...
                if (FindInFiles.this.saveException != null) throw FindInFiles.this.saveException;
            }
        } else replaceInFileNoConfirm(file);
    }
    ...
    private void replaceInFileNoConfirm(File file) throws SaveException {
        ...
        if (buffer != null) {
            if (!buffer.isLoaded()) {
                if (!buffer.initialized()) buffer.initialize();
                buffer.load();
            ...
            if (buffer.isModified() != wasModified) Sidebar.setUpdateFlagInAllFrames(SIDEBAR_REPAINT_BUFFER_LIST);
            if (getReplacementCount() > oldReplacementCount) ++numFilesModified;
            ...
            if (modified) buf.writeBuffer();
            if (modified) ++numFilesModified;
        }
        replacedInFile(file);
    }
    ...
    private void replaceInFileConfirm(File file) throws SaveException {
        ...
        confirmDialog.show();
        if (confirmDialog.cancelled()) cancelled = true;
        ...
        if (close) {
            ...
            if (!buffer.isModified()) buffer.kill();
        } else {
    ...
    private void replacedInFile(File file) {
        if (outputBuffer != null) {
            ...
            try {
                if (numFilesModified == 1) outputBuffer.appendLine("Replaced in:");
            ...
            if (SwingUtilities.isEventDispatchThread()) {
                ...
                for (EditorIterator iter = new EditorIterator(); iter.hasNext(); ) {
                    Editor ed = iter.nextEditor();
                    if (ed.getBuffer() == outputBuffer) {
                        if (end == null) end = outputBuffer.getEnd();
                        ed.moveDotTo(end);
                        ed.setUpdateFlag(REPAINT);
                        ed.updateDisplay();
                        ed.repaintNow();
    ...
    private void checkFile(File file) throws CheckFileException {
        if (file.isRemote()) checkFileError(file, "file is not local");
        if (file.isDirectory()) checkFileError(file, "file is a directory");
        if (!file.isFile()) checkFileError(file, "file not found");
        if (!file.canRead()) checkFileError(file, "file is not readable");
        boolean writable = file.canWrite();
        if (!writable) {
            if (Editor.preferences().getBooleanProperty(Property.P4_AUTO_EDIT)) {
                if (P4.autoEdit(file)) writable = file.canWrite();
            }
            if (!writable) checkFileError(file, "file is read only");
        }
    }
    ...
    private void completed() {
        ...
        if (replacementCount == 0) {
            ...
            if (replacementCount > 1) sb.append('s');
            sb.append(" in ");
            sb.append(numFilesModified);
            sb.append(" file");
            if (numFilesModified > 1) sb.append('s');
        }
        if (cancelled) sb.append(" (operation cancelled)"); else sb.append(" (" + numFilesExamined + " files examined)");
        if (outputBuffer != null) {
            outputBuffer.appendStatusLine(sb.toString());
            outputBuffer.renumber();
            for (EditorIterator it = new EditorIterator(); it.hasNext(); ) {
        ...
        } else MessageDialog.showMessageDialog(getEditor(), sb.toString(), "ReplaceInFiles");
    }

    public static void findInFiles() {
        final Editor editor = Editor.currentEditor();
        final File dir = editor.getCurrentDirectory();
        if (dir != null && !dir.isRemote()) findOrReplaceInFiles(editor, false);
    }

    public static void replaceInFiles() {
        final Editor editor = Editor.currentEditor();
        final File dir = editor.getCurrentDirectory();
        if (dir != null && !dir.isRemote()) findOrReplaceInFiles(editor, true);
    }
    ...
    private static void findOrReplaceInFiles(Editor editor, boolean replace) {
        ...
        d.show();
        editor.repaintNow();
        if (d.getFindInFiles() == null) return;
        if (findInFiles != null) {
            Buffer buf = findInFiles.getOutputBuffer();
            if (Editor.getBufferList().contains(buf)) buf.kill();
        }
        ...
        if (outputBuffer != null) {
            ...
            } else outputBuffer.setUnsplitOnClose(true);
            editor.makeNext(outputBuffer);
    ...
    public static void listFiles() {
        if (findInFiles != null) findInFiles.listFiles(Editor.currentEditor());
    }
    ...
    private static final class Filter {
        ...
        public Filter(String s) throws Exception {
            ...
            if (!processFilter(file.getName())) throw new Exception("process pattern failed");
        }
