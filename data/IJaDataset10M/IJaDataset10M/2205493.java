...
public class OntologyValidator {
     ...
     */
    public void validate(InputStream rdf, Map<String, InputStream> ontologies, Validation validation, String objid) throws ServerException {
        Document rdfDoc = DOM.streamToDOM(rdf, true);
        NodeList relationsNodelist = xpathselector.selectNodeList(rdfDoc, "/rdf:RDF/rdf:Description[@rdf:about='info:fedora/" + objid + "']/*");
     ...
     * @throws ServerException if something failed
     */
    void checkOntology(NodeList relations, Document ontology, String contentModel, Validation validation) throws ServerException {
        Node classNode = xpathselector.selectNode(ontology, "/rdf:RDF/owl:Class[@rdf:about='info:fedora/" + contentModel + "_class']");
        NodeList objectRelationDeclarations = xpathselector.selectNodeList(ontology, "/rdf:RDF/owl:ObjectProperty/@rdf:about");
        for (int i = 0; i < objectRelationDeclarations.getLength(); i++) {
            String relationName = objectRelationDeclarations.item(i).getNodeValue();
            NodeList restrictions = xpathselector.selectNodeList(classNode, "rdfs:subClassOf/" + "owl:Restriction" + "[owl:onProperty/@rdf:resource='" + relationName + "']");
            for (int j = 0; j < restrictions.getLength(); j++) {
                Node restriction = restrictions.item(j);
                String allValuesFromRestriction = xpathselector.selectString(restriction, "owl:allValuesFrom/@rdf:resource");
                String someValuesFromRestriction = xpathselector.selectString(restriction, "owl:someValuesFrom/@rdf:resource");
     ...
     */
    private void checkMaxCardinality(String maxCardinality, String relationName, NodeList objectRelations, Validation validation, String contentModel) {
        ...
        if (count > cardint) {
            ...
            validation.getObjectProblems().add("Contentmodel '" + contentModel + "' require that the object must have less than " + "'" + cardint + "' relations with the name '" + relationName + "' but" + " the object has '" + count + "'");
        }
    }
     ...
     * @param contentModel   the content model pid (for reporting)
     */
    private void checkMinCardinality(String minCardinality, String relationName, NodeList relations, Validation validation, String contentModel) {
        ...
        if (count < cardint) {
            validation.setValid(false);
            validation.getObjectProblems().add("Contentmodel '" + contentModel + "' require that the object must have " + "'" + cardint + "' or more relations with the name '" + relationName + "' but" + " the object has only '" + count + "'");
        }
    }
     ...
     * @param contentModel the content model pid (for reporting)
     */
    private void checkCardinality(String cardinality, String relationName, NodeList relations, Validation validation, String contentModel) {
        int count = countRelations(relationName, relations);
        int cardint;
        try {
        ...
        if (count != cardint) {
            validation.setValid(false);
            validation.getObjectProblems().add("Contentmodel '" + contentModel + "' require that the object must have " + "exactly '" + cardint + "' relations with the name '" + relationName + "' but" + " the object has '" + count + "'");
        }
    }
    ...
    private int countRelations(String relationName, NodeList objectRelations) {
        ...
        for (int i = 0; i < objectRelations.getLength(); i++) {
            ...
            if (objectRelationName.equals(relationName)) {
                count++;
            }
        }
        return count;
     ...
     * @throws ServerException if something failed
     */
    private void checkSomeValuesFrom(String someValuesFromRestriction, String relationName, NodeList objectRelations, Validation validation, String contentModel) throws ServerException {
        if (someValuesFromRestriction.endsWith("_class")) {
            someValuesFromRestriction = someValuesFromRestriction.substring(0, someValuesFromRestriction.length() - "_class".length());
        }
        ...
        if (!foundGoodRelation) {
            validation.setValid(false);
            validation.getObjectProblems().add("Contentmodel '" + contentModel + "' require that this object should have " + "at least one relation of the name '" + relationName + "' to an object with the contentmodel '" + someValuesFromRestriction + "'");
        }
    }
     ...
     * @throws ServerException if something failed.
     */
    private void checkAllValuesFrom(String allValuesFromRestriction, String relationName, NodeList objectRelations, Validation validation, String contentModel) throws ServerException {
        if (allValuesFromRestriction.endsWith("_class")) {
            allValuesFromRestriction = allValuesFromRestriction.substring(0, allValuesFromRestriction.length() - "_class".length());
        }
        for (int i = 0; i < objectRelations.getLength(); i++) {
            Node relation = objectRelations.item(i);
            String objectRelationName = relation.getNamespaceURI() + relation.getLocalName();
            if (objectRelationName.equals(relationName)) {
                ...
                if (!targetObject.getContentModels().contains(allValuesFromRestriction)) {
                    validation.setValid(false);
                    validation.getObjectProblems().add("Relation '" + relationName + "' refers to resource '" + targetObject.GetObjectPID() + "' which, by content model '" + contentModel + "' should be of the type '" + allValuesFromRestriction + "'");
                }
            }
        }
    }
    ...
    private String selectTextualNode(Node doc, String xpath) {
        Node cardinalityRestrictionNode = xpathselector.selectNode(doc, xpath);
