...
public class ChannelList extends JTable {
    ...
    public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
        ...
        if (c != null && c instanceof JComponent) ((JComponent) c).setToolTipText(getTooltip(row, column) + "");
        return c;
    }
...
class Model extends AbstractTableModel {
    ...
    public void addUser(String name, String client, String clan, int ping, int flags) {
        if (name == null) return;
        if (client == null) client = "CHAT";
        if (clan == null) clan = "";
        Row row = findUser(name);
        if (row == null) {
            row = new Row(name, client, clan, ping, flags);
            if (opsOnTop && (flags & Flags.USER_CHANNELOP) > 0 || (flags & Flags.USER_BLIZZREP) > 0 || (flags & Flags.USER_ADMIN) > 0) users.add(0, row); else users.add(row);
        } else {
            if (name.length() > 0) row.setName(name);
            if (client.length() > 0) row.setClient(client);
            row.setPing(ping);
            row.setFlags(flags);
    ...
    private class Row {
        ...
        public ImageIcon getGameIcon() {
            if ((flags & Flags.USER_SQUELCHED) > 0) return GameIcons.getIcon("SQUELCHED"); else if ((flags & Flags.USER_ADMIN) > 0) return GameIcons.getIcon("SYSOP"); else if ((flags & Flags.USER_BLIZZREP) > 0) return GameIcons.getIcon("BLIZZARD"); else if ((flags & Flags.USER_SPEAKER) > 0) return GameIcons.getIcon("SPEAKER"); else if ((flags & Flags.USER_CHANNELOP) > 0) return GameIcons.getIcon("OPS"); else if (client != null && !client.isEmpty()) return GameIcons.getIcon(client); else return null;
        }

        public ImageIcon getPingIcon() {
            if ((flags & Flags.USER_NOUDP) > 0) return GameIcons.getIcon("DISCONNECTED"); else if (ping == 0) return null; else if (ping < 200) return GameIcons.getIcon("1g"); else if (ping < 300) return GameIcons.getIcon("2g"); else if (ping < 400) return GameIcons.getIcon("3y"); else if (ping < 500) return GameIcons.getIcon("4y"); else if (ping < 600) return GameIcons.getIcon("5r");
            return GameIcons.getIcon("6r");
        }

        public boolean equals(Object o) {
            if ((o instanceof Row) && ((Row) o).getName().equalsIgnoreCase(getName())) return true;
            if ((o instanceof String) && ((String) o).equalsIgnoreCase(getName())) return true;
            return false;
        }
