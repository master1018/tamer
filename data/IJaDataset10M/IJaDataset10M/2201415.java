...
public class DocumentBasedStructuredValueTest extends TestCase {
   ...
   */
    private static class MockStructuredValueObserver implements ObservableStructuredValue.Listener<Key, Integer> {

        Queue<Pair<Map<Key, Integer>, Map<Key, Integer>>> expectations = new LinkedList<Pair<Map<Key, Integer>, Map<Key, Integer>>>();

        public void expectValuesChanged(Map<Key, Integer> oldValues, Map<Key, Integer> newValues) {
        ...
        @Override
        public void onValuesChanged(Map<Key, ? extends Integer> oldValues, Map<Key, ? extends Integer> newValues) {
    ...
    public void testInitialiser() {
        Initializer init = DocumentBasedStructuredValue.createInitialiser(Serializer.INTEGER, CollectionUtils.immutableMap(Key.NAME1, 23, Key.NAME2, 42));
        TestUtil.assertInitializerValues(CollectionUtils.immutableMap(Key.NAME1.toString(), "23", Key.NAME2.toString(), "42"), init);
        init = DocumentBasedStructuredValue.createInitialiser(Serializer.INTEGER, CollectionUtils.immutableMap(Key.NAME1, (Integer) null));
    ...
    public void testSetSingleFromNullProducesEvent() {
        ...
        MockStructuredValueObserver observer = new MockStructuredValueObserver();
        value.addListener(observer);
        observer.expectValuesChanged(CollectionUtils.immutableMap(Key.NAME1, (Integer) null), CollectionUtils.immutableMap(Key.NAME1, 42));
        value.set(Key.NAME1, 42);
    ...
    public void testSetNullProducesEvent() {
        ...
        MockStructuredValueObserver observer = new MockStructuredValueObserver();
        value.addListener(observer);
        observer.expectValuesChanged(CollectionUtils.immutableMap(Key.NAME1, 10), CollectionUtils.immutableMap(Key.NAME1, (Integer) null));
        value.set(Key.NAME1, null);
        observer.checkExpectationsSatisfied();
    }
    ...
    public void testSetManyProducesOneEvent() {
        ...
        MockStructuredValueObserver observer = new MockStructuredValueObserver();
        value.addListener(observer);
        observer.expectValuesChanged(CollectionUtils.immutableMap(Key.NAME1, 10, Key.NAME2, 20), CollectionUtils.immutableMap(Key.NAME1, 5, Key.NAME2, 7));
   ...
   */
    ...
    private static <N> ObservableStructuredValue<Key, Integer> createTargetOn(ValueContext<N, ?> context) {
        return createTargetOn2(context);
    }

    private static <N, E extends N> ObservableStructuredValue<Key, Integer> createTargetOn2(ValueContext<N, E> context) {
        return DocumentBasedStructuredValue.create(DefaultDocumentEventRouter.create(context.doc), context.container, Serializer.INTEGER, Key.class);
    }
   ...
   */
    private static ValueContext<?, ?> substrate(Map<Key, Integer> values) {
        return substrate(BasicFactories.observableDocumentProvider().create("tagname", Collections.<String, String>emptyMap()), values);
    }
   ...
   * @return a map-context view of the document state.
   */
    private static <N, E extends N> ValueContext<N, E> substrate(ObservableMutableDocument<N, E, ?> doc, Map<Key, Integer> values) {
        E container = doc.createChildElement(doc.getDocumentElement(), CONTAINER_TAG, Collections.<String, String>emptyMap());
        for (Map.Entry<Key, Integer> entry : values.entrySet()) {
            doc.setElementAttribute(container, entry.getKey().toString(), Serializer.INTEGER.toString(entry.getValue()));
        }
   ...
   */
    ...
    private <N, E extends N> void setValue2(ValueContext<N, E> context, Key name, Integer value) {
        context.doc.setElementAttribute(context.container, name.toString(), Serializer.INTEGER.toString(value));
    }
   ...
   */
    ...
    private <N, E extends N> void setValues2(ValueContext<N, E> context, Map<Key, Integer> values) {
