...
public class PdfParser extends Parser {
    ...
    private static ParserFieldEnum[] fl = { ParserFieldEnum.title, ParserFieldEnum.author, ParserFieldEnum.subject, ParserFieldEnum.content, ParserFieldEnum.producer, ParserFieldEnum.keywords, ParserFieldEnum.creation_date, ParserFieldEnum.modification_date, ParserFieldEnum.language, ParserFieldEnum.number_of_pages, ParserFieldEnum.ocr_content };
    ...
    private void extractContent(PDDocument pdf) throws IOException {
        ...
        if (info != null) {
            ...
            if (d != null) addField(ParserFieldEnum.creation_date, d);
            d = getDate(getModificationDate(info));
            if (d != null) addField(ParserFieldEnum.modification_date, d);
        }
        ...
        for (String frag : frags) addField(ParserFieldEnum.content, StringUtils.replaceConsecutiveSpaces(frag, " ").trim());
    ...
    protected void parseContent(StreamLimiter streamLimiter, LanguageEnum lang) throws IOException {
        ...
        try {
            ...
            if (pdf.isEncrypted()) throw new IOException("Encrypted PDF.");
            extractContent(pdf);
    ...
    private void extractImagesForOCR(PDDocument pdf, LanguageEnum lang) throws IOException, SearchLibException {
        OcrManager ocr = ClientCatalog.getOcrManager();
        if (ocr == null || ocr.isDisabled()) return;
        if (!getFieldMap().isMapped(ParserFieldEnum.ocr_content)) return;
        ...
        while (iter.hasNext()) {
            ...
            if (images != null) {
                ...
                while (imageIter.hasNext()) {
                    ...
                    File imageFile = File.createTempFile("osspdfimg", '.' + image.getSuffix());
                    File textFile = File.createTempFile("ossocr", ".txt");
                    image.write2file(imageFile);
                    ocr.ocerize(imageFile, textFile, lang);
                    addField(ParserFieldEnum.ocr_content, FileUtils.readFileToString(textFile, "UTF-8"));
                    imageFile.delete();
                    textFile.delete();
                }
            }
