...
public class TestUtils {
   ...
   * correctly.
   */
    public static void fixItemIds(TopicMapIF tm, LocatorIF origbaseloc) throws MalformedURLException {
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                if (iid.startsWith(base)) {
                    if (AbstractTopicMapExporter.mayCollide(iid.substring(base.length() + 1))) topic.removeItemIdentifier(itemid); else {
        ...
        if (tm.getStore() instanceof InMemoryTopicMapStore) ((InMemoryTopicMapStore) tm.getStore()).setBaseAddress(origbaseloc);
    }
}
