...
/**
 * Classe contenant les op�rations sur les objets d'un treillis
 * @author Inconnu
 */
public class BasicObjectProjection extends BasicProjection {
    ...
    public ConceptLattice perform(BasicSet entry) {
        ...
        result = new ConceptLattice(topNode, "ProjectionOn" + entry.toString());
        return result;
    }

    /**
	 * Algorithme recursif permettant de construire le trellis Cette algorithme parcours
	 * r�cursivement les noeuds et lors du backtrack, marque les noeuds indispensable et les
    ...
    private FormalConcept performRec(FormalConcept n, BasicSet extent) {
        ...
        while (itPaire.hasNext()) {
            ...
            if (i2.equals(i1)) {
                FormalConcept nx;
                if (nodei2.hasBeenVisited()) nx = nodes.get(nodei2); else nx = performRec(nodei2, extent);
                n.setVisited(true);
                nodes.put(n, nx);
                return nx;
            }
    ...
    /**
	 * Cette classe permet de ranger les differents �lements dans un ordre precis. Les noeuds
	 * poss�dant les extents ayant le plus de relations avec les extents choisis sont plac�s devants
	 */
    private class Paire implements Comparable<Paire> {
        ...
        public int compareTo(Paire b) {
            if (nbExtentInt > b.nbExtentInt) return -1; else return 1;
        }
