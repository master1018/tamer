...
public class TestDoubleSpecial1 {
    ...
    private void testDouble1(double x, double y) {
        ...
        double z = x + y;
        if (x > 1.1) {
            assert pcMatches(PC1, pc) : makePCAssertString("TestDoubleSpecial1.testDouble1 if x > 1.1", PC1, TestUtils.getPathCondition());
            z = y;
        } else {
            assert (pcMatches(PC2, pc) || pcMatches(PC3, pc)) : makePCAssertString("TestDoubleSpecial1.testDouble1 x <= 1.1", "either\n" + PC2 + "\nor\n" + PC3, TestUtils.getPathCondition());
        }
        pc = trimPC(TestUtils.getPathCondition());
        if (z > 30.0) {
            assert (pcMatches(PC4, pc) || pcMatches(PC5, pc)) : makePCAssertString("TestDoubleSpecial1.testDouble1 z <= 30.0", "one of\n" + (PC4 + " && " + pc) + "\nor\n" + (PC5 + " && " + pc), TestUtils.getPathCondition());
            z = 91.0;
        } else {
            assert (pcMatches(PC6, pc) || pcMatches(PC7, pc) || pcMatches(PC8, pc) || pcMatches(PC9, pc)) : makePCAssertString("TestDoubleSpecial1.testDouble1 z <= 30.0", "one of\n" + (PC6 + " && " + pc) + "\nor\n" + (PC7 + " && " + pc) + "\nor\n" + (PC8 + " && " + pc) + "\nor\n" + (PC9 + " && " + pc), TestUtils.getPathCondition());
        }
        pc = trimPC(TestUtils.getPathCondition());
    }
