...
public class StrTable {
    ...
    public StrTable set(int rowIndex, String colName, String val) {
        ...
        if (rows.containsKey(rowIndex)) row = rows.get(rowIndex); else {
            row = new HashMap<String, String>();
            rows.put(rowIndex, row);
        }
        row.put(colName, val);
        int columnSize;
        if (!columns.containsKey(colName)) columnSize = Math.max(colName.length(), val.length()); else if (columns.get(colName) >= (columnSize = val.length())) return this;
    ...
    private static StringBuilder right(StringBuilder result, String s, int sz) {
        result.append(s);
        if ((sz -= s.length()) > 0) for (int i = 0; i < sz; i++) result.append(" ");
        return result;
    }
    ...
    public String toString() {
        ...
        if (columns.isEmpty()) return result.toString();
