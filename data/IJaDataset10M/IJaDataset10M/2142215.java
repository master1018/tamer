 ...
 *
 */
public class StripSentenceCorpusWrapper<T> extends CorpusWrapper<T, Tree<T>> {

    private int bufferSize = 15;

    public StripSentenceCorpusWrapper(Corpus<Tree<T>> master) throws IOException {
        super(master);
    }
    ...
    private class SSCIterator extends WrapperIterator implements SequenceIterator<T> {
        ...
        private void ensureBuffer() {
            while (buffer.size() < bufferSize && masterIterator.hasNext()) {
                for (T t : masterIterator.next().getLeaves()) buffer.add(new Token(t));
