...
public class Actions {
    ...
    public static Action openStock(StockAPINode node) {
        return new OpenStock(node);
    }

    public static Action addFolder(RootAPINode node) {
        return AddFolder.addFolder(node);
    }

    public static Action renameFolder() {
        return RenameFolder.getDefault();
    }

    private static class OpenStock extends AbstractAction implements HelpCtx.Provider {
        ...
        public void actionPerformed(ActionEvent e) {
            if (node != null) {
                final StockAPI stock = node.getLookup().lookup(StockAPI.class);
                if (stock != null) {
    ...
    private static class AddFolder extends NodeAction {
        ...
        public static Action addFolder(RootAPINode node) {
            if (ADDFOLDER == null) ADDFOLDER = new AddFolder(node);
            return ADDFOLDER;
        }
        ...
        protected void performAction(Node[] nodes) {
            ...
            if (ret.equals(DialogDescriptor.OK_OPTION)) {
                ...
                if ((folderName != null) && (folderName.hashCode() != "".hashCode())) {
                    ...
                    if (!root.getRoot().folderNameExists(folderName)) {
                        ...
                        Message message = new DialogDescriptor.Message("Folder already exists.", DialogDescriptor.INFORMATION_MESSAGE);
                        DialogDisplayer.getDefault().notify(message);
                    }
                } else {
                    Message message = new DialogDescriptor.Message("Invalid folder name.", DialogDescriptor.WARNING_MESSAGE);
                    DialogDisplayer.getDefault().notify(message);
                }
            }
        }
    ...
    private static class RenameFolder extends NodeAction {
        ...
        protected void performAction(Node[] nodes) {
            ...
            if (nodes.length > 1) {
                Message message = new DialogDescriptor.Message("You can rename only one folder.", DialogDescriptor.INFORMATION_MESSAGE);
                DialogDisplayer.getDefault().notify(message);
                return;
            }
        ...
        protected boolean enable(Node[] nodes) {
            ...
            for (int i = 0; i < nodes.length; i++) if (!(nodes[i] instanceof FolderAPINode)) return false;
            return true;
        }
