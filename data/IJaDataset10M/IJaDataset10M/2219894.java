...
public class StackTracerVisitor extends JavassistInstructionVisitor {
    ...
    private void checkType(CtClass expected, CtClass actual) {
        if (doCheck) {
            try {
                if (actual != null && !actual.subtypeOf(expected)) throw new RuntimeException("Type missmatch: expected: " + expected.getName() + ", actual: " + actual.getName() + "!");
                if (actual == null && !expected.subtypeOf(objectType)) {
                    throw new RuntimeException("Type missmatch: expected: " + expected.getName() + ", actual: null!");
                }
            } catch (NotFoundException e) {
    ...
    private void checkArray(CtClass actual) {
        if (doCheck) {
            if (!actual.isArray()) throw new RuntimeException("Type missmatch: array expected!");
        }
    }
    ...
    private int jvmCategory(CtClass type) {
        if (type == doubleType || type == longType) return 2; else return 1;
    }
    ...
    public void postVisit(BytecodeInstruction instr) {
        if (lookingAhead) return;
        if (arrayPushed) computeConsumedBy();
    }
