...
public class ComplementNaiveBayes extends Classifier implements OptionHandler, WeightedInstancesHandler {
    ...
    */
    ...
    public String[] getOptions() {
        ...
        int current = 0;
        if (getNormalizeWordWeights()) options[current++] = "-N";
        options[current++] = "-S";
    ...
    public void setOptions(String[] options) throws Exception {
        ...
        if (val.length() != 0) setSmoothingParameter(Double.parseDouble(val));
    }
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        for (int c = 0; c < numClasses; c++) {
            for (int idx = 0; idx < numAttributes; idx++) {
                if (instances.classIndex() == idx) {
                    if (!instances.attribute(idx).isNominal()) throw new Exception("ComplementNaiveBayes cannot " + "handle non-nominal class attribute");
                } else if (!instances.attribute(idx).isNumeric()) throw new Exception("Attribute " + instances.attribute(idx).name() + " is not numeric! " + "ComplementNaiveBayes requires " + "that all attributes (except the " + "class attribute) are numeric.");
            }
        }
        ...
        while (enumInsts.hasMoreElements()) {
            ...
            for (int a = 0; a < instance.numValues(); a++) if (instance.index(a) != instance.classIndex()) {
                if (!instance.isMissing(a)) {
                    ...
                    if (numOccurrences < 0) throw new Exception("Numeric attribute" + " values must all be greater" + " or equal to zero.");
        ...
        if (m_normalizeWordWeights == true) for (int c = 0; c < numClasses; c++) {
            double sum = 0;
            for (int w = 0; w < numAttributes; w++) {
                if (w != classIndex) sum += Math.abs(wordWeights[c][w]);
            }
            for (int w = 0; w < numAttributes; w++) {
                if (w != classIndex) {
    ...
    public double classifyInstance(Instance instance) throws Exception {
        if (wordWeights == null) throw new Exception("Error. The classifier has not been built " + "properly.");
        ...
        int minidx = 0;
        for (int i = 0; i < numClasses; i++) if (valueForClass[i] < valueForClass[minidx]) minidx = i;
        return minidx;
    }
    ...
    public String toString() {
        ...
        for (int c = 0; c < numClasses; c++) result.append(header.classAttribute().value(c)).append("\t");
        result.append("\n");
        for (int w = 0; w < numAttributes; w++) {
            result.append(header.attribute(w).name()).append("\t");
            for (int c = 0; c < numClasses; c++) result.append(Double.toString(wordWeights[c][w])).append("\t");
            result.append("\n");
        }
