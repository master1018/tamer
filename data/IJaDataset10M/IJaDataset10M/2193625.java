...
public class Flash {
    ...
    public void interrogate() {
        ...
        while (go) {
            ...
            for (int i = 0; i < flashCards.length; i++) {
                String question;
                if (keysFirst) question = flashCards[i].key; else question = flashCards[i].value;
                ...
                if (Misc.isEmpty(response)) i--; else {
                    if (keysFirst && response.toLowerCase().equalsIgnoreCase(flashCards[i].value)) {
                        ...
                        if (flashCards[i].numFailed > 0) {
                            ...
                            if (keysFirst) sb.append(" - " + flashCards[i].value); else sb.append(" - " + flashCards[i].key);
                        }
                        sb.append("\n");
                        textFrame.append(sb.toString());
                        flashCards[i].numFailed++;
                        if (queryTilCorrect) i--; else complete = false;
                    }
                }
            }
            if (complete) {
    ...
    public static void main(String[] args) {
        if (args.length == 0) Misc.printExit("\nEnter the full path text for a tab delimited text file " + "containing columns of key, value, and optionally notes or hints.\n");
        new Flash(args);
    }
    ...
    public FlashCard[] parseFile(File file) {
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (tokens.length == 3) al.add(new FlashCard(tokens[0].trim(), tokens[1].trim(), tokens[2].trim())); else if (tokens.length == 2) al.add(new FlashCard(tokens[0].trim(), tokens[1].trim(), ""));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
