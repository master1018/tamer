 ...
 */
public class MoleculeScene extends AbstractScene implements MoleculeStateChangeListener, PropertySceneChangeListener {
    ...
    protected transient MoleculeSceneChangeListener moleculeSceneChangeListener = null;
            ...
            public void atomInfoChanged(AtomInfoChangeEvent aice) {
                switch(aice.getChangeType()) {
                    case AtomInfoChangeEvent.COLOR_VALUE:
                        ...
                        while (items.hasNext()) {
                            sa = items.next();
                            sa.setColor(atomInfo.getColor(sa.getAtom().getSymbol()));
                        }
                    ...
                    case AtomInfoChangeEvent.ALL_CANGED:
                        MoleculeStateChangeEvent msce = new MoleculeStateChangeEvent(MoleculeScene.this.molecule);
        ...
        for (int i = 0; i < noOfAtoms; i++) {
            ...
            while (connAtoms.hasMoreElements()) {
                ...
                if (connList.get(theIndex).equals(BondType.SINGLE_BOND)) {
                    if (enable3D) {
                        screenBond = new SingleScreenBond3D(screenAtom, screenAtoms.get(index));
                    } else {
                        screenBond = new SingleScreenBond(screenAtom, screenAtoms.get(index));
                    }
                    ...
                    screenBond = new WeakScreenBond(screenAtom, screenAtoms.get(index));
                    screenBonds.add(screenBond);
                    screenAtom.getScreenBondList().add(screenBond);
                    ...
                    if (enable3D) {
                        ...
                        screenBond = new MultipleScreenBond(screenAtom, screenAtoms.get(index));
                    }
                    screenBonds.add(screenBond);
                    screenAtom.getScreenBondList().add(screenBond);
                }
    ...
    private void sortZBuffer() {
        ...
        for (i = noOfAtoms - 1; --i >= 0; ) {
            ...
            for (j = 0; j <= i; j++) {
                a = zBuffer[j];
                b = zBuffer[j + 1];
                if (screenAtoms.get(a).getCurrentZ() > screenAtoms.get(b).getCurrentZ()) {
                    zBuffer[j + 1] = a;
                    zBuffer[j] = b;
                    flipped = true;
                }
    ...
    public void draw(Graphics2D g2d) {
        ...
        if (propertyScenes != null) {
            for (PropertyScene scene : propertyScenes) {
                if (scene.getType() == PropertyScene.PropertySceneType.TEXTURED_PLANAR_CUT) {
                    scene.draw(g2d);
                } else if (scene.getType() == PropertyScene.PropertySceneType.TEXTURED_VDW_SURFACE) {
                    scene.draw(g2d);
                }
            }
        }
        ...
        if (propertyScenes != null) {
            for (PropertyScene scene : propertyScenes) {
                if ((scene.getType() != PropertyScene.PropertySceneType.TEXTURED_PLANAR_CUT) && (scene.getType() != PropertyScene.PropertySceneType.TEXTURED_VDW_SURFACE)) {
                    scene.draw(g2d);
                }
            }
        }
     ...
     */
    public synchronized Molecule getTransformedMolecule(Matrix3D tMat) throws Exception {
    ...
    public void selectAtomIndex(int atomIndex, boolean select) {
        ...
        if (select) selectionStack.push(atom); else selectionStack.remove(atom);
    }
        ...
        for (int i = screenAtoms.size(); --i >= 0; ) {
            ...
            if (atom.contains(x, y)) {
                ...
                if (noDepth) {
                    ...
                    for (int j = 0; j < molecule.getNumberOfAtoms(); j++) pathList.add(j);
    ...
    public void invertSelection() {
        ...
        for (int i = screenAtoms.size(); --i >= 0; ) {
            atom = screenAtoms.get(zBuffer[i]);
            if (atom.isSelected()) atom.setSelected(false); else {
                atom.setSelected(true);
     ...
     */
    public void selectIfContainedIn(Point3D center, double radius, boolean clearPrevious) {
        if (!isVisible()) return;
    ...
    public ScreenAtom getClosestSelectedAtom(ScreenAtom sa) {
        ...
        for (int i = 0; i < selectionStack.size(); i++) {
            ...
            if (atom == sa) continue;
            if (atom.getAtom().getIndex() == sa.getAtom().getIndex()) continue;
        ...
        if (minIdx == -1) return null; else return selectionStack.get(minIdx);
    }
    ...
    public void moleculeChanged(MoleculeStateChangeEvent msce) {
        ...
        System.out.println("MoleculeScene => Molecule Changed : " + msce + "; from : " + msce.getSource());
        switch(msce.getEventType()) {
            ...
            case MoleculeStateChangeEvent.MAJOR_MODIFICATION:
                try {
                    MoleculeBuilder mb = (MoleculeBuilder) Utility.getDefaultImplFor(MoleculeBuilder.class).newInstance();
                    mb.rebuildConnectivity(msce);
                } catch (Exception e) {
                    System.err.println("MoleculeScene => " + "Error while refresing geometry : " + e.toString());
                    e.printStackTrace();
                }
                break;
        }
    ...
    public void addDistanceTracker(int atomIndex1, int atomIndex2) {
        trackers.add(new DistanceTracker(screenAtoms.get(atomIndex1), screenAtoms.get(atomIndex2)));
        fireMoleculeSceneChangeListenerSceneChanged(sceneChangeEvent);
    }
     ...
     */
    public void addAngleTracker(int atomIndex1, int atomIndex2, int atomIndex3) {
        trackers.add(new AngleTracker(screenAtoms.get(atomIndex1), screenAtoms.get(atomIndex2), screenAtoms.get(atomIndex3)));
        fireMoleculeSceneChangeListenerSceneChanged(sceneChangeEvent);
    }
     ...
     */
    public void addDihedralTracker(int atomIndex1, int atomIndex2, int atomIndex3, int atomIndex4) {
        trackers.add(new DihedralTracker(screenAtoms.get(atomIndex1), screenAtoms.get(atomIndex2), screenAtoms.get(atomIndex3), screenAtoms.get(atomIndex4)));
        fireMoleculeSceneChangeListenerSceneChanged(sceneChangeEvent);
    }
     ...
     */
    public void showFragment(FragmentationScheme scheme, Fragment fragment, boolean showIt) {
        if (fragmentSceneMap == null) {
            fragmentSceneMap = new HashMap<FragmentationScheme, ArrayList<FragmentScene>>();
        }
     ...
     */
    public boolean isFragmentVisible(FragmentationScheme scheme, Fragment fragment) {
        if (fragmentSceneMap == null) {
            return false;
        }
        if (fragmentSceneMap.containsKey(scheme)) {
    ...
    public void addPropertyScene(PropertyScene ps) {
        if (propertyScenes == null) propertyScenes = new ArrayList<PropertyScene>();
     ...
     */
    public Molecule getSelectionAsMolecule() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        if (selectionStack == null) return null;
        if (selectionStack.size() == 0) return null;
        Molecule mol = (Molecule) Utility.getDefaultImplFor(Molecule.class).newInstance();
     ...
     * @throws IllegalAccessException if unable to instantiate Molecule class
     */
    public Molecule getSelectionAsNewMolecule() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        if (selectionStack == null) return null;
        if (selectionStack.size() == 0) return null;
        Molecule mol = (Molecule) Utility.getDefaultImplFor(Molecule.class).newInstance();
        mol.setTitle(this.molecule.getTitle() + "-part");
        ...
        while (selections.hasNext()) {
            ...
            mol.addAtom(new Atom(atom.getSymbol(), atom.getCharge(), atom.getAtomCenter(), index));
            index++;
        }
        return mol;
    }
     ...
     */
    public void setSelectionStack(Stack<ScreenAtom> selectionStack, boolean compareEach) {
        ...
        if (!compareEach) {
            ...
            for (ScreenAtom sa1 : screenAtoms) {
                for (ScreenAtom sa2 : selectionStack) {
                    if ((sa2.getAtom() == sa1.getAtom()) || (sa2.getAtom().equals(sa1.getAtom()))) {
     ...
     *
     */
    public synchronized void addMoleculeSceneChangeListener(MoleculeSceneChangeListener listener) {
     ...
     *
     */
    public synchronized void removeMoleculeSceneChangeListener(MoleculeSceneChangeListener listener) {
        if (listenerList != null) {
     ...
     *
     */
    protected void fireMoleculeSceneChangeListenerSceneChanged(MoleculeSceneChangeEvent event) {
            ...
            for (FragmentScene fScene : fragmentSceneMap.get(scheme)) {
                if (fScene.isVisible()) {
                    ...
                    for (ScreenAtom sa : selectionStack) {
                        ...
                        FragmentAtomEditEvent faee = new FragmentAtomEditEvent(this);
                        faee.setMolecule(molecule);
                        faee.setFragmentationScheme(scheme);
                        faee.setFragment(frag);
                        faee.setFragmentAtom(fa);
                        faee.setEventType(FragmentAtomEditEvent.FragmentAtomEditEventType.ATOM_ADDED);
    ...
    public void addSelectionAsFragments(FragmentAtomEditListener fael) {
        try {
            Iterator<FragmentationScheme> fss = molecule.getAllFragmentationSchemes();
            while (fss.hasNext()) {
                FragmentationScheme scheme = fss.next();
                Fragment frag = (Fragment) Utility.getDefaultImplFor(Fragment.class).newInstance();
                for (ScreenAtom sa : selectionStack) {
                    FragmentAtom fa = new FragmentAtom(sa.getAtom());
                    frag.addFragmentAtom(fa);
                    if (fael == null) continue;
     ...
     *        notify any changes
     */
    public void removeSelectionFromVisibleFragments(FragmentAtomEditListener fael) {
        if (fragmentSceneMap == null) {
            return;
        }
        for (FragmentationScheme scheme : fragmentSceneMap.keySet()) {
            for (FragmentScene fScene : fragmentSceneMap.get(scheme)) {
                if (fScene.isVisible()) {
                    ...
                    for (ScreenAtom sa : selectionStack) {
                        ...
                        faee.setFragment(frag);
                        faee.setFragmentAtom(fa);
                        faee.setEventType(FragmentAtomEditEvent.FragmentAtomEditEventType.ATOM_REMOVED);
                        fael.fragmentAtomEdited(faee);
                    }
                }
            }
