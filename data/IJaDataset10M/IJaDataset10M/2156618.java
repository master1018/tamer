...
public class MomentOfInertiaDescriptor implements IMolecularDescriptor {
    ...
    public DescriptorSpecification getSpecification() {
        return new DescriptorSpecification("http://www.blueobelisk.org/ontologies/chemoinformatics-algorithms/#momentOfInertia", this.getClass().getName(), "$Id: c64636ae7f6d9abcd8c6e81d90715d1bb45a0ce9 $", "The Chemistry Development Kit");
    }
    ...
    public DescriptorValue calculate(IAtomContainer container) {
        if (!GeometryTools.has3DCoordinates(container)) return getDummyDescriptorValue(new CDKException("Molecule must have 3D coordinates"));
        ...
        eval[0] = eval[2];
        eval[2] = etmp;
        if (Math.abs(eval[1]) > 1e-3) retval.add(eval[0] / eval[1]); else retval.add(1000);
        ...
        if (Math.abs(eval[2]) > eps) pri = Math.pow(eval[0] * eval[1] * eval[2], 1.0 / 3.0); else pri = Math.sqrt(eval[0] * ccf / MolecularFormulaManipulator.getTotalExactMass(formula));
