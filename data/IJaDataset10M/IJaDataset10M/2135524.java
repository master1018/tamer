...
public class MclStorageManager implements MetaContactListListener {
    ...
    private static final Logger logger = Logger.getLogger(MclStorageManager.class.getName());
     ...
     */
    void start(BundleContext bc, MetaContactListServiceImpl mclServiceImpl) throws IOException, XMLException {
        bundleContext = bc;
     ...
     */
    private void initVirginDocument(MetaContactListServiceImpl mclServiceImpl, Document contactListDocument) {
     ...
     */
    private void processGroupXmlNode(MetaContactListServiceImpl mclServiceImpl, String accountID, Element groupNode, MetaContactGroup parentGroup, Map parentProtoGroups) {
        ...
        for (int i = 0; childContacts != null && i < childContacts.getLength(); i++) {
            ...
            try {
                ...
                mclServiceImpl.loadStoredMetaContact(currentMetaGroup, uid, displayName, protoContacts, accountID);
            } catch (Throwable thr) {
     ...
     */
    private List extractProtoContacts(Element metaContactNode, String accountID, Map protoGroups) {
    ...
    private Element createProtoContactGroupNode(ContactGroup protoGroup) {
        ...
        protoGroupElement.setAttribute(PARENT_PROTO_GROUP_UID_ATTR_NAME, protoGroup.getParentContactGroup().getUID());
        Element persDataNode = contactListDocument.createElement(PERSISTENT_DATA_NODE_NAME);
    ...
    public void metaContactMoved(MetaContactMovedEvent evt) {
        ...
        if (metaContactNode == null) {
            logger.error("Save after metacontact moved. Contact not found: " + evt.getSourceMetaContact());
            return;
        }
    ...
    static class StoredProtoContactDescriptor {
        ...
        public String toString() {
            return "StoredProtocoContactDescriptor[ " + " contactAddress=" + contactAddress + " persistenData=" + persistentData + " parentProtoGroup=" + ((parentProtoGroup == null) ? "" : parentProtoGroup.getGroupName()) + "]";
        }
         ...
         * exists.
         */
        private static StoredProtoContactDescriptor findContactInList(String contactAddress, List list) {
            if (list == null || list.size() == 0) return null;
            Iterator iter = list.iterator();
            while (iter.hasNext()) {
                StoredProtoContactDescriptor desc = (StoredProtoContactDescriptor) iter.next();
                if (desc.contactAddress.equals(contactAddress)) return desc;
            }
            return null;
        }
    }
