...
public class File4 {
    ...
    public static byte[] readAllBytes(final String fname) throws FileNotFoundException, IOException {
    ...
    private static void copyDirectory(File source, File target) throws IOException {
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                copy(Path4.combine(source.getAbsolutePath(), files[i]), Path4.combine(target.getAbsolutePath(), files[i]));
            }
        }
    }
