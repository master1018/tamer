...
public class TemplateComposer extends AbstractComposer<TemplatedMailContext> {
    ...
    private final String textTemplateSuffix;

    private final String htmlTemplateSuffix;
	 ...
	 */
    @Inject
    public TemplateComposer(@MailTemplatePath String baseTemplatePath, @MailTemplateSuffixText String textTemplateSuffix, @MailTemplateSuffixHtml String htmlTemplateSuffix, VelocityEngine velocityEngine) {
        super();
    ...
    public void composeImpl(MimeMessageHelper helper, TemplatedMailContext context) throws MailPreparationException {
        try {
            ...
            throw new MailPreparationException("Unable to compose templated mail content due to an I/O exception: " + ioe.getMessage(), ioe);
        } catch (final VelocityException ve) {
            throw new MailPreparationException("Unable to compose templated mail content due to a Velocity exception: " + ve.getMessage(), ve);
        } catch (final MessagingException me) {
            throw new MailPreparationException("unable to compose templated mail content due to a messaging exception: " + me.getMessage(), me);
        }
    }
}
