...
public class LTexture implements LRelease {
    ...
    public final void checkReplace() {
        this.replace = Format.BILINEAR == format || Format.BILINEAR == format || Format.REPEATING_BILINEAR == format;
    ...
    private synchronized void loadTextureBuffer() {
        ...
        GLEx.gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, srcPixelFormat, imageData.texWidth, imageData.texHeight, 0, srcPixelFormat, GL.GL_UNSIGNED_BYTE, imageData.source);
        if (GLEx.isVbo()) {
    ...
    public void draw(BufferedImage bit, int x, int y) {
        if (GLEx.gl != null) {
            ...
            while (height >= 1 || width >= 1 && level < 4) {
                GLEx.gl10.glTexSubImage2D(GL11.GL_TEXTURE_2D, level, x, y, width, height, srcPixelFormat, GL11.GL_UNSIGNED_BYTE, data.getSource());
                if (height == 1 || width == 1) {
                    break;
                }
                level++;
                ...
                BufferedImage bitmap2 = GraphicsUtils.getResize(bit, width, height);
                bit.flush();
                bit = null;
                bit = bitmap2;
            }
    ...
    public void draw(LPixmap pixmap, int x, int y) {
        ...
        if (GLEx.gl != null) {
            ...
            GLEx.gl.glTexSubImage2D(GL.GL_TEXTURE_2D, 0, x, y, width, height, srcPixelFormat, GL.GL_UNSIGNED_BYTE, buffer);
            if (GLEx.isVbo()) {
                GLEx.updateHardwareBuff(this);
            }
        }
    ...
    public void setFormat(Format format) {
        ...
        gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, minFilter);
        gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, maxFilter);
        if (GLContext.getCapabilities().GL_EXT_texture_mirror_clamp) {
            gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_S, EXTTextureMirrorClamp.GL_MIRROR_CLAMP_TO_EDGE_EXT);
            gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_T, EXTTextureMirrorClamp.GL_MIRROR_CLAMP_TO_EDGE_EXT);
        } else {
    ...
    public void setVertCords(int width, int height) {
        if (dataCords == null) {
            dataCords = new float[] { 0.0f, 0.0f, width, 0.0f, 0.0f, height, width, height, xOff, yOff, widthRatio, yOff, xOff, heightRatio, widthRatio, heightRatio };
    ...
    public void setTexCords(float texXOff, float texYOff, float texWidthRatio, float texHeightRatio) {
        if (dataCords == null) {
            dataCords = new float[] { 0.0f, 0.0f, imageData.width, 0.0f, 0.0f, imageData.height, imageData.width, imageData.height, xOff, yOff, widthRatio, yOff, xOff, heightRatio, widthRatio, heightRatio };
            data = BufferUtils.createFloatBuffer(dataCords);
            dataSize = data.capacity() * 4;
            vertexSize = 8 * 4;
            texSize = 8 * 4;
    ...
    public LTexture getSubTexture(final int x, final int y, final int width, final int height) {
        this.loadTexture();
        ...
        sub.widthRatio = (((float) width / LTexture.this.width) * widthRatio) + sub.xOff;
        sub.heightRatio = (((float) height / LTexture.this.height) * heightRatio) + sub.yOff;
    ...
    private LTexture copy(int width, int height, boolean flipHorizontal, boolean flipVertial) {
        this.loadTexture();
    ...
    public final LImage getImage() {
        ...
        if (imageData != null) {
            if (imageData.pixels != null) {
                ...
                if (data != null) {
                    image = createPixelImage(data, imageData.texWidth, imageData.texHeight, imageData.width, imageData.height, imageData.hasAlpha);
                }
            } else if (imageData.pixmap != null) {
                int[] data = imageData.pixmap.getData();
                if (data != null) {
                    image = createPixelImage(data, imageData.pixmap.getTexWidth(), imageData.pixmap.getTexHeight(), imageData.pixmap.getWidth(), imageData.pixmap.getHeight(), imageData.pixmap.hasAlpha());
                }
    ...
    private static final LImage createPixelImage(int[] pixels, int texWidth, int texHeight, int width, int height, boolean alpha) {
    ...
    public void setColor(int corner, float r, float g, float b, float a) {
        if (colors == null) {
            colors = new GLColor[] { new GLColor(1, 1, 1, 1f), new GLColor(1, 1, 1, 1f), new GLColor(1, 1, 1, 1f), new GLColor(1, 1, 1, 1f) };
        }
        colors[corner].r = r;
        colors[corner].g = g;
        colors[corner].b = b;
    ...
    public void draw(float x, float y, float width, float height, float x1, float y1, float x2, float y2) {
        if (GLEx.self != null) {
            GLEx.self.draw(this, colors, x, y, width, height, x1, y1, x2, y2, false);
        }
    }
