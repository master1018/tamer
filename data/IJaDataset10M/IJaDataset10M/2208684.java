...
public class MD5Crypter {
    ...
    private static MD5Crypter instance;

    private MD5Crypter() {
        md5Init();
    }

    public static synchronized MD5Crypter getInstance() {
        if (instance == null) {
            instance = new MD5Crypter();
        }
        return instance;
    }
    ...
    private void md5Final() {
        ...
        int index, padLen;
        longsToBytes(bits, count, 8);
        index = (int) (count[0] >>> 3) & 0x3f;
        padLen = (index < 56) ? (56 - index) : (120 - index);
        md5Update(PADDING, padLen);
        md5Update(bits, 8);
        longsToBytes(digest, state, 16);
    }
    ...
    private void longsToBytes(byte[] output, long[] input, int len) {
        int i, j;
    ...
    private void bytesTolongs(long[] output, byte[] input, int len) {
        int i, j;
	 ...
	 * @return
	 */
    public static long b2iu(byte bt) {
        return bt < 0 ? bt & 0x7F + 128 : bt;
    }

    /**	byteHEX()，用来把一个byte类型的数转换成十六进制的ASCII表示，因为java中的byte的toString
	 * 无法实现这一点，我们又没有C语言中的sprintf(outbuf,"%02X",ib)
	 * @param bt
	 * @return
	 */
    public static String byteHEX(byte bt) {
    ...
    public static void main(String[] args) {
        System.out.println(MD5Crypter.getInstance().encode("890218"));
    }
}
