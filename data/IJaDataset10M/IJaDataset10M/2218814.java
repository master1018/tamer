...
public class CredentialController extends CrawlerController {
    ...
    private class DeleteAlert extends AlertController {
        ...
        protected DeleteAlert(CredentialItem deleteCredential) throws InterruptedException {
            super("Please, confirm that you want to delete the credential: " + deleteCredential.getPattern(), Messagebox.YES | Messagebox.NO, Messagebox.QUESTION);
    ...
    public List<CredentialItem> getCredentialList() {
        synchronized (this) {
            if (credentialList != null) return credentialList;
            try {
                Client client = getClient();
                if (client == null) return null;
                CredentialManager credentialManager = client.getWebCredentialManager();
                credentialList = new ArrayList<CredentialItem>();
                totalSize = credentialManager.getCredentials(like, getActivePage() * getPageSize(), getPageSize(), credentialList);
    ...
    public void onDelete() throws SearchLibException, InterruptedException {
        ...
        if (!isWebCrawlerEditPatternsRights()) throw new SearchLibException("Not allowed");
        new DeleteAlert(selectedCredential);
        onCancel();
    }

    public void onSave() throws InterruptedException, SearchLibException, MalformedURLException {
        Client client = getClient();
        ...
        } else selectedCredential.copy(currentCredential);
        client.saveJobs();
        onCancel();
    }
}
