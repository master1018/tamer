...
public class ContextIterable<T> extends RandomIterable<T> implements ContextListener<T> {
	 ...
	 */
    public T next() {
        if ((numToReturn - numReturned) == 1 && context != null) context.removeContextListener(this);
        return super.next();
    }
    ...
    public void eventOccured(ContextEvent<T> ev) {
        if (ev.getType() == ContextEvent.EventType.AGENT_REMOVED) removeEvent(ev.getTarget());
    }
}
