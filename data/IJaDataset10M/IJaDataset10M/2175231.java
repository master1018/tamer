...
public class MRetSchemaConfig extends X_C_RetSchema_Config {
    ...
    public static Object getRangeApplyValue(Integer retSchemaConfigID, BigDecimal amt, String trxName) {
        if (Util.isEmpty(retSchemaConfigID, true) || amt == null) return null;
        Object valueObj = DB.getSQLObject(trxName, "SELECT value_apply FROM c_retschema_range WHERE c_retschema_config_id = ? AND ? BETWEEN value_from AND value_to ORDER BY value_from", new Object[] { retSchemaConfigID, amt });
        return valueObj;
    }
    ...
    public BigDecimal getValorBigDecimal() {
        ...
        if (getParamType().equals(PARAMTYPE_Valor)) value = new BigDecimal(getValor().replaceAll(",", "."));
        return value;
    }
    ...
    public String getValorString() {
        String value = null;
        if (getParamType().equals(PARAMTYPE_Valor)) value = getValor();
        return value;
    }
    ...
    public List<X_C_RetSchema_Range> getRanges(boolean reload) {
        if (ranges == null || reload) {
            ...
            String sql = " SELECT * " + " FROM C_RetSchema_Range " + " WHERE C_RetSchema_Config_ID = ? ";
            try {
                PreparedStatement pstmt = DB.prepareStatement(sql);
                pstmt.setInt(1, getC_RetSchema_Config_ID());
                ResultSet rs = pstmt.executeQuery();
                while (rs.next()) {
                    X_C_RetSchema_Range retSchemaRange = new X_C_RetSchema_Range(getCtx(), rs, get_TrxName());
