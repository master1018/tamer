...
public class IGStaticValue extends IGOperation {
    ...
    public int getInt(SourceLocation aSrc) throws ZamiaException {
        if (!getType().isInteger()) throw new ZamiaException("IGStaticValue: Integer type expected here.", aSrc);
        return fNum.intValue();
    }
    ...
    public IGStaticValue getValue(int aIdx, SourceLocation aSrc) throws ZamiaException {
        ...
        if (fArrayValues == null) return null;
    ...
    public boolean isTrue() {
        if (fIsBuiltinBool) return fTruthValue;
        return fEnumOrd == 1;
    }
    ...
    public static IGStaticValue computeBinary(IGStaticValue aA, IGStaticValue aB, BinOp aOp, IGTypeStatic aResType, SourceLocation aSrc, ASTErrorMode aErrorMode, ErrorReport aReport) throws ZamiaException {
        ...
        switch(tA.getCat()) {
            case ARRAY:
                ...
                switch(aOp) {
                    ...
                    case EQUAL:
                        if (c1 != c2) {
                            return aResType.getEnumLiteral(0, aSrc, aErrorMode, aReport);
                        }
                        for (int i = 0; i < c1; i++) {
                        ...
                        return aResType.getEnumLiteral(1, aSrc, aErrorMode, aReport);
                    case NEQUAL:
                        if (c1 != c2) {
                            return aResType.getEnumLiteral(1, aSrc, aErrorMode, aReport);
                        }
            ...
            if (curC == BIT_U || newC == BIT_U) {
                ...
                switch(curC) {
                    ...
                    case BIT_L:
                        if (newC == BIT_X || newC == BIT_DC) {
                            curC = BIT_X;
                        } else if (newC == BIT_0 || newC == BIT_1 || newC == BIT_W) {
                            curC = newC;
                        } else if (newC == BIT_H) {
                            curC = BIT_W;
                        }
    ...
    public static BigInteger getBigInt(String aString) {
        ...
        for (int i = 0; i < w; i++) {
            char bit = aString.charAt(i);
            if (getLong(bit) != 0) res = res.multiply(two).add(one); else res = res.multiply(two);
        }
        return res;
    }
    ...
    public static String convert(BigInteger aNum, int aWidth) {
        ...
        for (int i = width - 1; i >= 0; i--) {
            if (aNum.testBit(i)) ret.append(BIT_1); else ret.append(BIT_0);
        }
        return ret.toString();
    }
