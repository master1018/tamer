...
public class SimpleTag extends AbstractTag {
    ...
    public static Object[] SPEC_STRING_TO_TYPE = { Boolean.class.getName(), new Integer(TYPE_BOOLEAN), Byte.class.getName(), new Integer(TYPE_BYTE), Character.class.getName(), new Integer(TYPE_CHARACTER), Double.class.getName(), new Integer(TYPE_DOUBLE), Float.class.getName(), new Integer(TYPE_FLOAT), Integer.class.getName(), new Integer(TYPE_INTEGER), Long.class.getName(), new Integer(TYPE_LONG), Short.class.getName(), new Integer(TYPE_SHORT), String.class.getName(), new Integer(TYPE_STRING) };

    public static StringIntMap ourStringToType = new StringIntMap(SPEC_STRING_TO_TYPE);

    public static int stringToType(String s) {
        return ourStringToType.stringToInt(s);
    }

    public void writeAsAttribute(XmlSerializer xser, IndentingPrintWriter out, String name, Object value) {
        int id = xser.addObject(value);
        String[] attrs = { STR_ATTR_CLASS, value.getClass().getName(), STR_ATTR_ID, Integer.toString(id), STR_ATTR_VALUE, value.toString() };
        printElement(out, name, attrs, true, true);
    }

    public void writeAsChild(XmlSerializer xser, IndentingPrintWriter out, String name, Object value) {
        int id = xser.addObject(value);
        String[] attrs = { STR_ATTR_CLASS, value.getClass().getName(), STR_ATTR_ID, Integer.toString(id) };
        printElement(out, name, attrs, false, false);
        out.print(escapeCharacters(value.toString()));
        printClosingElement(out, name);
    }

    public void write(XmlSerializer xser, IndentingPrintWriter out, String name, Object value, boolean printClassName) throws LTSException {
        if (value instanceof String || xser.stringSerializePrimitives()) writeAsChild(xser, out, name, value); else writeAsAttribute(xser, out, name, value);
    }
    ...
    public Object read(XmlSerializer xser, Element node, boolean forgiving) throws LTSException {
        ...
        if (null == value || "".equals(value)) value = XMLUtils.getChildText(node);
        Object o = null;
