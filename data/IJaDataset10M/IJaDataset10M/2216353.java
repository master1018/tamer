...
public class ParallelPlotter extends PlotterAdapter implements MouseListener {
    ...
    public void setPlotColumn(int index, boolean plot) {
        if (plot) this.colorColumn = index; else this.colorColumn = -1;
        repaint();
    }
        ...
        switch(columnMappingType) {
            case WEIGHTS:
                if (this.dataTable.isSupportingColumnWeights()) {
                    ...
                    for (int i = 0; i < this.dataTable.getNumberOfColumns(); i++) {
                        if (colorColumn != i) indices.add(new WeightIndex(i, Math.abs(this.dataTable.getColumnWeight(i))));
                    }
            ...
            case RANDOM:
                ...
                for (int i = 0; i < mapping.length; i++) {
                    mapping[i] = i;
                    if (colorColumn != i) indices.add(i);
                }
    ...
    public void paintParallelPlot(Graphics g) {
        ...
        prepareData();
        if ((colorColumn >= 0) && (colorColumn < min.length) && !Double.isInfinite(min[colorColumn]) && !Double.isInfinite(max[colorColumn]) && (dataTable.isNominal(colorColumn) || (min[colorColumn] != max[colorColumn])) && (dataTable.getNumberOfRows() > 0)) {
    ...
    private void draw(Graphics g, int pixWidth, int pixHeight) {
        if (this.dataTable.isSupportingColumnWeights()) drawWeights(g, pixWidth, pixHeight);
