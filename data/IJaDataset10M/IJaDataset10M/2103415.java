...
public class TreeViewOverview extends Canvas implements ITreeChangeListener {
            ...
            synchronized (TreeViewOverview.this) {
                if (mTree != null) {
                    ...
                    if (mViewport != null) {
                        ...
                        e.gc.fillRectangle((int) mViewport.x, (int) mViewport.y, (int) Math.ceil(mViewport.width), (int) Math.ceil(mViewport.height));
                        e.gc.setAlpha(255);
                        e.gc.setForeground(Display.getDefault().getSystemColor(SWT.COLOR_DARK_GRAY));
                        e.gc.setLineWidth((int) Math.ceil(2 / mScale));
                        e.gc.drawRectangle((int) mViewport.x, (int) mViewport.y, (int) Math.ceil(mViewport.width), (int) Math.ceil(mViewport.height));
                    }
                }
            }
        }
    ...
    private void paintRecursive(GC gc, DrawableViewNode node, Path connectionPath) {
        ...
        float childSpacing = (1.0f * (DrawableViewNode.NODE_HEIGHT - 2 * TreeView.LINE_PADDING)) / N;
        for (int i = 0; i < N; i++) {
            ...
            float y1 = (float) node.top + TreeView.LINE_PADDING + childSpacing * i + childSpacing / 2;
            float x2 = child.left;
    ...
    private void setBounds() {
        if (mViewport != null && mTree != null) {
            ...
            mBounds.width = Math.max(mViewport.x + mViewport.width, mTree.bounds.x + mTree.bounds.width) - mBounds.x;
            mBounds.height = Math.max(mViewport.y + mViewport.height, mTree.bounds.y + mTree.bounds.height) - mBounds.y;
        } else if (mTree != null) {
