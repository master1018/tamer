...
public class SextantApp implements ExmaraldaApplication {
        ...
        if (file != null) {
            ...
            if (doc != null) {
                ...
                if (root.getName().equals("segmented-transcription")) {
                    ui.status("<b>" + file.getName() + "</b> is a transcription.", true);
                    exsFile = file;
                    exsDoc = doc;
                    checkForAnnotations();
                } else if (root.getName().equals("annotation")) {
                    ui.status("<b>" + file.getName() + "</b> is a standoff-annotation file.", true);
                    esaFile = file;
                    esaDoc = doc;
                    checkForTranscriptions();
                } else if (root.getName().equals("search-result-list")) {
                    ui.status("<b>" + file.getName() + "</b> is a concordance.", true);
                    checkConc();
                }
            }
        }
    ...
    private void checkForTranscriptions() {
        ...
        File trForAnn = new File(esaFile.getParentFile().getPath() + File.separator + esaDoc.getRootElement().getAttributeValue("target"));
        if (trForAnn.exists()) {
            ui.status("Annotation <b>" + esaFile.getName() + "</b> annotates transcription <b>" + trForAnn.getName() + "</b> (exists!)", true);
            transcriptions.add(trForAnn);
            exsFile = trForAnn;
            ui.updateTrans(transcriptions);
        } else {
            ui.status("Annotation <b>" + esaFile.getName() + "</b> annotates transcription <b>" + trForAnn.getPath() + "</b> (doesn't exist!)", false);
        }
    }
    ...
    private void checkForAnnotations() {
        ...
        ui.updateTrans(transcriptions);
        try {
            XPath annXPath = XPath.newInstance("//ud-information[@attribute-name='@sextant-annotation']");
            List anns = annXPath.selectNodes(exsDoc);
            if (anns.size() > 0) {
                ui.status("transcription has " + anns.size() + " annotation" + ((anns.size() == 0 || anns.size() > 1) ? "s:" : ":"), true);
                for (Element e : (List<Element>) anns) {
                    annotations.add(new File(exsFile.getParentFile().getPath() + File.separator + e.getText()));
    ...
    public void visualize() {
        if (selectedType.equals("annotation")) {
            ui.status("visualizing annotation: " + selectedFile.getName(), true);
        } else if (selectedType.equals("transcription")) {
            ui.status("visualizing transcription: " + selectedFile.getName(), true);
        } else {
    ...
    public void convert() {
        if (selectedType.equals("concordance")) {
            ui.status("converting concordance: " + selectedFile.getName(), true);
        } else {
    ...
    public void validate() {
        if (selectedType == null) {
            ui.status("please select either a transcription or an annotation for validation", false);
        } else {
            if (selectedType.equals("annotation")) {
                ui.status("validating annotation: " + selectedFile.getName(), true);
                validateAnnotation(selectedFile);
            } else if (selectedType.equals("transcription")) {
                ui.status("validating transcription: " + selectedFile.getName(), true);
            } else {
    ...
    private void validateAnnotation(File f) {
        ...
        try {
            ...
            try {
                ...
                for (Element e : segs) {
                    if (trSegments.contains(e.getAttributeValue("href", xlink).substring(1))) {
                    } else {
                        error += "annotatated segment " + e.getAttributeValue("href", xlink).substring(1) + " does not exist in " + exsFile.getName() + ".<br/>";
                    }
                }
                if (error.length() == 0) {
            ...
            ui.status("cannot verify against schema. maybe you are not connected to the internet.", false);
        } catch (SAXException e) {
            ui.status("is not valid: " + e.getMessage(), false);
        } catch (IOException e) {
            ui.status("cannot open annotation. maybe the file does not exist.", false);
        }
    }
}
