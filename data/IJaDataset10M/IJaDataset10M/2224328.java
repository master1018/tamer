...
public class DatabaseImpl extends AbstractDatabase {
    ...
    protected PreparedStatement[] getChangeFieldStatements(Connection c, String tableName, FieldDescriptor fd, boolean autoIncrement) throws SQLException {
        String cfs = "ALTER TABLE " + tableName + " ALTER COLUMN " + fd.getName() + " " + getTypeDeclaration(fd) + (fd.isNullable() ? " NULL" : "NOT NULL");
    ...
    protected String getTypeDeclaration(FieldDescriptor fd) throws SQLException {
        String sst = fd.getSuggestedSqlTypeName();
        if (sst != null && sst.length() > 0) {
            return sst;
        }
        int dataType = fd.getSqlType();
        ...
        if (dataType == Types.DOUBLE) {
            ...
            return super.getTypeDeclaration(fd);
        }
    }
