...
public class PointGrid extends WWObjectImpl implements OrderedRenderable, Highlightable {
    ...
    protected boolean mustRegenerateGeometry(DrawContext dc) {
        if (this.currentPoints == null || dc.getVerticalExaggeration() != this.previousExaggeration) return true;
        return this.getAltitudeMode() != WorldWind.ABSOLUTE && this.frameID - this.visGeomRegenFrame > this.getGeometryRegenerationInterval();
    }
        ...
        if (this.isHighlighted() && !this.isHighlightOnePosition()) {
            if (this.getHighlightAttributes() != null) {
                ...
                if (this.getAttributes() != null) {
                    ...
                    if (getAttributes().getPointSize() != null) actAttrs.setPointSize(DEFAULT_HIGHLIGHT_SCALE * this.getAttributes().getPointSize());
                } else {
                    actAttrs.copy(defaultAttributes);
                    if (defaultAttributes.getPointSize() != null) actAttrs.setPointSize(DEFAULT_HIGHLIGHT_SCALE * defaultAttributes.getPointSize());
                }
            }
            if (actAttrs.getPointSize() == null) actAttrs.setPointSize(defaultAttributes.getPointSize() * DEFAULT_HIGHLIGHT_SCALE);
            if (actAttrs.getPointColor() == null) actAttrs.setPointColor(DEFAULT_HIGHLIGHT_POINT_COLOR);
    ...
    protected Color determineHighlightColor() {
        Color color = null;
        if (this.getHighlightAttributes() != null) color = this.getHighlightAttributes().getPointColor(); else if (this.getAttributes() != null) color = this.getAttributes().getPointColor();
    ...
    protected Double determineHighlightPointSize() {
        ...
        if (this.getHighlightAttributes() != null) {
            ...
            if (size != null) size = size * DEFAULT_HIGHLIGHT_SCALE;
        }
    ...
    protected Extent computeExtentAndEyeDistance(DrawContext dc) {
        if (WWUtil.isEmpty(this.corners)) return null;
        ...
        for (Position pos : this.corners) {
            ...
            if (d < this.eyeDistance) this.eyeDistance = d;
        }
    ...
    public void render(DrawContext dc) {
        ...
        if (dc.getSurfaceGeometry() == null) return;
    ...
    protected void makeOrderedRenderable(DrawContext dc) {
        if (this.mustRegenerateGeometry(dc)) {
            ...
            if (this.currentPoints == null || this.currentPoints.limit() == 0) return;
    ...
    protected void beginDrawing(DrawContext dc) {
        GL gl = dc.getGL();
        int attrMask = GL.GL_POINT_BIT | GL.GL_CURRENT_BIT | GL.GL_COLOR_BUFFER_BIT | GL.GL_ENABLE_BIT;
        if (dc.isPickingMode()) attrMask |= GL.GL_DEPTH_BUFFER_BIT | GL.GL_TRANSFORM_BIT | GL.GL_VIEWPORT_BIT | GL.GL_DEPTH_BUFFER_BIT;
    ...
    protected void doDrawOrderedRenderable(DrawContext dc) {
        if (dc.isPickingMode()) this.pickPoints(dc); else this.drawPoints(dc);
    }
        ...
        try {
            ...
            try {
                while (points.hasRemaining()) {
                    ...
                    if (!dc.getView().getFrustumInModelCoordinates().contains(pt)) continue;
                    Vec4 sp = dc.getView().project(pt);
                    if (!dc.getPickFrustums().containsInAny(sp.x, sp.y)) continue;
    ...
    protected void setPointColor(DrawContext dc, Color color) {
        if (color == null) color = this.getActiveAttributes().getPointColor();
        if (color == null) color = DEFAULT_POINT_COLOR;
        dc.getGL().glColor4ub((byte) color.getRed(), (byte) color.getGreen(), (byte) color.getBlue(), (byte) color.getAlpha());
    }

    protected void setPointSize(DrawContext dc, Double size) {
        GL gl = dc.getGL();
        if (size == null) size = this.getActiveAttributes().getPointSize();
        if (size == null) size = DEFAULT_POINT_SIZE;
        double altitude = dc.getView().getEyePosition().getAltitude();
        if (dc.isPickingMode()) {
            size *= 2;
        } else if (altitude > 100e3) {
            if (altitude < 200e3) size = Math.max(0.8 * size, 1); else if (altitude < 400e3) size = Math.max(0.6 * size, 1); else if (altitude < 700e3) size = Math.max(0.4 * size, 1); else if (altitude < 1e6) size = Math.max(0.2 * size, 1); else size = Math.max(0.1 * size, 1);
        }
    ...
    protected boolean intersectsFrustum(DrawContext dc) {
        if (this.getExtent() == null) {
            ...
            if (this.getExtent() == null) return false;
        }
        if (dc.isPickingMode()) return dc.getPickFrustums().intersectsAny(this.getExtent()); else return dc.getView().getFrustumInModelCoordinates().intersects(this.getExtent());
    }

    protected FloatBuffer computeGridPoints(DrawContext dc, FloatBuffer coords) {
        int numCoords = 3 * this.numPositions;
        if (coords == null || coords.capacity() < numCoords || coords.capacity() > 1.5 * numCoords) coords = BufferUtil.newFloatBuffer(numCoords);
        coords.rewind();
        Iterator<? extends Position> posIter = this.positions.iterator();
        while (posIter.hasNext()) {
    ...
    protected Vec4 computePoint(DrawContext dc, Position pos) {
        if (this.getAltitudeMode() == WorldWind.CLAMP_TO_GROUND) return dc.getTerrain().getSurfacePoint(pos.getLatitude(), pos.getLongitude(), 0);
        if (this.getAltitudeMode() == WorldWind.RELATIVE_TO_GROUND) return dc.getTerrain().getSurfacePoint(pos);
        if (this.applyVerticalExaggeration) pos = new Position(pos, dc.getVerticalExaggeration() * pos.getAltitude());
