...
public class JavacElements implements Elements {
    ...
    public static JavacElements instance(Context context) {
        ...
        if (instance == null) instance = new JavacElements(context);
        return instance;
    }
     ...
     */
    public static <A extends Annotation> A getAnnotation(Symbol annotated, Class<A> annoType) {
        if (!annoType.isAnnotation()) throw new IllegalArgumentException("Not an annotation type: " + annoType);
        String name = annoType.getName();
        for (Attribute.Compound anno : annotated.getAnnotationMirrors()) if (name.equals(anno.type.tsym.flatName().toString())) return AnnotationProxyMaker.generateAnnotation(anno, annoType);
        return null;
    }
    ...
    public static <A extends Annotation> A getAnnotation(ClassSymbol annotated, Class<A> annoType) {
        ...
        while (annotated.name != annotated.name.table.names.java_lang_Object) {
            ...
            if (result != null || !inherited) break;
            Type sup = annotated.getSuperclass();
            if (sup.tag != TypeTags.CLASS || sup.isErroneous()) break;
    ...
    public PackageSymbol getPackageElement(CharSequence name) {
        ...
        if (strName.equals("")) return syms.unnamedPackage;
        return SourceVersion.isName(strName) ? nameToSymbol(strName, PackageSymbol.class) : null;
    }

    public ClassSymbol getTypeElement(CharSequence name) {
        String strName = name.toString();
        return SourceVersion.isName(strName) ? nameToSymbol(strName, ClassSymbol.class) : null;
    }
    ...
    private <S extends Symbol> S nameToSymbol(String nameStr, Class<S> clazz) {
        ...
        try {
            if (sym == null) sym = javaCompiler.resolveIdent(nameStr);
            sym.complete();
    ...
    private JCTree matchAnnoToTree(AnnotationMirror findme, Element e, JCTree tree) {
        ...
        Vis vis = new Vis();
        tree.accept(vis);
        if (vis.result == null) return null;
     ...
     */
    private JCTree matchAnnoToTree(Attribute.Compound findme, List<Attribute.Compound> annos, List<JCAnnotation> trees) {
    ...
    private JCTree matchAnnoToTree(final Attribute.Compound findme, final Attribute attr, final JCTree tree) {
        if (attr == findme) return (tree.type.tsym == findme.type.tsym) ? tree : null;
    ...
    public String getDocComment(Element e) {
        ...
        JCTree tree = treeTop.fst;
        JCCompilationUnit toplevel = treeTop.snd;
        if (toplevel.docComments == null) return null;
    ...
    private void addMembers(Scope scope, Type type) {
        members: for (Scope.Entry e = type.asElement().members().elems; e != null; e = e.sibling) {
            ...
            while (overrider.scope != null) {
                if (overrider.sym.kind == e.sym.kind && (overrider.sym.flags() & Flags.SYNTHETIC) == 0) {
                    if (overrider.sym.getKind() == ElementKind.METHOD && overrides((ExecutableElement) overrider.sym, (ExecutableElement) e.sym, (TypeElement) type.asElement())) {
                        continue members;
                    }
                }
            ...
            boolean initializer = kind == ElementKind.CONSTRUCTOR || kind == ElementKind.INSTANCE_INIT || kind == ElementKind.STATIC_INIT;
            if (!derived || (!initializer && e.sym.isInheritedIn(scope.owner, types))) scope.enter(e.sym);
        }
    }
    ...
    public List<Attribute.Compound> getAllAnnotationMirrors(Element e) {
        ...
        while (sym.getKind() == ElementKind.CLASS) {
            ...
            if (sup.tag != TypeTags.CLASS || sup.isErroneous() || sup.tsym == syms.objectType.tsym) {
                break;
            }
            sym = sup.tsym;
    ...
    private boolean isInherited(Type annotype) {
        for (Attribute.Compound anno : annotype.tsym.getAnnotationMirrors()) {
            if (anno.type.tsym == syms.inheritedType.tsym) return true;
        }
        return false;
    }
    ...
    private static boolean containsAnnoOfType(List<Attribute.Compound> annos, Type type) {
        for (Attribute.Compound anno : annos) {
            if (anno.type.tsym == type.tsym) return true;
        }
        return false;
    }
    ...
    public boolean overrides(ExecutableElement riderEl, ExecutableElement rideeEl, TypeElement typeEl) {
        ...
        return rider.name == ridee.name && rider != ridee && !rider.isStatic() && ridee.isMemberOf(origin, types) && rider.overrides(ridee, origin, types, false);
    }
    ...
    public void printElements(java.io.Writer w, Element... elements) {
        for (Element element : elements) (new PrintingProcessor.PrintingElementVisitor(w, this)).visit(element).flush();
    }
    ...
    private Pair<JCTree, JCCompilationUnit> getTreeAndTopLevel(Element e) {
        ...
        if (tree == null || enterEnv.toplevel == null) return null;
    ...
    public Pair<JCTree, JCCompilationUnit> getTreeAndTopLevel(Element e, AnnotationMirror a, AnnotationValue v) {
        ...
        if (elemTreeTop == null) return null;
        if (a == null) return elemTreeTop;
        JCTree annoTree = matchAnnoToTree(a, e, elemTreeTop.fst);
        if (annoTree == null) return elemTreeTop;
    ...
    private Env<AttrContext> getEnterEnv(Symbol sym) {
        TypeSymbol ts = (sym.kind != Kinds.PCK) ? sym.enclClass() : (PackageSymbol) sym;
