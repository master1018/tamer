...
public class Skill_Track extends StdSkill {
    ...
    public boolean tick(Tickable ticking, int tickID) {
        ...
        if (tickID == Tickable.TICKID_MOB) {
            if (nextDirection == -999) return true;
            if ((theTrail == null) || (affected == null) || (!(affected instanceof MOB))) return false;
            ...
            if (nextDirection == 999) {
                ...
                if ((mob.isMonster()) && (mob.location() != null)) {
                    ...
                    if ((nextRoom != null) && (nextExit != null)) {
                        ...
                        if (!nextExit.isOpen()) unInvoke(); else {
                            ...
                            if ((reclose) && (mob.location() == nextRoom)) {
                                ...
                                if ((opExit != null) && (opExit.hasADoor()) && (opExit.isOpen())) {
    ...
    public void executeMsg(Environmental myHost, CMMsg msg) {
        ...
        MOB mob = (MOB) affected;
        if ((msg.amISource(mob)) && (msg.amITarget(mob.location())) && (msg.targetMinor() == CMMsg.TYP_LOOK)) nextDirection = CMLib.tracking().trackNextDirectionFromHere(theTrail, mob.location(), false);
    }
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        if ((commands.size() > 1) && (((String) commands.lastElement()).toUpperCase().startsWith("RADIUS=")) && (CMath.isInteger(((String) commands.lastElement()).substring(7)))) {
        ...
        if (givenTarget == null) givenTarget = CMLib.map().getRoom(mobName);
        if (givenTarget == null) givenTarget = CMLib.map().getArea(mobName);
        tickStatus = Tickable.STATUS_MISC6 + 4;
        if ((givenTarget == null) && (thisRoom.fetchInhabitant(mobName) != null)) {
            mob.tell("Try 'look'.");
            tickStatus = Tickable.STATUS_NOT;
            return false;
        }
        Vector rooms = new Vector();
        if (givenTarget instanceof Area) rooms.addElement(((Area) givenTarget).getRandomMetroRoom()); else if (givenTarget instanceof Room) rooms.addElement(givenTarget); else if ((givenTarget instanceof MOB) && (((MOB) givenTarget).location() != null)) rooms.addElement(((MOB) givenTarget).location()); else if (mobName.length() > 0) {
        ...
        if (rooms.size() <= 0) {
            try {
                ...
                for (Enumeration r = checkSet.elements(); r.hasMoreElements(); ) {
                    ...
                    if (R.fetchInhabitant(mobName) != null) rooms.addElement(R);
                }
        ...
        if (rooms.size() > 0) {
            ...
            if ((cacheCode == 1) && (rooms.size() == 1)) theTrail = (Vector) cachedPaths.get(CMLib.map().getExtendedRoomID(thisRoom) + "->" + CMLib.map().getExtendedRoomID((Room) rooms.firstElement()));
            tickStatus = Tickable.STATUS_MISC6 + 9;
            if (theTrail == null) theTrail = CMLib.tracking().findBastardTheBestWay(thisRoom, rooms, flags, radius);
            tickStatus = Tickable.STATUS_MISC6 + 10;
            if ((cacheCode == 1) && (rooms.size() == 1) && (theTrail != null)) cachedPaths.put(CMLib.map().getExtendedRoomID(thisRoom) + "->" + CMLib.map().getExtendedRoomID((Room) rooms.firstElement()), theTrail);
        }
        ...
        if ((success) && (theTrail != null)) {
            ...
            if (thisRoom.okMessage(mob, msg)) {
                ...
                if (mob.fetchEffect(newOne.ID()) == null) mob.addEffect(newOne);
                mob.recoverEnvStats();
