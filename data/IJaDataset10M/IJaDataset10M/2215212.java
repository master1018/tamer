...
public class BasicTextEncoder extends AbstractVisualEncoder {
   ...
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(BasicTextEncoder.class);
    ...
    protected boolean canEncodeVisualObjectType(IAfferentObject afferentObject) {
        try {
            ...
            for (String kind : types) if (_commonTypeName.equalsIgnoreCase(kind)) return true;
            return false;
    ...
    public boolean isDirty(IAfferentObject afferentObject, IChunk oldChunk, IPerceptualMemory memory) {
        return super.isDirty(afferentObject, oldChunk, memory) || textHasChanged(afferentObject, oldChunk);
    }
    ...
    protected void updateSlots(IAfferentObject afferentObject, IChunk encoding, IVisualMemory memory) {
        ...
        ((IMutableSlot) encoding.getSymbolicChunk().getSlot(IVisualModule.VALUE_SLOT)).setValue(getHandler().getText(afferentObject));
    }
    ...
    protected boolean textHasChanged(IAfferentObject afferentObject, IChunk encoding) {
        ...
        Object oldValue = encoding.getSymbolicChunk().getSlot(IVisualModule.VALUE_SLOT).getValue();
        String oldText = "";
