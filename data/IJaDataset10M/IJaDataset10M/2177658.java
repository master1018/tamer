...
public class DIMERecord {
    ...
    public DIMERecord(byte byte1, byte byte2, byte[] options, byte[] id, byte[] type, byte[] data) {
        _byte1 = byte1;
        _byte2 = byte2;
        if (options == null) options = DataUtils.EMPTY_BYTE_ARRAY;
        if (id == null) id = DataUtils.EMPTY_BYTE_ARRAY;
        if (type == null) type = DataUtils.EMPTY_BYTE_ARRAY;
        if (data == null) data = DataUtils.EMPTY_BYTE_ARRAY;
        _options = options;
        _id = id;
        _type = type;
        _data = data;
    ...
    public DIMERecord(byte typeId, byte[] options, byte[] id, byte[] type, byte[] data) {
        this(VERSION, (byte) (typeId | RESERVED), options, id, type, data);
    }
    ...
    public static DIMERecord createFromStream(InputStream in) throws IOException {
        ...
        if (dataLength < 0) throw new IOException("data too big.");
    ...
    public void setFirstRecord(boolean first) {
        if (first) _byte1 |= MB_MASK; else _byte1 &= ~MB_MASK;
    }
    ...
    public void setLastRecord(boolean last) {
        if (last) _byte1 |= ME_MASK; else _byte1 &= ~ME_MASK;
    }
    ...
    public String getIdentifier() {
        if (_idString == null) _idString = new String(getId());
        return _idString;
    }
    ...
    public Map<String, String> getOptionsMap() throws DIMEMessageException {
        if (_optionsMap == null) _optionsMap = parseOptions(getOptions());
        return _optionsMap;
    }
    ...
    static void validateFirstBytes(byte one, byte two) {
        if ((one & VERSION_MASK) != VERSION) throw new IllegalArgumentException("invalid version: " + (((one & VERSION_MASK) >> 3) & 0x1F));
        if ((two & RESERVED_MASK) != RESERVED) throw new IllegalArgumentException("invalid reserved: " + (two & RESERVED_MASK));
    }
    ...
    private void validate() {
        ...
        switch(maskedType) {
            case TYPE_UNCHANGED:
                if (getTypeLength() != 0) throw new IllegalArgumentException("TYPE_UNCHANGED requires 0 type length");
                break;
            ...
            case TYPE_ABSOLUTE_URI:
                break;
            case TYPE_UNKNOWN:
                if (getTypeLength() != 0) throw new IllegalArgumentException("TYPE_UNKNOWN requires 0 type length");
                break;
            case TYPE_NONE:
                if (getTypeLength() != 0 || getDataLength() != 0) throw new IllegalArgumentException("TYPE_NONE requires 0 type & data length");
                break;
            default:
                throw new IllegalArgumentException("invalid type: " + ((maskedType >> 4) & 0x0F));
        }
    }
    ...
    private static byte[] readInformation(int length, InputStream in) throws IOException {
        if (length == 0) return DataUtils.EMPTY_BYTE_ARRAY;
    ...
    private static void skipPaddedData(int length, InputStream in) throws IOException {
        ...
        while (skipped < padding) {
            ...
            if (current == -1 || current == 0) throw new IOException("eof"); else skipped += current;
        }
    }
    ...
    private static void fillBuffer(byte[] buffer, InputStream in) throws IOException {
        ...
        while (offset < buffer.length) {
            ...
            if (read == -1) throw new IOException("eof"); else offset += read;
        }
    }
     ...
     */
    private static Map<String, String> parseOptions(byte[] options) throws DIMEMessageException {
        Map<String, String> map = new HashMap<String, String>();
        int offset = 0;
        while (offset < options.length) {
            if (options.length - offset < 4) throw new DIMEMessageException("illegal options field");
            ...
            offset += 2;
            if (options.length - offset < valueLength) throw new DIMEMessageException("illegal options field");
