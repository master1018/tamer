...
public class Matrix3D {
    ...
    public Matrix3D replaceColumn(int i, double[] newColumn) {
        ...
        for (int j = 0; j < 3; j++) B.data[j][i] = newColumn[j];
        return B;
    }

    public Matrix3D multiply(Matrix3D B) {
        Matrix3D C = new Matrix3D();
        for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++) for (int k = 0; k < 3; k++) C.data[i][j] += (data[i][k] * B.data[k][j]);
        return C;
    }

    public Matrix3D multiply(double b) {
        Matrix3D C = new Matrix3D();
        for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++) C.data[i][j] = b * data[i][j];
        return C;
    }
