...
public class DatabaseConnection {
    ...
    public void dbSetup(String url, String indexUrl, String resourceUrl, String userid, String userPassword, String cypherPassword) {
        ...
        try {
            if (cypherPassword == null || cypherPassword.trim().equals("")) passwordString = userPassword; else passwordString = cypherPassword + " " + userPassword;
            if (throttle == 0) {
    ...
    public void upgradeDb(String version) {
        ...
        if (version.equals("0.95")) {
            if (dbTableExists("words")) executeSql("Drop table words;");
            if (dbTableExists("NoteResources")) executeSql("Drop table NoteResources;");
        }
    ...
    public void beginTransaction() {
        ...
        if (!query.exec("Begin Transaction")) logger.log(logger.EXTREME, "Begin transaction has failed: " + query.lastError());
    }

    public void commitTransaction() {
        NSqlQuery query = new NSqlQuery(getConnection());
        if (!query.exec("Commit")) logger.log(logger.EXTREME, "Transaction commit has failed: " + query.lastError());
    }
