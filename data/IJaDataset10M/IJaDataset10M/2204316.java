...
public class TernaryPlotMenu extends AbstractMenuDialog {
    ...
    public TernaryPlotMenu(final CoDaPackMain mainApp) {
        super(mainApp, "Ternary Plot Menu", true);
    }
    ...
    public void acceptButtonActionPerformed() {
        ...
        if (selectedNames.length == 3 || selectedNames.length == 4) {
            ...
            CoDaPackMain.outputPanel.addOutput(new OutputPlotHeader("Ternary plot", selectedNames));
            if (selectedNames.length == 3) {
                TernaryPlot2dBuilder builder = new TernaryPlot2dBuilder(selectedNames, data);
                builder.mapping(mapping);
                if (groupedBy != null) {
                    builder.groups(coda.Utils.reduceData(df.getDefinedGroup(groupedBy), selection), coda.Utils.getCategories(df.getCategoricalData(groupedBy), selection));
                }
