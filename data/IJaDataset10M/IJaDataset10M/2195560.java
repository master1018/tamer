...
class ConfigView extends ConfigObject {
    ...
    protected void setProperty(ConfigCommand command) {
        ...
        if (argc != 4) {
            syntaxError("Incorrect number of arguments to " + command.commandName);
        }
        if (!isName(argv[1])) {
            syntaxError("The first argument to " + command.commandName + " must be the instance name");
        }
        if (!isName(argv[2])) {
            syntaxError("The second argument to " + command.commandName + " must be a property name");
        }
        attr = (String) argv[2];
        val = argv[3];
        if (attr.equals("Screen") || attr.equals("Window")) {
            ...
            cs = (ConfigScreen) configContainer.findConfigObject("Screen", (String) val);
            if (!screens.add(cs)) {
                syntaxError(attr + " \"" + ((String) val) + "\" has already been added to " + instanceName);
            }
        } else if (attr.equals("ViewPlatform")) {
            if (!(val instanceof String)) {
                syntaxError("value for ViewPlatform " + " must be an instance name");
            }
            configViewPlatform = (ConfigViewPlatform) configContainer.findConfigObject("ViewPlatform", (String) val);
            ...
            configEnv = (ConfigPhysicalEnvironment) configContainer.findConfigObject("PhysicalEnvironment", (String) val);
        } else if (attr.equals("PhysicalBody")) {
            if (!(val instanceof String)) {
                syntaxError("value for PhysicalBody " + "must be an instance name");
            }
            configBody = (ConfigPhysicalBody) configContainer.findConfigObject("PhysicalBody", (String) val);
        } else if (attr.equals("BackClipPolicy")) {
            if (!(val instanceof String)) {
                syntaxError("value for BackClipPolicy must be a string");
            }
            sval = (String) val;
            if (sval.equals("PHYSICAL_EYE")) backClipPolicy = View.PHYSICAL_EYE; else if (sval.equals("PHYSICAL_SCREEN")) backClipPolicy = View.PHYSICAL_SCREEN; else if (sval.equals("VIRTUAL_EYE")) backClipPolicy = View.VIRTUAL_EYE; else if (sval.equals("VIRTUAL_SCREEN")) backClipPolicy = View.VIRTUAL_SCREEN; else syntaxError("Invalid value for BackClipPolicy " + sval);
        } else if (attr.equals("FrontClipPolicy")) {
            if (!(val instanceof String)) {
                syntaxError("value for FrontClipPolicy must be a string");
            }
            sval = (String) val;
            if (sval.equals("PHYSICAL_EYE")) frontClipPolicy = View.PHYSICAL_EYE; else if (sval.equals("PHYSICAL_SCREEN")) frontClipPolicy = View.PHYSICAL_SCREEN; else if (sval.equals("VIRTUAL_EYE")) frontClipPolicy = View.VIRTUAL_EYE; else if (sval.equals("VIRTUAL_SCREEN")) frontClipPolicy = View.VIRTUAL_SCREEN; else syntaxError("Invalid value for FrontClipPolicy " + sval);
        } else if (attr.equals("ScreenScalePolicy")) {
            if (!(val instanceof String)) {
                syntaxError("value for ScreenScalePolicy must be a string");
            }
            sval = (String) val;
            if (sval.equals("SCALE_SCREEN_SIZE")) screenScalePolicy = View.SCALE_SCREEN_SIZE; else if (sval.equals("SCALE_EXPLICIT")) screenScalePolicy = View.SCALE_EXPLICIT; else syntaxError("Invalid value for ScreenScalePolicy " + sval);
            ...
            sval = (String) val;
            if (sval.equals("SCREEN_VIEW")) viewPolicy = View.SCREEN_VIEW; else if (sval.equals("HMD_VIEW")) viewPolicy = View.HMD_VIEW; else syntaxError("Invalid value for ViewPolicy " + sval);
        } else if (attr.equals("WindowEyepointPolicy")) {
            if (!(val instanceof String)) {
                syntaxError("value for WindowEyepointPolicy " + "must be a string");
            }
            sval = (String) val;
            if (sval.equals("RELATIVE_TO_SCREEN")) windowEyepointPolicy = View.RELATIVE_TO_SCREEN; else if (sval.equals("RELATIVE_TO_COEXISTENCE")) windowEyepointPolicy = View.RELATIVE_TO_COEXISTENCE; else if (sval.equals("RELATIVE_TO_WINDOW")) windowEyepointPolicy = View.RELATIVE_TO_WINDOW; else if (sval.equals("RELATIVE_TO_FIELD_OF_VIEW")) windowEyepointPolicy = View.RELATIVE_TO_FIELD_OF_VIEW; else syntaxError("Invalid value for WindowEyepointPolicy " + sval);
        } else if (attr.equals("WindowMovementPolicy")) {
            if (!(val instanceof String)) {
                syntaxError("value for WindowEyeMovementPolicy " + "must be a string");
            }
            sval = (String) val;
            if (sval.equals("VIRTUAL_WORLD")) windowMovementPolicy = View.VIRTUAL_WORLD; else if (sval.equals("PHYSICAL_WORLD")) windowMovementPolicy = View.PHYSICAL_WORLD; else syntaxError("Invalid value for WindowMovementPolicy " + sval);
        } else if (attr.equals("WindowResizePolicy")) {
            if (!(val instanceof String)) {
                syntaxError("value for WindowResizePolicy " + "must be a string");
            }
            sval = (String) val;
            if (sval.equals("VIRTUAL_WORLD")) windowResizePolicy = View.VIRTUAL_WORLD; else if (sval.equals("PHYSICAL_WORLD")) windowResizePolicy = View.PHYSICAL_WORLD; else syntaxError("Invalid value for WindowResizePolicy " + sval);
        } else if (attr.equals("CenterEyeInCoexistence")) {
            if (val instanceof Point3d) centerEyeInCoexistence = (Point3d) val; else syntaxError("value for CenterEyeInCoexistence " + "must be a Point3d");
            ...
            syntaxError("Unknown " + command.commandName + " \"" + attr + "\"");
        }
    }
    ...
    protected Viewer createViewer(boolean setVisible) {
        ...
        if (configBody == null) physicalBody = new PhysicalBody(); else physicalBody = configBody.j3dPhysicalBody;
        if (configEnv == null) physicalEnvironment = new PhysicalEnvironment(); else physicalEnvironment = configEnv.j3dPhysicalEnvironment;
        ...
        if (screens.size() == 0) {
            throw new IllegalStateException(errorMessage(creatingCommand, "View \"" + instanceName + "\" has no canvases or screens"));
        }
