...
public class Commit {
    ...
    public String toStringVerbose() {
        ...
        sb.append(" ");
        for (int i = 0; i < getNCreates(); ++i) sb.append("+" + (getCreate(i) - 4));
        for (int i = 0; i < getNDeletes(); ++i) sb.append("-" + (getDelete(i) - 4));
        return sb.toString();
    }

    private StringBuilder toStringBase() {
        StringBuilder sb = new StringBuilder();
        sb.append("Commit ").append(new SimpleDateFormat("yyyy-MM-dd HH:mm").format(getDate())).append(" num ").append(getNum());
        return sb;
    }
}
