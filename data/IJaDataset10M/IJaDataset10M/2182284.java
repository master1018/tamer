...
public class ReaderWriterTest {
    ...
    public static class MyProvider implements MessageBodyReader<TestObject>, MessageBodyWriter<TestObject> {

        public boolean isReadable(Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
            return type.equals(TestObject.class);
        }

        public TestObject readFrom(Class<TestObject> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, String> httpHeaders, InputStream entityStream) throws IOException, WebApplicationException {
            byte[] result = ReadFromStream.readFromStream(1024, entityStream);
            return new TestObject(new String(result).replaceAll("^BeginTest\\{", "").replaceAll("\\}EndTest$", ""));
        }
        ...
        public void writeTo(TestObject t, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders, OutputStream entityStream) throws IOException, WebApplicationException {
            new PrintStream(entityStream).append("BeginTest{").append(t.value).append("}EndTest");
        }
    }
    ...
    public void readObject() throws IOException {
        ...
        Assert.assertEquals("test", ReaderUtility.read(TestObject.class, "application/test-content", "BeginTest{test}EndTest").value);
    }

    @Test
    public void writeObject() throws IOException {
        Assert.assertEquals("test", new String(WriterUtility.getBytes("test", "text/plain")));
        Assert.assertEquals("BeginTest{test}EndTest", WriterUtility.asString(new TestObject("test"), "application/test-content"));
    }
}
