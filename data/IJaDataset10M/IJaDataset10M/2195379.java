...
public class MinMax {
    ...
    public static Object minMax(Object arg, boolean returnMax, NamedCollator collation) {
        if (arg instanceof Values) {
            ...
            if (cur == Sequence.eofValue) return Values.empty;
            Object result = convert(cur);
            for (; ; ) {
                pos = tlist.nextPos(pos);
                cur = tlist.getPosNext(pos);
                if (cur == Sequence.eofValue) return result;
                cur = convert(cur);
                if (result instanceof Number || cur instanceof Number) {
                    ...
                    if (rcode == -3) throw new IllegalArgumentException("values cannot be compared");
                    ...
                    if (castNeeded) result = Arithmetic.convert(result, code);
                } else {
                    if (!Compare.atomicCompare(flags, result, cur, collation)) result = cur;
                }
            }
        } else {
            arg = convert(arg);
    ...
    static Object convert(Object arg) {
        ...
        if (arg instanceof UntypedAtomic) arg = (Double) XDataType.doubleType.valueOf(TextUtils.stringValue(arg));
        return arg;
    }
}
