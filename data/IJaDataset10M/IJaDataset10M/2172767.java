...
public class TracePath {
    ...
    private class RerunTrace implements Trace<ArcPath> {
        ...
        public Map<String, Collection<Path>> trace(ArcPath path) {
            ...
            start = findLine(start, p.x + options.arcSize(), p.y + options.arcSize() / 2);
            ...
            result.get("path").add(p = pathList.get(start));
            ap = (ArcPath) p;
            start = findLine(0, new X2YMatcher(ap.x + options.arcSize() / 2 + 1, ap.y + options.arcSize()));
            result.get("path").add(p = pathList.get(start));
            lp = (LinePath) p;
            return result;
        }
    ...
    private class OptionTrace implements Trace<ArcPath> {
        ...
        public Map<String, Collection<Path>> trace(ArcPath path) {
            ...
            Path p = null;
            start = findLine(start, path.x + path.width, path.y + path.height / 2);
            result.get("path").add(p = pathList.get(start));
            lp = (LinePath) p;
            ...
            result.get("path").add(p = pathList.get(start));
            lp = (LinePath) p;
            start = findArc(start, lp.x2 - options.arcSize() / 2 - 1, lp.y - options.arcSize());
            result.get("path").add(p = pathList.get(start));
            ap = (ArcPath) p;
            start = findLine(start, ap.x + options.arcSize(), path.y + options.arcSize() / 2);
            result.get("path").add(p = pathList.get(start));
            lp = (LinePath) p;
    ...
    private <P extends Path> int find(Class<P> type, int start, XYMatcher matcher) {
