...
public class SecretKeyComposite extends Composite {
    ...
    public SecretKeyComposite(Composite parent, int style, String n, int s, int m) {
        ...
        max = m;
        initialize();
        if (s > 0) spnrSecret.setSelection(s);
    }
    ...
    public SecretKeyComposite(Composite parent, int style, String n, FlexiBigInt s, FlexiBigInt m) {
        ...
        secret = s;
        initialize();
        if (secret == null) secret = new FlexiBigInt(bigMax.bitCount(), new JavaSecureRandomWrapper(new SecureRandom()));
    ...
    private void createGroupSecret() {
        ...
        groupSecret.setText(name + Messages.getString("ECDHWizSK.groupSecret"));
        Label label = new Label(groupSecret, SWT.NONE);
        label.setText(Messages.getString("SecretKeyComposite.1"));
        if (large) {
            ...
            largeSecret.addModifyListener(new ModifyListener() {
                ...
                public void modifyText(ModifyEvent e) {
                    try {
                        ...
                        if (fbi.max(bigMax).equals(fbi)) largeSecret.setText(secret.toString(10)); else secret = fbi;
                    } catch (Exception ex) {
                        largeSecret.setText(secret.toString(10));
                    }
                }
            ...
            btnGenerate.setText(Messages.getString("ECDHWizSK.btnGenerate"));
