...
public class ExtendedNetworkElementFactory {
    ...
    public static DataLinkInterface[] getDataLinks(int nodeID) throws SQLException {
    ...
    public static DataLinkInterface getDataLink(int nodeID, int ifindex) throws SQLException {
        DataLinkInterface[] nodes = null;
    ...
    public static DataLinkInterface[] getDataLinks(int nodeID, String IpAddress) throws SQLException {
        DataLinkInterface[] nodes = null;
        Connection conn = Vault.getDbConnection();
    ...
    public static DataLinkInterface[] getDataLinksFromNodeParent(int nodeID) throws SQLException {
        DataLinkInterface[] nodes = null;
        Connection conn = Vault.getDbConnection();
        try {
    ...
    public static DataLinkInterface[] getDataLinksFromNodeParent(int nodeID, int ifindex) throws SQLException {
        DataLinkInterface[] nodes = null;
        Connection conn = Vault.getDbConnection();
        try {
    ...
    public static StpInterface[] getStpInterface(int nodeID, int ifindex) throws SQLException {
        ...
        Connection conn = Vault.getDbConnection();
        try {
            String sqlQuery = "SELECT DISTINCT(stpnode.nodeid) AS droot, stpinterfacedb.* FROM " + "((SELECT DISTINCT(stpnode.nodeid) AS dbridge, stpinterface.* FROM " + "stpinterface LEFT JOIN stpnode ON SUBSTR(stpportdesignatedbridge,5,16) = stpnode.basebridgeaddress " + "WHERE stpinterface.status != 'D' AND stpinterface.nodeid = ? AND stpinterface.ifindex = ?) AS stpinterfacedb " + "LEFT JOIN stpnode ON SUBSTR(stpportdesignatedroot, 5, 16) = stpnode.basebridgeaddress) order by stpinterfacedb.stpvlan, stpinterfacedb.ifindex;";
            PreparedStatement stmt = conn.prepareStatement(sqlQuery);
            stmt.setInt(1, nodeID);
            stmt.setInt(2, ifindex);
            ResultSet rs = stmt.executeQuery();
    ...
    protected static AtInterface[] rs2AtInterface(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (element != null) node.m_lastPollTime = EventConstants.formatToString(new Date(((Timestamp) element).getTime()));
    ...
    protected static StpNode[] rs2StpNode(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (element != null) {
                node.m_stpprotocolspecification = ((Integer) element).intValue();
            }
    ...
    protected static DataLinkInterface[] rs2DataLink(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            node.m_parentipaddress = getIpAddress(node.get_nodeparentid(), node.get_parentifindex());
            if (node.get_ifindex() == -1) {
                node.m_ipaddress = getIpAddress(node.get_nodeId());
            } else {
                node.m_ipaddress = getIpAddress(node.get_nodeId(), node.get_ifindex());
            }
            vector.addElement(node);
        }
    ...
    public static String getIpAddress(int nodeid, int ifindex) throws SQLException {
        String ipaddr = null;
        Connection conn = Vault.getDbConnection();
        try {
