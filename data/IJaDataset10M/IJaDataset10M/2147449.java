...
public class FilterSameSetWmeEventFollowsRelation implements MyProcessInstance {
    ...
    public static TreeSet buildFollowsRelations(ProcessInstance pi) {
        ...
        if (ates.size() > 1) {
            ...
            for (int i = 1; i < ates.size(); i++) {
                try {
                    relation = ates.get(i - 1).getElement().trim() + ates.get(i - 1).getType().trim() + " > " + ates.get(i).getElement().trim() + ates.get(i).getType().trim();
    ...
    public int getNumberSimilarPIs() {
        return Integer.parseInt((String) processInstance.getAttributes().get(MethodsForWorkflowLogDataStructures.NUM_SIMILAR_INSTANCES));
    }
    ...
    private void setNumberSimilarPIs(String number) {
        processInstance.getAttributes().put(MethodsForWorkflowLogDataStructures.NUM_SIMILAR_INSTANCES, number);
    }
    ...
    public void addGroupedPiIdentifier(String idToAppend) {
        if (processInstance.getAttributes().containsKey(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS)) {
            String identifiers = (String) processInstance.getAttributes().get(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS);
            identifiers += ", " + idToAppend;
            processInstance.getAttributes().put(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS, identifiers);
        } else {
            processInstance.getAttributes().put(FilterPerWorkflowModelElementAndEvent.IDENTIFIERS, idToAppend);
        }
    }
}
