 ...
 */
public class GraphDisplay extends JComponent implements Observer {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (controller.getEditingMode() == Controller.MODE_ALGORITHM) {
            ...
            if (dj != null) graph = dj.getGraph(); else graph = controller.getGraph();
        } else graph = controller.getGraph();
        for (Edge edge : graph.getEdgeList()) edge.getVisual().draw(offG, screenSize);
        if (newEdgeLine != null) {
            offG.setColor(Visual.RED);
            offG.setStroke(new BasicStroke(7));
            offG.draw(newEdgeLine);
        }
        for (Node node : graph.getNodeList()) node.getVisual().draw(offG, screenSize);
    ...
    public void setBounds(int x, int y, int width, int height) {
        ...
        if (controller.getEditingMode() != Controller.MODE_ALGORITHM) graph = controller.getGraph(); else {
            State dj = controller.getState(controller.getCurrentStep());
            if (dj != null) graph = dj.getGraph(); else graph = controller.getGraph();
        }
        for (Edge edge : graph.getEdgeList()) edge.getVisual().updateLocation(screenSize);
        for (Node node : graph.getNodeList()) node.getVisual().updateLocation(screenSize);
        repaint();
    }
    ...
    public Node findNode(Point point) {
        ...
        for (Node node : graph.getNodeList()) if (node.getVisual().hit(screenSize, point)) return node;
        return null;
    }
    ...
    public Edge findEdge(Point point) {
        Graph graph = controller.getGraph();
        for (Edge edge : graph.getEdgeList()) if (edge.getVisual().hit(screenSize, point)) return edge;
        return null;
    }
