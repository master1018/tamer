...
public class UpgradeDatabase {
    ...
    private static Log mLogger = LogFactory.getFactory().getInstance(UpgradeDatabase.class);

    public static void upgradeDatabase(Connection con) throws RollerException {
        try {
            Statement versionStatement = con.createStatement();
            ResultSet versionResultSet = versionStatement.executeQuery("select dbversion from rollerconfig");
            versionResultSet.next();
            String dbversion = versionResultSet.getString(1);
            if (dbversion != null) return;
            PreparedStatement rootCatStatement = con.prepareStatement("select a.id from weblogcategoryassoc as a, weblogcategory as c " + "where c.websiteid=? and a.categoryid=c.id and a.ancestorid is null and a.relation='PARENT'");
            PreparedStatement rootCatCreateStatement = con.prepareStatement("insert into weblogcategory (id,name,description,websiteid,image) " + "values (?,'root','root',?,NULL)");
            PreparedStatement updateWebsiteStatement = con.prepareStatement("update website set bloggercatid=?, defaultcatid=? where id=?");
            PreparedStatement catsStatement = con.prepareStatement("select id from weblogcategory where websiteid=? and id<>?");
            PreparedStatement assocCreateStatement = con.prepareStatement("insert into weblogcategoryassoc (id,categoryid,ancestorid,relation) " + "values (?,?,?,'PARENT')");
            PreparedStatement rootFolderStatement = con.prepareStatement("select id from folder where websiteid=? and parentid is null");
            PreparedStatement foldersStatement = con.prepareStatement("select id,parentid from folder where websiteid=?");
            PreparedStatement folderAssocCreateStatement = con.prepareStatement("insert into folderassoc (id,folderid,ancestorid,relation) " + "values (?,?,?,'PARENT')");
            Statement websitesStatement = con.createStatement();
            ResultSet websitesResultSet = websitesStatement.executeQuery("select id from website");
            ...
            versionUpdateStatement.executeUpdate("update rollerconfig set dbversion='1.0'");
