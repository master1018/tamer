...
public class RouteEditTableModel extends javax.swing.table.AbstractTableModel implements PropertyChangeListener {
    ...
    void initTable(JTable table, Route route) {
        _table = table;
        _route = route;
        if (_route != null) _route.addPropertyChangeListener(this);
        initTable(table);
    }
    ...
    public String getColumnName(int col) {
        switch(col) {
            ...
            case TRAINCOLUMN:
                return rb.getString("TrainDirection");
            case MAXMOVESCOLUMN:
                return rb.getString("MaxMoves");
            case PICKUPCOLUMN:
                return rb.getString("Pickups");
            case DROPCOLUMN:
                return rb.getString("Drops");
            case WAITCOLUMN:
                {
                    if (_showWait) return rb.getString("Wait"); else return rb.getString("Time");
                }
            case MAXLENGTHCOLUMN:
                return rb.getString("MaxLength");
            case GRADE:
                return rb.getString("Grade");
            case TRAINICONX:
                return rb.getString("X");
            case TRAINICONY:
                return rb.getString("Y");
            case COMMENTCOLUMN:
                return rb.getString("Comment");
            case UPCOLUMN:
                return "";
            case DOWNCOLUMN:
                return "";
    ...
    public Class<?> getColumnClass(int col) {
        switch(col) {
            ...
            case WAITCOLUMN:
                {
                    if (_showWait) return String.class; else return JComboBox.class;
                }
            case MAXLENGTHCOLUMN:
                return String.class;
            case GRADE:
    ...
    public Object getValueAt(int row, int col) {
        if (row >= list.size()) return "ERROR unknown " + row;
        RouteLocation rl = _route.getLocationById(list.get(row));
        if (rl == null) return "ERROR unknown route location " + row;
        switch(col) {
            case IDCOLUMN:
                return rl.getId();
            case NAMECOLUMN:
            ...
            case COMMENTCOLUMN:
                {
                    if (rl.getComment().equals("")) return rb.getString("Add"); else return rb.getString("Edit");
                }
            case UPCOLUMN:
                return rb.getString("Up");
            case DOWNCOLUMN:
                return rb.getString("Down");
            case DELETECOLUMN:
    ...
    public void setValueAt(Object value, int row, int col) {
        ...
        switch(col) {
            ...
            case WAITCOLUMN:
                {
                    if (_showWait) setWait(value, row); else setDepartureTime(value, row);
                }
                break;
            case MAXLENGTHCOLUMN:
    ...
    private void setMaxTrainMoves(Object value, int row) {
        ...
        if (moves <= 100) {
            ...
            JOptionPane.showMessageDialog(null, rb.getString("MaximumLocationMoves"), rb.getString("CanNotChangeMoves"), JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void setWait(Object value, int row) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, rb.getString("EnterWaitTimeMinutes"), rb.getString("WaitTimeNotValid"), JOptionPane.ERROR_MESSAGE);
            return;
        }
        rl.setWait(wait);
    }
    ...
    private void setMaxTrainLength(Object value, int row) {
        ...
        if (length <= Setup.getTrainLength()) {
            ...
            JOptionPane.showMessageDialog(null, MessageFormat.format(rb.getString("DepartureLengthNotExceed"), new Object[] { length, Setup.getTrainLength() }), rb.getString("CanNotChangeMaxLength"), JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void setGrade(Object value, int row) {
        ...
        if (grade <= 6 && grade >= -6) {
            ...
            JOptionPane.showMessageDialog(null, rb.getString("MaxGrade"), rb.getString("CanNotChangeGrade"), JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void setComment(int row) {
        ...
        RouteLocation rl = _route.getLocationById(list.get(row));
        Object comment = JOptionPane.showInputDialog(null, rb.getString("Comment"), MessageFormat.format(rb.getString("EnterCommentLocation"), new Object[] { rl.getName() }), JOptionPane.PLAIN_MESSAGE, null, null, rl.getComment());
    ...
    private JComboBox getTimeComboBox() {
        ...
        timeBox.addItem("");
        for (int i = 0; i < 24; i++) {
            if (i < 10) hour = "0" + Integer.toString(i); else hour = Integer.toString(i);
            for (int j = 0; j < 60; j = j + 5) {
                if (j < 10) minute = "0" + Integer.toString(j); else minute = Integer.toString(j);
    ...
    private void removePropertyChangeRouteLocations() {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (rl != null) rl.removePropertyChangeListener(this);
        }
    }
    ...
    public void dispose() {
        ...
        removePropertyChangeRouteLocations();
        if (_route != null) _route.removePropertyChangeListener(this);
        list.clear();
        fireTableDataChanged();
    }
