...
public class GuiceBerryUniverseTest {
    ...
    @Test
    public void testExceptionWhenGbeDoesNotInstallGuiceBerryModule0() {
        GuiceBerryEnvSelector guiceBerryEnvSelector = DefaultEnvSelector.of(NonGuiceBerryEnvSinceItDoesNotInstallGuiceBerryModule.class);
        TestDescription testDescription = bogusTestDescription();
        GuiceBerryUniverse.TestCaseScaffolding testCaseScaffolding = new GuiceBerryUniverse.TestCaseScaffolding(testDescription, guiceBerryEnvSelector, universe);
        try {
            testCaseScaffolding.runBeforeTest();
        } catch (IllegalArgumentException e) {
            Assert.assertEquals(String.format("The GuiceBerry Env '%s' must call 'install(new GuiceBerryModule())' " + "in its 'configure()' method, so as to install the bindings defined there.", NonGuiceBerryEnvSinceItDoesNotInstallGuiceBerryModule.class.getName()), e.getMessage());
        }
    ...
    @Test
    public void testExceptionWhenGbeDoesNotInstallGuiceBerryModule1() {
        GuiceBerryEnvSelector guiceBerryEnvSelector = DefaultEnvSelector.of(NonGuiceBerryEnvNonAbstractModuleSinceItDoesNotInstallGuiceBerryModule.class);
        TestDescription testDescription = bogusTestDescription();
        GuiceBerryUniverse.TestCaseScaffolding testCaseScaffolding = new GuiceBerryUniverse.TestCaseScaffolding(testDescription, guiceBerryEnvSelector, universe);
        try {
            testCaseScaffolding.runBeforeTest();
        } catch (IllegalArgumentException e) {
            Assert.assertEquals(String.format("The GuiceBerry Env '%s' must call 'binder.install(new GuiceBerryModule()' " + "in its 'configure(Binder)' method, so as to install the bindings defined there.", NonGuiceBerryEnvNonAbstractModuleSinceItDoesNotInstallGuiceBerryModule.class.getName()), e.getMessage());
        }
        testCaseScaffolding.runAfterTest();
    }
    ...
    @Test
    public void testFailingTestDoesNotSpoilThreadLocal() {
        GuiceBerryEnvSelector guiceBerryEnvSelector = DefaultEnvSelector.of(MyGuiceBerryEnvThatThrowsOnTestWrapperBeforeTest.class);
        TestDescription testDescription = bogusTestDescription();
        GuiceBerryUniverse.TestCaseScaffolding testCaseScaffolding = new GuiceBerryUniverse.TestCaseScaffolding(testDescription, guiceBerryEnvSelector, universe);
        ...
        testCaseScaffolding.runAfterTest();
        Assert.assertEquals("The thread local tear down must be done even if the" + "TestWrapper fails.", null, universe.currentTestDescriptionThreadLocal.get());
