...
public class CheckClusterer extends CheckScheme {
   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (tmpStr.length() != 0) setNumInstances(Integer.parseInt(tmpStr)); else setNumInstances(40);
   ...
   */
    public String[] getOptions() {
        ...
        if ((m_Clusterer != null) && (m_Clusterer instanceof OptionHandler)) options = ((OptionHandler) m_Clusterer).getOptions(); else options = new String[0];
        if (options.length > 0) {
            result.add("--");
            for (i = 0; i < options.length; i++) result.add(options[i]);
        }
   ...
   */
    protected void runTests(boolean weighted, boolean multiInstance, boolean updateable) {
        ...
        boolean PRel;
        if (!multiInstance) PRel = canPredict(false, false, false, false, true, multiInstance)[0]; else PRel = false;
        if (PNom || PNum || PStr || PDat || PRel) {
            if (weighted) instanceWeights(PNom, PNum, PStr, PDat, PRel, multiInstance);
            ...
            if (handleMissingPredictors) canHandleMissing(PNom, PNum, PStr, PDat, PRel, multiInstance, true, 100);
   ...
   */
    protected boolean[] updatingEquality(boolean nominalPredictor, boolean numericPredictor, boolean stringPredictor, boolean datePredictor, boolean relationalPredictor, boolean multiInstance) {
        ...
        try {
            ...
            result[0] = false;
            print("Problem during");
            if (built) print(" testing"); else print(" training");
            println(": " + ex.getMessage() + "\n");
        }
        return result;
    }
