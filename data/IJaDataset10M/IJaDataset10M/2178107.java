...
public class HelpFormatter {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            while (linePos < line.length()) {
                if (length >= line.length() - linePos) {
                    ...
                    while (whiteSpacePos >= linePos && !Character.isWhitespace(line.charAt(whiteSpacePos))) {
                        whiteSpacePos--;
                    }
                    if (whiteSpacePos >= linePos) {
                        final int printedChars = whiteSpacePos - linePos + 1;
                        while (whiteSpacePos > linePos && Character.isWhitespace(line.charAt(whiteSpacePos - 1))) {
                            whiteSpacePos--;
                        }
     ...
     * @return the formatted {@link String}
     */
    public static String formatUsage(CommandCollection commands, Set<Option> optionsValidWithoutCommand) {
        final StringBuilder sb = new StringBuilder();
