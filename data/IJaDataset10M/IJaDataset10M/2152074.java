...
public class Skill_ScrollCopy extends StdSkill {
    ...
    public boolean invoke(MOB mob, Vector commands, Environmental givenTarget, boolean auto, int asLevel) {
        ...
        while (T.numLearnedAbilities() > 0) {
            ...
            if (A != null) T.delAbility(A);
        }
        thisSpell.setProficiency(50);
        T.addAbility(thisSpell);
        if (!thisSpell.canBeLearnedBy(T, mob)) return false;
        ...
        if (success) {
            if (mob.location().show(mob, target, this, CMMsg.MSG_HANDS, "<S-NAME> memorize(s) '" + thisSpell.name() + "' from <T-NAME>.")) {
                ...
                if ((mob.fetchAbility(thisSpell.ID()) != null) && (CMLib.ableMapper().qualifyingClassLevel(mob, this) >= 0) && (CMLib.ableMapper().qualifyingClassLevel(mob, thisSpell) >= 0)) {
                    if ((thisSpell.practicesRequired(mob) > 0) || (thisSpell.trainsRequired(mob) > 0)) {
                        ...
                        if (xp >= 0) CMLib.leveler().postExperience(mob, null, null, xp, false);
                    }
                }
            }
        } else mob.location().show(mob, null, CMMsg.MSG_HANDS, "<S-NAME> attempt(s) to memorize '" + thisSpell.name() + "' from " + target.name() + ", but fail(s).");
        return success;
    }
}
