...
public class PolyGoneSegment extends Solid {
    ...
    * @preconditions sides > 2 */
    public PolyGoneSegment(double[] rmins, double[] rmaxs, double[] zs, double phimin, double phimax, int sides, Appearance appearance) {
        construct(rmins, rmaxs, zs, phimin, phimax, sides, appearance);
    }
    ...
    private void construct(double[] rmins, double[] rmaxs, double[] zs, double phimin, double phimax, int sides, Appearance appearance) {
        ...
        for (int j = 0; j < sides + 1; j++) {
            ...
            for (int i = 0; i < zs.length; i++) {
                outer[i][j] = new Point3d(rmaxs[i] * Math.cos(phi), rmaxs[i] * Math.sin(phi), zs[i]);
                inner[i][j] = new Point3d(rmins[i] * Math.cos(phi), rmins[i] * Math.sin(phi), zs[i]);
            }
        }
        ...
        for (int i = 0; i < zs.length; i++) {
            outermin[i] = new Point3d(rmaxs[i] * smin * Math.cos(phimin0), rmaxs[i] * smin * Math.sin(phimin0), zs[i]);
            innermin[i] = new Point3d(rmins[i] * smin * Math.cos(phimin0), rmins[i] * smin * Math.sin(phimin0), zs[i]);
            outermax[i] = new Point3d(rmaxs[i] * smax * Math.cos(phimax0), rmaxs[i] * smax * Math.sin(phimax0), zs[i]);
            innermax[i] = new Point3d(rmins[i] * smax * Math.cos(phimax0), rmins[i] * smax * Math.sin(phimax0), zs[i]);
        }
