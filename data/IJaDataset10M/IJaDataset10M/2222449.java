...
public class CompressionModel<T> extends LevelStatisticsModel<T> {
    ...
    public NullModel nullModel = null;

    public CompressionModel() {
        super();
    }

    public CompressionModel(Corpus<T> corpus) throws IOException {
        super();
        add(corpus);
    }
    ...
    public void add(T token) {
        super.add(token);
        if (!indices.containsKey(token)) indices.put(token, new ArrayList<Integer>());
    ...
    public double compression(T token) {
        if (compCache.containsKey(token)) return compCache.get(token);
        BitString bs = BitString.zeros(total);
        if (indices.containsKey(token)) for (int i : indices.get(token)) bs.set(i - 1, true);
    ...
    public double compressionNormalized(T token) {
        if (normCompCache.containsKey(token)) return normCompCache.get(token);
        if (nullModel == null) nullModel = new NullModel(trials, res, total, exp);
        double p = probability(token);
        double mean = nullModel.getMean(p), std = Math.sqrt(nullModel.getVariance(p));
    ...
    public String out(T token, boolean printToken) {
        return super.out(token, printToken) + ", " + compression(token) + ", " + compressionNormalized(token);
    }
    ...
    public class NullModel {
        ...
        public double getMean(double probability) {
            ...
            if (!map.containsKey(i)) add(probability);
        ...
        public void add(double probability) {
            ...
            for (int j = 0; j < trials; j++) {
                ...
                for (int k = 0; k < length; k++) array.add(Global.random.nextDouble() < probability);
