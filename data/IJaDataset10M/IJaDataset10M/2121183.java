package com.ibm.celldt.environment.ui.deploy.wizard;
...
import com.ibm.celldt.environment.ui.deploy.DeployPlugin;
import com.ibm.celldt.environment.ui.deploy.events.AbstractCellTargetJob;
import com.ibm.celldt.environment.ui.deploy.events.ImportJobWrapper;
import com.ibm.celldt.environment.ui.deploy.events.Messages;
import com.ibm.celldt.remotetools.core.IRemoteFileTools;
...
import com.ibm.celldt.remotetools.exception.RemoteExecutionException;
...
public class CellImportResourcesPage extends AbstractCellWizardPage {
    ...
    class VerifyPathTargetJob extends AbstractCellTargetJob {
        ...
        public void run() {
            try {
                ...
                if (isDir) hasPath = fileTools.hasDirectory(path); else hasPath = fileTools.hasFile(path);
    ...
    protected void createDirectoryStructureOptions(Composite optionsGroup, Font font) {
        createSelectedDir = new Button(optionsGroup, SWT.RADIO | SWT.LEFT);
        ...
        copyDirContentsOnly = new Button(optionsGroup, SWT.RADIO | SWT.LEFT);
    ...
    private boolean doesResourceTableContain(String name) {
        ...
        for (int i = 0; i < items.length; ++i) {
            if ((items[i].getText(0) != null) && (items[i].getText(0).equals(name))) return true;
        }
        return false;
    }
    ...
    public boolean finish() {
        ...
        if (!destDir.exists()) {
            boolean create = MessageDialog.openQuestion(getContainer().getShell(), Messages.CellImportResourcesPage_17, Messages.CellImportResourcesPage_18);
            if (create) destDir.mkdirs(); else return false;
        }
        if (destDir.exists() && destDir.isFile()) {
            boolean createDir = MessageDialog.openQuestion(getContainer().getShell(), Messages.CellImportResourcesPage_19, Messages.CellImportResourcesPage_20);
            if (createDir) {
                destDir.delete();
                destDir.mkdirs();
            } else return false;
        ...
        if (!destDir.canWrite()) {
            MessageDialog.openInformation(getContainer().getShell(), Messages.CellImportResourcesPage_21, Messages.CellImportResourcesPage_22);
            return false;
        }
        saveDirtyEditors();
        saveWidgetValues();
        ...
        MessageDialog.openInformation(getContainer().getShell(), Messages.CellImportResourcesPage_26, Messages.CellImportResourcesPage_27);
        return true;
    }
    ...
    public void handleEvent(Event e) {
        ...
        if (sourceValid) setErrorMessage(null); else sourceValid = true;
    }
    ...
    protected boolean validatePath(String path, boolean isDir) {
        ...
        try {
            ...
            MessageDialog.openError(getContainer().getShell(), "Interrupted", "The operation was canceled");
        }
        return false;
    }
    ...
    private String removeTrailingSlashes(String path) {
        ...
        if (path.charAt(path.length() - 1) == '/') return removeTrailingSlashes(path.substring(0, path.length() - 1)); else return path;
    }
