...
public class Interpreter implements MacroConstants {
    ...
    public static final int NONE = 0, STEP = 1, TRACE = 2, FAST_TRACE = 3, RUN = 4, RUN_TO_CARET = 5;
    ...
    public void run(String macro) {
        if (additionalFunctions != null) {
            if (!(macro.endsWith("\n") || additionalFunctions.startsWith("\n"))) macro = macro + "\n" + additionalFunctions; else macro = macro + additionalFunctions;
        }
        Tokenizer tok = new Tokenizer();
        Program pgm = tok.tokenize(macro);
        if (pgm.hasVars && pgm.hasFunctions) saveGlobals2(pgm);
        run(pgm);
    }
    ...
    public String run(String macro, String arg) {
        argument = arg;
        calledMacro = true;
        if (IJ.getInstance() == null) setBatchMode(true);
    ...
    public void run(Program pgm) {
        ...
        pushGlobals();
        if (func == null) func = new Functions(this, pgm);
        func.plot = null;
        doStatements();
        finishUp();
    }
    ...
    public void runMacro(Program pgm, int macroLoc, String macroName) {
        ...
        if (func == null) func = new Functions(this, pgm);
        func.plot = null;
        if (macroLoc == 0) doStatements(); else doBlock();
        finishUp();
    ...
    void doStatements() {
        while (!done) doStatement();
    }

    final void doStatement() {
        getToken();
        if (debugMode != NONE && editor != null && !done && token != ';' && token != FUNCTION) editor.debug(this, debugMode);
        switch(token) {
            case VAR:
                doVar();
                break;
            ...
            case '(':
                ...
                String s = getString();
                inPrint = false;
                if (s != null && s.length() > 0 && !s.equals("NaN") && !s.equals("[aborted]")) IJ.log(s);
                return;
            case ARRAY_FUNCTION:
    ...
    int pushArgs() {
        ...
        if (nextToken() != ')') {
            do {
                if (count == MAX_ARGS) error("Too many arguments");
                int next = nextToken();
                int nextPlus = pgm.code[pc + 2] & 0xff;
                if (next == STRING_CONSTANT || next == STRING_FUNCTION) args[count] = new Variable(0, 0.0, getString()); else if (next == USER_FUNCTION) {
                    int savePC = pc;
                    getToken();
                    ...
                    if (simpleFunctionCall) {
                        ...
                        if (v2 == null) error("No return value");
                        args[count] = v2;
                    } else args[count] = new Variable(0, getExpression(), null);
                    ...
                    if (v != null) {
                        int type = v.getType();
                        if (type == Variable.VALUE) value = v.getValue(); else if (type == Variable.ARRAY) array = v.getArray(); else str = v.getString();
                    }
                    ...
                    Variable v = lookupVariable();
                    v = getArrayElement(v);
                    if (v.getString() != null) args[count] = new Variable(0, 0.0, v.getString(), null); else {
                        pc = savePC;
        ...
        int nArgs = count;
        while (count > 0) push(args[--count], this);
        getRightParen();
        return nArgs;
    }
    ...
    void setupArgs(int nArgs) {
        ...
        if (nextToken() != ')') {
            do {
                getToken();
                if (i >= 0) stack[i].symTabIndex = tokenAddress;
                i--;
                count--;
                getToken();
            } while (token == ',');
        ...
        if (count != 0) error(nArgs + " argument" + (nArgs == 1 ? "" : "s") + " expected");
        getRightParen();
    }
    ...
    void doReturn() {
        ...
        if (token != ';') {
            ...
            putTokenBack();
            if (isString) str = getString(); else if (isArrayFunction) {
                getToken();
                array = func.getArrayFunction(pgm.table[tokenAddress].type);
            } else if (array == null) value = getExpression();
        }
        if (inFunction) {
            if (returnException == null) returnException = new ReturnException();
            ...
            finishUp();
            if (value != 0.0 || array != null) error("Macros can only return strings");
            returnValue = str;
            done = true;
        }
    }
    ...
    void doFor() {
        ...
        getToken();
        if (token != VAR) putTokenBack();
        do {
            if (nextToken() != ';') getAssignmentExpression();
            getToken();
        } while (token == ',');
        if (token != ';') error("';' expected");
        int condPC = pc;
        ...
        double cond = 1;
        while (true) {
            if (pgm.code[pc + 1] != ';') cond = getLogicalExpression();
            if (startPC == 0) checkBoolean(cond);
            getToken();
            if (token != ';') error("';' expected");
            int incPC = pc;
            if (startPC != 0) pc = startPC; else {
                while (token != ')') {
                    getToken();
                    if (token == '{' || token == ';' || token == '(' || done) error("')' expected");
                }
            }
            startPC = pc;
            if (cond == 1) doStatement(); else {
                skipStatement();
                break;
            }
            pc = incPC;
    ...
    void doWhile() {
        ...
        do {
            pc = savePC;
            isTrue = getBoolean();
            if (isTrue) doStatement(); else skipStatement();
        } while (isTrue && !done);
    }
    ...
    void doDo() {
        ...
        do {
            doStatement();
            getToken();
            if (token != WHILE) error("'while' expected");
            isTrue = getBoolean();
            if (isTrue) pc = savePC;
        } while (isTrue && !done);
    }
    ...
    final void skipStatement() {
        ...
        switch(token) {
            ...
            case IF:
                ...
                skipStatement();
                getToken();
                if (token == ELSE) skipStatement(); else putTokenBack();
                break;
            case FOR:
                skipParens();
                skipStatement();
    ...
    final void skipBlock() {
        ...
        do {
            getToken();
            if (token == '{') count++; else if (token == '}') count--; else if (done) {
                error("'}' expected");
                return;
            }
        } while (count > 0);
    ...
    final void skipSimpleStatement() {
        ...
        getToken();
        while (!finished && !done) {
            if (token == ';') finished = true; else if (token == ELSE || (token == PREDEFINED_FUNCTION && pgm.code[pc - 1] != '.')) error("';' expected"); else getToken();
        }
    }
    ...
    int getExpressionType() {
        ...
        if (tok == STRING_CONSTANT) return Variable.STRING;
        if (tok == STRING_FUNCTION) {
            ...
            if (type == DIALOG) {
                ...
                if (name.equals("getNumber") || name.equals("getCheckbox")) return STRING_FUNCTION;
            } else if (type == FILE) {
                int token2 = pgm.code[pc + 4];
                String name = pgm.table[token2 >> TOK_SHIFT].str;
                if (name.equals("exists") || name.equals("isDirectory") || name.equals("length") || name.equals("getLength") || name.equals("rename") || name.equals("delete")) return STRING_FUNCTION;
            } else if (type == LIST) {
                int token2 = pgm.code[pc + 4];
                String name = pgm.table[token2 >> TOK_SHIFT].str;
        ...
        if (tok == ARRAY_FUNCTION) return Variable.ARRAY;
        if (tok == USER_FUNCTION) return USER_FUNCTION;
        if (tok != WORD) return Variable.VALUE;
        Variable v = lookupVariable(rightSideToken >> TOK_SHIFT);
        if (v == null) return Variable.VALUE;
        int type = v.getType();
        if (type != Variable.ARRAY) return type;
        if (pgm.code[pc + 3] == '.') return Variable.VALUE;
        if (pgm.code[pc + 3] != '[') return Variable.ARRAY;
        int savePC = pc;
        getToken();
        getToken();
        checkingType = true;
        ...
        if (index < 0 || index >= array.length) return Variable.VALUE;
    ...
    final void doArrayElementAssignment() {
        Variable v = lookupLocalVariable(tokenAddress);
        if (v == null) error("Undefined identifier");
        ...
        if (expressionType == Variable.ARRAY) error("Arrays of arrays not supported");
        getToken();
        int op = token;
        ...
        Variable[] array = v.getArray();
        if (array == null) error("Array expected");
        if (index < 0 || index >= array.length) error("Index (" + index + ") out of 0-" + (array.length - 1) + " range");
        int next = nextToken();
        switch(expressionType) {
            ...
            case Variable.ARRAY:
                getToken();
                if (token == ARRAY_FUNCTION) array[index].setArray(func.getArrayFunction(pgm.table[tokenAddress].type));
                break;
            default:
                switch(op) {
                    case '=':
    ...
    final void doUserFunctionAssignment() {
        ...
        pc = savePC;
        if (!simpleAssignment) getAssignmentExpression(); else {
            getToken();
            Variable v1 = lookupLocalVariable(tokenAddress);
            if (v1 == null) v1 = push(tokenAddress, 0.0, null, this);
            getToken();
            if (token != '=') error("'=' expected");
            getToken();
            Variable v2 = runUserFunction();
            ...
            if (done) return;
            int type = v2.getType();
            if (type == Variable.VALUE) v1.setValue(v2.getValue()); else if (type == Variable.ARRAY) v1.setArray(v2.getArray()); else v1.setString(v2.getString());
        }
    }
    ...
    boolean isSimpleFunctionCall(boolean assignment) {
        ...
        do {
            getToken();
            if (token == '(') count++; else if (token == ')') count--; else if (done) error("')' expected");
        } while (count > 0);
        getToken();
        if (assignment) return token == ';'; else return token == ',' || token == ')';
    }

    final void doStringAssignment() {
        Variable v = lookupLocalVariable(tokenAddress);
        if (v == null) {
            if (nextToken() == '=') v = push(tokenAddress, 0.0, null, this); else error("Undefined identifier");
        }
        getToken();
        if (token == '=') v.setString(getString()); else if (token == PLUS_EQUAL) v.setString(v.getString() + getString()); else error("'=' or '+=' expected");
    }
    ...
    final void doArrayAssignment() {
        ...
        getToken();
        if (token == ARRAY_FUNCTION) v.setArray(func.getArrayFunction(pgm.table[tokenAddress].type)); else if (token == WORD) {
    ...
    final void doIf() {
        ...
        if (b) doStatement(); else skipStatement();
        int next = nextToken();
        if (next == ';') {
            getToken();
            next = nextToken();
        ...
        if (next == ELSE) {
            getToken();
            if (b) skipStatement(); else doStatement();
        }
    }
    ...
    final double getLogicalExpression() {
        ...
        int next = nextToken();
        if (!(next == LOGICAL_AND || next == LOGICAL_OR)) return v1;
        checkBoolean(v1);
        getToken();
        int op = token;
        double v2 = getLogicalExpression();
        checkBoolean(v2);
        if (op == LOGICAL_AND) return (int) v1 & (int) v2; else if (op == LOGICAL_OR) return (int) v1 | (int) v2;
        return v1;
    }
    ...
    final double getBooleanExpression() {
        ...
        if (tok == STRING_CONSTANT || tok == STRING_FUNCTION || isString(next)) s1 = getString(); else v1 = getExpression();
        next = nextToken();
        if (next >= EQ && next <= LTE) {
            getToken();
            int op = token;
            if (s1 != null) return compareStrings(s1, getString(), op);
        ...
        } else if (s1 != null) v1 = Tools.parseDouble(s1, 0.0);
        return v1;
    }
    ...
    boolean isString(int token) {
        ...
        if (pgm.code[pc + 2] == '[') {
            Variable[] array = v.getArray();
            if (array != null && array.length > 0) return array[0].getType() == Variable.STRING;
        }
    ...
    final double getAssignmentExpression() {
        ...
        if ((pgm.code[pc + 1] & 0xff) == WORD && (tokPlus2 == '=' || tokPlus2 == PLUS_EQUAL || tokPlus2 == MINUS_EQUAL || tokPlus2 == MUL_EQUAL || tokPlus2 == DIV_EQUAL)) {
            ...
            getToken();
            double value = 0.0;
            if (token == '=') value = getAssignmentExpression(); else {
                value = v.getValue();
                switch(token) {
                    case PLUS_EQUAL:
            ...
            v.setValue(value);
            return value;
        } else return getLogicalExpression();
    }

    final void checkBoolean(double value) {
        if (!(value == 0.0 || value == 1.0)) error("Boolean expression expected");
    }

    void doVar() {
        getToken();
        while (token == WORD) {
            if (nextToken() == '=') doAssignment(); else {
                Variable v = lookupVariable(tokenAddress);
                if (v == null) push(tokenAddress, 0.0, null, this);
            }
            getToken();
    ...
    final void getComma() {
        getToken();
        if (token != ',') {
            if (looseSyntax) putTokenBack(); else error("',' expected");
        }
    }
    ...
    void showError(String title, String msg, String[] variables) {
        ...
        if (!gd.wasCanceled() && showVariables) updateDebugWindow(variables, null);
    }
    ...
    public TextWindow updateDebugWindow(String[] variables, TextWindow debugWindow) {
        ...
        if (debugWindow == null) debugWindow = new TextWindow("Debug", "Name\tValue", "", 300, 400);
        ...
        for (int i = 0; i < lines; i++) {
            if (i < n) panel.setLine(i, variables[i]); else panel.setLine(i, "");
        }
        for (int i = lines; i < n; i++) debugWindow.append(variables[i]);
        return debugWindow;
    }
    ...
    final String getString() {
        ...
        while (true) {
            getToken();
            if (token == '+') str += getStringTerm(); else {
                putTokenBack();
                break;
            }
        }
    ...
    final String getStringTerm() {
        ...
        switch(token) {
            ...
            case USER_FUNCTION:
                ...
                if (v == null) error("No return value");
                str = v.getString();
                if (str == null) {
                    double value = v.getValue();
                    if ((int) value == value) str = IJ.d2s(value, 0); else str = "" + value;
                }
                break;
            case WORD:
            ...
            default:
                ...
                if ((int) value == value) str = IJ.d2s(value, 0); else {
                    str = "" + value;
                    if (inPrint && value != Double.POSITIVE_INFINITY && value != Double.NEGATIVE_INFINITY && value != Double.NaN && (str.length() - str.indexOf('.')) > 6 && str.indexOf('E') == -1) str = IJ.d2s(value, 4);
                }
        }
        return str;
    }
    ...
    final double getFactor() {
        ...
        switch(token) {
            ...
            case STRING_FUNCTION:
                ...
                if (Double.isNaN(value)) error("Numeric value expected");
                break;
            case USER_FUNCTION:
                v = runUserFunction();
                if (v == null) error("No return value");
                if (done) value = 0; else {
                    if (v.getString() != null) error("Numeric return value expected"); else value = v.getValue();
                }
                break;
            case TRUE:
                value = 1.0;
            ...
            case WORD:
                ...
                if (!(next == PLUS_PLUS || next == MINUS_MINUS)) break;
                getToken();
                if (token == PLUS_PLUS) v.setValue(v.getValue() + (checkingType ? 0 : 1)); else v.setValue(v.getValue() - (checkingType ? 0 : 1));
                break;
            case (int) '(':
    ...
    final double getArrayLength(Variable v) {
        getToken();
        getToken();
        if (!(token == WORD && tokenString.equals("length"))) error("'length' expected");
        Variable[] array = v.getArray();
        if (array == null) error("Array expected");
        return array.length;
    }
    ...
    final double getStringExpression() {
        ...
        while (true) {
            ...
            if (token == '+') {
                ...
                putTokenBack();
                value += getTerm();
            } else if (token == '-') value -= getTerm(); else {
                putTokenBack();
                break;
            }
        }
    ...
    Variable push(Variable var, Interpreter interp) {
        if (stack == null) stack = new Variable[STACK_SIZE];
        if (topOfStack >= (STACK_SIZE - 2)) interp.error("Stack overflow"); else topOfStack++;
        stack[topOfStack] = var;
        return var;
    }

    void pushGlobals() {
        if (pgm.globals == null) return;
        if (stack == null) stack = new Variable[STACK_SIZE];
    ...
    void trimStack(int previousTOS, int previousStartOfLocals) {
        for (int i = previousTOS + 1; i <= topOfStack; i++) stack[i] = null;
    ...
    void undefined() {
        if (nextToken() == '(') error("Undefined identifier"); else error("Undefined variable");
    }
    ...
    void dumpStack() {
        IJ.log("");
        IJ.log("Stack");
        if (stack != null) for (int i = topOfStack; i >= 0; i--) IJ.log(i + " " + pgm.table[stack[i].symTabIndex].str + " " + stack[i]);
    }
    ...
    void finishUp() {
        ...
        if (rgbWeights != null) ColorProcessor.setWeightingFactors(rgbWeights[0], rgbWeights[1], rgbWeights[2]);
    ...
    public static void abort() {
        if (instance != null) instance.abortMacro();
    }
    ...
    public static void abort(Interpreter interp) {
        if (interp != null) interp.abortMacro();
    }
    ...
    public void abortMacro() {
        ...
        if (func != null && !(macroName != null && macroName.indexOf(" Tool") != -1)) func.abortDialog();
    ...
    public static void addBatchModeImage(ImagePlus imp) {
        ...
        if (imageTable == null) imageTable = new Vector();
    ...
    public static void removeBatchModeImage(ImagePlus imp) {
        if (imageTable != null && imp != null) {
            ...
            if (index != -1) imageTable.removeElementAt(index);
        }
    }

    public static int[] getBatchModeImageIDs() {
        if (!batchMode || imageTable == null) return new int[0];
    ...
    public static int getBatchModeImageCount() {
        if (!batchMode || imageTable == null) return 0; else return imageTable.size();
    }
    ...
    public static RoiManager getBatchModeRoiManager() {
        ...
        if (interp != null && isBatchMode() && RoiManager.getInstance() == null) {
            if (interp.func.roiManager == null) interp.func.roiManager = new RoiManager(true);
    ...
    public void setEditor(Editor ed) {
        if (ed != null && editor == null) ed.fixLineEndings();
        editor = ed;
        if (ed != null) debugMode = STEP; else debugMode = NONE;
    }
    ...
    public double getVariable(String name) {
        ...
        for (int i = 0; i <= topOfStack; i++) {
            index = stack[i].symTabIndex;
            if (pgm.table[index].str.equals(name)) return stack[i].getValue();
        }
        return Double.NaN;
    }
    ...
    public String getStringVariable(String name) {
        ...
        for (int i = topOfStack; i >= 0; i--) {
            index = stack[i].symTabIndex;
            if (pgm.table[index].str.equals(name)) return stack[i].getString();
        }
        return null;
    }
