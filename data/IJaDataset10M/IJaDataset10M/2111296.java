 ...
 */
public class TPPAttributeSearch extends ASSearch implements WindowListener, RankedOutputSearch, OptionHandler, TechnicalInformationHandler, TPPModelEventListener, CapabilitiesHandler {
    ...
    public TechnicalInformation getTechnicalInformation() {
        ...
        result.setValue(Field.TITLE, "Predicting Functional Residues of Protein Sequence Alignments as a Feature Selection Task");
        return result;
    }

    /** Use TPP to select attributes. The Evaluator is ignored. */
    public int[] search(ASEvaluation evaluator, Instances data) throws Exception {
        if (!(evaluator instanceof TPPAttributeEvaluation)) {
            JOptionPane.showMessageDialog(null, "You must use use the TPPAttributeEvaluation method with TPPAttributeSearch");
            return null;
        }
        epochs = 0;
        ...
        if (showView && numOutputDimensions == 2) {
            ...
            if (panel == null) panel = new ScatterPlotViewPanel();
        ...
        if (showView && numOutputDimensions == 2) separator = new SeparatePointsInScatterPlot(model); else separator = new SeparatePoints(model);
        ...
        if (igSearch != null) {
            ...
            for (int i = 0; i < attributeIndices.length; i++) attributeIndices[i] = (int) igAttributes[attributeIndices[i]][0];
        }
        for (int i = 0; i < attributeIndices.length; i++) attributeIndices[i] = model.indexOf(model.getNumericAttributes().get(attributeIndices[i]));
    ...
    public void modelChanged(TPPModelEvent e) {
        if (e.getType() == TPPModelEvent.PROJECTION_CHANGED) {
            ...
            if (previousProjection != null && pursuit != null) {
                ...
                double difference = previousProjection.minus(currentProjection).normF();
                if (difference > 0.0) {
                    epochs++;
                    if (difference / previous < getConvergenceLimit() || epochs > getEpochLimit()) {
    ...
    public double[][] rankedAttributes() throws Exception {
        ...
        int selected = (numToSelect <= 0 ? attributeIndices.length : numToSelect);
    ...
    public Enumeration listOptions() {
        ...
        newVector.addElement(new Option("\tSpecify the number of attributes to select.", "N", 1, "-N <number of attributes>"));
        newVector.addElement(new Option("\tThe limit at which the search is taken to have converged (default=" + CONVERGENCE_LIMIT_DEFAULT + ").", "L", 1, "-L <convergence limit>"));
        newVector.addElement(new Option("\tThe maximum number of training cycles (default=" + EPOCH_LIMIT_DEFAULT + ").", "C", 1, "-C <number of cycles>"));
        newVector.addElement(new Option("\tThe number of output dimensions (default=2).", "D", 1, "-D <number of output dimensions>"));
        newVector.addElement(new Option("\tTPP is most efficient as an attribute selection algorithm when selecting from approximately 100 attributes. Set this parameter to, eg 100, to preselect attributes using simple IG and Ranker search.", "P", 1, "-P <number of attributes>"));
        newVector.addElement(new Option("\tWhether to show the resulting data visualisations (only has effect if number of output dimensions is 2).", "V", 1, "-V"));
    ...
    public String[] getOptions() {
        return new String[] { "-N " + getNumToSelect() + "-P " + getNumToSelect() + (getShowView() ? " -V" : "") + " -L " + getConvergenceLimit() + " -C " + getEpochLimit() + " -D " + getNumOutputDimensions() };
    }
    ...
    public static void main(String[] a) throws Exception {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) instancesFileName = chooser.getSelectedFile().getPath();
