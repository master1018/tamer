...
public class Generic extends StaticReflector {
    ...
    private void addMethod(Class[] types, Procedure p) {
        if (methodTable == null) {
            ...
            for (int i = 0; i < methodTable.length; i = i + Invoke.BUCKET_SIZE) if (Invoke.parameterTypesMatch(((Class[]) methodTable[i]), types)) {
                install(i, types, p);
                return;
            }
            grow();
    ...
    public Object apply(Object[] args) {
        ...
        if (m.minArgs == 0 && m.maxArgs == Integer.MAX_VALUE) return m.apply(new Object[] { U.vectorToList(args) }); else return m.apply(args);
    }

    protected synchronized void reset() {
        for (int i = 0; i < methodTable.length; i = i + Invoke.BUCKET_SIZE) methodTable[i] = resetParameters((Class[]) methodTable[i]);
    }

    private Object[] resetParameters(Class[] parameters) {
        Class[] parameters2 = new Class[parameters.length];
        for (int i = 0; i < parameters2.length; i++) parameters2[i] = Import.forName(parameters[i].getName());
        return parameters2;
    }
}
