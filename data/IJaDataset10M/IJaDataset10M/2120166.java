...
public class CalendarTest extends junit.framework.TestCase {
    ...
    public void test_clear() {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (5 == index) {
                assertEquals("Field " + index + " Should equal to 20.", 20, calendar.get(index));
            } else if (11 == index) {
                assertEquals("Field " + index + " Should equal to 17.", 17, calendar.get(index));
            } else {
                assertEquals("Field " + index + " Should equal to " + defaults[i] + ".", defaults[i], calendar.get(index));
            }
        }
        calendar.set(2008, 3, 20, 17, 28, 12);
        calendar.clear();
    ...
    public void test_getInstance() {
        ...
        assertEquals(Calendar.SUNDAY, us_calendar.getFirstDayOfWeek());
        assertEquals(Calendar.MONDAY, ch_calendar.getFirstDayOfWeek());
        Calendar gmt_calendar = Calendar.getInstance(TimeZone.getTimeZone("GMT"), Locale.US);
        assertEquals(TimeZone.getTimeZone("GMT"), gmt_calendar.getTimeZone());
        Calendar est_calendar = Calendar.getInstance(TimeZone.getTimeZone("EST"), Locale.US);
        assertEquals(TimeZone.getTimeZone("EST").getID(), est_calendar.getTimeZone().getID());
    }
