...
public class RB2CubicBezierSpline extends CubicBezierSpline {
	 ...
	 */
    public RB2CubicBezierSpline(double[] grid, double[] fvalues, double deviation_0, double deviation_dim, boolean cdfMode) {
	 ...
	 * @param cdfMode indicates spline is in cdf mode
	 */
    public RB2CubicBezierSpline(double a, double b, int n, double[] fvalues, double deviation_0, double deviation_dim, boolean cdfMode) {
	 ...
	 * @param deviation_dim value of the first derivative of f at the right border of the grid
	 */
    public RB2CubicBezierSpline(double a, double b, int n, double[] fvalues, double deviation_0, double deviation_dim) {
    ...
    protected double[] solveLGS() {
        ...
        b1[0] = a1[a1.length - 1] = 1;
        double[] twoTmp = new double[a1.length + 1];
        double[] right = new double[a1.length + 1];
        ...
        right[0] = (fvalues[1] - fvalues[0]) * Math.pow(distance[0] * distance[0], -1) - deviation[0] * Math.pow(distance[0], -1);
        right[right.length - 1] = (fvalues[dim - 2] - fvalues[dim - 1]) * Math.pow(distance[dim - 2] * distance[dim - 2], -1) + deviation[1] * Math.pow(distance[dim - 2], -1);
