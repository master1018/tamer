...
public class ApplicationListenerInvocationHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        if (method.getName().equals("handleAbout") && 1 == method.getParameterTypes().length && method.getParameterTypes()[0].equals(_com_apple_eawt_ApplicationEvent)) {
            handleAbout(args[0]);
            return null;
        } else if (method.getName().equals("handlePreferences") && 1 == method.getParameterTypes().length && method.getParameterTypes()[0].equals(_com_apple_eawt_ApplicationEvent)) {
            handlePreferences(args[0]);
            return null;
        } else if (method.getName().equals("handleQuit") && 1 == method.getParameterTypes().length && method.getParameterTypes()[0].equals(_com_apple_eawt_ApplicationEvent)) {
            handleQuit(args[0]);
            return null;
        } else {
            Method adapterMethod = _com_apple_eawt_ApplicationAdapter_Instance.getClass().getMethod(method.getName(), method.getParameterTypes());
