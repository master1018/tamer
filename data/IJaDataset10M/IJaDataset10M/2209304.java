...
public class SetDefaultSearchRequestDelegate extends SimpleRequestDelegate {
   ...
   * Logger definition
   */
    private static final transient Log LOGGER = LogFactory.getLog(SetDefaultSearchRequestDelegate.class);
    ...
    public boolean request(IRequest request, IActivationBuffer buffer, double requestTime) {
        IVisualLocationBuffer vlb = ((IVisualModule) buffer.getModule()).getVisualLocationBuffer();
        ChunkTypeRequest ctr = (ChunkTypeRequest) request;
        ChunkTypeRequest pattern = new ChunkTypeRequest(((IVisualModule) vlb.getModule()).getVisualLocationChunkType(), ctr.getConditionalSlots());
