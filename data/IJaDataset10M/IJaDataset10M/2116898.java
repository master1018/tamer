...
public class JettyWebappContainerFeature implements TestCaseFeature {
    ...
    protected void startJetty() throws Exception {
        ...
        System.setProperty("org.mortbay.log.class", CommonsLoggingJettyLogger.class.getName());
        svr = new Server(serverPort);
        System.setProperty("webapptest.webapproot", webappsRoot);
        System.setProperty("webapptest.extraclasspath", extraClassPath == null ? "" : extraClassPath);
        XmlConfiguration xml = new XmlConfiguration(JettyWebappContainerFeature.class.getResourceAsStream("jetty/jetty.xml"));
        xml.configure(svr);
    ...
    public String getAbsoluteURL(String relativeURL) {
        return getServerURLPrefix() + (relativeURL.startsWith("/") ? relativeURL.substring(1) : relativeURL);
    }
    ...
    public static void main(String[] args) {
        ...
        TestCaseFeature f = new JettyWebappContainerFeature(wr.getAbsolutePath());
        LineNumberReader in = new LineNumberReader(new InputStreamReader(System.in));
        boolean shutdown = false;
        do {
            f.beforeRun(null, null);
            System.out.println("Jetty is started serving webapps in " + wr.getAbsolutePath());
            while (true) {
                try {
