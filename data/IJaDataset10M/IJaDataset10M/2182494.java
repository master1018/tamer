...
public class AnalyzerConfig {
    ...
    public void addAnalyzer(Element config) throws DatabaseConfigurationException {
        ...
        if (id == null || id.length() == 0) defaultAnalyzer = analyzer; else analyzers.put(id, analyzer);
    }
    ...
    protected static Analyzer configureAnalyzer(Element config) throws DatabaseConfigurationException {
        ...
        if (className != null && className.length() != 0) {
            try {
                ...
                if (!Analyzer.class.isAssignableFrom(clazz)) throw new DatabaseConfigurationException("Lucene index: analyzer class has to be" + " a subclass of " + Analyzer.class.getName());
                return (Analyzer) clazz.newInstance();
            } catch (ClassNotFoundException e) {
                throw new DatabaseConfigurationException("Lucene index: analyzer class " + className + " not found.");
            } catch (IllegalAccessException e) {
                throw new DatabaseConfigurationException("Exception while instantiating analyzer class " + className + ": " + e.getMessage(), e);
