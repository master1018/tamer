...
public final class Map extends BaseObject implements ThingOwner, Serializable {
    ...
    public void fractalize(int x1, int y1, int x2, int y2, int gran) {
        ...
        for (int y = y1; y <= y2; y += gran) for (int x = x1; x <= x2; x += gran) {
            if (Rand.r(2) == 0) setTile(x + g, y, getTileFull(x, y)); else setTile(x + g, y, getTileFull(x + gran, y));
            if (Rand.r(2) == 0) setTile(x, y + g, getTileFull(x, y)); else setTile(x, y + g, getTileFull(x, y + gran));
        }
        for (int y = y1; y <= y2; y += gran) for (int x = x1; x <= x2; x += gran) {
            int c;
            switch(Rand.d(4)) {
