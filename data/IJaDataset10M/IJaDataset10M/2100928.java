...
public class PageRenderer implements org.zkoss.zk.ui.sys.PageRenderer {
    ...
    public void render(Page page, Writer out) throws IOException {
        ...
        boolean pageOnly = au;
        if (!pageOnly) pageOnly = (exec.isIncluded() || "page".equals(ctl)) && !"desktop".equals(ctl);
        if (pageOnly) renderPage(exec, page, out, au); else {
    ...
    protected void renderPage(Execution exec, Page page, Writer out, boolean au) throws IOException {
        ...
        if (!au && ((PageCtrl) page).getOwner() == null) writeln(out, HtmlPageRenders.outUnavailable(exec));
    }
    ...
    protected void renderComplete(Execution exec, Page page, Writer out) throws IOException {
        ...
        for (Iterator it = page.getRoots().iterator(); it.hasNext(); ) ((ComponentCtrl) it.next()).redraw(out);
    ...
    public static void afterRenderHtml(Execution exec, Page page, Writer out, Object param) throws IOException {
        ...
        if (rcs.length() > 0) {
            if (out instanceof StringWriter) {
                ...
                if (j < 0) j = buf.lastIndexOf("</html>");
                if (j >= 0) buf.insert(j, rcs); else buf.append(rcs);
            } else {
                out.write(rcs);
            }
        }
    ...
    public static void afterRenderTag(Execution exec, Object param) {
        if (param != null) exec.setAttribute(ATTR_RENDER_CONTEXT, null);
    }
    ...
    public static String childrenToContent(Component comp) {
        ...
        if ((c = comp.getFirstChild()) != null) {
            ...
            for (; c != null; c = c.getNextSibling()) childrenToContent(comp, c, sb);
    ...
    private static final void childrenToContent(Component owner, Component comp, StringBuffer sb) {
        ...
        if (!(comp instanceof AbstractTag)) throw new UiException("Non-ZHTML components are not allowed, " + comp + ", for " + owner);
        final AbstractTag tag = (AbstractTag) comp;
        sb.append(tag.getPrologHalf(true));
        for (Component c = tag.getFirstChild(); c != null; c = c.getNextSibling()) childrenToContent(owner, c, sb);
