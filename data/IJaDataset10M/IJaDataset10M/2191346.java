...
public class GenericNumericDefinition implements NumericDefinition, Convert {
    ...
    public int getBinarySize(String usage, int numDigits, boolean positive, boolean sync) {
        ...
        switch(getTypeCode(usage, positive)) {
            ...
            case Type.ftPostiveBinaryInt:
                if (sync) {
                    storageLength = getBinSizes(numDigits, positive, syncSizesUsed, syncDigitsAvailable, positiveSyncDigitsAvailable);
                } else {
                    storageLength = getBinSizes(numDigits, positive, compSizesUsed, digitsAvailable, positiveDigitsAvailable);
                }
        }
        System.out.println(" >> SL " + usage + " " + numDigits + " " + positive + " " + getTypeCode(usage, positive) + " >> " + storageLength);
    ...
    protected final int getBinSizes(int numDigits, boolean positive, int[] compSizes, int[] digits, int[] positiveDigits) {
        ...
        for (int i = 0; i < digits.length - 1; i++) {
            if (digits[i] >= numDigits || (positive && positiveDigits[i] >= numDigits)) {
    ...
    public int getTypeIdentifier(String usage, String picture, boolean signed) {
        ...
        } else if (picture.indexOf('Z') >= 0 || picture.indexOf('-') >= 0 || picture.indexOf('+') >= 0 || picture.indexOf('.') >= 0) {
