...
public class SPObjectSnapshotHierarchyListener extends AbstractSPListener {
     ...
     */
    private final Map<SPObjectSnapshot<?>, SPObjectSnapshotUpdateListener> listenerMap = new HashMap<SPObjectSnapshot<?>, SPObjectSnapshotUpdateListener>();
    ...
    public void childAdded(SPChildEvent e) {
        if (e.getChild() instanceof SQLTable) {
            ...
            if (session.getWorkspace().getSnapshotCollection().isMagicEnabled() && upstreamType != null) {
                List<UserDefinedSQLTypeSnapshot> udtSnapshots = session.getWorkspace().getSnapshotCollection().getChildren(UserDefinedSQLTypeSnapshot.class);
                ...
                if (!isSnapshot && (upstreamType.getParent() == null || (upstreamType.getParent() instanceof SnapshotCollection && !upstreamType.getParent().equals(session.getWorkspace().getSnapshotCollection())) || (upstreamType.getParent() instanceof DomainCategory && upstreamType.getParent().getParent() instanceof SnapshotCollection && !upstreamType.getParent().getParent().equals(session.getWorkspace().getSnapshotCollection())))) {
                    reassignType(sqlColumn);
                    isSnapshot = true;
                }
                if (!isSnapshot) {
                    UserDefinedSQLType columnProxyType = sqlColumn.getUserDefinedSQLType();
                    createSPObjectSnapshot(columnProxyType, upstreamType, session.getWorkspace().getSnapshotCollection(), this);
    ...
    public void childRemoved(SPChildEvent e) {
        if (e.getChild() instanceof SQLTable) {
            ...
            for (SQLColumn col : e.getChild().getChildren(SQLColumn.class)) {
                ...
                if (session.getWorkspace().getSnapshotCollection().isMagicEnabled() && col.getUserDefinedSQLType().isMagicEnabled()) {
            ...
            if (session.getWorkspace().getSnapshotCollection().isMagicEnabled() && colType.isMagicEnabled()) {
    ...
    private void cleanupSnapshot(UserDefinedSQLType typeRemoved) {
        ...
        if (typeRemoved.getParent() == session.getSystemWorkspace() || (typeRemoved.getParent() instanceof DomainCategory && typeRemoved.getParent().getParent() == session.getSystemWorkspace())) return;
        ...
        if (udtSnapshot == null) throw new NullPointerException("The type " + typeRemoved + " is a snapshot type but does not have a snapshot object in the project.");
        ...
        try {
            if (udtSnapshot.isDomainSnapshot()) {
                ...
                if (udtSnapshot.getSPObject().getUpstreamType().getUpstreamType() != null) throw new IllegalStateException("We currently do not support having a domain " + "reference an upstream type of a domain.");
    ...
    private void addUpdateListener(UserDefinedSQLType upstreamSnapshotType) {
        if (upstreamSnapshotType == null) return;
        SnapshotCollection collection = session.getWorkspace().getSnapshotCollection();
        SPObject upstreamTypeParent = upstreamSnapshotType.getParent();
        if (upstreamTypeParent == null || upstreamTypeParent.equals(collection) || (upstreamTypeParent instanceof DomainCategory && upstreamTypeParent.getParent().equals(collection))) {
    ...
    public static void createSPObjectSnapshot(UserDefinedSQLType typeProxy, UserDefinedSQLType upstreamType, SnapshotCollection collection, SPObjectSnapshotHierarchyListener updateListener) {
        ...
        if (upstreamTypeParent != null && !upstreamTypeParent.equals(collection) && !(upstreamTypeParent instanceof DomainCategory && upstreamTypeParent.getParent().equals(collection))) {
            ...
            if (upstreamType.getUpstreamType() != null) {
                ...
                if (isUpstreamDomainSnapshot) throw new IllegalStateException("We currently don't handle domains that have an upstream type of another domain.");
                UserDefinedSQLTypeSnapshot upstreamSnapshot = null;
                boolean existingSnapshotFound = false;
                for (SPObjectSnapshot<?> workspaceSnapshot : collection.getSPObjectSnapshots()) {
                    if (workspaceSnapshot.getOriginalUUID().equals(upUpStreamType.getUUID()) && workspaceSnapshot instanceof UserDefinedSQLTypeSnapshot) {
        ...
        } else if (upstreamTypeParent != null && (upstreamTypeParent.equals(collection) || (upstreamTypeParent instanceof DomainCategory && upstreamTypeParent.getParent().equals(collection)))) {
            for (SPObjectSnapshot<?> snapshot : collection.getSPObjectSnapshots()) {
    ...
    private static boolean setUpstreamTypeToExistingSnapshot(UserDefinedSQLType typeProxy, UserDefinedSQLType upstreamType, SnapshotCollection collection) {
        ...
        List<UserDefinedSQLTypeSnapshot> typeSnapshots = collection.getChildren(UserDefinedSQLTypeSnapshot.class);
        for (UserDefinedSQLTypeSnapshot typeSnapshot : typeSnapshots) {
            if (typeSnapshot.isDomainSnapshot() && !(upstreamType.getParent() instanceof DomainCategory)) continue;
        ...
        if (transactionCount == 0) {
            try {
                ...
                for (Entry<UserDefinedSQLType, PropertyChangeEvent> entry : upstreamTypeChangeEventMap.entrySet()) {
                    ...
                    createSPObjectSnapshot(source, newValue, session.getWorkspace().getSnapshotCollection(), this);
                    if (oldValue != null && session.getWorkspace().getSnapshotCollection().isMagicEnabled() && source.isMagicEnabled()) {
    ...
    private void reassignType(SQLColumn column) {
        ...
        if (upstreamTypeParent == null) {
            ...
            List<UserDefinedSQLTypeSnapshot> snapshots = upstreamTypeParent.getChildren(UserDefinedSQLTypeSnapshot.class);
            ...
            List<SPObjectSnapshot> snapshots = upstreamTypeParent.getParent().getChildren(SPObjectSnapshot.class);
        ...
        for (UserDefinedSQLTypeSnapshot snapshot : session.getWorkspace().getSnapshotCollection().getChildren(UserDefinedSQLTypeSnapshot.class)) {
