    ...
    private void calculateMatrix() {
        for (int i = 0; i < seq1.length() + 1; i++) {
            ...
            for (int j = 0; j < seq2.length() + 1; j++) {
                if (i == 0) matrix[i][j] = j; else if (j == 0) matrix[i][j] = i; else matrix[i][j] = min(matrix[i][j - 1] + 1, matrix[i - 1][j] + 1, matrix[i - 1][j - 1] + (seq1.charAt(i - 1) == seq2.charAt(j - 1) ? 0 : 1));
            }
        }
    }
    ...
    public List<EditOperation> calculatePath() {
        ...
        while (i > 0 && j > 0 && dist > 0) {
            ...
            if (dist == ins + 1) {
                ...
                if (dist == sub) addOrUpdate(ops, SKIP, 1); else addOrUpdate(ops, SUBSTITUTE, 1);
                i--;
                j--;
            }
            dist = matrix[i][j];
        }
        if (i == 0) addOrUpdate(ops, INSERT, j); else if (j == 0) addOrUpdate(ops, DELETE, i); else addOrUpdate(ops, SKIP, i);
        return ops;
    }

    private void addOrUpdate(LinkedList<EditOperation> ops, EditOperation.Kind kind, int length) {
        if (length == 0) return;
        if (!ops.isEmpty() && ops.getFirst().getKind() == kind) ops.getFirst().incLength(length); else ops.addFirst(new EditOperation(kind, length));
    }
