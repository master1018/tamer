...
public class TLD extends RandomizableClassifier implements OptionHandler, MultiInstanceCapabilitiesHandler, TechnicalInformationHandler {
   ...
   */
    public void buildClassifier(Instances exs) throws Exception {
        ...
        for (int u = 0; u < numegs; u++) {
            ...
            if (example.classValue() == 1) pos.add(example); else neg.add(example);
        }
        ...
        double[] pSumVal = new double[m_Dimension], nSumVal = new double[m_Dimension];
        double[] maxVarsP = new double[m_Dimension], maxVarsN = new double[m_Dimension];
        ...
        for (int v = 0; v < nnum; v++) {
            ...
            for (int w = 0, t = 0; w < m_Dimension; w++, t++) {
                if (!Double.isNaN(m_MeanN[v][w])) {
                    for (int u = 0; u < nxi.numInstances(); u++) if (!nxi.instance(u).isMissing(t)) m_SumN[v][w] += nxi.instance(u).weight();
                    numExsN[w]++;
        ...
        for (int w = 0; w < m_Dimension; w++) {
            pSumVal[w] /= numExsP[w];
            nSumVal[w] /= numExsN[w];
            if (numExsP[w] > 1) meanVarP[w] = meanVarP[w] / (numExsP[w] - 1.0) - pSumVal[w] * numExsP[w] / (numExsP[w] - 1.0);
            if (numExsN[w] > 1) meanVarN[w] = meanVarN[w] / (numExsN[w] - 1.0) - nSumVal[w] * numExsN[w] / (numExsN[w] - 1.0);
        ...
        for (int x = 0; x < m_Dimension; x++) {
            ...
            if (varMeanP[x] <= ZERO) varMeanP[x] = ZERO;
            b = a / varMeanP[x] + 2.0;
            ...
            if (varMeanN[x] <= ZERO) varMeanN[x] = ZERO;
            b = a / varMeanN[x] + 2.0;
        ...
        for (int x = 0, y = 0; x < m_Dimension; x++, y++) {
            ...
            w = m_ParamsN[4 * x + 2];
            m = m_ParamsN[4 * x + 3];
            if (getDebug()) System.err.println("???Negative: (" + exs.attribute(1).relation().attribute(y) + "): a=" + a + ", b=" + b + ", w=" + w + ", m=" + m);
        }
        if (m_UseEmpiricalCutOff) {
            double[] pLogOdds = new double[pnum], nLogOdds = new double[nnum];
            for (int p = 0; p < pnum; p++) pLogOdds[p] = likelihoodRatio(m_SumP[p], m_MeanP[p], m_VarianceP[p]);
            for (int q = 0; q < nnum; q++) nLogOdds[q] = likelihoodRatio(m_SumN[q], m_MeanN[q], m_VarianceN[q]);
   ...
   */
    public double classifyInstance(Instance ex) throws Exception {
        ...
        for (int w = 0, t = 0; w < m_Dimension; w++, t++) {
            for (int u = 0; u < exi.numInstances(); u++) if (!exi.instance(u).isMissing(t)) n[w] += exi.instance(u).weight();
            sSq[w] = sSq[w] * (n[w] - 1.0);
            if (sSq[w] <= 0.0) sSq[w] = 0.0;
        }
    ...
    private double likelihoodRatio(double[] n, double[] xBar, double[] sSq) {
        ...
        for (int x = 0; x < m_Dimension; x++) {
            ...
            for (int y = 1; y <= halfN; y++) LLP += Math.log(b / 2.0 + n[x] / 2.0 - (double) y);
            if (n[x] / 2.0 > halfN) LLP += TLD_Optm.diffLnGamma(b / 2.0);
            a = m_ParamsN[4 * x];
            b = m_ParamsN[4 * x + 1];
            w = m_ParamsN[4 * x + 2];
            m = m_ParamsN[4 * x + 3];
            ...
            for (int y = 1; y <= halfN; y++) LLN += Math.log(b / 2.0 + n[x] / 2.0 - (double) y);
            if (n[x] / 2.0 > halfN) LLN += TLD_Optm.diffLnGamma(b / 2.0);
        }
        return LLP - LLN;
    }
    ...
    private void findCutOff(double[] pos, double[] neg) {
        ...
        while ((p < pNum) && (n < nNum)) {
            ...
            count++;
            if ((fstAccu + sndAccu > maxAccu) || ((fstAccu + sndAccu == maxAccu) && (Math.abs(split) < minDistTo0))) {
   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (runString.length() != 0) setNumRuns(Integer.parseInt(runString)); else setNumRuns(1);
   ...
   */
    public String[] getOptions() {
        ...
        if (getUsingCutOff()) result.add("-C");
        result.add("-R");
...
class TLD_Optm extends Optimization {
   ...
   */
    protected double objectiveFunction(double[] x) {
        ...
        for (int j = 0; j < numExs; j++) {
            if (Double.isNaN(xBar[j])) continue;
            NLL += 0.5 * (b + num[j]) * Math.log((1.0 + num[j] * w) * (a + sSq[j]) + num[j] * (xBar[j] - m) * (xBar[j] - m));
            if (Double.isNaN(NLL) && m_Debug) {
                System.err.println("???????????1: " + a + " " + b + " " + w + " " + m + "|x-: " + xBar[j] + "|n: " + num[j] + "|S^2: " + sSq[j]);
                System.exit(1);
            }
            NLL -= 0.5 * (b + num[j] - 1.0) * Math.log(1.0 + num[j] * w);
            if (Double.isNaN(NLL) && m_Debug) {
                System.err.println("???????????2: " + a + " " + b + " " + w + " " + m + "|x-: " + xBar[j] + "|n: " + num[j] + "|S^2: " + sSq[j]);
                System.exit(1);
            }
            int halfNum = ((int) num[j]) / 2;
            for (int z = 1; z <= halfNum; z++) NLL -= Math.log(0.5 * b + 0.5 * num[j] - (double) z);
            if (0.5 * num[j] > halfNum) NLL -= diffLnGamma(0.5 * b);
            if (Double.isNaN(NLL) && m_Debug) {
                System.err.println("???????????3: " + a + " " + b + " " + w + " " + m + "|x-: " + xBar[j] + "|n: " + num[j] + "|S^2: " + sSq[j]);
                System.exit(1);
            }
        ...
        if (m_Debug) System.err.println("?????????????5: " + NLL);
        if (Double.isNaN(NLL)) System.exit(1);
        return NLL;
    }
   ...
   */
    protected double[] evaluateGradient(double[] x) {
        ...
        for (int j = 0; j < numExs; j++) {
            if (Double.isNaN(xBar[j])) continue;
            double denorm = (1.0 + num[j] * w) * (a + sSq[j]) + num[j] * (xBar[j] - m) * (xBar[j] - m);
            da += 0.5 * (b + num[j]) * (1.0 + num[j] * w) / denorm - 0.5 * b / a;
            db += 0.5 * Math.log(denorm) - 0.5 * Math.log(1.0 + num[j] * w) - 0.5 * Math.log(a);
            int halfNum = ((int) num[j]) / 2;
            for (int z = 1; z <= halfNum; z++) db -= 1.0 / (b + num[j] - 2.0 * (double) z);
            if (num[j] / 2.0 > halfNum) db -= 0.5 * diffFstDervLnGamma(0.5 * b);
            dw += 0.5 * (b + num[j]) * (a + sSq[j]) * num[j] / denorm - 0.5 * (b + num[j] - 1.0) * num[j] / (1.0 + num[j] * w);
        ...
        switch(index) {
            case 0:
                ...
                for (int j = 0; j < numExs; j++) {
                    if (Double.isNaN(xBar[j])) continue;
                    double denorm = (1.0 + num[j] * w) * (a + sSq[j]) + num[j] * (xBar[j] - m) * (xBar[j] - m);
                    h[0] += 0.5 * b / (a * a) - 0.5 * (b + num[j]) * (1.0 + num[j] * w) * (1.0 + num[j] * w) / (denorm * denorm);
                    h[1] += 0.5 * (1.0 + num[j] * w) / denorm - 0.5 / a;
                    h[2] += 0.5 * num[j] * num[j] * (b + num[j]) * (xBar[j] - m) * (xBar[j] - m) / (denorm * denorm);
                    h[3] -= num[j] * (b + num[j]) * (m - xBar[j]) * (1.0 + num[j] * w) / (denorm * denorm);
                }
                break;
            case 1:
                a = x[0];
                ...
                for (int j = 0; j < numExs; j++) {
                    ...
                    int halfNum = ((int) num[j]) / 2;
                    for (int z = 1; z <= halfNum; z++) h[1] += 1.0 / ((b + num[j] - 2.0 * (double) z) * (b + num[j] - 2.0 * (double) z));
                    if (num[j] / 2.0 > halfNum) h[1] -= 0.25 * diffSndDervLnGamma(0.5 * b);
                    h[2] += 0.5 * (a + sSq[j]) * num[j] / denorm - 0.5 * num[j] / (1.0 + num[j] * w);
            ...
            case 2:
                ...
                for (int j = 0; j < numExs; j++) {
                    if (Double.isNaN(xBar[j])) continue;
                    double denorm = (1.0 + num[j] * w) * (a + sSq[j]) + num[j] * (xBar[j] - m) * (xBar[j] - m);
                    h[0] += 0.5 * num[j] * num[j] * (b + num[j]) * (xBar[j] - m) * (xBar[j] - m) / (denorm * denorm);
                    h[1] += 0.5 * (a + sSq[j]) * num[j] / denorm - 0.5 * num[j] / (1.0 + num[j] * w);
                    h[2] += 0.5 * (b + num[j] - 1.0) * num[j] * num[j] / ((1.0 + num[j] * w) * (1.0 + num[j] * w)) - 0.5 * (b + num[j]) * (a + sSq[j]) * (a + sSq[j]) * num[j] * num[j] / (denorm * denorm);
                    h[3] -= num[j] * num[j] * (b + num[j]) * (m - xBar[j]) * (a + sSq[j]) / (denorm * denorm);
                }
                break;
            case 3:
                a = x[0];
                ...
                for (int j = 0; j < numExs; j++) {
                    if (Double.isNaN(xBar[j])) continue;
                    double denorm = (1.0 + num[j] * w) * (a + sSq[j]) + num[j] * (xBar[j] - m) * (xBar[j] - m);
                    h[0] -= num[j] * (b + num[j]) * (m - xBar[j]) * (1.0 + num[j] * w) / (denorm * denorm);
                    h[1] += num[j] * (m - xBar[j]) / denorm;
                    h[2] -= num[j] * num[j] * (b + num[j]) * (m - xBar[j]) * (a + sSq[j]) / (denorm * denorm);
                    h[3] += num[j] * (b + num[j]) * ((1.0 + num[j] * w) * (a + sSq[j]) - num[j] * (m - xBar[j]) * (m - xBar[j])) / (denorm * denorm);
                }
        }
        return h;
    }
