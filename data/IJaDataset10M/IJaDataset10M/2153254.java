 ...
 * @author Perry Cheng
 */
public final class VM_OffsetArray implements VM_Uninterruptible {

    private VM_Offset[] data;

    public static VM_OffsetArray create(int size) throws VM_PragmaInterruptible {
        if (VM.runningVM) VM._assert(false);
    ...
    public VM_Offset get(int index) throws VM_PragmaInline {
        if (VM.runningVM || VM.writingImage) VM._assert(false);
        return data[index];
    }
    ...
    public Object getBacking() throws VM_PragmaInline {
        if (!VM.writingImage) VM.sysFail("VM_OffsetArray.getBacking called when not writing boot image");
        return data;
    }
}
