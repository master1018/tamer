...
public class WikiWordWidget extends TextWidget implements PageReferencer {
    ...
    public static final String NONEXISTENT_PAGE_TOKEN = "";
    ...
    public String render() throws Exception {
        ...
        if (parentPage.getPageCrawler().pageExists(parentPage, PathParser.parse(getWikiWord()))) return makeLinkToExistingWikiPage(qualifiedName); else return makeLinkToNonExistentWikiPage(qualifiedName);
    }
    ...
    private String makeLinkToNonExistentWikiPage(String qualifiedName) {
        ...
        html.append("?edit");
        html.append("\">" + NONEXISTENT_PAGE_TOKEN + "</a>");
    ...
    public void renameMovedPageIfReferenced(WikiPage pageToBeMoved, String newParentName) throws Exception {
        ...
        if (refersTo(reference, QualifiedNameOfPageToBeMoved)) {
            ...
            if (referenceTail.length() > 0) childPortionOfReference += referenceTail;
            String newQualifiedName;
            if ("".equals(newParentName)) newQualifiedName = "." + childPortionOfReference; else newQualifiedName = "." + newParentName + "." + childPortionOfReference;
    ...
    public String makeRenamedRelativeReference(String newQualifiedReference) throws Exception {
        ...
        try {
            if (rawReference.startsWith(".")) renamedRawReference = newQualifiedReference; else {
                ...
                if (newQualifiedReference.startsWith(parentPrefix)) {
                    if (rawReference.startsWith("^")) renamedRawReference = "^" + referenceRemainder.substring(referenceRemainder.indexOf(".") + 1); else renamedRawReference = referenceRemainder;
                }
            }
    ...
    static boolean refersTo(String qualifiedReference, String qualifiedTarget) {
        if (qualifiedReference.equals(qualifiedTarget)) return true;
        if (qualifiedReference.startsWith(qualifiedTarget + ".")) return true;
        return false;
    }
    ...
    private String getQualifiedWikiWord() throws Exception {
        ...
        if (expandedPath == null) return getText();
    ...
    private String rename(String oldQualifiedName, String newPageName) {
        ...
        if (lastDotIndex < 1) newQualifiedName = newPageName; else newQualifiedName = oldQualifiedName.substring(0, lastDotIndex + 1) + newPageName;
