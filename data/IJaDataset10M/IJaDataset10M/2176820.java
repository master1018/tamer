...
final class MIDletSelector implements CommandListener {
     ...
     */
    private Command backCmd = new Command(Resource.getString(ResourceConstants.BACK), Command.BACK, 2);

    /**
     * the Command object for "Launch".
     */
    private Command launchCmd = new Command(Resource.getString(ResourceConstants.LAUNCH), Command.ITEM, 1);
     ...
     */
    MIDletSelector(RunningMIDletSuiteInfo theSuiteInfo, Display theDisplay, Displayable theParentDisplayable, ApplicationManager theManager) throws Throwable {
    ...
    private void setupList(MIDletSuiteStorage mss) {
        if (mlist == null) {
            mlist = new List(Resource.getString(ResourceConstants.AMS_SELECTOR_SEL_TO_LAUNCH), Choice.IMPLICIT);
            for (int i = 0; i < mcount; i++) {
                Image icon = null;
                if (minfo[i].icon != null) {
                    icon = RunningMIDletSuiteInfo.getIcon(suiteInfo.suiteId, minfo[i].icon, mss);
                }
