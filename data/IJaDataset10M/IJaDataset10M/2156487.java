...
public class WWBufferUtil {
    ...
    public static DoubleBuffer newDoubleBuffer(int size, boolean allocateDirect) {
        ...
        return allocateDirect ? newDirectByteBuffer(SIZEOF_DOUBLE * size).asDoubleBuffer() : DoubleBuffer.allocate(size);
    }
    ...
    public static int sizeOfPrimitiveType(Object dataType) {
        ...
        if (AVKey.INT8.equals(dataType)) return 1; else if (AVKey.INT16.equals(dataType)) return SIZEOF_SHORT; else if (AVKey.INT32.equals(dataType)) return SIZEOF_INT; else if (AVKey.FLOAT32.equals(dataType)) return SIZEOF_FLOAT; else if (AVKey.FLOAT64.equals(dataType)) return SIZEOF_DOUBLE;
        return -1;
    }
    ...
    public static double[] computeExtremeValues(BufferWrapper buffer, double missingDataSignal) {
        ...
        if (Double.compare(min, Double.MAX_VALUE) == 0 || Double.compare(max, -Double.MAX_VALUE) == 0) return null;
