...
public class FieldContext {
    ...
    private boolean arrayPrimitive() {
        return field.getType().isArray() && PrimitiveWrapper.isPrimitive(field.getType().getComponentType());
    }
    ...
    public Object value() {
        ...
        if (result instanceof Date) return ((Date) result).getTime(); else if (isPlural()) return ((Collection<?>) result).toArray(); else return result;
    }
    ...
    public void setProperty(Object v) {
        try {
            field.setAccessible(true);
            if (field.getType() == Date.class) v = new Date((Long) v); else if (isPluralPrimitive()) v = Arrays.asList((Object[]) v);
            field.set(subject, v);
        } catch (Exception e) {
            Utils.runtime(e);
        }
    ...
    public void applyTo(Node n) {
        if (value() != null) n.setProperty(name(), value());
    }
    ...
    public void applyFrom(Node n) {
        if (n.hasProperty(name())) setProperty(n.getProperty(name()));
    }
