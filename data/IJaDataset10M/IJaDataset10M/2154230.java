...
public class BaseCalendarModel extends IntegrationEntityModel implements IBaseCalendarModel {
    ...
    public void initCreate() {
        editing = false;
        boolean codeGenerated = (configurationDAO.getConfiguration() != null) ? configurationDAO.getConfiguration().getGenerateCodeForBaseCalendars() : false;
    ...
    private static void loadingExceptionsWithTheirTypes(BaseCalendar baseCalendar) {
    ...
    public EffortDuration getWorkableTime() {
        ...
        return getBaseCalendar().getCapacityOn(PartialDay.wholeDay(selectedDate));
    }
    ...
    @Override
    public void createException(CalendarExceptionType type, LocalDate startDate, LocalDate endDate, Capacity capacity) {
    ...
    public Capacity getCapacityAt(Days day) {
        ...
        return getBaseCalendar().getCapacityConsideringCalendarDatasOn(selectedDate, day);
    }
    ...
    public void unsetDefault(Days day) {
        if (getBaseCalendar() != null) {
            Capacity previousCapacity = getBaseCalendar().getCapacityConsideringCalendarDatasOn(selectedDate, day);
            getBaseCalendar().setCapacityAt(day, previousCapacity, selectedDate);
        }
    }
    ...
    public CalendarData getCurrentVersion() {
        return getBaseCalendar().getCalendarData(LocalDate.fromDateFields(new Date()));
    }
    ...
    public LocalDate getDateValidFrom() {
        if (getBaseCalendar() != null) {
            LocalDate validFromDate = getBaseCalendar().getValidFrom(selectedDate);
    ...
    public void checkChangeExpiringDate(CalendarData version, Date date) {
        ...
        if ((index < getBaseCalendar().getCalendarDataVersions().size() - 1)) {
            LocalDate nextExpiringDate = getBaseCalendar().getCalendarDataVersions().get(index + 1).getExpiringDate();
            if ((nextExpiringDate == null) || (newExpiringDate.compareTo(nextExpiringDate) < 0)) {
                return;
            }
        }
        throw new ValidationException(_("This date can not include the whole next work week"));
    }
    ...
    @Override
    public void checkInvalidValuesCalendar(BaseCalendar entity) throws ValidationException {
        if (baseCalendarDAO.thereIsOtherWithSameName(entity)) {
            InvalidValue[] invalidValues2 = { new InvalidValue(_("{0} already exists", entity.getName()), BaseCalendar.class, "name", entity.getName(), entity) };
            throw new ValidationException(invalidValues2, _("Could not save new calendar"));
        }
    }
    ...
    public void generateCalendarCodes() {
        if (getBaseCalendar().isCodeAutogenerated()) {
            baseCalendar.generateCalendarExceptionCodes(getNumberOfDigitsCode());
        }
    }
    ...
    public boolean isDefaultCalendar(BaseCalendar baseCalendar) {
        ...
        return baseCalendar.getId().equals(defaultCalendar.getId());
    }
    ...
    @Override
    public void updateException(CalendarExceptionType type, LocalDate startDate, LocalDate endDate, Capacity capacity) {
        for (LocalDate date = new LocalDate(startDate); date.compareTo(new LocalDate(endDate)) <= 0; date = date.plusDays(1)) {
            if (baseCalendar.getOwnExceptionDay(date) != null) {
                if (type == null) {
                    getBaseCalendar().removeExceptionDay(date);
                } else {
    ...
    @Override
    public void setStartDate(CalendarAvailability calendarAvailability, LocalDate startDate) throws IllegalArgumentException {
        if (getBaseCalendar() != null) {
    ...
    @Override
    public void setEndDate(CalendarAvailability calendarAvailability, LocalDate endDate) throws IllegalArgumentException {
        if (getBaseCalendar() != null) {
    ...
    @Override
    public boolean isLastActivationPeriod(CalendarAvailability calendarAvailability) {
        if (getBaseCalendar() != null) {
            return getBaseCalendar().isLastCalendarAvailability(calendarAvailability);
        }
        return false;
    }
