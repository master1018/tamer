...
public final class Processor {
    ...
    private int readMem(int vaddr, int size) throws MipsException {
        if (Lib.test(dbgProcessor)) System.out.println("\treadMem vaddr=0x" + Lib.toHexString(vaddr) + ", size=" + size);
        Lib.assertTrue(size == 1 || size == 2 || size == 4);
        int value = Lib.bytesToInt(mainMemory, translate(vaddr, size, false), size);
        if (Lib.test(dbgProcessor)) System.out.println("\t\tvalue read=0x" + Lib.toHexString(value, size * 2));
        return value;
    }
    ...
    private void writeMem(int vaddr, int size, int value) throws MipsException {
        if (Lib.test(dbgProcessor)) System.out.println("\twriteMem vaddr=0x" + Lib.toHexString(vaddr) + ", size=" + size + ", value=0x" + Lib.toHexString(value, size * 2));
        Lib.assertTrue(size == 1 || size == 2 || size == 4);
    ...
    public static final String exceptionNames[] = { "syscall      ", "page fault   ", "TLB miss     ", "read-only    ", "bus error    ", "address error", "overflow     ", "illegal inst " };
    ...
    private class Instruction {
        ...
        private void fetch() throws MipsException {
            if ((Lib.test(dbgDisassemble) && !Lib.test(dbgProcessor)) || Lib.test(dbgFullDisassemble)) System.out.print("PC=0x" + Lib.toHexString(registers[regPC]) + "\t");
        ...
        private void decode() {
            ...
            mask = 0xFFFFFFFF;
            branch = true;
            if (test(Mips.SIZEB)) size = 1; else if (test(Mips.SIZEH)) size = 2; else if (test(Mips.SIZEW)) size = 4; else size = 0;
            nextPC = registers[regNextPC] + 4;
            if (test(Mips.DSTRA)) dstReg = regRA; else if (format == Mips.IFMT) dstReg = rt; else if (format == Mips.RFMT) dstReg = rd; else dstReg = -1;
            if (format == Mips.RFMT) jtarget = registers[rs]; else if (format == Mips.IFMT) jtarget = registers[regNextPC] + (imm << 2); else if (format == Mips.JFMT) jtarget = (registers[regNextPC] & 0xF0000000) | (target << 2); else jtarget = -1;
            if (test(Mips.UNSIGNED)) {
                imm &= 0xFFFF;
            }
            addr = registers[rs] + imm;
            if (test(Mips.SRC1SH)) src1 = sh; else src1 = registers[rs];
            if (test(Mips.SRC2IMM)) src2 = imm; else src2 = registers[rt];
            if (test(Mips.UNSIGNED)) {
                src1 &= 0xFFFFFFFFL;
                src2 &= 0xFFFFFFFFL;
            }
        ...
        private void print() {
            ...
            if (operation == Mips.INVALID) {
                System.out.print("invalid: op=" + Lib.toHexString(op, 2) + " rs=" + Lib.toHexString(rs, 2) + " rt=" + Lib.toHexString(rt, 2) + " rd=" + Lib.toHexString(rd, 2) + " sh=" + Lib.toHexString(sh, 2) + " func=" + Lib.toHexString(func, 2) + "\n");
                return;
            }
        ...
        private void execute() throws MipsException {
            ...
            switch(operation) {
                ...
                case Mips.LOAD:
                    ...
                    if (!test(Mips.UNSIGNED)) dst = Lib.extend(value, 0, size * 8); else dst = value;
                    break;
                case Mips.LWL:
        ...
        private void writeBack() throws MipsException {
            if (test(Mips.OVERFLOW) && Lib.test(dst, 31) != Lib.test(dst, 32)) throw new MipsException(exceptionOverflow);
