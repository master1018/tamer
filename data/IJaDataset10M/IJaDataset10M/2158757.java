...
public class PtCountSimComparisonKMLWriter extends PtCountSimComparisonWriter {
	 ...
	 */
    private Map<String, String> boardCountsLoadCurveGraphMap, alightCountsLoadCurveGraphMap, occupancyCountsLoadCurveGraphMap;
    ...
    private void createStyles() {
        ...
        for (StyleType styleType : new StyleType[] { this.redCrossStyle, this.redMinusStyle, this.yellowCrossStyle, this.yellowMinusStyle, this.greenCrossStyle, this.greenMinusStyle, this.greyCrossStyle, this.greyMinusStyle }) {
            ...
            icon.setColor(new byte[] { colors.get(styleType)[0], colors.get(styleType)[1], colors.get(styleType)[2], colors.get(styleType)[3] });
	 ...
	 */
    private PlacemarkType createPlacemark(final String stopid, final CountSimComparison csc, final double relativeError, final int timestep, PtCountsType type) {
	 ...
	 * @param counts
	 */
    private void writeStopData(final List<CountSimComparison> countSimComparisonList, final FolderType folder, PtCountsType type) {
        Id stopid;
	 ...
	 */
    private String createPlacemarkDescription(final String stopid, final CountSimComparison csc, final double relativeError, final int timestep, PtCountsType type) {
    ...
    private void addCountsSimRealPerHourGraphs(final FolderType folder, final int timestep, final TimeSpanType timespan) {
        try {
            String filenameBoard = getSimRealGraphName(PtCountsType.Boarding, timestep), filenameAlight = getSimRealGraphName(PtCountsType.Alighting, timestep), filenameOccupancy = getSimRealGraphName(PtCountsType.Occupancy, timestep);
            PtCountsSimRealPerHourGraph graphBoard = new PtCountsSimRealPerHourGraph(this.boardCountComparisonFilter.getCountsForHour(null), this.iter, filenameBoard, PtCountsType.Boarding), graphAlight = new PtCountsSimRealPerHourGraph(this.alightCountComparisonFilter.getCountsForHour(null), this.iter, filenameAlight, PtCountsType.Alighting), graphOccupancy = new PtCountsSimRealPerHourGraph(this.occupancyCountComparisonFilter.getCountsForHour(null), this.iter, filenameOccupancy, PtCountsType.Occupancy);
            ...
            addGraph2Screen(filenameOccupancy, graphOccupancy.getChartTitle(), timespan, folder, 1, 0.5, 0.98, 0.48);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
