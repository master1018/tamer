...
public abstract class ArchiveRecord extends InputStream {
    ...
    public int read(byte[] b, int offset, int length) throws IOException {
        ...
        if (read == -1 || read == 0) {
            ...
            if (read == -1) {
                String msg = "Premature EOF before end-of-record: " + getHeader().getHeaderFields();
                if (isStrict()) {
    ...
    protected String outputCdx(final String strippedFileName) throws IOException {
        close();
        ArchiveRecordHeader h = getHeader();
        StringBuilder buffer = new StringBuilder(ArchiveFileConstants.CDX_LINE_BUFFER_SIZE);
