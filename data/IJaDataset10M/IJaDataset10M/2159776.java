...
public class FunctionParser {
    ...
    public double parse(String formula) {
        ...
        for (Object token : tokens) {
            ...
            if (count < 0) throw new IllegalArgumentException("Schliessende Klammer mit fehlender öffnender Klammer");
            if (token == SEPARATOR && count == 0) throw new IllegalArgumentException("Komma ausserhalb Klammern");
        }
        if (count > 0) throw new IllegalArgumentException("Schliessende Klammer fehlt");
        if (count < 0) throw new IllegalArgumentException("Schliessende Klammer zuviel");
        ...
        while (size > 1) {
            parse(tokens, 0);
            if (tokens.size() >= size) {
                throw new IllegalArgumentException("Die Formel wird nicht vollständig geparst, evtl. wurde eine Operation vergessen?");
            }
            size = tokens.size();
        }
        if (size != 1 || !(tokens.get(0) instanceof Double)) throw new IllegalArgumentException("Unbekannter Fehler in der Formel");
    ...
    private void parse(List<Object> formula, int offset) {
        ...
        while (begin + length < formula.size()) {
            ...
            if (end == OPEN) {
                ...
                if (length == 0) throw new IllegalArgumentException("Fehlende Ausdrücke, z.B. leere Klammer");
                parse(formula, begin, length);
    ...
    private void parse(List<Object> formula, int offset, int length) {
            ...
            if (function != null) {
                if (i == offset || (!(formula.get(i - 1) instanceof Double) && !(formula.get(i - 1) instanceof double[]))) {
                    ...
                    if (arguments instanceof Double) {
                        ...
                        throw new IllegalArgumentException("Fehlende Argumente für " + function + ", gefunden: " + arguments);
                    }
                    if (!function.validNrOfArguments(values.length)) throw new IllegalArgumentException("Falsche Anzahl Argumente für " + function + ", gefunden: " + values.length);
                    formula.remove(i + 1);
        ...
        for (int i = offset; i < offset + length; i++) {
            ...
            if (check instanceof String) {
                ...
                if (operation == null) {
                    throw new IllegalArgumentException("Element " + check + " wird wie Operation benutzt, aber eine Operation mit dem Namen gibt es nicht.");
                }
            ...
            for (int i = offset + 1; i < offset + length - 1; i++) {
                ...
                if (check instanceof IOperation) {
                    ...
                    if (operation.getPriority() == priority) {
                        ...
                        if (!(left instanceof Double)) throw new IllegalArgumentException("Operation nicht von Zahlen umgeben");
                        if (!(right instanceof Double)) throw new IllegalArgumentException("Operation nicht von Zahlen umgeben");
            ...
            if (!Character.isWhitespace(current)) {
                if (current == '(') {
                    ...
                    while (end < length) {
                        ...
                        if (Character.isDigit(next)) end++; else if (next == '.' && !pointSeen) {
                            pointSeen = true;
                            end++;
                        } else break;
                    }
                    ...
                    if (best == null) throw new IllegalArgumentException("An dieser Formel stimmt was nicht");
                    offset += bestLength;
                    parts.add(best);
                }
            } else offset++;
