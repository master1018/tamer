...
public class SelfConnectionLiner implements Liner, DOMStorable {
    ...
    public void lineout(ConnectionFigure conn) {
        ...
        if (connection.getStartFigure() != connection.getEndFigure()) return;
        Figure owner = connection.getStartFigure();
        final Rectangle2D.Double r1 = owner.getBounds();
        final Point2D.Double p1 = ConnectorGeom.angleToPointGeom(r1, ConnectorGeom.pointToAngleGeom(r1, connection.getPoint(1)));
        connection.removeAllNodes();
        path.moveTo(p1.x, p1.y);
        double deltaX = Math.min(64, r1.width);
        double deltaY = Math.min(64, r1.height);
        if (r1.x + r1.width / 2 > p1.x) deltaX = -deltaX;
        if (r1.y + r1.height / 2 > p1.y) deltaY = -deltaY;
        path.curveTo(p1.x + deltaX, p1.y - deltaY, p1.x + deltaX, p1.y + deltaY, p1.x, p1.y);
        path.invalidatePath();
    }
