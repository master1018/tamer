...
public class OboOwlApiProcessor extends EntityProcessorBase {
    ...
    protected class RowsIterator implements Iterator<Map<String, Object>> {
        ...
        public boolean hasNext() {
            if (itr == null) itr = this.wrapper.getOntology().getClassesInSignature().iterator();
            return itr.hasNext();
        }
        ...
        public Map<String, Object> next() {
            if (hasNext()) {
                ...
                if (DEBUG) LOG.debug("Processing '" + cls + "' owl class for indexing");
                ...
                if (synonyms != null && synonyms.length > 0) row.put("synonym", synonyms);
                String[] altIds = wrapper.getAltId(cls);
                if (altIds != null && altIds.length > 0) row.put("alternate_id", altIds);
                String[] subsets = wrapper.getSubsets(cls);
                if (subsets != null && subsets.length > 0) row.put("subset", subsets);
                ...
                for (OWLGraphEdge edge : outgoing) {
                    if (edge.getQuantifiedPropertyList().size() > 1) {
                        ...
                        if (prop.getQuantifier() == OWLQuantifiedProperty.Quantifier.SOME || prop.getQuantifier() == OWLQuantifiedProperty.Quantifier.SUBCLASS_OF) {
                            if (targetId != null) closureIdList.add(targetId);
                            if (targetLabel != null) closureLabelList.add(targetLabel);
                            if (prop.getQuantifier() == OWLQuantifiedProperty.Quantifier.SOME) {
                                ...
                                if ("go:regulates".equals(propId)) {
                                    if (targetId != null) regulatesList.add(targetId);
                                    if (targetLabel != null) {
