...
public class Main {
    ...
    public static void main(String[] args) throws URISyntaxException {
        ...
        try {
            ...
            if (!quietMode) {
                ...
                System.out.println();
                if (htmlDiff) System.out.println("Diff type: html"); else System.out.println("Diff type: tag");
                System.out.println("Writing " + (htmlOut ? "html" : "xml") + " output to " + outputFile);
            }
            if (css.length > 0) {
                if (!quietMode) System.out.println("Adding external css files:");
            ...
            if (htmlDiff) {
                ...
                ContentHandler postProcess = htmlOut ? filter.xsl(result, "org/outerj/daisy/diff/tagheader.xsl") : result;
                postProcess.startDocument();
                postProcess.startElement("", "diffreport", "diffreport", new AttributesImpl());
                postProcess.startElement("", "diff", "diff", new AttributesImpl());
                System.out.print(".");
                DaisyDiff.diffTag(new BufferedReader(new InputStreamReader(oldStream)), new BufferedReader(new InputStreamReader(newStream)), postProcess);
                System.out.print(".");
                postProcess.endElement("", "diff", "diff");
                postProcess.endElement("", "diffreport", "diffreport");
                postProcess.endDocument();
        ...
        if (quietMode) System.out.println(); else System.out.println("done");
    }
    ...
    private static void doCSS(String[] css, ContentHandler handler) throws SAXException {
        ...
        for (String cssLink : css) {
            ...
            attr.addAttribute("", "media", "media", "CDATA", "print");
