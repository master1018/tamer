...
public class FunMin extends CollatingFunction {
    ...
    public Sequence eval(Sequence contextSequence, Item contextItem) throws XPathException {
        ...
        if (arg.isEmpty()) result = Sequence.EMPTY_SEQUENCE; else {
            ...
            while (iter.hasNext()) {
                ...
                if (item instanceof QNameValue) throw new XPathException(this, ErrorCodes.FORG0006, "Cannot compare " + Type.getTypeName(item.getType()), arg);
                AtomicValue value = item.atomize();
                if (value.getType() == Type.UNTYPED_ATOMIC) value = value.convertTo(Type.DOUBLE);
                if (min == null) min = value; else {
                    ...
                    if (Type.subTypeOf(value.getType(), Type.NUMBER)) {
                        if (!Type.subTypeOf(min.getType(), Type.NUMBER)) throw new XPathException(this, ErrorCodes.FORG0006, "Cannot compare " + Type.getTypeName(min.getType()) + " and " + Type.getTypeName(value.getType()), min);
                        if (((NumericValue) value).isNaN()) {
                            value = value.promote(min);
                            if (value.getType() == Type.FLOAT) min = FloatValue.NaN; else min = DoubleValue.NaN;
                            continue;
                        }
                        min = min.promote(value);
                    }
                    if (value instanceof ComputableValue) {
                        if (!(min instanceof ComputableValue)) throw new XPathException(this, ErrorCodes.FORG0006, "Cannot compare " + Type.getTypeName(min.getType()) + " and " + Type.getTypeName(value.getType()), min);
                        value = value.promote(min);
                        min = min.min(collator, value);
                        computableProcessing = true;
                    } else {
                        if (computableProcessing) throw new XPathException(this, ErrorCodes.FORG0006, "Cannot compare " + Type.getTypeName(min.getType()) + " and " + Type.getTypeName(value.getType()), value);
                        if (Collations.compare(collator, value.getStringValue(), min.getStringValue()) < 0) min = value;
                    }
                }
            }
            result = min;
