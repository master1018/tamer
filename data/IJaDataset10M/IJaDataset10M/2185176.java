...
public class ConsumerManager {
    ...
    public ConsumerManager() throws ConsumerException {
        _httpClient = HttpClientFactory.getInstance(_maxRedirects, Boolean.FALSE, _socketTimeout, _connectTimeout, CookiePolicy.IGNORE_COOKIES);
        _realmVerifier = new RealmVerifier(false);
        _realmVerifier.setEnforceRpId(false);
        if (Association.isHmacSha256Supported()) _prefAssocSessEnc = AssociationSessionType.DH_SHA256; else _prefAssocSessEnc = AssociationSessionType.DH_SHA1;
    }
    ...
    public void setMaxAssocAttempts(int maxAssocAttempts) throws ConsumerException {
        if (maxAssocAttempts > 0 || _allowStateless) this._maxAssocAttempts = maxAssocAttempts; else throw new IllegalArgumentException("Associations and stateless mode " + "cannot be both disabled at the same time.");
    ...
    public void setAllowStateless(boolean allowStateless) {
        if (_allowStateless || _maxAssocAttempts > 0) this._allowStateless = allowStateless; else throw new IllegalArgumentException("Associations and stateless mode " + "cannot be both disabled at the same time.");
    }
    ...
    public void setPrivateAssociationStore(ConsumerAssociationStore associations) throws ConsumerException {
        if (associations == null) throw new ConsumerException("Cannot set null private association store, " + "needed for consumer nonces.");
    ...
    public void setConnectTimeout(int connectTimeout) {
        ...
        _httpClient.getHttpConnectionManager().getParams().setConnectionTimeout(_connectTimeout);
    }
    ...
    public void setMaxRedirects(int maxRedirects) {
        ...
        _httpClient.getParams().setParameter("http.protocol.max-redirects", new Integer(_maxRedirects));
    }
    ...
    private int call(String url, Message request, ParameterList response) throws MessageException {
        ...
        try {
            post.setRequestEntity(new StringRequestEntity(request.wwwFormEncoding(), "application/x-www-form-urlencoded", "UTF-8"));
            ...
            _log.error("Error talking to " + url + " response code: " + responseCode, e);
        } finally {
    ...
    public DiscoveryInformation associate(List discoveries) {
        ...
        while (itr.hasNext() && attemptsLeft > 0) {
            ...
            if (assoc != null && !Association.FAILED_ASSOC_HANDLE.equals(assoc.getHandle())) return discovered;
        }
        if (discoveries.size() > 0) {
            DiscoveryInformation d0 = (DiscoveryInformation) discoveries.get(0);
            _log.warn("Association failed; using first entry: " + d0.getOPEndpoint());
            return d0;
        } else {
    ...
    private int associate(DiscoveryInformation discovered, int maxAttempts) {
        if (_maxAssocAttempts == 0) return 0;
        URL opUrl = discovered.getOPEndpoint();
        String opEndpoint = opUrl.toString();
        _log.info("Trying to associate with " + opEndpoint + " attempts left: " + maxAttempts);
        Association a = _associations.load(opEndpoint);
        if (a != null && (Association.FAILED_ASSOC_HANDLE.equals(a.getHandle()) || a.getExpiry().getTime() - System.currentTimeMillis() > _preExpiryAssocLockInterval * 1000)) {
        ...
        if (_prefAssocSessEnc.isVersion2() == discovered.isVersion2()) requests.put(_prefAssocSessEnc, null);
        ...
        while (attemptsLeft > 0 && !reqStack.empty()) {
            try {
                ...
                if (DEBUG) _log.debug("Trying association type: " + assocReq.getType());
                ...
                if (status == HttpStatus.SC_OK) {
                    ...
                    assocResp = AssociationResponse.createAssociationResponse(respParams);
                    ...
                    if (respType.equals(assocReq.getType()) || (!discovered.isVersion2() && respType.getHAlgorithm() == null && createAssociationRequest(respType, opUrl) != null)) {
                        _associations.save(opEndpoint, assoc);
                        _log.info("Associated with " + discovered.getOPEndpoint() + " handle: " + assoc.getHandle());
                        break;
                    } else _log.info("Discarding association response, " + "not matching consumer criteria");
                } else if (status == HttpStatus.SC_BAD_REQUEST) {
                    _log.info("Association attempt failed.");
                    AssociationError assocErr = AssociationError.createAssociationError(respParams);
                    AssociationSessionType opType = AssociationSessionType.create(assocErr.getSessionType(), assocErr.getAssocType());
                    if (alreadyTried.keySet().contains(opType)) continue;
                    AssociationRequest newReq = createAssociationRequest(opType, opUrl);
                    if (newReq != null) {
                        if (DEBUG) _log.debug("Retrieved association type " + "from the association error: " + newReq.getType());
        ...
        if (Association.FAILED_ASSOC_HANDLE.equals(handle) && _failedAssocExpire > 0) _associations.save(opEndpoint, Association.getFailedAssociation(_failedAssocExpire));
     ...
     */
    private AssociationRequest createAssociationRequest(AssociationSessionType type, URL opUrl) {
        try {
            if (_minAssocSessEnc.isBetter(type)) return null;
            ...
            if (type.getHAlgorithm() != null) {
                ...
                if (DiffieHellmanSession.isDhSupported(type) && Association.isHmacSupported(type.getAssociationType())) assocReq = AssociationRequest.createAssociationRequest(type, dhSess);
            } else if (opUrl.getProtocol().equals("https") && Association.isHmacSupported(type.getAssociationType())) assocReq = AssociationRequest.createAssociationRequest(type);
            if (assocReq == null) _log.warn("Could not create association of type: " + type);
            return assocReq;
        } catch (OpenIDException e) {
    ...
    public AuthRequest authenticate(DiscoveryInformation discovered, String returnToUrl, String realm) throws MessageException, ConsumerException {
        if (discovered == null) throw new ConsumerException("Authentication cannot continue: " + "no discovery information provided.");
        Association assoc = _associations.load(discovered.getOPEndpoint().toString());
        if (assoc == null) {
            associate(discovered, _maxAssocAttempts);
            assoc = _associations.load(discovered.getOPEndpoint().toString());
        }
        String handle = assoc != null ? assoc.getHandle() : Association.FAILED_ASSOC_HANDLE;
        String claimedId, delegate;
        if (discovered.hasClaimedIdentifier()) {
            claimedId = discovered.getClaimedIdentifier().getIdentifier();
            delegate = discovered.hasDelegateIdentifier() ? discovered.getDelegateIdentifier() : claimedId;
        } else {
            claimedId = AuthRequest.SELECT_ID;
            delegate = AuthRequest.SELECT_ID;
        }
        if (!_allowStateless && Association.FAILED_ASSOC_HANDLE.equals(handle)) throw new ConsumerException("Authentication cannot be performed: " + "no association available and stateless mode is disabled");
        _log.info("Creating authentication request for" + " OP-endpoint: " + discovered.getOPEndpoint() + " claimedID: " + claimedId + " OP-specific ID: " + delegate);
        if (!discovered.isVersion2()) returnToUrl = insertConsumerNonce(discovered.getOPEndpoint().toString(), returnToUrl);
        ...
        if (!AuthRequest.SELECT_ID.equals(claimedId)) authReq.setImmediate(_immediateAuth);
        return authReq;
    }
    ...
    public boolean verifyReturnTo(String receivingUrl, AuthSuccess response) {
        if (DEBUG) _log.debug("Verifying return URL; receiving: " + receivingUrl + "\nmessage: " + response.getReturnTo());
        URL receiving;
        URL returnTo;
        try {
        ...
        if (receivingPath.length() > 0 && receivingPath.charAt(receivingPath.length() - 1) != '/') receivingPath.append('/');
        StringBuffer returnToPath = new StringBuffer(returnTo.getPath());
        if (returnToPath.length() > 0 && returnToPath.charAt(returnToPath.length() - 1) != '/') returnToPath.append('/');
        if (!receiving.getProtocol().equals(returnTo.getProtocol()) || !receiving.getAuthority().equals(returnTo.getAuthority()) || !receivingPath.toString().equals(returnToPath.toString())) {
            if (DEBUG) _log.debug("Return URL schema, authority or " + "path verification failed.");
            return false;
        }
        try {
            ...
            if (receivingParams == null) {
                if (DEBUG) _log.debug("Return URL query parameters verification failed.");
                return false;
            }
            ...
            while (iter.hasNext()) {
                ...
                if (receivingValues == null || receivingValues.size() != returnToValues.size() || !receivingValues.containsAll(returnToValues)) {
                    if (DEBUG) _log.debug("Return URL query parameters verification failed.");
                    return false;
                }
            }
    ...
    public Map extractQueryParams(URL url) throws UnsupportedEncodingException {
        ...
        while (iter.hasNext()) {
            ...
            String key = equalPos > -1 ? URLDecoder.decode(keyValue.substring(0, equalPos), "UTF-8") : URLDecoder.decode(keyValue, "UTF-8");
            String value;
            if (equalPos <= -1) value = null; else if (equalPos + 1 > keyValue.length()) value = ""; else value = URLDecoder.decode(keyValue.substring(equalPos + 1), "UTF-8");
            List existingValues = (List) paramsMap.get(key);
            if (existingValues == null) {
                List newValues = new ArrayList();
                newValues.add(value);
                paramsMap.put(key, newValues);
            } else existingValues.add(value);
        }
        return paramsMap;
    }
    ...
    public boolean verifyNonce(AuthSuccess authResp, DiscoveryInformation discovered) {
        ...
        if (nonce == null) nonce = extractConsumerNonce(authResp.getReturnTo(), discovered.getOPEndpoint().toString());
        if (nonce == null) return false;
        return (NonceVerifier.OK == _nonceVerifier.seen(discovered.getOPEndpoint().toString(), nonce));
    }
    ...
    public String insertConsumerNonce(String opUrl, String returnTo) {
        ...
        if (privateAssoc == null) {
            try {
                ...
                privateAssoc = Association.generate(getPrefAssocSessEnc().getAssociationType(), "", _failedAssocExpire);
        ...
        try {
            ...
            returnTo += "&openid.rpsig=" + URLEncoder.encode(privateAssoc.sign(returnTo), "UTF-8");
    ...
    public String extractConsumerNonce(String returnTo, String opUrl) {
        if (DEBUG) _log.debug("Extracting consumer nonce...");
        String nonce = null;
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            try {
                ...
                if (keyVal.length == 2 && "openid.rpsig".equals(keyVal[0])) {
                    ...
                    if (DEBUG) _log.debug("Extracted consumer nonce signature: " + signature);
                }
            } catch (UnsupportedEncodingException e) {
    ...
    private DiscoveryInformation verifyDiscovered(AuthSuccess authResp, DiscoveryInformation discovered) throws DiscoveryException {
        ...
        if (authResp.isVersion2()) return verifyDiscovered2(authResp, discovered); else return verifyDiscovered1(authResp, discovered);
    }
    ...
    private DiscoveryInformation verifyDiscovered1(AuthSuccess authResp, DiscoveryInformation discovered) throws DiscoveryException {
        if (authResp == null || authResp.isVersion2() || authResp.getIdentity() == null) {
            if (DEBUG) _log.error("Invalid authentication response: " + "cannot verify v1 discovered information");
            return null;
        }
        String assertId = authResp.getIdentity();
        if (discovered != null && !discovered.isVersion2() && discovered.getClaimedIdentifier() != null) {
            if (DEBUG) _log.debug("Verifying discovered information " + "for OpenID1 assertion about ClaimedID: " + discovered.getClaimedIdentifier().getIdentifier());
            String discoveredId = discovered.hasDelegateIdentifier() ? discovered.getDelegateIdentifier() : discovered.getClaimedIdentifier().getIdentifier();
            if (assertId.equals(discoveredId)) return discovered;
        }
        _log.info("Proceeding with stateless mode / bare response verification...");
        DiscoveryInformation firstServiceMatch = null;
        if (DEBUG) _log.debug("Performing discovery on the ClaimedID in the assertion: " + assertId);
        ...
        while (iter.hasNext()) {
            ...
            if (service.isVersion2() || !service.hasClaimedIdentifier() || service.hasDelegateIdentifier() || !assertId.equals(service.getClaimedIdentifier().getIdentifier())) continue;
            if (DEBUG) _log.debug("Found matching service: " + service);
            if (firstServiceMatch == null) firstServiceMatch = service;
            Association assoc = _associations.load(service.getOPEndpoint().toString(), authResp.getHandle());
            if (assoc != null) {
                if (DEBUG) _log.debug("Found existing association for  " + service + " Not looking for another service endpoint.");
                return service;
            }
        }
        if (firstServiceMatch == null) _log.error("No service element found to match " + "the identifier in the assertion.");
    ...
    private DiscoveryInformation verifyDiscovered2(AuthSuccess authResp, DiscoveryInformation discovered) throws DiscoveryException {
        if (authResp == null || !authResp.isVersion2() || authResp.getIdentity() == null || authResp.getClaimed() == null) {
            if (DEBUG) _log.debug("Discovered information doesn't match " + "auth response / version");
            return null;
        }
        String assertId = authResp.getIdentity();
        Identifier respClaimed = _discovery.parseIdentifier(authResp.getClaimed(), true);
        String respEndpoint = authResp.getOpEndpoint();
        if (DEBUG) _log.debug("Verifying discovered information for OpenID2 assertion " + "about ClaimedID: " + respClaimed.getIdentifier());
        if (discovered != null && discovered.hasClaimedIdentifier() && discovered.getClaimedIdentifier().equals(respClaimed)) {
            String opSpecific = discovered.hasDelegateIdentifier() ? discovered.getDelegateIdentifier() : discovered.getClaimedIdentifier().getIdentifier();
            if (opSpecific.equals(assertId) && discovered.isVersion2() && discovered.getOPEndpoint().toString().equals(respEndpoint)) {
                if (DEBUG) _log.debug("ClaimedID in the assertion was previously discovered: " + respClaimed);
                return discovered;
            }
        }
        DiscoveryInformation firstServiceMatch = null;
        if (DEBUG) _log.debug("Performing discovery on the ClaimedID in the assertion: " + respClaimed);
        List discoveries = _discovery.discover(respClaimed);
        if (DEBUG) _log.debug("Looking for a service element to match " + "the ClaimedID and OP endpoint in the assertion...");
        Iterator iter = discoveries.iterator();
        while (iter.hasNext()) {
            DiscoveryInformation service = (DiscoveryInformation) iter.next();
            if (DiscoveryInformation.OPENID2_OP.equals(service.getVersion())) continue;
            String opSpecific = service.hasDelegateIdentifier() ? service.getDelegateIdentifier() : service.getClaimedIdentifier().getIdentifier();
            if (!opSpecific.equals(assertId) || !service.isVersion2() || !service.getOPEndpoint().toString().equals(respEndpoint)) continue;
            ...
            Association assoc = _associations.load(service.getOPEndpoint().toString(), authResp.getHandle());
            if (assoc != null) {
                if (DEBUG) _log.debug("Found existing association, " + "not looking for another service endpoint.");
                return service;
            }
        }
        if (firstServiceMatch == null) _log.error("No service element found to match " + "the ClaimedID / OP-endpoint in the assertion.");
        return firstServiceMatch;
    }
     ...
     */
    private VerificationResult verifySignature(AuthSuccess authResp, DiscoveryInformation discovered, VerificationResult result) throws AssociationException, MessageException, DiscoveryException {
        ...
        Identifier claimedId = discovered.isVersion2() ? _discovery.parseIdentifier(authResp.getClaimed()) : discovered.getClaimedIdentifier();
        String handle = authResp.getHandle();
        URL op = discovered.getOPEndpoint();
        Association assoc = _associations.load(op.toString(), handle);
        if (assoc != null) {
            _log.info("Found association: " + assoc.getHandle() + " verifying signature locally...");
            ...
            if (HttpStatus.SC_OK == respCode) {
                ...
                if (vrfyResp.isSignatureVerified()) {
                    ...
                    if (invalidateHandle != null) _associations.remove(op.toString(), invalidateHandle);
                    result.setVerifiedId(claimedId);
                    if (DEBUG) _log.debug("Direct signature verification succeeded " + "with OP: " + op);
                } else {
                    if (DEBUG) _log.debug("Direct signature verification failed " + "with OP: " + op);
                    result.setStatusMsg("Direct signature verification failed.");
                }
            } else {
                DirectError err = DirectError.createDirectError(responseParams);
                if (DEBUG) _log.debug("Error verifying signature with the OP: " + op + " error message: " + err.keyValueFormEncoding());
                result.setStatusMsg("Error verifying signature with the OP: " + err.getErrorMsg());
            }
        }
        Identifier verifiedID = result.getVerifiedId();
        if (verifiedID != null) _log.info("Verification succeeded for: " + verifiedID); else _log.error("Verification failed for: " + authResp.getClaimed() + " reason: " + result.getStatusMsg());
        return result;
    }
}
