...
public abstract class RenumbererTestBase extends TestCase {
   ...
   */
    enum Type {
        ...
        /** No attributes */
        NONE, /** t=h1 */
        HEADING, /** t=li without listyle */
        LIST, /** t=li with listyle = decimal */
        DECIMAL
    }
   ...
   */
    ParagraphHtmlRenderer renderer = new ParagraphHtmlRenderer() {
        ...
        @Override
        public void updateRendering(HasImplNodelets element, String type, String listStyle, int indent, Alignment alignment, Direction direction) {
        }
   ...
   */
    static class Renderer extends ParagraphRenderer implements PermanentMutationHandler {
        ...
        Renderer(ParagraphHtmlRenderer htmlRenderer, OrderedListRenumberer renumberer, FinalTaskRunner finalRaskRunner) {
   ...
   */
    ...
    protected void setUp() {
        ...
        LineRendering.registerParagraphRenderer(Editor.ROOT_HANDLER_REGISTRY, new Renderer(renderer, renumberer, runner));
   ...
   */
    void doRandomTest(int testIterations, int seed) {
        ...
        final int DECIMALS_TO_OTHERS = 4;
        final int UPDATE_TO_ADD_REMOVE = 4;
                ...
                while (i < endSection && line != null) {
                    ...
                    while (length > 0 && i < endSection && line != null) {
                        ...
                        if (r.nextInt(UPDATE_TO_ADD_REMOVE) == 0) {
                            ...
                            boolean add = index == 0 || totalLines < SIZE / 2 ? true : (totalLines > SIZE * 2 ? false : r.nextBoolean());
                            if (add) {
   ...
   */
    Line create(int index, Type type, int indent, boolean createAndUpdateSeparately) {
        ...
        if (createAndUpdateSeparately) {
            ...
            l = Line.fromLineElement(doc.createElement(loc, "line", attributes(type, indent, false, true)));
        }
        assertNotNull(l);
        return l;
    }
    ...
    Map<String, String> attributes(Type type, int indent, boolean alwaysSetStyle, boolean noNulls) {
        ...
        switch(type) {
            case HEADING:
                ...
                break;
            case LIST:
                t = Paragraph.LIST_TYPE;
                break;
            case DECIMAL:
                t = Paragraph.LIST_TYPE;
                lt = Paragraph.LIST_STYLE_DECIMAL;
                break;
        }
   ...
   */
        ...
        while (line != null) {
            ...
            if (line.isDecimalListItem()) {
                ...
                if (num != line.getCachedNumberValue()) {
                    ...
                    printInfo(line, msg);
                    fail("Wrong number on iteration " + iter + ". " + msg + ". See stdout & stderr for debug details");
                }
            } else {
                numbers.setNumber(1);
            }
    ...
    void printInfo(Line badLine, String msg) {
        ...
        while (line != null) {
            int indent = line.getIndent();
            stream.println(CollectionUtils.repeat('.', line.getIndent()) + line.toString() + " indent:" + indent + CollectionUtils.repeat(' ', 20) + line.getCachedNumberValue() + "  (" + i + ")");
            if (line == badLine) {
                stream.println("\n\n\n");
                stream = System.err;
                stream.println(msg);
