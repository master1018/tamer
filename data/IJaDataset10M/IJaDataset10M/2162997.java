...
public class JavaProcessDialog extends JDialog {
    ...
    private JTable getJpsTable() {
        if (jpsTable == null) {
            String[] header = { M17N.get("JavaProcessDialog.jpsTable.name"), M17N.get("JavaProcessDialog.jpsTable.pid") };
            List<VirtualMachineDescriptor> vmList = VirtualMachine.list();
            if (vmList == null || vmList.isEmpty()) {
                JOptionPane.showMessageDialog(null, M17N.get("JavaProcessDialog.warn.jps_not_found"), "", JOptionPane.ERROR_MESSAGE);
            }
            String[][] data = new String[vmList.size()][2];
            int iVm = 0;
            for (VirtualMachineDescriptor vm : vmList) {
                data[iVm++] = new String[] { vm.displayName(), vm.id() };
            }
            ...
            jpsTable.addMouseListener(new MouseInputAdapter() {
                ...
                public void mouseClicked(MouseEvent event) {
                    if (SwingUtilities.isLeftMouseButton(event) && event.getClickCount() == 2) {
                        String pid = jpsTable.getValueAt(jpsTable.rowAtPoint(event.getPoint()), 1).toString();
