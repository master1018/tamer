...
class IDispatchProxy extends IDispatchImpl {
   ...
   */
    ...
    @Override
    protected Variant invoke(int dispId, int flags, Variant[] params) throws HResultException, InstantiationException, InvocationTargetException {
        ...
        try {
            if (dispId == 0 && (flags & COM.DISPATCH_METHOD) != 0 && params.length >= 2) {
                Variant dispIdVar = params[0];
                dispId = dispIdVar.getInt();
                if (javaDispatch.isMethod(dispId)) {
                    MethodAdaptor method = javaDispatch.getMethod(dispId);
                    Object target = getTarget();
                    Object jthis = method.needsThis() ? JsValueGlue.get(new JsValueIE6(params[1]), classLoader, method.getDeclaringClass(), "this") : null;
            ...
            if (dispId == DISPID_MAGIC_GETGLOBALREF && myGlobalRef != 0) {
                ...
                if (javaDispatch.isMethod(dispId)) {
                    ...
                    if (flags == COM.DISPATCH_PROPERTYGET) {
                        return SwtOleGlue.convertObjectToVariant(classLoader, dispIdOracle, field.getType(), javaDispatch.getFieldValue(dispId));
                    } else if ((flags & (COM.DISPATCH_PROPERTYPUT | COM.DISPATCH_PROPERTYPUTREF)) != 0) {
                        javaDispatch.setFieldValue(dispId, JsValueGlue.get(new JsValueIE6(params[0]), classLoader, field.getType(), "Setting field '" + field.getName() + "'"));
                        return new Variant();
                    }
                }
            } else {
