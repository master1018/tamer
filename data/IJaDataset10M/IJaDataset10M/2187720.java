package it.infocostitre.activitytabs.provider;
...
public class Market {
    ...
    public static final String APK_DOWNLOAD_DETAILS_PAGE_PREFIX = "http://code.google.com/p/chartdroid/downloads/detail?name=";

    public static final String APK_DOWNLOAD_URL_PREFIX = "http://chartdroid.googlecode.com/files/";

    public static final String APK_DOWNLOAD_FILENAME_CHARTDROID = "Chartdroid-1.9.10.apk";

    public static final Uri APK_DOWNLOAD_URI_CHARTDROID = Uri.parse(APK_DOWNLOAD_URL_PREFIX + APK_DOWNLOAD_FILENAME_CHARTDROID);
    ...
    public static void intentLaunchWithMarketFallback(Activity context, Intent intent, int request_code, String package_name) {
        if (isIntentAvailable(context, intent)) {
            if (request_code < 0) context.startActivity(intent); else context.startActivityForResult(intent, request_code);
        } else {
