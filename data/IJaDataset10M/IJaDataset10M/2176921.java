...
public class PerceptronClassifier<E> implements ScoredClassifier<E>, Serializable {
    ...
    public PerceptronClassifier(Corpus<ObjectHandler<Classified<E>>> corpus, FeatureExtractor<? super E> featureExtractor, KernelFunction kernelFunction, String corpusAcceptCategory, int numIterations, String outputAcceptCategory, String outputRejectCategory) throws IOException {
        ...
        for (int iteration = 0; iteration < numIterations; ++iteration) {
            for (int i = 0; i < featureVectors.length; ++i) {
                double yHat = prediction(featureVectors[i], featureVectors, polarities, weights, basisIndexes, currentPerceptronIndex);
                boolean accept = yHat > 0.0;
                if (accept == polarities[i]) {
                    if (currentPerceptronIndex >= 0) ++weights[currentPerceptronIndex];
                } else {
        ...
        int next = 0;
        for (int i = 0; i <= currentPerceptronIndex; ++i) if (!renumbering.containsKey(basisIndexes[i])) renumbering.put(basisIndexes[i], next++);
        ...
        for (int i = currentPerceptronIndex + 1; --i >= 0; ) {
            ...
            weightSum += weights[i];
            if (polarities[i]) mBasisWeights[newIndex] += weightSum; else mBasisWeights[newIndex] -= weightSum;
        }
    }
    ...
    public ScoredClassification classify(E in) {
        ...
        double sum = 0.0;
        for (int i = mBasisVectors.length; --i >= 0; ) sum += mBasisWeights[i] * mKernelFunction.proximity(mBasisVectors[i], inputVector);
        return sum > 0 ? new ScoredClassification(new String[] { mAcceptCategory, mRejectCategory }, new double[] { sum, -sum }) : new ScoredClassification(new String[] { mRejectCategory, mAcceptCategory }, new double[] { -sum, sum });
    }
    ...
    class CorpusCollector implements ObjectHandler<Classified<E>> {
        ...
        public void handle(Classified<E> classified) {
            ...
            mInputAcceptList.add(mAcceptCategory.equals(c.bestCategory()) ? Boolean.TRUE : Boolean.FALSE);
        }
        ...
        boolean[] polarities() {
            ...
            for (int i = 0; i < categories.length; ++i) categories[i] = mInputAcceptList.get(i).booleanValue();
            return categories;
        }
    }
    ...
    static class Externalizer<F> extends AbstractExternalizable {
        ...
        public Object read(ObjectInput in) throws ClassNotFoundException, IOException {
            @SuppressWarnings("unchecked") FeatureExtractor<? super F> featureExtractor = (FeatureExtractor<? super F>) in.readObject();
            ...
            for (int i = 0; i < basisLen; ++i) basisVectors[i] = (Vector) in.readObject();
            int[] basisWeights = new int[basisLen];
            for (int i = 0; i < basisLen; ++i) basisWeights[i] = in.readInt();
            String acceptCategory = in.readUTF();
            String rejectCategory = in.readUTF();
            return new PerceptronClassifier<F>(featureExtractor, kernelFunction, symbolTable, basisVectors, basisWeights, acceptCategory, rejectCategory);
        }
        ...
        public void writeExternal(ObjectOutput out) throws IOException {
            ...
            for (int i = 0; i < mClassifier.mBasisVectors.length; ++i) out.writeObject(mClassifier.mBasisVectors[i]);
            for (int i = 0; i < mClassifier.mBasisWeights.length; ++i) out.writeInt(mClassifier.mBasisWeights[i]);
            out.writeUTF(mClassifier.mAcceptCategory);
            out.writeUTF(mClassifier.mRejectCategory);
        }
    }

    static final int INITIAL_BASIS_SIZE = 32 * 1024;
}
