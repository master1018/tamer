...
public class ModelProvider {
    ...
    public static class ListModelFactory {
        ...
        public static ListModelList getBeanListModelList(int count, String label) {
            ...
            for (int i = 0; i < count; i++) {
                list.add(new SampleBean(label + " " + rand.nextInt(count), rand.nextInt(count), cal.getTime()));
        ...
        public static ListModelList getArrayListModelList(int count, String label) {
            ...
            Calendar cal = Calendar.getInstance();
            for (int i = 0; i < count; i++) {
                list.add(new String[] { label + " " + rand.nextInt(count), rand.nextInt(count) + "", df.format(cal.getTime()) });
                cal.add(Calendar.DAY_OF_MONTH, 1);
            }
            return new ListModelList(list);
        }
        ...
        public static void addListModelListAry(ListModelList model, String label) {
            model.add(new String[] { label, getRandom().nextInt(10) + "", df.format(new Date()) });
        }
        ...
        public static void updateListModelListAry(ListModelList model, int index, String label) {
            model.set(index, new String[] { label, getRandom().nextInt(10) + "", df.format(new Date()) });
        }
    }
    ...
    public static class TreeModelFactory {
        ...
        public static DefaultTreeModel getSingleColDefaultTreeModel(int level1, int level2, int level3, String label) {
            ...
            for (int i = 0, j = list.size(); i < j; i++) list2.add(new DefaultTreeNode(list.get(i)));
            ...
            for (int i = 0; i < level1; i++) {
                ...
                for (int j = 0; j < level2; j++) {
                    ...
                    for (int k = 0; k < level3; k++) {
                        subSubList.add(new DefaultTreeNode(new SampleBean(label + " " + rand.nextInt(level3), rand.nextInt(level3), cal.getTime())));
                        cal.add(Calendar.MINUTE, 1);
                    }
                    subList.add(new DefaultTreeNode(new SampleBean(label + " " + rand.nextInt(level2), rand.nextInt(level2), cal.getTime()), subSubList));
                    cal.add(Calendar.HOUR_OF_DAY, 1);
                }
                list.add(new DefaultTreeNode(new SampleBean(label + " " + rand.nextInt(level1), rand.nextInt(level1), cal.getTime()), subList));
                cal.add(Calendar.DAY_OF_MONTH, 1);
            }
        ...
        public static DefaultTreeModel getArrayDefaultTreeModel(int level1, int level2, int level3, String label) {
            ...
            for (int i = 0; i < level1; i++) {
                ...
                for (int j = 0; j < level2; j++) {
                    List subSubList = new ArrayList(level3);
                    for (int k = 0; k < level3; k++) {
                        subSubList.add(new DefaultTreeNode(new String[] { label + " " + rand.nextInt(level3), rand.nextInt(level3) + "", df.format(cal.getTime()) }));
                        cal.add(Calendar.MINUTE, 1);
                    }
                    subList.add(new DefaultTreeNode(new String[] { label + " " + rand.nextInt(level2), rand.nextInt(level2) + "", df.format(cal.getTime()) }, subSubList));
                    cal.add(Calendar.HOUR_OF_DAY, 1);
                }
                list.add(new DefaultTreeNode(new String[] { label + " " + rand.nextInt(level1), rand.nextInt(level1) + "", df.format(cal.getTime()) }, subList));
                cal.add(Calendar.DAY_OF_MONTH, 1);
            }
            return new DefaultTreeModel(new DefaultTreeNode(null, list));
        }
        ...
        public static void addDefaultTreeModelAry(Treeitem item, String label) {
            if (item == null || item.getValue() == null) return;
            DefaultTreeNode node = (DefaultTreeNode) ((DefaultTreeNode) item.getValue()).getParent();
            node.add(new DefaultTreeNode(new String[] { label, getRandom().nextInt(10) + "", df.format(new Date()) }));
        }
            ...
            return new RowRenderer() {
                ...
                public void render(Row row, Object data) {
                    if (row instanceof Group) {
                        ...
                        row.appendChild(new Label(getGroupHead(row, (SampleBean) obj[0], ((Integer) obj[1]).intValue())));
                    } else {
                        SampleBean b = (SampleBean) data;
                        row.appendChild(new Label(b.getName()));
                        row.appendChild(new Label(b.getNumber() + ""));
            ...
            return new ListitemRenderer() {
                ...
                public void render(Listitem item, Object data) throws Exception {
                    if (item instanceof Listgroup) {
                        Object[] obj = (Object[]) data;
                        item.appendChild(new Listcell(getGroupHead(item, (SampleBean) obj[0], ((Integer) obj[1]).intValue())));
                    } else {
                        SampleBean b = (SampleBean) data;
                        item.appendChild(new Listcell(b.getName()));
                        item.appendChild(new Listcell(b.getNumber() + ""));
                ...
                private String getGroupHead(Listitem item, SampleBean bean, int index) {
                    Listheader hd = (Listheader) item.getListbox().getListhead().getChildren().get(index);
                    String field = ((FieldComparator) hd.getSortAscending()).getRawOrderBy();
                    try {
                        return Fields.get(bean, field).toString();
                    } catch (NoSuchMethodException ex) {
                        throw UiException.Aide.wrap(ex);
