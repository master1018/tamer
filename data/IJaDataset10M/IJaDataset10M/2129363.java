...
public class DiffPrint {
   ...
   */
    public abstract static class Base {
        ...
        protected void setupOutput() {
            if (outfile == null) outfile = new PrintWriter(new OutputStreamWriter(System.out));
        }
      ...
      set to 0.  */
        protected void analyze_hunk(Diff.change hunk) {
            ...
            for (next = hunk; next != null; next = next.link) {
                ...
                for (i = next.line0; i <= l0 && !nontrivial; i++) if (!ignore.execute(file0[i])) nontrivial = true;
                for (i = next.line1; i <= l1 && !nontrivial; i++) if (!ignore.execute(file1[i])) nontrivial = true;
            }
            first0 = f0;
            last0 = l0;
            first1 = f1;
            last1 = l1;
            if (!nontrivial) show_from = show_to = 0;
            deletes = show_from;
            inserts = show_to;
        }
       ...
       We print the translated (real) line numbers.  */
        protected void print_number_range(char sepchar, int a, int b) {
            if (++b > ++a) outfile.print("" + a + sepchar + b); else outfile.print(b);
        }

        public static char change_letter(int inserts, int deletes) {
            if (inserts == 0) return 'd'; else if (deletes == 0) return 'a'; else return 'c';
        }
    }
    ...
    public static class NormalPrint extends Base {
       ...
       describing changes in consecutive lines.  */
        protected void print_hunk(Diff.change hunk) {
            analyze_hunk(hunk);
            if (deletes == 0 && inserts == 0) return;
            print_number_range(',', first0, last0);
            outfile.print(change_letter(inserts, deletes));
            print_number_range(',', first1, last1);
            outfile.println();
            if (deletes != 0) for (int i = first0; i <= last0; i++) print_1_line("< ", file0[i]);
            if (inserts != 0 && deletes != 0) outfile.println("---");
            if (inserts != 0) for (int i = first1; i <= last1; i++) print_1_line("> ", file1[i]);
        }
    }
        ...
        protected void print_hunk(Diff.change hunk) {
            ...
            if (inserts != 0) {
                ...
                for (int i = first1; i <= last1; i++) {
                    if (!inserting) outfile.println(i - first1 + first0 + "a");
                    inserting = true;
                    if (".".equals(file1[i])) {
                        outfile.println("..");
                        outfile.println(".");
                        ...
                        inserting = false;
                    } else print_1_line("", file1[i]);
                }
                if (inserting) outfile.println(".");
            }
        }
    }
   ...
   */
    public static class ContextPrint extends Base {
        ...
        protected void print_context_label(String mark, File inf, String label) {
            setupOutput();
            if (label != null) outfile.println(mark + ' ' + label); else if (inf.lastModified() > 0) outfile.println(mark + ' ' + inf.getPath() + '\t' + new Date(inf.lastModified())); else outfile.println(mark + ' ' + inf.getPath());
        }
        ...
        protected void print_hunk(Diff.change hunk) {
            ...
            if (deletes != 0) {
                ...
                for (int i = first0; i <= last0; i++) {
                    while (next != null && next.line0 + next.deleted <= i) next = next.link;
                    String prefix = " ";
                    if (next != null && next.line0 <= i) prefix = (next.inserted > 0) ? "!" : "-";
            ...
            if (inserts != 0) {
                Diff.change next = hunk;
                for (int i = first1; i <= last1; i++) {
                    while (next != null && next.line1 + next.inserted <= i) next = next.link;
                    String prefix = " ";
                    if (next != null && next.line1 <= i) prefix = (next.deleted > 0) ? "!" : "+";
   ...
   */
    public static class UnifiedPrint extends ContextPrint {
        ...
        private void print_number_range(int a, int b) {
            if (b < a) outfile.print(b + ",0"); else super.print_number_range(',', a, b);
        }
   ...
   */
    ...
    public static void main(String[] argv) throws IOException {
        ...
        if (script == null) System.err.println("No differences"); else {
            Base p;
            switch(style) {
                case 'e':
                    p = new EdPrint(a, b);
