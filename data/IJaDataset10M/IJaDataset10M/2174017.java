...
public class DirectArrayRowAddress extends DirectAddress implements Address.ArrayAddress {
    ...
    public DirectArrayRowAddress(final double[] data, final int row, final Address chain, final int col0, final int col1, final Set<Address.Flags> flags, final boolean contiguous, final int rows, final int cols) {
        super(data, row, row + 1, chain, col0, col1, flags, contiguous, rows, cols);
    }

    @Override
    public ArrayAddress toFortran() {
        return isFortran() ? this : new DirectArrayRowAddress(data, row0, this.chain, col0, col1, EnumSet.of(Address.Flags.FORTRAN), contiguous, rows, cols);
    }

    @Override
    public ArrayAddress toJava() {
        return isFortran() ? new DirectArrayRowAddress(data, row0 + 1, this.chain, col0 + 1, col1 + 1, EnumSet.noneOf(Address.Flags.class), contiguous, rows, cols) : this;
    }
