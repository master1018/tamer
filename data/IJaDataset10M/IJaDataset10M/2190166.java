...
public class Terminal extends CommandGene implements IMutateable, ICloneable {
    ...
    public Terminal(final GPConfiguration a_conf, Class a_returnType) throws InvalidConfigurationException {
   ...
   * @since 3.0
   */
    public Terminal(final GPConfiguration a_conf, Class a_returnType, double a_minValue, double a_maxValue) throws InvalidConfigurationException {
   ...
   * @since 3.2
   */
    public Terminal(final GPConfiguration a_conf, Class a_returnType, double a_minValue, double a_maxValue, boolean a_wholeNumbers) throws InvalidConfigurationException {
        this(a_conf, a_returnType, a_minValue, a_maxValue, a_wholeNumbers, 0);
    }

    public Terminal(final GPConfiguration a_conf, Class a_returnType, double a_minValue, double a_maxValue, boolean a_wholeNumbers, int a_subReturnType) throws InvalidConfigurationException {
    ...
    protected void setRandomValue(int a_value) {
        ...
        m_value_int = (int) Math.round(randomGen.nextDouble() * (m_upperBounds - m_lowerBounds) + m_lowerBounds);
    }

    protected void setRandomValue(long a_value) {
        RandomGenerator randomGen = getGPConfiguration().getRandomGenerator();
        m_value_long = Math.round(randomGen.nextDouble() * (m_upperBounds - m_lowerBounds) + m_lowerBounds);
    }
    ...
    protected void setRandomValue(float a_value) {
        RandomGenerator randomGen = getGPConfiguration().getRandomGenerator();
        m_value_float = (float) (randomGen.nextFloat() * (m_upperBounds - m_lowerBounds) + m_lowerBounds);
        if (m_wholeNumbers) {
        ...
        if (a_percentage > 0.85d) {
            ...
            if (retType == CommandGene.FloatClass) {
                ...
                if (m_value_float >= (m_upperBounds - m_lowerBounds) / 2) {
                    newValuef = m_value_float - getGPConfiguration().getRandomGenerator().nextFloat() * rangef;
                } else {
                    newValuef = m_value_float + getGPConfiguration().getRandomGenerator().nextFloat() * rangef;
                }
                if (Math.abs(newValuef - m_lowerBounds) < DELTA || Math.abs(m_upperBounds - newValuef) < DELTA) {
                    setRandomValue(m_value_float);
                } else {
                    setValue(newValuef);
                }
                ...
                if (m_value_double >= (m_upperBounds - m_lowerBounds) / 2) {
                    newValueD = m_value_double - getGPConfiguration().getRandomGenerator().nextFloat() * rangeD;
                } else {
                    newValueD = m_value_double + getGPConfiguration().getRandomGenerator().nextFloat() * rangeD;
                }
                if (Math.abs(newValueD - m_lowerBounds) < DELTA || Math.abs(m_upperBounds - newValueD) < DELTA) {
                    setRandomValue(m_value_double);
                } else {
                    setValue(newValueD);
                }
                ...
                if (m_value_int >= (m_upperBounds - m_lowerBounds) / 2) {
                    newValueI = m_value_int - (int) Math.round(getGPConfiguration().getRandomGenerator().nextInt() * range);
                } else {
                    newValueI = m_value_int + (int) Math.round(getGPConfiguration().getRandomGenerator().nextFloat() * range);
                }
                ...
                if (m_value_long >= (m_upperBounds - m_lowerBounds) / 2) {
                    newValueL = m_value_long - Math.round(getGPConfiguration().getRandomGenerator().nextInt() * range);
                } else {
                    newValueL = m_value_long + Math.round(getGPConfiguration().getRandomGenerator().nextFloat() * range);
                }
   ...
   */
    public Object clone() {
        try {
            Terminal result = new Terminal(getGPConfiguration(), getReturnType(), m_lowerBounds, m_upperBounds);
