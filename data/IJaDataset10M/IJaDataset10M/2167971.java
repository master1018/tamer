...
public class HTMLOutputEventHandler extends EventHandler {
    ...
    private void checkFolder(String fName) {
        ...
        if (!f.exists()) f.mkdir();
        if (!f.isDirectory()) Out.fatalException(new Exception(fName + " is not a directory!"));
    }
    ...
    public void recieveError(Connection source, String text) {
        append(source, text, cs.getErrorColor());
    }
    ...
    public void recieveInfo(Connection source, String text) {
        append(source, text, cs.getInfoColor());
    }
    ...
    public void recieveDebug(Connection source, String text) {
        append(source, text, cs.getDebugColor());
    }
    ...
    private String getIcon(int product, int icon, int flags) {
        ...
        if (icon != 0) return HexDump.DWordToPretty(icon);
        return HexDump.DWordToPretty(product);
    }

    private String getLagIcon(int flags, int ping) {
        if ((flags & 0x10) != 0) return "PLUG";
        if (ping < 0) return "LAG6";
        if (ping < 10) return "LAG0";
        if (ping < 200) return "LAG1";
        if (ping < 300) return "LAG2";
        if (ping < 400) return "LAG3";
        if (ping < 500) return "LAG4";
        if (ping < 600) return "LAG5";
        return "LAG6";
    }
    ...
    private void append2(Connection source, String text, Color col, String text2, Color col2) {
        ...
        out += getColor(col);
        out += text;
        if (!col.equals(col2)) out += getColor(col2);
        out += text2;
        logChat(source, out);
    }
    ...
    private void append3(Connection source, String text, Color col, String text2, Color col2, String text3, Color col3) {
        ...
        if (!col.equals(col2)) out += getColor(col2);
        out += text2;
        if (!col2.equals(col3)) out += getColor(col3);
        out += text3;
        logChat(source, out);
    }
    ...
    public void recieveChat(Connection source, BNetUser user, String text) {
        append2(source, "<" + user.toString() + "> ", cs.getUserNameColor(user.getFlags()), text, cs.getChatColor(user.getFlags()));
    }

    @Override
    public void recieveBroadcast(Connection source, String username, int flags, String text) {
        append3(source, "(Broadcast) ", cs.getTypeColor(), "<" + username + "> ", cs.getUserNameColor(flags), text, cs.getChatColor(flags));
    }

    @Override
    public void recieveEmote(Connection source, BNetUser user, String text) {
        append(source, "<" + user.toString() + " " + text + "> ", cs.getEmoteColor(user.getFlags()));
    }

    @Override
    public void whisperSent(Connection source, BNetUser user, String text) {
        append2(source, "<To: " + user.toString() + "> ", cs.getUserNameColor(user.getFlags()), text, cs.getWhisperColor(user.getFlags()));
    }

    @Override
    public void whisperRecieved(Connection source, BNetUser user, String text) {
        append2(source, "<From: " + user.toString() + "> ", cs.getUserNameColor(user.getFlags()), text, cs.getWhisperColor(user.getFlags()));
    }
}
