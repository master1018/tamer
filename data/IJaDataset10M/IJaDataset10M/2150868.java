...
public class Header {
    ...
    public Header(BitInputStream is, byte[] headerWarmup, StreamInfo streamInfo) throws IOException, BadHeaderException {
        ...
        ByteData rawHeader = new ByteData(16);
        ...
        switch(bsType) {
            case 0:
                if (!isKnownFixedBlockSizeStream) throw new BadHeaderException("Unknown Block Size (0)");
        ...
        switch(srType) {
            case 0:
                if (streamInfo == null) throw new BadHeaderException("Bad Sample Rate (0)");
        ...
        switch(bpsType) {
            case 0:
                if (streamInfo != null) bitsPerSample = streamInfo.getBitsPerSample(); else throw new BadHeaderException("Bad BPS (" + bpsType + ")");
                break;
            case 1:
                bitsPerSample = 8;
                break;
        ...
        if (sampleRateHint != 0) {
            ...
            if (sampleRateHint == 12) sampleRate = sampleRateCode * 1000; else if (sampleRateHint == 13) sampleRate = sampleRateCode; else sampleRate = sampleRateCode * 10;
        }
