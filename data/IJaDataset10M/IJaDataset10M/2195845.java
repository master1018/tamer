...
public class ShoppingSipServlet extends SipServlet implements TimerListener {
    ...
    protected void doProvisionalResponse(SipServletResponse sipServletResponse) throws ServletException, IOException {
        ...
        if (status == SipServletResponse.SC_SESSION_PROGRESS && "INVITE".equalsIgnoreCase(sipServletResponse.getMethod()) && (isHelpCall == null || Boolean.FALSE.equals(isHelpCall))) {
    ...
    protected void doHelpCallSuccessResponse(SipServletResponse resp) throws ServletException, IOException {
        logger.info("Got OK");
    ...
    protected void doErrorResponse(SipServletResponse response) throws ServletException, IOException {
        ...
        if (response.getStatus() == SipServletResponse.SC_UNAUTHORIZED || response.getStatus() == SipServletResponse.SC_PROXY_AUTHENTICATION_REQUIRED) {
            if (!"true".equals(response.getSession().getAttribute("FirstResponseRecieved"))) {
                ...
                authInfo.addAuthInfo(response.getStatus(), response.getChallengeRealms().next(), getServletContext().getInitParameter("caller.sip"), getServletContext().getInitParameter("caller.password"));
                SipServletRequest challengeRequest = response.getSession().createRequest(response.getRequest().getMethod());
                challengeRequest.addAuthHeader(response, authInfo);
                NetworkConnection connection = (NetworkConnection) sipSession.getAttribute("connection");
                if (connection != null) {
                    try {
    ...
    protected void doRegister(SipServletRequest req) throws ServletException, IOException {
        ...
        if (expires == 0) {
            ...
            logger.info("User " + fromURI + " registered this contact address " + address + " with an Expire time of " + expires);
        }
        resp.send();
    }
    ...
    public void timeout(ServletTimer timer) {
        ...
        try {
            ...
            SipServletRequest sipServletRequest = sipFactory.createRequest(sipApplicationSession, "INVITE", fromAddress, toAddress);
            if (attributes.get("adminApproval") != null) {
                ...
                sipServletRequest.getSession().setAttribute("speechUri", java.net.URI.create("data:" + URLEncoder.encode("ts(" + stringBuffer + ")", "UTF-8")));
            }
            ...
            while (attrIterator.hasNext()) {
                Map.Entry<java.lang.String, java.lang.Object> entry = (Map.Entry<java.lang.String, java.lang.Object>) attrIterator.next();
