...
public class Crypt {
    ...
    private static int byteMask[] = { 0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01 };
    ...
    public static void initDes() {
        synchronized (lock) {
            ...
            for (int i = 0; i < 4; i++) for (int j = 0; j < 256; j++) for (int k = 0; k < 2; k++) ePerm32Tab[i][j][k] = 0;
            for (int bit = 0; bit < 48; bit++) {
            ...
            for (int i = 0; i < 16; i++) for (int j = 0; j < 64; j++) for (int k = 0; k < 2; k++) efp[i][j][k] = 0;
    ...
    private static void setupSalt(byte[] s) {
        if ((s[0] == currentSalt[0]) && (s[1] == currentSalt[1])) return;
        currentSalt[0] = s[0];
        currentSalt[1] = s[1];
        int saltbits = 0;
        for (int i = 0; i < 2; i++) {
