...
public final class VFSFilePane extends JPanel implements PropertyChangeListener {
    ...
    class DetailsTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            if ((table.convertColumnIndexToModel(column) != COLUMN_FILENAME) || (listViewWindowsStyle && !table.isFocusOwner())) {
                isSelected = false;
            }
        ...
        public void mouseClicked(MouseEvent evt) {
            ...
            if (source instanceof JList) {
                ...
                if ((index >= 0) && (list != null) && listSelectionModel.isSelectedIndex(index)) {
                    Rectangle r = list.getCellBounds(index, index);
                    evt = new MouseEvent(list, evt.getID(), evt.getWhen(), evt.getModifiers(), r.x + 1, r.y + (r.height / 2), evt.getClickCount(), evt.isPopupTrigger(), evt.getButton());
                }
            } else {
                return;
            }
            ...
            if (col == COLUMN_FILENAME) {
                ...
                if (f != null) {
                    ...
                    if (!newDisplayName.equals(oldDisplayName)) {
                        ...
                        AbstractVFSFileSystemView fsv = chooser.getFileSystemView();
                        FileObject f2 = fsv.createFileObject(VFSUtils.getParentDirectory(f), newFileName);
                        if (!VFSUtils.exists(f2) && VFSFilePane.this.getModel().renameFile(f, f2)) {
                            if (fsv.isParent(chooser.getCurrentDirectory(), f2)) {
                                if (chooser.isMultiSelectionEnabled()) {
                                    chooser.setSelectedFiles(new FileObject[] { f2 });
                                } else {
