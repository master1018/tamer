...
public class IDQuoteCache {
    ...
    private class IDQuoteCacheQuote {
        ...
        public IDQuoteCacheQuote(long day_volume, float day_low, float day_high, float day_open, float day_close, float bid, float ask) {
        ...
        public IDQuote toQuote(Symbol symbol, TradingDate date, TradingTime time) {
            return new IDQuote(symbol, date, time, day_volume, (double) day_low, (double) day_high, (double) day_open, (double) day_close, (double) bid, (double) ask);
        }

        public boolean equals(long day_volume, float day_low, float day_high, float day_open, float day_close, float bid, float ask) {
            return (day_volume == this.day_volume && day_low == this.day_low && day_high == this.day_high && day_open == this.day_open && day_close == this.day_close && bid == this.bid && ask == this.ask);
        }
    }
    ...
    public static synchronized IDQuoteCache getInstance() {
        if (instance == null) instance = new IDQuoteCache();
        return instance;
    }
    ...
    public double getQuote(Symbol symbol, int quoteType, int timeOffset) throws QuoteNotLoadedException {
        ...
        if (quote != null) return quote.getQuote(quoteType); else throw QuoteNotLoadedException.getInstance();
    }
    ...
    public IDQuote getQuote(Symbol symbol, int timeOffset) throws QuoteNotLoadedException {
        IDQuoteCacheQuote quote = getQuoteCacheQuote(symbol, timeOffset);
        if (quote != null) return quote.toQuote(symbol, date, offsetToTime(timeOffset)); else throw QuoteNotLoadedException.getInstance();
    }
    ...
    private HashMap getQuotesForTime(int timeOffset) throws QuoteNotLoadedException {
        assert timeOffset >= 0;
        if (timeOffset >= cache.size()) throw QuoteNotLoadedException.getInstance();
        HashMap quotesForTime = (HashMap) cache.get(timeOffset);
        if (quotesForTime == null) throw QuoteNotLoadedException.getInstance();
    ...
    public void load(List quotes) {
        if (quotes.size() > 0) {
            ...
            for (Iterator iterator = quotes.iterator(); iterator.hasNext(); ) {
                ...
                map.put(quote.getSymbol(), new IDQuoteCacheQuote(quote.getDayVolume(), (float) quote.getDayLow(), (float) quote.getDayHigh(), (float) quote.getDayOpen(), (float) quote.getDayClose(), (float) quote.getBid(), (float) quote.getAsk()));
            }
            cache.add(map);
            times.add(time);
            fireQuotesAdded();
    ...
    public TradingTime offsetToTime(int timeOffset) {
        ...
        if (timeOffset < times.size()) time = (TradingTime) times.get(timeOffset);
        return time;
    }
    ...
    public int getFirstTimeOffset() {
        if (times.size() == 0) return -1; else return 0;
    }
    ...
    private TradingTime getNewestTime(List quotes) {
        ...
        for (Iterator iterator = quotes.iterator(); iterator.hasNext(); ) {
            IDQuote quote = (IDQuote) iterator.next();
            if (time == null || quote.getTime().compareTo(time) < 0) time = quote.getTime();
        }
        return time;
    }
}
