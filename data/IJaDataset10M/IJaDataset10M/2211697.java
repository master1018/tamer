...
import static com2.sun.tools.javac.code.BoundKind.*;
...
public class Type implements PrimitiveType {
    ...
    public static List<Type> baseTypes(List<Type> ts) {
        if (ts.nonEmpty()) {
            ...
            if (t != ts.head || baseTypes != ts.tail) return baseTypes.prepend(t);
        }
        return ts;
    }
     ...
     */
    public String toString() {
        String s = (tsym == null || tsym.name == null) ? "<none>" : tsym.name.toString();
    ...
    public static String toString(List<Type> ts) {
        if (ts.isEmpty()) {
            ...
            for (List<Type> l = ts.tail; l.nonEmpty(); l = l.tail) buf.append(",").append(l.head.toString());
            return buf.toString();
        }
    }
    ...
    public String stringValue() {
        ...
        if (tag == BOOLEAN) return ((Integer) constValue()).intValue() == 0 ? "false" : "true"; else if (tag == CHAR) return String.valueOf((char) ((Integer) constValue()).intValue()); else return constValue().toString();
    }
    ...
    public boolean isFalse() {
        return tag == BOOLEAN && constValue() != null && ((Integer) constValue()).intValue() == 0;
    }
    ...
    */
    ...
    public static boolean isErroneous(List<Type> ts) {
        for (List<Type> l = ts; l.nonEmpty(); l = l.tail) if (l.head.isErroneous()) return true;
        return false;
    }
    ...
    public boolean isCompound() {
        return tsym.completer == null && (tsym.flags() & COMPOUND) != 0;
    }
    ...
    public static boolean contains(List<Type> ts, Type t) {
        for (List<Type> l = ts; l.tail != null; l = l.tail) if (l.head.contains(t)) return true;
        return false;
    }
    ...
    public boolean containsSome(List<Type> ts) {
        for (List<Type> l = ts; l.nonEmpty(); l = l.tail) if (this.contains(ts.head)) return true;
        return false;
    }
    ...
    public <R, P> R accept(TypeVisitor<R, P> v, P p) {
        if (isPrimitive()) return v.visitPrimitive(this, p); else throw new AssertionError();
    }

    public static class WildcardType extends Type implements javax2.lang.model.type.WildcardType {
        ...
        public String toString() {
            ...
            if (kind != UNBOUND) s.append(type);
            if (moreInfo && bound != null && !isPrintingBound) try {
        ...
        public Type map(Mapping f) {
            Type t = type;
            if (t != null) t = f.apply(t);
            if (t == type) return this; else return new WildcardType(t, kind, tsym, bound);
        }
        ...
        public Type getExtendsBound() {
            if (kind == EXTENDS) return type; else return null;
        }

        public Type getSuperBound() {
            if (kind == SUPER) return type; else return null;
        }
    ...
    public static class ClassType extends Type implements DeclaredType {
        ...
        public List<Type> getTypeArguments() {
            if (typarams_field == null) {
                complete();
                if (typarams_field == null) typarams_field = List.nil();
            }
        ...
        public boolean isErroneous() {
            return getEnclosingType().isErroneous() || isErroneous(getTypeArguments()) || this != tsym.type && tsym.type.isErroneous();
        }
         ...
         */
        public boolean isRaw() {
            return this != tsym.type && tsym.type.allparams().nonEmpty() && allparams().isEmpty();
        }
        ...
        public Type map(Mapping f) {
            ...
            if (outer1 == outer && typarams1 == typarams) return this; else return new ClassType(outer1, typarams1, tsym);
        }

        public boolean contains(Type elem) {
            return elem == this || (isParameterized() && (getEnclosingType().contains(elem) || contains(getTypeArguments(), elem)));
        }
    ...
    public static class ArrayType extends Type implements javax2.lang.model.type.ArrayType {
        ...
        public Type map(Mapping f) {
            ...
            if (elemtype1 == elemtype) return this; else return new ArrayType(elemtype1, tsym);
        }
    ...
    public static class MethodType extends Type implements Cloneable, ExecutableType {
        ...
        public boolean equals(Object obj) {
            ...
            while (!args1.isEmpty() && !args2.isEmpty()) {
                if (!args1.head.equals(args2.head)) return false;
                args1 = args1.tail;
                args2 = args2.tail;
            }
            if (!args1.isEmpty() || !args2.isEmpty()) return false;
            return restype.equals(m.restype);
        }

        public int hashCode() {
            int h = METHOD;
            for (List<Type> thisargs = this.argtypes; thisargs.tail != null; thisargs = thisargs.tail) h = (h << 5) + thisargs.head.hashCode();
        ...
        public Type map(Mapping f) {
            ...
            if (argtypes1 == argtypes && restype1 == restype && thrown1 == thrown) return this; else return new MethodType(argtypes1, restype1, thrown1, tsym);
        }
        ...
        public void complete() {
            for (List<Type> l = argtypes; l.nonEmpty(); l = l.tail) l.head.complete();
            restype.complete();
            for (List<Type> l = thrown; l.nonEmpty(); l = l.tail) l.head.complete();
        }
    ...
    public static class CapturedType extends TypeVar {
        ...
        @Override
        public String toString() {
            return "capture#" + (hashCode() & 0xFFFFFFFFL) % PRIME + " of " + wildcard;
        }
    ...
    public static class UndetVar extends DelegatedType {
        ...
        public String toString() {
            if (inst != null) return inst.toString(); else return qtype + "?";
        }

        public Type baseType() {
            if (inst != null) return inst.baseType(); else return this;
        }
    }
    ...
    public static class ErrorType extends ClassType implements javax2.lang.model.type.ErrorType {
