...
public class TreeInfo {
    ...
    public static TreeInfo instance(Context context) {
        ...
        if (instance == null) instance = new TreeInfo(context);
        return instance;
    }
    ...
    public static JCMethodInvocation firstConstructorCall(JCTree tree) {
        ...
        while (stats.nonEmpty() && isSyntheticInit(stats.head)) stats = stats.tail;
    ...
    public static boolean isNull(JCTree tree) {
        if (tree.getTag() != JCTree.LITERAL) return false;
    ...
    public static int firstStatPos(JCTree tree) {
        if (tree.getTag() == JCTree.BLOCK && ((JCBlock) tree).stats.nonEmpty()) return ((JCBlock) tree).stats.head.pos; else return tree.pos;
    }
    ...
    public static int endPos(JCTree tree) {
        if (tree.getTag() == JCTree.BLOCK && ((JCBlock) tree).endpos != Position.NOPOS) return ((JCBlock) tree).endpos; else if (tree.getTag() == JCTree.SYNCHRONIZED) return endPos(((JCSynchronized) tree).body); else if (tree.getTag() == JCTree.TRY) {
            JCTry t = (JCTry) tree;
            ...
            case (JCTree.METHODDEF):
                {
                    ...
                    if (node.mods.pos != Position.NOPOS) return node.mods.pos;
                    if (node.typarams.nonEmpty()) return getStartPos(node.typarams.head);
            ...
            case (JCTree.ERRONEOUS):
                {
                    ...
                    if (node.errs != null && node.errs.nonEmpty()) return getStartPos(node.errs.head);
                }
        }
        return tree.pos;
    }
    ...
    public static int getEndPos(JCTree tree, Map<JCTree, Integer> endPositions) {
        ...
        switch(tree.getTag()) {
            ...
            case (JCTree.ERRONEOUS):
                {
                    JCErroneous node = (JCErroneous) tree;
                    if (node.errs != null && node.errs.nonEmpty()) return getEndPos(node.errs.last(), endPositions);
                }
        }
    ...
    public static JCTree declarationFor(final Symbol sym, final JCTree tree) {
        class DeclScanner extends TreeScanner {
            ...
            public void visitTopLevel(JCCompilationUnit that) {
                if (that.packge == sym) result = that; else super.visitTopLevel(that);
            }

            public void visitClassDef(JCClassDecl that) {
                if (that.sym == sym) result = that; else super.visitClassDef(that);
            }

            public void visitMethodDef(JCMethodDecl that) {
                if (that.sym == sym) result = that; else super.visitMethodDef(that);
            }

            public void visitVarDef(JCVariableDecl that) {
                if (that.sym == sym) result = that; else super.visitVarDef(that);
            }

            public void visitTypeParameter(JCTypeParameter that) {
                if (that.type != null && that.type.tsym == sym) result = that; else super.visitTypeParameter(that);
            }
        }
        ...
        class PathFinder extends TreeScanner {
            ...
            public void scan(JCTree tree) {
                if (tree != null) {
                    ...
                    if (tree == node) throw new Result(path);
                    super.scan(tree);
                    path = path.tail;
                }
            }
    ...
    public static JCTree referencedStatement(JCLabeledStatement tree) {
        JCTree t = tree;
        do t = ((JCLabeledStatement) t).body; while (t.getTag() == JCTree.LABELLED);
        switch(t.getTag()) {
            case JCTree.DOLOOP:
            case JCTree.WHILELOOP:
            case JCTree.FORLOOP:
    ...
    public static JCTree skipParens(JCTree tree) {
        if (tree.getTag() == JCTree.PARENS) return skipParens((JCParens) tree); else return tree;
    }
    ...
    public static long firstFlag(long flags) {
        int flag = 1;
        while ((flag & StandardFlags) != 0 && (flag & flags) == 0) flag = flag << 1;
        return flag;
    }
     ...
     */
    public static final int notExpression = -1, noPrec = 0, assignPrec = 1, assignopPrec = 2, condPrec = 3, orPrec = 4, andPrec = 5, bitorPrec = 6, bitxorPrec = 7, bitandPrec = 8, eqPrec = 9, ordPrec = 10, shiftPrec = 11, addPrec = 12, mulPrec = 13, prefixPrec = 14, postfixPrec = 15, precCount = 16;
