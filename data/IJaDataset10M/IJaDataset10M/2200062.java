...
public class CounterProvider {
    ...
    private static final String STATEMENT_COUNTER_WITHOUT_POINT = "%nADD 1 TO " + COUNT_STATEMENT + "%n";

    private static final String STATEMENT_COUNTER = "    10 " + COUNT_STATEMENT + "-TXT PIC X(%d) VALUE \"S-%d \".%n" + "    10 " + COUNT_STATEMENT + " PIC 9(18) VALUE ZERO.%n" + NEW_LINE_IN_COVERAGE_FILE;

    private static final String COUNT_BRANCH = "COUNT-BRANCH-%d";

    private static final String BRANCH_COUNTER_WITHOUT_POINT = "%nADD 1 TO " + COUNT_BRANCH + "%n";

    private static final String BRANCH_COUNTER = "    10 " + COUNT_BRANCH + "-TXT PIC X(%d) VALUE \"B-%d \".%n" + "    10 " + COUNT_BRANCH + " PIC 9(18) VALUE ZERO.%n" + NEW_LINE_IN_COVERAGE_FILE;

    private static final String COUNT_CONDITON = "COUNT-CONDITON-%d-%d";

    private static final String CONDITION_COUNTER_WITHOUT_POINT = "%nADD 1 TO " + COUNT_CONDITON + "%n";

    private static final String CONDITION_COUNTER_IF = "IF %s THEN %n %s ELSE %s END-IF %n";

    private static final String CONDITION_COUNTER = "    10 " + COUNT_CONDITON + "-TXT PIC X(%d) VALUE \"C-%d-%d \".%n" + "    10 " + COUNT_CONDITON + " PIC 9(18) VALUE ZERO.%n" + NEW_LINE_IN_COVERAGE_FILE;
    ...
    private Integer getTruthTableLine(int variables, int line) {
        ...
        for (int position = 0; position < variables; position++) {
            truthTableLineAdapted = truthTableLineAdapted + ONE + truthTableLine.charAt(position);
        }
    ...
    public String newBranchCounter() {
        this.branchCounter++;
        return String.format(BRANCH_COUNTER_WITHOUT_POINT, new Integer(this.branchCounter));
    }
    ...
    public String newStatementCounter() {
        ...
        return String.format(STATEMENT_COUNTER_WITHOUT_POINT, new Integer(this.statementCounter));
    }
    ...
    public void generateBranchCounter(ProgramUnit programUnit, PrintWriter printWriter) {
        ...
        for (int i = begin + 1; i <= end; i++) {
            ...
            Integer outIntegerLentgh = new Integer(2 + outInteger.toString().length() + 1);
            printWriter.printf(BRANCH_COUNTER, outInteger, outIntegerLentgh, outInteger, outInteger);
        }
    }
    ...
    public void generateConditionCounter(ProgramUnit programUnit, PrintWriter printWriter) {
        ...
        for (int i = begin + 1; i <= end; i++) {
            Integer outInteger = new Integer(i);
            Double basicBooleanTerms = new Double(nodeCounter.getBasicBooleanCounter(i));
            double basicBooleanCounters = Math.pow(2, basicBooleanTerms.doubleValue());
            Integer outIntegerLentgh = new Integer(2 + outInteger.toString().length() + 1 + 2 * basicBooleanTerms.intValue() + 1);
            for (int j = 0; j < basicBooleanCounters; j++) {
                Integer basicBooleanCounter = this.getTruthTableLine(basicBooleanTerms.intValue(), j);
                printWriter.printf(CONDITION_COUNTER, outInteger, basicBooleanCounter, outIntegerLentgh, outInteger, basicBooleanCounter, outInteger, basicBooleanCounter);
            }
        }
    }
    ...
    public void generateStatementCounter(ProgramUnit programUnit, PrintWriter printWriter) {
        ...
        for (int i = begin + 1; i <= end; i++) {
            Integer outInteger = new Integer(i);
            Integer outIntegerLentgh = new Integer(2 + outInteger.toString().length() + 1);
            printWriter.printf(STATEMENT_COUNTER, outInteger, outIntegerLentgh, outInteger, outInteger);
        }
    }
}
