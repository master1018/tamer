...
public class ConnectionEndpointLocator implements Locator {
    ...
    private static RectD2D figureBounds;
 ...
 */
    ...
    private int calculateConnectionLocation(Point loc, Point topLeft, Point center) {
        ...
        if (loc.x - center.x != 0) m2 = (double) (loc.y - center.y) / (double) (loc.x - center.x);
        if (loc.x == center.x) {
            if (loc.y < center.y) return 3; else return 1;
        } else if (Math.abs(m2) <= Math.abs(m1)) {
            if (loc.x < center.x) return 4; else return 2;
        } else {
            if (loc.y < center.y) return 3; else return 1;
        }
    }
    ...
    private int calculateConnectionLocation(Point startPoint, Point endPoint) {
        if (Math.abs(endPoint.x - startPoint.x) > Math.abs(endPoint.y - startPoint.y)) {
            if (endPoint.x > startPoint.x) return 2; else return 4;
        } else {
            if (endPoint.y > startPoint.y) return 1; else return 3;
        }
    }

    private double calculateTan(Point startPoint, Point endPoint) {
        double tan = 0;
        if (endPoint.x == startPoint.x) tan = 1.0; else tan = (double) (endPoint.y - startPoint.y) / (double) (endPoint.x - startPoint.x);
        if (tan > 1) tan = 1.0; else if (tan < -1) tan = -1.0;
        return tan;
    }

    private int calculateYShift(int figureWidth, int figureHeight) {
        int yShift = 0;
        if (vDistance < 0) yShift = -figureHeight; else if (vDistance == 0) yShift = -figureHeight / 2;
        return yShift;
    }
    ...
    private IFigure getConnectionOwner() {
        IFigure connOwner;
        if (isEnd()) connOwner = conn.getTargetAnchor().getOwner(); else connOwner = conn.getSourceAnchor().getOwner();
        return connOwner;
    }
 ...
 */
    public void relocate(IFigure figure) {
        ...
        int quadrant;
        if (connOwner != null) {
            RectD2D connOwnerBounds = connOwner.getBounds();
        ...
        } else quadrant = calculateConnectionLocation(startPoint, endPoint);
        int cos = 1;
        transposer.setEnabled(false);
        if (quadrant == 1 || quadrant == 3) transposer.setEnabled(true);
        if (quadrant == 3 || quadrant == 4) cos = -1;
