   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (outputString.length() != 0) {
            ...
            if (!validExt) {
                if (outputString.lastIndexOf('.') != -1) outputString = (outputString.substring(0, outputString.lastIndexOf('.'))) + FILE_EXTENSION; else outputString = outputString + FILE_EXTENSION;
            }
            try {
    ...
    public void cancel() {
        if (getWriteMode() == CANCEL) {
            if (m_outputFile != null && m_outputFile.exists()) {
                if (m_outputFile.delete()) System.out.println("File deleted.");
            }
            resetOptions();
        }
    }
        ...
        if (m_outputFile != null) {
            try {
                ...
                if (out.lastIndexOf(File.separatorChar) == -1) {
                    ...
                    if (dir.exists()) success = file.createNewFile(); else {
                        dir.mkdirs();
                        success = file.createNewFile();
                    }
                }
