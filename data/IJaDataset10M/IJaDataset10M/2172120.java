...
public class StdBanker extends StdShopKeeper implements Banker {
    ...
    public void setWhatIsSoldMask(long newSellCode) {
        ...
        if (!isSold(ShopKeeper.DEAL_CLANBANKER)) whatIsSoldMask = ShopKeeper.DEAL_BANKER; else whatIsSoldMask = ShopKeeper.DEAL_CLANBANKER;
    }
    ...
    public void addDepositInventory(MOB mob, Item thisThang) {
        ...
        } else addDepositInventory(mob.Name(), thisThang);
    }

    public boolean delDepositInventory(MOB mob, Item thisThang) {
        if (isSold(ShopKeeper.DEAL_CLANBANKER)) {
            if (mob.getClanID().length() > 0) return delDepositInventory(mob.getClanID(), thisThang);
        } else return delDepositInventory(mob.Name(), thisThang);
        return false;
    }
    ...
    protected Item makeItem(String data) {
        ...
        if (x < 0) return null;
        Item I = null;
        if (data.substring(0, x).equals("COINS")) I = CMClass.getItem("StdCoins"); else I = CMClass.getItem(data.substring(0, x));
        if (I != null) {
            CMLib.coffeeMaker().setPropertiesStr(I, data.substring(x + 1), true);
            if ((I instanceof Coins) && (((Coins) I).getDenomination() == 0.0) && (((Coins) I).getNumberOfCoins() > 0)) ((Coins) I).setDenomination(1.0);
            I.recoverEnvStats();
            I.text();
            return I;
        }
    ...
    public void bankLedger(MOB mob, String msg) {
        ...
        } else CMLib.beanCounter().bankLedger(bankChain(), mob.Name(), date + ": " + msg);
    }
    ...
    public Item findDepositInventory(String mob, String likeThis) {
        Vector V = getRawPDDepositInventory(mob);
        if (CMath.s_int(likeThis) > 0) for (int v = 0; v < V.size(); v++) {
            DatabaseEngine.PlayerData PD = (DatabaseEngine.PlayerData) V.elementAt(v);
            if (PD.xml.startsWith("COINS;")) return makeItem(PD.xml);
        } else for (int v = 0; v < V.size(); v++) {
            DatabaseEngine.PlayerData PD = (DatabaseEngine.PlayerData) V.elementAt(v);
            Item I = makeItem(PD.xml);
            if (I == null) continue;
            if (CMLib.english().containsString(I.Name(), likeThis)) return I;
        }
        return null;
    }

    public long timeInterval() {
        return (location().getArea().getTimeObj().getHoursInDay()) * Tickable.TIME_MILIS_PER_MUDHOUR * location().getArea().getTimeObj().getDaysInMonth();
    }
    ...
    public MoneyLibrary.DebtItem getDebtInfo(MOB mob) {
        ...
        for (int d = 0; d < debt.size(); d++) if (debt.elementAt(d).debtor.equalsIgnoreCase(whom)) return debt.elementAt(d);
        return null;
    }
        ...
        try {
            if (tickID == Tickable.TICKID_MOB) {
                ...
                synchronized (bankChain().intern()) {
                    ...
                    long timeInterval = 1;
                    if (((L == null) || (L.longValue() < System.currentTimeMillis())) && (location != null) && (location.getArea() != null) && (location.getArea().getTimeObj() != null) && (CMLib.flags().isInTheGame(this, true))) {
                    ...
                    if (proceed) {
                        ...
                        for (int v = 0; v < bankDataV.size(); v++) {
                            ...
                            if (!userNames.contains(name)) {
                                if (!CMLib.players().playerExists(name)) {
                                    if ((CMLib.clans().getClan(name)) == null) delAllDeposits(name); else userNames.addElement(name);
                                } else userNames.addElement(name);
                            }
                        }
                        ...
                        for (int u = 0; u < userNames.size(); u++) {
                            ...
                            for (int v = 0; v < items.size(); v++) {
                                ...
                                if (I instanceof Coins) coinItem = (Coins) I; else if (itemInterest != 0.0) totalValue += I.value();
                            }
                            double newBalance = 0.0;
                            if (coinItem != null) newBalance = coinItem.getTotalValue();
                            newBalance += CMath.mul(newBalance, coinInterest);
                            if (totalValue > 0) newBalance += CMath.mul(totalValue, itemInterest);
                            for (int d = debts.size() - 1; d >= 0; d--) {
                                ...
                                if (debtor.equalsIgnoreCase(name)) {
                                    ...
                                    if ((timeRemaining < 0) && (newBalance < ((dueAmount) + intDue))) newBalance = -1.0; else {
                                        ...
                                        } else CMLib.beanCounter().adjustDebt(debtor, bankChain(), intDue, reason, intRate, debtDueAt);
                                    }
                            ...
                            if (newBalance < 0) {
                                for (int v = 0; v < items.size(); v++) {
                                    ...
                                    if (!(I instanceof Coins)) getShop().addStoreInventory(I);
                                }
                                delAllDeposits(name);
                                CMLib.beanCounter().delAllDebt(name, bankChain());
                            } else if ((coinItem == null) || (newBalance != coinItem.getTotalValue())) {
                                if (coinItem != null) {
                                    if (newBalance > coinItem.getTotalValue()) CMLib.beanCounter().bankLedger(bankChain(), name, CMLib.utensils().getFormattedDate(this) + ": Deposit of " + CMLib.beanCounter().nameCurrencyShort(this, newBalance - coinItem.getTotalValue()) + ": Interest paid."); else CMLib.beanCounter().bankLedger(bankChain(), name, CMLib.utensils().getFormattedDate(this) + ": Withdrawl of " + CMLib.beanCounter().nameCurrencyShort(this, coinItem.getTotalValue() - newBalance) + ": Interest charged.");
                                ...
                                if (coinItem != null) addDepositInventory(name, coinItem);
                            }
                        }
                        for (int d = debts.size() - 1; d >= 0; d--) CMLib.beanCounter().delAllDebt(debts.elementAt(d).debtor, bankChain());
                    }
                }
            }
        } catch (Exception e) {
    ...
    public double getBalance(MOB mob) {
        ...
        if ((old != null) && (old instanceof Coins)) return ((Coins) old).getTotalValue();
        return 0;
    }

    public double totalItemsWorth(MOB mob) {
        Vector V = null;
        if (isSold(ShopKeeper.DEAL_CLANBANKER)) V = getDepositedItems(mob.getClanID()); else V = getDepositedItems(mob.Name());
        double min = 0;
        for (int v = 0; v < V.size(); v++) {
            Item I = (Item) V.elementAt(v);
            if (!(I instanceof Coins)) min += I.value();
        }
        return min;
    }
    ...
    public void executeMsg(Environmental myHost, CMMsg msg) {
            ...
            switch(msg.targetMinor()) {
                ...
                case CMMsg.TYP_DEPOSIT:
                    if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
                        if (msg.tool() instanceof Container) ((Container) msg.tool()).emptyPlease();
                        ...
                        if (msg.tool() instanceof Coins) {
                            ...
                            if ((old == null) && (!isSold(ShopKeeper.DEAL_CLANBANKER)) && (msg.source().isMarriedToLiege())) {
                            ...
                            if ((old != null) && (old instanceof Coins)) newValue += ((Coins) old).getTotalValue();
                            ...
                            if (item != null) addDepositInventory(owner, item);
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) CMLib.commands().postSay(this, mob, "Ok, Clan " + owner.getClanID() + " now has a balance of " + CMLib.beanCounter().nameCurrencyLong(this, getBalance(owner)) + ".", true, false); else CMLib.commands().postSay(this, mob, "Ok, your new balance is " + CMLib.beanCounter().nameCurrencyLong(this, getBalance(owner)) + ".", true, false);
                            recoverEnvStats();
                ...
                case CMMsg.TYP_BORROW:
                    if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
                        ...
                        if (old instanceof Coins) {
                            ...
                            } else CMLib.commands().postDrop(this, old, true, false);
                            ...
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) CMLib.beanCounter().adjustDebt(msg.source().getClanID(), bankChain(), amt, "Bank Loan", interestRate, dueAt); else CMLib.beanCounter().adjustDebt(msg.source().Name(), bankChain(), amt, "Bank Loan", interestRate, dueAt);
                        }
                    }
                    break;
                case CMMsg.TYP_WITHDRAW:
                    if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
                        ...
                        if (old instanceof Coins) {
                            ...
                            MOB owner = msg.source();
                            if ((!isSold(ShopKeeper.DEAL_CLANBANKER)) && (msg.source().isMarriedToLiege()) && ((depositInventoryItem == null) || ((depositInventoryItem instanceof Coins) && (((Coins) depositInventoryItem).getTotalValue() < ((Coins) old).getTotalValue())))) {
                            ...
                            if ((depositInventoryItem != null) && (depositInventoryItem instanceof Coins) && (old instanceof Coins) && (((Coins) depositInventoryItem).getTotalValue() >= ((Coins) old).getTotalValue())) {
                                ...
                                if ((coins == null) || (coins.getNumberOfCoins() <= 0)) {
                                    if (isSold(ShopKeeper.DEAL_CLANBANKER)) CMLib.commands().postSay(this, mob, "I have closed the account for Clan " + owner.getClanID() + ". Thanks for your business.", true, false); else CMLib.commands().postSay(this, mob, "I have closed that account. Thanks for your business.", true, false);
                                    return;
                                }
                                addDepositInventory(owner, coins);
                                if (isSold(ShopKeeper.DEAL_CLANBANKER)) CMLib.commands().postSay(this, mob, "Ok, Clan " + owner.getClanID() + " now has a balance of " + CMLib.beanCounter().nameCurrencyLong(this, coins.getTotalValue()) + ".", true, false); else CMLib.commands().postSay(this, mob, "Ok, your new balance is " + CMLib.beanCounter().nameCurrencyLong(this, coins.getTotalValue()) + ".", true, false);
                            } else if (depositInventoryItem != null) CMLib.commands().postSay(this, mob, "But, your balance is " + CMLib.beanCounter().nameCurrencyLong(this, ((Coins) depositInventoryItem).getTotalValue()) + ".", true, false);
                        } else {
                            if ((!delDepositInventory(msg.source(), old)) && (!isSold(ShopKeeper.DEAL_CLANBANKER)) && (msg.source().isMarriedToLiege())) delDepositInventory(msg.source().getLiegeID(), old);
                            CMLib.commands().postSay(this, mob, "Thank you for your trust.", true, false);
                            if (location() != null) location().addItemRefuse(old, CMProps.getIntVar(CMProps.SYSTEMI_EXPIRE_PLAYER_DROP));
                            CMMsg msg2 = CMClass.getMsg(mob, old, this, CMMsg.MSG_GET, null);
                            if (location().okMessage(mob, msg2)) location().send(mob, msg2);
                        }
                    }
                    return;
                ...
                case CMMsg.TYP_LIST:
                    {
                        ...
                        if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
                            Vector V = null;
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) V = getDepositedItems(mob.getClanID()); else {
                                V = getDepositedItems(mob.Name());
                                if (mob.isMarriedToLiege()) {
                                    Vector V2 = getDepositedItems(mob.getLiegeID());
                                    if ((V2 != null) && (V2.size() > 0)) CMParms.addToVector(V2, V);
                                }
                            }
                            for (int v = V.size() - 1; v >= 0; v--) if (V.elementAt(v) instanceof LandTitle) {
                            ...
                            if (!otherThanCoins) str = new StringBuffer("\n\r^N"); else str.append("\n\r\n\r");
                            double balance = getBalance(mob);
                            if (balance > 0) {
                                if (isSold(ShopKeeper.DEAL_CLANBANKER)) str.append("Clan " + mob.getClanID() + " has a balance of ^H" + CMLib.beanCounter().nameCurrencyLong(this, balance) + "^?."); else str.append("Your balance is ^H" + CMLib.beanCounter().nameCurrencyLong(this, balance) + "^?.");
                            }
                            Vector<MoneyLibrary.DebtItem> debts = null;
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) debts = CMLib.beanCounter().getDebt(mob.getClanID(), bankChain()); else debts = CMLib.beanCounter().getDebt(mob.Name(), bankChain());
                            if (debts != null) for (int d = 0; d < debts.size(); d++) {
                                MoneyLibrary.DebtItem debt = debts.elementAt(d);
                                long debtDueAt = debt.due;
                                double intRate = debt.interest;
                                double dueAmount = debt.amt;
                                long timeRemaining = debtDueAt - System.currentTimeMillis();
                                if (timeRemaining > 0) str.append("\n\r" + ((isSold(ShopKeeper.DEAL_CLANBANKER)) ? "Clan " + mob.getClanID() : "You") + " owe ^H" + CMLib.beanCounter().nameCurrencyLong(this, dueAmount) + "^? in debt.\n\r" + "Monthly interest is " + (intRate * 100.0) + "%.  " + "The loan must be paid in full in " + (timeRemaining / timeInterval()) + " months.");
                            }
                            if (coinInterest != 0.0) {
        ...
        } else if (msg.sourceMinor() == CMMsg.TYP_RETIRE) delAllDeposits(msg.source().Name());
        super.executeMsg(myHost, msg);
    }
    ...
    public boolean okMessage(Environmental myHost, CMMsg msg) {
        ...
        if (msg.amITarget(this)) {
            switch(msg.targetMinor()) {
                ...
                case CMMsg.TYP_DEPOSIT:
                    {
                        if (!CMLib.coffeeShops().ignoreIfNecessary(msg.source(), finalIgnoreMask(), this)) return false;
                        if (msg.tool() == null) return false;
                        double balance = getBalance(mob);
                        if ((isSold(ShopKeeper.DEAL_CLANBANKER)) && ((msg.source().getClanID().length() == 0) || (CMLib.clans().getClan(msg.source().getClanID()) == null))) {
                        ...
                        if (balance < minbalance) {
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) CMLib.commands().postSay(this, mob, "Clan " + msg.source().getClanID() + " will need a total balance of " + CMLib.beanCounter().nameCurrencyShort(this, minbalance) + " for me to hold that.", true, false); else CMLib.commands().postSay(this, mob, "You'll need a total balance of " + CMLib.beanCounter().nameCurrencyShort(this, minbalance) + " for me to hold that.", true, false);
                            return false;
                        }
                    }
                    return true;
                case CMMsg.TYP_WITHDRAW:
                    {
                        ...
                        if (isSold(ShopKeeper.DEAL_CLANBANKER)) {
                            ...
                            if ((msg.source().getClanID().length() == 0) || (C == null)) {
                                CMLib.commands().postSay(this, mob, "I'm sorry, I only do business with Clans, and you aren't part of one.", true, false);
                                return false;
                            }
                        ...
                        if ((msg.tool() != null) && (!msg.tool().okMessage(myHost, msg))) return false;
                        MOB owner = msg.source();
                        ...
                        if (msg.tool() instanceof Coins) {
                            ...
                            if ((!isSold(ShopKeeper.DEAL_CLANBANKER)) && (owner.isMarriedToLiege()) && (balance < ((Coins) msg.tool()).getTotalValue())) {
                        ...
                        } else if (findDepositInventory(thename, msg.tool().Name()) == null) {
                            if ((!isSold(ShopKeeper.DEAL_CLANBANKER)) && (msg.source().isMarriedToLiege()) && (findDepositInventory(msg.source().getLiegeID(), msg.tool().Name()) != null)) owner = CMLib.players().getPlayer(msg.source().getLiegeID()); else {
                                CMLib.commands().postSay(this, mob, "You want WHAT?", true, false);
                                return false;
                            }
                        } else if (msg.tool() instanceof Item) {
                            double debt = (isSold(ShopKeeper.DEAL_CLANBANKER)) ? CMLib.beanCounter().getDebtOwed(mob.getClanID(), bankChain()) : CMLib.beanCounter().getDebtOwed(mob.Name(), bankChain());
                            if ((debt > 0.0) && ((collateral - ((Item) msg.tool()).value()) < debt)) {
                        ...
                        if (msg.tool() instanceof Coins) {
                            if (((Coins) msg.tool()).getTotalValue() > balance) {
                                if (isSold(ShopKeeper.DEAL_CLANBANKER)) CMLib.commands().postSay(this, mob, "I'm sorry, Clan " + thename + " has only " + CMLib.beanCounter().nameCurrencyShort(this, balance) + " in its account.", true, false); else CMLib.commands().postSay(this, mob, "I'm sorry, you have only " + CMLib.beanCounter().nameCurrencyShort(this, balance) + " in that account.", true, false);
                                return false;
                            }
                            if (minbalance == 0) return true;
                            if (((Coins) msg.tool()).getTotalValue() > (balance - minbalance)) {
                                if ((balance - minbalance) > 0) CMLib.commands().postSay(this, mob, "I'm sorry, you may only withdraw " + CMLib.beanCounter().nameCurrencyShort(this, balance - minbalance) + "  at this time.", true, false); else CMLib.commands().postSay(this, mob, "I am holding other items in trust, so you may not withdraw funds at this time.", true, false);
                                return false;
                            }
                        }
                    }
                ...
                case CMMsg.TYP_BORROW:
                    {
                        ...
                        } else if ((numberDeposited(thename) == 0) && ((isSold(ShopKeeper.DEAL_CLANBANKER)) || (!msg.source().isMarriedToLiege()) || (numberDeposited(msg.source().getLiegeID()) == 0))) {
                            StringBuffer str = new StringBuffer("");
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) str.append("The Clan " + thename + " does not have an account with us, I'm afraid."); else str.append("You don't have an account with us, I'm afraid.");
                        ...
                        if (debt > 0.0) {
                            StringBuffer str = new StringBuffer("");
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) str.append("The Clan " + thename + " already has a " + CMLib.beanCounter().nameCurrencyShort(this, debt) + " loan out with us."); else str.append("You already have a " + CMLib.beanCounter().nameCurrencyShort(this, debt) + " loan out with us.");
                            CMLib.commands().postSay(this, mob, str.toString() + "^T", true, false);
                            return false;
                        }
                        ...
                        if (collateralRemaining > 0) {
                            StringBuffer str = new StringBuffer("");
                            if (isSold(ShopKeeper.DEAL_CLANBANKER)) str.append("The Clan " + thename + " "); else str.append("You ");
                            str.append("will need to deposit enough items with us as collateral.  You'll need items worth " + CMLib.beanCounter().nameCurrencyShort(this, collateralRemaining) + " more to qualify.");
                            CMLib.commands().postSay(this, mob, str.toString() + "^T", true, false);
                            return false;
                        }
                        return true;
                ...
                case CMMsg.TYP_LIST:
                    {
                        ...
                        if (isSold(ShopKeeper.DEAL_CLANBANKER)) {
                            ...
                            if (bankChain().length() > 0) str.append("\n\rI am a banker for " + bankChain() + ".");
                            CMLib.commands().postSay(this, mob, str.toString() + "^T", true, false);
                            return false;
                        } else return true;
                    }
