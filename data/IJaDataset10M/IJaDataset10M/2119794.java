...
public class TestDatabase {
    ...
    private static void deleteServerFiles() {
        final FilenameFilter serverFilesFilter = new FilenameFilter() {
            ...
            public boolean accept(final File dir, final String name) {
                return Arrays.asList(serverFileStem + ".lck", serverFileStem + ".log", serverFileStem + ".lobs", serverFileStem + ".script", serverFileStem + ".properties").contains(name);
            }
        };
   ...
   *        Schema names
   */
    private static void setupSchema(final DatabaseConnectionOptions dataSource, final String... schemas) {
        ...
        try {
            if (dataSource != null) {
                ...
                for (final String schema : schemas) {
                    for (final String scriptType : new String[] { "pre_schema", "schema", "post_schema", "data" }) {
                        final String scriptResource = String.format("/testdatabase/%s.%s.sql", schema, scriptType).toLowerCase(Locale.ENGLISH);
    ...
    public TestDatabase() {
        schemas = new String[] { "books", "publisher sales", "for_lint" };
    }
   ...
   */
    ...
    public Database getDatabase(final SchemaCrawlerOptions schemaCrawlerOptions) throws SchemaCrawlerException {
        final Database database = SchemaCrawlerUtility.getDatabase(getConnection(), schemaCrawlerOptions);
        return database;
    }
    ...
    public Schema getSchema(final SchemaCrawlerOptions schemaCrawlerOptions, final String schemaName) throws SchemaCrawlerException, SQLException {
   ...
   */
    public void startDatabase(final boolean silent) throws SchemaCrawlerException {
        ...
        deleteServerFiles();
        org.hsqldb.server.Server.main(new String[] { "-database.0", serverFileStem, "-dbname.0", "schemacrawler", "-silent", Boolean.toString(silent), "-trace", Boolean.toString(!silent), "-no_system_exit", "true" });
   ...
   */
    ...
    private void createDatabase(final String url) throws SchemaCrawlerException {
        makeDataSource(url);
        setupSchema(connectionOptions, schemas);
    }

    private void makeDataSource(final String url) throws SchemaCrawlerException {
        connectionOptions = new DatabaseConnectionOptions("org.hsqldb.jdbc.JDBCDriver", url);
