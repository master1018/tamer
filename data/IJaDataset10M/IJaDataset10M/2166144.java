...
public class HelpLoader {
    ...
    private void init() {
        ...
        if (home != null) {
            ...
            initFile(local);
            try {
                local = new File(new File(home.getPath()).getParentFile().getParentFile(), helproot + "/" + language);
                System.out.println("local: " + local.getAbsolutePath());
                initFile(local);
            } catch (Exception e) {
    ...
    private void initFile(File local) {
        ...
        try {
            System.out.println("+ " + String.valueOf(counter) + "\thelp text(s) from:\t" + local.getCanonicalPath());
        } catch (IOException e) {
        }
    }
        ...
        if (file.getName().toLowerCase().endsWith(".jar") && file.isFile()) {
            try {
                ...
                while (je != null) {
                    ...
                    if (je.getName().toLowerCase().matches(helproot + "/" + language + "/.*.htm") && !exists(mnemo)) {
                ...
                jis.close();
                System.out.println("+ " + String.valueOf(counter) + "\thelp text(s) from:\t" + file.getCanonicalPath());
