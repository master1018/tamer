...
public class Automaton {
    ...
    protected void init(AutomatonBuildHelp help, int stateFinalityType, int stateNumberOfTransitionsType) {
        ...
        if (alphabetLength <= Byte.MAX_VALUE) {
            transitionsLabel = new GenericWholeArrray(GenericWholeArrray.TYPE_BYTE, help.transitionsAlloced);
        } else if (alphabetLength <= Short.MAX_VALUE) {
            transitionsLabel = new GenericWholeArrray(GenericWholeArrray.TYPE_SHORT, help.transitionsAlloced);
        } else {
            transitionsLabel = new GenericWholeArrray(GenericWholeArrray.TYPE_INT, help.transitionsAlloced);
        }
        transitionsTo = new int[help.transitionsAlloced];
        stateTransitions = new int[help.statesAlloced];
        stateFinalities = new GenericWholeArrray(stateFinalityType, help.statesAlloced);
        stateNumberOfTransitions = new GenericWholeArrray(stateNumberOfTransitionsType, help.statesAlloced);
    ...
    public void addState(AutomatonBuildHelp help, int state) {
        if (statesStored <= state) {
            if (help.statesAlloced <= state) {
                ...
                stateTransitions = GenericWholeArrray.realloc(stateTransitions, mem, statesStored);
    ...
    public void addTransition(AutomatonBuildHelp help, int stateFrom, int label, int stateTo) {
        ...
        if (transitionsStored == help.transitionsAlloced) {
            ...
            transitionsFrom = GenericWholeArrray.realloc(transitionsFrom, mem, transitionsStored);
            transitionsLabel.realloc(mem, transitionsStored);
            transitionsTo = GenericWholeArrray.realloc(transitionsTo, mem, transitionsStored);
        ...
        stateNumberOfTransitions.setElement(stateFrom, stateNumberOfTransitions.elementAt(stateFrom) + 1);
        transitionsStored++;
        if (label == EPSILON) {
        ...
        for (i = 1; i < transitionsStored; i++) {
            if (trCmp(j - 1, i) != 0) {
                ...
                if (transitionsFrom[j] == q) {
                    stateNumberOfTransitions.setElement(q, stateNumberOfTransitions.elementAt(q) + 1);
                } else {
                    q = transitionsFrom[j];
    ...
    protected Automaton removeEpsilonTransitions(StatePDA[] finalities) {
        ...
        Automaton result = new Automaton(help, stateFinalities.getType(), stateNumberOfTransitions.getType());
        ...
        for (i = 0; i < statesStored; i++) {
            ...
            for (; j < n; j++) {
                result.addTransition(help, i, transitionsLabel.elementAt(tr + j), transitionsTo[tr + j]);
            }
            if (ec.closure(i) == 0) {
                continue;
            }
            for (k = ec.closure(i); k != Constants.NO; k = ec.next.seq[k]) {
                ...
                if (newsf != Constants.NO && (sf == Constants.NO || finalities[newsf].precedes(finalities[sf]))) {
    ...
    public Automaton determinize(StatePDA[] finalities) {
        ...
        if (initialStates.seqStored == 0) {
            return new Automaton(bHelp, stateFinalities.getType(), stateNumberOfTransitions.getType());
        }
        Automaton a;
        if (hasEpsilonTransitions) {
        ...
        Automaton result = new Automaton(bHelp, stateFinalities.getType(), stateNumberOfTransitions.getType());
        ...
        while (!dHelp.queueIsEmpty()) {
            ...
            result.stateFinalities.setElement(set, a.computeSetFinality(dHelp.states.seq, dHelp.sets.seq[set], finalities));
    ...
    protected AutomatonMinimizationHelp hopcroftMinimize(int labelsStored) {
        ...
        while (mHelp.firstClass != Constants.NO) {
            ...
            for (state = mHelp.classesFirstState[q1]; state != Constants.NO; state = mHelp.statesNext[state]) {
                for (tr = getNextTransition(state, a, Constants.NO); tr != Constants.NO; tr = getNextTransition(state, a, tr)) {
            ...
            for (i = 0; i < classes.seqStored; i++) {
                ...
                if (mHelp.classesNewPower[q0] != mHelp.classesPower[q0]) {
                    ...
                    for (j = mHelp.classesFirstLetter[q0]; j != Constants.NO; j = mHelp.lettersNext[j]) {
                        mHelp.addLetter(mHelp.classesNewClass[q0], mHelp.lettersLetter[j]);
    ...
    public Automaton minimize() {
        ...
        for (i = 0; i < mHelp.classesStored; i++) {
            bHelp.transitionsAlloced += stateNumberOfTransitions.elementAt(mHelp.classesFirstState[i]);
        }
        Automaton result = new Automaton(bHelp, stateFinalities.getType(), stateNumberOfTransitions.getType());
        ...
        for (i = 0; i < mHelp.classesStored; i++) {
            ...
            for (j = 0; j < n; j++) {
                result.addTransition(bHelp, i, transitionsLabel.elementAt(tr + j), mHelp.statesClassNumber[transitionsTo[tr + j]]);
            }
        }
        j = 0;
        for (i = 0; i < mHelp.classesStored; i++) {
            state = mHelp.classesFirstState[i];
            result.stateFinalities.setElement(i, stateFinalities.elementAt(state));
            result.stateTransitions[i] = j;
            result.stateNumberOfTransitions.setElement(i, stateNumberOfTransitions.elementAt(state));
    ...
    public StatePDA[] toFSM(TripleTransitions tripleTransitions) {
        ...
        TransitionPDA[] oldTransitions = tripleTransitions.labels.getCopyOfTransitions();
        ...
        for (i = 0; i < statesStored; i++) {
            ...
            for (j = 0; j < n; j++) {
                ...
                fsmStates[i].addTransition(new TransitionPDA(t.getConstraints(), fsmStates[transitionsTo[tr]], t.getType()), null);
            }
