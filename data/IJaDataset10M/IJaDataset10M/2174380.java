...
import net.sf.brightside.PokerMaster.metamodel.ICardable;
...
public class Deck {
    ...
    private ICardable[] gCards = new Card[NUM_CARDS];
    ...
    */
    public synchronized ICardable deal() {
    ...
    * @return the card dealt
    */
    public synchronized ICardable dealCard() {
    ...
    */
    public synchronized int findCard(ICardable c) {
        int i = position;
        int n = c.getIndex();
        while (i < NUM_CARDS && n != gCards[i].getIndex()) i++;
    ...
    private synchronized int findDiscard(Card c) {
        int i = 0;
        int n = c.getIndex();
        while (i < position && n != gCards[i].getIndex()) i++;
    ...
    */
    public synchronized void extractHand(Hand h) {
        for (int i = 1; i <= h.size(); i++) this.extractCard(h.getCard(i));
    }
    ...
    */
    public synchronized void extractCard(ICardable c) {
        int i = findCard(c);
        if (i != -1) {
            ICardable t = gCards[i];
    ...
    */
    public synchronized ICardable extractRandomCard() {
        int pos = position + randInt(NUM_CARDS - position);
        ICardable c = gCards[pos];
    ...
    */
    public synchronized ICardable pickRandomCard() {
    ...
    */
    public synchronized ICardable getCard(int i) {
        return gCards[i];
    }
    ...
    public String toString() {
        ...
        s.append("* ");
        for (int i = 0; i < position; i++) s.append(gCards[i].toString() + " ");
        s.append("\n* ");
        for (int i = position; i < NUM_CARDS; i++) s.append(gCards[i].toString() + " ");
        return s.toString();
    }
