...
public class PointTest extends JDOPersistenceTestCase {
    ...
    public PointTest(String name) {
        super(name);
        if (!initialised) {
            addClassesToSchema(new Class[] { PointHolder.class });
            initialised = true;
        }
    }
    ...
    public void testDetachAttach() throws Exception {
        try {
            ...
            assertTrue("Value of the java.awt.Point that was detached is incorrect : X should have been 0 but was " + detachedPoint.getPoint().getX(), (int) detachedPoint.getPoint().getX() == 0);
            assertTrue("Value of the java.awt.Point that was detached is incorrect : Y should have been 0 but was " + detachedPoint.getPoint().getY(), (int) detachedPoint.getPoint().getY() == 0);
            int detachedX = 400;
            int attachedY = 300;
            ...
            try {
                ...
                assertTrue("Value of the java.awt.Point that was retrieved is incorrect : X should have been " + detachedX + " but was " + point.getPoint().getX(), (int) point.getPoint().getX() == detachedX);
                assertTrue("Value of the java.awt.Point that was retrieved is incorrect : Y should have been " + attachedY + " but was " + point.getPoint().getY(), (int) point.getPoint().getY() == attachedY);
                detachedPoint = (PointHolder) pm.detachCopy(point);
                tx.commit();
            } catch (Exception e) {
            ...
            try {
                tx.begin();
                PointHolder point = (PointHolder) pm.getObjectById(pointId);
                assertTrue("Value of the java.awt.Point that was retrieved is incorrect : X should have been 200 but was " + point.getPoint().getX(), (int) point.getPoint().getX() == 200);
                assertTrue("Value of the java.awt.Point that was retrieved is incorrect : Y should have been 200 but was " + point.getPoint().getY(), (int) point.getPoint().getY() == 200);
                detachedPoint = (PointHolder) pm.detachCopy(point);
                tx.commit();
            } catch (Exception e) {
                fail("Error whilst retrieving object containing SCO Point : " + e.getMessage());
