...
public class NewInstanceTest extends JDWPSyncTestCase {
    ...
    public void testNewInstance001() {
        ...
        logWriter.println(" VirtualMachine.ClassesBySignature: classes=" + classes + "; refTypeTag=" + refTypeTag + "; typeID= " + typeID + "; status=" + status);
        assertAllDataRead(reply);
        assertEquals("VirtualMachine::ClassesBySignature returned invalid TypeTag,", JDWPConstants.TypeTag.CLASS, refTypeTag, JDWPConstants.TypeTag.getName(JDWPConstants.TypeTag.CLASS), JDWPConstants.TypeTag.getName(refTypeTag));
        packet = new CommandPacket(JDWPCommands.ReferenceTypeCommandSet.CommandSetID, JDWPCommands.ReferenceTypeCommandSet.MethodsCommand);
        ...
        for (int i = 0; i < declared; i++) {
            ...
            logWriter.println("  methodID=" + methodID + "; name=" + name + ";  signature=" + signature + "; modBits=" + modBits);
            if (name.equals("<init>")) {
                targetMethodID = methodID;
            }
        }
        assertAllDataRead(reply);
        packet = new CommandPacket(JDWPCommands.EventRequestCommandSet.CommandSetID, JDWPCommands.EventRequestCommandSet.SetCommand);
        ...
        CommandPacket event = debuggeeWrapper.vmMirror.receiveCertainEvent(JDWPConstants.EventKind.METHOD_ENTRY);
        ...
        for (int i = 0; i < events; i++) {
            ...
            logWriter.println("  EVENT_THREAD event " + i + ": eventKind=" + eventKind + "; requestID=" + newRequestID + "; threadID=" + threadID);
        ...
        packet = new CommandPacket(JDWPCommands.EventRequestCommandSet.CommandSetID, JDWPCommands.EventRequestCommandSet.ClearCommand);
        packet.setNextValueAsByte(JDWPConstants.EventKind.METHOD_ENTRY);
        packet.setNextValueAsInt(requestID);
        reply = debuggeeWrapper.vmMirror.performCommand(packet);
        checkReplyPacket(reply, "EventRequest::Clear command");
        assertAllDataRead(reply);
        packet = new CommandPacket(JDWPCommands.ClassTypeCommandSet.CommandSetID, JDWPCommands.ClassTypeCommandSet.NewInstanceCommand);
        packet.setNextValueAsClassID(typeID);
        ...
        logWriter.println(" ClassType.NewInstance: newObject.tag=" + newObject.tag + "; newObject.objectID=" + newObject.objectID);
        TaggedObject exception = reply.getNextValueAsTaggedObject();
        logWriter.println(" ClassType.NewInstance: exception.tag=" + exception.tag + "; exception.objectID=" + exception.objectID);
        assertNotNull("newObject is null", newObject);
        assertTrue("newObject.objectID is 0", newObject.objectID != 0);
        assertEquals("ClassType::NewInstance returned invalid newObject.tag,", JDWPConstants.Tag.OBJECT_TAG, newObject.tag, JDWPConstants.Tag.getName(JDWPConstants.Tag.OBJECT_TAG), JDWPConstants.Tag.getName(newObject.tag));
        ...
        assertAllDataRead(reply);
        packet = new CommandPacket(JDWPCommands.ObjectReferenceCommandSet.CommandSetID, JDWPCommands.ObjectReferenceCommandSet.ReferenceTypeCommand);
        ...
        logWriter.println(" ObjectReference.ReferenceType: refTypeTag=" + newRefTypeTag + "; typeID= " + newTypeID);
        assertEquals("ObjectReference::ReferenceType returned invalid newRefTypeTag,", JDWPConstants.TypeTag.CLASS, newRefTypeTag, JDWPConstants.TypeTag.getName(JDWPConstants.TypeTag.CLASS), JDWPConstants.TypeTag.getName(newRefTypeTag));
        ...
        logWriter.println(" ClassType.NewInstance: exception.tag=" + exception.tag + "; exception.objectID=" + exception.objectID);
        assertNotNull("newObject is null", newObject);
        assertEquals("ClassType::NewInstance returned invalid newObject.objectID,", 0, newObject.objectID);
        assertEquals("ClassType::NewInstance returned invalid invalid newObject.tag,", JDWPConstants.Tag.OBJECT_TAG, newObject.tag, JDWPConstants.Tag.getName(JDWPConstants.Tag.OBJECT_TAG), JDWPConstants.Tag.getName(newObject.tag));
        assertNotNull("ClassType::NewInstance returned invalid exception = null", newObject);
        assertTrue("ClassType::NewInstance returned invalid exception.objectID = 0", exception.objectID != 0);
        assertEquals("ClassType::NewInstance returned invalid invalid exception.tag,", JDWPConstants.Tag.OBJECT_TAG, exception.tag, JDWPConstants.Tag.getName(JDWPConstants.Tag.OBJECT_TAG), JDWPConstants.Tag.getName(exception.tag));
        assertAllDataRead(reply);
        packet = new CommandPacket(JDWPCommands.VirtualMachineCommandSet.CommandSetID, JDWPCommands.VirtualMachineCommandSet.ResumeCommand);
        reply = debuggeeWrapper.vmMirror.performCommand(packet);
    ...
    public void testNewInstance002() {
        ...
        checkReplyPacket(reply, "VirtualMachine::ClassesBySignature command");
        int classes = reply.getNextValueAsInt();
        assertEquals("VirtualMachine::ClassesBySignature returned invalid cluss number,", 1, classes);
        byte refTypeTag = reply.getNextValueAsByte();
        long typeID = reply.getNextValueAsReferenceTypeID();
        int status = reply.getNextValueAsInt();
        logWriter.println(" VirtualMachine.ClassesBySignature: classes=" + classes + "; refTypeTag=" + refTypeTag + "; typeID= " + typeID + "; status=" + status);
        ...
        logWriter.println(" ClassType.NewInstance: ErrorCode=" + error + "(" + JDWPConstants.Error.getName(error) + ")");
        assertEquals("ClassType.NewInstance returned invalid error code,", JDWPConstants.Error.ILLEGAL_ARGUMENT, error, JDWPConstants.Error.getName(JDWPConstants.Error.ILLEGAL_ARGUMENT), JDWPConstants.Error.getName(error));
