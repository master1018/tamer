...
public class ViewsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private void updateOverrideState(boolean override) {
        ...
        if (store.getBoolean(IPreferenceConstants.OVERRIDE_PRESENTATION) != override) {
            store.setValue(IPreferenceConstants.OVERRIDE_PRESENTATION, override);
        }
    ...
    private void hookOverrideListener() {
        ...
        overrideListener = new IPropertyChangeListener() {
            ...
            public void propertyChange(PropertyChangeEvent event) {
                if (event.getProperty().equals(IPreferenceConstants.OVERRIDE_PRESENTATION)) {
                    updateOverride();
                }
            }
        };
    ...
    private void updateOverride() {
        boolean override = getPreferenceStore().getBoolean(IPreferenceConstants.OVERRIDE_PRESENTATION);
    ...
    private void setPresentationSelection() {
        for (int i = 0; i < presentationFactories.length; ++i) {
            if (currentPresentationFactoryId.equals(presentationFactories[i].getAttribute(IWorkbenchConstants.TAG_ID))) {
    ...
    private void refreshPresentationFactories() {
        currentPresentationFactoryId = PrefUtil.getAPIPreferenceStore().getString(IWorkbenchPreferenceConstants.PRESENTATION_FACTORY_ID);
        presentationFactories = Platform.getExtensionRegistry().getConfigurationElementsFor(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_PRESENTATION_FACTORIES);
        Arrays.sort(presentationFactories, new Comparator() {
            ...
            public int compare(Object a, Object b) {
                ...
                return collator.compare(el1.getAttribute(IWorkbenchConstants.TAG_NAME), el2.getAttribute(IWorkbenchConstants.TAG_NAME));
            }
        });
    }
    ...
    private boolean updatePresentationPreferences() {
        ...
        PrefUtil.getAPIPreferenceStore().putValue(IWorkbenchPreferenceConstants.PRESENTATION_FACTORY_ID, id);
        return true;
    }
    ...
    private void setR30Preferences() {
        ...
        perspBarLocation = apiStore.getDefaultString(IWorkbenchPreferenceConstants.DOCK_PERSPECTIVE_BAR);
        perspLeftButton.setSelection(perspBarLocation.equals(IWorkbenchPreferenceConstants.LEFT));
        perspTopLeftButton.setSelection(perspBarLocation.equals(IWorkbenchPreferenceConstants.TOP_LEFT));
        perspTopRightButton.setSelection(perspBarLocation.equals(IWorkbenchPreferenceConstants.TOP_RIGHT));
    ...
    private void setViewAlignDefault(IPreferenceStore internalStore) {
        ...
        viewAlignment = internalStore.getDefaultInt(IPreferenceConstants.VIEW_TAB_POSITION);
    ...
    private void setEditorAlignDefault(IPreferenceStore internalStore) {
        ...
        editorAlignment = internalStore.getDefaultInt(IPreferenceConstants.EDITOR_TAB_POSITION);
    ...
    private void refreshThemeCombo(String themeToSelect) {
        ...
        if (currentTheme.getId().equals(IThemeManager.DEFAULT_THEME)) {
            defaultThemeString = MessageFormat.format(WorkbenchMessages.ViewsPreference_currentThemeFormat, new Object[] { defaultThemeString });
        }
        ...
        for (int i = 0; i < descs.length; i++) {
            ...
            if (descs[i].getId().equals(currentTheme.getId())) {
                themeString = MessageFormat.format(WorkbenchMessages.ViewsPreference_currentThemeFormat, new Object[] { themeString });
            }
    ...
    protected void createEnableAnimationsPref(Composite composite) {
        ...
        enableAnimations.setSelection(apiStore.getBoolean(IWorkbenchPreferenceConstants.ENABLE_ANIMATIONS));
    ...
    protected void performDefaults() {
        ...
        String presID = apiStore.getDefaultString(IWorkbenchPreferenceConstants.PRESENTATION_FACTORY_ID);
    ...
    public boolean performOk() {
        ...
        if (fastViewLoc.equals(INITIAL_VAL)) {
            ...
            apiStore.setValue(IWorkbenchPreferenceConstants.INITIAL_FAST_VIEW_BAR_LOCATION, fastViewLoc);
        }
