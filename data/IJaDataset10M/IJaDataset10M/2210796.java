...
public abstract class AbstractDatabaseTest {
    ...
    protected static IJdbcDbDialect platform;
    ...
    protected static IJdbcDbDialect getDbDialect(boolean useExisting) {
        ...
        if (useExisting) {
            if (platform == null) {
                platform = JdbcDbDialectFactory.createPlatform(createDataSource(), new Parameters());
            }
            result = platform;
        } else {
    ...
    protected static DataSource createDataSource() {
        return new DriverDataSourceProperties("src/test/resources/test-jdbc.properties").getDataSource();
    }

    protected Table buildTestTable() {
        IJdbcDbDialect platform = getDbDialect(true);
        Table table = new Table("TEST", new Column("TEST_ID", TypeMap.INTEGER, null, true, true, true), new Column("TEST_TEXT", TypeMap.VARCHAR, "1000", false, false, false));
    ...
    protected void delete(String tableName) {
        ...
        dbDialect.getSqlTemplate().update(String.format("delete from %s%s%s", quoteString, tableName, quoteString));
    }
    ...
    protected int count(String tableName, String where) {
        ...
        String quoteString = dbDialect.getDbDialectInfo().getIdentifierQuoteString();
        return connection.queryForInt(String.format("select count(*) from %s%s%s %s %s", quoteString, tableName, quoteString, StringUtils.isNotBlank(where) ? "where" : "", StringUtils.isNotBlank(where) ? where : ""));
    }

    protected void prepareInsertIntoTestTable(ISqlTransaction transaction, String tableName, int flushAt, boolean batchMode) {
        transaction.setInBatchMode(batchMode);
        transaction.setNumberOfRowsBeforeBatchFlush(flushAt);
        transaction.prepare(String.format("insert into %s (TEST_ID, TEST_TEXT) values(?, ?)", tableName));
    }

    protected int batchInsertIntoTestTable(int numberToInsert, int numberToStartAt, ISqlTransaction transaction) {
        int updatedCount = 0;
        for (int i = 0; i < numberToInsert; i++) {
            updatedCount += transaction.update(numberToStartAt + i, new Object[] { numberToStartAt + i, Integer.toString(numberToStartAt + i) }, new int[] { Types.INTEGER, Types.VARCHAR });
        }
        return updatedCount;
    }

    protected void printOutTable(String tableName) {
        log.log(LogLevel.INFO, getDbDialect().getSqlTemplate().query(String.format("select * from %s", tableName)).toString());
    }
}
