...
public class PlansAnalyse {
    ...
    private static final Logger log = Logger.getLogger(PlansAnalyse.class);
    ...
    public void run(final Population plans) {
        ...
        double[] lic_cnt = new double[2];
        for (int i = 0; i < lic_cnt.length; i++) {
            lic_cnt[i] = 0;
        }
        double[] mt_cnt = new double[6];
        for (int i = 0; i < mt_cnt.length; i++) {
            mt_cnt[i] = 0;
        }
        double a_cnt = 0;
        double[] at_cnt = new double[5];
        for (int i = 0; i < at_cnt.length; i++) {
            at_cnt[i] = 0;
        }
        double leg_cnt = 0;
        double[] mtype_cnt = new double[4];
        for (int i = 0; i < mtype_cnt.length; i++) {
            mtype_cnt[i] = 0;
        }
        ...
        for (Person pp : plans.getPersons().values()) {
            ...
            if (p.getCarAvail() == null) {
                ...
                idx += 0;
            } else if (p.getCarAvail().equals(SOMETIMES)) {
                idx += 1;
            } else if (p.getCarAvail().equals(ALWAYS)) {
                idx += 2;
            } else {
                Gbl.errorMsg("pid=" + p.getId() + ": Haeh?");
            }
            mt_cnt[idx]++;
            if (p.getPlans().size() != 1) {
                Gbl.errorMsg("pid=" + p.getId() + ": There must be exactly one plan per person!");
            }
            Plan plan = p.getPlans().get(0);
            for (PlanElement pe : plan.getPlanElements()) {
                if (pe instanceof Activity) {
                    Activity a = (Activity) pe;
                    if (a.getType().substring(0, 1).equals(H)) {
                        at_cnt[0]++;
                    } else if (a.getType().substring(0, 1).equals(W)) {
                        at_cnt[1]++;
                    } else if (a.getType().substring(0, 1).equals(E)) {
                        at_cnt[2]++;
                    } else if (a.getType().substring(0, 1).equals(S)) {
                        at_cnt[3]++;
                    } else if (a.getType().substring(0, 1).equals(L)) {
                        at_cnt[4]++;
                    } else {
                        Gbl.errorMsg("pid=" + p.getId() + ": Haeh?");
                    }
            ...
            for (PlanElement pe : plan.getPlanElements()) {
                if (pe instanceof Leg) {
                    ...
                    if (l.getMode().equals(TransportMode.car)) {
                        ...
                        mtype_cnt[1]++;
                    } else if (l.getMode().equals(TransportMode.bike)) {
                        mtype_cnt[2]++;
                    } else if (l.getMode().equals(TransportMode.walk)) {
                        mtype_cnt[3]++;
                    } else {
                        Gbl.errorMsg("pid=" + p.getId() + ": Haeh?");
                    }
