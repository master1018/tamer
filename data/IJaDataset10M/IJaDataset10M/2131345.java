package org.formaria.editor.color;
...
public class ColorWheelPanel extends AbstractColorChooserPanel implements ActionListener, MouseListener, MouseMotionListener, MouseWheelListener, ChangeListener {
   ...
   */
    public ColorWheelPanel() {
        ...
        if (font9pt == null) font9pt = new Font("Arial", 0, 9);
        ...
        imagePicker.setForeground(getForeground());
        ...
        brightnessSlider.setToolTipText(getLabel("Formaria.ctrlDrag", "CTRL+drag to adjust the color wheel"));
        fixedPanel.add(brightnessSlider);
        resetBtn = new JButton();
        resetBtn.setBounds(237, 109, 10, 10);
        resetBtn.setBackground(getBackground());
        resetBtn.addActionListener(this);
        resetBtn.setToolTipText(getLabel("Formaria.reset", "Reset the color wheel sauturation and brightness"));
        ...
        saturationSlider.setToolTipText(getLabel("Formaria.ctrlDrag", "CTRL+drag to adjust the color wheel"));
        fixedPanel.add(saturationSlider);
        useWebColors = new JCheckBox(getLabel("Formaria.webSafeColors", "Use web safe colors"));
        ...
        decimalRGB = new JCheckBox(getLabel("Formaria.decimalRGB", "Decimal RGB"));
        ...
        baseColorLabel.setToolTipText(getLabel("Formaria.systemColorsTooltip", "Right click for system colours"));
        ...
        JLabel hueLabel = new JLabel(getLabel("Formaria.hue", "Hue") + " °");
        ...
        JLabel satLabel = new JLabel(getLabel("Formaria.saturation", "Saturation") + " %");
        ...
        JLabel brightLabel = new JLabel(getLabel("Formaria.brightness", "Brightness") + " %");
   ...
   */
    ...
    public void setColor(Color c) {
        ...
        if ((0.5 * c.getRed() + c.getGreen() + 0.3 * c.getBlue()) < 220.0) baseColorLabel.setForeground(Color.white); else baseColorLabel.setForeground(Color.black);
        String colorStr;
        if (decimalRGB.isSelected()) {
            colorStr = " " + Integer.toString(c.getRed()) + "." + Integer.toString(c.getGreen()) + "." + Integer.toString(c.getBlue());
        } else {
            colorStr = " " + ModelColor.toHexString(c.getRed()) + ModelColor.toHexString(c.getGreen()) + ModelColor.toHexString(c.getBlue());
        }
        ...
        if (hasChooser) getColorSelectionModel().setSelectedColor(c);
    }
   ...
   */
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == resetBtn) resetColorWheel(); else if (source instanceof JMenuItem) {
            Color sysColor = getSystemColor(((JMenuItem) source).getText());
            if (sysColor != null) setColor(sysColor);
            resetColor();
            ...
            if (hex.length() == 0) resetColor(); else if (decimalRGB.isSelected()) {
                ...
                try {
                    ...
                    if (pos2 > 0) {
                        ...
                        if (pos2 > 0) {
                            ...
                            pos = ++pos2;
                            if (pos2 < hex.length()) b = Integer.parseInt(hex.substring(pos));
                        }
                    }
                ...
                for (int i = hex.length(); i < 6; i++) hex += "0";
                try {
                    setColor(new Color(ModelColor.hex2dec(hex.substring(0, 2)), ModelColor.hex2dec(hex.substring(2, 4)), ModelColor.hex2dec(hex.substring(4, 6))));
                } catch (NumberFormatException nfe) {
                    setColor(Color.red);
    ...
    private void resetColor() {
        if (chooserColor != null) {
            if (!busy) {
                ...
                if (selectedIttenColours != null) selectedIttenColours[0].setHSV(h, s, v);
                ...
                if (!moveHue(pt)) {
                    if (outerCircle.contains(pt)) {
                        ...
                        for (int i = 0; i < ModelColor.NUM_SEGMENTS; i++) {
                            if (paths[bandOffset + i].contains(pt)) hue = i * 15;
                        }
                        int hueInc = (hue / 15) % 2;
                        ...
                        if (!pixelColor.equals(Color.white)) setColor(pixelColor);
                    }
                }
            }
        }
        if (displayScheme) imagePicker.repaint();
    }
            ...
            if (paths != null) {
                ...
                for (int i = 0; i < numPaths; i++) {
                    if (paths[i].contains(pt.x, pt.y)) {
                        ...
                        if (adjustRollover) modelColor = new ModelColor(modelColor.H, saturationMultipler * modelColor.S, brightnessMultipler * modelColor.V);
                        rolloverColor = new Color(modelColor.getRed(), modelColor.getGreen(), modelColor.getBlue());
                        if (ring < 4) rolloverColor = rolloverColor.darker(); else rolloverColor = rolloverColor.brighter().brighter();
                        break;
                    }
                }
            }
        }
        if (rolloverPath != oldPath) repaint();
    }
   ...
   */
     ...
     */
        ...
        public void paintWheel(Graphics2D g2d) {
            if (paths == null) paths = new GeneralPath[ModelColor.NUM_COLOR_RINGS * ModelColor.NUM_SEGMENTS];
            ...
            if (width > 200) {
                ...
                AttributedString as = new AttributedString(getLabel("Formaria.warm", "WARM"));
                as.addAttribute(TextAttribute.FAMILY, fontFamily);
                ...
                sin = Math.sin(angle);
                cos = Math.cos(angle);
                as = new AttributedString(getLabel("Formaria.cold", "COLD"));
                as.addAttribute(TextAttribute.FAMILY, fontFamily);
                as.addAttribute(TextAttribute.SIZE, (float) (ringThickness / 1.5));
                ...
                sin = Math.sin(angle);
                cos = Math.cos(angle);
                as = new AttributedString(getLabel("Formaria.saturation", "Saturation"));
                as.addAttribute(TextAttribute.FAMILY, fontFamily);
                ...
                g2d.setTransform(at);
                g2d.drawString(as.getIterator(), 0.0F, 0.0F);
                String brightnessText = getLabel("Formaria.brightness", "Brightness");
    ...
    private class ColorDocumentListener implements DocumentListener {
        ...
        private static final String MARKER = "Formaria.UI.ColorWheel.DocumentEvent";
     ...
     */
        ...
        public void synchronize(DocumentEvent evt) {
            ...
            if (hueEdit.getText().length() == 0) _hasAllValues = false;
            if (brightEdit.getText().length() == 0) _hasAllValues = false;
    ...
    private static String getLabel(String labelName, String defaultValue) {
        if (ColorWheelPanel.labelBundle == null) return defaultValue;
        try {
   ...
   * @return true if the colors should change to match the brightness and saturation
   */
    private boolean shouldAdjustWheel() {
        if (adjustWheel == NEVER_ADJUST) return false; else if (adjustWheel == ALWAYS_ADJUST) return true; else if (ctrlKeyDown) return true;
        return false;
    }
   ...
   * @param state the color wheel's new adjustment mode ( CTRL_ADJUST | ALWAYS_ADJUST | NEVER_ADJUST );
   */
