...
public class MPrintFormatItem extends X_AD_PrintFormatItem {
    ...
    protected boolean afterSave(boolean newRecord, boolean success) {
        if (newRecord && (getPrintName() != null) && (getPrintName().length() > 0) && getAD_Column_ID() != 0) {
            String sql = "SELECT printname " + "FROM ad_column as c " + "INNER JOIN ad_element_trl as e ON e.ad_element_id = c.ad_element_id " + "WHERE c.ad_column_id = ? AND ad_language = ?";
