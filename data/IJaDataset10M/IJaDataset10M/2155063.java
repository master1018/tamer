...
public class Drawable extends XID {
   ...
   */
    public Rectangle copyIntoXImage(XImage dest, Rectangle bounds, int destX, int destY) {
        ...
        while (!bounds.isEmpty()) {
            if (copyIntoXImageImpl(dest, bounds.x, bounds.y, bounds.width, bounds.height, destX, destY)) return bounds;
            newBounds = getBounds(newBounds);
            bounds = newBounds.intersection(bounds);
            tries--;
            if (tries < 0) throw new RuntimeException("copyIntoXImage is buggy");
        }
        return bounds;
    }
   ...
   */
    private native boolean copyIntoXImageImpl(XImage destinationImage, int x, int y, int width, int height, int destX, int destY);
    ...
    protected void finalize() throws Throwable {
        for (int i = 0; i < gcCachedCount; i++) gcCache[i].disposeImpl();
        gcCachedCount = 0;
        super.finalize();
    }
