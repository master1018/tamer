                    ...
                    for (int i = 0; i < DLNAMediaSubtitle.subExtensions.length; i++) {
                        ...
                        if (fName.length() > ext.length() && fName.startsWith(fileName) && fName.endsWith("." + ext)) {
                            ...
                            if (media != null) {
                                for (DLNAMediaSubtitle sub : media.getSubtitlesCodes()) {
                                    if (f.equals(sub.getFile())) {
                                        exists = true;
                                    } else if (ext.equals("idx") && sub.getType() == DLNAMediaSubtitle.MICRODVD) {
                                        sub.setType(DLNAMediaSubtitle.VOBSUB);
                                        exists = true;
                                    } else if (ext.equals("sub") && sub.getType() == DLNAMediaSubtitle.VOBSUB) {
                                        sub.setFile(f);
                                        exists = true;
                                    }
                                }
                            ...
                            if (!exists) {
                                ...
                                sub.setId(100 + (media == null ? 0 : media.getSubtitlesCodes().size()));
                                sub.setFile(f);
                                sub.checkUnicode();
