...
import org.mandarax.kernel.meta.*;
import org.mandarax.kernel.validation.*;
import org.mandarax.reference.AdvancedKnowledgeBase;
import org.mandarax.reference.validation.TestCaseImpl;
import org.mandarax.sql.*;
...
public class TestData implements LogCategories {
    ...
    public static SimplePredicate simplePredicate2() {
        ...
        SimplePredicate p = new SimplePredicate("is father of 2", struct);
        return p;
    }
    ...
    public static TestCase testCase1() {
        Query query = query1();
        ClauseSet[] assumptions = { fact1() };
        Map[] expectedReplacements = new Map[0];
        return lfactory.createTestCase(query, assumptions, TestCase.AT_BOTTOM, -1, true, expectedReplacements);
    }
    ...
    public static TestCase testCase2() {
        ...
        ClauseSet[] assumptions = { fact1() };
        Map[] expectedReplacements = new Map[0];
        return lfactory.createTestCase(query, assumptions, TestCase.AT_BOTTOM, -1, false, expectedReplacements);
    }
     ...
     * @return a test case.
     */
    public static TestCase testCase3() {
        Query query = query2();
        ClauseSet[] assumptions = { fact1() };
        Map[] expectedReplacements = new Map[0];
        return lfactory.createTestCase(query, assumptions, TestCase.AT_BOTTOM, 42, true, expectedReplacements);
    }
     ...
     * @return a test case.
     */
    public static TestCase testCase4() {
        Query query = query2();
        ClauseSet[] assumptions = { fact1(), fact2(), andRule(), orRule(), sqlClauseSet() };
        Map[] expectedReplacements = new Map[0];
        return lfactory.createTestCase(query, assumptions, TestCase.AT_BOTTOM, 42, true, expectedReplacements);
    }
     ...
     * @return a test case.
     */
    public static TestCase testCase5() {
        Query query = query2();
        ClauseSet[] assumptions = { fact1() };
        Map[] expectedReplacements = new Map[1];
        expectedReplacements[0] = new Hashtable();
        expectedReplacements[0].put(lfactory.createVariableTerm("x", Integer.TYPE), new Integer(42));
        return lfactory.createTestCase(query, assumptions, TestCase.ON_TOP, 42, true, expectedReplacements);
    }
     ...
     * @return a test case.
     */
    public static TestCase testCase6() {
        Query query = query2();
        ClauseSet[] assumptions = { fact1(), fact2(), andRule(), orRule(), sqlClauseSet() };
        Map[] expectedReplacements = new Map[2];
        expectedReplacements[0] = new Hashtable();
        expectedReplacements[0].put(lfactory.createVariableTerm("x", Integer.TYPE), new Integer(42));
        expectedReplacements[0].put(lfactory.createVariableTerm("dob", java.util.Date.class), new Date());
        expectedReplacements[1] = new Hashtable();
        expectedReplacements[1].put(lfactory.createVariableTerm("x", String.class), "test");
        return lfactory.createTestCase(query, assumptions, TestCase.ON_TOP, 42, true, expectedReplacements);
    }
}
