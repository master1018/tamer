...
public abstract class Account extends PersistentObject implements Cloneable {
    ...
    public void setTransactions(List transactions) {
        ...
        if (this.transactions != null) this.transactions.removeCollectionObserver(transactionsObserver);
    ...
    public double getBalance() {
        ...
        for (int i = 0; i < objs.length; i++) {
            ...
            if (getCurrency() != null && !getCurrency().equals(transaction.getSecurity().getCurrency())) amount = CurrencyConverter.getInstance().convert(transaction.getDate(), amount, transaction.getSecurity().getCurrency(), getCurrency());
            result += amount;
        }
        return result;
    }
    ...
    public List getPortfolio() {
        ...
        for (Iterator iter2 = getTransactions().iterator(); iter2.hasNext(); ) {
            ...
            if (position == null) map.put(transaction.getSecurity(), new PortfolioPosition(this, transaction.getSecurity(), transaction.getQuantity(), transaction.getAmount())); else position.add(transaction.getQuantity(), transaction.getAmount());
        }
    ...
    public PortfolioPosition getPortfolio(Security security) {
        ...
        for (Iterator iter2 = getTransactions().iterator(); iter2.hasNext(); ) {
            Transaction transaction = (Transaction) iter2.next();
            if (position.getSecurity().equals(transaction.getSecurity())) position.add(transaction.getQuantity(), transaction.getAmount());
        }
        return position;
    }
