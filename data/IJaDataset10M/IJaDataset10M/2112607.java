...
public class ToXMLInstanceConverter {
     ...
     *             thrown.
     */
    public void convert(Object obj, String suffix) throws UnsupportedTypeException {
        ...
        if (!themeFile.exists()) themeFile.mkdirs();
        ...
        try {
            ...
            if (modelPs != null) modelPs.close();
            if (themePs != null) themePs.close();
        }
    }
     ...
     *             thrown.
     */
    public void convert(Object obj, PrintStream instanceXMLStream, PrintStream themeXMLStream) throws UnsupportedTypeException {
        ...
        visited.clear();
        buildModel(obj);
        if (instanceXMLStream != null) writeInstanceXML(instanceXMLStream, "", obj.getClass().getSimpleName());
        if (themeXMLStream != null) writeThemeXML(themeXMLStream);
    }

    private static final String[] colors = new String[] { "Yellow", "Green", "Red", "Blue", "Gray", "Black", "White" };

    private static final String[] shapes = new String[] { "Box", "Ellipse", "Circle", "Hexagon", "Octagon", "Dbl Octagon", "Tpl Octagon" };
    ...
    private void writeThemeXML(PrintStream ps) {
        ...
        for (AlloySig sig : SigFactory.getInstance().getAllSigs()) {
            if (sig.isPrimitive()) ps.println("<node visible=\"no\">"); else ps.println("<node shape=\"" + shapes[i % shapes.length] + "\" color=\"" + colors[i % colors.length] + "\">");
    ...
    private void writeInstanceXML(PrintStream ps, String filename, String command) {
        ps.println("<?xml version=\"1.0\"?>");
        ps.println("<alloy>");
        ps.println("<instance filename=\"" + filename + "\" command=\"" + command + "\">");
        ps.println();
        for (AlloySig sig : SigFactory.getInstance().getAllSigs()) {
            String s = "  <sig name=\"{0}\" extends=\"{1}\">";
            String extendsSigName = "univ";
            if (sig.getExtendsSig() != null) extendsSigName = sig.getExtendsSig().getName();
    ...
    private void buildModel(Object obj) throws UnsupportedTypeException {
        AlloySig sig = SigFactory.getInstance().getAlloySig(obj.getClass().getSimpleName());
    ...
    private void handleClass(Class clz, AlloyAtom atom) throws UnsupportedTypeException {
        ...
        if (superClz != null) {
            ...
            AlloySig superSig = SigFactory.getInstance().getAlloySig(superClz.getSimpleName());
            AlloySig sig = SigFactory.getInstance().getAlloySig(clz.getSimpleName());
    ...
    private void handleFields(Class clz, AlloyAtom atom) throws UnsupportedTypeException {
        ...
        if (obj == null) return;
        AlloySig clzSig = SigFactory.getInstance().getAlloySig(clz.getSimpleName());
        ...
        try {
            for (Field f : fields) {
                ...
                if (fieldType.isArray()) {
                    ...
                    if (fieldValue != null) {
                        AlloySig fieldSig = SigFactory.getInstance().getAlloySig(fieldValue.getClass().getSimpleName());
                        AlloyAtom fieldValueAtom = AtomFactory.getInstance().getAlloyAtom(fieldSig, fieldValue);
                        AlloyField af = FieldFactory.getInstance().createAlloyField(f.getName());
    ...
    private void handleArrayField(Object obj, Field f, AlloySig clzSig, AlloyAtom atom) throws IllegalArgumentException, IllegalAccessException {
        ...
        for (Object fieldValue : KoratArrayManager.getArrayValues(obj, f.getName())) {
            ...
            if (fieldValue != null) {
                ...
                AlloySig fieldSig = SigFactory.getInstance().getAlloySig(elemType.getSimpleName());
                AlloyAtom fieldValueAtom = AtomFactory.getInstance().getAlloyAtom(fieldSig, fieldValue);
                AlloyField af = FieldFactory.getInstance().createAlloyField(f.getName() + "_" + idx);
                af.getTypes().add(clzSig);
                af.getTypes().add(fieldSig);
                af.getValues().add(atom);
                af.getValues().add(fieldValueAtom);
    ...
    private void handlePrimitives(Object obj, Field f, AlloySig objSig, AlloyAtom objAtom) throws UnsupportedTypeException, IllegalAccessException {
        if (f.getType() == int.class) {
            ...
            AlloyAtom atom = AtomFactory.getInstance().getIntAlloyAtom(sig, fieldValue);
            sig.addAtom(atom);
            AlloyField af = FieldFactory.getInstance().createAlloyField(f.getName());
            ...
            throw new UnsupportedTypeException("The only primitive type that is supported is int.");
        }
    }
}
