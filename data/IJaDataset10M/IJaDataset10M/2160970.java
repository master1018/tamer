...
public class ComposedQuery {
   ...
   * superqueries */
    ...
    public void init() {
        ...
        if (derivedSections[FROM] != null) fromAnalyzerOQL += "FROM " + derivedSections[FROM];
    }
    ...
    protected void addSubquery(ComposedSubquery q) {
        if (subqueries.size() == 0) prependFromToKeyset();
   ...
   * They are all added as projections (this has to change) */
    protected void prependFromToKeyset() {
        ...
        for (int i = 0; i < keyset.size(); i++) checkProjectionInteger((String) e.nextElement());
        for (StringTokenizer st = new StringTokenizer(sections[FROM] == null ? "" : sections[FROM], ","); st.hasMoreTokens(); ) {
            ...
            if (j == -1) throw new RuntimeException("invalid FROM");
            label = label.substring(j + 1).trim();
            keysetLabels.addElement(label);
            keyset.addElement(addProjection(label));
        }
        while (e.hasMoreElements()) checkProjectionInteger((String) e.nextElement());
    }
    ...
    protected String computeQuery(String derivedSections[], boolean typeAnalysisOnly) {
        ...
        if ((o = derivedSections[FROM]) != null) {
            sb.append(" FROM ");
            sb.append(o);
            if (!typeAnalysisOnly && derivedSections.length == 5 && derivedSections[VARFROM] != null && derivedSections[VARFROM].trim().length() > 0) sb.append(",").append(derivedSections[VARFROM]);
        }
        if (!typeAnalysisOnly) {
        ...
        if (!typeAnalysisOnly) return ret;
        ArgumentReplacer ar = new ArgumentReplacer(ret);
        Dictionary d = new Hashtable();
        int j = 1;
        for (Enumeration e = ar.getArgumentNames(); e.hasMoreElements(); ) d.put(e.nextElement(), "$" + (j++));
        return ar.replaceValues(d);
    }

    /** execute the contained query in the given database */
    public Grouper execute(org.makumba.Transaction db, Attributes a, Evaluator v, int offset, int limit) throws LogicException {
        analyze();
        String[] vars = new String[5];
        vars[0] = derivedSections[FROM];
        for (int i = 1; i < 5; i++) vars[i] = derivedSections[i] == null ? null : v.evaluate(derivedSections[i]);
        return new Grouper(previousKeyset, ((MultipleAttributeParametrizer) queries.getResource(new MultipleKey(vars))).execute(db, a, offset, limit).elements());
    }
    ...
    NamedResources queries = new NamedResources("Composed queries", new NamedResourceFactory() {
        ...
        protected Object makeResource(Object nm, Object hashName) {
            ...
            for (int i = 0; i < 5; i++) {
                if (mk.elementAt(i) instanceof String) sections[i] = (String) mk.elementAt(i);
            }
    ...
    public synchronized void analyze() {
        if (projections.isEmpty()) prependFromToKeyset();
        if (typeAnalyzerOQL == null) typeAnalyzerOQL = computeQuery(derivedSections, true);
    }
    ...
    public Object checkExprSetOrNullable(String s) {
        ...
        int m = 0;
        while (true) {
            while (n < s.length() && !isMakId(s.charAt(n))) n++;
    ...
    public Object checkId(String s) {
        ...
        if (dd == null) throw new org.makumba.InvalidValueException("no such label " + s.substring(0, dot));
        while (true) {
            int dot1 = s.indexOf(".", dot + 1);
            if (dot1 == -1) {
                FieldDefinition fd = dd.getFieldDefinition(s.substring(dot + 1));
                if (fd == null) throw new org.makumba.NoSuchFieldException(dd, s.substring(dot + 1));
                if (fd.getType().equals("set")) return fd;
                return null;
            }
            FieldDefinition fd = dd.getFieldDefinition(s.substring(dot + 1, dot1));
            if (fd == null) throw new org.makumba.NoSuchFieldException(dd, s.substring(dot + 1, dot1));
            if (!fd.getType().startsWith("ptr")) throw new InvalidFieldTypeException(fd, "pointer");
            if (!fd.isNotNull()) return s.substring(0, dot1);
