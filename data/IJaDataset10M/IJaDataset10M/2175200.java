...
public class TableEditAction extends HTMLTextEditAction {
    ...
    public TableEditAction(int type) throws IllegalArgumentException {
        super("");
        if (type < 0 || type >= NAMES.length) throw new IllegalArgumentException("Invalid type");
        this.type = type;
    ...
    protected void wysiwygEditPerformed(ActionEvent e, JEditorPane ed) {
        ...
        if (td == null || tr == null || document == null) return;
        CompoundUndoManager.beginCompoundEdit(document);
        try {
            if (type == INSERT_CELL) document.insertAfterEnd(td, "<td></td>"); else if (type == DELETE_CELL) removeCell(td); else if (type == INSERT_ROW) insertRowAfter(tr); else if (type == DELETE_ROW) removeRow(tr); else if (type == INSERT_COL) insertColumnAfter(td); else if (type == DELETE_COL) removeColumn(td);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private void removeCell(Element td) throws Exception {
        ...
        if (tr != null && td.getName().equals("td")) {
            if (td.getEndOffset() != tr.getEndOffset()) remove(td); else if (getRowCellCount(tr) <= 1) remove(tr); else {
    ...
    private void insertRowAfter(Element tr) throws Exception {
        ...
        if (table != null && tr.getName().equals("tr")) {
            HTMLDocument doc = (HTMLDocument) tr.getDocument();
            if (tr.getEndOffset() != table.getEndOffset()) doc.insertAfterEnd(tr, getRowHTML(tr)); else {
    ...
    private void removeRow(Element tr) throws Exception {
        Element table = HTMLUtils.getParent(tr, HTML.Tag.TABLE);
        if (table != null && tr.getName().equals("tr")) {
            if (tr.getEndOffset() != table.getEndOffset()) remove(tr); else if (getTableRowCount(table) <= 1) remove(table); else {
                StringWriter out = new StringWriter();
    ...
    private void remove(Element el) throws BadLocationException {
        ...
        if (el.getEndOffset() > document.getLength()) len = document.getLength() - start;
    ...
    private void removeColumn(Element td) throws Exception {
        ...
        for (int i = 0; i < table.getElementCount(); i++) {
            ...
            if (row.getName().equals("tr")) {
                ...
                if (e != null && e.getName().equals("td")) removeCell(e);
            }
        }
    }
        ...
        for (int i = 0; i < table.getElementCount(); i++) {
            ...
            if (row.getName().equals("tr")) {
                ...
                for (int j = 0; j < cellCount; j++) {
                    ...
                    if (j == tdIndex) rowHTML += cell;
                }
                int tds = row.getElementCount() - 1;
                if (tds < tdIndex) {
                    for (; tds <= tdIndex; tds++) rowHTML += cell;
                }
    ...
    private String getRowHTML(Element tr) {
        ...
        if (tr.getName().equals("tr")) {
            for (int i = 0; i < tr.getElementCount(); i++) if (tr.getElement(i).getName().equals("td")) trTag += "<td></td>";
        }
        trTag += "</tr>";
        return trTag;
    }
    ...
    protected void updateWysiwygContextState(JEditorPane wysEditor) {
        ...
        if ((isInTd && !isEnabled()) || (isEnabled() && !isInTd)) updateEnabled();
    }
