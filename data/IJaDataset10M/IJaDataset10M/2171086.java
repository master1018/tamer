...
public class ByPartIndexer<KeyType> extends HGKeyIndexer<KeyType, HGPersistentHandle> {
    ...
    private synchronized HGProjection[] getProjections(HyperGraph graph) {
        if (projections == null) {
            ...
            if (type == null) throw new HGException("Could not find type with handle " + getType());
            projections = new HGProjection[dimensionPath.length];
            for (int j = 0; j < dimensionPath.length; j++) {
                if (!(type instanceof HGCompositeType)) return null;
                projections[j] = ((HGCompositeType) type).getProjection(dimensionPath[j]);
                if (projections[j] == null) throw new HGException("There's no projection '" + dimensionPath[j] + "' in type '" + type + "'");
                type = (HGAtomType) graph.get(projections[j].getType());
            }
            HGProjection ours = projections[dimensionPath.length - 1];
            HGHandle enclosingType = dimensionPath.length > 1 ? projections[dimensionPath.length - 2].getType() : getType();
            HGHandle atomProj = hg.findOne(graph, hg.and(hg.type(AtomProjection.class), hg.incident(enclosingType), hg.incident(ours.getType()), hg.eq("name", ours.getName())));
            if (atomProj != null) projectionType = graph.getTypeSystem().getAtomType(HGAtomRef.class); else projectionType = graph.get(ours.getType());
        }
        return projections;
    }
    ...
    public ByteArrayConverter<KeyType> getConverter(HyperGraph graph) {
        if (projectionType == null) getProjections(graph);
    ...
    public KeyType getKey(HyperGraph graph, Object atom) {
        Object result = atom;
        for (HGProjection p : getProjections(graph)) result = p.project(result);
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof ByPartIndexer)) return false;
