...
public class S3MLoader extends ModuleLoader {
    ...
    private Module loadModule(RandomAccess raf) throws InvalidFormatException, IOException {
        ...
        for (int n = 0; n < 32; n++) channelSettings[n] = raf.read();
        int[] orders = new int[numOrders];
        for (int n = 0; n < numOrders; n++) orders[n] = raf.read();
        for (int n = 0; n < numOrders; n++) {
            if (orders[n] > numPatterns) orders[n] = numPatterns;
        }
        Instrument[] instruments = new Instrument[numInstruments];
        for (int n = 0; n < numInstruments; n++) instruments[n] = loadInstrument(raf, n);
        Pattern[] patterns = new Pattern[numPatterns + 1];
        for (int n = 0; n < numPatterns; n++) patterns[n] = loadPattern(raf);
        ...
        for (int n = 0; n < channelSettings.length; n++) {
            ...
            if (channelSetting == 255 || channelSetting == 127) panning[n] = 0.5; else {
                int pan = channelSetting & 0x07f;
                if (pan >= 16) throw new InvalidFormatException("Adlib channels not supported: " + pan);
                panning[n] = pan < 8 ? l : r;
            }
            volume[n] = 1;
        }
        module = new Module(moduleName, id, "ScreamTracker " + parseTrackerVersion(trackerVersion), instruments, patterns, orders, 0, initialTempo, initialSpeed, 1, Module.TRACK_PANNING, volume, panning);
        return module;
    }
    ...
    private Track[] loadEmptyTracks(int numtracks) {
        ...
        for (int n = 0; n < numtracks; n++) {
            ...
            for (int d = 0; d < 64; d++) tracks[n].initDivision(d, Track.NO_INSTRUMENT, Instrument.NO_NOTE, null, null, null);
        }
        return tracks;
    }
    ...
    private Pattern loadPattern(RandomAccess raf) throws IOException {
        ...
        for (int n = 0; n < 64; n++) {
            ...
            while ((data = raf.read() & 0x0ff) != 0) {
                ...
                if ((data & 32) != 0) {
                    note = raf.read() & 0x0ff;
                    if (note == 255) note = Instrument.NO_NOTE; else if (note == 254) {
                        keyOff = true;
    ...
    private int[] translateEffect(int effect, int arg1, int arg2) {
        switch(effect) {
            ...
            case 4:
                if (arg1 != 0x0f && arg2 != 0x0f) return new int[] { Effect.MOD_VOLUME_SLIDE, arg1, arg2 }; else if (arg1 == 0x0f) return new int[] { Effect.MOD_EXTENDED_FINE_VOLUME_SLIDE_DOWN, 0, arg2 }; else if (arg2 == 0x0f) return new int[] { Effect.MOD_EXTENDED_FINE_VOLUME_SLIDE_UP, 0, arg1 };
            case 5:
                if (arg1 < 0x0e) return new int[] { Effect.XM_SLIDE_DOWN, arg1, arg2 }; else if (arg1 == 0x0e) return new int[] { Effect.XM_EXTENDED_FINE_SLIDE_DOWN, 0, arg2 }; else if (arg1 == 0x0f) return new int[] { Effect.XM_EXTRA_FINE_SLIDE_DOWN, 0, arg2 };
            case 6:
                if (arg1 < 0x0e) return new int[] { Effect.XM_SLIDE_UP, arg1, arg2 }; else if (arg1 == 0x0e) return new int[] { Effect.XM_EXTENDED_FINE_SLIDE_UP, 0, arg2 }; else if (arg1 == 0x0f) return new int[] { Effect.XM_EXTRA_FINE_SLIDE_UP, 0, arg2 };
            case 7:
    ...
    private Instrument loadInstrument(RandomAccess raf, int instrumentNum) throws InvalidFormatException, IOException {
        ...
        Sample sample = null;
        if (type != 0) {
            sample = new Sample("sample-" + instrumentNum, instrumentName, defaultVolume / 64.0, 0.5, sampleLength, loop ? Sample.FORWARD : Sample.NO_LOOP, loopBegin, loopEnd - loopBegin, 0, 0, new S3MUnits(c2speed, false));
            sample.setData(readSampleData(raf, sampleOffset, sampleLength, stereo, bits16, instrumentNum));
        }
    ...
    private short[] readSampleData(RandomAccess raf, int offset, int length, boolean stereo, boolean bits16, int sampleNum) throws InvalidFormatException, IOException {
        ...
        if (!stereo && !bits16) {
            ...
            for (int m = 0; m < data.length; m++) data[m] = (short) ((raf.read() & 0x0ff) + ((raf.read() & 0x0ff)) * 256 - 32768);
        } else throw new InvalidFormatException("Unsupported sample format: " + (stereo ? "stereo" : "mono") + " " + (bits16 ? "16" : "8") + " bit");
        raf.seek(pos);
        return data;
    }
    ...
    private String readInstrumentFileName(RandomAccess raf) throws IOException {
        ...
        int count = 0;
        while (count++ < 12) nameSB.append((char) raf.read());
    ...
    private String readInstrumentName(RandomAccess raf) throws IOException {
        ...
        int count = 0;
        int data = 0;
        while (count++ < 28 && ((data = raf.read()) != 0)) nameSB.append((char) data);
        raf.seek(pos + 28);
        return nameSB.toString();
    }
    ...
    private void verifyID(RandomAccess raf) throws IOException, InvalidFormatException {
        int id = raf.read();
        if (id != 0x01a) throw new InvalidFormatException("Id = " + id + ", 0x01A expected!");
    }
    ...
    private void verifyFileType(RandomAccess raf) throws IOException, InvalidFormatException {
        int type = raf.read();
        if (type != 16) throw new InvalidFormatException("Song type = " + type + ", 16 expected");
    }
}
