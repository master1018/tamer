...
import org.waveprotocol.box.server.JsonRpcServlet;
...
public final class SearchServlet extends HttpServlet {
     ...
     */
        public static SearchRequest parseSearchRequest(HttpServletRequest req, HttpServletResponse response) {
            String query = req.getParameter("query");
            String index = req.getParameter("index");
            String numResults = req.getParameter("numResults");
            SearchRequest searchRequest = SearchRequest.newBuilder().setQuery(query).setIndex(Integer.parseInt(index)).setNumResults(Integer.parseInt(numResults)).build();
    ...
    @Inject
    public SearchServlet(SessionManager sessionManager, EventDataConverterManager converterManager, @Named("DataApiRegistry") OperationServiceRegistry operationRegistry, WaveletProvider waveletProvider, ConversationUtil conversationUtil, ProtoSerializer serializer) {
        ...
        JsonRpcServlet.initialize(waveletProvider, operationRegistry, conversationUtil, converterManager);
    }
   ...
   */
    private SearchResponse performSearch(SearchRequest searchRequest, ParticipantId user) {
        ...
        LOG.fine("Performing query: " + searchRequest.getQuery() + " [" + searchRequest.getIndex() + ", " + (searchRequest.getIndex() + searchRequest.getNumResults()) + "]");
        ...
        SearchResult searchResult = (SearchResult) jsonRpcResponse.getData().get(ParamsProperty.SEARCH_RESULTS);
        int totalGuess;
   ...
   */
    private void serializeObjectToServlet(MessageLite message, HttpServletResponse resp) throws IOException {
        if (message == null) {
