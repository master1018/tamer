...
public abstract class LoginBox extends HandlerAwareComposite {
    ...
    protected LoginBox(AuthManager authManager, HistoryProvider historyProvider, EventBus eventBus, IneDispatch ineDispatch) {
        ...
        createUI();
        initWidget(vp);
        if (IFConsts.COOKIE_TRUE.equals(Cookies.getCookie(IFConsts.COOKIE_NEEDSTAYSIGNEDIN))) getCheckBox().setValue(true);
    }
    ...
    private void doLogin() {
        ...
        authManager.doLogin(userName.getText(), password.getText(), captchaWidget.getCaptchaText(), new LoginCallback());
    }
    ...
    protected void doRedirectLogic(AuthStatusResultBase result) {
        ...
        pre.setHierarchicalTokensWithParam(historyProvider.getToken().substring(historyProvider.getToken().indexOf(NavigationProperties.REDIRECT) + NavigationProperties.REDIRECT.length() + PlaceHandler.EQUALS_SIGN.length(), historyProvider.getToken().length()));
