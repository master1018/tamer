...
public class NotifyLeaseHandlerImpl implements LeaseHandler {
    ...
    public long renew(SpaceUID aUID, long aLeaseDuration) throws UnknownLeaseException, LeaseDeniedException, IOException {
        ...
        if (!myResult) throw new UnknownLeaseException();
        log(new LeaseRenewal(myOID, myExpiry));
        return myDuration;
    }

    public void cancel(SpaceUID aUID) throws UnknownLeaseException, IOException {
        boolean myResult;
        OID myOID = ((SpaceNotifyUID) aUID).getOID();
        DiskTxn myTxn = DiskTxn.newTxn();
        try {
    ...
    private static final class LogTask implements Task {
        ...
        public void run() {
            try {
                ...
                EventQueue.theLogger.log(Level.SEVERE, "Failed to log lease action", anException);
            }
        }
    }
}
