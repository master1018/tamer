...
public class Main {
    ...
    static String bytesToString(byte[] b) {
        ...
        int length = b.length;
        for (int i = 0; i < length; ++i) hexBytes.append(Integer.toHexString(b[i] & 0xff));
    ...
    private static final Fileset getFiles(String[] s, int startPos, boolean fileListFromStdin, char separator) {
        if (fileListFromStdin) return new Fileset(System.in, separator); else return new Fileset(s, startPos, s.length);
    }
}
...
class Tokenizer {
    ...
    boolean isSeparator(int c) {
        if (Character.isWhitespace(separator)) return Character.isWhitespace((char) c); else return c == separator;
    }
    ...
    String nextToken() {
        ...
        try {
            ...
            while ((c = r.read()) != -1) {
                if (isSeparator(c)) break; else buf.append((char) c);
            }
...
class Fileset {
    ...
    Fileset(InputStream is, char separator) {
        ...
        String name;
        while (!"".equals(name = st.nextToken())) files.add(new File(name));
    }
