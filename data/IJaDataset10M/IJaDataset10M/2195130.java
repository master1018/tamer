...
public class Parameter implements Serializable {
    ...
    public Parameter(String[] s) throws ec.util.BadParameterException {
        if (s.length == 0) throw new BadParameterException("Parameter created with length 0");
        for (int x = 0; x < s.length; x++) {
            if (s[x] == null) throw new BadParameterException("Parameter created with null string");
            if (x == 0) param = s[x]; else param += (delimiter + s[x]);
        }
    }

    /** Creates a new parameter from the single path item in s. */
    public Parameter(String s) throws BadParameterException {
        if (s == null) throw new BadParameterException("Parameter created with null string");
        param = s;
    }
    ...
    public Parameter(String s, String[] s2) {
        ...
        for (int x = 0; x < s2.length; x++) {
            if (s2[x] == null) throw new BadParameterException("Parameter created with null string"); else param += (delimiter + s2[x]);
        }
    }

    /** Returns a new parameter with s added to the end of the current path items. */
    public Parameter push(String s) {
        if (s == null) throw new BadParameterException("Parameter pushed with null string");
    ...
    public Parameter pop() {
        ...
        if (x == -1) return null; else return new Parameter(param.substring(0, x));
    }
    ...
    public Parameter popn(int n) {
        ...
        for (int y = 0; y < n; y++) {
            ...
            if (x == -1) return null; else s = param.substring(0, x);
        }
    ...
    public String top() {
        int x = param.lastIndexOf(delimiter);
        if (x == -1) return param; else return param.substring(x + 1);
    }
