...
public abstract class VM_TypeDescriptorParsing implements VM_ClassLoaderConstants {
   ...
   * <code>String.charAt()</code> method inexpensive?</small> */
    public static boolean isJavaClassName(String s) throws InterruptiblePragma {
   ...
   * first and last characters of the array that are to be checked. */
    public static boolean isJavaClassNameInternalForm(char val[], int first, int last) {
        if (val[first++] != ClassTypeCode) return false;
    ...
    public static void validateAsTypeDescriptor(VM_Atom a) throws IllegalArgumentException, InterruptiblePragma {
        try {
            String s = a.toUnicodeString();
        } catch (java.io.UTFDataFormatException udfe) {
            IllegalArgumentException iae = new IllegalArgumentException("The atom in question does not represent a valid UTF8 string, so it's not a type descriptor.");
            iae.initCause(udfe);
            throw iae;
        }
    }
  ...
  */
    public static void validateAsTypeDescriptor(String s) throws IllegalArgumentException, InterruptiblePragma {
        char val[] = s.toCharArray();
        int i = 0;
        if (val.length == 0) malformed("is the empty string", s);
        while (val[i] == '[') {
            if (++i >= val.length) malformed("has just '[' chars", s);
        }
        if (VM.VerifyAssertions) VM._assert(i < val.length);
        if (val[i] == VoidTypeCode && i != 0) malformed("can't have an array of void", s);
        if (isJavaPrimitive(val[i])) {
            if (i != val.length - 1) malformed("nothing should follow the primitive typecode '" + Character.toString(val[i]) + "'", s);
            return;
        }
        if (VM.VerifyAssertions) VM._assert(val[i] != '[' && !isJavaPrimitive(val[i]));
        if (val[i] != ClassTypeCode) malformed("unknown character '" + Character.toString(val[i]) + "'", s);
        if (!isJavaClassNameInternalForm(val, i, val.length - 1)) malformed("doesn't end with a valid class name in internal form", s);
    }

    private static boolean isJavaPrimitive(char c) {
        byte b = (byte) c;
        if (c != (char) b) return false;
   ...
   * malformed type name. */
    private static void malformed(String msg, String typeName) throws IllegalArgumentException {
        throw new IllegalArgumentException("Malformed type name" + ((msg == null) ? "" : ": " + msg) + ": \"" + typeName + "\"");
    }
}
