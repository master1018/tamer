...
final class IterStreamReader implements XMLStreamReader {
   ...
   */
    ...
    /** Attribute types. */
    private static final String[] ATTYPES = { "ID", "IDREF", "IDREFS", "NMTOKEN", "NMTOKENS", "ENTITY", "ENTITIES" };
    ...
    public String getAttributeValue(final String s, final String s1) {
        ...
        for (int a = 0; a < atts.size(); a++) {
            ...
            if (s == null || s.equals(getAttributeNamespace(a))) return getAttributeValue(a);
        }
        return null;
    }
    ...
    public int getTextCharacters(final int ss, final char[] ac, final int ts, final int l) {
        checkType(CHARACTERS, COMMENT);
    ...
    public int next() throws XMLStreamException {
        if (next && item == null || !next && !hasNext()) throw new NoSuchElementException();
        next = false;
    ...
    public int nextTag() throws XMLStreamException {
        next();
        while (kind == CHARACTERS && isWhiteSpace() || kind == CDATA && isWhiteSpace() || kind == SPACE || kind == PROCESSING_INSTRUCTION || kind == COMMENT) {
            next();
        }
        checkType(START_ELEMENT, END_ELEMENT);
        return kind;
    ...
    public void require(final int t, final String uri, final String ln) throws XMLStreamException {
        checkType(t);
