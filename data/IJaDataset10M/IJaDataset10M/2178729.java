...
public abstract class Averagable extends ResultObjectAdapter implements Cloneable, Readable {
    ...
    public final void buildAverage(Averagable averagable) {
        if (!averagable.getClass().equals(this.getClass())) throw new RuntimeException("Cannot build average of different averagable types (" + this.getClass().getName() + "/" + averagable.getClass().getName() + ").");
        if (!averagable.getName().equals(this.getName())) throw new RuntimeException("Cannot build average of different averagable types (" + this.getName() + "/" + averagable.getName() + ").");
        if (averageCount == 0) {
    ...
    public final double getAverage() {
        ...
        if (averageCount > 0) average = getMakroAverage();
        if (Double.isNaN(average)) average = getMikroAverage();
        return average;
    }
    ...
    public final double getVariance() {
        ...
        if (averageCount > 0) variance = getMakroVariance();
        if (Double.isNaN(variance)) variance = getMikroVariance();
        return variance;
    }
    ...
    public final double getStandardDeviation() {
        ...
        if (averageCount > 0) sd = getMakroStandardDeviation();
        if (Double.isNaN(sd)) sd = getMikroStandardDeviation();
        return sd;
    }
    ...
    public final double getMikroStandardDeviation() {
        ...
        if (Double.isNaN(variance)) return Double.NaN; else if (variance < 0.0d) return 0.0d; else return Math.sqrt(variance);
    }
    ...
    public final double getMakroVariance() {
        ...
        if (Double.isNaN(result)) return Double.NaN; else if (result < 0.0d) return 0.0d; else return result;
    }
    ...
    private final String formatValue(double value) {
        if (Double.isNaN(value)) return "unknown"; else {
            if (formatPercent()) {
    ...
    public String toString() {
        ...
        if (averageCount > 0) {
            ...
            if (!Double.isNaN(makroAverage)) {
                ...
                if (!Double.isNaN(sd)) result.append(" +/- " + formatDeviation(sd));
            }
        }
        if (makroUsable) {
            result.append(" (mikro: ");
