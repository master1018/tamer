...
public class DocumentHistory extends BaseObject implements Serializable {
    ...
    public void setReferenceNos(String referenceNos) {
    ...
    public boolean equals(Object object) {
        if (!(object instanceof DocumentHistory)) {
            ...
            DocumentHistory rhs = (DocumentHistory) object;
            return (new EqualsBuilder()).append(userId, rhs.userId).append(sentToDate, rhs.sentToDate).append(pdfFilePath, rhs.pdfFilePath).append(receivedFromRef, rhs.receivedFromRef).append(orderBy, rhs.orderBy).append(pdfBytes, rhs.pdfBytes).append(date, rhs.date).append(id, rhs.id).append(changesAsString, rhs.changesAsString).append(docFilePath, rhs.docFilePath).append(keywordRefs, rhs.keywordRefs).append(sentToRef, rhs.sentToRef).append(body, rhs.body).append(referenceNos, rhs.referenceNos).append(docBytes, rhs.docBytes).append(sentTo, rhs.sentTo).append(action, rhs.action).append(documentId, rhs.documentId).append(sentVia, rhs.sentVia).append(receivedFromDate, rhs.receivedFromDate).append(statusId, rhs.statusId).append(receivedFrom, rhs.receivedFrom).append(sentViaRef, rhs.sentViaRef).append(subject, rhs.subject).append(sentViaDate, rhs.sentViaDate).isEquals();
        }
    }

    public String toString() {
        return (new ToStringBuilder(this)).append("sentVia", sentVia).append("action", action).append("id", id).append("statusId", statusId).append("referenceNos", referenceNos).append("body", body).append("pdfBytes", pdfBytes).append("baseSearchClass", getBaseSearchClass()).append("sentViaRef", sentViaRef).append("sentToRef", sentToRef).append("date", date).append("orderBy", orderBy).append("documentId", documentId).append("userId", userId).append("changesAsString", changesAsString).append("sentToDate", sentToDate).append("keywordRefs", keywordRefs).append("receivedFromRef", receivedFromRef).append("user", user).append("receivedFrom", receivedFrom).append("sentTo", sentTo).append("docBytes", docBytes).append("docFilePath", docFilePath).append("subject", subject).append("sentViaDate", sentViaDate).append("status", status).append("receivedFromDate", receivedFromDate).append("pdfFilePath", pdfFilePath).toString();
    }

    public int hashCode() {
        return (new HashCodeBuilder(0x75b41561, 0x6286fed5)).append(userId).append(sentToDate).append(pdfFilePath).append(receivedFromRef).append(orderBy).append(pdfBytes).append(date).append(id).append(changesAsString).append(docFilePath).append(keywordRefs).append(sentToRef).append(body).append(referenceNos).append(docBytes).append(sentTo).append(user).append(action).append(documentId).append(sentVia).append(receivedFromDate).append(statusId).append(status).append(receivedFrom).append(sentViaRef).append(subject).append(sentViaDate).toHashCode();
    }
}
