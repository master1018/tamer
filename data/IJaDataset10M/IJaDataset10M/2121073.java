...
public class VarTreeOps {
    ...
    public static <F extends RankedSymbol, V extends Variable> Set<F> computeRankedSymbols(Tree<? extends BiSymbol<F, V>> varTree) {
        ...
        while (!toDo.isEmpty()) {
            ...
            if (s.getSymbol().isInnerType()) ret.add(s.getSymbol().asInnerSymbol());
            for (Tree<? extends BiSymbol<F, V>> s0 : s.getSubTrees()) toDo.push(s0);
        }
        return ret;
    }
	 ...
	 */
    public static <F extends RankedSymbol, V extends Variable, U extends Tree<F>> U replaceVariables(Tree<? extends BiSymbol<F, V>> varTree, List<U> replaceTrees, TreeCreator<F, U> tc) {
        if (varTree.getSymbol().isLeafType()) {
            ...
            } else return replaceTrees.get(nr);
        } else {
	 ...
	 */
    public static <F extends RankedSymbol, V extends Variable, U extends Tree<BiSymbol<F, V>>> U replaceOneVariable(Tree<? extends BiSymbol<F, V>> varTree, U replaceTree, TreeCreator<BiSymbol<F, V>, U> tc) {
        if (varTree == null) {
	 ...
	 * @return a tree on which the variables are replaced by the trees specified in the given list
	 */
    public static <F extends RankedSymbol, V extends Variable, U extends Tree<F>> U replaceVariables(Tree<BiSymbol<F, V>> varTree, Map<V, U> replaceTrees, TreeCreator<F, U> tc) {
        if (varTree.getSymbol().isLeafType()) {
            V v = varTree.getSymbol().asLeafSymbol();
            ...
            } else return replaceTrees.get(varTree.getSymbol().asLeafSymbol());
        } else {
            F sf = varTree.getSymbol().asInnerSymbol();
	 ...
	 * via the given map 
	 */
    public static <F extends RankedSymbol, W extends Variable, Y extends Tree<BiSymbol<F, W>>> Y makeTreeToBiTree(Tree<F> vTree, Map<F, ? extends W> map, TreeCreator<BiSymbol<F, W>, Y> tc) {
        F symbol = vTree.getSymbol();
        if (map.containsKey(symbol)) {
            if (symbol.getArity() != 0) throw new IllegalArgumentException("All symbols that shall be replaced must have arity 0.");
