package com.eptica.ias.models.accounts.salutation.impl;

import java.util.*;
import org.hibernate.criterion.*;
import org.hibernate.*;
import org.springframework.orm.hibernate3.SessionFactoryUtils;
import com.eptica.ias.models.accounts.salutation.*;
import com.eptica.ias.util.*;

/**
 * Hibernate implementation of the SalutationModel interface.<br>
 ...
 */
public class SalutationDAOHibernate implements SalutationDAO {

    private static final org.apache.commons.logging.Log logger = org.apache.commons.logging.LogFactory.getLog(SalutationDAOHibernate.class);

    private static final String[] stringColumns = new String[] { "nameLocale" };

    /**
     * Get a SalutationModel using the primary key value
     *
     * @param salutationId the primary key value
     * @return the corresponding SalutationModel, null otherwise
     */
    private SalutationModel get(final Integer salutationId) {
        SalutationModel modelFound = null;
        try {
            modelFound = (SalutationModel) getCurrentSession().get(SalutationModel.class, salutationId);
        } catch (HibernateException he) {
            throw SessionFactoryUtils.convertHibernateAccessException(he);
        }
        if (modelFound == null && logger.isDebugEnabled()) {
            logger.debug("get returned null with pk=" + salutationId);
        }
        return modelFound;
    }

    /**
     * get a SalutationModel from an object
     *
     * @param salutation The SalutationModel sample to search for
     * @param searchTemplate the specific parameters such as named queries, extra infos, limitations, order, ...
     * @return the corresponding SalutationModel, null otherwise
     */
    public SalutationModel get(final SalutationModel salutation) {
        AssertUtil.notNull(salutation, "The salutation cannot be null");
        AssertUtil.isTrue(salutation.hasPrimaryKey(), "The salutation must have its primary key set. " + salutation.toString());
        return get(salutation.getSalutationId());
    }

    /**
     * Search a SalutationModel using a sample. 
     *
     * @param salutation The SalutationModel sample to search for
     * @param searchTemplate the specific parameters such as named queries, extra infos, limitations, order, ...
     * @return the resulting collection of SalutationModel
     */
    @SuppressWarnings("unchecked")
    public Collection<SalutationModel> find(final SalutationModel salutation, final SearchTemplate searchTemplate) {
        AssertUtil.notNull(salutation, "The salutation cannot be null");
        prepareSearchTemplate(searchTemplate);
        if (searchTemplate != null && searchTemplate.hasNamedQuery()) {
            return namedQueryUtil.findByNamedQuery(searchTemplate, salutation);
        }
        Criteria criteria = getCriteria(salutation, searchTemplate);
        ...
        return (Collection<SalutationModel>) c;
    }

    /**
     * return the number of salutation that matches the given model using the defined searchMode
     *
     * @param salutation the model to look for
     * @param searchTemplate the specific parameters such as named queries, extra infos, limitations, order, ...
     * @return the number of matched salutation 
     */
    public int findCount(final SalutationModel salutation, final SearchTemplate searchTemplate) {
        AssertUtil.notNull(salutation, "The salutation cannot be null");
        prepareSearchTemplate(searchTemplate);
        if (searchTemplate != null && searchTemplate.hasNamedQuery()) {
            return namedQueryUtil.numberByNamedQuery(searchTemplate, salutation).intValue();
        }
        Criteria criteria = getCriteria(salutation, searchTemplate);
     ...
     */
    private Criteria getCriteria(final SalutationModel salutation, final SearchTemplate searchTemplate) {
        AssertUtil.notNull(salutation, "The salutation cannot be null");
        Criteria criteria = getCurrentSession().createCriteria(SalutationModel.class);
        ...
        if (salutation != null) {
            criteria.add(getExample(salutation, searchTemplate));
        }
        return criteria;
    }
     ...
     */
    private Example getExample(final SalutationModel salutation, final SearchTemplate searchTemplate) {
        AssertUtil.notNull(salutation, "The salutation cannot be null");
        Example example = Example.create(salutation);
        example = example.excludeZeroes();
        if (!salutation.hasSalutationId()) {
            example.excludeProperty("salutationId");
        }
        if (!salutation.hasNameLocale()) {
    ...
    /**
     * Save a collection of SalutationModel.
     *
     * @param salutations The salutations collection to save
     * @param searchTemplate the specific parameters such as named queries, extra infos, limitations, order, ...
     */
    public void save(final Collection<SalutationModel> salutations) {
        AssertUtil.notNull(salutations, "The salutations collection must not be null");
        if (logger.isDebugEnabled()) {
            logger.debug("About to save a collection of salutations (" + salutations.size() + " items)");
        }
        for (SalutationModel salutation : salutations) {
            AssertUtil.notNull(salutation, "The table.modelsVar collection must not contain null item");
            try {
                getCurrentSession().saveOrUpdate(salutation);
            } catch (HibernateException he) {
                throw SessionFactoryUtils.convertHibernateAccessException(he);
            }
        }
        if (logger.isDebugEnabled()) {
            logger.debug("Save collection of salutations done");
        }
    }

    /**
     * Delete a collection of SalutationModel.
     *
     * @param salutations The salutations collection to delete
     * @param searchTemplate the specific parameters such as named queries, extra infos, limitations, order, ...
     */
    public void delete(final Collection<SalutationModel> salutations) {
        AssertUtil.notNull(salutations, "The salutations collection must not be null");
        if (logger.isDebugEnabled()) {
            logger.debug("About to delete a collection of salutations (" + salutations.size() + " items)");
        }
        for (SalutationModel salutation : salutations) {
            AssertUtil.notNull(salutation, "The table.modelsVar collection must not contain null item");
            AssertUtil.isTrue(salutation.hasPrimaryKey(), "The table.modelsVar collection must not contain item with no primary key set");
            try {
    ...
    private String cacheRegion = "com.eptica.ias.models.accounts.salutation.SalutationModel";
