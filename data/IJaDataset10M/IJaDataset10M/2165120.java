...
public class JavaAnalysisPanel extends JPanel implements ActionListener {
    ...
    protected static final org.apache.log4j.Logger Log = org.apache.log4j.Logger.getLogger(JavaAnalysisPanel.class);

    protected static final String PUBLIC_KEYWORD = "public", ABSTRACT_KEYWORD = "abstract", FINAL_KEYWORD = "final", CLASS_KEYWORD = "class", INTERFACE_KEYWORD = "interface", EXTENDS_KEYWORD = "extends", IMPLEMENTS_KEYWORD = "implements", METADATA_CATEGORY = "Java Analysis", METADATA_KEYWORD_UNDECLARED = "undeclared", METADATA_KEYWORD_DECLARED = "declared in ", JAVA_ANALYSIS = VueResources.getString("analyze.java.javaAnalysis"), HELP_TEXT = VueResources.getString("dockWindow.JavaAnalysis.helpText"), DEFAULT_FILE_CHOOSER_TITLE = VueResources.getString("FileChooser.openDialogTitleText"), DEFAULT_FILE_CHOOSER_OPEN_BUTTON = VueResources.getString("FileChooser.openButtonText"), DEFAULT_FILE_CHOOSER_OPEN_BUTTON_TOOLTIP = VueResources.getString("FileChooser.openButtonToolTipText"), CHOOSE_FILES = VueResources.getString("analyze.java.chooseFiles"), ANALYZE_CLASSES = VueResources.getString("analyze.java.analyzeClasses"), ANALYZE_INNER_CLASSES = VueResources.getString("analyze.java.analyzeInnerClasses"), ANALYZE_INTERFACES = VueResources.getString("analyze.java.analyzeInterfaces"), ANALYZE_INNER_INTERFACES = VueResources.getString("analyze.java.analyzeInnerInterfaces"), ANALYZE = VueResources.getString("analyze.java.analyze"), ANALYZE_TOOLTIP = VueResources.getString("analyze.java.analyzeTooltip"), STOP = VueResources.getString("analyze.java.stop"), PARSING = VueResources.getString("analyze.java.parsing"), DISPLAYING = VueResources.getString("analyze.java.displaying"), NO_JAVA_CLASS = VueResources.getString("analyze.java.noJavaClass"), JAVA_ANALYSIS_ERROR = VueResources.getString("analyze.java.error");

    protected static final Color DECLARED_COLOR = Color.BLACK, UNDECLARED_COLOR = Color.LIGHT_GRAY;

    protected static final LWComponent.StrokeStyle ABSTRACT_STROKE_STYLE = LWComponent.StrokeStyle.DASHED, NONABSTRACT_STROKE_STYLE = LWComponent.StrokeStyle.SOLID;

    protected static final float FINAL_STROKE_WIDTH = 2f, NONFINAL_STROKE_WIDTH = 1f;

    protected static final Class<? extends RectangularShape> CLASS_SHAPE = Ellipse2D.Float.class, INTERFACE_SHAPE = Rectangle2D.Float.class;

    protected static final char DOLLAR_TOKEN = '$', UNDERSCORE_TOKEN = '_', COMMA_TOKEN = ',', SEMICOLON_TOKEN = ';', LEFT_ANGLE_TOKEN = '<', RIGHT_ANGLE_TOKEN = '>';

    protected static final int CLASSES_MASK = 1, INNER_CLASSES_MASK = 2, INTERFACES_MASK = 4, INNER_INTERFACES_MASK = 8, HALF_GUTTER = 4;
    ...
    protected JCheckBox classesCheckBox = null, innerClassesCheckBox = null, interfacesCheckBox = null, innerInterfacesCheckBox = null;
    ...
    protected void analyze(int mask) {
        ...
        if (files != null) {
            ...
            double viewerCenterX = viewerCenter.getX(), viewerCenterY = viewerCenter.getY();
            if (DEBUG) {
    ...
    protected int parseSourceFile(File file, String filename, int mask, double x, double y) throws java.io.IOException {
        int foundCount = 0;
        boolean analyzeClasses = (mask & CLASSES_MASK) == CLASSES_MASK, analyzeInnerClasses = (mask & INNER_CLASSES_MASK) == INNER_CLASSES_MASK, analyzeInterfaces = (mask & INTERFACES_MASK) == INTERFACES_MASK, analyzeInnerInterfaces = (mask & INNER_INTERFACES_MASK) == INNER_INTERFACES_MASK, isPublic = false, isAbstract = false, isFinal = false;
        FileReader stream = new FileReader(file);
        BufferedReader reader = new BufferedReader(stream);
        StreamTokenizer tokenizer = new StreamTokenizer(reader);
        String sourceName = filename.replace(JavaSourceFileFilter.FILE_EXTENSION, "");
        if (DEBUG) {
            Log.info("parseSourceFile() analyzing " + filename + " for" + (analyzeClasses ? " classes" + (analyzeInnerClasses ? " (including inner classes)" : "") : "") + (analyzeInterfaces ? (analyzeClasses ? "," : "") + " interfaces" + ((analyzeInnerInterfaces ? " (including inner interfaces)" : "")) : "") + ".");
        }
                ...
                if (tokenizer.sval.equals(PUBLIC_KEYWORD)) {
                        ...
                        if (tokenizer.nextToken() == StreamTokenizer.TT_WORD) {
                            ...
                            if (tokenizer.nextToken() == StreamTokenizer.TT_WORD) {
                                ...
                                if (analyzeInterfaces) {
                                    if (tokenizer.nextToken() == StreamTokenizer.TT_WORD && tokenizer.sval.equals(IMPLEMENTS_KEYWORD)) {
                    ...
                    if (analyzeInterfaces) {
                        if (tokenizer.nextToken() == StreamTokenizer.TT_WORD) {
                            ...
                            skipGeneric(tokenizer);
                            if (tokenizer.nextToken() == StreamTokenizer.TT_WORD && tokenizer.sval.equals(EXTENDS_KEYWORD)) {
                                boolean parsingInterfaces = true;
    ...
    protected int parseClassFile(DataInputStream stream, String filename, int mask, double x, double y) throws java.io.IOException {
        int foundCount = 0;
        boolean analyzeClasses = (mask & CLASSES_MASK) == CLASSES_MASK, analyzeInnerClasses = (mask & INNER_CLASSES_MASK) == INNER_CLASSES_MASK, analyzeInterfaces = (mask & INTERFACES_MASK) == INTERFACES_MASK, analyzeInnerInterfaces = (mask & INNER_INTERFACES_MASK) == INNER_INTERFACES_MASK;
        if (DEBUG) {
            Log.info("parseClassFile() analyzing " + filename + " for" + (analyzeClasses ? " classes" + (analyzeInnerClasses ? " (including inner classes)" : "") : "") + (analyzeInterfaces ? (analyzeClasses ? "," : "") + " interfaces" + ((analyzeInnerInterfaces ? " (including inner interfaces)" : "")) : "") + ".");
        }
        ...
        if (classInfo.isJavaClass) {
            ...
            String classNameWithoutPackage = classRef.nameWithoutPackage, sourceName = attributeInfo.sourceName;
            ...
            if (accessFlags.isInterface) {
                if (analyzeInterfaces && (analyzeInnerInterfaces || isNotInner)) {
                    newInterface(accessFlags.isPublic, accessFlags.isAbstract, accessFlags.isFinal, (isNotInner ? null : sourceName), classRef.name, interfaces.names, x, y);
                }
            } else {
                if (analyzeClasses && (analyzeInnerClasses || isNotInner)) {
                    newClass(accessFlags.isPublic, accessFlags.isAbstract, accessFlags.isFinal, (isNotInner ? null : sourceName), classRef.name, superclassRef.name, (analyzeInterfaces ? interfaces.names : null), x, y);
                }
            }
            foundCount = 1;
        }
    ...
    protected void newClass(boolean isPublic, boolean isAbstract, boolean isFinal, String sourceName, String className, String extendsClassName, ArrayList<String> implementsInterfaceNames, double x, double y) {
        StringBuffer debugMessage = null;
        LWNode classNode = findOrCreateClassNode(className, x, y);
        if (DEBUG) {
            debugMessage = new StringBuffer("newClass() found " + (isPublic ? "public " : "") + (isAbstract ? "abstract " : "") + (isFinal ? "final " : "") + "class " + className);
        }
    ...
    protected void newInterface(boolean isPublic, boolean isAbstract, boolean isFinal, String sourceName, String interfaceName, ArrayList<String> extendsInterfaceNames, double x, double y) {
        StringBuffer debugMessage = null;
        LWNode interfaceNode = findOrCreateInterfaceNode(interfaceName, x, y);
        if (DEBUG) {
            debugMessage = new StringBuffer("newInterface() found " + (isPublic ? "public " : "") + (isAbstract ? "abstract " : "") + (isFinal ? "final " : "") + "interface " + interfaceName);
        }
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (source == classesCheckBox || source == interfacesCheckBox) {
            ...
            int analysisMask = (classesCheckBox.isSelected() ? CLASSES_MASK : 0) | (innerClassesCheckBox.isEnabled() && innerClassesCheckBox.isSelected() ? INNER_CLASSES_MASK : 0) | (interfacesCheckBox.isSelected() ? INTERFACES_MASK : 0) | (innerInterfacesCheckBox.isEnabled() && innerInterfacesCheckBox.isSelected() ? INNER_INTERFACES_MASK : 0);
    ...
    protected static void addToGridBag(Container container, Component component, int gridX, int gridY, int gridWidth, int gridHeight, int anchor, int fill, double weightX, double weightY, Insets insets) {
        GridBagConstraints constraints = new GridBagConstraints(gridX, gridY, gridWidth, gridHeight, weightX, weightY, anchor, fill, insets, 0, 0);
    ...
    private class ClassInfo {
        ...
        ClassInfo(DataInputStream stream) throws java.io.IOException {
            ...
            if (isJavaClass) {
                short compilerMinorVersion = stream.readShort(), compilerMajorVersion = stream.readShort();
                if (DEBUG) {
    ...
    private class AccessFlags {
        ...
        protected static final int ACC_PUBLIC = 0x0001, ACC_FINAL = 0x0010, ACC_SUPER = 0x0020, ACC_INTERFACE = 0x0200, ACC_ABSTRACT = 0x0400;
    ...
    private class FieldInfo {
        ...
        FieldInfo(DataInputStream stream) throws java.io.IOException {
            ...
            for (int fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++) {
                short accessFlags = stream.readShort(), nameIndex = stream.readShort(), descriptorIndex = stream.readShort(), attributeCount = stream.readShort();
