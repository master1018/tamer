...
public class Discretizer {
    ...
    private Vector<Double> splitPoints;
    ...
    private void discretize() {
        ...
        while (iteration < 15 && heap.getMax() != null && heap.getMax().interval.getFrequency() > (0.2 * totalFreq)) {
            ...
            if (leftSplit != null) heap.add(leftSplit);
            ...
            if (rightSplit != null) heap.add(rightSplit);
            iteration++;
        }
    }
    ...
    private HeapItem getBestSplit(Interval interval) {
        ...
        if (bestScore > -100000.0) return new HeapItem(interval, bestScore, bestSplit);
        return null;
    }
    ...
    private double getScore(Interval interval, int split) {
        ...
        int end = interval.getEndIdx();
        int leftFreq = 0;
        for (int i = start; i < split; i++) leftFreq += (frequencies.elementAt(i)).intValue();
        int rightFreq = 0;
        for (int i = split; i < end; i++) rightFreq += (frequencies.elementAt(i)).intValue();
    ...
    private class Interval {
        ...
        public Interval(int start, int end) {
            ...
            endIdx = end;
            frequency = 0;
            for (int i = startIdx; i < endIdx; i++) frequency += (frequencies.elementAt(i)).intValue();
    ...
    private class MaxHeap {
        ...
        public HeapItem getMax() {
            if (items.size() > 0) return items.elementAt(0);
            return null;
        }
        ...
        private void setItemAt(int idx) {
            if (items.size() > idx) {
                int max = idx;
                if (items.size() > getLeft(idx) && getValueAt(getLeft(idx)) > getValueAt(idx)) max = getLeft(idx);
                if (items.size() > getRight(idx) && getValueAt(getRight(idx)) > getValueAt(idx)) max = getRight(idx);
                if (max != idx) {
                    HeapItem temp = items.elementAt(idx);
        ...
        private double getValueAt(int idx) {
            if (items.size() > idx) return (items.elementAt(idx)).score;
