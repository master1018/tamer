...
public abstract class AbstractCompressionUriHandler extends AbstractUriHandler {
    ...
    public final void handle(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        boolean compressionEnabled = !parameterService.is(ParameterConstants.TRANSPORT_HTTP_COMPRESSION_DISABLED_SERVLET);
        if (compressionEnabled) {
            int compressionLevel = parameterService.getInt(ParameterConstants.TRANSPORT_HTTP_COMPRESSION_LEVEL);
            int compressionStrategy = parameterService.getInt(ParameterConstants.TRANSPORT_HTTP_COMPRESSION_STRATEGY);
            log.debug("@doFilter");
            ...
            @SuppressWarnings("rawtypes") Enumeration e = req.getHeaders("Accept-Encoding");
    ...
    protected abstract void handleWithCompression(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException;
}
