...
import org.hyperion.rs2.content.MagicHandler;
...
import org.hyperion.rs2.net.mysql.DatabaseManager;
...
import org.hyperion.util.xStreamManager;
...
public class World {
    ...
    private void loadConfiguration() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        try {
            parser = new ConfigurationParser(fis);
            ...
            Map<String, Map<String, String>> complexMappings = parser.getComplexMappings();
    ...
    public void load(final PlayerDetails pd) {
        engine.submitWork(new Runnable() {
            ...
            public void run() {
                ...
                if (GameEngine.isStartedSystemUpdate()) {
                    code = 14;
                }
                if (Constants.sqlEnabled) {
                    lr.getPlayer().getId();
                    lr.getPlayer().getOffenses();
                    lr.getPlayer().getRightss();
                    lr.getPlayer().getMessages();
                    if (lr.getPlayer().blackMarks >= 10) {
                        code = 4;
                    }
                }
                if (code != 2) {
    ...
    /**
     * Gets the player ID by username
     *
