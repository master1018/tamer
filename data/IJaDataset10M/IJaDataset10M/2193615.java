...
public class Interpreter implements Opcodes {
    ...
    private static final int CMP_GREATER_MASK = (1 << CMP_GT) | (1 << CMP_GE) | (1 << CMP_NE);

    private static final int CMP_LESS_MASK = (1 << CMP_LT) | (1 << CMP_LE) | (1 << CMP_NE);

    private static final int CMP_EQUAL_MASK = (1 << CMP_GE) | (1 << CMP_LE) | (1 << CMP_EQ);
    ...
    private Value[] fillParameters(BasicBlocks bb, Object cls, Object[] params) {
        Value[] locals = new Value[bb.getMaxLocals()];
        for (int i = 0; i < locals.length; i++) locals[i] = new Value();
        String myType = bb.getMethodInfo().getType();
        String[] myParamTypes = TypeSignature.getParameterTypes(myType);
        int slot = 0;
        if (!bb.getMethodInfo().isStatic()) locals[slot++].setObject(cls);
    ...
    public Object interpretMethod(BasicBlocks bb, Object instance, Object[] myParams) throws InterpreterException, InvocationTargetException {
        if ((GlobalOptions.debuggingFlags & GlobalOptions.DEBUG_INTERPRT) != 0) GlobalOptions.err.println("Interpreting " + bb);
        Value[] locals = fillParameters(bb, instance, myParams);
        Value[] stack = new Value[bb.getMaxStack()];
        for (int i = 0; i < stack.length; i++) stack[i] = new Value();
        ...
        big_loop: for (; ; ) {
            if (iter == null || !iter.hasNext()) {
                if (nextBlock == null) return Void.TYPE;
            ...
            try {
                ...
                if ((GlobalOptions.debuggingFlags & GlobalOptions.DEBUG_INTERPRT) != 0) {
                    ...
                    for (int i = 0; i < stacktop; i++) {
                        if (i > 0) GlobalOptions.err.print(",");
                    ...
                    for (int i = 0; i < locals.length; i++) GlobalOptions.err.print(locals[i] + ",");
                    GlobalOptions.err.println("]");
                }
                ...
                switch(opcode) {
                    ...
                    case opc_saload:
                        {
                            ...
                            if (opcode == opc_laload || opcode == opc_daload) stacktop++;
                            break;
                        }
                    case opc_istore:
                    case opc_fstore:
                    ...
                    case opc_sastore:
                        {
                            ...
                            try {
                                switch(opcode) {
                                    case opc_bastore:
                                        if (array instanceof byte[]) ((byte[]) array)[index] = (byte) value.intValue(); else ((boolean[]) array)[index] = value.intValue() != 0;
                                        break;
                                    case opc_castore:
                    ...
                    case opc_dup_x2:
                        {
                            ...
                            for (int i = 0; i < depth + 1; i++) stack[stacktop - i].setValue(stack[stacktop - i - 1]);
                    ...
                    case opc_dup2_x2:
                        {
                            ...
                            for (int i = 0; i < depth + 2; i++) stack[stacktop + 1 - i].setValue(stack[stacktop - 1 - i]);
                    ...
                    case opc_jsr_w:
                        ...
                        break;
                    case opc_ret:
                        nextBlock = (Block) locals[instr.getLocalSlot()].objectValue();
                        break;
                    ...
                    case opc_getfield:
                        {
                            ...
                            if (cls == null) throw new InvocationTargetException(new NullPointerException());
                    ...
                    case opc_invokeinterface:
                        {
                            ...
                            if (opcode == opc_invokespecial && ref.getName().equals("<init>") && stack[--stacktop].getNewObject() != null) {
                                ...
                                if (!newObj.getType().equals(ref.getClazz())) throw new InterpreterException("constructor doesn't match new");
                    ...
                    case opc_arraylength:
                        {
                            Object array = stack[--stacktop].objectValue();
                            if (array == null) throw new InvocationTargetException(new NullPointerException());
                    ...
                    case opc_checkcast:
                        {
                            ...
                            if (obj != null && !env.instanceOf(obj, instr.getClazzType())) throw new InvocationTargetException(new ClassCastException(obj.getClass().getName()));
                            break;
                        }
                    case opc_instanceof:
                        {
                    ...
                    case opc_multianewarray:
                        {
                            ...
                            for (int i = dimension - 1; i >= 0; i--) dims[i] = stack[--stacktop].intValue();
                            try {
                ...
                for (int i = 0; i < handlers.length; i++) {
                    if (handlers[i].getType() == null || env.instanceOf(obj, handlers[i].getType())) {
                        stacktop = 0;
