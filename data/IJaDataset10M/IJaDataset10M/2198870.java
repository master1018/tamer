...
public class InteractionBuilder {
    ...
    public InteractionBuilder setFixedCount(Object count) {
        ...
        } else minCount = maxCount = convertCount(count, true);
        return this;
    }
    ...
    public InteractionBuilder setRangeCount(Object minCount, Object maxCount, boolean inclusive) {
        ...
        if (this.minCount > this.maxCount) throw new InvalidSpecException("lower bound of invocation count must come before upper bound");
        return this;
    }
    ...
    public InteractionBuilder addEqualPropertyName(String name) {
        if (name.equals(Specification._.toString())) invConstraints.add(WildcardMethodNameConstraint.INSTANCE); else invConstraints.add(new EqualPropertyNameConstraint(name));
        return this;
    }
    ...
    public InteractionBuilder addEqualMethodName(String name) {
        if (name.equals(Specification._.toString())) invConstraints.add(WildcardMethodNameConstraint.INSTANCE); else invConstraints.add(new EqualMethodNameConstraint(name));
        return this;
    }
    ...
    public InteractionBuilder setArgListKind(boolean isPositional) {
        ...
        if (isPositional) invConstraints.add(new PositionalArgumentListConstraint(argConstraints)); else {
    ...
    public InteractionBuilder addEqualArg(Object arg) {
        argConstraints.add(arg == Specification._ ? WildcardArgumentConstraint.INSTANCE : new EqualArgumentConstraint(arg));
        return this;
    }
    ...
    public InteractionBuilder setConstantResult(Object constant) {
        if (constant != Specification._) resultGenerator = new ConstantResultGenerator(constant);
        return this;
    }
    ...
    public IMockInteraction build() {
        return new MockInteraction(line, column, text, minCount, maxCount, invConstraints, resultGenerator);
    }

    private static int convertCount(Object count, boolean inclusive) {
        if (!(count instanceof Number)) throw new InvalidSpecException("invocation count must be a number");
        int intCount = ((Number) count).intValue();
        if (!inclusive) intCount--;
        if (intCount < 0) throw new InvalidSpecException("invocation count must be >= 0");
        return intCount;
    }
}
