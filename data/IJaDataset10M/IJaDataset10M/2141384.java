...
public class AddSelectedSourceFolderOperation extends LoadpathModifierOperation {
    ...
    public boolean isValid(List elements, int[] types) throws RubyModelException {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            switch(types[i]) {
                case DialogPackageExplorerActionGroup.RUBY_PROJECT:
                    if (isSourceFolder((IRubyProject) object)) return false;
                    break;
                case DialogPackageExplorerActionGroup.SOURCE_FOLDER:
                    break;
    ...
    public String getDescription(int type) {
        ...
        if (obj instanceof IRubyElement) {
            ...
            if (type == DialogPackageExplorerActionGroup.RUBY_PROJECT) return Messages.format(NewWizardMessages.PackageExplorerActionGroup_FormText_ProjectToBuildpath, name);
            if (type == DialogPackageExplorerActionGroup.SOURCE_FOLDER) return Messages.format(NewWizardMessages.PackageExplorerActionGroup_FormText_PackageToBuildpath, name);
