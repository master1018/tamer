...
public class EditToolbar extends JPanel implements GraphObserver {
    ...
    private JButton addButton, deleteButton, modifyButton, undoButton, redoButton;
    ...
    public EditToolbar(GraphController graphCtrl) {
        ...
        nodeEdit.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_nodeEdit")));
        edgeEdit.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_edgeEdit")));
        addButton.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_addEdit")));
        deleteButton.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_deleteEdit")));
        modifyButton.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_modifyEdit")));
        undoButton.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_undoEdit")));
        redoButton.setIcon(new ImageIcon(Messages.getResourceURL("app", "icon_redoEdit")));
        nodeEdit.setToolTipText(Messages.getString("app", "EditToolbar.nodeEdit"));
        edgeEdit.setToolTipText(Messages.getString("app", "EditToolbar.edgeEdit"));
        addButton.setToolTipText(Messages.getString("app", "EditToolbar.addButton"));
        deleteButton.setToolTipText(Messages.getString("app", "EditToolbar.deleteButton"));
        modifyButton.setToolTipText(Messages.getString("app", "EditToolbar.modifyButton"));
        undoButton.setToolTipText(Messages.getString("app", "EditToolbar.undoButton"));
        redoButton.setToolTipText(Messages.getString("app", "EditToolbar.redoButton"));
    ...
    public void setEnabled(boolean state, boolean undo, boolean redo) {
        ...
        if (graphCtrl.getSelectedEdge() != null || graphCtrl.getSelectedNode() != null) deleteButton.setEnabled(state); else deleteButton.setEnabled(false);
        modifyButton.setEnabled(state);
        undoButton.setEnabled(undo);
        redoButton.setEnabled(redo);
        if (specialCharButtons != null) for (JButton spButton : specialCharButtons) {
            if (graphCtrl.getSelectedEdge() != null) spButton.setEnabled(state); else spButton.setEnabled(false);
        }
    }

    public void updateButtons() {
        if (graphCtrl.getSelectedEdge() != null || graphCtrl.getSelectedNode() != null) deleteButton.setEnabled(enabled); else deleteButton.setEnabled(false);
    }

    public void setUndoState() {
        undoButton.setEnabled(getGraphActionListener().getUndoManager().isUndoable());
        redoButton.setEnabled(getGraphActionListener().getUndoManager().isRedoable());
    }
    ...
    class addButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            if (graphActionListener.getEditMode() == EditMode.EDITING_NODES) graphActionListener.addNode();
        }
    }
    ...
    class deleteButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            if (graphActionListener.getSelectedEdge() != null) graphActionListener.removeEdge(graphActionListener.getSelectedEdge()); else if (graphActionListener.getSelectedNode() != null) graphActionListener.removeNode(graphActionListener.getSelectedNode());
        }
    }
    ...
    class SpecialCharButtonListener implements ActionListener, FocusListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            JButton button;
            char label;
            if ((other == null) && !(other instanceof JTextComponent)) return;
        ...
        public void focusGained(FocusEvent event) {
            if (!(event.getOppositeComponent() instanceof JTextComponent)) other = null; else other = (JTextComponent) event.getOppositeComponent();
        }
    ...
    public void graphSelectionChanged() {
        ...
        if (graphCtrl.getSelectedEdge() == null || graphCtrl.getGraphTextComponentFocusState()) {
            for (JButton spButton : specialCharButtons) spButton.setEnabled(false);
        } else {
            for (JButton spButton : specialCharButtons) spButton.setEnabled(true);
        }
    }
