...
public class L2PetDataTable {
    ...
    public static L2PetDataTable getInstance() {
        if (_instance == null) _instance = new L2PetDataTable();
        return _instance;
    }
    ...
    public void loadPetsData() {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                if (!_petTable.containsKey(petId)) _petTable.put(petId, new FastMap<Integer, L2PetData>());
    ...
    public void addPetData(L2PetData[] petLevelsList) {
        for (L2PetData element : petLevelsList) addPetData(element);
    }
    ...
    public static int getFoodItemId(int npcId) {
        if (isWolf(npcId)) return 2515; else if (isSinEater(npcId)) return 2515; else if (isHatchling(npcId)) return 4038; else if (isStrider(npcId)) return 5168; else if (isBaby(npcId)) return 7582; else return 0;
    }
