...
public class Pattern {
    ...
    public void setPattern(String pattern) {
        if (!(pattern == "" || pattern == null)) {
            if (pattern.length() <= Constants.MAX_PAT_LENGTH) {
                ...
                buildShiftingTable();
                buildCycleTable();
                if (pattern.length() == Constants.MAX_PAT_LENGTH) isexpandable = false; else isexpandable = true;
            }
    ...
    private void buildShiftingTable() {
        ...
        for (int patpos = 1; patpos < pattern.length(); patpos++) {
            if (pattern.charAt(patpos) == pattern.charAt(vglind)) shiftingtable[patpos] = shiftingtable[vglind]; else shiftingtable[patpos] = vglind;
            while ((vglind >= 0) && (pattern.charAt(patpos) != pattern.charAt(vglind))) vglind = shiftingtable[vglind];
            vglind++;
        }
    }
    ...
    private void buildCycleTable() {
        ...
        for (int patpos = 1; patpos < pattern.length(); patpos++) {
            ...
            for (i = 0; i < (cycledepthtable[patpos]) + 1; i++) workdata[i] = 0;
