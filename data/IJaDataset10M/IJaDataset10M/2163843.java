...
public class SubstanceDefaultTreeCellRenderer extends JLabel implements TreeCellRenderer {
    ...
    public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded, boolean leaf, int row, boolean hasFocus) {
        ...
        if (treeUI instanceof SubstanceTreeUI) {
            ...
            boolean isDropLocation = (dropLocation != null && dropLocation.getChildIndex() == -1 && tree.getRowForPath(dropLocation.getPath()) == row);
            ...
            if (sel) this.setForeground(UIManager.getColor("Tree.selectionForeground")); else this.setForeground(UIManager.getColor("Tree.textForeground"));
        }
        if (SubstanceLookAndFeel.isCurrentLookAndFeel()) SubstanceStripingUtils.applyStripedBackground(tree, row, this);
        if (!tree.isEnabled()) {
    ...
    private SubstanceColorScheme getColorSchemeForState(JTree tree, SubstanceTreeUI ui, ComponentState activeState) {
        SubstanceColorScheme scheme = (activeState == ComponentState.ENABLED) ? ui.getDefaultColorScheme() : SubstanceColorSchemeUtilities.getColorScheme(tree, ColorSchemeAssociationKind.HIGHLIGHT, activeState);
        if (scheme == null) {
    ...
    protected void firePropertyChange(String propertyName, Object oldValue, Object newValue) {
        if ("text".equals(propertyName)) super.firePropertyChange(propertyName, oldValue, newValue);
    }
