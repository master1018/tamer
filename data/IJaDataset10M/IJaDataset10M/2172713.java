 ...
 *
 */
public class ConversionsPropertiesHandler extends DefaultHandler {
	 ...
	 */
    public ConversionsPropertiesHandler(String xmlProperties) throws RequestResolvingException {
        try {
            XMLReader xmlReader = XMLReaderFactory.createXMLReader();
            xmlReader.setContentHandler(this);
            if (xmlProperties != null) {
                xmlReader.parse(new InputSource(new ByteArrayInputStream(xmlProperties.getBytes())));
            }
        } catch (Throwable e) {
    ...
    public void applyPathProperties(ConversionsPath cp) throws RequestResolvingException {
        for (int i = 0; i < cp.getPath().size(); i++) {
            cp.getPath().get(i).getConversionActionArguments().setProperties(properties.get(i));
        }
    }
}
