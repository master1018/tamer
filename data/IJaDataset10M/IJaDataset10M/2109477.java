...
public class InterestData {
    ...
    public synchronized boolean equals(Object obj) throws ClassCastException {
        if (null == obj) throw new ClassCastException("Null value");
        if (!(obj instanceof InterestData)) throw new ClassCastException("Not a InterestData");
        synchronized (obj) {
    ...
    public synchronized void setStartTime(VersionNumber startTime) {
        if (VersionNumber.getMinimumVersion().after(startTime)) startTime = VersionNumber.getMinimumVersion();
    ...
    public synchronized void setStopTime(VersionNumber stopTime) {
        if (VersionNumber.getMaximumVersion().before(stopTime)) stopTime = VersionNumber.getMaximumVersion();
        _stopTime = stopTime;
        _dirty = true;
    }
    ...
    public synchronized boolean addExclude(VersionNumber version) {
        if (_excludedVersions.size() >= VersioningInterestManager.MAX_FILL) {
            if (Log.isLoggable(Log.FAC_ENCODING, Level.FINE)) Log.fine(Log.FAC_ENCODING, "addExclude full, not adding {0}", version);
            return false;
        }
    ...
    public synchronized Interest buildInterest() {
        ...
        if (Log.isLoggable(Log.FAC_ENCODING, Level.FINEST)) Log.finest(Log.FAC_ENCODING, "Exclusion: start version {0}", ContentName.componentPrintURI(startTimeMinusOneComponent));
        ...
        if (VersionNumber.getMaximumVersion().after(_stopTime)) stopTimePlusOneComponent = _stopTime.addAndReturn(1).getVersionBytes(); else stopTimePlusOneComponent = VersioningProfile.TOP_EXCLUDE_VERSION_MARKER;
        exStop = new ExcludeComponent(stopTimePlusOneComponent);
        if (null == lastComponentExcluded || !lastComponentExcluded.equals(exStop)) components.add(exStop);
        components.add(new ExcludeAny());
        if (Log.isLoggable(Log.FAC_ENCODING, Level.FINEST)) Log.finest(Log.FAC_ENCODING, "Exclusion: stop  version {0}", exStop.toString());
        Exclude exclude;
        try {
        ...
        if (Log.isLoggable(Log.FAC_ENCODING, Level.FINEST)) Log.finest(Log.FAC_ENCODING, "Exclusion: {0}", exclude.toString());
        Interest interest = Interest.last(_name, exclude, (Integer) _name.count(), (Integer) 3, (Integer) 2, null);
        getDensity();
        _dirty = false;
        _interest = interest;
        return _interest;
    ...
    public synchronized boolean contains(VersionNumber version) {
        if (_startTime.compareTo(version) <= 0 && _stopTime.compareTo(version) >= 0) return true;
        return false;
    }
    ...
    public synchronized InterestData splitLeft(int count) {
        ...
        if (count > 0) transferLeft(left, count);
        return left;
    }
    ...
    public synchronized InterestData splitRight(int count) {
        ...
        if (count > 0) transferRight(right, count);
        return right;
    }
    ...
    public void transferLeft(InterestData left, int count) {
        ...
        if (Log.isLoggable(Log.FAC_ENCODING, Level.FINE)) Log.fine(Log.FAC_ENCODING, String.format("TransferLeft to %s from %s", left.toString(), this.toString()));
    }
    ...
    public void transferRight(InterestData right, int count) {
        ...
        if (Log.isLoggable(Log.FAC_ENCODING, Level.FINE)) Log.fine(Log.FAC_ENCODING, String.format("TransferRight from %s to %s", this.toString(), right.toString()));
    }
    ...
    protected synchronized void addExcludeUnbounded(VersionNumber version) {
        if (Log.isLoggable(Log.FAC_ENCODING, Level.FINER)) Log.finer(Log.FAC_ENCODING, String.format("addExcludeUnbounded %s", version.toString()));
