...
public class Creature {
    ...
    public int getStrikingPower(Creature target, int myElevation, int targetElevation, HazardTerrain myHexTerrain, HazardTerrain targetHexTerrain, HazardHexside myHexside, HazardHexside targetHexside) {
        ...
        int dice = getPower();
        dice += myHexTerrain.getPowerBonusStrikeFrom(myType.isNativeIn(myHexTerrain), targetType.isNativeIn(myHexTerrain));
        if (myHexside.equals(HazardHexside.DUNE) && myType.isNativeDune()) {
            ...
            dice++;
        } else if (!myType.isNativeDune() && targetHexside.equals(HazardHexside.DUNE)) {
            dice--;
        }
        LOGGER.finest("Found " + dice + " dice for " + myType.getName() + " [" + myHexTerrain.getName() + "  " + myElevation + ", " + myHexside.getName() + " ] vs. " + targetType.getName() + " [" + targetHexTerrain.getName() + " " + targetElevation + ", " + targetHexside.getName() + " ]");
        return dice;
    }
    ...
    public int getStrikingSkill(Creature target, int myElevation, int targetElevation, HazardTerrain myHexTerrain, HazardTerrain targetHexTerrain, HazardHexside myHexside, HazardHexside targetHexside) {
        ...
        attackerSkill -= myHexTerrain.getSkillPenaltyStrikeFrom(myType.isNativeIn(myHexTerrain), targetType.isNativeIn(myHexTerrain));
        if (myElevation > targetElevation) {
            if (myHexside.equals(HazardHexside.TOWER)) {
                attackerSkill++;
            }
        } else if (myElevation < targetElevation) {
            if ((targetHexside.equals(HazardHexside.SLOPE) && !myType.isNativeSlope()) || targetHexside.equals(HazardHexside.TOWER)) {
                attackerSkill--;
            }
        }
        LOGGER.finest("Found skill " + attackerSkill + "  for " + myType.getName() + " [" + myHexTerrain.getName() + "  " + myElevation + ", " + myHexside.getName() + " ] vs. " + targetType.getName() + " [" + targetHexTerrain.getName() + " " + targetElevation + ", " + targetHexside.getName() + " ]");
    ...
    public int adjustHits(int damage) {
        ...
        LOGGER.log(Level.INFO, "Critter " + getDescription() + ": " + oldhits + " + " + damage + " => " + tmp_hits + "; " + excess + " excess");
        if (tmp_hits >= getPower()) {
            LOGGER.log(Level.INFO, "Critter " + getDescription() + " is now dead: (hits=" + tmp_hits + " > power=" + getPower() + ")");
            setDead(true);
        }
        setHits(tmp_hits);
        return excess;
