...
public class ContiPerfRule implements MethodRule {
    ...
    protected ContiPerfRule(ReportContext context, Object suite) {
        if (context == null) this.context = JUnitReportContext.createInstance(suite); else this.context = context;
        if (suite != null) {
    ...
    public Statement apply(Statement base, FrameworkMethod method, Object target) {
        ...
        if (context.getReportModules().size() == 0) {
            ...
            if (context.getReportModules().size() == 0) context = Config.instance().createDefaultReportContext(AssertionFailedError.class);
        }
        String testId = methodName(method, target);
        base = new PerfTestStatement(base, testId, executionConfig(method, testId), requirements(method, testId), context);
        try {
            if (runBefores != null) {
    ...
    private static ReportContext createReportContext(ReportModule... modules) {
        ReportContext context = new JUnitReportContext();
        for (ReportModule module : modules) context.addReportModule(module);
        return context;
    }
    ...
    private ExecutionConfig executionConfig(FrameworkMethod method, String methodName) {
        ...
        if (annotation != null) return configurePerfTest(annotation, methodName);
        if (defaultExecutionConfig != null) return defaultExecutionConfig;
    ...
    private PerformanceRequirement requirements(FrameworkMethod method, String testId) {
        ...
        if (annotation != null) return ContiPerfUtil.mapRequired(annotation);
        if (defaultRequirements != null) return defaultRequirements;
        return null;
    }
    ...
    public static ExecutionConfig configurePerfTest(PerfTest annotation, String testId) {
        ...
        if (annotation == null) config = new ExecutionConfig(1);
        int count = Config.instance().getInvocationCount(testId);
        if (count >= 0) config.setInvocations(count);
        return config;
    }
