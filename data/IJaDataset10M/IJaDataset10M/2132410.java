...
public class DumpFilter implements Filter {
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        if (dumpDirString != null) {
            ...
            if (!dumpDir.exists() || !dumpDir.isDirectory() || !dumpDir.canWrite()) log.warn("Ignoring dump directory (is it a writable directory?): %s", dumpDir); else this.dumpDir = dumpDir;
        }
    }
    ...
    private void dumpBytes(String label, byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            if (b >= 0x20 && b <= 0x7e) charSb.append(' ').append((char) b); else charSb.append("##");
        }
        ...
        if (dumpDir != null) {
            ...
            for (int i = 1; i < 100 && file.exists(); i++) file = new File(file.getAbsolutePath() + "." + i);
