...
public abstract class Proxy {
    ...
    Proxy(Proxy chainProxy, String proxyHost, int proxyPort) throws UnknownHostException {
        ...
        if (chainProxy == null) this.proxyIP = InetAddress.getByName(proxyHost);
   ...
   */
    public static Proxy parseProxy(String proxy_entry) {
        ...
        if (st.hasMoreTokens()) proxy_user = st.nextToken();
        if (st.hasMoreTokens()) proxy_password = st.nextToken();
        try {
            if (proxy_user == null) proxy = new Socks5Proxy(proxy_host, proxy_port); else if (proxy_password == null) proxy = new Socks4Proxy(proxy_host, proxy_port, proxy_user); else {
    ...
    protected void startSession() throws SocksException {
        try {
            if (chainProxy == null) proxySocket = new Socket(proxyIP, proxyPort); else if (proxyIP != null) proxySocket = new SocksSocket(chainProxy, proxyIP, proxyPort); else proxySocket = new SocksSocket(chainProxy, proxyHost, proxyPort);
