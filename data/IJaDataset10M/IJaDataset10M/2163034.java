...
public class CellEditors {
    ...
    public FormattedFieldEditor createFormattedFieldEditor(JFormattedTextField.AbstractFormatter formatter, int alignment) {
        ...
        if (alignment >= 0) field.setHorizontalAlignment(alignment);
    ...
    public SpinnerEditor createSpinnerEditor(SpinnerModel model, Format format) {
        ...
        if (format != null) {
            ...
            if (formatter instanceof InternationalFormatter) ((InternationalFormatter) formatter).setFormat(format);
        }
    ...
    public TableCellEditor createTableCellEditor(ObjEntityViewField field) {
        ...
        if (lookup) {
            ...
            if (format instanceof MapFormat) {
                ...
                ComboBoxModel comboData = new DefaultComboBoxModel((mapFormat).getValues());
            ...
            if (dataType == DataTypeEnum.BOOLEAN_TYPE_VALUE) editor = createCheckBoxEditor(); else editor = createTextFieldEditor(alignment);
        }
        return editor;
    }
    ...
    public void installEditors(JTable table) {
        ...
        if (!(m instanceof DOTableModel)) return;
    ...
    public class FormattedFieldEditor extends DefaultCellEditor {
        ...
        public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
            ...
            if (((JFormattedTextField) editorComponent).isEditValid()) editorComponent.setBorder(editStateBorder); else editorComponent.setBorder(invalidStateBorder);
