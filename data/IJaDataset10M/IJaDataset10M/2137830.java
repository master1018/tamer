...
public class Images extends Sink implements ClickListener, LoadListener {
    ...
    private static final String[] sImages = new String[] { "rembrandt/JohannesElison.jpg", "rembrandt/LaMarcheNocturne.jpg", "rembrandt/SelfPortrait1628.jpg", "rembrandt/SelfPortrait1640.jpg", "rembrandt/TheArtistInHisStudio.jpg", "rembrandt/TheReturnOfTheProdigalSon.jpg" };

    public static SinkInfo init() {
        return new SinkInfo("Images", "This page demonstrates GWT's support for images.  Notice in " + "particular how it uses the image's onLoad event to display a 'wait " + "spinner' between the back and forward buttons.") {
    ...
    public void onClick(Widget sender) {
        if (sender == prevButton) loadImage(curImage - 1); else if (sender == nextButton) loadImage(curImage + 1);
    }
    ...
    private void loadImage(int index) {
        if (index < 0) index = sImages.length - 1; else if (index > sImages.length - 1) index = 0;
        curImage = index;
