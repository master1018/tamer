...
public class SOAPHeaderElement extends MessageElement implements javax.xml.soap.SOAPHeaderElement {
    ...
    public SOAPHeaderElement(Element elem) {
        ...
        String val = elem.getAttributeNS(soapConstants.getEnvelopeURI(), Constants.ATTR_MUST_UNDERSTAND);
        try {
        ...
        actor = elem.getAttributeNS(roleQName.getNamespaceURI(), roleQName.getLocalPart());
        if (soapConstants == SOAPConstants.SOAP12_CONSTANTS) {
            String relayVal = elem.getAttributeNS(soapConstants.getEnvelopeURI(), Constants.ATTR_RELAY);
    ...
    public SOAPHeaderElement(String namespace, String localPart, String prefix, Attributes attributes, DeserializationContext context) throws AxisFault {
        ...
        SOAPConstants soapConstants = getSOAPConstants();
        String val = attributes.getValue(soapConstants.getEnvelopeURI(), Constants.ATTR_MUST_UNDERSTAND);
        setMustUnderstandFromString(val, (soapConstants == SOAPConstants.SOAP12_CONSTANTS));
        QName roleQName = soapConstants.getRoleAttributeQName();
        actor = attributes.getValue(roleQName.getNamespaceURI(), roleQName.getLocalPart());
        if (soapConstants == SOAPConstants.SOAP12_CONSTANTS) {
            String relayVal = attributes.getValue(soapConstants.getEnvelopeURI(), Constants.ATTR_RELAY);
            relay = ((relayVal != null) && (relayVal.equals("true") || relayVal.equals("1"))) ? true : false;
        }
        processed = false;
        ...
        if (val != null && val.length() > 0) {
            if ("0".equals(val)) {
                ...
                if ("true".equalsIgnoreCase(val)) {
                    ...
                    throw new AxisFault(Messages.getMessage("badMUVal", val, new QName(namespaceURI, name).toString()));
                }
            } else {
                throw new AxisFault(Messages.getMessage("badMUVal", val, new QName(namespaceURI, name).toString()));
            }
    ...
    protected void outputImpl(SerializationContext context) throws Exception {
        if (!alreadySerialized) {
            ...
            if (actor != null) {
                setAttribute(roleQName.getNamespaceURI(), roleQName.getLocalPart(), actor);
            }
            String val;
            if (context.getMessageContext() != null && context.getMessageContext().getSOAPConstants() == SOAPConstants.SOAP12_CONSTANTS) val = mustUnderstand ? "true" : "false"; else val = mustUnderstand ? "1" : "0";
            setAttribute(soapVer.getEnvelopeURI(), Constants.ATTR_MUST_UNDERSTAND, val);
            if (soapVer == SOAPConstants.SOAP12_CONSTANTS && relay) {
                setAttribute(soapVer.getEnvelopeURI(), Constants.ATTR_RELAY, "true");
            }
        }
    ...
    public NamedNodeMap getAttributes() {
        ...
        SOAPConstants soapConstants = getSOAPConstants();
        String mustUnderstand = attributes.getValue(soapConstants.getEnvelopeURI(), Constants.ATTR_MUST_UNDERSTAND);
        QName roleQName = soapConstants.getRoleAttributeQName();
        ...
        if (mustUnderstand == null) {
            if (soapConstants == SOAPConstants.SOAP12_CONSTANTS) {
                ...
                setAttributeNS(soapConstants.getEnvelopeURI(), Constants.ATTR_MUST_UNDERSTAND, "0");
            }
        }
        if (actor == null) {
