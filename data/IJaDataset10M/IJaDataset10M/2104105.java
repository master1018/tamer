...
public class APDUManager {
    ...
    private static final String SAT_APDU_PROP = "com.sun.midp.io.j2me.apdu.satselectapdu";
     ...
     */
    static Handle openSATConnection(int slot, SecurityToken securityToken) throws IOException {
        Handle h = null;
        init();
        if (false) {
            if (satHandle != null && satHandle.opened && satHandle.cad.getCardSessionId() != -1 && satHandle.cardSessionId == satHandle.cad.getCardSessionId()) {
        ...
        synchronized (sync[slot]) {
            ...
            if (satAPDU != null) {
                ...
                try {
                    ...
                    ok = len >= 10 && len <= 22;
                    int Lc = (apdu[4] & 0xFF);
                    if (ok && (len < Lc + 5 || len > Lc + 5 + 1 || apdu[0] != (byte) 0x00 || apdu[1] != (byte) 0xA4 || apdu[2] != (byte) 0x04)) {
                        ok = false;
                    }
                    if (ok && len == Lc + 5) {
                        apdu[len] = 0x7F;
                ...
                if (ok) {
                    try {
                        h = new Handle(slot, cads[slot].selectApplication(true, apdu), securityToken);
                    } catch (IOException e) {
     ...
     * connection cannot be established with the card.
     */
    public static Handle openACLConnection(byte[] apdu, int slot, SecurityToken securityToken) throws IOException {
        Handle h = null;
        init();
        try {
            if (cads[slot] == null) {
        ...
        try {
            return new Handle(slot, cads[slot].selectApplication(true, apdu), securityToken);
        } catch (IOException e) {
            if (cads[slot].isAlive()) {
                cads[slot].clean();
                throw e;
     ...
     */
    public static Handle selectApplication(byte[] selectAPDU, int slot, SecurityToken securityToken) throws IOException {
        init();
        synchronized (sync[slot]) {
            try {
                if (cads[slot] == null) {
            ...
            try {
                return new Handle(slot, cads[slot].selectApplication(false, selectAPDU), securityToken);
            } catch (IOException e) {
                if (cads[slot].isAlive()) {
                    cads[slot].clean();
                    throw e;
     ...
     */
    public static void checkSlotNumber(int slot) throws ConnectionNotFoundException {
        try {
            init();
        } catch (IOException e) {
            throw new ConnectionNotFoundException("Invalid configuration");
        }
        if (slot < 0 || slot >= getSlotCount()) {
            throw new ConnectionNotFoundException("Invalid slot identifier: " + slot);
        }
    }
     ...
     */
    public static byte[] exchangeAPDU(Handle h, byte[] apduData) throws IOException {
     ...
     */
    public static int parseDottedBytes(String src, byte[] dest, int offset) {
        int i = 0;
        int len = 0;
        int j;
