    ...
    public void initHLID() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (bo != null && (bo.getKeyType() != null && bo.getKeyType().intValue() == DOBO.KEY_TYPE_HIGH_LOW)) {
                    String keyCol = bo.getKeyCol();
                    conBus = bo.getDataBase().getConnection();
                    String maxKeySql = new StringBuffer("select max(").append(keyCol).append(") from ").append(tableName).toString();
                    ...
                    stmtBusi.close();
                    conBus.close();
                    StringBuffer sqlUpdate = new StringBuffer("update DO_KEYS SET next_hi=").append(intMaxValue.intValue() + 1).append(" WHERE table_name=?");
