...
public class BufferCacheTest extends TestCase {
    ...
    public void testLookupIndex() {
        for (int i = 0; i < S.length; i++) {
            ...
            if (i > 0) assertEquals(i, index); else assertEquals(-1, index);
        }
    }
    ...
    public void testGetBuffer() {
        for (int i = 0; i < S.length; i++) {
            ...
            if (i > 0) assertEquals(i, b.peek(1) - '0'); else assertEquals(null, b);
        }
    }
    ...
    public void testLookupBuffer() {
        for (int i = 0; i < S.length; i++) {
            ...
            if (i > 0) assertTrue("" + i, S[i] == b.toString()); else {
    ...
    public void testInsensitiveLookupBuffer() {
        for (int i = 0; i < S.length; i++) {
            ...
            if (i > 0) assertTrue("test" + i, S[i] == b.toString()); else assertTrue("test" + i, S[i] != b.toString());
        }
    }
    ...
    public void testToString() {
        for (int i = 0; i < S.length; i++) {
            ...
            assertEquals(S[i], b);
            if (i > 0) assertTrue(S[i] == b); else assertTrue(S[i] != b);
        }
    }
}
