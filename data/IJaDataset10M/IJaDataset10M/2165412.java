...
public class DirectoryEntryNode extends MyAbstractNode implements DetailViewProvider, EditorProvider, Refreshable {
    ...
    private static class ExportLDIFAction extends NodeAction {
        ...
        protected void performAction(Node[] activatedNodes) {
            ...
            final String secViewName = Messages.getString("SecondaryDirectoryViewNode.name");
            final boolean isSec = activatedNodes[0].getDisplayName().equals(secViewName) || activatedNodes[0].getParentNode().getName().equals(secViewName) || activatedNodes[0].getParentNode().getParentNode().getName().equals(secViewName);
            if (chooser.showDialog(MainTreeTopComponent.getDefault(), "Export") == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    params.add(new Parameter(ExportCommandExecutor.EXPORTPOINT_PARAMETER, dn));
                    String path = chooser.getSelectedFile().getCanonicalPath();
                    if (!path.endsWith(".ldif")) path = path + ".ldif";
                    ...
                    final ListenerParameter listeners[] = new ListenerParameter[] { new ListenerParameter(ExportCommandExecutor.EXCEPTIONLISTENER_PARAMETER, new ToolCommandListener() {
                    ...
                    }), new ListenerParameter(ExportCommandExecutor.OUTPUTLISTENER_PARAMETER, new ToolCommandListener() {

                        public void notify(Serializable o) {
                            handle.progress(o.toString());
                        }
                    }), new ListenerParameter(ExportCommandExecutor.ERRORLISTENER_PARAMETER, new ToolCommandListener() {

                        public void notify(Serializable o) {
                            final IOException e = new IOException(o.toString());
                            ErrorManager.getDefault().annotate(e, "Error during LDIF export");
                            ErrorManager.getDefault().notify((Throwable) o);
                        }
                    }) };
                    handle.start();
    ...
    private static class ImportLDIFAction extends NodeAction {
        ...
        protected void performAction(Node[] activatedNodes) {
            ...
            lcd = (LDAPConnectionDescriptor) activatedNodes[0].getLookup().lookup(LDAPConnectionDescriptor.class);
            if (chooser.showDialog(MainTreeTopComponent.getDefault(), "Import") == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    if (parentNode instanceof RealmNode && parentNode instanceof Refreshable) ((Refreshable) parentNode).refresh();
    ...
    static final class ImportWorker extends SwingWorker {
        ...
        public Object construct() {
            try {
                for (final File importFile : this.importFiles) if (importFile != null) {
                    if (logger.isDebugEnabled()) logger.debug("import following temporary file: " + importFile);
                    final NameCallback nc = new NameCallback("Bind DN");
                    final PasswordCallback pc = new PasswordCallback("Password", false);
                    lcd.getCallbackHandler().handle(new Callback[] { nc, pc });
                    final List<Parameter> params = new ArrayList<Parameter>();
    ...
    private static void importTempFile(File importFile, LDAPConnectionDescriptor lcd, LdifActionProgressBar bar) throws Exception {
        ...
        if (isRootImportLdifFile(importFile)) Util.deleteRecursively(ctx, targetName, "^cn=administrator[s]?$");
        ...
        while ((strLine = br.readLine()) != null) {
            ...
            } else content.append(strLine).append(System.getProperty("line.separator"));
        }
        in.close();
    ...
    static class ChildEntries extends AbstractAsyncArrayChildren {
        ...
        protected Collection asyncInitChildren() {
            try {
                final LDAPConnectionDescriptor lcd = ((DirectoryEntryNode) getNode()).getConnectionDescriptor();
                if (lcd == null) return Collections.EMPTY_LIST;
                final DirContext ctx = lcd.createDirectoryFacade().createDirContext();
                NamingEnumeration<NameClassPair> bindings;
                try {
                    bindings = ctx.list(ctx.getNameParser("").parse(dn));
                    final List<String> names = new ArrayList<String>();
                    if (null != bindings) while (bindings.hasMoreElements()) {
                        final NameClassPair b = bindings.next();
                        final String name = b.isRelative() ? b.getName() + (dn.length() > 0 ? "," + dn : "") : b.getName();
                        names.add(name);
                    }
                    if (mutable) return names; else return Collections.EMPTY_LIST;
                } finally {
        ...
        protected Node[] createNodes(Object key) {
            if (key instanceof Node[]) return (Node[]) key;
    ...
    public DirectoryEntryNode(Children c, Node node, LDAPConnectionDescriptor lcd, String dn) {
        super(c, new ProxyLookup(new Lookup[] { Lookups.fixed(new Object[] { lcd }), node.getLookup() }));
        this.dn = this.rdn = dn;
    }
    ...
    public LDAPConnectionDescriptor getConnectionDescriptor() {
        final LDAPConnectionDescriptor lcd = (LDAPConnectionDescriptor) getLookup().lookup(LDAPConnectionDescriptor.class);
        final String secViewName = Messages.getString("SecondaryDirectoryViewNode.name");
        String levelOne = "";
        String levelTwo = "";
        String levelThree = "";
    ...
    public Action[] getActions(boolean context) {
        if (isWritable()) {
            if (this.getParentNode() instanceof RealmNode) return new Action[] { SystemAction.get(ExportLDIFAction.class), SystemAction.get(ImportLDIFAction.class), null, SystemAction.get(RefreshAction.class) }; else return new Action[] { SystemAction.get(ExportLDIFAction.class), null, SystemAction.get(RefreshAction.class) };
        } else return new Action[] { SystemAction.get(ExportLDIFAction.class), null, SystemAction.get(RefreshAction.class) };
    }
    ...
    public void setName(String s) {
        ...
        final String rest = LDAPDirectory.idToUpperCase(this.dn).replace(LDAPDirectory.idToUpperCase(this.rdn) + ",", "");
        final boolean isRightDN = (sEdit.startsWith("CN=") || sEdit.startsWith("L=")) && sEdit.endsWith(rest);
        if (null == s || s.length() == 0 || isRightDN == false) {
            DialogDisplayer.getDefault().notify(new NotifyDescriptor(Messages.getString("DirectoryEntryNode.nameInvalid", s), Messages.getString("DirectoryEntryNode.cantChangeName"), NotifyDescriptor.DEFAULT_OPTION, NotifyDescriptor.ERROR_MESSAGE, null, null));
            return;
        }
        try {
            final LDAPConnectionDescriptor lcd = getConnectionDescriptor();
