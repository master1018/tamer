...
public class SudokuPuzzleView extends View {
    ...
    public Position getPositionAt(float px, float py, float fuzzy) {
        ...
        px -= offsetX;
        if (px < (-cellWidth * fuzzy) || px >= (cellWidth * (size + fuzzy))) return null;
        py -= offsetY;
        if (py < (-cellHeight * fuzzy) || py >= (cellHeight * (size + fuzzy))) return null;
    ...
    public void highlightDigit(Integer digit) {
        if (highlightedDigit == digit) return;
    ...
    public void markPosition(Position position) {
        if (eq(position, markedPosition)) return;
    ...
    public void invalidatePosition(Position position) {
        if (position == null || puzzle == null) return;
        if (Constants.LOG_V) Log.v(TAG, "invalidatePosition(" + position + ")");
        ...
        invalidate((int) Math.floor(x0), (int) Math.floor(y0), (int) Math.ceil(x1), (int) Math.ceil(y1));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Constants.LOG_V) Log.v(TAG, "Draw time: " + (t1 - t0));
    }
    ...
    private void onDraw0(Canvas canvas) {
        ...
        if (theme.isDrawAreaColors(puzzle.getPuzzleType())) drawAreaColors(canvas, clipBounds); else drawBackground(canvas);
        drawExtraRegions(canvas, clipBounds);
        if (puzzle.isSolved()) drawCongrats(canvas); else if (paused) drawPaused(canvas); else {
    ...
    private void drawExtraRegions(Canvas canvas, int row, int col) {
        if (puzzle.isExtraRegion(row, col)) {
            canvas.drawRect(0, 0, cellWidth, cellHeight, theme.getExtraRegionPaint(puzzle.getPuzzleType(), puzzle.getExtraRegionCode(row, col)));
        }
    }
    ...
    private void drawHighlightedCells(Canvas canvas, Rect clipBounds) {
        if (highlightedDigit == null || theme.getHighlightDigitsPolicy() == HighlightDigitsPolicy.NEVER) return;
        for (int row = 0; row < size; row++) {
            float y = row * cellHeight;
            if (y > clipBounds.bottom || y + cellHeight < clipBounds.top) continue;
            for (int col = 0; col < size; col++) {
    ...
    private void drawHighlightedcell(Canvas canvas, int numValues, float x, float y) {
        if (numValues != 1 && theme.getHighlightDigitsPolicy() == HighlightDigitsPolicy.ONLY_SINGLE_VALUES) return;
        canvas.save();
        canvas.translate(x, y);
        canvas.clipRect(0, 0, cellWidth, cellHeight);
        if (numValues == 1) canvas.drawColor(theme.getHighlightedCellColorSingleDigit()); else canvas.drawColor(theme.getHighlightedCellColorMultipleDigits());
        canvas.restore();
    }
    ...
    private void drawMarkedPosition(Canvas canvas) {
        ...
        canvas.translate(x, y);
        Paint paint = puzzle.isClue(markedPosition.row, markedPosition.col) ? theme.getMarkedPositionCluePaint() : theme.getMarkedPositionPaint();
        canvas.clipRect(0, 0, cellWidth, cellHeight);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (Constants.LOG_V) Log.v(TAG, "onMeasure(" + MeasureSpec.toString(widthMeasureSpec) + ", " + MeasureSpec.toString(heightMeasureSpec) + ")");
