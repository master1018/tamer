...
public class WorkflowFactory {
    ...
    public static Workflow getWorkflow(Collection collection) throws IOException, WorkflowConfigurationException {
        if (workflowCache == null) workflowCache = new HashMap<String, Workflow>();
    ...
    private static Step createFirstStep(Workflow workflow, Node workflowNode) throws TransformerException, WorkflowConfigurationException {
        ...
        Role role = null;
        if (roleNode != null) role = workflow.getRoles().get(roleNode.getTextContent());
    ...
    private static LinkedHashMap<String, Role> getRoles(Node workflowNode) throws WorkflowConfigurationException {
        ...
        for (int i = 0; i < roleNodes.getLength(); i++) {
            ...
            if (descriptionNode != null) roleDescription = descriptionNode.getTextContent();
            Node scopeNode = roleNodes.item(i).getAttributes().getNamedItem("scope");
            String roleScope = null;
            if (scopeNode != null) roleScope = scopeNode.getTextContent();
            ...
            Role.Scope scope;
            if (roleScope == null || roleScope.equalsIgnoreCase("collection")) scope = Role.Scope.COLLECTION; else if (roleScope.equalsIgnoreCase("item")) scope = Role.Scope.ITEM; else if (roleScope.equalsIgnoreCase("repository")) scope = Role.Scope.REPOSITORY; else throw new WorkflowConfigurationException("An invalid role scope has been specified it must either be item or collection.");
