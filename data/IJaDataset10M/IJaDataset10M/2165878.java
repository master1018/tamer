...
public class CoffeeTables extends StdLibrary implements StatisticsLibrary {
    ...
    public void update() {
        if (CMSecurity.isDisabled("STATS")) return;
        if (todays != null) CMLib.database().DBUpdateStat(todays.startTime(), todays.data());
    }
    ...
    public boolean activate() {
        if (thread == null) thread = new ThreadEngine.SupportThread("THStats" + Thread.currentThread().getThreadGroup().getName().charAt(0), MudHost.TIME_SAVETHREAD_SLEEP, this, CMSecurity.isDebugging("SAVETHREAD"));
    ...
    public void run() {
        if ((!CMSecurity.isDisabled("SAVETHREAD")) && (!CMSecurity.isDisabled("STATSTHREAD"))) {
