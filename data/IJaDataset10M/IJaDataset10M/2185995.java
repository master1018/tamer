...
public class TextureGraph2d<V, E> extends DefaultDrawableGraph2d<V, E> {
    ...
    protected void drawVertices(GL2 gl, GLU glu, Camera cam) {
        for (V v : graph.getVertices()) {
            if (highlights.get(v)) drawVertexNode(gl, glu, cam, v, layout.get(v), formatter.getHighlightedVertexColor()); else drawVertexNode(gl, glu, cam, v, layout.get(v), formatter.getVertexColor());
        }
    }
