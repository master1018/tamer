...
public class XMI212UMLHandler extends UMLHandler {
    ...
    public XMI212UMLHandler(XMLResource xmiResource, XMLHelper helper, Map<?, ?> options) {
    ...
    protected void handleProxy(InternalEObject proxy, String uriLiteral) {
        if (uriLiteral.startsWith(XMI2UMLResource.UML_METAMODEL_URI) || uriLiteral.startsWith(XMI2UMLResource.UML_METAMODEL_2_1_1_URI) || uriLiteral.startsWith(XMI2UMLResource.UML_METAMODEL_2_1_URI) || uriLiteral.startsWith(XMI2UMLResource.UML_METAMODEL_2_2_URI)) {
            if (uriLiteral.endsWith(PRIMITIVE_TYPE_BOOLEAN)) {
                ...
                uriLiteral = UMLResource.UML_METAMODEL_URI + (index == -1 ? "#_0" : uriLiteral.substring(index));
            }
        } else if (uriLiteral.startsWith(XMI2UMLResource.STANDARD_PROFILE_URI) || uriLiteral.startsWith(XMI2UMLResource.STANDARD_PROFILE_2_1_1_URI) || uriLiteral.startsWith(XMI2UMLResource.STANDARD_PROFILE_2_1_URI) || uriLiteral.startsWith(XMI2UMLResource.STANDARD_PROFILE_2_2_URI)) {
            int index = uriLiteral.indexOf('#');
            uriLiteral = UMLResource.STANDARD_PROFILE_URI + (index == -1 ? "#_0" : uriLiteral.substring(index));
        }
    ...
    protected void processElement(String name, String prefix, String localName) {
        if (EMOFExtendedMetaData.EXTENSION.equals(localName) && XMI2UMLResource.XMI_NS_URI.equals(helper.getURI(prefix)) && attribs != null && EcorePackage.eNS_URI.equals(attribs.getValue(EMOFExtendedMetaData.XMI_EXTENDER_ATTRIBUTE))) {
    ...
    protected void setAttribValue(EObject object, String name, String value) {
        if (IDREF_ATTRIB.equals(name) && (!recordUnknownFeature || types.peek() != UNKNOWN_FEATURE_TYPE)) {
