...
public abstract class AllocPolicy extends Sim_entity {
    ...
    public void processOtherEvent(Sim_event ev) {
        if (ev == null) {
            System.out.println(resName_ + ".processOtherEvent(): " + "Error - an event is null.");
            return;
        }
        System.out.println(resName_ + ".processOtherEvent(): Unable to " + "handle request from an event with a tag number " + ev.get_tag());
    }
    ...
    protected boolean sendAck(int tag, boolean status, int gridletId, int destId) {
        ...
        switch(tag) {
            ...
            case GridSimTags.GRIDLET_SUBMIT_ACK:
                ...
                super.sim_schedule(outputPort_, GridSimTags.SCHEDULE_NOW, tag, new IO_data(array, 8, destId));
                success = true;
                break;
            default:
    ...
    protected boolean sendCancelGridlet(int tag, Gridlet gl, int gridletId, int destId) {
        ...
        super.sim_schedule(outputPort_, GridSimTags.SCHEDULE_NOW, tag, new IO_data(gl, gridletSize, destId));
        return true;
    }
