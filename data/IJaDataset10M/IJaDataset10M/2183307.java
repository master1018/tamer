...
public class AntlrFormatterTest {
    ...
    public AntlrFormatterTest(int id, File filename, String content, AntlrFormatterOptions options) {
        this.file = filename;
        this.id = id;
    ...
    private static PrintWriter createWriter(String filename, String ext) throws FileNotFoundException {
        return new PrintWriter(OUTPUT + "/" + filename + ext);
    }

    private static void write(String filename, String ext, String content) throws FileNotFoundException {
    ...
    public void format() {
        try {
            ...
            printer.render(formattedTree, createWriter(file.getName(), "d.tree"));
            String stringTree = printer.renderAsString(tree, "Grammar");
            String formattedStringTree = printer.renderAsString(formattedTree, "Grammar");
    ...
    public static void setUp() throws Exception {
        ...
        output.mkdirs();
        Collection<File> files = AntlrTestUtility.collectFiles(output, false, "source", "tree");
    ...
    public static Collection<Object[]> getPaths() {
        ...
        int i = 0;
        for (File file : files) {
            paths.add(new Object[] { i++, file, AntlrTestUtility.readContent(file), AntlrFormatterOptions.DEFAULT });
        }
        return paths;
    }
}
