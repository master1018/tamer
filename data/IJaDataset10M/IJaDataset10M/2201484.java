...
public class OMAC implements IMac {
    ...
    public void init(Map attrib) throws InvalidKeyException {
        ...
        if (os != null) {
            ...
            if (outputSize < 0 || outputSize > blockSize) throw new IllegalArgumentException("truncated size out of range");
        } else outputSize = blockSize;
        ...
        if (Lu != null) {
            ...
            if (Lu.length != blockSize) Lu = new byte[blockSize];
        } else Lu = new byte[blockSize];
        if (Lu2 != null) {
            Arrays.fill(Lu2, (byte) 0);
            if (Lu2.length != blockSize) Lu2 = new byte[blockSize];
        } else Lu2 = new byte[blockSize];
        boolean msb = (L[0] & 0x80) != 0;
        for (int i = 0; i < blockSize; i++) {
            Lu[i] = (byte) (L[i] << 1 & 0xFF);
            if (i + 1 < blockSize) Lu[i] |= (byte) ((L[i + 1] & 0x80) >> 7);
        }
        if (msb) {
            if (blockSize == 16) Lu[Lu.length - 1] ^= C1; else if (blockSize == 8) Lu[Lu.length - 1] ^= C2; else throw new IllegalArgumentException("unsupported cipher block size: " + blockSize);
        }
        if (Configuration.DEBUG) log.fine("Lu = " + Util.toString(Lu).toLowerCase());
        msb = (Lu[0] & 0x80) != 0;
        for (int i = 0; i < blockSize; i++) {
            Lu2[i] = (byte) (Lu[i] << 1 & 0xFF);
            if (i + 1 < blockSize) Lu2[i] |= (byte) ((Lu[i + 1] & 0x80) >> 7);
        }
        if (msb) {
            if (blockSize == 16) Lu2[Lu2.length - 1] ^= C1; else Lu2[Lu2.length - 1] ^= C2;
        }
        if (Configuration.DEBUG) log.fine("Lu2 = " + Util.toString(Lu2).toLowerCase());
        if (M != null) {
            Arrays.fill(M, (byte) 0);
            if (M.length != blockSize) M = new byte[blockSize];
        } else M = new byte[blockSize];
        if (Y != null) {
            Arrays.fill(Y, (byte) 0);
            if (Y.length != blockSize) Y = new byte[blockSize];
        } else Y = new byte[blockSize];
        index = 0;
        init = true;
    }
    ...
    public void update(byte[] buf, int off, int len) {
        if (!init) throw new IllegalStateException("not initialized");
        if (off < 0 || len < 0 || off + len > buf.length) throw new IndexOutOfBoundsException("size=" + buf.length + "; off=" + off + "; len=" + len);
        for (int i = 0; i < len; ) {
            if (index == blockSize) {
                process();
                index = 0;
    ...
    public void digest(byte[] out, int off) {
        if (!init) throw new IllegalStateException("not initialized");
        if (off < 0 || off + outputSize > out.length) throw new IndexOutOfBoundsException("size=" + out.length + "; off=" + off + "; len=" + outputSize);
        byte[] T = new byte[blockSize];
        byte[] L = Lu;
        if (index < blockSize) {
            M[index++] = (byte) 0x80;
            while (index < blockSize) M[index++] = 0;
            L = Lu2;
        }
        for (int i = 0; i < blockSize; i++) T[i] = (byte) (M[i] ^ Y[i] ^ L[i]);
    ...
    private void process() {
        for (int i = 0; i < blockSize; i++) M[i] = (byte) (M[i] ^ Y[i]);
