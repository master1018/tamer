...
public class TopicMapLinkPlugin extends DefaultPlugin {
    ...
    public String generateHTML(ContextTag contextTag) {
        if (contextTag == null) throw new OntopiaRuntimeException("Plugin must have a parent tolog:context tag.");
        TopicMapIF tm = contextTag.getTopicMap();
        if (tm == null) return "<span>No topic map!</span>";
        ...
        if (reifier != null) tmtitle = TopicStringifiers.toString(reifier); else {
            TopicMapReferenceIF reference = tm.getStore().getReference();
            if (reference != null) tmtitle = reference.getTitle();
        }
