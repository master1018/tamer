...
public class SpeedsLabel extends HSISubcomponent {
    ...
    public void paint(Graphics2D g2) {
        ...
        g2.drawString(wind_dir_text, wind_dir_x, hsi_gc.border_top + (int) (line_height_medium * 2));
        g2.drawString(wind_speed_text, wind_speed_x, hsi_gc.border_top + (int) (line_height_medium * 2));
        if (wind_speed > 4) {
            AffineTransform original_at = null;
            if (wind_speed > 0) {
                original_at = g2.getTransform();
                AffineTransform rotate = AffineTransform.getRotateInstance(Math.toRadians((double) (wind_direction - heading + this.aircraft.magnetic_variation())), wind_dir_arrow_x, wind_dir_arrow_y);
                g2.transform(rotate);
            }
            GeneralPath polyline;
            g2.draw(new Line2D.Double(wind_dir_arrow_x, wind_dir_arrow_y - (wind_dir_arrow_length / 2), wind_dir_arrow_x, wind_dir_arrow_y + (wind_dir_arrow_length / 2)));
