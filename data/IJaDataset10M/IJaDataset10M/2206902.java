package org.openscience.cdk.applications.taverna.test.qsar.descriptors.molecular;
...
import org.openscience.cdk.applications.taverna.qsar.descriptors.molecular.MomentOfInertia;
 ...
 *
 */
public class MomentOfInertiaTest extends CDKTavernaTestCase {
    ...
    private String descriptorName = "MomentOfInertia_Descriptor";
    ...
    public void test_LocalWorkerMomentOfInertia() throws CDKException, Exception {
        MomentOfInertia test = new MomentOfInertia();
        ...
        if (resultListNOTCalculatedStructures.size() != 0) {
            ...
            CDKIOFileWriter.writeListOfCMLChemFilesToFile(resultListNOTCalculatedStructures, "Error_" + descriptorName + ".txt", TavernaTestData.getPathForWritingFilesOfUnitTests());
        }
        ...
        if (resultListCalculatedStructures.size() != 0) {
            for (Iterator iter = resultListCalculatedStructures.iterator(); iter.hasNext(); ) {
                ...
                if (!((String) fileName.get(fileName.size() - 1)).equalsIgnoreCase("MomentOfInertia_Descriptor")) {
                    hasFileNameAnError = true;
                }
            }
        }
