...
public class MyAuthenticator extends Authenticator {
    ...
    protected PasswordAuthentication getPasswordAuthentication() {
        ...
        if (mapping.containsKey(reqURL)) return mapping.get(reqURL);
        if (mapping.containsKey(shortReq)) return mapping.get(shortReq);
        ...
        jd.setVisible(true);
        if (username.getText().length() > 0 || password.getPassword().length > 0) return new PasswordAuthentication(username.getText(), password.getPassword()); else return null;
    }
}
