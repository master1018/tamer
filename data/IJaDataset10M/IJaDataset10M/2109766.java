...
public class PersonPopulate {
   ...
   */
    ...
    private PhoneDb getNewPhone(int i) {
        ...
        PhoneDb phone = findOne(PhoneDb.FINDBY_PHONE_NUMBER, new String[] { PhoneDb.PARAM_PHONE_NUMBER }, new Object[] { targetPhone });
        if (phone == null) {
            phone = new PhoneDb();
    ...
    private PhotoDb getNewPhoto(int i) {
        ...
        PhotoDb photo = findOne(PhotoDb.FINDBY_PHOTO, new String[] { PhotoDb.PARAM_PHOTO }, new Object[] { targetPhoto });
        if (photo == null) {
            photo = new PhotoDb();
    ...
    private Name getNewName(int i) {
        ...
        Name name = findOne(NameDb.FINDBY_FAMILY_NAME, new String[] { NameDb.PARAM_FAMILY_NAME }, new Object[] { targetName });
        if (name == null) {
            name = new NameDb();
    ...
    private Organization getDbOrganization(int i, String type) {
        ...
        Organization organization = findOne(OrganizationDb.FINDBY_NAME, new String[] { OrganizationDb.PARAM_NAME }, new Object[] { targetOrg });
    ...
    private Organization getPersonOrganization(int i, String type) {
        String targetOrg = "Organization_" + (i % 10);
        PersonOrganizationDb organization = findOne(PersonOrganizationDb.PERSON_ORG_FINDBY_NAME, new String[] { PersonOrganizationDb.PARAM_NAME }, new Object[] { targetOrg });
        if (organization == null) {
            organization = new PersonOrganizationDb();
            organization.setDescription("Description");
            organization.setEndDate(new Date(System.currentTimeMillis() + (24L * 3600L * 1000L * 365L * 2L)));
            organization.setAddress(getNewOrganizationAddress(i * 3));
            organization.setName(targetOrg);
            organization.setSalary(String.valueOf(i * 1000));
            organization.setStartDate(new Date(System.currentTimeMillis() - (24L * 3600L * 1000L * 365L * 2L)));
    ...
    private ListField getNewEmail(int i) {
        ...
        ListField email = findOne(EmailDb.FINDBY_EMAIL, new String[] { EmailDb.PARAM_EMAIL }, new Object[] { targetAddress });
        if (email == null) {
            email = new EmailDb();
    ...
    private BodyType getNewBodyType(int i) {
        BodyType bodyType = findOne(BodyTypeDb.FINDBY_HEIGHT, new String[] { BodyTypeDb.PARAM_HEIGHT }, new Object[] { new Float(i % 10) });
        if (bodyType == null) {
    ...
    private Address getNewAddress(int i) {
        Address address = findOne(AddressDb.FINDBY_POSTCODE, new String[] { AddressDb.PARAM_POSTCODE }, new Object[] { String.valueOf(i % 10) });
        if (address == null) {
    ...
    public void destroyPerson(int i, long key) {
        List<Person> people = find(PersonDb.FINDBY_LIKE_PERSONID, new String[] { PersonDb.PARAM_PERSONID }, new Object[] { getPersonId(i, key) });
        for (Person o : people) {
    ...
    protected Person getPerson(String id) {
        return find(PersonDb.FINDBY_PERSONID, new String[] { PersonDb.PARAM_PERSONID }, new Object[] { id });
    }
}
