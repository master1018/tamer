...
import co.edu.unal.ungrid.registration.evolutionary.operator.TwoOffspringConvexRealEncodingCrossover;
import co.edu.unal.ungrid.registration.evolutionary.operator.TwoOffspringCrossoverOperator;
import co.edu.unal.ungrid.registration.evolutionary.operator.TwoOffsringCrossoverOperatorFactory;
...
public class TwoOffsringGeneticSelection implements Serializable, XmlConfigurable, Verifiable {
    ...
    public void config(Element root) {
        TwoOffspringCrossoverOperator cop = TwoOffsringCrossoverOperatorFactory.getInstance(XmlUtil.getParameter(root, "crossover-operator-class"));
        if (cop != null) {
            setCrossoverOperator(cop);
        } else {
            setCrossoverOperator(new TwoOffspringConvexRealEncodingCrossover());
        }
    ...
    public Individual selectAtRandom(GeneticMethodParameters params, Generation generation) {
        return generation.getRandIndividual();
    }

    public Individual selectByTournament(GeneticMethodParameters params, Generation generation) {
        Individual ind = null;
        if (params != null && generation != null) {
            if (params.getInt(GeneticMethodParameters.TOURNAMENT_SIZE) == params.getInt(GeneticMethodParameters.POPULATION_SIZE)) {
                ind = generation.getBestValue();
            } else {
                int nTounamentSize = params.getInt(GeneticMethodParameters.TOURNAMENT_SIZE);
                for (int t = 0; t < nTounamentSize; t++) {
                    Individual rand = generation.getRandIndividual();
                    if (rand != null) {
    ...
    public void setCrossoverOperator(TwoOffspringCrossoverOperator cop) {
        assert cop != null;
        m_crossover = cop;
    }
    ...
    private TwoOffspringCrossoverOperator m_crossover;
