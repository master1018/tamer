 ...
 */
public class TranslationDifference extends TranslationDualListen {

    protected static String[] itsArgs = new String[] { "Difference", "Get difference between two other points", "MonitorPoint 1", "java.lang.String", "MonitorPoint 2", "java.lang.String" };

    public TranslationDifference(PointDescription parent, String[] init) {
        super(parent, init);
    }
   ...
   *@return Difference between the two values */
    protected Object doCalculations(Object val1, Object val2) {
        if (val1 instanceof Integer && val2 instanceof Integer) {
            return new Integer(((Integer) val1).intValue() - ((Integer) val2).intValue());
        } else if (val1 instanceof Long && val2 instanceof Long) {
            return new Long(((Long) val1).longValue() - ((Long) val2).longValue());
        } else if (val1 instanceof Number && val2 instanceof Number) {
            return new Double(((Number) val1).doubleValue() - ((Number) val2).doubleValue());
        } else {
            System.err.println("TranslationDifference: " + itsParent.getName() + ": Unsupported types");
            return null;
        }
    }
