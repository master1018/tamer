...
public class Options extends TypedProperties {
    ...
    public String[] setFromCmdLine(String[] args) {
        ...
        for (int i = 0; i < args.length + 1; i++) {
            if (i == args.length || args[i].startsWith("-")) {
                key = trimQuote(key);
                if (key != null) {
                    if (value != null && value.length() > 0) setProperty(key, trimQuote(value)); else setProperty(key, "true");
                }
                if (i == args.length) break; else {
    ...
    public void setInto(Object obj) {
        ...
        if (defaults != null) {
            for (Iterator itr = defaults.entrySet().iterator(); itr.hasNext(); ) {
                ...
                if (!containsKey(entry.getKey())) setInto(obj, entry);
            }
        }
        for (Iterator itr = entrySet().iterator(); itr.hasNext(); ) setInto(obj, (Map.Entry) itr.next());
    }
    ...
    private void setInto(Object obj, Map.Entry entry) {
        ...
        try {
            ...
            if (!matchOptionToSetter(entry.getKey().toString(), match)) return;
            Method setter = (Method) match[1];
            Class[] paramTypes = setter.getParameterTypes();
            Object[] values = new Object[paramTypes.length];
            String[] strValues = (entry.getValue() == null) ? new String[1] : Strings.split(entry.getValue().toString(), ",", -1);
            for (int i = 0; i < strValues.length; i++) values[i] = stringToObject(strValues[i], paramTypes[i]);
            for (int i = strValues.length; i < values.length; i++) values[i] = getDefaultValue(paramTypes[i]);
            setter.invoke(match[0], values);
        } catch (Throwable t) {
            throw new IllegalArgumentException(obj + "." + entry.getKey() + " = " + entry.getValue());
        }
    }
    ...
    private static String trimQuote(String val) {
        if (val != null && val.startsWith("'") && val.endsWith("'")) return val.substring(1, val.length() - 1);
        return val;
    }
	 ...
	 *					the key
	 */
    private static boolean matchOptionToSetter(String key, Object[] match) throws Exception {
        ...
        for (int i = 0; i < meths.length; i++) {
            if (meths[i].getName().equals(set)) setter = meths[i]; else if (meths[i].getName().equals(get)) getter = meths[i];
        }
        if (setter == null && getter == null) return false;
        if (find.length > 1) {
            Object inner = null;
            if (getter != null) inner = getter.invoke(match[0], null);
            if (inner == null) {
	 ...
	 */
    private static Object stringToObject(String str, Class type) throws Exception {
        if (str == null || type == String.class) return str;
        if (type == Class.class) return Class.forName(str);
        if (type.isPrimitive() || Number.class.isAssignableFrom(type)) if (str.length() > 2 && str.endsWith(".0")) str = str.substring(0, str.length() - 2);
        if (type.isPrimitive()) for (int i = 0; i < _primWrappers.length; i++) if (type == _primWrappers[i][0]) return stringToObject(str, (Class) _primWrappers[i][1]);
        Exception err = null;
        try {
        ...
        if (!type.isAssignableFrom(subType)) throw err;
    ...
    private Object getDefaultValue(Class type) {
        for (int i = 0; i < _primWrappers.length; i++) if (_primWrappers[i][0] == type) return _primWrappers[i][2];
        return null;
    }
}
