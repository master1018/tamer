...
public class ExponentialFormat extends DecimalFormat implements RevisionHandler {
    ...
    public int width() {
        ...
        if (sign) return 1 + digits + 2 + exp; else return digits + 2 + exp;
    }
    ...
    public StringBuffer format(double number, StringBuffer toAppendTo, FieldPosition pos) {
        ...
        if (sign) {
            ...
            if (s.charAt(0) == '-') s.setCharAt(0, '*'); else s.deleteCharAt(0);
        }
    ...
    private String pattern() {
        ...
        s.append("0.");
        for (int i = 0; i < digits - 1; i++) if (trailing) s.append('0'); else s.append('#');
        s.append('E');
