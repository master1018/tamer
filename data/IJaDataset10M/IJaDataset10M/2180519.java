 ...
 */
public class BatchConverter implements XBatchConverter, XServiceName, XServiceInfo, XTypeProvider {
    ...
    private boolean convertDirectory(String sSourceURL, String sTargetURL, String sHeading) {
        ...
        try {
            if (!sfa2.exists(sTargetURL)) {
                sfa2.createFolder(sTargetURL);
            }
        } catch (CommandAbortedException e) {
            handler.endDirectory(sSourceURL, false);
            return true;
        ...
        try {
            Object xPipeObj = xComponentContext.getServiceManager().createInstanceWithContext("com.sun.star.io.Pipe", xComponentContext);
    ...
    private IndexPageEntry convertFile(String sSourceFileURL, String sTargetURL) {
        ...
        try {
            Object desktop = xComponentContext.getServiceManager().createInstanceWithContext("com.sun.star.frame.Desktop", xComponentContext);
        ...
        if (UnoRuntime.queryInterface(XTextDocument.class, xDocument) != null) {
            bText = true;
        } else if (UnoRuntime.queryInterface(XSpreadsheetDocument.class, xDocument) != null) {
            bSpreadsheet = true;
        }
        ...
        if (bText && writerFilterData != null) {
            exportProps.put("FilterData", writerFilterData);
        }
        if (bSpreadsheet && calcFilterData != null) {
            exportProps.put("FilterData", calcFilterData);
        }
        try {
