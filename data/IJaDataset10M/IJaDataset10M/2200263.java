...
public class CreateRequestFactoryMethodProposal extends ASTRewriteCorrectionProposal {

    private static final Image ICON = JavaPlugin.getImageDescriptorRegistry().get(JavaPluginImages.DESC_MISC_PUBLIC);

    private static final int RELEVANCE = 100;

    public static List<IJavaCompletionProposal> createProposals(ICompilationUnit serviceCompilationUnit, ASTNode problemNode) {
        try {
            String serviceName = serviceCompilationUnit.findPrimaryType().getElementName();
            IType requestContext = RequestFactoryUtils.getRequestForService(serviceCompilationUnit.findPrimaryType().getFullyQualifiedName(), serviceCompilationUnit.getJavaProject());
            ...
            return Collections.<IJavaCompletionProposal>singletonList(new CreateRequestFactoryMethodProposal(serviceName, requestContext.getCompilationUnit(), requestContext, serviceMethod));
    ...
    private static String generateDisplayName(String typeName, String methodName) {
        return MessageFormat.format("Generate method ''{0}'' in type ''{1}''", methodName, typeName);
    }
    ...
    private CreateRequestFactoryMethodProposal(String serviceName, ICompilationUnit requestCompilationUnit, IType requestContext, MethodDeclaration method) {
        super(generateDisplayName(requestContext.getElementName(), method.getName().getIdentifier()), requestCompilationUnit, null, RELEVANCE, ICON);
    ...
    public void apply(IDocument document) {
        ...
        if (!entityList.isEmpty()) {
            done = generateProxyLocator(proxies.get(0).getPackageFragment()) && addServiceMethods();
        }
        if (!done) {
        }
    }
    ...
    protected MethodDeclaration createMethodDeclaration(ASTRewrite rewriter) {
        ...
        try {
            ...
            for (IType proxy : proxies) {
                IAnnotation annotation = proxy.getAnnotation("ProxyForName");
        ...
        MethodDeclaration methodDeclaration = (MethodDeclaration) rewriter.createStringPlaceholder(CodegenUtils.format(buf.toString(), CodeFormatter.K_CLASS_BODY_DECLARATIONS), ASTNode.METHOD_DECLARATION);
   ...
   */
    private boolean addServiceMethods() {
        ...
        try {
            ...
            StringBuffer buf = new StringBuffer();
            for (IType entity : entityList) {
                buf.append(CodegenUtils.format(RequestFactoryCodegenUtils.constructServiceMethods(entity, method), CodeFormatter.K_CLASS_BODY_DECLARATIONS));
            }
            MethodDeclaration methodDecl = (MethodDeclaration) listRewriter.getASTRewrite().createStringPlaceholder(buf.toString(), ASTNode.METHOD_DECLARATION);
            listRewriter.insertLast(methodDecl, null);
   ...
   */
    private boolean generateProxyLocator(IPackageFragment pack) {
        for (IType entity : entityList) {
            CompilationUnitCreator creator = new CompilationUnitCreator(projectEntities, null);
            try {
                creator.create(entity, pack, entity.getElementName() + "Proxy", RpcType.PROXY, new NullProgressMonitor());
                creator.create(entity, entity.getPackageFragment(), entity.getElementName() + "Locator", RpcType.LOCATOR, new NullProgressMonitor());
            } catch (CoreException e) {
                AppEngineRPCPlugin.log(e);
                return false;
            }
