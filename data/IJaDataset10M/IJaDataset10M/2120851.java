...
public class SkinNode extends Node implements Savable, BoneChangeListener {
    ...
    protected void validateSkins() {
        ...
        for (Spatial child : skins.getChildren()) if (!(child instanceof Geometry)) throw new IllegalStateException("'skins' contains non-Geometry child: " + child.getName() + " of type " + child.getClass().getName());
    }
     ...
     */
    public void addBoneInfluence(int geomIndex, int vert, Bone bone, float weight) {
        if (weight == 0) return;
    ...
    public String getAnimationString() {
        if (skeleton != null && skeleton.getAnimationController() != null && skeleton.getAnimationController().getActiveAnimation() != null) {
            return skeleton.getAnimationController().getActiveAnimation().getName();
        }
        return null;
    }

    public void addBoneInfluence(int geomIndex, int vert, String boneId, float weight) {
        if (weight == 0) return;
        if (cache == null) recreateCache();
        ArrayList<BoneInfluence> infs = cache[geomIndex][vert];
        if (infs == null) {
    ...
    public void assignSkeletonBoneInfluences() {
        ...
        if (cache == null) return;
        validateSkins();
        for (int index = cache.length - 1; index >= 0; --index) assignSkeletonBoneInfluences(index);
    ...
    public void assignSkeletonBoneInfluences(Geometry skinGeo) {
        if (skeleton == null) throw new IllegalStateException("No skeleton assigned yet");
        if (cache == null) throw new IllegalStateException("No skins initialized yet");
        validateSkins();
        for (int index = skins.getQuantity() - 1; index >= 0; --index) if (getSkin(index) == skinGeo) {
            assignSkeletonBoneInfluences(index);
            return;
        }
        throw new IllegalArgumentException("Geometry is not one of our skins: " + skinGeo.getName());
    }

    protected void assignSkeletonBoneInfluences(int i) {
        for (ArrayList<BoneInfluence> biList : cache[i]) if (biList != null) for (int j = 0; j < biList.size(); j++) biList.get(j).assignBone(skeleton);
    }
    ...
    public void regenInfluenceOffsets() {
        if (cache == null) return;
        validateSkins();
        for (int index = cache.length - 1; index >= 0; --index) regenInfluenceOffsets(index);
    }
    ...
    public synchronized void updateSkin() {
        ...
        for (int index = cache.length - 1; index >= 0; --index) {
            ...
            if (cache[index].length * 3 > verts.limit()) throw new IllegalStateException("Skin " + getName() + ':' + geom.getName() + " has more influences than " + " vertexes.  " + cache[index].length + " vs. " + verts.limit() + "/3");
            if (cache[index].length * 3 > norms.limit()) throw new IllegalStateException("Skin " + getName() + ':' + geom.getName() + " has more influences than " + " normals.  " + cache[index].length + " vs. " + norms.limit() + "/3");
            if (cache[index].length * 3 < verts.limit()) logger.log(Level.WARNING, "Skin ''{0}:{1}'' has fewer " + "influences than vertexes.  {2} vs {3}/3", new Object[] { getName(), geom.getName(), cache[index].length, verts.limit() });
            if (cache[index].length * 3 < norms.limit()) logger.log(Level.WARNING, "Skin ''{0}:{1}'' has fewer " + "influences than normals.  {2} vs {3}/3", new Object[] { getName(), geom.getName(), cache[index].length, norms.limit() });
    ...
    public void write(JMEExporter e) throws IOException {
        ...
        cullRegionMappings();
        if (geometryRegions.size() > 0) cap.write(geometryRegions, "geometryRegions", null);
    }
    ...
    public void remapInfluences(VertMap mappings, int geomIndex) {
        ...
        ArrayList<BoneInfluence>[] newInfls = new ArrayList[getSkin(geomIndex).getVertexCount()];
        cache[geomIndex] = newInfls;
        for (int x = 0; x < infls.length; x++) {
            for (int y = 0; y < infls[x].size(); y++) {
                BoneInfluence bi = infls[x].get(y);
                if (bi.bone != null) addBoneInfluence(geomIndex, mappings.getNewIndex(x), bi.bone, bi.weight); else addBoneInfluence(geomIndex, mappings.getNewIndex(x), bi.boneId, bi.weight);
            }
        }
        normalizeWeights(geomIndex);
    }
    ...
    public void removeGeometry(int geomIndex) {
        ...
        for (int x = 0; x < cache.length - 1; x++) newCache[x] = cache[(x < geomIndex) ? x : (x + 1)];
        cache = newCache;
    }
    ...
    public void assimilate(SkinNode otherSkinNode, String geoNameRegex) {
        ...
        if (otherSkins == null || otherSkins.getQuantity() < 1) {
            logger.log(Level.WARNING, "Not merging SkinNode ''{0}'' into ''{1}'' " + "since no skin meshes for former", new String[] { otherSkinNode.getName(), getName() });
            return;
        }
        BoneAnimation origAnim = null;
        int origFrame = -1;
        boolean origActive = false;
        AnimationController ac = (skeleton == null) ? null : skeleton.getAnimationController();
        if (ac == null) {
        ...
        if (otherCache == null) throw new IllegalArgumentException("Other skin has skin mesh(es), but no influence cache: " + otherSkinNode.getName());
        if (otherSkins.getQuantity() != otherSkinNode.getCache().length) throw new IllegalArgumentException("SkinNode '" + otherSkinNode.getName() + "' has skin geo vs. cache count mismatch: " + otherSkins.getQuantity() + " vs. " + otherSkinNode.getCache().length);
        logger.log(Level.FINE, "Merging SkinNode ''{0}'' into ''{1}''", new String[] { otherSkinNode.getName(), getName() });
        if (otherSkinNode.bindMatrix != null) throw new IllegalArgumentException("Skin bindMatrixes are not supported for assimilation");
        ArrayList<BoneInfluence>[] transferredInfluences;
        String skinRegion = (otherSkinNode instanceof SkinTransferNode) ? ((SkinTransferNode) otherSkinNode).getSkinRegion() : null;
        Geometry g;
        validateSkins();
        otherSkinNode.validateSkins();
        if (geoNameRegex == null && skinRegion != null && skins != null && skins.getQuantity() > 0) {
            int rmCount = 0;
            cullRegionMappings();
            ...
            for (int i = childCount - 1; i >= 0; i--) {
                ...
                if (!geometryRegions.get(g.getName()).equals(skinRegion)) continue;
                if (otherSkinNode.hasSkinGeometry(g.getName(), skinRegion)) {
                    logger.log(Level.INFO, "Retaining geometry ''{0}''", g.getName());
                    continue;
                }
        ...
        for (int i = otherSkins.getQuantity() - 1; i >= 0; i--) {
            ...
            if (geoNameRegex == null) {
                if (skinRegion != null && hasSkinGeometry(g.getName(), skinRegion)) continue;
            } else {
                if (!g.getName().matches(geoNameRegex)) continue;
            }
            transferredInfluences = otherCache[i];
            assimilate(otherSkinNode.removeSkinGeometry(g.getName()), transferredInfluences, skinRegion);
        }
        if (ac != null) {
    ...
    @SuppressWarnings("unchecked")
    protected void assimilate(Geometry newSkinGeo, ArrayList<BoneInfluence>[] newInfluences, String skinRegion) {
        addSkin(newSkinGeo);
        if (skinRegion != null) geometryRegions.put(newSkinGeo.getName(), skinRegion);
        ArrayList<BoneInfluence>[][] newCache = (ArrayList<BoneInfluence>[][]) new ArrayList[skins.getQuantity()][];
        for (int i = 0; i < cache.length; i++) newCache[i] = cache[i];
        newCache[newCache.length - 1] = (ArrayList<BoneInfluence>[]) new ArrayList[newInfluences.length];
        cache = newCache;
        for (int i = 0; i < newInfluences.length; i++) for (BoneInfluence bi : newInfluences[i]) addBoneInfluence(newCache.length - 1, i, bi.boneId, bi.weight);
        assignSkeletonBoneInfluences(newSkinGeo);
        regenInfluenceOffsets(newSkinGeo);
        logger.log(Level.INFO, "Assimilating Geometry ''{0}''", newSkinGeo.getName());
    }
    ...
    public Geometry removeSkinGeometry(String geoName) {
        ...
        Geometry g;
        validateSkins();
        for (int i = 0; i < childCount; i++) if (getSkin(i).getName().equals(geoName)) return removeSkinGeometry(i);
        return null;
    }
    ...
    public boolean hasSkinGeometry(String geoName, String skinRegion) {
        ...
        validateSkins();
        for (Spatial child : skins.getChildren()) if (child.getName().equals(geoName) && (skinRegion == null || (geometryRegions.containsKey(geoName) && geometryRegions.get(geoName).equals(skinRegion)))) return true;
        return false;
    }
    ...
    public Geometry removeSkinGeometry(int i) {
        if (skins == null) return null;
        validateSkins();
        if (i >= skins.getQuantity()) throw new IllegalArgumentException("Can't remove child index " + i + " when there are only " + skins.getQuantity() + " children");
    ...
    protected void cullRegionMappings() {
        ...
        EACH_KEY: for (String key : geometryRegions.keySet()) {
    ...
    public void pose(int frameNum) {
        AnimationController ac = (skeleton == null) ? null : skeleton.getAnimationController();
        if (ac == null) throw new IllegalStateException("Can't pose without an AnimationController set");
        BoneAnimation animation = ac.getActiveAnimation();
        if (animation == null) throw new IllegalStateException("Controller has no active animation");
        ac.setActive(false);
    ...
    public void pose(String animationName, int frameNum) {
        if (animationName == null) throw new IllegalArgumentException("Target animation must be specified");
        String origAnimationName = getAnimationString();
        AnimationController ac = (skeleton == null) ? null : skeleton.getAnimationController();
        if (ac == null) throw new IllegalStateException("Can't pose without an AnimationController set");
        if (ac.getActiveAnimation() == null) throw new IllegalStateException("Controller has no such animation as '" + animationName + "'");
        pose(frameNum);
    }
