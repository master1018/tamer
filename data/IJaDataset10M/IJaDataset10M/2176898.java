...
import org.drftpd.transfer.Transfer;
import org.drftpd.transfer.TransferState;
...
public class UserManagementHandler extends CommandInterface {
    ...
    public CommandResponse doSITE_ADDIP(CommandRequest request) throws ImproperUsageException {
        ...
        if (args.length < 2) {
            return new CommandResponse(501, session.jprintf(_bundle, _keyPrefix + "addip.specify", request.getUser()));
        }
        CommandResponse response = StandardCommandManager.genericResponse("RESPONSE_200_COMMAND_OK");
        User myUser;
        try {
            myUser = GlobalContext.getGlobalContext().getUserManager().getUserByName(args[0]);
            if (session.getUserNull(request.getUser()).isGroupAdmin() && !session.getUserNull(request.getUser()).getGroup().equals(myUser.getGroup())) {
                return StandardCommandManager.genericResponse("RESPONSE_530_ACCESS_DENIED");
            }
            ReplacerEnvironment env = new ReplacerEnvironment();
            env.add("targetuser", myUser.getName());
            for (int i = 1; i < args.length; i++) {
                String string = args[i].replace(",", "");
                env.add("mask", string);
                try {
                    myUser.addIPMask(string);
                    response.addComment(session.jprintf(_bundle, _keyPrefix + "addip.success", env, request.getUser()));
                    logger.info("'" + session.getUserNull(request.getUser()).getName() + "' added ip '" + string + "' to '" + myUser.getName() + "'");
    ...
    public CommandResponse doGenericAddUser(boolean isGAdduser, CommandRequest request) throws ImproperUsageException {
        ...
        if (session.getUserNull(request.getUser()).isGroupAdmin()) {
            ...
            int users;
            users = GlobalContext.getGlobalContext().getUserManager().getAllUsersByGroup(session.getUserNull(request.getUser()).getGroup()).size();
            logger.debug("Group " + session.getUserNull(request.getUser()).getGroup() + " is " + GlobalContext.getGlobalContext().getUserManager().getAllUsersByGroup(session.getUserNull(request.getUser()).getGroup()));
            if (users >= session.getUserNull(request.getUser()).getKeyedMap().getObjectInt(UserManagement.GROUPSLOTS)) {
                return new CommandResponse(452, session.jprintf(_bundle, _keyPrefix + "adduser.noslots", request.getUser()));
            }
        ...
        try {
            ...
            if (newGroup != null) {
                ...
                logger.info("'" + request.getUser() + "' added '" + newUser.getName() + "' with group " + newUser.getGroup() + "'");
        ...
        while (st.hasMoreTokens()) {
            String string = st.nextToken().replace(",", "");
            env.add("mask", string);
            try {
    ...
    public CommandResponse doSITE_CHANGE(CommandRequest request) throws ImproperUsageException {
        ...
        try {
            userToChange = GlobalContext.getGlobalContext().getUserManager().getUserByName(username);
        ...
        if ("ratio".equals(command)) {
            ...
            if (session.getUserNull(request.getUser()).isGroupAdmin() && !session.getUserNull(request.getUser()).isAdmin()) {
                if (!session.getUserNull(request.getUser()).getGroup().equals(userToChange.getGroup())) {
                    return StandardCommandManager.genericResponse("RESPONSE_530_ACCESS_DENIED");
                }
                if (ratio == 0F) {
                    int usedleechslots = 0;
                    for (Iterator iter = GlobalContext.getGlobalContext().getUserManager().getAllUsersByGroup(session.getUserNull(request.getUser()).getGroup()).iterator(); iter.hasNext(); ) {
                        if (((User) iter.next()).getKeyedMap().getObjectFloat(UserManagement.RATIO) == 0F) {
                            usedleechslots++;
                        }
                    }
                    if (usedleechslots >= session.getUserNull(request.getUser()).getKeyedMap().getObjectInt(UserManagement.LEECHSLOTS)) {
                        return new CommandResponse(452, session.jprintf(_bundle, _keyPrefix + "changeratio.nomoreslots", request.getUser()));
                    }
                } else if (ratio < session.getUserNull(request.getUser()).getMinRatio() || ratio > session.getUserNull(request.getUser()).getMaxRatio()) {
                    ...
                    return new CommandResponse(452, session.jprintf(_bundle, _keyPrefix + "changeratio.invalidratio", env, request.getUser()));
                }
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed ratio for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObjectFloat(UserManagement.RATIO) + "' to '" + ratio + "'");
                userToChange.getKeyedMap().setObject(UserManagement.RATIO, new Float(ratio));
                env.add("newratio", Float.toString(userToChange.getKeyedMap().getObjectFloat(UserManagement.RATIO)));
                response.addComment(session.jprintf(_bundle, _keyPrefix + "changeratio.success", env, request.getUser()));
            } else {
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed ratio for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObjectFloat(UserManagement.RATIO) + " to '" + ratio + "'");
                userToChange.getKeyedMap().setObject(UserManagement.RATIO, new Float(ratio));
                env.add("newratio", Float.toString(userToChange.getKeyedMap().getObjectFloat(UserManagement.RATIO)));
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed credits for '" + userToChange.getName() + "' from '" + userToChange.getCredits() + " to '" + credits + "'");
            userToChange.setCredits(credits);
            env.add("newcredits", Bytes.formatBytes(userToChange.getCredits()));
            response.addComment(session.jprintf(_bundle, _keyPrefix + "changecredits.success", env, request.getUser()));
        } else if ("comment".equals(command)) {
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed comment for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObjectString(UserManagement.COMMENT) + " to '" + fullCommandArgument + "'");
            userToChange.getKeyedMap().setObject(UserManagement.COMMENT, fullCommandArgument);
            env.add("comment", userToChange.getKeyedMap().getObjectString(UserManagement.COMMENT));
            response.addComment(session.jprintf(_bundle, _keyPrefix + "changecomment.success", env, request.getUser()));
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed idle_time for '" + userToChange.getName() + "' from '" + userToChange.getIdleTime() + " to '" + idleTime + "'");
            userToChange.setIdleTime(idleTime);
            env.add("newidletime", "" + idleTime);
            response.addComment(session.jprintf(_bundle, _keyPrefix + "changeidletime.success", env, request.getUser()));
            ...
            try {
                ...
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed num_logins for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObjectInt(UserManagement.MAXLOGINS) + "' '" + userToChange.getKeyedMap().getObjectInt(UserManagement.MAXLOGINSIP) + "' to '" + numLogins + "' '" + numLoginsIP + "'");
                ...
                response.addComment(session.jprintf(_bundle, _keyPrefix + "changenumlogins.success", env, request.getUser()));
            ...
            try {
                ...
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed gadmin min/max ratio for user '" + userToChange.getName() + "' group '" + userToChange.getGroup() + "' from '" + userToChange.getMinRatio() + "/" + userToChange.getMaxRatio() + "' to '" + minRatio + "/" + maxRatio + "'");
                if (minRatio < 1 || maxRatio < minRatio) return StandardCommandManager.genericResponse("RESPONSE_501_SYNTAX_ERROR");
                userToChange.setMinRatio(minRatio);
                userToChange.setMaxRatio(maxRatio);
                response.addComment(session.jprintf(_bundle, _keyPrefix + "changegadminratio.success", env, request.getUser()));
            } catch (NumberFormatException ex) {
                return StandardCommandManager.genericResponse("RESPONSE_501_SYNTAX_ERROR");
            }
            ...
            try {
                ...
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed max simultaneous download/upload slots for '" + userToChange.getName() + "' from '" + userToChange.getMaxSimDown() + "' '" + userToChange.getMaxSimUp() + "' to '" + maxdn + "' '" + maxup + "'");
                ...
                env.add("maxdn", "" + maxdn);
                env.add("maxup", "" + maxup);
                response.addComment(session.jprintf(_bundle, _keyPrefix + "changemaxsim.success", env, request.getUser()));
            } catch (NumberFormatException ex) {
                return StandardCommandManager.genericResponse("RESPONSE_501_SYNTAX_ERROR");
            }
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed primary group for '" + userToChange.getName() + "' from '" + userToChange.getGroup() + "' to '" + commandArguments[0] + "'");
            userToChange.setGroup(commandArguments[0]);
            env.add("primgroup", userToChange.getGroup());
            response.addComment(session.jprintf(_bundle, _keyPrefix + "changeprimgroup.success", env, request.getUser()));
            ...
            try {
                ...
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed group_slots for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObjectInt(UserManagement.GROUPSLOTS) + "' " + userToChange.getKeyedMap().getObjectInt(UserManagement.LEECHSLOTS) + "' to '" + groupSlots + "' '" + groupLeechSlots + "'");
                ...
                env.add("groupleechslots", "" + userToChange.getKeyedMap().getObjectInt(UserManagement.LEECHSLOTS));
                response.addComment(session.jprintf(_bundle, _keyPrefix + "changegroupslots.success", env, request.getUser()));
            } catch (NumberFormatException ex) {
                return StandardCommandManager.genericResponse("RESPONSE_501_SYNTAX_ERROR");
            }
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed created for '" + userToChange.getName() + "' from '" + new Date(userToChange.getKeyedMap().getObjectLong(UserManagement.CREATED)) + "' to '" + myDate + "'");
            userToChange.getKeyedMap().setObject(UserManagement.CREATED, myDate);
            response = new CommandResponse(200, session.jprintf(_bundle, _keyPrefix + "changecreated.success", env, request.getUser()));
        } else if ("wkly_allotment".equals(command)) {
            if (commandArguments.length != 1) {
                throw new ImproperUsageException();
            }
            long weeklyAllotment = Bytes.parseBytes(commandArguments[0]);
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed wkly_allotment for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObjectLong(UserManagement.WKLY_ALLOTMENT) + "' to " + weeklyAllotment + "'");
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed tagline for '" + userToChange.getName() + "' from '" + userToChange.getKeyedMap().getObject(UserManagement.TAGLINE, "") + "' to '" + fullCommandArgument + "'");
    ...
    public CommandResponse doSITE_CHGRP(CommandRequest request) throws ImproperUsageException {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            try {
                ...
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' removed '" + myUser.getName() + "' from group '" + string + "'");
                response.addComment(myUser.getName() + " removed from group " + string);
            } catch (NoSuchFieldException e1) {
                try {
                    myUser.addSecondaryGroup(string);
                    logger.info("'" + session.getUserNull(request.getUser()).getName() + "' added '" + myUser.getName() + "' to group '" + string + "'");
    ...
    public CommandResponse doSITE_CHPASS(CommandRequest request) throws ImproperUsageException {
        ...
        try {
            ...
            myUser.commit();
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed password for '" + myUser.getName() + "'");
    ...
    public CommandResponse doSITE_DELIP(CommandRequest request) throws ImproperUsageException {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            try {
                ...
                logger.info("'" + session.getUserNull(request.getUser()).getName() + "' removed ip '" + string + "' from '" + myUser + "'");
    ...
    public CommandResponse doSITE_DELUSER(CommandRequest request) throws ImproperUsageException {
        ...
        User myUser;
        try {
            myUser = GlobalContext.getGlobalContext().getUserManager().getUserByName(delUsername);
        } catch (NoSuchUserException e) {
            return new CommandResponse(452, e.getMessage());
        } catch (UserFileException e) {
        ...
        myUser.commit();
        logger.info("'" + session.getUserNull(request.getUser()).getName() + "' deleted user '" + myUser.getName() + "' with reason '" + reason + "'");
    ...
    public CommandResponse doSITE_GINFO(CommandRequest request) throws ImproperUsageException {
        ...
        if (session.getUserNull(request.getUser()).isGroupAdmin() && !session.getUserNull(request.getUser()).getGroup().equals(group)) {
            return StandardCommandManager.genericResponse("RESPONSE_530_ACCESS_DENIED");
        }
        CommandResponse response = StandardCommandManager.genericResponse("RESPONSE_200_COMMAND_OK");
        ReplacerEnvironment env = new ReplacerEnvironment();
    ...
    public CommandResponse doSITE_GIVE(CommandRequest request) throws ImproperUsageException {
        Session session = request.getSession();
        if (!GlobalContext.getConfig().checkPermission("give", session.getUserNull(request.getUser()))) {
            return StandardCommandManager.genericResponse("RESPONSE_530_ACCESS_DENIED");
        }
        if (!request.hasArgument()) {
            throw new ImproperUsageException();
        ...
        logger.info("'" + session.getUserNull(request.getUser()).getName() + "' transfered " + Bytes.formatBytes(credits) + " ('" + credits + "') to '" + myUser.getName() + "'");
        myUser.updateCredits(credits);
        return new CommandResponse(200, "OK, gave " + Bytes.formatBytes(credits) + " of your credits to " + myUser.getName());
    }
    ...
    public CommandResponse doSITE_GROUP(CommandRequest request) throws ImproperUsageException {
        ...
        CommandResponse response = new CommandResponse(200);
        Collection users = GlobalContext.getGlobalContext().getUserManager().getAllUsersByGroup(group);
        ;
    ...
    public CommandResponse doSITE_GRPREN(CommandRequest request) throws ImproperUsageException {
        ...
        Collection users = GlobalContext.getGlobalContext().getUserManager().getAllUsersByGroup(oldGroup);
        ;
    ...
    public CommandResponse doSITE_PASSWD(CommandRequest request) throws ImproperUsageException {
        ...
        logger.info("'" + session.getUserNull(request.getUser()).getName() + "' changed his password");
    ...
    public CommandResponse doSITE_PURGE(CommandRequest request) throws ImproperUsageException {
        ...
        User myUser;
        try {
            myUser = GlobalContext.getGlobalContext().getUserManager().getUserByNameUnchecked(delUsername);
        } catch (NoSuchUserException e) {
            return new CommandResponse(452, e.getMessage());
        } catch (UserFileException e) {
            return new CommandResponse(452, "Couldn't getUser: " + e.getMessage());
        ...
        myUser.purge();
        logger.info("'" + session.getUserNull(request.getUser()).getName() + "' purged '" + myUser.getName() + "'");
        return StandardCommandManager.genericResponse("RESPONSE_200_COMMAND_OK");
    }
    ...
    public CommandResponse doSITE_READD(CommandRequest request) throws ImproperUsageException {
        ...
        logger.info("'" + session.getUserNull(request.getUser()).getName() + "' readded '" + myUser.getName() + "'");
        myUser.commit();
        return StandardCommandManager.genericResponse("RESPONSE_200_COMMAND_OK");
    }
    ...
    public CommandResponse doSITE_RENUSER(CommandRequest request) throws ImproperUsageException {
        ...
        try {
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' renamed '" + oldUsername + "' to '" + myUser.getName() + "'");
        } catch (NoSuchUserException e) {
    ...
    public CommandResponse doSITE_SEEN(CommandRequest request) throws ImproperUsageException {
        ...
        return new CommandResponse(200, "User was last seen: " + user.getKeyedMap().getObjectDate(UserManagement.LASTSEEN));
    }
    ...
    public CommandResponse doSITE_TAGLINE(CommandRequest request) throws ImproperUsageException {
        ...
        logger.info("'" + request.getUser() + "' changed his tagline from '" + u.getKeyedMap().getObject(UserManagement.TAGLINE, "") + "' to '" + request.getArgument() + "'");
    ...
    public CommandResponse doSITE_TAKE(CommandRequest request) throws ImproperUsageException {
        Session session = request.getSession();
        if (!GlobalContext.getConfig().checkPermission("take", session.getUserNull(request.getUser()))) {
            return StandardCommandManager.genericResponse("RESPONSE_530_ACCESS_DENIED");
        }
        if (!request.hasArgument()) {
            throw new ImproperUsageException();
        ...
        try {
            ...
            logger.info("'" + session.getUserNull(request.getUser()).getName() + "' took " + Bytes.formatBytes(credits) + " ('" + credits + "') from '" + myUser.getName() + "'");
            myUser.updateCredits(-credits);
        } catch (NumberFormatException ex) {
            return new CommandResponse(452, "The string " + amt + " cannot be interpreted");
        } catch (Exception ex) {
        ...
        return new CommandResponse(200, "OK, removed " + credits + "b from " + myUser.getName() + ".");
    }
    ...
    public CommandResponse doSITE_USER(CommandRequest request) throws ImproperUsageException {
        ...
        response.addComment(request.getSession().jprintf(_bundle, _keyPrefix + "user", null, myUser.getName()));
        return response;
    }
    ...
    public CommandResponse doSITE_USERS(CommandRequest request) {
        ...
        if (request.hasArgument()) {
            Permission perm = new Permission(Permission.makeUsers(new StringTokenizer(request.getArgument())), true);
    ...
    private CommandResponse doListConnections(CommandRequest request, String type, boolean up, boolean down, boolean idle, boolean command, boolean statusUsers, boolean statusSpeed, boolean restrictUser) {
        Session session = request.getSession();
        CommandResponse response = StandardCommandManager.genericResponse("RESPONSE_200_COMMAND_OK");
        long speedup = 0;
        long speeddn = 0;
    ...
    public CommandResponse doSITE_BAN(CommandRequest request) throws ImproperUsageException {
        ...
        if (st.hasMoreTokens()) {
            ...
            while (st.hasMoreTokens()) banMsg += " " + st.nextToken();
        } else {
            banMsg = "Banned by " + session.getUserNull(request.getUser()).getName() + " for " + banTime + "m";
        }
        myUser.getKeyedMap().setObject(UserManagement.BAN_TIME, new Date(System.currentTimeMillis() + (banTime * 60000)));
    ...
    public CommandResponse doSITE_BANS(CommandRequest request) {
        ...
        for (User user : myUsers) {
            long timeleft = user.getKeyedMap().getObjectDate(UserManagement.BAN_TIME).getTime() - System.currentTimeMillis();
            if (timeleft > 0) {
                ReplacerEnvironment env = new ReplacerEnvironment();
                env.add("timeleft", "" + (timeleft / 60000));
                response.addComment(request.getSession().jprintf(_bundle, _keyPrefix + "bans", env, user.getName()));
            }
        }
        return response;
    }
