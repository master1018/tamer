...
public class PersonSetPrimLoc extends PersonAlgorithm implements PlanAlgorithmI {
    ...
    private void buildWorkFacQuadTree() {
        ...
        for (Facility f : this.facilities.getFacilities().values()) {
            if (f.getActivity(WORK) != null) {
                if (f.getCenter().getX() < minx) {
                    minx = f.getCenter().getX();
                }
                if (f.getCenter().getY() < miny) {
                    miny = f.getCenter().getY();
                }
                if (f.getCenter().getX() > maxx) {
                    maxx = f.getCenter().getX();
                }
                if (f.getCenter().getY() > maxy) {
                    maxy = f.getCenter().getY();
                }
            }
        }
        minx -= 1.0;
    ...
    private final Zone getPrimActZone(final ArrayList<Entry> from_loc_entries) {
        if (from_loc_entries.isEmpty()) {
            Gbl.errorMsg("From Loc Entries are empty! This should not happen!");
        }
    ...
    private final Facility getPrimActFacility(final ArrayList<Facility> facs, final String act_type) {
        if (facs.isEmpty()) {
            Gbl.errorMsg("facs are empty! This should not happen!");
        }
    ...
    public void run(final Person person) {
        ...
        if (!work_list.isEmpty()) {
            ...
            if (to_facs == null) {
                System.out.println("      Person id=" + person.getId() + ": no work fac in to_zone id=" + to_zone.getId() + ". Getting a close one...");
            ...
            Facility to_fac = null;
            if (to_facs == null) {
                System.out.println("      Person id=" + person.getId() + ": no educ fac in to_zone id=" + to_zone.getId() + ". Getting a close one...");
