...
public class Watcher {
    ...
    public static String getCallingClassSignature(int level) {
        ...
        if (callingTrace == null) return "No Caller";
    ...
    protected static StackTraceElement getCaller(String pIgnoredClassNames[]) {
        ...
        for (StackTraceElement trace : traces) {
            ...
            boolean found = isTargetTrace(traceClassName, thisClassName, pIgnoredClassNames);
	 ...
	 * @return
	 */
    private static boolean isTargetTrace(String traceClassName, String pWatcherName, String pIgnoredClassNames[]) {
        if (traceClassName.endsWith(pWatcherName) == true) return false;
        if (traceClassName.contains("$") == true) return false;
        if (pIgnoredClassNames == null) return true;
        for (String ignoredClassName : pIgnoredClassNames) {
            if (traceClassName.endsWith(ignoredClassName) == true) return false;
        }
        return true;
    }
    ...
    public static StackTraceElement getTraceAbovePackage(String pPackageName, int count) {
        ...
        for (StackTraceElement trace : traces) {
            String traceClassName = trace.getClassName();
            currentMatch = false;
            if (traceClassName.startsWith(pPackageName)) currentMatch = true;
            if (currentMatch == false && previousMatch == true) {
                matchCounter++;
                if (matchCounter >= count) return trace;
            }
