...
public class JDOQuery {
    ...
    public List getParameterNames() {
        if (param_names == null) return Collections.EMPTY_LIST; else return param_names;
    }
    ...
    public Class getParameterType(String name) {
        if (params == null || !params.containsKey(name)) throw new OntopiaRuntimeException("Parameter with name '" + name + "' does not exist.");
        return (Class) params.get(name);
    }

    public void addParameter(String name, Class klass) {
        if (klass == null) throw new NullPointerException("JDO parameter class must not be null.");
    ...
    public Collection getVariableNames() {
        if (variables == null) return Collections.EMPTY_SET; else return variables.keySet();
    }
    ...
    public Class getVariableType(String name) {
        if (variables == null || !variables.containsKey(name)) throw new OntopiaRuntimeException("JDO variable '" + name + "' does not exist.");
        return (Class) variables.get(name);
    }

    public void addVariable(String name, Class klass) {
        if (klass == null) throw new NullPointerException("The class of JDO variable '" + name + "' must not be null.");
        checkExistingName(name);
    ...
    protected void checkExistingName(String name) throws RuntimeException {
        if (params != null && params.containsKey(name)) throw new OntopiaRuntimeException("Parameter with name '" + name + "' already exists.");
        if (variables != null && variables.containsKey(name)) throw new OntopiaRuntimeException("Variable with name '" + name + "' already exists.");
    }
    ...
    public String[] getSelectedColumnNames() {
        ...
        for (int i = 0; i < colnames.length; i++) {
            ...
            if (selected instanceof JDOVariable) colnames[i] = ((JDOVariable) selected).getName(); else if (selected instanceof JDOAggregate) colnames[i] = ((JDOVariable) ((JDOAggregate) selected).getValue()).getName(); else throw new OntopiaRuntimeException("Not able to figure out column name.");
        }
        return colnames;
    }
