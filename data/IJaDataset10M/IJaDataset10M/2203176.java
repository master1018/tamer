package org.primordion.user.app.climatechange.mdcs.m2_1jscience;
 ...
 * It's roleName must be a specially-formatted combination of its initial point,
 * terminal point, and optional subscript.
 * @author ken
 *
 */
public class Flux extends Quantity {
    ...
    public String getTex() {
        String regex = String.format("%s|%s|%s", "(?<=[A-Z])(?=[A-Z][a-z])", "(?<=[^A-Z])(?=[A-Z])", "(?<=[A-Za-z])(?=[^A-Za-z])");
