...
public class InMemoryDeviationDAO implements DeviationDAO {
    ...
    public Deviation getDeviation(int tenantId, int item1Id, int item1TypeId, int item2Id, int item2TypeId) {
        for (Deviation deviation : deviations) if (deviation.getTenantId() == tenantId && deviation.getItem1TypeId() == item1TypeId && deviation.getItem1Id() == item1Id && deviation.getItem2Id() == item2Id && deviation.getItem2TypeId() == item2TypeId) return deviation;
        return null;
    }

    public List<Deviation> getDeviationsOrdered(int tenantId, int itemTypeId, int itemId, @Nullable final Long minCountConstraint, @Nullable final Integer maxNumberOfDeviationsConstraint) {
        ...
        if (maxNumberOfDeviationsConstraint != null && result.size() > maxNumberOfDeviationsConstraint) result = result.subList(0, maxNumberOfDeviationsConstraint);
    ...
    public Set<TenantItem> getItemIds(final int tenantId, final TIntSet itemTypeIds) {
        ...
        for (Deviation deviation : deviations) {
            if (!itemTypeIds.contains(deviation.getItem1TypeId()) && !itemTypeIds.contains(deviation.getItem2TypeId())) continue;
    ...
    public int insertDeviation(Deviation deviation) {
        Deviation old = getDeviation(deviation.getTenantId(), deviation.getItem1Id(), deviation.getItem1TypeId(), deviation.getItem2Id(), deviation.getItem2TypeId());
        if (old != null) {
