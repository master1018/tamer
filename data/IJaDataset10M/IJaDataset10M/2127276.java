...
class CharArrayState extends State {
    ...
    int getchar() {
        if (offset >= limit) throw new ArrayIndexOutOfBoundsException(offset + " >= " + limit);
        return data[offset];
    }
    ...
    int indexOf(int c, int offset) {
        ...
        while (offset < limit) {
            if (data[offset] == c) return offset;
            offset += 1;
        }
        return -1;
    }
    ...
    public String getGroupString(int n) {
        validateGroup(n);
        return String.copyValueOf(data, pstart[n] - 1, pend[n] - pstart[n]);
    }
}
...
public class Regexp {
    ...
    public Result match(String data, int offset) {
        ...
        if (match(s)) return new Result(s);
        return null;
    }
    ...
    private final Result searchForward(State state) {
        ...
        if (firstc != -1) {
            ...
            while ((i = state.indexOf(firstc, state.offset)) != -1) {
                state.offset = i;
                if (match(state)) return new Result(state);
                state.offset = i + 1;
            }
        } else {
    ...
    public static void main(String args[]) throws Exception {
        ...
        if (fin.read(bytes) != bytes.length) System.out.println("Huh?");
        fin.close();
        ...
        while (args[arg].charAt(0) == '-') {
            if (args[arg].equals("-time")) timeit = true; else if (args[arg].equals("-reverse")) reverse = true; else if (args[arg].equals("-ignore")) mapCase = true; else System.out.println("Unknown option: " + args[arg]);
            arg += 1;
        }
        reg = Regexp.compile(args[arg], mapCase);
        System.out.println("Processing " + args[arg] + " = " + reg.toString());
        arg += 1;
        if (reg.firstCharacter() != -1) System.out.println("First Character: " + new Character((char) reg.firstCharacter()));
        if (timeit) {
            int count = 50;
            ...
            System.out.println((float) ((1000.0 * count) / (System.currentTimeMillis() - start)) + " per second.");
        } else {
    ...
    public final String toString() {
        if (next != null && next != success) return toStringThis() + next.toString();
        return toStringThis();
    }
