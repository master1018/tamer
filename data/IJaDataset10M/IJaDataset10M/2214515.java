...
public class FunReplace extends FunMatches {
    ...
    public static final FunctionSignature signatures[] = { new FunctionSignature(new QName("replace", Function.BUILTIN_FUNCTION_NS), FUNCTION_DESCRIPTION_3_PARAM + FUNCTION_DESCRIPTION_COMMON, new SequenceType[] { INPUT_ARG, PATTERN_ARG, REPLACEMENT_ARG }, RETURN_TYPE), new FunctionSignature(new QName("replace", Function.BUILTIN_FUNCTION_NS), FUNCTION_DESCRIPTION_4_PARAM + FUNCTION_DESCRIPTION_COMMON, new SequenceType[] { INPUT_ARG, PATTERN_ARG, REPLACEMENT_ARG, FLAGS_ARG }, RETURN_TYPE) };
    ...
    public void setArguments(List<Expression> arguments) throws XPathException {
        ...
        arg = new DynamicCardinalityCheck(context, Cardinality.ZERO_OR_ONE, arg, new Error(Error.FUNC_PARAM_CARDINALITY, "1", mySignature));
        if (!Type.subTypeOf(arg.returnsType(), Type.ATOMIC)) arg = new Atomize(context, arg);
        steps.add(arg);
        arg = arguments.get(1);
        ...
        if (arguments.size() == 4) {
            arg = arguments.get(3);
            arg = new DynamicCardinalityCheck(context, Cardinality.EXACTLY_ONE, arg, new Error(Error.FUNC_PARAM_CARDINALITY, "4", mySignature));
            if (!Type.subTypeOf(arg.returnsType(), Type.ATOMIC)) arg = new Atomize(context, arg);
            steps.add(arg);
        }
    }
    ...
    public Sequence eval(Sequence contextSequence, Item contextItem) throws XPathException {
        ...
        if (stringArg.isEmpty()) result = StringValue.EMPTY_STRING; else {
            String string = stringArg.getStringValue();
            Sequence patternSeq = getArgument(1).eval(contextSequence, contextItem);
            String pattern = translateRegexp(patternSeq.getStringValue());
            Sequence replaceSeq = getArgument(2).eval(contextSequence, contextItem);
            String replace = replaceSeq.getStringValue();
            for (int i = 0; i < replace.length(); i++) {
                if (replace.charAt(i) == '\\') {
                    try {
                        if (!(replace.charAt(i + 1) == '\\' || replace.charAt(i + 1) == '$')) throw new XPathException(this, ErrorCodes.FORX0004, "The value of $replacement contains a '\\' character that is not part of a '\\\\' pair, unless it is immediately followed by a '$' character.", replaceSeq);
                        i++;
                    } catch (Exception e) {
            ...
            int flags = 0;
            if (getSignature().getArgumentCount() == 4) flags = parseFlags(getArgument(3).eval(contextSequence, contextItem).getStringValue());
            try {
