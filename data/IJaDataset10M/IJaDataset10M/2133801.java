...
public class GroupsAssignGroupsTest extends AbstractHttpUnitTest {

    public GroupsAssignGroupsTest(String name) {
        super(name);
    }

    public void setUp() throws Exception {
        super.setUp();
        request = new GetMethodWebRequest(TestProperties.getProperties("application.contextURL") + "/content.jsp?tab=group&submenu=group.assign&body=group.assign.group");
        response = conversation.getResponse(request);
    }

    public void testGroupsAssignGroupsPageWithTidy() throws Exception {
        outputFile = new File(TestProperties.getProperties("output.file.html") + "\\groups\\groupsAssignGroups.html");
        fw = new FileWriter(outputFile);
        fw.write(response.getText());
        fw.close();
    }

    public void testGroupsAssignGroupsTables() throws Exception {
        WebTable table[] = { response.getTableWithSummary("rootLayout/parent"), response.getTableWithSummary("rootLayout/inner"), response.getTableWithSummary("rootLayout/border"), response.getTableWithSummary("rootLayout/border_top_left"), response.getTableWithSummary("rootLayout/border_top_right"), response.getTableWithSummary("rootLayout/content"), response.getTableWithSummary("groupsAssignGroups/parent"), response.getTableWithSummary("groupsAssignGroups/search"), response.getTableWithSummary("groupsAssignGroups/assign") };
    ...
    public void testGroupsAssignGroupsForms() throws Exception {
        forms = response.getForms();
        assertEquals("form count", 2, forms.length);
        parameters1 = forms[0].getParameterNames();
        assertEquals("Parameteranzahl falsch", 9, parameters1.length);
        assertEquals("falscher Parameter", "viewTarget", parameters1[0]);
        assertEquals("falscher Parameter", "grNameContainer", parameters1[1]);
        assertEquals("falscher Parameter", "submenu", parameters1[2]);
        assertEquals("falscher Parameter", "grStateContainer", parameters1[3]);
        assertEquals("falscher Parameter", "grName", parameters1[4]);
        assertEquals("falscher Parameter", "tab", parameters1[5]);
        assertEquals("falscher Parameter", "grState", parameters1[6]);
        assertEquals("falscher Parameter", "body", parameters1[7]);
        assertEquals("falscher Parameter", "searchTarget", parameters1[8]);
