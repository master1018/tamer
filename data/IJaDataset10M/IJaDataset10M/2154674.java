...
public class NXJLink {
    ...
    private static String joinCP(String cp1, String cp2) {
        if (cp1.length() > 0) {
            if (cp2.length() > 0) return cp1 + File.pathSeparatorChar + cp2;
            return cp1;
        }
        return cp2;
    }
    ...
    public int run(String[] args) throws TinyVMException, IOException {
        ...
        try {
            start(bootclasspath, classpath, classnames, all, stream, debugFile, bigEndian, debug, options, verbose);
        } finally {
            try {
                stream.close();
            } catch (IOException e) {
    ...
    void start(String bootclasspath, String classpath, String[] classes, boolean all, FileOutputStream stream, String debugFile, boolean bigEndian, int debug, int options, boolean verbose) throws FileNotFoundException, TinyVMException {
        ...
        try {
            ...
            tinyVM.link(joinCP(bootclasspath, classpath), classes, all, stream, bigEndian, options, debug, debugStream);
        } finally {
            if (debugStream != null) {
                try {
                    debugStream.close();
