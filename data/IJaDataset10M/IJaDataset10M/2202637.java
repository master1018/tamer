...
public abstract class GoalFeeder<T> {
   ...
   * Logger definition
   */
    private static transient Log LOGGER = LogFactory.getLog(GoalFeeder.class);
   ...
   */
    public GoalFeeder(IModel model, boolean throwExceptionOnInvalidResponse) {
        ...
        for (IGoalInjector<T> cons : _constructors) if (cons.getResponder() != null) _responders.add(cons.getResponder());
        _model = model;
   ...
   * @return
   */
    protected abstract Collection<IGoalInjector<T>> assembleInjectors();
   ...
   */
    public IGoalInjector<T> getInjector(T experimentTask) {
        for (IGoalInjector<T> constructor : _constructors) if (constructor.handles(experimentTask)) return constructor;
        return null;
    }
   ...
   */
    public IChunk nextGoal(final T experimentTask) {
        ...
        if (injector == null) {
            if (LOGGER.isDebugEnabled()) LOGGER.debug("No constructor found for " + experimentTask);
            return null;
        }
        while (!waitForModel()) {
        ...
        IChunk goalChunk = injector.injectGoal(experimentTask, _model, GoalFeeder.this);
        GatingTimedEvent gte = injector.createGate(ACTRRuntime.getRuntime().getClock(_model).getTime(), experimentTask);
        if (LOGGER.isDebugEnabled()) LOGGER.debug(injector + " returned " + goalChunk + " and " + gte + " for " + experimentTask);
    ...
    protected void insertAndRelease(double maxTimeToElapse) {
        double now = ACTRRuntime.getRuntime().getClock(_model).getTime();
        GatingTimedEvent gte = new GatingTimedEvent(now, now + maxTimeToElapse, this);
        insertAndRelease(gte);
    }

    protected synchronized void insertAndRelease(GatingTimedEvent gte) {
    ...
    protected synchronized void release() {
        if (_currentGate != null) {
            if (LOGGER.isDebugEnabled()) LOGGER.debug("Releasing previous gate " + _currentGate);
            _currentGate.abort();
            _currentGate = null;
        }
    }
   ...
   */
    protected boolean waitForModel() {
        ...
        try {
            if (LOGGER.isDebugEnabled()) LOGGER.debug("Waiting for model to reach gate");
            _currentGate.waitForModel();
            if (LOGGER.isDebugEnabled()) LOGGER.debug("Model reached gate, resuming");
        } catch (InterruptedException e) {
            if (LOGGER.isDebugEnabled()) LOGGER.debug("Interrupted while waiting for model to reach gate, resuming");
        }
   ...
   */
    public void respond(IInstantiation instantiation) {
        for (IGoalResponder responder : _responders) if (responder.handles(instantiation)) {
            if (LOGGER.isDebugEnabled()) LOGGER.debug("Will use " + responder + " to respond to " + instantiation);
            responder.respond(instantiation);
            return;
        }
        String message = "No clue how to respond to " + instantiation + ", no responder claimed responsibility";
        if (Logger.hasLoggers(_model)) Logger.log(_model, Logger.Stream.EXCEPTION, message);
        LOGGER.warn(message);
