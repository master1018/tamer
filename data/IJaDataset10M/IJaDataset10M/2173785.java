...
public class SqlPlan extends JPanel {
    ...
    public void loadSqlPlan(String sqlId, boolean isDetail) {
        ...
        ButtonPlanActionListener buttonListener = new ButtonPlanActionListener(jButtonPlanCursorCache, tabsSQLPlan, database, sqlId, isDetail);
    ...
    private class ButtonPlanActionListener implements ActionListener {
        ...
        public ButtonPlanActionListener(final JButton jButtonPlan, final JTabbedPane tabsSQLPlan, final ASHDatabase database, final String sqlId, final boolean isDetail) {
            super();
        ...
        private void updatePlanTab(String param) {
            if (param.equalsIgnoreCase("Update")) {
                ...
                for (int i = 0; i < tabsSQLPlan.getTabCount(); i++) {
                    if (tabsSQLPlan.getTitleAt(i).equalsIgnoreCase(CURSORCACHE)) {
                ...
                int iTabTmp = 0;
                for (int i = 0; i < tabsSQLPlan.getTabCount(); i++) {
                    if (tabsSQLPlan.getTitleAt(i).equalsIgnoreCase(AWR)) {
        ...
        private void update10g11gEE() {
            ...
            try {
                if (!isDetail) {
                    list = database.getSqlsTemp().getSqlPlanHashValue(sqlId);
                } else {
                    list = database.getSqlsTempDetail().getSqlPlanHashValue(sqlId);
                }
            ...
            while (listIter.hasNext()) {
                ...
                ExplainPlanModel10g2 sqlPlanModel = (ExplainPlanModel10g2) database.getSqlPlanModelByPlanHashValue(planHashValue, sqlId);
                if (sqlPlanModel != null) {
                    ...
                    emptyText.setText("No data for SQL_ID: " + sqlId + " and " + phvTabTitle);
        ...
        private void update9i() {
            ...
            while (listIter.hasNext()) {
                Double planHashValue = listIter.next();
                String phvTabTitle = "PHV: " + planHashValue.longValue();
                ExplainPlanModel9i sqlPlanModel = (ExplainPlanModel9i) database.getSqlPlanModelByPlanHashValue(planHashValue, sqlId);
                if (sqlPlanModel != null) {
                    JScrollPane scrollPane = new JScrollPane(createTreeTable(sqlPlanModel));
                    tabsSQLPlan.addTab(phvTabTitle, scrollPane);
                } else {
                    ...
                    emptyText.setEditable(false);
                    emptyText.setBackground(new Color(255, 250, 237));
                    emptyText.setText("No data for Hash Value: " + sqlId + " and " + phvTabTitle);
                    tabsSQLPlan.addTab(phvTabTitle + " n/a", emptyText);
                }
                i++;
            }
    ...
    protected JXTreeTable createTreeTable(TreeTableModel model) {
        ...
        if (model instanceof ExplainPlanModel10g2) {
            ...
            while (tableColumnExtListIter.hasNext()) {
                TableColumnExt tmpColumn = (TableColumnExt) tableColumnExtListIter.next();
                if (i == 0 || i == 11 || i == 13 || i == 20 || i == 21 || i == 22) {
	 ...
	 * @param isDetail
	 */
    protected void addTabs10g11gEE(JTabbedPane tabsSQLPlan, HashMap<Integer, Double> tabIdPlanHashValue, String sqlId, boolean isDetail) {
        List<Double> list = null;
        try {
            if (!isDetail) {
                list = database.getSqlsTemp().getSqlPlanHashValue(sqlId);
	 ...
	 * @param isDetail
	 */
    protected void addTabs10g11gSE(JTabbedPane tabsSQLPlan, HashMap<Integer, Double> tabIdPlanHashValue, String sqlId, boolean isDetail) {
        List<Double> list = database.getSqlPlanHashValueBySqlId(sqlId);
        int i = 0;
        Iterator<Double> listIter = list.iterator();
        while (listIter.hasNext()) {
	 ...
	 * @param isDetail
	 */
    protected void addTabs9i(JTabbedPane tabsSQLPlan, HashMap<Integer, Double> tabIdPlanHashValue, String sqlId, boolean isDetail) {
        List<Double> list = database.getSqlPlanHashValueBySqlId(sqlId);
        int i = 0;
        Iterator<Double> listIter = list.iterator();
        while (listIter.hasNext()) {
