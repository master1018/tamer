...
public class XmlUtils {
    ...
    public static String marshallSpmlR2Request(RequestType request, boolean encode) throws Exception {
        ...
        if (type.endsWith("Type")) type = type.substring(0, type.length() - 4);
    ...
    public static String marshallSpmlR2Request(RequestType request, String requestType, boolean encode) throws Exception {
        String marshalledRequest = marshal(request, SPMLR2Constants.SPML_NS, requestType);
    ...
    public static RequestType unmarshallSpmlR2Request(String request, boolean decode) throws Exception {
        if (decode) request = decode(request);
    ...
    public static String marshallSpmlR2Response(ResponseType response, boolean encode) throws Exception {
        String type = response.getClass().getSimpleName();
        if (type.endsWith("Type")) type = type.substring(0, type.length() - 4);
        return marshallSpmlR2Response(response, type, encode);
    }

    public static String marshallSpmlR2Response(ResponseType response, String responseType, boolean encode) throws Exception {
        String marshalledResponse = marshalledResponse = XmlUtils.marshal(response, SPMLR2Constants.SPML_NS, responseType);
    ...
    public static ResponseType unmarshallSpmlR2Response(String response, boolean decode) throws Exception {
        if (decode) response = decode(response);
        JAXBElement e = (JAXBElement) unmarshal(response, new String[] { SPMLR2Constants.SPML_PKG });
    ...
    public static String marshal(Object msg, String msgQName, String msgLocalName) throws Exception {
        JAXBContext jaxbContext = JAXBUtils.getJAXBContext(spmlContextPackages, constructionType, spmlContextPackages.toString(), XmlUtils.class.getClassLoader(), new HashMap<String, Object>());
    ...
    public static Object unmarshal(Node domMsg, String userPackages[]) throws Exception {
        ...
        JAXBContext jaxbContext = JAXBUtils.getJAXBContext(contextPackages, constructionType, contextPackages.toString(), XmlUtils.class.getClassLoader(), new HashMap<String, Object>());
