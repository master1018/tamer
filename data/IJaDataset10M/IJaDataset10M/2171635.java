...
class NewProjectDialog extends JDialog {
     ...
     */
    public NewProjectDialog(ProjectProperties projectProperties, JFrame par, String projFileName, boolean foldersMissing) {
        super(par, true);
        this.projectProperties = projectProperties;
        this.foldersMissing = foldersMissing;
        projectProperties.m_dialogOK = false;
        if (projFileName == null) projectProperties.reset();
        ...
        if (m_sourceLocaleField.getMaximumRowCount() < 20) m_sourceLocaleField.setMaximumRowCount(20);
        ...
        if (m_targetLocaleField.getMaximumRowCount() < 20) m_targetLocaleField.setMaximumRowCount(20);
        ...
        if (projFileName == null) {
            ...
            projectProperties.setProjectRoot(ndc.getSelectedFile().getAbsolutePath() + File.separator);
            projectProperties.setProjectFile(projectProperties.getProjectRoot() + OConsts.PROJ_FILENAME);
            PreferenceManager.pref.setPreference(OConsts.PREF_CUR_DIR, ndc.getSelectedFile().getParent());
            projectProperties.setProjectName(projectProperties.getProjectFile().substring(projectProperties.getProjectRoot().length()));
            projectProperties.setSourceRoot(projectProperties.getProjectRoot() + OConsts.DEFAULT_SRC + File.separator);
            projectProperties.setLocRoot(projectProperties.getProjectRoot() + OConsts.DEFAULT_LOC + File.separator);
            projectProperties.setGlossaryRoot(projectProperties.getProjectRoot() + OConsts.DEFAULT_GLOS + File.separator);
            projectProperties.setTMRoot(projectProperties.getProjectRoot() + OConsts.DEFAULT_TM + File.separator);
        } else {
            projectProperties.setProjectFile(projFileName);
            projectProperties.setProjectRoot(projectProperties.getProjectFile().substring(0, projectProperties.getProjectFile().lastIndexOf(File.separator) + 1));
        }
        projectProperties.setProjectInternal(projectProperties.getProjectRoot() + OConsts.DEFAULT_INTERNAL + File.separator);
        ...
        if (foldersMissing) {
            ...
            if (!f.exists() || !f.isDirectory()) m_srcRootField.setForeground(Color.RED);
            f = new File(m_locRootField.getText());
            if (!f.exists() || !f.isDirectory()) m_locRootField.setForeground(Color.RED);
            f = new File(m_glosRootField.getText());
            if (!f.exists() || !f.isDirectory()) m_glosRootField.setForeground(Color.RED);
            f = new File(m_tmRootField.getText());
            if (!f.exists() || !f.isDirectory()) m_tmRootField.setForeground(Color.RED);
        }
        updateUIText();
        pack();
    ...
    private void doBrowseDirectoy() {
        ...
        if (curDir.equals("")) curDir = PreferenceManager.pref.getPreference(OConsts.PREF_CUR_DIR);
        if (!curDir.equals("")) {
            File dir = new File(curDir);
            if (dir.exists() && dir.isDirectory()) {
        ...
        switch(m_browseTarget) {
            case 1:
                PreferenceManager.pref.setPreference(OConsts.PREF_SRC_DIR, browser.getSelectedFile().getParent());
                projectProperties.setSourceRoot(str);
                m_srcRootField.setText(projectProperties.getSourceRoot());
                if (new File(projectProperties.getSourceRoot()).exists() && new File(projectProperties.getSourceRoot()).isDirectory()) m_srcRootField.setForeground(java.awt.SystemColor.textText);
                break;
            case 2:
                PreferenceManager.pref.setPreference(OConsts.PREF_LOC_DIR, browser.getSelectedFile().getParent());
                projectProperties.setLocRoot(str);
                m_locRootField.setText(projectProperties.getLocRoot());
                if (new File(projectProperties.getLocRoot()).exists() && new File(projectProperties.getLocRoot()).isDirectory()) m_locRootField.setForeground(java.awt.SystemColor.textText);
                break;
            case 3:
                PreferenceManager.pref.setPreference(OConsts.PREF_GLOS_DIR, browser.getSelectedFile().getParent());
                projectProperties.setGlossaryRoot(str);
                m_glosRootField.setText(projectProperties.getGlossaryRoot());
                if (new File(projectProperties.getGlossaryRoot()).exists() && new File(projectProperties.getGlossaryRoot()).isDirectory()) m_glosRootField.setForeground(java.awt.SystemColor.textText);
                break;
            case 4:
                PreferenceManager.pref.setPreference(OConsts.PREF_TM_DIR, browser.getSelectedFile().getParent());
                projectProperties.setTMRoot(str);
                m_tmRootField.setText(projectProperties.getTMRoot());
                if (new File(projectProperties.getTMRoot()).exists() && new File(projectProperties.getTMRoot()).isDirectory()) m_tmRootField.setForeground(java.awt.SystemColor.textText);
                break;
        }
    }
    ...
    private void doOK() {
        if (!projectProperties.verifySingleLangCode(m_sourceLocaleField.getSelectedItem().toString())) {
            JOptionPane.showMessageDialog(this, OStrings.getString("NP_INVALID_SOURCE_LOCALE") + OStrings.getString("NP_LOCALE_SUGGESTION"), OStrings.TF_ERROR, JOptionPane.ERROR_MESSAGE);
        ...
        if (!projectProperties.verifySingleLangCode(m_targetLocaleField.getSelectedItem().toString())) {
            JOptionPane.showMessageDialog(this, OStrings.getString("NP_INVALID_TARGET_LOCALE") + OStrings.getString("NP_LOCALE_SUGGESTION"), OStrings.TF_ERROR, JOptionPane.ERROR_MESSAGE);
            m_targetLocaleField.requestFocusInWindow();
            return;
        }
        projectProperties.setSourceRoot(m_srcRootField.getText());
        if (!projectProperties.getSourceRoot().endsWith(File.separator)) projectProperties.setSourceRoot(projectProperties.getSourceRoot() + File.separator);
        if (foldersMissing && !new File(projectProperties.getSourceRoot()).exists()) {
            JOptionPane.showMessageDialog(this, OStrings.getString("NP_SOURCEDIR_DOESNT_EXIST"), OStrings.TF_ERROR, JOptionPane.ERROR_MESSAGE);
            m_srcRootField.requestFocusInWindow();
            return;
        }
        projectProperties.setLocRoot(m_locRootField.getText());
        if (!projectProperties.getLocRoot().endsWith(File.separator)) projectProperties.setLocRoot(projectProperties.getLocRoot() + File.separator);
        if (foldersMissing && !new File(projectProperties.getLocRoot()).exists()) {
            JOptionPane.showMessageDialog(this, OStrings.getString("NP_TRANSDIR_DOESNT_EXIST"), OStrings.TF_ERROR, JOptionPane.ERROR_MESSAGE);
            m_locRootField.requestFocusInWindow();
            return;
        }
        projectProperties.setGlossaryRoot(m_glosRootField.getText());
        if (!projectProperties.getGlossaryRoot().endsWith(File.separator)) projectProperties.setGlossaryRoot(projectProperties.getGlossaryRoot() + File.separator);
        if (foldersMissing && !new File(projectProperties.getGlossaryRoot()).exists()) {
            JOptionPane.showMessageDialog(this, OStrings.getString("NP_GLOSSDIR_DOESNT_EXIST"), OStrings.TF_ERROR, JOptionPane.ERROR_MESSAGE);
            m_glosRootField.requestFocusInWindow();
            return;
        }
        projectProperties.setTMRoot(m_tmRootField.getText());
        if (!projectProperties.getTMRoot().endsWith(File.separator)) projectProperties.setTMRoot(projectProperties.getTMRoot() + File.separator);
        if (foldersMissing && !new File(projectProperties.getTMRoot()).exists()) {
            JOptionPane.showMessageDialog(this, OStrings.getString("NP_TMDIR_DOESNT_EXIST"), OStrings.TF_ERROR, JOptionPane.ERROR_MESSAGE);
