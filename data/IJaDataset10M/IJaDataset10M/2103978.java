...
public class Domain {
    ...
    public Domain(CgsuiteObject contextObject, CgsuiteMethod contextMethod, List<CgsuitePackage> packageImports, Map<String, CgsuiteClass> classImports) {
        ...
        if (contextObject != null) this.namespace.put("this", contextObject);
    }
    ...
    public void put(String str, CgsuiteObject object) {
        CgsuiteClass type = CgsuitePackage.lookupClass(str, packageImports, classImports);
        if (type != null) throw new InputException("Cannot use class name as variable name: " + str);
    ...
    public CgsuiteObject script(CgsuiteTree tree) throws CgsuiteException {
        try {
            if (tree.getType() != EOF) throw new MalformedParseTreeException(tree);
            mode = Mode.NORMAL;
    ...
    private CgsuiteObject statement(CgsuiteTree tree) throws CgsuiteException {
        ...
        switch(tree.token.getType()) {
            ...
            case CONTINUE:
                ...
                return null;
            case RETURN:
                returnValue = (tree.getChildCount() == 0 ? CgsuiteObject.NIL : expression(tree.getChild(0)));
                mode = Mode.RETURNING;
                controlToken = tree.getToken();
                return null;
            case CLEAR:
    ...
    private CgsuiteObject expression2(CgsuiteTree tree) throws CgsuiteException {
        ...
        switch(tree.token.getType()) {
            ...
            case ELSEIF:
                ...
                if (cond) return statementSequence(tree.getChild(1)); else if (tree.getChildCount() > 2) return expression(tree.getChild(2)); else return CgsuiteObject.NIL;
            case ELSE:
            ...
            case ASSIGN:
                ...
                return x;
            case OR:
                if (bool(expression(tree.getChild(0)), tree)) return CgsuiteBoolean.TRUE; else return cgsuiteBool(expression(tree.getChild(1)), tree);
            case AND:
                if (!bool(expression(tree.getChild(0)), tree)) return CgsuiteBoolean.FALSE; else return cgsuiteBool(expression(tree.getChild(1)), tree);
            case NOT:
            ...
            case IS:
                ...
                if (y.getCgsuiteClass() != CgsuiteClass.TYPE) throw new InputException(tree.getToken(), "Right-hand argument must be a class.");
            ...
            case PLUSMINUS:
                lo = new CgsuiteSet();
                for (CgsuiteTree child : tree.getChildren()) lo.add(expression(child));
                ro = new CgsuiteSet();
                try {
                    for (CgsuiteObject obj : lo) ro.add(obj.invokeMethod("op neg"));
                } catch (InputException exc) {
                    exc.addToken(tree.getToken());
                    throw exc;
                }
            ...
            case UNARY_AST:
                if (tree.getChildCount() == 0) n = 1; else n = naturalNumber(expression(tree.getChild(0)), tree, "Argument to *");
            ...
            case INTEGER:
                return CgsuiteInteger.parseInteger(tree.getText());
            case INF:
                return RationalNumber.POSITIVE_INFINITY;
            case STRING:
                return new CgsuiteString(tree.getText().substring(1, tree.getText().length() - 1));
            case IDENTIFIER:
                id = tree.getText();
                try {
                    x = lookup(id);
            ...
            case THIS:
                ...
                if (x == null) throw new InputException(tree.getToken(), "Reference to \"" + tree.getText() + "\" is not permitted in a static context.");
                return x;
            case STATEMENT_SEQUENCE:
                return statementSequence(tree);
            case SQUOTE:
            ...
            case EXPLICIT_MAP:
                ...
                for (CgsuiteTree child : tree.getChildren()) map.put(expression(child.getChild(0)).simplify().createCrosslink(), expression(child.getChild(1)).simplify().createCrosslink());
                return map;
            case EXPLICIT_SET:
                CgsuiteSet set = new CgsuiteSet();
                for (CgsuiteTree child : tree.getChildren()) set.add(expression(child).simplify().createCrosslink());
                return set;
            case EXPLICIT_LIST:
                CgsuiteList array = new CgsuiteList();
                for (CgsuiteTree child : tree.getChildren()) array.add(expression(child).simplify().createCrosslink());
                return array;
            case ERROR:
    ...
    public static CgsuitePackage findPackage(CgsuiteTree tree) throws CgsuiteException {
        if (tree.checkedPackage()) return tree.getPackageValue();
    ...
    public static String stringifyDotSequence(CgsuiteTree tree) throws CgsuiteException {
        switch(tree.getType()) {
            ...
            case DOT:
                ...
                if (parent == null) return null; else return parent + "." + tree.getChild(1).getText();
            default:
                return null;
        }
    }
    ...
    private List<String> procedureParameters(CgsuiteTree tree) throws CgsuiteException {
        switch(tree.token.getType()) {
            case PROCEDURE_PARAMETER_LIST:
                ...
                for (int i = 0; i < tree.getChildCount(); i++) parameters.add(tree.getChild(i).getText());
                return parameters;
            default:
                throw new MalformedParseTreeException(tree);
        }
    ...
    private CgsuiteObject loop(CgsuiteTree tree) throws CgsuiteException {
        ...
        for (CgsuiteTree child : tree.getChildren()) {
            switch(child.getToken().getType()) {
                ...
                case BY:
                    ...
                    if (leq(byG, CgsuiteInteger.ZERO, child)) forward = false;
                    break;
                case WHILE:
        ...
        while (true) {
            if (g != null) {
                ...
                if (toG != null) {
                    boolean ok;
                    if (forward) ok = leq(g, toG, tree); else ok = leq(toG, g, tree);
                    if (!ok) break;
                }
            }
            if (it != null) {
            ...
            if (whereCondition == null || bool(expression(whereCondition), whereCondition)) {
                ...
                if (mode == Mode.RETURNING) {
                    ...
                    if (target != null) target.add(retval);
                    if (sumTarget) {
                        try {
        ...
        if (target != null) return target; else if (sumTarget) return runningSum; else return retval;
    }
    ...
    private CgsuiteObject binopExpression(CgsuiteTree tree) throws CgsuiteException {
        ...
        try {
            switch(tree.token.getType()) {
                ...
                case LEQ:
                    return CgsuiteBoolean.valueOf(leq(x, y, tree));
                case GEQ:
                    return x.invokeMethod("op >=", y);
                case LT:
                    return x.invokeMethod("op <", y);
    ...
    public CgsuiteObject eq(CgsuiteObject x, CgsuiteObject y, CgsuiteTree tree) throws CgsuiteException {
        if (x instanceof CgsuiteInteger && y instanceof CgsuiteInteger || x instanceof RationalNumber && y instanceof RationalNumber || x instanceof CanonicalShortGame && y instanceof CanonicalShortGame) {
            return CgsuiteBoolean.valueOf(x.equals(y));
        }
        return x.invokeMethod("op ==", y);
    }
    ...
    private CgsuiteObject upExpression(CgsuiteTree tree) throws CgsuiteException {
        ...
        for (CgsuiteTree child : tree.getChildren()) {
            if (child.getToken().getType() == UNARY_AST) starChild = child; else nonStarChild = child;
        }
        ...
        if (starChild == null) m = 0; else if (starChild.getChildCount() == 0) m = 1; else m = naturalNumber(expression(starChild.getChild(0)), starChild, "Argument to *");
                ...
                for (CgsuiteTree child : tree.getChildren()) {
                    ...
                    if (obj instanceof Game) {
                        ...
                        for (CgsuiteObject subObj : (CgsuiteCollection) obj) {
                            ...
                            if (!(subObj instanceof Game)) throw new InputException(child.getToken(), "All specified options must be games or collections of games."); else options.add(subObj);
                        }
                    } else {
    ...
    private boolean hasPass(CgsuiteTree tree) {
        for (CgsuiteTree child : tree.getChildren()) {
            if (child.getType() == PASS) return true;
        }
        return false;
    }
    ...
    private LoopyGame.Node loopyNode(CgsuiteTree tree, Map<String, LoopyGame.Node> nodeMap) throws CgsuiteException {
        ...
        switch(tree.getToken().getType()) {
            case NODE_LABEL:
                ...
                if (nodeMap.containsKey(nodeName)) throw new InputException(tree.getChild(0).getToken(), "Duplicate node label: " + nodeName);
    ...
    private void loopyNode2(CgsuiteTree tree, Map<String, LoopyGame.Node> nodeMap, LoopyGame.Node curNode) throws CgsuiteException {
        if (tree.getToken().getType() != SLASHES) throw new MalformedParseTreeException(tree);
        if (tree.getChild(0).getType() == SLASHES) {
            curNode.addLeftEdge(loopyNode(tree.getChild(0), nodeMap));
        } else {
            if (tree.getChild(0).getType() != EXPRESSION_LIST) throw new MalformedParseTreeException(tree);
                ...
                switch(tree.getChild(0).getChild(i).getType()) {
                    ...
                    default:
                        ...
                        if (obj instanceof CgsuiteCollection) {
                            for (CgsuiteObject x : (CgsuiteCollection) obj) addLeftEdge(tree.getChild(0).getChild(i), curNode, x);
                        } else {
        ...
        if (tree.getChild(1).getType() == SLASHES) {
            ...
            if (tree.getChild(1).getType() != EXPRESSION_LIST) throw new MalformedParseTreeException(tree);
            for (int i = 0; i < tree.getChild(1).getChildCount(); i++) {
                switch(tree.getChild(1).getChild(i).getType()) {
                    ...
                    default:
                        ...
                        if (obj instanceof CgsuiteCollection) {
                            for (CgsuiteObject x : (CgsuiteCollection) obj) addRightEdge(tree.getChild(1).getChild(i), curNode, x);
                        } else {
    ...
    private void addLeftEdge(CgsuiteTree tree, LoopyGame.Node node, CgsuiteObject obj) {
        if (obj instanceof CgsuiteInteger) node.addLeftEdge(CanonicalShortGame.construct((CgsuiteInteger) obj)); else if (obj instanceof RationalNumber) node.addLeftEdge(CanonicalShortGame.construct((RationalNumber) obj)); else if (obj instanceof CanonicalShortGame) node.addLeftEdge((CanonicalShortGame) obj); else if (obj instanceof LoopyGame) node.addLeftEdge((LoopyGame) obj); else throw new InputException(tree.getToken(), "Object of type " + obj.getCgsuiteClass().getQualifiedName() + " not permitted in a loopy game constructor.");
    }

    private void addRightEdge(CgsuiteTree tree, LoopyGame.Node node, CgsuiteObject obj) {
        if (obj instanceof CgsuiteInteger) node.addRightEdge(CanonicalShortGame.construct((CgsuiteInteger) obj)); else if (obj instanceof RationalNumber) node.addRightEdge(CanonicalShortGame.construct((RationalNumber) obj)); else if (obj instanceof CanonicalShortGame) node.addRightEdge((CanonicalShortGame) obj); else if (obj instanceof LoopyGame) node.addRightEdge((LoopyGame) obj); else throw new InputException(tree.getToken(), "Object of type " + obj.getCgsuiteClass().getQualifiedName() + " not permitted in a loopy game constructor.");
    }
            ...
            case FUNCTION_CALL_ARGUMENT_LIST:
                ...
                for (CgsuiteTree child : tree.getChildren()) {
                    if (child.getToken().getType() == BIGRARROW) {
                        ...
                        throw new InputException(child.getToken(), "Ordinary arguments must precede optional arguments in function call.");
                    } else {
                        list.add(expression(child).simplify());
                    }
                }
    ...
    private Map<String, CgsuiteObject> optionalArgumentMap(CgsuiteTree tree) throws CgsuiteException {
        ...
        switch(tree.getToken().getType()) {
            case FUNCTION_CALL_ARGUMENT_LIST:
                for (CgsuiteTree child : tree.getChildren()) {
                    if (child.getToken().getType() == BIGRARROW) {
                        if (map == null) map = new HashMap<String, CgsuiteObject>();
    ...
    private Callable invocationTarget(CgsuiteObject x, CgsuiteTree tree) throws CgsuiteException {
        if (x instanceof CgsuiteClass) {
            ...
            if (type.getScript() != null) return new Script(type);
            CgsuiteMethod ctor = type.lookupConstructor();
            if (ctor == null) throw new InputException(tree.token, "No constructor available: " + type.getQualifiedName()); else return ctor;
    ...
    public static CgsuiteBoolean cgsuiteBool(CgsuiteObject x, CgsuiteTree tree) throws CgsuiteException {
        if (!(x instanceof CgsuiteBoolean)) {
            x = x.simplify();
            if (!(x instanceof CgsuiteBoolean)) throw new InputException(tree.token, "Expression is not a boolean.");
        }
    ...
    private int naturalNumber(CgsuiteObject x, CgsuiteTree tree, String errorMessageAntecedent) throws CgsuiteException {
        ...
        if (intValue < 0) throw new InputException(tree.token, errorMessageAntecedent + " is not a natural number.");
        return intValue;
    }
    ...
    private int integer(CgsuiteObject x, CgsuiteTree tree, String errorMessageAntecedent) throws CgsuiteException {
        if (!(x instanceof CgsuiteInteger)) {
            x = x.simplify();
            if (!(x instanceof CgsuiteInteger)) throw new InputException(tree.token, errorMessageAntecedent + " is not an integer.");
        }
    ...
    private RationalNumber number(CgsuiteObject x, CgsuiteTree tree, String errorMessageAntecedent) throws CgsuiteException {
        if (x instanceof CgsuiteInteger) {
            ...
            x = x.simplify();
            if (!(x instanceof RationalNumber)) throw new InputException(tree.token, errorMessageAntecedent + " is not a number.");
        }
    ...
    private CanonicalShortGame canonicalGame(CgsuiteObject x, CgsuiteTree tree) throws CgsuiteException {
        if (!(x instanceof CanonicalShortGame)) {
            x = x.simplify();
            if (x instanceof CgsuiteInteger) x = CanonicalShortGame.construct((CgsuiteInteger) x); else if (x instanceof RationalNumber) x = CanonicalShortGame.construct((RationalNumber) x);
            if (!(x instanceof CanonicalShortGame)) throw new InputException(tree.token, "Not a canonical game.");
        }
    ...
    private class Script implements Callable {
        ...
        public CgsuiteObject invoke(List<? extends CgsuiteObject> arguments, Map<String, CgsuiteObject> optionalArguments) throws CgsuiteException {
            if (!arguments.isEmpty() || !optionalArguments.isEmpty()) throw new InputException(type.getQualifiedName() + " is a script, and must be called without arguments.");
    ...
    private enum Mode {
        ...
        NORMAL, BREAKING, CONTINUING, RETURNING
    }
