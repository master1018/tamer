...
public class SQLTypeOperation {
    ...
    private static SQLTypeOperation getSQLTypeOperation(String s, boolean bCursor) {
        if (s.equalsIgnoreCase("SELECT")) {
            if (bCursor) return SQLTypeOperation.CursorSelect; else return SQLTypeOperation.Select;
        } else if (s.equalsIgnoreCase("DELETE")) return SQLTypeOperation.Delete; else if (s.equalsIgnoreCase("UPDATE")) return SQLTypeOperation.Update; else if (s.equalsIgnoreCase("INSERT")) return SQLTypeOperation.Insert; else if (s.equalsIgnoreCase("COMMIT")) return SQLTypeOperation.Commit; else if (s.equalsIgnoreCase("ROLLBACK")) return SQLTypeOperation.Rollback; else if (s.equalsIgnoreCase("CREATE")) return SQLTypeOperation.Create; else if (s.equalsIgnoreCase("DROP")) return SQLTypeOperation.Drop; else if (s.equalsIgnoreCase("LOCK")) return SQLTypeOperation.Lock; else if (s.equalsIgnoreCase("DECLARE")) return SQLTypeOperation.Declare; else if (s.equalsIgnoreCase("_SELECT")) {
            if (bCursor) return SQLTypeOperation.CursorSelect; else return SQLTypeOperation.Select;
        }
        return null;
    }
    ...
    private static String addLeadingTablePrefix(String begining, String env, String csForcedReplacedPrefix, String querry) {
        int nPos = -1;
        int i = 0;
        do {
    ...
    public static String addEnvironmentPrefix(String env, String csQuery, SQLTypeOperation typeOperation, String csForcedReplacedPrefix) {
        ...
        if (csQueryUpper.indexOf(" JOIN ") == -1) return addEnvironmentPrefixStandardParser(env, csQuery, csQueryUpper, typeOperation, csForcedReplacedPrefix, false);
        int nStart = 0;
    ...
    private static String setPrefixIfRequired(String env, String csRight, String csForcedReplacedPrefix) {
        if (csRight.indexOf('.') == -1) return env + "." + csRight;
        if (!StringUtil.isEmpty(csForcedReplacedPrefix)) {
            if (StringUtil.startsWithNoCase(csRight, csForcedReplacedPrefix)) {
