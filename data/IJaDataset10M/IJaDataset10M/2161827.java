...
public class JavaEnumElementBuilder extends CompoundDataBuilder<UnresolvedFieldInfo> {
    ...
    public JavaEnumElementBuilder(BuildDataManager buildDataManager, ExpressionElementManager expressionManager) {
    ...
    public void stateChanged(StateChangeEvent<AstVisitEvent> event) {
        ...
        if (isActive()) {
            if (type.equals(JavaEnumElementStateManager.ENUM_ELEMENT_STATE.ENTER_ENUM_ELEMENT)) {
                ...
                buildAnonymousClass(trigger.getStartLine(), trigger.getStartColumn(), trigger.getEndLine(), trigger.getEndColumn());
            ...
            } else if (type.equals(JavaEnumElementStateManager.ENUM_ELEMENT_STATE.ENTER_ENUM_ARGUMENT)) {
            } else if (type.equals(JavaEnumElementStateManager.ENUM_ELEMENT_STATE.EXIT_ENUM_ARGUMENT)) {
    ...
    protected void buildAnonymousClass(int startLine, int startColumn, int endLine, int endColumn) {
        if (null != buildManager && !enumClassStack.isEmpty()) {
            ...
            final FileInfo currentFile = DataManager.getInstance().getFileInfoManager().getCurrentFile(Thread.currentThread());
            ...
            UnresolvedClassTypeInfo superClassReference = new UnresolvedClassTypeInfo(UnresolvedClassImportStatementInfo.getClassImportStatements(buildManager.getAllAvaliableNames()), enumClass.getFullQualifiedName());
    ...
    protected void buildEnumElement(int startLine, int startColumn, int endLine, int endColumn) {
        ...
        if (null != name && name.length > 0 && !enumClassStack.isEmpty() && null != buildManager) {
            ...
            UnresolvedClassInfo enumClass = enumClassStack.peek();
            UnresolvedClassConstructorCallInfo _initializer = new UnresolvedClassConstructorCallInfo(enumClass.getClassType());
            ...
            UnresolvedFieldInfo element = new UnresolvedFieldInfo(elementName, UnresolvedClassTypeInfo.getInstance(enumClass), enumClass, _initializer, startLine, startColumn, endLine, endColumn);
    ...
    /**
     * �񋓎q�Ɉ�����Ă���ꍇexpressionElementList�Ɋi�[����
     * ����ExpressionManager��G���Ă���̂ł���܂�X�}�[�g�łȂ�??
     */
