...
public class DataService {
    ...
    public static boolean canTrade(String pokemon) {
        for (int i = 0; i < m_nonTrades.size(); i++) {
            if (m_nonTrades.get(i).equalsIgnoreCase(pokemon)) return false;
        }
        return true;
    }
    ...
    private void initialisePokemon(PokemonSpecies species, Section s) {
        if (species != null) {
            ...
            if (species.getType2() == null) species.setType2("");
            PokemonType[] types;
            ...
            for (int j = 0; j < stringBaseStats.length; j++) species.getBaseStats()[j] = Integer.parseInt(stringBaseStats[j]);
            ...
            if (species.getHabitat() == null) species.setHabitat("");
            String[] stringEffortPoints = s.get("EffortPoints").split(",");
            for (int j = 0; j < stringEffortPoints.length; j++) species.getEffortPoints()[j] = Integer.parseInt(stringEffortPoints[j]);
            ...
            for (int j = 0; j < stringCompatibility.length; j++) species.getCompatibility()[j] = Integer.parseInt(stringCompatibility[j]);
            ...
            for (int j = 0; j < stringEvolutions.length; j = j + 3) {
                ...
                if (stringEvolutions[j] != null && !stringEvolutions[j].equalsIgnoreCase("")) {
                    ...
                    if (evo.getType() == EvolutionTypes.Level) evo.setLevel(Integer.parseInt(stringEvolutions[j + 2])); else if (evo.getType() != EvolutionTypes.Happiness && evo.getType() != EvolutionTypes.HappinessDay && evo.getType() != EvolutionTypes.HappinessNight && evo.getType() != EvolutionTypes.Trade) evo.setAttribute(stringEvolutions[j + 2]);
                    if (ep < evos.length) {
                        evos[ep] = evo;
                    }
                }
