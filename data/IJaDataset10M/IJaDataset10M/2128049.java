...
public class PushUploadTest extends LimeTestCase {
    ...
    public void testDownloadHTTP10() throws Exception {
        ...
        HttpRequest request = new BasicHttpRequest("GET", url, HttpVersion.HTTP_1_0);
    ...
    public void testDownloadHeadHTTP11() throws Exception {
        establishPushConnection();
        HttpRequest request = new BasicHttpRequest("HEAD", url, HttpVersion.HTTP_1_1);
        HttpResponse response = sendRequest(request);
        assertEquals(200, response.getStatusLine().getStatusCode());
        assertFalse(socket.isClosed());
    ...
    public void testDuplicatePushes() throws Exception {
        ...
        try {
            connection.initialize(injector.getInstance(HeadersFactory.class).createUltrapeerHeaders(null), new EmptyResponder(), 1000);
            ...
            if (reply == null) throw new IOException("didn't get query reply in time");
            PushRequest push = new PushRequestImpl(GUID.makeGuid(), (byte) 3, reply.getClientGUID(), 0, new byte[] { (byte) 127, (byte) 0, (byte) 0, (byte) 1 }, PUSH_PORT);
     ...
     */
    private Socket getSocketFromPush() throws IOException, BadPacketException {
        BlockingConnection connection = injector.getInstance(BlockingConnectionFactory.class).createConnection("localhost", PORT);
        try {
            connection.initialize(injector.getInstance(HeadersFactory.class).createUltrapeerHeaders(null), new EmptyResponder(), 1000);
            QueryRequest query = injector.getInstance(QueryRequestFactory.class).createQuery("txt", (byte) 3);
            ...
            PushRequest push = new PushRequestImpl(GUID.makeGuid(), (byte) 3, guid, 0, new byte[] { (byte) 127, (byte) 0, (byte) 0, (byte) 1 }, PUSH_PORT);
            connection.send(push);
            connection.flush();
            try {
                serverSocket.setSoTimeout(1000);
     ...
     */
    private void establishPushConnection() throws BadPacketException, IOException {
        closeConnection();
        socket = getSocketFromPush();
        in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
        out = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));
        assertEquals("Expected 'GIV', got null", "GIV 0:" + new GUID(guid).toString() + "/file", in.readLine());
    ...
    private void writeRequest(BufferedWriter out, HttpRequest request) throws IOException {
    ...
    private HttpResponse readResponse(BufferedReader in) throws IOException, ProtocolException, UnsupportedEncodingException {
    ...
    private String readBody(HttpResponse response) throws IOException {
        ...
        while (contentLength == -1 || body.length() < contentLength) {
            int c = in.read();
            if (c == -1) {
                fail("Unexpected end of stream while reading body (read " + body.length() + ", expected " + contentLength + "): " + body.toString());
            }
            body.append((char) c);
        }
    ...
    private static class MyNetworkManager extends NetworkManagerImpl {
        ...
        @Inject
        public MyNetworkManager(Provider<UDPService> udpService, Provider<Acceptor> acceptor, Provider<DHTManager> dhtManager, Provider<ConnectionManager> connectionManager, Provider<ActivityCallback> activityCallback, OutOfBandStatistics outOfBandStatistics, NetworkInstanceUtils networkInstanceUtils) {
            super(udpService, acceptor, dhtManager, connectionManager, activityCallback, outOfBandStatistics, networkInstanceUtils);
        }
    ...
    private static class MyConnectionManager extends ConnectionManagerImpl {
        ...
        @Inject
        public MyConnectionManager(NetworkManager networkManager, Provider<HostCatcher> hostCatcher, @Named("global") Provider<ConnectionDispatcher> connectionDispatcher, @Named("backgroundExecutor") ScheduledExecutorService backgroundExecutor, Provider<SimppManager> simppManager, CapabilitiesVMFactory capabilitiesVMFactory, RoutedConnectionFactory managedConnectionFactory, Provider<MessageRouter> messageRouter, Provider<QueryUnicaster> queryUnicaster, SocketsManager socketsManager, ConnectionServices connectionServices, Provider<NodeAssigner> nodeAssigner, Provider<IPFilter> ipFilter, ConnectionCheckerManager connectionCheckerManager, PingRequestFactory pingRequestFactory, NetworkInstanceUtils networkInstanceUtils) {
            super(networkManager, hostCatcher, connectionDispatcher, backgroundExecutor, simppManager, capabilitiesVMFactory, managedConnectionFactory, messageRouter, queryUnicaster, socketsManager, connectionServices, nodeAssigner, ipFilter, connectionCheckerManager, pingRequestFactory, networkInstanceUtils);
        }
