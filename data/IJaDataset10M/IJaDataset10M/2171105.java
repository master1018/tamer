...
public class HTMLViewerPlusPlus extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        settings = getSharedPreferences("com.william.codepad.settings", MODE_PRIVATE);
        ...
        mSlidingDrawer.setOnDrawerOpenListener(new SlidingDrawer.OnDrawerOpenListener() {
            ...
            public void onDrawerOpened() {
                HTMLViewerPlusPlus.this.setTitle("CodePad - Project Home");
        ...
        drawerContent.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                int selectionRowID = (int) parent.getItemIdAtPosition(position);
                ...
                if (selectedFileString.equals("..")) {
                    ...
                    switch(helper.displayMode) {
                        case RELATIVE:
                            clickedFile = new File(helper.currentDirectory.getAbsolutePath() + helper.directoryEntries.get(selectionRowID));
                            break;
                        case ABSOLUTE:
                            clickedFile = new File(helper.directoryEntries.get(selectionRowID));
                            break;
                    }
                    if (clickedFile != null) {
    ...
    void loadFile(Uri uri, String mimeType) {
        ...
        try {
            bf = new BufferedReader(new InputStreamReader(new FileInputStream(f)), 100);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (!f.exists()) {
        ...
        contentString.append("<html><head><title>" + uri.getLastPathSegment() + "</title>");
        contentString.append("<link href='file:///android_asset/prettify.css' rel='stylesheet' type='text/css'/> ");
        contentString.append("<script src='file:///android_asset/prettify.js' type='text/javascript'></script> ");
        contentString.append(handler.getFileScriptFiles());
        contentString.append("</head><body onload='prettyPrint()'><code class='" + handler.getFilePrettifyClass() + "'>");
        ...
        contentString.append(handler.getFileFormattedString(codeString.toString()));
        contentString.append("</code><br /><br /><br /><br /><br /></body></html> ");
        mWebView.getSettings().setUseWideViewPort(true);
        mWebView.loadDataWithBaseURL("file:///android_asset/", contentString.toString(), handler.getFileMimeType(), "", "");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            alert.setTitle("Exit").setMessage("Quit Android CodePad?").setPositiveButton("Yes", new DialogInterface.OnClickListener() {
