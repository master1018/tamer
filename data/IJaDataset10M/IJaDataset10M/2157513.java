...
public class FImportacao extends FDetalhe implements ActionListener, ChangeListener, InsertListener, DeleteListener, CarregaListener, TabelaEditListener, MouseListener {
    ...
    private JButtonPad btGerarAdicoes = new JButtonPad("Gerar adi��es", Icone.novo("btGerar.gif"));
    ...
    private enum GRID_ADICAO {

        CODNCM, CODADIC, VLRTXSISCOMEXADIC, PESOLIQUIDO, VLRTHCMI, VMLEMI, VLRFRETEMI, VMCV, VLRADUANEIRO, VLRII, VLRIPI, VLRPIS, VLRCOFINS, VLRBASEICMS, VLRICMSRECOLHER
    }
    ...
    private void adicToolTips() {
        btRateioFrete.setToolTipText("Rateia o valor total do frete e THC entre os �tens");
    ...
    private void montaTela() {
        ...
        adicDescFK(txtRazFor, 143, 20, 243, 20, "RazFor", "Raz�o Social do Fornecedor");
        ...
        adicDescFK(txtDescPlanoPag, 143, 60, 243, 20, "DescPlanoPag", "Descri��o do plano de pagamento");
        ...
        adicCampo(txtVeiculo, 519, 20, 180, 20, "Veiculo", "Ve�culo", ListaCampos.DB_SI, false);
        ...
        adicCampo(txtPresCarga, 110, 60, 100, 20, "PresCarga", "Presen�a de carga", ListaCampos.DB_SI, false);
    ...
    private void calcPISCOFINS() {
        try {
            ...
            float I = 0.00f;
            if (A > 0) {
                I = txtAliqICMSImp.getVlrBigDecimal().floatValue() / 100f;
            } else {
                I = txtAliqICMSUF.getVlrBigDecimal().floatValue() / 100f;
            }
            float M = txtAliqII.getVlrBigDecimal().floatValue() / 100f;
            float K = txtAliqPIS.getVlrBigDecimal().floatValue() / 100f;
            float L = txtAliqCOFINS.getVlrBigDecimal().floatValue() / 100f;
            float J = txtAliqIPI.getVlrBigDecimal().floatValue() / 100f;
    ...
    public void valorAlterado(TabelaEditEvent evt) {
        if (evt.getTabela() == tabAdicao && tabAdicao.getSelectedRow() > -1 && (tabAdicao.getColunaEditada() == GRID_ADICAO.CODADIC.ordinal() || tabAdicao.getColunaEditada() == GRID_ADICAO.VLRTXSISCOMEXADIC.ordinal()) && tabAdicao.isCellEditable(tabAdicao.getLinhaEditada(), tabAdicao.getColunaEditada())) {
