...
class AssertionFailureError extends Error {
    ...
    public static final long serialVersionUID = 0xffff00L;
...
public class Lib {
    ...
    public static void alphaImage(BufferedImage img, int alpha) {
        ...
        for (int i = 0; i < argb.length; i++) {
            if ((argb[i] & 0xff000000) != 0) argb[i] &= alpha;
        }
        img.setRGB(0, 0, img.getWidth(), img.getHeight(), argb, 0, img.getWidth());
    }
    ...
    public static boolean test(char flag) {
        if (debugFlags == null) return false; else if (debugFlags[(int) '+']) return true; else if (flag >= 0 && flag < 0x80 && debugFlags[(int) flag]) return true; else return false;
    }
    ...
    public static int bytesToInt(byte[] array, int offset) {
        return (int) ((((int) array[offset + 0] & 0xFF) << 0) | (((int) array[offset + 1] & 0xFF) << 8) | (((int) array[offset + 2] & 0xFF) << 16) | (((int) array[offset + 3] & 0xFF) << 24));
    }
    ...
    public static int extract(int bits, int lowest, int size) {
        if (size == 32) return (bits >> lowest); else return ((bits >> lowest) & ((1 << size) - 1));
    }
    ...
    public static long extract(long bits, int lowest, int size) {
        if (size == 64) return (bits >> lowest); else return ((bits >> lowest) & ((1L << size) - 1));
    }
    ...
    public static void checkField(Class cls, String fieldName, Class fieldType) {
        try {
            ...
            Lib.assertTrue(Modifier.isPublic(field.getModifiers()) && !Modifier.isStatic(field.getModifiers()) && !Modifier.isFinal(field.getModifiers()));
        } catch (Exception e) {
            Lib.assertNotReached();
        }
    }
