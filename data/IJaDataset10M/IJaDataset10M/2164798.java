...
public class Cache extends NameSet {
    ...
    private class Element {
        ...
        public Element(int _ttl, byte cred, int src) {
            rrset = null;
            credibility = cred;
            ttl = _ttl;
            srcid = src;
        ...
        public Element(Record r, byte cred, int src) {
            rrset = new RRset();
            credibility = cred;
            ttl = -1;
            srcid = src;
        ...
        public Element(RRset r, byte cred, int src) {
            rrset = r;
            credibility = cred;
            ttl = -1;
            timeIn = System.currentTimeMillis();
 ...
 */
    public void addRecord(Record r, byte cred, Object o) {
        ...
        if (r.getTTL() == 0) return;
        Element element = (Element) findExactSet(name, type, dclass);
        if (element == null || cred > element.credibility) addSet(name, type, dclass, element = new Element(r, cred, src)); else if (cred == element.credibility) {
            if (element.srcid != src) element.rrset.clear();
            element.update(r);
        }
    }
 ...
 */
    public void addRRset(RRset rrset, byte cred, Object o) {
        ...
        int src = (o != null) ? o.hashCode() : 0;
        if (rrset.getTTL() == 0) return;
        Element element = (Element) findExactSet(name, type, dclass);
        if (element == null || cred > element.credibility) addSet(name, type, dclass, new Element(rrset, cred, src));
    }
 ...
 */
    public void addNegative(Name name, short type, short dclass, int ttl, byte cred, Object o) {
        int src = (o != null) ? o.hashCode() : 0;
        Element element = (Element) findExactSet(name, type, dclass);
        if (element == null || cred > element.credibility) addSet(name, type, dclass, new Element(ttl, cred, src));
    }
 ...
 * @see Credibility
 */
    public SetResponse lookupRecords(Name name, short type, short dclass, byte minCred) {
        SetResponse cr = null;
        Object[] objects = findSets(name, type, dclass);
        if (objects == null) return new SetResponse(SetResponse.UNKNOWN);
        int nelements = 0;
        for (int i = 0; i < objects.length; i++) {
            ...
            if (element.expiredTTL()) {
                ...
                objects[i] = null;
            } else if (element.credibility < minCred) objects[i] = null; else nelements++;
        }
        if (nelements == 0) return new SetResponse(SetResponse.UNKNOWN);
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            if (type != Type.CNAME && type != Type.ANY && rrset.getType() == Type.CNAME) {
                ...
                cr = lookupRecords(cname.getTarget(), type, dclass, minCred);
                if (cr.isUnknown()) cr.set(SetResponse.PARTIAL, cname);
                cr.addCNAME(cname);
                return cr;
            }
    ...
    private RRset[] findRecords(Name name, short type, short dclass, byte minCred) {
        SetResponse cr = lookupRecords(name, type, dclass, minCred);
        if (cr.isSuccessful()) return cr.answers(); else return null;
    }
 ...
 * @see Credibility
 */
    public RRset[] findRecords(Name name, short type, short dclass) {
 ...
 * @see Credibility
 */
    public RRset[] findAnyRecords(Name name, short type, short dclass) {
 ...
 */
    public void addMessage(Message in) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (isAuth && r.getName().equals(queryName)) cred = Credibility.AUTH_ANSWER; else if (isAuth) cred = Credibility.AUTH_NONAUTH_ANSWER; else cred = Credibility.NONAUTH_ANSWER;
            addRecord(r, cred, in);
        }
        if (ancount == 0 || rcode == Rcode.NXDOMAIN) {
            ...
            if (isAuth) cred = Credibility.AUTH_AUTHORITY; else cred = Credibility.NONAUTH_AUTHORITY;
            if (soa != null) {
                int ttl = Math.min(soa.getTTL(), soa.getMinimum());
                if (ancount == 0) addNegative(queryName, queryType, queryClass, ttl, cred, in); else {
                    Record[] cnames;
                    cnames = in.getSectionArray(Section.ANSWER);
                    int last = cnames.length - 1;
                    Name cname;
                    cname = ((CNAMERecord) cnames[last]).getTarget();
                    addNegative(cname, queryType, queryClass, ttl, cred, in);
                }
            }
        }
        e = in.getSection(Section.AUTHORITY);
        ...
        while (e.hasMoreElements()) {
            Record r = (Record) e.nextElement();
            if (isAuth) cred = Credibility.AUTH_ADDITIONAL; else cred = Credibility.NONAUTH_ADDITIONAL;
            addRecord(r, cred, in);
        }
    }
}
