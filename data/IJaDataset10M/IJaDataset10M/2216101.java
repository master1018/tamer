...
public class Forest extends JComponent implements MouseListener, MouseMotionListener, Serializable {
    ...
    public static final int HEALTHY = 0, ON_FIRE = 1, BURNT = 2;
	...
	*/
    public void setProbabilities(double[] p) {
        if (p.length != 4) p = new double[] { 0.5, 0.5, 0.5, 0.5 }; else for (int i = 0; i < 4; i++) probabilities[i] = Functions.getProbability(p[i]);
    }
	...
	*/
    public void setStates(int x, int y, int s) {
        ...
        if (x >= 0 & x < columns & y >= 0 & y < rows) {
            ...
            states[x][y] = s;
            if (s == ON_FIRE) {
                if (oldState == HEALTHY) fire.addElement(p); else if (oldState == BURNT) {
                    fire.addElement(p);
                    burnt.removeElement(p);
                }
            } else if (s == BURNT) {
                if (oldState == HEALTHY) burnt.addElement(p); else if (oldState == ON_FIRE) {
                    burnt.addElement(p);
	...
	*/
    public void setStates(int[][] s) {
        ...
        for (int i = 0; i < columns; i++) for (int j = 0; j < rows; j++) {
            if (states[i][j] == ON_FIRE) fire.addElement(new Point(i, j)); else if (states[i][j] == BURNT) fire.addElement(new Point(i, j));
        }
    }
	...
	*/
    public void mouseMoved(MouseEvent e) {
        if (e.getSource() == this) {
            int xMouse = e.getX() / treeSize;
            int yMouse = e.getY() / treeSize;
            if (xMouse < columns & yMouse < rows) setToolTipText("(" + xMouse + ", " + yMouse + ")"); else setToolTipText("Forest");
        }
    }
