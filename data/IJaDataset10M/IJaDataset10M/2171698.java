	...
	*/
    ...
    private String getDocumentAsString(String docName) throws XMLDBException {
        ...
        try {
            ...
            if (doc != null) return serializer.serialize(doc); else return null;
        } catch (Exception e) {
            throw new XMLDBException(ErrorCodes.VENDOR_ERROR, "Error getting document " + docName, e);
        } finally {
            if (broker != null) brokerPool.release(broker);
        }
    }
    ...
    private String getSchemaFilename(String targetNamespace) throws XMLDBException {
        if (targetNamespace == null) throw new XMLDBException(ErrorCodes.VENDOR_ERROR, "null is not a valid namespace!");
        ...
        if ("schema-index".equals(root.getNodeName())) {
            ...
            for (int i = 0; i < schemas.getLength(); i++) {
                ...
                if ("schema".equals(schema.getNodeName())) {
                    ...
                    if ((targetNamespaceAttr != null) && (targetNamespace.equals(targetNamespaceAttr.getNodeValue()))) return schema.getAttributes().getNamedItem("resourceName").getNodeValue();
                }
            }
        } else {
            ...
            if (schemaFilename != null) {
                try {
                    ...
                    if (schema != null) {
                        ...
                        if (castorSchema != null) schemaCache.put(targetNamespace, castorSchema);
                    } else throw new XMLDBException(ErrorCodes.VENDOR_ERROR, "Schema document for target namespace " + targetNamespace + " not found even though it is in the index.", null);
                } catch (IOException e) {
    ...
    public XMLType getType(QName qname) throws XMLDBException {
        if ((qname.getNamespaceURI() == null) || ("".equals(qname.getNamespaceURI()))) throw new IllegalArgumentException("QName " + qname.toString() + " is not fully qualified.");
    ...
    public ElementDecl getElement(QName qname) throws XMLDBException {
        ...
        if (element == null) {
            ...
            if (schema != null) {
                ...
                if (element != null) elementCache.put(qname, element); else return null;
            } else {
                return null;
            }
        }
