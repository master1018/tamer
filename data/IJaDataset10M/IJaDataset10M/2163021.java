...
public final class SnmpCollection implements ReadyRunnable {
	 ...
	 */
    boolean failed() {
        return !hasIpNetToMediaTable() && !hasRouteTable() && !hasCdpCacheTable() && !hasVlanTable();
    }
    ...
    public void run() {
        if (suspendCollection) {
            log().debug("SnmpCollection.run: address: " + m_address.getHostAddress() + " Suspended!");
        } else {
            m_ipNetToMedia = new IpNetToMediaTable(m_address);
            m_ipRoute = new IpRouteTable(m_address);
            m_CdpCache = new CdpCacheTable(m_address);
            if (log().isDebugEnabled()) log().debug("run: collecting : " + m_agentConfig);
            ...
            if (m_collectVlanTable && m_collectIpRouteTable && m_collectCdpTable) {
                ...
                walker = SnmpUtils.createWalker(m_agentConfig, "ipNetToMediaTable/ipRouteTable/vlanTable", new CollectionTracker[] { m_ipNetToMedia, m_ipRoute, m_vlanTable });
            } else if (m_collectVlanTable && m_collectCdpTable) {
                walker = SnmpUtils.createWalker(m_agentConfig, "ipNetToMediaTable/vlanTable/cdpCacheTable", new CollectionTracker[] { m_ipNetToMedia, m_vlanTable, m_CdpCache });
            } else if (m_collectIpRouteTable) {
                walker = SnmpUtils.createWalker(m_agentConfig, "ipNetToMediaTable/ipRouteTable", new CollectionTracker[] { m_ipNetToMedia, m_ipRoute });
            } else if (m_collectVlanTable) {
                walker = SnmpUtils.createWalker(m_agentConfig, "ipNetToMediaTable/vlanTable", new CollectionTracker[] { m_ipNetToMedia, m_vlanTable });
            } else if (m_collectCdpTable) {
                walker = SnmpUtils.createWalker(m_agentConfig, "ipNetToMediaTable/cdpCacheTable", new CollectionTracker[] { m_ipNetToMedia, m_CdpCache });
            } else {
                walker = SnmpUtils.createWalker(m_agentConfig, "ipNetToMediaTable", new CollectionTracker[] { m_ipNetToMedia });
            }
            walker.start();
            try {
                walker.waitFor();
            ...
            if (!this.hasIpNetToMediaTable()) log().info("SnmpCollection.run: failed to collect ipNetToMediaTable for " + m_address.getHostAddress());
            if (!this.hasRouteTable()) log().info("SnmpCollection.run: failed to collect ipRouteTable for " + m_address.getHostAddress());
            if (!this.hasCdpCacheTable()) log().info("SnmpCollection.run: failed to collect dpCacheTable for " + m_address.getHostAddress());
            if (m_collectVlanTable && !this.hasVlanTable()) log().info("SnmpCollection.run: failed to collect Vlan for " + m_address.getHostAddress());
            Vlan vlan = null;
            if (this.hasVlanTable()) {
                if (!m_vlanClass.equals("org.opennms.netmgt.linkd.snmp.CiscoVlanTable") && !m_vlanClass.equals("org.opennms.netmgt.linkd.snmp.IntelVlanTable")) {
                    runAndSaveSnmpVlanCollection(new Vlan(TRUNK_VLAN_INDEX, TRUNK_VLAN_NAME, VlanCollectorEntry.VLAN_STATUS_OPERATIONAL));
                } else {
                    if (log().isDebugEnabled()) log().debug("SnmpCollection.run: start collection for " + getVlanTable().getEntries().size() + " VLAN entries ");
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (status == null || status != VlanCollectorEntry.VLAN_STATUS_OPERATIONAL) {
                            if (log().isInfoEnabled()) log().info("SnmpCollection.run: skipping VLAN " + vlan + " NOT ACTIVE or null ");
                            continue;
                        }
                        ...
                        if (type == null || type != VlanCollectorEntry.VLAN_TYPE_ETHERNET) {
                            if (log().isInfoEnabled()) log().info("SnmpCollection.run: skipping VLAN " + vlan + " NOT ETHERNET TYPE");
                            continue;
                        }
            ...
            if (log().isDebugEnabled()) log().debug("SnmpCollection.run: saving collection into database ");
    ...
    private void runAndSaveSnmpVlanCollection(Vlan vlan) {
        ...
        if (snmpvlancollection.failed()) {
            if (log().isDebugEnabled()) log().debug("SnmpCollection.run: no bridge info found");
        } else {
            if (log().isDebugEnabled()) log().debug("SnmpCollection.run: adding bridge info to snmpcollection");
    ...
    public void schedule() {
        if (m_scheduler == null) throw new IllegalStateException("Cannot schedule a service whose scheduler is set to null");
    ...
    public void unschedule() {
        if (m_scheduler == null) throw new IllegalStateException("rescedule: Cannot schedule a service whose scheduler is set to null");
        if (runned) {
    ...
    public boolean equals(ReadyRunnable run) {
        if (run instanceof SnmpCollection && this.getPackageName().equals(run.getPackageName())) {
            ...
            if (c.getTarget().equals(m_address) && c.getPort() == getPort() && c.getReadCommunity().equals(getReadCommunity())) return true;
        }
        return false;
    }
