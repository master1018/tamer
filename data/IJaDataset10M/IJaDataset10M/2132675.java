        ...
        do {
            switch(parser.nextToken()) {
                ...
                case XmlPullParser.END_TAG:
                    if (stack.isEmpty()) done = true; else {
                        prefix = parser.getPrefix();
                        elementName = parser.getName();
                        xml.writeEndTag(prefix, elementName);
                        for (int i = stack.size() - 1; i >= 0; i--) if (stack.get(i).equals((prefix != null ? prefix + ":" : "") + elementName)) {
                            for (int j = stack.size() - 1; j >= i; j--) stack.remove(j);
                            break;
                        }
                    }
                    break;
                case XmlPullParser.START_TAG:
                    ...
                    xml.writeStartTag(prefix, elementName, attributePrefixes, attributeNames, attributeValues);
                    break;
            }
        } while (!done);
