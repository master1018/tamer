...
public final class CSSFactory {
    ...
    public static final TermFactory getTermFactory() {
        if (tf == null) {
            try {
                @SuppressWarnings("unchecked") Class<? extends TermFactory> clazz = (Class<? extends TermFactory>) Class.forName(DEFAULT_TERM_FACTORY);
    ...
    public static final SupportedCSS getSupportedCSS() {
        if (css == null) {
            try {
                @SuppressWarnings("unchecked") Class<? extends SupportedCSS> clazz = (Class<? extends SupportedCSS>) Class.forName(DEFAULT_SUPPORTED_CSS);
                Method m = clazz.getMethod("getInstance");
    ...
    public static final RuleFactory getRuleFactory() {
        if (rf == null) {
            try {
                @SuppressWarnings("unchecked") Class<? extends RuleFactory> clazz = (Class<? extends RuleFactory>) Class.forName(DEFAULT_RULE_FACTORY);
                Method m = clazz.getMethod("getInstance");
    ...
    public static final void registerNodeDataInstance(Class<? extends NodeData> clazz) {
        try {
            @SuppressWarnings("unused") NodeData test = clazz.newInstance();
            ndImpl = clazz;
    ...
    public static final NodeData createNodeData() {
        if (ndImpl == null) {
            try {
                @SuppressWarnings("unchecked") Class<? extends NodeData> clazz = (Class<? extends NodeData>) Class.forName(DEFAULT_NODE_DATA_IMPL);
    ...
    public static final StyleSheet getUsedStyles(Document doc, URL base, String media) {
        ...
        Traversal<StyleSheet> traversal = new CSSAssignTraversal(doc, (Object) pair, NodeFilter.SHOW_ELEMENT);
    ...
    private static final class CSSAssignTraversal extends Traversal<StyleSheet> {
        ...
        protected void processNode(StyleSheet result, Node current, Object source) {
            ...
            try {
                if (isEmbeddedStyleSheet(elem, media)) {
                    result = CSSParserFactory.append(extractElementText(elem), SourceType.EMBEDDED, result, base);
                    log.debug("Matched embedded CSS style");
                } else if (isLinkedStyleSheet(elem, media)) {
                    URL uri = new URL(base, elem.getAttribute("href"));
                    result = CSSParserFactory.append(uri, SourceType.URL, result, base);
                    log.debug("Matched linked CSS style");
                } else {
                    if (elem.getAttribute("style") != null && elem.getAttribute("style").length() > 0) {
                        result = CSSParserFactory.append(elem.getAttribute("style"), SourceType.INLINE, elem, true, result, base);
                        log.debug("Matched inline CSS style");
                    }
                    if (elem.getAttribute("XDefaultStyle") != null && elem.getAttribute("XDefaultStyle").length() > 0) {
                        result = CSSParserFactory.append(elem.getAttribute("XDefaultStyle"), SourceType.INLINE, elem, false, result, base);
        ...
        private static boolean isEmbeddedStyleSheet(Element e, String media) {
            return "style".equalsIgnoreCase(e.getNodeName()) && isAllowedMedia(e, media);
        }

        private static boolean isLinkedStyleSheet(Element e, String media) {
            return e.getNodeName().equals("link") && "stylesheet".equalsIgnoreCase(e.getAttribute("rel")) && "text/css".equalsIgnoreCase(e.getAttribute("type")) && isAllowedMedia(e, media);
        }
        ...
        private static String extractElementText(Element e) {
            ...
            if (text != null && text.getNodeType() == Node.TEXT_NODE) return ((Text) text).getData();
            return null;
        }
        ...
        private static boolean isAllowedMedia(Element e, String media) {
            ...
            if (mediaList == null || mediaList.length() == 0 || mediaList.contains(media) || mediaList.contains("all")) return true;
            return false;
        }
    }
