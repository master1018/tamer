...
public class DifferentialNavigationSystem extends NXTDevice implements INXTDevice {
    ...
    public void updateActuatorSystem(DNSCommand cmd) {
        ...
        if (type.equals("forward")) df.forward(); else if (type.equals("backward")) df.backward(); else if (type.equals("stop")) df.stop(); else if (type.equals("rotateLeft")) df.rotateLeft(); else if (type.equals("rotateRight")) df.rotateRight(); else if (type.equals("travel")) df.travel(value); else if (type.equals("rotate")) df.rotate(value); else if (type.equals("setTravelSpeed")) df.setTravelSpeed(value); else if (type.equals("setRotateSpeed")) df.setRotateSpeed(value); else if (type.equals("setAcceleration")) df.setAcceleration((int) value);
    }
    ...
    public void updateVelocity(Velocity v) {
        ...
        if (linear != 0 && angular == 0) {
            ...
            if (forward) df.forward(); else df.backward();
        } else if (angular != 0 && linear == 0) {
            boolean left = (angular > 0);
            if (left) df.rotateLeft(); else df.rotateRight();
        }
        Delay.msDelay(1000);
        df.stop();
    }
    ...
    private void poseToTransform(Pose p) {
        ...
        ftr.transform.translation.x = FRONT_OFFSET;
        ftr.transform.translation.y = 0;
        ftr.transform.translation.z = 0;
        ftr.transform.rotation.w = 1;
