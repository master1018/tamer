...
public class ExportPDF implements Service {
    ...
    private static final int MAP_WIDTH_PX = 1160;

    private static final int MAP_HEIGHT_PX = 870;
    ...
    public Element exec(Element params, ServiceContext context) throws Exception {
        ...
        if (isLandscape) pagesize = pagesize.rotate();
        BoundingBox bb = Util.parseBoundingBox(params);
        if (bb != null) mm.setBoundingBox(bb);
        ...
        if (copyright != null) document.addHeader("Copyright note", copyright);
        if (copyright != null) pw.setPageEvent(new PE(copyright));
        document.open();
        if (title != null) {
        ...
        if (drawArrow) {
            ...
            BufferedImage rect = ImageUtils.createRect(mapimage.getWidth() + arrow.getWidth() + 10, mapimage.getHeight(), Color.WHITE);
            BufferedImage merged;
            merged = ImageMerger.merge(rect, mapimage, 0, 0);
            merged = ImageMerger.merge(merged, arrow, -1, 5);
            mapImageFullPath = GlobalTempFiles.getInstance().getFile().getPath();
        ...
        if (params.getChildText("boundingbox") != null) {
            PdfPTable t = getBBTable(mm.getBoundingBox());
            t.setSpacingBefore(30);
            document.add(t);
        }
        document.close();
        return new Element("response").addContent(new Element("pdf").addContent(new Element("url").setText(MapUtil.getTempUrl() + "/" + pdfFile.getName())));
    }
    ...
    private boolean isLandscape(String o) {
        if (o.equalsIgnoreCase("landscape")) return true; else if (o.equalsIgnoreCase("portrait")) return false; else return false;
    }
    ...
    static class PE extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            if (showWaterMark) {
                ...
                contentunder.showTextAligned(com.lowagie.text.Element.ALIGN_CENTER, _watermarkText, document.getPageSize().getWidth() / 2, document.getPageSize().getHeight() / 2, 45);
                contentunder.endText();
                contentunder.restoreState();
            }
            PdfContentByte cb = writer.getDirectContent();
            ColumnText.showTextAligned(cb, com.lowagie.text.Element.ALIGN_CENTER, footer, (document.right() - document.left()) / 2 + document.leftMargin(), document.bottom() - 10, 0);
        }
    }
}
