...
public class AppResourceManager extends ResourceManager {
     ...
     */
    public static final byte TYPE_STRING = (byte) 0x01;

    /**
     * Constant for binary type resource.
     */
    public static final byte TYPE_BINARY = (byte) 0x10;

    /**
     * Constant for end type.
     */
    public static final byte TYPE_END = (byte) 0x00;
    ...
    protected byte getResourceType(int id) throws ResourceException {
        ...
        throw new ResourceException(ResourceException.RESOURCE_NOT_FOUND, "Resource not found.");
    }
    ...
    public byte[] getData(int id) throws ResourceException {
        ...
        if (!(o instanceof byte[])) {
            throw new ResourceException(ResourceException.WRONG_RESOURCE_TYPE, "Resource is not of type BINARY");
        }
        return (byte[]) o;
    }
        ...
        for (int i = 0; i < bundleReaders.length; i++) {
            ...
            try {
                if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                    Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": getResource reader=" + reader.getResourceName());
                    Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + " getResource for id \"" + id + "\"");
                }
                if (isCaching()) {
                    if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                        Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": Using caching instance.");
                    }
                    hashCode = resourceCache.getHashCodeForResource(getBaseName(), locales[i], id);
                    if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                        Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": hashCode=" + hashCode);
                    }
                    resInCache = resourceCache.lookup(hashCode);
                    if (resInCache != null) {
                        resource = resInCache.getValue();
                        if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                            Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": Resource was in cache.");
                        }
                    } else {
                        if (reader.isValidResourceID(id)) {
                            type = reader.getResourceType(id);
                            length = reader.getResourceLength(id);
                            resource = convertToResourceType(id, type, length, reader);
                            if (resource == null) {
                                if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                                    Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": Cannot " + "convert resource to object");
                                }
                                throw new ResourceException(ResourceException.UNKNOWN_RESOURCE_TYPE, "Resource type is unknown.");
                            }
                            resourceCache.addResource(new Resource(hashCode, length, resource));
                        } else {
                            if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                                Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "Resource id = " + id + " is NOT valid.");
                            }
                            continue;
                        }
                    }
                    ...
                    if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                        Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "Using NOTcaching instance.");
                    }
                    if (reader.isValidResourceID(id)) {
                        type = reader.getResourceType(id);
                        length = reader.getResourceLength(id);
                        ...
                        if (resource == null) {
                            if (Logging.REPORT_LEVEL <= Logging.WARNING) {
                                Logging.report(Logging.WARNING, LogChannels.LC_JSR238, classname + ": Cannot " + "convert resource to object");
                            }
                            throw new ResourceException(ResourceException.UNKNOWN_RESOURCE_TYPE, "Resource type is unknown.");
                        }
                    } else {
                ...
                if (Logging.REPORT_LEVEL <= Logging.WARNING) {
                    Logging.report(Logging.WARNING, LogChannels.LC_JSR238, classname + ": " + "IOException: " + ioe.getMessage());
                }
                throw new ResourceException(ResourceException.DATA_ERROR, "Error reading resource");
            }
            if (resource == null) {
                throw new ResourceException(ResourceException.UNKNOWN_RESOURCE_TYPE, "Unknown resource type \"" + type + "\"");
            } else {
                if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                    Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "Returning resource object " + resource);
                }
                return cloneResource(resource);
            }
        }
        throw new ResourceException(ResourceException.RESOURCE_NOT_FOUND, "Invalid resource id " + id + ".");
    }
     ...
     */
    protected Object convertToResourceType(int resourceID, byte type, int length, ResourceBundleReader reader) throws IOException {
        if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
            Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "converting resource id = \"" + resourceID + "\" to object" + " type :\"" + type + "\" with reader : " + reader);
        }
        byte[] data = null;
        if (length != 0) {
        ...
        if (type == TYPE_STRING) {
            try {
                ...
                if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                    Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "data converted to utf8 string \"" + (String) o + "\"");
                }
                return o;
            ...
            if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "data not converted using pure binary");
            }
            return data;
        }
        return null;
    ...
    protected Object cloneResource(Object resource) {
        if (resource instanceof String) {
            if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "returning cloned string resource");
            }
            ...
            if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
                Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "returning cloned binary resource");
            }
            return clone;
        }
        return resource;
    ...
    public String getString(int id) throws ResourceException {
        ...
        if (!(o instanceof String)) {
            throw new ResourceException(ResourceException.WRONG_RESOURCE_TYPE, "Resource is not of type STRING");
        }
        return (String) o;
    }
    ...
    public boolean isValidResourceID(int id) {
        if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
            Logging.report(Logging.INFORMATION, LogChannels.LC_JSR238, classname + ": " + "validating resourceId=" + id);
        }
        if (id < 0) {
            throw new IllegalArgumentException("Illegal resource ID.");
        }
    ...
    protected final String getUTF8(byte[] bytearr) throws IllegalArgumentException {
        ...
        while (count < bytearr.length) {
            ...
            if ((utf32 & 0x80) == 0) {
                ...
                numOfBytes = 4;
                utf32 &= ~0xFFFFFFF8;
            } else throw new IllegalArgumentException("malformed input: around byte " + count);
            while (--numOfBytes != 0) {
