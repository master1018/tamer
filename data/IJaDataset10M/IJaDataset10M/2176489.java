...
public class TexHardLineWrapAction implements IEditorActionDelegate {
    ...
    private void doWrap(TexSelections selection) throws BadLocationException {
        ...
        while (index < lines.length) {
            if (tools.isLineCommandLine(lines[index]) || tools.isLineCommentLine(lines[index]) || lines[index].trim().length() == 0) {
                ...
                index++;
                if (index < lines.length) buff.append(delimiter);
                continue;
            }
            if (fix) {
            ...
            while (selectedLine.length() > 0) {
                wsLast = tools.getLastWSPosition(selectedLine, (lineLength - correctIndentation.length())) + 1;
                if (wsLast == 0) {
                    wsLast = tools.getFirstWSPosition(selectedLine, (lineLength - correctIndentation.length())) + 1;
                }
                if (wsLast == 0 || wsLast > selectedLine.length() || selectedLine.length() < (lineLength - correctIndentation.length())) {
                ...
                if (index < lines.length || selectedLine.length() > 0) buff.append(delimiter);
            }
        }
    ...
    private boolean isList(String txt) {
        ...
        if ((bi = txt.indexOf("\\begin")) != -1) {
            ...
            if (env.equals("itemize") || env.equals("enumerate") || env.equals("description")) rv = true;
        }
        return rv;
    }
