...
public class GridTool extends StateTool {
    ...
    public GridTool(GridModel gridModel) {
        ...
        for (int i = 0; i < gridModel.gridLayouts.length; i++) {
            ...
            if (gridModel.gridLayouts[i] == gridModel.getGridLayout()) layout.setSelected(true);
            final int place = i;
            layout.addItemListener(new ItemListener() {

                public void itemStateChanged(ItemEvent e) {
                    if (e.getStateChange() == ItemEvent.SELECTED) GridTool.this.gridModel.setGridLayout(GridTool.this.gridModel.gridLayouts[place]);
                }
            });
        ...
        for (int i = 0; i < gridModel.gridSizes.length; i++) {
            ...
            if (gridModel.gridSizes[i] == gridModel.getGranularity()) gran.setSelected(true);
            final int place = i;
    ...
    public void propertyChange(PropertyChangeEvent e) {
        if (e.getPropertyName().equals(StateTool.ACTIVATED)) activated(((Boolean) e.getNewValue()).booleanValue());
    }
    ...
    public void removeYourself(ToolsBar toolSetBar) {
        ...
        if (but != null) but.removeMouseListener(gridMenuListener);
