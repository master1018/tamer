...
public abstract class StoredProcParamDescBase {
    ...
    public boolean isColOut() {
        if (m_sColType == DatabaseMetaData.procedureColumnOut) return true;
        return false;
    }

    public boolean isColInOut() {
        if (m_sColType == DatabaseMetaData.procedureColumnInOut) return true;
        return false;
    }

    public boolean isColIn() {
        if (m_sColType == DatabaseMetaData.procedureColumnIn) return true;
        return false;
    }
    ...
    public boolean registerIntoCallableStatement(int nParamId, DbPreparedCallableStatement callableStatement) {
        nParamId++;
        if (m_sColType == DatabaseMetaData.procedureColumnOut) return callableStatement.registerOutParameter(nParamId, m_colDescriptionInfo);
        if (m_sColType == DatabaseMetaData.procedureColumnInOut) callableStatement.registerOutParameter(nParamId, m_colDescriptionInfo);
        return fillInValue(nParamId, callableStatement);
    }

    public String toString() {
        ListCoupleRender lst = ListCoupleRender.set("Column description: ");
        if (m_sColType == DatabaseMetaData.procedureColumnOut) lst.set("Way", "Out");
        if (m_sColType == DatabaseMetaData.procedureColumnIn) lst.set("Way", "In");
        if (m_sColType == DatabaseMetaData.procedureColumnInOut) lst.set("Way", "InOut");
