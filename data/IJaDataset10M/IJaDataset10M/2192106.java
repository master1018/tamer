...
public class JikesRVMChangeConfigurationDialog extends StatusDialog {
    ...
    public static final void changeConfiguration(Shell parent, JikesRVMInstall install, boolean useDefaults) {
        new JikesRVMChangeConfigurationDialog(parent, install, useDefaults).open();
    }

    public static final void changeConfiguration(JikesRVMInstall install, boolean useDefaults) {
    ...
    private JikesRVMChangeConfigurationDialog(Shell parent, JikesRVMInstall install, boolean useDefaults) {
        ...
        if (useDefaults) {
            rvmRootField.setText(install.getRvmRoot() != null ? install.getRvmRoot() : nonNull(install.getDefaultRvmRoot()));
            rvmBuildField.setText(install.getRvmBuild() != null ? install.getRvmBuild() : nonNull(install.getDefaultRvmBuild()));
        } else {
    ...
    protected Control createDialogArea(Composite ancestor) {
        ...
        commandLineDebuggerTypeButton = addRadioButton(parent, JikesRVMLauncherMessages.getString("JikesRVMPreferencePage.commandLineDebugger"), install.getDebugMode() == JikesRVMInstall.DebugMode.COMMAND_LINE, indent, debuggerTypesButtons);
        commandLineDebuggerTypeButton.addSelectionListener(debuggerTypesSelectionListener);
        guiDebuggerTypeButton = addRadioButton(parent, JikesRVMLauncherMessages.getString("JikesRVMPreferencePage.guiDebugger"), install.getDebugMode() == JikesRVMInstall.DebugMode.GUI, indent, debuggerTypesButtons);
    ...
    protected void updateStatusLine() {
        ...
        for (int i = 0; i < stati.length; i++) {
            if (stati[i] != null) {
                boolean isBiggerSeverity = stati[i].getSeverity() > currentSeverity;
                boolean isBetterErrorMessage = stati[i].getSeverity() == currentSeverity && (currentMessage == null || "".equals(currentMessage));
                if (isBiggerSeverity) {
    ...
    private void useDefaults() {
        ...
        updateStatus(new Status(IStatus.INFO, install.getId(), 0, JikesRVMLauncherMessages.getString("configureDialog.usingDefaults"), null));
    }
    ...
    private IStatus validateRvmRootLocation() {
        ...
        if (!f.exists()) {
            return new Status(IStatus.ERROR, install.getId(), 0, JikesRVMLauncherMessages.getString("configureDialog.nofileRvmRoot"), null);
        }
        if (!f.isDirectory()) {
            return new Status(IStatus.ERROR, install.getId(), 0, JikesRVMLauncherMessages.getString("configureDialog.notADirRvmRoot"), null);
        }
        return null;
    }
    ...
    private IStatus validateRvmBuildLocation() {
        ...
        if (!f.exists()) {
            return new Status(IStatus.ERROR, install.getId(), 0, JikesRVMLauncherMessages.getString("configureDialog.nofileRvmBuild"), null);
        }
        if (!f.isDirectory()) {
            return new Status(IStatus.ERROR, install.getId(), 0, JikesRVMLauncherMessages.getString("configureDialog.notADirRvmBuild"), null);
        }
        return null;
    }

    protected void okPressed() {
        final boolean overridingEnv = false;
        synchronized (this) {
            if (overridingEnv) {
   ...
   */
    private Button addRadioButton(Composite parent, String label, boolean selection, int indent, List radioButtons) {
