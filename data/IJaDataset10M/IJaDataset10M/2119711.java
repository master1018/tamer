...
public class MethodSpecSignature extends Signature {
    ...
    public static void decodeAndAttach(BasicTypesDirectory directory, MethodSpecEntry methodSpec) throws InvalidSignatureException {
        new MethodSpecSignature(directory, methodSpec);
    }

    public MethodSpecSignature(BasicTypesDirectory directory, MethodSpecEntry methodSpec) throws InvalidSignatureException {
        setBinarySignature(methodSpec.getInstantiation());
        setDirectory(directory);
        nextToken();
        if (currentToken != PREFIX_GENERIC_INSTANCE) throw new InvalidSignatureException(currentToken, PREFIX_GENERIC_INSTANCE);
        nextToken();
