...
public class FlatNetworkLearning extends SupervisedLearning implements Serializable {
    ...
    public FlatNetworkLearning(NeuralNetwork network) {
        FlatNetworkPlugin plugin = (FlatNetworkPlugin) network.getPlugin(FlatNetworkPlugin.class);
        FlatNetwork flat = plugin.getFlatNetwork();
        if (this.flat == null) throw new EncogEngineError("This learning rule only works with a network that has a FlatNetworkPlugin attached.");
        init(flat);
    }
        ...
        if (this.lastLearningType != this.learningType || this.lastTrainingSet != trainingSet) {
            ...
            switch(this.learningType) {
                ...
                case BackPropagation:
                    this.training = new TrainFlatNetworkBackPropagation(this.flat, this.lastTrainingSet, this.getLearningRate(), this.getMomentum());
                    break;
            }
