...
public class ScrollbarTest {
    ...
    public static void main(String[] args) {
        ...
        final Color listForeground = display.getSystemColor(SWT.COLOR_LIST_FOREGROUND);
        final Color listBackground = display.getSystemColor(SWT.COLOR_LIST_BACKGROUND);
        shell.addListener(SWT.Paint, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                int end = Math.min(list.length - 1, start + rect.height / height);
                for (int i = start; i <= end; i++) {
                    gc.drawText(list[i], x + 2, (i - start) * height + 2);
                }
            }
        });
        shell.addListener(SWT.Resize, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                int width = 0;
                for (int i = 0; i < list.length; i++) {
                    width = Math.max(width, gc.textExtent(list[i]).x);
                }
                width += 4;
                ...
                gc.dispose();
                if (hSelection != hBar.getSelection() || vSelection != vBar.getSelection()) {
                    shell.redraw();
                }
            }
        });
