...
public abstract class GraphPinScene<N, E, P> extends ObjectScene {
    ...
    public final void removeNode(N node) {
        assert node != null && nodes.contains(node);
        for (P pin : new HashSet<P>(nodePins.get(node))) removePin(pin);
        nodes.remove(node);
    ...
    public final void removePin(P pin) {
        ...
        for (E edge : findPinEdges(pin, true, false)) setEdgeSource(edge, null);
        for (E edge : findPinEdges(pin, false, true)) setEdgeTarget(edge, null);
        pins.remove(pin);
    ...
    public final void setEdgeSource(E edge, P sourcePin) {
        assert edge != null && edges.contains(edge);
        if (sourcePin != null) assert pins.contains(sourcePin);
        P oldPin = edgeSourcePins.put(edge, sourcePin);
        if (GeomUtil.equals(oldPin, sourcePin)) return;
        if (oldPin != null) pinOutputEdges.get(oldPin).remove(edge);
        if (sourcePin != null) pinOutputEdges.get(sourcePin).add(edge);
    ...
    public final void setEdgeTarget(E edge, P targetPin) {
        assert edge != null && edges.contains(edge);
        if (targetPin != null) assert pins.contains(targetPin);
        P oldPin = edgeTargetPins.put(edge, targetPin);
        if (GeomUtil.equals(oldPin, targetPin)) return;
        if (oldPin != null) pinInputEdges.get(oldPin).remove(edge);
        if (targetPin != null) pinInputEdges.get(targetPin).add(edge);
    ...
    public final Collection<E> findPinEdges(P pin, boolean allowOutputEdges, boolean allowInputEdges) {
        ...
        if (allowInputEdges) list.addAll(pinInputEdges.get(pin));
        if (allowOutputEdges) list.addAll(pinOutputEdges.get(pin));
        return list;
    }
    ...
    public final Collection<E> findEdgesBetween(P sourcePin, P targetPin) {
        ...
        for (E edge : inputEdges) if (outputEdges.contains(edge)) list.add(edge);
        return list;
    }
