...
public class HTMLHistoryDumper implements HistoryDumper {
    ...
    protected void dumpHistory(PrintStream stream, Collection<HistoryItem> historyItems, int indentLevel) {
        ...
        for (HistoryItem item : historyItems) {
            for (int j = 0; j < SPACEPADDING * indentLevel; j++) stream.print(" ");
            if (item instanceof TermMacroHistoryItem) {
                stream.println("<ul>");
                dumpHistory(stream, (TermMacroHistoryItem) item, indentLevel + 1);
                stream.println("</ul>");
            } else stream.println("<li>" + getItemDesc(item, null));
        }
        stream.println("<ul>");
    }
    ...
    public String getLinkFromID(OBOSession history, String id) {
        if (generateLink) return "<a href='file:" + id + "'>" + id + " (" + id + ")" + "</a>"; else return id;
    }

    public String getLinkFromIDWithName(OBOSession history, String id, String name) {
        if (generateLink) return "<a href='file:" + id + "'>" + name + " (" + id + ")" + "</a>"; else return id;
    }

    public String getItemDesc(HistoryItem item, OBOSession history) {
        if (item instanceof CreateObjectHistoryItem) {
            return "Created object " + getLinkFromID(history, ((CreateObjectHistoryItem) item).getObjectID()) + " of type " + ((CreateObjectHistoryItem) item).getObjectType() + ".";
        } else if (item instanceof CreateLinkHistoryItem) {
            CreateLinkHistoryItem citem = (CreateLinkHistoryItem) item;
            StringBuffer out = new StringBuffer("");
            out.append("Copied ");
            out.append(getLinkFromIDWithName(history, citem.getTarget(), history.getObject(citem.getTarget()).getName()) + " (as " + citem.getTypeID() + "), ");
            out.append(" to ");
            ...
            out.append(getLinkFromIDWithName(history, tr.getChild(), history.getObject(tr.getChild()).getName()) + " (as " + tr.getType() + "), ");
            out.append(" to ");
            ...
            out.append("Deleted " + getLinkFromIDWithName(history, tr.getChild(), (tr.getChild() == null ? null : history.getObject(tr.getChild()).getName())) + " from " + getLinkFromIDWithName(history, tr.getParent(), (tr.getParent() == null ? null : history.getObject(tr.getParent()).getName())) + " with " + tr.getType());
            return out.toString();
