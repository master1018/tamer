...
@Scope("prototype")
public class EditReportGeneratorCommandImpl extends AbstractEditProjectOrDomainEntityCommand implements EditReportGeneratorCommand {
	 ...
	 */
    @Autowired
    public EditReportGeneratorCommandImpl(AssistantFacade assistantManager, UserRepository userRepository, ProjectRepository projectRepository, ProjectCommandFactory projectCommandFactory, AnnotationCommandFactory annotationCommandFactory, CommandHandler commandHandler) {
    ...
    public void execute() {
        ...
        try {
            ReportGenerator existing = getProjectRepository().findReportGeneratorByProjectOrDomainAndName(projectOrDomain, getName());
            if (reportGeneratorImpl == null) {
                throw EntityException.uniquenessConflict(ReportGenerator.class, existing, FIELD_NAME, EntityExceptionActionType.Creating);
            } else if (!existing.equals(reportGeneratorImpl)) {
                throw EntityException.uniquenessConflict(ReportGenerator.class, existing, FIELD_NAME, EntityExceptionActionType.Updating);
            }
        } catch (NoSuchEntityException e) {
        }
        if (reportGeneratorImpl == null) {
            reportGeneratorImpl = getProjectRepository().persist(new ReportGeneratorImpl(projectOrDomain, editedBy, getName(), getText()));
        } else {
