...
public class PFX extends ASN1Sequence implements java.io.Serializable {
    ...
    private void addMacData(char[] pwd) throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException, InvalidAlgorithmParameterException, ASN1Exception {
        ...
        try {
            ...
            AlgorithmIdentifier algId = new AlgorithmIdentifier(new ASN1ObjectIdentifier(SHA_OID), new ASN1Null());
    ...
    public boolean checkIntegrity(char[] pwd) throws NoSuchAlgorithmException, InvalidAlgorithmParameterException, InvalidKeyException, InvalidKeySpecException {
        byte[] dataToVerify;
        if (INTEGRITY_MODE_ != INTEGRITY_MODE_PASSWORD) throw new IllegalStateException("bad integrity mode (not password integrity)!");
        dataToVerify = macData_.getMacData().getDigest();
        PBEKeySpec keySpec = new PBEKeySpec(pwd);
        SecretKeyFactory skf = SecretKeyFactory.getInstance("PbeWithSHAAnd3_KeyTripleDES_CBC");
        SecretKey key = skf.generateSecret(keySpec);
        Mac hmac = Mac.getInstance("HmacSHA1");
        PBEParameterSpec params = new PBEParameterSpec(macData_.getSalt(), macData_.getIterationCount());
        ...
        for (int i = 0; i < macData.length; i++) {
            if ((macData[i] == dataToVerify[i]) && (value == true)) value = true; else value = false;
        }
        return value;
    }
     ...
     */
    public java.security.cert.X509Certificate checkIntegrity(X509Certificate cert) throws GeneralSecurityException {
        if (INTEGRITY_MODE_ != INTEGRITY_MODE_PUBLIC_KEY) throw new IllegalStateException("bad integrity mode (not password integrity)!");
    ...
    public void decode(Decoder dec) throws ASN1Exception, IOException {
        super.decode(dec);
        if (macData_ != null) INTEGRITY_MODE_ = INTEGRITY_MODE_PASSWORD; else if (authSafe_.getContent() instanceof SignedData) INTEGRITY_MODE_ = INTEGRITY_MODE_PUBLIC_KEY; else INTEGRITY_MODE_ = INTEGRITY_MODE_NONE;
    }
    ...
    public AuthenticatedSafe getAuthSafe() throws ASN1Exception, IllegalStateException {
        ...
        byte[] encodedData;
        if (authSafe_.getContent() instanceof Data) encodedData = ((Data) authSafe_.getContent()).getByteArray(); else {
            if (authSafe_.getContent() instanceof SignedData) {
                ASN1Type content = ((SignedData) authSafe_.getContent()).getContent();
                if (content instanceof Data) encodedData = ((Data) content).getByteArray(); else throw new IllegalStateException("unable to extract authSafe encoded data!");
            } else {
    ...
    public String toString() {
        ...
        if (macData_ == null) res = res + " No MacData\n"; else {
            try {
        ...
        if (INTEGRITY_MODE_ == INTEGRITY_MODE_NONE) res = res + "NO INTEGRITY PROTECTION\n"; else if (INTEGRITY_MODE_ == INTEGRITY_MODE_PASSWORD) res = res + "Integrity is guaranteed by password integrity mode\n"; else if (INTEGRITY_MODE_ == INTEGRITY_MODE_PUBLIC_KEY) res = res + "Integrity is guaranteed by public-key integrity mode\n";
        res = res + "}";
        return res;
    }
}
