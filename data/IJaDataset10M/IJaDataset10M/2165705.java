...
public class JobManagerCommandHandler extends CommandInterface {
        ...
        for (Job job : treeSet) {
            for (Range range : rangeList) {
                if (range.contains(job.getIndex())) {
                    env.add("job", job);
                    getJobManager().stopJob(job);
                    response.addComment(request.getSession().jprintf(_bundle, env, _keyPrefix + "removejob.success"));
                }
            }
        }
        return response;
    ...
    public CommandResponse doSTOPJOBS(CommandRequest request) {
        ...
        return new CommandResponse(200, "All JobTransfers will stop after their current transfer");
    }
