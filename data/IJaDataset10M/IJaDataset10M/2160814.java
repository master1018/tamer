...
public class RunXQTS extends FilterConsumer {
    ...
    public static final String XQTS_RESULT_NAMESPACE = "http://www.w3.org/2005/02/query-test-XQTSResult";

    static Object testSuiteResultElementType;

    static {
        NamespaceBinding namespaceNodes = new NamespaceBinding(null, XQTS_RESULT_NAMESPACE, new NamespaceBinding("q", XQuery.QEXO_FUNCTION_NAMESPACE, NamespaceBinding.predefinedXML));
        Symbol sym = Symbol.make(XQTS_RESULT_NAMESPACE, "test-suite-result", "");
        testSuiteResultElementType = new XName(sym, namespaceNodes);
    }

    static Object testRunElementType = Symbol.make(XQTS_RESULT_NAMESPACE, "test-run", "");

    static Object testSuiteElementType = Symbol.make(XQTS_RESULT_NAMESPACE, "test-suite", "");

    static Object testCaseElementType = Symbol.make(XQTS_RESULT_NAMESPACE, "test-case", "");
    ...
    private void writeQexoAttribute(String name, String value) {
        xqlog.startAttribute(Symbol.make(XQuery.QEXO_FUNCTION_NAMESPACE, name, "q"));
        xqlog.write(value);
        xqlog.endAttribute();
    }
    ...
    private void writeVerbose(String name, String value) {
        if (useComments) {
            ...
            xqlog.writeBreakFill();
            xqlog.endComment();
        } else writeQexoAttribute(name, value);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            try {
                ...
                XMLPrinter xqlog = new XMLPrinter(new BufferedOutputStream(new FileOutputStream(runner.logFileName)), FilePath.valueOf(runner.logFileName));
                runner.xqlog = xqlog;
    ...
    public RunXQTS(CharArrayOutPort out) {
        ...
        expectFailures("ST-WhereExpr001", "testsuite error (4024) - should not require error");
        expectFailures("K2-Literals-34", "MainModule without QueryBody");
        expectFailures("K2-NameTest-68", "depends on typing");
        expectFailures("ST-Axes001|ST-Axes002|ST-Axes003|ST-Axes004|ST-Axes005|" + "ST-Axes006|ST-Axes007|ST-Axes008|ST-Axes009|ST-Axes010|" + "ST-Axes011|ST-Axes012|ST-Axes013|ST-Axes014|ST-Axes015", "depends on static typing feature");
        expectFailures("fn-id-dtd-5|fn-id-dtd-7|fn-id-dtd-8|fn-id-dtd-9|" + "fn-id-dtd-12|fn-id-dtd-13|fn-id-dtd-15|fn-id-dtd-16|" + "fn-id-dtd-17|fn-id-dtd-18|fn-id-dtd-19|" + "fn-id-dtd-20|fn-id-dtd-21|fn-id-dtd-23|", "fn:id only works with xml:id so far");
        expectFailures("fn-idref-dtd-5|fn-idref-dtd-7|fn-idref-dtd-8|" + "fn-idref-dtd-9|fn-idref-dtd-12|fn-idref-dtd-13|" + "fn-idref-dtd-14|fn-idref-dtd-15|fn-idref-dtd-16|" + "fn-idref-dtd-17|fn-idref-dtd-18|fn-idref-dtd-19|" + "fn-idref-dtd-20|fn-idref-dtd-21|fn-idref-dtd-23|", "fn:idref doesn't do much yet");
        expectFailures("fn-normalize-unicode1args-1|" + "fn-normalize-unicode1args-2|" + "fn-normalize-unicode1args-3|" + "fn-normalize-unicode1args-4|" + "fn-normalize-unicode1args-5|" + "fn-normalize-unicode1args-6|" + "fn-normalize-unicode2args-1|" + "fn-normalize-unicode2args-2|" + "fn-normalize-unicode2args-3|" + "fn-normalize-unicode-1|" + "fn-normalize-unicode-3|fn-normalize-unicode-4|" + "fn-normalize-unicode-5|fn-normalize-unicode-6|" + "fn-normalize-unicode-7|K-NormalizeUnicodeFunc-4|" + "K-NormalizeUnicodeFunc-5|K-NormalizeUnicodeFunc-6|" + "K-NormalizeUnicodeFunc-7|K-NormalizeUnicodeFunc-8|" + "K-NormalizeUnicodeFunc-11|K-NormalizeUnicodeFunc-12", "fn:normalize-unicode not unimplemented yet");
        expectFailures("vardeclerr|K-InternalVariablesWith-17|K-InternalVariablesWith-18", "missing check for circular definitions");
        expectFailures("K-TimeAddDTD-1|K-TimeAddDTD-2|K-TimeSubtractDTD-1", "bad interaction between fields and millis");
        expectFailures("op-time-greater-than-2", "comparing xs:time doesn't handle differing timezones");
        expectFailures("K-SubstringBeforeFunc-5|K-SubstringAfterFunc-5|" + "K-ContainsFunc-5|K-StartsWithFunc-5|K-EndsWithFunc-5", "some string functions don't support collation argument");
        expectFailures("caselessmatch04", "regex/unicode special case");
        expectFailures("string-queries-results-q4|K2-FunctionProlog-7|K2-FunctionProlog-17|K2-FunctionProlog-18|K2-FunctionProlog-19|K2-FunctionProlog-22|XMark-Q18|XMark-All", "function conversion incorrect for user-defined functions");
        expectFailures("caselessmatch10|caselessmatch11", "regex range subtraction not implemented");
    }
    ...
    private void expectFailures(String testNames, String reason) {
        while (testNames != null) {
            ...
            if (testName.length() > 0) expectedFailures.put(testName, reason);
        }
    }

    public void startElement(Object type) {
        if (inStartTag) handleStartTag();
        attributes.clear();
        inStartTag = true;
    ...
    boolean tagMatches(String localName) {
        if (localName.equals(currentElementSymbol.getLocalName())) return true;
        return false;
    }
    ...
    void report(String result, String comment) {
        ...
        if (failExpected == null) {
            if (failed) {
                ...
                failCount++;
            } else if ("cannot tell".equals(result)) cannotTellCount++; else passCount++;
        } else {
            if (failed) xfailCount++; else {
        ...
        if (comment != null) writeAttribute("comment", comment);
    }

    public void evalTest(String testName) throws Throwable {
        ...
        String filename = directory + '/' + XQueryQueryOffsetPath + testFilePath + testQueryName + XQueryFileExtension;
        InPort in;
        expectedErrors = expectedErrorsBuf.toString();
        try {
            in = InPort.openFile(filename);
        } catch (java.io.FileNotFoundException ex) {
            String xfilename = directory + '/' + XQueryXQueryOffsetPath + testFilePath + testQueryName + XQueryXFileExtension;
        ...
        try {
            if (contextItem != null) {
                ...
                comp = null;
            } else comp = xqueryLanguage.parse(in, messages, Language.PARSE_IMMEDIATE);
            if (messages.seenErrors()) throw new SyntaxException(messages);
            ...
            if (expectedErrors.indexOf(errorString) >= 0) {
                ...
                if (failExpected == null) failExpected = "'import schema' not implemented";
                report("fail", null);
            } else if (error.message != null && error.message.indexOf("unknown type xs:NOTATION") >= 0 && (expectedErrors.equals("|XPST0080|") || expectedErrors.equals("|XPST0017|"))) {
                report("fail", null);
            } else if (error.message != null && error.message.indexOf("unknown type xs:ENTITY") >= 0 && (expectedErrors.equals("|XPTY0004|"))) {
                report("fail", null);
            } else if (error.message != null && error.message.indexOf("unknown function") >= 0 && (expectedErrors.equals("|XPDY0002|") || expectedErrors.equals("|XPTY0004|") || expectedErrors.equals("|XQDY0025|") || expectedErrors.equals("|FODC0001|") || (expectedErrors.equals("|XPST0017|") && (error.message.endsWith(" fn:id") || error.message.endsWith(" fn:idref"))))) {
                report("fail", null);
            } else if (expectedErrors.length() > 1) report("pass", "static error: " + error + " expected:" + expectedErrors); else report("fail", "static error: " + error.message);
            return;
        }
        in.close();
        ...
        ctx.consumer = xout;
        try {
            if (contextItem != null) ctx.runUntilDone(); else ModuleExp.evalModule(env, ctx, comp, null, null);
        } catch (OutOfMemoryError ex) {
            System.err.println("caught " + ex);
            System.exit(-1);
        } catch (Throwable ex) {
            if (ex instanceof NumberFormatException && expectedErrors.indexOf("|FORG0001|") >= 0) report("pass", "caught NumberFormatException expected:" + expectedErrors); else if (ex instanceof ClassCastException && (expectedErrors.indexOf("|XPTY0004|") >= 0 || expectedErrors.indexOf("|XPTY0020|") >= 0 || expectedErrors.indexOf("|FORG0001|") >= 0 || expectedErrors.indexOf("|FOAR0002|") >= 0)) report("pass", "caught ClassCastException expected:" + expectedErrors); else if (expectedErrors.length() > 1) report("pass", "caught " + ex + " expected:" + expectedErrors); else {
        ...
        if (messages.seenErrors()) {
            if (expectedErrors.length() > 1) report("pass", "error: " + messages.getErrors() + " expected: " + expectedErrors); else report("fail", "error: " + messages.getErrors());
            return;
        }
        ...
        for (int ialt = 0; ialt < numOutputFileAlts; ialt++) {
            ...
            String outname = directory + '/' + ResultOffsetPath + testFilePath + outputFileAlts.elementAt(ialt);
        ...
        if (!foundMatchingOutput) {
            if (expectedErrors.length() > 1) {
                ...
                report("fail", null);
                if (verbose && expectedFailures.get(testName) == null) displayDifference = true;
            }
        }
    ...
    private static int grabAttribute(String str, int start) {
        ...
        for (int i = start; ; ) {
            ...
            if (inAttr == 0 && (ch == '\"' || ch == '\'')) inAttr = ch; else if (ch == inAttr) return i;
        }
    }

    public static boolean matches(String arg1, String arg2, String compare) {
        ...
        for (; ; ) {
            ...
            if ((c1 == '&' && arg1.charAt(i1 + 1) == '#' && i1 + 3 < len1) || (c2 == '&' && arg2.charAt(i2 + 1) == '#' && i2 + 3 < len2)) {
                try {
                    if (c1 == '&') {
                        int base1 = 10;
                        i1 += 2;
                        ...
                        i1 = semi1;
                    } else if (c1 >= 0xD800 && c1 < 0xDC00 && i1 + 1 < len1) c1 = (c1 - 0xD800) * 0x400 + (arg1.charAt(++i1) - 0xDC00) + 0x10000;
                    if (c2 == '&') {
                        int base2 = 10;
                        i2 += 2;
                        if (arg2.charAt(i2) == 'x') {
                        ...
                        i2 = semi2;
                    } else if (c2 >= 0xD800 && c2 < 0xDC00 && i2 + 1 < len2) c2 = (c2 - 0xD800) * 0x400 + (arg2.charAt(++i2) - 0xDC00) + 0x10000;
                } catch (Throwable ex) {
                    return false;
                }
                ...
                if (c1 == '<' && isXML) {
                    ...
                    intag = false;
                } else if (intag && Character.isWhitespace((char) c1) && inAttr == 0) {
                    start_attr1 = i1 + 1;
                    start_attr2 = i2 + 1;
                    ;
                ...
                for (; ; ) {
                    ...
                    for (; ; ) {
                        ...
                        c1 = arg1.charAt(i1++);
                        if (!Character.isWhitespace((char) c1)) break;
                    }
                    for (; ; ) {
                        if (i2 >= len2) return false;
                        c2 = arg2.charAt(i2++);
                        if (!Character.isWhitespace((char) c2)) break;
                    }
                    boolean done1 = c1 == '/' || c1 == '>';
                    boolean done2 = c2 == '/' || c2 == '>';
                ...
                for (int i = attrs1.size(); --i >= 0; ) {
                    ...
                    for (int j = attrs2.size(); ; ) {
                        ...
                        if (attr1.equals(attr2)) break;
                    }
                }
                start_attr1 = 0;
                start_attr2 = 0;
                intag = false;
            } else if (isFloatChar(c1) ? (isFloatChar(c2) || (i2 > 0 && isFloatChar(arg2.charAt(i2 - 1)))) : (isFloatChar(c2) && (i1 > 0 && isFloatChar(arg1.charAt(i1 - 1))))) {
                int start1 = i1, start2 = i2;
                while (start1 > 0 && isFloatChar(arg1.charAt(start1 - 1))) start1--;
                while (start2 > 0 && isFloatChar(arg2.charAt(start2 - 1))) start2--;
                int end1 = i1, end2 = i2;
                while (end1 < len1 && isFloatChar(arg1.charAt(end1))) end1++;
                while (end2 < len2 && isFloatChar(arg2.charAt(end2))) end2++;
                ...
                try {
                    ...
                    if (Float.floatToIntBits(f1) != Float.floatToIntBits(f2)) return false;
                } catch (Throwable ex) {
                    return false;
                }
                i1 = end1;
                ...
                i2++;
            } else if (isXML && c1 == '/' && c2 == '>' && i1 + 1 < len1 && i2 + 2 < len2 && arg1.charAt(i1 + 1) == '>' && arg2.charAt(i2 + 1) == '<' && arg2.charAt(i2 + 2) == '/') {
                for (i2 = i2 + 3; ; i2++) {
                    if (i2 >= len2) return false;
                    char c = arg2.charAt(i2);
                    if (c == '>') break;
                    if (!XName.isNamePart(c)) return false;
                }
                i1 = i1 + 2;
                i2 = i2 + 1;
            } else return false;
    ...
    boolean isSelected(String testName) {
        if ("Constr-cont-document-3".equals(testName)) return false;
    ...
    public void endElement() {
        ...
        if (tagMatches("test-case")) {
            ...
            contextItem = null;
            Environment env = Environment.getCurrent();
            while (!externalVariablesSet.empty()) env.remove((Symbol) externalVariablesSet.pop());
            ...
            if (isSelected(testName)) {
                ...
                String filename = directory + '/' + XQueryQueryOffsetPath + testFilePath + name + XQueryFileExtension;
                InPort in;
                try {
                    in = InPort.openFile(filename);
            ...
            if ("userdefined".equals(inputFile)) inputFile = "emptydoc";
            ...
            try {
                ...
                if (symbol != null) Environment.getCurrent().define(symbol, null, value); else contextItem = value;
            } catch (Throwable ex) {
                System.err.println("caught " + ex);
            ...
            String path = inputValue instanceof Values ? "collection:" + inputFile : "file://" + directory + '/' + inputValue;
            Symbol symbol = Symbol.parse(variable);
            externalVariablesSet.push(symbol);
            ...
            NodeUtils.setSavedCollection("collection:" + collectionID, collectionDocuments.canonicalize());
            collectionID = null;
            ...
            if (collectionID == null) throw new Error("<input-document> not in <collection>");
            try {
            ...
            String mclass = Compilation.mangleURI(uri) + '.' + XQuery.makeClassName(mpath);
    ...
    public void endAttribute() {
        ...
        String qname = (prefix == null || prefix.length() == 0 ? local : prefix + ":" + local);
        cout.setLength(attrValueStart);
        attributes.addAttribute(uri, local, qname, "CDATA", attrValue);
    }

    public void beforeContent() {
        if (!inAttribute && inStartTag) handleStartTag();
    }
}
