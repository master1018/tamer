...
class MulUnit extends Unit implements Externalizable {
    ...
    MulUnit(Unit unit1, int power1, Unit unit2, int power2) {
        ...
        if (power1 == 1) factor = unit1.factor; else factor = Math.pow(unit1.factor, (double) power1);
        if (power2 < 0) {
            for (int i = -power2; --i >= 0; ) factor /= unit2.factor;
        } else {
            for (int i = power2; --i >= 0; ) factor *= unit2.factor;
        }
        next = unit1.products;
    ...
    public Unit sqrt() {
        if ((power1 & 1) == 0 && (power2 & 1) == 0) return times(unit1, power1 >> 1, unit2, power2 >> 1);
        return super.sqrt();
    }

    static MulUnit lookup(Unit unit1, int power1, Unit unit2, int power2) {
        for (MulUnit u = unit1.products; u != null; u = u.next) {
            if (u.unit1 == unit1 && u.unit2 == unit2 && u.power1 == power1 && u.power2 == power2) return u;
        }
        return null;
    }
