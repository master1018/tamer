...
final class AntTopicMatcher extends AbstractTopicMatcher implements Serializable {
    ...
    public boolean equals(Object o) {
        return o instanceof AntTopicMatcher && ((AntTopicMatcher) o).pattern.equals(pattern);
    }
    ...
    private static boolean doMatch(String pattern, String path, boolean fullMatch) {
        if (path.startsWith(DEFAULT_PATH_SEPARATOR) != pattern.startsWith(DEFAULT_PATH_SEPARATOR)) return false;
        ...
        if (pathIdxStart > pathIdxEnd) {
            if (pattIdxStart > pattIdxEnd) return pattern.endsWith(DEFAULT_PATH_SEPARATOR) ? path.endsWith(DEFAULT_PATH_SEPARATOR) : !path.endsWith(DEFAULT_PATH_SEPARATOR);
            if (!fullMatch) return true;
            if (pattIdxStart == pattIdxEnd && pattDirs[pattIdxStart].equals("*") && path.endsWith(DEFAULT_PATH_SEPARATOR)) return true;
            for (int i = pattIdxStart; i <= pattIdxEnd; i++) if (!pattDirs[i].equals("**")) return false;
            return true;
        } else if (pattIdxStart > pattIdxEnd) return false; else if (!fullMatch && "**".equals(pattDirs[pattIdxStart])) return true;
        while (pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
