...
public class XMLPropertyUtil {
    ...
    public static XMLPropertyUtil getInstance() {
        if (null == ourInstance) ourInstance = new XMLPropertyUtil();
        return ourInstance;
    }
	 ...
	 */
    public Method getWriteMethod(Class clazz, String name) throws LTSException {
	 ...
	 * @throws LTSException If a problem occurs.
	 */
    public void writeProperty(boolean throwException, Object o, Method method, Object value) throws LTSException {
        try {
            Object[] data = { value };
            method.invoke(o, data);
        } catch (Exception e) {
            if (throwException) throw new LTSException(e);
        }
    }
    ...
    public Object convertValue(Class clazz, String value) throws LTSException {
        try {
            ...
            Object result;
            if (clazz == String.class) result = value; else if (clazz == Integer.class || clazz == Integer.TYPE) result = new Integer(value); else if (clazz == Short.class || clazz == Short.TYPE) result = new Short(value); else if (clazz == Long.class || clazz == Long.TYPE) result = new Long(value); else if (clazz == Double.class || clazz == Double.TYPE) result = new Double(value); else if (clazz == Float.class || clazz == Float.TYPE) result = new Float(value); else if (clazz == Byte.class || clazz == Byte.TYPE) result = new Byte(value); else if (clazz == Boolean.class || clazz == Boolean.TYPE) result = new Boolean(value); else if (clazz == Character.class || clazz == Character.TYPE) result = new Character(value.charAt(0)); else {
                String msg = "Unsupported type: " + clazz.getName();
                throw new LTSException(msg);
            }
            return result;
        } catch (NumberFormatException e) {
            String msg = "Error converting the value, " + value + " to an instance of " + clazz.getName();
    ...
    public void setProperty(boolean throwException, Object o, String name, String value) throws LTSException {
        ...
        if (null == prop) {
            ...
            String msg = "The class, " + clazz.getName() + ", does not have a property " + "named " + name;
            throw new LTSException(msg);
        }
        Method method = prop.getWriteMethod();
        if (null == method) {
            if (!throwException) return;
            String msg = "Property " + name + " is not writeable for class " + clazz.getName();
            throw new LTSException(msg);
        }
        try {
