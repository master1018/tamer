package com.memomics.cytoscape_plugin.actions;
...
public class DeleteEdit extends CyAbstractEdit {
    ...
    public DeleteEdit(CyNetwork net, int[] nodeInd, int[] edgeInd) {
        super("Delete");
        if (net == null) throw new IllegalArgumentException("network is null");
        this.net = net;
        nodes = new int[nodeInd.length];
        for (int i = 0; i < nodeInd.length; i++) nodes[i] = nodeInd[i];
        edges = new int[edgeInd.length];
        for (int i = 0; i < edgeInd.length; i++) edges[i] = edgeInd[i];
