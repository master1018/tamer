...
public class WrapperSpace extends AbstractSpace {
    ...
    public Cell translate(Cell c) {
        ...
        if (c.id.startsWith("home-id:") && prefix.equals("home-id:")) return new Cell(this, c.id);
        return new Cell(this, prefix + c.id);
    }

    public String detranslate(String id) {
        if (id.startsWith("home-id:") && !prefix.equals("home-id:")) return id;
        if (!id.startsWith(prefix)) throw new ZZImpossibleCellException("Cell id " + id + " does not have the" + " correct prefix");
        return id.substring(prefix.length());
    }

    public Cell detranslate(Cell c) {
        if (c.space != this) throw new IllegalArgumentException("Cell " + c + " is not from the" + " correct space");
    ...
    public Cell getHomeCell() {
        return translate(base.getHomeCell());
    }
    ...
    public Cell N(Cell c, Dim dim, int dir, Obs o) {
        return translate(base.N(detranslate(c), ((WrapperDim) dim).base, dir, o));
    }

    public void delete(Cell c) {
        base.delete(detranslate(c));
    }

    public void setSpan(Cell c, Span s) {
        base.setSpan(detranslate(c), s);
    }
