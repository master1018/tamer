...
public class WorkbenchControlPanel extends PopupDialog {
    ...
    public WorkbenchControlPanel(WebGraph g, WebOutline o) {
        ...
        if (g != null) switch(g.defaultFilter) {
        ...
        if (g != null) automatic.setState(g.getAutomaticLayout()); else g.disable();
        Panel panel;
    ...
    public boolean handleEvent(Event event) {
        if (event.id == Event.ACTION_EVENT) {
            if (event.target == applyButton) writeBack(); else if (event.target == okButton) {
                writeBack();
                close();
            } else if (event.target == cancelButton) close(); else return super.handleEvent(event);
        } else if (event.id == Event.WINDOW_DESTROY) dispose(); else return super.handleEvent(event);
        return true;
    }
}
