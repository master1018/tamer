...
public class SynonymTypeManager extends AbstractGUIComponent {
    ...
    protected static final Logger logger = Logger.getLogger(SynonymTypeManager.class);
    ...
    protected JLabel noTypeLabel = new JLabel("Select type to edit");

    protected JButton commitButton = new JButton("Save Changes");

    protected JButton revertButton = new JButton("Revert");

    protected String[] scopes = { "<unspecified scope>", "Synonym", "Exact Synonym", "Narrow Synonym", "Broad Synonym" };
    ...
    private class TypeEditor extends JPanel implements GenericEditorComponent {
        ...
        public Object createNewValue() {
            return new TypeWrapper(new SynonymTypeImpl("NEWSYNTYPE", "<new synonym type>"));
        }
    }
    ...
    public void init() {
        ...
        typeList = new ListEditor(new TypeEditor(), noTypeLabel, new Vector<Object>(), true, true, true, true, false);
    ...
    protected static class TypeWrapper {
        ...
        public boolean isChanged() {
            return !oldtype.getID().equals(id) || !oldtype.getName().equals(name) || oldtype.getScope() != scope;
        }
    ...
    protected void saveTypes() {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (names.contains(rtw.getID())) {
                JOptionPane.showMessageDialog(this, "Could not commit changes " + "because multiple categories " + "are using the id " + rtw.getName());
                return;
            }
            if (!OBOConstants.isOBOIdentifierToken(rtw.getID())) {
                JOptionPane.showMessageDialog(this, "Illegal characters in synonym " + "category id \"" + rtw.getID() + "\".\nSynonym category ids may only " + "contain letters,\ndigits, underscores, and dashes.");
                return;
            }
            v.add(rtw.getSynType());
            names.add(rtw.getID());
        }
        Vector<SynonymType> oldtypes = new Vector<SynonymType>(SessionManager.getManager().getSession().getSynonymTypes());
        ...
        for (int i = 0; i < oldtypes.size(); i++) {
            ...
            for (int j = 0; j < data.size(); j++) {
                ...
                if (tw.getSynType() == syntype) {
                    ...
                    newtypes.remove(tw);
                    if (tw.isChanged()) {
                        SynonymTypeHistoryItem typeitem = new SynonymTypeHistoryItem(tw.getSynType(), new SynonymTypeImpl(tw.getID(), tw.getName(), tw.getScope()), false, false);
            ...
            if (!found) {
                Iterator<IdentifiedObject> it = SessionManager.getManager().getSession().getObjects().iterator();
                while (it.hasNext()) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        if (s.getSynonymType() != null) {
                            if (s.getSynonymType().equals(syntype)) {
                                JOptionPane.showMessageDialog(this, "Could not commit changes " + "because " + syntype + " " + "is still in use in the " + "ontology");
                                return;
                            }
                        }
                    }
                }
                SynonymTypeHistoryItem typeitem = new SynonymTypeHistoryItem(syntype, null, false, true);
                item.addItem(typeitem);
            }
        }
        for (int i = 0; i < newtypes.size(); i++) {
            TypeWrapper tw = (TypeWrapper) newtypes.get(i);
            SynonymTypeHistoryItem typeitem = new SynonymTypeHistoryItem(null, SessionManager.getManager().getSession().getObjectFactory().createSynonymType(tw.getID(), tw.getName(), tw.getScope()), true, false);
            item.addItem(typeitem);
        }
        GUIUtil.setSelections(item, SelectionManager.getGlobalSelection(), SelectionManager.getGlobalSelection());
