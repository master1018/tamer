...
public class struct {
    ...
    static class FormatDef {
        ...
        int doPack(ByteStream buf, int count, int pos, PyObject[] args) {
            if (pos + count > args.length) throw StructError("insufficient arguments to pack");
            int cnt = count;
            while (count-- > 0) pack(buf, args[pos++]);
            return cnt;
        }

        void doUnpack(ByteStream buf, int count, PyList list) {
            while (count-- > 0) list.append(Py.java2py(unpack(buf)));
        }
        ...
        long get_long(PyObject value) {
            if (value instanceof PyLong) {
                ...
                if (v == Py.NoConversion) throw Py.OverflowError("long int too long to convert");
        ...
        double get_float(PyObject value) {
            if (!(value instanceof PyFloat)) throw StructError("required argument is not an float");
    ...
    static class PadFormatDef extends FormatDef {
        ...
        int doPack(ByteStream buf, int count, int pos, PyObject[] args) {
            while (count-- > 0) buf.writeByte(0);
            return 0;
        }

        void doUnpack(ByteStream buf, int count, PyList list) {
            while (count-- > 0) buf.readByte();
        }
    }
    ...
    static class StringFormatDef extends FormatDef {
        ...
        int doPack(ByteStream buf, int count, int pos, PyObject[] args) {
            ...
            if (!(value instanceof PyString)) throw StructError("argument for 's' must be a string");
            String s = value.toString();
            int len = s.length();
            buf.writeString(s, 0, Math.min(count, len));
            if (len < count) {
                count -= len;
                for (int i = 0; i < count; i++) buf.writeByte(0);
            }
            return 1;
        }
    ...
    static class PascalStringFormatDef extends StringFormatDef {
        ...
        int doPack(ByteStream buf, int count, int pos, PyObject[] args) {
            PyObject value = args[pos];
            if (!(value instanceof PyString)) throw StructError("argument for 'p' must be a string");
    ...
    static class CharFormatDef extends FormatDef {
        ...
        void pack(ByteStream buf, PyObject value) {
            if (!(value instanceof PyString) || value.__len__() != 1) throw StructError("char format require string of length 1");
    ...
    public static String pack(PyObject[] args) {
        if (args.length < 1) Py.TypeError("illegal argument type for built-in operation");
        ...
        for (int j = 0; j < len; j++) {
            ...
            if (Character.isDigit(c)) {
                num = Character.digit(c, 10);
                while (++j < len && Character.isDigit((c = format.charAt(j)))) num = num * 10 + Character.digit(c, 10);
                if (j >= len) break;
            }
            FormatDef e = getentry(c, f);
    ...
    public static PyTuple unpack(String format, String string) {
        ...
        for (int j = 0; j < flen; j++) {
            ...
            if (Character.isDigit(c)) {
                num = Character.digit(c, 10);
                while (++j < flen && Character.isDigit((c = format.charAt(j)))) num = num * 10 + Character.digit(c, 10);
                if (j > flen) break;
            }
            FormatDef e = getentry(c, f);
