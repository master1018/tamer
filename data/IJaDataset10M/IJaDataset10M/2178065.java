...
public class InsertRecordGroup {
    ...
    public int getSize() {
        ...
        for (InsertRecord ir : records) size += ir.getSize();
        return size;
    }

    public void serialize(ReusableBuffer buffer) {
        buffer.putInt(databaseId);
        for (InsertRecord ir : records) ir.serialize(buffer);
    }
    ...
    public static class InsertRecord {
        ...
        public int getSize() {
            if (value != null) return Byte.SIZE / 8 + Integer.SIZE / 8 * 2 + key.length + value.length; else return Byte.SIZE / 8 + Integer.SIZE / 8 * 2 + key.length;
        }
    ...
    public String toString() {
        ...
        for (InsertRecord record : records) sb.append(record + "\n");
        return sb.toString();
    }
}
