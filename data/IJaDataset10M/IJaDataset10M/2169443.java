...
public class OrmTable<UJO_IMPL extends Ujo> extends QuickUjo implements ExtendedOrmUjo<UJO_IMPL> {
    ...
    public Object readValue(final UjoProperty property) {
        ...
        if (property.isTypeOf(OrmUjo.class)) {
            ...
            } else if (result != null && mySession != null && mySession != ((OrmUjo) result).readSession()) {
                ((OrmUjo) result).writeSession(mySession);
            }
        } else if (property instanceof RelationToMany && mySession != null && mySession.getHandler().isPersistent(property)) {
    ...
    public UjoProperty[] readChangedProperties(boolean clear) {
        final UjoProperty[] result = changes == null || changes.isEmpty() ? UjoPropertyListImpl.EMPTY : changes.toArray(new UjoProperty[changes.size()]);
        if (clear) {
            changes = null;
        }
        return result;
     ...
     */
    @SuppressWarnings({ "unchecked" })
    public final <UJO extends UJO_IMPL, VALUE> UJO_IMPL set(final UjoProperty<UJO, VALUE> property, final VALUE value) {
    ...
    public <UJO extends UJO_IMPL> ForeignKey readFK(UjoProperty<UJO, ? extends OrmUjo> property) throws IllegalStateException {
        ...
        if (session != null) {
            final OrmUjo ujo = value instanceof OrmUjo ? (OrmUjo) value : this;
