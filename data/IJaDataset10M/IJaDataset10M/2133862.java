...
public class DistributionTourFactory implements TourFactory {
    ...
    private double getMarginalInsertionCosts(TourActivity act_i, TourActivity act_j, TourActivity newAct) {
        double tt_acti2newAct = getTravelTime(act_i.getLocationId(), newAct.getLocationId(), act_i.getEarliestArrTime() + act_i.getServiceTime());
        double earliestArrTimeAtNewAct = act_i.getEarliestArrTime() + act_i.getServiceTime() + tt_acti2newAct;
        double marginalCost = getGeneralizedCosts(act_i.getLocationId(), newAct.getLocationId(), act_i.getEarliestArrTime() + act_i.getServiceTime()) + getGeneralizedCosts(newAct.getLocationId(), act_j.getLocationId(), earliestArrTimeAtNewAct + newAct.getServiceTime()) - getGeneralizedCosts(act_i.getLocationId(), act_j.getLocationId(), act_i.getEarliestArrTime() + act_i.getServiceTime());
        return marginalCost;
    }
