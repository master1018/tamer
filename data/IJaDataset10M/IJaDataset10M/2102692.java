...
import de.bea.domingo.util.GregorianDateRange;
...
import de.bea.domingo.util.GregorianTimeRange;
...
public final class InternationalProxy implements Serializable {
    ...
    protected DateTime createDateTime(final Calendar calendar) {
        ...
        try {
            ...
            if (isTimeOnly(correct)) {
                dateTime.setAnyDate();
            } else if (isDateOnly(correct)) {
                dateTime.setAnyTime();
            }
            return dateTime;
    ...
    /**
     * Converts a Notes date range to a gregorian date range.
     *
     * @param dateRange a Notes DateRange object
     * @return a DDateRange object
     */
    public DDateRange createDateRange(final DateRange dateRange) {
        try {
            Calendar from = createCalendar(dateRange.getStartDateTime());
            Calendar to = createCalendar(dateRange.getStartDateTime());
            if (isDateOnly(from)) {
                return new GregorianDateRange(from, to);
            } else if (isTimeOnly(from)) {
                return new GregorianTimeRange(from, to);
            } else {
                return new GregorianDateTimeRange(from, to);
            }
        } catch (NotesException e) {
            getMonitor().error("Cannot convert date range", e);
            return null;
        }
    }
    ...
    /**
     * Checks if a DateTime object only has a time but not a date.
     *
     * @param dateTime a Notes DateTime object
     * @return <code>true</code> if a date is defined, else <code>false</code>
     */
    protected boolean isTimeOnly(final DateTime dateTime) {
        try {
    ...
    /**
     * Checks if a DateTime object only has a date but not a time.
     *
     * @param dateTime a Notes DateTime object
     * @return <code>true</code> if a time is defined, else <code>false</code>
     */
    protected boolean isDateOnly(final DateTime dateTime) {
        try {
    ...
    /**
     * Checks if a Calendar object only defines a time but not a date.
     *
     * @param calendar a Calendar object
     * @return <code>true</code> if a time is defined, else <code>false</code>
     */
    protected boolean isTimeOnly(final Calendar calendar) {
        return calendar instanceof GregorianTime || (!calendar.isSet(Calendar.YEAR) && !calendar.isSet(Calendar.MONTH) && !calendar.isSet(Calendar.DATE));
    }

    /**
     * Checks if a Calendar object only defines a date but not a time.
     *
     * @param calendar a Calendar object
     * @return <code>true</code> if a date is defined, else <code>false</code>
     */
    protected boolean isDateOnly(final Calendar calendar) {
        return calendar instanceof GregorianDate || (!calendar.isSet(Calendar.HOUR_OF_DAY) && !calendar.isSet(Calendar.MINUTE) && !calendar.isSet(Calendar.SECOND));
    }

    /**
     * Converts a Notes DateTime into a <code>java.util.Date</code> without
     * respecxtlo locale settings like time zones. For invalid dates (e.g. no
     * date- and no time-component available) a null value is returned.
     *
     * @param dateTime a Notes DateTime object
