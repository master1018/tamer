...
public class DynamicTransfo {
    ...
    public void setId(String id) throws IOException {
        ...
        if (byShow) {
            ...
            if (lenom.charAt(1) != '0' && lenom.charAt(1) != '2') lenom.setCharAt(1, '0');
        } else {
            if (temp.length > 1) lenom = new StringBuffer(temp[1]); else {
                lenom = new StringBuffer(dec[3]);
        ...
        if (nbClasse != meansTransformationVectorPool.size()) new Error("nbclasse " + nbClasse + " " + meansTransformationVectorPool.size());
        ...
        for (int i = 0; i < nbClasse; i++) {
            ...
            for (int lig = 0; lig < veclen; lig++) for (int col = 0; col < veclen; col++) mat[lig][col] = s.nextFloat();
            for (int col = 0; col < veclen; col++) vect[col] = s.nextFloat();
        }
        for (int i = 0; i < senonePool.size(); i++) ((Senone) senonePool.get(i)).resetTransfo();
        s.close();
    }
}
