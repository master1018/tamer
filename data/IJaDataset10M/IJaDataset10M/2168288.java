package reconcile.weka.gui.graphvisualizer;
...
public class DotParser implements GraphConstants {
   ...
   */
        ...
        try {
            ...
            if (tk.ttype == tk.TT_WORD) {
                if (tk.sval.equalsIgnoreCase("digraph")) {
                    ...
                    stmtList(tk);
                } else if (tk.sval.equalsIgnoreCase("graph")) System.err.println("Error. Undirected graphs cannot be used"); else System.err.println("Error. Expected graph or digraph at line " + tk.lineno());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    protected void stmtList(StreamTokenizer tk) throws Exception {
        tk.nextToken();
        if (tk.ttype == '}' || tk.ttype == tk.TT_EOF) return; else {
            stmt(tk);
            stmtList(tk);
        }
    }
    ...
    protected void stmt(StreamTokenizer tk) {
        if (tk.sval.equalsIgnoreCase("graph") || tk.sval.equalsIgnoreCase("node") || tk.sval.equalsIgnoreCase("edge")) ; else {
            try {
                nodeID(tk);
                int nodeindex = m_nodes.indexOf(new GraphNode(tk.sval, null));
                tk.nextToken();
                if (tk.ttype == '[') nodeStmt(tk, nodeindex); else if (tk.ttype == '-') edgeStmt(tk, nodeindex); else System.err.println("error at lineno " + tk.lineno() + " in stmt");
            } catch (Exception ex) {
    ...
    protected void nodeStmt(StreamTokenizer tk, final int nindex) throws Exception {
        ...
        if (tk.ttype == ']' || tk.ttype == tk.TT_EOF) return; else if (tk.ttype == tk.TT_WORD) {
            if (tk.sval.equalsIgnoreCase("label")) {
                tk.nextToken();
                if (tk.ttype == '=') {
                    tk.nextToken();
                    if (tk.ttype == tk.TT_WORD || tk.ttype == '"') temp.lbl = tk.sval; else {
                ...
                if (tk.ttype == '=') {
                    tk.nextToken();
                    if (tk.ttype == tk.TT_WORD || tk.ttype == '"') ; else {
    ...
    protected void edgeStmt(StreamTokenizer tk, final int nindex) throws Exception {
        ...
        tk.nextToken();
        if (tk.ttype == '[') edgeAttrib(tk, e); else tk.pushBack();
    }
    ...
    protected void edgeAttrib(StreamTokenizer tk, final GraphEdge e) throws Exception {
        ...
        if (tk.ttype == ']' || tk.ttype == tk.TT_EOF) return; else if (tk.ttype == tk.TT_WORD) {
            if (tk.sval.equalsIgnoreCase("label")) {
                ...
                if (tk.ttype == '=') {
                    tk.nextToken();
                    if (tk.ttype == tk.TT_WORD || tk.ttype == '"') System.err.println("found label " + tk.sval); else {
                        System.err.println("couldn't find label at line " + tk.lineno());
                        tk.pushBack();
                    }
                } else {
