...
public class GenRace extends StdRace {
    ...
    public int[] getAgingChart() {
        if (agingChart == null) agingChart = (int[]) super.getAgingChart().clone();
        return agingChart;
    }
    ...
    public Weapon myNaturalWeapon() {
        if (weaponBuddy != null) return weaponBuddy.myNaturalWeapon();
        if (naturalWeapon != null) return naturalWeapon;
    ...
    public String healthText(MOB viewer, MOB mob) {
        if ((healthBuddy != null) && (healthBuddy != this)) return healthBuddy.healthText(viewer, mob);
    ...
    public void affectCharStats(MOB affectedMob, CharStats affectableStats) {
        if (adjStats != null) for (int i : CharStats.CODES.ALL()) affectableStats.setStat(i, affectableStats.getStat(i) + adjStats.getStat(i));
        if (setStats != null) for (int i : CharStats.CODES.ALL()) if (setStats.getStat(i) != 0) affectableStats.setRacialStat(i, setStats.getStat(i));
    }
    ...
    public Vector myResources() {
        if (resourceChoices == null) return new Vector();
    ...
    protected String getRaceLocatorID(Race R) {
        ...
        if (R == CMClass.getRace(R.ID())) return R.ID();
    ...
    public String racialParms() {
        ...
        for (int i = 0; i < bodyMask().length; i++) bbody.append(("" + bodyMask()[i]) + ";");
        ...
        if (adjEStats == null) str.append("<ESTATS/>"); else str.append(CMLib.xml().convertXMLtoTag("ESTATS", CMLib.coffeeMaker().getEnvStatsStr(adjEStats)));
        if (adjStats == null) str.append("<ASTATS/>"); else str.append(CMLib.xml().convertXMLtoTag("ASTATS", CMLib.coffeeMaker().getCharStatsStr(adjStats)));
        if (setStats == null) str.append("<CSTATS/>"); else str.append(CMLib.xml().convertXMLtoTag("CSTATS", CMLib.coffeeMaker().getCharStatsStr(setStats)));
        if (adjState == null) str.append("<ASTATE/>"); else str.append(CMLib.xml().convertXMLtoTag("ASTATE", CMLib.coffeeMaker().getCharStateStr(adjState)));
        if (startAdjState == null) str.append("<STARTASTATE/>"); else str.append(CMLib.xml().convertXMLtoTag("STARTASTATE", CMLib.coffeeMaker().getCharStateStr(startAdjState)));
        str.append(CMLib.xml().convertXMLtoTag("DISFLAGS", "" + disableFlags));
        if (myResources().size() == 0) str.append("<RESOURCES/>"); else {
        ...
        if ((outfit(null) == null) || (outfit(null).size() == 0)) str.append("<OUTFIT/>"); else {
            str.append("<OUTFIT>");
        ...
        if (naturalWeapon == null) str.append("<WEAPON/>"); else {
            str.append("<WEAPON>");
        ...
        if ((racialAbilityNames == null) || (racialAbilityNames.length == 0)) str.append("<RABILITIES/>"); else {
        ...
        if ((racialEffectNames == null) || (racialEffectNames.length == 0)) str.append("<REFFECTS/>"); else {
            str.append("<REFFECTS>");
        ...
        if ((culturalAbilityNames == null) || (culturalAbilityNames.length == 0)) str.append("<CABILITIES/>"); else {
        ...
        if (xtraValues == null) xtraValues = CMProps.getExtraStatCodesHolder(this);
        for (int i = this.getSaveStatIndex(); i < getStatCodes().length; i++) str.append(CMLib.xml().convertXMLtoTag(getStatCodes()[i], getStat(getStatCodes()[i])));
        str.append("</RACE>");
        return str.toString();
    }
    ...
    public void setRacialParms(String parms) {
        ...
        if (playerval.length() > 0) {
            if (playerval.startsWith("T")) availability = Area.THEME_FANTASY; else if (playerval.startsWith("F")) availability = 0; else switch(CMath.s_int(playerval)) {
                case 0:
        ...
        if ((avail != null) && (avail.length() > 0) && (CMath.isNumber(avail))) availability = CMath.s_int(avail);
        ...
        for (int v = 0; v < V.size(); v++) if (v < bodyMask().length) bodyMask()[v] = CMath.s_int((String) V.elementAt(v));
        adjEStats = null;
        String eStats = CMLib.xml().getValFromPieces(raceData, "ESTATS");
        if (eStats.length() > 0) {
            adjEStats = (EnvStats) CMClass.getCommon("DefaultEnvStats");
        ...
        for (int v = 0; v < aV.size(); v++) getAgingChart()[v] = CMath.s_int((String) aV.elementAt(v));
        clrStatChgDesc();
        ...
        if ((xV != null) && (xV.size() > 0)) {
            ...
            for (int x = 0; x < xV.size(); x++) {
                ...
                if ((!iblk.tag.equalsIgnoreCase("RSCITEM")) || (iblk.contents == null)) continue;
        ...
        if ((oV != null) && (oV.size() > 0)) {
            ...
            for (int x = 0; x < oV.size(); x++) {
                ...
                if ((!iblk.tag.equalsIgnoreCase("OFTITEM")) || (iblk.contents == null)) continue;
                ...
                } else Log.errOut("GenRace", "Unknown newOne race: " + CMLib.xml().getValFromPieces(iblk.contents, "OFCLASS"));
            }
        }
        naturalWeapon = null;
        ...
        if ((xV != null) && (xV.size() > 0)) {
            ...
            for (int x = 0; x < xV.size(); x++) {
                XMLLibrary.XMLpiece iblk = (XMLLibrary.XMLpiece) xV.elementAt(x);
                if ((!iblk.tag.equalsIgnoreCase("RABILITY")) || (iblk.contents == null)) continue;
        ...
        if ((xV != null) && (xV.size() > 0)) {
            ...
            for (int x = 0; x < xV.size(); x++) {
                XMLLibrary.XMLpiece iblk = (XMLLibrary.XMLpiece) xV.elementAt(x);
                if ((!iblk.tag.equalsIgnoreCase("REFFECT")) || (iblk.contents == null)) continue;
        ...
        if ((xV != null) && (xV.size() > 0)) {
            ...
            for (int x = 0; x < xV.size(); x++) {
                XMLLibrary.XMLpiece iblk = (XMLLibrary.XMLpiece) xV.elementAt(x);
                if ((!iblk.tag.equalsIgnoreCase("CABILITY")) || (iblk.contents == null)) continue;
        ...
        xtraValues = CMProps.getExtraStatCodesHolder(this);
        for (int i = this.getSaveStatIndex(); i < getStatCodes().length; i++) setStat(getStatCodes()[i], CMLib.xml().getValFromPieces(raceData, getStatCodes()[i]));
    }

    protected static String[] CODES = { "ID", "NAME", "CAT", "WEAR", "VWEIGHT", "BWEIGHT", "VHEIGHT", "FHEIGHT", "MHEIGHT", "AVAIL", "LEAVE", "ARRIVE", "HEALTHRACE", "BODY", "ESTATS", "ASTATS", "CSTATS", "ASTATE", "NUMRSC", "GETRSCID", "GETRSCPARM", "WEAPONCLASS", "WEAPONXML", "NUMRABLE", "GETRABLE", "GETRABLEPROF", "GETRABLEQUAL", "GETRABLELVL", "NUMCABLE", "GETCABLE", "GETCABLEPROF", "NUMOFT", "GETOFTID", "GETOFTPARM", "BODYKILL", "NUMREFF", "GETREFF", "GETREFFPARM", "GETREFFLVL", "AGING", "DISFLAGS", "STARTASTATE", "EVENTRACE", "WEAPONRACE", "HELP" };
    ...
    public String getStat(String code) {
        ...
        switch(getCodeNum(code)) {
            ...
            case 14:
                return (adjEStats == null) ? "" : CMLib.coffeeMaker().getEnvStatsStr(adjEStats);
            case 15:
            ...
            case 19:
                return "" + ((Item) myResources().elementAt(num)).ID();
            case 20:
                return "" + ((Item) myResources().elementAt(num)).text();
            case 21:
            ...
            case 32:
                return "" + ((outfit(null) != null) ? ((Item) outfit(null).elementAt(num)).ID() : "");
            case 33:
                return "" + ((outfit(null) != null) ? ((Item) outfit(null).elementAt(num)).text() : "");
            case 34:
                return "" + destroyBodyAfterUse();
            case 35:
            ...
            case 39:
                return CMParms.toStringList(getAgingChart());
            case 40:
                return "" + disableFlags;
            case 41:
    ...
    public boolean tick(Tickable myChar, int tickID) {
        if (eventBuddy != null) if (!eventBuddy.tick(myChar, tickID)) return false;
        return super.tick(myChar, tickID);
    }

    public void executeMsg(Environmental myHost, CMMsg msg) {
        if (eventBuddy != null) eventBuddy.executeMsg(myHost, msg);
        super.executeMsg(myHost, msg);
    }

    public boolean okMessage(Environmental myHost, CMMsg msg) {
        if ((eventBuddy != null) && (!eventBuddy.okMessage(myHost, msg))) return false;
    ...
    public void setStat(String code, String val) {
        ...
        switch(getCodeNum(code)) {
            ...
            case 8:
                ...
                break;
            case 9:
                availability = CMath.s_parseBitIntExpression(Area.THEME_DESCS, val);
                break;
            case 10:
                leaveStr = val;
                break;
            ...
            case 12:
                {
                    ...
                    try {
                        if (healthBuddy == null) healthBuddy = (Race) CMClass.unsortedLoadClass("RACE", val, true);
                    } catch (Exception e) {
                    }
                    break;
                }
            ...
            case 14:
                ...
                if (val.length() > 0) {
                    adjEStats = (EnvStats) CMClass.getCommon("DefaultEnvStats");
                    adjEStats.setAllValues(0);
                    CMLib.coffeeMaker().setEnvStats(adjEStats, val);
                }
                break;
            case 15:
                adjStats = null;
            ...
            case 17:
                ...
                break;
            case 18:
                if (CMath.s_int(val) == 0) resourceChoices = null; else resourceChoices = new Vector(CMath.s_int(val));
                break;
            case 19:
                {
                    if (resourceChoices == null) resourceChoices = new Vector();
                    if (num >= resourceChoices.size()) resourceChoices.addElement(CMClass.getItem(val)); else resourceChoices.setElementAt(CMClass.getItem(val), num);
                    break;
                }
            case 20:
                {
            ...
            case 24:
                {
                    if (racialAbilityNames == null) racialAbilityNames = new String[num + 1];
                    racialAbilityNames[num] = val;
                    break;
                }
            case 25:
                {
                    if (racialAbilityProficiencies == null) racialAbilityProficiencies = new int[num + 1];
                    racialAbilityProficiencies[num] = CMath.s_parseIntExpression(val);
                    break;
                }
            case 26:
                {
                    if (racialAbilityQuals == null) racialAbilityQuals = new boolean[num + 1];
                    racialAbilityQuals[num] = CMath.s_bool(val);
                    break;
                }
            case 27:
                {
                    if (racialAbilityLevels == null) racialAbilityLevels = new int[num + 1];
            ...
            case 29:
                {
                    if (culturalAbilityNames == null) culturalAbilityNames = new String[num + 1];
                    culturalAbilityNames[num] = val;
                    break;
                }
            case 30:
                {
                    if (culturalAbilityProficiencies == null) culturalAbilityProficiencies = new int[num + 1];
                    culturalAbilityProficiencies[num] = CMath.s_int(val);
                    break;
                }
            case 31:
                if (CMath.s_int(val) == 0) outfitChoices = null; else outfitChoices = new Vector(CMath.s_int(val));
                break;
            case 32:
                {
                    if (outfitChoices == null) outfitChoices = new Vector();
                    if (num >= outfitChoices.size()) outfitChoices.addElement(CMClass.getItem(val)); else outfitChoices.setElementAt(CMClass.getItem(val), num);
                    break;
                }
            case 33:
                {
            ...
            case 36:
                {
                    if (racialEffectNames == null) racialEffectNames = new String[num + 1];
                    racialEffectNames[num] = val;
                    break;
                }
            case 37:
                {
                    if (racialEffectParms == null) racialEffectParms = new String[num + 1];
                    racialEffectParms[num] = val;
                    break;
                }
            case 38:
                {
                    if (racialEffectLevels == null) racialEffectLevels = new int[num + 1];
            ...
            case 42:
                {
                    ...
                    if (eventBuddy == null) eventBuddy = (Race) CMClass.unsortedLoadClass("RACE", val, true);
                    break;
                }
            case 43:
                {
                    weaponBuddy = CMClass.getRace(val);
                    if (weaponBuddy == null) weaponBuddy = (Race) CMClass.unsortedLoadClass("RACE", val, true);
                    break;
                }
            case 44:
                {
    ...
    protected int getCodeNum(String code) {
        while ((code.length() > 0) && (Character.isDigit(code.charAt(code.length() - 1)))) code = code.substring(0, code.length() - 1);
    ...
    public boolean sameAs(Race E) {
        ...
        if (((GenRace) E).racialParms().equals(racialParms())) return true;
        return false;
    }
}
