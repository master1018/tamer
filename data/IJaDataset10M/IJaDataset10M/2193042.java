...
import emofilt.gui.SimpleInput;
...
public class EmofiltPlayer {
    ...
    public String genWavFile() {
        String tmpAudioFilePath = Emofilt._config.getString("tmpDir") + Emofilt._config.getString("tmpAudioFile");
    ...
    public String genWavFile(String outPath) {
        ...
        String tmpPhoFilePath = Emofilt._config.getString("tmpDir") + Emofilt._config.getString("tmpPhoFile");
        String voiceName = _emofilt.getVoice().getName();
        String execString = wavGenCmd + " " + wavGenDBPrefix + databasePath + voiceName + "/" + voiceName + " " + wavGenInPrefix + tmpPhoFilePath + " " + wavGenOutPrefix + outPath + " " + formatOption;
    ...
    public String genPhoFile(String text, boolean male) {
        String tmpPhoFile = Emofilt._config.getString("tmpDir") + Emofilt._config.getString("tmpPhoFile");
    ...
    public String genPhoFile(String text, String outFilePath, boolean male) {
        ...
        if (language.compareTo("fr") == 0) {
            String className = Emofilt._config.getString(language + Constants.PHO_GEN_SUFFIX);
            try {
                phoGenerator = (PhoGenInterface) (getClass().getClassLoader().loadClass("emofilt.generator." + className).newInstance());
            } catch (Exception e) {
                e.printStackTrace();
                return e.getMessage();
            }
            ...
            if (method.compareTo("mary") == 0) {
                ...
                _logger.error("no external phonetizer application specified: " + method);
            }
        }
        if (phoGenerator != null) {
            phoGenerator.init(Emofilt._config, _logger);
            return phoGenerator.genPhoFile(text, outFilePath, _emofilt.getActLanguage(), male);
        }
    ...
    public void play(String filePath) {
        ...
        while (filePlayer.isAlive()) {
            ...
            if (!_playing) filePlayer.stopPlayback();
        }
        filePlayer = null;
        _playing = false;
    }
