 ...
 */
public class bezcurve extends GLSkeleton<GLJPanel> implements GLEventListener, KeyListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        ctrlpointBuf = GLBuffers.newDirectFloatBuffer(ctrlpoints[0].length * ctrlpoints.length);
    ...
    public void reshape(GLAutoDrawable drawable, int x, int y, int w, int h) {
        ...
        gl.glLoadIdentity();
        if (w <= h) gl.glOrtho(-5.0, 5.0, -5.0 * (float) h / (float) w, 5.0 * (float) h / (float) w, -5.0, 5.0); else gl.glOrtho(-5.0 * (float) w / (float) h, 5.0 * (float) w / (float) h, -5.0, 5.0, -5.0, 5.0);
