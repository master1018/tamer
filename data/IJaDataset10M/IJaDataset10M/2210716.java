...
public class SpelExpressionEvaluator implements IStandardExpressionEvaluator {
    ...
    private static final CacheMap<String, SpelExpression> CACHE = new CacheMap<String, SpelExpression>("SpelExpressionEvaluator.CACHE", true, 100, EXPRESSION_CACHE_SIZE);
    ...
    public final Object evaluate(final Arguments arguments, final TemplateResolution templateResolution, final String spelExpression, final Object root) {
        ...
        try {
            ...
            final Map<String, Object> additionalContextVariables = computeAdditionalContextVariables(arguments, templateResolution);
            if (additionalContextVariables != null && !additionalContextVariables.isEmpty()) {
                contextVariables.putAll(additionalContextVariables);
            }
            final SpelEvaluationContext context = new SpelEvaluationContext(DEFAULT_EVALUATION_CONTEXT, contextVariables);
            final SpelExpression exp = getExpression(spelExpression);
            return exp.getValue(context, root);
        } catch (Exception e) {
            throw new ExpressionEvaluationException("Exception evaluating SpEL expression", templateResolution.getTemplateName(), spelExpression, e);
        }
    }
    ...
    protected Map<String, Object> computeAdditionalContextVariables(@SuppressWarnings("unused") final Arguments arguments, @SuppressWarnings("unused") final TemplateResolution templateResolution) {
