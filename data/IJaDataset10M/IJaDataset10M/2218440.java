...
public class BasicToken extends TokenBase implements Token {
    ...
    public Token subtoken(int beginIndex) {
        ...
        final BasicToken token = new BasicToken(text.substring(beginIndex), start.offsetBy(beginIndex), end);
    ...
    public Token subtoken(int beginIndex, int endIndex) {
        ...
        final BasicToken token = new BasicToken(text.substring(beginIndex, endIndex), start.offsetBy(beginIndex), start.offsetBy(endIndex - 1));
        token.addTags(getTags());
        return token;
    }
