...
public class ProjectBasesMethod {
    ...
    public void readMeshTriangle() {
        ...
        for (int i = 1; i <= eListBig.size(); i++) feLT.assignTo(eListBig.at(i));
    }
    ...
    public void run() {
        ...
        for (int c = 1; c <= nCol; c++) {
            ...
            for (int r = 1; r <= nRow; r++) {
                A.set(r, c, base.get(upSideNodes.at(r).globalIndex));
            }
        }
        for (int r = 1; r <= nRow; r++) {
        ...
        for (int i = 1; i <= nRow; i++) diagLmd.set(i, i, 1.5 / 4.0);
    ...
    public void run2() {
        ...
        AAT.mult(ff, g);
        SparseMatrix diagLmd = new SparseMatrixRowMajor(nRow, nRow);
        for (int i = 1; i <= nRow; i++) diagLmd.set(i, i, 10.0 / 4.0);
        C.axpy(1.0, new CompressedRowMatrix(diagLmd, false));
        g.add(new FullVector(x0));
        FullVector x = new FullVector(x0);
        Solver sol = new Solver();
