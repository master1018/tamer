...
public class PropertyManager {
    ...
    private static final int PROP_CONSOLE_TEXT = 36;

    private static final int PROP_COUNT = 37;

    public static int getPropertyNumber(String infoType) {
        if (infoType == null) return -1;
        for (int i = 0; i < PROP_COUNT; i++) if (infoType.equalsIgnoreCase(getPropertyName(i))) return i;
        return -1;
    }
    ...
    public static Object getProperty(Viewer viewer, String returnType, String infoType, Object paramInfo) {
        if (propertyTypes.length != PROP_COUNT * 3) Logger.warn("propertyTypes is not the right length: " + propertyTypes.length + " != " + PROP_COUNT * 3);
        Object info;
        ...
        if (requestedReadable) returnType = (isReadableAsString(infoType) ? "String" : "JSON");
        if (returnType.equalsIgnoreCase("String")) return (info == null ? "" : info.toString());
        if (requestedReadable) return Escape.toReadable(infoType, info); else if (returnType.equalsIgnoreCase("JSON")) return "{" + Escape.toJSON(infoType, info) + "}";
        return info;
    }
    ...
    static Object getModelProperty(Viewer viewer, String propertyName, Object propertyValue) {
        ...
        for (int i = 1; i < names.length; i++) {
            if ((n = Parser.parseInt(names[i])) != Integer.MIN_VALUE) args[i] = new ScriptVariable(Token.integer, n); else args[i] = new ScriptVariable(Token.string, names[i]);
        }
        ...
        switch(arg.tok) {
            case Token.integer:
                ...
                if (property instanceof List<?>) {
                    ...
                    if (pt < 0) pt += v.size();
                    if (pt >= 0 && pt < v.size()) return extractProperty(v.get(pt), args, ptr);
                    return "";
                }
                ...
                if (property instanceof Matrix3f) {
                    ...
                    float[][] f = new float[][] { new float[] { m.m00, m.m01, m.m02 }, new float[] { m.m10, m.m11, m.m12 }, new float[] { m.m20, m.m21, m.m22 } };
                    if (pt < 0) pt += 3;
                    if (pt >= 0 && pt < 3) return extractProperty(f, args, --ptr);
                    return "";
                }
                ...
                if (property instanceof float[][]) {
                    ...
                    if (pt >= 0 && pt < fflist.length) return extractProperty(fflist[pt], args, ptr);
                    return "";
                }
                if (property instanceof int[][]) {
                    int[][] iilist = (int[][]) property;
                    if (pt < 0) pt += iilist.length;
                    if (pt >= 0 && pt < iilist.length) return extractProperty(iilist[pt], args, ptr);
                    return "";
                }
            ...
            case Token.string:
                ...
                if (property instanceof Map<?, ?>) {
                    ...
                    if (h.containsKey(key)) return extractProperty(h.get(key), args, ptr);
                    return "";
                }
                if (property instanceof List<?>) {
                    List<Object> v = (List<Object>) property;
                    ...
                    for (pt = 0; pt < v.size(); pt++) {
                        Object o = v.get(pt);
                        if (o instanceof Map<?, ?>) v2.add(extractProperty(o, args, ptr));
                    }
                    return v2;
                }
                break;
    ...
    private static boolean isReadableAsString(String infoType) {
        for (int i = readableTypes.length; --i >= 0; ) if (infoType.equalsIgnoreCase(readableTypes[i])) return true;
        return false;
    }
    ...
    private static Object getPropertyAsObject(Viewer viewer, String infoType, Object paramInfo, String returnType) {
        ...
        switch(id) {
            ...
            case PROP_IMAGE:
                ...
                int pt;
                if ((pt = params.indexOf("height=")) >= 0) height = Parser.parseInt(params.substring(pt + 7));
                if ((pt = params.indexOf("width=")) >= 0) width = Parser.parseInt(params.substring(pt + 6));
                if (width < 0 && height < 0) height = width = -1; else if (width < 0) width = height; else height = width;
        ...
        for (int i = 0; i < PROP_COUNT; i++) {
            ...
            data[i] = (name.charAt(0) == 'X' ? "" : name + (paramType != "" ? " " + getParamType(i) + (paramDefault != "" ? " #default: " + getDefaultParam(i) : "") : ""));
        }
        Arrays.sort(data);
        StringBuffer info = new StringBuffer("getProperty ERROR\n" + infoType + "?\nOptions include:\n");
        for (int i = 0; i < PROP_COUNT; i++) if (data[i].length() > 0) info.append("\n getProperty ").append(data[i]);
