...
public class ParkingScoreExecutor {
    ...
    public void performScoring(AfterMobsimEvent event) {
        for (Person person : event.getControler().getPopulation().getPersons().values()) {
            double score = 0.0;
            LinkedList<ActivityImpl> parkingTargetActivities = ParkingGeneralLib.getParkingTargetActivities(person.getSelectedPlan());
            for (ActivityImpl targetActivity : parkingTargetActivities) {
                Id parkingFacilityId = ParkingGeneralLib.getArrivalParkingAct(person.getSelectedPlan(), targetActivity).getFacilityId();
                score += ParkingRoot.getParkingScoringFunction().getScore(targetActivity, person.getSelectedPlan(), parkingFacilityId, false);
            }
    ...
    private void printParkingScoreGraphForPersonGroups() {
        if (ParkingRoot.getPersonGroupsForStatistics() != null) {
            PersonGroupParkingScoreGraphicGenerator.generateGraphic(ParkingRoot.getPersonGroupsForStatistics(), GlobalRegistry.controler.getControlerIO().getOutputFilename("personGroupsParkingScore.png"));
        }
    }

    private void updateParkingScoreSumInPersonGroupsForPerson(Id personId, Double sumOfParkingScoresForPerson) {
        if (ParkingRoot.getPersonGroupsForStatistics() != null) {
            PersonGroups personGroupsStatistics = ParkingRoot.getPersonGroupsForStatistics();
            ...
            Double currentSum = (Double) personGroupsStatistics.getAttributeValueForGroupToWhichThePersonBelongs(personId, attribute);
            if (currentSum == null) {
                currentSum = 0.0;
            }
            currentSum += sumOfParkingScoresForPerson;
            personGroupsStatistics.setAttributeValueForGroupToWhichThePersonBelongs(personId, attribute, currentSum);
        }
    }
}
