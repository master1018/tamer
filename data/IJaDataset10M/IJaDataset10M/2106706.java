 ...
 *
 */
public class DeliveryNote implements Serializable, ArticleContainer, Statable, DetachCallback {
    ...
    public static List<DeliveryNoteID> getDeliveryNoteIDs(PersistenceManager pm, AnchorID vendorID, AnchorID customerID, long rangeBeginIdx, long rangeEndIdx) {
        ...
        if (rangeBeginIdx >= 0 && rangeEndIdx >= 0) query.setRange(rangeBeginIdx, rangeEndIdx);
    ...
    public DeliveryNote(User creator, LegalEntity vendor, LegalEntity customer, String deliveryNoteIDPrefix, long _deliveryNoteID) {
        if (creator == null) throw new NullPointerException("creator");
        if (vendor == null) throw new NullPointerException("vendor");
        if (customer == null) throw new NullPointerException("customer");
        ObjectIDUtil.assertValidIDString(deliveryNoteIDPrefix, "deliveryNoteIDPrefix");
        if (_deliveryNoteID < 0) throw new IllegalArgumentException("_deliveryNoteID < 0");
        PersistenceManager pm = JDOHelper.getPersistenceManager(vendor);
        if (pm == null) throw new IllegalStateException("vendor is not persistent! Could not get a PersistenceManager from it!");
    ...
    public AnchorID getVendorID() {
        if (vendorID == null && !vendorID_detached) vendorID = (AnchorID) JDOHelper.getObjectId(vendor);
        return vendorID;
    }

    public AnchorID getCustomerID() {
        if (customerID == null && !customerID_detached) customerID = (AnchorID) JDOHelper.getObjectId(customer);
        return customerID;
    }
    ...
    public Collection<Article> getArticles() {
        if (_articles == null) _articles = Collections.unmodifiableSet(articles);
        return _articles;
    }
	 ...
	 * @param article
	 */
    public void addArticle(Article article) throws DeliveryNoteEditException {
        ...
        if (isFinalized()) throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_DELIVERY_NOTE_FINALIZED, "DeliveryNote is finalized, can not change any more!", articleID);
        if (!vendor.getPrimaryKey().equals(articleOrder.getVendor().getPrimaryKey()) || !customer.getPrimaryKey().equals(articleOrder.getCustomer().getPrimaryKey())) {
            throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_ANCHORS_DONT_MATCH, "Vendor and customer are not equal for the Article to add and the delivery note, can not add the article!");
        }
        if (!article.isAllocated()) throw new IllegalArgumentException("Article is not allocated! " + article);
        DeliveryNoteID deliveryNoteID = article.getDeliveryNoteID();
        if (deliveryNoteID != null) {
            throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_ARTICLE_ALREADY_IN_DELIVERY_NOTE, "Article already in a delivery note. Article " + articleID + ", DeliveryNote " + deliveryNoteID, articleID, deliveryNoteID);
        }
        if (!article.getOffer().getOfferLocal().isAccepted()) {
            throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_OFFER_NOT_ACCEPTED, "At least one involved offer is not confirmed!", articleID);
        }
        if (article.isReversed()) throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_REVERSED_ARTICLE, "A reversed Article cannot be added to a DeliveryNote!", articleID);
        if (article.isReversing() && article.getReversedArticle().getDeliveryNoteID() == null) throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_REVERSING_ARTICLE, "The reversed Article is not in a DeliveryNote! Cannot add the reversing Article to one in this case!", articleID);
    ...
    public void removeArticle(Article article) throws DeliveryNoteEditException {
        if (isFinalized()) throw new DeliveryNoteEditException(DeliveryNoteEditException.REASON_DELIVERY_NOTE_FINALIZED, "DeliveryNote is finalized, can not change any more!");
    ...
    public void setFinalized(User finalizer) {
        ...
        if (!isValid()) throw new IllegalStateException("This DeliveryNote is not valid! Call validate() before!");
    ...
    protected PersistenceManager getPersistenceManager() {
        ...
        if (pm == null) throw new IllegalStateException("This instance of Order is currently not attached to the datastore. Cannot obtain PersistenceManager!");
        return pm;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DeliveryNote)) return false;
        DeliveryNote o = (DeliveryNote) obj;
        return Util.equals(this.organisationID, o.organisationID) && Util.equals(this.deliveryNoteIDPrefix, o.deliveryNoteIDPrefix) && this.deliveryNoteID == o.deliveryNoteID;
    }

    @Override
    public int hashCode() {
        return Util.hashCode(this.organisationID) ^ Util.hashCode(this.deliveryNoteIDPrefix) ^ Util.hashCode(this.deliveryNoteID);
    }
    ...
    public void setState(State currentState) {
        if (currentState == null) throw new IllegalArgumentException("state must not be null!");
        if (!currentState.getStateDefinition().isPublicState()) throw new IllegalArgumentException("state.stateDefinition.publicState is false!");
    ...
    public List<State> getStates() {
        if (_states == null) _states = CollectionUtil.castList(Collections.unmodifiableList(states));
        return _states;
    }
    ...
    public Set<ReceptionNote> getReceptionNotes() {
        if (_receptionNotes == null) _receptionNotes = Collections.unmodifiableSet(receptionNotes);
    ...
    public void bookDeliverProductTransfer(DeliverProductTransfer transfer, Set<Anchor> involvedAnchors, boolean rollback) {
        ...
        if (!vendorIsFrom && !vendorIsTo && !customerIsFrom && !customerIsTo) throw new IllegalArgumentException("Transfer \"" + transfer.getPrimaryKey() + "\" && DeliveryNote \"" + this.getPrimaryKey() + "\": Transfer and delivery note are not related!");
        int articleCount = 0;
        ...
        if (rollback) deliveryNoteLocal.decDeliveredArticleCount(articleCount); else deliveryNoteLocal.incDeliveredArticleCount(articleCount);
    }
}
