...
public class Activator extends AbstractUIPlugin {
    ...
    private void installMetricsAnalyzers() throws CoreException {
        IExtensionPoint p = Platform.getExtensionRegistry().getExtensionPoint(Anlyzer_Extension_ID);
        ...
        if (p != null) {
            ...
            for (int i = 0; i < x.length; i++) {
                IConfigurationElement[] analyzerExtentions = x[i].getConfigurationElements();
                for (int j = 0; j < analyzerExtentions.length; j++) {
                    ...
                    if (kind.equals("analyzer")) {
                        IConfigurationElement[] metricExtentions = analyzerExtention.getChildren();
                        ...
                        for (int k = 0; k < metricExtentions.length; k++) {
                            ...
                            MetricValueDescriptor descriptor = new MetricValueDescriptor(metricExtention.getAttribute("metricName"), MetricUtility.transferLevel(metricExtention.getAttribute("level")), metricExtention.getAttribute("description"), MetricUtility.transferPropagete(metricExtention.getAttribute("propagate")));
                            descriptors.add(descriptor);
                        }
                        metrics.addAll(descriptors);
                        System.out.println("Total " + descriptors.size() + " metric descriptors..");
                        Analyzer analyzer = (Analyzer) analyzerExtention.createExecutableExtension("analyzerClass");
    ...
    private void installMathOperators() throws CoreException {
        IExtensionPoint p = Platform.getExtensionRegistry().getExtensionPoint(MathOperator_Extension_ID);
        if (p != null) {
            IExtension[] x = p.getExtensions();
            for (int i = 0; i < x.length; i++) {
                IConfigurationElement[] operatorExtentions = x[i].getConfigurationElements();
                for (int j = 0; j < operatorExtentions.length; j++) {
                    ...
                    if (kind.equals("mathOperator")) {
                        MathOperator operator = (MathOperator) operatorExtention.createExecutableExtension("operatorClass");
    ...
    private void installImages() {
        ...
        imgReg.put(MetricUtility.transferLevel(Constants.PROJECT), getImageDescriptor("project.gif"));
        imgReg.put(MetricUtility.transferLevel(Constants.PACKAGEROOT), getImageDescriptor("packageRoot.gif"));
        imgReg.put(MetricUtility.transferLevel(Constants.PACKAGEFRAGMENT), getImageDescriptor("package.gif"));
        imgReg.put(MetricUtility.transferLevel(Constants.COMPILATIONUNIT), getImageDescriptor("java.gif"));
        imgReg.put(MetricUtility.transferLevel(Constants.TYPE), getImageDescriptor("class.gif"));
        imgReg.put(MetricUtility.transferLevel(Constants.METHOD), getImageDescriptor("method.gif"));
    ...
    public void showMetricView() throws PartInitException {
        ...
        page.showView("PKUMetric Plugin.MetricsView", null, IWorkbenchPage.VIEW_ACTIVATE);
    }
