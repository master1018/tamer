...
class LGridSVGGenerator extends LShapeSVGGenerator<IGrid> {
    ...
    protected LGridSVGGenerator(final SVGGElement elt, final boolean withTransformation) {
        ...
        if (gridElt != null) setSubGridElement(gridElt, prefix);
        gridElt = getLaTeXDrawElement(elt, LNamespace.XML_TYPE_GRID);
        if (gridElt != null) setMainGridElement(gridElt, prefix);
        setLabelGridElement(getLaTeXDrawElement(elt, LNamespace.XML_TYPE_TEXT));
        setNumber(elt);
        if (withTransformation) applyTransformations(elt);
    }
    ...
    private void setDimensionGridElement(final SVGGElement elt, final String prefix) {
        ...
        if (v != null) shape.setXLabelSouth(Boolean.parseBoolean(v));
        v = elt.getAttribute(prefix + LNamespace.XML_GRID_Y_WEST);
        if (v != null) shape.setYLabelWest(Boolean.parseBoolean(v));
    }
    ...
    private void setLabelGridElement(final SVGElement labelElt) {
        if (labelElt == null) shape.setLabelsSize(0); else {
    ...
    private void setMainGridElement(final SVGElement mainGridElt, final String prefix) {
        ...
        if (isGridDotted) shape.setLineColour(CSSColors.INSTANCE.getRGBColour(mainGridElt.getFill())); else shape.setLineColour(mainGridElt.getStroke());
        if (val == null) {
            double st = mainGridElt.getStrokeWidth();
            if (!Double.isNaN(st)) shape.setGridWidth(st);
        } else try {
    ...
    private void setSubGridElement(final SVGElement subGridElt, final String prefix) {
        ...
        if (isGridDotted) shape.setSubGridColour(CSSColors.INSTANCE.getRGBColour(subGridElt.getFill())); else shape.setSubGridColour(subGridElt.getStroke());
        if (val == null) {
            double st = subGridElt.getStrokeWidth();
            if (!Double.isNaN(st)) shape.setSubGridWidth(st);
        } else try {
    ...
    private void createSVGSubGridDots(final SVGDocument document, final SVGElement elt, final String prefix, final double subGridDiv, final double unit, final double xSubStep, final double ySubStep, final double minX, final double maxX, final double minY, final double maxY, final int subGridDots, final double subGridWidth, final double tlx, final double tly, final double brx, final double bry, final Color subGridColour) {
        ...
        for (i = 0, n = tlx; i < nbX; i++, n += xSubStep) for (j = 0, m = tly; j <= nbY; j++, m += ySubStep) for (k = 0; k < subGridDots; k++) {
        ...
        for (j = 0, n = tly; j < nbY; j++, n += ySubStep) for (i = 0, m = tlx; i <= nbX; i++, m += xSubStep) for (k = 0; k < subGridDots; k++) {
            dot = new SVGCircleElement(document);
    ...
    private void createSVGGridLabels(final SVGDocument document, final SVGElement elt, final String prefix, final double minX, final double maxX, final double minY, final double maxY, final double tlx, final double tly, final double xStep, final double yStep, final double gridWidth, final double absStep) {
        ...
        if (isYLabelWest) for (i = tly + (isXLabelSouth ? -width - gridLabelsSize / 4. : width + labelHeight), j = maxY; j >= minY; i += absStep, j--) {
            ...
            texts.appendChild(text);
        } else for (i = tly + (isXLabelSouth ? -width - gridLabelsSize / 4. : width + labelHeight), j = maxY; j >= minY; i += absStep, j--) {
            label = String.valueOf((int) j);
            text = new SVGTextElement(document);
    ...
    protected void createSVGGrid(final SVGElement elt, final SVGDocument document) {
        if (elt == null || document == null) return;
        final String prefix = LNamespace.LATEXDRAW_NAMESPACE + ':';
        ...
        if (subGridDots > 0) createSVGSubGridDots(document, elt, prefix, subGridDiv, unit, xSubStep, ySubStep, minX, maxX, minY, maxY, subGridDots, shape.getSubGridWidth(), tlx, tly, brx, bry, subGridColor); else if (subGridDiv > 1) createSVGSubGridDiv(document, elt, prefix, subGridDiv, xSubStep, ySubStep, minX, maxX, minY, maxY, subGridDots, shape.getSubGridWidth(), tlx, tly, brx, bry, subGridColor, posX, posY, xStep, yStep);
        if (shape.getGridDots() > 0) createSVGGridDots(document, elt, prefix, absStep, minX, maxX, minY, maxY, tlx, tly, brx, bry, unit, posX, posY, xStep, yStep, gridWidth, linesColor); else createSVGGridDiv(document, elt, prefix, minX, maxX, minY, maxY, tlx, tly, brx, bry, posX, posY, xStep, yStep, gridWidth, linesColor);
        if (shape.getLabelsSize() > 0) createSVGGridLabels(document, elt, prefix, minX, maxX, minY, maxY, tlx, tly, xStep, yStep, gridWidth, absStep);
        if (LNumber.INSTANCE.equals(shape.getRotationAngle() % (Math.PI * 2), 0.)) setSVGRotationAttribute(elt);
    }
