...
public class TryVariation extends AbstractRaptorAction {
    ...
    public TryVariation() {
        ...
        setDescription("If checked, this will update the position to the latest moves as" + " they arrive. If unchecked this will not update the position to the latest " + "moves as they arrive. This is useful when you are looking at a previous move.");
    ...
    public void run() {
        if (getChessBoardControllerSource() != null) {
            if (getChessBoardControllerSource().isToolItemSelected(ToolBarItemKey.TRY_VARIATION) && getChessBoardControllerSource() instanceof InactiveController) {
