...
public class StatisticsView extends ViewPart implements ISelectionListener, IRepositoryListener {
    ...
    public void createPartControl(final Composite parent) {
        ...
        UIPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(new IPropertyChangeListener() {
            ...
            public void propertyChange(final PropertyChangeEvent event) {
                ...
                } else if (m_calculateOvertime && (PreferenceConstants.UI_OVERTIME_REGULARTIME.equals(event.getProperty()) || PreferenceConstants.UI_OVERTIME_COLORIZE.equals(event.getProperty()))) {
                    updateStatisticLabels();
                }
            }
        });
    ...
    public void selectionChanged(final IWorkbenchPart part, final ISelection selection) {
        if (selection instanceof IAdaptable) {
            ...
            if (!m_selectedDay.equals(day) || project == null && m_selectedProject != null || project != null && !project.equals(m_selectedProject)) {
    ...
    protected void createControls(final Composite parent) {
        ...
        final String rows[] = new String[] { UIPlugin.getDefault().getString("day.label"), UIPlugin.getDefault().getString("week.label"), UIPlugin.getDefault().getString("month.label"), UIPlugin.getDefault().getString("year.label") };
        String columns[];
        m_calculateOvertime = UIPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.UI_OVERTIME_CALCULATE);
        if (m_calculateOvertime) {
            columns = new String[] { UIPlugin.getDefault().getString("project.label"), UIPlugin.getDefault().getString("statisticsView.mainProject.label"), UIPlugin.getDefault().getString("statisticsView.all.label"), UIPlugin.getDefault().getString("statisticsView.overtime.label") };
        } else {
            columns = new String[] { UIPlugin.getDefault().getString("project.label"), UIPlugin.getDefault().getString("statisticsView.mainProject.label"), UIPlugin.getDefault().getString("statisticsView.all.label") };
        }
    ...
    protected void updateStatisticLabels() {
        try {
            ...
            if (m_selectedProject != null) {
                final List<ProjectSummary> projectPath = RepositoryFactory.getRepository().getProjectRepository().getProjectPath(m_selectedProject.getId());
                ...
                if (overtime != null && j < overtime.length) {
                    ...
                    if (UIPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.UI_OVERTIME_COLORIZE)) {
                        if (overtime[j] < 0) {
                            ...
                            m_statisticLabels[i].setForeground(m_statisticLabels[i].getDisplay().getSystemColor(SWT.COLOR_DARK_GREEN));
                        }
                    } else {
        ...
        try {
            ...
            if (timePolicies != null && !timePolicies.isEmpty()) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    for (final DayInfo dayInfo : RepositoryFactory.getRepository().getWorkItemRepository().getDayInfos(DayInfoFilter.filterMonth(year, month))) {
                            ...
                            if (dayInfo == null || dayInfo.getRegularTime() < 0) {
                                ...
                                for (final TimePolicy timePolicy : timePolicies) {
                                    if ((regularTime = timePolicy.getRegularTime(day)) >= 0) {
                                        overtime = (dayInfo != null ? dayInfo.getTimeContingents().get(TimeContingent.WORKTIME) : 0) - regularTime;
                                        break;
                                    }
                                }
