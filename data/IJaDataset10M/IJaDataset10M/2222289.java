...
public final class Compare {
        ...
        for (Map.Entry<Integer, ArrayList<Placement>> entry : games.entrySet()) {
            ...
            for (int rot = 0; rot < BoardUtil.NUMBER_ROTATIONS; ++rot) {
                ...
                for (int i = 0; numberDifferent <= maxDifferent && i < moveNumber; ++i) {
                    ...
                    if (move.m_isSetup != gameMove.m_isSetup || !move.m_color.equals(gameMove.m_color) || !GoPoint.equals(move.m_point, gameRotatedPoint)) ++numberDifferent;
                }
                if (numberDifferent == 0) return Integer.toString(numberGame); else if (numberDifferent < maxDifferent) result = Integer.toString(numberGame) + "?";
            }
        }
        return result;
    }
    ...
    public static void compare(ArrayList<String> filenames) throws Exception {
        Board board = null;
        Map<Integer, ArrayList<Placement>> games = new TreeMap<Integer, ArrayList<Placement>>();
        for (int gameNumber = 0; gameNumber < filenames.size(); ++gameNumber) {
            ...
            if (board == null) board = new Board(size); else if (size != board.getSize()) throw new Exception("Board size in " + filename + " does not match other games");
            ArrayList<Placement> moves = getPlacements(tree.getRoot());
            String duplicate = checkDuplicate(board, moves, games, false, false);
    ...
    public static ArrayList<Placement> getPlacements(ConstNode node) {
        ...
        while (node != null) {
            for (GoColor c : BLACK_WHITE_EMPTY) {
                ...
                for (GoPoint p : list) result.add(new Placement(true, c, p));
            }
            Move move = node.getMove();
            if (move != null) result.add(new Placement(move));
