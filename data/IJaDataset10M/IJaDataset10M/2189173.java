...
public class RRDatabase {
	 ...
	 * @throws IOException  if there was a problem reading data from the database.
	 */
    public DataChunk getData(ConsolidationFunctionType type, long step) throws IOException {
        ArrayList<Archive> possibleArchives = getArchiveList(type);
        if (possibleArchives.size() == 0) {
            throw new IllegalArgumentException("Database does not contain an Archive of consolidation function type " + type);
        }
