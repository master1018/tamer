...
public class UploadDocumentCommand implements Command {
   ...
   */
    public enum MediaType {
        ...
        CSV("text/csv"), DOC("application/msword"), HTML("text/html"), HTM("text/html"), ODT("application/vnd.oasis.opendocument.text"), RTF("application/rtf"), TXT("text/plain"), AEA("application/octet-stream"), AES("application/octet-stream"), SRT("text/plain"), SUB("text/plain");
    ...
    public static final UploadDocumentCommand INSTANCE = new UploadDocumentCommand();
   ...
   */
    ...
    public void execute(GttService service, String[] args) throws IOException, ServiceException {
    ...
    private DocumentEntry createEntryFromArgs(String[] args) throws IOException {
        ...
        if (parser.containsKey("weburl")) {
            ...
            DocumentSource docSource = new DocumentSource(DocumentSource.Type.HTML, url);
            ...
            DocumentSource docSource = new DocumentSource(DocumentSource.Type.WIKI, url);
            entry.setDocumentSource(docSource);
        } else if (parser.containsKey("knolurl")) {
            String url = parser.getValue("knolurl");
            System.out.println("... with knol contents from " + url);
            DocumentSource docSource = new DocumentSource(DocumentSource.Type.KNOL, url);
            entry.setDocumentSource(docSource);
        ...
        if (parser.containsKey("tmids")) {
            ...
            System.out.println("...by adding translation memories with ids: " + tmIds);
        ...
        if (parser.containsKey("glids")) {
            ...
            System.out.println("...by adding glossaries with ids: " + glIds);
    ...
    private void printResults(DocumentEntry entry) {
        System.out.println("...done, document was successfully created with " + "attributes.");
        String id = entry.getId().substring(entry.getId().lastIndexOf('/') + 1);
        System.out.println("->" + "id = " + id + ", title = '" + entry.getTitle().getPlainText() + "'");
    }

    public String helpString() {
        return "Creates translation documents." + "\n\t--srclang <lang id>\t; id of source language" + "\n\t--targetlang <lang id>\t; id of target language" + "\n\t--title <title>\t; a name for the document" + "\n\t--file <filename>\t; if content for translation is " + "in a local file" + "\n\t--weburl <url>\t; if content for translation is a web page" + "\n\t--wikipediaurl <url>\t; if content for translation is a " + "wikipedia article" + "\n\t--knolurl <url>\t; if content for translation is a " + "knol article" + "\n\t--tmids <id, id, ...>\t; Optional param to attach translation " + "memories to document, value must be comma separated tm ids" + "\n\t--glids <id, id, ...>\t; Optional param to attach glossaries " + "to document, value must be comma separated glossary ids";
    }
}
