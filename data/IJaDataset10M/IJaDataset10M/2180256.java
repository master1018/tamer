...
final class CompoundFile extends BaseCompoundFile {
   ...
   */
    public CompoundFile(ExcelDataOutput data, int l, OutputStream os, jxl.read.biff.CompoundFile rcf) throws CopyAdditionalPropertySetsException, IOException {
        ...
        numPropertySets = 4 + (additionalPropertySets != null ? additionalPropertySets.size() : 0);
        ...
        if (numBigBlockDepotBlocks > blockChainLength - 1) {
            ...
            numExtensionBlocks = (int) Math.ceil((double) bbdBlocksLeft / (double) (BIG_BLOCK_SIZE / 4 - 1));
   ...
   */
    private void writeHeader() throws IOException {
        ...
        IntegerHelper.getFourBytes(numBigBlockDepotBlocks, headerBlock, NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
        IntegerHelper.getFourBytes(sbdStartBlockChain, headerBlock, SMALL_BLOCK_DEPOT_BLOCK_POS);
        IntegerHelper.getFourBytes(numSmallBlockDepotChainBlocks, headerBlock, NUM_SMALL_BLOCK_DEPOT_BLOCKS_POS);
        IntegerHelper.getFourBytes(extensionBlock, headerBlock, EXTENSION_BLOCK_POS);
        IntegerHelper.getFourBytes(numExtensionBlocks, headerBlock, NUM_EXTENSION_BLOCK_POS);
        IntegerHelper.getFourBytes(rootStartBlock, headerBlock, ROOT_START_BLOCK_POS);
        int pos = BIG_BLOCK_DEPOT_BLOCKS_POS;
        int blocksToWrite = Math.min(numBigBlockDepotBlocks, (BIG_BLOCK_SIZE - BIG_BLOCK_DEPOT_BLOCKS_POS) / 4);
        int blocksWritten = 0;
        ...
        for (int extBlock = 0; extBlock < numExtensionBlocks; extBlock++) {
            blocksToWrite = Math.min(numBigBlockDepotBlocks - blocksWritten, BIG_BLOCK_SIZE / 4 - 1);
            ...
            int nextBlock = (blocksWritten == numBigBlockDepotBlocks) ? -2 : extBlock + 1;
   ...
   */
    private void writeSmallBlockDepotChain() throws IOException {
        ...
        for (Iterator i = additionalPropertySets.iterator(); i.hasNext(); ) {
            ReadPropertyStorage rps = (ReadPropertyStorage) i.next();
            if (rps.data.length <= SMALL_BLOCK_THRESHOLD && rps.data.length != 0) {
