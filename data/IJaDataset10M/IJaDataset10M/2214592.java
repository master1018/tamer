    ...
    protected void transformInverse(int x, int y, float[] out) {
        ...
        switch(type) {
            case RECT_TO_POLAR:
                ...
                if (x != centreX) m = Math.abs(((float) (y - centreY)) / ((float) (x - centreX))); else m = 0;
            ...
            case POLAR_TO_RECT:
                ...
                float theta2;
                if (theta >= 1.5f * ImageMath.PI) theta2 = ImageMath.TWO_PI - theta; else if (theta >= ImageMath.PI) theta2 = theta - ImageMath.PI; else if (theta >= 0.5f * ImageMath.PI) theta2 = ImageMath.PI - theta; else theta2 = theta;
                t = (float) Math.tan(theta2);
                if (t != 0) m = 1.0f / t; else m = 0;
                if (m <= ((float) (height) / (float) (width))) {
                    if (theta2 == 0) {
                        xmax = 0;
                        ymax = centreY;
