...
public class ServerStoredContactListJabberImpl {
     ...
     */
    private void fireContactRemoved(ContactGroup parentGroup, ContactJabberImpl contact) {
        if (parentOperationSet == null) {
            logger.debug("No presence op. set available. Bailing out.");
            return;
        }
    ...
    private ContactGroupJabberImpl findContactGroupByNameCopy(String name) {
        ...
        while (contactGroups.hasNext()) {
            ContactGroupJabberImpl contactGroup = (ContactGroupJabberImpl) contactGroups.next();
            if (contactGroup.getNameCopy().equals(name)) return contactGroup;
        }
        return null;
    }
    ...
    public ContactGroup findContactGroup(ContactJabberImpl child) {
        ...
        while (contacts.hasNext()) {
            ...
            if (contact.equals(child)) return rootGroup;
        }
        return null;
    }
    ...
    ContactJabberImpl createVolatileContact(String id) {
        VolatileContactJabberImpl newVolatileContact = new VolatileContactJabberImpl(id, this);
        ContactGroupJabberImpl theVolatileGroup = getNonPersistentGroup();
        if (theVolatileGroup == null) {
            theVolatileGroup = new VolatileContactGroupJabberImpl(VOLATILE_GROUP_NAME, this);
    ...
    ContactJabberImpl createUnresolvedContact(ContactGroup parentGroup, String id) {
        ...
        if (parentGroup instanceof ContactGroupJabberImpl) ((ContactGroupJabberImpl) parentGroup).addContact(newUnresolvedContact); else if (parentGroup instanceof RootContactGroupJabberImpl) ((RootContactGroupJabberImpl) parentGroup).addContact(newUnresolvedContact);
    ...
    public void createGroup(String groupName) throws OperationFailedException {
        ...
        ContactGroupJabberImpl newGroup = new ContactGroupJabberImpl(newRosterGroup, new Vector().iterator(), this, true);
     ...
     */
    public void moveContact(ContactJabberImpl contact, ContactGroupJabberImpl newParent) {
     ...
     */
    void fireContactResolved(ContactGroupJabberImpl parentGroup, ContactJabberImpl contact) {
        if (parentOperationSet == null) {
            logger.debug("No presence op. set available. Bailing out.");
            return;
        }
                ...
                while (iter1.hasNext()) {
                    ...
                    if (findContactGroup(gr.getName()) == null) {
                        ...
                        if (!gr.getName().equals(contactGroup.getGroupName())) {
                            if (contactGroup instanceof ContactGroupJabberImpl) ((ContactGroupJabberImpl) contactGroup).removeContact(contact); else if (contactGroup instanceof RootContactGroupJabberImpl) ((RootContactGroupJabberImpl) contactGroup).removeContact(contact);
    ...
    private class ImageRetriever extends Thread {
        ...
        private byte[] getAvatar(ContactJabberImpl contact) {
            try {
                ...
                logger.error("Cannot load image for contact " + this + " : " + exc.getMessage(), exc);
            }
            return null;
        }
    }
