...
public class StreamSerializer {
   ...
   */
    ...
    public StreamSerializer(int mode, String encoding, String eol) {
        this.mode = mode;
        if (encoding == null) encoding = (mode == Stylesheet.OUTPUT_HTML) ? "ISO-8859-1" : "UTF-8";
    ...
    public void serialize(final Node node, final OutputStream out) throws IOException {
        serialize(node, out, false);
    }

    void serialize(Node node, final OutputStream out, boolean convertToCdata) throws IOException {
        while (node != null) {
    ...
    private void doSerialize(final Node node, final OutputStream out, boolean convertToCdata) throws IOException {
        if (out == null) throw new NullPointerException("no output stream");
        htmlEncoded = false;
        String value, prefix;
        Node children;
        String uri = node.getNamespaceURI();
        short nt = node.getNodeType();
        if (convertToCdata && nt == Node.TEXT_NODE) nt = Node.CDATA_SECTION_NODE;
        switch(nt) {
            case Node.ATTRIBUTE_NODE:
                prefix = node.getPrefix();
                if (XMLConstants.XMLNS_ATTRIBUTE_NS_URI.equals(uri) || XMLConstants.XMLNS_ATTRIBUTE.equals(prefix) || (prefix != null && prefix.startsWith("xmlns:"))) {
                    String nsuri = node.getNodeValue();
                    if (isDefined(nsuri, prefix)) break;
                    String name = node.getLocalName();
                    if (name == null) {
                        name = node.getNodeName();
                        int ci = name.indexOf(':');
                        if (ci != -1) name = name.substring(ci + 1);
                    }
                    define(nsuri, name);
                ...
                if (mode == Stylesheet.OUTPUT_HTML && a_nodeName.equals(a_nodeValue) && isHTMLBoolean((Attr) node, a_nodeName)) break;
                out.write(EQ);
            ...
            case Node.TEXT_NODE:
                ...
                if (!"yes".equals(node.getUserData("disable-output-escaping"))) value = encode(value, false, false);
                out.write(encodeText(value));
                break;
            ...
            case Node.COMMENT_NODE:
                ...
                if (cp != null && cp.getNodeType() == Node.DOCUMENT_NODE) out.write(encodeText(eol));
                break;
            ...
            case Node.DOCUMENT_FRAGMENT_NODE:
                if (mode == Stylesheet.OUTPUT_XML) {
                    ...
                    if (!"yes".equals(node.getUserData("omit-xml-declaration")) && xmlDeclaration) {
                        Document doc = (node instanceof Document) ? (Document) node : null;
                        String version = (doc != null) ? doc.getXmlVersion() : null;
                        if (version == null) version = (String) node.getUserData("version");
                        ...
                        if ((doc != null && doc.getXmlStandalone()) || "yes".equals(node.getUserData("standalone"))) out.write(" standalone=\"yes\"".getBytes("US-ASCII"));
                        out.write(0x3f);
                        out.write(KET);
                    ...
                    String contentType = mediaType + "; charset=" + ((encoding.indexOf(' ') != -1) ? "\"" + encoding + "\"" : encoding);
                    Document doc = (node instanceof Document) ? (Document) node : node.getOwnerDocument();
                    Node html = null;
                    for (Node ctx = node.getFirstChild(); ctx != null; ctx = ctx.getNextSibling()) {
                        if (ctx.getNodeType() == Node.ELEMENT_NODE && isHTMLElement(ctx, "html")) {
                            html = ctx;
                            break;
                        }
                    }
                    if (html != null) {
                        Node head = null;
                        for (Node ctx = html.getFirstChild(); ctx != null; ctx = ctx.getNextSibling()) {
                        ...
                        if (head != null) {
                            ...
                            for (Node ctx = head.getFirstChild(); ctx != null; ctx = ctx.getNextSibling()) {
                                if (isHTMLElement(ctx, "meta")) {
                                    ...
                                    for (int i = 0; i < len; i++) {
                                        ...
                                        if ("http-equiv".equalsIgnoreCase(attrName)) httpEquiv = attr.getNodeValue(); else if ("content".equalsIgnoreCase(attrName)) content = attr;
                                    }
                            ...
                            if (meta == null) {
                                ...
                                if (first == null) head.appendChild(meta); else head.insertBefore(meta, first);
                ...
                children = node.getFirstChild();
                if (children != null) serialize(children, out, convertToCdata);
                break;
    ...
    boolean isHTMLElement(Node node, String name) {
        ...
        if (!name.equalsIgnoreCase(localName)) return false;
        String uri = node.getNamespaceURI();
        return (uri == null || HTML_URIS.contains(uri));
    }

    boolean isDefined(String uri, String prefix) {
        if (XMLConstants.XML_NS_URI.equals(uri)) return "xml".equals(prefix);
        if (XMLConstants.XMLNS_ATTRIBUTE_NS_URI.equals(uri)) return "xmlns".equals(prefix);
        if (prefix == null) prefix = "";
        for (Iterator i = namespaces.iterator(); i.hasNext(); ) {
            Map ctx = (Map) i.next();
            String val = (String) ctx.get(uri);
            if (val != null && val.equals(prefix)) return true;
        }
        return false;
    }
    ...
    String define(String uri, String prefix) {
        if (namespaces.isEmpty()) return prefix;
    ...
    final byte[] encodeText(String text) throws IOException {
        encoder.reset();
        boolean htmlNeedingEncoding = (mode == Stylesheet.OUTPUT_HTML && !htmlEncoded);
    ...
    String encode(String text, boolean encodeCtl, boolean inAttr) {
        ...
        for (int i = 0; i < len; i++) {
            char c = text.charAt(i);
            if (c == '<') {
                if (buf == null) buf = new StringBuffer(text.substring(0, i));
                buf.append("&lt;");
            } else if (c == '>') {
                if (buf == null) buf = new StringBuffer(text.substring(0, i));
                buf.append("&gt;");
            } else if (c == '&') {
                if (mode == Stylesheet.OUTPUT_HTML && (i + 1) < len && text.charAt(i + 1) == '{') {
                ...
                if (buf == null) buf = new StringBuffer(text.substring(0, i));
                if (mode == Stylesheet.OUTPUT_HTML) buf.append("&#x27;"); else buf.append("&apos;");
            } else if (c == '"' && inAttr) {
                if (buf == null) buf = new StringBuffer(text.substring(0, i));
                buf.append("&quot;");
            } else if (encodeCtl) {
