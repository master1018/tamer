...
public class ProbeDialog extends JFrame implements ActionListener, DocumentListener {
    ...
    public ProbeDialog(DashHierarchy props, DataRepository data) {
        ...
        box.setBorder(BorderFactory.createTitledBorder(resources.getString("L_Regression")));
        ...
        significance.setToolTipText(resources.getString("Significance_Description"));
        box = new JPanel();
        box.setLayout(new BoxLayout(box, BoxLayout.Y_AXIS));
        box.setBorder(BorderFactory.createTitledBorder(resources.getString("Average")));
    ...
    private String formatPercent(double value, int numDecimalPoints) {
        if (Double.isNaN(value) || Double.isInfinite(value)) return notEnoughData;
        percentFormat.setMaximumFractionDigits(numDecimalPoints);
        return percentFormat.format(value);
    }

    private String format(double value, int numDecimalPoints) {
        if (Double.isInfinite(value) || Double.isNaN(value)) return notEnoughData; else return FormatUtil.formatNumber(value, numDecimalPoints);
    }
    ...
    private static final boolean[] PROBE_SHOW_REGRESSION = { true, true, true, true, false, false, false, false, false };

    private static final String[] PROBE_X = { EST_OBJ_LOC, EST_OBJ_LOC, EST_NC_LOC, EST_NC_LOC, EST_OBJ_LOC, EST_NC_LOC, EST_OBJ_LOC, EST_NC_LOC, ACT_NC_LOC };

    private static final String[] PROBE_Y = { ACT_NC_LOC, ACT_TIME, ACT_NC_LOC, ACT_TIME, ACT_NC_LOC, ACT_NC_LOC, ACT_TIME, ACT_TIME, ACT_TIME };
    ...
    private String formatField(String key, String value) {
        ...
        if (value == null) return label + " = " + unknownValue; else return label + " = " + value;
    }

    private void setAFields(boolean enableFields, String b0Text, String b1Text, String rsText, String sigText, String varText, String sdText) {
    ...
    private void setARanges(boolean enableFields, String proText, String ranText, String upiText, String lpiText) {
    ...
    private void setCFields(boolean enableFields, String b0Text, String b1Text) {
    ...
    private void setCRanges(boolean enableFields, String proText) {
    ...
    private void doEnable(JComponent c) {
        for (int i = c.getComponentCount(); i-- > 0; ) c.getComponent(i).setForeground(label.getForeground());
    }
    ...
    private class CorrelationWorker extends SwingWorker {
        ...
        public Object construct() {
            if (x_name == null || x_name.length() == 0 || y_name == null || y_name.length() == 0) return null;
    ...
    private void updateValuesAfterCorrelation(String x_name, String y_name) {
        ...
        if (corr == null) {
            ...
            setAFields(enableFields, format(corr.r.beta0, 4), format(corr.r.beta1, 4), format(r_squared, 4), formatPercent(corr.c.p, 2), format(var, 2), format(corr.r.stddev, 3));
            setColorFromRange(rSquared, r_squared, R_SQUARED_COLORS);
            setColorFromRange(significance, corr.c.p, SIGNIFICANCE_COLORS);
            setCFields(enableFields, format(0, 4), format(corr.r.y_avg / corr.r.x_avg, 4));
        }
        ...
        findRange();
        if (chartDlg != null) chartDlg.updateDialog(corr, x_name, y_name, showRegression, showAverage);
        if (!showRegression) doDisable(regressionBox);
        if (!showAverage) doDisable(averageBox);
    }

    private Object[][] R_SQUARED_COLORS = { { null, Color.red }, { new Double(0.5), Color.orange.darker() }, { new Double(0.7), Color.blue }, { new Double(0.9), Color.black } };

    private Object[][] SIGNIFICANCE_COLORS = { { null, Color.black }, { new Double(0.0501), Color.blue }, { new Double(0.1201), Color.orange.darker() }, { new Double(0.2001), Color.red } };

    private void setColorFromRange(JComponent c, double val, Object[][] colorRanges) {
    ...
    private void findRange() {
        ...
        if (corr == null || Double.isNaN(est) || Double.isNaN(prob)) {
            ...
            enableFields = true;
            setARanges(enableFields, format(corr.r.projection, 2), format(corr.r.range, 2), format(corr.r.UPI, 2), format(corr.r.LPI, 2));
            setCRanges(enableFields, format((corr.r.y_avg / corr.r.x_avg) * est, 2));
        }
        if (!showRegression) doDisable(regressionBox);
        if (!showAverage) doDisable(averageBox);
        pack();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if ("filter".equals(cmd)) {
            if (filterDlg == null) filterDlg = new FilterDialog(hier, this, this); else filterDlg.setVisible(true);
        } else if ("chart".equals(cmd)) {
            if (chartDlg == null) chartDlg = new ChartDialog(this, corr, (String) xName.getSelectedItem(), (String) yName.getSelectedItem(), showRegression, showAverage); else chartDlg.setVisible(true);
        } else if ("close".equals(cmd)) {
            close();
        } else if ("applyFilter".equals(cmd)) {
            ...
            } else showRegression = showAverage = true;
            if (!programaticallyChangingXY) correlate();
        } else if (e.getSource() == estimate || e.getSource() == percent || e.getSource() == docChangeTimer) {
            findRange();
        } else if (!programaticallyChangingXY && (e.getSource() == xName || e.getSource() == yName)) {
            programaticallyChangingXY = true;
