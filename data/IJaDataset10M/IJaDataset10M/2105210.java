...
public abstract class PersistentManagerBase extends ManagerBase implements Lifecycle, PropertyChangeListener {
    ...
    public void setMaxIdleBackup(int backup) {
        if (backup == this.maxIdleBackup) return;
    ...
    public void setMaxIdleSwap(int max) {
        if (max == this.maxIdleSwap) return;
    ...
    public void setMinIdleSwap(int min) {
        if (this.minIdleSwap == min) return;
    ...
    public boolean isLoaded(String id) {
        try {
            if (super.findSession(id) != null) return true;
        } catch (IOException e) {
    ...
    public void setSaveOnRestart(boolean saveOnRestart) {
        if (saveOnRestart == this.saveOnRestart) return;
    ...
    public void processExpires() {
        ...
        int expireHere = 0;
        if (log.isDebugEnabled()) log.debug("Start expire sessions " + getName() + " at " + timeNow + " sessioncount " + sessions.length);
        ...
        if (log.isDebugEnabled()) log.debug("End expire sessions " + getName() + " processingTime " + (timeEnd - timeNow) + " expired sessions: " + expireHere);
    ...
    public void unload() {
        ...
        if (n == 0) return;
        if (log.isDebugEnabled()) log.debug(sm.getString("persistentManager.unloading", String.valueOf(n)));
        for (int i = 0; i < n; i++) try {
            swapOut(sessions[i]);
        } catch (IOException e) {
            ;
    ...
    public void start() throws LifecycleException {
        ...
        if (store == null) log.error("No Store configured, persistence disabled"); else if (store instanceof Lifecycle) ((Lifecycle) store).start();
    }
    ...
    protected void processMaxIdleSwaps() {
        if (!isStarted() || maxIdleSwap < 0) return;
        Session sessions[] = findSessions();
        long timeNow = System.currentTimeMillis();
        if (maxIdleSwap >= 0) {
            for (int i = 0; i < sessions.length; i++) {
                ...
                synchronized (session) {
                    ...
                    if (timeIdle > maxIdleSwap && timeIdle > minIdleSwap) {
                        if (log.isDebugEnabled()) log.debug(sm.getString("persistentManager.swapMaxIdle", session.getIdInternal(), new Integer(timeIdle)));
                        try {
                            swapOut(session);
                        } catch (IOException e) {
                            ;
    ...
    protected void processMaxActiveSwaps() {
        if (!isStarted() || getMaxActiveSessions() < 0) return;
        Session sessions[] = findSessions();
        if (getMaxActiveSessions() >= sessions.length) return;
        if (log.isDebugEnabled()) log.debug(sm.getString("persistentManager.tooManyActive", new Integer(sessions.length)));
        ...
        for (int i = 0; i < sessions.length && toswap > 0; i++) {
            synchronized (sessions[i]) {
                ...
                if (timeIdle > minIdleSwap) {
                    if (log.isDebugEnabled()) log.debug(sm.getString("persistentManager.swapTooManyActive", sessions[i].getIdInternal(), new Integer(timeIdle)));
                    try {
                        swapOut(sessions[i]);
                    } catch (IOException e) {
                        ;
    ...
    protected void processMaxIdleBackups() {
        if (!isStarted() || maxIdleBackup < 0) return;
        Session sessions[] = findSessions();
        long timeNow = System.currentTimeMillis();
        if (maxIdleBackup >= 0) {
            for (int i = 0; i < sessions.length; i++) {
                ...
                synchronized (session) {
                    ...
                    if (timeIdle > maxIdleBackup) {
                        if (log.isDebugEnabled()) log.debug(sm.getString("persistentManager.backupMaxIdle", session.getIdInternal(), new Integer(timeIdle)));
                        try {
                            writeSession(session);
                        } catch (IOException e) {
                            ;
