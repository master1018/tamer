...
public class ToolContentDialog extends JDialog {
    ...
    void removeIngredient(ContentItem contentItem) {
        VirtualRecipeAssistantEnvironment.sendPercept(new Percept(Percepts.INGREDIENTREMOVED, new Identifier(contentItem.getIngredient().getTitle()), new Identifier(tool.getName())));
    ...
    public boolean addIngredient(Ingredient ingredient, int amount) {
        ...
        if (capacity != -1 && requiredCapacity > capacity) {
            VirtualRecipeAssistantEnvironment.sendPercept(new Percept(Percepts.NOCAPACITY, new Identifier(tool.getName()), new Identifier(ingredient.getName())));
            return false;
        } else {
            VirtualRecipeAssistantEnvironment.sendPercept(new Percept(Percepts.INGREDIENTADDED, new Identifier(ingredient.getName()), new Numeral(amount), new Identifier(tool.getName())));
            if (contentItems.containsKey(ingredient.getName())) contentItems.get(ingredient.getName()).add(amount); else {
                ContentItem contentItem = new ContentItem(ingredient, amount, this);
                add(contentItem);
                contentItems.put(ingredient.getName(), contentItem);
            }
            if (capacity != -1) capacity -= requiredCapacity;
            validate();
        }
        return true;
    }
...
@SuppressWarnings("serial")
class ContentItem extends JLabel implements MouseListener, ActionListener {
    ...
    public ContentItem(Ingredient ingredient, int amount, ToolContentDialog toolContentDialog) {
        ...
        if (ingredient.getImageIcon() != null) setIcon(new ImageIcon(ingredient.getImageIcon().getImage().getScaledInstance(48, 48, Image.SCALE_SMOOTH)));
