...
public class SHAiButtonUser33 extends SHAiButtonUser {
    ...
    */
    private static final byte[] ffBlock = new byte[] { (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF, (byte) 0x0FF };
    ...
    */
    public SHAiButtonUser33(SHAiButtonCopr copr, OneWireContainer33 owc, boolean formatDevice, byte[] authSecret) throws OneWireException, OneWireIOException {
        ...
        for (int i = 0; i < 8; i++) NullSecret[i] = 0x00;
        if (!this.ibc33.loadFirstSecret(NullSecret, 0)) throw new OneWireException("Failed to null out device secret.");
        if (!owc.installMasterSecret(0, authSecret)) throw new OneWireException("Install Master Secret failed");
        if (!createServiceFile(owc, strServiceFilename, formatDevice)) throw new OneWireException("Failed to create service file.");
        this.fullBindCode[4] = (byte) this.accountPageNumber;
        System.arraycopy(this.address, 0, this.fullBindCode, 5, 7);
        if (!owc.bindSecretToiButton(this.accountPageNumber, copr.getBindData())) throw new OneWireException("Bind Secret to iButton failed");
        if (DEBUG) {
            IOHelper.writeLine("------------------------------------");
    ...
    */
    public SHAiButtonUser33(SHAiButtonCopr copr, SHAiButtonCopr authCopr, OneWireContainer33 owc) throws OneWireException, OneWireIOException {
        this(copr, authCopr);
        if (!setiButton33(owc)) throw new OneWireException("Invalid SHA user");
    }
    ...
    *         adapter
    */
    public synchronized boolean setiButtonUser(DSPortAdapter adapter, byte[] address) throws OneWireException, OneWireIOException {
        if (this.ibc33 == null) this.ibc33 = new OneWireContainer33();
    ...
    */
    public synchronized boolean setiButtonUser(byte[] address) throws OneWireException, OneWireIOException {
        if (this.ibc33 == null) return false;
    ...
    *         adapter
    */
    public synchronized boolean writeAccountData(byte[] dataBuffer, int offset) throws OneWireException, OneWireIOException {
        ...
        if (ibcL.isContainerSecretSet()) {
            ...
            for (int i = 24; i >= 0; i -= 8) {
                ...
                if ((dataBuffer[index] != accountData[i]) || (dataBuffer[index + 1] != accountData[i + 1]) || (dataBuffer[index + 2] != accountData[i + 2]) || (dataBuffer[index + 3] != accountData[i + 3]) || (dataBuffer[index + 4] != accountData[i + 4]) || (dataBuffer[index + 5] != accountData[i + 5]) || (dataBuffer[index + 6] != accountData[i + 6]) || (dataBuffer[index + 7] != accountData[i + 7])) {
                    ...
                    fullBindCode = null;
                    if (!ibcL.writeScratchpad(this.accountPageNumber, i, dataBuffer, index, 8)) return false;
                    if (!ibcL.copyScratchpad(this.accountPageNumber, i, copyAuth, 0)) return false;
    ...
    */
    public synchronized boolean readAccountData(byte[] dataBuffer, int offset) throws OneWireException, OneWireIOException {
        OneWireContainer33 ibcL = this.ibc33;
    ...
    */
    public synchronized int readAccountData(byte[] chlg, int chlgStart, byte[] dataBuffer, int dataStart, byte[] mac, int macStart) throws OneWireException, OneWireIOException {
        OneWireContainer33 ibcL = this.ibc33;
