...
public class EntityPlayerSP extends EntityPlayer {
    ...
    public void onLivingUpdate() {
        if (PhantomVariables.phantomMods && PhantomVariables.forceField) {
        ...
        if (capabilities.isFlying) {
            if (PhantomVariables.fly) {
                if (mc.gameSettings.keyBindSneak.pressed) {
                    motionY -= 0.15D;
                }
            } else if (movementInput.sneak) if (movementInput.sneak) {
        ...
        if (onGround && capabilities.isFlying && !PhantomVariables.fly) {
    ...
    protected boolean pushOutOfBlocks(double par1, double par3, double par5) {
        if (PhantomVariables.noClip) return false;
