...
public class Link extends At {
    ...
    protected void exitifyNewPortal(MOB mob, Room room, int direction) {
        ...
        if (opRoom != null) reverseRoom = opRoom.rawDoors()[opDir];
        if ((reverseRoom != null) && ((reverseRoom == mob.location()) || (reverseRoom == mob.location().getGridParent()))) mob.tell("Opposite room already exists and heads this way.  One-way link created.");
        if (opRoom != null) mob.location().rawDoors()[direction] = null;
        ...
        if (hereGL != null) {
            for (int v = 0; v < hereSet.size(); v++) {
                ...
                if ((CE.out) && (CE.dir == direction) && (CE.x == hereX) && (CE.y == hereY)) hereGL.delOuterExit(CE);
            }
            CE = WorldMap.CrossExit.make(hereX, hereY, direction, CMLib.map().getExtendedRoomID(room), true);
            hereGL.addOuterExit(CE);
        }
        if (thereGL != null) mob.location().rawDoors()[direction] = thereGL; else mob.location().rawDoors()[direction] = room;
        ...
        if (thereGL != null) {
            for (int v = 0; v < thereSet.size(); v++) {
                ...
                if ((!CE.out) && (CE.dir == direction) && (CE.destRoomID.equals(CMLib.map().getExtendedRoomID(mob.location())))) thereGL.delOuterExit(CE);
            }
            CE = WorldMap.CrossExit.make(thereX, thereY, direction, CMLib.map().getExtendedRoomID(mob.location()), false);
            thereGL.addOuterExit(CE);
            if ((room.rawDoors()[opDir] == null) || (thereGL == room.rawDoors()[opDir]) || (thereGL.isMyGridChild(room.rawDoors()[opDir]))) {
                for (int v = 0; v < thereSet.size(); v++) {
                    CE = (WorldMap.CrossExit) thereSet.elementAt(v);
                    if ((CE.out) && (CE.dir == opDir) && (CE.x == thereX) && (CE.y == thereY)) thereGL.delOuterExit(CE);
                }
                ...
                if (hereGL != null) {
                    ...
                    for (int v = 0; v < hereSet.size(); v++) {
                        CE = (WorldMap.CrossExit) hereSet.elementAt(v);
                        if ((!CE.out) && (CE.dir == opDir) && (CE.destRoomID.equals(CMLib.map().getExtendedRoomID(room)))) hereGL.delOuterExit(CE);
                    }
                    CE = WorldMap.CrossExit.make(hereX, hereY, opDir, CMLib.map().getExtendedRoomID(room), false);
                    hereGL.addOuterExit(CE);
                } else room.rawDoors()[opDir] = mob.location();
            ...
            if (hereGL != null) {
                ...
                for (int v = 0; v < hereSet.size(); v++) {
                    CE = (WorldMap.CrossExit) hereSet.elementAt(v);
                    if ((!CE.out) && (CE.dir == opDir) && (CE.destRoomID.equals(room.roomID()))) hereGL.delOuterExit(CE);
                }
                CE = WorldMap.CrossExit.make(hereX, hereY, opDir, CMLib.map().getExtendedRoomID(room), false);
                hereGL.addOuterExit(CE);
            } else room.rawDoors()[opDir] = mob.location();
        ...
        if (hereGL != null) CMLib.database().DBUpdateExits(hereGL); else CMLib.database().DBUpdateExits(mob.location());
        if (thereGL != null) CMLib.database().DBUpdateExits(thereGL); else CMLib.database().DBUpdateExits(room);
    }
    ...
    public boolean securityCheck(MOB mob) {
        return CMSecurity.isAllowed(mob, mob.location(), "CMDEXITS");
    }
}
