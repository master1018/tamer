...
public class GridInformationService extends GridSimCore {
    ...
    private LinkedList resList_;

    private LinkedList arList_;

    private LinkedList gisList_;

    private ArrayList netList_;
     ...
     * @post $none
     */
    public GridInformationService(String name, double baud_rate) throws Exception {
        ...
        while (Sim_system.running()) {
            ...
            switch(ev.get_tag()) {
                ...
                case GridSimTags.REQUEST_REGIONAL_GIS:
                    ...
                    super.send(super.output, 0.0, ev.get_tag(), new IO_data(gisList_, SIZE * gisList_.size(), id));
                    break;
                ...
                case GridSimTags.RESOURCE_LIST:
                    id = ((Integer) ev.get_data()).intValue();
                    super.send(super.output, 0.0, ev.get_tag(), new IO_data(resList_, SIZE * resList_.size(), id));
                    break;
                case GridSimTags.RESOURCE_AR_LIST:
                    id = ((Integer) ev.get_data()).intValue();
                    super.send(super.output, 0.0, ev.get_tag(), new IO_data(arList_, SIZE * arList_.size(), id));
                    break;
    ...
    protected void processOtherEvent(Sim_event ev) {
        if (ev == null) {
            System.out.println("GridInformationService.processOtherEvent(): " + "Unable to handle a request since the event is null.");
            return;
        }
        System.out.println("GridInformationSevice.processOtherEvent(): " + "Unable to handle a request from " + GridSim.getEntityName(ev.get_src()) + " with event tag = " + ev.get_tag());
    }
    ...
    private void notifyAllEntity() {
        System.out.println(super.get_name() + ": Notify all GridSim entities for shutting down.");
