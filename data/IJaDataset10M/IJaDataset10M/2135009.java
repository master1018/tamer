...
public class Home3DAttributesController implements Controller {
   ...
   */
    public enum Property {
        ...
        OBSERVER_FIELD_OF_VIEW_IN_DEGREES, OBSERVER_HEIGHT, GROUND_COLOR, GROUND_PAINT, SKY_COLOR, SKY_PAINT, LIGHT_COLOR, WALLS_ALPHA
    }
   ...
   */
    public TextureChoiceController getGroundTextureController() {
        if (this.groundTextureController == null) {
            this.groundTextureController = new TextureChoiceController(this.preferences.getLocalizedString(Home3DAttributesController.class, "groundTextureTitle"), this.preferences, this.viewFactory, this.contentManager);
   ...
   */
    public TextureChoiceController getSkyTextureController() {
        if (this.skyTextureController == null) {
            this.skyTextureController = new TextureChoiceController(this.preferences.getLocalizedString(Home3DAttributesController.class, "skyTextureTitle"), this.preferences, this.viewFactory, this.contentManager);
   ...
   */
    protected void updateProperties() {
        setObserverFieldOfViewInDegrees((int) (Math.round(Math.toDegrees(this.home.getObserverCamera().getFieldOfView())) + 360) % 360);
   ...
   */
    public void setObserverFieldOfViewInDegrees(int observerFieldOfViewInDegrees) {
        if (observerFieldOfViewInDegrees != this.observerFieldOfViewInDegrees) {
            ...
            this.propertyChangeSupport.firePropertyChange(Property.OBSERVER_FIELD_OF_VIEW_IN_DEGREES.name(), oldObserverFieldOfViewInDegrees, observerFieldOfViewInDegrees);
        }
    }
   ...
   */
    public void modify3DAttributes() {
        ...
        HomeTexture groundTexture = getGroundPaint() == EnvironmentPaint.TEXTURED ? getGroundTextureController().getTexture() : null;
        int skyColor = getSkyColor();
        HomeTexture skyTexture = getSkyPaint() == EnvironmentPaint.TEXTURED ? getSkyTextureController().getTexture() : null;
        ...
        doModify3DAttributes(home, observerCameraFieldOfView, observerCameraZ, groundColor, groundTexture, skyColor, skyTexture, lightColor, wallsAlpha);
        if (this.undoSupport != null) {
            UndoableEdit undoableEdit = new Home3DAttributesModificationUndoableEdit(this.home, this.preferences, oldObserverCameraFieldOfView, oldObserverCameraZ, oldGroundColor, oldGroundTexture, oldSkyColor, oldSkyTexture, oldLightColor, oldWallsAlpha, observerCameraFieldOfView, observerCameraZ, groundColor, groundTexture, skyColor, skyTexture, lightColor, wallsAlpha);
   ...
   */
    private static class Home3DAttributesModificationUndoableEdit extends AbstractUndoableEdit {
        ...
        private Home3DAttributesModificationUndoableEdit(Home home, UserPreferences preferences, float oldObserverCameraFieldOfView, float oldObserverCameraZ, int oldGroundColor, HomeTexture oldGroundTexture, int oldSkyColor, HomeTexture oldSkyTexture, int oldLightColor, float oldWallsAlpha, float observerCameraFieldOfView, float observerCameraZ, int groundColor, HomeTexture groundTexture, int skyColor, HomeTexture skyTexture, int lightColor, float wallsAlpha) {
            this.home = home;
            this.preferences = preferences;
        ...
        public void undo() throws CannotUndoException {
            super.undo();
            doModify3DAttributes(this.home, this.oldObserverCameraFieldOfView, this.oldObserverCameraZ, this.oldGroundColor, this.oldGroundTexture, this.oldSkyColor, this.oldSkyTexture, this.oldLightColor, this.oldWallsAlpha);
        }

        @Override
        public void redo() throws CannotRedoException {
            super.redo();
            doModify3DAttributes(this.home, this.observerCameraFieldOfView, this.observerCameraZ, this.groundColor, this.groundTexture, this.skyColor, this.skyTexture, this.lightColor, this.wallsAlpha);
        }
   ...
   */
    private static void doModify3DAttributes(Home home, float observerCameraFieldOfView, float observerCameraZ, int groundColor, HomeTexture groundTexture, int skyColor, HomeTexture skyTexture, int lightColor, float wallsAlpha) {
