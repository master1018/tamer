...
public class TooltipBehavior implements ViewBehavior {
    ...
    protected static final Logger logger = Logger.getLogger(TooltipBehavior.class);
    ...
    protected class TooltipEventHandler extends PBasicInputEventHandler {
        ...
        public void showTooltip() {
            ...
            if (currentTooltip != null && lastMousePos != null) {
                ...
                tooltipHolder.setBubbleOffset(new Point2D.Double(0, canvas.getCamera().getHeight() - tooltipHolder.getBubbleBounds().getHeight()));
                tooltipHolder.setTooltipFlareVisible(tooltipFlareVisible);
                canvas.getCamera().addChild(tooltipHolder);
                final PActivity a = tooltipHolder.animateScaleInFromPoint(getTooltipFadeInTime());
                canvas.getCamera().addActivity(a);
            }
        }

        public void hideTooltip(final PNode destroyedTooltip, final PNode destroyedHolder) {
            if (destroyedHolder != null) {
                ...
                activity.setDelegate(new PActivityDelegate() {
                    ...
                    public void activityFinished(PActivity arg0) {
                        if (canvas.getCamera().getChildrenReference().contains(destroyedHolder)) canvas.getCamera().removeChild(destroyedHolder);
                    }
        ...
        public void mouseEntered(PInputEvent event) {
            ...
            for (int i = path.getNodeStackReference().size() - 1; i >= 0; i--) {
                ...
                TooltipFactory tf = (TooltipFactory) node.getAttribute(TooltipFactory.KEY);
                if (tf != null) {
                    lastEntered = node;
                    lastTooltipFactory = tf;
                    long delay = tf.getDelay();
                    popupTimer.setInitialDelay((int) Math.min(delay, getTooltipVisibleDelay()));
                    popupTimer.start();
                    return;
                }
            }
