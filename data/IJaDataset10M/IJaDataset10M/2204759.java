...
public class SpringFactory implements FlexFactory {
    ...
    public FactoryInstance createFactoryInstance(String id, ConfigMap properties) {
        ...
        if (Log.isDebug()) Log.getLogger("Configuration").debug("SpringFactory creating factory instance for destination: " + id + " with spring id: " + instance.getSource() + " with properties: " + properties);
        return instance;
    }
    ...
    static class SpringFactoryInstance extends FactoryInstance {
        ...
        public Object lookup() {
            if (applicationInstance != null) return applicationInstance;
            ApplicationContext appContext = WebApplicationContextUtils.getWebApplicationContext(flex.messaging.FlexContext.getServletConfig().getServletContext());
            if (appContext == null) {
                if (Log.isError()) Log.getLogger("Configuration").error("SpringFactory - not able to look up the WebApplicationContext for Spring.  Check your web.xml to ensure Spring is installed properly in this web application.");
                return null;
            }
            ...
            try {
                ...
                if (appContext.isSingleton(beanName)) {
                    if (Log.isDebug()) Log.getLogger("Configuration").debug("SpringFactory creating singleton component with spring id: " + beanName + " for destination: " + getId());
                    synchronized (this) {
                        if (applicationInstance != null) return applicationInstance;
                        inst = appContext.getBean(beanName);
                        if (inst instanceof FlexConfigurable) ((FlexConfigurable) inst).initialize(getId(), getProperties());
                        applicationInstance = inst;
                    }
                } else {
                    if (Log.isDebug()) Log.getLogger("Configuration").debug("SpringFactory creating non-singleton component with spring id: " + beanName + " for destination: " + getId());
                    inst = appContext.getBean(beanName);
                    if (inst instanceof FlexConfigurable) ((FlexConfigurable) inst).initialize(getId(), getProperties());
                }
                return inst;
