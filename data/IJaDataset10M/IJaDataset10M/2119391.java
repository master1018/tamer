...
import com.dotmarketing.factories.GroupFactory;
...
public class WebFormFactory {
    ...
    public static void getReportHeaderAndData(List<WebForm> webForms, List<String> reportHeaders, List<String> customHeaders, List<List<String>> reportData) {
        for (WebForm webForm : webForms) {
            if (!reportHeaders.contains("Title") && UtilMethods.isSet(webForm.getTitle())) reportHeaders.add("Title");
            if (!reportHeaders.contains("Prefix") && UtilMethods.isSet(webForm.getPrefix())) reportHeaders.add("Prefix");
            if (!reportHeaders.contains("First Name") && UtilMethods.isSet(webForm.getFirstName())) reportHeaders.add("First Name");
            if (!reportHeaders.contains("Middle Initial") && UtilMethods.isSet(webForm.getMiddleInitial())) reportHeaders.add("Middle Initial");
            if (!reportHeaders.contains("Middle Name") && UtilMethods.isSet(webForm.getMiddleName())) reportHeaders.add("Middle Name");
            if (!reportHeaders.contains("Last Name") && UtilMethods.isSet(webForm.getLastName())) reportHeaders.add("Last Name");
            if (!reportHeaders.contains("Full Name") && UtilMethods.isSet(webForm.getFullName())) reportHeaders.add("Full Name");
            if (!reportHeaders.contains("Organization") && UtilMethods.isSet(webForm.getOrganization())) reportHeaders.add("Organization");
            if (!reportHeaders.contains("Address") && UtilMethods.isSet(webForm.getAddress())) reportHeaders.add("Address");
            if (!reportHeaders.contains("Address 1") && UtilMethods.isSet(webForm.getAddress1())) reportHeaders.add("Address 1");
            if (!reportHeaders.contains("Address 2") && UtilMethods.isSet(webForm.getAddress2())) reportHeaders.add("Address 2");
            if (!reportHeaders.contains("City") && UtilMethods.isSet(webForm.getCity())) reportHeaders.add("City");
            if (!reportHeaders.contains("State") && UtilMethods.isSet(webForm.getState())) reportHeaders.add("State");
            if (!reportHeaders.contains("Zip") && UtilMethods.isSet(webForm.getZip())) reportHeaders.add("Zip");
            if (!reportHeaders.contains("Country") && UtilMethods.isSet(webForm.getCountry())) reportHeaders.add("Country");
            if (!reportHeaders.contains("Phone") && UtilMethods.isSet(webForm.getPhone())) reportHeaders.add("Phone");
            if (!reportHeaders.contains("Email") && UtilMethods.isSet(webForm.getEmail())) reportHeaders.add("Email");
            if (UtilMethods.isSet(webForm.getCustomFields())) {
                ...
                for (String field : fields) {
                    ...
                    if (splitted.length == 2) {
                        ...
                        if (!reportHeaders.contains(name)) reportHeaders.add(name);
                        if (!customHeaders.contains(name)) customHeaders.add(name);
                    }
                }
            }
        }
        for (WebForm webForm : webForms) {
            try {
                ...
                for (String header : subList) {
                    if (header.equals("Title")) row.add(UtilMethods.isSet(webForm.getTitle()) ? webForm.getTitle() : ""); else if (header.equals("Prefix")) row.add(UtilMethods.isSet(webForm.getPrefix()) ? webForm.getPrefix() : ""); else if (header.equals("First Name")) row.add(UtilMethods.isSet(webForm.getFirstName()) ? webForm.getFirstName() : ""); else if (header.equals("Middle Initial")) row.add(UtilMethods.isSet(webForm.getMiddleInitial()) ? webForm.getMiddleInitial() : ""); else if (header.equals("Middle Name")) row.add(UtilMethods.isSet(webForm.getMiddleName()) ? webForm.getMiddleName() : ""); else if (header.equals("Last Name")) row.add(UtilMethods.isSet(webForm.getLastName()) ? webForm.getLastName() : ""); else if (header.equals("Full Name")) row.add(UtilMethods.isSet(webForm.getFullName()) ? webForm.getFullName() : ""); else if (header.equals("Organization")) row.add(UtilMethods.isSet(webForm.getOrganization()) ? webForm.getOrganization() : ""); else if (header.equals("Address")) row.add(UtilMethods.isSet(webForm.getAddress()) ? webForm.getAddress() : ""); else if (header.equals("Address 1")) row.add(UtilMethods.isSet(webForm.getAddress1()) ? webForm.getAddress1() : ""); else if (header.equals("Address 2")) row.add(UtilMethods.isSet(webForm.getAddress2()) ? webForm.getAddress2() : ""); else if (header.equals("City")) row.add(UtilMethods.isSet(webForm.getCity()) ? webForm.getCity() : ""); else if (header.equals("State")) row.add(UtilMethods.isSet(webForm.getState()) ? webForm.getState() : ""); else if (header.equals("Zip")) row.add(UtilMethods.isSet(webForm.getZip()) ? webForm.getZip() : ""); else if (header.equals("Country")) row.add(UtilMethods.isSet(webForm.getCountry()) ? webForm.getCountry() : ""); else if (header.equals("Phone")) row.add(UtilMethods.isSet(webForm.getPhone()) ? webForm.getPhone() : ""); else if (header.equals("Email")) row.add(UtilMethods.isSet(webForm.getEmail()) ? webForm.getEmail() : ""); else {
    ...
    public static String getReportCode(List<String> reportHeaders, List<String> customHeaders, List<List<String>> reportData, String reportName, String reportComments, Date reportDate) {
        ...
        int dataColums = 0;
        if (dataSize > 0) dataColums = (reportData.get(0)).size();
    ...
    public static void emailExcelWebForms(String groupName, String reportType, Date initialDate, Date finalDate, String subject, String fromEmail, String fromName) {
        try {
            StringBuffer emailsSB = new StringBuffer();
            Group group = GroupFactory.getGroupByName(groupName);
            List<User> users = GroupFactory.getAllUsersInGroup(group.getGroupId());
            for (User user : users) {
