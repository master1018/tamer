...
public class ConcurrentStatisticCalculator extends StatisticCalculator {
        ...
        try {
            ...
            for (int i = 0; i < indexes.length - 1; ++i) {
                ...
                tasks.add(new Callable<Statistic<T>>() {
                    ...
                    @Override
                    public Statistic<T> call() {
                        return ConcurrentStatisticCalculator.super.evaluate(population.subList(indexes[idx], indexes[idx + 1]));
                    }
                });
            }
            try {
    ...
    private static <T extends Gene<?>> Statistic<T> join(final List<Future<Statistic<T>>> results) throws InterruptedException, ExecutionException {
        if (results.isEmpty()) {
            return new Statistic<T>(null, null, 0.0, 0.0, 0.0, 0.0);
        }
        ...
        return new Statistic<T>(bestPhenotype, worstPhenotype, meanFitness, varianceFitness, meanAge, varianceAge);
    }
}
