...
public class pdclaslom extends ventanaPad implements PAD {
    ...
    CGridEditable jt = new CGridEditable(3) {
        ...
        public void cambiaColumna(int col, int row) {
            try {
                if (col == 0) {
                    String nombArt = pro_codiE.getNombArt(pro_codiE.getText());
                    if (nombArt == null) jt.setValor("**PRODUCTO NO ENCONTRADO**", row, 1); else jt.setValor(nombArt, row, 1);
                }
            } catch (Exception k) {
    ...
    public pdclaslom(EntornoUsuario eu, Principal p, Hashtable ht) {
        ...
        jf = p;
        eje = true;
        setTitulo("PAD Clasificacion de Lomos (V 1.0)");
        try {
            if (ht != null) {
    ...
    private void jbInit() throws Exception {
        iniciar(502, 522);
        strSql = "SELECT cll_codi from claslomos " + " group by cll_codi " + " order by cll_codi ";
        ...
        cabecera.addElement("Producto");
        cabecera.addElement("Nombre");
        cabecera.addElement("Kilos Min");
        ...
        cLabel4.setText("Tipo");
        cLabel4.setBounds(new Rectangle(186, 3, 29, 16));
        cll_codiE.setBounds(new Rectangle(218, 3, 42, 17));
        this.getContentPane().add(nav, BorderLayout.NORTH);
        this.getContentPane().add(statusBar, BorderLayout.SOUTH);
        Pprinc.add(Bcancelar, new GridBagConstraints(1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 0, 0, 50), 0, 0));
        Pprinc.add(Baceptar, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 50, 0, 0), 0, 0));
        Pprinc.add(jt, new GridBagConstraints(0, 1, 2, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 2, 0, 0), 0, 0));
        Pprinc.add(Pcabe, new GridBagConstraints(0, 0, 2, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 0, 0), 0, 0));
    ...
    void irGrid() {
        if (nav.pulsado != navegador.ADDNEW && nav.pulsado != navegador.EDIT) return;
        if (cll_codiE.isNull()) {
        ...
        try {
            if (nav.pulsado == navegador.ADDNEW) {
                s = "SELECT * FROM claslomos WHERE " + " cll_codi = " + cll_codiE.getValorInt();
                if (dtCon1.select(s)) {
    ...
    void guardaDatos(int cllCodi) {
        ...
        try {
            ...
            for (int n = 0; n < nRow; n++) {
                if (jt.getValInt(n, 0) == 0) continue;
                dtAdd.addNew();
    ...
    void borDatos(int cllCodi) throws SQLException, java.text.ParseException {
        s = "DELETE FROM claslomos " + " WHERE cll_codi =" + cllCodi;
    ...
    void verDatLin(int cllCodi) throws Exception {
        s = "SELECT l.pro_codi,p.pro_nomb,l.cll_kilos " + " FROM claslomos l,v_articulo p" + " WHERE l.cll_codi = " + cllCodi + " and l.pro_codi = p.pro_codi " + " order by l.cll_kilos";
    ...
    void activar(int modo, boolean act) {
        if (modo == navegador.TODOS) jt.setEnabled(act);
    ...
    public void ej_query1() {
        ...
        s = creaWhere(s, v);
        s += " group by cll_codi" + " order by cll_codi";
        try {
            if (!dtCons.select(s)) {
                mensaje("");
    ...
    int cambiaLineaJT() {
        if (pro_codiE.getValorInt() == 0) return -1;
        try {
