...
public class DiskIndex {
    ...
    public byte[] lookup(byte[] key) {
        ...
        int endBlockOffset;
        if (indexPosition == blockIndex.getNumEntries() - 1) endBlockOffset = -1; else {
    ...
    public long numKeys() {
        ...
        try {
            lastBlock = mmaped ? getBlock(lastBlockStartOffset, lastBlockEndOffset, dbFiles[getBlockFileId(numBlocks - 1, blockIndex)]) : getBlock(lastBlockStartOffset, lastBlockEndOffset, dbFileChannels[getBlockFileId(numBlocks - 1, blockIndex)]);
        } catch (IOException e) {
            Logging.logError(Logging.LEVEL_ERROR, this, e);
        }
        long lastBlockEntryCount = lastBlock.getNumEntries();
        lastBlock.free();
        if (numBlocks == 1) return lastBlockEntryCount;
        int firstBlockStartOffset = 0;
        int firstBlockEndBlockOffset = getBlockOffset(1, blockIndex);
        if (getBlockFileId(1, blockIndex) > getBlockFileId(0, blockIndex)) firstBlockEndBlockOffset = -1;
        BlockReader firstBlock = null;
        try {
            firstBlock = mmaped ? getBlock(firstBlockStartOffset, firstBlockEndBlockOffset, dbFiles[getBlockFileId(0, blockIndex)]) : getBlock(firstBlockStartOffset, firstBlockEndBlockOffset, dbFileChannels[getBlockFileId(0, blockIndex)]);
        } catch (IOException e) {
            Logging.logError(Logging.LEVEL_ERROR, this, e);
        }
    ...
    public ResultSet<byte[], byte[]> rangeLookup(final byte[] from, final byte[] to, final boolean ascending) {
        if (mmaped) {
        ...
        } else return new DiskIndexIterator(this, blockIndex, from, to, ascending, dbFileChannels);
    }

    public InternalDiskIndexIterator internalRangeLookup(final byte[] from, final byte[] to, final boolean ascending) {
        if (mmaped) {
            final ByteBuffer[] map = new ByteBuffer[dbFiles.length];
            for (int i = 0; i < dbFiles.length; i++) {
                dbFiles[i].position(0);
        ...
        } else return new InternalDiskIndexIterator(this, blockIndex, from, to, ascending, dbFileChannels);
    }
    ...
    protected BlockReader getBlock(int startBlockOffset, int endBlockOffset, ByteBuffer map) {
        if (startBlockOffset > map.limit()) return null;
        if (endBlockOffset == -1) endBlockOffset = map.limit();
    ...
    protected BlockReader getBlock(int startBlockOffset, int endBlockOffset, FileChannel channel) throws IOException {
        if (startBlockOffset > channel.size()) return null;
        if (endBlockOffset == -1) endBlockOffset = (int) channel.size();
        BlockReader targetBlock;
        if (compressed) {
