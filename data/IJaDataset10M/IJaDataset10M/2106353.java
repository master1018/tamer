...
public class Periodic {
	 ...
	 * executon on.
	 */
    public Periodic(Runnable r, SchedulingThreadPool scheduler) {
    ...
    public synchronized boolean rescheduleIfLater(long newDelay) {
        ...
        if (future == null) {
            ...
            if (newDelay > 0) future = scheduler.invokeLater(d, newDelay); else scheduler.invokeLater(d);
            return true;
    ...
    public synchronized boolean rescheduleIfSooner(long newDelay) {
        if (future == null) return rescheduleIfLater(newDelay);
        long newTime = System.currentTimeMillis() + newDelay;
        if (newTime < nextExecuteTime) {
            future.cancel(false);
            nextExecuteTime = newTime;
            if (newDelay > 0) future = scheduler.invokeLater(d, newDelay); else {
                future = null;
