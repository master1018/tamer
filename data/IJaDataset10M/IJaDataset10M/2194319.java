...
public class SbEncoder extends SbCodec implements Encoder {
   ...
   */
    public int encode(final Bits bits, final float[] in) {
        ...
        if (low_mode == 0) dtx = 1; else dtx = 0;
        ...
        Lpc.wld(lpc, autocorr, rc, lpcSize);
        ...
        if ((vbr_enabled != 0 || vad_enabled != 0) && dtx == 0) {
            ...
            if (vbr_enabled != 0) {
                ...
                while (modeid != 0) {
                    ...
                    if (v1 == 10) thresh = Vbr.hb_thresh[modeid][v1]; else thresh = (vbr_quality - v1) * Vbr.hb_thresh[modeid][v1 + 1] + (1 + v1 - vbr_quality) * Vbr.hb_thresh[modeid][v1];
                ...
                int modeid;
                if (relative_quality < 2.0) modeid = 1; else modeid = submodeSelect;
                submodeID = modeid;
            }
        }
        bits.pack(1, 1);
        if (dtx != 0) bits.pack(0, SB_SUBMODE_BITS); else bits.pack(submodeID, SB_SUBMODE_BITS);
        if (dtx != 0 || submodes[submodeID] == null) {
            ...
            if (dtx != 0) return 0; else return 1;
        }
        ...
        for (int sub = 0; sub < nbSubframes; sub++) {
            ...
            if (submodes[submodeID].innovation == null) {
                ...
                if (submodes[submodeID].double_codebook != 0) {
                    ...
                    for (i = 0; i < subframeSize; i++) target[i] *= 2.5f;
                    ...
                    for (i = 0; i < subframeSize; i++) innov2[i] *= scale * (1f / 2.5f);
   ...
   */
    public int getBitRate() {
        if (submodes[submodeID] != null) return lowenc.getBitRate() + sampling_rate * submodes[submodeID].bits_per_frame / frameSize; else return lowenc.getBitRate() + sampling_rate * (SB_SUBMODE_BITS + 1) / frameSize;
    }
