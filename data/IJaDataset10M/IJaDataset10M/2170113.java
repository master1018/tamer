...
/**
 * Ein globales Signal das ausgel�st wird, kurz bevor sich eine Datei �ndert.
 */
public class BeforeFileChangedSignal extends QObject {

    private static Logger logger = Logger.getLogger("");

    Signal1<String> signal = new Signal1<String>();

    private static BeforeFileChangedSignal instance = new BeforeFileChangedSignal();

    private BeforeFileChangedSignal() {
    }

    public static BeforeFileChangedSignal getInstance() {
        return instance;
    }
    ...
    public static void emit(String path) {
        ...
        if (knownAsMetadataFile) {
            BeforeMetadataFileEditedSignal.emit(Model.getMetadataFiles().get(path));
        }
        if (!exists && known) {
