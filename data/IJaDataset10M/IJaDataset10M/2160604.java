...
public class LanguageRule {
    ...
    public List<String> split(String text, Settings settings) {
        ...
        PriorityQueue<RuleMatcher> queue = new PriorityQueue<RuleMatcher>(ruleList.size(), RuleMatcherComparator.getInstance());
        ...
        while (queue.size() > 0) {
            ...
            if (matcher.getRule().isBreaking()) {
                String segment = text.substring(start, matcher.getBreakPosition());
            ...
            while ((queue.size() > 0) && (matcher.getBreakPosition() < position)) {
                queue.remove();
