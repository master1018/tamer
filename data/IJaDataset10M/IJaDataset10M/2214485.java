...
public class PMDReporter implements Reporter, ReportListener {
    ...
    public void ruleViolationAdded(IRuleViolation pRuleViolation) {
        ...
        if (fCurrentSourceFile != null) {
            metaData = fCurrentSourceFile.getBestMatch(pRuleViolation.getClassName(), pRuleViolation.getMethodName(), pRuleViolation.getBeginLine());
        }
        Audit audit = new Audit(metaData, pRuleViolation.getRule().getName(), pRuleViolation.getRule().getPriorityName(), pRuleViolation.getDescription(), fCurrentVersion);
