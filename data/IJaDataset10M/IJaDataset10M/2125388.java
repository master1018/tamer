...
public class Clock {
    ...
    public String getDateString() {
        ...
        return toWeekString(cal.get(Calendar.DAY_OF_WEEK)) + ", " + toMonthString(cal.get(Calendar.MONTH)) + " " + Integer.toString(cal.get(Calendar.DATE)) + ", " + toClockString(cal.get(Calendar.YEAR) % 100);
    }

    public String getTimeString() {
        Calendar cal = getCalendar();
        return toClockString(cal.get(Calendar.HOUR)) + (((cal.get(Calendar.SECOND) % 2) == 0) ? ":" : " ") + toClockString(cal.get(Calendar.MINUTE));
    }
