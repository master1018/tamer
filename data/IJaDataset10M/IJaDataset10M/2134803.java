...
public class ContextAddDialog extends EnhancedDialog {
    ...
    public ContextAddDialog(Component comp) {
        ...
        for (int i = 0; i < actionsList.length; i++) {
            ...
            if (actionSet.getActionCount() != 0) actionSets.add(actionSet);
        }
    ...
    public String getSelection() {
        if (!isOK) return null;
        if (separator.isSelected()) return "-"; else if (action.isSelected()) {
            return ((AbstractContextOptionPane.MenuItem) list.getSelectedValue()).actionName;
    ...
    class ActionHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (source == ok) ok(); else if (source == cancel) cancel(); else if (source == combo) updateList();
        }
    }
}
