public class RicochetTest {
    ...
    public void testNull() throws Throwable {
        if (testRepetition > (1+REPEAT/100))  return;  
    ...
    private static int opI2(int x, int y) { return opI(x, y); }  
    ...
    private static void stress() {
        if (STRESS <= 0) return;
        int count = STRESS + (STRESS_COUNT++ & 0x1);  
