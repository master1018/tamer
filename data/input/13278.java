public class MethodHandlesTest {
    ...
    static final Object[][][] ACCESS_CASES = {
        { { false, PUBLIC }, { false, PACKAGE }, { false, PRIVATE }, { false, EXAMPLE } }, 
        { { false, PUBLIC }, { false, PACKAGE }, { true,  PRIVATE }, { true,  EXAMPLE } }, 
        { { false, PUBLIC }, { true,  PACKAGE }, { true,  PRIVATE }, { true,  EXAMPLE } }, 
        { { true,  PUBLIC }, { true,  PACKAGE }, { true,  PRIVATE }, { true,  EXAMPLE } }, 
    };
    ...
    static boolean testModeMatches(int testMode, boolean isStatic) {
        ...
        case TEST_UNREFLECT:            return true;  
        }
    ...
    public void testSpreadArguments() throws Throwable {
        ...
        for (Class<?> argType : new Class[]{Object.class, Integer.class, int.class}) {
            ...
            for (int nargs = 0; nargs < 10; nargs++) {
                if (argType == int.class && nargs >= 6)  continue; 
                for (int pos = 0; pos < nargs; pos++) {
                    if (argType == int.class && pos > 0)  continue; 
    ...
    public void testInsertArguments() throws Throwable {
        ...
        for (int nargs = 0; nargs <= 4; nargs++) {
            for (int ins = 0; ins <= 4; ins++) {
                if (ins > MAX_ARG_INCREASE)  continue;  
                for (int pos = 0; pos <= nargs; pos++) {
        ...
        for (int i = 0; i <= 6; i++) {
            ...
            for (Class<?> argType : new Class[]{Object.class, Integer.class, int.class}) {
                for (int j = -1; j < i; j++) {
                    ...
                    if (argType.isPrimitive() && j != i-1)  continue; 
                    if (done.add(type))
                        testInvokers(type);
