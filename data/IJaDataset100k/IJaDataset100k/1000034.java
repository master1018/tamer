...
public class ChartGenerator_JTable {
    ...
    public JFreeChart createEmptyChart(String title) {
        JFreeChart chart = ChartFactory.createXYLineChart(title, null, null, null, PlotOrientation.VERTICAL, false, true, false);
        return chart;
    }
    ...
    public JFreeChart getCategoryChart(String chartType, String title, String xLabel, String yLabel, JTable table, int numberOfCategory, int[][] pairs, String other) {
        String type;
        if (chartType == null || chartType.length() == 0) type = "bar"; else type = chartType.toLowerCase();
        dataTable = table;
        columnModel = dataTable.getColumnModel();
    ...
    public JFreeChart getXYChart(String chartType, String title, String xLabel, String yLabel, JTable table, int numberOfSeries, int[][] pairs, String other) {
        String type;
        if (chartType == null || chartType.length() == 0) type = "line"; else type = chartType.toLowerCase();
        dataTable = table;
        columnModel = dataTable.getColumnModel();
    ...
    private void setOtherCondition(String otherCondition) {
        ...
        if (otherCondition.toLowerCase().indexOf("3d") != -1) dimension = "3D"; else dimension = "2D";
        if (otherCondition.toLowerCase().indexOf("counter_clockwise") != -1) rotation = "counter_clockwise"; else if (otherCondition.toLowerCase().indexOf("clockwise") != -1) rotation = "clockwise"; else if (otherCondition.toLowerCase().indexOf("ring") != -1) rotation = "ring"; else rotation = "";
        if (otherCondition.toLowerCase().indexOf("horizontal") != -1) orientation = PlotOrientation.HORIZONTAL; else if (otherCondition.toLowerCase().indexOf("vertical") != -1) orientation = PlotOrientation.VERTICAL; else orientation = PlotOrientation.VERTICAL;
        if (otherCondition.toLowerCase().indexOf("stacked") != -1) layout = "stacked"; else if (otherCondition.toLowerCase().indexOf("layered") != -1) layout = "layered"; else if (otherCondition.toLowerCase().indexOf("waterfall") != -1) layout = "waterfall"; else layout = "";
        if (otherCondition.toLowerCase().indexOf("year") != -1) timeType = "year"; else if (otherCondition.toLowerCase().indexOf("month") != -1) timeType = "month"; else if (otherCondition.toLowerCase().indexOf("day") != -1) timeType = "day"; else if (otherCondition.toLowerCase().indexOf("minute") != -1) timeType = "minute"; else timeType = "";
        lineCondition = "";
        if (otherCondition.toLowerCase().indexOf("noline") != -1) lineCondition += " noline";
        if (otherCondition.toLowerCase().indexOf("nofill") != -1) lineCondition += " nofill";
        if (otherCondition.toLowerCase().indexOf("noshape") != -1) lineCondition += " noshape";
        try {
    ...
    private DefaultPieDataset createPieDataset() {
        try {
            ...
            for (int index = 0; index < 1; index++) for (int i = 0; i < xyLength; i++) {
                if (indepValues[i][index] == null || indepValues[i][index] == "null") x[i][index] = 0.0; else x[i][index] = Double.parseDouble(indepValues[i][index]);
            }
            for (int index = 0; index < 1; index++) for (int i = 0; i < xyLength; i++) {
    ...
    private XYDataset createXYNDDataset() {
        ...
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) x[i][index] = indepValues[i][index];
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) y[i][index] = Double.parseDouble(depValues[i][index]);
    ...
    private XYDataset createXYDataset_Time(String time_type) {
        ...
        for (int i = 0; i < no_series; i++) {
            ...
            if (time_type.equalsIgnoreCase("Day")) series = new TimeSeries(serieName, Day.class); else if (time_type.equalsIgnoreCase("Minute")) series = new TimeSeries(serieName, Minute.class); else if (time_type.equalsIgnoreCase("Month")) series = new TimeSeries(serieName, Month.class); else series = new TimeSeries(serieName);
            for (int j = 0; j < xyLength; j++) {
                if (time_type.equalsIgnoreCase("Day")) series.add(DateParser.parseDay(x[j][i]), y[j][i]); else if (time_type.equalsIgnoreCase("Minute")) series.add(DateParser.parseMinute(x[j][i]), y[j][i]); else if (time_type.equalsIgnoreCase("Month")) series.add(DateParser.parseMonth(x[j][i]), y[j][i]);
            }
    ...
    private XYDataset createXYDataset_Index() {
        ...
        int row_count = 0;
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) {
        ...
        for (int i = 0; i < row_count; i++) series1.add(y_freq[i], raw_xvalue[i]);
    ...
    private XYDataset createXYDataset_Dot() {
        ...
        for (int i = 1; i < row_count; i++) {
            for (int j = 0; j < i; j++) if (raw_xvalue[i] == raw_xvalue[j]) y_freq[i] += 0.1;
        }
        XYSeriesCollection dataset = new XYSeriesCollection();
        XYSeries series1 = new XYSeries("X");
        for (int i = 0; i < row_count; i++) series1.add(y_freq[i], raw_xvalue[i]);
    ...
    private XYDataset createXYDataset() {
        ...
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) x[i][index] = indepValues[i][index];
        try {
            for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) if (depValues[i][index] != "null") y[i][index] = Double.parseDouble(depValues[i][index]);
        } catch (NumberFormatException e) {
            System.out.println("Data format error!");
            return null;
        }
        ...
        try {
            for (int i = 0; i < no_series; i++) {
                ...
                for (int j = 0; j < xyLength; j++) {
                    if (x[j][i] != "null" && x[j][i] != "NaN") series.add(Double.parseDouble(x[j][i]), y[j][i]);
                }
                dataset.addSeries(series);
            }
        } catch (NumberFormatException e) {
    ...
    private XYDataset createXYQQDataset() {
        ...
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) if (depValues[i][index] != "null") {
    ...
    private XYDataset createXYQQDDDataset() {
        ...
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) if (depValues[i][index] != "null") raw_y[i] = depValues[i][index];
    ...
    private TableXYDataset createTableXYDataset() {
        ...
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) x[i][index] = indepValues[i][index];
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) {
            if (depValues[i][index] == "null") y[i][index] = 0.0; else y[i][index] = Double.parseDouble(depValues[i][index]);
        }
    ...
    private BoxAndWhiskerCategoryDataset createBoxAndWhiskerDataset(int seriesCount, int categoryCount, String[] seriesName, String[][] categoryName, double[][][] values) {
        ...
        for (int s = 0; s < seriesCount; s++) {
            for (int c = 0; c < categoryCount; c++) {
                ...
                for (int i = 0; i < Array.getLength(values[s][c]); i++) list.add(new Double(values[s][c][i]));
    ...
    private CategoryDataset createCategoryDataset() {
        ...
        String[][] y = new String[xyLength][1];
        try {
            for (int index = 0; index < no_category; index++) for (int i = 0; i < xyLength; i++) x[i][index] = Double.parseDouble(indepValues[i][index]);
            for (int index = 0; index < 1; index++) for (int i = 0; i < xyLength; i++) y[i][index] = depValues[i][index];
            final DefaultCategoryDataset dataset = new DefaultCategoryDataset();
            for (int j = 0; j < xyLength; j++) for (int i = 0; i < no_category; i++) {
    ...
    private CategoryDataset createCategoryDataset_Time(String time_type) {
        ...
        String[][] y = new String[xyLength][1];
        for (int index = 0; index < no_category; index++) for (int i = 0; i < xyLength; i++) x[i][index] = indepValues[i][index];
        for (int index = 0; index < 1; index++) for (int i = 0; i < xyLength; i++) y[i][index] = depValues[i][index];
        final DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        for (int j = 0; j < xyLength; j++) for (int i = 0; i < no_category; i++) {
            if (x[j][i] != "NaN") {
    ...
    private CategoryDataset createCategoryStatDataset() {
        ...
        try {
            for (int index = 0; index < no_category; index++) for (int i = 0; i < xyLength; i++) {
    ...
    private BoxAndWhiskerCategoryDataset createBoxAndWhiskerCategoryDataset() {
        ...
        int CATEGORY_COUNT = no_category;
        String[][] values_storage = new String[SERIES_COUNT][CATEGORY_COUNT];
        for (int s = 0; s < SERIES_COUNT; s++) for (int c = 0; c < CATEGORY_COUNT; c++) {
    ...
    private IntervalXYDataset createIntervalXYDataset() {
        ...
        for (int i = 0; i < xyLength; i++) x[i] = Double.parseDouble(indepValues[i][0]);
        for (int i = 0; i < xyLength; i++) {
    ...
    private IntervalXYDataset createIntervalXYDataset(int bin_size) {
        ...
        int max_y = 0;
        for (int i = 0; i < row_count; i++) {
            if (raw_xvalue[i] > max_x) max_x = raw_xvalue[i];
            if (raw_xvalue[i] < min_x) min_x = raw_xvalue[i];
        }
    ...
    private IntervalXYDataset createIntervalXYDataset_Time(String timeType) {
        ...
        for (int index = 0; index < no_series; index++) {
            for (int i = 0; i < xyLength; i++) x[i][index] = indepValues[i][index];
        }
        for (int index = 0; index < no_series; index++) for (int i = 0; i < xyLength; i++) y[i][index] = Double.parseDouble(depValues[i][index]);
        ...
        for (int ind = 0; ind < no_series; ind++) {
            ...
            if (start_dep < 0) start_dep = 0;
            String serieName = independentHeaders[ind].substring(0, start_ind);
            if (serieName.length() == 0) serieName = "Serie" + ind;
            ...
            try {
                if (timeType.equalsIgnoreCase("year")) {
                    ...
                    for (int i = 0; i < xyLength; i++) series.add(new Year(Integer.parseInt(x[i][ind])), y[i][ind]);
                    collection.addSeries(series);
                } else if (timeType.equalsIgnoreCase("month")) {
                    series = new TimeSeries(serieName, Month.class);
                    for (int i = 0; i < xyLength; i++) series.add(DateParser.parseMonth(x[i][ind]), y[i][ind]);
                    collection.addSeries(series);
                } else if (timeType.equalsIgnoreCase("day")) {
                    series = new TimeSeries(serieName, Day.class);
                    for (int i = 0; i < xyLength; i++) series.add(DateParser.parseDay(x[i][ind]), y[i][ind]);
                    collection.addSeries(series);
                } else if (timeType.equalsIgnoreCase("minute")) {
                    series = new TimeSeries(serieName, Minute.class);
                    for (int i = 0; i < xyLength; i++) series.add(DateParser.parseMinute(x[i][ind]), y[i][ind]);
                    collection.addSeries(series);
                }
    ...
    private JFreeChart createPieChart(String titleLabel, PieDataset dataset) {
        if (dimension.equalsIgnoreCase("3D")) {
            JFreeChart chart = ChartFactory.createPieChart3D(titleLabel, dataset, true, true, false);
        ...
        if (rotation.equalsIgnoreCase("ring")) {
            JFreeChart chart = ChartFactory.createRingChart(titleLabel, dataset, false, true, false);
        ...
        JFreeChart chart = ChartFactory.createPieChart(titleLabel, dataset, true, true, false);
    ...
    private JFreeChart createXYLineChart(String title, String xLabel, String yLabel, XYDataset dataset) {
        JFreeChart chart = ChartFactory.createXYLineChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        ...
        if (lineCondition.indexOf("noshape") != -1) renderer.setBaseShapesVisible(false); else renderer.setBaseShapesVisible(true);
        if (lineCondition.indexOf("noline") != -1) renderer.setBaseLinesVisible(false);
    ...
    private JFreeChart createXYBarChart(String title, String xLabel, String yLabel, IntervalXYDataset dataset) {
        if (timeType.length() != 0) {
            JFreeChart chart = ChartFactory.createXYBarChart(title, xLabel, true, yLabel, dataset, orientation, true, true, false);
            chart.setBackgroundPaint(Color.white);
            XYPlot plot = chart.getXYPlot();
            ...
            JFreeChart chart = ChartFactory.createXYBarChart(title, xLabel, false, yLabel, dataset, orientation, true, true, false);
            chart.setBackgroundPaint(Color.white);
            XYPlot plot = chart.getXYPlot();
            plot.setRenderer(new ClusteredXYBarRenderer());
            XYItemRenderer renderer = plot.getRenderer();
    ...
    private JFreeChart createXYAreaChart(String title, String xLabel, String yLabel, XYDataset dataset) {
        JFreeChart chart = ChartFactory.createXYAreaChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        chart.setBackgroundPaint(Color.white);
        XYPlot plot = chart.getXYPlot();
        plot.setBackgroundPaint(Color.lightGray);
    ...
    private JFreeChart createTableXYAreaChart(String title, String xLabel, String yLabel, TableXYDataset dataset) {
        JFreeChart chart = ChartFactory.createStackedXYAreaChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
    ...
    private JFreeChart createCategoryLineChart(String title, String xLabel, String yLabel, CategoryDataset dataset) {
        JFreeChart chart = ChartFactory.createLineChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        chart.setBackgroundPaint(Color.white);
    ...
    private JFreeChart createCategoryBarChart(String title, String xLabel, String yLabel, CategoryDataset dataset) {
        JFreeChart chart;
        if (dimension.equalsIgnoreCase("3d")) {
            chart = ChartFactory.createBarChart3D(title, xLabel, yLabel, dataset, orientation, true, true, false);
        ...
        if (layout.equalsIgnoreCase("stacked")) {
            chart = ChartFactory.createStackedBarChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        } else if (layout.equalsIgnoreCase("waterfall")) {
            chart = ChartFactory.createWaterfallChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
            CategoryPlot plot = chart.getCategoryPlot();
            ...
            chart = ChartFactory.createBarChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        }
        chart.setBackgroundPaint(Color.white);
        CategoryPlot plot = chart.getCategoryPlot();
        plot.setBackgroundPaint(Color.lightGray);
    ...
    private JFreeChart createCategoryAreaChart(String title, String xLabel, String yLabel, CategoryDataset dataset) {
        JFreeChart chart = ChartFactory.createAreaChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        chart.setBackgroundPaint(Color.white);
        CategoryPlot plot = chart.getCategoryPlot();
        plot.setForegroundAlpha(0.5f);
        plot.setAxisOffset(new RectangleInsets(5.0, 5.0, 5.0, 5.0));
    ...
    private JFreeChart createCategoryEventFreqChart_Time(String title, String xLabel, String yLabel, CategoryDataset dataset) {
        JFreeChart chart = ChartFactory.createBarChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        chart.setBackgroundPaint(new Color(0xFF, 0xFF, 0xCC));
        CategoryPlot plot = chart.getCategoryPlot();
        plot.getDomainAxis().setMaximumCategoryLabelWidthRatio(10.0f);
        plot.setRangeAxis(new DateAxis(yLabel));
        CategoryToolTipGenerator toolTipGenerator = new StandardCategoryToolTipGenerator("", DateFormat.getDateInstance());
    ...
    private JFreeChart createYIntervalChart(String title, String xLabel, String yLabel, IntervalXYDataset dataset) {
        JFreeChart chart = ChartFactory.createScatterPlot(title, xLabel, yLabel, dataset, orientation, true, true, false);
        XYPlot plot = chart.getXYPlot();
    ...
    private JFreeChart createBoxAndWhiskerChart(String title, String xLabel, String yLabel, BoxAndWhiskerCategoryDataset dataset) {
        ...
        if (dataset.getColumnCount() * dataset.getRowCount() < 5) {
            ...
            if (dataset.getColumnCount() == 1) renderer.setItemMargin(0.5);
            ...
            if (dataset.getColumnCount() == 1) renderer.setItemMargin(renderer.getItemMargin() * 2); else domainAxis.setCategoryMargin(domainAxis.getCategoryMargin() * 2);
        }
        return chart;
    }

    private JFreeChart createXYZBubbleChart(String title, String xLabel, String yLabel, XYZDataset dataset) {
        JFreeChart chart = ChartFactory.createBubbleChart(title, xLabel, yLabel, dataset, orientation, true, true, false);
        XYPlot plot = (XYPlot) chart.getPlot();
        plot.setForegroundAlpha(0.65f);
        XYItemRenderer renderer = plot.getRenderer();
   ...
   */
    private void setDateAxis(XYPlot plot) {
        ...
        if (timeType.equalsIgnoreCase("Year")) generator = new StandardXYToolTipGenerator("{1} = {2}", new SimpleDateFormat("yyyy"), new DecimalFormat("0")); else if (timeType.equalsIgnoreCase("Day")) generator = new StandardXYToolTipGenerator(StandardXYToolTipGenerator.DEFAULT_TOOL_TIP_FORMAT, new SimpleDateFormat("d-MMM-yy"), new DecimalFormat("#,##0.00")); else if (timeType.equalsIgnoreCase("Month")) generator = new StandardXYToolTipGenerator(StandardXYToolTipGenerator.DEFAULT_TOOL_TIP_FORMAT, new SimpleDateFormat("MM-yy"), new DecimalFormat("#,##0.00")); else generator = new StandardXYToolTipGenerator("", DateFormat.getDateInstance(), new DecimalFormat("#,##0.00"));
   ...
   */
    private Double[] createValueList(String in) {
        ...
        for (int i = 0; i < count; i++) values[i] = st.nextToken();
   ...
   */
    private void do_normalQQ(String[] raw_y, int row_count, String disChoice) {
        ...
        try {
            ...
            if (disChoice == NORMAL) stdResiduals = RandomGenerator.getGeneratedArray(y, RandomGenerator.NORMAL); else if (disChoice == POISSON) stdResiduals = RandomGenerator.getGeneratedArray(y, RandomGenerator.POISSON);
