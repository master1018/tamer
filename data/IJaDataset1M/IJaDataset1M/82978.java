...
public class IdSoundnessChecker {
    ...
    public void endDocument() throws SAXException {
        for (String token : map.keySet()) {
            ...
            if (!entry.hadId) {
                for (LocatorImpl idrefLoc : entry.idrefLocs) error("missing_id", token, idrefLoc);
            }
        }
    }

    public void attribute(Name elementName, Name attributeName, String value, Locator locator) throws SAXException {
        ...
        if (idType != Datatype.ID_TYPE_NULL) {
            ...
            switch(idType) {
                case Datatype.ID_TYPE_ID:
                    if (tokens.length == 1) id(tokens[0], locator); else if (tokens.length == 0) error("id_no_tokens", locator); else error("id_multiple_tokens", locator);
                    break;
                case Datatype.ID_TYPE_IDREF:
                    if (tokens.length == 1) idref(tokens[0], locator); else if (tokens.length == 0) error("idref_no_tokens", locator); else error("idref_multiple_tokens", locator);
                    break;
                case Datatype.ID_TYPE_IDREFS:
                    if (tokens.length > 0) {
                        for (int j = 0; j < tokens.length; j++) idref(tokens[j], locator);
                    } else error("idrefs_no_tokens", locator);
                    break;
            }
        }
    }
    ...
    private void idref(String token, Locator locator) {
        ...
        if (entry.hadId) return;
        if (entry.idrefLocs == null) entry.idrefLocs = new ArrayList<LocatorImpl>();
