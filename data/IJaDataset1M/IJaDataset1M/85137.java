...
public final class AlignWithMoveStrategyProvider implements MoveStrategy, MoveProvider {
    ...
    public Point locationSuggested(Widget widget, Point originalLocation, Point suggestedLocation) {
        ...
        Collection<Rectangle> regions = collector.getRegions(widget);
        {
            ...
            if (snap) point.x = xs - widgetBounds.x;
            if (interractionLayer != null) lineWidget1.setControlPoints(snap ? Arrays.asList(new Point(x, Math.min(bounds.y, y1)), new Point(x, Math.max(bounds.y + bounds.height, y2))) : Collections.<Point>emptyList(), true);
        }
        {
            boolean snap = false;
            ...
            if (snap) point.y = ys - widgetBounds.y;
            if (interractionLayer != null) lineWidget2.setControlPoints(snap ? Arrays.asList(new Point(Math.min(bounds.x, x1), y), new Point(Math.max(bounds.x + bounds.width, x2), y)) : Collections.<Point>emptyList(), true);
        }
        return point;
    }
    ...
    public void movementStarted(Widget widget) {
        if (interractionLayer != null) {
            if (lineWidget1 == null) lineWidget1 = decorator.createLineWidget(interractionLayer.getScene());
