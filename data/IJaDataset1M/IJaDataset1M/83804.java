...
public class Display extends JPanel {
    ...
    private static final String cvsid = "$Id: Display.java,v 1.10 2006/08/29 04:02:32 vona Exp $";
    ...
    public Display(Visolate visolate) {
        ...
        Behavior frameBehavior = new Behavior() {
            ...
            public void processStimulus(java.util.Enumeration criteria) {
                synchronized (frameTasks) {
                    for (Iterator<Runnable> it = frameTasks.iterator(); it.hasNext(); ) it.next().run();
                    frameTasks.clear();
                }
                wakeupOn(frameCriterion);
            }
        ...
        if (IMMEDIATE_MODE) graphicsContext.setBackground(background); else rootBG.addChild(background);
        if (!IMMEDIATE_MODE) locale.addBranchGraph(rootBG);
        ...
        tmpBuf.append("000.");
        for (int i = 0; i < LOC_FRACTION_DIGITS; i++) tmpBuf.append("0");
    ...
    private class RenderThread extends Thread {
        ...
        public void run() {
            for (; ; ) {
                if (lastTime >= 0) {
                    long diffTime;
                    for (diffTime = System.currentTimeMillis() - lastTime; diffTime < MIN_FRAME_TIME; diffTime = System.currentTimeMillis() - lastTime) Thread.yield();
                }
        ...
        private void render(Node node) {
            if (node instanceof Group) {
                ...
                if (appearance != null) graphicsContext.setAppearance(appearance);
                for (Enumeration e = shape.getAllGeometries(); e.hasMoreElements(); ) {
                    Geometry geometry = (Geometry) e.nextElement();
                    if (geometry != null) graphicsContext.draw(geometry);
                }
            }
        }
    ...
    private void updateNativeDPI() {
        try {
            ...
            if ((tmp >= MIN_NATIVE_DPI) && (tmp <= MAX_NATIVE_DPI)) nativeDPI = tmp;
            updateView();
    ...
    private void updateDPI() {
        try {
            ...
            if ((tmp >= MIN_DPI) && (tmp <= MAX_DPI)) dpi = tmp;
            updateView();
        } catch (NumberFormatException e) {
        }
    }
    ...
    public void reset() {
        boolean doFit = false;
        if (modelBG == null) doFit = true; else modelBG.detach();
    ...
    private void enableZoom(boolean enable) {
        if (enableZoomButton.isSelected() != enable) enableZoomButton.setSelected(enable);
    ...
    private void enablePan(boolean enable) {
        if (enablePanButton.isSelected() != enable) enablePanButton.setSelected(enable);
        if (enable == panEnabled) return;
        panEnabled = enable;
    }

    private void enableTilt(boolean enable) {
        if (enableTiltButton.isSelected() != enable) enableTiltButton.setSelected(enable);
        if (enable == tiltEnabled) return;
        tiltEnabled = enable;
        if (!enable) {
            tiltX = 0.0;
            tiltY = 0.0;
    ...
    private void updateZoom(int delta, boolean accel) {
        ...
        delta *= ACCEL_FACTOR;
        if (accel) delta *= ACCEL_FACTOR;
        int newDPI = dpi + delta;
        if (newDPI < MIN_DPI) newDPI = MIN_DPI;
        if (newDPI > MAX_DPI) newDPI = MAX_DPI;
        dpi = newDPI;
        updateView();
    }
    ...
    public BufferedImage getStill() throws InterruptedException {
        ...
        ImageComponent2D imageComponent = new ImageComponent2D(ImageComponent.FORMAT_RGB, width, height, false, false);
        raster = new javax.media.j3d.Raster(new Point3f(0.0f, 0.0f, 0.0f), javax.media.j3d.Raster.RASTER_COLOR, 0, 0, width, height, imageComponent, null);
        waitForViewUpdate();
        readRasterPending = true;
        view.repaint();
        while (readRasterPending) Thread.yield();
   ...
   */
    ...
    private class MyCanvas3D extends Canvas3D {
        ...
        MyCanvas3D(GraphicsConfiguration gc) {
            ...
            if (IMMEDIATE_MODE) stopRenderer();
            ...
            addMouseMotionListener(new MouseMotionAdapter() {
                ...
                public void mouseDragged(MouseEvent e) {
                    int x = e.getX();
                    int y = e.getY();
                    if ((e.getModifiersEx() & (MouseEvent.ALT_DOWN_MASK | MouseEvent.BUTTON2_DOWN_MASK)) != 0) updateZoom(y - lastY, e.isShiftDown()); else if ((e.getModifiersEx() & (MouseEvent.CTRL_DOWN_MASK | MouseEvent.BUTTON3_DOWN_MASK)) != 0) updateTilt(x - lastX, y - lastY, e.isShiftDown()); else updatePan(x - lastX, lastY - y, e.isShiftDown());
                    lastX = x;
                    lastY = y;
                }
        ...
        public void postSwap() {
            super.postSwap();
            synchronized (view) {
                if (viewUpdatePending > 0) viewUpdatePending--;
            }
    ...
    private GraphicsConfiguration getGC(Container container) {
        GraphicsDevice graphicsDevice = (container != null) ? container.getGraphicsConfiguration().getDevice() : GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();
        try {
    ...
    private GraphicsConfiguration getGC(GraphicsDevice graphicsDevice) {
        ...
        if (gc == null) gc = fallback;
        if (gc == null) throw new RuntimeException("FATAL ERROR: failed to initialize 3D graphics");
    ...
    public Color3b getRandomColor() {
        ...
        double g = c[1];
        double b = c[2];
        return new Color3b((byte) (((int) Math.floor(r * rCeil)) * rSpread), (byte) (((int) Math.floor(g * gCeil)) * gSpread), (byte) (((int) Math.floor(b * bCeil)) * bSpread));
    }
