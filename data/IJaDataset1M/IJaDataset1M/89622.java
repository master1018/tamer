...
public class NotificationTray extends JPanel {
    ...
    public NotificationTray(Component owner) {
        ...
        notificationButton.setPreferredSize(new Dimension(image.getIconHeight(), image.getIconWidth()));
        ...
        notificationButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                if (notificationQueueWindow == null) {
                    notificationQueueWindow = new NotificationWindow("Notification Message History");
                    JComponent contentPane = notificationQueueWindow.getContentPane();
                    contentPane.setLayout(new BorderLayout());
                    notifyList = new JTable(new DefaultTableModel(new Object[][] {}, new String[] { "@", "Notifications" }) {

                        Class[] types = new Class[] { ImageIcon.class, String.class };
                    ...
                    notifyList.setShowGrid(false);
                    notifyList.addMouseListener(new MouseAdapter() {

                        @Override
                        public void mouseClicked(MouseEvent me) {
                            if ((notifyList.getSelectedRow() >= 0) && (me.getClickCount() == 2)) {
                                DefaultTableModel dtm = (DefaultTableModel) notifyList.getModel();
                                JOptionPane.showMessageDialog(notifyList, dtm.getValueAt(notifyList.getSelectedRow(), 1), "Notification", JOptionPane.DEFAULT_OPTION, (ImageIcon) dtm.getValueAt(notifyList.getSelectedRow(), 0));
                            }
                        }
                    });
                    contentPane.add(new JScrollPane(notifyList), BorderLayout.CENTER);
                    ...
                    clearAll.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent ae) {
                            ...
                            DefaultTableModel dtm = (DefaultTableModel) notifyList.getModel();
                            dtm.setDataVector(new Object[][] {}, new String[] { "@", "Notifications" });
                            notifyList.getColumn("@").setMaxWidth(20);
                            notifyList.sizeColumnsToFit(0);
                        }
                    });
                ...
                DefaultTableModel dtm = (DefaultTableModel) notifyList.getModel();
                dtm.setDataVector(messages, new String[] { "@", "Notifications" });
                notifyList.getColumn("@").setMaxWidth(20);
                notifyList.sizeColumnsToFit(0);
                notificationQueueWindow.setPreferredSize(MiscResource.getInstance().getSplashScreenDimension());
     ...
     */
    public void notify(String title, String message, boolean queueIt, int autoCloseTime) {
     ...
     * @param type the NotificationType (info, warn, error...)
     */
    public void notify(String title, String message, boolean queueIt, NotificationType type) {
     ...
     * @param type the NotificationType (info, warn, error...)
     */
    public void notify(String title, String message, boolean queueIt, int autoCloseTime, NotificationType type) {
    ...
    public void notify(NotificationWindow nw) {
        ...
        p.x -= ((p.x - nw.getPreferredSize().width) >= 0 ? nw.getPreferredSize().width : 0);
        p.y -= ((p.y - nw.getPreferredSize().height) >= 0 ? nw.getPreferredSize().height : 0);
        Point rp = new Point(owner.getX() + owner.getWidth(), owner.getY() + owner.getHeight());
        SwingUtilities.convertPointToScreen(rp, owner);
        if (p.x + nw.getPreferredSize().width > rp.x) p.x = (rp.x - nw.getPreferredSize().width);
        if (p.y + nw.getPreferredSize().height > rp.y) p.y = (rp.y - nw.getPreferredSize().height);
    ...
    public void setTrayIcon(ImageIcon trayIcon) {
        notificationButton.setPreferredSize(new Dimension(trayIcon.getIconHeight(), trayIcon.getIconWidth()));
