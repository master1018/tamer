...
import org.contineo.core.security.GroupRule;
...
public class RightsRecordsManager {
    ...
    private void saveRules(int id, String username) {
        ...
        if (sqlerrors) {
            Messages.addMessage(FacesMessage.SEVERITY_ERROR, Messages.getMessage("errors.action.saverules"), Messages.getMessage("errors.action.saverules"));
        } else {
            Messages.addMessage(FacesMessage.SEVERITY_INFO, Messages.getMessage("msg.action.saverules"), Messages.getMessage("msg.action.saverules"));
        }
        if (recursive) {
