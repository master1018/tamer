...
public class FailureTab extends TestRunTab implements IMenuListener {
    ...
    public String getAllFailedTestNames() {
        ...
        for (int i = 0; i < fTable.getItemCount(); i++) {
            ...
            if (failureTrace != null) {
                ...
                String line;
                try {
                    while ((line = bufferedReader.readLine()) != null) trace.append(line + lineDelim);
                } catch (IOException e) {
    ...
    private void updateTableItem(TestRunInfo testInfo, TableItem tableItem) {
        ...
        if (testInfo.getStatus() == ITestRunListener.STATUS_FAILURE) tableItem.setImage(fFailureIcon); else tableItem.setImage(fErrorIcon);
    ...
    void handleDoubleClick(MouseEvent e) {
        if (fTable.getSelectionCount() > 0) new OpenTestAction(fRunnerViewPart, getClassName(), getMethodName(), true).run();
    }
    ...
    public void selectNext() {
        ...
        int index = fTable.getSelectionIndex();
        if (index == -1) index = 0;
        if (fMoveSelection) index = Math.min(fTable.getItemCount() - 1, index + 1); else fMoveSelection = true;
        selectTest(index);
    }
    ...
    public void selectPrevious() {
        ...
        if (fMoveSelection) index = Math.max(0, index - 1); else fMoveSelection = true;
        selectTest(index);
    }
