...
class CJKFont extends BaseFont {
    ...
    CJKFont(String fontName, String enc, boolean emb) throws DocumentException {
        ...
        if (!isCJKFont(nameBase, enc)) throw new DocumentException("Font '" + fontName + "' with '" + enc + "' encoding is not a CJK font.");
        ...
        if (enc.startsWith("Identity-")) {
            ...
            if (c == null) {
                c = readCMap(s);
                if (c == null) throw new DocumentException("The cmap " + s + " does not exist as a resource.");
                c[CID_NEWLINE] = '\n';
                allCMaps.put(s, c);
            }
            translationMap = c;
            ...
            if (c == null) {
                ...
                if (s == null) throw new DocumentException("The resource cjkencodings.properties does not contain the encoding " + enc);
    ...
    public int getWidth(int char1) {
        int c = char1;
        if (!cidDirect) c = translationMap[c];
        int v;
        if (vertical) v = vMetrics.get(c); else v = hMetrics.get(c);
        if (v > 0) return v; else return 1000;
    }
    ...
    public int getWidth(String text) {
        ...
        for (int k = 0; k < text.length(); ++k) {
            ...
            int v;
            if (vertical) v = vMetrics.get(c); else v = hMetrics.get(c);
            if (v > 0) total += v; else total += 1000;
        }
        return total;
    }
    ...
    private PdfDictionary getFontBaseType(PdfIndirectReference CIDFont) {
        ...
        if (style.length() > 0) name += "-" + style.substring(1);
        name += "-" + CMap;
    ...
    private float getBBox(int idx) {
        ...
        for (int k = 0; k < idx; ++k) ret = tk.nextToken();
    ...
    static char[] readCMap(String name) {
        try {
            ...
            for (int k = 0; k < 0x10000; ++k) c[k] = (char) ((is.read() << 8) + is.read());
            is.close();
            return c;
        } catch (Exception e) {
        }
    ...
    static String convertToVCIDMetrics(int keys[], IntHashtable v, IntHashtable h) {
        ...
        for (start = 0; start < keys.length; ++start) {
            ...
            if (lastValue != 0) {
                ++start;
                break;
            } else lastHValue = h.get(lastCid);
        }
        if (lastValue == 0) return null;
        if (lastHValue == 0) lastHValue = 1000;
        StringBuffer buf = new StringBuffer();
        buf.append('[');
        buf.append(lastCid);
        int state = FIRST;
    ...
    public int getUnicodeEquivalent(int c) {
        if (cidDirect) return translationMap[c];
        return c;
    }

    public int getCidCode(int c) {
        if (cidDirect) return c;
