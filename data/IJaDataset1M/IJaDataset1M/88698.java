 ...
 */
public class ModelBag implements RevisionHandler {
   ...
   */
    public void shuffle(Random rand) {
        if (m_models.length < 2) return;
        for (int i = 0; i < m_models.length; ++i) {
            int swap_index = rand.nextInt(m_models.length - 1);
            if (swap_index >= i) ++swap_index;
            swap(i, swap_index);
        }
    }
    ...
    public int[] sortInitialize(int num, boolean greedy, Instances instances, int metric) throws Exception {
        ...
        if (greedy) {
            ...
            for (int i = 1; i < num; ++i) {
                ...
                double metric_value = evaluatePredictions(instances, tempPredictions, metric);
        ...
        updateBestTimesChosen();
        if (m_debug) {
            System.out.println("Sort Initialization added best " + m_numChosen + " models to the bag.");
        }
        return bestModels;
    }
   ...
   */
    public void backwardEliminate(Instances instances, int metric) throws Exception {
        ...
        m_numChosen--;
        if (m_debug) {
            System.out.println("Removing model " + m_modelIndex[bestIndex] + " (" + bestIndex + ") " + bestPerformance);
        }
        if (bestPerformance > m_bestPerformance) {
            updateBestTimesChosen();
            m_bestPerformance = bestPerformance;
