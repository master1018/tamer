...
public class ServerAuthenticatorNone implements ServerAuthenticator {
   ...
   */
    ...
    */
    public ServerAuthenticator startSession(Socket s) throws IOException {
        ...
        int version = in.read();
        if (version == 5) {
            if (!selectSocks5Authentication(in, out, 0)) return null;
        } else if (version == 4) {
            in.unread(version);
        } else return null;
   ...
   */
    public static boolean selectSocks5Authentication(InputStream in, OutputStream out, int methodId) throws IOException {
        ...
        int bread = 0;
        while (bread < num_methods) bread += in.read(method_ids, bread, num_methods - bread);
        for (int i = 0; i < num_methods; ++i) if (method_ids[i] == methodId) {
            found = true;
