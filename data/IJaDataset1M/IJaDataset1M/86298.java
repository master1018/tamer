 ...
 * Time: 15:24:25
 */
public class GetCardsFromEnemyHandActor extends AbstractSimpleActor implements IGameFieldEventListener {
    ...
    public GetCardsFromEnemyHandActor(Vector<Card> cards, int num, Boolean mond, int mode) {
        super();
        this.cards = cards;
        this.num = num;
        this.mond = mond;
    ...
    public Boolean playRole() {
        ...
        if (!gfQuery) {
            if (Director.dirDebug) Logger.debug(0, "Running GET CARDS FROM ENEMY HAND IN " + actorDirector.toString(), null);
            ...
            cardsSelected = false;
            Phaser.getInstance().getGameField().selectFromEnemyHand(cards, num, mond);
            gfQuery = true;
        }
        while (!cardsSelected) {
            try {
