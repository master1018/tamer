...
public class FlagLabel extends JLabel {
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (GREY) filter = new GrayFilter(true, 100 - BRIGHTNESS); else filter = new TransparentFilter(BRIGHTNESS);
    ...
    protected void paintComponent(Graphics g) {
        if ((ROUNDFLAG) && (!this.homeCountry)) {
            RoundRectangle2D.Double oval = new RoundRectangle2D.Double(0.0D, 0.0D, getSize().getWidth(), getSize().getHeight(), 9.0D, 9.0D);
            g.setClip(oval);
        }
