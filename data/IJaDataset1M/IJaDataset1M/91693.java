...
class Volume {
    ...
    protected Volume(ImagePlus imp) {
        ...
        volumeLimits[0][2] = 0;
        volumeLimits[1][0] = 0;
        volumeLimits[1][1] = heightY;
        volumeLimits[1][2] = depthZ;
        volumeLimits[2][0] = widthX;
        volumeLimits[2][1] = 0;
        volumeLimits[2][2] = depthZ;
        volumeLimits[3][0] = widthX;
        volumeLimits[3][1] = heightY;
        volumeLimits[3][2] = 0;
        volumeLimits[4][0] = 0;
        volumeLimits[4][1] = 0;
        volumeLimits[4][2] = depthZ;
        volumeLimits[5][0] = 0;
        volumeLimits[5][1] = heightY;
        volumeLimits[5][2] = 0;
        volumeLimits[6][0] = widthX;
        volumeLimits[6][1] = 0;
        volumeLimits[6][2] = 0;
        volumeLimits[7][0] = widthX;
        volumeLimits[7][1] = heightY;
        volumeLimits[7][2] = depthZ;
        widthXm1 = widthX - 1;
        heightYm1 = heightY - 1;
        depthZm1 = depthZ - 1;
        data3D = new byte[depthZ][heightY][widthX];
        getMinMax();
        init();
    }
    ...
    private boolean getMinMax() {
        ...
        if (firstTime) {
            if (zAspect == 1) zAspect = (float) (cal.pixelDepth / cal.pixelWidth);
            if (zAspect == 0) zAspect = 1;
            lut = new Lut();
            lut.readLut(imp);
            oldMin = min;
            oldMax = max;
        ...
        boolean changed;
        if (oldMin == min && oldMax == max) changed = false; else changed = true;
        oldMin = min;
        oldMax = max;
        return changed;
    }
    ...
    private void volumeDotsOrig() {
        ...
        for (int i = bufferPixels.length - 1; i >= 0; i--) {
            ...
            if (val > 255) val = 255;
            if (val > 0) bufferPixels[i] = lut.colors[val];
        }
    }
            ...
            for (int y = yMin; y <= yMax; y++) {
                ...
                for (int x = xMin; x <= xMax; x++) {
                    if (isInside(x, y)) {
                        ...
                        for (int n = nd; n >= 0; n--) {
                            z += d_z;
                            if (xd1 >= 0 && xd1 < width && yd1 >= 0 && yd1 < height && zd1 >= 0 && zd1 < depth) {
                                int val = trilinear(zd1, yd1, xd1);
                                int f = 255 - 10 * Math.abs(val - threshold + 30);
                                if (f < 0) f = 0;
                                if (f > 10) {
                                    alpha += f;
                                    V += f * 255;
                                    if (++k >= renderDepth) break;
                                }
                                f = 255 - 10 * Math.abs(val - threshold);
                                if (f < 0) f = 0;
                                if (f > 10) {
                                    alpha2 += f;
                                    V2 += f * 255;
    ...
    private int trilinear(double z, double y, double x) {
        ...
        return (int) ((v100 - v000) * dx + (v010 - v000) * dy + (v001 - v000) * dz + (v110 - v100 - v010 + v000) * dx * dy + (v011 - v010 - v001 + v000) * dy * dz + (v101 - v100 - v001 + v000) * dx * dz + (v111 + v100 + v010 + v001 - v110 - v101 - v011 - v000) * dx * dy * dz + v000);
    }
    ...
    protected void draw() {
        if (drawMode == JRenderer3D.VOLUME_DOTS) volumeDots(); else if (drawMode == JRenderer3D.VOLUME_SLICE_NEAREST_NEIGHBOR) volumeSliceNearestNeighbor(); else if (drawMode == JRenderer3D.VOLUME_SLICE_TRILINEAR) volumeSliceTrilinear(); else if (drawMode == JRenderer3D.VOLUME_DOTS) volumeDots(); else if (drawMode == JRenderer3D.VOLUME_PROJECTION_TRILINEAR_FRONT) volumeProjection_trilinear_front();
    }
    ...
    protected void setVolumeLut(int volume_lutNr) {
        if (lut != null) lut.setLut(volume_lutNr);
    }
