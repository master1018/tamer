   ...
   */
    public String toStringMatrix() {
        ...
        for (j = 1; j < getColCount(); j++) {
            ...
            if (!getShowStdDev()) result.append("r" + "@{\\hspace{0.1cm}}" + "c"); else result.append("r" + "@{\\hspace{0cm}}" + "c" + "@{\\hspace{0cm}}" + "r" + "@{\\hspace{0.1cm}}" + "c");
        }
        result.append("}\n\\\\\n\\hline\n");
        if (!getShowStdDev()) result.append("Dataset & " + cells[0][1]); else result.append("Dataset & \\multicolumn{3}{c}{" + cells[0][1] + "}");
        for (j = 2; j < cells[0].length; j++) {
            if (!isMean(j)) continue;
            if (!getShowStdDev()) result.append("& " + cells[0][j] + " & "); else result.append("& \\multicolumn{4}{c}{" + cells[0][j] + "} ");
        }
        result.append("\\\\\n\\hline\n");
        ...
        for (i = 1; i < cells.length - 1; i++) {
            ...
            for (n = 0; n < cells[0].length; n++) {
                if (n == 0) {
                    ...
                    if (getShowStdDev()) {
                        if (isMean(n - 1)) {
                            if (!cells[i][n].trim().equals("")) result.append(" & $\\pm$ & "); else result.append(" &       & ");
                        } else result.append(" & ");
                    } else {
                        result.append(" & ");
                    }
        ...
        if (!getShowStdDev()) result.append("\\footnotesize "); else result.append("\\scriptsize ");
        result.append("\\par}\n\\end{table}" + "\n");
    ...
    public String toStringSummary() {
        ...
        for (i = 0; i < getColCount(); i++) {
            ...
            for (j = 0; j < getColCount(); j++) {
                ...
                if (j == i) result += " & - "; else result += "& " + m_NonSigWins[i][j] + " (" + m_Wins[i][j] + ") ";
            }
            result += "\\\\\n";
        }
