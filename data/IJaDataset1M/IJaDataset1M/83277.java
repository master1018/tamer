package org.jcvi.vics.compute.service.search;
...
public class WebSearcher {
    ...
    private class DrupalSearchResultDocHandler extends DefaultHandler {
        ...
        private String _resultInsertSql = "insert into website_ts_result " + "( node_id, docid, accession, docname, headline ) " + "values ( ?,?,?,?,? )";
        ...
        private void endRecord() {
            ...
            try {
                ...
                _statement.setString(3, String.valueOf(_currentResultNode.getObjectId()) + "_" + String.valueOf(_currentRecordNo));
                _statement.setString(4, _currentTitle);
                _statement.setString(5, _currentBlurb);
                _statement.executeUpdate();
            } catch (SQLException e) {
                _logger.error("Unable to add websearch result record to the DB. Title: '" + _currentTitle + "'; Blurb: '" + _currentBlurb + "'", e);
            }
        }
    ...
    public int populateSearchResult(SearchTask searchTask) throws Exception {
        ...
        try {
            ...
            if (webSearchURL != null && webSearchURL.trim().length() > 0) {
                if (webSearchURL.endsWith(".htm") || webSearchURL.endsWith(".html")) {
                    webSearchURL = webSearchURL + "?" + "query" + "=" + URLEncoder.encode(searchQuery, DEFAULT_ENCODING);
                } else {
    ...
    private int readSearchResult(URL searchURL, SearchTask st) throws Exception {
