...
public class Query extends Function implements Optimizable {
    ...
    public void setArguments(List<Expression> arguments) throws XPathException {
        ...
        arg = new DynamicCardinalityCheck(context, Cardinality.EXACTLY_ONE, arg, new org.exist.xquery.util.Error(org.exist.xquery.util.Error.FUNC_PARAM_CARDINALITY, "2", mySignature));
        steps.add(arg);
        if (arguments.size() == 3) {
            arg = arguments.get(2);
            arg = new DynamicCardinalityCheck(context, Cardinality.EXACTLY_ONE, arg, new org.exist.xquery.util.Error(org.exist.xquery.util.Error.FUNC_PARAM_CARDINALITY, "2", mySignature));
        ...
        if (!steps.isEmpty()) {
            ...
            if (steps.size() == 1 && firstStep.getAxis() == Constants.SELF_AXIS) {
                ...
                if (outerExpr != null && outerExpr instanceof LocationStep) {
                    ...
                    if (test.getName() == null) contextQName = new QName(null, null, null); else if (test.isWildcardTest()) contextQName = test.getName(); else contextQName = new QName(test.getName());
                    if (outerStep.getAxis() == Constants.ATTRIBUTE_AXIS || outerStep.getAxis() == Constants.DESCENDANT_ATTRIBUTE_AXIS) contextQName.setNameType(ElementValue.ATTRIBUTE);
                ...
                if (test.getName() == null) contextQName = new QName(null, null, null); else if (test.isWildcardTest()) contextQName = test.getName(); else contextQName = new QName(test.getName());
                if (lastStep.getAxis() == Constants.ATTRIBUTE_AXIS || lastStep.getAxis() == Constants.DESCENDANT_ATTRIBUTE_AXIS) contextQName.setNameType(ElementValue.ATTRIBUTE);
    ...
    public NodeSet preSelect(Sequence contextSequence, boolean useContext) throws XPathException {
        if (contextSequence != null && !contextSequence.isPersistentSet()) return NodeSet.EMPTY_SET;
    ...
    public Sequence eval(Sequence contextSequence, Item contextItem) throws XPathException {
        ...
        if (contextSequence != null && !contextSequence.isPersistentSet()) return Sequence.EMPTY_SEQUENCE;
        NodeSet result;
        if (preselectResult == null) {
            long start = System.currentTimeMillis();
            Sequence input = getArgument(0).eval(contextSequence);
            if (!(input instanceof VirtualNodeSet) && input.isEmpty()) result = NodeSet.EMPTY_SET; else {
                ...
                try {
                    if (Type.subTypeOf(key.getType(), Type.ELEMENT)) result = index.query(context, getExpressionId(), docs, inNodes, qnames, (Element) ((NodeValue) key).getNode(), NodeSet.ANCESTOR, options); else result = index.query(context, getExpressionId(), docs, inNodes, qnames, key.getStringValue(), NodeSet.ANCESTOR, options);
                } catch (IOException e) {
    ...
    protected Item getKey(Sequence contextSequence, Item contextItem) throws XPathException {
        ...
        if (!(Type.subTypeOf(key.getType(), Type.STRING) || Type.subTypeOf(key.getType(), Type.NODE))) throw new XPathException(this, "Second argument to ft:query should either be a query string or " + "an XML element describing the query. Found: " + Type.getTypeName(key.getType()));
        return key;
    }

    public int getDependencies() {
        final Expression stringArg = getArgument(0);
        if (Type.subTypeOf(stringArg.returnsType(), Type.NODE) && !Dependency.dependsOn(stringArg, Dependency.CONTEXT_ITEM)) {
    ...
    protected Properties parseOptions(Sequence contextSequence, Item contextItem) throws XPathException {
        if (getArgumentCount() < 3) return null;
