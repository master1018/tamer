...
public class OdeRocketStructure extends OdeCylinderStructure {
            ...
            if (!physicsNode.isStatic()) {
                ...
                callback = new RocketCallback() {
                    ...
                    public void applyForce(float forceAmount) {
                        ...
                        if (Math.abs(forceAmount) <= FastMath.FLT_EPSILON) return;
                        dynamicPhysics.updateWorldVectors();
                        force.set(direction);
                        dynamicPhysics.getWorldRotation().mult(force, force);
    ...
    public void buildRelationships(Map<OdePhysicalStructure, PhysicsNode> physicsMap, OdePhysicsEnvironment compiledEnvironment) {
            ...
            particleUpdate = new Updatable() {
                ...
                public void update(float time) {
                    ...
                    if (getMaxForce() > 0) {
                        ...
                        if (percent < FastMath.FLT_EPSILON) percent = 0;
        ...
        ;
        if (particleUpdate != null) UpdateManager.getInstance().add(particleUpdate, Stream.PHYSICS_RENDER);
    }
