...
public class AttackDialog extends JDialog {
    ...
    protected void handleTableUpdate(TableModelEvent e) {
        ...
        if ((dmgColumn == e.getColumn()) || (e.getColumn() == TableModelEvent.ALL_COLUMNS) || (e.getType() == TableModelEvent.DELETE) || (e.getType() == TableModelEvent.INSERT)) {
            m_totalDmg = 0;
            for (int i = 0; i < m_table.getRowCount(); i++) {
    ...
    public class AttackTableModel extends DefaultTableModel {
		 ...
		 */
        private Object[][] columns = { { "Bonus", Integer.class, null, Boolean.valueOf(false), COLUMN_KEY_BONUS }, { "Fudge", Integer.class, new Integer(0), Boolean.valueOf(true), COLUMN_KEY_FUDGE }, { "Increment", Integer.class, new Integer(m_attack.getRangeAsInt()), Boolean.valueOf(false), COLUMN_KEY_INCREMENT }, { "Range", Integer.class, null, Boolean.valueOf(true), COLUMN_KEY_RANGE }, { "Roll", Integer.class, null, Boolean.valueOf(false), COLUMN_KEY_ROLL }, { "Total", Integer.class, null, Boolean.valueOf(false), COLUMN_KEY_TOTAL }, { "Target", PcgCombatant.class, null, Boolean.valueOf(true), COLUMN_KEY_TARGET }, { "AC", Integer.class, null, Boolean.valueOf(true), COLUMN_KEY_AC }, { "Crit Roll", Integer.class, null, Boolean.valueOf(false), COLUMN_KEY_CRITROLL }, { "Crit Total", Integer.class, null, Boolean.valueOf(false), COLUMN_KEY_CRITTOTAL }, { "Hit", Boolean.class, Boolean.valueOf(false), Boolean.valueOf(true), COLUMN_KEY_HIT }, { "Crit", Boolean.class, Boolean.valueOf(false), Boolean.valueOf(true), COLUMN_KEY_CRIT }, { "Dmg", String.class, null, Boolean.valueOf(true), COLUMN_KEY_DMG }, { "Dmg Tot", Integer.class, null, Boolean.valueOf(true), COLUMN_KEY_DMGTOT } };
        ...
        private void recalcRow(int row, int column) {
            int attTotal = 0;
            int attackModifier = getIntAt(row, columnFromKey(COLUMN_KEY_BONUS)) + getIntAt(row, columnFromKey(COLUMN_KEY_FUDGE));
            if ((getIntAt(row, columnFromKey(COLUMN_KEY_INCREMENT)) > 0) && (getIntAt(row, columnFromKey(COLUMN_KEY_RANGE)) > 0)) {
                attackModifier += ((int) Math.floor((double) getIntAt(row, columnFromKey(COLUMN_KEY_RANGE)) / (double) getIntAt(row, columnFromKey(COLUMN_KEY_INCREMENT))) * -2);
            }
            ...
            if ((column == columnFromKey(COLUMN_KEY_BONUS)) || (column == columnFromKey(COLUMN_KEY_FUDGE)) || (column == columnFromKey(COLUMN_KEY_INCREMENT)) || (column == columnFromKey(COLUMN_KEY_RANGE)) || (column == columnFromKey(COLUMN_KEY_ROLL))) {
            ...
            if ((column == columnFromKey(COLUMN_KEY_HIT)) || (column == columnFromKey(COLUMN_KEY_CRIT))) {
                if (((Boolean) getValueAt(row, columnFromKey(COLUMN_KEY_HIT))).booleanValue()) {
                    ...
                    if (damageString.indexOf("/") >= 0) {
                        ...
                        damageString = (String) JOptionPane.showInputDialog(AttackDialog.this, "This weapon appears to have more than one possible damage " + "die listed.  Please choose one:", "Multiple Damage Dice", JOptionPane.QUESTION_MESSAGE, null, heads, heads[1]);
        ...
        private void rollAttack(int row) {
            ...
            if ((getIntAt(row, columnFromKey(COLUMN_KEY_ROLL)) >= m_attack.getCritRangeMin(row)) && ((Boolean) getValueAt(row, columnFromKey(COLUMN_KEY_HIT))).booleanValue()) {
