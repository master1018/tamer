...
public class Astro implements Constants {
    ...
    public static double atan2Deg(double numerator, double denomenator) {
        double result;
        if ((denomenator == 0.0 || Double.isNaN(denomenator)) && (numerator == 0.0 || Double.isNaN(numerator))) result = 0.0; else {
	 ...
	 */
    public static double greatCircle(double startLatitude, double startLongitude, double endLatitude, double endLongitude) {
        double C = (Astro.sin(startLatitude) * Astro.sin(endLatitude)) + (Astro.cos(startLatitude) * Astro.cos(endLatitude) * Astro.cos(endLongitude - startLongitude));
        double A = Math.atan(Math.sqrt(1.0 - (C * C)) / C) + (Math.PI * (C - Math.abs(C)) / (2.0 * C));
        return (A);
    }
    ...
    public static double[] lunarEphemeris(double MJD) {
        ...
        double L = P2 * Astro.FRAC(0.374897 + 1325.552410 * T);
        double LS = P2 * Astro.FRAC(0.993133 + 99.997361 * T);
        double D = P2 * Astro.FRAC(0.827361 + 1236.853086 * T);
        ...
        double L_MOON = P2 * Astro.FRAC(L0 + DL / 1296.0E3);
    ...
    public static double mod(double numerator, double denomenator) {
        ...
        if (result < 0.0) result += denomenator;
        return (result);
    }
    ...
    public static String twilightString(double sunrise, double sunset, String twilightName) {
        if (Astro.isEvent(sunrise)) {
            if (Astro.isEvent(sunset)) {
                return (twilightName + getRiseSetString(sunrise, " begins at ") + getRiseSetString(sunset, " and ends at "));
            } else {
