...
public class Interface extends StandardNamedObject {
    ...
    private static class ServiceComparator {
        ...
        public boolean equals(Object o) {
            boolean rc = false;
            if (o != null) {
                if (o == this) rc = true; else if (o instanceof Service) rc = m_svcname.equals(((Service) o).getName()); else if (o instanceof String) rc = m_svcname.equals(o);
            }
            return rc;
        }
    }
    ...
    public Interface(String name, ArrayList<Service> services) {
        setName(name);
        if (services == null) m_services = new ArrayList<Service>(); else m_services = services;
    }
    ...
    public Interface(String name, String service, long losttime) {
        ...
        if (service != null) {
            svc = new Service(service);
            if (losttime > 0) svc.addOutage(losttime);
            m_services.add(svc);
        }
    }
    ...
    public Interface(String name, String service, long losttime, long regainedtime) {
        ...
        if (service != null) {
            svc = new Service(service);
            if (losttime > 0) {
                if (regainedtime > 0) svc.addOutage(losttime, regainedtime); else svc.addOutage(losttime);
            }
            m_services.add(svc);
        }
    }
    ...
    public void addService(Service service) {
        if (service != null) m_services.add(service);
    }
    ...
    public int getServiceCount() {
        if (m_services != null && m_services.size() > 0) return m_services.size();
        return -1;
    }
    ...
    public boolean equals(Object obj) {
        if (obj != null) {
            if (obj instanceof String) return ((String) obj).equals(getName()); else if (obj instanceof Interface) return obj == this;
        }
        return false;
    }
}
