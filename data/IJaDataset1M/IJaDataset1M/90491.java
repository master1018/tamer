 ...
 */
public class OperationSetPersistentPresenceSSHImpl implements OperationSetPersistentPresence {

    private static final Logger logger = Logger.getLogger(OperationSetPersistentPresenceSSHImpl.class);
     ...
     * created us.
     */
    public OperationSetPersistentPresenceSSHImpl(ProtocolProviderServiceSSHImpl provider) {
        this.parentProvider = provider;
        contactListRoot = new ContactGroupSSHImpl("RootGroup", provider);
        parentProvider.addRegistrationStateChangeListener(new UnregistrationListener());
    }
     ...
     */
    public void changeContactPresenceStatus(ContactSSH sshContact, PresenceStatus newStatus) {
        PresenceStatus oldStatus = sshContact.getPresenceStatus();
        sshContact.setPresenceStatus(newStatus);
        fireContactPresenceStatusChangeEvent(sshContact, sshContact.getParentContactGroup(), oldStatus);
    ...
    public void addContactPresenceStatusListener(ContactPresenceStatusListener listener) {
        synchronized (contactPresenceStatusListeners) {
            if (!contactPresenceStatusListeners.contains(listener)) contactPresenceStatusListeners.add(listener);
        }
    }
     ...
     * changing it.
     */
    public void fireContactPresenceStatusChangeEvent(Contact source, ContactGroup parentGroup, PresenceStatus oldValue) {
        ContactPresenceStatusChangeEvent evt = new ContactPresenceStatusChangeEvent(source, parentProvider, parentGroup, oldValue, source.getPresenceStatus());
     ...
     */
    public void fireSubscriptionEvent(ContactSSH source, ContactGroup parentGroup, int eventID) {
    ...
    public void fireSubscriptionMovedEvent(Contact source, ContactGroup oldParent, ContactGroup newParent) {
        SubscriptionMovedEvent evt = new SubscriptionMovedEvent(source, this.parentProvider, oldParent, newParent);
        Iterator listeners = null;
        synchronized (subscriptionListeners) {
            listeners = new ArrayList(subscriptionListeners).iterator();
        }
     ...
     * @param eventID an identifier of the event to dispatch.
     */
    public void fireServerStoredGroupEvent(ContactGroupSSHImpl source, int eventID) {
        ServerStoredGroupEvent evt = new ServerStoredGroupEvent(source, eventID, (ContactGroupSSHImpl) source.getParentContactGroup(), this.parentProvider, this);
        Iterator listeners = null;
        synchronized (serverStoredGroupListeners) {
            listeners = new ArrayList(serverStoredGroupListeners).iterator();
        }
        while (listeners.hasNext()) {
            ServerStoredGroupListener listener = (ServerStoredGroupListener) listeners.next();
    ...
    public void addProviderPresenceStatusListener(ProviderPresenceStatusListener listener) {
        synchronized (providerPresenceStatusListeners) {
            if (!providerPresenceStatusListeners.contains(listener)) this.providerPresenceStatusListeners.add(listener);
        }
    }
    ...
    public void addServerStoredGroupChangeListener(ServerStoredGroupListener listener) {
        synchronized (serverStoredGroupListeners) {
            if (!serverStoredGroupListeners.contains(listener)) serverStoredGroupListeners.add(listener);
        }
    }
    ...
    public void addSubsciptionListener(SubscriptionListener listener) {
        synchronized (subscriptionListeners) {
            if (!subscriptionListeners.contains(listener)) this.subscriptionListeners.add(listener);
        }
    }
    ...
    public void createServerStoredContactGroup(ContactGroup parent, String groupName) {
        ContactGroupSSHImpl newGroup = new ContactGroupSSHImpl(groupName, parentProvider);
     ...
     * @param contactGroup the group to add
     */
    public void addSSHGroupAndFireEvent(ContactGroupSSHImpl parent, ContactGroupSSHImpl contactGroup) {
     ...
     *   registered.
     */
    public void publishPresenceStatus(PresenceStatus status, String statusMessage) throws IllegalArgumentException, IllegalStateException {
     ...
     */
    private void changePresenceStatusForContact(ContactSSH contact, PresenceStatus newStatus) {
        PresenceStatus oldStatus = contact.getPresenceStatus();
        contact.setPresenceStatus(newStatus);
        fireContactPresenceStatusChangeEvent(contact, findContactParent(contact), oldStatus);
    }
     ...
     */
    private void changePresenceStatusForAllContacts(ContactGroup parent, PresenceStatus newStatus) {
     ...
     * belongs to or null if no parent was found.
     */
    public ContactGroupSSHImpl findContactParent(ContactSSH sshContact) {
        return (ContactGroupSSHImpl) sshContact.getParentContactGroup();
    }
    ...
    public void removeServerStoredContactGroup(ContactGroup group) throws IllegalArgumentException {
        ContactGroupSSHImpl sshGroup = (ContactGroupSSHImpl) group;
        ...
        this.fireServerStoredGroupEvent(sshGroup, ServerStoredGroupEvent.GROUP_REMOVED_EVENT);
    }
    ...
    public void renameServerStoredContactGroup(ContactGroup group, String newName) {
        ...
        this.fireServerStoredGroupEvent((ContactGroupSSHImpl) group, ServerStoredGroupEvent.GROUP_RENAMED_EVENT);
    }
    ...
    public void subscribe(ContactGroup parent, String contactIdentifier) throws IllegalArgumentException, IllegalStateException, OperationFailedException {
        ...
        ContactSSH sshContact = new ContactSSHImpl(contactIdentifier, parentProvider);
     ...
     */
    public void addContactToList(ContactGroup parent, ContactSSH sshContact) {
        ((ContactGroupSSHImpl) parent).addContact(sshContact);
        fireSubscriptionEvent(sshContact, parent, SubscriptionEvent.SUBSCRIPTION_CREATED);
        fireContactPresenceStatusChangeEvent(sshContact, parent, SSHStatusEnum.NOT_AVAILABLE);
    ...
    public void unsubscribe(Contact contact) throws IllegalArgumentException, IllegalStateException, OperationFailedException {
        ContactGroupSSHImpl parentGroup = (ContactGroupSSHImpl) ((ContactSSHImpl) contact).getParentContactGroup();
        parentGroup.removeContact((ContactSSHImpl) contact);
        fireSubscriptionEvent((ContactSSHImpl) contact, ((ContactSSHImpl) contact).getParentContactGroup(), SubscriptionEvent.SUBSCRIPTION_REMOVED);
    }
    ...
    public Contact createUnresolvedContact(String address, String persistentData, ContactGroup parent) {
        ContactSSH contact = new ContactSSHImpl(address, parentProvider);
     ...
     */
    public ProtocolProviderServiceSSHImpl findProviderForSSHUserID(String sshUserID) {
        BundleContext bc = SSHActivator.getBundleContext();
        String osgiQuery = "(&" + "(" + ProtocolProviderFactory.PROTOCOL + "=" + ProtocolNames.SSH + ")" + "(" + ProtocolProviderFactory.USER_ID + "=" + sshUserID + ")" + ")";
    ...
    public ContactGroup createUnresolvedContactGroup(String groupUID, String persistentData, ContactGroup parentGroup) {
        ContactGroupSSHImpl newGroup = new ContactGroupSSHImpl(ContactGroupSSHImpl.createNameFromUID(groupUID), parentProvider);
    ...
    private class UnregistrationListener implements RegistrationStateChangeListener {
        ...
        public void registrationStateChanged(RegistrationStateChangeEvent evt) {
            ...
            while (groupsIter.hasNext()) {
                ContactGroupSSHImpl group = (ContactGroupSSHImpl) groupsIter.next();
    ...
    private ContactGroupSSHImpl getNonPersistentGroup() {
        for (int i = 0; i < getServerStoredContactListRoot().countSubgroups(); i++) {
            ContactGroupSSHImpl gr = (ContactGroupSSHImpl) getServerStoredContactListRoot().getGroup(i);
    ...
    public ContactSSHImpl createVolatileContact(String contactAddress) {
        ContactSSHImpl newVolatileContact = new ContactSSHImpl(contactAddress, this.parentProvider);
        ...
        if (theVolatileGroup == null) {
            ...
            theVolatileGroup = new ContactGroupSSHImpl("NotInContactList", parentProvider);
