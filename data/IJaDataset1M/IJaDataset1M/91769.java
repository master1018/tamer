...
two objects.
*/
public abstract class BinaryList implements Serializable {
    ...
    public Object get(Object key) {
        ...
        while (min <= max) {
            ...
            int c;
            if ((c = compare(data, key)) < 0) min = center + 1; else if (c > 0) max = center - 1; else break;
        }
        if (compare(key, getKey(returnMe)) == 0) return (returnMe);
        return (null);
    }
    ...
    private int findIndex(Object key) {
        ...
        while (min <= max) {
            ...
            if (compare(data, key) < 0) min = center + 1; else max = center - 1;
        }
        return (min);
    }
