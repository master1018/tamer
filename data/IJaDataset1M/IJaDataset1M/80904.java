...
public class RobotRegistrationServlet extends HttpServlet {
    ...
    @Inject
    private RobotRegistrationServlet(AccountStore accountStore, @Named(CoreSettings.WAVE_SERVER_DOMAIN) String domain, TokenGenerator tokenGenerator) {
   ...
   */
    private void doRegisterGet(HttpServletRequest req, HttpServletResponse resp, String message) throws IOException {
   ...
   */
    private void onRegisterSuccess(HttpServletRequest req, HttpServletResponse resp, RobotAccountData robotAccount) throws IOException {
        RobotRegistrationSuccessPage.write(resp.getWriter(), new GxpContext(req.getLocale()), robotAccount.getId().getAddress(), robotAccount.getConsumerSecret());
        resp.setContentType("text/html");
        resp.setStatus(HttpServletResponse.SC_OK);
    }
}
