...
public class FirebirdDialectExt extends CommonHibernateDialect implements HibernateDialect {
    ...
    public int getColumnLength(int columnSize, int dataType) {
        if (dataType == Types.BIGINT || dataType == Types.DECIMAL || dataType == Types.DOUBLE || dataType == Types.FLOAT || dataType == Types.NUMERIC || dataType == Types.REAL) {
    ...
    public String[] getAddAutoIncrementSQL(TableColumnInfo column, DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
        ...
        String generatorSql = getCreateSequenceSQL(sequenceName, incrementNotSupported, minValueNotSupported, maxValueNotSupported, startValueNotSupported, cacheNotSupported, cycle, qualifier, prefs);
        String triggerName = "CREATE_" + column.getColumnName().toUpperCase();
        String trigTemplate = "CREATE TRIGGER $triggerName$ FOR $tableName$ BEFORE INSERT POSITION 0 AS " + "BEGIN NEW.$columnName$ = GEN_ID($sequenceName$, 1); END";
        StringTemplate st = new StringTemplate(trigTemplate);
        HashMap<String, String> valuesMap = DialectUtils.getValuesMap(ST_COLUMN_NAME_KEY, column.getColumnName());
    ...
    public String[] getAddUniqueConstraintSQL(String tableName, String constraintName, TableColumnInfo[] columns, DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
        ...
        HashMap<String, String> valuesMap = DialectUtils.getValuesMap(ST_TABLE_NAME_KEY, tableName, ST_CONSTRAINT_NAME_KEY, constraintName);
        return new String[] { DialectUtils.getAddUniqueConstraintSQL(st, valuesMap, columns, qualifier, prefs, this) };
    }
