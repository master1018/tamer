...
class Neuron {
    ...
    double calculate() {
        ...
        switch(applyFormula) {
            ...
            case SOFTMAX:
                ...
                if (normalization > 0) retVal = Math.exp(input) / normalization; else retVal = 1 / activeLayer.getNeuronCount();
                break;
            case EXTERNAL:
                break;
        }
    ...
    public String toString() {
        String retVal = "";
        retVal += "[Neuron: ";
        if (delta > 0) retVal += "delta=" + delta + ", ";
        if (multi != 1) retVal += "multi=" + multi + ", ";
        retVal += "weight=" + weight + " ";
        for (int i = 0; i < getSynapseCount(); i++) {
            retVal += getSynapse(i).toString();
            if (i < getSynapseCount() - 1) retVal += ", ";
        }
        retVal += "]";
        return retVal;
    }
