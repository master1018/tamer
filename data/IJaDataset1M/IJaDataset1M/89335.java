...
public class TextDemo1 extends TestCase {
    ...
    String d1 = "Trying to come up with simple examples that illustrate " + "our general Vob design is not easy; however, the most difficult part " + "of this demo was selecting the underlying text stream to use. " + "As you can see, we didn't manage to make a choice: we wrote our own. " + "Here, you can see how the referential text model allows displaying " + "reorganizations of the original text in a meaningful manner. ";

    int[] str2 = new int[] { d1.indexOf("Here, y"), d1.length(), 0, d1.indexOf("As you can"), -1, -1, d1.indexOf("As you can"), d1.indexOf("Here, y") };

    String[] d2str = new String[] { "To clearly demonstrate this view, the text needs to " + "have been rearranged and edited. " };
        ...
        try {
            synchronized (this) {
                ...
                for (int i = 0; i < str2.length; i += 2) {
                    if (str2[i] < 0) curs.insert(d2str[-str2[i] - 1]); else curs.insert((TextSpan) all.subSpan(str2[i], str2[i + 1]));
                }
