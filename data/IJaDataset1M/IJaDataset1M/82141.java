...
public final class MxlDirectionType {
    ...
    public static MxlDirectionType read(Element e) {
        ...
        switch(n.charAt(0)) {
            case 'd':
                if (n.equals(MxlDynamics.ELEM_NAME)) content = MxlDynamics.read(firstChild);
                break;
            case 'i':
                if (n.equals("image")) content = MxlImage.read(firstChild);
                break;
            case 'p':
                if (n.equals(MxlPedal.ELEM_NAME)) content = MxlPedal.read(firstChild);
                break;
            case 'm':
                if (n.equals(MxlMetronome.ELEM_NAME)) content = MxlMetronome.read(firstChild);
                break;
            case 'w':
                if (n.equals(MxlWedge.ELEM_NAME)) content = MxlWedge.read(firstChild); else if (n.equals(MxlWords.ELEM_NAME)) content = MxlWords.read(firstChild);
                break;
        }
        if (content != null) return new MxlDirectionType(content); else return null;
    }
