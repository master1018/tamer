    ...
    private void updateData(ConversionType ct) {
        ...
        try {
            for (int i = 0; i < conversionUnits.size(); i++) {
                ct.addConversion(Conversion.createInstance(conversionUnits.get(0).toString(), "", conversionUnits.get(i).toString(), "", factor.get(i).toString(), 0));
            }
            ...
            while (iter.hasNext()) {
                ...
                try {
                    Date date = new SimpleDateFormat("MM/dd/yyyy hh:mma z").parse(dateFormat.format(minDate) + " " + next.trim().replaceAll("ET", "EST"));
