...
public class AtomPartRegExPredicate extends AtomRegExPredicate {
    ...
    public boolean satisfies(HyperGraph hg, HGHandle handle) {
        ...
        if (projected != null) return satisfies(projected.getValue()); else return false;
    }
    ...
    public String toString() {
        ...
        if (dimensionPath != null) {
            for (int i = 0; i < dimensionPath.length; i++) {
                ...
                if (i + 1 < dimensionPath.length) result.append(".");
            }
            result.append(", ");
        }
    ...
    public boolean equals(Object x) {
        if (!(x instanceof AtomPartRegExPredicate)) return false;
        if (!super.equals(x)) return false; else {
