...
public class IDEWidgetsPanel extends JPanel {
    ...
    public IDEWidgetsPanel(MeTA ideInstance) {
        ...
        scrollPanel = new JScrollPane(thePanel, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    ...
    private void showAddRemoveWidgetsDialog() {
        ...
        contentPane.add(new JScrollPane(widgetsListUI, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED), BorderLayout.CENTER);
        ...
        addWidget.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                IDEFileFilter fileFilter = new IDEFileFilter("bsh", "BeanShell script files");
                fileChooser.addChoosableFileFilter(fileFilter);
                fileChooser.setFileView(new IDEFileView());
                if (fileChooser.showOpenDialog(IDEWidgetsPanel.this) == IDEFileChooser.APPROVE_OPTION) {
                    if (!fileChooser.getSelectedFile().exists()) {
                        return;
                    }
                    try {
                        ideInstance.getStartupScriptEngine().sourceWidget(fileChooser.getSelectedFile().getAbsolutePath());
                        String widgetDir = StringResource.getInstance().getWidgetsDir();
                        String fileName = widgetDir + File.separatorChar + fileChooser.getSelectedFile().getName();
                        BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(fileChooser.getSelectedFile().getAbsolutePath()), "UTF-8"));
                        String theLine;
                        ...
                        System.err.println("Error while adding new widget: " + e.toString());
                        e.printStackTrace();
                    }
        ...
        removeWidget.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                String widgetDir = StringResource.getInstance().getWidgetsDir();
                String fileName = widgetDir + File.separatorChar + widget.getWidgetID() + ".bsh";
    ...
    public void addWidget(IDEWidget widget) {
        if (!(widget instanceof IDEWidget)) {
            throw new UnsupportedOperationException("Only instance of " + "IDEWidget can be added to IDEWidgetsPanel");
        }
        if (widget.getWidgetID() == null) {
            throw new UnsupportedOperationException("WidgetID can not " + "be null! Please provide a valid WidgetID for the " + "widget to be added.");
        }
        if (widgetsList.contains(widget)) {
            throw new UnsupportedOperationException("A widget with WidgetID '" + widget.getWidgetID() + "' is already present! This" + " widget is not added.");
        }
