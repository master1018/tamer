...
public class CategoryListPanel extends ListPanelLayout {
    ...
    protected AbstractBudgetPanel initActions() {
        ...
        editButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (getSelectedCategory() != null) new CategoryModifyDialog().loadSource(getSelectedCategory()).openWindow();
            }
        });
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (getSelectedCategory() != null) {
                    ...
                    if (deleteButton.getText().equals(Translate.getInstance().get(TranslateKeys.DELETE))) {
                        if (DataInstance.getInstance().getTransactions(c).size() > 0 || c.getChildren().size() > 0 || JOptionPane.showConfirmDialog(CategoryListPanel.this, Translate.getInstance().get(TranslateKeys.NO_TRANSACTIONS_USING_CATEGORY), Translate.getInstance().get(TranslateKeys.PERMANENT_DELETE_CATEGORY), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.NO_OPTION) {
                            DataInstance.getInstance().deleteSource(c);
                        } else {
                            DataInstance.getInstance().deleteSourcePermanent(c);
                            if (c.getParent() != null) c.getParent().getChildren().remove(c);
                        }
                    } else {
        ...
        tree.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent arg0) {
                if (arg0.getClickCount() >= 2) editButton.doClick(); else super.mouseClicked(arg0);
            }
        });
        return this;
    }
    ...
    public AbstractBudgetPanel updateContent() {
        ...
        for (Category c : DataInstance.getInstance().getCategories()) {
            ...
            if (c.isIncome()) income += c.getBudgetedAmount(); else expenses += c.getBudgetedAmount();
        }
        if (income < expenses) balanceLabel.setForeground(Color.RED); else balanceLabel.setForeground(Color.BLACK);
        StringBuffer sb = new StringBuffer();
        sb.append(Translate.getInstance().get(TranslateKeys.BUDGET_NET_INCOME)).append(" ").append(Translate.getInstance().get(PrefsInstance.getInstance().getSelectedInterval().getName())).append(": ").append(((income - expenses) >= 0 ? "" : "-")).append(Translate.getInstance().get(TranslateKeys.CURRENCY_SIGN)).append(Formatter.getInstance().getDecimalFormat().format(Math.abs((double) Math.abs(income - expenses) / 100.0)));
        ...
        for (DefaultMutableTreeNode node : nodes) {
            ...
            if (l != null && l.isUnrolled()) tree.expandPath(new TreePath(node.getPath()));
        }
        return this;
    }
    ...
    public Category getSelectedCategory() {
        if (selectedSource instanceof Category) return (Category) selectedSource; else return null;
    }
}
