...
public class ParameterList {
    ...
    public ParameterList(String s) throws ParseException {
        ...
        for (; ; ) {
            ...
            int type = tk.getType();
            String name, value;
            if (type == HeaderTokenizer.Token.EOF) break;
            if ((char) type == ';') {
                tk = h.next();
                if (tk.getType() == HeaderTokenizer.Token.EOF) break;
                if (tk.getType() != HeaderTokenizer.Token.ATOM) throw new ParseException("Expected parameter name, " + "got \"" + tk.getValue() + "\"");
                name = tk.getValue().toLowerCase(Locale.ENGLISH);
                tk = h.next();
                if ((char) tk.getType() != '=') throw new ParseException("Expected '=', " + "got \"" + tk.getValue() + "\"");
                tk = h.next();
                type = tk.getType();
                if (type != HeaderTokenizer.Token.ATOM && type != HeaderTokenizer.Token.QUOTEDSTRING) throw new ParseException("Expected parameter value, " + "got \"" + tk.getValue() + "\"");
                value = tk.getValue();
                lastName = name;
                if (decodeParameters) putEncodedName(name, value); else list.put(name, value);
            } else {
                if (applehack && type == HeaderTokenizer.Token.ATOM && lastName != null && (lastName.equals("name") || lastName.equals("filename"))) {
    ...
    public String get(String name) {
        ...
        if (v instanceof MultiValue) value = ((MultiValue) v).value; else if (v instanceof Value) value = ((Value) v).value; else value = (String) v;
        return value;
    }
    ...
    public void set(String name, String value, String charset) {
        if (encodeParameters) {
            ...
            if (ev != null) list.put(name.trim().toLowerCase(Locale.ENGLISH), ev); else set(name, value);
        } else set(name, value);
    }
        ...
        while (e.hasNext()) {
            ...
            if (v instanceof MultiValue) {
                ...
                for (int i = 0; i < vv.size(); i++) {
                    Object va = vv.get(i);
                    if (va instanceof Value) sb.addNV(ns + i + "*", ((Value) va).encodedValue); else sb.addNV(ns + i, (String) va);
                }
            } else if (v instanceof Value) sb.addNV(name + "*", ((Value) v).encodedValue); else sb.addNV(name, (String) v);
        }
        return sb.toString();
    }
    ...
    private static class ToStringBuffer {
        ...
        public void addNV(String name, String value) {
            ...
            if (used + value.length() > 76) {
                ...
                if (lastlf >= 0) used += s.length() - lastlf - 1; else used += s.length();
            } else {
                sb.append(value);
                used += value.length();
            }
    ...
    private static Value encodeValue(String value, String charset) {
        if (MimeUtility.checkAscii(value) == MimeUtility.ALL_ASCII) return null;
        byte[] b;
        try {
    ...
    private static Value decodeValue(String value) throws ParseException {
        ...
        try {
            int i = value.indexOf('\'');
            if (i <= 0) {
                if (decodeParametersStrict) throw new ParseException("Missing charset in encoded value: " + value);
                return v;
            }
            String charset = value.substring(0, i);
            int li = value.indexOf('\'', i + 1);
            if (li < 0) {
                if (decodeParametersStrict) throw new ParseException("Missing language in encoded value: " + value);
                return v;
            }
