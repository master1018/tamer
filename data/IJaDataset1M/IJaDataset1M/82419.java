...
public abstract class AvalancheVSDummyMode extends DummyMode {
    ...
    /** Block colors */
    public static final int[] BLOCK_COLORS = { Block.BLOCK_COLOR_RED, Block.BLOCK_COLOR_GREEN, Block.BLOCK_COLOR_BLUE, Block.BLOCK_COLOR_YELLOW, Block.BLOCK_COLOR_PURPLE };

    /** Fever map files list */
    public static final String[] FEVER_MAPS = { "Fever", "15th", "15thDS", "7", "Compendium" };

    /** Chain multipliers */
    public static final int[] CHAIN_POWERS = { 4, 12, 24, 33, 50, 101, 169, 254, 341, 428, 538, 648, 763, 876, 990, 999 };
    ...
    public static final int CHAIN_DISPLAY_NONE = 0, CHAIN_DISPLAY_YELLOW = 1, CHAIN_DISPLAY_PLAYER = 2, CHAIN_DISPLAY_SIZE = 3;
    ...
    protected void loadOtherSetting(GameEngine engine, CustomProperties prop, String name) {
        ...
        if (feverMapSet[playerID] >= 0 && feverMapSet[playerID] < FEVER_MAPS.length) loadMapSetFever(engine, playerID, feverMapSet[playerID], true);
    }
    ...
    public void playerInit(GameEngine engine, int playerID) {
        ...
        for (int i = 0; i < Piece.PIECE_COUNT; i++) engine.nextPieceEnable[i] = (PIECE_ENABLE[i] == 1);
    ...
    public boolean onReady(GameEngine engine, int playerID) {
        if (engine.statc[0] == 0) return readyInit(engine, playerID);
        return false;
    }
    ...
    public boolean readyInit(GameEngine engine, int playerID) {
        ...
        } else if (feverMapSet[playerID] >= 0 && feverMapSet[playerID] < FEVER_MAPS.length) loadMapSetFever(engine, playerID, feverMapSet[playerID], true);
        if (useMap[playerID]) {
            if (owner.replayMode) {
                engine.createFieldIfNeeded();
    ...
    protected int calcChainMultiplier(GameEngine engine, int playerID, int chain) {
        if (newChainPower[playerID]) return calcChainNewPower(engine, playerID, chain); else return calcChainClassicPower(engine, playerID, chain);
    }

    protected int calcChainNewPower(GameEngine engine, int playerID, int chain) {
        if (chain > CHAIN_POWERS.length) return CHAIN_POWERS[CHAIN_POWERS.length - 1]; else return CHAIN_POWERS[chain - 1];
    }

    protected int calcChainClassicPower(GameEngine engine, int playerID, int chain) {
        if (chain == 2) return 8; else if (chain == 3) return 16; else if (chain >= 4) return 32 * (chain - 3); else return 0;
    }
    ...
    protected void addOjama(GameEngine engine, int playerID, int pts) {
        ...
        if (hurryupSeconds[playerID] > 0 && engine.statistics.time > hurryupSeconds[playerID]) rate >>= engine.statistics.time / (hurryupSeconds[playerID] * 60);
        if (rate <= 0) rate = 1;
    ...
    protected void gameOverCheck(GameEngine engine, int playerID) {
        ...
        if (big[playerID]) {
            if (!engine.field.getBlockEmpty(1, 0)) engine.stat = GameEngine.STAT_GAMEOVER;
        } else if (!engine.field.getBlockEmpty(2, 0) || (dangerColumnDouble[playerID] && !engine.field.getBlockEmpty(3, 0))) engine.stat = GameEngine.STAT_GAMEOVER;
    }
    ...
    protected void loadFeverMap(GameEngine engine, int playerID, Random rand, int chain, int subset) {
        engine.createFieldIfNeeded();
        engine.field.reset();
        engine.field.stringToField(propFeverMap[playerID].getProperty(feverMapSubsets[playerID][subset] + "." + numColors[playerID] + "colors." + chain + "chain"));
    ...
    protected void updateOjamaMeter(GameEngine engine, int playerID) {
        int width = 6;
        if (engine.field != null) width = engine.field.getWidth();
        int blockHeight = receiver.getBlockGraphicsHeight(engine, playerID);
        int value = ojama[playerID] * blockHeight / width;
        if (ojama[playerID] >= 5 * width) engine.meterColor = GameEngine.METER_COLOR_RED; else if (ojama[playerID] >= width) engine.meterColor = GameEngine.METER_COLOR_ORANGE; else if (ojama[playerID] >= 1) engine.meterColor = GameEngine.METER_COLOR_YELLOW; else engine.meterColor = GameEngine.METER_COLOR_GREEN;
        if (value > engine.meterValue) engine.meterValue++; else if (value < engine.meterValue) engine.meterValue--;
    }
    ...
    public void renderLast(GameEngine engine, int playerID) {
        ...
        if (engine.chain > 0 && chainDisplay[playerID] > 0 && chainDisplayType[playerID] != CHAIN_DISPLAY_NONE) receiver.drawMenuFont(engine, playerID, baseX + (engine.chain > 9 ? 0 : 1), textHeight, engine.chain + " CHAIN!", getChainColor(engine, playerID));
        if (zenKeshi[playerID] || zenKeshiDisplay[playerID] > 0) receiver.drawMenuFont(engine, playerID, baseX + 1, textHeight + 1, "ZENKESHI!", EventReceiver.COLOR_YELLOW);
    }

    protected int getChainColor(GameEngine engine, int playerID) {
        if (chainDisplayType[playerID] == CHAIN_DISPLAY_PLAYER) return (playerID == 0) ? EventReceiver.COLOR_RED : EventReceiver.COLOR_BLUE; else if (chainDisplayType[playerID] == CHAIN_DISPLAY_SIZE) return (engine.chain >= rensaShibari[playerID]) ? EventReceiver.COLOR_GREEN : EventReceiver.COLOR_RED; else return EventReceiver.COLOR_YELLOW;
    }
    ...
    protected void drawX(GameEngine engine, int playerID) {
        ...
        for (int i = 0; i < ((dangerColumnDouble[playerID] && !big[playerID]) ? 2 : 1); i++) {
            if ((engine.field == null) || (engine.field.getBlockEmpty(baseX + i, 0))) {
                if (big[playerID]) receiver.drawMenuFont(engine, playerID, 2, 0, "e", EventReceiver.COLOR_RED, 2.0f); else if (engine.displaysize == 1) receiver.drawMenuFont(engine, playerID, 4 + (i * 2), 0, "e", EventReceiver.COLOR_RED, 2.0f); else receiver.drawMenuFont(engine, playerID, 2 + i, 0, "e", EventReceiver.COLOR_RED);
            }
        }
    }

    protected void drawHardOjama(GameEngine engine, int playerID) {
        if (engine.field != null) for (int x = 0; x < engine.field.getWidth(); x++) for (int y = 0; y < engine.field.getHeight(); y++) {
            int hard = engine.field.getBlock(x, y).hard;
            if (hard > 0) {
                if (engine.displaysize == 1) receiver.drawMenuFont(engine, playerID, x * 2, y * 2, String.valueOf(hard), EventReceiver.COLOR_YELLOW, 2.0f); else receiver.drawMenuFont(engine, playerID, x, y, String.valueOf(hard), EventReceiver.COLOR_YELLOW);
            }
        }
    }
    ...
    protected void drawScores(GameEngine engine, int playerID, int x, int y, int headerColor) {
        ...
        if (scgettime[0] > 0 && lastscore[0] > 0 && lastmultiplier[0] > 0) receiver.drawScoreFont(engine, playerID, x + 4, y, "+" + lastscore[0] + "e" + lastmultiplier[0], EventReceiver.COLOR_RED); else receiver.drawScoreFont(engine, playerID, x + 4, y, String.valueOf(score[0]), EventReceiver.COLOR_RED);
        y++;
        receiver.drawScoreFont(engine, playerID, x, y, "2P: ", EventReceiver.COLOR_BLUE);
        if (scgettime[1] > 0 && lastscore[1] > 0 && lastmultiplier[1] > 0) receiver.drawScoreFont(engine, playerID, x + 4, y, "+" + lastscore[1] + "e" + lastmultiplier[1], EventReceiver.COLOR_BLUE); else receiver.drawScoreFont(engine, playerID, x + 4, y, String.valueOf(score[1]), EventReceiver.COLOR_BLUE);
    }
    ...
    protected void drawOjama(GameEngine engine, int playerID, int x, int y, int headerColor) {
        ...
        if (ojamaAdd[0] > 0) ojamaStr1P = ojamaStr1P + "(+" + String.valueOf(ojamaAdd[0]) + ")";
        String ojamaStr2P = String.valueOf(ojama[1]);
        if (ojamaAdd[1] > 0) ojamaStr2P = ojamaStr2P + "(+" + String.valueOf(ojamaAdd[1]) + ")";
    ...
    public void renderResult(GameEngine engine, int playerID) {
        ...
        drawResult(engine, playerID, receiver, 3, EventReceiver.COLOR_ORANGE, "ATTACK", String.format("%10d", ojamaSent[playerID]), "CLEARED", String.format("%10d", engine.statistics.lines), "MAX CHAIN", String.format("%10d", engine.statistics.maxChain), "PIECE", String.format("%10d", engine.statistics.totalPieceLocked), "ATTACK/MIN", String.format("%10g", apm), "PIECE/SEC", String.format("%10g", engine.statistics.pps), "TIME", String.format("%10s", GeneralUtil.getTime(owner.engine[0].statistics.time)));
    }
}
