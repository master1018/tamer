...
public class StyleManager {
    ...
    public static class Style {
        ...
        public boolean equals(Object o) {
            ...
            Style that = (Style) o;
            return (this.proportional == that.proportional && this.size == that.size && this.face == that.face && this.foreColor == that.foreColor && this.backColor == that.backColor && this.reverse == that.reverse);
        }
        ...
        final boolean isDifferent(Style other, int type, boolean fakeReverse) {
            ...
            switch(type) {
                case TAP_TYPE:
                    return (this.face != other.face || this.proportional != other.proportional);
                case FGC_TYPE:
                    return (this.foreColor != other.foreColor || this.reverse != otherReverse);
                case BGC_TYPE:
                    return (this.backColor != other.backColor || this.reverse != otherReverse);
                case RS_TYPE:
        ...
        final CharacterStyle getSpan(int type, boolean fakeReverse) {
            switch(type) {
                case TAP_TYPE:
                    ...
                    return new TextAppearanceSpan((proportional ? "sans-serif" : "monospace"), face, -1, null, null);
                case FGC_TYPE:
        ...
        int measureStyle(int hint) {
            switch(hint) {
                ...
                case GlkStyleHint.Oblique:
                    if (face == Typeface.BOLD_ITALIC || face == Typeface.ITALIC) {
                        return 1;
                    } else {
                        return 0;
                    }
    ...
    public void applyStyle(int newStyleNum, boolean fakeReverse, Spannable text) {
        ...
        Style currentStyle = (currentStyleNum == -1 ? null : styles[currentStyleNum]);
        Style newStyle = styles[newStyleNum];
        for (int s = 0; s < 4; s++) {
            changeSpan = (currentStyle == null || currentStyle.isDifferent(newStyle, s, fakeReverse));
            if (!changeSpan && (s == 1 || s == 2)) {
                changeSpan = fakeReverse != currentReverse;
            }
            if (changeSpan) {
                if (oldSpans[s] != null) {
                    text.setSpan(oldSpans[s], oldStarts[s], textLen, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
                newSpan = newStyle.getSpan(s, fakeReverse);
                if (newSpan != null) {
                    text.setSpan(newSpan, textLen, textLen, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
                }
                oldSpans[s] = newSpan;
                oldStarts[s] = textLen;
            }
