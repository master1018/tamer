...
public final class Element2LatexImpl implements Element2Latex {
     ...
     *                          and so on.
     */
    private void addBackupPredicate(final String name, final String argNumber, final String latexPattern) {
     ...
     *                          and so on.
     */
    private void addBackupFunction(final String name, final String argNumber, final String latexPattern) {
        final String key = name + "_" + argNumber;
                ...
                try {
                    ...
                    if (external >= 0) {
                        ...
                        if (Element2LatexImpl.this.labels.getReferences() != null && Element2LatexImpl.this.labels.getReferences().size() > 0) {
                            ...
                            final DefaultKernelQedeqBo newProp = (DefaultKernelQedeqBo) Element2LatexImpl.this.labels.getReferences().getQedeqBo(label);
                            if (newProp != null) {
                                if (newProp.getExistenceChecker().predicateExists(shortName, arguments)) {
                                    definition = newProp.getLabels().getPredicate(shortName, arguments);
                                }
                            }
                        }
                    }
                        ...
                        if (Element2LatexImpl.this.labels.getReferences() != null && Element2LatexImpl.this.labels.getReferences().size() > 0) {
                            ...
                            if (newProp != null) {
                                if (newProp.getExistenceChecker().functionExists(shortName, arguments)) {
                                    definition = newProp.getLabels().getFunction(shortName, arguments);
                                }
                            }
                        }
                    }
