...
public class OrderingTest extends TestCase {
    ...
    public void testFrom() {
        ...
        new EqualsTester(caseInsensitiveOrdering).addEqualObject(Ordering.from(String.CASE_INSENSITIVE_ORDER)).addNotEqualObject(orderingFromOrdering).addNotEqualObject(Ordering.natural()).testEquals();
    }
    ...
    public void testExplicit_two() {
        ...
        new EqualsTester(c).addEqualObject(Ordering.explicit(42, 5)).addNotEqualObject(Ordering.explicit(5, 42)).addNotEqualObject(Ordering.explicit(42)).testEquals();
        reserializeAndAssert(c);
    }
    ...
    public void testCompound_static() {
        Comparator<String> comparator = Ordering.compound(Iterables.unmodifiableIterable(ImmutableList.of(byCharAt(0), byCharAt(1), byCharAt(2), byCharAt(3), byCharAt(4), byCharAt(5))));
    ...
    public void testReverse() {
        ...
        new EqualsTester(reverseOrder).addEqualObject(numberOrdering.reverse()).addNotEqualObject(Ordering.natural().reverse()).addNotEqualObject(Collections.reverseOrder()).testEquals();
    }
    ...
    public void testOnResultOf_natural() {
        ...
        new EqualsTester(comparator).addEqualObject(Ordering.natural().onResultOf(StringLengthFunction.StringLength)).addNotEqualObject(DECREASING_INTEGER).testEquals();
        reserializeAndAssert(comparator);
    ...
    public void testOnResultOf_chained() {
        ...
        new EqualsTester(comparator).addEqualObject(DECREASING_INTEGER.onResultOf(StringLengthFunction.StringLength)).addNotEqualObject(DECREASING_INTEGER.onResultOf(Functions.constant(1))).addNotEqualObject(Ordering.natural()).testEquals();
        reserializeAndAssert(comparator);
    ...
    public void testCombinationsExhaustively_startingFromExplicit() {
        testExhaustively(Ordering.explicit("a", "b", "c", "d"), Arrays.asList("b", "d"));
    }
    ...
    private static <T> void testExhaustively(Ordering<? super T> ordering, List<T> list) {
        @SuppressWarnings("unchecked") Scenario<T> starter = new Scenario<T>((Ordering) ordering, list);
   ...
   */
    private static class Scenario<T> {
        ...
        void testMinAndMax() {
            ...
            assertEquals(strictlyOrderedList.get(strictlyOrderedList.size() - 1), ordering.max(shuffledList));
        }
