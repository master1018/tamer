...
public abstract class FlowView extends BoxView {
   ...
   */
     ...
     */
        protected View createView(FlowView fv, int offset, int spanLeft, int rowIndex) {
            ...
            int flowAxis = fv.getFlowAxis();
            int span = (int) child.getPreferredSpan(flowAxis);
            if (span <= spanLeft) return child; else if (child.getBreakWeight(flowAxis, offset, spanLeft) > BadBreakWeight) return child.breakView(flowAxis, offset, 0, spanLeft); else return null;
        }
    }
   ...
   */
     ...
     */
        public void replace(int offset, int length, View[] views) {
            if (length > 0) {
                for (int count = 0; count < length; ++count) children.remove(offset);
            }
     ...
     */
        public float getPreferredSpan(int axis) {
            throw new AssertionError("This method must not be called in " + "LogicalView.");
        }
   ...
   */
    public int getFlowAxis() {
        int axis = getAxis();
        int flowAxis;
        if (axis == X_AXIS) flowAxis = Y_AXIS; else flowAxis = X_AXIS;
        return flowAxis;
    }
   ...
   */
    protected void layout(int width, int height) {
        ...
        if (rebuild) strategy.layout(this);
   ...
   */
    protected int getViewIndexAtPosition(int pos) {
        if (!isAllocationValid()) layout(getWidth(), getHeight());
