...
public class VSBattleMode extends DummyMode {
    ...
    private static final int EVENT_NONE = 0, EVENT_SINGLE = 1, EVENT_DOUBLE = 2, EVENT_TRIPLE = 3, EVENT_FOUR = 4, EVENT_TSPIN_SINGLE_MINI = 5, EVENT_TSPIN_SINGLE = 6, EVENT_TSPIN_DOUBLE = 7, EVENT_TSPIN_TRIPLE = 8, EVENT_TSPIN_DOUBLE_MINI = 9, EVENT_TSPIN_EZ = 10;
    ...
    public void renderSetting(GameEngine engine, int playerID) {
        if (engine.statc[4] == 0) {
            if (engine.statc[2] < 9) {
                ...
                drawMenu(engine, playerID, receiver, 14, EventReceiver.COLOR_GREEN, 7, "LOAD", String.valueOf(presetNumber[playerID]), "SAVE", String.valueOf(presetNumber[playerID]));
                ...
                drawMenu(engine, playerID, receiver, 0, EventReceiver.COLOR_CYAN, 9, "GARBAGE", GARBAGE_TYPE_STRING[garbageType[playerID]], "CHANGERATE", garbagePercent[playerID] + "%", "COUNTERING", GeneralUtil.getONorOFF(garbageCounter[playerID]), "BLOCKING", GeneralUtil.getONorOFF(garbageBlocking[playerID]), "SPIN BONUS", strTSpinEnable, "KICK SPIN", GeneralUtil.getONorOFF(enableTSpinKick[playerID]), "SPIN TYPE", (spinCheckType[playerID] == 0) ? "4POINT" : "IMMOBILE", "EZIMMOBILE", GeneralUtil.getONorOFF(tspinEnableEZ[playerID]), "B2B", strB2BType, "COMBO", GeneralUtil.getONorOFF(enableCombo[playerID]));
            } else {
                drawMenu(engine, playerID, receiver, 0, EventReceiver.COLOR_CYAN, 19, "BIG", GeneralUtil.getONorOFF(big[playerID]), "SE", GeneralUtil.getONorOFF(enableSE[playerID]), "HURRYUP", (hurryupSeconds[playerID] == -1) ? "NONE" : hurryupSeconds[playerID] + "SEC", "INTERVAL", String.valueOf(hurryupInterval[playerID]));
                drawMenu(engine, playerID, receiver, 8, EventReceiver.COLOR_PINK, 23, "BGM", String.valueOf(bgmno), "SHOW STATS", GeneralUtil.getONorOFF(showStats));
                drawMenu(engine, playerID, receiver, 12, EventReceiver.COLOR_CYAN, 25, "USE MAP", GeneralUtil.getONorOFF(useMap[playerID]), "MAP SET", String.valueOf(mapSet[playerID]), "MAP NO.", (mapNumber[playerID] < 0) ? "RANDOM" : mapNumber[playerID] + "/" + (mapMaxNo[playerID] - 1));
            }
        } else {
    ...
    public void renderLast(GameEngine engine, int playerID) {
        if (playerID == 0) {
            ...
            if ((hurryupSeconds[playerID] >= 0) && (engine.timerActive) && (engine.statistics.time >= hurryupSeconds[playerID] * 60) && (engine.statistics.time < (hurryupSeconds[playerID] + 5) * 60)) {
        ...
        if (showStats) {
            ...
            if (owner.receiver.getNextDisplayType() == 2) {
                ...
                if (garbageSent[playerID] >= 10) receiver.drawDirectFont(engine, playerID, x - 44, y + 142, String.valueOf(garbageSent[playerID])); else receiver.drawDirectFont(engine, playerID, x - 36, y + 142, String.valueOf(garbageSent[playerID]));
                receiver.drawDirectFont(engine, playerID, x - 44, y + 190, "WINS", fontColor, 0.5f);
                if (winCount[playerID] >= 10) receiver.drawDirectFont(engine, playerID, x - 44, y + 204, String.valueOf(winCount[playerID])); else receiver.drawDirectFont(engine, playerID, x - 36, y + 204, String.valueOf(winCount[playerID]));
            }
        }
        if ((lastevent[playerID] != EVENT_NONE) && (scgettime[playerID] < 120)) {
            ...
            switch(lastevent[playerID]) {
                ...
                case EVENT_TRIPLE:
                    ...
                    break;
                case EVENT_FOUR:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 3, 21, "FOUR", EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 3, 21, "FOUR", EventReceiver.COLOR_ORANGE);
                    break;
                case EVENT_TSPIN_SINGLE_MINI:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-MINI-S", EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-MINI-S", EventReceiver.COLOR_ORANGE);
                    break;
                case EVENT_TSPIN_SINGLE:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-SINGLE", EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-SINGLE", EventReceiver.COLOR_ORANGE);
                    break;
                case EVENT_TSPIN_DOUBLE_MINI:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-MINI-D", EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-MINI-D", EventReceiver.COLOR_ORANGE);
                    break;
                case EVENT_TSPIN_DOUBLE:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-DOUBLE", EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-DOUBLE", EventReceiver.COLOR_ORANGE);
                    break;
                case EVENT_TSPIN_TRIPLE:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-TRIPLE", EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 1, 21, strPieceName + "-TRIPLE", EventReceiver.COLOR_ORANGE);
                    break;
                case EVENT_TSPIN_EZ:
                    if (lastb2b[playerID]) receiver.drawMenuFont(engine, playerID, 3, 21, "EZ-" + strPieceName, EventReceiver.COLOR_RED); else receiver.drawMenuFont(engine, playerID, 3, 21, "EZ-" + strPieceName, EventReceiver.COLOR_ORANGE);
                    break;
            }
            if (lastcombo[playerID] >= 2) receiver.drawMenuFont(engine, playerID, 2, 22, (lastcombo[playerID] - 1) + "COMBO", EventReceiver.COLOR_CYAN);
        }
    }
    ...
    public void calcScore(GameEngine engine, int playerID, int lines) {
        ...
        if (lines > 0) {
            ...
            if (engine.b2b) {
                ...
                if (pts > 0) {
                    if ((version >= 1) && (lastevent[playerID] == EVENT_TSPIN_TRIPLE) && (!engine.useAllSpinBonus)) ptsB2B += 2; else ptsB2B += 1;
                    if (b2bType[playerID] == 1) pts += ptsB2B;
                }
            } else {
                lastb2b[playerID] = false;
            }
    ...
    public void onLast(GameEngine engine, int playerID) {
        ...
        if (garbage[playerID] >= 4) engine.meterColor = GameEngine.METER_COLOR_RED; else if (garbage[playerID] >= 3) engine.meterColor = GameEngine.METER_COLOR_ORANGE; else if (garbage[playerID] >= 1) engine.meterColor = GameEngine.METER_COLOR_YELLOW; else engine.meterColor = GameEngine.METER_COLOR_GREEN;
    ...
    public void renderResult(GameEngine engine, int playerID) {
        ...
        drawResult(engine, playerID, receiver, 2, EventReceiver.COLOR_ORANGE, "ATTACK", String.format("%10d", garbageSent[playerID]));
        drawResultStats(engine, playerID, receiver, 4, EventReceiver.COLOR_ORANGE, STAT_LINES, STAT_PIECE);
        drawResult(engine, playerID, receiver, 8, EventReceiver.COLOR_ORANGE, "ATK/LINE", String.format("%10g", apl));
        drawResult(engine, playerID, receiver, 10, EventReceiver.COLOR_ORANGE, "ATTACK/MIN", String.format("%10g", apm));
        drawResultStats(engine, playerID, receiver, 12, EventReceiver.COLOR_ORANGE, STAT_LPM, STAT_PPS, STAT_TIME);
    }
