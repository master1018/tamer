...
public class RecoderUtils {
     ...
     */
    public static boolean isModelPart(ServiceConfiguration sc, ProgramModelElement pme) {
        ...
        if (pme instanceof Member) {
            ...
            if (pme instanceof Constructor) {
                return parent.getConstructors().indexOf(pme) >= 0 && isModelPart(sc, parent);
            }
            if (pme instanceof Method) {
                return parent.getMethods().indexOf(pme) >= 0 && isModelPart(sc, parent);
            }
            if (pme instanceof Field) {
                return parent.getFields().indexOf(pme) >= 0 && isModelPart(sc, parent);
            }
            if (pme instanceof ClassType) {
                return parent.getTypes().indexOf(pme) >= 0 && isModelPart(sc, parent);
            }
        }
        return false;
    }
