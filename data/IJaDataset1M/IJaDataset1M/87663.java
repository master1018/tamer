...
/**
 * Uma produ��o de uma gram�tica
 * Esta produ�ao � da forma A -> X1 X2 X3 ... Xn
 *
...
public class Production implements Comparable {
    ...
    public int firstSymbol() {
        for (int i = 0; i < rhs.size(); i++) if (!grammar.isSemanticAction(rhs.get(i))) return rhs.get(i);
        return 0;
    }
    ...
    public String toString() {
        ...
        if (rhs.size() == 0) bfr.append(" " + Grammar.EPSILON_STR); else for (int j = 0; j < rhs.size(); j++) {
            if (grammar.isSemanticAction(rhs.get(j))) {
                bfr.append(" #").append(rhs.get(j) - grammar.FIRST_SEMANTIC_ACTION());
            } else bfr.append(" ").append(grammar.symbols[rhs.get(j)]);
        }
        return bfr.toString();
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (lhs != p.lhs) return false; else if (rhs.size() != p.rhs.size()) return false; else for (int i = 0; i < rhs.size(); i++) if (rhs.get(i) != p.rhs.get(i)) return false;
        return true;
    }

    public int compareTo(Object o) {
        Production p = (Production) o;
        if (lhs != p.lhs) return lhs - p.lhs; else {
            boolean e1 = grammar.isEpsilon(rhs);
            boolean e2 = grammar.isEpsilon(p.rhs);
            if (e1 && e2) return 0; else if (e1) return 1; else if (e2) return -1; else for (int i = 0; i < rhs.size() && i < p.rhs.size(); i++) {
