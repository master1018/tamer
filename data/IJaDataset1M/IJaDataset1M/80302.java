...
public final class Parser implements TokenId {
    ...
    public ASTList parseMember(SymbolTable tbl) throws CompileError {
        ...
        if (mem instanceof MethodDecl) return parseMethod2(tbl, (MethodDecl) mem); else return mem;
    }
    ...
    public ASTList parseMember1(SymbolTable tbl) throws CompileError {
        ...
        if (lex.lookAhead() == Identifier && lex.lookAhead(1) == '(') {
            ...
            isConstructor = true;
        } else d = parseFormalType(tbl);
        if (lex.get() != Identifier) throw new SyntaxError(lex);
        String name;
        if (isConstructor) name = MethodDecl.initName; else name = lex.getString();
        d.setVariable(new Symbol(name));
        if (isConstructor || lex.lookAhead() == '(') return parseMethod1(tbl, isConstructor, mods, d); else return parseField(tbl, mods, d);
    }
    ...
    private FieldDecl parseField(SymbolTable tbl, ASTList mods, Declarator d) throws CompileError {
        ...
        int c = lex.get();
        if (c == ';') return new FieldDecl(mods, new ASTList(d, new ASTList(expr))); else if (c == ',') throw new CompileError("only one field can be declared in one declaration", lex); else throw new SyntaxError(lex);
    }

    private MethodDecl parseMethod1(SymbolTable tbl, boolean isConstructor, ASTList mods, Declarator d) throws CompileError {
        if (lex.get() != '(') throw new SyntaxError(lex);
        ASTList parms = null;
        if (lex.lookAhead() != ')') while (true) {
            parms = ASTList.append(parms, parseFormalParam(tbl));
            int t = lex.lookAhead();
            if (t == ',') lex.get(); else if (t == ')') break;
        }
        lex.get();
        d.addArrayDim(parseArrayDimension());
        if (isConstructor && d.getArrayDim() > 0) throw new SyntaxError(lex);
        ASTList throwsList = null;
        if (lex.lookAhead() == THROWS) {
            lex.get();
            while (true) {
                throwsList = ASTList.append(throwsList, parseClassType(tbl));
                if (lex.lookAhead() == ',') lex.get(); else break;
            }
        }
    ...
    public MethodDecl parseMethod2(SymbolTable tbl, MethodDecl md) throws CompileError {
        Stmnt body = null;
        if (lex.lookAhead() == ';') lex.get(); else {
            body = parseBlock(tbl);
            if (body == null) body = new Stmnt(BLOCK);
        }
    ...
    private ASTList parseMemberMods() {
        ...
        while (true) {
            t = lex.lookAhead();
            if (t == ABSTRACT || t == FINAL || t == PUBLIC || t == PROTECTED || t == PRIVATE || t == SYNCHRONIZED || t == STATIC || t == VOLATILE || t == TRANSIENT || t == STRICT) list = new ASTList(new Keyword(lex.get()), list); else break;
        }
        return list;
    }
    ...
    public Stmnt parseStatement(SymbolTable tbl) throws CompileError {
        int t = lex.lookAhead();
        if (t == '{') return parseBlock(tbl); else if (t == ';') {
            lex.get();
    ...
    private Stmnt parseBlock(SymbolTable tbl) throws CompileError {
        ...
        while (lex.lookAhead() != '}') {
            ...
            if (s != null) body = (Stmnt) ASTList.concat(body, new Stmnt(BLOCK, s));
        }
        lex.get();
        if (body == null) return new Stmnt(BLOCK); else return body;
    }
    ...
    private Stmnt parseDo(SymbolTable tbl) throws CompileError {
        int t = lex.get();
        Stmnt body = parseStatement(tbl);
        if (lex.get() != WHILE || lex.get() != '(') throw new SyntaxError(lex);
        ASTree expr = parseExpression(tbl);
        if (lex.get() != ')' || lex.get() != ';') throw new SyntaxError(lex);
        return new Stmnt(t, expr, body);
    }
    ...
    private Stmnt parseFor(SymbolTable tbl) throws CompileError {
        ...
        if (lex.lookAhead() == ';') {
            lex.get();
            expr1 = null;
        } else expr1 = parseDeclarationOrExpression(tbl2, true);
        if (lex.lookAhead() == ';') expr2 = null; else expr2 = parseExpression(tbl2);
        if (lex.get() != ';') throw new CompileError("; is missing", lex);
        if (lex.lookAhead() == ')') expr3 = null; else expr3 = parseExprList(tbl2);
        if (lex.get() != ')') throw new CompileError(") is missing", lex);
    ...
    private Stmnt parseSwitchBlock(SymbolTable tbl) throws CompileError {
        ...
        if (s == null) throw new CompileError("empty switch block", lex);
        int op = s.getOperator();
        if (op != CASE && op != DEFAULT) throw new CompileError("no case or default in a switch block", lex);
        ...
        while (lex.lookAhead() != '}') {
            ...
            if (s2 != null) {
                ...
                if (op2 == CASE || op2 == DEFAULT) {
                    ...
                    s = s2;
                } else s = (Stmnt) ASTList.concat(s, new Stmnt(BLOCK, s2));
            }
        }
        lex.get();
        return body;
    ...
    private Stmnt parseStmntOrCase(SymbolTable tbl) throws CompileError {
        int t = lex.lookAhead();
        if (t != CASE && t != DEFAULT) return parseStatement(tbl);
        lex.get();
        Stmnt s;
        if (t == CASE) s = new Stmnt(t, parseExpression(tbl)); else s = new Stmnt(DEFAULT);
        if (lex.get() != ':') throw new CompileError(": is missing", lex);
        return s;
    }
    ...
    private Stmnt parseTry(SymbolTable tbl) throws CompileError {
        ...
        while (lex.lookAhead() == CATCH) {
            ...
            if (d.getArrayDim() > 0 || d.getType() != CLASS) throw new SyntaxError(lex);
            if (lex.get() != ')') throw new SyntaxError(lex);
    ...
    private Stmnt parseReturn(SymbolTable tbl) throws CompileError {
        int t = lex.get();
        Stmnt s = new Stmnt(t);
        if (lex.lookAhead() != ';') s.setLeft(parseExpression(tbl));
        if (lex.get() != ';') throw new CompileError("; is missing", lex);
        return s;
    }
    ...
    private Stmnt parseContinue(SymbolTable tbl) throws CompileError {
        ...
        if (t2 != ';') throw new CompileError("; is missing", lex);
        return s;
    }
    ...
    private Stmnt parseDeclarationOrExpression(SymbolTable tbl, boolean exprList) throws CompileError {
        ...
        if (isBuiltinType(t)) {
            ...
            if (i >= 0) if (lex.lookAhead(i) == Identifier) {
                ASTList name = parseClassType(tbl);
                int dim = parseArrayDimension();
        ...
        Stmnt expr;
        if (exprList) expr = parseExprList(tbl); else expr = new Stmnt(EXPR, parseExpression(tbl));
        if (lex.get() != ';') throw new CompileError("; is missing", lex);
        return expr;
    }
    ...
    private Stmnt parseExprList(SymbolTable tbl) throws CompileError {
        ...
        for (; ; ) {
            ...
            if (lex.lookAhead() == ',') lex.get(); else return expr;
        }
    }
    ...
    private Stmnt parseDeclarators(SymbolTable tbl, Declarator d) throws CompileError {
        ...
        for (; ; ) {
            ...
            int t = lex.get();
            if (t == ';') return decl; else if (t != ',') throw new CompileError("; is missing", lex);
        }
    }

    private Declarator parseDeclarator(SymbolTable tbl, Declarator d) throws CompileError {
        if (lex.get() != Identifier || d.getType() == VOID) throw new SyntaxError(lex);
        String name = lex.getString();
    ...
    private ASTree parseInitializer(SymbolTable tbl) throws CompileError {
        if (lex.lookAhead() == '{') return parseArrayInitializer(tbl); else return parseExpression(tbl);
    }
    ...
    public ASTree parseExpression(SymbolTable tbl) throws CompileError {
        ...
        if (!isAssignOp(lex.lookAhead())) return left;
        int t = lex.get();
    ...
    private ASTree parseBinaryExpr(SymbolTable tbl) throws CompileError {
        ...
        for (; ; ) {
            ...
            if (p == 0) return expr; else expr = binaryExpr2(tbl, expr, p);
        }
    }
    ...
    private ASTree binaryExpr2(SymbolTable tbl, ASTree expr, int prec) throws CompileError {
        int t = lex.get();
        if (t == INSTANCEOF) return parseInstanceOf(tbl, expr);
        ASTree expr2 = parseUnaryExpr(tbl);
        for (; ; ) {
            int t2 = lex.lookAhead();
            int p2 = getOpPrecedence(t2);
            if (p2 != 0 && prec > p2) expr2 = binaryExpr2(tbl, expr2, p2); else return BinExpr.makeBin(t, expr, expr2);
        }
    }
    ...
    private int getOpPrecedence(int c) {
        if ('!' <= c && c <= '?') return binaryOpPrecedence[c - '!']; else if (c == '^') return 7; else if (c == '|') return 8; else if (c == ANDAND) return 9; else if (c == OROR) return 10; else if (c == EQ || c == NEQ) return 5; else if (c == LE || c == GE || c == INSTANCEOF) return 4; else if (c == LSHIFT || c == RSHIFT || c == ARSHIFT) return 3; else return 0;
    }
    ...
    private ASTree parseCast(SymbolTable tbl) throws CompileError {
        ...
        } else return parsePostfix(tbl);
    }

    private boolean nextIsBuiltinCast() {
        int t;
        int i = 2;
        while ((t = lex.lookAhead(i++)) == '[') if (lex.lookAhead(i++) != ']') return false;
    ...
    private int nextIsClassType(int i) {
        int t;
        while (lex.lookAhead(++i) == '.') if (lex.lookAhead(++i) != Identifier) return -1;
        while ((t = lex.lookAhead(i++)) == '[') if (lex.lookAhead(i++) != ']') return -1;
        return i - 1;
    }
        ...
        while (true) {
            switch(lex.lookAhead()) {
                ...
                case '[':
                    if (lex.lookAhead(1) == ']') {
                        int dim = parseArrayDimension();
                        if (lex.get() != '.' || lex.get() != CLASS) throw new SyntaxError(lex);
                        expr = parseDotClass(expr, dim);
                    } else {
                        index = parseArrayIndex(tbl);
                        if (index == null) throw new SyntaxError(lex);
                ...
                case '.':
                    ...
                    } else throw new CompileError("missing member name", lex);
                    break;
                case '#':
                    lex.get();
                    t = lex.get();
                    if (t != Identifier) throw new CompileError("missing static member name", lex);
                    str = lex.getString();
    ...
    private ASTree parseMethodCall(SymbolTable tbl, ASTree expr) throws CompileError {
        if (expr instanceof Keyword) {
            ...
            if (token != THIS && token != SUPER) throw new SyntaxError(lex);
        } else if (expr instanceof Symbol) ; else if (expr instanceof Expr) {
            int op = ((Expr) expr).getOperator();
            if (op != '.' && op != MEMBER) throw new SyntaxError(lex);
        }
    ...
    private ASTree parsePrimaryExpr(SymbolTable tbl) throws CompileError {
        ...
        switch(t = lex.get()) {
            ...
            case Identifier:
                ...
                if (decl == null) return new Member(name); else return new Variable(name, decl);
            case StringL:
            ...
            case '(':
                expr = parseExpression(tbl);
                if (lex.get() == ')') return expr; else throw new CompileError(") is missing", lex);
            default:
                if (isBuiltinType(t) || t == VOID) {
                    int dim = parseArrayDimension();
                    if (lex.get() == '.' && lex.get() == CLASS) return parseDotClass(t, dim);
                }
    ...
    private NewExpr parseNew(SymbolTable tbl) throws CompileError {
        ...
        if (isBuiltinType(t)) {
            ...
            if (lex.lookAhead() == '{') init = parseArrayInitializer(tbl);
    ...
    private ASTList parseArraySize(SymbolTable tbl) throws CompileError {
        ASTList list = null;
        while (lex.lookAhead() == '[') list = ASTList.append(list, parseArrayIndex(tbl));
        return list;
    }
