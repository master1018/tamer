 ...
 */
public class pointp extends GLSkeleton<GLJPanel> implements GLEventListener, KeyListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        for (int i = 0; i < 250; i++) {
            ...
            gl.glVertex3f((float) ((Math.random() * (max - min)) - max), (float) ((Math.random() * (max - min)) - max), (float) (((Math.random() * (max - min)) - max)));
        }
        gl.glEnd();
        gl.glEndList();
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (key != null) switch(key.getKeyChar()) {
            case 'b':
                ...
                break;
            case 'c':
                gl.glPointParameterfv(GL2.GL_ATTENUATION_EXT, constant, 0);
                break;
            case 'f':
                gl.glMatrixMode(GL2.GL_MODELVIEW);
                gl.glTranslatef(0.0f, 0.0f, 0.5f);
            ...
            case 'l':
                ...
                break;
            case 'q':
                gl.glPointParameterfv(GL2.GL_ATTENUATION_EXT, quadratic, 0);
                break;
            case '+':
                if (psize < (pmax[0] + 1.0f)) psize = psize + 1.0f;
                gl.glPointSize(psize);
                break;
            case '-':
                if (psize >= 2.0f) psize = psize - 1.0f;
                gl.glPointSize(psize);
                break;
        }
        gl.glCallList(1);
