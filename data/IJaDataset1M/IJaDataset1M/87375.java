...
public class TerritoryStatus {
    ...
    public void NotifyTaskPerform(final CM_Task task) {
        if (task.GetTaskType() == CM_TaskType.Land_LandGrab) WasAttacked_LandGrab = true; else if (task.GetTaskType() == CM_TaskType.Land_Attack_Stabilize) WasAttacked_Stabalize = true; else if (task.GetTaskType() == CM_TaskType.Land_Attack_Offensive) WasAttacked_Offensive = true; else if (task.GetTaskType() == CM_TaskType.Land_Attack_Trade) WasAttacked_Trade = true;
    }

    public void NotifyTaskPerform(final NCM_Task task) {
        if (task.GetTaskType() == NCM_TaskType.Land_Reinforce_Block) WasReinforced_Block = true; else if (task.GetTaskType() == NCM_TaskType.Land_Reinforce_FrontLine) WasReinforced_Frontline = true; else if (task.GetTaskType() == NCM_TaskType.Land_Reinforce_Stabilize) WasReinforced_Stabalize = true;
    }
}
