...
public class ModuleReaderTest {
    ...
    public void testPlain() throws IOException, SAXException {
        ...
        Assert.assertTrue("Could not retrieve correct number of functions from plain module file", funcs.size() == 1);
        Assert.assertTrue("Could not retrieve function 'names' from plain module file", funcs.containsKey("names"));
        Assert.assertTrue("Object is not instance of FunctionIF.", funcs.get("names") instanceof FunctionIF);
    }
    ...
    public void testEncrypted() throws IOException, SAXException {
        ...
        Map funcs = modReader.read(StreamUtils.getInputStream(inpFile));
        Assert.assertTrue("Could not retrieve correct number of functions from encrypted module file", funcs.size() == 1);
        Assert.assertTrue("Could not retrieve function 'names' from encrypted module file", funcs.containsKey("names"));
        Assert.assertTrue("Object is not instance of FunctionIF.", funcs.get("names") instanceof FunctionIF);
    }
}
