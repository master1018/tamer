...
public class SequenceValue extends CollectionValue {
    ...
    public SequenceValue(Type elemType, Collection values) {
        ...
        while (it.hasNext()) add((Value) it.next());
    }
    ...
    public SequenceValue(Type elemType, int[] ranges) {
        ...
        while (i < ranges.length) {
            ...
            for (int j = lower; j <= upper; j++) add(new IntegerValue(j));
            i += 2;
        }
    }
    ...
    public boolean includesAll(CollectionValue v) {
        ...
        while (it.hasNext()) {
            ...
            if (!fElements.contains(elem)) return false;
        }
        return true;
    }
    ...
    public int count(Value v) {
        int res = 0;
        Iterator it = fElements.iterator();
        while (it.hasNext()) if (v.equals(it.next())) res++;
        return res;
    }
    ...
    public SequenceValue subSequence(int lower, int upper) {
        SequenceValue res = new SequenceValue(elemType());
        for (int i = lower; i < upper; i++) res.add((Value) fElements.get(i));
        return res;
    }
    ...
    public SequenceValue flatten() {
        if (!elemType().isCollection()) throw new RuntimeException("Sequence `" + type() + "' cannot be flattened");
    ...
    public boolean equals(Object obj) {
        ...
        if (obj.getClass().equals(getClass())) {
            ...
            return seq2.type().isSubtypeOf(this.type()) && fElements.equals(seq2.fElements);
        }
        return false;
    }
