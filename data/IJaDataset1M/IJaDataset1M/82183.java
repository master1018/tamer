...
class DuplicateAttributeDetector {
    ...
    void addAttribute(NameClass nc) throws RestrictionViolationException {
        ...
        for (Alternative a = alternatives; a != null; a = a.parent) {
            for (int i = a.endIndex; i < lim; i++) checkAttributeOverlap(nc, (NameClass) nameClasses.elementAt(i));
            lim = a.startIndex;
        }
        for (int i = 0; i < lim; i++) checkAttributeOverlap(nc, (NameClass) nameClasses.elementAt(i));
