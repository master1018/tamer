...
public class TimeService extends InterfaceBWebsideController {
    ...
    public synchronized void finish(WorkItemRecord itemRecord, String _sessionHandle) {
        try {
            TaskInformation taskinfo = getTaskInformation(itemRecord.getSpecificationID(), itemRecord.getTaskID(), _sessionHandle);
            checkInWorkItem(itemRecord.getID(), itemRecord.getWorkItemData(), new Element(taskinfo.getDecompositionID()), _sessionHandle);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
class InternalRunner extends Thread {
    ...
    public void run() {
        ...
        if (!stopping) t.finish(itemRecord, _sessionHandle);
    }
}
