...
public class Reader {
    ...
    public ImagePlus open(String arg, boolean verbose) {
        ...
        if (arg.equals("")) {
            ...
            if (directory != null) {
                ...
                if (directoryHandler != null && directoryHandler.isDirectory()) fc.setCurrentDirectory(directoryHandler);
            }
    ...
    private void printImDirData(LSMFileInfo lsmFi) {
        for (int i = 0; i < lsmFi.imageDirectories.size(); i++) {
            ...
            System.err.println("=============\n");
            ImageDirectory imDir = (ImageDirectory) lsmFi.imageDirectories.get(i);
            System.err.println("ImDir data:\n" + imDir.toString());
            if (imDir.TIF_CZ_LSMINFO != null) System.err.println("CZ-Info data:\n" + imDir.TIF_CZ_LSMINFO.toString()); else System.err.println("CZ-Info data is null (not set)");
            System.err.println("=================================================");
        }
    }
    ...
    public void updateMetadata(ImagePlus imp) {
        ...
        if (imp.getOriginalFileInfo() instanceof LSMFileInfo) {
            ...
            try {
                file = new RandomAccessFile(new File(lsm.directory + System.getProperty("file.separator") + lsm.fileName), "r");
    ...
    private ChannelWavelengthRange getLambdaStamps(RandomAccessStream stream, long position) {
        ...
        try {
            ...
            for (int i = 0; i < channelWavelength.Channels; i++) {
                channelWavelength.StartWavelength[i] = ReaderToolkit.swap(stream.readDouble());
                channelWavelength.EndWavelength[i] = ReaderToolkit.swap(stream.readDouble());
    ...
    private EventList getEventList(RandomAccessStream stream, long position, double firstTimeStamp) {
        ...
        try {
            ...
            if (eventList.NumberEvents > 0) for (int i = 0; i < eventList.NumberEvents; i++) {
                ...
                eventList.events[i].SizeEventListEntry = ReaderToolkit.swap(stream.readInt());
                ...
                EventDescription += IJ.d2s(eventList.events[i].Time - firstTimeStamp) + "\t" + EventType + "\t";
                EventNote = ReaderToolkit.readNULLASCII2(stream, eventList.events[i].SizeEventListEntry - 16);
            ...
            while (tag.entry != 0x0FFFFFFFF) {
                ...
                if (Recording.isRecording(tag.entry)) {
                    ...
                    while (tag.entry != 0x0FFFFFFFF) {
                        ...
                        recording.records = getRecords(stream, tag, Recording.data, recording.records);
                    }
        ...
        while (tag.entry != 0x0FFFFFFFF) {
            ...
            if (Laser.isLaser(tag.entry)) {
                ...
                while (tag.entry != 0x0FFFFFFFF) {
                    ...
                    if (tag.entry != 0x0FFFFFFFF) l.records = getRecords(stream, tag, l.data, l.records);
                }
                v.add(l);
                tag.entry = 0;
            }
    ...
    private IlluminationChannel[] getIlluminationBlock(RandomAccessStream stream) {
        ...
        return (IlluminationChannel[]) vdc.toArray(new IlluminationChannel[vdc.size()]);
    }
        ...
        while (tag.entry != 0x0FFFFFFFF) {
            ...
            if (Track.isTrack(tag.entry)) {
                ...
                while (tag.entry != 0x0FFFFFFFF) {
                    ...
                    if (tag.entry != 0x0FFFFFFFF) t.records = getRecords(stream, tag, t.data, t.records);
                }
                v.add(t);
                tag.entry = 0;
            }
    ...
    private DetectionChannel[] getDetectionChannelBlock(RandomAccessStream stream) {
        ScanInfoTag tag = new ScanInfoTag();
    ...
    private LinkedHashMap<String, Object> getRecords(RandomAccessStream stream, ScanInfoTag tag, Object[][] data, LinkedHashMap<String, Object> lhm) {
        try {
            String value = "";
            long l = 0;
            double d = 0;
            long position = stream.getFilePointer();
            if (tag.type == 2) value = ReaderToolkit.readSizedNULLASCII(stream, tag.size);
            ...
            for (int i = 0; i < data.length; i++) {
                if (((Long) data[i][0]).longValue() == tag.entry) {
                    if (tag.type == 2) {
                        if (IJ.debugMode) IJ.log("Tag recognized: [" + Long.toHexString(tag.entry) + "] -->" + (String) data[i][2]);
            ...
            if (tag.type == 4) lhm.put("<UNKNOWN@" + (position - 12) + ">", new Long(l));
            if (tag.type == 5) lhm.put("<UNKNOWN@" + (position - 12) + ">", new Double(d));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return lhm;
