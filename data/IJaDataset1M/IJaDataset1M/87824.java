...
public class JMEDesktop extends Quad {
    ...
    public JMEDesktop(String name) {
        ...
        awtWindow = new Frame() {
            ...
            public boolean isVisible() {
                if (awtWindow.isFocusableWindow() && new Throwable().getStackTrace()[1].getMethodName().startsWith("requestFocus")) {
                    return false;
                }
    ...
    protected void setupDefaultInputBindings() {
        getInputHandler().addAction(getButtonUpdateAction(InputHandler.BUTTON_ALL), InputHandler.DEVICE_MOUSE, InputHandler.BUTTON_ALL, InputHandler.AXIS_NONE, false);
    ...
    public void onKey(final char character, final int keyCode, final boolean pressed) {
        try {
            ...
            logger.logp(Level.SEVERE, this.getClass().toString(), "onKey(character, keyCode, pressed)", "Exception", e);
    ...
    public void onButton(final int swingButton, final boolean pressed, final int x, final int y) {
        ...
        try {
            ...
            logger.logp(Level.SEVERE, this.getClass().toString(), "onButton(swingButton, pressed, x, y)", "Exception", e);
        } catch (InvocationTargetException e) {
            logger.logp(Level.SEVERE, this.getClass().toString(), "onButton(swingButton, pressed, x, y)", "Exception", e);
        }
    }
    ...
    public void onWheel(final int wheelDelta, final int x, final int y) {
        ...
        try {
            ...
            logger.logp(Level.SEVERE, this.getClass().toString(), "onWheel(wheelDelta, x, y)", "Exception", e);
        } catch (InvocationTargetException e) {
            logger.logp(Level.SEVERE, this.getClass().toString(), "onWheel(wheelDelta, x, y)", "Exception", e);
        }
    }
    ...
    public void onMove(int xDelta, int yDelta, final int newX, final int newY) {
        ...
        try {
            ...
            logger.logp(Level.SEVERE, this.getClass().toString(), "onMove(xDelta, yDelta, newX, newY)", "Exception", e);
        } catch (InvocationTargetException e) {
            logger.logp(Level.SEVERE, this.getClass().toString(), "onMove(xDelta, yDelta, newX, newY)", "Exception", e);
        }
    }
    ...
    private void sendAWTKeyEvent(int keyCode, boolean pressed, char character) {
        ...
        if (keyCode != 0) {
            ...
            if (focusOwner != null) {
                if (pressed) {
                    KeyEvent event = new KeyEvent(focusOwner, KeyEvent.KEY_PRESSED, System.currentTimeMillis(), getCurrentModifiers(-1), keyCode, character);
                    ...
                    if (character != KeyEvent.CHAR_UNDEFINED) {
                        dispatchEvent(focusOwner, new KeyEvent(focusOwner, KeyEvent.KEY_TYPED, System.currentTimeMillis(), getCurrentModifiers(-1), 0, character));
                    }
                }
                if (!pressed) {
                    anInt.value = keyCode;
                    ...
                    dispatchEvent(focusOwner, new KeyEvent(focusOwner, KeyEvent.KEY_RELEASED, System.currentTimeMillis(), getCurrentModifiers(-1), keyCode, character));
                }
            }
        }
    }
    ...
    private void sendAWTWheelEvent(int wheelDelta, int x, int y) {
        ...
        final MouseWheelEvent event = new MouseWheelEvent(comp, MouseEvent.MOUSE_WHEEL, System.currentTimeMillis(), getCurrentModifiers(-1), pos.x, pos.y, 1, false, MouseWheelEvent.WHEEL_UNIT_SCROLL, Math.abs(wheelDelta), wheelDelta > 0 ? -1 : 1);
    ...
    private void sendAWTMouseEvent(int x, int y, boolean pressed, int swingButton) {
        ...
        if (comp != null) {
            ...
            final Point pos = convertPoint(desktop, x, y, comp);
            final MouseEvent event = new MouseEvent(comp, eventType, time, getCurrentModifiers(swingButton), pos.x, pos.y, clickCount, swingButton == MouseEvent.BUTTON2 && pressed, swingButton >= 0 ? swingButton : 0);
            dispatchEvent(comp, event);
            if (clicked) {
                comp = componentAt(x, y, desktop, true);
                final Point clickedPos = convertPoint(desktop, x, y, comp);
                final MouseEvent clickedEvent = new MouseEvent(comp, MouseEvent.MOUSE_CLICKED, time, getCurrentModifiers(swingButton), clickedPos.x, clickedPos.y, clickCount, false, swingButton);
        ...
        if (comp == null || comp.isFocusable()) {
            for (Component p = comp; p != null; p = p.getParent()) {
                if (p instanceof JInternalFrame) {
                    try {
                        ...
                        logger.logp(Level.SEVERE, this.getClass().toString(), "setFocusOwner(Component comp)", "Exception", e);
                    }
                }
            }
            ...
            if (oldFocusOwner != comp) {
                if (oldFocusOwner != null) {
                    dispatchEvent(oldFocusOwner, new FocusEvent(oldFocusOwner, FocusEvent.FOCUS_LOST, false, comp));
                }
                KeyboardFocusManager.getCurrentKeyboardFocusManager().clearGlobalFocusOwner();
                if (comp != null) {
                    dispatchEvent(comp, new FocusEvent(comp, FocusEvent.FOCUS_GAINED, false, oldFocusOwner));
                }
            }
            awtWindow.setFocusableWindowState(false);
        }
            ...
            synchronized (parent.getTreeLock()) {
                if (parent instanceof Container) {
                    ...
                    for (int i = 0; i < ncomponents; i++) {
                        ...
                        if (comp != null && comp.isVisible() && (dragAndDropSupport == null || !dragAndDropSupport.isDragPanel(comp)) && comp.contains(x - comp.getX(), y - comp.getY())) {
                            child = comp;
                            break;
                        }
                    }
    ...
    private void sendEnteredEvent(Component comp, Component lastComponent, int buttonMask, Point pos) {
        if (comp != null && comp != lastComponent) {
            ...
            final MouseEvent event = new MouseEvent(comp, MouseEvent.MOUSE_ENTERED, System.currentTimeMillis(), buttonMask, pos.x, pos.y, 0, false, 0);
            dispatchEvent(comp, event);
        }
    }

    private void sendExitedEvent(Component lastComponent, int buttonMask, Point pos) {
        final MouseEvent event = new MouseEvent(lastComponent, MouseEvent.MOUSE_EXITED, System.currentTimeMillis(), buttonMask, pos.x, pos.y, 1, false, 0);
            ...
            synchronized (paintLockRunnable) {
                ...
                if (wait) {
                    try {
                        ...
                        logger.logp(Level.SEVERE, this.getClass().toString(), "run()", "Exception", e);
                    }
                }
            }
        }
    ...
    private static class MyPopupFactory extends PopupFactory {
        ...
        public Popup getPopup(Component owner, Component contents, int x, int y) throws IllegalArgumentException {
            while (!(owner instanceof JDesktopPane)) {
                ...
                if (owner == null) {
                    logger.warning("jME Popup creation failed, default popup created - desktop not found in component hierarchy of " + owner);
    ...
    private class ButtonAction extends InputAction {
        ...
        public void performAction(InputActionEvent evt) {
            onButton(swingButtonIndex != InputHandler.BUTTON_ALL ? swingButtonIndex : getSwingButtonIndex(evt.getTriggerIndex()), evt.getTriggerPressed(), lastXin, lastYin);
        }
    }
    ...
    private class XUpdateAction extends InputAction {
        ...
        public void performAction(InputActionEvent evt) {
            ...
            onMove((int) (screenWidth * evt.getTriggerDelta() * getSpeed()), 0, (int) (screenWidth * evt.getTriggerPosition() * getSpeed()), lastYin);
        }
    }
    ...
    private class YUpdateAction extends InputAction {
        ...
        public void performAction(InputActionEvent evt) {
            ...
            onMove(0, (int) (screenHeight * evt.getTriggerDelta() * getSpeed()), lastXin, (int) (screenHeight * evt.getTriggerPosition() * getSpeed()));
        }
    }
