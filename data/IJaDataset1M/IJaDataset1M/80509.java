...
public class ResultViewerGui implements Gui {
        ...
        plotButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (validFile(currentFileTextField.getText())) {
                    if (simulator != null && simulator.getExperiment() != null) simulator.getExperiment().endExperiment();
                    try {
            ...
            if (f.exists()) {
                ...
                if (directories != null) {
                    for (String dir : directories) {
                        ...
                        if (!dirResult.isEmpty()) result += dirResult + "###";
                    }
                }
                return result;
            }
    ...
    private void startButton() {
        simulationState = RUN;
        if (simulator != null && simulator.getExperiment() != null && simulator.getExperiment().hasEnded()) loadCurrentFile();
    }

    private void pauseButton() {
        if (simulationState == RUN) simulationState = PAUSED; else simulationState = RUN;
    }
    ...
    @Override
    public synchronized void run(Simulator simulator, Renderer rendererTo, Experiment experiment, EvaluationFunction evaluationFunction, int maxNumberOfSteps) {
        if (graphViz == null) {
        ...
        if (showNeuralNetwork) graphViz.show();
        ...
        try {
            ...
            while (!experiment.hasEnded() && currentStep < maxNumberOfSteps) {
                ...
                if (sleepBetweenControlSteps > 0) Thread.sleep(sleepBetweenControlSteps);
                long startTime = System.currentTimeMillis();
                if (simulationState == RUN) {
                    ...
                    currentStep++;
                    updatePlaySlider(currentStep, maxNumberOfSteps);
                    if (showNeuralNetwork) graphViz.changeNeuralNetwork(((NeuralNetworkController) simulator.getEnvironment().getRobots().get(0).getEvolvingController()).getNeuralNetwork());
                }
            }
            ...
            if (rendererComponent != null) rendererComponent.removeKeyListener(experiment.getEnvironment());
        }
    }
    ...
    public class FileTree extends JPanel {
            ...
            tree.addTreeSelectionListener(new TreeSelectionListener() {
                ...
                public void valueChanged(TreeSelectionEvent e) {
                    ...
                    for (int i = 0; i < tp.getPathCount(); i++) {
                        ...
                        if (i != tp.getPathCount() - 1) filename += "/";
                    }
                    System.out.println("clicked " + filename);
                    File f = new File(filename);
                    if (f.exists()) {
                        currentFilename = filename;
                        if (!currentFileTextField.getText().equals(filename)) currentFileTextField.setText(filename);
                    }
                }
            });
            tree.addMouseListener(new MouseListener() {
                ...
                public void mousePressed(MouseEvent arg0) {
                    if (arg0.getClickCount() == 2) {
                        ...
                        if (f.isDirectory()) changeDirectory(f.getAbsolutePath()); else loadCurrentFile();
                    }
                }
        ...
        public void changeDirectory(String dir) {
            ...
            try {
                ...
                System.out.println("changing to " + dir);
        ...
        private DefaultMutableTreeNode addNodes(DefaultMutableTreeNode curTop, File dir) {
            ...
            for (int i = 0; i < ol.size(); i++) {
                ...
                if (!f.getName().startsWith(".")) {
                    ...
                    if (f.isDirectory()) DefaultMutableTreeNode.add(new DefaultMutableTreeNode());
