...
public class NXTCommand implements NXTProtocol {
    ...
    public FileInfo findFirst(String wildCard) {
        ...
        if (reply[2] == 0) {
            ...
            if (reply.length > 3) {
                ...
                int lastPos = name.toString().indexOf(".") + 4;
    ...
    public int getBatteryLevel() {
        ...
        nxtComm.sendData(request);
        byte[] reply = nxtComm.readData();
        if (reply[1] != GET_BATTERY_LEVEL) System.out.println("Weird data reply received.");
        if (reply[2] != 0) System.out.println("NXT reports the check battery command did not work.");
    ...
    public byte[] LSGetStatus(byte port) {
        ...
        nxtComm.sendData(request);
        byte[] reply = nxtComm.readData();
        if (reply[2] == ErrorMessages.COMMUNICATION_BUS_ERROR) System.out.println("NXTCommand.LSGetStatus() error: Communications Bus Error"); else if (reply[2] == ErrorMessages.PENDING_COMMUNICATION_TRANSACTION_IN_PROGRESS) System.out.println("NXTCommand.LSGetStatus() error: Pending communication transaction in progress"); else if (reply[2] == ErrorMessages.SPECIFIED_CHANNEL_CONNECTION_NOT_CONFIGURED_OR_BUSY) System.out.println("NXTCommand.LSGetStatus() error: Specified channel connection not configured or busy"); else if (reply[2] != 0) System.out.println("NXTCommand.LSGetStatus() Error Number " + reply[2]);
    ...
    public static void open(NXTInfo nxt) throws RuntimeException {
        singleton.nxtComm.setAddress(nxt.btDeviceAddress);
        try {
            singleton.nxtComm.open();
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage());
    ...
    private static void connectFirst() {
        NXTInfo[] nxts = getDevices();
        String address = null;
        if (nxts[0] != null) {
        ...
        singleton.nxtComm.setAddress(address);
    }

    public static NXTInfo[] getDevices() {
        System.out.println("Searching... (this may take up to a minute)");
        return singleton.nxtComm.search(null);
    }

    public static NXTInfo[] getKnownDevices(int kind) {
        return singleton.nxtComm.getDevices(kind);
    }
    ...
    public static void close() {
        if (!singleton.nxtComm.isOpen()) {
            System.out.println("NXTCommand::close - no connection");
            return;
        }
