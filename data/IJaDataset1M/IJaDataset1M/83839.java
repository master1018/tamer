...
public class Win32IDETool extends IDETool {
    ...
    public int parseOneArgument(String args[], int i) throws ArrayIndexOutOfBoundsException {
    ...
    public void execute(String args[]) throws Exception {
        ...
        VC6VMProject loopgenProject = new VC6VMProject(this, "loopgen", "i386", VC6VMProject.LOOPGEN);
        ...
        if (isRomizing()) {
            romgenProject = new VC6VMProject(this, "romgen", "i386", VC6VMProject.ROMGEN);
        ...
        VC6VMProject targetProject = new VC6VMProject(this, "cldc_vm", "i386", VC6VMProject.TARGET);
    ...
    Properties queryUserSettings() throws Exception {
        ...
        String preverify = workspace + "/build/share/bin/win32_i386/" + "preverify.exe";
        ...
        dialog.getUserInput(props, getBuildOptions(), getGeneratorOptions(), getPaths());
        try {
...
abstract class IDEOutputFile {
    ...
    public void openOutputFile(String fileName) throws Exception {
...
class VC6WorkSpace extends IDEOutputFile {
    ...
    void writeHeader() {
        putln("Microsoft Developer Studio Workspace File, " + "Format Version 6.00");
...
abstract class VC6Project extends IDEProject {
    ...
    public abstract String getTargetTypeString();

    public abstract Vector getAvailableConfigs();
    ...
    public void writeHeader() {
        putln("# Microsoft Developer Studio Project File - " + "Name=\"" + getName() + "\" - Package Owner=<4>");
        putln("# Microsoft Developer Studio Generated Build File, " + "Format Version 6.00");
        putln("# ** DO NOT EDIT **");
        putln();
    ...
    void writeNMakeMessage() {
        nmakemsg("This is not a valid makefile. " + "To build this project using NMAKE,");
        ...
        nmakemsg("by defining the macro CFG on the command line. " + "For example:");
        nmakemsg("");
        nmakemsg("NMAKE /f \"" + getName() + ".mak\" CFG=\"" + getDefaultConfigName() + "\"");
        nmakemsg("");
...
abstract class VC6NMakeProject extends VC6Project {
    ...
    public abstract NMakefile getNMakefile(IDETool tool, String fileName);
    ...
    void writeConfigs() {
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            putln("# PROP BASE Cmd_Line \"NMAKE /nologo /f \"" + getName() + ".mak\"\"");
            ...
            putln("# PROP Cmd_Line \"NMAKE /nologo /f \"" + getName() + ".mak\"\"");
    ...
    void writeTarget() {
        ...
        putln();
        putln("# PROP Default_Filter " + "\"ico;cur;bmp;dlg;rc2;rct;bin;rgs;gif;jpg;jpeg;jpe\"");
        putln("# End Group");
        putln("# End Target");
        putln("# End Project");
    }
...
class VC6JavaAPINMakefile extends NMakefile {
    ...
    void writeNativesTable() {
        ...
        puttabln("@echo Creating $@");
        puttabln("@\"$(JAVA)\" -cp jcc.jar JavaCodeCompact " + "-writer CLDC_HI_Natives -o $@ classes.zip");
        putln();
    }
...
class VC6ROMImageNMakefile extends NMakefile {
    ...
    void writeROMImageRule() {
        ...
        String romconfig = workspace + File.separator + "src\\vm\\cldcx_rom.cfg";
        romconfig = tool().getAbsolutePath(romconfig);
        String rompath = workspace + File.separator + "src\\vm";
...
class VC6LoopNMakefile extends NMakefile {
    ...
    void writeLoopRule() {
        ...
        puttabln("$(LOOPGEN) -generate +GenerateDebugAssembly " + "-outputdir ..\\cldc_vm\\Debug");
        puttabln("$(LOOPGEN) +GenerateOopMaps " + "-outputdir ..\\cldc_vm\\Debug");
        puttabln("$(LOOPGEN) -generateoptimized " + "-outputdir ..\\cldc_vm\\Release");
        puttabln("$(LOOPGEN) +GenerateOopMaps " + "-outputdir ..\\cldc_vm\\Release");
        puttabln("$(LOOPGEN) -generateoptimized " + "-outputdir ..\\cldc_vm\\Product");
        puttabln("$(LOOPGEN) +GenerateOopMaps " + "-outputdir ..\\cldc_vm\\Product");
        if (win32Tool().isRomizing()) {
            if (win32Tool().isOptionEnabled(win32Tool().DEBUG_ROMGEN)) {
                puttabln("$(LOOPGEN) -generate +GenerateDebugAssembly " + "-outputdir ..\\romgen\\Debug");
                puttabln("$(LOOPGEN) +GenerateOopMaps " + "-outputdir ..\\romgen\\Debug");
            } else {
                puttabln("$(LOOPGEN) -generateoptimized " + "-outputdir ..\\romgen\\Release");
                puttabln("$(LOOPGEN) +GenerateOopMaps " + "-outputdir ..\\romgen\\Release");
            }
        }
        putln();
    }
    ...
    void writeClean() {
        ...
        if (win32Tool().isRomizing()) {
            if (win32Tool().isOptionEnabled(win32Tool().DEBUG_ROMGEN)) {
                puttabln("-@del /Q ..\\romgen\\Debug\\Interpreter_i386.asm" + junk);
                puttabln("-@del /Q ..\\romgen\\Debug\\OopMaps.cpp" + junk);
            } else {
                puttabln("-@del /Q ..\\romgen\\Release\\Interpreter_i386.asm" + junk);
                puttabln("-@del /Q ..\\romgen\\Release\\OopMaps.cpp" + junk);
            }
        }
    }
}
...
class VMSourceHandler {
    ...
    public VMSourceHandler(IDETool tool, IDEProject proj, String arch, String os) {
        this.tool = tool;
        this.proj = proj;
        this.arch = arch;
        this.os = os;
...
class VC6VMProject extends VC6Project {
    ...
    public static final int LOOPGEN = 1;

    public static final int ROMGEN = 2;
    ...
    void openDatabase() throws Exception {
        ...
        database.get(tool().getPlatformArg(), tool().getDatabaseArg(), globalProps);
        database.compute();
    }
    ...
    public void writeProjectBlock() {
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            String linkLine = "LINK32 gdi32.lib user32.lib wsock32.lib " + "/nologo /subsystem:console /incremental:no " + "/machine:I386";
            if (outdir.equals("Debug")) {
                linkLine += " /debug";
            }
    ...
    void writeSourceFileRules() throws Exception {
        ...
        putln("# Begin Group \"Source Files\"");
        putln();
        putln("# PROP Default_Filter " + "\"cpp;c;cxx;rc;def;r;odl;hpj;bat;for;f90;asm\"");
    ...
    void writeAsmAndOopMapsFileRules(boolean isAsm) {
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            putln("# Begin Custom Build - Performing Custom Build Step on " + "$(InputName).asm");
            putln("OutDir=.\\" + config.outputDir);
            putln("InputPath=\\\"" + config.outputDir + "\\" + stem + ext + "\"");
            putln("InputName=" + stem);
            putln();
            putln("\"$(OutDir)\\$(InputName).obj\" : $(SOURCE) " + "\"$(INTDIR)\" \"$(OUTDIR)\"");
            if (isAsm) {
                putln("    \"" + mlpath + "\" /nologo /c /coff /Zi /FR " + "/Fo\"$(OutDir)\\$(InputName).obj\" " + srcName);
            } else {
                put("    \"cl.exe\" /c /I . ");
                putIncludePaths();
    ...
    void writeFooter() {
        putln("# Begin Group \"Resource Files\"");
        putln();
        putln("# PROP Default_Filter " + "\"ico;cur;bmp;dlg;rc2;rct;bin;cnt;rtf;gif;jpg;jpeg;jpe\"");
        putln("# End Group");
        putln("# End Target");
        putln("# End Project");
    }
...
class Win32IDESettingsDialog extends JFrame implements ActionListener {
    ...
    public void getUserInput(Properties props, Vector buildOptions, Vector generatorOptions, Vector paths) throws Exception {
        this.props = props;
    ...
    private void addOne(JPanel panel, GridBagLayout gridbag, GridBagConstraints c, JComponent comp, boolean indent) {
    ...
    private void addTwo(JPanel panel, GridBagLayout gridbag, GridBagConstraints c, JComponent one, JComponent two, boolean indent) {
        c.fill = GridBagConstraints.BOTH;
        c.weightx = 1.0;
        c.ipadx = 2;
        c.gridwidth = 1;
    ...
    class MyScrollablePanel extends JPanel implements Scrollable {
        ...
        public Dimension getPreferredScrollableViewportSize() {
            ...
            return new Dimension((int) (d1.getWidth()), (int) (d2.getHeight()));
        }
    ...
    private void createUI() {
        ...
        String items3[] = { "true", "false", "default" };
        String items2[] = { "true", "false" };
        ...
        for (Enumeration e = generatorOptions.elements(); e.hasMoreElements(); ) {
            String name = (String) e.nextElement();
        ...
        JScrollPane leftPane = new JScrollPane(leftPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
        JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftPane, rightOuterPanel);
