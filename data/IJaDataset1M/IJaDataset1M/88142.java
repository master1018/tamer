...
public class Transactions {
    ...
    public int nextNum(boolean readonly) {
        int n;
        do n = nextNum.incrementAndGet(); while ((n % 2) != (readonly ? 0 : 1));
        return n;
    }
    ...
    public synchronized void addFinal(Transaction tran) {
	 ...
	 */
    public synchronized void remove(Transaction tran) {
        verify(trans.remove(tran));
        if (tran.isReadWrite() && tran.isCommitted()) locks.commit(tran); else locks.remove(tran);
        finalization();
    }
    ...
    private void finalization() {
        ...
        while (!finals.isEmpty() && finals.peek().asof() < oldest) locks.remove(finals.poll());
        assert !trans.isEmpty() || finals.isEmpty();
        assert !trans.isEmpty() || locks.isEmpty();
    }

    private long oldestReadWriteTran() {
        for (Transaction t : trans) if (t.isReadWrite()) return t.asof();
        return FUTURE;
    }
    ...
    private void limitFinalsSize() {
        Transaction t = null;
        synchronized (this) {
            if (finals.size() <= MAX_FINALS_SIZE) return;
            for (Transaction tran : trans) if (tran.isReadWrite()) {
                t = tran;
                break;
            }
        }
    ...
    synchronized void addShadows(Transaction tcompleting, Long offset) {
        ByteBuf copy = null;
        for (Transaction t : trans) if (t != tcompleting) copy = t.shadow(db.dest, offset, copy);
    }
    ...
    public synchronized List<Integer> tranlist() {
        ...
        for (Transaction t : trans) list.add(t.num);
        return list;
    }
