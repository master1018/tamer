package com.informa.unitils.scriptsources;

import static com.informa.utils.IOUtils.asResource;
import static com.informa.utils.IOUtils.textFrom;
 ...
 * @author Tim Ducheyne
 */
public class ClasspathScriptSource extends BaseDatabaseTask implements ScriptSource {
    ...
    protected void assertNoDuplicateVersionIndexes(List<Script> scripts) {
        for (int i = 0; i < scripts.size() - 1; i++) {
            ...
            if (script1.getVersion().getIndexes().equals(script2.getVersion().getIndexes())) {
                throw new UnitilsException("Found 2 database scripts with the same version index: " + script1.getName() + " and " + script2.getName() + " both have version index " + script1.getVersion().getIndexesString());
            }
        }
    }
    ...
    protected String preprocess(String scriptContent) {
    ...
    private List<File> getFiles(List<String> locations, String propertyName) {
        ...
        for (String fileLocation : locations) {
            File file;
            try {
                file = asResource(fileLocation).getFile();
