...
public class AddressTestBasic {
    ...
    @Test
    public void testEQ() {
        Address[] lefties = new Address[] { zero, one, large, veryLarge, largest };
        Address[] righties = new Address[] { zero(), one(), large(), veryLarge(), largest() };
        for (int i = 0; i < lefties.length; i++) {
            for (int j = 0; j < righties.length; j++) {
                if (i == j) assertTrue(lefties[i].EQ(righties[j])); else assertFalse(lefties[i].EQ(righties[j]));
            }
        }
    }
    ...
    public void testNE() {
        ...
        for (int i = 0; i < lefties.length; i++) {
            for (int j = 0; j < righties.length; j++) {
                if (i == j) assertFalse(lefties[i].NE(righties[j])); else assertTrue(lefties[i].NE(righties[j]));
            }
        }
    }
