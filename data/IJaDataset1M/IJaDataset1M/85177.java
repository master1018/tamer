...
public class CompressionStream {
    ...
    private CompressionStream() {
        ...
        mcBounds[0] = new Point3d(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
        mcBounds[1] = new Point3d(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY);
        qcBounds[0] = new Point3i(Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE);
        qcBounds[1] = new Point3i(Integer.MIN_VALUE, Integer.MIN_VALUE, Integer.MIN_VALUE);
    ...
    private int getVertexComponents(int vertexFormat) {
        int components = 0;
        vertexColors = vertexColor3 = vertexColor4 = vertexNormals = vertexTextures = vertexTexture2 = vertexTexture3 = vertexTexture4 = false;
        ...
        if (vertexTextures) throw new UnsupportedOperationException("\ncompression of texture coordinates is not supported");
        return components;
    }
    ...
    void quantize(HuffmanTable huffmanTable) {
        ...
        if (xRange > yRange) positionRangeMaximum = xRange; else positionRangeMaximum = yRange;
        if (zRange > positionRangeMaximum) positionRangeMaximum = zRange;
        ...
        while (i.hasNext()) {
            ...
            if (o instanceof CompressionStreamElement) {
                ...
                if (o instanceof CompressionStreamColor) lastElementColor = true; else if (o instanceof CompressionStreamNormal) lastElementNormal = true;
            }
        }
    ...
    void outputCommands(HuffmanTable huffmanTable, CommandStream outputBuffer) {
        ...
        while (i.hasNext()) {
            Object o = i.next();
            if (o instanceof CompressionStreamElement) ((CompressionStreamElement) o).outputCommand(huffmanTable, outputBuffer);
        }
        outputBuffer.end();
    }
    ...
    private class PositionQuant extends CompressionStreamElement {
        ...
        void quantize(CompressionStream s, HuffmanTable t) {
            ...
            scale = (2.0 / positionRangeMaximum) * (((double) ((1 << (value - 1)) - 1)) / ((double) (1 << (value - 1))));
        }
    ...
    private class MeshReference extends CompressionStreamElement {
        ...
        void quantize(CompressionStream s, HuffmanTable t) {
            ...
            if (v.color != null && !lastElementColor && !(lastElementNormal && lastLastElementColor)) {
                lastColor[0] = v.color.rAbsolute;
                lastColor[1] = v.color.gAbsolute;
                lastColor[2] = v.color.bAbsolute;
                lastColor[3] = v.color.aAbsolute;
            }
            if (v.normal != null && !lastElementNormal && !(lastElementColor && lastLastElementNormal)) {
        ...
        public String toString() {
            return "meshReference: stripFlag " + stripFlag + " meshIndex " + meshIndex;
        }
    }
    ...
    void addVertex(Point3f pos, int stripFlag) {
        stream.add(new CompressionStreamVertex(this, pos, (Vector3f) null, (Color3f) null, stripFlag, NO_MESH_PUSH));
    }
    ...
    void addVertex(Point3f pos, Vector3f norm, int stripFlag) {
        stream.add(new CompressionStreamVertex(this, pos, norm, (Color3f) null, stripFlag, NO_MESH_PUSH));
    }
    ...
    void addVertex(Point3f pos, Color3f color, int stripFlag) {
        stream.add(new CompressionStreamVertex(this, pos, (Vector3f) null, color, stripFlag, NO_MESH_PUSH));
    }
    ...
    void addVertex(Point3f pos, Vector3f norm, Color3f color, int stripFlag) {
        stream.add(new CompressionStreamVertex(this, pos, norm, color, stripFlag, NO_MESH_PUSH));
    }
    ...
    void addVertex(Point3f pos, int stripFlag, int meshFlag) {
        stream.add(new CompressionStreamVertex(this, pos, (Vector3f) null, (Color3f) null, stripFlag, meshFlag));
    }
    ...
    void addVertex(Point3f pos, Vector3f norm, int stripFlag, int meshFlag) {
        stream.add(new CompressionStreamVertex(this, pos, norm, (Color3f) null, stripFlag, meshFlag));
    }
    ...
    void addVertex(Point3f pos, Color3f color, int stripFlag, int meshFlag) {
        stream.add(new CompressionStreamVertex(this, pos, (Vector3f) null, color, stripFlag, meshFlag));
    }
    ...
    void addVertex(Point3f pos, Vector3f norm, Color3f color, int stripFlag, int meshFlag) {
        stream.add(new CompressionStreamVertex(this, pos, norm, color, stripFlag, meshFlag));
    }
    ...
    void addVertex(Point3f pos, Vector3f norm, Object color, int stripFlag, int meshFlag) {
        if (vertexColor3) stream.add(new CompressionStreamVertex(this, pos, norm, (Color3f) color, stripFlag, meshFlag)); else stream.add(new CompressionStreamVertex(this, pos, norm, (Color4f) color, stripFlag, meshFlag));
    }
    ...
    private class ByCopyGeometry implements GeometryAccessor {
        ...
        public void processVertex(int v, int stripFlag) {
            ...
            if ((r == meshBuffer.NOT_FOUND) || (vertexNormals && noMeshNormalSubstitution && (!normals[v].equals(meshBuffer.getNormal(r))))) {
                Vector3f n = vertexNormals ? normals[v] : null;
                Object c = vertexColor3 ? (Object) colors3[v] : vertexColor4 ? (Object) colors4[v] : null;
                addVertex(p, n, c, stripFlag, MESH_PUSH);
                meshBuffer.push(p, c, n);
            } else {
                if (vertexNormals && !noMeshNormalSubstitution && (!normals[v].equals(meshBuffer.getNormal(r)))) addNormal(normals[v]);
                if (vertexColor3 && (!colors3[v].equals(meshBuffer.getColor3(r)))) addColor(colors3[v]); else if (vertexColor4 && (!colors4[v].equals(meshBuffer.getColor4(r)))) addColor(colors4[v]);
    ...
    private void getVertexIndices(int v, IndexArrays ia, VertexIndices vi) {
        ...
        if (vertexNormals) vi.ni = ia.normalIndices[v];
        if (vertexColors) vi.ci = ia.colorIndices[v];
    }
    ...
    private class IndexedByCopyGeometry extends ByCopyGeometry {
        ...
        public void processVertex(int v, int stripFlag) {
            ...
            if ((r == meshBuffer.NOT_FOUND) || (vertexNormals && noMeshNormalSubstitution && (vi.ni != meshBuffer.getNormalIndex(r)))) {
                Point3f p = positions[vi.pi];
                Vector3f n = vertexNormals ? normals[vi.ni] : null;
                Object c = vertexColor3 ? (Object) colors3[vi.ci] : vertexColor4 ? (Object) colors4[vi.ci] : null;
                addVertex(p, n, c, stripFlag, MESH_PUSH);
                meshBuffer.push(vi.pi, vi.ci, vi.ni);
            } else {
                if (vertexNormals && !noMeshNormalSubstitution && vi.ni != meshBuffer.getNormalIndex(r)) addNormal(normals[vi.ni]);
                if (vertexColor3 && vi.ci != meshBuffer.getColorIndex(r)) addColor(colors3[vi.ci]); else if (vertexColor4 && vi.ci != meshBuffer.getColorIndex(r)) addColor(colors4[vi.ci]);
                addMeshReference(stripFlag, r);
            }
        }
    }
    ...
    private void processVertexCopy(VertexCopy vc, int stripFlag) {
        ...
        if ((r == meshBuffer.NOT_FOUND) || (vertexNormals && noMeshNormalSubstitution && (!vc.n.equals(meshBuffer.getNormal(r))))) {
            addVertex(vc.p, vc.n, vc.c, stripFlag, MESH_PUSH);
            meshBuffer.push(vc.p, vc.c, vc.n);
        } else {
            if (vertexNormals && !noMeshNormalSubstitution && (!vc.n.equals(meshBuffer.getNormal(r)))) addNormal(vc.n);
            if (vertexColor3 && (!vc.c3.equals(meshBuffer.getColor3(r)))) addColor(vc.c3); else if (vertexColor4 && (!vc.c4.equals(meshBuffer.getColor4(r)))) addColor(vc.c4);
            addMeshReference(stripFlag, r);
        }
    }
    ...
    private void processIndexedVertexCopy(VertexCopy vc, VertexIndices vi, int stripFlag) {
        ...
        if ((r == meshBuffer.NOT_FOUND) || (vertexNormals && noMeshNormalSubstitution && (vi.ni != meshBuffer.getNormalIndex(r)))) {
            ...
            if (vertexNormals && !noMeshNormalSubstitution && vi.ni != meshBuffer.getNormalIndex(r)) addNormal(vc.n);
            if (vertexColor3 && vi.ci != meshBuffer.getColorIndex(r)) addColor(vc.c3); else if (vertexColor4 && vi.ci != meshBuffer.getColorIndex(r)) addColor(vc.c4);
            addMeshReference(stripFlag, r);
        }
    }
    ...
    private abstract class InterleavedGeometry implements GeometryAccessor {
        ...
        InterleavedGeometry(GeometryArray ga) {
            if (vertexTextures) {
                if (vertexTexture2) tstride = 2; else if (vertexTexture3) tstride = 3; else if (vertexTexture4) tstride = 4;
                tcount = ga.getTexCoordSetCount();
                vstride += tcount * tstride;
            }
            if (vertexColors) {
                coffset = vstride;
                if (vertexColor3) vstride += 3; else vstride += 4;
            }
            if (vertexNormals) {
                noffset = vstride;
                vstride += 3;
    ...
    private class InterleavedGeometryFloat extends InterleavedGeometry {
        ...
        void copyVertex(int pi, int ni, int ci, VertexCopy vc) {
            int voffset;
            voffset = pi * vstride;
            vc.p = new Point3f(vdata[voffset + poffset + 0], vdata[voffset + poffset + 1], vdata[voffset + poffset + 2]);
            if (vertexNormals) {
                voffset = ni * vstride;
                vc.n = new Vector3f(vdata[voffset + noffset + 0], vdata[voffset + noffset + 1], vdata[voffset + noffset + 2]);
            }
            if (vertexColor3) {
                voffset = ci * vstride;
                vc.c3 = new Color3f(vdata[voffset + coffset + 0], vdata[voffset + coffset + 1], vdata[voffset + coffset + 2]);
                vc.c = vc.c3;
            } else if (vertexColor4) {
                voffset = ci * vstride;
                vc.c4 = new Color4f(vdata[voffset + coffset + 0], vdata[voffset + coffset + 1], vdata[voffset + coffset + 2], vdata[voffset + coffset + 3]);
                vc.c = vc.c4;
            }
        }
    }
     ...
     * interleaved geometry arrays.
     */
    private class IndexedInterleavedGeometryFloat extends InterleavedGeometryFloat {
    ...
    private class InterleavedGeometryNIO extends InterleavedGeometry {
        ...
        InterleavedGeometryNIO(GeometryArray ga) {
            ...
            if (BufferWrapper.getBufferType(buffer) == BufferWrapper.TYPE_FLOAT) {
        ...
        void copyVertex(int pi, int ni, int ci, VertexCopy vc) {
            int voffset;
            voffset = pi * vstride;
            vc.p = new Point3f(fbw.get(voffset + poffset + 0), fbw.get(voffset + poffset + 1), fbw.get(voffset + poffset + 2));
            if (vertexNormals) {
                voffset = ni * vstride;
                vc.n = new Vector3f(fbw.get(voffset + noffset + 0), fbw.get(voffset + noffset + 1), fbw.get(voffset + noffset + 2));
            }
            if (vertexColor3) {
                voffset = ci * vstride;
                vc.c3 = new Color3f(fbw.get(voffset + coffset + 0), fbw.get(voffset + coffset + 1), fbw.get(voffset + coffset + 2));
                vc.c = vc.c3;
            } else if (vertexColor4) {
                voffset = ci * vstride;
                vc.c4 = new Color4f(fbw.get(voffset + coffset + 0), fbw.get(voffset + coffset + 1), fbw.get(voffset + coffset + 2), fbw.get(voffset + coffset + 3));
                vc.c = vc.c4;
            }
        }
    }
    ...
    private class ByRefGeometry implements GeometryAccessor {
        ...
        ByRefGeometry(GeometryArray ga) {
            ...
            if (debug && positionsF != null) System.out.println("float positions");
            positionsD = ga.getCoordRefDouble();
            if (debug && positionsD != null) System.out.println("double positions");
            if (positionsF == null && positionsD == null) throw new UnsupportedOperationException("\nby-reference access to Point3{d,f} arrays");
            initialPositionIndex = ga.getInitialCoordIndex();
            if (vertexColors) {
                colorsB = ga.getColorRefByte();
                if (debug && colorsB != null) System.out.println("byte colors");
                colorsF = ga.getColorRefFloat();
                if (debug && colorsF != null) System.out.println("float colors");
                if (colorsB == null && colorsF == null) throw new UnsupportedOperationException("\nby-reference access to Color{3b,3f,4b,4f} arrays");
                initialColorIndex = ga.getInitialColorIndex();
            }
            if (vertexNormals) {
                normals = ga.getNormalRefFloat();
                if (debug && normals != null) System.out.println("float normals");
                if (normals == null) throw new UnsupportedOperationException("\nby-reference access to Normal3f array");
        ...
        void copyVertex(int pi, int ni, int ci, VertexCopy vc) {
            pi *= 3;
            if (positionsF != null) {
                vc.p = new Point3f(positionsF[pi + 0], positionsF[pi + 1], positionsF[pi + 2]);
            } else {
                vc.p = new Point3f((float) positionsD[pi + 0], (float) positionsD[pi + 1], (float) positionsD[pi + 2]);
            }
            ni *= 3;
            if (vertexNormals) {
                vc.n = new Vector3f(normals[ni + 0], normals[ni + 1], normals[ni + 2]);
            }
            if (vertexColor3) {
                ci *= 3;
                if (colorsB != null) {
                    vc.c3 = new Color3f((colorsB[ci + 0] & 0xff) * ByteToFloatScale, (colorsB[ci + 1] & 0xff) * ByteToFloatScale, (colorsB[ci + 2] & 0xff) * ByteToFloatScale);
                } else {
                    vc.c3 = new Color3f(colorsF[ci + 0], colorsF[ci + 1], colorsF[ci + 2]);
                }
                vc.c = vc.c3;
            } else if (vertexColor4) {
                ci *= 4;
                if (colorsB != null) {
                    vc.c4 = new Color4f((colorsB[ci + 0] & 0xff) * ByteToFloatScale, (colorsB[ci + 1] & 0xff) * ByteToFloatScale, (colorsB[ci + 2] & 0xff) * ByteToFloatScale, (colorsB[ci + 3] & 0xff) * ByteToFloatScale);
                } else {
                    vc.c4 = new Color4f(colorsF[ci + 0], colorsF[ci + 1], colorsF[ci + 2], colorsF[ci + 3]);
                }
                vc.c = vc.c4;
            }
        }
        ...
        public void processVertex(int v, int stripFlag) {
            copyVertex(v + initialPositionIndex, v + initialNormalIndex, v + initialColorIndex, vc);
            processVertexCopy(vc, stripFlag);
        }
    }
    ...
    private class ByRefGeometryNIO implements GeometryAccessor {
        ...
        void copyVertex(int pi, int ni, int ci, VertexCopy vc) {
            pi *= 3;
            if (positionsF != null) {
                vc.p = new Point3f(positionsF.get(pi + 0), positionsF.get(pi + 1), positionsF.get(pi + 2));
            } else {
                vc.p = new Point3f((float) positionsD.get(pi + 0), (float) positionsD.get(pi + 1), (float) positionsD.get(pi + 2));
            }
            ni *= 3;
            if (vertexNormals) {
                vc.n = new Vector3f(normals.get(ni + 0), normals.get(ni + 1), normals.get(ni + 2));
            }
            if (vertexColor3) {
                ci *= 3;
                if (colorsB != null) {
                    vc.c3 = new Color3f((colorsB.get(ci + 0) & 0xff) * ByteToFloatScale, (colorsB.get(ci + 1) & 0xff) * ByteToFloatScale, (colorsB.get(ci + 2) & 0xff) * ByteToFloatScale);
                } else {
                    vc.c3 = new Color3f(colorsF.get(ci + 0), colorsF.get(ci + 1), colorsF.get(ci + 2));
                }
                vc.c = vc.c3;
            } else if (vertexColor4) {
                ci *= 4;
                if (colorsB != null) {
                    vc.c4 = new Color4f((colorsB.get(ci + 0) & 0xff) * ByteToFloatScale, (colorsB.get(ci + 1) & 0xff) * ByteToFloatScale, (colorsB.get(ci + 2) & 0xff) * ByteToFloatScale, (colorsB.get(ci + 3) & 0xff) * ByteToFloatScale);
                } else {
                    vc.c4 = new Color4f(colorsF.get(ci + 0), colorsF.get(ci + 1), colorsF.get(ci + 2), colorsF.get(ci + 3));
                }
                vc.c = vc.c4;
            }
        }
    ...
    void addGeometryArray(GeometryArray ga) {
        ...
        if (streamType != getStreamType(ga)) throw new IllegalArgumentException("GeometryArray has inconsistent dimensionality");
        if (vertexComponents != getVertexComponents(vertexFormat)) throw new IllegalArgumentException("GeometryArray has inconsistent vertex components");
    ...
    void print() {
        ...
        System.out.println("X, Y, Z centers (" + ((float) center[0]) + " " + ((float) center[1]) + " " + ((float) center[2]) + ")\n" + "scale " + ((float) scale) + "\n");
        Iterator i = stream.iterator();
        while (i.hasNext()) {
    ...
    public CompressionStream(int positionQuant, int colorQuant, int normalQuant, Shape3D shapes[]) {
        ...
        if (shapes == null) throw new IllegalArgumentException("null Shape3D array");
        if (shapes.length == 0) throw new IllegalArgumentException("zero-length Shape3D array");
        if (shapes[0] == null) throw new IllegalArgumentException("Shape3D at index 0 is null");
        long startTime = 0;
        if (benchmark) startTime = System.currentTimeMillis();
        Geometry g = shapes[0].getGeometry();
        if (!(g instanceof GeometryArray)) throw new IllegalArgumentException("Shape3D at index 0 is not a GeometryArray");
        ...
        for (int s = 0; s < shapes.length; s++) {
            ...
            if (!(g instanceof GeometryArray)) throw new IllegalArgumentException("Shape3D at index " + s + " is not a GeometryArray");
        ...
        if (benchmark) {
            ...
            System.out.println("\nCompressionStream:\n" + shapes.length + " shapes in " + (t / 1000f) + " sec");
        }
    }
    ...
    public CompressionStream(int positionQuant, int colorQuant, int normalQuant, GeometryInfo geometry[]) {
        ...
        if (geometry == null) throw new IllegalArgumentException("null GeometryInfo array");
        if (geometry.length == 0) throw new IllegalArgumentException("zero-length GeometryInfo array");
        if (geometry[0] == null) throw new IllegalArgumentException("GeometryInfo at index 0 is null");
        long startTime = 0;
        if (benchmark) startTime = System.currentTimeMillis();
        ...
        if (benchmark) {
            long t = System.currentTimeMillis() - startTime;
            System.out.println("\nCompressionStream:\n" + geometry.length + " GeometryInfo objects in " + (t / 1000f) + " sec");
        }
    }
