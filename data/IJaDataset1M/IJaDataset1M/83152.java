...
public class ParserGoal extends SourceGoal_c {
    ...
    protected SourceFile createDummyAST() {
        ...
        TopLevelDecl decl = nf.ClassDecl(pos, nf.FlagsNode(pos, Flags.PUBLIC), nf.Id(pos, name), null, Collections.<TypeNode>emptyList(), nf.ClassBody(pos, Collections.<ClassMember>emptyList()));
    ...
    public boolean runTask() {
        ...
        Node ast = null;
        try {
            if (Report.should_report("parser", 1)) Report.report(1, "" + source);
            ...
            eq.enqueue(ErrorInfo.IO_ERROR, e.getMessage(), new Position(job().source().path(), job().source().name(), 1, 1, 1, 1));
        }
        if (ast == null) {
            ast = createDummyAST();
        }
