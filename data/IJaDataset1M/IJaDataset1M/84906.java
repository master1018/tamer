...
public class StereotypeHelper {
    ...
    public static String getAnnotations(Element element, boolean tab) {
        ...
        while (i.hasNext()) {
            ...
            if (i.hasNext()) annotations.append("\n" + (tab ? "\t" : ""));
        }
    ...
    public static String getAnnotation(Element element, Stereotype stereotype) {
                ...
                if (value instanceof Collection) {
                    ...
                    while (j.hasNext()) {
                        ...
                        if (next instanceof org.eclipse.emf.ecore.impl.DynamicEObjectImpl) {
                            list += getAnnotation((org.eclipse.emf.ecore.impl.DynamicEObjectImpl) next, element);
                    ...
                    if (!((String) value).endsWith(".class") && !(((String) value).startsWith("{") && ((String) value).endsWith("}"))) {
                        value = "\"" + value + "\"";
                    }
                    String s = (String) value;
                    if (property.getName().equals("message") && s.startsWith("{") && s.endsWith("}")) {
                        value = "\"" + value + "\"";
                    }
                } else if (value instanceof EnumerationLiteral) {
                    EnumerationLiteral literal = (EnumerationLiteral) value;
                    value = ImportHelper.getImportedName(((Enumeration) literal.eContainer()).getName()) + "." + literal.getName();
                }
                if (first) {
                    annotation.append("(");
                    first = false;
        ...
        return "@" + ImportHelper.getImportedName(stereotype.getName()) + annotation.toString();
    }

    private static NamedElement findDefinition(String nameSuffix, Element element) {
    ...
    private static NamedElement findMember(String nameSuffix, Element element) {
        ...
        for (Iterator iter = members.iterator(); iter.hasNext(); ) {
            ...
            if (next instanceof Stereotype || next instanceof org.eclipse.uml2.uml.Class || next instanceof org.eclipse.uml2.uml.Enumeration || next instanceof org.eclipse.uml2.uml.EnumerationLiteral) {
    ...
    private static String getAnnotation(DynamicEObjectImpl impl, Element element) {
        ...
        NamedElement definition = findDefinition(name.substring(name.lastIndexOf('_') + 1), element);
        return "@" + ImportHelper.getImportedName(definition.getName()) + getFeature(eClass, impl, element, definition);
    }

    protected static String getFeature(EClass eClass, DynamicEObjectImpl impl, Element element, NamedElement definition) {
        ...
        for (Iterator i = eClass.getEAllAttributes().iterator(); i.hasNext(); ) {
            ...
            if (value instanceof DynamicEObjectImpl) {
                buffer.append(getAnnotation((DynamicEObjectImpl) value, element));
                ...
                while (j.hasNext()) {
                    Object next = j.next();
                    if (next instanceof org.eclipse.emf.ecore.impl.DynamicEObjectImpl) {
                        buffer.append(getAnnotation((org.eclipse.emf.ecore.impl.DynamicEObjectImpl) next, element));
                    } else if (next instanceof NamedElement) {
                        buffer.append("\"");
                        ...
                        NamedElement definition2 = findDefinition(name.substring(name.lastIndexOf('_') + 1), element);
                        buffer.append(ImportHelper.getImportedName(definition2.getName()) + "." + literal.getName());
        ...
        for (Iterator i = eClass.getEAllReferences().iterator(); i.hasNext(); ) {
            ...
            if (value != null) {
                ...
                if (value instanceof org.eclipse.emf.ecore.impl.DynamicEObjectImpl) {
                    buffer.append(getAnnotation((org.eclipse.emf.ecore.impl.DynamicEObjectImpl) value, element));
    ...
    public static Object getValue(Element property, String stereotype, String attribute) {
        Stereotype s = getStereotype(property, stereotype);
        if (s == null) throw new RuntimeException();
        return property.getValue(s, attribute);
    }

    public static String getValue(Element property, String stereotype, String attribute, String defaultValue) {
        Stereotype s = getStereotype(property, stereotype);
        if (s == null) {
            return defaultValue;
        }
    ...
    public static Integer getMaxLength(Property property) {
        ...
        for (Stereotype stereotype : stereotypes) {
            if (stereotype.getName().equals("org.hibernate.validator.Length")) {
                Property attr = stereotype.getAttribute("max", null);
                if (attr != null) {
                    return (Integer) property.getValue(stereotype, attr.getName());
                }
            }
        }
        return null;
    ...
    public static boolean isObfuscated(Element property) {
        EList<Stereotype> stereotypes = property.getAppliedStereotypes();
        for (Stereotype stereotype : stereotypes) {
            if (stereotype.getName().equals("net.taylor.security.obfuscation.Obfuscated")) {
                return true;
            }
        }
        return false;
