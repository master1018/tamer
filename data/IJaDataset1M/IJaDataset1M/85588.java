...
public class NewFileTemplate {
    ...
    public FileObject createFileFromTemplate(FileObject currFile, String content) {
        ...
        if (handleContent()) return create(currFile, true); else return null;
    }
        ...
        try {
            if (!f.exists()) {
                if (create) {
                    ...
                    if (fileName == null || fileName.length() == 0) return null; else fo = createFromTemplateFile(templateName, df, fileName, params);
                }
            } else {
    ...
    private FileObject createFromTemplateFile(String templateName, DataFolder folder, String name, Map<String, Object> parameters) {
    ...
    private boolean handleContent() {
        ...
        if (i != -1) {
            if (i2 != -1) fileName = content.substring(i2 + 1, i); else fileName = content.substring(0, i);
