...
public class VoteBoxEventsTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        matcher = new VoteBoxEventMatcher(ActivatedEvent.getMatcher(), AssignLabelEvent.getMatcher(), AuthorizedToCastEvent.getMatcher(), BallotReceivedEvent.getMatcher(), CastBallotEvent.getMatcher(), LastPollsOpenEvent.getMatcher(), OverrideCancelConfirmEvent.getMatcher(), OverrideCancelDenyEvent.getMatcher(), OverrideCancelEvent.getMatcher(), OverrideCastConfirmEvent.getMatcher(), OverrideCastDenyEvent.getMatcher(), OverrideCastEvent.getMatcher(), PollsClosedEvent.getMatcher(), PollsOpenEvent.getMatcher(), SupervisorEvent.getMatcher(), VoteBoxEvent.getMatcher());
    }

    public byte[] getBlob() {
        int n = (int) (Math.random() * 100);
        byte[] array = new byte[n];
        for (int i = 0; i < n; i++) array[i] = (byte) (Math.random() * 256);
        return array;
    }
    ...
    public void testAuthorizedToCast() {
        byte[] nonce = getBlob();
        byte[] ballot = getBlob();
        AuthorizedToCastEvent event = new AuthorizedToCastEvent(50, 65, nonce, ballot);
        ASExpression sexp = event.toSExp();
        assertEquals("(authorized-to-cast 65 " + (new BigInteger(nonce)).toString() + " " + StringExpression.makeString(ballot).toString() + ")", sexp.toString());
        AuthorizedToCastEvent event2 = (AuthorizedToCastEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertEquals(event.getNode(), event2.getNode());
    ...
    public void testCastBallot() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(cast-ballot " + nonce.toString() + " " + ballot.toString() + ")", sexp.toString());
    ...
    public void testBallotReceived() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(ballot-received 65 " + (new BigInteger(nonce)).toString() + ")", sexp.toString());
        BallotReceivedEvent event2 = (BallotReceivedEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertEquals(event.getNode(), event2.getNode());
        assertTrue(Arrays.equals(event.getNonce(), event2.getNonce()));
    }
    ...
    public void testOverrideCancel() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(override-cancel 65 " + (new BigInteger(nonce)).toString() + ")", sexp.toString());
        OverrideCancelEvent event2 = (OverrideCancelEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertEquals(event.getNode(), event2.getNode());
        assertTrue(Arrays.equals(event.getNonce(), event2.getNonce()));
    }
    ...
    public void testOverrideCast() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(override-cast 65 " + (new BigInteger(nonce)).toString() + ")", sexp.toString());
    ...
    public void testOverrideCancelConfirm() {
        byte[] nonce = getBlob();
        OverrideCancelConfirmEvent event = new OverrideCancelConfirmEvent(50, nonce);
        ASExpression sexp = event.toSExp();
        assertEquals("(override-cancel-confirm " + (new BigInteger(nonce)).toString() + ")", sexp.toString());
        OverrideCancelConfirmEvent event2 = (OverrideCancelConfirmEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertTrue(Arrays.equals(event.getNonce(), event2.getNonce()));
    }
    ...
    public void testOverrideCancelDeny() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(override-cancel-deny " + (new BigInteger(nonce)).toString() + ")", sexp.toString());
        OverrideCancelDenyEvent event2 = (OverrideCancelDenyEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertTrue(Arrays.equals(event.getNonce(), event2.getNonce()));
    }
    ...
    public void testOverrideCastConfirm() {
        byte[] nonce = getBlob();
        byte[] ballot = getBlob();
        OverrideCastConfirmEvent event = new OverrideCastConfirmEvent(50, nonce, ballot);
        ASExpression sexp = event.toSExp();
        assertEquals("(override-cast-confirm " + (new BigInteger(nonce)).toString() + " " + StringExpression.makeString(ballot).toString() + ")", sexp.toString());
        OverrideCastConfirmEvent event2 = (OverrideCastConfirmEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertTrue(Arrays.equals(event.getNonce(), event2.getNonce()));
        assertTrue(Arrays.equals(event.getBallot(), event2.getBallot()));
    }
    ...
    public void testOverrideCastDeny() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(override-cast-deny " + (new BigInteger(nonce)).toString() + ")", sexp.toString());
        OverrideCastDenyEvent event2 = (OverrideCastDenyEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertTrue(Arrays.equals(event.getNonce(), event2.getNonce()));
    }

    public void testLastPollsOpen() {
        LastPollsOpenEvent event = new LastPollsOpenEvent(50, new PollsOpenEvent(0, 123456, "hi"));
        ASExpression sexp = event.toSExp();
        assertEquals("(last-polls-open (polls-open 123456 hi))", sexp.toString());
        LastPollsOpenEvent event2 = (LastPollsOpenEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        assertEquals(event.getPollsOpenMsg().getTimestamp(), event2.getPollsOpenMsg().getTimestamp());
        assertEquals(event.getPollsOpenMsg().getKeyword(), event2.getPollsOpenMsg().getKeyword());
    }
    ...
    public void testActivated() {
        ...
        ASExpression sexp = event.toSExp();
        assertEquals("(activated ((status 50 (supervisor 123456 active)) (status 65 (votebox 3 ready 75 20 30))))", sexp.toString());
        ActivatedEvent event2 = (ActivatedEvent) matcher.match(50, sexp);
        assertEquals(event.getSerial(), event2.getSerial());
        SupervisorEvent supStatus2 = (SupervisorEvent) event2.getStatuses().get(0).getStatus();
        ...
        assertEquals(vbStatus.getProtectedCount(), vbStatus2.getProtectedCount());
