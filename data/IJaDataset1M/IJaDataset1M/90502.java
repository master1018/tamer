    ...
    public void testProcessorModel() {
        {
            ...
            for (int i = 0; i < 4; ++i) assertEquals(p.getTrackCount(i), -1);
        ...
        {
            final ProcessorModel p = new ProcessorModel(new Format[] { new Format("abc"), new Format("xyz") }, new ContentDescriptor("abc"));
            for (int i = 0; i < 4; ++i) assertEquals(p.getTrackCount(i), 2);
