...
public class MProductUPC extends X_M_ProductUPC {
    ...
    public static MProductUPC getDefault(Properties ctx, int productID, String trxName) {
        ...
        ResultSet rs = null;
        String sql = "SELECT * FROM M_ProductUPC " + "WHERE M_Product_ID = ? AND IsDefault = 'Y' AND IsActive = 'Y'";
        try {
    ...
    public static List<MProductUPC> getOfProduct(Properties ctx, int productID, String trxName) {
        ...
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        String sql = "SELECT * FROM M_ProductUPC " + "WHERE M_Product_ID = ? " + "ORDER BY IsDefault DESC, Created DESC";
        try {
            pstmt = DB.prepareStatement(sql, trxName);
            pstmt.setInt(1, productID);
            rs = pstmt.executeQuery();
    ...
    protected boolean afterSave(boolean newRecord, boolean success) {
        if (success) {
            if (isDefault()) {
                MProductUPC previousDefaultUPC = MProductUPC.getDefault(getCtx(), getM_Product_ID(), get_TrxName());
    ...
    private boolean validateUniqueUPC() {
        String sql = "SELECT M_Product_ID FROM M_ProductUPC " + "WHERE AD_Client_ID = ? AND UPC = ? AND M_ProductUPC_ID <> ? ";
        Integer productID = (Integer) DB.getSQLObject(get_TrxName(), sql, new Object[] { getAD_Client_ID(), getUPC(), getM_ProductUPC_ID() });
        if (productID != null && productID > 0) {
            MProduct product = MProduct.get(getCtx(), productID);
            String productStr = "'" + product.getValue() + " " + product.getName() + "'";
            log.saveError("SaveError", Msg.translate(getCtx(), "DuplicateUPCError") + " " + productStr);
        }
    ...
    private boolean setDefaultUPC() {
        ...
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        String sql = "SELECT * FROM M_ProductUPC " + "WHERE M_Product_ID = ? AND IsActive = 'Y' AND M_ProductUPC_ID <> ? " + "ORDER BY Created DESC";
        try {
