 ...
 * @author acheca
 */
public class GetNextChapterAction extends CMSPortletAction {
    ...
    public void processAction(ActionMapping mapping, ActionForm form, PortletConfig config, ActionRequest req, ActionResponse res) throws IOException {
        ...
        try {
            ...
            try {
                ...
                ToolMenuItem currentChapter = (StringUtils.isEmpty(chapterId)) ? (ToolMenuItem) chaptersMenu.getChildren().get(0) : chaptersMenu.getItemByLink(chapterId);
                ToolMenuItem chapter = chaptersMenu.getNextItem(currentChapter);
                if (chapter == null) {
                    ...
                    req.setAttribute("prevChapters", "true");
                    ToolMenuItem chapterNext = chaptersMenu.getNextItem(chapter);
                    if (chapterNext != null) {
                        writer.write(chapter.getLink() + ":true:true");
                        req.setAttribute("nextChapter", "true");
                    } else {
