...
public class SourceToHTMLConverter {
    ...
    public static void convertRoot(ConfigurationImpl configuration, RootDoc rd, String outputdir) {
        ...
        for (int i = 0; i < pds.length; i++) {
            if (!(configuration.nodeprecated && Util.isDeprecated(pds[i]))) convertPackage(configuration, pds[i], outputdir);
        }
        ClassDoc[] cds = rd.specifiedClasses();
        for (int i = 0; i < cds.length; i++) {
            if (!(configuration.nodeprecated && (Util.isDeprecated(cds[i]) || Util.isDeprecated(cds[i].containingPackage())))) convertClass(configuration, cds[i], getPackageOutputDir(outputdir, cds[i].containingPackage()));
        }
    }
    ...
    public static void convertPackage(ConfigurationImpl configuration, PackageDoc pd, String outputdir) {
        ...
        for (int i = 0; i < cds.length; i++) {
            if (!(configuration.nodeprecated && Util.isDeprecated(cds[i]))) convertClass(configuration, cds[i], classOutputdir);
        }
    }
    ...
    private static void writeToFile(Content body, String outputDir, String className, ConfigurationImpl configuration) throws IOException {
        ...
        head.addContent(HtmlTree.TITLE(new StringContent(configuration.getText("doclet.Window_Source_title"))));
