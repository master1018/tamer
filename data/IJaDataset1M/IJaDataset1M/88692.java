...
public class Svdlib {
    ...
    static double svd_fsign(double a, double b) {
        if ((a >= 0.0 && b >= 0.0) || (a < 0.0 && b < 0.0)) return a; else return -a;
    }
    ...
    static void svd_opb(SMat A, double[] x, double[] y, double[] temp) {
        ...
        for (int i = 0; i < A.cols; i++) {
            int end = pointr[i + 1];
            for (int j = pointr[i]; j < end; j++) temp[rowind[j]] += value[j] * (x[i]);
        }
        for (int i = 0; i < A.cols; i++) {
            int end = pointr[i + 1];
            for (int j = pointr[i]; j < end; j++) y[i] += value[j] * temp[rowind[j]];
        }
        return;
    }
    ...
    static void svd_opa(SMat A, double[] x, double[] y) {
        ...
        for (int i = 0; i < A.cols; i++) {
            int end = pointr[i + 1];
            for (int j = pointr[i]; j < end; j++) y[rowind[j]] += value[j] * x[i];
        }
        return;
    }
    ...
    static SMat svdConvertDtoS(DMat D) {
        ...
        for (j = 0, n = 0; j < D.cols; j++) {
            S.pointr[j] = n;
            for (i = 0; i < D.rows; i++) if (D.value[i][j] != 0) {
                S.rowind[n] = i;
    ...
    static DMat svdTransposeD(DMat D) {
        ...
        for (r = 0; r < D.rows; r++) for (c = 0; c < D.cols; c++) N.value[c][r] = D.value[r][c];
        return N;
    }
...
nnzero   number of nonzero elements in input matrix (matrix A)      
    ...
    static int check_parameters(SMat A, long dimensions, long iterations, double endl, double endr, boolean b) {
        int error_index;
        error_index = 0;
        if (endl > endr) error_index = 2; else if (dimensions > iterations) error_index = 3; else if (A.cols <= 0 || A.rows <= 0) error_index = 4; else if (iterations <= 0 || iterations > A.cols || iterations > A.rows) error_index = 5; else if (dimensions <= 0 || dimensions > iterations) error_index = 6;
        if (0 != error_index) svd_error("svdLAS2 parameter error: %s\n", error_msg[error_index]);
        return (error_index);
    }
...
LAS2         ritvec, lanso
    ...
    public SVDRec svdLAS2(SMat A, int dimensions, int iterations, double[] end, double kappa) {
        ...
        if (dimensions <= 0 || dimensions > m) dimensions = m;
        if (iterations <= 0 || iterations > m) iterations = m;
        if (iterations < dimensions) iterations = dimensions;
        if (SVDVerbosity > 0) write_header(iterations, dimensions, end[0], end[1], true, kappa, A.rows, A.cols, A.vals);
        ...
        steps = lanso(A, iterations, dimensions, end[0], end[1], ritz, bnd, wptr, ref_neig, n);
        int neig = ref_neig[0];
        if (SVDVerbosity > 0) {
        ...
        ritvec(n, A, R, kappa, ritz, bnd, wptr[6], wptr[9], wptr[5], steps, neig);
        if (SVDVerbosity > 1) {
            printf("\nSINGULAR VALUES: ");
        ...
        } else if (SVDVerbosity > 0) printf("SINGULAR VALUES FOUND     = %6d\n", R.d);
        if (transpose) {
            DMat swap = R.Ut;
            R.Ut = R.Vt;
            R.Vt = swap;
...
imtql2
    ...
    long ritvec(int n, SMat A, SVDRec R, double kappa, double[] ritz, double[] bnd, double[] alf, double[] bet, double[] w2, int steps, long neig) {
        ...
        s = null;
        rotateArray(R.Vt.value, R.Vt.rows * R.Vt.cols, x * R.Vt.cols);
...
UTILITY	svd_imin, svd_imax
    ...
    int lanso(SMat A, int iterations, int dimensions, double endl, double endr, double[] ritz, double[] bnd, double[][] wptr, int[] neigp, int n) {
        ...
        bet = wptr[9];
        wrk = wptr[5];
        double[] ref_rnm = new double[] { 0d };
        double[] ref_tol = new double[] { 0d };
        stpone(A, wptr, ref_rnm, ref_tol, n);
        double tol = ref_tol[0];
        double rnm = ref_rnm[0];
        if (0 == rnm || 0 != ierr) return 0;
        eta[0] = eps1;
        oldeta[0] = eps1;
        ll = 0;
        first = 1;
        ...
        while (!ENOUGH) {
            ...
            j = lanczos_step(A, first, last, wptr, alf, eta, oldeta, bet, ref_ll, ref_ENOUGH, ref2_rnm, ref2_tol, n);
            ll = ref_ll[0];
            ENOUGH = ref_ENOUGH[0];
            tol = ref2_tol[0];
            rnm = ref2_rnm[0];
            if (ENOUGH) j = j - 1; else j = last - 1;
            first = j + 1;
            bet[j + 1] = rnm;
            l = 0;
            for (int id2 = 0; id2 < j; id2++) {
                if (l > j) break;
                for (i = l; i <= j; i++) if (0 == bet[i + 1]) break;
                if (i > j) i = j;
                ...
                if (0 != ierr) {
                    ...
                    svd_error("  l = %ld  i = %ld\n", l, i);
                    for (id3 = l; id3 <= i; id3++) svd_error("  %ld  %lg  %lg  %lg\n", id3, ritz[id3], wrk[id3], bnd[id3]);
                }
                for (id3 = l; id3 <= i; id3++) bnd[id3] = rnm * fabs(bnd[id3]);
                l = i + 1;
            }
            ...
            if (neig < dimensions) {
                if (0 == neig) {
                    last = first + 9;
                    intro = first;
                } else last = first + svd_imax(3, 1 + ((j - intro) * (dimensions - neig)) / neig);
...
UTILITY	svd_imin, svd_imax
    ...
    int lanczos_step(SMat A, int first, int last, double[][] wptr, double[] alf, double[] eta, double[] oldeta, double[] bet, int[] ll, boolean[] refEnough, double[] rnmp, double[] tolp, int n) {
        ...
        for (j = first; j < last; j++) {
            ...
            if (j <= MAXLL && (Math.abs(alf[j - 1]) > 4.0 * Math.abs(alf[j]))) ll[0] = j;
            ...
            purge(n, ll[0], wptr[0], wptr[1], wptr[4], wptr[3], wptr[5], eta, oldeta, j, ref_rnm, tol);
            rnm = ref_rnm[0];
            if (rnm <= tol) rnm = 0.0;
        }
        rnmp[0] = rnm;
...
BLAS		svd_dswap
    ...
    void ortbnd(double[] alf, double[] eta, double[] oldeta, double[] bet, int step, double rnm) {
        ...
        if (0 != rnm) {
            if (step > 1) {
                oldeta[0] = (bet[1] * eta[1] + (alf[0] - alf[step]) * eta[0] - bet[step] * oldeta[0]) / rnm + eps1;
            }
            for (i = 1; i <= step - 2; i++) oldeta[i] = (bet[i + 1] * eta[i + 1] + (alf[i] - alf[step]) * eta[i] + bet[i] * eta[i - 1] - bet[step] * oldeta[i]) / rnm + eps1;
        }
        oldeta[step - 1] = eps1;
...
MISC		random
    ...
    double startv(SMat A, double[][] wptr, int step, int n) {
        ...
        Random random = new Random(918273L + step);
        r = wptr[0];
        for (id = 0; id < 3; id++) {
            if (id > 0 || step > 0 || rnm2 == 0) for (i = 0; i < n; i++) r[i] = random.nextDouble();
...
UTILITY	svd_dmin
    ...
    int error_bound(boolean[] enough, double endl, double endr, double[] ritz, double[] bnd, int step, double tol) {
        ...
        for (i = ((step + 1) + (step - 1)) / 2; i >= mid + 1; i -= 1) if (fabs(ritz[i - 1] - ritz[i]) < eps34 * fabs(ritz[i])) if (bnd[i] > tol && bnd[i - 1] > tol) {
    ...
    void store(int n, storeVals isw, int j, double[] s) {
        switch(isw) {
            case STORQ:
                ...
                break;
            case RETRQ:
                if (null == LanStore[j + MAXLL]) throw new Error(String.format("svdLAS2: store (RETRQ) called on index %d (not allocated)", j + MAXLL));
            ...
            case RETRP:
                ...
                if (null == LanStore[j]) throw new Error(String.format("svdLAS2: store (RETRP) called on index %d (not allocated)", j));
