...
public class WSGraph extends JPanel implements Observer {
    ...
    private Color getNeededColor(VerificationFormula vf) {
        ...
        if (vf.getStatus() == Status.RESULTOK) {
            return Color.GREEN;
        }
        if (vf.getStatus() == Status.RESULTWRONG) {
            return new Color(255, 112, 64);
        }
    ...
    private String translateRule(Rule rule) {
        ...
        switch(rule) {
            case ASSIGN:
                return Messages.getString("hoare", "rule.assign.desc");
            case IF:
                return Messages.getString("hoare", "rule.alt1.desc");
            case IFELSE:
                return Messages.getString("hoare", "rule.alt2.desc");
            case COMPOUND:
                return Messages.getString("hoare", "rule.comp.desc");
            case STATSEQ:
                return Messages.getString("hoare", "rule.sequence.desc");
            case ITERATION:
                return Messages.getString("hoare", "rule.iter.desc");
            case STRONGPRE:
                return Messages.getString("hoare", "rule.SV.desc");
            case WEAKPOST:
                return Messages.getString("hoare", "rule.SN.desc");
            default:
                return "";
        }
    }
    ...
    private void changeCell(VerificationFormula vf) {
        ...
        if (cell != null) {
            cell.setUserObject(VertexPanelFactory.getVertexPanel(gui, cell, cell == getSelectedCell(), cell == opened));
    ...
    private String getCellText(VerificationFormula vf, boolean full) {
        ...
        TextStyle style;
        if (full) {
            format = "<html><div style=\"padding:10pt;font-size:20pt;text-align:center;\">%s" + " <b>{%s}</b><br><i>%s</i><br><b>{%s}</b> %s</div></html>";
            style = TextStyle.FULL;
        } else {
            format = "<html><div style=\"padding:5pt;\">%s" + " <b>{%s}</b> <i>%s</i> <b>{%s}</b> %s</div></html>";
        ...
        if (vf.getParent() != null) {
            ...
            if (Rule.IF.equals(rule)) {
                if (full) {
                    resultPost = "(" + vf.getPreAssertion(style) + ") => " + vf.getParent().getPostAssertion(style) + ")";
    ...
    private ArrayList<VerificationFormula> getChanges(VerificationFormula test) {
        ...
        if (test.hasChanged() && test.isRuleApplied()) {
            changes.add(test);
        }
        return changes;
    }
