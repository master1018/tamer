...
public final class SingletonEhCacheProvider implements CacheProvider {
    ...
    public final Cache buildCache(String name, Properties properties) throws CacheException {
        try {
            ...
            if (cache == null) {
                SingletonEhCacheProvider.LOG.warning("Could not find a specific ehcache configuration for cache named [" + name + "]; using defaults.");
    ...
    private URL loadResource(String configurationResourceName) {
        ...
        if (LOG.isLoggable(Level.FINE)) {
            LOG.fine("Creating EhCacheProvider from a specified resource: " + configurationResourceName + " Resolved to URL: " + url);
        }
        if (url == null) {
            if (LOG.isLoggable(Level.WARNING)) {
                LOG.warning("A configurationResourceName was set to " + configurationResourceName + " but the resource could not be loaded from the classpath." + "Ehcache will configure itself using defaults.");
            }
        }
        return url;
    }
