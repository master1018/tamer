...
public class CookieJar {
    ...
    static final Pattern pNvPair = Pattern.compile("^\\s*([^=\\s]+)(\\s*=\\s*(.+))*$", Pattern.CASE_INSENSITIVE);
    ...
    private static class Cookie implements Cloneable {
        ...
        public String getKey() {
            ...
            if (null != this.path) ret += this.path;
            return ret;
        }
        ...
        public String getHeader(String path, int secure) {
            ...
            if ((null == this.path || this.path.equals("/") || this.path.startsWith(path)) && (this.secure <= secure) && (this.max_age > System.currentTimeMillis())) {
                if (this.version > 0) {
                    ...
                    if (null != this.path) sb.append(" $Path=").append(this.path).append(";");
                    if (null != this.domain) sb.append(" $Domain=").append(this.domain).append(";");
                } else {
                    sb.append(this.name).append("=").append(this.value).append(";");
                }
            }
        ...
        while (t.hasMoreTokens()) {
            ...
            if (m.matches()) {
                ...
                if (n.compareToIgnoreCase("max-age") == 0) {
                    ...
                    if (cookie.max_age < 0) cookie.max_age = 0;
                    cookie.max_age *= 1000;
