    ...
    static class RawInet4AddressRangeAuthorizer implements IRawInetAddressAuthorizer {
        ...
        public boolean isAuthorized(final byte[] addrBytes) {
            ...
            for (int i = 0; i < addrAsShorts.length; i++) {
                if ((startAddress[i] != SHORT_MASK) && ((addrAsShorts[i] < startAddress[i]) || (addrAsShorts[i] > endAddress[i]))) {
                    return false;
                }
            }
            return true;
        ...
        for (int i = 0; i < octets.length; i++) {
            if (octets[i].contains(RANGE_TOKEN)) {
                ...
                if (upperBounds < lowerBounds) {
                    throw new IllegalArgumentException("Byte Range must be specificed as '<higher bounds>" + RANGE_TOKEN + "<lower bounds>'. Provided: " + octets[i]);
                }
