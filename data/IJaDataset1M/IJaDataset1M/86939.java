...
public class TigerDimeReadUtils {
    ...
    private static class XMLTreeDescription {
        ...
        boolean isValid() {
            ...
            if (_parsed == INVALID) {
                ...
                if (LOG.isDebugEnabled()) LOG.debug("unexpected block size: " + _blockSize);
                return false;
            } else if (!TigerDimeUtils.DIGEST.equals(_algorithm)) {
                if (LOG.isDebugEnabled()) LOG.debug("unsupported digest algorithm: " + _algorithm);
                return false;
            } else if (_hashSize != TigerDimeUtils.HASH_SIZE) {
                if (LOG.isDebugEnabled()) LOG.debug("unexpected block size: " + _blockSize);
                return false;
            } else if (!TigerDimeUtils.SERIALIZED_TREE_TYPE.equals(_serializationType)) {
                if (LOG.isDebugEnabled()) LOG.debug("unexpected serialization type: " + _serializationType);
                return false;
            }
            return true;
        }
        ...
        private boolean parse() {
            ...
            if (offset > 0 && offset < data.indexOf(TigerDimeUtils.DTD_SYSTEM_ID)) {
                data = data.substring(0, offset) + TigerDimeUtils.SYSTEM_STRING + data.substring(offset + "system".length());
            }
            if (LOG.isDebugEnabled()) LOG.debug("XMLTreeDescription read: " + data);
            Document doc = null;
            try {
            ...
            if (treeDesc == null) {
                if (LOG.isDebugEnabled()) LOG.debug("couldn't find hashtree element: " + data);
                return false;
            }
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (el.getTagName().equals("file")) parseFileElement(el); else if (el.getTagName().equals("digest")) parseDigestElement(el); else if (el.getTagName().equals("serializedtree")) parseSerializedtreeElement(el);
                }
            }
            return true;
        }
        ...
        private void parseFileElement(Element e) {
            try {
                ...
                if (LOG.isDebugEnabled()) LOG.debug("couldn't parse file size: " + e.getNodeValue(), nfe);
            }
            try {
                _blockSize = Integer.parseInt(e.getAttribute("segmentsize"));
            } catch (NumberFormatException nfe) {
                if (LOG.isDebugEnabled()) LOG.debug("couldn't parse block size: " + e.getNodeValue(), nfe);
            }
        }
        ...
        private void parseDigestElement(Element e) {
            ...
            try {
                ...
                if (LOG.isDebugEnabled()) LOG.debug("couldn't parse hash size: " + e.getNodeValue(), nfe);
            }
        }
        ...
        private void parseSerializedtreeElement(Element e) {
            ...
            try {
                ...
                if (LOG.isDebugEnabled()) LOG.debug("couldn't parse depth: " + e.getNodeValue(), nfe);
            }
        }
    }
    ...
    private static final class Resolver implements EntityResolver {
        ...
        public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException {
            if (systemId.equals(TigerDimeUtils.DTD_SYSTEM_ID)) {
                ...
                is.setPublicId(TigerDimeUtils.DTD_PUBLIC_ID);
                is.setSystemId(TigerDimeUtils.DTD_SYSTEM_ID);
                return is;
            }
            if (publicId == null) throw new SAXException("Can't resolve SYSTEM entity at '" + systemId + "'"); else throw new SAXException("Can't resolve PUBLIC entity '" + publicId + "' at '" + systemId + "'");
        }
    }
    ...
    private static class HashTreeDescription {
        ...
        byte[] getRoot() throws IOException {
            if (DATA.length < TigerDimeUtils.HASH_SIZE) throw new IOException("invalid data");
        ...
        List<List<byte[]>> getAllNodes(long fileSize) throws IOException {
            ...
            if (data.length % TigerDimeUtils.HASH_SIZE != 0) {
                if (LOG.isDebugEnabled()) LOG.debug("illegal size of data field for HashTree");
            ...
            while (genIndex <= depth && hashIterator.hasNext()) {
                ...
                if (parent == null) {
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("parent");
                        String str = "";
                        for (byte[] b : parent) str = str + Base32.encode(b) + "; ";
                        LOG.debug(str);
                        str = "";
                        LOG.debug("newparent");
                        List<byte[]> newparent = HashTreeUtils.createParentGeneration(generation, new Tiger());
                        for (byte[] b : newparent) str = str + Base32.encode(b) + "; ";
                        LOG.debug(str);
                        str = "";
                        LOG.debug("generation");
                        for (byte[] b : generation) str = str + Base32.encode(b) + "; ";
                        LOG.debug(str);
                        str = "";
                    }
                    throw new IOException("corrupted hash tree detected");
                } else if (generation.size() == parent.size() * 2 - 1 || generation.size() == parent.size() * 2) {
                    List<byte[]> calculatedParent = HashTreeUtils.createParentGeneration(generation, new Tiger());
                    if (isMatching(parent, calculatedParent)) {
                        genIndex++;
                        parent = generation;
                        allNodes.add(Collections.unmodifiableList(generation));
                        if (genIndex <= depth && hashIterator.hasNext()) generation = new ArrayList<byte[]>(parent.size() * 2);
                        verified = true;
                    }
                }
            }
            if (!verified) throw new IOException("corrupted hash tree detected");
    ...
    static List<List<byte[]>> nodesFromRecords(Iterator<DIMERecord> iterator, long fileSize, String root32) throws IOException {
        if (!iterator.hasNext()) throw new IOException("no xml record");
        DIMERecord xmlRecord = iterator.next();
        if (!iterator.hasNext()) throw new IOException("no tree record");
        ...
        while (iterator.hasNext()) {
            if (LOG.isWarnEnabled()) LOG.warn("more elements in the dime record.");
            iterator.next();
        }
        ...
        if (!xtd.isValid()) throw new IOException("invalid XMLTreeDescription " + xtd.toString());
        if (xtd.getFileSize() != fileSize) throw new IOException("file size attribute was " + xtd.getFileSize() + " expected " + fileSize);
        HashTreeDescription htr = new HashTreeDescription(hashTree);
        if (!Base32.encode(htr.getRoot()).equals(root32)) throw new IOException("Root hashes do not match");
