...
public class OneWireContainer33 extends OneWireContainer {
    ...
    protected static final byte[] ffBlock = { (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF };
    ...
    */
    public OneWireContainer33() {
        ...
        System.arraycopy(ffBlock, 0, challenge, 0, 8);
        setAdapter = false;
    ...
    */
    public boolean isSecretWriteProtected() throws OneWireIOException, OneWireException {
        if (!container_check) container_check = this.checkStatus();
    ...
    */
    protected boolean checkStatus() throws OneWireIOException, OneWireException {
        if (!container_check) {
            ...
            if ((mem[0] == (byte) 0xAA) || (mem[0] == (byte) 0x55)) secretProtected = true; else secretProtected = false;
            ...
            if ((mem[4] == (byte) 0xAA) || (mem[4] == (byte) 0x55)) memoryPages[1].writeOnce = true; else memoryPages[1].writeOnce = false;
    ...
    */
    public static boolean isMACValid(int addr, byte[] SerNum, byte[] memory, byte[] mac, byte[] challenge, byte[] secret) {
        ...
        MT[55] = (byte) 0x80;
        for (int i = 56; i < 62; i++) MT[i] = (byte) 0x00;
        MT[62] = (byte) 0x01;
        MT[63] = (byte) 0xB8;
        int[] AtoE = new int[5];
    ...
    */
    public boolean installMasterSecret(int page, byte[] newSecret) throws OneWireIOException, OneWireException {
        for (int i = 0; i < secret.length; i++) secret[i] = 0x00;
        if (loadFirstSecret(secret, 0)) {
            if (newSecret.length == 0) return false;
            byte[] input_secret = null;
            byte[] sp_buffer = new byte[8];
            int secret_mod_length = newSecret.length % 47;
            if (secret_mod_length == 0) input_secret = newSecret; else {
            ...
            return true;
        } else throw new OneWireException("Load first secret failed");
    }
    ...
    */
    public boolean bindSecretToiButton(int pageNum, byte[] bindData) throws OneWireIOException, OneWireException {
        if (!writeDataPage(pageNum, bindData)) return false;
    ...
    * @throws OneWireException
    */
    public void readScratchpad(byte[] scratchpad, int offset, byte[] extraInfo) throws OneWireIOException, OneWireException {
