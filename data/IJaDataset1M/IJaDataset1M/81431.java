...
public final class ListTagsDialog extends AbstractDialog implements MouseListener {
    ...
    private void init() {
        ...
        if (tags == null) {
            ...
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (t.lineNumber() > editor.getDotLineNumber()) break;
                index = i;
            }
        }
        ...
        if (h > 0) list.setFixedCellHeight(h);
        ...
        if (getSize().width > maxWidth) setSize(new Dimension(maxWidth, getSize().height));
    ...
    protected void ok() {
        ...
        if (tags != null && index >= 0 && tags.size() > index) tag = (Tag) tags.get(index);
    }
    ...
    public static void listTags() {
        ...
        d.show();
        Tag tag = d.getTag();
        if (tag instanceof LocalTag) TagCommands.gotoLocalTag(editor, (LocalTag) tag, false);
    }
}
