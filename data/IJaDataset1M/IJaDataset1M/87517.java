...
public class JDSpline extends JDPolyline implements JDPolyConvert {
   ...
   */
    ...
    JDSpline(LXObject lxObj, double[] ptsx, double[] ptsy, boolean closed) {
        ...
        for (i = 0; i < summit.length; i++) summit[i] = (Point.Double) newPts.get(i);
        isClosed = closed;
        step = 10;
        updateShape();
    ...
    void deleteSummit() {
        ...
        if (breakId == 0) {
            ...
            for (int i = 0, nb = 0; i < summit.length; i++) if (i < breakId - 1 || i > breakId + 1) nSummit[nb++] = summit[i];
        }
        summit = nSummit;
        updateShape();
    }
    ...
    boolean canDeleteSummit(int id) {
        ...
        breakId = -1;
        if ((id < 0) || (id >= summit.length) || summit.length <= 4) return false;
    ...
    void breakShape() {
        ...
        if (sumId < summit.length) {
            ...
            for (i = 0; i < summit.length; i++) nSummit[i] = summit[i];
    ...
    JDSpline(JDFileLoader f) throws IOException {
        ...
        if (summit.length < 2) throw new IOException("Invalid summit number for JDSpline at line " + l);
        while (!f.isEndBlock()) {
   ...
   */
    public void connect(JDPolyline pline) {
        if (pline instanceof JDSpline) {
            ...
            for (i = 0; i < pline.getSummitNumber(); i++) nSummit[i + 2 + summit.length] = new Point.Double(pline.getSummit(i).x, pline.getSummit(i).y);
            summit = nSummit;
            updateShape();
        } else {
    ...
    void updateShape() {
        ...
        for (i = 0; i < summit.length - 1; i += 3) {
            JDUtils.computeSpline(summit[i + 0].x, summit[i + 0].y, summit[i + 1].x, summit[i + 1].y, summit[i + 2].x, summit[i + 2].y, summit[i + 3].x, summit[i + 3].y, step, i == (summit.length - 4), nb, null, ptsx, ptsy);
            nb += step;
        }
        if (hasShadow()) {
