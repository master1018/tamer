...
public class SampleColoring<VertexType extends BaseVertex, EdgeType extends BaseEdge<VertexType>> extends Algorithm implements AutomatedAlgorithm {
    ...
    public void doColoring() {
        if (graph == null) throw new InvalidGraphException();
        for (VertexType v : graph) {
            v.setColor(graph.getInDegree(v));
            dispatchEvent(new VertexEvent<VertexType, EdgeType>(graph, v, VertexEvent.EventType.COLOR_CHANGE));
        }
    }
