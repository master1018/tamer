...
public class DefWalker {
    ...
    public NodeDef expect(Class elemType) throws XOMException {
        if (pos >= defs.length) throw new XOMException("Expecting a Node of type " + elemType.getName() + " but no " + "Nodes remain.");
        if (!(elemType.isAssignableFrom(defs[pos].getClass()))) throw new XOMException("Expecting a Node of type " + elemType.getName() + " but " + "found a Node of type " + defs[pos].getClass().getName());
        return defs[pos++];
    }
    ...
    public NodeDef[] expectArray(Class elemType) {
        ...
        while (pos < defs.length && elemType.isAssignableFrom(defs[pos].getClass())) found.addElement(defs[pos++]);
        NodeDef[] ret = new NodeDef[found.size()];
        for (int i = 0; i < found.size(); i++) ret[i] = (NodeDef) (found.elementAt(i));
        return ret;
    }
    ...
    public Object expectTypeArray(Class elemType) {
        ...
        for (int i = 0; i < found.size(); i++) Array.set(ret, i, found.elementAt(i));
        return ret;
    }
}
