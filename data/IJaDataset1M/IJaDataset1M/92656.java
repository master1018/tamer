...
public abstract class AbstractWiggle implements Graph, Query {
    ...
    public float[] get(int start, int end, int resolutionIndex) {
        if (buffer5 == null) throw new RuntimeException("Wiggle needs to be initialized!");
        if (lastStart == start && lastEnd == end && lastRes == resolutionIndex) return last;
    ...
    private float[] merge(float[] ds) {
        ...
        for (int i = 0; i < ds.length - 1; i += 2) {
            ...
            if (out[i / 2] > max) max = out[i / 2];
        }
        if (ds.length % 2 == 1) out[out.length - 1] = ds[ds.length - 1];
        return out;
    }
