...
public class ConfiguredFeatureAdapter extends SimpleFeatureAdapter implements IConfiguredFeatureAdapter {
    ...
    public boolean equals(Object object) {
        ...
        if (object instanceof ConfiguredFeatureAdapter) {
            try {
                ...
                return ad.getConfiguredSite().equals(getConfiguredSite()) && ad.getFeature(null).equals(getFeature(null));
    ...
    public IFeatureAdapter[] getIncludedFeatures(IProgressMonitor monitor) {
        try {
            IIncludedFeatureReference[] included = getFeature(null).getIncludedFeatureReferences();
            ConfiguredFeatureAdapter[] result = new ConfiguredFeatureAdapter[included.length];
            ...
            for (int i = 0; i < included.length; i++) {
                ...
                try {
                    feature = fref.getFeature(new SubProgressMonitor(subMonitor, 1));
                    childConfigured = adapter.getConfiguredSite().isConfigured(feature);
                ...
                result[i] = new ConfiguredFeatureAdapter(adapter, feature, childConfigured, updated, fref.isOptional());
