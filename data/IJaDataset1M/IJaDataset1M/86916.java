...
public class cStringIO {
    ...
    public static class StringIO extends PyObject {
        ...
        public transient boolean softspace = false;
        ...
        public void seek(long pos, int mode) {
            if (mode == 1) this.pos = (int) pos + this.pos; else if (mode == 2) this.pos = (int) pos + count; else this.pos = Math.max(0, (int) pos);
        }
        ...
        public String readline(int length) {
            ...
            if (length != -1 && pos + length < newpos) newpos = pos + length;
        ...
        public void write(String s) {
            ...
            if (newpos >= buf.length) expandCapacity(newpos);
            if (newpos > count) count = newpos;
        ...
        public void writeChar(char ch) {
            if (pos + 1 >= buf.length) expandCapacity(pos + 1);
            buf[pos++] = ch;
        ...
        private final void opencheck() {
            if (buf == null) throw Py.ValueError("I/O operation on closed file");
        }
    }
