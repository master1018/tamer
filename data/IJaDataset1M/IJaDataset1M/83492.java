...
public class Scanning extends org.mmtk.vm.Scanning {
   ...
   */
    public static Set<ObjectValue> getRoots() {
        if (threadIteratorTable == null) return Collections.emptySet();
    ...
    public void computeStaticRoots(TraceLocal trace) {
        if (threadIteratorTable != null) {
            ...
            if (StackFrame.ASSERT_WILL_NOT_MOVE) {
                assert trace.willNotMoveInCurrentCollection(threadIteratorTable.getObjectValue()) : threadIteratorTable.getObjectValue() + " has been traced but willNotMoveInCurrentCollection is still false";
            }
            if (Trace.isEnabled(Item.ROOTS)) {
