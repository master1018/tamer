...
public abstract class VM_Thread {
   ...
   * create one.
   */
    protected static final InterruptedException proxyInterruptException = new InterruptedException("park interrupted");
   ...
   */
    ...
    protected final void changeThreadState(State oldState, State newState) {
        ...
        if (state == oldState) {
            ...
            throw new IllegalThreadStateException("Illegal thread state change from " + oldState + " to " + newState + " when in state " + state + " in thread " + name);
        }
    }
   ...
   */
    ...
    public static void sleep(long millis, int ns) throws InterruptedException {
        ...
        try {
            ...
            if (myThread.state != State.RUNNABLE) myThread.changeThreadState(State.SLEEPING, State.RUNNABLE);
   ...
   */
    ...
    private void handlePossibleRecursiveException() {
        if (uncaughtExceptionCount > 1 && uncaughtExceptionCount <= VM.maxSystemTroubleRecursionDepth + VM.maxSystemTroubleRecursionDepthBeforeWeStopVMSysWrite) {
   ...
   */
    public int dump(char[] dest, int offset) {
        offset = VM_Services.sprintf(dest, offset, getIndex());
        if (daemon) {
            offset = VM_Services.sprintf(dest, offset, "-daemon");
        }
        if (isBootThread()) {
            offset = VM_Services.sprintf(dest, offset, "-Boot");
        }
        if (isMainThread()) {
            offset = VM_Services.sprintf(dest, offset, "-main");
        }
        if (isIdleThread()) {
            offset = VM_Services.sprintf(dest, offset, "-idle");
        }
        if (isGCThread()) {
            offset = VM_Services.sprintf(dest, offset, "-collector");
        }
        if (beingDispatched) {
