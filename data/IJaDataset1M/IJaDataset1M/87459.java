...
public class REXPFactory {
    ...
    public REXPFactory(REXP r) throws REXPMismatchException {
        ...
        if (r instanceof REXPNull) {
            ...
            if (r instanceof REXPLanguage) type = (type == XT_LIST_TAG) ? XT_LANG_TAG : XT_LANG_NOTAG;
    ...
    public int parseREXP(byte[] buf, int o) throws REXPMismatchException {
        ...
        if (xt == XT_BOOL) {
            ...
            o++;
            if (o != eox) {
                if (eox != o + 3) System.err.println("Warning: bool SEXP size mismatch\n");
                o = eox;
            }
            return o;
        }
        ...
        if (xt == XT_LIST_NOTAG || xt == XT_LIST_TAG || xt == XT_LANG_NOTAG || xt == XT_LANG_TAG) {
            ...
            while (o < eox) {
                ...
                if (name == null) l.add(lc.cont); else l.put(name, lc.cont);
            }
            cont = (xt == XT_LANG_NOTAG || xt == XT_LANG_TAG) ? new REXPLanguage(l, getAttr()) : new REXPList(l, getAttr());
            if (o != eox) {
        ...
        if (xt == XT_LIST || xt == XT_LANG) {
            ...
            if (o < eox) {
                ...
                if (tagf.cont != null && (tagf.cont.isString() || tagf.cont.isSymbol())) rootList.setKeyAt(elIndex, tagf.cont.asString());
            }
            if (isRoot) {
        ...
        if (xt == XT_VECTOR || xt == XT_VECTOR_EXP) {
            ...
            if (getAttr() != null && getAttr().asList().at("names") != null) {
                ...
                String names[] = null;
                if (nam.isString()) names = nam.asStrings(); else if (nam.isVector()) {
                    RList l = nam.asList();
                    Object oa[] = l.toArray();
                ...
                cont = (xt == XT_VECTOR_EXP) ? new REXPExpressionVector(l, getAttr()) : new REXPGenericVector(l, getAttr());
            } else cont = (xt == XT_VECTOR_EXP) ? new REXPExpressionVector(new RList(v), getAttr()) : new REXPGenericVector(new RList(v), getAttr());
            return o;
        }
        if (xt == XT_ARRAY_STR) {
            int c = 0, i = o;
            ...
            if (c > 0) {
                ...
                while (o < eox) {
                    if (buf[o] == 0) {
                        try {
                            if (buf[i] == -1) {
                                if (buf[i + 1] == 0) s[c] = null; else s[c] = new String(buf, i + 1, o - i - 1, RConnection.transferCharset);
                            } else s[c] = new String(buf, i, o - i, RConnection.transferCharset);
        ...
        if (xt == XT_STR || xt == XT_SYMNAME) {
            ...
            try {
                if (xt == XT_STR) cont = new REXPString(new String[] { new String(buf, o, i - o, RConnection.transferCharset) }, getAttr()); else cont = new REXPSymbol(new String(buf, o, i - o, RConnection.transferCharset));
            } catch (Exception e) {
        ...
        if (xt == XT_SYM) {
            ...
            cont = new REXPSymbol(sym.getREXP().asString());
            o = eox;
            return o;
        }
        if (xt == XT_CLOS) {
    ...
    public int getBinaryLength() throws REXPMismatchException {
        int l = 0;
        int rxt = type;
        if (type == XT_LIST || type == XT_LIST_TAG || type == XT_LIST_NOTAG) rxt = (cont.asList() != null && cont.asList().isNamed()) ? XT_LIST_TAG : XT_LIST_NOTAG;
        if (type == XT_VECTOR_STR) rxt = XT_ARRAY_STR;
            ...
            case XT_ARRAY_BOOL:
                {
                    ...
                    io += 4;
                    if (ba.length > 0) {
                        for (int i = 0; i < ba.length; i++) buf[io++] = (byte) ((ba[i] == REXPLogical.NA) ? 2 : ((ba[i] == REXPLogical.FALSE) ? 0 : 1));
            ...
            case XT_VECTOR_EXP:
                {
                    ...
                    if (lst != null) {
                        ...
                        while (i < lst.size()) {
                            ...
                            if (rxt == XT_LIST_TAG || rxt == XT_LANG_TAG) io = new REXPFactory(new REXPSymbol(lst.keyAt(i))).getBinaryRepresentation(buf, io);
                            i++;
                        }
                    }
                    break;
