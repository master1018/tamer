...
public class ZipLoader extends ClassLoader {
   ...
   * The format of the archive is the same as classes.zip. */
    ...
    public Class loadClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        if (index >= 0) clas = (Class) loadedClasses.elementAt(index + 1); else if (zar == null && loadedClasses.size() == 2 * size) clas = Class.forName(name); else {
            ...
            if (member == null) {
                ...
                try {
                    ...
                    if (2 * size == loadedClasses.size()) close();
                } catch (java.io.IOException ex) {
                    throw new ClassNotFoundException("IOException while loading " + member_name + " from ziparchive \"" + name + "\": " + ex.toString());
                }
            }
    ...
    public void loadAllClasses() throws java.io.IOException {
        java.util.Enumeration e = this.zar.entries();
        while (e.hasMoreElements()) {
            java.util.zip.ZipEntry member = (java.util.zip.ZipEntry) e.nextElement();
