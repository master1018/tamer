...
public class BuildingDetailPanel extends ColopediaGameObjectTypePanel<BuildingType> {
    ...
    public void addSubTrees(DefaultMutableTreeNode root) {
        ...
        Map<BuildingType, DefaultMutableTreeNode> buildingHash = new HashMap<BuildingType, DefaultMutableTreeNode>();
    ...
    public void buildDetail(String id, JPanel panel) {
        ...
        if (buildingType.getGoodsRequired().isEmpty()) {
            ...
            if (buildingType.getGoodsRequired().size() > 1) {
                panel.add(getGoodsButton(goodsRequired.getType(), goodsRequired.getAmount()), "span, split " + buildingType.getGoodsRequired().size());
        ...
        if (!labels.isEmpty()) {
            panel.add(localizedLabel(StringTemplate.template("colopedia.buildings.modifiers").addAmount("%number%", labels.size())), "newline, top");
            int count = 0;
        ...
        panel.add(getDefaultTextArea(Messages.message(buildingType.getDescriptionKey()), 20), "span, growx");
    }
}
