...
public class FedoraObjectSearchResource extends BaseRestResource {
    ...
    static final String[] SEARCHABLE_FIELDS = { "pid", "label", "state", "ownerId", "cDate", "mDate", "dcmDate", "title", "creator", "subject", "description", "publisher", "contributor", "date", "type", "format", "identifier", "source", "language", "relation", "coverage", "rights" };

    @GET
    @Produces({ HTML, XML })
    public Response searchObjects(@QueryParam("terms") String terms, @QueryParam("query") String query, @QueryParam("maxResults") @DefaultValue("25") int maxResults, @QueryParam("sessionToken") String sessionToken, @QueryParam("resultFormat") @DefaultValue(HTML) String format) {
        try {
    ...
    @Path("nextPID")
    @POST
    public Response getNextPID(@QueryParam("numPIDs") @DefaultValue("1") int numPIDS, @QueryParam(RestParam.NAMESPACE) String namespace, @QueryParam("format") @DefaultValue(HTML) String format) throws Exception {
        try {
            Context context = getContext();
