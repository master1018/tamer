    ...
    protected Ecore2EcoreMappingRoot getMappingRoot(IStructuredSelection structuredSelection) {
        if (IFile.class.isInstance(structuredSelection.getFirstElement())) {
            ...
            if (ECORE2ECORE_FILE_EXTENSION.equals(file.getFullPath().getFileExtension())) {
                return (Ecore2EcoreMappingRoot) EcoreUtil.getObjectByType(new ResourceSetImpl().getResource(URI.createPlatformResourceURI(file.getFullPath().toString(), true), true).getContents(), Ecore2EcorePackage.Literals.ECORE2_ECORE_MAPPING_ROOT);
            }
        }
        return null;
    }
                    ...
                    public Object caseEStructuralFeature(EStructuralFeature eStructuralFeature) {
                        ...
                        if (input == null) {
                            ...
                            if (xmlRepresentation == XMLResource.XMLInfo.UNSPECIFIED) {
                                xmlRepresentation = eStructuralFeature instanceof EReference && ((EReference) eStructuralFeature).isContainment() ? XMLResource.XMLInfo.ELEMENT : XMLResource.XMLInfo.ATTRIBUTE;
                            }
