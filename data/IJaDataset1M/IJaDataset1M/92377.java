...
public class Chat extends QDialog {
    ...
    private void appendMessage(final String from, final String message) {
        if (from.equals("") || message.equals("")) return;
    ...
    void showInformation() {
        if (ui.listWidget.count() == 1) {
            QMessageBox.information(this, tr("Chat"), tr("Espera a que unan más personas al chat."));
        }
    }
    ...
    class Client extends QObject {
        ...
        protected void newConnection(QHostAddress address, int port) {
            if (inConnection.isEmpty()) {
                for (Connection conn : peers) {
                    if (conn.equals(address, port)) return;
                }
        ...
        protected synchronized void readyForUse() {
            ...
            if (!nick.equals("")) newParticipant__nick.emit(nick);
        ...
        protected void removeConnection(Connection con) {
            if (peers.removeElement(con)) {
                ...
                if (!nick.equals("")) participantLeft__nick.emit(nick);
            }
        }
    }
    ...
    static class Connection extends QTcpSocket {
        ...
        private int dataLengthForCurrentDataType() {
            if (bytesAvailable() <= 0 || readDataIntoBuffer(MaxBufferSize) <= 0 || !buffer.endsWith("" + SeparatorToken)) return 0;
            buffer.chop(1);
    ...
    class PeerManager extends QObject {
        ...
        public PeerManager(Server server) {
            ...
            for (String string : envVariables) {
                ...
                for (String entry : environment) {
                    if (new QRegExp(string).exactMatch(entry)) break;
                    index++;
                }
            ...
            updateAddresses();
            broadcastSocket.bind(new QHostAddress(QHostAddress.SpecialAddress.Any), broadcastPort, new QUdpSocket.BindMode(QUdpSocket.BindFlag.ShareAddress, QUdpSocket.BindFlag.ReuseAddressHint));
        ...
        void sendBroadcastDatagram() {
            ...
            for (QHostAddress address : broadcastAddresses) {
                if (broadcastSocket.writeDatagram(datagram, address, broadcastPort) == -1) validBroadcastAddresses = false;
            }
        ...
        void readBroadcastDatagram() {
            while (broadcastSocket.hasPendingDatagrams()) {
                ...
                if (broadcastSocket.readDatagram(datagram, info) == -1) continue;
        ...
        private void updateAddresses() {
            ...
            if (broadcastAddresses.isEmpty()) updateAddresses(true);
        }
