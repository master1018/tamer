...
class SceneFrame extends AbstractLibraryFrame {
    ...
    private static final FileFilter FILE_FILTER = new Actions.ExtensionFilter("PatchEdit Scene Files (*" + FILE_EXTENSION + ")", FILE_EXTENSION);
    ...
    void enableActions() {
        Actions.setEnabled(table.getRowCount() > 0, Actions.EN_SAVE | Actions.EN_SAVE_AS | Actions.EN_SEARCH | Actions.EN_TRANSFER_SCENE | Actions.EN_UPDATE_SCENE);
        Actions.setEnabled(table.getSelectedRowCount() > 0, Actions.EN_DELETE | Actions.EN_UPDATE_SELECTED);
        Actions.setEnabled(table.getSelectedRowCount() == 1, Actions.EN_COPY | Actions.EN_EDIT | Actions.EN_CUT | Actions.EN_EXPORT | Actions.EN_REASSIGN | Actions.EN_STORE | Actions.EN_UPLOAD);
    ...
    public ArrayList getPatchCollection() {
        ...
        for (int i = 0; i < myModel.getRowCount(); i++) ar.add(myModel.getPatchAt(i));
        return ar;
    }
    ...
    void updateScene(int[] indx) {
        ...
        final ProgressMonitor progressMonitor = new ProgressMonitor(table, "Update Scene", "", 0, task.getLengthOfTask());
    ...
    private class SceneListModel extends PatchTableModel {
            ...
            try {
                switch(col) {
                    ...
                    case BANK_NUM:
                        ...
                        if (bn != null) return bn[myScene.getBankNumber()]; else return String.valueOf(myScene.getBankNumber());
                    case PATCH_NUM:
                        String[] pn = myPatch.getDriver().getPatchNumbers();
                        if (pn != null) return pn[myScene.getPatchNumber()]; else return String.valueOf(myScene.getPatchNumber());
                    case COMMENT:
        ...
        public boolean isCellEditable(int row, int col) {
            return (col > PATCH_NAME && !((col == BANK_NUM || col == PATCH_NUM) && ((Scene) list.get(row)).getPatch().hasNullDriver()));
        }
        ...
        protected void selectEditor(MouseEvent e) {
            ...
            if ((row != oldrow) || (col != oldcol)) {
                ...
                if (patchNumbers.length > 1) {
                    ...
                    } else if (col == PATCH_NUM) for (int i = 0; i < patchNumbers.length; i++) {
