...
public abstract class AbstractPaymentController extends AbstractTransferController<PaymentData, PaymentID, PaymentResult> implements PaymentController {
    ...
    private static List<PaymentID> getPaymentIDs(List<PaymentData> transferDatas) {
        ...
        for (PaymentData data : transferDatas) paymentIDs.add(PaymentID.create(data.getPayment().getOrganisationID(), data.getPayment().getPaymentID()));
        return paymentIDs;
    }

    @Override
    protected void _serverBegin() {
        if (isSkipServerStages()) return;
        List<PaymentResult> payBeginServerResults = null;
        try {
            for (PaymentData paymentData : getTransferDatas()) paymentData.prepareUpload();
            try {
                payBeginServerResults = getAccountingManager().payBegin(getTransferDatasForServer());
            } finally {
                for (PaymentData paymentData : getTransferDatas()) paymentData.restoreAfterUpload();
            }
            if (payBeginServerResults.size() != getTransferDatas().size()) throw new IllegalStateException("accountingManager.payBegin(List) returned an invalid count of results! payBeginServerResults.size()=" + payBeginServerResults.size() + "; getTransferDatas().size()=" + getTransferDatas().size());
            ...
            for (PaymentData paymentData : getTransferDatas()) paymentData.getPayment().setPayBeginServerResult(x.getPaymentResult());
        } catch (Throwable t) {
            PaymentResult payBeginServerResult = new PaymentResult(SecurityReflector.getUserDescriptor().getOrganisationID(), t);
            for (PaymentData paymentData : getTransferDatas()) paymentData.getPayment().setPayBeginServerResult(payBeginServerResult);
        }
    }
    ...
    protected void _serverDoWork() {
        ...
        try {
            ...
            if (payDoWorkServerResults.size() != getTransferDatas().size()) throw new IllegalStateException("accountingManager.payDoWork(List, List, boolean) returned an invalid count of results! payDoWorkServerResults.size()=" + payDoWorkServerResults.size() + "; paymentIDs.size()=" + getTransferIDs().size() + "; getTransferDatas().size()=" + getTransferDatas().size());
            ...
            for (Iterator<PaymentData> itD = getTransferDatas().iterator(); itD.hasNext(); ) (itD.next()).getPayment().setPayDoWorkServerResult(x.getPaymentResult());
        } catch (Throwable t) {
            PaymentResult payDoWorkServerResult = new PaymentResult(SecurityReflector.getUserDescriptor().getOrganisationID(), t);
            for (PaymentData paymentData : getTransferDatas()) paymentData.getPayment().setPayDoWorkServerResult(payDoWorkServerResult);
        }
    }
    ...
    protected void _serverEnd() {
        ...
        try {
            ...
            if (payEndServerResults.size() != getTransferDatas().size()) throw new IllegalStateException("accountingManager.payEnd(List, List, boolean) returned an invalid count of results! payEndServerResults.size()=" + payEndServerResults.size() + "; paymentIDs.size()=" + getTransferIDs().size() + "; getTransferDatas().size()=" + getTransferDatas().size());
            ...
            for (PaymentData paymentData : getTransferDatas()) paymentData.getPayment().setPayEndServerResult(x.getPaymentResult());
        } catch (Throwable t) {
            PaymentResult payEndServerResult = new PaymentResult(SecurityReflector.getUserDescriptor().getOrganisationID(), t);
            for (PaymentData paymentData : getTransferDatas()) paymentData.getPayment().setPayEndServerResult(payEndServerResult);
        }
    }

    @Override
    public boolean isRollbackRequired() {
        if (isForceRollback()) return true;
        for (PaymentData paymentData : getTransferDatas()) {
            Payment payment = paymentData.getPayment();
            if (payment.isFailed() || payment.isForceRollback()) return true;
        }
        return false;
    }
