...
public class ShadowedRenderPass extends Pass {
    ...
    */
    public void setShadowColor(ColorRGBA shadowColor) {
        if (shadowColor == null) throw new IllegalArgumentException("shadowColor must not be null!");
    ...
    */
    ...
    protected void getShadowLights() {
        ...
        for (int x = occluders.size(); --x >= 0; ) getShadowLights(occluders.get(x));
    }
        ...
        if (s instanceof Geometry) {
            ...
            if (ls != null) {
                for (int q = ls.getQuantity(); --q >= 0; ) {
                    Light l = ls.get(q);
                    if (l.isShadowCaster() && (l.getType() == Light.Type.Directional || l.getType() == Light.Type.Point) && !shadowLights.contains(l)) {
                        shadowLights.add(l);
                    }
                }
            }
    ...
    protected void setupOccluderMeshes() {
        ...
        occluderMeshes.clear();
        for (int x = occluders.size(); --x >= 0; ) setupOccluderMeshes(occluders.get(x));
        meshes.keySet().retainAll(occluderMeshes);
    }

    protected void setupOccluderMeshes(Spatial spat) {
        if (spat instanceof TriMesh) addOccluderMeshes((TriMesh) spat); else if (spat instanceof Node) {
            Node node = (Node) spat;
    ...
    protected void addShadowVolumes(Light light) {
        if (enabled) {
            for (int i = occluderMeshes.size(); --i >= 0; ) {
                ...
                if (lv != null) volumeNode.getChildren().add(lv);
            }
        }
    }
