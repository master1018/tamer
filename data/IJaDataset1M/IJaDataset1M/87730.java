...
public class Locking implements IndirectHintProducer {
     ...
     */
    private <S extends Grid.Region, T extends Grid.Region> void getHints(Grid grid, Class<S> regionType1, Class<T> regionType2, HintsAccumulator accu) throws InterruptedException {
                        ...
                        if (potentialPositions.cardinality() > 1) {
                            for (int i = 0; i < 9; i++) {
                                if (potentialPositions.get(i)) {
                                    ...
                                    if (!region2Cells.contains(cell)) isInCommonSet = false;
                                }
                            }
                            if (isInCommonSet) {
                                if (isDirectMode) {
                                    lookForFollowingHiddenSingles(grid, regionType1, accu, i1, region1, region2, value);
                                } else {
    ...
    private <S extends Grid.Region> void lookForFollowingHiddenSingles(Grid grid, Class<S> regionType1, HintsAccumulator accu, int i1, Grid.Region region1, Grid.Region region2, int value) throws InterruptedException {
    ...
    private IndirectHint createLockingHint(Grid.Region p1, Grid.Region p2, Cell hcell, int value) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (cell.hasPotentialValue(value)) cellPotentials.put(cell, SingletonBitSet.create(value));
        }
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (!p1Cells.contains(cell)) {
                if (cell.hasPotentialValue(value)) cellRemovablePotentials.put(cell, SingletonBitSet.create(value));
            } else if (cell.hasPotentialValue(value)) highlightedCells.add(cell);
        }
        Cell[] cells = new Cell[highlightedCells.size()];
        highlightedCells.toArray(cells);
        if (isDirectMode) return new DirectLockingHint(this, cells, hcell, value, cellPotentials, cellRemovablePotentials, p1, p2); else return new LockingHint(this, cells, value, cellPotentials, cellRemovablePotentials, p1, p2);
    }

    @Override
    public String toString() {
        if (isDirectMode) return "Direct Intersections"; else return "Intersections";
    }
}
