...
public class RadialTreeLayout extends AbstractTreeLayout {
    ...
    private Point2D constructNode(RootedTree tree, Node node, double angleStart, double angleFinish, double xPosition, double yPosition, double length, TreeLayoutCache cache) throws Graph.NoEdgeException {
        ...
        if (!tree.isExternal(node)) {
            ...
            for (i = 0; i < children.size(); ++i) {
                ...
                Point2D childPoint = constructNode(tree, child, a1, a2, nodePoint.getX(), nodePoint.getY(), childLength, cache);
            ...
            Point2D nodeLabelPoint = new Point2D.Double(xPosition + ((length + 1.0) * directionX), yPosition + ((length + 1.0) * directionY));
            Line2D nodeLabelPath = new Line2D.Double(nodePoint, nodeLabelPoint);
            cache.nodeLabelPaths.put(node, nodeLabelPath);
            Point2D nodeBarPoint = new Point2D.Double(xPosition + ((length - 1.0) * directionX), yPosition + ((length - 1.0) * directionY));
            Line2D nodeBarPath = new Line2D.Double(nodePoint, nodeBarPoint);
            cache.nodeBarPaths.put(node, nodeBarPath);
        } else {
            Point2D taxonPoint = new Point2D.Double(xPosition + ((length + 1.0) * directionX), yPosition + ((length + 1.0) * directionY));
    ...
    private void constructHilight(RootedTree tree, Node node, double angleStart, double angleFinish, double xPosition, double yPosition, double length, TreeLayoutCache cache) {
