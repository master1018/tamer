...
class BrowseMode extends Mode {
    ...
    enum Menu {
        ...
        ATTRIBUTE, ATTROP, DOMAIN, MBEAN, NONE, OPERATION, SERVER
    }
    ...
    void accessAttribute(MBeanAttributeInfo attr) {
        ...
        if (attr.isWritable()) {
            ...
            if (newValue != null && newValue.length() > 0) {
                Jmx.getInstance().setAttribute(context.server, context.mbean, attr.getName(), TclString.newInstance(newValue));
    ...
    private String getAttropMenu() {
        ...
        for (MBeanAttributeInfo attr : attrArray) {
            if (this.pattern != null && !PERL5_MATCHER.matches(attr.getName(), this.pattern)) continue;
        ...
        if (operArray.length > 0) msb.append("\n Operation List:\n\n");
        for (MBeanOperationInfo op : operArray) {
            if (this.pattern != null && !PERL5_MATCHER.matches(op.getName(), this.pattern)) continue;
