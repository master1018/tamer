...
public class StandardGroundVehicleMaintenance implements GroundVehicleMaintenance, Serializable {
    ...
    public void addVehicle(Vehicle vehicle) throws BuildingException {
        if (vehicles.contains(vehicle)) throw new BuildingException("Building already contains vehicle.");
        if (!(vehicle instanceof GroundVehicle)) throw new BuildingException("Vehicle is not ground vehicle.");
        if (vehicle.getMass() > (getVehicleCapacity() - getCurrentVehicleMass())) throw new BuildingException("Vehicle exceeds total capacity of building");
    ...
    public void removeVehicle(Vehicle vehicle) throws BuildingException {
        if (!containsVehicle(vehicle)) throw new BuildingException("Vehicle not in building."); else vehicles.remove(vehicle);
    }
