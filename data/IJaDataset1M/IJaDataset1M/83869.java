...
public class Utils {
    ...
    public String GetLongTime(Calendar date, boolean b24HourMode) {
        ...
        if (b24HourMode) {
            ...
            s = String.format("%tk:%tM", date, date);
            if (date.get(Calendar.AM_PM) == 0) s = String.format("%tl:%tM am", date, date, date.get(Calendar.AM_PM));
            if (date.get(Calendar.AM_PM) == 1) s = String.format("%tl:%tM pm", date, date, date.get(Calendar.AM_PM));
        }
        return s;
    }
    ...
    public static boolean IsTimeOverdued(Calendar calDate, Calendar calDueDate) {
        if ((calDueDate.compareTo(calDate) == 0) || (calDueDate.compareTo(calDate) == 1)) return true;
        return false;
    }

    public static boolean IsInTimeRange(Calendar calDateStart, Calendar calDate, int iDurationInMinutes) {
        if (calDate.get(Calendar.HOUR_OF_DAY) == calDateStart.get(Calendar.HOUR_OF_DAY)) if (calDate.get(Calendar.MINUTE) >= calDateStart.get(Calendar.MINUTE)) if (calDate.get(Calendar.MINUTE) <= (calDateStart.get(Calendar.MINUTE) + iDurationInMinutes)) return true;
        return false;
    }
