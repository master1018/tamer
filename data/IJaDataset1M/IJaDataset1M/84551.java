...
public class ContactsSyncAdapterTest extends ProviderTestCase<ContactsProvider> {
    ...
    private static void addGroupMembership(ContactEntry p, Account account, String groupId) {
        ...
        final String serverId = ContactsSyncAdapter.getCanonicalGroupsFeedForAccount(account) + "/" + groupId;
    ...
    public void testUpdateProviderContactMethods() throws ParseException {
        ...
        Cursor cursor;
        cursor = provider.query(Contacts.ContactMethods.CONTENT_URI, null, null, null, Contacts.ContactMethods.DATA);
        try {
            checkNextEmail(cursor, personId, "a11111", false, Contacts.ContactMethods.TYPE_HOME, null);
            checkNextEmail(cursor, personId, "a22222", true, Contacts.ContactMethods.TYPE_WORK, null);
            checkNextEmail(cursor, personId, "a33333", false, Contacts.ContactMethods.TYPE_OTHER, null);
            checkNextEmail(cursor, personId, "a44444", false, Contacts.ContactMethods.TYPE_CUSTOM, "lucky");
            checkNextPostal(cursor, personId, "b11111", false, Contacts.ContactMethods.TYPE_HOME, null);
            checkNextPostal(cursor, personId, "b22222", false, Contacts.ContactMethods.TYPE_WORK, null);
            checkNextPostal(cursor, personId, "b33333", true, Contacts.ContactMethods.TYPE_OTHER, null);
            checkNextPostal(cursor, personId, "b44444", false, Contacts.ContactMethods.TYPE_CUSTOM, "lucky");
            checkNextIm(cursor, personId, "c11111", ContactMethods.encodeCustomImProtocol("p1"), false, Contacts.ContactMethods.TYPE_HOME, null);
            checkNextIm(cursor, personId, "c22222", null, true, Contacts.ContactMethods.TYPE_WORK, null);
            checkNextIm(cursor, personId, "c33333", ContactMethods.encodePredefinedImProtocol(ContactMethods.PROTOCOL_SKYPE), false, Contacts.ContactMethods.TYPE_OTHER, null);
            checkNextIm(cursor, personId, "c44444", ContactMethods.encodePredefinedImProtocol(ContactMethods.PROTOCOL_ICQ), false, Contacts.ContactMethods.TYPE_CUSTOM, "l2");
            assertTrue(dumpCursor(cursor), cursor.isLast());
        } finally {
            cursor.close();
        }
    ...
    public void testUpdateProviderPhones() throws ParseException {
        ...
        long personId = getSinglePersonId(provider, entryTitle);
        Cursor cursor;
        cursor = provider.query(Contacts.Phones.CONTENT_URI, null, null, null, Contacts.Phones.NUMBER);
        try {
    ...
    public void testUpdateProviderOrganization() throws ParseException {
        ...
        long personId = getSinglePersonId(provider, entryTitle);
        Cursor cursor;
        cursor = provider.query(Contacts.Organizations.CONTENT_URI, null, null, null, Contacts.Organizations.COMPANY);
        try {
            checkNextOrganization(cursor, personId, "11111", "title1", true, Contacts.Organizations.TYPE_WORK, null);
            checkNextOrganization(cursor, personId, "22222", "title2", false, Contacts.Organizations.TYPE_OTHER, null);
            checkNextOrganization(cursor, personId, "33333", "title3", false, Contacts.Organizations.TYPE_CUSTOM, "label1");
            assertTrue(dumpCursor(cursor), cursor.isLast());
        } finally {
            cursor.close();
        }
    ...
    public void testUpdateProviderExtensions() throws ParseException {
        ...
        long personId = getSinglePersonId(provider, entryTitle);
        Cursor cursor;
        cursor = provider.query(Contacts.Extensions.CONTENT_URI, null, null, null, Contacts.Extensions.NAME);
        try {
    ...
    public void testPhotoAccess() throws IOException, InterruptedException {
        ...
        Cursor cursor = getProvider().getDatabase().query(ContactsProvider.sPhotosTable, null, null, null, null, null, null);
        try {
        ...
        getMockContentResolver().update(ph, values, null, null);
        cursor = getProvider().getDatabase().query(ContactsProvider.sPhotosTable, null, null, null, null, null, null);
        try {
        ...
        InputStream inputStream = Contacts.People.openContactPhotoInputStream(getMockContentResolver(), p);
        byte[] inputBytes = new byte[100];
        int totalBytesRead = 0;
        while (true) {
            int numBytesRead = inputStream.read(inputBytes, totalBytesRead, inputBytes.length - totalBytesRead);
    ...
    private void checkNextPhoto(Cursor cursor, boolean expectedDirty, String expectedLocalVersion, String expectedServerVersion) {
    ...
    private Uri addPerson(ContactsProvider provider, Account account, String name, String syncId) {
        ContentValues values = new ContentValues();
    ...
    private static void checkNextGroup(Cursor cursor, GroupEntry entry, Long syncLocalId) {
        assertTrue(cursor.moveToNext());
        assertEquals(dumpRow(cursor), entry.getId(), feedFromEntry(entry) + "/" + getString(cursor, SyncConstValue._SYNC_ID));
        assertEquals(dumpRow(cursor), entry.getEditUri(), entry.getId() + "/" + getString(cursor, SyncConstValue._SYNC_VERSION));
        assertEquals(dumpRow(cursor), ACCOUNT.name, getString(cursor, SyncConstValue._SYNC_ACCOUNT));
        assertEquals(dumpRow(cursor), ACCOUNT.type, getString(cursor, SyncConstValue._SYNC_ACCOUNT_TYPE));
        ...
        if (syncLocalId != null) {
            assertEquals(dumpRow(cursor), syncLocalId, getString(cursor, SyncConstValue._SYNC_LOCAL_ID));
        }
    }
    ...
    private static ContactEntry newPerson(String title, String notes, String id, String version) {
        ...
        entry.setEditUri(entry.getId() + "/" + version);
        entry.setLinkEditPhoto(ContactsSyncAdapter.getContactsFeedForAccount(ACCOUNT) + "/" + id + "/v1", "image/jpg");
        return entry;
    }
    ...
    private void checkNextCursorToEntry(Cursor cursor, Entry expectedEntry) throws ParseException {
        Entry newEntry = newEmptyEntry(expectedEntry);
        assertTrue(cursor.moveToNext());
        String createUri = ContactsSyncAdapter.cursorToEntryImpl(getMockContentResolver(), cursor, newEntry, ACCOUNT);
        assertNull(createUri);
        assertEquals(expectedEntry.getEditUri(), newEntry.getEditUri());
        if (expectedEntry instanceof ContactEntry) {
            ((ContactEntry) newEntry).setLinkEditPhoto(((ContactEntry) expectedEntry).getLinkEditPhotoHref(), ((ContactEntry) expectedEntry).getLinkEditPhotoType());
        }
    ...
    private static void checkNextDeletedCursorToEntry(Cursor cursor, Entry expectedEntry) throws ParseException {
        Entry newEntry = newEmptyEntry(expectedEntry);
        assertTrue(cursor.moveToNext());
    ...
    private static void checkNextPerson(ContactsProvider provider, Cursor cursor, ContactEntry entry, Long syncLocalId) {
        assertTrue(cursor.moveToNext());
        assertEquals(dumpRow(cursor), entry.getId(), feedFromEntry(entry) + "/" + getString(cursor, SyncConstValue._SYNC_ID));
        assertEquals(dumpRow(cursor), entry.getEditUri(), entry.getId() + "/" + getString(cursor, SyncConstValue._SYNC_VERSION));
        assertEquals(dumpRow(cursor), ACCOUNT.name, getString(cursor, SyncConstValue._SYNC_ACCOUNT));
        ...
        Cursor groupCursor = provider.getDatabase().query(ContactsProvider.sGroupmembershipTable, null, Contacts.GroupMembership.PERSON_ID + "=?", new String[] { getString(cursor, People._ID) }, null, null, null);
        try {
            for (Object object : entry.getGroups()) {
                ...
                assertEquals(groupMembership.getGroup(), ContactsSyncAdapter.getCanonicalGroupsFeedForAccount(ACCOUNT) + "/" + getString(groupCursor, GroupMembership.GROUP_SYNC_ID));
            }
    ...
    private static void checkNextDeleted(Cursor cursor, Entry entry, Long syncLocalId) {
        ...
        assertEquals(dumpRow(cursor), ACCOUNT.type, getString(cursor, SyncConstValue._SYNC_ACCOUNT_TYPE));
        if (syncLocalId != null) {
            assertEquals(dumpRow(cursor), syncLocalId.toString(), getString(cursor, SyncConstValue._SYNC_LOCAL_ID));
        }
    }
    ...
    private void checkCursorToEntry(ContactsProvider provider, ContactEntry entry) throws ParseException {
        copyTempProviderToReal(provider);
        final ContentProvider clientDiffsProvider = getClientDiffs();
        Cursor cursor = ContactsSyncAdapter.getCursorForTableImpl(clientDiffsProvider, entry.getClass());
        try {
            assertTrue(cursor.moveToFirst());
            assertEquals(dumpCursor(cursor), 1, cursor.getCount());
            ContactEntry newEntry = new ContactEntry();
            String editUrl = ContactsSyncAdapter.cursorToEntryImpl(getMockContentResolver(), cursor, newEntry, ACCOUNT);
            entry.setId(null);
    ...
    private static void checkNextNumber(Cursor cursor, long personId, String number, boolean isPrimary, int type, String label) {
        assertTrue(cursor.moveToNext());
        ...
        assertEquals(dumpCursor(cursor), isPrimary, getLong(cursor, Contacts.Phones.ISPRIMARY) != 0);
    ...
    private static void checkNextContactMethod(Cursor cursor, long personId, String data, String auxData, int kind, boolean isPrimary, int type, String label) {
        assertTrue(cursor.moveToNext());
        ...
        assertEquals(dumpCursor(cursor), isPrimary, getLong(cursor, Contacts.ContactMethods.ISPRIMARY) != 0);
    ...
    private static void addPhoneNumber(ContactEntry entry, String number, boolean isPrimary, byte rel, String label) {
    ...
    private static void addPostal(ContactEntry entry, String address, boolean isPrimary, byte rel, String label) {
    ...
    private static void checkNextPostal(Cursor cursor, long personId, String address, boolean isPrimary, int type, String label) {
        checkNextContactMethod(cursor, personId, address, null, Contacts.KIND_POSTAL, isPrimary, type, label);
    }

    private static void addIm(ContactEntry entry, String address, byte protocol, String protocolString, boolean isPrimary, byte rel, String label) {
    ...
    private static void checkNextIm(Cursor cursor, long personId, String address, String auxData, boolean isPrimary, int type, String label) {
        checkNextContactMethod(cursor, personId, address, auxData, Contacts.KIND_IM, isPrimary, type, label);
    }

    private static void addEmail(ContactEntry entry, String address, boolean isPrimary, byte rel, String label) {
    ...
    private static void checkNextEmail(Cursor cursor, long personId, String address, boolean isPrimary, int type, String label) {
        checkNextContactMethod(cursor, personId, address, null, Contacts.KIND_EMAIL, isPrimary, type, label);
    }
    ...
    private static void addOrganization(ContactEntry entry, String name, String title, boolean isPrimary, byte type, String label) {
    ...
    private static void addExtendedProperty(ContactEntry entry, String name, String value, String blob) {
    ...
    private static void checkNextOrganization(Cursor cursor, long personId, String company, String title, boolean isPrimary, int type, String label) {
        assertTrue(cursor.moveToNext());
        ...
        assertEquals(dumpRow(cursor), isPrimary, getLong(cursor, Contacts.Organizations.ISPRIMARY) != 0);
        assertEquals(dumpRow(cursor), type, getLong(cursor, Contacts.Phones.TYPE));
        assertEquals(dumpRow(cursor), label, getString(cursor, Contacts.Phones.LABEL));
    }

    private static void checkNextExtension(Cursor cursor, long personId, String name, String value) {
        assertTrue(cursor.moveToNext());
    ...
    void checkEntries(ContactsProvider provider, boolean deleted, Object... entriesAndLocalSyncIds) {
        String table;
