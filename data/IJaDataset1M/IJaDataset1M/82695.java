...
public class KMLCoordinates {
    ...
    public KMLCoordinates(double[] x, double[] y, double z) {
        this.x = x;
        this.y = y;
        if (x.length != y.length) throw new RuntimeException("Cannot create coordinate system with unbalanced entries");
    ...
    public KMLCoordinates(double[] x, double[] y, double[] z) {
        ...
        this.y = y;
        this.z = z;
        if (x.length != y.length && x.length != z.length) throw new RuntimeException("Cannot create coordinate system with unbalanced entries");
        length = x.length;
    }
    ...
    public static XMLObjectParser COORDINATESPARSER = new AbstractXMLObjectParser() {
        ...
        public Object parseXMLObject(XMLObject xo) throws XMLParseException {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (st2.countTokens() != 3) throw new XMLParseException("All KML coordinates must contain (X,Y,Z) values.  Three dimensions not found in element '" + line + "'");
