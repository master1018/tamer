...
public class FileSearch {
    ...
    private void iteratorDir(FileObject[] files, List result) {
        for (FileObject file : files) {
            if (file.isFolder()) {
                if (!file.getName().equals("build")) iteratorDir(file.getChildren(), result);
            } else {
                if (file.canWrite()) {
                    if (!isMatch(file.getNameExt())) result.add(new FileObjectQuickListItem(file));
                }
            }
        }
    }
