...
public class SHA1 {
    ...
    public String generateHash(String string) {
        ...
        toHashString(string);
        generateHash();
        return getGeneratedHash();
    }

    /**
	 * This function transform the given string to separated chars and 
	 * then transform this chars to byte
	 */
    private void toHashString(String input) {
        int i, len;
        byte x;
        len = input.length();
        for (i = 0; i < len; i++) {
    ...
    /**
	 * 
	 * This method must first invoke. Here everything needed is initialized
	 */
    private void init() {
        state[0] = 0x67452301;
        state[1] = 0xEFCDAB89;
