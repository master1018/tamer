...
public class BaseRuleGrammar extends BaseGrammar implements RuleGrammar, Serializable {
    ...
    public void setEnabled(String[] ruleNames, boolean enabled) throws IllegalArgumentException {
        for (int i = 0; i < ruleNames.length; i++) {
            ...
            if (r == null) {
                throw new IllegalArgumentException("Unknown Rule: " + ruleNames[i]);
            } else if (r.isEnabled != enabled) {
                r.isEnabled = enabled;
            }
        }
    ...
    public RuleName resolve(RuleName name) throws GrammarException {
        ...
        for (int i = 0; i < imports.length; i++) {
            ...
            if (iFullGrammarName == null) throw new GrammarException("Error: badly formed import " + imports[i], null);
            RuleGrammar gref = myRec.getRuleGrammar(iFullGrammarName);
            if (gref == null) {
                RecognizerUtilities.debugMessageOut("Warning: import of unknown grammar " + imports[i] + " in " + getName());
                continue;
            }
            if (!iSimpleName.equals("*") && gref.getRule(iSimpleName) == null) {
                RecognizerUtilities.debugMessageOut("Warning: import of undefined rule " + imports[i] + " in " + getName());
                continue;
            }
            if (iFullGrammarName.equals(fullGrammarName) || iGrammarName.equals(fullGrammarName)) {
                if (iSimpleName.equals("*")) {
                    if (gref.getRule(simpleName) != null) {
                        matches.addElement(new RuleName(iFullGrammarName + "." + simpleName));
                    }
                    continue;
                } else {
        ...
        if (matches.size() == 0) {
            ...
            b.append("Warning: ambiguous reference " + rn + " in " + getName() + " to ");
    ...
    protected void resolveAllRules() throws GrammarException {
        ...
        if (imports != null) {
            for (int i = 0; i < imports.length; i++) {
                ...
                if (GI == null) {
                    b.append("Undefined grammar " + gname + " imported in " + getName() + "\n");
                }
            }
        }
        if (b.length() > 0) {
    ...
    protected void resolveRule(Rule r) throws GrammarException {
        ...
        if (r instanceof BaseRuleName) {
            ...
            if (resolved == null) {
                throw new GrammarException("Unresolvable rulename in grammar " + getName() + ": " + rn.toString(), null);
            } else {
