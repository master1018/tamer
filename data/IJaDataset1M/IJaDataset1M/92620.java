...
public class Label2Id {
    ...
    public void loadLabelAndIdFromFile(File parentDir, String filename) {
        ...
        if (file1.exists()) {
            try {
                BufferedReader in = new BomStrippingInputStreamReader(new FileInputStream(new File(parentDir, filename)), "UTF-8");
                String line;
                while ((line = in.readLine()) != null) {
                    line.trim();
                    int p = line.indexOf(' ');
                    label2Id.put(line.substring(0, p).trim(), line.substring(p + 1).trim());
                    id2Label.put(line.substring(p + 1).trim(), line.substring(0, p).trim());
                }
                in.close();
            } catch (IOException e) {
            }
        } else {
            if (LogService.minVerbosityLevel > 0) System.out.println("No label list file in initialisation phrase.");
        }
    }
    ...
    public void writeLabelAndIdToFile(File parentDir, String filename) {
        try {
            PrintWriter out = new PrintWriter(new OutputStreamWriter(new FileOutputStream(new File(parentDir, filename)), "UTF-8"));
        ...
        for (int i = 0; i < className.length; ++i) {
            if (className[i] instanceof String) {
                ...
                for (int j = 0; j < items.length; ++j) {
                    if (items[j].endsWith(ConstantParameters.SUFFIXSTARTTOKEN)) items[j] = items[j].substring(0, items[j].lastIndexOf(ConstantParameters.SUFFIXSTARTTOKEN));
