...
public class ConsultaTramitacionAction extends BaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (formularioConsulta.getNivelAutenticacion() == Constants.MODO_AUTENTICACION_ANONIMO) {
            ...
            log.debug("Fecha: " + formularioConsulta.getFecha() + " HoraInicial: " + formularioConsulta.getHoraInicial() + " HoraFinal: " + formularioConsulta.getHoraFinal());
            SimpleDateFormat df = new SimpleDateFormat(Constants.FORMATO_FECHAS);
            String fecha = formularioConsulta.getFecha();
            fechaInicial = df.parse(fecha + " " + formularioConsulta.getHoraInicial() + ":" + formularioConsulta.getMinInicial());
            fechaFinal = df.parse(fecha + " " + formularioConsulta.getHoraFinal() + ":" + formularioConsulta.getMinFinal());
        }
        List logs = delegate.obtenerAuditoria(fechaInicial, fechaFinal, formularioConsulta.getNivelAutenticacion(), autenticacion);
