...
public class ScrollLayer implements ComponentListener {
    ...
    public void draggingHorizontalSlider(int dx) {
        if (hslider.vx + dx + hsliderRS.getWidth() < rightBt.vx - rightBtRS.getWidth() && hslider.vx + dx - hsliderRS.getWidth() > leftBt.vx + leftBtRS.getWidth()) {
            hslider.move(dx, 0);
    ...
    public void draggingVerticalSlider(int dy) {
        if (vslider.vy + dy + vsliderRS.getHeight() < upBt.vy - upBtRS.getHeight() && vslider.vy + dy - vsliderRS.getHeight() > downBt.vy + downBtRS.getHeight()) {
            vslider.move(0, dy);
    ...
    public void fade(AnimationManager am, int duration, float alphaOffset) throws ClassCastException {
        ...
        final Glyph[] wids = { vgutter, vslider, upBt, downBt, hgutter, hslider, rightBt, leftBt };
        for (Glyph g : wids) {
            Animation anim = fac.createTranslucencyAnim(duration, (Translucent) g, alphaOffset, true, IdentityInterpolator.getInstance(), null);
