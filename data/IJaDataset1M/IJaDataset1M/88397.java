 ...
 */
public final class JavaFunctionType extends JavaTypeImpl implements FunctionType, JavaGenericDeclaration {
    ...
    public JavaFunctionType(JavaFunctionType other) {
        ...
        for (final JavaFunctionArgument otherArg : other.arguments) {
            JavaFunctionArgument newArg = newArgument(otherArg.getName().getIdentifier(), otherArg.getType());
    ...
    private JavaFunctionType lookupParameterization(Map<JavaTypeVariable, JavaReferenceType> typeVarMap) {
        return paramCache != null ? paramCache.get(typeVarMap) : null;
    }

    private void cacheParameterization(Map<JavaTypeVariable, JavaReferenceType> typeVarMap, JavaFunctionType type) {
        if (paramCache == null) {
    ...
    private JavaFunctionType createParameterization(Map<JavaTypeVariable, JavaReferenceType> typeVarMap) {
        final JavaFunctionType result = new JavaFunctionType(resolveType(returnType, typeVarMap));
        for (final JavaFunctionArgument arg : arguments) {
            result.addArgument(new JavaFunctionArgument(arg.getName().getIdentifier(), resolveType(arg.getType(), typeVarMap)));
        }
    ...
    public JavaFunctionType parameterize(Map<JavaTypeVariable, JavaReferenceType> typeVarMap) {
        if (!hasTypeVariables()) {
            throw new UnsupportedOperationException("Not a generic type");
        }
    ...
    void printArguments(StringBuffer buf, boolean internalName) {
        ...
        for (final JavaFunctionArgument arg : arguments) {
            ...
            buf.append(internalName ? argType.toInternalName() : argType.toReferenceString());
            first = true;
        }
        buf.append(')');
    }
