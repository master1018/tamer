...
public class NameNodeDataTest extends AbstractIndexes {
    ...
    public void testGetRandomDataNode() {
        ...
        assertTrue(matches(getStatus()[0], name) || matches(getStatus()[1], name) || matches(getStatus()[2], name) || matches(getStatus()[3], name));
    }

    public void testAddDataNode() {
        IndexVersion version = new IndexVersion(NN_INDEX_ONE);
        IndexLocation ilA = new IndexLocation(getStatus()[0].getAddress(), version, IndexState.LIVE);
        IndexLocation ilB = new IndexLocation(getStatus()[0].getAddress(), version.nextVersion(), IndexState.LIVE);
    ...
    public void testCreateReplicationPlan() throws Exception {
        ...
        for (Map.Entry<InetSocketAddress, IndexLocation[]> entry : plan.entrySet()) {
            ...
            assertTrue(replicationPlan.contains(getSetup()[0][0]) || replicationPlan.contains(getSetup()[0][1]));
        }
    }
