...
public class VerticalText {
    ...
    protected void shortenChunkArray() {
        if (currentChunkMarker < 0) return;
        ...
        for (int j = currentChunkMarker - 1; j >= 0; --j) chunks.remove(j);
    }
    ...
    public int go(boolean simulate) {
        ...
        } else if (!simulate) throw new NullPointerException(MessageLocalization.getComposedMessage("verticaltext.go.with.simulate.eq.eq.false.and.text.eq.eq.null"));
        int status = 0;
        for (; ; ) {
            if (maxLines <= 0) {
                status = NO_MORE_COLUMN;
                if (chunks.isEmpty()) status |= NO_MORE_TEXT;
                break;
            }
            if (chunks.isEmpty()) {
                status = NO_MORE_TEXT;
