...
public abstract class Controller extends DomainsWs2 {
    ...
    protected SearchFilterWs2 getSearchFilter() {
        if (searchFilter == null) searchFilter = getDefaultSearchFilter();
        return searchFilter;
    }
    ...
    public IncludesWs2 getIncludes() {
        if (includes == null) includes = getDefaultIncludes();
        return includes;
    }
   ...
   */
    ...
    protected boolean needsLookUp(IncludesWs2 inc) {
        return (inc.isArtistRelations() || inc.isLabelRelations() || inc.isReleaseRelations() || inc.isReleaseGroupRelations() || inc.isRecordingRelations() || inc.isWorkRelations() || inc.isUrlRelations() || inc.isTags() || inc.isRatings() || inc.isUserTags() || inc.isUserRatings());
    }
    ...
    private void updateRelations(EntityWs2 entity, EntityWs2 transit, IncludesWs2 inc) throws MBWS2Exception {
        for (RelationWs2 rel : transit.getRelationList().getRelations()) {
            ...
            if (inc.isWorkRelations() && rel.getTargetType().equals(RelationWs2.TO_WORK)) {
                ...
                if (wkWs2.getArtistCredit() != null || !wkWs2.getWritersString().isEmpty()) continue;
                Work work = new Work();
                ...
                rel.setTarget(wkWs2);
            } else if (getEntity() instanceof WorkWs2 && inc.isRecordingRelations() && rel.getTargetType().equals(RelationWs2.TO_RECORDING)) {
            ...
            } else if (getEntity() instanceof WorkWs2 && inc.isReleaseGroupRelations() && rel.getTargetType().equals(RelationWs2.TO_RELEASE_GROUP)) {
    ...
    public WebService getQueryWs() {
        if (queryWs == null) queryWs = getDefaultQueryWs();
        return queryWs;
    }
    ...
    public WebService getAnnotationWs() {
        if (annotationWs == null) annotationWs = getDefaultAnnotationWs();
        return annotationWs;
    }
