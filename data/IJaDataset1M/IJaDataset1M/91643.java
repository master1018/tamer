...
public class EventsWebAPI implements ViewTool {
    ...
    public Recurance getEventRecurance(Event ev) {
        if (ev != null) return (Recurance) InodeFactory.getChildOfClass(ev, Recurance.class); else return null;
    }
    ...
    public File getEventImage(Event event) {
        ...
        if (event.getInode() > 0) {
            ...
            while (it.hasNext()) {
                ...
                if (UtilMethods.isImage(file.getFileName())) return file;
            }
        }
        file = new File();
        return file;
    ...
    public String getEventsCategoriesList(String inode) throws DotDataException, DotSecurityException {
        User user = null;
        try {
            if (request.getSession() != null) user = (com.liferay.portal.model.User) request.getSession().getAttribute(com.dotmarketing.util.WebKeys.CMS_USER);
    ...
    public String getEventsCategoriesListByEntityName(String entityName, String inode) throws DotDataException, DotSecurityException {
        ...
        int div = 1;
        if (totalItems % 2 != 0) div = ((int) ((double) totalItems / (double) 2)) + 1; else div = (totalItems / 2);
        int i = 0;
        ...
        for (; parentCatsIter.hasNext() && (i < div); ) {
            ...
            for (Category tempCat : tempSet) {
                ...
                while (catsIter.hasNext() && (i < div)) {
                    ...
                    if ((inode != null) && (-1 < inode.indexOf("" + cat.getInode()))) retList.append("<input type=\"checkbox\" id=\"categoryId\" name=\"categoryId\" value=\"" + cat.getInode() + "\" checked />&nbsp;" + cat.getCategoryName() + " <br/>"); else retList.append("<input type=\"checkbox\" id=\"categoryId\" name=\"categoryId\" value=\"" + cat.getInode() + "\" />&nbsp;" + cat.getCategoryName() + " <br/>");
                }
            }
        }
        retList.append("</div>");
        retList.append("<div style=\"float:left; \">");
        if (catTitle != null) retList.append("<b>" + catTitle + "</b><br/>");
        if (catsIter != null) {
            while (catsIter.hasNext()) {
                ++i;
                cat = (Category) catsIter.next();
        ...
        if (cats != null) {
            ...
            while (st.hasMoreTokens()) {
                try {
                    ...
                    cat.setInode(Long.parseLong(st.nextToken()));
                    parents.add(cat);
                } catch (Exception e) {
                }
            }
    ...
    public String getMonthsMenu(String fromYear, String month, String toYear) {
        ...
        try {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                calendarMenu = calendarMenu + "\n<li class=\"" + tabClass + "\"><a href=\"javascript:doChangeMonth(" + fromYear + ", " + displayMonth + ", " + toYear + ")\">" + getMenuMonth(displayMonth) + "</a></li>";
    ...
    public List getEventsInCalendar(String yearStr, String monthStr, String catId) {
        ...
        for (int i = 1; i <= totalDays; i++) {
            ...
            while (evIterator.hasNext()) {
                ...
                if (cal.getTime().compareTo(evStartDate.getTime()) >= 0 && cal.getTime().compareTo(evEndDate.getTime()) <= 0) weekEvents.add(ev);
            }
            prevcol = col;
        }
        if (row == 4) {
    ...
    public List getEventsByDateRange(Date from, Date to) {
        return EventFactory.getEventsByDateRange(from, to);
    }
