...
public class TextHover extends DefaultTextHover implements ITextHoverExtension {
    ...
    public String getHoverInfo(ITextViewer textViewer, IRegion hoverRegion) {
        ...
        if (hover != null) return html(hover);
        ...
        CompositeMap cursorMap = QuickAssistUtil.findMap(map, doc, hoverRegion.getOffset());
        ...
        if (hoverRegion.equals(info.getMapNameRegion()) || hoverRegion.equals(info.getMapEntTagNameRegion())) {
            ...
            @SuppressWarnings("unchecked") Set<String> keySet = cursorMap.keySet();
            ...
            @SuppressWarnings("unchecked") Map<String, String> nsMap = cursorMap.getNamespaceMapping();
            Map<String, String> reverseNsMap = new HashMap<String, String>();
            if (nsMap != null) for (String key : nsMap.keySet()) {
            ...
            for (String key : reverseNsMap.keySet()) {
                ...
                if (region == null) continue;
                if (RegionUtil.isSubRegion(region, hoverRegion)) {
                    return html("<pre>" + info.getAttrRealValue(realKey) + "</pre>");
                }
            }
        }
        return html(word);
        ...
        try {
            ...
            for (Attribute a : list) {
                if (attrName.equalsIgnoreCase(a.getName())) {
                    sb.append(a.getName() + "<br/>" + SxsdUtil.notNull(a.getDocument()));
                    if (SxsdUtil.getTypeNameNotNull(a.getAttributeType()).length() > 0) sb.append("<br/>Type : " + SxsdUtil.getTypeNameNotNull(a.getAttributeType()));
                    return sb.toString();
                }
            }
        } catch (Exception e) {
    ...
    public IRegion getHoverRegion(ITextViewer textViewer, int offset) {
        ...
        try {
            ...
            if (text == null || text.length() == 0) return super.getHoverRegion(textViewer, offset);
            int s = offset - ls, e = offset - ls;
            char c = text.charAt(s);
            if (isWordPart(c)) {
                while (s >= 0 && isWordPart(text.charAt(s))) s--;
                s++;
                while (e < text.length() && isWordPart(text.charAt(e))) e++;
    ...
    private String getMarkerInfo(ISourceViewer sourceViewer, IRegion hoverRegion) {
        ...
        } else model = sourceViewer.getAnnotationModel();
        if (model == null) return null;
        @SuppressWarnings("unchecked") Iterator<Annotation> e = model.getAnnotationIterator();
        while (e.hasNext()) {
            Annotation a = e.next();
