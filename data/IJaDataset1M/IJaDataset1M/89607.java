...
public abstract class DocUtil {
    ...
    private static void addSubContent(char[] data, int offs, Vector<ElementSpec> vec) {
        ElementSpec es1 = new ElementSpec(NotebookDocument.charAttrSet, ElementSpec.StartTagType);
        vec.addElement(es1);
        ElementSpec es = new ElementSpec(NotebookDocument.contentAttrSet, ElementSpec.ContentType, data, offs, data.length);
        vec.addElement(es);
        vec.addElement(new ElementSpec(null, ElementSpec.EndTagType));
    }

    protected static void startTag(ElementType e, MutableAttributeSet a, int pos, Vector<ElementSpec> vec) {
    ...
    static void createComponent(Component c, MutableAttributeSet attr, Vector<ElementSpec> vec) {
        if (c == null) return;
    ...
    static void createCellHandle(MutableAttributeSet attr, Vector<ElementSpec> vec) {
        startTag(cellHandle, attr, 0, vec);
        addSubContent(NEWLINE, 0, vec);
        endTag(vec);
    }

    static void createInsertionPoint(MutableAttributeSet attr, Vector<ElementSpec> vec) {
    ...
    static void createCellGroupMember(NotebookDocument doc, HGHandle cellH, MutableAttributeSet attr, Vector<ElementSpec> vec) {
        CellGroupMember cgm = (CellGroupMember) ThisNiche.graph.get(cellH);
        if (cgm instanceof CellGroup) createCellGroup(doc, cellH, attr, vec); else if (cgm instanceof Cell) {
            if (!CellUtils.isInputCell(cgm)) createOutputCell(doc, cellH, attr, vec); else createCell(doc, cellH, attr, vec);
        }
    }
    ...
    static void createCell(NotebookDocument doc, HGHandle cellH, MutableAttributeSet attr, Vector<ElementSpec> vec) {
        ...
        if (!CellUtils.isHTML(cell)) startTag(commonCell, attr, 0, vec); else startTag(htmlCell, attr, 0, vec);
        ...
        CellUtils.addEventPubSub(AttributeChangeEvent.HANDLE, cellH, doc.getHandle(), AttributeChangeHandler.getHandle());
        if (NotebookDocument.DIRECT_EVENTING) {
            CellUtils.addEventPubSub(CellTextChangeEvent.HANDLE, cellH, doc.getHandle(), CellTextChangeHandler.getHandle());
            CellUtils.addEventPubSub(EvalCellEvent.HANDLE, cellH, doc.getHandle(), EvalCellHandler.getHandle());
        } else {
            CellUtils.addMutualEventPubSub(CellTextChangeEvent.HANDLE, cellH, doc.getHandle(), CellTextChangeHandler.getHandle());
            CellUtils.addMutualEventPubSub(EvalCellEvent.HANDLE, cellH, doc.getHandle(), EvalCellHandler.getHandle());
        }
    }

    static void createOutputCell(NotebookDocument doc, HGHandle cellH, MutableAttributeSet attr, Vector<ElementSpec> vec, boolean genInsP) {
        createOutputCell(doc, cellH, attr, vec, genInsP, null);
    }

    static void createOutputCell(NotebookDocument doc, HGHandle cellH, MutableAttributeSet attr, Vector<ElementSpec> vec, boolean genInsP, EvalResult e) {
        attr.addAttribute(ATTR_CELL, cellH);
        ...
        attr = isError ? getDocStyle(doc, StyleType.error) : getDocStyle(doc, StyleType.outputCell);
        ...
        if (NotebookDocument.DIRECT_EVENTING) CellUtils.addEventPubSub(AttributeChangeEvent.HANDLE, cellH, doc.getHandle(), AttributeChangeHandler.getHandle()); else CellUtils.addMutualEventPubSub(AttributeChangeEvent.HANDLE, cellH, doc.getHandle(), AttributeChangeHandler.getHandle());
    }

    static void createOutputCell(NotebookDocument doc, HGHandle cellH, MutableAttributeSet attr, Vector<ElementSpec> vec) {
        createOutputCell(doc, cellH, attr, vec, true, null);
    }

    private static void createOutputCellContents(NotebookDocument doc, Cell c, MutableAttributeSet attr, Vector<ElementSpec> vec, EvalResult e) {
        ...
        Component comp = null;
        if (val != null) {
            if (!(val instanceof Component)) text = val.toString(); else comp = (Component) val;
        }
        if (comp == null) {
            if (!text.endsWith("\n")) text += "\n";
            addContent(text, vec);
        } else createComponent(comp, attr, vec);
        endTag(vec);
    }
    ...
    static void eval_result_in_aux_thread(final NotebookDocument doc, final Cell cell) {
        Runnable r = new Runnable() {
            ...
            @Override
            public void run() {
                EvalResult res = CellUtils.eval_result(cell, getEngineName(doc, cell), calc_eval_ctx(doc.getBook(), cell));
    ...
    private static EvaluationContext calc_eval_ctx(CellGroupMember top, CellGroupMember cell) {
        ...
        if (res != null && ThisNiche.graph.get(res) instanceof RuntimeContext) return ThisNiche.getEvaluationContext(res);
        CellGroup par = CellUtils.getParentGroup(ThisNiche.handleOf(cell));
        if (par == top || par == null) {
            if (par != null) res = CellUtils.getEvalContextH(par);
            if (res == null) res = ThisNiche.TOP_CONTEXT_HANDLE;
    ...
    static void createCellGroup(NotebookDocument doc, HGHandle cell_groupH, MutableAttributeSet attr, Vector<ElementSpec> vec) {
        ...
        createInsertionPoint(attr, vec);
        if (NotebookDocument.DIRECT_EVENTING) {
            CellUtils.addEventPubSub(AttributeChangeEvent.HANDLE, cell_groupH, doc.getHandle(), AttributeChangeHandler.getHandle());
            CellUtils.addEventPubSub(CellGroupChangeEvent.HANDLE, cell_groupH, doc.getHandle(), CellGroupChangeHandler.getHandle());
        } else {
            CellUtils.addMutualEventPubSub(AttributeChangeEvent.HANDLE, cell_groupH, doc.getHandle(), AttributeChangeHandler.getHandle());
            CellUtils.addMutualEventPubSub(CellGroupChangeEvent.HANDLE, cell_groupH, doc.getHandle(), CellGroupChangeHandler.getHandle());
        }
    }
}
