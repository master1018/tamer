 ...
 * @version $Id: RLOCRemove.java 144 2008-11-07 18:44:51Z jamescarroll $
 */
public class RLOCRemove {
    ...
    public void setOutputEdifFile(String filename) {
        _newedif = filename;
        if (_newedif.indexOf(".") == -1) _newedif = _newedif + _ext;
    }
    ...
    private static void _parseArgs(String[] args) {
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-design")) {
                i++;
                _designname = args[i];
                if (_designname.indexOf(".") == -1) _designname = _designname + _ext;
            } else if (args[i].equals("-edifout")) {
                i++;
                _newedif = args[i];
            } else if (args[i].equals("-type")) {
                i++;
                if (args[i].equals("-l")) _type = "LOC"; else _type = "RLOC";
            } else {
                _usage();
                System.exit(-1);
            }
