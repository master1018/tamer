...
public class TableColumnChooser extends JDialog {
    ...
    public TableColumnChooser(JFrame frame, SQLColumn sqlColumn, CompletionListener completor) {
        ...
        if (existingAlias != null || existingColumn != null) existingCompleted = false;
        ...
        for (Iterator it = tables.iterator(); it.hasNext(); ) {
            ...
            if (table.alias != null) assignedTables.put(table, table.alias);
        }
    ...
    private void initComponents(final List tables) {
        ...
        columnList.setSelectionMode(sqlColumn.isRepeatable() ? ListSelectionModel.MULTIPLE_INTERVAL_SELECTION : ListSelectionModel.SINGLE_SELECTION);
        ...
        getRootPane().registerKeyboardAction(cancelListener, KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0, true), JComponent.WHEN_IN_FOCUSED_WINDOW);
    ...
    private void performInsert() {
        ...
        if (cols.length == 0 || cols[0] == EMPTY_COL) return;
        ...
        if (sqlColumn.isRepeatable() == false) closeDialog();
    }

    private void columnListValueChanged(ListSelectionEvent evt) {
        Object[] cols = columnList.getSelectedValues();
        if (cols.length > 0 && cols[0] != EMPTY_COL) insertButton.setEnabled(true);
    }
