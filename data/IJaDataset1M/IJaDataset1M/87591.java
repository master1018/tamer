package org.berlin.math.lisp;

import org.berlin.math.lisp.OctaneScheme.Pair;
import org.berlin.math.lisp.OctaneScheme.Procedure;
 ...
 */
public class BuiltInFunction extends Procedure implements FunctionConstants {
	 ...
	 */
    public Object apply(OctaneScheme interp, Object args) {
        int nArgs = SchemeUtil.length(args);
        if (nArgs < minArgs) {
            return SchemeUtil.error("too few args, " + nArgs + ", for " + this.getName() + ": " + args);
        } else if (nArgs > maxArgs) {
            return SchemeUtil.error("too many args, " + nArgs + ", for " + this.getName() + ": " + args);
        }
        ...
        switch(idNumber) {
            ...
            case CXR:
                ...
                return x;
            default:
                return SchemeUtil.error("internal error: unknown primitive: " + this + " applied to " + args);
        }
    }
    ...
    public static Environment installBuiltInFunctions(Environment env) {
        ...
        env.defineBuiltInProc("cons", CONS, 2).defineBuiltInProc("*", TIMES, 0, n).defineBuiltInProc("+", PLUS, 0, n).defineBuiltInProc("-", MINUS, 1, n).defineBuiltInProc("/", DIVIDE, 1, n).defineBuiltInProc("caaaar", CXR, 1).defineBuiltInProc("caaadr", CXR, 1).defineBuiltInProc("caaar", CXR, 1).defineBuiltInProc("caadar", CXR, 1).defineBuiltInProc("caaddr", CXR, 1).defineBuiltInProc("caadr", CXR, 1).defineBuiltInProc("caar", CXR, 1).defineBuiltInProc("caddr", THIRD, 1).defineBuiltInProc("cadr", SECOND, 1).defineBuiltInProc("cdaaar", CXR, 1).defineBuiltInProc("cdadr", CXR, 1).defineBuiltInProc("cdar", CXR, 1).defineBuiltInProc("cddaar", CXR, 1).defineBuiltInProc("cddr", CXR, 1).defineBuiltInProc("car", CAR, 1).defineBuiltInProc("cdr", CDR, 1).defineBuiltInProc("*", TIMES, 0, n).defineBuiltInProc("+", PLUS, 0, n).defineBuiltInProc("-", MINUS, 1, n).defineBuiltInProc("/", DIVIDE, 1, n);
        return env;
    }
}
