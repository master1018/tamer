...
public final class ConsoleFoundation {
   ...
   * @exception GrinderException If an error occurs.
   */
    public ConsoleFoundation(Resources resources, Logger logger, Timer timer, ConsoleProperties properties) throws GrinderException {
        m_timer = timer;
        ...
        m_container.registerComponentInstance(StatisticsServicesImplementation.getInstance());
        m_container.registerComponentImplementation(SampleModelImplementation.class);
        m_container.registerComponentImplementation(SampleModelViewsImplementation.class);
        ...
        m_container.registerComponentImplementation(FileDistributionImplementation.class, FileDistributionImplementation.class, new Parameter[] { new ComponentParameter(DistributionControlImplementation.class), new ComponentParameter(ProcessControlImplementation.class), new ConstantParameter(properties.getDistributionDirectory()), new ConstantParameter(properties.getDistributionFileFilterPattern()) });
   ...
   */
    public UI createUI(Class uiClass) {
        ...
        final ErrorQueue errorQueue = (ErrorQueue) m_container.getComponentInstanceOfType(ErrorQueue.class);
   ...
   */
    public void shutdown() {
        final ConsoleCommunication communication = (ConsoleCommunication) m_container.getComponentInstanceOfType(ConsoleCommunication.class);
   ...
   */
     ...
     * @param timer A timer.
     */
        public WireFileDistribution(final FileDistribution fileDistribution, ConsoleProperties properties, Timer timer) {
            timer.schedule(new TimerTask() {

                public void run() {
                    fileDistribution.scanDistributionFiles();
                }
            }, properties.getScanDistributionFilesPeriod(), properties.getScanDistributionFilesPeriod());
   ...
   */
     ...
     */
        public WireMessageDispatch(ConsoleCommunication communication, final SampleModel model, final SampleModelViews sampleModelViews, DispatchClientCommands dispatchClientCommands) {
            ...
            messageDispatchRegistry.set(RegisterTestsMessage.class, new AbstractHandler() {

                public void send(Message message) {
                    model.registerTests(((RegisterTestsMessage) message).getTests());
                }
            });
            messageDispatchRegistry.set(ReportStatisticsMessage.class, new AbstractHandler() {

                public void send(Message message) {
                    model.addTestReport(((ReportStatisticsMessage) message).getStatisticsDelta());
                }
            });
            messageDispatchRegistry.set(RegisterExpressionViewMessage.class, new AbstractHandler() {

                public void send(Message message) {
                    sampleModelViews.registerStatisticExpression(((RegisterExpressionViewMessage) message).getExpressionView());
                }
            });
