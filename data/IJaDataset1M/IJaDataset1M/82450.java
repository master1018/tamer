...
public class NumericInput extends Input {
    ...
    public NumericInput(double value, double min, double max, int precision, final BorderStyle border, final boolean isHighlighted) {
        super(asString(value, precision), new Behavior(min, max, precision), border, isHighlighted);
    }

    public NumericInput(long value, double min, double max, int precision, final BorderStyle border, final boolean isHighlighted) {
    ...
    public static class Behavior implements Customizer {
        ...
        public Behavior(double minValue, double maxValue, int precision) {
            ...
            if (precision == 0) formatter.setParseIntegerOnly(true); else if (precision > 0) formatter.setMaximumFractionDigits(precision);
        }
        ...
        public boolean isValid(String inputString) {
            ...
            if (index == length - 1 && (last == comma || last == minus || last == point)) return true;
            if (index != length) return false;
            index = inputString.indexOf(point);
            if (index > 0 && precision > 0 && (length - index > precision + 1)) return false;
        ...
        public String help(String badInput) {
            ...
            b.append(badInput);
            if (isUnbounded()) b.append("<br>You must type a number.<br>"); else {
                b.append("<br>You must type a number between ");
                b.append(minAndMax());
                b.append(".<br>");
            }
            b.append("Commas are okay.<br>");
            if (precision == 0) b.append("Numbers may not contain a decimal point.<br>"); else {
                b.append("Numbers may contain a decimal point");
                if (precision < 0) b.append(".<br>"); else {
        ...
        public void prepare(JTextField current) {
            ...
            if (isUnbounded()) current.setToolTipText("Enter a number"); else current.setToolTipText("Enter a number between " + minAndMax());
            current.setCaretPosition(current.getText().length());
        }

        private String minAndMax() {
            return formatter.format(minValue) + " and " + formatter.format(maxValue);
        }
    }
