...
public class MapHelper {
    ...
    public MapObject createMap(String mapString, boolean useMarkers) {
        int firstNewline = mapString.indexOf('\n');
        int numberOfNewlines = countMatches(mapString, "\n");
        MapObject map = new MapObject(firstNewline, numberOfNewlines + 1);
    ...
    public MapObject createMap(String mapString) {
    ...
    public String mapToString(MapObject map) {
    ...
    public String visitToString(MapObject map) {
        StringBuffer buffer = new StringBuffer();
        for (int y = 0; y < map.getHeight(); y++) {
            for (int x = 0; x < map.getWidth(); x++) buffer.append(map.getPath(x, y));
            buffer.append("\n");
        }
        if (buffer.length() > 0) buffer.setLength(buffer.length() - 1);
        return buffer.toString();
    ...
    private String tileFor(MapObject map, int x, int y) {
