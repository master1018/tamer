...
public class LegacyInterpreter extends AtmelInterpreter implements LegacyInstrVisitor {
    ...
    protected void runLoop() {
        ...
        while (shouldRun) {
            ...
            if (sleeping) sleepLoop(); else {
                if (globalProbe.isEmpty()) fastLoop(); else instrumentedLoop();
            }
        }
    }
    ...
    public void visit(LegacyInstr.ELPMPI i) {
        ...
        if ((tmp_0 & 0xFFFF) == 0xFFFF) writeIORegisterByte(RAMPZ, (byte) (getIORegisterByte(RAMPZ) + 1));
        cyclesConsumed += 3;
    }
    ...
    public int extended(int addr) {
        if (RAMPZ > 0) return (getIORegisterByte(RAMPZ) & 1) << 16 | addr; else return addr;
    }
