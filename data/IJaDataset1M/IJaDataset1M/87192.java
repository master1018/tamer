...
public abstract class AbstractSerialConnectionConfig extends AbstractConnectionConfig implements jmri.jmrix.ConnectionConfig {
    ...
    public String getInfo() {
        ...
        if (t != null) return t; else return JmrixConfigPane.NONE;
    }
    ...
    public void refreshPortBox() {
        ...
        if (portBox.getActionListeners().length > 0) portBox.removeActionListener(portBox.getActionListeners()[0]);
        portBox.removeAllItems();
        log.debug("getting fresh list of available Serial Ports");
        if (v.size() == 0) v.add(0, rb.getString("noPortsFound"));
    ...
    public void loadDetails(final JPanel details) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                ...
                if (v != null) log.debug("Found " + v.size() + " ports"); else log.debug("Zero-length port vector");
            }
        ...
        if (baudBox.getActionListeners().length > 0) baudBox.removeActionListener(baudBox.getActionListeners()[0]);
        baudBox.removeAllItems();
        if (log.isDebugEnabled()) log.debug("after remove, " + baudBox.getItemCount() + " items, first is " + baudBox.getItemAt(0));
        for (int i = 0; i < baudList.length; i++) baudBox.addItem(baudList[i]);
        if (log.isDebugEnabled()) log.debug("after reload, " + baudBox.getItemCount() + " items, first is " + baudBox.getItemAt(0));
        opt1List = adapter.validOption1();
        opt1BoxLabel = new JLabel(adapter.option1Name());
        if (opt1Box.getActionListeners().length > 0) opt1Box.removeActionListener(opt1Box.getActionListeners()[0]);
        ...
        if (opt2Box.getActionListeners().length > 0) opt2Box.removeActionListener(opt2Box.getActionListeners()[0]);
    ...
    public void setDisabled(boolean disabled) {
        if (adapter != null) adapter.setDisabled(disabled);
    }

    public String getConnectionName() {
        if ((adapter != null) && (adapter.getSystemConnectionMemo() != null)) return adapter.getSystemConnectionMemo().getUserName(); else return null;
    }
    ...
    class ComboBoxRenderer extends JLabel implements ListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            if (port.equals(invalidPort)) {
                if (isSelected) list.setSelectionForeground(Color.red);
    ...
    protected static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(AbstractSerialConnectionConfig.class.getName());
}
