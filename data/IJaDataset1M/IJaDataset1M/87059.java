...
public class SecoView extends ViewPart {
    ...
    public void createPartControl(final Composite par) {
        this.parent = par;
        parent.setBackground(new org.eclipse.swt.graphics.Color(parent.getDisplay(), 255, 255, 255));
        update();
    }
    ...
    void update() {
        ...
        if (plugin.getNicheLocation() == null && memento != null) plugin.setNicheLocation(restoreState());
        clean();
        if (plugin.getNicheLocation() == null) {
            ...
            if (!success) {
                ...
                noNicheGUI = new NoNicheGUI(parent, SWT.NONE);
                force_redraw();
            } else swingControl = new SwingControl(parent, SWT.NONE) {
    ...
    public void saveState(IMemento memento) {
        if (ThisNiche.graph != null) memento = memento.createChild("niche", ThisNiche.graph.getLocation());
    }

    private String restoreState() {
        if (memento == null) return null;
        String res = (memento.getChild("niche") != null) ? memento.getChild("niche").getID() : null;
        memento = null;
        return res;
    }
    ...
    private class NoNicheGUI extends Composite {
        ...
        public NoNicheGUI(final Composite parent, int style) {
            ...
            setBackground(new org.eclipse.swt.graphics.Color(getDisplay(), 255, 255, 255));
            text = new Label(this, SWT.WRAP);
            text.setText("No niche specified.\n " + "Please select one.");
            text.setBackground(new org.eclipse.swt.graphics.Color(getDisplay(), 255, 255, 255));
            org.eclipse.swt.graphics.Font font = new org.eclipse.swt.graphics.Font(getDisplay(), new FontData("Times New Roman", 16, 0));
            text.setFont(font);
    ...
    static class GoToDeclarationAction extends AbstractAction {
        ...
        public GoToDeclarationAction() {
            ...
            putValue(AbstractAction.ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_F3, 0));
        }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                if (ui.getSelectionStart() != ui.getSelectionEnd()) {
                    resolveAndOpen(sup, doc.getText(ui.getSelectionStart(), ui.getSelectionEnd() - ui.getSelectionStart()), pos);
                    return;
                }
        ...
        private void resolveAndOpen(ScriptSupport sup, String s, int pos) {
            ...
            if (s.indexOf("(") > 0) {
                ...
                if (cls != null) PluginU.searchAndOpen(cls.getName()); else PluginU.searchAndOpen(s);
            }
        }
    ...
    public static int getWordStart(NotebookDocument doc, int offset) throws BadLocationException {
        return Utilities.find(doc, new FinderFactory.AcceptorBwdFinder(new FwdAcceptor()), offset, 0);
    }

    public static int getWordEnd(NotebookDocument doc, int offset) throws BadLocationException {
        int ret = Utilities.find(doc, new SmartFwdFinder(), offset, -1);
    ...
    static class FwdAcceptor implements Acceptor {
        ...
        public boolean accept(char ch) {
            if (ch == '(') numParanthes++;
            if (ch == ')') numParanthes--;
            if (numParanthes == 0 && (ch == '.' || ch == ' ' || ch == ',')) return false;
            return ch != ';' && ch != '\n' && ch != '=';
        }
    }
}
