...
public abstract class AbstractTestCaseGenerator {
    ...
    protected void notifyClients(Object testCase) {
        ...
        if (TestCradle.getInstance().isPredicateOK()) {
            for (ITestCaseListener client : clients) client.notifyNewTestCase(testCase);
        }
        for (ITestCaseListener special : specialClients) special.notifyNewTestCase(testCase);
    }

    protected void notifyTestFinished(long numOfExplored, long numOfGenerated) {
        for (ITestCaseListener client : clients) client.notifyTestFinished(numOfExplored, numOfGenerated);
        for (ITestCaseListener special : specialClients) special.notifyTestFinished(numOfExplored, numOfGenerated);
    }
    ...
    protected void log(int[] vector) {
        if (PRINT_CASES == false) return;
        for (int i = 0; i < vector.length; i++) out.print(vector[i] + " ");
    }
