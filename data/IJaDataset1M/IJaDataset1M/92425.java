...
public class SudokuExplainer {
    ...
    private List<Hint> selectedHints = new ArrayList<Hint>();

    Set<Cell> givenCells = new HashSet<Cell>();

    Map<Cell, BitSet> removedPotentials = new HashMap<Cell, BitSet>();
    ...
    public SudokuExplainer() {
        ...
        frame.setLocation((screenWidth - windowSize.width) / 2 + insets.left, (screenHeight - windowSize.height) / 2 + insets.top);
        frame.setVisible(true);
    }

    private void resetFilterCache() {
        givenCells = new HashSet<Cell>();
        removedPotentials = new HashMap<Cell, BitSet>();
    }
    ...
    private boolean isWorth(Hint hint) {
        ...
        boolean isWorth;
        if (hint instanceof DirectHint) isWorth = isWorth(givenCells, (DirectHint) hint); else isWorth = isWorth(removedPotentials, givenCells, (IndirectHint) hint);
        return isWorth;
    }
    ...
    private boolean isWorth(Map<Cell, BitSet> removedPotentials, Set<Cell> givenCells, IndirectHint hint) {
        if (hint instanceof WarningHint) return true;
        ...
        for (Cell cell : removablePotentials.keySet()) {
            ...
            if (!newRemove.isEmpty()) return true;
        }
        Cell cell = hint.getCell();
        if (cell != null && !givenCells.contains(cell)) return true;
        return false;
    }
        ...
        if (unfilteredHints != null) {
            ...
            if (isFiltered) {
                for (Hint hint : unfilteredHints) {
                    if (isWorth(hint)) addFilteredHintAndUpdateFilter(hint);
                }
            } else {
                for (Hint hint : unfilteredHints) filteredHints.add(hint);
            }
        }
    }
    ...
    private void addFilteredHintAndUpdateFilter(Hint hint) {
        ...
        if (hint instanceof DirectHint) {
            ...
            if (cell != null) givenCells.add(cell);
        }
    }
    ...
    private void repaintHints() {
        if (selectedHints.size() == 1) frame.setCurrentHint(selectedHints.get(0), true); else {
            frame.setCurrentHint(null, !selectedHints.isEmpty());
            if (selectedHints.size() > 1) paintMultipleHints(selectedHints);
        }
    }
    ...
    private void paintMultipleHints(List<Hint> hints) {
        ...
        for (Hint hint : hints) {
            Cell cell = hint.getCell();
            if (cell != null) greenPotentials.put(cell, SingletonBitSet.create(hint.getValue()));
            if (hint instanceof IndirectHint) {
                ...
                for (Cell rCell : removable.keySet()) {
                    ...
                    if (redPotentials.containsKey(rCell)) redPotentials.get(rCell).or(values); else redPotentials.put(rCell, (BitSet) values.clone());
                }
            }
        }
    ...
    public void cellValueTyped(Cell cell, int value) {
        ...
        cell.setValue(value);
        if (value == 0 || oldValue != 0) solver.rebuildPotentialValues(); else solver.cancelPotentialValues();
    ...
    public void candidateTyped(Cell cell, int candidate) {
        if (cell.hasPotentialValue(candidate)) cell.removePotentialValue(candidate); else cell.addPotentialValue(candidate);
    ...
    public void hintsSelected(Collection<HintNode> nodes) {
        ...
        for (HintNode node : nodes) {
            ...
            if (hint != null) this.selectedHints.add(hint);
        }
        repaintHints();
    }
    ...
    private Hint getNextHintImpl() {
        ...
        selectedHints.clear();
        Hint hint = null;
        if (newHint.isValueSet()) hint = newHint.getValue();
        return hint;
    }
    ...
    public void getAllHints() {
        try {
            ...
            resetFilterCache();
            filterHints();
            if (!filteredHints.isEmpty()) selectedHints.add(filteredHints.get(0));
            repaintAll();
        } catch (Throwable ex) {
            displayError(ex);
        }
    ...
    public void applySelectedHints() {
        for (Hint hint : selectedHints) hint.apply();
        clearHints();
        repaintAll();
    }
    ...
    private void repaintHintsTree() {
        if (filteredHints == null) {
            ...
            if (root != null && selectedHints.size() == 1) selected = root.getNodeFor(selectedHints.get(0));
    ...
    public void pasteGrid() {
        ...
        if (message == null || !message.isFatal()) solver.rebuildPotentialValues(); else copy.copyTo(grid);
        if (message != null) JOptionPane.showMessageDialog(frame, message.toString(), "Paste", (message.isFatal() ? JOptionPane.ERROR_MESSAGE : JOptionPane.WARNING_MESSAGE));
    }
    ...
    public void saveGrid(File file) {
        ...
        if (message != null) JOptionPane.showMessageDialog(frame, message.toString(), "Paste", JOptionPane.ERROR_MESSAGE);
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            if (lookAndFeelClassName == null) lookAndFeelClassName = UIManager.getSystemLookAndFeelClassName();
