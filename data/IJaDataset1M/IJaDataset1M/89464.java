...
public class LatLon extends Coordinate implements Serializable, Externalizable {
    ...
    public static double distanceToKilomters(final double aDist) {
        final int kilo = 1000;
        return Math.sqrt(aDist) * Settings.getProjection().scaleFactor() * Projection.EARTH_CIRCMFENCE_IN_METERS / kilo;
    }
    ...
    public static double distanceInMeters(final double lat1, final double lon1, final double lat2, final double lon2) {
        ...
        return (Math.acos(Math.sin(p1lat) * Math.sin(p2lat) + Math.cos(p1lat) * Math.cos(p2lat) * Math.cos(Math.toRadians(lon2 - lon1))) * factor);
    }
    ...
    public static double distanceInMeters(final Node point1, final Node point2) {
        return distanceInMeters(point1.getLatitude(), point1.getLongitude(), point2.getLatitude(), point2.getLongitude());
    }
    ...
    /**
        ...
        */
    @Override
    public void readExternal(final ObjectInput aIn) throws IOException, ClassNotFoundException {
