...
public final class VM_GCMapIteratorGroup implements VM_SizeConstants {
   ...
   */
    public VM_GCMapIterator selectIterator(VM_CompiledMethod compiledMethod) throws VM_PragmaUninterruptible {
        ...
        if (type == bootImageCompilerIterator.getType()) return bootImageCompilerIterator;
        if (type == runtimeCompilerIterator.getType()) return runtimeCompilerIterator;
        if (type == hardwareTrapIterator.getType()) return hardwareTrapIterator;
        if (type == fallbackCompilerIterator.getType()) return fallbackCompilerIterator;
        if (jniIterator != null && type == jniIterator.getType()) return jniIterator;
        if (testOptCompilerIterator != null && type == testOptCompilerIterator.getType()) return testOptCompilerIterator;
