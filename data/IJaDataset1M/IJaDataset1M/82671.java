    ...
    private void addCostOfElectricalChargingToTheScore() {
        for (Id personId : chargingTimes.keySet()) {
            ...
            for (ChargeLog chargeLog : curChargingTime.getChargingTimes()) {
                ...
                double chargingPrice = ParametersPSF.getHubPriceInfo().getPrice(chargingStartTime, ParametersPSF.getHubLinkMapping().getHubNumber(chargeLog.getLinkId().toString())) / 3600000;
                double energyCharged = chargeLog.getEnergyCharged();
                double negativeUtilitiesForCharging = -1 * chargingPrice * energyCharged * ParametersPSF.getMainChargingPriceScalingFactor();
                ParametersPSF.getEvents().processEvent(new AgentMoneyEventImpl(chargingStartTime, personId, negativeUtilitiesForCharging));
