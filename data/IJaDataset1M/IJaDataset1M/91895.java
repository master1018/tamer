...
public class XOMTestCase extends TestCase {
    ...
    public static void assertEquals(String message, Attribute expected, Attribute actual) {
        ...
        if ("xml:base".equals(expected.getQualifiedName())) {
            ...
            if (value1.startsWith("../")) {
                ...
                assertTrue(message, value1.endsWith('/' + value2) || value2.endsWith('/' + value1));
            }
        } else {
            assertEquals(message, value1, value2);
            assertEquals(message, expected.getLocalName(), actual.getLocalName());
            assertEquals(message, expected.getQualifiedName(), actual.getQualifiedName());
            assertEquals(message, expected.getNamespaceURI(), actual.getNamespaceURI());
        }
    }
    ...
    public static void assertEquals(String message, DocType expected, DocType actual) {
        if (actual == expected) return;
        nullCheck(message, expected, actual);
        assertEquals(message, expected.getPublicID(), actual.getPublicID());
        assertEquals(message, expected.getSystemID(), actual.getSystemID());
        assertEquals(message, expected.getRootElementName(), actual.getRootElementName());
    }
    ...
    public static void assertEquals(String message, Element expected, Element actual) {
        ...
        nullCheck(message, expected, actual);
        assertEquals(message, expected.getLocalName(), actual.getLocalName());
        assertEquals(message, expected.getNamespacePrefix(), actual.getNamespacePrefix());
        assertEquals(message, expected.getNamespaceURI(), actual.getNamespaceURI());
        assertEquals(message, expected.getAttributeCount(), actual.getAttributeCount());
        ...
        for (int i = 0; i < expected.getNamespaceDeclarationCount(); i++) {
            ...
            assertEquals(message, uri1, actual.getNamespaceURI(prefix1));
        }
        for (int i = 0; i < actual.getNamespaceDeclarationCount(); i++) {
            ...
            assertEquals(message, uri1, expected.getNamespaceURI(prefix1));
        }
    ...
    private static boolean hasAdjacentTextNodes(Element element) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child instanceof Text) {
                if (previousWasText) return true; else if ("".equals(child.getValue())) return true; else previousWasText = true;
            } else {
    ...
    public static void assertEquals(String message, Node expected, Node actual) {
        ...
        try {
            if (expected instanceof Document) {
                ...
                assertEquals(message, (DocType) expected, (DocType) actual);
            } else if (expected instanceof Comment) {
                assertEquals(message, (Comment) expected, (Comment) actual);
            } else if (expected instanceof ProcessingInstruction) {
                assertEquals(message, (ProcessingInstruction) expected, (ProcessingInstruction) actual);
            } else if (expected instanceof Attribute) {
                assertEquals(message, (Attribute) expected, (Attribute) actual);
            } else if (expected instanceof Namespace) {
                assertEquals(message, (Namespace) expected, (Namespace) actual);
            } else {
