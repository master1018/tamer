...
public class TradeList {
    ...
    public class TradeItem {
        ...
        public TradeItem(L2ItemInstance item, long count, long price) {
            ...
            for (byte i = 0; i < 6; i++) _elemDefAttr[i] = item.getElementDefAttr(i);
        }
        ...
        if (item.isStackable()) {
            for (TradeItem exclItem : _items) {
                if (exclItem.getItem().getItemId() == item.getItemId()) {
                    if (item.getCount() <= exclItem.getCount()) return null; else return new TradeItem(item, item.getCount() - exclItem.getCount(), item.getReferencePrice());
                }
            }
        }
    ...
    public synchronized TradeItem addItem(int objectId, long count, long price) {
        ...
        if (!(item.isTradeable() || (getOwner().isGM() && Config.GM_TRADE_RESTRICTED_ITEMS)) || item.getItemType() == L2EtcItemType.QUEST) return null;
    ...
    public synchronized TradeItem removeItem(int objectId, int itemId, long count) {
        ...
        for (TradeItem titem : _items) {
            if (titem.getObjectId() == objectId || titem.getItem().getItemId() == itemId) {
                ...
                if (count != -1 && titem.getCount() > count) titem.setCount(titem.getCount() - count); else _items.remove(titem);
                return titem;
            }
        }
        return null;
    ...
    public synchronized void updateItems() {
        for (TradeItem titem : _items) {
            ...
            if (item == null || titem.getCount() < 1) removeItem(titem.getObjectId(), -1, -1); else if (item.getCount() < titem.getCount()) titem.setCount(item.getCount());
        }
    }
    ...
    private boolean TransferItems(L2PcInstance partner, InventoryUpdate ownerIU, InventoryUpdate partnerIU) {
        for (TradeItem titem : _items) {
            ...
            if (ownerIU != null) {
                if (oldItem.getCount() > 0 && oldItem != newItem) ownerIU.addModifiedItem(oldItem); else ownerIU.addRemovedItem(oldItem);
            }
            if (partnerIU != null) {
                if (newItem.getCount() > titem.getCount()) partnerIU.addModifiedItem(newItem); else partnerIU.addNewItem(newItem);
            }
        }
        return true;
    }
    ...
    public int countItemsSlots(L2PcInstance partner) {
        ...
        for (TradeItem item : _items) {
            ...
            if (!template.isStackable()) slots += item.getCount(); else if (partner.getInventory().getItemByItemId(item.getItem().getItemId()) == null) slots++;
        }
        return slots;
    }
    ...
    private void doExchange(TradeList partnerList) {
        ...
        if ((!getOwner().getInventory().validateWeight(partnerList.calcItemsWeight())) || !(partnerList.getOwner().getInventory().validateWeight(calcItemsWeight()))) {
            ...
            if (ownerIU != null) _owner.sendPacket(ownerIU); else _owner.sendPacket(new ItemList(_owner, false));
            if (partnerIU != null) _partner.sendPacket(partnerIU); else _partner.sendPacket(new ItemList(_partner, false));
    ...
    public synchronized int privateStoreBuy(L2PcInstance player, FastSet<ItemRequest> items) {
        ...
        for (ItemRequest item : items) {
            ...
            if (template == null) continue;
            weight += item.getCount() * template.getWeight();
            if (!template.isStackable()) slots += item.getCount(); else if (playerInventory.getItemByItemId(item.getItemId()) == null) slots++;
        }
        ...
        for (ItemRequest item : items) {
            ...
            if (oldItem.getCount() > 0 && oldItem != newItem) ownerIU.addModifiedItem(oldItem); else ownerIU.addRemovedItem(oldItem);
            if (newItem.getCount() > item.getCount()) playerIU.addModifiedItem(newItem); else playerIU.addNewItem(newItem);
    ...
    public synchronized boolean privateStoreSell(L2PcInstance player, ItemRequest[] items) {
        ...
        for (ItemRequest item : items) {
            ...
            if (!oldItem.isTradeable()) continue;
            ...
            if (oldItem.getCount() > 0 && oldItem != newItem) playerIU.addModifiedItem(oldItem); else playerIU.addRemovedItem(oldItem);
            if (newItem.getCount() > item.getCount()) ownerIU.addModifiedItem(newItem); else ownerIU.addNewItem(newItem);
            if (newItem.isStackable()) {
