...
public class SumoTraciConnection {
    ...
    public SumoTraciConnection(String configFile, int randomSeed, boolean useGeoOffset) {
        ...
        if (useGeoOffset) geoOffset = lookForGeoOffset(configFile);
    }

    public SumoTraciConnection(SocketAddress sockAddr) throws IOException, InterruptedException {
        socket = new Socket();
        if (log.isDebugEnabled()) log.debug("Connecting to remote TraCI server at " + socket.toString());
        int waitTime = 500;
    ...
    public void runServer() throws IOException {
        ...
        try {
            for (int i = 0; i < CONNECT_RETRIES; i++) {
                ...
                socket = new Socket();
                if (log.isDebugEnabled()) log.debug("Connecting to local port " + remotePort);
                try {
                    socket.connect(new InetSocketAddress(InetAddress.getLocalHost(), remotePort));
                    log.info("Connection to SUMO established.");
                    break;
                } catch (ConnectException ce) {
                    log.debug("Server not ready, retrying in " + waitTime + "ms");
    ...
    private void runSUMO() throws IOException {
        ...
        if (sumoEXE == null) throw new RuntimeException("System property " + SUMO_EXE_PROPERTY + " must be set");
        args.add(0, sumoEXE);
        args.add("-c");
        args.add(configFile);
        ...
        if (log.isDebugEnabled()) log.debug("Executing SUMO with cmdline " + args);
    ...
    public void close() throws InterruptedException {
        for (Vehicle v : vehicles.values()) v.alive = false;
        if (socket != null) {
            if (closeQuery != null) {
                try {
        ...
        if (httpRetriever != null) httpRetriever.close();
        vehicles.clear();
    ...
    public Rectangle2D queryBounds() throws IOException {
        if (isClosed()) throw new IllegalStateException("connection is closed");
        try {
            ...
            for (Lane r : lanes.values()) {
                ...
                if (boundsAll == null) boundsAll = bounds; else boundsAll = boundsAll.createUnion(bounds);
            }
            return boundsAll;
        } catch (IOException e) {
    ...
    private void updateVehiclesPosition() throws IOException {
        MultiVehiclePositionQuery mvpQuery = new MultiVehiclePositionQuery(socket, activeVehicles);
        Map<String, Point2D> vehiclesPosition = mvpQuery.getVehiclesPosition2D();
        for (Map.Entry<String, Point2D> entry : vehiclesPosition.entrySet()) {
            String id = entry.getKey();
            Point2D pos = entry.getValue();
            if (!activeVehicles.contains(id) || !vehicles.containsKey(id)) throw new IllegalStateException("should never happen");
    ...
    protected void notifyDeparted(Set<String> created) {
        for (String id : created) {
            for (VehicleLifecycleObserver observer : vehicleLifecycleObservers) observer.vehicleDeparted(id);
        }
    }

    protected void notifyArrived(Set<String> destroyed) {
        for (String id : destroyed) {
            for (VehicleLifecycleObserver observer : vehicleLifecycleObservers) observer.vehicleArrived(id);
        }
    }

    protected void notifyTeleportStarting(Set<String> starting) {
        for (String id : starting) {
            for (VehicleLifecycleObserver observer : vehicleLifecycleObservers) observer.vehicleTeleportStarting(id);
        }
    }

    protected void notifyTeleportEnding(Set<String> ending) {
        for (String id : ending) {
            for (VehicleLifecycleObserver observer : vehicleLifecycleObservers) observer.vehicleTeleportEnding(id);
        }
    }
    ...
    public Vehicle getVehicle(String id) {
        if (isClosed()) throw new IllegalStateException("connection is closed");
        if (vehicles.containsKey(id)) return vehicles.get(id); else throw new IllegalArgumentException("Vehicle ID " + id + " does not exist");
    }
    ...
    public Lane getLane(String roadID) throws IOException {
        if (isClosed()) throw new IllegalStateException("connection is closed");
        try {
            if (cachedLanes == null) queryLanes();
        ...
        try {
            ContentHandler sumoConfHandler = new DefaultHandler() {
                ...
                public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
                    if (localName.equals("net-file")) doRead = true;
                }

                @Override
                public void characters(char[] ch, int start, int length) throws SAXException {
                    if (doRead) {
                        throw new DataFoundException(new String(ch, start, length));
                    }
                }
            };
            String netFile = null;
            ...
            String basePath = configFile.substring(0, configFile.lastIndexOf(File.separatorChar));
            ...
            ContentHandler netDescHandler = new DefaultHandler() {
                ...
                @Override
                public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
                    if (localName.equals("net-offset")) doRead = true;
                }
            ...
            try {
                xmlReader.parse(new InputSource(new FileInputStream(absNetFile)));
                return null;
            } catch (DataFoundException dfe) {
                String geoOffsetStr = dfe.getMessage();
                String[] geoOffsetStrFields = geoOffsetStr.split(",");
                return new Point2D.Float(Float.parseFloat(geoOffsetStrFields[0]), Float.parseFloat(geoOffsetStrFields[1]));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Map<String, RoadmapPosition> getAllVehiclesRoadmapPos() throws IOException {
        ...
        try {
            ...
            MultiVehiclePositionQuery mvpq = new MultiVehiclePositionQuery(socket, vehicleIDs);
