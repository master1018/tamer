...
public class ConfiguredUniverse extends SimpleUniverse {
    ...
    public ConfiguredUniverse(ConfigContainer userConfig, LocaleFactory localeFactory, HiResCoord origin) {
        ...
        if (c == null || c.size() == 0) throw new IllegalArgumentException("no views defined in configuration file");
        viewer = (Viewer[]) c.toArray(new Viewer[1]);
        c = configContainer.getViewingPlatforms();
        if (c == null || c.size() == 0) {
            createDefaultViewingPlatform(configContainer.getViewPlatformTransformCount());
        } else {
    ...
    ConfiguredUniverse(int transformCount, Canvas3D[] canvases, URL userConfig, LocaleFactory localeFactory, HiResCoord origin, boolean setVisible) {
        ...
        if (userConfig == null) {
            ...
            configContainer = new ConfigContainer(userConfig, setVisible, transformCount, false);
            Collection c = configContainer.getViewers();
            if (c == null || c.size() == 0) throw new IllegalArgumentException("no views defined in configuration file");
            viewer = (Viewer[]) c.toArray(new Viewer[1]);
            ...
            if (c == null || c.size() == 0) {
                ...
                while (i.hasNext()) {
                    ...
                    if (vp.getViewers() == null && viewer.length == 1 && c.size() == 1) {
    ...
    private void setDerivedAttachPolicy(Viewer v, ViewingPlatform vp) {
        if (v.getView().getWindowEyepointPolicy() != View.RELATIVE_TO_FIELD_OF_VIEW) {
    ...
    public void setVisible(boolean visible) {
        for (int i = 0; i < viewer.length; i++) if (viewer[i] != null) viewer[i].setVisible(visible);
    }
    ...
    public Map getNamedSensors() {
        if (configContainer == null) return null; else return configContainer.getNamedSensors();
    }
    ...
    public Map getNamedBehaviors() {
        if (configContainer == null) return null; else return configContainer.getNamedViewPlatformBehaviors();
    }
