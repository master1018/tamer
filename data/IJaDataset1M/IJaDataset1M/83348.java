...
public abstract class Loadings {
    ...
    public static Set<Class<?>> scanModules(Class<?> mainModule) {
        ...
        for (Class<?> type : list) {
            if (scan) scanModuleInPackage(modules, type.getPackage().getName()); else {
                if (isModule(type)) {
                    if (log.isDebugEnabled()) log.debugf(" > add '%s'", type.getName());
                    modules.add(type);
    ...
    protected static void scanModuleInPackage(Set<Class<?>> modules, String packageName) {
        if (log.isDebugEnabled()) log.debugf(" > scan '%s'", packageName);
        List<Class<?>> subs = Scans.me().scanPackage(packageName);
        for (Class<?> sub : subs) {
            if (isModule(sub)) {
                if (log.isDebugEnabled()) log.debugf("   >> add '%s'", sub.getName());
                modules.add(sub);
            } else if (log.isTraceEnabled()) {
    ...
    public static void evalHttpMethod(ActionInfo ai, Method method) {
        if (method.getAnnotation(GET.class) != null) ai.getHttpMethods().add("GET");
        if (method.getAnnotation(POST.class) != null) ai.getHttpMethods().add("POST");
        if (method.getAnnotation(PUT.class) != null) ai.getHttpMethods().add("PUT");
        if (method.getAnnotation(DELETE.class) != null) ai.getHttpMethods().add("DELETE");
    }
    ...
    public static void evalAt(ActionInfo ai, At at, String def) {
        if (null != at) {
            ...
            if (!Strings.isBlank(at.key())) ai.setPathKey(at.key());
        }
    }
    ...
    public static boolean isModule(Class<?> classZ) {
        ...
        if (!Modifier.isPublic(classModify) || Modifier.isAbstract(classModify) || Modifier.isInterface(classModify)) return false;
        for (Method method : classZ.getMethods()) if (method.isAnnotationPresent(At.class)) return true;
        return false;
    }
}
