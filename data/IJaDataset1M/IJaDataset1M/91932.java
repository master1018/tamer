...
public abstract class AbstractNamer implements Namer {
    ...
    public Collection<AddSynonymHistoryItem> generateSynonymChanges(OBOSession session) {
        Collection<AddSynonymHistoryItem> items = new ArrayList<AddSynonymHistoryItem>();
    ...
    public Collection<DefinitionChangeHistoryItem> generateDefinitionChanges(OBOSession session) {
        Collection<DefinitionChangeHistoryItem> items = new ArrayList<DefinitionChangeHistoryItem>();
        for (IdentifiedObject io : session.getObjects()) {
            if (io.isBuiltIn()) continue;
            if (!(io instanceof DefinedObject)) continue;
            if (((DefinedObject) io).getDefinition() == null) continue;
    ...
    protected boolean isSubclassByName(LinkedObject lo, String superName) {
        if (lo.getName().equals(superName)) return true;
        for (Link tr : lo.getParents()) {
            if (ReasonerUtil.isSubclass(tr.getType(), OBOProperty.IS_A) && isSubclassByName(tr.getParent(), superName)) return true;
        }
        return false;
    }
}
