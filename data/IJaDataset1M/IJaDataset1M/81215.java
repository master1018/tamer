...
public class JRenderer3D {
	 ...
	 */
    public static final int POINT_SPHERE = Point3D.SPHERE;

    /**
	 * Draws a point as a (2D) circle.
	 */
    public static final int POINT_CIRCLE = Point3D.CIRCLE;

    /**
	 * Draws a point as a dot. Size information has no effect.
	 */
    public static final int POINT_DOT = Point3D.DOT;
    ...
    private int volume_lutNr = LUT_ORIGINAL;
	...
	*************************************************************/
    ...
    private void finishAndDrawText() {
        ...
        if (bufferedImage == null || bufferedImage.getHeight() != bufferHeight || bufferedImage.getWidth() != bufferWidth) {
        ...
        if (legend && surfacePlot != null) {
            ...
            if (lutNr >= LUT_GRAY && lutNr <= LUT_THERMAL) {
                ...
                for (double value = 0; value + delta <= d; value += stepValue) {
                    String s;
                    if (Math.floor(minStart + value) - (minStart + value) == 0) s = "" + (int) (minStart + value); else s = "" + (int) Math.round((minStart + value) * 1000) / 1000.;
                    double pos = ((value + delta) * 256 / d);
                    int y;
                    if (pos >= 0) {
                        y = (int) (-pos + 255 + ys);
                        if (isInverse) y = (int) (pos + ys);
    ...
    double calcStepSize(double range, double targetSteps) {
        ...
        if (magMsd > 6) magMsd = 10.0; else if (magMsd > 3) magMsd = 5.0; else if (magMsd > 2) magMsd = 4.0; else if (magMsd > 1) magMsd = 2.0;
    ...
    private void clearCubeLines3D() {
        if (cubeLines3D != null) this.cubeLines3D.clear();
    }
    ...
    public void setSurfacePlot(ImagePlus imp) {
        ...
        if (surfacePlot_imagePlusTexture != null) surfacePlot.setSurfacePlotTextureImage(surfacePlot_imagePlusTexture);
    ...
    public void setSurfacePlotMode(int surfacePlot_plotMode) {
        ...
        if (surfacePlot != null) surfacePlot.setSurfacePlotMode(surfacePlot_plotMode);
    }
    ...
    public void clearLines() {
        if (this.lines3D != null) this.lines3D.clear();
    }
    ...
    public void clearText() {
        if (this.text3D != null) this.text3D.clear();
    }
    ...
    public void add3DCube(int xMin, int yMin, int zMin, int xMax, int yMax, int zMax, int rgb) {
        double[][] cube = { { xMin - xCenter, yMin - yCenter, zMin - zCenter }, { xMax - xCenter, yMin - yCenter, zMin - zCenter }, { xMin - xCenter, yMin - yCenter, zMax - zCenter }, { xMax - xCenter, yMin - yCenter, zMax - zCenter }, { xMin - xCenter, yMax - yCenter, zMin - zCenter }, { xMax - xCenter, yMax - yCenter, zMin - zCenter }, { xMin - xCenter, yMax - yCenter, zMax - zCenter }, { xMax - xCenter, yMax - yCenter, zMax - zCenter } };
    ...
    public void setVolume(ImagePlus imagePlus) {
        ...
        this.zAspectRatio = volume.getZaspectRatio();
        transform.setZAspectRatio(zAspectRatio);
        volume.setBuffers(bufferPixels, zbufferPixels, bufferWidth, bufferHeight);
        volume.setVolumeLut(volume_lutNr);
        volume.setVolumeDrawMode(volume_drawMode);
    ...
    public void setSurfacePlotLut(int surfacePlot_lutNr) {
        ...
        if (surfacePlot != null) surfacePlot.setSurfacePlotLut(surfacePlot_lutNr);
    }
    ...
    public void setSurfacePlotLight(double surfacePlot_light) {
        ...
        if (surfacePlot != null) surfacePlot.setSurfacePLotSetLight(surfacePlot_light);
    }
    ...
    public void setVolumeThreshold(int threshold) {
        ...
        if (volume != null) volume.setVolumeThreshold(threshold);
    }
    ...
    public void removeLastLine3D() {
        if (lines3D != null) {
            ...
            if (size > 0) this.lines3D.remove(size - 1);
        }
    }
    ...
    public void surfacePlotSetInverse(boolean b) {
        if (surfacePlot != null) surfacePlot.setInverse(b);
    }

    public double getTransformScale() {
        if (transform != null) return transform.getScale(); else return 1;
    }
