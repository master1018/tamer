...
public class WeaponProfsTokenTest extends AbstractChooseTokenTestCase {
    ...
    static CDOMTokenLoader<CDOMTemplate> loader = new CDOMTokenLoader<CDOMTemplate>(CDOMTemplate.class);
    ...
    public void testInvalidInputAnyList() throws PersistenceLayerException {
        if (isListLegal() && isAnyLegal()) {
            ...
            assertFalse(getToken().parse(primaryContext, primaryProf, getSubTokenString() + "|ANY" + getJoinCharacter() + "LIST"));
            assertNoSideEffects();
        }
    }
    ...
    public void testInvalidInputListAny() throws PersistenceLayerException {
        if (isListLegal() && isAnyLegal()) {
            construct(primaryContext, "TestWP1");
            assertFalse(getToken().parse(primaryContext, primaryProf, getSubTokenString() + "|LIST" + getJoinCharacter() + "ANY"));
            assertNoSideEffects();
        }
    }
    ...
    public void testRoundRobinComplexToo() throws PersistenceLayerException {
        if (isPrimitiveLegal()) {
            if (isTypeLegal()) {
                ...
                construct(secondaryContext, "TestWP1");
                construct(secondaryContext, "TestWP2");
                runRoundRobin(getSubTokenString() + getPrefix() + "|DEITYWEAPON" + getJoinCharacter() + "TestWP2" + getJoinCharacter() + "!TYPE=OtherTestType" + getJoinCharacter() + "!TYPE=TestAltType.TestThirdType.TestType");
                assertTrue(primaryContext.ref.validate());
                assertTrue(secondaryContext.ref.validate());
            }
        }
