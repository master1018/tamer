...
public class SvnDirScanner extends DirectoryScanner {
    ...
    private ISVNStatus[] list(File file) {
        ...
        if (files == null) {
            try {
                files = svnClient.getStatus(file, false, true);
    ...
    protected void scandir(File dir, String vpath, boolean fast) {
        ...
        for (int i = 0; i < newfiles.length; i++) {
            ...
            if (SVNNodeKind.DIR == newfiles[i].getNodeKind() || (SVNNodeKind.UNKNOWN == newfiles[i].getNodeKind() && newfiles[i].getFile().isDirectory())) {
                if (isIncluded(name)) {
            ...
            } else if (SVNNodeKind.FILE == newfiles[i].getNodeKind() || (SVNNodeKind.UNKNOWN == newfiles[i].getNodeKind() && newfiles[i].getFile().isFile())) {
                if (isIncluded(name)) {
