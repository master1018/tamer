...
public abstract class LearnProcessManager implements LearnMethodListener {
    ...
    protected LinkedHashMap<CardFile, List<Item>> scheduler = new LinkedHashMap<CardFile, List<Item>>();
    ...
    protected List<LearnProcListener> learnProcListeners = new ArrayList<LearnProcListener>();

    protected LearnMethodFactory lmFactory;

    public LearnProcessManager(ItemValuater itemValuater, LearnMethodFactory factory) {
    ...
    private void startProcessing(CardFile cardFile) {
        ...
        if (scheduledFileItems.size() > 0) {
            ...
            if (procIt.hasNext()) startProcessing(procIt.next()); else terminateProcess(false);
        }
    }
    ...
    private void terminateProcess(boolean wasInterrupted) {
        ...
        for (LearnProcListener learnProcListener : learnProcListeners) learnProcListener.processFinished(wasInterrupted);
    }
    ...
    public void addLearnProcessManagerProcessListener(LearnProcListener l) {
        if (l == null || learnProcListeners.contains(l)) return;
