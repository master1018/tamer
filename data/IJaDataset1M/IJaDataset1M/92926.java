...
public class CheckOptionHandler extends Check {
    ...
    protected OptionHandler m_OptionHandler = new com.sodad.weka.classifiers.rules.ZeroR();
   ...
   */
    public void setOptions(String[] options) throws Exception {
        ...
        if (tmpStr.length() == 0) tmpStr = com.sodad.weka.classifiers.rules.ZeroR.class.getName();
   ...
   */
    public boolean checkCanonicalUserOptions() {
        ...
        try {
            ...
            if (getDebug()) println(printOptions(userOptions));
            if (getDebug()) println("  Setting canonical user options");
            handler.setOptions((String[]) userOptions.clone());
            if (getDebug()) println("  Checking canonical user options");
   ...
   */
    public boolean checkResettingOptions() {
        ...
        try {
            ...
            if (getDebug()) println("  Resetting to default options");
            handler.setOptions(getCopy(defaultOptions));
            if (getDebug()) println("  Checking default options match previous default");
   ...
   */
    public void doTests() {
        ...
        if (m_Success) m_Success = checkSetOptions();
        if (m_Success) m_Success = checkDefaultOptions();
        if (m_Success) m_Success = checkRemainingOptions();
        if (m_Success) m_Success = checkCanonicalUserOptions();
        if (m_Success) m_Success = checkResettingOptions();
    }
   ...
   */
    public static void main(String[] args) {
        ...
        runCheck(check, args);
        if (check.getSuccess()) System.exit(0); else System.exit(1);
    }
}
