...
public class JaxenXPathRuleQuery extends AbstractXPathRuleQuery {
    ...
    public List<Node> evaluate(Node node, RuleContext data) {
        ...
        try {
            initializeXPathExpression(data.getLanguageVersion().getLanguageVersionHandler().getXPathHandler().getNavigator());
    ...
    private void initializeXPathExpression(Navigator navigator) throws JaxenException {
        if (initializationStatus == InitializationStatus.FULL || (initializationStatus == InitializationStatus.PARTIAL && navigator == null)) {
            return;
        }
