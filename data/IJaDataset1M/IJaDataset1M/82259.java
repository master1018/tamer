...
public abstract class Appender {
    ...
    public void finalize() {
        logger.logDebug("finalize done on appender - className: " + this.getClass().getName() + " interval " + intervalMillis);
    }
        ...
        if (eventQueue != null) {
            synchronized (eventQueueLockToken) {
                if (eventQueue.size() < maxQueueSize) {
                    ...
                    logger.logWarn("Unable to log " + data + " to appender " + this + "because event queue is full.");
                }
            }
        } else {
            outputData(data);
    ...
    public String toString() {
        return "Appender(className=" + this.getClass().getName() + " intervalMillis=" + this.intervalMillis + ")";
    }

    public IntervalData newIntervalData(PerfMon owner, long timeStart) {
        return new IntervalData(owner, timeStart, medianCalculator != null ? medianCalculator.clone() : null, thresholdCalculator != null ? thresholdCalculator.clone() : null);
    }

    public static final class AppenderID extends ObjectID {
