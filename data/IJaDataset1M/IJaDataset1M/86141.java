...
import com.creawor.hz_market.resource.business_resource.BusinessResource;
import com.creawor.km.util.DateUtil;

public class BusinessResHandle {

    /**
	 * Logger for this class
	 */
    private static final Logger logger = Logger.getLogger(AdResHandle.class);
    ...
    public ArrayList readExcel(InputStream is) {
        ...
        try {
            ...
            for (int i = 2; i < rownum; i++) {
                BusinessResource vo = new BusinessResource();
                ...
                if (cell[0] != null && cell[0].getContents().trim().length() != 0) {
                    vo.setCompany(cell[0].getContents().trim());
                } else {
                    continue;
                }
                if (cell[1] != null && cell[1].getContents().length() != 0) {
                    vo.setProductType(cell[1].getContents());
                } else {
                    vo.setProductType(" ");
                }
                if (cell[2] != null && cell[2].getContents().length() != 0) {
                    vo.setProduceState(cell[2].getContents());
                } else {
                    vo.setProduceState("");
                }
                if (cell[3] != null && cell[3] != null && cell[3].getContents().length() != 0) {
    ...
    public String insertData(ArrayList al) {
        ...
        try {
            ...
            for (int i = 0; i < al.size(); i++) {
                ...
                BusinessResource vo = (BusinessResource) al.get(i);
                try {
                    String company = vo.getCompany();
                    String type = vo.getProductType();
                    Date day = vo.getInsertDay();
                    String dateStr = DateUtil.getStr(day, null);
                    dateStr = dateStr.substring(0, 7);
                    String sqlStr = "select * from business_resource where product_type='" + type + "' and left(convert(varchar(10),insert_day,120),7)='" + dateStr + "' and company='" + company + "'";
                    ...
                    returnStr += vo.getInsertDay() + vo.getCompany() + vo.getProductType() + "�Ѿ�����<br>";
                    continue;
                }
                tx.commit();
            }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            ArrayList al = ex.readExcel("E:\\business_res.xls");
            for (int i = 0; i < al.size(); i++) {
                BusinessResource vo = (BusinessResource) al.get(i);
                System.out.println("VO:::" + vo.getCompany());
                System.out.println("PublishdateVO:::" + vo.getProductType());
            }
            System.out.println("starting....");
            ex.insertData(al);
        } catch (Exception e) {
