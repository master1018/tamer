...
public final class U {
    ...
    public static int arrayIndexOf(byte[] source, byte[] what, int start) {
        loop: for (int i = start; i < source.length - what.length; i++) {
	 ...
	 */
    public static <T, C extends Collection<T>, W extends Collection<? extends T>, V extends Collection<? extends T>> C concat(W first, V second, Class<C> clazz) {
        try {
	 ...
	 */
    public static <T, V extends Collection<T>> V addAll(V dest, Iterable<? extends T> src) {
        for (T t : src) {
            dest.add(t);
        }
        return dest;
	 ...
	 * @return the created and filled-in HashMap
	 */
    public static <K, V> LinkedHashMap<K, V> newLinkedHashMap(Iterable<? extends Map.Entry<? extends K, ? extends V>> src) {
