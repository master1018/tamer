 ...
 */
public class MockOperationSetBasicTelephony implements OperationSetBasicTelephony, CallChangeListener {
     ...
     */
    public void answerCallParticipant(CallParticipant participant) throws OperationFailedException {
        MockCallParticipant callParticipant = (MockCallParticipant) participant;
     ...
     *   encounter an error while performing this operation.
     */
    public void hangupCallParticipant(CallParticipant participant) throws OperationFailedException {
     ...
     *   method
     */
    public void putOnHold(CallParticipant participant) throws OperationFailedException {
    }
    ...
    public Call placeCall(String toAddress) throws Exception {
        ...
        MockCallParticipant callPArt = (MockCallParticipant) newCall.getCallParticipants().next();
    ...
    protected void fireCallEvent(int eventID, Call sourceCall) {
        ...
        logger.debug("Dispatching a CallEvent to " + callListeners.size() + " listeners. event is: " + cEvent.toString());
        Iterator listeners = new ArrayList(callListeners).iterator();
        while (listeners.hasNext()) {
            CallListener listener = (CallListener) listeners.next();
            if (eventID == CallEvent.CALL_INITIATED) listener.outgoingCallCreated(cEvent); else if (eventID == CallEvent.CALL_RECEIVED) listener.incomingCallReceived(cEvent); else if (eventID == CallEvent.CALL_ENDED) listener.callEnded(cEvent);
        }
    }
